...
#ifndef __MDI_UTILS_H__
#define __MDI_UTILS_H__
...
#define REDIRECTOR_CONTROL_CLOSE_COMMAND "/system/bin/mdi_redirector_ctrl --leave-testing-mode"

#define INVALID_DMC_SESSION_ID 0
...
enum {
    CMD_PACKET_TYPE_HEADER_ONLY = 0,
    CMD_PACKET_TYPE_WITH_PAYLOAD = 1
} CMD_PACKET_TYPE;
...
#define MDI_LOG(suffix, tag, ...) do { \
LOG_##suffix(tag, __VA_ARGS__); \
} while (0)

#define MDI_LOGE(tag, ...) MDI_LOG(E, tag, __VA_ARGS__)
#define MDI_LOGW(tag, ...) MDI_LOG(W, tag, __VA_ARGS__)
#define MDI_LOGI(tag, ...) MDI_LOG(I, tag, __VA_ARGS__)
#define MDI_LOGD(tag, ...) MDI_LOG(D, tag, __VA_ARGS__)
#define MDI_LOGV(tag, ...) MDI_LOG(V, tag, __VA_ARGS__)
