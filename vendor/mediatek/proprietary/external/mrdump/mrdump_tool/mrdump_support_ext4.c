...
static int format_a_data_block(int myfds, int mybs, uint32_t lba)
{
    ...
    delaylength += mybs;
    if (delaylength >= MRDUMP_MAX_BANDWIDTH) {
        delaylength = 0;
        fdatasync(myfds);
        int res = posix_fadvise(myfds, 0, 0, POSIX_FADV_DONTNEED);
        if (res != 0)
            MD_LOGI("%s: posix_fadvise failed(res:%d)\n", __func__, res);
        usleep(100);
    }
...
static bool fs_lba_maker(const struct palloc_file *pfile, bool need_to_reinit)
{
    ...
    fdatasync(pfile->fd);
    int res = posix_fadvise(pfile->fd, 0, 0, POSIX_FADV_DONTNEED);
    if (res != 0)
        MD_LOGI("%s: posix_fadvise failed(res:%d)\n", __func__, res);
    sync();
