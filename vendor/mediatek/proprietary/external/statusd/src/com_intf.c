...
{
	unsigned char frame[STATUSD_MAX_DATA_LEN];
	int client_sockfd = 0;
...
void* child_poll_data(void *vargp)
{
	...
	statusd_data_callback icallback;
	struct timeval timeout;
	...
	while(1){
		...
		if (ret >0) {
			...
			if ((cli_info->cli_sock_fd > 0) && FD_ISSET(cli_info->cli_sock_fd,&fdr)){
				LOGINTF(LOG_NOTICE, "client%d fd is ok", cli_info->module_id);
...
int statusd_deregister_cominf(char module_id)
{
	...
	LOGINTF(LOG_DEBUG, "Enter");
	pthread_mutex_lock(&statusd_fd_lock);
	LOGINTF(LOG_NOTICE, "client%d fd is bad", module_id);
