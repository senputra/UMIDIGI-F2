...
static uint32_t g_uart_switch = 1;
...
void set_uart_switch(unsigned int enable)
{
    g_uart_switch = enable;
}
...
#ifdef CFG_KERNEL_TIMESTAMP_SUPPORT
    extern unsigned long long timer_get_ap_timestamp(void);
    extern unsigned long long timer_get_ap_suspend(void);
    unsigned long long ns = timer_get_ap_timestamp();
    unsigned char suspend = timer_get_ap_suspend();
