...
#define SCP_SLP_PROT_EN (P_APB_SLP_PROT_EN | D_APB_SLP_PROT_EN)

static void __hal_cache_invalidate_one_cache_line(uint32_t cache_type, uint32_t address)
{
    /* Invalidate CACHE line by address */
    CACHE_SEL(cache_type)->CACHE_OP =
        (address & CACHE_OP_TADDR_MASK) |
        ((CACHE_INVALIDATE_ONE_LINE_BY_ADDRESS << CACHE_OP_OP_OFFSET) | CACHE_OP_EN_MASK);
}
...
hal_cache_status_t hal_cache_init(uint32_t cache_type)
{
    ...
    if (CACHE_SEL(cache_type)->CACHE_CON & CACHE_CON_MCEN_MASK) {
        ...
        CACHE_SEL(cache_type)->CACHE_OP |=
            ((CACHE_FLUSH_ALL_LINES << CACHE_OP_OP_OFFSET) | CACHE_OP_EN_MASK);
        CACHE_SEL(cache_type)->CACHE_OP &= ~CACHE_OP_OP_MASK;
        CACHE_SEL(cache_type)->CACHE_OP |=
            ((CACHE_INVALIDATE_ALL_LINES << CACHE_OP_OP_OFFSET) | CACHE_OP_EN_MASK);
    }
...
{
    /* Enable CACHE */
    CACHE_SEL(cache_type)->CACHE_CON |=
        CACHE_CON_MCEN_MASK | CACHE_CON_CNTEN0_MASK | CACHE_CON_CNTEN1_MASK;
...
hal_cache_status_t hal_cache_set_size(uint32_t cache_type, hal_cache_size_t cache_size)
{
    ...
    PRINTF_E("CACHE_CON, addr:%p\n", &CACHE_SEL(cache_type)->CACHE_CON);
    PRINTF_E("CACHE_CON, value:0x%x\n", (unsigned int)CACHE_SEL(cache_type)->CACHE_CON);
...
hal_cache_status_t hal_cache_flush_one_cache_line(uint32_t cache_type, uint32_t address)
{
    ...
    CACHE_SEL(cache_type)->CACHE_OP =
        (address & CACHE_OP_TADDR_MASK) |
        ((CACHE_FLUSH_ONE_LINE_BY_ADDRESS << CACHE_OP_OP_OFFSET) | CACHE_OP_EN_MASK);
...
{
#if defined(MT6779) || defined(MT6785)
    DRV_WriteReg32(BUS_QOS, DRV_Reg32(BUS_QOS) | NO_INS_DRAM_ACC_EN);
#else
    DRV_WriteReg32(SCP_SLP_PROTECT_CFG,
                   DRV_Reg32(SCP_SLP_PROTECT_CFG) | SCP_SLP_PROT_EN);
#endif

#ifdef CFG_AXI_SLP_PROT_ON
    /* enable axi slp prot */
    DRV_WriteReg32(SCP_SLP_PROTECT_CFG,
                   DRV_Reg32(SCP_SLP_PROTECT_CFG) | P_AXI_SLP_PROT_EN);
    /* wait for axi slp prot being ready */
    while ((DRV_Reg32(SCP_SLP_PROTECT_CFG) & P_AXI_SLP_PROT_EN) == 0);
#endif
}
...
{
    return (((uint64_t)hal_cache_hit_countu(cache_type) << 32) +
            (uint64_t)hal_cache_hit_countl(cache_type));
}

uint64_t cache_access_count(uint32_t cache_type)
{
    return (((uint64_t)hal_cache_access_countu(cache_type) << 32) +
            (uint64_t)hal_cache_access_countl(cache_type));
}

void bus_qos(uint32_t set)
{
    if (set)
        DRV_WriteReg32(BUS_QOS,
                       DRV_Reg32(BUS_QOS) | ((2 << AXI_ARULTRA) | (2 << AXI_AWULTRA)));
    else
        DRV_WriteReg32(BUS_QOS,
                       DRV_Reg32(BUS_QOS) & ~((2 << AXI_ARULTRA) | (2 << AXI_AWULTRA)));
}
