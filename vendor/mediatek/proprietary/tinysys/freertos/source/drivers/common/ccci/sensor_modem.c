        ...
        case ACTIVATE:
            if ((buffer_in == NULL) || (size_in < sizeof(int))) {
                ...
                if (0 == value) { //disable modem
                    mdm_command[0] = sizeof(UINT32);
                    mdm_command[1] = MTK_MDM_DISABLE;
                    ccci_geo_fence_send(md_id, mdm_command, sizeof(mdm_command));
                } else { //enable modem
                    mdm_command[0] = sizeof(UINT32);
...
static int sensor_modem_1_run_algorithm(struct data_t *output)
{
    ...
    struct cell_info *cells = (struct cell_info *)(md_list[MD_SYS1].ccism_rx_msg + sizeof(struct ccci_header));
    int i;

    CCCI_INF_MSG(MD_SYS1, "SNR", "run algorithm %d (%d %d %d) %d\n",
                 md_list[MD_SYS1].state, ccci_h->peer_channel, ccci_h->seq_num, ccci_h->data[1], cells->length);
    if (ccci_h->peer_channel != CCCI_CELLINFO_CHANNEL_RX || md_list[MD_SYS1].state != MD_BOOT_STAGE_2)
        return 0;
    if (cells->length > sizeof(modem_vec_t) * MAX_CELL_NUMBER || cells->length < sizeof(modem_vec_t) ||
            cells->length % sizeof(modem_vec_t) != 0)
        return 0;
    output->data_exist_count = cells->length / sizeof(modem_vec_t);
    for (i = 0; i < output->data_exist_count; i++) {
        output->data[i].time_stamp = read_xgpt_stamp_ns();
        output->data[i].sensor_type = SENSOR_TYPE_MODEM_1;
        memcpy(&output->data[i].modem_t, &cells->data[i], sizeof(modem_vec_t));
    }
    return 0;
}
...
static int sensor_modem_3_run_algorithm(struct data_t *output)
{
    ...
    struct cell_info *cells = (struct cell_info *)(md_list[MD_SYS3].ccism_rx_msg + sizeof(struct ccci_header));
    int i;

    CCCI_INF_MSG(MD_SYS3, "SNR", "run algorithm %d (%d %d %d) %d\n",
                 md_list[MD_SYS3].state, ccci_h->peer_channel, ccci_h->seq_num, ccci_h->data[1], cells->length);
    if (ccci_h->peer_channel != CCCI_CELLINFO_CHANNEL_RX || md_list[MD_SYS3].state != MD_BOOT_STAGE_2)
        return 0;
    if (cells->length > sizeof(modem_vec_t) * MAX_CELL_NUMBER || cells->length < sizeof(modem_vec_t) ||
            cells->length % sizeof(modem_vec_t) != 0)
        return 0;
    ...
    for (i = 0; i < output->data_exist_count; i++) {
        output->data[i].time_stamp = read_xgpt_stamp_ns();
        output->data[i].sensor_type = SENSOR_TYPE_MODEM_3;
        memcpy(&output->data[i].modem_t, &cells->data[i], sizeof(modem_vec_t));
    }
    return 0;
}
...
static int sensor_modem_init(struct ccci_modem *md)
{
    ...
    ret = sensor_subsys_algorithm_register_data_buffer(sensor->sensor_type, MAX_CELL_NUMBER);
