...
AudioTask *task_voip_new(void)
{
    ...
    /* task attribute*/
    task_handle->max_queue_size = VOIP_MAX_MSG_QUEUE_SIZE;
    ...
    task_handle->task_scene = TASK_SCENE_VOIP;

    task_handle->playback_hwbuf_handle = -1;
    task_handle->capture_hwbuf_handle = -1;
    task_handle->AurisysScenario = AURISYS_SCENARIO_DSP_VOIP;

    task_handle->reserved_mem_id = VOIP_DMA_ID;

    task_handle->task_profile_id = TASK_VOIP_TIME_ID1;
...
void task_voip_delete(AudioTask *task)
{
