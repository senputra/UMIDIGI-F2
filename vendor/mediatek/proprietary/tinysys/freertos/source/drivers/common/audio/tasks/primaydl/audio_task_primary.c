...
#define PRIMARY_MAX_MSG_QUEUE_SIZE (24)
#define LOCAL_TASK_STACK_SIZE (2048)
#define LOCAL_TASK_NAME "primary_playback"
#define LOCAL_TASK_PRIORITY (2)
#define LOCAL_TASK_OUPUT_BUFFER_SIZE (4096)


AudioTask *task_primary_new(void)
{
    ...
    /* task attribute*/
    task_handle->max_queue_size = PRIMARY_MAX_MSG_QUEUE_SIZE;
    ...
    task_handle->task_scene = TASK_SCENE_PRIMARY;

    task_handle->playback_hwbuf_handle = -1;
    task_handle->capture_hwbuf_handle = -1;

    task_handle->reserved_mem_id = PRIMARY_DMA_ID;

    task_handle->task_profile_id = TASK_PRIMARY_TIME_ID1;
...
#else
    task_handle->AurisysScenario = AURISYS_SCENARIO_DSP_PRIMARY;
#endif
...
void task_primary_delete(AudioTask *task)
{
