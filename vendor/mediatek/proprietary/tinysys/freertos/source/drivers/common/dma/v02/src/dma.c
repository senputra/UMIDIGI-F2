...
/*
 * scp_dual_address_append: append scp_dual address for DMA_dual.
 * @addr: verify and append scp dual address
 * Return appended address
 */
uint32_t scp_dual_address_append(uint32_t addr)
{
    if (SCP_CORE == SCP_B_ID){
#define CM4_ADDRESS_TCM_START    (0x00000000)
#define CM4_ADDRESS_TCM_END      (0x000BFFFF)
            //PRINTF_E("SCP dual addr %x\n",addr);
        if (addr >= CM4_ADDRESS_TCM_START && addr <= CM4_ADDRESS_TCM_END) {
            addr = addr + drv_reg32(SCP_B_ADDR_OFFSET);
            if (addr > CM4_ADDRESS_TCM_END) {
                /*address out of boundary*/
                PRINTF_E("SCP dual addr out of boundary%x\n",addr);
                configASSERT(0);
            }
        }
    }
...
int32_t mt_config_gdma(int32_t channel, struct mt_gdma_conf *config, DMA_CONF_FLAG flag)
{
    ...
    if (!config) {
        dbgmsg("GDMA_%d config null\n", channel);
        return -DMA_ERR_INV_CONFIG;
    }

    if (config->count > MAX_TRANSFER_LEN) {
        dbgmsg("GDMA_%d count:0x%x over 0x%x\n", channel, config->count, MAX_TRANSFER_LEN);
        return -DMA_ERR_INV_CONFIG;
    }

    if (config->count <= 0) {
        dbgmsg("GDMA_%d count:0x%x, not valid value\n", channel, config->count);
        return -DMA_ERR_INV_CONFIG;
    }

    if (config->limiter) {
        dbgmsg("GDMA_%d counter over 0x%x\n", channel, MAX_SLOW_DOWN_CNTER);
        return -DMA_ERR_INV_CONFIG;
    }
    ...
    switch (flag) {
        case ALL:
            ...
            if (config->wplen) {
                dbgmsg("wplen\n");
                drv_write_reg32(DMA_WPPT(dma_channel), config->wplen);
            }
            if (config->wpto) {
                dbgmsg("wpto\n");
                drv_write_reg32(DMA_WPTO(dma_channel), config->wpto);
            }
            ...
            dbgmsg("GDMA_%d src:0x%x, dst:0x%x, count:0x%x, con:0x%x\n\r", channel, config->src, config->dst, config->count, dma_con);
            break;
