...
{
    for (int32_t channel = 0; channel < NR_GDMA_CHANNEL; channel++) {
        uint32_t base = DMA_BASE_CH(channel);
        drv_write_reg32(DMA_START(base), DMA_START_CLR_BIT);
        drv_write_reg32(DMA_ACKINT(base), DMA_ACK_BIT);
    }
}
...
int32_t mt_config_gdma(int32_t channel, struct mt_gdma_conf *config, DMA_CONF_FLAG flag)
{
    ...
    switch (flag) {
        case ALL:
            ...
            //      channel, config->src, config->dst, config->count, dma_con);
            break;
...
void mt_dump_dma_struct()
{
    ...
    for (i = 0; i < NUMS_DMA_ID; i ++) {
        if (dma_id_ctrl[i].in_use != DMA_ID_NO_USE || dma_id_ctrl[i].dma_usage_count > 0) {
            PRINTF_E("dma_ctrl id:%ld,use:%ld,ch:%ld,usage:%lu\n", i,
                     dma_id_ctrl[i].in_use, dma_id_ctrl[i].dma_channel_num,
                     dma_id_ctrl[i].dma_usage_count);
        }
    }
    for (i = 0; i < NR_GDMA_CHANNEL; i ++) {
        if (dma_channel[i].in_use != 0 || dma_channel[i].ch_usage_count > 0) {
            PRINTF_E("dma ch:%ld,use:%ld,count:%lu\n", i, dma_channel[i].in_use,
                     dma_channel[i].ch_usage_count);
        }
    }
    return;
...
uint32_t mt_dma_list_remove(void)
{
    ...
    //           DmaListItem_value , scp_dma_list_current);
...
void mt_dump_dma_list(void)
{
    ...
    for (i = 0; i < DMA_QUEUE_SIZE; i++) {
        PRINTF_E("[SCP]DMAlistctrl[%d]id=%u,dst=%lu,src=%lu,len=%lu,in_list=%lu\n\r",
                 i, dma_list_ctrl[i].dma_id_list,
                 dma_list_ctrl[i].dst_addr_list,
                 dma_list_ctrl[i].src_addr_list, dma_list_ctrl[i].len_list,
                 dma_list_ctrl[i].in_list);
    }
