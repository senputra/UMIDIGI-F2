...
#define DUMP_IPI_MSG(description, p_ipi_msg) \
    do { \
        if ((void *)description == NULL || (void *)(p_ipi_msg) == NULL) \
            break; \
        ...
        if ((p_ipi_msg)->data_type == AUDIO_IPI_MSG_ONLY) { \
            ...
            AUD_LOG_D("%s, task: %d, msg_id: 0x%x, ack_type: %d, " \
                     ...
                     (unsigned int)(p_ipi_msg)->msg_id, \
                      ...
                      (unsigned int)(p_ipi_msg)->param1, \
                      (unsigned int)(p_ipi_msg)->param2, \
                      (unsigned int)(p_ipi_msg)->dma_info.data_size, \
                      (unsigned int)(p_ipi_msg)->dma_info.rw_idx, \
                      (unsigned int)(p_ipi_msg)->dma_info.hal_buf.data_size); \
        } \
    } while (0)
