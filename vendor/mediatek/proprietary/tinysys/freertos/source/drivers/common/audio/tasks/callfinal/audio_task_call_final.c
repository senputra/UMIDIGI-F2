...
AudioTask *task_call_final_new(void)
{
    ...
    if (ret) {
        AUD_LOG_E("Call final init_common_task fail!!\n");
        return NULL;
    }

    task = (AudioTask *)AUDIO_MALLOC(sizeof(AudioTask));
    if (task == NULL) {
        AUD_LOG_E("Call final AUDIO_MALLOC fail!!\n");
        return NULL;
    }
    ...
    task_handle->task_scene = TASK_SCENE_CALL_FINAL;

    task_handle->playback_hwbuf_handle = -1;
    task_handle->capture_hwbuf_handle = -1;
    task_handle->ref_hwbuf_handle = -1;

    task_handle->reserved_mem_id = CALL_FINAL_DMA_ID;

    task_handle->task_profile_id = TASK_CALL_FINAL_TIME_ID1;
...
#else
    task_handle->AurisysScenario = AURISYS_SCENARIO_DSP_CALL_FINAL;
#endif

    task_handle->debug_flag = TASK_DEBUG_INFO;

    AUD_LOG_D("Call final new\n");
...
void task_call_final_delete(AudioTask *task)
{
    int ret = 0;
    ret = deinit_common_task(&task_handle);
    if (ret) {
        AUD_LOG_E("Call final deinit_common_task ret[%d]!!\n", ret);
    }
    if (task == NULL) {
        AUD_LOG_E("Call Final task is NULL!!\n");
        return;
    }
    AUDIO_FREE(task);
