...
uint64_t max_eint_isr_duration_time = 0;
unsigned int max_eint_duration_index = 0;

unsigned int EINT_Mask_Store[EINT_REG_SET_SIZE];
unsigned int EINT_Domain0_Store[EINT_REG_SET_SIZE];
unsigned int EINT_Domain1_Store[EINT_REG_SET_SIZE];
...
void mt_eint_mask_all(void)
{
    ...
    for(i = 0; i < EINT_REG_SET_SIZE; i++) {
        ...
        //error_msg("[EINT]mask reg=%x\n\r", READ_REG(EINT_MASK_BASE + (i * 4)));
    }
...
unsigned int mt_can_en_debounce(unsigned int eint_num)
{
    ...
    if(eint_num < EINT_MAX_CHANNEL && sens != EDGE_SENSITIVE) {
        return 1;
    } else {
        error_msg("[EINT]err debce num:%d sens:%d(0:edge)\n", eint_num, sens);
        return 0;
    }
}
...
void mt_eint_set_hw_debounce(unsigned int eint_num, unsigned int us)
{
    ...
    if(us >= MAX_HW_DEBOUNCE_TIME) {
        error_msg ("[EINT]err debce time %d num:%d,max=%d\n\r", us, eint_num, MAX_HW_DEBOUNCE_TIME);
        return;
    }
...
/*
 * mt_set_domain1_all: Enable all EINT on domain1(SCP B).
 */
void mt_eint_domain1_set_all(void)
{
    unsigned int val = 0xFFFFFFFF, i;

    for(i = 0; i < EINT_REG_SET_SIZE; i++) {
        WRITE_REG(val, (EINT_D1_EN_BASE + (i * 4)));
    }
...
/*
 * mt_set_domain1_all: Disable all EINT on domain1(SCP B).
 */
void mt_eint_domain1_clr_all(void)
{
    unsigned int val = 0x0, i;
...
/*
 * mt_eint_domain1_set: Enable specific EINT on domain1(SCP B).
 * @eint_num: the EINT number to set
 */
void mt_eint_domain1_set(unsigned int eint_num)
{
    unsigned int st;

    if(eint_num < EINT_MAX_CHANNEL) {
        st = READ_REG(((eint_num / 32) * 4 + EINT_D1_EN_BASE));
        st |= (1 << (eint_num % 32));
        WRITE_REG(st, ((eint_num / 32) * 4 + EINT_D1_EN_BASE));
    } else {
        error_msg("[EINT]num err\n\r");
    }
...
/*
 * mt_eint_domain1_clr: Disable specific EINT on domain1(SCP B).
 * @eint_num: the EINT number to clear
 */
void mt_eint_domain1_clr(unsigned int eint_num)
{
    unsigned int st;
...
/*
 * mt_eint_domain1_save_all: Save and disable all EINT on domain1(SCP B).
 * @eint_num: the EINT number to set
 */
void mt_eint_domain1_save_all(void)
{
    unsigned int val = 0x0, i;

    for(i = 0; i < EINT_REG_SET_SIZE; i++) {
        EINT_Domain1_Store[i] = READ_REG((EINT_D1_EN_BASE + (i * 4)));
        WRITE_REG(val, (EINT_D1_EN_BASE + (i * 4)));
    }
...
/*
 * mt_eint_domain1_restore_all: Restore all EINT on domain1(SCP B).
 * @eint_num: the EINT number to clear
 */
void mt_eint_domain1_restore_all(void)
{
    unsigned int i;

    for(i = 0; i < EINT_REG_SET_SIZE; i++) {
        WRITE_REG(EINT_Domain1_Store[i], (EINT_D1_EN_BASE + (i * 4)));
    }
...
/*
 * mt_eint_domain1_save: Save and diable specific EINT on domain1(SCP B).
 * @eint_num: the EINT number to set
 */
unsigned int mt_eint_domain1_save(unsigned int eint_num)
{
    unsigned int st;
    unsigned int bit = 1 << (eint_num % 32);
    ...
    if(eint_num < EINT_MAX_CHANNEL) {
        st = READ_REG(((eint_num / 32) * 4 + EINT_D1_EN_BASE));
        WRITE_REG((st & (~bit)), ((eint_num / 32) * 4 + EINT_D1_EN_BASE));
        return (st & bit);
    } else {
        error_msg("[EINT]num err\n\r");
        return 0;
...
/*
 * mt_eint_domain1_restore: Restore specific EINT on domain1(SCP B).
 * @eint_num: the EINT number to clear
 */
void mt_eint_domain1_restore(unsigned int eint_num, unsigned int val)
{
    unsigned int st;
    unsigned int bit = 1 << (eint_num % 32);
    ...
    for(index = 0; index < EINT_MAX_CHANNEL; index++) {
        ...
        if(status_check) {
            ...
            if(EINT_FUNC.eint_func[index]) {
#ifdef CFG_XGPT_SUPPORT
                /* Get start time */
                xFirstExecutionStamp = read_xgpt_stamp_ns();
#else
...
#ifdef CFG_XGPT_SUPPORT
                /* Get finish time */
                xLastExecutionStamp = read_xgpt_stamp_ns();
#else
                ...
                /*record max duration*/
                if (xLastExecutionStamp > max_eint_isr_duration_time){
                    max_eint_isr_duration_time = xLastExecutionStamp;
                    max_eint_duration_index = index;
                }
...
void mt_eint_registration(unsigned int eint_num, unsigned int sens, unsigned int pol, void (EINT_FUNC_PTR)(int), unsigned int unmask, unsigned int is_auto_umask)
{
    if (eint_num < EINT_MAX_CHANNEL) {
        ...
        /* assign to domain for SCP, Domain 0 for SCP A, Domain 1 for SCP B */
        if (SCP_CORE == SCP_B_ID) {
            mt_eint_domain1_set(eint_num);
        } else {
...
/*
 * eint_unregistration: unregister a EINT.
 * @eint_num: the EINT number to unregister
 */
void mt_eint_unregistration(unsigned int eint_num)
{
    if (eint_num < EINT_MAX_CHANNEL) {
        ...
        EINT_FUNC.eint_auto_umask[eint_num] = 0;
        EINT_FUNC.is_deb_en[eint_num] = 0;
        EINT_FUNC.deb_time[eint_num] = 0;
        /* assign to domain for SCP, Domain 0 for SCP A, Domain 1 for SCP B */
        if (SCP_CORE == SCP_B_ID) {
            mt_eint_domain1_clr(eint_num);
        } else {
...
unsigned int mt_eint_get_def_pol (unsigned int eint_num)
{
    ...
    /* Save config, Domain 0 for SCP A, Domain 1 for SCP B */
    if (SCP_CORE == SCP_B_ID) {
        domain = mt_eint_domain1_save(eint_num);
    } else {
    ...
    if (SCP_CORE == SCP_B_ID) {
        mt_eint_domain1_restore(eint_num, domain);
    } else {
...
int mt_eint_init(void)
{
    ...
    /* mask all eint until owner unmask it */
    mt_eint_mask_all();
...
void mt_eint_dump_config(unsigned int eint_num)
{
    ...
    if(eint_num < EINT_MAX_CHANNEL) {
        ...
        debug_msg("[EINT]number=%d,mask=%d,sens=%d,pol=%d,deb_en=%d,deb_time=%d,deb_reg_value=%d,eint_func=%p,int_status=%d\n\r"
        ,eint_num,eint_mask,eint_sens,eint_pol,is_deb_en,deb_time,deb_reg_value,eint_func,int_status);
