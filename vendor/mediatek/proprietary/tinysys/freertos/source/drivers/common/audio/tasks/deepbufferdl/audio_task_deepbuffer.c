...
#define DEEPBUFFER_MAX_MSG_QUEUE_SIZE (24)
#define LOCAL_TASK_STACK_SIZE (2048)
#define LOCAL_TASK_NAME "deep_playback"
#define LOCAL_TASK_PRIORITY (2)
#define LOCAL_TASK_OUPUT_BUFFER_SIZE (1024*4)

AudioTask *task_deepbuffer_new(void)
{
    ...
    /* task attribute*/
    task_handle->max_queue_size = DEEPBUFFER_MAX_MSG_QUEUE_SIZE;
    ...
    task_handle->task_scene = TASK_SCENE_DEEPBUFFER;

    task_handle->playback_hwbuf_handle = -1;
    task_handle->capture_hwbuf_handle = -1;

    task_handle->reserved_mem_id = DEEPBUFFER_DMA_ID;

    task_handle->task_profile_id = TASK_DEEPBUFFER_TIME_ID1;
    task_handle->task_profile_interval = 70000000;
...
#else
    task_handle->AurisysScenario = AURISYS_SCENARIO_DSP_DEEP_BUF;
#endif
...
void task_deepbufferdelete(AudioTask *task)
{
