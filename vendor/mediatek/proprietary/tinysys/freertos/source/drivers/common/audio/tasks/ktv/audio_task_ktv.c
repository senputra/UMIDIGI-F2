...
AudioTask *task_ktv_new(void)
{
    ...
    if (ret) {
        AUD_LOG_E("KTV init_common_task fail!!\n");
        return NULL;
    }

    task = (AudioTask *)AUDIO_MALLOC(sizeof(AudioTask));
    if (task == NULL) {
        AUD_LOG_E("KTV AUDIO_MALLOC fail!!\n");
        return NULL;
    }
    ...
    task_handle->task_scene = TASK_SCENE_KTV;

    task_handle->playback_hwbuf_handle = -1;
    task_handle->capture_hwbuf_handle = -1;
    task_handle->ref_hwbuf_handle = -1;

    task_handle->reserved_mem_id = KTV_DMA_ID;

    task_handle->task_profile_id = TASK_KTV_TIME_ID1;
...
#else
    task_handle->AurisysScenario = AURISYS_SCENARIO_DSP_KTV;
#endif
...
void task_ktv_delete(AudioTask *task)
{
    AUD_LOG_V("KTV delete, task_scene = %d\n", task->scene);

    int ret = 0;
    ret = deinit_common_task(&task_handle);
    if (ret) {
        AUD_LOG_E("KTV deinit_common_task ret[%d]!!\n", ret);
    }
    if (task == NULL) {
        AUD_LOG_E("KTV task is NULL!!\n");
        return;
    }
    AUDIO_FREE(task);
