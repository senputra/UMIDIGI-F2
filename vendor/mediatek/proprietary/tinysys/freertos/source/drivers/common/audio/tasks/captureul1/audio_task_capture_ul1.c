...
#define CAPTUREUL1_MAX_MSG_QUEUE_SIZE (16)
#define LOCAL_TASK_STACK_SIZE (2560)
#define LOCAL_TASK_NAME "capture_ul1"
#define LOCAL_TASK_PRIORITY (2)
#define LOCAL_TASK_OUPUT_BUFFER_SIZE (7680)

AudioTask *task_capture_ul1_new(void)
{
    ...
    /* task attribute*/
    task_handle->max_queue_size = CAPTUREUL1_MAX_MSG_QUEUE_SIZE;
    ...
    task_handle->task_scene = TASK_SCENE_CAPTURE_UL1;
    ...
    task_handle->AurisysScenario = AURISYS_SCENARIO_DSP_RECORD;
#endif

    task_handle->reserved_mem_id = CAPTURE_UL1_DMA_ID;

    task_handle->task_profile_id = TASK_CAPTURE_UL1_TIME_ID1;
    task_handle->task_profile_interval = 60000000;

    task_handle->mtask_processing_buf_out_size = LOCAL_TASK_OUPUT_BUFFER_SIZE;
    task_handle->mtask_processing_buf_in_size = LOCAL_TASK_OUPUT_BUFFER_SIZE;
    task_handle->mtask_default_buffer_size = LOCAL_TASK_OUPUT_BUFFER_SIZE;

    task_handle->mtask_ref_buf = AUDIO_MALLOC(LOCAL_TASK_OUPUT_BUFFER_SIZE);
    task_handle->mtask_ref_buf_size = LOCAL_TASK_OUPUT_BUFFER_SIZE;
...
void task_capture_ul1_delete(AudioTask *task)
{
    AUD_LOG_V("%s(+), task_scene = %d\n", __func__, task->scene);

    int ret = 0;

    if (task_handle->mtask_ref_buf != NULL)
       AUDIO_FREE(task_handle->mtask_ref_buf);
