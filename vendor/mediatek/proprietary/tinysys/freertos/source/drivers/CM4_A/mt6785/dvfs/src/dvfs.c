...
void request_clk_sys_opp(uint32_t req_opp)
{
    ...
    switch (req_opp) {
        ...
        case CLK_OPP2: /* 165 MHz, U2/2 */
            pll_opp_switch(6);
            break;
        case CLK_OPP3: /* 218 MHz, MAINPLL */
            pll_opp_switch(5);
            break;
        case CLK_OPP4: /* 330 MHz, U2 */
            break;
        case CLK_OPP5: /* 416 MHz, UNIVPLL */
            pll_opp_switch(4);
            break;
        default:
...
void request_dfs_opp(uint32_t req_opp)
{
    ...
    switch ((clk_opp_enum)req_opp) {
        ...
        case CLK_OPP5: /* 416M (univpll) */
            /* turn on clk-sys */
            if (clk_sys_is_on_by_dvfs == 0) {
                clk_sys_is_on_by_dvfs = 1;
                turn_on_clk_sys_from_isr(NEED_WAIT);
#ifdef CFG_TESTSUITE_SUPPORT
                request_clk_sys_opp(CLK_OPP5);
#endif
            }
...
uint32_t is_ulposc_calibrated(ulposc_id_enum ulposc_id, freq_enum target_clk)
{
    ...
    if (current_val > (target_val * 930 / 1000) && current_val < (target_val * 1070 / 1000)) {
        mlprintf(LOG_DVFS_NOTE, "ULPOSC%d is calibrated (%ld MHz)\n", ulposc_id + 1, (current_val * 26) / 1024);
        return 1;
    } else {
        mlprintf(LOG_DVFS_NOTE, "ULPOSC%d is not calibrated (%ld MHz)\n", ulposc_id + 1, (current_val * 26) / 1024);
        return 0;
    }
}
...
void dvfs_init(void)
{
    ...
    DRV_SetReg32(CLK_CTRL_SLP_CTRL, (0x1 << DIS_RAM_CK_CS_ISO) || (0x1 << SLOW_CLK_WAKE_INTERLEAVE_DIS_BIT));
#else
    mlprintf(LOG_DVFS_NOTE, "use legacy-wakeup\n");

    /* CLK_CTRL_SLP_CTRL 0xC40A0 = 0x1 */
    DRV_WriteReg32(CLK_CTRL_SLP_CTRL, (0x1 << DIS_RAM_CK_CS_ISO) || (0x1 << SLOW_CLK_WAKE_DIS_BIT));
#endif
