...
void spk_irq_update(int irq_num)
{
...
{
    if ((irq_id > Soc_Aud_IRQ_MCU_MODE_NUM) || (irq_id < Soc_Aud_IRQ_MCU_MODE_IRQ0_MCU_MODE)) {
...
void vow_set_bargein_int(bool enable, unsigned int irq_id)
{
    ...
    if (enable) {
        ...
        rdata = DRV_Reg32(AFE_IRQ_MCU_CON1); // set IRQ7 sampling rate
        rdata &= ~(0xF0000000);
        rdata |= 0x40000000;
        DRV_WriteReg32(AFE_IRQ_MCU_CON1, rdata);

        rdata = 0xa0;
        DRV_WriteReg32(AFE_IRQ_MCU_CNT7, rdata); // set counter be 160, 10ms

        rdata = DRV_Reg32(AFE_IRQ_MCU_EN); // select irq7 to NOT ap
        rdata &= ~(0x80);
        DRV_WriteReg32(AFE_IRQ_MCU_EN, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_EN1); // select irq7 to scp
        rdata |= 0x800000;
        DRV_WriteReg32(AFE_IRQ_MCU_EN1, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_CON); // poyen temp : irq7 enable
        rdata |= 0x80;
        DRV_WriteReg32(AFE_IRQ_MCU_CON, rdata);

    } else {

        rdata = DRV_Reg32(AFE_IRQ_MCU_CON);  //AFE_IRQ_MCU_CON0[7] = 0  irq7 disable
        rdata &= ~(0x80);
        DRV_WriteReg32(AFE_IRQ_MCU_CON, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_CLR); //AFE_IRQ_MCU_CLR[7] = 1 clr irq7
        rdata |= 0x80;
        DRV_WriteReg32(AFE_IRQ_MCU_CLR, rdata);
        rdata = DRV_Reg32(AFE_IRQ_MCU_CLR); //AFE_IRQ_MCU_CLR[7] = 1
        rdata |= 0x80;
        DRV_WriteReg32(AFE_IRQ_MCU_CLR, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_CLR); //AFE_IRQ_MCU_CLR[23] = 1 // clr irq7 related
        rdata |= 0x800000;
        DRV_WriteReg32(AFE_IRQ_MCU_CLR, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_EN1); //AFE_IRQ_MCU_EN1[23] = 0 // select irq7 to NOT scp
        rdata &= ~(0x800000);
        DRV_WriteReg32(AFE_IRQ_MCU_EN1, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_EN); //AFE_IRQ_MCU_EN[7] = 1 // select irq7 to AP
        rdata |= 0x80;
        DRV_WriteReg32(AFE_IRQ_MCU_EN, rdata);

        rdata = 0x0;
        DRV_WriteReg32(AFE_IRQ_MCU_CNT7, rdata); //AFE_IRQ_MCU_CNT7 = 0 irq7 counter clear
        rdata = DRV_Reg32(AFE_IRQ_MCU_CON1);
        rdata &= ~(0xF0000000);
        DRV_WriteReg32(AFE_IRQ_MCU_CON1, rdata); //clear irq sampling rate
...
void vow_set_mem_awb(bool enable)
{
    ...
    if (enable) {
        rdata = DRV_Reg32(AFE_DAC_CON0); //trun on mem interface, AWB: bit6
        rdata |= 0x00000040;
        DRV_WriteReg32(AFE_DAC_CON0, rdata);
    } else {
        rdata = DRV_Reg32(AFE_DAC_CON0); //trun off mem interface, AWB: bit6
        rdata &= ~(0x00000040);
        DRV_WriteReg32(AFE_DAC_CON0, rdata);
    }
    AUD_LOG_D("-vow_set_mem_awb %d\n", enable);
