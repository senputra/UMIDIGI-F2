...
{
	if ((irq_id > MT6768_IRQ_NUM) || (irq_id < MT6768_IRQ_0)) {
...
void vow_set_bargein_int(bool enable, unsigned int irq_id)
{
    ...
    if (enable) {
        ...
        rdata = DRV_Reg32(AFE_IRQ_MCU_EN); // select irq to NOT ap
        rdata &= ~(1 << irq_id);
        DRV_WriteReg32(AFE_IRQ_MCU_EN, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_EN1); // select irq to scp
        rdata |= (1 << (irq_id + 16));
        DRV_WriteReg32(AFE_IRQ_MCU_EN1, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_CON); // irq enable
        rdata |= (1 << irq_id);
        DRV_WriteReg32(AFE_IRQ_MCU_CON, rdata);

    } else {

        rdata = DRV_Reg32(AFE_IRQ_MCU_CON);  // irq disable
        rdata &= ~(1 << irq_id);
        DRV_WriteReg32(AFE_IRQ_MCU_CON, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_CLR); //clr irq
        rdata |= (1 << irq_id);
        DRV_WriteReg32(AFE_IRQ_MCU_CLR, rdata);
        rdata = DRV_Reg32(AFE_IRQ_MCU_CLR);
        rdata |= (1 << irq_id);
        DRV_WriteReg32(AFE_IRQ_MCU_CLR, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_CLR);// clr irq related
        rdata |= (1 << (irq_id + 16));
        DRV_WriteReg32(AFE_IRQ_MCU_CLR, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_EN1);// select irq to NOT scp
        rdata &= ~(1 << (irq_id + 16));
        DRV_WriteReg32(AFE_IRQ_MCU_EN1, rdata);

        rdata = DRV_Reg32(AFE_IRQ_MCU_EN); // select irq to AP
        rdata |= (1 << irq_id);
        DRV_WriteReg32(AFE_IRQ_MCU_EN, rdata);
