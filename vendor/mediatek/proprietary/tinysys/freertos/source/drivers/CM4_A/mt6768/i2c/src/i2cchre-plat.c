...
static inline void mtI2cStartEnable(struct MtI2cDev *i2c) {
    ...
    /* Enable interrupt */
    i2c_writew(I2C_IBI | I2C_TRANSAC_COMP, i2c, OFFSET_INTR_MASK);
    ...
    if (xfer) {
        ...
        if (atomicCmpXchgByte((uint8_t *)&state->masterState,
            ...
            if (atomicCmpXchg32bits(&xfer->id, id, 0)) {
                ...
                atomicWriteByte(&state->masterState, MT_I2C_MASTER_IDLE);
                I2CERR("i2c%lu: sync call error, %u, %lu, %lu\n", pdev->id,
    ...
    if (xfer) {
        ...
        if (atomicCmpXchgByte((uint8_t *)&state->masterState,
            ...
            if (atomicCmpXchg32bits(&xfer->id, id, 0)) {
                ...
                atomicWriteByte(&state->masterState, MT_I2C_MASTER_IDLE);
                I2CERR("i2c%lu: retry delay error, %u, %lu, %lu\n", pdev->id,
                    state->masterState, xfer->id, id);
            }
        }
        return 0;
...
int32_t mt_i2c_test_1(void) {
    ...
    if (ret < 0) {
        I2CERR("i2c test 1 i2cMasterRequest for ch0 failed\r\n");
        return ret;
    }
    /*write*/
    ret = i2cMasterTx(0, EEPROM_ADD, tx_data, 8, i2cCallback, NULL);
    if (ret < 0)
        I2CLOG("i2c test 1 i2cMasterTx ch0 failed\r\n");
    i2cMasterRelease(0);
    ...
    if (ret < 0) {
        I2CLOG("i2c test 1 i2cMasterRequest for ch1 failed\r\n");
        return ret;
    }
    /*write*/
    ret = i2cMasterTx(1, EEPROM_ADD, tx_data, 2, i2cCallback, NULL);
    if (ret < 0)
        I2CLOG("i2c test 1 i2cMasterTx for ch1 failed\r\n");
    i2cMasterRelease(1);
...
int32_t mt_i2c_test_2(void) {
    ...
    ret = i2cMasterRequest(0, EEPROM_SPEED);
    if (ret < 0) {
        I2CLOG("i2c test 2 i2cMasterRequest for ch0 failed\r\n");
        return ret;
    }
    /*read*/
    ret = i2cMasterTxRx(0, EEPROM_ADD, tx_data, 1, rx_data, 8, i2cCallback, NULL);
    if (ret < 0) {
        I2CLOG("i2c test 2 i2cMasterTxRx for ch0 failed\r\n");
        return ret;
    }
    i2cMasterRelease(0);
...
int32_t mt_i2c_test_3(void) {
    ...
    ret = i2cMasterRequest(1, EEPROM_SPEED);
    if (ret < 0) {
        I2CLOG("i2c test 3 i2cMasterRequest for ch1 failed\r\n");
        return ret;
    }
    /*read*/
    ret = i2cMasterTxRx(1, EEPROM_ADD, tx_data, 1, rx_data, 8, i2cCallback, NULL);
    if (ret < 0) {
        I2CLOG("i2c test 3 i2cMasterTxRx for ch1 failed\r\n");
        return ret;
    }
    i2cMasterRelease(1);
