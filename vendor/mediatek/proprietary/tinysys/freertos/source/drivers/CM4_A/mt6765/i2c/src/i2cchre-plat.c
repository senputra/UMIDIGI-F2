...
static inline void _i2c_writew(uint16_t value, struct MtI2cDev *i2c, uint16_t offset) {
    mt_reg_sync_writel(value, (i2c->base) + (offset));
}

static inline uint16_t _i2c_readw(struct MtI2cDev *i2c, uint8_t offset) {
...
static inline void mtI2cStartEnable(struct MtI2cDev *i2c) {
    ...
    /* we must firstly start oneshot timer, then set i2c start bit */
...
#if 0/* for shrink code size as it's not used */
void i2c_dump_dma_info(struct MtI2cDev *i2c) {
...
static void mtI2cIsrEvent(struct MtI2cDev *i2c) {
    ...
    if (intr_stat & (I2C_HS_NACKERR | I2C_ACKERR)) {
        I2CERR("i2c%ld:addr=0x%x,ACK error=0x%x,retry=%d\n",
                    i2c->id, i2c->addr, intr_stat, i2c->state.retry);
        if (!i2c->state.retry)
            i2c_dump_info(i2c);
        /* need init hw here, because the i2c error occured  */
        mt_i2c_init_hw(i2c);
...
static int32_t mtk_i2c_calculate_speed(struct MtI2cDev *i2c,
    ...
    for (sample_cnt = 1; sample_cnt <= MAX_SAMPLE_CNT_DIV; sample_cnt++) {
        ...
        if (cnt_mul < best_mul) {
            ...
            step_div = step_cnt;
            I2CERR("best_mul %lu sample_div %lu step_div %lu min_div %lu\n",
                   best_mul, sample_div, step_div, min_div);
            if (best_mul == min_div)
                break;
        }
    }
...
static inline int mtI2cSpeedSet(struct MtI2cDev *i2c) {
    ...
    if ((i2c->speedInHz == 0) || i2c_set_speed(i2c) < 0) {
        I2CERR("i2c_set_speed fail, speedInHz %lu\n", i2c->speedInHz);
...
int32_t i2c_init(void) {
    ...
    /* set I3C0 clock src clk_sys_div4 26Mhz/4 = 6.5Mhz for I3C0 (MT6765)*/
     DRV_SetReg32(SCP_I3C_BCLK_SW, SCP_I3C_BCLK_SW_CLKSYS_DIV4);
    ...
    } while (((reg & SCP_BCLK_CK_STA_MASK) != SCP_I3C_BCLK_CK_STA_CLKSYS_DIV4) && tmo_cnt);
   ...
   /* init i2c0(MT6765 has only i2c0) interrupt and hw registers */
    i2c_clock_enable(&mMtI2cDevs[0]);
    mt_i2c_mask_and_clear_int(&mMtI2cDevs[0]);
    mt_i2c_init_hw(&mMtI2cDevs[0]);
    i2c_set_speed(&mMtI2cDevs[0]);
   /* clear pending irq to avoid old CIRQ trigger */
    request_irq(I2C0_IRQn, i2c0_irq_handler, "I2C0");
#ifdef CFG_VCORE_DVFS_SUPPORT
    scp_wakeup_src_setup(I2C0_WAKE_CLK_CTRL, 1);
#endif
    i2c_clock_disable(&mMtI2cDevs[0]);
...
int32_t mt_i2c_dma_tx(struct MtI2cDev *i2c) {
    ...
    i2c_writew(0x1f, i2c, OFFSET_INTR_MASK);
...
#define I2C0_SLT_TEST_SLAVE_ADD 0x50/* 0x50~0x53 */
#define I2C0_SLT_TEST_DATA_ADD 0x02/* or 0x03 */
#define I2C0_SLT_TEST_PATTERN 0x5a

int32_t mt_i2c_slt_i2c0(void) {
    int32_t ret = 0;
    struct MtI2cDev *i2c = &mMtI2cDevs[0];
    volatile uint32_t timeout = 0;
    uint16_t intr_stat = 0;
    ...
    i2c_writew(control_reg, i2c, OFFSET_CONTROL);
    i2c_writew(0x1801, i2c, OFFSET_EXT_CONF);
    i2c_writew(0x01, i2c, OFFSET_TRANSAC_LEN);
    i2c_writew(0x02, i2c, OFFSET_TRANSFER_LEN);
    i2c_writew(0x00, i2c, OFFSET_TRANSFER_LEN_AUX);
    i2c_writew(0x0f, i2c, OFFSET_TIMING);
    i2c_writew(0x0f, i2c, OFFSET_LTIMING);
    i2c_writew(0x04, i2c, OFFSET_CLOCK_DIV);
    i2c_writew(0x03, i2c, OFFSET_IO_CONFIG);
    i2c_writew(0x01, i2c, OFFSET_FIFO_ADDR_CLR);
    i2c_writew(0x28, i2c, OFFSET_DEBUGCTRL);
    i2c_writew(I2C0_SLT_TEST_DATA_ADD, i2c, OFFSET_DATA_PORT);
    i2c_writew(I2C0_SLT_TEST_PATTERN, i2c, OFFSET_DATA_PORT);
    I2C_MB();
    i2c_writew(0x01, i2c, OFFSET_START);
    ...
    while (i2c_readw(i2c, OFFSET_START)) {
        timeout--;
        if (timeout == 0) {
            I2CERR("i2c%ld:addr=0x%x,write timeout\n", i2c->id, i2c->addr);
            i2c_dump_info(i2c);
            ret = -1;
            goto out;
        }
   ...
   i2c_writew(0x1ff, i2c, OFFSET_INTR_STAT);
   if (intr_stat & (I2C_HS_NACKERR | I2C_ACKERR)) {
        I2CERR("i2c%ld:addr=0x%x,write ACK error,intr= 0x%x\n",
    ...
    while (i2c_readw(i2c, OFFSET_START)) {
        timeout--;
        if (timeout == 0) {
            I2CERR("i2c%ld:addr=0x%x,read timeout\n", i2c->id, i2c->addr);
            i2c_dump_info(i2c);
            ret = -1;
            goto out;
        }
    ...
    i2c_writew(0x1ff, i2c, OFFSET_INTR_STAT);
    if (intr_stat & (I2C_HS_NACKERR | I2C_ACKERR)) {
        I2CERR("i2c%ld:addr=0x%x,read ACK error,intr= 0x%x\n",
                    i2c->id, i2c->addr, intr_stat);
        i2c_dump_info(i2c);
        ret = -1;
        goto out;
    ...
    data = 0xffff & (i2c_readw(i2c, OFFSET_DATA_PORT));
    if (data != I2C0_SLT_TEST_PATTERN) {
        I2CERR("i2c%ld:addr=0x%x,compare error data=0x%x\n",
