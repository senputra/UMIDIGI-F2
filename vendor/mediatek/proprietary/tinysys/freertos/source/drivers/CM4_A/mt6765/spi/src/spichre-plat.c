...
#define SPI_RECORD_LEN          2

static struct spi_info_buf rec_info_buf;
...
struct spi_info_buf {
    struct spi_info spi_infos[SPI_RECORD_LEN];
    uint8_t head;
    uint8_t tail;
};

void spi_record_err(int err){
    struct spi_info *info_p = &rec_info_buf.spi_infos[rec_info_buf.tail];
    rec_info_buf.tail = (rec_info_buf.tail + 1) % SPI_RECORD_LEN;
    info_p->err = err;
    info_p->timestamp = get_boot_time_ns();
    if(rec_info_buf.head == rec_info_buf.tail)
        rec_info_buf.head = (rec_info_buf.head + 1) % SPI_RECORD_LEN;
}
void spi_send_err_to_ap(void) {
    error_info err_infos;
    err_infos.case_id = ERR_CASE_SPI;
    err_infos.sensor_id = ERR_SENSOR_SPI;
    struct spi_info *info_p;
    while(rec_info_buf.head != rec_info_buf.tail) {
        info_p = &rec_info_buf.spi_infos[rec_info_buf.head];
...
void Spi_release_lock(uint32_t id)
{
    if(id < SPI_HOST_NUM)
    {
        ...
        SPI_ERR("release lock invalid id=%d\n", (int)id);
}
...
void Spi_enable_clk(uint32_t id)
{
    switch (id) {
        ...
        case 1:
            ...
            break;
        default:
            SPI_ERR("enable clk invalid id=%d\n", (int)id);
            break;
    }
...
void Spi_disable_clk(uint32_t id)
{
    switch (id) {
        ...
        case 1:
            ...
            break;
        default:
            SPI_ERR("disable clk invalid id=%d\n", (int)id);
            break;
    }
    SPI_MSG("Spi_disable_clk\n");
}

/*Used for SCP SPI SLT and DVT Call*/
#if defined(CFG_SLT_SUPPORT) || defined(SPI_TEST_DEBUG)
void spi_gpio_set(uint32_t id, int muxId)
{
    SPI_MSG("spi_gpio_set id = %d\n", id);

    switch (id) {
        case 0:
            if (muxId == 0) {
                SPI_REG_SET32(SCP_SPI0_GPIO_BASE , (SPI_REG_GET32(SCP_SPI0_GPIO_BASE) & 0x000FFFFF) | 0x33300000);
                //GPIO13:MI, GPIO14:CS, GPIO15:MO,
                SPI_REG_SET32(SCP_SPI0_GPIO_BASE_2 , (SPI_REG_GET32(SCP_SPI0_GPIO_BASE_2) & 0xFFFFFFF0) | 0x00000003);
                //GPIO16:CLK,
            } else if (muxId == 1)
                SPI_REG_SET32(SCP_SPI0_GPIO_BASE_2 , (SPI_REG_GET32(SCP_SPI0_GPIO_BASE_2) & 0xFFF0000F) | 0x00033330);
                //GPIO17:MO, GPIO18:CK, GPI19:MI, GPIO20:CS
            else if (muxId == 2) {
                SPI_REG_SET32(SCP_SPI0_GPIO_BASE_2 , (SPI_REG_GET32(SCP_SPI0_GPIO_BASE_2) & 0x000FFFFF) | 0x55500000);
                //GPIO21:MI, GPIO22:CS, GPI23:MO
                SPI_REG_SET32(SCP_SPI0_GPIO_BASE_3 , (SPI_REG_GET32(SCP_SPI0_GPIO_BASE_3) & 0xFFFFFFF0) | 0x00000005);
                //GPIO24:CK
            }
            break;
        case 1:
            if (muxId == 0)
                SPI_REG_SET32(SCP_SPI1_GPIO_BASE , (SPI_REG_GET32(SCP_SPI1_GPIO_BASE) & 0x0000FFFF) | 0x22220000);
                //GPIO4:MI, GPIO5:CS, GPIO6:MO, GPIO7:CLK
            break;
        default:
            SPI_ERR("spi_gpio_set invalid id=%d\n", (int)id);
            break;
    }
}
#endif
...
void spiTimerCallback(uint32_t timerId, void *cookie)
{
    ...
    SPI_ERR("spi%lu: irq:%d maskIrqstate:%lu pending:%s,active:%s\n",
            xfer->id, irqNum, maskIrq, NVIC_GetPendingIRQ(irqNum)?"Y":"N",
            NVIC_GetActive(irqNum)?"Y":"N");

    spi_record_err(SPI_ERR_TIMEOUT);
    SCPSpiTimeoutHandler(xfer->id, xfer->base);
    SPI_CHRE_READ(xfer->base, SPI_REG_STATUS1);
    SCPDMACallBack[xfer->id].callback(SCPDMACallBack[xfer->id].cookie, SPI_ERR_TIMEOUT);
