...
#if DVFS_DEBUG
#define DVFSFUC(fmt, arg...) PRINTF_D(DVFSTAG "%s("fmt")\n", __func__, ##arg)
#else
#define DVFSFUC(...)
#endif
...
void request_clk_sys_opp(uint32_t req_opp)
{
    ...
    switch (req_opp) {
        case CLK_OPP0:
            ...
            break;
        case CLK_OPP1:
            pll_opp_switch(2); /* 208 MHz */
            break;
        case CLK_OPP2:
            pll_opp_switch(4); /* 312 MHz */
            break;
        case CLK_OPP3:
...
#ifdef CFG_PMIC_WRAP_SUPPORT
unsigned int pmic_scp_set_vcore(uint32_t req_volt)
{
    unsigned int wdata = (req_volt - 50000) / 625;
    unsigned int rdata = 0;
    unsigned int ret = 0;

    pwrap_scp_write(PMIC_RG_BUCK_VCORE_SSHUB_EN_ADDR,
                    (0x0 &
                     PMIC_RG_BUCK_VCORE_SSHUB_EN_MASK) <<
                    PMIC_RG_BUCK_VCORE_SSHUB_EN_SHIFT);

    pwrap_scp_write(PMIC_RG_BUCK_VCORE_SSHUB_VOSEL_ADDR,
                    (wdata &
    ...
    pwrap_scp_read(PMIC_RG_BUCK_VCORE_SSHUB_VOSEL_ADDR, &rdata);

    ret = (rdata >> PMIC_RG_BUCK_VCORE_SSHUB_VOSEL_SHIFT) &
          PMIC_RG_BUCK_VCORE_SSHUB_VOSEL_MASK;

    DVFSLOG("vcore: 0x%x %d (mVolt)\n", wdata, rdata, ((ret * 625) + 50000) / 100);
    return (ret * 625) + 50000;
}

unsigned int pmic_scp_set_vsram_others(uint32_t req_volt)
{
    unsigned int wdata = (req_volt - 50000) / 625;
    unsigned int rdata = 0;
    unsigned int ret = 0;

    pwrap_scp_write(PMIC_RG_LDO_VSRAM_OTHERS_SSHUB_EN_ADDR,
                    (0x0 &
                     PMIC_RG_LDO_VSRAM_OTHERS_SSHUB_EN_MASK) <<
                    PMIC_RG_LDO_VSRAM_OTHERS_SSHUB_EN_SHIFT);

    pwrap_scp_write(PMIC_RG_LDO_VSRAM_OTHERS_SSHUB_VOSEL_ADDR,
                    (wdata &
    ...
    pwrap_scp_read(PMIC_RG_LDO_VSRAM_OTHERS_SSHUB_VOSEL_ADDR, &rdata);

    ret = (rdata >> PMIC_RG_LDO_VSRAM_OTHERS_SSHUB_VOSEL_SHIFT) &
          PMIC_RG_LDO_VSRAM_OTHERS_SSHUB_VOSEL_MASK;

    DVFSLOG("vsram_others: 0x%x %d (mVolt)\n", rdata, ((ret * 625) + 50000) / 100);
    return (ret * 625) + 50000;
}
...
void request_dfs_opp(uint32_t req_opp)
{
    ...
    switch ((clk_opp_enum)req_opp) {
        case CLK_OPP0: /* 165M (=ulposc2 div2) */
...
void scp_set_opp(void)
{
    ...
    mlprintf(LOG_DVFS_NOTE, "DVFS request: %d => %d\n", (int)cur_opp, (int)req_opp);
...
uint32_t get_ulposc_clk_by_freqmeter(ulposc_id_enum ulposc)
{
    ...
    unsigned int clk_misc_cfg_0, clk_dbg_cfg, clk26cali_0, clk26cali_1;
    unsigned int abist_idx;
    ...
    while (DRV_Reg32(CLK26CALI_0) & (0x1 << 4)) {
        i++;
        if (i > 1000000) {
            mlprintf(LOG_DVFS_DEBUG, "freq-meter timeout!\n");
            is_fmeter_timeout = 1;
            break;
        }
    }
    ...
    DRV_ClrReg32(CLK26CALI_0, 0x1 << 12);

    /* get freq meter result */
    if (!is_fmeter_timeout)
        result = DRV_Reg32(CLK26CALI_1) & 0xffff;
    else
        result = 0;
    ...
    mlprintf(LOG_DVFS_DEBUG, "freq=%ud MHz\n",
             (result * 26) / 1024);
...
uint32_t is_ulposc_calibrated(ulposc_id_enum ulposc_id, freq_enum target_clk)
{
    ...
    /* check if calibrated value is in the range of target value +- 7% */
    if (current_val > (target_val * 930 / 1000) && current_val < (target_val * 1070 / 1000)) {
        mlprintf(LOG_DVFS_NOTE, "ULPOSC%d is calibrated (%ldMHz)\n", ulposc_id + 1, (current_val * 26) / 1024);
        return 1;
    } else {
        mlprintf(LOG_DVFS_NOTE, "ULPOSC%d is not calibrated (%ldMHz)\n", ulposc_id + 1, (current_val * 26) / 1024);
        return 0;
    }
}
...
uint32_t ulposc_cali_process(ulposc_id_enum ulposc_id, freq_enum target_clk)
{
    ...
    do {
        middle = (min + max) / 2;
        if (middle == min) {
            mlprintf(LOG_DVFS_DEBUG, "middle(%ld) == min(%ld)\n", middle, min);
            break;
        }
    ...
    if (current_val > (target_val * 850 / 1000) && current_val < (target_val * 1150 / 1000)) {
        mlprintf(LOG_DVFS_NOTE, "ULPOSC%d calibration done: %ldM\n", ulposc_id + 1, current_val * 26 / 1024);
    } else {
        mlprintf(LOG_DVFS_EMERG, "ULPOSC%d calibration fail: %ldM\n", ulposc_id + 1,  current_val * 26 / 1024);
        configASSERT(0);
    }
...
void config_ulposc1_registers(void)
{
     ...
     */
    /* OSC1_RSV= 00000000 */
    DRV_ClrReg32(ULPOSC1_CON0, RG_OSC1_RSV_MSK << RG_OSC1_RSV_BIT);
    ...
    mlprintf(LOG_DVFS_DEBUG, "ULPOSC1_CON0 = 0x%x, ULPOSC1_CON1 = 0x%x\n",
...
void config_ulposc2_registers(void)
{
     ...
     */
    /* OSC2_RSV= 00000000 */
    DRV_ClrReg32(ULPOSC2_CON0, RG_OSC2_RSV_MSK << RG_OSC2_RSV_BIT);
    ...
    mlprintf(LOG_DVFS_DEBUG, "ULPOSC2_CON0 = 0x%x, ULPOSC2_CON1 = 0x%x\n",
...
void ulposc_cali_init(void)
{
    ...
    if (is_ulposc_calibrated(ULPOSC_1, FREQ_250MHZ) == 0) {
        /* calibrate ULPOSC1 */
...
void dvfs_init(void)
{
    ...
    DRV_WriteReg32(CPU_VREQ_CTRL, 0x08010001);
#else
    ...
    mlprintf(LOG_DVFS_DEBUG, "%s done\n", __func__);
}
...
int request_pmic_lp_mode_for_vow_from_isr(int enabled)
{
    ...
    if (enabled == SCP_ENABLE) {
        ...
        if (drv_reg32(CURRENT_FREQ_REG) != clk_opp_table[CLK_OPP0]) {
            mlprintf(LOG_DVFS_ERROR, "Not allow to enter PMIC VOW LP mode due to DVFS == OPP0\n");
            return -1;
        }
    } else
