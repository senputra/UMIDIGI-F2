...
static void wait_bus_ack(void)
{
    ...
    if (count >= 1000000) {
        mlprintf(LOG_DVFS_EMERG, "bus ack fail\n");
        configASSERT(0);
    }
}
...
void enable_infra(scp_reserve_mem_id_t dma_id, uint32_t wait_ack)
{
    ...
    if (dma_id >= NUMS_MEM_ID) {
        mlprintf(LOG_DVFS_EMERG, "err id %d\n", dma_id);
        configASSERT(0);
        return;
    }
...
void disable_infra(scp_reserve_mem_id_t dma_id)
{
    ...
    if (infra_user_list[dma_id] == 0) {
        mlprintf(LOG_DVFS_EMERG, "infra_user_list[%d]==0\n", dma_id);
        configASSERT(0);
        return;
    }

    if (infra_req_cnt == 0) {
        mlprintf(LOG_DVFS_EMERG, "infra_req_cnt==0,id=%d\n", dma_id);
        configASSERT(0);
        return;
    }
...
void wait_dram_ack(void)
{
    ...
    if (count >= 1000000) {
        mlprintf(LOG_DVFS_EMERG, "DRAM ack fail\n");
        configASSERT(0);
    }
}
...
void disable_AP_resource(scp_reserve_mem_id_t dma_id)
{
    ...
    if (dma_user_list[dma_id] == 0) {
        mlprintf(LOG_DVFS_EMERG, "dma_user_list[%d]==0\n", dma_id);
        configASSERT(0);
        return;
    }

    if (ap_resource_req_cnt == 0) {
        mlprintf(LOG_DVFS_EMERG, "ap_resource_req_cnt==0,id=%d\n", dma_id);
        configASSERT(0);
        return;
    }
