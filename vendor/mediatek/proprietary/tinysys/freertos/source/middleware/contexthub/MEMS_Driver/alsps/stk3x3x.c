...
#define ALS_NAME                         "stk3x3x_l"
#define PS_NAME                          "stk3x3x_p"
...
enum stk3x3xState {
    ...
    STATE_SET_PS_THDH,
    STATE_SET_PS_THDL,
    STATE_SET_SPEC_ADDR1,
    STATE_SET_SPEC_ADDR2,
    STATE_SET_SPEC_ADDR3,
    STATE_SETUP_EINT,
    STATE_CORE
};
...
SRAM_REGION_BSS static struct stk3x3xTask {
...
static struct stk3x3xTask *stk3x3xDebugPoint;
...
static int stk3x3x_read_als_data(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
...
static int stk3x3x_get_als_value(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    int als_data = 0;
...
static int stk3x3x_read_ps(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                          void *inBuf, uint8_t inSize, uint8_t elemInSize,
                          void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    if (rxTransferDataInfo(&mTask.dataInfo, inBuf, inSize, elemInSize, outBuf, outSize, elemOutSize)) {
        sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, ERROR_EVT);
        osLog(LOG_ERROR, "stk3x3x ps, rx dataInfo error\n");
        return -1;
    }
...
static int stk3x3x_get_ps_status(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    ...
    /*
    if((mTask.rxBuf[0] & 0xFE) != 0){
        //osLog(LOG_ERROR, "stk3x3x stk3x3x_get_ps_status value is not ps event, mTask.rxBuf[0]= %d!!!\n", mTask.rxBuf[0]);
        return -1;
    }*/
...
static int stk3x3x_get_ps_raw_data(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_clr_int(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.txBuf[0] = STK_FLAG_REG;
...
static int stk3x3x_enable_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static void psGetData(void *sample)
{
    ...
    void get_ps_data(void *cookie, size_t tx, size_t rx, int err)
    {
        ...
        if (err == 0) {
            ...
            osLog(LOG_INFO, "stk3x3x: read ps data i2c error (%d)\n", err);
    }
...
static void stk3x3x_ps_set_threshold(uint32_t threshold_high, uint32_t threshold_low)
{
    int ret;
    char txBuf[6];
    ...
    ret = i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, &txBuf[0], 5,
                NULL, 0);
    if (ret) {
        osLog(LOG_INFO, "stk3x3x: set ps threshold i2c error (%d)\n", ret);
        return;
    }
}
...
{
    mTask.psCali = cali[0];
    stk3x3x_ps_set_threshold(mTask.ps_threshold_high + mTask.psCali,
...
static void psSetThreshold(uint32_t threshold_high, uint32_t threshold_low)
{
    ...
    stk3x3x_ps_set_threshold(mTask.ps_threshold_high, mTask.ps_threshold_low);
}

static int stk3x3x_mask_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                         void *inBuf, uint8_t inSize, uint8_t elemInSize,
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_unmask_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                         void *inBuf, uint8_t inSize, uint8_t elemInSize,
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mt_eint_unmask(mTask.hw->eint_num);
...
static int stk3x3x_set_als_debounce_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                      void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                      void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_set_ps_debounce_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                     void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                     void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_set_als_power_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                   void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                   void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_set_als_power_off(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                    void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                    void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.als_debounce_on = 0;
...
static int stk3x3x_als_ratechg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3x3x_set_ps_power_on(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                  void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                  void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    uint8_t cur = mTask.rxBuf[0];
    cur &= (~(0x45));
    cur |= STK_STATE_EN_PS_MASK;
    if (!(mTask.rxBuf[0] & STK_STATE_EN_ALS_MASK))
...
static int stk3x3x_set_ps_power_off(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                   void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                   void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    uint8_t cur = mTask.rxBuf[0];
...
static int stk3x3x_ps_ratechg(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
...
static int stk3x3x_get_alsps_state(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                               void *inBuf, uint8_t inSize, uint8_t elemInSize,
                               void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.txBuf[0] = STK_STATE_REG;
...
static int stk3x3x_set_wait_time(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_set_sw_reset(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_set_alsps_ctrl(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.txBuf[0] = STK_STATE_REG;
...
static int stk3x3x_set_als2_ctrl(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.txBuf[0] = STK_ALSCTRL2_REG;
    mTask.txBuf[1] = STK_ALSC_GAIN16;
    return i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 7,
                       i2cCallBack, next_state);
}

static int stk3x3x_set_intell_wait_ps(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.txBuf[0] = STK_INTELLI_WAIT_PS_REG;
    mTask.txBuf[1] = STK_INTELL_20;
    return i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 7,
                       i2cCallBack, next_state);
}

static int stk3x3x_set_spec_addr1(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_set_spec_addr2(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.txBuf[0] = STK_SPEC2_REG;
    mTask.txBuf[1] = STK_SPEC2_ADDR;
    return i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 4,
                       i2cCallBack, next_state);
}

static int stk3x3x_set_spec_addr3(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                              void *inBuf, uint8_t inSize, uint8_t elemInSize,
                              void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mTask.txBuf[0] = STK_SPEC3_REG;
    mTask.txBuf[1] = STK_SPEC3_ADDR;
    return i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 4,
                       i2cCallBack, next_state);
}

static int stk3x3x_set_ps_thdl(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
...
static int stk3x3x_set_ps_thdh(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    if (mTask.hw->polling_mode_ps == 0) {
...
SRAM_REGION_FUNCTION static void stk3x3x_eint_handler(int arg)
{
    osLog(LOG_ERROR, "stk3x3x stk3x3x_eint_handler enter!!!\n");
    alsPsInterruptOccur();
}

static int stk3x3x_setup_eint(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                             void *inBuf, uint8_t inSize, uint8_t elemInSize,
                             void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    mt_eint_dis_hw_debounce(mTask.hw->eint_num);
    mt_eint_registration(mTask.hw->eint_num, LEVEL_SENSITIVE, LOW_LEVEL_TRIGGER, stk3x3x_eint_handler, EINT_INT_UNMASK,
                         EINT_INT_AUTO_UNMASK_OFF);
    sensorFsmEnqueueFakeI2cEvt(i2cCallBack, next_state, SUCCESS_EVT);
    return 0;
}

static void stk3x3xSetDebugTrace(int32_t trace)
{
    int i, ret = 0;
    ...
    for(i=0; i<11;i++){
        ...
        if(ret < 0){
            osLog(LOG_ERROR, "stk3x3x i2cMasterTxRxSync FAIL!!!\n");
...
static int stk3x3x_register_core(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                                void *inBuf, uint8_t inSize, uint8_t elemInSize,
                                void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    ...
    mInfo.setDebugTrace = stk3x3xSetDebugTrace;
...
static struct sensorFsm stk3x3xFsm[] = {
    /* sample als */
    sensorFsmCmd(STATE_SAMPLE_ALS, STATE_GET_ALS_DATA, stk3x3x_read_als_data),
    sensorFsmCmd(STATE_GET_ALS_DATA, STATE_SAMPLE_ALS_DONE, stk3x3x_get_als_value),

    /* sample ps */
    sensorFsmCmd(STATE_SAMPLE_PS, STATE_GET_PS_FLG, stk3x3x_read_ps),
    sensorFsmCmd(STATE_GET_PS_FLG, STATE_GET_PS_RAW_DATA, stk3x3x_get_ps_status),
    sensorFsmCmd(STATE_GET_PS_RAW_DATA, STATE_CLR_INT, stk3x3x_get_ps_raw_data),
    sensorFsmCmd(STATE_CLR_INT, STATE_EN_EINT, stk3x3x_clr_int),
    sensorFsmCmd(STATE_EN_EINT, STATE_SAMPLE_PS_DONE, stk3x3x_enable_eint),

    /* als enable state */
    sensorFsmCmd(STATE_ALS_ENABLE, STATE_ALS_POWER_ON, stk3x3x_get_alsps_state),
    sensorFsmCmd(STATE_ALS_POWER_ON, STATE_ALS_SET_DEBOUNCE, stk3x3x_set_als_power_on),
    sensorFsmCmd(STATE_ALS_SET_DEBOUNCE, STATE_ALS_ENABLE_DONE, stk3x3x_set_als_debounce_on),

    /* als disable state */
    sensorFsmCmd(STATE_ALS_DISABLE, STATE_ALS_POWER_OFF, stk3x3x_get_alsps_state),
    sensorFsmCmd(STATE_ALS_POWER_OFF, STATE_ALS_DISABLE_DONE, stk3x3x_set_als_power_off),

    /* ps enable state */
    sensorFsmCmd(STATE_PS_ENABLE, STATE_PS_POWER_ON, stk3x3x_get_alsps_state),
    sensorFsmCmd(STATE_PS_POWER_ON, STATE_PS_SET_DEBOUNCE, stk3x3x_set_ps_power_on),
    sensorFsmCmd(STATE_PS_SET_DEBOUNCE, STATE_PS_UNMASK_EINT, stk3x3x_set_ps_debounce_on),
    sensorFsmCmd(STATE_PS_UNMASK_EINT, STATE_PS_ENABLE_DONE, stk3x3x_unmask_eint),
    /* ps disable state */
    sensorFsmCmd(STATE_PS_DISABLE, STATE_GET_ALSPS_STATE, stk3x3x_mask_eint),
    sensorFsmCmd(STATE_GET_ALSPS_STATE, STATE_PS_POWER_OFF, stk3x3x_get_alsps_state),
    sensorFsmCmd(STATE_PS_POWER_OFF, STATE_PS_DISABLE_DONE, stk3x3x_set_ps_power_off),

    /* als rate change */
    sensorFsmCmd(STATE_ALS_RATECHG, STATE_ALS_RATECHG_DONE, stk3x3x_als_ratechg),
    /* ps rate change */
    sensorFsmCmd(STATE_PS_RATECHG, STATE_PS_RATECHG_DONE, stk3x3x_ps_ratechg),
    /* init state */
    sensorFsmCmd(STATE_RESET, STATE_SET_SW_RST, stk3x3x_set_wait_time),
    sensorFsmCmd(STATE_SET_SW_RST, STATE_SET_ALSPS_CTRL, stk3x3x_set_sw_reset),
    sensorFsmCmd(STATE_SET_ALSPS_CTRL, STATE_SET_PS_THDH, stk3x3x_set_alsps_ctrl),
    sensorFsmCmd(STATE_SET_PS_THDH, STATE_SET_PS_THDL, stk3x3x_set_ps_thdh),
    sensorFsmCmd(STATE_SET_PS_THDL, STATE_SET_ALS2_CTRL, stk3x3x_set_ps_thdl),
    sensorFsmCmd(STATE_SET_ALS2_CTRL, STATE_SET_INTELL_WAIT_PS, stk3x3x_set_als2_ctrl),
    sensorFsmCmd(STATE_SET_INTELL_WAIT_PS, STATE_SET_SPEC_ADDR1, stk3x3x_set_intell_wait_ps),
    sensorFsmCmd(STATE_SET_SPEC_ADDR1, STATE_SET_SPEC_ADDR2, stk3x3x_set_spec_addr1),
    sensorFsmCmd(STATE_SET_SPEC_ADDR2, STATE_SET_SPEC_ADDR3, stk3x3x_set_spec_addr2),
    sensorFsmCmd(STATE_SET_SPEC_ADDR3, STATE_SETUP_EINT, stk3x3x_set_spec_addr3),
    sensorFsmCmd(STATE_SETUP_EINT, STATE_CORE, stk3x3x_setup_eint),
    sensorFsmCmd(STATE_CORE, STATE_INIT_DONE, stk3x3x_register_core),
};

static int stk3x3xInit(void)
{
    int ret = 0;
    osLog(LOG_INFO, "%s: task starting\n", __func__);
    stk3x3xDebugPoint = &mTask;

    mTask.hw = get_cust_alsps("stk3x3x");
    if (NULL == mTask.hw) {
    ...
    if (mTask.deviceId == STK3335_PID) {  // stk3x3x device id is fixed
        mTask.ledctrl_val = 0x60;
        osLog(LOG_INFO, "stk3335: auto detect success:0x%x\n", mTask.deviceId);
        goto success_out;
    } else {
        mTask.ledctrl_val = 0x20; /* PID is 0x15 */
        i2cMasterRelease(mTask.hw->i2c_num);
        osLog(LOG_ERROR, "stk3x3x: read id fail!!!\n");
        ret = -1;
        goto err_out;
    }
success_out:
    alsSensorRegister();
    psSensorRegister();
    registerAlsPsDriverFsm(stk3x3xFsm, ARRAY_SIZE(stk3x3xFsm));
err_out:
    return ret;
}

#ifndef CFG_OVERLAY_INIT_SUPPORT
MODULE_DECLARE(stk3x3x, SENS_TYPE_ALS, stk3x3xInit);
#else
#include "mtk_overlay_init.h"
OVERLAY_DECLARE(stk3x3x, OVERLAY_ID_ALSPS, stk3x3xInit);
#endif
