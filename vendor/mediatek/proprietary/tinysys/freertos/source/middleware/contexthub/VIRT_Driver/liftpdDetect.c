...
#define _LIFTPD_CUSTOMIZED_CONFIG_
#define _SHORT_ACC_LATENCY_

struct liftpdDetectorTask {
    uint32_t taskId;
    uint32_t handle;
...
#endif
// *****************************************************************************
static void liftpdInit()
{
#ifndef _LIFTPD_CUSTOMIZED_CONFIG_
    ...
    osLog(LOG_INFO, "LIFTPD INIT DONE\n");
}

void liftpdGestureConfig(uint8_t on)
{
#ifdef _LIFTPD_CUSTOMIZED_CONFIG_
...
static void liftpdDetectorAlgo(struct TripleAxisDataEvent *ev)
{
        ...
        while (mResample.input_count > 0) {
            ...
            if (!mTask.prox_is_shaded) {
                if (gesture_alg_output.lift_result) {
                    ...
                    osLog(LOG_INFO, "lift notify @%lld, %u\n\n", rtcGetTime(), mResample.algo_time_stamp);
                }
            }
            if (gesture_alg_output.put_result) {
                union EmbeddedDataPoint sample;
                sample.idata = 0;
                osEnqueueEvt(sensorGetMyEventType(SENS_TYPE_TILT), sample.vptr, NULL);
                osLog(LOG_INFO, "putdown notify @%lld, %u\n\n", rtcGetTime(), mResample.algo_time_stamp);
            }
...
static bool liftpdDetectorPower(bool on, void *cookie)
{
    //Initialize only when any sensor is the first registered sensor
    osLog(LOG_INFO, "liftpdDetectPower:%d \n", on);
...
static bool liftpdDetectorSetRate(uint32_t rate, uint64_t latency, void *cookie)
{
    sensorSignalInternalEvt(mTask.handle, SENSOR_INTERNAL_EVT_RATE_CHG, rate, latency);
    return true;
}

static bool liftpdDetectorFirmwareUpload(void *cookie)
{
...
static bool liftpdDetectorFlush(void *cookie)
{
    return osEnqueueEvt(sensorGetMyEventType(SENS_TYPE_TILT), SENSOR_DATA_EVENT_FLUSH, NULL);
}

static void liftpdDetectorHandleEvent(uint32_t evtType, const void* evtData)
{
    ...
    switch (evtType) {
        ...
        case EVT_SENSOR_ACC:
            ...
            liftpdDetectorAlgo((struct TripleAxisDataEvent *)evtData);
        ...
        case EVT_SENSOR_PROX: {
            ...
            osLog(LOG_INFO, "EVT_SENSOR_PROX:%u, prox_is_shaded:%u\n", (uint32_t)ev.fdata, !(uint32_t)ev.fdata);
...
static const struct SensorOps mSops = {
    .sensorPower = liftpdDetectorPower,
    .sensorFirmwareUpload = liftpdDetectorFirmwareUpload,
    .sensorSetRate = liftpdDetectorSetRate,
    .sensorFlush = liftpdDetectorFlush,
};

static bool liftpdDetectorStart(uint32_t taskId)
{
    mTask.taskId = taskId;
...
static void liftpdDetectorEnd()
{
}
INTERNAL_APP_INIT(APP_ID_MAKE(APP_ID_VENDOR_MTK, MTK_APP_ID_WRAP(SENS_TYPE_TILT, 0, 0)), 0, liftpdDetectorStart,
                  liftpdDetectorEnd, liftpdDetectorHandleEvent);
