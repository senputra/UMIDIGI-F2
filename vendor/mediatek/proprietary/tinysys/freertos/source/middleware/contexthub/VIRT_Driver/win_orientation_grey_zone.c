...
#define PROPOSAL_MIN_TIME_SINCE_FLAT_ENDED              NS2US(500000000ull)      // MTK: origin(500 ms)
#define PROPOSAL_MIN_TIME_SINCE_SWING_ENDED             NS2US(300000000ull)      // MTK: origin(300 ms)
#define PROPOSAL_MIN_TIME_SINCE_ACCELERATION_ENDED      NS2US(500000000ull)      // MTK: origin(500 ms)

#define FLAT_ANGLE                          75                          // MTK: origin(80)
#define FLAT_TIME                           NS2US(40000000ull)          // MTK: origin(1 sec)
...
#define FILTER_TIME_CONSTANT                NS2US(200000000ull)         // MTK: origin(200 ms)
...
#define ADJACENT_ORIENTATION_ANGLE_GREY     25

#define DEV_LESS_THAN_GAP                   0
#define DEV_IN_GREY_ZONE                    1
#define DEV_MORE_THAN_GAP                   2

#define RET_IN_GREY_ZONE                    5
...
// ----------------------------------------------
// 1. [Return value] 0 ~ 3, and 5 (in grey zone or flat)
...
static int isOrientationAngleAcceptable(int current_rotation, int rotation,
                                        int orientation_angle)
{
    ...
    int lower_bound_grey_zone, upper_bound_grey_zone;
    if (current_rotation >= 0) {
        ...
        if ((rotation == current_rotation) || (rotation == (current_rotation + 1) % 4)) {
            ...
            lower_bound_grey_zone = rotation * 90 - 45 + ADJACENT_ORIENTATION_ANGLE_GREY / 2;
            if (rotation == 0) {
                if ((orientation_angle >= 315) && (orientation_angle < lower_bound_grey_zone + 360)) {
                    return DEV_LESS_THAN_GAP;
                } else if ((orientation_angle >= 315) && (orientation_angle < lower_bound + 360)) {
                    return ((rotation == (current_rotation + 1) % 4)) ? DEV_IN_GREY_ZONE : DEV_LESS_THAN_GAP;
                }
            } else {
                if (orientation_angle < lower_bound_grey_zone) {
                    return DEV_LESS_THAN_GAP;
                } else if (orientation_angle < lower_bound) {
                    return ((rotation == (current_rotation + 1) % 4)) ? DEV_IN_GREY_ZONE : DEV_LESS_THAN_GAP;
                }
            }
        }
        ...
        if ((rotation == current_rotation) || (rotation == (current_rotation + 3) % 4)) {
            ...
            upper_bound_grey_zone = rotation * 90 + 45 - ADJACENT_ORIENTATION_ANGLE_GREY / 2;
            if (rotation == 0) {
                if ((orientation_angle <= 45) && (orientation_angle > upper_bound_grey_zone)) {
                    return DEV_LESS_THAN_GAP;
                } else if ((orientation_angle <= 45) && (orientation_angle > upper_bound)) {
                    return ((rotation == (current_rotation + 3) % 4)) ? DEV_IN_GREY_ZONE : DEV_LESS_THAN_GAP;
                }
            } else {
                if (orientation_angle > upper_bound_grey_zone) {
                    return DEV_LESS_THAN_GAP;
                } else if (orientation_angle > upper_bound) {
                    return ((rotation == (current_rotation + 3) % 4)) ? DEV_IN_GREY_ZONE : DEV_LESS_THAN_GAP;
                }
            }
        }
    }
    return DEV_MORE_THAN_GAP;
}
        ...
        while (mResample.input_count > 0) {
            ...
            if (!skip_sample) {
                ...
                if (magnitude < NEAR_ZERO_MAGNITUDE) {
                    ...
                    // Add flat as a special report value
                    if (now < mTask.flat_time + PROPOSAL_MIN_TIME_SINCE_FLAT_ENDED) {
                        updatePredictedRotation(now, RET_IN_GREY_ZONE);
                    } else if (mTask.overhead) {
                        clearPredictedRotation();
                        ...
                        if (isTiltAngleAcceptable(nearest_rotation, tilt_angle)
                                && isOrientationAngleAcceptable(mTask.current_rotation,
                                                                nearest_rotation,
                                                                orientation_angle) == DEV_MORE_THAN_GAP) {
                            updatePredictedRotation(now, nearest_rotation);
                        } else if (isTiltAngleAcceptable(nearest_rotation, tilt_angle)
                                   && isOrientationAngleAcceptable(mTask.current_rotation,
                                           nearest_rotation,
                                           orientation_angle) == DEV_IN_GREY_ZONE) {
                            updatePredictedRotation(now, RET_IN_GREY_ZONE);
                        } else {
                            clearPredictedRotation();
                        }
            ...
            // Handling the grey zone event at the end of the samples
            if (mTask.predicted_rotation != RET_IN_GREY_ZONE || mResample.input_count == 1) {
                ...
                if ((proposed_rotation != old_proposed_rotation) && (proposed_rotation >= 0)) {
                    // Avoid update RET_IN_GREY_ZONE to mTask.current_rotation
                    // We need use isOrientationAngleAcceptable() to check the previous 0, 1, 2, 3
                    if (proposed_rotation != RET_IN_GREY_ZONE) mTask.current_rotation = proposed_rotation;
