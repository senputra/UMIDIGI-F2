...
//#define _LIFT_CUSTOMIZED_CONFIG_
//#define _SHORT_ACC_LATENCY_

struct liftDetectTask {
    uint32_t taskId;
    uint32_t handle;
    uint32_t anyMotionHandle;
    uint32_t noMotionHandle;
    uint32_t accelHandle;
    uint32_t accel_normal_latency;
    enum {
        STATE_DISABLED,
...
static struct liftDetectTask mTask;
static resampling_t mResample;
static swMotion_t mMotion;
#ifdef _LIFT_CUSTOMIZED_CONFIG_
...
{
#ifndef _LIFT_CUSTOMIZED_CONFIG_
...
#endif
#ifdef _SHORT_ACC_LATENCY_
    mTask.accel_normal_latency = 50000000ull;
#else
    mTask.accel_normal_latency = ACCEL_NORMAL_LATENCY;
#endif
    init_data_resampling(&mResample, GESTURE_INPUT_INTERVAL);
    memset(&mMotion, false, sizeof(swMotion_t));
    osLog(LOG_INFO, "LIFT INIT DONE\n");
}

void liftGestureConfig(uint8_t on)
{
#ifdef _LIFT_CUSTOMIZED_CONFIG_
...
static void liftDetectAlgo(struct TripleAxisDataEvent *ev)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        while (mResample.input_count > 0) {
            ...
            if (gesture_alg_output.lift_result) {
                ...
                sample.idata = 1;
                osEnqueueEvt(sensorGetMyEventType(SENS_TYPE_LIFT), sample.vptr, NULL);
...
static void configAnyMotion(bool on)
{
    ...
    if (on && (!mTask.anyMotionHandle)) {
        ...
        if (!getAnyMotionSrc) {
            osLog(LOG_INFO, "[WARNING] LIFT: anyMotion sensor is not available\n");
        }
...
static void configNoMotion(bool on)
{
    ...
    if (on && (!mTask.noMotionHandle)) {
        ...
        if (!getNoMotionSrc) {
            osLog(LOG_INFO, "[WARNING] LIFT: noMotion sensor is not available\n");
        }
    ...
    if (on && (!mTask.accelHandle)) {
        for (i = 0; sensorFind(SENS_TYPE_ACCEL, i, &mTask.accelHandle) != NULL; i++) {
            if (acc_class == NORMAL_ACC_WINDOW) {
                if (sensorRequest(mTask.taskId, mTask.accelHandle, ACCEL_MIN_RATE, mTask.accel_normal_latency)) {
                    getAccelSensor = true;
        ...
        if (!getAccelSensor) {
            osLog(LOG_INFO, "[WARNING] LIFT: Accel sensor is not available\n");
        }
...
static bool liftDetectPower(bool on, void *cookie)
{
    osLog(LOG_INFO, "LiftDetectPower:%d\n", on);
...
static bool liftDetectSetRate(uint32_t rate, uint64_t latency, void *cookie)
{
    sensorSignalInternalEvt(mTask.handle, SENSOR_INTERNAL_EVT_RATE_CHG, rate, latency);
    return true;
}

static bool liftDetectFirmwareUpload(void *cookie)
{
    sensorSignalInternalEvt(mTask.handle, SENSOR_INTERNAL_EVT_FW_STATE_CHG, 1, 0);
    return true;
}

static bool liftDetectFlush(void *cookie)
{
    return osEnqueueEvt(sensorGetMyEventType(SENS_TYPE_LIFT), SENSOR_DATA_EVENT_FLUSH, NULL);
}

static void liftDetectHandleEvent(uint32_t evtType, const void* evtData)
{
    ...
    switch (evtType) {
        case EVT_APP_START:
            ...
            osLog(LOG_INFO, "LIFT EVT_APP_START\n");
            break;
        ...
        case EVT_SENSOR_ACC:
            ...
            liftDetectAlgo((struct TripleAxisDataEvent *)evtData);
...
static const struct SensorOps mSops = {
    ...
    .sensorFirmwareUpload = liftDetectFirmwareUpload,
    .sensorSetRate = liftDetectSetRate,
    .sensorFlush = liftDetectFlush,
};

static bool liftDetectStart(uint32_t taskId)
{
    mTask.taskId = taskId;
...
static void liftDetectEnd()
{
}
INTERNAL_APP_INIT(APP_ID_MAKE(APP_ID_VENDOR_MTK, MTK_APP_ID_WRAP(SENS_TYPE_LIFT, 0, 0)), 0, liftDetectStart,
                  liftDetectEnd, liftDetectHandleEvent);
