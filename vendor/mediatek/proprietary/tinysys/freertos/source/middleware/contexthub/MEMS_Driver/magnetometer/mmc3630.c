...
static void magGetSensorInfo(struct sensorInfo_t *data)
{
    ...
    memcpy(&data->mag_dev_info, &mTask.mag_dev_info, sizeof(data->mag_dev_info));
}
...
static void mmc3630_convert_otp(uint8_t *data)
{
    ...
    if (stemp >= 32)
        ...
        stemp = stemp;
    mTask.otpMatrix[1] = stemp * 6  + 1000;
    stemp = (((data[0] & 0xC0) >> 6) | ((data[1] & 0x0F) << 2));
    if (stemp >= 32)
        stemp = 32 - stemp;
    else
        stemp = stemp;

    mTask.otpMatrix[2] = (stemp * 6 + 1000) * 135 / 100;

    //osLog(LOG_INFO, " mmc3630 read otp %d %d %d ",mTask.otpMatrix[0],mTask.otpMatrix[1],mTask.otpMatrix[2]);
}
#ifdef VENDOR_EDIT
...
static void mmc3630_selftest(int32_t *testResult)
{
    ...
    osLog(LOG_ERROR, "%s diff: %d %d %d \n", __func__, abs(curr_raw[0] - prev_raw[0]), abs(curr_raw[1] - prev_raw[1]),
          abs(curr_raw[2] - prev_raw[2]));
...
#ifdef VENDOR_EDIT
    mInfo.selfTest = mmc3630_selftest;
#endif//VENDOR_EDIT
                          ...
                          void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    ...
    data[0].z = data[0].z * mTask.otpMatrix[2] / 1000;
...
OVERLAY_DECLARE(mmc3630, OVERLAY_ID_MAG, mmc3630Init);
#endif
