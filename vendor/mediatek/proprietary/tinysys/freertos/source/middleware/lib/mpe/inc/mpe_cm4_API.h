...
* Define
******************************************************************************/
#define MPE_CM4_VERSION_HEAD 'M','P','E','_','C','M','4','_','V','E','R','_'
#define MPE_MAJOR_VERSION '1','6','0','6','1','4'  // y,y,m,m,d,d
#define MPE_MINOR_VERSION '0','1' //rev
#define MPE_CM4_VER_INFO MPE_CM4_VERSION_HEAD,MPE_MAJOR_VERSION,MPE_MINOR_VERSION
...
typedef struct {
    float vel_out[3];      /* velocities output in NED frame [vn, ve, vd] in meter/s *///12
    float heading_out;     /* output heading in radian*///4
    float heading_out_acc; /* heading accuracy in radian *///4
    float height;         /*barometer height in meter*///4
    mtk_int32 vout;            /* output calculated velocity/AR status [static, move, uncertain],[0,1,99]*///4
    mtk_int32 validation_flag; /*valid flag for mpe result[ valid, invalid], [1,0]*///4
...
******************************************************************************/
void mpe_init(void);
void mpe_data_entry(MPE_SENSOR_COMMON *data_in, mtk_int32 data_size, U1 data_type);
mtk_int8 mpe_data_synch(mtk_uint32 algo_type, mtk_uint32 algo_group);
void mpe_start_run1(MPE_SENSOR_RESULT *result, MPE_ALGO_TYPE algo_type);
void mpe_get_adr_flag(MNL_location_output_t *adr_result);
void GyroSensor_set_timebase_correction_parameter(mtk_uint32 event_delay_ns, mtk_int8 TB_PERCENT);
void mpe_gps_entry ( MNL_location_input_t *gps_in );
