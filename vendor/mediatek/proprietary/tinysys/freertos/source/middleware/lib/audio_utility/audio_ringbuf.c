...
#include <hal_cache.h>
#define CACHE_LINE_SIZE portBYTE_ALIGNMENT
#else
#include <scp_ipi.h>
#include <cache_internal.h>
#define CACHE_LINE_SIZE MTK_CACHE_LINE_SIZE
#endif


#define AUDIO_BUF_ALIGNEMNT (8)
#define AUDIO_DMA_ALIGNMENT (128)
...
static int dma_length_alignment(unsigned int len, unsigned int dst_addr,
                                unsigned int src_addr)
{
    if (len % CACHE_LINE_SIZE || dst_addr % CACHE_LINE_SIZE ||
...
#endif
    if (dma_length_alignment(len, dst_addr, src_addr) == 0) {
        memcpy((void *)dst_addr, (void *)src_addr, len);
#if !defined(CFG_MTK_AUDIODSP_SUPPORT)
        AUD_LOG_W("aud data not align, len:0x%x, dst_addr:0x%x, src_addr:0x%x",
        ...
        ret = scp_dma_transaction(dst_addr,
                                  ...
                                  NO_RESERVED);
        if (ret != 0) {
            AUD_LOG_E("scp_dma_transaction error:%d", ret);
        }
#ifdef CFG_MTK_AUDIODSP_SUPPORT
                                  ...
                                  unsigned int count)
{
    int spaceIHave;
    char *end = RingBuf1->pBufBase + RingBuf1->bufLen;
    spaceIHave = RingBuf_getFreeSpace(RingBuf1);    /* count buffer data I have */
    if (spaceIHave < count) {   /* if not enough, assert */
        AUD_LOG_W("spaceIHave %d < count %d\n", spaceIHave,
                  count); /* ASSERT(spaceIHave >= count); */
        return;
    }
    if (RingBuf1->pRead <= RingBuf1->pWrite) {
        int w2e = end - RingBuf1->pWrite;
        if (count <= w2e) {
            memcpy(RingBuf1->pWrite, buf, count);
            if ((((unsigned int)RingBuf1->pWrite) % CACHE_LINE_SIZE) ||
                (count % CACHE_LINE_SIZE)) {
                ALOGW("%s RingBuf1->pWrite = %p count %d line %d\n", __func__, RingBuf1->pWrite,
                      count, __LINE__);
            }
            hal_cache_region_operation(HAL_CACHE_TYPE_DATA, HAL_CACHE_OPERATION_FLUSH,
                                       (void *)RingBuf1->pWrite, count);
            RingBuf1->pWrite += count;
            if (RingBuf1->pWrite >= end) {
                RingBuf1->pWrite -= RingBuf1->bufLen;
            }
            ...
            if ((((unsigned int)RingBuf1->pWrite) % CACHE_LINE_SIZE) ||
                (w2e % CACHE_LINE_SIZE)) {
                ALOGW("%s RingBuf1->pWrite = %p w2e %d line %d\n", __func__, RingBuf1->pWrite,
                      w2e, __LINE__);
            }
            hal_cache_region_operation(HAL_CACHE_TYPE_DATA, HAL_CACHE_OPERATION_FLUSH,
                                       (void *)RingBuf1->pWrite, w2e);
            memcpy(RingBuf1->pBufBase, buf + w2e, count - w2e);
            if ((((unsigned int)RingBuf1->pBufBase) % CACHE_LINE_SIZE) ||
                ((count - w2e) % CACHE_LINE_SIZE)) {
                ALOGW("%s RingBuf1->pBufBase = %p count - w2 %d line %d\n", __func__,
                      RingBuf1->pBufBase,  count - w2e, __LINE__);
            }
            hal_cache_region_operation(HAL_CACHE_TYPE_DATA, HAL_CACHE_OPERATION_FLUSH,
                                       (void *)RingBuf1->pBufBase, count - w2e);
            RingBuf1->pWrite = RingBuf1->pBufBase + count - w2e;
        }
    }
    else {
                                ...
                                unsigned int count)
{
    ...
    if (RingBuf1->pRead <= RingBuf1->pWrite) {
        hal_cache_region_operation(HAL_CACHE_TYPE_DATA, HAL_CACHE_OPERATION_INVALIDATE,
                                   (void *)RingBuf1->pRead, count);
        memcpy(buf, RingBuf1->pRead, count);
        if ((((unsigned int)RingBuf1->pRead) % CACHE_LINE_SIZE) ||
            (count % CACHE_LINE_SIZE)) {
            ALOGW("%s RingBuf1->pRead = %p count %d line %d\n", __func__,  RingBuf1->pRead,
                  count, __LINE__);
        }
        RingBuf1->pRead += count;
        if (RingBuf1->pRead >= RingBuf1->pBufEnd) {
        ...
        if (count <= r2e) {
            ...
            hal_cache_region_operation(HAL_CACHE_TYPE_DATA, HAL_CACHE_OPERATION_INVALIDATE,
                                       (void *)RingBuf1->pRead, r2e);
            if ((((unsigned int)RingBuf1->pRead) % CACHE_LINE_SIZE) ||
                (r2e % CACHE_LINE_SIZE)) {
                ALOGW("%s RingBuf1->pRead = %p r2e %d line %d\n", __func__, RingBuf1->pRead,
                      r2e, __LINE__);
            }
            memcpy(buf, RingBuf1->pRead, r2e);
            hal_cache_region_operation(HAL_CACHE_TYPE_DATA, HAL_CACHE_OPERATION_INVALIDATE,
                                       (void *)RingBuf1->pBufBase, count - r2e);
            if ((((unsigned int)RingBuf1->pBufBase) % CACHE_LINE_SIZE) ||
                ((count - r2e) % CACHE_LINE_SIZE)) {
                ALOGW("%s RingBuf1->pBufBase = %p count - r2e %d line %d\n", __func__,
                      RingBuf1->pBufBase, count - r2e, __LINE__);
            }
            memcpy(buf + r2e, RingBuf1->pBufBase, count - r2e);
            RingBuf1->pRead = RingBuf1->pBufBase + count - r2e;
        }
