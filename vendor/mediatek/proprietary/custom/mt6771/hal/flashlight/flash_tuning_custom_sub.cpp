...
#endif
static int FlashIMapFunc_sub(int duty, int dutyLt)
{
...
static int dutyMaskFunc_sub(int d, int dLt)
{
    // TODO: setup mask current
    if (FlashIMapFunc_sub(d, dLt) > 1100)
        return 0;
    return 1;
}
...
void cust_getFlashMaxIDuty_sub(int dutyNum, int dutyNumLt, int* duty, int* dutyLt)
{
    ...
    max_I = FlashIMapFunc_sub(max_duty, max_dutyLt);

    for (int i = -1; i < dutyNum; i++) {
        for (int j = -1; j < dutyNumLt; j++) {
            if (dutyMaskFunc_sub(i,j) == 1)
                if (FlashIMapFunc_sub(i,j) >= max_I) {
                    max_I = FlashIMapFunc_sub(i,j);
                    max_duty = i;
                    max_dutyLt = j;
                }
        }
...
FLASH_PROJECT_PARA& cust_getFlashProjectPara_sub(int aeScene, int isForceFlash, NVRAM_CAMERA_STROBE_STRUCT* nvrame)
{
    ...
    para.dutyAvailableMaskFunc = dutyMaskFunc_sub;
