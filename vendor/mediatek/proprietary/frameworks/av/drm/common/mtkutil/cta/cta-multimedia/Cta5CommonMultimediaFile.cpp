...
bool Cta5CommonMultimediaFile::setCtaMultimediaHeader(int fd) {
    ...
    if (count > 4294967294) {
        ALOGE("[ERROR][CTA5]setCtaMultimediaHeader - too many header count.[%lu] reset", count);
        count = 4294967294;
    }
    ...
    if ((uint32_t)(4294967294/32) < mmHeaderCount) {
        ALOGE("[ERROR][CTA5]setCtaMultimediaHeader - too many mmHeaderCount.[%lu] reset", mmHeaderCount);
        header_size = 4294967295 - 1; // max of uint32
    } else {
        header_size = (uint32_t)(mmHeaderCount * 32);
    }
    ...
    for(uint32_t i = 0; i < mmHeaderCount; i++) {
        pos = (uint32_t)(pos + (uint32_t)(i * 32));
    ...
    mmCtaHeaderSize = (off64_t)(CTA_MAGIC_LEN + sizeof(count) + header_size);
    ALOGD("setCtaMultimediaHeader - mmCtaHeaderSize[%ld]", (long) mmCtaHeaderSize);
    return true;
}

uint32_t Cta5CommonMultimediaFile::verifyCtaMultimediaHeader(int fd) {
    ...
    read(fd, (uint8_t *)&count, sizeof(count));
    ALOGD("verifyCtaMultimediaHeader - count: 0x%02x, 0x%02x, 0x%02x, 0x%02x",
        ((uint8_t*)&count)[0], ((uint8_t*)&count)[1], ((uint8_t*)&count)[2], ((uint8_t*)&count)[3]);
    ALOGD("verifyCtaMultimediaHeader - count: 0x%08x", count);
    header_count = Cta5FileUtil::nettohostl(count);
    ALOGD("verifyCtaMultimediaHeader - header count[%lu]", header_count);
    // 3. read headers
    uint32_t header_size = 0L;
    if ((uint32_t)(4294967294/32) < header_count) {
        ALOGE("[ERROR][CTA5]verifyCtaMultimediaHeader - too many header_count.[%lu] reset", header_count);
        header_size = 4294967295 - 1; // max of uint32
    } else {
        header_size = (uint32_t)(header_count * 32);
    }

    // get cta mm header size
    uint32_t totalMmCtaHeaderSize = CTA_MAGIC_LEN + sizeof(count) + header_size;
...
String8 Cta5CommonMultimediaFile::getOriginalMimetype(int fd, const String8 &key) {
    ...
    uint32_t totalMmCtaHeaderSize = verifyCtaMultimediaHeader(fd);
