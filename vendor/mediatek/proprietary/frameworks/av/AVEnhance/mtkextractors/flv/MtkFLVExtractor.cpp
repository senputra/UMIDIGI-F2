...
FLV_ERROR_TYPE flvParser::flv_setup_seektable() {
    ...
    if (mfile->mMeta->filepositions && mfile->mMeta->times &&
            mfile->mMeta->timescnt == mfile->mMeta->fileposcnt) {
...
FLV_ERROR_TYPE flvParser::flv_parse_amf_obj(
    ...
    if (*offset > (int64_t)amf_data_len - 1) {
        ALOGE("amf error: *offset %u, amf_data_len %u", *offset, amf_data_len);
        return FLV_ERROR;
    }
    ...
    *offset = *offset+1;
    ...
    switch (amf_type) {
        case FLV_AMF_V0_TYPE_NUMBER:
        {
            ...
            // key = times && depth == 3
            if (*offset > (int64_t)amf_data_len - 8) {
                ALOGE("amf error: *offset %u, amf_data_len %u", *offset, amf_data_len);
                return FLV_ERROR;
            }
        ...
        {
            if (*offset > (int64_t)amf_data_len - 2) {
                ALOGE("amf error: *offset %u, amf_data_len %u", *offset, amf_data_len);
                return FLV_ERROR;
            }
            ...
            if (*offset > (int64_t)amf_data_len - strsize) {
                ALOGE("amf error: *offset %u, amf_data_len %u, strsize %u", *offset, amf_data_len, strsize);
                return FLV_ERROR;
            }
        ...
        case FLV_AMF_V0_TYPE_OBJECT:
        {
            ...
            while(*offset <  amf_data_len){
              ...
              if (*offset > (int64_t)amf_data_len - 3) {
                  ALOGE("amf error: *offset %u, amf_data_len %u", *offset, amf_data_len);
                  return FLV_ERROR;
              }
              strsize = flv_byteio_get_2byte(amf_data+(*offset));
              ...
              flv_byteio_get_string((uint8_t*)strVal, strsize+1,amf_data+(*offset));
              (*offset) = (*offset)+strsize;
              ret = flv_parse_amf_obj(amf_data, amf_data_len, offset, strVal, metaInfo, depth + 1);
              if (ret != FLV_OK) return ret;
            }
            break;
        }
        ...
        {
            if (*offset > (int64_t)amf_data_len - 4) {
                ALOGE("amf error: *offset %u, amf_data_len %u", *offset, amf_data_len);
                return FLV_ERROR;
            }
        ...
        {
            if (*offset > (int64_t)amf_data_len - 10) {
                ALOGE("amf error: *offset %u, amf_data_len %u", *offset, amf_data_len);
                return FLV_ERROR;
            }
            numVal = flv_amf_number2double(flv_byteio_get_8byte(amf_data+(*offset)));
        ...
        case FLV_AMF_V0_TYPE_MIXED_ARRAY:
        {
            ...
            while (*offset < amf_data_len) {
                ...
                ret = flv_parse_amf_obj(amf_data, amf_data_len, offset, strVal, metaInfo, depth + 1);  // depth == 1
                if (ret != FLV_OK) return ret;
            }
            break;
        }
        ...
        default: //unsupported type, we couldn't skip
            ALOGW("unsupported amf type %d", amf_type);
            return FLV_ERROR;
    }
    ...
    if (depth == 1 && key && (FLV_AMF_V0_TYPE_NUMBER == amf_type || FLV_AMF_V0_TYPE_BOOL == amf_type)) {
        if (0 == strcmp(key, "duration")) {
         ...
         // metaInfo->fileposidx can only use at this
         if (metaInfo->fileposidx < metaInfo->fileposcnt) {
             metaInfo->filepositions[metaInfo->fileposidx++] = (uint64_t)numVal;
         } else {
             ALOGW("seektable may not accurate, filepositions may in another place, idx=cnt=%llu",
                     (unsigned long long)metaInfo->fileposidx);
         }
    } else if (depth==3 && key  && (0 == strcmp(key, "times")) && FLV_AMF_V0_TYPE_NUMBER == amf_type){
         // metaInfo->timesidx can only use at this
         if (metaInfo->timesidx < metaInfo->timescnt) {
             metaInfo->times[metaInfo->timesidx++] = (uint64_t)numVal;
         } else {
             ALOGW("seektable may not accurate, times may in another place, idx=cnt=%llu",
                     (unsigned long long)metaInfo->timesidx);
        }
    }
    return FLV_OK;
}
...
FLV_ERROR_TYPE flvParser::flv_parse_onMetaData(flv_tag_str* pMeta_tag,flv_meta_str* metaInfo)
{
    ...
    FLV_ERROR_TYPE ret;
    if (!pMeta_tag || !pMeta_tag->tag_data) {
        ALOGE("flv_parse_onMetaData ERROR");
        return FLV_ERROR;
    }
    if (pMeta_tag->tag_header.tag_data_size < 14) {
        ALOGE("broken onMetaData tag: tag_data_size %u", pMeta_tag->tag_header.tag_data_size);
        return FLV_ERROR;
    }
...
int32_t flvParser::flv_search_tag_pattern(uint8_t **data, uint32_t size) {
    ...
    if (size < FLV_TAG_PREV_SIZE + FLV_TAG_HEADER_SIZE) {
        ALOGD("flv_search_tag: no avc key frame, size too small %u", size);
        return size;
    }
...
uint32_t flvParser::flv_search_video_tag_pattern(uint8_t *data, uint32_t size) {
    ...
    if (size < 15) {
        ALOGD("no video tag: remaining size %u < 15", size);
        return 0;
    }
    do {
        do {
...
uint32_t flvParser::flv_search_audio_tag_pattern(uint8_t *data, uint32_t size) {
    ...
    if (size < 15) {
        ALOGD("no audio tag: remaining size %u < 15", size);
        return 0;
    }
    do {
        do {
...
void flv_byteio_get_string(uint8_t *string, uint32_t strlen, uint8_t *data) {
    uint i = 0;
    char c;
    if (strlen >= 256 || strlen <= 1) {
        ALOGE("flv_byteio_get_string Error: because strlen %u (>= 256 or <= 1)", strlen);
        return;
    }

    while (i < (strlen - 1) && (c = flv_byteio_get_byte(data++))) {
        string[i++] = c;
    }
    ...
    if (false == bSeekable) {
        ...
        for (size_t i = 0; i < mTracks.size(); ++i) {
            ...
            for (size_t i = 0; i < mVideoFrames.size(); i++) {
                ...
                // out should be released as it will not be used. Otherwise it will cause memory leak.
                out->release();
                out = NULL;
            }
...
status_t FLVExtractor::parseAVCCodecSpecificData(
    ...
    *buffer_size = 0;
    ...
    for (size_t i = 0; i < numSeqParameterSets; ++i) {
        ...
        if (size < length) {
            ALOGE("stream error: error AVC CSD");
            return ERROR_MALFORMED;
        }
        ...
        case FLV_VIDEO_CODEC_ID_AVC:
        {
            ...
            if (0 == AVCPacketType) {
                ...
                if (OK == parseAVCCodecSpecificData(pTag->tag_data+FLV_VIDEO_AVC_TAG_DATA_OFFSET,
                            pTag->tag_header.tag_data_size - FLV_VIDEO_AVC_TAG_DATA_OFFSET, &buffer_size)) {
                    ...
                    if (ts < 0) {
                        ALOGW("ts < 0: ts %lld, tag_header.tag_ts %llu, CompositionTime %x, u4Var %x",
                                (long long)ts,  (unsigned long long)pTag->tag_header.tag_ts, CompositionTime, u4Var);
                    }
            ...
            if (mWantsNALFragments == false) {
                 ...
                 * So in order to make a frame can contains 10 nals, then MediaBuffer need at least allocate memory of
                 * (tag_data_size + 25B) because mNalLengthSize may be 1
                 */
                if (pTag->tag_header.tag_data_size > UINT32_MAX - 25) {
                    ALOGE("tag_data_size is too big: %u", pTag->tag_header.tag_data_size);
                    // free buffer
                    free(pTag->tag_data);
                    pTag->tag_data = NULL;
                    mVideoFrames.removeItemsAt(0);
                ...
                MediaBufferBase *tempBuffer = MediaBufferBase::Create(pTag->tag_header.tag_data_size + 25);
                ...
                while (pTag->tag_data_offset < (pTag->tag_header.tag_data_size - mNALLengthSize)) {
                    ...
                    if (this_nal_size == 0) {
                        ALOGW("skip this nal because size is 0");
                    } else if (full_frame_size > UINT32_MAX - 4 ||
                            full_frame_size + 4 > UINT32_MAX - this_nal_size ||
                            full_frame_size + 4 + this_nal_size > tempBuffer->size()) {
                        ALOGE("nal is invalid: this_nal_size %u, fullFrameSize %u, tempBuferSize %zu",
                                this_nal_size, full_frame_size, tempBuffer->size());
                        tempBuffer->release();
                        tempBuffer = NULL;
                        data = NULL;
