    ...
    DataSourceHelper *source, off_t *inout_pos, ape_parser_ctx_t *ape_ctx, bool parseall)
{
    ...
    bool ret = false;
    *inout_pos = 0; //ape info should read from 0 position.
    ...
    *inout_pos += 2;
    ...
    {
        if (ape_ctx->blocksperframe > ((UINT_MAX - ape_ctx->totalsamples) / (ape_ctx->totalframes - 1))){
            goto GetApeInfo_Exit;
        } else {
            ape_ctx->totalsamples += ape_ctx->blocksperframe * (ape_ctx->totalframes - 1);
        }
    }
    if (ape_ctx->seektablelength >= 4)
    {
        ...
        {
            if (source->readAt(*inout_pos, &seekaddr, 4) != 4)
            {
    ...
    //junk(maybe 0) -> descriptor -> header ->wave(maybe 0) ->seektable ->content
    ape_ctx->seektablefilepos = ape_ctx->junklength + ape_ctx->descriptorlength +
                                ape_ctx->headerlength + ape_ctx->wavheaderlength;
    ape_ctx->firstframe = ape_ctx->seektablefilepos + ape_ctx->seektablelength;
      ...
      mUseFloatPCM(useFloatPCM)
{
    ...
    // malloc and copy seek table
    mTableOfContents = (uint32_t *)malloc(mSt_bound * 4);
        if (mTableOfContents == NULL) {
        ALOGE("APEExtractor has no builtin seektable return ");
        return;
    }
    memcpy(mTableOfContents, table_of_contents, mSt_bound* 4);
}
...
media_status_t APESource::start()
{
    ...
    if (fileoffset < mTableOfContents[0]) {
        ALOGD("raw data smaller than 1 frame, can not playback");
        return AMEDIA_OK;
    }

    if ((mTableOfContents != NULL) &&
            (mSt_bound > 1) &&
            (fileoffset < mTableOfContents[mSt_bound - 1]))
    {
        int i = 1;
        for (i = 1; i < mSt_bound; i++)
        {
    ...
    *out = NULL;
    ...
    if (options != NULL && options->getSeekTo(&seekTimeUs, &mode))
    {

        {
            ...
            if ((mTotalsample > 0) && mTableOfContents != NULL
                    && (mTableOfContents[0] > 0) && (mSamplesPerFrame > 0)
      ...
      mSamplesPerFrame(0)
{
    ...
    if (ape_ctx.samplerate > (INT_MAX / (ape_ctx.bps * ape_ctx.channels))) {
        return;
    } else {
        AMediaFormat_setInt32(mMeta, AMEDIAFORMAT_KEY_BIT_RATE,
                ape_ctx.bps * ape_ctx.channels * ape_ctx.samplerate);
    }
    ...
    if (fileoffset < ape_ctx.seektable[0]) {
        ALOGD("raw data smaller than 1 frame, can not playback");
        return;
    }
    if ((mSt_bound > 1) && (fileoffset < ape_ctx.seektable[mSt_bound - 1]))
    {
        int i = 1;
        for (i = 1; i < mSt_bound; i++)
        {
    ...
    mTableOfContents = ape_ctx.seektable;  //record seek table buffer and free it in destructor
    mInitCheck = OK;
