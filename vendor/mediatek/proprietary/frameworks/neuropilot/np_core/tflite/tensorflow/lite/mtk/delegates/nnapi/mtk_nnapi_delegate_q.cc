...
#define LOG_TAG "MtkNnApiDelegateQ"
...
#include "tensorflow/lite/mtk/delegates/nnapi/mtk_nnapi_delegate_kernel.h"
...
class NNAPIOpBuilder {
 public:
  NNAPIOpBuilder(const NnApi* nnapi, TfLiteContext* context,
                 ...
                 ANnModelEncryptor* nn_model_encryptor)
      : nnapi_(nnapi),
        context_(context),
        ...
        nn_model_(nn_model),
        nn_model_encryptor_(nn_model_encryptor) {}
  ...
  /// M: NeuroPilot {@
  TfLiteStatus AddVectorFloat16Operand(const _Float16* values,
                                       uint32_t num_values) {
    return AddVectorOperand<_Float16>(values, num_values,
  ...
  TfLiteStatus AddDequantize(int nn_input_index, int lite_index,
    ...
    if (dequantized_ann_index == -1) {
      ...
      if (context_->encryption_level) {
          RETURN_TFLITE_ERROR_IF_NN_ERROR(
              context_,
              nnapi_->ANnModelEncryption_addOperand(nn_model_encryptor_, &operand_type));
      } else {
          RETURN_TFLITE_ERROR_IF_NN_ERROR(
              context_,
      ...
      if (context_->encryption_level) {
          RETURN_TFLITE_ERROR_IF_NN_ERROR(
              context_, nnapi_->ANnModelEncryption_addOperation(
                            nn_model_encryptor_, ANEURALNETWORKS_DEQUANTIZE, 1,
  ...
  TfLiteStatus FinalizeAddOperation(ANeuralNetworksOperationType type) {
    ...
    if (context_->encryption_level) {
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context_,
            nnapi_->ANnModelEncryption_addOperation(
                nn_model_encryptor_, type, static_cast<uint32_t>(augmented_inputs_.size()),
  ...
  TfLiteStatus AddNewInputConstantTensor(
    ...
    if (context_->encryption_level) {
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context_, nnapi_->ANnModelEncryption_setOperandValue(
                          nn_model_encryptor_, ann_tensor_index, new_tensor->data.raw,
                          new_tensor->bytes));
    } else {
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
 ...
 private:
  ...
  TfLiteStatus AddScalarOperand(T value, int32_t nn_type) {
    ...
    if (context_->encryption_level) {
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context_, nnapi_->ANnModelEncryption_setOperandValue(
                          nn_model_encryptor_, ann_index, &value, sizeof(T)));
    } else {
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context_, nnapi_->ANeuralNetworksModel_setOperandValue(
  ...
  TfLiteStatus AddVectorOperand(const T* values, uint32_t num_values,
    ...
    if (context_->encryption_level) {
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context_, nnapi_->ANnModelEncryption_setOperandValue(
                          nn_model_encryptor_, ann_index, values, sizeof(T) * num_values));
    } else {
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context_, nnapi_->ANeuralNetworksModel_setOperandValue(
  ...
  TfLiteStatus AddTensor(int tensor_index, bool hybrid_op,
    ...
    if (nn_type == ANEURALNETWORKS_TENSOR_QUANT8_SYMM_PER_CHANNEL) {
      if (context_->encryption_level) {
          RETURN_TFLITE_ERROR_IF_NN_ERROR(
              context_,
              nnapi_->ANnModelEncryption_setOperandSymmPerChannelQuantParams(
                  nn_model_encryptor_, ann_tensor_index, &ann_perchannel_params));
      } else {
          RETURN_TFLITE_ERROR_IF_NN_ERROR(
              context_,
    ...
    if (tensor->allocation_type == kTfLiteMmapRo) {
      if (IsQuantized(tensor_type) && need_int8_conversion) {
        ...
        if (allocation_memory_mapping_->count(mmap_alloc) == 0) {
          ...
          if (context_->encryption_level == 0) {
        ...
        if (context_->encryption_level == 0) {
            ...
            RETURN_TFLITE_ERROR_IF_NN_ERROR(
                context_, nnapi_->ANnModelEncryption_setOperandValueFromFd(
                              nn_model_encryptor_, ann_tensor_index, mmap_alloc->fd(),
                              offset, tensor->bytes));
        }
#endif
      } else {
        if (context_->encryption_level) {
            RETURN_TFLITE_ERROR_IF_NN_ERROR(
                context_,
                nnapi_->ANnModelEncryption_setOperandValue(
                    nn_model_encryptor_, ann_tensor_index, tensor->data.raw, tensor->bytes));
        } else {
            RETURN_TFLITE_ERROR_IF_NN_ERROR(
                context_,
  ...
  ANnModelEncryptor* const nn_model_encryptor_;
...
NNAPIDelegateKernel::MappingFn NNAPIDelegateKernel::Map(
    ...
    const TfLiteRegistration* registration, bool is_accelerator_specified) {
    TFLITE_MTK_LOG_INFO("Map OP:%d, version:%d", builtin_code, version);
  switch (builtin_code) {
    case kTfLiteBuiltinAdd:
      if (version <= 2) {
    ...
    case kTfLiteBuiltinTransposeConv:
      if (version == 1 && android_sdk_version >= kMinSdkVersionForNNAPI12) {
        return [](const NNAPIOpMappingArgs& mapping_args)
          ...
          mapping_args.builder->AddTensorInput(mapping_args.node->
                                               inputs->data[/*kDataInputTensor*/ 2],
                                               hybrid_op);
          mapping_args.builder->AddTensorInput(mapping_args.node->
                                               inputs->data[/*kWeightsTensor*/ 1], hybrid_op);
          ...
          mapping_args.builder->AddTensorInput(mapping_args.node->
                                               inputs->data[/*kOutputShapeTensor*/ 0],
                                               hybrid_op);
    ...
    case kTfLiteBuiltinCustom:
      if (registration->custom_name == nullptr) {
        TFLITE_MTK_LOG_ERROR(
          "Empty custom name form a kTfLiteBuiltinCustom OP");
        return nullptr;
      }
      ...
      } else if (strstr(registration->custom_name, "MTK_") == nullptr) {
        TFLITE_MTK_LOG_INFO(
            "Skip non MTK built-in OP: %s", registration->custom_name);
        return nullptr;
      } else {
        ...
        if (strcmp(registration->custom_name, "MTK_DEPTH_TO_SPACE") == 0) {
          ...
          return [](const NNAPIOpMappingArgs& mapping_args)
            ...
            std::vector<CustomOpParam>* params =
                helper.GetParams(mapping_args.registration->custom_name);
            if (params != nullptr) {
              // Add the extra operand to NNAPI
              for (auto i = 0; i < params->size(); i++) {
                  CustomOpParam param = params->at(i);
                  switch (param.type) {
                    case kTfLiteFloat32:
              ...
              TFLITE_MTK_LOG_INFO("No params for %s",
                                  mapping_args.registration->custom_name);
            }
            ...
            uint8_t* oem_scalar = nullptr;
            const char* target =
                helper.FindTarget(mapping_args.registration->custom_name);
            if (target != nullptr) {
...
TfLiteStatus NNAPIDelegateKernel::Init(TfLiteContext* context,
  ...
  if (!nn_model_) {
    ...
    if (context->encryption_level) {
        ANnModelEncryptor* encryptor = nullptr;
        srand(time(NULL));
        uint8_t keys[16] = {};
        for (int i=0; i < 16; i++) {
            keys[i] = rand()%256;
        ...
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context, nnapi_->ANnModelEncryption_create(&encryptor, reinterpret_cast<void *>(model),
                                                 (EncryptionLevel)context->encryption_level, keys));
        RETURN_TFLITE_ERROR_IF_NN_ERROR(
            context, nnapi_->ANnModelEncryption_setDebugFlag(encryptor,
                                                             ::tflite::mtk::IsPrintLogSupported()));
        nn_model_encryptor_.reset(encryptor);
    }
  ...
  if (!nn_compilation_) {
    ...
    std::vector<uint8_t> buffers_for_token;
    if((cache_dir = context->cache_dir)) {
        if(auto buffers = reinterpret_cast<const flatbuffers::Vector<flatbuffers::Offset<Buffer>>*>
           (context->buffers)) {
              ...
              if(buffer) {
                  if(auto* array = buffer->data()) {
                      for(int i=0; i<array->size(); i++) {
                        if(*(array->data() + i)) {
                            buffers_for_token.push_back(*(array->data() + i));
                        } else {
                            buffers_for_token.push_back((uint8_t)1);
                        }
                      }
                  }
              }
            }
            model_token = (const char*)buffers_for_token.data();
        }
    }
...
TfLiteStatus NNAPIDelegateKernel::Invoke(TfLiteContext* context,
  ...
  for (auto absolute_input_index : TfLiteIntArrayView(node->inputs)) {
    ...
    if (tensor->allocation_type != kTfLiteMmapRo) {
      ...
      if (ann_type_equivalent != kTfLiteNoType) {
        const auto num_elements = NumElements(tensor);
        RETURN_TFLITE_ERROR_IF_ION_ERROR(nn_input_memory_->ion_memory_lock());
        ...
        if (tensor->type == kTfLiteUInt8 &&
          ...
          RETURN_TFLITE_ERROR_IF_ION_ERROR(nn_input_memory_->ion_memory_unlock());
          return kTfLiteError;
        }
        RETURN_TFLITE_ERROR_IF_ION_ERROR(nn_input_memory_->ion_memory_unlock());
        size_t type_size;
  ...
  output_offset = 0;
  RETURN_TFLITE_ERROR_IF_ION_ERROR(nn_output_memory_->ion_memory_lock());
  for (auto output_index : TfLiteIntArrayView(node->outputs)) {
    TfLiteTensor* tensor = &context->tensors[output_index];
    if (tensor->buffer_handle != kTfLiteNullBufferHandle) {
      continue;
  ...
  RETURN_TFLITE_ERROR_IF_ION_ERROR(nn_output_memory_->ion_memory_unlock());
...
TfLiteStatus NNAPIDelegateKernel::AddOpsAndTensors(TfLiteContext* context) {
  ...
  for (auto node_index : nodes_) {
    ...
    int nn_op_type = Map(context, reg->builtin_code, reg->version,
                         nnapi_->android_sdk_version, node, reg,
        ...
        {context, &builder, node, &model_state_outputs_,
         ...
         reg, ::tflite::mtk::CustomOpHelper::GetInstance()});
...
TfLiteStatus NNAPIDelegateKernel::BuildGraph(
  ...
  if (context->encryption_level) {
      RETURN_TFLITE_ERROR_IF_NN_ERROR(
          context, nnapi_->ANnModelEncryption_identifyInputsAndOutputs(
                       nn_model_encryptor_.get(), inputs.size(), inputs.data(),
                       outputs.size(), outputs.data()));

      if (nnapi_->android_sdk_version >= kMinSdkVersionForNNAPI11) {
            RETURN_TFLITE_ERROR_IF_NN_ERROR(
                context,
                nnapi_->ANnModelEncryption_relaxComputationFloat32toFloat16(
                    nn_model_encryptor_.get(), context->allow_fp32_relax_to_fp16));
      }

      RETURN_TFLITE_ERROR_IF_NN_ERROR(
          context, nnapi_->ANnModelEncryption_finish(nn_model_encryptor_.get(), true));
  }
...
using ::tflite::delegate::nnapi::mtk::NNAPIDelegateKernel;

MtkStatefulNnApiDelegate::MtkStatefulNnApiDelegate(Options options)
  ...
  TFLITE_LOG_PROD_ONCE(tflite::TFLITE_LOG_INFO,
                       "Created MTK TFLite delegate Q for NNAPI.");
  Prepare = DoPrepare;
  ...
  flags = mtk::GetMtkNnApiDelegateFlag();
}

MtkStatefulNnApiDelegate::MtkStatefulNnApiDelegate()
    : MtkStatefulNnApiDelegate(Options()) {}

const MtkStatefulNnApiDelegate::Options MtkStatefulNnApiDelegate::GetOptions(
    TfLiteDelegate* delegate) {
  auto delegate_data = reinterpret_cast<Data*>(delegate->data_);
  MtkStatefulNnApiDelegate::Options options;
...
const std::vector<MtkStatefulNnApiDelegate::MemoryRegistration>&
MtkStatefulNnApiDelegate::GetTensorMemoryMap(TfLiteDelegate* delegate) {
  auto delegate_data = reinterpret_cast<Data*>(delegate->data_);
  return delegate_data->tensor_memory_map;
}

TfLiteBufferHandle MtkStatefulNnApiDelegate::RegisterNnapiMemory(
...
TfLiteStatus MtkStatefulNnApiDelegate::DoCopyFromBufferHandle(
...
TfLiteStatus MtkStatefulNnApiDelegate::DoCopyToBufferHandle(
    TfLiteContext* context, TfLiteDelegate* delegate,
    TfLiteBufferHandle buffer_handle, TfLiteTensor* tensor) {
  return kTfLiteError;
}

void MtkStatefulNnApiDelegate::DoFreeBufferHandle(TfLiteContext* context,
                                               TfLiteDelegate* delegate,
...
using ::tflite::delegate::nnapi::mtk::kMinSdkVersionForNNAPI;
using ::tflite::delegate::nnapi::mtk::kMinSdkVersionForNNAPI12;

TfLiteStatus MtkStatefulNnApiDelegate::DoPrepare(TfLiteContext* context,
                                              TfLiteDelegate* delegate) {
  ...
  if (nnapi->android_sdk_version >= kMinSdkVersionForNNAPI12) {
    ...
    if (device_name_ptr) {
      ...
      if (device_count <= 1 && !mtk::IsForceUseNnApi()) {
        return kTfLiteOk;
      }
    }
  }
  ...
  for (int node_index : TfLiteIntArrayView(plan)) {
    ...
    if (NNAPIDelegateKernel::Map(context, registration->builtin_code,
                                 ...
                                 node, registration,
                                 is_accelerator_specified)) {
      supported_nodes.push_back(node_index);
    } else {
    /// M: NeuroPilot {@
    context->ReportError(context, "OP(%d):%s is not delegated to NNAPI\n",
...
MtkNnApiDelegateQ::MtkNnApiDelegateQ() {
  delegate_.reset(new MtkStatefulNnApiDelegate());
}

MtkNnApiDelegateQ::~MtkNnApiDelegateQ(void) {
}
