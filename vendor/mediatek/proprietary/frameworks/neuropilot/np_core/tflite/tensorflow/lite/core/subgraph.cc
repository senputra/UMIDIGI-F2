...
void DumpOpOutput(tflite::BuiltinOperator builtin_code,
  ...
  if (tflite::mtk::GetOpDumpPath(&path)) {
    ...
    TFLITE_MTK_LOG_OP_DUMP("%s", kString.c_str());
  }
...
Subgraph::Subgraph(ErrorReporter* error_reporter,
  ...
  context_.cache_dir = nullptr;
  context_.buffers = nullptr;
  context_.encryption_level = 0;
...
TfLiteStatus Subgraph::Invoke() {
  ...
  if (should_apply_nnapi_delegate_ && !applied_nnapi_delegate_) {
    /// M: NeuroPilot {@
    MtkNnApiDelegateBuilder()(&mtk_nnapi_delegate_);
    TfLiteDelegate* delegate = mtk_nnapi_delegate_->GetTFLiteDelegate();
    if (delegate != nullptr) {
      TF_LITE_ENSURE_OK(&context_, ModifyGraphWithDelegate(delegate));
    } else {
