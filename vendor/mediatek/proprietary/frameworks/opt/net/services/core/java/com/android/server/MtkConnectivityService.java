...
public class MtkConnectivityService extends ConnectivityService {
    private static final String TAG = MtkConnectivityService.class.getSimpleName();
    ...
    // M: Ignore MMS request in Tdd data only network mode
    private static final boolean TDD_DATA_ONLY_SUPPORT =
        ((android.os.SystemProperties.get("ro.vendor.mtk_tdd_data_only_support")).equals("1"));
    ...
    private static final SparseArray<String> sMagicDecoderRing = MessageUtils.findMessageNames(
            new Class[] { AsyncChannel.class, MtkConnectivityService.class, NetworkAgent.class,
    ...
    // Disable data usage alert by pushing limit up to 1 TB
    private static final long DISABLE_DATA_ALERT_BYTES = 1024 * 1024 * 1024 * 1024 * 1024L;
    private static final String AUTO_TETHERING_INTENT =
    ...
    private static boolean sIsAutoTethering =
            android.os.SystemProperties.getBoolean("persist.vendor.net.auto.tethering", false);
    // @}

    // M: Support default DNS server for some specific network operators. @{
    ...
    static class LegacyTypeTracker {
        ...
        LegacyTypeTracker(@NonNull MtkConnectivityService service) {
            mService = service;
    ...
    public MtkConnectivityService(Context context, INetworkManagementService netManager,
    ...
    @VisibleForTesting
    protected MtkConnectivityService(Context context, INetworkManagementService netManager,
            INetworkStatsService statsService, INetworkPolicyManager policyManager,
        ...
        // M: Support CCP mode testing  @{
        boolean isCcpMode = mSystemProperties.getBoolean("persist.vendor.op12.ccp.mode", false);
        if (isCcpMode) {
            log("isCcpMode enabled, don't assign default networkRequest");
        // M: Support auto tethering for machine testing
        ...
        mTetheringReceiver = new ConnectivityServiceReceiver();
        mContext.registerReceiver(mTetheringReceiver, filterC);
        // @}

        // M: Support default v4 DNS server.
        String dns = mContext.getResources().getString(
                com.android.internal.R.string.config_default_dns_server);
        try {
            mDefaultDns = NetworkUtils.numericToInetAddress(dns);
            if (VDBG) log("mDefaultDns:" + mDefaultDns);
        ...
        ///M: Disable captive_portal_use_https as it is not support in China 1/2 @{
        disableCaptivePortalUseHttps(context);
        /// @}
        ///M: [CDR-CDS-183] IPv6 Addressing -- Cellulor 1/2 @{
        ipv6CellulorInit();
        /// @}

        //  M: Support Tethering add-on mechanism @{
        mMtkTethering = new MtkTethering(mContext, mTethering);
        // @}
    }
    ...
    // M: Support add-on extension class
    @Override
    ...
    public NetworkRequest requestNetwork(NetworkCapabilities networkCapabilities,
        ...
        // M: If TDD Data only feature enable, request by China Telecom to ignore mms request.
        if (ignoreMmsReqInTddOnly(networkCapabilities)) {
            if (DBG) log("requestNetwork: ignore mms request in TDD Data only network mode");
            return null;
        }
...
//        }
    ...
    private void updateDnses(LinkProperties newLp, LinkProperties oldLp, int netId) {
        ...
        /* M: append default dns to avoid no DNS servers issues */
        try {
        ...
        /** M: [ALPS04115651]ipv6 provisioned network, but provide ipv4+ipv6 dnses
         **    giving ipv6 dns higher query priority @{*/
        if (newLp.isIpv6Provisioned()) {
            ...
            newLp.setDnsServers(sortedDnses);
        }
        // @}
    ...
    private NetworkCapabilities mixInCapabilities(NetworkAgentInfo nai, NetworkCapabilities nc) {
        ...
        if (nai.everConnected &&
            ...
            if (!TextUtils.isEmpty(diff)) {
                Slog.e(TAG, "BUG: " + nai + " lost immutable capabilities:" + diff);
            }
        }
    ...
    private void makeDefault(NetworkAgentInfo newNetwork) {
        ...
        /// M: ALPS05026776 fix AOSP proxy change timing issue
        mProxyTracker.sendProxyBroadcast();
    ...
    private void updateNetworkInfo(NetworkAgentInfo networkAgent, NetworkInfo newInfo,
                                   /** M: ALPS04654002. Add a new argument @isNetCreated to identify
                                    * the new NetworkAgent in place of setting nai.networkIfo = null
                                    * temporarily which will indroduce timing issue @{ 3/5
                                    */
    ...
    /**
     * M: Support Auto tethering function when power up
     *
     */
    private class ConnectivityServiceReceiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ...
            if (AUTO_TETHERING_INTENT.equals(action)) {
                ...
                if (sIsAutoTethering) {
                    ...
                    if (isTetheringSupported()) {
                        mTethering.setUsbTethering(true);
                    }
                    // Wait for 3 seconds
                    mHandler.sendMessageDelayed(mHandler.obtainMessage(
                            EVENT_ENABLE_MOBILE_DATA_FOR_TETHERING), 3000);
                }
            } else if (ACTION_DISABLE_DATA_ALERT.equals(action)) {
                Thread thread = new Thread("IGNORE_DATA_USAGE_ALERT") {
                    public void run() {
                        Slog.d(TAG, "Disable data usage alert");
    ...
    /**
     * M: This API is only for TDD data only to ingnore MMS request.
     *
     * @param nc The network request capabilities.
     */
    private boolean ignoreMmsReqInTddOnly(NetworkCapabilities nc) {
        boolean ignore = false;
        if (TDD_DATA_ONLY_SUPPORT) {
            if (nc.hasCapability(NetworkCapabilities.NET_CAPABILITY_MMS)
                ...
                try {
                    android.net.StringNetworkSpecifier ns = (android.net.StringNetworkSpecifier)
                            (nc.getNetworkSpecifier());
                    if (ns != null) {
                ...
                com.mediatek.internal.telephony.MtkLteDataOnlyController ldoc =
                        new com.mediatek.internal.telephony.MtkLteDataOnlyController(mContext);
                if (subId != android.telephony.SubscriptionManager.INVALID_SUBSCRIPTION_ID) {
                    if (!ldoc.checkPermission(subId)) {
                        ignore = true;
                    }
                } else {
                    if (!ldoc.checkPermission()) {
                        ignore = true;
                    }
                }
            }
    ...
    /**
     * M: Add default DNS server for network connection with Internet capabilty
     *
     * @param netId The network id of network connection
     * @param lp The new link properties of this network
     * @return The default DNS server to add
     */
    private InetAddress addDefaultDnsServer(int netId, LinkProperties lp) {
        ...
        if (nai == null || lp == null || mDefaultDns == null || dnses.contains(mDefaultDns) ||
                !nai.networkCapabilities.hasCapability(
                        NetworkCapabilities.NET_CAPABILITY_INTERNET)) {
            return null;
        }

        // Add default DNS only when the original DNS list is empty to avoid the
        // dns priority reverses when private DNS is enabled but the original dns
        // not support private DNS
        if (!dnses.isEmpty()) {
            log("Not append default DNS for original DNSes not empy:" + dnses);
            return null;
        }

        if (isDefaultDnsEnabled && nai.networkCapabilities.hasTransport(
                    NetworkCapabilities.TRANSPORT_CELLULAR)) {
            isDefaultDnsEnabled = !(TelephonyManager.getDefault().getSimCountryIso().
                            equalsIgnoreCase(java.util.Locale.US.getCountry()));
            log("isDefaultNetwork:" + TelephonyManager.getDefault().getSimCountryIso()
                    + ";" + java.util.Locale.US.getCountry());
        }
        ...
        if (isDefaultDnsEnabled) {
            ...
            if (mDefaultDns instanceof Inet4Address && hadIPv4) {
                ...
                return mDefaultDns;
            } else if (mDefaultDns instanceof java.net.Inet6Address && hadIPv6) {
    ...
    /// M: judge modem machine testing is running or not.
    private boolean isTestSim() {
        return mSystemProperties.get("vendor.gsm.sim.ril.testsim").equals("1") ||
                   mSystemProperties.get("vendor.gsm.sim.ril.testsim.2").equals("1") ||
                   mSystemProperties.get("vendor.gsm.sim.ril.testsim.3").equals("1") ||
                   mSystemProperties.get("vendor.gsm.sim.ril.testsim.4").equals("1");
    }
    ...
    public IBinder startOrGetTestNetworkService() {
        /// M: ALPS04699198 TestNetworkService.enforceTestNetworkPermissions cannot
        //  be accessed by methods defined in mediatek-framework-net.jar. @{
        /** AOSP origin code
        ...
        */
        return startOrGetTestNetworkService(mContext, mNMS);
        /// @}
    }

    ///M: [CDR-CDS-183] IPv6 Addressing -- Cellulor 2/2 @{
    private static final String ACTION_OPERATOR_CONFIG_CHANGED =
            "com.mediatek.common.carrierexpress.operator_config_changed";
    private static final boolean USP_SUPPORTED
            = !(android.os.SystemProperties
                    .get("ro.vendor.mtk_carrierexpress_pack", "no").equals("no"));
    private static final String SYSTEM_PROPERTY_OPTR = "persist.vendor.operator.optr";
    // Only support single sim project.
    private static final int MAX_CELLULAR_INTERFACE = 5;

    private void ipv6CellulorInit() {
        boolean isOp07 = mSystemProperties.get(SYSTEM_PROPERTY_OPTR).equals("OP07");
        if (DBG) {
        ...
        if (USP_SUPPORTED) {
            ...
            mContext.registerReceiver(new SimOperatorReceiver(), operatorIntentFilter);
            if (isOp07) {
    ...
    private void enableIpv6PrivacyCellulor(boolean enable) {
        log("enableIpv6PrivacyCellulor: " + enable);
        int mode = enable ? INetd.IPV6_ADDR_GEN_MODE_STABLE_PRIVACY
                : INetd.IPV6_ADDR_GEN_MODE_EUI64;
        for (int i=0; i<=MAX_CELLULAR_INTERFACE; i++) {
            try {
                mNetd.setIPv6AddrGenMode("ccmni"+i, mode);
    ...
    private class SimOperatorReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_OPERATOR_CONFIG_CHANGED.equals(action)) {
                handleOperatorConfigChanged();
            }
        }

        private void handleOperatorConfigChanged() {
            String operator = mSystemProperties.get(SYSTEM_PROPERTY_OPTR);
            if (DBG) {
                log("operator=" + operator);
            }
            enableIpv6PrivacyCellulor("OP07".equals(operator));
        }
    }
    /// 2/2 @}

    ///M: disable captive_portal_use_https by default 2/2 @{
    ...
    private void disableCaptivePortalUseHttps(Context context) {
        String useHttps = DeviceConfig.getProperty(
                DeviceConfig.NAMESPACE_CONNECTIVITY, NAME_USE_HTTPS);
        Log.d(TAG, "Use https=" + useHttps);
        if (useHttps == null) {
            DeviceConfig.setProperty(DeviceConfig.NAMESPACE_CONNECTIVITY, NAME_USE_HTTPS,
                    "0", false);
