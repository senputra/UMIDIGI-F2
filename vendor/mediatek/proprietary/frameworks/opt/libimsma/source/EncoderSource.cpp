...
struct EncoderSource::Rotator : public AHandler {
    Rotator(int32_t multiId,const sp<ALooper> &looper);
...
EncoderSource::Rotator::Rotator(int32_t multiId,const sp<ALooper> &looper)
    : mMultiInstanceID(multiId),
      ...
      mStop(false)
{
    VT_LOGI("[ID=%d][Rotator]create+++ always use FakeRotate",mMultiInstanceID);
    ...
    mSource(source)
{
    ...
    mRotatorLooper->setName("rotate_looper");
        mRotator = new Rotator(mMultiInstanceID,mRotatorLooper);
