    ...
    */
    ...
    public boolean isDataAllowed(ApnContext apnContext, @RequestNetworkType int requestType,
        ...
        if (isDataRetryRestrictEnabled()) {
            ...
            if (apnContextDefault != null && apnContext != null) {
                ApnSetting apnSettingDefault = apnContextDefault.getApnSetting();
                if (apnSettingDefault != null && apnSettingDefault.getPermanentFailed()
                        && apnSettingDefault.canHandleType(apnContext.getApnTypeBitmask())) {
        ...
        if (isDataAllowed) {
            ...
            if (apnContext.getState() == DctConstants.State.RETRYING) {
                ...
                if (dataConnectionReasons.contains(
                            DataDisallowedReasonType.MTK_DATA_RETRY_NOT_ALLOWED)) {
                    str.append(" Keep retrying.");
                } else {
                /// @}
    ...
    protected boolean cleanUpAllConnectionsInternal(boolean detach, String reason) {
        ...
        for (ApnContext apnContext : mApnContexts.values()) {
            ...
            if (reason.equals(Phone.REASON_RADIO_TURNED_OFF)
                    && apnContext.getApnType().equals(PhoneConstants.APN_TYPE_IMS)) {
                log("cleanUpAllConnectionsInternal: Ignore ims for apnType = "
        ...
        if ( imsConnect &&  mDisconnectPendingCount == 0) {
            if (DBG) log("cleanUpAllConnectionsInternal: legacy ims only connect, power off radio immediately");
    ...
    protected void onDisconnectDone(ApnContext apnContext) {
        ...
        if (mDisconnectPendingCount == 0) {
            for (ApnContext apncontext : mApnContexts.values()) {
                if (apncontext.getApnType().equals(PhoneConstants.APN_TYPE_IMS)
                        && apncontext.getState() == DctConstants.State.CONNECTED) {
                    if (DBG) log("all data disconnect except ims, power off radio immediately");
                    mPhone.getServiceStateTracker().processPendingRadioPowerOffAfterDataOff();
                    break;
                }
            }
            ...
            case DctConstants.EVENT_TRY_SETUP_DATA:
                ...
                if (isDataRetryRestrictEnabled()) {
                    ...
                    if (apnSetting != null && apnSetting.getPermanentFailed()) {
                        apnSetting.setPermanentFailed(false);
                        log("set permanentFailed as false for cc33 operators");
                    }
                }
                /// @}
    ...
    private DataRetryOperator getDataRetryOperator() {
        ...
        if (operator != null) {
            ...
            for (int i = 0; i < KDDI_OPERATOR.length; i++) {
                if (operator.startsWith(KDDI_OPERATOR[i])) {
                    return DataRetryOperator.KDDI;
                }
            }
        }
    ...
    private boolean isDataRetryRestrictEnabled() {
        return (mDataRetryOperator == DataRetryOperator.TELCEL
                ...
                || mDataRetryOperator == DataRetryOperator.KDDI);
    }
