...
public class MtkImsPhoneCallTracker extends ImsPhoneCallTracker implements ImsPullCall {
        ...
        public void onTextCapabilityChanged(ImsCall call, int localCapability,
            ...
            if(localCapability == 1 && remoteCapability == 1) {
                AudioManager audioManager =
                        (AudioManager) mPhone.getContext().getSystemService(
                            Context.AUDIO_SERVICE);
                audioManager.setParameters("ims_rtt_capability=1");
                log("onTextCapabilityChanged: set audio RTT capability");
            }
        }
    ...
    protected void updateForSrvccCompleted() {
        ...
        // ALPS05103421 SRVCC during swap call, should reset the flag,
        // or next time add call will fail
        if (mHoldSwitchingState != HoldSwapState.INACTIVE) {
    ...
    protected void checkVoiceCapabilityForRttCallType(ImsFeature.Capabilities capability) {
        ...
        if (mImsRttCallType == IMS_RTT_CALL_TYPE_CS && !isVoiceCapable) {
            AudioManager audioManager =
                (AudioManager) mPhone.getContext().getSystemService(Context.AUDIO_SERVICE);
