...
public class RttEmcGuardTimerUtil {
    ...
    private void unregisterRttReceiver() {
        /*M:  unregister only if reciever is registered,
        if registered mIsRegisteredReceiver will always be true*/
    ...
    public void startRttEmcGuardTimer() {
        ...
        //M: destroy any existeing reciever
        stopRttEmcGuardTimer();
        // register new reciever after destroying old one
        if (isRttEmcCarrierSupport() && !mIsRegisteredReceiver) {
            Rlog.d(TAG, "initRttEmcGuardTimer , mIsRegisteredReceiver :" + mIsRegisteredReceiver);
            mAlarmManager = (AlarmManager) mContext.getSystemService(Context.ALARM_SERVICE);
            registerRttReceiver();
            mIsRegisteredReceiver  = true;
        } else {
             Rlog.e(TAG, "initRttEmcGuardTimer , mIsRttEmcGuardTimerSupported :" +
                     mIsRttEmcGuardTimerSupported);
             return;
        }
    ...
    //M: checking rttEmcSupport for carrier
    private boolean isRttEmcCarrierSupport() {
        // Setting config manager to use carrier config values
        ...
        // get carrier config with default values
        ...
        // get guard timer support from carrier config manager
        mIsRttEmcGuardTimerSupported = carrierConfig.getBoolean
                (MtkCarrierConfigManager.MTK_KEY_EMC_RTT_GUARD_TIMER_BOOL);
        return mIsRttEmcGuardTimerSupported;
    }
}