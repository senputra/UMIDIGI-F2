...
android::AudioFtmBaseVirtual *gAudioFtmBase;
static void *AudioHwhndl_factory = NULL;
bool wavePlaybackExit = 0;
...
int Common_Audio_init(void)
{
    ...
    if (gAudioHardware == NULL||AudioHwhndl_factory==NULL)
    {
        ...
        else
        {
            ...
            if (func1 == NULL)
            {
                ALOGD("-Common_Audio_init dlsym createMTKAudioHardware fail");
        ...
        if (gAudioFtmBase == NULL)
        {
            create_AudioFtm* func2 = (create_AudioFtm*)dlsym(AudioHwhndl_factory, "pfuncGetAudioFtmByDL");
            ALOGD("%s %d func2 %p",__FUNCTION__,__LINE__,func2);

            if (func2 == NULL)
            {
                ALOGD("-Common_Audio_init dlsym pfuncGetAudioFtmByDL fail");
                dlclose(AudioHwhndl_factory);
                AudioHwhndl_factory = NULL;
                return false;
            }
            ...
            gAudioFtmBase = func2();
            ALOGD("%s %d gAudioFtm %p",__FUNCTION__,__LINE__,gAudioFtmBase);
        }
    }
#endif
    return true;
...
{
    return gAudioFtmBase->HeadsetMic_Receiver_Loopback(bEnable, bHeadsetMic);
    return 0;
}
...
//-------------------------------------------------------------------
//FUNCTION:
//      SpecificBuildInMicTest
//DESCRIPTION:
//      this function is called to test different buildinmic scenario.
//
//PARAMETERS:
//      type:    [IN]    (char)0 mean default, others set specific BuildInMic
//
//RETURN VALUE:
//      TRUE is success, otherwise is fail
//
...
//      None
//-------------------------------------------------------------------
int SpecificBuildInMicTest(char type)
{
    return gAudioFtmBase->SpecificBuildInMicTest(type);
}
...
bool SpeakerCalibration(int calibStage)
{
    return gAudioFtmBase->SpeakerCalibration(calibStage);
}
...
static void *Audio_Wave_Playabck_routine(void *arg)
{
    ...
    {
        ALOGE("%s(), pWavePlaydata open file fail: %s",
              __FUNCTION__, pWavePlaydata->FileName);
    ...
    // read wave header
    int read_size = 0;
    read_size = fread((void *)&pWavePlaydata->mWaveHeader,
                      WAVE_HEADER_SIZE, 1, pWavePlaydata->pFile);
    ALOGD("%s(), file name: %s, BitsPerSample: %d, Channels: %d, SampleRate: %d",
          __FUNCTION__, pWavePlaydata->FileName,
          pWavePlaydata->mWaveHeader.BitsPerSample,
          pWavePlaydata->mWaveHeader.NumChannels, pWavePlaydata->mWaveHeader.SampleRate);
  ...
  //create output stream
    AudioMTKStreamOutInterface *streamOutput = gAudioHardware->openOutputStream(0, &format, &channels, &sampleRate, &status);
...
int Audio_Wave_playback(void *arg)
{
    ...
    if (pWavePlaydata->WavePlayThread == (pthread_t) NULL)
    {
        ...
        ALOGD("pthread_create WavePlayThread");
...
void SetRecordEnhance(bool bEnable)
{
    android::AudioParameter param = android::AudioParameter();
    if(!bEnable)
    {
#ifndef MTK_BASIC_PACKAGE
        param.addInt(android::String8(android::AudioParameter::keyInputSource), AUDIO_SOURCE_VOICE_UNLOCK);
        ALOGD("SetRecordEnhance: Input Source %d", AUDIO_SOURCE_VOICE_UNLOCK);
    ...
    else
    {
        ...
        ALOGD("SetRecordEnhance: Input Source %d", AUDIO_SOURCE_MIC);
        gAudioStreamIn->setParameters(param.toString());
    }
}
...
bool recordInit(int device_in, uint32_t *sample_rate)
{
    if (gAudioStreamIn == NULL)
    {
        ...
        if (device_in == MATV_I2S)
        {
            ...
            param.addInt(android::String8(android::AudioParameter::keyInputSource), AUDIO_SOURCE_MATV);
        }
...
#endif
        {
            ...
            param.addInt(android::String8(android::AudioParameter::keyInputSource), AUDIO_SOURCE_VOICE_RECOGNITION);
        }
...
int readRecordData(void *pbuffer, int bytes)
{
    ...
    while (bytes > 0) {
        nBytes = gAudioStreamIn->read(((char *)pbuffer) + nTotalBytes, bytes);
        bytes -= nBytes;
        nTotalBytes += nBytes;
    }
...
const unsigned int AUD_VIBRFACTORY_FILTER_COEF_Table[VIBSPK_FILTER_NUM][2][6][3] =
{
...
#endif
static    AudioMTKStreamOutInterface *vibspkStreamOutput = NULL;
    ...
    ********************************************/
    ...
    while (1)
    {
        ...
        if (counter == TONE_FRAME_COUNT)
        {
            ...
            if (adc_value < (int32_t)adc_min_vol)
            {
            ...
            if (freq_coarse_done == true && center_freq > (int32_t)freq_fine_max)
            {
                break;
            }
        }
...
extern "C" {
    ...
    int HeadsetMic_Receiver_Loopback(char bEnable, char bHeadsetMic) { return true; }
    ...
    bool SpeakerCalibration(int calibStage) { return false; }
