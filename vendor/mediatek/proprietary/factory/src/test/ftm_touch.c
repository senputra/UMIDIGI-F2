...
pthread_mutex_t enter_mutex = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t event_mutex = PTHREAD_MUTEX_INITIALIZER;
#ifdef FEATURE_FTM_TOUCH_MODE
#define touch_exit_notice1       "Çë¼ÌÐøµã»÷ÆÁÄ»»®Ïß!"
#define touch_exit_notice2       "Çë¼ÌÐøµã»÷ÆÁÄ»ÍË³ö!"
#define touch_exit_notice3       "                   "
#endif
...
#ifdef FEATURE_FTM_TOUCH_MODE
static int timer_flag1 = -1;
static int touch_timeout_flag;
static struct itimerval v = {0};
#endif
...
/*data size was defined by y_max/TOUCH_L1, set max resoluation is 2160*4096*/
struct detect_date {
	int count_up[105];
	int count_left[105];
	int count_down[105];
	int count_right[105];
	int count_mid_upright[105];
	int count_mid_transverse[105];
	int count_oblique1[105];
	int count_oblique2[105];
};
...
int save()
{
    ...
    for (i = 0; i < 8; i++) {
        if(EOF == fscanf(file, "%d,",&(def[i]))) {
		LOGE(TAG "fscanf EOF happened, i=%d\n", i);
		break;
	}
    }
    fclose(file);
...
#ifdef FEATURE_FTM_TOUCH_MODE
void touch_show_continue_function(void);
#endif
static struct touch *tpd;
static void *touch_update_thread(void *priv) {
    tpd = (struct touch *)priv;
    struct input_event data;
    int x=0,y=0,p=0,px=0,py=0,d=0;
    ...
    while (1) {
        ...
        if (tpd->scene==ITEM_TOUCH_CHOOSER) {
            ...
            if (ev_head_num == ev_tail_num)  {
				#ifdef FEATURE_FTM_TOUCH_MODE
				if(timer_flag1 == 0){
					signal(SIGALRM, touch_show_continue_function);
                    setitimer(ITIMER_REAL, &v, 0);
                    timer_flag1 = 1;
					ui_color(0, 0, 0, 255);
					ui_text(tpd->width/10,tpd->height/2,touch_exit_notice1);
					ui_flip();
				}else if(touch_timeout_flag){
					ui_color(0, 0, 0, 255);
					ui_text(tpd->width/10,tpd->height/2,touch_exit_notice1);
					ui_color(255, 255, 255, 255);
					ui_text(tpd->width/10,tpd->height/2,touch_exit_notice2);
					ui_flip();
				}
				#endif
				continue;
...
#ifdef FEATURE_FTM_TOUCH_MODE
static int paintview_key_handler1(int key, void *priv)
{
	int handled = 0;
	struct paintview *pv;
	if(priv == NULL)
	...
	if(touch_timeout_flag){
		touch_timeout_flag = 0;
		pv->exit(pv);
	}
...
int touch_entry(struct ftm_param *param, void *priv) {
    ...
    tpd->exit_thd = false;  
	#ifdef FEATURE_FTM_TOUCH_MODE
    ui_init_paintview(&(tpd->empty), paintview_key_handler1, &(tpd->empty));
	#else
    ...
    #ifdef FEATURE_FTM_TOUCH_MODE
    ui_init_paintview(&(tpd->zoom), paintview_key_handler1, &(tpd->zoom));
    ui_init_paintview(&(tpd->freemode), paintview_key_handler1, &(tpd->freemode));
	#else
	...
	tpd->menu.set_btn(&(tpd->menu), &lbtn, &cbtn, &rbtn);
	#endif
...
#ifdef FEATURE_FTM_TOUCH_MODE
void touch_show_exit_function(void)
{
	touch_timeout_flag = 1;
}
void touch_show_continue_function(void)
{
	ui_color(255,0,0,255);
	ui_text(tpd->width/10,tpd->height/2,touch_exit_notice1);
	ui_flip();
    signal(SIGALRM, touch_show_exit_function);
	setitimer(ITIMER_REAL, &v, 0);
}
#endif
int touch_init(void)
