...
static MINT32 camera_preview_test()
{
    ...
    {
        rawVal = ftm_get_prop("FTMCameraTest"); // 0: Test Pattern mode , 1 : Normal capture & save jpeg
        if (rawVal != NULL)
        {
            if (atoi(rawVal) == 0)
            {
                         ...
                         AnaImage* I)
{
    ...
    nread = fread(rawTmp, sizeof(MUINT16), size << 2, pFp);
                         ...
                         AnaImage* I)
{
    ...
    switch(format)
    {
        ...
        case YUVFmt_YVYU : sprintf(szFileName, "%s/YUVImg_YVYU.yuv" , MEDIA_PATH);
            break;
        default: sprintf(szFileName, "%s/YUVImg_Unkonw.yuv" , MEDIA_PATH);
            break;
    }
    ...
    nread = fread(yuvTmp , sizeof(MUINT8), size << 2 , pFp);
                           ...
                           MUINT32 inShutter)
{
    ...
    double breakTH = 0;
    double midp = 0, sgain = 1, gmin = 0, gmax = 0;
    MBOOL *breakPoint;
    MBOOL flag = MFALSE;
    ...
    if (dataRange)
        ...
        FTM_CAMERA_ERR("dataRange is 0\n");

    if(startValue > breakTH && (abs((int)((rangeAll>>1) - dataRange)) < 3 || abs((int)( (rangeAll >> 2) - dataRange)) < 3))      // bit-10 error detection
    {
        flag = MTRUE;
        gPinErrType = 31;
    }
    ...
    if(gPinError_pass)
    {
        ...
        if(end < minEnd )
        {
            ...
            if (end)
                ...
                FTM_CAMERA_ERR("div 0 end\n");
            if (start)
                gmax = maxStart / start;
            else
                FTM_CAMERA_ERR("div 0 start\n");
...
*!  brief for Check CRC
********************************************************************************/
static MINT32 Camera_check_crc(MUINT8 *out_buffer, MUINT32 crc, MUINT32 size, const MUINT32 buf_xsize_byte, const MUINT32 buf_stride_byte)
{
    MUINT32 crc_accum = 0;

    FTM_CAMERA_DBG("[Camera_check_crc] xsize(%d) stride(%d) size(%d)\n", buf_xsize_byte, buf_stride_byte, size);
...
static void vAutoCapCb(void *a_pParam)
{
    ...
    if(pImgBufInfo->eImgType == PURE_RAW10_TYPE)
    {
        ...
        FTM_CAMERA_DBG("[vAutoCapCb] addr(%p),size(%u)\n",pImgBufInfo->RAWImgBufInfo.bufAddr, pImgBufInfo->RAWImgBufInfo.imgSize);
        ...
        if(NULL == pFp)
        {
            ...
            FTM_CAMERA_ERR("[vAutoCapCb] capture fail\n");
            gCapCBDone = MTRUE;
            return;
        }
    ...
    else if(pImgBufInfo->eImgType == YUV_TYPE)
    {
        ...
        FTM_CAMERA_DBG("[vAutoCapCb] addr(%p),size(%u)\n",pImgBufInfo->imgBufInfo.bufAddr, pImgBufInfo->imgBufInfo.imgSize);
...
static void vAutoCapCb_patg(void *a_pParam)
{
    ...
    if(pImgBufInfo->eImgType == PURE_RAW10_TYPE)
    {
        ...
        if((gPinError_pass == MFALSE) || u32ATA_Mode)
        {
            ...
            if(NULL == pFp)
            {
                FTM_CAMERA_ERR("[vAutoCapCb_patg] Can't open file to save RAW image");
                FTM_CAMERA_ERR("[vAutoCapCb_patg] capture fail\n");
                gCapCBDone = MTRUE;
                return;
            }
            ...
            if(0 != fsync(fileno(pFp)))
            {
                FTM_CAMERA_ERR("[vAutoCapCb_patg] RAW fync fail");
                FTM_CAMERA_ERR("[vAutoCapCb_patg] capture fail\n");
                gCapCBDone = MTRUE;
                fclose(pFp);
                return;
    ...
    else if(pImgBufInfo->eImgType == YUV_TYPE)
    {
        ...
        if((gPinError_pass == MFALSE) || u32ATA_Mode)
        {
            ...
            if(NULL == pFp)
            {
                FTM_CAMERA_ERR("[vAutoCapCb_patg] Can't open file to save YUV image");
                FTM_CAMERA_ERR("[vAutoCapCb_patg] capture fail\n");
                gCapCBDone = MTRUE;
                return;
            }
            ...
            if(0 != fsync(fileno(pFp)))
            {
                FTM_CAMERA_ERR("[vAutoCapCb_patg] YUV fync fail");
                FTM_CAMERA_ERR("[vAutoCapCb_patg] capture fail\n");
                gCapCBDone = MTRUE;
                fclose(pFp);
                return;
...
static int camera_capture_autotest_patg()
{
    ...
    {
        rACDKCapConfig.eOutputFormat = JPEG_RAW10_TYPE;
    }
    }
    else
    {
...
#ifndef FTM_CAMERA_POSTVIEW_SKIP_REDRAW
    tv->redraw(tv);
#endif
    cam->exit_thd = MFALSE;
...
//    PFLASH_LIGHT_FUNCTION_STRUCT pstFlash;
    ...
    property_get("vendor.camera.acdk.manual", rawVal, "0");  // 1: Manual mode, 2 : auto mode
    if(atoi(rawVal) == 1)
    {
        gcamera_autoTest = 0;
    }
    ...
    if(gcamera_autoTest)
    {
        ...
        if(isTestDone == 1)
        {
            char *rawVal_ata = NULL;
            rawVal_ata = ftm_get_prop("FTMCameraTest"); // 0: Test Pattern mode , 1 : Normal capture & save jpeg
            if (rawVal_ata != NULL && atoi(rawVal_ata) == 1)
            {
                FTM_CAMERA_DBG("ATA3.0 Auto-Test Pass\n");
                cam->mod->test_result = FTM_TEST_PASS;
            }
            else
            {
...
*
********************************************************************************/
int camera_main3_preview_entry(struct ftm_param *param, void *priv)
{
    srcDev = 0x20; //main3 sensor
    return camera_entry(param,priv);
}
...
*
********************************************************************************/
int camera_main4_preview_entry(struct ftm_param *param, void *priv)
{
    srcDev = 0x40; //main4 sensor
    return camera_entry(param,priv);
}
...
*
********************************************************************************/
int camera_sub2_preview_entry(struct ftm_param *param, void *priv)
{
    srcDev = 0x10; //sub2 sensor
    return camera_entry(param,priv);
}
...
*
********************************************************************************/
extern "C" int camera_main3_init(void)
{
    int ret = 0;
    struct ftm_module *mod;
    struct camera *cam;
    ...
    mod = ftm_alloc(ITEM_MAIN3_CAMERA, sizeof(struct camera));
    cam  = mod_to_camera(mod);
    ...
    ret = ftm_register(mod, camera_main3_preview_entry, (void*)cam);
    return ret;
}
...
*
********************************************************************************/
extern "C" int camera_main4_init(void)
{
    int ret = 0;
    struct ftm_module *mod;
    struct camera *cam;
    ...
    mod = ftm_alloc(ITEM_MAIN4_CAMERA, sizeof(struct camera));
    cam  = mod_to_camera(mod);
    ...
    ret = ftm_register(mod, camera_main4_preview_entry, (void*)cam);
    return ret;
}
...
*
********************************************************************************/
extern "C" int camera_sub2_init(void)
{
    int ret = 0;
    struct ftm_module *mod;
    struct camera *cam;
    ...
    mod = ftm_alloc(ITEM_SUB2_CAMERA, sizeof(struct camera));
    cam  = mod_to_camera(mod);
    ...
    ret = ftm_register(mod, camera_sub2_preview_entry, (void*)cam);
    return ret;
}
