							...
							u_register_t flags)
{
	...
	if (caller_security_state == SECURE) {
		...
		assert(handle == CM_GET_CONTEXT(SECURE));
			...
			case T_SET_DRM_INT: {
#if MICROTRUST_LOG
				DBG_PRINTF("T_SET_DRM_INT: set fiq '%d'\n", (uint32_t)x1);
#endif
			...
			case T_CLR_DRM_INT: {
#if MICROTRUST_LOG
				DBG_PRINTF("T_CLR_DRM_INT: clear fiq '%d'\n", (uint32_t)x1);
#endif
...
#if MICROTRUST_LOG
				DBG_PRINTF("T_SW_GENERATE_INT: trigger interrupt '%d'\n", (uint32_t)x1);
#endif
			...
			case T_GET_RAND_SEED: {
				x1 = teei_rng();
#if MICROTRUST_LOG
				DBG_PRINTF("T_GET_RAND_SEED rand [%lx]\n\r", x1);
#endif
...
#if MICROTRUST_LOG
				UART_INIT;
				DBG_PRINTF("N_GET_SE_OS_STATE [0x%x]\n\r",
