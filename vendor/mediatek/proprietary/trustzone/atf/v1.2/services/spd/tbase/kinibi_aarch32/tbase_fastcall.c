 ...
 *   * Disable FIQs, if occurring when tbase is not ok to handle them.
 */
...
/* extern void mt_atf_trigger_WDT_FIQ(void); */

/*************************************************************************************/
/* Common setup for normal fastcalls and fastcalls to tbase */

static void tbase_setup_entry_common(cpu_context_t *s_context, cpu_context_t *ns_context, uint32_t call_offset)
{
	/* Set up registers */
...
void tbase_setup_entry_monitor(cpu_context_t *ns_context)
{
	...
	/* Offset into registerFile */
	uint64_t registerOffset = page_align(registerFileEnd[REGISTER_FILE_NWD] - registerFileStart[REGISTER_FILE_NWD],
		 UP) + (uint64_t)registerAddress - registerFileStart[REGISTER_FILE_MONITOR];
 ...
 * Returns 0 in case of successful call, and non-zero in case of error.
		...
		fc_response_t *resp)
{
	...
	if ((tbaseExecutionStatus&TBASE_STATUS_FASTCALL_OK_BIT) == 0) {
		...
		DBG_PRINTF("mon_fc FASTCALL_NOT_OK\n\r");
		return 1;
	}
	if (tbase_ctx->state == TBASE_STATE_OFF) {
		DBG_PRINTF("mon_fc TBASE_STATE_OFF\n\r");
		return 1;
	}
...
void tbase_fiqforward_configure(uint32_t intrNo, uint32_t enable)
{
	...
	DBG_PRINTF("%s: fwd-int %d is %s\n\r",
			__func__,
			intrNo,
			(enable == TBASE_FLAG_SET)?"on":"off");
}
		...
		uint64_t flags)
{
	...
	if (caller_security_state == SECURE) {
		...
		case TBASE_SMC_FASTCALL_RETURN:
		{
			...
			DBG_PRINTF("fc_h TBASE_SMC_FASTCALL_RETURN\n\r");
			tbase_synchronous_sp_exit(tbase_ctx, 0, 1);
			break;
		}
		case TBASE_SMC_FASTCALL_CONFIG_OK:
		{
			DBG_PRINTF("fc_h TBASE_SMC_FASTCALL_CONFIG_OK\n\r");
		...
		{
			DBG_PRINTF("fc_h TBASE_SMC_FASTCALL_STATUS\n\r");
			tbase_status(x1, x2);
			SMC_RET1(handle, smc_fid);
			break;
		}
		...
		{
			DBG_PRINTF("fc_h TBASE_SMC_FASTCALL_INPUT\n\r");
			smc_fid = plat_tbase_input(x1, &x2, &(tbase_ctx->tbase_input_fastcall));
			SMC_RET3(handle, smc_fid, page_align(registerFileEnd[REGISTER_FILE_NWD]
				 - registerFileStart[REGISTER_FILE_NWD], UP)
				 + (uint64_t)&(tbase_ctx->tbase_input_fastcall)
				 - registerFileStart[REGISTER_FILE_MONITOR], x2);
			break;
		}
		case TBASE_SMC_FASTCALL_DUMP:
		{
			DBG_PRINTF("fc_h TBASE_SMC_FASTCALL_DUMP\n\r");
			/*tbase_PlatformDump(); */
			SMC_RET1(handle, smc_fid);
			break;
		}
		...
		{
			DBG_PRINTF("fc_h TBASE_SMC_FASTCALL_FORWARD_FIQ\n\r");
		...
		{
			/* What now? */
			DBG_PRINTF("fc_h SMC_UNK %x\n\r", smc_fid);
			SMC_RET1(handle, SMC_UNK);
			break;
		}
		}
	} else {
		DBG_PRINTF("fc_h smc:0x%x flags:0x%lx x1:0x%lx x2:0x%lx\n\r", smc_fid, flags, x1, x2);
		/* Handle AEE Dump even if t-base is not ready, as this will not enter tbase */
		if (smc_fid == TBASE_SMC_AEE_DUMP) {
			/* N-world can request AEE Dump function */
		...
		if ((tbaseExecutionStatus&TBASE_STATUS_FASTCALL_OK_BIT) == 0) {
			...
			DBG_PRINTF("fc_h FASTCALL_NOT_OK\n\r");
			SMC_RET1(handle, SMC_UNK);
			return 0;
		}
		if (tbase_ctx->state == TBASE_STATE_OFF) {
			DBG_PRINTF("fc_h TBASE_STATE_OFF\n\r");
			SMC_RET1(handle, SMC_UNK);
			return 0;
		}
		...
		uint64_t flags)
{
	...
	if (caller_security_state == SECURE) {
		...
		if ((tbaseExecutionStatus&TBASE_STATUS_SMC_OK_BIT) == 0) {
			/* TBASE must be initialized to be usable */
			DBG_PRINTF("sc_h FASTCALL_NOT_OK\n\r");
			/* TODO: What is correct error code? */
			SMC_RET1(handle, SMC_UNK);
			return 1;
		}
		if (tbase_ctx->state == TBASE_STATE_OFF) {
			DBG_PRINTF("sc_h TBASE_STATE_OFF\n\r");
			return 1;
		}
		...
		void *cookie)
{
	...
	NOTICE("%s core %d id %d!\n", __func__, (int)linear_id, (int) id);

#ifndef PLAT_mt6799
	/* Compatible to old routing model */
	if (id == FIQ_SMP_CALL_SGI) {
		...
		if ((tbaseExecutionStatus&TBASE_STATUS_SMC_OK_BIT) == 0) {
			...
			DBG_PRINTF("fiq_h FASTCALL_NOT_OK\n\r");
			return 1;
		}
		if (tbase_ctx->state == TBASE_STATE_OFF) {
			DBG_PRINTF("fiq_h TBASE_STATE_OFF\n\r");
			return 1;
		}
		/* Save the non-secure context before entering the TSP */
		cm_el1_sysregs_context_save(NON_SECURE);
