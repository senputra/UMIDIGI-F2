...
{
	int64_t mpidr = read_mpidr();
	int32_t linear_id = platform_get_core_pos(mpidr);
	base_context *tbase_ctx = &secure_context[linear_id];

	ssert(tbase_ctx->state == TBASE_STATE_ON);

	/* DBG_PRINTF("\r\ntbase_cpu_suspend_handler %d\r\n", linear_id); */

	base_ctx->state = TBASE_STATE_SUSPEND;
}
