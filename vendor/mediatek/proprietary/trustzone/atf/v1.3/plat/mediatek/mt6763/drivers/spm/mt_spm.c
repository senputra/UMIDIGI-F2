...
static char *dyna_load_pcm_path[] = {
	[DYNA_LOAD_PCM_SUSPEND_LP4_CH2] = "pcm_suspend_lp4_2ch.bin",
	[DYNA_LOAD_PCM_SUSPEND_LP4_CH1] = "pcm_suspend_lp4_1ch.bin",
	[DYNA_LOAD_PCM_SUSPEND_LP3_CH1] = "pcm_suspend_lp3_1ch.bin",
...
int spm_load_pcm_firmware(void)
{
	...
	for (i = DYNA_LOAD_PCM_SUSPEND_LP4_CH2; i < DYNA_LOAD_PCM_MAX; i++) {
...
void spm_irq0_handler(__uint64_t x1)
{
	...
	mmio_write_32(SPM_SWINT_CLR, PCM_SW_INT0 | PCM_SW_INT1);
}
...
void save_pwr_ctrl(struct pwr_ctrl *pwrctrl, unsigned int index, unsigned int val)
{
	switch (index)
	{
		...
		case PWR_CONN_APSRCREQ_INFRA_MASK_B:
			...
			break;
		case PWR_SSPM_APSRCREQ_INFRA_MASK_B:
...
unsigned int load_pwr_ctrl(struct pwr_ctrl *pwrctrl, unsigned int index)
{
	...
	switch (index)
	{
		...
		case PWR_CONN_APSRCREQ_INFRA_MASK_B:
			...
			break;
		case PWR_SSPM_APSRCREQ_INFRA_MASK_B:
			val = pwrctrl->sspm_apsrcreq_infra_mask_b;
			break;
		case PWR_MD_DDR_EN_0_MASK_B:
