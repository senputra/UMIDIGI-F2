...
void __spm_set_cpu_status(int cpu)
{
	if (cpu >= 0 && cpu < 4) {
		...
		mmio_write_32(ARMPLL_CLK_CON,
				(mmio_read_32(ARMPLL_CLK_CON) & ~(MUXSEL_SC_ARMPLL1_LSB | MUXSEL_SC_ARMPLL2_LSB)) |
...
void __spm_set_power_control(const struct pwr_ctrl *pwrctrl)
{
	...
	mmio_write_32(SPM_SRC_MASK,
		...
		((pwrctrl->sspm_apsrcreq_infra_mask_b & 0x1) << 20) |
	...
	mmio_write_32(SPM_SRC2_MASK,
		...
		((pwrctrl->ufs_srcclkena_mask_b & 0x1) << 20) |
		((pwrctrl->ufs_vrf18_req_mask_b & 0x1) << 21) |
...
void __spm_kick_pcm_to_run(struct pwr_ctrl *pwrctrl)
{
	...
	/* cannot modify pcm_flags1[4] which is from bootup setting */
	con0 = mmio_read_32(SPM_SW_RSV_2) & (SPM_FLAG1_RESERVED_BIT4);
	mmio_write_32(SPM_SW_RSV_2, con0 |
			((pwrctrl->pcm_flags1 & ~(SPM_FLAG1_ENABLE_VPROC_BY_MT6311)) |
			(is_ext_buck ? SPM_FLAG1_ENABLE_VPROC_BY_MT6311 : 0)));
