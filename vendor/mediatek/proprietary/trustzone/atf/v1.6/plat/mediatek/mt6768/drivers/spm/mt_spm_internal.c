										...
										const struct pcm_desc *pcmdesc)
{
	...
	INFO("r12 = 0x%x, r12_ext = 0x%x, raw_sta = 0x%x, idle_sta = 0x%x, req_sta =  0x%x, event_reg = 0x%x\n",
			wakesta->r12, wakesta->r12_ext, wakesta->raw_sta, wakesta->idle_sta,
			wakesta->req_sta, wakesta->event_reg);

	INFO("isr = 0x%x, raw_ext_sta = 0x%x, wake_misc = 0x%x\n",
			wakesta->isr, wakesta->raw_ext_sta, wakesta->wake_misc);
#endif
	return wr;
}
...
void __spm_set_power_control(const struct pwr_ctrl *pwrctrl)
{
	...
	mmio_write_32(SPM_AP_STANDBY_CON,
		...
		((pwrctrl->wfi_type & 0x1) << 1) |
		((pwrctrl->mp0_cputop_idle_mask & 0x1) << 2) |
		((pwrctrl->mp1_cputop_idle_mask & 0x1) << 3) |
...
void __spm_clean_after_wakeup(void)
{
	...
	/* set spm to connsys ack hw mask */
	mmio_write_32(SPM2CONN_ACK_WAIT_CYCLE, SPM2CONN_ACK_WAIT_MASK_VALUE);
