...
static char *dyna_load_pcm_path[] = {
	...
	[DYNA_LOAD_PCM_SUSPEND_LP4_3600] = "pcm_allinone_lp4_3600.bin",
...
void save_pwr_ctrl(struct pwr_ctrl *pwrctrl, unsigned int index, unsigned int val)
{
	...
	case PWR_MP0_CPU3_WFI_EN:
		...
		break;
	/* MP0_CPU4_WFI_EN */
	case PWR_MP0_CPU4_WFI_EN:
		pwrctrl->mp0_cpu4_wfi_en = val;
		break;
	/* MP0_CPU5_WFI_EN */
	case PWR_MP0_CPU5_WFI_EN:
		pwrctrl->mp0_cpu5_wfi_en = val;
		break;
	/* MP0_CPU6_WFI_EN */
	case PWR_MP0_CPU6_WFI_EN:
		pwrctrl->mp0_cpu6_wfi_en = val;
		break;
	/* MP0_CPU7_WFI_EN */
	case PWR_MP0_CPU7_WFI_EN:
...
void spm_register_init(void)
{
	...
	/* Init armpll gating setting, 0: hw mode, 1:sw mode
	 * HW mode: spm control. SW mode: sspm control
	 * bit[0]: armpll
	 * bit[1]: armbpll
	 * bit[2]: ccipll
	 * bit[3]: armbbpll
	 */
	mmio_write_32(AMRPLL_CLK_SEL,
		(ARMPLL_CLK_SEL_LSB | ARMBPLL_CLK_SEL_LSB | ARMBBPLL_CLK_SEL_LSB));

	/* Change XO settle time from 3ms (default) to 1ms */
	mmio_write_32(SPM_CLK_SETTLE, 0x21);
