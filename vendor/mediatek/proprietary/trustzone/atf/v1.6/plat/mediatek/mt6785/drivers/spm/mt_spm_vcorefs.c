...
void spm_dvfsfw_init(uint64_t boot_up_opp, uint64_t dram_issue)
{
	...
	if (__ddr_max_freq == 4266)
		mmio_write_32(SPM_VCORE_DVFS_SHORTCUT00, 0x08800001);
	else
...
{
#if 1
	uint32_t dvfs_mask = SPM_FLAG_DISABLE_VCORE_DVS | SPM_FLAG_DISABLE_VCORE_DFS;
...
static uint32_t dvfsrc_sw_req3;

void dvfsrc_backup_req(void)
{
	dvfsrc_sw_req3 = mmio_read_32(DVFSRC_SW_REQ3);
	mmio_write_32(DVFSRC_SW_REQ3,
		(dvfsrc_sw_req3 & ~(DDR_SW_AP_MASK << DDR_SW_AP_SHIFT)));
}

void dvfsrc_restore_req(void)
{
	mmio_write_32(DVFSRC_SW_REQ3, dvfsrc_sw_req3);
}
