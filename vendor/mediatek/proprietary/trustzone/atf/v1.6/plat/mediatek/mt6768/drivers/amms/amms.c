	...
	struct amms_msg_request *request)
{
	 ...
	 */
	/* already free , no need to notify kernel do again*/
	spin_lock(&amms_pending_lock);
	if (is_POS_free == AMMS_POS_OWN_BY_MD) {
		spin_unlock(&amms_pending_lock);
		goto pos_mpu_out;
		...
		unsigned long long base_addr)
{
	...
	if (md_pos_length && md_pos_addr && !is_POS_MPU_applied) {
		is_POS_MPU_applied = 1;
		amms_md_pos_mpu_change(AMMS_POS_OWNER_AP);
	}
	return 0;
}
...
int sip_amms_pos_stress_touch(void)
{
	struct amms_msg_request request;

	AMMS_NOTICE("[ap cmd] is_POS_free= %d\n"
		, is_POS_free);
	request.seq_id = 0xff;
	if (is_POS_free == 1) {/*0:MD own 1:AP own*/
		request.cmd = AMMS_CMD_ALLOCATE_POS_BUFFER;
	} else if (is_POS_free == 0) {
		request.cmd = AMMS_CMD_DEALLOCATE_POS_BUFFER;
	} else {
		AMMS_NOTICE("%s:bypass during processing\n", __func__);
	}
