...
static unsigned int drambus26m_resource_usage;

#define __WAKE_SRC_FOR_DRAM_BUS26M_COMMON__ (\
	(R12_PCM_TIMER_EVENT) | \
	(R12_KP_IRQ_B) | \
	(R12_APWDT_EVENT_B) | \
	(R12_APXGPT1_EVENT_B) | \
...
#define WAKE_SRC_FOR_DRAM_BUS26M	\
	(__WAKE_SRC_FOR_DRAM_BUS26M_COMMON__)
#else
#define WAKE_SRC_FOR_DRAM_BUS26M	\
	(__WAKE_SRC_FOR_DRAM_BUS26M_COMMON__\
		| R12_SEJ_EVENT_B)
...
static struct pwr_ctrl dram_bus26m_ctrl = {
	.wake_src = WAKE_SRC_FOR_DRAM_BUS26M,
...
struct spm_lp_scen __spm_dram_bus26m = {
	.pwrctrl = &dram_bus26m_ctrl,
};

void spm_dram_bus26m_storage_mask_set(void)
{
	struct pwr_ctrl *pwrctrl;

	pwrctrl = __spm_dram_bus26m.pwrctrl;
...
void spm_dram_bus26m_args(uint64_t arg1, uint64_t arg2, uint64_t arg3)
{
	struct pwr_ctrl *pwrctrl;
	...
	drambus26m_resource_usage = arg3;
}

void spm_dram_bus26m(int StateId)
{
	spm_conservation(StateId, 0
		, &__spm_dram_bus26m, drambus26m_resource_usage);
}

void spm_dram_bus26m_finish(int StateId)
{
