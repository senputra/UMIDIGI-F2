...
uint64_t mediatek_sip_handler_secure(uint32_t smc_fid,
			  u_register_t x1,
			  u_register_t x2,
			  u_register_t x3,
			  u_register_t x4,
...
uint64_t mediatek_sip_handler_hypervisor(uint32_t smc_fid,
			  u_register_t x1,
			  u_register_t x2,
			  u_register_t x3,
			  u_register_t x4,
...
uint64_t mediatek_sip_handler_bootloader(uint32_t smc_fid,
			  u_register_t x1,
			  u_register_t x2,
			  u_register_t x3,
			  u_register_t x4,
...
uint64_t mediatek_sip_handler_kernel(uint32_t smc_fid,
			  u_register_t x1,
			  u_register_t x2,
			  u_register_t x3,
			  u_register_t x4,
			  ...
			  u_register_t flags)
{
	...
	case MTK_SIP_KERNEL_WDT_AARCH64:
		...
		NOTICE("teearg->atf_aee_debug_buf_start : 0x%llx\n",
	...
	case MTK_SIP_KERNEL_ATF_DEBUG_AARCH64:
		...
		case ATF_TEST_DO_PANIC:
			...
			assert(0);
			break;
		case ATF_TEST_RAW_DUMP:
			INFO("UT for RAW dump!\n");
			atf_crash_flag_addr_ptr = mt_log_get_crash_flag_addr();
...
#endif
	case MTK_SIP_TEE_CONTROL_AARCH32:
	case MTK_SIP_TEE_CONTROL_AARCH64:
		switch (x1) {
		case TEE_OP_ID_SET_PENDING:
			sip_tee_set_pending();
			SMC_RET0(handle);
			break;
		}
			...
			u_register_t flags)
{
	...
	if (!ns) {
		/* SiP SMC service secure world's call */
		rc = mediatek_sip_handler_secure(smc_num, x1, x2, x3, x4,
					cookie, handle, flags);
	} else {
		if (is_from_bootloader(smc_ori)) {
			/* SiP SMC service secure bootloader's call */
			rc = mediatek_sip_handler_bootloader(smc_num, x1, x2, x3, x4,
						cookie, handle, flags);
#ifdef MTK_ENABLE_GENIEZONE
		} else if (is_from_hyp(smc_ori)) {
			/* SiP SMC service hypervisor's call */
			rc = mediatek_sip_handler_hypervisor(smc_num, x1, x2, x3, x4,
						cookie, handle, flags);
#endif
		} else if (is_from_kernel(smc_ori)) {
			/* SiP SMC service secure kernel's call */
			rc = mediatek_sip_handler_kernel(smc_num, x1, x2, x3, x4,
						cookie, handle, flags);
		}
	}
