...
{
	uint64_t bRet = SPM_FIRMWARE_STATUS_NOT_LOADED;
...
void spm_pwr_ctrl_args(uint64_t x1, uint64_t x2, uint64_t x3)
{
	...
	case SPM_PWR_CTRL_SUSPEND:
		...
		break;
	case SPM_PWR_CTRL_IDLE_DRAM:
		save_pwr_ctrl(__spm_dram_bus26m.pwrctrl, x2, x3);
		break;
	case SPM_PWR_CTRL_IDLE_SYSPLL:
		save_pwr_ctrl(__spm_syspll.pwrctrl, x2, x3);
		break;
	case SPM_PWR_CTRL_IDLE_BUS26M:
		save_pwr_ctrl(__spm_dram_bus26m.pwrctrl, x2, x3);
		break;
...
uint64_t spm_get_pwr_ctrl_args(uint64_t x1, uint64_t x2, uint64_t x3)
{
	...
	case SPM_PWR_CTRL_SUSPEND:
		...
		break;
	case SPM_PWR_CTRL_IDLE_DRAM:
		ret = load_pwr_ctrl(__spm_dram_bus26m.pwrctrl, x2);
		break;
	case SPM_PWR_CTRL_IDLE_SYSPLL:
		ret = load_pwr_ctrl(__spm_syspll.pwrctrl, x2);
		break;
	case SPM_PWR_CTRL_IDLE_BUS26M:
		ret = load_pwr_ctrl(__spm_dram_bus26m.pwrctrl, x2);
		break;
...
void spm_args(uint64_t x1, uint64_t x2, uint64_t x3)
{
	...
	case SPM_ARGS_SUSPEND_FINISH:
		spm_suspend_finish();
		break;
	case SPM_ARGS_IDLE_DRAM:
		spm_dram_bus26m(MTK_IDLEDRAM_MODE);
		break;
	case SPM_ARGS_IDLE_DRAM_FINISH:
		spm_dram_bus26m_finish(MTK_IDLEDRAM_MODE);
		break;
	case SPM_ARGS_IDLE_BUS26M:
		spm_dram_bus26m(MTK_IDLEBUS26M_MODE);
		break;
	case SPM_ARGS_IDLE_BUS26M_FINISH:
		spm_dram_bus26m_finish(MTK_IDLEBUS26M_MODE);
		break;
	case SPM_ARGS_IDLE_SYSPLL:
		spm_syspll(MTK_IDLESYSPLL_MODE);
		break;
	case SPM_ARGS_IDLE_SYSPLL_FINISH:
		spm_syspll_finish(MTK_IDLESYSPLL_MODE);
		break;
...
void spm_register_init(void)
{
	...
	mmio_write_32(SPM_CLK_CON, mmio_read_32(SPM_CLK_CON) | (0x3 << 2) |
			REG_SPM_LOCK_INFRA_DCM_LSB | REG_SRCCLKEN_MASK_LSB |
	...
	/* Init for SRCLKEN_RC */
	/* SPM_POWER_ON_VAL1[6] = 1 */
	mmio_write_32(SPM_SPARE_CON_SET, 0x2);
	...
	mmio_write_32(SPM_RESOURCE_ACK_CON0, 0x00000000);
	mmio_write_32(SPM_RESOURCE_ACK_CON1, 0x00000000);
	mmio_write_32(SPM_RESOURCE_ACK_CON2, 0x00000000);
	mmio_write_32(SPM_RESOURCE_ACK_CON3, 0xCCCC4E4E);
	...
	/* Set infra_ao_dbg_con for infra bus protect debugging */
	/* PCM_DEBUG_CON = 0x1 */
	/* INFRA_AO_DBG_CON0[12:5] = 0xD */
	/* INFRA_AO_DBG_CON1[12:5] = 0x4E */
	mmio_write_32(PCM_DEBUG_CON,
		      (mmio_read_32(PCM_DEBUG_CON) | 0x1));
	mmio_write_32(INFRA_AO_DBG_CON0,
		      (mmio_read_32(INFRA_AO_DBG_CON0) & (~(0xFF << 5))) | 0xD);
	mmio_write_32(INFRA_AO_DBG_CON1,
		      (mmio_read_32(INFRA_AO_DBG_CON1) & (~(0xFF << 5))) | 0x4E);
}
...
void spm_boot_init(void)
{
	...
	if ((mmio_read_32(GPIO_BANK) & TRAP_UFS_FIRST) != 0) {
		...
		spm_dram_bus26m_storage_mask_set();
		spm_syspll_storage_mask_set();
	}
}
