...
static void ufs_crypto_power_ctl(enum ufs_crypto_power power)
{
	if (power == UFS_CRYPTO_POWER_ON) {
		...
		/*
		 * AES CG should use CCF interface in kernel.
		 * Because there may not only CG bit need control in BB chip.
		 */
...
static void ufs_device_reset_ctl(uint32_t rst_n)
{
	uint32_t reg;
...
int32_t ufs_generic_ctl(uint32_t ctl, uint32_t param2, uint32_t param3)
{
	...
	else if (ctl == UFS_DEVICE_RESET_CTL)
		ufs_device_reset_ctl(param2);
	else
