			  ...
			  uint64_t flags)
{
	...
	switch (smc_fid) {
	case MTK_SIP_TEE_HAL_APC_SET_AARCH32:
	case MTK_SIP_TEE_HAL_APC_SET_AARCH64:
		rc = sip_tee_hal_req(SIP_APC_MODULE_SET, x1, x2, x3);
		SMC_RET1(handle, rc);
		break;
	case MTK_SIP_TEE_HAL_MASTER_TRANS_AARCH32:
	case MTK_SIP_TEE_HAL_MASTER_TRANS_AARCH64:
		rc = sip_tee_hal_req(SIP_APC_MASTER_SET, x1, x2, x3);
		SMC_RET1(handle, rc);
		break;
	default:
		rc = SMC_UNK;
...
#endif
#endif
	case MTK_SIP_HYP_GET_RND_AARCH32:
	case MTK_SIP_HYP_GET_RND_AARCH64:
