...
func mtkFodDefaults(ctx android.LoadHookContext) {
    type props struct {
        Cflags []string
        Shared_libs []string
        Static_libs []string
...
func init() {
    android.RegisterModuleType("mtk_fod_defaults", mtkFodDefaultsFactory)
}

func mtkFodDefaultsFactory() android.Module {
    module := cc.DefaultsFactory()
    android.AddLoadHook(module, mtkFodDefaults)
    return module
}
