...
DebugInfo::DebugInfo(const char *filename) : DebugInfo() {
    readFile(filename);

    std::vector<char> actions;
    actions.push_back(static_cast<char>('a'));
    actions_ = &actions;
...
int DebugInfo::update() {
    ...
    if(readFile(DEBUG_SESSION_DUMP_FILE)){
        return -1;
    }
...
int DebugInfo::readFile(const char *filename) {
