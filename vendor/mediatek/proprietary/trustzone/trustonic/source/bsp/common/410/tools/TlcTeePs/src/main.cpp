...
static int printUsage(char *path) {
    fprintf(stderr, "\nusage: %s [-hakfmorst]\n", ::basename(path));
    fprintf(stderr, "Copyright (c) Trustonic Limited 2015-2017.\n");
    fprintf(stderr, MOBICORE_COMPONENT_BUILD_TAG "\n");
    fprintf(stderr, "-i file\t\tDump file to read\n");
...
int main(int argc, char *args[]) {
    ...
    while ((c = ::getopt(argc, args, "haAkKfFmMoOgGrRtsSTi")) != -1) {
        switch (::tolower(c)) {
            case 'h': // Help
                errFlag++;
                break;
            ...
            case 'i':
            {
                DebugInfo debug_info = DebugInfo(args[2]);
                return EXIT_SUCCESS;
            }
            case '?' :
                errFlag++;
