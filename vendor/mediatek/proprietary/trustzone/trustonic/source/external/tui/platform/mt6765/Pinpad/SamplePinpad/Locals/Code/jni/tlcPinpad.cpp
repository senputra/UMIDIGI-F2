...
static const char* securePinpadTrustedApp[] = {
    "/vendor/app/mcRegistry/07890000000000000000000000000003.tlbin",
    "/vendor/app/mcRegistry/07890000000000000000000000000006.tlbin"
};
...
bool securePinpadConnect()
{
    ...
    uint8_t nApiLevelIndex = 0;

    LOG_D("Opening Kinibi device");
    mcRet = mcOpenDevice(MC_DEVICE_ID_DEFAULT);//0x53525643 for SELinux permissive mode
    if (mcRet != MC_DRV_OK) {
        LOG_E("Error opening device: %d", mcRet);
        return false;
    }

    LOG_D("Allocating buffer for TCI");
    ...
    if (versionInfo.versionTlApi >= 0x00010011)
        nApiLevelIndex = 1;

	ssize_t assetSize = getAssetContent(securePinpadTrustedApp[nApiLevelIndex], NULL, 0);
    if (0 <= assetSize) {
        /* the TA is found in the assets of the APK, load it from here */
        nTrustedAppLength = assetSize;
        pTrustedAppData = (uint8_t *) malloc(nTrustedAppLength);
        if (pTrustedAppData != NULL) {
            assetSize = getAssetContent(securePinpadTrustedApp[nApiLevelIndex], pTrustedAppData, nTrustedAppLength);
            if ((assetSize != nTrustedAppLength) || (assetSize <= 0)) {
                LOG_E("%s:%i Wrong  assetSize = %i", __func__, __LINE__, (int)assetSize);
            ...
            LOG_E("%s:%i Not enough memory", __func__, __LINE__);
            free(tci);
            tci = NULL;
            return false;
        }
	    ...
	    nTrustedAppLength = getFileContent(
			    securePinpadTrustedApp[nApiLevelIndex],
			    &pTrustedAppData);

	    if (nTrustedAppLength != 0) {
		    LOG_I("%s:%i nTrustedAppLength = %u", __func__, __LINE__, nTrustedAppLength);
	    } else {
    ...
    LOG_D("mcOpenTrustlet() succeeded");
    return true;
}
...
static bool tlcWrapAndStore(const uint8_t* data, uint32_t length, const char* filename)
{
    ...
    LOG_D(" Provisioning: tlcWrapAndStore(data[0]=0x%x, length=%d)", data[0], length);
    ...
    LOG_D(" Provisioning: tci->len is %i", tci->len);
    pDataStorageFile = fopen((char*)filename, "wb");
    if (pDataStorageFile == NULL) {
        LOG_D("Provisioning: Opening of file %s failed!", filename);
        LOG_ERRNO("Opening");
        return false;
    }
    size_t write_val;
    ...
    LOG_D(" Provisioning: write_val = %i\t\t", (int)write_val);
...
bool securePinpadProvisionItem(uint8_t itemID, const char* path,
    ...
    LOG_D(" securePinpadProvisionItem: pathOfSO = %s", pathOfSO);
    LOG_D(" securePinpadProvisionItem: listOfSOFile[%d] = %s", itemID, listOfSOFile[itemID]);
    LOG_D(" securePinpadProvisionItem: begin of item data = 0x%X;0x%X;0x%X;0x%X;0x%X",
            item[0], item[1], item[2], item[3], item[4]);
    LOG_D(" securePinpadProvisionItem: end of item data = 0x%X;0x%X;0x%X;0x%X;0x%X",
...
bool readFile(uint8_t *dst, const uint32_t maxLen, uint32_t *len, const char* filename) {
    ...
    *len = 0;
    ...
    tmp = fseek(pDataStorageFile , 0 , SEEK_END);
    if (tmp != 0) {
        LOG_E("fseek error: %s", strerror(errno));
        fclose(pDataStorageFile);
        return false;
    }
    ...
    LOG_D("File %s has size %li", filename, fileSize);
    if (fileSize > maxLen) {
    ...
    LOG_D(" File starts with: 0x%x   ", dst[0]);
    *len = read_val;
    fclose(pDataStorageFile);
    return true;
}
...
const char *securePinpadVerify(void) {
    ...
    default:
        LOG_D("Trusted Application Response: Unknown response data");
...
void securePinpadDisconnect() {
    ...
    LOG_D("Closing the Trusted Application session");
    ...
    LOG_D("mcCloseSession() succeeded");
    // -------------------------------------------------------------
    // Step 6: Close the MobiCore device
    LOG_D("Closing MobiCore device");
