    ...
    uint8_t** ppContent)
{
   ...
   if (fseek(pStream, 0L, SEEK_END) != 0)
   {
      ...
      LOG_E("Error: Cannot seek file: %s.\n", pPath);
      goto error;
   }
   ...
   if (filesize < 0)
   {
      ...
      LOG_E("Error: Cannot ftell file: %s.\n", pPath);
      goto error;
   }
   ...
   {
      fprintf(stderr, "Error: Cannot read file: %s.\n", pPath);
      LOG_E("Error: Cannot seek_set file: %s.\n", pPath);
      goto error;
   }
...
bool readFile(uint8_t *dst, const uint32_t maxLen, uint32_t *len, const char* filename) {
    ...
    *len = 0;
    ...
    LOG_I("File %s has size %li", filename, fileSize);
    if (fileSize > maxLen) {
