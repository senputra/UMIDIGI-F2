        ...
        unsigned long long start, unsigned long long end, int region, unsigned long long access_permission)
{
    ...
    //map register set
    if((ret = drApiMapPhysicalBuffer((uint64_t)EMI_MPU_BASE, SIZE_4KB, MAP_HARDWARE, (void **)&EMI_MPU_VA_BUFFER)) != DRAPI_OK)
    {
        drApiLogPrintf("[emi_mpu_set_region_protection] map EMI_MPU_BASE failed! ret=%d\n", ret);
        return -3;
    }

    if ((region == SHARED_SECURE_MEMORY_MPU_REGION_ID) || (region == TUI_MEMORY_MPU_REGION_ID))
    {
    ...
    //unmap register set
    if (drApiUnmapBuffer(EMI_MPU_VA_BUFFER) != DRAPI_OK)
    {
        drApiLogPrintf("[emi_mpu_set_region_protection] unmap EMI_MPU_BASE failed!\n");
    }
