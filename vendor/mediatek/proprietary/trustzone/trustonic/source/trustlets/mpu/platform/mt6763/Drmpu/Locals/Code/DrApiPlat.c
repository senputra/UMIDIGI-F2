...
#endif
        //unmap register set
        if (drApiUnmapBuffer(MT_APHW_VER_VA_BUFFER) != DRAPI_OK)
        {
            drApiLogPrintf("[mt_platform_init] unmap MT_VER_BASE failed!\n");
            return;
        }
...
int enable_4G(void)
{
    ...
    if((ret = drApiMapPhysicalBuffer((uint64_t)INFRACFG_AO_BASE, SIZE_4KB, MAP_HARDWARE, (void **)&INFRACFG_AO_BASE_VA)) != DRAPI_OK)
    {
        drApiLogPrintf("[emi_mpu_set_region_protection] map INFRACFG_AO_BASE failed! ERROR: %d\n", ret);
        return -2;
    }
    ...
    if (drApiUnmapBuffer(INFRACFG_AO_BASE_VA) != DRAPI_OK)
    {
        drApiLogPrintf("[emi_mpu_set_region_protection] unmap INFRACFG_AO_BASE failed!\n");
        return -1;
    }
