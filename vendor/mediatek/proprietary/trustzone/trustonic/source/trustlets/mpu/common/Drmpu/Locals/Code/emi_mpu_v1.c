...
int emi_mpu_set_protection(struct emi_region_info_t *region_info)
{
    ...
    if (region_info->region >= EMI_MPU_REGION_NUM) {
        drApiLogPrintf("[emi_mpu_set_protection] region %d out of range\n",
            region_info->region);
        return -4;
    }
    ...
    //map register set
    if (drApiMapPhysicalBuffer((uint64_t)EMI_MPU_BASE, SIZE_4KB, MAP_HARDWARE, (void **)&EMI_MPU_VA_BUFFER) != DRAPI_OK) {
        drApiLogPrintf("[emi_mpu_set_protection] map EMI_MPU_BASE failed!\n");
        return -2;
    }
    ...
    if (drApiUnmapBuffer(EMI_MPU_VA_BUFFER) != DRAPI_OK) {
        drApiLogPrintf("[emi_mpu_set_protection] unmap EMI_MPU_BASE failed!\n");
        return -1;
    }
