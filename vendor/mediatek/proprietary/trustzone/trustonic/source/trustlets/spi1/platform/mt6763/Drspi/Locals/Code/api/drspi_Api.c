...
//	tlset_module_apc(DAPC_INDEX_SPI0,0,1); // lock SPI, only secure world can access
	if(UseSecureSpi == 0)
	{
		...
		if (ret == TLAPI_OK) {
			if (SharedMemStatus) {
				...
				TLSPIMSG("[Trustlet TlSPi] ERROR!! Non-sharedMemory Allocation is not supported!!\n");
				/*Shared mem not enabled*/
				//drSecMemAllocPA(&((uint64_t)tx_dma_addr),256*1024,4);//allocate tx dma
				//drSecMemAllocPA(&((uint64_t)rx_dma_addr),256*1024,4);//allocate rx dma
			}
		} else {
			TLSPIMSG("[Trustlet TlSPi] ERROR!!spi drSecMemQueryPlReservedMem return error!\n");
		}
		tl_ret = tlset_master_transaction(DAPC_MASTER_INDEX_SPI1,SECURE_TRANSACTION);	//spi measter use secure transaction
		UseSecureSpi = 1 ;
	}
	...
	if (spiData.is_dma_used == 1) {
		if (SharedMemStatus) {
			...
			spiData.rx_dma = SharedMemAddr + SharedMemSize*1/2 ;
...
//	tlset_master_transaction(9,0);	//spi measter use non-secure transaction
//	tlset_module_apc(DAPC_INDEX_SPI0, 0, 0); // unlock SPI make normal world can access
	return ret;
