...
void *drUtilsMapTaskBuffer(const taskid_t taskId, const void *startVirtClient, const size_t len,
...
#if TBASE_API_LEVEL >= 5
    {
        ...
        if (DRAPI_OK!= (drRet = drApiMapTaskBuffer(taskId,
                startVirtClient, len, attr, (void **) &pBuf))) {
            drDbgPrintLnf(LOG_TAG ":drUtilsMapTaskBuffer error:0x%X\n", drRet);
        }
    }
 ...
 */_NORETURN void drIpchLoop(void) {
    ...
    for (;;) {
        if (DRAPI_OK != drApiIpcCallToIPCH(&ipcClient, &ipcMsg, &ipcData)) {
            drDbgPrintLnf("drIpchLoop(): drApiIpcCallToIPCH failed\n");
            continue;
        }
        ...
        switch (ipcMsg) {
            case MSG_CLOSE_TRUSTLET: {
                drDbgPrintLnf("drIpchLoop(): Acknowledging trustlet close\n");
            ...
            case MSG_CLOSE_DRIVER: {
                /* Acknowledge */
                drDbgPrintLnf("[IPC] MSG_CLOSE_DRIVER\n");
            ...
            case MSG_GET_DRIVER_VERSION: {
                drDbgPrintLnf("[IPC] MSG_GET_DRIVER_VERSION\n");
            ...
            case MSG_RQ: {
                ...
                drApiLogPrintf("[%s] GF_DR version: GF_DR_v%d.%d\n", __func__,
...
void drIpchInit(void) {
    ...
    if (DRAPI_OK
        ...
        drDbgPrintLnf("[Driver DrGoodixUtils] drIpchInit(): drApiStartThread failed");
    }
}
