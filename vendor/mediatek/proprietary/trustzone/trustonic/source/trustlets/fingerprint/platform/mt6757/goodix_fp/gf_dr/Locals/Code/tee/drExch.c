        ...
        switch (mr1) {
                ...
                switch (faultedThread) {
                    ...
                    case DRIVER_THREAD_NO_IPCH:
                        ...
                        if (DRAPI_OK != drRestartThread(faultedThread, ip, sp)) {
                            drDbgPrintLnf(
                                    "[Driver DrGoodixUtils] drExchLoop(): restarting thread failed");
                        }
                    ...
                    default:
                        drDbgPrintLnf(
                                "[Driver DrGoodixUtils] drExchLoop(): Unknown thread. This should never happen");
                        break;
                }
            ...
            case TRAP_UNDEF_INSTR:
                ...
                if (DRAPI_OK != drApiStopThread(DRIVER_THREAD_NO_IPCH)) {
                    drDbgPrintLnf(
                            "[Driver DrGoodixUtils] drExchLoop(): Unable to stop IPC handler thread");
                }
