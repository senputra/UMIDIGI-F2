...
gf_error_t gf_so_load_persistent_object(int8_t *name, uint8_t **data, uint32_t *data_len) {
    ...
    do {
        ...
        if ((NULL == name) || (NULL == data) || (NULL == data_len)) {
            ...
            GF_LOGE(LOG_TAG "[%s] invalid parameters, name:0x%p, data:%p data_len:0x%p", __func__, 
                    name, (void *)data, (void *)data_len);
            break;
        }

        err = gf_tee_open_object((uint8_t*) name, SO_MODE_READ, &so_handle);
        if (GF_SUCCESS != err) {
            GF_LOGI(LOG_TAG "[%s] so(%s) not exist, err:%d", __func__, name, err);
            break;
        }
        ...
        *data_len = 0;
        ...
        if (GF_SUCCESS != err || ((int32_t)*data_len) <= 0) {
            GF_LOGE(
                    LOG_TAG "[%s] fail to get object size, err:%d, size:%d", __func__, 
              			err, *data_len);
...
gf_error_t gf_so_save_persistent_object(int8_t *name, uint8_t *data, uint32_t data_len) {
    ...
    do {
        if ((NULL == name) || (NULL == data) || (0 == data_len)) {
            err = GF_ERROR_BAD_PARAMS;
            GF_LOGE(LOG_TAG "[%s] invalid parameters, name:0x%p, data:0x%p, data_len:%d", __func__, 
                    name, (void*)data, data_len);
            break;
        }
