...
#if defined(GF_CHIP_SERIES_MILAN_A) || defined(GF_CHIP_SERIES_MILAN_F)
...
typedef enum {
    ...
    FINGER_STATUS_ENROLLING,
//< Current finger is enrolling, can not be used to recognize
} gf_finger_status_t;
 ...
 */
gf_error_t gf_algo_get_finger_study_count(uint32_t group_id, uint32_t finger_id, uint8_t *save_flag);
/**
 * \brief get the finger' save count to check whether to save the template
 */
 ...
 */
gf_error_t gf_algo_get_acquired_info(uint16_t *buf, uint32_t buf_len, gf_operation_type_t operation,
        int32_t *image_quality, int32_t *valid_area);
...
gf_error_t gf_algo_enroll(uint32_t *group_id, uint32_t *finger_id, uint16_t *samples_remaining,
        uint32_t *duplicate_finger_id, uint64_t secure_user_id, uint64_t authenticator_id);
 ...
 */
gf_error_t gf_algo_authenticate(int32_t study_enable_flag, uint32_t *group_id, uint32_t *finger_id,
        uint64_t *secure_user_id, uint64_t *authenticator_id, uint8_t *update_flag,
        int32_t *image_quality, int32_t *valid_area);
...
/**
 * \brief Check the limit condition for the current user.
 *
 * \return If it is valid, then return #GF_SUCCESS,otherwise return #GF_ERROR_REACH_FINGERS_UPLIMIT
 */
gf_error_t gf_algo_fingers_limit_check(void);
 ...
 */
gf_error_t gf_algo_dump_algo_debug_info(gf_operation_type_t operation, gf_dump_data_t *base);

// for frr far test
gf_error_t gf_frr_far_init(gf_test_frr_far_init_t* init_params);
...
gf_error_t gf_frr_far_authenticate(uint16_t *buf, uint32_t buf_len, gf_test_frr_far_t *test_frr_far);
gf_error_t gf_frr_far_dump_info(gf_test_frr_far_t *test);

void gf_frr_far_delete_tran(void);
void gf_frr_far_cancel(void);
void gf_frr_far_destroy(void);
gf_error_t gf_frr_far_reset_calibration(uint32_t is_update);
...
gf_error_t gf_algo_test_enroll(uint32_t *group_id, uint32_t *finger_id, uint16_t *samples_remaining, uint32_t *duplicate_finger_id);
void gf_algo_cancel_test_enroll(void);

gf_error_t gf_algo_test_authenticate(int32_t study_enable_flag, uint32_t *group_id, uint32_t *finger_id,
        uint8_t *update_flag, int32_t *image_quality, int32_t *valid_area);
...
gf_error_t gf_algo_get_calibration_params(void* calibration_params);
 ...
 *
 * \param raw_data[in] Point to base frame raw data.
 *
 * \param raw_data_len[in] The length of raw data in uint16_t.
 *
 * \return If success, return #GF_SUCCESS. If fail, return #GF_ERROR_BAD_PARAMS/
 */
gf_error_t gf_algo_set_base_rawdata(uint16_t *raw_data, uint32_t raw_data_len);

/**
 * \brief Get the initialized base frame from algorithm module.
 *
 * \param raw_data[out] Point to base frame raw data, Note: don't free this buffer.
 *
 * \param raw_data_len[out] The length of raw data in bytes, not uint16_t.
 *
 * \return If success, return #GF_SUCCESS. If fail, return #GF_ERROR_BAD_PARAMS/
 */
gf_error_t gf_algo_get_base_rawdata(uint16_t **raw_data, uint32_t *raw_data_len);
...
/**
 * \brief gf_algo_set_nav_base
 *
 * \param base[in] base data
 *
 * \param len[in] length of base data
 *
 *\
 * \return If success, return #GF_SUCCESS,otherwise return error code, eg:
 *  \li \c #GF_ERROR_BAD_PARAMS
 */
gf_error_t gf_algo_set_nav_base(uint16_t* base, uint32_t len);

gf_error_t gf_algo_get_nav_base(uint16_t **buf, uint32_t *buf_len);

gf_error_t gf_algo_get_last_identify_id(uint32_t *id);
...
gf_error_t gf_algo_fingerfeature_study(uint8_t *update_flag);
gf_error_t gf_algo_check_cover_broken(uint16_t *raw_data, uint32_t raw_len, int32_t *cover_broken_num,
        gf_operation_type_t operation, uint32_t *retry_flag);
