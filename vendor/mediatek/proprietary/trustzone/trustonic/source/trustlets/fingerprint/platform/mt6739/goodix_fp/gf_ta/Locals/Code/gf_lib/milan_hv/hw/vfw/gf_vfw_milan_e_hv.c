...
#define MILAN_E_HV_OTP_CRC_LEN 30

#define LOG_TAG "[gf_vfw_milan_e_hv]"

/* 0x00~0x18, 0x1A, 0x1C, 0x1D, 0x1E, 0x1F. 30 Bytes */
uint8_t vfw_milan_e_hv_get_otp_data_crc(uint8_t* otp_buf) {
    ...
    temp_data[25] = otp_buf[0x1A];
    cpl_memcpy(&temp_data[26], &otp_buf[0x1C], 4);

    crc = gf_milan_hv_get_crc8(temp_data, MILAN_E_HV_OTP_CRC_LEN);
