...
gf_error_t gf_ta_test_spi(gf_test_spi_t *cmd) {
    ...
    do {
        ...
        cmd->code_fw_version[code_offset++] = '0' + (g_oswego_m_fw_cfg.fw_data[5] & 0x0f);
        cmd->code_fw_version[code_offset++] = '0' + (g_oswego_m_fw_cfg.fw_data[6] & 0x0f);
        cmd->code_fw_version[code_offset++] = 'M';
        cmd->code_fw_version[code_offset++] = '_';

        cmd->code_fw_version[code_offset++] = '0' + (g_oswego_m_fw_cfg.fw_data[12] >> 4 & 0x0f);
        cmd->code_fw_version[code_offset++] = '.';
        cmd->code_fw_version[code_offset++] = '0';
        cmd->code_fw_version[code_offset++] = '0' + (g_oswego_m_fw_cfg.fw_data[12] & 0x0f);
        cmd->code_fw_version[code_offset++] = '.';
        cmd->code_fw_version[code_offset++] = '0' + (g_oswego_m_fw_cfg.fw_data[13] >> 4 & 0x0f);
        cmd->code_fw_version[code_offset++] = '0' + (g_oswego_m_fw_cfg.fw_data[13] & 0x0f);
        cmd->code_fw_version[code_offset++] = '\0';
    } while (0);
...
gf_error_t gf_ta_test_pixel_open_step1(gf_cmd_header_t *cmd) {
    ...
    do {
        ...
        err = gf_oswego_m_download_cfg(CONFIG_TEST_PIXEL_OPEN_A);
        if (err != GF_SUCCESS) {
            break;
        }
...
gf_error_t gf_ta_test_bad_point(gf_cmd_header_t *cmd) {
    ...
    do {
        ...
        if (GF_SUCCESS != err) {
            GF_LOGE(LOG_TAG "[%s] bad point init fail, err=%s, errno=%d", __func__,
                    gf_strerror(err), err);
            break;
        }
        ...
        if (GF_SUCCESS != err) {
            GF_LOGE(LOG_TAG "[%s] save base data fail, err=%s, errno=%d", __func__,
                    gf_strerror(err), err);
            break;
        }
...
static gf_error_t gf_ta_test_download_fw(gf_test_download_fw_cfg_t *cmd) {
    ...
    do {
        ...
        err = gf_oswego_m_test_download_fw(cmd->fw_data, cmd->fw_data_len);
    } while (0);
...
static gf_error_t gf_ta_test_download_cfg(gf_test_download_fw_cfg_t *cmd) {
    ...
    do {
        ...
        err = gf_oswego_m_test_download_cfg(cmd->cfg_data, cmd->cfg_data_len);
    } while (0);
...
static gf_error_t gf_ta_test_rawdata_saturated(gf_cmd_header_t *cmd) {
    gf_error_t err = GF_SUCCESS;
    ...
    do {
        ...
        err = gf_state_execute_operation(OPERATION_TEST_RAWDATA_SATURATED, &cmd->reset_flag);
        gf_hw_data_sample_resume();
    } while (0);
...
static gf_error_t gf_ta_test_sensor_fine(gf_cmd_header_t *cmd) {
    gf_error_t err = GF_SUCCESS;
    ...
    do {
        ...
        err = gf_state_execute_operation(OPERATION_TEST_SENSOR_FINE_STEP1, &cmd->reset_flag);
    } while (0);
...
static gf_error_t gf_ta_test_sensor_fine_finish(gf_cmd_header_t *cmd) {
    gf_error_t err = GF_SUCCESS;
...
gf_error_t gf_ta_test_chip_series_invoke_cmd_entry_point(uint32_t cmd_id, void *buffer) {
    ...
    switch (cmd_id) {
        ...
        case GF_CMD_TEST_RAWDATA_SATURATED: {
            err = gf_ta_test_rawdata_saturated((gf_cmd_header_t *) buffer);
            break;
        }

        case GF_CMD_TEST_SENSOR_FINE: {
            err = gf_ta_test_sensor_fine((gf_cmd_header_t *) buffer);
            break;
        }

        case GF_CMD_TEST_SENSOR_FINE_FINISH: {
            err = gf_ta_test_sensor_fine_finish((gf_cmd_header_t *) buffer);
            break;
        }
...
gf_error_t gf_ta_test_chip_series_driver_cmd(gf_test_driver_cmd_t *cmd) {
    ...
    do {
        ...
        if (0 == cpl_strncmp((const int8_t*) cmd->cmd_buf, (const int8_t*) "erase_fw",
                cpl_strlen((const int8_t*) "erase_fw"))) {
            err = gf_oswego_m_test_erase_fw();
        } else if (0 == cpl_strncmp((const int8_t*) cmd->cmd_buf, (const int8_t*) "esd_exception",
            ...
            if (g_test_esd_exception_count > 10) {
                ...
                (const int8_t*) "write_product_info",
                cpl_strlen((const int8_t*) "write_product_info"))) {
            err = gf_oswego_m_write_product_info(cmd->product_cfg_idx);
        }
    } while (0);
