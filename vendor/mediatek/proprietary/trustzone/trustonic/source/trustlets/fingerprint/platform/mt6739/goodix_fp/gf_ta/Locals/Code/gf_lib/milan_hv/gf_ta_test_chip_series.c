...
gf_error_t gf_ta_test_interrupt_pin(gf_cmd_header_t *cmd) {
    ...
    do {
        ...
        err = gf_hw_generate_image_irq_by_manual();
    } while (0);
...
gf_error_t gf_ta_test_get_version(gf_test_get_version_t *cmd) {
    ...
    do {
        ...
        err = gf_hw_get_vendor_id(vendor_id, 64);
        if (err != GF_SUCCESS) {
            break;
        }
...
gf_error_t gf_ta_test_bad_point(gf_cmd_header_t *cmd) {
    ...
    do {
        ...
        err = gf_state_execute_operation(OPERATION_TEST_BAD_POINT_RECODE_BASE, &cmd->reset_flag);
    } while (0);
