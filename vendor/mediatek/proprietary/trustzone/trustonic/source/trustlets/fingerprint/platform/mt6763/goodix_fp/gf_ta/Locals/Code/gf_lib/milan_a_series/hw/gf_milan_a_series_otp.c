...
static uint8_t otp_crc[SIZE_OTP] = { 0 };
...
static gf_chip_type_t g_chip_type = GF_CHIP_5206;
...
uint16_t milan_a_series_otp_get_checksum16(const uint8_t * usBuf, const uint16_t usLen) {
    ...
    for (i = 0; i < usLen; i++) {
        usCheckSum = usCheckSum + tmp_cfg[i];
    }
            ...
            if ((0 == ((otp_buffer[VERSION_OTP_ADD] & 0x70) >> 4)) && (otp_buffer[0x3F])) {
                ...
                for (index = 0; index < DAC_OTP_SIZE; index++) {
                    ...
                    if (index < 4) {  // \u7b2c\u4e00\u7ec4DAC
                        ...
                        int lub_otp_value = otp_buffer[0x3A + (index - 5) / 2];
                        int high_flag = (index - 1) % 2;
 ...
 *  get sensor type
 */
gf_error_t gf_milan_a_series_init_chip_type(uint8_t* otp_buffer) {
    ...
    do {
        ...
        GF_LOGD(LOG_TAG "[%s] chip_type=0x%02X, 0x%02X",
                __func__, otp_buffer[GF_MILAN_A_SERIES_OTP_CHIP_TYPE_HIGH],
                otp_buffer[GF_MILAN_A_SERIES_OTP_CHIP_TYPE_LOW]);
        if (0x80 != otp_buffer[GF_MILAN_A_SERIES_OTP_CHIP_TYPE_HIGH]) {
            ret = GF_ERROR_UNSUPPORT_CHIP;
            break;
        }

        switch (otp_buffer[GF_MILAN_A_SERIES_OTP_CHIP_TYPE_LOW]) {
            case 0x10:
                g_chip_type = GF_CHIP_5206;
                break;
            case 0x20:
                g_chip_type = GF_CHIP_5216;
                break;
            case 0x30:
                g_chip_type = GF_CHIP_5208;
                break;
            case 0x40:
                g_chip_type = GF_CHIP_5218;
                break;
            case 0x22:
                g_chip_type = GF_CHIP_GX556;
                break;
            default:
...
static gf_error_t milan_a_series_get_otp_info(uint8_t *buf) {
    ...
    do {
        ...
        for (offset = 0; offset < GF_MILAN_A_SERIES_OTP_INFO_LENGTH; offset++) {
            ...
            if (offset < 0x20) {
                ret = gf_milan_a_series_secspi_write_word(GF_MILAN_A_SERIES_OTP_INFO_CONTROL,
                        control_value_1);
                if (GF_SUCCESS != ret) {
                    GF_LOGE(LOG_TAG "[%s] :write 0x%x failed, ret=%d.",
                            __func__, GF_MILAN_A_SERIES_OTP_INFO_CONTROL, ret);
                    break;
                }
            }
            ...
            if (GF_SUCCESS != ret) {
                GF_LOGE(LOG_TAG "[%s] :write 0x%x failed, offset:%d, ret = %d.",
                        __func__, control_reg, read_value, ret);
                break;
            }
            ...
            GF_LOGI(LOG_TAG "[%s]org 0x%X, otp_info[%d]=%X.",
                    __func__, read_value, offset, otp_info[offset]);
        }
...
gf_chip_type_t gf_milan_a_series_get_chip_type(void) {
    return g_chip_type;
}
...
/*uint8_t otp_buffer[SIZE_OTP] = {0};*/

gf_error_t gf_milan_a_series_otp_check_crc(uint8_t *otp_buf) {
    ...
    do {
        ...
        if (GF_SUCCESS == err) {
            GF_LOGD(LOG_TAG "[%s] success to get otp", __func__);
            if (otp_buf[SENSOR_OTP_TYPE_OFFSET] == SENSOR_OTP_TYPE_STANDARD) {
                GF_LOGD(LOG_TAG "[%s] standar sensor otp[0x39] 0x%x.", __func__,
                        otp_buf[SENSOR_OTP_TYPE_OFFSET]);
                break;
            }
            crc = milan_a_series_otp_get_crc8(otp_crc, GF_MILAN_A_SERIES_OTP_INFO_LENGTH - 1);

            // HV & LV compatible
            if (otp_buf[HV_LV_SWITCH_OFFSET] == 0xFF) {
                GF_LOGD(LOG_TAG
                        "[%s] not check otp crc for special LV sensor HV_LV_SWITCH_OFFSET[%d]",
                        __func__, otp_buf[HV_LV_SWITCH_OFFSET]);
                break;
            }

            if (otp_buf[CRC_OFFSET] == crc) {
                GF_LOGD(LOG_TAG "[%s] otp crc check passed, read crc=0x%02X, calculator=0x%02X",
                        __func__, otp_buf[0x1F], crc);
                break;
            } else {
                GF_LOGE(LOG_TAG "[%s] otp crc check failed, read crc=0x%02X, calculator=0x%02X",
                        __func__, otp_buf[0x1F], crc);
            }

            // add sensor_type 2 crc check
            cpl_memcpy(sensor_otp_buf, otp_crc, GF_MILAN_A_SERIES_OTP_INFO_LENGTH);
            ...
            if (otp_buf[CRC_OFFSET] == crc) {
                GF_LOGI(LOG_TAG "[%s] sensor type 2 otp crc check passed", __func__);
                GF_LOGI(LOG_TAG "[%s] read crc=0x%02X, calculator=0x%02X",
                        __func__, otp_buf[0x1F], crc);
                break;
            }
            ...
            GF_MILAN_A_SERIES_OTP_INFO_LENGTH - 1);
            if (otp_buf[CRC_OFFSET] == crc) {
                GF_LOGI(LOG_TAG "[%s] sensor type 2(0x16 to 0) otp crc check passed", __func__);
                GF_LOGI(LOG_TAG "[%s] read crc=0x%02X, calculator=0x%02X",
                        __func__, otp_buf[0x1F], crc);
                break;
            }
        ...
        retry++;
        GF_LOGE(LOG_TAG "[%s] failed to get otp info, retry=%u times", __func__, retry);
