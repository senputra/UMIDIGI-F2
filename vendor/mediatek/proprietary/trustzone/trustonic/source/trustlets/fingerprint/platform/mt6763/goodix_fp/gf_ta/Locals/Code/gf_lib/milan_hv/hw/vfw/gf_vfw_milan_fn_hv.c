...
#define MILAN_FN_HV_OTP_CRC_LEN 31

#define LOG_TAG "[gf_vfw_milan_fn_hv]"

/* 0x00~0x18, 0x1A, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F.  31 Bytes */
uint8_t vfw_milan_fn_hv_get_otp_data_crc(uint8_t* otp_buf) {
    uint8_t crc = 0;
    ...
    crc = gf_milan_hv_get_crc8(temp_data, MILAN_FN_HV_OTP_CRC_LEN);

    return crc;
}

gf_error_t vfw_milan_fn_hv_set_general_reg(void) {
    gf_error_t err = GF_SUCCESS;

    do {
        uint16_t reg_data = 0x0000;
        err = gf_milan_hv_write_word_with_endian_exchange(MILAN_HV_REG_ESD_DETECT, reg_data);
        if (GF_SUCCESS != err) {
            break;
        }

        reg_data = 0x1111;
        err = gf_milan_hv_write_word_with_endian_exchange(MILAN_HV_REG_GPIO_CTRL, reg_data);
        if (GF_SUCCESS != err) {
            break;
        }
...
gf_error_t vfw_milan_fn_hv_update_dac_to_regs(uint16_t dac_groups[]) {
    gf_error_t err = GF_SUCCESS;
    uint16_t reg_data = 0;
    FUNC_ENTER();
