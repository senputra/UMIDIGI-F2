...
static uint8_t stable_acquire_state = 0;
...
{ 0, MODE_IMAGE },                      /* OPERATION_TEST_CHECK_FINGER_EVENT, 36*/
...
{ 0, MODE_DEBUG},                       /* OPERATION_TEST_INTERRUPT_PIN, 43*/
{ 2, MODE_IMAGE },                      /* OPERATION_TEST_PRE_SPI 44*/
...
void gf_state_set_mode(gf_operation_type_t operation, uint8_t *reset_flag) {
    ...
    do {
        ...
        if (1 == stable_acquire_state) {
            err = gf_milan_a_series_set_config_for_stable_acquire_image(0);
            if (GF_SUCCESS == err) {
                g_is_normal_cfg = 1;
                stable_acquire_state = 0;
            }
        }
        ...
        if ((OPERATION_ENROLL == operation) || (OPERATION_TEST_FRR_FAR_RECORD_ENROLL == operation)
                || (OPERATION_TEST_UNTRUSTED_ENROLL == operation)) {
            err = gf_milan_a_series_set_config_for_stable_acquire_image(1);
            if (GF_SUCCESS == err) {
                g_is_normal_cfg = 0;
                stable_acquire_state = 1;
            }
        }
        /*if (((MODE_NAV == mode)
                && (GF_NAV_EVENT_DETECT_METHOD_POLLING == g_config.support_nav_get_data_method))
                || (MODE_FF == mode) || (MODE_IMAGE == mode)) {
