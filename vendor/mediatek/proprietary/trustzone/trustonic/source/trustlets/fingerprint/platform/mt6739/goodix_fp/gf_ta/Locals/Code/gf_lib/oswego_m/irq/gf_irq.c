...
#define UNDER_SATURATED_THRESHOLD 100
#define OVER_SATURATED_THRESHOLD 4000
...
static gf_error_t irq_test_rawdata_saturated(gf_irq_t *irq);
static gf_error_t irq_test_interrupt_pin(gf_irq_t *irq);
static gf_error_t irq_test_sensor_fine_step1(gf_irq_t *irq);
static gf_error_t irq_test_sensor_fine_step2(gf_irq_t *irq);
...
static gf_error_t irq_process(gf_irq_t *irq) {
    ...
    switch (irq->operation) {
        ...
        case OPERATION_TEST_SENSOR_FINE_STEP1:
            err = irq_test_sensor_fine_step1(irq);
            break;

        case OPERATION_TEST_SENSOR_FINE_STEP2:
            err = irq_test_sensor_fine_step2(irq);
            break;

        case OPERATION_TEST_RAWDATA_SATURATED:
            err = irq_test_rawdata_saturated(irq);
            break;
...
static gf_error_t check_rawdata_saturated(gf_test_rawdata_saturated_t *test_rawdata_saturated,
        uint32_t irq_type) {
    gf_error_t err = GF_SUCCESS;
    uint32_t i = 0;
    uint16_t under_saturated_pixel_count = 0;
    uint16_t over_saturated_pixel_count = 0;
    uint16_t *pixel = g_image_data->raw_data;
    uint32_t saturated_pixel_threshold = 0;
    ...
    do {
        ...
        for (; i < g_image_data->raw_data_len; i++) {
            if (*pixel < UNDER_SATURATED_THRESHOLD) {
                under_saturated_pixel_count++;
            } else if (*pixel > OVER_SATURATED_THRESHOLD) {
        ...
        if (irq_type & GF_IRQ_NAV_MASK) {
            saturated_pixel_threshold = g_sensor.nav_col * g_sensor.nav_row / 10;
        } else {
            saturated_pixel_threshold = g_sensor.col * g_sensor.row / 10;
        }

        GF_LOGD(LOG_TAG "[%s] the saturated pixel threshold num=%u",
                __func__, saturated_pixel_threshold);

        if (test_rawdata_saturated != NULL) {
            test_rawdata_saturated->under_saturated_pixel_count = under_saturated_pixel_count;
            test_rawdata_saturated->over_saturated_pixel_count = over_saturated_pixel_count;
            test_rawdata_saturated->saturated_pixel_threshold = saturated_pixel_threshold;
        }

        if (under_saturated_pixel_count > saturated_pixel_threshold) {
            err = GF_ERROR_TOO_MUCH_UNDER_SATURATED_PIXELS;
            GF_LOGE(LOG_TAG "[%s] too much under saturated pixels, under_saturated_pixel_count=%u",
                    __func__, under_saturated_pixel_count);
        }
        if (over_saturated_pixel_count > saturated_pixel_threshold) {
            err = GF_ERROR_TOO_MUCH_OVER_SATURATED_PIXELS;
            GF_LOGE(LOG_TAG "[%s] too much over saturated pixels, over_saturated_pixel_count=%u",
                    __func__, over_saturated_pixel_count);
        }
    } while (0);
...
static gf_error_t irq_test_sensor_fine_step1(gf_irq_t *irq) {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_IMAGE_MASK) {
            gf_sensor_fine_test_step1(g_image_data->raw_data, g_image_data->raw_data_len);
            err = gf_state_execute_operation(OPERATION_TEST_SENSOR_FINE_STEP2,
...
static gf_error_t irq_test_sensor_fine_step2(gf_irq_t *irq) {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_IMAGE_MASK) {
            gf_sensor_fine_test_step2(g_image_data->raw_data, g_image_data->raw_data_len);
            err = gf_sensor_fine_test(&(irq->test_sensor_fine.average_pixel_diff));
        }
    } while (0);
...
static gf_error_t irq_test_rawdata_saturated(gf_irq_t *irq) {
    gf_error_t err = GF_SUCCESS;
    static uint8_t rawdata_saturated_time = 1;
    FUNC_ENTER();
    do {
        if (NULL == irq) {
            err = GF_ERROR_BAD_PARAMS;
        ...
        if (irq->irq_type & GF_IRQ_IMAGE_MASK) {
            err = check_rawdata_saturated(&(irq->test_rawdata_saturated), irq->irq_type);
            if (err != GF_SUCCESS) {
                if (rawdata_saturated_time < 3) {
                    gf_hw_data_sample_resume();
                    rawdata_saturated_time++;
                    irq->irq_type &= (~GF_IRQ_IMAGE_MASK);
                    break;
                }
                GF_LOGE(LOG_TAG "[%s] test rawdata saturated failed. err=%s, errno=%d",
                        __func__, gf_strerror(err), err);
                rawdata_saturated_time = 1;
                gf_hw_data_sample_suspend();
            }
        }
...
static gf_error_t irq_test_spi_performance(gf_irq_t *irq) {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_IMAGE_MASK) {
            ...
            err = gf_oswego_m_get_fw_version(fw_version, 10);
...
static gf_error_t irq_test_real_time_data(gf_irq_t *irq) {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_IMAGE_MASK) {
            err = gf_hw_read_register(0x8030, irq->test_real_time_data.fpc_key_data,
                    FPC_KEY_DATA_LEN);
            if (err != GF_SUCCESS) {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_IMAGE_MASK) {
            if (OPERATION_TEST_PIXEL_OPEN_STEP1 == irq->operation) {
                ...
                err = gf_state_execute_operation(OPERATION_TEST_KEY_MODE,
                        &irq->cmd_header.reset_flag);
                err = gf_oswego_m_download_cfg(CONFIG_TEST_PIXEL_OPEN_B);
                g_is_normal_cfg = 0;
                ...
                err = gf_pixel_test(&(irq->bad_pixel_num));
                gf_pixel_test_cancel();
                err = gf_state_execute_operation(OPERATION_TEST_KEY_MODE,
                        &irq->cmd_header.reset_flag);
                g_is_normal_cfg = 0;
            } else {
                GF_LOGE(LOG_TAG "[%s] no such a operation in test open short, operation=%s, "
                        "operation_code=%d", __func__, gf_stroperation(irq->operation),
                        irq->operation);
            }
        }
    } while (0);
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_NAV_MASK) {
            ...
            if (irq->nav_result.finger_up_flag > 0 || irq->nav_result.nav_code != GF_NAV_NONE) {
                ...
                if (0 == (irq->irq_type & GF_IRQ_FINGER_UP_MASK)) {
                    uint32_t irq_type = 0;
...
static gf_error_t irq_nav_base(gf_irq_t *irq) {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_NAV_MASK) {
            static uint8_t saturate_time_nav = 1;
            err = check_rawdata_saturated(NULL, irq->irq_type);
            if (err == GF_SUCCESS) {
                ...
                gf_nav_set_base(g_nav_data->raw_data[0], g_nav_data->raw_data_len);
            } else if (saturate_time_nav < 3) {
                gf_hw_data_sample_resume();
                saturate_time_nav++;
                break;
            } else {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_IMAGE_MASK) {
            if (OPERATION_FINGER_BASE == irq->operation) {
                static uint8_t saturate_time = 1;
                err = check_rawdata_saturated(NULL, irq->irq_type);
                if (err == GF_SUCCESS) {
                    err = gf_base_update_finger_base(g_image_data->raw_data,
                            g_image_data->raw_data_len,
                            &irq->cmd_header.reset_flag);
                    saturate_time = 3;
                } else if (saturate_time < 3) {
                    gf_hw_data_sample_resume();
                    saturate_time++;
                    break;
                } else {
...
static gf_error_t irq_enroll(gf_irq_t *irq) {
    ...
    do {
        ...
        if (irq->irq_type & GF_IRQ_FINGER_UP_MASK) {
            if ((g_config.support_sensor_broken_check > 0)
                    && (irq->operation != OPERATION_TEST_UNTRUSTED_ENROLL)) {
                gf_broken_check_on_up_irq(g_image_data->raw_data, g_image_data->raw_data_len, irq);
            }
...
gf_error_t gf_irq(gf_irq_t *cmd) {
    ...
    do {
        ...
        if ((GF_IRQ_STEP_GET_IRQ_TYPE == cmd->step) || (GF_IRQ_STEP_POLLING == cmd->step)) {
            ...
            if (cmd->irq_type & (GF_IRQ_NAV_MASK | GF_IRQ_IMAGE_MASK)) {
                if (OPERATION_TEST_INTERRUPT_PIN == cmd->operation) {
                    GF_LOGD(LOG_TAG "[%s] For interrupt pin test, chip mode is debug. "
                            "So debug mode should return image irq", __func__);
                    gf_hw_clear_irq(irq_type);
        ...
        if (GF_IRQ_STEP_PRE_GET_IMAGE == cmd->step) {
            ...
            if (OPERATION_NAV == cmd->operation) {
                ...
                err = gf_oswego_m_pre_get_nav_image();
            } else if (OPERATION_NAV_BASE == cmd->operation) {
                cpl_memset(g_nav_data, 0, sizeof(gf_nav_data_t));
                err = gf_oswego_m_pre_get_nav_base();
            } else {
                gf_get_time_delta();
                // TODO(goodix): can't memset g_image_data here, because broken check rawdata will
                // be erased
                // cpl_memset(g_image_data, 0, sizeof(gf_image_data_t));
                err = gf_oswego_m_pre_get_image();
            }
            ...
            if (0 == g_config.support_set_spi_speed_in_tee) {
                gf_oswego_m_get_spi_speed(cmd->irq_type, &cmd->speed);
                break;
            }
        }
        ...
        if (GF_IRQ_STEP_GET_IMAGE == cmd->step) {
            ...
            if (OPERATION_NAV == cmd->operation) {
                err = gf_oswego_m_get_nav_image(g_nav_origin_data->data[0],
                ...
                do {
                    err = gf_oswego_m_get_nav_base(g_nav_origin_data->data[0],
                            &g_nav_origin_data->data_len);
                    if (GF_SUCCESS == err) {
                        checksum_times = 0;
                        break;
                ...
                do {
                    err = gf_oswego_m_get_image(g_image_origin_data->data,
        ...
        if (GF_IRQ_STEP_POST_GET_IMAGE == cmd->step) {
            ...
            if (OPERATION_NAV == cmd->operation) {
                err = gf_oswego_m_post_get_nav_image(g_nav_data->raw_data[0],
                ...
                err = gf_oswego_m_post_get_nav_base(g_nav_data->raw_data[0],
                ...
                err = gf_oswego_m_post_get_image(g_image_data->raw_data,
                        &g_image_data->raw_data_len);
                if (err != GF_SUCCESS) {
                    break;
                }
    ...
    if (GF_SUCCESS == err) {
        ...
        if (OPERATION_NAV_BASE == cmd->operation || OPERATION_NAV == cmd->operation) {
