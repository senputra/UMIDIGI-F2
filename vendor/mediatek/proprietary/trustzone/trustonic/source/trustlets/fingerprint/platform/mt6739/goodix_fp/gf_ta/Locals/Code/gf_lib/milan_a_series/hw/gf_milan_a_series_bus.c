...
void gf_milan_a_series_bus_init_global_variable(void) {
...
static gf_error_t gf_milan_a_series_secspi_write(uint16_t addr, uint8_t *buf, uint32_t buf_len) {
    ...
    do {
        ...
        if (buf != NULL) {
            ...
            gf_hw_endian_exchange(g_tx_buf.buf, buf_len);  // change the read data to little endian.
        }
...
//    uint32_t remain_len = 0;
    ...
    do {
        ...
        for (; retry_times < SPI_RETRY_COUNT; retry_times++) {
            ...
            if (GF_SUCCESS != err) {
                GF_LOGE(LOG_TAG "[%s] spi address send error", __func__);
                break;
            }
...
gf_error_t gf_milan_a_series_secspi_read_bytes_with_speed(uint16_t addr, uint8_t *buf,
