...
gf_error_t gf_tee_object_exist(uint8_t *object_id) {
    ...
    do {
        ...
        err = TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, object_id, strlen((const char*)object_id),
                TEE_HANDLE_FLAG_PERSISTENT, &object_handle);
        if (TEE_SUCCESS != err) {
...
gf_error_t gf_tee_open_object(uint8_t *object_id, gf_tee_mode_t mode, GF_TEE_HANDLE *so_handle) {
    ...
    do {
        ...
        if ((NULL == object_id) || (NULL == so_handle)
            ...
            GF_LOGE(
                    LOG_TAG "[%s] invalid parameter, object_id:0x%08X, mode:%d, so_handle:0x%08X", __func__, (uint32_t)object_id, mode, (uint32_t)so_handle);
            err = GF_ERROR_BAD_PARAMS;
            break;
        }

        err = TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, object_id, strlen((const char*)object_id),
                TEE_HANDLE_FLAG_PERSISTENT, (TEE_ObjectHandle *) so_handle);
        if (TEE_SUCCESS == err) {
        ...
        if (SO_MODE_WRITE == mode) {
            err = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, object_id, strlen((const char*)object_id),
                    TEE_HANDLE_FLAG_PERSISTENT, TEE_HANDLE_NULL, NULL, 0,
...
gf_error_t gf_tee_get_object_size(GF_TEE_HANDLE so_handle, uint32_t *object_size) {
    ...
    do {
        ...
        if ((NULL == so_handle) || (NULL == object_size)) {
            GF_LOGE(
                    LOG_TAG "[%s] invalid parameter, so_handle:0x%08X, object_size:0x%08X", __func__, (uint32_t)so_handle, (uint32_t)object_size);
            err = GF_ERROR_BAD_PARAMS;
            break;
        }
...
gf_error_t gf_tee_read_object_data(GF_TEE_HANDLE so_handle, uint8_t *buffer, uint32_t size) {
    ...
    do {
        ...
        if ((NULL == so_handle) || (NULL == buffer)) {
            GF_LOGE(
                    LOG_TAG "[%s] invalid parameter, so_handle:0x%08X, buffer:0x%08X", __func__, (uint32_t)so_handle,(uint32_t) buffer);
            err = GF_ERROR_BAD_PARAMS;
            break;
        }
...
gf_error_t gf_tee_write_object_data(GF_TEE_HANDLE so_handle, uint8_t *buffer, uint32_t size) {
    ...
    do {
        if ((NULL == so_handle) || (NULL == buffer) || (size == 0)) {
            GF_LOGE(
                    LOG_TAG "[%s] invalid parameter, so_handle:0x%08X, buffer:0x%08X, size:%d", __func__, (uint32_t)so_handle, (uint32_t)buffer, size);
            err = GF_ERROR_BAD_PARAMS;
            break;
        }
...
gf_error_t gf_tee_delete_object(uint8_t *object_id) {
    ...
    do {
        ...
        if (NULL == object_id) {
            GF_LOGE( LOG_TAG "[%s] invalid parameter, object_id:0x%08X", __func__, (uint32_t)object_id);
            err = GF_ERROR_BAD_PARAMS;
            break;
        }
