...
void gf_config_init(void) {
    ...
    /***************** chip type *****************/
#if defined(GF316M)
    g_config.chip_type = GF_CHIP_316M;
#elif defined(GF318M)
    g_config.chip_type = GF_CHIP_318M;
#elif defined(GF3118M)
    g_config.chip_type = GF_CHIP_3118M;
#elif defined(GF516M)
    g_config.chip_type = GF_CHIP_516M;
#elif defined(GF518M)
    g_config.chip_type = GF_CHIP_518M;
#elif defined(GF5118M)
    g_config.chip_type = GF_CHIP_5118M;
#elif defined(GF816M)
...
void gf_config_init_by_chip_type(gf_chip_type_t chip_type) {
    ...
    if ((GF_CHIP_316M == chip_type) || (GF_CHIP_516M == chip_type) || (GF_CHIP_816M == chip_type)) {
        g_config.enrolling_min_templates = 10;
    } else if ((GF_CHIP_318M == chip_type) || (GF_CHIP_3118M == chip_type)
            || (GF_CHIP_518M == chip_type) || (GF_CHIP_5118M == chip_type)) {
...
#else
    g_config.max_authenticate_failed_attempts = 5;
#endif
    ...
    /************** bad point test ***************/
    g_config.bad_point_test_max_frame_number = 2;
