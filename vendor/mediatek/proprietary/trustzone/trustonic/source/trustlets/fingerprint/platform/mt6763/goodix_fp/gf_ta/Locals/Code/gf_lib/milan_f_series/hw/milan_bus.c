...
#define LOG_TAG "[milan_bus]"

#define GF_SPI_MTK_PACKAGE_LEN    (1 << 10)
#define GF_SPI_PACKAGE_LEN      ((GF_RAWDATA_LEN + 4) * 5 + 1)
#define GF_SPI_TX_HEADER_LEN  5
...
static gf_error_t gf_milan_secspi_write(uint16_t addr, uint8_t *buf, uint32_t buf_len) {
    gf_error_t err = GF_SUCCESS;
    uint32_t retry_times = 0;

    do {
        if ((buf_len > GF_SPI_TX_PACKAGE_LEN) || (buf_len % 2 != 0)) {
 ...
 */
static gf_error_t gf_milan_secspi_read(uint16_t addr, uint8_t *buf, uint32_t buf_len,
        uint32_t speed) {
    gf_error_t err = GF_SUCCESS;
    uint32_t retry_times = 0;
...
gf_error_t gf_milan_secspi_write_word(uint16_t addr, uint16_t value) {
    ...
    err = gf_milan_secspi_write(addr, (uint8_t *) &data, sizeof(uint16_t));

    return err;
}

gf_error_t gf_milan_secspi_write_bytes(uint16_t addr, uint8_t *buf, uint32_t buf_len) {
    return gf_milan_secspi_write(addr, buf, buf_len);
}
...
gf_error_t gf_milan_secspi_read_word(uint16_t addr, uint16_t *value) {
    ...
    err = gf_milan_secspi_read(addr, (uint8_t *) &data, sizeof(uint16_t), GF_SPI_SPEED_LOW);
...
gf_error_t gf_milan_secspi_read_bytes(uint16_t addr, uint8_t *buf, uint32_t buf_len) {
    return gf_milan_secspi_read(addr, buf, buf_len, GF_SPI_SPEED_LOW);
}

gf_error_t gf_milan_secspi_read_bytes_by_speed(uint16_t addr, uint8_t *buf, uint32_t buf_len,
        uint32_t speed) {
    if (speed == 0) {
        return gf_milan_secspi_read(addr, buf, buf_len, GF_SPI_SPEED_LOW);
    }
    return gf_milan_secspi_read(addr, buf, buf_len, speed);
}

gf_error_t gf_milan_reg_access(milan_reg_access_t *reg_data) {
    gf_error_t err = GF_SUCCESS;
    uint8_t temp_buffer[MILAN_REG_MAX_SIZE] = { 0 };
    FUNC_ENTER();
    if (reg_data->reg_size > sizeof(temp_buffer)) {
        GF_LOGE(LOG_TAG "[%s] illegal register size", __func__);
        err = GF_ERROR_BAD_PARAMS;
        return err;
    }

    if (reg_data->write) {
        cpl_memcpy(temp_buffer, reg_data->dataptr, reg_data->reg_size);
        gf_hw_endian_exchange(temp_buffer, reg_data->reg_size);
        err = gf_milan_secspi_write_bytes(reg_data->reg, temp_buffer, reg_data->reg_size);
    } else {
        err = gf_milan_secspi_read_bytes(reg_data->reg, temp_buffer, reg_data->reg_size);
        gf_hw_endian_exchange(temp_buffer, reg_data->reg_size);
        cpl_memcpy(reg_data->dataptr, temp_buffer, reg_data->reg_size);
    }

    GF_LOGD(LOG_TAG "[%s] %s=0x%04x(%d bytes), %s=0x%02X 0x%02X 0x%02X 0x%02X",
            __func__, reg_data->write ? "WRITE" : "READ", reg_data->reg, reg_data->reg_size,
            err == GF_SUCCESS ? "SUCCESS" : "FAIL",
            reg_data->reg_size > 0 ? reg_data->dataptr[0] : 0,
            reg_data->reg_size > 1 ? reg_data->dataptr[1] : 0,
            reg_data->reg_size > 2 ? reg_data->dataptr[2] : 0,
            reg_data->reg_size > 3 ?reg_data->dataptr[3] : 0);
    FUNC_EXIT(err);
    return err;
}
