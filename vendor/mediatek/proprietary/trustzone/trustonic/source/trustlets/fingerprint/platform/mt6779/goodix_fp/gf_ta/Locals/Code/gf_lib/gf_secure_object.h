...
#define GF_FINGER_SECURE_ID             "auth_token.so"

#define GF_OTP_INFO_SECURE_ID           "gf_otp_info.so"

#define GF_FINGER_OBJECT_ID_MAX_LEN     64

#define GF_TEST_FRR_FAR_FINGER_OBJECT_ID             "test_frr_far_finger_%d_%d.so"
#define GF_TEST_FRR_FAR_FINGER_LST_OBJECT_ID         "test_frr_far_finger_lst"
...
gf_error_t gf_so_get_auth_token_pdu(uint8_t *buf, uint32_t buf_len, uint64_t secure_user_id, uint64_t authenticator_id);
gf_error_t gf_so_load_auth_token(uint64_t *secure_user_id, uint64_t *authenticator_id, uint8_t *buf, uint32_t buf_len);
