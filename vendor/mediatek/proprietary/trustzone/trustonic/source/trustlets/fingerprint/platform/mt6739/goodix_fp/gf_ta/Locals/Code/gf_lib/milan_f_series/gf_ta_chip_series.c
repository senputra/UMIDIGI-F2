...
static gf_error_t gf_ta_detect_nav_event(gf_detect_nav_event_t *cmd) {
    ...
    do {
        ...
        for (; total_frame_num < g_config.nav_config.maxNvgFrameNum;) {
            err = gf_milan_get_nav_image(g_nav_origin_data->data[0], &g_nav_origin_data->data_len);
            if (GF_SUCCESS != err) {
                break;
            }

            err = gf_milan_post_get_nav_image(g_nav_data->raw_data[total_frame_num],
        ...
        if (total_frame_num > 0) {
            if (total_frame_num < g_config.nav_config.maxNvgFrameNum) {
                g_nav_data->finger_up_flag[total_frame_num - 1] = 1;
            } else {
    ...
    if ((0 == finger_up_flag) || (g_config.nav_double_click_interval_in_ms == 0)) {
    ...
    GF_LOGI(LOG_TAG "[%s] nav_code=%d, total_frame_num=%d, finger_up_flag=%u", __func__, nav_code,
            total_frame_num, finger_up_flag);
...
static gf_error_t gf_ta_init(gf_init_t *cmd) {
    ...
    do {
        err = gf_milan_chip_init(cmd->otp_info, &(cmd->otp_info_len));
        ...
        gf_hw_enable_encrypt(g_config.support_hw_encrypt);
    ...
    switch (cur_operation) {
        ...
        case OPERATION_TEST_PIXEL_OPEN_STEP2:
            ...
            if (0 == g_is_normal_cfg) {
                gf_milan_update_dac_for_sensor_pixel_open_test(1, 0);
            }
            g_is_normal_cfg = 1;
