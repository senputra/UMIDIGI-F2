...
#define UDATA_ENC_LEN    200000  /**< Size of the biggest pictures */
...
/* Secret PIN */
static const uint8_t secretPIN[] = {1, 2, 3, 4};
...
static tlApiResult_t static_initTui() {
    ...
    if (res != TLAPI_OK) {
        tlDbgPrintf(TLTAG "Get Screen Info failed. Cannot access TUI 0x%08X\n", res);
    } else {
...
static tlApiResult_t static_setNextChar(uint8_t keynum, uint8_t* const index) {
    ...
    tlApiTuiGraphicContext_t graphicContext = {0};
#endif
    ...
    sCoordinates.yOffset = layout->textarea.taTop + ((3-(*index)) * layout->textarea.taMargin);

#if TBASE_API_LEVEL >= 6
    res = tlApiTuiGetScreenContext(&graphicContext);
    if (res != TLAPI_OK) {
        tlDbgPrintf(TLTAG "tlApiTuiGetScreenContext failed with 0x%x.\n", res);
        return res;
    }

    res = tlApiTuiDrawImage(graphicContext, &sImage, sCoordinates.xOffset, sCoordinates.yOffset);
    if (res != TLAPI_OK) {
        tlDbgPrintf(TLTAG "tlApiTuiDrawImage failed with 0x%x.\n", res);
    } else {
        res = tlApiTuiFlipFramebuffers();
        if (res != TLAPI_OK) {
            tlDbgPrintf(TLTAG "tlApiTuiFlipFramebuffers failed with 0x%x.\n", res);
        }
    }
#else
 ...
 *
 * Show the negative picture of the button.
 *
 * @return  TLAPI_OK upon success or specific error
 */
 ...
 *
 * Show the picture of the released button.
 *
 * @return  TLAPI_OK upon success or specific error
 */
...
static transactionState_t processEventFromDriver(bool *tryAgain) {
    ...
    *tryAgain = true;
    ...
    static uint8_t currentTouchedButton = PAD_NOKEY_VAL;
...
#if TBASE_API_LEVEL >= 6
    if (ret == E_TLAPI_TUI_MISSED_EVENTS) {
        tlDbgPrintf(TLTAG "Touch overflow.\n", ret);
        return ST_IMAGE_SET;
    }
    ...
    if (pressed) {
        ...
        if (touched_key.val != currentTouchedButton) {
            ...
            /* Always release currentTouchedButton when the new touch event doesn't
             * match the currentTouchedButton
             */
...
static tciReturnCode_t processCmdProvisionImage(tciMessage_t* pmessage) {
    ...
    // should be in NWd memory.
    // Here destination is in NWd memory to make this example simple.
    ret = tlApiWrapObject(
            secureMemory,
            0,
            len,
...
static tciReturnCode_t processCmdLaunchPinPad(
    ...
    tlApiTuiImage_t sImage;
#if TBASE_API_LEVEL >= 6
    tlApiTuiImageInfo_t info = {0};
    tlApiTuiRawImage_t rawImage = {0};
    tlApiTuiGraphicContext_t graphicContext = {0};
#endif
    tlApiTuiCoordinates_t sCoordinates;
...
#if TBASE_API_LEVEL >= 6
    res = tlApiTuiGetImageInfo(&sImage, &info);
    if (res != TLAPI_OK) {
        tlDbgPrintf(TLTAG "tlApiTuiGetImageInfo failed with 0x%x.\n", res);
        ret = RET_ERR_PINPAD_NO_RESOURCE;
        goto exit;
    }

    rawImage.data = tlApiMalloc(info.decodingSize, 0);
    if (NULL == rawImage.data) {
        rawImage.size = 0;
        tlDbgPrintf(TLTAG "tlApiMalloc failed 0x%x for 0x%x.\n", res, info.decodingSize);
        tlDbgPrintf(TLTAG "falling back to legacy tlApiTuiSetImage()\n");
    }
    else {
        rawImage.size = info.decodingSize;
    }

    /*
     * UX optimization to do tlApiTuiDecodeImage before doing the
     * tlApiTuiOpenSession which will black the screen. tlApiTuiDrawBuffer
     * after the session is there to complement.
     *
     * Could take this approach with all images if desired, for example.
     */
    if (rawImage.data) {
        res =  tlApiTuiDecodeImage(&sImage, &rawImage);
        tlDbgPrintf(TLTAG "rawImage:\n\t.width=%d\n\t.height=%d\n\t.stride=%d\n",
                    rawImage.width, rawImage.height, rawImage.stride, res);

        if (res != TLAPI_OK) {
            tlDbgPrintf(TLTAG "tlApiTuiDecodeImage failed with 0x%x.\n", res);
            ret = RET_ERR_PINPAD_NO_RESOURCE;
            goto exit_free;
        }
    }
    ...
    tlDbgPrintf(TLTAG "TUI session is opened, draw image/buffer.\n");
    *pState = ST_INITIALIZED;
    ...
    if (rawImage.data) {
        // No clipping, we want the full image
        res = tlApiTuiDrawBuffer(graphicContext,
                &rawImage,
                sCoordinates.xOffset,
                sCoordinates.yOffset);
        if (res != TLAPI_OK) {
            tlDbgPrintf(TLTAG "tlApiTuiDrawBuffer failed with 0x%x.\n", res);
            ret = RET_ERR_PINPAD_NO_RESOURCE;
            goto exit_free;
        } else {
            res = tlApiTuiFlipFramebuffers();
    ...
    else { // malloc failed, so we could not decode the image (not enough memory
           //in the workspace). fallback to the legacy setImage
        res = tlApiTuiSetImage(&sImage, sCoordinates);
        if (res != TLAPI_OK) {
            tlDbgPrintf(TLTAG "Set image failed with 0x%x, cancelling.\n", res);
        }
