...
int32_t cmdq_rec_initialize_cmd_buffer(void)
{
	...
	memcpy((void *)sec_mem_tag_buf, (uint8_t *)(tee_parm_vaddr + (TEE_PARAMETER_ADDR - TEE_PARAMETER_BASE)),
				TAG_STRUCT_MAX_SIZE);
	...
	if (IS_TAG_FMT((struct tz_tag*)(sec_mem_tag_ptr))) {
		...
		struct tz_tag* res_cmdq_tag = find_tag((unsigned long)sec_mem_tag_ptr, TZ_TAG_RESERVED_MEM_CMDQ_BUF);

		if (!IS_NULL_TAG(res_cmdq_tag)) {
			gCmdBufferBasePA = res_cmdq_tag->u.res_mem_cfg.start;
			gCmdqBufferShareSize = res_cmdq_tag->u.res_mem_cfg.size;
			CMDQ_LOG("gCmdBufferBasePA:0x%x gCmdqBufferShareSize:0x%x\n", gCmdBufferBasePA, gCmdqBufferShareSize);
		} else {
			CMDQ_ERR("get cmd buffer base and size failed, res_cmdq_tag:%p\n", res_cmdq_tag);
		}
	} else {
		...
		CMDQ_LOG("legacy flow --> gCmdBufferBasePA:0x%x gCmdqBufferShareSize:0x%x\n", gCmdBufferBasePA, gCmdqBufferShareSize);
	}

	/* map this range as command buffer */
	status = cmdq_tz_map_physical_addr_by_flag(&gCmdBufferBaseVA, (uint64_t)gCmdBufferBasePA,
		gCmdqBufferShareSize, MAP_READABLE | MAP_WRITABLE | MAP_UNCACHED | MAP_IO);
	...
	if (!sec_mem_arg->shared_secmem) {
		...
		CMDQ_LOG(
			"ready to map cmd buffer va directly, gCmdBufferBasePA:0x%x gCmdqBufferShareSize:0x%x\n",
			gCmdBufferBasePA, gCmdqBufferShareSize);

		result = drApiMapPhysicalBuffer((uint64_t)gCmdBufferBasePA, gCmdqBufferShareSize, MAP_READABLE | MAP_WRITABLE, (void **)&gCmdBufferBaseVA);
		CMDQ_LOG("pure drApiMapPhysicalBuffer, result:%x\n", result);
		if (result != DRAPI_OK)
			CMDQ_ERR("drApiMapPhysicalBuffer error:(0x%x 0x%x 0x%x)\n", result, DRAPI_ERROR_MAJOR(result), DRAPI_ERROR_DETAIL(result));

		CMDQ_LOG(
			"ready to map cmd buffer by cmdq_tz_map_physical_addr_by_flag, gCmdBufferBasePA:0x%x gCmdqBufferShareSize:0x%x\n",
			gCmdBufferBasePA, gCmdqBufferShareSize);
		/* map this range as command buffer */
		status = cmdq_tz_map_physical_addr_by_flag(&gCmdBufferBaseVA, (uint64_t)gCmdBufferBasePA,
			gCmdqBufferShareSize, MAP_READABLE | MAP_WRITABLE);
	}
...
int32_t cmdqRecInitialize(void)
{
	...
	for (index = 0; index < CMD_MAX_FIXED_HANDLE; index++)
	{
		...
		CMDQ_LOG("handle[%d] pBuffer:0x%p\n", index, gRecordArray[index].pBuffer);
...
int32_t cmdq_rec_realloc_cmd_buffer(cmdqRecHandle handle, uint32_t size)
{
	...
	CMDQ_LOG("REC: realloc size from base:0x%p\n", handle->pBuffer);

	/* reset extended area */
	memset(handle->pBuffer, handle->bufferSize, size -handle->bufferSize);

	CMDQ_LOG("REC: realloc size from %d to %u bytes\n", handle->bufferSize, size);
