...
static U8 *write_byte, *read_byte;
...
exit_work_func:
	...
	{
		end_cmd = 0;
		if (i2c_write_bytes_dma_tui(&i2c, GTP_READ_COOR_ADDR, &end_cmd, 3) < 0) {
...
int map_i2c_dma_buffer(void)
{
	...
	TUI_LOGD("i2c dma mmap read pa=0x%x va=0x%x\n", read_byte_pa, read_byte);
	TUI_LOGD("i2c dma mmap write pa=0x%x va=0x%x\n", write_byte_pa, write_byte);
}
...
void set_and_map_i2c_dma_pa(uint32_t physAddr, uint32_t size)
{
	...
	TUI_LOGD("i2c dma write pa:0x%x read pa:0x%x\n", write_byte_pa, read_byte_pa);
	map_i2c_dma_buffer();
}
