                                  ...
                                  uint32_t size)
{
	...
	uint32_t mpu_attr;
	TUI_LOGD("%s physAddr=0x%08x%08x, size=0x%x\n", __func__, VALUE_64(physAddr), size);
	...
	if (protect_cnt == 2) {
		...
		TUI_LOGD("%s addr_start=0x%08x%08x, addr_end=%08x%08x\n", __func__, VALUE_64(addr_start),VALUE_64(addr_end));
                                    ...
                                    uint32_t size)
{
	...
	TUI_LOGD("%s addr=0x%08x%08x, size=0x%x\n", __func__, VALUE_64(physAddr), size);
    /*first address will be passed in third call*/
	if (protect_cnt==1 && mpu_protected) {
		...
		/* Clean memory */
		TUI_LOGD("%s addr_start=0x%08x%08x, addr_end=0x%08x%08x total=0x%x\n", __func__, VALUE_64(addr_start), VALUE_64(addr_end), total_mem_size);
