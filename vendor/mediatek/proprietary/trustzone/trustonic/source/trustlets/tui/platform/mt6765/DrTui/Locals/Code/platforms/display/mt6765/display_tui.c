...
int disp_m4u_init()
{
	...
	ret = drApiMapPhysicalBuffer(DDP_REG_BASE_SMI_LARB0, 0x1000,
	...
	ret = drApiMapPhysicalBuffer(DDP_REG_BASE_SMI_LARB1, 0x1000,
...
void* disp_tui_map_fb(uint64_t fb_pa, size_t size)
{
	...
	ret = drApiMapPhysicalBuffer(fb_pa, size, attr, &fb_va);
	if(ret != DRAPI_OK) {
...
{
	disp_tui_draw_buffer(disp_fb_va, lcm_width, lcm_height, fb_pitch, fb_Bpp);
	return 0;
}
...
int disp_tui_leave()
{
	...
	if (last_addr)
		tui_m4u_free(last_addr, X_RES * Y_RES * 4);
	last_addr = 0;
	tui_m4u_deinit();
