...
void cmdq_tz_print_hex_dump(const char *prefix_str, const void *buf, size_t len)
{
	...
	drDbgPrintf("%s hex dump, buf: %p, len: %d\n", prefix_str, buf, len);
	for (i = 0; i < len; i += rowsize) {
		...
		{
		case 3:
			drDbgPrintf("%s %p: %08x %08x %08x\n", prefix_str, ptr, ptr[0], ptr[1], ptr[2]);
			break;
		case 2:
			drDbgPrintf("%s %p: %08x %08x\n", prefix_str, ptr, ptr[0], ptr[1]);
			break;
		case 1:
			drDbgPrintf("%s %p: %08x\n", prefix_str, ptr, ptr[0]);
			break;
		case 0:
			drDbgPrintf("%s %p:  %08x %08x %08x %08x\n", prefix_str, ptr, ptr[0], ptr[1], ptr[2], ptr[3]);
			break;
		default:
			break;
		}
...
{
	drApiResult_t drRes = drApiMapPhysicalBuffer(pa, length, flag, (void **)va);

	if (drRes != DRAPI_OK) {
		CMDQ_ERR("%s drApiMapPhysicalBuffer(0x%p, 0x%08x%x, 0x%x, 0x%x) = ERR(%x, %x:%x)\n",
			__func__, *va, (uint32_t)(pa >> 32), (uint32_t)pa, length, flag, drRes, DRAPI_ERROR_MAJOR(drRes), DRAPI_ERROR_DETAIL(drRes));
		return -(CMDQ_REC_ERR_NOMEM);
	} else {
		CMDQ_LOG("%s drApiMapPhysicalBuffer(0x%p, 0x%08x%x, 0x%x, 0x%x) = STATUS(%x)\n",
			__func__, *va, (uint32_t)(pa >> 32), (uint32_t)pa, length, flag, drRes);
	}
...
int32_t cmdq_tz_map_physical_addr(uint8_t **va, const uint64_t pa, const uint32_t length)
{
	...
	return cmdq_tz_map_physical_addr_by_flag(va, pa, length, flag);
}

int32_t cmdq_tz_unmap_physical_addr(const uint8_t *va, const uint32_t length)
{
	const drApiResult_t status = drApiUnmapBuffer((void *)va);

	if (DRAPI_OK != status) {
		CMDQ_ERR("unmap_physical_addr va:0x%p size:0x%08x err:%x, %x:%x\n",
...
int32_t cmdqTzInitialize(void)
{
	...
	if(0 > cmdq_tz_init_module_base_VA())
	{
		...
		CMDQ_LOG("INIT: map module to VA base 0x%p\n", gCmdqRegBaseVA);
	}
...
ThreadStruct* cmdq_tz_get_thread_struct_by_id(const int32_t thread, const int32_t line)
{
	if (thread < 0 || thread >= CMDQ_MAX_THREAD_COUNT) {
		CMDQ_ERR("Thread: %d get failed line:%d\n", thread, line);
		return NULL;
	}
...
void cmdq_tz_dump_thread(uint32_t thread, uint32_t line)
{
	...
	uint32_t value[10] = { 0 };
	pThread = cmdq_tz_get_thread_struct_by_id(thread, line);
...
void cmdq_tz_dump_sec_metadata(iwcCmdqCommand_t *pCommandDec,
	struct tlApiCmdqExecMetadata_t *pMetadata, TaskStruct *pTask)
{
	uint32_t i = 0;
	...
	CMDQ_ERR("addrList size:%u list:%p\n",
...
static uint32_t *cmdq_tz_dump_pc(const TaskStruct *pTask, int thread, const char *tag)
{
	...
	if (pcVA) {
		...
		CMDQ_ERR("[%s]Thread %d PC(VA): 0x%p, 0x%08x:0x%08x\n",
...
int32_t cmdq_tz_insert_security_reg_command(TaskStruct *pTask,
	iwcCmdqCommand_t *pCommandDesc,
	struct tlApiCmdqExecMetadata_t *pMetadata)
{
...
TaskStruct* cmdq_tz_acquire_task_with_metadata(iwcCmdqCommand_t *pCommandDesc,
	struct tlApiCmdqExecMetadata_t *pMetadata)
{
	TaskStruct *pTask   = NULL;
	...
	do {
		if (!pTask) {
			CMDQ_ERR("TASK: no free task, gCmdqFreeTask:0x%p prev:0x%p next:0x%p\n",
...
int32_t cmdq_tz_reset_HW_thread(int32_t thread)
{
	...
	CMDQ_MSG("EXEC: test warm reset address 0x%p\n", CMDQ_THR_WARM_RESET(thread));
...
void cmdq_tz_remove_task_from_thread_array_by_cookie(
		ThreadStruct *pThread, int32_t index, TASK_STATE_ENUM targetTaskState)
{
	if ((!pThread) || (CMDQ_MAX_TASK_IN_THREAD <= index) || (0 > index))
	{
...
int32_t cmdq_tz_exec_task_async(TaskStruct *pTask, int32_t thread)
{
	...
	CMDQ_LOG("<--EXEC: pTask:0x%p thr:%d status:%d\n", pTask, thread, status);
	return status;
}
...
void cmdqTzHandleIRQ(int32_t thread)
{
	...
	if (!isAlreadySuspended || isInvalidInstruction)
	{
	...
	TaskStruct **ppTaskOut)
{
	...
	do {
		...
		if(!cmdq_tz_is_a_secure_thread(thread)) {
		...
		if(0 > status){
			cmdq_tz_dump_sec_metadata(&pIwcMessage->command, pMetadata, pTask);
			break;
		}
...
int32_t cmdqTzCancelTask(iwcCmdqCancelTask_t *pCancelTask)
{
	...
	do {
		...
		if (!pTask) {
			CMDQ_ERR("CANCEL_TASK: thread:%d pCurTask:%u is NULL task\n", thread, cookie);
			break;
		}

		CMDQ_ERR(
			"CANCEL_TASK: start to remove all tasks on thread:%d since error task:cookie:%u currCookie:%u\n",
	...
	if (0 >= pThread->taskCount) {
		...
		CMDQ_ERR("CANCEL_TASK: resume HW thread\n");
		cmdq_tz_resume_HW_thread(thread);
	}
