...
static void mutex_dump_analysis(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		DDPDUMP("MUTEX%d :SOF=%s,EOF=%s,WAIT=%d,module=(%s)\n",
			i, ddp_get_mutex_sof_name(REG_FLD_VAL_GET(SOF_FLD_MUTEX0_SOF, val)),
			ddp_get_mutex_sof_name(REG_FLD_VAL_GET(SOF_FLD_MUTEX0_EOF, val)),
			REG_FLD_VAL_GET(SOF_FLD_MUTEX0_SOF_WAIT, val), mutex_module);
