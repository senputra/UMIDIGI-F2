...
static int _disp_tui_pan_disp_single(tuiFbInfoV2_t *pFbInfo)
{
...
static int _convert_disp_input_to_ovl(struct OVL_CONFIG_STRUCT *ovl_cfg, tuiFbInfoV2_t *pFbInfo)
{
	int ret = 0;
	...
	ovl_cfg->addr = pFbInfo->fbPhysAddr;
	ovl_cfg->vaddr = NULL;
	ovl_cfg->src_x = 0;
	ovl_cfg->src_y = 0;
...
static int _disp_tui_pan_disp_multi(tuiFbInfoV2_t *pFbInfo)
{
	cmdqRecHandle cmdq_handle = gCmdq_handle;
	...
	if(cmdq_handle) {
		...
		_cmdq_insert_wait_frame_done_token_mira(cmdq_handle);//dxs+
	...
	disp_m4u_config_port(cmdq_handle, M4U_PORT_DISP_2L_OVL0_LARB0, 0, 1);
...
int disp_tui_pan_display(tuiFbInfoV2_t *pFbInfo)
{
	if (gDisp_version == SINGLE_WINDOWS_TUI)
...
int disp_tui_leave()
{
	...
	if (gDisp_version == SINGLE_WINDOWS_TUI) {
		...
		disp_m4u_config_port(cmdq_handle, M4U_PORT_DISP_2L_OVL0_LARB0, 1, 0);
	}
