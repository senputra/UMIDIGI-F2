	    ...
	    unsigned int bg_color, void *handle)
{
	...
	if ((bg_w > OVL_MAX_WIDTH) || (bg_h > OVL_MAX_HEIGHT)) {
		DDPERR("ovl_roi, exceed OVL max bg size(%dx%d)\n", bg_w, bg_h);
		ASSERT(0);
	}
		...
		void *handle, int is_dual)
{
	...
	if (ovl_partial_roi != NULL) {
		...
		DDPDBG("layer partial (%d,%d)(%d,%d,%dx%d) to (%d,%d)(%d,%d,%dx%d)\n",
		       cfg->src_x, cfg->src_y, cfg->dst_x, cfg->dst_y, cfg->dst_w,
		       cfg->dst_h, src_x, src_y, dst_x, dst_y, dst_w, dst_h);
	}
...
static void print_layer_config_args(int module, int local_layer,
