...
int32_t cmdq_tz_map_physical_addr(uint8_t **va, const uint64_t pa, const uint32_t length)
{
	...
	if (DRAPI_OK != drRes) {
		...
		CMDQ_LOG("drApiMapPhysicalBuffer(0x%x, 0x%x, 0x%x) = STATUS(%x)\n",
			*va, pa, length, drRes);
	}
...
void cmdq_tz_dump_status(const char *tag)
{
	...
	CMDQ_LOG("[%s]IRQ flag:0x%08x,0x%08x, Execing:%d, Exec Thread:0x%x, CMDQ_CURR_LOADED_THR: 0x%08x\n",
		 tag,
...
int32_t cmdq_tz_insert_task_to_thread_multiple_tasks_impl(TaskStruct *pTask, int32_t thread)
{
	...
	CMDQ_LOG("EXEC: reuse HW thread(%d), cookie:%d\n", thread, cookie);
...
void cmdqTzHandleIRQ(int32_t thread)
{
	...
	{
		CMDQ_ERR("IRQ: thread %d got interrupt but IRQ flat is 0x%08x in SWd\n", thread, value);
		cmdq_tz_dump_thread(thread, __LINE__);
		return;
	}
		...
		TaskStruct *pTask, int32_t thread, const uint32_t cookie, TASK_STATE_ENUM targetTaskState)
{
	...
	CMDQ_LOG("HACK: update task %p status %d and release it, thread: %d, cookie:%d\n",
...
int32_t cmdq_tz_poll_task_done(TaskStruct *pTask, int32_t thread)
{
	...
	do {
		...
		if (value >= cookie) {
			...
			/* update done thread info */
			//CMDQ_REG_SET32(CMDQ_SHARED_IRQ_RAISED_FLAG, (0x1 << thread));
										...
										TaskStruct **ppTaskOut)
{
	...
	do {
		...
		CMDQ_LOG("THREAD: test secure thread %d\n", thread);
