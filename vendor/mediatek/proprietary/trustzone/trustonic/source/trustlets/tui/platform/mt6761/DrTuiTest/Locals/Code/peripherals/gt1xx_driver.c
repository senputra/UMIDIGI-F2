...
static U64 write_byte_pa = 0xA3C00000;
static U64 read_byte_pa = 0xA3C10000;
...
int i2c_read_bytes_dma_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *rxbuf, s32 len)
{
	...
	TUI_LOGI("i2c_read_bytes_dma_tui1\n ");
	...
	while (left > 0)
	{
		...
		TUI_LOGI("i2c_read_bytes_dma_tui2\n");
		...
		if (left > MAX_DMA_TRANSACTION_LENGTH)
		{
			...
			for(retry = 0;retry < 5;retry++){
				ret = i2c_write_64(i2c_device, write_byte_pa, GTP_ADDR_LENGTH);
				if(ret != I2C_OK)
					continue;
				ret = i2c_read_64(i2c_device, read_byte_pa, transfer_lenth);
				if(ret == I2C_OK)
					break;
			}

			if(ret != I2C_OK){
				TUI_LOGE("I2C transfer error. errno:%d\n ", ret);
				return -1;
			}
...
int i2c_read_bytes_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *rxbuf, s32 len)
{
	...
	while (left > 0)
	{
		...
		if (left > MAX_TRANSACTION_LENGTH)
		{
			...
			for(retry = 0;retry < 5;retry++){
				ret = i2c_write_64(i2c_device, (uintptr_t)buffer, GTP_ADDR_LENGTH);
				if(ret != I2C_OK)
					continue;
				ret = i2c_read_64(i2c_device, (uintptr_t)buffer, transfer_lenth);
				if(ret == I2C_OK)
					break;
			}
		...
		TUI_LOGV("Read data: 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x \n",buffer[0], buffer[1],buffer[2],buffer[3],buffer[4],buffer[5],buffer[6],buffer[7]);
	}
...
int i2c_write_bytes_dma_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *txbuf, s32 len)
{
	...
	while (left > 0)
	{
		...
		for(retry = 0;retry < 5;retry++){
			ret = i2c_write_64(i2c_device, write_byte_pa, transfer_lenth);
			if(ret == I2C_OK)
				break;
		}

		TUI_LOGI("write data: 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x \n",write_byte[0], write_byte[1],write_byte[2],write_byte[3],write_byte[4],write_byte[5],write_byte[6],write_byte[7]);
	}
...
int i2c_write_bytes_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *txbuf, s32 len)
{
	...
	while (left > 0)
	{
		...
		for(retry = 0;retry < 5;retry++){
			ret = i2c_write_64(i2c_device, (uintptr_t)buffer, transfer_lenth);
			if(ret == I2C_OK)
				break;
		}

		TUI_LOGV("write data: 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x \n",buffer[0], buffer[1],buffer[2],buffer[3],buffer[4],buffer[5],buffer[6],buffer[7]);
	}
...
s32 gt1x_touch_event_handler(struct mt_i2c_t *i2c, u8 *data, struct tpd_data *pointerdata)
{
	...
	if (touch_num > GTP_MAX_TOUCH) {
		TUI_LOGE("Illegal finger number!\n");
		return -1;
	}

	TUI_LOGI(TAG "finger number:%d!\n", touch_num);

	memcpy(touch_data, data, 11);
	TUI_LOGI("I2C touch_data:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x, 0x%x, 0x%x, 0x%x!\n",touch_data[0], touch_data[1],touch_data[2],touch_data[3],touch_data[4],touch_data[5],touch_data[6],touch_data[7],touch_data[8], touch_data[9], touch_data[10]);

	/* read the remaining coor data */
	if (touch_num > 1) {
		ret = i2c_read_bytes_dma_tui(i2c, (GTP_READ_COOR_ADDR + 11), &touch_data[11], 1 + 8 * touch_num + 2 - 11);
		if (ret) {
			TUI_LOGE("Read coordinate i2c error.\n");
			return ret;
		}
	}
	...
	if ((touch_data[0] & 0x10) && key_value) {
		TUI_LOGI("TOUCH handler touch_data[0]:0x%x, key_value: 0x%x\n", touch_data[0], key_value);
	} else if (touch_num) {
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				input_x = GTP_WARP_X(TOUCH_RESOLUTION_WIDTH, input_x);
				input_y = GTP_WARP_Y(TOUCH_RESOLUTION_HEIGHT, input_y);
				...
				last_x = input_x;
				last_y = input_y;
				TUI_LOGI("touch event down:ID:%d, (%d, %d)\n", i , input_x, input_y);
		...
		TUI_LOGI("touch event up:ID:%d, (%d, %d)\n", i , last_x, last_y);
		TUI_LOGI("Released Touch.\n");
		pre_index = 0;
	}

	if (!pre_event && !cur_event) {
		TUI_LOGI("Additional Int Pulse.\n");
		ret = -1;
	} else {
		pre_event = cur_event;
		ret = 0;
...
int gt1xx_tpd_read_data(struct tpd_data *pointerdata)
{
	...
	if ((finger & 0x80) == 0) {
		TUI_LOGE("buffer not ready:0x%02x\n", finger);
		return -1;
	}
...
exit_work_func:
	...
	{
		end_cmd = 0;
		if (i2c_write_bytes_dma_tui(&i2c, GTP_READ_COOR_ADDR, &end_cmd, 1) < 0) {
			TUI_LOGE("I2C write end_cmd  error!\n");
		}
	}
	TUI_LOGI("%s: %d finish, ret=%d\n", __func__, __LINE__, ret);
...
{
	int err = 0;
	TUI_LOGI("%s: enter init function\n", __func__);
	//test_touch();

	if(&gt1xx_tui_obj == NULL){
		TUI_LOGE(TAG "%s: drApiMalloc tui memory fail\n", __func__);
		return -1;
	}
	...
	if(err){
		TUI_LOGE("%s: register common tui api fail\n", __func__);
		return err;
	}
	return err;
}
...
int map_i2c_hw_register(void)
{
	...
	TUI_LOGV("i2c0_map_regiser CFG, hasMapped [%x]\n", (i2c0_va));
	TUI_LOGV("i2c1_map_regiser CFG, hasMapped [%x]\n", (i2c1_va));
	TUI_LOGV("i2c2_map_regiser CFG, hasMapped [%x]\n", (i2c2_va));
	TUI_LOGV("i2c3_map_regiser CFG, hasMapped [%x]\n", (i2c3_va));
	TUI_LOGV("i2c4_map_regiser CFG, hasMapped [%x]\n", (i2c4_va));
	TUI_LOGV("i2c5_map_regiser CFG, hasMapped [%x]\n", (i2c5_va));
	TUI_LOGV("fwq0   base=%x\n",I2C0_BASE );
	TUI_LOGV("fwq1   base=%x\n",(U32*)I2C1_BASE );
	TUI_LOGV("fwq2   base=%x\n",I2C2_BASE );
}

int map_i2c_dma_buffer(void)
{
	uint32_t mapFlag = MAP_READABLE | MAP_WRITABLE | MAP_UNCACHED | MAP_IO;
	drApiMapPhysicalBuffer(((uint64_t)read_byte_pa /*& 0x00000000FFFFFFFF*/),0x100,mapFlag,(void **)&read_byte );
	drApiMapPhysicalBuffer(((uint64_t)write_byte_pa /*& 0x00000000FFFFFFFF*/),0x100,mapFlag,(void **)&write_byte );
	TUI_LOGD("i2c dma mmap read pa=0x%08x%08x va=0x%p\n", VALUE_64(read_byte_pa), read_byte);
}
...
{
	write_byte_pa = physAddr;
	read_byte_pa = (physAddr+0x10000);
	TUI_LOGD("i2c dma write pa:0x%08x%08x read pa:0x%08x%08x\n", VALUE_64(write_byte_pa), VALUE_64(read_byte_pa));
	map_i2c_dma_buffer();
}

void test_touch(void)
{
	TUI_LOGD("test_touch+4\n");
