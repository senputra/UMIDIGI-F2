...
static inline void _i2c_writel(mt_i2c * i2c, U16 offset, U16 value)
{
	I2C_WRITE((i2c->base) + (offset), value);
}

static inline U32 _i2c_readl(mt_i2c * i2c, U16 offset)
{
	return I2C_READ((i2c->base) + (offset));
}

#define raw_i2c_writel(i2c, ofs, val, ch_ofs) \
	do { \
		_i2c_writel(i2c, i2c->mch ? ofs + ch_ofs : ofs, val); \
	} while (0)

#define raw_i2c_readl(i2c, ofs, ch_ofs) \
	({ \
		U16 value = 0; \
		value = _i2c_readl(i2c, i2c->mch ? ofs + ch_ofs : ofs); \
		value; \
	})

#define i2c_writel(i2c, ofs, val) raw_i2c_writel(i2c, ofs, val, AP_MCH_OFFSET)

#define i2c_readl(i2c, ofs) raw_i2c_readl(i2c, ofs, AP_MCH_OFFSET)

#define i2c_writel_shadow(i2c, ofs, val) raw_i2c_writel(i2c, ofs, val, 0)

#define i2c_readl_shadow(i2c, ofs) raw_i2c_readl(i2c, ofs, 0)
...
static inline void i2c_secure_enable(mt_i2c * i2c)
{
	...
	switch(i2c->id) {
		case I2C0:
			set_module_apc(DAPC_INDEX_PERISYS_I2C0, DOMAIN_AP, SECURE_TRANSACTION);
			break;
		case I2C1: /*off chip, on chip, remap*/
			set_module_apc(DAPC_INDEX_PERISYS_I2C1, DOMAIN_AP, SECURE_TRANSACTION);
			break;
		case I2C2:
			set_module_apc(DAPC_INDEX_PERISYS_I2C2, DOMAIN_AP, SECURE_TRANSACTION);
			break;
		case I2C3:
			set_module_apc(DAPC_INDEX_PERISYS_I2C3, DOMAIN_AP, SECURE_TRANSACTION);
			break;
		case I2C4: /*off chip, on chip, remap*/
			set_module_apc(DAPC_INDEX_PERISYS_I2C4, DOMAIN_AP, SECURE_TRANSACTION);
			break;
		case I2C5:
			set_module_apc(DAPC_INDEX_PERISYS_I2C5, DOMAIN_AP, SECURE_TRANSACTION);
			break;
		default:
			I2CERR("invalid para: i2c->id=%d\n", i2c->id);
	}
...
static inline void i2c_secure_disable(mt_i2c * i2c)
{
	...
	switch(i2c->id) {
		case I2C0:
			set_module_apc(DAPC_INDEX_PERISYS_I2C0, DOMAIN_AP, NON_SECURE_TRANSACTION);
			break;
		case I2C1: /*off chip, on chip, remap*/
			set_module_apc(DAPC_INDEX_PERISYS_I2C1, DOMAIN_AP, NON_SECURE_TRANSACTION);
			break;
		case I2C2:
			set_module_apc(DAPC_INDEX_PERISYS_I2C2, DOMAIN_AP, NON_SECURE_TRANSACTION);
			break;
		case I2C3:
			set_module_apc(DAPC_INDEX_PERISYS_I2C3, DOMAIN_AP, NON_SECURE_TRANSACTION);
			break;
		case I2C4: /*off chip, on chip, remap*/
			set_module_apc(DAPC_INDEX_PERISYS_I2C4, DOMAIN_AP, NON_SECURE_TRANSACTION);
			break;
		case I2C5:
			set_module_apc(DAPC_INDEX_PERISYS_I2C5, DOMAIN_AP, NON_SECURE_TRANSACTION);
			break;
		default:
			I2CERR("invalid para: i2c->id=%d\n", i2c->id);
	}
...
void _config_mt_i2c(mt_i2c *i2c)
{
	switch(i2c->id) {
		case I2C0:
			...
			i2c->pdmabase = (unsigned long)(DMA_BASE_VA + 0x80);
			break;
		case I2C1: /*remap*/
			i2c->base = (unsigned long)I2C1_BASE;
			i2c->pdmabase = (unsigned long)(DMA_BASE_VA + 0x480);
			break;
		case I2C2:
		...
		case I2C4: /*remap*/
			...
			i2c->pdmabase = (unsigned long)(DMA_BASE_VA + 0x200);
			break;
		case I2C5:
			i2c->base = (unsigned long)I2C3_BASE;
			i2c->pdmabase = (unsigned long)(DMA_BASE_VA + 0x580);
			break;
		default:
			I2CERR("invalid para: i2c id = %d\n",i2c->id);
			break;
	...
	U32 *timing_sample_cnt)
{
	...
	if (speed_khz > MAX_HS_MODE_SPEED) {
		max_step_cnt = MAX_HS_STEP_CNT_DIV;
	} else if (speed_khz > MAX_FS_MODE_SPEED) {
		max_step_cnt = MAX_HS_STEP_CNT_DIV;
	} else {
	...
	if (sclk > khz) {
		I2CERR("%s mode: unsupported speed (%ldkhz)\n",
			(speed_khz > MAX_FS_MODE_SPEED) ? "HS" : "ST/FT", (long int)khz);
		return -EINVAL_I2C;
	}
...
S32 i2c_set_speed(mt_i2c *i2c, U32 clk_src_in_hz)
{
	S32 ret;
	U32 step_cnt;
	U32 sample_cnt;
	U32 l_step_cnt;
	U32 l_sample_cnt;
	U32 speed_hz = i2c->speed;
	U32 duty = HALF_DUTY_CYCLE;
	i2c->hs_only = 1; /* without master code */
...
void _i2c_write_reg(mt_i2c *i2c)
{
	...
	if (i2c->mch)
		i2c_writel(i2c, OFFSET_INTR_STAT, 0x1FF);
	else
		i2c_writel(i2c, OFFSET_INTR_STAT,
			   (I2C_HS_NACKERR | I2C_ACKERR | I2C_TRANSAC_COMP));
	/* Clear fifo address */
	i2c_writel(i2c, OFFSET_FIFO_ADDR_CLR, 0x5);
	/* Setup the interrupt mask flag */
	if (i2c->poll_en)
		i2c_writel(i2c, OFFSET_INTR_MASK, 0); /* Disable interrupt */
	else {
		if (i2c->mch)
			i2c_writel(i2c, OFFSET_INTR_MASK,
				   I2C_TRANSAC_COMP | I2C_IBI |
				   I2C_MASTER_ERR); /* Enable interrupt */
		else
			i2c_writel(i2c, OFFSET_INTR_MASK,
				   i2c_readl(i2c, OFFSET_INTR_MASK) |
				   (I2C_HS_NACKERR | I2C_ACKERR |
				   I2C_TRANSAC_COMP)); /* Enable interrupt */
	}
	/* Set transfer len */
	...
	if (i2c->dma_en) {
		if (I2C_MASTER_RD == i2c->op) {
			...
			if (i2c->is_64_bit) {
				I2C_WRITE(i2c->pdmabase + OFFSET_RX_MEM_ADDR, lower_32_bits(i2c->msg_buf_64));
				I2C_WRITE(i2c->pdmabase + OFFSET_RX_MEM_ADDR2, upper_32_bits(i2c->msg_buf_64));
			} else {
			...
			if (i2c->is_64_bit) {
				I2C_WRITE(i2c->pdmabase + OFFSET_TX_MEM_ADDR, lower_32_bits(i2c->msg_buf_64));
				I2C_WRITE(i2c->pdmabase + OFFSET_TX_MEM_ADDR2, upper_32_bits(i2c->msg_buf_64));
			} else {
...
static S32 _i2c_deal_result(mt_i2c *i2c)
{
	...
	if (!(tmo == 0) && TRANSFER_ERROR == FALSE)	{
		...
		if (i2c->irq_stat & I2C_IBI)
			I2CERR("I2C_IBI\n");
		if (i2c->irq_stat & I2C_MASTER_ERR)
			I2CERR("I2C_MASTER_ERR\n");

		_i2c_dump_info(i2c);

		i2c_writel_shadow(i2c, OFFSET_SOFTRESET, 0x1);
		i2c_writel_shadow(i2c, OFFSET_SLAVE_ADDR, 0x0);
		if (i2c->mch)
			i2c_writel_shadow(i2c, OFFSET_INTR_STAT, 0x1FF);
		else
			i2c_writel_shadow(i2c, OFFSET_INTR_STAT,
					  (I2C_HS_NACKERR|I2C_ACKERR|I2C_TRANSAC_COMP));
		i2c_writel_shadow(i2c, OFFSET_FIFO_ADDR_CLR, 0x5);
		/* restart arbitration */
		if (i2c->mch)
			i2c_writel(i2c, OFFSET_START, 0x0002);
	}
	return ret;
}
...
S32 _i2c_transfer_interface(mt_i2c *i2c)
{
	...
	return_value = i2c_set_speed(i2c, I2C_CLK_RATE);
	if (return_value < 0) {
