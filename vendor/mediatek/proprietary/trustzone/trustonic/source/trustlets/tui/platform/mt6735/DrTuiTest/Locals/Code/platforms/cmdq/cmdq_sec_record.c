...
static uint32_t gCmdBufferBasePA = CMD_BUFFER_BASE_PA;
...
{
	int32_t status = 0;
	status = cmdq_tz_map_physical_addr(&gCmdBufferBaseVA, (uint64_t)gCmdBufferBasePA, CMD_BUFFER_SIZE);
...
{
	return cmdq_tz_unmap_physical_addr(gCmdBufferBaseVA, CMD_BUFFER_SIZE);
}
...
int32_t cmdq_rec_realloc_cmd_buffer(cmdqRecHandle handle, uint32_t size)
{
	...
	CMDQ_MSG("REC: realloc size from base :0x%x \n", handle->pBuffer);
...
int32_t cmdqRecCreate(CMDQ_SCENARIO_ENUM scenario, cmdqRecHandle *pHandle)
{
	...
	do {
		if (NULL == handle) {
			CMDQ_ERR("Handle: no free handle, gCmdqFreeRecord[0x%08x], prev[0x%08x], next[0x%08x]\n",
...
int32_t cmdq_rec_make_message(cmdqRecHandle handle, iwcCmdqMessage_t* outMessage)
{
	int32_t status;
	...
	CMDQ_MSG("Submit task scenario: %d, priority: %d, engine: 0x%llx, size: %d, buffer: 0x%08x\n",
		 handle->scenario, handle->priority, handle->engineFlag, handle->blockSize, handle->pBuffer);
	...
	if (gCmdqTaskCookie == 1){
		outMessage->command.resetExecCnt = 1;
	}
...
int32_t cmdqRecFlushNoWait(cmdqRecHandle handle, TaskStruct** outTask)
{
	int32_t status;
	iwcCmdqMessage_t message = {0};
	...
	//status = cmdqTzComposeAndSubmitTaskAsync(&message, NULL, outTask);
...
int32_t cmdq_rec_parse_instruction(const uint32_t *pCmd, char *textBuf, int bufLen)
{
	int reqLen = 0;
...
int32_t cmdqRecDumpCommand(cmdqRecHandle handle)
{
	...
	CMDQ_MSG("REC 0x%p command buffer TRANSLATED:\n", handle->pBuffer);
	for (i = 0, pCmd = handle->pBuffer; i < handle->blockSize; i += 8, pCmd += 2) {
		cmdq_rec_parse_instruction(pCmd, textBuf, 128);
		CMDQ_MSG("%s", textBuf);
	}
	CMDQ_MSG("======REC 0x%p command END\n", handle->pBuffer);
...
#define CMDQ_TEST_MMSYS_DUMMY_PA		(CMDQ_TEST_MMSYS_BASE_PA + CMDQ_TEST_MMSYS_DUMMY_OFFSET)
	...
	CMDQ_MSG("test: after map, va: 0x%x\n", va);
