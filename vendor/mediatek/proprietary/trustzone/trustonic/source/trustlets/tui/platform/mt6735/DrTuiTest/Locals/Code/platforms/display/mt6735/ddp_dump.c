...
static void  mmsys_config_dump_analysis(void)
{
    ...
    for (i = 0; i < 32; i++)
    {
        if ((valid0 & (1 << i)) == 0)
        {
            DDPDUMP("%s:\t\tNot Valid,",ddp_signal_0(i));
        }
        else
        {
            DDPDUMP("%s:\t\t    Valid,",ddp_signal_0(i));
        }
        if ((ready0 & (1 << i)) == 0)
        {
            DISP_LOG_RAW("Not Ready");
        }
        else
        {
            DISP_LOG_RAW("    Ready");
    ...
    for (i = 0; i < 24; i++)
    {
        if ((valid1 & (1 << i)) == 0)
        {
            DDPDUMP("%s:\t\tNot Valid,",ddp_signal_1(i));
        }
        else
        {
            DDPDUMP("%s:\t\t    Valid,",ddp_signal_1(i));
        }
        if ((ready1 & (1 << i)) == 0)
        {
			DISP_LOG_RAW("Not Ready");
