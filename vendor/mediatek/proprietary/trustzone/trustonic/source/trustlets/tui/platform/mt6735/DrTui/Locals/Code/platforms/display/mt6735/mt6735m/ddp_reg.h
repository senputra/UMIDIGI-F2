...
static inline unsigned long disp_addr_convert(unsigned long va)
{
  ...
  DDPAEE("can not find reg addr for va=0x%lx! \n", va);
  return 0;
}
...
#define DISP_REG_MASK(handle, reg32, val, mask) 	\
	do { \
		 if(handle==NULL) \
		 { \
			mt_reg_sync_writel((unsigned int)(DISP_REG_GET(reg32)&~(mask))|(val),(volatile unsigned long*)(reg32) );\
		 } \
		 else \
		 { \
