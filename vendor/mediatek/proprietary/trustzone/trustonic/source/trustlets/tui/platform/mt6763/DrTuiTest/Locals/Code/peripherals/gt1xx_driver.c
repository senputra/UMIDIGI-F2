...
int i2c_read_bytes_dma_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *rxbuf, s32 len)
{
	...
	TUI_LOGV("i2c_read_bytes_dma_tui1\n ");
	...
	while (left > 0)
	{
		...
		TUI_LOGV("i2c_read_bytes_dma_tui2\n");
...
int i2c_read_bytes_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *rxbuf, s32 len)
{
	...
	while (left > 0)
	{
		...
		if (left > MAX_TRANSACTION_LENGTH)
		{
			...
			for(retry = 0;retry < 5;retry++){
				ret = i2c_write_64(i2c_device, buffer, GTP_ADDR_LENGTH);
				if(ret != I2C_OK)
					continue;
				ret = i2c_read_64(i2c_device, buffer, transfer_lenth);
				if(ret == I2C_OK)
					break;
			}
...
int i2c_write_bytes_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *txbuf, s32 len)
{
	...
	while (left > 0)
	{
		...
		for(retry = 0;retry < 5;retry++){
			ret = i2c_write_64(i2c_device, buffer, transfer_lenth);
			if(ret == I2C_OK)
				break;
		}
...
s32 gt1x_touch_event_handler(struct mt_i2c_t *i2c, u8 *data, struct tpd_data *pointerdata)
{
	...
	if ((touch_data[0] & 0x10) && key_value) {
		TUI_LOGV("TOUCH handler touch_data[0]:0x%x, key_value: 0x%x\n", touch_data[0], key_value);
	} else if (touch_num) {
	...
	if (CHK_BIT(cur_event, BIT_TOUCH)) {
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++) {
			if (i == id) {
				...
				last_x = input_x;
				last_y = input_y;
				TUI_LOGV("touch event down:ID:%d, (%d, %d)\n", i , input_x, input_y);
		...
		TUI_LOGD("touch event up:ID:%d, (%d, %d)\n", i , last_x, last_y);
		TUI_LOGV("Released Touch.\n");
		pre_index = 0;
	}

	if (!pre_event && !cur_event) {
		TUI_LOGD("Additional Int Pulse.\n");
		ret = -1;
	} else {
		pre_event = cur_event;
		ret = 0;
