...
int rdma_reg_backup(enum DISP_MODULE_ENUM module)
{
	...
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_GLOBAL_CON));
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_MEM_CON));
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_MEM_START_ADDR));
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_MEM_SRC_PITCH));
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_INT_ENABLE));
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_SIZE_CON_0));
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_SIZE_CON_1));
	disp_tui_reg_backup((void*)(base_addr + DISP_REG_RDMA_FIFO_CON));
...
int rdma_wait_frame_done_by_polling(enum DISP_MODULE_ENUM module)
{
	...
	while(1) {
		...
		DDPERR("%s, %d, RDMA INT status=0x%x\n", __func__, __LINE__, regval);
		...
		if(regval & (1<<RDMA_INT_FRAME_DONE_BIT)) {
			DDPERR("error: rdma frame done 0x%x\n", regval);
