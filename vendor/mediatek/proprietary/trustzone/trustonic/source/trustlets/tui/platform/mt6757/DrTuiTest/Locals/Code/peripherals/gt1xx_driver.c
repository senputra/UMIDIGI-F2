...
int i2c_read_bytes_dma_tui(struct mt_i2c_t *i2c_device, u16 addr, u8 *rxbuf, s32 len)
{
	...
	TUI_LOGV(TAG "i2c_read_bytes_dma_tui1\n ");
    ...
    while (left > 0)
    {
		...
		TUI_LOGV(TAG "i2c_read_bytes_dma_tui2\n");
        ...
        if (left > MAX_DMA_TRANSACTION_LENGTH)
        {
			...
			if(ret != I2C_OK){
				TUI_LOGE(TAG "I2C transfer error. errno:%d\n ", ret);
				return -1;
			}
...
s32 gt1x_touch_event_handler(struct mt_i2c_t *i2c, u8 *data, struct tpd_data *pointerdata)
{
	...
	TUI_LOGV(TAG "finger number:%d!", touch_num);

	memcpy(touch_data, data, 11);
	TUI_LOGV(TAG "I2C  touch_data:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x, 0x%x, 0x%x, 0x%x!",touch_data[0], touch_data[1],touch_data[2],touch_data[3],touch_data[4],touch_data[5],touch_data[6],touch_data[7],touch_data[8], touch_data[9], touch_data[10]);
 ...
 */
	...
	if ((touch_data[0] & 0x10) && key_value) {
		TUI_LOGV(TAG "TOUCH handler touch_data[0]:0x%x, key_value: 0x%x\n", touch_data[0], key_value);
	} else if (touch_num) {
...
int gt1xx_tpd_read_data(struct tpd_data *pointerdata)
{
	...
	TUI_LOGV(TAG "gt1xx_tpd_read_data\n ");
	finger = point_data[0];
	...
	if (!gtp_rawdiff_mode)
	{
		...
		if (ret < 0)
		{
			TUI_LOGE(TAG "I2C write end_cmd  error!\n");
		}
...
{
	int err = 0;
	TUI_LOGI(TAG "%s: enter init function\n", __func__);
	//test_touch();
	...
	if(err){
		TUI_LOGE(TAG "%s: register common tui api fail\n", __func__);
		return err;
	}
    return err;
}
