...
int set_master_transaction(unsigned int master_index, unsigned int transaction_type)
{
    ...
    base = (volatile unsigned int*) ((unsigned int)DEVAPC_MAS_SEC + master_register_index * 4);
...
void set_module_apc(unsigned int module, E_MAS_DOM domain_num, APC_ATTR permission_control)
{
    ...
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D0_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else if (domain_num == E_DOMAIN_1)
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D1_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else if (domain_num == E_DOMAIN_2)
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D2_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else if (domain_num == E_DOMAIN_3)
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D3_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else if (domain_num == E_DOMAIN_4)
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D4_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else if (domain_num == E_DOMAIN_5)
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D5_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else if (domain_num == E_DOMAIN_6)
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D6_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else if (domain_num == E_DOMAIN_7)
    {
        base = (volatile unsigned int*) ((unsigned int)DEVAPC_D7_APC_0 + (module/MOD_NO_IN_1_DEVAPC) * 4);
    }
    else
    {
        drDbgPrintLnf("[DEVAPC] set_module_apc: domain_num %d is overflow!!\n", domain_num);
        return;
    }
