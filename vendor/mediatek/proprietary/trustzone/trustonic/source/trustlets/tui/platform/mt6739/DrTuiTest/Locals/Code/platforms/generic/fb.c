                                  ...
                                  uint32_t size)
{
	...
	drApiResult_t r;
	drTuiHalError_t ret = TUI_HAL_OK;
	uint32_t addr_start =  physAddr;
	uint32_t addr_end;
	uint64_t mpu_attr;

	TUI_LOGD("%s addr=0x%x, size=0x%x\n", __func__, addr_start, size);
    /*first address will be passed in third call*/
	if (protect_cnt == 2) {
		addr_start = physAddr;
		addr_end = physAddr + (X_RES*BYTES_PER_PIXEL*Y_RES)*BUFFER_COUNT + EXTRA_MEM_SIZE -1;

		/* Enble MPU protection */
#if TUI_ENABLE_MPU
		mpu_attr = SET_ACCESS_PERMISSON(EMI_MPU_UNLOCK, \
		...
		TUI_LOGD("%s addr_start=0x%x, addr_end=0x%x\n", __func__, addr_start, addr_end);
...
void tuiHalPrintFBInfo(const tuiFbInfoV2_t *pFbInfo, const char *msg)
{
	...
	TUI_LOGV("x=%d,y=%d,density=%d,bpp=%d,pitch=%d,type=%d,sec=%d,size=%d,phys=0x%x,va=0x%p\n",
                                   ...
                                   uint32_t sHeight)
{
	...
	return drTuiFBBlit(pFbInfo, pImage, orientation, dx, dy, sx, sy, sWidth,
			sHeight);
}
