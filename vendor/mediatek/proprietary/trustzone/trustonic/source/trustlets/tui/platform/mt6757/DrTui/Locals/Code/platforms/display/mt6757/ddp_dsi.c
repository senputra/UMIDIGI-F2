...
unsigned int dsi_phy_get_clk(DISP_MODULE_ENUM module)
{
	...
	DDPDBG("%s, pcw: %d, prediv: %d, posdiv: %d, txdiv0: %d, txdiv1: %d\n", __func__, pcw,
...
#else
			/* use irq, but disp tui has not use tee irq until now!*/
#endif
		}
...
int ddp_dsi_is_busy(DISP_MODULE_ENUM module)
{
	...
	DDPDBG("%s is %s\n", ddp_get_module_name(module), is_busy ? "busy" : "idle");
	return is_busy;
}
...
({ \
	...
	if (dsi_reg_op_debug) \
		DDPDBG("[mipitx/inreg]%p=0x%08x\n", (void *)addr, val); \
	val; })

#define MIPITX_OUTREG32(addr, val) {\
		if (dsi_reg_op_debug) {	\
			DDPDBG("[mipitx/reg]%p=0x%08x\n", (void *)addr, val);\
		} \
...
int ddp_dsi_trigger(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	if ((_dsi_context[i].dsi_params.mode == CMD_MODE) && (module == DISP_MODULE_DSIDUAL)) {
		/* write redundant value for delay in pulling down DSI_DUAL_EN */
		DSI_OUTREGBIT(cmdq, DSI_COM_CTRL_REG, DSI_REG[0]->DSI_COM_CTRL, DSI_DUAL_EN, 1);
...
int ddp_dsi_init(void)
{
	...
	DSI_PHY_REG[0] = (PDSI_PHY_REGS)DDP_REG_BASE_MIPITX0;
	DSI_CMDQ_REG[0] = (PDSI_CMDQ_REGS)(DISPSYS_DSI0_BASE+0x200);
	DSI_REG[1] = (PDSI_REGS)DISPSYS_DSI1_BASE;
	DSI_PHY_REG[1] = (PDSI_PHY_REGS)DDP_REG_BASE_MIPITX1;
