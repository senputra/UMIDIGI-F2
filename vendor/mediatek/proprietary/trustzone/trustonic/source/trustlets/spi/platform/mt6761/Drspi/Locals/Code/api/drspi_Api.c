...
_TLAPI_EXTERN_C tlApiResult_t drSpiSend0(const void *tx_buf, void	*rx_buf, uint32_t len, struct mt_chip_conf *chip_conf, uint32_t flag)
{
	...
	int32_t tl_ret;
	static uint32_t UseSecureSpi = 0 , retFirstFail;
...
//	tlset_module_apc(DAPC_INDEX_SPI0,0,1); // lock SPI, only secure world can access
	...
	if (UseSecureSpi == 0) {
		TLSPIMSG("[Trustlet TlSPi] Querymemory fail already.Exit spi transfer.\n");
		return retFirstFail;
	}
	...
	if (spiData.is_dma_used == 1) {
		if (SharedMemStatus) {
			...
			TLSPIMSG("[Trustlet TlSPi] SECMEM_PL_RESERVED_SPI,tx_dma:0x%x,rx_dma:0x%x\n",
	...
	TLSPIMSG("[Trustlet TlSPi] spiData.len=%d is_dma_used:%d\n", spiData.len, spiData.is_dma_used);
