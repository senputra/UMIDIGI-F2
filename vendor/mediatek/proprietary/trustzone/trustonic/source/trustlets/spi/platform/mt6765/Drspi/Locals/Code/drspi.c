...
void dump_reg_config(void)
{
	...
	SPI_MSG("SPI_REG_TX_SRC_64=0x%x\n",SPI_READ(SPI_REG_TX_SRC_64));
	SPI_MSG("SPI_REG_RX_DST_64=0x%x\n",SPI_READ(SPI_REG_RX_DST_64));
...
//	SPI_MSG("start xfer 0x%p, mode %d, len %u\n",xfer,mode,xfer->len);
	...
	SPI_MSG("xfer %p: len %04u tx %p/%08x rx %p/%08x, mode %d, data %x/%x\n",
			xfer, xfer->len,
			xfer->tx_buf, xfer->tx_dma,
			xfer->rx_buf, xfer->rx_dma, mode,*(uint32_t *)(xfer->tx_buf),*(uint32_t *)(xfer->rx_buf));
	...
	*/
	...
	if((mode == DMA_TRANSFER)){
		...
		for(i = 0; i < cnt; i++){
			//SPI_MSG("tx_buf addr is:%p\n", (uint32_t *)xfer->tx_buf + i);		
			//SPI_MSG("tx_buf data is:%x\n",*( ( uint32_t * ) xfer->tx_buf + i ) );
			//SPI_MSG("tx_dma addr is:%x\n", SPI_TX_DMA_VA_BASE+get_pa_offset(xfer->tx_dma)+4*i);			i
			if (xfer->tx_buf)
				SPI_WRITE((SPI_TX_DMA_VA_BASE + 4 * i) ,
					  *((uint32_t *)xfer->tx_buf + i));
		}
	}
    dump_reg_config();
...
int32_t SpiMapRegister(void)
{
    ...
    do
    {
		...
		SPI_MSG("spi_map SPI_PIN_SWITCH_MODE gpio, hasMapped [%x]\n", (SpiGpioBaseVA));
	   #endif
        gRegHasMapped = true;
