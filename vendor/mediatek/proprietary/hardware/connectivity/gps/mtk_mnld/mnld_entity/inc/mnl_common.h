...
//If this bit set, will not notify in SI mode
#define MNL_NFW_CTRL_NEGATIVE_NOTIFY_BIT (0x08)
//If this bit set, will notify only once in one session
#define MNL_NFW_CTRL_SESSION_NOTIFY_BIT (0x10)
...
typedef struct {
    ...
    //bit3: Switch of FW notify positive or negative;
    int bitmap_nfw_ctrl;
} MNL_CONFIG_T;
/*---------------------------------------------------------------------------*/
typedef struct {
