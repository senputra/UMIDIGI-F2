...
size_t BluetoothAudioSession::OutWritePcmData(const void* buffer,
  ...
  PcmParameters& pcmConfig = audio_config_.pcmConfig();
  do {
    std::unique_lock<std::recursive_mutex> lock(mutex_);
    if (!IsSessionReady()) break;
    size_t availableToWrite = mDataMQ->availableToWrite();
    /** M: Align BT Hal buffer size with multiples of 6 @{ */
    if (pcmConfig.bitsPerSample == BitsPerSample::BITS_24) {
      availableToWrite = (availableToWrite/6) * 6;
    }
    /** @} */
    if (availableToWrite) {
