...
int wfaStaSetSecurity(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    set_param_str("akmSuiteType", ssec->akmSuiteType);
    ...
    set_param_str("serverCert", ssec->serverCert);
    set_param_str("domainMatch", ssec->domainMatch);
    set_param_str("domainSuffixMatch", ssec->domainSuffixMatch);
...
int wfaStaSetEapTLS(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    if (setTLS) {
        ...
        set_param_str("serverCert", setTLS->serverCert);
        set_param_str("domainMatch", setTLS->domainMatch);
        set_param_str("domainSuffixMatch", setTLS->domainSuffixMatch);
...
int wfaStaSetEapTTLS(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    if (setTTLS) {
        ...
        set_param_str("serverCert", setTTLS->serverCert);
        set_param_str("domainMatch", setTTLS->domainMatch);
        set_param_str("domainSuffixMatch", setTTLS->domainSuffixMatch);
...
int wfaStaSetPEAP(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    if (setPEAP) {
        ...
        set_param_str("serverCert", setPEAP->serverCert);
        set_param_str("domainMatch", setPEAP->domainMatch);
        set_param_str("domainSuffixMatch", setPEAP->domainSuffixMatch);
...
int wfaStaDevExecAction(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)//chitest1
{
    dutCmdResponse_t *execActionResp = &gGenericResp;
    dutCommand_t *dutCmd = (dutCommand_t *)caCmdBuf;
    caStaDevExecAction_t *staDevExecAction= (caStaDevExecAction_t *) &dutCmd->cmdsu.devExecAction;

    DPRINT_INFO(WFA_OUT, "Entering %s ...\n", __func__);
    DPRINT_INFO(WFA_OUT, "prog %s intf %s serverCert %s ...\n", &staDevExecAction->prog, &staDevExecAction->intf, &staDevExecAction->serverCertTrust);

    //strcpy(&execActionResp->cmdru.devExecResp.serverCertTrustResult[0], "OverrideNotAllowed");
    strcpy(&execActionResp->cmdru.devExecResp.serverCertTrustResult, &staDevExecAction->serverCertTrust);

    execActionResp->status = STATUS_COMPLETE;
    wfaEncodeTLV(WFA_STA_DEV_EXEC_ACTION_RESP_TLV, sizeof(dutCmdResponse_t), (BYTE *)execActionResp, respBuf);
    *respLen = WFA_TLV_HDR_LEN + sizeof(dutCmdResponse_t);

    DPRINT_INFO(WFA_OUT, "ServerCertTrustResult %s\n", &execActionResp->cmdru.devExecResp.serverCertTrustResult);
...
#define GET_PARAM(x) GET_PARAM_V(x, 0)
    ...
    GET_PARAM(serverCert);
    GET_PARAM(domainMatch);
    GET_PARAM(domainSuffixMatch);
    GET_PARAM(tlsCipher);
    GET_PARAM(username);
    GET_PARAM_V(peapVersion, UNSET_VALUE);
...
#define SET_PARAM(x) \
    ...
    SET_PARAM(serverCert);
    SET_PARAM(domainMatch);
    SET_PARAM(domainSuffixMatch);
    SET_PARAM(tlsCipher);
    SET_PARAM(username);
    SET_PARAM(peapVersion);
    SET_PARAM(prefer);
...
int set_eap_common(const char* intf, const int id)
{
    ...
    if (set_wpa_common(intf, id) != STATUS_COMPLETE) {
        DPRINT_INFO(WFA_OUT, "\n[Error]set_wpa_common fail ...\n");
        return STATUS_ERROR;
    }

    akms = get_param_str("AKMSuiteType");
    ...
    if (akms) {
        ...
        while (akm) {
            ...
            if (str_same(akm, "1")) {
                strcat(buf, "IEEE8021X");
            } else if (str_same(akm, "2")) {
                strcat(buf, "WPA-PSK");
            } else if (str_same(akm, "3")) {
                strcat(buf, "FT-EAP");
            } else if (str_same(akm, "4")) {
                strcat(buf, "FT-PSK");
            } else if (str_same(akm, "5")) {
                strcat(buf, "WPA-EAP-SHA256");
            } else if (str_same(akm, "6")) {
                strcat(buf, "WPA-PSK");
            } else if (str_same(akm, "8")) {
                strcat(buf, "SAE");
            } else if (str_same(akm, "9")) {
                strcat(buf, "FT-SAE");
            } else if (str_same(akm, "11")) {
                strcat(buf, "WPA-EAP-SUITE-B");
            } else if (str_same(akm, "12")) {
                strcat(buf, "WPA-EAP-SUITE-B-192");
            } else if (str_same(akm, "13")) {
                strcat(buf, "FT-EAP-SHA384");
            } else if (str_same(akm, "14")) {
                if (gDut.sta_pmf == WFA_REQUIRED || gDut.sta_pmf == WFA_OPTIONAL)
                    strcat(buf, "WPA-EAP-SHA256 FILS-SHA256");
                else
                    strcat(buf, "WPA-EAP FILS-SHA256");
                if (set_network(intf, id, "erp", "1") < 0)
                     return STATUS_ERROR;
            } else if (str_same(akm, "15")) {
                if (gDut.sta_pmf == WFA_REQUIRED || gDut.sta_pmf == WFA_OPTIONAL)
                    strcat(buf, "WPA-EAP-SHA256 FILS-SHA384");
                else
                    strcat(buf, "WPA-EAP FILS-SHA384");
                if (set_network(intf, id, "erp", "1") < 0)
                     return STATUS_ERROR;
            } else if (str_same(akm, "18")) {
                strcat(buf, "OWE");
            }
            akm = strtok(NULL, ";");
        }
    ...
    if (val) {
        if (0 == strcmp(val, "DEFAULT")) {
            wSPRINTF(buf, "/system/etc/security/cacerts");
            chk_exist(buf);
            if (set_network_quoted(intf, id, "ca_path", buf) < 0)
                return STATUS_ERROR;
        } else {
            ext = strstr(val, ".pem");
            if (ext)
    ...
    val = get_param_str("serverCert");
    if (val) {
        ext = strstr(val, ".pem");
        if (ext)
            *ext = 0;

        wSPRINTF(buf, EAP_CERT_PATH"/%s.pem.sha256", val);
        chk_exist(buf);
        if (shell("cat %s", buf) == WFA_SUCCESS) {
            //"hash://server/sha256/5a1bc1296205e6fdbe3979728efe3920798885c1c4590b5f90f43222d239ca6a"
            wSPRINTF(buf, "hash://server/sha256/%s", gDut.shellResult);
            if (set_network_quoted(intf, id, "ca_cert", buf) < 0)
                return STATUS_ERROR;
            }
    }

    val = get_param_str("domainMatch");
    if (val) {
        if (set_network_quoted(intf, id, "domain_match", val) < 0)
            return STATUS_ERROR;
    }

    val = get_param_str("domainSuffixMatch");
    if (val) {
        if (set_network_quoted(intf, id, "domain_suffix_match", val) < 0)
            return STATUS_ERROR;
    }

    val = get_param_str("clientCertificate");
    if (val) {
        wSPRINTF(buf, EAP_CERT_PATH"/%s", val);
        chk_exist(buf);
