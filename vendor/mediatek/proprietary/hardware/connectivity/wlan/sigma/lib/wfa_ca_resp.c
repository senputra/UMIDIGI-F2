...
#endif
{
    ...
    wfaStaGenericResp,
#endif
    wfaStaDevExecActionResp,              /* WFA_STA_DEV_EXEC_ACTION_TLV (90) */
...
int wfaStaDevExecActionResp(BYTE *cmdBuf)
{
    int done=0;
    dutCmdResponse_t *staDevExecActionResp = (dutCmdResponse_t *) (cmdBuf + 4);

    DPRINT_INFO(WFA_OUT, "Entering %s serverCertTrustResult %s...\n", __func__, staDevExecActionResp->cmdru.devExecResp.serverCertTrustResult);

    switch(staDevExecActionResp->status)
    {
    case STATUS_RUNNING:
    ...
    case STATUS_COMPLETE:
        //sprintf(gRespStr, "status,COMPLETE,ServerCertTrust,Accepted\r\n");
        //sprintf(gRespStr, "status,COMPLETE,ServerCertTrust,OverrideNotAllowed\r\n");

        sprintf(gRespStr, "status,COMPLETE,ServerCertTrust,%s\r\n", staDevExecActionResp->cmdru.devExecResp.serverCertTrustResult);
    ...
    DPRINT_INFO(WFA_OUT, "Send to UCC %s ...\n", __func__);
    DPRINT_INFO(WFA_OUT, "status,COMPLETE,ServerCertTrust,%s\r\n", staDevExecActionResp->cmdru.devExecResp.serverCertTrustResult);
