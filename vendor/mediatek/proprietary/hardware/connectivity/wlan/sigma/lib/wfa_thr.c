    ...
    while(1)
    {
        ...
        case DIRECT_SEND:
            ...
            if (myProfile->profile == PROF_IPTV || myProfile->profile == PROF_FILE_TX || myProfile->profile == PROF_MCAST)
            {
                ...
                // must initiate iOptLen otherwise getsockopt would fail
                int iOptVal=0, iOptLen = sizeof(int);

                if (getsockopt(mySock, SOL_SOCKET, SO_SNDBUF, (char*)&iOptVal, (socklen_t *)&iOptLen) == 0)
                {
                    DPRINT_INFO(WFA_OUT, "wfa_wmm_thread:: stream %d get socket size=%d\n", myStreamId, iOptVal);
                    // do not adjust socket buffer size here, we will send unblockingly in wfaSendBitrateData.
                }
#else
                int iOptVal, iOptLen;
...
#ifdef CONFIG_MTK_COMMON
                        if(myProfile->profile != PROF_TRANSC && gtgCaliRTD != 0 )
#else
                        if(gtgCaliRTD != 0 )
#endif
                        {
...
#ifdef CONFIG_MTK_COMMON
                    sprintf(e2eResults, "/data/local/e2e%u-%i.txt", (unsigned int) currtime.tv_sec, myStreamId);
#else
