...
#define uart_printf(...) tile_driver_printf_no_prefix(__VA_ARGS__)
#endif //END LDVT
#else //not arm
#define tile_driver_printf printf_prefix
#define tile_driver_printf_no_prefix printf
...
typedef enum TILE_FUNC_ID_ENUM
{
    ...
    TILE_FUNC_LSC_ID = (45),
    TILE_FUNC_NSL2_ID = (213),
    TILE_FUNC_UDM_ID = (322),
    TILE_FUNC_C24_ID = (92),
    ...
    TILE_FUNC_NSL2B_ID = (223),
    TILE_FUNC_NBC2_ID = (210),
...
#define ERROR_MESSAGE_DATA(n, CMD) \
    ...
    CMD(n, ISP_MESSAGE_INVALID_DIRECT_LINK_REG_FILE_ERROR, ISP_TPIPE_MESSAGE_FAIL)\
    CMD(n, ISP_MESSAGE_INVALID_CAL_DUMP_FILE_ERROR, ISP_TPIPE_MESSAGE_FAIL)\
    CMD(n, ISP_MESSAGE_DISABLE_C_MODEL_DIRECT_LINK, ISP_TPIPE_MESSAGE_FAIL)\
    CMD(n, ISP_MESSAGE_ZERO_SL_HRZ_COMP_ERROR, ISP_TPIPE_MESSAGE_FAIL)\
    /* tdr inverse */\
...
#define COMMON_TILE_INTERNAL_REG_LUT(CMD, a, b, c, d, e) \
    ...
    /* skip tile mode by c model */\
    CMD(a, b, c, d, e, int,,, run_c_model_direct_link,,,,, 2)\
    /* debug mode with invalid offset to enable recursive forward*/\
    CMD(a, b, c, d, e, int,,, recursive_forward_en,,,,, 2)\
    /* max tile width */\
    CMD(a, b, c, d, e, int,,, max_log_tile_width,,,,, 2)\
    /* max tile height */\
    CMD(a, b, c, d, e, int,,, max_log_tile_height,,,,, 2)\
    /* max log input width */\
    CMD(a, b, c, d, e, int,,, max_log_input_width,,,,, 2)\
    /* max log input height */\
    CMD(a, b, c, d, e, int,,, max_log_input_height,,,,, 2)\
    /* max input width */\
    CMD(a, b, c, d, e, int,,, max_input_width,,,,, 2)\
    /* max input height */\
    CMD(a, b, c, d, e, int,,, max_input_height,,,,, 2)\
    /* backup input width */\
    CMD(a, b, c, d, e, int,,, backup_max_input_width,,,,, 2)\
    /* backup input height */\
    CMD(a, b, c, d, e, int,,, backup_max_input_height,,,,, 2)\
    /* max log output width */\
    CMD(a, b, c, d, e, int,,, max_log_output_width,,,,, 2)\
    /* max log output height */\
    CMD(a, b, c, d, e, int,,, max_log_output_height,,,,, 2)\
    /* Tile IRQ */\
...
#define TILE_WRAPPER_REG_COPY(a, b, c, d, e, f, g, h, i, j, k, m, n, p,...) \
	...
	{\
		printf("replace reg %s, old val: %d, new val: %d\r\n", #i, (a)->i, (b)->i);\
		(a)->i = (b)->i;\
	}\
	...
	{\
		...
		{\
			...
			{\
				...
				{\
					(d)++;\
					printf("Diff reg %s, isp: %d, direct: %d\r\n", #i, (a)->i, (b)->i);\
				}\
			}\
		}\
		else\
        ...
        {\
            ...
            {\
				...
				{\
					...
					{\
						memset(((b) + (a))->func_name, 0x20, MAX_TILE_FUNC_NAME_SIZE - 1);\
						memcpy(((b) + (a))->func_name + MAX_TILE_FUNC_NAME_SIZE - sizeof(#e), #e, sizeof(#e));\
