...
MBOOL CcuDrvImp::shutdown()
{
    ...
    ccu_cmd_st cmd_t = { {MSG_TO_CCU_SHUTDOWN, 0, 0, CCU_CAM_TG_NONE}, CCU_ENG_STATUS_INVALID };
    	ccu_cmd_st cmd_r;
...
MBOOL CcuDrvImp::waitIrq(CCU_WAIT_IRQ_ST* pWaitIrq)
{
    ...
    LOG_DBG("CCU_IOCTL_WAIT_IRQ ioctl call, arg is CCU_WAIT_IRQ_STRUCT, size: %zu\n", sizeof(CCU_WAIT_IRQ_STRUCT));

    Ta=dbgTmr.getUs();
    Ret = ioctl(this->m_Fd,CCU_IOCTL_WAIT_IRQ,&wait);
    Tb=dbgTmr.getUs();

    LOG_DBG("CCU_IOCTL_WAIT_IRQ ioctl call, time duraction : %d\n", (Tb-Ta));
...
MBOOL CcuDrvImp::sendCmd( ccu_cmd_st *pCMD_T, ccu_cmd_st *pCMD_R, MBOOL bSync )
{
    ...
    LOG_INF("CCU_IOCTL_ENQUE_COMMAND: msg(%d), tg(%d), in(%x), out(%x), st(%d)\n",
                pCMD_T->task.msg_id, \
                pCMD_T->task.tg_info, \
	...
	if ( bSync )
    {
		...
		//LOG_INF("(%d),(%p),(%p),(%x),(%x),(%d), \n",cmd_get.task.msg_id,
		LOG_INF("CCU_IOCTL_DEQUE_COMMAND ack: msg(%d)->ack(%d), tg(%d), in(%x), out(%x), st(%d), \n",
		        pCMD_T->task.msg_id, \
                pCMD_R->task.msg_id, \
                pCMD_T->task.tg_info, \
...
MBOOL CcuDrvImp::waitCmd( ccu_cmd_st *pCMD_R )
{
	...
	//LOG_INF("(%d),(%p),(%p),(%x),(%x),(%d), \n",cmd_get.task.msg_id,
	LOG_INF("CCU_IOCTL_DEQUE_COMMAND result: (%d),(%x),(%x),(%d), \n",pCMD_R->task.msg_id, \
				  pCMD_R->task.in_data_ptr, \
				  pCMD_R->task.out_data_ptr, \
...
size_t loadFileToBuffer(FILE *fp, unsigned int *buf)
{
    ...
    {
        rewind(fp);
        LOG_INF("read file into buffer, szTell=%zx\n", szTell);
        szRead = fread ( (void*) buf, sizeof(char), szTell, fp );
        LOG_INF("read file done\n");
    }
    else
    {
        LOG_ERR("file size <= 0, szTell=%zx\n", szTell);
    }

    if(szRead != szTell)
    {
        LOG_ERR("read file error: szRead=%zx, szTell=%zx\n", szRead, szTell);
    }
...
MBOOL loadBin(char *binPath, unsigned int *memBaseAddr, unsigned int *buf, unsigned int memSize, unsigned int offset)
{
    ...
    LOG_INF("open Bin file result:%p\n", pFile);
    if (pFile == NULL)
    {
...
MBOOL CcuDrvImp::checkSensorSupport(uint32_t sensor_idx)
{
    ...
    LOG_INF("open sensorPm file result:%p\n", pFile);
    ...
    LOG_INF("open sensorDm file result:%p\n", pFile);
