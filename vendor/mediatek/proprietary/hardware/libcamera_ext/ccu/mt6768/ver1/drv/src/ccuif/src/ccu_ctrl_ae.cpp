...
int CcuCtrlAe::init(MUINT32 sensorIdx, ESensorDev_T sensorDev)
{
	...
	if (!m_pDrvCcu->loadSensorBin(sensorIdx))
    {
...
bool CcuCtrlAe::ccuCtrlPreprocess_SensorInit(void *inDataPtr)
{
    ...
    infoIn->u32SensorId = m_sensorIdx;
    ...
    if (!m_pDrvCcu->setI2CChannel(sensorI2CInfo[m_sensorIdx].i2c_id))
    {
	...
	compatInfoIn->u32I2CId = sensorI2CInfo[m_sensorIdx].i2c_id;
