...
void CcuCtrlAe::fillUpAeInfo(AE_INFO_T *rAEISPInfo, CCU_AeAlgo *aeAlgoData, AE_CORE_INIT *aeInitData)
{
    ...
    LOG_DBG_MUST("[fillUpAeInfo] CamId %d", aeAlgoData->m_u4CamId);
    LOG_DBG_MUST("[fillUpAeInfo] m_bAEStable %d", aeAlgoData->m_bAEStable);
...
*/
   ...
   //#endif
    ...
    //Butterfly effect
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_ENABLE, aeAlgoData->m_bBtEffectEnable);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_IN_EFFECT, aeAlgoData->m_bBtInEffect);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_OVER, aeAlgoData->m_bBtOver);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_ORG_STABLE, aeAlgoData->m_bBtOrgStable);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_STABLE, aeAlgoData->m_bBtStable);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_PRE_DEV_STABLE, aeAlgoData->m_bBtPreDevStable);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_PRE_DEV_INDEX, aeAlgoData->m_u4BtPreDevIndex);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_B_V, aeAlgoData->m_u4BtBValue);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_C_V, aeAlgoData->m_u4BtCurValue);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_B_TICKET, aeAlgoData->m_u4BtBTicket);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_C_TICKET, aeAlgoData->m_u4BtCTicket);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_RightAfterWT, aeAlgoData->m_bBtRightAfterSync);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_SYNC, aeAlgoData->m_bBtSync);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_MAPPING, aeAlgoData->m_bBtMapping);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_TH, aeAlgoData->m_u4BtTh);
    setDebugTag(a_rAEDebugInfo,AE_TAG_BF_SPEED, aeAlgoData->m_u4BtSpeed);
...
bool CcuCtrlAe::ccuCtrlPreprocess(ccu_msg_id msgId, void *inDataPtr, void *inDataBuf)
{
    ...
    switch(msgId)
    {
        ...
        case MSG_TO_CCU_SET_AP_AE_ONCHANGE_DATA:
        {
...
bool CcuCtrlAe::ccuCtrlPreprocess_OnchangeData(void *inDataPtr, void *inDataBuf)
{
    ...
    LOG_DBG("Save NVRAM address in onchange data for EXIF dump.");
...
bool CcuCtrlAe::ccuCtrlPreprocess_SensorInit(void *inDataPtr, void *inDataBuf)
{
    ...
    if(sensorI2CInfo[sensor_nm_idx].slave_addr == 0)
    ...
    if(sensorI2CInfo[sensor_nm_idx].i2c_id != 2
       && sensorI2CInfo[sensor_nm_idx].i2c_id != 4
       && sensorI2CInfo[sensor_nm_idx].i2c_id != 6
       && sensorI2CInfo[sensor_nm_idx].i2c_id != 7)
    {
        LOG_DBG_MUST("i2c bus not support\n");
        return false;
    }

    //======== Get i2c dma buffer mva =========
    i2cBufIoArg.sensor_idx = sensor_nm_idx;
    i2cBufIoArg.i2c_id = sensorI2CInfo[sensor_nm_idx].i2c_id;
    ...
    compatInfoIn->sensorI2cSlaveAddr = sensorI2CInfo[sensor_nm_idx].slave_addr;
