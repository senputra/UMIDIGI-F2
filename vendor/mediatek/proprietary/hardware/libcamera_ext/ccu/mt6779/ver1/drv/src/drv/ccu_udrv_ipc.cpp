...
MBOOL CcuDrvIpc::sendCmd(struct ccu_msg *msg)
{
    ...
    m_ccuIpcPtr->ack = CCU_IPC_NO_ACK;
    m_ccuIpcPtr->write_cnt = m_ccuIpcPtr->write_cnt + 1;

    //since IPC buffer is on CCU SRAM, copy data with assignment
    //memcpy might access on unaligned address, causing SIGBUS error
    m_ccuIpcPtr->msg.feature_type = msg->feature_type;
    ...
    m_ccuIpcPtr->msg.sensor_idx = msg->sensor_idx;
    ...
    *m_ccuIntTriggerPtr = 1; //trigger interrupt
    ...
    while(m_ccuIpcPtr->ack == CCU_IPC_NO_ACK)
    {
        loopCount++;
    ...
    if(timeout)
    {
        LOG_ERR("CCU cmd timeout, ftype(%d), msg_id(%d), duration_us(%lf), ackValue(%d), loopCount(%d)", 
            msg->feature_type, msg->msg_id, duration, ackValue, loopCount);
        return MFALSE;
    }

    LOG_DBG("-, ftype(%d), msg_id(%d), duration_us(%lf), loopCount(%d)", 
        msg->feature_type, msg->msg_id, duration, loopCount);
    return MTRUE;
}
