...
{
    LOG_DBG("+: with sensorIdx, sensorDev\n",__FUNCTION__);
...
int AbsCcuCtrlBase::init()
{
    ...
    //     return -CCU_CTRL_ERR_GENERAL;
    // }
    m_pBufferList = ccuMgr->getCcuBufferList(m_featureType, m_sensorIdx);
    ...
    LOG_DBG("mailboxInBuf va: %p, mva: 0x%x ", m_pBufferList->mailboxInBuf.getVa(), m_pBufferList->mailboxInBuf.getMva());
    LOG_DBG("mailboxOutBuf va: %p, mva: 0x%x ", m_pBufferList->mailboxOutBuf.getVa(), m_pBufferList->mailboxOutBuf.getMva());

    for(int i=0 ; i<CCU_MSG_CNT_MAX ; i++)
    {
        LOG_DBG("CtrlMsgBufs[%d] va: %p, mva: 0x%x ",i , m_pBufferList->CtrlMsgBufs[i].getVa(), m_pBufferList->CtrlMsgBufs[i].getMva());
    }
...
MBOOL AbsCcuCtrlBase::resolveCmdIOBuf(const struct CcuCtrlParamDescriptor *paramDescptr, CcuIOBufInfo *inDataBufInfo, CcuIOBufInfo *outDataBufInfo)
{
    ...
    else if((inDataSize > CCU_IPC_IBUF_CAPACITY))
    {
        ...
        inDataBufInfo->addr_ap = (MUINT8 *)m_pBufferList->mailboxInBuf.getVa();
        inDataBufInfo->addr_ap_interm = (MUINT8 *)m_pBufferList->mailboxInBuf.getVa();
        inDataBufInfo->addr_ccu = m_pBufferList->mailboxInBuf.getMva();
    }
    else
    {
    ...
    else if((outDataSize > CCU_IPC_OBUF_CAPACITY))
    {
        ...
        outDataBufInfo->addr_ap  = (MUINT8 *)m_pBufferList->mailboxOutBuf.getVa();
        outDataBufInfo->addr_ap_interm = (MUINT8 *)m_pBufferList->mailboxOutBuf.getVa();
        outDataBufInfo->addr_ccu = m_pBufferList->mailboxOutBuf.getMva();
    }
    else
    {
...
MBOOL AbsCcuCtrlBase::copyIntermCmdInDataIoBuf(CcuIOBufInfo *inDataBufInfo)
{
    LOG_DBG("Intermediate In Data IO buffer copy+");
...
MBOOL AbsCcuCtrlBase::copyIntermCmdOutDataIoBuf(CcuIOBufInfo *outDataBufInfo)
{
    LOG_DBG("Intermediate Out Data IO buffer copy+");

    MUINT32 *interIoBuf = (MUINT32 *)outDataBufInfo->addr_ap_interm;
    MUINT32 *realIoBuf = (MUINT32 *)outDataBufInfo->addr_ap;
...
bool AbsCcuCtrlBase::validateCommand(uint32_t msgId)
{
    enum ccu_feature_type fType = _getFeatureType();
    ...
    if((msgId < cmdSetInfo->msgIdMin) || (msgId >= cmdSetInfo->msgIdMax))
    {
        LOG_ERR("msgId invalid: ft(%d), id(%d), min(%d), max(%d)\n", fType, msgId, cmdSetInfo->msgIdMin, cmdSetInfo->msgIdMax);
        return false;
    }
...
int AbsCcuCtrlBase::ccuControl(uint32_t msgId, void *inDataPtr, void *outDataPtr)
{
    enum ccu_feature_type featureType;
    featureType = _getFeatureType();

    LOG_DBG("[%s]+: ft(%d), msgId(%d)\n", __FUNCTION__, featureType, msgId);

    if(!m_initialized)
    {
        LOG_ERR("CcuCtrl not initialized. ft(%d), msgId(%d)", featureType, msgId);
        return -CCU_CTRL_ERR_GENERAL;
    }
    ...
    //check if command id is in valid range
    if(!validateCommand(msgId))
    {
        return -CCU_CTRL_ERR_GENERAL;
    }

    //Get control param descriptor from pre-defined list
    const struct CcuCtrlParamDescriptor *paramDescptr = CcuCmdParamInfo::getParamDescriptor(featureType, msgId);

    UTIL_TRACE_BEGIN(paramDescptr->msg_literal);

    DEBUG_LOAD_ONLY(LOG_DBG_MUST("cotnrol msgId: %d, msg: %s.\n", msgId, paramDescptr->msg_literal))
    ...
    if(paramDescptr->need_input)
    {
        ...
        CcuControlHandleInput(msgId, paramDescptr, inDataPtr, inDataBufInfo.addr_ap_interm);
    }

    //Preprocess: final call for modifying input data before send to CCU (if needed)
    ...
    {
        copyIntermCmdInDataIoBuf(&inDataBufInfo);
    }

    uint32_t ret = sendCcuCommnadIpc(msgId, inDataBufInfo.addr_ccu, outDataBufInfo.addr_ccu);
    if(ret != CCU_CTRL_SUCCEED)
    {
        LOG_WRN("sendCcuCommnadIpc failed, msgId(%d)", msgId);
        return -CCU_CTRL_ERR_GENERAL;
    }
    ...
    if(paramDescptr->need_output)
    {
        ...
        LOG_DBG("need outpute, outDataPtr %p outDataBufInfo.addr_ap %p outDataBufInfo.addr_ap_interm %p\n",
            outDataPtr,outDataBufInfo.addr_ap, outDataBufInfo.addr_ap_interm);

        if(outDataBufInfo.bufType == CCU_SRAM_IO)
        {
            copyIntermCmdOutDataIoBuf(&outDataBufInfo);
        }

        memcpy(outDataPtr, outDataBufInfo.addr_ap_interm, paramDescptr->output_non_ptr_total_size);
    }

    //Postprocess: for modifying output data from CCU (if needed)
...
int AbsCcuCtrlBase::CcuControlHandleInput(uint32_t msgId, const struct CcuCtrlParamDescriptor *paramDescptr, void *inDataPtr, void *inDataBuf)
{
    LOG_DBG("+:%s\n",__FUNCTION__);
    ...
    if(ptrDescptr != NULL)
    {
        struct CcuBuffer *ctrlMsgBuf = &m_pBufferList->CtrlMsgBufs[msgId];
        LOG_DBG("ringCnt(%d), using ringIdx(%d)\n", ctrlMsgBuf->ringCnt, ctrlMsgBuf->ringIdx);
        ...
        int8_t *ion_ptr_content_va = (int8_t *)ctrlMsgBuf->getVa();
        CCU_COMPAT_PTR_T ion_ptr_content_mva = (CCU_COMPAT_PTR_T)ctrlMsgBuf->getMva();
        ...
        for(i=0; ptrDescptr->valid; i++)
        {
            ...
            *((CCU_COMPAT_PTR_T *)compat_ptr_member_addr) = ion_ptr_content_mva;
            ...
            LOG_DBG("ptrDescptr[%d], compat_ptr_member_val: %x\n", i, *((CCU_COMPAT_PTR_T *)compat_ptr_member_addr));
            ...
            LOG_DBG("ptrDescptr[%d], ion_ptr_content_mva: %x\n", i, ion_ptr_content_mva);
...
int AbsCcuCtrlBase::sendCcuCommnadIpc(uint32_t msgId, MUINT32 inDataAddrCcu, MUINT32 outDataAddrCcu)
{
    LOG_DBG("+:%s\n",__FUNCTION__);

    enum ccu_feature_type featureType = _getFeatureType();

    LOG_DBG("sending ccu ipc command : featureType(%d), msdId(%d), tg(-), sensorIdx(%d), inDataAddrCcu(0x%x)\n", 
        featureType, msgId, m_sensorIdx, inDataAddrCcu);

    struct ccu_msg msg = {featureType, msgId, inDataAddrCcu, outDataAddrCcu, CCU_CAM_TG_NONE, m_sensorIdx};

    LOG_DBG_MUST("RCDBG: [%s] m_pDrvCcu(%p)", __FUNCTION__, m_pDrvCcu);
...
enum ccu_tg_info AbsCcuCtrlBase::getCcuTgInfowithsensorDev(ESensorDev_T sensorDev, MUINT32 sensorIdx)
{
    return CcuDrvUtil::sensorDevToTgInfo(sensorDev, sensorIdx);
}
