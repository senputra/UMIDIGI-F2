...
enum ccu_tg_info CcuDrvUtil::sensorDevToTgInfo(ESensorDev_T sensorDev, MUINT32 sensorIdx)
{
    ...
    if (pIHalSensorList == NULL) {
        AEE_ASSERT_CCU_USER("Sensor TG conversion error: HalSensorList is null");
        return CCU_CAM_TG_NONE;
    }
    IHalSensor* pIHalSensor = pIHalSensorList->createSensor("ccu_ctrl", sensorIdx);
    if (pIHalSensor == NULL) {
        AEE_ASSERT_CCU_USER("Sensor TG conversion error: HalSensor is null");
        return CCU_CAM_TG_NONE;
    }
    ...
    switch(rSensorDynamicInfo.TgInfo){
        ...
        default:
            ...
            AEE_ASSERT_CCU_USER("Sensor TG conversion error: sensorDev(%d), sensorIdx(%d) => converted TG(%d)", 
                sensorDev, sensorIdx, rSensorDynamicInfo.TgInfo);
...
MUINT32 CcuDrvUtil::sensorIdxToDev(MUINT32 sensorIdx)
{
    ...
    IHalSensorList* const pIHalSensorList = MAKE_HalSensorList();
    if (pIHalSensorList == NULL) {
        AEE_ASSERT_CCU_USER("Sensor dev conversion error: HalSensorList is null");
    ...
    {        
        AEE_ASSERT_CCU_USER("Sensor dev conversion error:  sensorIdx(%d) => sensorDev(%d)", 
        sensorIdx, sensorDev);
...
double CcuDrvUtil::diffTimeUs(double ts, double te)
{
    if(ts > te)
        return DBL_MAX - ts + te;
    else
        return te - ts;
}

void CcuDrvUtil::delayInUs(double delay)
{
    double duration = 0;
