...
// Since SRC for throttle may have problem, using AWS(interpolation) to replace
#define ENABLE_AWS_FOR_THROTTLE_CONTROL
// Debugging only
#define ENABLE_SRC_DUMP
// Debugging only
#define ENABLE_AWS_DUMP
...
#define USB_SPH_DUMP_DLSRCOUT_NAME "dlSRCout"
#define USB_SPH_DUMP_DLAWSOUT_NAME "dlAWSout"
#define USB_SPH_DUMP_USB2UL_NAME "usbSRCul"
#define USB_SPH_DUMP_ULSRCOUT_NAME "ulSRCout"
#define USB_SPH_DUMP_ULAWSOUT_NAME "ulAWSout"
#define USB_SPH_DUMP_SRCAWS_PROPERTY "vendor.usbsph.srcaws.pcm.dump"

#define USB_SPH_DUMP_DL_ALIASING_STREAM_OUT_PROPERTY "vendor.streamout.pcm.dump"
#define USB_SPH_DUMP_UL_ALIASING_STREAM_IN_PROPERTY "vendor.streamin.pcm.dump"
...
void *AudioUSBPhoneCallController::speechDLThread(void *arg) {
    ...
    if (pcmHAL2USBDumpFile == NULL) {
        pcmHAL2USBDumpFile = pcmDumpOpen(USB_SPH_DUMP_HAL2USB_NAME, USB_SPH_DUMP_DL_ALIASING_STREAM_OUT_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
    }
    ...
    controller->mUSBOutStream.mWriteDumpByteAddr = 0;
    controller->mUSBOutStream.mReadDumpByteAddr = 0;
    controller->mUSBOutStream.mAWSoutDumpByteAddr = 0;
    ...
    if (pcmDL2HALDumpFile == NULL) {
        pcmDL2HALDumpFile = pcmDumpOpen(USB_SPH_DUMP_DL2HAL_NAME, USB_SPH_DUMP_DL_ALIASING_STREAM_OUT_PROPERTY, pcmULConfig->rate, audio_format_from_pcm_format(pcmULConfig->format), pcmULConfig->channels);
    }

    FILE *pcmDLSRCOUTDumpFile = NULL;
#if defined(ENABLE_SRC_DUMP)
    pcmDLSRCOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_DLSRCOUT_NAME, USB_SPH_DUMP_SRCAWS_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
#if 0
    if (pcmDLSRCOUTDumpFile == NULL) {
        pcmDLSRCOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_DLSRCOUT_NAME, USB_SPH_DUMP_DL_ALIASING_STREAM_OUT_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
    }
#endif
#endif

    FILE *pcmDLAWSOUTDumpFile = NULL;
#if defined(ENABLE_AWS_DUMP)
    pcmDLAWSOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_DLAWSOUT_NAME, USB_SPH_DUMP_SRCAWS_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
#if 0
    if (pcmDLAWSOUTDumpFile == NULL) {
        pcmDLAWSOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_DLAWSOUT_NAME, USB_SPH_DUMP_DL_ALIASING_STREAM_OUT_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
    }
#endif
#endif
    // prepare rate, format, channel conversion
    initBliSrc(&controller->mUSBOutStream);
#if defined(ENABLE_AWS_FOR_THROTTLE_CONTROL)
    initAwsSrc(&controller->mUSBOutStream);
#endif
    ...
    while (controller->isEnable()) {
        ...
        if (bytesAfterBliSrc > 0) {
            pcmDumpWriteData(pcmDLSRCOUTDumpFile, pBufferAfterBliSrc, bytesAfterBliSrc);
        }

        // aws
        void *pBufferAfterAwsSrc = NULL;
        uint32_t bytesAfterAwsSrc = 0;
        doAwsSrc(&controller->mUSBOutStream,
                 pBufferAfterBliSrc, bytesAfterBliSrc,
                 &pBufferAfterAwsSrc, &bytesAfterAwsSrc);

        if (bytesAfterAwsSrc > 0) {
            pcmDumpWriteData(pcmDLAWSOUTDumpFile, pBufferAfterAwsSrc, bytesAfterAwsSrc);
        }
        // bit conversion
        ...
        controller->mUSBOutStream.mReadDumpByteAddr = controller->mUSBOutStream.mReadDumpByteAddr + readSize;
        controller->mUSBOutStream.mWriteDumpByteAddr = controller->mUSBOutStream.mWriteDumpByteAddr + bytesAfterPending;
        controller->mUSBOutStream.mAWSoutDumpByteAddr = controller->mUSBOutStream.mAWSoutDumpByteAddr + bytesAfterAwsSrc;
    ...
    deinitAwsSrc(&controller->mUSBOutStream);
    ...
    // close src
    pcmDumpClose(pcmDLAWSOUTDumpFile);
    pcmDumpClose(pcmDLSRCOUTDumpFile);
...
void *AudioUSBPhoneCallController::speechULThread(void *arg) {
    ...
    if (pcmUSB2HALDumpFile == NULL) {
        pcmUSB2HALDumpFile = pcmDumpOpen(USB_SPH_DUMP_USB2HAL_NAME, USB_SPH_DUMP_UL_ALIASING_STREAM_IN_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
    }
    ...
    controller->mUSBInStream.mWriteDumpByteAddr = 0;
    controller->mUSBInStream.mReadDumpByteAddr = 0;
    controller->mUSBInStream.mAWSoutDumpByteAddr = 0;
    ...
    if (pcmHAL2ULDumpFile == NULL) {
        pcmHAL2ULDumpFile = pcmDumpOpen(USB_SPH_DUMP_HAL2UL_NAME, USB_SPH_DUMP_UL_ALIASING_STREAM_IN_PROPERTY, pcmDLConfig->rate, audio_format_from_pcm_format(pcmDLConfig->format), pcmDLConfig->channels);
    }

    FILE *pcmULSRCOUTDumpFile = NULL;
#if defined(ENABLE_SRC_DUMP)
    pcmULSRCOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_ULSRCOUT_NAME, USB_SPH_DUMP_SRCAWS_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
#if 0
    if (pcmULSRCOUTDumpFile == NULL) {
        pcmULSRCOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_ULSRCOUT_NAME, USB_SPH_DUMP_UL_ALIASING_STREAM_IN_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
    }
#endif
#endif

    FILE *pcmULAWSOUTDumpFile = NULL;
#if defined(ENABLE_AWS_DUMP)
    pcmULAWSOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_ULSRCOUT_NAME, USB_SPH_DUMP_SRCAWS_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
#if 0
    if (pcmULAWSOUTDumpFile == NULL) {
        pcmULAWSOUTDumpFile = pcmDumpOpen(USB_SPH_DUMP_ULSRCOUT_NAME, USB_SPH_DUMP_UL_ALIASING_STREAM_IN_PROPERTY, proxy->alsa_config.rate, audio_format_from_pcm_format(proxy->alsa_config.format), proxy->alsa_config.channels);
    }
#endif
#endif
    // prepare rate, format, channel conversion
...
#if defined(ENABLE_AWS_FOR_THROTTLE_CONTROL)
    initAwsSrc(&controller->mUSBInStream);
#endif
    ...
    while (controller->isEnable()) {
        ...
        if (bytesAfterBliSrc > 0) {
            pcmDumpWriteData(pcmULSRCOUTDumpFile, pBufferAfterBliSrc, bytesAfterBliSrc);
        }

        // aws
        void *pBufferAfterAwsSrc = NULL;
        uint32_t bytesAfterAwsSrc = 0;
        doAwsSrc(&controller->mUSBInStream,
                 pBufferAfterBliSrc, bytesAfterBliSrc,
                 &pBufferAfterAwsSrc, &bytesAfterAwsSrc);
        if (bytesAfterAwsSrc > 0) {
            pcmDumpWriteData(pcmULAWSOUTDumpFile, pBufferAfterAwsSrc, bytesAfterAwsSrc);
        }
        ...
        controller->mUSBInStream.mReadDumpByteAddr = controller->mUSBInStream.mReadDumpByteAddr + readSize;
        controller->mUSBInStream.mWriteDumpByteAddr = controller->mUSBInStream.mWriteDumpByteAddr + writeSize;
        controller->mUSBInStream.mAWSoutDumpByteAddr = controller->mUSBInStream.mAWSoutDumpByteAddr + bytesAfterAwsSrc;
        // hal 2 ul time
        clock_gettime(CLOCK_REALTIME, &mNewTime);
        latency[2] = calc_time_diff(mNewTime, mOldTime);
        mOldTime = mNewTime;
    ...
    deinitAwsSrc(&controller->mUSBInStream);
...
status_t AudioUSBPhoneCallController::initBliSrc(struct USBStream *stream) {
    ...
    if (stream->direction == SPH_DL) {
        ...
        stream->mSrcOutRate = targetRate = proxy_get_sample_rate(proxy);
        targetChannels = proxy_get_channel_count(proxy);
        if (targetChannels == 2 &&
            audio_format_from_pcm_format(stream->pcmConfigHAL2AFE.format) == AUDIO_FORMAT_PCM_16_BIT) {
            stream->mIsSrcOut_2ch16bit = true;
        } else {
            stream->mIsSrcOut_2ch16bit = false;
        }
    } else {
        ...
        stream->mSrcOutRate = targetRate = stream->pcmConfigHAL2AFE.rate;
    ...
    //    {
    ALOGD("%s(), dir %d, sample_rate: %d => %d, num_channels: %d => %d, mStreamAttributeSource->audio_format: 0x%x, mIsSrcOut_2ch16bit %d",
          __FUNCTION__, stream->direction, sourceRate,  targetRate, sourceChannels, targetChannels, format, stream->mIsSrcOut_2ch16bit);
...
status_t AudioUSBPhoneCallController::throttleInit(struct USBStream *stream) {
    ...
    ALOGD("%s(), dir %d, state %d, TargetAvail %u, KickInAvailDiff %u, current in rate %u, current out rate %u",
          __FUNCTION__,
          stream->direction,
...
status_t AudioUSBPhoneCallController::throttleSetSrcOutRate(struct USBStream *stream, unsigned int rate) {
    stream->throttleCurrentOutRate = rate;
    return NO_ERROR;
}

status_t AudioUSBPhoneCallController::throttleResetCurrentRate(struct USBStream *stream) {
    if (stream->awssrcOutBuffer != NULL && stream->mIsSrcOut_2ch16bit) {
        throttleSetSrcOutRate(stream, getStreamWriteRate(stream));
    } else {
...
status_t AudioUSBPhoneCallController::throttleSetCompensateInRate(struct USBStream *stream,
                                                                  ...
                                                                  int state) {
    if (stream->awssrcOutBuffer != NULL && stream->mIsSrcOut_2ch16bit) {
        float rateFactor = state == USB_THROTTLE_DECREASE ? (1 - 0.001f * changeMsPerSecond) : (1 + 0.001f * changeMsPerSecond);
        throttleSetSrcOutRate(stream, getStreamWriteRate(stream) * rateFactor);
    } else {
    ...
    ALOGV("%s(), dir %d, current in rate %u, current out rate %u, state %d, changeMsPerSecond %u",
          __FUNCTION__,
          stream->direction,
          stream->throttleCurrentInRate,
          stream->throttleCurrentOutRate,
...
static const uint32_t kAwsSrcOutputBufferSize = 0x10000;  // 64k
status_t AudioUSBPhoneCallController::initAwsSrc(struct USBStream *stream) {

    if (stream->mIsSrcOut_2ch16bit) {
        stream->awssrcOutBuffer = new char[kAwsSrcOutputBufferSize];
        ASSERT(stream->awssrcOutBuffer != NULL);
        ALOGD("%s(),dir [%d] mIsSrcOut_2ch16bit [%d]", __FUNCTION__, stream->direction, stream->mIsSrcOut_2ch16bit);
    } else {
        ALOGW("%s(),dir [%d] mIsSrcOut_2ch16bit [%d]", __FUNCTION__, stream->direction, stream->mIsSrcOut_2ch16bit);
    }
    return NO_ERROR;
}

status_t AudioUSBPhoneCallController::deinitAwsSrc(struct USBStream *stream) {

    if (stream->awssrcOutBuffer != NULL) {
        delete[] stream->awssrcOutBuffer;
        stream->awssrcOutBuffer = NULL;
    }

    return NO_ERROR;
}

status_t AudioUSBPhoneCallController::doAwsSrc(struct USBStream *stream, void *pInBuffer, uint32_t inBytes,
                                               void **ppOutBuffer, uint32_t *pOutBytes) {

    if (stream->mIsSrcOut_2ch16bit && stream->awssrcOutBuffer != NULL &&
        (stream->throttleState == USB_THROTTLE_INCREASE || stream->throttleState == USB_THROTTLE_DECREASE) &&
        stream->throttleCurrentOutRate != stream->mSrcOutRate) {
        uint32_t inputFrameCount = inBytes/(2 * sizeof(int16_t));
        uint32_t outputFrameCount = (inputFrameCount * stream->throttleCurrentOutRate) / stream->mSrcOutRate;
        uint32_t outBytes;
        if (inputFrameCount == outputFrameCount) {
            if (stream->throttleState == USB_THROTTLE_INCREASE) {
                outputFrameCount = inputFrameCount + 1;
            } else if (inputFrameCount > 16){
                outputFrameCount = inputFrameCount - 1;
            }
        }
        outBytes = outputFrameCount * 2 * sizeof(int16_t);
        audio_service_aws_src_process((int16_t *)pInBuffer, inBytes, (int16_t *) stream->awssrcOutBuffer, outBytes);
        *ppOutBuffer = stream->awssrcOutBuffer;
        *pOutBytes = outBytes;
        ALOGV("%s(),dir [%d], i_bytes %d, i_fc %d, o_bytes %d, o_fc %d, b4 rate %d, af rate %d R[%d]/SO[%d]/W[%d]",
            __FUNCTION__, stream->direction, inBytes, inputFrameCount, outBytes, outputFrameCount, stream->mSrcOutRate, stream->throttleCurrentOutRate,
            stream->mReadDumpByteAddr, stream->mAWSoutDumpByteAddr, stream->mWriteDumpByteAddr);
    } else {
        *ppOutBuffer = pInBuffer;
        *pOutBytes = inBytes;
    }
