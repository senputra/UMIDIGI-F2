...
#include "mt6771_afc.h"

// XO_COFST_FPM (for adjusting internal capacitor)
#define MT6358_XO_COFST_FPM_ADDR        0x796
#define MT6358_XO_COFST_FPM_MASK        0x3
#define MT6358_XO_COFST_FPM_SHIFT       14

// XO_CORE_IDAC_FPM (for adjusting OSC FPM setting)
#define MT6358_XO_CORE_IDAC_FPM_ADDR    0x79a
#define MT6358_XO_CORE_IDAC_FPM_MASK    0x3
#define MT6358_XO_CORE_IDAC_FPM_SHIFT   14

// XO_AAC_FPM_SWEN (for Cal_OSC)
#define MT6358_XO_AAC_FPM_SWEN_ADDR     0x79e
#define MT6358_XO_AAC_FPM_SWEN_MASK     0x1
#define MT6358_XO_AAC_FPM_SWEN_SHIFT    14

// XO_CDAC_FPM (for capID)
#define MT6358_XO_CDAC_FPM_ADDR         0x794
#define MT6358_XO_CDAC_FPM_MASK         0xFF
#define MT6358_XO_CDAC_FPM_SHIFT        0

// AUXADC (for temperature)
#define MT6358_AUXADC_ADC_RQST_CH7_ADDR  0x1108
#define MT6358_AUXADC_ADC_RQST_AND_CH7_MASK     1
#define MT6358_AUXADC_ADC_RQST_AND_CH7_SHIFT    7

#define MT6358_AUXADC_ADC_OUT_CH7_ADDR  0x10aa
// MT6358_AUXADC_ADC_OUT_CH7_MASK is 0x7FFF but we also need RDY (15th) bit
#define MT6358_AUXADC_ADC_OUT_AND_RDY_CH7_MASK  0xFFFF
#define MT6358_AUXADC_ADC_RDY_CH7_MASK          0x1
#define MT6358_AUXADC_ADC_RDY_CH7_SHIFT         15

#define OFDM_PD_EN_BIT 22
#define CCK_PD_EN_BIT 21

PLT_CORE_AFC plt_core_afc = core_afc;

void WIFI_MCR_Read(unsigned int addr, unsigned int *val)
{
    int ioctl_sock = 0;
    struct iwreq wrq;
    NDIS_TRANSPORT_STRUCT rNdisStruct;
    struct CMD_ACCESS_REG *prMCRStruct;

    prMCRStruct = (struct CMD_ACCESS_REG *)rNdisStruct.ndisOidContent;
    /* zeroize */
    memset(&wrq, 0, sizeof(struct iwreq));

    prMCRStruct->u4Address = addr;
    prMCRStruct->u4Data = 0;
    /* zeroize */
    rNdisStruct.ndisOidCmd = OID_CUSTOM_MCR_RW;
    rNdisStruct.inNdisOidlength = sizeof(struct CMD_ACCESS_REG);
    rNdisStruct.outNdisOidLength = sizeof(struct CMD_ACCESS_REG);
    ...
    ioctl(ioctl_sock, IOCTL_GET_STRUCT, &wrq);
    close(ioctl_sock);

    *val = prMCRStruct->u4Data;
    ERR("Get Mcr ,reg[0x%08x], val[0x%08x]\n", addr, *val);

}

void WIFI_MCR_Write(unsigned int addr, unsigned int val)
{
    int ioctl_sock = 0;
    struct iwreq wrq;
    NDIS_TRANSPORT_STRUCT rNdisStruct;
    ...
    ioctl_sock = socket(AF_INET, SOCK_DGRAM, 0);
    ioctl(ioctl_sock, IOCTL_SET_STRUCT, &wrq);
    close(ioctl_sock);

    ERR("set Mcr ,reg[0x%08x], val[0x%08x]\n", addr, val);
...
static float query_temperature()
{
    ...
    update_pmic_reg(MT6358_AUXADC_ADC_RQST_CH7_ADDR, MT6358_AUXADC_ADC_RQST_AND_CH7_MASK,
                    MT6358_AUXADC_ADC_RQST_AND_CH7_SHIFT, 1);

    while (--retry >= 0) {
        ret = read_pmic_reg(MT6358_AUXADC_ADC_OUT_CH7_ADDR, &orig);
        ready_bit = (orig >> MT6358_AUXADC_ADC_RDY_CH7_SHIFT) & MT6358_AUXADC_ADC_RDY_CH7_MASK;
    ...
    auxadc = orig & MT6358_AUXADC_ADC_OUT_AND_RDY_CH7_MASK;
    DBG("Auxadc[%d]\n", auxadc);
...
int core_afc(float *temperature)
{
    ...
    //disable OFDM in RX mode
    WIFI_MCR_Read(0x60200600, &reg_val);
    reg_val &= ~(BIT(OFDM_PD_EN_BIT));
    WIFI_MCR_Write(0x60200600, reg_val);
    ...
    ret = update_pmic_reg(MT6358_XO_COFST_FPM_ADDR, MT6358_XO_COFST_FPM_MASK, MT6358_XO_COFST_FPM_SHIFT, 0);
    ...
    ret = update_pmic_reg(MT6358_XO_CDAC_FPM_ADDR, MT6358_XO_CDAC_FPM_MASK, MT6358_XO_CDAC_FPM_SHIFT, 0x55);
    ...
    ret = update_pmic_reg(MT6358_XO_CORE_IDAC_FPM_ADDR, MT6358_XO_CORE_IDAC_FPM_MASK, MT6358_XO_CORE_IDAC_FPM_SHIFT, 0x2);
    ...
    ret = update_pmic_reg(MT6358_XO_AAC_FPM_SWEN_ADDR, MT6358_XO_AAC_FPM_SWEN_MASK, MT6358_XO_AAC_FPM_SWEN_SHIFT, 0);
    ...
    ret = update_pmic_reg(MT6358_XO_AAC_FPM_SWEN_ADDR, MT6358_XO_AAC_FPM_SWEN_MASK, MT6358_XO_AAC_FPM_SWEN_SHIFT, 1);
    ...
    for (unsigned int i = 0; i < cap_id_num; i++) {
        // Set cap id
        ret = update_pmic_reg(MT6358_XO_CDAC_FPM_ADDR, MT6358_XO_CDAC_FPM_MASK, MT6358_XO_CDAC_FPM_SHIFT, cap_id_arr[i]);
    ...
    *temperature = query_temperature();
    ...
    if (best_cap_id >= 0) {
        ret = update_pmic_reg(MT6358_XO_CDAC_FPM_ADDR, MT6358_XO_CDAC_FPM_MASK, MT6358_XO_CDAC_FPM_SHIFT, best_cap_id);
