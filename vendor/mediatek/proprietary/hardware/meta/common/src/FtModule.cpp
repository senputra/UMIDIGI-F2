...
{  
    ...
    if(path==NULL || strlen(path)>255)
    {
        META_LOG("Your input directory is NULL or too long!");
        return;   
    }

    META_LOG("del_dir = %s", path);
    char *pathname = (char*)malloc(_MAXDIR_);  
    if(pathname==NULL)
    {
        META_LOG("del_dir() pathname is NULL");
        return;       
    }
    memset(pathname,0,_MAXDIR_);
    ...
    if(dp == NULL)  
    {  
        META_LOG("Your input directory is not exist!");  
        free(pathname);  
        pathname = NULL;  
        return;  
    }  
    ...
    {  
        ...
        if((strlen(pathname)+strlen(dirp->d_name)+1) > 255)
        {
            META_LOG("del_dir() strlen(pathname)+strlen(dirp->d_name)+1 >255 ");
            closedir(dp);  
    	    free(pathname);  
            pathname = NULL;  
            return;
	...
	META_LOG("pathname=%s", pathname);
        ...
        {   
            ret = remove(pathname);
            META_LOG("delete file=%s ret=%d errno=%d", pathname,ret,errno);
	}  
    ...
    if(strcmp(path,MDDB_FILE_FOLDER_EX) != 0)
    {
        rmdir(path);  	
    }
