...
int getEngLoad() {
    ...
    value = (strcmp("eng", property_value) == 0);
    return value;
}
...
int getUserLoad() {
    ...
    mtk_property_get("ro.build.type", property_value, "");
    value = (strcmp("user", property_value) == 0);
    return value;
}
    ...
    unsigned int length)
{
    ...
    for (unsigned int i=0; i<length; i++) {
        ...
        // if the buffer size is not enough for both temp and null terminator
        if ((strlen(buffer) + strlen(temp) + 1) > 4096) {
            // we print it directly because it shouldn’t happen often.
            LOGMUX(LOG_DEBUG, "syslogdump ERROR: buffer size is low for %s %s",
                prefix, temp);
        } else {
            strncat(buffer, temp, strlen(temp));
        }
    }
    LOGMUX(LOG_DEBUG, "%s %s", prefix, buffer);
...
static void dumpBuffer(GSM0710_Buffer *pBuf) {
    LOGMUX(LOG_DEBUG, "size %d, start %p, current %p, end %p",
            pBuf->datacount, pBuf->readp, pBuf->writep, pBuf->endp);

    if (pBuf->readp + pBuf->datacount > pBuf->endp) {// it's ring buffer
        syslogdump("dumpBuffer(R-E):", pBuf->readp, (pBuf->endp - pBuf->readp + 1));
        syslogdump("dumpBuffer(H-W):", pBuf->data,
                pBuf->datacount - ((pBuf->endp - pBuf->readp + 1)));
    } else {
        syslogdump("dumpBuffer:", pBuf->readp, pBuf->datacount);
    }
}
        ...
        if (c != prefix_length) {
            ...
            if (c < 0) {
                ...
                if (writeErrno==ETXTBSY || writeErrno==ENODEV) {
                    ...
                    mtk_property_set(PROPERTY_MODEM_EE, "1");
                    pthread_mutex_unlock(&write_frame_lock);
                    return length;
                } else {
...
static int logical_channel_establish(Channel *channel)
{
    if (setup_pty_interface(channel) != 0) {
        ...
        LOGMUX(LOG_DEBUG, "Connecting %s to virtual channel %d on %s", channel->ptsname, channel->id, serial.devicename);
    }
...
static int setup_pty_interface(Channel *channel)
{
    ...
    LOGMUX(LOG_VERBOSE, "setup_pty_interface SYSCHECK(channel->fd = open(channel->devicename, O_RDWR | O_NONBLOCK));");
    ...
    //create thread
    LOGMUX(LOG_DEBUG, "New channel properties: number: %d fd: %d device: %s", channel->id, channel->fd, channel->devicename);
...
{
    LOGMUX(LOG_VERBOSE, "Enter, LackofAtciChannel=%d",LackofAtciChannel);
    ...
    for (read_action_count = 0; read_action_count < MUXD_PTY_READ_ROUND; read_action_count++) {
        ...
        if (channel->tx_fc_off) {
            ...
            LOGMUX(LOG_VERBOSE, "Wait for FC On siganl");
            ...
            //channel->tx_fc_off = 0;
            LOGMUX(LOG_VERBOSE, "Notify by FC On siganl,try to read data and to send it");
        }
        pthread_mutex_unlock(&channel->tx_fc_lock);
#endif  /* __MUXD_FLOWCONTROL__ */

        if ((len = read(channel->fd, buf, sizeof(buf))) >= 0) {
            LOGMUX(LOG_VERBOSE, "pseudo_device_read read(channel->fd, buf, sizeof(buf))");
            LOGMUX(LOG_VERBOSE, "pseudo_device_read len=%d", len);
            ...
            if (channel->id == DEFAULT_MUX_UT_LB_TEST_CHNL_NUM) {
                ...
                LOGMUX(LOG_VERBOSE, "UT mode Simulate wirte data to modem with len=%d", len);
            } else {
#endif
            if (len > 0)
    ...
    LOGMUX(LOG_VERBOSE, "End of Execute 4 read actions for this time's read_fd is select");
    return 0;
...
{
    LOGMUX(LOG_VERBOSE, "Enter");
    LOGMUX(LOG_VERBOSE, "GSM0710 buffer (up-to-date): free %d, stored %d", gsm0710_buffer_free(buf), gsm0710_buffer_length(buf));
...
{
    if (frame->length > 0) {
        LOGMUX(LOG_VERBOSE, "frame_data ptr=0x%02X, frame_len=%d", (unsigned int)frame->data, frame->length);
...
static GSM0710_Frame *gsm0710_base_buffer_get_frame(GSM0710_Buffer *buf, int passed_depth_level)
{
    ...
    LOGMUX(LOG_VERBOSE, "Enter: depth_level=%d", passed_depth_level);
    ...
    while (!buf->flag_found && gsm0710_buffer_length(buf) > 0) {
        if (*buf->readp == GSM0710_FRAME_FLAG) {
            buf->flag_found = 1;
            LOGMUX(LOG_VERBOSE, "Find Start Flag");
        }

        pthread_mutex_lock(&buf->datacount_lock); /* need to lock to operate on buf->datacount*/
        LOGMUX(LOG_VERBOSE, "buf_readp val=0x%02X", *(buf->readp));
        gsm0710_buffer_inc(buf->readp, buf->datacount);
        LOGMUX(LOG_VERBOSE, "current buf_readp val=0x%02X", *(buf->readp));
        pthread_mutex_unlock(&buf->datacount_lock);
    }
    if (!buf->flag_found) { // no frame started
        LOGMUX(LOG_VERBOSE, "Leave. No start frame 0xf9 found in bytes stored in GSM0710 buffer");
        ...
        if (buf->input_sleeping == 1) {
            ...
            LOGMUX(LOG_VERBOSE, "Signal thread serial device read(): case2");
    ...
    while (gsm0710_buffer_length(buf) > 0 && (*buf->readp == GSM0710_FRAME_FLAG)) {
        pthread_mutex_lock(&buf->datacount_lock); /* need to lock to operate on buf->datacount*/
        gsm0710_buffer_inc(buf->readp, buf->datacount);
        LOGMUX(LOG_VERBOSE, "skip continus start flag:current buf_readp=0x%02X", *(buf->readp));
        pthread_mutex_unlock(&buf->datacount_lock);
    }
    ...
    LOGMUX(LOG_VERBOSE, "Ready to check Addr Field:buf_readp val=0x%02X", *(buf->readp));
    local_readp = buf->readp;
    pthread_mutex_lock(&buf->datacount_lock);
    // use lock to gurantee that the three variable are same value.
    local_datacount = buf->datacount;
    local_datacount_backup = buf->datacount;      /* current no. of stored bytes in buffer */
    pthread_mutex_unlock(&buf->datacount_lock);
    LOGMUX(LOG_DEBUG, "local_datacount %d", local_datacount);
    if (local_datacount >= length_needed) {                         /* enough data stored for 0710 frame header+footer? */
        if ((frame = (GSM0710_Frame *)malloc_r(sizeof(GSM0710_Frame))) != NULL) {
            ...
            /* Parse Address Field */
            LOGMUX(LOG_VERBOSE, "UIH Addr Filed value=0x%02X", *local_readp);
            ...
            LOGMUX(LOG_VERBOSE, "UIH Length Filed Byte#1=0x%02X,frame_length=%d", *local_readp, frame->length);
            fcs = r_crctable[fcs ^ *local_readp];
        ...
        LOGMUX(LOG_VERBOSE, "Before: frame_length=%d", frame->length);
        length_needed += frame->length; /*length_needed : 1 length byte + payload + 1 fcs byte + 1 end frame flag */
        LOGMUX(LOG_VERBOSE, "length_needed: %d, available in local_datacount: %d, frame_len=%d", length_needed, local_datacount, frame->length);
        ...
        LOGMUX(LOG_ERR, "Leave, not enough bytes stored in buffer for header information yet");
        dumpBuffer(buf);
        return NULL;
    }
    ...
    while (gsm0710_buffer_length(buf) > 0) {
        if (!buf->adv_found_esc && GSM0710_FRAME_ADV_FLAG == *(buf->readp)) { /* Whole frame parsed for escape chars, closing flag found */
            ...
            if (r_crctable[fcs ^ data[buf->adv_length - 1]] != 0xCF) {
                ...
                buf->received_count++;
                buf->flag_found = 0;
                LOGMUX(LOG_VERBOSE, "Leave success");
                return frame;
            }
        }
...
static int chat(int serial_device_fd, char * cmd, int to __unused)
{
    ...
    LOGMUX(LOG_VERBOSE, "Wrote %d bytes", wrote);
    ...
    while (1) {
        ...
        if (sel < 0) {
            ...
            if (FD_ISSET(serial_device_fd, &rfds)) {
                ...
                syslogdump("<s ", buf, len);
                gsm0710_buffer_write(buf_for_bootup, buf, len);
                LOGMUX(LOG_VERBOSE, "serial device buff: %s", buf_for_bootup->data);
                //errno = 0;
...
void write_tx_msc_response(void * input __unused) {
    while (1) {
        ...
        if (tx_msc_response_cache == NULL) {
            LOGMUX(LOG_ERR, "tx_msc_response_cache is null");
            pthread_mutex_unlock(&write_tx_msc_response_lock);
            continue;
        } else if (strlen(tx_msc_response_cache) == 0) {
            LOGMUX(LOG_ERR, "tx_msc_response_cache is invalid");
        ...
        LOGMUX(LOG_VERBOSE, "enter, data length:%d", length);
    ...
    case GSM0710_CONTROL_MSC:
        ...
        if (frame->control & GSM0710_PF)  // Check if the P/F var needs to be set again (cleared in write_frame)
            uih_pf_bit_received = 1;
        LOGMUX(LOG_VERBOSE, "Sending 1st MSC command App->Modem");
    ...
    if (frame->length > 0) {
        ...
        if ((type & GSM0710_CR) == GSM0710_CR) {
            ...
            if (supported) {
                ...
                tx_msc_response_cache = (char *)calloc(1, sizeof(char) *  frame->length+1);
                memcpy(tx_msc_response_cache, frame->data, frame->length);
                tx_msc_response_cache[frame->length] = '\0';
                pthread_mutex_unlock(&write_tx_msc_response_lock);
...
int extract_frames(GSM0710_Buffer *buf)
{
    ...
    while ((frame = cmux_mode ? gsm0710_advanced_buffer_get_frame(buf) : gsm0710_base_buffer_get_frame(buf, 0))) {
        ...
        if (buf->input_sleeping == 1) {
            ...
            //pthread_cond_signal(&newdataready_signal);
            /* [Bug] Note by LS: It should signal the thread waits for the bufferready_signal instead of newdataready_signal */
            LOGMUX(LOG_VERBOSE, "Signal thread serial device read(): case1");
            pthread_cond_signal(&buf->bufferready_signal);
        }
        pthread_mutex_unlock(&buf->bufferready_lock);

        if ((GSM0710_FRAME_IS(GSM0710_TYPE_UI, frame) || GSM0710_FRAME_IS(GSM0710_TYPE_UIH, frame))) {
            LOGMUX(LOG_VERBOSE, "Frame is UI or UIH");
                ...
                if (frame != NULL) {
                    ...
                    while (1) {
                        if ((write_result = write(channel->fd, frame->data, frame->length)) >= 0) {
                            int fsync_result = -1;
                            LOGMUX(LOG_DEBUG,
                                    "write() returned. Written %d/%d bytes of frame to %s,count %d",
                                    write_result, frame->length, channel->ptsname, buf->datacount);
                            ...
                            if (fsync_result < 0) {
                                LOGMUX(LOG_VERBOSE, "fsync() failed (%d, %d - %s)", fsync_result, errno, strerror(errno));
                            }
...
uih_process_done:
            ...
            case GSM0710_TYPE_UA:
                LOGMUX(LOG_VERBOSE, "Frame is UA");
                ...
                if (channellist[frame->channel].opened) {
                    ...
                    if (channellist[frame->channel].disc_ua_pending == 1) {
                        ...
                        LOGMUX(LOG_DEBUG, "Logical channel %d closed", frame->channel);
                        ...
                        LOGMUX(LOG_DEBUG, "Channel Opened:ignore this UA frame for channel=%d", frame->channel);
                    }
                    ...
                    if (channellist[frame->channel].sabm_ua_pending == 1) {
                        ...
                        if (frame->channel == 0) {
                            ...
                            LOGMUX(LOG_DEBUG, "Logical channel %d opened", frame->channel);
                        }
                ...
                if (channellist[frame->channel].opened) {
                    ...
                    if (frame->channel == 0) {
                        ...
                        if(frame->channel == 26) {
                            ...
                            mtk_property_set("vendor.lackof.atci.channel", "1");
                            LackofAtciChannel = 1;
                        }
                    }
                }
    ...
    while (buf != NULL) {
        ...
        while (!(buf->datacount > 0) || !buf->newdataready) {   /* true if no data available in buffer or no new data written since last extract_frames() call */
            if (buf->input_sleeping != 1) {                 /* when is sleeping, thread_serial_device_read is wait */
                LOGMUX(LOG_VERBOSE, "assemble_frame_thread put to sleep. GSM0710 buffer stored %d, newdataready: %d", buf->datacount, buf->newdataready);
                ...
                LOGMUX(LOG_VERBOSE, "assemble_frame_thread awoken. GSM0710 buffer stored %d, newdataready: %d", buf->datacount, buf->newdataready);
            } else {
...
#if 0
    mtk_property_set("ctl.stop", "ril-daemon");
    rild_started = 0;
#endif
    return 0;
}
...
void signal_treatment(int param)
{
    ...
    case SIGUSR1:
        LOGMUX(LOG_WARNING, "MUXD recv SIGUSR1,stop_muxd=%d",stop_muxd);
...
int thread_serial_device_read(void *vargp)
{
    ...
    LOGMUX(LOG_VERBOSE, "Enter");
    {
        ...
        case MUX_STATE_MUXING:
        {
            ...
            LOGMUX(LOG_VERBOSE, "Serial Data");
            ...
            if ((length = gsm0710_buffer_free(buf)) > 0) { /*available space in buffer (not locked since we want to utilize all available space)*/
                if ((len = read(serial->fd, buffer, min(length, sizeof(buffer)))) > 0) {
                    ...
                    LOGMUX(LOG_VERBOSE, "Waiting for data from serial device");
                } else {
                    switch (errno) {
                    case EINTR:
                        LOGMUX(LOG_ERR, "Interrupt signal EINTR caught");
                ...
                while (!((gsm0710_buffer_free(buf)) > 0)) {
                    LOGMUX(LOG_VERBOSE, "ser_read_thread put to sleep. GSM0710 buffer has %d bytes free", gsm0710_buffer_free(buf));
                    ...
                    LOGMUX(LOG_VERBOSE, "ser_read_thread awoken");
                }
                buf->input_sleeping = 0; /* unset sleeping flag - must be inside lock*/
                LOGMUX(LOG_VERBOSE, "Internal re-assembly buffer partly flushed, free space: %d", gsm0710_buffer_free(buf));
                pthread_mutex_unlock(&buf->bufferready_lock);
            }
            LOGMUX(LOG_VERBOSE, "Leave, keep watching");
            return 0;
            break;
        }
        default:
    ...
    LOGMUX(LOG_VERBOSE, "Leave, stop watching");
    return 1;
}
...
int start_muxer(Serial *serial)
{
    ...
    int timeout = 10;
    if (isEngLoad) {
        // Increase timeout to avoid assert (performance issue)
        timeout = 20;
    }
    if (poll_modem_ready(serial->fd, timeout) < 0)
...
int watchdog(Serial *serial)
{
    ...
    LOGMUX(LOG_VERBOSE, "Serial state is %d", serial->state);
    ...
    case MUX_STATE_INITILIZING:
        ...
        for (i = 1; i <= vir_ports; i++) {
            LOGMUX(LOG_DEBUG, "Allocating logical channel %d/%d ", i, vir_ports);
        ...
        if (rild_started == 0) {
            ...
            mtk_property_set("vendor.ril.mux.start", "1");
            mtk_property_set("vendor.ril.mtk", "1");
...
void *poll_thread(void *vargp)
{
    ...
    while (1) {
        ...
        if (select((poll_thread_arg->fd) + 1, &fdr, &fdw, NULL, NULL) > 0) {
            LOGMUX(LOG_VERBOSE, "poll_thread select found");
...
int main(int argc, char *argv[])
{
    ...
    mtk_property_set(PROPERTY_MODEM_EE, "0");
    mtk_property_get("ro.vendor.mtk_md1_support", prop_value, "0");
    enableMd1 = atoi(prop_value);
    mtk_property_get("ro.vendor.mtk_md2_support", prop_value, "0");
    enableMd2 = atoi(prop_value);
    mtk_property_get("ro.vendor.mtk_md5_support", prop_value, "0");
    enableMd5 = atoi(prop_value);
    isUserLoad = getUserLoad();
    isEngLoad = getEngLoad();
    ...
    while ((opt = getopt(argc, argv, "dov:s:t:p:f:n:h?m:b:P:")) > 0) {
        ...
        case 'b':
            ...
            cmux_port_speed = cmux_port_speed >= 0 ? cmux_port_speed : 5;
            break;
        default:
        case '?':
        case 'h':
    ...
    if ((serial.in_buf = gsm0710_buffer_init()) == NULL ||
            (serial.in_buf_for_bootup = gsm0710_buffer_init()) == NULL ||
            (cmux_mode == 1 &&
    ...
    while ((main_exit_signal == 0) && ((rc = watchdog(&serial)) == 0)) {
        ...
        /* Add codes for test force assert functionality by LS */
        {
            ...
            mtk_property_get("vendor.ril.mux.force.assert.flag", property_value, NULL);
            if (prop_value[0] == 'Y') {
                LOGMUX(LOG_ERR, "Detect Muxd Force Assert Flag as Y");
                mtk_property_set("vendor.ril.mux.force.assert.flag", "N");
    ...
    if (g_set_force_assert_flag == 1) {
        ...
        mtk_property_set("vendor.ril.mux.report.case", "1");
        mtk_property_set("vendor.ril.muxreport", "1");
