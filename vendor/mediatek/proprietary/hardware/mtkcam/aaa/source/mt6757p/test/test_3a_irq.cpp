...
public:
    ...
    {
        CAM_LOGD("[%s] +", __FUNCTION__);
        m_prIrq = NS3Av3::IEventIrq::createInstance(NS3Av3::IEventIrq::ConfigParam(0, 0, 5000, NS3Av3::IEventIrq::E_Event_Vsync), LOG_TAG);
...
private:
    static MVOID* threadLoop(MVOID* arg)
    {
        ...
        while (i < 30)
        {
            CAM_LOGD("[%s] (%d)", __FUNCTION__, i);
            _this->calculate();
            NS3Av3::IEventIrq::Duration rDuration;
            MINT32 i4EventCnt = _this->m_prIrq->wait(rDuration);
            CAM_LOGD("[%s] (%d, %d, %d)", __FUNCTION__, i4EventCnt, rDuration.i4Duration0, rDuration.i4Duration1);
            _this->apply();
            i ++;
        }
        CAM_LOGD("[%s] -", __FUNCTION__);
    ...
    MINT32 calculate()
    {
        ...
        for (i = 0; i < 50; i++)
        {
            ...
            CAM_LOGD("[%s] %d:dTest(%f)", __FUNCTION__, i, dTest);
        }
        m_dTest = dTest;
        CAM_LOGD("[%s]- dTest(%f)", __FUNCTION__, dTest);
        return 0;
    }

    MINT32 apply()
    {
        CAM_LOGD("[%s] m_dTest(%f)", __FUNCTION__, m_dTest);
        m_prIrq->mark();
        return 0;
    }
...
public:
    ...
    {
        CAM_LOGD("[%s] this(%p)", __FUNCTION__, this);
    }
    ...
    MBOOL initialize()
    {
        if (onInit())
        {
            NS3Av3::UPDATE_ENTRY_SINGLE(m_Request.halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, (MINT32)0);
            m_Request.appMeta = m_pTemplate->getMtkData(CAMERA3_TEMPLATE_PREVIEW);
            m_Controls.push_back(m_Request);
            NS3Av3::UPDATE_ENTRY_SINGLE(m_Request.halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, (MINT32)1);
            m_Controls.push_back(m_Request);
            NS3Av3::UPDATE_ENTRY_SINGLE(m_Request.halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, (MINT32)2);
            m_Controls.push_back(m_Request);
                        ...
                        )
    {
        ...
        if (_msgType == NS3Av3::IHal3ACb::eID_NOTIFY_3APROC_FINISH)
        {
            ...
            // aquire new request
            NS3Av3::UPDATE_ENTRY_SINGLE(m_Request.halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, magic+1);
            if (m_fgPrecap == 2)
            {
                m_fgPrecap = 0;
            ...
            {   // emulate precapture trigger
                NS3Av3::UPDATE_ENTRY_SINGLE(m_Request.appMeta, MTK_CONTROL_AE_PRECAPTURE_TRIGGER, (MUINT8)MTK_CONTROL_AE_PRECAPTURE_TRIGGER_START);
            }
            else
            {
                NS3Av3::UPDATE_ENTRY_SINGLE(m_Request.appMeta, MTK_CONTROL_AE_PRECAPTURE_TRIGGER, (MUINT8)MTK_CONTROL_AE_PRECAPTURE_TRIGGER_IDLE);
            }
            NS3Av3::UPDATE_ENTRY_SINGLE(m_Request.appMeta, MTK_STATISTICS_LENS_SHADING_MAP_MODE, (MUINT8)MTK_STATISTICS_LENS_SHADING_MAP_MODE_ON);
            m_Controls.push_back(m_Request);
            // set
            m_pIHal3A->set(m_Controls);
        }
...
private:
    MBOOL powerOnSensor()
    {
        ...
        {
            CAM_LOGE("sensor power on failed: %d", m_sensorIdx);
            goto lbExit;
        }
        //
        ret = MTRUE;
    ...
    MBOOL onInit()
    {
        ...
        if (!powerOnSensor())
        {
            CAM_LOGE("Fail to power on sensor");
            return MFALSE;
        }

        m_pIHal3A = MAKE_Hal3A(0, LOG_TAG);
        if (!m_pIHal3A)
        {
            CAM_LOGE("Fail to create IHal3A");
            return MFALSE;
        }
    ...
    MBOOL onUninit()
    {
        if (m_pHalSensor)
        {
            ...
            {
                CAM_LOGE("sensor power off failed: %d", m_sensorIdx);
                return MFALSE;
            }
            m_pHalSensor = NULL;
        }
    ...
    {
        CAM_LOGD("[%s] i4Magic(%d)", __FUNCTION__, i4Magic);
        return MTRUE;
    }
    MBOOL deque(MINT32 i4Magic)
    {
        CAM_LOGD("[%s] i4Magic(%d)", __FUNCTION__, i4Magic);
        NS3Av3::MetaSet_T result, result2;
        isp_reg_t regBuf;
        ...
        CAM_LOGD("[%s] AE(%d), AF(%d), AWB(%d)", __FUNCTION__, u1AeState, u1AfState, u1AwbState);
        ...
        if (NS3Av3::QUERY_ENTRY_SINGLE(result.appMeta, MTK_STATISTICS_LENS_SHADING_MAP_MODE, u1ShadingMapMode))
        {
            if (u1ShadingMapMode == MTK_STATISTICS_LENS_SHADING_MAP_MODE_ON)
            {
                ...
                {
                    CAM_LOGD("[%s] {%3.3f, %3.3f, %3.3f, %3.3f}", __FUNCTION__, rMap[0], rMap[1], rMap[2], rMap[3]);
                }
                else
                {
                    CAM_LOGE("Fail to get shading map");
                }
            }
        }
        if (NS3Av3::GET_ENTRY_ARRAY(result.halMeta, MTK_PROCESSOR_CAMINFO, rCamInfo.data, sizeof(NSIspTuning::RAWIspCamInfo_U)))
        {
            CAM_LOGD("rCamInfo.u4Id(%d)", rCamInfo.u4Id);
            CAM_LOGD("rCamInfo.eIspProfile(%d)", rCamInfo.eIspProfile);
            CAM_LOGD("rCamInfo.eSensorMode(%d)", rCamInfo.eSensorMode);
            CAM_LOGD("rCamInfo.eIdx_Scene(%d)", rCamInfo.eIdx_Scene);
            CAM_LOGD("rCamInfo.u4ISOValue(%d)", rCamInfo.u4ISOValue);
            CAM_LOGD("rCamInfo.eIdx_ISO(%d)", rCamInfo.eIdx_ISO);
            CAM_LOGD("rCamInfo.eIdx_PCA_LUT(%d)", rCamInfo.eIdx_PCA_LUT);
            CAM_LOGD("rCamInfo.eIdx_CCM(%d)", rCamInfo.eIdx_CCM);
            CAM_LOGD("rCamInfo.eIdx_Shading_CCT(%d)", rCamInfo.eIdx_Shading_CCT);
            CAM_LOGD("rCamInfo.i4ZoomRatio_x100(%d)", rCamInfo.i4ZoomRatio_x100);
            CAM_LOGD("rCamInfo.i4LightValue_x10(%d)", rCamInfo.i4LightValue_x10);
        }
    ...
    NS3Av3::MetaSet_T           m_Request;
    List<NS3Av3::MetaSet_T>     m_Controls;
...
static void usage()
{
    CAM_LOGD("Usage: test_3a <0: normal preview, 1: pip preview>\n");
}
...
static void TestCamInfo()
{
    ...
    {
        CAM_LOGD("TestCamInfo");
        CAM_LOGD("%d, %d", rCamInfo.eIspProfile, rCamInfo2.eIspProfile);
        CAM_LOGD("%d, %d", rCamInfo.eSensorMode, rCamInfo2.eSensorMode);
        CAM_LOGD("%d, %d", rCamInfo.eIdx_Scene, rCamInfo2.eIdx_Scene);
        CAM_LOGD("%d, %d", rCamInfo.u4ISOValue, rCamInfo2.u4ISOValue);
        CAM_LOGD("%d, %d", rCamInfo.eIdx_ISO, rCamInfo2.eIdx_ISO);
        CAM_LOGD("%d, %d", rCamInfo.eIdx_PCA_LUT, rCamInfo2.eIdx_PCA_LUT);
        CAM_LOGD("%d, %d", rCamInfo.eIdx_CCM, rCamInfo2.eIdx_CCM);
        CAM_LOGD("%d, %d", rCamInfo.eIdx_Shading_CCT, rCamInfo2.eIdx_Shading_CCT);
        CAM_LOGD("%d, %d", rCamInfo.i4ZoomRatio_x100, rCamInfo2.i4ZoomRatio_x100);
        CAM_LOGD("%d, %d", rCamInfo.i4LightValue_x10, rCamInfo2.i4LightValue_x10);
    }
}
