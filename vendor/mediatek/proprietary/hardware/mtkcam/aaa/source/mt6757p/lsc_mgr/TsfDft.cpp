...
init(ESensorMode_T eFullSensorMode)
{
    ...
    m_pTsfIn = new ILscBuf(m_u4SensorDev, id, MTRUE, strName, ILscBuf::E_LSC_IMEM);
    ...
    m_pTsfOut = new ILscBuf(m_u4SensorDev, id, MTRUE, strName, ILscBuf::E_LSC_IMEM);
...
tsfReformTbl()
{
        ...
        ((rTrfmA.u4Y + rTrfmA.u4H) >= (rTrfmB.u4Y + rTrfmB.u4H)))
    {
        ...
        for (i = 0; i < RING_TBL_NUM; i++)
        {
            ...
            MBOOL fgSuccess = rTbl.cropOut(rTrfmRel, m_rLscRingTbl[i]);
            if (!fgSuccess)
            {
                LSC_ERR("Fail to Crop out in tsfReformTbl");
            }
        }
        return MTRUE;
    }
...
tsfBatchCap(const TSF_INPUT_STAT_T& rInputStat, const ILscTbl& rInputTbl, ILscTbl& rOutputTbl)
{
    ...
    else
    {
        ...
        if (m_bDump)
        {
            ...
            rInputTbl.toBuf(*m_pTsfIn);
            // SDBLK dump
            if (m_pTsfIn)
            {
                if (!m_pTsfIn->dump(strFile))
                {
...
doThreadFunc()
{
    ...
    while (m_fgThreadLoop)
    {
        ...
        if (E_LSC_TSF_TYPE_0 == getType())
        {
            ...
            ILscTable::RsvdData rsv(rStat.u4Ratio);
            rOutputTbl.setRsvdData(rsv);
