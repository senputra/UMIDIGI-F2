...
private:
    ...
    MBOOL                       notifyResult4CCU(MUINT32 i4PreFrmId, Result_T *rPreResult, MUINT8 u1IsGetExif, MBOOL bIsCapEnd);
...
postCmd(void* pArg)
{
    ...
    CAM_LOGD_IF(m_fgLogEn, "[%s] m_rCmdQ size(%d), PreMagicReq(%d), rPreResult Addr(%p)",
        __FUNCTION__, m_rCmdQ.size(), rCmd.u4MagicNumReq, rCmd.rPreResult);
...
onThreadLoop(MVOID* pArg)
{
    ...
    while (_this->getCmd(rCmd))
    {
        if(rCmd.u4MagicNumReq != 0)
        {
            ...
            //3. upadte result
            _this->notifyResult4CCU(rCmd.u4MagicNumReq, rCmd.rPreResult, rCmd.u1IsGetExif, rCmd.bIsCapEnd);
            //4. update driver
...
MBOOL
ThreadSetCCUParaImp::
notifyResult4CCU(MUINT32 i4PreFrmId, Result_T *rPreResult, MUINT8 u1IsGetExif, MBOOL bIsCapEnd)
{
    CAM_LOGD_IF(m_fgLogEn, "[%s] Previous FrmId(%d) rPreResult Addr(%p) +", __FUNCTION__, i4PreFrmId, rPreResult);
    // set AE info to previous result Q
    AE_SENSOR_PARAM_T rAESensorInfo;
    IAeMgr::getInstance().getSensorParams(m_u4SensorDev, rAESensorInfo);
    ...
    rPreResult->i4SensorSensitivity = rAESensorInfo.u4Sensitivity;//ISO value updated
    ...
    // set data to exif
    CAM_LOGD_IF(m_fgLogEn, "[%s] u4Rto(%d), i8SensorExposureTime(%lld), i8SensorFrameDuration(%lld), i4SensorSensitivity(%d)", __FUNCTION__,
        u4Rto,rPreResult->i8SensorExposureTime,rPreResult->i8SensorFrameDuration,rPreResult->i4SensorSensitivity);
    ...
    CAM_LOGD_IF(m_fgLogEn, "[%s] vecExifInfo.size(%d), vecDbg3AInfo.size(%d), vecDbgIspInfo.size(%d), m_rParam.u1IsGetExif(%d), bIsCapEnd(%d)", __FUNCTION__,
        rPreResult->vecExifInfo.size(), rPreResult->vecDbg3AInfo.size(), rPreResult->vecDbgIspInfo.size(), u1IsGetExif, bIsCapEnd);

    if (!IspTuningMgr::getInstance().getCamInfo(m_u4SensorDev, rPreResult->rCamInfo))
    {
        CAM_LOGE("Fail to get CamInfo");
    }

    if (u1IsGetExif || bIsCapEnd)
    {
        if( rPreResult->vecExifInfo.size() > 0 && rPreResult->vecDbg3AInfo.size() > 0 && rPreResult->vecDbgIspInfo.size() > 0)
        {
            // standard exif
            m_pHal3A->send3ACtrl(E3ACtrl_GetExifInfo, (MINTPTR)(&(rPreResult->vecExifInfo.editTop())), NULL);
    ...
    CAM_LOGD_IF(m_fgLogEn, "[%s] Previous FrmId(%d) rPreResult Addr(%p) rPreResult i4FrmId(%d) -", __FUNCTION__, i4PreFrmId, rPreResult, rPreResult->i4FrmId);
    return MTRUE;
}
