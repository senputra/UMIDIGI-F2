...
//#endif
    ...
    MINT32                  convertResultToMeta(const Result_T& rResult, MetaSet_T& rMetaResult) const;
    MINT32                  convertP2ResultToMeta(const ResultP2_T& rResultP2, MetaSet_T* pResult) const;

    MINT32                  updateResult(MINT32 i4FrmId, const MetaSet_T& rMetaResult);
    MetaSet_T&              getValidBuf(MINT32& i4UpdatePreMetaRQIdx);
...
//LCS output
    ...
    MUINT32                 m_u4LCSOCapacity;
    Mutex                   m_LCSO_Lock_Out;

private:
    virtual MVOID       clearLCSOList_Out();
    virtual MINT32      updateLCSList_Out(const ISP_LCS_OUT_INFO_T& rLCSinfo);
    virtual MINT32      getLCSList_info_Out(MINT32 i4FrmId, ISP_LCS_OUT_INFO_T& rLCSinfo);
    virtual MINT32      getLCSListLast_Out(ISP_LCS_OUT_INFO_T& rLCSinfo);
    virtual ISP_LCS_OUT_INFO_T*   getLCSList_info_Out(MINT32 i4FrmId);  /*be careful to get refernce! must be protected by mutex*/
...
protected:
    ...
    DefaultKeyedVector<std::string, MUINT8> m_Users;
    ...
    // for repeat tuning
    void*                   mpTuning;
    // for previous metaResultQueue index
    MINT32                  mi4PreMetaResultQueueIdx;
};
