...
set2ShutterControl(MINT32 i4SensorDev, MUINT32 a_u4LEExpTime, MUINT32 a_u4SEExpTime)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB_2, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER, (MUINTPTR)&a_u4LEExpTime, (MUINTPTR)&a_u4SEExpTime, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
setSensorAWBGain(MINT32 i4SensorDev, strSensorAWBGain *strSensorAWBInfo)
{
    ...
    if(ret) {
        CAM_LOGE("Err CMD_SENSOR_SET_EXP_TIME, Sensor dev:%d GR:%d %d R:%d %d B:%d %d GB:%d %d\n", i4SensorDev, strSensorAWBInfo->u4GR,
        strSensorAWBInfo->u4R, strSensorAWBInfo->u4B,
        strSensorAWBInfo->u4GB);
    }

    CAM_LOGD("[setSensorAWBGain1] Id:%d GR:%d %d R:%d %d B:%d %d GB:%d %d\n", i4SensorDev, strSensorAWBInfo->u4GR,
    strSensorAWBInfo->u4R, strSensorAWBInfo->u4B,
    strSensorAWBInfo->u4GB);
...
updateStereoFrameSyncFps(MINT32 i4SensorDev, MINT32 i4SensorDev2, MUINT32 i4ExpTimeMain,MUINT32 i4ExpTimeMain2)
{
    ...
    if(i4ExpTimeMain == 0 && i4ExpTimeMain2 == 0)
    {
        m_i4FrameSyncMainFps = 0;
        m_i4FrameSyncMain2Fps = 0;
        m_i4FrameSyncSubFps = 0;
        m_i4FrameSyncSub2Fps = 0;
        CAM_LOGD("[updateStereoFrameSyncFps] not HW sync frame, skip frame sync setting\n");
        return (ret);
    }
