...
MBOOL AwbMgr::doCapAWB(MINT32 i4SceneLV, MVOID *aaoBuf, MBOOL bApplyToHW)
{
    ...
    if(m_bEMDumpCapAAO)
    {
        ...
        fCapAAO = fopen(m_sEMDumpCapAAOFileName, "w");
        if (NULL != fCapAAO)
        {
            fwrite(aaoBuf, 2, m_rAWBWindowConfig.i4NumX[m_eSensorMode]*m_rAWBWindowConfig.i4NumY[m_eSensorMode]*4, fCapAAO);
            fclose(fCapAAO);
        }
        else
            AWB_LOG("[%s()] i4SensorDev:%d line:%d Open AAO file error", __FUNCTION__, m_eSensorDev, __LINE__);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AwbMgr::setCamScenarioMode(MUINT32 u4CamScenarioMode)
{
    CAM_SCENARIO_T eCamScenarioMode = static_cast<CAM_SCENARIO_T>(u4CamScenarioMode);

    if( m_eCamScenarioMode != eCamScenarioMode) {
        AWB_LOG( "[%s()] m_eSensorDev: %d, m_eCamScenarioMode: %d -> %d, m_i4AWBNvramIdx: %d -> %d\n", __FUNCTION__, m_eSensorDev, m_eCamScenarioMode, eCamScenarioMode, m_i4AWBNvramIdx, AWBNVRAMMapping[eCamScenarioMode]);

        m_eCamScenarioMode = eCamScenarioMode;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// Eng mode dump capture AAO
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AwbMgr::setEMDumpCapAAO(MBOOL fgSave, const char* filename)
{
    m_bEMDumpCapAAO = fgSave;
    sprintf(m_sEMDumpCapAAOFileName, "%s_awbaao.bin", filename);
