...
ESensorDev_T LcsDrvImp::ConvertSensorDevType(const MUINT32 &aSensorDev)
{
    switch(aSensorDev)
    {
        ...
        case SENSOR_DEV_SUB_2: return ESensorDev_SubSecond;
...
clearList_In()
{
    ...
    CAM_LOGD("[%s] Size(%d)", __FUNCTION__, m_rLCS_IN_info_Queue.size());
...
//width mapping
    if(apLcsInInfo->sTGOut.w < 256){
        lcs_cfg.bLCS_EN = MFALSE;
        lcs_cfg.bLCS_Bypass = MTRUE;
    }
    else if(apLcsInInfo->sTGOut.w < 512){
        lcsOutWidth = 32;
    }
    else if(apLcsInInfo->sTGOut.w < 960){
        lcsOutWidth = 64;
    }
    else if(apLcsInInfo->sTGOut.w < 1280){
        lcsOutWidth = 96;
    }
    else if(apLcsInInfo->sTGOut.w < 1920){
        lcsOutWidth = 128;
    }
    else if(apLcsInInfo->sTGOut.w < 2560){
        lcsOutWidth = 192;
    }
    else if(apLcsInInfo->sTGOut.w < 3584){
        lcsOutWidth = 256;
    }
    else if(apLcsInInfo->sTGOut.w < 5120){
        lcsOutWidth = 320;
    }
    else{
        lcsOutWidth = 384;
...
//height mapping
    if(lcs_cfg.bLCS_EN){
        if(apLcsInInfo->sTGOut.h < 256){
            lcs_cfg.bLCS_EN = MFALSE;
            lcs_cfg.bLCS_Bypass = MTRUE;
        }
        else if(apLcsInInfo->sTGOut.h < 512){
            lcsOutHeight = 32;
        }
        else if(apLcsInInfo->sTGOut.h < 960){
            lcsOutHeight = 64;
        }
        else if(apLcsInInfo->sTGOut.h < 1280){
            lcsOutHeight = 96;
        }
        else if(apLcsInInfo->sTGOut.h < 1920){
            lcsOutHeight = 128;
        }
        else if(apLcsInInfo->sTGOut.h < 2560){
            lcsOutHeight = 192;
        }
        else if(apLcsInInfo->sTGOut.h < 3584){
            lcsOutHeight = 256;
        }
        else if(apLcsInInfo->sTGOut.h < 5120){
            lcsOutHeight = 320;
        }
        else{
            lcsOutHeight = 384;
