...
IMP_CCT_CTRL( ACDK_CCT_OP_QUERY_ISP_ID )
{
    ...
    *reinterpret_cast<MUINT32 *>(puParaOut) = sizeof(MUINTPTR) == 4 ? 0x6757AA00 : 0x6757AAFF; // 00 means 32bit; FF means 64bit
...
#define CHECK_INDEX_RANGE(_category)\
    ...
    switch(eCategory) {
        ...
        case EIsp_Category_HLR:
            CHECK_INDEX_RANGE(HLR);
            ISP_MGR_HLR_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_HLR_T*)(bufAccess.pBuffer))[u4Index]);
            ISP_MGR_HLR2_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_HLR_T*)(bufAccess.pBuffer))[u4Index]);
            //ISP_MGR_SL2_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
            //NSIspTuningv3::ISP_MGR_CFA_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(NSIspTuningv3::EIspProfile_NormalCapture);
            break;
#endif
...
//            ISP_MGR_SL2J_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_SL2_T*)(bufAccess.pBuffer))[u4Index]);
        ...
        case EIsp_Category_ABF:
            CHECK_INDEX_RANGE(ABF);
            ISP_MGR_NBC2_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_ABF_T*)(bufAccess.pBuffer))[u4Index]);
            //ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
            //NSIspTuningv3::ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(NSIspTuningv3::EIspProfile_NormalCapture);
            break;
...
*/
        ...
        {
            ggm.lut_rb.lut[i].GGM_R = m_rISPToneMap.IHDR_GGM[setIndex].lut_rb.lut[i].GGM_R = pGammaAccess->gamma.r_tbl[i];
            ggm.lut_g.lut[i].GGM_G = m_rISPToneMap.IHDR_GGM[setIndex].lut_g.lut[i].GGM_G = pGammaAccess->gamma.g_tbl[i];
            ggm.lut_rb.lut[i].GGM_B = m_rISPToneMap.IHDR_GGM[setIndex].lut_rb.lut[i].GGM_B = pGammaAccess->gamma.b_tbl[i];
        }
        else
        {
            ggm.lut_rb.lut[i].GGM_R = m_rISPToneMap.GGM[setIndex].lut_rb.lut[i].GGM_R = pGammaAccess->gamma.r_tbl[i];
            ggm.lut_g.lut[i].GGM_G = m_rISPToneMap.GGM[setIndex].lut_g.lut[i].GGM_G = pGammaAccess->gamma.g_tbl[i];
            ggm.lut_rb.lut[i].GGM_B = m_rISPToneMap.GGM[setIndex].lut_rb.lut[i].GGM_B = pGammaAccess->gamma.b_tbl[i];
        }
#endif
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_QUERY_ISP_INDEX_SET )
{
    ...
    *pu4RealParaOutLen = sizeof(o_type);
    ...
    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_QUERY_ISP_INDEX_SET ) done, SL2F(%d) DBS(%d) OBC(%d) BPC(%d) NR1(%d) PDC(%d) RMM(%d) HLR(%d) RNR(%d) SL2(%d) UDM(%d) ANR(%d) ANR2(%d) CCR(%d) ABF(%d) BOK(%d) HFG(%d) EE(%d) NR3D(%d) MFB(%d) MIX3(%d)\n",
    pCustNvramRegIdx->SL2F,pCustNvramRegIdx->DBS, pCustNvramRegIdx->OBC, pCustNvramRegIdx->BNR_BPC, pCustNvramRegIdx->BNR_NR1,pCustNvramRegIdx->BNR_PDC,
    pCustNvramRegIdx->RMM, pCustNvramRegIdx->HLR, pCustNvramRegIdx->RNR, pCustNvramRegIdx->SL2, pCustNvramRegIdx->UDM, pCustNvramRegIdx->ANR, pCustNvramRegIdx->ANR2, pCustNvramRegIdx->CCR,
    pCustNvramRegIdx->ABF, pCustNvramRegIdx->BOK, pCustNvramRegIdx->HFG, pCustNvramRegIdx->EE, pCustNvramRegIdx->NR3D, pCustNvramRegIdx->MFB, pCustNvramRegIdx->MIXER3);
