...
static MBOOL dumpHwBuf(char* fileName, char* folderName, BufInfo& rLastBuf, MBOOL isHwBuf = 0)
{
    ...
    if (NULL == fp)
    {
        ...
        MINT32 err = mkdir(folderName, S_IRWXU | S_IRWXG);
        CAM_LOGD("err = %d", err);
        return MFALSE;
    } else
    {
...
dequeueHwBuf()
{
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        CAM_LOGD_IF(m_bDebugEnable, "port(%d), index(%d), va[%p]/pa[0x%08x]/#(%d), Size(%d), timeStamp(%" PRId64 ")",
                  m_rPort.index, index, rLastBuf.mVa, rLastBuf.mPa, rLastBuf.mMetaData.mMagicNum_hal, rLastBuf.mSize, rLastBuf.mMetaData.mTimeStamp);
        ...
        if (m_bAAODumpEn) {
            ...
            char fileName[64];
            char folderName[64];
            strncpy(folderName, "/data/vendor/aao", 17);
        ...
        if(m_bAAOHWDumpEn){
            char fileName[64];
            char folderName[64];
            strncpy(folderName, "/data/vendor/aaoHwBuf", 22);
