...
public:
 ...
 //   MBOOL getInitInputParam(SYNC_CTC_INIT_PARAM &CCTInitParam, NORMAL_GAIN_INIT_INPUT_PARAM &NoramlGainInitParam);
    ...
    MRESULT setCamScenarioMode(MUINT32 u4CamScenarioMode);
    ...
    MRESULT setEMDumpCapAAO(MBOOL fgSave, const char* filename);
    MBOOL setMWBColorTemperature(int colorTemperature); // for mwb
...
*/
    ...
    // Face detection
    MBOOL m_bFDenable;
    MBOOL m_bFaceAWBAreaChage;
    AWBMeteringArea_T m_eAWBFDArea;
    AWBZOOM_WINDOW_T m_eZoomWinInfo;

    // Eng mode dump capture AAO
    MBOOL m_bEMDumpCapAAO;
    char m_sEMDumpCapAAOFileName[512];
