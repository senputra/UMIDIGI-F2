...
MRESULT AfMgr::getPdInfoForSttCtrl(MINT32 i4SensorIdx, MINT32 i4SensorMode, MUINT32 &u4PDOSizeW, MUINT32 &u4PDOSizeH, FEATURE_PDAF_STATUS &PDAFStatus)
{
    ...
    else if( sensorStaticInfo.PDAF_Support == 1)
    {
        ...
        if( (isTGInfoValid) && (isFrontalBINDisable) && (isFullMode) && (isBPCItblValid))
        {
             ...
             */
            u4PDOSizeW = (_PDO_STRIDE_ALIGN_((u4Pdo_xsz+1)/2 + 1));
...
MBOOL AfMgr::Start()
{
    ...
    m_u4FrameCount      = 0;
    m_bIsDoAF           = MFALSE;
    m_i4ThermalLensPos  = 0;
    m_i4PostureDac      = 0;
    m_i4IsAfterAutoMode = 0;
    ...
    m_sAFInput.i4IsSupportN2Frame = 1;
#endif
...
MBOOL AfMgr::CamPwrOnState()
{
    ...
    memset( &GetCamCalData,    0, sizeof( CAM_CAL_DATA_STRUCT));
...
MINT32 AfMgr::CCTOPAFGetNVRAMParam( MVOID *a_pAFNVRAM, MUINT32 *a_pOutLen)
{
    ...
    memcpy( &m_sNVRam, m_ptrNVRam, sizeof(NVRAM_LENS_PARA_STRUCT));
    memcpy( a_pAFNVRAM, m_ptrNVRam, sizeof(NVRAM_LENS_PARA_STRUCT));
...
NS3A::EAfState_T AfMgr::UpdateStateAutoMode( NS3A::EAfState_T &eInCurSate, AF_EVENT_T &sInEvent)
{
    ...
    case EVENT_CMD_SET_AF_REGION :
        ...
        m_bLatchROI = MTRUE;
        NewState = eInCurSate;
        m_bIsAPROITooLarge = (m_sArea_APCmd.i4H > (m_i4TGSzH * 6 / 10)) ? MTRUE : MFALSE;
...
MINT32 AfMgr::getAFMaxAreaNum()
{
    if (m_i4AFMaxAreaNum == -1)
    {
        ...
        m_i4AFMaxAreaNum = MCUDrv::isSupportLens( m_i4CurrSensorDev, m_i4CurrSensorId);
...
MRESULT AfMgr::doAF( MVOID *ptrInAFData)
{
    ...
    if(IspMgrAFStatHWPreparing(m_u4FrameCount))
    {
         ...
         */
        CAM_LOGD( "#(%5d,%5d) HW-Preparing", m_u4ReqMagicNum, m_u4StaMagicNum);
    }
    else if( m_bPauseAF)
    {
        CAM_LOGD_IF( m_i4DgbLogLv, "#(%5d,%5d) By pass AF", m_u4ReqMagicNum, m_u4StaMagicNum);
    }
    else
    {
        //get current lens position.
        getLensInfo( m_sCurLensInfo);
        //record current thermal lens position.
        m_i4ThermalLensPos = m_sAFOutput.i4AFPos; //m_sAFOutput.i4ThermalLensPos;
        //record current posture DAC.
        m_i4PostureDac = m_sAFOutput.i4AFPos; //m_sAFOutput.i4PostureDac;
...
MVOID AfMgr::UpdatePDParam( MINT32 &i4InROISel)
{
    if( (m_PDBuf_Type!=EPDBuf_NotDef) && (m_i4PDSensorType != SensorType_NO_PDAF))
    {
        ...
        if( m_i4DgbLogLv)
        {
            ptrMsgBuf += sprintf( ptrMsgBuf, "#(%5d,%5d) %s 0x%x ", m_u4ReqMagicNum, m_u4StaMagicNum, __FUNCTION__, m_PDBuf_Type);
        }
...
MVOID AfMgr::setFocusDistance( MFLOAT lens_focusDistance)
{
            ...
            (i4ResultDac <= 1023))
    {
        ...
        if (m_pIAfAlgo)
        {
            m_pIAfAlgo->setMFPos(m_i4MFPos);
            m_pIAfAlgo->trigger();
            m_i8MvLensTS = MoveLensTo( i4ResultDac, AF_MGR_CALLER);
        }
        else
        {
            CAM_LOGD("Null m_pIAfAlgo");
    ...
    else
    {
        if (m_eLIB3A_AFMode == LIB3A_AF_MODE_MF)
        {
            ...
            if (m_pIAfAlgo)
            {
                m_pIAfAlgo->setMFPos(m_i4MFPos);
                m_pIAfAlgo->trigger();
                m_i8MvLensTS = MoveLensTo( m_i4MFPos, AF_MGR_CALLER);
            }
        }
    }
}
...
MBOOL AfMgr::ConfigHWReg( AF_CONFIG_T &sInHWCfg, AREA_T &sOutHWROI, MINT32 &i4OutHWBlkNumX, MINT32 &i4OutHWBlkNumY, MINT32 &i4OutHWBlkSizeX, MINT32 &i4OutHWBlkSizeY)
{
    ...
    if (sInHWCfg.sRoi.i4W == 0 || sInHWCfg.sRoi.i4H == 0)
    {
        CAM_LOGE( "WAR-ROI : [X]%d [Y]%d [W]%d [H]%d ",
                  sInHWCfg.sRoi.i4X,
                  sInHWCfg.sRoi.i4Y,
                  sInHWCfg.sRoi.i4W,
                  sInHWCfg.sRoi.i4H);
...
#endif
    ISP_MGR_AF_STAT_CONFIG_T::getInstance( static_cast<ESensorDev_T>(m_i4CurrSensorDev)).init(m_i4CurrSensorDev, m_i4SensorIdx);
...
MVOID AfMgr::IspMgrAFStatUpdateHw()
{
    ...
    {
        Mutex::Autolock lock( m_CmdQLock);
        if (m_sAFOutput.sAFStatConfig.sRoi.i4W != 0 && m_sAFOutput.sAFStatConfig.sRoi.i4H != 0)
            memcpy( &m_sHWCfg, &m_sAFOutput.sAFStatConfig, sizeof(AF_CONFIG_T));
        sem_post(&m_SemCmdQ);
    }
    else
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL AfMgr::sendAFCtrl(MUINT32 eAFCtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    MBOOL i4Ret = MTRUE;
...
MVOID AfMgr::SetCurFrmNum( MUINT32 u4FrmNum, MUINT32 u4FrmNumCur)
{
    ...
    m_eEvent = EVENT_NONE;
    CAM_LOGD_IF( m_i4DgbLogLv&2, "Req #%5d, Stat #%5d", m_u4ReqMagicNum, m_u4StaMagicNum);
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID AfMgr::setDAFInfo()
{
    ...
    if( m_sDAF_TBL.is_daf_run & E_DAF_RUN_STEREO)
    {
        ...
        m_sDAF_TBL.daf_vec[CurDafTblIdx].thermal_lens_pos = m_i4ThermalLensPos;
        m_sDAF_TBL.daf_vec[CurDafTblIdx].posture_dac      = m_i4PostureDac;
