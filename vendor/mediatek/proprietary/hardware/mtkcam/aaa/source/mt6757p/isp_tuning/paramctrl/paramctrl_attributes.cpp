...
map_CT_value2index(MUINT32 const u4CT) const
{
    ...
    while ( (t_low <= t_high) && (t_high > 1) )
    {
        ...
        if ( (t_u4CT < t_u4mid) && (t_u4CT >= m_rIspParam.ISPColorTbl.ISPColor.COLOR_CT_Env[t_mid-1]) )
        {
    ...
    if (t_high < 2){
        if ( (t_u4CT < m_rIspParam.ISPColorTbl.ISPColor.COLOR_CT_Env[t_high]) && (t_u4CT >= m_rIspParam.ISPColorTbl.ISPColor.COLOR_CT_Env[t_low])){
...
{
      if (u4CT >= m_rIspParam.ISPColorTbl.ISPColor.COLOR_CT_Env[ISP_NVRAM_COLOR_CT_NUM-1]){
...
setAEInfo(AE_INFO_T const &rAEInfo_)
{
    ...
    // refine ISO value with HDR ratio for ISP
    if (m_pIspTuningCustom->is_to_invoke_remap_ISP_ISO_with_HDR_Ratio(m_rIspCamInfo))
    {
        MINT32 preISO = rAEInfo.u4RealISOValue;
        rAEInfo.u4RealISOValue = m_pIspTuningCustom->remap_ISP_ISO_with_HDR_Ratio(rAEInfo.rHdrAEInfo.rHdrEVInfo, rAEInfo.u4RealISOValue, 1, m_rIspParam.ISPRegs.Iso_Env);
        CAM_LOGD_IF(m_bDebugEnable, "[refine ISO as ratio change](old, new, ratio)=(%d, %d, %d)", preISO, rAEInfo.u4RealISOValue, rAEInfo.i4LESE_Ratio);
    }
...
map_ISO_value2index(MUINT32 const u4Iso) const
{
    ...
    while ( (t_low <= t_high) && (t_high > 1) )
    {
        ...
        if ( (t_Iso < t_u4mid) && (t_Iso >= m_rIspParam.ISPRegs.Iso_Env.IDX_Partition[t_mid-1]) )
        {
    ...
    if (t_high < 2){
        if ( (t_Iso < m_rIspParam.ISPRegs.Iso_Env.IDX_Partition[t_high]) && (t_Iso >= m_rIspParam.ISPRegs.Iso_Env.IDX_Partition[t_low])){
...
{
    MUINT32 t_low = 0, t_high = m_rIspParam.ISPRegs.Zoom_Env.u2Length - 1;

    MUINT32 t_zoom = i4ZoomRatio_x100;
    MUINT32 t_ceiling =m_rIspParam.ISPRegs.Zoom_Env.IDX_Partition[t_high];
    CAM_LOGD_IF(m_bDebugEnable,"t_ceiling(%d)", t_ceiling);
    ...
    {
        int t_mid = (t_low + t_high)/2;
        MUINT32 t_u4mid = m_rIspParam.ISPRegs.Zoom_Env.IDX_Partition[t_mid];

        if ( (t_zoom < t_u4mid) && (t_zoom >= m_rIspParam.ISPRegs.Zoom_Env.IDX_Partition[t_mid-1]) )
        {
            return static_cast<EIndex_ZOOM_T>(t_mid);
        }
        else if (t_zoom >= t_u4mid)
        {
            t_low = t_mid + 1 ;
        }
        else if (t_zoom < t_u4mid)
    ...
    if (t_high < 2){
        if ( (t_zoom < m_rIspParam.ISPRegs.Zoom_Env.IDX_Partition[t_high]) && (t_zoom >= m_rIspParam.ISPRegs.Zoom_Env.IDX_Partition[t_low])){
            return static_cast<EIndex_ZOOM_T>(t_high);
        }
        else{
            return static_cast<EIndex_ZOOM_T>(t_low);
        }
    }

    CAM_LOGE("ZOOM_MAPPING_ERROR");
    return eIDX_ZOOM_0;  //default
}
...
{
      if (u4Iso >= m_rIspParam.ISPRegs.Iso_Env.IDX_Partition[m_rIspParam.ISPRegs.Iso_Env.u2Length-1]){
...
{
      if (u4Zoom >= m_rIspParam.ISPRegs.Zoom_Env.IDX_Partition[m_rIspParam.ISPRegs.Zoom_Env.u2Length-1]){
...
{
    CAM_LOGD_IF(m_bDebugEnable, "[%s] idx %d", __FUNCTION__);
...
{
    MY_LOG_IF(m_bDebugEnable, "[%s] FD_ratio = %d", __FUNCTION__, FD_ratio);
...
{
    CAM_LOGD_IF(m_bDebugEnable, "[%s] (ctrl, arg1, arg2) = (%d, %d, %d)", __FUNCTION__, ctrl, arg1, arg2);
    ...
    default:
        CAM_LOGE("%s: ctrl not supported (ctrl, arg1, arg2) = (%d, %d, %d)", __FUNCTION__, ctrl, arg1, arg2);
        break;
    }
}
