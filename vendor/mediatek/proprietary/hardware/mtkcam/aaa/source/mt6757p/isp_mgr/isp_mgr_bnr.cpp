...
apply(RAWIspCamInfo& rRawIspCamInfo, TuningMgr& rTuning, MINT32 i4SubsampleIdex)
{
    ...
    MBOOL bPDC_EN =  m_bBPCIValid &&
                    (isPDCEnable()&( isAF_PDCEnable() | (reinterpret_cast<ISP_CAM_BNR_PDC_CON_T*>(REG_INFO_VALUE_PTR(CAM_BNR_PDC_CON))->PDC_EN)));
...
unlockBPCIBuf()
{
    ...
    std::string strNameP1 = "BPCI_tbl";//same in setPBCIBuf
    if (m_pPDCBuffer != NULL)
    {
        static_cast<IImageBuffer*>(m_pPDCBuffer)->unlockBuf(strNameP1.c_str());
        // destroy buffer
        sp<IImageBuffer> temp = static_cast<IImageBuffer*>(m_pPDCBuffer);
        m_pPDCBuffer = NULL;
    }
