...
MBOOL aaoSeparation(const MVOID* AAOSrc , const AAO_BUF_CONFIG_T& rAAOBufConfig, AAO_BUF_T& rAAOBuf)
{
    ...
    MUINT16* pawb = (MUINT16*)rAAOBuf.m_pAAOSepAWBBuf;
        ...
        */
        ...
        if(rAAOBuf.m_pAAOSepAEBuf!=0)//ae
        {
            if(rAAOBufConfig.m_bEnableHDRYConfig)
            {
                ...
                for(int k=0;k < u4FistBlkNumX;k++)//(120/16=7)
                {
                    //LSB
                ...
                for(int k=0;k<u4SecondBlkNumX;k++)//(8/2=4)
                {
        ...
        if(rAAOBuf.m_pAAOSepOverCntBuf!=0)//overexpcnt
        {
            if(rAAOBufConfig.m_bEnableOverCntConfig)
            {
                ...
                //last data
                for(int k=0;k<u4SecondBlkNumXOver;k+=8)
                {
        ...
        if(rAAOBuf.m_pAAOSepLSCBuf!=0 && rAAOBuf.m_pAAOSepAWBBuf!=0)//lsc, awb
        {
            if (rAAOBufConfig.m_bEnableTSFConfig)    // c4 is available
            {
                ...
                while (x >= 16)
                {
                    ...
                    Vc8Merge0 = vshlq_n_u16(Vc8Merge0,4);              //left shift 4 bit
                    uint16x8_t Vdst0 = vorrq_u16(Vc8Merge0,Vc4Res0);
                    uint16x8_t Vdst0Awb = vshlq_n_u16(Vdst0,4); // Awb shift to MSB
                    ...
                    Vc8Merge1 = vshlq_n_u16(Vc8Merge1,4);              //left shift 4 bit
                    uint16x8_t Vdst1 = vorrq_u16(Vc8Merge1,Vc4Res1);
                    uint16x8_t Vdst1Awb = vshlq_n_u16(Vdst1,4); // Awb shift to MSB

                    //Store a single vector into memory
                    vst1q_u16 (plsc, Vdst0);
                    vst1q_u16 (pawb, Vdst0Awb);
                    plsc+=8;
                    pawb+=8;
                    vst1q_u16 (plsc, Vdst1);
                    vst1q_u16 (pawb, Vdst1Awb);
                    plsc+=8;
                    pawb+=8;
                ...
                while (x)
                {
                    ...
                    *plsc++ = ((c4 & 0x8) ? (((c8-1)<<4)|c4) : ((c8<<4)|c4));
                    *pawb++ = (((c4 & 0x8) ? (((c8-1)<<4)|c4) : ((c8<<4)|c4)) << 4); // Awb shift to MSB
                    x--;
                }
            }
