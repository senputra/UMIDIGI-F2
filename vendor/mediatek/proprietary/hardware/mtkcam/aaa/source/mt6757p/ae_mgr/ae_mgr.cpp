...
AeMgr(ESensorDev_T eSensorDev)
    ...
    , m_u4AAOSepAWBsize(AAO_SEP_AWB_SIZE_COEF*AAO_STT_BLOCK_NUMX*AAO_STT_BLOCK_NUMY)
    , m_u4AAOSepAEsize(AAO_SEP_AE_SIZE_COEF*AAO_STT_BLOCK_NUMX*AAO_STT_BLOCK_NUMY)
    , m_u4AAOSepHistsize(AAO_SEP_HIST_SIZE_COEF*AAO_STT_HIST_BIN_NUM)
    , m_u4AAOSepOverCntsize(AAO_SEP_OC_SIZE_COEF*AAO_STT_BLOCK_NUMX*AAO_STT_BLOCK_NUMY)
...
MRESULT AeMgr::PreviewAEInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    if ((m_pSensorGainThread == NULL) && (i4SutterDelay != i4SensorGainDelay) && (!m_pIAeTest->IsPerframeCtrl()) ){
...
MRESULT AeMgr::cameraPreviewInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    property_get("vendor.debug.ae_mgr_ccu.enable", value, "0");
...
// AEInit
    ...
    if ((m_pICcu != NULL) && (rSensorDynamicInfo.TgInfo != CAM_TG_1)) {
        uninitCCU();
        m_bCCUAEFlag = MFALSE;
    }
    AE_LOG( "[%s()] i4SensorDev:%d , CCU enable:%d , TgInfo:%d\n", __FUNCTION__, m_eSensorDev, m_bCCUAEFlag, rSensorDynamicInfo.TgInfo);
...
// AEInit(rParam);
    ...
    if(!m_bIsCCUStart && m_bCCUAEFlag && IsCCUStart())
    {
...
MRESULT AeMgr::setFDInfo(MVOID* a_sFaces, MINT32 i4tgwidth, MINT32 i4tgheight)
{
    ...
    if(CUST_ENABLE_FACE_AE() == MTRUE) {
        ...
        if(m_bFDenable == MTRUE){
            ...
            for(ifaceIdx=0; ifaceIdx < eAEFDArea.u4Count; ifaceIdx++) {
                ...
                if(pFaces->fa_cv[ifaceIdx] > 0)
                {
                    ...
                    eAEFDArea.rAreas[ifaceIdx].i4Landmark[0][1] = -1000 + eAEFDArea.rAreas[ifaceIdx].i4Landmark[0][1]*2000/(i4tgwidth);
                    eAEFDArea.rAreas[ifaceIdx].i4Landmark[0][2] = -1000 + eAEFDArea.rAreas[ifaceIdx].i4Landmark[0][2]*2000/(i4tgheight);
                    ...
                    eAEFDArea.rAreas[ifaceIdx].i4Landmark[1][1] = -1000 + eAEFDArea.rAreas[ifaceIdx].i4Landmark[1][1]*2000/(i4tgwidth);
                    eAEFDArea.rAreas[ifaceIdx].i4Landmark[1][2] = -1000 + eAEFDArea.rAreas[ifaceIdx].i4Landmark[1][2]*2000/(i4tgheight);
                    ...
                    eAEFDArea.rAreas[ifaceIdx].i4Landmark[2][1] = -1000 + eAEFDArea.rAreas[ifaceIdx].i4Landmark[2][1]*2000/(i4tgwidth);
                    eAEFDArea.rAreas[ifaceIdx].i4Landmark[2][2] = -1000 + eAEFDArea.rAreas[ifaceIdx].i4Landmark[2][2]*2000/(i4tgheight);
                    eAEFDArea.rAreas[ifaceIdx].i4Landmark[2][3] = -1000 + eAEFDArea.rAreas[ifaceIdx].i4Landmark[2][3]*2000/(i4tgheight);
                }

                AE_LOG( "[%s():Modified] AE FD meter area Idx:%d Left:%d Right:%d Top:%d Bottom:%d TGWidth:%d TGHeight:%d i8TimeStamp: %lld\n",__FUNCTION__, ifaceIdx,
                            eAEFDArea.rAreas[ifaceIdx].i4Left, eAEFDArea.rAreas[ifaceIdx].i4Right,
                            eAEFDArea.rAreas[ifaceIdx].i4Top, eAEFDArea.rAreas[ifaceIdx].i4Bottom ,i4tgwidth,i4tgheight,eAEFDArea.i8TimeStamp );
...
MRESULT AeMgr::setAEMinMaxFrameRate(MINT32 i4NewAEMinFps, MINT32 i4NewAEMaxFps)
{
    ...
    if ((m_i4AEMinFps != i4NewMinFPS) || (m_i4AEMaxFps != i4NewMaxFPS)) {
        ...
        if(m_pIAeAlgo != NULL) {
            ...
            m_bAEMonitorStable = MFALSE;
            if(m_bCCUAEFlag && IsCCUWorking() && IsCCUAEWorking())
                 m_bSetCCUFrameRate = MTRUE;
            m_u4AEScheduleCnt = 0;
            m_eAEState = MTK_CONTROL_AE_STATE_SEARCHING;
            m_i4WaitVDNum = (m_i4IspGainDelayFrames+ISP_GAIN_DELAY_OFFSET+1);
...
MRESULT AeMgr::setCamScenarioMode(MUINT32 u4NewCamScenarioMode, MBOOL bInit)
{
    ...
    if( m_eCamScenarioMode != eNewCamScenarioMode || bInit) {
        ...
        m_rAEInitInput.eAEScenario = m_eCamScenarioMode;
        m_u4AENVRAMIdx = AENVRAMMapping[m_eCamScenarioMode];
        if(m_p3ANVRAM != NULL) {
...
MRESULT AeMgr::setAEMode(MUINT32 u4NewAEMode)
{
    ...
    if ((MINT32)eNewAEMode != (MINT32)m_eAEMode){
        if ((m_bAEMonitorStable == MTRUE) && (m_eAEMode == LIB3A_AE_MODE_OFF)){
            m_bAEMonitorStable = MFALSE;
            m_u4AEScheduleCnt = 0;
        }
...
MBOOL bIsHDRflow =((m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)||(m_eAETargetMode == AE_MODE_ZVHDR_TARGET));
    ...
    if(m_eAETargetMode == AE_MODE_MVHDR_TARGET) {
        ...
        if(m_rSensorVCInfo.u2VCModeSelect == 0) {
            ...
            if(m_strIVHDROutputSetting.u4SEExpTimeInUS != 0) {
                ...
                rInput.u2ShutterRatio = m_rSensorVCInfo.u2VCShutterRatio;
                AE_LOG_IF(m_3ALogEnable, "[%s()] The AE algo class is NULL  i4SensorDev = %d u4SEExpTimeInUS:%d\n", __FUNCTION__, m_eSensorDev, m_strIVHDROutputSetting.u4SEExpTimeInUS);
            }
        }
        rInput.u2ShutterRatio = m_rSensorVCInfo.u2VCShutterRatio;
    ...
    if(m_pIAeAlgo != NULL) {
        if(pAEStatisticBuf != NULL) {
            ...
            AE_LOG_IF(m_3ALogEnable, "[%s()] The AAO buffer pointer is NULL  i4SensorDev = %d line:%d u4AAOUpdate:%d i4ActiveAEItem:%d\n", __FUNCTION__, m_eSensorDev, __LINE__, u4AAOUpdate, i4ActiveAEItem);
        }
        // monitor AE state
        ...
        if((m_bPerframeAEFlag == MTRUE) && (bAAASchedule == MFALSE)) {
            if (bIsHDRflow){
                if (m_eAETargetMode == AE_MODE_MVHDR_TARGET){
                    ...
                    AE_LOG_IF(m_3ALogEnable, "[%s()] This is Touch HDR AE flow, bPerframe:%d \n",__FUNCTION__,m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableTouchSmooth);
                } else {
                    bCalculateAE  = MTRUE ;
                    bApplyAE = MTRUE ;
                    AE_LOG_IF(m_3ALogEnable, "[%s()] This is Perframe HDR AE flow \n",__FUNCTION__);
                }
            } else {
                if(m_bAEOneShotControl == MTRUE) {
                    m_bPerframeAEFlag = MFALSE;
                }
                else if(m_bTouchAEAreaChage == MTRUE){
                    if (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableTouchSmooth){
                    ...
                    AE_LOG_IF(m_3ALogEnable,  "[%s()] This is Touch AE flow, bPerframe:%d \n",__FUNCTION__,m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableTouchSmooth);
                } else if (m_bAElimitor == MTRUE){
                    m_bPerframeAEFlag = MFALSE;
                    AE_LOG_IF(m_3ALogEnable,  "[%s()] This is Limiter AE flow \n",__FUNCTION__);
                } else {
                    bCalculateAE  = MTRUE ;
                    bApplyAE = MTRUE ;
        ...
        AE_LOG_IF(m_3ALogEnable,  "[%s():1] The AE algo class is NULL  i4SensorDev = %d \n", __FUNCTION__, m_eSensorDev);
    }
    ...
    if(m_eAEMode == LIB3A_AE_MODE_OFF) {
        ...
        if ((i4ActiveItem & E_AE_AE_CALC) || (bCalculateAE == MTRUE)) {
            if(m_bAELock || m_bAdbAELock) {
                ...
                AE_LOG_IF(m_3ALogEnable,  "[%s()] AE_STATE_ONE_SHOT", __FUNCTION__);
            } else if(m_bTouchAEAreaChage == MTRUE) {
                ...
                AE_LOG_IF(m_3ALogEnable,  "[%s()] Stereo or VHDR or Slow Motion flow\n", __FUNCTION__);
                } else {
            ...
            if(m_eZoomWinInfo.bZoomChange == MTRUE) {
                if(m_pIAeAlgo != NULL) {
                    ...
                    AE_LOG_IF(m_3ALogEnable,  "[%s()] The AE algo class is NULL  i4SensorDev = %d line:%d", __FUNCTION__, m_eSensorDev, __LINE__);
                }
                ...
                if (m_bPerframeAEFlag){
                    ...
                    AE_LOG_IF(m_3ALogEnable,"[%s():PassToAlgo-TG] u4PreAEidx[N-1]/[N-2]/[N-3]::%d/%d/%d u4PreTgIntAEidx[N-1]/[N-2]/[N-3]::%d/%d/%d FrameTgCnt(%d) HwMagicNum(%d)\n", __FUNCTION__,
                            rAEInput.u4PreAEidx[0],
                            rAEInput.u4PreAEidx[1],
                            rAEInput.u4PreAEidx[2],
                            rAEInput.u4PreTgIntAEidx[0],
                            rAEInput.u4PreTgIntAEidx[1],
                            rAEInput.u4PreTgIntAEidx[2],
                            rAEInput.CycleInfo.m_i4FrameTgCnt,
                            rAEInput.CycleInfo.m_u4HwMagicNum);
                }
                if(m_pIAeAlgo != NULL) {
                    if(m_bRestoreAE == MFALSE) {
                        if ((m_bHDRshot == MFALSE)&&(m_u4HDRcountforrevert<1)){
                            if (m_u4HFSMSkipFrame == 0){
                                ...
                                rAEInput.i8TimeStamp = i8TimeStamp;
            ...
            if(m_pIAeAlgo != NULL) {
                ...
                if(bIsHDRflow) {
                    ...
                    m_bPerframeAEFlag = MFALSE; // temp: turn off perframe AE stereo
                    UpdateSensorISPParams(AE_AUTO_FRAMERATE_STATE);
                    m_bPerframeAEFlag = m_rAEInitInput.rAEPARAM.bPerFrameAESmooth;
                    // Save to Evsetting Queue (for algorithm)
                    updatePreEvSettingQueue(m_rAEOutput.rPreviewMode.u4Eposuretime,m_rAEOutput.rPreviewMode.u4AfeGain,m_rAEOutput.rPreviewMode.u4IspGain);
        ...
        if(m_bEnableAE && (m_eAEMode!= LIB3A_AE_MODE_OFF)) {
            ...
            if(m_pIAeAlgo != NULL) {
                if((m_bRestoreAE == MFALSE)&&(m_bHDRshot == MFALSE)&&(m_u4HDRcountforrevert<1)){
                    ...
                    AE_LOG_IF(bValid, "[%s()] EPIPECmd_AE_SMOOTH bValid(%d) \n", __FUNCTION__, bValid);
                    if(bValid == MTRUE) {
...
MVOID *pAEStatisticBuf;
    ...
    if (IsCCUWorking())
    {
        AE_LOG_IF(m_3ALogEnable, "aeccu_doAFAEAAO\n");
        controlCCU();
        getCCUResult();
...
MBOOL bIsHDRFlow= ((m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)||(m_eAETargetMode == AE_MODE_ZVHDR_TARGET));
    ...
    memset(&rAEOutput, 0, sizeof(strAEOutput));
    memset(&rAEInput , 0, sizeof(strAEInput ));
    AE_LOG( "[doAFAE]:%d %d i8TimeStamp:%lld ActiveAE:%d eAETargetMode:%d bTouchAEAreaChage:%d\n", m_i4TimeOutCnt, m_i4WaitVDNum, i8TimeStamp, i4ActiveAEItem, m_eAETargetMode, m_bTouchAEAreaChage);
    if (IsCCUWorking())
    {
        AE_LOG("aeccu_doAFAE\n");
        controlCCU();
        getCCUResult();
        AE_LOG("aeccu_doAFAE_over\n");
    }
    if (m_u4HwMagicNum != u4AAOUpdate){
        m_u4HwMagicNum = u4AAOUpdate;
        bAAOUpdate = MTRUE;
    ...
    if(m_bEnableAE && (m_bAdbAELock == MFALSE)) {
        if((m_pIAeAlgo != NULL)&& (pAEStatBuf != NULL)) {
            ...
            AE_LOG_IF(m_3ALogEnable,  "[%s()] The AE algo class is NULL  i4SensorDev = %d line:%d u4AAOUpdate:%d i4ActiveAEItem:%d\n", __FUNCTION__, m_eSensorDev, __LINE__, u4AAOUpdate, i4ActiveAEItem);
        }
        ...
        if (m_eAEMode == LIB3A_AE_MODE_OFF){
            UpdateSensorISPParams(AE_SENSOR_PER_FRAME_STATE);
        }else if ((i4ActiveItem & E_AE_AE_CALC)  || (bCalculateAE == MTRUE)) {
            if (bIsHDRFlow){
                m_bAEStable = MTRUE;
                m_bAEMonitorStable = m_bAEStable;
            } else {
                if (m_bPerframeAEFlag){
                    ...
                    AE_LOG_IF(m_3ALogEnable, "[%s():PassToAlgo] rAEInput.PreEvSetting[N-1]:%d/%d/%d rAEInput.PreEvSetting[N-2]:%d/%d/%d rAEInput.PreEvSetting[N-3]:%d/%d/%d rAEInput.CycleInfo.m_i4FrameCnt:%d\n", __FUNCTION__,
                        rAEInput.PreEvSetting[0].u4Eposuretime,
                        rAEInput.PreEvSetting[0].u4AfeGain    ,
                        rAEInput.PreEvSetting[0].u4IspGain    ,
                        rAEInput.PreEvSetting[1].u4Eposuretime,
                ...
                else if (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableTouchSmooth && m_bPerframeAEFlag && (!bAAASchedule) &&(!m_bAEReadyCapture)){
                    rAEInput.eAeState = AE_STATE_TOUCH_PERFRAME;
...
MRESULT AeMgr::doCapAE()
{
    ...
    if(((m_eCamMode == eAppMode_EngMode)||(m_bzcHDRshot == MTRUE)) && ((m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)|| (m_eAETargetMode == AE_MODE_ZVHDR_TARGET))) {   // engineer mode and target mode
        ...
        if ((m_bAEReadyCapture == MFALSE)&&(m_bAECaptureUpdate == MTRUE)&&(m_bStrobeOn == MFALSE)&&(m_bHDRshot==MFALSE))
            updatePreEvSettingQueue(mCaptureMode.u4Eposuretime,mCaptureMode.u4AfeGain,mCaptureMode.u4IspGain);
    }
...
MRESULT AeMgr::doBackAEInfo(MBOOL bTorchMode)
{
 ...
 memset(&rAEInput, 0, sizeof(strAEInput));
    ...
    if(m_bEnableAE) {
      ...
      if(bTorchMode){
        //mPreviewModeBackupTorch = m_rAEOutput.rPreviewMode; // never enter this condition in mt6757på
      }else{
...
void AeMgr::setIsp(int isp)
{
    ...
    MUINT32 u4PreviewBaseISO=m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MiniISOGain;
    ...
    rAEInfo2ISP.u4RealISOValue = u4PreviewBaseISO*((rAEInfo2ISP.u4AfeGain*isp)>>10)/1024;
...
AE_EXP_GAIN_MODIFY_T rSensorInputData, rSensorOutputData;
    ...
    if(((m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)|| (m_eAETargetMode == AE_MODE_ZVHDR_TARGET)) && ( m_eCamMode == eAppMode_EngMode || m_bAEReadyCapture == MFALSE)){
        if(m_eAETargetMode == AE_MODE_ZVHDR_TARGET) {
            ...
            if( (m_u4VHDRratio == 0) || (m_u4VHDRratio == 9999)){ //Auto ratio
            ...
            //m_i4HdrOnOff = sAEInfo->HdrEvSetting.i4HdrOnOff;
...
MRESULT AeMgr::getNvramData(MINT32 i4SensorDev, MBOOL isForce)
{
    ...
    AE_LOG("[%s()] m_i4SensorDev:%d i4OBCtableidx: %d [0][1][2][3](offset/gain) = [%d/%d][%d/%d][%d/%d][%d/%d] \n",
            ...
            m_rISPNvramOBC_Table[0].offst0, m_rISPNvramOBC_Table[0].gain0,
            m_rISPNvramOBC_Table[1].offst0, m_rISPNvramOBC_Table[1].gain0,
            m_rISPNvramOBC_Table[2].offst0, m_rISPNvramOBC_Table[2].gain0,
            m_rISPNvramOBC_Table[3].offst0, m_rISPNvramOBC_Table[3].gain0);
    return S_AE_OK;
}
...
MRESULT AeMgr::getExposureInfo(ExpSettingParam_T &strHDRInputSetting)
{
    ...
    if((u4Newtableindex< strCurrentCaptureTable.u4TotalIndex) && (u4Newtableindex>=0)) {
...
if(m_bPerframeAEFlag && (!m_bEnSWBuffMode)){
            ...
            if(u2FrameRate > m_i4AEMaxFps) {
                AE_LOG_IF(m_3ALogEnable,"[UpdateSensorISPParams] Frame rate too large:%d (max: %d)\n", u2FrameRate, m_i4AEMaxFps);
                u2FrameRate = m_i4AEMaxFps;
            }
        ...
        case AE_SENSOR_PER_FRAME_STATE:
             ...
             if(m_SensorQueueCtrl.uInputIndex != m_SensorQueueCtrl.uOutputIndex) {
                 ...
                 AE_LOG_IF(m_3ALogEnable,"[%s()] u4FrameDuration = %6.3f => m_u4UpdateFrameRate_x10 = %6.3f\n",
                    __FUNCTION__, m_SensorQueueCtrl.rSensorParamQueue[m_SensorQueueCtrl.uOutputIndex].u4FrameDuration, m_u4UpdateFrameRate_x10);

                 if(m_u4UpdateFrameRate_x10 > m_i4AEMaxFps) {
                     AE_LOG( "[AE_SENSOR_PER_FRAME_STATE] Frame rate too large:%d (max: %d)\n", m_u4UpdateFrameRate_x10, m_i4AEMaxFps);
                     m_u4UpdateFrameRate_x10 = m_i4AEMaxFps;
                 }
                 ...
                 updatePreEvSettingQueue(m_u4UpdateShutterValue, m_u4UpdateGainValue, u4ISPGain);
...
MRESULT AeMgr::updatePreviewParamsByiVHDR(strAEOutput *sAEInfo)
{
    ...
    if(m_p3ANVRAM != NULL) {
        strIVHDRInputSetting.u41xGainISO = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MiniISOGain;
        strIVHDRInputSetting.u4SaturationGain = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MinGain;
    } else {
...
{
    if(m_u4VHDRratio >= 1000){ // Ratio range: 1000~9998: shutter only HDR
        m_strIVHDROutputSetting.u4SEExpTimeInUS = (sAEInfo->HdrEvSetting.rHdrEVInfo.i4LEExpo*1000 + (m_u4VHDRratio >> 1))/m_u4VHDRratio;
        m_strIVHDROutputSetting.u4SESensorGain = sAEInfo->HdrEvSetting.rHdrEVInfo.i4LEAfeGain;
        m_strIVHDROutputSetting.u4SEISPGain = sAEInfo->HdrEvSetting.rHdrEVInfo.i4LEIspGain;
        m_strIVHDROutputSetting.u4LE_SERatio_x100 = (m_u4VHDRratio+5)/10;
    }
    else {
        if(m_pIAeAlgo != NULL) {// Ratio range: 100~999: set short exposure by setting shutter & gain
            ...
            rEvSetting.uFlag =0;
            if( (m_u4VHDRratio < 100) ||  (m_u4VHDRratio > 1000) ){
                AE_LOG( "[%s()] HDR ratio is out of range = %d ", __FUNCTION__, m_u4VHDRratio);
                return E_AE_PARAMETER_ERROR;
            }
            m_pIAeAlgo->setHdrRatio(m_u4VHDRratio);
            m_pIAeAlgo->calRatio2AEInfoIf(rEvSetting);
            m_pIAeAlgo->updateHdrInfo(HdrEvSetting);
            m_strIVHDROutputSetting.u4SEExpTimeInUS = HdrEvSetting.rHdrEVInfo.i4SEExpo;
            m_strIVHDROutputSetting.u4SESensorGain = HdrEvSetting.rHdrEVInfo.i4SEAfeGain;
            m_strIVHDROutputSetting.u4SEISPGain = HdrEvSetting.rHdrEVInfo.i4SEIspGain;
            m_strIVHDROutputSetting.u4LE_SERatio_x100 = HdrEvSetting.rHdrEVInfo.i4HdrRatio;
        } else {
            AE_LOG( "[%s()] The AE algo class is NULL  i4SensorDev = %d line:%d", __FUNCTION__, m_eSensorDev, __LINE__);
        }
    }
    AE_LOG( "[%s()] Engineer Mode HDR AE(In Ratio:%d) = LE(%d, %d, %d), SE(%d, %d, %d), R(%d) ", __FUNCTION__,
        m_u4VHDRratio,
...
{
    MINT32 err = S_AE_OK;
    if (IsCCUWorking() && IsCCUAEWorking() && m_bCCUAEFlag){
        m_bSetFrameRateValue = m_bSetShutterValue = m_bSetGainValue = MFALSE;
    ...
    if((m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)|| (m_eAETargetMode == AE_MODE_ZVHDR_TARGET)) {   // mVHDR/iVHDR sensor control
        ...
        if(m_bSetGainValue) {    // update sensor gain value
            if (!m_pSensorGainThread){  // 0/0/2 sensor
                if(m_eAETargetMode == AE_MODE_ZVHDR_TARGET) {
                    err = AAASensorMgr::getInstance().setSensorDualGain((ESensorDev_T)m_eSensorDev, m_u4UpdateGainValue, m_u4UpdateGainValue2);
                }
                else {
                    err = AAASensorMgr::getInstance().setSensorGain((ESensorDev_T)m_eSensorDev, m_u4UpdateGainValue);
                }
            } else {                    // 0/1/2 sensor
                ThreadSensorGain::Cmd_T rCmd(0, 0, m_u4UpdateGainValue,m_u4UpdateGainValue2, m_eSensorTG);
                m_pSensorGainThread->postCmd(&rCmd);
            }
            m_bSetGainValue = MFALSE;
            if (FAILED(err)) {
...
MFLOAT u2FrameRate = 300;
    ...
    rAEInfo2ISP.bAELimiter = m_bAElimitor;
    rAEInfo2ISP.bDisableOBC = IsCCUWorking();
    rAEInfo2ISP.u4EISExpRatio = m_u4EISExpRatio;
    ...
    AE_LOG_IF(m_3ALogEnable,"[updateAEInfo2ISP] Frame rate = %6.3f\n", u2FrameRate);
    if(u2FrameRate > m_i4AEMaxFps) {
        AE_LOG_IF(m_3ALogEnable,"[updateAEInfo2ISP] Frame rate too large:%6.3f (max: %d)\n", u2FrameRate, m_i4AEMaxFps);
        u2FrameRate = m_i4AEMaxFps;
    }
    ...
    if (IsCCUWorking() && (u4ISPGain != 0x00)){
    ...
    AE_LOG_IF((u4ISPGain != 0x00), "[updateAEInfo2ISP] State:%d VDNum:%d Shutter:%d Gain:%d %d %d Flare offset:%d %d FrameDuration:%lld DisableOBC:%d ExpRatio:%d eAEstate:%d CurrentidxF:%d\n", eNewAEState, m_i4WaitVDNum,
        ...
        rAEInfo2ISP.i2FlareOffset, i2FlareGain, m_rSensorCurrentInfo.u8FrameDuration, rAEInfo2ISP.bDisableOBC, rAEInfo2ISP.u4EISExpRatio, m_eAEState, rAEInfo2ISP.u4AEidxCurrentF);
    return S_AE_OK;
}
...
{
    AE_LOG( "[%s()] i4SensorDev:%d FlickerFRActive %d->%d\n", __FUNCTION__, m_eSensorDev, m_bEnableFlickerFPSstate,a_bFlickerFPSAvtive);
    m_bEnableFlickerFPSstate = a_bFlickerFPSAvtive;
    if (m_bCCUAEFlag && IsCCUWorking() && IsCCUAEWorking()){
        m_bControlCCUInfochange = MTRUE;
    }else{
...
if ((m_bPerframeAEFlag == MFALSE) || (m_i4TimeOutCnt>0)){
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::getStereoDenoiseInfo(AE_MODE_CFG_T& strStereoDenoiseInfo)
{
     strStereoDenoiseInfo = m_rAEOutput.rPreviewMode;
    ...
    if (m_bStereoManualPline && m_u4StereoDenoiseMode){
        ...
        strStereoDenoiseInfo.u4IspGain = m_rAEOutput.rPreviewMode.u4IspGain*i4StereoDenoiserto/1000;
        strStereoDenoiseInfo.u4RealISO = m_rAEOutput.rPreviewMode.u4RealISO*i4StereoDenoiserto/1000;
        AE_LOG("[getStereoDenoiseInfo][More OB2] Denoise mode :%d Dev:%d, Stereo ZSD capture - Ratio = %d OB2 = %d->%d ISO=%d->%d\n", m_u4StereoDenoiseMode, m_eSensorDev, i4StereoDenoiserto,
            m_rAEOutput.rPreviewMode.u4IspGain, strStereoDenoiseInfo.u4IspGain, m_rAEOutput.rPreviewMode.u4RealISO, strStereoDenoiseInfo.u4RealISO);
    }
    return S_AE_OK;
}
