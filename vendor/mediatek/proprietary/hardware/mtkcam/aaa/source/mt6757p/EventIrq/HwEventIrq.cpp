...
init(const IEventIrq::ConfigParam& rCfg, const char* strUser)
{
    ...
    m_pSttPipe = (INormalPipe*)INormalPipeUtils::get()->createDefaultNormalPipe(rCfg.i4SensorIndex, strUser);
    ...
    CAM_LOGD("[%s](%s) this(%p) m_pSttPipe(%p), userKey(%d), cfg(%d, %d, %d, %d)", __FUNCTION__, strUser, this, m_pSttPipe, m_i4UserKey,
...
flush()
{
    ...
    while(m_bWaiting && count < 10)
    {
        ...
        usleep(50); // avoid wait m_pSttPipe done, but m_bWaiting still is true
        if(m_bWaiting)
            usleep(1000);
        count++;
    }

    IBufMgr* pBufMgr = Hal3ASttCtrl::getInstance(m_i4SensorDev)->getBufMgr(BUF_AFO);
    if(pBufMgr)
        pBufMgr->abortDequeue();
    else
...
waitDequeue(BUF_PORTS port)
{
    ...
    Hal3ASttCtrl::getInstance(m_i4SensorDev)->getBufMgr(port)->waitDequeue();
