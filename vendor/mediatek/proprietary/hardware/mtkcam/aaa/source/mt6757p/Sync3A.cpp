...
#include <debug_exif/ver2/dbg_id_param.h>        // for DEBUG_XXX_KEY_ID
...
Sync3AWrapper()
    ...
    , m_pMTKSyncAwb(NULL)
{
    ::memset(&m_rAwbSyncInput, 0, sizeof(SYNC_AWB_INPUT_INFO_STRUCT));  //  initialize, Set all to 0
    ::memset(&m_rAwbSyncOutput, 0, sizeof(SYNC_AWB_OUTPUT_INFO_STRUCT));  //  initialize, Set all to 0
    ::memset(&m_rAeSyncOutput, 0, sizeof(SYNC_AE_OUTPUT_STRUCT));  //  initialize, Set all to 0
}
...
syncAwbInit()
{
    ...
    IAwbMgr::getInstance().CCTOPAWBGetNVRAMParam(m_i4Master, CAM_SCENARIO_PREVIEW, &rAWBInitData.main_param);
    IAwbMgr::getInstance().CCTOPAWBGetNVRAMParam(m_i4Slave, CAM_SCENARIO_PREVIEW, &rAWBInitData.sub_param);
...
is2ASyncFinish()
{
    ...
    {
        CAM_LOGD("[%s] m_i4AwbSyncCount = (%d)", __FUNCTION__, m_i4AwbSyncCount);
...
isPerframeAE()
{
    //get Perframe AE Flag
    MBOOL   m_bMasterPerframeAE;
    MBOOL   m_bSlavePerframeAE;

    IAeMgr::getInstance().getPerframeAEFlag(m_i4Master, m_bMasterPerframeAE);
    IAeMgr::getInstance().getPerframeAEFlag(m_i4Slave, m_bSlavePerframeAE);

    return (m_bMasterPerframeAE & m_bSlavePerframeAE);
}
...
sync(MINT32 i4Sensor, MINT32 i4Param, MVOID* pSttBuf)
{
    ...
    if(!(i4Param & E_SYNC3A_DO_SW_SYNC))
    {
        ...
        if(i4Sensor == m_i4Slave)
        {
            if (bIsHwSync && (m_i4AeSchedule == 0))
                wait(i4Sensor);
        }
    ...
    } else
    {
        ...
        else
        {
            ...
            //else
            //    AAASensorMgr::getInstance().updateStereoFrameSyncFps(m_i4Master, m_i4Slave,0,0);
            post(i4Sensor);
            m_pMasterSttBuf = NULL;
            m_pSlaveSttBuf = NULL;
        }
