...
doCaptureEnd()
{
    ...
    StatisticBufInfo* rPSOBufInfo = Hal3ASttCtrl::getInstance(m_SensorDevId)->getBufMgr(BUF_PSO)->dequeueSwBuf();
    ...
    // TSF
        ...
        rTsfInfo.u4FrmReq = m_pStateMgr->queryMagicNumberRequest();
...
doUpdate()
{
    ...
    rPSOBufInfo = Hal3ASttCtrl::getInstance(m_SensorDevId)->getBufMgr(BUF_PSO)->dequeueSwBuf();
    AAA_TRACE_END_L;
    if(rBufInfo == NULL || rPSOBufInfo == NULL) return S_3A_OK;
...
*/
        ...
        {
            // restore AE
            CAM_LOGD_IF(m_i4EnableLog,"[%s] restore AE", __FUNCTION__);
            IAeMgr::getInstance().doRestoreAEInfo(m_SensorDevId, MTRUE);
            IAeMgr::getInstance().setRestore(m_SensorDevId, 0);
            // restore AWB
            CAM_LOGD_IF(m_i4EnableLog,"[%s] restore AWB", __FUNCTION__);
            IAwbMgr::getInstance().restore(m_SensorDevId);
        }
    ...
    }else
    {
        ...
        {
            // restore AE
            CAM_LOGD_IF(m_i4EnableLog,"[%s] FlashOff : restore AE", __FUNCTION__);
            IAeMgr::getInstance().doRestoreAEInfo(m_SensorDevId, MTRUE);
            IAeMgr::getInstance().setRestore(m_SensorDevId, 0);
            // restore AWB
            CAM_LOGD_IF(m_i4EnableLog,"[%s] FlashOff : restore AWB", __FUNCTION__);
            IAwbMgr::getInstance().restore(m_SensorDevId);
            m_pStateMgr->resetRestoreCount();
        }
    }
