                   ...
                   ISP_NVRAM_LCS_T const& p1Lcs, ISP_NVRAM_PGN_T const& p2Pgn, ISP_NVRAM_CCM_T const& p2CCM, MUINT16* pLcsBuffer)
{
    ...
    mInInfo.eLceProfile = static_cast<eLCE_PROFILE_T>(rRawIspCamInfo.eIspProfile);
    ...
    if (mAlgoEn)
    {
        ...
        if (mLogEn)
        {
            CAM_LOGD("%s: LCE EXIF: %x/; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x"
                , __FUNCTION__
