...
LensDrv::init(unsigned int a_u4CurrSensorDev )
{
        ...
        if (m_userCnt_main == 0)
        {
            if (m_fdMCU_main == -1)
            {
                ...
                if (m_fdMCU_main < 0)
                {
                    ...
                    {
                        DRV_DBG("main Lens error opening %s", strerror(errno));
                ...
                if (err <= 0)
                {
                    DRV_DBG("[mcuIOC_S_SETDRVNAME] please check kernel driver");
                    close(m_fdMCU_main);
                    m_fdMCU_main = -1;
                    return MCUDrv::MCU_NO_ERROR;
                }
        ...
        if (m_userCnt_main2 == 0)
        {
            if (m_fdMCU_main2 == -1)
            {
                ...
                if (m_fdMCU_main2 < 0)
                {
                    ...
                    {
                        DRV_DBG("main2 Lens error opening %s", strerror(errno));
                        return MCUDrv::MCU_INVALID_DRIVER;
                    }
                }
        ...
        if (m_userCnt_sub == 0)
        {
            if (m_fdMCU_sub == -1)
            {
                ...
                if (m_fdMCU_sub < 0)
                {
                    ...
                    {
                        DRV_DBG("Sub Lens error opening %s", strerror(errno));
                        return MCUDrv::MCU_INVALID_DRIVER;
                    }
                }
    ...
    {
        a_u4CurrLensIdx=MCUDrv::m_u4CurrLensIdx_sub2;

        DRV_DBG("sub2 lens init() [m_userCnt]%d  +\n", m_userCnt_sub2);
        DRV_DBG("[sub2 Lens Driver]%s\n", MCUDrv::m_LensInitFunc_sub2[a_u4CurrLensIdx].LensDrvName);
        ...
        if (m_userCnt_sub2 == 0)
        {
            if (m_fdMCU_sub2 == -1)
            {
                m_fdMCU_sub2 = open("/dev/SUB2AF", O_RDWR);
                ...
                if (m_fdMCU_sub2 < 0)
                {
                    ...
                    {
                        DRV_DBG("Sub2 Lens error opening %s", strerror(errno));
                        return MCUDrv::MCU_INVALID_DRIVER;
                    }
                }

                mcuMotorName motorName;
                memcpy(motorName.uMotorName, MCUDrv::m_LensInitFunc_sub2[a_u4CurrLensIdx].LensDrvName, 32);
                DRV_DBG("Select Driver Name : %s\n", motorName.uMotorName);

                int err = ioctl(m_fdMCU_sub2,mcuIOC_S_SETDRVNAME,&motorName);
                if (err <= 0)
                {
                    DRV_DBG("[mcuIOC_S_SETDRVNAME] please check kernel driver");
                    close(m_fdMCU_sub2);
        ...
        m_userCnt_sub2++;
        DRV_DBG("sub2 lens init() [m_userCnt]%d [fdMCU_sub2]%d - \n", m_userCnt_sub2,m_fdMCU_sub2);
    }
...
LensDrv::uninit(unsigned int a_u4CurrSensorDev )
{
    ...
    {
        DRV_DBG("sub2 lens uninit() [m_userCnt]%d [fdMCU_sub2]%d + \n", m_userCnt_sub2, m_fdMCU_sub2);
        Mutex::Autolock lock(mLock);
        if (m_userCnt_sub2 == 1)
        {
            if (m_fdMCU_sub2 > 0) close(m_fdMCU_sub2);
            m_fdMCU_sub2 = -1;
        }
        m_userCnt_sub2--;

        if (m_userCnt_sub2 < 0)   {m_userCnt_sub2 = 0;}

        DRV_DBG("sub2 lens uninit() [fdMCU_sub]%d - \n", m_fdMCU_sub2);
    }
    else
        return MCUDrv::MCU_INVALID_DRIVER;
...
LensDrv::moveMCU(int a_i4FocusPos,unsigned int a_u4CurrSensorDev )
{
    ...
    if (a_fdMCU < 0) {
        ...
        if (a_u4CurrLensIdx == 0)  {  // no error log for dummy lens
            ...
            DRV_DBG("[moveMCU] invalid m_fdMCU =%d\n", a_fdMCU);
            return MCUDrv::MCU_INVALID_DRIVER;
        }
    }

    if (a_i4FocusPos < 0 || a_i4FocusPos > 1023)
    {
        DRV_DBG("[moveMCU] ioctl - mcuIOC_T_MOVETO, position %d",  a_i4FocusPos);
        return -1;
    }

    err = ioctl(a_fdMCU,mcuIOC_T_MOVETO,(unsigned long)a_i4FocusPos);
    if (err < 0) {
        DRV_DBG("[moveMCU] ioctl - mcuIOC_T_MOVETO, error %s",  strerror(errno));
        return err;
    }
...
LensDrv::getMCUInfo(mcuMotorInfo *a_pMotorInfo, unsigned int a_u4CurrSensorDev )
{
    ...
    if (a_fdMCU < 0) {
        ...
        if (a_u4CurrLensIdx == 0)  {  // no error log for dummy lens
            ...
            DRV_DBG("[getMCUInfo] invalid m_fdMCU =%d\n", a_fdMCU);
            a_pMotorInfo->bIsMotorOpen = 0;
            return MCUDrv::MCU_INVALID_DRIVER;
        }
    }
    ...
    if (err < 0) {
        DRV_DBG("[getMCUInfo] ioctl - mcuIOC_G_MOTORINFO, error %s",  strerror(errno));
        return err;
    }
...
LensDrv::setMCUInfPos(int a_i4FocusPos,unsigned int a_u4CurrSensorDev )
{
    ...
    if (a_fdMCU < 0) {
        ...
        if (a_u4CurrLensIdx == 0)  {  // no error log for dummy lens
            ...
            DRV_DBG("[setMCUInfPos] invalid m_fdMCU =%d\n", a_fdMCU);
            return MCUDrv::MCU_INVALID_DRIVER;
        }
    }
    ...
    if (err  < 0) {
        DRV_DBG("[setMCUInfPos] ioctl - mcuIOC_T_SETINFPOS, error %s",  strerror(errno));
        return err;
    }
...
LensDrv::setMCUMacroPos(int a_i4FocusPos,unsigned int a_u4CurrSensorDev )
{
    ...
    if (a_fdMCU < 0) {
        if (a_u4CurrLensIdx == 0)  {  // no error log for dummy lens
            ...
            DRV_DBG("[setMCUMacroPos] invalid m_fdMCU =%d\n", a_fdMCU);
            return MCUDrv::MCU_INVALID_DRIVER;
        }
    }
    ...
    if (err < 0) {
        DRV_DBG("[setMCUMacroPos] ioctl - mcuIOC_T_SETMACROPOS, error %s",  strerror(errno));
        return err;
    }
...
LensDrv::getMCUCalPos(mcuMotorCalPos *a_pMotorCalPos,unsigned int a_u4CurrSensorDev)
{
    ...
    if (err < 0) {
        DRV_DBG("[getMCUInfo] ioctl - mcuIOC_G_MOTORCALPOS, error %s",  strerror(errno));
        return err;
    }
...
LensDrv::setMCUParam(int a_CmdId, int a_Param,unsigned int a_u4CurrSensorDev )
{
    ...
    DRV_DBG("setMCUParam() - a_CmdId = %d \n", a_CmdId);
    DRV_DBG("setMCUParam() - a_Param = %d \n", a_Param);
    ...
    if (a_fdMCU < 0) {
        if (a_u4CurrLensIdx == 0)  {  // no error log for dummy lens
            ...
            DRV_DBG("[setMCUParam] invalid m_fdMCU =%d\n", a_fdMCU);
            return MCUDrv::MCU_INVALID_DRIVER;
        }
    }
    ...
    motorCmd.u4CmdID = (u32)a_CmdId;
    motorCmd.u4Param = (u32)a_Param;

    err = ioctl(a_fdMCU, mcuIOC_S_SETPARA, &motorCmd);
    if (err < 0) {
        DRV_DBG("[setMCUMacroPos] ioctl - mcuIOC_T_SETPARA, error %s",  strerror(errno));
        return err;
    }
