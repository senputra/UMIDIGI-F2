...
init(const ConfigParam& rCfg, const char* strUser)
{
    ...
    CAM_LOGD("[%s] cfg(%d, %d, %d)", __FUNCTION__, m_rCfgParam.i4SensorDev, m_rCfgParam.i4TgInfo, m_rCfgParam.eEventType);
}
...
wait(Duration& rDuration)
{
    ...
    cur = getUsTime();
    CAM_LOGD("[%s] cur0(%d)", __FUNCTION__, cur);
    dif0 = cur - m_i4TimeStampMark;
    m_EventCond.wait(m_EventMtx);
    cur = getUsTime();
    CAM_LOGD("[%s] cur1(%d)", __FUNCTION__, cur);
    dif1 = cur - m_i4TimeStampEvent;
    rDuration.i4Duration0 = dif0;
    rDuration.i4Duration1 = dif1;
    return i4EventCnt;
...
threadLoop(MVOID* arg)
{
    ...
    while (_this->m_fgThreadOn)
    {
        ...
        CAM_LOGD("[%s] event(%ld)", __FUNCTION__, _this->m_i4TimeStampEvent);
