...
void sensorListenHandler(ASensorEvent event)
{
    switch(event.type)
    {
...
config()
{
    ...
    FlashMgr::getInstance().setIsDriverInControl(m_i4SensorDev,MTRUE);
    AAA_TRACE_END_D;
#endif
...
setParams(Param_T const &rNewParam, MBOOL bUpdateScenario)
{
    ...
    CAM_LOGD("[%s] m_bEnable3ASetParams = %d", __FUNCTION__, m_bEnable3ASetParams);
...
setAfParams(AF_Param_T const &rNewParam)
{
        ...
        if (m_pStateMgr != NULL)
            m_pStateMgr->setAfMode(rNewParam.u4AfMode);
...
getCurrResult(MUINT32 i4FrmId, Result_T& rResult, MINT32 i4SubsampleIndex) const
{
    ...
    CAM_LOGD_IF(m_3ALogEnable, "[%s] #lsc(%d) m_rParam.i4RawType(%d), m_rParam.u1ShadingMapMode(%d), m_bIsCapEnd(%d)", __FUNCTION__, pLsc->getOnOff(), m_rParam.i4RawType, m_rParam.u1ShadingMapMode, m_bIsCapEnd);
...
*/
    ...
    if (m_rParam.u1IsGetExif || m_bIsCapEnd)
    {
        ...
        {
            CAM_LOGD_IF(m_3ALogEnable, "[%s] vecDbgShadTbl vecDbgIspInfo vecDbg3AInfo - Size(%d, %d, %d) Addr(%p, %p, %p)", __FUNCTION__, rResult.vecDbgShadTbl.size(), rResult.vecDbgIspInfo.size(), rResult.vecDbg3AInfo.size(),
                                        &rDbgShadTbl, &rDbgIspInfo, &rDbg3AInfo);
            getP1DbgInfo(rDbg3AInfo, rDbgShadTbl, rDbgIspInfo);
        }
        else
            CAM_LOGE("[%s] vecDbgShadTbl vecDbgIspInfo vecDbg3AInfo - Size(%d, %d, %d) Addr(%p, %p, %p)", __FUNCTION__, rResult.vecDbgShadTbl.size(), rResult.vecDbgIspInfo.size(), rResult.vecDbg3AInfo.size(),
                                        &rDbgShadTbl, &rDbgIspInfo, &rDbg3AInfo);
...
postCommand(ECmd_T const r3ACmd, const ParamIspProfile_T* pParam)
{
    ...
    if ((m_rParam.u1CaptureIntent == MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE) && (r3ACmd == ECmd_Update) && (m_rParam.u4AeMode != MTK_CONTROL_AE_MODE_OFF))
    {
        ...
        CAM_LOGD_IF(m_3ALogEnable,"[%s] i4OperMode(%d) bUpdateStatus(%d) mbIsHDRShot(%d) bAELock(%d) EV(%d) EVchanged (%d)", __FUNCTION__, i4OperMode, bUpdateStatus, mbIsHDRShot, bAELock, m_i4EVCap, bIsEVchaged);
        if (bUpdateStatus) //should't update captureparams after precapture until capture start
        {
/*
...
*/
#if CAM3_FLASH_FEATURE_EN
            if(!fgMainFlash && !mbIsHDRShot && (i4OperMode != EOperMode_Meta) && !bAELock)
#endif
            {
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    switch (e3ACtrl)
    {
        ...
        case E3ACtrl_SetAEEISRecording:
            IAeMgr::getInstance().IsEISRecording(m_i4SensorDev, iArg1);
            break;
        ...
        case E3ACtrl_SetMinMaxFps:
            if((MUINT32)iArg1 != m_i4OverrideMinFrameRate || (MUINT32)iArg2 != m_i4OverrideMaxFrameRate)
            {
        ...
        case E3ACtrl_SetShadingSdblkCfg:
            if (NSIspTuning::EOperMode_Meta != IspTuningMgr::getInstance().getOperMode(m_i4SensorDev))
            {
                ...
                i4Ret = IAwbMgr::getInstance().setEMDumpCapAAO(m_i4SensorDev, (iArg1 ? MTRUE : MFALSE), reinterpret_cast<const char*>(iArg2));
            }
            break;
        ...
        case E3ACtrl_SetOperMode:
            ...
            CAM_LOGD_IF(m_3ALogEnable, "[%s] prev_mode(%d), new_mode(%d)", __FUNCTION__, i4OperMode, iArg1);
            break;
        ...
        case E3ACtrl_GetDualZoomInfo:
            ...
            {
                MINT32 data = 0;
                IAfMgr::getInstance().sendAFCtrl(m_i4SensorDev, EAFCtrl_GetPostureDAC, reinterpret_cast<MINTPTR>(&data), NULL);
                rDualZoomInfo->i4AFDAC = data;
            }
            rDualZoomInfo->bAFDone = IAfMgr::getInstance().getAFStable(m_i4SensorDev);
...
notifyResult4TG(MINT32 i4PreFrmId, Result_T *rPreResult)
{
    ...
    CAM_LOGD_IF(m_3ALogEnable,"[%s] u4Rto(%d),OBCGain(%d),i8SensorExposureTime(%lld),i8SensorFrameDuration(%lld),i4SensorSensitivity(%d)", __FUNCTION__,
        u4Rto,OBCGain,rPreResult->i8SensorExposureTime,rPreResult->i8SensorFrameDuration,rPreResult->i4SensorSensitivity);
    ...
    CAM_LOGD_IF(m_3ALogEnable,"[%s] vecExifInfo.size(%d), vecDbg3AInfo.size(%d), vecDbgIspInfo.size(%d),m_rParam.u1IsGetExif(%d),m_bIsCapEnd(%d)", __FUNCTION__,
        rPreResult->vecExifInfo.size(),rPreResult->vecDbg3AInfo.size(),rPreResult->vecDbgIspInfo.size(),m_rParam.u1IsGetExif,m_bIsCapEnd);
...
notify4CCU(MUINT32 u4PreFrmId, Result_T *rPreResult)
{
    if(m_bCCUEn && m_pThreadSetCCUPara)
    {
        ThreadSetCCUPara::Cmd_T rCmd(u4PreFrmId, rPreResult, m_rParam.u1IsGetExif, m_bIsCapEnd);
