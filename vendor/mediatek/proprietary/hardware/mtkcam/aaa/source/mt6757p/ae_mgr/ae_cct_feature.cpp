...
)
{
    ...
    if((m_pIAeAlgo != NULL)&&(!pAEExpParam->bFlareAuto)) {
        ...
        CAM_LOGD("[CCTOPAEApplyExpParam] m_pIAeAlgo = %d bFlareAuto:%d\n", m_pIAeAlgo, pAEExpParam->bFlareAuto);
    }
...
)
{
    ...
    MUINT32 u4AENVRAMIdx = AENVRAMMapping[eCamScenarioMode];
    AE_NVRAM_T *pAENVRAM = reinterpret_cast<AE_NVRAM_T*>(a_pAENVRAM);

    CAM_LOGD("[ACDK_CCT_OP_DEV_AE_APPLY_INFO]\n");

    m_rAEInitInput.rAENVRAM = *pAENVRAM;
    m_p3ANVRAM->rAENVRAM[u4AENVRAMIdx] = m_rAEInitInput.rAENVRAM;
...
)
{
    ...
    // CCT workaround for forcing finer-EV interpolation
    setAEFlickerMode(MTK_CONTROL_AE_ANTIBANDING_MODE_60HZ);
    setAEFlickerMode(MTK_CONTROL_AE_ANTIBANDING_MODE_50HZ);
...
{
    CAM_LOGD("[ACDK_CCT_OP_AE_GET_ISP_OB]\n");
/*
...
{
    CAM_LOGD("[ACDK_CCT_OP_AE_SET_ISP_OB]\n");
/*
