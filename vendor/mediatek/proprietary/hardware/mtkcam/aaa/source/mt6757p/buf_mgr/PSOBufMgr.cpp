    ...
    , m_bPreStop(MFALSE)
{
    ...
    property_get("vendor.debug.pso_mgr.enable", value, "0");
...
dequeueHwBuf()
{
    ...
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.pso.dump.enable", value, "0");
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        CAM_LOGD_IF(m_bDebugEnable, "port(%d), index(%d), va[%p]/pa[0x%08x]/#(%d), Size(%d), rawType(%d)",
                  m_rPort.index, index, rLastBuf.mVa, rLastBuf.mPa, rLastBuf.mMetaData.mMagicNum_hal, rLastBuf.mSize, rLastBuf.mMetaData.mRawType);
            ...
            const ConfigInfo_T& rConfigInfo)
{
    ...
    if(m_rBufInfo.m_pPSOSepBuf != NULL && m_rBufInfo.m_u4PSOSepsize != 0){
        CAM_LOGD_IF(m_fgLog, "[%s] m_rBufInfo->m_pPSOSepBuf = 0x%x, \n", __FUNCTION__, m_rBufInfo.m_pPSOSepBuf);
        CAM_LOGD_IF(m_fgLog, "[%s] m_rBufInfo->m_u4PSOSepsize = 0x%x, \n", __FUNCTION__, m_rBufInfo.m_u4PSOSepsize);
    }
