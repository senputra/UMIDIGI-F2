...
tsfKeep(const TSF_INPUT_STAT_T& rInputStat, const ILscTbl& rTblPrior, ILscTbl& rOutputTbl)
{
    ...
    LSC_LOG_BEGIN("Sensor(%d), #(%d), ratio(%d)", __FUNCTION__, m_u4SensorDev, rInputStat.u4FrmId, u4Ratio);
    ...
    LSC_LOG_END("Sensor(%d), #(%d)", __FUNCTION__, m_u4SensorDev, rInputStat.u4FrmId);
