...
doPvAWB(MINT32 i4SensorDev, MINT32 i4FrameCount, MBOOL bAEStable, MINT32 i4SceneLV, MVOID *pAWBStatBuf, MBOOL bApplyToHW)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub2 = AwbMgr::getInstance(ESensorDev_SubSecond).doPvAWB(i4FrameCount, bAEStable, i4SceneLV, pAWBStatBuf, bApplyToHW);
...
MRESULT IAwbMgr::setCamScenarioMode(MINT32 i4SensorDev, MUINT32 u4CamScenarioMode)
{
    ...
    ret_main = ret_sub = ret_main2 = ret_sub2 = S_AWB_OK;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAwbMgr::setEMDumpCapAAO(MINT32 i4SensorDev, MBOOL fgSave, const char* filename)
{
    if (i4SensorDev & ESensorDev_Main)
        return AwbMgr::getInstance(ESensorDev_Main).setEMDumpCapAAO(fgSave, filename);
    if (i4SensorDev & ESensorDev_Sub)
        return AwbMgr::getInstance(ESensorDev_Sub).setEMDumpCapAAO(fgSave, filename);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AwbMgr::getInstance(ESensorDev_MainSecond).setEMDumpCapAAO(fgSave, filename);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AwbMgr::getInstance(ESensorDev_SubSecond).setEMDumpCapAAO(fgSave, filename);
    CAM_LOGE("Err IAwbMgr::setEMDumpCapAAO()\n");
