...
static android::Mutex g_VCMDevLock; // VCM drivers only use one API due to custom_drv setting.
...
MUINT32 MCUDrv::ctrlMCUParam(MUINT64 a_CmdId, MUINT64 *a_Param)
{
    ...
    if (a_CmdId == MCU_CMD_SET_SOF_TS)
    {
        m_SofTS = *a_Param;
        return 1;
    }
    else if (a_CmdId == MCU_CMD_SET_AFO_TS)
    {
        m_AfoTS = *a_Param;
        return 1;
    }
...
MUINT32 MCUDrv::getMCUInfo(mcuMotorInfo *a_pMotorInfo)
{
    ...
    a_pMotorInfo->u8SofTimestamp      = m_SofTS;
    a_pMotorInfo->u8AfoTimestamp      = m_AfoTS;
