...
updateSensor(MVOID* input)
{
    ...
    if(m_bSetShutterValue) {    // update shutter value
        ...
        AE_SETTING_LOG_IF(m_3ALogEnable,"[%s()] Exposure time LE/SE value = %d/%d, LV = %d\n", __FUNCTION__, pinput->u4LEHDRshutter, pinput->u4SEHDRshutter,pinput->i4Pass2SensorLVvalue);
...
updateSensorExp(MVOID* input)
{
  ...
  AE_SETTING_LOG( "[%s] restore sensorExp  input(%d) LE(%d) SE(%d) LV(%d)",__FUNCTION__,*psensorExp,u4ExpLEstore,u4ExpSEstore);
...
controltrigger(MUINT32 index, MBOOL btrigger)
{
    ...
    switch(etrigger){
        ...
        case E_AE_SETTING_CPU_STORE:
            ...
            AE_SETTING_LOG("[%s()] Exp : %d , %d Afe: %d , %d",__FUNCTION__,u4ExpLEperframe,u4ExpSEperframe,u4AfeLEperframe,u4AfeSEperframe);
            break;
        default:
