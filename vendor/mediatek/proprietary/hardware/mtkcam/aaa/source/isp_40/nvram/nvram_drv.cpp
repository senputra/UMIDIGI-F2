...
)
{
    ...
    //android P upgrade to Q issue: NVRAM version file struct change leeds to "nv_version" read fail
    if (version == -1) {
        logI("read nv_version fail : try read nvram file first");
                          ...
                          unsigned long a_u4NvramDataSize)
{
    ...
    if (err != NVRAM_NO_ERROR && a_eNvramDataType == CAMERA_NVRAM_VERSION) {
        //android P upgrade to Q issue: NVRAM version file struct change leeds to "nv_version" read fail
        memset(a_pNvramData, -1, g_u4NvramDataSize[a_eNvramDataType]);
        logI("read nv_version fail set version to -1 and try to read nvram file first.");
        return NVRAM_NO_ERROR;
    } else if (err != NVRAM_NO_ERROR) {
        logE("err(%d)", err);
    }
    return err;
}
