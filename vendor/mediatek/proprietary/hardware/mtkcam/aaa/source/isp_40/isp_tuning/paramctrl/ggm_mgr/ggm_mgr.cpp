...
getTonemapCurve_Red(MFLOAT*& p_in_red, MFLOAT*& p_out_red, MINT32 *pCurvePointNum)
{
    for (MINT32 i = 0; i < ENC_BUF_SIZE; i++) {
        m_encBuf[i] = m_ggm.lut_rb.lut[i].GGM_R;
    }
...
{
    for (MINT32 i = 0; i < ENC_BUF_SIZE; i++) {
        m_encBuf[i] = m_ggm.lut_g.lut[i].GGM_G;
    }
...
{
    for (MINT32 i = 0; i < ENC_BUF_SIZE; i++) {
        m_encBuf[i] = m_ggm.lut_rb.lut[i].GGM_B;
    }
...
setTonemapCurve_Red(MFLOAT *p_in_red, MFLOAT *p_out_red, MINT32 *pCurvePointNum)
{
    ...
    for (MINT32 i = 0; i < ENC_BUF_SIZE; i++) {
        m_ggm.lut_rb.lut[i].GGM_R = m_encBuf[i];
    }
...
setTonemapCurve_Green(MFLOAT *p_in_green, MFLOAT *p_out_green, MINT32 *pCurvePointNum)
{
    ...
    for (MINT32 i = 0; i < ENC_BUF_SIZE; i++) {
        m_ggm.lut_g.lut[i].GGM_G = m_encBuf[i];
    }
...
setTonemapCurve_Blue(MFLOAT *p_in_blue, MFLOAT *p_out_blue, MINT32 *pCurvePointNum)
{
    ...
    for (MINT32 i = 0; i < ENC_BUF_SIZE; i++) {
        m_ggm.lut_rb.lut[i].GGM_B = m_encBuf[i];
    }
        ...
        if ( j <  512 ) {
            if (j % 8 == 0) {
                ...
                if (wk2>=64) {
                    ...
                    CAM_LOGD("Warning: The slope is too large that can't be correct encoded with gamma format!");
                }
            }
        }
        else if ( j <  1024 ) {
            ...
            if (j % 32 == 0) {
                ...
                if (wk2>=64) {
                    ...
                    CAM_LOGD("Warning: The slope is too large that can't be corrected encoded with gamma format!");
                }
            }
        }
        else if ( j <  4096 ) {
