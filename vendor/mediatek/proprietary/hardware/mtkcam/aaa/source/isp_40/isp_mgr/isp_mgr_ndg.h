...
private:
    ...
    enum
    {
        ERegInfo_DIP_X_NDG_RAN_0,
        ERegInfo_DIP_X_NDG_RAN_2,
        ERegInfo_DIP_X_NDG_RAN_3,
        ERegInfo_DIP_X_NDG_CROP_X,
        ERegInfo_DIP_X_NDG_CROP_Y,
        ERegInfo_NUM
    };
    RegInfo_T m_rIspRegInfo[ERegInfo_NUM];

public:
    ISP_MGR_NDG(ESensorDev_T const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, m_u4StartAddr, eSensorDev)
        , m_bEnable(MTRUE)
        , m_bCCTEnable(MTRUE)
        , m_u4StartAddr(REG_ADDR_P2(DIP_X_NDG_RAN_0))
    {
        // register info addr init
        INIT_REG_INFO_ADDR_P2(DIP_X_NDG_RAN_0);
        INIT_REG_INFO_ADDR_P2(DIP_X_NDG_RAN_2);

        INIT_REG_INFO_ADDR_P2(DIP_X_NDG_RAN_3);
        INIT_REG_INFO_ADDR_P2(DIP_X_NDG_CROP_X);
        INIT_REG_INFO_ADDR_P2(DIP_X_NDG_CROP_Y);

        INIT_REG_INFO_VALUE(DIP_X_NDG_RAN_0, 0xD9000000);
    }
