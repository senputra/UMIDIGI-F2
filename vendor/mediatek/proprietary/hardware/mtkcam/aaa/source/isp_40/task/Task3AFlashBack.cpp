...
Task3AFlashBackImp::run(MINT32 i4Opt, const TaskData& rData)
{
    ...
    if(!(i4Opt & TASK_OPTION_BYP_FLASH_BACK)){
        ...
        if(rData.i4FlashTypeByTask == FLASH_HAL_SCENARIO_PRE_FLASH && rData.bFlashOpenByTask && !rData.bFlashOnOff){
            ...
            CAM_LOGD("[%s] Frame Count(%d), WAIT CLOSE FLASH, addr(%x)",__FUNCTION__, m_i4FrameCount, pAAOBuffer->getPart(AAOSepBuf));
        ...
        if(rData.i4FlashTypeByTask == -1 && !rData.bFlashOpenByTask && !rData.bFlashOnOff && m_i4FlashOffFrameCount == 0){
            ...
            m_pLastAAOBuffer = reinterpret_cast<MVOID *>(pAAOBuffer->getPart(AAOSepBuf));
            FlashMgr::getInstance(m_i4SensorDev)->setFlashOnAAOBuffer(m_pLastAAOBuffer);
            CAM_LOGD("[%s] Frame Count(%d) Off(%d), GET FLASH OPEN DATA, addr(%x)",__FUNCTION__, m_i4FrameCount, m_i4FlashOffFrameCount, pAAOBuffer->getPart(AAOSepBuf));
        }
        //===== Bypass Flash Partial Data =====
        if(m_i4FlashOffFrameCount != 0 && m_i4FrameCount - m_i4FlashOffFrameCount == 1){
            CAM_LOGD("[%s] Frame Count(%d) Off(%d), BYPASS PARTIAL, addr(%x)",__FUNCTION__, m_i4FrameCount, m_i4FlashOffFrameCount, pAAOBuffer->getPart(AAOSepBuf));
        }
        //===== Get Flash Close Data =====
        if(m_i4FlashOffFrameCount != 0 && m_i4FrameCount - m_i4FlashOffFrameCount == 2){
            CAM_LOGD("[%s] Frame Count(%d) Off(%d), GET FLASH CLOSE DATA, addr(%x/%x)",
                        __FUNCTION__, m_i4FrameCount, m_i4FlashOffFrameCount, m_pLastAAOBuffer, pAAOBuffer->getPart(AAOSepBuf));
            m_i4FlashOffFrameCount = 0;
