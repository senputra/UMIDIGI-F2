...
TaskAFTriggerImp::run(MINT32 i4Opt, const TaskData& rData)
{
    ...
    IAeMgr::getInstance().getAEMeteringYvalue(m_i4SensorDev, rAeWinSize, &iYvalue);
    ...
    IAeMgr::getInstance().getAEBlockYvalues(m_i4SensorDev, rAEInfo.aeBlockV, 25);
    IAeMgr::getInstance().getSensorParams(m_i4SensorDev, rSensorInfo);
    IAeMgr::getInstance().getRTParams(m_i4SensorDev, AEFrameParam);
    rAEInfo.i4IsAEStable= IAeMgr::getInstance().IsAEStable(m_i4SensorDev);
    rAEInfo.i4IsAELocked= IAeMgr::getInstance().IsAELock(m_i4SensorDev) || IAeMgr::getInstance().IsAPAELock(m_i4SensorDev);
    rAEInfo.i4ISO = rSensorInfo.u4Sensitivity;
    rAEInfo.i4SceneLV=IAeMgr::getInstance().getLVvalue(m_i4SensorDev,MTRUE);
    ...
    rAEInfo.i4DeltaBV=IAeMgr::getInstance().getDeltaBV(m_i4SensorDev);

    #if 1 // reduce CPU usage, useless code
    MUINT16 u2YCnt;
    IAeMgr::getInstance().getAEMeteringBlockAreaValue(m_i4SensorDev,rAeWinSize, m_uAEBlockAreaY, &u2YCnt);
    ...
    if(m_i4FrameCount >= 90 && i4OperMode == NSIspTuning::EOperMode_Normal) // Change to 90, because af doesn't converge after kill mediaServer.
    {
