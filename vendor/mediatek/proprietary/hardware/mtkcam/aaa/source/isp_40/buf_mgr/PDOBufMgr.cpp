...
class PDOBufMgrImp : public PDOBufMgr
{
    struct ImgoInfo
    {
        ...
        ImgoInfo()
            ...
            , mTimeStamp(0)
        {
            memset( &mMotorInfo, 0, sizeof(mcuMotorInfo));
        }
...
protected:
    ...
    */
    virtual MBOOL deque_pure_raw(StatisticBufInfo &oSttBufInfo, mcuMotorInfo **oMCUInfo);
    /**
...
private:
    ...
    mcuMotorInfo           m_sMotorInfo;
    MUINT64                m_SofTimeStamp;
};
    ...
    , m_SofTimeStamp(0)
{
    ...
    //
    memset(&m_sMotorInfo, 0, sizeof(m_sMotorInfo));
...
dequeueHwBuf()
{
    ...
    memset( &rDQBuf, 0, sizeof(QBufInfo));
    memset( &rHwBuf, 0, sizeof(StatisticBufInfo));
    ...
    switch( m_i4PDDataPath)
    {
            ...
            if(m_pSttPipe)
            {
                ...
                else
                {
                    CAM_LOGD_IF( m_bDebugEnable,
                                 "[%s] dev(%d), deque is not successful. dequed(%d) buf list size(%d)",
                                 __FUNCTION__,
                                 m_i4SensorDev,
                                 bDequed,
                                 rDQBuf.mvOut.size());
        ...
        case FEATURE_PDAF_SUPPORT_LEGACY:
        {
            if(m_i4IonDevFd)
            {
                if(deque_pure_raw(rHwBuf, &pMotorInfo))
                {
        ...
        case FEATURE_PDAF_SUPPORT_VIRTUAL_CHANNEL:
        {
            if(m_pCamsvSttPipe)
            {
                ...
                if( bDequed && rDQBuf.mvOut.size())
                {
                    ...
                    rHwBuf.mLensPosition    = m_i4LensPosPre;
                    rHwBuf.mTimeStamp       = rLastDQBuf.mMetaData.mTimeStamp; /* nano second */
                    if (rHwBuf.mTimeStamp == 0 && (m_SofTimeStamp != pMotorInfo->u8SofTimestamp))
                    {
                        CAM_LOGD_IF(m_bDebugEnable, "[%s] PDO done(%lld), SOF(%lld), AFO(%lld)", __FUNCTION__, (MINT64)_GET_TIME_STAMP_NS_(), (MINT64)pMotorInfo->u8SofTimestamp, (MINT64)pMotorInfo->u8AfoTimestamp);
                        rHwBuf.mTimeStamp = pMotorInfo->u8SofTimestamp;
                        m_SofTimeStamp = pMotorInfo->u8SofTimestamp;
                    }
                }
                else
                {
    ...
    FLASHResultToMeta_T* pFLASHResult = (FLASHResultToMeta_T*)pResultPoolObj->getResult(rHwBuf.mMagicNumber,E_FLASH_RESULTTOMETA);
...
MBOOL
PDOBufMgrImp::
deque_pure_raw(StatisticBufInfo &oSttBufInfo, mcuMotorInfo **oMCUInfo)
{
    MBOOL ret = MFALSE;
    ...
    {
        {
            ...
            if(m_vecPureRawBufInfo.size())
            {
                ...
                //
                memcpy( &m_sMotorInfo, &(pureRawBufInfo->mMotorInfo), sizeof(mcuMotorInfo));

                (*oMCUInfo) = &m_sMotorInfo;
...
enque_pure_raw(StatisticBufInfo &iSttBufInfo)
{
    ...
    else
    {
        ...
        //
        IPDMgr::getInstance().imgoIsEnqueued(m_i4SensorDev, &iSttBufInfo);
        ...
        if(imgBufHeap)
        {
            ...
            if( (ImageDescRawType)(bufFmt)==eIMAGE_DESC_RAW_TYPE_PURE)
            {
                    ...
                    ::ion_import(user->m_i4IonDevFd, pureRawBufInfo.mImgHeapID, &(pureRawBufInfo.mIonBuf_handle))==0)
                {
                    ...
                    {
                        //
                        MCUDrv::getInstance(user->m_i4SensorDev)->getMCUInfo( &(pureRawBufInfo.mMotorInfo));
                        //
                        pureRawBufInfo.mImgSize           = imgBuf->getImgSize();
                        pureRawBufInfo.mImgBitsPerPixel   = imgBuf->getImgFormat();
                        pureRawBufInfo.mPlaneBitsPerPixel = imgBuf->getPlaneBitsPerPixel(0);
                        pureRawBufInfo.mPlaneCount        = imgBuf->getPlaneCount();
                        pureRawBufInfo.mBufSizeInBytes    = imgBuf->getBufSizeInBytes(0);
                        pureRawBufInfo.mBufStridesInBytes = imgBuf->getBufStridesInBytes(0);
...

                        {
                                ...
                                user->m_vecPureRawBufInfo.size())
                            {
                                for(vector<ImgoInfo>::iterator itr = user->m_vecPureRawBufInfo.begin(); itr !=user->m_vecPureRawBufInfo.end(); itr++)
                                {
                                    ...
                                    //
                                    StatisticBufInfo sttBufInfo;
                                    sttBufInfo.mMagicNumber      = itr->mMagicNum;
                                    sttBufInfo.mSize             = itr->mBufSizeInBytes;
                                    sttBufInfo.mStride           = itr->mBufStridesInBytes;
                                    sttBufInfo.mImgoSizeW        = itr->mImgSize.w;
                                    sttBufInfo.mImgoSizeH        = itr->mImgSize.h;
                                    sttBufInfo.mImgoBitsPerPixel = itr->mImgBitsPerPixel;
                                    sttBufInfo.mImgoFmt          = itr->mFmt;
                                    sttBufInfo.mImgoBuf_handle   = itr->mIonBuf_handle;
                                    sttBufInfo.mImgoBuf_share_fd = itr->mIonBuf_share_fd;
                                    sttBufInfo.mImgHeapID        = itr->mImgHeapID;
                                    sttBufInfo.mLensPosition     = itr->mLensPos;
                                    IPDMgr::getInstance().imgoIsEnqueued(user->m_i4SensorDev, &sttBufInfo);
                                }
