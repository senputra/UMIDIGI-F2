...
notifyResult4CCU(ThreadSetCCUParaImp::Cmd_T rCmd)
{
    ...
    NSIspTuning::ILscMgr* pLsc = NSIspTuning::ILscMgr::getInstance(static_cast<MUINT32>(m_u4SensorDev));
    ...
    if (rCmd.u1IsGetExif || rCmd.bIsCapEnd)
    {
        ...
        if( pAllResult->vecExifInfo.size() > 0 && pAllResult->vecDbg3AInfo.size() > 0 && pAllResult->vecDbgIspInfo.size() > 0)
        {
            ...
            //ILscMgr::getInstance(static_cast<MUINT32>(m_u4SensorDev))->getDebugInfo(rDbg3AInfo1.rShadigDebugInfo);
