...
#define MY_INST NS3Av3::INST_T<IspTuningBufCtrl>
...
getInstance(MINT32 i4SensorDevId)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<IspTuningBufCtrl>();
    } );
...
updateHint(MVOID *hint, MINT32 i4MagicNum){
    ...
    for (; it != mBufferInfoQ.end(); it++, i4Pos++)
    {
        ...
        else if (it->i4MagicNum == i4MagicNum)
        {
            ...
            ALOGE("hhhhh dump p1 cb buffer(%s)", strlscdump);
            //call dump util
