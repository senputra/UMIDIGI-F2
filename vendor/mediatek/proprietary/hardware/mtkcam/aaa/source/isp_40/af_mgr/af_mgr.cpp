...
MRESULT AfMgr::passAFBuffer( MVOID *ptrInAFData)
{
    ...
    MINT32 CurPos = (m_i4DbgMotorDisable == 1) ? m_i4DbgMotorMPos : m_sAFOutput.i4AFPos;
    ...
    else
    {
        ...
        for ( MUINT32 j = 0; j < nblkH; j++)
        {
            ...
            for ( MUINT32 i = 0; i < nblkW; i++)
            {
                ...
                {
                    m_i4IsAFOValid &= MFALSE; // once false, always false
                    break;
                }
            }
            pDMABuf += u4Stride;
            if(m_i4IsAFOValid == MFALSE)
            {
                break;
            }
        }
        ...
        // For debug FV only.
        if (m_i4DgbLogLv && m_i4IsAFOValid)
        {
            //Debug only.
            MUINT64 FV_H0 = 0;
            MUINT64 FV_H1 = 0;
...
MBOOL AfMgr::IspMgrAFStatHWPreparing()
{
    ...
    if( m_i4DgbLogLv || bSkip==MTRUE)
    {
        ...
        ptrMsgBuf += sprintf( ptrMsgBuf,
                              "bSkip(%d) PauseAF(%d) Afmode(%d) VsyncUpdate(%d) IsTargetAssitMove(%d) isHWRdy(%d) ROISel(%d) ZECChg(%d) DbgPDVerifyEn(%d)",
                              bSkip,
                              m_bPauseAF,
                              m_eCurAFMode,
                              m_i4VsyncUpdate,
