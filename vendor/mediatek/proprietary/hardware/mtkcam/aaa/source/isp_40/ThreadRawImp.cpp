...
ThreadRawImp::onAFThreadLoop(MVOID *arg)
{
    ...
    while (_this->mbAFThreadLoop && IsSupportAF) {
        ...
        if(_this->mpTaskMgr->dequeStt(Task_Update_AF) >= 0)
        {
            ...
            rData.bIsAfTriggerInPrecap = (bIsAFStart && (bIsPrecapTrigger || isExistPrecap));
            // Nelson Coverity : bIsAfStart is always false under this condition (bIsAFStart && (bIsPrecapTrigger || isExistPrecap));
