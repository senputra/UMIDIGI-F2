...
#define OB_TO_INT(a) ((-(TO_INT((a), 13)))) /*10bit*/
...
getStaticMetadata() const
{
    ...
    if (fgOK)
    {
        ...
        /* Need to update OB value to android framework for DNG capture
         and framework will do OB on the pure raw whitch was return by hal layer */
        UPDATE_ENTRY_ARRAY(rMeta, MTK_SENSOR_BLACK_LEVEL_PATTERN, i4SensorOB, 4);
        UPDATE_ENTRY_SINGLE(rMeta, MTK_SENSOR_INFO_WHITE_LEVEL, (MINT32)4095);
...
convertColorTransform(const NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspParam)
{
    ...
    const ISP_NVRAM_CCM_T& ccm1 = rIspParam.ISPColorTbl.ISPMulitCCM.dynamic_CCM[i4RefIllum1];    // format 4.9, -7.99~8.0
    ...
    if (fgSupport2ColorMatrices)
    {
        const ISP_NVRAM_CCM_T& ccm2 = rIspParam.ISPColorTbl.ISPMulitCCM.dynamic_CCM[i4RefIllum2];    // format 4.9, -7.99~8.0
...
inline static AWB_GAIN_T _getAwbGain(UINT8 eIdx_CCM, const NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspParam)
{
    ...
    awb = rIspParam.ISPColorTbl.ISPMulitCCM.dynamic_CCM_AWBGain[eIdx_CCM];
...
getShadingMapFromHal(const IMetadata& halMeta, const IMetadata& appMeta) const
{
    ...
    if (QUERY_ENTRY_SINGLE<IMetadata::Memory>(halMeta, MTK_LSC_TBL_DATA, lscData))
    {
        MUINT8 u1ShadingMode = 0;
        QUERY_ENTRY_SINGLE(appMeta, MTK_SHADING_MODE, u1ShadingMode);
        shadingMeta = getShadingMapFromMem(u1ShadingMode, lscData);
    }
    return shadingMeta;
}
