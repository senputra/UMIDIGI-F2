...
Task3ATouchImp::run(MINT32 i4Opt, const TaskData& rData)
{
    ...
    if(!(i4Opt & TASK_OPTION_BYP_AWB))
    {
        ...
        if(pAAOBuffer)
#ifndef MT6757P_API
            IAwbMgr::getInstance().doAFAWB(m_i4SensorDev, i4AoeCompLv, reinterpret_cast<MVOID *>(pAAOBuffer->getPart(AAOSepLSCBuf)));
#else
            IAwbMgr::getInstance().doAFAWB(m_i4SensorDev, i4AoeCompLv, reinterpret_cast<MVOID *>(pAAOBuffer->getPart(AAOSepAWBBuf)));
#endif
        else
            CAM_LOGW("[%s] AAO buffer is NULL", __FUNCTION__);
        m_rTimer.End();
    ...
    // update torch exposure setting while flash on
    if(rData.bFlashOpenByTask == MTRUE)
        FlashMgr::getInstance(m_i4SensorDev)->setAlgoExpPara(rAeInfo.u4Eposuretime, rAeInfo.u4AfeGain, rAeInfo.u4IspGain, 1);
