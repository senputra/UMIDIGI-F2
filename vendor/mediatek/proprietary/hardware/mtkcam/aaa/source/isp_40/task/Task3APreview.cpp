...
Task3APreviewImp::run(MINT32 i4Opt, const TaskData& rData)
{
...
#if 0
            //Test
            AAA_TRACE_D("PsoInfoNon-Optimize");
            AAO_PROC_INFO_T rPSOInfoTest;
            // Get the previous setting of PSO, for example : OB, AWB, CCM, LSC info.
            IResultPool::getInstance(m_i4SensorDev)->getAAOInfoTest(AE_V4P0_BLOCK_NO, pPSOBuffer->mMagicNumber, rPSOInfoTest);
            // Get PSO buf info
            rPSOInfoTest.Pso_Output_Path = (PSO_PATH_SEL_ENUM)pPSOBuffer->mRawType; // PSO before OB or after OB
            // AE
            IAeMgr::getInstance().setAAOProcInfo(m_i4SensorDev, reinterpret_cast<MVOID *>(pPSOBuffer->getPart(PSOSepBuf)), &rPSOInfoTest);
            AAA_TRACE_END_D;
#else
            AAA_TRACE_D("PsoInfo");
            AAA_TRACE_MGR(PsoInfo);
        ...
        else
        {
            ...
            if(pAAOBuffer)
            {
                IAeMgr::getInstance().doPvAE(m_i4SensorDev,
                                             ...
                                             reinterpret_cast<MVOID *>(pAAOBuffer->getPart(AAOSepBuf)),
                                             i4ActiveAEItem,
                                             pAAOBuffer->mMagicNumber,
                                             bIsScheduleBy3A,pAAOBuffer->mu1AeMode);
            }
            else
...
#else
                                       reinterpret_cast<MVOID *>(pAAOBuffer->getPart(AAOSepAWBBuf)));
#endif
        m_rTimer.End();
        AAA_TRACE_END_MGR;
        AAA_TRACE_END_D;
    ...
    if(!(i4Opt & TASK_OPTION_BYP_TSF))
    {
...
#ifndef CAM3_FLASH_ON_SHADING_CT_3_TABLE
        if(rData.bFlashOnOff || rData.bFlashOpenByTask) //is out or 3A HAL(Task) open flash Ex: [out]torch or main flash, [3A HAL(Task)] [re-flash or touch
        {
            CAM_LOGD("[%s] (lock Ratio) Inform ILscMgr that flash is ON. (FlashOnOff, FlashOpenByTask) (%d, %d)",__FUNCTION__, rData.bFlashOnOff, rData.bFlashOpenByTask);
            //i4IsLockRto = 1;
            NSIspTuningv3::IspTuningMgr::getInstance().enableDynamicShading(m_i4SensorDev, MFALSE);
        }
        else
        {
            //i4IsLockRto = 0;
        ...
        NSIspTuningv3::IspTuningMgr::getInstance().sendIspTuningIOCtrl(
            m_i4SensorDev, NSIspTuningv3::IspTuningMgr::E_ISPTUNING_UPDATE_LSC_INFO, NSIspTuningv3::IspTuningMgr::E_SHADING_RATIO_FREE_RUN, 0);
