...
#define MY_INST NS3Av3::INST_T<ISP_MGR_PGN_T>
...
getInstance(ESensorDev_T const eSensorDev)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_PGN_T>(eSensorDev);
    } );
...
setIspPregain()
{
    ...
    // CAM_PGN_GAIN_1
    reinterpret_cast<ISP_DIP_X_PGN_GAIN_1_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_GAIN_1))->PGN_GAIN_B = m_rIspPregain.i4B;
    reinterpret_cast<ISP_DIP_X_PGN_GAIN_1_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_GAIN_1))->PGN_GAIN_GB = m_rIspPregain.i4G;

    // CAM_PGN_GAIN_2
    reinterpret_cast<ISP_DIP_X_PGN_GAIN_2_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_GAIN_2))->PGN_GAIN_GR = m_rIspPregain.i4G;
    reinterpret_cast<ISP_DIP_X_PGN_GAIN_2_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_GAIN_2))->PGN_GAIN_R = m_rIspPregain.i4R;

    // CAM_PGN_OFST_1
    MUINT32 OFFS = (m_i4IspFlareOffset >= 0) ? static_cast<MUINT32>(m_i4IspFlareOffset) : static_cast<MUINT32>(4096 + m_i4IspFlareOffset);
    reinterpret_cast<ISP_DIP_X_PGN_OFST_1_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_OFST_1))->PGN_OFST_B = OFFS;
    reinterpret_cast<ISP_DIP_X_PGN_OFST_1_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_OFST_1))->PGN_OFST_GB = OFFS;

    // CAM_PGN_OFST_2
    reinterpret_cast<ISP_DIP_X_PGN_OFST_2_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_OFST_2))->PGN_OFST_GR = OFFS;
    reinterpret_cast<ISP_DIP_X_PGN_OFST_2_T*>(REG_INFO_VALUE_PTR(DIP_X_PGN_OFST_2))->PGN_OFST_R = OFFS;
...
{
    PUT_REG_INFO(DIP_X_PGN_SATU_1, satu_1);
    PUT_REG_INFO(DIP_X_PGN_SATU_2, satu_2);
    PUT_REG_INFO(DIP_X_PGN_GAIN_1, gain_1);
    PUT_REG_INFO(DIP_X_PGN_GAIN_2, gain_2);
    PUT_REG_INFO(DIP_X_PGN_OFST_1, ofst_1);
    PUT_REG_INFO(DIP_X_PGN_OFST_2, ofst_2);
...
{
    GET_REG_INFO(DIP_X_PGN_SATU_1, satu_1);
    GET_REG_INFO(DIP_X_PGN_SATU_2, satu_2);
    GET_REG_INFO(DIP_X_PGN_GAIN_1, gain_1);
    GET_REG_INFO(DIP_X_PGN_GAIN_2, gain_2);
    GET_REG_INFO(DIP_X_PGN_OFST_1, ofst_1);
    GET_REG_INFO(DIP_X_PGN_OFST_2, ofst_2);
...
MBOOL
ISP_MGR_PGN_T::
get(ISP_NVRAM_PGN_T& rParam, const dip_x_reg_t* pReg) const
{
    if (pReg)
    {
...
apply(const RAWIspCamInfo& /*rRawIspCamInfo*/, dip_x_reg_t* pReg)
{
    ...
    CAM_LOGD_IF(IsDebugEnabled(),"%s(): isEnable() = %d\n", __FUNCTION__, fgOnOff);

    // TOP
    ISP_WRITE_ENABLE_BITS(pReg, DIP_X_CTL_RGB_EN, PGN_EN, fgOnOff);
    //ISP_MGR_CTL_EN_P2_T::getInstance(m_eSensorDev).setEnable_PGN(fgOnOff);
