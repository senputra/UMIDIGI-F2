    ...
    , m_u8TimeEnque(0)
{
    ...
    CAM_LOGI("[%s] IsCCUSupport(%d)", __FUNCTION__, m_bIsCCUSupport);
    ...
    CAM_LOGI("[%s] SensorDev(%d) -", __FUNCTION__, m_i4SensorDev);
}
...
dequeueHwBuf()
{
    CAM_LOGI_IF(m_bDebugEnable,"[%s] m_i4SensorDev(%d) + \n", __FUNCTION__, m_i4SensorDev);
    ...
    if (m_pICcuCtrlAf != NULL)
    {
        ...
        CAM_TRACE_END();
        CAM_LOGI_IF(m_bDebugEnable, "Dev(%d) deque Hw buffer from CCU. #(%d) - #(%d)\n",
...
enqueueHwBuf()
{
    ...
    if (m_pICcuCtrlAf != NULL)
    {
        ...
        CAM_LOGI_IF(m_bDebugEnable, "enque Hw buffer back CCU.\n");
...
dequeueSwBuf()
{
    ...
    if (m_pICcuCtrlAf != NULL)
    {
        if (m_pICcuCtrlAf->afo_done_valid == MTRUE)
        {
            ...
            CAM_LOGI_IF(m_bDebugEnable, "port(CCU), va[0x%p]/#(%d), Size(%d), Stride(%d), timeStamp(%" PRId64 ")",
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        CAM_LOGI_IF(m_bDebugEnable, "Get HW Buffer Idx : %d (%d)\n", QBufIdx, DeQBufIdx);
        int size = m_HW_QBuf[QBufIdx].mvOut.size();
        BufInfo rLastBuf = m_HW_QBuf[QBufIdx].mvOut.at( size-1 );

        MUINT64 timestamp = (MUINT64)rLastBuf.mMetaData.mTimeStamp;
        MCUDrv::getInstance(m_i4SensorDev)->ctrlMCUParam(MCU_CMD_SET_SOF_TS, &timestamp);
        // MCUDrv::getInstance(m_i4SensorDev)->ctrlMCUParam(MCU_CMD_SET_AFO_TS, &m_u8TimeDequeDone);
        ...
        CAM_LOGI_IF(m_bDebugEnable, "port(%d), va[0x%p]/pa[0x%p]/#(%d), Size(%d), timeStamp(%" PRId64 ")",
    ...
    if (m_bDebugEnable)
    {
        ...
        if (property_get_int32("vendor.afo.dump.enable", 0)) {
            ...
            {
                CAM_LOGI_IF(m_bDebugEnable,"%s\n", fileName);
            }
        }

        // dump afo hw data for ut
        if(property_get_int32("vendor.afohw.dump.enable", 0)){
            char fileName[64];
            char folderName[64];
            MINT32 slen;
            ...
            if(dumpHwBuf(fileName, folderName, m_rHwBuf, MTRUE))
            {
                CAM_LOGI_IF(m_bDebugEnable,"%s\n", fileName);
            }
        }
...
waitDequeue()
{
    ...
    if ((m_u8TimeEnque - m_u8TimeDequeDone > TIME_25MS) && (m_u8TimeEnque > m_u8TimeDequeDone))
    {
        ...
        CAM_LOGI("[%s] DequeCnt = %d, FrameDuration = %lld", __FUNCTION__, DequeCnt, rSensorInfo.u8FrameDuration);
        CAM_LOGI("[%s] TimeEnque = %lld, TimeDequeDone = %lld", __FUNCTION__, m_u8TimeEnque, m_u8TimeDequeDone);
        ...
        for (i = 0; i < DequeCnt; i++)
        {
            ...
            CAM_TRACE_END();
            int size = m_HW_QBuf[QBufIdx].mvOut.size();
            if (m_bAbort || (size == 0 && ret == MTRUE) || (ret == MFALSE))
            {
                CAM_LOGE("[%s] m_pSttPipe deque fail", __FUNCTION__);
                return -1;
            }
            CAM_LOGI_IF(m_bDebugEnable, "Deque HW Buffer Idx : %d (%d)\n", QBufIdx, m_HW_DeQBufIdx);
            m_HW_DeQBufIdx++;
            ...
            for (MINT32 j = 0; j < P1_HW_BUF_MAX_NUM; j++)
            {
                ...
                CAM_LOGI_IF(m_bDebugEnable, "Enque HW Buffer Idx : %d (%d)\n", QBufIdx, m_HW_EnQBufIdx);
