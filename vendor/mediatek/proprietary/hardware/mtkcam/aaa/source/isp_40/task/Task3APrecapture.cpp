...
Task3APrecaptureImp::run(MINT32 i4Opt, const TaskData& rData)
{
    ...
    if(!(i4Opt & TASK_OPTION_BYP_AE))
    {
        ...
        else
        {
            // Normal AE
            if(pAAOBuffer)
                IAeMgr::getInstance().doPreCapAE(m_i4SensorDev, pAAOBuffer->mTimeStamp, 0, reinterpret_cast<MVOID *>(pAAOBuffer->getPart(AAOSepBuf)), i4ActiveAEItem, pAAOBuffer->mMagicNumber, bIsScheduleBy3A);
            else
                CAM_LOGW("[%s] MvHDR or PSO buffer is NULL(%p, %p)", __FUNCTION__, pMvHdrBuffer, pPSOBuffer);
        }
        m_rTimer.End();
