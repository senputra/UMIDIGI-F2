...
int FlashPlatformAdapter::setAeCCUOnOff(int enable)
{
    if(enable == 0)
    {
        mCCUWorkingStatus = IAeMgr::getInstance().checkCCUWorkingStatus(mSensorDev);
        IAeMgr::getInstance().setCCUOnOff(mSensorDev, enable);
        logI("setAeCCUOnOff(): set AE CCU (%d)=>(%d).", mCCUWorkingStatus, enable);
    }
    else if(enable == 1)
    {
        IAeMgr::getInstance().setCCUOnOff(mSensorDev, mCCUWorkingStatus);
        logI("setAeCCUOnOff(): set AE CCU (0)=>(%d).", mCCUWorkingStatus);
    }
    logI("setAeCCUOnOff(): mCCUWorkingStatus(%d), enable(%d).", mCCUWorkingStatus, enable);
    return 0;
}
