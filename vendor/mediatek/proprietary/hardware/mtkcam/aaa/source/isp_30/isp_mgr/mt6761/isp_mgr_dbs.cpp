...
#define MY_INST_DBS NS3Av3::INST_T<ISP_MGR_DBS_T>
static std::array<std::array<MY_INST_DBS, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonDBS;

#define MY_INST_DBS2 NS3Av3::INST_T<ISP_MGR_DBS2_T>
static std::array<std::array<MY_INST_DBS2, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonDBS2;
...
getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    if(i4SensorIdx >= SENSOR_IDX_MAX || i4SensorIdx < 0) {
        MY_LOG("[%s] Unsupport sensor Index: %d\n", __FUNCTION__, i4SensorIdx, eSensorTG);
    }

    MY_INST_DBS& rSingleton = gMultitonDBS[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_DBS_T>(eSensorDev, eSensorTG);
    } );
...
getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    MY_INST_DBS2& rSingleton = gMultitonDBS2[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_DBS2_T>(eSensorDev, eSensorTG);
    } );
