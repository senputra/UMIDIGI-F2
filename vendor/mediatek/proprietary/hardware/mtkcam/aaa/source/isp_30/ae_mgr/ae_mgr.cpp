...
, m_bEnableTSFConfig(AE_TSF_ENABLE)
...
MRESULT AeMgr::PreviewAEInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    #endif
    AE_LOG("[%s()] m_eSensorDev:%d m_i4SensorIdx:%d AEconfig:%d/%d/%d/%d/%d/%d AE_PERFRAME_EN:%d m_bPerframeAEFlag:%d\n", __FUNCTION__,
        ...
        AE_PERFRAME_EN, m_bPerframeAEFlag);
...
MRESULT AeMgr::Start()
{
    ...
    AE_LOG("[%s] m_eSensorDev:%d m_eSensorTG:%d m_i4SensorIdx:%d m_eISPProfile:%d\n", __FUNCTION__, m_eSensorDev, m_eSensorTG, m_i4SensorIdx, m_eISPProfile);
    ...
    err = ISP_MGR_AE_STAT_CONFIG_T::getInstance(m_eSensorDev, m_eSensorTG).setAEconfigParam(m_bEnableTSFConfig);
...
MRESULT AeMgr::uninit()
{
    ...
    AE_LOG("[%s] reset m_eAEScene from %d to LIB3A_AE_SCENE_AUTO\n", __FUNCTION__, m_eAEScene);
    m_eAEScene = LIB3A_AE_SCENE_AUTO;
    AE_LOG("[%s] reset m_bAFAELock from %d to 0\n", __FUNCTION__, m_bAFAELock); // avoid AFAELock not being reset when doing EVC
    setAFAELock(MFALSE);
    return S_AE_OK;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
MRESULT AeMgr::setAEEVCompIndex(MINT32 i4NewEVIndex, MFLOAT fStep)
{
    ...
    if (m_i4EVIndex != i4NewEVIndex)
    {
        ...
        AE_LOG("[%s()] m_i4EVIndex: %d EVComp:%d fEVCompStep:%f PreAEState:%d m_bAELock:%d\n", __FUNCTION__, m_i4EVIndex, m_eAEEVcomp, m_fEVCompStep, m_ePreAEState, m_bAELock);
        if (m_pIAeAlgo != NULL)
        {
            // AE Auto Test
...
MRESULT AeMgr::setCamScenarioMode(MUINT32 u4NewCamScenarioMode, MBOOL bInit)
{
    ...
    if (m_eCamScenarioMode != eNewCamScenarioMode || bInit)
    {
        ...
        AE_LOG("[%s()] m_eSensorDev:%d m_i4SensorIdx:%d AE max block width:%d heigh:%d AAO separation size(To AeAlgo):%d/%d/%d/%d AAOLineByte:%d AEconfig:%d/%d/%d/%d/%d/%d AE_PERFRAME_EN:%d m_bPerframeAEFlag:%d\n",
            ...
            m_u4BlockNumX, m_u4BlockNumY, m_bEnableHDRYConfig, m_bEnableOverCntConfig, m_bEnableTSFConfig, m_bDisablePixelHistConfig,
            AE_PERFRAME_EN, m_bPerframeAEFlag);
...
MBOOL AeMgr::updateAEScenarioMode(EIspProfile_T eIspProfile)
{
    if (m_eISPProfile != eIspProfile)
    {
        ...
        else
        {
            ...
            AE_LOG("[%s()-setEVIdxInfo] FinerEVIdxBase: %d MaxIdx/MinIdx/Idx/MaxIdxF/MinIdxF/IdxF:%d/%d/%d/%d/%d/%d\n", __FUNCTION__, m_u4FinerEVIdxBase, m_u4IndexMax, m_u4IndexMin, m_u4Index, m_u4IndexFMax, m_u4IndexFMin, m_u4IndexF);
            m_pIAeAlgo->setEVIdxInfo(m_u4IndexMax , m_u4IndexMin , m_u4Index);
...
MRESULT AeMgr::setAPAELock(MBOOL bAPAELock)
{
    if (m_bAPAELock != bAPAELock)
    {
        ...
        else
        {
            ...
            m_eAEState = MTK_CONTROL_AE_STATE_LOCKED;
            // avoid being too late to prepare capture parameters after lock
...
{
    if ((u4XOffset <= 8192) && (u4YOffset <= 8192) && (u4Width <= 16384) && (u4Height <= 16384))
    {
...
MRESULT AeMgr::doPvAE(MINT64 i8TimeStamp, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule)
{
    ...
    if (m_pIAeAlgo != NULL)
    {
        m_pIAeTest->monitorAvgY(m_pIAeAlgo->getBrightnessAverageValue());
    }
    else
    {
        m_pIAeTest->monitorAvgY(0);
    }
    ...
    MBOOL bSkipAEAlgo = ((i4ActiveAEItem & E_AE_DUMMY) != 0) ? MTRUE : MFALSE;
    AE_LOG_IF(bSkipAEAlgo, "[%s()] skip AE algo in dummy frame\n", __FUNCTION__);
    strAEOutput rAEOutput;
    ...
    else if (m_bEnableAE && !bSkipAEAlgo)
    {
        if (bCalculateAE)
        {
            // Calculte AE
    ...
    m_bAECaptureUpdate = MFALSE; // reset have been Updated Capture param flag, need to be true when preview 1st frame update2ISP(AF)
    return S_AE_OK;
}
...
MRESULT AeMgr::doAFAE(MINT64 i8TimeStamp, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule)
{
    ...
    AE_LOG("[%s()]:%d %d i8TimeStamp:%lld ActiveAE:%d eAETargetMode:%d bTouchAEAreaChage:%d\n", __FUNCTION__, m_i4TimeOutCnt, m_i4WaitVDNum, (long long)i8TimeStamp, i4ActiveAEItem, m_eAETargetMode, m_bTouchAEAreaChage);
...
MRESULT AeMgr::doCapAE()
{
    ...
    AE_LOG("[%s()] i4SensorDev:%d CamMode:%d TargetMode:%d Shutter:%d ISO:%d\n", __FUNCTION__, m_eSensorDev, m_eCamMode, m_eAETargetMode, mCaptureMode.u4Eposuretime, mCaptureMode.u4RealISO);

    // 4 - cell for flash capture
    if (m_bStrobeOn)
    {
        AE_LOG("[%s()] Strobe Mode On, ModifyCaptureParamsBySensorMode, new_mode(%d)\n", __FUNCTION__, m_eSensorMode);
        ModifyCaptureParamsBySensorMode(m_eSensorMode, ESensorMode_Capture);
    }

    if ((m_bIsAutoFlare == FALSE) && (m_eSensorMode == ESensorMode_Capture) && m_bEnableAE)
    {
        AE_FLOW_CALCULATE_INFO_T inputinfo;
    ...
    #endif
    {
        ...
        {
            AE_EXP_INFO rAECoreCtrlCPUInfo;
            memset(&rAECoreCtrlCPUInfo, 0, sizeof(AE_EXP_INFO));
...
void AeMgr::setPfPara(int exp, int afe, int isp)
{
    ...
    rAEInfo2ISP.bAELimiter = m_bAElimitor;
    rAEInfo2ISP.u4EISExpRatio = m_u4EISExpRatio;
    rAEInfo2ISP.bDisableOBC = MFALSE;
...
MRESULT AeMgr::doRestoreAEInfo(MBOOL bRestorePrvOnly, MBOOL bTorchMode)
{
    ...
    if (m_bEnableAE)
    {
        ...
        AE_LOG("[%s()] Preview Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d flare offset/gain:%d %d\n", __FUNCTION__,
            m_rAEOutput.rPreviewMode.u4Eposuretime, m_rAEOutput.rPreviewMode.u4AfeGain, m_rAEOutput.rPreviewMode.u4IspGain,
            m_rAEOutput.rPreviewMode.u4RealISO, m_rAEOutput.rPreviewMode.i2FlareOffset, m_rAEOutput.rPreviewMode.i2FlareGain);
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    m_rAEInitInput.eAETargetMode = m_eAETargetMode;
    m_rAEInitInput.bTSFEnable = AE_TSF_ENABLE;
    AE_LOG("[%s()] i4SensorDev:%d SensorIdx:%d AE meter:%d CamMode:%d AEScene:%d Flicker :%d %d EVcomp:%d ISO:%d %d MinFps:%d MaxFps:%d Limiter:%d TSFEnable:%d\n", __FUNCTION__,
        ...
        m_i4AEMinFps, m_i4AEMaxFps, m_bAElimitor, AE_TSF_ENABLE);
...
MRESULT AeMgr::prepareCapParams(MBOOL bRemosaicEn, MBOOL IsEVChange)
{
    ...
    if(m_bAPAELock && (!IsEVChange)){
        ...
        MUINT32 exp_ratio = m_uBinning_Exp_Ratio;

        if (bRemosaicEn && exp_ratio > 1 && m_u4AEISOSpeed == LIB3A_AE_ISO_SPEED_AUTO)
        {
        ...
        AE_LOG("[%s()] Output Capture Params.: Shutter:%d, Sensor gain:%d, ISP gain:%d, ISO:%d\n",
                __FUNCTION__,
                m_rAEOutput.rCaptureMode[0].u4Eposuretime,
                m_rAEOutput.rCaptureMode[0].u4AfeGain,
                m_rAEOutput.rCaptureMode[0].u4IspGain,
                m_rAEOutput.rCaptureMode[0].u4RealISO);
...
MRESULT AeMgr::getPreviewParams(AE_MODE_CFG_T &a_rPreviewInfo)
{
    ...
    else
    {
        ...
        AE_LOG_IF(m_3ALogEnable, "[%s()] m_pIAeAlgo is NULL\n", __FUNCTION__);
    }
    a_rPreviewInfo = m_rAEOutput.rPreviewMode;
    if (a_rPreviewInfo.u2FrameRate > m_i4AEMaxFps)
    {
        AE_LOG_IF(m_3ALogEnable, "[%s()] Frame rate too large:%d->%d\n", __FUNCTION__, a_rPreviewInfo.u2FrameRate, m_i4AEMaxFps);
        a_rPreviewInfo.u2FrameRate = m_i4AEMaxFps;
    }
    AE_LOG_IF(m_3ALogEnable, "[%s()] SensorDev:%d Preview Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d flare offset/gain:%d/%d\n", __FUNCTION__, m_eSensorDev,
        a_rPreviewInfo.u4Eposuretime, a_rPreviewInfo.u4AfeGain, a_rPreviewInfo.u4IspGain,
        a_rPreviewInfo.u4RealISO, a_rPreviewInfo.i2FlareOffset, a_rPreviewInfo.i2FlareGain);
    return S_AE_OK;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
...
MRESULT AeMgr::getExposureInfo(ExpSettingParam_T &strHDRInputSetting)
{
    ...
    if (m_bRealISOSpeed == MFALSE)
    {
        ...
        if (u4Newtableindex < strCurrentCaptureTable.u4TotalIndex)
        {
            ...
            AE_LOG("[%s()] m_i4SensorDev:%d Modify Capture Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d Index:%d tablediffindex:%d\n", __FUNCTION__, m_eSensorDev,
                m_rAEOutput.rCaptureMode[0].u4Eposuretime,m_rAEOutput.rCaptureMode[0].u4AfeGain, m_rAEOutput.rCaptureMode[0].u4IspGain,
                m_rAEOutput.rCaptureMode[0].u2FrameRate, m_rAEOutput.rCaptureMode[0].i2FlareGain,
                m_rAEOutput.rCaptureMode[0].i2FlareOffset, m_rAEOutput.rCaptureMode[0].u4RealISO,
                m_i4AEidxNext, i4TableDeltaIdx);
        }
        else
        {
    ...
    if (m_pIAeAlgo != NULL)
    {
        ...
        // m_pIAeAlgo->getAESWHdrInfo(rSWHdrInfo); // temp for build pass
    ...
    for (i = 0; i < AE_HISTOGRAM_BIN; i += 16)
    {
        AE_LOG("[%s()] Input Histogram%3d~%3d:%4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d\n", __FUNCTION__, i, i + 15,
            strHDRInputSetting.u4Histogram[i], strHDRInputSetting.u4Histogram[i + 1], strHDRInputSetting.u4Histogram[i + 2], strHDRInputSetting.u4Histogram[i + 3],
            strHDRInputSetting.u4Histogram[i + 4], strHDRInputSetting.u4Histogram[i + 5], strHDRInputSetting.u4Histogram[i + 6], strHDRInputSetting.u4Histogram[i + 7],
            strHDRInputSetting.u4Histogram[i + 8], strHDRInputSetting.u4Histogram[i + 9], strHDRInputSetting.u4Histogram[i + 10], strHDRInputSetting.u4Histogram[i + 11],
            strHDRInputSetting.u4Histogram[i + 12], strHDRInputSetting.u4Histogram[i + 13], strHDRInputSetting.u4Histogram[i + 14], strHDRInputSetting.u4Histogram[i + 15]);
    }
...
MRESULT AeMgr::getCaptureParams(AE_MODE_CFG_T &a_rCaptureInfo)
{
    ...
    AE_LOG("[%s()] m_i4SensorDev:%d Capture Exp mode:%d Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d flare offset/gain:%d/%d\n", __FUNCTION__, m_eSensorDev,
        a_rCaptureInfo.u4ExposureMode, a_rCaptureInfo.u4Eposuretime, a_rCaptureInfo.u4AfeGain, a_rCaptureInfo.u4IspGain,
        a_rCaptureInfo.u4RealISO, a_rCaptureInfo.i2FlareOffset, a_rCaptureInfo.i2FlareGain);
    return S_AE_OK;
}
...
MRESULT AeMgr::updateCaptureParams(AE_MODE_CFG_T &a_rCaptureInfo, MBOOL bRemosaicEn)
{
    ...
    AE_LOG("[%s()] m_i4SensorDev:%d Exp mode:%d Capture Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d flare offset/gain:%d/%d\n", __FUNCTION__, m_eSensorDev,
        mCaptureMode.u4ExposureMode, mCaptureMode.u4Eposuretime, mCaptureMode.u4AfeGain, mCaptureMode.u4IspGain,
        mCaptureMode.u4RealISO, mCaptureMode.i2FlareOffset, mCaptureMode.i2FlareGain);
    ...
    if (m_bEnableAE)
    {
        if (bRemosaicEn)
        {
            ...
            if (m_p3ANVRAM != NULL)
            {
                ...
                ModifyCaptureParamByCustom(exp_ratio, &mCaptureMode, &rCaptureMode, (MUINT32)m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MaxGain);
                memcpy(&mCaptureMode, &rCaptureMode, sizeof(AE_MODE_CFG_T));
                AE_LOG("[%s()] enable remosaic binning transform, m_i4SensorDev:%d exp_ratio:%d Capture Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d\n", __FUNCTION__, m_eSensorDev,
...
MRESULT AeMgr::setSensorDirectly(CaptureParam_T &a_rCaptureInfo, MBOOL bDirectlySetting)
{
    ...
    {
        #if defined(VHDR)
        if ((m_eCamMode == eAppMode_EngMode) && ((m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET) || (m_eAETargetMode == AE_MODE_ZVHDR_TARGET)))
        {
            // engineer mode and target mode
            UpdateSensorISPParams(AE_TWO_SHUTTER_INIT_STATE);
        }
...
MRESULT AeMgr::UpdateSensorISPParams(AE_STATE_T eNewAEState)
{
    ...
    switch (eNewAEState)
    {
        ...
        case AE_AF_STATE:
            ...
            m_i4WaitVDNum++;
            AE_LOG("[doAFAE] AF Mode SensorDev:%d Shutter:%-5d Sensor gain:%-5d Isp gain:%-4d ISO:%-4d flare offset/gain:%d/%d m_i4WaitVDNum:%d\n", m_eSensorDev,
                ...
                m_rAEOutput.rAFMode.i2FlareOffset, m_rAEOutput.rAFMode.i2FlareGain, m_i4WaitVDNum);
            AE_LOG("[doAFAE] Capture SensorDev:%d Shutter:%-5d Sensor gain:%-5d Isp gain:%-4d ISO:%-4d flare offset/gain:%d/%d\n", m_eSensorDev,
                ...
                m_rAEOutput.rCaptureMode[0].i2FlareOffset, m_rAEOutput.rCaptureMode[0].i2FlareGain);
            break;
        case AE_PRE_CAPTURE_STATE:
            ...
            AE_LOG("[doPreCapAE] Preview Exp mode:%d Shutter:%-5d Sensor gain:%-5d Isp gain:%-4d ISO:%-4d flare offset/gain:%d/%d\n",
                ...
                m_rAEOutput.rPreviewMode.u4RealISO, m_rAEOutput.rPreviewMode.i2FlareOffset, m_rAEOutput.rPreviewMode.i2FlareGain);
            AE_LOG("[doPreCapAE] AF Mode Exp mode:%d Shutter:%-5d Sensor gain:%-5d Isp gain:%-4d ISO:%-4d flare offset/gain:%d/%d\n",
                ...
                m_rAEOutput.rAFMode.u4RealISO, m_rAEOutput.rAFMode.i2FlareOffset, m_rAEOutput.rAFMode.i2FlareGain);
            AE_LOG("[doPreCapAE] Capture Exp mode:%d Shutter:%-5d Sensor gain:%-5d Isp gain:%-4d ISO:%-4d flare offset/gain:%d/%d\n",
                ...
                m_rAEOutput.rCaptureMode[0].u4RealISO, m_rAEOutput.rCaptureMode[0].i2FlareOffset, m_rAEOutput.rCaptureMode[0].i2FlareGain);
            updateAEInfo2ISP(AE_AUTO_FRAMERATE_STATE, m_rAEOutput.rPreviewMode.u4IspGain);
            break;
        case AE_CAPTURE_STATE:
            ...
            AE_LOG("[doCapAE] SensorDev:%d Exp Mode:%d Capture Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d flare offset/gain:%d/%d\n", m_eSensorDev, mCaptureMode.u4ExposureMode, mCaptureMode.u4Eposuretime,
                mCaptureMode.u4AfeGain, mCaptureMode.u4IspGain, mCaptureMode.u4RealISO, mCaptureMode.i2FlareOffset, mCaptureMode.i2FlareGain);

            // Update to isp tuning
            updateAEInfo2ISP(AE_CAPTURE_STATE, mCaptureMode.u4IspGain);
            m_i4WaitVDNum = 0; // need to reset when doing CShot (if not, may only ISP gain update)
            break;
        case AE_POST_CAPTURE_STATE:
            AE_LOG("[doPostCapAE] SensorDev:%d Capture Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d FrameRate:%d flare offset/gain:%d/%d\n", m_eSensorDev, mCaptureMode.u4Eposuretime,
                mCaptureMode.u4AfeGain, mCaptureMode.u4IspGain, mCaptureMode.u4RealISO, mCaptureMode.u2FrameRate, mCaptureMode.i2FlareOffset, mCaptureMode.i2FlareGain);
...
MRESULT AeMgr::ModifyCaptureParamsBySensorMode(MINT32 i4newSensorMode, MINT32 i4oldSensorMode)
{
    ...
    switch (i4newSensorMode)
    {
        ...
        case ESensorMode_Custom1:
            u4NewSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom12PreRatio;
            break;
        case ESensorMode_Custom2:
            u4NewSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom22PreRatio;
            break;
        case ESensorMode_Custom3:
            u4NewSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom32PreRatio;
            break;
        case ESensorMode_Custom4:
            u4NewSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom42PreRatio;
            break;
        case ESensorMode_Custom5:
            u4NewSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom52PreRatio;
            break;
    ...
    switch (i4oldSensorMode)
    {
        ...
        case ESensorMode_SlimVideo2:
            ...
            break;
        case ESensorMode_Custom1:
            u4OldSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom12PreRatio;
            break;
        case ESensorMode_Custom2:
            u4OldSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom22PreRatio;
            break;
        case ESensorMode_Custom3:
            u4OldSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom32PreRatio;
            break;
        case ESensorMode_Custom4:
            u4OldSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom42PreRatio;
            break;
        case ESensorMode_Custom5:
            u4OldSensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Custom52PreRatio;
            break;
        case ESensorMode_Preview:
        default:
    ...
    if (u4OldSensitivityRatio != 0)
    {
        ...
        m_uBinning_Exp_Ratio =  u4OldSensitivityRatio / u4NewSensitivityRatio;
        m_uBinning_Exp_Ratio =  m_uBinning_Exp_Ratio < 1 ? 1:m_uBinning_Exp_Ratio;
    }
    else
    {
        u4SensitivityRatio = 1024;
    ...
    AE_LOG("[%s()] i4SensorDev:%d SensorMode(old/new):(%d,%d) SensitivityRatio(old/new):(%d,%d) Binning_Exp_Ratio:%d u4BinningRatio=%d\n",
           __FUNCTION__, m_eSensorDev,i4oldSensorMode,i4newSensorMode,u4OldSensitivityRatio,u4NewSensitivityRatio,m_uBinning_Exp_Ratio,u4BinningRatio);

    if (i4newSensorMode != ESensorMode_Capture) // exit remosaic
    {
        ...
        AE_LOG("[%s] i4SensorDev:%d New Capture Exp/Afe/Isp:%d/%d/%d Binning:%d Flare offset/gain:%d/%d\n", __FUNCTION__, m_eSensorDev,
            mCaptureMode.u4Eposuretime, mCaptureMode.u4AfeGain, mCaptureMode.u4IspGain, u4BinningRatio,
...
MRESULT AeMgr::updateAEInfo2ISP(AE_STATE_T eNewAEState, MUINT32 u4ISPGain)
{
    ...
    rAEInfo2ISP.bDisableOBC = MFALSE;
    rAEInfo2ISP.u4EISExpRatio = m_u4EISExpRatio;
    rAEInfo2ISP.bAEScenarioChange = MFALSE;
...
MINT32 AeMgr::getAEInitExpoSetting(AEInitExpoSetting_T &a_rAEInitExpoSetting)
{
    ...
    if ((m_eAETargetMode == AE_MODE_IVHDR_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET) || (m_eAETargetMode == AE_MODE_ZVHDR_TARGET) || m_bVHDRChangeFlag)
    {
        ...
        m_rAEInitInput.rAEPARAM.strAEParasetting.bEnablePreIndex = MTRUE;
        m_pIAeAlgo->initAE(&m_rAEInitInput, &rAEOutput, &m_rAEStatCfg);
        // resetZVHDRFlag(); // temp for build pass
        copyAEInfo2mgr(&m_rAEOutput.rPreviewMode, &rAEOutput);
        if ((m_eAETargetMode != AE_MODE_IVHDR_TARGET) && (m_eAETargetMode != AE_MODE_MVHDR_TARGET) && (m_eAETargetMode != AE_MODE_ZVHDR_TARGET))
        {
...
MRESULT AeMgr::switchExpSettingByShutterISOpriority(AE_EXP_SETTING_T &a_strExpInput, AE_EXP_SETTING_T &a_strExpOutput)
{
    if ((a_strExpInput.u4Sensitivity == 0) && (a_strExpInput.u4ExposureTime != 0))
    {
        ...
        if ((MUINT32)a_strExpOutput.u4Sensitivity < m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MiniISOGain)
        {
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::updateCustomPline()
{
    if ((m_i4AEMinFps == LIB3A_AE_FRAMERATE_MODE_30FPS) && (m_i4AEMaxFps == LIB3A_AE_FRAMERATE_MODE_30FPS)){
        m_bManualAEtable = MTRUE;
        m_u4ManualAEtablePreviewidx = AE_PLINE_CUSTOM_30FPS;
        m_u4ManualAEtableCaptureidx = AE_PLINE_CUSTOM_30FPS;
    } else {
        m_u4ManualAEtablePreviewidx = 0 ;
        m_u4ManualAEtableCaptureidx = 1 ;
        m_bManualAEtable = MFALSE;
    }
    AE_LOG( "[%s()] i4SensorDev:%d m_i4AEMinFps/m_i4AEMaxFps: %d/%d m_u4ManualAEtablePreviewidx/m_u4ManualAEtableCaptureidx:%d/%d m_bManualAEtable:%d \n", __FUNCTION__, m_eSensorDev, m_i4AEMinFps ,m_i4AEMaxFps, m_u4ManualAEtablePreviewidx, m_u4ManualAEtableCaptureidx, m_bManualAEtable);

    if(m_pIAeAlgo != NULL) {
        m_pIAeAlgo->getEVIdxInfo(m_u4IndexMax ,m_u4IndexMin ,m_u4Index);
        AE_LOG("[%s()-getEVIdxInfo] MaxIdx/MinIdx/Idx/MaxIdxF/MinIdxF/IdxF :%d/%d/%d/%d/%d/%d \n", __FUNCTION__, m_u4FinerEVIdxBase, m_u4IndexMax, m_u4IndexMin, m_u4Index, m_u4IndexFMax, m_u4IndexFMin, m_u4IndexF);
        setAEScene(m_eAEScene);
        setIsoSpeed(m_u4AEISOSpeed);
        setAEMinMaxFrameRatePlineIdx(m_i4AEMinFps ,m_i4AEMaxFps);
        searchPreviewIndexLimit();
