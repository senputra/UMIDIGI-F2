...
setIspProfile(EIspProfile_T const eIspProfile)
{
    ...
    switch(m_rIspCamInfo.eIspProfile) {
        ...
        case EIspProfile_PureRAW_TPipe_Capture_SWNR:
            if (m_eSensorDev == ESensorDev_Main)
                eSoftwareScenario = eSoftwareScenario_Main_IP_Raw_TPipe_Stream;
            else if (m_eSensorDev == ESensorDev_MainSecond)
                eSoftwareScenario = eSoftwareScenario_Main2_IP_Raw_TPipe_Stream;
            else
                eSoftwareScenario = eSoftwareScenario_Sub_IP_Raw_TPipe_Stream;
            break;
        // N3D
...
setAWBInfo(AWB_INFO_T const &rAWBInfo)
{
   ...
   if (checkParamChange(m_rIspCamInfo.rAWBInfo.rCurrentAWBGain.i4R, rAWBInfo.rCurrentAWBGain.i4R) ||
    ...
    if (checkParamChange(m_rIspCamInfo.eIdx_CT, eIdx_CT)) {
        MY_LOG_IF(m_bDebugEnable, "[+m_rIspCamInfo.eIdx_CT](old, new)=(%d, %d)", m_rIspCamInfo.eIdx_CT, eIdx_CT);
    ...
    if (isDynamicCCM())
    {
        if (m_rIspParam.ISPMulitCCM.CCM_Method) { // smooth CCM
            MBOOL bInvokeSmoothCCMwPrefGain = m_pIspTuningCustom->is_to_invoke_smooth_ccm_with_preference_gain(m_rIspCamInfo);
            m_pCcmMgr->calculateCCM(m_rIspCamInfo.rFlashInfo.isFlash, bInvokeSmoothCCMwPrefGain, 0, m_rIspCamInfo);
            MY_LOG_IF(m_bDebugEnable, "m_rIspCamInfo.rFlashInfo.isFlash(%d)", m_rIspCamInfo.rFlashInfo.isFlash);
        }
        else { // dynamic CCM
            ...
            m_ISP_DYM.sCcm_tbl_P1_Idx = eIdx_CCM_new;
...
map_CT_value2index(MUINT32 const u4CT) const
{
    ...
    MUINT32 t_u4CT = u4CT;
    MUINT32 t_ceiling =m_rIspParam.ISPPca.COLOR_CT_Env[t_high];
    ...
    while ( (t_low <= t_high) && (t_high > 1) )
    {
        ...
        MUINT32 t_u4mid = m_rIspParam.ISPPca.COLOR_CT_Env[t_mid];

        if ( (t_u4CT < t_u4mid) && (t_u4CT >= (MUINT32)m_rIspParam.ISPPca.COLOR_CT_Env[t_mid-1]) )
        {
    ...
    if (t_high < 2){
        if ( (t_u4CT < (MUINT32)m_rIspParam.ISPPca.COLOR_CT_Env[t_high]) && (t_u4CT >= (MUINT32)m_rIspParam.ISPPca.COLOR_CT_Env[t_low])){
...
{
      if (u4CT >= (MUINT32)m_rIspParam.ISPPca.COLOR_CT_Env[ISP_NVRAM_COLOR_CT_NUM-1]){
...
{
    MY_LOG_IF(m_bDebugEnable, "setFlashInfo(): rFlashInfo.isFlash(%d)", rFlashInfo.isFlash);

    Mutex::Autolock lock(m_Lock);

    m_rIspCamInfo.rFlashInfo = rFlashInfo;
    MY_LOG_IF(m_bDebugEnable, "setFlashInfo(): m_rIspCamInfo.rFlashInfo.isFlash(%d)", m_rIspCamInfo.rFlashInfo.isFlash);
...
setCamMode(MINT32 const i4CamMode)
{
    if (m_i4CamMode != i4CamMode) {
        ...
        case eAppMode_EngMode:
             ...
             MBOOL bret = mkdir("/sdcard/isp", S_IRWXU | S_IRWXG | S_IRWXO);
...
{
    MY_LOG_IF(m_bDebugEnable, "[%s] (ctrl, arg1, arg2) = (%d, %x, %x)", __FUNCTION__, ctrl, (MUINT32)arg1, (MUINT32)arg2);
    ...
    case IspTuningMgr::E_ISPTUNING_UPDATE_OBC_INFO:
        ...
        break;
    default:
        MY_LOG("%s: ctrl not supported (ctrl, arg1, arg2) = (%d, %x, %x)", __FUNCTION__, ctrl, (MUINT32)arg1, (MUINT32)arg2);
        break;
    }
}
...
{
    MY_LOG_IF(m_bDebugEnable, "[%s] flkEnable = %d", __FUNCTION__, flkEnable);
