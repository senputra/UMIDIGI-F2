...
put(const ILscMgr::TSF_INPUT_INFO_T& rInStat, MUINT32 mySize)
{
    ...
    if(rInStat.prAwbStat)
    {
        ...
        MUINT8 *pSrc=vecStat.data();

        int lineSize=150*4;
        int lscSize=120*4;
        for(int i=0;i<90;i++)
        {
            ::memcpy(pSrc+i*lscSize, rInStat.prAwbStat+i*lineSize, lscSize);
        }
...
tsfCfgChg()
{
    ...
    tsfSetBufAndRatio(&m_ptrTsfRingBuf[((u4TblIdx+3) & RING_TBL_MSK)], m_rStat[((u4TblIdx+3) & RING_TBL_MSK)].u4Ratio);
    m_u4TblIdx = ((m_u4TblIdx+1) & RING_TBL_MSK);
    CAM_LOGD_IF(fgLogEn, "setBuf with m_ptrTsfRingBuf((u4TblIdx+3) & RING_TBL_MSK)(%d)", ((u4TblIdx+3) & RING_TBL_MSK));
...
update(const ILscMgr::TSF_INPUT_INFO_T& rInputInfo)
{
    ...
    if (m_bTSFInit && m_bTSF)
    {
        ...
        if(fgLogEn) {
            ...
            for(int i=0;i<4;i++)
            {
                CAM_LOGD_IF(fgLogEn, "i: %d, %p, %d", i, m_ptrTsfRingBuf[i], ((MINT32*)(m_ptrTsfRingBuf[i]->getTable()))[3]);
            }
        ...
        else
        {
            ...
            if (rInputInfo.eCmd!=ILscMgr::E_TSF_CMD_KEEP)
            {
                m_u4TblIdxRear=((m_u4TblIdxRear+1)&RING_TBL_MSK);
                m_ptrTsfRingBuf[(u4TblIdx+3)&RING_TBL_MSK]=m_pTsfRingBuf[m_u4TblIdxRear];
                CAM_LOGD_IF(fgLogEn, "E_TSF_CMD_KEEP: don't increase index number");
            }
            ILscBuf* rOutputBuf = m_ptrTsfRingBuf[(u4TblIdx+3)&RING_TBL_MSK];
            ...
            CAM_LOGD_IF(fgLogEn, "tsfSetBufAndRatio: getType(%d), m_u4ProcCnt(%d), m_u4TblIdx(%d), (u4TblIdx+3)&RING_TBL_MSK(%d)",
                getType(), m_u4ProcCnt, m_u4TblIdx, (u4TblIdx+3)&RING_TBL_MSK);
            m_u4ProcCnt++;
            AAA_TRACE_END_D;
        }
