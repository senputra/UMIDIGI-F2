...
public:
    ...
    virtual ESoftwareScenario scenarioMapping(EIspProfile_T eIspProfile);
    virtual MVOID       configUpdate(MUINT32 u4TgInfo, MINT32 i4MagicNum);
...
public:  //    Ctor/Dtor.
                        Hal3AYuvImp(MINT32 const i4SensorDevId);
    virtual             ~Hal3AYuvImp(){}
...
init(MINT32 i4SensorOpenIndex)
{
    ...
    if (m_pIHalSensor == NULL)
    {
        ...
        else
        {
            ...
            if ((senInfo.TgInfo != CAM_TG_1) && (senInfo.TgInfo != CAM_TG_2) && (senInfo.TgInfo != CAM_SV_1) && (senInfo.TgInfo != CAM_SV_2))
            {
...
generateP2(MINT32 flowType, const NSIspTuning::ISP_INFO_T& rIspInfo, void* pTuningBuf, ResultP2_T* pResultP2)
{
    ...
    IspTuningMgr::getInstance().validatePerFrameP2(m_i4SensorDev, 1, rIspInfo, pTuningBuf);
...
validateP1(const ParamIspProfile_T& rParamIspProfile, MBOOL fgPerframe)
{
    ...
    MY_LOG_IF(1, "[%s] 123", __FUNCTION__);
...
using namespace NSIspio;*/
    ...
    switch (u4TgInfo)
    {
        case CAM_TG_1: u4TgInfo = NSImageio::NSIspio::EPortIndex_TG1I; break;
        case CAM_TG_2: u4TgInfo = NSImageio::NSIspio::EPortIndex_TG2I; break;
        case CAM_SV_1: u4TgInfo = NSImageio::NSIspio::EPortIndex_CAMSV_TG1I; break;
        case CAM_SV_2: u4TgInfo = NSImageio::NSIspio::EPortIndex_CAMSV_TG2I; break;
        default:
            MY_ERR("[%s] Err tg idx(%d) err, force using EPortIndex_TG1I", __FUNCTION__, u4TgInfo);
            u4TgInfo = NSImageio::NSIspio::EPortIndex_TG1I;
            break;
    }

#if 1 //ISP_P1_UPDATE
    NSCam::NSIoPipeIsp3::NSCamIOPipe::INormalPipe_FrmB* pPipe = NSCam::NSIoPipeIsp3::NSCamIOPipe::INormalPipe_FrmB::createInstance(m_i4SensorIdx, "aaa_hal_yuv");
    if ( MFALSE == pPipe->sendCommand(NSImageioIsp3::NSIspio::EPIPECmd_SET_P1_UPDATE, u4TgInfo, i4MagicNum, 0) )
    {
        MY_ERR("[%s] sensorIdx(%d) tg(%d) magic(%d)", __FUNCTION__, m_i4SensorIdx, u4TgInfo, i4MagicNum);
    }
    ...
    case ECmd_Update:
        {
            ...
            {
                ISync3A::Sync3A_Remapping_AeSetting_T rAeSetting;
                // 2A sync: remapping AE setting to YUV sensor, and update setting to sensor
                MINT32 i4Sync = ISync3AMgr::getInstance()->getSync3A()->getRemappingAeSetting(&rAeSetting);
                CAM_LOGD("[%s] Exp(%d), Afe(%d), Isp(%d)", __FUNCTION__, rAeSetting.u4Eposuretime, rAeSetting.u4AfeGain, rAeSetting.u4IspGain);
                m_pIHalSensor->sendCommand(m_i4SensorDev,SENSOR_CMD_SET_SENSOR_EXP_TIME,(MUINTPTR)&rAeSetting.u4Eposuretime,0,0);
                m_pIHalSensor->sendCommand(m_i4SensorDev,SENSOR_CMD_SET_SENSOR_GAIN,(MUINTPTR)&rAeSetting.u4AfeGain,0,0);
            }
#endif
