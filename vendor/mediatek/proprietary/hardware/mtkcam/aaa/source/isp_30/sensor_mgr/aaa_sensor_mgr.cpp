...
//    CAM_LOGD_IF(m_bDebugEnable,"[getSensorVCinfo]  \n");
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_GET_SENSOR_VC_INFO, (MUINTPTR)&rSensorVCInfo, (MUINTPTR)&i4SensorSenraio, 0);
    } else {
...
setSensorGain(MINT32 i4SensorDev, MUINT32 a_u4SensorGain)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        /*if (m_bEnableBuffMode_MainTwo)
            ...
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_GAIN, (MUINTPTR)&a_u4SensorGain, 0, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
setSensorDualGain(MINT32 i4SensorDev, MUINT32 a_u4LESensorGain, MUINT32 a_u4SESensorGain)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
            ...
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_DUAL_GAIN, (MUINTPTR)&a_u4LESensorGain, (MUINTPTR)&a_u4SESensorGain, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
setSensorFrameRate(MINT32 i4SensorDev, MUINT32 a_u4SensorFrameRate)
{
    ...
    if(u4SensorFrameRate == u4SensorFrameRateCur){
        ...
        if(i4SensorDev == ESensorDev_Main) {
            ...
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_VIDEO_FRAME_RATE, (MUINTPTR)&u4SensorFrameRate, 0, 0);
...
setSensorExpLine(MINT32 i4SensorDev, MUINT32 a_u4ExpLine)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_EXP_LINE, (MUINTPTR)&a_u4ExpLine, 0, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
setFlickerFrameRateActive(MINT32 i4SensorDev, MBOOL a_bFlickerFPSAvtive)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_FLICKER_FRAME_RATE, (MUINTPTR)&u4FlickerInfo, 0, 0);
...
setCaptureParams(MINT32 i4SensorDev, MUINT32 a_u4ExpTime, MUINT32 a_u4SensorGain)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret_shutter = setSensorExpTime(NSCam::SENSOR_DEV_MAIN_3, a_u4ExpTime);
        ret_gain = setSensorGain(NSCam::SENSOR_DEV_MAIN_3, a_u4SensorGain);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
setCaptureLineBaseParams(MINT32 i4SensorDev, MUINT32 a_u4ExpLine, MUINT32 a_u4SensorGain)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret_shutter = setSensorExpLine(NSCam::SENSOR_DEV_MAIN_3, a_u4ExpLine);
        ret_gain = setSensorGain(NSCam::SENSOR_DEV_MAIN_3, a_u4SensorGain);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
...
MRESULT AAASensorMgr::setPreviewMaxFrameRate(MINT32 i4SensorDev, MUINT32 frameRate, MUINT32 u4SensorMode)
{
    ...
    // Set max frame rate
        ...
        if(i4SensorDev == ESensorDev_Main) {
            ...
            ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_MAX_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&u4SensorMode, (MUINTPTR)&frameRate, 0);
...
MUINT32 AAASensorMgr::getPreviewDefaultFrameRate(MINT32 i4SensorDev, MUINT32 u4SensorMode)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        err = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&u4SensorMode, (MUINTPTR)&frmRate, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
set2ShutterParams(MINT32 i4SensorDev, MUINT32 a_u4LEExpTime, MUINT32 a_u4SEExpTime, MUINT32 a_u4SensorGain)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER_GAIN, (MUINTPTR)&a_u4LEExpTime, (MUINTPTR)&a_u4SEExpTime, (MUINTPTR)&a_u4SensorGain);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
set2ShutterControl(MINT32 i4SensorDev, MUINT32 a_u4LEExpTime, MUINT32 a_u4SEExpTime,MINT32 a_i4LVvalue)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_HDR_SHUTTER, (MUINTPTR)&a_u4LEExpTime, (MUINTPTR)&a_u4SEExpTime, (MUINTPTR)&a_i4LVvalue);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
setSensorAWBGain(MINT32 i4SensorDev, strSensorAWBGain *strSensorAWBInfo)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_HDR_AWB_GAIN, (MUINTPTR)strSensorAWBInfo, 0, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
getRollingShutter(MINT32 i4SensorDev, MUINT32& tline, MUINT32& vsize) const
{
    ...
    if (i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_GET_SENSOR_ROLLING_SHUTTER, (MUINTPTR)&tline, (MUINTPTR)&vsize, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
AAASensorMgr::setSensorOBLock(MINT32 i4SensorDev, MBOOL bLockSensorOB)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_OB_LOCK, (MUINTPTR)&bLockSensorOB, 0, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
setStereoFrameSyncExpAndFps(MINT32 i4SensorDev, MUINT32 i4ExpTime ,MUINT32 i4Fps)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_EXP_FRAME_TIME, (MUINTPTR)&i4ExpTime, (MUINTPTR)&i4Fps, 0);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
updateStereoFrameSyncFps(MINT32 i4SensorDev, MINT32 i4SensorDev2, MUINT32 i4ExpTimeMain,MUINT32 i4ExpTimeMain2)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHwSyncDrvObj->sendCommand(HW_SYNC_CMD_GET_FRAME_TIME, NSCam::SENSOR_DEV_MAIN_3, (MUINTPTR)ptr_expTime, (MUINTPTR)ptr_frmTime);
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
enableSensorI2CBuffMode(MINT32 i4SensorDev, MBOOL bEnableBuffMode)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_I2C_BUF_MODE_EN, (MUINTPTR)&bEnableBuffMode, 0, 0);
