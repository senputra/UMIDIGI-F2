...
{
    MBOOL ret_main, ret_sub, ret_sub2, ret_main2, ret_main3;
    ret_main = ret_sub = ret_sub2 = ret_main2 = ret_main3 = MTRUE;
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub = AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).init(i4SensorIdx);
    ...
    if (i4SensorDev & ESensorDev_MainThird)
        ret_main3 = AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).init(i4SensorIdx);

    return ret_main && ret_sub && ret_sub2 && ret_main2 && ret_main3;
}
...
MBOOL IAFOBufMgrWrapper::uninit(MINT32 const i4SensorDev)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub = AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).uninit();
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).uninit();
    if (i4SensorDev & ESensorDev_MainThird)
        ret_main3 = AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).uninit();
...
MBOOL IAFOBufMgrWrapper::DMAInit(MINT32 const i4SensorDev)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub = AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).DMAInit(MTRUE);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).DMAInit(MTRUE);
    if (i4SensorDev & ESensorDev_MainThird)
        ret_main3 = AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).DMAInit(MTRUE);
...
MBOOL IAFOBufMgrWrapper::DMAUninit(MINT32 const i4SensorDev)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub = AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).DMAUninit();
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).DMAUninit();
    if (i4SensorDev & ESensorDev_MainThird)
        ret_main3 = AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).DMAUninit();
...
MBOOL IAFOBufMgrWrapper::AFStatEnable(MINT32 const i4SensorDev, MBOOL En)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_sub = AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).AFStatEnable(En);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).AFStatEnable(En);
    if (i4SensorDev & ESensorDev_MainThird)
        ret_main3 = AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).AFStatEnable(En);
...
MBOOL IAFOBufMgrWrapper::enqueueHwBuf(MINT32 const i4SensorDev, BufInfo_T& rBufInfo)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).enqueueHwBuf(rBufInfo);
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).enqueueHwBuf(rBufInfo);
...
MBOOL IAFOBufMgrWrapper::dequeueHwBuf(MINT32 const i4SensorDev, BufInfo_T& rBufInfo)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).dequeueHwBuf(rBufInfo);
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).dequeueHwBuf(rBufInfo);
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).dequeueHwBuf(rBufInfo);
...
MBOOL IAFOBufMgrWrapper::updateDMABaseAddr(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).updateDMABaseAddr(AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).getNextHwBuf());
    else if (i4SensorDev & ESensorDev_MainSecond)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).updateDMABaseAddr(AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).getNextHwBuf());
...
MBOOL IAFOBufMgrWrapper::setTGInfo(MINT32 const i4SensorDev, MINT32 const i4TGInfo)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).setTGInfo(i4TGInfo);
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).setTGInfo(i4TGInfo);
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).setTGInfo(i4TGInfo);
...
MBOOL IAFOBufMgrWrapper::initPipe(MINT32 const i4SensorDev, MINT32 const i4SensorIdx, MINT32 const i4TGInfo)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).initPipe(i4SensorIdx, i4TGInfo);
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).initPipe(i4SensorIdx, i4TGInfo);
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).initPipe(i4SensorIdx, i4TGInfo);
...
MBOOL IAFOBufMgrWrapper::uninitPipe(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).uninitPipe();
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).uninitPipe();
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).uninitPipe();
...
MBOOL IAFOBufMgrWrapper::startPipe(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).startPipe();
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).startPipe();
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).startPipe();
...
MBOOL IAFOBufMgrWrapper::stopPipe(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).stopPipe();
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).stopPipe();
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).stopPipe();
...
MBOOL IAFOBufMgrWrapper::dequePipe(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).dequePipe();
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).dequePipe();
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).dequePipe();
...
BufInfo_T* IAFOBufMgrWrapper::dequeSwPipe(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).dequeSwPipe();
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).dequeSwPipe();
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).dequeSwPipe();
...
MBOOL IAFOBufMgrWrapper::enquePipe(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).enquePipe();
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).enquePipe();
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).enquePipe();
...
MBOOL IAFOBufMgrWrapper::abortDeque(MINT32 const i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        ...
        return AFOBufMgrWrapper::getInstance(ESensorDev_SubSecond).abortDeque();
    else if (i4SensorDev & ESensorDev_MainSecond)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainSecond).abortDeque();
    else if (i4SensorDev & ESensorDev_MainThird)
        return AFOBufMgrWrapper::getInstance(ESensorDev_MainThird).abortDeque();
...
AFOBufMgrWrapper& AFOBufMgrWrapper::getInstance(MINT32 const i4SensorDev)
{
    ...
    case ESensorDev_MainThird: //  Main Second Sensor
        return  AFOBufMgrDev<ESensorDev_MainThird>::getInstance();
    case ESensorDev_Sub: //  Sub Sensor
        return  AFOBufMgrDev<ESensorDev_Sub>::getInstance();
    case ESensorDev_SubSecond: //  Sub Sensor
        return  AFOBufMgrDev<ESensorDev_SubSecond>::getInstance();
    default:
...
MBOOL AFOBufMgrWrapper::updateDMABaseAddr(MUINT32 u4BaseAddr)
{
    ...
    if (m_eSensorTG == ESensorTG_1)
    {
        ...
        if (NULL != handle)
        {
            ...
            IOPIPE_SET_MODUL_REG(handle, CAM_AFO_OFST_ADDR,  0);
...
MBOOL AFOBufMgrWrapper::DMAInit(MBOOL En)
{
    ...
    if (m_eSensorTG == ESensorTG_1)
    {
        if (MFALSE==sendCommandNormalPipe(NSImageioIsp3::NSIspio::EPIPECmd_SET_MODULE_EN, NSImageioIsp3::NSIspio::EModule_ESFKO, MTRUE, MNULL))
        {
            MY_ERR("EPIPECmd_SET_MODULE_En fail: EModule_ESFKO");
            return MFALSE;
        }

        IOPIPE_SET_MODUL_ENABLE(NSImageioIsp3::NSIspio::EModule_AFO, En);
    ...
    else if (m_eSensorTG == ESensorTG_2)
    {
        ...
        if (MFALSE==sendCommandNormalPipe(NSImageioIsp3::NSIspio::EPIPECmd_SET_MODULE_SEL, EPipe_P1Sel_SGG_EN_D, 1,-1))
        {
            CAM_LOGE("EPipe_P1Sel_SGG_EN_D fail");
        }
        if (MFALSE==sendCommandNormalPipe(NSImageioIsp3::NSIspio::EPIPECmd_SET_MODULE_SEL, EPipe_P1Sel_SGG_D, 1,-1))
        {
...
MBOOL AFOBufMgrWrapper::DMAUninit()
{
    ...
    if (m_eSensorTG == ESensorTG_1)
    {
        IOPIPE_SET_MODUL_ENABLE(NSImageioIsp3::NSIspio::EModule_AFO, MFALSE);
    }
    else if (m_eSensorTG == ESensorTG_2)
    {
    ...
    waitIrq.UserInfo.Type = (m_TgInfo == CAM_TG_1) ? ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
    waitIrq.UserInfo.Status = (m_TgInfo == CAM_TG_1) ? CAM_CTL_INT_P1_STATUS_AF_DON_ST : CAM_CTL_INT_P1_STATUS_D_AF_DON_ST;
