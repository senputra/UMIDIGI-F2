...
static std::array<std::array<MY_INST_LCE, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonLCE;
...
ISP_MGR_LCE_T &ISP_MGR_LCE_T::getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    MY_INST_LCE& rSingleton = gMultitonLCE[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_LCE_T>(eSensorDev, eSensorTG);
    } );
...
MBOOL ISP_MGR_LCE_T::apply(EIspProfile_T eIspProfile,isp_reg_t* pReg)
{
    addressErrorCheck("Before ISP_MGR_LCE_T::apply()");
    ...
    ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN_P2, LCE_EN, m_bEnable);
