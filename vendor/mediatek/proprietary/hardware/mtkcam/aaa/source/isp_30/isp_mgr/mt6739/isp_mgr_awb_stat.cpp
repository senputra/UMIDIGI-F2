...
MBOOL ISP_MGR_AWB_STAT_CONFIG_T::config(AWB_STAT_CONFIG_T& rAWBStatConfig, MBOOL bHBIN2Enable, int imgW, int imgH, int blkW, int blkH, MINT32 i4SensorIndex)
{
    (void)bHBIN2Enable;
    (void)imgW;
    (void)imgH;
...
SetTGInfo(MINT32 const i4TGInfo)
{
	...
	CAM_LOGD("[%s()] i4TGInfo: %d , m_eSensorTG: %d \n", __FUNCTION__, i4TGInfo, m_eSensorTG);
#endif
    return MTRUE;
}
...
apply()
{
    CAM_LOGD("%s(): m_eSensorDev = %d, m_i4SensorIndex = %d\n", __FUNCTION__, m_eSensorDev, m_i4SensorIndex);
    dumpRegInfo("AWB_STAT_TG1");
    MUINTPTR handle;
    INormalPipe_FrmB* pPipe = INormalPipe_FrmB::createInstance(m_i4SensorIndex,"isp_mgr_awb_stat_tg1");
    // get module handle
    if (MFALSE == pPipe->sendCommand(NSImageioIsp3::NSIspio::EPIPECmd_GET_MODULE_HANDLE,
                           NSImageioIsp3::NSIspio::EModule_AWB, (MINTPTR)&handle, (MINTPTR)(&("isp_mgr_awb_stat_tg1"))))
    {
        //Error Handling
    ...
    if (MFALSE==pPipe->sendCommand(NSImageioIsp3::NSIspio::EPIPECmd_SET_MODULE_CFG_DONE, handle, MNULL, MNULL))
    {
        //Error Handling
...
lbExit:
    // release handle
    if (MFALSE==pPipe->sendCommand(NSImageioIsp3::NSIspio::EPIPECmd_RELEASE_MODULE_HANDLE, handle, (MINTPTR)(&("isp_mgr_awb_stat_tg1")), MNULL))
    {
        // Error Handling
        MY_ERR("EPIPECmd_SET_MODULE_CFG_DONE fail");
    }
