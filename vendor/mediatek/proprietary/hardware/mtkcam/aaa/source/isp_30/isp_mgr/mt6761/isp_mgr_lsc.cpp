...
static std::array<std::array<MY_INST_LSC, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonLSC;

#define MY_INST_LSC2 NS3Av3::INST_T<ISP_MGR_LSC2_T>
static std::array<std::array<MY_INST_LSC2, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonLSC2;
...
getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    MY_INST_LSC& rSingleton = gMultitonLSC[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_LSC_T>(eSensorDev, eSensorTG);
    } );
...
{
    MY_LOG_IF(m_bDebugEnable, "[%s]Ratio(%d)\n", __FUNCTION__, u4Rto);
    m_rIspRegInfo[ERegInfo_CAM_LSC_RATIO].val = (u4Rto<<24)|(u4Rto<<16)|(u4Rto<<8)|u4Rto;
...
getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    MY_INST_LSC2& rSingleton = gMultitonLSC2[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_LSC2_T>(eSensorDev, eSensorTG);
    } );
...
apply(EIspProfile_T eIspProfile)
{
    addressErrorCheck("Before ISP_MGR_LSC2_T::apply()");
    ...
    //MUINT32 u4Addr = REG_INFO_VALUE(CAM_LSCI_BASE_ADDR);
    //MY_LOG_IF(1, "[%s] yang lsc p1 (%d %d %d %d), u4BaseAddr(0x%08x)\n", __FUNCTION__, u4XNum, u4YNum, u4Wd, u4Ht, u4Addr);
...
//    REG_INFO_VALUE(CAM_LSC_GAIN_TH)     = 0x03F00000;
    ...
    addressErrorCheck("After ISP_MGR_LSC2_T::apply()");
...
apply(EIspProfile_T eIspProfile, isp_reg_t* pReg)
{
    ...
    //MUINT32 u4Addr = REG_INFO_VALUE(CAM_LSCI_BASE_ADDR);
    //MY_LOG_IF(1, "[%s] yang lsc p2 (%d %d %d %d), u4BaseAddr(0x%08x)\n", __FUNCTION__, u4XNum, u4YNum, u4Wd, u4Ht, u4Addr);
