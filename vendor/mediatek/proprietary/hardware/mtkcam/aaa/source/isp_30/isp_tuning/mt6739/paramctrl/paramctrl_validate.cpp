...
validate(MINT32 const i4FrameID, MBOOL const fgForce)
{
    ...
    if(!fgRet)
        MY_LOG("[%s] fgRet %d", __func__, fgRet);
...
MERROR_ENUM
Paramctrl::
validatePerFrameP2(MINT32 flowType, const ISP_INFO_T& rIspInfo, void* pRegBuf)
{
    MERROR_ENUM err = MERR_UNKNOWN;

    MY_LOG_IF(m_bDebugEnable, "[validatePerFrameP2] i4FrameID = %d", rIspInfo.rCamInfo.u4Id);
...
{
    MY_LOG_IF(m_bDebugEnable, "eIspProfile = %d, eSensorMode = %d", eIspProfile, eSensorMode);

    INDEX_T const*const pDefaultIndex = m_pIspTuningCustom->getDefaultIndex(
            static_cast<EIspProfile_T>(eIspProfile), static_cast<ESensorMode_T>(eSensorMode), EIndex_Scene_T(0), static_cast<EIndex_ISO_T>(0)
        );
...
MERROR_ENUM
Paramctrl::
do_validatePerFrameP2(MINT32 flowType, const ISP_INFO_T& rIspInfo, void* pRegBuf)
{
    MERROR_ENUM err = MERR_OK;
    ...
    if (isDynamicTuning())
    {
        ...
        //  a) Get default index setting.
        INDEX_T const*const pDefaultIndex = m_pIspTuningCustom->getDefaultIndex(
            rIspInfo.rCamInfo.eIspProfile, rIspInfo.rCamInfo.eSensorMode, rIspInfo.rCamInfo.eIdx_Scene, rIspInfo.rCamInfo.eIdx_ISO
        );
        if  ( ! pDefaultIndex )
        {
            MY_ERR("[ERROR][validatePerFrame]pDefaultIndex==NULL");
        ...
        //  b) Customize the index setting.
        m_pIspTuningCustom->evaluate_nvram_index(rIspInfo.rCamInfo, idxmgr);
        ...
        MY_LOG_IF(m_bDebugEnable, "IndexMgr::getIdx = [OBC(%d),BPC(%d),NR1(%d),CFA(%d),ANR(%d),CCR(%d),EE(%d)]\n"
                ,idxmgr.getIdx_OBC()
                ,idxmgr.getIdx_BPC()
                ,idxmgr.getIdx_NR1()
                ,idxmgr.getIdx_CFA()
        ...
        (isDynamicBypass() == MTRUE) ? (prepare_rdy = prepareHw_PerFrame_Partial(rIspInfo.rCamInfo, idxmgr))
                                     : (prepare_rdy = prepareHw_PerFrame_All(rIspInfo, idxmgr));

    }


    if(!prepare_rdy || ! applyToHw_PerFrame_P2(flowType, rIspInfo.rCamInfo, pRegBuf))  //  Apply the ispmgr's buffer to H/W.
    {
        err = MERR_SET_ISP_REG;
        goto lbExit;
    }
