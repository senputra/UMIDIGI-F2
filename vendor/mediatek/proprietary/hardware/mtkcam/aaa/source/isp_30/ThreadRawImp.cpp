...
ThreadRawImp::createEventThread()
{
    ...
    else
    {
        ...
        if(m_pEventIrq == NULL)
        {
            ...
            const char* const callerName = "Hal3ARaw_thread";
            IHalSensor* pHalSensor = pHalSensorList->createSensor(callerName, mi4SensorIdx);
            MY_ASSERT(pHalSensor != nullptr);

            SensorDynamicInfo senInfo;
            MINT32 ret = pHalSensor->querySensorDynamicInfo(mi4SensorDev, &senInfo);
            MY_ASSERT(ret == MTRUE);
            ...
            IrqConfig.i4TgInfo = senInfo.TgInfo;
...
ThreadRawImp::onEventThreadLoop(MVOID *arg)
{
    ...
    while(1)
    {
        ...
        if(_this->mEvent == E_3AEvent_AE_I2C){
            ...
            MY_LOGW("[%s] E_3AEvent_Flash_On, wait P1_DON start +", __FUNCTION__);
            ...
            MY_LOGW("[%s] E_3AEvent_Flash_Off, wait P1_DON start +", __FUNCTION__);
            if(_this->m_pEventIrq)
               _this->m_pEventIrq->wait(duration);
            MY_LOGW("[%s] E_3AEvent_Flash_Off, wait P1_DON start -", __FUNCTION__);
...
ThreadRawImp::onAFThreadLoop(MVOID *arg)
{
    ...
    while (_this->mbAFThreadLoop && IsSupportAF) {
        ...
        if(_this->mpTaskMgr->dequeStt(Task_Update_AF) >= 0)
        {
            ...
            rData.bIsAfTriggerInPrecap = (bIsAFStart && (bIsPrecapTrigger || isExistPrecap)) || rReq.rParam.u1FlashTrigAF;    // Nelson Coverity : bIsAfStart is always false under this condition (bIsAFStart && (bIsPrecapTrigger || isExistPrecap));
            ...
            TASK_RESULT eResult = _this->mpTaskMgr->execute(Task_Update_AF);
            CAM_TRACE_END();
            AAA_TRACE_END_MGR;
            ...
            MY_LOGD_IF(_this->mu4LogEn, "[Hal3A::AFThreadFunc] TaskMgr::execute(AFUpdate) done, eResult(%d)\n", eResult);
...
getCurrResult4AF(AFParam_T& AFParam)
{
    ...
    if(pAllResult)
    {
        ...
        MUINT8 u1AfState = rAFResult.u1AfState;
        if(AFParam.u1AfTrig == MTK_CONTROL_AF_TRIGGER_START){
            if( AFParam.u4AfMode == MTK_CONTROL_AF_MODE_AUTO || AFParam.u4AfMode == MTK_CONTROL_AF_MODE_MACRO)
                ...
                rAFResult.u1AfState = MTK_CONTROL_AF_STATE_NOT_FOCUSED_LOCKED;
            CAM_LOGD("[%s] Hack af state when auto focus, u1AfMode(%d), u1AfState(%d->%d)", __FUNCTION__, AFParam.u4AfMode, u1AfState, rAFResult.u1AfState);
        }
        ...
        {
            if (u1AfState == MTK_CONTROL_AF_STATE_NOT_FOCUSED_LOCKED || u1AfState == MTK_CONTROL_AF_STATE_FOCUSED_LOCKED )//////if AF state in (4/6) , no need to change AF state for xts test
                rAFResult.u1AfState = u1AfState ;
            else
            {
                ...
                CAM_LOGD( "[%s] FlashBack (%d),u1AfMode(%d), overwrites AF state(%d)", __FUNCTION__, IsFlashBackExist,AFParam.u4AfMode, rAFResult.u1AfState);
            }
