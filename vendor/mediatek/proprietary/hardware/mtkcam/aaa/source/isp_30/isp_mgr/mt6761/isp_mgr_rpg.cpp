...
#define MY_INST_RPG NS3Av3::INST_T<ISP_MGR_RPG_T>
static std::array<std::array<MY_INST_RPG, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonRPG;
...
getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    MY_INST_RPG& rSingleton = gMultitonRPG[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_RPG_T>(eSensorDev, eSensorTG);
    } );
...
apply(EIspProfile_T eIspProfile, isp_reg_t* pReg)
{
    ...
    if (m_eSensorTG == ESensorTG_1) {
        ...
        // TOP
        ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN_P1, RPG_EN, isEnable());
        ...
        // TOP
        ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN_P1_D, RPG_D_EN, isEnable());
