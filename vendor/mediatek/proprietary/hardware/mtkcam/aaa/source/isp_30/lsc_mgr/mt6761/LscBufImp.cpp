...
#define DEFAULT_TABLE_SIZE (32*32*4*4*4)
...
LscImemBuf(MUINT32 sensorDev, MUINT32 u4Id, MBOOL usecache, const char* strName)
    ...
    , m_LogEnable(0)
{
    GET_PROP("vendor.debug.lsc_mgr.log", 0, m_LogEnable);
    initMemBuf(usecache);
}
...
setConfig(ILscBuf::Config rConfig)
{
    ...
    MUINT32 u4Size = (rConfig.i4BlkX+1) * (rConfig.i4BlkY+1) * 4 * 4 * 4 * 4;
...
setConfig(ILscBuf::Config rConfig)
{
    ...
    if (u4Size != getSize())
    {
        ...
        {
            MUINT32 LSCI_XSIZE = (m_rConfig.i4BlkX+1)*4*4*4;
...
setTable(const void* data, MUINT32 u4Size)
{
    ...
    {
        LSC_LOG("src(%p), dest(%p), size(%d), imemsize(%d)", data, pDest, u4Size, getSize());
        return MFALSE;
    }
    ::memcpy(pDest, data, getSize());
    return MTRUE;
}
...
setConfig(ILscTable::Config rCfg)
{
    ...
    LSC_LOG("[%s] rCfg %p", __func__, &rCfg);
    return true;
}
