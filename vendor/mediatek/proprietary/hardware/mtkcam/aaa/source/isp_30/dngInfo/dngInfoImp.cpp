...
{
public:
    static DngInfoImp*          getInstance(MINT32 i4SensorIdx);
...
#define MY_INST NS3Av3::INST_T<DngInfoImp>
...
{
    return DngInfoImp::getInstance(static_cast<MINT32>(sensorIndex));
}


DngInfoImp*
DngInfoImp::
getInstance(MINT32 i4SensorIdx)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<DngInfoImp>(i4SensorIdx);
    } );
...
DngInfoImp(MINT32 i4SensorIdx)
    ...
    , m_pNvram3A(NULL)
{
    auto u4SensorDev = static_cast<MUINT32>(NS3Av3::mapSensorIdxToDev(i4SensorIdx));
    ...
    if(err!=0)
    {
        CAM_LOGE("Fail to getBufAndRead(CAMERA_NVRAM_DATA_ISP)!");
    }
    else
    {
        rShadingMapSize.w = pNvram_ISP->ISPRegs.LSC[ESensorMode_Capture].ctl2.bits.LSC_SDBLK_XNUM + 2;
        rShadingMapSize.h = pNvram_ISP->ISPRegs.LSC[ESensorMode_Capture].ctl3.bits.LSC_SDBLK_YNUM + 2;
    ...
    {
        MUINT32 u4FNum = m_pNvram3A->rAENVRAM[AENVRAMMapping[CAM_SCENARIO_PREVIEW]].rDevicesInfo.u4LensFno;
        MUINT32 u4FL   = m_pNvram3A->rAENVRAM[AENVRAMMapping[CAM_SCENARIO_PREVIEW]].rDevicesInfo.u4FocusLength_100x;
...
getDynamicNoiseProfile(MUINT32 iso) const
{
    ...
    {
        double s, o;
        s = (rNoiseProfile[i].S.a * iso + rNoiseProfile[i].S.b)/2.0;
...
convertColorTransform(const NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspParam)
{
    ...
    #else
    const ISP_NVRAM_CCM_T& ccm1 = rIspParam.ISPMulitCCM.dynamic_CCM[i4RefIllum1];
    #endif
    in_CCM[0][0] = (double) TO_INT(ccm1.cnv_1.bits.G2G_CNV_00, 13) / 512;
    in_CCM[0][1] = (double) TO_INT(ccm1.cnv_1.bits.G2G_CNV_01, 13) / 512;
    in_CCM[0][2] = (double) TO_INT(ccm1.cnv_2.bits.G2G_CNV_02, 13) / 512;
    in_CCM[1][0] = (double) TO_INT(ccm1.cnv_3.bits.G2G_CNV_10, 13) / 512;
    in_CCM[1][1] = (double) TO_INT(ccm1.cnv_3.bits.G2G_CNV_11, 13) / 512;
    in_CCM[1][2] = (double) TO_INT(ccm1.cnv_4.bits.G2G_CNV_12, 13) / 512;
    in_CCM[2][0] = (double) TO_INT(ccm1.cnv_5.bits.G2G_CNV_20, 13) / 512;
    in_CCM[2][1] = (double) TO_INT(ccm1.cnv_5.bits.G2G_CNV_21, 13) / 512;
    in_CCM[2][2] = (double) TO_INT(ccm1.cnv_6.bits.G2G_CNV_22, 13) / 512;
    ...
    if (fgSupport2ColorMatrices)
    {
        ...
        #else
        const ISP_NVRAM_CCM_T& ccm2 = rIspParam.ISPMulitCCM.dynamic_CCM[i4RefIllum2];
        #endif
        in_CCM[0][0] = (double) TO_INT(ccm2.cnv_1.bits.G2G_CNV_00, 13) / 512;
        in_CCM[0][1] = (double) TO_INT(ccm2.cnv_1.bits.G2G_CNV_01, 13) / 512;
        in_CCM[0][2] = (double) TO_INT(ccm2.cnv_2.bits.G2G_CNV_02, 13) / 512;
        in_CCM[1][0] = (double) TO_INT(ccm2.cnv_3.bits.G2G_CNV_10, 13) / 512;
        in_CCM[1][1] = (double) TO_INT(ccm2.cnv_3.bits.G2G_CNV_11, 13) / 512;
        in_CCM[1][2] = (double) TO_INT(ccm2.cnv_4.bits.G2G_CNV_12, 13) / 512;
        in_CCM[2][0] = (double) TO_INT(ccm2.cnv_5.bits.G2G_CNV_20, 13) / 512;
        in_CCM[2][1] = (double) TO_INT(ccm2.cnv_5.bits.G2G_CNV_21, 13) / 512;
        in_CCM[2][2] = (double) TO_INT(ccm2.cnv_6.bits.G2G_CNV_22, 13) / 512;
