 ...
 * MEDIATEK FOR SUCH MEDIATEK SOFwTWARE AT ISSUE.
 *
...
MRESULT AeMgr::setSceneMode(MUINT32 u4NewAEScene)
{
    ...
    if ((m_i4AEMaxFps == m_i4AEMinFps) && (m_i4AEMaxFps <= LIB3A_AE_FRAMERATE_MODE_30FPS) && CUST_FIX_FPS_PLINE)
    {
        if (m_i4AEMinFps == LIB3A_AE_FRAMERATE_MODE_15FPS)
        {
            eAEScene = LIB3A_AE_SCENE_FIX_15FPS;
            AE_LOG_IF(m_3ALogEnable, "[%s()] Modify scene mode:%d for 3rd AP fix 15fps\n", __FUNCTION__, eAEScene);
        }
        else if (m_i4AEMinFps == LIB3A_AE_FRAMERATE_MODE_30FPS)
        {
            eAEScene = LIB3A_AE_SCENE_FIX_30FPS;
            AE_LOG_IF(m_3ALogEnable, "[%s()] Modify scene mode:%d for 3rd AP fix 30fps\n", __FUNCTION__, eAEScene);
        }
        else
        {
    ...
    if (m_eAEScene != eAEScene)
    {
        ...
        else
        {
            ...
            m_u4MaxISO = 800;
            AE_LOG("[%s()] The AE algo class is NULL, i4SensorDev:%d line:%d MaxShutter:%d MaxISO:%d\n", __FUNCTION__, m_eSensorDev, __LINE__, m_u4MaxShutter, m_u4MaxISO);
        }
    }
...
MRESULT AeMgr::setAECamMode(MUINT32 u4NewAECamMode)
{
    ...
    if (m_eCamMode != eNewAECamMode)
    {
        ...
        AE_LOG("m_eCamMode:%d AECamMode:%d \n", m_eCamMode, m_eAECamMode);
    }
    return S_AE_OK;
}
...
MRESULT AeMgr::setAEAutoFlickerMode(MUINT32 u4NewAEAutoFLKMode)
{
    ...
    {
        AE_LOG("[%s()] m_eAEAutoFlickerMode:%d->%d (0->50Hz;1->60Hz)\n", __FUNCTION__, m_eAEAutoFlickerMode, eNewAEAutoFLKMode);
...
MRESULT AeMgr::setIsoSpeed(MUINT32 a_u4ISO)
{
    ...
    memset(&ePreviewPLineTableID, 0, sizeof(eAETableID));
    memset(&eCapturePLineTableID, 0, sizeof(eAETableID));

    eAEScene = m_eAEScene;
    AE_LOG_IF(m_3ALogEnable, "[%s()] ISO:%d %d\n", __FUNCTION__, a_u4ISO, m_u4AEISOSpeed);
...
MRESULT AeMgr::setAEScene(LIB3A_AE_SCENE_T a_eAEScene)
{
    ...
    if (m_pAEPlineTable != NULL)
    {
        ...
        if (FAILED(mr))
        {
            CAM_LOGE("[%s()] Set capture table ERROR:%d PLineID:%d %d\n", __FUNCTION__, m_eAEScene, ePreviewPLineTableID, eCapturePLineTableID);
        }
...
MRESULT AeMgr::setAETable(eAETableID a_eAEPreTableID, eAETableID a_eAECapTableID)
{
    ...
    AE_LOG_IF(m_3ALogEnable, "[%s()] TableID(Pre/Cap):%d/%d Flicker(Mode/Auto):%d/%d i4SensorMode:%d\n", __FUNCTION__,
    ...
    // } else {
    if (m_bManualAEtable)
        mr = setManualAETable(m_u4ManualAEtablePreviewidx, m_u4ManualAEtableCaptureidx);
...
MRESULT AeMgr::getAEPlineTable(eAETableID eTableID, strAETable &a_AEPlineTable)
{
    ...
    memset(&strAEPlineTable, 0, sizeof(strAETable));
...
MRESULT AeMgr::searchPreviewIndexLimit()
{
    ...
    if (m_u4IndexMax > m_u4IndexMin)
    {
        ...
        if (itemp > (MINT32)m_pPreviewTableNew->u4TotalIndex)
        {
            ...
            m_u4IndexMax = m_pPreviewTableNew->u4TotalIndex -(MUINT32)1;
            AE_LOG("[%s] Max (min BV) reach Preview limitation, need extend preview table\n", __FUNCTION__);
            if (m_pPreviewTableCurrent != NULL)
...
MRESULT AeMgr::setAEMinMaxFrameRatePlineIdx(MINT32 a_eAEMinFrameRate, MINT32 a_eAEMaxFrameRate)
{
    ...
    for (i4PlineIndex = (MINT32)m_u4IndexMin; i4PlineIndex <= (MINT32)m_u4IndexMax; i4PlineIndex++)
    {
        ...
        {
            AE_LOG("[%s()] The preview current Pline table is NULL :%d %d %d %d %d %d\n", __FUNCTION__, m_i4AEMinFps, m_u4IndexMax, m_u4IndexMin, m_u4Index, i4PlineIndex, i4fps);
        }
    }
    ...
    AE_LOG("[%s():e] m_u4FinerEVIdxBase:%d MinFps/MaxIdx/MinIdx/Idx/MaxIdxF/MinIdxF/IdxF/SearchIdx/Searchfps:%d/%d/%d/%d/%d/%d/%d/%d/%d\n", __FUNCTION__, m_u4FinerEVIdxBase, m_i4AEMinFps, m_u4IndexMax, m_u4IndexMin, m_u4Index, m_u4IndexFMax, m_u4IndexFMin, m_u4IndexF, i4PlineIndex, i4fps);
...
MRESULT AeMgr::setAEMaxFrameRateGainIndexRange()
{
    ...
    i430fps = 300;
    AE_LOG_IF(m_3ALogEnable, "[%s] Set Gain Index Range for LCE\n", __FUNCTION__);
    ...
    for (i4PlineIndex = (MINT32)m_u4IndexMin; i4PlineIndex <= (MINT32)m_u4IndexMax; i4PlineIndex++)
    {
        ...
        {
            AE_LOG("[%s] The preview new Pline table is NULL\n", __FUNCTION__);
        }
    }

    AE_LOG_IF(m_3ALogEnable, "[%s] m_u4LCEGainStartIdx:%d m_u4LCEGainEndIdx:%d\n", __FUNCTION__, m_u4LCEGainStartIdx, m_u4LCEGainEndIdx);
    return S_AE_OK;
}
 ...
 *
 ********************************************************************************/
MRESULT AeMgr::setManualAETable(MUINT32 a_AEPreTableID, MUINT32 a_AECapTableID)
{
    MRESULT mr = S_AE_OK;
    eAETableID ePreviewPLineTableID, eCapturePLineTableID;
    MUINT32 i;
    ...
    switch (a_AEPreTableID)
    {
        case AE_PLINE_EISRECORD:
            ePreviewPLineTableID = AETABLE_SCENE_INDEX9;
            break;
        case AE_PLINE_SM240FPS:
            ePreviewPLineTableID = AETABLE_SCENE_INDEX10;
            break;
        case AE_PLINE_STEREO_PREVIEW:
            ePreviewPLineTableID = AETABLE_SCENE_INDEX11;
            break;
        case AE_PLINE_STEREO_CAPTURE:
            ePreviewPLineTableID = AETABLE_SCENE_INDEX12;
            break;
        case AE_PLINE_STEREO_VIDEO:
            ePreviewPLineTableID = AETABLE_SCENE_INDEX13;
            break;
        case AE_PLINE_CUSTOM_30FPS:
            ePreviewPLineTableID = AETABLE_VIDEO_AUTO;
            break;
        default:
            AE_LOG("[%s()] The Preview AE table enum value is incorrectly:%d\n", __FUNCTION__, a_AEPreTableID);
            ePreviewPLineTableID = m_pPreviewTableCurrent->eID;
            break;
    }
    switch (a_AECapTableID)
    {
        case AE_PLINE_EISRECORD:
            eCapturePLineTableID = AETABLE_SCENE_INDEX9;
            break;
        case AE_PLINE_SM240FPS:
            eCapturePLineTableID = AETABLE_SCENE_INDEX10;
            break;
        case AE_PLINE_STEREO_PREVIEW:
            eCapturePLineTableID = AETABLE_SCENE_INDEX11;
            break;
        case AE_PLINE_STEREO_CAPTURE:
            eCapturePLineTableID = AETABLE_SCENE_INDEX12;
            break;
        case AE_PLINE_STEREO_VIDEO:
            eCapturePLineTableID = AETABLE_SCENE_INDEX13;
            break;
        case AE_PLINE_CUSTOM_30FPS:
            eCapturePLineTableID = AETABLE_VIDEO_AUTO;
            break;
        default:
            AE_LOG("[%s()] The Capture AE table enum value is incorrectly:%d\n", __FUNCTION__, a_AECapTableID);
    ...
    {
        // ePreviewPLineTableID = m_pPreviewTableCurrent->eID;
        ePreviewPLineTableID = m_pPreviewTableNew->eID;
...
{
    AE_LOG_IF(m_3ALogEnable, "[%s:s] m_u4Index/m_u4IndexF/m_i4DeltaSensitivityIdx/m_i4DeltaBVIdx : %d/%d/%d/%d\n", __FUNCTION__, m_u4Index, m_u4IndexF, m_i4DeltaSensitivityIdx, m_i4DeltaBVIdx);
    ...
    AE_LOG("[%s:e] m_u4Index/m_u4IndexF/Exp/Afe/Isp/ISO : %d/%d/%d/%d/%d/%d \n", __FUNCTION__, m_u4Index, m_u4IndexF, m_rAEOutput.rPreviewMode.u4Eposuretime, m_rAEOutput.rPreviewMode.u4AfeGain, m_rAEOutput.rPreviewMode.u4IspGain , m_rAEOutput.rPreviewMode.u4RealISO);
    return S_AE_OK;
}

MRESULT AeMgr::ModifyCaptureParamByCustom(MUINT32 binning_ratio, AE_MODE_CFG_T *sAEInputInfo, strAEOutput *sAEOutputInfo, MUINT32 maxAfeGain)
{
    ...
    if (sAEInputInfo->u4Eposuretime < 20000)
    {
...
MRESULT AeMgr::ModifyCaptureParamByCustom(MUINT32 binning_ratio, AE_MODE_CFG_T *sAEInputInfo, AE_MODE_CFG_T *sAEOutputInfo, MUINT32 maxAfeGain)
{
    #ifdef CustomModifyFourCell
    //ALOGD("[%s] u4Eposuretime:%d, u4AfeGain:%d u4IspGain:%d u4RealISO:%d\n", __FUNCTION__, sAEInputInfo->u4Eposuretime, sAEInputInfo->u4AfeGain, sAEInputInfo->u4IspGain, sAEInputInfo->u4RealISO);
    if (CustomModifyGetExpo || sAEInputInfo->u4Eposuretime < 20000)
    {
