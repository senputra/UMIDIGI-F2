...
char gb_MT6739_ISP_ID_TEXT[ISP_ID_TEXT_LENGTH] = "6739";
...
MINT32 giCctCmdOutBufSize[CCT_CMD_OUTBUF_SIZE_COUNT][2] = {
   ...
   {  FT_CCT_OP_SET_PROP,                       0 },
   {  FT_CCT_OP_GET_PROP,                       128 },
                ...
                MUINT32 *pu4RealParaOutLen )
{
    ...
    case FT_CCT_OP_SET_PROP:
        if (puParaIn != NULL && u4ParaInLen > 0) {
            ...
            strncpy(cmd, (char *)puParaIn, 127);
            strncpy(val, (char *)(puParaIn+cmdlen+1), 127);
            cmd[127] = 0;
            val[127] = 0;
        ...
        *pu4RealParaOutLen = 0;
        break;
    case FT_CCT_OP_GET_PROP:
        if (puParaIn != NULL && u4ParaInLen > 0 && puParaOut != NULL && u4ParaOutLen == 128) {
            char cmd[128];
            char val[PROPERTY_VALUE_MAX];

            strlen((char *)puParaIn);
            strncpy(cmd, (char *)puParaIn, 127);
            cmd[127] = 0;
            ...
            strncpy((char *)puParaOut, (char *)val, 127);
            puParaOut[127] = 0;
            *pu4RealParaOutLen = 128;
        }
        else {
            err = CCTIF_BAD_PARAM;
            *pu4RealParaOutLen = 0;
                ...
                MUINT32 *pu4RealParaOutLen )
{
    ...
    case FT_CCT_OP_GET_ID:
        ...
        if ( ISP_ID_TEXT_LENGTH != u4ParaOutLen || ! pu4RealParaOutLen || ! puParaOut ) {
            ...
            *pu4RealParaOutLen = 0;
            ...
            strncpy((char *)puParaOut, gb_MT6739_ISP_ID_TEXT, ISP_ID_TEXT_LENGTH);
    ...
    */
    ...
    case FT_CCT_OP_ISP_GET_SMOOTH_CCM:
        ...
        reinterpret_cast<o_type*>(puParaOut)->Enable = m_rBuf_ISP.ISPMulitCCM.CCM_Method;
        *pu4RealParaOutLen = sizeof(o_type);
        MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_CCM_GET_SMOOTH_SWITCH ) done, %d\n", m_rBuf_ISP.ISPMulitCCM.CCM_Method);
        }
        break;
    case FT_CCT_OP_ISP_SET_SMOOTH_CCM:
        ...
        *pu4RealParaOutLen = 0;
        ...
        m_rBuf_ISP.ISPMulitCCM.CCM_Method = reinterpret_cast<i_type*>(puParaIn)->Enable ? 1 : 0;
...
#if 1 // yiyi, shading
        {
            MY_LOGD("Shading Set Property: vendor.debug.lsc_mgr.opt123");
            property_set( "vendor.debug.lsc_mgr.opt123", "1" );
...
cctNvram_SetIspNvramData(ISP_NVRAM_REGISTER_STRUCT *pIspRegs)
{
    ...
    //index = pIspRegs->Idx.LSC;
    //if (index < NVRAM_LSC_TBL_NUM) {
    //    m_rISPRegs.LSC[index] = (pIspRegs->LSC[index]);
    //    m_rISPRegsIdx.LSC = static_cast<MUINT8>(index);
    //    ISP_NVRAM_LSC_T::getInstance((ESensorDev_T)m_eSensorEnum).put((pIspRegs->LSC[index]));
    //}
    //
    ...
    //    ISP_NVRAM_NSL2A_T::getInstance((ESensorDev_T)m_eSensorEnum).put((pIspRegs->SL2[index]));
    //}
    //
    //index = pIspRegs->Idx.CFA;
    //if (index < NVRAM_CFA_TBL_NUM) {
    //    m_rISPRegs.CFA[index] = (pIspRegs->CFA[index]);
    //    m_rISPRegsIdx.CFA = static_cast<MUINT8>(index);
    //    ISP_NVRAM_CFA_T::getInstance((ESensorDev_T)m_eSensorEnum).put((pIspRegs->CFA[index]));
    //}
    ...
    if (index < NVRAM_CCR_TBL_NUM) {
        ...
        ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).put((pIspRegs->CCR[index]));
    }
