...
{
    NVRAM_DRV_LOG("isNvramExist is not support in isp 30 dev %d, id %d", dev, id);
    return 0;
}
...
void NvramDrv::setAndroidMode(int isAndroid)
{
    ...
    NVRAM_DRV_LOG("[%s] isAndroid %d", __func__, isAndroid);
    mAndroidMode=0;
}
...
)
{
    ...
    else if (a_eNvramDataType == CAMERA_NVRAM_DATA_AF)   // strobe
        targetVersion = NVRAM_CAMERA_AF_FILE_VERSION;
    else if (a_eNvramDataType == CAMERA_NVRAM_DATA_FLASH_CALIBRATION)   // flash calibration
...
{
    int err = NVRAM_NO_ERROR;
    NVRAM_DRV_LOG("[readNoDefault] id=%d dev=%d a_u4NvramDataSize=%lu",a_eSensorType,a_eNvramDataType,a_u4NvramDataSize);
...
{
    int err = NVRAM_NO_ERROR;
    NVRAM_DRV_LOG("[writeNvram] sensor type = %d; NVRAM data type = %d, a_u4SensorID %lu\n", a_eSensorType, a_eNvramDataType, a_u4SensorID);
    ...
    if(a_u4NvramDataSize != g_u4NvramDataSize[a_eNvramDataType])
    {
        logI("writeNvram- g_u4NvramDataSize=%d", g_u4NvramDataSize[a_eNvramDataType]);
        NVRAM_DRV_LOG("[readNvrameEx] buf size is error, line=%d",__LINE__);
        return NVRAM_READ_PARAMETER_ERROR;
    }
...
//seanlin 121221 avoid camera has not inited<
    ...
    case CAMERA_DATA_AE_PLINETABLE:
        ...
        break;
    case CAMERA_NVRAM_DATA_AF:
        i4FileInfo =     AP_CFG_RDCL_CAMERA_AF_LID;
        break;
    case CAMERA_NVRAM_DATA_FLASH_CALIBRATION:
        i4FileInfo =     AP_CFG_RDCL_CAMERA_FLASH_CALIBRATION_LID;
        break;
...
//        logI("check nvram demon");
    ...
    if (a_eSensorType == DUAL_CAMERA_SUB_2_SENSOR) {
        lseek(rNvramFileID.iFileDesc, i4RecSize*3, SEEK_SET);
    }

    if (a_eSensorType == DUAL_CAMERA_MAIN_3_SENSOR) {
        lseek(rNvramFileID.iFileDesc, i4RecSize*4, SEEK_SET);
    }

    if (i4RecSize > g_u4NvramDataSize[a_eNvramDataType]) {
        i4RecSize = g_u4NvramDataSize[a_eNvramDataType];
    }
    ...
    if (a_eSensorType == DUAL_CAMERA_SUB_SENSOR)
        sh=2;
    else if (a_eSensorType == DUAL_CAMERA_SUB_2_SENSOR)
        sh=3;
    else if (a_eSensorType == DUAL_CAMERA_MAIN_SECOND_SENSOR)
        sh=1;
    else if (a_eSensorType == DUAL_CAMERA_MAIN_3_SENSOR)
        sh=4;
    int ret;
...
int NvramDrv::writeNvramReal(void* buf, int id, int dev)
{
    ...
    if (dev == DUAL_CAMERA_SUB_2_SENSOR) {
        lseek(rNvramFileID.iFileDesc, i4RecSize*3, SEEK_SET);
    }

    if (dev == DUAL_CAMERA_MAIN_3_SENSOR) {
        lseek(rNvramFileID.iFileDesc, i4RecSize*4, SEEK_SET);
    }
