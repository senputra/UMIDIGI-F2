...
static LscMgrDefault* _pLscMain3;
...
createInstance(ESensorDev_T const eSensorDev, MINT32 i4SensorIdx, ESensorTG_T const eSensorTG, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<LscMgrDefault>((ESensorDev_T)eSensorDev, i4SensorIdx, eSensorTG, rIspNvram);
    } );
...
getSensorResolution()
{
    ...
    for (i = 0; i < u4NumSensorModes; ++i)
    {
        convertSensorCrop(!m_fgSensorCropInfoNull, m_rSensorCropWin[m_eFullSensorMode], m_rSensorCropWin[i], m_rTransformCfg[i]);
...
calSl2Config(MUINT32 u4Wd, MUINT32 u4Ht, SL2_CFG_T& rSl2Cfg) const
{
    ...
    ISP_MGR_NSL2_T::getInstance(m_eSensorDev).setNSL2Size(u4Wd,u4Ht);
...
{
    m_fgOnOff = fgOnOff;
    ISP_MGR_LSC_T::getInstance(m_eSensorDev, m_eSensorTG).enable(fgOnOff);
}
...
GetNSL2Param() const
{
        ...
        case 0:
            fgSl2OnOff = 0;
            i4Idx = m_eSensorMode*SHADING_SUPPORT_CT_NUM+m_u4CTIdx;
            if (i4Idx >= NVRAM_NSL2_TBL_NUM)
           {
                LSC_ERR("0: SL2 idx(%d) exceeds NVRAM_SL2_TBL_NUM", m_u4CTIdx);
                return;
           }
            break;
        case 1:
            ...
            if (i4Idx >= NVRAM_NSL2_TBL_NUM)
            {
                 LSC_ERR("1: SL2 idx(%d) exceeds NVRAM_SL2_TBL_NUM", m_u4CTIdx);
                return;
            }
            break;
        case 2:
           ...
           if (i4Idx >= NVRAM_NSL2_TBL_NUM)
            {
                LSC_ERR("2: SL2 idx(%d) exceeds NVRAM_SL2_TBL_NUM", m_u4CTIdx);
                return;
             }
            break;
        }
       ...
       if (getTsfOnOff())
       {
            m_rSl2CfgF = m_pNvramOtp->getIspNvram()->ISPRegs.NSL2[i4Idx];
            m_rSl2CfgF.cen.bits.SL2_CENTR_X = m_rSl2Cfg.i4CenterX;
            m_rSl2CfgF.cen.bits.SL2_CENTR_Y = m_rSl2Cfg.i4CenterY;
            m_rSl2CfgF.rr_con0.bits.SL2_R_0 = m_rSl2Cfg.i4R0;
            m_rSl2CfgF.rr_con0.bits.SL2_R_1 = m_rSl2Cfg.i4R1;
            m_rSl2CfgF.rr_con1.bits.SL2_R_2 = m_rSl2Cfg.i4R2;
        }
        else
            m_rSl2CfgF = m_pNvramOtp->getIspNvram()->ISPRegs.NSL2[i4Idx];

        LSC_LOG_IF(g_LogEnable,"[%s] SL2 OnOff(%d), Idx(%d)", __FUNCTION__, fgSl2OnOff, i4Idx);
        LSC_LOG_IF(g_LogEnable,"[%s] SL2 cen(0x%08x), rr0(0x%08x), rr1(0x%08x), gain(0x%08x)",
            __FUNCTION__, m_rSl2CfgF.cen, m_rSl2CfgF.rr_con0, m_rSl2CfgF.rr_con1, m_rSl2CfgF.gain);
        if (NSIspTuningv3::IspTuningMgr::getInstance().getOperMode(m_eSensorDev) != NSIspTuning::EOperMode_Meta)
        {
            ...
            ISP_MGR_NSL2_T::getInstance(m_eSensorDev).put(m_rSl2CfgF);
        }
