...
dumpRegInfoP1(char const*const pModuleName)
{
    ...
    if (m_bDebugEnable || fgModuleDbg) {
        ...
        for (MUINT32 i = 0; i < m_u4RegInfoNum; i++) {
            MY_ERR("%s: [addr] 0x%08x [value] 0x%08x", pModuleName, pRegInfo[i].addr, pRegInfo[i].val);
        }
    }
    if (pP1Reg != NULL)
    {
       writeRegs(static_cast<RegInfo_T*>(m_pRegInfo), m_u4RegInfoNum, static_cast<isp_reg_t*>(pP1Reg), u4P1RegSize);
    } else {
