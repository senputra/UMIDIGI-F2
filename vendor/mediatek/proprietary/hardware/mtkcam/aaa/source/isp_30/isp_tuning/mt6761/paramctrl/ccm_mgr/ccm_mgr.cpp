...
createInstance(ESensorDev_T const eSensorDev, ISP_NVRAM_REGISTER_STRUCT& rIspNvramReg, ISP_NVRAM_MULTI_CCM_STRUCT& rISPMultiCCM, IspTuningCustom* pIspTuningCustom)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<CcmMgr>(eSensorDev, rIspNvramReg, rISPMultiCCM, pIspTuningCustom);
    } );
...
calculateCCM(AWB_INFO_T const& rAWBInfo, MBOOL bWithPreferenceGain, MINT32 i4FlashOnOff, MINT32 i4SmoothMethod, MBOOL bRWBSensor, MINT32 ProMehod, MINT32 ProMax)
{
    ...
    if (bWithPreferenceGain)
        SmoothCCM(rAWBInfo.rCurrentAWBGain, 3, 9, m_rISPMultiCCM, m_rCCMOutput, m_rCCMWeight, i4FlashOnOff, i4SmoothMethod, ProMehod, ProMax);
    else
        SmoothCCM(rAWBInfo.rCurrentFullAWBGain, 3, 9, m_rISPMultiCCM, m_rCCMOutput, m_rCCMWeight, i4FlashOnOff, i4SmoothMethod, ProMehod, ProMax);
    }
