...
static std::array<std::array<MY_INST_BNR, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonBNR;

#define MY_INST_BNR2 NS3Av3::INST_T<ISP_MGR_BNR2_T>
static std::array<std::array<MY_INST_BNR2, SENSOR_IDX_MAX>, EISP_MGR_REG_SET_NUM> gMultitonBNR2;
...
getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    MY_INST_BNR& rSingleton = gMultitonBNR[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_BNR_T>(eSensorDev, eSensorTG);
    } );
...
apply(EIspProfile_T eIspProfile)
{
    ...
    if (m_eSensorTG == ESensorTG_1) {
        ...
        // HW constraint
        reinterpret_cast<ISP_CAM_BNR_BPC_CON_T*>(REG_INFO_VALUE_PTR(CAM_BNR_BPC_CON))->BNR_EDGE = 15;
...
getInstance(ESensorDev_T const eSensorDev, ESensorTG_T const eSensorTG)
{
    ...
    MY_INST_BNR2& rSingleton = gMultitonBNR2[i4TGIdx][i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_BNR2_T>(eSensorDev, eSensorTG);
    } );
