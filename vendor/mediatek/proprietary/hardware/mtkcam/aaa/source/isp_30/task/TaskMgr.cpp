    ...
    switch(eCmd)
    {
        ...
        case ECmd_PrecaptureStart:
            ...
            if (i4FlashFlow == 0){
                ...
                vecTaskEnum.push_back(TASK_ENUM_3A_PRECAPTURE);
                CAM_LOGW("[%s] TaskEvent:aePrecaptureStart_Flash2, fgLampflashCond(%d)", __FUNCTION__, rOpt.fgLampflashCond);
            }
            break;
        ...
        case ECmd_TouchAEStart:
            CAM_LOGW("[%s] TaskEvent:TouchAEStart, fgLampflashCond(%d)/bCustEnableFlash(%d)/bStrobeBVTrigger(%d)/WithCheckFlashBack", __FUNCTION__,
    ...
    for(it = vecTaskEnum.begin(); it != vecTaskEnum.end(); it++)
    {
        ...
        if(bStartAction)
        {
            if(pTask == NULL)
            {
                ...
                CAM_LOGW("[%s] Create Task(%d)", __FUNCTION__, eTaskEnum);
            } else
            {
            ...
            {
                std::lock_guard<std::mutex> lock((*pTaskMutex));
                MINT32 i4Type = pTask->getInfo().i4Type;

                //===== AF different 2A, Because AF Task Object has racing codition. =====
                //===== If 3A thread call AF destroy, AF thread will can't call Task Object =====
                if(i4Type == TASK_TYPE_AF)
                {
                    //===== cancel task =====
                    pTask->cancel();
                    CAM_LOGW("[%s] cancel Task(%d)", __FUNCTION__, eTaskEnum);
                }
                else
                {
                    //===== delete task =====
...
TaskMgrImp::execute(TASK_UPDATE eUpdate)
{
    ...
    switch(eUpdate)
    {
        case Task_Update_3A:
        {
            ...
            // Sync2A Task
            if(m_vSyncTaskQueue.size() && !m_rTask3AData.bIsDualCamWithCamSv)
            {
...
TaskMgrImp::prepareOption()
{
    ...
    //===== Normal flow =====
    MINT32 i4Opt = 0;
    if(m_v3ATaskQueue.size() > 1 && !(m_v3ATaskQueue.size() == 2 && findTask(TASK_ENUM_3A_FLASH_BACK, m_v3ATaskQueue) != NULL)) // exist Touch/Precap/Flash/Capture task
    {
    ...
    if(ISync3AMgr::getInstance()->isActive() && !m_rTask3AData.bIsDualCamWithCamSv)
    {
...
TaskMgrImp::findHighPriorityTask(vector<ITask*> vTaskQueue)
{
    ...
    for(it = vTaskQueue.begin(); it != vTaskQueue.end(); it++)
    {
        ...
        if(temp->getInfo().i4Enum == TASK_ENUM_3A_CAPTURE)
        {
            taskList.push_back(temp);
            bIsSingleTask = 0;
        }
        else if(temp->getInfo().i4Priority > i4Priority)
        {
            pTask = temp;
