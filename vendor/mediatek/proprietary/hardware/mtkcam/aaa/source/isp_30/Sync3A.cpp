...
const uint32_t kAESyncSttBufSize = 1024 * 90;
static MUINT8 gMasterAESyncSttBuf[kAESyncSttBufSize];
static MUINT8 gSlaveAESyncSttBuf[kAESyncSttBufSize];
...
syncAeMain(const StatisticBufInfo* pMasterSttBuf, const StatisticBufInfo* pSlaveSttBuf)
{
    ...
    rAeSyncInput.main_ch.aao_buffer_info.pAEStatisticBuf = (MVOID*) gMasterAESyncSttBuf; // reinterpret_cast<MVOID *>(IAAOBufMgr::getInstance().getCurrHwBuf(m_i4Master));
    ...
    rAeSyncInput.sub_ch.aao_buffer_info.pAEStatisticBuf = (MVOID*) gSlaveAESyncSttBuf; //reinterpret_cast<MVOID *>(IAAOBufMgr::getInstance().getCurrHwBuf(m_i4Slave));
...
sync(MINT32 i4Sensor, MINT32 i4Param, MVOID* pSttBuf, MINT32 i4FrameNum)
{
    ...
    if(pSttBuf != NULL) {
        if(i4Sensor == m_i4Master) {
           ...
           const MUINT8* pSrc = reinterpret_cast<const MUINT8*>(m_pMasterSttBuf->mVa);
           memcpy(gMasterAESyncSttBuf, pSrc, m_pMasterSttBuf->mSize);
        }
        else if(i4Sensor == m_i4Slave) {
           m_pSlaveSttBuf = reinterpret_cast<StatisticBufInfo*>(pSttBuf);
           const MUINT8* pSrc = reinterpret_cast<const MUINT8*>(m_pSlaveSttBuf->mVa);
           memcpy(gSlaveAESyncSttBuf, pSrc, m_pSlaveSttBuf->mSize);
        }
    }
