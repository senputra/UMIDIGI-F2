...
#define MY_INST_CFA NS3Av3::INST_T<ISP_MGR_CFA_T>
static std::array<MY_INST_CFA, SENSOR_IDX_MAX> gMultitonCFA;
...
getInstance(ESensorDev_T const eSensorDev)
{
    ...
    MY_INST_CFA& rSingleton = gMultitonCFA[i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_CFA_T>(eSensorDev, eSensorTG);
    } );
...
apply(EIspProfile_T eIspProfile, isp_reg_t* pReg)
{
    ...
    // TOP ==> don't care
    ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN_P2, CFA_EN, 1);
