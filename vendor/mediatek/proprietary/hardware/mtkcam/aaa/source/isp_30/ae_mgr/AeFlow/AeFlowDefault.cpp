...
schedulePvAE(MVOID* input, MVOID* output)
{
    ...
    if (m_bAAASchedule)
    {
        // Stereo case
        m_u4AEScheduleCnt = 0;
        MINT32 i4ActiveItem = pinputinfo->i4ActiveItem;
        bCalculateAE = i4ActiveItem & E_AE_AE_CALC;
        bApplyAE     = i4ActiveItem & E_AE_AE_APPLY;
        m_bPerframeAEFlag = MFALSE;
        AE_FLOW_LOG("[monitorAndReschedule:Stereo-3A-Schedule] ActiveItem:%d Calc:%d Apply:%d Magic:%d AEMonitorStable:%d VdCnt:%d u4CwvY(cur/pre):%d/%d\n",
...
calculateAE(MVOID* input, MVOID* output)
{
    ...
    switch(pinputinfo->eAEFlowType)
    {
        ...
        case E_AE_FLOW_CPU_DOAFAE:
            ...
            {
                rAEInput.eAeState = AE_STATE_TOUCH_PERFRAME;
                AE_FLOW_LOG("[doAFAE] PerFrame touch AE state\n");
            }
            else
            {
                rAEInput.eAeState = AE_STATE_ONE_SHOT;
    ...
    AE_FLOW_LOG_IF(m_3ALogEnable,"[%s()] i4MagicNum:%d i4IsAFDone:%d i4AfDac:%d i4AF2AE_IsSceneStable:%d\n", __FUNCTION__,
        pinputinfo->i4AF2AE_MagicNum, pinputinfo->i4AF2AE_IsAFDone, pinputinfo->i4AF2AE_AfDac, pinputinfo->i4AF2AE_IsSceneStable);
    ...
    if ((pinputinfo->u4AEMode == LIB3A_AE_MODE_OFF) && ((pinputinfo->u4TargetExp == 0) || (pinputinfo->u4TargetISO == 0)))
    {
        ...
        if (strExpOutput.bOverSpec == MTRUE)
        {
            MUINT32 u4IndexMax = 0, u4IndexMin = 0, u4Index = 0;
            ...
            AE_FLOW_LOG("[%s()] u4Index:%d u4IndexMin:%d u4IndexMax:%d i4DeltaIndex:%d\n", __FUNCTION__, u4Index, u4IndexMin, u4IndexMax, strExpOutput.i4DeltaIndex);
            if (m_pIAeAlgo != NULL)
            {
                m_pIAeAlgo->setEVIdxInfo(u4IndexMax ,u4IndexMin ,u4Index);
            }
    ...
    {   // enable adb log
        ...
        {
            AE_FLOW_LOG("[%s()] The AE algo class is NULL, i4SensorDev:%d line:%d\n", __FUNCTION__, m_eSensorDev, __LINE__);
        }
    }
}
...
monitorAndReschedule(MBOOL bFaceAEAreaChage, MBOOL bTouchAEAreaChage, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MUINT32 u4WOFDCnt, MBOOL *bAECalc, MBOOL *bAEApply)
{
    ...
    MBOOL bReschedule = (m_bAEMonitorStable && (bStatChange || (bFaceAEAreaChage /*&&(u4WOFDCnt > m_pAEParam->pFaceSmooth->u4FD_Lost_MaxCnt)*/) || bTouchAEAreaChage));
    //AE Exit condition
