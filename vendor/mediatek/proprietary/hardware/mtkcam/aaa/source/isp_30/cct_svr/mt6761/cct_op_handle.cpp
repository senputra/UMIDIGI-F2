...
char gb_MT6761_ISP_ID_TEXT[ISP_ID_TEXT_LENGTH] = "6761";
...
{
    ISP_MGR_NR3D_T::getInstance((ESensorDev_T)m_eSensorEnum).setEnable(fgOn);
    NSIspTuningv3::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOG("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    return ISP_MGR_NR3D_T::getInstance((ESensorDev_T)m_eSensorEnum).isEnable();
}
    ...
    case FT_CCT_OP_GET_ID:
        ...
        if ( ISP_ID_TEXT_LENGTH != u4ParaOutLen || ! pu4RealParaOutLen || ! puParaOut ) {
            ...
            *pu4RealParaOutLen = 0;
            ...
            strncpy((char *)puParaOut, gb_MT6761_ISP_ID_TEXT, ISP_ID_TEXT_LENGTH);
    ...
    case FT_CCT_OP_AE_BRECKET_STEP:
        *pu4RealParaOutLen = 0;
        ...
        if(u4ParaInLen == sizeof(MINT32) && puParaIn != NULL)
        {
            ...
            strSensorParams.u8ExposureTime  = ( MUINT32 )( ( MINT64 ) aeOutput.EvSetting.u4Eposuretime*1000);
...
{
    MY_LOG("[%s] LEGACY FUNCTION ", __FUNCTION__);
    return  CCTIF_NO_ERROR;
}
