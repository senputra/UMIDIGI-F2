...
PDMgr::PDMgr(MINT32 const i4SensorDev) :
    ...
    m_i4NVRamIdx(AF_CAM_SCENARIO_NUM),
                ...
                if(m_pIHalSensor)
                {
                    ...
                    if(!err)
                    {
                        ...
                        else if( m_profile.u4PDAF_support_type==PDAF_SUPPORT_RAW_LEGACY)
                        {
                            ...
                            MBOOL bIsLowRamDevice = ::property_get_bool("ro.config.low_ram", MFALSE);
                            MINT32 i4ProperitaryRequest = 0;

                            if (!QUERY_ENTRY_SINGLE(rConfigInfo.CfgAppMeta, MTK_CONFIGURE_SETTING_PROPRIETARY, i4ProperitaryRequest))
                            {
                                i4ProperitaryRequest = 0;
                            }

                            CAM_LOGW("[%s] Support PDAF : IsLowRamDevice(%d), ProperitaryRequest(%d)",
                                     __FUNCTION__, bIsLowRamDevice, i4ProperitaryRequest);

                            if (bIsLowRamDevice == MTRUE && i4ProperitaryRequest == 0)
                            {
                                CAM_LOGW("[%s] disable PDAF due to low ram device", __FUNCTION__);
                                oPDAFStatus = FEATURE_PDAF_UNSUPPORT;
                                m_profile.bSensorModeSupportPD = 0;
                                m_profile.u4PDAF_support_type = PDAF_SUPPORT_NA;
                            }
                        }
                        else
                        {
...
{
    if(afNVRamIdx<AF_CAM_SCENARIO_NUM)
    {
            ...
            if( m_profile.bSensorModeSupportPD)
            {
                ...
                if(_err ==0 && m_pLensNVRam)
                {
                    ...
                    if(m_bDebugEnable)
                    {
                        ...
                        {
                            CAM_LOGD_IF(m_bDebugEnable, "DBG : Lens NVRAM #%d, File Path: %s", i, m_pLensNVRam->rLENSNVRAM[i].rFilePath);
                        }
                    }
                    ...
                    if( afNVRamIdx<AF_CAM_SCENARIO_NUM)
                    {
                        //
                        m_i4NVRamIdx = afNVRamIdx;
                        m_pNVRam     = &(m_pLensNVRam->rLENSNVRAM[m_i4NVRamIdx]);
    ...
    if( m_pIPdAlgo && m_pAnalysisData_Calculate && pResBuf)
    {
            ...
            (     m_pAnalysisData_Calculate->numDa<=MAX_SIZE_OF_PD_ROI) )
        {
            ...
            if (!m_i4DbgDisPdHandle)
            {
                ...
                PD_OUTPUT_T sHandlePDOutputList[AF_PSUBWIN_NUM];
                ...
                for (MUINT32 i=0; i<m_pAnalysisData_Calculate->numDa; i++)
                {
                    ...
                    ret = m_pIPdAlgo->handlePD(m_pAnalysisData_Calculate->Da[i].sHandlePDInput, sHandlePDOutputList[i]);
                    PD_TRACE_FMT_END(TRACE_LEVEL_DEF);
                }
            ...
            {
                {
                    ...
                    if(m_i4NVRamIdx<AF_CAM_SCENARIO_NUM)
                    {
                        NVRAM_LENS_DATA_PARA_STRUCT *pNVRam = &(m_pLensNVRam->rLENSNVRAM[m_i4NVRamIdx]);
                                ...
                                if(sPDExtractInput.pRawBuf || sPDExtractInput.pPDBuf)
                                {
                                    ...
                                    for( MUINT32 i=0; i<pAnalysisData->numDa; i++)
                                    {
                                        ...
                                        ret = m_pIPdAlgo->extractPD(
                                                  sExtPDInputList[i],
                                                  pAnalysisData->Da[i].sHandlePDInput.sPDExtractData);
                                    PD_TRACE_FMT_END(TRACE_LEVEL_DEF);
                                    }
