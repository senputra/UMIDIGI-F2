...
calculateAE(MVOID* input, MVOID* output)
{
    ...
    if (m_eAETargetMode == AE_MODE_IVHDR_TARGET){
        updatePreviewParamsByiVHDR(pinputinfo->bIs60Hz, poutputinfo);
    } else if (m_eAETargetMode == AE_MODE_MVHDR_TARGET){
        updatePreviewParamsBymVHDR(pinputinfo->bIs60Hz, poutputinfo);
    } else if (m_eAETargetMode == AE_MODE_ZVHDR_TARGET){
        updatePreviewParamsByzVHDR(pinputinfo->bIs60Hz, poutputinfo);
    }
...
MVOID
AeFlowVHDR::
updatePreviewParamsByiVHDR(MBOOL bIs60Hz, strAEOutput *sAEInfo)
{
    ...
    strIVHDRInputSetting.bIs60HZ = bIs60Hz;
...
MVOID
AeFlowVHDR::
updatePreviewParamsBymVHDR(MBOOL bIs60Hz, strAEOutput *sAEInfo)
{
    IVHDRExpSettingInputParam_T strIVHDRInputSetting;
...
MVOID
AeFlowVHDR::
updatePreviewParamsByzVHDR(MBOOL bIs60Hz, strAEOutput *sAEInfo)
{
   (void)bIs60Hz;
   (void)sAEInfo;
}
