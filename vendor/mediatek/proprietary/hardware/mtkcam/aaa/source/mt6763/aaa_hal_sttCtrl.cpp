...
{
    // Check AFO enable or not by normal pipe information
    MBOOL enable_AFO = MTRUE;

    {
        NormalPipe_QueryInfo qry;
        MBOOL bIsTwinON = MFALSE;
        INormalPipeModule::get()->query(0, ENPipeQueryCmd_D_Twin,0,0, qry);
        INormalPipe* pPipe = (INormalPipe*)INormalPipeUtils::get()->createDefaultNormalPipe(m_i4SensorIdx, "CCU_AF");
        pPipe->sendCommand( NSCam::NSIoPipe::NSCamIOPipe::ENPipeCmd_GET_DTwin_INFO, (MINTPTR)(&bIsTwinON), 0, 0);
        pPipe->destroyInstance("CCU_AF");

        MUINT32 IsSupportCCU_AF = 0;

        CAM_LOGD("[%s] IsSupportCCU_AF(%d), D_TWIN(%d), bIsTwinON(%d)", __FUNCTION__, IsSupportCCU_AF, qry.D_TWIN, bIsTwinON);

        if ((property_get_int32("vendor.debug.af_ccu.enable", 0) > 0) ||
            ((qry.D_TWIN > 0) && (bIsTwinON > 0)))
        {
            IsSupportCCU_AF = 1;
            CAM_LOGD("[%s] Change CCU_AF(%d)", __FUNCTION__, IsSupportCCU_AF);
        }
        enable_AFO = (IsSupportCCU_AF > 0) ? 0 : 1;
    }
...
querySensorStaticInfo()
{
    ...
    IHalSensor* pIHalSensor __unused  = pIHalSensorList->createSensor( "sttCtrl", m_i4SensorDev);
    if (!pHalSensorList)
    {
