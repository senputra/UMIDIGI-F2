...
allocateBuf(MUINT32 const i4BufSize)
{
    ...
    if(m_rBufInfo.mVa == (MUINTPTR)NULL || m_i4TotalBufSize == 0){
        CAM_LOGD_IF(m_fgLog, "[%s] before m_rBufInfo->mVa = 0x%d, \n", __FUNCTION__, (MINT32)m_rBufInfo.mVa);
        m_i4TotalBufSize = i4BufSize;
        m_rBufInfo.mVa = (MUINTPTR) new MUINT8[m_i4TotalBufSize + 1];
        CAM_LOGD_IF(m_fgLog, "[%s] after  m_rBufInfo->mVa = 0x%d, \n", __FUNCTION__, (MINT32)m_rBufInfo.mVa);
    }
    return MFALSE;
}
...
freeBuf()
{
    Mutex::Autolock lock(m_Lock);
    if(m_rBufInfo.mVa != (MUINTPTR)NULL && m_i4TotalBufSize != 0){
        CAM_LOGD_IF(m_fgLog, "[%s] m_rBufInfo->mVa = 0x%d, \n", __FUNCTION__, (MINT32)m_rBufInfo.mVa);
...
{
    Mutex::Autolock lock(m_Lock);
    if(m_rBufInfo.mVa == (MUINTPTR)NULL){
