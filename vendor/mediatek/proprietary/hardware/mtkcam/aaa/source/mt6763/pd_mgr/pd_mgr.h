...
public:
     ...
     * @brief start pd mgr.
     */
    EPDBUF_TYPE_t start(MINT32 i4SensorIdx, SPDProfile_t *iPDProfile, MVOID *ptrInTuningData, MINT32 &i4OutInfoSz, MINT32 **ptrOutInfo);
    /**
     ...
     */
    MRESULT UpdatePDParam( MUINT32 u4FrmNum, MINT32 i4InputPDAreaNum, AREA_T *tInputPDArea);
...
private:
     ...
     * @brief pd convert
     */
    MUINT16* PDConvert(MUINT8* pInBuf, MUINT32 inBufSize, MUINT32 inBufStride);
    /**
     * @brief pd core flow
     */
    MRESULT PDCoreFlow();
     ...
     */
    MRESULT SetCaliData2PDCore(MVOID* ptrInTuningData, MVOID* ptrInCaliData, MINT32 i4InPDInfoSz, MINT *ptrOutPDInfo);
    /**
     * @brief set pd calibration data.
     */
    MRESULT setPDCaliData( MVOID *ptrInTuningData, MINT32 &i4OutInfoSz, MINT32 **ptrOutInfo);
    /**
     ...
     */
    MRESULT SetCaliData2PDOpenCore(MVOID* ptrInTuningData, MVOID* ptrInCaliData, MINT32 i4InPDInfoSz, MINT32 *ptrOutPDInfo);
    /**
     * @brief config PD hw module setting, need to be called after setPDCaliData.
     */
    MRESULT ConfigurePDHWSetting();
    /**
     * @brief operator
     */
     ...
     * @brief PDO
     */
    MBOOL initMemBuf( IMEM_BUF_INFO& rBufInfo, MUINT32 const u4Size);
    MBOOL uninitMemBuf( IMEM_BUF_INFO &rBufInfo);
    MBOOL allocMemBuf( IMEM_BUF_INFO &rBufInfo, MUINT32 const u4Size);
    MBOOL freeMemBuf( IMEM_BUF_INFO &rBufInfo);
...
private:
    ...
    PD_EXTRACT_DATA_T   m_sPDExtractOutput[AF_PSUBWIN_NUM];
    PD_OUTPUT_T         m_sPDOutput[AF_PSUBWIN_NUM];
    PD_INPUT_T          m_sPDInput[AF_PSUBWIN_NUM];

    // 3rd PD library flow control
    vector<PDBUF_CFG_T> m_vecCfgSetting;

    // flow control
    SPDOHWINFO_T     m_sPDOHWInfo;
    SDUALPDVCINFO_T  m_sDualPDVCInfo;
    IMem3A          *m_pIMemDrv;
    IMEM_BUF_INFO    m_rMemBufInfo;
    MUINT8          *m_pPDORes;
