    ...
    else if( (m_profile.u4PDPipeCtrl == FEATURE_PDAF_SUPPORT_PBN_PDO) || (m_profile.u4PDPipeCtrl == FEATURE_PDAF_SUPPORT_BNR_PDO))
    {
        ...
        if( PDAF_support_type == PDAF_SUPPORT_RAW)
        {
            ...
            if( m_profile.i4SensorMode==SENSOR_SCENARIO_ID_NORMAL_CAPTURE)
                ...
                tbl_idx = eIDX_PDC_2;
            else if( m_profile.i4SensorMode==SENSOR_SCENARIO_ID_NORMAL_PREVIEW)
                tbl_idx = eIDX_PDC_3;
            else
...
MBOOL PDMgr::initMemBuf( IMEM_BUF_INFO &rBufInfo, MUINT32 const u4Size)
{
    ...
    if( !m_pIMemDrv)
    {
        ...
        else
        {
            ...
            if (ret == MTRUE)
            {
                ...
                {
                    CAM_LOGD( "sensor(0x%02x), memID(%d), virtAddr(0x%p), phyAddr(0x%p), size(%d)",
                              m_i4CurrSensorDev,
                              m_rMemBufInfo.memID,
                              (void*)m_rMemBufInfo.virtAddr,
                              (void*)m_rMemBufInfo.phyAddr,
                              m_rMemBufInfo.size);
                }
            }
            else
    ...
    {
        CAM_LOGD( "m_pIMemDrv(0x%p) already exists.", (void*)m_pIMemDrv);
    }
...
MBOOL PDMgr::allocMemBuf( IMEM_BUF_INFO& rBufInfo, MUINT32 const u4Size)
{
    ...
    if( !rBufInfo.virtAddr)
    {
        ...
        if( 0 == m_pIMemDrv->allocVirtBuf(&rBufInfo))
        {
            ...
            {
                CAM_LOGE( "mapPhyAddr error, virtAddr(0x%p), size(%d)\n", (void*)(rBufInfo.virtAddr), rBufInfo.size);
                ret = MFALSE;
            }
            else
            {
    ...
    {
        ret = MTRUE;
        CAM_LOGD( "Already Exist! virtAddr(0x%p), size(%d)\n", (void*)(rBufInfo.virtAddr), u4Size);
    }
    return ret;
}
//----------------------------------------------------------------------------------------------------
MBOOL PDMgr::freeMemBuf( IMEM_BUF_INFO& rBufInfo)
{
    ...
    if( 0 == m_pIMemDrv->unmapPhyAddr(&rBufInfo))
    {
        ...
        {
            CAM_LOGD("freeVirtBuf OK, memID(%d), virtAddr(0x%p), phyAddr(0x%p)", rBufInfo.memID, (void*)(rBufInfo.virtAddr), (void*)(rBufInfo.phyAddr));
            rBufInfo.virtAddr = 0;
            ret = MTRUE;
        }
        else
        {
            CAM_LOGD("freeVirtBuf Fail, memID(%d), virtAddr(0x%p), phyAddr(0x%p)", rBufInfo.memID, (void*)(rBufInfo.virtAddr), (void*)(rBufInfo.phyAddr));
            ret = MFALSE;
        }
    }
    else
...
MRESULT PDMgr::SetCaliData2PDCore(MVOID* ptrInTuningData, MVOID* ptrInCaliData, MINT32 i4InPDInfoSz, MINT *ptrOutPDInfo)
{
    ...
    {
        CAM_LOGE("pIHalSensor is NULL!! Can not get PD_block_info and vc_info from sensor kernel driver");
    }

    CAM_LOGD( "SensorMode: %d(%d), SensorInfo : OffsetX(%d), OffsetY(%d), PitchX(%d), PitchY(%d), PairNu(%d), SubBlkW(%d), SubBlkH(%d), BlockNumX(%d), BlockNumY(%d), LeFirst(%d), MirrorFlip(%d)",
              sensorMode,
    ...
    {
        /* Condition checking */
        if( sPDSensorInfo.i4BlockNumX == 0 && sPDSensorInfo.i4BlockNumY == 0)
        {
            CAM_LOGE( "Please check block number setting in kernel driver: i4BlockNumX(%d) i4BlockNumY(%d)", sPDSensorInfo.i4BlockNumX, sPDSensorInfo.i4BlockNumY);
            sPDSensorInfo.i4BlockNumX = (m_profile.uImgXsz-2*sPDSensorInfo.i4OffsetX)/sPDSensorInfo.i4PitchX;
            sPDSensorInfo.i4BlockNumY = (m_profile.uImgYsz-2*sPDSensorInfo.i4OffsetY)/sPDSensorInfo.i4PitchY;
        }

        if( sPDSensorInfo.i4PairNum > MAX_PD_PAIR_NUM)
        {
            CAM_LOGE( "Please check pair number setting in kernel driver: i4PairNum(%d)", sPDSensorInfo.i4PairNum);
            sPDSensorInfo.i4PairNum = MAX_PD_PAIR_NUM;
        }
        ...
        CAM_LOGD( "[Core] bits(%d) isPack(%d) w(%d) h(%d) stride(%d) blknumX(%d) blknumY(%d) OffX(%d) OffY(%d) pairNum(%d) pitchX(%d) pitchY(%d) subblkH(%d) subblkW(%d) crop(%d %d %d %d)",
                  a_sPDConfig.i4Bits,
                  ...
                  a_sPDConfig.i4FullRawXOffset,
                  a_sPDConfig.i4FullRawYOffset,
                  a_sPDConfig.i4FullRawWidth,
                  a_sPDConfig.i4FullRawHeight);
    }
    else //dualpd related
    {
    ...
    m_pPDBufMgrCore->SetPDBlockInfo( sPDSensorInfo, m_profile);
...
MRESULT PDMgr::PDOpenCoreFlow()
{
        ...
        (    m_curLensPos<=1023))
    {
        ...
        AAA_TRACE_D("GET_CUR_SETTING_INFO");
    ...
    if( m_bEnPDBufThd)
    {
        if( ptrInputData)
        {
            ...
            if( Val==1)
            {
                ...
                CAM_LOGD_IF( m_bDebugEnable, "[%s] Input buffer(va:%p sz:0x%x stride:0x%x AGain:%d iso:%d lens:%d); LRBuf(%p)",
                    __FUNCTION__,
