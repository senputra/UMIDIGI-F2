...
{
    shading_tbl_dbg = property_get_int32("vendor.debug.shading.lsc_tbl", 0);
...
public:
    ...
    virtual MBOOL                   cropToGain(const TransformCfg_T& trfm, ILscTable& output, vector<MUINT16*> &vecGain, MUINT32 mode) const {return MFALSE;}
...
public:
    ...
    virtual MBOOL                   cropToGain(const ILscTable::TransformCfg_T& trfm, ILscTable& output, vector<MUINT16*> &vecGain, MUINT32 mode) const {return MFALSE;}
    virtual MBOOL                   convert(ILscTable& output) const;
};
