...
//static CaliData g_CaliDataDark[2];  /* fix build warning */
...
#define STATE_END2 6
    ...
    static int typeId = getStrobeType(m_sensorDev);
    if(m_pfFrameCount==0){
...
logI("cctCalibration st=%d ln=%d",state, __LINE__);
    ...
    if(state==STATE_INIT)
    {
        ...
        int calIsp;
        int calAfe;
        calExp = FlashUtil::getPropInt("vendor.flash_cal_exp",-1);
        calIsp = FlashUtil::getPropInt("vendor.flash_cal_isp",-1);
        calAfe = FlashUtil::getPropInt("vendor.flash_cal_afe",-1);
...
NextId:
        if(m_pfFrameCount==preStateEndCnt+1)
        {
            ...
            if(g_dutyArr[curId]==-1 && g_dutyArrLt[curId]==-1)
            {
                frameTar -= 25;
            }
        ...
        if(curId>=(int)g_dutyArr.size())
        {
            if(ratioRound==1)
            {
                ...
                else
                {
                    ...
                    for(i=0;i<sz;i++)
                        dd[i]=g_CaliData2[i];
                    qsort(dd, sz, sizeof(CaliData), cmpCaliData);
                    for(i=0;i<sz;i++)
                        g_CaliData2[i]=dd[i];
                    delete []dd;
                    for(i=0;i<sz;i++)
...
//                preStateEndCnt = m_pfFrameCount;
    ...
    else if(state==STATE_END)
    {
        ...
        if(maxYTest<100)
            g_flashQuickCalibrationResult = E_FLASH_CALI_ENVIRONMENT_WRONG_Y_IS_LOW_ERR;
    ...
    logI("CalNum = %d\n", calTestN);
       ...
       nv3A = getNv3ABuf();
        ...
        short ITab1m[40];
        short ITab2m[40];
        memset(ITab1, 0, sizeof(ITab1));
        memset(ITab2, 0, sizeof(ITab2));
        memset(ITab1m, 0, sizeof(ITab1m));
        memset(ITab2m, 0, sizeof(ITab2m));
...
//          e_errBrightness_environmentNotDark,
    //        e_errBrightness_overExposure,
        ...
        else
        {
            ...
            else if(g_flashQuickCalibrationResult==E_FLASH_CALI_ENVIRONMENT_WRONG_Y_IS_LOW_ERR)
                {
                fprintf(fpResult, "@err_detail E_FLASH_CALI_ENVIRONMENT_WRONG_Y_IS_LOW_ERR\n");
                fprintf(fpResult, "1. Environment may not correct.\n");
                fprintf(fpResult, "2. init exp may not correct. set correct value to cust_getFlashQuick2CalibrationExp_main()\n");
            }
