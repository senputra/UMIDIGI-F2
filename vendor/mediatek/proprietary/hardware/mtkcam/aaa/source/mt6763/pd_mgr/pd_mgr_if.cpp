...
EPDBUF_TYPE_t IPDMgr::start(MINT32 i4SensorDev, MINT32 i4SensorIdx, SPDProfile_t *iPDProfile, MVOID *ptrInTuningData, MINT32 &i4OutInfoSz, MINT32 **ptrOutInfo)
{
    if (i4SensorDev & ESensorDev_Main)
        return PDMgr::getInstance(ESensorDev_Main).start(i4SensorIdx, iPDProfile, ptrInTuningData, i4OutInfoSz, ptrOutInfo);
    if (i4SensorDev & ESensorDev_Sub)
        return PDMgr::getInstance(ESensorDev_Sub).start(i4SensorIdx, iPDProfile, ptrInTuningData, i4OutInfoSz, ptrOutInfo);
    if (i4SensorDev & ESensorDev_MainSecond)
        return PDMgr::getInstance(ESensorDev_MainSecond).start(i4SensorIdx, iPDProfile, ptrInTuningData, i4OutInfoSz, ptrOutInfo);
    if (i4SensorDev & ESensorDev_SubSecond)
        return PDMgr::getInstance(ESensorDev_SubSecond).start(i4SensorIdx, iPDProfile, ptrInTuningData, i4OutInfoSz, ptrOutInfo);
    return PDMgr::getInstance(ESensorDev_Main).start(i4SensorIdx, iPDProfile, ptrInTuningData, i4OutInfoSz, ptrOutInfo);
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IPDMgr::preStop(MINT32 sensorDev)
{
    if (sensorDev & ESensorDev_Main)
        return PDMgr::getInstance(ESensorDev_Main).preStop();
    if (sensorDev & ESensorDev_Sub)
        return PDMgr::getInstance(ESensorDev_Sub).preStop();
    if (sensorDev & ESensorDev_MainSecond)
        return PDMgr::getInstance(ESensorDev_MainSecond).preStop();
    if (sensorDev & ESensorDev_SubSecond)
        return PDMgr::getInstance(ESensorDev_SubSecond).preStop();
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IPDMgr::UpdatePDParam(MINT32 i4SensorDev, MUINT32 u4FrmNum, MINT32 i4InputPDAreaNum, AREA_T *tInputPDArea)
{
    if (i4SensorDev & ESensorDev_Main)
        return PDMgr::getInstance(ESensorDev_Main).UpdatePDParam(u4FrmNum, i4InputPDAreaNum, tInputPDArea);
    if (i4SensorDev & ESensorDev_Sub)
        return PDMgr::getInstance(ESensorDev_Sub).UpdatePDParam(u4FrmNum, i4InputPDAreaNum, tInputPDArea);
    if (i4SensorDev & ESensorDev_MainSecond)
        return PDMgr::getInstance(ESensorDev_MainSecond).UpdatePDParam(u4FrmNum, i4InputPDAreaNum, tInputPDArea);
    if (i4SensorDev & ESensorDev_SubSecond)
        return PDMgr::getInstance(ESensorDev_SubSecond).UpdatePDParam(u4FrmNum, i4InputPDAreaNum, tInputPDArea);
