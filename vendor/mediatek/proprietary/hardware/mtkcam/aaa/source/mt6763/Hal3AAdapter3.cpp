...
#define _TEST_TONEMAP2      (1<<11)
    ...
    //MUINT32 u4Magic = rP2Param.i4MagicNum;  /* fix build warning */
...
*/
    ...
    ::memset(&mBackupCamInfo, 0, sizeof(mBackupCamInfo));
    mBackupCamInfo_copied = MFALSE;
...
convert3AROI(const android::Vector<MINT32> &vec3AROI, MetaSet_T& rMetaResult, const mtk_camera_metadata_tag_t &tag) const
{
    ...
    //MUINT32 u4SensorDev = mu4SensorDev;  /* fix build warning */
...
{
    (void)i4StartNum;
    MY_LOGD("[%s] Middleware StartCapture by vector size(%zu)", __FUNCTION__, requestQ.size());

    android::List<MetaSet_T> adapter; // carry size
    if (requestQ.size())
    {
        MetaSet_T dummy;
        adapter.push_back(dummy);
        adapter.push_back(dummy);
        adapter.push_back(*requestQ[0]);
        return startCapture(adapter, adapter.size());
    }
...
{
    android::List<MetaSet_T> adapter;
    for (MetaSet_T* it : requestQ)
        adapter.push_back(*it);

    return startRequestQ(adapter, adapter.size());
}
    ...
    mpHal3aObj->sendCommand(NS3Av3::ECmd_CameraPreviewStart);
    {
        ...
        for (int i = 0; i < getCapacity() && i < i4RequestSize; i++, it++)
        {
            if (i == HAL3A_REQ_PROC_KEY)
            {
                ...
                QUERY_ENTRY_SINGLE(it->halMeta, MTK_3A_SKIP_HIGH_QUALITY_CAPTURE, u1IsSkipHQC);
...
#define REQ_SIZE 3
    ...
    // remove other request to align the size = 3 for set function
    MINT32 removeSize = 0;
    android::List<MetaSet_T> newRequestQ;
    if(i4RequestSize > REQ_SIZE)
        removeSize = i4RequestSize - REQ_SIZE;

    MetaSetList_T::const_iterator it = requestQ.begin();
    for (MINT32 i = 0; i < i4RequestSize; i++, it++)
    {
        // if the size of metadata list > 3, then resize metadata list to 3 element.
        if(removeSize != 0 && i >= removeSize)
        {
            MetaSet_T rMetaResult =(*it);
            MY_LOGD("[%s] push_back rMetaResult magic(%d)", __FUNCTION__, rMetaResult.MagicNum);
            if(i == (i4RequestSize-1) )
            {
                ...
                MY_LOGD("[%s] MTK_CONTROL_AE_MODE(%d), MTK_HAL_REQUEST_REPEAT(%d)", __FUNCTION__, u1AeMode, uRepeatTag);
            }
        ...
        if(i == (i4RequestSize-1) )
        {
            if(removeSize != 0)
            {
                MY_LOGD("[%s] newRequestQ.size(%zu)", __FUNCTION__, newRequestQ.size());
                set(newRequestQ, newRequestQ.size());
            }
            else
            set(requestQ, i4RequestSize);
        }
                    ...
                    case MTK_CONTROL_AE_ANTIBANDING_MODE:
                        {
                            ...
                            MUINT32 u4Cmp = ((MUINT32)i4Mode != mParams.u4AntiBandingMode) ? 1 : 0;
...
get(MUINT32 frmId, MetaSet_T& result)
{
    ...
    //MBOOL fgLogEn0 = mu4LogEn & HAL3AADAPTER3_LOG_GET_0;  /* fix build warning */
    //MBOOL fgLogEn1 = mu4LogEn & HAL3AADAPTER3_LOG_GET_1;  /* fix build warning */
    ...
    if (-1 == mpHal3aObj->getResult(frmId, mResult))
    {
    ...
    for (; it != mMetaResultQueue.end(); it++)
    {
        ...
        if (frmId == (MUINT32)u4Id)
        {
            ...
            if (i4ret != MTRUE) //should update non repeat metadata list
            {
                ...
                convertResultToMeta(mResult, result);
                AAA_TRACE_END_D;
                mMetaSet = result;
            }
            else {
...
setP2Params(P2Param_T &rNewP2Param, const MetaSet_T& control,ResultP2_T* pResultP2, NSIspTuning::ISP_INFO_T& rReprocInput)
{
    ...
    MBOOL fgLogEn0 = (mu4LogEn & HAL3AADAPTER3_LOG_SET_0) ? MTRUE : MFALSE;
    MBOOL fgLogEn1 = (mu4LogEn & HAL3AADAPTER3_LOG_SET_1) ? MTRUE : MFALSE;
    //MBOOL fgLogEn2 = (mu4LogEn & HAL3AADAPTER3_LOG_SET_2) ? MTRUE : MFALSE;  /* fix build warning */
    ...
    MINT32 i4Id;
    MUINT32 u4ManualMode = 0;
    //MINT32 i4BrightnessMode = 1;  /* fix build warning */
    //MINT32 i4ContrastMode = 1;  /* fix build warning */
    //MINT32 i4HueMode = 1;  /* fix build warning */
    //MINT32 i4SaturationMode = 1;  /* fix build warning */
    //MINT32 i4EdgeMode = 1;  /* fix build warning */
...
else
{
    ...
    // Restore caminfo
    if (pCaminfoBuf == NULL && mBackupCamInfo_copied)
    {
        MINT32 i4TgWidth = 0;
        MINT32 i4TgHeight = 0;
        mpHal3aObj->queryTgSize(i4TgWidth,i4TgHeight);

        mBackupCamInfo.rCropRzInfo.i4FullW = i4TgWidth;
        mBackupCamInfo.rCropRzInfo.i4FullH = i4TgHeight;

        pCaminfoBuf = &mBackupCamInfo;
        bBackupCaminfo = MFALSE;

        CAM_LOGD("[%s] Restore caminfo,size(%zu)/mode(%d)/profile(%d)/FrmId(%d)/TG(%d,%d)",__FUNCTION__, sizeof(mBackupCamInfo),
                mBackupCamInfo.eSensorMode,
                mBackupCamInfo.eIspProfile,
                 u4Id, i4TgWidth, i4TgHeight);
    }

    if (pCaminfoBuf && u1IspProfile != EIspProfile_YUV_Reprocess)
    {
        ...
        if (u1Exif)
        {
            ...
            if (pResult)
            {
                ...
                MINT32  i4DumpIspProfile = -1;
                GET_PROP("vendor.debug.camera.dump.p2.debuginfo", 0, u4DebugInfo);
                GET_PROP("vendor.debug.camera.dump.p2.profile", -1, i4DumpIspProfile);
                if (u4DebugInfo != 0 || u1DumpExif)
                {
                    ...
                    if( i4DumpIspProfile == -1 ||
                       (u4DebugInfo == 1 && i4DumpIspProfile == rIspInfo.rCamInfo.eIspProfile)||
                       (u4DebugInfo == 2 && i4DumpIspProfile != rIspInfo.rCamInfo.eIspProfile))
                    {
                ...
                MUINT32 dumpAAO = 0;
                GET_PROP("vendor.debug.camera.AAO.dump", 0, dumpAAO);
        ...
        // Backup caminfo
        if(bBackupCaminfo && pCaminfoBuf != NULL)
        {
            mBackupCamInfo_copied = 1;
            mBackupCamInfo = *pCaminfoBuf;
            MY_LOGD_IF(fgLog, "[%s] Backup caminfo,size(%zu)/mode(%d)/profile(%d)",__FUNCTION__, sizeof(mBackupCamInfo),
                mBackupCamInfo.eSensorMode,
                mBackupCamInfo.eIspProfile);
        }
    ...
    {
        MY_LOGD_IF(fgLog, "mpTuning(%p) pTuningBuf(%p)", mpTuning, pTuningBuf);
    ...
    if (pCaminfoBuf)
    {
        ...
        CAM_LOGW("[%s] Skip metadata update when no caminfo received", __FUNCTION__);
    }
...
setSensorMode(MINT32 i4SensorMode)
{
    ...
    if(mBackupCamInfo_copied)
    {
        mBackupCamInfo.eSensorMode = (ESensorMode_T)i4SensorMode;
