    ...
    case IspTuningMgr::E_ISPTUNING_NOTIFY_SENSOR_TYPE:
        {
             ...
             *********************************************************************************/
            m_u4PDFmtType = arg2;
            ISP_MGR_BNR_T::getInstance(m_eSensorDev).setPDSensorInfo(m_u4PDFmtType);
            ISP_MGR_BNR2_T::getInstance(m_eSensorDev).setPDSensorInfo(m_u4PDFmtType);
            ...
            if(!err)
            {
                res = ISP_MGR_BNR2_T::getInstance(m_eSensorDev).setBPCIBuf(buf);
            }
            else{
            ...
            if( m_u4PDFmtType==PDAF_SUPPORT_RAW ||
                m_u4PDFmtType==PDAF_SUPPORT_CAMSV ||
                m_u4PDFmtType==PDAF_SUPPORT_CAMSV_LEGACY)
            {
                if( err != (int)NVRAM_DATA_READ_ERROR && res == MTRUE)
                {
                    CAM_LOGD( "[%s] Configuring BPCI table flow is finished fro curent PD sensor format(%d)",
                              __FUNCTION__,
                              m_u4PDFmtType);
                }
                else
                {
                    CAM_LOGD( "[%s] Configuring BPCI table flow may not be finished fro curent PD sensor format(%d), getTableFromNvRam(%d), setBPCIBuf(%d)",
                              __FUNCTION__,
                              m_u4PDFmtType,
                              err,
                              res);
