...
void dumpYTab(const T *yTab, int dutyNum, int dutyNumLt)
{
    ...
    for (j = 0; j < dutyNumLt; j++) {
        ...
        for (i = 0; i < dutyNum; i++) {
            snprintf(engItem, sizeof(engItem), "%4.2f ", (float)yTab[i + j * dutyNumLt]);
    ...
    , mSetCharger(0)
{
    ...
    memset(&mFlashStateStack[0], 0, 10*sizeof(int));
    ...
    if (ctNum == 2) {
        g_pStrobe2 = StrobeDrv::getInstance(typeId, 2);
    } else {
        g_pStrobe2 = NULL;
    }
    ...
    mPreFlashFrmCnt=0;
    mDefaultFrmRate = AAASensorMgr::getInstance().getPreviewDefaultFrameRate((ESensorDev_T)m_sensorDev, AeMgr::getInstance(m_sensorDev).getSensorMode());
    ...
    */
    ...
    int propCtrl = FlashUtil::getPropInt("vendor.flash_control",-1);
    if (propCtrl != -1)
        setIsDriverInControl(propCtrl);
...
{
    strncpy(mEngName, fname, 200*sizeof(char)); // size defined in constructor
    return 0;
}
...
void FlashMgrM::hw_setCapPline(FLASH_PROJECT_PARA* pPrjPara, FlashAlg* pStrobeAlg)
{
    ...
    int propPline;
    propPline = FlashUtil::getPropInt("vendor.flash_pline",-1);
...
void FlashMgrM::hw_setFlashProfile(FlashAlg* pStrobeAlg, FLASH_PROJECT_PARA* pPrjPara, NVRAM_CAMERA_STROBE_STRUCT* pNvram)
{
    ...
    /* is charger ready */
    g_pStrobe->isChargerReady(&isChargerReady);
    mCapRep.isChargerReady = isChargerReady;
    ...
    } else if ((mCapInfo.vBat < mpStrobeNvbuf->engLevel.vBatL &&
                mpStrobeNvbuf->engLevel.IChangeByVBatEn == 1)
            || (isLow == 1)) {
        /* low battery */
        mCapRep.isBatLow = 1;
    ...
    if(cust_isDualFlashSupport(m_sensorDev)==1)
    {
        ...
        if( (mfPropOn==-1 || mfPropOn==0) && (g_isManualFlashEn==0))
        {
            ...
            //dumpYTab(engTable, pPrjPara->dutyNum + 1, pPrjPara->dutyNumLT + 1);
#else
    ...
    else
    {
        ...
        if( (mfPropOn==-1 || mfPropOn==0) && (g_isManualFlashEn==0)){
            ...
            int duty;
            duty = FlashUtil::getPropInt(PROP_MF_DUTY_STR,-1);
            if(g_isManualFlashEn==1)
            {
                duty=g_manualDuty;
            }
            engTable[duty]=pNvram->engTab.yTab[duty];
            if (engTable[duty] == -1)
                engTable[duty] = 100;
        }
    }
...
void FlashMgrM::hw_setExpPara(FlashAlgExpPara* expPara, int sensorType, FLASH_PROJECT_PARA* pPrjPara)
{
    ...
    if(isFlash==1) mPreFlashFrmCnt++;

    logV("mPreFlashFrmCnt=%d",mPreFlashFrmCnt);

    if((cust_getFlashFrameRateDelay(m_sensorDev) == 0 && mPreFlashFrmCnt==1) ||
       (cust_getFlashFrameRateDelay(m_sensorDev) == 1 && mPreFlashFrmCnt>=1 && mPreFlashFrmCnt<=2))
        return;
...
*/
    ...
    // reset framerate when exp < 33ms
    if(exp < 33333){
        MINT32 frmRate = 300;
        AAASensorMgr::getInstance().setPreviewMaxFrameRate(
            m_sensorDev,
            frmRate,
            AeMgr::getInstance(m_sensorDev).getSensorMode());
    }
...
int FlashMgrM::setPreFlashOnOff(int en)
{
    if(en==1)
    {
        logV("setPreFlashOnOff 1");
        turnOnPf();
        FLASH_INFO_T finfo;
        finfo.flashMode = FLASHLIGHT_FORCE_OFF;
        finfo.isFlash = 1; // pre-flash
...
int FlashMgrM::getFlashOnFrmCnt()
{
    return mPreFlashFrmCnt;
}
//==============================
...
void FlashMgrM::hw_convert3ASta(FlashAlgStaData* staData, void* staBuf, int winX, int winY)
{
    ...
    convert3aSta_romeHw_y(staBuf, winX, winY, A3Y, 4, mAAOMode);
...
void FlashMgrM::hw_speedUpExpPara(FlashAlgExpPara* expPara, int maxAfe)
{
    ...
    if(g>1)
    {
        ...
        expNew = reducedExp;
        double align;
        if(mCapInfo.flickerMode==FlashMgr::e_Flicker60)
...
void FlashMgrM::hw_isoToGain(int iso, int* afe, int* isp)
{
    ...
    if(devInfo.u4MaxGain >= 16320) {
        // Modify this if sensor support larger than 16320
        logV("sensor max gain larger than 16320, set to 16320");
        maxGain = 16320;
    }
    else
        maxGain = devInfo.u4MaxGain;

    float g;
    g = (float)iso/devInfo.u4MiniISOGain;
    logV("line=%d hw_isoToGain=iso=%d gain=%5.3f",__LINE__, iso, g);

    if(g<maxGain/1024.0f)
    {
        *afe=g*1024;
        *isp = 1024;
    }
    ...
    {
        *afe = maxGain;
        *isp = (g*1024/maxGain)*1024;
    }
    logV("dev_1xGainIso %d", (int)devInfo.u4MiniISOGain);
    logV("dev_minG %d", (int)devInfo.u4MinGain);
    logV("dev_maxG %d", (int)maxGain);
...
void FlashMgrM::hw_capIsoToGain(int iso, int* afe, int* isp)
{
    ...
    logV("hw_capIsoToGain dev_maxG %d", (int)maxGain);
...
FlashMgrM* FlashMgrM::getInstance(int sensorDev)
{
    ...
    {
        static FlashMgrM objSub2(sensorDev);
        FlashMgr::getInstance().setChildSub2(&objSub2);
        return &objSub2;
    }
}
//==============================
...
int FlashMgrM::init(int sensorId)
{
    ...
    pthread_create(&mThread, NULL, onSetTorchThread, this);
     ...
     bCheckCaliResult = FlashUtil::getPropInt("vendor.flash_check_cali",0);
...
int FlashMgrM::initSub(int sensorId, int isTemp)
{
    ...
    if(cust_isDualFlashSupport(m_sensorDev)==1)
    {
      ...
      conv.duty = mpStrobeNvbuf->engLevel.pfDuty;
      conv.dutyLt = mpStrobeNvbuf->engLevelLT.pfDuty;
      duty2ind(conv);
      g_expPara.duty = conv.ind;
    }
...
//==============================
int FlashMgrM::setCharger(int ready)
{
    g_pStrobe->setCharger(ready);
    if (cust_isDualFlashSupport(m_sensorDev) == 1)
        g_pStrobe2->setCharger(ready);
...
int FlashMgrM::start()
{
     ...
     * becuase using camera is not the only behavior to open camera device.
...
int FlashMgrM::isAFLampOn()
{
    int bOn;
    g_pStrobe->isOn(&bOn);
    if(cust_isDualFlashSupport(m_sensorDev)==1 && bOn==0)
        g_pStrobe2->isOn(&bOn);
...
int FlashMgrM::getDebugInfo(FLASH_DEBUG_INFO_T* p)
{
    ...
    logV("m_bAlgoTagReady=%d ln=%d",m_bAlgoTagReady, __LINE__);
...
int FlashMgrM::cctFlashLightTest(void* pIn)
{
    ...
    logV("%s() line=%d", __FUNCTION__, __LINE__);
    if(cust_isDualFlashSupport(m_sensorDev)==1){
        g_pStrobe2->setDuty(mpStrobeNvbuf->engLevelLT.torchDuty);
        g_pStrobe2->setTimeOutTime(2000);
        g_pStrobe2->setOnOff(1);
    }
...
int FlashMgrM::doPfOneFrame(FlashExePara* para, FlashExeRep* rep)
{
    ...
    if(m_pfFrameCount==0)
    {
        ...
        {
            //int spMode;  /* fix build warning */
            int ratioEn;
            int ratioQ2En;
            ratioEn = FlashUtil::getPropInt("vendor.flash_ratio",0);
            ratioQ2En = FlashUtil::getPropInt("vendor.flash_ratio_quick2",0);
            if(ratioEn==1)
            {
    ...
    else if(g_spMode==e_SpModeQuickCalibration2)
    {
...
// Panel Debug +
static int countStateEnd=0;
// Panel Debug -
// Panel extend +
static int countStatisticStable=0;
static bool fgFrameRateExtend=false;
// Panel extend -
...
#define FLASH_STATUS_ON 1
    ...
    {
        // Panel Debug +
        countStateEnd=FlashUtil::getPropInt("vendor.flash_count",0);
        // Panel Debug -
        // Panel extend +
        countStatisticStable=0;
        fgFrameRateExtend=false;
        // Panel extend -
        if(g_isManualFlashEn==1)
            setFlashMode(LIB3A_FLASH_MODE_FORCE_ON);
    ...
    if(mFlashState==FLASH_STATE_RUN)
    {
        ...
        logV("doPfOneFrame state=run, countStatisticStable=%d", countStatisticStable);
        // Panel extend +
        if(fgFrameRateExtend==false && countStatisticStable > 0){
            m_pfFrameCount--;
            countStatisticStable--;
        } else {
            // Panel extend -
        ...
        if(m_pfFrameCount-mPreRunFrm>=mRunCycleFrame)
        {
            run(in, rep);
                ...
                if(rep->nextIsFlash==1){
                    if(mFlashHwState == FLASH_STATUS_OFF){
                        countStatisticStable=cust_getFlashSttStableFrame(m_sensorDev);
                        countStatisticStable=FlashUtil::getPropInt("vendor.flash_stt_stable_count",countStatisticStable);
                    }
            ...
            if(rep->isEnd==1){
                    // Panel Debug +
                    logV("doPfOneFrame run : isEnd=%d",rep->isEnd);
                    rep->isEnd=0;
                    logV("doPfOneFrame run : isEnd=%d",rep->isEnd);
                    // Panel Debug -
                mFlashState=FLASH_STATE_END;
    ...
    if(pfBmpEn==1)
    {
        char aeF[256];
        char awbF[256];
        snprintf(aeF, sizeof(aeF), "/sdcard/flashdata/bmp/pf_ae_%03d_%02d.bmp",mDebugSceneCnt,m_pfFrameCount);
        snprintf(awbF, sizeof(awbF), "/sdcard/flashdata/bmp/pf_awb_%03d_%02d.bmp",mDebugSceneCnt,m_pfFrameCount);
        convert3aSta_romeHw_bmp((void*)in->staBuf, in->staX, in->staY,  aeF, awbF, mAAOMode);
    }
    // Panel Debug +
    logV("doPfOneFrame -: isEnd=%d",rep->isEnd);
    logV("doPfOneFrame -: isEnd=%d, countStateEnd=%d",rep->isEnd, countStateEnd);
    // Panel Debug +
...
int FlashMgrM::start(FlashExePara* in)
{
    ...
    //int ii=0;  /* fix build warning */

    chTemp[0]=0;
    snprintf(chTemp, 1024*sizeof(char), "[%d] ",j);
    for(i=0; i<prjPara.dutyNum; i++)
    {
        snprintf(ch, sizeof(ch), "%5.2f ",(float)mpStrobeNvbuf->engTab.yTab[i]);
        strncat(chTemp, ch, sizeof(chTemp) - strlen(chTemp) - 1);
    }
    logV("%s",chTemp);
    ...
    {
        logV("[Flash Awb] Prepare to do flash awb init");
    ...
    case ESensorDev_SubSecond: //  Sub Second Sensor
        awbParam = getAWBParam<ESensorDev_SubSecond>();
        break;
    default:
        CAM_LOGE("m_eSensorDev = %d", m_sensorDev);
        return MFALSE;
    }
    algIn.AwbRotationMatrix.i4Cos = buf->rAWBNVRAM[AWBNVRAMMapping[CAM_SCENARIO_PREVIEW]].rAlgoCalParam.rRotationMatrix.i4Cos;
    algIn.AwbRotationMatrix.i4Sin = buf->rAWBNVRAM[AWBNVRAMMapping[CAM_SCENARIO_PREVIEW]].rAlgoCalParam.rRotationMatrix.i4Sin;
...
int FlashMgrM::run(FlashExePara* para, FlashExeRep* rep)
{
   ...
   if((cust_getFlashFrameRateDelay(m_sensorDev) == 0 && mPreFlashFrmCnt==1) ||
    ...
    mpFlashAlg->AddStaData10(&staData, &g_expPara, &isNext, &paraNext, 0);
    ...
    if(m_iteration>10 || isNext==0)
    {
        ...
        logV("Estimate+");
        mpFlashAlg->Estimate(&g_expPara, &pFaceInfo, &isLowRef);
...
int FlashMgrM::setAeFlashMode(int aeMode, int flashMode)
{
    ...
    mFlash1Mode = fm;
    logV("setAeFlashMode ae_mode=%d flash_mode=%d f1_mode=%d",aeMode, flashMode, mFlash1Mode);
    setFlashMode(fm);
    return 0;
}
...
int FlashMgrM::setFlashMode(int mode)
{
    ...
    {
        logV("capturing");
        logV("setFlashMode- ln=%d", __LINE__);
        return 0;
    }
...
{
    m_flashOnPrecapture = flashOnCapture;
    return 0;
}
//==============================
int FlashMgrM::isFlashOnCalibration()
{
    logV("isFlashOnCalibration() line=%d mIsCalibration=%d",__LINE__,mIsCalibration);
...
int FlashMgrM::setCaptureFlashOnOff(int en)
{
    ...
    if(en==1)
    {
        ...
        if(cust_isDualFlashSupport(m_sensorDev)==1)
        {
            ...
            if (mCapRep.isBatLow == 0 &&
                    mIsMultiCapture == 0 &&
                    mCapRep.isChargerReady) {
...
int FlashMgrM::getFlashState()
{
    ...
    {
        logV("isLow == 2 ln=%d",__LINE__);
        return MTK_FLASH_STATE_CHARGING;
    }

    {
        Mutex::Autolock lock(mLock);
        logV("getFlashState %d %d %d", mFlashStateStack[0], mFlashStateStack[1], mFlashStateStack[2]);
...
int FlashMgrM::setPfParaToAe()
{
    ...
    logV("setPfParaToAe afe, isp %d %d %d, ind=%d", exp, afe, isp, bestInd);
    return 0;
}
void dispStrobeNvram(NVRAM_CAMERA_STROBE_STRUCT* nv)
{
