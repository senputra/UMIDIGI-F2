    ...
    m_bAcceVd( MFALSE)
{
    ...
    memset( &m_sNVRam,        0, sizeof(NVRAM_LENS_DATA_PARA_STRUCT));
...
MRESULT AfMgr::getPdInfoForSttCtrl(MINT32 /*i4SensorIdx*/, MINT32 i4SensorMode, MUINT32 &u4PDOSizeW, MUINT32 &u4PDOSizeH, FEATURE_PDAF_STATUS &PDAFStatus)
{
    ...
    else if( m_i4PDAF_support_type==PDAF_SUPPORT_RAW)
    {
        ...
        if( i4SensorMode==SENSOR_SCENARIO_ID_NORMAL_CAPTURE)
            tbl_idx = eIDX_PDC_1;
        else if( i4SensorMode==SENSOR_SCENARIO_ID_NORMAL_VIDEO)
            tbl_idx = eIDX_PDC_2;
        else if( i4SensorMode==SENSOR_SCENARIO_ID_NORMAL_PREVIEW)
            tbl_idx = eIDX_PDC_3;
        else
        ...
        MBOOL isBPCItblValid = (isBPCItbl) &&
                               (0<(u4Pdo_xsz+1)) &&
                               ((u4Pdo_xsz+1)<=u4BINSzW) &&
                               (0<(u4Pdo_ysz+1)) &&
                               ((u4Pdo_ysz+1)<=u4BINSzH) &&
                               ((u4Bpci_ysz+1)==1) &&
                               (0<u4Bpci_xsz);

        //
        CAM_LOGD( "[%s] Dev 0x%04x, sensormode(%d) TGSZ: W(%d) H(%d), BINSZ: W(%d) H(%d), isTGInfoValid(%d)/isFrontalBINDisable(%d)/isFullMode(%d)/isBPCItblValid(%d)/tbl_idx(%d)/BPCITbl: xSz(%d) ySz(%d) pa(0x%x) va(0x%x) memID(%d)/PDO: xSz(0x%x BYTE) ySz(0x%x)",
                  __FUNCTION__,
                  m_i4CurrSensorDev,
                  i4SensorMode,
                  u4TGSzW,
                  ...
                  u4Bpci_xsz,
                  u4Bpci_ysz,
                  (MUINT32)phyAddrBpci_tbl,
                  (MUINT32)virAddrBpci_tbl,
                  memID,
                  u4Pdo_xsz,
                  u4Pdo_ysz);

        if( (isTGInfoValid) && (isFrontalBINDisable) && ((isFullMode)||(i4SensorMode==SENSOR_SCENARIO_ID_NORMAL_VIDEO)||(i4SensorMode==SENSOR_SCENARIO_ID_NORMAL_PREVIEW)) && (isBPCItblValid))
        {
            /*
             * PDAF Raw Data mode is supported once frontal binning is not enalbed in full mode.
             */
            u4PDOSizeW = (_PDO_STRIDE_ALIGN_((u4Pdo_xsz+1)*2))/2;
            u4PDOSizeH = u4Pdo_ysz+1;
...
MBOOL AfMgr::Start()
{
    ...
    // The previous LaunchCamTrigger has not been finished ==> enable LaunchCamTrigger
    if(m_eAFState!=NS3A::E_AF_FOCUSED_LOCKED && m_eAFState!=NS3A::E_AF_NOT_FOCUSED_LOCKED)
    {
        if(m_i4LaunchCamTriggered==E_LAUNCH_AF_WAITING || m_i4LaunchCamTriggered==E_LAUNCH_AF_TRIGGERED)
        {
    ...
    m_i4EnableAF = (m_i4CurrLensId==0xFFFF) ? 0 : 1;

    CAM_LOGD( "AF-%-15s: Dev(0x%04x), PixId (%d)(0:B, 1:GB, 2:GR, 3:R), SensorID (0x%04x), LensId (0x%04x), EnableAF (%d), PD_Support(%d)",
              __FUNCTION__,
              m_i4CurrSensorDev,
              m_i4PixelId,
              m_i4CurrSensorId,
    ...
    //===============
    // Get data from NVRam : before get data from nvram, lensSearch MUST be done first!!
    //===============
    ...
    if( m_eCamScenarioMode < AF_CAM_SCENARIO_NUM)
    {
        ...
        memcpy( &m_sNVRam, &m_ptrLensNVRam->rLENSNVRAM[u4AFNVRAMIdx], sizeof(m_ptrLensNVRam->rLENSNVRAM[u4AFNVRAMIdx]));
    }
    else
    {
    ...
    CAM_LOGD( "AF-%-15s: Dev(0x%04x), ParamInfo : Scenario mode(%d), [nvram] ver(%d), Sz(%d, %zu, %zu, %zu, %zu, %zu, %zu),Normal Num(%d) Macro Num(%d) Normal min step(%d) Macro min step(%d), [AFParam] AFS_MODE(%d), AFC_MODE(%d), VAFC_MODE(%d), TBLL(%d), [DefAFConfig] AF_EXT_ENABLE(%d)",
              __FUNCTION__,
              m_i4CurrSensorDev,
              m_eCamScenarioMode,
    ...
    if(m_sNVRam.rAFNVRAM.i4Coefs[11]!=0)  // the timeout of AEStable-wait-Face is not default value
    {
        m_i4AEStableTriggerTimeout = m_sNVRam.rAFNVRAM.i4Coefs[11];
    }
    if(m_sNVRam.rAFNVRAM.i4Coefs[12]!=0)  // the timeout of ValidPD-wait-Face is not default value
    {
        m_i4ValidPDTriggerTimeout = m_sNVRam.rAFNVRAM.i4Coefs[12];
    }
    ...
    //===============
    // PD information from PD mgr and set  to Hybird AF.
    //===============
    MINT32  PDInfoSz = 0;
    MINT32 *PDInfo   = NULL;
    ...
    CAM_LOGD( "AF-%-15s: Dev(0x%04x), PD Flow (%d), SensorMode(%d), PDAF_Support_Type(%d), SensorModeSupportPDAF(%d), PDPipeCtrl(%d), ImgSz(%d, %d), FullSz(%d, %d), IsZSD %d, AETargetMode(%d), PD Info to Hybrid AF : Sz(%d), Data(%d %d..)",
              __FUNCTION__,
              m_i4CurrSensorDev,
              m_i4PDAF_support_type,
              m_i4SensorMode,
    ...
    m_pIAfAlgo->setAFParam( (*m_pAFParam), m_sHWCfg, m_sNVRam.rAFNVRAM, m_sNVRam.rDualCamNVRAM);
    ...
    if( m_pMcuDrv == NULL)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                m_i4OISDisable = m_i4DbgOISDisable = property_get_int32("vendor.debug.af_ois.disable", 0);
...
MINT32 AfMgr::CCTOPAFSetFocusRange( MVOID *a_pFocusRange)
{
    ...
    if( m_pIAfAlgo)
    {
        AF_CONFIG_T const *ptrHWCfgDef = NULL;
        getAFConfig( m_i4CurrSensorDev, &ptrHWCfgDef);
        m_pIAfAlgo->setAFParam( (*m_pAFParam), (*ptrHWCfgDef), m_sNVRam.rAFNVRAM, m_sNVRam.rDualCamNVRAM);
    }
...
MINT32 AfMgr::CCTOPAFApplyNVRAMParam( MVOID *a_pAFNVRAM, MUINT32 u4CamScenarioMode)
{
    ...
    // set nvram
    if (u4CamScenarioMode < AF_CAM_SCENARIO_NUM)
    {
        memcpy( &m_ptrLensNVRam->rLENSNVRAM[u4CamScenarioMode], (NVRAM_LENS_DATA_PARA_STRUCT *)a_pAFNVRAM, sizeof(NVRAM_LENS_DATA_PARA_STRUCT));
        CAM_LOGD( "%s, Apply to Phone[Scenario mode] %d", __FUNCTION__, u4CamScenarioMode);
        CAM_LOGD( "Apply to Phone[Thres Main] %d", m_ptrLensNVRam->rLENSNVRAM[u4CamScenarioMode].rAFNVRAM.sAF_Coef.i4THRES_MAIN);
        CAM_LOGD( "Apply to Phone[Thres Sub] %d",  m_ptrLensNVRam->rLENSNVRAM[u4CamScenarioMode].rAFNVRAM.sAF_Coef.i4THRES_SUB);
        CAM_LOGD( "Apply to Phone[HW_TH] %d",      m_ptrLensNVRam->rLENSNVRAM[u4CamScenarioMode].rAFNVRAM.sAF_TH.i4HW_TH[0]);
        CAM_LOGD( "Apply to Phone[Statgain] %d",   m_ptrLensNVRam->rLENSNVRAM[u4CamScenarioMode].rAFNVRAM.i4StatGain);
    }
    ...
    {
        memcpy( &m_sNVRam, (NVRAM_LENS_DATA_PARA_STRUCT *)a_pAFNVRAM, sizeof(NVRAM_LENS_DATA_PARA_STRUCT));
        if( m_pIAfAlgo)
        {
            AF_CONFIG_T const *ptrHWCfgDef = NULL;
            getAFConfig( m_i4CurrSensorDev, &ptrHWCfgDef);
...
NS3A::EAfState_T AfMgr::UpdateStateAutoMode( NS3A::EAfState_T &eInCurSate, AF_EVENT_T &sInEvent)
{
    ...
    case EVENT_CMD_AUTOFOCUS :
        ...
        if (m_bIsAPROITooLarge)
        {
            if (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFS && m_sNVRam.rAFNVRAM.i4Revs[1])
            {
                m_sAFInput.i4FullScanStep = 30;
                m_eLIB3A_AFMode = LIB3A_AF_MODE_FULLSCAN;

                if (m_pIAfAlgo)
                {
                    m_pIAfAlgo->setAFMode(m_eLIB3A_AFMode);
                    CAM_LOGW("[%s] run FullScan searching due to AP ROI is too large", __FUNCTION__);
                }
            }
        }

        // get Autofocus when [(CenterROI exist) or (AE locked)] ==> NotToSkipAlgo to speed up SEARCH_END
        if(m_i4IsCAFWithoutFace==1 || m_sAFOutput.i4IsLockAERequest==1)
        {
            m_bNotToSkipAlgo = MTRUE;
        }
    ...
    case EVENT_CMD_TRIGGERAF_WITH_AE_STBL :
        ...
        if (m_bIsAPROITooLarge)
        {
            if (m_i4IsInitAF == 1)
            {
                CAM_LOGI( "AF-%-15s: Dev %d, moveMCU %d",
                  __FUNCTION__,
                  m_i4CurrSensorDev,
                  m_i4InitPos);
    ...
    case EVENT_CMD_SET_AF_REGION :
        ...
        m_bLatchROI = MTRUE;
        NewState = eInCurSate;
        m_bIsAPROITooLarge = (m_sArea_APCmd.i4H > (m_i4TGSzH * 0.8)) ? MTRUE : MFALSE;
        CAM_LOGI_IF(m_bIsAPROITooLarge, "[%s] AP ROI is too large", __FUNCTION__);
        break;
    ...
    case EVENT_SEARCHING_END :
        if (m_bIsAPROITooLarge)
        {
            if (m_eLIB3A_AFMode == LIB3A_AF_MODE_FULLSCAN && m_sNVRam.rAFNVRAM.i4Revs[1])
            {
                m_eLIB3A_AFMode = LIB3A_AF_MODE_AFS;
                if (m_pIAfAlgo)
                {
                    m_pIAfAlgo->setAFMode(m_eLIB3A_AFMode);
                    CAM_LOGW("[%s] change AF mode from FULLSCAN to AFS", __FUNCTION__);
                }
            }
            m_bIsAPROITooLarge = MFALSE;
        }
...
NS3A::EAfState_T AfMgr::UpdateStateContinuousMode( NS3A::EAfState_T &eInCurSate, AF_EVENT_T &sInEvent)
{
    ...
    case EVENT_CMD_CHANGE_MODE :
         ...
         */
        m_bIsAPROITooLarge = m_bLatchROI = m_bForceTrigger = m_bTriggerCmdVlid = m_bWaitForceTrigger = m_bMgrTriggerCmd = m_bCheckStateByAFDone = MFALSE;

        if( eInCurSate==NS3A::E_AF_INACTIVE || m_i4LaunchCamTriggered==E_LAUNCH_AF_IDLE)
        {
    ...
    case EVENT_CMD_AUTOFOCUS :
        if( m_bForceTrigger==MTRUE)
        {
            ...
            m_bCheckStateByAFDone = m_bNeedLock = MTRUE;
            m_bMgrTriggerCmd = MFALSE;
        }
        else
        {
...
{
    NS3A::EAfState_T ret;
    if(Timeout_WaitForceTrigger && Timeout_AutoFocus)
    {
        Timeout_Count++;
        if(Timeout_Count>40)
        {
    ...
    if(AP_NOT_SHOW_FDROI_AUTO && m_i4IsCAFWithoutFace==1 && ret==NS3A::E_AF_PASSIVE_SCAN && m_sAFOutput.i4ROISel==AF_ROI_SEL_FD)
    {
        ...
        if(m_bNeedLock)
        {
            ret = NS3A::E_AF_FOCUSED_LOCKED;
        }
        else
        {
            ret = NS3A::E_AF_PASSIVE_FOCUSED;
        }
            ...
            if( m_ptrLensNVRam != NULL)
            {
                ...
                if( AFNVRAMMapping[a_eCamScenarioMode] != AFNVRAMMapping[m_eCamScenarioMode])
                {
                    ...
                    CAM_LOGD_IF( m_i4DgbLogLv,
                                 ...
                                 m_u4StaMagicNum,
                                 __FUNCTION__,
                                 m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4Offset,
                                 m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4NormalNum,
                                 m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4MacroNum);
...
MINT32 AfMgr::getAFMaxAreaNum()
{
    ...
    maxAFAreaNum = MCUDrv::isSupportLens( m_i4CurrSensorDev, rSensorStaticInfo.sensorDevID)==0 ? 0 : AF_WIN_NUM_SPOT;

    CAM_LOGD( "[%s] (%d)", __FUNCTION__, maxAFAreaNum);
...
MINT32 AfMgr::isAFSupport()
{
    ...
    if (m_i4IsEnableFVInFixedFocus == -1)
    {
        if (isAFSupport == 0)
        {
            m_i4IsEnableFVInFixedFocus = property_get_int32("vendor.debug.af_fv.switch", 0);
...
{
    rAFInfo.i4IsAFDone      = m_sAFOutput.i4IsAFDone;
...
{
    CAM_LOGD_IF( m_i4DgbLogLv,
                 "cmd-%s FrameId(%5d), AfeGain(%5d), IspGain(%4d), AEStable(%d), AELock(%d), hlrEnable(%d), Eposuretime(%d), DeltaIdx(%4d), RealISO(%d), MaxISO(%d), AEStableCnt(%d), bAEScenarioChange(%d)",
                 __FUNCTION__,
...
MRESULT AfMgr::doAF( MVOID* /*ptrInAFData*/)
{
    ...
    else
    {
        ...
        if (m_i4MFPos != m_sAFOutput.i4AFPos && m_eLIB3A_AFMode == LIB3A_AF_MODE_MF)
        {
            m_sAFOutput.i4AFPos = m_i4MFPos;
        }
    ...
    if(  m_i4IsSearchDone_PreState!=m_i4IsSearchDone_CurState)
    {
        if( m_i4IsSearchDone_CurState==MFALSE)
        {
            ...
            if(m_sAFOutput.i4ROISel == AF_ROI_SEL_CENTER)
            {
                m_i4IsCAFWithoutFace = 1;
            }
        }
        else
        {
    ...
    if( (m_i4IsSearchDone_PreState!=m_i4IsSearchDone_CurState) ||
            ...
            (m_i4DgbLogLv))
    {
        CAM_LOGD( "#(%5d,%5d) %s Dev(%d): [Status] IsSearchDn(%d->%d), IsSelHWROI(%d->%d), ROISel(%d), IsAFDone(%d->%d), IsMonitorFV(%d->%d), Event(0x%x)",
                  m_u4ReqMagicNum,
                  m_u4StaMagicNum,
                  __FUNCTION__,
                  m_i4CurrSensorDev,
...
MRESULT AfMgr::passPDBuffer( MVOID *ptrInPDData)
{
    ...
    else
    {
        ...
        if( (m_i4PDAF_support_type==PDAF_SUPPORT_RAW) || (m_i4PDAF_support_type==PDAF_SUPPORT_RAW_DUALPD))
        {
            magicNum = ptrPDHWData->mMagicNumber; // has magicnumber from buffer info
        }
        else if( (m_i4PDAF_support_type==PDAF_SUPPORT_CAMSV) || (m_i4PDAF_support_type==PDAF_SUPPORT_CAMSV_DUALPD))
        {
            //do nothing
        }
        else
...
MVOID AfMgr::UpdatePDParam( MINT32 &i4InROISel)
{
    if( (m_PDBuf_Type!=EPDBUF_NOTDEF) && (m_i4PDAF_support_type!=PDAF_SUPPORT_NA))
    {
        ...
        if( m_i4DgbLogLv)
        {
            ptrMsgBuf += sprintf( ptrMsgBuf, "#(%5d,%5d) %s PDBufType(0x%x), ", m_u4ReqMagicNum, m_u4StaMagicNum, __FUNCTION__, m_PDBuf_Type);
        }
        ...
        if( m_i4DgbLogLv)
        {
            ptrMsgBuf += sprintf( ptrMsgBuf, "(X,Y,W,H) : ROI(%d, %d, %d, %d) =>", PDRoi.i4X, PDRoi.i4Y, PDRoi.i4W, PDRoi.i4H);
        }
        ...
        if( (AF_START_MAGIC_NUMBER-1)<=m_u4ReqMagicNum)
        {
            /* exectuing from first request */
            AAA_TRACE_D("UpdatePDParam (%d)", m_u4StaMagicNum);
            IPDMgr::getInstance().UpdatePDParam( m_i4CurrSensorDev, m_u4StaMagicNum, m_i4PDAreaNum, m_sArea_PD);
            AAA_TRACE_END_D;
        }
    }
    ...
    else
    {
        ...
        for( MUINT32 j=0; j<nblkH; j++)
        {
            ...
            for( MUINT32 i=0; i<nblkW; i++)
            {
                ...
                m_u8FV_H0 += (*ptrSata).u4FILH0;
                FV_H1 += (*ptrSata).u4FILH1;
                m_u8FV_V  += (*ptrSata).u4FILV;
        ...
        CAM_LOGD_IF( m_i4DgbLogLv,
                     ...
                     m_u4ConfigHWNum,
                     i4CurPos,
                     (unsigned long long)m_u8FV_H0,
                     (unsigned long long)FV_H1,
                     (unsigned long long)FV_H2,
                     (unsigned long long)m_u8FV_V);
...
MUINT64 AfMgr::MoveLensTo( MINT32 &i4TargetPos, MUINT32 u4Caller)
{
    ...
    if( m_pMcuDrv)
    {
        ...
        else if( m_i4MvLensTo!=i4TargetPos)
        {
            if( u4Caller==AF_MGR_CALLER)
            {
                CAM_LOGI("#(%5d,%5d) %s Dev(%d) DAC(%d), [H0]%lld  [V]%lld", m_u4ReqMagicNum, m_u4StaMagicNum, __FUNCTION__, m_i4CurrSensorDev, i4TargetPos, m_u8FV_H0, m_u8FV_V);
            }
            else
            {
                CAM_LOGI("#(%5d,%5d) cmd-%s Dev(%d) DAC(%d), [H0]%lld  [V]%lld", m_u4ReqMagicNum, m_u4StaMagicNum, __FUNCTION__, m_i4CurrSensorDev, i4TargetPos, m_u8FV_H0, m_u8FV_V);
            }
...
MRESULT AfMgr::readOTP(CAMERA_CAM_CAL_TYPE_ENUM enCamCalEnum)
{
    ...
    if( enCamCalEnum == CAMERA_CAM_CAL_DATA_3A_GAIN)
    {
        ...
        if( 0<i4MacroPos && i4MacroPos<1024 && i4MacroPos>i4InfPos )
        {
            ...
            if(m_sNVRam.rAFNVRAM.i4Coefs[10] == 10000)
            {
...
MVOID AfMgr::setFocusDistance( MFLOAT lens_focusDistance)
{
    ...
    if ( (m_eLIB3A_AFMode == LIB3A_AF_MODE_MF) &&
            ...
            (i4ResultDac <= 1023))
    {
        CAM_LOGI( "#(%5d,%5d) cmd-%s Dev(%d) DAC(%d->%d) Dist(%f); fDist_Macro_rev %f, fDist_Inf_rev %f, fDAC_Macro %f, fDAC_Inf %f",
                  m_u4ReqMagicNum,
                  m_u4StaMagicNum,
                  __FUNCTION__,
                  m_i4CurrSensorDev,
        ...
        if (m_pIAfAlgo)
        {
            m_pIAfAlgo->setMFPos(m_i4MFPos);
            m_pIAfAlgo->trigger();
            m_u8MvLensTS = MoveLensTo( i4ResultDac, AF_MGR_CALLER);
        }
        else
        {
            CAM_LOGD("Null m_pIAfAlgo");
    ...
    else
    {
        if (m_eLIB3A_AFMode == LIB3A_AF_MODE_MF)
        {
            ...
            if (m_pIAfAlgo)
            {
                m_pIAfAlgo->setMFPos(m_i4MFPos);
                m_pIAfAlgo->trigger();
                m_u8MvLensTS = MoveLensTo( m_i4MFPos, AF_MGR_CALLER);
            }
        }
    }
}
...
MFLOAT AfMgr::getFocusDistance()
{
    ...
    if ( m_sFocusDis.i4LensPos != LensPos)
    {
        ...
        CAM_LOGI("cmd-%s Pos %d->%d, Dis %f->%f; fDist_Macro_rev %f, fDist_Inf_rev %f, fDAC_Macro %f, fDAC_Inf %f",
                 __FUNCTION__,
...
MBOOL AfMgr::IspMgrAFStatHWPreparing()
{
...
#if defined(MTKCAM_CMDQ_SUPPORT)
    MUINT32 u4FrameCount = m_u4HalMagicNum;
#else
    ...
    {
        bSkip = MTRUE;
        ptrMsgBuf += sprintf( ptrMsgBuf, "Host pause AF : ");
    }
    ...
    {
        bSkip = MTRUE;
        ptrMsgBuf += sprintf( ptrMsgBuf, "Current FrmCount is less AF_START_MAGIC_NUMBER : ");
    }
    else if(m_bNotToSkipAlgo==MTRUE)
    {
        bSkip = MFALSE;
        CAM_LOGD("%s NotToSkipAlgo to speed up SEARCH_END", __FUNCTION__);
    }
    ...
    else
    {
         ...
         */
        isHWRdy = ISP_MGR_AF_STAT_CONFIG_T::getInstance(static_cast<ESensorDev_T>(m_i4CurrSensorDev)).isHWRdy( m_u4StaMagicNum);
        bSkip   = ((isHWRdy==MFALSE                      ) &&
    ...
    {
        ptrMsgBuf += sprintf( ptrMsgBuf,
                              "bSkip(%d) PauseAF(%d) FrmCnt(%d) Afmode(%d) VsyncUpdate(%d) IsTargetAssitMove(%d) isHWRdy(%d) ROISel(%d) ZECChg(%d) IsAPROITooLarge(%d)",
                              bSkip,
                              m_bPauseAF,
                              u4FrameCount,
                              m_eCurAFMode,
...
MVOID AfMgr::IspMgrAFStatUpdateHw()
{
    ...
    if (m_i4DgbLogLv & 8)
    {
        MUINT32 TakeEffectFrame = property_get_int32("vendor.debug.af_config.count", 3);
        MUINT32 ConfigDetectIdx = m_sAFOutput.sAFStatConfig.u4ConfigNum & CONFIG_DETECT_IDX;
        MUINT32 i = 0;

        // Record frame count
        for (i = 0; i < MAX_CONFIG_DETECT_NUM; i++)
        {
            if (m_sConfigDetect[i].i4ConfigNum > 0)
            {
                m_sConfigDetect[i].i4FrameCnt++;
            }
        }

        // Record Algo Config number
        m_sConfigDetect[ConfigDetectIdx].i4ConfigNum = m_sAFOutput.sAFStatConfig.u4ConfigNum;
        m_sConfigDetect[ConfigDetectIdx].i4FrameCnt  = 0;

        // Detect perframe config
        for (i = 0; i < MAX_CONFIG_DETECT_NUM; i++)
        {
            if (property_get_int32("vendor.debug.af_config.check", 0) == 0)
            {
                if (m_sConfigDetect[i].i4ConfigNum == m_u4ConfigHWNum)
                {
                    if (m_sConfigDetect[i].i4FrameCnt > TakeEffectFrame)
                    {
                        char dbgMsgBuf[32];
                        sprintf( dbgMsgBuf, "AFO deqeue isn't N+%d Frame", TakeEffectFrame);
                        CAM_LOGE( "[%s] #(%d) AF perframe config fail", __FUNCTION__, m_u4ConfigHWNum);
                        memset( &m_sConfigDetect,   0, sizeof(AF_CONFIG_DETECT_T)*MAX_CONFIG_DETECT_NUM);
                    ...
                    else
                    {
                        ...
                        m_sConfigDetect[i].i4FrameCnt  = 0;
                    }
                    break;
                }
            }
            else
            {
                if (m_sConfigDetect[i].i4FrameCnt > TakeEffectFrame)
                {
                    CAM_LOGE( "[%s] #(%d) AF perframe config fail", __FUNCTION__, m_u4ConfigHWNum);
                    memset( &m_sConfigDetect,   0, sizeof(AF_CONFIG_DETECT_T)*MAX_CONFIG_DETECT_NUM);
                    AEE_ASSERT_AF("AF Timing isn't perframe");
                    break;
                }
...
MINT32 AfMgr::sendAFCtrl(MUINT32 eAFCtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    default:
        i4Ret = -1;
        CAM_LOGD("[%s] , unsupport cmd(%d)", __FUNCTION__, eAFCtrl);
        break;
    }
    return i4Ret;
}
