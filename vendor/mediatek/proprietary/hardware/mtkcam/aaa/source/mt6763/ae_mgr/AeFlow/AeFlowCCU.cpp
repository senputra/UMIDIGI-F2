...
init()
{
    ...
    //m_pICcu = ICcu::createInstance((const MINT8 *)"AeFlow", ICcu::AEAWB, ICcu::AEAWBCTRL);
...
start(MVOID* pAEInitInput)
{
    ...
    if(!m_bIsCCUStart){
        ...
        if (m_pICcu->init() != MTRUE) {
            AE_FLOW_LOG("ICCU init fail");
            m_bCCUIsSensorSupported = MFALSE;
            return;
        }
        ...
        //AE_FLOW_LOG("rSensorinfoInput.eWhichSensor : %x\n",rSensorinfoInput.eWhichSensor);
        if(m_pICcu->ccu_check_sensor_support(rSensorinfoInput.eWhichSensor) != MTRUE) {
            AE_FLOW_LOG("CCU not support this sensor\n");
            m_pICcu->uninit();
            m_bCCUIsSensorSupported = MFALSE;
            return;
        }
        ...
        // boot CCU
        if (m_pICcu->ccu_boot(rSensorinfoInput.eWhichSensor) !=0 ) {
            AE_FLOW_LOG("ICCU boot fail");
            boot_failed = MTRUE;
            m_pICcu->uninit();
            m_bCCUIsSensorSupported = MFALSE;
            return;
        ...
        if ( CCUIF_SUCCEED != m_pICcu->ccu_control(MSG_TO_CCU_SENSOR_INIT, &rSensorinfoInput, &rSensorinfoOutput))
        {
        ...
        if (CCUIF_SUCCEED != m_pICcu->ccu_control(MSG_TO_CCU_AE_INIT, &rAEInitparam, NULL))
        {
...
startCCUAE()
{
    if(!m_bIsCCUAEWorking && IsCCUWorking()){
        ...
        AE_FLOW_LOG( "[%s()]\n", __FUNCTION__);
        m_pICcu->ccu_control(MSG_TO_CCU_AE_START, NULL, NULL);
...
controlCCU(AE_FLOW_CONTROL_INFO_T* pcontrolInfo)
{
    ...
    if(IsCCUWorking() && IsCCUAEWorking()){
        ...
        if (m_bControlCCUInfochange)
        {
            ...
            */
            m_pICcu->ccu_control(MSG_TO_CCU_SET_AP_AE_ONCHANGE_DATA, &rOnchangeData, NULL);
            m_bControlCCUInfochange = MFALSE;
        ...
        m_pICcu->ccu_control(MSG_TO_CCU_SET_AP_AE_FRAME_SYNC_DATA, &rCcuFrameSyncData, NULL);

        // Fill APAE data
        AE_FLOW_LOG("CCU_ZoomRatio: %d\n", pcontrolInfo->i4ZoomRatio);
        ccu_ae_roi rAPAERoi;
        ...
        m_pICcu->ccu_control(MSG_TO_CCU_SET_AE_EV, &(pcontrolInfo->eAEEVcomp), NULL);
        ...
        //AE_LOG_IF(m_3ALogEnable, "[%s()]rSkipAlgoData.do_skip:0x%x\n", __FUNCTION__, rSkipAlgoData.do_skip);
		    m_pICcu->ccu_control(MSG_TO_CCU_SET_SKIP_ALGO, &rSkipAlgoData, NULL);
	    ...
	    max_framerate_data.framerate = (MUINT16)pcontrolInfo->i4AEMaxFps;
            ...
            m_pICcu->ccu_control(MSG_TO_CCU_SET_MAX_FRAMERATE, &max_framerate_data, NULL);
	    m_bSetCCUFrameRate = MFALSE;
        }
        ...
        m_pICcu->ccu_control(MSG_TO_CCU_SET_AE_ROI, &rAPAERoi, NULL);
...
getCCUResult(AE_CORE_MAIN_OUT* poutput)
{
    ...
    m_pICcu->ccu_control(MSG_TO_CCU_GET_CCU_OUTPUT, NULL, poutput);
...
{
    if(IsCCUWorking()&& IsCCUAEWorking()){
        m_pICcu->ccu_control(MSG_TO_CCU_SET_AP_AE_OUTPUT, input, NULL);
    }
}
