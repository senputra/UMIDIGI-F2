...
LaserDrv::setLaserCalibrationData(unsigned int OffsetData, unsigned int XTalkData)
{
    ...
    int err = NvBufUtil::getInstance().getBufAndReadNoDefault(CAMERA_NVRAM_DATA_AF, ESensorDev_Main, (void*&)pNVRAM_LASER);
    int val;
...
LaserDrv::getLaserOffsetCalib(void)
{
    ...
    err = NvBufUtil::getInstance().getBufAndReadNoDefault(CAMERA_NVRAM_DATA_AF, ESensorDev_Main, (void*&)pNVRAM_LASER);
    ...
    if( err == 0 )
    {
        ...
        NvBufUtil::getInstance().write(CAMERA_NVRAM_DATA_AF, ESensorDev_Main);
    }
#endif
