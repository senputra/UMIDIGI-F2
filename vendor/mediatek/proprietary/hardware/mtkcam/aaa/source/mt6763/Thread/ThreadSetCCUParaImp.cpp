...
notifyResult4CCU(ThreadSetCCUParaImp::Cmd_T rCmd)
{
    ...
    if (!IspTuningMgr::getInstance().getCamInfo(m_u4SensorDev, rCmd.rPreResult->rCamInfo))
    {
    ...
    if (rCmd.u1IsGetExif || rCmd.bIsCapEnd)
    {
        ...
        {
            CAM_LOGE_IF(m_fgLogEn, "[%s] Condition Failed for vecExifInfo.size %d, vecDbg3AInfo.size (%d,%d), vecDbgIspInfo.size (%d,%d)"
                                 , __FUNCTION__
