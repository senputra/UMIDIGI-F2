...
sendIntent(intent2type<eIntent_CameraPreviewEnd>)
{
    ...
    m_pStateMgr->updateIsDoRestore(MFALSE);
...
sendIntent(intent2type<eIntent_PrecaptureStart>)
{
    ...
    CAM_LOGD("bNeedToTurnOffLamp=%d\n", bNeedToTurnOffLamp);
    if(m_pStateMgr->getRestoreCount() > 0)
    {
        MINT32 maxFrameRate = m_pStateMgr->getMaxFrameRate() / 100;
        ...
        MY_LOG("[%s] update MaxFrameRate(%d)", __FUNCTION__, maxFrameRate);
    }
...
sendIntent(intent2type<eIntent_CaptureStart>)
{
    ...
    //reset Detect per frame
    DebugUtil::getInstance(m_SensorDevId)->resetDetect(DBG_AE);
    DebugUtil::getInstance(m_SensorDevId)->resetDetect(DBG_AF);
    ...
    if ((!m_pStateMgr->isCapQueueEmpty()) && (FlashMgr::getInstance().getFlashMode(m_SensorDevId) != LIB3A_FLASH_MODE_FORCE_TORCH))
    {
    IAwbMgr::getInstance().doCapFlashAWBDisplay(m_SensorDevId);
    }
...
doCaptureEnd()
{
    ...
    //MBOOL bAEStable = MTRUE;  /* fix build warning */
    ...
    if (i4Ret)
    {
        ...
        IHal3AResultBufInfo::getInstance(m_SensorDevId)->getAAOInfo(AE_V4P0_BLOCK_NO, rBufInfo->mMagicNumber, rAAOInfo);
        // Get PSO buf info
    ...
    // TSF
        ...
        m_pStateMgr->updateIsDoRestore(MTRUE);
    }else {
...
doUpdate()
{
    ...
    CAM_LOGD_IF(m_i4EnableLog
            , "[StateCameraPreview::doUpdate()] enter, frameCnt=%d, IsDoRestore(%d)\n"
            , m_pStateMgr->getFrameCount(), m_pStateMgr->getIsDoRestore());
    ...
    if(m_pStateMgr->getIsDoRestore())
    {
        // restore AE
        CAM_LOGD("[%s] FlashOff : IsDoRestore AE", __FUNCTION__);
        IAeMgr::getInstance().doRestoreAEInfo(m_SensorDevId, MTRUE);
        IAeMgr::getInstance().setRestore(m_SensorDevId, 0);
        // restore AWB
        CAM_LOGD("[%s] FlashOff : IsDoRestore AWB", __FUNCTION__);
    ...
    if(m_pStateMgr->getEnableShortExposure())
    {
        ...
        CAM_LOGD_IF(m_i4EnableLog,"[%s] bypass doPvAE", __FUNCTION__);
    }
    AAA_TRACE_END_L;
    ...
    if(!m_pStateMgr->getEnableShortExposure())
    {
        if(Hal3ASttCtrl::getInstance(m_SensorDevId)->isMvHDREnable())
        {
            ...
            IAeMgr::getInstance().doPvAE(m_SensorDevId, rMvHDRBufInfo->mTimeStamp, reinterpret_cast<MVOID *>(rMvHDRBufInfo->mVa), 0, rMvHDRBufInfo->mMagicNumber, 0);
        } else
        {
            CAM_LOGD_IF(m_i4EnableLog, "[%s] queryMagicNumberRequest(%d)", __FUNCTION__, m_pStateMgr->queryMagicNumberRequest());
            // Get PSO info
            IHal3AResultBufInfo::getInstance(m_SensorDevId)->getAAOInfo(AE_V4P0_BLOCK_NO, rBufInfo->mMagicNumber, rAAOInfo);
            // Get PSO buf info
            rAAOInfo.Pso_Output_Path = (PSO_PATH_SEL_ENUM)rPSOBufInfo->mRawType; // PSO before OB or after OB
            // AE
            IAeMgr::getInstance().setAAOProcInfo(m_SensorDevId, reinterpret_cast<MVOID *>(rPSOBufInfo->getPart(PSOSepBuf)), &rAAOInfo);
            IAeMgr::getInstance().doPvAE(m_SensorDevId, rBufInfo->mTimeStamp, reinterpret_cast<MVOID *>(rBufInfo->getPart(AAOSepBuf)), 0, rBufInfo->mMagicNumber, 0);
        }
    }
    AAA_TRACE_END_L;
    // notify AE RT params.
    ...
    // AWB
    AAA_TRACE_L(AWB);
    IAwbMgr::getInstance().doPvAWB(m_SensorDevId, m_pStateMgr->getFrameCount(), bAEStable, i4AoeCompLv, reinterpret_cast<MVOID *>(rBufInfo->getPart(AAOSepLSCBuf)));
...
doRestore(MBOOL bIsFlashOn)
{
    ...
    if(bIsFlashOn)
    {
        ...
        {
            // frame 1 : modify frame rate to prevent the timing of opening flash is when sensor exposuring.
            IAeMgr::getInstance().doRestoreAEInfo(m_SensorDevId, MTRUE);
            ...
            AAASensorMgr::getInstance().setPreviewMaxFrameRate(
                ...
                frmRate,
                IAeMgr::getInstance().getSensorMode(m_SensorDevId));
            MY_LOG("[%s] update flash off for precapture , MaxFrameRate(%d)", __FUNCTION__, frmRate);

        }
        else if(m_pStateMgr->getRestoreCount() == 2 && !m_pStateMgr->getEnableShortExposure())
        {
            // restore AE
            ...
            AAASensorMgr::getInstance().setPreviewMaxFrameRate(
                ...
                maxFrameRate,
                IAeMgr::getInstance().getSensorMode(m_SensorDevId));
            MY_LOG("[%s] update flash off for precapture , MaxFrameRate(%d)", __FUNCTION__, maxFrameRate);
        ...
        else if(m_pStateMgr->getRestoreCount() == 3 && !m_pStateMgr->getEnableShortExposure())
        {
            // set flag for p1done to query.
