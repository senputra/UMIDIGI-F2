...
#define AF_START_MAGIC_NUMBER 4
    ...
    ThreadRawSMVRImp *_this = reinterpret_cast<ThreadRawSMVRImp*>(arg);
    //IEventIrq::Duration duration;  /* fix build warning */
    ...
    while (_this->mbAFThreadLoop && IsSupportAF) {
        ...
        if(_this->mpAFEventIrq->waitDequeue(BUF_AFO) >= 0)
        {
            ...
            if (Hal3ASttCtrl::getInstance(_this->mi4SensorDev) != NULL)
            {
                if (Hal3ASttCtrl::getInstance(_this->mi4SensorDev)->getBufMgr(BUF_AFO) != NULL)
                {
                    if(_this->mbPreStop)
                        break;
                    Hal3ASttCtrl::getInstance(_this->mi4SensorDev)->getBufMgr(BUF_AFO)->enqueueHwBuf();
                }
            }
        }
        else
...
addReqestQ(IThreadRaw::ReqQ_T &rReq)
{
    ...
    MY_LOGD_IF(mu4LogEn, "[%s] mReqQ size(%d) + cmd(%d) magic(%d)", __FUNCTION__, (MINT32)mReqQ.size(), rReq.eCmd, rReq.rParam.u4MagicNum);
}
