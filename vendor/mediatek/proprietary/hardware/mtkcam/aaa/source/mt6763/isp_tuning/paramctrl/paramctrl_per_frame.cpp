...
#include <isp_interpolation/isp_feature.h>
#include <ccm_mgr.h>
#include <ggm_mgr.h>
#include <gma_mgr.h>
#include <lce_mgr.h>
...
prepareHw_PerFrame_PDC()
{
    ...
    MBOOL enPDC_PDFmt = /* checking PDC flow can be enabled by checking PD sensor type */
            ( m_u4PDFmtType==PDAF_SUPPORT_RAW) ||
            ( m_u4PDFmtType==PDAF_SUPPORT_CAMSV) ||
            ( m_u4PDFmtType==PDAF_SUPPORT_CAMSV_LEGACY);

    MBOOL dbg_disable_pdc = property_get_int32("vendor.isp.pdc.disable", 0);

    if( /* (m_rIspCamInfo.eSensorMode != ESensorMode_Capture)|| */
        /* (m_rIspCamInfo.BinInfo.fgBIN) ||*/
        ( !m_rIspCamInfo.fgCCTInfo.Bits.fgPDC)  ||
        (  m_DualPD_PureRaw) || /*for dual pd out pure raw in EM mode*/
        ( !enPDC_PDFmt)||
        ( dbg_disable_pdc))
    {
        rBnr.setPDCEnable(MFALSE);
        CAM_LOGD_IF( m_bDebugEnable,
                     "%s(): set PDC as disable in feature control, DualPDPureRaw(%d), eSensorMode(%d), fgBIN(%d), isCCTPDCEnable(%d), PDFmtType(%d), dbg_disable_pdc(%d)",
                     __FUNCTION__,
                     m_DualPD_PureRaw,
                     m_rIspCamInfo.eSensorMode,
                     m_rIspCamInfo.BinInfo.fgBIN,
                     m_rIspCamInfo.fgCCTInfo.Bits.fgPDC,
                     m_u4PDFmtType,
                     dbg_disable_pdc);
...
#if 0
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.rmm.disable", value, "1"); // 0: enable, 1: disable
    MBOOL bDisable = atoi(value);
...
#if 0
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.rmg.disable", value, "1"); // 0: enable, 1: disable
    MBOOL bDisable = atoi(value);
...
#if 0
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.rmm2.disable", value, "1"); // 0: enable, 1: disable
    MBOOL bDisable = atoi(value);
    MBOOL fgPgnEn = !rIspCamInfo.fgRPGEnable;
    MBOOL fgPureRaw = rIspCamInfo.i4RawType == NSIspTuning::ERawType_Pure;
...
#if 0
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.rmg2.disable", value, "1"); // 0: enable, 1: disable
    MBOOL bDisable = atoi(value);
    MBOOL fgPgnEn = !rIspCamInfo.fgRPGEnable;
    MBOOL fgPureRaw = rIspCamInfo.i4RawType == NSIspTuning::ERawType_Pure;
...
prepareHw_PerFrame_LSC_2(const ISP_INFO_T& rIspInfo)
{
    ...
    MINT32 i4SensorMode = rIspInfo.rCamInfo.eSensorMode;
    ...
    if (fgLsc2En && rIspInfo.rLscData.size())
    {
        ...
        else
        {
            ...
            if (m_pLscMgr->getRawSize(m_pLscMgr->getSensorMode(), u4W, u4H))
            {
                ...
                pHalSensorList->querySensorStaticInfo(m_eSensorDev, &sensorStaticInfo);
                MUINT32 u4RawFmtType = sensorStaticInfo.rawFmtType;
                MBOOL b4CellSupport = (u4RawFmtType == SENSOR_RAW_4CELL);
                CAM_LOGW("SensorStaticInfo 4CellSensor_Support(%d)", b4CellSupport);
                //
                if(b4CellSupport)
                {
                // get sensor info
                ...
                #define scenario_case(scenario, KEY, size_var)          \
                        case scenario:                                  \
                            size_var.w = sensorStaticInfo.KEY##Width;  \
                            size_var.h = sensorStaticInfo.KEY##Height; \
                            break;
                ...
                #undef scenario_case
                        default:
                            MY_LOGE("not support sensor scenario(0x%x)", i4SensorMode);
                            return MFALSE;
                    }

                    CAM_LOGD_IF(m_bDebugEnable, "[%s] Process Lsc Data @(%d), Size(%dx%d), grid(%dx%d), HwRto(%d)", __FUNCTION__,
                        rIspInfo.rCamInfo.u4Id, size.w, size.h, rCfg.i4GridX, rCfg.i4GridY, rRsvd.u4HwRto);
                    u4TblW = size.w;
                    u4TblH = size.h;
                }
                else
                ...
                ILscTbl rTbl(ILscTable::GAIN_FIXED, u4TblW, u4TblH, rCfg.i4GridX, rCfg.i4GridY);
                #else
                ILscTbl rTbl(ILscTable::HWTBL, u4TblW, u4TblH, rCfg.i4GridX, rCfg.i4GridY);
                #endif
                ...
                //Yang dump
                char *myString=(char*)calloc(256, sizeof(char));
                m_pLscMgr->CCTGetSdblkFileCfg(&myString);
                if(strlen(myString)==0 && rIspInfo.strFileNamePrefix.size())
                {
                    std::string strFile = rIspInfo.strFileNamePrefix +"-"+ std::to_string(profile);
                    strncpy(myString, strFile.c_str(), strlen(strFile.c_str()));
                }
                if(strlen(myString)>0)
                {
                    CAM_LOGE("Yang final dump with name %s", myString);
                    char strlscdump[256];
                    sprintf(strlscdump, "%s_LSC2.lsc",myString);
                    ...
                    CAM_LOGW("[%s] No shading data dump(%d)", __FUNCTION__);
...
prepareHw_PerFrame_ANR_TBL(const RAWIspCamInfo& rIspCamInfo)
{
    ...
    if(bDisable || !(rNbc.isANR1Enable())){
        ...
        if (m_rIspCamInfo.fgFDEnable && m_IspFdFeatureCtrl && ((rIspCamInfo.eIspProfile == EIspProfile_Capture) || (rIspCamInfo.eIspProfile == EIspProfile_Flash_Capture) ||
			(rIspCamInfo.eIspProfile == EIspProfile_MFNR_Before_Blend) || (rIspCamInfo.eIspProfile == EIspProfile_MFNR_Single) || (rIspCamInfo.eIspProfile == EIspProfile_MFNR_After_Blend)) )
        {
            FD_ANR_TBL(rIspCamInfo,
                     m_rIspParam.ISPColorTbl.FD_ANR,
                     m_fd_tuning_info,
                     anr_tbl);
        }
#if 0
...
prepareHw_PerFrame_EE(const RAWIspCamInfo& rIspCamInfo)
{
    ...
    if ((bDisable) || (!rSeee.isCCTEEEnable()) ||
        ...
        (rIspCamInfo.eEdgeMode == MTK_EDGE_MODE_ZERO_SHUTTER_LAG)||
        ...
        if( m_rIspParam.ISPColorTbl.ISPColor.COLOR_Method && m_IspInterpCtrl)
        {
                SmoothCOLOR( bIsCapture,
                 ...
                 if (m_rIspCamInfo.fgFDEnable && m_IspFdFeatureCtrl && ((rIspCamInfo.eIspProfile == EIspProfile_Capture) || (rIspCamInfo.eIspProfile == EIspProfile_Flash_Capture) ||
			         (rIspCamInfo.eIspProfile == EIspProfile_MFNR_Before_Blend) || (rIspCamInfo.eIspProfile == EIspProfile_MFNR_Single) || (rIspCamInfo.eIspProfile == EIspProfile_MFNR_After_Blend)) )
                 {
                     FD_Color( rIspCamInfo,
                               m_rIspParam.ISPColorTbl.FD_COLOR,
                               m_fd_tuning_info,
                               color);
                 }
        }
