...
template <class T>
int convert3aSta_romeHw_yrgb(void* buf, int w, int h, T* y, T* r, T* g, T* b, int aaoBitMode)
{
    unsigned char* prgb=NULL;
    int ind=0;
    prgb = ((unsigned char*)buf);
...
int convert3aSta_romeHw_bmp(void* arr, int w, int h, const char* aeF, const char* awbF, int aaoBitMode);
...
template <class T>
int convert3aSta_romeHw_y(void* buf, int w, int h, T* y, int gain, int aaoBitMode)
{
    unsigned short* py = ((unsigned short*)buf)+w*h*2;
    for(int j=0;j<w*h;j++)
    {
        *y++ = (aaoBitMode == 0)? ((*py) >> 4)*gain: ((*py) >> 6)*gain;
        py++;
    }
	return 0;
}
