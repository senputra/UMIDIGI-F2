...
#else
	MINT32 err = mkdir(dir,S_IRWXU | S_IRWXG | S_IRWXO);
        if(err != 0)
        {
            ALOGE("createDir(): err(%d)\n", err);
        }
#endif
...
int convert3aSta_romeHw_bmp(void* arr, int w, int h, const char* aeF, const char* awbF, int aaoBitMode)
{
    ...
    g = new short[w*h];
    b = new short[w*h];
    convert3aSta_romeHw_yrgb(arr, w, h, y, r, g, b, aaoBitMode);
    arrayToBmp(aeF, y, y, y, w, h);
    arrayToBmp(awbF, r, g, b, w, h);
    delete []y;
    delete []r;
