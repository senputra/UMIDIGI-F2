...
typedef struct {
    ...
    strEvPline *pCurrentTable __attribute__ ((aligned (4)));    // current structure table
}strAETable_CCT;

typedef struct {
    strAETable_CCT sPlineTable[MAX_PLINE_TABLE]; //AE Pline Table}strAEPLineTable;
}strAEPLineTable_CCT;

typedef struct {
    strAESceneMapping    sAEScenePLineMapping;   // Get PLine ID for different AE mode
    strAEPLineTable_CCT  AEPlineTable;
    strAEPLineInfomation AEPlineInfo;
    strAEPLineGainList   AEGainList;
} AE_PLINETABLE_T_CCT;
#endif //CAM_CCT_HAL1_AEPLINE
