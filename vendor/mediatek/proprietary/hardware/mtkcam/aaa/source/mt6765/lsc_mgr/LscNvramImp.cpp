...
importEEPromData()
{
    ...
    MINT32 i4SensorDevID = static_cast<MINT32>(m_eSensorDev);

    MUINT32 u4PowerOn = 0;
    MINT32 sensorIdx = NS3Av3::mapSensorDevToIdx(m_eSensorDev);
    ...
    IHalSensor* pIHalSensor = pIHalSensorList->createSensor(LOG_TAG, sensorIdx);
    pIHalSensor->sendCommand(m_eSensorDev, SENSOR_CMD_GET_SENSOR_POWER_ON_STETE, (MUINTPTR)&u4PowerOn, 0, 0);
    if (0 == (m_eSensorDev & u4PowerOn))
    {
        LSC_LOG("Sensor(%d) is not power on", m_eSensorDev);
        LSC_LOG_END();
        return E_LSC_NO_OTP;
    }
    LSC_LOG("Sensor(%d) power on done, (m_eSensorDev & u4PowerOn)(%d)", m_eSensorDev, (m_eSensorDev & u4PowerOn));
...
doThreadFunc()
{
    ...
    MUINT32 logEnable = 0;
    GET_PROP("debug.lsc_mgr.log", 0, logEnable);
    if(logEnable)
    {
        if (access("/data/vendor/shading", 6) != -1)
        {
            ...
            sprintf(strFileName, "/data/vendor/shading/%d_golden_%dx%d_%d.otp", m_eSensorDev, m_pGolden->getConfig().i4GridX, m_pGolden->getConfig().i4GridY, m_pGolden->getBayer());
            m_pGolden->dump(strFileName);
            sprintf(strFileName, "/data/vendor/shading/%d_unit_%dx%d_%d.otp", m_eSensorDev, m_pUnit->getConfig().i4GridX, m_pUnit->getConfig().i4GridY, m_pUnit->getBayer());
            m_pUnit->dump(strFileName);
            sprintf(strFileName, "/data/vendor/shading/%d_unit_%dx%d_%d_goldenFmt.txt", m_eSensorDev, m_pUnit->getConfig().i4GridX, m_pUnit->getConfig().i4GridY, m_pUnit->getBayer());
            dumpOtpFmt(strFileName, *m_pUnit);
            LSC_LOG("OTP file dumped to /data/vendor/shading");
        }
        else
        {
