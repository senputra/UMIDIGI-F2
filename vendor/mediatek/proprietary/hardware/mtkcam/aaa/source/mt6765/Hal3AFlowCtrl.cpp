...
CameraArea_T _transformArea2Active(const MUINT32 i4SensorIdx, const MINT32 i4SensorMode, const CameraArea_T& rArea)
{
    ...
    CAM_LOGW_IF(g_DebugTransEn, "[%s] in(%d,%d,%d,%d), out(%d,%d,%d,%d)", __FUNCTION__,
              input.p.x, input.p.y, input.s.w, input.s.h,
              output.p.x, output.p.y, output.s.w, output.s.h);
    CAM_LOGW_IF(g_DebugTransEn, "[%s] rArea(%d,%d,%d,%d), rOut(%d,%d,%d,%d)", __FUNCTION__,
...
config(const ConfigInfo_T& rConfigInfo)
{
    ...
    g_DebugTransEn = ::property_get_int32("vendor.debug.3a.transformArea2Active", 0);
...
postCommand(ECmd_T const eCmd, const ParamIspProfile_T* pParam)
{
    ...
    if (eCmd == ECmd_Update)
    {
        ...
        if (rParamIspProf.iValidateOpt != ParamIspProfile_T::EParamValidate_None)
        {
            ...
            if (bMainFlash &&(m_u4FlashOnIdx==-1))
            {
...
on3AProcFinish(MUINT32 u4MagicNum, MUINT32 u4MagicNumCur)
{
    ...
    if(u4PreMagicNum != u4MagicNum)
    {
        ...
        MINT32 StereoData[2];
        StereoData[0] = rPrerResult.i4MasterIdx;
        StereoData[1] = rPrerResult.i4SlaveIdx;
...
#if CAM3_STEREO_FEATURE_EN
        MINT32 StereoData[2];
        StereoData[0] = m_rResult.i4MasterIdx;
        StereoData[1] = m_rResult.i4SlaveIdx;
        UPDATE_ENTRY_ARRAY(m_rCapParam.metadata, MTK_STEREO_SYNC2A_MASTER_SLAVE, StereoData, 2);
#endif
    ...
    {
        GET_PROP_P("vendor.debug.rrz.dump", 0, m_bRRZDump);
        if (m_bRRZDump == 1)
    ...
    if (m_pCbSet)
    {
        ...
        MY_LOGD_IF(m_fgLogEn, "[%s] #(%d,%d), SOF(0x%x), m_rCapParam.i8ExposureTime(%ld), m_rCapParam.capType(%d), RequestCbSet.vNumberSet[0](%d), m_rResult.u1AfState(%d), m_rResult.u1AeState(%d), m_rResult.u1FlashState(%d), m_rResult.u4AeMode(%d), m_rResult.fgAeBvTrigger(%d), m_rResult.MasterIdx(%d), m_rResult.SlaveIdx(%d)"
                    , __FUNCTION__, (MINT32)u4MagicNum, (MINT32)u4MagicNumCur, i4CurId, (long)m_rCapParam.i8ExposureTime, m_rCapParam.u4CapType, RequestCbSet.vNumberSet[0], m_rResult.u1AfState, m_rResult.u1AeState, m_rResult.u1FlashState, m_rResult.u1AeMode, m_rResult.fgAeBvTrigger, m_rResult.i4MasterIdx, m_rResult.i4SlaveIdx);
