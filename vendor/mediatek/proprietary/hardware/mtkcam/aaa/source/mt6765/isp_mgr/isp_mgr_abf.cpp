...
{
public:
    AbfTuningProxy(MUINT32 sensorDev) :
    ...
    int getTuningIndex(MINT32 openId, int inputIso) {
        ...
        if (pParamctrl!=NULL){
            ...
            MY_LOG("pParamctrl == NULL, return index 0");
            return 0;
        }
...
#define MY_INST NS3Av3::INST_T<AbfTuningProxy>
static std::array<MY_INST, SENSOR_IDX_MAX> gMultiton;

void *abfGetTuningData(MUINT32 sensorDev, MINT32 const openId, int iso)
{
    if(openId >= SENSOR_IDX_MAX || openId < 0) {
        MY_LOGE("[%s] Unsupport sensor Index: %d\n", __FUNCTION__, openId);
        return NULL;
    }

    MY_INST& rSingleton = gMultiton[openId];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<AbfTuningProxy>(sensorDev);
    } );

    return (rSingleton.instance)->getTuningData(openId, iso);
}
