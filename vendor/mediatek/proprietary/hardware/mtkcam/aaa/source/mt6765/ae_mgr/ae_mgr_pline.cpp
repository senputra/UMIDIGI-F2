...
MRESULT AeMgr::setAEISOSpeed(MUINT32 u4NewAEISOSpeed)
{
    ...
    if (m_u4AEISOSpeed != eAEISOSpeed) {
        ...
        if(eAEISOSpeed>0){
         IspTuningMgr::getInstance().setAEManual(m_eSensorDev , MTRUE);
         AE_LOG_IF(m_3ALogEnable,"[setAEISOSpeed] Lock shading ratio\n");
        }else{
         IspTuningMgr::getInstance().setAEManual(m_eSensorDev , MFALSE);
         AE_LOG_IF(m_3ALogEnable,"[setAEISOSpeed] Unlock shading ratio\n");
        }
...
MRESULT AeMgr::updateAEidxtoExpsetting( )
{
    ...
    AE_LOG("[updateAEidxtoExpsetting:e] m_u4Index/m_u4IndexF/Exp/Afe/Isp/ISO : %d/%d/%d/%d/%d/%d \n", m_u4Index, m_u4IndexF, m_rAEOutput.rPreviewMode.u4Eposuretime,m_rAEOutput.rPreviewMode.u4AfeGain, m_rAEOutput.rPreviewMode.u4IspGain , m_rAEOutput.rPreviewMode.u4RealISO);
    return S_AE_OK;
}
