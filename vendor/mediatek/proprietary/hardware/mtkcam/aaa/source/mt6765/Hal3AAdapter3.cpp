...
// Use to control per frame property
static MINT32 perFramePropEn = 0;
#define GET_PROP_P(prop, dft, val)\
{\
    if(perFramePropEn) { val = property_get_int32(prop, (dft)); }\
    else { val = (dft); }\
}
...
static MINT32 m_i4CopyLscP1En=0;
...
inline static
CameraArea_T _remaptransformArea(const MUINT32 i4SensorIdx, const MINT32 i4SensorMode, const CameraArea_T& rArea, const MRect& mPrvCropRegion, const MRect& mActiveArraySize)
{
    if (rArea.i4Left == 0 && rArea.i4Top == 0 && rArea.i4Right == 0 && rArea.i4Bottom == 0)
    {
        return rArea;
    ...
    //MSize size(rArea.i4Right- rArea.i4Left,rArea.i4Bottom- rArea.i4Top);
    //MPoint point(rArea.i4Left, rArea.i4Top);
    MRect input, output; //(point,size);

    if (mPrvCropRegion.p.x || mPrvCropRegion.p.y || mPrvCropRegion.s.w || mPrvCropRegion.s.h)
    {
        MINT32 _left   = (int)((float)(rArea.i4Left  -mActiveArraySize.p.x) * mPrvCropRegion.s.w / mActiveArraySize.s.w + mPrvCropRegion.p.x + 0.5);
        MINT32 _right  = (int)((float)(rArea.i4Right -mActiveArraySize.p.x) * mPrvCropRegion.s.w / mActiveArraySize.s.w + mPrvCropRegion.p.x + 0.5);
        MINT32 _top    = (int)((float)(rArea.i4Top   -mActiveArraySize.p.y) * mPrvCropRegion.s.h / mActiveArraySize.s.h + mPrvCropRegion.p.y + 0.5);
        MINT32 _bottom = (int)((float)(rArea.i4Bottom-mActiveArraySize.p.y) * mPrvCropRegion.s.h / mActiveArraySize.s.h + mPrvCropRegion.p.y + 0.5);
        input = MRect(MPoint(_left, _top), MSize(_right-_left, _bottom-_top));
    }
    else
    {
        input = MRect(MPoint(rArea.i4Left, rArea.i4Top), MSize(rArea.i4Right-rArea.i4Left,rArea.i4Bottom-rArea.i4Top));
    }
...
static inline
CameraArea_T _remaptransformArea2Active(const MUINT32 i4SensorIdx, const MINT32 i4SensorMode, const CameraArea_T& rArea, const MRect& mPrvCropRegion, const MRect& mActiveArraySize)
{
    if (rArea.i4Left == 0 && rArea.i4Top == 0 && rArea.i4Right == 0 && rArea.i4Bottom == 0)
    {
        return rArea;
    ...
    if (mPrvCropRegion.s.w && mPrvCropRegion.s.h)
    {
        rOut.i4Left    = (int)((float)(output.p.x            -mPrvCropRegion.p.x) * mActiveArraySize.s.w / mPrvCropRegion.s.w + mActiveArraySize.p.x + 0.5);
        rOut.i4Right   = (int)((float)(output.p.x+output.s.w -mPrvCropRegion.p.x) * mActiveArraySize.s.w / mPrvCropRegion.s.w + mActiveArraySize.p.x + 0.5);
        rOut.i4Top     = (int)((float)(output.p.y            -mPrvCropRegion.p.y) * mActiveArraySize.s.h / mPrvCropRegion.s.h + mActiveArraySize.p.y + 0.5);
        rOut.i4Bottom  = (int)((float)(output.p.y+output.s.h -mPrvCropRegion.p.y) * mActiveArraySize.s.h / mPrvCropRegion.s.h + mActiveArraySize.p.y + 0.5);
    }
    else
    {
        rOut.i4Left   = output.p.x;
    ...
    for (i = 0; i < src.count(); i++)
    {
        ...
        case MTK_SENSOR_INFO_MAX_FRAME_DURATION:
            ...
            for (j = 0; j < entry.count(); j++)
            {
                ...
                MY_LOGD_IF(fgLogPf, "[%s] val(%ld)", __FUNCTION__, (long)val);
            }
            break;
...
Hal3AAdapter3(MINT32 const i4SensorIdx)
    ...
    , mi4PreTgHeight(0)
    , mi4AppfgCrop(0)
    , mbDoPreCapInFlashAF(MFALSE)
    , mbDbgInfoEnable(MTRUE)
    , mu4ProcessP2(MTRUE)
{
/*
...
*/
    ...
    mBackupCamInfo.resize(sizeof(NSIspTuning::RAWIspCamInfo));
...
config(const ConfigInfo_T& rConfigInfo)
{
    ...
    mbDoPreCapInFlashAF = cust_isDoPreCapInFlashAF(mu4SensorDev);
    GET_PROP("vendor.debug.camera.dbginfo", dbgInfoEnable, mbDbgInfoEnable);

    MUINT32 defaultValue = 1;
#if CAM3_DEFAULT_ISP
    defaultValue = 0;
#endif
    GET_PROP("debug.hal3av3.p2", defaultValue, mu4ProcessP2);
    ...
    mi4PreTgHeight = 0;
    mi4AppfgCrop = 0;
    GET_PROP("vendor.debug.aaa.perframe_prop.enable", 0, perFramePropEn);
    ...
    MY_LOGD("[%s] mMapMat size(%d), mbDbgInfoEnable(%d)", __FUNCTION__, mMapMat.size(), mbDbgInfoEnable);
    return mpHal3aObj->config(rConfigInfo);
}
...
start(MINT32 /*i4StartNum*/)
{
    ...
    MY_LOGD("[%s]+ sensorDev(%d), sensorIdx(%d), mpHal3aObj(%p), mbDoPreCapInFlashAF(%d)",
        __FUNCTION__, mu4SensorDev, mi4SensorIdx, mpHal3aObj, mbDoPreCapInFlashAF);
    ...
    i4SensorPreviewDelay = 2; //shutter delay = 2
...
MINT32
Hal3AAdapter3::
convert3AROI(const android::Vector<MINT32> &vec3AROI, MetaSet_T& rMetaResult, const mtk_camera_metadata_tag_t &tag) const
{
    ...
    MY_LOGD_IF(mu4LogEn,"[%s] size(%d)", __FUNCTION__, (MINT32)vec3AROI.size());
    if( vec3AROI.size()!=0)
    {
        ...
        MY_LOGD_IF(mu4LogEn,"[%s] ROINum(%d)", __FUNCTION__, i4ROINum);
        ...
        for(MINT32 i = 0; i < i4ROINum; i++)
        {
            ...
            MY_LOGD_IF(mu4LogEn,"[%s] ROINum(%d), ROIIndx(%d), Left(%d), Top(%d), Right(%d), Bottom(%d)"
                , __FUNCTION__, i4ROINum, i, rROI.i4Left, rROI.i4Top, rROI.i4Right, rROI.i4Bottom);

            rROI = _remaptransformArea2Active(i4SensorIdx, i4SensorMode, rROI, mPrvCropRegion, mActiveArraySize);

            MY_LOGD_IF(mu4LogEn,"[%s] Active Left(%d), Top(%d), Right(%d), Bottom(%d)"
        ...
        rMetaResult.appMeta.update(tag, entryNew);
...
convertResultToMeta(const Result_T& rResult, MetaSet_T& rMetaResult) const
{
    ...
    {
        MY_LOGD_IF(fgLog, "[%s] RollingShutterSkew(%ld)", __FUNCTION__, (long)rResult.i8SensorRollingShutterSkew);
        MY_LOGD_IF(fgLogPf, "[%s] ExpTime(%ld) FrmDuration(%ld) ISO(%d) RSS(%ld)", __FUNCTION__,
        (long)rResult.i8SensorExposureTime, (long)rResult.i8SensorFrameDuration, rResult.i4SensorSensitivity, (long)rResult.i8SensorRollingShutterSkew);
    }
    else
    {
        MY_LOGD_IF(fgLogPf, "[%s] ExpTime(%ld) FrmDuration(%ld) ISO(%d)", __FUNCTION__,
        (long)rResult.i8SensorExposureTime, (long)rResult.i8SensorFrameDuration, rResult.i4SensorSensitivity);
    }
    ...
    if (QUERY_ENTRY_SINGLE(rMetaResult.appMeta, MTK_HOT_PIXEL_MODE, u1Mode))
    {
        MY_LOGD_IF(fgLog, "[%s] MTK_HOT_PIXEL_MODE(%d)", __FUNCTION__, u1Mode);
    }
    if (QUERY_ENTRY_SINGLE(rMetaResult.appMeta, MTK_STATISTICS_HOT_PIXEL_MAP_MODE, u1Mode))
    {
        MY_LOGD_IF(fgLog, "[%s] MTK_STATISTICS_HOT_PIXEL_MAP_MODE(%d)", __FUNCTION__, u1Mode);
    }
    ...
    if( rResult.vecFocusAreaPos.size()!=0)
    {
        ...
        for( MUINT32 i=0; i<rResult.vecFocusAreaPos.size(); i+=2)
        {
            ...
            pos = _remaptransformArea2Active(i4SensorIdx, i4SensorMode, pos, mPrvCropRegion, mActiveArraySize);
    ...
    MY_LOGD_IF(mu4LogEn, "[%s] MTK_3A_FEATURE_AE_ROI", __FUNCTION__);
    convert3AROI(rResult.vecAEROI, rMetaResult, MTK_3A_FEATURE_AE_ROI);

    MY_LOGD_IF(mu4LogEn, "[%s] MTK_3A_FEATURE_AF_ROI", __FUNCTION__);
    convert3AROI(rResult.vecAFROI, rMetaResult, MTK_3A_FEATURE_AF_ROI);

    MY_LOGD_IF(mu4LogEn, "[%s] MTK_3A_FEATURE_AWB_ROI", __FUNCTION__);
    convert3AROI(rResult.vecAWBROI, rMetaResult, MTK_3A_FEATURE_AWB_ROI);
...
startCapture(const vector<MetaSet_T*>& requestQ, MINT32 i4StartNum)
{
    ...
    MY_LOGD("[%s]+ sensorDev(%d), sensorIdx(%d), mpHal3aObj(%p), RequestQSize(%d)",
        __FUNCTION__, mu4SensorDev, mi4SensorIdx, mpHal3aObj, requestQ.size());
...
startRequestQ(const vector<MetaSet_T*>& requestQ)
{
    ...
    MY_LOGD("[%s]+ sensorDev(%d), sensorIdx(%d), RequestQSize(%d)", __FUNCTION__, mu4SensorDev, mi4SensorIdx, requestQ.size());

    MBOOL ret = doInitMetaResultQ(); //start() doesn't call this
    if(!ret)
        CAM_LOGE("[%s]- Fail to metaResultQueue init", __FUNCTION__);
    ...
    {
        // the last metadata do set function,
        ...
        {
            MY_LOGD("[%s] Ring buffer - MetaResult size(%d) Capacity(%d)", __FUNCTION__, (MINT32)mMetaResultQueue.size(), mu4MetaResultQueueCapacity);
            ...
            rMetaResult = *requestQ[i];
            // assign value to mMetaResult
            mMetaResult = rMetaResult;
            mpHal3aObj->send3ACtrl(E3ACtrl_ValidatePass1, rMetaResult.MagicNum, 0);
        }
    }
    mu1Start = 1;
    mu1Stop = 0;
...
preset(const std::vector<MetaSet_T*>& requestQ)
{
    ...
    //mpHal3aObj->preset(mParams);                          // No need to transfer due to lack of Full CCU feature
...
#define OVERRIDE_AF  (1<<2)
    ...
    MUINT8 u1PreCapStart = 0; // 4-cell
    MUINT8 u1AfTrigStart = 0; // 4-cell
    ...
    MY_LOGD_IF(fgLogEn0, "[%s] Ring buffer - MetaResult size(%d) Capacity(%d)", __FUNCTION__, (MINT32)mMetaResultQueue.size(), mu4MetaResultQueueCapacity);
    MetaSet_T& rMetaResult = getValidBuf(i4UpdatePreMetaRQIdx);
    ...
    // If Magic Num truly has a number in it, we have to request Tuning Node
    if (i4FrmId > 0)
    {
        rRequestSet.vNumberSet.clear();
    ...
    if (!it->Dummy && i4FrmId > 0)
    {
        ...
        cnt = metadata.count();
        MY_LOGD_IF(fgLogEn0, "[%s], count(%d)", __FUNCTION__, cnt);
...
/*            QUERY_ENTRY_SINGLE(it->halMeta, MTK_HAL_REQUEST_BRIGHTNESS_MODE, i4BrightnessMode);
        ...
        QUERY_ENTRY_SINGLE(it->halMeta, MTK_HAL_REQUEST_PRECAPTURE_START, u1PreCapStart);
        QUERY_ENTRY_SINGLE(it->halMeta, MTK_HAL_REQUEST_AF_TRIGGER_START, u1AfTrigStart);
        MY_LOG_IF(fgLogEn0, "[%s] MTK_HAL_REQUEST_PRECAPTURE_START(%d), MTK_HAL_REQUEST_AF_TRIGGER_START(%d)", __FUNCTION__, u1PreCapStart, u1AfTrigStart);
        // for HAL HDR
        ...
        QUERY_ENTRY_SINGLE(it->halMeta, MTK_3A_MANUAL_AWB_COLORTEMPERATURE, mParams.i4MWBColorTemperature);
        MY_LOGD_IF(fgLogEn0,"[%s] RepeatTag(%d), i4FrmId(%d), MinFrmDuration(%d), AeIsoSpeedMode(%d), IspProfile(%d), RawType(%d), DisableP1(%d), IsSingleFrameHDR(%d), MtkHdrMode(%d), MWBColorTemperature(%d)",
                             __FUNCTION__, uRepeatTag, i4FrmId, i8MinFrmDuration, mParams.i4IsoSpeedMode, u1IspProfile, mParams.i4RawType, rRequestSet.fgDisableP1,
        ...
        MY_LOGD_IF(fgLogEn0, "[%s] mAfParams.u1AfPause(%d)", __FUNCTION__, mAfParams.u1AfPause);
        ...
        if( QUERY_ENTRY_SINGLE(it->halMeta, MTK_3A_DUMMY_AFTER_REQUEST_FRAME, bDummyAfterCapture))
        {
            mParams.bDummyAfterCapture = bDummyAfterCapture;
        }
        MY_LOGD_IF(bDummyAfterCapture, "[%s] bDummyAfterCapture(%d)", __FUNCTION__, bDummyAfterCapture);
        ...
        if (QUERY_ENTRY_SINGLE(it->halMeta, MTK_3A_PRV_CROP_REGION, rSclCropRect))
        {
            fgCrop = MTRUE;
        ...
        if(!uRepeatTag)
        {
            mi4AppfgCrop = QUERY_ENTRY_SINGLE(metadata, MTK_SCALER_CROP_REGION, rSclCropRect);
        ...
        if (!uRepeatTag) // not repeating tag, parse app meta
        {
            ...
            for (j = 0; j < cnt; j++)
            {
                ...
                case MTK_CONTROL_MODE:  // dynamic
                    {
                        ...
                        MUINT32 u4Cmp = (u1ControlMode != mParams.u1ControlMode) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_MODE(%d -> %d)", __FUNCTION__, mParams.u1ControlMode, u1ControlMode);
                ...
                case MTK_CONTROL_CAPTURE_INTENT:
                    {
                        ...
                        MUINT32 u4Cmp = (u1CapIntent != mParams.u1CaptureIntent) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_CAPTURE_INTENT(%d -> %d)", __FUNCTION__, mParams.u1CaptureIntent, u1CapIntent);
                ...
                case MTK_CONTROL_AWB_MODE:  // dynamic
                    {
                        ...
                        MUINT32 u4Cmp = (u1Mode != mParams.u4AwbMode) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_AWB_MODE(%d -> %d)", __FUNCTION__, mParams.u4AwbMode, u1Mode);
                ...
                case MTK_CONTROL_AWB_REGIONS:
                    {
                        ...
                        else
                        {
                            ...
                            for (MINT32 k = 0; k < numRgns; k++)
                            {
                                ...
                                rArea = _remaptransformArea(mi4SensorIdx,mi4SensorMode,rArea, mPrvCropRegion, mActiveArraySize);
                                rArea = _clipArea(i4TgWidth,i4TgHeight,rArea);
                ...
                case MTK_CONTROL_AE_ANTIBANDING_MODE:
                    {
                        ...
                        MUINT32 u4Cmp = (i4Mode != mParams.u4AntiBandingMode) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_AE_ANTIBANDING_MODE(%d -> %d)", __FUNCTION__, mParams.u4AntiBandingMode, i4Mode);
                ...
                case MTK_CONTROL_AE_MODE:
                    {
                        MUINT8 u1Mode = entry.itemAt(0, Type2Type< MUINT8 >());
                        MUINT32 u4Cmp = (u1Mode != mParams.u4AeMode) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_AE_MODE(%d -> %d)", __FUNCTION__, mParams.u4AeMode, u1Mode);
                ...
                case MTK_CONTROL_AE_PRECAPTURE_TRIGGER:
                    {
                        ...
                        MUINT32 u4Cmp = (u1AePrecapTrig != mParams.u1PrecapTrig) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_AE_PRECAPTURE_TRIGGER(%d -> %d)", __FUNCTION__, mParams.u1PrecapTrig, u1AePrecapTrig);
                ...
                case MTK_FLASH_MODE:
                    {
                        MUINT8 u1Mode = entry.itemAt(0, Type2Type< MUINT8 >());
                        MUINT32 u4Cmp = (u1Mode != mParams.u4StrobeMode) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_FLASH_MODE(%d -> %d)", __FUNCTION__, mParams.u4StrobeMode, u1Mode);
                ...
                case MTK_SENSOR_EXPOSURE_TIME:
                    {
                        ...
                        MY_LOGD_IF(fgLogEn1, "[%s] MTK_SENSOR_EXPOSURE_TIME(%ld)", __FUNCTION__, (long)i8AeExposureTime);
                    }
                    break;
                ...
                case MTK_SENSOR_FRAME_DURATION:
                    {
                        ...
                        MY_LOGD_IF(fgLogEn1, "[%s] MTK_SENSOR_FRAME_DURATION(%ld)", __FUNCTION__, (long)i8FrameDuration);
                    }
                    break;
                ...
                case MTK_CONTROL_AF_MODE:
                    {
                        ...
                        MUINT32 u4Cmp = (u1AfMode != mAfParams.u4AfMode) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_AF_MODE(%d -> %d)", __FUNCTION__, mAfParams.u4AfMode, u1AfMode);
                ...
                case MTK_CONTROL_AF_TRIGGER:
                    {
                        ...
                        MUINT32 u4Cmp = (u1AfTrig != mAfParams.u1AfTrig) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_CONTROL_AF_TRIGGER(%d -> %d)", __FUNCTION__, mAfParams.u1AfTrig, u1AfTrig);
                ...
                case MTK_SHADING_MODE:
                    {
                        ...
                        MUINT32 u4Cmp = (u1ShadingMode != mParams.u1ShadingMode) ? 1 : 0;
                        MY_LOGD_IF(u4Cmp, "[%s] MTK_SHADING_MODE(%d -> %d)", __FUNCTION__, mParams.u1ShadingMode, u1ShadingMode);
...
*/
            ...
            else
            {
                if (mu1FdEnable != MFALSE)
                {
                    ...
                    rFaceMeta.timestamp = 0;
                    rAFFaceMeta.number_of_faces = 0;
                    rAFFaceMeta.timestamp = 0;
    ...
    if ((mi8PreMinFrmDuration != i8MinFrmDuration) && i8MinFrmDuration > 0)
    {
        ...
        mi8PreMinFrmDuration = i8MinFrmDuration;
    }
    // set m_Params
    ...
    if(!mbDoPreCapInFlashAF)
    {
        // AF trigger start
        if (u1AfTrig == MTK_CONTROL_AF_TRIGGER_START || u1AfTrigStart)
        {
    ...
    if ( (u1AePrecapTrig == MTK_CONTROL_AE_PRECAPTURE_TRIGGER_START &&
        mParams.u4AeMode != MTK_CONTROL_AE_MODE_OFF)|| u1PreCapStart)
    {
        MY_LOGD_IF(fgLog, "[%s] Precapture Trigger @ i4FrmId(%d)", __FUNCTION__, i4FrmId);
        mpHal3aObj->sendCommand(NS3Av3::ECmd_PrecaptureStart);
    }
    if (u1AePrecapTrig == MTK_CONTROL_AE_PRECAPTURE_TRIGGER_CANCEL&&
        mParams.u4AeMode != MTK_CONTROL_AE_MODE_OFF)
    {
        MY_LOGD_IF(fgLog, "[%s] Precapture Cancel @ i4FrmId(%d)", __FUNCTION__, i4FrmId);
...
#define OVERRIDE_AF  (1<<2)
    ...
    for (MINT32 i = 0; i < requestQ.size(); i++)
    {
        ...
        MY_LOGD_IF(fgLogEn0, "[%s] Ring buffer - MetaResult size(%d) Capacity(%u)", __FUNCTION__, (MINT32)mMetaResultQueue.size(), mu4MetaResultQueueCapacity);
        MetaSet_T& rMetaResult = getValidBuf(i4UpdatePreMetaRQIdx);
...
/*        QUERY_ENTRY_SINGLE(it->halMeta, MTK_HAL_REQUEST_BRIGHTNESS_MODE, mParams.i4BrightnessMode);
        ...
        QUERY_ENTRY_SINGLE(it->halMeta, MTK_3A_ISP_PROFILE, u1IspProfile);
        QUERY_ENTRY_SINGLE(it->halMeta, MTK_P1NODE_RAW_TYPE, mParams.i4RawType);
        fgCrop = QUERY_ENTRY_SINGLE(it->halMeta, MTK_3A_PRV_CROP_REGION, rSclCropRect);
        if (fgCrop)
        {
            mPrvCropRegion.p.x = rSclCropRect.p.x;
            mPrvCropRegion.p.y = rSclCropRect.p.y;
                    ...
                    case MTK_CONTROL_AWB_LOCK:
                        {
                            MUINT8 bLock = entry.itemAt(0, Type2Type< MUINT8 >());
                            mParams.bIsAWBLock = (bLock==MTK_CONTROL_AWB_LOCK_ON) ? MTRUE : MFALSE;
                            MY_LOGD_IF(fgLogEn1, "[%s] MTK_CONTROL_AWB_LOCK(%d)", __FUNCTION__, (MINT32)bLock);
                        }
                        break;
                    case MTK_CONTROL_AWB_MODE:  // dynamic
                        {
...
startCapture(const List<MetaSet_T>& requestQ, MINT32 /*i4StartNum*/, MINT32 i4RequestQSize)
{
    ...
    {
        CAM_LOGE("[%s] Start Capture Request Less Than 3 !", __FUNCTION__);
        return 0;
    }
...
set(const List<MetaSet_T>& requestQ, MINT32 i4RequestQSize)
{
    ...
    if (m_i4SubsampleCount > 1)  //Sorry for the following dumbass violence code, due to the constantness of every single element in requestQ, we have no choice but to copy each of them
    {
        vector<MetaSet_T> local;
        for (auto& it : requestQ)
            local.push_back(it);
...
getValidBuf(MINT32& i4UpdatePreMetaRQIdx)
{
    ...
    MY_LOGD_IF(mu4LogEn, "[%s] hal count(%d) ",__FUNCTION__, it->halMeta.count());
    if(it->halMeta.count() > 0)
...
get(MUINT32 frmId, MetaSet_T& result)
{
    ...
    if (-1 == mpHal3aObj->getResult(frmId, rResult))
    {
        ...
        // check the metadata is exist or not
        MetaSetList_T::iterator it = mMetaResultQueue.begin();
        for (; it != mMetaResultQueue.end(); it++)
        {
            MINT32 u4Id = 0;
            QUERY_ENTRY_SINGLE(it->halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, u4Id);
            if (frmId == (MUINT32)u4Id)
            {
                CAM_LOGW("[%s] Fail to get R[%d], but return the original metadata", __FUNCTION__, frmId);
                result = *it;
                return 0;
            }
        }

        // the metadata isn't exist, then reture -1 to MW for drop frame.
...
_reprocess(const MetaSet_T& control, MINT32 i4IspProfile, NSIspTuning::ISP_INFO_T& rReprocInput)
{
    ...
    {
        if( rReprocInput.rCamInfo.rFlashInfo.isFlash != 0){
    ...
    IMetadata::Memory rLscData;
    if (QUERY_ENTRY_SINGLE(control.halMeta, MTK_LSC_TBL_DATA, rLscData) && ((!rReprocInput.rCamInfo.fgRPGEnable) && (i4RawType == NSIspTuning::ERawType_Pure) || m_i4CopyLscP1En))
    {
        ...
        if(rLscData.size()!=0)
        {
            ...
            if(mu4DumpLscP1En)
            {
                std::string strFile = rReprocInput.strFileNamePrefix +"-"+ std::to_string(rReprocInput.rCamInfo.eIspProfile) + ".lsc";
                char filename[256];
                strncpy(filename, strFile.c_str(), strlen(strFile.c_str()));
                filename[strlen(strFile.c_str())] = 0;
                sp<IFileCache> fidLscDump;
                fidLscDump = IFileCache::open(filename);
                if (fidLscDump->write(&rReprocInput.rLscData[0], rReprocInput.rLscData.size()) != rReprocInput.rLscData.size())
                {
    ...
    if (QUERY_ENTRY_SINGLE(control.halMeta, MTK_P1NODE_SCALAR_CROP_REGION, rP1Crop) &&
        ...
        (rReprocInput.rCamInfo.fgRPGEnable  ||
         rReprocInput.rCamInfo.eIspProfile == NSIspTuning::EIspProfile_N3D_Denoise  ||
         rReprocInput.rCamInfo.eIspProfile == NSIspTuning::EIspProfile_N3D_Denoise_toGGM))
    {
...
_dumpAAO(const char* filename, MUINT32 SensorDev, MINT32 MagicNumberRequest)
{
    ...
    N = pData->AAO.size();
    if (fwrite(pData->AAO.editArray(), sizeof(char), N, fptr)!=N) {
...
setP2Params(P2Param_T &rNewP2Param, const MetaSet_T& control,ResultP2_T* pResultP2, NSIspTuning::ISP_INFO_T& rReprocInput)
{
    ...
    if (QUERY_ENTRY_SINGLE(control.halMeta, MTK_HAL_REQUEST_EDGE_MODE, rNewP2Param.i4halEdgeMode))
    {
        rReprocInput.rCamInfo.rIspUsrSelectLevel.eIdx_Edge = (EIndex_Isp_Edge_T)rNewP2Param.i4halEdgeMode;
    }
    if (QUERY_ENTRY_SINGLE(control.appMeta, MTK_EDGE_MODE, rNewP2Param.u1appEdgeMode))
    {
        rReprocInput.rCamInfo.eEdgeMode = (mtk_camera_metadata_enum_android_edge_mode_t)rNewP2Param.u1appEdgeMode;
    }
        MY_LOGD_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF),"[%s] FrameId(%d), HueMode(%d),BrightnessMode(%d), ContrastMode(%d), SaturationMode(%d), i4halEdgeMode(%d), u1appEdgeMode(%d)"
            , __FUNCTION__, i4Id, rNewP2Param.i4HueMode, rNewP2Param.i4BrightnessMode, rNewP2Param.i4ContrastMode, rNewP2Param.i4SaturationMode, rNewP2Param.i4halEdgeMode, rNewP2Param.u1appEdgeMode);
    ...
    MFLOAT* pTonemapRed = NULL;
    MFLOAT* pTonemapGreen = NULL;
    MFLOAT* pTonemapBlue = NULL;
    {
        IMetadata::IEntry entry;
        entry = control.appMeta.entryFor(MTK_TONEMAP_CURVE_RED);
        if (entry.tag() != IMetadata::IEntry::BAD_TAG)
        {
            MUINT32 size = entry.count();
            rNewP2Param.u4TonemapCurveRedSize = size;
            if(size > 0) pTonemapRed = new MFLOAT[size];
            GET_ENTRY_ARRAY(control.appMeta, MTK_TONEMAP_CURVE_RED, pTonemapRed, size);
        ...
        {
            MUINT32 size = entry.count();
            rNewP2Param.u4TonemapCurveGreenSize = size;
            if(size > 0) pTonemapGreen = new MFLOAT[size];
            GET_ENTRY_ARRAY(control.appMeta, MTK_TONEMAP_CURVE_GREEN, pTonemapGreen, size);
        ...
        {
            MUINT32 size = entry.count();
            rNewP2Param.u4TonemapCurveBlueSize = size;
            if(size > 0) pTonemapBlue = new MFLOAT[size];
            GET_ENTRY_ARRAY(control.appMeta, MTK_TONEMAP_CURVE_BLUE, pTonemapBlue, size);
    ...
    if(pTonemapRed) delete [] pTonemapRed;
    if(pTonemapGreen) delete [] pTonemapGreen;
    if(pTonemapBlue) delete [] pTonemapBlue;
...
_readDump(TuningParam* pTuningBuf, const MetaSet_T& control, MetaSet_T* pResult, ISP_INFO_T* pIspInfo, MINT32 i4Format)
{
    ...
    if (!rule.isREADEnable("3AHAL") && pIspInfo == NULL)
        return MFALSE;
    if ((pIspInfo->rCamInfo.eIspProfile == NSIspTuning::EIspProfile_Capture) ||
        (pIspInfo->rCamInfo.eIspProfile == NSIspTuning::EIspProfile_MFNR_After_Blend) ||
        (pIspInfo->rCamInfo.eIspProfile == NSIspTuning::EIspProfile_MFNR_MFB) ||
        (pIspInfo->rCamInfo.eIspProfile == NSIspTuning::EIspProfile_MFNR_Single) ||
        (pIspInfo->rCamInfo.eIspProfile == NSIspTuning::EIspProfile_MFNR_Before_Blend))
    {
        MINT32 i4ReqNo = 0;
        QUERY_ENTRY_SINGLE(control.halMeta, MTK_PIPELINE_REQUEST_NUMBER, i4ReqNo);
        char strDump[512] = {'\0'};
        std::string strispProfileName = IspTuningMgr::getIspProfileName(pIspInfo->rCamInfo.eIspProfile);
        ...
        if (i4Format == E_Lsc_Output)
        {
            ...
            if (fidLscRead){
                MY_LOG("[%s] %s ILscTbl::lscdata",__FUNCTION__, strDump, pIspInfo->rLscData.size());
...
setIsp(MINT32 flowType, const MetaSet_T& control, TuningParam* pTuningBuf, MetaSet_T* pResult)
{
    ...
    if (mu4ProcessP2 == 0 || pTuningBuf == NULL)
    {
    ...
    else
    {
        ...
        //MBOOL  bHasCaminfo = MFALSE;
        NSIspTuning::ISP_INFO_T rIspInfo;

        if (!QUERY_ENTRY_SINGLE(control.halMeta, MTK_HAL_REQUEST_REQUIRE_EXIF, mP2Params.u1Exif))
            mP2Params.u1Exif = 0;

        if (!QUERY_ENTRY_SINGLE(control.halMeta, MTK_HAL_REQUEST_DUMP_EXIF, mP2Params.u1DumpExif))
            mP2Params.u1DumpExif = 0;

        MINT32 i4MagicNum = 0;
        QUERY_ENTRY_SINGLE(control.halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, i4MagicNum);
            mP2Params.i4MagicNum = i4MagicNum;

        if (!QUERY_ENTRY_SINGLE(control.halMeta, MTK_STEREO_FEATURE_DENOISE_MODE, mP2Params.i4DenoiseMode))
            mP2Params.i4DenoiseMode = 0;

        if (!QUERY_ENTRY_SINGLE(control.halMeta, MTK_3A_REPEAT_RESULT, mP2Params.u1RepeatResult))
            mP2Params.u1RepeatResult = 0;
/*
...
*/
        ...
        MY_LOGD_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s] u1RepeatResult(%d):(%d)", __FUNCTION__, mP2Params.u1RepeatResult, mP2Params.i4MagicNum);

        //IMetadata::Memory caminfoBuf;
        //bHasCaminfo = QUERY_ENTRY_SINGLE<IMetadata::Memory>(control.halMeta, MTK_PROCESSOR_CAMINFO, caminfoBuf);

        MBOOL bBackupCaminfo = QUERY_ENTRY_SINGLE<IMetadata::Memory>(control.halMeta, MTK_PROCESSOR_CAMINFO, mBackupCamInfo);

        MINT32 i4IspProfile = -1;
        //MUINT8 u1IspProfile = NSIspTuning::EIspProfile_Preview;
        if (!QUERY_ENTRY_SINGLE(control.halMeta, MTK_3A_ISP_PROFILE, mP2Params.u1IspProfile)){
            mP2Params.u1IspProfile = NSIspTuning::EIspProfile_Preview;
        } else {
            i4IspProfile = mP2Params.u1IspProfile;
        }
        ...
        if (!bBackupCaminfo)
        {
            ...
            ((NSIspTuning::RAWIspCamInfo*) (mBackupCamInfo.array()))->rCropRzInfo.i4FullW = i4TgWidth;
            ((NSIspTuning::RAWIspCamInfo*) (mBackupCamInfo.array()))->rCropRzInfo.i4FullH = i4TgHeight;

            ((NSIspTuning::RAWIspCamInfo*) (mBackupCamInfo.array()))->rLCS_Info.rOutSetting = m_LastLCSConfig;

            //caminfoBuf = mBackupCamInfo;      //  Nelson: Meaningless code, caminfoBuf won't be accessed from this line on;

            //bBackupCaminfo = MFALSE;
            CAM_LOGD("[%s] Restore caminfo,size(%d)/mode(%d)/profile(%d)/FrmId(%d)/TG(%d,%d)",__FUNCTION__, mBackupCamInfo.size(),
                    ((NSIspTuning::RAWIspCamInfo*) mBackupCamInfo.array())->eSensorMode,
                    ((NSIspTuning::RAWIspCamInfo*) mBackupCamInfo.array())->eIspProfile,
                     mP2Params.i4MagicNum, i4TgWidth, i4TgHeight);
        }

        // LOGIC!?
        if (bBackupCaminfo)
        {  // CamInfo really has something in it

            // Generate file name prefix
            rIspInfo.strFileNamePrefix = "";
            MINT32 captureDump = property_get_int32("vendor.debug.camera.dump.lsc2", 0);

            if(fgGenPrefix || captureDump)
            {
                MINT32 i4Size = 512;
            ...
            ::memcpy(&rIspInfo.rCamInfo, mBackupCamInfo.array(), sizeof(NSIspTuning::RAWIspCamInfo));

            MINT32 NR3D_Data[14];
            if(!GET_ENTRY_ARRAY(control.halMeta, MTK_3A_ISP_NR3D_SW_PARAMS, NR3D_Data, 14))
            {
            ...
            if (!QUERY_ENTRY_SINGLE(control.halMeta, MTK_ISP_P2_IN_IMG_FMT, mP2Params.i4P2InImgFmt))
                mP2Params.i4P2InImgFmt = 0;

            rIspInfo.rCamInfo.i4P2InImgFmt = mP2Params.i4P2InImgFmt;

            //Feature Control Overwrite
            if (!QUERY_ENTRY_SINGLE(control.halMeta, MTK_3A_ISP_BYPASS_LCE, mP2Params.bBypassLCE))
                mP2Params.bBypassLCE = MFALSE;

            if(mP2Params.bBypassLCE){
                rIspInfo.rCamInfo.bBypassLCE = MTRUE;
                MY_LOG_IF(fgLog, "[%s]+ Feature Turn OFF LCE by metadata",__FUNCTION__);
            }
            else{
                if (pTuningBuf->pLcsBuf == NULL )
                {
                     MY_LOGE("[-No Lcso Buffer ]");
                     rIspInfo.rCamInfo.bBypassLCE = MTRUE;
                }
            }
            setP2Params(mP2Params, control, &rResultP2, rIspInfo); // parse p2 params & set to ISP

            MUINT8 u1CapIntent = MTK_CONTROL_CAPTURE_INTENT_PREVIEW;
            QUERY_ENTRY_SINGLE(control.appMeta, MTK_CONTROL_CAPTURE_INTENT, u1CapIntent);
            rIspInfo.isCapture = 0
                || u1CapIntent == MTK_CONTROL_CAPTURE_INTENT_VIDEO_SNAPSHOT
                || u1CapIntent == MTK_CONTROL_CAPTURE_INTENT_ZERO_SHUTTER_LAG
                || u1CapIntent == MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE
                ;
            ...
            if(rIspInfo.strFileNamePrefix.size()!=0)
            {
                char filename[256];
                genFileName_LSC2(filename, sizeof(filename), &rIspInfo.hint);
                std::string strfilename(filename);
                rIspInfo.strFileNamePrefix = strfilename;
            }
            ...
            if (mP2Params.u1Exif && mbDbgInfoEnable)
            {
                ...
                if (pResult)
                {
                    ...
                    MINT32 CaptureDump;
                    MINT32 PreviewDump;
                    GET_PROP_P("vendor.debug.camera.dump.p2.debuginfo", 0, CaptureDump);
                    GET_PROP_P("vendor.debug.camera.dump.isp.preview", 0, PreviewDump);
                    if ((CaptureDump && rIspInfo.isCapture) || PreviewDump || mP2Params.u1DumpExif)
                    {
                        char strFile[512] = {'\0'};
                        const char *ispProfileName = IspTuningMgr::getIspProfileName(rIspInfo.rCamInfo.eIspProfile);
                        sprintf(strFile, "/sdcard/camera_dump/%09d-%04d-%04d-%d-%04d-%s.tuning", rIspInfo.i4UniqueKey, i4FrmNo, i4ReqNo, mu4SensorDev, rIspInfo.rCamInfo.u4Id, ispProfileName);
...
//=========================================================================================================
            MINT32 dumpAAO;
            MINT32 dumpAAOPreview;
            GET_PROP_P("vendor.debug.camera.AAO.dump", 0, dumpAAO);
            GET_PROP_P("vendor.debug.camera.AAO.dump.preivew", 0, dumpAAOPreview);
            if ((dumpAAO && rIspInfo.isCapture) || dumpAAOPreview)
            {
                char strFile[512] = {'\0'};
                sprintf(strFile, "/sdcard/camera_dump/%09d-%04d-%04d-%d.hw_aao", rIspInfo.i4UniqueKey, i4FrmNo, i4ReqNo, mu4SensorDev);
                _dumpAAO(strFile, mu4SensorDev, mP2Params.i4MagicNum);
            }
//=========================================================================================================
//                      Cannot change the property name of NDD dump using !!!!!!
//=========================================================================================================
            ...
            /*if(bBackupCaminfo)
            {
                mBackupCamInfo = caminfoBuf;
                MY_LOGD_IF(fgLog, "[%s] Backup caminfo,size(%d)/mode(%d)/profile(%d)",__FUNCTION__, mBackupCamInfo.size(),
                    ((NSIspTuning::RAWIspCamInfo*) mBackupCamInfo.array())->eSensorMode,
                    ((NSIspTuning::RAWIspCamInfo*) mBackupCamInfo.array())->eIspProfile);
            }*/
        ...
        else if(mP2Params.u1IspProfile == EIspProfile_YUV_Reprocess){
            rIspInfo.rCamInfo.bBypassLCE = MTRUE;
            _reprocess(control, i4IspProfile, rIspInfo);
            mpHal3aObj->setIspPass2(flowType, rIspInfo, pTuningBuf, NULL);
            _reprocResult(rIspInfo, pResult);
        ...
        if (mP2Params.u1TonemapMode != MTK_TONEMAP_MODE_FAST && mP2Params.u1TonemapMode != MTK_TONEMAP_MODE_HIGH_QUALITY)
        {
            mpHal3aObj->getP2Result(mP2Params, &rResultP2);
...
setSensorMode(MINT32 i4SensorMode)
{
    ...
    if(mBackupCamInfo.size())
    {
        ((NSIspTuning::RAWIspCamInfo*) (mBackupCamInfo.array()))->eSensorMode = (ESensorMode_T)i4SensorMode;
        MY_LOGD("[%s] mBackupCamInfo SensorMode(%d)", __FUNCTION__, mi4SensorMode);
    }
...
notifyP1PwrOn()
{
    ...
    return mpHal3aObj->notifyP1PwrOn();
}
...
notifyP1PwrOff()
{
    ...
    return mpHal3aObj->notifyP1PwrOff();
}
...
{
    if (e3ACtrl == E3ACtrl_SetAwbMode){
        if (mParams.u4AwbMode != (MUINT32)i4Arg1)
         {
           MY_LOGD("[%s] E3ACtrl_T(%#x) update awbmode(%d--->%d)", __FUNCTION__, E3ACtrl_SetAwbMode, mParams.u4AwbMode, (MUINT32)i4Arg1);
           mParams.u4AwbMode = (MUINT32)i4Arg1;
         }
         else
         {
           MY_LOGD("[%s] E3ACtrl_T(%#x) no need to update awbmode(%d)", __FUNCTION__, E3ACtrl_SetAwbMode, mParams.u4AwbMode);
         }
        return 0;
    }
...
doNotifyCb(MINT32 _msgType, MINTPTR _ext1, MINTPTR _ext2, MINTPTR _ext3)
{
    ...
    case eID_NOTIFY_LCS_ISP_PARAMS:
        ...
        break;
    case eID_NOTIFY_RAW_STTMAGICNUM:
        send3ACtrl(E3ACtrl_SetAF2AEInfo, 0, 0);
        break;
    case eID_NOTIFY_P1_DONE_FROMSTT:
        mpHal3aObj->SttP1Done(_ext1, NULL);
        break;
