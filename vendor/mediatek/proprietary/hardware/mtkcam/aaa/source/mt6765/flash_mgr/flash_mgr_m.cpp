...
*/
    ...
    AeMgr::getInstance(m_sensorDev).setExp(exp);

    //err = AAASensorMgr::getInstance().setSensorGain(m_sensorDev, afe);
    AeMgr::getInstance(m_sensorDev).setAfe(afe);
...
int FlashMgrM::getExp(){
    return g_pfExpPara.exp;
}
...
int FlashMgrM::start(FlashExePara* in)
{
    ...
    m_iteration=0;
    mpFlashAlg->Reset();
    if(isFlashAWBv2Enabled(m_sensorDev))
    {
