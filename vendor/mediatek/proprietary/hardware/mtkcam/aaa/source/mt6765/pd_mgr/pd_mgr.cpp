...
EPDBUF_TYPE_t PDMgr::start(MINT32 i4SensorIdx, SPDProfile_t *iPDProfile, PD_NVRAM_T *ptrInPDNvRam)
{
    ...
    ISP_MGR_PDO_CONFIG_T::getInstance(static_cast<MUINT32>(m_i4CurrSensorDev)).start( m_i4SensorIdx);
...
MRESULT PDMgr::stop()
{
    ...
    // stop isp_mgr_pdo
    ISP_MGR_PDO_CONFIG_T::getInstance(static_cast<MUINT32>(m_i4CurrSensorDev)).stop();
...
MRESULT PDMgr::ConfigurePDHWSetting(SPDProfile_t *iPDProfile, PD_ALGO_TUNING_T *ptrInTuningData)
{
    ...
    else if( (iPDProfile->u4PDPipeCtrl == FEATURE_PDAF_SUPPORT_PBN_PDO) || (iPDProfile->u4PDPipeCtrl == FEATURE_PDAF_SUPPORT_BNR_PDO))
    {
        ...
        if( PDAF_support_type == PDAF_SUPPORT_RAW)
        {
            ...
            MINT32   memID=0;
            MBOOL isBPCItbl __unused = ISP_MGR_BNR2::getInstance(static_cast<MUINT32>(m_i4CurrSensorDev)).getBPCItable(
                                  u4Bpci_xsz,
                                  u4Bpci_ysz,
                                  u4Pdo_xsz,
                                  u4Pdo_ysz,
        ...
        ISP_MGR_PDO_CONFIG_T::getInstance(static_cast<MUINT32>(m_i4CurrSensorDev)).config( m_sPDOHWInfo);
...
MRESULT PDMgr::SetCaliData2PDCore(PD_ALGO_TUNING_T* ptrInTuningData, PD_CALIBRATION_DATA_T* ptrInCaliData)
{
    ...
    if( m_bDebugEnable)
    {
        ...
        if( fp!=NULL)
        {
            ...
            //
            CAM_LOGD( "[%s] tuning data is dumpped to /sdcard/pdo/__pd_tuning_data.bin (size=%d)",
                      __FUNCTION__,
                      (int)(sizeof(PD_ALGO_TUNING_T)));
        }
        else
        {
    ...
    {
        CAM_LOGE("Calculing FullRawOffset is fail:(%d,%d)!=(%d,%d). crop information(%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d)",
...
#define USE_DRV_PD_COORDINATOR 1  //[TODO] set as 0
    ...
    switch( m_profile.u4PDAF_support_type)
    {
        ...
        default:
        {
            ...
            unsigned int crop_x __unused = algCfg.i4FullRawXOffset;
            unsigned int crop_y __unused = algCfg.i4FullRawYOffset;
    ...
    if( (m_pIPdAlgo!=NULL))
    {
            ...
            (   m_sAnalysisData.numDa<=MAX_SIZE_OF_PD_ROI) )
        {
            ...
            if (!m_i4DbgDisPdHandle)
            {
                ...
                {
                    if( (MUINT32)(m_sAnalysisData.Da[i].sHandlePDInput.i4CurLensPos) != m_curLensPos)
                    {
                    ...
                    PD_TRACE_FMT_BEGIN(TRACE_LEVEL_DEF, "handlePD");
...
MVOID PDMgr::PDVerificationFlow()
{
    ...
    if( m_i4DbgPDVerifyRun && m_bAEStable)
    {
        ...
        if( pos<= (MUINT32)(m_i4CaliAFPos_Macro))
        {
...
#define PD_VERIFICATION_WINDOW (9+1)
                ...
                {
                    CAM_LOGW( "[%s] can not dump verification data to /sdcard/pdo/__pd_verification_dump.pd !!", __FUNCTION__);
                }
            }
        }
        else
        ...
        if( ptrInputData)
        {
            ...
            if( Val==1)
            {
                ...
                if( m_profile.BufType & MSK_CATEGORY_OPEN)
                {
                    ...
                    PD_TRACE_FMT_BEGIN(TRACE_LEVEL_DEF, "ProcessBuf#%d", ptrInputData->magicNumber);
                ...
                else
                {
                    if( m_curLensPos)
                    {
                        ...
                        //
                        PD_TRACE_FMT_BEGIN(TRACE_LEVEL_DEF, "ConvertPDBufFormat#%d", ptrInputData->magicNumber);
                                ...
                                if( ptrConvertedLRBuf && m_PDBlockInfo.i4SubBlkW && m_PDBlockInfo.i4SubBlkH)
                                {
                                    ...
                                    if( (PDXsz==_xsz) && (PDYsz==_ysz) && (PDBufSz==_bufSz))
                                    {
                                        sprintf(fileName,
                                                ...
                                                (unsigned long long)(_GET_TIME_STAMP_US_()),
                                                ptrInputData->magicNumber,
                                                (unsigned int)(_bufSz*sizeof(MUINT16)),
                                                (unsigned int)(_xsz*sizeof(MUINT16)),
                                                _xsz,
                                                _ysz,
                                                ptrInputData->curLensPos);
                                        fp = fopen(fileName, "w");
                        ...
                        for( MUINT32 i=0; i<m_sAnalysisData.numDa; i++)
                        {
                            ...
                            PD_TRACE_FMT_BEGIN(TRACE_LEVEL_DEF, "extract_Fmt%d", sPDExtractInput.sFocusWin.sAFType);
                            ret = m_pIPdAlgo->extractPD(
                                      sPDExtractInput,
                                      m_sAnalysisData.Da[i].sHandlePDInput.sPDExtractData);
                            PD_TRACE_FMT_END(TRACE_LEVEL_DEF);

                            if( m_bDebugEnable || ret==E_3A_ERR)
                            {
                                PD_EXTRACT_DATA_T *pExtractedData = &(m_sAnalysisData.Da[i].sHandlePDInput.sPDExtractData);

                                CAM_LOGD( "#(%d) [%s] extractPD : win#%d type(%d->%d/%d), ROI[x(%4d->%4d/%4d),y(%4d->%4d/%4d),w(%4d->%4d/%4d),h(%4d->%4d/%4d)], buf[%p/%p/%p/%p]",
                                          m_MagicNumber,
                                          __FUNCTION__,
                                          i,
