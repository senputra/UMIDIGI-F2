...
public:
    ...
    {
        ConfigParam()
            : i4SensorDev(1), i4SensorIndex(0), u4TimeoutMs(5000), eEventType(E_Event_Vsync)
            {}
        ConfigParam(MINT32 _i4SensorDev, MINT32 _i4SensorIndex, MUINT32 _u4TimeoutMs, E_Event_Type _eEventType)
            : i4SensorDev(_i4SensorDev), i4SensorIndex(_i4SensorIndex), u4TimeoutMs(_u4TimeoutMs), eEventType(_eEventType)
            {}
        MINT32 i4SensorDev;
    ...
    virtual MINT32         waitDequeue(BUF_PORTS port) = 0;
protected:
    virtual ~IEventIrq(){}
};
};
