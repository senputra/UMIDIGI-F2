...
{
    CAM_LOGD("[%s] mMagicNumber(%d), i4SubsampleCount(%d)", __FUNCTION__, pStatisticBufInfo->mMagicNumber, m_rConfigInfo.i4SubsampleCount);
#if (CAM3_3A_ISP_40_EN)
    // update AE mode to statistic information.
    IResultPool* pResultPoolObj = IResultPool::getInstance(m_i4SensorDev);
    if(pResultPoolObj)
    {
        MINT32 magic = pStatisticBufInfo->mMagicNumber - m_rConfigInfo.i4SubsampleCount + 1;
        AEResultToMeta_T* pAEResult = (AEResultToMeta_T*)pResultPoolObj->getResult(magic,E_AE_RESULTTOMETA);
        pStatisticBufInfo->mu1AeMode = LIB3A_AE_MODE_ON;
        if(pAEResult != NULL)
        {
        ...
        FLASHResultToMeta_T* pFLASHResult = (FLASHResultToMeta_T*)pResultPoolObj->getResult(magic,E_FLASH_RESULTTOMETA);
