...
)
{
    ...
    if(m_pIAeAlgo != NULL) {
        ...
        CAM_LOGD("The AE algo class is NULL (AE target:%d)\n", u4AETargetValue);
    }
...
)
{
    ...
    if (pAEExpParam->u4GainMode == 0) { // AfeGain and isp gain
        ...
        if(m_p3ANVRAM != NULL) {
            u4AFEGain = (pAEExpParam->u4ISO * 1024) / m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MiniISOGain;
        } else {
    ...
    IspTuningMgr::getInstance().setIspFlareGainOffset((MUINT32)m_eSensorDev, m_rAEOutput.rPreviewMode.i2FlareGain, (-1*m_rAEOutput.rPreviewMode.i2FlareOffset));

    // Update capture exposure time/gain/flare
    if(m_p3ANVRAM != NULL) {
        if(m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Cap2PreRatio <= 300) {
            u4BinningRatio = 4;
        } else if(m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Cap2PreRatio <= 450) {
            u4BinningRatio = 3;
        } else if(m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Cap2PreRatio <= 768) {
            u4BinningRatio = 2;
        } else {
            u4BinningRatio = 1;
        }
    ...
    CAM_LOGD("[PV Flare Gain] = %d\n", pAEExpParam->u2FlareGain);
    CAM_LOGD("[CAP Flare] = %d\n", pAEExpParam->u2CaptureFlareValue);
    CAM_LOGD("[CAP Flare Gain] = %d\n", pAEExpParam->u2CaptureFlareGain);
    CAM_LOGD("[Flare Auto] = %d\n", pAEExpParam->bFlareAuto);
...
)
{
    ...
    if (pcfg_in->u4GainMode == 0) { // AfeGain and isp gain
        ...
        if(m_p3ANVRAM != NULL) {
            cap_mode.u4AfeGain = (pcfg_in->u4ISO * 1024) / m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MiniISOGain;
        } else {
            CAM_LOGD("NVRAM is NULL\n");
    ...
    CAM_LOGD("[CCTOSetCaptureParams] -- Cap. Flare Offset = %d\n", cap_mode.i2FlareOffset);
    CAM_LOGD("[CCTOSetCaptureParams] -- Cap. Flare Gain = %d\n", cap_mode.i2FlareGain);
    CAM_LOGD("[CCTOSetCaptureParams] -- Flare Auto = %d\n", m_bIsAutoFlare);
...
)
{
    ...
    CAM_LOGD("[CCTOGetCaptureParams] -- Cap Flare Offset = %d\n", ae_mode.i2FlareOffset);
    CAM_LOGD("[CCTOGetCaptureParams] -- Cap Flare Gain = %d\n", ae_mode.i2FlareGain);
    CAM_LOGD("[CCTOGetCaptureParams] -- Flare Auto = %d\n", m_bIsAutoFlare);
...
)
{
    ...
    if(m_p3ANVRAM != NULL) {
        pAEExpParamOut->u4ISO = m_rAEOutput.rPreviewMode.u4IspGain*(((m_rAEOutput.rPreviewMode.u4AfeGain * m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4MiniISOGain) + 512) / 1024) / 1024;
    } else {
        CAM_LOGD("NVRAM is NULL\n");
...
)
{
    ...
    // set nvram
    m_p3ANVRAM->rAENVRAM[eCamScenarioMode] = *pAENVRAM;

    // apply rAENVRAM only when rAENVRAM is the same set with the new one(pAENVRAM) ==> AENVRAMMapping[m_eCamScenarioMode] == u4CamScenarioMode
    MUINT32 u4AENVRAMIdx = AENVRAMMapping[m_eCamScenarioMode];
    CAM_LOGD("[ACDK_CCT_OP_DEV_AE_APPLY_INFO] applyScenario(%d), currentScenario(%d) -> mappingScenario(%d) \n", (int)eCamScenarioMode, (int)m_eCamScenarioMode, u4AENVRAMIdx);

    if (u4AENVRAMIdx == eCamScenarioMode)
    {
...
)
{
    ...
    memcpy(a_pAENVRAM, m_p3ANVRAM->rAENVRAM, sizeof(AE_NVRAM_T)*CAM_SCENARIO_NUM);
...
//    m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx] = m_rAEInitInput.rAENVRAM;
...
{
    CAM_LOGD("[ACDK_CCT_OP_AE_GET_ISP_OB] a_pIspOB=%d, a_pOutLen=%d\n",*a_pIspOB,*a_pOutLen);
/*
...
{
    CAM_LOGD("[ACDK_CCT_OP_AE_SET_ISP_OB] a_IspOB=%d\n",a_IspOB);
/*
...
)
{
    ...
    CAM_LOGD("[ACDK_CCT_OP_AE_GET_ISP_RAW_GAIN]\n");
...
{
    CAM_LOGD("[ACDK_CCT_OP_AE_SET_ISP_RAW_GAIN]\n");
    CAM_LOGD("ISP RAW Gain = %d\n", a_IspRAWGain);

    IspTuningMgr::getInstance().setISPAEGain((MUINT32)m_eSensorDev, MFALSE, a_IspRAWGain>>1);
...
)
{
    ...
    CAM_LOGD("[ACDK_CCT_OP_AE_SET_SENSOR_EXP_TIME]\n");
    CAM_LOGD("Exposure Time = %d\n", a_ExpTime);


    err = AAASensorMgr::getInstance().setSensorExpTime((ESensorDev_T)m_eSensorDev, a_ExpTime);
    if (FAILED(err)) {
        CAM_LOGE("[CCTOPAESetSensorExpTime] AAASensorMgr::getInstance().setSensorExpTime fail\n");
        return err;
    }
...
) const
{
    ...
    CAM_LOGD("[ACDK_CCT_OP_AE_SET_SENSOR_EXP_LINE]\n");
    CAM_LOGD("Exposure Line = %d\n", a_ExpLine);

    // Set exposure line
    err = AAASensorMgr::getInstance().setSensorExpLine((ESensorDev_T)m_eSensorDev, a_ExpLine);
    if (FAILED(err)) {
        CAM_LOGE("[CCTOPAESetSensorExpLine] AAASensorMgr::getInstance().setSensorExpLine fail\n");
        return err;
    }
...
) const
{
    ...
    CAM_LOGD("[ACDK_CCT_OP_AE_SET_SENSOR_GAIN]\n");
    CAM_LOGD("Sensor Gain = %d\n", a_SensorGain);

    err = AAASensorMgr::getInstance().setSensorGain((ESensorDev_T)m_eSensorDev, a_SensorGain);

    if (FAILED(err)) {
        CAM_LOGE("[CCTOPAESetSensorGain] AAASensorMgr::getInstance().setSensorGain fail\n");
        return err;
    }
...
)
{
    ...
    CAM_LOGD("[ACDK_CCT_OP_AE_CAPTURE_MODE]\n");

    // Update capture exposure time/gain/flare
    if(a_CaptureMode == 0) {   // preview mode
        u4SensitivityRatio = m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Cap2PreRatio;
    } else if(a_CaptureMode == 2) {
        u4SensitivityRatio = 1024 *m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Cap2PreRatio  / m_p3ANVRAM->rAENVRAM[m_u4AENVRAMIdx].rDevicesInfo.u4Video2PreRatio;
    }
    ...
    bStrobeOn = FlashMgr::getInstance(m_eSensorDev)->isFlashOnCapture();
#else
    bStrobeOn = FlashMgr::getInstance().isFlashOnCapture(m_eSensorDev);
#endif

    CAM_LOGD("Capture mode = %d Binning:%d bStrobeOn:%d\n", a_CaptureMode, u4BinningRatio, bStrobeOn);
    ...
    if(a_CaptureMode == 0) {      // preview mode or video mode
        if(bStrobeOn == TRUE) {
            ...
            if(m_pIAeAlgo != NULL) {
                ...
                CAM_LOGD("The AE algo class is NULL (15)\n");
            }
        ...
        CAM_LOGE("[ACDK_CCT_OP_AE_CAPTURE_MODE] Err capture mode:%d\n", a_CaptureMode);
   }
...
{
    CAM_LOGD("[ACDK_CCT_OP_DEV_AE_GET_FLARE_CALIBRATION]\n");
    ...
    *a_pOutLen = sizeof(MUINT32);
    ...
    CAM_LOGD("[Flare Calibration] Offset : %d Threshold:%d\n", *a_pAEFlareOffset, a_FlareThres);
