...
MBOOL allocWorkingBuf(sp<NSCam::IImageBuffer>& pImgBuf,  NRWorkingBufferInfo &MyWorkingBufInfo)
{
    ...
    for (i=0; i<MyWorkingBufInfo.WorkingBufferNum; i++)
    {
        ...
        if(pWorkingBuffer->MemType == NR_MEMORY_ION) {
            SWNR_LOGD("working buffer: normal heap");
    ...
    MUINT32 openId)
{
    ...
    IHalSensorList* const pHalSensorList = MAKE_HalSensorList();
    MUINT sensorDev = MAKE_HalSensorList()->querySensorDevIdx(openId);
    Paramctrl* pParamctrl = Paramctrl::getInstance((MUINT32)sensorDev, openId);
    ...
    pIspTuningCustom = IspTuningCustom::createInstance(
            static_cast<MUINT32>(sensorDev),
            sensorDevID);
    SWNR_ASSERT(pIspTuningCustom != NULL);

    // 1. update swnr type
    NRType = ESWNRType_2_DrvNRObject((ESWNRType)get_swnr_type(sensorDev));//always return eSWNRType_SW2
    {
    ...
    // get tuning info from NVRam
    {
        ...
        memcpy(&pTuningInfo->u.NR2, &(NvramData.fswnr.NR) , sizeof(NR2_PARAM));
        ...
        pTuningInfo->u.NR2.NR_SD      = generateSeed();
...
MBOOL SwNR::doSwNR(SWNRParam const& param, NSCam::IImageBuffer* pBuf)
{
    ...
    MyNRInitInfo.pNR2TuningInfo = &mTuningInfo.u.NR2;
...
#define addPair(debug_info, index, id, value)           \
        ...
        //
        if (mNRType == DRV_NR_OBJ_SW2 || mNRType == DRV_NR_OBJ_SW2_HIGH_PERF || mNRType == DRV_NR_OBJ_SW2_LOW_POWER) // fast swnr
        {
            addPair(dbgInfo , idx , NR_K      , mTuningInfo.u.NR2.NR_K);
            addPair(dbgInfo , idx , NR_S      , mTuningInfo.u.NR2.NR_S);
            addPair(dbgInfo , idx , NR_SD     , mTuningInfo.u.NR2.NR_SD);
            addPair(dbgInfo , idx , NR_BLD_W  , mTuningInfo.u.NR2.NR_BLD_W);
            addPair(dbgInfo , idx , NR_BLD_TH , mTuningInfo.u.NR2.NR_BLD_TH);
            addPair(dbgInfo , idx , NR_SMTH   , mTuningInfo.u.NR2.NR_SMTH);
            addPair(dbgInfo , idx , NR_NTRL_TH_1_Y   , mTuningInfo.u.NR2.NR_NTRL_TH_1_Y);
            addPair(dbgInfo , idx , NR_NTRL_TH_2_Y   , mTuningInfo.u.NR2.NR_NTRL_TH_2_Y);
            addPair(dbgInfo , idx , NR_NTRL_TH_1_UV   , mTuningInfo.u.NR2.NR_NTRL_TH_1_UV);
            addPair(dbgInfo , idx , NR_NTRL_TH_2_UV   , mTuningInfo.u.NR2.NR_NTRL_TH_2_UV);
        }
        else {
...
MVOID SwNR::dumpParam(char * const filename)
{
    ...
    {
        dumpParams(&mTuningInfo.u.NR2, fp);
    ...
    MINT32 const perfLevel )
{
    ...
    {
        SWNR_LOGD("[%s] PerfServiceNative_userRegScn succeed! handle = %d\n", LOG_TAG, handle_perf_serv_iir);
