...
createInstance(MUINT32 const eSensorDev, MINT32 i4SensorIdx)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<LscMgrDefault>(eSensorDev, i4SensorIdx);
    } );
...
LscMgrDefault(MUINT32 eSensorDev, MINT32 i4SensorIdx)
    ...
    , m_i4DbgCt(-1)
{
    GET_PROP("debug.lsc_mgr.log", 0, g_LogEnable);
...
init()
{
    ...
    GET_PROP("debug.lsc_mgr.ct", "-1", m_i4DbgCt);
...
loadTableFlow(MBOOL fgForceRestore)
{
    ...
    if (m_eAlignFlag != E_LSC_ALIGN_MTK_OTP_ALIGN_OK)
    {
        ...
        {
            if(m_pNvramOtp->getOtpState() == E_LSC_ALIGN_NO_OTP)
            {
...
getSensorResolution()
{
    ...
    {
        LSC_ERR("Fail to make HAL sensor list!");
        return MFALSE;
    }
...
{
    return m_pTsf->updateRatio(static_cast<MUINT32>(i4MagicNum),u4Rto);
}
#else
MBOOL
LscMgrDefault::
...
{
    return m_pTsf->updateRatio(u4MagicNum,u4Rto);
}
#endif
...
syncBufAndRatio(ILscBuf** pBuf, MUINT32 ratio, E_LSC_MODULE_T eModu)
{
    ...
    if (LSC_P1 == eModu)
    {
        ...
        CAM_LOGD_IF(fgLogEn, "[%s] LSCI addr %p", __func__, *pBuf);
