...
#include "../common/hal3a/buf_mgr/IBufMgr.h"
#else
#include <IBufMgr.h>
#endif
...
public:
    struct Cmd_T
    {
        ...
        Cmd_T(MUINT32 _u4Magic, MUINT32 _u4ExpTime, MUINT32 _u4SensorGain, MINT32 _i4TgInfo)
            ...
            , i4TgInfo(_i4TgInfo)
            {}
        Cmd_T(MUINT32 _u4Magic, MUINT32 _u4ExpTime, MUINT32 _u4SensorLEGain, MUINT32 _u4SensorSEGain, MINT32 _i4TgInfo)
            : u4MagicNum(_u4Magic)
            , u4ExposureTime(_u4ExpTime)
            , u4SensorGain(0)
...
public:
    struct Cmd_T
    {
        ...
        Cmd_T(MUINT32 _u4Magic, Result_T* _rPreResult, MUINT8 _u1IsGetExif, MBOOL _bIsCapEnd, AE_SENSOR_PARAM_T _rAESensorInfo)
            : u4MagicNumReq(_u4Magic)
            , rPreResult(_rPreResult)
...
class ThreadAEStart : public IThread
{
public:
    struct Cmd_T
    {
        MBOOL                   bAAOMode;
        MUINT32                 u4Scenario4AEAWB;

        Cmd_T()
            : bAAOMode(1)//EBitMode_14Bit
            , u4Scenario4AEAWB(0)//CAM_SCENARIO_PREVIEW
            {}

        Cmd_T(MBOOL _bAAOMode, MUINT32 _u4Scenario4AEAWB)
            : bAAOMode(_bAAOMode)
            , u4Scenario4AEAWB(_u4Scenario4AEAWB)
            {}
    };
    static ThreadAEStart*       createInstance(MUINT32 u4SensorDev, MINT32 i4SensorIndex);
    virtual MBOOL               destroyInstance() = 0;
    virtual MBOOL               postCmd(void* pArg) = 0;
    virtual MBOOL               waitFinished() = 0;
    virtual                     ~ThreadAEStart(){}
};

class ThreadAFStart : public IThread
{
public:
    struct Cmd_T
    {
        MBOOL                   bAAOMode;
        MUINT32                 u4Scenario4AF;

        Cmd_T()
            : bAAOMode(1)//EBitMode_14Bit
            , u4Scenario4AF(0)//CAM_SCENARIO_PREVIEW
            {}

        Cmd_T(MBOOL _bAAOMode, MUINT32 _u4Scenario4AF)
            : bAAOMode(_bAAOMode)
            , u4Scenario4AF(_u4Scenario4AF)
            {}
    };
    static ThreadAFStart*       createInstance(MUINT32 u4SensorDev, MINT32 i4SensorIndex);
    virtual MBOOL               destroyInstance() = 0;
    virtual MBOOL               postCmd(void* pArg) = 0;
    virtual MBOOL               waitFinished() = 0;
    virtual                     ~ThreadAFStart(){}
};
