...
GmaMgr*
GmaMgr::
createInstance(MUINT32 const eSensorDev, ISP_NVRAM_TONEMAP_PARAM_T& rIspNvramReg, ISP_NVRAM_GMA_TUNING_PARAM_T* pIspNvramGmaPara)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<GmaMgr>(eSensorDev, rIspNvramReg, pIspNvramGmaPara);
    } );

    return rSingleton.instance.get();
}

GmaMgr::GmaMgr(MUINT32 const eSensorDev, ISP_NVRAM_TONEMAP_PARAM_T& rIspNvramReg, ISP_NVRAM_GMA_TUNING_PARAM_T* pIspNvramGmaPara)
    ...
    , mfgGMA_HDR(MFALSE)
{
    ...
    if (mAlgoEn) {
        CAM_ULOG_ASSERT(Utils::ULog::MOD_ISP_MGR, m_pGmaAlgo != nullptr, "[%s]: m_pGmaAlgo is nullptr!", __FUNCTION__);
        m_pGmaAlgo->GmaReset();
    }
