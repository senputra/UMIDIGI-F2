...
MINT32 FlickerHal::init(MINT32 i4SensorOpenIndex)
{
    ...
    if (propH) {
        m_u4BlkH = propH;
        logV("Update window height(%d).", m_u4BlkH);
    }
    propW = FlickerUtil::getPropInt("vendor.flk_w",0);
    if (propW) {
        m_u4BlkW = propW;
        logV("Update window width(%d).", m_u4BlkW);
    }
    ...
    logV("flicker_init(%d,%d,%d,%d).",m_u4PixelsInLine, m_u4BlkH * 3, m_u4BlkW, (int)m_u4SensorPixelClkFreq);
    ta = FlickerUtil::getMs();
    flicker_init(m_u4PixelsInLine, m_u4BlkH * 3, m_u4BlkW, m_u4SensorPixelClkFreq);
...
//=============================================
MINT32 FlickerHal::setWindowInfo(int* tgInfo, MUINT32* blkH, MUINT32* blkW, MUINT32* winH, MUINT32* winW)
{
    logV("func=%s line=%d",__FUNCTION__, __LINE__);
    int imgW=0;
    int imgH=0;
    ...
    *tgInfo = rSensorDynamicInfo.TgInfo;
...
//    if(atoi(value))
    ...
    ISP_MGR_FLK_CONFIG_T::getInstance(static_cast<MUINT32>(m_sensorDev)).config(strFlkWinCfg, *tgInfo);
...
MBOOL FlickerHal::Start()
{
    ...
    setWindowInfo(&mTgInfo, &m_u4BlkH, &m_u4BlkW, &m_u4FlickerHeight, &m_u4FlickerWidth);
