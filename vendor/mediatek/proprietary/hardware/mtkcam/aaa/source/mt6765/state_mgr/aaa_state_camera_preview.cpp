...
doUpdate()
{
    ...
    if(!m_pStateMgr->getEnableShortExposure())
    {
        ...
        } else
        {
            ...
            AAA_TRACE_L(getAAOInfo);
            IHal3AResultBufInfo::getInstance(m_SensorDevId)->getAAOInfo(AE_V4P0_BLOCK_NO, rBufInfo->mMagicNumber, m_rAAOInfo);
            AAA_TRACE_END_L;
            m_rAAOInfo.Pso_Output_Path = (PSO_PATH_SEL_ENUM)rPSOBufInfo->mRawType; // PSO before OB or after OB
            // AE
            AAA_TRACE_L(setAAOProcInfo);
            IAeMgr::getInstance().setAAOProcInfo(m_SensorDevId, reinterpret_cast<MVOID *>(rPSOBufInfo->getPart(PSOSepBuf)), &m_rAAOInfo);
            AAA_TRACE_END_L;
            AAA_TRACE_L(doPvAE);
    ...
    // AWB
    AAA_TRACE_L(AWB);
    IAwbMgr::getInstance().doPvAWB(m_SensorDevId, m_pStateMgr->getFrameCount(), bAEStable, i4AoeCompLv, reinterpret_cast<MVOID *>(rBufInfo->getPart(AAOSepLSCBuf)), rRTParams.u4PreviewShutterSpeed_us);
