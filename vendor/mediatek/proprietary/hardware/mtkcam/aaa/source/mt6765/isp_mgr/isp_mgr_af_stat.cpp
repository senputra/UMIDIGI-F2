...
start(MINT32 i4SensorDev, MINT32 i4SensorIdx)
{
    ...
    if (m_bIsAFSupport != 0)
    {
        CAM_LOGI("[%s] Support AF", __FUNCTION__);
        ...
        *-----------------------------------------------------------------------------------------------------*/
        ...
        {
            CAM_LOGI("[%s] AF control on CCU", __FUNCTION__);
            m_pICcuCtrlAf = NSCcuIf::ICcuCtrlAf::createInstance((const uint8_t *)"isp_mgr_af",(MUINT32)i4SensorDev);
            if (m_pICcuCtrlAf->init((MUINT32)i4SensorIdx, (MUINT32)i4SensorDev) == CCU_CTRL_SUCCEED)
            {
    ...
    {
        CAM_LOGI("[%s] disable AF", __FUNCTION__);
    }
    ...
    CAM_LOGI("[%s] SensorDev(%d) SensorIdx(%d) -", __FUNCTION__, i4SensorDev, i4SensorIdx);
}
...
CheckBLKNumX( MINT32 &i4InHWBlkNumX, MINT32 &i4OutHWBlkNumX)
{
    ...
    {
        CAM_LOGI("%s WAR : set x blk to 1", __FUNCTION__);
        i4OutHWBlkNumX = MIN_AF_HW_WIN_X;
    }
    else if( i4InHWBlkNumX>MAX_AF_HW_WIN_X)
    {
        CAM_LOGI("%s WAR : set x blk to 128", __FUNCTION__);
...
CheckBLKNumY( MINT32 &i4InHWBlkNumY, MINT32 &i4OutHWBlkNumY)
{
    ...
    {
        CAM_LOGI("%s WAR : set y blk to 1", __FUNCTION__);
        i4OutHWBlkNumY = MIN_AF_HW_WIN_Y;
    }
    else if( i4InHWBlkNumY>MAX_AF_HW_WIN_Y)
    {
        CAM_LOGI("%s WAR : set y blk to 128", __FUNCTION__);
...
config(AF_CONFIG_T &a_sAFConfig, AREA_T &sOutHWROI, MINT32 &i4OutHWBlkNumX, MINT32 &i4OutHWBlkNumY, MINT32 &i4OutHWBlkSizeX, MINT32 &i4OutHWBlkSizeY, MINT32 &i4FirstTimeConfig)
{
    ...
    {
        CAM_LOGI_IF( m_bDebugEnable, "[%s] disable AF", __FUNCTION__);
        return MFALSE;
    }
    ...
    CAM_LOGI_IF( m_bDebugEnable, "(0x1A004800) : AFCon 0x%x, V_Gonly %d, V_AVG_LVL %d",
           reg_af_con.Raw,
           af_v_gonly,
           af_v_avg_lvl);
            ...
            (BIN_SZ_W < (Roi2HWCoord.i4X+Roi2HWCoord.i4W))) /*X*/
    {
        ...
        CAM_LOGI("HW-%s WAR : X %d->%d, W %d->%d", __FUNCTION__, Roi2HWCoord.i4X, x, Roi2HWCoord.i4W, w);
        Roi2HWCoord.i4X = x;
        Roi2HWCoord.i4W = w;
    }
            ...
            (BIN_SZ_H < (Roi2HWCoord.i4Y+Roi2HWCoord.i4H))) /*Y*/
    {
        ...
        CAM_LOGI("HW-%s WAR : Y %d->%d, H %d->%d", __FUNCTION__, Roi2HWCoord.i4Y, y, Roi2HWCoord.i4H, h);
        Roi2HWCoord.i4Y = y;
        Roi2HWCoord.i4H = h;
    }
    ...
    if( win_h_size > 80)
    {
        ...
        CAM_LOGI("HW-%s WAR : BlkSzW set to 80, blkNumX %d->%d", __FUNCTION__, i4OutHWBlkNumX, nh);
    ...
    else if( win_h_size < minHSz)
    {
        win_h_size = minHSz;
        MINT32 nh = Roi2HWCoord.i4W / win_h_size;
        CAM_LOGI("HW-%s WAR : BlkSzW set to min Sz %d, (%d,%d), blkNumX %d->%d", __FUNCTION__, minHSz, af_v_avg_lvl, af_v_gonly, i4OutHWBlkNumX, nh);
    ...
    if (win_v_size > 255)
    {
        ...
        CAM_LOGI("HW-%s WAR : BlkSzH set to 255, blkNumY %d->%d", __FUNCTION__, i4OutHWBlkNumY, nv);
    ...
    else if( win_v_size < minVSz)
    {
        win_v_size = minVSz;
        MINT32 nv = Roi2HWCoord.i4H / win_v_size;
        CAM_LOGI("HW-%s WAR : BlkSzH set to min Sz 1, blkNumY %d->%d", __FUNCTION__, i4OutHWBlkNumY, nv);
    ...
    CAM_LOGI_IF( m_bDebugEnable, "(0x1A004838) : AF_blk_sz [W]%d [H]%d",
                 win_h_size,
                 win_v_size);
    ...
    if( BIN_SZ_W < (MINT32)(Roi2HWCoord.i4X+i4OutHWBlkNumX*win_h_size))
    {
        ...
        CAM_LOGI("HW-%s WAR : X %d->%d", __FUNCTION__, Roi2HWCoord.i4X, x);
        Roi2HWCoord.i4X = x;
    }
    ...
    if( BIN_SZ_H < (MINT32)(Roi2HWCoord.i4Y+i4OutHWBlkNumY*win_v_size))
    {
        ...
        CAM_LOGI("HW-%s WAR : Y %d->%d", __FUNCTION__, Roi2HWCoord.i4Y, y);
        Roi2HWCoord.i4Y = y;
    }
    ...
    CAM_LOGI_IF( m_bDebugEnable, "(0x1A00483C) : AF_blk_num [nX]%d [nY]%d",
                 i4OutHWBlkNumX,
                 i4OutHWBlkNumY);
    ...
    CAM_LOGI_IF( m_bDebugEnable, "ROI-BIN : [X]%4d [Y]%4d [W]%4d [H]%4d -> [X]%4d [Y]%4d [W]%4d [H]%4d",
                 Roi2HWCoordTmp.i4X,
                 Roi2HWCoordTmp.i4Y,
                 Roi2HWCoordTmp.i4W,
                 Roi2HWCoordTmp.i4H,
    ...
    CAM_LOGI_IF( m_bDebugEnable, "ROI-TG  : [X]%4d [Y]%4d [W]%4d [H]%4d -> [X]%4d [Y]%4d [W]%4d [H]%4d",
    ...
    MUINT32 ysize = reg_af_blk_1.Bits.AF_BLK_YNUM;
    {
        ...
        {
            CAM_LOGI_IF( m_bDebugEnable, "Ext Mode : [xsize]%d [AF_H_GONLY]%d", xsize, reg_af_con.Bits.AF_H_GONLY);
            xsize = reg_af_blk_1.Bits.AF_BLK_XNUM*32;
            reg_af_con.Bits.AF_H_GONLY = 0;
            CAM_LOGI_IF( m_bDebugEnable, "Ext Mode Constraint : [xsize]%d [AF_H_GONLY]%d", xsize, reg_af_con.Bits.AF_H_GONLY);
        }
    }

    CAM_LOGI_IF( m_bDebugEnable, "i4FirstTimeConfig(%d)", i4FirstTimeConfig);
    /*-----------------------------------------------------------------------------------------------------
     *
     *                                       AF HW Control Flow
     *
    ...
    if (m_bIsCCUSupport > 0)
    {
        ...
        memcpy((void *)&m_rIspAFRegInfo.data_b, (void *)&m_rIspAFRegInfo.data, sizeof(CAM_REG_AF_DATA));
        {
            ...
            if (m_pICcuCtrlAf)
            {
                ...
                else if (m_pICcuCtrlAf->afo_done_valid)
                {
                    if (m_bPreStop == MFALSE)
                    {
                        if (m_bIsZECChg > 0)
                        {
                            ...
                            CAM_LOGI_IF( m_bDebugEnable, "CCU : Config(%d) AFDone(%d), ExecuteAF(%d), P1Done(%d), DelayTime(%d)", a_sAFConfig.u4ConfigNum, AFTimeReadOut, m_i4AFExecuteTime, m_i4P1ReadOutTime, i4Tplus2DelayTime);
                ...
                {
                    CAM_LOGI("[%s][CCU] AF done is invalid\n", __FUNCTION__);
                }
            }
        }
        ...
        if (m_bDebugEnable & 2)
        {
            ...
            {
                CAM_LOGI("CCU_REG_S : Addr : %04x\tData : %x", m_rIspAFRegInfo.addr[i], m_rIspAFRegInfo.data.val[i]);
                CAM_LOGI("CCU_REG_A : Addr : %04x\tData : %x", m_rIspAFRegInfo.addr[i], m_rIspAFRegInfo.data_a.val[i]);
                CAM_LOGI("CCU_REG_B : Addr : %04x\tData : %x", m_rIspAFRegInfo.addr[i], m_rIspAFRegInfo.data_b.val[i]);
            }
            CAM_LOGI("CCU_binWidth : %d, configNum : %d", m_rIspAFRegInfo.binWidth, m_rIspAFRegInfo.configNum);
        }
        ...
        if (memcmp((void *)&m_rIspAFRegInfo.data, (void *)&m_rPreIspRegData, sizeof(CAM_REG_AF_DATA)) != 0)
        {
            ...
            CAM_LOGI("HW-AFOSz %d %d ; AFSz %d", xsize, ysize, reg_af_size.Bits.AF_IMAGE_WD);
            ...
            CAM_LOGI("HW-Start %d %d ; BlkSz %d %d ; BlkNum %d %d",
                     ...
                     reg_af_blk_1.Bits.AF_BLK_XNUM,
                     reg_af_blk_1.Bits.AF_BLK_YNUM);
            CAM_LOGI("HW-TH_H %d %d ; TH_V %d ; TH_G %d",
            ...
            CAM_LOGI("HW-SGG %d, %d, %d, %d, %d, %d, %d ; SGGGain %d",
            ...
            CAM_LOGI("HW-EXT Mode %d", reg_af_con.Bits.AF_EXT_STAT_EN);
            CAM_LOGI("HW-TH_R %d TH_B %d", reg_af_th_1.Bits.AF_R_SAT_TH, reg_af_th_1.Bits.AF_B_SAT_TH);
            CAM_LOGI("HW-SGG5 %d, %d, %d, %d, %d, %d, %d ; SGG5Gain %d",
            ...
            CAM_LOGI("HW-%s Config(%d)", __FUNCTION__, a_sAFConfig.u4ConfigNum);
        }
...
ConfigUpdate()
{
    if (memcmp( m_rIspRegInfo, m_rIspRegInfoPre, ERegInfo_NUM*sizeof(RegInfo_T)) != 0)
    {
        ...
        CAM_LOGI("HW : CAM_AFO_XSIZE(0x%x) CAM_AFO_YSIZE(0x%x) CAM_AF_SIZE(0x%x)",
                  REG_INFO_VALUE(CAM_AFO_XSIZE), REG_INFO_VALUE(CAM_AFO_YSIZE), REG_INFO_VALUE(CAM_AF_SIZE));

        CAM_LOGI("HW : CAM_AF_CON(0x%x) CAM_AF_VLD(0x%x) CAM_AF_BLK_0(0x%x) CAM_AF_BLK_1(0x%x)",
                  REG_INFO_VALUE(CAM_AF_CON), REG_INFO_VALUE(CAM_AF_VLD), REG_INFO_VALUE(CAM_AF_BLK_0), REG_INFO_VALUE(CAM_AF_BLK_1));

        CAM_LOGI("HW : CAM_AF_TH_0(0x%x) CAM_AF_TH_1(0x%x) CAM_AF_TH_2(0x%x)",
                  REG_INFO_VALUE(CAM_AF_TH_0), REG_INFO_VALUE(CAM_AF_TH_1), REG_INFO_VALUE(CAM_AF_TH_2));

        CAM_LOGI("HW : CAM_SGG1_PGN(0x%x) CAM_SGG1_GMRC_1(0x%x) CAM_SGG1_GMRC_2(0x%x)",
                  REG_INFO_VALUE(CAM_SGG1_PGN), REG_INFO_VALUE(CAM_SGG1_GMRC_1), REG_INFO_VALUE(CAM_SGG1_GMRC_2));

        CAM_LOGI("HW : CAM_SGG5_PGN(0x%x) CAM_SGG5_GMRC_1(0x%x) CAM_SGG5_GMRC_2(0x%x)",
                  REG_INFO_VALUE(CAM_SGG5_PGN), REG_INFO_VALUE(CAM_SGG5_GMRC_1), REG_INFO_VALUE(CAM_SGG5_GMRC_2));

        CAM_LOGI("HW : CAM_AF_FLT_1(0x%x) CAM_AF_FLT_2(0x%x) CAM_AF_FLT_3(0x%x) CAM_AF_FLT_4(0x%x)",
                  REG_INFO_VALUE(CAM_AF_FLT_1), REG_INFO_VALUE(CAM_AF_FLT_2), REG_INFO_VALUE(CAM_AF_FLT_3), REG_INFO_VALUE(CAM_AF_FLT_4));

        CAM_LOGI("HW : CAM_AF_FLT_5(0x%x) CAM_AF_FLT_6(0x%x) CAM_AF_FLT_7(0x%x) CAM_AF_FLT_8(0x%x)",
                  REG_INFO_VALUE(CAM_AF_FLT_5), REG_INFO_VALUE(CAM_AF_FLT_6), REG_INFO_VALUE(CAM_AF_FLT_7), REG_INFO_VALUE(CAM_AF_FLT_8));

        CAM_LOGI("HW-%s Config(%d)", __FUNCTION__, m_i4ConfigNum);
    }
...
apply(TuningMgr& rTuning, MINT32 &i4Magic, MINT32 i4SubsampleIdex)
{
    if (m_bIsCCUSupport > 0)
    {
        ...
        CAM_LOGI_IF( m_bDebugEnable, "[CCU] rTuning(%p), i4Magic(%d), i4SubsampleIdex(%d)", &rTuning, i4Magic, i4SubsampleIdex);
    }
    else
    {
        /*-----------------------------------------------------------------------------------------------------
        ...
        if (m_bIsAFSupport != MFALSE)
        {
            ...
            if (m_bDebugEnable)
            {
                CAM_LOGI("HW-%s #(%d) : Magic #(%d), Config #(%d)", __FUNCTION__, i4Magic, m_sAFMapping[MappingIdx].i4MagicNum, m_sAFMapping[MappingIdx].i4ConfigNum);
                if (m_bDebugEnable & 2)
                {
                    for (MINT32 i = 0; i < MAX_MAPPING_NUM; i++)
                    {
                        CAM_LOGI("HW-%s (%d),  Magic #(%d), Config #(%d)", __FUNCTION__, i, m_sAFMapping[i].i4MagicNum, m_sAFMapping[i].i4ConfigNum);
                    }
                }
            }
        }
...

        {
            ...
            if( m_bIsApplied==MFALSE)
            {
                ...
                CAM_LOGI("HW-%s Magic(%d) , Config(%d)", __FUNCTION__, m_i4Magic, m_i4AppliedConfigNum);
            }
        }
    }

    CAM_LOGI_IF(m_bDebugEnable, "%s", __FUNCTION__);
    return MTRUE;
}
...
isHWRdy( MUINT32  &i4Config)
{
    ...
    if( m_bIsApplied==MTRUE)
    {
        ...
        {
            CAM_LOGI("HW-Rdy (%d) - (%d)", m_i4AppliedConfigNum, i4Config);
        }
    }
    else
    {
    ...
    CAM_LOGI_IF((ret != MTRUE) ? MTRUE : MFALSE, "%s Config[%d, %d] IsApplied(%d)", __FUNCTION__, m_i4AppliedConfigNum, i4Config, m_bIsApplied);
...
getConfigNum( MUINT32  &i4Magic)
{
    ...
    if (m_bIsCCUSupport <= 0)
    {
        ...
        CAM_LOGI_IF(m_bDebugEnable, "HW-%s : Magic #(%d), Config #(%d)", __FUNCTION__, i4Magic, ConfigNum);
    }
...
sendAFConfigCtrl(MUINT32 eAFCtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    switch(eAFCtrl)
    {
        case EAFConfigCtrl_GetAFConfigLatency:
            ...
            if ((MUINT32)iArg1 > 0)
            {
                for (MUINT32 i = 0; i < MAX_CONFIG_DETECT_NUM; i++)
                {
                    ...
                    {
                        CAM_LOGI_IF(m_bDebugEnable, "HW-%s : Config #(%d), Latency #(%d)",
        ...
        case EAFConfigCtrl_SetTimeReadOutPerLine:
            ...
            CAM_LOGI("%s : ReadOutTimePerLine(%d), P1ReadOutTime(%d)", __FUNCTION__, m_i4ReadOutTimePerLine, m_i4P1ReadOutTime);
            break;
        ...
        case EAFConfigCtrl_GetFrameRateInfo:
            ...
            *(reinterpret_cast<MUINT32*>(iArg2)) = m_u4CurrentFPS;
            CAM_LOGI("%s : GetFrameRateInfo - SensorMaxFPS(%d), CurrentFPS(%d)", __FUNCTION__, *(reinterpret_cast<MUINT32*>(iArg1)), *(reinterpret_cast<MUINT32*>(iArg2)));
            break;
...
isCCUSupport(MINT32 i4SensorDev, MINT32 i4SensorIdx)
{
    ...
    CAM_LOGI("[%s] (%d) Dev(%d) Idx(%d) ", __FUNCTION__, m_bIsCCUSupport, i4SensorDev, i4SensorIdx);
    if (m_bIsCCUSupport == -1)
    {
        ...
        if (m_bIsCCUSupport)
        {
            ...
            if (m_u4SensorMaxFPS > CCU_SUPPORT_FRAME_RATE && m_u4CurrentFPS <= CCU_SUPPORT_FRAME_RATE)
            {
                m_u4EnHighFPSMode = 1;
                m_bIsCCUSupport = 1;
                CAM_LOGI("[%s] Enable CCU_AF due to adjusting frame rate. SensorMaxFPS(%d), CurrentFPS(%d)", __FUNCTION__, m_u4SensorMaxFPS, m_u4CurrentFPS);
            }
            else if (!(VerticalBlanking > 0 && VerticalBlanking > CCU_SUPPORT_VBLANKRANGE))
            {
                m_bIsCCUSupport = 0;
                CAM_LOGI("[%s] Disable CCU_AF due to VBlanking is too small", __FUNCTION__);
            }
            #endif
            ...
            {
                m_bIsCCUSupport = 0;
                CAM_LOGI("[%s] Disable CCU_AF(%d) by ADB", __FUNCTION__, m_bIsCCUSupport);
            }

            if (m_bIsSMVR != MFALSE)
            {
                m_bIsCCUSupport = 0;
                CAM_LOGI("[%s] SMVR doen't support CCU", __FUNCTION__);
            }
        }
        ...
        {
            m_bIsCCUSupport = 1;
            CAM_LOGI("[%s] Enable CCU_AF(%d) by ADB", __FUNCTION__, m_bIsCCUSupport);
        }
    }
    else
    {
        CAM_LOGI("[%s] Enable CCU_AF(%d)", __FUNCTION__, m_bIsCCUSupport);
    }
...
reconfig(MVOID *pDBinInfo, MVOID *pOutRegCfg)
{
    if (m_bIsCCUSupport <= 0)
    {
        ...
        CAM_LOGI_IF(m_bDebugEnable, "[%s] : DBin (%d , %d) -> (%d , %d)", __FUNCTION__,
        ...
        CAM_LOGI_IF(m_bDebugEnable, "[%s] CAM_AF_SIZE : %x +", __FUNCTION__, *(pIspReg+u4Offset));
        *(pIspReg+u4Offset) = reg_af_size.Raw;
        ...
        if (psDBinInfo->CurBinOut_W > psDBinInfo->TarBinOut_W)
        {
            ...
            CAM_LOGI_IF(m_bDebugEnable, "[%s] CAM_AF_VLD : %x +", __FUNCTION__, *(pIspReg+u4Offset));
