...
public:
    ISP_MGR_GGM(MUINT32 const eSensorDev)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_NUM, m_u4StartAddr, eSensorDev)
        , m_bEnable(MTRUE)
        , m_bLNREnable(MFALSE)
        , m_bCCTEnable(MTRUE)
        , m_u4StartAddr(REG_ADDR_P2(DIP_X_GGM_CTRL))
    {
        // register info addr init
        INIT_REG_INFO_ADDR_P2(DIP_X_GGM_CTRL); // CAM+0x1480

        // CAM_GGM_RB_GMT
        MUINT32 u4StartAddr = REG_ADDR_P2(DIP_X_GGM_LUT_RB[0]);
        for (MINT32 i = 0; i < GGM_LUT_SIZE; i++) {
            m_rIspRegInfo_GGM_RB[i].addr = u4StartAddr + 4*i;
            //CAM_LOGD("m_rIspRegInfo_GGM_RB[%d].addr = 0x%8x", i, m_rIspRegInfo_GGM_RB[i].addr);
        }

        // CAM_GGM_G_GMT
        u4StartAddr = REG_ADDR_P2(DIP_X_GGM_LUT_G[0]);
        for (MINT32 i = 0; i < GGM_LUT_SIZE; i++) {
            m_rIspRegInfo_GGM_G[i].addr = u4StartAddr + 4*i;
            //CAM_LOGD("m_rIspRegInfo_GGM_G[%d].addr = 0x%8x", i, m_rIspRegInfo_GGM_G[i].addr);
        }
    }
...
public: // Interfaces.
    ...
    MVOID
    setLNREnable(MBOOL bEnable)
    {
        m_bLNREnable = bEnable;
    }
