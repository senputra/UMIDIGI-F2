...
dequeueSwBuf()
{
    ...
    if (m_pICcuCtrlAf != NULL)
    {
        if (m_pICcuCtrlAf->afo_done_valid == MTRUE)
        {
            ...
            NSIspTuningv3::ISP_MGR_AF_STAT_CONFIG_T::getInstance(static_cast<MUINT32>(m_i4SensorDev)).sendAFConfigCtrl(EAFConfigCtrl_GetAFConfigLatency, m_rHwBuf.mConfigNum, reinterpret_cast<MINTPTR>(&m_u4ConfigLatency));
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        m_rHwBuf.mConfigNum = NSIspTuningv3::ISP_MGR_AF_STAT_CONFIG_T::getInstance(static_cast<MUINT32>(m_i4SensorDev)).getConfigNum( m_rHwBuf.mMagicNumber);
        m_rHwBuf.mConfigLatency = m_u4ConfigLatency;
