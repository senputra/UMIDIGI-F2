...
sendIntent(intent2type<eIntent_VsyncUpdate>)
{
    ...
    if(u4PrecapCnt == 1) // pre capture start
    {
        m_pStateMgr->startTimeMonitor();
    }
    ...
    {
        EPrecapState_T preState = m_pStateMgr->getPrecapState();
        ...
        EPrecapState_T curState = m_pStateMgr->getPrecapState();
        if(curState != preState)
        {
            m_pStateMgr->setTakeTime(eTakeTime_AE);
        }
    }
    ...
    if ((m_pStateMgr->getPrecapState() == ePrecap_Flash) && (m_pStateMgr->mAFStateCntSet.bNeedToDoPrecapAF == 0))
    {
        ...
        if(curState != preState)
        {
            m_pStateMgr->setTakeTime(eTakeTime_Flash);
        }
    }
...
sendPrecapIntent(intent2type<eIntent_VsyncUpdate>, state2type<ePrecap_Flash>, MVOID* pBufInfo)
{
    ...
    {
        CAM_LOGW("[%s] Flash has opened, bypass Flash status", __FUNCTION__);
        m_pStateMgr->proceedPrecapState();
        return S_3A_OK;
    }
  MRESULT err = S_3A_OK;
    ...
    if(flashOnCnt == 1)
    {
        ...
        CAM_LOGW("[%s] update flash on for precapture , frmRate(%d)", __FUNCTION__, frmRate);
    ...
    else if(flashOnCnt == 2 && m_pStateMgr->getPreCapFlashOn() == MFALSE)
    {
        ...
        AE_MODE_CFG_T rPreviewInfo;
        IAeMgr::getInstance().getPreviewParams(m_SensorDevId,rPreviewInfo);
        // set minimal frame length
        MINT32 frmRate = 300;
        AAASensorMgr::getInstance().setPreviewMaxFrameRate(
            m_SensorDevId,
            frmRate,
            IAeMgr::getInstance().getSensorMode(m_SensorDevId));
        // use exposure time to extend frame length
        AAASensorMgr::getInstance().setSensorExpTime(
            m_SensorDevId,
            FlashMgr::getInstance().getExp(m_SensorDevId)
        );
        if(cust_getFlashFrameRateDelay(m_SensorDevId) == 0)
        {
            // set flag for p1done to query.
            m_pStateMgr->updatePreCapFlashOn(MTRUE);
            CAM_LOGW("[%s] update flash on for precapture , MaxFrameRate(%d)", __FUNCTION__, frmRate);
        }
    }
    else if(flashOnCnt == 3 && m_pStateMgr->getPreCapFlashOn() == MFALSE)
    {
        if(cust_getFlashFrameRateDelay(m_SensorDevId) == 1)
        {
            // set flag for p1done to query.
            m_pStateMgr->updatePreCapFlashOn(MTRUE);
            CAM_LOGW("[%s] update flash on for precapture", __FUNCTION__);
        }
    }
    ...
    if(rep.isEnd==1)
    {
        ...
        if (isFlashAWBv2Enabled(m_SensorDevId)) {
