...
enum
{
    ...
    e_SensorSub2=DUAL_CAMERA_SUB_2_SENSOR,
};
...
{   \
    ...
    if (senorDev & e_SensorSub2)  \
        errSub2 = FlashMgrM::getInstance(e_SensorSub2)->function();  \
...
{   \
    ...
    if (senorDev & e_SensorSub2)  \
        errSub2 = FlashMgrM::getInstance(e_SensorSub2)->function(v1);  \
    return errOp(errMain, errSub, errMain2, errSub2);    \
}

#define def_Imp_Cond(function) \
int FlashMgr::function(int senorDev)  \
{   \
    int errMain=0; \
    int errSub=0; \
    int errMain2=0;   \
    int errSub2=0; \
    if ((senorDev & e_SensorMain) && FlashMgrM::getInstance(e_SensorMain)->getIsDriverInControl())    \
        errMain = FlashMgrM::getInstance(e_SensorMain)->function();    \
    if ((senorDev & e_SensorSub) && FlashMgrM::getInstance(e_SensorSub)->getIsDriverInControl()) \
        errSub = FlashMgrM::getInstance(e_SensorSub)->function();  \
    if ((senorDev & e_SensorMain2) && FlashMgrM::getInstance(e_SensorMain2)->getIsDriverInControl())    \
        errMain2 = FlashMgrM::getInstance(e_SensorMain2)->function();  \
    if ((senorDev & e_SensorSub2) && FlashMgrM::getInstance(e_SensorSub2)->getIsDriverInControl())    \
        errSub2 = FlashMgrM::getInstance(e_SensorSub2)->function();  \
    return errOp(errMain, errSub, errMain2, errSub2);    \
}


#define def_Imp1_Cond(function) \
int FlashMgr::function(int senorDev, int v1)  \
{   \
    int errMain=0; \
    int errSub=0; \
...
{   \
    ...
    else if (senorDev & e_SensorSub2)  \
        return FlashMgrM::getInstance(e_SensorSub2)->function();  \
    else \
...
#define def_ImpBody1(function,senorDev,p1) \
    ...
    if (senorDev & e_SensorSub2)  \
        errSub2 = FlashMgrM::getInstance(e_SensorSub2)->function(p1);  \
    return errOp(errMain, errSub, errMain2, errSub2);    \
...
#define def_ImpBody2(function,senorDev,p1,p2) \
    ...
    if (senorDev & e_SensorSub2)  \
        errSub2 = FlashMgrM::getInstance(e_SensorSub2)->function(p1,p2);  \
    return errOp(errMain, errSub, errMain2, errSub2);    \
...
int FlashMgr::setChildSub2(void* child)
{
    mChildSub2 = child;
    return 0;
}
...
int FlashMgr::setTorchLevel(int level)
{
    ...
    if(mChildSub2!=0)
        ((FlashMgrM*)mChildSub2)->setTorchLevel(level);
    return 0;
}
...
int FlashMgr::cctGetQuickCalibrationResult(int senorDev)
{
    ...
    if (senorDev & e_SensorSub2)
        errSub2 = FlashMgrM::getInstance(e_SensorSub2)->cctGetQuickCalibrationResult();

    return errMain|errSub|errMain2|errSub2;
...
def_Imp(start)
def_Imp(stop)
def_Imp_Cond(turnOffFlashDevice)
...
def_Imp_Cond(endPrecapture)
def_Imp(capCheckAndFireFlash_Start)

def_Imp(notifyAfEnter)
def_Imp(notifyAfExit)
def_Imp_Cond(setCapPara)
def_Imp_Cond(setPfParaToAe)



def_Imp1(setMultiCapture)
def_Imp1_Cond(setCaptureFlashOnOff)
def_Imp1(setIsDriverInControl)
// Modify
...
int FlashMgr::getFlashState(int senorDev)
{
    ...
    if (senorDev & e_SensorSub2)
        ret |= FlashMgrM::getInstance(e_SensorSub2)->getFlashState();
    return ret;
}
...
// one para
//===================================================
def_Imp1_Cond(setAFLampOnOff)
//def_Imp1(setFlashMode)
def_Imp1(setCamMode)
def_Imp1(setDigZoom)
def_Imp1_Cond(setTorchOnOff)
def_Imp1(setPreFlashOnOff)
def_Imp1(setAAOMode)
...
def_Imp1_Cond(setIsFlashOnCapture)
...
def_ImpTypeGet(getIsDriverInControl)
def_ImpTypeGet(isAFLampOn)
def_ImpTypeGet(isFlashOnCapture)
def_ImpTypeGet(isFlashOnCalibration)
def_ImpTypeGet(isNeedFiringFlash)
def_ImpTypeGet(getFlashOnFrmCnt)
def_ImpTypeGet(getExp)
...
{
    if(!FlashMgrM::getInstance(senorDev)->getIsDriverInControl()){
        rep->isEnd=1;
        return 1;
    }
