...
Hal3ARawImp(MINT32 const i4SensorDevId)
    ...
    , m_bPreStop(MFALSE)
    , m_bIsAfTrigInLowLight(MFALSE)
    , m_i4KeepPrecapParamsCount(0)
...
config(const ConfigInfo_T& rConfigInfo)
{
    ...
    m_bDoPreCapInFlashAF = cust_isDoPreCapInFlashAF(m_i4SensorDev);
    ...
    CAM_LOGD("[%s] setAAOMode(%d), m_bDbgInfoEnable(%d)", __FUNCTION__, rConfigInfo.i4BitMode, m_bDbgInfoEnable);
...
generateP2(MINT32 flowType, const NSIspTuning::ISP_INFO_T& rIspInfo, void* pTuningBuf, ResultP2_T* pResultP2)
{
    ...
    if (pResultP2)
    {
        ...
        else
        {
            ...
            MBOOL bDump;
            GET_PROP_P("vendor.debug.tuning.dump_capture", 0, bDump);
    ...
    CAM_LOGD("[%s] LSC2_EN(%d) , DEPI_EN(%d)",
             __FUNCTION__,
             ((dip_x_reg_t*)pRegBuf)-> DIP_X_CTL_RGB_EN.Bits.LSC2_EN,
             ((dip_x_reg_t*)pRegBuf)-> DIP_X_CTL_DMA_EN.Bits.DEPI_EN);
...
#define F(_f, _arg) { \
    if((rNewParam._arg != m_rParam._arg) || m_bIsFirstSetParams) { \
        CAM_LOGD("[%s] update_"#_arg, __FUNCTION__); \
...
#define FZoom(_f) { \
        if(rNewParam.rScaleCropRect.i4Xwidth != 0 && rNewParam.rScaleCropRect.i4Yheight != 0) { \
            if(((rNewParam.rScaleCropRect.i4Xoffset != m_rParam.rScaleCropRect.i4Xoffset) \
                ...
                CAM_LOGD("[%s] update_zoom", __FUNCTION__); \
...
/*    IspTuningMgr::getInstance().setIspUserIdx_Bright(m_i4SensorDev, rNewParam.i4BrightnessMode);
    ...
    {
        IAeMgr::getInstance().setAEMinMaxFrameRate(m_i4SensorDev, m_rParam.i4MinFps, m_rParam.i4MaxFps);
        IAwbMgr::getInstance().setMaxFPS(m_i4SensorDev, m_rParam.i4MaxFps);
    }
    ...
    {
        CAM_LOGD("[%s] update AE MeteringAreas", __FUNCTION__);
    ...
    if((rNewParam.u4CamMode != m_rParam.u4CamMode) || m_bIsFirstSetParams)
    {
        ...
        {
            CAM_LOGD("[%s] update AE CamMode", __FUNCTION__);
    ...
    if (bUpdateScenario)
    {
        if(((m_sParam.CaptureIntent != m_sPreParam.CaptureIntent)
            ...
            {
                CAM_LOGD("[%s] update CamScenarioMode", __FUNCTION__);
    ...
    {
        CAM_LOGD("[%s] update OBC Table", __FUNCTION__);
        MVOID* pObc = NULL;
        MUINT16 u2Obcidx = 0;
    ...
    if (rNewParam.u4AeMode == MTK_CONTROL_AE_MODE_OFF)
    {
        ...
        {
            CAM_LOGD("[%s] update AE SensorParams", __FUNCTION__);
    ...
    {
        CAM_LOGD("[%s] update AE ScenarioMode", __FUNCTION__);
        ...
        rNewParam.u1ColorCorrectMode == MTK_COLOR_CORRECTION_MODE_TRANSFORM_MATRIX)
    {
        ...
        if(bNoEqual)
        {
            CAM_LOGD("[%s] update AWB/ISP ColorCorrection", __FUNCTION__);
...
#if CAM3_FLASH_FEATURE_EN
    // ====================================== Flash ==============================================
    F2(FlashMgr::getInstance().setAeFlashMode, u4AeMode, u4StrobeMode);
    F(FlashMgr::getInstance().setCamMode, u4CamMode);
    F2(FlashMgr::getInstance().setEvComp, i4ExpIndex, fExpCompStep);
    F(FlashMgr::getInstance().setDigZoom, i4ZoomRatio);
#endif
    ...
    if(m_bDoPreCapInFlashAF)
    {
        if(m_i4KeepPrecapParamsCount > 0 && !m_bIsAfTrigInLowLight)
           CAM_LOGD("[%s] Skip precapture flag", __FUNCTION__);
    }

#if CAM3_FLICKER_FEATURE_EN
    // ====================================== Flicker ==============================================
    F(FlickerHalBase::getInstance().setFlickerMode, u4AntiBandingMode);
#endif
...
setAfParams(AF_Param_T const &rNewParam)
{
        ...
        if (m_bDoPreCapInFlashAF &&
                ((rNewParam.u1AfTrig == MTK_CONTROL_AF_TRIGGER_START)
                && (m_i4KeepPrecapParamsCount > 0 && !m_bIsAfTrigInLowLight)))
        {
            CAM_LOGD("[%s] skip AF start.", __FUNCTION__);
            rAFPAram.u1AfTrig  = 0;
        }
        else
...
autoFocus()
{
    if(m_bDoPreCapInFlashAF)
    {
        MBOOL isAEAFTrigger = ((m_rParam.u1PrecapTrig == MTK_CONTROL_AE_PRECAPTURE_TRIGGER_START) && (m_rAfParam.u1AfTrig == MTK_CONTROL_AF_TRIGGER_START));
        if(m_i4KeepPrecapParamsCount > 0 && !m_bIsAfTrigInLowLight && isAEAFTrigger)
        {
            CAM_LOGD("[%s] skip AF start.", __FUNCTION__);
            return MTRUE;
        }
    ...
    {
        MBOOL bPreCapForLowLight = MFALSE;
        if(m_bDoPreCapInFlashAF)
        {
            MINT32 i4FlashMode = FlashMgr::getInstance().getFlashMode(m_i4SensorDev);
            MBOOL isBvTrigger = IAeMgr::getInstance().IsStrobeBVTrigger(m_i4SensorDev);
            MBOOL isNeedFlashOn = ((i4FlashMode == LIB3A_FLASH_MODE_FORCE_ON) || (i4FlashMode == LIB3A_FLASH_MODE_AUTO && isBvTrigger));
            CAM_LOGD("[%s]IsStrobeBVTrigger(%d), m_bIsAfTrigInLowLight(%d), flashMode(%d), isNeedFlashOn(%d)",__FUNCTION__,
                isBvTrigger, m_bIsAfTrigInLowLight, i4FlashMode, isNeedFlashOn);

            bPreCapForLowLight = (isNeedFlashOn && !m_bIsAfTrigInLowLight && !m_bIsAFTrigInPrecapState);
            if(bPreCapForLowLight)
            {
                // replaced to precapture flow when low light
                MY_LOG("[%s] precaptue flow begin for low light.", __FUNCTION__);
                m_pStateMgr->sendCmd(ECmd_PrecaptureStart);
                m_bIsAfTrigInLowLight = MTRUE;
                m_bIsAFTrigInPrecapState = MTRUE;
...
{
#if CAM3_AF_FEATURE_EN
    CAM_LOGD("[%s() m_bDoPreCapInFlashAF(%d), m_bIsAfTrigInLowLight(%d)] +", __FUNCTION__, m_bDoPreCapInFlashAF, m_bIsAfTrigInLowLight);
    if (m_bDoPreCapInFlashAF && m_bIsAfTrigInLowLight)
    {
        if(m_pStateMgr->getStateStatus().eCurrState == eState_Precapture)
        {
            MY_LOG("[%s] precaptue flow end for low light.", __FUNCTION__);
...
setPreFlashOnOff(MBOOL bOnOff)
{
    ...
    // update flash condition
    if(m_FlashCond.bPreFlashOnOff != bOnOff)
    {
        CAM_LOGW("[%s] bPreFlashOnOff(%d->%d)", __FUNCTION__, m_FlashCond.bPreFlashOnOff, bOnOff);
        m_FlashCond.bPreFlashOnOff = bOnOff;
    }
...
setFlashLightOnOff(MBOOL bOnOff, MBOOL bMainPre)
{
    ...
    // update flash condition
    if((m_FlashCond.bFlashOnOff != bOnOff) || (m_FlashCond.bFlashMainPre != bMainPre))
    {
        CAM_LOGW("[%s] bFlashOnOff(%d->%d), bFlashMainPre(%d->%d)", __FUNCTION__, m_FlashCond.bFlashOnOff, bOnOff, m_FlashCond.bFlashMainPre, bMainPre);
        m_FlashCond.bFlashOnOff   = bOnOff;
        m_FlashCond.bFlashMainPre = bMainPre;
    }
    if (!bOnOff)
    {
    ...
    else //flash on
    {
        if (bMainPre)
        {
            ...
            if (FlashMgr::getInstance().isAFLampOn(m_i4SensorDev))
            {
                CAM_LOGW("[%s] get RestoreCount(%d)", __FUNCTION__, m_pStateMgr->getRestoreCount());
                if(m_pStateMgr->getRestoreCount() > 0)
                {
                    ...
                    CAM_LOGW("[%s] set MaxFrameRate(%d) before Main flash on, MaxFps(%d)", __FUNCTION__, maxFrameRate, m_rParam.i4MaxFps);
...
chkMainFlashOnCond()
{
    ...
    // update flash condition
    if((m_FlashCond.u4AeMode != m_rParam.u4AeMode)|| (m_FlashCond.u4StrobeMode != m_rParam.u4StrobeMode) || (m_FlashCond.bIsFlashOnCapture != isFlashOnCapture())
        || (m_FlashCond.u1CaptureIntent != m_rParam.u1CaptureIntent) || (m_FlashCond.bChkMainFlash != bChkFlash) || (m_FlashCond.i4HWSuppportFlash != m_i4HWSuppportFlash)
        || (m_FlashCond.i4FlashState != FlashMgr::getInstance().getFlashState(m_i4SensorDev)))
    {
        CAM_LOGW("[%s] u4AeMode(%d->%d), u4StrobeMode(%d->%d), isFlashOnCapture(%d->%d), u1CaptureIntent(%d->%d), bChkFlash(%d->%d), m_i4HWSuppportFlash(%d->%d), i4FlashState(%d->%d)", __FUNCTION__
            , m_FlashCond.u4AeMode, m_rParam.u4AeMode, m_FlashCond.u4StrobeMode, m_rParam.u4StrobeMode, m_FlashCond.bIsFlashOnCapture, isFlashOnCapture()
            , m_FlashCond.u1CaptureIntent, m_rParam.u1CaptureIntent, m_FlashCond.bChkMainFlash, bChkFlash, m_FlashCond.i4HWSuppportFlash, m_i4HWSuppportFlash
            , m_FlashCond.i4FlashState, FlashMgr::getInstance().getFlashState(m_i4SensorDev));
        m_FlashCond.u4AeMode            = m_rParam.u4AeMode;
        m_FlashCond.u4StrobeMode        = m_rParam.u4StrobeMode;
        m_FlashCond.bIsFlashOnCapture   = isFlashOnCapture();
        m_FlashCond.u1CaptureIntent     = m_rParam.u1CaptureIntent;
        m_FlashCond.bChkMainFlash       = bChkFlash;
        m_FlashCond.i4HWSuppportFlash   = m_i4HWSuppportFlash;
        m_FlashCond.i4FlashState        = FlashMgr::getInstance().getFlashState(m_i4SensorDev);
    }
    //
...
chkPreFlashOnCond()
{
    ...
    // update flash condition
    if((m_FlashCond.u4AeMode != m_rParam.u4AeMode)|| (m_FlashCond.u4StrobeMode != m_rParam.u4StrobeMode) || (m_FlashCond.bChkPreFlash != bChkFlash)
        || (m_FlashCond.i4HWSuppportFlash != m_i4HWSuppportFlash) || (m_FlashCond.i4FlashState != FlashMgr::getInstance().getFlashState(m_i4SensorDev)))
    {
        CAM_LOGW("[%s] u4AeMode(%d->%d), u4StrobeMode(%d->%d), bChkFlash(%d->%d), m_i4HWSuppportFlash(%d->%d), i4FlashState(%d->%d)", __FUNCTION__
            , m_FlashCond.u4AeMode, m_rParam.u4AeMode, m_FlashCond.u4StrobeMode, m_rParam.u4StrobeMode, m_FlashCond.bChkPreFlash, bChkFlash
            , m_FlashCond.i4HWSuppportFlash, m_i4HWSuppportFlash, m_FlashCond.i4FlashState, FlashMgr::getInstance().getFlashState(m_i4SensorDev));
        m_FlashCond.u4AeMode            = m_rParam.u4AeMode;
        m_FlashCond.u4StrobeMode        = m_rParam.u4StrobeMode;
        m_FlashCond.bChkPreFlash        = bChkFlash;
        m_FlashCond.i4HWSuppportFlash   = m_i4HWSuppportFlash;
        m_FlashCond.i4FlashState        = FlashMgr::getInstance().getFlashState(m_i4SensorDev);
    }
...
getCurrResult(MUINT32 i4FrmId, Result_T& rResult, MINT32 /*i4SubsampleIndex*/) const
{
    ...
    rResult.vecDbgIspInfo.clear();
    rResult.vecDbgShadTbl.clear();
    rResult.vecDbgN3DInfo.clear();
    ...
    rResult.fgShadingMapOn = pLsc->getOnOff();
    #if 0
    // Shading map
    ...
    {
        rResult.i4MasterIdx = m_rLastStereoParam.i4MasterIdx;
        rResult.i4SlaveIdx = m_rLastStereoParam.i4SlaveIdx;
        CAM_LOGD("[%s] rResult (MasterIdx, SlaveIdx)(%d,%d)", __FUNCTION__, rResult.i4MasterIdx, rResult.i4SlaveIdx);
    }
#endif
/*
    // Tonemap
    ...
    switch(r3ACmd)
    {
        ...
        case ECmd_CameraPreviewEnd:
            {// back next state when previewEnd or uninit command on AF state.
                ...
                // check if flash need to turn off
                MBOOL isNeedTurnOffFlash = (FlashMgr::getInstance().isAFLampOn(m_i4SensorDev) && !m_pStateMgr->getIsFlashOpened());
                if(isNeedTurnOffFlash)
                {
                    CAM_LOGD("[%s] turn off flash", __FUNCTION__);
                    FlashMgr::getInstance().setAFLampOnOff(m_i4SensorDev, MFALSE);
                }
            }
            break;
        case ECmd_CameraPreviewStart:
            if(m_bDoPreCapInFlashAF) m_i4KeepPrecapParamsCount = 0;
            break;
        case ECmd_Update:
            {
#if CAM3_STEREO_FEATURE_EN
                ...
                if (m_bDoPreCapInFlashAF)
                {
                    if(m_pStateMgr->getStateStatus().eCurrState == eState_CameraPreview)
                    {
                        if(m_bIsAfTrigInLowLight)
                        {
                            MY_LOG("[%s] precaptue flow end for low light.", __FUNCTION__);
                            m_bIsAfTrigInLowLight = MFALSE;
                            GET_PROP("vendor.debug.precapture.count", 10, m_i4KeepPrecapParamsCount);
                        }
                        /*if(m_i4KeepPrecapParamsCount > 0)
                        {
                            MY_LOG("[%s] m_i4KeepPrecapParamsCount(%d)", __FUNCTION__, m_i4KeepPrecapParamsCount);
                            m_i4KeepPrecapParamsCount--;
                        }*/
                    }
                }
            }
...
*/
            ...
            if(!fgMainFlash && !mbIsHDRShot && (i4OperMode != EOperMode_Meta) &&
                (!bAELock || IAeMgr::getInstance().getEVCompensateIndex(m_i4SensorDev) != 0))
#endif
            {
    ...
    else if(m_bDoPreCapInFlashAF && ((r3ACmd == ECmd_PrecaptureStart) && m_i4KeepPrecapParamsCount > 0 && !m_bIsAfTrigInLowLight))
    {
        MY_LOG("[%s] skip precapture start.", __FUNCTION__);
    }
    else
    {
        m_pStateMgr->sendCmd(r3ACmd);
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    switch (e3ACtrl)
    {
        ...
        case E3ACtrl_SetOperMode:
            ...
            cust_setFlashPartCamMode(static_cast<int>(iArg1));
        ...
        case E3ACtrl_GetSupportedInfo:
            {
                ...
                sp<IMetadataProvider> pMetadataProvider = NSMetadataProviderManager::valueFor(m_i4SensorIdx);
                if (pMetadataProvider != NULL)
                {
                    IMetadata metaStaticInfo = pMetadataProvider->getMtkStaticCharacteristics();
                    if(!QUERY_ENTRY_SINGLE(metaStaticInfo, MTK_LENS_INFO_AVAILABLE_FOCAL_LENGTHS , fFocusLength))
                            CAM_LOGD("[%s] FocusLength QUERY fail", __FUNCTION__);
                }else
                {
                    CAM_LOGD("[%s] pMetadataProvider is null", __FUNCTION__);
                }
                ...
                rFeatureParam->u4FocusLength_100x = (MUINT32)(roundf(fFocusLength*100));
