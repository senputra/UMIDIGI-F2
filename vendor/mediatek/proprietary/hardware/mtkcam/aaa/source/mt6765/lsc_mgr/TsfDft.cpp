...
dump(const char* filename) const
{
    ...
    {
    #if STAT16BIT
        fwrite(vecStat.pData, vecStat.pDataSize, 2, fptr);
    #else
        fwrite(vecStat.pData, vecStat.pDataSize, 1, fptr);
    #endif
...
put(const ILscMgr::TSF_INPUT_INFO_T& rInStat, MUINT8 /*fgOpt*/)
{
    ...
    AAA_TRACE_D("TSF_AAO_CP");
    if(rInStat.prAwbStat)
    {
        vecStat.pDataSize=rInStat.u4SizeAwbStat/sizeof(MUINT16);
        vecStat.pData=(MUINT16 *)rInStat.prAwbStat;
    }
    AAA_TRACE_END_D;
    return fgRet;
}
...
init(ESensorMode_T eFullSensorMode)
{
    ...
    GET_PROP("debug.lsc_mgr.cfg", "1", m_u1CfgOpt);
...
#if STAT16BIT
    rProcInfo.ImgAddr = (MUINT16*)rInputStat.vecStat.pData;
#else
    rProcInfo.ImgAddr = (MUINT8*)rInputStat.vecStat.pData;
#endif
...
tsfRun(const TSF_INPUT_STAT_T& rInputStat, ILscBuf* rInputBuf, ILscBuf** rBufPrior, ILscBuf** rOutputBuf)
{
    ...
    else if (eState == E_TSF_READY)
    {
        ...
        if (tsfGetResult(*rOutputBuf))
        {
            ...
            //set dirty for updating gainTable
            m_GainMutex.lock();
            m_bGainTableIsDirty = MTRUE;
            m_GainMutex.unlock();
...
doThreadFunc()
{
    ...
    while (m_fgThreadLoop)
    {
        ...
        if (E_LSC_TSF_TYPE_0 == getType())
        {
            ...
            NSIspTuningv3::ISP_MGR_LSC_T::getInstance(static_cast<MUINT32>(m_u4SensorDev)).putRatio(rStat.u4Ratio);
        }
