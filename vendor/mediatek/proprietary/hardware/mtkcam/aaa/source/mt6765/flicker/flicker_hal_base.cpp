...
static MBOOL g_uniInfo = false;
static int g_uniSensor = ESensorDev_Main;
...
int FlickerHalBase::update(int sensorDev, FlickerInput* in, FlickerOutput* out)
{
    ...
    //def_ImpBody2(update, sensorDev, in, out);
    int ret;
	out->flickerResult = gFlickerResult;
...
MBOOL FlickerHalBase::Stop(int sensorDev)
{
    ...
    int errSub2=0;
    if (sensorDev & ESensorDev_Main)
        errMain = FlickerHal::getInstance(ESensorDev_Main)->Stop();
    if (sensorDev & ESensorDev_Sub)
        errSub = FlickerHal::getInstance(ESensorDev_Sub)->Stop();
    if (sensorDev & ESensorDev_MainSecond)
        errMain2 = FlickerHal::getInstance(ESensorDev_MainSecond)->Stop();
    if (sensorDev & ESensorDev_SubSecond)
        errSub2 = FlickerHal::getInstance(ESensorDev_SubSecond)->Stop();

	setUniInfo(sensorDev, false);
    return errOp(errMain, errSub, errMain2, errSub2);
}
...
int FlickerHalBase::setUniInfo(int sensorDev, MBOOL const i4UniInfo)
{
	...
	if (!i4UniInfo && sensorDev == g_uniSensor) {
		g_uniInfo = i4UniInfo;
		def_ImpBody1(setUniInfo, sensorDev, i4UniInfo);
	} else if (i4UniInfo && !g_uniInfo) {
		g_uniInfo = i4UniInfo;
        g_uniSensor = sensorDev;
		def_ImpBody1(setUniInfo, sensorDev, i4UniInfo);
    }
