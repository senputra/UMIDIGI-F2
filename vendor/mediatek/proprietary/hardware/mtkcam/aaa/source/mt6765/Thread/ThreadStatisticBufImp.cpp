...
destroyInstance()
{
    DebugUtil::getInstance(m_i4SensorDev)->update(m_pName, "destroyInstance", 0);
    waitFinished();
    Mutex::Autolock autoLock(m_ModuleMtx);
    CAM_LOGD_IF(m_fgLogEn,"[%s] +\n", __FUNCTION__);

    CAM_LOGD_IF(m_fgLogEn,"[%s]sem_wait : m_SemFinish \n", __FUNCTION__);
    DebugUtil::getInstance(m_i4SensorDev)->update(m_pName, "destroyInstance", 1);
    ::sem_wait(&m_SemFinish);
    CAM_LOGD_IF(m_fgLogEn,"[%s]pthread_join : m_rThread \n", __FUNCTION__);
    DebugUtil::getInstance(m_i4SensorDev)->update(m_pName, "destroyInstance", 2);
...
changeThreadSetting()
{
    ...
    int priority  = NICE_CAMERA_STT;
    if(strncmp(m_pName, "AFO", 3)==0)
    {
        priority  = NICE_CAMERA_STT_AF;
    }
    //  set
    ...
    CAM_LOGD(
        "[%s] name(%s), tid(%d), policy:(expect, result)=(%d, %d), priority:(expect, result)=(0x%08x, 0x%08x)", __FUNCTION__
        , m_pName, ::gettid()
