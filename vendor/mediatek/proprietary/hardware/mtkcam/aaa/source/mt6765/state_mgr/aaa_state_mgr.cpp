...
#include <flash_tuning_custom.h>
 ...
 StateMgr::StateMgr()
   ...
   , mSemAF()
   , m_bIsDoRestore(MFALSE)
   , m_u8TimeStampRecord(0)
   , m_u8TakeTimeAE(0)
   , m_u8TakeTimeFlash(0)
   , m_u8TakeTimeDoAFAE(0)
 {
   mpCurrentState = NULL;
 }
...
MVOID StateMgr::postToAFTrigger()
{
 ...
 MRESULT StateMgr::transitState(EState_T const eCurrState, EState_T const eNewState)
 {
   ...
   if(cust_isDoPreCapInFlashAF(mSensorDevId))
   {
     // reset restore count
   ...
   CAM_LOGW("[StateCommon::transitState] %s --> %s", mpIState[mStateStatus.ePrevState]->getName(), mpIState[mStateStatus.eCurrState]->getName());
   return S_3A_OK;
 }
...
MRESULT StateMgr::sendCmd(ECmd_T eCmd)
{
   ...
   MINT32 newIntent = static_cast<EIntent_T>(eCmd);       // Seperate to fix Coverity
   EIntent_T eNewIntent = static_cast<EIntent_T>(newIntent);
...
MVOID StateMgr::clearCapQueue()
{
    CAM_LOGD("[%s] Cap Queue not empty, might not do capture end before, size(%d)", __FUNCTION__, m_capQueue.size());
    m_capQueue.clear();
