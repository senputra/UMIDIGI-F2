...
private:
    ...
    enum
    {
        ERegInfo_DIP_X_PCA_CON1,
        ERegInfo_DIP_X_PCA_CON2,
...
public:
    ISP_MGR_PCA(MUINT32 const eSensorDev, EPCAMode_T const ePCAMode)
        : ISP_MGR_BASE_T(m_rIspRegInfo, ERegInfo_PCA_CON_NUM, m_u4StartAddr, eSensorDev)
        , m_ePCAMode(ePCAMode)
        , m_bEnable(MTRUE)
        , m_bCCTEnable(MTRUE)
        , m_u4StartAddr(REG_ADDR_P2(DIP_X_PCA_CON1))
    {
        // PCA_CON
        INIT_REG_INFO_ADDR_P2(DIP_X_PCA_CON1); // DIP_X+0x1E00
        INIT_REG_INFO_ADDR_P2(DIP_X_PCA_CON2); // DIP_X+0x1E04

        // PCA_LUT
        MUINT32 u4StartAddr = REG_ADDR_P2(DIP_X_PCA_TBL[0]);
        MINT32 i4PCA_BIN_NUM = (m_ePCAMode == EPCAMode_180BIN) ? PCA_BIN_NUM : PCA_BIN_NUM*2;
        //CAM_LOGD("i4PCA_BIN_NUM = %d", i4PCA_BIN_NUM);

        for (MINT32 i = 0; i < i4PCA_BIN_NUM; i++) {
            m_rIspRegInfo_PCA_LUT[i].addr = u4StartAddr + 4*i;
            //CAM_LOGD("m_rIspRegInfo_PCA_LUT[%d].addr = 0x%8x", i, m_rIspRegInfo_PCA_LUT[i].addr);
        }
    }
...
public:
    static MyType&  getInstance(MUINT32 const eSensorDev, EPCAMode_T const ePCAMode = EPCAMode_180BIN);
