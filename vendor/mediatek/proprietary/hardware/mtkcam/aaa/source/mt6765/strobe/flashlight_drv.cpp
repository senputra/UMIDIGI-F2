...
StrobeDrv *FlashlightDrv::getInstance(int typeId, int ctId)
{
    logI("getInstance %d %d", typeId, ctId);

    logI("getInstance line=%d",__LINE__);
    FlashlightDrv *p = NULL;
...
int FlashlightDrv::isChargerReady(int *chargerStatus)
{
    logI("isChargerReady()");
    ...
    int err;
    err = pKDrv->sendCommandRet(
            FLASH_IOC_IS_CHARGER_READY, mTypeId, mCtId, chargerStatus);
    logI("isChargerReady(%d)", *chargerStatus);
...
int FlashlightDrv::isLowPower(int* bLow)
{
    ...
    pKDrv = StrobeGlobalDriver::getInstance();
    int err;
    err = pKDrv->sendCommandRet(FLASH_IOC_IS_LOW_POWER, mTypeId, mCtId, bLow);
    if (*bLow != 0)
...
int FlashlightDrv::lowPowerDetectStart(int lowPowerDuty)
{
    ...
    pKDrv = StrobeGlobalDriver::getInstance();
    int err;
    err = pKDrv->sendCommand(FLASH_IOC_LOW_POWER_DETECT_START, mTypeId, mCtId, lowPowerDuty);
...
int FlashlightDrv::lowPowerDetectEnd()
{
    ...
    pKDrv = StrobeGlobalDriver::getInstance();
    int err;
    err = pKDrv->sendCommand(FLASH_IOC_LOW_POWER_DETECT_END, mTypeId, mCtId, 0);
...
int FlashlightDrv::getPartId(int* partId)
{
    ...
    pKDrv->openkd();
    int err;
    err = pKDrv->sendCommandRet(FLASH_IOC_GET_PART_ID, mTypeId, mCtId, partId);
    pKDrv->closekd();
...
int FlashlightDrv::getVBat(int* vbat)
{
    ...
    if (fp != 0) {
        ...
        ret = fscanf(fp,"%d",&v);
        if (ret == 1) { //read 1
            *vbat = v/1000; /* convert from uV to mV */
        } else {
...
int FlashlightDrv::setStep(int step)
{
    ...
    int err;
    err = pKDrv->sendCommand(FLASH_IOC_SET_STEP, mTypeId, mCtId, step);
    m_step = step;
...
int FlashlightDrv::setDuty(int duty)
{
    ...
    int err;
    err = pKDrv->sendCommand(FLASH_IOC_SET_DUTY, mTypeId, mCtId, duty);
    m_duty = duty;
...
int FlashlightDrv::setCharger(int ready)
{
    logI("setCharger() ready = %d\n", ready);
    ...
    if ((ready && mSetCharger == 1) ||
        ...
        StrobeGlobalDriver* pKDrv;
        pKDrv = StrobeGlobalDriver::getInstance();
        ret = pKDrv->sendCommand(FLASH_IOC_SET_SCENARIO, mTypeId, mCtId, scenario);
    }
...
int FlashlightDrv::setTimeOutTime(int ms)
{
    ...
    int err;
    err = pKDrv->sendCommand(FLASH_IOC_SET_TIME_OUT_TIME_MS, mTypeId, mCtId, ms);
    mTimeOutTime = ms;
...
int FlashlightDrv::setPreOn()
{
    ...
    int err;
    err = pKDrv->sendCommand(FLASH_IOC_PRE_ON, mTypeId, mCtId, 0);
    m_preOnTime=getMs();
...
int FlashlightDrv::setReg(int reg, int val)
{
    ...
    int err2;
    int err3;
    err1 = pKDrv->sendCommand(FLASH_IOC_SET_REG_ADR, mTypeId, mCtId, reg);
    err2 = pKDrv->sendCommand(FLASH_IOC_SET_REG_VAL, mTypeId, mCtId, val);
    err3 = pKDrv->sendCommand(FLASH_IOC_SET_REG, mTypeId, mCtId, 0);
...
int FlashlightDrv::getReg(int reg, int* val)
{
    ...
    StrobeGlobalDriver* pKDrv;
    pKDrv = StrobeGlobalDriver::getInstance();
    ret = pKDrv->sendCommand(FLASH_IOC_GET_REG, mTypeId, mCtId, reg);
    *val = ret;
...
int FlashlightDrv::init_real()
{
    ...
    int err;
    err = pKDrv->init(mTypeId, mCtId);
    /* TODO: need to handle return value here */
    m_isOn = 0;
    m_duty = -1;
    m_step = 0;
    m_preOnTime = -1;
...
int FlashlightDrv::uninit()
{
    ...
    if (!mUsers) {
        ...
        pKDrv = StrobeGlobalDriver::getInstance();
        int err;
        err = pKDrv->uninit(mTypeId, mCtId);
    } else {
...
int FlashlightDrv::hasLowPowerDetect(int *hasLPD)
{
    ...
    err = pKDrv->sendCommandRet(FLASH_IOC_HAS_LOW_POWER_DETECT, mTypeId, mCtId, hasLPD);
...
/*
   ...
   pKDrv = StrobeGlobalDriver::getInstance();
   int err;
   err = pKDrv->sendCommand(FLASH_IOC_LOW_POWER_DETECT_START, mTypeId, mCtId, 0);
   return err;
   }
 ...
 *isLowPower=0;
 ...
 pKDrv = StrobeGlobalDriver::getInstance();
 int err;
 err = pKDrv->sendCommand(FLASH_IOC_LOW_POWER_DETECT_END, mTypeId, mCtId, (long)isLowPower);
 logI("isLowPower=%d", *isLowPower);
 return err;

 }*/

int FlashlightDrv::setStrobeInfo(
