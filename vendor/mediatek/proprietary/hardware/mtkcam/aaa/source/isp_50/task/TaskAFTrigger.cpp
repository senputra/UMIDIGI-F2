    ...
    if(getStatus() == TASK_STATUS_INIT)
    {
        ...
        else
        {
            if(m_bIsAfTriggerInPrecap)
            {
                ...
                MBOOL bIsStrobeBVTrigger = IAeMgr::getInstance().IsStrobeBVTrigger(m_i4SensorDev);
                CAM_LOGD("[%s] AF trigger when precapture, isAFLampOn(%d), bIsStrobeBVTrigger(%d)", __FUNCTION__, isFlashOn, bIsStrobeBVTrigger);
                // preCapture with (FlashOn && DarkEnv) ==> need AF
                if(isFlashOn == MTRUE && bIsStrobeBVTrigger)
                {
