...
put(const ILscMgr::TSF_INPUT_INFO_T& rInStat, MUINT32 StatFullSize, MUINT8 /*fgOpt*/)
{
    ...
    {
        vecStat.resize(StatFullSize/sizeof(MUINT16));
        ::memcpy(vecStat.data(), rInStat.prAwbStat, StatFullSize);
    }
    return fgRet;
}
...
tsfInit()
{
    ...
    m_rTsfEnvInfo.StatStrideSize = m_srideSize;
    m_rTsfEnvInfo.StatFullSize = m_srideSize*m_blockH;
...
update(const ILscMgr::TSF_INPUT_INFO_T& rInputInfo)
{
    ...
    if (m_bTSFInit && m_bTSF)
    {
        ...
        rStat.put(rInputInfo, m_srideSize*m_blockH, m_fgStatOpt);
