...
void copyTuningPara(FLASH_TUNING_PARA* p, NVRAM_FLASH_TUNING_PARA* nv_p)
{
    ...
    p->yDecreEvWeight = nv_p->yDecreEVTarget;
...
int FlashCustomAdapter::init()
{
    ...
    for(int i=0; i<FLASH_CUSTOM_MAX_DUTY_NUM; i++){
        mFlashWBGain[i].i2R =
                mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[i].i4R;
        mFlashWBGain[i].i2B =
                mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[i].i4B;
    }
...
int FlashCustomAdapter::setWBGainByDutyIndex(int dutyIndex, double r, double g, double b)
{
    ...
    mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[dutyIndex].i4R = r;
    mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[dutyIndex].i4G = g;
    mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[dutyIndex].i4B = b;
    return 0;
}

int FlashCustomAdapter::getWBRGainByDutyIndex(int dutyIndex)
{
    return mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[dutyIndex].i4R;
}

int FlashCustomAdapter::getWBGGainByDutyIndex(int dutyIndex)
{
    return mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[dutyIndex].i4G;
}

int FlashCustomAdapter::getWBBGainByDutyIndex(int dutyIndex)
{
    return mpFlashCaliNvbuf->Flash_Calibration[mFlashCaliNVRAMIdx].flashWBGain[dutyIndex].i4B;
}
