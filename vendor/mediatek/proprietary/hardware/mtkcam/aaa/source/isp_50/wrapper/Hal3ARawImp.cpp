...
init(MINT32 i4SensorOpenIdx)
{
    ...
    if(!gTuningInitCtrl[m_i4SensorIdx].bHadModuleId)
    {
        ...
        err=NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_FEATURE, m_i4SensorDev, v, MTRUE);
        if(err != 0)
            MY_LOGE("[%s] CAMERA_NVRAM_DATA_FEATURE getBufAndRead ERROR \n", __FUNCTION__);
        err=NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_LENS, m_i4SensorDev, w, MTRUE);
        if(err != 0)
            MY_LOGE("[%s] CAMERA_NVRAM_DATA_LENS getBufAndRead ERROR \n", __FUNCTION__);
        //create instance after nvram data valid
        IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setValidFlag(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev));
        CAM_LOGD("[%s] create instance after nvram data valid \n", __FUNCTION__);
...
{
    MY_LOGD_IF(m_3ALogEnable, "[%s] sensor(%d) #(%d) profile(%d) rpg(%d) u1TonemapMode(%d)", __FUNCTION__, m_i4SensorDev, rParamIspProfile.i4MagicNum, rParamIspProfile.eIspProfile, rParamIspProfile.iEnableRPG, m_rParam.u1TonemapMode);
    ...
    IspTuningMgr::getInstance().setToneMapMode(m_i4SensorDev, m_rParam.u1TonemapMode);
    //LCS callback
    ISP_LCS_IN_INFO_T lcs_info;
    m_pTuning->getLCSparam(m_i4SensorDev, lcs_info);
...
/*    IspTuningMgr::getInstance().setIspUserIdx_Bright(m_i4SensorDev, rNewParam.i4BrightnessMode);
    ...
    //IspTuningMgr::getInstance().setEnableSuperNightMode(m_i4SensorDev, myflag);
...
{
    MY_LOGD_IF(m_3ALogEnable, "Dev(%d) i4FrameNum(%d) MasterIdx = %d, SlaveIdx = %d, m_bIsSkipSync3A(%d) rNewParam(%d,%d,%d)(%d,%d)", m_i4SensorDev, rNewParam.i4FrameNum, rNewParam.i4MasterIdx, rNewParam.i4SlaveIdx, m_bIsSkipSync3A,
