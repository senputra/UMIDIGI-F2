...
#define MY_INST NS3Av3::INST_T<ISP_MGR_SL2_T>
...
getInstance(ESensorDev_T const eSensorDev)
{
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_SL2_T>(eSensorDev);
    } );
...
transform_SL2(const CROP_RZ_INFO_T& rCropRz, ISP_NVRAM_SL2_T const& rParam)
{
    ...
    MINT32 maxR         = _maxR(SL2_CENTR_X, SL2_CENTR_Y, rCropRz.i4RRZinW, rCropRz.i4RRZinH);
    ...
    if (rCropRz.fgRRZOnOff)
    {
        if (rCropRz.i4RRZoutW == 0 || rCropRz.i4RRZoutH == 0)
        {
            CAM_LOGE("Unexpected crop info width(%d), rz width(%d)", rCropRz.i4RRZoutW, rCropRz.i4RRZoutH);
            AEE_ASSERT_SL2("Unexpected crop info for SL2");
        }
        else
        {
            MFLOAT fRzRtoX = (MFLOAT)rCropRz.i4RRZoutW / (MFLOAT)rCropRz.i4RRZcropW;
            MFLOAT fRzRtoY = (MFLOAT)rCropRz.i4RRZoutH / (MFLOAT)rCropRz.i4RRZcropH;
            SL2_CENTR_X = (SL2_CENTR_X - rCropRz.i4RRZofstX);
            SL2_CENTR_Y = (SL2_CENTR_Y - rCropRz.i4RRZofstY);
    ...
    CAM_LOGD_IF(m_bDebugEnable,"SL2, Center(%d, %d), RZ_COM(%d, %d), RRZ_OUT(%d, %d)",
              SL2_CENTR_X, SL2_CENTR_Y,
              SL2_HRZ_COM, SL2_VRZ_COM,
              rCropRz.i4RRZoutW ,rCropRz.i4RRZoutH );
    ...
    if(getSL2HOnOff()){
        ...
        REG_INFO_VALUE(DIP_X_SL2H_RZ)       = (SL2_VRZ_COM<<16) | SL2_HRZ_COM;
