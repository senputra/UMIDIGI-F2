...
#else
        sizeof(NR2_PARAM) == sizeof(NR2TuningInfo),
#endif
...
static const NVRAM_CAMERA_FEATURE_SWNR_STRUCT *getTuningFromNvram(MUINT32 openId, SWNR_SCENARIO scenario, MUINT32 idx, MINT32 magicNo, IMetadata* pInHalMeta)
{
    int err;
    ...
    if (scenario == SWNR_SCENARIO_MFNR)
    {
    ...
    pMapInfo = bIspHidl ? (CAM_IDX_QRY_COMB *)mapInfo.array() : &rMapping_Info;
    idx = pMgr->query(static_cast<ESensorDev_T>(sensorDev), NSIspTuning::EModule_SWNR, *pMapInfo, __FUNCTION__);
    SWNR_LOGD("query nvram(%d) index: %d ispprofile: %d sensor: %d ismfll(%d)", magicNo, idx, pMapInfo->eIspProfile, pMapInfo->eSensorMode, scenario == SWNR_SCENARIO_MFNR );
#endif
    ...
    case SWNR_SCENARIO_MFNR:
        ...
        break;
#ifdef MTKCAM_STEREO_DENOISE_SUPPORT
    case SWNR_SCENARIO_DUAL:
        pNRNvram = &(pNvram->nr_dual[idx]);
        break;
#endif
    default: // SWNR_SCENARIO_NORMAL
...
static MBOOL getSensorDevice(MUINT* pSensorDev, MUINT *pSensorDevId, MUINT32 openId)
{
    ...
    *pSensorDev = MAKE_HalSensorList()->querySensorDevIdx(openId);
    ...
    case SENSOR_DEV_SUB_2:
        pHalSensorList->querySensorStaticInfo(NSCam::SENSOR_DEV_SUB_2, &rSensorStaticInfo);
        break;

    case SENSOR_DEV_MAIN_3:
        pHalSensorList->querySensorStaticInfo(NSCam::SENSOR_DEV_MAIN_3, &rSensorStaticInfo);
        break;
    ...
    for (i=0; i<MyWorkingBufInfo.WorkingBufferNum; i++)
    {
        ...
        if(pWorkingBuffer->MemType == NR_MEMORY_ION) {
            ...
            if(pWorkingBuffer->Size > pImgBuf->getBufSizeInBytes(0))
                SWNR_LOGE("SWNRAlgo want %d bytes wxh(%dx%d), but real size is %zu", pWorkingBuffer->Size, pWorkingBuffer->Width, pWorkingBuffer->Height, pImgBuf->getBufSizeInBytes(0));
...
static MUINT32 getNeighber(Paramctrl* pParamctrl, MUINT32 iso, MUINT32& iso_low, MUINT32& iso_high)
{
    ...
    {
        MUINT32 iso2;
        iso2 = pParamctrl->map_ISO_index2value((EISO_T)idx);
        if (iso2 > iso) {
            if (idx > 0) {
                idx -= 1;
                iso_low = pParamctrl->map_ISO_index2value((EISO_T)idx);
                iso_high = iso2;
            } else {
            ...
            iso_low = iso2;
            iso_high = pParamctrl->map_ISO_index2value((EISO_T)(idx + 1));
        }
    }
    return idx;
}
...
#warning FIXME: temp mark(0) to close interpolation flow. Need open after SmoothFSWNR fix!
    ...
    {
        const NVRAM_CAMERA_FEATURE_SWNR_STRUCT *pNvramLow   = getTuningFromNvram(openId, scenario, idx, magicNo, pInHalMeta);
        const NVRAM_CAMERA_FEATURE_SWNR_STRUCT *pNvramHigh  = getTuningFromNvram(openId, scenario, idx + 1, magicNo, pInHalMeta);
        ...
        // get nvram
        const NVRAM_CAMERA_FEATURE_SWNR_STRUCT *pTuningNvram = getTuningFromNvram(openId, scenario, idx, magicNo, pInHalMeta);
        if (!pTuningNvram) {
            SWNR_LOGE("update from nvram failed");
            return MFALSE;
        }
...
MVOID SwNR::init(MSize requestSize)
{
    mDbgReuseWb = ::property_get_int32("vendor.debug.camera.capturenr.wb", 1);
    if(mDbgReuseWb) {
        if(requestSize.w != 0 && requestSize.h != 0) {
            CAM_ULOGM_TAG_BEGIN("swnr:allocWb");
            size_t bufBoundaryInBytes[3] = {0};
            IImageBufferAllocator::ImgParam imgParam =
                        IImageBufferAllocator::ImgParam(
                            NR2_BUFFER_SIZE(requestSize.w, requestSize.h),
                            (*bufBoundaryInBytes)
                        );
            IImageBufferAllocator* allocator = IImageBufferAllocator::getInstance();
            ...
            CAM_ULOGM_TAG_END();
            SWNR_LOGD("get request sensor size (%dx%d) to alloc Wb(%d)", requestSize.w, requestSize.h,
                    (int)NR2_BUFFER_SIZE(requestSize.w, requestSize.h));
        } else {
            SWNR_LOGE("no size info: MTK_HAL_REQUEST_SENSOR_SIZE");
        }
    }
...
{
    return doSwNR(param, pBuf, -1, nullptr);
}

MBOOL SwNR::doSwNR(SWNRParam const& param, NSCam::IImageBuffer* pBuf, MINT32 magicNo, IMetadata* pInHalMeta)
{
    MBOOL ret = MFALSE;
    SWNR_SCENARIO scenario;
    ...
    SWNR_ASSERT(getTuningInfo(&mTuningInfo, &mNRType, param.iso, scenario, muOpenId, magicNo, pInHalMeta) != 0);
    ...
    // 1. create instance
    MyNR = MTKNR::createInstance(mNRType); // DRV_NR_OBJ_SW2_LOW_POWER, DRV_NR_OBJ_SW2_LOW_POWER, DRV_NR_OBJ_SW2_HIGH_PERF
    ...
    //error handle of VpuStream NULL
    if(!(MyNR->NRMain(&MyNRImageInfo, NULL) == S_NR_OK))  {
        SWNR_LOGD("MyNR->NRMain(&MyNRImageInfo, NULL) != S_NR_OK");
        mNRType = DRV_NR_OBJ_SW2;
        MyNR = MTKNR::createInstance(mNRType); // DRV_NR_OBJ_SW2
        SWNR_ASSERT(MyNR != NULL);
        SWNR_ASSERT(MyNR->NRInit(&MyNRInitInfo, 0) == S_NR_OK);
        MyWorkingBufInfo.Width = pBuf->getImgSize().w;
        MyWorkingBufInfo.Height = pBuf->getImgSize().h;
...
MBOOL SwNR::getTuningFromFile(char * const filename, SwNR::TUNINGINFO& pTuningInfo)
{
    ...
    size_t result = fread(&p, sizeof(DEBUG_RESERVEA_INFO_T), 1, fp);
    fclose(fp);
