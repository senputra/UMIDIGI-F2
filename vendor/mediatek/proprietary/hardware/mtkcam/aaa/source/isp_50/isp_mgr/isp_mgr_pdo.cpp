...
#define __I__(param) rRegs[E_DUMP_##param].Addr = _A_P1(param)
        ...
        __I__(CAM_PDO_XSIZE);
        __I__(CAM_PDO_YSIZE);
        __I__(CAM_PDO_STRIDE);
        __I__(CAM_PBN_TYPE);
        __I__(CAM_PBN_LST);
        __I__(CAM_PDE_TBLI1);
        __I__(CAM_PDE_CTL);
        __I__(CAM_PDI_BASE_ADDR);
        __I__(CAM_PDI_XSIZE);
        __I__(CAM_PDI_YSIZE);
    ...
    typedef enum
    {
        E_DUMP_CAM_CTL_DMA_EN = 0,
        E_DUMP_CAM_CTL_EN2,
        E_DUMP_CAM_PDO_XSIZE,
        E_DUMP_CAM_PDO_YSIZE,
        E_DUMP_CAM_PDO_STRIDE,
        E_DUMP_CAM_PBN_TYPE,
        E_DUMP_CAM_PBN_LST,
        E_DUMP_CAM_PDE_TBLI1,
        E_DUMP_CAM_PDE_CTL,
        E_DUMP_CAM_PDI_BASE_ADDR,
        E_DUMP_CAM_PDI_XSIZE,
        E_DUMP_CAM_PDI_YSIZE,
...
config( SPDOHWINFO_T &sInPdohwCfg)
{
    ...
    else
    {
...
#else
        CAM_REG_PDE_CTL reg_pde_ctl;
        reg_pde_ctl.Raw = 0;
        reg_pde_ctl.Bits.PDE_OBIT = sInPdohwCfg.u4BitDepth;

        // pde setting
        REG_INFO_VALUE(CAM_PDE_TBLI1)     = 0;
        REG_INFO_VALUE(CAM_PDE_CTL)       = reg_pde_ctl.Raw;
        ...
        CAM_LOGD( "[%s] shieldPD CAM_PDE_TBLI1(0x%08x) CAM_PDE_CTL(0x%08x) CAM_PDI_BASE_ADDR(0x%08x) CAM_PDI_XSIZE(0x%08x) CAM_PDI_YSIZE(0x%08x)",
                  __FUNCTION__,
                  REG_INFO_VALUE(CAM_PDE_TBLI1),
                  REG_INFO_VALUE(CAM_PDE_CTL),
...
#define _DUMP(param) CAM_LOGD("%-40s:    0x%08X\n", #param, rDump.rRegs[DUMP_INFO::E_DUMP_##param].Data)
        ...
        CAM_LOGD( "PDO_EN(%d), PDI_EN(%d)", cam_ctl_dma_en.Bits.PDO_EN, cam_ctl_dma_en.Bits.PDI_EN);

        CAM_REG_CTL_EN2 cam_ctl_en2;
        cam_ctl_en2.Raw = rDump.rRegs[DUMP_INFO::E_DUMP_CAM_CTL_EN2].Data;
        CAM_LOGD( "PDE_EN(%d), SGG5_EN(%d)", cam_ctl_en2.Bits.PDE_EN, cam_ctl_en2.Bits.SGG5_EN);
