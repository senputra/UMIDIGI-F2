...
static MBOOL _dumpHwBuf(char* fileName, char* folderName, StatisticBufInfo& rLastBuf, MBOOL isHwBuf __unused = 0)
{
    ...
    if(getStatus() == TASK_STATUS_INIT)
    {
        ...
        if(!rData.bIsHqCap)
        {
            ...
            if(rData.bRemosaicEn)
                IAeMgr::getInstance().prepareCapParams(m_i4SensorDev,rData.bRemosaicEn,MFALSE);
        }
        ...
        //force unlock shading ratio
        MBOOL forceUnlock = 2;
        NSIspTuningv3::IspTuningMgr::getInstance().sendIspTuningIOCtrl(m_i4SensorDev, NSIspTuningv3::IspTuningMgr::E_ISPTUNING_UPDATE_LSC_INFO, forceUnlock, 0);
        m_rTimer.End();
        ...
        if(!rData.bIsHqCap)
        {
            ...
            if(pAAOBuffer)
            {
                ...
                IAwbMgr::getInstance().doPvAWB(m_i4SensorDev,
                                               ...
                                               AEPerframeInfo.rAEISPInfo.u4Exposuretime_us);
            }
            else
                CAM_LOGW("[%s] AAO buffer is NULL", __FUNCTION__);
#endif
            NSIspTuningv3::IspTuningMgr::getInstance().sendIspTuningIOCtrl(m_i4SensorDev, NSIspTuningv3::IspTuningMgr::E_ISPTUNING_UPDATE_LSC_INFO, MFALSE, 0);
    ...
    } else
    {
        ...
        if(m_i4RequestMagic <= i4StatisticMagic)
        {
            ...
            if(pAAOBuffer && pPSOBuffer)
                IAeMgr::getInstance().doCapFlare(m_i4SensorDev, reinterpret_cast<MVOID *>(pAAOBuffer->mVa), FlashMgr::getInstance(m_i4SensorDev)->isFlashOnCapture());
            else
