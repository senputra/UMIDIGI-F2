...
static std::array<MY_INST_DBS, SENSOR_IDX_MAX> gMultitonDBS;

#define MY_INST_DBS2 NS3Av3::INST_T<ISP_MGR_DBS2_T>
static std::array<MY_INST_DBS2, SENSOR_IDX_MAX> gMultitonDBS2;
...
getInstance(ESensorDev_T const eSensorDev)
{
    ...
    MY_INST_DBS& rSingleton = gMultitonDBS[i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_DBS_T>(eSensorDev);
    } );
...
apply(EIspProfile_T /*eIspProfile*/, TuningMgr& rTuning, MINT32 i4SubsampleIdex)
{
    ...
    reinterpret_cast<ISP_CAM_DBS_CTL_T*>(REG_INFO_VALUE_PTR(CAM_DBS_CTL))->DBS_LE_INV_CTL = 0;
...
getInstance(ESensorDev_T const eSensorDev)
{
    ...
    MY_INST_DBS2& rSingleton = gMultitonDBS2[i4SensorIdx];
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<ISP_MGR_DBS2_T>(eSensorDev);
    } );
