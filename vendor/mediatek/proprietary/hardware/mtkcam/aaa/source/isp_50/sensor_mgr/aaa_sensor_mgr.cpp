...
setSensorATR(MINT32 i4SensorDev, MUINT32 a_u4LimitGain, MUINT32 a_u4LtcRate)
{
    ...
    /*if(i4SensorDev>0) {
        ret = m_pHalSensorObj->sendCommand(i4SensorDev, SENSOR_CMD_SET_SENSOR_HDR_ATR, (MUINTPTR)&a_u4LimitGain, (MUINTPTR)&a_u4LtcRate, 0); // temp for test
    }else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }*/
...
setFlickerFrameRateActive(MINT32 i4SensorDev, MBOOL a_bFlickerFPSAvtive)
{
    ...
    if(i4SensorDev >0) {
       ...
       CAM_LOGD_IF(m_bDebugEnable,"sensorDev(%d/%d) a_bFlickerFPSAvtive(%d) m_bFlickerState(%d)\n", i4SensorDev,i4SensorIdx ,a_bFlickerFPSAvtive, m_bFlickerState[i4SensorIdx]);
