...
Task3ATouchImp::run(MINT32 i4Opt, const TaskData& rData)
{
    ...
    //===== AE calculation =====
    if( (!(i4Opt & TASK_OPTION_BYP_AE)) && m_i4SkipDoAFAECount == 0)
    {
        ...
        else
        {
            ...
            if(pAAOBuffer && pPSOBuffer)
            {
                IAeMgr::getInstance().doAFAE(m_i4SensorDev,
                                             ...
                                             i4ActiveAEItem,
                                             pAAOBuffer->mMagicNumber,
                                             bIsScheduleBy3A,rData.bFlashOpenByTask);
            }
            else
    ...
    //===== done =====
    if(IAeMgr::getInstance().IsAEStable(m_i4SensorDev) && m_i4SkipDoAFAECount == 0)
    {
        if(rData.bFlashOnOff || rData.bFlashOpenByTask)
