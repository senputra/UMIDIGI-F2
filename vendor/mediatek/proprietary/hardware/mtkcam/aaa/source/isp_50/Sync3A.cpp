...
protected:
    ...
    ICcuCtrl3ASync*             m_pCcuCtrl3ASync;
...
init(MINT32 i4Master, MINT32 i4Slave)
{
    ...
    else
    {
        ...
        if(m_pCcuCtrl3ASync == NULL)
        {
            ...
            CAM_LOGD("[%s] CCU_SYNC_DBG Create ICcuCtrl3ASync",__FUNCTION__);
        }
...
syncAwbInit()
{
    ...
    NVRAM_CAMERA_3A_STRUCT* p3aNvram;
    NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_3A, i4Master, (void*&)p3aNvram, MFALSE);
    rAWBInitData.main_param = p3aNvram->AWB[CAM_SCENARIO_PREVIEW];
    rAWBInitData.main_sync_param = p3aNvram->AWBSYNC;
    NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_3A, i4Slave, (void*&)p3aNvram, MFALSE);
...
syncAwbMain(const StatisticBufInfo* pMasterSttBuf, const StatisticBufInfo* pSlaveSttBuf)
{
    ...
    IAwbMgr::getInstance().getAAOConfig(i4Master, rMasterAWBConfig);
    IAwbMgr::getInstance().getAAOConfig(i4Slave, rSlaveAWBConfig);
    ...
    IspTuningMgr::getInstance().getCamInfo(static_cast<NSIspTuning::ESensorDev_T>(i4Master), rCamInfo);
    ccm = rCamInfo.rMtkCCM;
    ...
    m_rAwbSyncInput.main_ch.aao_buf_info.stat_line_size = IAeMgr::getInstance().getAAOLineByteSize(m_i4Master);  // TODO
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[0] = ((float)Complement2(ccm.cnv_1.bits.G2G_CNV_00,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[1] = ((float)Complement2(ccm.cnv_1.bits.G2G_CNV_01,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[2] = ((float)Complement2(ccm.cnv_2.bits.G2G_CNV_02,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[3] = ((float)Complement2(ccm.cnv_3.bits.G2G_CNV_10,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[4] = ((float)Complement2(ccm.cnv_3.bits.G2G_CNV_11,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[5] = ((float)Complement2(ccm.cnv_4.bits.G2G_CNV_12,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[6] = ((float)Complement2(ccm.cnv_5.bits.G2G_CNV_20,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[7] = ((float)Complement2(ccm.cnv_5.bits.G2G_CNV_21,13) / 512);
    m_rAwbSyncInput.main_ch.ccm.CCmatrix[8] = ((float)Complement2(ccm.cnv_6.bits.G2G_CNV_22,13) / 512);

    IspTuningMgr::getInstance().getCamInfo(static_cast<NSIspTuning::ESensorDev_T>(i4Slave), rCamInfo);
    ccm = rCamInfo.rMtkCCM;
    if(sizeof(AWB_OUTPUT_T) == sizeof(AWB_OUTPUT_N3D_T))
    {
    ...
    m_rAwbSyncInput.sub_ch.aao_buf_info.stat_line_size = IAeMgr::getInstance().getAAOLineByteSize(m_i4Slave);  // TODO
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[0] = ((float)Complement2(ccm.cnv_1.bits.G2G_CNV_00,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[1] = ((float)Complement2(ccm.cnv_1.bits.G2G_CNV_01,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[2] = ((float)Complement2(ccm.cnv_2.bits.G2G_CNV_02,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[3] = ((float)Complement2(ccm.cnv_3.bits.G2G_CNV_10,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[4] = ((float)Complement2(ccm.cnv_3.bits.G2G_CNV_11,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[5] = ((float)Complement2(ccm.cnv_4.bits.G2G_CNV_12,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[6] = ((float)Complement2(ccm.cnv_5.bits.G2G_CNV_20,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[7] = ((float)Complement2(ccm.cnv_5.bits.G2G_CNV_21,13) / 512);
    m_rAwbSyncInput.sub_ch.ccm.CCmatrix[8] = ((float)Complement2(ccm.cnv_6.bits.G2G_CNV_22,13) / 512);
...
update(MINT32 i4Opt)
{
    ...
    if (i4MeMasterOnOff != -1)
    {
        ...
        if(m_pCcuCtrl3ASync && m_pCcuCtrl3ASync->isSupport3ASync())
            IAeMgr::getInstance().CCUManualControl(m_i4Master, m_rManualAeMaster.u4ExpTime, m_rManualAeMaster.u4AfeGain, m_rManualAeMaster.u4IspGain, m_rManualAeMaster.fgOnOff);
    }
    ...
    if (i4MeSlaveOnOff != -1)
    {
        ...
        if(m_pCcuCtrl3ASync && m_pCcuCtrl3ASync->isSupport3ASync())
            IAeMgr::getInstance().CCUManualControl(m_i4Slave, m_rManualAeSlave.u4ExpTime, m_rManualAeSlave.u4AfeGain, m_rManualAeSlave.u4IspGain, m_rManualAeSlave.fgOnOff);
    }

    if (i4Opt & (ISync3A::E_SYNC3A_DO_AE | ISync3A::E_SYNC3A_DO_AE_PRECAP))
    {
        AE_MODE_CFG_T rAEInfoMaster = {};
        AE_MODE_CFG_T rAEInfoSlave = {};
        ...
        {
            IAeMgr::getInstance().updatePreviewParams(m_i4Master, rAEInfoMaster, m_rAeSyncOutput.main_ch.Output.i4AEidxNext, m_rAeSyncOutput.main_ch.Output.i4AEidxNextF);
            IAeMgr::getInstance().updatePreviewParams(m_i4Slave, rAEInfoSlave, m_rAeSyncOutput.sub_ch.Output.i4AEidxNext, m_rAeSyncOutput.sub_ch.Output.i4AEidxNextF);
            ...
            IAeMgr::getInstance().updateStereoDenoiseRatio(m_i4Master, (MINT32*) &m_rAeSyncOutput.main_ch.Output.u4DeltaBVRatio[0]);
            IAeMgr::getInstance().updateStereoDenoiseRatio(m_i4Slave, (MINT32*) &m_rAeSyncOutput.sub_ch.Output.u4DeltaBVRatio[0]);
        }
...
syncAeSetting(MINT32 i4MasterDev, MINT32 i4SlaveDev)
{
    if(m_pCcuCtrl3ASync && m_pCcuCtrl3ASync->isSupport3ASync())
    {
        ...
        syncAESettingData.master_sensor_idx = rParams.i4MasterIdx;
        CAM_LOGD("[%s] CCU_SYNC_DBG MSG_TO_CCU_SYNC_AE_SETTING, M(Idx/Dev)(%d/%d)",__FUNCTION__, rParams.i4MasterIdx, i4MasterDev);
    ...
    IAeMgr::getInstance().getPreviewParams(i4MasterDev, rMasterPreviewInfo);
    IAeMgr::getInstance().getPreviewParams(i4SlaveDev, rSlavePreviewInfo);

    IAeMgr::getInstance().getRTParams(i4MasterDev, rMasterFrameParams);
    IAeMgr::getInstance().getCurrentPlineTable(i4MasterDev, strPreviewAEPlineTableMain, strCaptureAEPlineTabMain, strStrobeAEPlineTableMain, strStrobeAFPlineTabMain);
    IAeMgr::getInstance().getCurrentPlineTable(i4SlaveDev, strPreviewAEPlineTableMain2, strCaptureAEPlineTabMain2, strStrobeAEPlineTableMain2, strStrobeAFPlineTabMain2);
    ...
    rInput.MasterCamAeIdxF = rMasterFrameParams.u4AEIndexF;
    rInput.MasterFinerEVIdxBase = rMasterPreviewInfo.u4AEFinerEVIdxBase;
    rInput.SlaveFinerEVIdxBase = rSlavePreviewInfo.u4AEFinerEVIdxBase;
    ...
    IAeMgr::getInstance().updatePreviewParams(i4SlaveDev, rMasterPreviewInfo, rOutput.u4SlaveAEindex, rOutput.u4SlaveAEindexF);
    IAeMgr::getInstance().setExp(i4SlaveDev, rMasterPreviewInfo.u4Eposuretime);
    IAeMgr::getInstance().setAfe(i4SlaveDev, rMasterPreviewInfo.u4AfeGain);
    IAeMgr::getInstance().setIsp(i4SlaveDev, rMasterPreviewInfo.u4IspGain);
}
VOID
Sync3AWrapper::
syncAwbSetting(MINT32 i4MasterDev, MINT32 i4SlaveDev)
{
    SYNC_AWB_DIRECT_GAIN_MAPPING_STRUCT rInput;
    ...
    rInput.MasterCalibrationData = rMasterAWBCalData;
    rInput.SlaveCalibrationData  = rSlaveAWBCalData;
    ...
    // update output to slave
    AWB_SYNC_OUTPUT_N3D_T rAwbSyncSlave;
    rAwbSyncSlave.rAWBGain          = m_rAwbSyncOutput.sub_ch.rAwbGain;
    rAwbSyncSlave.i4CCT             = m_rAwbSyncOutput.sub_ch.i4CCT;
    rAwbSyncSlave.rSyncAWBGain      = m_rAwbSyncOutput.sub_ch.rAwbGain;
    rAwbSyncSlave.i4XR              = m_rAwbSyncInput.sub_ch.awb_output_struct.rAWBSyncInput_N3D.i4XR;
...
updateFrameSync(MBOOL bEnable)
{
    ...
    if(m_pCcuCtrl3ASync && m_pCcuCtrl3ASync->isSupportFrameSync())
    {
        ...
        CAM_LOGD("[%s] ICcuCtrl3ASync is NULL", __FUNCTION__);
}
...
protected:
    ...
    mutable Mutex               m_LockSync3AWait;
...
enableSync(MBOOL fgOnOff)
{
    ...
    {
        GET_PROP("vendor.debug.sync3A.delay", "3", m_i4AwbSyncCount);
        MINT32 i4Ret = ::sem_init(&m_semSync, 0, 0);
        if(i4Ret!=0)
        {
            CAM_LOGE("m_semSync sem_init fail");
...
#if 0
        MINT32 i4Ret = m_Condition.waitRelative(m_LockSync3AWait, SEM_TIME_LIMIT_NS);
        if(i4Ret != 0)
...
sync2ASetting(MINT32 i4MasterIdx, MINT32 i4SlaveIdx)
{
    ...
    if(pSync3AWrap)
    {
        ...
        Stereo_Param_T rParams = ISync3AMgr::getInstance()->getStereoParams();
        rParams.i4MasterIdx = i4MasterIdx;
        rParams.i4SlaveIdx = i4SlaveIdx;
        CAM_LOGD("[%s] M(Idx/Dev)(%d/%d) S(Idx/Dev)(%d/%d)", __FUNCTION__, i4MasterIdx, i4MasterDev, i4SlaveIdx, i4SlaveDev);
        ISync3AMgr::getInstance()->setStereoParams(rParams);
...
init(MINT32 i4Policy, MINT32 i4MasterIdx, MINT32 i4SlaveIdx)
{
    ...
    MINT32 IsSupportAFMaster = IAfMgr::getInstance().isAFSupport(m_i4Master);
    MINT32 IsSupportAFSlave = IAfMgr::getInstance().isAFSupport(m_i4Slave);
    ...
    ICcuCtrl3ASync* pCcuCtrl3ASync = ICcuCtrl3ASync::createInstance();
...
setStereoParams(Stereo_Param_T const &rNewParam)
{
    ...
    if (i4MasterIdx == i4SlaveIdx)
    {
        CAM_LOGD("[%s] M&S(Dev, Idx) : (%d,%d), new(%d,%d)", __FUNCTION__, m_i4Master, i4MasterIdx, m_i4Slave, i4SlaveIdx);
        return;
    }
