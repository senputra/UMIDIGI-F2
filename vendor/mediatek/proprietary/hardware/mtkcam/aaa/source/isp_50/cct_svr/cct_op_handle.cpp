...
MVOID
CctHandle::
setIspOnOff_ADBS(MBOOL const fgOn)
{
    ISP_MGR_ADBS_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    NSIspTuningv3::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    //ISP_MGR_SL2_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
    //ISP_MGR_SL2_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(NSIspTuning::EIspProfile_NormalCapture);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
...
//    //NSIspTuningv3::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
//    MY_LOGD("[%s] HLR not enabled, fgOn(%d)", __FUNCTION__, fgOn);
//}
...
MBOOL
CctHandle::
getIspOnOff_ADBS() const
{
    return ISP_MGR_ADBS_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
}
...
#define SET_ISP_ON_OFF(_category)\
    ...
    switch  ( u4Category )
    {
        ...
        SET_ISP_ON_OFF(HFG);
        SET_ISP_ON_OFF(EE);
        //SET_ISP_ON_OFF(NR3D);// based on Choo, CCT should not control NR3D
        SET_ISP_ON_OFF(MFB);
