...
MRESULT AeMgr::UpdateSensorParams(AE_SENSOR_PARAM_T strSensorParams)
{
    ...
    if (strSensorParams.u8ExposureTime < 0)
    {
        strSensorParams.u8ExposureTime = 0;
        CAM_LOGD("[%s()] i4SensorDev:%d Exposure time less than 0:%lld\n", __FUNCTION__, m_eSensorDev, (long long)strSensorParams.u8ExposureTime);
    }

    if (strSensorParams.u4Sensitivity < 0)
    {
        strSensorParams.u4Sensitivity = 0;
        CAM_LOGD("[%s()] i4SensorDev:%d ISO less than 0:%d\n", __FUNCTION__, m_eSensorDev, strSensorParams.u4Sensitivity);
    }
    ...
    m_uManualModeIndex = m_SensorQueueCtrl.uInputIndex;
    m_SensorQueueCtrl.uOutputIndex = m_SensorQueueCtrl.uInputIndex;

    CAM_LOGD("[%s()] i4SensorDev:%d line:%d Idx:%d %d %d FrameDuration:%d %lld Exposure Time:%d %lld ISO:%d %d\n", __FUNCTION__, m_eSensorDev, __LINE__,
        m_SensorQueueCtrl.uInputIndex, m_SensorQueueCtrl.uOutputIndex, m_uManualModeIndex,
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::getSensorParamsCCU(AE_SENSOR_PARAM_T &a_rSensorInfo)
{
    GetCurrentInfo_AELock();
    a_rSensorInfo = m_rSensorCurrentInfoCCU;
    return S_AE_OK;
}
