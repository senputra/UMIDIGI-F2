...
getInstance(MINT32 const i4SensorOpenIndex)
{
    ...
    switch (i4SensorDevId)
    {
        ...
        {
            static Hal3ARawImp _singleton(SENSOR_DEV_MAIN);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_SUB:
        {
            static Hal3ARawImp _singleton(SENSOR_DEV_SUB);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_MAIN_2:
        {
            static Hal3ARawImp _singleton(SENSOR_DEV_MAIN_2);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_SUB_2:
        {
            static Hal3ARawImp _singleton(SENSOR_DEV_SUB_2);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_MAIN_3:
        {
            static Hal3ARawImp _singleton(SENSOR_DEV_MAIN_3);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        default:
...
Hal3ARawImp(MINT32 const i4SensorDevId)
    ...
    , m_rNew3AInfo()
    , mPostCommandMtx()
    , m_rIdxCache(static_cast<ESensorDev_T>(m_i4SensorDev))
    , m_i4BypassStt(0)
{
...
{
    getPropInt("vendor.debug.camera.log", &m_3ALogEnable, 0);
    getPropInt("vendor.debug.idxcache.log", &m_i4IdxCacheLogEnable, 0);
    getPropInt("vendor.debug.camera.copy.p1.lsc", &m_i4CopyLscP1En, 0);
    if ( m_3ALogEnable == 0 ) {
        getPropInt("vendor.debug.camera.log.hal3a", &m_3ALogEnable, 0);
    }
    ...
    if(!gTuningInitCtrl[m_i4SensorIdx].bHadModuleId)
    {
        ...
        NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_3A, m_i4SensorDev, p, MFALSE);
        NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_STROBE, m_i4SensorDev, q, MFALSE);
        NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_FLASH_CALIBRATION, m_i4SensorDev, r, MFALSE);
        NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_ISP, m_i4SensorDev, s, MFALSE);
        NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_SHADING, m_i4SensorDev, t, MFALSE);
        NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_IDX_TBL, m_i4SensorDev, u, MFALSE);
    ...
    ILscMgr::createInstance((ESensorDev_T)m_i4SensorDev, m_i4SensorIdx)->init();
    ...
    if(!gTuningInitCtrl[m_i4SensorIdx].bHadModuleId)
    {
        closeCustomTuningLibrary(m_i4sensorId, gTuningInitCtrl[m_i4SensorIdx].bHadModuleId);
...
config(const ConfigInfo_T& rConfigInfo)
{
    ...
    m_bIsFirstSetParams = MTRUE;
    m_bIsHighQualityCaptureOn = MFALSE;
    getPropInt("vendor.debug.camera.cammode", &m_i4CamModeEnable, 0);
    ...
    if(m_pThreadRaw)
        m_pThreadRaw->config(rConfigInfo);
    ...
    u4AFNVRAMIndex = m_pIdxMgr->query(static_cast<ESensorDev_T>(m_i4SensorDev), NSIspTuning::EModule_AF, rMapping_InfoAF, __FUNCTION__);
    IAfMgr::getInstance(m_i4SensorDev).setNVRAMIndex( u4AFNVRAMIndex);
    IPDMgr::getInstance().config(m_i4SensorDev, m_i4SensorIdx, m_u4SensorMode, u4AFNVRAMIndex);
     ...
     *******************************************************/
    AllResult_T *pAllResult = m_pResultPoolObj->getResultByReqNum(ConfigMagic, __FUNCTION__);
    ...
    pAllResult->ModuleResult[E_LSC_CONFIGRESULTTOISP]->write(&rLSCConfigResult); // frame id 1 for config
    AAA_TRACE_END_D;
    ...
    pAllResult->ModuleResult[E_AF_CONFIGRESULTTOISP]->write(&AFResultConfig); // frame id 1 for config
    AAA_TRACE_END_D;

    // ResultPool - update PDO config result to ResutlPool
    AAA_TRACE_D("PDOConfig2Pool");
    isp_pdo_cfg_t PDOResultConfig;
    IPDMgr::getInstance().getPDOHWCfg(m_i4SensorDev, &PDOResultConfig);
    pAllResult->ModuleResult[E_PDO_CONFIGRESULTTOISP]->write(&PDOResultConfig); // frame id 1 for config
    AAA_TRACE_END_D;
#endif
    ...
    pAllResult->ModuleResult[E_FLK_CONFIGRESULTTOISP]->write(&FLKResultConfig); // frame id 1 for config
    AAA_TRACE_END_D;
#endif
...
#if CAM3_STEREO_FEATURE_EN
    ISync3A::getInstance()->DevCount(MTRUE, m_i4SensorDev);
    ISync3AMgr::getInstance()->sendSync3ACtrl(ISync3AMgr::ESync3ACtrl_SetIsStart, (MINTPTR)(MTRUE), NULL);
#endif
...
#endif
    getPropInt("vendor.3a.cct.enable", &m_3ACctEnable, 0);
    if ( m_3ACctEnable == 1 ) {
...
stop()
{
...
#if CAM3_STEREO_FEATURE_EN
    ISync3A::getInstance()->DevCount(MFALSE, m_i4SensorDev);
    ISync3AMgr::getInstance()->sendSync3ACtrl(ISync3AMgr::ESync3ACtrl_SetIsStart, (MINTPTR)(MFALSE), NULL);

    MBOOL fgIsActive = ISync3A::getInstance()->isActive();
    if(fgIsActive)
    {
        ISync3A::getInstance()->uninit();
    ...
    AAA_TRACE_D("AE Stop");
/*#if CAM3_STEREO_FEATURE_EN
    if (ISync3A::getInstance()->isActive())
    {
...
#define FZoom(_f) { \
    ...
    else if (rNewParam.u4AeMode == MTK_CONTROL_AE_MODE_OFF && m_i4SubsampleCount > 1)
    {
        AE_SENSOR_PARAM_T strSensorParams;
        strSensorParams.u4Sensitivity   = rNewParam.i4Sensitivity;
        strSensorParams.u8ExposureTime  = rNewParam.i8ExposureTime;
    ...
    if (IAeMgr::getInstance().sendAECtrl(m_i4SensorDev, EAECtrl_GetNeedPresetControlCCU, NULL, NULL, NULL, NULL) && m_i4SubsampleCount > 1){
...
setStereoParams(Stereo_Param_T const & rNewParam)
{
    ...
    if(!m_bIsSkipSync3A)
    {
        ...
        MINT32 i4IsStart = 0;
        ISync3AMgr::getInstance()->sendSync3ACtrl(ISync3AMgr::ESync3ACtrl_GetIsStart,  reinterpret_cast<MINTPTR>(&i4IsStart), NULL);

        MBOOL fgIsInit = ISync3A::getInstance()->isInit();
            ...
            rNewParam.i4Sync2AMode == NS3Av3::E_SYNC2A_MODE::E_SYNC2A_MODE_DUAL_ZOOM )
        {
            if(!fgIsInit && (rNewParam.i4MasterIdx != rNewParam.i4SlaveIdx) )
                ISync3A::getInstance()->init(rNewParam.i4MasterIdx, rNewParam.i4SlaveIdx);
        ...
        //if(i4IsDoSync3A == MTRUE)
            ISync3A::getInstance()->setStereoParams(rNewParam);

        if(rNewParam.i4Sync2AMode == NS3Av3::E_SYNC2A_MODE::E_SYNC2A_MODE_NONE)
        {
            if(fgIsInit)
                ISync3A::getInstance()->uninit();
            m_pTaskMgr->sendEvent(ECmd_Sync3AEnd);
    ...
    if (rNewParam.i4MasterIdx != m_rLastStereoParam.i4MasterIdx)
    {
        m_rLastStereoParam.bIsChange4Master = MTRUE;
    }
    else
    {
        m_rLastStereoParam.bIsChange4Master = MFALSE;
    }

    if (rNewParam.i4SlaveIdx != m_rLastStereoParam.i4SlaveIdx)
    {
        m_rLastStereoParam.bIsChange4Slave = MTRUE;
    }
    else
    {
        m_rLastStereoParam.bIsChange4Slave = MFALSE;
    }
    ...
    if(isChanged)
    {
        CAM_LOGD("[%s] Dev(%d) m_rLastStereoParam (%d,%d)(%d,%d,%d)(%d,%d)switchOn(%d)IsChange(%d,%d)", __FUNCTION__,
        m_i4SensorDev,
        ...
        m_rLastStereoParam.bIsChange4Master,
        m_rLastStereoParam.bIsChange4Slave);
    }
#endif
    return MTRUE;
}
...
getIsDoSync3A(MINT32 i4Sync3ASwitchOn)
{
    MINT32 i4Ret = MFALSE;
#if CAM3_STEREO_FEATURE_EN
    MINT32 i4SwitchCamCount = ISync3A::getInstance()->getSwitchCamCount();
...
MUINT32
Hal3ARawImp::
getNvramIndex(CAM_IDX_QRY_COMB& info, EModule_T module, MUINT32& index)
{
    index = (MUINT32)m_rIdxCache.query(module, IDXCACHE_STAGE_P1, IDXCACHE_VALTYPE_CURRENT, info, __FUNCTION__);
...
MINT32
Hal3ARawImp::
getCurrResult(MUINT32 i4FrmId) const
{
    ...
    AllResult_T *pAllResult = m_pResultPoolObj->getResultByReqNum(i4FrmId, __FUNCTION__);
    if(pAllResult == NULL)
    {
    ...
    *(const_cast<MINT32*>(&m_rNew3AInfo.i4SensorSensitivity)) = rAEResult.i4SensorSensitivity;
    ...
    MBOOL bIsUpdated = pAllResult->ModuleResult[E_FLASH_RESULTTOMETA]->isValidate();
    if(pFLASHResult != NULL && bIsUpdated) {
    ...
    if (rHALResult.fgKeep == MTRUE)
        FlashMgr::getInstance(m_i4SensorDev)->setIsFlashOnCapture(MFALSE);
    ...
    if (ISync3A::getInstance()->isActive() && i4FrmId > u4AEStableMagic)
    {
        MINT32 i4Master = ISync3A::getInstance()->getMasterDev();
        MINT32 i4Slave = ISync3A::getInstance()->getSlaveDev();
    ...
    if (m_rParam.u1IsGetExif || m_bIsCapEnd)
    {
        ...
        if (ISync3A::getInstance()->isActive())
        {
            ...
            if(pAllResult->vecDbgN3DInfo.size() != 0)
                ISync3A::getInstance()->setDebugInfo((void*)(&rDbgN3DInfo));
        }
#endif
    }
    AAA_TRACE_END_MGR;
    ...
    pAllResult->ModuleResult[E_HAL_RESULTTOMETA]->write(&rHALResult);
    pAllResult->ModuleResult[E_AE_RESULTTOMETA]->write(&rAEResult);
    pAllResult->ModuleResult[E_AWB_RESULTTOMETA]->write(&rAWBResult);
    pAllResult->ModuleResult[E_LSC_RESULTTOMETA]->write(&rLSCResult);
    pAllResult->ModuleResult[E_FLASH_RESULTTOMETA]->write(&rFLASHResult);
    pAllResult->ModuleResult[E_FLK_RESULTTOMETA]->write(&rFLKResult);
    pAllResult->ModuleResult[E_LSC_CONFIGRESULTTOISP]->write(&rLSCConfigResult);
    pAllResult->ModuleResult[E_AE_CONFIGRESULTTOISP]->write(&AEResultConfig);
    pAllResult->ModuleResult[E_FLK_CONFIGRESULTTOISP]->write(&FLKResultConfig);
    pAllResult->ModuleResult[E_AWB_CONFIGRESULTTOISP]->write(&AWBResultConfig);
    pAllResult->ModuleResult[E_AE_RESULTINFO]->write(&AEResultInfo);
    pAllResult->ModuleResult[E_AWB_RESULTINFO4ISP]->write(&AWBResultInfo);
    AAA_TRACE_END_MGR;
    ...
    pAllResult->rEarlyCB.u1AfState = rAFResultFromMgr.afState;
    pAllResult->rEarlyCB.u1AeState = rAEResult.u1AeState;
    pAllResult->rEarlyCB.u1AeMode = rAEResult.u1AeMode;
    pAllResult->rEarlyCB.u1FlashState = rFLASHResult.u1FlashState;
    pAllResult->rEarlyCB.fgAeBvTrigger = rAEResult.fgAeBvTrigger;
    ...
    if( IsSupportAF && (!i4AFEnable)
        && pAllResult->rEarlyCB.u1AfState != MTK_CONTROL_AF_STATE_NOT_FOCUSED_LOCKED
        &&  pAllResult->rEarlyCB.u1AfState != MTK_CONTROL_AF_STATE_FOCUSED_LOCKED )
    {
        CAM_LOGD_IF(m_3ALogEnable, "[%s] overwrite AF state(%d)", __FUNCTION__, pAllResult->rEarlyCB.u1AfState);
        pAllResult->rEarlyCB.u1AfState = MTK_CONTROL_AF_STATE_NOT_FOCUSED_LOCKED;
    }
    ...
    {
        pAllResult->rEarlyCB.i4MasterIdx = m_rLastStereoParam.i4MasterIdx;
        pAllResult->rEarlyCB.i4SlaveIdx = m_rLastStereoParam.i4SlaveIdx;
        CAM_LOGD_IF(m_3ALogEnable, "[%s] rEarlyCB (MasterIdx, SlaveIdx)(%d,%d)", __FUNCTION__, pAllResult->rEarlyCB.i4MasterIdx, pAllResult->rEarlyCB.i4SlaveIdx);
    }
#endif
    ...
    MINT32 debugData = -2;
    getPropInt("vendor.debug.aaa_state.hdrMode", &debugData, -2);
...
postCommand(ECmd_T const r3ACmd, const ParamIspProfile_T* pParam)
{
        ...
        (r3ACmd == ECmd_Update) && (m_rParam.u4AeMode != MTK_CONTROL_AE_MODE_OFF) && (!bDummyAfterCapture))
    {
        ...
        // high quality cap
        MINT32 iForceHqc = 1;
        getPropInt("vendor.debug.camera.force_hqc", &iForceHqc, 1);
    ...
    AllResult_T *pAllResult = m_pResultPoolObj->getResultByReqNum(m_i4SttMagicNumber, __FUNCTION__);
    if(pAllResult)
    {
        FLASHResultToMeta_T* pFLASHResult = (FLASHResultToMeta_T*)(pAllResult->ModuleResult[E_FLK_RESULTTOMETA]->read());
    ...
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_IspProfile, (MVOID*)&(rQueryCommandQ.eIspProfile));
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_SensorMode, (MVOID*)&(rQueryCommandQ.eSensorMode));
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_FrontBin, (MVOID*)&(rQueryCommandQ.eFrontBin));
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_Flash, (MVOID*)&(rQueryCommandQ.eFlash));
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_DriverIC, (MVOID*)&(rQueryCommandQ.eDriverIC));
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_FaceDetection, (MVOID*)&(rQueryCommandQ.eFaceDetection));
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_App, (MVOID*)&(rQueryCommandQ.eApp));
    IdxMgr::createInstance(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev))->setMappingInfoByDim(static_cast<NSIspTuning::ESensorDev_T>(m_i4SensorDev), m_rParam.i4MagicNum, EDim_Zoom, (MVOID*)&(rQueryCommandQ.eZoom_Idx));

    CAM_IDX_QRY_COMB rMapping_Info;
    m_pIdxMgr->getMappingInfo(static_cast<ESensorDev_T>(m_i4SensorDev), rMapping_Info, m_rParam.i4MagicNum);

    MINT32 cacheEnable = 1;
    getPropInt("vendor.debug.cache.mode", &cacheEnable, 1);

    m_rIdxCache.setEnable(cacheEnable);
    m_rIdxCache.updateKey(rMapping_Info, IDXCACHE_STAGE_P1);
...
#if CAM3_STEREO_FEATURE_EN
        //For DualCam control flash on off
        MBOOL bIsTurnOff =  HAL3A_POLICY_FLASH_ONOFF_DUALCAM(m_i4SensorDev, m_rParam, m_rLastStereoParam);
        if (bIsTurnOff)
        {
            FlashHal::getInstance(m_i4SensorDev)->setOnOff(MFALSE, FLASH_HAL_SCENARIO_MAIN_FLASH);
            m_bFlashOpenedByTask = MFALSE;
...
#define SET_COMM_INFO1(_module, _chksum, _ver) \
    ...
    //Fix coverity:overrun-buffer
    int str_remands = (STRING_DEBUG_INFO_SIZE-1);
    int str_cnt = 0;
#if 1 //for test
    ...
    case E3ACtrl_NOTIFY_AF_FSC_INFO:
        ...
        case E3ACtrl_SetShadingEngMode:
            ...
            if (NSIspTuning::EOperMode_Meta != m_i4OperMode)
            {
                ...
                {
                case 0: // Disable
                    ILscMgr::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->enableDynamicShading(MFALSE, (MUINT32)m_i4OperMode);
                    ...
                    break;
                case 1: // Auto
                    ILscMgr::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->enableDynamicShading(MTRUE, (MUINT32)m_i4OperMode);
                    ILscMgr::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->setTsfOnOff(MFALSE);
        ...
        {
            if (m_rParam.u4AeMode == MTK_CONTROL_AE_MODE_ON_ALWAYS_FLASH ||
                m_rParam.u4AeMode == MTK_CONTROL_AE_MODE_ON_AUTO_FLASH ||
                ...
                *(reinterpret_cast<MINT32*>(iArg1)) = 0;
            }
        }
            break;
       ...
       case E3ACtrl_GetOperMode:
        ...
        case E3ACtrl_GetDualZoomInfo:
            {
                ...
                rDualZoomInfo->bSyncAFDone = ISync3A::getInstance()->isAFSyncFinish();
                rDualZoomInfo->bSync2ADone = ISync3A::getInstance()->is2ASyncFinish();
        ...
        case E3ACtrl_Sync3A_IsActive:
            *(reinterpret_cast<MBOOL*>(iArg1)) = ISync3A::getInstance()->isActive();
            break;
        case E3ACtrl_Sync3A_Sync2ASetting:
            {
                MINT32 i4MasterIdx = (MINT32)iArg1;
                MINT32 i4SlaveIdx = (MINT32)iArg2;
                CAM_LOGD("[%s] Dev(%d) MasterIdx(%d) SlaveIdx(%d)", __FUNCTION__, m_i4SensorDev, i4MasterIdx, i4SlaveIdx);
                ISync3A::getInstance()->sync2ASetting(i4MasterIdx, i4SlaveIdx);
            }
            break;
        case E3ACtrl_SetSync3ADevDoSwitch:
            {
                ...
                if (i4Sync3ADevDoSwitch == MTRUE && i4SwitchCamCount == 2)
                    ISync3A::getInstance()->setSwitchCamCount(i4Sync3ADevDoSwitch);
...
/*#if CAM3_STEREO_FEATURE_EN
    // For AE/AWB/AF Sync
    if(u4RawFmtType == SENSOR_RAW_MONO && ISync3A::getInstance()->isActive())
    {
        IAwbMgr::getInstance().disableAWB(m_i4SensorDev);
        ISync3A::getInstance()->disableSyncSupport(ISync3A::E_SYNC3A_SUPPORT_AWB);

        IAfMgr::getInstance().setAFMode(m_i4SensorDev, MTK_CONTROL_AF_MODE_OFF);
        ISync3A::getInstance()->enableSyncSupport(ISync3A::E_SYNC3A_SUPPORT_AF);
    }
#endif*/
}
...
if(m_i4SubsampleCount <= 1)
{
    ...
    {
        pAllResult = m_pResultPoolObj->getResultByPresetKey(u4PresetKey, __FUNCTION__);
        if(pAllResult != NULL)
        {
            i4ReqMagic = pAllResult->rRequestInfo.i4ReqMagic;
            ...
            pAEResult->i4SensorSensitivity = rAEInfo.u4RealISOValue;//ISO value updated
            pAEResult->i8SensorExposureTime = rAEInfo.u4Eposuretime;
            pAllResult->ModuleResult[E_AE_RESULTTOMETA]->write(pAEResult);

            // Update CCU result
            CCUResultInfo_T *pCCUResult = (CCUResultInfo_T*)(pAllResult->ModuleResult[E_CCU_RESULTINFO4OVERWRITE]->read());
            pCCUResult->u4P1DGNGain = rAEInfo.u4IspGain;
            pAllResult->ModuleResult[E_CCU_RESULTINFO4OVERWRITE]->write(pCCUResult);
    ...
    else
    {
        ...
        pAllResult = m_pResultPoolObj->getResultByReqNum(i4ReqMagic, __FUNCTION__);
    ...
    AllMetaResult_T* pMetaResult = m_pResultPoolObj->getMetadata(i4ReqMagic, __FUNCTION__);
    ...
    m_pResultPoolObj->returnMetadata(pMetaResult, __FUNCTION__);
    ...
    if(u4LTMReqNumber > 0)
    {
        pAllResult = m_pResultPoolObj->getResultByReqNum(u4LTMReqNumber, __FUNCTION__);
        if(pAllResult != NULL)
        {
            /************************************
             * 3. update object to result pool
             ************************************/
            // Update LTM result
            ISPResultToMeta_T *pISPResult = (ISPResultToMeta_T*)(pAllResult->ModuleResult[E_ISP_RESULTTOMETA]->read());

            pISPResult->rCamInfo.rCCU_Result.LTM.CLIP_TH = rLTMInfo.ltm_ct;
            ...
            for (int numy = 0; numy < CCU_LTM_BLK_Y_NUM; numy++)
                for (int numx = 0; numx < CCU_LTM_BLK_X_NUM; numx++){
                    ...
                    for (int k = 0; k < (LTM_BIN - 1); k++){
                        if((k%2) == 0){
                            pISPResult->rCamInfo.rCCU_Result.LTM.LTM_Curve.lut[Bin_Pos * 16 + (k/2)].bits.LTMTC_TONECURVE_LUT_L = rLTMInfo.ltm_curve[numy][numx][k];
                        }
                        else{
                            pISPResult->rCamInfo.rCCU_Result.LTM.LTM_Curve.lut[Bin_Pos * 16 + (k/2)].bits.LTMTC_TONECURVE_LUT_H = rLTMInfo.ltm_curve[numy][numx][k];
                        }
                    }
                    //special case (colltct k=32(in each bin) on the final part of 1-dim array)
                    if((Bin_Pos % 2) == 0)
                        pISPResult->rCamInfo.rCCU_Result.LTM.LTM_Curve.lut[(CCU_LTM_BLK_Y_NUM * CCU_LTM_BLK_X_NUM ) * 16 + (Bin_Pos / 2)].bits.LTMTC_TONECURVE_LUT_L = rLTMInfo.ltm_curve[numy][numx][(LTM_BIN - 1)];
                    else
                        pISPResult->rCamInfo.rCCU_Result.LTM.LTM_Curve.lut[(CCU_LTM_BLK_Y_NUM * CCU_LTM_BLK_X_NUM ) * 16 + (Bin_Pos / 2)].bits.LTMTC_TONECURVE_LUT_H = rLTMInfo.ltm_curve[numy][numx][(LTM_BIN - 1)];
                }

            pISPResult->rCamInfo.rCCU_Result.LTM.LTM_Valid = CCU_VALID_PRESENT;

            //LTM twin A
            ::memcpy(&(pISPResult->rCamInfo.rCCU_Result.LTM.P1_LTM_Reg[0]), &(rLTMInfo.ltm_hw_reg[0][0]), sizeof(MUINT32)*LTM_HW_REG_MAX);
            //LTM twin B
            ::memcpy(&(pISPResult->rCamInfo.rCCU_Result.LTM.P1_LTM_Reg[1]), &(rLTMInfo.ltm_hw_reg[1][0]), sizeof(MUINT32)*LTM_HW_REG_MAX);
            //LTMS
            ::memcpy(&pISPResult->rCamInfo.rCCU_Result.LTM.P1_LTMS_Reg, &(rLTMInfo.ltms_hw_reg[0]), sizeof(MUINT32)*LTMS_HW_REG_MAX);
            //LTMSO
            ::memcpy(&(pISPResult->rCamInfo.rCCU_Result.LTM.LTMSO[0]), rLTMInfo.ltmso_buffer_addr_va, rLTMInfo.ltmso_size);  //ltmso_size should be 2160 * 4

            // Update LTM info
            pAllResult->ModuleResult[E_ISP_RESULTTOMETA]->write(pISPResult);
    ...
    if(u4HLRReqNumber > 0)
    {
        pAllResult = m_pResultPoolObj->getResultByReqNum(u4HLRReqNumber, __FUNCTION__);
        if(pAllResult != NULL)
        {
            /************************************
             * 3. update object to result pool
            ...
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y0.bits.HLR_SAT_0        = rHLRInfo.hlr_out.hlr_sat_0;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y0.bits.HLR_SAT_1        = rHLRInfo.hlr_out.hlr_sat_1;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y1.bits.HLR_SAT_2        = rHLRInfo.hlr_out.hlr_sat_2;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y1.bits.HLR_SAT_3        = rHLRInfo.hlr_out.hlr_sat_3;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y2.bits.HLR_SAT_4        = rHLRInfo.hlr_out.hlr_sat_4;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y2.bits.HLR_SAT_5        = rHLRInfo.hlr_out.hlr_sat_5;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y3.bits.HLR_SAT_6        = rHLRInfo.hlr_out.hlr_sat_6;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_y3.bits.HLR_SAT_7        = rHLRInfo.hlr_out.hlr_sat_7;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x0.bits.HLR_NODE_0       = rHLRInfo.hlr_out.hlr_node_0;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x0.bits.HLR_NODE_1       = rHLRInfo.hlr_out.hlr_node_1;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x1.bits.HLR_NODE_2       = rHLRInfo.hlr_out.hlr_node_2;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x1.bits.HLR_NODE_3       = rHLRInfo.hlr_out.hlr_node_3;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x2.bits.HLR_NODE_4       = rHLRInfo.hlr_out.hlr_node_4;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x2.bits.HLR_NODE_5       = rHLRInfo.hlr_out.hlr_node_5;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x3.bits.HLR_NODE_6       = rHLRInfo.hlr_out.hlr_node_6;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_x3.bits.HLR_NODE_7       = rHLRInfo.hlr_out.hlr_node_7;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s0.bits.HLR_SLP_0        = rHLRInfo.hlr_out.hlr_slp_0;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s0.bits.HLR_SLP_1        = rHLRInfo.hlr_out.hlr_slp_1;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s0.bits.HLR_SLP_2        = rHLRInfo.hlr_out.hlr_slp_2;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s1.bits.HLR_SLP_3        = rHLRInfo.hlr_out.hlr_slp_3;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s1.bits.HLR_SLP_4        = rHLRInfo.hlr_out.hlr_slp_4;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s1.bits.HLR_SLP_5        = rHLRInfo.hlr_out.hlr_slp_5;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s2.bits.HLR_SLP_6        = rHLRInfo.hlr_out.hlr_slp_6;
            // Allen-HH TODO ( 7 Lines ) :
            //pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s2.bits.HLR_RED_PRT_STR  = rHLRInfo.hlr_out.hlr_red_prt_str;
            //pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s2.bits.HLR_BLUE_PRT_STR = rHLRInfo.hlr_out.hlr_blue_prt_str;
            //pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.est_s2.bits.HLR_MAX_RAT      = rHLRInfo.hlr_out.hlr_max_rat;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.lmg.bits.HLR_BLD_FG          = rHLRInfo.hlr_out.hlr_bld_fg;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.lmg.bits.HLR_BLD_HIGH        = rHLRInfo.hlr_out.hlr_bld_high;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.lmg.bits.HLR_BLD_SLP         = rHLRInfo.hlr_out.hlr_bld_slp;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.prt.bits.HLR_PRT_TH          = rHLRInfo.hlr_out.hlr_prt_th;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.prt.bits.HLR_PRT_SLP         = rHLRInfo.hlr_out.hlr_prt_slp;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.prt.bits.HLR_PRT_EN          = rHLRInfo.hlr_out.hlr_prt_en;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.clp.bits.HLR_CLP_VAL         = rHLRInfo.hlr_out.hlr_clip_val;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.efct.bits.HLR_OFF_CLP_VAL    = rHLRInfo.hlr_out.hlr_off_clip_val;
            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.efct.bits.HLR_EFCT_ON        = rHLRInfo.hlr_out.hlr_efct_on;
            //pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.ctl.bits.HLR_TILE_EDGE       = rHLRInfo.hlr_out.hlr_tile_edge;
            //pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.ctl2.bits.HLR_TDR_WD         = rHLRInfo.hlr_out.hlr_tdr_wd;
            //pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.ctl2.bits.HLR_TDR_HT         = rHLRInfo.hlr_out.hlr_tdr_ht;
            //pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Data.lkmsb.bits.CAMCTL_HLR_R1_LKMSB=rHLRInfo.hlr_out.hlr_r1_lkmsb;

            pISPResult->rCamInfo.rCCU_Result.HLR.HLR_Valid = CCU_VALID_PRESENT;
...
ThreadAEStart(MVOID* arg)
{
    ...
    AllResult_T *pAllResult = _this->m_pResultPoolObj->getResultByReqNum(ConfigMagic, __FUNCTION__);
...
ThreadAFStart(MVOID* arg)
{
    ...
    IAfMgr::getInstance(_this->m_i4SensorDev).getStaticInfo(_this->m_rAFStaticInfo, LOG_TAG);
    ::pthread_exit((MVOID*)0);
}
...
Hal3ARawImp::handleBadPicture(MINT32 i4ReqMagic, MINT32 i4P1DoneSttNum /* = -1 */)
{
    ...
    if(i4Ret)
    {
        ...
        AllResult_T *pAllResult = m_pResultPoolObj->getResultByReqNum(i4MagicOfPartialFrame, __FUNCTION__);
        if(pAllResult)
        {
            HALResultToMeta_T* pHalResult = (HALResultToMeta_T*)(pAllResult->ModuleResult[E_HAL_RESULTTOMETA]->read());
            FLASHResultToMeta_T* pFLASHResult = (FLASHResultToMeta_T*)(pAllResult->ModuleResult[E_FLASH_RESULTTOMETA]->read());
            AllMetaResult_T *pMetaResult = m_pResultPoolObj->getMetadata(i4MagicOfPartialFrame, __FUNCTION__);
            if(pHalResult != NULL) {
                pHalResult->fgBadPicture = MTRUE;
                pAllResult->ModuleResult[E_HAL_RESULTTOMETA]->write(pHalResult);
            }
            if(pFLASHResult != NULL) {
                pFLASHResult->u1FlashState = MTK_FLASH_STATE_PARTIAL;
                pAllResult->ModuleResult[E_FLASH_RESULTTOMETA]->write(pFLASHResult);
