...
*/
    ...
    if(!(i4Opt & TASK_OPTION_BYP_FLASH) && !(m_i4SkipFrame >= 0))
    {
        ...
        if(pAAOBuffer)
        {
            ...
            if(FlashMgr::getInstance(m_i4SensorDev)->isHighBrightnessFlow())
            {
                AE_PERFRAME_INFO_T AEPerframeInfo;
                IAeMgr::getInstance().getAEInfo(m_i4SensorDev, AEPerframeInfo);
                FlashMgr::getInstance(m_i4SensorDev)->setAlgoExpPara((AEPerframeInfo.rAEISPInfo.u8P1Exposuretime_ns/1000), AEPerframeInfo.rAEISPInfo.u4P1SensorGain, AEPerframeInfo.rAEISPInfo.u4P1DGNGain, 0);
            }
#endif
