...
public:
    ...
    virtual MBOOL       prepareCCUCB(MUINT32 u4PreFrmId, ISP_NVRAM_OBC_T const &rOBCResult);
    virtual MBOOL       notify4CCU();//{return MTRUE;}
...
getInstance(MINT32 const i4SensorOpenIndex)
{
    ...
    switch (i4SensorDevId)
    {
        ...
        {
            static Hal3AYuvImp _singleton(SENSOR_DEV_MAIN);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_SUB:
        {
            static Hal3AYuvImp _singleton(SENSOR_DEV_SUB);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_MAIN_2:
        {
            static Hal3AYuvImp _singleton(SENSOR_DEV_MAIN_2);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_SUB_2:
        {
            static Hal3AYuvImp _singleton(SENSOR_DEV_SUB_2);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        case SENSOR_DEV_MAIN_3:
        {
            static Hal3AYuvImp _singleton(SENSOR_DEV_MAIN_3);
            _singleton.init(i4SensorOpenIndex);
            return &_singleton;
        }
        /*case SENSOR_DEV_MAIN:
...
Hal3AYuvImp(MINT32 const i4SensorDevId)
    ...
    , m_rYuvFlParam()
{
    MY_LOG("[%s] sensorDev(%d)", __FUNCTION__, i4SensorDevId);
}
...
MBOOL
Hal3AYuvImp::
prepareCCUCB(MUINT32 /*u4PreFrmId*/, ISP_NVRAM_OBC_T const &/*rOBCResult*/)
{
    return MTRUE;
}
...
MINT32 Hal3AYuvImp::updateAeFlashCaptureParams()
{
    ...
    MINT32 i4TestGain = 0;
    getPropInt("vendor.debug.aaa_hal_yuv.shutter", &i4TestShutter, -1);
    getPropInt("vendor.debug.aaa_hal_yuv.gain", &i4TestGain, -1);
