...
{
    ISP_MGR_DM_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    ISP_MGR_LDNR_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    ISP_MGR_YNR_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
//    EECtrl_Clip.SEEE_OVRSH_CLIP_EN = fgOn;//definition change: CAM_SEEE_CLIP_CTRL CAM+AA4H
    ...
    ISP_MGR_EE_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    ISP_MGR_CNR_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTCNREnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    ISP_MGR_COLOR::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    ISP_MGR_LPCNR_T::getInstance((ESensorDev_T)m_eSensorEnum).setEnable(0, fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    MY_LOGD("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    return  ISP_MGR_DM_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
}
...
{
    return  ISP_MGR_LDNR_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
}
...
{
    return ISP_MGR_EE_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
}
...
{
    return  ISP_MGR_COLOR::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
}
...
{
    return  ISP_MGR_LPCNR_T::getInstance((ESensorDev_T)m_eSensorEnum).isEnable(0);
}
                ...
                MUINT32 *pu4RealParaOutLen )
{
    ...
    case FT_CCT_OP_GET_SENSOR_MODE_NUM:
        ...
        break;
    case FT_CCT_OP_TEST_SET_PROP:
        *pu4RealParaOutLen = 0;
        break;
    case FT_CCT_OP_TEST_GET_PROP:
        *pu4RealParaOutLen = 0;
        break;
    default:
                ...
                MUINT32 *pu4RealParaOutLen )
{
    ...
    case FT_CCT_OP_ISP_GET_CCM_MATRIX:
        ...
        *pu4RealParaOutLen = 0;
        ...
        ISP_MGR_CCM_T::getInstance((ESensorDev_T)m_eSensorEnum).get(ISP_MGR_CCM::ECCM_D1, ccm);
    ...
    */
        ...
        //ISP_MGR_CCM_T::getInstance((ESensorDev_T)m_eSensorEnum).reset();
        ISP_MGR_CCM_T::getInstance((ESensorDev_T)m_eSensorEnum).put(ISP_MGR_CCM::ECCM_D1, rDst);
        NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
                    ...
                    MUINT32 *pu4RealParaOutLen )
{
    ...
    case FT_CCT_OP_AE_APPLY_CAPTURE_AE_PARAM:
        ...
        if(u4ParaInLen == sizeof(MINT32) && puParaIn != NULL)
        {
            ...
            strSensorParams.u8ExposureTime  = acdk_ae_cfg.u4Eposuretime*1000;
            //
...
MRESULT CctHandle::TakePicture(MUINT32 a_i4SensorMode, eACDK_CAP_FORMAT type, MINT32 expTime = -1)
{
    MY_LOGD("TakePicture: sensorMode=%d, type=%d", a_i4SensorMode, type);
    MRESULT result = S_CCT_CALIBRATION_OK;
    int ret = 0;
    ...
    //switch(a_i4SensorMode){
    //default:
    //case ACDK_CDVT_SENSOR_MODE_PREVIEW:
    //    cb_param.profile_mode = CT_PROFILE_MODE_PREVIEW;
    //    break;
    //case ACDK_CDVT_SENSOR_MODE_CAPTURE:
    //    cb_param.profile_mode = CT_PROFILE_MODE_CAPTURE;
    //    break;
    //case ACDK_CDVT_SENSOR_MODE_VIDEO:
    //    cb_param.profile_mode = CT_PROFILE_MODE_VIDEO;
    //    break;
    //}
    ...
    ret |= system_call("setprop jpeg.camera.dump.JpegNode 1");
    ret |= system_call("setprop debug.camera.dump.p2 1");
    ret |= system_call("sync");
    ret |= system_call("input keyevent KEYCODE_CAMERA");
    //ret |= system_call("/vendor/bin/camtool capture");
    //ret |= system_call("/mnt/vendor/cct/camtool2 capture");
    MY_LOGD("ret=%d", ret);

    usleep(1000*1000);
    while(1) {
        ret = system_call("test -e /data/vendor/camera_dump/*.jpg");
        if(!ret) {break;}
        MY_LOGD("ret=%d, wait a moment..", ret);
        usleep(500*1000);
    }
    while(1) {
    ...
    if ((dir=opendir("/mnt/vendor/cct")))
    {
        ...
        {
            if( ptr->d_name == strstr(ptr->d_name, "pure__") ) // start with "pure__"
            {
    ...
    if( pipeOut = popen("echo /mnt/vendor/cct/pure__* | sed -r 's/^.+\\///' | sed 's/_/ /g;s/x/ /g;s/[^0-9 ]*//g;'", "r") )
    {
        ...
        MY_LOGD("Width(%d), Height(%d), BitDepth(%d), ColorOrder(%d)", rawWidth, rawHeight, rawBitDepth, rawColorOrder );
    }
    else
    {
        MY_LOGE("Failed to Get Raw Width/Height/BitDepth/ColorOrder" );
    ...
    {
        fgets(rawFilePath, sizeof(rawFilePath)-1, pipeOut);
        for(int i = 0; i< 1024; i++)
        {
            if( rawFilePath[i] < 32 || rawFilePath[i] > 126 )
            {
                rawFilePath[i] = 0;
                break;
            }
        ...
        MY_LOGD("ImgSize(%d), FileSize(%d)", (rawWidth * rawHeight * 2), packFileSize );
    }
    else
    {
        MY_LOGE("Failed to Get rawFilePath/packFileSize" );
                                  ...
                                  ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
    ...
    for (MINT32 i4Exp = a_i4ExpStart; i4Exp <= a_i4ExpEnd; i4Exp += a_i4ExpInterval)
    {
        ...
        *   Take Picture
        */
        if((mrRet = TakePicture(a_i4SensorMode, PURE_RAW8_TYPE, (5 * i4Exp) )) != S_CCT_CALIBRATION_OK)
        {
                                         ...
                                         ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
    ...
    for (MINT32 i4Gain = a_i4GainStart; i4Gain <= a_i4GainEnd; i4Gain += a_i4GainInterval)
    {
        ...
        *   Take Picture
        */
        if((mrRet = TakePicture(a_i4SensorMode, PURE_RAW8_TYPE, (5 * a_i4ExpTime) )) != S_CCT_CALIBRATION_OK)
        {
            MY_LOGE("TakePicture Fail");
            a_prSensorTestOutput->i4ErrorCode = 1; // RAW analysis error
            goto Exit;
                                     ...
                                     ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
    ...
    *   Take Picture
    */
    if((mrRet = TakePicture(a_i4SensorMode, PROCESSED_RAW8_TYPE, (5 * i4DefaultExpTInUS) )) != S_CCT_CALIBRATION_OK)
    {
        MY_LOGE("TakePicture Fail ");
        prSensorCalibrationOutput->i4ErrorCode = 1; // RAW analysis error
        goto Exit;
    ...
    *   Take Picture
    */
    if((mrRet = TakePicture(a_i4SensorMode, PROCESSED_RAW8_TYPE, (5 * i4TargetExpTInUs_LA) )) != S_CCT_CALIBRATION_OK)
    {
        MY_LOGE("TakePicture Fail ");
        prSensorCalibrationOutput->i4ErrorCode = 1; // RAW analysis error
        goto Exit;
    ...
    *   Take Picture
    */
    if((mrRet = TakePicture(a_i4SensorMode, PROCESSED_RAW8_TYPE, (5 * i4TargetExpTInUs_UA) )) != S_CCT_CALIBRATION_OK)
    {
        MY_LOGE("TakePicture Fail ");
        prSensorCalibrationOutput->i4ErrorCode = 1; // RAW analysis error
        goto Exit;
                                                    ...
                                                    ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
    ...
    */
    ...
    for (MINT32 ExpT = i4ExpStartInUS; ExpT <= i4ExpEndInUS; ExpT += i4ExpInterval)
    {
        ...
        *   Take Picture
        */
        if((mrRet = TakePicture(a_i4SensorMode, PROCESSED_RAW8_TYPE, (5 * ExpT)) ) != S_CCT_CALIBRATION_OK)
        {
            MY_LOGE("TakePicture Fail ");
            prSensorCalibrationOutput->i4ErrorCode = 1; // RAW analysis error
            goto Exit;
...
GetShadingRaw(eACDK_CAMERA_MODE mode, UINT8 /*ColorTemp*/) {
    eACDK_CAP_FORMAT img_type = PROCESSED_RAW10_TYPE;
    ...
    case FT_CCT_OP_CAL_FLASH:
        {
            ...
            MINT32 fgRetTemp = TakePicture(CT_PROFILE_MODE_CAPTURE, PURE_RAW8_TYPE);
