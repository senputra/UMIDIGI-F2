...
dequeueHwBuf()
{
    ...
    MBOOL bEnable = property_get_int32("vendor.mvhdr.dump.enable", 0);
    MBOOL bEnableHW = property_get_int32("vendor.mvhdrhw.dump.enable", 0);
    ...
    if(m_pCamsvSttPipe != NULL)
    {
        ...
        if (bEnable) {
            ...
            char fileName[64];
            char folderName[64];
            strncpy(folderName, "/sdcard/mvhdr", sizeof(folderName));
            folderName[sizeof(folderName) - 1] = '\0';
            snprintf(fileName, sizeof(fileName), "/sdcard/mvhdr/mvhdr_%d_%d.raw", rLastBuf.mMetaData.mMagicNum_hal, count++);
        ...
        if(bEnableHW){
            char fileName[64];
            char folderName[64];
            strncpy(folderName, "/sdcard/mvhdrHwBuf", sizeof(folderName));
            folderName[sizeof(folderName)-1] = '\0';
            snprintf(fileName, sizeof(fileName), "/sdcard/mvhdrHwBuf/mvhdr_%d.hwbuf", rLastBuf.mMetaData.mMagicNum_hal);
...
abortDequeue()
{
    ...
    if(m_pCamsvSttPipe != NULL)
    {
     m_pCamsvSttPipe->abortDma(PORT_CAMSV_IMGO,LOG_TAG);
    }
    for(int i = 0; i < MAX_STATISTIC_BUFFER_CNT; i++)
        m_rBufInfo[i].flush();
}
