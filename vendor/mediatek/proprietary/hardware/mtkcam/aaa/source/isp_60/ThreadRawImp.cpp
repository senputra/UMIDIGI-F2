...
ThreadRawImp::createEventThread()
{
    ...
    if (mi4User > 0)
    {
        MY_LOGD_IF(mu4LogEn, "[%s] mi4User(%d)", __FUNCTION__, std::atomic_load((&mi4User)));
    }
    else
    {
        MY_LOGD("[%s] +", __FUNCTION__);
...
ThreadRawImp::postToEventThread(E_3AEvent_T eEvent, MINT32 opt)
{
    ...
    mEventQCond.notify_all();
    MY_LOGD_IF(mu4LogEn, "[%s] mEventQ size(%d) + rEvent(%d/%d)", __FUNCTION__, (MINT32)mEventQ.size(), rEvent.eEvent, rEvent.i4Opt);
}
...
ThreadRawImp::getEvent(IThreadRaw::EventQ_T &rEvent)
{
    ...
    MY_LOGD_IF(mu4LogEn, "[%s] mEventQ.size()=%d ", __FUNCTION__, (MINT32)mEventQ.size());

    if (mEventQ.size() == 0)
    {
        MY_LOGD_IF(mu4LogEn, "[%s] mEventQCond.wait +", __FUNCTION__);
        mEventQCond.wait_for(autoLock, std::chrono::nanoseconds(20000000000));
        MY_LOGD_IF(mu4LogEn, "[%s] mEventQCond.wait -", __FUNCTION__);
    }
    if (mEventQ.size() == 0)
    {
        MY_LOGD_IF(mu4LogEn, "[%s] mEventQ.size() = %d after mEventQCond.wait/waitRelative\n", __FUNCTION__, (MINT32)mEventQ.size());
        return MFALSE;
    }

    rEvent = *mEventQ.begin();
    MY_LOGD_IF(mu4LogEn, "mEventQ-size(%d), event(%d/%d)", (MINT32)mEventQ.size(), rEvent.eEvent, rEvent.i4Opt);
...
ThreadRawImp::clearEventQ()
{
    ...
    for (List<EventQ_T>::iterator it = mEventQ.begin(); it != mEventQ.end();)
    {
...
ThreadRawImp::onAFThreadLoop(MVOID *arg)
{
    ...
    MINT32 i4MasterDev = ISync3AMgr::getInstance()->getSync3A()->getAFMasterDev();
    MINT32 i4SlaveDev = ISync3AMgr::getInstance()->getSync3A()->getAFSlaveDev();
    MY_LOGD("[%s] Dev(%d) M&S(%d, %d)", __FUNCTION__, _this->mi4SensorDev, i4MasterDev, i4SlaveDev);
    if (i4SlaveDev == _this->mi4SensorDev && i4SlaveDev != i4MasterDev)
    {
        ISync3AMgr::getInstance()->getSync3A()->syncAF(_this->mi4SensorDev, MTRUE, MTRUE);
    }
#endif
    ...
    while (_this->mbAFThreadLoop && IsSupportAF)
    {
        ...
        if(_this->mpTaskMgr->dequeStt(Task_Update_AF) >= 0)
        {
            ...
            MY_LOGD_IF(_this->mu4LogEn, "[Hal3A::AFThreadFunc] AF waitIrq done and clear magicNumber buffer\n");
            ...
            for(MINT32 i = 0; i < VecI4MagicNumber.size(); i++)
            {
                ...
                if(i4MagicNum > 0)
                {
                    ...
                    pAllResult = _this->m_pResultPoolObj->getAllResult(i4MagicNum);
                    pMetaResult = _this->m_pResultPoolObj->getMetadata(i4MagicNum);
...
ThreadRawImp::resumeAFThread()
{
    if(mbAFPause)
    {
        ...
        if (mbAFThreadLoop)
        {
            MINT32 i4MasterDev = ISync3AMgr::getInstance()->getSync3A()->getAFMasterDev();
            MINT32 i4SlaveDev = ISync3AMgr::getInstance()->getSync3A()->getAFSlaveDev();
            MY_LOGD("[%s] resumAFSync Dev(%d) M&S(%d, %d)",__FUNCTION__, mi4SensorDev, i4MasterDev, i4SlaveDev);
            if (i4SlaveDev == mi4SensorDev && i4SlaveDev != i4MasterDev)
            {
                ISync3AMgr::getInstance()->getSync3A()->syncAF(mi4SensorDev, MTRUE, MTRUE);
            }
        }
#endif
        MINT32 i4SemValue = 0;
...
getRequest(IThreadRaw::ReqQ_T &rReq, MINT32 i4Count)
{
    ...
    MY_LOGD_IF(mu4LogEn, "[%s] mCmdQ.size()=%d i4Count=%d", __FUNCTION__, (MINT32)mReqQ.size(), i4Count);
    ...
    if(mReqQ.size() == 0 && i4Count != 0)
    {
        MY_LOGD_IF(mu4LogEn, "[%s] AF queue is zero, so break to doAF", __FUNCTION__);
        return MFALSE;
    }
#else
...
{
    MY_LOGD_IF(mu4LogEn, "[%s] + i4MagicNum(%d, %d)", __FUNCTION__, i4MagicNumber, AFParam.i4MagicNum);

    AAA_TRACE_HAL(getAllResult);
    AllResult_T *pAllResult = m_pResultPoolObj->getAllResult(i4MagicNumber);
    AAA_TRACE_END_HAL;
    ...
    if(pAllResult)
    {
        ...
        rAFResult.u2LensPosition = (MINT32)( 0<=rAFResultFromMgr.lensPosition && rAFResultFromMgr.lensPosition<=1023 ? rAFResultFromMgr.lensPosition : -1);
        ...
        m_pResultPoolObj->updateResult(LOG_TAG, i4MagicNumber, E_AF_RESULTTOMETA, &rAFResult);
        AAA_TRACE_END_MGR;

        AFResultConfig_T AFResultConfig;
        IAfMgr::getInstance(mi4SensorDev).getHWCfgReg(&AFResultConfig);
        m_pResultPoolObj->updateResult(LOG_TAG, i4MagicNumber, E_AF_CONFIGRESULTTOISP, &AFResultConfig);
        ...
        m_pResultPoolObj->updateResult(LOG_TAG, i4MagicNumber, E_PDO_CONFIGRESULTTOISP, &PDOResultConfig);

        MY_LOGD_IF(mu4LogEn, "[%s] AfState(%d), u1LensState(%d, %d), enableAFHw(%d), module update config and result(#%d) done", __FUNCTION__, rAFResult.u1AfState, rAFResult.u1LensState, rAFResultFromMgr.lensState, AFResultConfig.afIspRegInfo.enableAFHw, i4MagicNumber);
#endif
    }
    else
