...
public:
    ...
    MRESULT setAEEVCompIndex(MINT32 i4NewEVIndex, MFLOAT fStep, MBOOL is_preset = MFALSE);
    ...
    MRESULT setDummyFrameInfo(MUINT8 u1IsDummyFramemmy, MBOOL is_preset = MFALSE);
    // EM feature APIs.
    ...
    MBOOL setSensorSize(MUINT32 width, MUINT32 height);
    MRESULT reconfig();
    MRESULT setSkipCalCnt(MUINT32 u4SkipCalCnt);
...
private:
    ...
    MRESULT prepareCapParams(MBOOL bSetAEEVAndAELock=MFALSE);
    ...
    MRESULT setAEPlineIdx(MINT32 i4ManualPlineIdx, MINT32 i4CustomPlineMode);

    /* =============== ae_mgr_flow.cpp =============== */

    /* Common AE algo control */
    MRESULT prepareAlgoInitInfo (AE_CORE_INIT* pInfo);
    MRESULT prepareAlgoFrameInfo(AE_CORE_CTRL_CCU_VSYNC_INFO* pInfo, MBOOL is_preset = MFALSE);
...
private:
    ...
    MINT32  m_i4EVIndex;        // set usage
    MINT32  m_i4EVIndexPreset;  // preset usage
    ...
    LIB3A_AE_EVCOMP_T m_eAEEVcomp;      // set usage
    LIB3A_AE_EVCOMP_T m_eAEEVcompLock;;
    LIB3A_AE_EVCOMP_T m_eAEEVcompPreset;// preset usage
    ...
    mutable std::mutex m_LockAECalc;
    ...
    MUINT32 m_u4SkipCalCnt; // frame counter to skip AE calculation, if > 0: to skip, if = 0: not to skip
    ...
    MUINT32 m_u4SuperNightShotFrameIndex;
    ...
    MBOOL m_bIsRestoreFrame;

    // dummy frame check
    MUINT8 m_u1IsDummyFramePreset;
    MUINT8 m_u1IsDummyFrameSet;
};
