...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
class TSFOBufMgrImp : public TSFOBufMgr
{
public:
    virtual MBOOL destroyInstance();
                  TSFOBufMgrImp(MINT32 i4SensorDev, MINT32 i4SensorIdx, const ConfigInfo_T& rConfigInfo);
    virtual      ~TSFOBufMgrImp();
...
{
    TSFOBufMgrImp* pObj = new TSFOBufMgrImp(i4SensorDev,i4SensorIdx,rConfigInfo);
    return pObj;
}
    ...
    , m_bPreStop(MFALSE)
{
    ...
    property_get("vendor.debug.tsfo_mgr.enable", value, "0");
...
dequeueHwBuf()
{
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        //Check SensorId for raw dynamic switch
        CAM_LOGD("[%s][raw dynamic switch] m_i4SensorDev(%d) m_i4SensorId :%d, m_HW_QBuf[QBufIdx].mSensorId :%d ", __FUNCTION__, m_i4SensorDev, m_i4SensorId, m_HW_QBuf[QBufIdx].mSensorId);
        if(m_i4SensorId == -1)
        {
            CAM_LOGD("[%s][raw dynamic switch] m_i4SensorDev(%d) init - m_i4SensorId :%d, m_HW_QBuf[QBufIdx].mSensorId :%d ", __FUNCTION__, m_i4SensorDev, m_i4SensorId, m_HW_QBuf[QBufIdx].mSensorId);
            m_i4SensorId = m_HW_QBuf[QBufIdx].mSensorId;
        }
        else if(m_HW_QBuf[QBufIdx].mSensorId != m_i4SensorId)
        {
            CAM_LOGD("[%s][raw dynamic switch] m_i4SensorDev(%d) return NULL - m_i4SensorId :%d, m_HW_QBuf[QBufIdx].mSensorId :%d ", __FUNCTION__, m_i4SensorDev, m_i4SensorId, m_HW_QBuf[QBufIdx].mSensorId);
            m_i4SensorId = m_HW_QBuf[QBufIdx].mSensorId;
            return NULL;
        }

        CAM_TRACE_BEGIN("TSFO STT deque");
        //CAM_LOGD("m_pSttPipe hw +");
        ...
        //CAM_LOGD("TSFO dequeue size: %d, ret: %d", size, ret);
...
dequeueSwBuf()
{
    ...
    MBOOL bEnable = property_get_int32("vendor.tsfo.dump.enable", 0);
    MBOOL bEnableHW = property_get_int32("vendor.tsfohw.dump.enable", 0);
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        if(bWaitDeqHW)
        {
            ...
            if(m_bAbort || m_bPreStop || m_bForceReturn) {
                CAM_LOGD("[%s] TSFO buffer return NULL: EnQBufIdx(%d)/DeQBufIdx(%d)\n", __FUNCTION__, m_HW_EnQBufIdx, m_HW_DeQBufIdx);
                m_bForceReturn = MFALSE;
                return NULL;
            }
            CAM_LOGD("[%s] TSFO deque success - EnQBufIdx(%d) -> DeQBufIdx(%d)\n", __FUNCTION__, m_HW_EnQBufIdx, m_HW_DeQBufIdx);
        }
        ...
        if (bEnable) {
            ...
            strncpy(folderName, "/sdcard/tsfo", FILE_NAME_SIZE);
            sprintf(fileName, "/sdcard/tsfo/tsfo_%d_%d.raw", rLastBuf.mMetaData.mMagicNum_hal, count++);
        ...
        if(bEnableHW){
            char fileName[FILE_NAME_SIZE];
            char folderName[FILE_NAME_SIZE];
            strncpy(folderName, "/sdcard/tsfoHwBuf", FILE_NAME_SIZE);
            sprintf(fileName, "/sdcard/tsfoHwBuf/tsfo_%d.hwbuf", rLastBuf.mMetaData.mMagicNum_hal);
