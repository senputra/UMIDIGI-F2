...
transform_SLK(MUINT8 SubModuleIndex, const SL2_INFO_T& rSl2Info, const CROP_RZ_INFO_T& rCropRzInfo)
{
    ...
    if ((SubModuleIndex >= ESLK_D1) && rCropRzInfo.fgRRZOnOff)
    {
        ...
        else
        {
            ...
            //overflow case, RzRto < (1/8), due to MDP resize effect
            if((HRZ_COM >= (1<<HW_COM_DIP_DEPTH)) ||
                (VRZ_COM >= (1<<HW_COM_DIP_DEPTH))){
                HRZ_COM = 2048;
                VRZ_COM = 2048;

                CENTR_X = (MINT32)((MFLOAT)CENTR_X * fRzRtoX);
                CENTR_Y = (MINT32)((MFLOAT)CENTR_Y * fRzRtoY);
                MFLOAT fRzRtoMax = (fRzRtoX > fRzRtoY) ? fRzRtoX : fRzRtoY;

                R0 =   (MINT32)((MFLOAT)R0   * fRzRtoMax);
                R1 =   (MINT32)((MFLOAT)R1   * fRzRtoMax);
                R2 =   (MINT32)((MFLOAT)R2   * fRzRtoMax);
                maxR = (MINT32)((MFLOAT)maxR * fRzRtoMax);
            }
