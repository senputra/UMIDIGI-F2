...
tsfReformTbl()
{
        ...
        ((rTrfmA.u4Y + rTrfmA.u4H) >= (rTrfmB.u4Y + rTrfmB.u4H)))
    {
        ...
        for (i = 0; i < RING_TBL_NUM; i++)
        {
            ...
            if (!rTbl.cropOut(rTrfmRel, rTblOut))
            {
...
tsfSetSL2(const MTK_TSF_SL2_PARAM_STRUCT& rSL2)
{
    ...
    if (m_pLsc->getRawSize(m_eSensorMode, u4TgW, u4TgH) && u4BinW != 0 && u4BinH != 0)
    {
        ...
        CAM_LOGD_IF(fgLogEn, "set SLK from default value");
...
{
    MBOOL fgLogEn = (m_u4LogEn & EN_LSC_LOG_TSF_PER_FRAME) ? MTRUE : MFALSE;
    CAM_LOGD_IF(fgLogEn, "[%s] dump(%d), path(%s)", __func__, m_bDump, m_strDumpFileNormal.c_str());
