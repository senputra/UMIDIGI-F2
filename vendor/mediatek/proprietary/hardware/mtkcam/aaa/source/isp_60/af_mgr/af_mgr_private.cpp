...
MRESULT AfMgr::setAFArea( CameraFocusArea_T &sInAFArea)
{
    ...
    if ( 1 <= sInAFArea.u4Count)
    {
            ...
            (0 <= sInAFArea.rAreas[0].i4Top  && sInAFArea.rAreas[0].i4Bottom <= (MINT32)m_i4TGSzH))
        {
            ...
            CAM_LOGD_IF(LEVEL_FLOW,
                        "#(%5d,%5d) cmd-%s EasyTuing[3~5] : (%d, %d, %d), ISO(%d), Resize Percent(%d)",
                        m_u4ReqMagicNum,
                        m_u4StaMagicNum,
                        __FUNCTION__,
                        m_ptrNVRam->rAFNVRAM.i4EasyTuning[3],
                        m_ptrNVRam->rAFNVRAM.i4EasyTuning[4],
                        m_ptrNVRam->rAFNVRAM.i4EasyTuning[5],
                        m_sAFInput->i4ISO,
                        Percent_TouchAFROI);
                ...
                if ( ptrPDRes)
                {
                    ...
                    for (MINT32 i = 0; i < ptrPDRes->numRes; i++)
                    {
                        if (ptrPDRes->Res[i].sROIInfo.sType == eAF_ROI_TYPE_AP)
                        {
                            ...
                            {
                                //continue;
                                CAM_LOGD("[%s] Mismatch PDRoi(%d %d %d %d)!= APCmd(%d %d %d %d) ",
                                    __FUNCTION__,
                                    ptrPDRes->Res[i].sROIInfo.sPDROI.i4X, ptrPDRes->Res[i].sROIInfo.sPDROI.i4Y,
                                    ptrPDRes->Res[i].sROIInfo.sPDROI.i4W, ptrPDRes->Res[i].sROIInfo.sPDROI.i4H,
                                    m_sArea_APCmd.i4X, m_sArea_APCmd.i4Y, m_sArea_APCmd.i4W, m_sArea_APCmd.i4H);
                            }
                        }
                        ...
                        m_sAFInput->sPDInfo.sPDBlockInfo[_idx].i4ConfidenceExt   = ptrPDRes->Res[i].confidenceExt;

                        // print log if necessary
                        if ( LEVEL_STT)
                        {
                            CAM_LOGD("[%s] #%d (%d,%4d,%4d,%4d,%4d,%3d->%3d,%6d,%3d,%d,%d)",
                                     __FUNCTION__,
                                     _idx,
                                     ...
                                     m_sAFInput->sPDInfo.sPDBlockInfo[_idx].i4ConfidenceExt);
                        }
