...
dequeueHwBuf()
{
    ...
    switch( m_i4PDDataPath)
    {
        ...
        case FEATURE_PDAF_SUPPORT_PBN_PDO:
        {
            if(m_pSttPipe)
            {
                ...
                if (rDQBuf.mSensorId != m_i4SensorIdx)
                {
                    CAM_LOGW("[%s] dev(%d), CAM_AB and CAM_C are happened switching", __FUNCTION__, m_i4SensorDev);
                    return -1;
                }
                ...
                if( bDequed && rDQBuf.mvOut.size())
                {
                    ...
                    if (rLastDQBuf.mSize == 0)
                    {
                        /* for W+T statistic virtual port */
                        CAM_LOGD_IF( m_bDebugEnable, "[%s] dev(%d), statistic virtual port", __FUNCTION__, m_i4SensorDev);
                        usleep(30000);
                        return -1;
                    }
                }
    ...
    FLASHResultToMeta_T* pFLASHResult = (FLASHResultToMeta_T*)pResultPoolObj->getResult(rHwBuf.mMagicNumber,E_FLASH_RESULTTOMETA, __FUNCTION__);
