...
#define ENABLE_LIGHT_CALC_INIT_INDEX 0
...
AeMgr(MINT32 const i4SensorIndex)
    ...
    , m_eAEEVcompLock(LIB3A_AE_EV_COMP_00)
    ...
    , m_u4SuperNightShotFrameIndex(0)
    , m_bInitAE(MFALSE)
    ...
    , m_u4AdbISOspeed(0)
    , m_u4SkipCalCnt(0)
    , m_u1IsDummyFramePreset(0)
    , m_u1IsDummyFrameSet(0)
{
...
MRESULT AeMgr::Stop()
{
    ...
    AE_LOG( "[%s()] i4SensorDev:%d reset priority & super night shot&m_AePriorityParam, eAEScene:%d to LIB3A_AE_SCENE_AUTO, reset buff mode\n",__FUNCTION__, m_eSensorDev,m_eAEScene);
    return S_AE_OK;
}
...
MRESULT AeMgr::setAEEVCompIndex(MINT32 i4NewEVIndex, MFLOAT fStep, MBOOL is_preset)
{
    ...
    if (i4EVIndex != i4NewEVIndex)
    {
        ...
        if (!is_preset && m_i4AIS4Idx)
        {
            AE_LOG("[%s()] EV compensation with AIS4 change to %d->%d(%d)\n", __FUNCTION__, i4EVValue, i4EVValue + 10*m_i4AIS4Idx, m_i4AIS4Idx);
            i4EVValue = i4EVValue + 10*m_i4AIS4Idx;
        }
        ...
        if (is_preset)
        {
            if (m_u1IsDummyFramePreset && (!m_bSuperNightShotMode)) // modify dummy after capture ev value as preview ev value
            {
                eAEEVcomp = m_eAEEVcompLock;
                AE_LOG("[%s()] Dummy after capture, preset eEVcomp:%d\n", __FUNCTION__, m_eAEEVcompLock);
            }
        ...
        {
            if (m_u1IsDummyFrameSet && (!m_bSuperNightShotMode)) // modify dummy after capture ev value as preview ev value
            {
                eAEEVcomp = m_eAEEVcompLock;
                AE_LOG("[%s()] Dummy after capture, set eEVcomp:%d\n", __FUNCTION__, m_eAEEVcompLock);
            }
            m_i4EVIndex = i4EVIndex;
            m_eAEEVcomp = eAEEVcomp;
            // only prepare Capture params at set timing to avoid preset and set influence each other
            if (m_bAPAELock)
            {
                m_eAEState = MTK_CONTROL_AE_STATE_LOCKED;
                m_i4WaitVDNum = (m_i4IspGainDelayFrames + ISP_GAIN_DELAY_OFFSET + 1);
                prepareCapParams(MTRUE);
                AE_LOG("[%s()] SensorDev:%d EV compensation with AE lock, prepare capture param in advance at set timing\n", __FUNCTION__, m_eSensorDev);
            }
        }

        // resetCCUStable();
        AE_LOG("[%s()] SensorDev:%d m_i4EVIndex(set/preset):%d/%d m_eAEEVcomp(set/preset):%d/%d m_fEVCompStep:%f m_bAELock:%d is_preset:%d\n", __FUNCTION__,
            m_eSensorDev, m_i4EVIndex, m_i4EVIndexPreset, m_eAEEVcomp, m_eAEEVcompPreset, m_fEVCompStep, m_bAELock, is_preset);
        // m_eAEState = MTK_CONTROL_AE_STATE_SEARCHING;
    }
...
MRESULT AeMgr::setCamScenarioMode(MUINT32 u4NewCamScenarioMode, MBOOL bInit) // currently only for AE_MODE_4CELL_MVHDR_TARGET
{
    ...
    MUINT32 result=0;
    MINT32 i4SensorDevID, i4CurrModuleId, i4CurrLensId;
    ...
    if( m_eCamScenarioMode != eNewCamScenarioMode || bInit) {
        ...
        m_rAECustPARAM = getAEModuleParamData(m_eCamScenarioMode, m_i4SensorIdx, i4CurrModuleId, i4CurrLensId);
        AE_LOG("[%s()] (0x%8x)=m_pCamCalDrvObj->GetCamCalCalData SensorID:0x%0X ModuleID:0x%0X LensID:0x%0X", __FUNCTION__, result, i4SensorDevID, i4CurrModuleId, i4CurrLensId);
        /*
        switch(m_eSensorDev) {
...
MRESULT AeMgr::setNVRAMIndex(MUINT32 a_eNVRAMIndex, MBOOL bInit)
{
    if( m_u4AENVRAMIdx != a_eNVRAMIndex || bInit) {
        ...
        if(!bInit) {
            std::lock_guard<std::mutex> lock((m_LockAECalc));
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL AeMgr::setSensorSize(MUINT32 width, MUINT32 height)
{
    // Window size
    MUINT32 u4BlocksizeX = ((width / m_rAEConfigInfo.u4BlockNumW)/2)*2;
    MUINT32 u4BlocksizeY = ((height / m_rAEConfigInfo.u4BlockNumH)/2)*2;
    m_rAEConfigInfo.u4AEOverExpCntShift = (((u4BlocksizeX/4)*(u4BlocksizeY/2)>255)?MTRUE:MFALSE);
    AE_LOG( "[%s()] m_eSensorDev: %d, m_eSensorMode: %d, Hbinwidth/Hbinheight: %d/%d -> %d/%d, u4BlocksizeX/Y/Shift: %d/%d/%d, u4BlockNumX/Y: %d/%d\n",
            __FUNCTION__, m_eSensorDev, m_eSensorMode, m_u4Hbinwidth, m_u4Hbinheight, width, height, u4BlocksizeX, u4BlocksizeY, m_rAEConfigInfo.u4AEOverExpCntShift,
            m_rAEConfigInfo.u4BlockNumW, m_rAEConfigInfo.u4BlockNumH);
    m_u4Hbinwidth = width;
    ...
    AE_LOG_IF(m_3ALogEnable,"[%s()] i4AETgValidBlockWidth/Height: %d/%d -> %d/%d, i4PitchY/i4OriginY: %d/%d \n", __FUNCTION__,
           m_i4AETgValidBlockWidth, m_i4AETgValidBlockHeight, i4AETgValidBlockWidth, i4AETgValidBlockHeight, i4PitchY, i4OriginY);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::setSkipCalCnt(MUINT32 u4SkipCalCnt)
{
    AE_LOG( "[%s] m_eSensorDev: %d, m_u4SkipCalCnt: %d -> %d\n", __FUNCTION__, m_eSensorDev, m_u4SkipCalCnt, u4SkipCalCnt);
    m_u4SkipCalCnt = u4SkipCalCnt;
    return S_AE_OK;
}
...
MRESULT AeMgr::setAELock()
{
    ...
    {
        AE_LOG("[%s] SensorDev:%d m_bAELock:%d->0 m_bAPAELock:%d m_bAFAELock:%d\n", __FUNCTION__, m_eSensorDev, m_bAELock, m_bAPAELock, m_bAFAELock);
        // AE unlock
        m_bAELock = MFALSE;
        // m_eAEState = MTK_CONTROL_AE_STATE_SEARCHING;
    }
    ...
    {
        AE_LOG("[%s] SensorDev:%d m_bAELock:%d->1 m_bAPAELock:%d m_bAFAELock:%d\n", __FUNCTION__, m_eSensorDev, m_bAELock, m_bAPAELock, m_bAFAELock);
        // AE lock
        m_bAELock = MTRUE;
    }
    return S_AE_OK;
...
MRESULT AeMgr::setAPAELock(MBOOL bAPAELock, MBOOL bMetaSetCtrl)
{
    ...
    {
        AE_LOG("[%s()] SensorDev:%d m_bAPAELock:%d->%d\n", __FUNCTION__, m_eSensorDev, m_bAPAELock, bAPAELock);
        ...
        {
            // AE lock
            m_eAEEVcompLock  = m_eAEEVcomp;
            prepareCapParams();
            // m_eAEState = MTK_CONTROL_AE_STATE_LOCKED;
            AE_LOG("[%s()] m_eAEEVcompLock  = m_eAEEVcomp =%d\n", __FUNCTION__, m_eAEEVcompLock );
        }
    }
    return S_AE_OK;
}
...
MRESULT AeMgr::setAFAELock(MBOOL bAFAELock)
{
    ...
    {
        AE_LOG("[%s()] SensorDev:%d m_bAFAELock:%d->%d\n", __FUNCTION__, m_eSensorDev, m_bAFAELock, bAFAELock);
...
MRESULT AeMgr::doPvAE(MINT64 i8TimeStamp, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule)
{
    ...
    m_bAlgoResultUpdate = MFALSE;
    MBOOL resetStable = MFALSE;

    // P90 only for skip dual cam apply
    if(bAAASchedule && i4ActiveAEItem == E_AE_AE_APPLY){
        AE_LOG( "[%s()] for CCU AE, dual cam apply skip AE mgr state change, m_bAEStable:%d", __FUNCTION__, m_bAEStable);
    } else {
        resetStable = setAEMgrStateCPU(AE_MGR_STATE_DOPVAE);
    }
    ...
    AE_LOG_IF(m_bAlgoResultUpdate, "[%s():monitor] SensorDev: %d, Mag: %d, CWVY: %d, AEStable: %d, AlgoAPAELock/AlgoAFAELock: %d/%d, Index/F: %d/%d, Algo: %d/%d/%d (%d), Real: %d/%d/%d (%d), EV/BV/BVWOStrobe: %d/%d/%d bAAASchedule:%d i4ActiveAEItem:%d\n", __FUNCTION__,
              ...
              m_i4EVvalue, m_i4BVvalue, m_i4BVvalueWOStrobe, bAAASchedule, i4ActiveAEItem);
        ...
        if (m_bAEStable == MTRUE) {
            if (m_bAlgoAPAELock == MFALSE) {
                ...
                */
                ...
                if((!m_u1IsDummyFrameSet) && (!m_bSuperNightShotMode))
                {
                    m_eAEEVcompLock = m_eAEEVcomp;
                }
                m_eAEState = MTK_CONTROL_AE_STATE_LOCKED;
...
MRESULT AeMgr::doPreCapAE(MINT64 i8TimeStamp, MBOOL bIsStrobeFired, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule)
{
    ...
    if(m_bAEReadyCapture && !m_bCShot) {
        ...
        AE_LOG( "[%s()] trans to PRECAPTURE state and skip preCapAE, i4SensorDev:%d\n", __FUNCTION__, m_eSensorDev);
        return S_AE_OK;
    }
...
{
    MBOOL isNeedBackupFlahsFD = MFALSE;
    if (m_u4AEMgrStateCPU == AE_MGR_STATE_DOAFAE) {
        isNeedBackupFlahsFD = MTRUE;
    }
    setAEMgrStateCPU(AE_MGR_STATE_DORESTOREAE);
    AE_LOG( "[%s()] i4SensorDev:%d, m_rAESettingBackup.EvSetting: %d/%d/%d (ISO: %d), m_rAESettingBackup.u4Index/F: %d/%d (IndexBase: %d), isNeedBackupFlahsFD: %d +\n", __FUNCTION__, m_eSensorDev,
              m_rAESettingBackup.EvSetting.u4Eposuretime, m_rAESettingBackup.EvSetting.u4AfeGain, m_rAESettingBackup.EvSetting.u4IspGain, m_rAESettingBackup.u4ISO,
              m_rAESettingBackup.u4Index, m_rAESettingBackup.u4IndexF, m_rAESettingBackup.u4AEFinerEVIdxBase, isNeedBackupFlahsFD);
    m_rAESettingBackup.bResetIndex = MTRUE;
    UpdateSensorISPParams(AE_FLOW_STATE_RESTORE, m_rAESettingBackup);
    disableManualCCU(MTRUE);
    m_bAEStable = MFALSE;
    m_bAEReadyCapture = MFALSE;
    m_bIsRestoreFrame = MTRUE;
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    m_pIAeCalc = NS3A::IAeAlgo::createInstance<NS3A::EAAAOpt_OpenSource>(m_i4SensorIdx);
#else
    m_pIAeCalc = NS3A::IAeCalc::createInstance<NS3A::EAAAOpt_MTK>(m_i4SensorIdx);
    if (m_bCCUAEFlag){
        m_pIAeFlowCCU->init();
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::prepareCapParams(MBOOL bSetAEEVAndAELock)
{
    if(m_pIAeCalc != NULL) {
        ...
        if (m_bAPAELock)
        {
            ...
            {
                AE_LOG("[%s()] m_u4IndexF:%d m_eAEEVcompLock:%d, m_eAEEVcomp:%d\n", __FUNCTION__,m_u4IndexF,m_eAEEVcompLock,m_eAEEVcomp);
                m_pIAeCalc->recalcAEExpInfoEVcomp_v4p0(&m_rAESettingCapture,m_u4IndexF, &m_CaptureTableF, (CCU_LIB3A_AE_EVCOMP_T)m_eAEEVcompLock, (CCU_LIB3A_AE_EVCOMP_T)m_eAEEVcomp, m_rAESettingCapture.u4AEFinerEVIdxBase, m_u4IndexFMax, m_u4IndexFMin);
            }
            else
            {
                memcpy(&m_rAESettingCapture, &m_rAEAlgoOutput, sizeof(strAERealSetting));
            }
        }else{
          //m_pIAeCalc->switchCaptureState(&m_rAEAlgoOutput, &m_rAESettingCapture, 0);
          m_pIAeCalc->switchCaptureState(&m_rAEAlgoOutput, &m_rAESettingCapture, m_i4EVIndex*m_fEVCompStep*1000);
        }
...
MRESULT AeMgr::getCaptureSetting(strAERealSetting* pCapSetting)
{
    ...
    if(m_eAEManualCapPline == EAEManualPline_AIS2Capture) { // get capture setting from customized AIS2 manual pline table
        m_pIAeCalc->recalcAEExpInfo_v4p0(&m_rAEInfoPreview, (MUINT32 *)Log2x1000, pCapSetting, &m_rAEAlgoOutput, &strCurrentCaptureTable, m_pPreviewTableCurrent, &(m_pAEPlineTable->AEGainList));
        m_i4AIS4CWR = m_rAEInfoPreview.u4AECurrentTarget;
        m_i4AIS4CWV = m_rAEInfoPreview.u4MgrCWValue;
        AE_LOG("[%s()] from customized AIS2 manual pline table CWR:%d CWV:%d\n", __FUNCTION__, m_i4AIS4CWR, m_i4AIS4CWV);
    }
    else {
...
MINT32 AeMgr::getAEInitExpoSetting(AEInitExpoSetting_T &a_rAEInitExpoSetting)
{
    ...
    AE_LOG( "[%s()] m_eSensorDev: %d u4SensorMode: %d u4AETargetMode: %d ENABLE_LIGHT_CALC_INIT_INDEX:%d\n",
            __FUNCTION__, m_eSensorDev,
            a_rAEInitExpoSetting.u4SensorMode, a_rAEInitExpoSetting.u4AETargetMode,ENABLE_LIGHT_CALC_INIT_INDEX);
    ...
    if(((m_eSensorDev == ESensorDev_Sub) /*|| (m_eSensorDev == ESensorDev_Main)*/) && (m_bInitAE == MTRUE) && ENABLE_LIGHT_CALC_INIT_INDEX){
        UpdateLightInfo();
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::setDummyFrameInfo(MUINT8 u1IsDummyFramemmy, MBOOL is_preset)
{
    if (is_preset)
    {
        // preset
        m_u1IsDummyFramePreset = u1IsDummyFramemmy;
    }
    else
    {
        // set
        m_u1IsDummyFrameSet = u1IsDummyFramemmy;
    }
    AE_LOG("[%s()] SensorDev:%d is_preset:%d IsDummyFrame(Preset/Set):%d/%d\n", __FUNCTION__, m_eSensorDev, is_preset, m_u1IsDummyFramePreset, m_u1IsDummyFrameSet);
    return S_AE_OK;
}
