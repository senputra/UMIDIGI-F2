...
#define MY_INST NS3Av3::INST_T<AfCCU>
...
IAfCxU* AfCCU::getInstance(MINT32 sensorDev, MINT32 sensorIdx)
{
    ...
    if (isCCUInitStatus == CCU_CTRL_SUCCEED)
    {
        ...
        std::call_once(rSingleton.onceFlag, [&] {
            rSingleton.instance = std::make_unique<AfCCU>(sensorDev);
        } );

        // CAM_LOGD("[%s] SensorDev(%d), SensorIdx(%d) instance(%p)\n", __FUNCTION__, sensorDev, i4SensorIdx, rSingleton.instance.get());
...
MINT32 AfCCU::config(ConfigAFInput_T &configInput, ConfigAFOutput_T &configOutput)
{
    ...
    if (m_ptrCcu)
    {
        ...
        CAM_LOGD("%s Dev(%d) isConfigChange(%d), sTG_SZ(%d, %d), sBIN_SZ(%d, %d), AF_BLK_NUM(%d, %d)",
                 __FUNCTION__,
                 m_sensorDev,
                 configOutput.initHWConfigOutput.sInitHWCfg.u4IsConfigChanged,
                 configOutput.initHWConfigOutput.sInitHWCfg.sTG_SZ.i4W,
                 configOutput.initHWConfigOutput.sInitHWCfg.sTG_SZ.i4H,
                 configOutput.initHWConfigOutput.sInitHWCfg.sBIN_SZ.i4W,
                 configOutput.initHWConfigOutput.sInitHWCfg.sBIN_SZ.i4H,
...
{
    // collect all lock before calling ccu stop/uninit
    Mutex::Autolock lock( m_Lock);
    Mutex::Autolock datalock( m_DataLock);
...
MINT32 AfCCU::uninit()
{
    // move to this->stop since stop call ccu stop/uninit
    //Mutex::Autolock lock( m_Lock);
    //Mutex::Autolock datalock( m_DataLock);
