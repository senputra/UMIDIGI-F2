...
AAASensorMgr()
    ...
    , m_bFlickerState_MainThree(MFALSE)
    ...
    , m_bEnableBuffMode_MainThree(MFALSE)
{
...
init(MINT32 const i4SensorIdx)
{
    ...
    MINT32 i4BeforeUserCount = std::atomic_fetch_add((&m_Users), 1);
    CAM_LOGD("[%s()] FrameRate: Main/Sub/Main2/Sub2/Main3 = %d/%d/%d/%d/%d BeforeUserCount:%d\n", __FUNCTION__,
        m_u4SensorFrameRateMain, m_u4SensorFrameRateSub, m_u4SensorFrameRateMain2, m_u4SensorFrameRateSub2, m_u4SensorFrameRateMain3, i4BeforeUserCount);
...
setSensorExpTime(MINT32 i4SensorDev, MUINT32 a_u4ExpTime)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        if (m_i4FrameSyncSub2Fps == 0){
            /*if (m_bEnableBuffMode_SubTwo)
                ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_SUB_2, SENSOR_CMD_SET_SENSOR_EXP_TIME_BUF_MODE, (MUINTPTR)&a_u4ExpTime, 0, 0);
            else*/
        ...
        if (m_i4FrameSyncMain3Fps == 0){
            /*if (m_bEnableBuffMode_MainThree)
                ret = m_pHalSensorObj->sendCommand(NSCam::SENSOR_DEV_MAIN_3, SENSOR_CMD_SET_SENSOR_EXP_TIME_BUF_MODE, (MUINTPTR)&a_u4ExpTime, 0, 0);
            else*/
...
setFlickerFrameRateActive(MINT32 i4SensorDev, MBOOL a_bFlickerFPSAvtive)
{
    ...
    // Set frame rate
    CAM_LOGD_IF(m_bDebugEnable,"sensorDev(%d) a_bFlickerFPSAvtive(%d) m_bFlickerState(%d, %d, %d, %d, %d)\n", i4SensorDev, a_bFlickerFPSAvtive, m_bFlickerState_Main, m_bFlickerState_Sub, m_bFlickerState_MainTwo, m_bFlickerState_SubTwo, m_bFlickerState_MainThree);
    ...
    if(i4SensorDev == ESensorDev_Main) { 
        ...
        m_bFlickerState_MainThree = a_bFlickerFPSAvtive;
    } else{
        CAM_LOGD("[%s()] Sensor type: %d a_bFlickerFPSAvtive=%d", __FUNCTION__, i4SensorDev,a_bFlickerFPSAvtive);
        return E_AAA_SENSOR_NULL;
    }
...
MRESULT AAASensorMgr::setPreviewMaxFrameRate(MINT32 i4SensorDev, MUINT32 frameRate, MUINT32 u4SensorMode)
{
    ...
    // Set max frame rate
        ...
        if(i4SensorDev == ESensorDev_Main) {
            ...
            CAM_LOGD("[%s()] Sensor type:%d frameRate=%d", __FUNCTION__, i4SensorDev,frameRate);
            return E_AAA_SENSOR_NULL;
        }
    }
...
updateStereoFrameSyncFps(MINT32 i4SensorDev, MINT32 i4SensorDev2, MUINT32 i4ExpTimeMain,MUINT32 i4ExpTimeMain2)
{
    ...
    CAM_LOGD("[updateStereoFrameSyncFps] i4SensorDev/Dev2:%d/%d exp:%d/%d (master/slave) , Fps:%d/%d/%d/%d/%d (main/sub/main2/sub2/main3)\n",
        i4SensorDev, i4SensorDev2, i4ExpTimeMain, i4ExpTimeMain2, m_i4FrameSyncMainFps, m_i4FrameSyncSubFps, m_i4FrameSyncMain2Fps, m_i4FrameSyncSub2Fps, m_i4FrameSyncMain3Fps);
...
enableSensorI2CBuffMode(MINT32 i4SensorDev, MBOOL bEnableBuffMode)
{
    ...
    if(i4SensorDev == ESensorDev_Main) {
        ...
        CAM_LOGD("[enableSensorI2CBuffMode] SensorDev:%d EnableBuffMode:%d -> %d\n", i4SensorDev, m_bEnableBuffMode_MainThree, bEnableBuffMode);
        m_bEnableBuffMode_MainThree = bEnableBuffMode;
    } else{
        CAM_LOGE("[%s()] Sensor type error:%d", __FUNCTION__, i4SensorDev);
        return E_AAA_SENSOR_NULL;
    }
...
MUINT32 AAASensorMgr::getSensorBinningType(MINT32 i4SensorDev, NSCam::IHalSensor::ConfigParam sensorCfg)
{
    ...
    // Get default frame rate
    err = m_pHalSensorObj->sendCommand(i4SensorDev, SENSOR_CMD_GET_BINNING_TYPE, (MUINTPTR)&u4BinSumRatio, (MUINTPTR)&sensorCfg, 0);
    ...
    if(u4BinSumRatio < 1) u4BinSumRatio = 1;
