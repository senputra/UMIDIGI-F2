...
MBOOL AwbMgr::init(MINT32 i4SensorIdx, IAWBInitPara* initPara)
{
    ...
    m_i4DgbRtvEnable = property_get_int32("vendor.debug.awb.rtv.enable", 0);
    if(m_i4DgbRtvEnable)
    {
        m_i4RtvGroupId = property_get_int32("vendor.debug.awb.rtv.group_id", 0);
    }

    //m_IsSpeedAwb= property_get_int32("vendor.debug.awb_speedup.enable", 1);
    m_IsSpeedAwb = 0;
...
MBOOL AwbMgr::setSensorMode(MINT32 i4NewSensorMode, MINT32 i4BinWidth, MINT32 i4BinHeight, MINT32 i4QbinWidth, MINT32 i4QbinHeight)
{
    ...
    if ((m_eSensorMode != eNewSensorMode) || (m_i4BinW != i4BinWidth) || (m_i4BinH != i4BinHeight) || (mAaoW != i4QbinWidth) || (mAaoH != i4QbinHeight))
    {
        ...
        //for 3raw switch
        reconfigReg(mAaoW, mAaoH, &m_sAWBResultConfig);
    }
...
MRESULT AwbMgr::doPvAWB(MINT32 i4FrameCount, MBOOL bAEStable, MINT32 i4SceneLV, MVOID *aaoBuf, MUINT32 u4ExposureTime, MBOOL bApplyToHW)
{
    ...
    property_get("vendor.debug.awb_mgr.setMWB", value, "0");
    MINT32 m_setMWBCCT = atoi(value);
...
MBOOL AwbMgr::doCapAWB(MINT32 i4SceneLV, MVOID *aaoBuf, MBOOL bApplyToHW)
{
    ...
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.debug.awb_mgr.setFlashwin", value, "0");
    MINT32 m_setFlashwin = atoi(value);
...
#define SET_DBG_TAG(id, value)  rAWBDebugInfo.Tag[id].u4FieldValue = value;

    if(rAWBDebugInfo.Tag[AWB_TAG_VERSION].u4FieldValue == 0) //Save data if do not run algo.
    {
        // OTP value
        if(m_pNVRAM_3A != NULL)
        {
            SET_DBG_TAG(AWB_TAG_RAW_PREGAIN_NUM, mGainSetNum);

            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_R, m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rUnitGain.i4R);
            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_G, m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rUnitGain.i4G);
            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_B, m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rUnitGain.i4B);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_R, m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rGoldenGain.i4R);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_G, m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rGoldenGain.i4G);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_B, m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rGoldenGain.i4B);

            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_R_M, mUnitGain2.i4R);
            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_G_M, mUnitGain2.i4G);
            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_B_M, mUnitGain2.i4B);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_R_M, mGoldenGain2.i4R);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_G_M, mGoldenGain2.i4G);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_B_M, mGoldenGain2.i4B);

            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_R_L, mUnitGain3.i4R);
            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_G_L, mUnitGain3.i4G);
            SET_DBG_TAG(AWB_NVRAM_UNIT_GAIN_B_L, mUnitGain3.i4B);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_R_L, mGoldenGain3.i4R);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_G_L, mGoldenGain3.i4G);
            SET_DBG_TAG(AWB_NVRAM_GOLDEN_GAIN_B_L, mGoldenGain3.i4B);
        }
        SET_DBG_TAG(AWB_TAG_MODE, m_eAWBMode);
        SET_DBG_TAG(PWB_TAG_MWB_CCT, mMWBColorTemperature);
        SET_DBG_TAG(AWB_TAG_ALGO_SCENE_LV, m_i4SceneLV);

        // HW configuration
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_SIZE_X, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowSizeX);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_SIZE_Y, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowSizeY);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_PITCH_X, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowPitchX);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_PITCH_Y, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowPitchY);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_ORIGIN_X, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowOriginX);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_ORIGIN_Y, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowOriginY);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_NUM_X, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowNumX);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_WINDOW_NUM_Y, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4WindowNumY);
        SET_DBG_TAG(AWB_TAG_STAT_ERROR_THR, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4ErrorThreshold);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_AWBXY_WINR0, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4AWBXY_WINR[0]);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_AWBXY_WINL0, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4AWBXY_WINL[0]);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_AWBXY_WIND0, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4AWBXY_WIND[0]);
        SET_DBG_TAG(AWB_TAG_STAT_CONFIG_AWBXY_WINU0, m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4AWBXY_WINU[0]);

        // Face info
        const int u4MaxFaceIdx = 0;
        SET_DBG_TAG(AWB_TAG_FACE_NUM,  m_eAWBFDArea.u4Count);
        SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_WINR,  m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Right);
        SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_WINL,  m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Left);
        SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_WINT,  m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Top);
        SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_WINB,  m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Bottom);
        for (MINT32 i = AWB_LANDMARK_X0; i <= AWB_LANDMARK_Y1; i++)
        {
            SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_LEYE0  + (i - AWB_LANDMARK_X0), m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Landmark[AWB_LANDMARK_LEFT_EYE][i]);
            SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_REYE0  + (i - AWB_LANDMARK_X0), m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Landmark[AWB_LANDMARK_RIGHT_EYE][i]);
            SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_MOUTH0 + (i - AWB_LANDMARK_X0), m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Landmark[AWB_LANDMARK_MOUTH][i]);
        }
        for (MINT32 i = AWB_LANDMARK_UX; i <= AWB_LANDMARK_DY; i++)
        {
            SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_LEYE_UD0 + (i - AWB_LANDMARK_UX), m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Landmark[AWB_LANDMARK_LEFT_EYE][i]);
            SET_DBG_TAG(AWB_TAG_FACE_MGR_MAX_REYE_UD0 + (i - AWB_LANDMARK_UX), m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Landmark[AWB_LANDMARK_RIGHT_EYE][i]);
        }
        SET_DBG_TAG(AWB_TAG_FACE_TYPE, m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Type);
        SET_DBG_TAG(AWB_TAG_FACE_LANDMARK_CV, m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4LandmarkCV);
        SET_DBG_TAG(AWB_TAG_FACE_MOTION0, m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Motion[0]);
        SET_DBG_TAG(AWB_TAG_FACE_MOTION1, m_eAWBFDArea.rAreas[u4MaxFaceIdx].i4Motion[1]);
        SET_DBG_TAG(AWB_TAG_PORTRAIT_MW_ENABLE, m_eAWBFDArea.rPortInfo.i4Enable);
        SET_DBG_TAG(AWB_TAG_PORTRAIT_IS_VALUE, (MINT32)(m_eAWBFDArea.rPortInfo.fIsPortrait * 1000));
        SET_DBG_TAG(AWB_TAG_PORTRAIT_NO_VALUE, (MINT32)(m_eAWBFDArea.rPortInfo.fNoPortrait * 1000));
        SET_DBG_TAG(AWB_TAG_PORTRAIT_EXIST, m_eAWBFDArea.rPortInfo.i4IsPortrait);
        for(MINT32 i=0; i<m_eAWBFDArea.u4Count; i++)
        {
            if (m_eAWBFDArea.rAreas[i].i4GenderInfo == 2)
            {
                SET_DBG_TAG(AWB_TAG_GENDER_FEMALE_EXIST, 1);
                break;
            }
        }
        SET_DBG_TAG(ML_GENDER_DEBUG_INFO, m_eAWBFDArea.i4MLGenderDbgInfo);
    }
    return MTRUE;
...
*/
   ...
   if(GetCamCalData.Single2A.S2aAwb.rGainSetNum == 3)
    ...
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.debug.awb_mgr.setOTP", value, "0");
    MINT32 m_setOTP = atoi(value);

    if(m_setOTP == 1)
    {
        property_get("vendor.debug.awb_mgr.setOTPgainR", value, "0");
        m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rUnitGain.i4R = atoi(value);
        property_get("vendor.debug.awb_mgr.setOTPgainG", value, "0");
        m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rUnitGain.i4G = atoi(value);
        property_get("vendor.debug.awb_mgr.setOTPgainB", value, "0");
        m_pNVRAM_3A->AWB[m_i4AWBNvramIdx].rAlgoCalParam.rCalData.rUnitGain.i4B = atoi(value);

        property_get("vendor.debug.awb_mgr.setOTPgainR_mid", value, "0");
        mUnitGain2.i4R = atoi(value);
        property_get("vendor.debug.awb_mgr.setOTPgainG_mid", value, "0");
        mUnitGain2.i4G = atoi(value);
        property_get("vendor.debug.awb_mgr.setOTPgainB_mid", value, "0");
        mUnitGain2.i4B = atoi(value);

        property_get("vendor.debug.awb_mgr.setOTPgainR_low", value, "0");
        mUnitGain3.i4R = atoi(value);
        property_get("vendor.debug.awb_mgr.setOTPgainG_low", value, "0");
        mUnitGain3.i4G = atoi(value);
        property_get("vendor.debug.awb_mgr.setOTPgainB_low", value, "0");
...
MBOOL AwbMgr::AWBStatConfig()
{
    ...
    for (sensorMode = ESensorMode_Preview; sensorMode < ESensorMode_NUM; sensorMode++)
    {
        ...
        for (awbMode = LIB3A_AWB_MODE_OFF; awbMode < LIB3A_AWB_MODE_NUM; awbMode++)
        {
            ...
            m_rAWBStatCfg[AWB_STROBE_MODE_ON][sensorMode][awbMode].i4LinearOutputEn = m_rAWBStatParam.i4LinearOutputEn;
		...
		AWB_LOG_IF(m_i4DgbLog,"[AWBStatConfig] sensorMode = %d i4PreGainLimitR/G/B=%d/%d/%d i4LightSrcHighThresholdR/G/B=%d/%d/%d i4LightSrcLowThresholdR/G/B=%d/%d/%d i4ValidDataWidth=%d i4FormatShift=%d i4LinearOutputEn=%d i4ErrorThreshold=%d i4ErrorShiftBits=%d i4MoErrorThreshold=%d\n", sensorMode,
		...
		m_rAWBStatCfg[AWB_STROBE_MODE_ON][sensorMode][LIB3A_AWB_MODE_AUTO].i4LinearOutputEn, m_rAWBStatCfg[AWB_STROBE_MODE_ON][sensorMode][LIB3A_AWB_MODE_AUTO].i4ErrorThreshold,
...
MRESULT AwbMgr::setCamScenarioMode(MUINT32 u4NewCamScenarioMode)
{
    ...
    if( m_eCamScenarioMode != eNewCamScenarioMode) {
        ...
        IHalSensorList* const pIHalSensorList = MAKE_HalSensorList();
        if(pIHalSensorList != NULL)
            pIHalSensorList->querySensorStaticInfo(m_eSensorDev, &sInfo);
        // Get Module info
        CAMERA_CAM_CAL_TYPE_ENUM enCamCalEnum=CAMERA_CAM_CAL_DATA_MODULE_VERSION;
        result= pCamCalDrvObj->GetCamCalCalData(m_eSensorDev, enCamCalEnum, (void *)&GetCamCalData);
...
MRESULT AwbMgr::setFDInfo(MVOID* a_sFaces, MINT32 i4tgwidth, MINT32 i4tgheight)
{
    ...
    if(CUST_ENABLE_FACE_AWB() == MTRUE) {
        ...
        if(m_bFDenable == MTRUE){
            ...
            AWB_LOG_IF(m_i4DgbLog, "[%s()] m_eSensorDev:%d line:%d Face Number:%d FaceAWBChange:%d Portrait %d/%d/%d/%d", __FUNCTION__, m_eSensorDev, __LINE__, m_eAWBFDArea.u4Count, m_bFaceAWBAreaChage,
...
MRESULT AwbMgr::reconfigReg(MINT32 i4SensorWidth, MINT32 i4SensorHeight, AWBResultConfig_T *pResultConfig)
{
    mAaoW = i4SensorWidth;
    mAaoH = i4SensorHeight;
    AWB_LOG("[%s] mAaoW= %d, mAaoH= %d", __FUNCTION__,mAaoW,mAaoH);
...
MBOOL AwbMgr::getAWBStatInfo(AWB_STAT_INFO_T& rAWBStatInfo)
{
    ...
    rAWBStatInfo.i4LinearOutputEn = m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4LinearOutputEn;

    AWB_LOG_IF(m_i4DgbLog, "[%s()] winSizeX/winSizeY: %d/%d LowThreshold: %d/%d/%d HighThreshold: %d/%d/%d LinearOutputEn: %d \n", __FUNCTION__,
...
{
    m_rAWBStatCfg[AWB_STROBE_MODE_OFF][m_eSensorMode][m_eAWBMode].i4LinearOutputEn =  rAWBStatInfo.i4LinearOutputEn;
    m_rAWBStatCfg[AWB_STROBE_MODE_ON][m_eSensorMode][m_eAWBMode].i4LinearOutputEn =  rAWBStatInfo.i4LinearOutputEn;

    AWB_LOG_IF(m_i4DgbLog, "[%s()] i4LinearOutputEn: %d \n", __FUNCTION__,
               m_rAWBStatCfg[m_i4StrobeMode][m_eSensorMode][m_eAWBMode].i4LinearOutputEn);
