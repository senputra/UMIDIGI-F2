 ...
 ************************************************************************************************/
#ifndef _AE_FLOW_CUS_H_
#define _AE_FLOW_CUS_H_
...
namespace NS3Av3
{
class AeFlowCustom : public IAeFlow
{
public:
    ...
    virtual MVOID      pause() {}
    virtual MVOID      stop();
    virtual MBOOL      queryStatus(MUINT32 index) {return MTRUE;}
    ...
    virtual MVOID      controlCCU(AE_FLOW_CCU_CONTROL_INFO_T* pcontrolInfo){(void)pcontrolInfo;}
                       ...
                       AeFlowCustom(ESensorDev_T sensor);
    virtual            ~AeFlowCustom();

private:

    MBOOL IsCCUWorking();
    MBOOL IsCCUAEWorking();
    MBOOL getCCUStat(CCU_AE_STAT* poutput);
