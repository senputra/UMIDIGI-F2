...
MVOID
LscMgrDefault::
CCTOPGetSdblkFileCfg(char *output)
{
    m_pTsf->getDumpFileCfg(output);
}
...
getDebugInfo(SHADING_DEBUG_INFO_T &rShadingDbgInfo)
{
    ...
    if(m_pCurrentBuf!=NULL)
    {
        ILscBuf::Config rCfg = m_pCurrentBuf->getConfig();
        MUINT32 u4Addr = m_pCurrentBuf->getPhyAddr();
        MUINT32 u4Rto = m_u4Rto;

        debug.ctl1.val                        = 0x30400000;
        debug.ctl2.bits.LSC_SDBLK_XNUM        = rCfg.i4BlkX;
        debug.ctl3.bits.LSC_SDBLK_YNUM        = rCfg.i4BlkY;
        debug.ctl2.bits.LSC_SDBLK_WIDTH       = rCfg.i4BlkW;
        debug.ctl3.bits.LSC_SDBLK_HEIGHT      = rCfg.i4BlkH;
        debug.lblock.bits.LSC_SDBLK_lWIDTH    = rCfg.i4BlkLastW;
        debug.lblock.bits.LSC_SDBLK_lHEIGHT   = rCfg.i4BlkLastH;
        debug.ratio_0.val                     = (u4Rto<<24)|(u4Rto<<16)|(u4Rto<<8)|u4Rto;
        ...
        setDebugTag(rShadingDbgInfo, SHAD_TAG_CAM_LSC_GAIN_TH, (MUINT32)0);
...
MUINT32
LscMgrDefault::
getCurTsfoIndex() const
{
    return m_u4CurrTsfBufIdx;
}
...
MINT32
LscMgrDefault::
getCorresTsfo(MUINT32 u4CurrentBufIdx, android::Vector<MUINT8>& rData)
{
    MINT32 ret = 0;

    if((u4CurrentBufIdx < RING_BUF_NUM) && m_TsfoBufRing[u4CurrentBufIdx].size()!=0)
    {
        std::vector<MUINT8> &tsfoData = m_TsfoBufRing[u4CurrentBufIdx];

        rData.resize(tsfoData.size());
        ::memcpy(rData.editArray(), &tsfoData[0], tsfoData.size());
    }
    else
    {
        ...
        if(fgLogEn)
        {
            if(u4CurrentBufIdx < RING_BUF_NUM && u4CurrentBufIdx >= 0)
            {
                CAM_LOGD("dummy frame, no TSF data, abnormal idx %d, or wrong size %d",
                    u4CurrentBufIdx, m_TsfoBufRing[u4CurrentBufIdx].size());
            }
            else
            {
                CAM_LOGD("dummy frame, no TSF data, abnormal idx %d", u4CurrentBufIdx);
            }
        }
        //gen dummy data
