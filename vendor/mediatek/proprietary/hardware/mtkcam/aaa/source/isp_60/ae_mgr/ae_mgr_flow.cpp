...
MRESULT AeMgr::prepareAlgoFrameInfo(AE_CORE_CTRL_CCU_VSYNC_INFO* pInfo, MBOOL is_preset)
{
    /* assign AE mode info */
    ...
    pInfo->eAEComp               = (is_preset) ? (CCU_LIB3A_AE_EVCOMP_T) m_eAEEVcompPreset : (CCU_LIB3A_AE_EVCOMP_T) m_eAEEVcomp;
    ...
    AE_LOG_IF(m_3ALogEnable,"[%s] eAEScene: %d, eAECamMode: %d, eAEMeteringMode: %d, eAEComp: %d, eAEFlickerMode: %d, eAEAutoFlickerMode: %d is_preset:%d\n",
              __FUNCTION__, pInfo->eAEScene, pInfo->eAECamMode, pInfo->eAEMeteringMode, pInfo->eAEComp, pInfo->eAEFlickerMode, pInfo->eAEAutoFlickerMode, is_preset);
    ...
    pInfo->AECtrl.bIsCalculate   = (is_preset) ? !(m_bSkipCalAE || m_bFDSkipCalAE || !m_bEnableAE || m_u4SkipCalCnt || m_u1IsDummyFramePreset) : !(m_bSkipCalAE || m_bFDSkipCalAE || !m_bEnableAE || m_u4SkipCalCnt || m_u1IsDummyFrameSet);
    ...
    if (m_u4SkipCalCnt > 0) {
        AE_LOG("[%s] skip AE calculation, m_u4SkipCalCnt: %d\n", __FUNCTION__, m_u4SkipCalCnt);
        m_u4SkipCalCnt--;
    }
    AE_LOG_IF(m_3ALogEnable,"[%s] u4ReqNum: %d, eAEState: %d, u4CycleCnt: %d, bIsCalculate: %d, bIsAPLock/bIsAFLock: %d/%d, m_bSkipCalAE: %d, m_bFDSkipCalAE: %d, m_eAEMode: %d, m_bEnableAE: %d, m_u4SkipCalCnt: %d\n",
              __FUNCTION__, pInfo->camReqNumber, pInfo->AECtrl.eAEState, pInfo->u4CycleCnt, pInfo->AECtrl.bIsCalculate, pInfo->AECtrl.bIsAPLock, pInfo->AECtrl.bIsAFLock, m_bSkipCalAE, m_bFDSkipCalAE, m_eAEMode, m_bEnableAE, m_u4SkipCalCnt);

    AE_LOG("[%s()] SensorDev:%d bIsCalculate:%d is_preset:%d IsDummyFrame(Preset/Set):%d/%d\n", __FUNCTION__, m_eSensorDev, pInfo->AECtrl.bIsCalculate, is_preset, m_u1IsDummyFramePreset, m_u1IsDummyFrameSet);
...
MRESULT AeMgr::copyAlgoResult2Mgr(AE_CORE_MAIN_OUT* pAlgoOutput)
{
    ...
    {
        // skip update expousre setting when it is dummy frame
        // ex: hdr capture, 0-+EV may appear dummy frame between -EV and +EV which will influence 0 EV base Exposure setting
        ...
        {
            AE_LOG("[%s()] SensorDev:%d IsDummyFrame:%d, skip update expousre setting\n", __FUNCTION__, m_eSensorDev, m_u1IsDummyFrameSet);
        }
        ...
        if (m_rAEAlgoOutput.u4AEFinerEVIdxBase != m_u4FinerEVIdxBase)
        {
            // to overwrite incorrect finerEV indexF, in order to handle CCU deliver latency as p-line interpolation
...
MRESULT AeMgr::prepareCCUControlInfo(AE_FLOW_CCU_CONTROL_INFO_T* pInfo)
{
    ...
    {
        UpdateGyroInfo();
        prepareAlgoFrameInfo(&(pInfo->mFrameData), MTRUE);
