...
{
    auto i4SensorIdx = NS3Av3::mapSensorDevToIdx(sensorDev);
    ...
    std::call_once(rSingleton.onceFlag, [&] {
        rSingleton.instance = std::make_unique<AfMgr>(sensorDev);
    } );

    // CAM_LOGD("[%s] SensorDev(%d), SensorIdx(%d) instance(%p)\n", __FUNCTION__, sensorDev, i4SensorIdx, &(*(rSingleton.instance)));
    ...
    if (!err)
    {
        ...
        if ( m_pMcuDrv == NULL)
        {
            ...
            if (m_i4CurrModuleId == -1)
            {
                ...
                if (i4SensorDevID != -1)
                {
                    MINT32 ret = pCamCalDrvObj->GetCamCalCalDataV2(i4SensorDevID, eCamCalDataType, &partnum_data, sizeof(CAM_CAL_PART_NUM_STRUCT));
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MINT32 AfMgr::reconfig(ConfigInputData_T data)
{
    CAM_LOGD( "AF-%-15s: + Dev %d", __FUNCTION__, m_i4CurrSensorDev);

    MINT32 err = 0;

    if ( m_i4DgbDisableAF || (getAfSupport() == 0) )
    {
        err = 1;
    }

    CAM_LOGD("%s: isSwitchDone(%d), isAFSupport(%d)", __FUNCTION__, data.isSwitchDone, !err);

    if (s_pIAfCxU)
    {
        if (s_pIAfCxU->isCCUAF() && data.isSwitchDone == 0)
        {
            // CCUAF && hwSwitch NOT Done ==> only stop af
            if (!err)
            {
                s_pIAfCxU->abort();
                CAM_LOGD("%s: reconfig-abort AF before stop", __FUNCTION__);
        ...
        else
        {
            ...
            {
                s_pIAfCxU->abort();
                CAM_LOGD("%s: reconfig-abort AF before stop/uninit", __FUNCTION__);
                s_pIAfCxU->stop(m_sCurLensInfo);
                s_pIAfCxU->uninit();
            }

            // [TG/BIN size] get isp info
            if (!err)
            {
                // Get sensor information :
                // TG/BIN size
                m_i4TGSzW  = data.tgSzW;
                m_i4TGSzH  = data.tgSzH;
                m_i4BINSzW = data.binSzW;
                m_i4BINSzH = data.binSzH;
                CAM_LOGD( "AF-%-15s: Dev(0x%04x), TGSZ: W %d, H %d, BINSZ: W %d, H %d",
                          __FUNCTION__,
                          m_i4CurrSensorDev,
                          m_i4TGSzW,
            ...
            if (!err)
            {
                ...
                m_sDoAFOutput.afOutput.i4AFPos = m_i4InitPos;
            }
...
MINT32 AfMgr::start() // after start preview
{
    ...
    if (!err)
    {
        ...
        // To get m_bEnablePD before start()::setCropRegion to get the collect centerROI.
    ...
    if (!err)
    {
        ...
        m_sPDRois[eIDX_ROI_ARRAY_AP].info.sPDROI = m_sArea_Center;
        // LensInfo
...
MINT32 AfMgr::process(AFInputData_T data, AFCommand_T command)
{
    ...
    // To update StaMagicNum & m_u4ReqMagicNum
    m_u4ReqMagicNum = command.requestNum;
    if (m_i4IsCCUAF)
    {
        m_u4StaMagicNum = (m_u4ReqMagicNum > 2) ? (m_u4ReqMagicNum - 2) : 0;
        data.ptrAFStt = NULL;
    }
    else
    {
        if (data.ptrAFStt != NULL)
        {
            StatisticBufInfo *ptrStatInfo = reinterpret_cast<StatisticBufInfo *>(data.ptrAFStt);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MINT32 AfMgr::getAfSupport()
{
    MINT32 isAfSupport = 0;
    // with VCM ==> TRUE
    // else     ==> FALSE
    if (m_i4DgbDisableAF)
    {
    ...
    else if (m_i4LensSupport == -1)
    {
        ...
        m_i4LensSupport = pMcuDrv->isLensSupport(rSensorStaticInfo.sensorDevID) == 0 ? 0 : AF_WIN_NUM_SPOT;
    }
    isAfSupport = (m_i4LensSupport > 0) ? AF_WIN_NUM_SPOT : 0;
...
MRESULT AfMgr::setAFMode( MINT32 eAFMode, MUINT32 u4Caller)
{
    ...
    {
        if (m_eCurAFMode != MTK_CONTROL_AF_MODE_EDOF)
            return S_AF_OK;
    }
...
E_AF_STATE_T AfMgr::getAFState()
{
    ...
    if (AP_NOT_SHOW_FDROI_AUTO && m_i4IsCAFWithoutFace == 1 && ret == E_AF_PASSIVE_SCAN && m_sAFOutput->i4ROISel == AF_ROI_SEL_FD)
    {
...
MINT32 AfMgr::getMaxLensPos()
{
    ...
    CAM_LOGD_IF( LEVEL_FLOW, "[%s] EnableAF(%d), MCU (%p), MaxLensPos(%d)", __FUNCTION__, m_i4EnableAF, (void*)m_pMcuDrv, maxLensPos);
...
MINT32 AfMgr::getMinLensPos()
{
    ...
    CAM_LOGD_IF( LEVEL_FLOW, "[%s] EnableAF(%d), MCU (%p), MinLensPos(%d)", __FUNCTION__, m_i4EnableAF, (void*)m_pMcuDrv, minLensPos);

    return minLensPos;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// obsolete function for HAL1 only
MRESULT AfMgr::setMFPos( MINT32 a_i4Pos, MUINT32 u4Caller)
{
            ...
            (a_i4Pos <= 1023))
    {
        ...
        {
            CAM_LOGD( "sync-%s set MF pos (%d)->(%d)",
                      __FUNCTION__,
                      m_sAFOutput->i4AFPos,
                      a_i4Pos);
        }
        ...
        m_sDoAFInput.algoCommand.mfPos = m_i4MFPos;
        m_sDoAFInput.algoCommand.bTrigger = MTRUE;
    }
    else
    {
        if ( u4Caller == AF_MGR_CALLER)
        {
            CAM_LOGD_IF( LEVEL_FLOW, "%s skip set MF pos (%d)->(%d), lib_afmode(%d)",
                         __FUNCTION__,
                         m_sAFOutput->i4AFPos,
                         a_i4Pos,
                         m_eLIB3A_AFMode);
        ...
        {
            CAM_LOGD_IF( LEVEL_FLOW, "cmd-%s skip set MF pos (%d)->(%d), lib_afmode(%d)",
                         __FUNCTION__,
                         m_sAFOutput->i4AFPos,
                         a_i4Pos,
                         m_eLIB3A_AFMode);
        ...
        {
            CAM_LOGD_IF( LEVEL_FLOW, "sync-%s skip set MF pos (%d)->(%d), lib_afmode(%d)",
                         __FUNCTION__,
                         m_sAFOutput->i4AFPos,
                         a_i4Pos,
                         m_eLIB3A_AFMode);
...
{
    MINT32 ret = 1;
    CAM_LOGD_IF(LEVEL_FLOW, "%s, m_eAFState=%d", __FUNCTION__, m_eAFState);
...
{
    Mutex::Autolock lock(m_DataLock);
...
MVOID AfMgr::getFocusRange( MFLOAT *vnear, MFLOAT *vfar)
{
    ...
    if ( m_i4EnableAF == 0)
    {
        CAM_LOGD_IF(LEVEL_FLOW, "no focus lens");
        *vnear = 1 / 0.6;
        *vfar  = 1 / 3.0;
        return;
    }
