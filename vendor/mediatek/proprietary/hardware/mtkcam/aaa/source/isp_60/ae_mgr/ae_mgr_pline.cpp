...
*
********************************************************************************/
MRESULT AeMgr::setAEPlineIdx(MINT32 i4ManualPlineIdx, MINT32 i4CustomPlineMode)
{
    if(m_i4ManualPlineIdx != i4ManualPlineIdx)
    {
        strAETable strCurrentCaptureTable;
        MUINT32 u4AIS2ExtRatio = 100;
        strAERealSetting rAESettingCapture; // AE capture setting

        if(i4CustomPlineMode){  // 1 mean to do not equal setting calculate
            getOnePlineTable((eAETableID)i4ManualPlineIdx, strCurrentCaptureTable);
            if((m_i4AIS4CWR != 0) && (m_i4AIS4CWV != 0))
            {
                m_rAEInfoPreview.u4AECurrentTarget = m_i4AIS4CWR;
                m_rAEInfoPreview.u4MgrCWValue = m_i4AIS4CWV;
            }

            m_pIAeCalc->recalcAEExpInfo_v4p0(&m_rAEInfoPreview, (MUINT32 *)Log2x1000, &rAESettingCapture, &m_rAEAlgoOutput, &strCurrentCaptureTable, m_pPreviewTableCurrent, &(m_pAEPlineTable->AEGainList));
            u4AIS2ExtRatio = ((MINT64) 100* rAESettingCapture.EvSetting.u4Eposuretime*rAESettingCapture.u4ISO)/m_rAEAlgoOutput.EvSetting.u4Eposuretime/m_rAEAlgoOutput.u4ISO;
            m_i4AIS4Idx = GetDeltaEVIndex(u4AIS2ExtRatio);

            AE_LOG("[%s()] m_i4SensorDev:%d input setting:%d/%d/%d(%d) output setting:%d/%d/%d(%d) u4AIS2ExtRatio:%d i4AIS4Idx:%d CWR:%d CWV:%d\n",
               __FUNCTION__, m_eSensorDev, m_rAEAlgoOutput.EvSetting.u4Eposuretime, m_rAEAlgoOutput.EvSetting.u4AfeGain, m_rAEAlgoOutput.EvSetting.u4IspGain, m_rAEAlgoOutput.u4ISO,
               rAESettingCapture.EvSetting.u4Eposuretime, rAESettingCapture.EvSetting.u4AfeGain, rAESettingCapture.EvSetting.u4IspGain, rAESettingCapture.u4ISO, u4AIS2ExtRatio, m_i4AIS4Idx, m_rAEInfoPreview.u4AECurrentTarget, m_rAEInfoPreview.u4MgrCWValue);
        } else {
            m_i4AIS4Idx = 0;
        }

        AE_LOG("[%s()] Change AE Pline index %d->%d m_i4AIS4Idx:%d i4CustomPlineMode\n",__FUNCTION__, m_i4ManualPlineIdx, i4ManualPlineIdx, m_i4AIS4Idx, i4CustomPlineMode);
        m_i4ManualPlineIdx = i4ManualPlineIdx;
    }
    return S_AE_OK;
}
...
MINT32 AeMgr::GetDeltaEVIndex(MUINT32 u4ReduceRatio)
{
    MINT32 i4Idx = 0;
