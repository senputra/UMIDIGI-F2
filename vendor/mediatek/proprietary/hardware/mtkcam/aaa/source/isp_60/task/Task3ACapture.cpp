...
Task3ACaptureImp::run(MINT32 i4Opt, const TaskData& rData)
{
    ...
    if(getStatus() == TASK_STATUS_INIT)
    {
        ...
        rLscInputInfo.isFlash  = bFlashOnOff;
        ...
        rLscInputInfo.i4AEComp = AEPerframeInfo.rAEISPInfo.i4AEComp;

        //force unlock shading ratio
        MINT32 forceUnlock = 2;
        pLsc->evaluate_cus_Ratio(rLscInputInfo, forceUnlock);
        m_rTimer.End();
        ...
        if(!rData.bIsHqCap)
        {
            ...
            if(pAAOBuffer)
            {
                ...
                rLscInputInfo.bAELimiter = AEPerframeInfo.rAEUpdateInfo.bAELimiter;
                rLscInputInfo.i4AEComp = AEPerframeInfo.rAEISPInfo.i4AEComp;
                pLsc->evaluate_cus_Ratio(rLscInputInfo, MFALSE);
