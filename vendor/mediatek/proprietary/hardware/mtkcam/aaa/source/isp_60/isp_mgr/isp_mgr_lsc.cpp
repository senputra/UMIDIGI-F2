...
#define MY_INST NS3Av3 ::INST_T<ISP_MGR_LSC_T>
...
{
    CAM_LOGD_IF(m_bDebugEnable, "[%s] SubModuleIndex %d, eSensorDev(%d), u4BaseAddr(0x%08x)", __FUNCTION__, SubModuleIndex, m_eSensorDev, u4BaseAddr);
...
apply_P1(MUINT8 SubModuleIndex, const RAWIspCamInfo& rRawIspCamInfo, TuningMgr& rTuning, MINT32 i4SubsampleIdex)
{
    ...
    TUNING_MGR_WRITE_BITS_CAM((&rTuning), CAMCTL_R1_CAMCTL_DMA_EN , CAMCTL_LSCI_R1_EN , fgOnOff, i4SubsampleIdex);
    ...
    rTuning.tuningMgrWriteRegs(
        (TUNING_MGR_REG_IO_STRUCT*)(&m_rIspRegInfo[SubModuleIndex][0]),
