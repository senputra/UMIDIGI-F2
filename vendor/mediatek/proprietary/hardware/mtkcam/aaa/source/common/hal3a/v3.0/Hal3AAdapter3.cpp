...
createInstance(MINT32 const i4SensorIdx, const char* strUser)
{
    ...
    case 4:
        {
            static Hal3AAdapter3 _singleton(4);
            _singleton.init(strUser);
            return &_singleton;
        }
    default:
...
Hal3AAdapter3(MINT32 const i4SensorIdx)
    ...
    , mu1Override3A(OVERRIDE_AE|OVERRIDE_AWB|OVERRIDE_AF)
    , mu1AfTrigSMVR(MTK_CONTROL_AF_TRIGGER_IDLE)
    , mu1IspProfile(0xFF)
    , m_i4IsByPassCal(0)
{
...
config(const ConfigInfo_T& rConfigInfo)
{
    ...
    m_rConfigInfo = rConfigInfo;
    {
        ...
        {
            MY_LOGD("[%s] SMVR 3A - config Capacity", __FUNCTION__);
    ...
    //This is for SMVR change buffer size
    ...
    MY_LOGD("[%s] Req(#%d) setHistory/getValidateMetadata", __FUNCTION__, ConfigMagic);
    if (mpResultPoolObj)
    {
        mpResultPoolObj->setHistory(ConfigMagic, HistorySize);
        MBOOL ret = mpResultPoolObj->findValidateBuffer(ConfigMagic);
    }

    MINT32 i4Ret = mpHal3aObj->config(rConfigInfo);
    MY_LOGD("[%s]- mMapMat size(%zu)", __FUNCTION__, mMapMat.size()); // unsigned long int
    return i4Ret;
}
...
init(const char* strUser)
{
    ...
    if (m_Users.size())
    {
        MY_LOGD("[%s] sensorDev(%d), sensorIdx(%d), Still %zu users", __FUNCTION__, mu4SensorDev, mi4SensorIdx, m_Users.size());
    }
    else
    {
        doInit();
...
uninit(const char* strUser)
{
    ...
    if (!m_Users[string(strUser)])  CAM_LOGE("User(%s) did not create 3A!", strUser);
    else
    {
        m_Users[string(strUser)]--;
        if (!m_Users[string(strUser)])
        {
            ...
            else    MY_LOGD("[%s] sensorDev(%d), sensorIdx(%d), Still %zu users", __FUNCTION__, mu4SensorDev, mi4SensorIdx, m_Users.size());
        }
    }
...
MINT32
Hal3AAdapter3::
startRequestQ(const list<MetaSet_T>& requestQ, MINT32 i4RequestQSize)
{
    ...
    for (MINT32 i = 0; i < requestQ.size(); i++)
    {
        ...
        else
        {
            ...
            if(ret)
            {
                ...
                rRequestInfo.i4ReqMagic = requestQ[i]->MagicNum;
                rRequestInfo.i4StatisticMagic = InvalidValue;
                ret = mpResultPoolObj->setRequestInfo(rRequestInfo);

                AllMetaResult_T* pMetadata = mpResultPoolObj->getMetadata(requestQ[i]->MagicNum, __FUNCTION__);
                ...
                mpResultPoolObj->returnMetadata(pMetadata, __FUNCTION__);
            }
            else
...
parseMeta(const vector<MetaSet_T*>& requestQ)
{
    ...
    QUERY_ENTRY_SINGLE(_halmeta, MTK_3A_ISP_PROFILE, mu1IspProfile);
    ...
    MY_LOGD_IF(fgLogEn0,"[%s] RepeatTag(%d), AeIsoSpeedMode(%d), IspProfile(%d), RawType(%d), DisableP1(%d), IsSingleFrameHDR(%d), MtkHdrMode(%d), MWBColorTemperature(%d), DummyAfterCapture(%d)",
                        __FUNCTION__, u1RepeatTag, mParams.i4IsoSpeedMode, mu1IspProfile, mParams.i4RawType, mParams.i4DisableP1,
    ...
    if(mi4AppfgCrop)
    {
        ...
        if( QUERY_ENTRY_SINGLE(_halmeta, MTK_DUALZOOM_ZOOMRATIO, i4ZoomRatio))
        {
            ...
            {
                MY_LOGD_IF(fgLogEn0, "[%s] ActiveCropRgn(%d,%d,%d,%d) PrvCropRegion(%d,%d,%d,%d)", __FUNCTION__, ActiveArrayCropRegionSize.p.x, ActiveArrayCropRegionSize.p.y, ActiveArrayCropRegionSize.s.w, ActiveArrayCropRegionSize.s.h,
                                     mPrvCropRegion.p.x, mPrvCropRegion.p.y, mPrvCropRegion.s.w, mPrvCropRegion.s.h);
                CameraArea_T ActiveArrayCropRegionSize2TG;
                ActiveArrayCropRegionSize2TG.i4Left   = ActiveArrayCropRegionSize.p.x;
                ActiveArrayCropRegionSize2TG.i4Top    = ActiveArrayCropRegionSize.p.y;
                ActiveArrayCropRegionSize2TG.i4Right  = ActiveArrayCropRegionSize.p.x + ActiveArrayCropRegionSize.s.w;
                ActiveArrayCropRegionSize2TG.i4Bottom = ActiveArrayCropRegionSize.p.y + ActiveArrayCropRegionSize.s.h;
                ActiveArrayCropRegionSize2TG = _transformArea(mi4SensorIdx, mi4SensorMode, ActiveArrayCropRegionSize2TG);
                ActiveArrayCropRegionSize2TG = _clipArea(i4TgWidth, i4TgHeight, ActiveArrayCropRegionSize2TG);

                MY_LOGD_IF(fgLogEn0, "[%s] ActiveCropRgn2TG(%d,%d,%d,%d)", __FUNCTION__, ActiveArrayCropRegionSize2TG.i4Left, ActiveArrayCropRegionSize2TG.i4Top, ActiveArrayCropRegionSize2TG.i4Right, ActiveArrayCropRegionSize2TG.i4Bottom);

                MINT32 i4X = ActiveArrayCropRegionSize2TG.i4Left;
                MINT32 i4Y = ActiveArrayCropRegionSize2TG.i4Top;
                MINT32 i4W = ActiveArrayCropRegionSize2TG.i4Right - ActiveArrayCropRegionSize2TG.i4Left;
                MINT32 i4H = ActiveArrayCropRegionSize2TG.i4Bottom - ActiveArrayCropRegionSize2TG.i4Top;
                i4ZoomRatio = (((i4W*100)/mPrvCropRegion.s.w) < ((i4H*100)/mPrvCropRegion.s.h)) ? ((i4W*100)/mPrvCropRegion.s.w) : ((i4H*100)/mPrvCropRegion.s.h);

                MY_LOGD_IF(fgLogEn0, "[%s] i4ZoomRatio(%d)", __FUNCTION__, i4ZoomRatio);
            }
            else
            {
                MY_LOGD_IF(fgLogEn0, "[%s] ActiveRgn(%d,%d,%d,%d) PrvCropRegion(%d,%d,%d,%d)", __FUNCTION__, mActiveArraySize.p.x, mActiveArraySize.p.y, mActiveArraySize.s.w, mActiveArraySize.s.h,
                                     mPrvCropRegion.p.x, mPrvCropRegion.p.y, mPrvCropRegion.s.w, mPrvCropRegion.s.h);
                CameraArea_T ActiveArraySize2TG;
                ActiveArraySize2TG.i4Left   = mActiveArraySize.p.x;
                ActiveArraySize2TG.i4Top    = mActiveArraySize.p.y;
                ActiveArraySize2TG.i4Right  = mActiveArraySize.p.x + mActiveArraySize.s.w;
                ActiveArraySize2TG.i4Bottom = mActiveArraySize.p.y + mActiveArraySize.s.h;
                ActiveArraySize2TG = _transformArea(mi4SensorIdx, mi4SensorMode, ActiveArraySize2TG);
                ActiveArraySize2TG = _clipArea(i4TgWidth, i4TgHeight, ActiveArraySize2TG);

                MY_LOGD_IF(fgLogEn0, "[%s] ActiveRgn2TG(%d,%d,%d,%d)", __FUNCTION__, ActiveArraySize2TG.i4Left, ActiveArraySize2TG.i4Top, ActiveArraySize2TG.i4Right, ActiveArraySize2TG.i4Bottom);

                MINT32 i4X = ActiveArraySize2TG.i4Left;
                MINT32 i4Y = ActiveArraySize2TG.i4Top;
                MINT32 i4W = ActiveArraySize2TG.i4Right - ActiveArraySize2TG.i4Left;
                MINT32 i4H = ActiveArraySize2TG.i4Bottom - ActiveArraySize2TG.i4Top;
                i4ZoomRatio = (((i4W*100)/mPrvCropRegion.s.w) < ((i4H*100)/mPrvCropRegion.s.h)) ? ((i4W*100)/mPrvCropRegion.s.w) : ((i4H*100)/mPrvCropRegion.s.h);
            ...
            case MTK_CONTROL_AWB_MODE:  // dynamic
                {
                    ...
                    if(mbEnableOverride)
                        mu1Override3A &= (~OVERRIDE_AWB);
                }
                break;
            case MTK_CONTROL_AWB_REGIONS:
                {
                    if ((m_rStaticInfo.i4MaxRegionAwb == 0) || (mu1AfTrigSMVR == MTK_CONTROL_AF_TRIGGER_START))
                    {
                        if ((mAfParams.u1AfTrig != MTK_CONTROL_AF_TRIGGER_IDLE))
                            MY_LOGD_IF(fgLogEn1, "[%s] MTK_CONTROL_AWB_REGIONS u1AfTrig(%d)", __FUNCTION__, mAfParams.u1AfTrig);
                    }
                    else if (m_rStaticInfo.i4MaxRegionAwb != 0)
                    {
            ...
            case MTK_CONTROL_AE_MODE:
                {
                    ...
                    if(mbEnableOverride)
                        mu1Override3A &= (~OVERRIDE_AE);
                }
                break;
            case MTK_CONTROL_AE_REGIONS:    // dynamic
                {
                    if ((m_rStaticInfo.i4MaxRegionAe == 0) || (mu1AfTrigSMVR == MTK_CONTROL_AF_TRIGGER_START))
                    {
                        if ((mAfParams.u1AfTrig != MTK_CONTROL_AF_TRIGGER_IDLE))
                            MY_LOGD_IF(fgLogEn1, "[%s] MTK_CONTROL_AE_REGIONS u1AfTrig(%d)", __FUNCTION__, mAfParams.u1AfTrig);
                    }
                    else if (m_rStaticInfo.i4MaxRegionAe != 0)
                    {
            ...
            case MTK_CONTROL_AF_MODE:
                {
                    ...
                    if(mbEnableOverride)
                        mu1Override3A &= (~OVERRIDE_AF);
                }
                break;
            case MTK_CONTROL_AF_REGIONS:    // dynamic
                {
                    if ((m_rStaticInfo.i4MaxRegionAf == 0) || (mu1AfTrigSMVR == MTK_CONTROL_AF_TRIGGER_START))
                    {
                        if ((mAfParams.u1AfTrig != MTK_CONTROL_AF_TRIGGER_IDLE))
                            MY_LOGD_IF(fgLogEn1, "[%s] MTK_CONTROL_AF_REGIONS u1AfTrig(%d)", __FUNCTION__, mAfParams.u1AfTrig);
                    }
                    else if (m_rStaticInfo.i4MaxRegionAf != 0)
                    {
    ...
    {
        mu1Override3A = 0;       // reduce AWB,AF,AE default value
    }
...
setNormal(const vector<MetaSet_T*>& requestQ)
{
    ...
    MINT32 i4ReqPreSetKey = -1;
    mu1Override3A = (OVERRIDE_AE|OVERRIDE_AWB|OVERRIDE_AF);

    //This control parse Multi-metadata
    for(MINT32 i = 0; i < requestQ.size(); i++)
    {
        ...
        MY_LOGD_IF(fgLog, "[%s] with Presetkey %d %d, MagicNum %d, FrameNum %d, Dummy(%d)", __FUNCTION__, requestQ[i]->PreSetKey, m_i4Presetkey, requestQ[i]->MagicNum, requestQ[i]->FrameNum, requestQ[i]->Dummy);

        rConvertMagic4SMVR[(i % m_rConfigInfo.i4SubsampleCount)] = i4FrmId;

        if(i == 0)
        {
            i4ReqPreSetKey = requestQ[i]->PreSetKey;
        ...
        else if(it->Dummy || mParams.bDummyBeforeCapture || mParams.bDummyAfterCapture)
        {
            ...
            MY_LOGD("[%s] Req(#%d) FrameNum(#%d) Dummy(%d) Sync2AMode(%d) SyncAF(%d) HwSync(%d) Sync3ASwitchOn(%d)", __FUNCTION__, i4FrmId, i4FrameNum, it->Dummy, mStereoParam.i4Sync2AMode, mStereoParam.i4SyncAFMode, mStereoParam.i4HwSyncMode, mStereoParam.i4Sync3ASwitchOn);
#endif
        ...
        MY_LOGD_IF(mu4LogEn & HAL3AADAPTER3_LOG_SET_0, "[%s] Req(#%d) setHistory/getValidateMetadata/clearOldestResultPool", __FUNCTION__, i4FrmId);
        mpResultPoolObj->setHistory(i4FrmId, HistorySize);
        ...
        REQUEST_INFO_T rRequestInfo;
        rRequestInfo.i4ReqMagic = i4FrmId;
        rRequestInfo.i4PreSetKey = m_i4Presetkey;
        ret = mpResultPoolObj->setRequestInfo(rRequestInfo);

        // Result - 4. Assign metadata data
        AllMetaResult_T* pMetadata = mpResultPoolObj->getMetadata(i4FrmId, __FUNCTION__);
        if(pMetadata != NULL)
        {
            ...
            if (!mu1RepeatTag)
            {
                ...
                if(mAfParams.u1AfTrig == MTK_CONTROL_AF_TRIGGER_START)
                    mu1AfTrigSMVR = MTK_CONTROL_AF_TRIGGER_START;
    ...
    {
        AllResult_T* pAllResult = mpResultPoolObj->getResultByReqNum(rConvertMagic4SMVR[i], __FUNCTION__);
        if(pAllResult != NULL)
            ::memcpy(pAllResult->rOld3AInfo.i4ConvertMagic, rConvertMagic4SMVR, sizeof(rConvertMagic4SMVR));
        mpResultPoolObj->returnResult(pAllResult, __FUNCTION__);
    }
        ...
        default:
            {
                if (!m_ScnModeOvrd.empty())
                {
                    ...
                    mParams.u4AeMode  = (mu1Override3A & OVERRIDE_AE) ? rOverride.u1AeMode : mParams.u4AeMode;
                    mParams.u4AwbMode = (mu1Override3A & OVERRIDE_AWB) ? rOverride.u1AwbMode : mParams.u4AwbMode;
                    mAfParams.u4AfMode  = (mu1Override3A & OVERRIDE_AF) ? rOverride.u1AfMode : mAfParams.u4AfMode; //_convertAFMode(rOverride.u1AfMode);
    ...
    if (mu1IspProfile == 0xFF){
        ...
        NSIspTuning::EIspProfile_T eIspProfile = static_cast<NSIspTuning::EIspProfile_T>(mu1IspProfile);
    ...
    MY_LOGD_IF(mu4LogEn, "[%s] Req(#%d) mu1FdEnable(%d) mParams.bIsFDReady(%d)", __FUNCTION__, mParams.i4MagicNum, mu1FdEnable, mParams.bIsFDReady);
    ...
    if(i4ReqPreSetKey != m_i4Presetkey)
    {
...
get(MUINT32 frmId, MetaSet_T& result)
{
    ...
    {
        if(mSubSampleCount4Clear == 0)
        {
            //===== One result to update 4 metadata =====
            AllResult_T *pAllResult4ConReq = mpResultPoolObj->getResultByReqNum(frmId, __FUNCTION__);
            MINT32 rConvertMagic4SMVR[ConvertNum4SMVR] = {0,0,0,0};
            if(pAllResult4ConReq != NULL)
                ::memcpy(rConvertMagic4SMVR, pAllResult4ConReq->rOld3AInfo.i4ConvertMagic, sizeof(rConvertMagic4SMVR));
            mpResultPoolObj->returnResult(pAllResult4ConReq, __FUNCTION__);

            MY_LOGD_IF(fgLog, "[%s] One result(#%d) to update 4 metadata(#%d, #%d, #%d, #%d)"
                        , __FUNCTION__, rConvertMagic4SMVR[0], rConvertMagic4SMVR[0], rConvertMagic4SMVR[1], rConvertMagic4SMVR[2], rConvertMagic4SMVR[3]);

            for(MINT32 i=0; i < m_rConfigInfo.i4SubsampleCount; i++)
            {
                AllResult_T *pAllResult = mpResultPoolObj->getResultByReqNum(rConvertMagic4SMVR[0], __FUNCTION__);
                AllMetaResult_T* pMetaResult = mpResultPoolObj->getMetadata(rConvertMagic4SMVR[i], __FUNCTION__);
                ...
                if(i4ConvertType != 0)
                    mpResultPoolObj->convertToMetadataFlow(rConvertMagic4SMVR[0], i4ConvertType, pAllResult, pMetaResult);

                mpResultPoolObj->returnMetadata(pMetaResult, __FUNCTION__);
                mpResultPoolObj->returnResult(pAllResult, __FUNCTION__);
            }
        }
        pMetaResult = mpResultPoolObj->getMetadata(frmId, __FUNCTION__);
    ...
    else
    {
        ...
        pMetaResult = mpResultPoolObj->getMetadata(frmId, __FUNCTION__);
        AllResult_T *pAllResult = mpResultPoolObj->getResultByReqNum(frmId, __FUNCTION__);

        if(pMetaResult == NULL)
        {
            CAM_LOGW("[%s] Fail to get the specified metadata(#%d), return -1", __FUNCTION__, frmId);
            AAA_TRACE_END_HAL;
            AAA_TRACE_END_HAL;
            AAA_TRACE_END_D;
            return (-1);
        ...
        if(pHalResult != NULL)
        {
            if(pHalResult->fgBadPicture)
                CAM_LOGD("[%s] Bad Picture #(%d), fgBadPicture(%d)", __FUNCTION__, frmId, pHalResult->fgBadPicture);
    ...
    CAM_LOGI("[%s] frmId(%d), IsConvert4MetaResult(%d), ret(%d), u1CaptureIntent(%d), count(%d, %d), AwbState(%d) AeState(%d) FlashState(%d) AfState(%d) Exp(%lld) ISO(%d) FrameDuration(%lld)", __FUNCTION__,
                frmId, i4IsConvert4MetaResult, ret, u1CaptureIntent, result.appMeta.count(), result.halMeta.count(),
                u1AwbState, u1AeState, u1FlashState, u1AfState, (long long)i8AeExposureTime, i4AeSensitivity, (long long)i8FrameDuration);
    if(!ret)
    {
        CAM_LOGW("[%s] Fail to get MTK_CONTROL_CAPTURE_INTENT in result(#%d)", __FUNCTION__, frmId);
        AAA_TRACE_END_HAL;
        AAA_TRACE_END_D;
        //return (-1);
    }
    ...
    else if(m_rConfigInfo.i4SubsampleCount > 1 && m_rConfigInfo.i4RequestCount > 1 && mSubSampleCount4Clear == m_rConfigInfo.i4SubsampleCount)
    {
        pMetaResult->rMetaResult.halMeta.clear();
        pMetaResult->rMetaResult.appMeta.clear();
        mSubSampleCount4Clear = 0;
    ...
    MY_LOGD_IF(fgLog, "[%s] - pMetaResult:%p, SubSampleCount4Clear(%d)", __FUNCTION__, pMetaResult, mSubSampleCount4Clear);
    AAA_TRACE_END_HAL;
    AAA_TRACE_END_D;
    return i4Ret;
}
...
getCur(MUINT32 frmId, MetaSet_T& result)
{
    ...
    if(1 == i4Ret)
    {
        ...
        pAllResult = mpResultPoolObj->getResultBySttNum(frmId, __FUNCTION__);
        i4Validate = (mpResultPoolObj->isValidateBySttNum(frmId, E_HAL_RESULTTOMETA) && mpResultPoolObj->isValidateBySttNum(frmId, E_AE_RESULTTOMETA) /*&&
                      mpResultPoolObj->isValidateBySttNum(frmId, E_AF_RESULTTOMETA)*/ && mpResultPoolObj->isValidateBySttNum(frmId, E_AWB_RESULTTOMETA) &&
                      mpResultPoolObj->isValidateBySttNum(frmId, E_LSC_RESULTTOMETA) && mpResultPoolObj->isValidateBySttNum(frmId, E_FLASH_RESULTTOMETA) &&
                      mpResultPoolObj->isValidateBySttNum(frmId, E_FLK_RESULTTOMETA) /*&& mpResultPoolObj->isValidateBySttNum(frmId, E_ISP_RESULTTOMETA)*/);
    }
    ...
    if(-1 == i4Ret)
    {
        ...
        pAllResult = mpResultPoolObj->getResultByReqNum(rHistoryReqMagic[2], __FUNCTION__);//get the last request EX: req/stt:4/1
        i4Validate = (mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_HAL_RESULTTOMETA) && mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_AE_RESULTTOMETA) /*&&
                      mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_AF_RESULTTOMETA)*/ && mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_AWB_RESULTTOMETA) &&
                      mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_LSC_RESULTTOMETA) && mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_FLASH_RESULTTOMETA) &&
                      mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_FLK_RESULTTOMETA) /*&& mpResultPoolObj->isValidateByReqNum(rHistoryReqMagic[2], E_ISP_RESULTTOMETA)*/);
    }
    if(-2 == i4Ret)
    {
        i4ReqMagic = frmId;
        CAM_LOGW("[%s] Fail to get R[%d], result will be obtained.", __FUNCTION__, frmId);
        pAllResult = mpResultPoolObj->getResultByReqNum(frmId, __FUNCTION__);//req/stt:2/X
        i4Validate = (mpResultPoolObj->isValidateByReqNum(frmId, E_HAL_RESULTTOMETA) && mpResultPoolObj->isValidateByReqNum(frmId, E_AE_RESULTTOMETA) /*&&
                      mpResultPoolObj->isValidateByReqNum(frmId, E_AF_RESULTTOMETA)*/ && mpResultPoolObj->isValidateByReqNum(frmId, E_AWB_RESULTTOMETA) &&
                      mpResultPoolObj->isValidateByReqNum(frmId, E_LSC_RESULTTOMETA) && mpResultPoolObj->isValidateByReqNum(frmId, E_FLASH_RESULTTOMETA) &&
                      mpResultPoolObj->isValidateByReqNum(frmId, E_FLK_RESULTTOMETA) /*&& mpResultPoolObj->isValidateByReqNum(frmId, E_ISP_RESULTTOMETA)*/);
    }
    ...
    if(i4Validate == MTRUE)
    {
        AllMetaResult_T *pMetaResult = mpResultPoolObj->getMetadata(frmId, __FUNCTION__);
        if(pMetaResult == NULL)
            CAM_LOGE("[%s] pMetaResult is NULL Req(#%d)", __FUNCTION__, frmId);
        else
        {
...

            {
                ...
                CAM_LOGI("[%s] frmId(%d, %d), ret(%d), u1CaptureIntent(%d), count(%d, %d), AwbState(%d) AeState(%d) FlashState(%d) AfState(%d)", __FUNCTION__,
...
setFDInfo(MVOID* prFaces)
{
    ...
    {
        mParams.bIsFDReady = MTRUE;
        CAM_LOGD("[%s] bIsFDReady(%d)", __FUNCTION__, mParams.bIsFDReady);
    }
    ...
    else if(e3ACtrl == E3ACtrl_GetFlashCapDummyCnt)
    {
        ...
        if (QUERY_ENTRY_SINGLE(mMetaStaticInfo, MTK_FLASH_FEATURE_CUSTOMIZATION_AVAILABLE, u1SubFlashCustomization)){
            if(u1SubFlashCustomization == 1 && (mu4SensorDev == ESensorDev_Sub || mu4SensorDev == ESensorDev_SubSecond)){
                cntBefore = 0;
                cntAfter = 0;
                CAM_LOGW("[%s] sensordev(%d) no dummy for customized panel flash.", __FUNCTION__, mu4SensorDev);
            }
        }
...
parseConfig(const ConfigInfo_T& rConfigInfo)
{
    ...
    CAM_LOGD("[%s] Config Params: HDR mode %d, IspProf %d, MinFps %d, MaxFps %d"
...
addCallback(IHal3ACb* cb)
{
    ...
    for (std::list<IHal3ACb*>::iterator it = m_CallBacks.begin(); it != m_CallBacks.end(); it++)
    {
        if (cb == *it)
        {
            CAM_LOGD("[%s] Callback already exists!", __FUNCTION__);
            return m_CallBacks.size();
        }
    }
...
removeCallback(IHal3ACb* cb)
{
    ...
    // cannot be found
    CAM_LOGE("[%s] No such callback, remove failed", __FUNCTION__);
    return -m_CallBacks.size();
}
