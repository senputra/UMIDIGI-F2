...
postCommand(ECmd_T const eCmd, const ParamIspProfile_T* pParam)
{
    ...
    if (eCmd == ECmd_Update)
    {
        MY_LOGD("m_rParam.u4AeMode(%d), m_rParam.i8ExposureTime(%lld)", m_rParam.u4AeMode, (long long)m_rParam.i8ExposureTime);
...
on3AProcFinish(const RequestSet_T rRequestSet, MINT32 i4MagicNumCur)
{
    ...
    AEResultToMeta_T *pAEResult = (AEResultToMeta_T*)m_pResultPoolObj->getResult(i4MagicNum, E_AE_RESULTTOMETA);
    AFResultToMeta_T *pAFResult = (AFResultToMeta_T*)m_pResultPoolObj->getResult(i4MagicNum, E_AF_RESULTTOMETA);
    ...
    if (m_pCbSet)
    {
        ...
        MY_LOGD_IF(m_fgLogEn, "[%s] #(%d,%d), SOF(0x%x), m_rResult.i8SensorExposureTime(%lld), m_rCapParam.capType(%d)",
                   __FUNCTION__, i4MagicNum, i4MagicNumCur, i4CurId, (long long)m_rCapParam.i8ExposureTime, m_rCapParam.u4CapType);
...
updateResult(MINT32 i4MagicNum, MINT32 i4SubsampleIndex)
{
    ...
    MY_LOGD_IF(m_fgLogEn, "[%s] i4Ret(%d) -", __FUNCTION__, i4Ret);
    #if 0
