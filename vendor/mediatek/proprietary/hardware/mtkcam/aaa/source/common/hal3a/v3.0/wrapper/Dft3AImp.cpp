...
config(const ConfigInfo_T& /*rConfigInfo*/)
{
    if (m_pCamIO == NULL)
    {
        ...
        ISP_MGR_AE_STAT_CONFIG_T::getInstance(static_cast<MUINT32>(m_u4SensorDev)).config(rTmpAe);
        ISP_MGR_AWB_STAT_CONFIG_T::getInstance(static_cast<MUINT32>(m_u4SensorDev)).config(rTmpAwb, 0, m_i4QBINSzW, m_i4QBINSzH, blockwnum, blockhnum);
        // ISP_MGR_AF_STAT_CONFIG_T::getInstance(static_cast<MUINT32>(m_u4SensorDev)).config(testcfg, sArea_HW, i4HWBlkNumX, i4HWBlkNumY, i4HWBlkSizeX, i4HWBlkSizeY, i4FirstTimeConfig);
...
validateP1(const ParamIspProfile_T& rParamIspProfile, MBOOL fgPerframe)
{
    ...
    m_pTuning->validatePerFrameP1(m_u4SensorDev, rParamIspProfile.rRequestSet, fgPerframe);
    return MTRUE;
}
#endif
