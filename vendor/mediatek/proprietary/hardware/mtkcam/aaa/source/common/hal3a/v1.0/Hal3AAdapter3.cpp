...
#define EXPOSURETIMEDUMMY (10000000)
#define SENSITIVITYDUMMY (100)
/********************************************************************************************
...
CameraArea_T _transformArea(const MUINT32 i4SensorIdx, const MINT32 i4SensorMode, const CameraArea_T& rArea)
{
    ...
    Mutex::Autolock mapMatLock(Hal3AAdapter3::mMapMatLock);
...
createInstance(MINT32 const i4SensorIdx, const char* strUser)
{
    ...
    dbgInfoEnable = 0;
#endif
    GET_PROP("vendor.debug.camera.dbginfo", dbgInfoEnable, mbDbgInfoEnable);
...
{
    MY_LOGD("[%s]+ i4SubsampleCount(%d) RequestCount(%d)", __FUNCTION__, rConfigInfo.i4SubsampleCount, rConfigInfo.i4RequestCount);

    {
        Mutex::Autolock lockConfigSend3ACtrl(mLockConfigSend3ACtrl);
        if (rConfigInfo.i4SubsampleCount > 1 && rConfigInfo.i4RequestCount > 1) //for SMVR
        {
            MY_LOGD("Uninit Normal 3A");
            doUninit();
            m_rConfigInfo = rConfigInfo;
            MY_LOGD("Init SMVR 3A");
            doInit();
        }
        else
        {
            ...
            {
                MY_LOGD("Uninit SMVR 3A");
                doUninit();
                m_rConfigInfo = rConfigInfo;
                MY_LOGD("Init Normal 3A");
                doInit();
            }
        }
    ...
    for(int i=0; i<4; i++) { rSclCrop[i] = 0; }
    ...
    MY_LOGD("[%s]- mMapMat size(%zu)", __FUNCTION__, mMapMat.size());
    return i4Ret;
}
...
start(MINT32 /*i4StartNum*/)
{
    ...
    MY_LOGD("[%s]+ sensorDev(%d), sensorIdx(%d), mpHal3aObj(%p), Clear mpResultPoolObj(%p)",
        __FUNCTION__, mu4SensorDev, mi4SensorIdx, mpHal3aObj, mpResultPoolObj);

    if(mu1StartStatus != 0)
    {
        MY_LOGE("[%s]- Already started! status(%d), sensorDev(%d), sensorIdx(%d), mpHal3aObj(%p), Clear mpResultPoolObj(%p)",
            __FUNCTION__, mu1StartStatus, mu4SensorDev, mi4SensorIdx, mpHal3aObj, mpResultPoolObj);
        return 1;
    }
...
stop()
{
    ...
    if (mu1Start)
    {
        ...
        if (bIsActive)
        {
            ...
            rStereoParam.i4FrameNum = 0x7FFFFFFF;
            mpHal3aObj->send3ACtrl(E3ACtrl_SetStereoParams, (MINTPTR)&rStereoParam, 0);
        }

        mStereoParam.i4MasterIdx = 0;
        mStereoParam.i4SlaveIdx = 0;
...
init(const char* strUser)
{
    ...
    if (m_Users.size())
    {
        MY_LOGD("[%s] Still %zu users", __FUNCTION__, m_Users.size());
    }
    else
    {
        doInit();
    ...
    MY_LOGD("[%s] Done m_rConfigInfo.i4SubsampleCount(%d), User.count(%zu), User init(%s)", __FUNCTION__, m_rConfigInfo.i4SubsampleCount, m_Users.size(), strUser);
    return MTRUE;
}
...
uninit(const char* strUser)
{
    ...
    else
    {
        ...
        if (!m_Users[string(strUser)])
        {
            ...
            else    MY_LOGD("[%s] Still %zu users", __FUNCTION__, m_Users.size());
        }
    }

    MY_LOGD("[%s] Done m_rConfigInfo.i4SubsampleCount(%d), User.count(%zu), User uninit(%s)", __FUNCTION__, m_rConfigInfo.i4SubsampleCount, m_Users.size(), strUser);
    return MTRUE;
}
...
startCapture(const List<MetaSet_T>& requestQ, MINT32 /*i4StartNum*/, MINT32 i4RequestQSize)
{
    ...
    {
        MY_LOGW("[%s] Start Capture Request Less Than 3 !", __FUNCTION__);
        return 0;
    }
...
startCapture(const vector<MetaSet_T*>& requestQ, MINT32 /*i4StartNum*/)
{
    ...
    if(mu1StartStatus != 0)
    {
        MY_LOGE("[%s]- Already started! status(%d), sensorDev(%d), sensorIdx(%d), mpHal3aObj(%p)",
            __FUNCTION__, mu1StartStatus, mu4SensorDev, mi4SensorIdx, mpHal3aObj);
        return 1;
    }
...
#define REQ_SIZE 3
    ...
    if(mu1StartStatus != 0)
    {
        MY_LOGE("[%s]- Already started! status(%d), sensorDev(%d), sensorIdx(%d), Clear mpResultPoolObj(%p)",
            __FUNCTION__, mu1StartStatus, mu4SensorDev, mi4SensorIdx, mpResultPoolObj);
        return 1;
    }
    ...
    MUINT8 CapIntent = 0;
    for (MUINT32 i = 0; i < requestQ.back()->appMeta.count(); i++)
    {
    ...
    for (MUINT32 i = 0; i < requestQ.size(); i++)
    {
        ...
        {
            // ResultPool - Get Validate Metadata pool
            MINT32 i4Magic = requestQ[i]->MagicNum;
            MetaSet_T* pMetaResult = NULL;
            AllMetaResult_T* pAllMetaResult = NULL;
            // getValidateMetadata & assign request metadata
            pMetaResult = mpResultPoolObj->getValidateMetadata(i4Magic);
            *pMetaResult = (*requestQ[i]);
            // getMetadata & update metadata
            pAllMetaResult = mpResultPoolObj->getMetadata(i4Magic);
            AllResult_T *pAllResult = mpResultPoolObj->getAllResult(i4Magic);

            MY_LOGD("[%s] magic(%d), Metadata result(%p), All Result(%p)", __FUNCTION__, i4Magic, pAllMetaResult, pAllResult);
            if(pAllMetaResult != NULL && pAllResult != NULL)
            {
                UPDATE_ENTRY_SINGLE(pAllMetaResult->rMetaResult.halMeta, MTK_3A_ISP_PROFILE, IspProfile);
                // convert 3A result to metadata
                MINT32 i4IsConvert4MetaResult = E_NON_CONVERT;
                mpHal3aObj->send3ACtrl(E3ACtrl_GetCurrResult, i4Magic, NULL);

                {
                    Mutex::Autolock lock(pAllMetaResult->LockMetaResult);
                    i4IsConvert4MetaResult = pAllMetaResult->i4IsConverted;
                }
                if(!(i4IsConvert4MetaResult & E_2A_CONVERT))
                    mpResultPoolObj->convertToMetadata(pAllResult, pAllMetaResult);

                if(!(i4IsConvert4MetaResult & E_AF_CONVERT))
                    mpResultPoolObj->convertToMetadata4AF(pAllResult, pAllMetaResult);
                //
                mpHal3aObj->send3ACtrl(E3ACtrl_ValidatePass1, i4Magic, (MINTPTR)IspProfile);
            }
            else
                MY_LOGE("[%s] get Metadata result(%p) or All Result(%p) fail!", __FUNCTION__, pAllMetaResult, pAllResult);
        }
    }
    mu1Start = 1;
    mu1StartStatus = 2;
...
preset(const vector<MetaSet_T*>& requestQ)
{
    ...
    lock_guard<mutex> _lock(mutexParseMeta);    // Preset would contend this mutex with Vsync_Done Callback to ensure that there is no consecutive Preset taking place
...
#define OVERRIDE_AF  (1<<2)
    ...
    QUERY_ENTRY_SINGLE(_halmeta, MTK_P1NODE_MIN_FRM_DURATION, i8MinFrmDuration);
    ...
    IMetadata::Memory tempMeta;
    if(QUERY_ENTRY_SINGLE<IMetadata::Memory>(_halmeta, MTK_3A_CUST_PARAMS, tempMeta))
        mParams.rAeCustParam = *((AE_Cust_Param_T*)(tempMeta.array()));
    ...
    MUINT8 u1RemosaicEn = 0;
    if (!QUERY_ENTRY_SINGLE(_halmeta, MTK_HAL_REQUEST_REMOSAIC_ENABLE, u1RemosaicEn))
        mParams.u1RemosaicEn = 0;
    MY_LOGD_IF(mParams.u1RemosaicEn != u1RemosaicEn,"[%s] MTK_HAL_REQUEST_REMOSAIC_ENABLE(%d -> %d)", __FUNCTION__, mParams.u1RemosaicEn, u1RemosaicEn);
    mParams.u1RemosaicEn = u1RemosaicEn;

    MY_LOGD_IF(fgLogEn0,"[%s] RepeatTag(%d), MinFrmDuration(%lld), AeIsoSpeedMode(%d), IspProfile(%d), RawType(%d), DisableP1(%d), IsSingleFrameHDR(%d), MtkHdrMode(%d), MWBColorTemperature(%d), DummyAfterCapture(%d)",
                        __FUNCTION__, u1RepeatTag, (long long)i8MinFrmDuration, mParams.i4IsoSpeedMode, u1IspProfile, mParams.i4RawType, mParams.i4DisableP1,
    ...
    // vendor tag for customized torch duty
    if (!QUERY_ENTRY_SINGLE(_appmeta, MTK_FLASH_FEATURE_CUSTOMIZED_TORCH_DUTY, mParams.u1TorchDuty))
        mParams.u1TorchDuty = 1; // Torch default value

    MY_LOGD_IF(fgLogEn0,"[%s] MTK_HAL_REQUEST_PRECAPTURE_START(%d), MTK_HAL_REQUEST_AF_TRIGGER_START(%d), AfPause(%d), TorchDuty(%d)",
                        __FUNCTION__, mParams.u1PreCapStart, mAfParams.u1AfTrigStart, mAfParams.u1AfPause, mParams.u1TorchDuty);
    ...
    MY_LOGD_IF(fgLogEn0,"[%s] RepeatTag(%d), MinFrmDuration(%lld), AeIsoSpeedMode(%d), IspProfile(%d), RawType(%d), DisableP1(%d), IsSingleFrameHDR(%d), MtkHdrMode(%d), MWBColorTemperature(%d)",
                        __FUNCTION__, u1RepeatTag, (long long)i8MinFrmDuration, mParams.i4IsoSpeedMode, u1IspProfile, mParams.i4RawType, mParams.i4DisableP1,
                        mParams.u1IsSingleFrameHDR, u1MtkHdrMode, mParams.i4MWBColorTemperature);
    ...
    mStereoParam.i4ZoomRatio = mParams.i4ZoomRatio;
    mStereoParam.i4FrameNum = it->FrameNum;
    ...
    MINT32 i4LogEnable = ( (mParams.i4DenoiseMode != i4DenoiseMode)||(mStereoParam.i4Sync2AMode != i4Sync2AMode)||(mStereoParam.i4SyncAFMode != i4SyncAFMode)||(mStereoParam.i4HwSyncMode != i4HwSyncMode) );

    MY_LOGD_IF( (i4LogEnable || fgLogEn0), "[%s] Dev(%d) PreSetKey(#%d) FrameNum(%d), StereoParams : DenoiseMode(%d -> %d), Sync2A(%d -> %d), SyncAF(%d -> %d), HwSync(%d -> %d), M&S Idx(%d,%d), bIsByFrame(%d -> %d)", __FUNCTION__,
        mu4SensorDev, it->PreSetKey, mStereoParam.i4FrameNum,
    ...
    MBOOL ReparseMetaForDummy = ((mParams.i8ExposureTime == EXPOSURETIMEDUMMY) && (mParams.i4Sensitivity == SENSITIVITYDUMMY));

    MY_LOGD_IF(fgLog,
        "[%s], dummy(%d), hqc(%d), exif(%d), SCL CROP(%d,%d,%d,%d), AF SCL CROP(%d,%d,%d,%d) ReparseMetaForDummy(%d)",
        __FUNCTION__, it->Dummy, mParams.u1HQCapEnable, mParams.u1IsGetExif,
        rSclCrop[0], rSclCrop[1], rSclCrop[2], rSclCrop[3],
        mAfParams.rScaleCropArea.i4Left, mAfParams.rScaleCropArea.i4Top, mAfParams.rScaleCropArea.i4Right, mAfParams.rScaleCropArea.i4Bottom, ReparseMetaForDummy);
            ...
            case MTK_CONTROL_AWB_REGIONS:
                {
                    if (m_rStaticInfo.i4MaxRegionAwb != 0)
                    {
                        ...
                        for (MINT32 k = 0; k < numRgns; k++)
                        {
                            ...
                            MY_LOGD("[%s] Return to AP AWB_REGIONS L(%d) T(%d) R(%d) B(%d) W(%d)", __FUNCTION__, rArea.i4Left, rArea.i4Top, rArea.i4Right, rArea.i4Bottom, rArea.i4Weight);
            ...
            case MTK_CONTROL_AE_ANTIBANDING_MODE:
                {
                    ...
                    MY_LOGD_IF(mParams.u4AntiBandingMode != static_cast<MUINT32>(i4Mode), "[%s] MTK_CONTROL_AE_ANTIBANDING_MODE(%d -> %d)", __FUNCTION__, mParams.u4AntiBandingMode, i4Mode);
            ...
            case MTK_CONTROL_AE_REGIONS:    // dynamic
                {
                    if (m_rStaticInfo.i4MaxRegionAe != 0)
                    {
                        ...
                        for (MINT32 k = 0; k < numRgns; k++)
                        {
                            ...
                            MY_LOGD("[%s] Return to AP AE_REGIONS L(%d) T(%d) R(%d) B(%d) W(%d)", __FUNCTION__, rArea.i4Left, rArea.i4Top, rArea.i4Right, rArea.i4Bottom, rArea.i4Weight);
            ...
            case MTK_SENSOR_EXPOSURE_TIME:
                {
                    ...
                    MY_LOGD_IF(mParams.i8ExposureTime != i8AeExposureTime, "[%s] MTK_SENSOR_EXPOSURE_TIME(%lld -> %lld)", __FUNCTION__, (long long)mParams.i8ExposureTime, (long long)i8AeExposureTime);
            ...
            case MTK_SENSOR_FRAME_DURATION:
                {
                    ...
                    MY_LOGD_IF(mParams.i8FrameDuration != i8FrameDuration, "[%s] MTK_SENSOR_FRAME_DURATION(%lld -> %lld)", __FUNCTION__, (long long)mParams.i8FrameDuration, (long long)i8FrameDuration);
            ...
            case MTK_CONTROL_AF_REGIONS:    // dynamic
                {
                    if (m_rStaticInfo.i4MaxRegionAf != 0)
                    {
                        ...
                        for (MINT32 k = 0; k < numRgns; k++)
                        {
                            ...
                            MY_LOGD("[%s] Return to AP AF_REGIONS L(%d) T(%d) R(%d) B(%d) W(%d)", __FUNCTION__, rArea.i4Left, rArea.i4Top, rArea.i4Right, rArea.i4Bottom, rArea.i4Weight);
                            ...
                            if (rArea.i4Left < 0 || rArea.i4Top < 0 || rArea.i4Right < 0 || rArea.i4Bottom < 0)
                            {
                                MY_LOGE("[%s] MTK_CONTROL_AF_REGIONS mod L(%d) T(%d) R(%d) B(%d) W(%d)", __FUNCTION__, rArea.i4Left, rArea.i4Top, rArea.i4Right, rArea.i4Bottom, rArea.i4Weight);
                            }
                        }
            ...
            case MTK_COLOR_CORRECTION_MODE:
                {
                    ...
                    MY_LOGD_IF(mParams.u1ColorCorrectMode != u1ColorCrctMode, "[%s] MTK_COLOR_CORRECTION_MODE(%d -> %d)", __FUNCTION__, mParams.u1ColorCorrectMode, u1ColorCrctMode);
            ...
            case MTK_TONEMAP_MODE:
                {
                    ...
                    MY_LOGD_IF(mParams.u1TonemapMode != u1TonemapMode, "[%s] MTK_TONEMAP_MODE(%d -> %d)", __FUNCTION__, mParams.u1TonemapMode, u1TonemapMode);
            ...
            case MTK_FACE_FEATURE_FORCE_FACE_3A:
                {
                    ...
                    MY_LOGD_IF(mParams.i4ForceFace3A != i4ForceFace3A, "[%s] MTK_FACE_FEATURE_FORCE_FACE_3A(%d)", __FUNCTION__, mParams.i4ForceFace3A);
        ...
        default:
            {
                ...
                {
                    MY_LOGW("[%s] Scene mode(%d) overrides Fails", __FUNCTION__, mParams.u4SceneMode);
                }
            }
            break;
        }
    ...
    if(mParams.u4AwbMode != MTK_CONTROL_AWB_MODE_OFF && mParams.u1ColorCorrectMode == MTK_COLOR_CORRECTION_MODE_TRANSFORM_MATRIX)
    {
        mParams.u1ColorCorrectMode = MTK_COLOR_CORRECTION_MODE_FAST;
    }
...
setNormal(const vector<MetaSet_T*>& requestQ)
{
    ...
    else if (it->Dummy || mParams.bDummyBeforeCapture || mParams.bDummyAfterCapture)
    {
        ...
        MY_LOGD("[%s] Dev(%d) Req(#%d) FrameNum(#%d) Dummy(%d) Sync2AMode(%d) SyncAF(%d) HwSync(%d)", __FUNCTION__, mu4SensorDev, i4FrmId, i4FrameNum, it->Dummy, mStereoParam.i4Sync2AMode, mStereoParam.i4SyncAFMode, mStereoParam.i4HwSyncMode);
#endif
        MY_LOGD("[%s] Dev(%d) Req(#%d) FrameNum(#%d) Dummy(%d/%d/%d) CapInt(%d) AE(M:%d/E:%lld/F:%lld/S:%d) FLASH(M:%d)", __FUNCTION__, mu4SensorDev, i4FrmId, i4FrameNum,
            it->Dummy, mParams.bDummyBeforeCapture, mParams.bDummyAfterCapture, mParams.u1CaptureIntent,
            mParams.u4AeMode, (long long)mParams.i8ExposureTimeDummy, (long long)mParams.i8FrameDurationDummy, mParams.i4SensitivityDummy,
            mParams.u4StrobeMode);
    }

    mParams.bIsDummyFrame = it->Dummy;
    // Store in ResultPool
    ...
    // ResultPool - 2. Get validate metadata pool to update metadata by request magic
    MetaSet_T* prMetaResult = mpResultPoolObj->getValidateMetadata(i4FrmId);

    // assign data to metada pool
    ...
    for(MINT32 i = 0; i < it->vMagicNumSet.size(); i++)
    {
        if(it->vMagicNumSet[i] != i4FrmId)
            rRequestSet.vNumberSet.push_back(it->vMagicNumSet[i]);
    }
...
#define OVERRIDE_AF  (1<<2)
    ...
    for (MUINT32 i = 0; i < requestQ.size(); i++)
    {
        ...
        // ResultPool - 2. Get validate metadata pool to update metadata by request magic
        MetaSet_T* prMetaResult = mpResultPoolObj->getValidateMetadata(i4FrmId);
        // ResultPool - 3. update MagicNum and ConvertMagic
        prMetaResult->MagicNum = i4FrmId;
        rConvertMagic4SMVR[(i % m_rConfigInfo.i4SubsampleCount)] = i4FrmId;

        QUERY_ENTRY_SINGLE(_halmeta, MTK_P1NODE_MIN_FRM_DURATION, i8MinFrmDuration);
        //QUERY_ENTRY_SINGLE(_halmeta, MTK_HAL_REQUEST_ISO_SPEED, i4AeIsoSpeedMode);
/*        QUERY_ENTRY_SINGLE(_halmeta, MTK_HAL_REQUEST_BRIGHTNESS_MODE, mParams.i4BrightnessMode);
        QUERY_ENTRY_SINGLE(_halmeta, MTK_HAL_REQUEST_CONTRAST_MODE, mParams.i4ContrastMode);
        QUERY_ENTRY_SINGLE(_halmeta, MTK_HAL_REQUEST_HUE_MODE, mParams.i4HueMode);
        QUERY_ENTRY_SINGLE(_halmeta, MTK_HAL_REQUEST_SATURATION_MODE, mParams.i4SaturationMode);
        QUERY_ENTRY_SINGLE(_halmeta, MTK_HAL_REQUEST_EDGE_MODE, mParams.i4EdgeMode);*/
        QUERY_ENTRY_SINGLE(_halmeta, MTK_3A_ISP_PROFILE, u1IspProfile);
        ...
        if (fgOK)   // && (i4requestSize >= m_rConfigInfo.i4SubsampleCount *(HAL3A_REQ_PROC_KEY + 1)))      // Probably not in use anymore
        {
            ...
            if (!uRepeatTag) //not repeating tag, parse app meta
            {
                ...
                for (MUINT32 j = 0; j < _appmeta.count(); j++)
                {
                    ...
                    case MTK_CONTROL_AE_ANTIBANDING_MODE:
                        {
                            MUINT32 u4Mode = entry.itemAt(0, Type2Type< MUINT8 >());
                            MY_LOGD_IF(mParams.u4AntiBandingMode != u4Mode, "[%s] MTK_CONTROL_AE_ANTIBANDING_MODE(%d -> %d)", __FUNCTION__, mParams.u4AntiBandingMode, u4Mode);
                            mParams.u4AntiBandingMode = u4Mode;
                        }
                        break;
                    case MTK_CONTROL_AE_EXPOSURE_COMPENSATION:
                        {
...
get(MUINT32 frmId, MetaSet_T& result)
{
    ...
    if(pMetaResult == NULL)
    {
        MY_LOGW("[%s] Fail to get the specified result(#%d), return -1", __FUNCTION__, frmId);
        AAA_TRACE_END_HAL;
        AAA_TRACE_END_HAL;
        AAA_TRACE_END_D;
        return (-1);
    ...
    if(!(i4IsConvert4MetaResult & E_2A_CONVERT))
        mpResultPoolObj->convertToMetadata(pAllResult, pMetaResult);
    ...
    HALResultToMeta_T* pHalResult = (HALResultToMeta_T*)mpResultPoolObj->getResult(frmId,E_HAL_RESULTTOMETA);
    if(pHalResult != NULL)
    {
    ...
    // update FLASH_STATE for the case where convertToMetadata is done prior to flash result update.
    // such as calling handleBadPicture when turning off flash at p1Done.
    FLASHResultToMeta_T* pFLASHResult = (FLASHResultToMeta_T*)mpResultPoolObj->getResult(frmId,E_FLASH_RESULTTOMETA);
    ...
    if(!ret)
    {
        ...
        MY_LOGW("[%s] Fail to get MTK_CONTROL_CAPTURE_INTENT in result(#%d), update capture intent 1", __FUNCTION__, frmId);
        UPDATE_ENTRY_SINGLE<MUINT8>(result.appMeta, MTK_CONTROL_CAPTURE_INTENT, 1);
    }
    AAA_TRACE_END_HAL;
    ...
    MY_LOGD_IF(fgLog, "[%s] - pMetaResult(%p)", __FUNCTION__, pMetaResult);
    AAA_TRACE_END_HAL;
    AAA_TRACE_END_D;
    return i4Ret;
}
...
getCur(MUINT32 frmId, MetaSet_T& result)
{
    ...
    if(1 == i4Ret)
    {
        ...
        i4Validate = mpResultPoolObj->isValidateCur(frmId);
    }
    ...
    if(-1 == i4Ret)
    {
        ...
        i4Validate = mpResultPoolObj->isValidate(rHistoryReqMagic[2]);
    }
    if(-2 == i4Ret)
    {
        ...
        i4Validate = mpResultPoolObj->isValidate(frmId);
    }
    ...
    if(i4Validate == MTRUE)
    {
        ...
        if(pMetaResult == NULL)
        {
            MY_LOGE("[%s] pMetaResult is NULL Req(#%d), return -1", __FUNCTION__, frmId);
            return -1;
        }
        else
        {
            ...
            MUINT8 u1AfState = 0;
            MBOOL ret = QUERY_ENTRY_SINGLE(result.appMeta, MTK_CONTROL_CAPTURE_INTENT, u1CaptureIntent);
            MY_LOGD("[%s] frmId(%d), ret(%d), u1CaptureIntent(%d), count(%d)", __FUNCTION__, frmId, ret, u1CaptureIntent, pMetaResult->rMetaResult.appMeta.count());
            if(!ret) {
                MY_LOGW("[%s] Fail to get MTK_CONTROL_CAPTURE_INTENT in result(#%d), count(%d), return -1", __FUNCTION__, frmId, pMetaResult->rMetaResult.appMeta.count());
                return -1;
            }
            QUERY_ENTRY_SINGLE(result.appMeta, MTK_CONTROL_AWB_STATE, u1AwbState);
            QUERY_ENTRY_SINGLE(result.appMeta, MTK_CONTROL_AE_STATE, u1AeState);
            ...
            MY_LOGD("[%s] frmId(%d, %d), ret(%d), u1CaptureIntent(%d), count(%d, %d), AwbState(%d) AeState(%d) FlashState(%d) AfState(%d)", __FUNCTION__,
                        frmId, i4ReqMagic, ret, u1CaptureIntent, result.appMeta.count(), result.halMeta.count(),
                        u1AwbState, u1AeState, u1FlashState, u1AfState);
        }
    }
    else
    {
        MY_LOGW("[%s] - Validate:%d", __FUNCTION__, i4Validate);
        return (-1);
    }
    AAA_TRACE_END_HAL;
    return 0;
...
getReprocStdExif(const MetaSet_T& control)
{
    ...
    MBOOL chk = QUERY_ENTRY_SINGLE(mMetaStaticInfo, MTK_3A_FEATURE_LENS_AVAILABLE_FOCAL_LENGTHS_35MM, focusLength35mm);
    MY_LOGD("[%s] focusLength35mm(%d), chk(%d)", __FUNCTION__, focusLength35mm, chk);
    ...
    UPDATE_ENTRY_SINGLE<MINT32>(rMetaExif, MTK_3A_EXIF_FOCAL_LENGTH_35MM,     focusLength35mm);
...
{
    sp<IFileCache> fid = IFileCache::open(filename);
    if (fid.get())
    {
        IMetadata::Memory p3ADbg;
        if (QUERY_ENTRY_SINGLE<IMetadata::Memory>(metaExif, MTK_3A_EXIF_DBGINFO_AAA_DATA, p3ADbg))
        {
            MY_LOGD("[%s] %s, 3A(%p, %d)", __FUNCTION__, filename, p3ADbg.array(), (MINT32)p3ADbg.size());
        ...
        {
            MY_LOGD("[%s] %s, ISP(%p, %d)", __FUNCTION__, filename, pIspDbg.array(), (MINT32)pIspDbg.size());
        ...
        MY_LOGW("IFileCache open fail <%s>", filename);
        return MFALSE;
    }
}
...
_dumpAAO(const char* filename, MUINT32 SensorDev, MINT32 MagicNumberRequest)
{
    ...
    int N;
    MY_LOGD("_dumpAAO begin");
    pSttBufQ = ISttBufQ::getInstance(SensorDev);
    if (!pSttBufQ.get()) {
        MY_LOGW("ISttBufQ::getInstance(SensorDev=%d) error!!", SensorDev);
        goto lbExit;
    }

    pData = pSttBufQ->deque_byMagicNumberRequest(MagicNumberRequest);
    if (!pData.get()) {
        MY_LOGW("pSttBufQ->GetByMagicNumberRequest(%d) error!!", MagicNumberRequest);
        goto lbExit;
    }

    fptr = IFileCache::open(filename);
    if (fptr.get()==NULL) {
        MY_LOGW("open file(%s) error!!", filename);
        goto lbExit;
    }

    N = pData->AAO.size();
    if ((int)fptr->write(pData->AAO.editArray(), N) != N) {
        MY_LOGW("file(%s) write error!!", filename);
        goto lbExit;
    }
    MY_LOGD("_dumpAAO end");
...
/*
    ...
    MRect rSclCropRect;
    MBOOL fgCrop = QUERY_ENTRY_SINGLE(control.halMeta, MTK_3A_PRV_CROP_REGION, rSclCropRect);
    if (fgCrop)
    {
        mPrvCropRegion.p.x = rSclCropRect.p.x;
        mPrvCropRegion.p.y = rSclCropRect.p.y;
        ...
        // crop info for AE
        rNewP2Param.rScaleCropRect.i4Xoffset = rSclCropRect.p.x;
        rNewP2Param.rScaleCropRect.i4Yoffset = rSclCropRect.p.y;
        rNewP2Param.rScaleCropRect.i4Xwidth = rSclCropRect.s.w;
        rNewP2Param.rScaleCropRect.i4Yheight = rSclCropRect.s.h;
...

    {
        ...
        const MFLOAT * pTonemapRed;
        const MFLOAT * pTonemapGreen;
        const MFLOAT * pTonemapBlue;
        entry = control.appMeta.entryFor(MTK_TONEMAP_CURVE_RED);
        if (entry.tag() != IMetadata::IEntry::BAD_TAG)
        {
            rNewP2Param.u4TonemapCurveRedSize = entry.count();
            pTonemapRed = QUERY_ENTRY_REF_SINGLE<MFLOAT>(control.appMeta, MTK_TONEMAP_CURVE_RED);
            rNewP2Param.pTonemapCurveRed = pTonemapRed;
        }

        entry = control.appMeta.entryFor(MTK_TONEMAP_CURVE_GREEN);
        if (entry.tag() != IMetadata::IEntry::BAD_TAG)
        {
            rNewP2Param.u4TonemapCurveGreenSize = entry.count();
            pTonemapGreen = QUERY_ENTRY_REF_SINGLE<MFLOAT>(control.appMeta, MTK_TONEMAP_CURVE_GREEN);
            rNewP2Param.pTonemapCurveGreen = pTonemapGreen;
        }

        entry = control.appMeta.entryFor(MTK_TONEMAP_CURVE_BLUE);
        if (entry.tag() != IMetadata::IEntry::BAD_TAG)
        {
            rNewP2Param.u4TonemapCurveBlueSize = entry.count();
            pTonemapBlue = QUERY_ENTRY_REF_SINGLE<MFLOAT>(control.appMeta, MTK_TONEMAP_CURVE_BLUE);
            rNewP2Param.pTonemapCurveBlue = pTonemapBlue;
        }
    }

#if HAL3A_TEST_OVERRIDE
    if(mu4ManualModeP2 != 0) _test_p2(mu4ManualModeP2 ,mParams, rNewP2Param);
#endif
...
{
    FileReadRule rule;
    if (!rule.isREADEnable("3AHAL") || pIspInfo == NULL)
        return MFALSE;

    EIspProfile_T       rIspProfile;
#if (!CAM3_3A_ISP_30_EN && !CAM3_3A_ISP_40_EN)
    rIspProfile=pIspInfo->rCamInfo.rMapping_Info.eIspProfile;
#else
    rIspProfile=pIspInfo->rCamInfo.eIspProfile;
#endif

    if ((rIspProfile == NSIspTuning::EIspProfile_Capture) ||
        (rIspProfile == NSIspTuning::EIspProfile_MFNR_After_Blend) ||
        (rIspProfile == NSIspTuning::EIspProfile_MFNR_MFB) ||
        (rIspProfile == NSIspTuning::EIspProfile_MFNR_Single) ||
        (rIspProfile == NSIspTuning::EIspProfile_MFNR_Before_Blend))
    {
        MINT32 i4ReqNo = 0;
        ...
        if (QUERY_ENTRY_SINGLE(control.halMeta, MTK_HAL_REQUEST_INDEX_BSS, i4ReqNo)){
            MY_LOGD("[%s] MTK_HAL_REQUEST_INDEX_BSS(%d)",__FUNCTION__, i4ReqNo);
        }
        char strDump[512] = {'\0'};
        std::string strispProfileName = IspTuningMgr::getIspProfileName(rIspProfile);

        MY_LOG("[%s] i4ReqNo(%d) ispProfileName(%s) format(%d)",__FUNCTION__, i4ReqNo, strispProfileName.c_str(), i4Format);
        ...
        if (i4Format == E_Tuning_Output)
        {
            ...
            if (fidTuning)
            {
                MY_LOG("[%s] %s pRegBuf size(%d)",__FUNCTION__, strDump, queryTuningSize());
                size_t result = fread(pTuningBuf->pRegBuf, queryTuningSize(), 1, fidTuning);
                if(result != queryTuningSize())
                    CAM_LOGE("[%s] Reading error, result(%zu), queryTuningSize(%d)", __FUNCTION__, result, queryTuningSize());
                fclose(fidTuning);
            }
        }
        ...
        if (i4Format == E_Lsc_Output)
        {
            ...
            if (fidLscRead){
                MY_LOG("[%s] %s ILscTbl::lscdata, size(%zu)",__FUNCTION__, strDump, pIspInfo->rLscData.size());
                size_t result = fread(pIspInfo->rLscData.data(), pIspInfo->rLscData.size(), 1, fidLscRead);
                if(result != queryTuningSize())
                    CAM_LOGE("[%s] Reading error, result(%zu), queryTuningSize(%d)", __FUNCTION__, result, queryTuningSize());
                fclose(fidLscRead);
            }
        ...
        if (i4Format == E_MFB_Output && (rIspProfile == NSIspTuning::EIspProfile_MFNR_MFB))
        {
            MINT32 i4MfbSize = 0;
            send3ACtrl(E3ACtrl_GetMfbSize, (MINTPTR)&i4MfbSize, (MINTPTR)NULL);
            if (i4MfbSize == 0)
                return MTRUE;
            ...
            if (fidTuning)
            {
                MY_LOG("[%s] %s pMfbBuf size(%d)",__FUNCTION__, strDump, i4MfbSize);
                size_t result = fread(pTuningBuf->pMfbBuf, i4MfbSize, 1, fidTuning);
                if(result != queryTuningSize())
                    CAM_LOGE("[%s] Reading error, result(%zu), queryTuningSize(%d)", __FUNCTION__, result, queryTuningSize());
                fclose(fidTuning);
            }
...
{
    FileReadRule rule;
    if (!rule.isDumpEnable("3AHAL"))
        return MFALSE;
    IMetadata::Memory caminfoBuf;
    MBOOL bHasCaminfo = QUERY_ENTRY_SINGLE<IMetadata::Memory>(control.halMeta, MTK_PROCESSOR_CAMINFO, caminfoBuf);
    ...
    mpHal3aObj->setISPInfo(rNewP2Param, rIspInfo, 0);

    EIspProfile_T       rIspProfile;
#if (!CAM3_3A_ISP_30_EN && !CAM3_3A_ISP_40_EN)
    rIspProfile=rIspInfo.rCamInfo.rMapping_Info.eIspProfile;
#else
    rIspProfile=rIspInfo.rCamInfo.eIspProfile;
#endif
     ...
     if ((rIspProfile == NSIspTuning::EIspProfile_Capture) ||
         ...
         (rIspProfile == NSIspTuning::EIspProfile_MFNR_After_Blend)){
            if (pTuningBuf->pRegBuf != NULL)
             {
                 const char *ispProfileName = IspTuningMgr::getIspProfileName(rIspProfile);
                 char strTuningFile[512] = {'\0'};
                 sprintf(strTuningFile, "/sdcard/camera_dump/%09d-%04d-%04d-%d-%s.p2buf", rIspInfo.hint.UniqueKey, rIspInfo.hint.FrameNo, rIspInfo.hint.RequestNo, mu4SensorDev, ispProfileName);
                 FILE* fidTuning = fopen(strTuningFile, "wb");
                 if (fidTuning)
                 {
                      MY_LOG("[%s] %s pRegBuf size(%d)",__FUNCTION__, strTuningFile, queryTuningSize());
...
setIsp(MINT32 flowType, const MetaSet_T& control, TuningParam* pTuningBuf, MetaSet_T* pResult)
{
    ...
    else
    {
        ...
        P2Param_T rNewP2Param;
        NSIspTuning::ISP_INFO_T rIspInfo;
        IMetadata::Memory caminfoMeta;
        auto bHasCaminfo = QUERY_ENTRY_SINGLE<IMetadata::Memory>(control.halMeta, MTK_PROCESSOR_CAMINFO, caminfoMeta);
        if (bHasCaminfo) {
            pCaminfoBuf = (const NSIspTuning::RAWIspCamInfo *)(caminfoMeta.array());
        }
        ...
        MY_LOGD_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s] u1RepeatResult(%d): i4MagicNum(%d)", __FUNCTION__, rNewP2Param.u1RepeatResult, rNewP2Param.i4MagicNum);
        ...
        flowType=rNewP2Param.i4RawType && rNewP2Param.u1PGN;
        MY_LOGD_IF(fgLog,"flowType=%d,rNewP2Param.i4RawType=%d,rNewP2Param.u1PGN =%d",flowType,rNewP2Param.i4RawType,rNewP2Param.u1PGN);
        #endif
        ...
        QUERY_ENTRY_SINGLE(control.appMeta, MTK_CONTROL_MODE,                   rNewP2Param.u1ControlMode);
        ...
        MBOOL bResult3aMetaExif = MFALSE;
        if (pResult){
            if (QUERY_ENTRY_SINGLE(pResult->halMeta, MTK_3A_EXIF_METADATA, rNewP2Param.rexifMeta)) {
                ...
                bResult3aMetaExif = MTRUE;
            }
        }

        if (!bResult3aMetaExif){
...
//=========================================================================================================
        rIspInfo.isCapture = 0
                    ...
                    ;
        extract(&rIspInfo.hint, &control.halMeta);
        rIspInfo.hint.SensorDev = mu4SensorDev;
        ...
        if(_u4LSCDumpEn == 4095)
        {
            ...
            ILscMgr::getInstance(static_cast<MUINT32>(mu4SensorDev))->CCTOPGetSdblkFileCfg(namingPostfix);
            strncpy(rIspInfo.hint.additStr, namingPostfix, 31);
        }

        mpResultPoolObj->lockLastInfo();
        LastInfo_T vLastInfo = mpResultPoolObj->getLastInfo();

        MBOOL OptimizationPreview = (pCaminfoBuf == NULL && vLastInfo.mBackupCamInfo_copied);
        ...
        if (OptimizationPreview)
        {
            MINT32 i4TgWidth = 0;
            MINT32 i4TgHeight = 0;
            MUINT8 u1EffectMode = 0;
            mpHal3aObj->queryTgSize(i4TgWidth,i4TgHeight);
            // Pasre Pass2 module parameter
            QUERY_ENTRY_SINGLE(control.appMeta, MTK_CONTROL_EFFECT_MODE, u1EffectMode);

#if (!CAM3_3A_ISP_30_EN && !CAM3_3A_ISP_40_EN)
            vLastInfo.mBackupCamInfo.rCropRzInfo.i4TGoutW = i4TgWidth;
            vLastInfo.mBackupCamInfo.rCropRzInfo.i4TGoutH = i4TgHeight;
            vLastInfo.mBackupCamInfo.rMapping_Info.eSensorMode = static_cast<ESensorMode_T>(mi4SensorMode);
            vLastInfo.mBackupCamInfo.rMapping_Info.eIspProfile = static_cast<NSIspTuning::EIspProfile_T>(rNewP2Param.u1IspProfile);
#else
            vLastInfo.mBackupCamInfo.rCropRzInfo.i4FullW = i4TgWidth;
            vLastInfo.mBackupCamInfo.rCropRzInfo.i4FullH = i4TgHeight;
            vLastInfo.mBackupCamInfo.eSensorMode = static_cast<ESensorMode_T>(mi4SensorMode);
            vLastInfo.mBackupCamInfo.eIspProfile = static_cast<NSIspTuning::EIspProfile_T>(rNewP2Param.u1IspProfile);
#endif

#if (!CAM3_3A_ISP_30_EN)
                        vLastInfo.mBackupCamInfo.rLCS_Info.rOutSetting = m_LastLCSConfig;
#endif

            vLastInfo.mBackupCamInfo.eIdx_Effect = static_cast<NSIspTuning::EIndex_Effect_T>(u1EffectMode);

            pCaminfoBuf = &vLastInfo.mBackupCamInfo;

            MY_LOGD("[%s] Restore caminfo,copied(%d)/mode(%d)/profile(%d)/FrmId(%d)/TG(%d,%d)",__FUNCTION__, vLastInfo.mBackupCamInfo_copied,
                    static_cast<ESensorMode_T>(mi4SensorMode),
                    static_cast<NSIspTuning::EIspProfile_T>(rNewP2Param.u1IspProfile),
                     rNewP2Param.i4MagicNum, i4TgWidth, i4TgHeight);
        }
        mpResultPoolObj->unlockLastInfo();
        ...
        if (pCaminfoBuf)
        {
            ...
            if(!GET_ENTRY_ARRAY(control.halMeta, MTK_3A_ISP_NR3D_SW_PARAMS, rNewP2Param.NR3D_Data, 14))
                MY_LOG_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s]+ NR3D params get fail",__FUNCTION__);
            ...
            if(!OptimizationPreview)
            {
                ...
                ::memset(&rIspInfo.rCamInfo.rLCS_Info.rOutSetting, 0, sizeof(ISP_LCS_OUT_INFO_T));

                if(!rIspInfo.rCamInfo.bBypassLCE){

                    if (pTuningBuf->pLcsBuf == NULL )
                    {
                         MY_LOGW("[%s] [-No Lcso Buffer ]", __FUNCTION__);
                         rIspInfo.rCamInfo.bBypassLCE = MTRUE;
                    }
                    else
                    {
                        ISP_LCS_OUT_INFO_T *pLCSOut_Info = NULL;
                        IMetadata::Memory LCSOutInfoMeta;
                        QUERY_ENTRY_SINGLE(control.halMeta, MTK_CAMINFO_LCSOUT_INFO, LCSOutInfoMeta);

                        MINT32 i4Size = LCSOutInfoMeta.size();
                        MY_LOG_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s]LCSOutInfoMeta Size %d",__FUNCTION__, i4Size);
                        if(i4Size)
                        {
                            pLCSOut_Info = (ISP_LCS_OUT_INFO_T *)(LCSOutInfoMeta.array());
                            if((pLCSOut_Info->u4OutWidth == 0) || (pLCSOut_Info->u4OutHeight == 0)){

                                MY_LOGW("[%s] [LCSO Size Error]", __FUNCTION__);
                                MY_LOGW("[%s] LCS info: i4FrmId(%d), u4InWidth(%d), u4InHeight(%d), u4OutWidth(%d), u4OutHeight(%d), u4CropX(%d), u4CropY(%d), u4Lrzr1(%d), u4Lrzr2(%d), bFrontBin(%d), u4Qbin_Rto(%d)", __FUNCTION__
                                        , pLCSOut_Info->i4FrmId, pLCSOut_Info->u4InWidth, pLCSOut_Info->u4InHeight, pLCSOut_Info->u4OutWidth, pLCSOut_Info->u4OutHeight, pLCSOut_Info->u4CropX, pLCSOut_Info->u4CropY
                                        , pLCSOut_Info->u4Lrzr1, pLCSOut_Info->u4Lrzr2, pLCSOut_Info->bFrontBin, pLCSOut_Info->u4Qbin_Rto);

                                rIspInfo.rCamInfo.bBypassLCE = MTRUE;
                            }
                            else{
                                ::memcpy(&rIspInfo.rCamInfo.rLCS_Info.rOutSetting, pLCSOut_Info, sizeof(ISP_LCS_OUT_INFO_T));
                            }
                            MY_LOG_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s]LCSOutInfoMeta i4FrmId %d",__FUNCTION__, pLCSOut_Info->i4FrmId);
                        }
                        else
                        {
                            MY_LOGW("[%s] NO LCSOutInfoMeta i4FrmId %d, BypassLCE",__FUNCTION__, rNewP2Param.i4MagicNum);
                            rIspInfo.rCamInfo.bBypassLCE = MTRUE;
                        }
                    }
                }
            }
            MY_LOG_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s] bBypassLCE %d",__FUNCTION__, rIspInfo.rCamInfo.bBypassLCE);
#else
            mpHal3aObj->setISPInfo(rNewP2Param, rIspInfo, 0);
            MY_LOG_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s] bBypassLCE %d",__FUNCTION__, MTRUE);
#endif

            if (rule.isREADEnable("3AHAL"))
            ...
            mpHal3aObj->setP2Params(rNewP2Param, &rResultP2);

#if (!CAM3_3A_ISP_30_EN && !CAM3_3A_ISP_40_EN)
            // set ISP Profile to file naming hint
            rIspInfo.hint.IspProfile = rIspInfo.rCamInfo.rMapping_Info.eIspProfile;
#else
            rIspInfo.hint.IspProfile = rIspInfo.rCamInfo.eIspProfile;
#endif

            MY_LOGD_IF(fgLog, "[%s]+ sensorDev(%d), key(%09d), #(%d), flow(%d), ispProfile(%d), rpg(%d), pTuningBuf(%p)",
                __FUNCTION__, mu4SensorDev, rIspInfo.i4UniqueKey, rIspInfo.rCamInfo.u4Id, flowType, rIspInfo.hint.IspProfile, rIspInfo.rCamInfo.fgRPGEnable, pTuningBuf);

            if (rNewP2Param.u1Exif && pResult) {
                UPDATE_ENTRY_SINGLE(pResult->halMeta, MTK_3A_EXIF_METADATA, rNewP2Param.rexifMeta);
            }

            if (rNewP2Param.u1Exif && mbDbgInfoEnable)
            {
...
//=========================================================================================================
//                      Cannot change the property name of NDD dump using !!!!!!
//=========================================================================================================
            MINT32 dumpAAO        = property_get_int32("vendor.debug.camera.AAO.dump", 0);
            MINT32 dumpAAOPreview = property_get_int32("vendor.debug.camera.AAO.dump.preivew", 0);
//=========================================================================================================
//                      Cannot change the property name of NDD dump using !!!!!!
//=========================================================================================================
            if ((dumpAAO && rIspInfo.isCapture) || dumpAAOPreview)
            {
                ...
                if(_isFileExist(temp))
                {
                    ...
                    if ( result == 0 )
                        MY_LOGD("Renamed success: %s", filename);
                    else
                        MY_LOGD("Renamed fail: %s", temp);
                }
                else
                {
                    MY_LOGD("_dumpAAO E");
                    _dumpAAO(filename, mu4SensorDev, rNewP2Param.i4MagicNum);
                }
            }
            ...
            i4Ret = 0;
                ...
                rNewP2Param.i4P2InImgFmt == 5 // 0 or not exist: RAW->YUV, 1: YUV->YUV, 5: mono RAW->YUV
        )
        {

            MY_LOGD("[%s]  caminfo, mode(%d)/profile(%d)/FrmId(%d)", __FUNCTION__,
                    static_cast<ESensorMode_T>(mi4SensorMode),
                    static_cast<NSIspTuning::EIspProfile_T>(rNewP2Param.u1IspProfile),
                     rNewP2Param.i4MagicNum);

            flowType=0;
            rIspInfo.rCamInfo.bBypassLCE = MTRUE;
            rIspInfo.rCamInfo.i4P2InImgFmt = rNewP2Param.i4P2InImgFmt;
#if (CAM3_3A_ISP_30_EN)
            rIspInfo.rCamInfo.eIspProfile=static_cast<NSIspTuning::EIspProfile_T>(rNewP2Param.u1IspProfile);
#endif
            mpHal3aObj->setISPInfo(rNewP2Param, rIspInfo, 1);
        ...
        {
            MY_LOGE("[%s] NG (no caminfo)", __FUNCTION__);
            i4Ret = -1;
        }
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR i4Arg1, MINTPTR i4Arg2)
{
    if(e3ACtrl == E3ACtrl_SetLcsoParam){
        ISP_LCS_OUT_INFO_T tempLCS_Info = (*(ISP_LCS_OUT_INFO_T*)i4Arg1);
        m_LastLCSConfig = (*(ISP_LCS_OUT_INFO_T*)i4Arg1);
/**************************************************************************************************************
*                                                                                                             *
*                                  Update Result to Meta result Pool - Must using Lock                        *
*                                                                                                             *
**************************************************************************************************************/
        MY_LOGD_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s] Update LCS out info to metadata (#%d)", __FUNCTION__, tempLCS_Info.i4FrmId);
        AllMetaResult_T *pMetaResult = mpResultPoolObj->getMetadata(tempLCS_Info.i4FrmId);
        if(pMetaResult == NULL)
            MY_LOGW("[%s] pMetaResult is NULL", __FUNCTION__);
        else
        {
            Mutex::Autolock lock(pMetaResult->LockMetaResult);
            UPDATE_MEMORY(pMetaResult->rMetaResult.halMeta, MTK_CAMINFO_LCSOUT_INFO, tempLCS_Info);
        }
        return 0;
    }
    else if(e3ACtrl == E3ACtrl_GetSupportedInfo)
    {
        MINT32 i4Ret = MFALSE;
    ...
    else if(e3ACtrl == E3ACtrl_GetFlashCapDummyCnt)
    {
        ...
        MBOOL isLowPower = MFALSE;
        mpHal3aObj->send3ACtrl(E3ACtrl_GetIsFlashLowPower, (MINTPTR)&isLowPower, NULL);
        if(isLowPower){
            cntBefore = 0;
            cntAfter = 0;
            MY_LOGW("[%s] sensordev(%d) no dummy for low power flash.", __FUNCTION__, mu4SensorDev);
        }
...
doNotifyCb(MINT32 _msgType, MINTPTR _ext1, MINTPTR _ext2, MINTPTR _ext3)
{
    ...
    {
        lock_guard<mutex> _lock(mutexParseMeta);
    ...
    case eID_NOTIFY_LCS_ISP_PARAMS:
        ...
        break;
    case eID_NOTIFY_UPDATE_RESULTPOOL_DONE:
        postResultPoolThread(_ext1);
        break;
...
MVOID
Hal3AAdapter3::
postResultPoolThread(MINTPTR _ext1)
{
    MBOOL fgLog = mu4LogEn & HAL3AADAPTER3_LOG_PF;

    // ResultPool - Add command Info
    ResultPoolCmd4Convert_T Cmd;
    Cmd.i4MagicNumReq = (MINT32)_ext1;
    Cmd.eConvertType = E_2A_CONVERT;
    ...
    Cmd.i4SubsampleCount = i4LastSubsampleCount;

    // ResultPool - If modules validate, post thread to convert metadata.
    MINT32 i4Validate = mpResultPoolObj->isValidate(Cmd.i4MagicNumReq);
    MY_LOGD_IF(fgLog, "[%s] (Req, Validate) = (#%d, %d)", __FUNCTION__, Cmd.i4MagicNumReq, i4Validate);
    if(i4Validate)
       mpResultPoolObj->postCmdToThread(&Cmd);
}
...
parseConfig(const ConfigInfo_T& rConfigInfo)
{
    ...
    QUERY_ENTRY_SINGLE(rConfigInfo.CfgHalMeta, MTK_STEREO_WITH_CAMSV, bIsDualCamWithCamSv);
    mParams.bIsDualCamWithCamSv = bIsDualCamWithCamSv;
    ...
    MY_LOGD("[%s] Config Params: HDR mode %d, IspProf %d, MinFps %d, MaxFps %d, CamSv %d"
        , __FUNCTION__, mParams.u1HdrMode, mParams.eIspProfile, mParams.i4MinFps, mParams.i4MaxFps, mParams.bIsDualCamWithCamSv);
...
)
{
    ...
    for (List<IHal3ACb*>::iterator it = m_CallBacks.begin(); it != m_CallBacks.end(); it++)
    {
        IHal3ACb* pCb = *it;
