...
{
    {
        CAM_LOGI("[%s] %s flush + 0", __FUNCTION__, m_strName.c_str());
        Mutex::Autolock lock(m_LockWaiting);
        CAM_LOGI("[%s] %s flush + 1", __FUNCTION__, m_strName.c_str());
        m_bFlush = MTRUE;
    }
    MUINT32 count = 0;
    while(m_bWaiting && count < 3000) // wait 10ms * 1000 = 30s
    {
        CAM_LOGI("[%s] count(%d)", __FUNCTION__, count);
        flushDrv();
...
{
    {
        ...
        if(m_bFlush)
        {
            CAM_LOGI("[%s] %s flush", __FUNCTION__, m_strName.c_str());
            return 0;
        }
        m_bWaiting = MTRUE;
    }
