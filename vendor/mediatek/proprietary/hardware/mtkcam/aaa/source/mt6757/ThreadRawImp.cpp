...
public:
    ...
    virtual MVOID enableFLKThread();
    virtual MVOID disableFLKThread();
    virtual MVOID triggerFLKThread(StatisticBufInfo* rBufInfo);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    /**
    * @brief change FLK thread setting
    */
    virtual MVOID changeFLKThreadSetting();
    /**
    * @brief FLK thread execution function
    */
    static  MVOID*  onFLKThreadLoop(MVOID*);
...
protected: //private:
    ...
    pthread_t       mAFThread;
    pthread_t       mFLKThread;
    MINT32          mbAFThreadLoop;
    MINT32          mbFLKThreadLoop;
    AfStateMgr*     mpAfStateMgr;
    List<ReqQ_T>    mReqQ;
    Condition       mCmdQCond;
    Condition       mFlkCond;
    StatisticBufInfo* mFlkBufInfo;
...
/******************************************************************************
* change FLK thread setting
*******************************************************************************/
MVOID
ThreadRawImp::changeFLKThreadSetting()
{
    // (1) set name
    ::prctl(PR_SET_NAME, "FLKthread", 0, 0, 0);

    // (2) set policy/priority
    {
        int const expect_policy     = SCHED_OTHER;
        int const expect_priority   = NICE_CAMERA_FLK;
        int policy = 0, priority = 0;
        setThreadPriority(expect_policy, expect_priority);
        getThreadPriority(policy, priority);
        //
        MY_LOGD(
            "[Hal3AFlowCtrl::onFLKThreadLoop] policy:(expect, result)=(%d, %d), priority:(expect, result)=(%d, %d)"
            , expect_policy, policy, expect_priority, priority
        );
    }
...
/******************************************************************************
* FLK thread execution function
*******************************************************************************/
MVOID*
ThreadRawImp::onFLKThreadLoop(MVOID *arg)
{

    MY_LOGD("[%s] [FLKThread] tid: %d + \n" ,__FUNCTION__, gettid());

    ThreadRawImp *_this = reinterpret_cast<ThreadRawImp*>(arg);
    _this->changeFLKThreadSetting();

    while(_this->mbFLKThreadLoop)
    {
        Mutex::Autolock autoLock(_this->mFlkMtx);
        _this->mFlkCond.wait(_this->mFlkMtx);
        if(_this->mbFLKThreadLoop == 0)
        {
            continue;
        }

        MINT32 i4AeMode = IAeMgr::getInstance().getAEMode(_this->mi4SensorDev);
        ...
        if(i4AeMode != 0)
        {
            if(_this->mFlkBufInfo == NULL) continue;

            FlickerInput flkIn;
            AE_MODE_CFG_T previewInfo;
            IAeMgr::getInstance().getPreviewParams(_this->mi4SensorDev, previewInfo);
            flkIn.aeExpTime = previewInfo.u4Eposuretime;
            flkIn.afFullStat = IAfMgr::getInstance().getFLKStat(_this->mi4SensorDev);
            flkIn.pBuf = reinterpret_cast<MVOID*>(_this->mFlkBufInfo->mVa);
            FlickerHalBase::getInstance().update(_this->mi4SensorDev, &flkIn, &flkOut);
        }
        else {
            int flkResult;
            FlickerHalBase::getInstance().getFlickerResult(flkResult);
            flkOut.flickerResult = flkResult;
            MY_LOGD_IF(_this->mu4LogEn, "Preview, FLK not first open");
        }

        // Set flicker result to AE
        if(flkOut.flickerResult == HAL_FLICKER_AUTO_60HZ)
        {
            MY_LOGD_IF(_this->mu4LogEn, "setaeflicker 60hz");
            IAeMgr::getInstance().setAEAutoFlickerMode(_this->mi4SensorDev, 1);
        }
        else
        {
            MY_LOGD_IF(_this->mu4LogEn, "setaeflicker 50hz");
            IAeMgr::getInstance().setAEAutoFlickerMode(_this->mi4SensorDev, 0);
        }
        CAM_TRACE_FMT_END();
#endif
    }

    MY_LOGD("[%s] [FLKThread] End - \n", __FUNCTION__);

    return NULL;
}

MVOID
ThreadRawImp::triggerFLKThread(StatisticBufInfo* rBufInfo)
{
    mFlkBufInfo = rBufInfo;
    mFlkCond.signal();
}

/******************************************************************************
* Enable FLK thread when previewStart.
*******************************************************************************/
MVOID
ThreadRawImp::enableFLKThread()
{
    if (mbFLKThreadLoop== 0)
    {
        MY_LOGD("[%s] +", __FUNCTION__);
        mbFLKThreadLoop= 1;

        // create FLK thread
        MY_LOGD("[%s][FLKThread] Create", __FUNCTION__);
        pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_OTHER, NICE_CAMERA_FLK};
        MINT32 result = pthread_create(&mFLKThread, &attr, onFLKThreadLoop, this);
        if(result != 0)
          CAM_LOGE("[%s] result(%d)", __FUNCTION__, result);
    ...
    {
        MY_LOGD("[FLKThread] FLK is working");
    }
}

/******************************************************************************
* Disable FLK thread when previewEnd.
*******************************************************************************/
MVOID
ThreadRawImp::disableFLKThread()
{
    if (mbFLKThreadLoop == 1)
    {
        MY_LOGD("[%s] +", __FUNCTION__);
        mbFLKThreadLoop = 0;
        mFlkCond.broadcast();

        MY_LOGD("[FLKThread] Wait for pthread_join");
        pthread_join(mFLKThread, NULL);

        MY_LOGD("[%s] -", __FUNCTION__);
    } else
    {
        MY_LOGD("[FLKThread] FLK isn't working");
    }
}
...
{
    MY_LOGD("[%s] [AFThread] tid: %d + \n" ,__FUNCTION__, gettid());

    ThreadRawImp *_this = reinterpret_cast<ThreadRawImp*>(arg);
    IEventIrq::Duration duration;
    _this->changeAFThreadSetting();

    _this->mpHal3A->send3ACtrl(E3ACtrl_EnableGyroSensor, MTRUE, 0);
    ...
    _this->mpHal3A->send3ACtrl(E3ACtrl_EnableGyroSensor, MFALSE, 0);
...
*******************************************************************************/
MVOID
ThreadRawImp::enableAFVsyncThread()
{
...
*******************************************************************************/
MVOID
ThreadRawImp::disableAFVsyncThread()
{
...
*******************************************************************************/
MVOID
ThreadRawImp::changeAFVsyncThreadSetting()
{
    // (1) set name
...
*******************************************************************************/
MVOID*
ThreadRawImp::onAFVsyncThreadLoop(MVOID *arg)
{
    MY_LOGD("[%s] [AFVsyncThread] tid: %d + \n" ,__FUNCTION__, gettid());
