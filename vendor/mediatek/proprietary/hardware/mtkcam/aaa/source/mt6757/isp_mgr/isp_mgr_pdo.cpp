...
config( SPDOHWINFO_T &sInPdohwCfg)
{
    ...
    {
        REG_INFO_VALUE(CAM_BPCI_BASE_ADDR) = sInPdohwCfg.phyAddrBpci_tbl;
        REG_INFO_VALUE(CAM_BPCI_XSIZE)     = sInPdohwCfg.u4Bpci_xsz+1;
        REG_INFO_VALUE(CAM_BPCI_YSIZE)     = 1;
        REG_INFO_VALUE(CAM_BPCI_STRIDE)    = sInPdohwCfg.u4Bpci_xsz+1;

        REG_INFO_VALUE(CAM_PDO_XSIZE)      = sInPdohwCfg.i4TGSzW*2; //sInPdoCfg.u4Pdo_xsz;
        REG_INFO_VALUE(CAM_PDO_YSIZE)      = sInPdohwCfg.u4Pdo_ysz+1;
        REG_INFO_VALUE(CAM_PDO_STRIDE)     = _PDO_STRIDE_ALIGN_(sInPdohwCfg.i4TGSzW*2);
    }
...
apply(TuningMgr& rTuning, MINT32 &i4Magic, MINT32 i4SubsampleIdex)
{
    if (m_sPDOHWInfo.u1IsDualPD)
    {
        ...
        CAM_LOGD_IF(m_bDebugEnable, "u1IsDualPD applied!");
    }
    else
    {
    ...
    CAM_LOGD_IF(m_bDebugEnable, "%s, memID(%d)", __FUNCTION__, m_sPDOHWInfo.i4memID);
    return MTRUE;
}
}
