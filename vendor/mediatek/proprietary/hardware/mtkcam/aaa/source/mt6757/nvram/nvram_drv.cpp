...
static void getNvFileName(int dev, int id, char* outName)
{
    ...
    strcat(outName, devSuffix);
}

static int writeNv(int dev, int id, void* buf)
{
    logI("writeNv dev=%d id=%d buf=%p",dev, id, buf);
...
{
    logI("readNv dev=%d id=%d buf=%p",dev, id, buf);
    Mutex::Autolock lock(gNvLock);
    int mask = umask(0);
    logI("umask =%d",mask);
    if(gIsNvInit==0)
...
{
    logI("isNvExist dev=%d id=%d",dev, id);
    int isDemonDone;
    isDemonDone = checkDemonDone();
    logI("isNvExist isDemonDone=%d",isDemonDone);
    Mutex::Autolock lock(gNvLock);
    FILE* fp;
    char fname[MAX_FILE_PATH_LEN];
    getNvFileName(dev, id, fname);
    ...
    if(fp==0)
    {
        logI("isNvExist return 0");
        return 0;
    }
    fclose(fp);
    logI("isNvExist return 1");
    return 1;
}
...
)
{
    ...
    else if (a_eNvramDataType == CAMERA_NVRAM_DATA_FLASH_CALIBRATION)   // strobe
        targetVersion = NVRAM_CAMERA_FLASH_CALIBRATION_FILE_VERSION;


    else
        NVRAM_DRV_ERR("checkDataVersionNew(): incorrect data type\n");
...
unsigned int
NvramDrv::
readModuleIdFromEEPROM(CAMERA_DUAL_CAMERA_SENSOR_ENUM a_eSensorType, int forceWait)
{
                          ...
                          int version)
{
    ...
    {
        err = readDefaultData(a_eSensorType, u4SensorID, a_eNvramDataType, a_pNvramData);
        NVRAM_DRV_LOG("[readNvrameEx] CAMERA_DATA_PDC_TABLE err(%d)", err);
        if (err != NVRAM_NO_ERROR)
...
//    static bool bCustomInit = 0; //[ALPS00424402] [CCT6589] Len shading page --> Save to NVRAM --> CCT reboot failed
    ...
    case CAMERA_NVRAM_DATA_FLASH_CALIBRATION:
        ...
        ret_flashCalibration = cust_fillDefaultFlashCalibrationNVRam(a_eSensorType, a_pNvramData);
        break;
    ...
    case CAMERA_DATA_PDC_TABLE:
        ...
        {
            NVRAM_DRV_LOG("[readDefaultData] NVRAM_DEFAULT_DATA_READ_ERROR\n");
            return NVRAM_DEFAULT_DATA_READ_ERROR;
        }
        break;
    case CAMERA_NVRAM_DATA_FEATURE:
