...
MBOOL
IAwbMgr::
setSensorMode(MINT32 i4SensorDev, MINT32 i4NewSensorMode, int w, int h)
{
    MBOOL ret_main, ret_sub, ret_main2;
    ret_main = ret_sub = ret_main2 = MTRUE;

    if (i4SensorDev & ESensorDev_Main)
        ret_main = AwbMgr::getInstance(ESensorDev_Main).setSensorMode(i4NewSensorMode, w, h);
    if (i4SensorDev & ESensorDev_Sub)
        ret_sub = AwbMgr::getInstance(ESensorDev_Sub).setSensorMode(i4NewSensorMode, w, h);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AwbMgr::getInstance(ESensorDev_MainSecond).setSensorMode(i4NewSensorMode, w, h);
