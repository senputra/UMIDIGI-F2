...
{
public:
                            LscImemBuf(MUINT32 sensorDev, MUINT32 u4Id, const char* strName);
    virtual                 ~LscImemBuf();
...
showInfo() const
{
    LSC_LOG("sensor(0x%02x), name(%s), id(%d), memID(%d), virtAddr(%p), phyAddr(%p), size(%d), Img(%dx%d), Blk(%d,%d,%d,%d,%d,%d), Rto(%d)",
...
allocBuf(MUINT32 x_size, MUINT32 y_size)
{
    ...
    LSC_LOG("sensor(0x%02x), name(%s), id(%d), virtAddr(%p), phyAddr(%p), size(%d)",
...
freeBuf()
{
    ...
    m_pImgBuf = NULL;
    //
    LSC_LOG("sensor(0x%02x), name(%s), id(%d), virtAddr(%p), phyAddr(%p)", m_u4SensorDev, m_strName.c_str(), m_u4Id, pVirAddr, u4PhyAddr);
    return MTRUE;
}
...
setConfig(ILscBuf::Config rConfig)
{
    ...
    if (u4Size != getSize())
    {
        ...
        else
        {
            ...
            {
                LSC_LOG("sensor(0x%02x), id(%d), virtAddr(%p), phyAddr(%p), size(%d)",
...
ILscBuf(MUINT32 sensorDev, MUINT32 u4Id, MBOOL usecache, const char* strName, E_BUF_MODE_T eBufMode)
{
    ...
    {
        m_pImp = new LscImemBuf(sensorDev, u4Id, strName);
    }
    else
    {
