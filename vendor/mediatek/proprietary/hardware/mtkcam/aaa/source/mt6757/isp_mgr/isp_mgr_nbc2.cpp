...
get(ISP_NVRAM_ANR2_T& rParam, const dip_x_reg_t* pReg) const
{
    if (pReg)
    {
        ...
        GET_REG_INFO_BUF(DIP_X_ANR2_Y4LUT1, lut1);
        GET_REG_INFO_BUF(DIP_X_ANR2_Y4LUT2, lut2);
        GET_REG_INFO_BUF(DIP_X_ANR2_Y4LUT3, lut3);
        GET_REG_INFO_BUF(DIP_X_ANR2_L4LUT1, llut1);
        GET_REG_INFO_BUF(DIP_X_ANR2_L4LUT2, llut2);
        GET_REG_INFO_BUF(DIP_X_ANR2_L4LUT3, llut3);
        ...
        GET_REG_INFO_BUF(DIP_X_ANR2_ACTY, acty);
...
{
    MBOOL bANR2_ENC = reinterpret_cast<ISP_DIP_X_ANR2_CON1_T*>(REG_INFO_VALUE_PTR(DIP_X_ANR2_CON1))->ANR2_ENC;
    MBOOL bANR2_ENY = reinterpret_cast<ISP_DIP_X_ANR2_CON1_T*>(REG_INFO_VALUE_PTR(DIP_X_ANR2_CON1))->ANR2_ENY;

    MBOOL bCCR_EN = reinterpret_cast<ISP_DIP_X_CCR_CON_T*>(REG_INFO_VALUE_PTR(DIP_X_CCR_CON))->CCR_EN;

    MBOOL bNBC2_EN = ((isANR2Enable() & (bANR2_ENY|bANR2_ENC)) |
                      (isCCREnable() & bCCR_EN));
                       //isBOKEnable());
    ...
    if (!bNBC2_EN && ISP_MGR_SL2_T::getInstance(m_eSensorDev).getSL2COnOff()){

        ISP_WRITE_ENABLE_BITS(pReg, DIP_X_CTL_YUV_EN, SL2C_EN, bNBC2_EN);
        //ISP_MGR_SL2_T::getInstance(m_eSensorDev).setSL2CEnable(MFALSE);
    }
