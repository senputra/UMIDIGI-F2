...
dequeueHwBuf()
{
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        userKey = m_pSttPipe->attach(LOG_TAG);
        m_pSttPipe->sendCommand(ESPipeCmd_SET_TG_INT_LINE, (MINTPTR)m_i4Tglineno, 0, 0);
        MUINT32 nRet = m_pSttPipe->wait(EPipeSignal_TG_INT, EPipeSignal_ClearWait, userKey, timeout);
        if(nRet == MFALSE) {
        ...
        CAM_LOGD_IF(m_bDebugEnable,"[%s] S_%d: dma(x%x)cur_addr(0x%x)\n", __FUNCTION__, m_i4SensorDev, m_rPort.index, rBufAddr);
        CAM_TRACE_END();
