...
MINT32 LcsHalImp::ConfigLcsHal(const LCS_HAL_CONFIG_DATA &aConfigData)
{
    ...
    err = m_pLcsDrv->Init(mLcs_outW, mLcs_outH);
    if(err != LCS_RETURN_NO_ERROR)
    {
...
*
********************************************************************************/
MVOID LcsHalImp::LcsSyncISP(MINT32 i4FrmId, MINTPTR pLCSInfo)
{
        MINT32 err = NO_ERROR;
        if(m_pLcsDrv != NULL)
            m_pLcsDrv->SetLCSInfo(i4FrmId, pLCSInfo);
}
