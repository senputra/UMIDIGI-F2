...
sendIntent(intent2type<eIntent_VsyncUpdate>)
{
    ...
    CAM_LOGD("[StatePrecapture::sendIntent]<eIntent_VsyncUpdate> line=%d, frameCnt(%d), EPrecapState(%d), #(%d)"
    , __LINE__, u4FrmCnt, static_cast<int>(ePrecapState), u4MagicNum);
...
sendPrecapIntent(intent2type<eIntent_VsyncUpdate>, state2type<ePrecap_AE>, MVOID* pBufInfo)
{
    ...
    if(Hal3ASttCtrl::getInstance(m_SensorDevId)->isMvHDREnable())
    {
        ...
        IAeMgr::getInstance().doPreCapAE(m_SensorDevId, m_pStateMgr->getFrameCount(), bIsStrobeFired, reinterpret_cast<MVOID *>(rMvHDRBufInfo->mVa), 0, MTRUE, 0);
    } else
    {
