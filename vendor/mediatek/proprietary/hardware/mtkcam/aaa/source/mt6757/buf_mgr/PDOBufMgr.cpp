...
dequeueHwBuf()
{
    ...
    CAM_LOGD_IF(m_bDebugEnable, "port(%d), index(%d), va[0x%x]/pa[0x%08x]/#(%d), Size(%d), Stride(%d)",
              m_rPort.index, index, rHwBuf.mVa, rLastBuf.mPa, rHwBuf.mMagicNumber, rHwBuf.mSize, rHwBuf.mStride);

    // dump pdo data for debug
    if (bEnable) {
        char fileName[64];
        static MUINT32 count;
        sprintf(fileName, "/sdcard/pdo/pdo_%d_%d.raw", rLastBuf.mMetaData.mMagicNum_tuning, count++);
