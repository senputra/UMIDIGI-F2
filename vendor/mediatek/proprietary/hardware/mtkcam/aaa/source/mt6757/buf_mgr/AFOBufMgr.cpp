...
dequeueHwBuf()
{
    ...
    if(m_pSttPipe != NULL)
    {
        ...
        m_rHwBuf.mMagicNumber = rLastBuf.mMetaData.mMagicNum_tuning;
        ...
        CAM_LOGD_IF(m_bDebugEnable, "port(%d), va[%p]/pa[0x%08x]/#(%d), Size(%d)",
                  m_rPort.index, m_rHwBuf.mVa, rLastBuf.mPa, m_rHwBuf.mMagicNumber, m_rHwBuf.mSize);

        // dump afo data for debug
        if (bEnable) {
            char fileName[64];
            static MUINT32 count;
            sprintf(fileName, "/sdcard/afo/afo_%d_%d.raw", rLastBuf.mMetaData.mMagicNum_tuning, count++);
