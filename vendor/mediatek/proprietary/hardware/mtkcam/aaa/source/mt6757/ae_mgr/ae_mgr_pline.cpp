    ...
    if (m_eAEScene != eAEScene) {
        ...
        if(m_pIAeAlgo != NULL) {
            ...
            if(m_bEnAutoTest) {
                ...
                m_u4ATType = AE_AT_TYPE_SCENE;
                AE_LOG("AE Auto Test [AE_AT_TYPE_SCENE]: m_u4ATYval(%d) thresh(%d) m_u4ATType(%d)\n", m_u4ATYval, AEAutoTestThreshTable[m_u4ATType], m_u4ATType);
            }
    ...
    if (m_u4AEISOSpeed != eAEISOSpeed) {
        ...
        if(m_pIAeAlgo != NULL) {
            ...
            if(m_bEnAutoTest) {
                m_u4ATCnt = AE_AT_MAX_CNT_NUM;
                m_u4ATYval = m_pIAeAlgo->getBrightnessAverageValue();
                m_u4ATType = AE_AT_TYPE_ISO;
                AE_LOG("AE Auto Test [AE_AT_TYPE_ISO]: m_u4ATYval(%d) thresh(%d) m_u4ATType(%d)\n", m_u4ATYval, AEAutoTestThreshTable[m_u4ATType], m_u4ATType);
            }
...
{
    AE_LOG("[setAEFlickerModePlineIdx] Set Flicker value %d \n", a_eAEFlickerMode);
    setIsoSpeed(m_u4AEISOSpeed);
    return S_AE_OK;
}
...
{
    AE_LOG("[setAEFlickerAutoModePlineIdx]:%d \n", a_eAEFlickerAutoMode);
    setIsoSpeed(m_u4AEISOSpeed);
    return S_AE_OK;
}
...
MRESULT AeMgr::setAETable(eAETableID a_eAEPreTableID, eAETableID a_eAECapTableID)
{
    ...
    for (MINT32 i = 0; i<(MUINT32)MAX_PLINE_TABLE ; i++){
    ...
    *pAEPrePLineID = m_pAEMapPlineTable[0].ePLineID[LIB3A_SENSOR_MODE_PRVIEW];
    ...
    for(i=0; i<MAX_MAPPING_PLINE_TABLE; i++) {
        ...
        if(m_pAEMapPlineTable[i].eAEScene == a_eAESceneID) {
            ...
            AE_LOG("[getAEPLineMappingID] SceneID:%d Pre:%d CapID:%d\n", a_eAESceneID, *pAEPrePLineID, *pAECapPLineID);
            return S_AE_OK;
        }
    }
    return S_AE_OK;
...
MRESULT AeMgr::searchPreviewIndexLimit( )
{
    ...
    MINT32   itempIndexcur = m_u4Index;
    ...
    if(m_u4IndexMax > m_u4IndexMin){
        ...
        if(itemp >= (MINT32)m_pPreviewTableNew->u4TotalIndex) {   //capture range bigger then preview
            ...
            AE_LOG("[searchPreviewIndexLimit] Preview table index  %d~%d m_u4Index:%d Delta:%d itemp:%d \n",m_u4IndexMin, m_u4IndexMax, m_u4Index, iSenstivityDeltaIdx, itemp);
...
MRESULT AeMgr::setAEMinMaxFrameRatePlineIdx(MINT32 a_eAEMinFrameRate, MINT32 a_eAEMaxFrameRate)
{
    ...
    AE_LOG("[setAEMinMaxFrameRatePlineIdx:s] MaxFps/MaxIdx/MinIdx/Idx/Fps :%d/%d/%d/%d/%d \n",a_eAEMaxFrameRate, m_u4IndexMax, m_u4IndexMin, m_u4Index, i4fps);
  ...
  AE_LOG("[setAEMinMaxFrameRatePlineIdx:e] MinFps/MaxIdx/MinIdx/Idx/SearchIdx/Searchfps: %d/%d/%d/%d/%d/%d \n",m_i4AEMinFps, m_u4IndexMax, m_u4IndexMin, m_u4Index, i4PlineIndex, i4fps);
  return  S_AE_OK;
}
...
MRESULT AeMgr::setAEMaxFrameRateGainIndexRange()
{
  ...
  i430fps = 300;
  AE_LOG_IF(m_3ALogEnable,"Set Gain Index Range for LCE");
  ...
  for(i4PlineIndex = (MINT32)m_u4IndexMin; i4PlineIndex<= (MINT32)m_u4IndexMax ; i4PlineIndex++) {
    if(m_pPreviewTableNew != NULL) {
      if(m_pPreviewTableNew->pCurrentTable->sPlineTable[i4PlineIndex].u4Eposuretime > 0) {
        ...
        AE_LOG("[setAEMaxFrameRateGainIndexRange] The exposure time is zero\n");
        break;
      }
      ...
      AE_LOG("The preview new Pline table is NULL");
    }
  }

  AE_LOG_IF(m_3ALogEnable,"[setAEMaxFrameRateGainIndexRange]  m_u4LCEGainStartIdx:%d m_u4LCEGainEndIdx:%d", m_u4LCEGainStartIdx, m_u4LCEGainEndIdx);
  return  S_AE_OK;
}
...
MRESULT AeMgr::setManualAETable(MUINT32 a_AEPreTableID, MUINT32 a_AECapTableID)
{
    ...
    switch(a_AEPreTableID) {
        ...
        case AE_PLINE_STEREO_VIDEO:
            ...
            break;
        default:
            AE_LOG("[setManualAETable] The Preview AE table enum value is incorrectly:%d\n", a_AEPreTableID);
    ...
    switch(a_AECapTableID) {
        ...
        case AE_PLINE_STEREO_VIDEO:
            ...
            break;
        default:
            AE_LOG("[setManualAETable] The Capture AE table enum value is incorrectly:%d\n", a_AECapTableID);
...
{
    AE_LOG("[updateAEidxtoExpsetting:s] m_u4Index/m_i4DeltaSensitivityIdx/m_i4DeltaBVIdx : %d/%d/%d\n", m_u4Index, m_i4DeltaSensitivityIdx, m_i4DeltaBVIdx);

    if(m_pPreviewTableCurrent->pCurrentTable != NULL){
        if(m_bAdbAEPreviewUpdate) {
            m_rAEOutput.rPreviewMode.u4Eposuretime = m_u4AdbAEShutterTime;
            m_rAEOutput.rPreviewMode.u4AfeGain = m_u4AdbAESensorGain;
            m_rAEOutput.rPreviewMode.u4IspGain = m_u4AdbAEISPGain;
        } else {
    ...
    MUINT32 u4PreviewBaseISO = m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4MiniISOGain;
    ...
    AE_LOG("[updateAEidxtoExpsetting:e] m_u4Index/Exp/Afe/Isp/ISO : %d/%d/%d/%d/%d \n", m_u4Index, m_rAEOutput.rPreviewMode.u4Eposuretime,m_rAEOutput.rPreviewMode.u4AfeGain, m_rAEOutput.rPreviewMode.u4IspGain , m_rAEOutput.rPreviewMode.u4RealISO);
    return S_AE_OK;
}
