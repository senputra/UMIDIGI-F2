...
public:
    ...
    virtual MUINT32     queryMagicNumber() const {return 0;}
...
config()
{
    if (m_pCamIO == NULL)
    {
        ...
        ISP_MGR_AWB_STAT_CONFIG_T::getInstance(static_cast<ESensorDev_T>(m_u4SensorDev)).config(rTmpAwb, 0, m_i4QBINSzW, m_i4QBINSzH, blockwnum, blockhnum);
        ISP_MGR_AF_STAT_CONFIG_T::getInstance(static_cast<ESensorDev_T>(m_u4SensorDev)).config(testcfg, sArea_HW, i4HWBlkNumX, i4HWBlkNumY, i4HWBlkSizeX, i4HWBlkSizeY);
...
MINT32
Dft3AImp::
getCurrResult(MUINT32 i4FrmId, Result_T& rResult, MINT32 i4SubsampleCount) const
{
    CAM_LOGD("[%s]", __FUNCTION__);
    return 0;
}
...
postCommand(ECmd_T const r3ACmd, const ParamIspProfile_T* pParam)
{
    if (ECmd_Update == r3ACmd)
    {
        ...
        {
            const StatisticBufInfo* pAABuf = Hal3ASttCtrl::getInstance(m_u4SensorDev)->getBufMgr(BUF_AAO)->dequeueSwBuf();
            sprintf(strFile, "/sdcard/dft3a_aao/aao_%d", m_u4Count);
            pAABuf->dump(strFile);
        }
        if (enable & ENABLE_STT_FLOW_AFO)
        {
            const StatisticBufInfo* pAFBuf = Hal3ASttCtrl::getInstance(m_u4SensorDev)->getBufMgr(BUF_AFO)->dequeueSwBuf();
