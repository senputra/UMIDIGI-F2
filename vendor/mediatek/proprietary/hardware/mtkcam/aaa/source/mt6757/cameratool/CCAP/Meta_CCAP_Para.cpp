...
{
  // AWB NVRAM test data
    AWB_NVRAM_T rAWBNVRAMTestData[CAM_SCENARIO_NUM];

    memcpy(&rAWBNVRAMTestData,&(pREQ->cmd.dev_awb_nvram_info.rAWBNVRAM),sizeof(AWB_NVRAM_T)*CAM_SCENARIO_NUM);
    ...
    if(!bSendDataToCCT(ACDK_CCT_V2_OP_AWB_APPLY_CAMERA_PARA2,
                        ...
                        sizeof(AWB_NVRAM_T)*CAM_SCENARIO_NUM,
                        NULL,
                        0,
                        &u4RetLen))
    {
...
{
    AWB_NVRAM_T rAWBNVRAM[CAM_SCENARIO_NUM]; // TEST ONLY
