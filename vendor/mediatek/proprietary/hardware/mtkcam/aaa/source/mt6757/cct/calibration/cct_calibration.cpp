...
VOID AcdkCalibration:: vCaptureCallBack(VOID *a_pParam)
{
    ...
    if ((prImgBufInfo->eImgType >= PURE_RAW8_TYPE) && (prImgBufInfo->eImgType <= PROCESSED_RAW10_TYPE))    //webber check later
    {
        ...
        {
            char filename[128];
            sprintf(filename, "/data/cct/Raw_%dx%d.raw", m_u2RawImgWidth, m_u2RawImgHeight);
...
/////////////////////////////////////////////////////////////////////////
MRESULT AcdkCalibration::init(MINT32 dev, AcdkCctBase *a_pAcdkCctBaseObj)
{
...
MRESULT AcdkCalibration::takePicture(MUINT32 a_i4SensorMode, eACDK_CAP_FORMAT type)
{
    ...
    m_bCapDone = MFALSE;
//    mrRet = m_pAcdkCctBaseObj->takePicture(a_i4SensorMode, (MUINT32)type, vCapCb);
    CCTIF_FeatureCtrl(ACDK_CCT_OP_AF_DISABLE, NULL, 0, NULL, 0, &u4RetLen);
    mrRet = m_pAcdkCctBaseObj->takePicture_CameraDev(a_i4SensorMode, (MUINT32)type, vCapCb);
...
MRESULT AcdkCalibration::mrAnalyzeRAWImage()
{
    ...
    */
    if (m_pAcdkImgToolObj == NULL || m_pAcdkCctBaseObj == NULL)
    {
                                        ...
                                        ACDK_CDVT_SENSOR_TEST_OUTPUT_T *a_prSensorTestOutput)
{
  ...
  ACDK_LOGD("a_i4SensorMode = %d\n", a_i4SensorMode);
    ...
    */
    if (m_pAcdkImgToolObj == NULL || m_pAcdkCctBaseObj == NULL)
    {
        ACDK_LOGE(" Null m_pAcdkImgToolObj || m_pAcdkCctBaseObj\n");
        return E_CCT_CALIBRATION_NULL_OBJ;
    }
...
//    in->rAEPlinetableInfo.eSensorMode
    m_pAcdkCctBaseObj->startPreview(NULL);

    ACDK_LOGD("[Disable 3A]\n");
    m_pAcdkCctBaseObj->sendcommand(ACDK_CCT_V2_OP_AWB_DISABLE_AUTO_RUN, NULL, 0, NULL, 0, &u4RetLen);
    CCTIF_FeatureCtrl(ACDK_CCT_OP_AF_DISABLE, NULL, 0, NULL, 0, &u4RetLen);
...
Exit:
    ...
    ACDK_LOGD("[Restore 3A]\n");
    m_pAcdkCctBaseObj->sendcommand(ACDK_CCT_OP_AF_ENABLE, NULL, 0, NULL, 0, &u4RetLen);
    m_pAcdkCctBaseObj->sendcommand(ACDK_CCT_V2_OP_AWB_ENABLE_AUTO_RUN, NULL, 0, NULL, 0, &u4RetLen);
                                     ...
                                     ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
  ...
  prSensorCalibrationOutput->i4ErrorCode = 0; // No error
    ...
    */
    if((backup_bypass = NSIspTuningv3::IspTuningMgr::getInstance().getDynamicBypass(m_SensorDevId)) == -1)
    {
        ACDK_LOGE("[getDynamicBypass] Get Dynamic Bypass Fail \n");
        prSensorCalibrationOutput->i4ErrorCode = 1; // RAW analysis error
        goto Exit;
    }
    NSIspTuningv3::IspTuningMgr::getInstance().setDynamicBypass(m_SensorDevId, MTRUE);
                                                    ...
                                                    ACDK_CDVT_SENSOR_CALIBRATION_OUTPUT_T *prSensorCalibrationOutput)
{
  ...
  DOUBLE dGainBuffer = ((DOUBLE)a_i4GainBuffer)/100;
   ...
   // determine ExpStart, ExpEnd, and ExpInterval
    ...
    if (i4SequenceNo < 10)
    {
        ACDK_LOGE("[i4SequenceNo < 13], Luminance level is too bright \n");
        prSensorCalibrationOutput->i4ErrorCode = 3; // Luminance level is too bright
        goto Exit;
    }
...
//!
/////////////////////////////////////////////////////////////////////////
MRESULT SetShadingAE(AcdkCctBase *m_pAcdkCctBaseObj, MINT32 i4SensorDev)
{
    MRESULT mrRet = S_CCT_CALIBRATION_OK;
    ...
    AE_NVRAM_T pAENVRAM;
    MUINT32 gLength = 0;
    NS3Av3::IAeMgr::getInstance().CCTOPAEGetNVRAMParam(i4SensorDev, (MVOID *)&pAENVRAM, &gLength);
...
MRESULT AcdkCalibration::mrCalLenShading(INT32 a_i4XINIBorder, INT32 a_i4XENDBorder, INT32 a_i4YINIBorder, INT32 a_i4YENDBorder,  UINT16 a_u2AttnRatio, UINT32 a_u4Index, INT32 a_i4SensorMode, UINT8 a_u1FixShadingIndex, INT32 a_i4AvgWinSize)
{
    ...
    ACDK_LOGD(" XS(%d) XE(%d) YS(%d) YE(%d) a_u2AttnRatio(%d) Ct(%d)  SensorMode(%d) FixShadingIndex(%d) WinSize(%d)",
    ...
    if (SetShadingAE(m_pAcdkCctBaseObj, m_SensorDevId) != S_CCT_CALIBRATION_OK)
        goto Exit;
...

    {
        ...
        ACDK_LOGD("Attenuation coef[%d]: (%8f, %8f, %8f, %8f ,%8f, %8f)", a_u2AttnRatio,
        ...
        rTblFloat.setConfig(m_u2RawImgWidth, m_u2RawImgHeight, rLSCCaliINIParam.i4GridXNUM, rLSCCaliINIParam.i4GridYNUM);
        rTblFloat.setData(g_lsc_cali_info.tbl_info.src_tbl_addr_float, rTblFloat.getSize());
        char strFile[512] = {'\0'};
        sprintf(strFile, "/sdcard/shading/KLSC_F_CFG(%dx%d_%dx%d)_BD(%d,%d,%d,%d)_ATT(%d)_WIN(%d)_IDX(%d).log",
            rLSCCaliINIParam.i4GridXNUM, rLSCCaliINIParam.i4GridYNUM, m_u2RawImgWidth, m_u2RawImgHeight,
            a_i4XINIBorder, a_i4XENDBorder, a_i4YINIBorder, a_i4YENDBorder, a_u2AttnRatio, a_i4AvgWinSize, a_u4Index);
        rTblFloat.dump(strFile);
        sprintf(strFile, "/sdcard/shading/KLSC_G_CFG(%dx%d_%dx%d)_BD(%d,%d,%d,%d)_ATT(%d)_WIN(%d)_IDX(%d).log",
                    rLSCCaliINIParam.i4GridXNUM, rLSCCaliINIParam.i4GridYNUM, m_u2RawImgWidth, m_u2RawImgHeight,
                    a_i4XINIBorder, a_i4XENDBorder, a_i4YINIBorder, a_i4YENDBorder, a_u2AttnRatio, a_i4AvgWinSize, a_u4Index);
