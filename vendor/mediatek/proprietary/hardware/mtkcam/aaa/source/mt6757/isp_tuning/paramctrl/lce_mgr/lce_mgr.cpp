    ...
    , m_ePrev_Tuning_Set(MTKLCE_TUNING_SET_Preview)
{
    ...
    CAM_LOGD_IF(1, "[%s()] m_pLceAlgo(%p), m_eSensorDev(%d), m_pIspNvramLcePara(%d)", __FUNCTION__, m_pLceAlgo, m_eSensorDev, m_pIspNvramLcePara);
                   ...
                   ISP_NVRAM_LCS_T const& p1Lcs, ISP_NVRAM_PGN_T const& p2Pgn, ISP_NVRAM_CCM_T const& p2CCM, MUINT16* pLcsBuffer)
{
    ...
    int coverity = eIspProfile;
    mInInfo.eLceProfile = (eLCE_PROFILE_T)coverity;

    mInInfo.rLCELCSInfo.u4LightMap = pLcsBuffer;
    mInInfo.rLCELCSInfo.u4Width = ISP_LCS_OUT_WD;
    mInInfo.rLCELCSInfo.u4Height= ISP_LCS_OUT_HT;
    ...
    if (mAlgoEn)
    {
        ...
        if (mLogEn)
        {
            CAM_LOGD("%s: LCE EXIF: %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x; %x/%x/%x/%x/%x"
                , __FUNCTION__
