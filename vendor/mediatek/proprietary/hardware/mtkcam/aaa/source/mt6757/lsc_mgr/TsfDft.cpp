...
init(ESensorMode_T eFullSensorMode)
{
    ...
    GET_PROP("vendor.debug.lsc_mgr.cfg", "0", m_u1CfgOpt);
    ...
    m_pTsfIn = new ILscBuf(m_u4SensorDev, id, strName, ILscBuf::E_LSC_IMEM);
    ...
    m_pTsfOut = new ILscBuf(m_u4SensorDev, id, strName, ILscBuf::E_LSC_IMEM);
...
tsfReformTbl()
{
        ...
        ((rTrfmA.u4Y + rTrfmA.u4H) >= (rTrfmB.u4Y + rTrfmB.u4H)))
    {
        ...
        for (i = 0; i < RING_TBL_NUM; i++)
        {
            ...
            rTbl.cropOut(rTrfmRel, m_rLscRingTbl[i]);
        }
        return MTRUE;
    }
    else
...
tsfInit()
{
    ...
    else
    {
        ...
        m_rTsfEnvInfo.buf_tsfi.VA = NULL;
        m_rTsfEnvInfo.buf_tsfi.PA = NULL;
    ...
    else
    {
        ...
        m_rTsfEnvInfo.buf_tsfo.VA = NULL;
        m_rTsfEnvInfo.buf_tsfo.PA = NULL;
...
doThreadFunc()
{
    ...
    while (m_fgThreadLoop)
    {
        ...
        if (E_LSC_TSF_TYPE_0 == getType())
        {
            ...
            {
                LSC_ERR("TSF Error #(%d), rto(%d), tbl_idx(%d)", rStat.u4FrmId, rStat.u4Ratio, u4TblIdx);
            }
        }
        m_u4ProcCnt++;
        ::sem_post(&m_SemaExecDone);
...
getTbl(MUINT32 u4GridX, MUINT32 u4GridY, MUINT32 u4MagicNum, ILscTbl& output)
{
    ...
    for (i = 0; i < RING_TBL_NUM; i++)
    {
        if (m_rStat[i].u4FrmReq == u4MagicNum)//result magic
        {
            ...
            if (m_rLscRingTbl[i].cropOut(ILscTable::TransformCfg_T(width, height, u4GridX, u4GridY, 0, 0, width, height), output))
            {
                fgRet = MTRUE;
            }
            else
