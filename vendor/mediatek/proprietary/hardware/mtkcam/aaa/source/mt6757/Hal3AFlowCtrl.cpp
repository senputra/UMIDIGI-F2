...
postCommand(ECmd_T const eCmd, const ParamIspProfile_T* pParam)
{
    ...
    {
        // For turn off flash in leaving recording state
        m_u4FlashOnIdx = -1;
...
updateResult(MUINT32 u4MagicNum, MUINT32 u4MagicNumCur)
{
    ...
    m_rResultBuf.updateRepeatQueue(u4MagicNum);
    AAA_TRACE_END_L;
    ...
    if (!rResultNew.fgKeep)
    {
        ...
        AAA_TRACE_L(UpdateResultBuf);
        m_rResultBuf.updateResult(u4MagicNum, rResultNew);
        AAA_TRACE_END_L;
    }
    else
    {
    ...
    m_rResultBufCur.updateResult(u4MagicNumCur, rResultNew);
    AAA_TRACE_END_L;
    MY_LOGD_IF(m_fgLogEn, "[%s] u4MagicNumCur(%d) HQC", __FUNCTION__, u4MagicNumCur);
    AAA_TRACE_L(Broadcast);
...
getResultCur(MUINT32 i4FrmId, Result_T& rResult)
{
    ...
    if (-1 == i4Ret)
    {
        ...
        if (-1 != m_rResultBuf.getResult(i4FrmId, rResultAtStart))
        {
            ...
            if(!isStartEmpty && !isCurEmpty)
            {
                ...
                AAA_DEBUG_INFO_T& rDbg3AInfoStart = *reinterpret_cast<AAA_DEBUG_INFO_T*>(rResultAtStart.vecDbg3AInfo.editArray());
                AAA_DEBUG_INFO_T& rDbg3AInfoCur = *reinterpret_cast<AAA_DEBUG_INFO_T*>(rResult.vecDbg3AInfo.editArray());
...
//    updateImmediateResult(u4MagicNum);
    if (u4MagicNum >= m_u4FlashOnIdx && !m_p3AWrap->chkPreFlashOnCond() && !m_p3AWrap->chkMainFlashOnCond())
    {
        m_p3AWrap->setFlashLightOnOff(0, 1); // don't care main or pre
        m_u4FlashOnIdx = -1;
    }

    if (u4MagicNum >= m_u4MainFlashOnIdx && m_u4MainFlashOnIdx != -1)
    {
        m_u4MainFlashOnIdx = -1;
    }
...
{
    return m_rResultBuf.queryRepeatQueue(maginNum);
}
