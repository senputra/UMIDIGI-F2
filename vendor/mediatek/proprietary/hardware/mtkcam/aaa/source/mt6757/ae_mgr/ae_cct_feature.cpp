...
)
{
    ...
    if (pAEExpParam->u4GainMode == 0) { // AfeGain and isp gain
        ...
        if(m_p3ANVRAM != NULL) {
            u4AFEGain = (pAEExpParam->u4ISO * 1024) / m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4MiniISOGain;
        } else {
    ...
    if(m_p3ANVRAM != NULL) {
        if(m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4Cap2PreRatio <= 300) {
            u4BinningRatio = 4;
        } else if(m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4Cap2PreRatio <= 450) {
            u4BinningRatio = 3;
        } else if(m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4Cap2PreRatio <= 768) {
            u4BinningRatio = 2;
        } else {
            u4BinningRatio = 1;
        }
...
)
{
    ...
    if (pcfg_in->u4GainMode == 0) { // AfeGain and isp gain
        ...
        if(m_p3ANVRAM != NULL) {
            cap_mode.u4AfeGain = (pcfg_in->u4ISO * 1024) / m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4MiniISOGain;
        } else {
            CAM_LOGD("NVRAM is NULL\n");
...
)
{
    ...
    if(m_p3ANVRAM != NULL) {
        pAEExpParamOut->u4ISO = m_rAEOutput.rPreviewMode.u4IspGain*(((m_rAEOutput.rPreviewMode.u4AfeGain * m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4MiniISOGain) + 512) / 1024) / 1024;
    } else {
        CAM_LOGD("NVRAM is NULL\n");
...
{
    CAM_LOGD("[ACDK_CCT_OP_DEV_AE_GET_INFO]\n");
...
)
{
    ...
    m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx] = m_rAEInitInput.rAENVRAM;
...
)
{
    ...
    if(a_CaptureMode == 0) {   // preview mode
        u4SensitivityRatio = m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4Cap2PreRatio;
    } else if(a_CaptureMode == 2) {
        u4SensitivityRatio = 1024 *m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4Cap2PreRatio  / m_p3ANVRAM->rAENVRAM[m_eAENVRAMIdx].rDevicesInfo.u4Video2PreRatio;
    }
