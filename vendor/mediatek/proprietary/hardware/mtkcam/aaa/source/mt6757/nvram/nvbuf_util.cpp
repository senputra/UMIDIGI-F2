...
DEF_CriticalSection_Win(m_cs);
    ...
    #define logE(fmt, arg...)    ALOGE(LOG_TAG "MError: ln=%d: " fmt, __LINE__, ##arg)
#endif
...
static void getNvFileName(int dev, int id, char* outName)
{
    ...
    // else if(dev==int(DUAL_CAMERA_SUB_2_SENSOR))
    //     sprintf(devSuffix, "_sub_sec");
    logI("devSuffix=%s", devSuffix);
        ...
        for(i=0;i<nvRamArrSz;i++) {
            for(j=0;j<sensorArrSz;j++) {
                ...
                if(getMem((CAMERA_DATA_TYPE_ENUM)g_nvramArrInd[i], (CAMERA_DUAL_CAMERA_SENSOR_ENUM)g_sensorArrInd[j], p) == 0) {
                    logI("init Mem(%d, %d)", g_nvramArrInd[i], g_sensorArrInd[j]);
...
int NvBufUtil::getSensorIdAndModuleId(MINT32 sensorType ,MINT32 &sensorId, MUINT32 &moduleId)
{
...
static int getBufAndReadNoLock(CAMERA_DATA_TYPE_ENUM nvRamId, int sensorDev, void*& p, int bForceRead)
{
logI("getBufAndReadNoLock22+ ln=%d ramId=%d sensorDev=%d",__LINE__,(int)nvRamId,sensorDev);
...
static void dump_nv_buf(CAMERA_DATA_TYPE_ENUM nvRamId, CAMERA_DUAL_CAMERA_SENSOR_ENUM sensorDev, void* buf, int nvSize, int mode)
{
    ...
    logI("dump_nv_buf sensorDev=%d nvRamId=%d buf=%p nvSize=%d",sensorDev, nvRamId, buf, nvSize);
    ...
    if(mode == 1) strcat(fname, "_read");
    else if(mode == 2) strcat(fname, "_write");
    else return;
