...
{
    CAM_LOGD("[%s] Enable(%d)", __FUNCTION__, enable);
#if 0
    if(enable)
    {
...
init()
{
    ...
    {
        CAM_LOGD("[%s] N3D is active", __FUNCTION__);
        m_pStateMgr = new StateMgrN3d(m_i4SensorDev);
    }
    else
#endif
    {
...
stop()
{
    ...
    AAA_TRACE_END_D;
    // disable FLK thread
    AAA_TRACE_D("FLK THREAD disable");
    m_pThreadRaw->disableFLKThread();
    AAA_TRACE_END_D;
#endif
...
AAA_TRACE_S(AFFeatureEnable);
    ...
    AAA_TRACE_S(FlashFeatureEnable);
#if CAM3_FLASH_FEATURE_EN
    // Flash
    ...
    AAA_TRACE_S(FlickerFeatureEnable);
#if CAM3_FLICKER_FEATURE_EN
    // Flicker
...
querySensorStaticInfo()
{
    ...
    IspTuningMgr::getInstance().sendIspTuningIOCtrl(m_i4SensorDev, IspTuningMgr::E_ISPTUNING_SET_RAW_FMT_MONO, (u4RawFmtType == SENSOR_RAW_MONO ? MTRUE : MFALSE), 0);
