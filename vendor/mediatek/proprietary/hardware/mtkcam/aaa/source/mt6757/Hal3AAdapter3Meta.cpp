...
doInit()
{
    ...
    if (m_i4SubsampleCount > 1) //Slow motion video recording
    {
        ...
        CAM_LOGD("[%s] create E_FlowControl_Type(%d)", __FUNCTION__, Hal3AIf::E_FlowControl_Type_SMVR);
    ...
    CAM_LOGD("[%s] sensorDev(%d), sensorIdx(%d), mpHal3aObj(%p)",
        __FUNCTION__, mu4SensorDev, mi4SensorIdx, mpHal3aObj);

    unsigned int tuningsize = queryTuningSize();
    mpTuning = ::malloc(tuningsize);
    if (mpTuning == NULL) {
        CAM_LOGD("alloc tuning buffer fail");
    }
    ...
    //for(int i=0;i<MAX_METERING_AREAS;i++)
       // mParams.rMeteringAreas.rAreas[i].i4Weight = 0;    Might already be 0 after initialization Nelson Coverity:ALPS03022060
