...
public: // Dynamic Tuning
    ...
    virtual MVOID  getLCSparam(LCS_ISP_INFO_T &lcs_info);
    virtual MVOID*  getBPCItable();
...
public: // Attributes
    ...
    virtual MERROR_ENUM setFrontBinInfo(ISP_BIN_INFO_T const BinInfo);
    ...
    virtual EIndex_ISO_T map_ISO_value2index(MUINT32 const u4Iso) const;
    virtual MUINT32 map_ISO_index2value(EIndex_ISO_T const eIdx) const;
private:
    EIndex_Effect_T m_eIdx_Effect;
    ESensorDev_T    m_eSensorDev;
    EOperMode_T     m_eOperMode;
    MBOOL           m_bMono;     // MONO = 1, Bayer = 0
...
public:     ////    Exif.
    ...
    virtual MERROR_ENUM getDebugInfo_MultiPassNR(
        const NSIspTuning::ISP_INFO_T& rIspInfo,
        DEBUG_RESERVEA_INFO_T& rDebugInfo,
        const void* pRegBuf
    ) const;
...
protected:  // Invoked only by validatePerFrame()
    ...
    MERROR_ENUM do_validatePerFrameP2(MINT32 flowType, const ISP_INFO_T& rIspInfo, void* pRegBuf);

protected:  ////    All
    MBOOL   prepareHw_PerFrame_All();
    MBOOL   prepareHw_PerFrame_Partial();
    MBOOL   prepareHw_PerFrame_All(const ISP_INFO_T& rIspInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_Partial(const ISP_INFO_T& rIspInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_DynamicBypass_OBC(MBOOL fgOBPass);
    MBOOL   applyToHw_PerFrame_All(RequestSet_T const RequestSet, MINT32 i4SubsampleIdex);
...
protected:
    ...
    MVOID get_ISOIdx_neighbor(RAWIspCamInfo& rIspCamInfo, EIndex_ISO_T const IsoIdx, MUINT32 const u4Iso);

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
protected:  ////    Module.
    MBOOL   updateLscInfo(MINT32 i4IsLockRto);
    MVOID   prepareHw_PerFrame_Default();
    INDEX_T getISPIndex_byISOIdx(MUINT32 u4Idx_ISO, const RAWIspCamInfo& rIspCamInfo);
...
//    MBOOL   prepareHw_PerFrame_LCE();
    MBOOL   prepareHw_PerFrame_SL2G(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_DBS_2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_OBC_2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_BPC_2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_NR1_2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_PDC_2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_LSC_2(const ISP_INFO_T& rIspCamInfo);
    MBOOL   prepareHw_PerFrame_RMM_2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_RMG_2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_RNR(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_SL2(const ISP_INFO_T& rIspInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_PGN(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    MBOOL   prepareHw_PerFrame_UDM(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    ...
    MBOOL   prepareHw_PerFrame_ANR2(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
    ...
    MBOOL   prepareHw_PerFrame_HFG(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx);
...
protected:  ////    ISP ISO Partition.
    ...
    //  Reference to m_rIspParam.ISPComm
    ISP_NVRAM_ISO_INTERVAL_STRUCT&        m_rIspIsoEnv;
...
protected:  ////    ISP Register Parameters.
    ...
    public:
        ...
        IspNvramMgr& operator=(IndexMgr const& rIdxmgr)
        {
            ...
            //setIdx_CCM_CFC (rIdxmgr.getIdx_CCM_CFC());
