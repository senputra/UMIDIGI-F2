...
setPDCEnable(MINT32 sensorIndex, MBOOL enable)
{
    ...
    ISP_MGR_BNR_T::getInstance(dev).setPDCEnable(enable);
}
...
{
    auto dev = mapToESensorDev(sensorIndex);
    ISP_MGR_BNR_T::getInstance(dev).setPDCoutEnable(enable);
}
...
{
    // TODO: call ISP_MGR_NR3D_T if platform support 3DNR
    //auto dev = mapToESensorDev(sensorIndex);
    //ISP_MGR_NR3D_T::getInstance(sensorIndex).setConfig(param.onRegion, param.fullImgSize);
    //ISP_MGR_NR3D_T::getInstance(sensorIndex).post_apply(param.enable, pTuning);
}
