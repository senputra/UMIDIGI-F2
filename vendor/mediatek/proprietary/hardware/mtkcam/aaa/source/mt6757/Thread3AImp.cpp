...
Thread3AImp::clearCommandQ()
{
    ...
    for (List<CmdQ_T>::iterator it = mCmdQ.begin(); it != mCmdQ.end();)
    {
        ...
        {
            MY_LOGD("[%s] cmd size(%d), clear(%d)", __FUNCTION__, mCmdQ.size(), eCmd);
            it = mCmdQ.erase(it);
        }
        else //include 2 cases; 1). keep all cmds except for ECmd_Update, 2). keep at least 1 ECmd_Update in mCmdQ
        {
            MY_LOGD("[%s] Command not killed: cmd size(%d), beginning cmd(%d)", __FUNCTION__, mCmdQ.size(), eCmd);
            it++;
        }
    }
...
Thread3AImp::addCommandQ(ECmd_T const &r3ACmd, MUINTPTR i4Arg)
{
    ...
    MY_LOGD_IF(mu4LogEn, "[%s] mCmdQ size(%d) + cmd(%d) magic(%d)", __FUNCTION__, mCmdQ.size(), r3ACmd, rParam.i4MagicNum);
}
...
Thread3AImp::getCommand(IThread3A::CmdQ_T &rCmd, MBOOL &bGetCmd, MBOOL en_timeout)
{
    ...
    Mutex::Autolock autoLock(mModuleMtx);
    MY_LOGD_IF(fgLogEn, "[%s] mCmdQ.size()=%d, en_timeout(%d)", __FUNCTION__, mCmdQ.size(), en_timeout);
    ...
    {
        MY_LOGD("[%s] mCmdQ.size() = %d after mCmdQCond.wait/waitRelative\n", __FUNCTION__, mCmdQ.size());
    ...
    rCmd = *mCmdQ.begin();
    MY_LOGD_IF(fgLogEn, "CmdQ-size(%d), todo(%d, %d)", mCmdQ.size(), rCmd.eCmd, rCmd.rParamIspProfile.i4MagicNum);
    ...
    while (_this->getCommand(rCmd, bGetCmd, MFALSE))
    {
        ...
        switch (rCmd.eCmd)
        {
            ...
            default:
                ...
                MY_LOGD("rCmd(%d), magic#(%d)-", rCmd.eCmd, rCmd.rParamIspProfile.i4MagicNum);
            break;
        }
        DebugUtil::getInstance(_this->mpHal3A->getSensorDev())->update(LOG_TAG, "onThreadLoop", -1);
        //
