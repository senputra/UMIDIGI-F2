    ...
    , mMWBColorTemperature(0)
{
    ...
    mUnitGain2 = {0,0,0};
    mUnitGain3 = {0,0,0};
    m_rAWBInitInput.eAWBMode  = LIB3A_AWB_MODE_AUTO;
    ...
    m_rAWBOutput.rPreviewAWBGain = mGoldenGain2;
    m_rAWBOutput.rPreviewStrobeAWBGain = mGoldenGain2;
    m_rAWBOutput.rPreviewFullWBGain = mGoldenGain2;
    m_rAWBOutput.rCaptureAWBGain = mGoldenGain2;
    m_rAWBOutput.rCaptureFullWBGain = mGoldenGain2;
    m_rAWBOutput.rPreviewRAWPreGain2 = mGoldenGain2;
    m_rAWBOutput.rPreviewStrobeRAWPreGain2 = mGoldenGain2;
    m_rAWBOutput.rAWBInfo.rAlgGain = mGoldenGain2;
    m_rAWBOutput.rAWBSyncInput_N3D.rAlgGain = mGoldenGain2;
...
{//capture init/ preview reinit
    AWB_LOG("start ln=%d senDev=%d",__LINE__,m_eSensorDev);
...
MBOOL AwbMgr::setSensorMode(MINT32 i4NewSensorMode, int w, int h)
{
    ...
    AWB_LOG_IF(m_i4DgbLog,"[%s()] m_eSensorDev: %d\n", __FUNCTION__, m_eSensorDev);
    AWB_LOG_IF(m_i4DgbLog,"i4NewSensorMode: %d\n", i4NewSensorMode);
...
MRESULT AwbMgr::doPvAWB(MINT32 i4FrameCount, MBOOL bAEStable, MINT32 i4SceneLV, MVOID *aaoBuf, MBOOL bApplyToHW)
{
        ...
        (m_eAWBMode != LIB3A_AWB_MODE_OFF))
    {
        ...
        else if ((m_bStrobeModeChanged) && (!m_bInitState))
        {
            ...
            if (m_i4StrobeMode == AWB_STROBE_MODE_ON) {
                AAA_TRACE_G(setAWBStatConfig);
                AAA_TRACE_NL(setAWBStatConfig);
                m_pIAwbAlgo->setAWBStatConfig(m_rAWBStatCfg[AWB_STROBE_MODE_ON][m_eSensorMode][m_eAWBMode],
                    m_rAWBWindowConfig.i4SensorWidth[m_eSensorMode],
                    m_rAWBWindowConfig.i4SensorHeight[m_eSensorMode]);
                AAA_TRACE_END_G;
...
MBOOL AwbMgr::doPreCapAWB(MINT32 i4SceneLV, MVOID *aaoBuf, MBOOL bApplyToHW)
{
    ...
    else if (m_bEnableAWB && (((!m_bAWBLock) && (!m_bAdbAWBLock)) || (m_i4StrobeMode == AWB_STROBE_MODE_ON)) && (m_eAWBMode != LIB3A_AWB_MODE_OFF))
    {
        ...
        else if ((m_i4StrobeMode == AWB_STROBE_MODE_ON) && (m_bStrobeModeChanged) && (m_eAWBMode != MTK_CONTROL_AWB_MODE_MWB))
        {
            AAA_TRACE_G(setAWBStatConfig);
            AAA_TRACE_NL(setAWBStatConfig);
            m_pIAwbAlgo->setAWBStatConfig(m_rAWBStatCfg[AWB_STROBE_MODE_ON][m_eSensorMode][m_eAWBMode],
...
{
    AAA_TRACE_G(getDebugInfo);
    AAA_TRACE_NL(getDebugInfo);
...
{
    AAA_TRACE_G(getASDInfo);
    AAA_TRACE_NL(getASDInfo);
...
{
    AAA_TRACE_G(getAWBParentStat);
    AAA_TRACE_NL(getAWBParentStat);
...
MBOOL AwbMgr::SetAETargetMode(int mode) //for hdr
{
    if(mode==AE_MODE_IVHDR_TARGET || mode==AE_MODE_MVHDR_TARGET || mode==AE_MODE_ZVHDR_TARGET)
        setAWBNvramIdx(AWB_NVRAM_IDX_VHDR);
    else
        setAWBNvramIdx(AWB_NVRAM_IDX_NORMAL);
    return MTRUE;
}
