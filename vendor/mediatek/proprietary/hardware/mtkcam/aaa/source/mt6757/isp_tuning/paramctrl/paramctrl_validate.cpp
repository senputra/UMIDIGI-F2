...
validatePerFrame(RequestSet_T const RequestSet, MBOOL const fgForce, MINT32 i4SubsampleIdex)
{
    ...
    if (fgKeep)
    {
        ...
        if (!applyToHw_PerFrame_All(RequestSet, i4SubsampleIdex))
        {
...
check_ISO_env_info()
{
    ...
    if( m_rIspIsoEnv.u2Length > NVRAM_ISP_REGS_ISO_SUPPORT_NUM){
        CAM_LOGE("CUSTOM_ISO_ENV_LENGTH_ERROR");
        return MFALSE;
    }
    else{
        for(int i=0; i<(m_rIspIsoEnv.u2Length-1); i++)
        {
            if( (m_rIspIsoEnv.IDX_Partition[i] < m_rIspIsoEnv.IDX_Partition[i+1])&&
                (m_rIspIsoEnv.IDX_Partition[i] >= 0) &&
                (m_rIspIsoEnv.IDX_Part_Middle[i] < m_rIspIsoEnv.IDX_Part_Middle[i+1]) &&
                (m_rIspIsoEnv.IDX_Part_Middle[i] >= 0) )
            {
                temp_count++;
            }
        }
        if ( temp_count != (m_rIspIsoEnv.u2Length-1) ){
...
INDEX_T
Paramctrl::
getISPIndex_byISOIdx(MUINT32 u4Idx_ISO, const RAWIspCamInfo& rIspCamInfo)
{
    ...
    INDEX_T const*const pDefaultIndex = m_pIspTuningCustom->getDefaultIndex(
        rIspCamInfo.eIspProfile, rIspCamInfo.eSensorMode, rIspCamInfo.eIdx_Scene, static_cast<EIndex_ISO_T>(u4Idx_ISO)
    );
    if  ( ! pDefaultIndex )
    {
...
do_validatePerFrame(RequestSet_T const RequestSet ,MINT32 i4SubsampleIdex)
{
    ...
    if (isDynamicTuning())
    {
        ...
        CAM_LOGD_IF(m_bDebugEnable, "eIspProfile = %d, eSensorMode = %d, eIdx_Scene = %d, eIdx_ISO = %d",
    ...
    if(!prepare_rdy || ! applyToHw_PerFrame_All(RequestSet,i4SubsampleIdex))  //  Apply the ispmgr's buffer to H/W.
    {
        err = MERR_SET_ISP_REG;
        goto lbExit;
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID
Paramctrl::getLCSparam(LCS_ISP_INFO_T &lcs_info)
{
...
MVOID*
Paramctrl::getBPCItable(){
    return ISP_MGR_BNR2_T::getInstance(m_eSensorDev).getBPCItable();
}
...
do_validatePerFrameP2(MINT32 flowType, const ISP_INFO_T& _rIspInfo, void* pRegBuf)
{
    ...
    //YUV Reprocess
    rIspInfo_temp.rCamInfo.eIdx_ISO = map_ISO_value2index(rIspInfo_temp.rCamInfo.u4ISOValue);
    get_ISOIdx_neighbor(rIspInfo_temp.rCamInfo, rIspInfo_temp.rCamInfo.eIdx_ISO, rIspInfo_temp.rCamInfo.u4ISOValue);
#if 0
    ...
    if (isDynamicTuning())
    {
        ...
        CAM_LOGD_IF(m_bDebugEnable, "IndexMgr::getIdx = [OBC(%d),BPC(%d),NR1(%d),UDM(%d),GGM(%d),ANR(%d),CCR(%d),EE(%d)]\n"
                ,idxmgr.getIdx_OBC()
                ,idxmgr.getIdx_BPC()
                ,idxmgr.getIdx_NR1()
                ,idxmgr.getIdx_UDM()
        ...
        (isDynamicBypass() == MTRUE) ? (prepare_rdy = prepareHw_PerFrame_Partial(rIspInfo, idxmgr))
                                     : (prepare_rdy = prepareHw_PerFrame_All(rIspInfo, idxmgr));
    }
