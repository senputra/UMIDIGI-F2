    ...
    , m_pNvram3A(NULL)
{
    ...
    {
        MUINT32 u4FNum = m_pNvram3A->rAENVRAM[AE_NVRAM_IDX_NORMAL].rDevicesInfo.u4LensFno;
        MUINT32 u4FL   = m_pNvram3A->rAENVRAM[AE_NVRAM_IDX_NORMAL].rDevicesInfo.u4FocusLength_100x;
...
getStaticMetadata() const
{
    ...
    if (m_pNvram3A)
    {
        ...
        fFNum = (MFLOAT) m_pNvram3A->rAENVRAM[AE_NVRAM_IDX_NORMAL].rDevicesInfo.u4LensFno / 10.0f;
        fFocusLength = (MFLOAT) m_pNvram3A->rAENVRAM[AE_NVRAM_IDX_NORMAL].rDevicesInfo.u4FocusLength_100x / 100.0f;
...
getShadingMapFromMem(MBOOL fgShadOn, const IMetadata::Memory& lscData) const
{
    ...
    if (lscData.size())
    {
        ...
        else
        {
            if (!fgShadOn)
            {
                MINT32 i4Cnt = rCfg.i4GridX * rCfg.i4GridY * 4;
...
{
    IMetadata shadingMeta;
    const IMetadata::Memory* prLscData = QUERY_ENTRY_REF_SINGLE<IMetadata::Memory>(halMeta, MTK_LSC_TBL_DATA);
    if (prLscData)
    {
