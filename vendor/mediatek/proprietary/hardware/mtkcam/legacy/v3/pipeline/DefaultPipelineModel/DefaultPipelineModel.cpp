    ...
    , mInFlush(MFALSE)
{
    ...
    //
    {
        ...
        if ( mLogLevel == 0 ) {
            ::property_get("vendor.debug.camera.log.HwPipeline", cLogLevel, "0");
            mLogLevel = ::strtol(cLogLevel, NULL, 0);
        }
    }
    {
        char value[PROPERTY_VALUE_MAX] = {0};
        ::property_get("vendor.debug.camera.force_fd", value, "0");
...
)   const
{
    ...
    MY_LOGE_IF(pPool == 0, "NULL HalImageStreamBufferPool - stream:%#" PRIx64 "(%s)", streamId, pStreamInfo->getStreamName());
    MERROR err = pPool == 0 ? UNKNOWN_ERROR : pPool->acquireFromPool(__FUNCTION__, rpStreamBuffer, ::s2ns(10));
    MY_LOGA_IF(
        OK!=err || rpStreamBuffer==0,
        "[acquireFromPool] err:%d(%s) pStreamBuffer:%p stream:%#" PRIx64 "(%s)",
...
submitRequest(AppRequest& request)
{
   ...
   //add log to divide captureRequest or previewRequest
   if(mLogLevel >= 1){
        sp<IMetaStreamBuffer> pStremBuf = request.vIMetaBuffers[0];
        IMetadata* pMetadata = pStremBuf->tryReadLock(LOG_TAG);
        IMetadata::IEntry const eCapturInten = pMetadata->entryFor(MTK_CONTROL_CAPTURE_INTENT);
        pStremBuf->unlock(LOG_TAG,pMetadata);
        if(!eCapturInten.isEmpty() && eCapturInten.itemAt(0,Type2Type<MUINT8>()) == 2){
           MY_LOGI("This is captureRequest!! RequestNo: %d,FrameNo: %d",pFrame->getRequestNo(),pFrame->getFrameNo());
        }
    }

    //just for fix google camera ,preview & capture always capture intent 2
    ...
    IMetadata::IEntry const eCapturIntent = pMetadata->entryFor(MTK_CONTROL_CAPTURE_INTENT);
    if(!eCapturIntent.isEmpty() && eCapturIntent.itemAt(0,Type2Type<MUINT8>()) == 2){
        MY_LOGI("This is captureRequest!!capture intent: %d",eCapturIntent.itemAt(0,Type2Type<MUINT8>()));
    }

    sp<IImageStreamBuffer> pStream_Buffer = request.vOImageBuffers[0];
    sp<IImageStreamInfo> pStream_Info = const_cast<IImageStreamInfo*>(pStream_Buffer->getStreamInfo());
    MSize size = pStream_Info->getImgSize();
    MY_LOGD("rawtype size: %d x %d", size.w, size.h);

    if((!eCapturIntent.isEmpty() && eCapturIntent.itemAt(0,Type2Type<MUINT8>()) == 2) &&
    ((params.pAppRequest->vOImageBuffers.size() >= 2) || (size.w * size.h >= 1920 * 1088))){ //根据google实际choose的size进行设置
        rawType = 1; // ERawType_Pure for catpure
        entry.push_back(rawType, Type2Type<MINT32>());
        pMetadata->update(entry.tag(),entry);
        MY_LOGE("Capture:MTK_P1NODE_RAW_TYPE:%d",entry.itemAt(0,Type2Type<MINT32>()));
    }
    else{
        rawType = 0;
        entry.push_back(rawType, Type2Type<MINT32>());
        pMetadata->update(entry.tag(),entry);
        MY_LOGD("Preview:MTK_P1NODE_RAW_TYPE:%d",entry.itemAt(0,Type2Type<MINT32>()));
    }
    pStremBuf->unlock(LOG_TAG,pMetadata);
    ...
    android::Vector<android::String8>::iterator it = rLogs.begin();
    {
        ...
        for (size_t j = 0; j < rSet.size(); j++) {
            ...
            for (size_t k = 0; k < rSet[j].vIn.size(); k++) {
                ...
                str += String8::format("%#" PRIx64 ":%s ", p->getStreamId(), p->getStreamName());
            }
            str += " -> ";
