    ...
    //
{
    ...
    if ( 0 == mLogLevel ) {
        ::property_get("vendor.debug.camera.log.pipeline", cLogLevel, "0");
        mLogLevel = ::atoi(cLogLevel);
    }
}
...
onLastStrongRef(const void* /*id*/)
{
        ...
        struct Log
        {
            ...
            dump(IMyMap& rMap)
            {
                for (size_t i = 0; i < rMap.size(); i++) {
                    if  ( rMap.itemAt(i) != 0 ) {
                        MY_LOGW("Non-released stream:%#" PRIx64 "(%s)", rMap.keyAt(i), rMap.itemAt(i)->getStreamInfo()->getStreamName());
                    }
                }
            }
        };
    ...
    for (auto const& listener : mListeners)
    {
        ...
        MY_LOG2("destroyed, remove it from listners");
        p->onPipelineFrame(
            getFrameNo(),
...
MERROR
MAIN_CLASS_NAME::
setIsP2HalYuvJpegSupport(MBOOL value)
{
    if (value == NULL) {
        return BAD_VALUE;
    }
    //
    if (value) {
        MY_LOGD("setIsP2HalYuvJpegSupport true");
    }
    mIsP2HalYuvJpegSupport = value;
    return OK;
}
...
)   const
{
    ...
    if  ( ! pItem->mBitStatus.hasBit(eBUF_STATUS_ACQUIRE) ) {
        MY_LOGF_IF(pItem->mBuffer!=0, "[frameNo:%u streamId:%#" PRIx64 "] Non-null buffer but non-acquired status:%#x", getFrameNo(), streamId, pItem->mBitStatus.value);
        ...
        profile.print_overtime(10, "[requestNo:%u frameNo:%u streamId:%#" PRIx64 "]", getRequestNo(), getFrameNo(), streamId);
        CAM_TRACE_END();
        if  ( OK != err || pItem->mBuffer == 0 ) {
            MY_LOGE(
                "[requestNo:%u frameNo:%u streamId:%#" PRIx64 "] mBuffer:%p err:%d(%s)",
...
handleReleasedBuffers(UserId_T userId, sp<IAppCallback> pAppCallback)
{
    ...
    //  Callback to App.
    {
        if  ( pAppCallback == 0 ) {
            ...
            MY_LOG1("requestNo:%u frameNo:%u userId:%#" PRIxPTR " OAppMeta#(left:%zd this:%zu)",
                    getRequestNo(), getFrameNo(), userId, aAppMetaNumO, aAppMetaSetO.size());
