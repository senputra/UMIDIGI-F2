...
struct  evaluate_request::Imp
{
    ...
    dump(evaluate_request_pipeline::Params& rParams)
    {
        ...
        MY_LOGD("dump App/Img(%zu), Hal/Img(%zu), App/Meta(%zu), Hal/Meta(%zu)",
        ...
        for ( size_t i=0; i<aReqInfoSet.vAppImage.size(); i++) {
            MY_LOGD("App.Img-%zu : %#" PRIx64 " %s", i,  aReqInfoSet.vAppImage.valueAt(i)->getStreamId(), aReqInfoSet.vAppImage.valueAt(i)->getStreamName());
        }
        for ( size_t i=0; i<aReqInfoSet.vHalImage.size(); i++) {
            MY_LOGD("Hal.Img-%zu : %#" PRIx64 " %s", i,  aReqInfoSet.vHalImage.valueAt(i)->getStreamId(), aReqInfoSet.vHalImage.valueAt(i)->getStreamName());
        }
        for ( size_t i=0; i<aReqInfoSet.vAppMeta.size(); i++) {
            MY_LOGD("App.Meta-%zu : %#" PRIx64 " %s", i, aReqInfoSet.vAppMeta.valueAt(i)->getStreamId(),  aReqInfoSet.vAppMeta.valueAt(i)->getStreamName());
        }
        for ( size_t i=0; i<aReqInfoSet.vHalMeta.size(); i++) {
            MY_LOGD("Hal.Meta-%zu : %#" PRIx64 " %s", i, aReqInfoSet.vHalMeta.valueAt(i)->getStreamId(),  aReqInfoSet.vHalMeta.valueAt(i)->getStreamName());
        }
    }
    ...
    operator()(Params& rParams, IEvaluator const* pEvaluator)
    {
        ...
        for ( uint32_t i=0; i<rParams.subFrameCnt-1; i++ )
        ...
        for ( uint32_t i=0; i<rParams.subFrameCnt; i++ ) {
            ...
            vFrame.add(pFrame);
            MY_LOGD_IF(1, "Create PipelineFrame of Req(%d)/Frm(%u)", rParams.requestNo, rParams.frameNo+i);
        }
        //
        ...
        evaluate_request_pipeline::Params aEvaluateSubRequestPipelineParams;
        {
            ...
            MY_LOGD("Request BufMap:  <IN  Buf> Image(%zu), Meta(%zu)", rRequest.vIImageBuffers.size(), rRequest.vIMetaBuffers.size() );
            MY_LOGD("Request BufMap:  <Out Buf> Image(%zu), Meta(%zu)", rRequest.vOImageBuffers.size(), rRequest.vOMetaBuffers.size() );
            MY_LOGD("Request InfoMap: <IN  Buf> Image(%zu), Meta(%zu)", aRequestAppStreamsParams.vIAppImageStreams.size(), aRequestAppStreamsParams.vIAppMetaStreams.size() );
            MY_LOGD("Request InfoMap: <Out Buf> Image(%zu), Meta(%zu)", aRequestAppStreamsParams.vOAppImageStreams.size(), aRequestAppStreamsParams.vOAppMetaStreams.size() );
            //
            ...
            //
            MY_LOGD("SubRequest BufMap:  <IN  Buf> Image(%zu), Meta(%zu)", rRequest.vIImageBuffers.size(), rRequest.vIMetaBuffers.size() );
            MY_LOGD("SubRequest BufMap:  <Out Buf> Image(%zu), Meta(%zu)", rRequest.vOImageBuffers.size(), rRequest.vOMetaBuffers.size() );
            MY_LOGD("SubRequest InfoMap: <IN  Buf> Image(%zu), Meta(%zu)", aSubRequestAppStreamsParams.vIAppImageStreams.size(), aSubRequestAppStreamsParams.vIAppMetaStreams.size() );
            MY_LOGD("SubRequest InfoMap: <Out Buf> Image(%zu), Meta(%zu)", aSubRequestAppStreamsParams.vOAppImageStreams.size(), aSubRequestAppStreamsParams.vOAppMetaStreams.size() );
            //
...
/*            IStreamInfoMapHT const& rStreamMapHal = aEvaluateRequestPipelineParams.pStreamInfoSet->getHalImage();
    ...
    */        //
#ifdef _MULTIFRAME
            for ( uint32_t sub_idx=0; sub_idx<rParams.subFrameCnt; sub_idx++ ) {
                ...
                if (sub_idx==0) { // default: first subframe owns app image buffer
                    ...
                    MY_LOGD("Frm(%u) Image MapHal(%zu)/MapApp(%zu)", sub_idx, rStreamMapHal.size(), rStreamMapApp.size());
                    // H: ok
...
//                        IStreamBufferMapT const& rBufMapApp = rReq.vOImageBuffers;
                    ...
                    //
                    MY_LOGD("subframe-%u: HalImg(%zu) / AppImg(%zu)",
                    ...
                    aRefineMetaStreamBufferParams.editItemAt(sub_idx).pAppImageStreams = &rStreamMapApp;
                    MY_LOGD("Frm(%u) MapHal(%zu)/MapApp(%zu)", sub_idx, rStreamMapHal.size(), rStreamMapApp.size());
...
//                        IStreamBufferMapT const& rBufMapApp = rRequest.vIImageBuffers;
                        ...
                        if ( rBufMapApp.size()>0 )
                            MY_LOGE("Request might not include <IN> App/Img: %zu", rBufMapApp.size());
                    }
...
//                        IStreamBufferMapT const& rBufMapApp = rRequest.vOImageBuffers;
                        ...
                        //}
                        MY_LOGD("Request might include <Out> App/Img: %zu, but only main-frame does", rBufMapApp.size());
                    }
                    //
                    MY_LOGD("subframe-%u: HalImg(%zu) / AppImg(%zu)",
                            sub_idx, pBufMapHal->size(), pBufMapApp->size());
...
//                refine_meta_stream_buffer_params aRefineMetaStreamBufferParams =
  //                                               vaRefineMetaStreamBufferParams.editItemAt(i);
                ...
                if (sub_idx==0) { // default: first subframe owns app meta buffer
                    ...
                    MY_LOGD("Frm(%u) Meta MapHal(%zu)/MapApp(%zu)", sub_idx, rStreamMapHal.size(), rStreamMapApp.size());
                    // H
                    pBufMapHal->setCapacity(rStreamMapHal.size());
...
//                        IStreamBufferMapT const& rBufMapApp = rRequest.vIMetaBuffers;
                    ...
                    //rRequest.vOutputMetaBuffers   => need allocating here
                    {
                        ...
                        MY_LOGD("subframe-%u: HalMeta(%zu) / InAppMeta(%zu) / OutAppMeta(%zu)",
...
//                            mpMetadataConverter->dumpAll( *data_w, vFrame[0]->getRequestNo());
                ...
                //
                MY_LOGD("subframe-%u: HalMeta(%zu) / AppMeta(%zu)",
                         sub_idx, pBufMapHal->size(), pBufMapApp->size());
            }
#else
#endif
        ...
        //
        {
            for ( uint32_t i=0; i<rParams.subFrameCnt; i++ ) {
                ...
                MY_LOGE_IF(
                    OK !=err ,
                    "[refineRequestMetaStreamBuffers]-%u err:%d(%s)",
                    i, err, ::strerror(-err)
                );
            }
        }
        ...
        for ( uint32_t i=0; i<rParams.subFrameCnt; i++ ) {
            android::sp<PipelineFrameT> pFrame = vFrame.editItemAt(i);
            MY_LOGD("configure req(%d) frm(%u)", pFrame->getRequestNo(), i);
            if (i==0) {
                ...
                pFrame->setStreamInfoSet(aEvaluateRequestPipelineParams.pStreamInfoSet);
                 //
                MY_LOGD("Information of Request(main): NodeMap(%zu)", pFrame->getPipelineNodeMap()->size());
                ...
                for ( size_t j=0; j< pFrame->getPipelineNodeMap()->size(); j++ ) {
                    ...
                    for ( size_t k=0; k<rImageIOMapSet.size(); k++ ) {
                        MY_LOGD("InfoIOMapSet: nodeId %#" PRIxPTR " ImageIOMapSet(%zu/%zu) In(%zu) Out(%zu)",
                        ...
                        for ( size_t m=0; m<rImageIOMapSet[k].vIn.size(); m++ ) {
                            MY_LOGD("nodeId %#" PRIxPTR " In-streamId %#" PRIx64 " In-StreamName %s",
                        ...
                        for ( size_t m=0; m<rImageIOMapSet[k].vOut.size(); m++ ) {
                            MY_LOGD("nodeId %#" PRIxPTR " Out-streamId %#" PRIx64 " Out-StreamName %s",
                                    pFrame->getPipelineNodeMap()->nodeAt(j)->getNodeId(),
                    ...
                    for ( size_t k=0; k<rMetaIOMapSet.size(); k++ ) {
                        MY_LOGD("InfoIOMapSet: nodeId %#" PRIxPTR " MetaIOMapSet(%zu/%zu) In(%zu) Out(%zu)",
                ...
                //
                MY_LOGD("Information of Request(sub) : NodeMap(%zu)", pFrame->getPipelineNodeMap()->size());
                pFrame->getPipelineDAG().dump();
                //
    ...
    operator()(Params& rParams, IEvaluator const* pEvaluator) const
    {
        ...
        ////////////////////////////////////////////////////////////////////////////
        {
            ...
            for (size_t i = 0; i < rParams.pAppStreams->vIAppImageStreams.size(); i++) {
                ...
                MY_LOGD("add InAppImage(%zu/%zu) streamId: %#" PRIx64 " streamName: %s",
            ...
            for (size_t i = 0; i < rParams.pAppStreams->vOAppImageStreams.size(); i++) {
                ...
                MY_LOGD("add OutAppImage(%zu/%zu) streamId: %#" PRIx64 " streamName: %s",
            ...
            for (size_t i = 0; i < rParams.pAppStreams->vIAppMetaStreams.size(); i++) {
                ...
                MY_LOGD("add InAppMeta(%zu/%zu) streamId: %#" PRIx64 " streamName: %s",
        ...
        do
        {
            ...
            MY_LOGD("node config(%#" PRIxPTR "): <In> AppMeta(%zu)/HalMeta(%zu)/AppImage(%zu)/HalImage(%zu)",
                    nodeId, nodeCfg.vIAppMeta.size(), nodeCfg.vIHalMeta.size(),
                            nodeCfg.vIAppImage.size(), nodeCfg.vIHalImage.size());
            MY_LOGD("node config(%#" PRIxPTR "): <Out> AppMeta(%zu)/HalMeta(%zu)/AppImage(%zu)/HalImage(%zu)",
            ...
            {
                CAM_LOGD_IF(1, "[evaluate_request_pipeline] Skip this node:%#" PRIxPTR " <Out> App/Img(%zu) App/Meta(%zu) Hal/Img(%zu) Hal/Meta(%zu)"
...
lbContinue:
            ...
            //
            MY_LOGD("node request(%#" PRIxPTR "): <In> AppMeta(%zu)/HalMeta(%zu)/AppImage(%zu)/HalImage(%zu)",
                    nodeId, nodeReq.vIAppMeta.size(), nodeReq.vIHalMeta.size(),
                            nodeReq.vIAppImage.size(), nodeReq.vIHalImage.size());
            MY_LOGD("node request(%#" PRIxPTR "): <Out> AppMeta(%zu)/HalMeta(%zu)/AppImage(%zu)/HalImage(%zu)",
