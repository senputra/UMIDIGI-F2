...
INormalPipe_FrmB::createInstance(MUINT32 *pSensorIndex, MUINT32 sensorCnt,char const* szCallerName, MINT32 burstQnum)
{
    ...
    if (!pAllSensorInfo){
        ...
        PIPE_INF("[%d]createInstance, %s,Alloc pAllSensorInfo=0x%016llx",pSensorIndex[0],szCallerName, (unsigned long long)pAllSensorInfo);
    ...
    if (1 == sensorCnt) {//NPIPE_Sensor_0 ||  NPIPE_Sensor_1
        ...
        if (NULL == pNPipe)  {
            ...
            PIPE_INF("[%d]createInstance, %s,pNormalPipe_FrmB[%d]=0x%016llx create\n",pSensorIndex[0],szCallerName, pSensorIndex[0], (unsigned long long)pNPipe);
...
NormalPipe_FrmB::destroyInstance(char const* szCallerName)
{
    ...
    if (pNormalPipe_FrmB[idx]->mUserCnt == 0)  {
        ...
        mpCamIOPipe = NULL;
        PIPE_INF("Destroy NPipe[%d]=0x%016llx",idx, (unsigned long long)pNormalPipe_FrmB[idx]);
        ...
        for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
        {
            ...
            if (this->m_bStarted == MFALSE)
            {
                PIPE_INF("MyEnq: dma(0x%x) pa(0x%x) io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x)\n",_buf.mPortID.index,\
                    (MUINT32)_buf.mBuffer->getBufPA(0),\
...
#define __FMT(portidx,fmt)({\
    ...
    _fmt;})
        ...
        if(ISP_QuerySize(rQBuf.mvOut.at(i).mPortID.index,_op,(EImageFormat)__FMT(rQBuf.mvOut.at(i).mPortID.index,rQBuf.mvOut.at(i).mBuffer->getImgFormat()),rQBuf.mvOut.at(i).FrameBased.mDstSize.w,queryRst,_PixMode) == 0){
            ...
            if(rQBuf.mvOut.at(i).FrameBased.mDstSize.w != (MINT)queryRst.x_pix){
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            for (MUINT32 idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++)  {
                ...
                if (NULL == pframe) {
                    ...
                    mpSensorIdx[0],portID.index,idx, (MUINT32)rQTSBufInfo.vBufInfo.at(idx).u4BufPA[0]);
                    return MFALSE;
                }
                ...
                for(MUINT32 i=0;i<_size;i++){
                    if(MFALSE == this->mpDummyRequest[_PortMap(portID.index)]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_at,i,(MUINTPTR)&_buf))
                        ...
                        if(_buf.mBuffer != NULL){
                            if((MUINT32)_buf.mBuffer->getBufPA(0) == buff.mPa){
            ...
            PIPE_ERR("[MyDeQ]tg:0x%x dequeSz is 0x%x,[PorID=%d]\n",_NORMALPIPE_GET_TG_IDX(this->mpSensorIdx[0]), (MUINT32)rQTSBufInfo.vBufInfo.size(),portID.index);
            return MFALSE;
        }
        //
    }
...
NormalPipe_FrmB::configPipe(QInitParam const& vInPorts)
{
    ...
    if (mpEnabledSensorCnt != vInPorts.mSensorCfg.size()) {
        PIPE_ERR("NPipe::configPipe SensorCfgCnt is not match. (%d,%d)", mpEnabledSensorCnt, (MINT32)vInPorts.mSensorCfg.size());
        return MFALSE;
    }
    ...
    if (!mpCamIOPipe)
    {
        ...
        PIPE_DBG("DropCB(0x%016llx)/Cookie(0x%016llx)/PerFrameCtl(%d)", (unsigned long long)this->m_DropCB,(unsigned long long)this->m_returnCookie, vInPorts.m_PerFrameCtl);
    }
    else {
...
NormalPipe_FrmB::wait(MINT32 sensorIdx, EPipeSignal eType, uint64_t ns_timeout)
{
    ...
    if (this->mpCamIOPipe)
    {
        ...
        ret = MFALSE;
        PIPE_ERR("camiopipe obj isn't exist(0x%016llx)\n",(unsigned long long)this);
    }
    PIPE_DBG("wait-\n");
    return ret;
}
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    if ( !this->mConfigDone ) {
        PIPE_ERR("error:sendCommand(0x%x) fail, send before configpipe(0x%016llx,0x%x)",cmd,(unsigned long long)this,this->mConfigDone);
        return MFALSE;
    }
    ...
    case EPIPECmd_GET_HBIN_INFO:
        ...
        if ((2 < arg1) || (0 == arg2)) {
            PIPE_ERR("EPIPECmd_GET_HBIN_INFO Err: Invalid Para arg1(%d), arg2=0x%x!",(MINT32)arg1, (MUINT32)arg2);
            return MFALSE;
        }
        tg = _NORMALPIPE_GET_TG_IDX(arg1);
        PIPE_DBG("EPIPECmd_GET_HBIN_INFO SIdx(%d), BinInfoAddr=0x%x",(MINT32)arg1, (MUINT32)arg2);
        ...
        switch (tg) {
            ...
            case CAM_TG_2:
                default:
                PIPE_ERR("EPIPECmd_GET_HBIN_INFO Err: Invalid SensorIdx(%d)!",(MINT32)arg1);
                return MFALSE;
        }
        PIPE_DBG("EPIPECmd_GET_HBIN_INFO SIdx(%d),twin=%d,hbin_en=%d w=0x%x, h=0x%x!",(MINT32)arg1,mbTwoPixelEnable, hbin->mEnabled,hbin->size.w, hbin->size.h);
        break;
    case EPIPECmd_GET_EIS_INFO:
        //arg1 : sensor-idx
        //arg2 : NormalPipe_EIS_Info*
        if (0 == arg2) {
            PIPE_ERR("EPIPECmd_GET_EIS_INFO Err: Invalid Para arg1(%d)",(MINT32)arg1);
            return MFALSE;
        }
    ...
    if(ret != MTRUE){
        PIPE_ERR("error: sendCommand fail: (cmd,arg1,arg2,arg3)=(0x%08x,0x%08x,0x%08x,0x%08x)", cmd, (MUINT32)arg1, (MUINT32)arg2, (MUINT32)arg3);
    }
...
{
    if(this->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_OpendNum))
...
enque(IImageBuffer *pframe)
{
    ...
    PIPE_DBG("FrameMgr::enque+, 0x%x", (MUINT32)pframe->getBufPA(0));
...
NormalPipe_FrmB_Thread* NormalPipe_FrmB_Thread::CreateInstance(NormalPipe_FrmB* obj)
{
    if(obj == 0)
    {
        PIPE_ERR("create instance fail(0x%016llx)",(unsigned long long)obj);
        return NULL;
    }
...
//    this->m_bStart = MFALSE;
    //
    //pthread_join(this->m_EnqueThread, NULL); /*already detach in IspEnqueThread*/
...
MBOOL NormalPipe_FrmB_Thread::Enque(BufInfo* pBuf)
{
    ...
    if(_buf.mPa == (MUINT32)pBuf->mBuffer->getBufPA(0)){
    ...
    if(rQBufInfo.vBufInfo.size() == 1)
        ...
        PIPE_DBG("Enque port:0x%x PA:0x%x,exchange:0x%x, mag:0x%x\n",portID.index,(MUINT32)rQBufInfo.vBufInfo[0].u4BufPA[0],(MUINT32)rQBufInfo.vBufInfo[1].u4BufPA[0],pBuf->FrameBased.mMagicNum_tuning);
...
MBOOL NormalPipe_FrmB_Thread::DmaCfg(BufInfo* pParam)
{
    ...
    if (this->m_pNormalpipe_FrmB->mpCamIOPipe) {
        ...
        PIPE_DBG("Dmacfg_%d:cmd:0x%x, crop:(%d,%d,%d,%d),outsize:(%d,%d)\n",tgIdx,_cmd,_dmao.crop.x,_dmao.crop.y,(MINT32)_dmao.crop.w,(MINT32)_dmao.crop.h,(MINT32)_dmao.out.w,(MINT32)_dmao.out.h);
