...
ZsdShot(
    ...
    , mpSwNR(NULL)
    , mIso(0)
    , mSwNRCropRect(Rect(0,0,0,0))
{
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.debug.camera.zsddump", value, "0");
    ...
    uint32_t const  arg2)
{
    ...
    mShotMode = arg2;
    MY_LOGD("mpCaptureBufMgr %d 0x%p, mShotMode(%d)",  mpCaptureBufHandler->getStrongCount(), mpCaptureBufHandler.get(), mShotMode);
...
MVOID
ZsdShot::
doSwNR(ImgBufInfo const & rDstImgBufInfo)
{
#define SWNR_DEBUG 0

#define DUMP_ImgBufInfo(tempInfo,param) \
            do {                                             \
                MY_LOGD("%s:%d", #param, tempInfo.param);  \
            } while(0)

#define SWNR_DEBUG_DUMP(x) \
    DUMP_ImgBufInfo(x, eImgFmt);    \
    DUMP_ImgBufInfo(x, u4ImgWidth); \
    DUMP_ImgBufInfo(x, u4ImgHeight);\
    DUMP_ImgBufInfo(x, u4BufSize);  \
    DUMP_ImgBufInfo(x, u4BufVA);    \
    DUMP_ImgBufInfo(x, u4BufPA);    \
    DUMP_ImgBufInfo(x, i4MemID);    \
    DUMP_ImgBufInfo(x, i4BufSecu);  \
    DUMP_ImgBufInfo(x, i4BufCohe);  \
    DUMP_ImgBufInfo(x, u4Stride[0]);\
    DUMP_ImgBufInfo(x, u4Stride[1]);\
    DUMP_ImgBufInfo(x, u4Stride[2]);

    FUNCTION_LOG_START;

    MY_LOGD("VA=0x%X,size=%d",rDstImgBufInfo.u4BufVA,rDstImgBufInfo.u4BufSize);
#if SWNR_DEBUG
    SWNR_DEBUG_DUMP(rDstImgBufInfo);  
#endif

    //(1)allocate SwNR buffer
    MY_LOGD("allocate SwNR buffer");

    ImgBufInfo rSwNRImgBufInfo;
    rSwNRImgBufInfo.eImgFmt = eImgFmt_I420;//use I420 format
    MINT32 u4TWidth = rDstImgBufInfo.u4ImgWidth;
    MINT32 u4THeight = rDstImgBufInfo.u4ImgHeight;
    mSwNRMem.size = queryImgBufSize(rSwNRImgBufInfo.eImgFmt ,u4TWidth, u4THeight);//width, height ???
    MY_LOGD("allocate SwNR mem,(%dx%d) size = %d", u4TWidth, u4THeight,mSwNRMem.size);
    allocMem(mSwNRMem);// dealloc in onYuv_free
    setImageBuf(rSwNRImgBufInfo.eImgFmt, u4TWidth, u4THeight, rSwNRImgBufInfo, mSwNRMem);
#if SWNR_DEBUG
    SWNR_DEBUG_DUMP(rSwNRImgBufInfo);
#endif

    //(2)transform to I420
    MY_LOGD("new ISImager 1");
    ISImager *pISImager = ISImager::createInstance(rDstImgBufInfo);
    pISImager->setTargetBufInfo(rSwNRImgBufInfo);
    pISImager->setFormat(rSwNRImgBufInfo.eImgFmt);
    pISImager->setRotation(0);
    pISImager->setFlip(0);
    pISImager->setResize(rDstImgBufInfo.u4ImgWidth, rDstImgBufInfo.u4ImgHeight);
    pISImager->setROI(Rect(0, 0, rDstImgBufInfo.u4ImgWidth, rDstImgBufInfo.u4ImgHeight));
    ...
    //(3)generate IImageBuffer from ImgBufInfo
    //reference defaultbufhandler.cpp
    size_t bufStridesInBytes[] = { rSwNRImgBufInfo.u4Stride[0], rSwNRImgBufInfo.u4Stride[1], rSwNRImgBufInfo.u4Stride[2]};
    size_t bufBoundaryInBytes[] = {0,0,0};
    IImageBufferAllocator::ImgParam imgParam = IImageBufferAllocator::ImgParam(
                                                   (MINT32)rSwNRImgBufInfo.eImgFmt, 
                                                   MSize(
                                                       rSwNRImgBufInfo.u4ImgWidth,
                                                       rSwNRImgBufInfo.u4ImgHeight), 
                                                   bufStridesInBytes, 
                                                   bufBoundaryInBytes, 
                                                   Format::queryPlaneCount((MINT32)rSwNRImgBufInfo.eImgFmt));
    PortBufInfo_v1 portBufInfo = PortBufInfo_v1(
                                     rSwNRImgBufInfo.i4MemID,
                                     (MUINTPTR)rSwNRImgBufInfo.u4BufVA,
                                     0,
                                     rSwNRImgBufInfo.i4BufSecu,
                                     rSwNRImgBufInfo.i4BufCohe);
    char value[PROPERTY_VALUE_MAX] = {'\0'}; 
    property_get( "vendor.debug.enable.imagebuffer.log", value, "0");
    MBOOL mbEnableIImageBufferLog = atoi(value);
    ...
    IImageBuffer* tempIImageBuffer = pHeap->createImageBuffer();
    //reference defaultbufhandler.cpp end

    //reference to pass2node.capture.cpp
    if ( !tempIImageBuffer->lockBuf( LOG_TAG, eBUFFER_USAGE_SW_MASK | eBUFFER_USAGE_HW_MASK ) )
    {
        MY_LOGE("lock Buffer failed\n");
        return;
    }

    if ( !tempIImageBuffer->syncCache( eCACHECTRL_INVALID ) )
    {
        MY_LOGE("syncCache failed");
        return;
    }
    //reference to pass2node.capture.cpp end
#if SWNR_DEBUG
    MY_LOGD("getImgFormat():%d", tempIImageBuffer->getImgFormat()); 
    MY_LOGD("getImgSize().w:%d", tempIImageBuffer->getImgSize().w); 
    MY_LOGD("getImgSize().h:%d", tempIImageBuffer->getImgSize().h); 
    MY_LOGD("getBufVA(0):%d", tempIImageBuffer->getBufVA(0)); 
    MY_LOGD("getBufVA(1):%d", tempIImageBuffer->getBufVA(1)); 
    MY_LOGD("getBufVA(2):%d", tempIImageBuffer->getBufVA(2)); 
#endif

    //(4)doSwNR
    int scale = 100*(mShotParam.mi4PictureWidth+mShotParam.mi4PictureHeight)/(mSwNRCropRect.w+mSwNRCropRect.h);
    SwNRParam* pParam = SwNRParam::getInstance(getOpenId());
    if( !pParam )
    {
        MY_LOGE("cannot get SwNRParam, skip SwNR");
        return;
    }
    else
    {
        MY_LOGD("setParam(), mIso=%d,scale=%d,crop(w,h)=(%d,%d),jpeg(w,h)=(%d,%d)",mIso,scale,mSwNRCropRect.w,mSwNRCropRect.h,mShotParam.mi4PictureWidth,mShotParam.mi4PictureHeight);
        pParam->setParam(mIso,scale);
    }

    if( !mpSwNR )
    {
        MY_LOGD("[doSwNR] new SwNR( getOpenId() = %d )",getOpenId());
        mpSwNR = new SwNR( getOpenId() );//getSensorIdx()
    }
    if( mpSwNR )
    {
        MY_LOGD("[doSwNR] SwNR exist");
        mpSwNR->doSwNR(tempIImageBuffer);
    }
#if SWNR_DEBUG
    SWNR_DEBUG_DUMP(rSwNRImgBufInfo);
#endif
    ...
    //(5)transform to YUV
    MY_LOGD("new ISImager 2");
    pISImager = ISImager::createInstance(rSwNRImgBufInfo);
    pISImager->setTargetBufInfo(rDstImgBufInfo);
    pISImager->setFormat(rDstImgBufInfo.eImgFmt);
    pISImager->setRotation(0);
    pISImager->setFlip(0);
    pISImager->setResize(rDstImgBufInfo.u4ImgWidth, rDstImgBufInfo.u4ImgHeight);
    pISImager->setROI(Rect(0, 0, rDstImgBufInfo.u4ImgWidth, rDstImgBufInfo.u4ImgHeight));
...
onYuv_capture(CapBufQueNode & rNode)
{
    ...
    MY_LOGD_IF((unsigned int)mShotParam.mi4Rotation != rNode.mainImgNode.getRotation(), "Shot Param/Buffer Info rotation not sync");
...
#if 1
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.debug.camera.swnr.enable", value, "1");
    Capture_NR_Th_t NR_thres;
    NSCam::IHalSensorList* pHalSensorList = NSCam::IHalSensorList::get();
    SensorStaticInfo sensorStaticInfo;
    pHalSensorList->querySensorStaticInfo(pHalSensorList->querySensorDevIdx(getOpenId()), &sensorStaticInfo);
    MY_LOGD("sensorType=%d", sensorStaticInfo.sensorType);
    if( sensorStaticInfo.sensorType != SENSOR_TYPE_YUV )
    {
        get_capture_nr_th(
                ...
                getShotMode(),
                &NR_thres);
        NS3A::Hal3ABase *p3AObj = Hal3ABase::createInstance(static_cast<MUINT32>(MtkCamUtils::DevMetaInfo::queryHalSensorDev(getOpenId())));
        CaptureParam_T a_rCaptureInfo;
        p3AObj->getCaptureParams(0,0,a_rCaptureInfo);
        MY_LOGD("sensorDev=%d,shotMode=%d,iso=%d,swth=%d,enable=%d",pHalSensorList->querySensorDevIdx(getOpenId()),getShotMode(),a_rCaptureInfo.u4RealISO,NR_thres.swth,atoi(value));
        if ( (atoi(value) == 2) || (a_rCaptureInfo.u4RealISO >= NR_thres.swth && atoi(value) == 1) )
        {
            // udpate iso info to swnr
            SwNRParam* pParam = SwNRParam::getInstance(getOpenId());
            if( !pParam )
            {
                MY_LOGE("cannot get SwNRParam, skip SwNR");
            ...
            else
            {
                ...
                pHalSensorList->querySensorStaticInfo(
                                    pHalSensorList->querySensorDevIdx(getOpenId()),
                                    &mSensorStaticInfo);
                NSCamHW::Rect rSrc(0,0,mSensorStaticInfo.captureWidth,mSensorStaticInfo.captureHeight);//sensor
                NSCamHW::Rect rDst(0,0,mShotParam.mi4PictureWidth, mShotParam.mi4PictureHeight);//jpeg picture size

                int ratio = mShotParam.mu4ZoomRatio;
                ...
                MY_LOGD("SrcRect(w,h)=(%d,%d),DstRect(w,h)=(%d,%d),ratio=%d",rSrc.w,rSrc.h,rDst.w,rDst.h,ratio);
                NSCamHW::Rect rCrop = MtkCamUtils::calCrop(rSrc, rDst, ratio);

                mIso = a_rCaptureInfo.u4RealISO;
                mSwNRCropRect.w = rCrop.w;
                mSwNRCropRect.h = rCrop.h;
                doSwNR(rMainImgBufInfo);
            }
        }
...
onRaw_capture(CapBufQueNode & rNode)
{
    ...
    if( sensorStaticInfo.sensorType != SENSOR_TYPE_YUV )
    {
        ...
        if ( (atoi(value) == 2) || (a_rCaptureInfo.u4RealISO >= NR_thres.swth && atoi(value) == 1) )
        {
            MY_LOGD("set doSwNR true, pass iso = %d",a_rCaptureInfo.u4RealISO);
            pSingleShot->setSwNR(true, a_rCaptureInfo.u4RealISO);
        }
        p3AObj->destroyInstance();
    }
#endif
