...
MINT32 _convertResultToMeta(MUINT32 u4SensorDev, const Result_T& rResult, const IMetadata& staticMeta, MetaSet_T& rMetaResult)
{
    ...
    /*2127041*/
    MY_LOG_IF(1, "[%s] #(%d) AWB(%d) AE(%d) AF(%d) Lens(%d) Flash(%d) Flk(%d)", __FUNCTION__,
    ...
    MY_LOG_IF(fgLogPf, "[%s] ExpTime(%" PRId64 ") FrmDuration(%" PRId64 ") ISO(%d) RSS(%" PRId64 ")", __FUNCTION__,
    ...
    {
        MRational rMat;
        MFLOAT fVal = rResult.fColorCorrectMat[k];
    ...
    for (i = 0; i < src.count(); i++)
    {
        ...
        case MTK_SENSOR_INFO_MAX_FRAME_DURATION:
            ...
            for (j = 0; j < entry.count(); j++)
            {
                ...
                MY_LOG_IF(fgLogPf, "[%s] val(%" PRId64 ")", __FUNCTION__, val);
            }
            break;
...
#define _TEST_SCENE_MODE    (1<<16)
    ...
    if (u4Flag & _TEST_MANUAL_WB)
    {
        ...
        rParam.u4AwbMode = (u4Magic % 2) ? MTK_CONTROL_AWB_MODE_OFF : rParam.u4AwbMode;
        rParam.u1ColorCorrectMode = (u4Magic % 2) ? MTK_COLOR_CORRECTION_MODE_TRANSFORM_MATRIX : rParam.u1ColorCorrectMode;
        u4Channel = u4Wb & 0x3;
        rParam.fColorCorrectGain[u4Channel] = 1.0f; //(u4Magic % 2) ? 1.0f : 2.0f;
    ...
    if (u4Flag & _TEST_TONEMAP2)
    {
        ...
        MUINT32 u4Tonemap = 0;
        MFLOAT fGamma = 0;
        MINT8 u1PresetCurve = 0;
        GET_PROP("vendor.debug.hal3av3.tonemap2", "0", u4Tonemap);
        GET_PROP("vendor.debug.hal3av3.tonemap_gamma", "2.2", fGamma);
        GET_PROP("vendor.debug.hal3av3.tonemap_preset", "0", u1PresetCurve);
        //MY_LOG("fGamma(%f)",fGamma);
        if (u4Tonemap == 0)
        {
        ...
        else if (u4Tonemap == 12)
        {

            rParam.u1TonemapMode = MTK_TONEMAP_MODE_GAMMA_VALUE;
            rParam.fGamma = fGamma;
        }
        else if (u4Tonemap == 13)
        {
        	rParam.u1TonemapMode = MTK_TONEMAP_MODE_PRESET_CURVE;
            rParam.u1PresetCurve = u1PresetCurve;
        }
        else
        {
            rParam.u1TonemapMode = MTK_TONEMAP_MODE_FAST;
...
MINT32
Hal3AAdapter3::
start(MINT32 i4StartNum __unused)
{
    MY_LOG("[%s] mi4SensorIdx(0x%04x), mpHal3aObj(%p)",
            __FUNCTION__, mi4SensorIdx, mpHal3aObj);
    mMetaResultQueue.clear();

    MBOOL bIsAELock = mParams.bIsAELock;
    mParams = Param_T();
    mParams.bIsAELock = bIsAELock;

    setupStaticInfo();
    mpHal3aObj->sendCommand(NS3Av3::ECmd_CameraPreviewStart);

    MY_LOG("[%s] Update 1", __FUNCTION__);
    NS3Av3::ParamIspProfile_T _3AProf(NSIspTuning::EIspProfile_NormalPreview, 1, 0, MTRUE, NS3Av3::ParamIspProfile_T::EParamValidate_None);
...
stop()
{
    ...
    {
        MY_ERR("[%s] Not started yet! mi4SensorIdx(0x%04x), mpHal3aObj(%p)",
                    __FUNCTION__, mi4SensorIdx, mpHal3aObj);
        return 1;
    }
}
...
init()
{
    ...
    else
    {
        ...
        MY_LOG("[%s] mi4SensorIdx(0x%04x), mu4SensorDev(0x%04x), mpHal3aObj(%p)",
...
set(const List<MetaSet_T>& requestQ)
{
    ...
    MY_LOG_IF(fgLog, "[%s] mi4SensorIdx(%d), mu4Counter(%d), queueSize(%zu)", __FUNCTION__, mi4SensorIdx, mu4Counter++, requestQ.size());
    ...
    for (i = 0; i < getCapacity() && it != requestQ.end(); i++, it++)
    {
        ...
        if (i == HAL3A_REQ_PROC_KEY)
        {
            ...
            {
                if (mParams.u1IsGetExif==1)
                    MY_LOG_IF(fgLog, "[%s] Request EXIF(1->0)", __FUNCTION__);
            ...
            // for Raw type
            if (!QUERY_ENTRY_SINGLE(it->halMeta, MTK_P1NODE_RAW_TYPE, mParams.i4RawType))
                mParams.i4RawType = ERawType_Proc;
            MY_LOG_IF(1, "[%s] i4RawType(%d)", __FUNCTION__, mParams.i4RawType);
	...
	if(QUERY_ENTRY_SINGLE(metadata, MTK_3A_FEATURE_AE_REQUEST_ISO_SPEED, u4IsoSpeedMode))
	{
	    MY_LOG_IF(mParams.u4IsoSpeedMode != u4IsoSpeedMode, "[%s] MTK_3A_FEATURE_AE_REQUEST_ISO_SPEED(%d -> %d)", __FUNCTION__, mParams.u4IsoSpeedMode, u4IsoSpeedMode);
	    mParams.u4IsoSpeedMode = u4IsoSpeedMode;
	}

	MINT32 u4AwbValue = 0;
	if(QUERY_ENTRY_SINGLE(metadata, MTK_3A_FEATURE_AWB_REQUEST_VALUE, u4AwbValue))
	{
	    MY_LOG_IF(mParams.u4AwbValue != u4AwbValue, "[%s] MTK_3A_FEATURE_AWB_REQUEST_VALUE(%d -> %d)", __FUNCTION__, mParams.u4AwbValue, u4AwbValue);
	    mParams.u4AwbValue = u4AwbValue;
	}

	MUINT8 u1AeMeteringMode = 0;
	if(QUERY_ENTRY_SINGLE(metadata, MTK_3A_FEATURE_AE_REQUEST_METERING_MODE, u1AeMeteringMode))
	{
	    MY_LOG_IF(mParams.u4AeMeterMode != u1AeMeteringMode, "[%s] MTK_3A_FEATURE_AE_REQUEST_METERING_MODE(%d -> %d)", __FUNCTION__, mParams.u4AeMeterMode, u1AeMeteringMode);
        ...
        for (j = 0; j < cnt; j++)
        {
	     ...
	     mtk_camera_metadata_tag_t tag = (mtk_camera_metadata_tag_t)entry.tag();
            ...
            case MTK_SENSOR_EXPOSURE_TIME:
                {
                    ...
                    MY_LOG_IF(fgLogEn1, "[%s] MTK_SENSOR_EXPOSURE_TIME(%" PRId64 ")", __FUNCTION__, i8AeExposureTime);
                }
                break;
            ...
            case MTK_SENSOR_FRAME_DURATION:
                {
                    ...
                    MY_LOG_IF(fgLogEn1, "[%s] MTK_SENSOR_FRAME_DURATION(%" PRId64 ")", __FUNCTION__, i8FrameDuration);
                }
                break;
            ...
            case MTK_TONEMAP_MODE:
                ...
                break;
            case MTK_TONEMAP_GAMMA:
                {
                    MFLOAT fGamma = entry.itemAt(0, Type2Type< MFLOAT >());
                    mParams.fGamma = fGamma;
                    MY_LOG_IF(fgLogEn1, "[%s] MTK_TONEMAP_GAMMA(%f)", __FUNCTION__, fGamma);
                }
                break;
            case MTK_TONEMAP_PRESET_CURVE:
                {
                    MINT8 u1PresetCurve = entry.itemAt(0, Type2Type< MUINT8 >());
                    mParams.u1PresetCurve = u1PresetCurve;
                    MY_LOG_IF(fgLogEn1, "[%s] MTK_TONEMAP_PRESET_CURVE(%d)", __FUNCTION__, u1PresetCurve);
                }
                break;
    ...
    {
        MY_LOG_IF(fgLogEn1, "[%s] Queue fulls (%zu). Remove front", __FUNCTION__, mMetaResultQueue.size());
        ...
        default:
            {
                if (!m_ScnModeOvrd.isEmpty())
                {
                    ...
                    //mParams.u4AeMode  = rOverride.u1AeMode;
                    //mParams.u4AwbMode = rOverride.u1AwbMode;
                    //mParams.u4AfMode  = rOverride.u1AfMode; //_convertAFMode(rOverride.u1AfMode);
		      ...
		      mParams.u4AwbMode = (u1Override3A & OVERRIDE_AWB) ? rOverride.u1AwbMode : mParams.u4AwbMode;
                    mParams.u4AfMode  = (u1Override3A & OVERRIDE_AF) ? rOverride.u1AfMode : mParams.u4AfMode;
    ...
    {
        NSIspTuning::EIspProfile_T prof = NSIspTuning::EIspProfile_NormalPreview;
        switch (u1CapIntent)
        {
        case MTK_CONTROL_CAPTURE_INTENT_PREVIEW:
            prof = NSIspTuning::EIspProfile_NormalPreview;
            break;
        case MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE:
        case MTK_CONTROL_CAPTURE_INTENT_VIDEO_SNAPSHOT:
            prof = NSIspTuning::EIspProfile_NormalCapture;
            break;
        case MTK_CONTROL_CAPTURE_INTENT_VIDEO_RECORD:
            prof = NSIspTuning::EIspProfile_VideoPreview;
            break;
        }
...
get(MUINT32 frmId, MetaSet_T& result)
{
    ...
    /* Flash state should use the result of next frame, corresponding to Hal3ARaw::getCurrResult() */
    Result_T rResult_Next;
    if (-1 == mpHal3aObj->getResult(frmId+1, rResult_Next))
    {
        CAM_LOGW("[%s] Fail to get R[%d], current result will be obtained.", __FUNCTION__, frmId+1);
    }
    rResult.u1FlashState = rResult_Next.u1FlashState;
    ...
    for (; it != mMetaResultQueue.end(); it++)
    {
	   ...
	   MINT32 i4AppSize = 0;
	   MINT32 i4HalSize = 0;
          i4AppSize = it->appMeta.count();
          i4HalSize = it->halMeta.count();
          result.appMeta = it->appMeta;
          result.halMeta = it->halMeta;
          CAM_LOGD("[%s] i4Id(%d)/AppSize(%d->%d)/HalSize(%d->%d)", __FUNCTION__, u4Id, i4AppSize, result.appMeta.count(), i4HalSize, result.halMeta.count());
          _convertResultToMeta(rResult, mMetaStaticInfo, result);
          break;
#endif
        }
    }
    ...
    if (it == mMetaResultQueue.end())
    {
	    ...
	    MINT32 i4AppSize = 0;
	    MINT32 i4HalSize = 0;
        i4AppSize = mMetaResult.appMeta.count();
        i4HalSize = mMetaResult.halMeta.count();

         CAM_LOGW("[%s] fail to get the specified results, use current. i4Id(%d)/AppSize(%d)/HalSize(%d)", __FUNCTION__, i4Id, i4AppSize, i4HalSize);

		//QUERY_ENTRY_SINGLE(mMetaResult.halMeta, MTK_P1NODE_PROCESSOR_MAGICNUM, i4Id);
         result.appMeta = mMetaResult.appMeta;
	     result.halMeta = mMetaResult.halMeta;
          _convertResultToMeta(rResult, mMetaStaticInfo, result);
    #endif
	}
...
setIsp(MINT32 flowType, const MetaSet_T& control, void* pRegBuf, MetaSet_T* pResult)
{
    ...
    if (GET_ENTRY_ARRAY(control.halMeta, MTK_PROCESSOR_CAMINFO, rCamInfo.data, sizeof(NSIspTuning::RAWIspCamInfo_U)))
    {
        ...
        MUINT8 u1IspProfile = NSIspTuning::EIspProfile_NormalPreview;
...
MINT32
Hal3AAdapter3::
getDelay(IMetadata& delay_info __unused) const
{
    return 0;
}
...
MINT32
Hal3AAdapter3::
getDelay(MUINT32 tag __unused) const
{
...
doNotifyCb(MINT32 _msgType, MINTPTR _ext1, MINTPTR _ext2, MINTPTR _ext3)
{
    ...
    case eID_NOTIFY_3APROC_FINISH:
        MY_LOG_IF((mu4LogEn & HAL3AADAPTER3_LOG_PF), "[%s] _msgType(%d), _ext1(%" PRIdPTR "), _ext2(%" PRIdPTR "), _ext3(%" PRIdPTR ")", __FUNCTION__, _msgType, _ext1, _ext2, _ext3);
...
void
Hal3AAdapter3::
doDataCb(int32_t _msgType __unused, void* _data __unused, uint32_t _size __unused)
{}
