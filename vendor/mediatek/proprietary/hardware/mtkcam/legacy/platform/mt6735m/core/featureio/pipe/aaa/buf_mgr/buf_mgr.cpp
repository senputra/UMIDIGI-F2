...
//    MY_LOG_IF(m_bDebugEnable,"[m_pVirtIspDrvCQ0]:0x%08x \n", reinterpret_cast<MUINT32>(m_pVirtIspDrvCQ0));
  ...
  //  MY_LOG_IF(m_bDebugEnable,"[m_pVirtIspRegCQ0]:0x%08x \n", reinterpret_cast<MUINT32>(m_pVirtIspRegCQ0));
    ...
    if(MTRUE != DMAInit(camdma2type<ECamDMA_AAO>()))
      MY_ERR("DMAInit() fail \n");

    // Enable AA stat
    if(MTRUE != AAStatEnable(MTRUE))
      MY_ERR("AAStatEnable fail \n");
    ...
    if(MTRUE != DMAInit(camdma2type<ECamDMA_AFO>()))
     MY_ERR("DMAInit() fail \n");

    // Enable AF stat
    if(MTRUE != AFStatEnable(MTRUE))
     MY_ERR("AFStatEnable fail \n");
...
debugPrint(MINT32 i4DmaChannel)
{
    ...
    for (it = m_rHwBufList[i4DmaChannel].begin(); it != m_rHwBufList[i4DmaChannel].end(); it++ ) {
        MY_LOG_IF(m_bDebugEnable,"m_rHwBufList[%d].virtAddr:[0x%" PRIXPTR "]/phyAddr:[0x%" PRIXPTR "] \n",i4DmaChannel,it->virtAddr,it->phyAddr);
    }
