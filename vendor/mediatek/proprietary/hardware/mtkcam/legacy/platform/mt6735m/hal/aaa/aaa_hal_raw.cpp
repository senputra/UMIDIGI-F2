...
initRaw(MINT32 i4SensorDevId, MINT32 i4SensorOpenIndex)
{
    ...
    property_get("vendor.debug.aaa.ctrl.enable", value, "1023");
...
if (ENABLE_AF & m_3ACtrlEnable) {
    AfMgr::getInstance().setCallbacks(mpCbSet);
    //
}
...
{
    mP2Mtx.lock();
    IspTuningMgr::getInstance().validatePerFrameP2(flowType, rCamInfo, pRegBuf);
    mP2Mtx.unlock();
    return MTRUE;
}
...
{
    IspTuningMgr::getInstance().setIspProfile(rParamIspProfile_.eIspProfile);
    IspTuningMgr::getInstance().validatePerFrame(rParamIspProfile_.i4MagicNum, fgPerframe);
    return MTRUE;
}
...
getDebugInfo(android::Vector<MINT32>& keys, android::Vector< android::Vector<MUINT8> >& data) const
{
    ...
    DEBUG_RESERVEA_INFO_T& rShadigDebugInfo = *(DEBUG_RESERVEA_INFO_T*) pBuf;
...
getCurrResult(MUINT32 u4FrmId, Result_T& rResult) const
{
    ...
    mtk_camera_metadata_enum_android_control_awb_state_t eAwbState = MTK_CONTROL_AWB_STATE_INACTIVE; // add initial value for Coverity
    ...
    IspTuningMgr::getInstance().getColorCorrectionTransform(
        rResult.fColorCorrectMat[0], rResult.fColorCorrectMat[1], rResult.fColorCorrectMat[2],
        rResult.fColorCorrectMat[3], rResult.fColorCorrectMat[4], rResult.fColorCorrectMat[5],
        rResult.fColorCorrectMat[6], rResult.fColorCorrectMat[7], rResult.fColorCorrectMat[8]
    );

    // AF
    rResult.u1AfState = AfMgr::getInstance().getAFState();
    if (mpStateMgr->getStateStatus().eCurrState == eState_AF || mpStateMgr->getStateStatus().eNextState == eState_AF)
        //rResult.u1AfState = MTK_CONTROL_AF_STATE_ACTIVE_SCAN;
     {
	    if( m_rParam.u4AfMode == MTK_CONTROL_AF_MODE_AUTO || m_rParam.u4AfMode == MTK_CONTROL_AF_MODE_MACRO)
	        rResult.u1AfState = MTK_CONTROL_AF_STATE_ACTIVE_SCAN;
	    else if( m_rParam.u4AfMode == MTK_CONTROL_AF_MODE_CONTINUOUS_PICTURE)
	        rResult.u1AfState = MTK_CONTROL_AF_STATE_PASSIVE_SCAN;
	    else if( m_rParam.u4AfMode == MTK_CONTROL_AF_MODE_CONTINUOUS_VIDEO )
	        rResult.u1AfState = MTK_CONTROL_AF_STATE_NOT_FOCUSED_LOCKED;
      }
    MY_LOG("AF Mode(%d)/AF State(%d)", m_rParam.u4AfMode, rResult.u1AfState);

     rResult.u1LensState = AfMgr::getInstance().getLensState();
     rResult.fLensFocusDistance = AfMgr::getInstance().getFocusDistance();
     AfMgr::getInstance().getFocusRange(&rResult.fLensFocusRange[0], &rResult.fLensFocusRange[1]);

    /* The n-th FLASH_STATE cannot be determined without n+1-th request due to STATE_PARTIAL */
    /* Here we return n-1 state to result, and additional steps should be done in Hal3AAdapter3::get() */
    rResult.u1FlashState = FlashMgr::getInstance().getLastFrameFlashState(m_i4SensorDev);
#if 0
...
#endif
    // Flicker
    MINT32 i4FlkStatus = 0;
    FlickerHalBase::getInstance().getFlickerResult(i4FlkStatus);
    MUINT8 u1ScnFlk = MTK_STATISTICS_SCENE_FLICKER_NONE;
    if (i4FlkStatus == HAL_FLICKER_AUTO_50HZ) u1ScnFlk = MTK_STATISTICS_SCENE_FLICKER_50HZ;
    if (i4FlkStatus == HAL_FLICKER_AUTO_60HZ) u1ScnFlk = MTK_STATISTICS_SCENE_FLICKER_60HZ;
    ...
    if (m_rParam.u1TonemapMode != MTK_TONEMAP_MODE_CONTRAST_CURVE)
    {
        ...
        MFLOAT *pIn, *pOut;
        MINT32 i4NumPt;
        IspTuningMgr::getInstance().getTonemapCurve_Blue(pIn, pOut, &i4NumPt);
        for (i = 0; i < i4NumPt; i++)
        {
            rResult.vecTonemapCurveBlue.push_back(*pIn++);
            rResult.vecTonemapCurveBlue.push_back(*pOut++);
        }
        IspTuningMgr::getInstance().getTonemapCurve_Green(pIn, pOut, &i4NumPt);
        for (i = 0; i < i4NumPt; i++)
        {
            rResult.vecTonemapCurveGreen.push_back(*pIn++);
            rResult.vecTonemapCurveGreen.push_back(*pOut++);
        }
        IspTuningMgr::getInstance().getTonemapCurve_Red(pIn, pOut, &i4NumPt);
        for (i = 0; i < i4NumPt; i++)
        {
    ...
    if (!IspTuningMgr::getInstance().getCamInfo(rCamInfo))
    {
    ...
    // Exif
    if (m_rParam.u1IsGetExif || mbIsCapEnd)
    {
        rResult.vecExifInfo.resize(1);
        MY_LOG("[%s] u4FrmId(%d), get3AEXIFInfo", __FUNCTION__, u4FrmId);    // pass1 result
...
{
    MY_LOG_IF(m_3ALogEnable, "[%s] u4MagicNum(%d), m_u4FrmIdStat(%d)", __FUNCTION__, u4MagicNum, m_u4FrmIdStat);    // pass1 result
...
MBOOL Hal3ARaw::postCommandDerived(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    if (eCmd == ECmd_CameraPreviewEnd)
    {
        ...
        /*if (!bRet) AEE_ASSERT_3A_HAL("sem_wait_relativeTime(semAFProcEnd).");*/ //2315009
    ...
    mbIsCapEnd = MFALSE;
    MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG,"[Hal3ARaw::postCommand] before StateMgr::sendCmd(%d)\n", eCmd);
    if (mbMainFlashOnThisFrame && (eCmd == ECmd_Update) && (m_rParam.u4AeMode != MTK_CONTROL_AE_MODE_OFF && m_rParam.u4AeMode != MTK_CONTROL_AE_MODE_ON))
    {
    ...
    } else if( (m_i4HqcCount >= 0) && (eCmd == ECmd_Update) && (m_rParam.u4AeMode != MTK_CONTROL_AE_MODE_OFF && m_rParam.u4AeMode != MTK_CONTROL_AE_MODE_ON))
    {
        MY_LOG("m_i4HqcCount = %d", m_i4HqcCount);
        if((m_i4HqcCount == 0) && (mpStateMgr->getStateStatus().eCurrState == eState_CameraPreview))
        {
            mpStateMgr->sendCmd(ECmd_CaptureEnd);
            mbIsCapEnd = MTRUE;
        }
        m_i4HqcCount--;
    }
...
MBOOL Hal3ARaw::setParams(Param_T const &rNewParam)
{
    ...
    // ====================================== ISP tuning =============================================
    IspTuningMgr::getInstance().setSceneMode(rNewParam.u4SceneMode);
    IspTuningMgr::getInstance().setEffect(rNewParam.u4EffectMode);
    IspTuningMgr::getInstance().setIspUserIdx_Bright(rNewParam.u4BrightnessMode);
    IspTuningMgr::getInstance().setIspUserIdx_Hue(rNewParam.u4HueMode);
    IspTuningMgr::getInstance().setIspUserIdx_Sat(rNewParam.u4SaturationMode);
    IspTuningMgr::getInstance().setIspUserIdx_Edge(rNewParam.u4EdgeMode);
    IspTuningMgr::getInstance().setIspUserIdx_Contrast(rNewParam.u4ContrastMode);
    //IspTuningMgr::getInstance().setCamMode(rNewParam.u4CamMode);
    IspTuningMgr::getInstance().setEdgeMode(rNewParam.u1EdgeMode ? MTK_EDGE_MODE_FAST : MTK_EDGE_MODE_OFF);
    IspTuningMgr::getInstance().setNoiseReductionMode(rNewParam.u1NRMode);
    if (rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_GAMMA_VALUE || rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_PRESET_CURVE)
        IspTuningMgr::getInstance().setToneMapMode(MTK_TONEMAP_MODE_CONTRAST_CURVE);
    else
        IspTuningMgr::getInstance().setToneMapMode(rNewParam.u1TonemapMode);
    if (rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_GAMMA_VALUE || rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_PRESET_CURVE)
    {
        MINT8 type = 0; //use gamma or not
        MFLOAT gamma_val = 0.0;
        if (rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_GAMMA_VALUE)
        {
            gamma_val = (MFLOAT)rNewParam.fGamma;
        }
        else if (rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_PRESET_CURVE)
        {
            type = rNewParam.u1PresetCurve;
            if (type == 0)  gamma_val = 2.2;  
        }
        
        //MY_LOG("gamma_val(%f)",gamma_val);
        
        android::Vector<MFLOAT> vecIn, vecOut, vec_x, vec_y;
        MINT32 i4Cnt = 32;
        vecIn.resize(i4Cnt);
        vecOut.resize(i4Cnt);
        vec_x.resize(i4Cnt);
        ...
        MFLOAT* pArray_x = vec_x.editArray();
        MFLOAT* pArray_y = vec_y.editArray();
		    if (type == 0)  //gamma or preset with gamma 2.2
		    {
		        for (MINT32 i = 0; i < i4Cnt; i++)
		        {   
		            MFLOAT x, y;
		            x = (MFLOAT)i/(i4Cnt - 1);
		            y = pow(x,1.0/gamma_val);
		            *pArrayIn++ = x;
		            *pArrayOut++ = y;
		            MY_LOG_IF(m_3ALogEnable, "[TONEMAP](%f,%f)", x, y);
		        }
		    }
		    else if (type == 1)
		    {
		        for (MINT32 i = 0; i < i4Cnt; i++)
		        {
		            ...
		            if(x <= 0.018)
		                ...
		                y = 1.099 * pow(x,0.45) - 0.099;
		            *pArrayIn++ = x;     
		            *pArrayOut++ = y;
		            MY_LOG_IF(m_3ALogEnable, "[TONEMAP](%f,%f)", x, y);
		        }
		    }
        //IspTuningMgr::getInstance().GMA_curve(vecIn.editArray(), vecOut.editArray(), &type, &i4Cnt);
        
        //memcpy(vec_x.editArray(), vecIn.editArray(), i4Cnt);
        //memcpy(vec_y.editArray(), vecOut.editArray(), i4Cnt);
        //for (MINT32 i = 0; i < i4Cnt; i++){MY_LOG("[TONEMAP][vec_temp](%f,%f)", (MFLOAT)*pArray_x++, (MFLOAT)*pArray_y++);}
        pArrayIn = vecIn.editArray();
        pArrayOut = vecOut.editArray();
        pArray_x = vec_x.editArray();
        pArray_y = vec_y.editArray();
        for (MINT32 i = 0; i < i4Cnt; i++){*pArray_x++ =  *pArrayIn++;*pArray_y++ = *pArrayOut++;}
        IspTuningMgr::getInstance().setTonemapCurve_Red(vecIn.editArray(), vecOut.editArray(), &i4Cnt);
        //for (MINT32 i = 0; i < i4Cnt; i++){MY_LOG("[TONEMAP][vecIn][before_red](%f,%f)", vecIn, vecOut);}
        //memcpy(vecIn.editArray(), vec_x.editArray(), vec_x.size());
        //memcpy(vecOut.editArray(), vec_y.editArray(), vec_y.size());
        //for (MINT32 i = 0; i < i4Cnt; i++){MY_LOG("[TONEMAP][vecIn][before_green](%f,%f)", vecIn, vecOut);}
        pArrayIn = vecIn.editArray();
        pArrayOut = vecOut.editArray();
        pArray_x = vec_x.editArray();
        pArray_y = vec_y.editArray();
        for (MINT32 i = 0; i < i4Cnt; i++){*pArrayIn++ =  *pArray_x++;*pArrayOut++ = *pArray_y++;}
        IspTuningMgr::getInstance().setTonemapCurve_Green(vecIn.editArray(), vecOut.editArray(), &i4Cnt);
        //memcpy(vecIn.editArray(), vec_x.editArray(), vec_x.size());
        //memcpy(vecOut.editArray(), vec_y.editArray(), vec_y.size());
        pArrayIn = vecIn.editArray();
        pArrayOut = vecOut.editArray();
        ...
        pArray_y = vec_y.editArray();
        for (MINT32 i = 0; i < i4Cnt; i++){*pArrayIn++ =  *pArray_x++;*pArrayOut++ = *pArray_y++;}
        IspTuningMgr::getInstance().setTonemapCurve_Blue(vecIn.editArray(), vecOut.editArray(), &i4Cnt);
    }
    ...
    IspTuningMgr::getInstance().setColorCorrectionMode(rNewParam.u1ColorCorrectMode);
        ...
        rNewParam.u1ColorCorrectMode == MTK_COLOR_CORRECTION_MODE_TRANSFORM_MATRIX)
    {
        ...
        IspTuningMgr::getInstance().setColorCorrectionTransform(
    ...
    // ====================================== AF ==============================================
    AfMgr::getInstance().setAFMode(rNewParam.u4AfMode);
    if (m_bFaceDetectEnable == MFALSE) AfMgr::getInstance().setAFArea(rNewParam.rFocusAreas);
    //AfMgr::getInstance().setCamMode(rNewParam.u4CamMode);
    AfMgr::getInstance().setFullScanstep(rNewParam.i4FullScanStep);
    AfMgr::getInstance().setMFPos(rNewParam.i4MFPos);
    AfMgr::getInstance().setAndroidServiceState(rNewParam.bIsSupportAndroidService);
    if (rNewParam.u4AfMode == MTK_CONTROL_AF_MODE_OFF)
    {
        // focus dist
        AfMgr::getInstance().setFocusDistance(rNewParam.fFocusDistance);
    }
...
if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
    ...
    {
        AfMgr::getInstance().setFDInfo(prFaces);
...
if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
    ...
    IspTuningMgr::getInstance().setSensorMode(i4SensorMode);
    FlickerHalBase::getInstance().setSensorMode(m_i4SensorDev, i4SensorMode);
}
    IspTuningMgr::getInstance().setIspProfile(NSIspTuning::EIspProfile_NormalPreview);
    IspTuningMgr::getInstance().validate(0, MTRUE);
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR i4Arg1, MINTPTR i4Arg2)
{
    ...
    case E3ACtrl_GetExposureParam:
        {
            AE_MODE_CFG_T rCaptureInfo;
            // add initial value for Coverity
            rCaptureInfo.u4ExposureMode = 0;
            rCaptureInfo.u4Eposuretime = 0;
            rCaptureInfo.u4AfeGain = 0;
            rCaptureInfo.u4IspGain = 0;
            rCaptureInfo.u4RealISO = 0;
            rCaptureInfo.i2FlareGain = 0;
            rCaptureInfo.i2FlareOffset = 0;
            IAeMgr::getInstance().getCaptureParams(m_i4SensorDev, i4Arg1, 0, rCaptureInfo);
            CaptureParam_T &a_rCaptureInfo = *reinterpret_cast<CaptureParam_T*>(i4Arg2);
...
*/
            IEventIrq::ConfigParam IrqConfig(1, 0, IEventIrq::E_Event_Af);
