...
int ret = 0;
    ...
    if (!m_fgIsGdmaMode)    // Normal Mode (i.e. not GDMA).
    {
        ...
        {
            tpipeBufInfo[q].size = MAX_ISP_TILE_TDR_HEX_NUM;
            ...
            ISP_FUNC_INF("[Tpipe alloc]q(%d),va/pa(0x%lx, 0x%08x)",q,(unsigned long)g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
        }
    }
    else    // GDMA mode.
    {
    ...
    ISP_FUNC_INF("[m_pIspDrv]:0x%lx ",(unsigned long)m_pIspDrv);
    if (!m_pIspDrv) {
    ...
    ISP_FUNC_DBG("[m_pIspReg]:0x%lx ",(unsigned long)m_pIspReg);
    if ( NULL == m_pIspReg ) {
    ...
    ISP_FUNC_DBG("[m_pCdpDrv]:0x%lx ",(unsigned long)m_pCdpDrv);
    if ( NULL == m_pCdpDrv ) {
    ...
    if (!m_fgIsGdmaMode)    // Normal Mode (i.e. not GDMA).
    {
        ...
        ISP_FUNC_DBG("[m_pTdriDrv]:0x%lx ",(unsigned long)m_pTpipeDrv);
        if ( NULL == m_pTpipeDrv ) {
...
//js_test move to _disable.
    ...
    for(int q=0;q<ISP_DRV_CQ_NUM;q++)
    {
        ...
        ISP_FUNC_INF("[Tpipe free]q(%d),va/pa(0x%lx, 0x%08x)",q,(unsigned long)g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
    }
...
int ret = MTRUE;
    ...
    //
    ISP_FUNC_DBG("m_pIspReg:0x%lx ",(unsigned long)m_pVirtIspReg[cq]);
...
MINT32 cq
    ...
    ISP_FUNC_DBG(":E cq(%d) m_pIspReg(0x%lx) m_pPhyIspReg_bak(0x%lx) m_pIspDrv(0x%lx) m_pPhyIspDrv_bak(0x%lx)\n",cq,(unsigned long)m_pIspReg,(unsigned long)m_pPhyIspReg_bak,(unsigned long)m_pIspDrv,(unsigned long)m_pPhyIspDrv_bak);
...
{
    ISP_FUNC_DBG(":E m_pIspDrv(0x%lx) m_pPhyIspDrv(0x%lx)\n",(unsigned long)m_pIspDrv,(unsigned long)m_pPhyIspDrv_bak);
    //
...
int DMAI_B::_config( void )
{
    ...
    ISP_FUNC_DBG("id(0x%lx),CQ(%d),sceID(%u)",this->id(),this->CQ,DMAI_B::sceID);
    ISP_FUNC_DBG("%s:[base_pAddr:0x%lx]/[ofst_addr:0x%lx]",this->name_Str(),(unsigned long)this->dma_cfg.memBuf.base_pAddr,(unsigned long)this->dma_cfg.memBuf.ofst_addr);
    ...
    if((this->CQ == CAM_ISP_CQ1 || this->CQ == CAM_ISP_CQ2) && DMAI_B::sceID <= 12)
    {
     ...
     switch(this->id())
     {
        case ISP_DMA_TDRI:
            ...
            ISP_FUNC_DBG("m_pIspReg(0x%lx),CAM_TDRI_BASE_ADDR = 0x%lx",(unsigned long)m_pIspReg,(unsigned long)this->dma_cfg.memBuf.base_pAddr);
            break;
        case ISP_DMA_CQI:
...
//TEST 0418               ISP_WRITE_REG(m_pPhyIspReg, CAM_IMGI_CON2, 0x00000000);// TEST_MDP by m_pPhyIspReg
               ...
               // ISP_WRITE_BITS(m_pIspReg, CAM_IMGI_RING_BUF,EN, this->dma_cfg.ring_en);
                ISP_FUNC_DBG("[ISP_BUF]:isRO(%d) m_pIspReg=[0x%lx]/IMGI_BASE_ADDR=[0x%08X] ",m_pIspDrv->IsReadOnlyMode(),(unsigned long)m_pIspReg,ISP_READ_REG(m_pIspReg, CAM_IMGI_BASE_ADDR));
            }
...
int DMAO_B::_config( void )
{
    ...
    //ISP_FUNC_DBG("[id:0x%08d] ",this->id());
    ISP_FUNC_DBG("%s:[base_pAddr:0x%lx][ofst_addr:0x%08X][crop:%d %d] ",this->name_Str(),(unsigned long)this->dma_cfg.memBuf.base_pAddr,
     ...
     switch(this->id())
     {
        case ISP_DMA_IMGO:
            ...
            ISP_FUNC_DBG("[ISP_BUF]:isRO(%d),m_pIspReg=[0x%lx]",m_pIspDrv->IsReadOnlyMode(),(unsigned long)m_pIspReg);
...
unsigned int turnEn1Tag=0x00, turnEn2Tag=0x00, turnDmaTag=0x00; //unable to touch sn
    ...
    ISP_FUNC_DBG("int_en(0x%08x),intb_en(0x%08x),intc_en(0x%08x),dma_int(0x%x),dmab_int(0x%x),dmac_int(0x%x),muxS2(0x%x)", \
                                                          ...
                                                          ctl_mux_sel2.reg_val);
    ISP_FUNC_DBG("m_pIspReg(0x%lx) m_pPhyIspReg(0x%lx) m_pIspDrv(0x%lx) m_pPhyIspDrv(0x%lx)",(unsigned long)m_pIspReg,(unsigned long)m_pPhyIspReg,(unsigned long)m_pIspDrv,(unsigned long)m_pPhyIspDrv);
    /********************************************************************************************************************/
    /********************************************************************************************************************/
    /********************************************************************************************************************/
    /*** set to physical ***/
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if( CAM_ISP_PASS1_START == (*start) )
    {
        ...
        if( 1 < size )
        {
            ...
            ISP_FUNC_DBG("ENABLE CAM_ISP_CQ0C(0x%lx)",(unsigned long)(cq0cDescBufAddr + sizeof(CQ_RING_CMD_ST)));
            if(m_pPhyIspDrv->IsReadOnlyMode())
            {
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    for ( int rt_dma =_imgi_; rt_dma < _rt_dma_max_ ;rt_dma++ )
    {
        ...
        ISP_FUNC_DBG("[before]rt_dma(%d),size:hw(%lu),sw(%lu)",rt_dma, \
                                                        (unsigned long)ISP_BUF_CTRL::m_hwbufL[rt_dma].bufInfoList.size(),
                                                        (unsigned long)ISP_BUF_CTRL::m_swbufL[rt_dma].bufInfoList.size());
        ...
        if ((_imgo_==rt_dma) || (_img2o_==rt_dma))
        {
            ...
            ISP_FUNC_INF("rtBufCtrl.ctrl(%d)/id(%d)/ptr(0x%lx)",buf_ctrl.ctrl,buf_ctrl.buf_id,(unsigned long)buf_ctrl.data_ptr);
            ...
            //
        ISP_FUNC_DBG("[after]rt_dma(%d),size:hw(%lu),sw(%lu) ",rt_dma, \
                (unsigned long)ISP_BUF_CTRL::m_hwbufL[rt_dma].bufInfoList.size(), \
                (unsigned long)ISP_BUF_CTRL::m_swbufL[rt_dma].bufInfoList.size());
    }
...
int ISP_RGB_PIPE::_config( void )
{
    ...
    ISP_FUNC_DBG("[enable1:0x%08X] m_pIspDrv(0x%lx)\n",enable1,(unsigned long)m_pIspDrv);
...
//#define _USE_OLD_CDP_DRVIER_
    ...
    if(this->conf_rotDMA)
    {
        ISP_FUNC_DBG("%s:dispo:[base_pAddr:0x%lx] ",this->name_Str(),(unsigned long)this->dispo_out.memBuf.base_pAddr);
...
MINT32 ISP_BUF_CTRL::enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo )
{
    ...
    ISP_FUNC_DBG("dma(%d),id(%d),size(0x%x),VA(0x%lx),PA(0x%lx),S/C(%d/%d)",
                rt_dma,
                buf_info.memID,
                buf_info.size,
    ...
    else
    {
        ...
        ISP_FUNC_DBG("[_test]:sm_hwbufL[rt_dma].bufInfoList.size() = %lu",(unsigned long)m_hwbufL[rt_dma].bufInfoList.size());
        if( ISP_RT_BUF_SIZE <= m_hwbufL[rt_dma].bufInfoList.size() )
        {
            ISP_FUNC_ERR("Max buffer size(0x%lu)",(unsigned long)m_hwbufL[rt_dma].bufInfoList.size());
            return -1;
        }
...
//            ISP_FUNC_DBG("Keep going first wo mapphy error");
    ...
    if(ISP_PASS1 == this->path)
    {
        ...
        ISP_FUNC_DBG("rt_buf_info.ID(%d)/size(0x%x)/vAddr(0x%lx)/pAddr(0x%x)",rt_buf_info.memID,rt_buf_info.size,(unsigned long)rt_buf_info.base_vAddr,rt_buf_info.base_pAddr);
        ...
        if( NULL != bufInfo.next )
        {
            ...
            ISP_FUNC_INF("exchange 1st buf. by 2nd buf. and enque it.ID(%d)/size(0x%x)/vAddr(0x%lx)/pAddr(0x%x)",ex_rt_buf_info.memID,ex_rt_buf_info.size,(unsigned long)ex_rt_buf_info.base_vAddr,ex_rt_buf_info.base_pAddr);
    ...
    {
        ISP_FUNC_DBG("P2 dma(%d),memID(0x%x),size(%lu),vAddr(0x%lx),pAddr(0x%lx)", \
                                    rt_dma, \
                                    m_hwbufL[rt_dma].bufInfoList.back().memID, \
                                    (unsigned long)m_hwbufL[rt_dma].bufInfoList.size(), \
                                    (unsigned long)m_hwbufL[rt_dma].bufInfoList.back().base_vAddr,
                                    (unsigned long)m_hwbufL[rt_dma].bufInfoList.back().base_pAddr);
    }
...
MINT32 ISP_BUF_CTRL::dequeueHwBuf( MUINT32 dmaChannel )
{
    ...
    if( ISP_PASS1 == this->path )
    {
        if( _imgo_ == rt_dma || _img2o_ == rt_dma )
        {
            ...
            {
                ISP_FUNC_DBG("i(%d),dma(%d),id(0x%x),size(0x%x),VA(0x%lx),PA(0x%x),size(%lu)",i,rt_dma,deque_buf.data[i].memID,deque_buf.data[i].size,(unsigned long)deque_buf.data[i].base_vAddr,deque_buf.data[i].base_pAddr,(unsigned long)m_swbufL[rt_dma].bufInfoList.size());
    ...
    else
    {
        ...
        while(count--)
        {
            ...
            ISP_FUNC_DBG(" : m_hwbufL[%d].bufInfoList.size()=(0x%lu)\n",rt_dma,(unsigned long)m_hwbufL[rt_dma].bufInfoList.size());
            ...
            if(m_hwbufL[rt_dma].bufInfoList.size() && ISP_PASS2 == this->path)
            {
                ...
                //
                ISP_FUNC_DBG("dma(%d),swsize(%lu) ",rt_dma,(unsigned long)m_swbufL[rt_dma].bufInfoList.size());
...
MINT32 ISP_BUF_CTRL::dequeueSwBuf( MUINT32 dmaChannel ,stISP_FILLED_BUF_LIST& bufList )
{
    ...
    while ( m_swbufL[rt_dma].bufInfoList.size() )
    {
        ...
        //
        ISP_FUNC_DBG("05_dma(%d)/memID(0x%x)/size(%lu)/vaddr(0x%lx)/paddr(0x%lx)",rt_dma,
                                    bufList.pBufList->back().memID, \
                                    (unsigned long)bufList.pBufList->size(), \
                                    (unsigned long)bufList.pBufList->back().base_vAddr,
                                    (unsigned long)bufList.pBufList->back().base_pAddr);
    }
    //
    return 0;
}
...
MUINT32 ISP_BUF_CTRL::getCurrHwBuf( MUINT32 dmaChannel )
{
    ...
    ISP_FUNC_DBG("dma:(%d)/memID(0x%x)/vAddr:(0x%lx)/pAddr:(0x%lx) ",rt_dma, \
                                                        m_hwbufL[rt_dma].bufInfoList.front().memID, \
                                                        (unsigned long)m_hwbufL[rt_dma].bufInfoList.front().base_vAddr, \
                                                        (unsigned long)m_hwbufL[rt_dma].bufInfoList.front().base_pAddr);
    ...
    if(m_hwbufL[rt_dma].bufInfoList.size()>1)
    {
        ...
        {   //max should be two.
             ...
             it++;
             ISP_FUNC_INF("set prv buffer while vss buffer has not beed dequed, get prv (%d)_pa(0x%lx)",rt_dma,(unsigned long)((*it).base_pAddr));
             return ((*it).base_pAddr);
        }
        else
        {
            ISP_FUNC_ERR("too many imgi buffers (%lu)",(unsigned long)m_hwbufL[rt_dma].bufInfoList.size());
            return -1;
        }
    }
    else
...
debugPrint( MUINT32 dmaChannel )
{
    ...
    for ( it = m_hwbufL[rt_dma].bufInfoList.begin(); it != m_hwbufL[rt_dma].bufInfoList.end(); it++ ) {
        ISP_FUNC_DBG("m_hwbufL[%d].base_vAddr:[0x%lx]/base_pAddr:[0x%lx] ",rt_dma,(unsigned long)it->base_vAddr,(unsigned long)it->base_pAddr);
    }
    //
