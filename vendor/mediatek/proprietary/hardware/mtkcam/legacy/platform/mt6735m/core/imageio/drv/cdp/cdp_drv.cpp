...
{
    LOG_DBG("+,pIspReg(0x%lx)", (unsigned long)pIspReg);
...
{
    LOG_DBG("+,pPhyIspDrv(0x%08lx)", (unsigned long)pPhyIspDrv);
    ...
    MUINT32 *pu4OutPlane)
{
    ...
    *pu4OutPlane = u4OutPlane;
    //
    LOG_DBG("-,Result(%d),u4OutPlane(%d)", Result, *pu4OutPlane);
    ...
    CDP_DRV_BURST_LEN_ENUM  BurstLen)
{
    ...
    //Mark this if case, because ENUM BurstLen never bigger than CDP_DRV_MASK_MAX_BURST_LEN(0x1F)
    //if(BurstLen > CDP_DRV_MASK_MAX_BURST_LEN)
    //{
    //    LOG_ERR("BurstLen(%d) is out of %d",BurstLen,CDP_DRV_MASK_MAX_BURST_LEN);
    //    return MFALSE;
...
)
{
    ...
    LOG_DBG("cqPhy(0x%lx),Vido(0x%x),VidCo(0x%x),VidVo(0x%x)",
            (unsigned long)mpIspReg, PhyAddr, PhyAddrC, PhyAddrV);     // added for debug
    ...
    MUINT32     StrideV)
{
    ...
    LOG_DBG("cqPhy(0x%lx),Dispo(0x%x),DispCo(0x%x),DispVo(0x%x)",
            (unsigned long)mpIspReg, PhyAddr, PhyAddrC, PhyAddrV);     // added for debug
