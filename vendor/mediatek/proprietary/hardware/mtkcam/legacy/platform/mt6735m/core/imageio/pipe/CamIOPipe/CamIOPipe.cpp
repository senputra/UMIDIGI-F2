...
MBOOL CamIOPipe::init()
{
    ...
    if( m_pIspDrvShell )
    {
        ...
        PIPE_INF("reset TOP registers(0x%016llx)",(unsigned long long)m_pIspDrvShell->getPhyIspReg());
        //
...
enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo)
{
    ...
    PIPE_DBG("bufInfo,(%d),(0x%08x),(0x%08x),(0x%08x),(%d/%d)",
            bufInfo.memID,
            bufInfo.size,
            (MUINT32)bufInfo.base_vAddr,
            (MUINT32)bufInfo.base_pAddr,
            bufInfo.bufSecu,
            bufInfo.bufCohe);

    //buffer wanna exchange. to replace original buffer.
    if ( 2 <= rQBufInfo.vBufInfo.size() ) {
        PIPE_WRN("exchange 1st buf. by 2nd buf. and enque it.:size(%d)",(MINT32)rQBufInfo.vBufInfo.size());
        ...
        PIPE_DBG("exbufInfo,(%d),(0x%08x),(0x%08x),(0x%08x),(%d/%d)",
                exbufInfo.memID,
                exbufInfo.size,
                (MUINT32)exbufInfo.base_vAddr,
                (MUINT32)exbufInfo.base_pAddr,
                exbufInfo.bufSecu,
                exbufInfo.bufCohe);
    }
    //
...
dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo, MUINT32 const u4TimeoutMs /*= 0xFFFFFFFF*/)
{
    ...
    PIPE_DBG("dma:[0x%x]/size:[0x%x]/addr:[0x%x] ",dmaChannel,
                                                (MUINT32)bufList.size(),
                                                (MUINT32)bufList.front().base_vAddr);
    ...
    for ( MINT32 i = 0; i < (MINT32)rQBufInfo.vBufInfo.size() ; i++) {
        ...
        PIPE_DBG("dma:[0x%x]/size:[0x%x]/addr:[0x%x]/buf size:[%d]/memid:[%d]",   dmaChannel,
                                                                                (MUINT32)rQBufInfo.vBufInfo.size(),
                                                                                (MUINT32)rQBufInfo.vBufInfo[i].u4BufVA,
...
MBOOL CamIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts)
{
    ...
    if ( CAM_ISP_CQ_NONE != pass1_CQ) {
        if ( CAM_ISP_CQ0  == pass1_CQ ) { pass1_cq_en = (MINT32)CAM_CTL_EN2_CQ0_EN;}
    ...
    {
        PIPE_DBG("reset TOP for NCC(0x%016llx)",(unsigned long long)m_pIspDrvShell->getPhyIspReg());
    ...
    PIPE_DBG("in[%d]/out[%d]", (MINT32)vInPorts.size(), (MINT32)vOutPorts.size());
