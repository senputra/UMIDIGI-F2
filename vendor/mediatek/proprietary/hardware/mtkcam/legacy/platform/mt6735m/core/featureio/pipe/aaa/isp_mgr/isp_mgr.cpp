...
apply(EIspProfile_T eIspProfile)
{
    ...
    if(!bBPC_EN){
        bCT_EN = MFALSE;
        MY_LOG_IF(1," BPC is off, NR1 must be off");
    }
...
reset()
{
    MY_LOG_IF(m_fgLogEn,"[%s]", __FUNCTION__);
    return MTRUE;
}
...
apply(EIspProfile_T eIspProfile)
{
    ...
    if (m_rIspDrvMode[eIspProfile] & ISPDRV_MODE_CQ0) { // PASS 1
        ...
        MY_LOG_IF(m_fgLogEn,"[%s] ISPDRV_MODE_CQ0 addr 0x%0lx (LSC_EN, LSCI_EN) = (%d, %d)", __FUNCTION__,
                (unsigned long)getIspReg(drv_mode),
...
#endif
        MY_LOG_IF(m_fgLogEn, "%s ISPDRV_MODE_CQ0 \n"
    ...
    if (m_rIspDrvMode[eIspProfile] & ISPDRV_MODE_CQ1_SYNC) { // PASS 2 (CQ1)
        ...
        MY_LOG_IF(m_fgLogEn,"%s ISPDRV_MODE_CQ1 addr 0x%0lx (LSC_EN, LSCI_EN) = \n"
                "(%d, %d)\n", __FUNCTION__,
                (unsigned long)getIspReg(drv_mode),
                ISP_READ_BITS(getIspReg(drv_mode), CAM_CTL_EN1_SET, LSC_EN_SET),
                ISP_READ_BITS(getIspReg(drv_mode), CAM_CTL_DMA_EN_SET, LSCI_EN_SET));

        MY_LOG_IF(m_fgLogEn, "%s ISP_DRV_CQ1_SYNC \n"
            "CAM_LSCI_BASE_ADDR = 0x%08x\n"
            "CAM_LSCI_XSIZE     = 0x%08x\n"
            "CAM_LSCI_YSIZE     = 0x%08x\n"
            "CAM_LSCI_STRIDE    = 0x%08x\n"
    ...
    if (m_rIspDrvMode[eIspProfile] & ISPDRV_MODE_CQ2_SYNC) { // PASS 2 (CQ2)
        ...
        MY_LOG_IF(m_fgLogEn, "%s ISP_DRV_CQ2_SYNC \n"
            "CAM_LSCI_BASE_ADDR = 0x%08x\n"
            "CAM_LSCI_XSIZE     = 0x%08x\n"
            "CAM_LSCI_YSIZE     = 0x%08x\n"
            "CAM_LSCI_STRIDE    = 0x%08x\n"
...
{
    MY_LOG_IF(m_fgLogEn ,"[%s] %d", __FUNCTION__, enable);
    m_fgOnOff = enable;
}
...
setIspPregain()
{
    ...
    MY_LOG_IF(m_fgLogEn, "m_i4FlareGain = %d\n", m_i4FlareGain);
    MY_LOG_IF(m_fgLogEn, "m_rIspPregain.i4R = %d; m_rIspAWBGain.i4R = %d\n", m_rIspPregain.i4R , m_rIspAWBGain.i4R);
    MY_LOG_IF(m_fgLogEn, "m_rIspPregain.i4G = %d; m_rIspAWBGain.i4G = %d\n", m_rIspPregain.i4G , m_rIspAWBGain.i4G);
    MY_LOG_IF(m_fgLogEn, "m_rIspPregain.i4B = %d; m_rIspAWBGain.i4B = %d\n", m_rIspPregain.i4B , m_rIspAWBGain.i4B);

    // ISP flare offset
    m_i4IspFlareOffset = (m_i4FlareOffset * m_i4FlareGain + (AWB_SCALE_UNIT >> 1)) / AWB_SCALE_UNIT;
    MY_LOG_IF(m_fgLogEn, "m_i4IspFlareOffset = %d; m_i4FlareOffset = %d\n", m_i4IspFlareOffset , m_i4FlareOffset);
...
apply(EIspProfile_T eIspProfile)
{
    ...
    if(eIspProfile == EIspProfile_ZsdPreview_NCC)
    {
        reinterpret_cast<ISP_CAM_ANR_CON1_T*>(REG_INFO_VALUE_PTR(CAM_ANR_CON1))->ANR_MODE = 1;
    }
    else
    {
        reinterpret_cast<ISP_CAM_ANR_CON1_T*>(REG_INFO_VALUE_PTR(CAM_ANR_CON1))->ANR_FLT_MODE = 0;
        reinterpret_cast<ISP_CAM_ANR_CON1_T*>(REG_INFO_VALUE_PTR(CAM_ANR_CON1))->ANR_MODE = 0;
    }
