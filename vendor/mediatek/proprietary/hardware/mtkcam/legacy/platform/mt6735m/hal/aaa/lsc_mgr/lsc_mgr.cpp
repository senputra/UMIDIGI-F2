...
importEEPromData(void)
{
    ...
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    MINT32 dbg_1to3 = 0;
    property_get("vendor.debug.lsc_mgr.1to3", value, "-1");
    dbg_1to3 = atoi(value);
...
getCTIdx() {
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    MINT32 dbg_ct = 0;
    property_get("vendor.debug.lsc_mgr.ct", value, "-1");
    dbg_ct = atoi(value);
...
SetTBAToISP()     //  ISP <- PA
{
...

    {
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        MINT32 dbg_tbl = 0;
        property_get("vendor.debug.lsc_mgr.dumptbl", value, "-1");
        dbg_tbl = atoi(value);
...
LscMgr::UpdateSL2Param(void)
{
    ...
    MINT32 i4SL2En = 0;
    GET_PROP("vendor.debug.lsc_mgr.sl2", "-1", i4SL2En);
...
enableLsc(MBOOL const fgEnable)
{
    ...
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    MINT32 dbg_enable = 0;
    property_get("vendor.debug.lsc_mgr.enable", value, "-1");
    dbg_enable = atoi(value);

    GET_PROP("vendor.debug.lsc_mgr.log", "0", _bEnableMyLog);
...
fillTSFInitParams(MTK_TSF_ENV_INFO_STRUCT &params)
{
    ...
    MINT32 i4TsfSL2En = 0;
    GET_PROP("vendor.debug.lsc_mgr.tsfsl2", "-1", i4TsfSL2En);
...
isTSFEnable(void)
{
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    MINT32 manual_tsf = 0;
    property_get("vendor.debug.lsc_mgr.manual_tsf", value, "-1");
...
mThreadLoop(void *arg)
{
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    MINT32 dbg_tsf = 0;
    property_get("vendor.debug.lsc_mgr.dbg_tsf", value, "0");
    dbg_tsf = atoi(value);
...
//                    }
                ...
                {  // normal preview case
                    ...
                    {
                        MINT32 i4Batch = 1;
                        //GET_PROP("vendor.debug.lsc_mgr.tsf_pv_batch", "0", i4Batch);
...
updateTSFinput(LSCMGR_TSF_INPUT_SRC src, TSF_REF_INFO_T *ref, MVOID *stat)
{
    ...
    GET_PROP("vendor.debug.lsc_mgr.tsfdbg0", "0", u4Dbg0);
    GET_PROP("vendor.debug.lsc_mgr.tsfdbg1", "0", u4Dbg1);
