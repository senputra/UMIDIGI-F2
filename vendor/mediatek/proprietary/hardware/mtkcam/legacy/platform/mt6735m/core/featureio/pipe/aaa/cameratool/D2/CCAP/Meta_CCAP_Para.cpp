...
static       unsigned int       g_set_isp_para_buffer_size  = 0;
...
bool FT_ACDK_CCT_V2_OP_ISP_SET_TUNING_PARAS(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
    ...
    ACDK_LOGD("[%s] offset(%d), length(%d), totalLength(%d), isDone(%d), index(%d), ctgy(%d), g_set_isp_para_buffer(%p)",
        __FUNCTION__, offset, length, totalLength, isDone, index, ctgy, g_set_isp_para_buffer);
    ...
    if (!g_set_isp_para_buffer)
    {
        ...
        g_set_isp_para_buffer_size = totalLength;
        memset(g_set_isp_para_buffer, 0, totalLength);
        g_set_isp_para_module = ctgy; //set current module in use
    }
    else if( totalLength > g_set_isp_para_buffer_size)
    {
        ACDK_LOGE("[%s] totalLength is largetr than g_set_isp_para_buffer_size (%d, %d)", __FUNCTION__, totalLength, g_set_isp_para_buffer_size);
        pCNF->status = FT_CNF_FAIL;
        return false;
    }
    ...
    unsigned long long int cmd_length = (unsigned long long int)offset + (unsigned long long int)length;
    if (!(((cmd_length == (unsigned long long int)totalLength) && isDone) || ((cmd_length < (unsigned long long int)totalLength) && !isDone)))
    {
...
void META_CCAP_ADB_OP()
{
...
#if 1
    const char input_filename[] = "/mnt/vendor/cct/CCTinput";
    const char output_filename[] = "/mnt/vendor/cct/CCToutput";
#else
    ...
    ACDK_LOGD("[%s] file size(%I64u), sizeof(FT_CCT_CMD)=(%zu)",  __FUNCTION__, file_size, sizeof(FT_CCT_CMD));

    if( file_size > sizeof(FT_CCT_CMD) )
    {
        ACDK_LOGD("[%s] inBuf size(%I64u)",  __FUNCTION__, file_size - sizeof(FT_CCT_CMD));
    ...
    if (pFp == 0)
    {
        ACDK_LOGE("[%s] fail to open CCToutput. Error Code(%d), path=%s",  __FUNCTION__, pFp, output_filename);
        goto EXIT;
    }
#if 0
...
#if 1
    // create end file
    pFp = fopen("/mnt/vendor/cct/CCTend", "wb");
    fclose(pFp);
#endif
