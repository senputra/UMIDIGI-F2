...
public:
    ...
    void                reset()
    {
        MY_LOGD("mpSensorHal=%p",mpSensorHal);
        if (mpSensorHal)
        {
    ...
    , miLogLevel(0)
{
    ...
    ::property_get("vendor.debug.camera.prvcq.loglevel", cLogLevel, "0");
...
{
    MY_LOGD("this=%p, sizeof:%d", this, (int)sizeof(PreviewCmdQueThread));
}
...
postCommand(PrvCmdCookie::ECmdType const cmdType, PrvCmdCookie::ESemWait const semWait)
{
    ...
    //
    {
        ...
        //
        MY_LOGD("+ tid(%d), que size(%d)", ::gettid(), (int)mCmdCookieQ.size());
        ...
        mCmdCond.broadcast();
        MY_LOGD("- new command added(%d):  tid(%d), que size(%d)", cmdType, ::gettid(), (int)mCmdCookieQ.size());
    }
    //
    cmdCookie->waitSem();
...
getCommand(sp<PrvCmdCookie> &rCmdCookie)
{
    ...
    //
    MY_LOGD("- tid(%d), que size(%d), ret(%d)", ::gettid(), (int)mCmdCookieQ.size(), ret);
    //
    FUNCTION_OUT;
    //
    return ret;
...
getZoomValue()
{
    ...
    if( ! mvZoomIdx.empty() )
    {
        MY_LOGD_IF(miLogLevel, "Size(%d)", (int)mvZoomIdx.size());
...
updateOne()
{
    ...
    if( vDeBufPass2Out.size() > 1 )
    {
        ...
        "DISP(%f),VIDO(%f)", (double)vDeBufPass2Out.at(0).bufInfo.getTimeStamp_ns(), (double)vDeBufPass2Out.at(1).bufInfo.getTimeStamp_ns());
    }
    //
    //
lbExit:
    ...
    if(miLogLevel||(passTime/1000000)>1000)
    {
        MY_LOGI("Time(%3lld/%3lld/%3lld/%3lld)ms",
            (long long)passTime/1000000,
            (long long)pass1Time/1000000,
            (long long)pass2Time/1000000,
            (long long)vssTime/1000000);
    }
    //
    return ret;
}
...
checkDumpPass1(void)
{
    ...
    property_get("vendor.camera.dumpbuffer.enable", value, "0");
    ...
    uint32_t filenum)
{
    for (MUINT32 i = 0; i < src.size(); i++)
    {
        ...
        MY_LOGD("addr: 0x%x, size: %d, time: %f",
           (unsigned int)src.at(i).bufInfo.vBufInfo.at(0).u4BufVA,
           src.at(i).bufInfo.vBufInfo.at(0).u4BufSize,
           (double)src.at(i).bufInfo.getTimeStamp_ns());
...
dumpImg(
    ...
    uint32_t filenum)
{
    const char* filePath = "/sdcard/camera_dump/MtkDefault/";
    char fileName[100];
