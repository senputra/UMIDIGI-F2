...
HdrShot(char const*const pszShotName, uint32_t const u4ShotMode, int32_t const i4OpenId)
  ...
  , mu4W_dsmap(0)
  , mu4H_dsmap(0)
  , mNormalJpegThread(0) //102
  , mCaptureIMemThread(0) //107
  , mProcessIMemThread(0) //108
  //
  , mpHdrHal(NULL) //111
  //
  , mpIMemDrv(NULL) //115
  //
  , mTotalBufferSize(0) //116
  , mTotalKernelBufferSize(0) //117
  , mTotalUserBufferSize(0) //118
  , OriWeight(NULL) //173
  , BlurredWeight(NULL) //174
  //
  , mu4OutputFrameNum(0) //182
  , mu4TargetTone(0) //185
  //, mu4RunningNumber(0)
  , mfgIsForceBreak(MFALSE) //190
  , mHdrState(HDR_STATE_INIT) //192
  , mHdrRound(1) //194
  , mHdrRoundTotal(2) //195
  , mShutterCBDone(0) //197
  , mRawCBDone(0) //198
  , mJpegCBDone(0) //199
  , mCaptueIndex(0) //204
  , mSensorType(0) //205
  , mTestMode(0)
{
    ...
    for(MUINT32 i=0; i<eMaxOutputFrameNum; i++) {
        mpSourceImgBuf[i].virtAddr = 0;
        mpFirstRunSourceImgBuf[i].virtAddr = 0;
        mpSmallImgBuf[i].virtAddr = 0;
        mpSEImgBuf[i].virtAddr = 0;
        mWeightingBuf[i].virtAddr = 0;
        mpBlurredWeightMapBuf[i].virtAddr = 0;
        mpDownSizedWeightMapBuf[i].virtAddr = 0;
    }
    mpPostviewImgBuf.virtAddr = 0;
    mpResultImgBuf.virtAddr = 0;
    mpHdrWorkingBuf.virtAddr = 0;
    mpMavWorkingBuf.virtAddr = 0;
    mRawBuf.virtAddr = 0;
    mNormalJpegBuf.virtAddr = 0;
    mNormalThumbnailJpegBuf.virtAddr = 0;
    mHdrJpegBuf.virtAddr = 0;
    mHdrThumbnailJpegBuf.virtAddr = 0;
    mBlendingBuf.virtAddr = 0;
