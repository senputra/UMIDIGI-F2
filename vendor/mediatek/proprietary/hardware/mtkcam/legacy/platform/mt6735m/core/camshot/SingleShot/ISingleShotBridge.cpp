...
public:     ////    Settings.
    ...
    virtual MVOID   setSwNR(MBOOL enableSwNR);
    virtual MVOID   setSwNR(MBOOL enableSwNR, MUINT32 iso);
...
MVOID
ISingleShotBridge::
setSwNR(MBOOL enableSwNR)
{
    Mutex::Autolock _lock(mLock);
    return  getImp()->setSwNR(enableSwNR);
}
...
MVOID
ISingleShotBridge::
setSwNR(MBOOL enableSwNR, MUINT32 iso)
{
    Mutex::Autolock _lock(mLock);
    return  getImp()->setSwNR(enableSwNR, iso);
}
