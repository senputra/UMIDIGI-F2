...
class SwNRParam
{
    ...
    public:
        ...
        MVOID      setParam(
                        MUINT32 const iso,
                        MUINT32 const scale_x100 = 100,
                        MINT32 const perfLevel = -1);

        MBOOL      getTuningNR(NRTuningInfo* pTuning);
        MBOOL      getTuningNR2(NR2TuningInfo* pTuning);
        MBOOL      getTuningHFG(HFGTuningInfo* pTuning);	
        MBOOL      getTuningCCR(CCRTuningInfo* pTuning);
        MINT32     getPerfLevel() const { return miPerfLevel; }
        MINT32     getISO() const { return mIso; }		
...
class SwNR
{
    ...
    public:
        ...
        MBOOL doSwNR(NSCam::IImageBuffer* pBuf);
