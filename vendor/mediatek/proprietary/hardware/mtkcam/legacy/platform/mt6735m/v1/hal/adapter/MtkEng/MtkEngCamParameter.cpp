...
#define UPDATE_PARAMS(param, eMapXXX, key) \
    ...
    //
    {
        ...
        if (1 == save_shading)
        {
            ...
            LscMgr2::getInstance(eSensorDevID)->CCTOPSetSdblkFileCfg(MTRUE, ms8RawFilePath.string()); // LscMgr::getInstance(eSensorDevID)->CCTOPSetSdblkFileCfg(MBOOL fgSave, const char* filename)
        }
        else
        {
            LscMgr2::getInstance(eSensorDevID)->CCTOPSetSdblkFileCfg(MFALSE, "");
        }
    }
    //
    //p3AHal->send3ACtrl(NS3A::E3ACtrl_SetOperMode,NSIspTuning::EOperMode_EM, 0);
    //
    if (NSIspTuning::EOperMode_Meta != p3AHal->send3ACtrl(NS3A::E3ACtrl_GetOperMode, 0, 0))
    {
        /*
        ...
        */
        ...
        if (MtkCameraParameters::KEY_ENG_SHADING_TABLE_TSF == shading_table)
        {
                ...
                LscMgr2::getInstance(sensor)->setTsfOnOff(MTRUE);
        }
        else
        {
                ESensorDev_T sensor = static_cast<ESensorDev_T>(DevMetaInfo::queryHalSensorDev(getOpenId()));
                LscMgr2::getInstance(sensor)->setTsfOnOff(MFALSE);
        }

        ESensorDev_T sensor = static_cast<ESensorDev_T>(DevMetaInfo::queryHalSensorDev(getOpenId()));
        if (MtkCameraParameters::KEY_ENG_SHADING_TABLE_AUTO == shading_table)
        {
            IspTuningMgr::getInstance().enableDynamicShading(sensor, MTRUE);
        }
        else if (MtkCameraParameters::KEY_ENG_SHADING_TABLE_LOW == shading_table)
        {
            IspTuningMgr:: getInstance().enableDynamicShading(sensor, MFALSE);
            IspTuningMgr:: getInstance().setIndex_Shading(sensor, 0);
        }
        else if (MtkCameraParameters::KEY_ENG_SHADING_TABLE_MIDDLE == shading_table)
        {
            IspTuningMgr:: getInstance().enableDynamicShading(sensor, MFALSE);
            IspTuningMgr:: getInstance().setIndex_Shading(sensor, 1);
        }
        else if (MtkCameraParameters::KEY_ENG_SHADING_TABLE_HIGH == shading_table)
        {
            IspTuningMgr:: getInstance().enableDynamicShading(sensor, MFALSE);
            IspTuningMgr:: getInstance().setIndex_Shading(sensor, 2);
        }
    }
    //
