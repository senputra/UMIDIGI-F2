...
// don't turn on DEBUG_DUMP and vendor.mediatek.hdr.debug at the same time
...
{
    MY_LOGD_IF(streamInfo.get(), "%s: streamId(%08" PRIx64 ") %s",
    ...
    , mp3A(NULL)
{
    ...
    // log level
    property_get("vendor.debug.camera.log", value, "0");
    mLogLevel = atoi(value);
    if (mLogLevel == 0)
    {
        property_get("vendor.debug.camera.log.HDRNode", value, "0");
        mLogLevel = atoi(value);
    }

    // debug dump
    property_get("vendor.debug.camera.dumphdr", value, DEBUG_DUMP);
    ...
    // image
    {
        ...
        for (size_t i = 0; i < mapSet.size(); i++)
        {
            ...
            // get input image buffer
            {
                ...
                MY_LOGD("    IN  streamId(%08" PRIx64 ") name(%s)", in->streamId, streamName);
            ...
            for (size_t j = 0; j < mapSet[i].vOut.size(); j++)
            {
                ...
                MY_LOGD("    OUT streamId(%08" PRIx64 ") name(%s)", out->streamId, streamName);
    ...
    // metadata
    {
        ...
        for (size_t i = 0; i < mapSet.size(); i++)
        {
            ...
            for (size_t j = 0; j < mapSet[i].vIn.size(); j++)
            {
                ...
                MY_LOGD("    IN  streamId(%08" PRIx64 ") name(%s)", streamId, streamName);
            ...
            for (size_t j = 0; j < mapSet[i].vOut.size(); j++)
            {
                ...
                const char* streamName = mapSet[i].vOut[j]->getStreamName();
                MY_LOGD("    OUT streamId(%08" PRIx64 ") name(%s)", streamId, streamName);
...
MBOOL HDRNodeImp::isInImageStream(StreamId_T const streamId) const
{
    ...
    MY_LOGD_IF(0, "streamId(%08" PRIx64 ") is not in-stream", streamId);
    return MFALSE;
}
    ...
    sp<IImageBuffer>& imageBuffer)
{
    ...
    MY_LOGD_IF(mLogLevel >= 1, "    streamId(%08" PRIx64 ") buffer(%p) usage(0x%x) type(0x%x)",
    ...
    IMetadata*& metadata)
{
    ...
    MY_LOGD_IF(mLogLevel >= 1, "      streamId(%08" PRIx64 ") metadata(%p) type(0x%x)",
