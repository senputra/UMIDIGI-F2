...
MINT32 AcdkMhalPure::acdkMhalInit()
{
    ...
    if ((r = pthread_attr_init(&attr)) != 0) {
        ACDK_LOGD("thread ctrl [%s] fail: %d", "set attr", r);
        return MFALSE;
    }
    if ((r = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED)) != 0) {
        ACDK_LOGD("thread ctrl [%s] fail: %d", "enqThd detach", r);
        return MFALSE;
    }
...
MINT32 AcdkMhalPure::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    {
        ACDK_LOGD("imgImemBuf  : virAddr[%d]=0x%x",i,(int)mAcdkMhalPrvParam.imgImemBuf[i].virtAddr);
        ACDK_LOGD("dispImemBuf : virAddr[%d]=0x%x",i,(int)mAcdkMhalPrvParam.dispImemBuf[i].virtAddr);
    }
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
        ...
        //RL modify
        ACDK_LOGD("m_pICamIOPipe->enqueOutBuf[%d].u4BufPA(0x%x)",i,(int)rQBufInfo.vBufInfo[0].u4BufVA);
    ...
    while((acdkMhalGetState() & ACDK_MHAL_PREVIEW_MASK) || (ACDK_MHAL_PRE_CAPTURE == acdkMhalGetState()))
    {
        ...
        else
        {
            ...
            for(MUINT32 i = 0; i < imgo_num; ++i)
            {
                ...
                ACDK_LOGD_DYN(g_acdkMhalPureDebug,"imgo_virAddr[%d](0x%x),imgo_phyAddr[%d](0x%x)", i, (int)imgo_virAddr[i], i, (int)imgo_phyAddr[i]);
            }
        }
        ...
        if(dumpEnable == 1 || dumpEnable == 3)
        {
            ...
            ACDK_LOGD("prv VA(0x%x)", (int)imgo_virAddr[0]);
        ...
        else
        {
            ...
            ACDK_LOGD_DYN(g_acdkMhalPureDebug,"dispo_virAddr(0x%x),dispo_phyAddr(0x%x)", (int)dispo_virAddr, (int)dispo_phyAddr);

            if (delayframecnt >= mu4SensorDelay)
            {
               acdkMhalCBHandle(ACDK_CB_PREVIEW, (MUINT8*)dispo_virAddr, (MUINT8*)imgo_virAddr[0]);
               ACDK_LOGD("imgoVa(0x%x)", (int)imgo_virAddr[0]);
            }
            else
            {
               delayframecnt++;
            ...
            ACDK_LOGD_DYN(g_acdkMhalPureDebug,"prvCB back, enque dispo_virAddr(0x%x)", (int)dispo_virAddr);
        ...
        for(MUINT32 i = 0; i < imgo_num; ++i)
        {
            ACDK_LOGD_DYN(g_acdkMhalPureDebug,"pass1 enque imgo_virAddr[%d](0x%x)", i, (int)imgo_virAddr[i]);
            mPortID.index = EPortIndex_IMGO;
