...
Hal3AFlowCtrl::clearCommandQ()
{
    ...
    for (List<CmdQ_T>::iterator it = mCmdQ.begin(); it != mCmdQ.end();)
    {
        ...
        {
            MY_LOG("[%s] cmd size(%d), clear(%d)", __FUNCTION__, (int)mCmdQ.size(), eCmd);
            it = mCmdQ.erase(it);
        }
        else //include 2 cases; 1). keep all cmds except for ECmd_Update, 2). keep at least 1 ECmd_Update in mCmdQ
        {
            MY_LOG("[%s] Command not killed: cmd size(%d), beginning cmd(%d)", __FUNCTION__, (int)mCmdQ.size(), eCmd);
            it++;
        }
    }
...
Hal3AFlowCtrl::addCommandQ(ECmd_T const &r3ACmd, MINTPTR i4Arg)
{
    ...
    MY_LOG_IF(m_3ALogEnable, "[%s] mCmdQ size(%d) + cmd(%d) magic(%d)", __FUNCTION__, (int)mCmdQ.size(), r3ACmd, rParam.i4MagicNum);
}
...
Hal3AFlowCtrl::getCommand(CmdQ_T &rCmd, MBOOL &bGetCmd, MBOOL en_timeout)
{
    ...
    Mutex::Autolock autoLock(mModuleMtx);
    MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] mCmdQ.size()=%d, en_timeout(%d)", __FUNCTION__, (int)mCmdQ.size(), en_timeout);
    ...
    rCmd = *mCmdQ.begin();
    MY_LOG_IF(m_3ALogEnable, "CmdQ-size(%d), todo(%d, %d)", (int)mCmdQ.size(), rCmd.eCmd, rCmd.rParamIspProfile.i4MagicNum);
