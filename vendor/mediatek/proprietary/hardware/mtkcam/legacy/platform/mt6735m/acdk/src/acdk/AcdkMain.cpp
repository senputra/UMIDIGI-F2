...
#define MEDIA_PATH_CCT "/mnt/vendor/cct"
...
MINT32 AcdkMain::sensorInit()
{
    ...
    if(mSensorDev == SENSOR_DEV_NONE)
    {
      if(mSupportedSensorDev & SENSOR_DEV_MAIN)
		...
		{//Set Sub as default if no main sensor
...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    ACDK_LOGD("Main pHalSensor = 0x%p\n",m_pSensorHalObj);
    ...
    ACDK_LOGD("capVA = 0x%x, dispPA = 0x%x", (int)mCapIMemInfo.virtAddr, (int)mCapIMemInfo.phyAddr);
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
        ACDK_LOGD("prvVA[%d]  = 0x%x, prvPA[%d]  = %x", i, (int)mPrvIMemInfo[i].virtAddr, i, (int)mPrvIMemInfo[i].phyAddr);
        ACDK_LOGD("rrzoVA[%d] = 0x%x, rrzoPA[%d]  = %x", i, (int)mPrvIMemInfoRRZO[i].virtAddr, i, (int)mPrvIMemInfoRRZO[i].phyAddr);
        ACDK_LOGD("dispVA[%d] = 0x%x, dispPA[%d] = %x", i, (int)mDispIMemInfo[i].virtAddr, i, (int)mDispIMemInfo[i].phyAddr);
    }

    for(MINT32 i = 0; i < SURFACE_NUM; ++i)
    {
        ACDK_LOGD("surfaceVA[%d] = 0x%x, surfacePA[%d] = 0x%x", i, (int)mSurfaceIMemInfo[i].virtAddr, i, (int)mSurfaceIMemInfo[i].phyAddr);
    }
...
//#endif
    ...
    ACDK_LOGD("TgInfo[0] = %d, pixelMode[0] = %d\n", sensorDynamicInfo[0].TgInfo, sensorDynamicInfo[0].pixelMode);
    ...
    if(1)
    {
        ...
        setDimension(mode,acdkCapInfo.RAWImgBufInfo.imgWidth,acdkCapInfo.RAWImgBufInfo.imgHeight);
            ...
            if(dumpEnable == 4)
            {
                ...
                MINT32 cnt = 0;
                if(mSensorType == SENSOR_TYPE_YUV) //YUV
                    sprintf(szFileName, "%s/acdkCapPureRaw.yuv", mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
                else
                    sprintf(szFileName, "%s/acdkCapPureRaw.raw", mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
...
MINT32 AcdkMain::startPreview_CameraDev(Func_CB prvCb,  MUINT32 const mode)
{
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
         ACDK_LOGD("dispVA[%d] = %x, dispPA[%d] = %x, dispmemID[%d] = %x", i, (int)mDispIMemInfo[i].virtAddr, i , (int)mDispIMemInfo[i].phyAddr, i, (int)mDispIMemInfo[i].memID);
    }
    ...
    for(MINT32 i = 0; i < SURFACE_NUM; ++i)
    {
        ACDK_LOGD("surfaceVA[%d] = %x, surfacePA[%d] = %x",i,(int)mSurfaceIMemInfo[i].virtAddr,i,(int)mSurfaceIMemInfo[i].phyAddr);
    }
    ...
    if(capCb != NULL)
    {
        ...
        if(mCapType & 0x3C)
        {
            ...
            if((mCapType & PURE_RAW8_TYPE) || (mCapType & PURE_RAW10_TYPE))
            {
                ...
                sprintf(szFileName_cct, "%s/pure__%dx%d_10_%d", MEDIA_PATH_CCT,
                    acdkCapInfo.RAWImgBufInfo.imgWidth,acdkCapInfo.RAWImgBufInfo.imgHeight,sensorStaticInfo[0].sensorFormatOrder);
            }
            else
            {
                sprintf(szFileName, "%s/proc__%dx%d_10_%d", MEDIA_PATH,
                    acdkCapInfo.RAWImgBufInfo.imgWidth,acdkCapInfo.RAWImgBufInfo.imgHeight,sensorStaticInfo[0].sensorFormatOrder);
                sprintf(szFileName_cct, "%s/proc__%dx%d_10_%d", MEDIA_PATH_CCT,
                    acdkCapInfo.RAWImgBufInfo.imgWidth,acdkCapInfo.RAWImgBufInfo.imgHeight,sensorStaticInfo[0].sensorFormatOrder);
            }
            ...
            if(NULL == pFp )
            {
                ACDK_LOGE("Can't open file to read image");
                //fclose(pFp);
                return ACDK_RETURN_NULL_OBJ;
            }
            ...
            if(mCctOn)
            {
                ACDK_LOGD("Copy Raw from /sdcard to /data/cct for CCT: %s", szFileName_cct);
                FILE *pFp_cct = fopen(szFileName_cct, "wb");
                ...
                i4WriteCnt = fwrite( (void *)mCapIMemInfo.virtAddr, 1, captureSize, pFp_cct );

                fflush(pFp_cct);

                if(0 != fsync(fileno(pFp_cct)))
                {
                    ACDK_LOGE("fync fail");
                    fclose(pFp_cct);
                    return ACDK_RETURN_API_FAIL;
...
MINT32 AcdkMain::quickViewImg(MUINT32 qvFormat)
{
    ...
    //if(dumpEnable == 5)
    {
        MINT32 i4WriteCnt = 0;
        char szFileName[256];
        sprintf(szFileName, "%s/acdkQV1.yuv" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
    ...
    if(dumpEnable == 5)
    {
        MINT32 i4WriteCnt = 0;
        char szFileName[256];
        sprintf(szFileName, "%s/acdkQV2.rgb" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
...
MINT32 AcdkMain::handlePreviewCB(MVOID *param)
{
...
#if ACDK_USE_SENSORHAL == 1
    ACDK_LOGD_DYN(g_acdkMainDebug,"index(%u),VA(0x%lx),PA(0x%lx)",mSurfaceIndex,mSurfaceIMemInfo[mSurfaceIndex].virtAddr,mSurfaceIMemInfo[mSurfaceIndex].phyAddr);
    ACDK_LOGD_DYN(g_acdkMainDebug,"mOrientation(%u)",mOrientation);
#else
    ACDK_LOGD_DYN(g_acdkMainDebug,"index(%u),VA(0x%x),PA(0x%x),Size(0x%x)",mSurfaceIndex,(int)mSurfaceIMemInfo[mSurfaceIndex].virtAddr,(int)mSurfaceIMemInfo[mSurfaceIndex].phyAddr,mSurfaceIMemInfo[mSurfaceIndex].size);
    ACDK_LOGD_DYN(g_acdkMainDebug,"mOrientation(%u)",mOrientation);
    ...
    if(dumpEnable == 2 || dumpEnable == 3)
    {
        ...
        sprintf(szFileName, "%s/acdkPrv2.rgb", mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
    ...
    else
    {
        ...
        ACDK_LOGD("procQvMemInfo : vir(%x),phy(%x)",(int)ISrcMemInfo.virtAddr,(int)ISrcMemInfo.phyAddr);
        ...
        ACDK_LOGD("Dst VA(%x), Src VA(%p) size(%x)", (int)mSurfaceIMemInfo[mSurfaceIndex].virtAddr, pVirBufin, displaySize);
    }
...
MINT32 AcdkMain::handleRAWCB(MVOID *param)
{
    ...
    else
    {
        ...
        ACDK_LOGD("mRawIMemInfo : vir(%x),phy(%x),memID(%d)",(int)mRawIMemInfo.virtAddr,(int)mRawIMemInfo.phyAddr,mRawIMemInfo.memID);
    }
...
MINT32 AcdkMain::handleJPEGCB(MVOID *param)
{
    ...
    else
    {
        ...
        ACDK_LOGD("mJpgIMemInfo : vir(0x%x),phy(0x%x),memID(%d)",(int)mJpgIMemInfo.virtAddr,(int)mJpgIMemInfo.phyAddr,mJpgIMemInfo.memID);
    }
...
MINT32 AcdkMain::handleQVCB(MVOID *param)
{
    ...
    ACDK_LOGD("QV src VA(0x%" PRIxPTR")", (uintptr_t)pQVBufIn);
    ...
    else
    {
        ...
        ACDK_LOGD(" mQvIMemInfo : vir(0x%x),phy(0x%x),memID(%d)",(int)mQvIMemInfo.virtAddr,(int)mQvIMemInfo.phyAddr,mQvIMemInfo.memID);
    }
...
MINT32 AcdkMain::handleCapCB(MVOID *param)
{
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"JPEG src VA(0x%" PRIxPTR")", (uintptr_t)captureAddr);
    ...
    sprintf(szFileName, "%s/acdkCap.jpg", mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"captureAddr(%p),captureSize(%x)",captureAddr,captureSize);
...
MINT32 AcdkMain::saveJPEGImg()
{
    ...
    ACDK_LOGD("JPEG src VA(0x%x)", (int)mJpgIMemInfo.virtAddr);
...
MINT32 AcdkMain::saveRAWImg(MINT32 mode)
{
    ...
    if(mSensorType == SENSOR_TYPE_YUV) //YUV
    {
        sprintf(szFileName, "%s/YUVImg_YUY2.yuv" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
    }
    else
    {
        if(mCapType == PURE_RAW8_TYPE || g_dumpRAW == PURE_RAW8_TYPE)
        {
            sprintf(szFileName, "%s/acdkCapPureRaw8.raw" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
        }
        else if(mCapType == PURE_RAW10_TYPE || g_dumpRAW == PURE_RAW10_TYPE)
        {
            sprintf(szFileName, "%s/acdkCapPureRaw10.raw" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
        }
        else if(mCapType == PROCESSED_RAW8_TYPE || g_dumpRAW == PROCESSED_RAW8_TYPE)
        {
            sprintf(szFileName, "%s/acdkCapProcRaw8.raw" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
        }
        else if(mCapType == PROCESSED_RAW10_TYPE || g_dumpRAW == PROCESSED_RAW10_TYPE)
        {
            sprintf(szFileName, "%s/acdkCapProcRaw10.raw" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
        }
        else{
...
)
{
    ...
    else if(a_u4Ioctl == ACDK_CMD_CAPTURE)
    {
        ...
        ACDK_LOGD("CB:0x%p", pCapConfig->fpCapCB);
    ...
    else if(a_u4Ioctl == ACDK_CMD_SET_CCT_ON)
    {
        mCctOn = MTRUE;
    }
