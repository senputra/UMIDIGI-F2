...
{
    FUNC_START;
    MY_LOGI("RequestNo: %d,FrameNo: %d",pFrame->getRequestNo(),pFrame->getFrameNo());
    //
    if( ! pFrame.get() ) {
        MY_LOGE("Null frame");
        return BAD_VALUE;
...
)
{
    ...
    while( pEncodeFrame->mpJpeg_Main.get() == NULL ) {
        ...
        // do encode
        {
            ...
            MY_LOGD("main bistream size %zu, isHwEncodeSupportd: %d", pEncodeFrame->mpJpeg_Main->getBitstreamSize(),params.codecType);
        }
