...
freeBuffer()
{
    ...
    if ((int)mImgBufQue.size() < miBufCnt) {
        MY_LOGW("to free cnt(%d) < total cnt(%d)",(int)mImgBufQue.size(), miBufCnt);
    }
    mImgBufQue.clear();
...
dequeProvider(CapBufQueNode& rNode)
{
    ...
    if  ( ! mImgBufQue.empty() )
    {
        ...
        MY_LOGD_IF(
            ...
            "+ Que.size(%d); Buffer[%s@%p@%d@%s@(%d)%dx%d-%dBit@Timestamp(%lld)]",
            (int)mImgBufQue.size(),
            rNode.mainImgNode->getBufName(), rNode.mainImgNode->getVirAddr(), (int)rNode.mainImgNode->getBufSize(), rNode.mainImgNode->getImgFormat().string(),
            rNode.mainImgNode->getImgWidthStride(), rNode.mainImgNode->getImgWidth(), rNode.mainImgNode->getImgHeight(),
            rNode.mainImgNode->getBitsPerPixel(), (long long)rNode.mainImgNode->getTimestamp()
        );
...
dequeProvider(list<CapBufQueNode>* pvNode)
{
    ...
    if  ( ! mImgBufQue.empty() )
    {
        ...
        list<CapBufQueNode>::iterator it;
        for (it = mImgBufQue.begin(); it != mImgBufQue.end(); it++ ) {
            if ((int) mImgBufQue.size() > mStoredBufCnt){
                rNode = *mImgBufQue.begin();
                rNode.mainImgNode.setStatus(ImgBufQueNode::eSTATUS_TODO);
                mImgBufQue.erase(mImgBufQue.begin());
...
enqueProvider(unsigned int va, bool bIsFilled)
{
    ...
    if  ( ! mDequedImgBufQue.empty() )
    {
        list<CapBufQueNode>::iterator it;
        for (it = mDequedImgBufQue.begin(); it != mDequedImgBufQue.end(); ) {
            if ( va == (MUINTPTR)(*it).mainImgNode->getVirAddr() ){
                rNode = *it;
                //
                MY_LOGD_IF(
                    ENABLE_LOG_PER_FRAME,
...
dequeProcessor(CapBufQueNode& rNode, int index=0)
{
    ...
    while   ( !mImgBufQue.back().mainImgNode.isDONE() )
    {
        ...
        if  ( NO_ERROR != status )
        {
            MY_LOGW("wait status(%d), Que.size(%d)", status, (int)mImgBufQue.size());
        }
    }
    MY_LOGD("2");
...
CaptureBufMgr::~CaptureBufMgr()
{
    MY_LOGD("this=%p, mspHwBufPvdr=%p, sizeof:%d",
             this,  &mspHwBufPvdr, (int)sizeof(CaptureBufMgr));
}
...
createBuffer()
{
    ...
    MY_LOGW_IF( mbufInfo.virtAddr & (L1_CACHE_BYTES-1), "bufAddr(%lx) not aligned!", (unsigned long)mbufInfo.virtAddr);
    //
    MY_LOGD("CapBuffer + 0x%lx(0x%x)", (unsigned long)mbufInfo.virtAddr, mbufInfo.size);
    FUNCTION_OUT;
}
