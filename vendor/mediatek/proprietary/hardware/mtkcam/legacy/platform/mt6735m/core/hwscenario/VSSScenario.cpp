    ...
    for (MUINT32 i = 0; i < size; i++)
    {
        ...
        if(size > 1)
        {
            MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "P1(%d-%d/0x%08X)",
                ...
                (unsigned int)rQbufInfo.vBufInfo.at(0).u4BufVA);
        }
        else
        {
    ...
    for (MUINT32 i = 0; i < size; i++)
    {
        ...
        if(size > 1)
        {
            MY_LOGD_IF(0, "P2O(%d-%d/0x%08X)",
                        ...
                        (unsigned int)vQbufInfo.at(i).vBufInfo.at(0).u4BufVA);
        }
        else
        {
    ...
    for (MUINT32 i = 0; i < size; i++)
    {
        if(size > 1)
        {
            MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "P1(%d-%d/0x%08X/0x%08X/%d/%d.%06d)",
                ...
                (unsigned int)one.bufInfo.vBufInfo.at(i).u4BufVA,
                (unsigned int)one.bufInfo.vBufInfo.at(i).u4BufPA,
        ...
        //for debug scan line
        property_get( "vendor.debug.cam.scanline.P1", propertyValue, "0");
    ...
    vector<PortQTBufInfo> *pBufIn)
{
    ...
    //
    MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "SR, port = 0x%x", port);
    ...
    if (port & eID_Pass2DISPO)
    {
        ...
        MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "DISPO(%d/0x%08X)",
            ...
            (unsigned int)one.bufInfo.vBufInfo.at(0).u4BufVA);
        //for debug scan line
    ...
    CAM_TRACE_END();
    //for debug scan line
    property_get( "vendor.debug.cam.scanline.P2", propertyValue, "0");
    if( atoi(propertyValue) == 1 && debugScanLine != NULL )
    {
        if (port & eID_Pass2DISPO)
        {
    ...
    vector<PortQTBufInfo> *pBufIn)
{
    ...
    if(bTwoRun)
    {
        ...
        MY_LOGD_IF(1, "TR:In(%d/0x%08X)",
            ...
            (unsigned int)mTwoRunRotInfo.inBuf.vBufInfo.at(0).u4BufVA);
        //
        ...
        MY_LOGD_IF(1, "TR:Out(%d/0x%08X)",
            ...
            (unsigned int)mTwoRunRotInfo.outBuf.vBufInfo.at(0).u4BufVA);
        //
        mpPostProcPipe->sendCommand(EPIPECmd_SET_CONFIG_STAGE, (MINT32)eConfigSettingStage_UpdateTrigger, 0, 0);
        mpPostProcPipe->sendCommand((MINT32)EPIPECmd_SET_CURRENT_BUFFER, (MINT32)EPortIndex_IMGI,0,0);
        mpPostProcPipe->sendCommand((MINT32)EPIPECmd_SET_CURRENT_BUFFER, (MINT32)EPortIndex_VIDO,0,0);
        ...
        MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "TR:VIDO(%d/0x%08X)",
            ...
            (unsigned int)out.bufInfo.vBufInfo.at(0).u4BufVA);
        //
        EXIT2:
        MY_LOGD_IF(0, "TR:SP");
        mpPostProcPipe->stop();
    ...
    vector<PortQTBufInfo> *pBufIn)
{
    ...
    if(mbTwoRunPass2Dispo)
    {
        ...
        MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "Temp(%d/0x%08X)",
            ...
            (unsigned int)dispoBuf.bufInfo.vBufInfo.at(0).u4BufVA);
    }
    else
    {
        mapPortCfg(eID_Pass2VIDO, rPortID);
        ...
        MY_LOGD_IF(ENABLE_LOG_PER_FRAME, "Temp(%d/0x%08X)",
            ...
            (unsigned int)vidoBuf.bufInfo.vBufInfo.at(0).u4BufVA);
    }
    //
...
replaceQue(vector<PortBufInfo> *pBufOld, vector<PortBufInfo> *pBufNew)
{
    ...
    MY_LOGD("P1:Old(%d/0x%08X),New(%d/0x%08X)",
            ...
            (unsigned int)rQbufInfo.vBufInfo.at(1).u4BufVA);
    //
    return MTRUE;
}
...
allocTwoRunPass2TempBuf(MUINT32 bufSize)
{
    ...
    MY_LOGD("Temp(%d,0x%08X/0x%08X,%d)",
            ...
            (unsigned int)mTwoRunPass2Info.tempBuf.virtAddr,
            (unsigned int)mTwoRunPass2Info.tempBuf.phyAddr,
