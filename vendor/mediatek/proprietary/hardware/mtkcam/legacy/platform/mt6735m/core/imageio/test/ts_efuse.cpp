...
saveBufToFile(char const*const fname, MUINT8 *const buf, MUINT32 const size)
{
    ...
    LOG_INF("(name, buf, size) = (%s, %016llx, %d)", fname, (unsigned long long)buf, size);
...
int ef_test()
{
    ...
    switch(u4BitDepth)
    {
        ...
        if(MTRUE != (mpIspDrv->init()))
        {
            LOG_ERR("mpIspDrv init fail !!!!!!!!!!!!!!");
            return -1;
        }
        ...
        for(int q=0;q<BD_SEG_NUM;q++)
        {
               ...
               // rQBufInfo.vBufInfo[0].header   = NULL; //no need in D2
                for(int j=0;j<3;j++)
                {
                    ...
                    if(mpImemDrv->mapPhyAddr(&(imgo_buf[j])) < 0)
                    {
                    　　LOG_ERR("mpImemDrv->mapPhyAddr(&(imgo_buf[%d])) fail", j);
                    　　return -1;
                    }
                    ...
                    LOG_INF("IMGO m_pICamIOPipe->enqueOutBuf[%d].u4BufPA(%x).size(0x%x)",j,(MUINT32)rQBufInfo.vBufInfo[0].u4BufVA,rQBufInfo.vBufInfo[0].u4BufSize);
                ...
                for(int j=0;j<3;j++)
                {
                    if(mpImemDrv->unmapPhyAddr(&(imgo_buf[j])) < 0)
                    {
                        LOG_ERR("mpImemDrv->unmapPhyAddr(&(imgo_buf[%d])) fail", j);
                        return -1;
                    }
                    if(mpImemDrv->freeVirtBuf(&(imgo_buf[j])) < 0)
                    {
                        LOG_ERR("mpImemDrv->freeVirtBuf(&(imgo_buf[%d])) fail", j);
                        return -1;
                    }
                }
        }
