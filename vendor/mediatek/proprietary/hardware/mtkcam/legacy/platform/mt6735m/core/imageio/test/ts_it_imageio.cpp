...
saveBufToFile(char const*const fname, MUINT8 *const buf, MUINT32 const size)
{
    ...
    MY_LOGD("(name, buf, size) = (%s, %p, %d)", fname, (void *)buf, size);
    ...
    , u4SensorHeight(0)
{
    ...
    ::memset((void*)&sensorStaticInfo,0,sizeof(sensorStaticInfo));
    ::memset((void*)&sensorDynamicInfo, 0, sizeof(sensorDynamicInfo));
    mu4DeviceID = 0;
    mu4SkipFrame = 0;
    m_TestEnDequethread = 0;
    mInitCount = 0;
...
previewProc(MVOID * arg)
{
    ...
    do
    {
        ...
        #if 1
        MY_LOGD("Deque buf count : %d", (MINT32)rQTimeOutBufInfo.vBufInfo.size());
        for (MUINT32 i = 0; i < rQTimeOutBufInfo.vBufInfo.size(); i++) {
            MY_LOGD("Deque buf #%d : size(%d), VA(x%x), PA(x%x), memID(%d)", i,
                    ...
                    (MUINT32)rQTimeOutBufInfo.vBufInfo.at(i).u4BufVA,
                    (MUINT32)rQTimeOutBufInfo.vBufInfo.at(i).u4BufPA,
        ...
        rPortID.inout = 0;
        for (MUINT i = 0; i < vPostProcInPorts.size(); i++)
        {
        ...
        for (MUINT32 i=0; i<vPostProcOutPorts.size(); i++)
        {
        ...
        for(MUINT32 i=0;i<vPostProcInPorts.size();i++)
        {
