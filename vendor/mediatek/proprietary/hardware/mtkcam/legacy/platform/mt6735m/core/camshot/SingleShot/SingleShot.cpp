    ...
    , mSwNRCropRect(Rect(0,0,0,0))
{
    ...
    property_get("vendor.debug.camera.dump", value, "0");
...
startOne(SensorParam const & rSensorParam)
{
    ...
    {
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("vendor.debug.camera.shotmode", value, "0");
        int pty = atoi(value);
        if( pty > 0 && pty < 4 )
        {
            shotmode = pty;
...
#define DUMP_ImgBufInfo(tempInfo,param) \
    ...
    MBOOL ret = MTRUE;
#if 0
    DUMP_ImgBufInfo(rDstImgBufInfo,eImgFmt);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4ImgWidth);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4ImgHeight);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4BufSize);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4BufVA);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4BufPA);
    DUMP_ImgBufInfo(rDstImgBufInfo,i4MemID);
    DUMP_ImgBufInfo(rDstImgBufInfo,i4BufSecu);
    DUMP_ImgBufInfo(rDstImgBufInfo,i4BufCohe);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4Stride[0]);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4Stride[1]);
    DUMP_ImgBufInfo(rDstImgBufInfo,u4Stride[2]);
#endif
    //(1)allocate SwNR buffer
    MY_LOGD("allocate SwNR buffer");
    ImgBufInfo rSwNRImgBufInfo = querySwNRImgBufInfo();//free in unint() -> freeShotMem()
#if 0
    DUMP_ImgBufInfo(rSwNRImgBufInfo,eImgFmt);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4ImgWidth);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4ImgHeight);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4BufSize);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4BufVA);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4BufPA);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,i4MemID);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,i4BufSecu);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,i4BufCohe);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4Stride[0]);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4Stride[1]);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4Stride[2]);
#endif
    ...
    //find sonsorOpenId for getting SwNRParam and SwNR
    MUINT i, sensorOpenId = -1, sensorNum = pHalSensorList->queryNumberOfSensors();
    for(i=0; i<sensorNum; i++)
    {
        if(pHalSensorList->querySensorDevIdx(i) == static_cast<MUINT32>(mSensorParam.u4DeviceID))
        {
            sensorOpenId = i;
            MY_LOGD("[doSwNR] sensorOpenId(%d)",sensorOpenId);
            break;
        }
    }
    if( sensorOpenId == -1)
    {
        MY_LOGE("cannot find sensorOpenId");
        return;
    }

    ImgBufInfo rJpegImgBufInfo = queryJpegImgBufInfo();
    int scale = 100*(rJpegImgBufInfo.u4ImgWidth+rJpegImgBufInfo.u4ImgHeight)/(mSwNRCropRect.w+mSwNRCropRect.h);
    SwNRParam* pParam = SwNRParam::getInstance(sensorOpenId);
    if( !pParam )
    {
    ...
    {
        MY_LOGD("setParam(), mIso=%d,scale=%d,crop(w,h)=(%d,%d),jpeg(w,h)=(%d,%d)",mIso,scale,mSwNRCropRect.w,mSwNRCropRect.h,rJpegImgBufInfo.u4ImgWidth,rJpegImgBufInfo.u4ImgHeight);

        MINT32 perfLevel = -1;
        MUINT const sensorDev = pHalSensorList->querySensorDevIdx(sensorOpenId);
        ...
        pParam->setParam(mIso,scale,perfLevel);
    }

    if( !mpSwNR )
    {
        MY_LOGD("[doSwNR] new SwNR( sensorOpenId = %d )",sensorOpenId);
        mpSwNR = new SwNR( sensorOpenId );//getSensorIdx()
    }
    if( mpSwNR )
    {
        MY_LOGD("mpSwNR->doSwNR() start");
        mpSwNR->doSwNR(tempIImageBuffer);
        MY_LOGD("mpSwNR->doSwNR() end");
    }
#if 0
    DUMP_ImgBufInfo(rSwNRImgBufInfo,eImgFmt);
    DUMP_ImgBufInfo(rSwNRImgBufInfo,u4ImgWidth);
...
createYuvRawImg(ImgBufInfo const & rSrcImgBufInfo, Rect const rSrcCropRect, MUINT32 const u4Img1Rot, MUINT32 const u4Img1Flip, ImgBufInfo const & rDstImgBufInfo1, ImgBufInfo const &rDstImgBufInfo2 )
{
    ...
    {
        mbSwNR = false;
        mSwNRCropRect.w = rSrcCropRect.w;
        mSwNRCropRect.h = rSrcCropRect.h;
        doSwNR(rDstImgBufInfo1);
    }
    //
    FUNCTION_LOG_END;
...
registerImgBufInfo(ECamShotImgBufType const eBufType, ImgBufInfo const &rImgBuf)
{
     ...
     MY_LOGD("[registerImgBufInfo] (VA, PA, Size, ID) = (%p, %p, %d, %d)", rImgBuf.u4BufVA, rImgBuf.u4BufPA, rImgBuf.u4BufSize, rImgBuf.i4MemID);
...
allocImgMem(char const* const pszName, EImageFormat const eFmt, MUINT32 const u4Width, MUINT32 const u4Height, IMEM_BUF_INFO & rMem)
{
    ...
    if (0 == rMem.size)
    {
        ...
        allocMem(rMem);
        MY_LOGD("[allocImgMem] (va, pa, size) = (%p, %p, %d)",  rMem.virtAddr, rMem.phyAddr, rMem.size);
    }
    else
    {
        if (rMem.size < u4BufSize)
        {
            ...
            MY_LOGD("[allocImgMem] re-allocate (va, pa, size) = (%p, %p, %d)", rMem.virtAddr, rMem.phyAddr, rMem.size);
        }
    }
    profile.print();
    return MTRUE;
...
querySwNRImgBufInfo()
{
    EImageFormat eImgFmt = eImgFmt_I420;//use I420 format
    //
    MUINT32 u4Width = 0, u4Height = 0;
    getPictureDimension(u4Width, u4Height);
    //
    allocImgMem("SwNR", eImgFmt, u4Width, u4Height, mSwNRMem);
    ImgBufInfo rImgBufInfo;
    setImageBuf(eImgFmt, u4Width, u4Height,  rImgBufInfo, mSwNRMem);
...
preAllocMem()
{
    ...
    pthread_attr_t const attr = 
    {
        ...
        .sched_priority      = NICE_CAMERA_CAPTURE - 1
    };
...
checkIfFireFlash(MUINT32 countdown)
{
    if( mu4FlashCountDown > 0 &&  //not fire yet
        ( mu4FlashCountDown ==(int)(countdown+1)  || //count down
          mu4FlashCountDown > (int)countdown )        //fire immediately
        )
    {
        MY_LOGD("fire flash");
