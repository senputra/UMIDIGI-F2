...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL Hal3AFlowCtrl::setSensorAndPass1(const ParamIspProfile_T& /*rParamIspProfile*/, MBOOL /*fgPerframe*/)
{
    return MTRUE;
}
...
getResultCur(MUINT32 i4FrmId, Result_T& rResult)
{
    ...
    MY_LOG_IF(m_3ALogEnable, "[%s] i4Ret(%d) i4FrmId#(%d) i4ResultWaitCnt(%d)", __FUNCTION__, i4Ret, i4FrmId, i4ResultWaitCnt);
    if (-1 == i4Ret)
    {
        while (i4ResultWaitCnt)
        {
           MY_LOG_IF(m_3ALogEnable, "[%s] wait result #(%d) i4ResultWaitCnt(%d)", __FUNCTION__, i4FrmId, i4ResultWaitCnt);
           m_rResultCond.waitRelative(m_rResultMtx, (long long int)500000000);
           MY_LOG_IF(m_3ALogEnable, "[%s] wait result done #(%d), i4ResultWaitCnt(%d)", __FUNCTION__, i4FrmId, i4ResultWaitCnt);
           i4ResultWaitCnt--;
           i4Ret = m_rResultBufCur.getResult(i4FrmId, rResult);
    ...
    i4Ret = m_rResultBuf.getResult(i4FrmId, rResultatStart);
...
Hal3AFlowCtrl::notifyP1Done(MUINT32 u4MagicNum, MVOID* /*pvArg*/)
{
    ...
    if ((MINT32)u4MagicNum == mLastFlashOnFrmIdx)
    {
...
Hal3AFlowCtrl::isMeetMainFlashOnCondition()
{
    return
        ...
        ( ((m_rParam.u4AeMode == MTK_CONTROL_AE_MODE_ON_ALWAYS_FLASH)||((m_rParam.u4AeMode == MTK_CONTROL_AE_MODE_ON_AUTO_FLASH) && isStrobeBVTrigger()))
           && ((m_rParam.u1CaptureIntent == MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE) && (m_rParam.i4RawType == ERawType_Pure)) );
}
