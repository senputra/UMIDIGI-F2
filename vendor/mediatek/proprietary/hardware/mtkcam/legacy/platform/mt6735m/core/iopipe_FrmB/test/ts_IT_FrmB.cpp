...
pthread_mutex_t m_mutex = PTHREAD_MUTEX_INITIALIZER;    //mutex for read/write pass1 dequeued ring buffer count
...
#ifdef __LP64__
    pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_RR,/*PRIO_RT_ISPDEQUEUE_THREAD*/90, {0}};/*LukeHu+-*/
#else
    pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_RR,/*PRIO_RT_ISPDEQUEUE_THREAD*/90};/*LukeHu+-*/
#endif
    pthread_create(&m_Thread, &attr, _infiniteloop, this);
    //
#ifdef __LP64__
...
MVOID* _test_camio_::_infiniteloop(void* arg)
{
    ...
    while(_loop<max_cnt){
        ...
        if(_this->m_pNormalP_FrmB->deque(dequeBufInfo) == MTRUE){
            for(MUINT32 i=0;i<dequeBufInfo.mvOut.size();i++){
                printf("[E_L deque]: dma:0x%x, PA:0x%lx, crop:0x%x,0x%x,0x%x,0x%x_0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, dummy:0x%x, bufidx:0x%x\n",\
                dequeBufInfo.mvOut.at(i).mPortID.index,(unsigned long)dequeBufInfo.mvOut.at(i).mBuffer->getBufPA(0),\
            ...
            if(!Multi_thread)
            {
                ...
                for(int i=0;i<_replace.mvOut.size();i++){
                    printf("[E_L enque]: dma:0x%x, PA:0x%lx, crop:0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, bufidx:0x%x\n",\
                    _replace.mvOut.at(i).mPortID.index,(unsigned long)_replace.mvOut.at(i).mBuffer->getBufPA(0),\
...
MVOID* _test_camio_::_p2ThreadLoop(MVOID *arg)
{
    ...
    while(1)
    {
        ...
        {
            printf("===m_dequeBufInfoList size(%lu)=== \n",(unsigned long)_this->m_dequeBufInfoList.size());
        ...
        else
        {
            ...
            for(MUINT32 i=0;i<p1dequeBufInfo.mvOut.size();i++)
            {
                if(i==0)
                {
                    ...
                    {    //life cycle for variable
                        ...
                        if (dump)
                        {
                            ...
                            //output
                            for(MUINT32 j=0;j<dequeParams.mvOut.size();j++)
                            {
                                if(j==0)
                                {
...
int basic_flow(int num,int type)
{
    ...
    for(int i=0;i<(Enque_buf+Dummy_buf+replace_buf);i++)
    {
        if(enablePort & __IMG2O_ENABLE)
        {
            ...
            LOG_INF("img buffer(%d): img2o pa:0x%lx\n",i,(unsigned long)pImgBuffer[__IMG2O][i]->getBufPA(0));
        }
        if(enablePort & __IMGO_ENABLE)
        {
            if( !getOutputFmt_2(IMGO,SEN_PIX_BITDEPTH,mSensorInfo, &fmt))
            ...
            LOG_INF("img buffer(%d): imgo pa:0x%lx\n",i,(unsigned long)pImgBuffer[__IMGO][i]->getBufPA(0));
        }
    }
