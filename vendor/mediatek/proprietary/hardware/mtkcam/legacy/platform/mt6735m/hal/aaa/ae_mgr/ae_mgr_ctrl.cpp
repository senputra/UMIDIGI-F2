...
MRESULT AeMgr::UpdateSensorParams(AE_SENSOR_PARAM_T strSensorParams)
{
    ...
    if(strSensorParams.u8FrameDuration < strSensorParams.u8ExposureTime) {
        ...
        MY_LOG("[%s()] i4SensorDev = %d line:%d Frame Duration less than EXposure time: %lld %ld\n", __FUNCTION__, m_eSensorDev, __LINE__,
        (long long)strSensorParams.u8FrameDuration, (long)strSensorParams.u8ExposureTime);
    } else {
    ...
    MY_LOG("[%s()] i4SensorDev = %d line:%d Idx:%d %d FrameDuration:%d %lld Exposure Time:%d %ld ISO:%d %d\n", __FUNCTION__, m_eSensorDev, __LINE__,
        ...
        m_SensorQueueCtrl.rSensorParamQueue[m_SensorQueueCtrl.uInputIndex].u4ExposureTime, (long)strSensorParams.u8ExposureTime,
