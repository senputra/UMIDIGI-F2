...
class NormalPipe_FrmB : virtual public IHalCamIO, public INormalPipe_FrmB
{
...
public:
    ...
    virtual MBOOL   configFrame(QFrameParam const& rQParam){if(rQParam.mMagicNum){} return MFALSE;};

    //replace queue
    virtual MBOOL   replace(BufInfo const& bufOld, BufInfo const& bufNew){if(bufOld.mBufIdx){} if(bufNew.mBufIdx){} return MFALSE;};
