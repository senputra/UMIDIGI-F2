...
MBOOL Hal3AYuv::sendCommand(ECmd_T const eCmd, MINTPTR const /*i4Arg*/)
{
    ...
    else if (eCmd == ECmd_PrecaptureStart)
    {
        if (m_pStrobeDrvObj)
        {
            MBOOL fgFlashOn = ((MUINT32)eShotMode_ZsdShot == m_rParam.u4ShotMode) ? m_isFlashOnCapture : isAEFlashOn();
            if (fgFlashOn)
            {
    ...
    else if (eCmd == ECmd_CaptureEnd)
    {
        ...
        if (m_pStrobeDrvObj && m_bFlashActive == TRUE && m_rParam.u4ShotMode != (MUINT32)CAPTURE_MODE_BURST_SHOT)
        {
            m_pStrobeDrvObj->setOnOff(0);
            MY_LOG("setFire OFF\n");
