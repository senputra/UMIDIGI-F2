...
convertSensorCrop(MBOOL fgWithSensorCropInfo, const SensorCropInfo_T& rFullInfo, const SensorCropInfo_T& rCropInfo, ShadingTblCropCfg_T& rCropCfg)
{
    ...
    else
    {
        ...
        {
            LSC_LOG("Exceed right side, x_sc(%3.3f), w2s(%f), out_w(%3.3f)", x_sc, w2s, out_w);
            //w2s = out_w - x_sc;
            x_sc = 0.0f;
            w2s = out_w;
        }
        ...
        {
            LSC_LOG("Exceed bottom side, y_sc(%3.3f), h2s(%f), out_h(%3.3f)", y_sc, h2s, out_h);
            //h2s = out_h - y_sc;
            y_sc = 0.0f;
            h2s = out_h;
        }
...
static MVOID _LogShadingSpec(const char* str, const SHADING_TBL_SPEC& rSpec)
{
    LSC_LOG("%s-------------\n"
            ...
            rSpec.ratio_idx, rSpec.grgb_same, rSpec.data_type, (unsigned long)rSpec.table);
}
...
allocMemBuf(IMEM_BUF_INFO& rBufInfo, MUINT32 const u4Size)
{
    ...
    if (!rBufInfo.virtAddr)
    {
        ...
        if (0 == m_pIMemDrv->allocVirtBuf(&rBufInfo))
        {
            ...
            {
                LSC_ERR("mapPhyAddr error, virtAddr(%u), size(%d)\n", (unsigned int)rBufInfo.virtAddr, (MINT32)rBufInfo.size);
                ret = MFALSE;
            }
            else
            {
    ...
    {
        ret = MTRUE;
        LSC_LOG("Already Exist! virtAddr(%u), size(%d)\n", (unsigned int)rBufInfo.virtAddr, u4Size);
    }
    return ret;
}
...
freeMemBuf(IMEM_BUF_INFO& rBufInfo)
{
    ...
    if (0 == m_pIMemDrv->unmapPhyAddr(&rBufInfo))
    {
        ...
        {
            LSC_LOG("freeVirtBuf OK, memID(%d), virtAddr(%u), phyAddr(%u)\n", rBufInfo.memID, (unsigned int)rBufInfo.virtAddr, (unsigned int)rBufInfo.phyAddr);
            rBufInfo.virtAddr = 0;
            ret = MTRUE;
        }
        else
        {
            LSC_LOG("freeVirtBuf Fail, memID(%d), virtAddr(%u), phyAddr(%u)\n", rBufInfo.memID, (unsigned int)rBufInfo.virtAddr, (unsigned int)rBufInfo.phyAddr);
            ret = MFALSE;
        }
    }
    else
...
{
    LSC_LOG("memID(%d), virtAddr(%u), phyAddr(%u), size(%d)\n",
        rBufInfo.memID, (unsigned int)rBufInfo.virtAddr, (unsigned int)rBufInfo.phyAddr, rBufInfo.size);
    return MTRUE;
}
...
doShadingAlign(MVOID* pMtkLsc)
{
...
#endif
    {
        ...
        LSC_LOG("Start Align, IMG(%d x %d), Grid(%d x %d), Bayer(%d), GoldenGain(0x%08lx), UnitGain(0x%08lx)",
            rAlign.u4Width, rAlign.u4Height, rAlign.u1GainXNum, rAlign.u1GainYNum, rAlign.u1GainBayer, (unsigned long)pGoldenGainCp, (unsigned long)pUnitGainCp);
...
{
    LSC_LOG("[%s] fgForce(%d)", __FUNCTION__, fgForce);
...
{
    LSC_LOG("[%s] i4TGInfo(%d)", __FUNCTION__, i4TGInfo);
