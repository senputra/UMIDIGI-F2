...
onDestroyLocked()
{
    ...
    for ( size_t i = 0; i < mvReusableNodes.size(); i++ ) {
        NodeId_T const nodeid = mvReusableNodes.keyAt(i);
        switch ( nodeid ) {
            case eNODEID_P1Node:
            {
                mpNode_P1->uninit();
...
)
{
    ...
    if ( bRet ) {
        MY_LOGW_IF(1, "Need to reconfigure of node: openId(%d:%d) nodeId(%#" PRIxPTR ":%#" PRIxPTR ") nodeName(%s:%s)(%p:%p)",
        ...
        MY_LOGD_IF(0, "No need to reconfigure of node: openId(%d:%d) nodeId(%#" PRIxPTR ":%#" PRIxPTR ") nodeName(%s:%s)(%p:%p)",
                      rParams.openId, pNode->getOpenId(),
                      rParams.nodeId, pNode->getNodeId(),
                      rParams.nodeName, pNode->getNodeName(),
                      rParams.nodeName, pNode->getNodeName()
...
{
    Mutex::Autolock _l(mLock);
    MY_LOGD_IF( pStreamInfo.get(), "streamId:%#" PRIx64 " %s %p type:%d Size(%d,%d) MaxBufSize:%zu MinInitBufSize:%zu",
    ...
    MY_LOGD("operate on streampool index(%zd) size(%zu)", index, mvStreamStatus.size());
    ...
    if ( StreamStatus::UNINIT == mvStreamStatus.valueAt(index) ) {
        MY_LOGD("Create new halimage stream id:%#" PRIx64, streamid);
...
PipelineResourceMgr::
createStream(
    StreamId_T streamid __attribute__((unused)),
    android::sp<IImageStreamInfo>&        pStreamInfo
)
{
    //
    ...
    //return pPool;
    MY_LOGD("add to pool map %zu(%p)", mHalImageStreamBufferPoolMap.size(), pPool.get());
    ...
    switch ( nodeid ) {
        ...
        case eNODEID_JpegNode:
        {
            ...
            if ( index >= 0 ) {
                ...
                MY_LOGD("Modify init param of nodeid(%#" PRIxPTR ") openid(%d) nodename(%p:%s)",
                         param.nodeId, param.openId, param.nodeName, param.nodeName);
            } else {
                MY_LOGD("Base new init param of nodeid(%#" PRIxPTR ") openid(%d) nodename(%p:%s)",
                ...
                MY_LOGD("ADD new init param of nodeid(%#" PRIxPTR ") openid(%d) nodename(%p:%s)",
                         param.nodeId, param.openId, param.nodeName, param.nodeName);
            }
        }
        break;
...
)
{
    ...
    MY_LOGD("updata config param full raw-%zu", params->pvOutImage_full.size());
...
)
{
    ...
    if ( !params || nodeid!=eNODEID_P1Node )
        return -EINVAL;
    //
    MY_LOGD("full(%zu) resizer(%p)", params->pvOutImage_full.size(), params->pOutImage_resizer.get());
    //
    if ( ! isEqualMetaStreamInfo(mConfigParams_P1.pInAppMeta, params->pInAppMeta) ||
...
)
{
    ...
    ssize_t index = mvNodeStatus.indexOfKey(nodeid);
    if ( index < 0 ) {
        MY_LOGE("Node(%zd) must be add to status map", nodeid);
    }
    //
    MERROR err = OK;
        ...
        if ( bReconfigure || mvNodeStatus.valueAt(index)==NodeStatus::RECONFIG || mSceneStatus==SceneStatus::RECONFIG) {
            ...
            if ( mpNode_P1.get() ) {
                ...
                for( size_t i=0; i<mHalImageStreamBufferPoolMap.size(); i++ ) {
                    ...
                    if ( !bHasStream ) {
                        MY_LOGD("We should remove this pool from resourcemgr (%#" PRIx64 ")", streamid);
                        sp<HalImageStreamBufferPoolT>& pPool = mHalImageStreamBufferPoolMap.editValueFor(streamid);
                        pPool->uninitPool(LOG_TAG);
