...
status_t AcdkMhalOpenDevice(const char *name, const int id)
{
    int rc = 0;
                      ...
                      void *user)
{
    ...
    ACDK_LOGD("%s  va=%p, size=%x, offset=%x", __FUNCTION__, va, (int)size, (int)offset);
    ...
    switch  (uMsg_type & ~CAMERA_MSG_PREVIEW_METADATA)
    {
        ...
        case MTK_CAMERA_MSG_EXT_DATA:{
                ...
                ACDK_LOGD("%s  va=%p, size=%zu, offset=%zu", __FUNCTION__, va, size, offset);
                g_acdkMhalCapObserver.notify(ACDK_CB_CAPTURE, va, 0, (MUINT32)size,0);
                mCaptureDone = MTRUE;
            }
            break;
...
MINT32 AcdkMhalEng::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
...
#ifdef USING_HIDL
    char value[] = "MtkEng";
    property_set("vendor.mtk.client.appmode",value);
#else
    ...
    ACDK_LOGD("%s  device.name=%d", __FUNCTION__, mAcdkMhalPrvParam.sensorIndex);
#ifdef USING_HIDL
    switch(mAcdkMhalPrvParam.sensorIndex){
    case 0:
        AcdkMhalOpenDevice("device@1.0/internal/0", 0);
        break;
    case 1:
        AcdkMhalOpenDevice("device@1.0/internal/1", 1);
        break;
    default:
        AcdkMhalOpenDevice("device@1.0/internal/2", 2);
        break;
    }
#else
    switch(mAcdkMhalPrvParam.sensorIndex){
    case 0:
        AcdkMhalOpenDevice("0", -1);
        break;
    case 1:
        AcdkMhalOpenDevice("1", -1);
        break;
    default:
        AcdkMhalOpenDevice("2", -1);
        break;
    }
#endif
