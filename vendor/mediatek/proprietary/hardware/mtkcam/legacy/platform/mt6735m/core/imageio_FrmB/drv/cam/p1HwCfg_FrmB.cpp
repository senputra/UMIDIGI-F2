        ...
        if(mod == CAM_DMA_AAO) {
            ...
            if (HWRWCTRL_DBG_DUMP) {
                ...
                if(1){
                    ...
                    for (MUINT32 i = 0; i < (accessObj->m_iSize); i++, iDstPtr++, iSrcPtr++)
                    {
   ...
   IspDrv* pTarVirDrv)//Dst
{
    ...
    {
        //error parameter
        P1HWRW_ERR("_flushHwCfg2CQ: (0 == accessObj->m_iSize(%d)) || ((accessObj->m_iSize*4)(0x%x) > sizeof(isp_reg_t))(0x%x)",accessObj->m_iSize, (accessObj->m_iSize*4), (MUINT32)sizeof(isp_reg_t));
        return MFALSE;
    }
...
ObjAccessHW::~ObjAccessHW(void){
    P1HWRW_DBG("~ObjAccessHW, this=0x%p, sz=%d", this, (MINT32)sizeof(ObjAccessHW));
