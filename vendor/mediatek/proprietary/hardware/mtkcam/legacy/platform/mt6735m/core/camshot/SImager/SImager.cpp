    ...
    , mTargetBufInfo()
{
    ...
    MY_LOGD("src (VA, PA, Size, memId) = (0x%x, 0x%x, %d, %d)", (unsigned int)rImgBufInfo.u4BufVA, (unsigned int)rImgBufInfo.u4BufPA, rImgBufInfo.u4BufSize, rImgBufInfo.i4MemID);
...
{
    MY_LOGV("(notify_cb, user)=(%p, %p)", notify_cb, user);
    mpCbUser = user;
    mNotifyCb = notify_cb;
}
...
execute(MUINT32 const u4TimeoutMs)
{
    ...
    {
        encode(mSrcImgBufInfo, rTargetImgBuf, mTargetImgInfo.rROI, mTargetImgInfo.u4Rotation, mTargetImgInfo.u4Flip, mTargetImgInfo.u4Quality, mTargetImgInfo.u4IsSOI, meCodecType, mu4JpegSize);
    }
    else
    {
...
setTargetBufInfo(BufInfo const &rBufInfo)
{
    ...
    MY_LOGD(" Target Buf Info (size, VA, PA, id) = (%d, %x, %x, %d)",
                 mTargetBufInfo.u4BufSize, (unsigned int)mTargetBufInfo.u4BufVA, (unsigned int)mTargetBufInfo.u4BufPA, mTargetBufInfo.i4MemID);
...
setEncodeParam(MUINT32 const & u4IsSOI, MUINT32 const & u4Quality, MUINT32 const &eCodecType)
{
    ...
    MY_LOGI("enc param (SOI, Quality, Type) = (%d, %d, %d)", u4IsSOI, u4Quality, eCodecType);
    //FUNCTION_LOG_END;
    //
    return MTRUE;
}
...
)
{
    ...
    ret = pJpegCodec->encode(rSrcBufInfo, rDstBufInfo, rROI, u4Rotation, u4Flip,  u4Quality, u4IsSOI, eCodecType, u4EncSize);
