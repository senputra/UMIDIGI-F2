...
/*2258914*/
    ...
    }else if( (FlashMgr::getInstance().getFlashMode(m_SensorDevId)!= LIB3A_FLASH_MODE_FORCE_TORCH)
    		&& (!m_pStateMgr->mAFStateCntSet.bLampAlreadyOnBeforeSingleAF))
    { 
    	FlashMgr::getInstance().endPrecapture(m_SensorDevId);
    }
...
/*
        ...
        AeMgr::getInstance().getAESGG1Gain(&u4AFSGG1Gain);
        AfMgr::getInstance().setSGGPGN((MINT32) u4AFSGG1Gain);
*/
...
{m_pStateMgr->proceedAFState(); return S_3A_OK;}
	...
	/*2286181*/
	if(FlashMgr::getInstance().isAFLampOn(m_SensorDevId) == 0)
