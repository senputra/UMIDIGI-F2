...
postCmd(void* pArg)
{
    ...
    MY_LOG_IF(m_fgLogEn, "[%s] m_rCmdQ size(%d), magic(%d), exp(%d), gain(%d)",
        __FUNCTION__, (int)m_rCmdQ.size(), rCmd.u4MagicNum, rCmd.u4ExposureTime, rCmd.u4SensorGain);
    return MTRUE;
}
...
onThreadLoop(MVOID* pArg)
{
    ...
    while (_this->getCmd(rCmd))
    {
        ...
        if (pGainThread)
        {
            ThreadSensorGain::Cmd_T rCmdGain(rCmd.u4MagicNum, rCmd.u4ExposureTime, rCmd.u4SensorGain);
