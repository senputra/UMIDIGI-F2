...
{   \
    ...
    for(unsigned int j=0;j<_num_;j++){\
        if(j > 20)\
            break;\
        else{\
...
{                                                                                               \
    if(_tpipePoint_->_tpipeField_ != (int)_regPoint_->_regElement1_.Bits._regElement2){         \
        ...
        _regPoint_->_regElement1_.Bits._regElement2 = ~(_regPoint_->_regElement1_.Bits._regElement2); \
        ...
        LOG_INF("[Diff][0x%lx][Bit-%d]",                                                         \
                (unsigned long)(&_regPoint_->_regElement1_)-(unsigned long)(&_regPoint_->rsv_0000[0]), i);          \
    }                                                                                           \
}
...
MBOOL IspDrv::cqAddModule(MINT32 cq,MINT32 burstQIdx,MINT32 dupCqIdx, CAM_MODULE_ENUM moduleId)
{
    ...
    MUINTPTR dummyaddr;
    int realCQIdx=getRealCQIndex(cq,burstQIdx,dupCqIdx);
        LOG_DBG("- E. isp_cq[0x%x],[%d],burstqIdx[%d],dupCqIdx[%d]",realCQIdx,moduleId,burstQIdx,dupCqIdx);
    //
    Mutex::Autolock lock(cqVirDesLock);
...
MBOOL IspDrvImp::init(const char* userName)
{
    ...
    else    // 1st time open success.   // Sometimes GDMA will go this path, too. e.g. File Manager -> Phone Storage -> Photo.
    {
            ...
            LOG_DBG("m_RTBufTblSize(0x%x),m_pRTBufTbl(0x%lx)",m_RTBufTblSize,(unsigned long)m_pRTBufTbl);
    ...
    LOG_DBG("[m_pIMemDrv]:0x%lx", (unsigned long)m_pIMemDrv);    //Vent@20121107: Fix build warning: format '%x' expects argument of type 'unsigned int', but argument 5 has type 'IMemDrv*' [-Wformat]
    if ( NULL == m_pIMemDrv ) {
    ...
    for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
        for(int j=0;j<ISP_TUNING_QUEUE_NUM;j++){
            ...
            memset((MUINT8*)mTuningQueInf[i][j].pTuningQue,0,mTuningQueInf[i][j].queSize);
            LOG_INF("p2cq(%d),que(%d):pTuningQue(0x%lx)",i,j,(unsigned long)(mTuningQueInf[i][j].pTuningQue));
        }
        //
        ...
        memset((MUINT8*)mTuningQueIdx[i].keepP1Que.pTuningQue,0,mTuningQueIdx[i].keepP1Que.queSize);
        LOG_INF("p2cq(%d),keepP1Que(0x%lx)",i,(unsigned long)(mTuningQueIdx[i].keepP1Que.pTuningQue));
        //
        mTuningQueIdx[i].isApplyTuning = MFALSE;
        mTuningQueIdx[i].pCurReadP1TuningQue = NULL;
        mTuningQueIdx[i].pCurWriteTuningQue = NULL;
        ...
        for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
            for(int j=0;j<ISP_TUNING_QUEUE_NUM;j++){
                if((mTuningQueInf[i][j].queFd>=0) && (mTuningQueInf[i][j].pTuningQue!=0)){
                    ...
                    LOG_ERR("[Error]free TuningQue error i(%d),j(%d),fd(%d),va(0x%lx)",i,j,mTuningQueInf[i][j].queFd,(unsigned long)mTuningQueInf[i][j].pTuningQue);
                }
            }
            // free keepP1Que
            if((mTuningQueIdx[i].keepP1Que.queFd>=0) && (mTuningQueIdx[i].keepP1Que.pTuningQue!=0)){
                ...
                LOG_ERR("[Error]free keepP1Que error i(%d),fd(%d),va(0x%lx)",i,mTuningQueIdx[i].keepP1Que.queFd,(unsigned long)mTuningQueIdx[i].keepP1Que.pTuningQue);
            }
...
MBOOL IspDrvImp::cqTableControl(ISP_DRV_CQTABLE_CTRL_ENUM cmd,int burstQnum)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQTABLE_CTRL_ALLOC:
            ...
            LOG_VRB("v(0x%x)(0x%x)(0x%lx)(0x%lx)(0x%lx)(0x%lx)",mIspVirRegSize,m_ispVirRegBufInfo.size, \
                                                            (unsigned long)m_ispVirRegBufInfo.virtAddr,(unsigned long)m_ispVirRegBufInfo.phyAddr, \
                                                            (unsigned long)mpIspVirRegBufferBaseAddr,(unsigned long)mpIspVirRegAddrPA[0]);
            ...
            for (int i = 0; i < mTotalCQNum; i++) {
                ...
                LOG_INF("cq:[%d],virtIspAddr:virt[0x%lx]/phy[0x%lx]",i,(unsigned long)mpIspVirRegAddrVA[i], (unsigned long)mpIspVirRegAddrPA[i] );
            }
            ...
            LOG_DBG("cq(0x%x)(0x%x)(0x%lx)(0x%lx)(0x%lx)(0x%lx)",mIspCQDescSize,m_ispCQDescBufInfo.size, \
                                                            (unsigned long)m_ispCQDescBufInfo.virtAddr,(unsigned long)m_ispCQDescBufInfo.phyAddr, \
                                                            (unsigned long)mpIspCQDescBufferVirt,(unsigned long)mpIspCQDescBufferPhy);
            #endif  // __PMEM_ONLY__
            ...
            for (int i = 0; i < mTotalCQNum; i++)
            {
                ...
                LOG_INF("cq:[%d],mpIspCQDescriptor:Virt[0x%lx]/Phy[0x%lx],size/num(%lu/%d/%lu)",i,(unsigned long)mpIspCQDescriptorVirt[i],(unsigned long)mpIspCQDescriptorPhy[i],(unsigned long)sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT),CAM_MODULE_MAX,(unsigned long)sizeof(mIspCQDescInit));
            }
            ...
            if (sizeof(CQ_RTBC_RING_ST_FRMB) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data too large(%lu)>(%lu)",(unsigned long)sizeof(CQ_RTBC_RING_ST_FRMB),(unsigned long)sizeof(mIspCQDescInit));
                   return MFALSE;
            }
...
MBOOL IspDrvImp::dumpP2DebugLog(IspDumpDbgLogP2Package* pP2Package)
{
    ...
    LOG_INF("pP2Package(0x%lx)",(unsigned long)pP2Package);
...
MBOOL IspDrvImp::dumpCQTable(ISP_DRV_CQ_ENUM cq, MINT32 burstQIdx,MUINT32 dupCqIdx, ISP_DRV_CQ_CMD_DESC_STRUCT*  cqDesVa, MUINT32* cqVirVa)
{
    ...
    LOG_INF("+,realCQIdx(%d),cq(%d),burstQIdx(%d),dupCqIdx(%d),cqDesVa(0x%lx),cqVirVa(0x%lx)",realCQIdx,cq,burstQIdx,dupCqIdx,(unsigned long)cqDesVa,(unsigned long)cqVirVa);
    ...
    if(cqDesVa==0 || cqVirVa==0){
        ...
        LOG_INF("ISP_CQ_PA(0x%lx)",(unsigned long)mpIspCQDescriptorPhy[realCQIdx]);
...
MBOOL IspDrvImp::waitIrq(ISP_DRV_WAIT_IRQ_STRUCT* pwaitIrq)
{
    ...
    /* These comparison are always false
    ...
    MINT32                  caller)
{
    ...
    switch(userEnum)
    {
        ...
        case ISP_DRV_USER_FD:
            break;
        default:
            LOG_ERR("data[0x%lx] cn[0x%x] only ispDrv/isp_function/seninf have authorization to directly write hw registers\n",(unsigned long)pRegIo,Count);
            return MFALSE;
    }
    switch(caller)
    {
...
MBOOL IspDrvImp::kRefCntCtrl(ISP_REF_CNT_CTRL_STRUCT_FRMB* pCtrl)
{
    MINT32 Ret;
    //
    LOG_DBG("(%d)(%d)(0x%lx)",pCtrl->ctrl,pCtrl->id,(unsigned long)pCtrl->data_ptr);
    //
...
MBOOL IspDrvImp::rtBufCtrl(void *pBuf_ctrl)
{
   MINT32 Ret;
    ...
    if ( ISP_RT_BUF_CTRL_MAX_FRMB == pbuf_ctrl->ctrl ) {
        LOG_DBG("[rtbc]cq(%d),vIspV/P(0x%lx,0x%lx),descV/P(0x%lx,0x%lx),mmap(0x%lx)", \
            pbuf_ctrl->buf_id, \
            (unsigned long)IspDrv::mpIspVirRegAddrVA[pbuf_ctrl->buf_id], \
            (unsigned long)IspDrv::mpIspVirRegAddrPA[pbuf_ctrl->buf_id], \
            (unsigned long)IspDrv::mpIspCQDescriptorVirt[pbuf_ctrl->buf_id], \
            (unsigned long)IspDrv::mpIspCQDescriptorPhy[pbuf_ctrl->buf_id], \
            (unsigned long)this->getMMapRegAddr());
        return MTRUE;
    }
...
MBOOL IspDrvImp::updateCq0bRingBuf(void *pOBval)
{
 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (unsigned long)(va) - (unsigned long)(bva) ) + (unsigned long)(bpa) )
    ...
    LOG_DBG("[rtbc]va(0x%lx),pa(0x%lx)", (unsigned long)pcqrtbcring_va, (unsigned long)pcqrtbcring_pa);
    //
...
MBOOL IspDrvImp::_updateEnqCqRingBuf(void *pBuf_ctrl)
{
 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (unsigned long)(va) - (unsigned long)(bva) ) + (unsigned long)(bpa) )
    ...
    if ( 0 == (cam_tg_vf & 0x01) ) {
        //
        LOG_DBG("[rtbc]va(0x%lx),pa(0x%lx),ctrl(%d),dma(%d),PA(0x%x),o_size(%lu),2o_size(%lu)", \
                (unsigned long)pcqrtbcring_va, \
                (unsigned long)pcqrtbcring_pa, \
                pbuf_ctrl->ctrl, \
                pbuf_ctrl->buf_id, \
                pbuf_info->base_pAddr, \
                (unsigned long)pcqrtbcring_va->imgo_ring_size_frmb,\
                (unsigned long)pcqrtbcring_va->img2o_ring_size_frmb);
        ...
        //
        LOG_DBG("[rtbc]imgo_sz(%lu),img2o_sz(%lu), BufIdx=0x%x",(unsigned long)pcqrtbcring_va->imgo_ring_size_frmb,(unsigned long)pcqrtbcring_va->img2o_ring_size_frmb,pbuf_info->bufIdx);
        //
        {
            int *pdata = (int*)&pcqrtbcring_va->rtbc_ring_frmb[0];
            for ( i=0 ; i<(sizeof(CQ_RING_CMD_ST_FRMB)>>2)*size ; i++ ) {
                LOG_DBG("[rtbc][des] addr:(0x%lx) val:(0x%08x)",(unsigned long)(pdata+i),pdata[i]);
            }
        }
    }
    //VF_EN==1
        ...
        if ( pex_buf_info ) {
            ...
            if ( _imgo_ == pbuf_ctrl->buf_id) {
                ...
                if (!findTarget) {
                   LOG_ERR("[rtbc]error exIMG2O-Fail. Bufidx(0x%x/0x%x) old/new img2o buffer(0x%lx/0x%x)",\
                       pbuf_info->bufIdx,pex_buf_info->bufIdx,(unsigned long)(pcqrtbcring_va->rtbc_ring_frmb[i].cq_rtbc_frmb.img2o_base_pAddr_frmb),pex_buf_info->base_pAddr);
                   for ( i=0 ; i<pcqrtbcring_va->img2o_ring_size_frmb ; i++ ) {
    ...
    switch( pbuf_ctrl->ctrl ) {
        ...
        case ISP_RT_BUF_CTRL_CLEAR_FRMB:
            ...
            if (sizeof(CQ_RTBC_RING_ST_FRMB) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data exceed buffer size(%lu)>(%lu)",(unsigned long)sizeof(CQ_RTBC_RING_ST_FRMB),(unsigned long)sizeof(mIspCQDescInit));
            }
            break;
        default:
...
)
{
    ...
    if(path == eTuningQueAccessPath_featureio){
        ...
        LOG_DBG("p2Cq(%d),currQueueIdx(%d),pTuningQue(0x%lx)",p2Cq,currQueueIdx,(unsigned long)mTuningQueInf[p2Cq][currQueueIdx].pTuningQue);
...
#else
                searchTar = ((MUINT32)(mTuningQueIdx[p2Cq].curWriteIdx-1)>=(k+1))?(k+1):(mTuningQueIdx[p2Cq].curWriteIdx-1);
...
{
    LOG_DBG("cq: %d. ispVirRegAddr: 0x%lx.", cq, (unsigned long)ispVirRegAddr);
...
{
    LOG_VRB("mpIspVirRegBuffer(0x%lx)",(unsigned long)mpIspVirRegBuffer);
    if(mpIspVirRegBuffer != NULL)
    {
