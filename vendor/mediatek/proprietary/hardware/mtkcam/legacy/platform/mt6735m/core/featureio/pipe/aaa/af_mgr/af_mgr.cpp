...
MRESULT AfMgr::init(MINT32 i4SensorIdx, MINT32 isInitMCU)
{
    ...
    property_get("vendor.debug.afwin.w", value, "0");
    m_bDebugAFwinW = atoi(value);
    property_get("vendor.debug.afwin.h", value, "0");
...
MINT32 AfMgr::CCTOPAFCaliOperation(MVOID *a_pAFCaliData, MUINT32 *a_pOutLen)
{
    ...
    for (MINT32 i=0; i<pAFCaliData->i4Num; i++)
        MY_LOG("[AFCaliData] Vlu %d = %" PRIi64 "\n", i, pAFCaliData->i8Vlu[i]);
...
{
    if(m_i4AFCamMode != a_eCamMode)
    {
        m_i4AFCamMode = a_eCamMode;
	    MY_LOG("[setCamMode]%d\n", a_eCamMode);
	    m_tcaf_mode=0;
	    m_sAFInput.i4IsVDO= FALSE;

        m_bIsEngMode = (a_eCamMode == eAppMode_EngMode) ? MTRUE : MFALSE;
...
{
    MY_LOG_IF(m_bDebugEnable, "[setZoomWinInfo][w]%d[h]%d[x]%d[y]%d\n", u4Width, u4Height, u4XOffset, u4YOffset);
...
MRESULT AfMgr::setFDInfo(MVOID* a_sFaces)
{
    ...
    if (m_NVRAM_LENS.rAFNVRAM.i4FDWinPercent > 150)
    {
        m_NVRAM_LENS.rAFNVRAM.i4FDWinPercent = 150;
    }
    ...
    MY_LOG_IF(m_FDArea.i4Count > 0, "[setFDInfo]cnt:%d, X:%d Y:%d W:%d H:%d", m_FDArea.i4Count, m_FDArea.sRect[0].i4X, m_FDArea.sRect[0].i4Y, m_FDArea.sRect[0].i4W, m_FDArea.sRect[0].i4H);
...
MRESULT AfMgr::setOTInfo(MVOID* a_sOT)
{
    ...
    if (m_NVRAM_LENS.rAFNVRAM.i4FDWinPercent > 100)
    {
        m_NVRAM_LENS.rAFNVRAM.i4FDWinPercent = 100;
    }
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if( (m_sAFInput.i4IsVDO!=TRUE) || (m_sAFOutput.i4IsAFDone==FALSE ))
    {
        ...
        property_get("vendor.debug.af_ois.disable", value, "0");
    ...
    {
        MY_LOG_IF((m_bDebugEnable&1),"SensorEventAF gAcceValid=%d gPreAcceTS=%" PRIu64 " gAcceTS=%" PRIu64 " gAcceInfo[]=(%d %d %d)\n"
                  , gListenEnAcce, gPreAcceTS, gAcceTS, gAcceInfo[0], gAcceInfo[1], gAcceInfo[2]);
        MY_LOG_IF((m_bDebugEnable&1),"SensorEventAF gGyroValid=%d gPreGyroTS=%" PRIu64 " gGyroTS=%" PRIu64 " gGyroInfo[]=(%d %d %d)\n"
...
//            StereoHal::getInstance()->STEREODepthAFQuery();
    ...
    if (m_pMcuDrv)
    {
        MY_LOG_IF(m_bDebugEnable, "[AFStatH]%" PRIi64 " [AFStatV]%" PRIi64 " [moveMCUTG] %d", m_sAFInput.sAFStat.i8Stat24, m_sAFInput.sAFStat.i8StatV,m_sAFOutput.i4AFPos);
    ...
    if (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC || m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC_VIDEO)
    {
        if (m_sAFOutput.sAFArea.sRect[0].i4Info != m_i4AFPreStatus)
        {
            if (m_pAFCallBack)
            {
                Mutex::Autolock lock(m_Lock_cbq);

                // pl v2.0 ; debug and simulation
                char value[PROPERTY_VALUE_MAX] = {'\0'};
                property_get("vendor.debug.PLWFile.enable", value, "0");
                ...
                if (m_sAFOutput.sAFArea.sRect[0].i4Info < 3)
                {
                    ...
                    //m_pAFCallBack->doNotifyCb(I3ACallBack::eID_NOTIFY_AF_MOVING, , 0, 0);
                    CB_INFO_T cb_info( I3ACallBack::eID_NOTIFY_AF_MOVING, !m_sAFOutput.i4IsAFDone, 0, 0, m_sAFOutput.sAFArea, sizeof(AF_AREA_T));
                    m_cbq.push_back( cb_info);

                    MY_LOG("%s push to cbqueue (eID_NOTIFY_AF_MOVING) IsFocused(%d) AFState(%d->%d) cbqueue size(%d)",
                           __FUNCTION__,
                           m_sAFOutput.i4IsFocused,
                           m_i4AFPreStatus,
                           m_sAFOutput.sAFArea.sRect[0].i4Info,
                           m_cbq.size());
                }

                //m_pAFCallBack->doDataCb(I3ACallBack::eID_DATA_AF_FOCUSED, &m_sAFOutput.sAFArea, sizeof(m_sAFOutput.sAFArea));
                CB_INFO_T cb_info( I3ACallBack::eID_DATA_AF_FOCUSED, 0, 0, 0, m_sAFOutput.sAFArea, sizeof(AF_AREA_T));
                m_cbq.push_back( cb_info);
                MY_LOG("%s push to cbqueue (eID_DATA_AF_FOCUSED) IsFocused(%d) AFState(%d->%d) cbqueue size(%d)",
                       __FUNCTION__,
                       m_sAFOutput.i4IsFocused,
                       m_i4AFPreStatus,
                       m_sAFOutput.sAFArea.sRect[0].i4Info,
        ...
        if( m_i4AutoFocuscb)
        {
            /* Receive autofocus command in continuous mode*/

            if( m_sAFOutput.i4IsAFDone)
            {
                Mutex::Autolock lock(m_Lock_cbq);

                CB_INFO_T cb_info( I3ACallBack::eID_NOTIFY_AF_FOCUSED, m_sAFOutput.i4IsFocused, 0, 0, m_sAFOutput.sAFArea, sizeof(AF_AREA_T));
                m_cbq.push_back( cb_info);
                MY_LOG("%s push to cbqueue (eID_NOTIFY_AF_FOCUSED) IsFocused(%d) AFState(%d->%d) cbqueue size(%d)",
                       __FUNCTION__,
                       m_sAFOutput.i4IsFocused,
                       m_i4AFPreStatus,
                       m_sAFOutput.sAFArea.sRect[0].i4Info,
            ...
            else
            {
                MY_LOG_IF( m_bDebugEnable,
                           "%s Need callback message eID_NOTIFY_AF_FOCUSED when focusing done, IsAFDone(%d)",
                           __FUNCTION__,
...
MVOID AfMgr::setFlkWinConfig()
{
    ...
    else
    {
        ...
        ISP_WRITE_REG(m_pIspReg, CAM_AF_SIZE, (unsigned long)((i4W & 0x3FF) + ((i4H&0x3FF)<<16)) );

        if (m_sAFOutput.i4FDDetect==0)
        {
            // 13 bits (8192x8192) - double buffer, "must even position"
            ISP_WRITE_REG(m_pIspReg, CAM_AF_WIN_E, (unsigned long)((Boundary(FL_WIN_POS_MIN, i4X + i4W*2, i4WOri - FL_WIN_POS_MIN) & 0x1FFE)
                          + ((Boundary(FL_WIN_POS_MIN, i4Y + i4H*2, i4HOri - FL_WIN_POS_MIN)&0x1FFE)<<16)) );

            // 12 bits (4096x4096) - double buffer
            ISP_WRITE_REG(m_pIspReg, CAM_AF_SIZE_E, (unsigned long)((Boundary(FL_WIN_SIZE_MIN, i4W*2, FL_WIN_SIZE_MAX) & 0xFFE)
        ...
        else
        {
            ...
            // 13 bits (8192x8192) - double buffer, "must even position"
            ISP_WRITE_REG(m_pIspReg, CAM_AF_WIN_E, (unsigned long)((i4XE & 0x1FFE) + ((i4YE&0x1FFE)<<16)));

            // 12 bits (4096x4096) - double buffer
            ISP_WRITE_REG(m_pIspReg, CAM_AF_SIZE_E, (unsigned long)((Boundary(FL_WIN_SIZE_MIN, (m_FDArea.sRect[0].i4W>>1)<<1, i4WOri - i4XE - FL_WIN_POS_MIN) & 0xFFE)
...
MVOID AfMgr::setAFWinConfig(AF_AREA_T a_sAFArea)
{
    ...
    {
        // 13 bits (8192x8192) - double buffer, "must even position"
        ISP_WRITE_REG(m_pIspReg, CAM_AF_WIN_E, (unsigned long)((Boundary(FL_WIN_POS_MIN, i4X, i4WOri - FL_WIN_POS_MIN) & 0x1FFE)
                      + ((Boundary(FL_WIN_POS_MIN, i4Y, i4HOri - FL_WIN_POS_MIN)&0x1FFE)<<16)));

        // 12 bits (4096x4096) - double buffer
        ISP_WRITE_REG(m_pIspReg, CAM_AF_SIZE_E, (unsigned long)((Boundary(FL_WIN_SIZE_MIN, i4W, FL_WIN_SIZE_MAX) & 0xFFE)
                      + ((Boundary(FL_WIN_SIZE_MIN, i4H, FL_WIN_SIZE_MAX)&0xFFE)<<16)) );
    }
...
MVOID AfMgr::setAF_IN_HSIZE()
{
    ...
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_IN_SIZE, AF_IN_HSIZE, (unsigned long)Boundary(0, m_i4AF_in_Hsize, 8191));

    MY_LOG_IF(m_bDebugEnable, "[setAF_IN_HSIZE][SensorDev]%d [Pxl]%d [lin]%d [H]%d [V]%d\n",
              m_i4CurrSensorDev,
              i4Pxl_E - i4Pxl_S,
              i4lin_E - i4lin_S,
              m_i4AF_in_Hsize,
...
MVOID AfMgr::setAFWinTH(AF_CONFIG_T a_sAFConfig)
{
    ...
    if ((m_i4AF_TH[0] != a_sAFConfig.AF_TH[0]) || (m_i4AF_TH[1] != a_sAFConfig.AF_TH[1]))
    {
        ...
        ISP_WRITE_REG(m_pIspReg, CAM_AF_TH, (unsigned long)(0x01000000+(Boundary(0, a_sAFConfig.AF_TH[1], 255)<<16)+ Boundary(0, a_sAFConfig.AF_TH[0], 255)));
    ...
    //if (m_i4AF_THEX != a_sAFConfig.AF_TH[0])
    {
        ...
        ISP_WRITE_REG(m_pIspReg, CAM_AF_TH_E, (unsigned long)(Boundary(0, a_sAFConfig.AF_TH[0]+1, 255)));
...
MVOID AfMgr::setGMR(AF_CONFIG_T a_sAFConfig)
{
    ...
    ISP_WRITE_REG(m_pIspReg, CAM_SGG_GMR, (unsigned long)((Boundary(0, a_sAFConfig.i4SGG_GMR7, 255)<<16)+ (Boundary(0, a_sAFConfig.i4SGG_GMR6, 255)<<8) + Boundary(0, a_sAFConfig.i4SGG_GMR5, 255)));
...
MVOID AfMgr::setAFConfig(AF_CONFIG_T a_sAFConfig)
{
    ...
    // Q0.7.4
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_SGG_PGN, SGG_GAIN, (unsigned long)Boundary(0, a_sAFConfig.i4SGG_GAIN, 2047));
    // 8 bits
    //ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_SGG_GMR, SGG_GMR1, Boundary(0, a_sAFConfig.i4SGG_GMR1, 255));    // 512
    //ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_SGG_GMR, SGG_GMR2, Boundary(0, a_sAFConfig.i4SGG_GMR2, 255));    // 1024
    //ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_SGG_GMR, SGG_GMR3, Boundary(0, a_sAFConfig.i4SGG_GMR3, 255));    // 2048
    ...
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_DECI_1, (unsigned long)Boundary(0, a_sAFConfig.AF_DECI_1, 2));  // 1: 1/2 (double buffer)
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_ZIGZAG, (unsigned long)Boundary(0, a_sAFConfig.AF_ZIGZAG, 1));  // 1: shift
    if ((i4Pix_Id == 1) || (i4Pix_Id == 2))
    {
        ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_ODD, (unsigned long)(1));  // 1: odd
    }
    else
    {
        ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_ODD, (unsigned long)(0));  //0: even
    }

    //  MY_LOG("[Filter-init]%d %d %d %d", a_sAFConfig.AF_FILT1[4],a_sAFConfig.AF_FILT1[5],a_sAFConfig.AF_FILT1[6],a_sAFConfig.AF_FILT1[7]);

    ISP_WRITE_REG(m_pIspReg, CAM_AF_FILT1_P14, (unsigned long)((a_sAFConfig.AF_FILT1[3]<<24) + (a_sAFConfig.AF_FILT1[2]<<16) + (a_sAFConfig.AF_FILT1[1]<<8) + a_sAFConfig.AF_FILT1[0]));
    ISP_WRITE_REG(m_pIspReg, CAM_AF_FILT1_P58, (unsigned long)((a_sAFConfig.AF_FILT1[7]<<24) + (a_sAFConfig.AF_FILT1[6]<<16) + (a_sAFConfig.AF_FILT1[5]<<8) + a_sAFConfig.AF_FILT1[4]));
    ISP_WRITE_REG(m_pIspReg, CAM_AF_FILT1_P912, (unsigned long)((a_sAFConfig.AF_FILT1[11]<<24) + (a_sAFConfig.AF_FILT1[10]<<16) + (a_sAFConfig.AF_FILT1[9]<<8) + a_sAFConfig.AF_FILT1[8]));
    ISP_WRITE_REG(m_pIspReg, CAM_AF_VFILT_X01,(unsigned long)((((a_sAFConfig.AF_FILT2[1]&0x7FF)<<16) + (a_sAFConfig.AF_FILT2[0]&0x7FF))));
    ISP_WRITE_REG(m_pIspReg, CAM_AF_VFILT_X23,(unsigned long)((((a_sAFConfig.AF_FILT2[3]&0x7FF)<<16) + (a_sAFConfig.AF_FILT2[2]&0x7FF))));
}
...
AF_STAT_T AfMgr::Trans4WintoOneStat(MVOID *pAFStatBuf)
{
    ...
    if ((m_sAFOutput.i4FDDetect) && (m_FDArea.i4Count))
    {
        ...
        MY_LOG("FL FV %" PRIi64 "", statsH[MAX_AF_HW_WIN-1]);
    }
    else
    {
    ...
    MINT32 k=0;
    MY_LOG_IF((m_bDebugEnable&4),"AvF:    %" PRIi64 "    %" PRIi64 "    %" PRIi64 "    %" PRIi64 "    %" PRIi64 "    %" PRIi64 "",
...
AF_STAT_T AfMgr::TransAFtoOneStat(MVOID *pAFStatBuf)
{
    ...
    MY_LOG_IF((m_bDebugEnable&1),"[AFv2] [6x6] %" PRIi64 " %" PRIi64 " ", sAFStat.i8Stat24, sAFStat.i8StatV);
...
{
    MY_LOG_IF( m_bDebugEnable, "%s dev(%d) +", __FUNCTION__, m_i4CurrSensorDev);
    if(m_pAFCallBack!=NULL && m_cbq.size())
    {
        MUINT32 i=0;
        while(1)
        {
            ...
            switch( cb_info.msgType)
            {
            case I3ACallBack::eID_NOTIFY_AF_MOVING:
            case I3ACallBack::eID_NOTIFY_AF_FOCUSED:
                MY_LOG("%s NotifyCb msg(%d), ext1(%d), ext2(%d), ext3(%d) info(%d)", __FUNCTION__, cb_info.msgType, cb_info.ext1, cb_info.ext2, cb_info.ext3, cb_info.da.sRect[0].i4Info);
                m_pAFCallBack->doNotifyCb( cb_info.msgType, cb_info.ext1, cb_info.ext2, cb_info.ext3);

                break;
            case I3ACallBack::eID_DATA_AF_FOCUSED:
                MY_LOG("%s DataCb msg(%d), cnt(%d), sz(%d)", __FUNCTION__, cb_info.msgType, cb_info.da.i4Count, cb_info.dasz);
                m_pAFCallBack->doDataCb( cb_info.msgType, &cb_info.da, cb_info.dasz);
