...
cam_isp_cfa_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    if(imageioPackage.isApplyTuning){
        ...
        ISP_FUNC_DBG("cam_isp_cfa_cfg m_pIspDrv(0x%lx)\n",(unsigned long)m_pP2IspDrv);
...
cam_isp_g2g_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    ISP_FUNC_DBG("+,m_pP2IspDrv(0x%lx),cq(%d),subMode(%d),isApplyTuning(0x%08x)",(unsigned long)m_pP2IspDrv,imageioPackage.cq, imageioPackage.subMode,imageioPackage.isApplyTuning);
    if(imageioPackage.isApplyTuning){
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ...
        if(tuningPackage.eTuningCqFunc1==CAM_CDP_SL2_FEATUREIO){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            if(imageioPackage.hrzRatio){
                ...
                ISP_FUNC_ERR("[Error]hrzRatio=%d",(MINT32)imageioPackage.hrzRatio);
            }
        }
        //
    }
...
cam_isp_c42_cfg(void) {
    ISP_FUNC_DBG("cam_isp_c42_cfg m_pIspDrv(0x%lx)\n",(unsigned long)m_pIspDrv);
...
//    memset (&rawCentralCtrl, 0, sizeof(ISP_RAW_CENTRAL_CTRL_INFO));
    ...
    ISP_FUNC_DBG("[m_pIMemDrv]:0x%lx ",(unsigned long)m_pIMemDrv);
    if ( NULL == m_pIMemDrv ) {
    ...
    ISP_FUNC_INF("[m_pPhyIspDrv]:0x%lx ",(unsigned long)m_pPhyIspDrv);
    if (!m_pPhyIspDrv) {
    ...
    ISP_FUNC_DBG("[m_pP2CrzDrv]:0x%lx ",(unsigned long)m_pP2CdrzDrv);
    ...
    ISP_FUNC_INF("[m_pIspEventThread]:0x%lx ",(unsigned long)m_pIspEventThread);
...
int ret = 0;
    ...
    */
    ...
    isp driver
    =============================================*/
    ISP_FUNC_DBG("m_pIspDrv(0x%lx) m_pPhyIspDrv_bak(0x%lx)\n",(unsigned long)m_pIspDrv,(unsigned long)m_pPhyIspDrv_bak);
    if (m_pPhyIspDrv_bak) {
...
ispDrvSwitch2Virtual(MINT32 cq,MINT32 p1CqD,MINT32 burstQIdx,MINT32 p2DupCqIdx)
{
    ...
    // Warning: Unused Input Parameter
    if(p1CqD){}
...
MINT32 cq,MINT32 burstQIdx,MINT32 dupCqIdx,MINT32 isP2IspOnly
    ...
    switch(cq) {
        case NSIspDrv_FrmB::CAM_ISP_CQ0:
            ...
            ISP_FUNC_DBG("CQ0 addr:0x%lx(%d,%d)\n",(unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr(cq,burstQIdx,dupCqIdx),burstQIdx,dupCqIdx);
        ...
        case NSIspDrv_FrmB::CAM_ISP_CQ0C:
            ISP_FUNC_DBG("CAM_ISP_CQ0C(0x%lx),m_trigSsrc(%d)",(unsigned long)m_pPhyIspDrv_bak->getCQDescBufPhyAddr(cq,burstQIdx,dupCqIdx),this->m_trigSsrc);
...
{
    NSIspDrv_FrmB::IspDrv* _targetVirDrv = NULL;
    ISP_FUNC_DBG("+,m_pPhyIspDrv(0x%lx) ",(unsigned long)m_pPhyIspDrv_bak);
    //
...
controlBurstQueBuffer(EIspControlBufferCmd cmd, MUINT32 burstQueueNum)
{
    ...
    if(eIspControlBufferCmd_Alloc == cmd) {
        ...
        for(k=0;k<burstQueueNum;k++){
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    #if defined(__PMEM_ONLY__)
                    gpMdpMgrCfgData_FrmB[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTableVa = (MUINT32*)pmem_alloc_sync(MAX_ISP_TILE_TDR_HEX_NUM, &tpipeBufInfo[k][i][j].memID);
                    gpMdpMgrCfgData_FrmB[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTablePa = (MUINT32)pmem_get_phys(tpipeBufInfo[k][i][j].memID);
                    #else   // Not PMEM.
                    //
                    tpipeBufInfo[k][i][j].size = MAX_ISP_TILE_TDR_HEX_NUM;
                    ...
                    ISP_FUNC_INF("[Tpipe]i(%d),j(%d)va(0x%lx),pa(0x%08x)",i,j,(unsigned long)gpMdpMgrCfgData_FrmB[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTableVa,gpMdpMgrCfgData_FrmB[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
                    #endif
                }
            }
        }
        ...
        for(int cq=0;cq<totalCQNum;cq++)
        {
            ...
            ISP_FUNC_DBG("[m_pVirtIspDrv[cq%d]]:0x%lx ",cq,(unsigned long)m_pVirtIspDrv[cq]);
            if (!m_pVirtIspDrv[cq]) {
            ...
            ISP_FUNC_DBG("[m_pVirtIspReg[cq%d]]:0x%lx ",cq,(unsigned long)m_pVirtIspReg[cq]);
            ...
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    ...
                    if(tpipeBufInfo[k][i][j].memID >= 0) {
                        pmem_free((MUINT8*)gpMdpMgrCfgData_FrmB[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTableVa,(MAX_ISP_TILE_TDR_HEX_NUM, tpipeBufInfo[k][i][j].memID);
...
int DMAI_B::_config( void )
{
    ...
    ISP_FUNC_DBG("DMAI(%d),[%s]:cq(%d),pa(0x%lx),ofst(0x%lx),size(%d,%d,%d,%d), pixel_byte(%d),cropW(%d,%d,%d,%d),format(0x%x)", \
            (MINT32)this->id(), \
            this->name_Str(), \
            this->CQ, \
            (unsigned long)this->dma_cfg.memBuf.base_pAddr, \
            (unsigned long)this->dma_cfg.memBuf.ofst_addr,\
            (MINT32)this->dma_cfg.size.w, \
            (MINT32)this->dma_cfg.size.h, \
            (MINT32)this->dma_cfg.size.stride, (MINT32)this->dma_cfg.size.xsize, \
            this->dma_cfg.pixel_byte, \
            this->dma_cfg.crop.x, \
            this->dma_cfg.crop.y,\
            (MINT32)this->dma_cfg.crop.w,\
            (MINT32)this->dma_cfg.crop.h,\
            this->dma_cfg.format);
    ...
    _targetVirDrv = (NSIspDrv_FrmB::IspDrv*)this->m_pIspDrvShell->ispDrvGetCQIspDrv(this->CQ,this->burstQueIdx,this->dupCqIdx);
     switch(this->id())
     {
        case ISP_DMA_TDRI:
            ...
            ISP_FUNC_DBG("CAM_TDRI_BASE_ADDR = 0x%lx",(unsigned long)this->dma_cfg.memBuf.base_pAddr);
            break;
        case ISP_DMA_CQI:
            ...
            ISP_FUNC_DBG("[afterCQSwitch]m_pP2IspDrv(0x%lx)",(unsigned long)m_pP2IspDrv);
           break;
        case ISP_DMA_IMGI:
...
int DMAI_B::_disable( void )
{
    ...
    ISP_FUNC_DBG("[id:0x%08d] ",(MINT32)this->id());
...
unsigned int cq_module_id = 0xFF;
    ...
    ISP_FUNC_DBG("_,[id:0X%08X] ",(MUINT32)this->id());
...
int DMAO_B::_config( void )
{
    ...
    NSIspDrv_FrmB::IspDrv* _targetVirDrv = NULL;
    MUINT32 _targetDupQIdx = 0;
    ISP_FUNC_DBG("DMAO(%d),[%s]:cq/dup/burst(%d/%d/%d),pa(0x%lx),ofst(0x%lx),size(%d,%d,%d,%d), pixel_byte(%d),cropW(%d,%d,%d,%d),format(0x%x),fps(%d)", \
            (MINT32)this->id(), \
            this->name_Str(), \
            this->CQ, \
            this->dupCqIdx,\
    ...
    switch(this->id())
    {
        ...
        case ISP_DMA_IMGO:
            ...
            ISP_FUNC_DBG("DAMO-drv(0x%lx), x-size(%d),y(%d),stride(%d), baseaddr(0x%lx)",(unsigned long)_targetVirDrv,(MINT32)this->dma_cfg.size.xsize, (MINT32)this->dma_cfg.size.h, (MINT32)this->dma_cfg.size.stride, (unsigned long)this->dma_cfg.memBuf.base_pAddr);
...
int DMAO_B::_write2CQ(void)
{
    ...
    ISP_FUNC_DBG("+,");
    ISP_FUNC_DBG("[id:0X%08X],CQ/dup/burst(%d/%d/%d)",(MUINT32)this->id(),this->CQ,this->dupCqIdx,this->burstQueIdx);
...
int ISP_TOP_CTRL::_config( void )
{
    ...
    ISP_FUNC_DBG("m_pIspDrv(0x%lx) m_pPhyIspDrv(0x%lx),_targetVirDrv(0x%lx),m_pP2IspDrv(0x%lx)",(unsigned long)m_pIspDrv,(unsigned long)m_pPhyIspDrv,(unsigned long)_targetVirDrv,(unsigned long)m_pP2IspDrv);
...
{
    ISP_FUNC_DBG("path(%d),start(%d),m_pIspReg(0x%lx),m_pPhyIspReg(0x%lx),scenario(%d)\n",
                this->path,*((MUINT32*)pParam),(unsigned long)this->m_pIspReg,(unsigned long)this->m_pPhyIspReg,this->fmt_sel.bit_field.scenario);
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    MUINT32 tgStatus;
    Mutex   *_localVar;
    ISP_FUNC_DBG("ISP_TOP_CTRL::_disable:E 0x%lx 0x%lx\n",(unsigned long)this->m_pIspDrvShell->m_pIspDrv,(unsigned long)this->m_pIspDrvShell->m_pPhyIspDrv_bak);
    // lock for pass1 and pass2 respectively
    if(ISP_PASS1 == this->path)
    {
        _localVar = &this->m_pIspDrvShell->gPass1Lock;
    ...
    if (ISP_PASS1 == this->path)
    {
        ...
        if(ISP_PASS1 == this->path){
            ...
            ISP_FUNC_ERR("path error 0x%x\n",this->path);

        while(_clrPort--){
            ISP_FUNC_DBG("[TopCtrl::Disable][before]rt_dma(%d),size:hw(%lu)",_dma[_clrPort], (unsigned long)ISP_BUF_CTRL::m_hwbufL[_dma[_clrPort]].bufInfoList.size());
            buf_ctrl.ctrl = ISP_RT_BUF_CTRL_CLEAR_FRMB;
            ...
            ISP_FUNC_DBG("[TopCtrl::Disable][after]rt_dma(%d),size:hw(%lu) ",_dma[_clrPort], (unsigned long)ISP_BUF_CTRL::m_hwbufL[_dma[_clrPort]].bufInfoList.size());
        }
    }
    else if (ISP_PASS2 == this->path)
    {
...
int ISP_RAW_PIPE::_config( void )
{
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%08x),pTuningBuf(0x%lx)",tuningUpdateBit,(unsigned long)tuningPackage.pTuningBuf);
        }
...
int ISP_RAW_PIPE::setP1TuneCfg(MUINT32 mNum, MINT32 senDev)
{
    ...
    ISP_FUNC_DBG("P1Dbg_RPG: senDev=%d, mNum=0x%x, UpdateBit=0x%x, EnP1=0x%x, EnP1Dma=0x%x, pBuf=0x%lx", senDev, mNum, cfg.tuningUpdateBit, cfg.tuningEnP1,cfg.tuningEnP1Dma, (unsigned long)cfg.pTuningBuf);

    if (!cfg.pTuningBuf) {
        ISP_FUNC_ERR(" pTuningBuf=0x%lx", (unsigned long)cfg.pTuningBuf);
        return -1;
    }
    ...
    if (ISP_PASS1 == this->path){
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Obc) {
            ...
            if (cfg.tuningEnP1 & CAM_CTL_EN1_OB_EN) {
               //Enable
               ISP_FUNC_DBG("Enable Obc, 0x%lx/0x%lx/%lu", (unsigned long)cqVirtReg, (unsigned long)cfg.pTuningBuf, (unsigned long)(outInfo.reg_num)*sizeof(UINT32));
               ...
               //Disable
               ISP_FUNC_DBG("Disable Obc, 0x%lx/0x%lx/%lu", (unsigned long)cqVirtReg, (unsigned long)cfg.pTuningBuf, (unsigned long)(outInfo.reg_num)*sizeof(UINT32));
...
int ISP_RGB_PIPE::_config( void )
{
    ...
    if(imageioPackage.isApplyTuning==MTRUE){
        ...
        ISP_FUNC_DBG("tuningUpdateBit(0x%08x),pTuningBuf(0x%lx),hrz_crop(%d,%d)", \
            tuningUpdateBit,(unsigned long)tuningPackage.pTuningBuf,imageioPackage.hrzCropX,imageioPackage.hrzCropY);
    }
    imageioPackage.enFgMode = 0;
    imageioPackage.finalEn1 = this->enable1;
    imageioPackage.finalEn2 = this->enable2;
...
int ISP_MDP_PIPE::_config( void )
{
    ...
    ISP_FUNC_DBG("ISP_MDP_PIPE,crop(x:%d,y:%d,w:%d,h:%d)", \
        this->src_crop.x, \
        this->src_crop.y, \
        (MINT32)this->src_crop.w, \
        (MINT32)this->src_crop.h);
    ...
    if(enPureMdpOut == MFALSE){
        ISP_FUNC_DBG("imgxo va(0x%lx),pa(0x%lx),size(%lu)",(unsigned long)this->imgxo_out.memBuf.base_vAddr,(unsigned long)this->imgxo_out.memBuf.base_pAddr,(unsigned long)this->imgxo_out.memBuf.size);
...
unsigned int *data_ptr = NULL;
    ...
    if ( this->conf_cdrz )
    {
        ISP_FUNC_DBG("data_ptr:0x%lx",(unsigned long)data_ptr);
    }
...
MINT32 ISP_BUF_CTRL::enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo, MINT32 enqueCq, MINT32 dupCqIdx,MBOOL bImdMode)
{
    ...
    if ( ISP_PASS1   == this->path ) {
        ...
        if( NULL != bufInfo.next ){
            ISP_QUE_DBG(rt_dma,"dma(%d),id(%d),size(0x%x),VA(0x%lx),PA(0x%lx),S/C(%d/%d),bufidx(%d),replace:new(0x%x),bufidx(%d),bImdMode(%d)",\
                        rt_dma,\
                        buf_info.memID,\
                        buf_info.size,\
                        (unsigned long)buf_info.virtAddr,\
                        (unsigned long)buf_info.phyAddr,\
                        buf_info.bufSecu,\
                        buf_info.bufCohe,\
                        bufInfo.mBufIdx,\
            ...
            ISP_QUE_DBG(rt_dma,"dma(%d),id(%d),size(0x%x),VA(0x%lx),PA(0x%lx),S/C(%d/%d),bufidx(%d),bImdMode(%d)",\
                        rt_dma,\
                        buf_info.memID,\
                        buf_info.size,\
                        (unsigned long)buf_info.virtAddr,\
        ...
        P1DeQCnt[rt_dma] += 1;
        if( (_imgo_ == rt_dma) || (_img2o_ == rt_dma) ){
            ISP_QUE_DBG(rt_dma,"imgo/img2o, vf=0x%x fbc: 0x%x/0x%x_0x%x/0x%x\n",vf,fbc[0],fbc[1],(MUINT32)readReg(0x000040f4),(MUINT32)readReg(0x000040f8));
        }
...
EIspRetStatus ISP_BUF_CTRL::dequeueHwBuf( MUINT32 dmaChannel, stISP_FILLED_BUF_LIST& bufList )
{
    ...
    if ( ISP_PASS1   == this->path) {
        ...
        if ( _imgo_ == rt_dma || \
            ...
            for(MUINT32 i=0;i<count;i++) {
                ISP_QUE_DBG(rt_dma,"i(%d),dma(%d),id(0x%x),size(0x%x),xsize(0x%x),VA(0x%x),PA(0x%x),count(%d),cur sof(%d),frm_cnt(%d),mag(0x%x)",
                    ...
                    (MUINT32)deque_buf.data[i].base_vAddr, \
        ...
        if(!P2_UT_BypassMDP_TPIPE)
               {
                    ...
                    if(isDequeMdp==MTRUE){
                        ...
                        if(isDump==MTRUE || MTRUE==function_DbgLogEnable_VERBOSE){
                            ...
                            //
                            ISP_FUNC_INF("dequeBurstQueIdx(%d),p2Cq(%d),dequeDupCqIdx(%d),tpipeVa(0x%lx),tpipePa(0x%x)",
                                this->dequeBurstQueIdx,p2Cq,this->dequeDupCqIdx,(unsigned long)gpMdpMgrCfgData_FrmB[this->dequeBurstQueIdx][p2Cq][this->dequeDupCqIdx].ispTpipeCfgInfo.drvinfo.tpipeTableVa,
        ...
        //
                ISP_FUNC_DBG("dma(%d),hwsize(%lu),p2Cq(%d),dequeDupCqIdx(%d),imgxoEnP2(0x%x),memID(0x%x),size(%lu),vaddr(0x%lx),paddr(0x%lx)",rt_dma,(unsigned long)m_p2HwBufL[p2Cq][this->dequeDupCqIdx][rt_dma].bufInfoList.size(),p2Cq,this->dequeDupCqIdx, \
...
int ISP_PASS1_CTRL::keepP1HrzCfg( IspHrzCfg const hrz_cfg )
{
    if ( ISP_PASS1 == hrz_cfg.pass1_path ) {
        ...
        ISP_FUNC_ERR("hrz keeper path err 0x%x",(MUINT32)hrz_cfg.pass1_path);
        return -1;
    }
    return 0;
}
...
int ISP_PASS1_CTRL::setP1HrzCfg( IspHrzCfg const hrz_cfg )
{
    ...
    ISP_FUNC_DBG("ISP_PASS1_CTRL::setP1HrzCfg E:path(%d),hrz_in_w/h(%d/%d),hrz_out_w/h(%d/%d),m_num(0x%x)", \
        (MINT32)hrz_cfg.pass1_path, \
        (MINT32)hrz_cfg.hrz_in_size.w, \
        (MINT32)hrz_cfg.hrz_in_size.h, \
        (MINT32)hrz_cfg.hrz_out_size.w, \
        (MINT32)hrz_cfg.hrz_out_size.h, \
        (MUINT32)hrz_cfg.m_num_0);

    ISP_FUNC_DBG("twin(%d),path(%d)",(MINT32)hrz_cfg.isTwinMode,(MINT32)hrz_cfg.pass1_path);
    ...
    if ( ISP_PASS1 == hrz_cfg.pass1_path ) {
        ...
        ISP_FUNC_ERR("Not support pass1 path(%d)",(MINT32)hrz_cfg.pass1_path);
    }
...
{
    ISP_FUNC_DBG("E: pIspDrv(0x%lx)",(unsigned long)pIspDrv);
    //
...
MVOID* IspEventThreadImp::ispEventThread(void *arg)
{
    ...
    ISP_FUNC_DBG("E: tid=%d m_pPhyIspDrv(0x%lx)", gettid(),(unsigned long)m_pPhyIspDrv);
