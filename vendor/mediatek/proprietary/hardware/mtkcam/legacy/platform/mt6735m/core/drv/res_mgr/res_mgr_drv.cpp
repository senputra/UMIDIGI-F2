...
triggerCtrl()
{
    ...
    mmdvfs_set(
        ...
        MMDVFS_SENSOR_FPS,          mBwcParams.sensorFps,
        MMDVFS_PARAMETER_EOF);
    MUINT32 bwHighSensorSize = 0;
    #if (MTKCAM_FAKE_SENSOR == 1)
    // real sensor is 13M, even crop sensor to 3m/5m/8m
    // so keep using high venmpll
    bwHighSensorSize = 2560*1920;
    LOG_MSG("use fake sensor, bwHighSensorSize=2560*1920");
    #else
    bwHighSensorSize = 3264*2448;
    LOG_MSG("bwHighSensorSize=3264*2448");
    #endif
    if(mBwcParams.sensorSize >= bwHighSensorSize)
    {
        LOG_MSG("Phigh");
