...
EAfState_T AfMgr::getAFState()
{
    ...
    MY_LOG( "[%s] State(%d -> %d), Mode(%d), SearchDone(%d) Cmd: setAFMode(%d), AutoFocus(%d), CancelAutoFocus(%d), ChangeSearchingState(%d), AuotoFocusInContiPicMode(%d) LockAFInContiMode(%d) AutoFocusInAutoMode(%d) TriggerAlgoInAutoMode(%d)",
            __FUNCTION__,
            m_eAFState,
            NewState,
            m_eCurrAFMode,
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if (m_pMcuDrv)
    {
        MY_LOG("[AFStatH]%lld [AFStatV]%lld [moveMCU] %d, [afdone] %d(%d), [algostate] %d ", m_sAFInput.sAFStat.i8Stat24, m_sAFInput.sAFStat.i8StatV,m_sAFOutput.i4AFPos, m_i4IsAFAlgoSearchDone, m_sAFOutput.i4IsAFDone, m_pIAfAlgo->getAFState());
...
MRESULT AfMgr::calBestShotValue(MVOID *pAFStatBuf)
{
    ...
    MY_LOG("[calBestShotValue] %" PRIi64 "", m_i8BSSVlu);
