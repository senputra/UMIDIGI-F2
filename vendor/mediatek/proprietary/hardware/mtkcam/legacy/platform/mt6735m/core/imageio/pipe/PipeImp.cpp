...
configCdpOutPort(PortInfo const* oImgInfo,CdpRotDMACfg &a_rotDma)
{
 ...
 //SL TEST_MDP        a_rotDma.size.h = oImgInfo->u4ImgWidth;
    ...
    a_rotDma.crop.h = 0;
    //
    PIPE_DBG("[a_rotDma]w(%d),h(%d),stride(pxl)(%d)",(MUINT32)a_rotDma.size.w,(MUINT32)a_rotDma.size.h,(MUINT32)a_rotDma.size.stride);
    PIPE_DBG("[a_rotDma]crop(%d,%d,%d,%d)_f(0x%x,0x%x)",a_rotDma.crop.x,a_rotDma.crop.y,(MUINT32)a_rotDma.crop.w,(MUINT32)a_rotDma.crop.h,a_rotDma.crop.floatX,a_rotDma.crop.floatY);
...
PipeImp::configDmaPort(PortInfo const* portInfo,IspDMACfg &a_dma,MUINT32 pixel_Byte,MUINT32 swap, MUINT32 isBypassOffset,EIMAGE_STRIDE planeNum)
{
    ...
    if ( a_dma.size.stride < a_dma.size.w ) {
        PIPE_ERR("[Error]:stride size(%d) < image width(%d) byte size",(MUINT32)a_dma.size.stride,(MUINT32)a_dma.size.w);
    }
    //
    ...
    //
    PIPE_DBG("w(%d),h(%d),stride(pxl)(%d,%d,%d),crop(%d,%d,%d,%d)_f(0x%x,0x%x),ofst_addr(0x%x),pb((0x%x)(+<<2))",(MUINT32)a_dma.size.w,(MUINT32)a_dma.size.h,portInfo->u4Stride[ESTRIDE_1ST_PLANE],portInfo->u4Stride[ESTRIDE_2ND_PLANE],portInfo->u4Stride[ESTRIDE_3RD_PLANE], \
                                                        a_dma.crop.x,a_dma.crop.y,(MUINT32)a_dma.crop.w,(MUINT32)a_dma.crop.h,\
