...
//#include <mtkcam/hal/sensor_hal.h> //Allen
...
//#include "buf_mgr.h"
//#include <mtkcam/drv/isp_drv.h> //Allen
...
getSensorWidthHeight(MINT32 i4SensorDev, SENSOR_RES_INFO_T* a_rSensorResolution)
{
    ...
    MY_LOG("[getSensorWidthHeight] Sensor id:%d Prv:%d %d Video:%d %d Cap:%d %d Video1:%d %d Video2:%d %d\n", i4SensorDev, a_rSensorResolution->u2SensorPreviewWidth,
...
{
    MY_LOG_IF(m_bDebugEnable, "setFlickerFrameRateActive en=%d",a_bFlickerFPSAvtive);
    MINT32 ret = S_AAA_SENSOR_MGR_OK;
...
set2ShutterControl(MINT32 i4SensorDev, MUINT32 a_u4LEExpTime, MUINT32 a_u4SEExpTime)
{
    ...
    MY_LOG_IF(m_bDebugEnable, "[setiVHDRParams] Id:%d Shutter:%d %d\n", i4SensorDev, a_u4LEExpTime, a_u4SEExpTime);
...
setSensorAWBGain(MINT32 i4SensorDev, strSensorAWBGain *strSensorAWBInfo)
{
    ...
    if(ret) {
        MY_ERR("Err CMD_SENSOR_SET_EXP_TIME, Sensor dev:%d GR:%d R:%d B:%d GB:%d\n", i4SensorDev, strSensorAWBInfo->u4GR,
        strSensorAWBInfo->u4R, strSensorAWBInfo->u4B,
        strSensorAWBInfo->u4GB);
    }
