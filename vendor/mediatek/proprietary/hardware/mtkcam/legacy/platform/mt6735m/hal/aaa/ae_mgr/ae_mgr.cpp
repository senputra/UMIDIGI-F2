...
strAEOutput rAEOutput;
    ...
    copyAEInfo2mgr(&m_rAEOutput.rPreviewMode, &rAEOutput);
    MY_LOG_IF(m_3ALogEnable,"[getPreviewParams3] Exp. time: %d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n",
...
AE_EASYTUNING_PLINE_INFO_T rEasytuningTable;
    ...
    if(m_pIAeAlgo != NULL) {
        MY_LOG("[%s()] i4SensorDev:%d, m_pIAeAlgo:%p\n", __FUNCTION__, m_eSensorDev, &m_pIAeAlgo);
...
MRESULT AeMgr::AEInit(Param_T &rParam)
{
    ...
    if (!m_pIAeAlgo) {
        ...
        MY_LOG("[%s()] m_pIAeAlgo:%p\n", __FUNCTION__, &m_pIAeAlgo);
    }
