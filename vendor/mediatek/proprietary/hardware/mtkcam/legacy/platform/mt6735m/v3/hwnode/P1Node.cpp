...
#define MY_LOGD1(...)           MY_LOGD_IF(2<=mLogLevel, __VA_ARGS__)
#define MY_LOGD2(...)           MY_LOGD_IF(3<=mLogLevel, __VA_ARGS__)
...
protected: ////                     Data Member: Control p1node img dump
    MINT32                          enableP1RawDump;
    ...
    , mInFlightRequestCnt(0)
{
    ...
    //1:preivew ; 2:capture ; 3:ALL Dump
    enableP1RawDump = ::property_get_int32("vendor.debug.camera.p1node.dump",0);
...
)
{
    ...
    CAM_TRACE_CALL();
    MY_LOGD_IF(1<=mLogLevel,"active = %d,RequestNo: %d,FrameNo: %d", mActive,pFrame->getRequestNo(),pFrame->getFrameNo());
    if (!mActive) {
        hardwareOps_start();
    }
