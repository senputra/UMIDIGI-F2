...
enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo,MINT32 const enqueCq,MINT32 const dupCqIdx)
{
    ...
    PIPE_DBG("[%d]: (0x%08x,0x%lx,0x%lx)/(0x%08x,0x%lx,0x%lx)/(0x%08x,0x%lx,0x%lx) ",portID.index,
...
dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo, EDrvScenario eDrvSce, MINT32 const dequeCq,MINT32 const dequeBurstQIdx, MINT32 const dequeDupCqIdx, MUINT32 const u4TimeoutMs /*= 0xFFFFFFFF*/)
{
    ...
    for ( MINT32 i = 0; i < (MINT32)rQBufInfo.vBufInfo.size() ; i++) {
        ...
        PIPE_DBG("i(%d),va(0x%lx/0x%lx/0x%lx),pa(0x%lx/0x%lx/0x%lx),size(0x%lx/0x%lx/0x%lx)",
            ...
            (unsigned long)rQBufInfo.vBufInfo[i].u4BufSize[0],(unsigned long)rQBufInfo.vBufInfo[i].u4BufSize[1],(unsigned long)rQBufInfo.vBufInfo[i].u4BufSize[2]);
    }
    //
EXIT:
    return  ret;
...
configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, PipePackageInfo *pPipePackageInfo)
{
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ )
    {
        ...
        //
        PIPE_INF("vInPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),Size(0x%08x,0x%08x,0x%08x),VA(0x%lx,0x%lx,0x%lx),PA(0x%lx,0x%lx,0x%lx),memID(0x%x,0x%x,0x%x),magicNum(0x%x)",\
                                                        i,
                                                        vInPorts[i]->eImgFmt,
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ )
    {
        ...
        //
        PIPE_INF("vOutPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),Size(0x%08x,0x%08x,0x%08x),VA(0x%lx,0x%lx,0x%lx),PA(0x%lx,0x%lx,0x%lx),memID(0x%x,0x%x,0x%x)",i,\
                                                        vOutPorts[i]->eImgFmt,
    ...
    if(pPipePackageInfo->isV3==true){
        ...
        if(pPipePackageInfo->pTuningQue != NULL){
            ...
            if(dma_en & CAM_CTL_DMA_EN_LSCI_EN){
                ...
                PIPE_DBG("[LSCI](x,y)=(%lu,%lu),s(%lu),pa(0x%lx)",\
                    (unsigned long)m_camPass2Param.lsci.size.w,(unsigned long)m_camPass2Param.lsci.size.h,(unsigned long)m_camPass2Param.lsci.size.stride,\
        ...
        if(pPipePackageInfo->mP1PrivateData.ResizeDstH) {
            ...
            PIPE_ERR("[Error]wrong dstH(%d),hrz_info(%lf)",pPipePackageInfo->mP1PrivateData.ResizeDstH,hrzRatio);
        }
    ...
    PIPE_INF("Pass(%d),imgi.fmt(0x%x),meScenarioID(%d),isCropG1En(%d),cdrz_en(%d),mdpSrcFmt(0x%x),mdp_src(%lu,%lu,%lu,%lu),en_p2_dma(0x%08x),crop1TableIdx(%d),mdpCheck1(%d),mdpCheck2(%d),drvScenario(%d)",\
    this->m_pipePass,portInfo_imgi.eImgFmt,meScenarioID,isCropG1En,cdrz_en,m_camPass2Param.croppara.mdp_src_fmt, \
    (unsigned long)m_camPass2Param.croppara.mdp_Yin.w,(unsigned long)m_camPass2Param.croppara.mdp_Yin.h,(unsigned long)m_camPass2Param.croppara.mdp_Yin.stride,(unsigned long)m_camPass2Param.croppara.mdp_UVin.stride,\
    ...
    if (-1 != idx_imgo ) {    // only support frame mode for dma imgo in ip scenario
        PIPE_DBG("isSetMdpOut(%d),u4BufSize(0x%x)-u4BufVA(0x%lx)-u4BufPA(0x%lx)",isSetMdpOut, vOutPorts[idx_imgo]->u4BufSize[0],(unsigned long)vOutPorts[idx_imgo]->u4BufVA[0],
    ...
    if (-1 != idx_img2o ) {
        ...
        PIPE_DBG("u4BufSize(0x%x)-u4BufVA(0x%lx)-u4BufPA(0x%lx)", vOutPorts[idx_img2o]->u4BufSize[0],(unsigned long)vOutPorts[idx_img2o]->u4BufVA[0],
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    PIPE_DBG("tid(%d) (cmd,arg1,arg2,arg3)=(0x%08x,0x%lx,0x%lx,0x%lx)", gettid(), cmd, (unsigned long)arg1, (unsigned long)arg2, (unsigned long)arg3);
