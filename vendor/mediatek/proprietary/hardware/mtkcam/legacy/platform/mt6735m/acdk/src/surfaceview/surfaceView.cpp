        ...
        MUINT32 const orientation)
{
    ...
    ACDK_LOGD_DYN(g_overlayDebug,"phyAddr  = 0x%x", (int)phyAddr);
    ACDK_LOGD_DYN(g_overlayDebug,"virtAddr = 0x%x", (int)virtAddr);
...
MINT32 surfaceView::refresh()
{
    ...
    if(g_overlayFlag == 0)
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug,"src VA(0x%p), PA(0x%p)", mLayerInfo[0].src_base_addr,mLayerInfo[0].src_phy_addr );
       ...
       ACDK_LOGD("src VA(0x%p), PA(0x%p)",mLayerInfo[0].src_base_addr,mLayerInfo[0].src_phy_addr );
       if (ioctl(mFBfd, MTKFB_SET_OVERLAY_LAYER, &mLayerInfo[0]) < 0)
       {
           ACDK_LOGE("MTKFB_SET_OVERLAY_LAYER failed");
           return ACDK_RETURN_API_FAIL;
    ...
    else
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug,"use mmap");
        ACDK_LOGD_DYN(g_overlayDebug,"src VA(0x%p), size(%u)",mLayerInfo[0].src_base_addr,mLayerInfo[0].src_width * mLayerInfo[0].src_height * 2);
