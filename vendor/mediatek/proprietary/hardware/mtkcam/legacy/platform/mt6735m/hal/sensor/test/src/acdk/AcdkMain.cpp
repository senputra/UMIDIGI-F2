...
MINT32 AcdkMain::uninit()
{
    ...
    if(m_pSensorHalObj != NULL)
    {
        if(!m_pSensorHalObj->powerOff("acdk test", 1, &msensorArray[0]))
        {
            ACDK_LOGE("Faile to uninit sensorHal(0x%x)", err);
        }
         m_pSensorHalObj->destroyInstance("acdk test");
         m_pSensorHalObj = NULL;
    }
...
MINT32 AcdkMain::sensorInit()
{
    ...
    //====== Sensor Setting =====
    ACDK_LOGD("before testSearchSensor" );
//    testSearchSensor();


//return ACDK_RETURN_UNKNOWN_ERROR;
    //search sensor
  //  mSupportedSensorDev = m_pSensorHalObj->searchSensor();
    m_pSensorListObj = NSCam::IHalSensorList::get();
    mSupportedSensorDev=m_pSensorListObj->searchSensors();
    MUINT const sensorNum = m_pSensorListObj->queryNumberOfSensors();

 ACDK_LOGD("after testSearchSensor" );
...
/*    // set each sensor type info
  ...
  if(mSupportedSensorDev & SENSOR_DEV_ATV)
    ...
    sensorFound = mSupportedSensorDev > 0;
    if(    mSensorDev == SENSOR_DEV_NONE) {
//        mSensorDev = SENSOR_DEV_MAIN;
  ...
  /*  err = m_pSensorHalObj->sendCommand(mSensorDev,
    ...
    // Open sensor device
        ...
        for (MUINT i = 0; i < sensorNum; i++){
            if(m_pSensorListObj->querySensorDevIdx(i) == mSensorDev)
            {
                msensorArray[0] = i;
                mSensorIndex = i;
                break;
        ...
        //====== Sensor Init======
        //set current sensor device
        m_pSensorHalObj = m_pSensorListObj->createSensor("acdk test", 1, &msensorArray[0]);//modify
        ...
        if(m_pSensorHalObj == NULL)
        {
           ...
           goto sensorInitExit;
            ...
            m_pSensorHalObj->powerOn("acdk test", 1, &msensorArray[0]);
...
/*******************************************************************************
* convert v3 sensor type to v1 sensortype (halSensorType_s)
********************************************************************************/

MINT32 convertSensorType(MINT32 v3Type)
{
    int v1Type = 0xFF;//SENSOR_TYPE_UNKNOWN
    if(v3Type == SENSOR_TYPE_RAW) {
        v1Type = 0;//SENSOR_TYPE_RAW
    } else if(v3Type == SENSOR_TYPE_YUV) {
        v1Type = 1;//SENSOR_TYPE_YUV
    } else if(v3Type == SENSOR_TYPE_RGB) {
        v1Type = 4;//SENSOR_TYPE_RGB888
    } else if(v3Type == SENSOR_TYPE_JPEG) {
        v1Type = 5;//SENSOR_TYPE_JPEG
    }
    return v1Type;
}
...
/*
    ...
    */
    ...
    m_pSensorListObj->querySensorStaticInfo(NSCam::SENSOR_DEV_MAIN,&sensorStaticInfo[0]);
    m_pSensorListObj->querySensorStaticInfo(NSCam::SENSOR_DEV_SUB,&sensorStaticInfo[1]);
    ...
    err = m_pSensorHalObj->sendCommand(mSensorDev, SENSOR_CMD_GET_UNSTABLE_DELAY_FRAME_CNT,
    ...
    if(mSensorDev==SENSOR_DEV_MAIN)
        psensorStaticInfo = &sensorStaticInfo[0];
    else
        psensorStaticInfo = &sensorStaticInfo[1];
    mSensorResolution.SensorPreviewWidth  = psensorStaticInfo->previewWidth;
    mSensorResolution.SensorPreviewHeight =    psensorStaticInfo->previewHeight;
    mSensorResolution.SensorFullWidth     = psensorStaticInfo->captureWidth;
    mSensorResolution.SensorFullHeight    = psensorStaticInfo->captureHeight;
    mSensorResolution.SensorVideoWidth    = psensorStaticInfo->videoWidth;
    mSensorResolution.SensorVideoHeight   = psensorStaticInfo->videoHeight;
    mSensorType = convertSensorType( psensorStaticInfo->sensorType);
    //mSensorBitDepth =sensorStaticInfo[0].rawSensorBit;
                   //  sensorStaticInfo[0].rawSensorBit.sensorFormatOrder
    ...
    ACDK_LOGD("bit depth(%u)",psensorStaticInfo->rawSensorBit);
   // ACDK_LOGD("isPacked(%u)",mSensorFormatInfo.u4IsPacked);
    ACDK_LOGD("color order(%u)",psensorStaticInfo->sensorFormatOrder);
    ...
    */
        switch(psensorStaticInfo->rawSensorBit)
        {
            ...
            default : tempFormat = eImgFmt_UNKNOWN;
                      ACDK_LOGE("unknown raw image bit depth(%u)",psensorStaticInfo->rawSensorBit);
    ...
    else if(mSensorType == SENSOR_TYPE_YUV) //YUV
    {
        ...
        switch(psensorStaticInfo->sensorFormatOrder)
        {
            ...
            case SENSOR_OUTPUT_FORMAT_YCrYCb :
                ...
                break;
            default : tempFormat = eImgFmt_UNKNOWN;
                      ACDK_LOGE("unknown YUV type(0x%x)",psensorStaticInfo->sensorFormatOrder);
                      return ACDK_RETURN_INVALID_PARA;
        }
...
*
********************************************************************************/
MINT32 AcdkMain::startPreview(MUINT16 scenario)
{
    ACDK_LOGD("+");
    ...
    IHalSensor::ConfigParam configParam[2] = {0,0};

    configParam[0] =
     {
       index            : mSensorDev==SENSOR_DEV_MAIN ?0 :1,
       crop             : NSCam::MSize(psensorStaticInfo->previewWidth,psensorStaticInfo->previewHeight),
       scenarioId        : NSCam::SENSOR_SCENARIO_ID_NORMAL_PREVIEW,
       isBypassScenario : 0,
       isContinuous     : 1,
       HDRMode            : 0,            //iHDR mode : 0: no HDR, 1: IHDR, 2:MHDR
       framerate        : 0,          // Unit : FPS, 10based, 0: will run sensor mode default setting
       twopixelOn        : 0,
       debugMode        : 0,
     };

    if(scenario == SENSOR_SCENARIO_ID_NORMAL_CAPTURE ) {
        configParam[0].crop = NSCam::MSize(psensorStaticInfo->captureWidth, psensorStaticInfo->captureHeight);
        configParam[0].scenarioId = NSCam::SENSOR_SCENARIO_ID_NORMAL_VIDEO;
    } else if(scenario == SENSOR_SCENARIO_ID_NORMAL_VIDEO ) {
        configParam[0].crop = NSCam::MSize(psensorStaticInfo->videoWidth, psensorStaticInfo->videoHeight);
        configParam[0].scenarioId = NSCam::SENSOR_SCENARIO_ID_NORMAL_VIDEO;
    } else if(scenario == SENSOR_SCENARIO_ID_SLIM_VIDEO1 ) {
        configParam[0].crop = NSCam::MSize(psensorStaticInfo->video1Width, psensorStaticInfo->video1Height);
        configParam[0].scenarioId = NSCam::SENSOR_SCENARIO_ID_NORMAL_VIDEO;
    } else if(scenario == SENSOR_SCENARIO_ID_SLIM_VIDEO2 ) {
        configParam[0].crop = NSCam::MSize(psensorStaticInfo->video2Width, psensorStaticInfo->video2Height);
        configParam[0].scenarioId = NSCam::SENSOR_SCENARIO_ID_NORMAL_VIDEO;
    }
    ACDK_LOGD("scenario %d WxH %d x %d\n", psensorStaticInfo->captureWidth, psensorStaticInfo->captureHeight);
...
/*
    if(m_pSensorHalObj->configure(1,&configParam[0])==0){
                ACDK_LOGE("m_pSensorHalObj->configure() fail");
                  err = ACDK_RETURN_API_FAIL;
        goto startPreviewExit;
    }
*//*
...
*/
    ...
    mAcdkMhalPrvParam.sensorID           = mSensorDev;
    ...
    mAcdkMhalPrvParam.sensorType         = mSensorType==SENSOR_TYPE_RAW ?0 :1;
                                         //SENSOR_TYPE_RAW =1 in IHalsensor.h;//0 for RAW ;1 for YUV in sensor_hal.h
    mAcdkMhalPrvParam.sensorFormat       = sensorFormat;
    mAcdkMhalPrvParam.sensorColorOrder   = psensorStaticInfo->sensorFormatOrder;
                ...
                MINT32  const isSaveImg)
{
    ...
    MUINT32 rawBit = psensorStaticInfo->rawSensorBit, rawType = 0;
    ...
    property_get("vendor.camera.acdkdump.raw", rawVal, "0");  // 4 : 8bit-pure, 8 : 10bit-pure, 16 : 8bit-processed, 32 : 10bit-processed
    ...
    if(mode == PREVIEW_MODE)
    {
        sensorScenarioId = SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
    }
    else if(mode == VIDEO_MODE)
    {
        sensorScenarioId = SENSOR_SCENARIO_ID_NORMAL_VIDEO;
    }
    else
    {
        sensorScenarioId = SENSOR_SCENARIO_ID_NORMAL_CAPTURE;
    }
            ...
            if(mCapType & 0x3C)
            {
                ...
                else
                {
                    ...
                    if(mUnPack == MTRUE) //for specail mode use : unpack raw image to normal package
                    {
                        ...
                        switch(psensorStaticInfo->rawSensorBit)
                        {
                            case RAW_SENSOR_8BIT :   tempFmt = eImgFmt_BAYER8;
                                break;
                            case RAW_SENSOR_10BIT :  tempFmt = eImgFmt_BAYER10;
                                break;
                            case RAW_SENSOR_12BIT :  tempFmt = eImgFmt_BAYER12;
                                break;
                        ...
                        else
                        {
                            m_pAcdkUtilityObj->rawImgUnpack(mRawIMemInfo,
                                                            ...
                                                            psensorStaticInfo->rawSensorBit,
                                                            imgStride);
                    ...
                    acdkCapInfo.RAWImgBufInfo.bitDepth  = psensorStaticInfo->rawSensorBit;
                    acdkCapInfo.RAWImgBufInfo.eColorOrder = (eRAW_ColorOrder)psensorStaticInfo->sensorFormatOrder;
...

{
   ...
   Hal3ABase* p3AHal = Hal3ABase::createInstance(mSensorDev);
 //  SensorHal* pSensorHal = SensorHal::createInstance();
   ...
   //
   // Sensor Debug Info
   m_pSensorHalObj->setDebugInfo(&rCamExif);
...
MINT32 AcdkMain::saveRAWImg(MINT32 mode)
{
    ...
    else
    {
        ...
        else if(mCapType == PROCESSED_RAW10_TYPE || g_dumpRAW == PROCESSED_RAW10_TYPE)
        {
            ...
            ACDK_LOGE("CapType is not supported(%d_%d)", mCapType, g_dumpRAW);
            return ACDK_RETURN_ERROR_STATE;
        }
    }
...
)
{
    ...
    else if(a_u4Ioctl == ACDK_CMD_PREVIEW_START)
    {
        ...
        MUINT16 scenario = pCamPreview->u16PreviewTestPatEn;
        if(pCamPreview->eOperaMode == ACDK_OPT_FACTORY_MODE)
        {
            mIsFacotory = 1;
        }

        err = startPreview(scenario);
