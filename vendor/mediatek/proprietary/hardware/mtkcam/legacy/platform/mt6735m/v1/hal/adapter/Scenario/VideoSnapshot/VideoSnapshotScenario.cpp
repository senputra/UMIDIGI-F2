...
allocMem(MemTypeEnum type)
{
    ...
    MY_LOGD("T(%d),S(%d),I(%d),V(0x%08X~0x%08X),P(0x%08X~0x%08X)",
            ...
            (unsigned int)mIMemBufInfo[type].virtAddr,
            (unsigned int)mIMemBufInfo[type].virtAddr+mIMemBufInfo[type].size,
            (unsigned int)mIMemBufInfo[type].phyAddr,
            (unsigned int)mIMemBufInfo[type].phyAddr+mIMemBufInfo[type].size);
    //
    EXIT:
    FUNCTION_OUT;
    return result;
...
enable(MBOOL en)
{
    ...
    if(en)
    {
        if( (int)mJpgInfo[JpgType_Thumb].width != mpParamsMgr->getInt(CameraParameters::KEY_JPEG_THUMBNAIL_WIDTH) ||
            (int)mJpgInfo[JpgType_Thumb].height != mpParamsMgr->getInt(CameraParameters::KEY_JPEG_THUMBNAIL_HEIGHT))
        {
...
setImage(ImageInfo &img)
{
    ...
    //
    if( (int)rHWCrop.w < recWidth ||
        (int)rHWCrop.h < recHeight)
    {
    ...
    //
    if((int)(picWidth*picHeight) > (int)(mPreAllocYuvWidth*mPreAllocYuvHeight))
    {
...
transMainThumb(MBOOL en)
{
    ...
    if(en)
    {
        ...
        //
        if(mpIMemDrv->mapPhyAddr(&mIMemBufInfo[MemType_Pass1Out]) < 0)
        {
            MY_LOGE("mpIMemDrv->mapPhyAddr() error");
        }
    ...
    else
    {
        ...
        if(mpIMemDrv->unmapPhyAddr(&mIMemBufInfo[MemType_Pass1Out]) < 0)
        {
            MY_LOGE("mpIMemDrv->unmapPhyAddr() error");
        }
        //
    ...
    MBOOL           enableSOI)
{
    ...
    MY_LOGD("VA:Src(0x%08X~0x%08X),Dst(0x%08X~0x%08X)",
            (unsigned int)mIMemBufInfo[srcMemType].virtAddr,
            (unsigned int)mIMemBufInfo[srcMemType].virtAddr+mIMemBufInfo[srcMemType].size,
            (unsigned int)mIMemBufInfo[dstMemType].virtAddr,
            (unsigned int)mIMemBufInfo[dstMemType].virtAddr+mIMemBufInfo[dstMemType].size);
    MY_LOGD("PA:Src(0x%08X~0x%08X),Dst(0x%08X~0x%08X)",
            (unsigned int)mIMemBufInfo[srcMemType].phyAddr,
            (unsigned int)mIMemBufInfo[srcMemType].phyAddr+mIMemBufInfo[srcMemType].size,
            (unsigned int)mIMemBufInfo[dstMemType].phyAddr,
            (unsigned int)mIMemBufInfo[dstMemType].phyAddr+mIMemBufInfo[dstMemType].size);
    for(i=0; i<3; i++)
    {
        MY_LOGD("YUV[%d]:A(0x%08X)S(%d)",
                i,
                (unsigned int)yuvAddr[i],
                 yuvSize[i]);
        //
...
integrateJpg(void)
{
    ...
    if(mIsThumb)
    {
        MY_LOGD("memcpy:DST(0x%08X + %d = 0x%08X),SRC(0x%08X),Size(%d),thumbJpgType(%d)",
                (unsigned int)mIMemBufInfo[MemType_Jpg].virtAddr,
                mJpgInfo[JpgType_Img].bitStrSize,
                (unsigned int)mIMemBufInfo[MemType_Jpg].virtAddr+mJpgInfo[JpgType_Img].bitStrSize,
                (unsigned int)mIMemBufInfo[MemType_JpgThumb].virtAddr,
                mJpgInfo[thumbJpgType].bitStrSize,
                thumbJpgType);
        memcpy(
    ...
    MY_LOGD("memcpy:DST(0x%08X + %d = 0x%08X),SRC(0x%08X),Size(%d)",
            ...
            mJpgInfo[JpgType_Img].bitStrSize,
            (unsigned int)mIMemBufInfo[MemType_Jpg].virtAddr+mJpgInfo[JpgType_Img].bitStrSize,
            (unsigned int)mIMemBufInfo[MemType_JpgMain].virtAddr,
            mJpgInfo[JpgType_Main].bitStrSize);
    memcpy(
        (MUINT8*)(mIMemBufInfo[MemType_Jpg].virtAddr+mJpgInfo[JpgType_Img].bitStrSize),
    ...
    char*       pFileName)
{
    ...
    property_get("vendor.camera.vss.dumpbuffer.enable", value, "0");
    ...
    //
    const char* filePath = "/sdcard/camera_dump/VideoSnapshotScenario/";
    char fileName[100];
