    ...
    for (; it != rToposort.end(); it++)
    {
        ...
        if ( mpResourceMgr.get() ) {
            ...
            for ( size_t i=0; i<vNodeList.size(); i++ ) {
                //MY_LOGD("check reusable nodes (%d/%d) from resourcemgr...id:%" PRIx64, i, vNodeList.size(), it->id);
...
//#warning"[FIXME] Eveny node Id should belong to the global namespace..."
    ...
    mpResourceMgr->getReusableNodes(vNodeList);
    for ( size_t i=0; i<vNodeList.size(); i++ ) {
        MY_LOGD_IF(0, "reusable from resource mgr: Nodeid(%#" PRIxPTR ")", vNodeList[i]);
    }
    Vector<StreamId_T> vStreamList;
    mpResourceMgr->getReusableStreams(vStreamList);
    for ( size_t i=0; i<vStreamList.size(); i++ ) {
        MY_LOGD_IF(0, "reusable from resource mgr: Streamid(%#" PRIx64 ")", vStreamList[i]);
    }
    MY_LOGD("ResourceMgr has reusable node(%zu) stream(%zu)", vNodeList.size(), vStreamList.size());
    //
    ...
    for (size_t i = 0; i < pPipelineNodeMap->size(); i++) {
        MY_LOGD("SetNodeValue of %zu(id:%#" PRIxPTR ")/%zu", i, pPipelineNodeMap->nodeAt(i)->getNodeId(), pPipelineNodeMap->size());
    ...
    switch (requestType)
    {
        case eWait_1080pJpeg_Request:
        {
            ...
            for (; it != rToposort.end(); it++)
            {
                sp<IPipelineNode> pNode = pPipelineNodeMap->nodeAt(it->val);
                if  ( pNode == 0 ) {
                    MY_LOGE("NULL node (id:%" PRIx64 ")", it->id);
                    continue;
                }
                //
                pNode->uninit();
...
decideSensor(
    PipeConfigParams const& rParams,
    MBOOL skipJpeg __attribute__((unused)),
    MUINT sensorId,
    SensorParam& rSensorParams
)   const
{
   ...
   if( hasEncoderConsumer ) {
         if(maxStreamSize.w > sensorSize[0].w || maxStreamSize.h > sensorSize[0].h){
               ...
               //chane to preview mode (binning) instead of using video mode.
        ...
        //
        MY_LOGD("[decided] mode: %d, size(%d, %d)@%d ,maxStreamSize(%d , %d)",
                rSensorParams.mode, rSensorParams.size.w, rSensorParams.size.h, rSensorParams.fps, maxStreamSize.w, maxStreamSize.h);
        return OK;
    }
...
)   const
{
    ...
    //
    MERROR err = OK;
    MY_LOGV("node:Id: %#" PRIxPTR, nodeId);
    //
    switch  (nodeId)
    {
        case eNODEID_P1Node:
...
)   const
{
    ...
    if  ( mpHalMeta_Control.get() ) {
        ...
        if( idx_meta < 0 ) {
            MY_LOGW("cannot get streamBuffer of %#" PRIx64 "(%s)",
...
//#warning "[FIXME] queryMinimalUsage"
    ...
    //
    MY_LOGW("cannot find nodeId %#" PRIxPTR " streamId %#" PRIx64 "(%s)",
            nodeId,
...
#define PUSH_TO_RESIZE( pStreamInfo )  PUSH_TO_VECTOR(mvYuvStreams_Resizedraw, pStreamInfo)
    ...
    for( size_t i = 0; i < mvYuvStreams_Fullraw.size(); i++ ) {
        MY_LOGD("raw streamId:%#" PRIx64 " -> yuv streamId:%#" PRIx64,
...
#define addBufPlane(planes, height, stride)                                      \
    ...
    switch( imgFormat ) {
        ...
        case eImgFmt_YUY2:
            ...
            break;
        default:
            MY_LOGE("format not support yet %x", imgFormat);
            break;
    }
#undef  addBufPlane
