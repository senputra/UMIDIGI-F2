...
typedef struct CB_INFO_t
{
    ...
    CB_INFO_t()
        ...
        , dasz(0)
    {
        memset( &da, 0, sizeof(AF_AREA_T));
    }

    CB_INFO_t( MINT32 _msgType, MINT32 _ext1, MINT32 _ext2, MINT32 _ext3, AF_AREA_T _da, MUINT32 _dasz)
    {
        msgType = _msgType;
        ext1 = _ext1;
        ext2 = _ext2;
        ext3 = _ext3;
        memcpy( &da, &_da, sizeof(AF_AREA_T));
        dasz = _dasz;
    }

    CB_INFO_t operator =(const CB_INFO_t &in)
    {
        msgType = in.msgType;
        ext1 = in.ext1;
        ext2 = in.ext2;
        ext3 = in.ext3;
        memcpy( &da, &in.da, sizeof(AF_AREA_T));
        dasz = in.dasz;
        return (*this);
    }
} CB_INFO_T;
...
private:
    ...
    // call back flow control.
    mutable Mutex m_Lock_cbq;
    std::vector<CB_INFO_T> m_cbq;
};
