...
//
#if 1
#define FUNC_START     MY_LOGD_IF(1<=mLogLevel,"API +")
#define FUNC_END       MY_LOGD_IF(1<=mLogLevel,"API -")
#else
#define FUNC_START
#define FUNC_END
#endif
...
{
    FUNC_START;
    MY_LOGD_IF(1<= mLogLevel,"RequestNo: %d,FrameNo: %d",pFrame->getRequestNo(),pFrame->getFrameNo());
    //
    if( ! pFrame.get() ) {
        MY_LOGE("Null frame");
        return BAD_VALUE;
...
) const
{
    ...
    MY_LOGD_IF(mLogLevel >= 1, "stream %#" PRIx64 ": buffer: %p, usage: %x",
...
) const
{
    if( pStreamBuffer == NULL || pImageBuffer == NULL ) {
        MY_LOGE("pStreamBuffer or pImageBuffer should not be NULL");
        return;
    }
    //
...
)
{
    ...
    MY_LOGD_IF(mLogLevel >=1 ,"frame %u done", pFrame->getFrameNo());
...
#define PORT_IMG2O_USED  (0x4)
    ...
    MY_LOGD_IF(0, "stream id %#" PRIx64 ", occupied %x",
            streamId, rOccupied);
    return ret;
...
)
{
    ...
    enqueParams.mFrameNo = muEnqueCnt; //pRequest->getFrameNo();
