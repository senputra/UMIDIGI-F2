...
MINT32 EisDrvImp::Init()
{
    ...
    if(mEisoDmaBuf.memID == -5)
    {
        ...
        EIS_LOG_R("EisIMem : ID(%d),SZ(%u),VA(%p),PA(%p),cache(%d)",mEisoDmaBuf.memID,
                                                                    mEisoDmaBuf.size,
                                                                    (void*)mEisoDmaBuf.virtAddr,
                                                                    (void*)mEisoDmaBuf.phyAddr,
...
MINT32 EisDrvImp::UpdateEisoIdx()
{
...

    {
        ...
        if(g_debugDump_R > 0)
        {
        EIS_LOG_R("condition broadcast,TS(%" PRIi64 ")",eisoData.timeStamp);
        }
        mEisoCond.broadcast();
    }
...
MINT32 EisDrvImp::GetEisHwStatistic(EIS_STATISTIC_STRUCT *apEisStat,const MINT64 &aTimeStamp)
{
    ...
    if(g_debugDump_R > 0)
    {
    EIS_LOG_R("imgTS(%" PRIi64 ")",aTimeStamp);
    }
    ...
    else
    {
...

        {
            ...
            while(!mEisoData.empty())
            {
                ...
                {
                    EIS_LOG_R("miss,(va,pa,ts)=(%p,%p,%" PRIi64 ")",(void*)mEisoData.front().va,(void*)mEisoData.front().pa,mEisoData.front().timeStamp);
                    mEisoData.pop();
                }
            ...
            if(mEisoData.empty() && hasMatch == 0)
            {
                ...
                status = mEisoCond.waitRelative(mQueLock,(long long int)EISO_CONDITION_WAIT_TIME*1000000LL); //conditaional wait 5ms
                //EIS_LOG_R("eiso condition wait done");
                if( OK != status ) {
                    EIS_WRN_R("Timeout: waitEiso condition status[%s(%d)]", ::strerror(-status), -status);
                }
        ...
        if(hasMatch == 1)
        {
            ...
            if(g_debugDump_R > 0)
            {
            EIS_LOG_R("pEisoAddr(%p)",(void*)pEisoAddr);
            }
            ...
            for(MINT32 i = 0; i < EIS_MAX_WIN_NUM; ++i)
            {
                if(i != 0)
                {
                    ...
                    if(g_debugDump_R == 3)
                    {
                        EIS_LOG_R("i(%d),pEisoAddr(%p)",i,(void*)pEisoAddr);
                    }
                }
...
MVOID EisDrvImp::DumpReg()
{
    ...
    EIS_LOG_R("FIR_H:0x%08x",(regVal & 0x3F00) >> 8);
    EIS_LOG_R("GAIN_IIR_V:0x%08x",(regVal & 0xE0) >> 5);
    EIS_LOG_R("GAIN_IIR_H:0x%08x",(regVal & 0x1C) >> 2);
    EIS_LOG_R("proc_gain:0x%08x",regVal & 0x3);
...
void EisP1Cb::p1TuningNotify(MVOID *pInput,MVOID *pOutput)
{
    ...
    {
        EIS_ERR_R("Not support sensor type");
        return;
    }
