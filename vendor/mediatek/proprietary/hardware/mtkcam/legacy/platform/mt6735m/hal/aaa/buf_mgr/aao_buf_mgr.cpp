...
debugPrint()
{
    ...
    for (it = m_rHwBufList.begin(); it != m_rHwBufList.end(); it++ ) {
        MY_LOG("m_rHwBufList.virtAddr:[0x%" PRIXPTR "]/phyAddr:[0x%" PRIXPTR "] \n",it->virtAddr,it->phyAddr);
    }
...
{
    MY_LOG_IF(m_bDebugEnable,"%s() rBufInfo.virtAddr:[0x%" PRIXPTR "]/phyAddr:[0x%" PRIXPTR "] \n", __FUNCTION__, rBufInfo.virtAddr, rBufInfo.phyAddr);
...
dequeueHwBuf(BufInfo_T& rBufInfo)
{
    ...
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.aao.dump.enable", value, "0");   // if the value is 11, it mean input buffer for test
    ...
    MY_LOG_IF(m_bDebugEnable,"rBufInfo.virtAddr:[0x%" PRIXPTR "]/phyAddr:[0x%" PRIXPTR "] \n",rBufInfo.virtAddr,rBufInfo.phyAddr);
