...
{
    LOG_DBG("cb(0x%lx),cookie(0x%lx)", (unsigned long) pCameraIOCB, (unsigned long)user);
...
MBOOL IspDrv::setTurnTopEn1(MINT32 cq, MINT32 en1)
{
    ...
    if(cq == ISP_DRV_CQ01)
    {
        ...
        {
            if((MUINT32)en1 == ISP_DRV_TURNING_TOP_RESET)
            {
...
MBOOL IspDrv::setTurnTopEn2(MINT32 cq, MINT32 en2)
{
    ...
    if(cq == ISP_DRV_CQ01)
    {
        ...
        for(i = 0; i < ISP_INT_BIT_NUM; i++)
        {
            if((MUINT32)en2 == ISP_DRV_TURNING_TOP_RESET)
            {
...
MBOOL IspDrv::setTurnTopDma(MINT32 cq, MINT32 dma)
{
    ...
    if(cq == ISP_DRV_CQ01)
    {
        ...
        for(i = 0; i < ISP_INT_BIT_NUM; i++)
        {
            if((MUINT32)dma == ISP_DRV_TURNING_TOP_RESET)
            {
    ...
    if(tdriMgrInfoCq1.updateCqDescriptorNum)
    {
        ...
        for(i = 0; i < CAM_MODULE_MAX; i++)
        {
            ...
            if(tdriMgrInfoCq1.mTurUpdTable[i] == MTRUE)
            {
                ...
                LOG_DBG("i(%d) cqDescOffset(0x%x) cqDescRegNum(%d) pSrc(0x%lx) pDst(0x%lx)"
...
MBOOL IspDrvImp::init()
{
    ...
    if(!m_fgIsGdmaMode)    // Normal Mode
    {
        ...
        LOG_VRB("v(0x%x)(0x%x)(0x%lx)(0x%lx)(0x%lx)(0x%lx)",mIspVirRegSize,
                                                         m_ispVirRegBufInfo.size,
                                                         (unsigned long)m_ispVirRegBufInfo.virtAddr,
                                                         (unsigned long)m_ispVirRegBufInfo.phyAddr,
                                                         (unsigned long)mpIspVirRegBuffer,
                                                         (unsigned long)mpIspVirRegAddrPA[0]);
        ...
        LOG_VRB("cq(0x%x)(0x%x)(0x%lx)(0x%lx)(0x%lx)(0x%lx)",mIspCQDescSize,
                                                          m_ispCQDescBufInfo.size,
                                                          (unsigned long)m_ispCQDescBufInfo.virtAddr,
                                                          (unsigned long)m_ispCQDescBufInfo.phyAddr,
                                                          (unsigned long)mpIspCQDescBufferVirt,
                                                          (unsigned long)mpIspCQDescBufferPhy);
        ...
        for(MINT32 i = 0; i < ISP_DRV_DESCRIPTOR_CQ_NUM; i++)
        {
            ...
            LOG_INF("cq:[%d],mpIspCQDescriptor:Virt[0x%lx]/Phy[0x%p]",i,(unsigned long)mpIspCQDescriptorVirt[i],mpIspCQDescriptorPhy[i]);
        }
        ...
        {
            LOG_ERR("rtbc data too large(%lu)>(%lu)",(unsigned long)sizeof(CQ_RTBC_RING_ST),(unsigned long)sizeof(gIspCQDescInit));
        }
...
{
    LOG_DBG("mpIspHwRegAddr(0x%lx)",(unsigned long)mpIspHwRegAddr);
...
MBOOL IspDrvImp::cqRingBuf(MVOID *pBuf_ctrl)
{
    ...
    LOG_DBG("[rtbc](%lu)",(unsigned long)sizeof(CQ_RING_CMD_ST));
    ...
    switch(pbuf_ctrl->ctrl)
    {
        case ISP_RT_BUF_CTRL_ENQUE:
            ...
            else    //VF is ON
            {
                if(pex_buf_info)
                {
                    LOG_INF("[rtbc]exchange 1st buf. by 2nd buf. and enque it(0x%lx)",(unsigned long)pex_buf_info);
        ...
        case ISP_RT_BUF_CTRL_CLEAR:
            ...
            {
                LOG_ERR("rtbc data exceed buffer size(%lu)>(%lu)",(unsigned long)sizeof(CQ_RTBC_RING_ST),(unsigned long)sizeof(gIspCQDescInit));
            }
