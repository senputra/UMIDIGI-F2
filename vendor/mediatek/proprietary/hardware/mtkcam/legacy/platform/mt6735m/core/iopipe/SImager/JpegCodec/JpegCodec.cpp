...
EXIT:
    ...
    if(!ret)
    {
        ...
        MY_LOGE(" fmt 0x%x, wxh %dx%d, va 0x%x/0x%x, pa %p/%p fd %d/%d, size %d/%d",
                pSrcBufInfo->getImgFormat(),
                pSrcBufInfo->getImgSize().w       , pSrcBufInfo->getImgSize().h                       ,
                (unsigned int)pSrcBufInfo->getBufVA(0), (unsigned int)(plane>1 ? pSrcBufInfo->getBufVA(1) : 0),
                (void *)pSrcBufInfo->getBufPA(0)          , (void *)(plane>1 ? pSrcBufInfo->getBufPA(1) : 0)            ,
                pSrcBufInfo->getFD(0)             , plane>1 ? pSrcBufInfo->getFD(1) : 0               ,
                (int)pSrcBufInfo->getBufSizeInBytes(0) , (int)(plane>1 ? pSrcBufInfo->getBufSizeInBytes(1) : 0) );
        MY_LOGE("quality %u, dstV/P 0x%x/0x%x, size %d, soi %d",
                 u4Quality,
                 (unsigned int)pDstBufInfo->getBufVA(0), (unsigned int)pDstBufInfo->getBufPA(0),
                 (int)pDstBufInfo->getBufSizeInBytes(0),
                 u4IsSOI );
    }
    delete pJpgEncoder;
...
using namespace NSCam::Utils::Format;
    ...
    *pBuf = allocator->alloc_ion("allocYuvBuf", imgParam);
    ...
    //
    MY_LOGD("<YUV> ImgBitsPerPixel:%d BufSizeInBytes:%d", (int)(*pBuf)->getImgBitsPerPixel(), (int)(*pBuf)->getBufSizeInBytes(0));
    return  MTRUE;
}
