...
enqueInBuf(PortID const portID, QBufInfo const& rQBufInfo)
{
    ...
    PIPE_DBG("QBufInfo:(user, reserved, num)=(%x, %d, %d)", rQBufInfo.u4User, rQBufInfo.u4Reserved, (MINT32)rQBufInfo.vBufInfo.size());
    //
    return  MTRUE;
}
...
PIPE_DBG("+, CQ:%d",m_pass1_Raw_CQ);
    ...
    //
    PIPE_DBG("P1 in[%d]/out[%d]", (MINT32)vInPorts.size(), (MINT32)vOutPorts.size());
    ...
    if (-1 != idx_imgo ) {
        ...
        PIPE_INF("imgo_crop [%d, %d, %d, %d]_f(0x%x,0x%x)\n", \
                        ...
                        (MINT32)m_camPass1Param.imgo.crop.w,(MINT32)m_camPass1Param.imgo.crop.h, \
    ...
    if (-1 != idx_img2o ) {
        ...
        PIPE_INF("[3ASync]img2o crop=%d,%d",(MINT32)m_camPass1Param.img2o.crop.w, (MINT32)m_camPass1Param.img2o.crop.h );
...
irq(EPipePass pass,EPipeIRQ irq_int, int irqclear, int userEnum)
{
    ...
    PIPE_DBG("+ tid(%d) (pass,irq_int,clear)=(0x%08x,0x%08x,0x%08x)", gettid(), pass, irq_int,irqclear);
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    PIPE_DBG("+ tid(%d) (cmd,arg1,arg2,arg3)=(0x%08x,0x%08x,0x%08x,0x%08x)", gettid(), cmd, (MUINT32)arg1, (MUINT32)arg2, (MUINT32)arg3);
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_HRZ:
            ...
            //Arg3: sensorDev
            if (0 == arg1) {
                PIPE_ERR("[3ASync] SET_IMG2O NULL param(0x%x)!",(MUINT32)arg1);
                return MFALSE;
            }
            pUserImg2oPara = (DMACfg*)arg1;

            if (0 == pUserImg2oPara->out.stride) {
                PIPE_ERR("[3ASync] SET_IMG2O Invalid stride(0x%x)!",(MUINT32)((DMACfg*)arg1)->out.stride);
                //workaround first
            ...
            if (0 ==pUserImg2oPara->out.stride * pUserImg2oPara->crop.w * pUserImg2oPara->crop.h) {
                PIPE_ERR("[3ASync] SET_IMG2O Invalid Para(0x%x/0x%x/0x%x)!",(MUINT32)pUserImg2oPara->out.stride, (MUINT32)pUserImg2oPara->crop.w, (MUINT32)pUserImg2oPara->crop.h);
                return MFALSE;
            }

            if (0 == pUserImg2oPara->out.w * pUserImg2oPara->out.w) {
                PIPE_ERR("[3ASync] SET_IMG2O Invalid OutWind Para(0x%x/0x%x)!", (MUINT32)pUserImg2oPara->out.w, (MUINT32)pUserImg2oPara->out.h);
                return MFALSE;
            }


            PIPE_DBG("[3ASync]SET_IMG2O path:%d, m_num_0=%x -", (MINT32)arg2, pUserImg2oPara->m_num);
            ...
            if ( EPortIndex_TG1I == arg2){
               if (pUserImg2oPara->crop.x > this->m_camPass1Param.hrz_in_size.w) {
                    PIPE_ERR("[3ASync]ERROR: Crop invalid(x(%d)>size_w(%d))",pUserImg2oPara->crop.x, (MINT32)this->m_camPass1Param.hrz_in_size.w);
                    m_hrz_update_cnt = 0;
                    this->mRunTimeCfgLock.unlock();
                    return MFALSE;
                }
                if (pUserImg2oPara->crop.w > this->m_camPass1Param.hrz_in_size.w) {
                    PIPE_ERR("[3ASync]ERROR: Not Support Scale up(in(%d)<roi_w(%d))", (MINT32)pUserImg2oPara->crop.w, (MINT32)this->m_camPass1Param.hrz_in_size.w);
                    m_hrz_update_cnt = 0;
                    this->mRunTimeCfgLock.unlock();
                    return MFALSE;
                }
            ...
            this->mRunTimeCfgLock.unlock();
            PIPE_DBG("[3ASync]SET_IMG2O [%d], m_num_0=%x -", m_hrz_update_cnt, (MUINT32)pHrz_cfg->m_num_0);
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            if (((EPortIndex)arg1 != EPortIndex_TG1I)  && ((EPortIndex)arg1 != EPortIndex_TG2I)) {
                PIPE_ERR("[3ASync]NOT support PortIndex(0x%x)!",(MUINT32)arg1);
                ret = -1;
                return ret;
            }
            this->mRunTimeCfgLock.lock();
            if ( EPortIndex_TG1I == arg1 ) {
                PIPE_DBG("[3ASync]P1_UPDATE+, TG1I, magicNum=0x%x", (MUINT32)arg2);
        ...
        case EPIPECmd_SET_IMGO:
            {
                DMACfg* _pDma = NULL;
                if (0 == arg1) {
                    PIPE_ERR("[3ASync] SET_IMGO NULL param(0x%x)!",(MUINT32)arg1);
                    return MFALSE;
                }
                _pDma = (DMACfg*)arg1;

                if (0 == _pDma->out.stride) {
                    PIPE_ERR("[3ASync] SET_IMGO Invalid stride(0x%x)!",(MUINT32)_pDma->out.stride);
                }

                //if (0 ==_pDma->out.stride * _pDma->crop.w * _pDma->crop.h) {
                if (0 == _pDma->crop.w * _pDma->crop.h) {
                    PIPE_ERR("[3ASync] SET_IMGO Invalid Para(0x%x/0x%x/0x%x)!",(MUINT32)_pDma->out.stride, (MUINT32)_pDma->crop.w, (MUINT32)_pDma->crop.h);
                    return MFALSE;
                }

                if (0 == _pDma->out.w * _pDma->out.h) {
                    PIPE_ERR("[3ASync] SET_IMGO Invalid OutWind Para(0x%x/0x%x)!", (MUINT32)_pDma->out.w, (MUINT32)_pDma->out.h);
                    return MFALSE;
                }


                PIPE_DBG("[3ASync]SET_IMGO path:%d, m_num_0=%x -", (MINT32)arg2, _pDma->m_num);

                if (_pDma->crop.x > this->m_camPass1Param.tg_out_size.w) {
                    PIPE_ERR("[3ASync]ERROR:TG:0x%x Crop invalid(x(%d)>size_w(%d))",(MUINT32)arg2,(MINT32)_pDma->crop.x, (MINT32)this->m_camPass1Param.tg_out_size.w);
                    return MFALSE;
                }
                if (_pDma->crop.w > this->m_camPass1Param.tg_out_size.w) {
                    PIPE_ERR("[3ASync]ERROR:TG:0x%x Not Support Scale up(in(%d)<tg_w(%d))",(MUINT32)arg2, (MINT32)_pDma->crop.w, (MINT32)this->m_camPass1Param.tg_out_size.w);
                    return MFALSE;
                }
                ...
                if ((EPortIndex)arg2 != EPortIndex_TG1I) {
                    PIPE_ERR("[3ASync]NOT support TGIdx(0x%x)!",(MUINT32)arg2);
                    ret = -1;
                    return ret;
                }
                this->mRunTimeCfgLock.lock();
        ...
        case EPIPECmd_GET_MODULE_HANDLE: //handle-based
             ...
             if (((EPortIndex_TG1I == tg) && (!(this->mPath1 & ISP_PASS1)))/*||
                ...
                PIPE_ERR("GET_MODULE_HANDLE: User(%s), tg=%d NotMatch Path=0x%x",(char const*)arg3, tg, mPath1);
                return MFALSE;
             }
             if (EPortIndex_UNKNOW == tg ) {
                 CAM_LOGE("GET_MODULE_HANDLE:User(%s), tg=%d is Invalid",(char const*)arg3, tg);
                 return -1;
             }
             ...
             *((MINT32*)arg2) = (MINT32) (m_P1HWRWctrl[tg].getModuleHandle((CAM_MODULE_ENUM)p1ConvModuleID(arg1), (char const*)arg3));
             if (0 == *((MINT32*)arg2)) {
                PIPE_ERR("NOT Rdy || Not-supported module(0x%x ,%d)!",(MUINT32)arg1,p1ConvModuleID(arg1));
                ret = -1;
             }
             break;
        case EPIPECmd_RELEASE_MODULE_HANDLE: //handle-based
            ...
            //CAM_LOGD("Handle(0x%x), %s!",arg1, arg2);
             ...
             if (!arg1) {
                 PIPE_ERR("RELEASE_MODULE_HANDLE: Invalid Handle(0x%x)!",(MUINT32)arg1);
                 ret = -1;
             }else{
        ...
        case EPIPECmd_SET_MODULE_EN:
            ...
            tg = _p1GetModuleTgIdx(arg1,this->m_pass1TwinMode);
            if (EPortIndex_UNKNOW == tg ) {
                CAM_LOGE("SET_MODULE_EN:User(%s), tg=%d is Invalid",(char const*)arg3, tg);
                return -1;
            }
            if (((EPortIndex_TG1I == tg) && (!(this->mPath1 & ISP_PASS1)))/*||
                ((EPortIndex_TG2I == tg) && (!(this->mPath1 & ISP_PASS1_D)))*/) {
                PIPE_ERR("SET_MODULE_EN: User(%s), tg=%d NotMatch Path=0x%x",(char const*)arg3, tg, mPath1);
                return MFALSE;
            }
        ...
        case EPIPECmd_SET_MODULE_SEL:
            ...
            if (EPortIndex_UNKNOW == tg ) {
                CAM_LOGE("SET_MODULE_SEL:User(%s), tg=%d is Invalid",(char const*)arg3, tg);
                return -1;
            }
            if (((EPortIndex_TG1I == tg) && (!(this->mPath1 & ISP_PASS1)))/*||
                ((EPortIndex_TG2I == tg) && (!(this->mPath1 & ISP_PASS1_D)))*/) {
                PIPE_ERR("SET_MODULE_SEL: User(%s), tg=%d NotMatch Path=0x%x",(char const*)arg3, tg, mPath1);
                return MFALSE;
            }
