...
getZoom()
{
    ...
    if(!mvZoomRatio.empty())
    {
        MY_LOGD_IF(ENABLE_LOG_PER_FRAME,"Size(%d)", (int)mvZoomRatio.size());
...
updateOne()
{
    ...
    property_get("vendor.camera.dumpreg.enable", value, "0");
                    ...
                    list<CapBufQueNode> &vOutNode)
{
    ...
    MY_LOGD_IF(0, "raw cnt(%d)", (int)vQTBuf.at(0).bufInfo.vBufInfo.size());
    ...
    for (MINT32 i =0; i < size; i++)
    {
        ...
        for (it = vInNode.begin(); it != vInNode.end(); it++ )
        {
            ...
            {
                MY_LOGD_IF(0, "map QT->Node (0x%x)", (unsigned int)vQTBuf.at(0).bufInfo.vBufInfo[i].u4BufVA);
