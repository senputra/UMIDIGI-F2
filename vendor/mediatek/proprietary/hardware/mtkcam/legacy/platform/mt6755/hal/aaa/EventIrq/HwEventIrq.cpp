...
createInstance(const ConfigParam& rCfg, const char* strUser)
{
    ...
    case E_Event_Vsync:
        ...
        case NSIspTuning::ESensorDev_Main:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_Main, E_Event_Vsync>::getInstance();
            break;
        case NSIspTuning::ESensorDev_Sub:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_Sub, E_Event_Vsync>::getInstance();
            break;
        case NSIspTuning::ESensorDev_MainSecond:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_MainSecond, E_Event_Vsync>::getInstance();
            break;
        }
        break;
    case E_Event_Af:
        ...
        {
        case NSIspTuning::ESensorDev_Main:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_Main, E_Event_Af>::getInstance();
            break;
        case NSIspTuning::ESensorDev_Sub:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_Sub, E_Event_Af>::getInstance();
            break;
        case NSIspTuning::ESensorDev_MainSecond:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_MainSecond, E_Event_Af>::getInstance();
            break;
        }
        break;
    case E_Event_Vsync_Sensor:
        ...
        {
        case NSIspTuning::ESensorDev_Main:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_Main, E_Event_Vsync_Sensor>::getInstance();
            break;
        case NSIspTuning::ESensorDev_Sub:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_Sub, E_Event_Vsync_Sensor>::getInstance();
            break;
        case NSIspTuning::ESensorDev_MainSecond:
            pHwEventIrq = HwEventIrqType<NSIspTuning::ESensorDev_MainSecond, E_Event_Vsync_Sensor>::getInstance();
            break;
        }
        break;
    }
...
init(const ConfigParam& rCfg, const char* strUser)
{
    ...
    m_rWaitIrq.UserInfo.Type    = (rCfg.i4TgInfo == E_TG1) ? ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
    m_rWaitIrq.UserInfo.Status  =
        (m_rCfgParam.eEventType == E_Event_Vsync || m_rCfgParam.eEventType == E_Event_Vsync_Sensor) ?
            ((rCfg.i4TgInfo == E_TG1) ? CAM_CTL_INT_P1_STATUS_SOF1_INT_ST : CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST) :
            ((rCfg.i4TgInfo == E_TG1) ? CAM_CTL_INT_P1_STATUS_AF_DON_ST : CAM_CTL_INT_P1_STATUS_D_AF_DON_ST) /*AF*/;
    ...
    MY_LOG("[%s] %s (%p) m_pIspDrv(%p), userKey(%d), cfg(%d, %d, %d)", __FUNCTION__, strUser, this, m_pIspDrv, i4UserKey, m_rCfgParam.i4SensorDev, m_rCfgParam.i4TgInfo, m_rCfgParam.eEventType);
}
...
mark()
{
    if (m_pIspDrv == MNULL || !m_pIspDrv->markIrq(m_rWaitIrq))
    {
        MY_ERR("Error");
        return -1;
    }
...
query(Duration& rDuration)
{
    ...
    if (m_pIspDrv == MNULL || !m_pIspDrv->queryirqtimeinfo(&rWaitIrq))
    {
        MY_ERR("Error");
        return -1;
    }
...
wait(Duration& rDuration)
{
    ...
    if (m_pIspDrv == MNULL || !m_pIspDrv->waitIrq(&rWaitIrq))
    {
        MY_ERR("Error");
        return -1;
    }
...
{
    m_rWaitIrq.UserInfo.Type    = (i4TgInfo == E_TG1) ? ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
    m_rWaitIrq.UserInfo.Status  =
        (m_rCfgParam.eEventType == E_Event_Vsync || m_rCfgParam.eEventType == E_Event_Vsync_Sensor) ?
            ((i4TgInfo == E_TG1) ? CAM_CTL_INT_P1_STATUS_SOF1_INT_ST : CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST) :
            ((i4TgInfo == E_TG1) ? CAM_CTL_INT_P1_STATUS_AF_DON_ST : CAM_CTL_INT_P1_STATUS_D_AF_DON_ST) /*AF*/;
    m_rCfgParam.i4TgInfo = i4TgInfo;
    MY_LOG("[%s] tg(%d)", __FUNCTION__, i4TgInfo);
    return 0;
}
