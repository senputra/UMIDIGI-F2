...
#if 1
    const char input_filename[] = "/data/cct/CCTinput";
    const char output_filename[] = "/data/cct/CCToutput";
#else
    const char input_filename[] = "/sdcard/CCTinput";
    const char output_filename[] = "/sdcard/CCToutput";
#endif

    pFp = fopen(input_filename, "rb");

    if (pFp == 0)
    {
        ACDK_LOGE("[%s] fail to open CCTinput. Error Code(%d), path=%s",  __FUNCTION__, pFp, input_filename);
        goto EXIT;
    }
