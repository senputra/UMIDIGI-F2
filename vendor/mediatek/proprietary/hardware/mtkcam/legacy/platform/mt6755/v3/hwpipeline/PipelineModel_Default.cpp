    ...
    , mpResourceMgr(PipelineResourceMgr::getInstance(openId))
{
    ...
    property_get("vendor.jpeg.rotation.enable", value, "0");
...
#warning"[FIXME] Eveny node Id should belong to the global namespace..."
    ...
    for ( size_t i=0; i<vStreamList.size(); i++ ) {
        MY_LOGD("reusable from resource mgr: Streamid(%#" PRIx64 ")", vStreamList[i]);
    }
...
beginFlush()
{
    ...
    for (; it != rToposort.end(); it++)
    {
        ...
        if ( mpResourceMgr.get() ) {
            ...
            mpResourceMgr->getReusableNodes(vNodeList);
            for ( size_t i=0; i<vNodeList.size(); i++ ) {
                MY_LOGD("check reusable nodes (%zu/%zu) from resourcemgr...id:%" PRIxPTR , i, vNodeList.size(), it->id);
                if ( vNodeList[i] == it->id ) {
                    MBOOL bOwner = mpResourceMgr->isOwnerOfNode(it->id, this);
                    MY_LOGD("owner of node (id:%" PRIxPTR ") : %d", it->id, bOwner);
                    if  ( !bOwner ) {
...
)   const
{
    ...
    if  ( mpHalMeta_Control.get() ) {
        ...
        // set "the largest frame duration of streams" as "minimum frame duration"
        {
            ...
            MY_LOGD_IF(0, "The min frame duration is %ld", iMinFrmDuration);
...
)
{
    ...
    //App:Image
    {
        ...
        for( size_t i = 0; i < mvStreamDurations.size(); i++) {
            durations += String8::format("(stream %#" PRIx64 ": %ld) ",
...
) const
{
    ...
    if ( !mJpegRotationEnable ) {    // Enable by setproperty
        ...
        if ( pHalImage_Jpeg_YUV.get() ) {
            ...
            MY_LOGD_IF( 0, "Add new Jpeg_Yuv to map, StreamName:%s StreamId:%#" PRIx64 " ImageSize:%dx%d Transform:%d",
            ...
            rParams.vIHalImage.add(mpHalImage_Jpeg_YUV->getStreamId(), mpHalImage_Jpeg_YUV);
            MY_LOGD_IF( 0, "Add default Jpeg_Yuv to map, StreamName:%s StreamId:%#" PRIx64 " ImageSize:%dx%d Transform:%d",
    ...
    if  ( pHalImage_Thumbnail_YUV.get() ) {
        ...
        MY_LOGD_IF( 0, "Add Thumb_Yuv to map, StreamName:%s StreamId:%#" PRIx64 " ImageSize:%dx%d Transform:%d",
