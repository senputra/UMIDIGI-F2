    ...
    case SensorType::ACCELEROMETER:
        if(gListenLogOn)
        {
            MY_LOG("Acc(%f,%f,%f,%lld)",
                   ...
                   (long long)sensorEvent.timestamp);
        }
        gPreAcceTS = gAcceTS;
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
AfMgr &AfMgr::getInstance(MINT32 const i4SensorDev __unused)
{
...
AfMgr::AfMgr(ESensorDev_T eSensorDev)
{
    ...
    m_i4ReceiveAutoFocusCmd = FALSE;
    m_i4ReceiveCancelAutoFocusCmd = FALSE;
}
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AfMgr::Hal3AInitState(MINT32 i4SensorIdx __unused, MINT32 isInitMCU __unused)
{
    // --- init Laser ---
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AfMgr::Hal3AUninitState(MINT32 isInitMCU __unused)
{
    if (m_pMcuDrv)
    {
...
MRESULT AfMgr::init(MINT32 i4SensorIdx, MINT32 isInitMCU __unused)
{
    MY_LOG("[AF][%15s] + \n", __FUNCTION__);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AfMgr::uninit(MINT32 isInitMCU __unused)
{
    MY_LOG("[AF][%15s] +", __FUNCTION__);
    MRESULT ret __unused = S_3A_OK;
...
MINT32 AfMgr::CCTOPAFCaliOperation(MVOID *a_pAFCaliData, MUINT32 *a_pOutLen)
{
    ...
    for (MINT32 i=0; i<pAFCaliData->i4Num; i++)
        MY_LOG("[AFCaliData] Vlu %d = %lld\n", i, (long long)pAFCaliData->i8Vlu[i]);
...
MRESULT AfMgr::setAFMode(MINT32 eAFMode)
{
    ...
    m_i4ReceiveAutoFocusCmd = MFALSE;
    m_i4ReceiveCancelAutoFocusCmd = MFALSE;
...
NS3A::EAfState_T AfMgr::getAFState()
{
    ...
    NS3A::EAfState_T af_prestate = NS3A::E_AF_INACTIVE;
    ...
    {
        MY_LOG("%s state(%d), Null m_pIAfAlgo", __FUNCTION__, af_state);
    }


    af_prestate = af_state;

    if (m_eCurrAFMode == 1 || m_eCurrAFMode == 2)
    {

        if( m_i4ReceiveAutoFocusCmd == MTRUE)
        {
            if( m_i4SearchDone)
            {
                af_state = NS3A::E_AF_FOCUSED_LOCKED;
            }
            else
            {
                af_state = NS3A::E_AF_ACTIVE_SCAN;
            }
        }
        else
        {
            af_state = NS3A::E_AF_INACTIVE;
        }

        if (m_i4ReceiveCancelAutoFocusCmd == MTRUE)
        {
            af_state = NS3A::E_AF_INACTIVE;
            m_i4ReceiveCancelAutoFocusCmd = MFALSE;
            m_i4ReceiveAutoFocusCmd = MFALSE;
    ...
    if( m_bDebugEnable || (m_af_state!=af_state))
    {
        MY_LOG( "[%s] Override AF State (%d -> %d) , Pre AF State (%d), AutoFocusCmd(%d), CancelAutoFocusCmd(%d), isSearchDone(%d) AFDoneFlg(%d, %d)",
                __FUNCTION__,
                af_prestate,
                af_state,
                m_af_state,
                m_i4ReceiveAutoFocusCmd,
                m_i4ReceiveCancelAutoFocusCmd,
                m_i4SearchDone,
                mi4IsAFDoneFlg_pre,
                mi4IsAFDoneFlg_cur);
    }
...
MVOID AfMgr::DoAFPreprocessing()
{
    ...
    MINT32 i4ISO __unused =  m_sAFInput.i4ISO;
    MINT32 i4IsAEStable __unused = m_sAFInput.i4IsAEStable;
    AREA_T LastAFArea;
    MUINT8 AEBlockV[25];
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    MUINT32 i4curFrmNum __unused = m_cur_frm_num;
    ...
    if(m_NVRAM_LENS.rAFNVRAM.i4SensorEnable > 0)
    {
        MY_LOG_IF((m_bDebugEnable&1),"SensorEventAF gAcceValid=%d gPreAcceTS=%lld gAcceTS=%lld gAcceInfo[]=(%d %d %d)\n"
                  , gListenEnAcce, (long long)gPreAcceTS, (long long)gAcceTS, gAcceInfo[0], gAcceInfo[1], gAcceInfo[2]);
        MY_LOG_IF((m_bDebugEnable&1),"SensorEventAF gGyroValid=%d gPreGyroTS=%lld gGyroTS=%lld gGyroInfo[]=(%d %d %d)\n"
                  , gListenEnGyro, (long long)gPreGyroTS, (long long)gGyroTS, gGyroInfo[0], gGyroInfo[1], gGyroInfo[2]);
...
//==========
    ...
    mi4IsAFDoneFlg_cur = m_sAFOutput.i4IsAFDone;
...

    {
        ...
        if( m_pdaf_rawbuf)
        {
            ...
            if( m_bDebugEnable)
            {
                ...
                for( MUINT32 i=0; i<m_vFrmInfo.size(); i++)
                {
...
MVOID AfMgr::autoFocus()
{
    ...
    MY_LOG("%s AFCtrlMode(%d), ReceiveAutoFocusCmd(%d), ReceiveCancelAutoFocusCmd(%d), SearchDoneFlg(%d), AutoFocuscb(%d)",
           __FUNCTION__,
           m_eLIB3A_AFMode,
           m_i4ReceiveAutoFocusCmd,
           m_i4ReceiveCancelAutoFocusCmd,
...
MVOID AfMgr::cancelAutoFocus()
{
    ...
    {
        m_i4AutoFocus = FALSE;
        m_i4ReceiveCancelAutoFocusCmd = TRUE;
    }
...
MVOID AfMgr::setAperture(MFLOAT lens_aperture __unused)
{
...
MVOID AfMgr::setFilterDensity(MFLOAT lens_filterDensity __unused) {}
...
MVOID AfMgr::setFocalLength(MFLOAT lens_focalLength __unused) {}
...
MVOID AfMgr::setOpticalStabilizationMode (MINT32 ois_OnOff __unused)
{
    MUINT8 oismode=0;
...
MINT32 AfMgr::getLensState  ()
{
    ...
    MUINT8 lens_item_ivalue __unused;
...

    {
        ...
        if((m_i4EnableAF == 0) || (m_PDAF_Sensor_Support_Mode!=2))
        {
            ...
            if( mPDVCTest)
            {
                ...
                MY_LOG_IF(m_bDebugEnable, "[DAF] m_AAOPDBuf %p\n", (void*)m_AAOPDBuf);
