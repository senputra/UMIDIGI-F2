    ...
    , mbIsPureRawPDAFOn(MFALSE)
{
    ...
    //EIS
    mEisLosslessSupport = property_get_int32("vendor.debug.eis.lossless.support",(mEisLosslessSupport?1:0))==1;
}
...
onInit()
{
    ...
    if(mpIspSyncCtrl)
    {
        ...
        if( pHalSensorList != NULL )
        {
            ...
            property_get( "vendor.debug.pass1.pdafon", value, "-1");
            ...
            if( mbIsPureRawPDAFOn )
            {
                ...
                char value[PROPERTY_VALUE_MAX] = {'\0'};
                property_get( "vendor.debug.pass1.pdafframe", value, "-1");
                MINT32 val = atoi(value);
                if( val > -1 ) {
                    mPDAFFrameCount = val;
...
updateDelay(EQueryType_T const type)
{
    ...
    if(delayCnt > 0)
    {
        ...
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        MINT32 debug = true;
        property_get("vendor.debug.aaa.ctrl.enable", value, "767");
        debug = atoi(value);
        if (debug != 0)
        {
            Mutex::Autolock _l(mLock);
            ...
            if( !isFlashOn )
            {
                switch(mShotMode)
                {
                    case eShotMode_NormalShot:
                        {
                            ...
                            static const int __force_enable_mfll = ::property_get_int32("vendor.mtk_cam_zsdmfb_support", 0);
                    ...
                    case eShotMode_HdrShot:
                        ...
                        property_get( "ro.vendor.mtk_zsdhdr_support", propertyValue, "0");
...
#define START_PREVIEW_TIMEOUT_US (10*1000)
#define START_PREVIEW_TIMEOUT_COUNT (300)
    MINT32 waitCount = 0;
    CAM_TRACE_NAME("waitReadyToPreview");
    MY_LOGI("wait for start preview done +");
    while( !ret && waitCount < START_PREVIEW_TIMEOUT_COUNT )
    {
        usleep(START_PREVIEW_TIMEOUT_US);
        waitCount++;
        {
            Mutex::Autolock _l(mLock);
            ret = getFlag(muStateFlag, FLAG_3A_PREVIEW_OK);
    ...
    if( ret )
    {
        MY_LOGI("wait for start preview done - , use %d ms", waitCount*START_PREVIEW_TIMEOUT_US/1000);
    }
    else
    {
        MY_LOGE("%d ms passed, wait preview ready timeout!", waitCount*START_PREVIEW_TIMEOUT_US/1000);
        AEE_ASSERT("\nCRDISPATCH_KEY:MtkCam/DCNode:3A startpreview timeout");
    }
#undef START_PREVIEW_TIMEOUT_US
#undef START_PREVIEW_TIMEOUT_COUNT
