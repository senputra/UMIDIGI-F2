...
MBOOL
IAwbMgr::
getAWBGain(MINT32 i4SensorDev, AWB_GAIN_T &rAwbGain) const
{
    if (i4SensorDev & ESensorDev_Main) {
        AwbMgr::getInstance(ESensorDev_Main).getAWBGain(rAwbGain);
        return MTRUE;
    }
    else if (i4SensorDev & ESensorDev_Sub) {
        AwbMgr::getInstance(ESensorDev_Sub).getAWBGain(rAwbGain);
        return MTRUE;
    }
    if (i4SensorDev & ESensorDev_MainSecond) {
        AwbMgr::getInstance(ESensorDev_MainSecond).getAWBGain(rAwbGain);    
        return MTRUE;
    }
...
MRESULT
IAwbMgr::
CCTOPAWBGetAWBParam(MINT32 i4SensorDev, AWB_PARAM_T* out)
{
    if (i4SensorDev & ESensorDev_Main)
        return AwbMgr::getInstance(ESensorDev_Main).CCTOPAWBGetAWBParam(out);
    if (i4SensorDev & ESensorDev_Sub)
        return AwbMgr::getInstance(ESensorDev_Sub).CCTOPAWBGetAWBParam(out);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AwbMgr::getInstance(ESensorDev_MainSecond).CCTOPAWBGetAWBParam(out);
