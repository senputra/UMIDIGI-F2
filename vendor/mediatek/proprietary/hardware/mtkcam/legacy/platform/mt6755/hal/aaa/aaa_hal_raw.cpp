...
MVOID
Hal3ARaw::notifyP1Done(MUINT32 u4MagicNum, MVOID* pvArg __unused)
{
    ...
    if ((MINT32)u4MagicNum == mLastFlashOnFrmIdx && !(isMeetPreFlashOnCondition()) && !(isMeetMainFlashOnCondition()))
    {
...
getDebugInfo(android::Vector<MINT32>& keys, android::Vector< android::Vector<MUINT8> >& data) const
{
    ...
    SHADING_DEBUG_INFO_T& rShadigDebugInfo __unused = *(SHADING_DEBUG_INFO_T*) pBuf;
...
getCurrResult(MUINT32 u4FrmId, Result_T& rResult) const
{
    ...
    else if((mpStateMgr->getStateStatus().eCurrState == eState_AF && mpStateMgr->getStateStatus().eNextState == eState_Precapture)
            || mpStateMgr->getStateStatus().eCurrState == eState_Precapture)
    ...
    if((mpStateMgr->getStateStatus().eCurrState == eState_Precapture && mpStateMgr->getStateStatus().eNextState == eState_AF)
       || mpStateMgr->getStateStatus().eCurrState == eState_AF)
    ...
    {
        IMetadata metaExif;
        const EXIF_3A_INFO_T& rExifInfo __unused = rResult.vecExifInfo[0];
...
MVOID
Hal3ARaw::
updateImmediateResult(MUINT32 u4MagicNum __unused)
{
/*
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL Hal3ARaw::setZoom(MUINT32 u4ZoomRatio_x100 __unused, MUINT32 u4XOffset __unused, MUINT32 u4YOffset __unused, MUINT32 u4Width __unused, MUINT32 u4Height __unused)
{
    return MTRUE;
}
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MINT32 Hal3ARaw::getDelayFrame(EQueryType_T const eQueryType __unused) const
{
    return 0;
}
...
{
    MY_LOG("[%s] mpCbSet(%p), cb(%p)", __FUNCTION__, mpCbSet, cb);
    mpCbSet = cb;
    IAfMgr::getInstance().setCallbacks(m_i4SensorDev, mpCbSet);
    return 0;
}
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MINT32 Hal3ARaw::updateCaptureParams(CaptureParam_T &a_rCaptureInfo __unused)
{
    return S_3A_OK;
}
...
MINT32 Hal3ARaw::getCurrentHwId() const
{
    ...
    MBOOL fgRet __unused = pPipe->sendCommand(NSImageio::NSIspio::EPIPECmd_GET_CUR_SOF_IDX, (MINTPTR)&u4Id, 0, 0);
...
if (ENABLE_AF & m_3ACtrlEnable) {
    if (a_bEnable)  {
        if (mbAFThreadLoop== 0)
        {
            ...
            mbAFThreadLoop= 1;
            #ifdef __LP64__
                pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_OTHER, NICE_CAMERA_AF, {0}};
            #else
