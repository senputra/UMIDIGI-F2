...
sendIntent(intent2type<eIntent_VsyncUpdate>)
{
    ...
    MY_LOG_IF(1, "[StateCameraPreview::sendIntent<eIntent_VsyncUpdate>] Sensor(%d), bSyncEnable(%d), i4AfState(%d), frameCnt(%d)",
        m_SensorDevId, bSyncEnable, i4AfState, i4FrmCount);

    //BufInfo_T rBufInfo;
    BufInfo_T & rBufInfo = *(BufInfo_T*)(m_pStateMgr->mpAAOBuf);
    if (i4FrmCount < 0) {// AAO statistics is not ready
...
if (ENABLE_AWB & m_pHal3A->m_3ACtrlEnable) {
    ...
    if (m_pScheduler->jobAssignAndTimerStart(E_Job_Awb))
    {
        ...
        IAwbMgr::getInstance().doPvAWB(m_SensorDevId, i4SyncFrmCount, bAEStable, i4SceneLv, reinterpret_cast<MVOID *>(rBufInfo.virtAddr), bSyncEnable ? MFALSE : MTRUE);
    }
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    ...
    if (i4AfState != 2)
    {
        IAeMgr::getInstance().doPvAEmonitor(m_SensorDevId, i4SyncFrmCount, reinterpret_cast<MVOID *>(rBufInfo.virtAddr),
            i4ActiveAeItem, 1, 1, isNeedUpdateI2C);
        if( MTRUE )
        ...
        IAeMgr::getInstance().doPvAE(m_SensorDevId, i4SyncFrmCount, reinterpret_cast<MVOID *>(rBufInfo.virtAddr),
            i4ActiveAeItem&(E_AE_AE_CALC|E_AE_FLARE), 1, 1);
    }
    else
    {
...
if (ENABLE_LSC & m_pHal3A->m_3ACtrlEnable) {
    ...
    LscMgr2::E_TSF_CMD_T eTsfCmd = (0 == i4FrmCount) ? LscMgr2::E_TSF_CMD_BATCH : LscMgr2::E_TSF_CMD_RUN;
...
if (ENABLE_FLICKER & m_pHal3A->m_3ACtrlEnable) {
    ...
    {
        MBOOL fgMagicNumMatch = ISync3AMgr::getSync3A()->chkMagicNum(m_SensorDevId, i4FrmCount);
        if (ISync3AMgr::getSync3A()->getChkSyncEnable() && fgMagicNumMatch)
        {
            MY_LOG("[%s] Sync 2A: Sensor(%d), Sync ON", __FUNCTION__, m_SensorDevId);
            ISync3AMgr::getSync3A()->setChkSyncEnable(MFALSE);
...
if (ENABLE_3A_GENERAL & m_pHal3A->m_3ACtrlEnable) {
        ...
        ISync3AMgr::getSync3A()->isSyncEnable())
    {
        ...
        ISync3AMgr::getInstance()->setAFState(1);
    }
#endif
...
if (ENABLE_3A_GENERAL & m_pHal3A->m_3ACtrlEnable) {
    ...
    {
        MY_LOG("[%s] Sync 2A: Sensor(%d), Sync ON", __FUNCTION__, m_SensorDevId);
        ISync3AMgr::getSync3A()->syncEnable(0);
    }
}
#endif
