...
configMdpOutPort(PortInfo const* oImgInfo, MdpRotDMACfg &a_rotDma)
{
    ...
    a_rotDma.crop.h = 0;
    //
    PIPE_DBG("[a_rotDma]w(%lu),h(%lu),stride(pxl)(%lu),pa(0x%" PRIxPTR "),va(0x%" PRIxPTR "),size(%d)",
        a_rotDma.size.w, a_rotDma.size.h, a_rotDma.size.stride,
        a_rotDma.memBuf.base_pAddr, a_rotDma.memBuf.base_vAddr,
        a_rotDma.memBuf.size);
    PIPE_DBG("[a_rotDma]crop(%d,%d,%lu,%lu)_f(0x%x,0x%x)",
        a_rotDma.crop.x, a_rotDma.crop.y, a_rotDma.crop.w, a_rotDma.crop.h,
        a_rotDma.crop.floatX,a_rotDma.crop.floatY);
    PIPE_DBG("[a_rotDma]rot(%d),fmt(%d),plane(%d),seq(%d),pxlByte((0x%x))",
        a_rotDma.Rotation, a_rotDma.Format, a_rotDma.Plane, a_rotDma.Sequence, a_rotDma.pixel_byte);
    PIPE_DBG("[a_rotDma]c_ofst_addr(0x%x),v_ofst_addr(0x%x),uv_plane_swap(%d)",
        a_rotDma.memBuf_c.ofst_addr, a_rotDma.memBuf_v.ofst_addr, a_rotDma.uv_plane_swap);

    PIPE_DBG("[a_rotDma]va[0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR "]--pa[0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR "]",
        a_rotDma.memBuf.base_vAddr, a_rotDma.memBuf_c.base_vAddr, a_rotDma.memBuf_v.base_vAddr,
        a_rotDma.memBuf.base_pAddr, a_rotDma.memBuf_c.base_pAddr, a_rotDma.memBuf_v.base_pAddr);

    a_rotDma.bIsRwbEn = oImgInfo->bIsRwbEn; // add for RWB sensor
    PIPE_DBG("a_rotDma.bIsRwbEn = %d, oImgInfo->bIsRwbEn = %d", a_rotDma.bIsRwbEn, oImgInfo->bIsRwbEn); // add for RWB sensor
...
PipeImp::configDmaPort(PortInfo const* portInfo,IspDMACfg &a_dma,MUINT32 pixel_Byte,MUINT32 swap, MUINT32 isBypassOffset,EIMAGE_STRIDE planeNum)
{
    ...
    //
    PIPE_DBG("w(%lu),h(%lu),stride(%lu),xsize(%lu),crop(%d,%d,%lu,%lu)_f(0x%x,0x%x),ofst_addr(0x%x),pb((0x%x)(+<<2)),va(0x%" PRIxPTR "),pa(0x%" PRIxPTR "),pixel_Byte(%d)",
