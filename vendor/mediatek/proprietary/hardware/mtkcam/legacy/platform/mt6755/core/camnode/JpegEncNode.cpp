    ...
    , muJobDoneCount(0)
{
    ...
    addDataSupport( ENDPOINT_SRC, JPEG_ENC_HW_SRC_JPEG );
    addDataSupport( ENDPOINT_SRC, JPEG_ENC_HW_SRC_JPS );
    addDataSupport( ENDPOINT_DST, JPEG_ENC_SW_DST );
    addDataSupport( ENDPOINT_DST, JPEG_ENC_HW_DST );
    addDataSupport( ENDPOINT_DST, JPEG_ENC_HW_DST_JPEG );
    addDataSupport( ENDPOINT_DST, JPEG_ENC_HW_DST_JPS );
}
...
#define PREPARE_DST_BUFFER(data, dstdata, imgRequest)         \
    ...
    switch( data )
    {
        ...
        case JPEG_ENC_HW_SRC:
            ...
            break;
        case JPEG_ENC_HW_SRC_JPEG:
            PREPARE_DST_BUFFER(JPEG_ENC_HW_DST_JPEG, dstDataType, imgRequest);
            break;
        case JPEG_ENC_HW_SRC_JPS:
            PREPARE_DST_BUFFER(JPEG_ENC_HW_DST_JPS, dstDataType, imgRequest);
            break;
        default:
