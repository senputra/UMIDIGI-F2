...
#define DONT_USE_QUERYSUGGBURSTQNUM
#include <mtkcam/imageio/ispio_utility.h>
#undef DONT_USE_QUERYSUGGBURSTQNUM
...
class IspSyncControlImp : public IspSyncControlHw
{
    ...
    public: //operations
        ...
        //
        typedef struct _PASS1_METADATA_INFO
        {
            MUINT32                 magicNum;
            IImageBuffer*           pImgBuf;
            ResultMetadata          metadata; // ResultMetadata has constructor
            MBOOL                   isRrzo;
            EIS_CROP_INFO           eisCrop;
            EIS_STATISTIC_STRUCT    eisStatistic;
            //
            _PASS1_METADATA_INFO()
            {
                magicNum = 0;
                pImgBuf = nullptr;
                isRrzo = MFALSE;
                ::memset(&eisCrop,      0, sizeof(EIS_CROP_INFO));
                ::memset(&eisStatistic, 0, sizeof(EIS_STATISTIC_STRUCT));
            }
        }PASS1_METADATA_INFO;
        //
        typedef enum
...
getPass1PureRawStatus(MUINT32 magicNum)
{
    ...
    FUNC_NAME;
    MBOOL ret = MTRUE;
    if (!mbPureRaw || muPureRawPeriod==0)
    {
        MY_LOGD("mbPureRaw(%d),muPureRawPeriod(%d)", mbPureRaw, muPureRawPeriod);
        ret =MFALSE;
    }
    else if ((magicNum % muPureRawPeriod) == 0)
    {
        ret = MTRUE;
    }
    else
