...

    {
        ...
        switch( data )
        {
            ...
            case SHOTCB_JPS_SRC:
                ...
                MY_LOGD("setStatus(pPicCB->muFlag, JPS, PENDING)");
                break;
            case SHOTCB_DBGINFO_1_SRC:
                if( muInternalFlag & USE(DEBUGINFO_JPS) ){
                    setStatus(pPicCB->muFlag, DEBUGINFO_JPS, PENDING);
                    MY_LOGD("setStatus(pPicCB->muFlag, DEBUGINFO_JPS, PENDING)");
                }
                break;
            default:
                break;
...
processCallback()
{
    ...
    if( muInternalFlag & USE(JPEG) ){
        MY_LOGD("process JPEG");
    }else{
        MY_LOGD("dont process JPEG");
    }
    if( muInternalFlag & USE(JPS) ){
        MY_LOGD("process JPS");
    }else{
        MY_LOGD("dont process JPS");
    }
