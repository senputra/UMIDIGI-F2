...
#define CREATE_TEMP_DBG_INFO(var)                                                       \
    std::unique_ptr<IDbgInfoContainer, std::function<void(IDbgInfoContainer*p)> > var(  \
            IDbgInfoContainer::createInstance(),                                        \
            [](IDbgInfoContainer *p){if(p)p->destroyInstance();})

// workaround
#define updateRegToDebugInfo(x)  do{}while(0)
...
#define MY_LOGI(fmt, arg...)    do {ALOGI(MFLL_HAL_TAG fmt, ##arg);} while (0)
#define MY_LOGD(fmt, arg...)    do {ALOGD(MFLL_HAL_TAG fmt, ##arg);} while (0)
#define MY_LOGW(fmt, arg...)    do {ALOGW(MFLL_HAL_TAG fmt, ##arg);} while (0)
#define MY_LOGE(fmt, arg...)    do {ALOGE("error" MFLL_HAL_TAG fmt, ##arg);} while (0)
#define FUNCTION_LOG_START      MY_LOGD("[%s] - E.", __FUNCTION__)
#define FUNCTION_LOG_END        do {if(__builtin_expect(!ret, false)) MY_LOGE("[%s] fail", __FUNCTION__); MY_LOGD("[%s] - X. ret=%d", __FUNCTION__, ret);} while(0)
#define FUNCTION_LOG_END_NO_RET do {MY_LOGD("[%s] - X.", __FUNCTION__);} while(0)
#define FUNCTION_LOG_END_MUM    MY_LOGD("[%s] - X.", __FUNCTION__)

static const int g_enableLog = ::property_get_int32("vendor.mfll.db_level", 2);
#define MY_LOGD_IF(fmt, arg...) \
    do { \
        if (__builtin_expect( g_enableLog >= 3, false )) { \
            MY_LOGD(fmt, ##arg); \
        }\
    } while(0)
#define FUNCTION_LOG_START_IF   MY_LOGD_IF("[%s] - E.", __FUNCTION__)
#define FUNCTION_LOG_END_IF     do {if(__builtin_expect(!ret, false)) MY_LOGE("[%s] fail", __FUNCTION__); MY_LOGD_IF("[%s] - X. ret=%d", __FUNCTION__, ret);} while(0)
#define FUNCTION_LOG_END_MUM_IF MY_LOGD_IF("[%s] - X.", __FUNCTION__)
//
...
#define msleep(ms)              usleep(((int64_t)ms)*1000)

#define CHECK_OBJECT(x)         do{ if (__builtin_expect( x == NULL , false)) { MY_LOGE("%s is Null Object", #x); ret = MFALSE; goto lbExit;}}while(0)
#define CHECK_RET(fmt, arg...)  do{ if (__builtin_expect( !ret, false)){ MY_LOGE(fmt, ##arg); ret = MFALSE; goto lbExit; }}while(0)
...
// change to CPUCtrl (reference mt6761)
...
init()
{
    ...
    //
    property_get("vendor.mediatek.mfll.dump.all", value, "0");
    MBOOL dumpAll = atoi(value) || TEST_DUMP;
    //
    property_get("vendor.mediatek.mfll.dump.raw", value, "0");
    MBOOL dumpRaw = atoi(value) || dumpAll || TEST_DUMP_RAW;
    property_get("vendor.mediatek.mfll.dump.yuv", value, "0");
    MBOOL dumpYuv = atoi(value) || dumpAll || TEST_DUMP_YUV;
    property_get("vendor.mediatek.mfll.dump.mfb", value, "0");
    MBOOL dumpMfb = atoi(value) || dumpAll || TEST_DUMP_MFB;
    property_get("vendor.mediatek.mfll.dump.mixer", value, "0");
    MBOOL dumpMixer = atoi(value) || dumpAll || TEST_DUMP_MIXER;
    property_get("vendor.mediatek.mfll.dump.jpeg", value, "0");
    MBOOL dumpJpeg = atoi(value) || dumpAll || TEST_DUMP_JPEG;
    property_get("vendor.mediatek.mfll.dump.postview", value, "0");
    MBOOL dumpPostview = atoi(value) || dumpAll || TEST_DUMP_POSTVIEW;
    property_get("vendor.mediatek.mfll.dump.exif", value, "0");
    MBOOL dumpExif = atoi(value) || TEST_DUMP_EXIF;
    property_get("vendor.mediatek.mfll.dump.15raw", value, "0");
    MBOOL dump15Raw = atoi(value) || TEST_DUMP_15RAW;
    property_get("vendor.mediatek.mfll.dump.rwb", value, "0");
    ...
    if(dumpRwb)
        mDump |= MFLL_DUMP_RWB;
	MY_LOGD_IF("[updateInfo] - mDump=0x%x", mDump);

    //
    property_get("vendor.mediatek.mfll.dump.sdcard", value, TEST_DUMP_TO_SDCARD);
    ...
    property_get("vendor.mediatek.mfll.skipmfb", value, "0");
    mSkipMfb = atoi(value);
    if(mSkipMfb) {
    ...
    property_get("vendor.mediatek.mfll.bss", value, "-1");
    if(atoi(value) != -1) {
    ...
    ********************************************************************************/
    {
        ...
        MY_LOGD_IF("mfllthread all(%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld)"
                , mThreadMemory
                , mThreadCapture
        ...
        MY_LOGD_IF("mfllthread mem(%ld), cap(%ld), capPostProc(%ld), rawToYuy2(%ld), yuy2ToYv16(%ld) mfll(%ld), memc(%ld), blend(%ld), pv(%ld), full(%ld), thumbnail(%ld)"
                , mThreadMemory
                , mThreadCapture
                , mThreadCapturePostProc
                , mThreadRawToYuy2
...
sendCommand(MINT32 cmd, MINT32 arg1, MINT32 arg2, MINT32 arg3)
{
    ...
    MY_LOGD("sendCommand(0x%x) arg(%d,%d,%d)", cmd, arg1, arg2, arg3);
    ...
    switch(cmd) {
        case ECamShot_CMD_SET_NRTYPE:
            ...
            /*
             * 2018MAY15: force set NR mode to SWNR if requested HWNR  because
             * HWNR is not good enough.
             */
            if (arg1 == ECamShot_NRTYPE_HWNR) {
                MY_LOGD("force set NR type to SWNR due to requested HWNR");
                muNRType = ECamShot_NRTYPE_SWNR;
            }
            break;

        default:
            MY_LOGW("unsupported command 0x%x", cmd);
            ret = MFALSE;
            break;
    }
...
flowMfll()
{
    ...
    handleDataCallback(ECamShot_DATA_MSG_YUV
                        , 0
                        , (MUINTPTR)mDebugExif.getDbgContainer()
    ...
    ret = ret && setDebugInfo(mDebugExif.getDbgContainer());
...
flowParallelMemc(unsigned int ref)
{
    FUNCTION_LOG_START;
    MBOOL 	ret = MTRUE;
    MY_LOGD("parallelMemcProc %d", ref);
...
flowFull()
{
    ...
    WAIT(&mMemoryReady_Jpeg); // wait JPEG memory ready
    ret = ret && doFullJpeg();
...
flowThumbnail()
{
    ...
    WAIT(&mMemoryReady_Jpeg); // wait JPEG thumbnail ready
...
void MfllShot::readProperty(void)
{
    ...
    property_get("vendor.mediatek.mfll.mrp", p, "-1");
    v = atoi(p);
    if (v != -1) {
        switch (v) {
        case eMrpMode_None:
    ...
    /* RWB sensor support */
    property_get("vendor.mediatek.mfll.rwb", p, "-1");
    v = atoi(p);
    if (v != -1) {
        switch(v){
        case 0:
    ...
    /* Parallel MEMC */
    property_get("vendor.mediatek.mfll.pmm", p, "-1");
    v = atoi(p);
    if (v != -1) {
    ...
    property_get("vendor.mediatek.mfll.big", p, "-1");
    v = atoi(p);
    if (v != -1) {
    ...
    /* Dual Phase */
    property_get("vendor.mediatek.mfll.dualphase", p, "-1");
    v = atoi(p);
    if (v != -1) {
        mDualPhase = (v != 0 ? 1 : 0);
    }
...
void MfllShot::dualphaseHandleJpeg(void)
{
    ...
    IDbgInfoContainer* pDbg = mDebugExif.getDbgContainer();
...
initInStartOne(SensorParam const & rSensorParam)
{
    ...
    ********************************************************************************/
    ...
    {
        char value[PROPERTY_VALUE_MAX] = {'\0'};
    	property_get("vendor.mediatek.mfll.nr", value, "-1");
	...
	switch(msg.msgType) {
        ...
        case NSCamShot::ECamShot_DATA_MSG_YUV:
            ...
            {   //dbginfo
                if(mPass2Index == 0) {
                    IDbgInfoContainer* pDbgInfo = reinterpret_cast<IDbgInfoContainer*>(msg.ext2);
                    pDbgInfo->copyTo(mDebugExif.getDbgContainer());
                }
            }
            ANNOUNCE(&mCapP2Ready[mPass2Index]);
            ++mPass2Index;
...
doEis(MUINT32 ref, MINT32 &gmvX, MINT32 &gmvY)
{
	...
	for(MUINT32 j=0; j<EIS_MAX_WIN_NUM; j++)
	{
        ...
        MY_LOGD_IF("win(%d) lmv(%d,%d) trust(%d,%d)"
                , j
    ...
    MY_LOGD("eis_trush_th(%d),CUST_MFLL_EIS_OP_STEP_H(%d),CUST_MFLL_EIS_OP_STEP_V"
...
doMfb(MUINT32 ref)
{
...
#if TEST_3A
    // update 3A profile
    IHal3A_unique_ptr_T p3A(
            IHal3A::createInstance(IHal3A::E_Camera_1, mSensorParam.u4OpenID, LOG_TAG),
            mIHal3aDestructor
            );
    if(p3A.get() == nullptr) {
...

    {
        ...
        p3A->setIspProfile(_3A_profile, dbginfo.get());
        mDebugExif.setData(dbginfo.get(), MfllDebugExif::STAGE_MFB);
    }
#endif
        ...
        MfllDebugExif::STAGE stage /* = MfllDebugExif::STAGE_UNDEFINED */)
{
    ...
    MY_LOGD("srcRect xywh(%d,%d,%d,%d),dstRect xywh(%d,%d,%d,%d),cropRect(%d,%d,%d,%d)"
...

    {
        ...
        switch (stage) {
        case MfllDebugExif::STAGE_BFBLD:
            // we copy debug info of stage Before Blend as default debug info
            mDebugExif.copyDbgInfo(dbginfo.get());
            // and we also need to update MF data
        case MfllDebugExif::STAGE_AFBLD:
            mDebugExif.setData(dbginfo.get(), stage); // update MF data
            break;
        default:;
        }
    }
...
doMixing()
{
...

    {
        ...
        p3A->setIspProfile(_3A_profile, dbginfo.get());
        mDebugExif.setData(dbginfo.get(), MfllDebugExif::STAGE_AFBLD);
    }
#endif
			...
			)
{
    ...
    MY_LOGD("Src WH(%d %d), Des WH(%d %d)"
            , pSrcImgBufInfo->getImgSize().w, pSrcImgBufInfo->getImgSize().h
            , pJpgImgBufInfo->getImgSize().w, pJpgImgBufInfo->getImgSize().h
            );
			...
			, MUINT32 & u4JpegSize)
{
    ...
    if (__builtin_expect( pSrcImgBufInfo.get() == nullptr, false )) {
        MY_LOGE("pSrcImgBufInfo is nullptr");
        *(volatile int32_t*)(0x0) = 0xdeadfeed;
    }
    if (__builtin_expect( pJpgImgBufInfo.get() == nullptr, false )) {
        MY_LOGE("pJpgImgBufInfo is nullptr");
        *(volatile int32_t*)(0x0) = 0xdeadfeed;
    }
...
readTuningData(NVRAM_CAMERA_FEATURE_MFLL_STRUCT &rTuningData, MUINT32 sensorId)
{
    ...
    MY_LOGD_IF("readTuningData - from nvram");
    int err;
    ...
    MY_LOGI("readTuningData - max_frame(%d), clip(%d), bad_mv_range(%d), bad_mv_rate_th(%d), "\
            "mfll_iso_th(%d), ais_exp_th(%d), adv_en(%d), adv_iso(%d), adv_exp(%d), "\
            "ais_exp_th0(%d), ais_iso_th0(%d) "
...
getCaptureInfo(MUINT32 mfbMode, MUINT32 sensorId, MUINT32 &frameNumber, MUINT32 &exp, MUINT32 &iso,MBOOL &needUpdateAE)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
    MY_LOGI("mfbMode(%d), sensorId(%d)", mfbMode, sensorId);
    ...
    {
    	char value[PROPERTY_VALUE_MAX] = {'\0'};
    	property_get("vendor.mediatek.mfll.exp", value, "-1");
    	MINT32 forceExp = atoi(value);
        if(forceExp!=-1) {
            exp = forceExp * 1000;
        }
    	property_get("vendor.mediatek.mfll.iso", value, "-1");
    	MINT32 forceIso = atoi(value);
        if(forceIso!=-1) {
            iso = forceIso;
        }
    }
    MY_LOGI("precapture exp(%d) iso(%d)", exp, iso);
    ...
    MY_LOGI("final exp(%d) iso(%d)", exp, iso);
    ...
    {
    	char value[PROPERTY_VALUE_MAX] = {'\0'};
    	property_get("vendor.mediatek.mfll.force", value, "-1");
...
lbExit:
    MY_LOGI("mfbMode(%d), frameNumber(%d), iso_th(%d), iso(%d)", mfbMode, frameNumber, rTuningData.mfll_iso_th, iso);
    FUNCTION_LOG_END;
    return ret;
}
...
// utility: exif
//------------------------------------------------------------------------------
inline void setDebugTag(MfllDebugExif& rMfDebugInfo, MINT32 a_i4ID, MINT32 a_i4Value)
{
    rMfDebugInfo.setData(a_i4ID, a_i4Value);
}

MBOOL
MfllShot::
setDebugInfo(IDbgInfoContainer* pDbgInfoCtnr)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;

    {   //save mfll, bss, memc, ais as tag-value format
        MfllDebugExif& rMfDebugInfo = mDebugExif;
    ...
    {   //save gmv, eis, reg as hex list
        ...
        MVOID* pBuf = NULL;
        pDbgInfoCtnr = mDebugExif.getDbgContainer();
        if (pDbgInfoCtnr) {
