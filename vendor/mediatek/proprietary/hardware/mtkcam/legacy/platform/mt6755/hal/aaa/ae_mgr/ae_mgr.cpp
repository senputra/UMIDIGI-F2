...
strAEOutput rAEOutput;
    ...
    copyAEInfo2mgr(&m_rAEOutput.rPreviewMode, &rAEOutput);
    MY_LOG("[getPreviewParams3] Exp. time: %d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n",
...
//MINT32 i4LVValue_10x = 0;  /* fix build warning */
...
MUINT16 u2FrameRate = 300;
    ...
    MY_LOG_IF(m_3ALogEnable,"[updateAEInfo2ISP] State:%d Shutter:%d Gain:%d %d %d Flare offset:%d %d FrameDuration:%lld \n", eNewAEState, rAEInfo2ISP.u4Eposuretime, rAEInfo2ISP.u4AfeGain,
        rAEInfo2ISP.u4IspGain, rAEInfo2ISP.u4RealISOValue, rAEInfo2ISP.i2FlareOffset, i2FlareGain, (long long)m_rSensorCurrentInfo.u8FrameDuration);
    return S_AE_OK;
}
