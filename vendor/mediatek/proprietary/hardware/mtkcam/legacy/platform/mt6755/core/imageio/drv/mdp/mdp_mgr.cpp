...
//            cfgData.srcCropX,cfgData.srcCropFloatX,cfgData.srcCropY,cfgData.srcCropFloatY,cfgData.srcCropW,cfgData.srcCropH,cfgData.mdpSrcPlaneNum);
    ...
    if ((cfgData.dstPortCfg[0] == 1) && (cfgData.dstPortCfg[1] == 1))
    {
        if (cfgData.dstDma[0].srcCropX > cfgData.dstDma[1].srcCropX)
        {
            ...
            {
                LOG_INF("(Before)cfgData.dstDma[0].srcCrop(%u, %u), cfgData.dstDma[1].srcCrop(%u, %u)", cfgData.dstDma[0].srcCropX, cfgData.dstDma[0].srcCropW, cfgData.dstDma[1].srcCropX, cfgData.dstDma[1].srcCropW);
                cfgData.dstDma[0].srcCropX = cfgData.dstDma[1].srcCropX;
                cfgData.dstDma[0].srcCropW = cfgData.dstDma[1].srcCropW;
                LOG_INF("(After)cfgData.dstDma[0].srcCrop(%u, %u), cfgData.dstDma[1].srcCrop(%u, %u)", cfgData.dstDma[0].srcCropX, cfgData.dstDma[0].srcCropW, cfgData.dstDma[1].srcCropX, cfgData.dstDma[1].srcCropW);
            }
        }
        else
        {
...
//         if (cfgData.dstDma[index].withCropM && cfgData.dstDma[index].enSrcCrop)
        {
            ...
            switch (cfgData.dstDma[index].capbility)
            {
                case NSImageio::NSIspio::EPortCapbility_Rcrd:
                    ...
                    err2 = pDpStream->setSharpness(index, 0);    // Set sharpness disable because change capbility issue
                    break;
            ...
            if (cfgData.dstDma[index].memBuf.base_vAddr == 0 || cfgData.dstDma[index].memBuf.base_pAddr == 0) {
                ...
                LOG_ERR("dst memAddr is 0,VA(0x%" PRIxPTR "),PA(0x%" PRIxPTR ")",
                    cfgData.dstDma[index].memBuf.base_vAddr, cfgData.dstDma[index].memBuf.base_pAddr);
            ...
            LOG_INF("dstSizeList=(0x%8x,0x%8x,0x%8x),dstVirList=(%p,%p,%p),dstPhyList=(0x%8x,0x%8x,0x%8x),plane num(%d)",
...
MINT32 MdpMgrImp::startMdp(MDPMGR_CFG_STRUCT cfgData)
{
    ...
    LOG_DBG("+,cqIndx(%u),sceID(%u),cqPA(0x%x),tpipVa(%p),tpipPa(0x%x),isWaitBuf(%d),venc(%d)", \
...
DpIspStream *MdpMgrImp::allocDpStream(MUINT32 sceID)
{
    ...
    LOG_DBG("-,pRetDpIspStream(%p)", pRetDpIspStream);
