...
//********************************************************************************************//
    ...
    {
        srcHeight = 480;
        realWidth = srcHeight * pv_srcWidth / pv_srcHeight;
    }
    // for 1:1 --
//********************************************************************************************//
    ...
    MY_LOGD_IF(mLogLevel, "Do FD done");
    if(ret1)
        return false;
...
//********************************************************************************************//
    ...
    MY_LOGD_IF(mLogLevel, "Scenario SD Result1: %d",SD_Result );
    if(SD_Result>0)
        rIsDetected_SD = 1;
   else
        rIsDetected_SD = 0;
...
//                   relativeRatio <= fMaxRelativeRatio && relativeRatio >= (1.0 / fMaxRelativeRatio) )
    ...
    if (realWidth != 0)
    {
        ...
        {
            mpDetectedFaces->faces[i].rect[0] = ((mpDetectedFaces->faces[i].rect[0] + 1000) * srcWidth / realWidth) - 1000;
            mpDetectedFaces->faces[i].rect[2] = ((mpDetectedFaces->faces[i].rect[2] + 1000) * srcWidth / realWidth) - 1000;
        }
    }
    // for 1:1 --
...
doBufferAnalysis(ImgBufQueNode const& a_rQueNode)
{
    ...
    MY_LOGD_IF(mLogLevel, "plane: %d",plane );
