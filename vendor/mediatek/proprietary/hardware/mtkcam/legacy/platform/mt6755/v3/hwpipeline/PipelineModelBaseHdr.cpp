...
submitRequest(AppRequest& request, int SubRequestCnt)
{
    ...
    if  ( OK != err ) {
       ...
       for ( size_t i=0; i<params.vFrame.size(); i++ ) {
            sp<IPipelineFrame> pFrame = params.vFrame[i];
            MY_LOGE_IF(pFrame==0, "RequestNo:%u-%zu - NULL PipelineFrame", request.requestNo, i);
            MY_LOGD_IF(
                1,
                "RequestNo:%u-%zu Image:I|O=%zu|%zu Meta:I|O=%zu|%zu",
