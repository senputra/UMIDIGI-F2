...
int FlashMgrM::initSub(int sensorId, int isTemp)
{
    ...
    // set duty offset MA
    g_pStrobe->setDutyOffsetMA(mpStrobeNvbuf->engTab.dutyOffsetMA);
    for(int i=0;i<10;i++){
        logV("offset_mgr[%d]=%d",i, mpStrobeNvbuf->engTab.dutyOffsetMA[i]);
    }
#endif
    if(cust_isDualFlashSupport(m_sensorDev)==1)
    {
...
int FlashMgrM::doPfOneFrame(FlashExePara* para, FlashExeRep* rep)
{
    ...
    if(m_pfFrameCount==0)
    {
        ...
        else
        {
            ...
            ratioQEn = FlashUtil::getPropInt("vendor.flash_ratio_quick",0);
...
#ifdef FLASHLIGHT_OFFSET_CALI
        if (m_sensorDev==ESensorDev_Sub)
            err = cctCalibration_Sub(para, rep);
        else
#endif
