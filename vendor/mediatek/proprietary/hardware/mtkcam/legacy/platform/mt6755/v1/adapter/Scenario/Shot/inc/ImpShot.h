...
protected:  ////
    ...
    /**
     *  For ZSD mode, if post processing is much longer, the 3A information may
     *  be wrong because 3A information of Exif is being updated while encoding
     *  JPEG which means, if 3A has been changed at this time, the Exif information
     *  will be the one updated which is not we want.
     *
     *  Hence here are the two new functions to solve this problem.
     *  1: makeCamExif,         To make a CamExif instance contains 3A information
     *  2: makeExifHeaderEx,    To create Exif header with an existed CamExif instance
     *
     *  Caller can make CamExif instance with the correct timing, and make Exif
     *  header later by these two functions
     */

    /**
     *  Make CamExif (3A information Exif)
     *  @param u4CamMode        Cam mode, usually set as eAppMode_PhotoMode
     *  @param [out] rCamExif   The output CamExif, call by reference
     *  @return                 Returns ture if OK
     */
    virtual bool      makeCamExif(MUINT32 const u4CamMode, CamExif &rCamExif);

    /* make exif header with an existed CamExif instance */
    virtual bool      makeExifHeaderEx(MUINT32 const u4CamMode,
                                        MUINT8* const puThumbBuf,
                                        MUINT32 const u4ThumbSize,
                                        MUINT8* puExifBuf,
                                        MUINT32 &u4FinalExifSize,
                                        CamExif &rCamExif, // Exif instance
                                        IDbgInfoContainer* pDbg = NULL,
                                        MUINT32 u4ImgIndex = 0,
                                        MUINT32 u4GroupId = 0,
                                        MUINT32 u4FocusValH = 0,
    ...
    virtual bool      makeExifHeader(MUINT32 const u4CamMode,
                                        MUINT8* const puDbgBuf, 
                                        MUINT32 const u4DbgSize, 
                                        MUINT8* puExifBuf,
                                        MUINT32 &u4FinalExifSize,
    ...
    virtual MUINT32 queryGpuProcEn() const;
