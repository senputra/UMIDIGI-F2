...
onCmd_capture()
{
    ...
    //
    MBOOL enGpuProc = queryGpuProcEn();
    //
    pSingleShot->init();
    //
...
#undef  scenario_case
    ...
    MINT32 iOperMode = pHal3a->send3ACtrl(NS3A::E3ACtrl_GetOperMode, 0, 0);
    pHal3a->destroyInstance(LOG_TAG); 
    // Align Jpeg Size to RAW Size
    //Only when raw save and in normal mode
    if((mpEngParam->mi4EngRawSaveEn == 1 && getBootMode() == 0 && iOperMode != 2 /*EOperMode_Meta*/) && 
    ...
    else if ((mpEngParam->mi4EngRawSaveEn == 1 && getBootMode() == 0 && iOperMode != 2 /*EOperMode_Meta*/) && 
