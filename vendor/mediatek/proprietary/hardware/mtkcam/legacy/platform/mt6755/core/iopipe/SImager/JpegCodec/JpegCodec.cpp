...
EXIT:
    ...
    if(!ret)
    {
        ...
        MY_LOGE("fmt 0x%x, wxh %dx%d, va 0x%" PRIxPTR "/0x%" PRIxPTR ", pa 0x%" PRIxPTR "/0x%" PRIxPTR " fd %d/%d, size %zu/%zu",
        ...
        MY_LOGE("quality %u, dstV/P 0x%" PRIxPTR "/0x%" PRIxPTR ", size %zu, soi %d",
                 u4Quality,
...
using namespace NSCam::Utils::Format;
    ...
    *pBuf = allocator->alloc_ion("allocYuvBuf", imgParam);
    ...
    //
    MY_LOGD("<YUV> ImgBitsPerPixel:%zu BufSizeInBytes:%zu",
        (*pBuf)->getImgBitsPerPixel(), (*pBuf)->getBufSizeInBytes(0));
    return  MTRUE;
}
