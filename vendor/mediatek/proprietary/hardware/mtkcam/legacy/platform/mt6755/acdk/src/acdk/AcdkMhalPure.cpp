...
#ifdef __LP64__
    pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_OTHER, NICE_CAMERA_PASS1, {0}};
#else
...
MINT32 AcdkMhalPure::acdkMhalPreviewStart(MVOID *a_pBuffIn)
{
    ...
    {
        ACDK_LOGD("imgImemBuf  : virAddr[%d]=0x%" PRIxPTR,
            i, mAcdkMhalPrvParam.imgImemBuf[i].virtAddr);
        ACDK_LOGD("dispImemBuf : virAddr[%d]=0x%" PRIxPTR,
            i, mAcdkMhalPrvParam.dispImemBuf[i].virtAddr);
    }
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
        ...
        ACDK_LOGD("IMGO m_pICamIOPipe->enqueOutBuf[%d].u4BufPA(0x%" PRIxPTR ")",
            i, rQBufInfo.vBufInfo[0].u4BufVA[0]);
        if(is_yuv_sensor == 0) // raw sensor use rrzo
        {
            ...
            ACDK_LOGD("RRZO m_pICamIOPipe->enqueOutBuf[%d].u4BufPA(0x%" PRIxPTR ")",
                i, rQBufInfo.vBufInfo[0].u4BufVA[0]);
        }
    }
...
MINT32 AcdkMhalPure::acdkMhalCaptureStart(MVOID * a_pBuffIn)
{
    ...
    {
        if(timeoutcnt >= 3000000) //time=10min
        {
            ACDK_LOGD("wait mFocusDone done timeout");
...
MINT32 AcdkMhalPure::acdkMhalPreviewProc()
{
    ...
    while((acdkMhalGetState() & ACDK_MHAL_PREVIEW_MASK) || (ACDK_MHAL_PRE_CAPTURE == acdkMhalGetState()))
    {
        ...
        ACDK_LOGD_DYN(g_acdkMhalPureDebug,"[ROUND][dequeOutBuf][imgo]mID(0x%x),size(0x%x),addr(0x%" PRIxPTR "/0x%" PRIxPTR "),T(%d.%d),img(%d/%d/%d/%d),m_fc(0x%x/0x%x/%d)",
        ...
        else
        {
            ...
            for(MUINT32 i = 0; i < imgo_num; ++i)
            {
                ...
                ACDK_LOGD_DYN(g_acdkMhalPureDebug, "imgo_virAddr[%d](0x%" PRIxPTR "),imgo_phyAddr[%d](0x%" PRIxPTR ")",
                    i, imgo_virAddr[i], i, imgo_phyAddr[i]);
            }
            imgo_virA = imgo_virAddr[0];
            imgo_phyA = imgo_phyAddr[0];

            ACDK_LOGD("imgo_virAddr(0x%" PRIxPTR "),imgo_phyAddr(0x%" PRIxPTR ") size(%d)",
                imgo_virA, imgo_phyA, mAcdkMhalPrvParam.imgImemBuf[0].size);
        }
#ifdef ALL_SENSOR_USE_TG1
        mPortID.index = EPortIndex_RRZO;
#else
        ...
        ACDK_LOGD_DYN(g_acdkMhalPureDebug,"[ROUND][dequeOutBuf][rrzo]mID(0x%x),size(0x%x),addr(0x%" PRIxPTR "/0x%" PRIxPTR "),T(%d.%d),img(%d/%d/%d/%d),m_fc(0x%x/0x%x/%d)",
            rQTSBufInfo.vBufInfo[0].memID[0],
            rQTSBufInfo.vBufInfo[0].u4BufSize[0],
            rQTSBufInfo.vBufInfo[0].u4BufVA[0],
            rQTSBufInfo.vBufInfo[0].u4BufPA[0],
       ...
       else
       {
           ...
           for(MUINT32 i = 0; i < rrzo_num; ++i)
           {
               ...
               ACDK_LOGD_DYN(g_acdkMhalPureDebug, "rrzo_virAddr[%d](0x%" PRIxPTR "),rrzo_phyAddr[%d](0x%" PRIxPTR ")",
                   i, imgo_virAddr[i], i, imgo_phyAddr[i]);
           }
        ...
        if(dumpEnable == 1 || dumpEnable == 3)
        {
            ...
            ACDK_LOGD("prv VA(0x%" PRIxPTR ")", imgo_virAddr[0]);
    ...
    pipePackageInfo.isV3=0;
        ...
        {
            //====== Preview CallBack ======
            ACDK_LOGD_DYN(g_acdkMhalPureDebug, "dispo_virAddr(0x%" PRIxPTR "),dispo_phyAddr(0x%" PRIxPTR ")", dispo_virAddr, dispo_phyAddr);
        ...
        for(MUINT32 i = 0; i < imgo_num; ++i)
        {
            ACDK_LOGD_DYN(g_acdkMhalPureDebug, "pass1 enque imgo_virAddr[%d](0x%" PRIxPTR ")", i, imgo_virAddr[i]);
