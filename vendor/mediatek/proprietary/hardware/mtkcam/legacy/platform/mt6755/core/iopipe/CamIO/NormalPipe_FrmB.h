...
public:
    ...
    virtual MBOOL   configFrame(QFrameParam const& rQParam){(void)rQParam; return MFALSE;};

    //replace queue
    virtual MBOOL   replace(BufInfo const& bufOld, BufInfo const& bufNew){(void)bufOld; (void)bufNew; return MFALSE;};
