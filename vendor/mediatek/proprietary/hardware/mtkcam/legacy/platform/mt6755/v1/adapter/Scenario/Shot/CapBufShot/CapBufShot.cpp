...
)
{
    ...
    //  This command is to set p2out buffer.
    //
    //  Arguments:
    //      arg1
    //          [I] Pointer to IImageBuffer
    case eCmd_setP2outBuf:
        MY_LOGD("set p2out buf");
        p2outBuf = (IImageBuffer *)arg1;
        break;
...
onCmd_capture()
{
    ...
    if ( p2outBuf )
    {
        MY_LOGD("regist p2outBuf: (w,h)=(%d,%d)", p2outBuf->getImgSize().w, p2outBuf->getImgSize().h);
        pSingleShot->registerImageBuffer(NSCamShot::ECamShot_BUF_TYPE_YUV, p2outBuf);
        pSingleShot->sendCommand( NSCamShot::ECamShot_CMD_SET_P2OUT_BUF, MTRUE, 0, 0 );
    }
    else
    {
        MY_LOGD("p2outBuf is NULL");
    }
    //
    ...
    pSingleShot->enableDataMsg(
            NSCamShot::ECamShot_DATA_MSG_JPEG
                ...
                NSCamShot::ECamShot_DATA_MSG_META : NSCamShot::ECamShot_DATA_MSG_NONE)
#endif
#if 0
    ...
    {
        mpCapBufMgr->enableCompareShotTime(MTRUE);
        MINT32 pass2Style = NSCamShot::ECamShot_CMD_STYLE_NORMAL;
        switch( nrtype )
        {
            case NSCamShot::ECamShot_NRTYPE_NONE:
                pass2Style = NSCamShot::ECamShot_CMD_STYLE_NORMAL;
                break;
            case NSCamShot::ECamShot_NRTYPE_HWNR:
                pass2Style = NSCamShot::ECamShot_CMD_STYLE_VSS;
                break;
            case NSCamShot::ECamShot_NRTYPE_SWNR:
                pass2Style = NSCamShot::ECamShot_CMD_STYLE_NORMAL;
                break;
            default:
                pass2Style = NSCamShot::ECamShot_CMD_STYLE_NORMAL;
        ...
        CAM_LOGD("ZSD Shot, pass2Style=%d",pass2Style);
    ...
    //
    ret = pSingleShot->sendCommand( NSCamShot::ECamShot_CMD_SET_RWB_PROC_TYPE, enGpuProc, 0, 0 );
    //
...
fgCamShotNotifyCb(MVOID* user, NSCamShot::CamShotNotifyInfo const msg)
{
    ...
    if (NULL != pCapBufShot)
    {
        ...
        {
            pCapBufShot->mpShotCallback->onCB_P2done();
            return MFALSE;
        }
    }
...
fgCamShotDataCb(MVOID* user, NSCamShot::CamShotDataInfo const msg)
{
    ...
    if (NULL != pCapBufShot)
    {
        ...
        {
#if DNG_META_CB
            if(pCapBufShot->isDataMsgEnabled(NSShot::EIShot_DATA_MSG_RAW))
        ...
        {
            pCapBufShot->handleDNGMetaCB(msg.ext1);//metadata structure
        }
#endif
    }
