...
)
{
    ...
    if (0 == callerPostProc || ! ((NSImageio::NSIspio::IPostProcPipe *)callerPostProc)->init())
    {
    ...
    if(mInitCount==1)
    {
        ...
        LOG_DBG("[m_pIMemDrv]:%p", m_pIMemDrv);
        if ( NULL == m_pIMemDrv ) {
        ...
        if(!gTuningQueNode.empty() || !gTuningQueData.empty()) {
            ...
            for(int i=0;i<DEFAULT_P2_TUNING_QUE_BUFFER_NUM;i++) {
                ...
                LOG_INF("[TuningQue]i(%d),VA(0x%" PRIxPTR "),size(0x%x)\n",
    ...
    NSImageio::NSIspio::IPostProcPipe * /*callerPostProc*/)
{
    ...
    if(mInitCount==0)
    {
        ...
        if(mTuningQueNum != gTuningQueData.size()){
            LOG_ERR("[Error]gTuningQueData number(%d) is different from that be alloced(%zu)",
                mTuningQueNum, gTuningQueData.size());
            return -1;
        }
        if(mTuningQueNum != gTuningQueNode.size()){
            LOG_ERR("[Error]gTuningQueNode number(%d) is different from that be alloced(%zu)",
        ...
        for (unsigned int i = 0; i < mTuningQueNum; i++) {
            if((gTuningQueData[i].bufferInfo.memID>=0) && (gTuningQueData[i].bufferInfo.virtAddr!=0)){
                ...
                LOG_ERR("[Error]free TuningQue error i(%d),fd(%d),va(0x%" PRIxPTR ")",
                    i, gTuningQueData[i].bufferInfo.memID, gTuningQueData[i].bufferInfo.virtAddr);
            }
        }
        gTuningQueData.erase(gTuningQueData.begin(),gTuningQueData.end());
        gTuningQueNode.erase(gTuningQueNode.begin(),gTuningQueNode.end());
        LOG_DBG("[TuningQueSize]%zu-%zu", gTuningQueData.size(), gTuningQueNode.size());
    ...
    QFeatureData& rFeatureData)
{
    ...
    LOG_INF("bufPackageNum(%d),misV3(%d),magic(%zu),tuningData(%zu)",
        bufPackageNum, misV3, rParams.mvMagicNo.size(), rParams.mvTuningData.size());
    //
    ...
    {    //original version
        ...
        for (unsigned int i = 0; i < rParams.mvIn.size(); i++)
        {
            inPorts+=rParams.mvIn[i].mBuffer->getPlaneCount();
        }
        for (unsigned int i = 0; i < rParams.mvOut.size(); i++)
        {
        ...
        MINT32 moduletag=0;
        for (unsigned int i = 0; i < rParams.mvModuleData.size(); i++)
        {
        ...
        for (unsigned int i = 0; i < rParams.mvIn.size(); i++)
        {
            ...
            switch(p2PortIdx)
            {
                case EPostProcPortIdx_IMGI:
                    if(rParams.mvCropRsInfo.size() >0)
                    {
                        ...
                        for (unsigned int k = 0; k < rParams.mvCropRsInfo.size(); k++)
                        {
            ...
            LOG_DBG("[mvIn (%d)] fmt(%d),w/h(%d/%d),oft(%zu,%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/pa(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/size(0x%x,0x%x,0x%x) )",
                ports[cnt].index, ports[cnt].eImgFmt, ports[cnt].u4ImgWidth, ports[cnt].u4ImgHeight,
                rParams.mvIn[i].mBuffer->getExtOffsetInBytes(0), ports[cnt].xoffset, ports[cnt].yoffset,
                ports[cnt].u4Stride[0], ports[cnt].u4Stride[1], ports[cnt].u4Stride[2],
                ports[cnt].capbility,
                ports[cnt].u4BufVA[0], ports[cnt].u4BufVA[1], ports[cnt].u4BufVA[2],
                ports[cnt].u4BufPA[0], ports[cnt].u4BufPA[1], ports[cnt].u4BufPA[2],
        ...
        for (unsigned int i = 0; i < rParams.mvOut.size(); i++)
        {
            ...
            LOG_DBG("[mvOut (%d)] fmt(%d),w/h(%d/%d),oft(%zu/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/pa(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/size(0x%x,0x%x,0x%x) )",
                ports[cnt].index, ports[cnt].eImgFmt, ports[cnt].u4ImgWidth, ports[cnt].u4ImgHeight,
                rParams.mvOut[i].mBuffer->getExtOffsetInBytes(0), ports[cnt].xoffset, ports[cnt].yoffset,
                ports[cnt].u4Stride[0], ports[cnt].u4Stride[1], ports[cnt].u4Stride[2],
                ports[cnt].capbility,
                ports[cnt].u4BufVA[0], ports[cnt].u4BufVA[1], ports[cnt].u4BufVA[2],
                ports[cnt].u4BufPA[0], ports[cnt].u4BufPA[1], ports[cnt].u4BufPA[2],
        ...
        for (unsigned int i = 0; i < rParams.mvModuleData.size(); i++)
        {
            ...
            else
            {
                ...
                {
                    case NSImageio::NSIspio::EP2Module_STA_FEO:
                        LOG_DBG("[mvOut (%d)] fmt(%d),w(%d),h(%d),stride(%d/%d/%d),cap(%d),mem( va(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/pa(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/size(0x%x,0x%x,0x%x) )",
                            ports[cnt].index, ports[cnt].eImgFmt, ports[cnt].u4ImgWidth, ports[cnt].u4ImgHeight,
                            ports[cnt].u4Stride[0], ports[cnt].u4Stride[1], ports[cnt].u4Stride[2],
                            ports[cnt].capbility,
                            ports[cnt].u4BufVA[0], ports[cnt].u4BufVA[1], ports[cnt].u4BufVA[2],
                        ...
                        break;
                    case NSImageio::NSIspio::EP2Module_STA_LCEI:
                        LOG_DBG("[mvIn (%d)] fmt(%d),w(%d),h(%d),stride(%d/%d/%d),cap(%d),mem( va(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/pa(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")/size(0x%x,0x%x,0x%x) )",
                            ports[cnt].index, ports[cnt].eImgFmt, ports[cnt].u4ImgWidth, ports[cnt].u4ImgHeight,
                            ports[cnt].u4Stride[0], ports[cnt].u4Stride[1], ports[cnt].u4Stride[2],
                            ports[cnt].capbility,
                            ports[cnt].u4BufVA[0], ports[cnt].u4BufVA[1], ports[cnt].u4BufVA[2],
    ...
    {   //enque by vector, v3 and v1 [v3 only support vector enque]
        ...
        for (int q = 0; q < bufPackageNum; q++)
        {
            //mvin
            tmpC=0;
            for (unsigned int i = inStartIdx; i< rParams.mvIn.size() ;i++)
            {
            ...
            //mvout
            tmpC=0;
            for (unsigned int i = outStartIdx; i < rParams.mvOut.size(); i++)
            {
                //check cur element is next buffer group or not
                if(tmpC==0)
                {    //first element in this buffer group
        ...
        for(int q=0;q<bufPackageNum;q++)
        {
                ...
                switch(p2PortIdx)
                {
                    case EPostProcPortIdx_IMGI:
                        if(rParams.mvCropRsInfo.size() >0)
                        {
                            ...
                            // add for crop4 -----
                            for (unsigned int k = crop1StartIdx; k < rParams.mvCropRsInfo.size(); k++)
                            {
                                ...
                                switch(rParams.mvCropRsInfo[k].mGroupID)
                                {
                                    ...
                                    case 3:
                                        ...
                                        // add for crop4 +++++
                                    case 4:
                                        ports[cnt].crop4.x = rParams.mvCropRsInfo[k].mCropRect.p_integral.x;
                                        ports[cnt].crop4.floatX = rParams.mvCropRsInfo[k].mCropRect.p_fractional.x;
                                        ports[cnt].crop4.y = rParams.mvCropRsInfo[k].mCropRect.p_integral.y;
                                        ports[cnt].crop4.floatY = rParams.mvCropRsInfo[k].mCropRect.p_fractional.y;
                                        ports[cnt].crop4.w      = rParams.mvCropRsInfo[k].mCropRect.s.w;
                                        ports[cnt].crop4.h      = rParams.mvCropRsInfo[k].mCropRect.s.h;
                                        ports[cnt].resize4.tar_w = (rParams.mvCropRsInfo[k].mResizeDst.w == 0)?(rParams.mvCropRsInfo[k].mCropRect.s.w):(rParams.mvCropRsInfo[k].mResizeDst.w);
                                        ports[cnt].resize4.tar_h = (rParams.mvCropRsInfo[k].mResizeDst.h == 0)?(rParams.mvCropRsInfo[k].mCropRect.s.h):(rParams.mvCropRsInfo[k].mResizeDst.h);
                                        break;
                                        // add for crop4 -----
                                    default:
                        ...
                        else
                        {
                            ...
                            ports[cnt].crop4.y      = 0;
                            ports[cnt].crop4.floatY = 0;
                            ports[cnt].crop4.w      = 0;//ports[cnt].u4ImgWidth;
                            ports[cnt].crop4.h      = 0;//ports[cnt].u4ImgHeight;
                            ports[cnt].resize4.tar_w = 0;//ports[cnt].u4ImgWidth;
                            ports[cnt].resize4.tar_h = 0;//ports[cnt].u4ImgHeight;
                            // add for crop4 -----
                        }
                        LOG_DBG("[Crop] g1 x/fx/y/fy/w/h/tw/th(%d/%d/%d/%d/%d/%d/%d/%d),g2 x/fx/y/fy/w/h/tw/th(%d/%d/%d/%d/%d/%d/%d/%d),g2 x/fx/y/fy/w/h/tw/th(%d/%d/%d/%d/%d/%d/%d/%d)",\
                            ports[cnt].crop1.x,ports[cnt].crop1.floatX,ports[cnt].crop1.y,ports[cnt].crop1.floatY,ports[cnt].crop1.w,ports[cnt].crop1.h,ports[cnt].resize1.tar_w,\
            ...
            if (misV3){
                if (rParams.mvTuningData.size()==0) {
                    ...
                    LOG_DBG("q(%d),pTuningQue(%p)", q, pTuningQue);
                }
                //pass1 data
            ...
            else
            {
                ...
                if (bufPackage.idxINbatchLforBnode == (int)(mvEnqueuedBufBatch.size()))    //update real index for waited added buffer node in batchA
                {
...
#ifdef __LP64__
    LOG_DBG("timeout(%d),i8TimeoutNs(%ld),callerID(0x%lx),de_frameNo(%d)",
#else
    LOG_DBG("timeout(%d),i8TimeoutNs(%lld),callerID(0x%x),de_frameNo(%d)",
#endif
        timeout, i8TimeoutNs, callerID, rParams.mFrameNo);
    ...
    MINTPTR arg3)
{
    ...
    LOG_DBG("+,cmd(0x%x),arg1(0x%" PRIxPTR "),arg2(0x%" PRIxPTR "),arg3(0x%" PRIxPTR ")", cmd, arg1, arg2, arg3);
    ...
    void* &pTuningQueBuf)
{
    ...
    if(misV3)
    {
        ...
        if(!gTuningQueNode.empty()) {
            ...
            LOG_INF("[Warning]out of tuningque, add one more(0x%" PRIxPTR ")\n", gTuningQueData[gTuningQueData.size()-1].bufferInfo.virtAddr);
        }

        LOG_INF("[size]gTuningQueData(%zu),gTuningQueNode(%zu)\n", gTuningQueData.size(), gTuningQueNode.size());
    }
    else
    {
        LOG_ERR("v1 do not support this function");
    }
    //
    return ret;
}
    ...
    void* pTuningQueBuf)
{
    ...
    if (misV3)
    {
        ...
        if (isCheckPass == true) {
            ...
            LOG_ERR("[Error]tuningque buffer(%p) is invalid", pTuningQueBuf);
        }
        //
