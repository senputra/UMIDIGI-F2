...
init(MINT32 const i4SensorIdx)
{
    ...
    property_get("vendor.debug.aaa_sensor_buf_mgr.enable", value, "0");
...
updateDMABaseAddr(MUINT32 u4BaseAddr, MINT32 PDSel)
{
    ...
    if( PDSel==_PD_BUF_SEL_ && m_EnPDAF)
    {
        if( u4BaseAddr)
        {
            MY_LOG("%s [PD] m_eSensorDev(%d) u4BaseAddr=0x%x ISPAddr:0x%0x 0x%0x", __FUNCTION__, m_eSensorDev, u4BaseAddr, mpIspHwRegAddr, pisp);
        }
        else
    {
            MY_ERR("%s [PD] u4BaseAddr is NULL", __FUNCTION__);
    ...
    else if( PDSel!=_PD_BUF_SEL_ && m_EnMVHDR)
    {
        if( u4BaseAddr)
        {
            MY_LOG("%s [MVHDR] m_eSensorDev(%d) u4BaseAddr=0x%x ISPAddr:0x%0x 0x%0x", __FUNCTION__, m_eSensorDev, u4BaseAddr, mpIspHwRegAddr, pisp);
        }
    else
        {
    ...
    {
        MY_LOG("%s DEV(%d) Addr(%p), Sel(%d), En: CamSv1(%d), CamSv2(%d)", __FUNCTION__, m_eSensorDev, u4BaseAddr, PDSel, m_EnMVHDR, m_EnPDAF);
