...
int ef_test()
{
    ...
    QTimeStampBufInfo rQTSBufInfo;
        ...
        MINT32 err = pHalSensor->sendCommand(SENSOR_DEV_MAIN,
                                            ...
                                            0);
        if (err == -1)
            LOG_ERR("pHalSensor sendCommand SENSOR_CMD_SET_TEST_PATTERN_OUTPUT fail");
#endif
        //port TG
        ...
        for(int j=0;j<3;j++)
        {
            ...
            LOG_INF("IMGO m_pICamIOPipe->enqueOutBuf[%d].u4BufPA(0x%" PRIxPTR ").size(0x%x)",
                j, rQBufInfo.vBufInfo[0].u4BufVA[0], rQBufInfo.vBufInfo[0].u4BufSize[0]);
            ...
            LOG_INF("RRZO m_pICamIOPipe->enqueOutBuf[%d].u4BufPA(0x%" PRIxPTR ").size(0x%x)",
                j, rQBufInfo.vBufInfo[0].u4BufVA[0], rQBufInfo.vBufInfo[0].u4BufSize[0]);
        }
