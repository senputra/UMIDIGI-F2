...
#define MEDIA_PATH_CCT "/data/cct"
...
static MUINT32 get_boot_mode(void)
{
    ...
    if (s > 3) {
        ACDK_LOGE("s:%zu is more than 4", s);
        return 0;
    }
...
MINT32 AcdkMain::startPreview(Func_CB /*prvCb*/)
{
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
        ACDK_LOGD("prvVA[%d]  = 0x%" PRIxPTR ", prvPA[%d]  = 0x%" PRIxPTR,
            i, mPrvIMemInfo[i].virtAddr, i, mPrvIMemInfo[i].phyAddr);
        ACDK_LOGD("rrzoVA[%d] = 0x%" PRIxPTR ", rrzoPA[%d] = 0x%" PRIxPTR,
            i, mPrvIMemInfoRRZO[i].virtAddr, i, mPrvIMemInfoRRZO[i].phyAddr);
        ACDK_LOGD("dispVA[%d] = 0x%" PRIxPTR ", dispPA[%d] = 0x%" PRIxPTR,
            i, mDispIMemInfo[i].virtAddr, i, mDispIMemInfo[i].phyAddr);
    }

    for(MINT32 i = 0; i < SURFACE_NUM; ++i)
    {
        ACDK_LOGD("surfaceVA[%d] = 0x%" PRIxPTR ", surfacePA[%d] = 0x%" PRIxPTR,
            i, mSurfaceIMemInfo[i].virtAddr, i, mSurfaceIMemInfo[i].phyAddr);
    }
    ...
    ACDK_LOGD("TgInfo[0] = %d\n", sensorDynamicInfo[0].TgInfo);
    ACDK_LOGD("pixelMode[0] = %d\n", sensorDynamicInfo[0].pixelMode);
...
MINT32 AcdkMain::startPreview_CameraDev(Func_CB /*prvCb*/,  MUINT32 const mode)
{
    ...
    for(MINT32 i = 0; i < OVERLAY_BUFFER_CNT; ++i)
    {
         ACDK_LOGD("dispVA[%d] = 0x%" PRIxPTR ", dispPA[%d] = 0x%" PRIxPTR ", dispmemID[%d] = %d",
             i, mDispIMemInfo[i].virtAddr,
             i, mDispIMemInfo[i].phyAddr,
             i, mDispIMemInfo[i].memID);
    }
...
MINT32 AcdkMain::quickViewImg(MUINT32 qvFormat)
{
    ...
    ACDK_LOGD("procQvMemInfo : vir(0x%" PRIxPTR "),phy(0x%" PRIxPTR ")",
        procQvMemInfo.virtAddr, procQvMemInfo.phyAddr);
    ...
    ACDK_LOGD("surfaceIndex[%u],VA(0x%" PRIxPTR "),PA(0x%" PRIxPTR ")",
        mSurfaceIndex,
...
#else
    (void)u4GroupId; (void)u4ImgIndex; (void)u4FinalExifSize;
#endif
    return ret;
}
...
MINT32 AcdkMain::handlePreviewCB(MVOID *param)
{
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug,"index(%u),VA(0x%" PRIxPTR "),PA(0x%" PRIxPTR "),Size(%u)",
        mSurfaceIndex,
        mSurfaceIMemInfo[mSurfaceIndex].virtAddr,
        mSurfaceIMemInfo[mSurfaceIndex].phyAddr,
        mSurfaceIMemInfo[mSurfaceIndex].size);
    ...
    else
    {
        ...
        ACDK_LOGD("procQvMemInfo : vir(0x%" PRIxPTR "),phy(0x%" PRIxPTR ")",
            ISrcMemInfo.virtAddr, ISrcMemInfo.phyAddr);
        ...
        ACDK_LOGD("Dst VA(0x%" PRIxPTR "), Src VA(%p) size(%u)",
            mSurfaceIMemInfo[mSurfaceIndex].virtAddr, pVirBufin, displaySize);
    }
...
MINT32 AcdkMain::handleRAWCB(MVOID *param)
{
    ...
    else
    {
        ...
        ACDK_LOGD("mRawIMemInfo : vir(0x%" PRIxPTR "),phy(0x%" PRIxPTR "),memID(%d)",
            mRawIMemInfo.virtAddr, mRawIMemInfo.phyAddr, mRawIMemInfo.memID);
    }
...
MINT32 AcdkMain::handleJPEGCB(MVOID *param)
{
    ...
    else
    {
        ...
        ACDK_LOGD("mJpgIMemInfo : vir(0x%" PRIxPTR "),phy(0x%" PRIxPTR "),memID(%d)",
            mJpgIMemInfo.virtAddr, mJpgIMemInfo.phyAddr, mJpgIMemInfo.memID);
    }
...
MINT32 AcdkMain::handleQVCB(MVOID *param)
{
    ...
    else
    {
        ...
        ACDK_LOGD(" mQvIMemInfo : vir(0x%" PRIxPTR "),phy(0x%" PRIxPTR "),memID(%d)",
            mQvIMemInfo.virtAddr, mQvIMemInfo.phyAddr, mQvIMemInfo.memID);
    }
...
MINT32 AcdkMain::handleCapCB(MVOID *param)
{
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug, "JPEG src VA(%p)", captureAddr);
    ...
    ACDK_LOGD_DYN(g_acdkMainDebug, "captureAddr(%p),captureSize(%u)", captureAddr, captureSize);
