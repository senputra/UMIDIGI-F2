            ...
            , misV3(isV3)
{
    ...
    {    //p1+p2, get sensor dev firstly
        ...
        if (mHalSensorList)
        {
            ...
            LOG_INF("query from sensor openedSensorIndex (%d), eRawPxlID(%d)",
                mOpenedSensor,pixIdP2);
        }
        else
        {
            LOG_ERR("NULL mHalSensorList,tag/sidx/sdev(%d/0x%x/0x%x)",
                mStreamTag, openedSensorIndex, mOpenedSensor);
        }
        #else
    ...
    LOG_INF("tag/sidx/sdev(%d/0x%x/0x%x),swPipe cID(%p)",
        mStreamTag, openedSensorIndex, mOpenedSensor, (&mDequeuedBufList));
}
...
init()
{
    ...
    if (this->mpPostProcPipe)
    {
        ...
        for (unsigned int i = 0; i < cropPathNum; i++)
        {
            CropPathInfo crop;
...
#ifdef __LP64__
    LOG_DBG("sTag(%d),i8TimeoutNs(%ld)", mStreamTag, i8TimeoutNs);
#else
...
#ifdef __LP64__
    LOG_INF("sTag(%d),i8TimeoutNs(%ld)", mStreamTag, i8TimeoutNs);
#else
...
{
    for (unsigned int i = 0; i < mCropPaths.size(); i++)
    {

        MCropPathInfo crop;
        crop.mGroupIdx = mCropPaths[i].u4CropGroup;
        for (unsigned int j = 0; j < mCropPaths[i].PortIdxVec.size(); j++)
        {
    ...
    android::Vector<MINT32>& vGroupPortIdx)
{
    ...
    LOG_INF("return bcrz(%d)/size(%zu)", beforeCRZ, vGroupPortIdx.size());
    FUNCTION_LOG_END;
    return true;
}
...
{
    FUNCTION_LOG_START;
    LOG_INF("mSWScen(%d),mpHalPipeWrapper()%p", mSWScen, mpHalPipeWrapper);
    bool ret=true;
    //
    if (mSWScen == eSoftwareScenario_total_num)
    {
    ...
    MINTPTR arg2)
{
    ...
    if (!ret)
    {
        LOG_ERR("[Error]sendCommand(0x%x,0x%" PRIxPTR ",0x%" PRIxPTR ") Fail", cmd, arg1, arg2);
    }
    #else
