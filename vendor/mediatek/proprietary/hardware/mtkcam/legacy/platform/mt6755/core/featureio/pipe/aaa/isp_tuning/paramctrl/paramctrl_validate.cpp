...
validatePerFrame(MINT32 const i4FrameID, MBOOL const fgForce)
{
    ...
    { //dump NVRAM index mapping for debugging/check
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("vendor.debug.dump_nvram_idx.ctrl", value, "0");
        MUINT32 res = atoi(value); 
        dumpAllNvramIndexMapping(res);
    }
...
MVOID
Paramctrl::dumpAllNvramIndexMapping(MUINT32 bReset)
{
    //detect rising edge of bReset
    MBOOL bEnDump = MFALSE;
    if (bReset && (mbFlagDumpNvramMapping == MFALSE))
    {
        bEnDump = MTRUE;
    }
    mbFlagDumpNvramMapping = bReset ? MTRUE : MFALSE;
    ...
    FILE* fp;
    const char* fname = "/sdcard/nvram_idx_mapping.txt";
    fp = fopen(fname, "wb");
    if(fp == NULL)
    {
        MY_LOG("Open nvram_idx_mapping fail!!");
        return;
    }
    
    for (MUINT32 profile=EIspProfile_Preview; profile<EIspProfile_NUM; profile++)
    {
        for (MUINT32 sensor=ESensorMode_Preview; sensor<ESensorMode_NUM; sensor++)
        {
            for (MUINT32 iso=eIDX_ISO_100; iso<eNUM_OF_ISO_IDX; iso++)
            {
                idx_set = *reinterpret_cast<INDEX_T*>(getDefaultISPIndex(profile, sensor, iso));
                fprintf(fp, "[NVRAM index mapping] (%d, %d, %d)->(%d,%d,%d,%d,%d; %d,%d,%d,%d,%d; %d,%d,%d,%d)\n"
                    , profile, sensor, iso
...
INDEX_T
Paramctrl::
getISPIndex_byISO(MUINT32 u4Idx_ISO)
{
    ...
    INDEX_T const*const pDefaultIndex = m_pIspTuningCustom->getDefaultIndex(
        m_rIspCamInfo.eIspProfile, m_rIspCamInfo.eSensorMode, m_rIspCamInfo.eIdx_Scene, static_cast<EIndex_ISO_T>(u4Idx_ISO)
    );
    if  ( ! pDefaultIndex )
    {
        MY_ERR("[ERROR][getISPIndex_byISO]pDefaultIndex==NULL");
    }
    else{
        idxmgr = *pDefaultIndex;
    }
    MY_LOG_IF(m_bDebugEnable, "getISPIndex_byISO[BEFORE][evaluate_nvram_index] ISO idx(%d)", u4Idx_ISO);
    ...
    MY_LOG_IF(m_bDebugEnable, "getISPIndex_byISO[AFTER][evaluate_nvram_index] ISO idx(%d)", u4Idx_ISO);
