...
calculateCCM(AWB_INFO_T const& rAWBInfo, MBOOL bWithPreferenceGain)
{
    if (bWithPreferenceGain)
        SmoothCCM(rAWBInfo.rCurrentAWBGain, 3, 9, m_rISPMultiCCM, m_rCCMOutput, m_rCCMWeight, 0, 0, 0, 1);
    else
        SmoothCCM(rAWBInfo.rCurrentFullAWBGain, 3, 9, m_rISPMultiCCM, m_rCCMOutput, m_rCCMWeight, 0, 0, 0, 1);
}
