...
int FlashlightDrv::setDuty(int duty)
{
    ...
    // adjust duty offset MA
    if(mSensorDev == e_CAMERA_SUB_SENSOR && mIsCali == 0){
	err = pKDrv->sendCommand(FLASH_IOC_SET_DUTY_OFFSET_MA, mSensorDev, mStrobeId, mDutyOffsetMA[duty]);
	logI("setoffset() offsetMA= %d\n",mDutyOffsetMA[duty]);
    }
    // adjust duty offset MA end
#endif
    return err;
}
//======================================================
...
{
    logI("FlashlightDrv()\n");
    memset(&mTabId, 0, 10*sizeof(int));
    memset(&mCoolTMTab, 0, 10*sizeof(float));
    memset(&mTimeoutTab, 0, 10*sizeof(int));

#ifdef FLASHLIGHT_OFFSET_CALI
    mIsCali = 0;
    memset(&mDutyOffsetMA, 0, sizeof(short)*40);
#endif
}
...
#ifdef FLASHLIGHT_OFFSET_CALI
int FlashlightDrv::setDutyOffsetMA(short* dutyOffsetMA)
{
    logI("setDutyOffsetMA ln=%d", __LINE__);
    int i;
    for(i=0;i<mDutyNum;i++){
        mDutyOffsetMA[i] = dutyOffsetMA[i];
        logI("setDutyOffsetMA[%d] =%d",i, mDutyOffsetMA[i]);
    }
    logI("setDutyOffsetMA ln=%d", __LINE__);
    return 0;
}

int FlashlightDrv::setCali(int isCali)
{
    logI("setCali ln=%d, mIsCali=%d", __LINE__, isCali);
    mIsCali = isCali;
    return 0;
}
#endif
