...
#define _NOMRALPIPE_CVT_TGIDX(sensorIdx)({\
    MUINT32 tgidx = EPortIndex_UNKNOW;\
...
NormalPipe_FrmB::start()
{
    ...
    for (unsigned int i = 0; i < this->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_OpendNum); i++) {
...
#else
    for (unsigned int i = 0; i < mpEnabledSensorCnt; i++) {
...
NormalPipe_FrmB::enque(QBufInfo const& rQBuf)
{
    ...
    //all enque request r all push into enquereqest , but enque before start have special control flow at enquerequest()
    {
        for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
        {
            ...
            PIPE_INF("MyEnq: dma(0x%x) pa(0x%" PRIxPTR ") io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x)\n",
                _buf.mPortID.index,
                _buf.mBuffer->getBufPA(0),
                _buf.FrameBased.mCropRect.p.x, _buf.FrameBased.mCropRect.p.y,
                _buf.FrameBased.mCropRect.s.w, _buf.FrameBased.mCropRect.s.h,
                _buf.FrameBased.mDstSize.w,_buf.FrameBased.mDstSize.h,
...
NormalPipe_FrmB::DummyFrame(QBufInfo const& rQBuf){
    ...
    for (unsigned int i = 0; i < rQBuf.mvOut.size(); i++) {
        _buf = rQBuf.mvOut.at(i);
        ...
        PIPE_INF("MyEnq_dummy: dma(0x%x) pa(0x%" PRIxPTR ") io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x)\n", _buf.mPortID.index,
                _buf.mBuffer->getBufPA(0),
                _buf.FrameBased.mCropRect.p.x, _buf.FrameBased.mCropRect.p.y,
                _buf.FrameBased.mCropRect.s.w, _buf.FrameBased.mCropRect.s.h,
                _buf.FrameBased.mDstSize.w, _buf.FrameBased.mDstSize.h,
...
NormalPipe_FrmB::checkEnque(QBufInfo const& rQBuf)
{
    ...
    if (rQBuf.mvOut.size() != this->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_OpendNum)) {
        PIPE_ERR("error:current enabled port number:0x%x,enqued port:0x%zu, number r mismatch\n",
...
#define __FMT(portidx,fmt)({\
    ...
    _fmt;})
        ...
        if(ISP_QuerySize(rQBuf.mvOut.at(i).mPortID.index,_op,(EImageFormat)__FMT(rQBuf.mvOut.at(i).mPortID.index,rQBuf.mvOut.at(i).mBuffer->getImgFormat()),rQBuf.mvOut.at(i).FrameBased.mDstSize.w,queryRst,_PixMode) == 0){
            ...
            if (rQBuf.mvOut.at(i).FrameBased.mDstSize.w != (int)(queryRst.x_pix)) {
...
NormalPipe_FrmB::checkDropEnque(QBufInfo const& rQBuf)
{
    ...
    if ((this->m_PerFrameCtl) && ((MUINT32)_sof_idx == this->mPrvEnqSOFIdx)) {
        ...
        {
            PIPE_INF("warning:can't push enque into drv more than once within 1 frame,drop request(pa:0x%" PRIxPTR ",mag:0x%x)sof(%d/%" PRIdPTR ")\n",
                _popbuf.mBuffer->getBufPA(0), _popbuf.FrameBased.mMagicNum_tuning, this->mPrvEnqSOFIdx, _sof_idx);
        }
        if(this->m_DropCB)
    ...
    for (MUINT32 ii=0 ; ii<rQBuf.mvOut.size() ; ii++ ) {
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            for (MUINT32 idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++)  {
                ...
                if (idx >= 1) {
                    PIPE_ERR("[MyDeQ] dequed image from FBC r over 1 buffer(0x%zx) at portID:0x%x\n",
                ...
                if (NULL == pframe) {
                    PIPE_ERR("[MyDeQ:%d]Fail: PortId(%d), 0==pframe, idx=%d, pa=0x%" PRIxPTR,
                ...
                if(buff.mMetaData.m_bDummyFrame == MTRUE){
                    ...
                    //
                    PIPE_INF("MyDeq: dummy frame: dma(0x%x) pa(0x%" PRIxPTR ") io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x)\n",
                        portID.index,
                        buff.mPa,
                    ...
                    PIPE_INF("MyDeq: dma(0x%x) pa(0x%" PRIxPTR ") io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x)\n",
                        portID.index,
                        buff.mPa,
                        result.mCrop_s.p.x,result.mCrop_s.p.y,result.mCrop_s.s.w,result.mCrop_s.s.h,
                        result.mCrop_d.p.x,result.mCrop_d.p.y,result.mCrop_d.s.w,result.mCrop_d.s.h,
...
NormalPipe_FrmB::Reset()
{
    ...
    for (unsigned int i = 0; i < this->mpEnabledSensorCnt; i++)
    {
...
NormalPipe_FrmB::configPipe(QInitParam const& vInPorts)
{
    ...
    if (mpEnabledSensorCnt != vInPorts.mSensorCfg.size()) {
        PIPE_ERR("NPipe::configPipe SensorCfgCnt is not match. (%d,%zu)",
                mpEnabledSensorCnt, vInPorts.mSensorCfg.size());
    ...
    for (unsigned int i = 0; i < mpEnabledSensorCnt; i++)  {
        ...
        if (0 != pAllSensorInfo->mSenInfo[SIdx].mOccupiedOwner) {
        ...
        for (unsigned int j = 0; j < vInPorts.mPortInfo.size(); j++) {
    ...
    if (!mpCamIOPipe)
    {
        ...
        for (unsigned int i = 0; i < mpEnabledSensorCnt; i++)
        {
            ...
            if (NSCam::SENSOR_TYPE_YUV == _NORMALPIPE_GET_SENSORTYPE(sensorIdx)) {
                if ((mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1] != ePxlMode_Two_Single) && (mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1] != ePxlMode_One_Single)){
                    CAM_LOGE("error: YUV sensor not use one/two-pixel mode and one isp, sensorIdx=%d, tg=%d, PixelMode=%lu",
                        sensorIdx, _NORMALPIPE_GET_TG_IDX(sensorIdx), mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1]);
        ...
        for (unsigned int i = 0; i < vInPorts.mPortInfo.size(); i++) {
...
NormalPipe_FrmB::wait(ISignalConsumer *pConsumer, MINT32 sensorIdx, EPipeSignal eType, uint64_t ns_timeout)
{
    ...
    PIPE_DBG("wait-");
#else
    (void)pConsumer; (void)sensorIdx; (void)eType; (void)ns_timeout;
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
...
#if 1
        if (2 < arg1) {
            PIPE_ERR("EPIPECmd_GET_TG_OUT_SIZE Err: Invalid SensorIdx(0x%" PRIxPTR ")!", arg1);
            return MFALSE;
        }
    ...
    case EPIPECmd_GET_HBIN_INFO:
        ...
        if ((2 < arg1) || (0 == arg2)) {
            PIPE_ERR("EPIPECmd_GET_HBIN_INFO Err: Invalid Para arg1(0x%" PRIxPTR "), arg2=0x%" PRIxPTR "!", arg1, arg2);
            return MFALSE;
        }
        tg = _NORMALPIPE_GET_TG_IDX(arg1);

        PIPE_DBG("EPIPECmd_GET_HBIN_INFO SIdx(0x%" PRIxPTR "), BinInfoAddr=0x%" PRIxPTR, arg1, arg2);
        ...
        switch (tg) {
            ...
            case CAM_TG_2:
                ...
                break;
            default:
                PIPE_ERR("EPIPECmd_GET_HBIN_INFO Err: Invalid SensorIdx(0x%" PRIxPTR ")!", arg1);
                return MFALSE;
        }

        PIPE_DBG("EPIPECmd_GET_HBIN_INFO SIdx(0x%" PRIxPTR "),twin=%d,hbin_en=%d w=0x%x, h=0x%x!",
            arg1, mbTwinEnable, hbin->mEnabled, hbin->size.w, hbin->size.h);
        break;
    ...
    case EPIPECmd_GET_RMX_OUT_SIZE:
        ...
        //arg3 : height
        if (2 < arg1) {
            PIPE_ERR("EPIPECmd_GET_RMX_OUT_SIZE Err: Invalid SensorIdx(0x%" PRIxPTR ")!", arg1);
            return MFALSE;
        }
        tg = _NORMALPIPE_GET_TG_IDX(arg1);
        switch (tg) {
            ...
            default:
                ret = MFALSE;
                PIPE_ERR("EPIPECmd_GET_RMX_OUT_SIZE Fail! SIdx=0x%" PRIxPTR ", TG=%d", arg1, tg);
                break;
        }
    ...
    case EPIPECmd_GET_EIS_INFO:
        ...
        if (0 == arg2) {
            PIPE_ERR("EPIPECmd_GET_EIS_INFO Err: Invalid Para arg1(0x%" PRIxPTR ")", arg1);
            return MFALSE;
        }
        ...
        case eCmd_at:
            if (this->m_v_pQueue.size() > arg1)
                *(BufInfo*)arg2 = this->m_v_pQueue.at(arg1);
                ...
                PIPE_INF("size have been reduce to 0x%zx(0x%" PRIxPTR ") by enque\n", this->m_v_pQueue.size(), arg1);
                ret = MFALSE;
            }
            break;
        case eCmd_pop_back:
...
MBOOL NormalPipe_FrmB_Thread::Enque(BufInfo* pBuf)
{
    ...
    if (_buf.mPa == (MUINTPTR)(pBuf->mBuffer->getBufPA(0))) {
    ...
    if (rQBufInfo.vBufInfo.size() == 1)
        PIPE_DBG("Enque port:0x%x PA:0x%" PRIxPTR ",mag:0x%x\n",
            portID.index, rQBufInfo.vBufInfo[0].u4BufPA[0], pBuf->FrameBased.mMagicNum_tuning);
    else
        PIPE_DBG("Enque port:0x%x PA:0x%" PRIxPTR ",exchange:0x%" PRIxPTR ", mag:0x%x\n",
...
MVOID NormalPipe_FrmB_Thread::DummyFrameToEnque(MUINT32 _OpenedPort)
{
    ...
    for (unsigned int i = 0; i < _OpenedPort; i++) {
...
MVOID NormalPipe_FrmB_Thread::EnqueRequest(MUINT32 sof_idx)
{
    ...
    if(this->m_EnqueCnt++ < (this->m_pNormalpipe_FrmB->GetRingBufferDepth()-1)){ //-1 is because of first tuning is an immediate_request at enque()
        ...
        unsigned int nLost = min(sof_idx, this->m_pNormalpipe_FrmB->GetRingBufferDepth()-1);
    ...
    if(_size == 0){
        ...
        PIPE_DBG("CUR FRAME STATUS:0x%" PRIxPTR "\n", _drop_status);
        ...
        if(this->m_pNormalpipe_FrmB->m_PerFrameCtl){//Full mode means 1 request per frame, if requests r over 2, drop request
            ...
            for (unsigned int i = 0; i < _OpenedPort; i++) {
                ...
                while(_size > 1){
                    PIPE_INF("Warning: over 1 request in enqueRequest container(0x%" PRIxPTR "_0x%x)\n",
                        _size,
                        _OpenedPort);
                    ...
                    while(_size>_dropCnt){
                        ...
                        PIPE_INF("Warning: drop request: Port:0x%x, PA:0x%" PRIxPTR ", mag:0x%x\n",
                            _buf.mPortID.index,
                            _buf.mBuffer->getBufPA(0),
                            _buf.FrameBased.mMagicNum_tuning);
                        //pop older request
                    ...
                    if(__size == 0){
                        ...
                        PIPE_INF("Warning: reserved request: Port:0x%x, PA:0x%" PRIxPTR ", mag:0x%x\n",
                                _buf.mPortID.index,
                                _buf.mBuffer->getBufPA(0),
                                _buf.FrameBased.mMagicNum_tuning);
            ...
            for (unsigned int i = 0; i < this->m_DropEnQ.size(); i++) {
                PIPE_INF("drop enque request:0x%x(0x%zx)\n", this->m_DropEnQ.at(i), this->m_DropEnQ.size());
...
MVOID NormalPipe_FrmB_Thread::ImmediateRequest(MBOOL bByPassHwEnq){
    ...
    for (unsigned int i = 0; i < this->m_pNormalpipe_FrmB->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_OpendNum); i++) {
        this->m_pNormalpipe_FrmB->mpEnqueRequest[_PortMap(this->m_pNormalpipe_FrmB->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_Index,i))]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_front, (MUINTPTR)&_buf, 0);
        //
...
MVOID* NormalPipe_FrmB_Thread::IspEnqueThread(void *arg)
{
    ...
    while (1)
    {
        ...
        PIPE_DBG("TG_%d: cur SOF = 0x%" PRIxPTR "\n",
        ...
        if(_sof_idx != _sof_idx2){
            PIPE_INF("Warning: TG(0x%x): EnqueRequest over vsync(0x%" PRIxPTR "_0x%" PRIxPTR ")\n",
