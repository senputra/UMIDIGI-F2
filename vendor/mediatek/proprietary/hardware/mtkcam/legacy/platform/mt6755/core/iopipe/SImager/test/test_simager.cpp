...
allocBuf(IImageBuffer **pBuf, MSize const & imgSize, int const format)
{
    ...
    *pBuf = allocator->alloc_ion(LOG_TAG, imgParam);
    ...
    //
    MY_LOGD("ImgBitsPerPixel:%zu BufSizeInBytes:%lu %d",
        (*pBuf)->getImgBitsPerPixel(), (*pBuf)->getBufSizeInBytes(0), imgSize.size());
    return  MTRUE;
}
...
static MUINT32 u4SetROI_Cmd(int argc, char** argv)
{
    ...
    if ((rROI.p.x + rROI.s.w) > (int)g_u4SrcWidth || (rROI.p.y + rROI.s.h) > (int)g_u4SrcHeight)
    {
