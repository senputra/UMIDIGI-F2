...
{
    Mutex::Autolock _l(mLock);
    MY_LOGD_IF( pStreamInfo.get(), "streamId:%#" PRIx64 " %s %p type:%d MaxBufSize:%zu MinInitBufSize:%zu",
    ...
    MY_LOGD("operate on streampool index(%zu) size(%zu)", index, mvStreamStatus.size());
    ...
    switch ( nodeid ) {
        ...
        case eNODEID_P1Node_main2: // Stereo
        {
            ...
            if ( index >= 0 ) {
                ...
                MY_LOGD("Modify init param of nodeid(%#" PRIxPTR ") openid(%d) nodename(%s)",
                ...
                MY_LOGD("ADD new init param of nodeid(%#" PRIxPTR ") openid(%d) nodename(%s)",
                         param.nodeId, param.openId, param.nodeName);
            }
        }
        break;
...
)
{
    ...
    ssize_t index = mvNodeStatus.indexOfKey(nodeid);
    if ( index < 0 ) {
        MY_LOGE("Node(%#" PRIxPTR ") must be add to status map", nodeid);
    }
    //
    MERROR err = OK;
