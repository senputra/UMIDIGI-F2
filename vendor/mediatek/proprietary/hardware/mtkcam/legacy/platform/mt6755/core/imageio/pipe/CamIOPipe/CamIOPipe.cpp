...
enqueInBuf(PortID const portID, QBufInfo const& rQBufInfo)
{
    ...
    PIPE_DBG("QBufInfo:(user, reserved, num)=(%x, %d, %zu)", rQBufInfo.u4User, rQBufInfo.u4Reserved, rQBufInfo.vBufInfo.size());
    //
    return  MTRUE;
}
...
enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo,MBOOL bImdMode)
{
    ...
    //
    PIPE_DBG("PortID=%d, bufInfo,(%d),(0x%08x),(0x%" PRIxPTR "),(0x%" PRIxPTR ")",
    ...
    if ( 2 <= rQBufInfo.vBufInfo.size() ) {
        ...
        //
        PIPE_DBG("exbufInfo,(%d),(0x%08x),(0x%" PRIxPTR "),(0x%" PRIxPTR "),(%d/%d)",
                exbufInfo.memID,
                exbufInfo.size,
...
********************************************************************************/
    ...
    //
    PIPE_DBG("P1 in[%zu]/out[%zu]", vInPorts.size(), vOutPorts.size());
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_RRZ:
            ...
            //Arg3: sensorDev
            if (0 == arg1) {
                PIPE_ERR("[3ASync] SET_RRZ NULL param!");
                return MFALSE;
            }
            ...
            PIPE_DBG("[3ASync]SET_RRZ path:%d, m_num_0=0x%x -", (EPortIndex)arg2, pUserRrzPara->m_num);
            ...
            this->mRunTimeCfgLock.unlock();
            PIPE_DBG("[3ASync]SET_RRZ [%d/%d], m_num_0=0x%lx -", m_rrz_update_cnt, m_rrzd_update_cnt, pRrz_cfg->m_num_0);
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            if (((EPortIndex)arg1 != EPortIndex_TG1I)  && ((EPortIndex)arg1 != EPortIndex_TG2I)) {
                PIPE_ERR("[3ASync]NOT support PortIndex(0x%x)!", (EPortIndex)arg1);
                ret = -1;
                return ret;
            }
            this->mRunTimeCfgLock.lock();
            if ( EPortIndex_TG1I == arg1 ) {
                PIPE_DBG("[3ASync]P1_UPDATE+, TG1I, magicNum=0x%" PRIxPTR, arg2);
                ...
                //update param here
                PIPE_DBG("[3ASync]P1_UPDATE+, TG2I, magicNum=0x%" PRIxPTR, arg2);
        ...
        case EPIPECmd_SET_IMGO:
            {
                DMACfg* _pDma = NULL;
                if (0 == arg1) {
                    PIPE_ERR("[3ASync] SET_IMGO NULL param!");
                    return MFALSE;
                }
                _pDma = (DMACfg*)arg1;
                ...
                PIPE_DBG("[3ASync]SET_IMGO path:%d, m_num_0=0x%x -", (EPortIndex)arg2, _pDma->m_num);

                if (_pDma->crop.x > this->m_camPass1Param.tg_out_size.w) {
                    PIPE_ERR("[3ASync]ERROR:TG:0x%x Crop invalid(x(%d)>size_w(%lu))", (EPortIndex)arg2, _pDma->crop.x, this->m_camPass1Param.tg_out_size.w);
                    return MFALSE;
                }
                if (_pDma->crop.w > this->m_camPass1Param.tg_out_size.w) {
                    PIPE_ERR("[3ASync]ERROR:TG:0x%x Not Support Scale up(in(%lu)<tg_w(%lu))", (EPortIndex)arg2, _pDma->crop.w, this->m_camPass1Param.tg_out_size.w);
                    return MFALSE;
                }
                ...
                if (((EPortIndex)arg2 != EPortIndex_TG1I)  && ((EPortIndex)arg2 != EPortIndex_TG2I)) {
                    PIPE_ERR("[3ASync]NOT support TGIdx(0x%x)!", (EPortIndex)arg2);
                    ret = -1;
                    return ret;
                }
                this->mRunTimeCfgLock.lock();
        ...
        case EPIPECmd_GET_MODULE_HANDLE: //handle-based
             ...
             if (EPortIndex_UNKNOW == tg ) {
                 CAM_LOGE("GET_MODULE_HANDLE:User(%s), tg=%d is Invalid", (char*)arg3, tg);
                 return -1;
             }
             ...
             *((MINTPTR*)arg2) = (MINTPTR) (m_P1HWRWctrl[tg].getModuleHandle((CAM_MODULE_ENUM)p1ConvModuleID(arg1), (char const*)arg3));
             if (0 == *((MINT32*)arg2)) {
                PIPE_ERR("NOT Rdy || Not-supported module(0x%x, %d)!", (EModule)arg1, p1ConvModuleID(arg1));
                ret = -1;
             }
             break;
        ...
        case EPIPECmd_RELEASE_MODULE_HANDLE: //handle-based
            ...
            //CAM_LOGD("Handle(0x%x), %s!",arg1, arg2);
             if (!arg1) {
                 PIPE_ERR("RELEASE_MODULE_HANDLE: Invalid Handle(0x%" PRIxPTR ")!", arg1);
                 ret = -1;
             } else {
        ...
        case EPIPECmd_SET_MODULE_EN:
            ...
            if ( EModule_CAMSV_IMGO == arg1)  {
                ...
                tg = _p1GetModuleTgIdx(arg1,this->m_pass1TwinMode);
                if (EPortIndex_UNKNOW == tg ) {
                    CAM_LOGE("SET_MODULE_EN:User(%s), tg=%d is Invalid", (char*)arg3, tg);
                    return -1;
                }
                if (((EPortIndex_TG1I == tg) && (!(this->mPath1 & ISP_PASS1)))||
                            ((EPortIndex_TG2I == tg) && (!(this->mPath1 & ISP_PASS1_D)))) {
        ...
        case EPIPECmd_SET_MODULE_SEL:
            ...
            if (EPortIndex_UNKNOW == tg ) {
                CAM_LOGE("SET_MODULE_SEL:User(%s), tg=%d is Invalid", (char*)arg3, tg);
                return -1;
            }
            if (((EPortIndex_TG1I == tg) && (!(this->mPath1 & ISP_PASS1)))||
                ((EPortIndex_TG2I == tg) && (!(this->mPath1 & ISP_PASS1_D)))) {
