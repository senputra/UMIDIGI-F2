...
get3AStatusFromSensor()
{
    ...
    if(m_pIHalSensor) // add nullptr check for Coverity
    {
    ...
    {
        MY_ERR("m_pIHalSensor is nullptr! sendCommand is NOT executed!");
    }
...
MBOOL Hal3AYuv::setFDInfo(MVOID* a_sFaces)
{
    MINT32 i4Cnt;
    AREA_T rFDArea;
    rFDArea.i4Info = 0; // add initial value for Coverity
...
MINT32 Hal3AYuv::ASDLog2Func(MUINT32 numerator, MUINT32 denominator) const
{
    ...
    {
        for (x = (ASD_LOG2_LUT_CENTER > 0) ? ASD_LOG2_LUT_CENTER : 1 ; x < ASD_LOG2_LUT_NO ; x++)
        {
