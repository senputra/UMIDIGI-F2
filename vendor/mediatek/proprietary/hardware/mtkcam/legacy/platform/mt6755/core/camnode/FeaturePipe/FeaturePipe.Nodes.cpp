...
#define MY_LOGD_IF(cond, arg...)    {if (cond) { LOG_DBG(arg); }}
...
MBOOL FeaturePipeNode::sDebugLogEnable = MFALSE;
...
FeaturePipe::initFeatureGraph()
{
    ...
    //flag for debug info dump
    mDebugLogEnable = property_get_bool("vendor.debug.featurelog.enable", 0);
    FeaturePipeDataHandler::sDebugLogEnable = mDebugLogEnable;
    FeaturePipeNode::sDebugLogEnable = mDebugLogEnable;

	// mkdbg
	property_get("vendor.debug.fpipe.p2a.mk", sgmkDebugFlag, "0");
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        else
        {
            ...
            property_get("vendor.camera.featurepipe.dumpgpu", gpuDebug, "0");
            ...
            property_get("vendor.camera.featurepipe.dumpmdp", mdpDebug, "0");
...
#ifdef USING_MTK_CPUCTRL
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) && (misCpuCtrl == MTRUE) && mpCpuCtrl)
    {
        LOG_DBG("Disable vFB CpuCtrl");
        mpCpuCtrl->disable();
        mpCpuCtrl->uninit();
...
MVOID
FeaturePipe::startEISRecord()
{
    FUNCTION_LOG_START;
    if(mpEisSwNode != NULL)
    {
        reinterpret_cast<FeaturePipeEisNode*>(mpEisSwNode)->startVideoRecord();
    }
    if(mpP2aNode != NULL)
    {
        reinterpret_cast<FeaturePipeP2aNode*>(mpP2aNode)->startEISRecord();
    }
    FUNCTION_LOG_END;
...
MVOID
FeaturePipe::stopEISRecord()
{
    FUNCTION_LOG_START;
    {
        /* Clear EIS queued infos  */
        ...
        if(mpEisSwNode != NULL)
        {
            reinterpret_cast<FeaturePipeEisNode*>(mpEisSwNode)->stopVideoRecord();
            reinterpret_cast<FeaturePipeEisNode*>(mpEisSwNode)->removeQueuedInfos();
        }
        if(mpGpuNode != NULL)
        {
            reinterpret_cast<FeaturePipeGpuNode*>(mpGpuNode)->removeQueuedInfos();
        }
        if(mpMdpNode != NULL)
        {
            reinterpret_cast<FeaturePipeMdpNode*>(mpMdpNode)->removeQueuedInfos();
        }
        if(mpP2aNode != NULL)
        {
            reinterpret_cast<FeaturePipeP2aNode*>(mpP2aNode)->stopEISRecord();
        }
    }
    FUNCTION_LOG_END;
}
...
MVOID
FeaturePipe::notifyEISRecordStopping()
{
    FUNCTION_LOG_START;
    if(mpEisSwNode != NULL)
        reinterpret_cast<FeaturePipeEisNode*>(mpEisSwNode)->notifyRecordStopping();
    FUNCTION_LOG_END;
}
...
#ifdef USING_MTK_CPUCTRL
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) && (misCpuCtrl == MTRUE)
        && (misCpuCtrl_count == 3) && mpCpuCtrl )
    {
        LOG_DBG("Disable vFB CpuCtrl");
        mpCpuCtrl->disable();
        mpCpuCtrl->uninit();
...
FeaturePipe::setFeature(MUINT32 featureMask)
{
...
#ifdef USING_MTK_CPUCTRL
    if (FEATURE_MASK_IS_VFB_ENABLED(featureMask))
    {
        mpCpuCtrl = CpuCtrl::createInstance();
        if(mpCpuCtrl)
        {
            LOG_DBG("Enable vFB CpuCtrl");
            mpCpuCtrl->init();
    ...
    if(FEATURE_MASK_IS_VFB_ENABLED(featureDiff))
    {
        if(FEATURE_MASK_IS_VFB_ENABLED(featureMask) == true)
        {
            mpVFBCBThread = NULL;
            mpVFBCBThread = VFBCBThread::createInstance();
            if(mpVFBCBThread != NULL)
            {
                mpVFBCBThread->setCallbacks(mpCamMsgCbInfo);
                mpVFBCBThread->run("VFBCBThread");
            }
            else
            {
                LOG_ERR("VFB CB Thread create failed");
            }
        }
        else if(FEATURE_MASK_IS_VFB_ENABLED(featureMask) == false)
        {
            if(mpVFBCBThread != NULL)
            {
...
FeaturePipeP2aNode::onInit()
{
        ...
        // For Stereo Camera case
    {
        ...
        property_get("vendor.debug.stereovfb.enable", EnableOption, "0");
        if(EnableOption[0] == '1')
        {
        ...
        FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        mFullImgBGSize = P2A_FULLIMG_BUFFER_NUM;
        if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask) && FEATURE_MASK_IS_EIS_Q_ENABLED(mFeatureOnMask))
            mFullImgBGSize += getEisSupFrames_Hal1(); // From camera_custom_eis

        mFullImgGB = new sp<NativeBufferWrapper>[mFullImgBGSize];
        LOG_DBG("Allocate P2A buffer %dx%d, size(%d)", NR3D_WORKING_BUFF_WIDTH, NR3D_WORKING_BUFF_HEIGHT, mFullImgBGSize);

        for (MUINT8 i = 0; i < mFullImgBGSize; i++)
        {
            mFullImgGB[i] = new NativeBufferWrapper(NR3D_WORKING_BUFF_WIDTH, NR3D_WORKING_BUFF_HEIGHT,
                                                    HAL_PIXEL_FORMAT_YV12, NativeBufferWrapper::USAGE_HW_TEXTURE,
                                                    "FullImgGB" + std::to_string(i));

            gralloc_extra_ion_sf_info_t info;
            gralloc_extra_query(mFullImgGB[i]->getHandle(), GRALLOC_EXTRA_GET_IOCTL_ION_SF_INFO, &info);
            gralloc_extra_sf_set_status(&info, GRALLOC_EXTRA_MASK_YUV_COLORSPACE, GRALLOC_EXTRA_BIT_YUV_BT601_FULL);
            gralloc_extra_perform(mFullImgGB[i]->getHandle(), GRALLOC_EXTRA_SET_IOCTL_ION_SF_INFO, &info);
...
void
FeaturePipeP2aNode::
getGraphicBuffer(vector< sp<NativeBufferWrapper>* >& vpGB)
{
    for (int i=0; i<mFullImgBGSize; i++)
...
MBOOL
FeaturePipeP2aNode::startEISRecord()
{
    FUNCTION_LOG_START;
    Mutex::Autolock autoLock(mUninitMtx);
    mEISRecStart = MTRUE;
...
MBOOL
FeaturePipeP2aNode::stopEISRecord()
{
    FUNCTION_LOG_START;
    Mutex::Autolock autoLock(mUninitMtx);
    mEISRecStart = MFALSE;
...
handleP2Done(QParams& rParams)
{
...
#ifndef _DEBUG_DUMP
    char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.featurepipe.dumpp2a", EnableOption, "0");
    ...
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
    {
        MY_LOGD_IF(sDebugLogEnable,"Posting P2A->Switcher node full image buffer of frame %d", getMagicNo(rParams));
        handlePostBuffer(P2A_TO_SWITCHER, (MUINTPTR) &fullImgReq, getMagicNo(rParams));
        MY_LOGD_IF(sDebugLogEnable,"Posting P2A->VFB node DS image of frame %d", getMagicNo(rParams));
        handlePostBuffer(P2A_TO_VFBSW_DSIMG, (MUINTPTR) &dsImgReq, getMagicNo(rParams));
        MY_LOGD_IF(sDebugLogEnable,"Posting P2A->FD node DS image of frame %d", getMagicNo(rParams));
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask) ||FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(mFeatureOnMask))
    {
        ...
        tagReq.imgReq.mBuffer = rParams.mvIn[0].mBuffer;
        tagReq.needByPass = !mEISRecStart && FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask);
        LOG_DBG("Posting P2A->EIS node full image buffer of frame %d, needBypass(%d), eisRec(%d), featureMask(0x%x)", getMagicNo(rParams),tagReq.needByPass, mEISRecStart, mFeatureOnMask);
        handlePostBuffer(P2A_TO_EISSW, (MUINTPTR) &tagReq, getMagicNo(rParams));
...
executeP2A()
{
    ...
    property_get("vendor.debug.fpipe.p2a", p2aDebugFlag, "0");
...
//    if (FEATURE_MASK_IS_3DNR_ENABLED(mFeatureOnMask)) // Workaround for EIS GMV Queue can't unregister user => Pass 2 always register user (in FeaturePipeP2aNode::onInit()), Pass 1 EIS always set GMV, Pass 2 always get GMV.
    if (!FEATURE_MASK_IS_STEREO_CAMERA_ENABLED(mFeatureOnMask)) // CKH, stereo does not support EIS
    {
        ...
        if (mpEisHalFor3DNR != NULL)
        {
            ...
            property_get("vendor.camera.3dnr.forcegmv.enable", aInputValue, "0");
            if (aInputValue[0] == '1')
            {
                property_get("vendor.camera.3dnr.forcegmv.x", aInputValue, "0");
                i4TempNmvXFromQueue = atoi(aInputValue);

                property_get("vendor.camera.3dnr.forcegmv.y", aInputValue, "0");
    ...
    if( eisCrop2Idx == 0 )
    {
        LOG_WRN("Cannot find eisCrop2Idx, use 0");
    }
    ...
    {
        mQParams.mvOut.reserve(InQParams.mvOut.size());
    }
...
#undef TransCase
        ...
        if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
        {
            ...
            LOG_DBG("[EIS CRZ crop]ID(%d),crop(%d,%d),p_int(%d,%d),p_fra(%d,%d),resize(%d,%d)",
            ...
            eisCrop.mGroupID = 4;
            eisCrop.mCropRect.p_integral.x = 0;
            eisCrop.mCropRect.p_integral.y = 0;
            eisCrop.mCropRect.p_fractional.x = 0;
            eisCrop.mCropRect.p_fractional.y = 0;
            eisCrop.mCropRect.s.w = PostProcOutWidth;
            eisCrop.mCropRect.s.h = PostProcOutHeight;
            eisCrop.mResizeDst.w  = PostProcOutWidth;
            eisCrop.mResizeDst.h  = PostProcOutHeight;
            LOG_DBG("[EIS IMG3O crop]ID(%d),crop(%d,%d),p_int(%d,%d),p_fra(%d,%d),resize(%d,%d)",
                    eisCrop.mGroupID,eisCrop.mCropRect.s.w,eisCrop.mCropRect.s.h,eisCrop.mCropRect.p_integral.x,eisCrop.mCropRect.p_integral.y,
                    eisCrop.mCropRect.p_fractional.x,eisCrop.mCropRect.p_fractional.y,eisCrop.mResizeDst.w,eisCrop.mResizeDst.h);
            mQParams.mvCropRsInfo.push_back(eisCrop);

            NSCam::NSIoPipe::NSPostProc::MCrpRsInfo dispCrop;
            dispCrop = InQParams.mvCropRsInfo[eisCrop2Idx];

            LOG_DBG("[Disp ORI crop] ID(%d), crop(%d,%d),p_int(%d,%d),p_fra(%d,%d), cropNoDZ(%d,%d),p_intNoDZ(%d,%d),resize(%d,%d)",
                            dispCrop.mGroupID, dispCrop.mCropRect.s.w, dispCrop.mCropRect.s.h,
                            dispCrop.mCropRect.p_integral.x, dispCrop.mCropRect.p_integral.y,
                            dispCrop.mCropRect.p_fractional.x,dispCrop.mCropRect.p_fractional.y,
                            dispCrop.mCropRectExceptDZ.p_integral.x,
                            dispCrop.mCropRectExceptDZ.p_integral.y,
                            dispCrop.mCropRectExceptDZ.s.w,
                            dispCrop.mCropRectExceptDZ.s.h,
                            dispCrop.mResizeDst.w,dispCrop.mResizeDst.h);

            MBOOL enDisp = MFALSE;

            {

                //Keep crop EIS margin for preview of none-EIS effect
                MUINT32 newCropW = (dispCrop.mCropRect.s.w*100.0f/(EIS_FACTOR));
                MUINT32 newCropH = (dispCrop.mCropRect.s.h*100.0f/(EIS_FACTOR));

                newCropW &= (~1);
                newCropH &= (~1);

                MUINT32 newCropX = dispCrop.mCropRect.p_integral.x + ((dispCrop.mCropRect.s.w - newCropW) >> 1);
                MUINT32 newCropY = dispCrop.mCropRect.p_integral.y + ((dispCrop.mCropRect.s.h - newCropH) >> 1);

                MBOOL isInvalid = MFALSE;
                if (newCropX >= eisCropRect.x)
                {
                    dispCrop.mCropRect.p_integral.x = newCropX - eisCropRect.x;
                }else
                {
                    dispCrop.mCropRect.p_integral.x = 0;
                    isInvalid = MTRUE;
                    LOG_WRN("X MDP: %d  CRZ: %d New: %d", newCropX, eisCropRect.x, dispCrop.mCropRect.p_integral.x);
                }

                if (newCropY >= eisCropRect.y)
                {
                    dispCrop.mCropRect.p_integral.y = newCropY - eisCropRect.y;
                }else
                {
                    dispCrop.mCropRect.p_integral.y = 0;
                    isInvalid = MTRUE;
                    LOG_WRN("Y MDP: %d  CRZ: %d New: %d", newCropY, eisCropRect.y, dispCrop.mCropRect.p_integral.y);
                }

                if( newCropW <= eisCropRect.w )
                {
                    dispCrop.mCropRect.s.w = newCropW;
                }
                else
                {
                    dispCrop.mCropRect.s.w = PostProcOutWidth;
                    isInvalid = MTRUE;
                    LOG_WRN("W MDP: %d CRZ: %d. New: %d", newCropW, eisCropRect.w, dispCrop.mCropRect.s.w);
                }

                if( newCropH <= eisCropRect.h )
                {
                    dispCrop.mCropRect.s.h = newCropH;
                }
                else
                {
                    dispCrop.mCropRect.s.h = PostProcOutHeight;
                    isInvalid = MTRUE;
                    LOG_WRN("H MDP: %d CRZ: %d New: %d", newCropH, eisCropRect.h, dispCrop.mCropRect.s.h);
                }

                if( isInvalid )
                {
                    LOG_WRN("[Disp ORI crop] ID(%d), crop(%d,%d),p_int(%d,%d),p_fra(%d,%d), cropNoDZ(%d,%d),p_intNoDZ(%d,%d),resize(%d,%d)",
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mGroupID, InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.s.w, InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.s.h,
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.p_integral.x, InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.p_integral.y,
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.p_fractional.x, InQParams.mvCropRsInfo[eisCrop2Idx].mCropRect.p_fractional.y,
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.x,
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.p_integral.y,
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.s.w,
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mCropRectExceptDZ.s.h,
                                    InQParams.mvCropRsInfo[eisCrop2Idx].mResizeDst.w, InQParams.mvCropRsInfo[eisCrop2Idx].mResizeDst.h);
                    LOG_WRN("[Disp crop - MDP] ID(%d),crop(%d,%d),p_int(%d,%d),p_fra(%d,%d), cropNoDZ(%d,%d),p_intNoDZ(%d,%d),resize(%d,%d)",
                                    dispCrop.mGroupID, dispCrop.mCropRect.s.w, dispCrop.mCropRect.s.h,
                                    dispCrop.mCropRect.p_integral.x, dispCrop.mCropRect.p_integral.y,
                                    dispCrop.mCropRect.p_fractional.x,dispCrop.mCropRect.p_fractional.y,
                                    dispCrop.mCropRectExceptDZ.p_integral.x,
                ...
                LOG_DBG("[Disp crop - MDP] ID(%d),crop(%d,%d),p_int(%d,%d),p_fra(%d,%d), cropNoDZ(%d,%d),p_intNoDZ(%d,%d),resize(%d,%d)",
                                dispCrop.mGroupID, dispCrop.mCropRect.s.w, dispCrop.mCropRect.s.h,
                                dispCrop.mCropRect.p_integral.x, dispCrop.mCropRect.p_integral.y,
                                dispCrop.mCropRect.p_fractional.x,dispCrop.mCropRect.p_fractional.y,
                                dispCrop.mCropRectExceptDZ.p_integral.x,
                ...
                mQParams.mvCropRsInfo.push_back(dispCrop);
                enDisp = MTRUE;
            }
            ...
            for (int i=0; i < InQParams.mvOut.size(); i++)
            {
                if (InQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Disp || !mEISRecStart)
                {
                    LOG_DBG("[P2A isDisp(%d) sets mvOut (%d)] imgfmt(%d),w(%d),h(%d),type(%d),index(%d),inout(%d),mem( va(0x%x)/pa(0x%x)/size(0x%x) ),p2pxlid(%d),magicnum(%d), eisRec(%d)",
                        InQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Disp,
                        i,
                        InQParams.mvOut[i].mBuffer->getImgFormat(),
                        InQParams.mvOut[i].mBuffer->getImgSize().w,
                        InQParams.mvOut[i].mBuffer->getImgSize().h,
        ...
        else if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))  // For non-EIS case, mCropRsInfo will not be changed, so use the mCropRsInfo MW passed down (InQParams.mvCropRsInfo[1]).
        {
            ...
            mQParams.mvCropRsInfo.push_back(crop);
            MY_LOGD_IF(sDebugLogEnable,"[P2A vFB sets mCropRsInfo] groupid(%d),crop w(%d),h(%d),p_int(%d,%d),p_fra(%d,%d), resize w(%d),h(%d)",
                    crop.mGroupID,crop.mCropRect.s.w,crop.mCropRect.s.h,crop.mCropRect.p_integral.x,crop.mCropRect.p_integral.y,
                    crop.mCropRect.p_fractional.x,crop.mCropRect.p_fractional.y,crop.mResizeDst.w,crop.mResizeDst.h);
        }
    }
...
static const NSCam::NSIoPipe::PortID WDMAO( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_WDMAO, 1);
        ...
        MY_LOGD_IF(sDebugLogEnable,"[P2A sets WDMAO mvOut] imgfmt(%d),w(%d),h(%d),type(%d),index(%d),inout(%d),mem( va(0x%x)/pa(0x%x)/size(0x%x) ),p2pxlid(%d)",
        ...
        mQParams.mvCropRsInfo.push_back(crop);
        MY_LOGD_IF(sDebugLogEnable,"[P2A sets mCropRsInfo] groupid(%d),crop w(%d),h(%d),p_int(%d,%d),p_fra(%d,%d), resize w(%d),h(%d)",
                crop.mGroupID,crop.mCropRect.s.w,crop.mCropRect.s.h,crop.mCropRect.p_integral.x,crop.mCropRect.p_integral.y,
                crop.mCropRect.p_fractional.x,crop.mCropRect.p_fractional.y,crop.mResizeDst.w,crop.mResizeDst.h);
    }
    ...
    )
    {
        ...
        )   // If not 3DNR only or 3DNR State Machine is not NR3D_STATE_STOP.
        {
            ...
            if( sgmkDebugFlag[0] != '0' )
            {
                MY_LOGD_IF(sDebugLogEnable,"[P2A sets IMG3O mvOut] 3dnrSM0(%d S0P1W2),ES(0x%02X FsSzDfLrIn),WH(%d,%d),OfstXY(%d,%d),mem<va(0x%x)/pa(0x%x)/TS(%lld)/size(0x%x)>",
                    m3dnrStateMachine,
                    m3dnrErrorStatus,
        ...
        {
            MY_LOGD_IF(sDebugLogEnable,"[P2A not sets IMG3O mvOut] 3dnrSM0(%d S0P1W2),ES(0x%02X FsSzDfLrIn),WH(%d,%d),OfstXY(%d,%d),mem<va(0x%x)/pa(0x%x)/TS(%lld)/size(0x%x)>",
                    m3dnrStateMachine,
                    m3dnrErrorStatus,
                    moutput.mBuffer->getImgSize().w, moutput.mBuffer->getImgSize().h,
                    u4Img3oOffset_X, u4Img3oOffset_Y,
    ...
    if (FEATURE_MASK_IS_3DNR_ENABLED(mFeatureOnMask))
    {
        ...
        if (!m3dnrPrvFrmQueue.empty())
        {
            ...
            property_get("vendor.camera.3dnr.forceskip", aInputValue, "0");
            if (aInputValue[0] == '1')
            {
    ...
    if (FEATURE_MASK_IS_3DNR_ENABLED(mFeatureOnMask) ||
        ...
        FEATURE_MASK_IS_STEREO_CAMERA_ENABLED(mFeatureOnMask))
    {
        MY_LOGD_IF(sDebugLogEnable,"FStrm enq (frame %d)", getMagicNo(mQParams));
...
{
    //FUNCTION_LOG_START;
    MY_LOGD_IF(sDebugLogEnable,"data(%d), buf(0x%x), ext(0x%x)", data, buf, ext);
    switch (data)
    {
        case P2A_TO_VFBSW_DSIMG:
        case P2A_TO_SWITCHER:
        case P2A_TO_GPU:
            MY_LOGD_IF(sDebugLogEnable,"Recycling P2A node %s image of frame %d", (data == P2A_TO_VFBSW_DSIMG)? "DS": "full", ext);
...
threadLoopUpdate()
{
    ...
    if (!mqJobFromGpu.empty())
    {
        ...
        /* Post buffer to next node */
        MY_LOGD_IF(sDebugLogEnable,"Posting SWITCHER(GPU)->P2B node full image of frame %d", job.mFrameNo);
            ...
            (mGpuProcCount == 0))
        {
            while(!mqJobFromP2a.empty())
            {
                ...
                if (mVfbExtreme ||
                    FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
                {
                    MY_LOGD_IF(sDebugLogEnable,"Posting SWITCHER->GPU node full image of frame %d", job.mFrameNo);
                ...
                else
                {
                    ...
                    MY_LOGD_IF(sDebugLogEnable,"Posting SWITCHER(P2A)->P2B node full image of frame %d", job.mFrameNo);
                    handlePostBuffer(SWITCHER_TO_P2B, (MUINTPTR) &job.imgReq, job.mFrameNo);
                }
                ...
                MY_LOGD_IF(sDebugLogEnable,"Posting SWITCHER->VFB node full image of frame %d", job.mFrameNo);
...
FeaturePipeGpuNode::onInit()
{
    ...
    {
        char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("vendor.camera.featurepipe.dumpgpu", EnableOption, "0");
        if(EnableOption[0] == '2' || EnableOption[0] == '3')
        {
...
threadInit()
{
    FUNCTION_LOG_START;
    MRESULT mret;
    vector< sp<NativeBufferWrapper>* > vpGB;
    ...
    Warp_SPTR srcGBArray[vpGB.size()];
#else
    Warp_SPTR srcGBArray[1];
    /* Allocate Graphic buffer */
    mGfxYuvBuffer = new NativeBufferWrapper(FULL_IMAGE_MAX_WIDTH, FULL_IMAGE_MAX_HEIGHT,
                                            HAL_PIXEL_FORMAT_RGBA_8888, NativeBufferWrapper::USAGE_HW_TEXTURE,
                                            "GfxYuvBuffer");
...
#if GPU_YV12_SUPPORTED
            mGfxRgbaBuffer[i] = new NativeBufferWrapper(FULL_GPU_MAX_OUTPUT_WIDTH, FULL_GPU_MAX_OUTPUT_HEIGHT,
                                                        HAL_PIXEL_FORMAT_YV12, NativeBufferWrapper::USAGE_HW_RENDER,
                                                        "GfxRgbaBuffer" + std::to_string(i));
#else
            mGfxRgbaBuffer[i] = new NativeBufferWrapper(FULL_GPU_MAX_OUTPUT_WIDTH, FULL_GPU_MAX_OUTPUT_HEIGHT,
                                                        HAL_PIXEL_FORMAT_RGBA_8888, NativeBufferWrapper::USAGE_HW_RENDER,
                                                        "GfxRgbaBuffer" + std::to_string(i));
#endif
    ...
    Warp_SPTR dstGBArray[GPU_NODE_BUFFER_NUM];
    ...
    warp_info.ImgFmt = WARP_IMAGE_YV12;
    {
        int i = 0;
        {
            for (vector< sp<NativeBufferWrapper>* >::iterator it=vpGB.begin(); it!=vpGB.end(); it++)
            {
                srcGBArray[i++] = (**it)->getWarp_SPTR();
            }
        }
    }
#else
    ...
    {
        srcGBArray[0] = mGfxYuvBuffer->getWarp_SPTR();
    }
#endif //_WORKAROUND_GPU_FLASH
#if GPU_YV12_SUPPORTED
    ...
    for (MUINT8 i = 0; i < GPU_NODE_BUFFER_NUM; i++)
    {
        dstGBArray[i] = mGfxRgbaBuffer[i]->getWarp_SPTR();
    }
...
removeQueuedInfos()
{
    ...
    {
        Mutex::Autolock lock(mLock);
        LOG_DBG("need wait Fulljobs(%u) & warpJobs(%u) done", mqJobFullImg.size(), mqJobWarpMap.size());
        while(mqJobFullImg.size() > 0)
        {
            FeaturePipeNodeImgReqJob_s jobFullImg;
        ...
        /* In fact, mqJobWarpMap should be already empty before this calling...*/
        while(mqJobWarpMap.size() > 0)
        {
            FeaturePipeNodeImgReqJob_s jobWarpImg;
            jobWarpImg = mqJobWarpMap.front();
            mqJobWarpMap.pop();
            if(jobWarpImg.imgReq.mBuffer == NULL)
            {
                LOG_DBG("Not return dummp buffer (0x0) to pool");
            }
            else
                handleReturnBuffer(jobWarpImg.dataType, (MUINTPTR) &jobWarpImg.imgReq, jobWarpImg.mFrameNo);
        }
    }
    FUNCTION_LOG_END;
}
...
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    //FUNCTION_LOG_START;
    Mutex::Autolock lock(mLock);
    LOG_DBG("data(%d), buf(0x%x), ext(0x%x), queueFull/Warp(%u,%u)", data, buf, ext, mqJobFullImg.size(),mqJobWarpMap.size());

    MUINT32 numReadyJobs = MIN(mqJobFullImg.size(),mqJobWarpMap.size());

    FeaturePipeNodeImgReqJob_s job;
    job.dataType = (NSCamNode::NodeDataTypes) data;
    if (data == EISSW_TO_GPU) {
        job.imgReq = ((TagImgRequest*)buf)->imgReq;
        job.needByPass = ((TagImgRequest*)buf)->needByPass;
    } else {
        job.imgReq = *(ImgRequest*) buf;
    }
    job.imgReq = *(ImgRequest*) buf;
...
threadLoopUpdate()
{
...

    {
        ...
        MUINT32 readyJobs = MIN(mqJobFullImg.size(),mqJobWarpMap.size());

        /* Get warp job */
        jobWarpMap = mqJobWarpMap.front();
        mqJobWarpMap.pop();

        if(jobWarpMap.imgReq.mBuffer == NULL)
        {
            LOG_DBG("GPU receive dummy warp from EIS, frameNo(%u)", jobWarpMap.mFrameNo);
            handlePostBuffer(GPU_TO_MDP, (MUINTPTR) &jobWarpMap.imgReq, jobWarpMap.mFrameNo);
            return MTRUE;
        }
    ...
    /* By Pass GPU */
    MBOOL byPassGPU = (jobFullImg.needByPass || jobWarpMap.needByPass);
    if (byPassGPU)
    {
        int64_t fullTs = jobFullImg.imgReq.mBuffer->getTimestamp();
        /* Return buffer to previous node */
        if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
        {
            LOG_DBG("byPassGPU, Returning VFB node warp map buffer %d", jobWarpMap.mFrameNo);
            handleReturnBuffer(VFBSW_TO_GPU, (MUINTPTR) &jobWarpMap.imgReq, jobWarpMap.mFrameNo);
        }
        else if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
        {
            LOG_DBG("byPassGPU, Returning EIS node warp map buffer %d", jobWarpMap.mFrameNo);
            handleReturnBuffer(EISSW_TO_GPU, (MUINTPTR) &jobWarpMap.imgReq, jobWarpMap.mFrameNo);
        }

        LOG_DBG("byPassGPU, Returning P2A node full image buffer %d", jobFullImg.mFrameNo);
        handleReturnBuffer(jobFullImg.dataType, (MUINTPTR) &jobFullImg.imgReq, jobFullImg.mFrameNo);
        /* Post buffer to next node */
        if(FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) || FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
        {
            TagImgRequest fullImgReq;
            if(!pBufHandler->dequeBuffer(GPU_TO_MDP, &fullImgReq.imgReq))
            {
                LOG_AST(false, "dequeBuffer fail. Out of GPU_TO_MDP buffer", 0);
                return MFALSE;
            }
            const_cast<IImageBuffer*>(fullImgReq.imgReq.mBuffer)->setTimestamp(fullTs);
            fullImgReq.needByPass = MTRUE;
            LOG_DBG("byPassGPU, Posting GPU->MDP node GB of frame %d, timestamp(%llu)",
                    frameNo, fullImgReq.imgReq.mBuffer->getTimestamp());
    ...
    mGfxYuvBuffer->lock(NativeBufferWrapper::USAGE_SW, (void**)(&gfxYuvBuf));
    //***************************src YV12********************************//
    DpBlitStream dpBlit;
...
#endif
    {
        ...
        saveBufToFile(szFileName, (MUINT8*) jobFullImg.imgReq.mBuffer->getBufVA(0),
            jobFullImg.imgReq.mBuffer->getBufSizeInBytes(0)+
            jobFullImg.imgReq.mBuffer->getBufSizeInBytes(1)+
            jobFullImg.imgReq.mBuffer->getBufSizeInBytes(2));
    }
#else
...
#ifndef _WORKAROUND_GPU_FLASH
    if( !reinterpret_cast<FeaturePipeBufHandler*>(pBufHandler)->getGraphicBuffer((jobFullImg.dataType == SWITCHER_TO_GPU)? P2A_TO_SWITCHER: jobFullImg.dataType, const_cast<IImageBuffer*>(jobFullImg.imgReq.mBuffer), (Warp_SPTR*) &(warp_info.SrcGraphicBuffer)) )
    {
        LOG_ERR("Failed to get GB from ImageBuffer(%x)", const_cast<IImageBuffer*>(jobFullImg.imgReq.mBuffer));
        return MFALSE;
    }
#else
    warp_info.SrcGraphicBuffer = (void*)(mGfxYuvBuffer->getWarp_SPTR());
#endif //_WORKAROUND_GPU_FLASH
    ...
    if( !reinterpret_cast<FeaturePipeBufHandler*>(pBufHandler)->getGraphicBuffer(GPU_TO_MDP, const_cast<IImageBuffer*>(fullImgReq.mBuffer), (Warp_SPTR*) &(warp_info.DstGraphicBuffer)) )
    {
    ...
    NativeBufferWrapper::lock((Warp_SPTR)warp_info.DstGraphicBuffer,
                              NativeBufferWrapper::USAGE_SW, (void**)(&gfxRgbaBuf));
...
#endif
    const_cast<IImageBuffer*>(fullImgReq.mBuffer)->setTimestamp(jobFullImg.imgReq.mBuffer->getTimestamp());
    ...
    {
        /* Post buffer to next node */
        LOG_DBG("Posting GPU->MDP node GB of frame %d, timestamp(%llu)", frameNo, fullImgReq.mBuffer->getTimestamp());
        TagImgRequest outTagReq;
        outTagReq.imgReq = fullImgReq;
        handlePostBuffer(GPU_TO_MDP, (MUINTPTR) &outTagReq, frameNo);
    }
...
FeaturePipeMdpNode::onInit()
{
    ...
    if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("vendor.camera.featurepipe.dumpmdp", EnableOption, "0");
        if(EnableOption[0] == '1' || EnableOption[0] == '3')
        {
            pBufHandler = getBufferHandler(8001);
            if (pBufHandler == NULL)
...
removeQueuedInfos()
{
    ...
    {
        Mutex::Autolock lock(mLock);
        LOG_DBG("need wait mdpInfo(%u) cleared, queueFull/Qparam(%u,%u)", mqJobMdpInfo.size(),
            mqFullImgJob.size(), mqQParamsJob.size());
        while(! mqJobMdpInfo.empty())
        {
            mqJobMdpInfo.pop();
        }
    }
...
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    //FUNCTION_LOG_START;
    Mutex::Autolock lock(mLock);
    LOG_DBG("data(%d), buf(0x%x), ext(0x%x), queueFull/Qparam(%u,%u)", data, buf, ext,
        mqFullImgJob.size(), mqQParamsJob.size());
    ...
    if (data == GPU_TO_MDP) {
        ...
        // No need to queue job because input buf is QParam.
    }
...
threadLoopUpdate()
{
    ...
    // Check input buffer not NULL except EIS 2.5
    if(jobFullImg.imgReq.mBuffer == NULL &&
        ( FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask) || !FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    ){
        LOG_ERR("MDPNode input image is NULL with incorrent feature running. FeatureMask(0x%x), frameNo(%u)",
                mFeatureOnMask, frameNo);
        return MFALSE;
    }
    ...
    if (FEATURE_MASK_IS_VFB_ENABLED(mFeatureOnMask))
    {
        LOG_AST(!jobFullImg.needByPass, "VFB should NOT get into ByPass Mode !!");
    ...
    else if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
...
#endif
        PostProcOutWidth = imgiCrop.s.w;
        PostProcOutHeight = imgiCrop.s.h;
...
#if 0
        LOG_DBG("fullWH(%u,%u),imgiC(%u,%u),tmpC(%u,%u),PPOut(%u,%u), gpuTarget(%u,%u)", fullWidth, fullHeight, imgiCrop.p_integral.x, imgiCrop.p_integral.y,
                    tmpCrop.p_integral.x, tmpCrop.p_integral.y, PostProcOutWidth, PostProcOutHeight, gpuTargetW, gpuTargetH);

        LOG_DBG("MDPNode mdpCrop (%u,%u/%u,%u), W/H(%u,%u)",mdpCrop.p_integral.x, mdpCrop.p_integral.y,
             mdpCrop.p_fractional.x, mdpCrop.p_fractional.y, mdpCrop.s.w,mdpCrop.s.h);
#endif

        // ======= Get crop info if EIS 3.0 has queue buffers ========
        {
            MdpInfoReqJob_s curMdpInfo;
            MUINT32 qParamNo = getMagicNo(OutQParams);
            curMdpInfo.mFrameNo = qParamNo;
            curMdpInfo.mdpCrop = mdpCrop;
            //mdpInfo.mdpCrop.p_integral = mdpCrop.p_integral;
            //mdpInfo.mdpCrop.p_fractional = mdpCrop.p_fractional;
            //mdpInfo.mdpCrop.s = mdpCrop.s;
            Mutex::Autolock lock(mLock);

            if(jobFullImg.imgReq.mBuffer == NULL)// In EIS 3.0 queuing
            {
                mqJobMdpInfo.push(curMdpInfo);
                LOG_DBG("MDPNode push_back mdpCrop (%u),QParamNo(%u) (%u,%u/%u,%u), W/H(%u,%u)",
                    frameNo, qParamNo, mdpCrop.p_integral.x, mdpCrop.p_integral.y,
                    mdpCrop.p_fractional.x, mdpCrop.p_fractional.y, mdpCrop.s.w,mdpCrop.s.h);
            }
            else if(! mqJobMdpInfo.empty()) // In EIS 3.0 runningcase
            {
                mqJobMdpInfo.push(curMdpInfo);
                MdpInfoReqJob_s queudMdpInfo = mqJobMdpInfo.front();
                mqJobMdpInfo.pop();
                mdpCrop = queudMdpInfo.mdpCrop;

                LOG_DBG("MDPNode replaced mdpCrop (%u), QparamNo(%u->%u), (%u,%u/%u,%u,%ux%u)->(%u,%u/%u,%u,%ux%u)",
                    frameNo, qParamNo, queudMdpInfo.mFrameNo,
                    curMdpInfo.mdpCrop.p_integral.x, curMdpInfo.mdpCrop.p_integral.y,
                    curMdpInfo.mdpCrop.p_fractional.x, curMdpInfo.mdpCrop.p_fractional.y,
                    curMdpInfo.mdpCrop.s.w, curMdpInfo.mdpCrop.s.h,
                    mdpCrop.p_integral.x, mdpCrop.p_integral.y,
                    mdpCrop.p_fractional.x, mdpCrop.p_fractional.y, mdpCrop.s.w,mdpCrop.s.h);
            }
            else
            {
                LOG_DBG("MDPNode mdpCrop (%u), QParamNo(%u)(%u,%u/%u,%u), W/H(%u,%u)",mdpCrop.p_integral.x,
                frameNo, qParamNo, mdpCrop.p_integral.y,
                mdpCrop.p_fractional.x, mdpCrop.p_fractional.y, mdpCrop.s.w,mdpCrop.s.h);
            }
        }

        // ======= Set rec buffer invalid if EIS 3.0 queue buffers ========
        if(jobFullImg.imgReq.mBuffer == NULL)
        {
            MBOOL findRecOut = MFALSE;
            for(int i=0; i < OutQParams.mvOut.size(); i++)
            {
                if (OutQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Rcrd)
                {
                    OutQParams.mvOut[i].mBuffer->setTimestamp(0); // mark rec buffer invalid
                    findRecOut = MTRUE;
                }
            }
            if(! findRecOut)
                LOG_ERR("MDP input is NULL in EIS but QParam has no rec output buf!!, frameNo(%u)", frameNo);

            OutQParams.mDequeSuccess = MTRUE;
            if(OutQParams.mpfnCallback != NULL)
            {
                LOG_DBG("Callback of frame %d with invalid rec buf for EIS, frameNo(%u)", getMagicNo(OutQParams), frameNo);
        ...
        MBOOL bDispBypass = MFALSE;
        //NSCam::NSIoPipe::NSPostProc::MCrpRsInfo dispCrop;
        //dispCrop = OutQParams.mvCropRsInfo[eisCrop2Idx];

        //if ( (dispCrop.mCropRectExceptDZ.s.w == dispCrop.mCropRect.s.w) &&
        //     (dispCrop.mCropRectExceptDZ.s.h == dispCrop.mCropRect.s.h))
        {
            bDispBypass = MTRUE;
        }
#if 0
        ...
        if ((OutQParams.mvOut.size() == 1)  && bDispBypass &&
            (OutQParams.mvOut[0].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Disp) || jobFullImg.needByPass)
        {
            LOG_DBG("bypass MDP: mvOut Size=%d, ImgReq needByPass(%d)",OutQParams.mvOut.size(), jobFullImg.needByPass);
        }else
        {
            ...
            MUINT32 size[3] = {pImageBuffer->getImgSize().w * pImageBuffer->getImgSize().h* (DP_COLOR_BITS_PER_PIXEL(MDPInputFormat)>>3) ,
                               0, 0};
#endif
            ...
            for(int i=0; i < OutQParams.mvOut.size(); i++)
            {
                if ((OutQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Disp) && bDispBypass)
                {
                    LOG_DBG("MDP display bypass: mvOut index=%d",i);
                    continue;
                }
...
#undef TransCase
                ...
                if (mpStream->setRotation(outCnt, u4Rotation))
                {
                    LOG_ERR("DpIspStream->setDstConfig failed");
                    return MFALSE;
                }

                if (mpStream->setFlipStatus(outCnt, u4Flip))
                {
                    LOG_ERR("DpIspStream->setDstConfig failed");
                    return MFALSE;
                }
                ...
                if (OutQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Rcrd)
                {
                    ...
                    LOG_DBG("mvOut record: index=%d", i);
                }

                if (mpStream->setDstConfig(outCnt, //port
                ...
                if (mpStream->queueDstBuffer(outCnt, //port
                                             (void**)va,
                                             pa,
                                             size,
            ...
            for(int i=0; i < OutQParams.mvOut.size(); i++)
            {
                ...
                if ((OutQParams.mvOut[i].mPortID.capbility == NSCam::NSIoPipe::EPortCapbility_Disp) && bDispBypass)
                {
                    LOG_DBG("MDP display bypass: index=%d, time:%llu",i, OutQParams.mvOut[i].mBuffer->getTimestamp());
                    continue;
                }

                if (mpStream->dequeueDstBuffer(outCnt,(void**)va) < 0)
                {
                    LOG_ERR("dequeueDstBuffer failed");
                    return MFALSE;
                }
                LOG_DBG("MDP record: index=%d, time:%llu, newTime:%llu",i, OutQParams.mvOut[i].mBuffer->getTimestamp(),
                        pImageBuffer->getTimestamp());
                OutQParams.mvOut[i].mBuffer->setTimestamp(pImageBuffer->getTimestamp());
                outCnt++;
            }
            ...
            // dump
            {
                ...
                property_get("vendor.camera.featurepipe.dumpmdp", debugFlag, "0");
...
#ifndef _DEBUG_DUMP
    char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.featurepipe.dumpp2b", EnableOption, "0");
    if (EnableOption[0] == '1')
#endif
    {
...
static const NSCam::NSIoPipe::PortID LCEI( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_LCEI, 0);
    ...
    /* IMGI */
    MY_LOGD_IF(sDebugLogEnable,"Received VFB->P2B ds req(%x) mbuffer(%x)", &jobDsImg.imgReq, jobDsImg.imgReq.mBuffer);
    minput.mPortID = IMGI;
    ...
    {
        MY_LOGD_IF(sDebugLogEnable,"[P2B sets mvIn (%d)] imgfmt(%d),w(%d),h(%d),type(%d),index(%d),inout(%d),mem( va(0x%x)/pa(0x%x)/size(0x%x) ),p2pxlid(%d),magicnum(%d)",i,
    ...
    for (int i=0; i < OutQParams.mvOut.size(); i++)
    {
        MY_LOGD_IF(sDebugLogEnable,"[P2B gets mvOut (%d)] imgfmt(%d),w(%d),h(%d),type(%d),index(%d),inout(%d),mem( va(0x%x)/pa(0x%x)/size(0x%x) ),p2pxlid(%d),magicnum(%d)",i,
    ...
    for (int i=0; i < OutQParams.mvCropRsInfo.size(); i++)
    {
        ...
        MY_LOGD_IF(sDebugLogEnable,"[P2B sets mCropRsInfo] groupid(%d),crop w(%d),h(%d),p_int(%d,%d),p_fra(%d,%d), resize w(%d),h(%d)",
                crop.mGroupID,crop.mCropRect.s.w,crop.mCropRect.s.h,crop.mCropRect.p_integral.x,crop.mCropRect.p_integral.y,
                crop.mCropRect.p_fractional.x,crop.mCropRect.p_fractional.y,crop.mResizeDst.w,crop.mResizeDst.h);
