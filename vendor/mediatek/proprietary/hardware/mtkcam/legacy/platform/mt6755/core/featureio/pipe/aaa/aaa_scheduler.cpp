...
long long int AAA_Scheduler::getAEThreadVsTimeOutLimit_ns()
{
    long long int ret = ::getAEThreadVsTimeOutLimit_ns(mNormalizeM, mfps);
    if (mAllLogEn)
        ALOGD("[%s] (mfps, mNormalizeM, TimeOutLimit) = (%d, %d, %d)\n"
            , __FUNCTION__
            , mfps
...
MVOID AAA_Scheduler::resetCycleCnt()
{
    resetCycleCtr(mSensorDevId);
}
