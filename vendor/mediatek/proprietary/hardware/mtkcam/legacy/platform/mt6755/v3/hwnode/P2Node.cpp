    ...
    : public VirtualLightRefBase
{
    ...
    protected:
                                            ...
                                            , muDequeCnt(0)
                                        {
                                            ...
                                            ::property_get("vendor.debug.camera.P2Node.p2dump", cDumpValue, "0");
    ...
    : public VirtualLightRefBase
{
    ...
    protected:
                                        ...
                                        {
                                            char cDumpValue[PROPERTY_VALUE_MAX];
                                            ::property_get("vendor.debug.camera.P2Node.mdpdump", cDumpValue, "0");
                                            mDumpEnable = atoi(cDumpValue);
                                        }
...
inline MBOOL                        isOpaqueLocked(StreamId_T const streamId) const {
    ...
    , mpMdpProcessor(NULL)
{
    ...
    if ( mLogLevel == 0 ) {
        ::property_get("vendor.debug.camera.log.P2Node", cLogLevel, "0");
        mLogLevel = atoi(cLogLevel);
    }
}
...
) const
{
    ...
    if( isFullRawLocked(streamId) || isResizeRawLocked(streamId) ||
        isOpaqueLocked(streamId)  || isYuvReprocLocked(streamId))
        return MTRUE;
    //
...
) const
{
    ...
    MY_LOGD_IF(mLogLevel >= 1, "stream %#" PRIx64 ": buffer: %p, usage: 0x%x",
...
) const
{
    ...
    if( pStreamBuffer.get() == NULL ) {
        MY_LOGE("StreamId %#" PRIx64 ": pStreamBuffer == NULL",
                streamId);
        return;
    }
    //
...
) const
{
    ...
    MY_LOGD_IF(0,"stream %#" PRIx64 ": stream buffer %p, metadata: %p",
...
)
{
    ...
    for( size_t run_idx = 0 ; run_idx < imageIOMapSet.size(); run_idx++ )
    {
        ...
        // source
        {
            ...
            param_p2.bYuvReproc = isYuvReprocLocked(streamId);
            //
            {
                RWLock::AutoRLock _l(mConfigRWLock);
        ...
        MUINT32 occupied = 0;
        float ratio_s = 0.0f;
        sp<IImageStreamInfo> info_src = imageIOMap.vIn.valueAt(0);
        MSize sensor_size = info_src->getImgSize();
        MUINT32 target_transform = 0;
        if( ! tryGetMetadata<MSize>(pMeta_InHal->getMetadata(), MTK_HAL_REQUEST_SENSOR_SIZE, sensor_size) ) {
            MY_LOGE("cannot get MTK_HAL_REQUEST_SENSOR_SIZE");
        }
        else
            ratio_s = (float)sensor_size.w/(float)sensor_size.h;
        MY_LOGD_IF(mLogLevel >= 1, "src buffer size:%dx%d", sensor_size.w, sensor_size.h);
        #define abs(x,y) ((x)>(y)?(x)-(y):(y)-(x))
        vector<int> vDispatch(imageIOMap.vOut.size(), -1);
        enum type {none = 1 << 0, src = 1 << 1, others = 1 << 2};
        int mark = none;
        vector<type> vRatio(imageIOMap.vOut.size(), type::none);
        for( size_t i = 0; i < imageIOMap.vOut.size(); i++ )
        {
            float ratio = 0.0f;
            StreamId_T const streamId = imageIOMap.vOut.keyAt(i);
            MUINT32 transform = imageIOMap.vOut.valueAt(i)->getTransform();
            sp<IImageStreamInfo> info = imageIOMap.vOut.valueAt(i);
            ratio = (transform & eTransform_ROT_90) ? ((float)info->getImgSize().h/(float)info->getImgSize().w) :
                    ((float)info->getImgSize().w/(float)info->getImgSize().h);
            MY_LOGD_IF(mLogLevel >= 1, "imageIOMap.vOut: %zu/%zu w,h: %zux%zu",
                       i+1, imageIOMap.vOut.size(), info->getImgSize().w, info->getImgSize().h);
            if(isStream(mpOutFd, streamId))
                continue;
            else if(abs(ratio_s, ratio) > 0.1f )
            {
                mark |= type:: others;
                vRatio[i] = type::others;
                MY_LOGD_IF(mLogLevel >= 1,"output ratio is not the same as src buffer, so will use mdp to run 2nd. mark(0x%x)", mark);
            }
            else {
                mark |= type::src;
                vRatio[i] = type::src;
                MY_LOGD_IF(mLogLevel >=1 , "output ratio is the same as src buffer, mark(0x%x)", mark);
            }
        }
        #undef abs
        if( (mark&type::src && !(mark&type::others)) || (!(mark&type::src) && mark&type::others) ){
            MY_LOGD_IF(mLogLevel >= 1, "output are all the same ratio... ");
            std::fill(vRatio.begin(), vRatio.end(), type::none);
        }
        ...
        for( size_t i = 0; i < imageIOMap.vOut.size(); i++ )
        {
            ...
            PortID port_p2;
            if( vRatio[i]!=type::others && OK == mapPortId(streamId, transform, occupied, port_p2) ) {
                vDispatch[i] = 1; //for p2 output mdp output
                P2Processor::FrameOutput out;
                out.mPortId = port_p2;
                out.mHandle = StreamBufferHandle::create(this, pFrame, streamId);
                out.mTransform = transform;
                target_transform = transform;
                MY_LOGD_IF(mLogLevel >= 1,"select job for P2 streamId: %#" PRIx64 " transform:%x " , streamId, out.mTransform );
                //
                param_p2.vOut.push_back(out);
            } else {
                vDispatch[i] = 2; //for mdp output
                MY_LOGD_IF(mLogLevel >= 1,"select job for MDP streamId: %#" PRIx64 " Transform:%x " , streamId, transform );
            }
        }
        // create MDP handle
        auto SelectP2MaxOutput = [&](){
            if(vDispatch.size() == 0)
                MY_LOGE("vDispatch is null");
            int targetIdx = 0;
            MSize maxSize(0,0);
            size_t p2paramIdx = 0;
            for(size_t i=0 ;i<vDispatch.size();i++)
            {
                sp<IImageStreamInfo> info = imageIOMap.vOut.valueAt(i);
                MY_LOGD_IF(mLogLevel >= 1,"i:%zu wxh:%zux%zu" , i, info->getImgSize().w ,info->getImgSize().h);
                if(vDispatch[i]==1) {
                   if(info->getImgSize().w*info->getImgSize().h > maxSize.w*maxSize.h) {
                        maxSize = info->getImgSize();
                        targetIdx = p2paramIdx;
                   }
                   p2paramIdx ++;
                }
            }
            MY_LOGD_IF(mLogLevel >= 1,"SelectP2MaxOutputIdx : %d " , targetIdx );
            return targetIdx;
        };
        int tarOfMdpInput = SelectP2MaxOutput();
        MUINT32 const tarTransform = imageIOMap.vOut.valueAt(tarOfMdpInput)->getTransform();
        auto transtransform = [](MUINT32 const transform)->MUINT32 {
                                switch (transform)
                                {
                                #define TransCase( trans, result ) \
                                    case (trans):              \
                                        return result;         \
                                        break;
                                    TransCase(0                  , 0)
                                    TransCase(eTransform_FLIP_H  , eTransform_FLIP_H)
                                    TransCase(eTransform_FLIP_V  , eTransform_FLIP_V)
                                    TransCase(eTransform_ROT_180 , eTransform_ROT_180)
                                    TransCase(eTransform_ROT_90  , eTransform_ROT_270)
                                    TransCase(eTransform_FLIP_H|eTransform_ROT_90 , eTransform_FLIP_H|eTransform_ROT_270)
                                    TransCase(eTransform_FLIP_V|eTransform_ROT_90 , eTransform_FLIP_V|eTransform_ROT_270)
                                    TransCase(eTransform_ROT_270 , eTransform_ROT_90)
                                    default:
        ...
        for( size_t i = 0; i < imageIOMap.vOut.size(); i++ )
        {
            if(vDispatch[i] != 2) continue;
            StreamId_T const streamId = imageIOMap.vOut.keyAt(i);
            MUINT32 const transform = imageIOMap.vOut.valueAt(i)->getTransform();
            MDPProcessor::FrameOutput out;
            out.mHandle = StreamBufferHandle::create(this, pFrame, streamId);
            out.mTransform = tarTransform==0 ? transform : transtransform(tarTransform); //TODO: only consider mdp rotate or p2 rotate, just one buffer can rotate
            param_mdp.vOut.push_back(out);
            MY_LOGD_IF(mLogLevel >= 1,"MDP streamId: %#" PRIx64 " Transform:%x tarTransform:%x out.mTransform:%x " , streamId, transform, tarTransform, out.mTransform );
        }
        //
        ...
        if( param_mdp.vOut.size() > 0 ) {
           param_mdp.in.mHandle = param_p2.vOut[tarOfMdpInput].mHandle;
            //
...
#define PORT_IMG2O_USED  (0x4)
    ...
    MY_LOGD_IF(mLogLevel >= 1, "stream id %#" PRIx64 ", occupied 0x%x",
            streamId, rOccupied);
    return ret;
...
)
{
...

    {
        ...
        {
            //updateEntry<MUINT8>(&inMetaSet.halMeta, MTK_3A_ISP_PROFILE, EIspProfile_YUV_Reprocess);
        }
        if( pMeta_OutHal ) {
            ...
            if( dump ) {
                ...
                if( pImgBuf ) {
                    String8 log = String8::format("dump port %s, size %dx%d, fmt %d, stride ",
                            portName.string(),
                            pImgBuf->getImgSize().w, pImgBuf->getImgSize().h, pImgBuf->getImgFormat()
                            );
                    for( size_t i = 0; i < pImgBuf->getPlaneCount(); i++ )
                        log += String8::format("%zu,", pImgBuf->getBufStridesInBytes(i));
                    MY_LOGD("%s", log.string());

                    String8 filename = String8::format("%s%s_%dx%d_%u_%d.yuv",
                            DUMP_PATH,
                            portName.string(), pImgBuf->getImgSize().w, pImgBuf->getImgSize().h,
                            muDequeCnt, rand()%100000);
                    pImgBuf->saveToFile(filename.string());
                }
            }
        }
...
)
{
    ...
    auto calCropAspect = [](MSize srcSize, MSize dstSize) {
        MRect crop;
...
#define align2(x) (((x) + 1) & (~0x1))
        ...
        MY_LOGD("src %dx%d dst %dx%d crop %d,%d,%d,%d", srcSize.w, srcSize.h, dstSize.w, dstSize.h, crop.p.x, crop.p.y, crop.s.w, crop.s.h);
        return crop;
    };
    {
        MRect   const roi       = calCropAspect(pSrc->getImgSize(), vDst[0]->getImgSize());
