...
pthread_mutex_t g_mutex = PTHREAD_MUTEX_INITIALIZER;    //mutex for read/write pass1 dequeued ring buffer count
...
void _test_camio_::startThread(void)
{
...
#ifdef __LP64__
    pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_RR, 0, {0}};
#else
    ...
    //
#ifdef __LP64__
    pthread_attr_t const attr2 = {0, NULL, 1024 * 1024, 4096, SCHED_OTHER, NICE_CAMERA_PASS2, {0}};
#else
...
MVOID* _test_camio_::_infiniteloop(void* arg)
{
    ...
    while (_loop < max_cnt) {
        ...
        if(_this->m_pNormalP_FrmB->deque(dequeBufInfo) == MTRUE){
            for (unsigned int i = 0; i < dequeBufInfo.mvOut.size(); i++) {
                printf("[E_L deque]: %d_dma:0x%x, PA:0x%" PRIxPTR ", crop:0x%x,0x%x,0x%x,0x%x_0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, dummy:0x%x, bufidx:0x%x\n",
                _this->m_Sensoridx,
            ...
            if(!Multi_thread)
            {
                ...
                for (unsigned int i = 0; i < _replace.mvOut.size(); i++) {
                    printf("[E_L enque]: %d_dma:0x%x, PA:0x%" PRIxPTR ", crop:0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, bufidx:0x%x\n",
                    _this->m_Sensoridx,
            ...
            for (unsigned int i = 0; i < dequeBufInfo.mvOut.size(); i++) {
                if(dequeBufInfo.mvOut.at(i).mMetaData.m_bDummyFrame == MFALSE)
                    ...
                    if (dequeBufInfo.mvOut.at(i).mPortID == IMGO)
                        printf("[%d]===  get dummy frame(IMGO) ===\n", _this->m_Sensoridx);
                    else if (dequeBufInfo.mvOut.at(i).mPortID == RRZO)
                        printf("[%d]===  get dummy frame(RRZO) ===\n", _this->m_Sensoridx);
                    else
                        printf("[%d]===  get dummy frame ===\n", _this->m_Sensoridx);
                }
            }
            if(_break == MTRUE){
...
MVOID* _test_camio_::_p2ThreadLoop(MVOID *arg)
{
    ...
    while(1)
    {
        ...
        {
            printf("===m_dequeBufInfoList size(%lu)=== \n", _this->m_dequeBufInfoList.size());
        ...
        else
        {
            ...
            for (unsigned int i = 0; i < p1dequeBufInfo.mvOut.size(); i++)
            {
                if(i==0)
                {
                    ...
                    {    //life cycle for variable
                        ...
                        //LOG_INF("pass2 enque\n");
                        enqret=pStreamObj->enque(enqueParams);
                        if(enqret==MFALSE)
                        {
                            printf("[%d]===== p2 enque faillllllllllllllllllllllllllllllllll =====\n",_this->m_Sensoridx);
                        }
                        ...
                        char filename[256];
                        sprintf(filename, "/data/IT/IT_loop_%d_input_%dx%d_%lu_magic_0x%x_%d.raw",
                            _loop, enqueParams.mvIn[0].mBuffer->getImgSize().w,\
                            enqueParams.mvIn[0].mBuffer->getImgSize().h, enqueParams.mvIn[0].mBuffer->getBufStridesInBytes(0),
                            curMagic, _this->m_Sensoridx);
                        saveBufToFile(filename, reinterpret_cast<MUINT8*>(enqueParams.mvIn[0].mBuffer->getBufVA(0)),\
                            ...
                            //output
                        for (unsigned int j = 0; j < dequeParams.mvOut.size(); j++)
                        {
                            if(j==0)
                            {
 ...
 //           break;
  ...
  //          break;
   ...
   //         break;
    ...
    for (unsigned int i = 0; i < dualSensor; i++) {
    ...
    for (unsigned int i = 0; i < dualSensor; i++) {
        ...
        LOG_INF("senidx:0x%x:hwport size:0x%lx\n", i, lHwPortCfg[i].size());
    }
    ////////////////////////////////////////////////////////////////////////////
    //configpipe
    ...
    for (unsigned int k=0; k < dualSensor; k++) {
        for(int i=0;i<(Enque_buf+Dummy_buf+replace_buf);i++){
            if(enablePort & __RRZO_ENABLE){
                ...
                LOG_INF("senidx:0x%x img buffer(%d): rrzo pa:0x%" PRIxPTR "\n",
            ...
            if(enablePort & __IMGO_ENABLE){
                ...
                LOG_INF("senidx:0x%x img buffer(%d): imgo pa:0x%" PRIxPTR "\n",
 ...
 //           break;
  ...
  //          break;
   ...
   for (unsigned int i = 0; i < dualSensor; i++) {
       ...
       LOG_INF("senidx:0x%x:hwport size:0x%zx\n", i, lHwPortCfg[i].size());
   }
   ////////////////////////////////////////////////////////////////////////////
   //configpipe
   LOG_INF("start configpipe\n");
