    ...
    MUINT32 callerID)
{
    ...
    if(bufferListTag==EBufferListTag_Batch)
    {
        ...
        LOG_DBG("Lsize(%zu)", mvEnqueuedBufBatch.size());
        ...
        LOG_DBG("updateBNList(%zu)", mvEnqueueBufNode.size());
        ...
        LOG_DBG("updateBList(%zu)", mvEnqueuedBufBatch.size());
        ...
        if( bufNode.portEn & (1 << i))
        {
            ...
            switch(deQret)
            {
                case eIspRetStatus_Failed:
                    ...
                    {
                        LOG_INF("Fst mvOut mva(0x%" PRIxPTR ")",
                            mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvOut[0].mBuffer->getBufPA(0));
                    }
                    else
                    {
...
//    LOG_INF("QQ id : (%x)  callback (%x)",bufNode.idxInBufBatchList,mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mpfnCallback);
    ...
    if(retFrame==true)
    {
        ...
        if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvOut.size()>0)
        {
            LOG_INF("FmvOut(0x%" PRIxPTR "),dCq(%d/%d)",
                mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvOut[0].mBuffer->getBufPA(0),
        ...
        {    //slow motion support,//all buffer nodes in the batch are deque done
            if ((unsigned int)(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].dequedNum) == mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvMagicNo.size())
            {dequeSuccess=true;}
        }
