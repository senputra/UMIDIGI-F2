    ...
    if (mpSensorProvider != NULL)
    {
        ...
        for( MUINT32 i = 0; i < sensorData.size(); i++ )
        {
            ...
            int index = (sensorData.front().timestamp < sensorData.back().timestamp) ?
                         i : sensorData.size() - (i + 1);

            if(g_debugDump >= 2)
            {
                EIS_LOG("Acc(%f,%f,%f,%" PRId64 ")",
                        sensorData[index].acceleration[0],
                        sensorData[index].acceleration[1],
                        sensorData[index].acceleration[2],
                        sensorData[index].timestamp);
            }
            gAccInfo[0] = sensorData[index].acceleration[0];
            gAccInfo[1] = sensorData[index].acceleration[1];
            gAccInfo[2] = sensorData[index].acceleration[2];
            ...
            gGyroInfo[0] = sensorData[index].gyro[0];
            if( UNLIKELY(gGyroReverse == 1) )
            {
                gGyroInfo[1] = -sensorData[index].gyro[1];
                gGyroInfo[2] = -sensorData[index].gyro[2];
            }
            else
            {
                gGyroInfo[1] = sensorData[index].gyro[1];
...
#define UNREASONABLE_GYRO_VALUE (10.0f)
            ...
            else
            {
                EIS_ERR("Unreasonable gyro data(%f,%f,%f,%" PRId64 ")",
                    sensorData[index].gyro[0], sensorData[index].gyro[1], sensorData[index].gyro[2], sensorData[index].timestamp);
                gLastGyroTimestamp = sensorData[index].timestamp;
                gWaitGyroCond.signal();
            }
        }
    }
