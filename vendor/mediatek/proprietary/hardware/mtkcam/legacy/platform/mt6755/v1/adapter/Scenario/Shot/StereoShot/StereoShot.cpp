...
onCmd_capture()
{
    ...
    if(mu4ShotMode == NSCam::eShotMode_ZsdShot)
    {
      ...
      // mpCapBufMgr->dequeBuf(pImgBuf_Full_0);
        mpCapBufMgr->dequeBuf(pImgBuf_Prv_0, MFALSE);
        mpCapBufMgr_Main2->dequeBuf(pImgBuf_Full_1);
        ...
        // MY_LOGD("ZSD_TS final pImgBuf_Full_1=%d", pImgBuf_Full_1->getTimestamp()/1000000);
        ...
        mpCapBufMgr_Main2->enqueBuf(pImgBuf_Full_1);
...
handleJpsData(IImageBuffer* pJpeg, IDbgInfoContainer* pDbg_0, IDbgInfoContainer* pDbg_1)
{
    MUINT8* puJpegBuf = (MUINT8*)pJpeg->getBufVA(0);
    MUINT32 u4JpegSize = pJpeg->getBitstreamSize();
    DBG_DATA_STEREO_T dbgData;
    StereoHalBase* pStereoHal = StereoHalBase::createInstance();
    pStereoHal->STEREOGetDebugInfo(dbgData);
    MUINT8* puDbgBuf    = dbgData.dbgDataAddr;
    MUINT32 u4DbgSize   = dbgData.dbgDataSize;
    ...
    MY_LOGD("(dbg, size, exifHeaderBuf, size) = (%p, %d, %p, %d)", puDbgBuf, u4DbgSize, puExifHeaderBuf, u4ExifHeaderSize);
    ...
    mpShotCallback->onCB_CompressedImage(0,
                                         u4DbgSize,
                                         reinterpret_cast<uint8_t const*>(puDbgBuf),
                                         0,                         //header size
                                         0,                         //header buf
                                         0,                         //callback index
                                         false,                     //final image
