...
MINT32 _convertResultToMeta(MUINT32 u4SensorDev, const Result_T& rResult, const IMetadata& staticMeta, MetaSet_T& rMetaResult, const StaticInfo_T& rstaticInfo)
{
    ...
    {
        MY_LOG_IF(fgLog, "[%s] RollingShutterSkew(%lld)", __FUNCTION__,(long long)rResult.i8SensorRollingShutterSkew);
        MY_LOG_IF(fgLogPf, "[%s] ExpTime(%lld) FrmDuration(%lld) ISO(%d) RSS(%lld)", __FUNCTION__,
        (long long)rResult.i8SensorExposureTime, (long long)rResult.i8SensorFrameDuration, rResult.i4SensorSensitivity, (long long)rResult.i8SensorRollingShutterSkew);
    }
    else
    {
        MY_LOG_IF(fgLogPf, "[%s] ExpTime(%lld) FrmDuration(%lld) ISO(%d)", __FUNCTION__,
        (long long)rResult.i8SensorExposureTime, (long long)rResult.i8SensorFrameDuration, rResult.i4SensorSensitivity);
    }
    ...
    // log of control
    MINT32 rRgn[5] = {0}; // add initial value for Coverity
    MUINT8 u1Mode;
    ...
    double noise_profile[8] = {0}; // add initial value for Coverity
    ...
    for (i = 0; i < src.count(); i++)
    {
        ...
        case MTK_SENSOR_INFO_MAX_FRAME_DURATION:
            ...
            for (j = 0; j < entry.count(); j++)
            {
                ...
                MY_LOG_IF(fgLogPf, "[%s] val(%lld)", __FUNCTION__, (long long)val);
            }
            break;
...
#define OVERRIDE_AF  (1<<2)
    ...
    MY_LOG_IF(fgLog, "[%s] mi4SensorIdx(%d), mu4Counter(%d), queueSize(%d), queueSize4MW(%d)", __FUNCTION__, mi4SensorIdx, mu4Counter++, i4RequestSize, i4RequestQSize);
    if (i4RequestSize != getCapacity() && (mParams.u4StrobeMode != MTK_FLASH_MODE_TORCH))
    ...
    if (i4RequestSize > HAL3A_REQ_PROC_KEY)
    {
    ...
    {
        MY_LOG_IF(fgLog,"i4RequestSize(%d) should be greater than HAL3A_REQ_PROC_KEY(%d)", i4RequestSize, HAL3A_REQ_PROC_KEY);
    }
