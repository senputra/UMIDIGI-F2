...
{
    P1HWRW_DBG("handle=0x%" PRIxPTR ", Caller=%s", handle, szCallerName);
    ...
    IspDrv* pTarVirDrv)//Dst
{
    ...
    {
        //error parameter
        P1HWRW_ERR("_flushHwCfg2CQ: (0 == accessObj->m_iSize(%d)) || ((accessObj->m_iSize*4)(0x%x) > sizeof(isp_reg_t))(0x%zx)",
            accessObj->m_iSize, (accessObj->m_iSize*4), sizeof(isp_reg_t));
        return MFALSE;
    }
...
HwRWCtrl::dbgDump(MINTPTR handle, MUINT32 tgIdx)
{
    ...
    irq_TG_DONE.UserName= (char*)"HwRWCtrl";
    if (2 == tgIdx) {
...
{
    P1HWRW_DBG("~ObjAccessHW, this=%p, sz=%zu", this, sizeof(ObjAccessHW));
}
...
ObjAccessHW::initModuleRegVal(void)
{
    ...
    if (HWCFG_STATUS_NONE == getCfgStatus()) {
        ...
        P1HWRW_DBG("m_pVirtIspReg: %p, m_iOffset=0x%x", &m_pHwRWCtrl->m_pVirtIspReg, m_iOffset);
