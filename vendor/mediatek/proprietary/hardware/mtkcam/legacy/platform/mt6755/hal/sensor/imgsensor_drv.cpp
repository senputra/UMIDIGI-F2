...
)
{
    ...
    LOG_WRN("sizeof(MUINT16):%zu,sizeof(MUINT32):%zu, sizeof(UCHAR):%zu, sizeof(MUINT64):%zu"
            "sizeof(MUINTPTR):%zu, sizeof(MUINTPTR *):%zu, sizeof(MUINT32 *):%zu, sizeof(UCHAR *):%zu\n",
            sizeof(MUINT16), sizeof(MUINT32), sizeof(UCHAR), sizeof(MUINT64),
            sizeof(MUINTPTR), sizeof(MUINTPTR *), sizeof(MUINT32 *), sizeof(UCHAR *));
...
//#define SENDCMD_LOG // Open sendcommand log, for test only
    ...
    case CMD_SENSOR_SET_SENSOR_EXP_TIME:
        ...
        if(SENSOR_MAIN == sensorDevId) {
            ...
            if(FeaturePara[0] == 0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_EXP_TIME] m_LineTime = 0 %d\n", m_LineTimeInus[0]);
                FeaturePara[0] = 1;
            }
    ...
    case CMD_SENSOR_SET_AE_EXPOSURE_GAIN_SYNC:
        ...
        if(SENSOR_MAIN == sensorDevId) {
            ...
            if(FeaturePara[2]  == 0) {   // avoid the line number to zero
                LOG_MSG("[CMD_SENSOR_SET_SENSOR_SYNC] m_LineTime[0] = 0 %d\n", m_LineTimeInus[0]);
                FeaturePara[2]  = 1;
            }
            ...
            if(FeaturePara[2]  == 0) {   // avoid the line number to zero
               LOG_MSG("[CMD_SENSOR_SET_SENSOR_SYNC] m_LineTime[1] = 0 %d\n", m_LineTimeInus[1]);
               FeaturePara[2]  = 1;
            }
            FeaturePara[2] = (FeaturePara[2] ) | (((*(parg1+3))>>4) << 16); // Sensor gain from 10b to 6b base
            FeaturePara[3] = *(parg1+4);  // Delay frame cnt
    ...
    case CMD_SENSOR_SET_ESHUTTER_GAIN:
        ...
        if(FeaturePara[0] == 0) {   // avoid the line number to zero
            LOG_MSG("[CMD_SENSOR_SET_ESHUTTER_GAIN] m_LineTime = 0 %d\n", m_LineTimeInus[0]);
            FeaturePara[0]  = 1;
        }
    ...
    case CMD_SENSOR_GET_SENSOR_VC_INFO:
      ...
      case CMD_SENSOR_GET_SENSOR_PDAF_INFO:
        ...
        pFeaturePara = (MUINT8*)FeaturePara;
#ifdef __LP64__
        LOG_ERR("[SENSOR_FEATURE_GET_PDAF_INFO]0x%lx 0x%lx\n", FeaturePara[0], FeaturePara[1]);
#else
        LOG_ERR("[SENSOR_FEATURE_GET_PDAF_INFO]0x%llx 0x%llx\n", FeaturePara[0], FeaturePara[1]);
#endif
     ...
     case CMD_SENSOR_GET_PDAF_DATA:
        ...
        pFeaturePara = (MUINT8*)FeaturePara;
#ifdef __LP64__
        LOG_MSG("[CMD_SENSOR_GET_PDAF_DATA] 22 0x%lx 0x%lx 0x%lx", FeaturePara[0], FeaturePara[1], FeaturePara[2]);
#else
        LOG_MSG("[CMD_SENSOR_GET_PDAF_DATA] 22 0x%llx 0x%llx 0x%llx", FeaturePara[0], FeaturePara[1], FeaturePara[2]);
#endif
        break;
