...
)
{
    ...
    //
    sp<CamMsgCbInfo> camMsgCbInfo = NULL;
    MBOOL afMoveCb = MFALSE;

    {
        Mutex::Autolock lock(mLock);
        camMsgCbInfo = mspCamMsgCbInfo;
        afMoveCb = mAFMoveCb;
    }
    //
    if(camMsgCbInfo == 0)
    ...
    if( _msgType == I3ACallBack::eID_NOTIFY_AF_FOCUSED &&
        (camMsgCbInfo->mMsgEnabled & (CAMERA_MSG_FOCUS)))
    {
        camMsgCbInfo->mNotifyCb(CAMERA_MSG_FOCUS, _ext1, _ext2, camMsgCbInfo->mCbCookie);
    }
    //
    if( _msgType == I3ACallBack::eID_NOTIFY_AF_MOVING &&
        (camMsgCbInfo->mMsgEnabled & CAMERA_MSG_FOCUS_MOVE) &&
        afMoveCb)
    {
        camMsgCbInfo->mNotifyCb(CAMERA_MSG_FOCUS_MOVE, _ext1, _ext2, camMsgCbInfo->mCbCookie);
    }
    //
    if( _msgType == I3ACallBack::eID_NOTIFY_STEREO_WARNING &&
        (camMsgCbInfo->mMsgEnabled & MTK_CAMERA_MSG_EXT_NOTIFY_STEREO_WARNING) )
    {
        camMsgCbInfo->mNotifyCb(
                    MTK_CAMERA_MSG_EXT_NOTIFY,
                    MTK_CAMERA_MSG_EXT_NOTIFY_STEREO_WARNING,
                    _ext1,
                    camMsgCbInfo->mCbCookie
                    );
    }
    //
    if( _msgType == I3ACallBack::eID_NOTIFY_STEREO_DISTANCE &&
        (camMsgCbInfo->mMsgEnabled & MTK_CAMERA_MSG_EXT_NOTIFY_STEREO_DISTANCE) )
    {
        camMsgCbInfo->mNotifyCb(
                    MTK_CAMERA_MSG_EXT_NOTIFY,
...
)
{
    ...
    {
        camera_memory* mem = camMsgCbInfo->mRequestMemory(-1, _size + sizeof(int32_t), 1, mspCamMsgCbInfo->mCbCookie);
        if  ( mem )
        {
            if (mem->data && mem->size >= _size)
            {
                ...
                camMsgCbInfo->mDataCb(MTK_CAMERA_MSG_EXT_DATA, mem, 0, NULL, camMsgCbInfo->mCbCookie);
                mem->release(mem);
            }
        }
    }
