...
MINT32 u4PortID[DMA_PORT_TYPE_NUM][DMA_OUT_PORT_NUM] = {
    /*0*/{(MINT32)EPortIndex_IMG2O, (MINT32)EPortIndex_IMG3O, (MINT32)EPortIndex_WDMAO, (MINT32)EPortIndex_WROTO, (MINT32)EPortIndex_JPEGO, (MINT32)EPortIndex_FEO, (MINT32)EPortIndex_VIPI},
...
//    ESoftwareScenario scenTmp = swScen;
    ...
    PIPE_DBG("cropPath Array Size = (%zu), Scen_Map_CropPathInfo_STRUCT Size =(%zu), scenTmp:(%d)",
        sizeof(mCropPathInfo), sizeof(Scen_Map_CropPathInfo_STRUCT), drvScen);
...
queryScenarioInfo(EDrvScenario drvScen, vector<CropPathInfo>& vCropPaths)
{
    ...
    PIPE_DBG("cropPath Array Size = (%zu), Scen_Map_CropPathInfo_STRUCT Size =(%zu), drvScen:(%d)",
        sizeof(mCropPathInfo), sizeof(Scen_Map_CropPathInfo_STRUCT), drvScen);
...
enqueInBuf(PortID const portID, QBufInfo const& rQBufInfo,MINT32 const enqueCq,MINT32 const dupCqIdx)
{
    ...
    PIPE_DBG("QBufInfo:(user, reserved, num)=(0x%x, %d, %zu)", rQBufInfo.u4User, rQBufInfo.u4Reserved, rQBufInfo.vBufInfo.size());
...
enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo,MINT32 const enqueCq,MINT32 const dupCqIdx)
{
    ...
    PIPE_DBG("[%d]: (0x%08x,0x%" PRIxPTR ",0x%" PRIxPTR ")/(0x%08x,0x%" PRIxPTR ",0x%" PRIxPTR ")/(0x%08x,0x%" PRIxPTR ",0x%" PRIxPTR ")",
                                        portID.index,
...
dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo, EDrvScenario eDrvSce, MINT32 const dequeCq,MINT32 const dequeBurstQIdx, MINT32 const dequeDupCqIdx, MUINT32 const u4TimeoutMs /*= 0xFFFFFFFF*/)
{
    ...
    for ( MINT32 i = 0; i < (MINT32)rQBufInfo.vBufInfo.size() ; i++) {
        ...
        PIPE_DBG("i(%d),va(0x%" PRIxPTR "/0x%" PRIxPTR "/0x%" PRIxPTR "),pa(0x%" PRIxPTR "/0x%" PRIxPTR "/0x%" PRIxPTR "),size(0x%08x/0x%08x/0x%08x)",
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    //in/out dma check
    PIPE_DBG("in[%zu]/out[%zu]", vInPorts.size(), vOutPorts.size());
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ )
    {
        if ( 0 == vInPorts[i] ) { continue; }
        //
        PIPE_INF("vInPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),Size(0x%08x,0x%08x,0x%08x),VA(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR "),PA(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR "),memID(0x%x,0x%x,0x%x),magicNum(0x%x)",
                                                        i,
                                                        vInPorts[i]->eImgFmt,
        ...
        switch(vInPorts[i]->index)
        {
            ...
            case EPortIndex_UFDI:
                ...
                if ((meScenarioID!=eDrvScenario_CC_vFB_FB) &&
                    (meScenarioID!=eDrvScenario_CC_MFB_Mixing) &&
                    (meScenarioID!=eDrvScenario_VSS_MFB_Mixing))
                {
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ )
    {
        ...
        PIPE_INF("vOutPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),Size(0x%08x,0x%08x,0x%08x),VA(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR "),PA(0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR "),memID(0x%x,0x%x,0x%x)",
                                                        i,
                                                        vOutPorts[i]->eImgFmt,
                ...
                if (reinterpret_cast<Nr3DCfg*>(pPipePackageInfo->vModuleParams[i].moduleStruct))
                {
                    ...
                    if (idx_vipi != -1)
                    {
                        ...
                        //nr3d_cfg.vipi_offst is in byte, xoffset/yoffset are in pixel for tpipe
                          ...
                          PIPE_DBG("VIPI starting PA(0x%" PRIxPTR "/0x%" PRIxPTR "/0x%" PRIxPTR ")/VIP2I(0x%" PRIxPTR "/0x%" PRIxPTR "/0x%" PRIxPTR ")/VIP3I(0x%" PRIxPTR "/0x%" PRIxPTR "/0x%" PRIxPTR ")",
        ...
        if ((stISP_BUF_INFO*)(pPipePackageInfo->pPrivateData))
        {
            ...
            if ((((stISP_BUF_INFO*)(pPipePackageInfo->pPrivateData))->dma)==ISP_DMA_RRZO || (((stISP_BUF_INFO*)(pPipePackageInfo->pPrivateData))->dma)==ISP_DMA_RRZO_D)
            {
                ...
                if ((((stISP_BUF_INFO*)(pPipePackageInfo->pPrivateData))->rrz_info.dstH)) {
                    ...
                    PIPE_ERR("[Error]wrong dstH(%d),pCookie(%p),rrz_info(%p)", (((stISP_BUF_INFO*)(pPipePackageInfo->pPrivateData))->rrz_info.dstH), (pPipePackageInfo->pPrivateData), &(((stISP_BUF_INFO*)(pPipePackageInfo->pPrivateData))->rrz_info));
                }
                PIPE_DBG("[rrz]pCookie(%p),src(%d,%d,%d,%d),dst(%d,%d)",(pPipePackageInfo->pPrivateData),
...
//                en_p2_dma |=  CAM_CTL_DMA_EN_VIPI_EN;
    ...
    if (pPipePackageInfo->isV3 == true){
        if (pPipePackageInfo->pTuningQue != NULL){
            ...
            if (en_p1_dma & CAM_CTL_EN_P1_DMA_LSCI_EN){
                ...
                PIPE_DBG("[LSCI](x,y)=(%lu,%lu),s(%lu),pa(0x%" PRIxPTR ")",
                    m_camPass2Param.lsci.size.w, m_camPass2Param.lsci.size.h,
                    m_camPass2Param.lsci.size.stride,
            ...
            //
            PIPE_DBG("[V3]pIspPhyReg(%p)", pIspPhyReg);
        ...
        if (pPipePackageInfo->mP1PrivateData.ResizeDstH) {
            ...
            PIPE_ERR("[Error]wrong dstH(%d),hrz_info(%f)", pPipePackageInfo->mP1PrivateData.ResizeDstH, rrzRatio);
        }
    ...
    if ((pPipePackageInfo->isV3==false) && FORCE_EN_DIFFERENR_VIEW_TEST)
    {
    ...
    PIPE_DBG("Pass(%d),imgi.fmt(0x%x),meScenarioID(%d),isCropG1En(%d),m_CrzEn(%d),mdpSrcFmt(0x%x),mdp_src(%lu,%lu,%lu,%lu),en_p2_dma(0x%08x),crop1TableIdx(%d),mdpCheck1(%d),mdpCheck2(%d),drvScenario(%d),tcm_en(0x%x)",\
    ...
    if (-1 != idx_vipi ) {
        ...
        PIPE_DBG("[vipi]size(0x%08x),Va(0x%" PRIxPTR "),Pa(0x%" PRIxPTR "),offset(0x%08x),align(0x%08x),stride(0x%lx)",
        ...
        {    //multi-plane input image, vipi/vip2i/vip3i
            ...
            PIPE_DBG("[vip2i]size[%lu,%lu](0x%08x),offset(0x%08x),align(0x%08x),stride(0x%lx)", m_camPass2Param.vip2i.size.w, m_camPass2Param.vip2i.size.h,
            ...
            if (-1 != idx_vip3i )
            {
                ...
                PIPE_DBG("[vip3i]size[%lu,%lu](0x%08x),Va(0x%" PRIxPTR "),Pa(0x%" PRIxPTR "),offset(0x%08x),align(0x%08x),stride(0x%lx),vipi_uv_swap(%d)",
                        m_camPass2Param.vip3i.size.w, m_camPass2Param.vip3i.size.h,
                        m_camPass2Param.vip3i.memBuf.size,
                        m_camPass2Param.vip3i.memBuf.base_vAddr, m_camPass2Param.vip3i.memBuf.base_pAddr,
                        m_camPass2Param.vip3i.memBuf.ofst_addr, m_camPass2Param.vip3i.memBuf.alignment,
                        m_camPass2Param.vip3i.size.stride, vipi_uv_swap);
            }
        }
        else
        {
    ...
    if (-1 != idx_imgo ) {  // only support frame mode for dma imgo in ip scenario
        PIPE_DBG("isSetMdpOut(%d),u4BufSize(0x%x)-u4BufVA(0x%" PRIxPTR ")-u4BufPA(0x%" PRIxPTR ")",
                isSetMdpOut, vOutPorts[idx_imgo]->u4BufSize[0],
                vOutPorts[idx_imgo]->u4BufVA[0],
    ...
    if (-1 != idx_img2o ) {
        ...
        PIPE_DBG("u4BufSize(0x%x)-u4BufVA(0x%" PRIxPTR ")-u4BufPA(0x%" PRIxPTR ")",
                    vOutPorts[idx_img2o]->u4BufSize[0],
                    vOutPorts[idx_img2o]->u4BufVA[0],
    ...
    if (-1 != idx_img3o ) {
        ...
        PIPE_DBG("[img3o]size(0x%08x),Va(0x%" PRIxPTR "),Pa(0x%" PRIxPTR "),offset(0x%08x),align(0x%08x),stride(%lu)",
                m_camPass2Param.img3o.memBuf.size,
                m_camPass2Param.img3o.memBuf.base_vAddr, m_camPass2Param.img3o.memBuf.base_pAddr,
                m_camPass2Param.img3o.memBuf.ofst_addr,
                m_camPass2Param.img3o.memBuf.alignment, m_camPass2Param.img3o.size.stride);
        ...
        if (-1 != idx_img3bo )
        {
            ...
            PIPE_DBG("[img3bo]size[%lu,%lu](0x%08x),offset(0x%08x),align(0x%08x),stride(%lu)", m_camPass2Param.img3bo.size.w, m_camPass2Param.img3bo.size.h,
            ...
            if (-1 != idx_img3co )
            {
                ...
                PIPE_DBG("[img3co]size[%lu,%lu](0x%08x),Va(0x%" PRIxPTR "),Pa(0x%" PRIxPTR "),offset(0x%08x),align(0x%08x),stride(%lu),img3o_uv_swap(%d)",
                        m_camPass2Param.img3co.size.w, m_camPass2Param.img3co.size.h,
                        m_camPass2Param.img3co.memBuf.size,
                        m_camPass2Param.img3co.memBuf.base_vAddr, m_camPass2Param.img3co.memBuf.base_pAddr,
                        m_camPass2Param.img3co.memBuf.ofst_addr,
                        m_camPass2Param.img3co.memBuf.alignment,
                        m_camPass2Param.img3co.size.stride,
                        img3o_uv_swap);
            }
        }
    }
