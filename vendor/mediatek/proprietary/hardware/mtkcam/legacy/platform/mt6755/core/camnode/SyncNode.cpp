...
#define SYNC_THRESHOLD 33000000
...
stopPreview()
{
    ...
    if(isZSDMode){
        while(cap_buf_len == 0 || needWait){
            ...
            if(mQueueLock.tryLock() != 0){
                MY_LOGD("mQueueLock already locked");
            }
        }
    }
...
pushBuf(MUINT32 const data, IImageBuffer* const buf, MUINT32 const ext)
{
    ...
    if(isTimeSync()){
        ...
        if(data == SYNC_SRC_0 || data == SYNC_SRC_2){
...
setZSDMode(MBOOL _isZSD, CapBufMgr* _pCapBufMgr, CapBufMgr* _pCapBufMgr_main2, MINT32 _maxCapBufLen){
    ...
    mbEnable_CapBuf     = MTRUE;
    MY_LOGD("v1 setZSDMode isZSDMode=%d, mpCapBufMgr=%p, mpCapBufMgr_main2=%p, maxCapBufLen=%d, mbEnable_CapBuf=%d",
        isZSDMode,
        mpCapBufMgr,
        mpCapBufMgr_main2,
        maxCapBufLen,
