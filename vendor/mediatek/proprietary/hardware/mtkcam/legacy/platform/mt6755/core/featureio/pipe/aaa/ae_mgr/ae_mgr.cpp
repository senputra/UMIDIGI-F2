...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL AeMgr::updateAEScenarioMode(EIspProfile_T /*eIspProfile*/)  /* fix build warning */
{
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::doPvAEmonitor(MINT32 /*i4FrameCount*/, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule, MBOOL &bWillUpdateSensorbyI2C)   /* fix build warning */
{
    mVHDRInputParam_T rInput;
    mVHDROutputParam_T rOutput;
    MBOOL bCalculateAE = MFALSE;
    MBOOL bApplyAE = MFALSE;
    //MINT32 i4ActiveItem;  /* fix build warning */
...
MRESULT AeMgr::doPvAE(MINT32 /*i4FrameCount*/, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 /*u4AAOUpdate*/, MBOOL bAAASchedule)    /* fix build warning */
{
strAEInput rAEInput;
strAEOutput rAEOutput;
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::doAFAEmonitor(MINT32 /*i4FrameCount*/, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule, MBOOL &bWillUpdateSensorbyI2C)    /* fix build warning */
{
    MBOOL bCalculateAE = MFALSE;
    MBOOL bApplyAE = MFALSE;
    MINT32 i4ActiveItem;
...
MRESULT AeMgr::doAFAE(MINT32 i4FrameCount, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 /*u4AAOUpdate*/, MBOOL bAAASchedule)    /* fix build warning */
{
strAEInput rAEInput;
strAEOutput rAEOutput;
//AE_INFO_T rAEInfo2ISP;  /* fix build warning */
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::doPreCapAE(MINT32 /*i4FrameCount*/, MBOOL bIsStrobeFired, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule)  /* fix build warning */
{
strAEInput rAEInput;
strAEOutput rAEOutput;
//AE_INFO_T rAEInfo2ISP;  /* fix build warning */
...
//static int g_rExpOn2;  /* fix build warning */
//static int g_rAfeOn2;  /* fix build warning */
//static int g_rIspOn2;  /* fix build warning */

static int g_rIspAFLampOff;
//static int g_rFrm;  /* fix build warning */
//static AE_MODE_CFG_T g_lastAFInfo;  /* fix build warning */

int mapAFLampOffIsp2(int rollFrmTime, int exp, int afe, int isp, int expOn, int afeOn, int ispOn, int* expOn2=0, int* afeOn2=0)  /* fix build warning */
{
 ...
 //int rollFrameTime;  /* fix build warning */

 //int exp2;  /* fix build warning */
 //int afe2;  /* fix build warning */
 //int isp2;  /* fix build warning */
...
AE_EXP_GAIN_MODIFY_T rSensorInputData, rSensorOutputData;
    ...
    if((m_rAEPLineLimitation.bEnable == MTRUE) && (m_bAEReadyCapture == MFALSE)){
        if(m_rAEPLineLimitation.bEquivalent == MTRUE) {
            ...
            MY_LOG("[%s()] i4SensorDev:%d No rquivalent, Please use AE Pline table:%d ISO:%d Shutter:%d\n", __FUNCTION__, m_eSensorDev, m_rAEPLineLimitation.bEquivalent, m_rAEPLineLimitation.u4IncreaseISO_x100, m_rAEPLineLimitation.u4IncreaseShutter_x100);
        }
    }
...
MRESULT AeMgr::getNvramData(MINT32 i4SensorDev, MINT32 isForce)
{
    ...
    //void * pCurrentTable;  /* fix build warning*/
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::AEInit(Param_T & /*rParam*/)  /* fix build warning */
{
    strAEOutput rAEOutput;
    MINT32 i, j;
...
MRESULT AeMgr::setAERotateDegree(MINT32 i4RotateDegree)
{
    ...
    MY_LOG("[%s()] m_i4SensorDev:%d old:%d\n", __FUNCTION__, m_eSensorDev, i4RotateDegree);
...
MRESULT AeMgr::updateCaptureShutterValue()
{
    if((m_bEnableAE == MTRUE) && (m_bMultiCap == MFALSE) && (mCaptureMode.u4Eposuretime > m_rAEInitInput.rAEPARAM.strAEParasetting.u4LongCaptureThres) && (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableLongCaptureCtrl == MTRUE)&&(m_bRealISOSpeed != MTRUE)) {
        m_u4VsyncCnt--;
        if(m_u4VsyncCnt == (MUINT32)(m_i4TotalCaptureDelayFrame-1)) {
...
MRESULT AeMgr::monitorAndReschedule( MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply )
{
    ...
    //Skip after AEexit
    if (bAEExit == MTRUE){
     (m_u4AEExitStableCnt < (MUINT32)(i4framePerAECycle*NumCycleAESkipAfterExit))?(bAESkip = MFALSE):(bAESkip = MTRUE);
      m_u4AEExitStableCnt++;
    }else{
      m_u4AEExitStableCnt=0;
    }
