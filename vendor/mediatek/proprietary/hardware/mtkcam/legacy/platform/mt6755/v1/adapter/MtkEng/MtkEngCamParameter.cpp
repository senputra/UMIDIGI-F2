...
setParameters()
{
    ...
    //Check EV mode change
    {
        int32_t newParam = 0;
        newParam = mpParamsMgr->getInt(CameraParameters::KEY_EXPOSURE_COMPENSATION);
        Param_T oldParamAll;
        int32_t oldParam = cam3aParam.i4ExpIndex;
        if (newParam != oldParam)
        {
            mpEngCtrlNode->updateDelay(EQueryType_Ev);
            MY_LOGD("EV mode changed (%d) --> (%d)", oldParam, newParam);
        }
    }
    //check Saturation mode change
    {
        int32_t newParam = 0;
        UPDATE_PARAMS(newParam, eMapLevel, MtkCameraParameters::KEY_SATURATION);
        Param_T oldParamAll;
        int32_t oldParam = cam3aParam.u4SaturationMode;
        if (newParam != oldParam)
        {
            mpEngCtrlNode->updateDelay(EQueryType_Sat);
            MY_LOGD("SATURATION mode changed (%d) --> (%d)", oldParam, newParam);
        }
    }
    //check Brightness mode change
    {
        int32_t newParam = 0;
        UPDATE_PARAMS(newParam, eMapLevel, MtkCameraParameters::KEY_BRIGHTNESS);
        Param_T oldParamAll;
        int32_t oldParam = cam3aParam.u4BrightnessMode;
        if (newParam != oldParam)
        {
            mpEngCtrlNode->updateDelay(EQueryType_Bright);
            MY_LOGD("BRIGHTNESS mode changed (%d) --> (%d)", oldParam, newParam);
        }
    }
    //check Contrast mode change
    {
        int32_t newParam = 0;
        UPDATE_PARAMS(newParam, eMapLevel, MtkCameraParameters::KEY_CONTRAST);
        Param_T oldParamAll;
        int32_t oldParam = cam3aParam.u4ContrastMode;
        if (newParam != oldParam)
        {
            mpEngCtrlNode->updateDelay(EQueryType_Contrast);
