    ...
    , mRequestMap_image()
{
    ...
    if ( 0 == mLogLevel ) {
        ::property_get("vendor.debug.camera.log.inflightrequest", cLogLevel, "0");
        mLogLevel = ::atoi(cLogLevel);
    }
}
...
Mutex::Autolock _l(mLock);
    ...
    {
         MY_LOGD_IF(1, "Node: %#" PRIxPTR " has frameNo: %d in the front",
...
{
    MY_LOGD1("frame: %d, nodeId: %#" PRIxPTR ", msg: %d", frameNo, nodeId, message);

    Mutex::Autolock _l(mLock);
    if(mRequestMap_meta.indexOfKey(nodeId) < 0 && mRequestMap_image.indexOfKey(nodeId) < 0) {
        MY_LOGE("no node in meta/image mapper:  %#" PRIxPTR , nodeId);
        return;
    }
