    ...
    ,mFBBufferIndex(0)
{
    ...
    property_get("vendor.camera.acdkovl.disable", value1, "0");
    g_overlayFlag = atoi(value1);

    char value2[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.acdk.debug", value2, "0");
        ...
        MUINT32 const orientation)
{
    ...
    ACDK_LOGD_DYN(g_overlayDebug, "phyAddr  = 0x%" PRIxPTR, phyAddr);
    ACDK_LOGD_DYN(g_overlayDebug, "virtAddr = 0x%" PRIxPTR, virtAddr);
...
#else
    (void)layerNo; (void)pSrcIn; (void)srcFmt; (void)srcW; (void)srcH;
    (void)orientation; (void)hFlip; (void)vFlip;
#endif
    ACDK_LOGD("-");
    return status;
}
...
MINT32 surfaceView::refresh()
{
    ...
    if(g_overlayFlag == 0)
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug,"src VA(%p), PA(%p)", mLayerInfo[0].src_base_addr, mLayerInfo[0].src_phy_addr);
        ...
        ACDK_LOGD("src VA(%p), PA(%p)", mLayerInfo[0].src_base_addr, mLayerInfo[0].src_phy_addr);
        if (ioctl(mFBfd, MTKFB_SET_OVERLAY_LAYER, &mLayerInfo[0]) < 0)
        {
            ACDK_LOGE("MTKFB_SET_OVERLAY_LAYER failed");
            return ACDK_RETURN_API_FAIL;
    ...
    else
    {
        ...
        ACDK_LOGD_DYN(g_overlayDebug, "use mmap");
        ACDK_LOGD_DYN(g_overlayDebug, "src VA(%p), size(%u)",
            mLayerInfo[0].src_base_addr, mLayerInfo[0].src_width * mLayerInfo[0].src_height * 2);
