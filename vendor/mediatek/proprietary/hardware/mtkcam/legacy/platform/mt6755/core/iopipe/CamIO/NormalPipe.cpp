...
INormalPipe::createInstance(MUINT32 *pSensorIndex, MUINT32 sensorCnt, char const* szCallerName, MINT32 burstQnum)
{
    ...
    if (!pAllSensorInfo){
        ...
        CAM_LOGD("[%d]createInstance, %s,Alloc pAllSensorInfo=%p",
            pSensorIndex[0], szCallerName, pAllSensorInfo);
    ...
    if (1 == sensorCnt) {//NPIPE_Sensor_0 ||  NPIPE_Sensor_1
        ...
        if (NULL == pNPipe)  {
            ...
            CAM_LOGD("[%d]createInstance, %s,pNormalPipe[%d]=%p create\n",
 ...
 *
 ******************************************************************************/
unsigned long NormalPipe::mTwinEnable[CAM_SV_2 + 1][2] = {
    {0, ePxlMode_One_Single}, {0, ePxlMode_One_Single}, {0, ePxlMode_One_Single},
    {0, ePxlMode_One_Single}, {0, ePxlMode_One_Single}, {0, ePxlMode_One_Single},
    {0, ePxlMode_One_Single}, {0, ePxlMode_One_Single}, {0, ePxlMode_One_Single}}; //record this & twin mode,0 for this, 1 for twinmode
...
NormalPipe::destroyInstance(char const* szCallerName)
{
    ...
    if (MTRUE == mDumpNPipeFlag)
    {
        CAM_LOGD("(%p)NPipe[%d]->mUserCnt(%d) ", pNormalPipe[idx], idx, pNormalPipe[idx]->mUserCnt);
    }
    ...
    if (pNormalPipe[idx]->mUserCnt == 0)  {
        ...
        CAM_LOGD("Destroy NPipe[%d]=%p",idx, pNormalPipe[idx]);
...
NormalPipe::enque(QBufInfo const& rQBuf)
{
    ...
    for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
    {
        ...
        //
        if ((portID.index == PORT_RRZO.index) ||
            (portID.index == PORT_RRZO_D.index) ||
            (portID.index == PORT_IMGO.index) ||
            (portID.index == PORT_IMGO_D.index)) {
            CAM_LOGD("[MyEnQ:%d]PortId(%d),sz/va/pa/idx(%d,0x%" PRIxPTR ",0x%" PRIxPTR ",0x%x)",
...
NormalPipe::deque(QBufInfo& rQBuf, MUINT32 /*u4TimeoutMs*/)
{
    ...
    if ((!mpCamIOPipe) || (0 == port_cnt)) {
       CAM_LOGE("[deque] Error: !mpCamIOPipe=%p || rQBuf.mvOut.size=%d", mpCamIOPipe, port_cnt);
       return MFALSE;
    }
    if (!mConfigDone) {
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            for (MUINT32 idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++)  {
...
#ifdef __LP64__
                    CAM_LOGD("[MyDeQ:%d]PortId(%d),va=0x%lx,BufHeader=%p,PixelId=%x, MNum=0x%x, rrzDstSz=%d,%d,rrzSrcSz=%d,%d, time:%lu.%d",
#else
                    CAM_LOGD("[MyDeQ:%d]PortId(%d),va=0x%x,BufHeader=%p,PixelId=%x, MNum=0x%x, rrzDstSz=%d,%d,rrzSrcSz=%d,%d, time:%llu.%d",
#endif
                             mpSensorIdx[0], \
                             portID.index,\
                ...
                if ((portID.index == PORT_RRZO.index) ||(portID.index == PORT_RRZO_D.index)) {
                 ...
                 if (NULL == pframe) {
                      CAM_LOGE("[MyDeQ:%d]Fail: PortId(%d), 0==pframe, idx=%d, va=0x%" PRIxPTR,
            ...
            CAM_LOGD("[MyDeQ:%d] ###Fail### dequeSz(%zu)<1", mpSensorIdx[0], rQTSBufInfo.vBufInfo.size());
        }
        //
...
//    pixelCnt2 = _NORMALPIPE_GET_PIX_MODE(S2Idx);
    ...
    /*if (mpEnabledSensorCnt == 1)*/
    {
        ...
        CAM_LOGD("1sensor tg[%d], PixelMode=[%d], ISPMode=[%lu]", tg, pixelCnt, mTwinEnable[tg][1]);
    }
...
NormalPipe::configPipe(QInitParam const& vInPorts)
{
    ...
    if (mpEnabledSensorCnt != vInPorts.mSensorCfg.size()) {
        CAM_LOGE("NPipe::configPipe SensorCfgCnt is not match. (%d,%zu)",
        ...
        for (unsigned int i = 0; i < vInPorts.mPortInfo.size(); i++) {
            ...
            if ( PORT_IMGO == vInPorts.mPortInfo.at(i).mPortID ) {
                ...
                if (CAM_TG_1 == _NORMALPIPE_GET_TG_IDX((mpSensorIdx[0]))){
                    ...
                    CAM_LOGD("configPipe PORT_RRZO, mRrzoOut_size:%d,%d", mRrzoOut_size.w, mRrzoOut_size.h);
                    pOut->index    = vInPorts.mPortInfo.at(i).mPortID.index;
                    m_bEISChkFlg = MTRUE;
                }
...
NormalPipe::configFrame(QFrameParam const& rQParam)
{
    ...
    for ( i = 0 ; i < rQParam.mvOut.size(); i++ ) {
      ...
      //when you want to enable PDAF func, please check-in with Middleware at the same time
            ...
            CAM_LOGD("[3ASync]cfFrm:PortIdx[%d],TG(%d),mNum=%x,Crop:(x,y,fx,fy,w,h)=(%d,%d,%d,%d,%lu,%lu); out:(x,y,stride)=(%lu,%lu,%lu)",\
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    if ( !mConfigDone ) {
        CAM_LOGE("error:sendCommand(0x%x) fail, send before configpipe(%p,0x%x)",
            cmd, this, this->mConfigDone);
        return MFALSE;
    }
    ...
    switch (cmd) {
       ...
       case EPIPECmd_GET_TG_OUT_SIZE:
           ...
           #if 1
            if (2 < arg1) {
                 CAM_LOGE("EPIPECmd_GET_TG_OUT_SIZE Err: Invalid SensorIdx(0x%" PRIxPTR ")!", arg1);
                return MFALSE;
            }
       ...
       case EPIPECmd_GET_HBIN_INFO:
           ...
           if ((2 < arg1) || (0 == arg2)) {
                CAM_LOGE("EPIPECmd_GET_HBIN_INFO Err: Invalid Para arg1(0x%" PRIxPTR "), arg2=0x%" PRIxPTR "!", arg1, arg2);
               return MFALSE;
           }

           tg = _NORMALPIPE_GET_TG_IDX(arg1);

           CAM_LOGD("EPIPECmd_GET_HBIN_INFO SIdx(0x%" PRIxPTR "), BinInfoAddr=0x%" PRIxPTR, arg1, arg2);
           ...
           switch (tg) {
               ...
               case CAM_TG_2:
                   ...
                   break;
               default:
                   CAM_LOGE("EPIPECmd_GET_HBIN_INFO Err: Invalid SensorIdx(0x%" PRIxPTR ")!", arg1);
                   return MFALSE;
           }
           CAM_LOGD("EPIPECmd_GET_HBIN_INFO SIdx(0x%" PRIxPTR "),twin=%d,hbin_en=%d w=0x%x, h=0x%x!",
       ...
       case EPIPECmd_GET_RMX_OUT_SIZE:
            ...
            if (2 < arg1) {
                CAM_LOGE("EPIPECmd_GET_RMX_OUT_SIZE Err: Invalid SensorIdx(0x%" PRIxPTR ")!", arg1);
                return MFALSE;
            }
            tg = _NORMALPIPE_GET_TG_IDX(arg1);
            switch (tg) {
                ...
                default:
                    ret = MFALSE;
                    CAM_LOGE("EPIPECmd_GET_RMX_OUT_SIZE Fail! SIdx=0x%" PRIxPTR ", TG=%d", arg1, tg);
                    break;
            }
       ...
       case EPIPECmd_GET_EIS_INFO:
           ...
           if (0 == arg2) {
                CAM_LOGE("EPIPECmd_GET_EIS_INFO Err: Invalid Para arg1(0x%" PRIxPTR ")", arg1);
               return MFALSE;
           }
    ...
    if (ret != MTRUE) {
        CAM_LOGE("error: sendCommand fail: (cmd,arg1,arg2,arg3)=(0x%08x,0x%" PRIxPTR ",0x%" PRIxPTR ",0x%" PRIxPTR ")", cmd, arg1, arg2, arg3);
    }
...
checkDumpNormalPipe(void)
{
    ...
    property_get("vendor.cam.dumpnpipelog.enable", value, "0");
