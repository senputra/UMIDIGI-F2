...
dequeBuffer(MUINT32 const data, ImgRequest * pImgReq)
{
    ...
    if(isDequeProvider)
    {
        ...
        //
        #if 1
        MY_LOGD("buf(%p),heap(%p),VA(%#" PRIxPTR "/%#" PRIxPTR "/%#" PRIxPTR "),PA(%#" PRIxPTR "/%#" PRIxPTR "/%#" PRIxPTR "),BS(%d=%d+%d+%d),Id(%d),F(0x%08X),S(%dx%d),Str(%d,%d,%d),R(%d),U(%d)",
                pImgReq->mBuffer,
...
enqueBuffer(MUINT32 const data, IImageBuffer const * pImageBuffer)
{
    ...
    if(keepAddr == enqueAddr)
    {
        ...
        else
        {
            ...
            sprintf(debugMsg, "buf(%p),VA(%#" PRIxPTR "),BS(%d),Id(%d),TS(%d.%06d),F(0x%08X),S(%dx%d),Str(%d,%d,%d)",
            pImageBuffer,
            GET_IIMAGEBUFFER_BUF_VA(pImageBuffer, 0),
            (   GET_IIMAGEBUFFER_BUF_SIZE(pImageBuffer, 0)+
                GET_IIMAGEBUFFER_BUF_SIZE(pImageBuffer, 1)+
