...
#define DUMP_IImageBuffer( type, pbuf, fileExt, sid)           \
    do{                                                        \
        ...
        char filename[256];                                    \
        sprintf(filename, "%s%s_%d_%04dx%4d_%04zu_%02d_%d.%s", \
                dumppath,                                      \
                type,                                         \
                0,                                \
    ...
    while(1){
        ...
        if( bRet == MTRUE){
            for (unsigned int i = 0; i < dequeBufInfo.mvOut.size(); i++) {
                ...
                printf("[%d_E_L deque]: dma:0x%x, PA:0x%" PRIxPTR ", crop:0x%x,0x%x,0x%x,0x%x_0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, dummy:0x%x, bufidx:0x%x\n",
                    _this->m_sensoridx,
            ...
            if(CAM_V1 == ISPversion){//no replace buf
                ...
                for (unsigned int i = 0; i < dequeBufInfo.mvOut.size(); i++) {
                    ...
                    //
                    printf("[%d_E_L enque]: dma:0x%x, PA:0x%" PRIxPTR ", crop:0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, bufidx:0x%x\n",
                        _this->m_sensoridx,
                        dequeBufInfo.mvOut.at(i).mPortID.index,dequeBufInfo.mvOut.at(i).mBuffer->getBufPA(0),
            ...
            else
            {
                ...
                for (unsigned int i = 0; i < _replace.mvOut.size(); i++) {
                    ...
                    printf("[%d_E_L enque]: dma:0x%x, PA:0x%" PRIxPTR ", crop:0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, bufidx:0x%x, sofidx:0x%x\n",
                        _this->m_sensoridx,
...
int test_camio(int argc, char** argv)
{
    ...
    for (unsigned int i = sensorIdx; i < dualSensor; i++) {
    ...
    for (unsigned int i = sensorIdx; i < dualSensor; i++) {
        ...
        if(enablePort & __CAMSV_ENABLE){
            ...
            if(_cropsize.w != (int)(queryRst.x_pix))
        ...
        printf("senidx:0x%x:hwport size:0x%lx\n", i, lHwPortCfg[i].size());
    }
    ////////////////////////////////////////////////////////////////////////////
    //configpipe
    ...
    for (unsigned int k = sensorIdx; k < dualSensor; k++) {
        for(int i=0;i<(Enque_buf+Dummy_buf+replace_buf);i++){
            if(enablePort & __RRZO_ENABLE){
                ...
                printf("senidx:0x%x img buffer(%d): rrzo pa:0x%" PRIxPTR "\n",
            ...
            if(enablePort & __IMGO_ENABLE){
                ...
                printf("senidx:0x%x img buffer(%d): imgo pa:0x%" PRIxPTR "\n",
