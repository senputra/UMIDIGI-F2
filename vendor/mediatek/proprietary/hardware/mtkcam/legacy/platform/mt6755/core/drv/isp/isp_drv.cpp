...
ISP_DRV_CQ_MODULE_INFO_STRUCT mIspCQModuleInfo[CAM_MODULE_MAX] = {
            ...
            {CAM_CTL_TDR_DBG_STATUS, 0x4088,  1    }, // for control extension
...
MINT32  IspDrv::getRealCQIndex(MINT32 cqBaseEnum, MINT32 burstQIdx, MINT32 dupCqIdx)
{
    ...
    if (cqBaseEnum > ISP_DRV_CQ03)
    {
        ...
        realcqIdx = (burstQIdx * cqNumInfoControl(ISP_DRV_CQNUMINFO_CTRL_GET_P1DUPCQNUM) * ISP_DRV_P1_PER_CQ_SET_NUM) +
                    (cqBaseEnum + dupCqIdx*ISP_DRV_P1_PER_CQ_SET_NUM);
    }
    else
    {//pass2 cq
...
MBOOL IspDrv::cqAddModule(MINT32 cq, MINT32 burstQIdx, MINT32 dupCqIdx, CAM_MODULE_ENUM moduleId)
{
    int cmd;
    int realCQIdx = getRealCQIndex(cq, burstQIdx, dupCqIdx);
    LOG_DBG("- E. realCQIdx[0x%x], moduleId[%d], burstQIdx[%d], dupCqIdx[%d]", realCQIdx, moduleId, burstQIdx, dupCqIdx);
    //
    Mutex::Autolock lock(cqVirDesLock);
...
MINT32 IspDrvImp::isp_fd_close(MINT32 mIspFd)
{
    ...
    if (mIspFd >= 0)
    {
        ...
        for (unsigned int i = 0; i < (this->m_FdNd.size()); i++) {
            if (it->mIspFd == mIspFd)
            {
                mIonFd = it->mIonFd;
                LOG_INF("mIonFd(%d),at(%d/%zu).\n", mIonFd, i, this->m_FdNd.size());
...
MBOOL IspDrvImp::init(const char* userName)
{
    ...
    if (MFALSE == mIspOpenRetry)// 1st time open success.   // Sometimes GDMA will go this path, too. e.g. File Manager -> Phone Storage -> Photo.
    {
        ...
        LOG_DBG("m_RTBufTblSize(0x%x),m_pRTBufTbl(%p)", m_RTBufTblSize, m_pRTBufTbl);
    ...
    LOG_DBG("[m_pIMemDrv]:%p", m_pIMemDrv);    //Vent@20121107: Fix build warning: format '%x' expects argument of type 'unsigned int', but argument 5 has type 'IMemDrv*' [-Wformat]
    if ( NULL == m_pIMemDrv ) {
    ...
    for (int i = 0; i < ISP_DRV_P2_CQ_NUM; i++) {
        ...
        mTuningQueIdx[i].keepP2PreviewQue.pTuningQue.pTuningQue =
            (MUINT32*)pmem_alloc_sync(mTuningQueIdx[i].keepP2PreviewQue.queSize, &mTuningQueIdx[i].keepP2PreviewQue.queFd);
        ...
        mTuningQueIdx[i].keepP1Que.pTuningQue.pTuningQue =
            (MUINT32*)pmem_alloc_sync(mTuningQueIdx[i].keepP1Que.queSize, &mTuningQueIdx[i].keepP1Que.queFd);
    ...
    for (int i = 0; i < ISP_DRV_P2_CQ_NUM; i++) {
        for (int j = 0; j < ISP_TUNING_QUEUE_NUM; j++) {
            ...
            memset((MUINT8*)mTuningQueInf[i][j].pTuningQue, 0, mTuningQueInf[i][j].queSize);
            LOG_DBG("p2cq(%d),que(%d):pTuningQue(%p)", i, j, mTuningQueInf[i][j].pTuningQue);
        }
        ...
        memset((MUINT8*)mTuningQueIdx[i].keepP1Que.pTuningQue,0,mTuningQueIdx[i].keepP1Que.queSize);
        LOG_INF("p2cq(%d):keepP2PreviewQue(%p),keepP1Que(%p)", i, mTuningQueIdx[i].keepP2PreviewQue.pTuningQue, mTuningQueIdx[i].keepP1Que.pTuningQue);
        //
        mTuningQueIdx[i].isApplyTuning = MFALSE;
        mTuningQueIdx[i].pCurReadP1TuningQue = NULL;
        mTuningQueIdx[i].pCurReadP2TuningQue = NULL;
        ...
        for (int i=0; i < ISP_DRV_P2_CQ_NUM; i++) {
            for (int j = 0; j < ISP_TUNING_QUEUE_NUM; j++) {
                if ((mTuningQueInf[i][j].queFd >= 0) && (mTuningQueInf[i][j].pTuningQue != 0)) {
                    ...
                    LOG_ERR("[Error]free TuningQue error i(%d),j(%d),fd(%d),va(0x%08x)",
                        i, j, mTuningQueInf[i][j].queFd,mTuningQueInf[i][j].pTuningQue);
                }
            }
            ...
            if ((mTuningQueIdx[i].keepP2PreviewQue.queFd >= 0) && (mTuningQueIdx[i].keepP2PreviewQue.pTuningQue != 0)) {
                ...
                LOG_ERR("[Error]free keepP2PreviewQue error i(%d),fd(%d),va(0x%08x)",
                    i, mTuningQueIdx[i].keepP2PreviewQue.queFd, mTuningQueIdx[i].keepP2PreviewQue.pTuningQue);
            }
            // free keepP1Que
        ...
        for (int i = 0; i < ISP_DRV_P2_CQ_NUM; i++) {
            for (int j = 0; j < ISP_TUNING_QUEUE_NUM; j++) {
                if ((mTuningQueInf[i][j].queFd >= 0) && (mTuningQueInf[i][j].pTuningQue != 0)) {
                    ...
                    LOG_ERR("[Error]free TuningQue error i(%d),j(%d),fd(%d),va(%p)",
                        i, j, mTuningQueInf[i][j].queFd, mTuningQueInf[i][j].pTuningQue);
                }
            }
            // free keepP2PreviewQue
            if ((mTuningQueIdx[i].keepP2PreviewQue.queFd >= 0) && (mTuningQueIdx[i].keepP2PreviewQue.pTuningQue != 0)) {
                ...
                LOG_ERR("[Error]free keepP2PreviewQue error i(%d),fd(%d),va(%p)",
                    i, mTuningQueIdx[i].keepP2PreviewQue.queFd, mTuningQueIdx[i].keepP2PreviewQue.pTuningQue);
            }
            // free keepP1Que
            if ((mTuningQueIdx[i].keepP1Que.queFd >= 0) && (mTuningQueIdx[i].keepP1Que.pTuningQue != 0)) {
                ...
                LOG_ERR("[Error]free keepP1Que error i(%d),fd(%d),va(%p)", i, mTuningQueIdx[i].keepP1Que.queFd, mTuningQueIdx[i].keepP1Que.pTuningQue);
            }
...
MBOOL IspDrvImp::cqTableControl(ISP_DRV_CQTABLE_CTRL_ENUM cmd, int /*burstQnum*/)
{
    ...
    switch (cmd)
    {
        case ISP_DRV_CQTABLE_CTRL_ALLOC:
            ...
            mIspVirRegSize = ISP_BASE_RANGE * mTotalCQNum;
            ...
            LOG_VRB("v(0x%x)(0x%x)(0x%" PRIxPTR ")(0x%" PRIxPTR ")(%p)(%p)",
                mIspVirRegSize, m_ispVirRegBufInfo.size,
            ...
            for (int i = 0; i < mTotalCQNum; i++) {
                mpIspVirRegAddrVA[i] = mpIspVirRegBufferBaseAddr + (i * ISP_BASE_RANGE / sizeof(MUINT32));
                mpIspVirRegAddrPA[i] = mpIspVirRegAddrPA[0] + (i * ISP_BASE_RANGE / sizeof(MUINT32));
                LOG_DBG("cq:[%d],virtIspAddr:virt[%p]/phy[%p]", i, mpIspVirRegAddrVA[i], mpIspVirRegAddrPA[i] );
            }
            ...
            mpIspCQDescBufferVirt =
                (MUINT32*)( (((unsigned long)m_ispCQDescBufInfo.virtAddr) + ISP_DRV_CQ_DESCRIPTOR_ADDR_ALIGN) & (~ISP_DRV_CQ_DESCRIPTOR_ADDR_ALIGN) );
            ...
            mpIspCQDescBufferPhy =
                (MUINT32*)( (((unsigned long)m_ispCQDescBufInfo.phyAddr) + ISP_DRV_CQ_DESCRIPTOR_ADDR_ALIGN) & (~ISP_DRV_CQ_DESCRIPTOR_ADDR_ALIGN) );

            LOG_DBG("cq(0x%x)(0x%x)(0x%" PRIxPTR ")(0x%" PRIxPTR ")(%p)(%p)",
                mIspCQDescSize, m_ispCQDescBufInfo.size,
            ...
            for (int i = 0; i < mTotalCQNum; i++)
            {
                if ( 0 == i ) {
                    ...
                    mpIspCQDescriptorVirt[i] = (ISP_DRV_CQ_CMD_DESC_STRUCT *)(
                        (unsigned long)mpIspCQDescriptorVirt[i - 1] + sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT) * CAM_MODULE_MAX);
                    mpIspCQDescriptorPhy[i] = (MUINT32*)(
                        ((unsigned long)mpIspCQDescriptorPhy[i - 1]) + ((sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT) * CAM_MODULE_MAX)));
                }
                memcpy((MUINT8*)mpIspCQDescriptorVirt[i], mIspCQDescInit, sizeof(mIspCQDescInit));

                LOG_DBG("cq:[%d],mpIspCQDescriptor:Virt[%p]/Phy[%p],size/num(%zu/%d/%zu)",
                    i, mpIspCQDescriptorVirt[i], mpIspCQDescriptorPhy[i],
                    sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT), CAM_MODULE_MAX, sizeof(mIspCQDescInit));
            }
            ...
            if (sizeof(CQ_RTBC_RING_ST) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data too large(%zu)>(%zu)", sizeof(CQ_RTBC_RING_ST), sizeof(mIspCQDescInit));
                return MFALSE;
            }
...
//QQ    LOG_INF("CAM_FEO_ERR_STAT=0x%08x", readReg((((UINT32)&ispReg.CAM_FEO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    ...
    LOG_INF("[Tpipe]va(%p),pa(0x%08x),regPa(0x%08x)", pP2Package->tpipeTableVa, pP2Package->tpipeTablePa, regTpipePA);
    ...
    for (i = 0; i < dumpTpipeLine; i++) {
        LOG_INF("[Tpipe](%02d)-0x%08x-0x%08x-0x%08x-0x%08x-0x%08x-0x%08x-0x%08x-0x%08x-0x%08x-0x%08x", i,
            pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE], pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 1],
            pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 2], pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 3],
            pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 4], pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 5],
            pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 6], pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 7],
            pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 8], pP2Package->tpipeTableVa[i*DUMP_TPIPE_NUM_PER_LINE + 9]);
    }

    //dump isp register
    for (i = 0x4000; i <= 0x5000; i+=20) {
        LOG_INF("(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)",
            i, readReg(i, ISP_DRV_USER_ISPF),
            i + 4, readReg(i + 4, ISP_DRV_USER_ISPF),
            i + 8, readReg(i + 8, ISP_DRV_USER_ISPF),
            i + 12, readReg(i + 12, ISP_DRV_USER_ISPF),
            i + 16, readReg(i + 16, ISP_DRV_USER_ISPF));
    }
    LOG_INF("(0x%04x,0x%08x)(0x%04x,0x%08x)",
        0x5ff0, readReg(0x5ff0, ISP_DRV_USER_ISPF),
        0x5ff4, readReg(0x5ff4, ISP_DRV_USER_ISPF));
    ...
    LOG_INF("regCtlEnP2(0x%08x) regCtlTcmEn(0x%08x) regAnr1Con1(0x%08x) regAnr2Con1(0x%08x)",
        regCtlEnP2, regCtlTcmEn, regAnr1Con1, regAnr2Con1);
    ...
    LOG_INF("NSL2A(%x) NSL2B(%x) NBC(%x) NBC2(%x) TDR(%x)",
        nsl2aEn, nsl2bEn, nbcEn, nbc2En, tdrEn);
    LOG_INF("anr1_LceLink(%x) anr2_LceLink(%x) anr2_Mode(%x)",
        anr1_LceLink, anr2_LceLink, anr2_Mode);
...
MBOOL IspDrvImp::dumpCQTable(ISP_DRV_CQ_ENUM cq, MINT32 burstQIdx, MUINT32 dupCqIdx, ISP_DRV_CQ_CMD_DESC_STRUCT* cqDesVa, MUINT32* cqVirVa)
{
    ...
    LOG_INF("+,realCQIdx(%d),cq(%d),burstQIdx(%d),dupCqIdx(%d),cqDesVa(%p),cqVirVa(%p)",
        realCQIdx, cq, burstQIdx, dupCqIdx, cqDesVa, cqVirVa);
    ...
    if (cqDesVa == 0 || cqVirVa == 0) {
        ...
        LOG_INF("ISP_CQ_PA(%p)", mpIspCQDescriptorPhy[realCQIdx]);
        LOG_INF("[BasePA]CQ1(0x%x),CQ2(0x%x),CQ3(0x%x)",
            readReg(((MUINT32)((MUINT8*)&ispReg.CAM_CTL_CQ1_BASEADDR - (MUINT8*)&ispReg)), ISP_DRV_USER_ISPF),
            readReg(((MUINT32)((MUINT8*)&ispReg.CAM_CTL_CQ2_BASEADDR - (MUINT8*)&ispReg)), ISP_DRV_USER_ISPF),
            readReg((((MUINT8*)&ispReg.CAM_CTL_CQ3_BASEADDR - (MUINT8*)&ispReg)), ISP_DRV_USER_ISPF));
        LOG_INF("[REG]P2_EN(0x%x),P2_EN_DMA(0x%x),SCENARIO_REG(0x%x),CQ_EN_P2(0x%x)",
            readReg(((MUINT32)((MUINT8*)&ispReg.CAM_CTL_EN_P2 - (MUINT8*)&ispReg)), ISP_DRV_USER_ISPF),
            readReg(((MUINT32)((MUINT8*)&ispReg.CAM_CTL_EN_P2_DMA - (MUINT8*)&ispReg)), ISP_DRV_USER_ISPF),
            readReg(((MUINT32)((MUINT8*)&ispReg.CAM_CTL_SCENARIO - (MUINT8*)&ispReg)), ISP_DRV_USER_ISPF),
...
{
    MINT32 Ret;
    MUINT32 dumpStatus = (CAM_CTL_INT_P1_STATUS_PASS1_DON_ST |
                          CAM_CTL_INT_P1_STATUS_D_PASS1_DON_ST |
                          CAMSV2_INT_STATUS_PASS1_DON_ST |
                          CAMSV_INT_STATUS_PASS1_DON_ST |
                          CAM_CTL_INT_P1_STATUS_SOF1_INT_ST |
                          CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST |
                          CAMSV_INT_STATUS_TG_SOF1_ST |
                          CAMSV2_INT_STATUS_TG_SOF1_ST |
    ...
    if (pwaitIrq->UserInfo.UserKey < 0)/*cam1*/
        LOG_DBG(" - E. Type(%d),Status(0x%08x),Timeout(%d).",
            pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout);
    else
        LOG_DBG(" - E. Type(%d),Status(0x%08x),Timeout(%d).",
            pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout);
    //
    ...
    {   //change to non-clear wait and start to wait again
        if (pwaitIrq->UserInfo.UserKey < 0)/*cam1*/
            LOG_INF("ioctrl Ret(%d),Type(%d),Status(0x%08x),Timeout(%d),Tb-Ta(%d us).",
                Ret, pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout, Tb - Ta);
        else
            LOG_INF("ioctrl Ret(%d),Type(%d),Status(0x%08x),Timeout(%d),Tb-Ta(%d us).",
                Ret, pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout, Tb - Ta);
        ...
        if (pwaitIrq->UserInfo.UserKey < 0)/*cam1*/
            LOG_INF("Leave ERESTARTSYS,Type(%d),Status(0x%08x),Timeout(%d),Tb-Ta(%d us).",
                pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout, Tb - Ta);
        else
            LOG_INF("Leave ERESTARTSYS,Type(%d),Status(0x%08x),Timeout(%d),Tb-Ta(%d us).",
                pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout, Tb - Ta);
    }

    //err handler
    if (Ret < 0 || MTRUE == isp_drv_DbgLogEnable_DEBUG) {
        if (pwaitIrq->UserInfo.UserKey < 0)/*cam1*/
            LOG_INF("Type(%d),Status(0x%08x),Timeout(%d).",
                pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout);
        else
            LOG_INF("Type(%d),Status(0x%08x),Timeout(%d).",
                pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout);
        ...
        if (pwaitIrq->Type == 4 &&
            (pwaitIrq->Status == 0x80 || pwaitIrq->Status == 0x40 ||
             pwaitIrq->Status == 0x20 || pwaitIrq->Status == 0x2) ) {
    ...
    if (Ret < 0) {
        if (pwaitIrq->UserInfo.UserKey < 0)/*cam1*/
            LOG_ERR("ISP_WAIT_IRQ fail(%d). Type(%d), Status(0x%08x), Timeout(%d).",
                Ret, pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout);
        else
            LOG_ERR("ISP_WAIT_IRQ fail(%d). Type(%d), Status(0x%08x), Timeout(%d).",
                Ret, pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout);
    ...
    {   //v3
        ...
        LOG_DBG("christopher-1 sigNum(%d/%d)",
            waitirq.TimeInfo.passedbySigcnt,pwaitIrq->TimeInfo.passedbySigcnt);
...
MBOOL IspDrvImp::markIrq(ISP_DRV_WAIT_IRQ_STRUCT Irqinfo)
{
    ...
    if (Ret < 0)
    {
        LOG_ERR("mark irq fail, user key/type/status (%d/%d/0x%x)",
            Irqinfo.UserInfo.UserKey, Irqinfo.UserInfo.Type, Irqinfo.UserInfo.Status);
        return MFALSE;
    }
    return MTRUE;
}
...
{
    LOG_INF("flush irq, user key/type/status (%d/%d/0x%x)",
        Irqinfo.UserInfo.UserKey, Irqinfo.UserInfo.Type, Irqinfo.UserInfo.Status);
    ...
    if (Ret < 0)
    {
        LOG_ERR("flush irq fail, user key/type/status (%d/%d/0x%x)",
            Irqinfo.UserInfo.UserKey, Irqinfo.UserInfo.Type, Irqinfo.UserInfo.Status);
        return MFALSE;
    }
    return MTRUE;
...
{
    MINT32 Ret;
    LOG_DBG(" - E. key(%d),Type(%d),Status(0x%08x).",
        Irqinfo->UserInfo.UserKey, Irqinfo->UserInfo.Type, Irqinfo->UserInfo.Status);
    ...
    else
    {
        ...
        LOG_INF("christopher-2 sigNum(%d/%d)",
            qirq.TimeInfo.passedbySigcnt, Irqinfo->TimeInfo.passedbySigcnt);
    ...
    MINT32                  caller)
{
    ...
    switch (userEnum)
    {
        ...
        case ISP_DRV_USER_FD:
            break;
        default:
            LOG_ERR("data[%p] cn[0x%x] only ispDrv/isp_function/seninf have authorization to directly write hw registers\n",
                pRegIo, Count);
            return MFALSE;
    }
    switch (caller)
    ...
    MINT32        caller)
{
    ...
    switch (userEnum)
    {
        ...
        case ISP_DRV_USER_FD:
            break;
        default:
            LOG_ERR("ad[0x%x] vl[0x%lx] only ispDrv/isp_function/seninf have authorization to directly write hw registers\n",
                Addr, Data);
            return MFALSE;
    }
    switch (caller)
...
isp_reg_t* IspDrvImp::getCurHWRegValues()
{
    ...
    for (MUINT32 i = 0; i < (MUINT32)(size / sizeof(MUINT32)); i++)
    {
...
MBOOL IspDrvImp::kRefCntCtrl(ISP_REF_CNT_CTRL_STRUCT* pCtrl)
{
    MINT32 Ret;
    //
    LOG_DBG("(%d)(%d)(%p)", pCtrl->ctrl, pCtrl->id, pCtrl->data_ptr);
    //
...
MBOOL IspDrvImp::rtBufCtrl(void *pBuf_ctrl)
{
    ...
    if ( ISP_RT_BUF_CTRL_MAX == pbuf_ctrl->ctrl ) {
        LOG_DBG("[rtbc]cq(%d),vIspV/P(%p,%p),descV/P(%p,%p),mmap(%p)", \
            pbuf_ctrl->buf_id, \
...
//    else {
        ...
        if (MFALSE == cqRingBuf(pBuf_ctrl)) {
            LOG_ERR("MFALSE == cqRingBuf(pBuf_ctrl)");
            return MFALSE;
        }
#endif
//    }
    ...
    if (Ret < 0)
    {
        if ((EAGAIN == errno) && (ISP_RT_BUF_CTRL_ENQUE == pbuf_ctrl->ctrl ))
        {
            ...
            if (Ret >= 0)
            {
                LOG_ERR("ISP_BUFFER_CTRL-retry(%d)[errno(%d):%s]",
                    Ret, errno, strerror(errno));
                return MTRUE;
            }
        }
        LOG_ERR("ISP_BUFFER_CTRL(%d)[errno(%d):%s]",
            Ret, errno, strerror(errno));
        return MFALSE;
    }
    //
...
MUINT32 IspDrvImp::pipeCountDec(EIspDrvPipePath ePipePath)
{
    ...
    if (ret < 0)
    {
        LOG_ERR("ISP_REF_CNT_DEC fail(%d)[errno(%d):%s]",
            ret, errno, strerror(errno));
        Result = MFALSE;
    }
...
MBOOL IspDrvImp::updateCq0bRingBuf(void *pOBval)
{
 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (unsigned long)(va) - (unsigned long)(bva) ) + (unsigned long)(bpa) )
    ...
    LOG_DBG("[rtbc]va(%p),pa(%p)", pcqrtbcring_va, pcqrtbcring_pa);
    //
    ...
    ISP_WRITE_REG(this, CAM_CTL_CQ0B_BASEADDR,
        (unsigned long)this->getCQDescBufPhyAddr(ISP_DRV_CQ0B, 0, ISP_DRV_P1_DEFAULT_DUPCQIDX),
        ISP_DRV_USER_ISPF);
...
MBOOL IspDrvImp::_updateEnqCqRingBuf(void *pBuf_ctrl)
{
 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (unsigned long)(va) - (unsigned long)(bva) ) + (unsigned long)(bpa) )
    ...
    if ( _imgo_d_ == pbuf_ctrl->buf_id || _rrzo_d_ == pbuf_ctrl->buf_id ) {
        ...
        LOG_DBG("[rtbc] VF2(0x%x),cQ0c_pa=%p", cam_tg_vf, pcqrtbcring_pa);
    }
        ...
        LOG_DBG("[rtbc] CAMSV VF(0x%x),cQ0c_camsv_pa=%p", cam_tg_vf, pcqrtbcring_pa);
    }
        ...
        LOG_DBG("[rtbc] CAMSV VF(0x%x),cQ0c_camsv2_pa=%p", cam_tg_vf, pcqrtbcring_pa);
    }

    //VF_EN==0
    if ( 0 == (cam_tg_vf & 0x01) ) {
        //
        LOG_DBG("[rtbc]va(%p),pa(%p),ctrl(%d),dma(%d),PA(0x%x),o_size(%d),zo_size(%d)", \
                pcqrtbcring_va, \
                pcqrtbcring_pa, \
                pbuf_ctrl->ctrl, \
                pbuf_ctrl->buf_id, \
        ...
        if ( _imgo_ == pbuf_ctrl->buf_id || _rrzo_ == pbuf_ctrl->buf_id) {
            ...
            ISP_WRITE_REG(this, CAM_CTL_CQ0C_BASEADDR,
                ((unsigned long)this->getCQDescBufPhyAddr(ISP_DRV_CQ0C, 0, ISP_DRV_P1_DEFAULT_DUPCQIDX)) + ((1==size)?0:sizeof(CQ_RING_CMD_ST)),
                ISP_DRV_USER_ISPF);
        }
        else if ( _imgo_d_ == pbuf_ctrl->buf_id || _rrzo_d_ == pbuf_ctrl->buf_id) {
            //LOG_INF("[rtbc] Update CAM_CTL_CQ0C_D_BASEADDR");
            ISP_WRITE_REG(this, CAM_CTL_CQ0C_D_BASEADDR,
                ((unsigned long)this->getCQDescBufPhyAddr(ISP_DRV_CQ0C_D, 0, ISP_DRV_P1_DEFAULT_DUPCQIDX)) + ((1==size)?0:sizeof(CQ_RING_CMD_ST)),
                ISP_DRV_USER_ISPF);
        }
        //
        LOG_DBG("[rtbc]imgo_sz(%d),rrzo_sz(%d), BufIdx=0x%x",
            pcqrtbcring_va->imgo_ring_size, pcqrtbcring_va->rrzo_ring_size, pbuf_info->bufIdx);
        //
        {
            int *pdata = (int*)&pcqrtbcring_va->rtbc_ring[0];
            for ( i = 0; i < (sizeof(CQ_RING_CMD_ST) >> 2) * size; i++) {
                LOG_DBG("[rtbc][des] addr:(%p) val:(0x%08x)", pdata + i, pdata[i]);
            }
        }
    }
    //VF_EN==1
        ...
        if ( pex_buf_info ) {
            ...
            if ( _imgo_ == pbuf_ctrl->buf_id || _imgo_d_ == pbuf_ctrl->buf_id) {
                ...
                if (!findTarget) {
                    ...
                    for ( i = 0; i < pcqrtbcring_va->imgo_ring_size; i++) {
                        LOG_DBG("[rtbc][%d] old/new BufIdx(%d/%d)",
                            i, pcqrtbcring_va->rtbc_ring[i].cq_rtbc.imgo_buf_idx, pbuf_info->bufIdx);
                   }
                }
            }
            else if (_rrzo_ == pbuf_ctrl->buf_id || _rrzo_d_ == pbuf_ctrl->buf_id) {
                ...
                if (!findTarget) {
                    LOG_ERR("[rtbc]error exRRZO-Fail. Bufidx(0x%x/0x%x) old/new imgo buffer(0x%x/0x%x)",
                        pbuf_info->bufIdx, pex_buf_info->bufIdx,
                        pcqrtbcring_va->rtbc_ring[i].cq_rtbc.rrzo_base_pAddr, pex_buf_info->base_pAddr);
                    for ( i = 0; i < pcqrtbcring_va->rrzo_ring_size; i++ ) {
                        LOG_DBG("[rtbc]ByIdx:[%d] old/new BufIdx(%d/%d)",
                            i, pcqrtbcring_va->rtbc_ring[i].cq_rtbc.rrzo_buf_idx, pbuf_info->bufIdx);
                    }
                }
            }
            else {
    ...
    switch ( pbuf_ctrl->ctrl ) {
        ...
        case ISP_RT_BUF_CTRL_CLEAR:
            ...
            if (sizeof(CQ_RTBC_RING_ST) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data exceed buffer size(%zu)>(%zu)",
                    sizeof(CQ_RTBC_RING_ST), sizeof(mIspCQDescInit));
            }
...
)
{
    ...
    if ((mTuningQueIdx[p2Cq].curWriteIdx >= ISP_TUNING_QUEUE_NUM) ||
    ...
    if ((nextQueueIdx==mTuningQueIdx[p2Cq].curReadP1Idx) ||
        (nextQueueIdx==mTuningQueIdx[p2Cq].curReadP2Idx)) {
...
)
{
    ...
    if (path == eTuningQueAccessPath_featureio) {
        ...
        LOG_DBG("p2Cq(%d),currQueueIdx(%d),pTuningQue(%p)",
            p2Cq, currQueueIdx, mTuningQueInf[p2Cq][currQueueIdx].pTuningQue);
        ...
        do {
            ...
            LOG_DBG("tarQueueIdx(%d),currQueueIdx(%d),curPreviewApplyP2Idx(%d),curReadP2Idx(%d),curWriteIdx(%d)",
                tarQueueIdx, currQueueIdx,
                mTuningQueIdx[p2Cq].curPreviewApplyP2Idx,
                mTuningQueIdx[p2Cq].curReadP2Idx,
                mTuningQueIdx[p2Cq].curWriteIdx);
...
#ifdef __LP64__
            LOG_DBG("camCtlEnP1(0x%x),camCtlEnP2(0x%x),pCurReadP2TuningQue(%p),keepP2PreviewQue(%p),eUpdateFuncBit(0x%lx)",
#else
            LOG_DBG("camCtlEnP1(0x%x),camCtlEnP2(0x%x),pCurReadP2TuningQue(%p),keepP2PreviewQue(%p),eUpdateFuncBit(0x%0llx)",
#endif
                    camCtlEnP1, camCtlEnP2, mTuningQueIdx[p2Cq].pCurReadP2TuningQue,
                    mTuningQueIdx[p2Cq].keepP2PreviewQue.pTuningQue,
                    mTuningQueIdx[p2Cq].eCurReadP2UpdateFuncBit);
            //
                                ...
                                if ((magicNum & ISP_MAGIC_NUM_CAPTURE_SIGN) == 0x00) { // preview mode
                                    ...
                                    if (gIspTuningFuncBitMapp[i].eTuningCqFunc1 != CAM_DUMMY_) {
                                        ...
                                        for (j = 0; j < moduleSize; j++) {
                                            mTuningQueIdx[p2Cq].keepP2PreviewQue.pTuningQue[addrOffset >> 2] =
                                                mTuningQueInf[p2Cq][currQueueIdx].pTuningQue[addrOffset >> 2];
                                            addrOffset += 4;
                                        }
                                    }
...
//set by imageio path                            mTuningQueIdx[p2Cq].keepTpipeField.nr3d.nr3d_on_height = pReadP2TuningQue->CAM_NR3D_ON_SIZ.Bits.NR3D_ON_HT;
        ...
        LOG_DBG("checkNum(%d),curWriteIdx(%d),isInitP1(%d)",
            checkNum, mTuningQueIdx[p2Cq].curWriteIdx, mTuningQueIdx[p2Cq].isInitP1);
        for (k = 0; k < checkNum; k++) {
            ...
            mTuningQueIdx[p2Cq].pCurReadP1TuningQue =
                mTuningQueIdx[p2Cq].keepP1Que.pTuningQue;
            //
            // [3]update eCurReadP1UpdateFuncBit & keepP1UpdateFuncBit
            mTuningQueIdx[p2Cq].eCurReadP1UpdateFuncBit =
                mTuningQueInf[p2Cq][currQueueIdx].eUpdateFuncBit;
...
#ifdef __LP64__
            LOG_DBG("camCtlEnP1(0x%x),camCtlEnP1D(0x%x),keepP1UpdateFuncBit(0x%lx),eUpdateFuncBit(0x%lx)",
#else
            LOG_DBG("camCtlEnP1(0x%x),camCtlEnP1D(0x%x),keepP1UpdateFuncBit(0x%llx),eUpdateFuncBit(0x%llx)",
#endif
                camCtlEnP1, camCtlEnP1D, mTuningQueIdx[p2Cq].keepP1UpdateFuncBit, \
                mTuningQueInf[p2Cq][currQueueIdx].eUpdateFuncBit);
            //
            for (i = 0; i < eIspTuningMgrFuncBit_Num; i++) {
                if ((1LL << gIspTuningFuncBitMapp[i].eTuningFuncBit) & mTuningQueIdx[p2Cq].eCurReadP1UpdateFuncBit) {
                    switch (gIspTuningFuncBitMapp[i].eTuningCtlByte) {
                        case eTuningCtlByte_P1:
                            if (gIspTuningFuncBitMapp[i].camCtlEnP1Bit != -1) {
                                ...
                                if (gIspTuningFuncBitMapp[i].eTuningCqFunc1 != CAM_DUMMY_) {
                                    ...
                                    for (j = 0; j < moduleSize; j++) {
                                        mTuningQueIdx[p2Cq].keepP1Que.pTuningQue[addrOffset >> 2] =
                                            mTuningQueInf[p2Cq][currQueueIdx].pTuningQue[addrOffset >> 2];
                                        addrOffset += 4;
                                    }
                                }
...
)
{
    ...
    ISP_DRV_CQ_ENUM cq;
    //
    LOG_DBG("+,path(%d),softScenario(%d),magicNum(0x%x)",
        path, softScenario, magicNum);
    //
...
)
{
    ...
    mapCqToP2Cq(eCq, p2Cq);
#ifdef __LP64__
    LOG_DBG("path(%d),eCq(%d),p2Cq(%d),magicNum(0x%x),updateFuncBit(0x%lx)",
#else
    LOG_DBG("path(%d),eCq(%d),p2Cq(%d),magicNum(0x%x),updateFuncBit(0x%0llx)",
#endif
        path, eCq, p2Cq, magicNum, updateFuncBit);
    ...
    MUINT32 magicNum)
{
    ...
    LOG_DBG("-,ePath(%d),top(%d),cq(%d),p2Cq(%d),magicNum(0x%x),en(0x%08x)",
        ePath, top, cq, p2Cq, magicNum,en);
...
)
{
    ...
    if ((mTuningQueIdx[p2Cq].curWriteIdx < ISP_TUNING_QUEUE_NUM) &&
        ...
        if (searchCnt >= ISP_TUNING_QUEUE_NUM) {
            LOG_INF("[Error]search fail, magic#(0x%x)/Q#(0x%x)/curWriteIdx(0x%x)",
                magicNum,
                mTuningQueInf[p2Cq][mTuningQueIdx[p2Cq].curWriteIdx].magicNum,
                mTuningQueIdx[p2Cq].curWriteIdx);
        ...
        LOG_ERR("[Error]tuningQue curWriteIdx(%d) not ready",
            mTuningQueIdx[p2Cq].curWriteIdx);
        ret = MFALSE;
    }
    LOG_DBG("-,getQueIdx(%d),curWriteIdx(%d)",
        getQueIdx, mTuningQueIdx[p2Cq].curWriteIdx);
...
)
{
    ...
    LOG_INF("+,p2Cq(%d),magicNum(0x%x),start(%d),end(%d)",
        p2Cq, magicNum,start, end);
    ...
    if (i > end) {
        getQueIdx = -1;
        LOG_WRN("[warning]cant seach the identical magic#(0x%x),start(%d),end(%d)",
            magicNum, start, end);
        ret = MFALSE;
    }
EXIT:

    LOG_INF("-,getQueIdx(%d),curWriteIdx(%d)",
        getQueIdx,mTuningQueIdx[p2Cq].curWriteIdx);
...
)
{
    ...
    LOG_DBG("tarUdm.bypass(%d),desUdm.bypass(%d)",
        mTuningQueIdx[p2Cq].keepTpipeField.udm.bayer_bypass,
        pTuningField.udm.bayer_bypass);
...
MBOOL IspDrvImp::enqueP2Frame(MUINT32 callerID, MINT32 p2burstQIdx, MINT32 p2dupCQIdx, MINT32 frameNum)
{
    ...
    if (ret < 0)
    {
        LOG_ERR("ISP_DRV_BUFQUE_CTRL_ENQUE_FRAME fail(%d). callerID(0x%x).",
            ret, callerID);
        return MFALSE;
    }
    else
...
MBOOL IspDrvImp::waitP2Deque()
{
    ...
    if (ret < 0)
    {
        LOG_ERR(" ISP_DRV_BUFQUE_CTRL_WAIT_DEQUE fail(%d). callerID(0x%x).",
            ret, edBufQueV.callerID);
        return MFALSE;
    }
    else
    {
...
MBOOL IspDrvImp::dequeP2FrameSuccess(MUINT32 callerID, MINT32 p2dupCQIdx)
{
    ...
    if (ret < 0)
    {
        LOG_ERR("ISP_DRV_BUFQUE_CTRL_DEQUE_DONE fail(%d). callerID(0x%x).",
            ret,callerID);
        return MFALSE;
    }
    else
...
MBOOL IspDrvImp::waitP2Frame(MUINT32 callerID, MINT32 p2dupCQIdx, MINT32 timeoutUs)
{
    ...
    if (ret < 0)
    {
        LOG_ERR("ISP_DRV_BUFQUE_CTRL_WAIT_FRAME fail(%d).callerID(0x%x), Timeout(%d).",
            ret, callerID, timeoutUs);
        return MFALSE;
    }
    else
    {
...
{
    LOG_DBG("cq: %d. ispVirRegAddr: %p.", cq, ispVirRegAddr);
...
MBOOL IspDrvVirImp::reset(MINT32 /*rstpath*/)
{
    ...
    memset(mpIspVirRegBuffer, 0x00, ISP_BASE_RANGE);
    return MTRUE;
}
//-----------------------------------------------------------------------------
...
{
    LOG_VRB("mpIspVirRegBuffer(%p)", mpIspVirRegBuffer);
    if (mpIspVirRegBuffer != NULL)
    {
...
getIspCQModuleInfo(CAM_MODULE_ENUM eModule, ISP_DRV_CQ_MODULE_INFO_STRUCT &outInfo)
{
    ...
    LOG_DBG("getIspCQModuleInfo: eModule=0x%d, ofst=0x%x, reg_num=0x%x",
        eModule, outInfo.addr_ofst, outInfo.reg_num);
