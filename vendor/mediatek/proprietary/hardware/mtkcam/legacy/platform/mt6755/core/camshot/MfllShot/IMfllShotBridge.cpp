...
{
    /**
     *  If using dual phase, MfllShot will manager it's own lifetime */
    if (mpMfllShotImp->isUsingDualPhase() != MTRUE)
        delete  mpMfllShotImp;
...
uninit()
{
    ...
    if  ( 0 < mu4InitRefCount )
    {
        ...
        if  ( 0 == mu4InitRefCount )
        {
            /* If using dual phase, MfllShot may control it's life time */
            if (getImp()->isUsingDualPhase() != MTRUE)
                ret = getImp()->uninit();
        }
    }
    MY_LOGD("- mu4InitRefCount(%d), ret(%d)", mu4InitRefCount, ret);
