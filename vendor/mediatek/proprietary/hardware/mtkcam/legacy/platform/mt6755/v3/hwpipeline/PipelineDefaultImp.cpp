...
#undef hasUsage
...
#if ZSD_SUPPORTED
    mZsdMode       = (!mOperation_mode && !mbHasRecording && !mbHasRaw &&
                      !mbOpaqueReproc && !mbYuvReproc ) ? MTRUE : MFALSE;
    int32_t zsdDisable = ::property_get_int32("vendor.debug.camera3.hwpipeline.zsd.disable", 0);
    mZsdMode       = (zsdDisable)? MFALSE : mZsdMode;
#endif
    //
    if ( mOperation_mode )
    {
    ...
    MY_LOGI("max stream(%d, %d), raw(%d), jpeg(%d), hasRecording(%d), operation_mode(%d):burst(%d) zsdmode(%d)",
             mMaxStreamSize.w, mMaxStreamSize.h, mbHasRaw, mbHasJpeg,
             mbHasRecording, mOperation_mode, mDefaultBusrstNum, mZsdMode );
    //
    mbUseP1Node    = MTRUE;
    mbUseP2Node    = MTRUE;
...
#endif
{
    ...
    property_get("vendor.jpeg.rotation.enable", value, "1");
    ...
    if ( mLogLevel == 0 ) {
        mLogLevel = ::property_get_int32("vendor.debug.camera.log.hwpipeline", 0);
    }
#if ZSD_SUPPORTED
    mZsdInfo.mProcessor = ZsdProcessor::createInstance();
    mZsdInfo.mInfoCount = 30;
#endif
...
)
{
    ...
    // config zsd streams
    if ( mParams.mZsdMode )
        CHECK_ERROR(configContextLocked_Zsd(mpPipelineContext));
#endif
    // config node
...
#if ZSD_SUPPORTED
        if ( mParams.mZsdMode ) {
            Mutex::Autolock _l1(mZsdLock);
            mZsdInfo.mMap.add(pFrame->getRequestNo(), mZsdInfo.mSerialNo);
            if ( mZsdInfo.mMap.size() > (size_t)mZsdInfo.mInfoCount )
                mZsdInfo.mMap.removeItemsAt(0);
            if ( evaluateResult.zsdRequest ) {
                MY_LOGD("frame %u:%" PRId64 " is zsd request", pFrame->getRequestNo(),  mZsdInfo.mSerialNo);
                mZsdInfo.mvZsdRequest.push_back( std::make_pair(mZsdInfo.mSerialNo, evaluateResult.zsdRefNo) );
            }
            mZsdInfo.mSerialNo++;
        }
#endif
...
#if ZSD_SUPPORTED
    if ( mParams.mZsdMode)
        BuildStream(eStreamType_META_APP, mpHalMeta_DynamicP1);
    else
#endif
...
#if ZSD_SUPPORTED
    if ( mParams.mZsdMode )
        BuildStream(eStreamType_IMG_APP   , mpHalImage_P1_Raw);
    else
#endif
...
MERROR
PipelineDefaultImp::
configContextLocked_Zsd(sp<PipelineContext> /*pContext*/)
{
    FUNC_START;
#if ZSD_SUPPORTED
    ZsdProcessor::InfoSetT zsdInfoSet;
    if ( mpHalImage_P1_Raw.get() ) {
        zsdInfoSet.vImageConfig.add( mpHalImage_P1_Raw->getStreamId(),
                                     ZsdProcessor::ImageStreamConfig{mpHalImage_P1_Raw, true} );
    }
    if ( mpAppMeta_Control.get() ) {
        zsdInfoSet.vMetaConfig.add(  mpAppMeta_Control->getStreamId(),
                                     ZsdProcessor::MetaStreamConfig{mpAppMeta_Control, false} );
    }
    // zsdInfoSet.vMetaConfig.add(  mpAppMeta_DynamicP1->getStreamId(),
    //                              ZsdProcessor::MetaStreamConfig{mpAppMeta_DynamicP1, false} );
    if ( mpHalMeta_DynamicP1.get() ) {
        zsdInfoSet.vMetaConfig.add(  mpHalMeta_DynamicP1->getStreamId(),
                                     ZsdProcessor::MetaStreamConfig{mpHalMeta_DynamicP1, false} );
    }
    mZsdInfo.mProcessor->configStreamIds(zsdInfoSet);
#endif
    FUNC_END;
    //
    return OK;
...
#if ZSD_SUPPORTED
    if ( mParams.mZsdMode ) {
        if ( isZsdRequest(request) ) {
            // this default backtracking count should fill in custom folder
            int32_t zsdBacktrackingCount = ::property_get_int32("vendor.debug.camera3.hwpipeline.zsd.bt", 5);
            zsdBacktrackingCount = (zsdBacktrackingCount>8) ? 5 : zsdBacktrackingCount;
            result.zsdRequest = mZsdInfo.mProcessor->isZsdBuffersReady(mZsdInfo.mSerialNo-zsdBacktrackingCount);
            result.zsdRefNo = mZsdInfo.mSerialNo-zsdBacktrackingCount;
            //
            if ( result.zsdRequest && result.zsdRefNo>=0 ) {
                CAM_TRACE_NAME("evaluateRequestLocked_zsd");
                CHECK_ERROR( evaluateRequestLocked_zsd(request, result, aCollector) );
                return OK;
            }
        }
    }
...
)
{
    ...
    CHECK_ERROR( evaluateRequestLocked_updateIOMap(request, result, aCollector) );
    //
    CHECK_ERROR( evaluateRequestLocked_updateStreamBuffers_zsd(request, result, aCollector) );
...
)
{
    ...
    if( request.vOImageInfos_Jpeg.size() ) {
        ...
        // p2
        if( ! result.isTSflow ) {
            if( ! result.isYuvReprocIn() && ! result.isOpaqueReprocIn() && ! result.zsdRequest )
...
)
{
    ...
    if( request.vOImageInfos_Yuv.size() ) {
        ...
        if ( useResized && !result.reprocInfos.useOpaqueIn && !result.reprocInfos.useYuvIn && !result.zsdRequest) {
...
#if ZSD_SUPPORTED
    ZsdProcessor::BufferSetT zsdBufferSet;
    ZsdProcessor::BufferSetT zsdDequedBufferSet;
    bool bZsd_reg_success = false;

    if ( mParams.mZsdMode ) {
        for( size_t i = 0; i < request.pRequest->vIMetaBuffers.size(); i++ ) {
            if ( request.pRequest->vIMetaBuffers.keyAt(i)==eSTREAMID_END_OF_FWK ) {
                zsdBufferSet.vMetaSet.add(
                        request.pRequest->vIMetaBuffers.keyAt(i),
                        new ZsdProcessor::MetaItem(request.pRequest->vIMetaBuffers.valueAt(i), false) );
            }
        }
        bZsd_reg_success = ( mZsdInfo.mProcessor->registerAllocatedBuffers(zsdBufferSet, mZsdInfo.mSerialNo)==OK &&
                             mZsdInfo.mProcessor->dequeBuffers(zsdDequedBufferSet, mZsdInfo.mSerialNo)==OK ) ?
                           true : false;
    }
    // update zsd relative buffer if needed.
    // this buffers are allocated by ZsdProcessor.
    if ( mParams.mZsdMode && bZsd_reg_success ) {
        for ( size_t i=0; i<zsdDequedBufferSet.vMetaSet.size(); i++ ) {
            if ( zsdDequedBufferSet.vMetaSet.keyAt(i)==eSTREAMID_END_OF_FWK )
                continue;
            // MY_LOGD("set MetaBuffer(%" PRIx64 ":%p)", zsdDequedBufferSet.vMetaSet.keyAt(i),
            //         zsdDequedBufferSet.vMetaSet.valueAt(i)->pStreamBuffer.get() );
            result.vAppMetaBuffers.add(
                    zsdDequedBufferSet.vMetaSet.keyAt(i),
                    zsdDequedBufferSet.vMetaSet.valueAt(i)->pStreamBuffer
                    );
        }
        for ( size_t i=0; i<zsdDequedBufferSet.vImageSet.size(); i++ ) {
            // MY_LOGD("set ImageBuffer(%" PRIx64 ":%p)", zsdDequedBufferSet.vImageSet.keyAt(i),
            //         zsdDequedBufferSet.vImageSet.valueAt(i)->pStreamBuffer.get() );
            result.vAppImageBuffers.add(
                    zsdDequedBufferSet.vImageSet.keyAt(i),
                    zsdDequedBufferSet.vImageSet.valueAt(i)->pStreamBuffer
                    );
        }
    }
#endif
...
MERROR
PipelineDefaultImp::
evaluateRequestLocked_updateStreamBuffers_zsd(
    parsedAppRequest const& request,
    evaluateRequestResult& result,
    IOMapColloctor& /*aCollector*/
)
{
    ...
    // int bGetZsdBuffersSuccess = OK;
    // app meta
    {
        ZsdProcessor::BufferSetT zsdBufferSet;
        CHECK_ERROR( mZsdInfo.mProcessor->getZsdBuffers( zsdBufferSet, result.zsdRefNo, mZsdInfo.mSerialNo) );
        for ( size_t i=0; i<zsdBufferSet.vMetaSet.size(); i++ ) {
            if ( zsdBufferSet.vMetaSet.keyAt(i)==eSTREAMID_END_OF_FWK )
                continue;
            MY_LOGD("set zsd request MetaBuffer(%" PRIx64 ":%p)", zsdBufferSet.vMetaSet.keyAt(i),
                    zsdBufferSet.vMetaSet.valueAt(i)->pStreamBuffer.get() );
            result.vAppMetaBuffers.add(
                    zsdBufferSet.vMetaSet.keyAt(i),
                    zsdBufferSet.vMetaSet.valueAt(i)->pStreamBuffer
                    );
        }
        for ( size_t i=0; i<zsdBufferSet.vImageSet.size(); i++ ) {
            MY_LOGD("set zsd request ImageBuffer(%" PRIx64 ":%p)", zsdBufferSet.vImageSet.keyAt(i),
                    zsdBufferSet.vImageSet.valueAt(i)->pStreamBuffer.get() );
            result.vAppImageBuffers.add(
                    zsdBufferSet.vImageSet.keyAt(i),
                    zsdBufferSet.vImageSet.valueAt(i)->pStreamBuffer
                    );
        }
        for( size_t i = 0; i < request.pRequest->vIMetaBuffers.size(); i++ ) {
            result.vAppMetaBuffers.replaceValueFor(
                    request.pRequest->vIMetaBuffers.keyAt(i),
                    request.pRequest->vIMetaBuffers.valueAt(i)
                    );
        }
            ...
            // set "the largest frame duration of streams" as "minimum frame duration"
            {
                ...
                for ( size_t i=0; i<result.vAppImageBuffers.size(); i++ ) {
                    ...
                    if( mvStreamDurations.indexOfKey(streamId) < 0 ) {
                        MY_LOGW_IF( mLogLevel>=1, "Request App stream %#" PRIx64 " have not configured yet", streamId);
                        continue;
                    }
...
) const
{
    ...
    MBOOL bZsd = MFALSE;
    //
    // android.control.enableZsl? OR vendortag?
#if 1
    IMetadata::IEntry const& entryCapIntent = pAppMetaControl->entryFor(MTK_CONTROL_CAPTURE_INTENT);
    IMetadata::IEntry const& entryEnableZsl = pAppMetaControl->entryFor(MTK_CONTROL_ENABLE_ZSL);
    if ( mpHalImage_P1_Raw.get() &&
         ! entryCapIntent.isEmpty() &&
         MTK_CONTROL_CAPTURE_INTENT_STILL_CAPTURE==entryCapIntent.itemAt(0, Type2Type<MUINT8>() ) &&
         ! entryEnableZsl.isEmpty() &&
         MTK_CONTROL_ENABLE_ZSL_TRUE==entryEnableZsl.itemAt(0, Type2Type<MUINT8>() ) )
        bZsd = MTRUE;
#endif
    MY_LOGI_IF(bZsd, "req: %u is zsd request!(%p)", request.pRequest->requestNo, mpHalImage_P1_Raw.get());
    if( pAppMetaControl )
        request.pRequest->vIMetaBuffers[0]->unlock(
                __FUNCTION__, pAppMetaControl
            );
...
)
{
    ...
    if( mZsdInfo.mShutter.indexOfKey(frameNo) < 0  &&
        pStreamBuffer->getStreamInfo()->getStreamId() == eSTREAMID_META_APP_DYNAMIC_01 )
    {
        IMetadata* pMetadata = pStreamBuffer->tryReadLock(__FUNCTION__);
        if ( pMetadata ) {
            IMetadata::IEntry const entry = pMetadata->entryFor(MTK_SENSOR_TIMESTAMP);
            if ( entry.isEmpty() ) {
                pStreamBuffer->unlock(__FUNCTION__, pMetadata);
                return;
            }

            MINT64 const timestamp = entry.itemAt(0, Type2Type<MINT64>());

            mZsdInfo.mShutter.add(frameNo, timestamp);
            if( mZsdInfo.mShutter.size()>(size_t)mZsdInfo.mInfoCount ) mZsdInfo.mShutter.removeItemsAt(0);

            mZsdInfo.mvMetaSB.add(frameNo, pStreamBuffer);
            if( mZsdInfo.mvMetaSB.size()>(size_t)mZsdInfo.mInfoCount ) mZsdInfo.mvMetaSB.removeItemsAt(0);

            MY_LOGD_IF( mLogLevel>=2, "add shutter cache: frameNo(%u)->shutter(%" PRId64 ") SB(%" PRIx64 ":%p)",
                        frameNo, timestamp,
                        pStreamBuffer->getStreamInfo()->getStreamId(), pStreamBuffer.get() );
            pStreamBuffer->unlock(__FUNCTION__, pMetadata);
        }
    }
...
)
{
    ...
    // update shutter time
    {
        IMetadata* pMetadata = pStreamBuffer->tryWriteLock(__FUNCTION__);
        // ssize_t idx =  mZsd_shutter.indexOfKey(frameNo-5);
        // MY_LOGI("[%p], frameNo(%u) idx(%d)", pMetadata, frameNo-5, idx);

        if ( pMetadata ) {
            IMetadata::IEntry entry(MTK_SENSOR_TIMESTAMP);
            // MINT64 timestamp = mZsd_shutter.valueFor(frameNo-5);
            MINT64 timestamp = mZsdInfo.mShutter.valueFor(frameNo-5);
            entry.push_back(timestamp, Type2Type<MINT64>());
            MY_LOGI("get shutter from cache: frameNo(%u)->shutter(%" PRId64 ")", frameNo-5, timestamp);
            // dumpShutterCacheLocked();
            pMetadata->update(entry.tag(), entry);
            pStreamBuffer->unlock(__FUNCTION__, pMetadata);
            return true;
        }
...
dumpShutterCacheLocked()
{
    String8 str = String8::format("%s(%zu): ", __FUNCTION__, mZsdInfo.mShutter.size() );
    for( size_t i=0; i<mZsdInfo.mShutter.size(); i++) {
        str.appendFormat("(%zu:%u:%" PRId64 "); ", i, mZsdInfo.mShutter.keyAt(i), mZsdInfo.mShutter.valueAt(i));
    }
...
dumpZsdInfoLocked()
{
    MY_LOGD_IF( mLogLevel>=2, "zsd processor(%p) counter(%" PRId64 ") zsdRequest(%zu) zsdMsp(%zu) zsdShutter(%zu)",
            mZsdInfo.mProcessor.get(), mZsdInfo.mSerialNo, mZsdInfo.mvZsdRequest.size(),
            mZsdInfo.mMap.size(), mZsdInfo.mShutter.size() );
    String8 str = String8::format("dump map(%zu): ", mZsdInfo.mMap.size());
    // for ( size_t i=0; i<mZsdInfo.mMap.size(); i++) {
    //     str.appendFormat("(%zu:%u:%" PRId64 "); ", i, mZsdInfo.mMap.keyAt(i), mZsdInfo.mMap.valueAt(i));
    // }
    MY_LOGD_IF( mLogLevel>=2, "%s", str.string());
}
...
{
    CAM_TRACE_CALL();
    MY_LOGD_IF( mLogLevel>= 1, "frameNo:%u + userId:%#" PRIxPTR " OAppMeta#(left:%zd this:%zu)",
                frameNo, userId, nOutMetaLeft, vOutMeta.size());
    NSCam::Utils::CamProfile profile(__FUNCTION__, "Cam3DeviceCommon");
    //
#if ZSD_SUPPORTED
    if ( mParams.mZsdMode )
        updateFrameZsd(frameNo, userId, nOutMetaLeft, vOutMeta);
#endif
...
)
{
    ...
    CAM_TRACE_CALL();
    MY_LOGD_IF( mLogLevel>= 1, "frameNo:%u + userId:%#" PRIxPTR " OAppMeta#(left:%zd this:%zu)",
                frameNo, userId, nOutMetaLeft, rvOutMeta.size());
    //
    Mutex::Autolock _l1(mZsdLock);
    auto itZsd = mZsdInfo.mvZsdRequest.begin();
    bool bZsdCb = false;
    while ( itZsd != mZsdInfo.mvZsdRequest.end() ) {
        int64_t zsdRequestNo = mZsdInfo.mMap.valueFor(frameNo);
        if ( zsdRequestNo == (*itZsd).first ) {
            bZsdCb = true;
            Vector<sp<IMetaStreamBuffer>>::iterator it = rvOutMeta.begin();
            while( it != rvOutMeta.end() ) {
                sp<IMetaStreamBuffer> pBuffer = *it;
                MY_LOGD_IF( mLogLevel>=0, "frameNo:%u->zsdCounter(%" PRId64 ") StreamBuffer(%p:%s)",
                            frameNo, zsdRequestNo, pBuffer.get(), pBuffer->getStreamInfo()->getStreamName());
                ++it;
            }
            if ( nOutMetaLeft==0 ) {
                int status = mZsdInfo.mProcessor->notifyZsdDone( mZsdInfo.mMap.valueFor(frameNo));
                if( status != OK ) {
                    MY_LOGE("notify zsd done error - status:%d(%s)", status, ::strerror(-status));
                }
                // need to append shutter callback MetaStreamBuffer
                // updateZsdShutterLocked(frameNo, rvOutMeta.editItemAt(0));
                if( mZsdInfo.mvMetaSB.indexOfKey( (*itZsd).second )<0 ) {
                    MY_LOGE("cannot find previous MetaStreamBuffer of frame:%u", frameNo);
                    for(size_t i=0; i<mZsdInfo.mvMetaSB.size(); i++) {
                        MY_LOGD("(%zu/%zu):(%u/%u->%p)", i, mZsdInfo.mvMetaSB.size(),
                                mZsdInfo.mvMetaSB.keyAt(i), mZsdInfo.mMap.valueFor(frameNo),
                                mZsdInfo.mvMetaSB.valueAt(i).get() );
                    }
                } else {
                    rvOutMeta.insertAt(mZsdInfo.mvMetaSB.editValueFor( (*itZsd).second ), 0);
                }
                mZsdInfo.mvZsdRequest.erase(itZsd);
                break;
            }
        }
        ++itZsd;
    ...
    if ( ! bZsdCb ) {
        for ( size_t i=0; i< rvOutMeta.size(); i++ ) {
            cacheZsdShutterLocked(frameNo, rvOutMeta.editItemAt(i));
        }
        ZsdProcessor::BufferSetT returnedBufferSet;
        Vector<sp<IMetaStreamBuffer>>::iterator it = rvOutMeta.begin();
        int64_t zsdRequestNo = mZsdInfo.mMap.valueFor(frameNo);
        while( it != rvOutMeta.end() ) {
            sp<IMetaStreamBuffer> pBuffer = *it;
            MY_LOGD_IF( mLogLevel>=1, "frameNo:%u->zsdCounter(%" PRId64 ") sb(%p:%s)",
                        frameNo, zsdRequestNo, pBuffer.get(), pBuffer->getStreamInfo()->getStreamName());
            returnedBufferSet.vMetaSet.add( pBuffer->getStreamInfo()->getStreamId(),
                                            new ZsdProcessor::MetaItem(pBuffer, false) );
            ++it;
        }
       mZsdInfo.mProcessor->enqueBuffers(returnedBufferSet, zsdRequestNo);
        // #warning "MUST FIX: unsync of requestNo and frameNo!"
    }
    //
    for(size_t i=0; i<rvOutMeta.size(); i++) {
        if ( rvOutMeta[i]->getStreamInfo()->getStreamId() == mpHalMeta_DynamicP1->getStreamId() )
            rvOutMeta.removeItemsAt(i);
    }
    dumpZsdInfoLocked();
#endif
}
