...
*
********************************************************************************/
MBOOL PostProcPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, MUINT32 isUseDefaultTuning)
{
    int ret = 0;
    int idx_imgi = -1;
    int idx_vipi = -1;
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ )
    {
        ...
        {
            PIPE_INF("vInPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),size/va/pa/md(0x%8x/0x%8x/0x%8x/0x%8x)",
                                                        i,
                                                        vInPorts[i]->eImgFmt,
        ...
        {
            PIPE_INF("vInPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),size/va/pa/md(0x%8x/0x%8x/0x%8x/0x%8x), crop_x/y/fx/fy/w/h (%d/%d/%d/%d/%d/%d)",
                                                        i,
                                                        vInPorts[i]->eImgFmt,
                                                        vInPorts[i]->u4ImgWidth,
                                                        vInPorts[i]->u4ImgHeight,
                                                        ...
                                                        vInPorts[i]->u4BufVA,
                                                        vInPorts[i]->u4BufPA,
                                                        vInPorts[i]->memID,vInPorts[i]->crop.x,vInPorts[i]->crop.y,vInPorts[i]->crop.floatX,vInPorts[i]->crop.floatY,\
                                                        vInPorts[i]->crop.w,vInPorts[i]->crop.h);
        }
        //
        if ( EPortIndex_IMGI == vInPorts[i]->index )
        {
            ...
            //PIPE_INF("[imgi crop] x/y/fx/fy/w/h (%d/%d/%d/%d/%d/%d)",vInPorts[i]->crop.x,vInPorts[i]->crop.y,vInPorts[i]->crop.floatX,vInPorts[i]->crop.floatY,\
                //vInPorts[i]->crop.w,vInPorts[i]->crop.h);
        }
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ )
    {
        ...
        //
        PIPE_INF("vOutPorts:[%d]:(0x%x),w(%d),h(%d),stride(%d,%d,%d),type(%d),idx(%d),dir(%d),size/va/pa/md(0x%8x/0x%8x/0x%8x/0x%8x)",i,
                                                        vOutPorts[i]->eImgFmt,
...
#else
            PIPE_DBG("pass2:cc-mode only");
            isEn1HrzStatusFixed = 1;
            //ctl_mux_sel2 = 0xC0280308;    // 4078. add set PASS1_DONE_MUX to 3, BIN_OUT_SEL to 8
            ctl_mux_sel2 = 0xC0080308;    // 4078. add set PASS1_DONE_MUX to 3, BIN_OUT_SEL to 8
...
//SL TEST_RAW_YUV            tcm_en = 0x8c100003;//SL TEST_MDP         tcm_en = 0x0C100003; //SL TEST_MDP // from IspStreamCase0() 0x15004054 8C100003
    ...
    m_camPass2Param.isMdpCropEn=mdp_crop_en;
    //ctl_int
    ...
    //for tpipe
    m_camPass2Param.imgo_mux_en=(ctl_mux_sel2&0x00100000) >> 20;
    m_camPass2Param.imgo_mux=(ctl_mux_sel2&0x00000010) >>4;
    m_camPass2Param.img2o_mux_en=(ctl_mux_sel2&0x00200000) >> 21;
    m_camPass2Param.img2o_mux=(ctl_mux_sel2&0x00000040) >>6;
    ...
    PIPE_INF("Pass(%d),sceFmt(%d),imgi.fmt(%d),isByImgi(%d),sceID(%d),en1/en2/dma(0x%x/0x%x/0x%x) ,mux sel/sel2/sram(0x%x/0x%x/0x%x),isTurn(%d),tdri VA=0x%x tdr PA=0x%x\n",
        this->m_pipePass, \
        meScenarioFmt, \
