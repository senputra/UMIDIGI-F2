...
#define ISP_WRITE_BITS(RegBase, RegName, FieldName, Value)              \
    do {                                                                \
        unsigned long regAddr = ((unsigned long)(&(RegBase->RegName.Raw)) -  (unsigned long)RegBase); \
        if (                                                            \
            ( regAddr == 0x00004000 ) ||                                \
            ( regAddr == 0x00004004 ) ||                                \
            ( regAddr == 0x00004008 ) ||                                \
        ...
        {                                                               \
            TEMP_LOG_DBG("[%s, %s, line%04d] WARNING: Addr 0x%08x is enable bit, please use ISP_WRITE_ENABLE_BITS().", __FILE__, __func__, __LINE__, (unsigned long)(&(RegBase->RegName.Raw)) - (unsigned long)RegBase);  \
        }                                                               \
        ...
        {                                                               \
            TEMP_LOG_DBG("[%s, %s, line%04d] WARNING: Addr 0x%08x is seninf register, please use SENINF_WRITE_BITS().", __FILE__, __func__, __LINE__, (unsigned long)(&(RegBase->RegName.Raw)) - (unsigned long)RegBase);  \
        }                                                               \
    } while (0)
...
#define ISP_WRITE_REG(RegBase, RegName, Value)                          \
    do {                                                                \
        ...
        {                                                               \
            TEMP_LOG_DBG("[%s, %s, line%04d] WARNING: Addr 0x%08x is enable bit, please use ISP_WRITE_ENABLE_REG().", __FILE__, __func__, __LINE__, (unsigned long)(&(RegBase->RegName.Raw)) - (unsigned long)RegBase);  \
        }                                                               \
        if( regAddr < 0x00008000 )                                      \
        {                                                               \
        ...
        {                                                               \
            TEMP_LOG_DBG("[%s, %s, line%04d] WARNING: Addr 0x%08x is seninf register, please use SENINF_WRITE_REG().", __FILE__, __func__, __LINE__, (unsigned long)(&(RegBase->RegName.Raw)) - (unsigned long)RegBase);  \
        }                                                               \
    } while (0)
...
{\
    ...
    {\
        TEMP_LOG_DBG("[%s, %s, line%04d] WARNING: Addr 0x%08x is enable bit, please use ISP_WRITE_ENABLE_REG/BITS() or ISP_IOCTL_WRITE_ENABLE_REG/BITS().", __FILE__, __func__, __LINE__, (unsigned long)(&(RegBase->RegName.Raw)) - (unsigned long)RegBase); \
    }\
}
#define ISP_IOCTL_READ_REG(IspDrvPtr,RegBase,RegName)       IspDrvPtr->readReg((MUINT8*)(&(RegBase->RegName))-(MUINT8*)RegBase)
#define ISP_IOCTL_WRITE_REG(IspDrvPtr,RegBase,RegName,RegValue) \
{\
    ISP_CHECK_ENABLE_BIT(RegBase,RegName);\
    IspDrvPtr->writeReg((MUINT8*)(&(RegBase->RegName))-(MUINT8*)RegBase,RegValue);\
}
//
...
{\
    ...
    RegBase->RegName.Raw = ISP_IOCTL_READ_REG(IspDrvPtr,RegBase,RegName);\
    RegBase->RegName.Bits.FieldName = FieldValue;\
    IspDrvPtr->writeReg((MUINT8*)(&(RegBase->RegName))-(MUINT8*)RegBase, RegBase->RegName.Raw);\
    pthread_mutex_unlock(&IspRegMutex);\
}
...
//        virtual MBOOL   IsGdmaMode() = 0;
        ...
        virtual MUINT32 *getCQDescBufVirAddr(ISP_DRV_CQ_ENUM cq);
        virtual MUINT32 *getCQVirBufVirAddr(ISP_DRV_CQ_ENUM cq);
