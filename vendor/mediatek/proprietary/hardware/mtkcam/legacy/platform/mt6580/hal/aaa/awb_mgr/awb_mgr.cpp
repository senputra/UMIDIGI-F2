...
MY_LOG("cameraPreviewInit %d",__LINE__);
    ...
    if (m_rAWBOutput.rPreviewAWBGain.i4R == 0)
    {
        m_rAWBOutput.rPreviewAWBGain = m_pNVRAM_3A->rAWBNVRAM[m_i4AWBNvramIdx].rAlgoCalParam.rLightAWBGain.rDNP;
	    m_rAWBOutput.rPreviewRAWPreGain2 = m_pNVRAM_3A->rAWBNVRAM[m_i4AWBNvramIdx].rAlgoCalParam.rLightAWBGain.rDNP;
        MY_LOG("[%s] DNP default gain = %d/%d/%d",__FUNCTION__,m_rAWBOutput.rPreviewAWBGain.i4R,
                m_rAWBOutput.rPreviewAWBGain.i4G, m_rAWBOutput.rPreviewAWBGain.i4B);
    }
...
MRESULT AwbMgr::doPvAWB(MINT32 i4FrameCount, MBOOL bAEStable, MINT32 i4SceneLV, MVOID *pAWBStatBuf, MBOOL bApplyToHW)
{
    ...
    MY_LOG("[%s()] magic num : %d, bAEStable :%d , i4SceneLV : %d",__FUNCTION__,
                      i4FrameCount,bAEStable,i4SceneLV);
    ...
    else if ((m_bEnableAWB) && (!m_bAWBLock) && (!m_bAdbAWBLock) && (m_eAWBMode != LIB3A_AWB_MODE_OFF))
    {
        ...
        {
            MY_LOG("[%s()] m_bAWBModeChanged=%d m_bInitState=%d",__FUNCTION__,m_bAWBModeChanged,m_bInitState);
...
*/
                ...
                // update AE RAW pre-gain2
                //IspTuningMgr::getInstance().setIspAEPreGain2(m_i4SensorIdx, m_rAWBOutput.rPreviewRAWPreGain2);
                //MY_LOG("m_rAWBOutput.rPreviewRAWPreGain2 :R=%d, G=%d, B=%d",m_rAWBOutput.rPreviewRAWPreGain2.i4R,m_rAWBOutput.rPreviewRAWPreGain2.i4G,m_rAWBOutput.rPreviewRAWPreGain2.i4B);
                ISP_MGR_AE_RAWPREGAIN2_T::getInstance(static_cast<ESensorDev_T>(m_eSensorDev)).setRAWPregain2(m_rAWBOutput.rPreviewRAWPreGain2, m_i4SensorIdx);
