...
updateOne()
{
    ...
    if ( ! getHw()->deque(eID_Pass1DispOut, &vDeBufPass1Out, false) )
    {
        int i, tryCnt = 1;
        for (i = 0; i < tryCnt; i++)
        {
            ...
            mSensorInfo.reset();
            if (getHw()->deque(eID_Pass1DispOut, &vDeBufPass1Out, (i == tryCnt  - 1) ? true : false))
            {
                MY_LOGD("success.");
                break;
            }
...
*/
    if ( ! getHw()->deque(eID_Pass1DispOut, &dummy, false) )
    {
        ...
        for (i = 0; i < tryCnt; i++)
        {
            MY_LOGW("drop frame failed. try reset sensor(%d)", i);
            mSensorInfo.reset();
            if (getHw()->deque(eID_Pass1DispOut, &dummy, (i == tryCnt - 1) ? true : false ))
            {
                MY_LOGD("success.");
                break;
            }
