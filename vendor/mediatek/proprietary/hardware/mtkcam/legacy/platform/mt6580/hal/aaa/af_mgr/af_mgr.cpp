...
MRESULT AfMgr::init(MINT32 eSensorDev, MINT32 i4SensorIdx)
{
    ...
    m_i4LockAFInContiMode = m_i4AuotoFocusInContiPicMode = m_i4TriggerAlgoInAutoMode = m_i4AutoFocusInAutoMode = 0;
    m_i4InitAFCmd = 1;
    ...
    m_pMcuDrv->moveMCU(m_i4AFPos, m_i4CurrSensorDev);
    // --- init ISP Drv/Reg ---
    #ifdef ISPREADREG
    ...
    MY_LOG("[moveMCU] %d, [AFDac] %d", m_i4AFPos, m_sAFOutput.i4AFPos);
...
MRESULT AfMgr::uninit()
{
    ...
    if (m_pMcuDrv)
    {
        m_i4AFPos = m_sAFOutput.i4AFPos;
        MY_LOG("[uninitMcuDrv] - SensorDev: %d, DAC(%d)", m_i4CurrSensorDev, m_i4AFPos);
    ...
    if (m_eCurrAFMode == 3 || m_eCurrAFMode == 4)
    {
        if (i4isTurnOn == MTRUE)
        {
            ...
            if (m_pIAfAlgo)
            {
                ...
                if (DebugEnable == 0)
                {
                    m_pIAfAlgo->setAFMode(LIB3A_AF_MODE_OFF);
                    m_pIAfAlgo->triggerAF();
                    m_pIAfAlgo->cancelAF();
                    m_pIAfAlgo->setAFMode(LIB3A_AF_MODE_AFC);
                    MY_LOG("add new modify");
                }
                else
                {
...
EAfState_T AfMgr::getAFState()
{
    ...
    MINT32 i4AutoFocusInAutoMode  = m_i4AutoFocusInAutoMode;
    MINT32 i4TriggerAlgoInAutoMode  = m_i4TriggerAlgoInAutoMode;

    switch(m_eCurrAFMode)
    {
    case 1: //MTK_CONTROL_AF_MODE_AUTO :
    case 2: //MTK_CONTROL_AF_MODE_MACRO :
        if (m_i4ChangeSearchingState && m_i4AutoFocusInAutoMode)
        {
            if (m_i4SearchDone)
            {
                ...
                m_i4TriggerAlgoInAutoMode = m_i4AutoFocusInAutoMode = 0;
            }
            else
            {
        ...
        if (m_i4CancelAutoFocusCmd)
        {
            ...
            m_i4AutoFocusInAutoMode = 0;
            m_i4ChangeSearchingState = 0;
            m_i4TriggerAlgoInAutoMode = 0;
        }
    ...
    if ((m_eAFState != NewState) || i4SetAFModeCmd || i4AutoFocusCmd ||
         i4CancelAutoFocusCmd || i4ChangeSearchingState) {
        CAM_LOGI("[%s] State(%d->%d) Mode(%d) (%d, %d, %d, %d, %d, %d, %d, %d, %d)",
                __FUNCTION__,
                m_eAFState,
                NewState,
                m_eCurrAFMode,
                ...
                i4AutoFocusInAutoMode,
                i4TriggerAlgoInAutoMode);
    } else {
        MY_LOG("[%s] State(%d->%d) Mode(%d) (%d, %d, %d, %d, %d, %d, %d, %d, %d)",
                __FUNCTION__,
                m_eAFState,
                NewState,
                m_eCurrAFMode,
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    IAeMgr::getInstance().getAEMeteringBlockAreaValue(m_i4CurrSensorDev, rWinSize, uAEBlockAreaY, &u2YCnt);
    m_sAFInput.i4AEBlockAreaYCnt = u2YCnt;
    m_sAFInput.pAEBlockAreaYvalue = uAEBlockAreaY;
    m_sAFInput.i4IsFlashFrm=FlashMgr::getInstance().isAFLampOn(m_i4CurrSensorDev);
    ...
    // AF v1.2
    // i4Coef[7] > 0: enable new scenechange , else: disable
    IAeMgr::getInstance().getAEBlockYvalues(m_i4CurrSensorDev,m_aeBlockV, 25);
    ...
    if (m_eCurrAFMode == 1 || m_eCurrAFMode == 2) {
        if (m_i4TriggerAlgoInAutoMode == 1 && m_i4AutoFocusInAutoMode == 1) {
...
MVOID AfMgr::autoFocus()
{
    ...
    CAM_LOGI("%s AFCtrlMode(%d), SearchDoneFlg(%d) AutoFocusCmd(%d)",
           __FUNCTION__,
           m_eLIB3A_AFMode,
           m_i4SearchDone,
           m_i4AutoFocusCmd);
...
MVOID AfMgr::cancelAutoFocus()
{
    ...
    CAM_LOGI("%s AFCtrlMode(%d), SearchDoneFlg(%d) CancelAutoFocusCmd(%d)",
           __FUNCTION__,
           m_eLIB3A_AFMode,
           m_i4SearchDone,
