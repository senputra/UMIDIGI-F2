...
)
{
    ...
    NVRAM_DRV_LOG("[readNvramData]+ a_eSensorType=%d a_eNvramDataType=%d p=%p", a_eSensorType, a_eNvramDataType, a_pNvramData);
    F_ID rNvramFileID;
    int i4FileInfo;
    int i4RecSize;
    int i4RecNum;
...
//    static bool bCustomInit = 0; //[ALPS00424402] [CCT6589] Len shading page --> Save to NVRAM --> CCT reboot failed
    NVRAM_DRV_LOG("[readDefaultData] sensor ID = %ld; NVRAM data type = %d p=%p\n", a_u4SensorID, a_eNvramDataType,a_pNvramData);
    ...
    case CAMERA_NVRAM_DATA_3A:
        ...
        NVRAM_CAMERA_3A_STRUCT* p2;
        p2 = (NVRAM_CAMERA_3A_STRUCT*)a_pNvramData;
        NVRAM_DRV_LOG("readdefault: sin=%d",p2->rAWBNVRAM[0].rAlgoCalParam.rRotationMatrix.i4Sin);
        NVRAM_DRV_LOG("readdefault: cos=%d",p2->rAWBNVRAM[0].rAlgoCalParam.rRotationMatrix.i4Cos);
