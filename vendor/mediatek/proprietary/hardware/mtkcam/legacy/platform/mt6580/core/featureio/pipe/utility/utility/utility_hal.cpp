                       ...
                       MUINT32 u4ExifOrientation)
{
    ...
    rExifParam.u4Orientation = u4ExifOrientation;
    ...
    CAM_LOGI("Shane Test I mpParamsMgr->getOpenId()=%d", mpParamsMgr->getOpenId());
     Hal3ABase* p3AHal = 0;
    if(mpParamsMgr->getOpenId() == 0){
        CAM_LOGI("Shane Test I mpParamsMgr->getOpenId()=%d  deviceID=1",mpParamsMgr->getOpenId());
        // back cam device id == 1
        p3AHal = Hal3ABase::createInstance(1);
    }else{
        CAM_LOGI("Shane Test I mpParamsMgr->getOpenId()=%d  deviceID=2",mpParamsMgr->getOpenId());
        // front cam device id == 2
        p3AHal = Hal3ABase::createInstance(2);
    }
