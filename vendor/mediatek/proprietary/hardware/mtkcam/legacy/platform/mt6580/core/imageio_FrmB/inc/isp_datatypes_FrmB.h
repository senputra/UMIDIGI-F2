...
public:
    ...
    MUINT32  tgFps;          //tgFps
    IspDMACfg():
        format_en(0),swap(0),format(0),bus_size_en(0),bus_size(0),pixel_byte(1),ring_en(0),ring_size(0),memBuf_c_ofst(0),memBuf_v_ofst(0),v_flip_en(0),lIspColorfmt(eImgFmt_IMPLEMENTATION_DEFINED),capbility(0x00),tgFps(30)
        {};
};
...
public:
    ...
    public:
        MdpRotDMACfg():
            crop_en(0),pixel_byte(1),uv_plane_swap(0),Format(CDP_DRV_FORMAT_YUV422),Plane(CDP_DRV_PLANE_1),Sequence(CDP_DRV_SEQUENCE_YVYU),Rotation(CDP_DRV_ROTATION_0),Flip(0),capbility(0x00){};
};
...
public:
    ...
    IspRect img2o_crop;     //add here is because of following ROME arch.
public:
    IspHrzCfg(){
