...
updateOne()
{
    ...
    //*************************************************************
    pass1Time = passTime;
    if ( ! getHw()->deque(eID_Pass1Out, &vDeBufPass1Out, false) )
    {
        int i, tryCnt = 1;
        for (i = 0; i < tryCnt; i++)
        {
            ...
            mSensorInfo.reset();
            if (getHw()->deque(eID_Pass1Out, &vDeBufPass1Out, (i == tryCnt  - 1) ? true : false))
            {
                MY_LOGD("success.");
                break;
            }
...
lbExit:
    ...
    MY_LOGD_IF(miLogLevel, "Time(%3lld/%3lld/%3lld/%3lld)ms",
            passTime/1000000,
            pass1Time/1000000,
            pass2Time/1000000,
            vssTime/1000000);
...
dropFrame()
{
    ...
    if ( ! getHw()->deque(eID_Pass1Out, &dummy, false) )
    {
        ...
        for (i = 0; i < tryCnt; i++)
        {
            MY_LOGW("drop frame failed. try reset sensor(%d)", i);
            mSensorInfo.reset();
            if (getHw()->deque(eID_Pass1Out, &dummy, (i == tryCnt - 1) ? true : false ))
            {
                MY_LOGD("success.");
                break;
            }
