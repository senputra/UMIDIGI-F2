...
inline static
MINT32 _convertResultToMeta(const Result_T& rResult, const IMetadata& staticMeta, MetaSet_T& rMetaResult)
{
...
#define _TEST_SCENE_MODE    (1<<16)
    ...
    if (u4Flag & _TEST_TONEMAP2)
    {
        ...
        {
            MFLOAT fLinearCurve[] = {0.0f, 0.0f, 0.5f, 0.8f, 1.0f, 0.0f};
            rParam.u1TonemapMode = MTK_TONEMAP_MODE_CONTRAST_CURVE;
            rParam.vecTonemapCurveBlue.resize(64);
            rParam.vecTonemapCurveGreen.resize(6);
            rParam.vecTonemapCurveRed.resize(6);
            ::memcpy(rParam.vecTonemapCurveBlue.editArray(), fCurve, sizeof(MFLOAT)*64);
            ::memcpy(rParam.vecTonemapCurveGreen.editArray(), fLinearCurve, sizeof(MFLOAT)*6);
            ::memcpy(rParam.vecTonemapCurveRed.editArray(), fLinearCurve, sizeof(MFLOAT)*6);
        }
        else if (u4Tonemap == 13)
        {
            MFLOAT fLinearCurve[] = {0.0f, 1.0f, 0.3f, 0.7f, 0.5f, 1.0f, 1.0f, 0.0f};
            rParam.u1TonemapMode = MTK_TONEMAP_MODE_CONTRAST_CURVE;
            rParam.vecTonemapCurveBlue.resize(8);
            rParam.vecTonemapCurveGreen.resize(64);
            rParam.vecTonemapCurveRed.resize(8);
            ::memcpy(rParam.vecTonemapCurveBlue.editArray(), fLinearCurve, sizeof(MFLOAT)*8);
            ::memcpy(rParam.vecTonemapCurveGreen.editArray(), fCurve, sizeof(MFLOAT)*64);
            ::memcpy(rParam.vecTonemapCurveRed.editArray(), fLinearCurve, sizeof(MFLOAT)*8);
        }
        else if (u4Tonemap == 14)
        {
            MFLOAT fLinearCurve[] = {0.0f, 0.0f, 0.5f, 0.8f, 0.8f, 0.1f, 1.0f, 1.0f};
            rParam.u1TonemapMode = MTK_TONEMAP_MODE_CONTRAST_CURVE;
            rParam.vecTonemapCurveBlue.resize(8);
            rParam.vecTonemapCurveGreen.resize(64);
            rParam.vecTonemapCurveRed.resize(64);
        ...
        else if (u4Tonemap == 15)
        {
            MFLOAT fLinearCurve[] = {0.0f, 0.0f, 0.5f, 0.1f, 0.8f, 0.1f, 1.0f, 0.0f};
            rParam.u1TonemapMode = MTK_TONEMAP_MODE_CONTRAST_CURVE;
            rParam.vecTonemapCurveBlue.resize(8);
            rParam.vecTonemapCurveGreen.resize(8);
            rParam.vecTonemapCurveRed.resize(8);
            ::memcpy(rParam.vecTonemapCurveBlue.editArray(), fLinearCurve, sizeof(MFLOAT)*8);
            ::memcpy(rParam.vecTonemapCurveGreen.editArray(), fLinearCurve, sizeof(MFLOAT)*8);
            ::memcpy(rParam.vecTonemapCurveRed.editArray(), fLinearCurve, sizeof(MFLOAT)*8);
        }
        else
        {
...
start(MINT32 i4StartNum)
{
    ...
    MY_LOG("[%s] Update 2", __FUNCTION__);
...
getCur(MUINT32 frmId, MetaSet_T& result)
{
    ...
    for (; it != mMetaResultQueue.end(); it++)
    {
        ...
        if (frmId == u4Id)
        {
            ...
            i4AppSize = it->appMeta.count();
            i4HalSize = it->halMeta.count();
            CAM_LOGD("[%s] i4Id(%d)/AppSize(%d)/HalSize(%d)", __FUNCTION__, u4Id, i4AppSize, i4HalSize);
...
setIsp(MINT32 flowType, const MetaSet_T& control, void* pRegBuf, MetaSet_T* pResult)
{
    ...
    if (QUERY_ENTRY_SINGLE<IMetadata::Memory>(control.halMeta, MTK_PROCESSOR_CAMINFO, caminfoBuf))
    {
        NSIspTuning::RAWIspCamInfo rRawInfo;
        ::memcpy(&rRawInfo, caminfoBuf.array(), sizeof(NSIspTuning::RAWIspCamInfo));
