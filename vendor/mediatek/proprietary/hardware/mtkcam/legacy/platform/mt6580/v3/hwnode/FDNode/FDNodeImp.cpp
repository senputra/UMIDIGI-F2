...
// conflict define in aaa_hal_common.h & aaa_hal_cmd.h
namespace NS3Av3 {
// 3A ASD info
struct ASDInfo_T {
...
//*********************************************************//
    ...
    if(mp3AHal)
    {
        if(mpDetectedFaces!=0) {
            ...
            mp3AHal->setFDInfo(mpDetectedFaces);
...
init(InitParams const& rParams)
{
    ...
    #if 1
    mDupImage.AddrY = new unsigned char[(640*960*3)/2];
    mDupImage.AddrU = mDupImage.AddrY + 640*960;
    mDupImage.AddrV = mDupImage.AddrU + ((640*960) >> 2);
    #else
    ...
    mp3AHal = IHal3A::createInstance(IHal3A::E_Camera_3, getOpenId(), getNodeName());
    mbInited = MTRUE;
...
uninit()
{
    ...
    if(mp3AHal)
    {
        ...
        mp3AHal->destroyInstance(getNodeName());
        mp3AHal = nullptr;
    }
    #endif
        ...
        ASDInfo* Result)
{
    ...
    if(!mHalInited) {
        ...
        mpASDHalObj->mHalAsdInit((void*)&ASDInfo, mASDWorkingBuffer, (mSensorType==NSCam::SENSOR_TYPE_RAW)?0:1, ASDWidth/2, ASDHeight/2);
        mHalInited = 1;
    }
...
FdNodeImp::
ASDImp::
initASD(MINT32 id __attribute__((unused)))
{
    mSensorType = NSCam::SENSOR_TYPE_RAW;
...
)
{
    ...
    MY_LOGD("CropRegion: p.x:%d, p.y:%d, s.w:%d, s.h:%d, ",cropRegion.p.x, cropRegion.p.y, cropRegion.s.w, cropRegion.s.h);

    int oldCropW = cropRegion.s.w;
    int oldCropH = cropRegion.s.h;
    MSize imgSize;
    imgSize.w = mDupImage.w;
    imgSize.h = mDupImage.h;
    if((cropRegion.s.w * imgSize.h) > (cropRegion.s.h * imgSize.w)) //pillarbox
    {
        //MY_LOGD("pillarbox");
        cropRegion.s.w = (cropRegion.s.h * imgSize.w) / imgSize.h;
        ...
        cropRegion.s.h = (cropRegion.s.w * imgSize.h) / imgSize.w;
        //cropRegion.p.x = cropRegion.p.x;
        cropRegion.p.y = cropRegion.p.y + ((oldCropH - cropRegion.s.h) >> 1);
    }

    MY_LOGD("after CropRegion: p.x:%d, p.y:%d, s.w:%d, s.h:%d, ",cropRegion.p.x, cropRegion.p.y, cropRegion.s.w, cropRegion.s.h);
