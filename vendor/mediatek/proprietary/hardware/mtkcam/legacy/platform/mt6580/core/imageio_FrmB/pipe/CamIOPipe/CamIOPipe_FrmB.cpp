...
dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo, MUINT32 const u4TimeoutMs /*= 0xFFFFFFFF*/)
{
    ...
    for ( MINT32 i = 0; i < (MINT32)rQBufInfo.vBufInfo.size() ; i++) {
        ...
        //
//#if 0
        PIPE_INF("[MyDeQ]dma(0x%x)/s(%d)/id(0x%x)/bs(0x%x)/va(0x%x)/pa(0x%x)/t(%d.%d)/img(%d,%d,%d,%d,%d,%d,%d,%d,%d)/m(%d,%d)/fc(%d)", \
                dmaChannel, \
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_HRZ:
            ...
            PIPE_INF("[3ASync]SET_IMG2O path:%d, m_num_0=%x -", arg2, pUserImg2oPara->m_num);
            ...
            if ( EPortIndex_TG1I == arg2){
                ...
                if (_CmdQSwitchEn(this->mOpenedPort,mMagicNum0,mMagicNum1,mMagicNum2)) {
                    ...
                    PIPE_INF("[3ASync]SET_HRZ:TG1I Magic0(0x%x)!= Magic1(0x%x)-", mMagicNum0, mMagicNum1);
                }
            }
            else {
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            if ( EPortIndex_TG1I == arg1 ) {
                PIPE_INF("[3ASync]P1_UPDATE+, TG1I, magicNum=0x%x", arg2);
                ...
                if (_CmdQSwitchEn(this->mOpenedPort,mMagicNum0,mMagicNum1,mMagicNum2)){
                    ...
                    PIPE_INF("[3ASync]: P1_UPDATE:TG1I Magic0(0x%x)!= Magic1(0x%x)-", mMagicNum0, mMagicNum1);
                }
            }
            else {
                PIPE_ERR("error: unsupport TG:0x%x\n",arg1);
        ...
        case EPIPECmd_SET_IMGO:
            {
                ...
                if ( EPortIndex_TG1I == arg2 ) {
                    PIPE_INF("[3ASync]SET_IMGO+, TG1I, magicNum=0x%x", this->m_ImgoCfg.m_num);
                    ...
                    if (_CmdQSwitchEn(this->mOpenedPort,mMagicNum0,mMagicNum1,mMagicNum2)){
                        ...
                        if(this->m_bBypassImg2o == MFALSE){
                            if(this->mOpenedPort & CAM_CTL_DMA_EN_IMG2O_EN){
                                if ( 0 != m_CamPathPass1.setP1HrzCfg(hrz_cfg) ){
                                    PIPE_ERR("[3ASync]SET_IMGO setP1HrzCfg Fail");
                                    ret = -1;
                                }
                            }
                        }
                        ...
                        PIPE_INF("[3ASync]: SET_IMGO:TG1I Magic0(0x%x)!= Magic1(0x%x) != magic2(0x%x)", mMagicNum0, mMagicNum1,mMagicNum2);
                    }
                }
                else {
                    PIPE_ERR("error: unsupport TG:0x%x\n",arg2);
