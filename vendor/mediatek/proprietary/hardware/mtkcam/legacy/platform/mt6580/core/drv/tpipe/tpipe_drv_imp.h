...
#define EN_TPIPE_ALGORITHM_DBG   (0)
    ...
    #define NEW_LINE_CHAR           ""      // XLOG?() already includes a new line char at the end of line, so don't have to add one.
        ...
        #undef LOG_INF
        #define LOG_INF(fmt, arg...)        do { if (MTKCAM_LOGENABLE_DEFAULT) {ALOGI( "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)    // <Info>: Show general system information. Like OS version, start/end of Service...
    #endif

    #if (DBG_LOG_LEVEL <= DBG_LOG_LEVEL_DEBUG)        // 3
        #undef LOG_DBG
        #define LOG_DBG(fmt, arg...)        do { if (MTKCAM_LOGENABLE_DEFAULT) {ALOGD( "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)    // <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
    #endif

    #if (DBG_LOG_LEVEL <= DBG_LOG_LEVEL_VERBOSE)    // 2
        #undef LOG_VRB
        #define LOG_VRB(fmt, arg...)        do { if (MTKCAM_LOGENABLE_DEFAULT) {ALOGV( "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)    // <Verbose>: Show more detail debug information. E.g. Entry/exit of private function; contain of local variable in function or code block; return value of system function/API...
    #endif
#else   // Using LDVT.
    #include "uvvf.h"
