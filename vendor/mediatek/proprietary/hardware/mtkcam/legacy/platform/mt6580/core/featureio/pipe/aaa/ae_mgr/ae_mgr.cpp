...
MRESULT AeMgr::setAEMinMaxFrameRate(MINT32 i4NewAEMinFps, MINT32 i4NewAEMaxFps)
{
    ...
    if ( (m_i4AEMinFps != i4NewMinFPS) || (m_i4AEMaxFps != i4NewMaxFPS))  {
            if ( (m_bIsVideoRecording == 0) || (m_bIsEISon == MTRUE) ){
                ...
                m_i4AEMinFps = 100;
                m_i4AEMaxFps = i4NewMaxFPS;
                MY_LOG("m_i4AEMinFps:%d(skip) m_i4AEMinFps:%d(real) m_i4AEMaxFps:%d\n", m_i4AEMinFps, 100, m_i4AEMaxFps);
            }
    ...
    MY_LOG("[%s()] AEMgr skip the framerate setting m_i4AEMinFps: %d m_i4AEMaxFps:%d\n", __FUNCTION__, i4NewMinFPS, i4NewMaxFPS);
    }
    ...
    if(m_u4IndexTestCnt < 300) {
        ...
        switch(uTestidx) {
                case 1: // Shutter delay frames [Exp:10ms <-> 20ms, 1x ]
                    ...
                    break;
                case 2: // Gain delay frames [Gain:1x <-> 2x , 10ms ]
                    if( m_u4IndexTestCnt%20 < 10 ) {
                        m_rAEOutput.rPreviewMode.u4Eposuretime = 10000;
                        m_rAEOutput.rPreviewMode.u4AfeGain = 1024;
                    } else {
                    ...
                    break;
                case 3: // Delay frames IT [20ms/1x <-> 10ms/2x ]
                ...
                case 4: // Perframe shutter command [10ms<->20ms,1x]
                        ...
                        m_rAEOutput.rPreviewMode.u4AfeGain = 1024;
                    break;
                case 5: // Perframe gain command [1x<->2x,10ms]
                        m_rAEOutput.rPreviewMode.u4Eposuretime = 10000;
                        m_rAEOutput.rPreviewMode.u4AfeGain = 1024*(m_u4IndexTestCnt%2 + 1);
                    break;
                case 6: // Perframe command IT [10ms/3x<->30ms/1x]
                ...
                case 7: // Perframe framerate command [10ms/5x <-> 50ms/1x]
                    if(m_u4IndexTestCnt%2 == 0) {
                        m_rAEOutput.rPreviewMode.u4Eposuretime = 5000*2;
                        if(m_i4ShutterDelayFrames == m_i4SensorGainDelayFrames) {
                        ...
                        m_rAEOutput.rPreviewMode.u4Eposuretime = 5000*5;
                        if(m_i4ShutterDelayFrames == m_i4SensorGainDelayFrames) {
                            m_rAEOutput.rPreviewMode.u4AfeGain = 1024*2;
                        } else {
                            m_rAEOutput.rPreviewMode.u4AfeGain = 1024*5;
...
MRESULT AeMgr::monitorAndReschedule( MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply )
{
    ...
    MY_LOG("[%s()] Calc:%d Apply:%d Cnt:%d Frame:%d ReSchedule:%d Exit:%d Skip:%d bStatChange:%d bAAASchedule:%d frameDelay:%d Stable:%d AEMonitorStable:%d VdCnt:%d ISPdelay:%d FaceArea:%d TouchArea:%d u4CwvYcur:%d u4CwvYpre:%d isneedupdatesensorisp:%d\n",
    ...
    MY_LOG("[I2Cincorrect] m_bSetShutterValue:%d, m_bSetGainValue:%d, m_bSetFrameRateValue:%d \n",  m_bSetShutterValue, m_bSetGainValue, m_bSetFrameRateValue);
    }
