...
#warning"[FIXME] Eveny node Id should belong to the global namespace..."
    ...
    {
        CAM_TRACE_BEGIN("init P2VSS node");
    ...
    {
        CAM_TRACE_BEGIN("init JPEG node");
...
{
    CAM_TRACE_NAME("PipelineModel_Default::configure");
    MERROR err = OK;
    RWLock::AutoWLock _l(mRWLock);
    //
    ...
    //
    if (sensorParam.type == SENSOR_TYPE_YUV) {
        mIsYuvSensorType = MTRUE;
        MY_LOGD("mIsYuvSensorType true");
    } else {
        mIsYuvSensorType = MFALSE;
        MY_LOGD("mIsYuvSensorType false");
    }
    ...
    {
        CAM_TRACE_BEGIN("allocate hal pools");
...
onDispatchFrame(android::sp<IPipelineFrame> const& pFrame, NodeId_T nodeId)
{
    ...
    //
    MY_LOGD("frameNo:%d, nodeId=%d", pFrame->getFrameNo(), nodeId);
...
)   const
{
    ...
    //
    rSensorParams.type = sensorStaticInfo.sensorType;
    //
    ...
    if( hasEncoderConsumer ) {
        if (maxStreamSize.w > sensorSize[0].w || maxStreamSize.h > sensorSize[0].h) {
            // Remain original sensor mode and sensor size.
            ...
            MY_LOGD("[sensor] Use video mode due to larger maxStreamSize (%d,%d).",
                    maxStreamSize.w, maxStreamSize.h);
        } else {
            // Change to preview mode (binning) instead of using video mode.
            rSensorParams.mode = SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
            rSensorParams.size = sensorSize[0]; // use preview sensor size
        }
...
#warning"[FIXME] should not always enable img2o"
        ...
        {
            MINT32 const width_min = ALIGN_2(sensorSize.w * P1_SCALE_RATIO_100X / 100); //[TODO] not always times  P1_SCALE_RATIO_10X. It depends.
            MINT32 width = maxYuvStreamSize.w;

            MY_LOGD("ResizedBuffer: width = %d, width_min = %d, sensorWidth = %d",
                    width, width_min, sensorSize.w);

//#warning"[FIXME] should enable img2o resize"
            // make sure img2o width:  sensor.w * 0.25 <= width <= sensor.w
            if( width < width_min )
                width = width_min;
...
)
{
    ...
    if ( mConfigProfile.mbConfigFD )
    {
        MSize size(640, 480); //FIXME: hard-code here?
    ...
    )
{
    ...
    // config node
    {
        ...
        cfgParams.sensorParams.type = sensorParam.type;
        //
...
) const
{
    ...
    rParams.isP2HalYuvJpegSupport = MFALSE;
    //
    rParams.vIAppMeta.add(mpAppMeta_Control->getStreamId()  , mpAppMeta_Control);
    ...
    if( mvYuvStreams_Fullraw.size() > 0 )
    {
        ...
        for( size_t idx = 0; idx < mvYuvStreams_Fullraw.size(); idx++) {
            ...
            if  ( 0 <= index ) {
                ...
                if (isStream(mpHalImage_Jpeg_YUV, streamId)) {
                    MY_LOGD("p2node has stream mpHalImage_Jpeg_YUV");
                    rParams.isP2HalYuvJpegSupport = MTRUE;
                }
    ...
    if( mvYuvStreams_Fullraw.size() > 0 )
    {
        ...
        for( size_t idx = 0; idx < mvYuvStreams_Fullraw.size(); idx++) {
            ...
            if  ( 0 <= index ) {
                ...
                if (isStream(mpHalImage_Jpeg_YUV, streamId)) {
                    MY_LOGD("p2nodeVSS has stream mpHalImage_Jpeg_YUV");
                    rParams.isP2HalYuvJpegSupport = MTRUE;
                }
