...
, m_bFirstFrameAfterAEInit(MFALSE)
{
...
MRESULT AeMgr::PreviewAEInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    MY_LOG("[%s()] m_eSensorDev:%d m_eSensorTG:%d m_i4SensorIdx:%d In/OutStableThd:%d/%d\n", __FUNCTION__, m_eSensorDev, m_eSensorTG, m_i4SensorIdx,
        m_rAEInitInput.rAEPARAM.strAEParasetting.u4InStableThd, m_rAEInitInput.rAEPARAM.strAEParasetting.u4OutStableThd);
...
MRESULT AeMgr::cameraPreviewInit(MINT32 i4SensorIdx, Param_T &rParam)
{
    ...
    m_bFirstFrameAfterAEInit = MTRUE;
    m_bRealISOSpeed = 0;
    m_i4WaitVDNum = 0;
    m_bSetShutterValue = MFALSE;
    m_bSetGainValue = MFALSE;
...
MRESULT AeMgr::cameraPreviewReinit()
{
    ...
    else if (m_eCamMode == eAppMode_ZsdMode)
    {
        // ZSD Mode
        m_eAECamMode = LIB3A_AECAM_MODE_ZSD;
        m_bRealISOSpeed = 0;
...
{
    ALOGD("[%s()] m_isAeMeterAreaEn:%d->%d", __FUNCTION__, m_isAeMeterAreaEn, en);
...
MRESULT AeMgr::setAEMeteringArea(CameraMeteringArea_T const *sNewAEMeteringArea)
{
    ...
    MY_LOG_IF(m_3ALogEnable, "[%s()] m_isAeMeterAreaEn:%d", __FUNCTION__, m_isAeMeterAreaEn);
    ...
    for (i = 0; i < u4AreaCnt; i++)
    {
            ...
            (sAEMeteringArea->rAreas[i].i4Top != m_eAEMeterArea.rAreas[i].i4Top) || (sAEMeteringArea->rAreas[i].i4Bottom != m_eAEMeterArea.rAreas[i].i4Bottom))
        {
            ...
            if (counter > 2)
            {
                MY_LOG_IF(m_3ALogEnable, "[%s()] touch area more than two sides reach boundary, counter:%d\n", __FUNCTION__, counter);
                bAreaChage = MFALSE;
            }
        }
    }
...
MRESULT AeMgr::setAEEVCompIndex(MINT32 i4NewEVIndex, MFLOAT fStep)
{
    ...
    if (m_i4EVIndex != i4NewEVIndex)
    {
        ...
        MY_LOG("[%s()] m_i4EVIndex: %d EVComp:%d fEVCompStep:%f PreAEState:%d m_bAELock:%d\n", \
        ...
        if (m_pIAeAlgo != NULL)
        {
            ...
            if (m_bAELock)
            {
                ...
                MY_LOG("[%s()] EV compensation with AE lock, prepare capture param in advance\n", __FUNCTION__);
            }
        }
        else
        {
...
MINT32 AeMgr::getEVCompensateIndex()
{
    ...
    switch (m_eAEEVcomp)
    {
        ...
        default:
        case LIB3A_AE_EV_COMP_00 : {iEVIndex =   0; break;}
    }
    return iEVIndex;
}
...
MRESULT AeMgr::setSceneMode(MUINT32 u4NewAEScene)
{
    ...
    switch (eNewAEScene)
    {
        ...
        case MTK_CONTROL_SCENE_MODE_CANDLELIGHT:
            ...
            break;
        default:
            MY_LOG_IF(m_3ALogEnable, "[%s()] The Scene mode is not correctly:%d\n", __FUNCTION__, eNewAEScene);
            eAEScene = LIB3A_AE_SCENE_AUTO;
            break;
    }
    ...
    if (m_eAEScene != eAEScene)
    {
        ...
        if (m_pIAeAlgo != NULL)
        {
            ...
            MY_LOG("[%s()] m_u4MaxShutter:%d m_u4MaxISO:%d\n", __FUNCTION__, m_u4MaxShutter, m_u4MaxISO);
        }
        else
        {
            m_u4MaxShutter = 100000;
...
{
    MY_LOG("[%s] m_bAELock:%d, m_bAPAELock:%d, m_bAFAELock:%d, m_eAEState:%d, m_ePreAEState:%d\n", \
...
MRESULT AeMgr::doPvAE(MINT32 i4FrameCount, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule)
{
    ...
    if (m_pIAeAlgo != NULL)
    {
        if ((pAEStatisticBuf != NULL) && ((u4AAOUpdate == 1) || (i4ActiveAEItem & E_AE_FLARE) || (i4ActiveAEItem & E_AE_AE_CALC)))
        {
            ...
            if ((m_bIsAutoFlare == MTRUE) && (m_eAEMode != LIB3A_AE_MODE_OFF) && (!m_bAELock))
            {
...
MRESULT AeMgr::doPreCapAE(MINT32 i4FrameCount, MBOOL bIsStrobeFired, MVOID *pAEStatBuf, MINT32 i4ActiveAEItem, MUINT32 u4AAOUpdate, MBOOL bAAASchedule)
{
    ...
    if (m_bEnableAE && (m_bAdbAELock == MFALSE))
    {
        if (m_bAEReadyCapture == MFALSE)
        {
            ...
            if (((m_i4TimeOutCnt > 18) && (m_bStrobeOn == MTRUE)) || ((m_i4TimeOutCnt > 18) && (m_bStrobeOn == MFALSE)))
            {
...
MRESULT AeMgr::doCapAE()
{
    ...
    if ((m_bIsAutoFlare == FALSE) && (m_eSensorMode == ESensorMode_Capture))
    {
        ...
        MY_LOG("[%s] i4SensorDev:%d Update flare offset/gain:%d/%d\n", __FUNCTION__, m_eSensorDev, mCaptureMode.i2FlareOffset, mCaptureMode.i2FlareGain);
    }

    if ((m_eCamMode == eAppMode_EngMode) && ((m_eAETargetMode == AE_MODE_AOE_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET)))
    {
...
MRESULT AeMgr::doRestoreAEInfo(MBOOL bRestorePrvOnly)
{
    ...
    if (m_bEnableAE)
    {
        ...
        copyAEInfo2mgr(&m_rAEOutput.rPreviewMode, &rAEOutput);
        MY_LOG_IF(m_3ALogEnable, "[%s()] Preview Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare offset/gain:%d/%d\n", __FUNCTION__,
            m_rAEOutput.rPreviewMode.u4Eposuretime, m_rAEOutput.rPreviewMode.u4AfeGain, m_rAEOutput.rPreviewMode.u4IspGain,
            m_rAEOutput.rPreviewMode.u2FrameRate, m_rAEOutput.rPreviewMode.i2FlareOffset, m_rAEOutput.rPreviewMode.i2FlareGain);
...
MRESULT AeMgr::doCapFlare(MVOID *pAEStatBuf, MBOOL bIsStrobe)
{
    ...
    MY_LOG("[%s()] i4SensorDev:%d Capture flare offset/gain:%d/%d Sensor Mode:%d Strobe:%d Enable:%d %d %d avgY:%d\n", __FUNCTION__, m_eSensorDev,
        ...
        if (m_rAEPLineLimitation.bEquivalent == MTRUE)
        {
            if (m_rAEPLineLimitation.u4IncreaseISO_x100 > 100)
            {
                if (sAEOutputInfo->u4Eposuretime > (100 * m_u4MaxShutter / m_rAEPLineLimitation.u4IncreaseISO_x100))
                {
                    ...
                    MY_LOG("[%s()] i4SensorDev:%d Shutter:%d->%d Sensor gain:%d->%d Isp gain:%d->%d\n", __FUNCTION__, m_eSensorDev,
                        sAEOutputInfo->u4Eposuretime, rSensorOutputData.u4SensorExpTime, sAEOutputInfo->u4AfeGain, rSensorOutputData.u4SensorGain, sAEOutputInfo->u4IspGain, rSensorOutputData.u4IspGain);
            ...
            else if (m_rAEPLineLimitation.u4IncreaseISO_x100 < 100)
            {
                ...
                if (u4ISOValue > (m_u4MaxISO * m_rAEPLineLimitation.u4IncreaseISO_x100 / 100))
                {
                    ...
                    MY_LOG("[%s()] i4SensorDev:%d Shutter:%d->%d Sensor gain:%d->%d Isp gain:%d->%d Ratio:%d\n", __FUNCTION__, m_eSensorDev,
                        sAEOutputInfo->u4Eposuretime, rSensorOutputData.u4SensorExpTime, sAEOutputInfo->u4AfeGain, rSensorOutputData.u4SensorGain, sAEOutputInfo->u4IspGain, rSensorOutputData.u4IspGain, m_rAEPLineLimitation.u4IncreaseISO_x100);
                    sAEOutputInfo->u4Eposuretime = rSensorOutputData.u4SensorExpTime;
                    sAEOutputInfo->u4AfeGain = rSensorOutputData.u4SensorGain;
                    sAEOutputInfo->u4IspGain = rSensorOutputData.u4IspGain;
                }
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT AeMgr::prepareCapParams(MBOOL IsEVChange)
{
...
MRESULT AeMgr::setStrobeMode(MBOOL bIsStrobeOn)
{
    ...
    {
        // capture on, get capture parameters for flare again.
        // prepareCapParams();
    }
    return S_AE_OK;
}
...
MRESULT AeMgr::getPreviewParams(AE_MODE_CFG_T &a_rPreviewInfo)
{
    ...
    MY_LOG_IF(m_3ALogEnable, "[%s()] Preview Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d frame rate:%d flare offset/gain:%d/%d\n", __FUNCTION__,
        a_rPreviewInfo.u4Eposuretime, a_rPreviewInfo.u4AfeGain, a_rPreviewInfo.u4IspGain, a_rPreviewInfo.u4RealISO,
        a_rPreviewInfo.u2FrameRate, a_rPreviewInfo.i2FlareOffset, a_rPreviewInfo.i2FlareGain);
    return S_AE_OK;
}
...
MRESULT AeMgr::updatePreviewParams(AE_MODE_CFG_T &a_rPreviewInfo, MINT32 i4AEidxNext)
{
    ...
    MY_LOG("[%s()] m_i4SensorDev:%d i4AEidxNext:%d Preview Exp mode:%d Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d frame rate:%d flare offset/gain:%d/%d \n",
        __FUNCTION__, m_eSensorDev, i4AEidxNext, mPreviewMode.u4ExposureMode, mPreviewMode.u4Eposuretime,
        mPreviewMode.u4AfeGain, mPreviewMode.u4IspGain, mPreviewMode.u4RealISO, mPreviewMode.u2FrameRate, mPreviewMode.i2FlareOffset, mPreviewMode.i2FlareGain);
...
MRESULT AeMgr::getExposureInfo(ExpSettingParam_T &strHDRInputSetting)
{
    ...
    if ((m_i4AEidxNext + u4BinningIndex) < strCurrentCaptureTable.u4TotalIndex)
    {
        ...
        MY_LOG("[%s()] m_i4SensorDev:%d Modify Capture Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d frame rate:%d flare offset/gain:%d/%d Index:%d Binning:%d\n", __FUNCTION__, m_eSensorDev,
            ...
            m_rAEOutput.rCaptureMode[0].u2FrameRate, m_rAEOutput.rCaptureMode[0].i2FlareOffset, m_rAEOutput.rCaptureMode[0].i2FlareGain, m_i4AEidxNext, u4BinningIndex);
    }
...
MRESULT AeMgr::getCaptureParams(MINT8 index, MINT32 i4EVidx, AE_MODE_CFG_T &a_rCaptureInfo)
{
    ...
    MY_LOG("[%s()] m_i4SensorDev:%d Capture Exp mode:%d Shutter:%d Sensor gain:%d Isp gain:%d ISO:%d frame rate:%d flare offset/gain:%d/%d\n", __FUNCTION__, m_eSensorDev, a_rCaptureInfo.u4ExposureMode, a_rCaptureInfo.u4Eposuretime, a_rCaptureInfo.u4AfeGain,
        a_rCaptureInfo.u4IspGain, a_rCaptureInfo.u4RealISO, a_rCaptureInfo.u2FrameRate, a_rCaptureInfo.i2FlareOffset, a_rCaptureInfo.i2FlareGain);
    return S_AE_OK;
}
...
MRESULT AeMgr::updateCaptureParams(AE_MODE_CFG_T &a_rCaptureInfo)
{
    ...
    MY_LOG("[%s()] m_i4SensorDev:%d Capture Exp mode:%d Shutter:%5d Sensor gain:%4d Isp gain:%4d ISO:%4d frame rate:%d flare offset/gain:%d/%d\n",
    ...
    if (m_bEnableAE)
    {
        ...
        u4FinalGain = (mCaptureMode.u4AfeGain * mCaptureMode.u4IspGain) >> 10;
        mCaptureMode.u4RealISO = u4PreviewBaseISO * u4FinalGain / u4PreviewBaseGain;
        MY_LOG("[%s()] m_i4SensorDev:%d Modify  Exp mode:%d Shutter:%5d Sensor gain:%4d Isp gain:%4d ISO:%4d frame rate:%d flare offset/gain:%d/%d\n",
            __FUNCTION__, m_eSensorDev, mCaptureMode.u4ExposureMode, mCaptureMode.u4Eposuretime,
            mCaptureMode.u4AfeGain, mCaptureMode.u4IspGain, mCaptureMode.u4RealISO, mCaptureMode.u2FrameRate, mCaptureMode.i2FlareOffset, mCaptureMode.i2FlareGain);
    }
...
MRESULT AeMgr::UpdateSensorISPParams(AE_STATE_T eNewAEState)
{
    ...
    switch (eNewAEState)
    {
        ...
        case AE_REINIT_STATE:
            ...
            MY_LOG("[%s():init] i4SensorDev:%d Preview Exp Mode:%d Shutter:%d Sensor Gain:%d Isp Gain:%d Flare offset/gain:%d/%d Frame Rate:%d\n",
                __FUNCTION__, m_eSensorDev, m_rAEOutput.rPreviewMode.u4ExposureMode, m_rAEOutput.rPreviewMode.u4Eposuretime,
                m_rAEOutput.rPreviewMode.u4AfeGain, m_rAEOutput.rPreviewMode.u4IspGain, m_rAEOutput.rPreviewMode.i2FlareOffset, m_rAEOutput.rPreviewMode.i2FlareGain, m_rAEOutput.rPreviewMode.u2FrameRate);
        ...
        case AE_MANUAL_FRAMERATE_STATE:
            ...
            if (m_i4WaitVDNum <= (m_i4IspGainDelayFrames + ISP_GAIN_DELAY_OFFSET))
            {
                ...
                if ((m_bAdbAEDisableSmooth == MFALSE) && (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableAESmoothGain == MTRUE))
                {
                    ...
                    else
                    {
                        ...
                        if (m_i4WaitVDNum > m_i4IspGainDelayFrames && m_bAEStable == TRUE)
                        {
                            // a complete cycle
                ...
                #endif
                MY_LOG_IF(m_3ALogEnable, "[%s:e] i4SensorDev:%d VDNum %d, Delay %d/%d/%d, Prev %d/%d/%d, Output %d/%d/%d State:%d FrameRate:%d %d %d Flare Offset/gain:%d/%d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
            ...
            {
                MY_LOG_IF(m_3ALogEnable, "[%s()] Frame rate too large:%d %d\n", __FUNCTION__, u2FrameRate, m_i4AEMaxFps);
            ...
            if (u2FrameRate != 0)
            {
                ...
                if (m_rSensorCurrentInfo.u8FrameDuration < mPreviewMode.u4Eposuretime * 1000L)
                {
                    m_rSensorCurrentInfo.u8FrameDuration = mPreviewMode.u4Eposuretime * 1000L;
                }
            }
            else
            {
        ...
        case AE_AF_STATE:
            ...
            m_i4WaitVDNum++;
            MY_LOG("[doAFAE] AF Mode SensorDev:%d Shutter:%5d Sensor gain:%4d Isp gain:%4d ISO:%d frame rate:%d flare offset/gain:%d/%d m_i4WaitVDNum:%d\n", m_eSensorDev, m_rAEOutput.rAFMode.u4Eposuretime,
                m_rAEOutput.rAFMode.u4AfeGain, m_rAEOutput.rAFMode.u4IspGain, m_rAEOutput.rAFMode.u4RealISO,
                m_rAEOutput.rAFMode.u2FrameRate, m_rAEOutput.rAFMode.i2FlareOffset, m_rAEOutput.rAFMode.i2FlareGain, m_i4WaitVDNum);
            MY_LOG("[doAFAE] Capture SensorDev:%d Shutter:%5d Sensor gain:%4d Isp gain:%4d ISO:%d frame rate:%d flare offset/gain:%d/%d\n", m_eSensorDev, m_rAEOutput.rCaptureMode[0].u4Eposuretime,
                m_rAEOutput.rCaptureMode[0].u4AfeGain, m_rAEOutput.rCaptureMode[0].u4IspGain, m_rAEOutput.rCaptureMode[0].u4RealISO,
                m_rAEOutput.rCaptureMode[0].u2FrameRate, m_rAEOutput.rCaptureMode[0].i2FlareOffset, m_rAEOutput.rCaptureMode[0].i2FlareGain);
            break;
        case AE_PRE_CAPTURE_STATE:
            m_bAEStable = MTRUE;
            m_bAEMonitorStable = m_bAEStable;
            prepareCapParams(MFALSE);
            MY_LOG("[doPreCapAE] SensorDev:%d AE is Stable\n", m_eSensorDev);
            MY_LOG("[doPreCapAE] Preview Exp mode:%d Shutter:%5d Sensor gain:%4d Isp gain:%4d ISO:%4d frame rate:%3d flare offset/gain:%d %d\n",
                m_rAEOutput.rPreviewMode.u4ExposureMode, m_rAEOutput.rPreviewMode.u4Eposuretime,
                m_rAEOutput.rPreviewMode.u4AfeGain, m_rAEOutput.rPreviewMode.u4IspGain, m_rAEOutput.rPreviewMode.u4RealISO,
                m_rAEOutput.rPreviewMode.u2FrameRate, m_rAEOutput.rPreviewMode.i2FlareOffset, m_rAEOutput.rPreviewMode.i2FlareGain);
            MY_LOG("[doPreCapAE] AF Mode Exp mode:%d Shutter:%5d Sensor gain:%4d Isp gain:%4d ISO:%4d frame rate:%3d flare offset/gain:%d %d\n",
                m_rAEOutput.rAFMode.u4ExposureMode, m_rAEOutput.rAFMode.u4Eposuretime,
                m_rAEOutput.rAFMode.u4AfeGain, m_rAEOutput.rAFMode.u4IspGain, m_rAEOutput.rAFMode.u4RealISO,
                m_rAEOutput.rAFMode.u2FrameRate, m_rAEOutput.rAFMode.i2FlareOffset, m_rAEOutput.rAFMode.i2FlareGain);
            MY_LOG("[doPreCapAE] Capture Exp mode:%d Shutter:%5d Sensor gain:%4d Isp gain:%4d ISO:%4d frame rate:%3d flare offset/gain:%d %d\n",
...
MRESULT AeMgr::updateSensorbyI2C()
{
    ...
    if ((m_eAETargetMode == AE_MODE_AOE_TARGET) || (m_eAETargetMode == AE_MODE_MVHDR_TARGET))
    {
        // mVHDR / iVHDR sensor control
        if (m_bSetShutterValue || m_bSetGainValue)
        {
...
MRESULT AeMgr::updateAEInfo2ISP(AE_STATE_T eNewAEState, MUINT32 u4ISPGain)
{
    ...
    switch (eNewAEState)
    {
        ...
        case AE_MANUAL_FRAMERATE_STATE:
        case AE_AF_RESTORE_STATE:
            MY_LOG_IF(m_3ALogEnable, "[%s()] Preview init or auto\n", __FUNCTION__);
            rAEInfo2ISP.u4Eposuretime = m_rAEOutput.rPreviewMode.u4Eposuretime;
            rAEInfo2ISP.u4AfeGain = m_rAEOutput.rPreviewMode.u4AfeGain;
            ...
            break;
        case AE_AF_STATE:
            MY_LOG_IF(m_3ALogEnable, "[%s()] AEAF\n", __FUNCTION__);
        ...
        case AE_CAPTURE_STATE:
        case AE_POST_CAPTURE_STATE:
            MY_LOG_IF(m_3ALogEnable, "[%s] Capture\n", __FUNCTION__);
            if (m_pIAeAlgo != NULL)
            {
            ...
            break;
        case AE_SENSOR_PER_FRAME_STATE :
            MY_LOG_IF(m_3ALogEnable, "[%s()] perframe\n", __FUNCTION__);
        ...
        case AE_PRE_CAPTURE_STATE:
        case AE_SENSOR_PER_FRAME_CAPTURE_STATE:
            MY_LOG("[%s] Nothing to do State:%d\n", __FUNCTION__, eNewAEState);
            return S_AE_OK;
    }

    if (u2FrameRate > m_i4AEMaxFps)
    {
        MY_LOG_IF(m_3ALogEnable, "[%s] Frame rate is too large:%d %d\n", __FUNCTION__, u2FrameRate, m_i4AEMaxFps);
        u2FrameRate = m_i4AEMaxFps;
    }
    ...
    {
        m_rSensorCurrentInfo.u8FrameDuration = 10000000000L / u2FrameRate;
        if (m_rSensorCurrentInfo.u8FrameDuration < rAEInfo2ISP.u4Eposuretime * 1000L)
        {
            m_rSensorCurrentInfo.u8FrameDuration = rAEInfo2ISP.u4Eposuretime * 1000L;
        }
    }
    else
    {
    ...
    // IspTuningMgr::getInstance().setAEInfo((ESensorDev_T)m_eSensorDev, rAEInfo2ISP);
    IspTuningMgr::getInstance().setAEInfo(rAEInfo2ISP);
    if (m_i4AEidxCurrent < 0)
        m_i4AEidxCurrent = m_i4AEidxNext;
    MY_LOG("[%s] State:%d Index:Now(%d) Next(%d) LV:%d Exp/Afe/Isp/ISO:%d/%d/%d/%d Flare offset/gain:%d/%d FrameDuration:%lld\n", __FUNCTION__, eNewAEState,
        m_i4AEidxCurrent, m_i4AEidxNext, rAEInfo2ISP.i4LightValue_x10,
        rAEInfo2ISP.u4Eposuretime, rAEInfo2ISP.u4AfeGain, rAEInfo2ISP.u4IspGain, rAEInfo2ISP.u4RealISOValue,
        rAEInfo2ISP.i2FlareOffset, i2FlareGain, m_rSensorCurrentInfo.u8FrameDuration);
    return S_AE_OK;
}
...
MRESULT AeMgr::monitorAndReschedule(MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply)
{
    ...
    if (bAAASchedule)
    {
        ...
        *bAECalc = MFALSE;
        *bAEApply = MFALSE;
        MY_LOG_IF(m_3ALogEnable, "[%s()] Calc:%d Apply:%d bAAASchedule:%d Stable:%d AEMonitorStable:%d 4AvgYcur:%d u4AvgYpre:%d\n",
            __FUNCTION__, *bAECalc, *bAEApply, bAAASchedule,
            m_bAEStable, m_bAEMonitorStable, u4AvgYcur, u4AvgYStable);
        return 0;
    }
    ...
    */
    ...
    {
        if ((bReschedule) && (m_i4WaitVDNum > (m_i4IspGainDelayFrames + 1)))
        {
            // (touchAE || Face || statchange) when stable
            m_u4AEScheduleCnt = 0;
        }
        else if ((bReschedule) && (m_i4WaitVDNum <= (m_i4IspGainDelayFrames + 1)))
        {
            // (touchAE || Face || statchange) when stable
    ...
    if (m_bFirstFrameAfterAEInit == MTRUE)
    {
        if(m_bAELock) *bAECalc = MTRUE;
        else *bAECalc = MFALSE;
        *bAEApply = MTRUE;
        m_bFirstFrameAfterAEInit = MFALSE;
    }

    MY_LOG("[%s()] Calc:%d Apply:%d Cnt:%d Frame:%d frameDelay:%d Exit:%d Stable:%d AEMonitorStable:%d u4AvgYcur:%d u4AvgYpre:%d\n",
        __FUNCTION__, *bAECalc, *bAEApply, m_u4AEScheduleCnt, AEScheduleFrame, i4framePerAECycle, bAEEXit, m_bAEStable, m_bAEMonitorStable, u4AvgYcur, u4AvgYStable);
    MY_LOG_IF(m_3ALogEnable, "[%s()] ReSchedule:%d VdCnt:%d ISPdelay:%d bStatChange:%d FaceArea:%d TouchArea:%d\n",
        __FUNCTION__, bReschedule, m_i4WaitVDNum, m_i4IspGainDelayFrames, bStatChange, m_bFaceAEAreaChage, m_bTouchAEAreaChage);
