...
#if 0
#define ISP_FUNC_VRB(fmt, arg...)    CAM_LOGV("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_FUNC_DBG(fmt, arg...)    CAM_LOGD("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_FUNC_INF(fmt, arg...)    CAM_LOGI("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_FUNC_WRN(fmt, arg...)    CAM_LOGW("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_FUNC_ERR(fmt, arg...)    CAM_LOGE("[%s] " fmt, __FUNCTION__, ##arg)
#else
...
//    memset (&rawCentralCtrl, 0, sizeof(ISP_RAW_CENTRAL_CTRL_INFO));
    ...
    ISP_FUNC_WRN("g_pMdpMgr_FrmB(0x%lx)", (unsigned long)g_pMdpMgr_FrmB);
    ...
    */
    ...
    if(g_pMdpMgr_FrmB)
    {
        ...
        if(MDPMGR_STILL_USERS != mdpreturn)
        {
            ...
            g_pMdpMgr_FrmB = NULL;
            ISP_FUNC_WRN("+ destroy g_pMdpMgr_FrmB(0x%lx)", (unsigned long)g_pMdpMgr_FrmB);
        }
        else
        {
            ISP_FUNC_WRN("still users");
        }
    }
    else
    {
...
controlBurstQueBuffer(EIspControlBufferCmd cmd, MUINT32 burstQueueNum)
{
    ...
    if(eIspControlBufferCmd_Alloc == cmd) {
        ...
        m_pVirtIspDrv=(NSIspDrv_FrmB::IspDrv**)malloc(totalCQNum*sizeof(NSIspDrv_FrmB::IspDrv *));
        if(m_pVirtIspDrv==NULL)
        {
...
int ISP_TOP_CTRL::_config( void )
{
    ...
    ISP_FUNC_INF("ISP_TOP_CTRL:E");
    if(this->path == ISP_PASS1)
        sprintf(_str,"pass1");
    else
        sprintf(_str,"pass2");

    ISP_FUNC_INF("%s:path(%d),CQ/dup/burst(%d/%d/%d),en1(0x%08x),en2(0x%08x),dma(0x%08x),fmt(0x%08x),ctl(0x%08x),isIspOn(0x%x)", \
                                                              _str,\
                                                              this->path, \
                                                              this->CQ, \
                                                              this->dupCqIdx, \
                                                              ...
                                                              this->isIspOn);
    ISP_FUNC_INF("int_en(0x%08x),intb_en(0x%08x),intc_en(0x%08x),dma_int(0x%x),dmab_int(0x%x),dmac_int(0x%x),muxS2(0x%x)", \
                                                          this->ctl_int.int_en, \
    ...
    ISP_FUNC_INF("m_pIspDrv(0x%x) m_pPhyIspDrv(0x%x),_targetVirDrv(0x%x),m_pP2IspDrv(0x%x)",m_pIspDrv,m_pPhyIspDrv,_targetVirDrv,m_pP2IspDrv);
...
//if defined _rtbc_use_cq0c_, CQ0C_EN need to be enabled
    ...
    ISP_FUNC_INF("pass1: int_en:0x%x,intb_en:0x%x,intc_en:0x%x,dma_int:0x%x,dmab_int:0x%x,dmac_int:0x%x\n",\
        this->ctl_int.int_en,\
        this->ctl_int.intb_en,\
        this->ctl_int.intc_en,\
        this->ctl_int.dma_int,\
    ...
    ISP_FUNC_INF("pass1: int_en:0x%x,intb_en:0x%x,intc_en:0x%x,dma_int:0x%x,dmab_int:0x%x,dmac_int:0x%x,en2:0x%x\n", \
    ...
    if(ISP_PASS1 == this->path)
    {
        ...
        ISP_FUNC_INF("pass1:after clr: en1:0x%x,en2:0x%x,dma_en:0x%x,fmt_sel:0x%x,ctl_sel:0x%x,mux_sel:0x%x,mux_sel2:0x%x,mux_cfg:0x%x, pix_id:0x%x\n", \
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    if (ISP_PASS1 == this->path)
    {
        ...
        int start_bit = ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv, CAM_CTL_START);
        ISP_FUNC_INF("_isable:: start bit = %d", start_bit);
        if (start_bit != CAM_ISP_PASS2_START
                && start_bit != CAM_ISP_PASS2B_START
                && start_bit != CAM_ISP_PASS2C_START) {
            //C.K. add to clear all DMA when disable
            ISP_FUNC_INF("_isable:: clear DMA");
            ISP_WRITE_BITS(this->m_pPhyIspDrv , CAM_CTL_DMA_EN_CLR, LSCI_EN_CLR, 1,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(this->m_pPhyIspDrv, CAM_CTL_DMA_EN_CLR, 0, ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(this->m_pPhyIspDrv, CAM_CTL_DMA_EN, 0, ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(this->m_pPhyIspDrv, CAM_CTL_DMA_EN_SET, 0, ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(this->m_pPhyIspDrv, CAM_CTL_EN1_SET, 0, ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(this->m_pPhyIspDrv, CAM_CTL_EN2_SET, 0, ISP_DRV_USER_ISPF);
        }
        ...
        ISP_FUNC_INF("[TopCtrl:Disable]P1_en(0x%x/0x%x),FBC(0x%x/0x%x)", \
            ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAM_CTL_EN1),\
            ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAM_CTL_DMA_EN),\
            ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAM_CTL_IMGO_FBC),\
            ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAM_CTL_IMG2O_FBC));
...
int ISP_TOP_CTRL::_write2CQ(void)
{
    ...
    if(this->path == ISP_PASS1){
        for(int i=0;i<ISP_DRV_P1_CQ_DUPLICATION_NUM;i++){
            ...
            this->m_pIspDrvShell->cqAddModule((NSIspDrv_FrmB::ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,i,(NSIspDrv_FrmB::CAM_MODULE_ENUM)CAM_P1_MAGIC_NUM);
        }
    }else{
        ...
        if(_eis_cfg.bEIS_Bypass == 0){
            ...
            if(_eis_cfg.bEIS_EN){
                ...
                if(!_targetVirDrv->getIspCQModuleInfo(CAM_ISP_EIS, outInfo))
                    ISP_FUNC_ERR("[ERROR] CAM_ISP_EIS");
...
waitBufReady( MUINT32 dmaChannel )
{
    ...
    if(_imgo_ == rt_dma || _img2o_ == rt_dma)
    {
        ...
        ISP_FUNC_INF("rtBufCtrl.ctrl(%d)/id(%d)/ptr(0x%p)",buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.pExtend);
...
EIspRetStatus ISP_BUF_CTRL::dequeueHwBuf( MUINT32 dmaChannel, stISP_FILLED_BUF_LIST& bufList )
{
    ...
    if ( ISP_PASS1   == this->path) {
        ...
        if ( _imgo_ == rt_dma || \
            ...
            P1DeQCnt[rt_dma] -= 1;
            if( (_imgo_ == rt_dma) || (_img2o_ == rt_dma) ){
                ISP_QUE_INF(rt_dma,"imgo/img2o: vf=0x%x, fbc: 0x%x/0x%x_0x%x/0x%x\n",vf,fbc[0],fbc[1],readReg(0x000040f4),readReg(0x000040f8));
            }
            //
            count = deque_buf.count;
            //ISP_FUNC_DBG("deque_buf.count(%d)",deque_buf.count);
...
int ISP_PASS1_CTRL::setP1MagicNum( MUINT32 pass1_path, MUINT32 MagicNum, MUINT32 magicNumCnt)
{
    ...
    if ( ISP_PASS1 == pass1_path ) {
        ...
        ISP_FUNC_INF("setP1MagicNum idx[%d][0x%x]", _targetDupQIdx,MagicNum);
    }
    ...
    if ( hrz_cfg.isTwinMode) {
        ...
        return -1;
            ...
            */
            ...
            //resize = 32768*10/10;
#if 1 //hrz patch on 6582, 32768*(in_width-1)*(out_width-1)
            resize = (((hrz_cfg.hrz_in_size.w-1) << 15)+((hrz_cfg.hrz_out_size.w-1)/2))/
                        (hrz_cfg.hrz_out_size.w-1);
            oSize = hrz_cfg.hrz_out_size.w;
#else
...
class IspEventThreadImp : public IspEventThread
{
    ...
    {
        mInitCount=0;
        memset(&m_ispEventthread, 0, sizeof(pthread_t));
    }
    //
