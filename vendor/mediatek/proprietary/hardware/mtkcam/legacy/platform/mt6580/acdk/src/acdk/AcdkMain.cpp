...
#define MEDIA_PATH "/data/vendor/camera_dump"
...
AcdkMain::AcdkMain ()
    ...
    ,mSensorModeNum(0)
#endif
    ,ExtraSizeForImgi(L1_CACHE_BYTES)
    ,PureRaw_BufAddr(0)
    ,Capture_BufAddr(0)
    ,Capture_Size(0)
    ,mOperationMode(0)
    ...
    ,mCctOn(MFALSE)
{
    ...
    memset(&sensorDynamicInfo, 0, sizeof(sensorDynamicInfo));
...
MINT32 AcdkMain::init()
{
    ...
    mpIHal3A = Hal3ABase::createInstance(mSensorDev);
        ...
        {
            ACDK_LOGD("3A Hal::setParams() fail - err(%x)",mpIHal3A->getErrorCode());
        }
#if FIXED ==1
...
MINT32 AcdkMain::startPreview(Func_CB prvCb)
{
    ...
    //====== Allocate Memory ======
    /* Add extra size for IMGI read to prevent TF */
    sensorImgSize += this->ExtraSizeForImgi;
    createMemBuf(sensorImgSize, OVERLAY_BUFFER_CNT, mPrvIMemInfo);      // pass1 - IMGO Out
    /* Hide dummy extra size */
    sensorImgSize -= this->ExtraSizeForImgi;
    for (MUINT32 i = 0; i < OVERLAY_BUFFER_CNT; i++) {
        mPrvIMemInfo[i].size -= this->ExtraSizeForImgi;
    }
    ...
    if(1)
    {
        ...
        setDimension(mode, acdkCapInfo.imgBufInfo.imgWidth, acdkCapInfo.imgBufInfo.imgHeight);
            ...
            else
            {
                ...
                switch(mSensorFormatInfo.u4BitDepth) {
                case 0: //eImgFmt_BAYER8
                    acdkCapInfo.RAWImgBufInfo.imgSize =
                        acdkCapInfo.RAWImgBufInfo.imgWidth*acdkCapInfo.RAWImgBufInfo.imgHeight;
                    break;
                case 1: //eImgFmt_BAYER10
                    acdkCapInfo.RAWImgBufInfo.imgSize =
                        (acdkCapInfo.RAWImgBufInfo.imgWidth*acdkCapInfo.RAWImgBufInfo.imgHeight*10)/8;
                    break;
                case 2: //eImgFmt_BAYER12
                    acdkCapInfo.RAWImgBufInfo.imgSize =
                        (acdkCapInfo.RAWImgBufInfo.imgWidth*acdkCapInfo.RAWImgBufInfo.imgHeight*3)/2;
                    break;
                default:
                    acdkCapInfo.RAWImgBufInfo.imgSize = 1;
                    ACDK_LOGE("Unsupported pixel depth: %d", mSensorFormatInfo.u4BitDepth);
                    break;
                }
                ...
                MINT32  const isSaveImg)
{
    ...
    if(capCb != NULL)
    {
        ...
        if(mCapType & 0x3C)
        {
            ...
            if(mCctOn)
            {
                ACDK_LOGD("Copy Raw from /data/vendor/camera_dump to /mnt/vendor/cct for CCT: %s", szFileName_cct);
