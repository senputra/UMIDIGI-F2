...
)
{
    ...
    encode_frame* pEncodeFrame = NULL;
    {
        ...
        if( ret == NAME_NOT_FOUND ) {
            ...
            MY_LOGE("MYL ret == NAME_NOT_FOUND");
    ...
    while( pEncodeFrame->mpJpeg_Main.get() == NULL ) {
        ...
        {
            MY_LOGE("MYL get src buffers & internal dst buffer for bitstream");
        ...
        //IImageBuffer*          pWorkingJpeg         = NULL;
        //
        MERROR const err = getImageBufferAndLock(
                pFrame,
                stream_in,
        ...
        //dump yuv before encode
        {
            if (enableDump) {
                char filename[256];
                sprintf(filename, "/data/vendor/camera_dump/main_%d_%d_%d_%d.yuv", pFrame->getFrameNo(),
        ...
        //  udpate working buffer
        //pEncodeFrame->mpJpeg_Main = pWorkingJpeg;
        // 3. end
        {
    ...
    while( pEncodeFrame->mbHasThumbnail && pEncodeFrame->mpJpeg_Thumbnail.get() == NULL ) {
        ...
        // get working buffer
        //pWorkingJpeg = getJpegBuffer(MFALSE, thumbsize );
        //if( pWorkingJpeg == NULL ) {
        //    MY_LOGE("no working buffer");
        //    return;
        //}
        // do encode
        {
            MY_LOGD("mpJpeg_Thumbnail: mFlip=%d, flipMode=%d, orientation=%d", mFlip,
                    pEncodeFrame->mParams.flipMode, pEncodeFrame->mParams.orientation);
            //
            my_encode_params params;
            params.pSrc = pInImageBuffer.get();
        ...
        // dump thumbnail yuv
        {
            if (enableDump) {
                char filename[256];
                sprintf(filename, "/data/vendor/camera_dump/thumbnail_%d_%d_%d_%d.yuv", pFrame->getFrameNo(),
                    pInImageBuffer->getImgSize().w,
                    pInImageBuffer->getImgSize().h,
                    pEncodeFrame->mParams.orientation);
                NSCam::Utils::saveBufToFile(filename, (unsigned char*)pInImageBuffer->getBufVA(0), pInImageBuffer->getBufSizeInBytes(0));
        ...
        //  udpate working buffer
        //pEncodeFrame->mpJpeg_Thumbnail = pWorkingJpeg;
...
)
{
    if( mpCurEncFrame ) {
        ...
        MY_LOGE("myl frameNo cur %u ; frameNo %u",
                    mpCurEncFrame->mpFrame->getFrameNo(), frameNo);
...
)
{
    ...
    // get out buffer
    {
        ...
        sp<IImageBuffer>       pOutImageBuffer     = NULL;
        /*StreamId_T const streamId =  mpOutJpeg->getStreamId();
        MERROR const err = getImageBufferAndLock(
                pFrame,
                streamId,
                pOutImgStreamBuffer,
        ...
        // pack the jpeg bitstream to pOutImageBuffer
        if( OK != packJpeg(rpEncodeFrame, pOutImageBuffer.get()) ) {
    ...
    /*returnJpegBuffer(MTRUE, rpEncodeFrame->mpJpeg_Main);
    if( rpEncodeFrame->mpJpeg_Thumbnail )
        returnJpegBuffer(MFALSE, rpEncodeFrame->mpJpeg_Thumbnail);*/
}
...
)
{
    ...
    if( pOutImageBuffer == NULL ) {
        MY_LOGE("pOutImageBuffer == NULL");
        return BAD_VALUE;
    }

#if 0
    // out buffer
    size_t const jpegBufSize = pOutImageBuffer->getBufSizeInBytes(0);
    size_t jpegsize_final    = 0;
    MINT8* pJpegBuf = reinterpret_cast<MINT8*>(pOutImageBuffer->getBufVA(0));
    if( pJpegBuf == NULL ) {
        MY_LOGE("cannot get dst jpeg va");
        return BAD_VALUE;
    }
    ...
    if( rpEncodeFrame->mpJpeg_Main.get() ) {
        pMainJpegAddr = reinterpret_cast<void*>(rpEncodeFrame->mpJpeg_Main->getBufVA(0));
        mainJpegSize = rpEncodeFrame->mpJpeg_Main->getBitstreamSize();
    }
    if( pMainJpegAddr == NULL || mainJpegSize == 0 ) {
        MY_LOGE("main jpeg IImageBuffer %p, buf va %p size %zu",
                rpEncodeFrame->mpJpeg_Main.get(),
                pMainJpegAddr, mainJpegSize
                );
        return BAD_VALUE;
    }
#endif
...
//#define EXIF_HEADER_SIZE   (0xFFFF*4)
    // create exif header
    size_t exifSize  = rpEncodeFrame->exif.getHeaderSize();//EXIF_HEADER_SIZE
    //MY_LOGD("@@@exifSize %zu", exifSize);
...
#if 0
    if( exifSize + rpEncodeFrame->mpJpeg_Main->getBitstreamSize() + sizeof(camera3_jpeg_blob) >= jpegBufSize ) {
        MY_LOGE("jpeg dst buffer size not enough %zu + %zu + %zu >= %zu",
                exifSize,
    ...
    // 3. end
    {
        ...
        //
        memcpy(pJpegBuf + jpegBufSize - sizeof(camera3_jpeg_blob),
                &jpeg_end,
                sizeof(camera3_jpeg_blob)
              );
    }
...
lbExit:
    MY_LOGD("frame %u: exif header buf %p, size %zu",
    ...
    /*MY_LOGD("frame %u: header %d@%p, jpeg %d@%p final: %d@%p bufsize %zu",
            rpEncodeFrame->mpFrame->getFrameNo(),
            exifSize, pExifBuf,
            mainJpegSize, pMainJpegAddr,
            jpegsize_final, pJpegBuf, jpegBufSize);*/
...
)
{
    ...
    if(NULL == pImageBufferHeap.get()){
        MY_LOGE("myl NULL == pImageBufferHeap.get()");
        ...
        MY_LOGD("myl nodeID %#" PRIxPTR " streamID %#" PRIx64 " ",getNodeId(), streamId);
        ...
        updateStdExifParam(
                mpInYuv_main->getImgSize(),
                rpEncodeFrame->pInMeta_Hal,
                rpEncodeFrame->mParams,
                stdParams
                );
        //
        //rpEncodeFrame->exif.setExifParams(stdParams);
    ...
    else
    {
        ...
        MY_LOGD("myl mainOffset = %d, mainMaxSize = %d, mainMaxSize = %zu", mainOffset, mainMaxSize, mainMaxSize);

        size_t const bufStridesInBytes[3] = {mainMaxSize, 0 ,0};
        rpImageBuffer = pImageBufferHeap->createImageBuffer_FromBlobHeap(
                mainOffset, eImgFmt_JPEG, mpInYuv_main->getImgSize(), bufStridesInBytes
                );
        if (rpImageBuffer == NULL) {
