...
MBOOL IMemDrvImp::init(void)
{
   ...
   ref_cnt.data_ptr = (MINT32*)&mInitCount;
    ...
    //
        ...
        if(mInitCount==1)
           {
              mpM4UDrv->m4u_enable_m4u_func(M4U_PORT_IMGI);
              mpM4UDrv->m4u_enable_m4u_func(M4U_PORT_IMGO);
              mpM4UDrv->m4u_enable_m4u_func(M4U_PORT_IMG2O);
              mpM4UDrv->m4u_enable_m4u_func(M4U_PORT_LSCI);
              mpM4UDrv->m4u_enable_m4u_func(M4U_PORT_ESFKO);
              mpM4UDrv->m4u_enable_m4u_func(M4U_PORT_AAO);
              //
...
MBOOL IMemDrvImp::uninit(void)
{
    ...
    if ( mLocal_InitCount <= 0 )
    {
       ...
       if(mInitCount<=0)
       {
           ...
           port.ePortID = M4U_PORT_AAO;
           ret = mpM4UDrv->m4u_config_port(&port);
           mpM4UDrv->m4u_disable_m4u_func(M4U_PORT_IMGI);
           mpM4UDrv->m4u_disable_m4u_func(M4U_PORT_IMGO);
           mpM4UDrv->m4u_disable_m4u_func(M4U_PORT_IMG2O);
           mpM4UDrv->m4u_disable_m4u_func(M4U_PORT_ESFKO);
           mpM4UDrv->m4u_disable_m4u_func(M4U_PORT_AAO);
       }
       delete mpM4UDrv;
       mpM4UDrv = NULL;
    }
...
#endif
    IMEM_INF("mID[0x%x]/size[0x%x]/VA[0x%x]",pInfo->memID,pInfo->size,pInfo->virtAddr);  //kk test
    return 0;
}
//-----------------------------------------------------------------------------
...
//#elif   defined(__ISP_USE_STD_M4U__) || defined (__ISP_USE_ION__)
        ...
        if ( IMEM_MIN_ION_FD > pInfo->memID ) {
            ...
            mm_data.config_buffer_param.eModuleID = M4U_PORT_IMGI;////seanlin for 6582M4U_CLNTMOD_IMG;    // 1;
    ...
    void)
{
    ...
    mpM4UDrv->m4u_cache_flush_all(M4U_PORT_IMGI);////seanlin for 6582 M4U_CLNTMOD_IMG);
#endif
...
#if defined(__ISP_USE_STD_M4U__) || defined (__ISP_USE_ION__)
    MINT32 err=0;
    IMEM_DBG("+, c/m/va/sz/pa(%d/0x%x/0x%x0x%x/0x%x)",ctrl,pInfo->memID,pInfo->virtAddr,pInfo->size,pInfo->phyAddr);
    ...
    if( IMEM_MIN_ION_FD > pInfo->memID)
    {
        switch(ctrl)
        {
            case IMEM_CACHECTRL_ENUM_FLUSH:
                mpM4UDrv->m4u_cache_sync(M4U_PORT_IMGI,M4U_CACHE_FLUSH_BEFORE_HW_WRITE_MEM,pInfo->virtAddr,pInfo->size);
                break;
            case IMEM_CACHECTRL_ENUM_INVALID:
                mpM4UDrv->m4u_cache_sync(M4U_PORT_IMGI,M4U_CACHE_INVALID_AFTER_HW_WRITE_MEM,pInfo->virtAddr,pInfo->size);
                break;
            default:
    ...
    MINT32      memID)
{
    ...
    M4U_MODULE_ID_ENUM eM4U_ID = M4U_PORT_IMGI;////seanlin for 6582M4U_CLNTMOD_IMG;
    ...
    MINT32      memID)
{
    ...
    ret = mpM4UDrv->m4u_invalid_tlb_range(
            M4U_PORT_IMGI,////seanlin for 6582M4U_CLNTMOD_IMG,
            m4uVa,
            m4uVa + size-1);
