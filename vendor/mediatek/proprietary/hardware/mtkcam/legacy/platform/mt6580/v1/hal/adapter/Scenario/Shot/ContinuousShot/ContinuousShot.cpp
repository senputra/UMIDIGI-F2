...
)
{
    ...
    //          N/A
    case eCmd_capture:
        ::sem_init(&semMEnableCancelShot, 0, 0);
        ret = onCmd_capture();
        break;
...
#warning "[TODO] ContinuousShot::onCmd_cancel()"
...

    {
        ...
        else if(mpMultiShot != NULL)
        {
            MY_LOGD("stop MultiShot");
            mpMultiShot->stop();
    ...
    //case : mpMultiShot == NULL
    MY_LOGD("MultiShot not created, wait first shot done");
    ::sem_wait(&semMEnableCancelShot);
...
handleJpegData(MUINT8* const puJpegBuf, MUINT32 const u4JpegSize, MUINT8* const puThumbBuf, MUINT32 const u4ThumbSize)
{
    ...
    if(mu4ShotConut == 0)
    {
        MY_LOGD("first jpeg done, enable cancel shot");
        ::sem_post(&semMEnableCancelShot);
    }
