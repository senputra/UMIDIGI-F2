...
#define ACDK_PURE_SMT
#define ACDK_RAINIER_SMT
#if 0 //32bit only
typedef int             CPT_ARG_T;
typedef unsigned int    CPT_PA_T;
typedef unsigned int    CPT_VA_T;
#else //64bit
//typedef MUINTPTR        CPT_ARG_T;
//typedef MUINTPTR        CPT_PA_T;
//typedef MUINTPTR        CPT_VA_T;
#endif
...
//#define ACDK_LOGD(fmt,...)      ALOGD("[%s]" fmt, __FUNCTION__, ##__VA_ARGS__)
//#define ACDK_INFO(fmt,...)      ALOGI("[%s]" fmt, __FUNCTION__, ##__VA_ARGS__)
//#define ACDK_WARN(fmt,...)      ALOGW("[%s]" fmt, __FUNCTION__, ##__VA_ARGS__)
//#define ACDK_LOGE(fmt,...)      ALOGE("[%s]Err" fmt, __FUNCTION__, ##__VA_ARGS__)

//#define ACDK_LOGD(fmt,arg...)      ALOGD(fmt, ##arg)
//#define ACDK_INFO(fmt,arg...)      ALOGI(fmt, ##arg)
//#define ACDK_WARN(fmt,arg...)      ALOGW(fmt, ##arg)
//#define ACDK_LOGE(fmt,arg...)      ALOGE("{#%d:%s}Err : " fmt,__LINE__,__FILE__, ##arg)

#define ACDK_LOGD(fmt,arg...)      ALOGD("[%s] " fmt, __func__, ##arg)
#define ACDK_INFO(fmt,arg...)      ALOGI("[%s]INF(%5d):" fmt, __func__, __LINE__, ##arg)
#define ACDK_WARN(fmt,arg...)      ALOGW("[%s]WRN(%5d):" fmt, __func__, __LINE__, ##arg)
#define ACDK_LOGE(fmt,arg...)      ALOGE("{#%d:%s}Err : " fmt,__LINE__,__FILE__, ##arg)
...
#define ACDK_ASSERT(x, str); \
        if (x) {} \
            ...
            printf("[Assert %s, %d]: %s", __FILE__, __LINE__, str); \
            ALOGE("[Assert %s, %d]: %s", __FILE__, __LINE__, str); \
            while(1) { \
                usleep(500 * 1000); \
            } \
        }
