...
MVOID AfMgr::setAF_IN_HSIZE()
{
    ...
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_IN_SIZE, AF_IN_HSIZE, Boundary(0, m_i4AF_in_Hsize, 8191));
...
MVOID AfMgr::setAFConfig(AF_CONFIG_T a_sAFConfig)
{
    ...
    // Q0.7.4
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_SGG_PGN, SGG_GAIN, Boundary(0, a_sAFConfig.i4SGG_GAIN, 2047));
    // 8 bits
    ...
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_DECI_1, Boundary(0, a_sAFConfig.AF_DECI_1, 2));  // 1: 1/2 (double buffer)
    ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_ZIGZAG, Boundary(0, a_sAFConfig.AF_ZIGZAG, 1));  // 1: shift
    if ((i4Pix_Id == 1) || (i4Pix_Id == 2))
    {
        ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_ODD, 1);  // 1: odd
    }
    else
    {
        ISP_IOCTL_WRITE_BITS(m_pIspDrv, m_pIspReg, CAM_AF_CON, AF_ODD, 0);  //0: even
    }
