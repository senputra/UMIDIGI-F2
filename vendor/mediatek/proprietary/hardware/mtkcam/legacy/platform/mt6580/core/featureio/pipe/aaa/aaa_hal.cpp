...
Hal3A::Hal3A()
    ...
    , semAFProcEnd()
    , mSemAFNotify()
    , bEnAFNotifyThreadLoop(0)
    , bAfNeedSingleNotify(0)
{
    ...
    if (eCmd == ECmd_Update)
    {
...

        {
            ...
            ::sem_post(&g_semAFThreadstart);
                MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[AFThread] sem_post g_semAFThreadstart, semAFProcEnd before wait = %d\n", Val);
            }
        }
/*
...
MVOID * Hal3A::AFThreadFunc(void *arg)
{
    ...
    while (g_bAFThreadLoop) {
        ...
        MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[Hal3A::AFThreadFunc] sem_wait g_semAFThreadstart done\n");
        ...
        if (g_bAFThreadLoop != 0)
        {
            ...
            MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[Hal3A::AFThreadFunc] sem_post(&mSemAFNotify)\n");
...

            {
                ...
                MY_LOG_IF(sm_3ALogEnable & EN_3A_FLOW_LOG, "[AFThread] semAFProcEnd before post = %d\n", Val2);
...
MVOID* Hal3A::AFNotifyThreadLoop(void *arg)
{
    ...
    while (_this->bEnAFNotifyThreadLoop)
    {
        ...
        MY_LOG("[%s] mSemAFNotify wait done, bEnAFNotifyThreadLoop(%d)\n", __FUNCTION__, _this->bEnAFNotifyThreadLoop);
        if (!_this->bEnAFNotifyThreadLoop) break;
        _this->sendAFCallbacks();
        MY_LOG("[%s] Call sendAFCallbacks done, bEnAFNotifyThreadLoop(%d)\n", __FUNCTION__, _this->bEnAFNotifyThreadLoop);
    }
    MY_LOG("[%s] thread detach, thread terminates - \n", __FUNCTION__);
    return NULL;
}
...
MVOID  Hal3A::sendAFCallbacks()
{
    ...
    if (bAfNeedSingleNotify)
    {
        ...
        //IAfMgr::getInstance().SingleAF_CallbackNotify(m_i4SensorDev); // avoid too many af callbacks
