...
createInstance(ESensorDev_T const eSensorDev, MINT32 i4SensorIdx, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
{
    ...
    switch (eSensorDev)
        ...
        case ESensorDev_Main:       //  Main Sensor
            _pLscMain = LscMgr2RtoCycle::createInstance(ESensorDev_Main, i4SensorIdx, ESensorTG_1, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_Main(0x%08x), i4SensorIdx(%d)", _pLscMain, i4SensorIdx);
            return _pLscMain;
        case ESensorDev_MainSecond: //  Main Second Sensor
            _pLscMain2 = LscMgr2RtoCycle::createInstance(ESensorDev_MainSecond, i4SensorIdx, ESensorTG_1, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_MainSecond(0x%08x), i4SensorIdx(%d)", _pLscMain2, i4SensorIdx);
            return _pLscMain2;
        case ESensorDev_Sub:        //  Sub Sensor
            _pLscSub = LscMgr2RtoCycle::createInstance(ESensorDev_Sub, i4SensorIdx, ESensorTG_1, rIspNvram);
            LSC_LOG_END("RTOC ESensorDev_Sub(0x%08x), i4SensorIdx(%d)", _pLscSub, i4SensorIdx);
            return _pLscSub;
        }
}
...
LscMgr2(ESensorDev_T eSensorDev, MINT32 i4SensorIdx, NVRAM_CAMERA_ISP_PARAM_STRUCT& rIspNvram)
    ...
    , m_rTsfState(MTKTSF_STATE_STANDBY)
    , m_i4RtoFor1stFrm(32)
{
    ...
    memset (&m_rSensorResolution, 0, sizeof(m_rSensorResolution));
    memset (&m_rCurSensorCrop, 0, sizeof(m_rCurSensorCrop));
    memset (&m_rSensorCropWin, 0, sizeof(m_rSensorCropWin));
    memset (&m_rSensorCropCfg, 0, sizeof(m_rSensorCropCfg));
    memset (&m_Sema, 0, sizeof(m_Sema));
    sem_init(&m_Sema, 0, 0);
    memset (&m_rTsfRefInfo, 0, sizeof(m_rTsfRefInfo));
    memset (&m_rTsfEnvInfo, 0, sizeof(m_rTsfEnvInfo));
    m_rTsfEnvInfo.TS_TS       = 1;
    m_rTsfEnvInfo.MA_NUM      = 5;
    m_rTsfEnvInfo.Raw16_9Mode = 2;
    memset (&m_rTsfGetEnvInfo, 0, sizeof(m_rTsfGetEnvInfo));
    memset (&m_rTsfSetProc, 0, sizeof(m_rTsfSetProc));
    m_rTsfSetProc.LscRA=32;
    memset (&m_rTsfResult, 0, sizeof(m_rTsfResult));
    memset (&m_rTsfGetProc, 0, sizeof(m_rTsfGetProc));
    memset (&m_rTsfGetLog, 0, sizeof(m_rTsfGetLog));
    memset (&m_rTsfLscParam, 0, sizeof(m_rTsfLscParam));
    memset (&m_rTsfTbl, 0, sizeof(m_rTsfTbl));
}
...
RawLscTblDump(const char* filename)
{
    ...
    GET_PROP("vendor.debug.lsc_mgr.dump123", "0", i4Dbg);
...
calLscConfig(const SensorCropInfo_T& rCrop, MUINT8 u1XNum, MUINT8 u1YNum, ISP_NVRAM_LSC_T& rLscCfg)
{
    ...
    // Martin fix: START -----------------------------------------------------------------------------------
        ...
        LSC_LOG("u1XNUM = %d, u1YNum = %d", u1XNum, u1YNum);
...
{
    MINT32 dbg_enable = 0;
    GET_PROP("vendor.debug.lsc_mgr.manual_tsf", "-1", dbg_enable);
