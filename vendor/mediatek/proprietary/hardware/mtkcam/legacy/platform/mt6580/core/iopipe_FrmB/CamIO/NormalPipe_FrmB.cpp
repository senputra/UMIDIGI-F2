...
#undef PIPE_ERR
#undef PIPE_AST
//Enable debug under user-debug and eng
#if 1
#define PIPE_VRB(fmt, arg...)    CAM_LOGV("[%s] " fmt, __FUNCTION__, ##arg))
#define PIPE_DBG(fmt, arg...)    CAM_LOGD("[%s] " fmt, __FUNCTION__, ##arg)
#define PIPE_INF(fmt, arg...)    CAM_LOGI("[%s] " fmt, __FUNCTION__, ##arg)
#define PIPE_WRN(fmt, arg...)    CAM_LOGW("[%s] " fmt, __FUNCTION__, ##arg)
...
NormalPipe_FrmB::enque(QBufInfo const& rQBuf)
{
    ...
    PIPE_INF("MyEnq: +, UserEnq:%d/ UserDeq:%d", mStatistics.numOfUserEnqued, mStatistics.numOfUserDequed);
    //
    ...
    if(this->m_bStarted == MFALSE){
        ...
        for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
        {
            ...
            rQBufInfo.vBufInfo[0].mBufIdx = this->m_BufIdx;
    ...
    //all enque request r all push into enquereqest , but enque before start have special control flow at enquerequest()
    {
        for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
        {
            ...
            //C.K. add ratio check
            MUINT32 scale_ratio_int_100 = (MUINT32)( (_buf.FrameBased.mDstSize.w*100) / _buf.FrameBased.mCropRect.s.w );
            if ( 25 > scale_ratio_int_100 ) {
                PIPE_ERR("dma(0x%x) param ratio error: min 0.25 (%d/%d)=%d%%"
                    ,_buf.mPortID.index
                    ,_buf.FrameBased.mDstSize.w
                    ,_buf.FrameBased.mCropRect.s.w
                    ,scale_ratio_int_100);
            }
...
NormalPipe_FrmB::checkDropEnque(QBufInfo const& rQBuf)
{
    ...
    if(this->m_PerFrameCtl && _sof_idx == this->mPrvEnqSOFIdx){
        ...
        if(_popbuf.mBuffer != NULL)
            ...
            PIPE_INF("_popbuf null dereferenced");
...
{
#if 0    //mw no imgo size change in ISP20 . this flow is for avoid translation fault.
    MUINT32 r_index = 0;
    MUINT32 ret = 0;
    MUINT32 arrayIdx = 0;
    if (portIdx == PORT_IMGO.index)
        arrayIdx = 0;
    else if (portIdx == PORT_IMG2O.index)
        arrayIdx = 1;
    else
        return ret;
...
NormalPipe_FrmB::deque(QBufInfo& rQBuf, MUINT32 u4TimeoutMs)
{
    ...
    PIPE_INF("MyDeq: +, UserEnq:%d/ UserDeq:%d", mStatistics.numOfUserEnqued, mStatistics.numOfUserDequed);
    bool ret = MTRUE;
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            if (ret_PA != 0)
                ...
                if (NULL == pframe ) {
                    ...
                    //C.K. modified to make deque balance
                    //return MFALSE;
                    ret = MFALSE;
                    continue;
                }
                MINT64 ts = rQTSBufInfo.vBufInfo.at(idx).getTimeStamp_ns();
                // work around to prevent NE.
                // calling incorrect function pointer cause program execute at illegal address(0x00000000)
                //if (NULL == *pframe || (rQTSBufInfo.vBufInfo.at(idx).m_num_0 & _UNCERTAIN_MAGIC_NUM_FLAG_)==0) {
                //    pframe->setTimestamp(ts);
                //} else {
                //    PIPE_ERR("skip setTimestamp with m(%d)", rQTSBufInfo.vBufInfo.at(idx).m_num_0);
                //}
#endif
                ...
                for(MUINT32 i=0;i<_size;i++){
                    if(MFALSE == this->mpDummyRequest[_PortMap(portID.index)]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_at,i,(MUINTPTR)&_buf))
                        ...
                        if(_buf.mBuffer != NULL){
                            ...
                            PIPE_ERR("error:_buf null dereferenced");
                    }
                }
                #if (HAL3_IPBASE == 1)
                ...
                this->mpEnDeCheck[_PortMap(portID.index)]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_getsize,
                    (MUINTPTR)&__size,0);
                if(__size != 0){
                    if (this->mpEnDeCheck[_PortMap(portID.index)]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_front,
                        (MUINTPTR)&_EnDebuf, 0) == MFALSE) {
                        PIPE_ERR("error: mpEnDeCheck fail: port:0x%x\n", portID.index);
                    } else {
                        ...
                        if (_EnDebuf.mPa == buff.mPa) {
                            if (_EnDebuf.FrameBased.mMagicNum_tuning != result.mMagicNum_hal) {
                                PIPE_INF("drop this unknown frame back to MW(0x%x_0x%x_0x%x_0x%x)\n",_EnDebuf.mPa,buff.mPa,_EnDebuf.FrameBased.mMagicNum_tuning,result.mMagicNum_hal);
                                buff.mMetaData.m_bDummyFrame = MTRUE;
                                if (this->m_DropCB)
                                    this->m_DropCB(_EnDebuf.FrameBased.mMagicNum_tuning, this->m_returnCookie);
                            }
                            /* pop mpEnDeCheck if PA is match*/
                            this->mpEnDeCheck[_PortMap(portID.index)]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_pop,
                                0,0);
                        }
                    ...
                    //this log will be error if frame is not 1st 3 frames
                    PIPE_WRN("logic error , size shouldn't be 0\n");
                }
                ...
                if(buff.mMetaData.m_bDummyFrame == MTRUE){
                    ...
                    //
                    PIPE_INF("MyDeq: dummy frame: dma(0x%x) pa(0x%x) \
                    io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x) UserEnq:%d/ UserDeq:%d\n",
                    portID.index,\
                    buff.mPa,\
                    ...
                    mStatistics.numOfUserEnqued, mStatistics.numOfUserDequed);
                    ret = MFALSE;
                }
                else{
                    //avoid apply error m# due to bad system performance (ex. enque EnqueRequest over VYSNC)
                    ...
                    //
                    PIPE_INF("MyDeq: dma(0x%x) pa(0x%x) \
                    io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x) UserEnq:%d/ UserDeq:%d\n",
                    portID.index,\
                    buff.mPa,\
                    result.mCrop_s.p.x,result.mCrop_s.p.y,result.mCrop_s.s.w,result.mCrop_s.s.h, \
                ...
                if(buff.mPa != real_u4BufPA){
                    PIPE_WRN("real PA != PA deque from imgageio(replace buffer flow)\n",buff.mPa,real_u4BufPA);
                }
                buff.mPa = real_u4BufPA;
                #endif
...
NormalPipe_FrmB::Reset()
{
    ...
    this->mpHalSensor->configure(this->mpEnabledSensorCnt, &(this->m_sensorConfigParam[0]));
...
NormalPipe_FrmB::configPipe(QInitParam const& vInPorts)
{
    ...
    memset(this->m_sensorConfigParam, 0, sizeof(IHalSensor::ConfigParam) * 4);
    ...
    for (int i = 0; i < mpEnabledSensorCnt; i++)  {
        ...
        this->m_sensorConfigParam[i] = vInPorts.mSensorCfg.at(i); //used for sensorCfg
    ...
    mpHalSensor->configure(mpEnabledSensorCnt, &(this->m_sensorConfigParam[0]));
    for (int i = 0; i < mpEnabledSensorCnt; i++) {
...
NormalPipe_FrmB_Thread::NormalPipe_FrmB_Thread(NormalPipe_FrmB* obj)
{
    ...
    m_EnqueCnt = 0;
    memset(&m_EnqueThread, 0, sizeof(pthread_t));
...
MVOID NormalPipe_FrmB_Thread::DummyFrameToEnque(MUINT32 _OpenedPort)
{
    ...
    if(_size != 0){
        ...
        for(int i=0;i<_OpenedPort;i++){
            ...
            if (this->m_pNormalpipe_FrmB->m_BufIdx >= this->m_pNormalpipe_FrmB->GetRingBufferDepth())
                this->m_pNormalpipe_FrmB->m_BufIdx = this->m_pNormalpipe_FrmB->m_BufIdx % this->m_pNormalpipe_FrmB->GetRingBufferDepth();
        ...
        this->m_pNormalpipe_FrmB->m_BufIdx = (this->m_pNormalpipe_FrmB->m_BufIdx + 1) % this->m_pNormalpipe_FrmB->GetRingBufferDepth();
    }
...
#if (HAL3_IPBASE == 1)
MVOID NormalPipe_FrmB_Thread::ReplaceToDummy(MUINT32 _OpenedIndex, BufInfo* pBuf, BufInfo* dumBuf, MUINT32* _doReplace)
{
#if 0    //mw no imgo size change in ISP20 . this flow is for avoid translation fault.
    NSCam::NSIoPipe::PortID port;
    ...
    *_doReplace = 0;
    ...
    if (((port.index == PORT_IMGO.index) && (pBuf->FrameBased.mCropRect.s.h < YsizeofSmallBuf)) ||
        ((port.index == PORT_IMG2O.index) && (pBuf->FrameBased.mDstSize.h < YsizeofSmallBuf))) {

        this->m_pNormalpipe_FrmB->mpDummyRequest[_PortMap(this->m_pNormalpipe_FrmB->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_Index,i))]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_getsize,
            (MUINTPTR)&_size,0);
        if (_size != 0) {
            *_doReplace = 1;
            this->m_pNormalpipe_FrmB->mpDummyRequest[_PortMap(this->m_pNormalpipe_FrmB->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_Index,i))]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_front,
                (MUINTPTR)dumBuf, 0);
            /* Replace MW PA to dummy frame PA */
            pBuf->mPa = dumBuf->mBuffer->getBufPA(0);
            PIPE_DBG("ReplaceToDummy: replace(0x%x) to (0x%x), VA(0x%lx)", pBuf->mBuffer->getBufPA(0), pBuf->mPa,
                pBuf->mBuffer->getBufVA(0));

            /* Record into table */
            if (port.index == PORT_IMGO.index) {
                arrayIdx = 0;
            } else if (port.index == PORT_IMG2O.index) {
                arrayIdx = 1;
            }
            r_index = this->m_pNormalpipe_FrmB->m_r_enqCnt[arrayIdx] % recordMax;
            this->m_pNormalpipe_FrmB->m_replaceTable[r_index][arrayIdx]->usedPA = pBuf->mPa;
            this->m_pNormalpipe_FrmB->m_replaceTable[r_index][arrayIdx]->unusedPA = pBuf->mBuffer->getBufPA(0);
            this->m_pNormalpipe_FrmB->m_replaceTable[r_index][arrayIdx]->VA = (unsigned long)pBuf->mBuffer->getBufVA(0);
            this->m_pNormalpipe_FrmB->m_replaceTable[r_index][arrayIdx]->filled = 1;
            this->m_pNormalpipe_FrmB->m_r_enqCnt[arrayIdx]++;
            if (this->m_pNormalpipe_FrmB->m_r_enqCnt[arrayIdx] == cntMax)
                this->m_pNormalpipe_FrmB->m_r_enqCnt[arrayIdx] = 0;
...
MVOID NormalPipe_FrmB_Thread::DummyReqReturn(MUINT32 _OpenedIndex, BufInfo* dumBuf, MUINT32* _doReplace)
{
#if 0   //mw no imgo size change in ISP20 . this flow is for avoid translation fault.
    MUINT32 i = _OpenedIndex;
    if (*_doReplace == 1) {
        this->m_pNormalpipe_FrmB->mpDummyRequest[_PortMap(this->m_pNormalpipe_FrmB->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_Index,i))]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_pop, 0, 0);
        //dummy frame can't just drop, need to push back
        this->m_pNormalpipe_FrmB->mpDummyRequest[_PortMap(this->m_pNormalpipe_FrmB->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_Index,i))]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_push, (MUINTPTR)dumBuf, 0);
    }
#endif
}
#endif
...
#if (HAL3_IPBASE == 1)
        /* for replace PA when ysize is small, use dummy PA. */
        DummyReqReturn((MUINT32)i, &_dumBuf, &doReplace);
...
MVOID NormalPipe_FrmB_Thread::EnqueRequest(MUINT32 sof_idx)
{
    ...
    if(_size == 0){
        ...
        for(int i=0;i<_OpenedPort;i++){
            this->m_pNormalpipe_FrmB->mpEnqueReserved[_PortMap(this->m_pNormalpipe_FrmB->GetOpenedPortInfo(NormalPipe_FrmB::_GetPort_Index,i))]->sendCmd(NormalPipe_FrmB::QueueMgr::eCmd_getsize ,(MUINTPTR)&_size,0);
            if(_size != 0){
...
//        if(_t_wake-_t_wait >= 1000)// >1ms
        ...
        {
            //check error frame from kernel .  if error frame happen , skip this frame
            NormalPipe_FRM_STATUS _frm_status;
            _this->m_pNormalpipe_FrmB->sendCommand(EPIPECmd_GET_CUR_FRM_STATUS,(MINTPTR)&_frm_status,0,0);
            if(_frm_status == _err_frame_status)
                PIPE_WRN("warning : error frame happen at kernel. skip this frame. (SOF:%d)\n",_sof_idx);
            else
