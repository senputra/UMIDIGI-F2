...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    if ((m_pStateMgr->getPrecapState() == ePrecap_Num) && (m_pStateMgr->mAFStateCntSet.bNeedToDoPrecapAF == 0))
    {
        ...
        if (m_pStateMgr->getStateStatus().eNextState == eState_AF) {
            m_pStateMgr->transitState(eState_Precapture, eState_AF);
            m_pStateMgr->setNextState(eState_CameraPreview);
        } else if (m_pStateMgr->getStateStatus().eNextState != eState_Invalid) {
            m_pStateMgr->transitState(eState_Precapture, m_pStateMgr->getStateStatus().eNextState);
...
{
    MY_LOG("sendIntent(intent2type<eIntent_AFStart>) line=%d",__LINE__);
    m_pStateMgr->setNextState(eState_AF);
    return  S_3A_OK;
}
