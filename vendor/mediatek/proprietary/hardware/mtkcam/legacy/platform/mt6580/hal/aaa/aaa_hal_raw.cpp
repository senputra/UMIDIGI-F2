...
getCurrResult(MUINT32 u4FrmId, Result_T& rResult) const
{
    ...
    else if (!mbSetTonemapSuccess)
    {
        MINT32 i=0;
        MINT32 base_pt_num = 4;
        MFLOAT base[] = {0.0f, 0.0f, 1.0f, 1.0f};
        MFLOAT *pCurve = &base[0];
        for (i=0; i<base_pt_num; i++)
        {
            rResult.vecTonemapCurveRed.push_back(*pCurve);
            rResult.vecTonemapCurveGreen.push_back(*pCurve);
            rResult.vecTonemapCurveBlue.push_back(*pCurve);
            pCurve++;
        }
        MY_LOG_IF(m_3ALogEnable, "[%s] get base tonemap curve #(%d)", __FUNCTION__, rResult.i4FrmId);
    }
...
{
    MY_LOG_IF(m_3ALogEnable, "[%s] u4MagicNum(%d), m_u4FrmIdStat(%d)", __FUNCTION__, u4MagicNum, m_u4FrmIdStat);
...
MBOOL Hal3ARaw::postCommandDerived(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    if (eCmd == ECmd_CameraPreviewEnd)
    {
        ...
        if (!bRet) {
            ...
            AEE_ASSERT_3A_HAL("sem_wait_relativeTime(semAFProcEnd).");
        }
        ...
        if (mpStateMgr->getFrameCount() >= 0)
        {
            ...
            if (mbPostAESenThd)
            {
                ...
                if (!bRet) {
                    NSCam::Utils::CallStackLogger csl;
                    csl.logProcess(LOG_TAG);
                    //AEE_ASSERT_3A_HAL("sem_wait_relativeTime(semAESenThdEnd).");
                }
...
MBOOL Hal3ARaw::setParams(Param_T const &rNewParam)
{
    ...
    NSIspTuningv3::LscMgr2::getInstance(static_cast<ESensorDev_T>(m_i4SensorDev))->setLock(rNewParam.bIsAWBLock ? MTRUE : MFALSE);
    ...
    if (rNewParam.u1TonemapMode == MTK_TONEMAP_MODE_CONTRAST_CURVE)
    {
        ...
        mbSetTonemapSuccess &= IspTuningMgr::getInstance().setTonemapCurve_Blue(vecIn.editArray(), vecOut.editArray(), &i4Cnt);
        ...
        mbSetTonemapSuccess &= IspTuningMgr::getInstance().setTonemapCurve_Green(vecIn.editArray(), vecOut.editArray(), &i4Cnt);
        ...
        mbSetTonemapSuccess &= IspTuningMgr::getInstance().setTonemapCurve_Red(vecIn.editArray(), vecOut.editArray(), &i4Cnt);

        if (!mbSetTonemapSuccess) //at least 1 curve fails
        {
            MFLOAT base[] = {0.0f, 0.0f, 1.0f, 1.0f};
            pCurve = &base[0];
            MINT32 base_pt_num = 2;
            vecIn.resize(base_pt_num);
            vecOut.resize(base_pt_num);
            pArrayIn = vecIn.editArray();
            pArrayOut = vecOut.editArray();
            for (MINT32 i=0; i<base_pt_num; i++)
            {
                MFLOAT x, y;
                x = *pCurve++;
                y = *pCurve++;
                *pArrayIn++ = x;
                *pArrayOut++ = y;
                MY_LOG_IF(m_3ALogEnable, "[R/G/B]#%d(%f,%f)", rNewParam.u4MagicNum, x, y);
            }
            android::Vector<MFLOAT> vecInR = vecIn, vecInG = vecIn, vecInB = vecIn, vecOutR = vecOut, vecOutG = vecOut, vecOutB = vecOut;
            MBOOL ret_chk = IspTuningMgr::getInstance().setTonemapCurve_Red(vecInR.editArray(), vecOutR.editArray(), &base_pt_num);
            MY_LOG_IF(m_3ALogEnable, "#%d, Red tonemap reset base curve (%d)", rNewParam.u4MagicNum, ret_chk);
            base_pt_num = 2;
            ret_chk = IspTuningMgr::getInstance().setTonemapCurve_Green(vecInG.editArray(), vecOutG.editArray(), &base_pt_num);
            MY_LOG_IF(m_3ALogEnable, "#%d, Green tonemap reset base curve (%d)", rNewParam.u4MagicNum, ret_chk);
            base_pt_num = 2;
            ret_chk = IspTuningMgr::getInstance().setTonemapCurve_Blue(vecInB.editArray(), vecOutB.editArray(), &base_pt_num);
            MY_LOG_IF(m_3ALogEnable, "#%d, Blue tonemap reset base curve (%d)", rNewParam.u4MagicNum, ret_chk);
