...
init()
{
    ...
    if  ( mpPreviewCmdQueThread == 0 || OK != (status = mpPreviewCmdQueThread->run("CamAdapter::PreviewCmdQueThread")) )
    {
        MY_LOGE(
    ...
    if  ( mpCaptureCmdQueThread == 0 || OK != (status = mpCaptureCmdQueThread->run("CamAdapter::CaptureCmdQueThread") ) )
    {
        MY_LOGE(
