...
NextId:
    ...
    else if(state==STATE_END)
    {
...
#else
        fp = fopen("/data/vendor/flash/eng_all.txt","wt");
#endif
        if(fp!=0)
        {
        ...
        if(cust_isDualFlashSupport(m_sensorDev)==1)
        {
            writeCode("/data/vendor/flash/eng_code.txt", nv->engTab.yTab, (dutyNumLt+1)*(dutyNum+1), dutyNum+1);
            writeCodeGain("/data/vendor/flash/fwb_code.txt", nv3A->rFlashAWBNVRAM.rCalibrationData.flashWBGain, 1600, 1, dutyNum, dutyNumLt);
        }
        else
        {
            writeCode("/data/vendor/flash/eng_code.txt", nv->engTab.yTab, dutyNum, dutyNum+1);
            writeCodeGain("/data/vendor/flash/fwb_code.txt", nv3A->rFlashAWBNVRAM.rCalibrationData.flashWBGain, 1600, 0, dutyNum, dutyNumLt);
        }
#endif
        //----------------
        g_CaliData.clear();
...
FUNC_END:
    if(err!=0)
    {
        ...
        AeMgr::getInstance(m_sensorDev).getCaptureParams(0,0,capInfo);
