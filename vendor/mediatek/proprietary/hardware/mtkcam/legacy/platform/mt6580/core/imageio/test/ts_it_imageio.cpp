    ...
    , u4SensorHeight(0)
{
    ...
    mu4DeviceID=0;
    mu4SkipFrame=0;
    memset(sensorStaticInfo, 0, 3*sizeof(SensorStaticInfo));
    memset(sensorDynamicInfo,0, 3*sizeof(SensorDynamicInfo));
    memset(&m_TestEnDequethread, 0, sizeof(pthread_t));
    mInitCount=0;
	m_semTestEnDequethread.count = 0;
    mpIMemDrv = NULL;
    mode = 0;
    mSensorIndex = 0;
    mpCamIOPipe = NULL;
...
setConfigPortInfo()
{
    ...
    if(DUMMY_STRIDE_EN)
    {
        //align8 for RAW10, add dummy stride to avoid tpipe/hw contraint for imgi reading
        // 8(width, in pixel)*10/8 = 10, ensure byte is even
        u4SensorStride = ((u4SensorWidth + 7) & ~7) + 8;
    }
    else
    {
        u4SensorStride = ((u4SensorWidth + 7) & ~7) ;
    }
...
freeRawMem()
{
    if(vRawMem.size() > 0)
    {
        ...
        for (int i = 0; i < BUF_NUM; i++)
        {
            ...
            saveBufToFile(filename, reinterpret_cast<MUINT8*>(vRawMem.at(i).virtAddr), vRawMem.at(i).size /*(u4SensorWidth*u4SensorHeight*10)/8*/);
...
previewProc(MVOID * arg)
{
    ...
    do
    {
        ...
        if(DUMMY_STRIDE_EN)
        {
            port_imgi.u4Stride[0] = pTestIT->u4SensorWidth + 8;//add dummy stride to avoid tpipe/hw contraint for imgi reading, 8(width, in pixel)*10/8 = 10, ensure byte is even
        }
        else
        {
...
startPreview(int count)
{
    ...
    MUINT32 u4RawBufSize=0x0;
     if(DUMMY_STRIDE_EN)
    {
       //add dummy stride to avoid tpipe/hw contraint for imgi reading
        // 8(width, in pixel)*10/8 = 10, ensure byte is even
        u4RawBufSize =  ( ((u4SensorWidth + 7) & ~7)  + 8) * u4SensorHeight * 10 /  8;//(u4SensorWidth * u4SensorHeight * 10) / 8;
    }
     else
    {
        u4RawBufSize =  u4SensorWidth * u4SensorHeight * 10 /  8;
    }
    MINT32 ret = 0;
