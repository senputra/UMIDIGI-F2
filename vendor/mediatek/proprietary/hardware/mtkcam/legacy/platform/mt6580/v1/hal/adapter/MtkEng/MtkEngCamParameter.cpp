...
#define UPDATE_PARAMS(param, eMapXXX, key) \
    ...
    //
    {
        ...
        if (1 == save_shading)
        {
            ...
            p3AHal->send3ACtrl(NS3A::E3ACtrl_SetShadingSdblkCfg, MTRUE, (MINTPTR)ms8RawFilePath.string());

        }
        else
        {
            p3AHal->send3ACtrl(NS3A::E3ACtrl_SetShadingSdblkCfg, MFALSE, (MINTPTR)"");
        }
    }
    //
    if (NSIspTuning::EOperMode_Meta != p3AHal->send3ACtrl(NS3A::E3ACtrl_GetOperMode, 0, 0))
    {
        ...
        */
        ...
        if (MtkCameraParameters::KEY_ENG_SHADING_TABLE_TSF == shading_table)
        {
                ...
                p3AHal->send3ACtrl(NS3A::E3ACtrl_SetShadingOnOff, MTRUE, 0);

        }
        else
        {
                ESensorDev_T sensor = static_cast<ESensorDev_T>(DevMetaInfo::queryHalSensorDev(getOpenId()));
                p3AHal->send3ACtrl(NS3A::E3ACtrl_SetShadingOnOff, MFALSE, 0);
        }
