...
)
{
    ...
    if  ( msgTypeEnabled(CAMERA_MSG_RAW_IMAGE_NOTIFY) )
    {
        ...
        sp<Thread> pThread = new ZipImageCallbackThread(mpCamMsgCbInfo, NULL, 0, false, mu4ShotMode, CAMERA_MSG_RAW_IMAGE_NOTIFY);
    ...
    if  ( msgTypeEnabled(CAMERA_MSG_RAW_IMAGE) )
    {
        ...
        if  ( ! u4RawImgSize || ! puRawImgBuf )
        {
            ...
            if  ( pmem )
            {
                sp<Thread> pThread = new ZipImageCallbackThread(mpCamMsgCbInfo, pmem, 0, false, mu4ShotMode, CAMERA_MSG_RAW_IMAGE);
                if  ( pThread == 0 || pThread->run("CamAdapter::onCB_RawImage") != OK )
                {
                    MY_LOGW("Fail to run ZipImageCallbackThread (%p)", pThread.get());
                }
