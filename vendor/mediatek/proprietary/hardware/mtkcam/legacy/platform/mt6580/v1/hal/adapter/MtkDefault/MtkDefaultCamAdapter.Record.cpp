...
onHandleStartRecording()
{
    ...
    //
    {
        MUINT32 forceFPS = ::property_get_int32("vendor.debug.forceFPS.enable", 0);
        if(forceFPS > 0)
        {
            MY_LOGI("forceFPS(%d)", forceFPS);
            cam3aParam.i4MinFps = forceFPS*1000;
            cam3aParam.i4MaxFps = forceFPS*1000;
        }
    }
    //
    MY_LOGI("FPS:RH(%d),Min(%d),Max(%d)",mpParamsMgr->getRecordingHint(),cam3aParam.i4MinFps,cam3aParam.i4MaxFps);
    //
