...
initImageCreateThread()
{
    ...
    if  (
        ...
        ||  OK != (status = mpImageCreateThread->run("MultiShot::ImageCreateThread"))
        )
    {
        MY_LOGE(
    ...
    if  (
        ...
        ||  OK != (status = mpYuvImageCreateThread->run("MultiShot::YuvImageCreateThread"))
        )
    {
        MY_LOGE(
    ...
    if  (
        ...
        ||  OK != (status = mpThumbnailImageCreateThread->run("MultiShot::ThumbnailImageCreateThread"))
        )
    {
        MY_LOGE(
    ...
    if  (
        ...
        ||  OK != (status = mpJpegImageCreateThread->run("MultiShot::JpegImageCreateThread"))
        )
    {
        MY_LOGE(
...
createJpegImgSW(ImgBufInfo const & rSrcImgBufInfo, ImgBufInfo const & rDstImgBufInfo, MUINT32 & u4JpegSize)
{
    ...
    // after rotation, mThumbImgBufInfoYuv's u4ImgWidth and u4ImgHeight
    ImgBufInfo rSrcBuf = mThumbImgBufInfoYuv;
    ImgBufInfo rDstBuf = rDstImgBufInfo;
    if ( mShotParam.u4PictureRotation == 90 || mShotParam.u4PictureRotation == 270 ) {
        MUINT32 u4Width = mThumbImgBufInfoYuv.u4ImgHeight;
        MUINT32 u4Height = mThumbImgBufInfoYuv.u4ImgWidth;
        rSrcBuf.u4ImgWidth = u4Width;
        rSrcBuf.u4ImgHeight = u4Height;
        rSrcBuf.u4Stride[0] = queryImgStride(mThumbImgBufInfoYuv.eImgFmt, u4Width, 0);
        rSrcBuf.u4Stride[1] = queryImgStride(mThumbImgBufInfoYuv.eImgFmt, u4Width, 1);
        rSrcBuf.u4Stride[2] = queryImgStride(mThumbImgBufInfoYuv.eImgFmt, u4Width, 2);
        u4Width = rDstImgBufInfo.u4ImgHeight;
        u4Height = rDstImgBufInfo.u4ImgWidth;
        rDstBuf.u4ImgWidth = u4Width;
        rDstBuf.u4ImgHeight = u4Height;
        rDstBuf.u4Stride[0] = queryImgStride(rDstImgBufInfo.eImgFmt, u4Width, 0);
        rDstBuf.u4Stride[1] = queryImgStride(rDstImgBufInfo.eImgFmt, u4Width, 1);
        rDstBuf.u4Stride[2] = queryImgStride(rDstImgBufInfo.eImgFmt, u4Width, 2);
    }
    MtkCamUtils::CamProfile profile("createJpegImgSW", "MultiShot");
    ISImager *pISImager = ISImager::createInstance(rSrcBuf);
    CHECK_OBJECT(pISImager);
    pISImager->setTargetBufInfo(rDstBuf);
    pISImager->setStrideAlign(rSrcBuf.u4Stride);
    pISImager->setFormat(eImgFmt_JPEG);
    pISImager->setRotation(0);
    pISImager->setFlip(0);
    pISImager->setResize(rDstBuf.u4ImgWidth, rDstBuf.u4ImgHeight);
    pISImager->setEncodeParam(mJpegParam.fgThumbIsSOI, mJpegParam.u4ThumbQuality, JPEGENC_SW);
    pISImager->setROI(Rect(0, 0, rSrcBuf.u4ImgWidth, rSrcBuf.u4ImgHeight));
    pISImager->execute();
    u4JpegSize = pISImager->getJpegSize();
    pISImager->destroyInstance();
    profile.print();
