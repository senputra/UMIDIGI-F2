...
namespace NSACDK
{
    ...
    class AcdkFakeSensorList {
    public:
        static AcdkFakeSensorList*   get();

        MUINT                   queryNumberOfSensors() { return 1; }
        char const*             queryDriverName(MUINT const index) { return "Fakegarnett"; }
        MUINT                   queryType(MUINT type) { return 0; }
        MUINT                   querySensorDevIdx(MUINT const index) { return (1 << (index&0x1)); };//main or sub
        MUINT                   searchSensors() { return 0; }
        AcdkFakeSensor*         createSensor(
                                    char const* szCallerName,
                                    MUINT const uCountOfIndex,
                                    MUINT const*pArrayOfIndex
                                );
...
#if FAKE_SENSOR == 1
        AcdkFakeSensor *m_pSensorHalObj;
#else
...
#endif
        MUINT32 ExtraSizeForImgi; //IMGI may read more data at some size
        //PureRawBuffer
