...
)
{
    ...
    //
    MERROR err = OK;
    if ( StreamStatus::UNINIT == mvStreamStatus.valueAt((size_t)index) ) {
        MY_LOGD("Create new halimage stream id:%#" PRIxPTR, streamid);
        err = createStream(streamid, pStreamInfo);
        if ( OK != err )
            return NULL;
        MY_LOGD("After create pool(%p)", mHalImageStreamBufferPoolMap.valueAt((size_t)index).get());
        mvStreamInfo.add(streamid, pStreamInfo);
        mvReusableStreams.add(streamid, pStreamInfo);
        mvStreamStatus.replaceValueAt((size_t)index, StreamStatus::INITIALIZED);
        return mHalImageStreamBufferPoolMap.editValueAt((size_t)index);
    }
    //
    //if ( StreamStatus::RECONFIG == mvStreamStatus.valueAt(index) )
    // check whole image streaminfo
    MBOOL bReconfigure = MFALSE;
    if ( strcmp( pStreamInfo->getStreamName(),  mvStreamInfo[(size_t)index]->getStreamName() )      ||
         pStreamInfo->getStreamId()          != mvStreamInfo[(size_t)index]->getStreamId()          ||
         pStreamInfo->getStreamType()        != mvStreamInfo[(size_t)index]->getStreamType()        ||
         pStreamInfo->getMaxBufNum()         != mvStreamInfo[(size_t)index]->getMaxBufNum()         ||
         pStreamInfo->getMinInitBufNum()     != mvStreamInfo[(size_t)index]->getMinInitBufNum()     ||
         pStreamInfo->getUsageForConsumer()  != mvStreamInfo[(size_t)index]->getUsageForConsumer()  ||
         pStreamInfo->getUsageForAllocator() != mvStreamInfo[(size_t)index]->getUsageForAllocator() ||
         pStreamInfo->getImgFormat()         != mvStreamInfo[(size_t)index]->getImgFormat()         ||
         pStreamInfo->getImgSize()           != mvStreamInfo[(size_t)index]->getImgSize()           ||
         pStreamInfo->getTransform()         != mvStreamInfo[(size_t)index]->getTransform()
       ) {
       bReconfigure = MTRUE;
       mvStreamInfo.replaceValueAt((size_t)index, pStreamInfo);
    }
...
)   const
{
    ...
    MY_LOGE_IF(OK!=err || rpStreamBuffer==0,
            "[acquireFromPool] err:%d(%s) pStreamBuffer:%p stream:(%s)",
            err, ::strerror(-err), rpStreamBuffer==0 ? 0 : rpStreamBuffer.get(),
