...
dequeHWBuf(MUINT32 const u4TimeoutMs /*= 0xFFFFFFFF*/)
{
    ...
    MBOOL ret = mpCamIOPipe->dequeOutBuf(rPortID, rQTimeOutBufInfo);
    MY_LOGD("dequehwbuf() dequeoutbuf1=%d",ret);
    if (!ret)
    {
    ...
    if (mfgIsYUVPortON)
    {
        ...
        MY_LOGD("dequehwbuf() dequeoutbuf2=%d",ret);
        if (!ret)
        {
    ...
    MBOOL const fgIsContinuous)
{
    ...
    configParam[0] =
    {
        ...
        isBypassScenario    : u4ByPassScenario,
