...
protected:  ////                    Data Members. (Queue: dummy request)
...
{
    MBOOL result = MFALSE;
    if ( pFrame.appFrame->getIsP2HalYuvJpegSupport()) {
        result = MTRUE;
        MY_LOGD1("P2 hal yuv jpeg out port support");
    }
...
)
{
    ...
    CAM_TRACE_FMT_BEGIN("P1NodeImp::queue frameNo=%d", pFrame != NULL ? pFrame->getFrameNo():-1);
    MY_LOGD1("active=%d, frameNo=%d", mActive, pFrame != NULL ? pFrame->getFrameNo():-1);
    if (!mActive) {
        hardwareOps_start();
    }
...
void dumpP1Buffer(QBufInfo &deqBuf, sp<IPipelineFrame> const& pFrame) {
    for(size_t i = 0; i < deqBuf.mvOut.size(); i++) {
        ...
        sprintf(filename, "/data/vendor/camera_dump/P1_%d_%d_%dx%d_%d_0.raw",
...
)
{
    ...
    // When YUV sensor, 3A do not get SOF from P1 driver
    // Here provides a workaround for P1 driver
    if (sofIdx == 0) {
        MUINT32 sof = 0;
        if (!mpCamIO->sendCommand(EPIPECmd_GET_CUR_SOF_IDX, (MINTPTR)&sof, 0, 0)) {
            MY_LOGE("EPIPECmd_GET_CUR_SOF_IDX error");
        } else {
            sofIdx = sof;
            MY_LOGD("override sofIdx=%d", sofIdx);
        }
    }
    ...
    if(!initial) { // [TODO] && VALID 3A PROCESSED NOTIFY
        ...
        if (!exist) {
            Mutex::Autolock _l(mDummyRequestQueueLock);
            Que_T::iterator itDummy = mDummyRequestQueue.begin();
            for(; itDummy != mDummyRequestQueue.end(); itDummy++) {
                if ((*itDummy).magicNum == magicNum) {
                    node = *itDummy;
                    node.sofIdx = sofIdx;
                    node.dummyRequest = true;
                    mDummyRequestQueue.erase(itDummy);
                    break;
                }
            }
        }
        ...
        if (exist) {
            node.needQueueProcRequest = true;
            onProcessEnqueFrame(node);
        } else if (node.dummyRequest) {
            MY_LOGD("Match dummy request, enque magicNum: %d", node.magicNum);
            node.needQueueProcRequest = true;
            onProcessEnqueFrame(node);
        } else {
    ...
    if (mRequestQueue.empty()) {
        while(mControls3AList.size() <= ((size_t)m3AProcessedDepth)) {
            if (mControls3AList.size() == ((size_t)m3AProcessedDepth)) {
                MY_LOGD("Request queue is empty and 3A list not full, create node to compensate 3A list");
                createNode(NULL, &mDummyRequestQueue, &mDummyRequestQueueLock,
            ...
            if (frame.magicNum == magicNum) {
                ...
                if((magicNum & 0x40000000) != 0) {
                    MY_LOGW("magicNum from driver(%d) is uncertain",
                          magicNum);
                    ret = MFALSE;
                } else if ((magicNum & 0x20000000) != 0) {
                    // Drop the frame due to incorrect return from P1driver.
                    MY_LOGW("magicNum from driver(%d) is dummy, should(%d)",
                          magicNum, frame.magicNum);
                    ret = MFALSE;
                } else {
                    // Drop frame with dismatch magic number.
                    ret = MFALSE;
            ...
            if (enableP1RawDump() > 0) {
                ...
                if (tryGetMetadata<MUINT8>(&inAPP, MTK_CONTROL_CAPTURE_INTENT, capture_intent)) {
                    if (enableP1RawDump() == DUMP_P1_BUF_ALL || enableP1RawDump() == capture_intent) {
                        dumpP1Buffer(deqBuf, pFrame.appFrame);
                    }
                }
            }
        }
...
)
{
    ...
    //(1.1) clear dummy request queue
    {
        Mutex::Autolock _l(mDummyRequestQueueLock);
        while(!mDummyRequestQueue.empty()) {
            QueNode_T pFrame = *mDummyRequestQueue.begin();
            mDummyRequestQueue.erase(mDummyRequestQueue.begin());
            onFlushRequestFrame(pFrame);
        }
    }
    ...
    //(3.1) hwdummy return for mNumPreviewBuffer count of fullraw
    for(size_t i = 0; i < mNumPreviewBuffer; i++) {
        MERROR err = OK;
...
)
{
    ...
    if (pFrame.appFrame != 0) {
        ...
        if (mOutImage_resizer != NULL) {
            ...
            returnLockedImageBuffer(pFrame.appFrame, mOutImage_resizer->getStreamId());
        }
        for(size_t i = 0; i < mvOutImage_full.size(); i++) {
            streamId_Images.push_back(mvOutImage_full[i]->getStreamId());

            if (isP2HalYuvJpegSupport(pFrame) || mSensorParams.type == SENSOR_TYPE_YUV) {
                returnLockedImageBuffer(pFrame.appFrame, mvOutImage_full[i]->getStreamId());
            } else {
                if ((mDummyBufferFull != NULL) && (pFrame.img_full != 0)) {
                     MY_LOGD1("unlockBuf pFrame:%p", pFrame.img_full.get());
                     pFrame.img_full->unlockBuf(getNodeName());
                }
                returnUnlockedImageBuffer(pFrame.appFrame, mvOutImage_full[i]->getStreamId());
            }
        }
        ...
        if (pFrame.halFrame_full != 0) {
            ...
            if ((mDummyBufferFull != NULL) && (pFrame.img_full != 0)) {
                MY_LOGD1("Return processing halFrame_full is null, unlockBuf pFrame:%p dummyRequest:%d",
                        pFrame.img_full.get(), pFrame.dummyRequest);
                pFrame.img_full->unlockBuf(getNodeName());
            }
        }
        ...
        if (pFrame.halFrame_resizer != 0) {
            ...
            if ((mDummyBufferResizer != NULL) && (pFrame.img_resizer != 0)) {
                MY_LOGD1("Return processing halFrame_resizer is null, unlockBuf pFrame:%p dummyRequest:%d",
                        pFrame.img_resizer.get(), pFrame.dummyRequest);
                if (pFrame.dummyRequest) {
...
)
{
    ...
    if (pFrame.appFrame != 0) {
        ...
        if (pFrame.halFrame_full != 0) {
            ...
            if ((mDummyBufferFull != NULL) && (pFrame.img_full != 0)) {
                MY_LOGD1("Flush request halFrame_full is null, unlockBuf pFrame:%p dummyRequest:%d",
                        pFrame.img_full.get(), pFrame.dummyRequest);
                if (pFrame.dummyRequest) {
                    pFrame.img_full->unlockBuf(getNodeName());
                }
        ...
        if (pFrame.halFrame_resizer != 0) {
            ...
            if ((mDummyBufferResizer != NULL) && (pFrame.img_resizer != 0)) {
                MY_LOGD1("Flush request halFrame_resizer is null, unlockBuf pFrame:%p dummyRequest:%d",
                        pFrame.img_resizer.get(), pFrame.dummyRequest);
                if (pFrame.dummyRequest) {
                    pFrame.img_resizer->unlockBuf(getNodeName());
                }
...
)
{
    ...
    if (pFrame.appFrame != 0) {
        ...
        if (mOutImage_resizer != NULL) {
            streamId_Images.push_back(mOutImage_resizer->getStreamId());
            returnLockedImageBuffer(pFrame.appFrame, mOutImage_resizer->getStreamId(), success);
        }
        ...
        for(size_t i = 0; i < mvOutImage_full.size(); i++) {
            ...
            if (isP2HalYuvJpegSupport(pFrame) || mSensorParams.type == SENSOR_TYPE_YUV) {
                returnLockedImageBuffer(pFrame.appFrame, mvOutImage_full[i]->getStreamId(), success);
            } else {
                if ((mDummyBufferFull != NULL) && (pFrame.img_full != 0)) {
                     MY_LOGD1("unlockBuf pFrame:%p", pFrame.img_full.get());
                     pFrame.img_full->unlockBuf(getNodeName());
        ...
        if (pFrame.halFrame_full != 0) {
            ...
            } else if (pFrame.dummyRequest) {
                MY_LOGD1("Flush processing halFrame_full is null, unlockBuf");
                pFrame.img_full->unlockBuf(getNodeName());
            }
        }
        if (pFrame.halFrame_resizer != 0) {
            ...
            if (pFrame.img_resizer.get() ==
                ...
                MY_LOGD1("Flush processing halFrame_resizer is null, unlockBuf");
                pFrame.img_resizer->unlockBuf(getNodeName());
            }
        }
    }
...
)
{
    ...
    if(magicNum == 0) {
        MY_LOGE("Wrong magic number callback : %d", magicNum);
        return;
    }

    {
        Mutex::Autolock _ll(mProcessingQueueLock);
	auto it = mProcessingQueue.begin();
	for (; it != mProcessingQueue.end(); it++) {
	    if ((*it).magicNum == magicNum) {
                (*it).bHqcGet = key;
		break;
	...
	if (it == mProcessingQueue.end()) {
            MY_LOGE("cannot find the right node for num: %d, bHqcGet: %d", magicNum, key);
	}
    }
    ...
    if (mActive) {
        switch(_msgType)
        {
            ...
            case IHal3ACb::eID_NOTIFY_CURR_RESULT:
                onProcess3AResult((MUINT32)_ext1,(MUINT32)_ext2); //magic, key, val
                break;
            default:
                break;
        }
...
{
    FUNCTION_IN
    CAM_TRACE_NAME("DummyBuffer::init");
...
hwDummyBuffer_init(void)
{
    ...
    if (mvOutImage_full.size() != 0) {
        ...
        err = pDummyBuffer->init(mvOutImage_full[0], (mNumHardwareBuffer + mNumPreviewBuffer + mNumDummyBuffer));
        if (err != OK) {
    ...
    if (mOutImage_resizer != NULL) {
        ...
        err = pDummyBuffer->init(mOutImage_resizer, (mNumHardwareBuffer + mNumDummyBuffer));
        if (err != OK) {
...
{
#if SUPPORT_ISP
    CAM_TRACE_FMT_BEGIN("hardwareOps_enque: frameNo=%d",
            node.appFrame != NULL ? node.appFrame->getFrameNo():-1);

    MY_LOGD("+ frameNo=%d, magicNum=%d, dummy=%d, block=%d, dummyRequest=%d, queueProc=%d",
            node.appFrame != NULL ? node.appFrame->getFrameNo():-1,
            node.magicNum, dummy, block, node.dummyRequest, node.needQueueProcRequest);
    ...
    if (node.appFrame == NULL) {
        if (mpStreamPool_full != 0) {
                ...
                && (mdummyQueue.size() > 0))
            {
                ...
                mDummyFullBufferCount = mDummyFullBufferCount % mNumDummyBuffer;
                if (OK != hwDummyBuffer_acquire(mDummyBufferFull,
                                        (mNumHardwareBuffer + mNumPreviewBuffer + mDummyFullBufferCount),
                                        getNodeName(),
                                        imageBuffer)
                )
                {
                    MY_LOGE("full dummyRequest image buffer heap fail");
                    return BAD_VALUE;
                } else {
                    MY_LOGD1("full dummyRequest image buffer heap success %d",
                                      (mNumHardwareBuffer + mNumPreviewBuffer + mDummyFullBufferCount));
                    mDummyFullBufferCount++;
                }
                node.halFrame_full = NULL;
                node.img_full = imageBuffer;
            }
            ...
            {
                if (mSensorParams.type == SENSOR_TYPE_YUV) {
                    MY_LOGD1("config phase, is YUV enque real buffer");
                    ...
                    MY_LOGD1("config phase, not YUV enque dummy buffer");
                    mBufferPreviewCount = mBufferPreviewCount % mNumPreviewBuffer;
                    if (OK != hwDummyBuffer_acquire(mDummyBufferFull,
                                            (mNumHardwareBuffer + mBufferPreviewCount),
                                            getNodeName(),
                                            imageBuffer)
                    )
                    {
                        MY_LOGE("config phase, new full image buffer heap fail");
                        return BAD_VALUE;
                    } else {
                        MY_LOGD1("config phase, acquire full image buffer success %d",
                                mNumHardwareBuffer + mBufferPreviewCount);
                        mBufferPreviewCount++;
                    }
                    node.halFrame_full = NULL;
        ...
        if (mpStreamPool_resizer != 0) {
                ...
                && (mdummyQueue.size() > 0))
            {
                ...
                mDummyResizeBufferCount = mDummyResizeBufferCount % mNumDummyBuffer;
                if (OK != hwDummyBuffer_acquire(mDummyBufferResizer,
                                        (mNumHardwareBuffer + mDummyResizeBufferCount),
                                        getNodeName(),
                                        imageBuffer)
                )
                {
                    MY_LOGE("resize dummyRequest image buffer heap fail");
                    return BAD_VALUE;
                } else {
                    MY_LOGD1("resize dummyRequest image buffer heap success %d",
                                  (mNumHardwareBuffer + mDummyResizeBufferCount));
                    mDummyResizeBufferCount++;
                }
                node.halFrame_resizer = NULL;
                node.img_resizer = imageBuffer;
            }
        ...
        for (size_t i = 0; i < mvOutImage_full.size(); i++) {
            ...
            if (isP2HalYuvJpegSupport(node) || mSensorParams.type == SENSOR_TYPE_YUV) {
                MY_LOGD1("request phase, lockImageBuffer");
                ...
                {
                    MY_LOGE("request phase, new full image buffer heap fail");
                    return BAD_VALUE;
                } else {
                    MY_LOGD1("request phase, acquire full image buffer success %d",
                               mNumHardwareBuffer + mBufferPreviewCount);
                    mBufferPreviewCount++;
                }
            }
    ...
    if (node.needQueueProcRequest) {
        Mutex::Autolock _ll(mProcessingQueueLock);
        mProcessingQueue.push_back(node);
        MY_LOGD1("Push magicNum:%d to Processing queue size:%zu",
                 node.magicNum, mProcessingQueue.size());
    }
...
#if SUPPORT_3A
    if (mActive && mp3A) {
        MBOOL bHqcGet = MFALSE;
        {
            MUINT32 magic = deqBuf.mvOut[0].mMetaData.mMagicNum_hal;
            Mutex::Autolock _l(mProcessingQueueLock);
	    if (mProcessingQueue.empty()) {
            MY_LOGE("ProQ is empty");
	    } else {
                auto itr = mProcessingQueue.begin();
                for(; itr != mProcessingQueue.end(); itr++) {
                    if((*itr).magicNum == magic) {
                        bHqcGet = (*itr).bHqcGet;
                        break;
                    }
                }
                if(itr == mProcessingQueue.end()) {
                    MY_LOGE("Cannot find magicnum in ProcessingQueue magicNum: %d", magic);
                }
            }
        }
        ...
        MY_LOGD("magic=%d, result3A.appMeta.count=%d getCur(%d)",
                deqBuf.mvOut[0].mMetaData.mMagicNum_hal, result3A.appMeta.count(), bHqcGet);
    }
#endif
           ...
           Mutex *listLock)
{
    ...
    if (list!=NULL) {
        ...
        //add begin
        MINT32 rawType;
        tryGetMetadata<MINT32>(&metaInfo.appMeta, MTK_P1NODE_RAW_TYPE, rawType);
        MY_LOGD("rawType = %d", rawType);
        IMetadata::IEntry entry(MTK_P1NODE_RAW_TYPE);
        entry.push_back(rawType, Type2Type<MINT32>());
        metaInfo.halMeta.update(MTK_P1NODE_RAW_TYPE, entry);
        //add end
