...
#define CAM_MIPIRX_ANALOG_RANGE 0x4000
...
int SeninfDrvImp::init()  //kk test
{
    ...
    MINT32 mipiRx_config = 0x1500C000;
     MINT32 mipiRx_analog = 0x10011000;  /* Base address of CSI0 10011800*/
    MINT32 gpio_base_addr = 0x10017000; /* GPIO Baseaddress */
    ...
    mpCAMIODrvRegAddr = mpGpioHwRegAddr + (0x78 / 4); // DRV_CFG1
/*LOG_MSG("mpCAMIODrvRegAddr %p = 0x%x, State: init  \n",
             mpCAMIODrvRegAddr, (int)*mpCAMIODrvRegAddr);
*/
...
)
{
    ...
    if(ncsi2En == 1) {  // enable CSI2
        ...
        mt65xx_reg_writel(temp|0x00000008, mpCSI2RxAnalog0RegAddr + (0x10/4));
         ...
         temp = *(mpCSI2RxAnalog0RegAddr + (0x00));//clock lane input select main
         mt65xx_reg_writel(temp&0xFFFFFFEF, mpCSI2RxAnalog0RegAddr + (0x00));
         temp = *(mpCSI2RxAnalog0RegAddr + (0x0c/4));//data lane 0 input select main
         mt65xx_reg_writel(temp&0xFFFFFFEF, mpCSI2RxAnalog0RegAddr + (0x0C/4));
         temp = *(mpCSI2RxAnalog0RegAddr + (0x10/4));//data lane 1 input select main
         mt65xx_reg_writel(temp&0xFFFFFFEF, mpCSI2RxAnalog0RegAddr + (0x10/4));

        // CSI Power On Timing
        mt65xx_reg_writel(0x0000001c, mpCSI2RxAnalog0RegAddr + (0x28/4));////new setting for 6580
...
#endif
        // ISP CLK = 300Mhz , settel delay count =  (x ns *  300 M )/1000
...
)
{
     int ret = 0;
        ...
        temp = *(mpCSI2RxAnalog0RegAddr + (0x4C/4));//GPI*_IES = 0 for MIPI
         ...
         temp = *(mpCSI2RxAnalog0RegAddr + (0x00));//clock lane input select sub
         mt65xx_reg_writel(temp|0x00000010, mpCSI2RxAnalog0RegAddr + (0x00));

         temp = *(mpCSI2RxAnalog0RegAddr + (0x0c/4));//data lane 0 input select sub
         mt65xx_reg_writel(temp|0x00000010, mpCSI2RxAnalog0RegAddr + (0x0C/4));
         temp = *(mpCSI2RxAnalog0RegAddr + (0x10/4));//data lane 1 input select sub
...
)
{
    ...
    //    temp = *(mpCSI2RxAnalog0RegAddr + (0x48/4));//GPI*_IES = 0 for MIPI
   //     mt65xx_reg_writel(0xfff, mpCSI2RxAnalog0RegAddr + (0x48/4));
...
int SeninfDrvImp::setMclk1IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    if(mpCAMIODrvRegAddr != NULL) {
        ...
        *(mpCAMIODrvRegAddr) |= (ioDrivingCurrent<<8); // [6:4] = 0:2mA, 1:4mA, 2:6mA, 3:8mA
...
int SeninfDrvImp::autoDeskewCalibrationSeninf1(unsigned long lane_num)
{
    int ret = 0;
    unsigned int temp = 0;
