...
//    ISP_FUNC_DBG("");
    ISP_FUNC_DBG("ScenarioID: %d.", eScenarioID);
...
int ret = 0;
    ...
    if (!m_fgIsGdmaMode)    // Normal Mode (i.e. not GDMA).
    {
        ...
        for(int q=0;q<ISP_DRV_CQ_NUM;q++)
        {
            ...
            ISP_FUNC_DBG("[Tpipe alloc]q(%d),va/pa(0x%08x, 0x%08x)",q,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
        }
    }
    else    // GDMA mode.
    {
    ...
    ISP_FUNC_DBG("[m_pIspDrv]:0x%08x ",m_pIspDrv);
    if (!m_pIspDrv) {
    ...
    ISP_FUNC_DBG("[m_pIspEventThread]:0x%08x ",m_pIspEventThread);
...
//js_test move to _disable.
    ...
    for(int q=0;q<ISP_DRV_CQ_NUM;q++)
    {
        ...
        ISP_FUNC_DBG("[Tpipe free]q(%d),va/pa(0x%08x, 0x%08x)",q,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTableVa,g_MdpMgrCfgData[q].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
    }
...
//TEST 0418            ISP_WRITE_REG(m_pPhyIspReg ,CAM_CTL_TILE, 0x7A03001E);  // TEST_MDP // hardware do not read both tpipe width and height
        ...
        if(m_pIspDrv->IsReadOnlyMode())
        {
            ...
            ISP_FUNC_INF("[Pass2]0 AppTurn(%d),EN1:Set(0x%08x),Clr(0x%08x),En2:Set(0x%08x),Clr(0x%08x),DMA:Set(0x%08x),Clr(0x%08x),tcm(0x%08x)\n", \
                    ...
                    ISP_IOCTL_READ_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_EN2_SET),ISP_IOCTL_READ_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_EN2_CLR), \
                    ISP_IOCTL_READ_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_DMA_EN_SET),ISP_IOCTL_READ_REG(m_pIspDrv, m_pIspDrv->getRegAddrMap(),  CAM_CTL_DMA_EN_CLR), this->tcm_en);
        }
        else
        {
            ISP_WRITE_ENABLE_REG(m_pIspReg,  CAM_CTL_DMA_EN_CLR, reg_tmp);
            ...
            ISP_FUNC_INF("[Pass2]AppTurn(%d) EN1-Set(0x%x),Clr(0x%x)-En2:Set(0x%x),Clr(0x%x)-DMA:Set(0x%x),Clr(0x%x),tcm(0x%08x)\n", \
                    applyTurn,ISP_READ_REG(m_pIspReg,  CAM_CTL_EN1_SET),ISP_READ_REG(m_pIspReg,  CAM_CTL_EN1_CLR), \
                    ISP_READ_REG(m_pIspReg,  CAM_CTL_EN2_SET),ISP_READ_REG(m_pIspReg,  CAM_CTL_EN2_CLR), \
                    ISP_READ_REG(m_pIspReg,  CAM_CTL_DMA_EN_SET),ISP_READ_REG(m_pIspReg,  CAM_CTL_DMA_EN_CLR), this->tcm_en);
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    {
        //do not need this due to mdp control flow is changed to k2 version
        #if 0
...
MINT32 ISP_BUF_CTRL::enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo )
{
    ...
    ISP_FUNC_INF("dma(%d),id(%d),size(0x%x),VA(0x%lx),PA(0x%x),S/C(%d/%d)",
                rt_dma,
                buf_info.memID,
                buf_info.size,
                buf_info.virtAddr,
...
MINT32 ISP_BUF_CTRL::dequeueHwBuf( MUINT32 dmaChannel )
{
    ...
    if( ISP_PASS1 == this->path )
    {
        if( _imgo_ == rt_dma || _img2o_ == rt_dma )
        {
            ...
            {
                ISP_FUNC_INF("i(%d),dma(%d),id(0x%x),size(0x%x),VA(0x%lx),PA(0x%x),size(%d)",i,rt_dma,deque_buf.data[i].memID,deque_buf.data[i].size,deque_buf.data[i].base_vAddr,deque_buf.data[i].base_pAddr,m_swbufL[rt_dma].bufInfoList.size());
    ...
    else
    {
        ...
        while(count--)
        {
            ...
            if(m_hwbufL[rt_dma].bufInfoList.size() && ISP_PASS2 == this->path)
            {
                ...
                //for vss+prv case that user try to deque prv buffer while vss buffer has not beed dequeued done
                if(m_hwbufL[rt_dma].bufInfoList.size()==2)
                {   //for vss+prv case that user try to deque prv buffer while vss buffer has not beed dequeued done
                     list<stISP_BUF_INFO>::iterator it = m_hwbufL[rt_dma].bufInfoList.begin();
                     it++;
                     ISP_FUNC_INF("deq prv buf while vss buf has not beed dequed, (%d)_pa(0x%x)",rt_dma,(*it).base_pAddr);
                     (*it).status = ISP_BUF_FILLED;
                }
                else
                {   //only prv buffer
                    //change type
                ...
                {   //for vss+prv case that user try to deque prv buffer while vss buffer has not beed dequeued done
                     list<stISP_BUF_INFO>::iterator it = m_hwbufL[rt_dma].bufInfoList.begin();
                     it++;
                     m_swbufL[rt_dma].bufInfoList.push_back((*it));
                     m_hwbufL[rt_dma].bufInfoList.erase(it);
                }
                else
                {
...
MUINT32 ISP_BUF_CTRL::getCurrHwBuf( MUINT32 dmaChannel )
{
    ...
    ISP_FUNC_INF("dma:(%d)/memID(0x%x)/vAddr:(0x%lx)/pAddr:(0x%x)/size:(0x%x)/status:(%d) ",rt_dma, \
                                                        ...
                                                        m_hwbufL[rt_dma].bufInfoList.front().size, \
                                                        m_hwbufL[rt_dma].bufInfoList.front().status);

    //revise for Denali-2, cause we set imgi base address by isp CQ, not by mdp
