...
#define logI(fmt, arg...) CAM_LOGD(LOG_TAG " " fmt, ##arg) //ALOGD(fmt, ##arg)
...
void FlashMgrM::hw_setExpPara(FlashAlgExpPara* expPara, int sensorType, FLASH_PROJECT_PARA* /*pPrjPara*/)
{
    ...
    // valdate ISP
    IspTuningMgr::getInstance().validatePerFrame(sensorType, MFALSE);
...
int FlashMgrM::doPfOneFrame(FlashExePara* para, FlashExeRep* rep)
{
    ...
    {
        logV("doPfOneFrame quick calibration frm=%d",m_pfFrameCount);
        rep->isFlashCalibrationMode=1;
