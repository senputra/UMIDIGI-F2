...
public:
    ...
    /**
      *@brief  Register irq
      */
    inline int registerIrq( NSImageio_FrmB::NSIspio_FrmB::Irq_t irq ){return this->_registerIrq( irq );}
    ...
    CamPathPass1() :
        ...
        DupQIdx(0)
        {
            for(int i=0;i<p1Notify_node;i++)
                m_p1NotifyObj[i] = 0;
            for(int j=0;j<ISP_FUNCTION_MAX_NUM;j++)
...
public:     //// constructors.
    ...
    ):magicNum(0),dupCqIdx(0),burstQueIdx(0),isWaitBuf(0),mdp_imgxo_p2_en(0),scenario(0),isV3(0),isApplyTuning(0),isMdpCropEn(0),CQ(0),
      tpipe(0),p2SubMode(0),tdr_en(0),tcm_en(0),isIspOn(0),hrzRatio(0),hrzCropX(0),hrzCropY(0),isConcurrency(0),
      isEn1C24StatusFixed(0),isEn1CfaStatusFixed(0),isEn1HrzStatusFixed(0),isEn2CdrzStatusFixed(0),isEn2G2cStatusFixed(0),
      isEn2C42StatusFixed(0),isImg2oStatusFixed(0),isImgoStatusFixed(0),isAaoStatusFixed(0),isEsfkoStatusFixed(0),
      isFlkiStatusFixed(0),isLcsoStatusFixed(0),isEn1AaaGropStatusFixed(0),isShareDmaCtlByTurn(0),
      bypass_ispRawPipe(0),bypass_ispRgbPipe(0),bypass_ispYuvPipe(0),bypass_ispCdpPipe(0),pix_id(0)
