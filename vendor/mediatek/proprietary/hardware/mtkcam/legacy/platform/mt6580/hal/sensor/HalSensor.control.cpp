...
#define ENABLE_CSI_AUTO_DEKSKEW 0
#define MAIN_CSI_AUTO_DESKEW_THRESHOLD 1000*1000000
#define SUB_CSI_AUTO_DESKEW_THRESHOLD 700*1000000
#define CSI_BLANKING_RATIO 1120//1.12
//
...
)
{
    ...
    property_get("vendor.camera.hwsync.enable", value, "0");
    ...
    if((currSensorDev & SENSOR_DEV_MAIN)
        ...
        &&(sensorPara[0].MIPI_OPHY_TYPE != 1)){
            ...
            if(((((sensorPara[0].cropWidth * sensorPara[0].cropHeight)/1000) * (sensorPara[0].frameRate/10)*bpp)/(sensorPara[0].u1MIPILaneNum+1))
                ...
                pSeninfDrv->autoDeskewCalibrationSeninf1(sensorPara[0].u1MIPILaneNum+1);
            }


    } else if((currSensorDev & SENSOR_DEV_SUB)
        &&( sensorPara[1].inSrcTypeSel == MIPI_SENSOR)
        &&(sensorPara[1].MIPI_OPHY_TYPE != 1)){
            int bpp=10;
            ...
            if(((((sensorPara[1].cropWidth * sensorPara[1].cropHeight)/1000) * (sensorPara[1].frameRate/10)*bpp)/(sensorPara[1].u1MIPILaneNum+1))
                > (SUB_CSI_AUTO_DESKEW_THRESHOLD/CSI_BLANKING_RATIO)){
                SeninfDrv *const pSeninfDrv = SeninfDrv::createInstance();
                pSeninfDrv->autoDeskewCalibrationSeninf1(sensorPara[1].u1MIPILaneNum+1);
            }
    }
#endif
...
MVOID HalSensor::pixelModeArrange(MUINT currSensorDev)
{
    //One pixel, two pixel
#if 1
MY_LOGD("current only one pixel mode only");
