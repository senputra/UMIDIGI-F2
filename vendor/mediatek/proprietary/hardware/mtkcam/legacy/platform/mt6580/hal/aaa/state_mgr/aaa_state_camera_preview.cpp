...
sendIntent(intent2type<eIntent_VsyncUpdate>)
{
    ...
    MY_LOG_IF(1
            , "[StateCameraPreview::sendIntent<eIntent_VsyncUpdate>] enter, frameCnt=%d aeMode=%d\n"
            , m_pStateMgr->getFrameCount(), IAeMgr::getInstance().getAEMode(m_SensorDevId));

    if(m_pHal3A->mbRestore2A)
    {
        IAeMgr::getInstance().doRestoreAEInfo(m_SensorDevId, MTRUE);
        IAwbMgr::getInstance().restore(m_SensorDevId);
        m_pHal3A->mbRestore2A = MFALSE;
    }
    //BufInfo_T rBufInfo;
    BufInfo_T & rBufInfo = *(BufInfo_T*)(m_pStateMgr->mpAAOBuf);
    if (m_pStateMgr->getFrameCount() < 0 && (IAeMgr::getInstance().getAEMode(m_SensorDevId) != 0)) {// AAO statistics is not ready
...
sendIntent(intent2type<eIntent_CaptureEnd>)
{
    // Update frame count
    m_pStateMgr->updateFrameCount();
    MY_LOG("sendIntent(intent2type<eIntent_CaptureEnd>) line(%d), frame(%d)",__LINE__, m_pStateMgr->getFrameCount());
...
if (ENABLE_3A_GENERAL & m_pHal3A->m_3ACtrlEnable) {
    // One-shot AWB
    MINT32 i4SceneLV = IAeMgr::getInstance().getLVvalue(m_SensorDevId, MFALSE);
    IAwbMgr::getInstance().doCapAWB(m_SensorDevId, i4SceneLV, reinterpret_cast<MVOID *>(rBufInfo.virtAddr));
    MY_LOG("IAwbMgr::getInstance().doCapAWB() END");
    //pass WB gain info
    AWB_OUTPUT_T _a_rAWBOutput;
    IAwbMgr::getInstance().getAWBOutput(m_SensorDevId, _a_rAWBOutput);
    IAeMgr::getInstance().doCapFlare(m_SensorDevId, reinterpret_cast<MVOID *>(rBufInfo.virtAddr),FlashMgr::getInstance().isFlashOnCapture(m_SensorDevId) );
