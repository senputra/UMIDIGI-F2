...
#define LOG_TAG "Iop/P2PPWp.TD_FrmB"
//
...
DECLARE_DBG_LOG_VARIABLE(P2PipeWrapper_Thread_FrmB);
#undef LOG_VRB
#undef LOG_DBG
#undef LOG_INF
#undef LOG_WRN
#undef LOG_ERR
#undef LOG_AST
#define LOG_VRB(fmt, arg...)        do { if (P2PipeWrapper_Thread_FrmB_DbgLogEnable_VERBOSE && MTKCAM_LOGENABLE_DEFAULT) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define LOG_DBG(fmt, arg...)        do { if (P2PipeWrapper_Thread_FrmB_DbgLogEnable_DEBUG && MTKCAM_LOGENABLE_DEFAULT) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#define LOG_INF(fmt, arg...)        do { if (P2PipeWrapper_Thread_FrmB_DbgLogEnable_INFO && MTKCAM_LOGENABLE_DEFAULT ) { BASE_LOG_INF(fmt, ##arg); } } while(0)
#define LOG_WRN(fmt, arg...)        do { if (P2PipeWrapper_Thread_FrmB_DbgLogEnable_WARN   ) { BASE_LOG_WRN(fmt, ##arg); } } while(0)
#define LOG_ERR(fmt, arg...)        do { if (P2PipeWrapper_Thread_FrmB_DbgLogEnable_ERROR  ) { BASE_LOG_ERR(fmt, ##arg); } } while(0)
#define LOG_AST(cond, fmt, arg...)  do { if (P2PipeWrapper_Thread_FrmB_DbgLogEnable_ASSERT ) { BASE_LOG_AST(cond, fmt, ##arg); } } while(0)


using namespace NSCam;
using namespace NSIoPipe;
using namespace NSPostProc_FrmB;
extern PortIdxMappingP2toAll mPortIdxMappingP2toAll_FrmB[EPostProcPortIdx_NUM];
...
{
    resetDequeVariables();
    DBG_LOG_CONFIG(imageio, P2PipeWrapper_Thread_FrmB);
...
waitDequeVSSBufAgain()
{
    ...
    ret = mVssCond.waitRelative(mVssCondMtx, nsTimeoutToWait);
    ...
    MUINT32 callerID)
{
    ...
    if(bufferListTag==EBufferListTag_Batch)
    {
        ...
        if(isIPRaw)
        {
            LOG_INF("IP_RAW flush (%d/%d)",p2dequeCQidx + NSIspDrv_FrmB::ISP_DRV_CQ01,p2dupCQidx);
            mpPostProcPipe->sendCommand(0x1103,p2dequeCQidx + NSIspDrv_FrmB::ISP_DRV_CQ01,0,p2dupCQidx);
        }
...
dequeueJudgement()
{
    ...
    int dequeP2Cq=bufNode.p2cqIdx-NSIspDrv_FrmB::ISP_DRV_CQ01;
    ...
    switch(bufNode.bufTag)
    {
        ...
        if(deqret==eIspRetStatus_VSS_NotReady)
        {
            ...
            {    //still have other blocking wait buffer
                it++;
                bufNode=(*it);
                dequeP2Cq=bufNode.p2cqIdx-NSIspDrv_FrmB::ISP_DRV_CQ01;
...
)
{
    ...
    MUINT32 a=0,b=0;
    if(bufNode.portEn == ((1<<NSCam::NSIoPipe::NSPostProc_FrmB::EPostProcPortIdx_IMGI) | 1<<NSCam::NSIoPipe::NSPostProc_FrmB::EPostProcPortIdx_IMGO))
    {
        isIPRaw=MTRUE;
    }

    //[1] dequeue dma buffer
    for(int i = NSCam::NSIoPipe::NSPostProc_FrmB::EPostProcPortIdx_WDMAO ; i > NSCam::NSIoPipe::NSPostProc_FrmB::EPostProcPortIdx_IMGI ; i--)
    {   //dequeue from wdma, wrot,jpeg,img2o,img3o,...
        if( (int)((bufNode.portEn & (1 << i)) >> i))
        {
            portID.index=mPortIdxMappingP2toAll_FrmB[i].ePortIndex;
        ...
        for(int i = NSCam::NSIoPipe::NSPostProc_FrmB::EPostProcPortIdx_IMGI ; i > NSCam::NSIoPipe::NSPostProc_FrmB::EPostProcPortIdx_NOSUP ; i--)
        {
        if( (int)((bufNode.portEn & (1 << i)) >> i))
        {
                    portID.index=mPortIdxMappingP2toAll_FrmB[i].ePortIndex;
