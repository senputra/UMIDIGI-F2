...
//#undef LOG_TAG
//#endif
#define CAM_CAL_LOG_TAG "CAM_COMM_CAL"


#define MHAL_LOG(fmt, arg...)    ALOGD(CAM_CAL_LOG_TAG fmt, ##arg)
...
MUINT32 CamCommDoISPSlimShadingLoad(PCAM_CAL_SHADING_STRUCT pLscTbl);
MUINT32 CamCommDoISPDynamicShadingLoad(PCAM_CAL_SHADING_STRUCT pLscTbl);
MUINT32 CamCommDoISPFixShadingLoad(PCAM_CAL_SHADING_STRUCT pLscTbl);
MUINT32 CamCommDoISPSensorShadingLoad(PCAM_CAL_SHADING_STRUCT pLscTbl);
MUINT32 CamCommDoISPSvdShadingLoad(PCAM_CAL_SHADING_STRUCT pLscTbl);

CAM_COMM_LSC_CONVERT lCamCommConvertTable[CAM_CAL_LSC_ID_NO]={
    { CAM_CAL_LSC_ID_SLIM        , CamCommDoISPSlimShadingLoad},
    { CAM_CAL_LSC_ID_DYNAMIC        , CamCommDoISPDynamicShadingLoad},
    { CAM_CAL_LSC_ID_FIX            , CamCommDoISPFixShadingLoad},
    { CAM_CAL_LSC_ID_SENSOR        , CamCommDoISPSensorShadingLoad},
    { CAM_CAL_LSC_ID_SVD            , CamCommDoISPSvdShadingLoad}
};
...
{
    MUINT32 i;
    PCAM_CAL_SHADING_STRUCT lpLscTbl = pLscTbl;
    if(pLscTbl!=NULL)
    {
        for(i=0;i<CAM_CAL_LSC_ID_NO;i++)
        {
            if(pLscTbl->ShadingID == lCamCommConvertTable[i].ShadingID)
               {
                 lCamCommConvertTable[i].LscConvert((PCAM_CAL_SHADING_STRUCT)lpLscTbl);
...
MUINT32 CamCommDoISPSlimShadingLoad(PCAM_CAL_SHADING_STRUCT pLscTbl)
{
    PCAM_CAL_SHADING_STRUCT pEerom_data = pLscTbl;
    TBL_INFO_T tbl_info;
    MUINT32 uiSlimShadingBuffer_PV[CAM_CAL_LSC_DATA_SIZE_SLIM_LSC1]={0};
