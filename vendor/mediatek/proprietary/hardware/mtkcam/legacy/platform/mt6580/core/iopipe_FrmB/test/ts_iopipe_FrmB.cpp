...
#undef printf
#define printf(fmt, arg...)       ALOGD(fmt, ##arg);
...
class _test_camio
{
    _test_camio()
    {
        ...
        memset(&m_semThread, 0, sizeof(sem_t));
    };
    ~_test_camio(){};
    public:
    ...
    while(1){
        ...
        if(_this->m_pNormalP_FrmB->deque(dequeBufInfo) == MTRUE){
            ...
            if (shouldPrint<1)
            {
                ...
                {
                    // C.K. Modify
                    //sprintf(filename, "/sdcard/pictures/P1_%d_%d_%d_%d_%d_%d.raw", _this->m_enablePort, dequeBufInfo.mvOut.at(i).mMetaData.mMagicNum_hal,
                        //dequeBufInfo.mvOut.at(i).mBuffer->getImgSize().w,
                        //dequeBufInfo.mvOut.at(i).mBuffer->getImgSize().h,
                    sprintf(filename, "/sdcard/pictures/P1_%d_%d_%d_%d_%d_%d.raw",
                        _this->m_enablePort,
                        dequeBufInfo.mvOut.at(i).mMetaData.mMagicNum_hal,
                        dequeBufInfo.mvOut.at(i).mMetaData.mDstSize.w,
                        dequeBufInfo.mvOut.at(i).mMetaData.mDstSize.h,
                        dequeBufInfo.mvOut.at(i).mBuffer->getBufStridesInBytes(0),
                        shouldPrint);
            ...
            for(int i=0;i<_replace.mvOut.size();i++){
                ...
                printf("[E_L enque]: dma:0x%x, PA:0x%x, crop:0x%x,0x%x,0x%x,0x%x, size:0x%x,0x%x, mag:0x%x, sofIdx:0x%x, bufidx:0x%x\n",\
                ...
                _replace.mvOut.at(i).FrameBased.mSOFidx,\
            ...
            */
            ...
            for(int i=0;i<dequeBufInfo.mvOut.size();i++){
                ...
                //_replace.mvOut.at(i).FrameBased.mSOFidx = 0;
        ...
        if (_loop >= 30)
        {
            printf("Stop @ frame 30\n");
            _this->m_bStop = MTRUE;
            break;
        }
#endif
    }
    ALOGD("_infiniteloop End-1");
    ::sem_post(&_this->m_semThread);
    ALOGD("_infiniteloop End-2");
    return NULL;
}
...
int main_iopipe(int testcaseType,int testcaseNum)
{
    ...
    for (int i = 0; i < 3; i++){
        ...
        mpIspDrv->dumpCQTable((ISP_DRV_CQ_ENUM)ISP_DRV_CQ0,0, i,
            (ISP_DRV_CQ_CMD_DESC_STRUCT*)mpIspDrv->getCQDescBufVirAddr(ISP_DRV_CQ0,0,i),
                mpIspDrv->getCQVirBufVirAddr(ISP_DRV_CQ0,0,i));
    }
    ...
    */
    ...
    //getchar();
    ALOGD("call stopThread");
    pCamio->stopThread();
    printf("!!!!!!!! thread stoped... !!!!!!!!\n");
    ALOGD("call stopThread end");
    mpCamIO->stop();
    printf("!!!!!!!! isp stoped... !!!!!!!!\n");
    ALOGD("call mpCamIO->stop end");


    mpCamIO->uninit();
    mpCamIO->destroyInstance(LOG_TAG);
	pCamio->destroy();
    ALOGD("call mpCamIO->uninit end");
    //poweroff sensor
    pSensorHalObj->powerOff(LOG_TAG,1, &sensorArray[0]);
    ALOGD("pSensorHalObj->powerOff end");
    #if USE_MM_DVFS == 1
    printf("MM DVFS : LPM\n");
