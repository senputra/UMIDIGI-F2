...
MINT32 EisHalImp::Init()
{
    ...
    property_get("vendor.debug.eis.dumpR", value, "0");
    ...
    if( g_debugDump_R >= 3 )
    {
        ...
        if( mDumpFile == NULL )
        {
            EIS_WRN_R("Open file fail! (errno=%d)", errno);
        }
    }
...
MINT32 EisHalImp::Uninit()
{
    ...
    if(mUsers == 0)    // there is no user
    {
        ...
        if( mDumpFile != NULL )
        {
            fflush(mDumpFile);
            fclose(mDumpFile);
            EIS_LOG_R("Close file successfully");
            mDumpFile = NULL;
        }
...
MINT32 EisHalImp::ConfigEis(const EIS_HAL_CONFIG_DATA &aEisConfig)
{
    ...
    if(mIsEisConfig == 0)
    {
        ...
        if(g_debugDump_R >= 2)
        {
            ...
            if( m_pEisAlg->EisFeatureCtrl(EIS_FEATURE_GET_DEBUG_INFO, NULL, &lmvMemSize) != S_EIS_OK ||
                lmvMemSize == 0 )
            {
                EIS_ERR_R("EisFeatureCtrl(EIS_FEATURE_GET_DEBUG_INFO) fail(0x%x)",err);
    ...
    if(g_debugDump_R >= 1)
    {
        ...
        EIS_LOG_R("adv_shake_ext(%u)",eisAlgoInitData.eis_tuning_data.adv_shake_ext);
        EIS_LOG_R("stabilization_strength(%f)",eisAlgoInitData.eis_tuning_data.stabilization_strength);
...
MINT32 EisHalImp::DoEis(EIS_HAL_CONFIG_DATA *apEisConfig,MINT64 aTimeStamp)
{
    ...
    else
    {
        ...
        EIS_LOG_R("Sensor(%u),LMVIn(%u,%u),P1Target(%u,%u)=>GMV(%d,%d),CMV(%d,%d)",
                  mSensorIdx, mEisInput_W, mEisInput_H, mP1Target_W, mP1Target_H,
                  mGMV_X, mGMV_Y, mCmvX_Int, mCmvY_Int);

        if(g_debugDump_R >= 2)
        {
            EIS_LOG_R("DivH(%u),DivV(%u),EisWinNum(%u)", mEisAlgoProcData.DivH, mEisAlgoProcData.DivV, mEisAlgoProcData.EisWinNum);
...
MVOID EisHalImp::GetEisCustomize(EIS_TUNING_PARA_STRUCT *a_pDataOut)
{
    ...
    a_pDataOut->adv_shake_ext                    = customSetting.adv_shake_ext;
    a_pDataOut->stabilization_strength           = customSetting.stabilization_strength;
...
MVOID EisHalImp::DumpStatistic(const EIS_STATISTIC_STRUCT &aEisStat)
{
    ...
    for( int i = 0; i < EIS_MAX_WIN_NUM; ++i )
    {
        totalMVX += aEisStat.i4LMV_X[i];
        totalMVY += aEisStat.i4LMV_Y[i];
    }

    MPoint rawLMV(totalMVX/EIS_MAX_WIN_NUM, totalMVY/EIS_MAX_WIN_NUM);

    EIS_LOG_R("GMV(%d,%d),CMV(%d,%d):Raw(%d,%d)",
            mGMV_X, mGMV_Y, mCmvX_Int, mCmvY_Int, rawLMV.x, rawLMV.y);

    if( g_debugDump_R >= 3 )
    {
        for( int i = 0; i < EIS_MAX_WIN_NUM; ++i )
        {
            EIS_LOG_R("MB%d%d,(LMV_X,LMV_Y)=(%d,%d),(LMV_X2,LMV_Y2)=(%d,%d),MinSAD(%u),(NewTrust_X,NewTrust_Y)=(%u,%u),MinSAD2(%u),AvgSAD(%u)",
                     (i/4), (i%4),
                     aEisStat.i4LMV_X[i], aEisStat.i4LMV_Y[i],
                     aEisStat.i4LMV_X2[i], aEisStat.i4LMV_Y2[i],
                     aEisStat.SAD[i],
                     aEisStat.NewTrust_X[i], aEisStat.NewTrust_Y[i],
                     aEisStat.SAD2[i],
                     aEisStat.AVG_SAD[i]);
        }
        ...
        if( mDumpFile != NULL )
        {
            fprintf(mDumpFile, "GMV(%d,%d),CMV(%d,%d):Raw(%d,%d)\n",
                    mGMV_X, mGMV_Y, mCmvX_Int, mCmvY_Int, rawLMV.x, rawLMV.y);
        }
