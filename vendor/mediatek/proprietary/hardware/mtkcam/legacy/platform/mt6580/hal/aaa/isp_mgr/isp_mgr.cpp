...
apply(EIspProfile_T eIspProfile, isp_reg_t* pReg)
{
    ...
    property_get("vendor.isp.obc.disable", value1, "-1");
    MINT32 disable = atoi(value1);
...
apply(EIspProfile_T eIspProfile)
{
    ...
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.bnr.disable", value1, "-1");
    MINT32 disable = atoi(value1);
...
apply(EIspProfile_T eIspProfile)
{
    ...
    MY_LOG("[%s] eSwScn(%d), fgOnOff(%d)", __FUNCTION__, eSwScn, fgOnOff);
    dumpRegInfo();
...
apply(EIspProfile_T eIspProfile, isp_reg_t* pReg)
{
    ...
    MY_LOG("[%s] P2, fgOnOff(%d)", __FUNCTION__, fgOnOff);
    dumpRegInfo();
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.lock.wb.gain", value1, "-1");
    MBOOL bLockWbGain = atoi(value1);

    MY_LOG("ISP_MGR_PGN_T setIspAWBGain = %d %d %d, lock(%d)\n", rIspAWBGain.i4R, rIspAWBGain.i4G, rIspAWBGain.i4B, bLockWbGain);

    if(bLockWbGain!=-1 && bLockWbGain==1)
    {
        m_rIspAWBGain.i4R = 512;
        m_rIspAWBGain.i4G = 512;
        m_rIspAWBGain.i4B = 1088;
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.lock.flare", value1, "-1");
    MBOOL bLockFlare = atoi(value1);

    if((bLockFlare!=-1 && bLockFlare==1) || m_bLockFlare)
    {
        m_i4FlareGain = 512;
        m_i4FlareOffset = 0;
    }
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.pgn.disable", value1, "-1");
    MINT32 disable = atoi(value1);
    ...
    writeRegs(static_cast<RegInfo_T*>(m_pRegInfo), m_u4RegInfoNum, pReg);
    ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN1, PGN_EN, bPGN);
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.cfa.disable", value1, "-1");
    MINT32 disable = atoi(value1);
    ...
    writeRegs(static_cast<RegInfo_T*>(m_pRegInfo), m_u4RegInfoNum, pReg);
    ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN1, CFA_EN, bCFA);
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.ccm.disable", value1, "-1");
    MINT32 disable = atoi(value1);
    ...
    writeRegs(static_cast<RegInfo_T*>(m_pRegInfo), m_u4RegInfoNum, pReg);
    ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN1, G2G_EN, bCCM);
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.g2c.disable", value1, "-1");
    MINT32 disable = atoi(value1);
    ...
    writeRegs(static_cast<RegInfo_T*>(m_pRegInfo), m_u4RegInfoNum, pReg);
    ISP_WRITE_ENABLE_BITS(pReg, CAM_CTL_EN2, G2C_EN, bG2C);
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.nbc.disable", value1, "-1");
    MINT32 disable = atoi(value1);
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.pca.disable", value1, "-1");
    MINT32 disable = atoi(value1);
...
{
    char value1[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.ee.disable", value1, "-1");
    MINT32 disable = atoi(value1);
...
MBOOL
ISP_MGR_AE_RAWPREGAIN2_T::
setRAWPregain2(AWB_GAIN_T& rAWBRAWPregain2, MINT32 i4SensorIndex)
{
...
#warning "FIXME"
        ...
        INormalPipe_FrmB* pPipe = INormalPipe_FrmB::createInstance(m_i4SensorIndex, "isp_mgr_ae_rawpregain2");

        // get module handle
        if (MFALSE == pPipe->sendCommand(NSImageio_FrmB::NSIspio_FrmB::EPIPECmd_GET_MODULE_HANDLE,
                               NSImageio_FrmB::NSIspio_FrmB::EModule_AE, (MUINTPTR)&handle, (MUINTPTR)(&("isp_mgr_ae_rawpregain2")))) {
            //Error Handling
            MY_ERR("EPIPECmd_GET_MODULE_HANDLE fail");
            goto lbExit;
        }
        ...
        IOPIPE_SET_MODUL_REG(handle, CAM_AE_RAWPREGAIN2_0, m_rIspRegInfo[ERegInfo_CAM_AE_RAWPREGAIN2_0].val);
        IOPIPE_SET_MODUL_REG(handle, CAM_AE_RAWPREGAIN2_1, m_rIspRegInfo[ERegInfo_CAM_AE_RAWPREGAIN2_1].val);
#endif
    ...
    lbExit:
        ...
        // release handle
        if (MFALSE==pPipe->sendCommand(NSImageio_FrmB::NSIspio_FrmB::EPIPECmd_RELEASE_MODULE_HANDLE, handle, (MUINTPTR)(&("isp_mgr_ae_rawpregain2")), MNULL))
        {
            // Error Handling
            MY_ERR("EPIPECmd_SET_MODULE_CFG_DONE fail");
        }

        pPipe->destroyInstance("isp_mgr_ae_rawpregain2");
