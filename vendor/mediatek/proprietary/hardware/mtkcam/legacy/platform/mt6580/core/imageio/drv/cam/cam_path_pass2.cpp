...
else    // GDMA Mode.
{
    ...
    // [mdpmgr] collect crop information
    ...
    //need >> 11 to guarantee 0-19 bit for tpipe due to mdp do not do any shift in d2 architecture but in original 82
    DMAImgi.dma_cfg.crop.floatX=(p_parameter->imgi.crop.floatX)>>11;
    DMAImgi.dma_cfg.crop.floatY=(p_parameter->imgi.crop.floatY)>>11;
...
int CamPathPass2::getTpipePerform( struct CamPathPass2Parameter* p_parameter )
{
    ...
    if( p_parameter->en_Top.dma & (CAM_CTL_DMA_EN_DISPO_EN|CAM_CTL_DMA_EN_VIDO_EN)){
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
   ...
   // tdriPipe.tdri.top.tcm_load_en =p_parameter->fmt_sel.bit_field.cam_in_fmt;
    ISP_PATH_DBG("[Top]scenario(%d) mode(%d) debug_sel(%d) pixel_id(%d) cam_in_fmt(%d),mdp_crop_en(%d)",
        tdriPipe.tdri.top.scenario,tdriPipe.tdri.top.mode,tdriPipe.tdri.top.debug_sel,
        tdriPipe.tdri.top.pixel_id,tdriPipe.tdri.top.cam_in_fmt,tdriPipe.tdri.top.mdp_crop_en);

    //do not support imgo out and img2o out for pass2 in camera1
    tdriPipe.tdri.imgo.imgo_stride=0;  ///< imgo stride
    ...
    tdriPipe.tdri.imgo.imgo_xsize=0;   ///< imgo x size
    tdriPipe.tdri.imgo.imgo_ysize=0;   ///< imgo y size
    ...
    tdriPipe.tdri.img2o.img2o_stride=0;  ///< img2o stride
    ...
    tdriPipe.tdri.img2o.img2o_xsize=0;   ///< img2o x size
    tdriPipe.tdri.img2o.img2o_ysize=0;   ///< img2o y size
...
int CamPathPass2::dequeueBuf( MUINT32 dmaChannel ,stISP_FILLED_BUF_LIST& bufInfo )
{
    ...
    {
        ISP_PATH_ERR("ERROR:dequeueHwBuf(0x%x)",dmaChannel);
        return -1;
    }
