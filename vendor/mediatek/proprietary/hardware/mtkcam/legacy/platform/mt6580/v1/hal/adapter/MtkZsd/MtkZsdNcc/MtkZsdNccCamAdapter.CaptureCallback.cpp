...
{
    MY_LOGD("timestamp(%lld), size/buf=%d/%p", (long long)i8Timestamp, u4PostviewSize, puPostviewBuf);
#if 1
    //
    ...
    if  ( u4PostviewSize != pImgBuf->getBufSize() )
    {
        MY_LOGW(
            ...
            u4PostviewSize, (int)pImgBuf->getBufSize(), pImgBuf->getImgFormat().string(),
...
#if 1   //defined(MTK_CAMERA_BSP_SUPPORT)
        uint32_t const u4Type = MTK_CAMERA_MSG_EXT_DATA
#else
        uint32_t const u4Type = CAMERA_MSG_COMPRESSED_IMAGE
#endif
    )
        : Thread()
...
private:
    ...
    threadLoop()
    {
        switch(mu4Type)
        {
            ...
            {
                CAM_LOGD("(%d)[%s] CAMERA_MSG_RAW_IMAGE_NOTIFY", ::gettid(), mpszThreadName);
                mpCamMsgCbInfo->mNotifyCb(CAMERA_MSG_RAW_IMAGE_NOTIFY, 0, 0, mpCamMsgCbInfo->mCbCookie);
                break;
            }
            case CAMERA_MSG_RAW_IMAGE:
            {
                CAM_LOGD("(%d)[%s] CAMERA_MSG_RAW_IMAGE", ::gettid(), mpszThreadName);
                mpCamMsgCbInfo->mDataCb(CAMERA_MSG_RAW_IMAGE, mpImage, 0, NULL, mpCamMsgCbInfo->mCbCookie);
...
{
    MY_LOGD("timestamp(%lld), size/buf=%d/%p", (long long)i8Timestamp, u4RawImgSize, puRawImgBuf);
    //
    if  ( msgTypeEnabled(CAMERA_MSG_RAW_IMAGE_NOTIFY) )
    {
        MY_LOGD("CAMERA_MSG_RAW_IMAGE_NOTIFY");
        sp<Thread> pThread = new ZipImageCallbackThread(mpCamMsgCbInfo, NULL, 0, false, getParamsManager()->getShotMode(), CAMERA_MSG_RAW_IMAGE_NOTIFY);
        if  ( pThread == 0 || pThread->run("CamAdapter::onCB_RawImage") != OK )
        {
    ...
    if  ( msgTypeEnabled(CAMERA_MSG_RAW_IMAGE) )
    {
        ...
        if  ( ! u4RawImgSize || ! puRawImgBuf )
        {
            ...
            if  ( pmem )
            {
                sp<Thread> pThread = new ZipImageCallbackThread(mpCamMsgCbInfo, pmem, 0, false, getParamsManager()->getShotMode(), CAMERA_MSG_RAW_IMAGE);
                if  ( pThread == 0 || pThread->run("CamAdapter::onCB_RawImage") != OK )
                {
                    MY_LOGW("Fail to run ZipImageCallbackThread (%p)", pThread.get());
                }
...
)
{
    MY_LOGD(
        ...
        (long long)i8Timestamp, u4BitstreamSize, puBitstreamBuf, u4HeaderSize, puHeaderBuf, u4CallbackIndex, fgIsFinalImage
    );
    //
