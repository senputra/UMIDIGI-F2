...
Hal3AYuv*
Hal3AYuv::
createInstance(MINT32 const i4SensorDevId, MINT32 const i4SensorOpenIndex)
{
...
Hal3AYuv::Hal3AYuv()
    ...
    , m_AFzone()
    , m_AEzone()
    , m_sAFAREA()                     //[MAX_FOCUS_AREAS];      //
    , m_sAEAREA()                   //[MAX_METERING_AREAS];   //
    , m_imageXS(0)
    , m_imageYS(0)
    ...
    , m_semAFThreadStart()
{
    memset(&m_AEFlashlightInfo, 0, sizeof(m_AEFlashlightInfo));
}
...
MRESULT
Hal3AYuv::
initYuv(MINT32 i4SensorDev, MINT32 i4SensorOpenIndex)
{
    ...
    init(i4SensorDev, i4SensorOpenIndex);
    ...
    GET_PROP("vendor.debug.aaa_hal_yuv.log", "0", _bEnableMyLog);
...
MBOOL Hal3AYuv::postCommandDerived(ECmd_T const eCmd, MINTPTR const i4Arg)
{
    ...
    {
        //Init bAELockSupp, bAWBLockSupp
        FeatureParam_T FeatureParam = {}; //just for getSupportedParams() to init bAELockSupp, bAWBLockSupp
        getSupportedParams(FeatureParam);
...
MRESULT Hal3AYuv::EnableAFThread(MINT32 a_bEnable)
{
    ...
    if (a_bEnable)
    {
        if (m_bAFThreadLoop == 0)
        {
            ...
            pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_RR, NICE_CAMERA_AF};
...
getCurrResult(MUINT32 i4FrmId, Result_T& rResult) const
{
    ...
    // AE
    if(m_i4State == ECmd_PrecaptureStart)
    {
        if(m_bReadyToCapture)
        {
            if(m_i4AeStatus == SENSOR_AE_LOCKED || m_rParam.bIsAELock)
                rResult.u1AeState = MTK_CONTROL_AE_STATE_LOCKED;
            else
    ...
    else if(m_i4AeStatus == SENSOR_AE_LOCKED || m_rParam.bIsAELock)
        rResult.u1AeState = MTK_CONTROL_AE_STATE_LOCKED;
    else
        rResult.u1AeState = MTK_CONTROL_AE_STATE_CONVERGED;
    ...
    memset(&cur, 0, sizeof(SENSOR_AE_AWB_CUR_STRUCT));

    m_pSensorHal->sendCommand(m_i4SensorDev, SENSOR_CMD_GET_YUV_CURRENT_SHUTTER_GAIN_AWB_GAIN, (MUINTPTR)&cur, 0, 0);
    m_pSensorHal->sendCommand(m_i4SensorDev, SENSOR_CMD_GET_SENSOR_ROLLING_SHUTTER, (MUINTPTR)&tline, (MUINTPTR)&vsize, 0);

    rResult.i8SensorExposureTime = cur.SensorAECur.AeCurShutter * (33000000 / 488);/*NS*/
    if(m_rParam.i4MinFps==m_rParam.i4MaxFps&&m_rParam.i4MaxFps>0){
            MUINT32 u4FrameRate = m_rParam.i4MaxFps/1000;
            rResult.i8SensorFrameDuration = 1000000000L / u4FrameRate;
    } else
        rResult.i8SensorFrameDuration = 33000000L;
    rResult.i4SensorSensitivity = mapEnumToISO(m_rParam.u4IsoSpeedMode);
    rResult.i8SensorRollingShutterSkew = tline * vsize;

    // AWB
    switch(m_i4AwbStatus)
    {
        case SENSOR_AWB_BALANCING:
            rResult.u1AwbState = MTK_CONTROL_AWB_STATE_SEARCHING;
            break;
        case SENSOR_AWB_IDLE:
        case SENSOR_AWB_BALANCED:
            rResult.u1AwbState = MTK_CONTROL_AWB_STATE_CONVERGED;
            break;
        case SENSOR_AWB_LOCKED:
            rResult.u1AwbState = MTK_CONTROL_AWB_STATE_LOCKED;
            break;
        default:
            break;
    }

    if(m_rParam.bIsAWBLock)
        rResult.u1AwbState = MTK_CONTROL_AWB_STATE_LOCKED;

    for(MINT32 i = 0; i < 9; i++)
        rResult.fColorCorrectMat[i] = m_rParam.fColorCorrectMat[i];
    for(MINT32 i = 0; i < 4; i++)
        rResult.fColorCorrectGain[i] = m_rParam.fColorCorrectGain[i];
    ...
    if(!m_bIsdummylens)
    {
        ...
        rResult.u1AfState = MTK_CONTROL_AF_STATE_FOCUSED_LOCKED;
    MY_LOG("[%s] #(%d) AWB(%d) AE(%d) AF(%d)", __FUNCTION__, rResult.i4FrmId, rResult.u1AwbState, rResult.u1AeState, rResult.u1AfState);
...
get3AEXIFInfo(EXIF_3A_INFO_T& rExifInfo) const
{
    ...
    m_pSensorHal->sendCommand(m_i4SensorDev, SENSOR_CMD_GET_YUV_CURRENT_SHUTTER_GAIN_AWB_GAIN, (MUINTPTR)&cur, 0, 0);
    m_pSensorHal->sendCommand(m_i4SensorDev,SENSOR_CMD_GET_YUV_EXIF_INFO,(MUINTPTR)&rSensorInfo,0,0);

    rExifInfo.u4FNumber = rSensorInfo.FNumber>0 ? rSensorInfo.FNumber : 28;
    rExifInfo.u4FocalLength = 350;
    //rExifInfo.u1SceneMode = m_rParam.u4SceneMode;
    rExifInfo.u4AWBMode = m_rParam.u4AwbMode;
    rExifInfo.u4CapExposureTime = cur.SensorAECur.AeCurShutter * (33000000 / 488) / 1000;
    rExifInfo.u4FlashLightTimeus = m_bExifFlashOn;
    rExifInfo.u4AEISOSpeed = mapEnumToISO(m_rParam.u4IsoSpeedMode);    // in ISO value (not enum)
...
get3AStatusFromSensor()
{
    m_pSensorHal->sendCommand(m_i4SensorDev, SENSOR_CMD_GET_YUV_AF_STATUS, (MUINTPTR)&m_i4AfStatus, 0, 0);
    m_pSensorHal->sendCommand(m_i4SensorDev, SENSOR_CMD_GET_YUV_AE_STATUS, (MUINTPTR)&m_i4AeStatus, 0, 0);
    m_pSensorHal->sendCommand(m_i4SensorDev, SENSOR_CMD_GET_YUV_AWB_STATUS, (MUINTPTR)&m_i4AwbStatus, 0, 0);

    MY_LOG_IF(1, "[%s] AF(%d), AE(%d), AWB(%d)", __FUNCTION__, m_i4AfStatus, m_i4AeStatus, m_i4AwbStatus);
    return 0;
}
