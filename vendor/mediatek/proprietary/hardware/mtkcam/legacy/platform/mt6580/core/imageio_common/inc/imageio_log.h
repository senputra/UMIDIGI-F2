...
///////////////////////////////////////////////////////////////////////////
    ...
    #define BASE_LOG_VRB(fmt, arg...)       do { if (MTKCAM_LOGENABLE_DEFAULT) {ALOGV( "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)    // <Verbose>: Show more detail debug information. E.g. Entry/exit of private function; contain of local variable in function or code block; return value of system function/API...
    #define BASE_LOG_DBG(fmt, arg...)       do { if (MTKCAM_LOGENABLE_DEFAULT) {ALOGD( "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)    // <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
    #define BASE_LOG_INF(fmt, arg...)       do { if (MTKCAM_LOGENABLE_DEFAULT) {ALOGI( "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)    // <Info>: Show general system information. Like OS version, start/end of Service...
    #define BASE_LOG_WRN(fmt, arg...)       ALOGW( "[%s] WARNING: " fmt NEW_LINE_CHAR, __func__, ##arg)    // <Warning>: Some errors are encountered, but after exception handling, user won't notice there were errors happened.
    #define BASE_LOG_ERR(fmt, arg...)       ALOGE( "[%s, %s, line%04d] ERROR: " fmt NEW_LINE_CHAR, __FILE__, __func__, __LINE__, ##arg)    // When MP, will only show log of this level. // <Fatal>: Serious error that cause program can not execute. <Error>: Some error that causes some part of the functionality can not operate normally.
...
#else
#define PIPE_VRB(fmt, arg...)        do { if (pipe_DbgLogEnable_VERBOSE && MTKCAM_LOGENABLE_DEFAULT) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define PIPE_DBG(fmt, arg...)        do { if (pipe_DbgLogEnable_DEBUG && MTKCAM_LOGENABLE_DEFAULT) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#define PIPE_INF(fmt, arg...)        do { if (pipe_DbgLogEnable_INFO && MTKCAM_LOGENABLE_DEFAULT ) { BASE_LOG_INF(fmt, ##arg); } } while(0)
...
//Enable debug under user-debug and eng
#if 0
#define ISP_PATH_VRB(fmt, arg...)    CAM_LOGV("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_PATH_DBG(fmt, arg...)    CAM_LOGD("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_PATH_INF(fmt, arg...)    CAM_LOGI("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_PATH_WRN(fmt, arg...)    CAM_LOGW("[%s] " fmt, __FUNCTION__, ##arg)
#define ISP_PATH_ERR(fmt, arg...)    CAM_LOGE("[%s] " fmt, __FUNCTION__, ##arg)
#else
#define ISP_PATH_VRB(fmt, arg...)        do { if (path_DbgLogEnable_VERBOSE && MTKCAM_LOGENABLE_DEFAULT) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define ISP_PATH_DBG(fmt, arg...)        do { if (path_DbgLogEnable_DEBUG  && MTKCAM_LOGENABLE_DEFAULT ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#define ISP_PATH_INF(fmt, arg...)        do { if (path_DbgLogEnable_INFO  && MTKCAM_LOGENABLE_DEFAULT  ) { BASE_LOG_INF(fmt, ##arg); } } while(0)
