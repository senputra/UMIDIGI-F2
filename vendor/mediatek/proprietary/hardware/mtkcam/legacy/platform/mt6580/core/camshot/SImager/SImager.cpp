...
{
    //FUNCTION_LOG_START;
    MY_LOGD("roi (x, y, w, h) = (%d, %d, %d, %d)", rROI.x, rROI.y, rROI.w, rROI.h);
...
{
    //FUNCTION_LOG_START;
    MY_LOGD("size (w, h) = (%d, %d)", u4Width, u4Height);
...
{
    //FUNCTION_LOG_START;
    MY_LOGD("enc param (SOI, Quality) = (%d, %d)", u4IsSOI, u4Quality);
...
{
    FUNCTION_LOG_START;
    MY_LOGD("stride align = (%d, %d, %d)", u4StrideAlign[0], u4StrideAlign[1], u4StrideAlign[2]);
