...
Blending(void)
{
    ...
    // post view (skip postview callback if the display format is invalid
    if(mHdrRound==1 && (mPostviewFormat != eImgFmt_UNKNOWN)) {
        ret = ret
...
lbCaptureDone:
#if (HDR_SPEEDUP_BURSTSHOT == 0) && (HDR_SPEEDUP_MALLOC == 1)
    // ret only affects in with these macros defined.
    if(!ret) goto lbExit;
#endif
