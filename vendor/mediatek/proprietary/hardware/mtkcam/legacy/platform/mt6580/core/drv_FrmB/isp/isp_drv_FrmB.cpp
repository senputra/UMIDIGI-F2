...
#if 0
#define LOG_VRB(fmt, arg...)    CAM_LOGV("[%s] " fmt, __FUNCTION__, ##arg)
#define LOG_DBG(fmt, arg...)    CAM_LOGD("[%s] " fmt, __FUNCTION__, ##arg)
#define LOG_INF(fmt, arg...)    CAM_LOGI("[%s] " fmt, __FUNCTION__, ##arg)
...
MBOOL IspDrvImp::dumpP2DebugLog(IspDumpDbgLogP2Package* pP2Package)
{
    ...
    // avoid "deference after null check"
	if(pP2Package->pIspReg == NULL){
		LOG_INF("pIsgReg is NULL");
		}
	else if(pP2Package->pIspReg || pP2Package->pIspTpipeCfgInfo){
...
MBOOL IspDrvImp::waitIrq(ISP_DRV_WAIT_IRQ_STRUCT* pwaitIrq)
{
    ...
    if((pwaitIrq->bDumpReg == 0xfe) || ((waitirq.UserInfo.Status & dumpStatus) == 0)
        || (waitirq.SpecUser == ISP_DRV_WAITIRQ_SPEUSER_EIS && mDumpRegCount % 10 != 0))
    ...
    if( (Ret!=0) || (errno!=0))
        LOG_INF("UNWIND error check:%d_%d",Ret,errno);
    if( (Ret== (-512)) || (errno == EINTR) || (errno == 512) )//-ERESTARTSYS=-512
    {    //change to non-clear wait and start to wait again
        ...
        while((waitirq.Timeout > 0) && ( (Ret== (-512)) || (errno == EINTR) || (errno == 512) ))//receive restart system call again, -ERESTARTSYS=-512
        {
    ...
    if(Ret < 0) {
        LOG_ERR("ISP_WAIT_IRQ_FRMB fail(%d). Type(%d), Status(0x%08x), Timeout(%d), mDumpRegCount(%d).",
            Ret, pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout, mDumpRegCount);
        if (waitirq.SpecUser == ISP_DRV_WAITIRQ_SPEUSER_EIS) {
            android_atomic_inc(&mDumpRegCount);
        }
        return MFALSE;
    }
