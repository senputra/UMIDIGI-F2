...
{
    EIndex_Effect_T eEffect = MTK_CONTROL_EFFECT_MODE_OFF;

    eEffect = (EIndex_Effect_T)u4Effect;
/*
    switch (static_cast<MINT32>(u4Effect)) {
    case MEFFECT_OFF:
        eEffect = MTK_CONTROL_EFFECT_MODE_OFF;
        break;
    case MEFFECT_MONO:
        eEffect = MTK_CONTROL_EFFECT_MODE_MONO;
        break;
    case MEFFECT_SEPIA:
        eEffect = MTK_CONTROL_EFFECT_MODE_SEPIA;
        break;
    case MEFFECT_NEGATIVE:
        eEffect = MTK_CONTROL_EFFECT_MODE_NEGATIVE;
        break;
    case MEFFECT_SOLARIZE:
        eEffect = MTK_CONTROL_EFFECT_MODE_SOLARIZE;
        break;
/ *
    case MEFFECT_POSTERIZE:
        eEffect = MTK_CONTROL_EFFECT_MODE_POSTERIZE;
        break;
* /
    case MEFFECT_AQUA:
        eEffect = MTK_CONTROL_EFFECT_MODE_AQUA;
        break;
    case MEFFECT_BLACKBOARD:
        eEffect = MTK_CONTROL_EFFECT_MODE_BLACKBOARD;
        break;
    case MEFFECT_WHITEBOARD:
        eEffect = MTK_CONTROL_EFFECT_MODE_WHITEBOARD;
        break;
    default:
        MY_ERR("incorrect effect mode = %d", u4Effect);
    }*/


    if (i4SensorDev & ESensorDev_Main) {
        if (m_pParamctrl_Main) {
            m_pParamctrl_Main->setEffect(eEffect);
        }
        else {
            MY_ERR("m_pParamctrl_Main is NULL");
            return MFALSE;
    ...
    if (i4SensorDev & ESensorDev_Sub) {
        if (m_pParamctrl_Sub) {
            m_pParamctrl_Sub->setEffect(eEffect);
        }
        else {
            MY_ERR("m_pParamctrl_Sub is NULL");
            return MFALSE;
    ...
    if (i4SensorDev & ESensorDev_MainSecond) {
        if (m_pParamctrl_Main2) {
            m_pParamctrl_Main2->setEffect(eEffect);
        }
        else {
            MY_ERR("m_pParamctrl_Main2 is NULL");
            return MFALSE;
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL IspTuningMgr::validate(MINT32 const i4SensorDev, MBOOL const fgForce)
{
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL IspTuningMgr::validatePerFrame(MINT32 const i4SensorDev, MBOOL const fgForce)
{
    MY_LOG_IF(m_bDebugEnable,"%s(): i4SensorDev = %d\n", __FUNCTION__, i4SensorDev);
...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MBOOL IspTuningMgr::getDebugInfo(MINT32 const i4SensorDev, NSIspExifDebug::IspExifDebugInfo_T& rIspExifDebugInfo)
{
    if (i4SensorDev & ESensorDev_Main) {
        if (m_pParamctrl_Main) {
...
{
    MY_LOG("IspTuningMgr::setAWBGain rgb %d %d %d", rIspAWBGain.i4R, rIspAWBGain.i4G, rIspAWBGain.i4B);
    if (i4SensorDev & ESensorDev_Main) {
        if (m_pParamctrl_Main) {
