...
do_Normalization(unsigned int method)
{
	...
	HDR_LOGD_IF(mDebugMode, "[do_Normalization] method =%d", method);
...
do_FeatureExtraction(void)
{
	...
	HDR_LOGV("[do_FeatureExtraction] ret =%d", ret);
...
HdrShot::do_SetBmapBuffer(void)
{
    ...
    HDR_LOGD_IF(mDebugMode, "[do_SetBmapBuffer] bmap_width=%d", mHdrSetBmapInfo.bmap_width);
    HDR_LOGD_IF(mDebugMode, "[do_SetBmapBuffer] bmap_height=%d", mHdrSetBmapInfo.bmap_height);
    HDR_LOGD_IF(mDebugMode, "[do_SetBmapBuffer] bmap_image_size=%d", mHdrSetBmapInfo.bmap_image_size);
    for(MUINT32 i=0; i<OutputFrameNumGet(); i++) {
        HDR_LOGD_IF(mDebugMode, "[do_SetBmapBuffer] bmap_image_addr[%d]=%p"
                , i
...
do_OriWeightMapGet(void)
{
    ...
    for (MUINT32 i = 0; i < u4OutputFrameNum; i++) {
        HDR_LOGD_IF(mDebugMode, "[do_OriWeightMapGet] OriWeight[%d]->W/H: (%d, %d). Addr: %p."
                , i
        ...
        if(OriWeight[i]->weight_table_data != mHdrSetBmapInfo.bmap_image_addr[i]) {
            HDR_LOGV("OriWeight[%d]->weight_table_data=%p, it should be %p"
                    , i
                    , OriWeight[i]->weight_table_data
                    , mHdrSetBmapInfo.bmap_image_addr[i]);
        }
...
do_UpScaleWeightMap(void)
{
    ...
    // Show info.
    for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
        HDR_LOGV("[do_UpScaleWeightMap] BlurredWeight[%d]->W/H: (%d, %d). Addr: %p."
                , i
...
do_HdrCroppedResultGet(void)
{
    ...
    HDR_LOGD_IF(mDebugMode,"[do_HdrCroppedResultGet] rCroppedHdrResult:: W/H: (%d, %d). Addr: %p. Size: %d."
...
do_HdrNR(void)
{
    ...
    if(mHDRShotMode == eShotMode_ZsdHdrShot){
        ...
        HDR_LOGD_IF(mDebugMode, "zoom(%d)", mShotParam.mu4ZoomRatio);
    } else {
    // Already apply ZoomRatio in source image, don't need to crop with zoom ratio here.
        cropRect = calCrop(srcRect, dstRect, 100);
        HDR_LOGD_IF(mDebugMode, "zoom(%d)", 100);
    }

    HDR_LOGD_IF(mDebugMode, "srcRect xywh(%d,%d,%d,%d)", srcRect.p.x, srcRect.p.y, srcRect.s.w, srcRect.s.h);
    HDR_LOGD_IF(mDebugMode, "dstRect xywh(%d,%d,%d,%d)", dstRect.p.x, dstRect.p.y, dstRect.s.w, dstRect.s.h);
    HDR_LOGD_IF(mDebugMode, "cropRect xywh(%d,%d,%d,%d)", cropRect.p.x, cropRect.p.y, cropRect.s.w, cropRect.s.h);

    HDR_LOGD_IF(mDebugMode, "mNrtype(%d)", mNrtype);
    switch(mNrtype)
    {
        ...
        case ECamShot_NRTYPE_SWNR:
            {
                ...
                ISwNR *swnr = ISwNR::createInstance(getOpenId());
        ...
        default:
             HDR_LOGD_IF(mDebugMode, "Wrong Nr Type %d", mNrtype);
             ret = MFALSE;
