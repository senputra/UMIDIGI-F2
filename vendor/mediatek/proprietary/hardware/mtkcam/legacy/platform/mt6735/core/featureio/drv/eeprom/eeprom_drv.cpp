...
#define EEPROM_DRV_LOG(fmt, arg...)    ALOGD(EEPROM_LOG_TAG " " fmt, ##arg)
#define EEPROM_DRV_ERR(fmt, arg...)    ALOGE(EEPROM_LOG_TAG "Err: %5d: " fmt, __LINE__, ##arg)
...
*/
    ...
    if((pEerom_data->Shading.ISPComm.CommReg[EEPROM_INFO_IN_COMM_LOAD]!=CAL_DATA_LOAD)&&(!(m32EepromDataValidation&EEPROM_ERR_NO_SHADING)))
    {
        EEPROM_DRV_LOG("CamCommShadingTableConvert()add= %p\n", &pEerom_data->Shading);
