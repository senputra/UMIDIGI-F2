...
STEP2(IImageBuffer* Srcbufinfo, MUINT32 srcWidth, MUINT32 srcHeight, IImageBuffer* DSbufinfo, MtkCameraFaceMetadata* FaceInfo, void* FaceBeautyResultInfo)
{
    ...
    MY_LOGD("[STEP2] srcAdr 0x%x DSbufinfo 0x%x size ds %zu",(MUINT32)Srcbufinfo->getBufVA(0),(MUINT32)DSbufinfo->getBufVA(0),DSbufinfo->getBufSizeInBytes(0));
    ...
    MTKPipeFaceBeautyPos FacePos[15] = {{ .x = 0,
                                          .y = 0 }};
    int i=0;
    ...
    for(int i=0;i<faceNum;i++)
    {
        ...
        MY_LOGD("[STEP2] Beautified face %d-%d @[%d,%d]",FBmetadata.number_of_faces,i,FacePos[i].x,FacePos[i].y);
    }
...
STEP3(IImageBuffer* Srcbufinfo, void* FaceBeautyResultInfo) const
{
    ...
    #endif
    MY_LOGD("[STEP3] srcAdr 0x%x size  %zu",(MUINT32)mpDSImg->getBufVA(0),mpDSImg->getBufSizeInBytes(0));
