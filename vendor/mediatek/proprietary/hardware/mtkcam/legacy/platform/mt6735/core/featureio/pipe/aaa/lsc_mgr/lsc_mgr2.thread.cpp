...
updateTsfCfg(UINT32 u4W, UINT32 u4H, const ISP_NVRAM_LSC_T& rLscCfg, ELscScenario_T eLscScn, MBOOL fgForce)
{
    ...
    if (fgDumpEn)
    {
        ...
        if (dumpSdblk("/data/vendor/tsf0/tsfCfgChg", rLscCfg, reinterpret_cast<MUINT32*>(m_pu4ProcTbl)) != 0)
...
initThreadFunc()
{
    ...
    FILE* pFid = fopen("/data/vendor/DestinationGain.bin", "rb");
    if (pFid == NULL)
    {
        m_pGainTbl = 0;
    }
...
doThreadFunc()
{
    ...
    while (m_fgThreadLoop)
    {
        ...
        TSF_LOCK();
        MINT32 fgFreeze = 0;
        GET_PROP("vendor.debug.lsc_mgr.freeze", "0", fgFreeze);
        if (!fgFreeze)
        {
            if (fgPos)
            {
...
tsfBatchCap()
{
    ...
    if (fgDumpEn)
    {
        ...
        if (dumpSdblk("/data/vendor/tsf0/tsfInput", rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfSetProc.ShadingTbl)) != 0)
            LSC_ERR("dumpSdblk error");
    }
    ...
    tsfSetProcInfo(m_rTsfSetProc, fgDumpEn, "/data/vendor/tsf0/tsfAwbStat.bin");
