...
mainflow()
{
    ...
    if(!mTestMode)
    {
        ...
        if(!mJpegCBDone) {
            HDR_LOGE("send fake onCB_CompressedImage");
            MUINT32    u4ExifHeaderSize = 512;
...
setShotParam(void const* pParam, size_t const size)
{
    ...
    if(!ImpShot::setShotParam(pParam, size)) {
        HDR_LOGE("[HDR] HdrShot->setShotParam() fail.");
        ret = false;
    }
