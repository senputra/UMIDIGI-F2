...
*
********************************************************************************/
Return<void> MySensorListener_R::onEvent(const Event &e) {
    ...
    switch(e.sensorType)
    {
        case SensorType::ACCELEROMETER:
            ...
            gAccInfo[0] = sensorEvent.acceleration.x;
            gAccInfo[1] = sensorEvent.acceleration.y;
            gAccInfo[2] = sensorEvent.acceleration.z;
            break;
        ...
        case SensorType::GYROSCOPE:
            ...
            gGyroInfo[0] = sensorEvent.gyro.x;
            gGyroInfo[1] = sensorEvent.gyro.y;
            gGyroInfo[2] = sensorEvent.gyro.z;
            break;

        default:
            EIS_WRN("unknown type(%d)",sensorEvent.type);
            break;
    }
...
MINT32 EisHalImp_R::Init()
{
    ...
    {
        EIS_ERR("get SensorManager FAIL!");
    ...
    {
        EIS_ERR("get DefaultSensor FAIL! %d %d", mGyroSensorHandle, mAcceSensorHandle);
        goto enable_sensor_done;
    }
    //create SensorEventQueue and register callback
    mpMySensorListener = new MySensorListener_R();
    mpSensorManager->createEventQueue(mpMySensorListener,
    ...
    {
        EIS_ERR("createEventQueue FAIL!");
        goto enable_sensor_done;
    }
    //enable sensor
    if (mpEventQueue->enableSensor(mGyroSensorHandle, 33 * 1000 /* sample period */, 0 /* latency */) != Result::OK ||
        mpEventQueue->enableSensor(mAcceSensorHandle, 33 * 1000 /* sample period */, 0 /* latency */) != Result::OK)
    {
        EIS_ERR("enable Sensor FAIL!");
        goto enable_sensor_done;
    }
    else
    {
...
MINT32 EisHalImp_R::Uninit()
{
    ...
    if(mUsers == 0)    // there is no user
    {
        ...
        if (mpEventQueue != NULL)
        {
            ::android::hardware::Return<Result> ret = mpEventQueue->disableSensor(mGyroSensorHandle);
            if (!ret.isOk())
            {
                EIS_ERR("disable gyro sensor fail");
            }
            ret = mpEventQueue->disableSensor(mAcceSensorHandle);
            if (!ret.isOk())
            {
                EIS_ERR("disable accel sensor fail");
            }
            mpEventQueue = NULL;
        }
        if (mpMySensorListener != NULL)
        {
            mpMySensorListener = NULL;
        }
#endif
        ...
        if(mIsEisConfig == 0)
        {
            ...
            if(g_debugDump >= 2)
            {
                ...
                if( m_pEisAlg->EisFeatureCtrl(EIS_FEATURE_GET_DEBUG_INFO, NULL, &lmvMemSize) != S_EIS_OK ||
                    lmvMemSize == 0 )
                {
                    EIS_ERR("EisFeatureCtrl(EIS_FEATURE_GET_DEBUG_INFO) fail(0x%x)",err);
                    eisAlgoLogInfo.Eis_Log_Buf_Addr = NULL;
                    eisAlgoLogInfo.Eis_Log_Buf_Size = 0;
                }
                else
                {
                    CreateMemBuf(lmvMemSize, 1, &mEisDbgBuf);
    ...
    if(aEisPass == EIS_PASS_1)
    {
        ...
        else
        {
            ...
            if(g_debugDump >= 1)
            {
                ...
                EIS_LOG("DivH(%u),DivV(%u),EisWinNum(%u)", mEisAlgoProcData.DivH, mEisAlgoProcData.DivV, mEisAlgoProcData.EisWinNum);
            }
...
MVOID EisHalImp_R::GetEisCustomize(EIS_TUNING_PARA_STRUCT *a_pDataOut)
{
    ...
    if(g_debugDump >= 2)
    {
        a_pDataOut->advtuning_data.eis_debug = 1;
    }
