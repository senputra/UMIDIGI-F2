...
initPerf()
{
    ...
    if( gPerfHandle == - 1 )
    {
        ...
        //
        MINT32 minFreq = property_get_int32("vendor.debug.camera.cpu.minFreq", 0);
        pRegSC(gPerfHandle, CMD_SET_CPU_FREQ_BIG_LITTLE_MIN, 0, (minFreq != 0) ? minFreq : 819000, 0, 0); // set little core @ 819Mhz
        MY_LOGI("gPerfHandle(%d), get minFreq(%d), set minFreq(%d)", gPerfHandle, minFreq, (minFreq != 0) ? minFreq : 819000);
    }
    //
lbExit:
    return MTRUE;
