    ...
    for(i=0; i<u4AreaCnt; i++) {
        if((m_eZoomWinInfo.u4XOffset != 0) || (m_eZoomWinInfo.u4YOffset != 0)){
            if ( (m_eAEMeterArea.rAreas[i].i4Left != 0)||(m_eAEMeterArea.rAreas[i].i4Right != 0)||(m_eAEMeterArea.rAreas[i].i4Top != 0)||(m_eAEMeterArea.rAreas[i].i4Bottom != 0) ){
                sAEMeteringArea->rAreas[i].i4Left   = -1000 + ((m_eZoomWinInfo.u4XOffset)*2000 + (sAEMeteringArea->rAreas[i].i4Left  + 1000)*m_eZoomWinInfo.u4XWidth) / (m_rAEInitInput.rEZoomWin.u4XWidth +2*m_rAEInitInput.rEZoomWin.u4XOffset);
                sAEMeteringArea->rAreas[i].i4Right  = -1000 + ((m_eZoomWinInfo.u4XOffset)*2000 + (sAEMeteringArea->rAreas[i].i4Right + 1000)*m_eZoomWinInfo.u4XWidth) / (m_rAEInitInput.rEZoomWin.u4XWidth +2*m_rAEInitInput.rEZoomWin.u4XOffset);
                sAEMeteringArea->rAreas[i].i4Top    = -1000 + ((m_eZoomWinInfo.u4YOffset)*2000 + (sAEMeteringArea->rAreas[i].i4Top   + 1000)*m_eZoomWinInfo.u4YHeight) / (m_rAEInitInput.rEZoomWin.u4YHeight+2*m_rAEInitInput.rEZoomWin.u4YOffset);
                sAEMeteringArea->rAreas[i].i4Bottom = -1000 + ((m_eZoomWinInfo.u4YOffset)*2000 + (sAEMeteringArea->rAreas[i].i4Bottom+ 1000)*m_eZoomWinInfo.u4YHeight) / (m_rAEInitInput.rEZoomWin.u4YHeight+2*m_rAEInitInput.rEZoomWin.u4YOffset);
                MY_LOG("[%s()] AE Metering area is modified for zoom, Modified AE meter area Idx:%d Left:%d Right:%d Top:%d Bottom:%d InitWidth:%d InitHeight:%d\n",__FUNCTION__, i,
                    sAEMeteringArea->rAreas[i].i4Left, sAEMeteringArea->rAreas[i].i4Right,
                    sAEMeteringArea->rAreas[i].i4Top, sAEMeteringArea->rAreas[i].i4Bottom,m_rAEInitInput.rEZoomWin.u4XWidth,m_rAEInitInput.rEZoomWin.u4YHeight );
            }
        }
                ...
                if((m_eZoomWinInfo.u4XOffset != 0) || (m_eZoomWinInfo.u4YOffset != 0)){
                    if ( (m_eAEFDArea.rAreas[ifaceIdx].i4Left != 0)||(m_eAEFDArea.rAreas[ifaceIdx].i4Right != 0)  ||
                         (m_eAEFDArea.rAreas[ifaceIdx].i4Top != 0) ||(m_eAEFDArea.rAreas[ifaceIdx].i4Bottom != 0)     ){
                                 m_eAEFDArea.rAreas[ifaceIdx].i4Left   = -1000 + ((m_eZoomWinInfo.u4XOffset)*2000  + (m_eAEFDArea.rAreas[ifaceIdx].i4Left + 1000)*m_eZoomWinInfo.u4XWidth)  / (m_rAEInitInput.rEZoomWin.u4XWidth+2*m_rAEInitInput.rEZoomWin.u4XOffset) ;
                                 m_eAEFDArea.rAreas[ifaceIdx].i4Right  = -1000 + ((m_eZoomWinInfo.u4XOffset)*2000  + (m_eAEFDArea.rAreas[ifaceIdx].i4Right+ 1000)*m_eZoomWinInfo.u4XWidth)  / (m_rAEInitInput.rEZoomWin.u4XWidth+2*m_rAEInitInput.rEZoomWin.u4XOffset) ;
                                 m_eAEFDArea.rAreas[ifaceIdx].i4Top    = -1000 + ((m_eZoomWinInfo.u4YOffset)*2000  + (m_eAEFDArea.rAreas[ifaceIdx].i4Top+ 1000)*m_eZoomWinInfo.u4YHeight)   / (m_rAEInitInput.rEZoomWin.u4YHeight+2*m_rAEInitInput.rEZoomWin.u4YOffset) ;
                                 m_eAEFDArea.rAreas[ifaceIdx].i4Bottom = -1000 + ((m_eZoomWinInfo.u4YOffset)*2000  + (m_eAEFDArea.rAreas[ifaceIdx].i4Bottom+ 1000)*m_eZoomWinInfo.u4YHeight)/ (m_rAEInitInput.rEZoomWin.u4YHeight+2*m_rAEInitInput.rEZoomWin.u4YOffset) ;
                    }
        ...
        if(pAEStatisticBuf != NULL)  {
           {
            // Set FDarea to algorithm
               if(m_bFaceAEAreaChage == MTRUE) {   // Face AE window change
                 ...
                 MUINT32 u4FD_Lost_MaxCnt = 0;
                 if(m_rAEInitInput.rAEPARAM.pFaceSmooth!=NULL)
                 {
                    if(m_rAEInitInput.rAEPARAM.pFaceSmooth->u4FD_Lost_MaxCnt<0)
                        u4FD_Lost_MaxCnt = 3;
                    else
                        u4FD_Lost_MaxCnt = m_rAEInitInput.rAEPARAM.pFaceSmooth->u4FD_Lost_MaxCnt;
                 }
                 else
                    u4FD_Lost_MaxCnt = 3;
                 if (m_u4WOFDCnt > u4FD_Lost_MaxCnt){
                   m_bFaceAEAreaChage = MFALSE;
                  }
                  m_u4WOFDCnt ++;
               }
          ...
          localTimer.End();
            ...
            if((m_bIsAutoFlare == MTRUE) && ((bAAASchedule == 0)||(i4ActiveAEItem & E_AE_FLARE))){
                ...
                MY_LOG_IF(m_3ALogEnable,"[%s()][getPreviewFlare] i2FlareOffset = %d i2FlareGain = %d \n", __FUNCTION__, i2FlareOffset, i2FlareGain);
...
AE_EASYTUNING_PLINE_INFO_T rEasytuningTable;
    ...
    if(m_pIAeAlgo != NULL) {
        MY_LOG_IF(m_3ALogEnable,"[%s()] i4SensorDev:%d\n", __FUNCTION__, m_eSensorDev);
        ...
        MY_LOG_IF(m_3ALogEnable,"[getDebugInfo] TotalSize:%d AEPlineInfoSize:%d Mapping:%d AFTable:%d u4GainTableSize:%d\n", rAEPlineDebugInfo.u4Size, u4AEPlineInfoSize, u4AEPlineMappingSize, u4AFTableSize, u4GainTableSize);
...
MRESULT AeMgr::getCaptureParams(AE_MODE_CFG_T &a_rCaptureInfo)
{
    ...
    MY_LOG_IF(m_3ALogEnable,"[%s()] m_i4SensorDev:%d Capture Exp. mode:%d Shutter:%d Sensor gain:%d Isp gain:%d frame rate:%d flare:%d %d ISO:%d\n", __FUNCTION__, m_eSensorDev, a_rCaptureInfo.u4ExposureMode, a_rCaptureInfo.u4Eposuretime, a_rCaptureInfo.u4AfeGain,
...
MRESULT AeMgr::UpdateSensorISPParams(AE_STATE_T eNewAEState)
{
    ...
    switch(eNewAEState)
    {
        ...
        case AE_MANUAL_FRAMERATE_STATE:
            ...
            MY_LOG_IF(m_bPerframeAEFlag,"[%s:s] i4SensorDev:%d VDNum %d, Prev %d/%d/%d, Cur %d/%d/%d, Output %d/%d/%d WOUpdateSensor:%d\n", __FUNCTION__, m_eSensorDev, m_i4WaitVDNum,
                    m_u4PreExposureTime,
                    m_u4PreSensorGain,
                    m_u4PreIspGain,
...
#define max(a,b)  ((a) > (b) ? (a) : (b))
                    ...
                    if(m_u4SmoothIspGain != mPreviewMode.u4IspGain) {
                        ...
                        m_u4PreviousISPgain = m_u4SmoothIspGain;
...
MRESULT AeMgr::updateCaptureShutterValue()
{
    if((m_bEnableAE == MTRUE) && (m_bMultiCap == MFALSE) && (mCaptureMode.u4Eposuretime > m_rAEInitInput.rAEPARAM.strAEParasetting.u4LongCaptureThres) && (m_rAEInitInput.rAEPARAM.strAEParasetting.bEnableLongCaptureCtrl == MTRUE)&&(m_bRealISOSpeed != MTRUE)) {
        m_u4VsyncCnt--;
        if(m_u4VsyncCnt == (m_i4IspGainDelayFrames+1)) {
...
MRESULT AeMgr::monitorAndReschedule( MBOOL bAAASchedule, MUINT32 u4AvgYcur, MUINT32 u4AvgYStable, MBOOL *bAECalc, MBOOL *bAEApply )
{
    ...
    MY_LOG("[%s()] Calc:%d Apply:%d Cnt:%d Frame:%d ReSchedule:%d Exit:%d Skip:%d bStatChange:%d bAAASchedule:%d frameDelay:%d Stable:%d AEMonitorStable:%d VdCnt:%d ISPdelay:%d FaceArea:%d FaceWOCnt:%d TouchArea:%d u4AvgYcur:%d u4AvgYpre:%d isneedupdatesensorisp:%d\n",
              ...
              m_bAEStable, m_bAEMonitorStable, m_i4WaitVDNum, m_i4IspGainDelayFrames, m_bFaceAEAreaChage, m_u4WOFDCnt, m_bTouchAEAreaChage,u4AvgYcur, u4AvgYStable, m_bWillUpdateSensorbyI2C);
...
MRESULT AeMgr::updatePreEvSettingQueue(MUINT32 newExpsuretime, MUINT32 newAfegain, MUINT32 newIspGain)
    ...
    MY_LOG_IF(m_3ALogEnable,"[%s()] m_PreEvSettingQueue[N-1]:%d/%d/%d m_PreEvSettingQueue[N-2]:%d/%d/%d m_PreEvSettingQueue[N-3]:%d/%d/%d \n", __FUNCTION__,
