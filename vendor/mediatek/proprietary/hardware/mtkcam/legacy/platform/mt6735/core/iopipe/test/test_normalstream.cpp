...
#define P2_TUNINGQUE_NUM    10
    ...
    int threadData;
    int threadStatus;
    MUINT32 wrotoW = 0, wrotoH = 0, wdmaoW = 0, wdmaoH = 0;
    int cropW = 0, cropH = 0;
    MUINT32 imageW = 0, imageH = 0;
    MRect p1SrcCrop;
    MSize p1Dst;
...
//        applyTuningSetting(1, &pIspPhyReg[0]);
    ...
    printf("=================    p2 tuning =================\n");
    printf("0x00004018 (0x%x)\n",mpIspDrv->readReg(0x4018));
    printf("0x00004920 (0x%x)\n",mpIspDrv->readReg(0x4920));
    printf("0x00004924 (0x%x)\n",mpIspDrv->readReg(0x4924));
    printf("0x00004928 (0x%x)\n",mpIspDrv->readReg(0x4928));
    printf("0x0000492C (0x%x)\n",mpIspDrv->readReg(0x492C));
    printf("0x00004930 (0x%x)\n",mpIspDrv->readReg(0x4930));
    printf("0x00004934 (0x%x)\n",mpIspDrv->readReg(0x4934));
    printf("0x00004938 (0x%x)\n",mpIspDrv->readReg(0x4938));

    printf("0x00004BA0 (0x%x)\n",mpIspDrv->readReg(0x4BA0));
    printf("0x00004BA4 (0x%x)\n",mpIspDrv->readReg(0x4BA4));
    printf("0x00004BA8 (0x%x)\n",mpIspDrv->readReg(0x4BA8));
    printf("0x00004BAC (0x%x)\n",mpIspDrv->readReg(0x4BAC));
    printf("0x00004BB0 (0x%x)\n",mpIspDrv->readReg(0x4BB0));
    printf("0x00004BB4 (0x%x)\n",mpIspDrv->readReg(0x4BB4));
    printf("0x00004BB8 (0x%x)\n",mpIspDrv->readReg(0x4BB8));
    printf("0x00004BBC (0x%x)\n",mpIspDrv->readReg(0x4BBC));
    printf("0x00004BC0 (0x%x)\n",mpIspDrv->readReg(0x4BC0));
    printf("0x00004BC4 (0x%x)\n",mpIspDrv->readReg(0x4BC4));
    printf("0x00004BC8 (0x%x)\n",mpIspDrv->readReg(0x4BC8));
    ...
    {
        char filename[256];
        sprintf(filename, "/data/Tuning_basicP2_%d_wdmao%dx%d.yuv", casenum, wdmaoW,wdmaoH);
            saveBufToFile(filename, reinterpret_cast<MUINT8*>(wdmaoBuf.virtAddr), wdmaoW *wdmaoH * 2);
    } else if(casenum==2) {
        char filename[256];
        sprintf(filename, "/data/Tuning_basicP2_%d_wroto%dx%d.yuv", casenum, wrotoW,wrotoH);
        saveBufToFile(filename, reinterpret_cast<MUINT8*>(wrotoBuf.virtAddr), wrotoW *wrotoH * 2);
    } else if(casenum==3) {
        char filename[256];
        sprintf(filename, "/data/Tuning_basicP2_%d_wdmao%dx%d.yuv", casenum, wdmaoW,wdmaoH);
        ...
        //
        char filename2[256];
        sprintf(filename2, "/data/Tuning_basicP2_%d_wroto%dx%d.yuv", casenum, wrotoW,wrotoH);
...
int pass2_BurstQueueTest(int casenum, int type)
{
        ...
        printf("QQ : casenum(%d),type(%d) \n",casenum,type);
            ...
            IImageBuffer* imageBuffer_640x480_b10 = nullptr;
            IImageBuffer* imageBuffer_640x480_yuy2 = nullptr;
            IImageBuffer* imageBuffer_1280x720_b10 = nullptr;
