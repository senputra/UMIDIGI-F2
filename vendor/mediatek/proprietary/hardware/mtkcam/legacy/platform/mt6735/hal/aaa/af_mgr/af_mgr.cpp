...
#endif
    m_i4EnableAF = -1;
    MY_LOG("[uninit] - m_Users: %d, m_pIAfAlgo %p", m_Users, m_pIAfAlgo);
    return S_AF_OK;
}
...
MRESULT AfMgr::triggerAF()
{
    ...
    MY_LOG("%s AFCtrlMode(%d), AFLibMode(%d)", __FUNCTION__, m_eCurrAFMode, m_eLIB3A_AFMode);
...
MVOID AfMgr::setAE2AFInfo(AE2AFInfo_T rAEInfo)
{
    ...
    MY_LOG_IF( m_bDebugEnable, "%s ISO(%d), AEStable(%d), SceneLV(%d), ShutterValue(%d), IsFlashFrm(%d)",
               m_sAFInput.i4ISO,
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if( /* checking AF win Preparing codition */
        (m_sAFOutput.i4IsAFDone == MFALSE) &&
        (m_flkwin_syncflag>0) &&
        (m_NonInitState) &&
