...
)
{
    ...
    for( size_t run_idx = 0 ; run_idx < imageIOMapSet.size(); run_idx++ )
    {
        ...
        //
        param_p2.uFrameNo = pFrame->getFrameNo();
        // source
        {
        ...
        //
        MY_LOGD_IF(mLogLevel >= 1, "job: p2 out %zu, copy out %zu",
...
#define PORT_IMG2O_USED  (0x4)
    ...
    MY_LOGD_IF(0, "stream id %#" PRIx64 ", occupied 0x%x",
            streamId, rOccupied);
    return ret;
...
#define P2_DEBUG_DUMP_PATH "/sdcard/camera_dump"
    ...
    if (muDumpBuffer) {
        MY_LOGD("[YUV] frame(%d) size(%zu)", params.uFrameNo, params.vOut.size());
        ...
        if (!NSCam::Utils::makePath(P2_DEBUG_DUMP_PATH, 0660))
            MY_LOGW("fail to make folder[" P2_DEBUG_DUMP_PATH "]");

#define IMAGE_FORMAT_TO_STRING(e)                   \
        (e == eImgFmt_BAYER10) ? "bayer10" :        \
        (e == eImgFmt_BAYER12) ? "bayer12" :        \
        (e == eImgFmt_BAYER14) ? "bayer14" :        \
        (e == eImgFmt_FG_BAYER10) ? "fg_bayer10" :  \
        (e == eImgFmt_FG_BAYER12) ? "fg_bayer12" :  \
        (e == eImgFmt_FG_BAYER14) ? "fg_bayer14" :  \
        (e == eImgFmt_YV12) ? "yv12" :              \
        (e == eImgFmt_NV21) ? "nv21" :              \
        (e == eImgFmt_YUY2) ? "yuy2" :              \
        (e == eImgFmt_I420) ? "i420" :              \
        ...
        {
            sprintf(filename, P2_DEBUG_DUMP_PATH "/p2-%04d-%s-%dx%d-%d_%d_%d-%s.raw",
                    params.uFrameNo,
                    portname,
            ...
            MY_LOGD("[raw][in] frame(%d) port(%s) (%dx%d) fmt(0x%x) stride(%d, %d, %d)",
        ...
        //output
        for (size_t i = 0; i < params.vOut.size(); i++) {
            if (params.vOut[i].mPortId == PORT_WDMAO)
                sprintf(portname, "%s", "wdmao");
            else if (params.vOut[i].mPortId == PORT_WROTO)
            ...
            for (size_t i = 0; i < 3; i++)
                strides[i] = pImgBuf->getPlaneCount() > i ? pImgBuf->getBufStridesInBytes(i) : 0;
            {
                sprintf(filename, P2_DEBUG_DUMP_PATH "/p2-%04d-%s-%dx%d-%d_%d_%d-%s.%s",
                        params.uFrameNo,
                        portname,
                        pImgBuf->getImgSize().w,
                        pImgBuf->getImgSize().h,
                        ...
                        IMAGE_FORMAT_TO_STRING(pImgBuf->getImgFormat()),
                        formatname);
                MY_LOGD("[yuv][out] frame(%d) port(%s) size(%dx%d) fmt(0x%x) strd(%d, %d, %d)",
                        params.uFrameNo, portname,
                        pImgBuf->getImgSize().w, pImgBuf->getImgSize().h,
                        pImgBuf->getImgFormat(),
                        strides[0], strides[1], strides[2]);
