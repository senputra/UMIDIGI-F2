...
#define VHDR_LOGD_IF(cond, arg...)    {if (cond) { VHDR_LOG(arg); }}
...
MINT32 VHdrDrvImp::Init()
{
    ...
    property_get("vendor.debug.vhdr.dump", value, "0");
    g_debugDump = atoi(value);

#endif

    char lcsoValue[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.debug.lcso.dump", lcsoValue, "0");
...
MVOID VHdrDrvImp::SetFlareInfo(const MINT16 &aFlareGain,const MINT16 &aFlareOffset)
{
    ...
    VHDR_LOGD_IF(g_debugDump,"(gain,ofst)=I(%u,%u),O(%u,%u)",aFlareGain,aFlareOffset,mFlareGain,mFlareOffset);
}
...
MINT32 VHdrDrvImp::UpdateLcsoIdx()
{
    ...
    if(mSensorTg == CAM_TG_1)
    {
...

        {
            ...
            VHDR_LOGD_IF(g_debugDump,"Lcso:idx(%u->%u),savePA(0x%08x),NEXT(va,pa)=(0x%08x,0x%08x)",(mLcso_addrIdx == 0)?LCSO_BUFFER_NUM-1:mLcso_addrIdx-1,
                                                                                    mLcso_addrIdx,
                                                                                    mLcsoData.back().pa,
                                                                                    mLcso_virAddr,
                                                                                    mLcso_phyAddr);
    ...
    VHDR_LOGD_IF(g_debugDump,"post mTsSem");
    ::sem_post(&mTsSem);
...
MVOID VHdrDrvImp::UpdateLcsoTimeStamp(const MINT64 &aTimeStamp)
{
    ...
    VHDR_LOGD_IF(g_debugDump,"wait mTsSem");
    ::sem_wait(&mTsSem);
    if(g_debugDump > 0)
    {
        VHDR_LOG("got mTsSem");
    ...
    VHDR_LOGD_IF(g_debugDump,"TG(%u),TS(%lld)",mSensorTg,aTimeStamp);
