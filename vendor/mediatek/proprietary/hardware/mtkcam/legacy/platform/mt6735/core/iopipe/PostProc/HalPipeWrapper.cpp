    ...
    {   //enque by vector, v3 and v1 [v3 only support vector enque]
        ...
        for(int q=0;q<bufPackageNum;q++)
        {
            ...
            if(misV3){
				...
				LOG_INF("dmaEnPort(0x%08x),featureEntag (0x%x),q(%d),\n\
					     pTuningQue(0x%x),\n\
					     mP1PrivateData, crop(%d,%d),src(%d,%d),dst(%d,%d)",dmaEnPort,featureEntag,q,pTuningQue, \
					     	pipePackageInfo.mP1PrivateData.ResizeCropX, pipePackageInfo.mP1PrivateData.ResizeCropY, \
	                        pipePackageInfo.mP1PrivateData.ResizeSrcW,pipePackageInfo.mP1PrivateData.ResizeSrcH, \
	                        pipePackageInfo.mP1PrivateData.ResizeDstW,pipePackageInfo.mP1PrivateData.ResizeDstH);		
            }else {
...
{
	bool ret=true;
	switch(gTuningQueNode.size()){
		case 1:
			LOG_INF("i(0),pmem(0x%08x)",gTuningQueNode[0]);
			break;
		case 2:
			LOG_INF("i(0),pmem(0x%08x),\
				     i(1),pmem(0x%08x)",gTuningQueNode[0],gTuningQueNode[1]);
			break;
		case 3:
			LOG_INF("i(0),pmem(0x%08x),\
				     i(1),pmem(0x%08x),\
				     i(2),pmem(0x%08x)",gTuningQueNode[0],gTuningQueNode[1],gTuningQueNode[2]);
			break;
		case 4:
			LOG_INF("i(0),pmem(0x%08x),\
				     i(1),pmem(0x%08x),\
				     i(2),pmem(0x%08x),\
				     i(3),pmem(0x%08x)",gTuningQueNode[0],gTuningQueNode[1],gTuningQueNode[2],gTuningQueNode[3]);
			break;
		case 5:
			LOG_INF("i(0),pmem(0x%08x),\
				     i(1),pmem(0x%08x),\
				     i(2),pmem(0x%08x),\
				     i(3),pmem(0x%08x),\
				     i(4),pmem(0x%08x)",gTuningQueNode[0],gTuningQueNode[1],gTuningQueNode[2],gTuningQueNode[3],gTuningQueNode[4]);
			break;	
		default:
			if(gTuningQueNode.size() > 5) {
				LOG_INF("i(0),pmem(0x%08x),\
				     i(1),pmem(0x%08x),\
				     i(2),pmem(0x%08x),\
				     i(3),pmem(0x%08x),\
				     i(4),pmem(0x%08x)",gTuningQueNode[0],gTuningQueNode[1],gTuningQueNode[2],gTuningQueNode[3],gTuningQueNode[4]);
				for(MUINT32 i=5;i<gTuningQueNode.size();i++){
           			 LOG_INF("i(%d),pmem(0x%08x)\n",i,gTuningQueNode[i]);
        		}
				LOG_ERR("please add dumpMemInfo idx switch case:%d\n",gTuningQueNode.size());
			}
			break;
    ...
    if(misV3)
    {
        ...
        if(!gTuningQueNode.empty()) {
            ...
            LOG_INF("queNum(%d),detuningQue(0x%08x)\n\
            		 [size]gTuningQueData(%d),gTuningQueNode(%d)\n",gTuningQueNode.size(),pTuningQueBuf,gTuningQueData.size(),gTuningQueNode.size());
        } else {
            ...
            LOG_INF("[Warning]out of tuningque, add one more(0x%x)\n\
            		 [size]gTuningQueData(%d),gTuningQueNode(%d)\n",gTuningQueData[gTuningQueData.size()-1].bufferInfo.virtAddr,gTuningQueData.size(),gTuningQueNode.size());
        }
    ...
    void* pTuningQueBuf)
{
    ...
    if(misV3)
    {
        ...
        //
        dumpMemInfo();   //replace below for loop to one line log
