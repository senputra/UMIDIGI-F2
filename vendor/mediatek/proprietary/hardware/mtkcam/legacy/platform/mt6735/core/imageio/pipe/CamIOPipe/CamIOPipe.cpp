    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ ) {
        ...
        switch ( vInPorts[i]->index ) {
            ...
            case EPortIndex_TG2I:
                ...
                if ( pass1D_twin ) { PIPE_ERR("TG2 Not Support Twin"); return MFALSE; }
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_MODULE_EN:
            ...
            if ( EModule_CAMSV_IMGO == arg1)  {
                ...
                if (true == this->m_pass1TwinMode)
                {
                    ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV_PAK,0x06,ISP_DRV_USER_ISPF);
                }
                else
                {
                    ISP_WRITE_REG(pPhyIspDrv,CAMSV_CAMSV2_PAK,0x02,ISP_DRV_USER_ISPF);
                }
