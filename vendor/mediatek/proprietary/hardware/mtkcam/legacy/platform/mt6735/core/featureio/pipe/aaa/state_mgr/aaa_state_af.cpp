...
#define AFLAMP_OFF_PREPARE_FRAME 2//6
    ...
    MY_LOG_IF(m_pHal3A->m_3ALogEnable, "[StateAF::sendAFIntent](eIntent_VsyncUpdate,eAFState_AF), AFFrmCnt=%d"
        , m_pStateMgr->mAFStateCntSet.AFFrmCnt);

    MBOOL isEngMode = IAfMgr::getInstance().isAFEngMode(m_pHal3A->getSensorDev());
    MBOOL isAFTimeOut=MFALSE;
    if(NeedCheckTimeOut==MTRUE && isEngMode == MFALSE)
    {
        struct timespec t;
        t.tv_sec = t.tv_nsec = 0;
        clock_gettime(CLOCK_MONOTONIC, &t);
        MINT64 timestamp =((t.tv_sec) * 1000000000LL + t.tv_nsec)/1000000;
        if(timestamp - TimeStampAutoFocus > TIMEOUT_PERIOD)
        {
            IAfMgr::getInstance().TimeOutHandle(m_pHal3A->getSensorDev());
            isAFTimeOut=MTRUE;
        }
    }
...
{ return S_3A_OK; }
    ...
    if (m_pStateMgr->m_bIsFocuseFinish)
        return S_3A_OK;
    ...
    MY_LOG_IF(m_pHal3A->m_3ALogEnable,"AfMgr::getInstance().isFocusFinish() = %d", IAfMgr::getInstance().isFocusFinish(m_SensorDevId));
