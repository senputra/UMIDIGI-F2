...
handlePostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext) const
{
    ...
    snprintf( key, PROPERTY_KEY_MAX, "vendor.debug.camnode.scanline.%d", data);//see nodeDataTypes.h
...
dumpDataLink() const
{
    MY_LOGW("mlDataLink.size() = %d", mlDataLink.size());
    list<DataLink>::const_iterator iter;
    for( iter = mlDataLink.begin(); iter != mlDataLink.end(); iter++ )
    {
...
checkDumpData(MUINT32 const data) const
{
    ...
    char key[PROPERTY_KEY_MAX];
    snprintf( key, PROPERTY_KEY_MAX, "vendor.debug.camnode.dump.%d", data);
...
start()
{
    ...
    {   //update dump enable
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        char key[PROPERTY_KEY_MAX];
        snprintf( key, PROPERTY_KEY_MAX, "vendor.debug.%s.dump", getName());
        property_get( key, value, "0");
