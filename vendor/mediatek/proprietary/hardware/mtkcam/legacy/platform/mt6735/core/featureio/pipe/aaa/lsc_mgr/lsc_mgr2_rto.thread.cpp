...
updateTsfCfg(UINT32 u4W, UINT32 u4H, const ISP_NVRAM_LSC_T& rLscCfg, ELscScenario_T eLscScn, MBOOL fgForce)
{
    ...
    if (fgDumpEn)
    {
        ...
        if (dumpSdblk("/data/vendor/tsf1/tsfCfgChg", rLscCfg, reinterpret_cast<MUINT32*>(m_pu4ProcTbl)) != 0)
...
tsfBatch()
{
    ...
    tsfSetProcInfo(m_rTsfSetProc, MTRUE, "/data/vendor/tsf1/aao_batch.bin");
...
tsfBatchCap()
{
    ...
    tsfSetProcInfo(m_rTsfSetProc, fgDumpEn, "/data/vendor/tsf1/tsfAwbStat.bin");
    ...
    if (fgDumpEn)
    {
        ...
        if (dumpSdblk("/data/vendor/tsf1/tsfInput", rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfSetProc.ShadingTbl)) != 0)
            LSC_ERR("dumpSdblk error");
    }
...
tsfBatchCap()
{
    ...
    if (fgDumpEn)
    {
        ...
        if (dumpSdblk("/data/vendor/tsf2/tsfInput", rLscCfg, reinterpret_cast<MUINT32*>(m_rTsfSetProc.ShadingTbl)) != 0)
            LSC_ERR("dumpSdblk error");
    }
    ...
    tsfSetProcInfo(m_rTsfSetProc, fgDumpEn, "/data/vendor/tsf2/tsfAwbStat.bin");
    m_rTsfSetProc.ShadingTbl = (MINT32*) m_pu4ProcTbl;
    // disable temporal smooth
    m_prTsf->TsfFeatureCtrl(MTKTSF_FEATURE_CONFIG_SMOOTH, (void*)0, 0);
    ret = m_prTsf->TsfFeatureCtrl(MTKTSF_FEATURE_BATCH, &m_rTsfSetProc, &m_rTsfResult);
