...
create_fail_exit:
    ...
    if( (m_i4SensorIdx==ESensorDev_Main || m_i4SensorIdx!=ESensorDev_MainSecond) && m_i4CurrLensId!=0xffff)
    {
        ...
        MY_LOG("Wbuf_adr afmgr mode %d dac min %d max %d\n", m_eLIB3A_AFMode, m_DAF_TBL.af_dac_min, m_DAF_TBL.af_dac_max);
    }
    ...
    if( m_pLaserDrv )
    {
        int Disable = 0;
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        property_get("vendor.laser.calib.disable", value, "0");
        Disable = atoi(value);

        if( Disable == 0 )
        {
            MUINT32 OffsetData = (MUINT32)m_NVRAM_LENS.rAFNVRAM.i4Coefs[71];
            MUINT32 XTalkData  = (MUINT32)m_NVRAM_LENS.rAFNVRAM.i4Coefs[72];
            m_pLaserDrv->setLaserCalibrationData(OffsetData, XTalkData);
        }
        MUINT32 LaserMaxDistance = (MUINT32)m_NVRAM_LENS.rAFNVRAM.i4Coefs[73];
        MUINT32 LaserTableNum    = (MUINT32)m_NVRAM_LENS.rAFNVRAM.i4Coefs[74];
        m_pLaserDrv->setLaserGoldenTable((MUINT32 *)&m_NVRAM_LENS.rAFNVRAM.i4Coefs[75], LaserTableNum, LaserMaxDistance);
        m_sAFInput.sLaserInfo.i4AfWinPosCnt = 0;
        Disable = 0;
        property_get("vendor.laser.disable", value, "0");
        Disable = atoi(value);
...
MINT32 AfMgr::getAFMaxAreaNum()
{
    ...
    MCUDrv::lensSearch(m_i4CurrSensorDev, m_i4CurrSensorId);
    m_i4CurrLensId = MCUDrv::getCurrLensID(m_i4CurrSensorDev);
    MY_LOG_IF(m_bDebugEnable, "[lens][SensorDev]0x%04x, [SensorId]0x%04x, [CurrLensId]0x%04x", m_i4CurrSensorDev, m_i4CurrSensorId, m_i4CurrLensId);

    if (m_i4CurrLensId == 0xFFFF)
    {
        MY_LOG_IF(m_bDebugEnable, "[getAFMaxAreaNum][AF disable]%d\n", 0);
        return 0;
    }
    else
    {
...
MRESULT AfMgr::setMFPos(MINT32 a_i4Pos)
{
    ...
    {
        MY_LOG_IF( m_bDebugEnable, "cmd-%s skip set MF pos (%d)->(%d), lib_afmode(%d)\n", __FUNCTION__, m_sAFOutput.i4AFPos, a_i4Pos, m_eLIB3A_AFMode);
    }
    return S_AF_OK;
}
...
MVOID AfMgr::getAFRefWin(CameraArea_T &rWinSize)
{
    if (m_i4EnableAF == 0)
    {
        MY_LOG_IF((m_bDebugEnable&1), "[getAFRefWin] disableAF");
        rWinSize.i4Left   = 0;
        rWinSize.i4Right  = 0;
        rWinSize.i4Top    = 0;
        rWinSize.i4Bottom = 0;
...
MVOID AfMgr::DoAFPreprocessing()
{
    ...
    if( m_pLaserDrv )
    {
        MINT32 LaserStatus = m_pLaserDrv->getLaserCurStatus();
        ...
        MY_LOG_IF(m_bDebugEnable, "LaserInfo : DAC(%d), DIST(%d), CONF(%d), Status(%d)",
        ...
        if( (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFS) || (m_eLIB3A_AFMode == LIB3A_AF_MODE_MACRO) )
        {
                    ...
                    m_sAFInput.sLaserInfo.i4AfWinPosCnt > 1 )
            {
                ...
                MY_LOG_IF(m_bDebugEnable, "Laser TAF, CONF(%d) DAC(%d)", m_sAFInput.sLaserInfo.i4Confidence, m_sAFInput.sLaserInfo.i4CurPosDAC);
            }
        }
        ...
        if (1 == m_sAFOutput.i4FDDetect)
        {
            m_sAFInput.sLaserInfo.i4Confidence = 20;
            m_sAFInput.sLaserInfo.i4CurPosDAC = 0;
            MY_LOG_IF(m_bDebugEnable, "Laser FaceAF, CONF(%d) DAC(%d)", m_sAFInput.sLaserInfo.i4Confidence, m_sAFInput.sLaserInfo.i4CurPosDAC);
        }
    }
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if (m_i4EnableAF == 0)
    {
        ...
        mAFMgrInited = MTRUE;
        MY_LOG_IF(m_bDebugEnable, "disableAF");
        return S_AF_OK;
    }
    ...
    if( m_pLaserDrv )
    {
        ...
        {
            m_sAFInput.sLaserInfo.i4CurPosDAC  = m_pLaserDrv->getLaserCurDac();
            m_sAFInput.sLaserInfo.i4CurPosDist = m_pLaserDrv->getLaserCurDist();
        }
        else
        {
            m_sAFInput.sLaserInfo.i4CurPosDAC  = m_pLaserDrv->predictAFStartPosDac();
            m_sAFInput.sLaserInfo.i4CurPosDist = m_pLaserDrv->predictAFStartPosDist();
            m_sAFInput.sLaserInfo.i4AfWinPosCnt = 0;
        }
    }
    ...
    {
        MY_LOG_IF(m_bDebugEnable, "AF win Preparing\n");
    }
    else
    {
...
MRESULT AfMgr::getDebugInfo(AF_DEBUG_INFO_T &rAFDebugInfo)
{
    ...
    if (m_pIAfAlgo)
    {
        err = m_pIAfAlgo->getDebugInfo(rAFDebugInfo);
...
MRESULT AfMgr::readOTP()
{
    ...
    MINT32 i4InfPos, i4MacroPos, i4MidPos;

    i4InfPos = GetCamCalData.Single2A.S2aAf[0];
    i4MacroPos = GetCamCalData.Single2A.S2aAf[1];
    i4MidPos = GetCamCalData.Single2A.S2aAf[3];

    if ( 0 < i4MacroPos && i4MacroPos < 1024 && i4MacroPos > i4InfPos )
    {
        MY_LOG("OTP [Inf]%d [Macro]%d", i4InfPos, i4MacroPos);

        if( i4MacroPos > i4MidPos && i4MidPos > i4InfPos )
        {
            if( m_pLaserDrv )
            {
                m_pLaserDrv->setLensCalibrationData(i4MacroPos, i4MidPos);
            }
            MY_LOG("OTP [Laser define distance 50 cm]%d", i4MidPos);

            // pass laserAF 50cm otp data to algo
            if(m_NVRAM_LENS.rAFNVRAM.i4Coefs[5] > 0)
            {
                i4MacroPos = i4MidPos * 10000 + i4MacroPos;
            }
        }
...
MVOID AfMgr::autoFocus()
{
    ...
    if( m_pLaserDrv ) //calibration flow testing
    {
        ...
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        int Mode = 0;
        property_get("vendor.laser.calib.mode", value, "0");
        Mode = atoi(value);

        if( Mode == 1 )
        {
            MY_LOG("LaserCali : getLaserOffsetCalib Start\n");
            Offset = m_pLaserDrv->getLaserOffsetCalib();
        ...
        if( Mode == 2 )
        {
            ...
            XTalk = m_pLaserDrv->getLaserXTalkCalib();
