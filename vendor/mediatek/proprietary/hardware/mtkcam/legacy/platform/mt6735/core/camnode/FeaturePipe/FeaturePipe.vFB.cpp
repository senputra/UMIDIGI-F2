...
threadLoopUpdate()
{
    ...
    MY_LOGD_IF(sDebugLogEnable,"Returning P2A node DS image buffer and FD node face metadata buffer of frame %d", jobDsImg.mFrameNo);
    ...
    /* Post buffer to next node */
    MY_LOGD_IF(sDebugLogEnable,"Posting VFB->P2B node of frame %d, ds req(%x) mbuffer(%x)", frameNo, &dsImgReq, dsImgReq.mBuffer);
