...
#-----------------------------------------------------------
# version
MY_CUST_VERSION := rev:1.0.0

#-----------------------------------------------------------
MY_CUST_SENSOR_METADATA_PATH_PLATFORM := $(MTK_PATH_CUSTOM_PLATFORM)/hal/D1/imgsensor_metadata
MY_CUST_SENSOR_METADATA_PATH_PROJECT := $(MTK_PATH_SOURCE)/custom/$(MTK_TARGET_PROJECT)/hal/D1/imgsensor_metadata
MY_CUST_SENSOR_METADATA_PATH_PROJECT_1 := $(MTK_PATH_SOURCE)/custom/$(MTK_BASE_PROJECT)/hal/imgsensor_metadata

#-----------------------------------------------------------
#
define my-all-config.sensor-metadata-under
$(patsubst ./%,%, \
  $(shell if [ -d $(1) ] ; then find $(1) -maxdepth 4 \( -name 'config_static_metadata.*.h' \) -and -not -name '.*' ; fi) \
 )
endef
#
...
MY_CUST_SENSOR_METADATA_FILE_LIST += $(call my-all-config.sensor-metadata-under, $(MY_CUST_SENSOR_METADATA_PATH_PROJECT_1))
...
LOCAL_C_INCLUDES += $(MTK_PATH_SOURCE)
...
$(MY_CUST_SENSOR_METADATA_FINAL_FILE): $(MY_CUST_SENSOR_METADATA_FILE_LIST)
	...
	@for x in $(MY_CUST_SENSOR_METADATA_FILE_LIST); do echo $$x | awk -F/ '{print "#include<"$$(NF-5) "/"$$(NF-4) "/" $$(NF-3) "/" $$(NF-2) "/" $$(NF-1) "/" $$NF ">";}' >> $@; done
