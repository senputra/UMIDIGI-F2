        ...
        if ((sAEMeteringArea->rAreas[i].i4Right > sAEMeteringArea->rAreas[i].i4Left) && (sAEMeteringArea->rAreas[i].i4Bottom > sAEMeteringArea->rAreas[i].i4Top)){
            ...
            if(counter > 2) {
                ...
                AE_LOG( "[%s():Modified] AE meter area Idx:%d Left:%d Right:%d Top:%d Bottom:%d Weight:%d\n",
                        __FUNCTION__, i, sAEMeteringArea->rAreas[i].i4Left, sAEMeteringArea->rAreas[i].i4Right, sAEMeteringArea->rAreas[i].i4Top, sAEMeteringArea->rAreas[i].i4Bottom, sAEMeteringArea->rAreas[i].i4Weight);
                AE_LOG( "[%s():Previous] AE meter area Idx:%d Left:%d Right:%d Top:%d Bottom:%d Weight:%d counter:%d\n",
                        __FUNCTION__, i, m_eAEMeterArea.rAreas[i].i4Left, m_eAEMeterArea.rAreas[i].i4Right, m_eAEMeterArea.rAreas[i].i4Top, m_eAEMeterArea.rAreas[i].i4Bottom, m_eAEMeterArea.rAreas[i].i4Weight, counter);
            ...
            AE_LOG( "[%s():du] m_bAFTouchROISet = MFALSE;\n", __FUNCTION__);
        }
    }
    if(bAreaChage == MTRUE) {
...
MRESULT AeMgr::setAEMode(MUINT32 u4NewAEMode)
{
    ...
    MY_LOG_IF(eNewAEMode == MTK_CONTROL_AE_MODE_OFF , "%s() : %d %d AEState:%d %d\n", __FUNCTION__, m_eAEMode, eNewAEMode, m_eAEState, m_ePreAEState);
...
MUINT16 u2FrameRate = 300;
    ...
    IspTuningMgr::getInstance().setAEInfo((ESensorDev_T)m_eSensorDev, rAEInfo2ISP);
    MY_LOG("[updateAEInfo2ISP] State:%d Shutter:%d Gain:%d %d %d Flare offset:%d %d FrameDuration:%lld \n", eNewAEState, rAEInfo2ISP.u4Eposuretime, rAEInfo2ISP.u4AfeGain,
