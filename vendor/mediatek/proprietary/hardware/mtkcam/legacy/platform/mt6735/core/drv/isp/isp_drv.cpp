...
MBOOL IspDrvImp::cqTableControl(ISP_DRV_CQTABLE_CTRL_ENUM cmd,int burstQnum)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQTABLE_CTRL_ALLOC:
            ...
            LOG_INF("v(0x%x)(0x%x)(0x%x)(0x%x)(0x%x)(0x%x)",mIspVirRegSize,m_ispVirRegBufInfo.size, \
                                                            m_ispVirRegBufInfo.virtAddr,m_ispVirRegBufInfo.phyAddr, \
                                                            mpIspVirRegBufferBaseAddr,mpIspVirRegAddrPA[0]);  // Q open
            ...
            for (int i = 0; i < mTotalCQNum; i++) {
                ...
                LOG_INF("cq:[%d],virtIspAddr:virt[0x%08x]/phy[0x%08x]",i,mpIspVirRegAddrVA[i], mpIspVirRegAddrPA[i] );  // Q open
            }
...
IspDrv* IspDrvVirImp::getInstance(MINT32 cq, MUINT32* ispVirRegAddr, isp_reg_t* ispVirRegMap)
{
    ...
    if ( ((unsigned long)ispVirRegAddr) & ISP_DRV_VIR_ADDR_ALIGN ) {
        LOG_ERR("NOT 8 bytes alignment, cq: %d. ispVirRegAddr: 0x%08x.", cq, ispVirRegAddr);
        return NULL;
    }
    //FIXME, need CHECK
