...
configureForSingleRun(void)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
    if(mfgIsForceBreak) {HDR_LOGD_IF(mDebugMode, "force break at %s", __FUNCTION__); return MFALSE;}
...
ImageRegistratoin(void)
{
    ...
    &&    do_FeatureExtraction()
        ;
                                            ...
                                            #endif
    if(ret != MTRUE){
        HDR_LOGD_IF(mDebugMode, "rank method fail, try normalization again");
    	ret = do_Normalization(0);          // Normalization method
    	ret = createSEImg();
        ret = do_SE();
    	ret = do_FeatureExtraction();
        HDR_LOGD_IF(mDebugMode, "normalization do_FeatureExtraction ret = %d", ret);
        ret = MTRUE;
    }
...
releaseSourceRawImgBuf(void)
{
    ...
    if (!DONT_MEMCPY){
        for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
        {
            HDR_LOGD("mpSourceRawImgBuf[%d] = %p", i, mpSourceRawImgBuf[i]);
...
releaseSourceImgBuf(void)
{
    ...
    for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
    {
        HDR_LOGD("mpSourceImgBuf[%d] = %p", i, mpSourceImgBuf[i]);
...
requestSEImgBuf(void)
{
    FUNCTION_LOG_START;
    MBOOL    ret = MTRUE;
    HDR_LOGD("[requestSEImgBuf] - E. u4OutputFrameNum: %d.", mu4OutputFrameNum);
    switch (mu4OutputFrameNum)    // Allocate buffers according to u4OutputFrameNum, note that there are no "break;" in case 3/case 2.
    {
...
requestHdrWorkingBuf(void)
{
    ...
    HDR_LOGD("hdrBufferSize=%d", hdrBufferSize);

    // b)    MAV
    HDR_LOGD("mu4W_small=%d mu4H_small=%d", mu4W_small, mu4H_small);
    ret = ret && mpHdrHal->MavWorkingBuffSizeGet(mu4W_small, mu4H_small, &mavBufferSize);
    if(!ret) {
        HDR_LOGE("can't get mav working buffer size");
        ret = MFALSE;
        goto lbExit;
    }
    HDR_LOGD("mavBufferSize=%d", mavBufferSize);
...
requestOriWeightMapBuf(void)
{
    ...
    HDR_LOGV("[requestOriWeightMapBuf] u4Size: %d. u4AlignedSize: %d. u4TableSize: %d. u4AlignedTableSize: %d.", u4Size, u4AlignedSize, u4TableSize, u4AlignedTableSize);
    ...
    for (MUINT32 i = 0; i < mu4OutputFrameNum; i++)
    {
        ...
        HDR_LOGD("[requestOriWeightMapBuf] addr[%d] = %p", i, (void*)mWeightingBuf[i]->getBufVA(0));
        #else
...
requestBlurredWeightMapBuf(void)
{
    ...
    MUINT32 u4TableSize = sizeof(HDR_PIPE_WEIGHT_TBL_INFO);
    MUINT32 u4AlignedTableSize = getAlignedSize(u4TableSize);
    HDR_LOGV("[requestBlurredWeightMapBuf] u4Size: %d. u4AlignedSize: %d. u4TableSize: %d. u4AlignedTableSize: %d.", u4Size, u4AlignedSize, u4TableSize, u4AlignedTableSize);
...
)
{
    ...
    HDR_LOGD("opening file [%s]\n", fname);
    int fd = open(fname, O_RDWR | O_CREAT, S_IRWXU);
    if (fd < 0) {
        HDR_LOGE("failed to create file [%s]: %s", fname, strerror(errno));
        return 0x80000000;
    }

    HDR_LOGD("writing %d bytes to file [%s]\n", size, fname);
    while (written < size) {
        nw = ::write(fd, pbuf + written, size - written);
        if (nw < 0) {
            HDR_LOGE("failed to write to file [%s]: %s", fname, strerror(errno));
            break;
        }
        written += nw;
        cnt++;
    }
    HDR_LOGD("done writing %d bytes to file [%s] in %d passes\n", size, fname, cnt);
    ::close(fd);
    ...
    if (NULL != self)
    {
        switch(msg.msgType) {
            case NSCamShot::ECamShot_DATA_MSG_POSTVIEW:
                HDR_LOGD("[fgCamShotDataCb] ECamShot_DATA_MSG_POSTVIEW start");
                self->mpCamExif[self->mCaptueIndex] = new CamExif;
                self->update3AExif(self->mpCamExif[self->mCaptueIndex]);
                self->mCaptueIndex++;
                HDR_LOGD("[fgCamShotDataCb] ECamShot_DATA_MSG_POSTVIEW end");
                break;
            case NSCamShot::ECamShot_DATA_MSG_YUV:
                HDR_LOGD("[fgCamShotDataCb] ECamShot_DATA_MSG_YUV start");
                ...
                HDR_LOGD("[fgCamShotDataCb] ECamShot_DATA_MSG_YUV end");
                break;
        }
    }
#endif
...
handleYuvData(MUINT8* const puBuf, MUINT32 const u4Size)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
    HDR_LOGD_IF(mDebugMode, "[handleYuvData] (puBuf, size) = (%p, %d)", puBuf, u4Size);

    if(mTestMode) {
        HDR_LOGE("[%s] mTestMode", __FUNCTION__);
        return MTRUE;
    }
...
handlePostViewData(MUINT8* const puBuf, MUINT32 const u4Size)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
    HDR_LOGD_IF(mDebugMode, "[handlePostViewData] + (puBuf, size) = (%p, %d)", puBuf, u4Size);
...
handleJpegData(MUINT8* const puJpegBuf, MUINT32 const u4JpegSize, MUINT8* const puThumbBuf, MUINT32 const u4ThumbSize, MUINT32 const u4Index, MBOOL bFinal)
{
    FUNCTION_LOG_START;
    MBOOL ret = MTRUE;
    HDR_LOGD_IF(mDebugMode, "[handleJpegData] + (puJpgBuf, jpgSize, puThumbBuf, thumbSize, u4Index, bFinal) = (%p, %d, %p, %d, %d, %d)"
            , puJpegBuf
            , u4JpegSize
            , puThumbBuf
            , u4ThumbSize
    ...
    HDR_LOGD_IF(mDebugMode, "[handleJpegData] (thumbbuf, size, exifHeaderBuf, size) = (%p, %d, %p, %d)",
...
createSourceAndSmallImg(void)
{
    ...
    if(mHDRShotMode == eShotMode_ZsdHdrShot){
        ...
        HDR_LOGD("Current ISO value = %d",cap3AParam.u4RealISO) ;
        ...
        for(MUINT32 i=0; i < capBufLen; i++) {
    	    ...
    	    HDR_LOGD("dequeBuf[%d]:%p from CapBufMgr",i, (void *)pImgBuf);

            if (DONT_MEMCPY){
                HDR_LOGD("Skip MEMCPY");
            ...
            if(mPrivateDataSize == 0) {
                ...
                HDR_LOGD("mPrivateDataSize = %d", mPrivateDataSize);
            }
            EIspProfile_T profile;
            profile = (mNrtype==ECamShot_NRTYPE_SWNR ? EIspProfile_VSS_Capture_SWNR : EIspProfile_VSS_Capture);
            HDR_LOGD("ZSD mNrtype = %d profile = %d", mNrtype, profile);

    	    HDR_LOGD("enqueBuf:%p to CapBufMgr", (void *)pImgBuf);
        ...
        HDR_LOGD("Roll back 3A value iso = %d",cap3AParam.u4RealISO) ;
    ...
    //
    HDR_LOGD("[createSourceAndSmallImg] Modify i4YuvEvIdx [%d, %d]"
...
GetStride(MUINT32 srcWidth, EImageFormat srcFormat, MUINT32 *pStride)
{
    ...
    HDR_LOGD("stride(%d,%d,%d)", pStride[0], pStride[1], pStride[2]);
...
CDPResize(IImageBuffer* pInputBuf, IImageBuffer* pOutputBuf, MUINT32 transform)
{
    ...
    if((desWidth>32*srcWidth)
        ...
        while(1) {
            //prepare target
            HDR_LOGD_IF(mDebugMode, "[CDPResize] - prepare target");
            ...
            HDR_LOGD_IF(mDebugMode, "[CDPResize] - desWidth=%d desHeight=%d", desWidth, desHeight);
            HDR_LOGD_IF(mDebugMode, "[CDPResize] - tempWidth[0]=%d tempHeight[0]=%d", tempWidth[0], tempHeight[0]);
            HDR_LOGD_IF(mDebugMode, "[CDPResize] - tempWidth[1]=%d tempHeight[1]=%d", tempWidth[1], tempHeight[1]);
            ...
            //scale up
            HDR_LOGD_IF(mDebugMode, "[CDPResize] - scale up");
...
createSEImg(void)
{
    ...
    for (MUINT32 i = 0; i < u4OutputFrameNum; i++)
    {
        HDR_LOGD("[createSEImg] - CDP %d/%d.", i, u4OutputFrameNum);
...
encodeRawToYuv(IImageBuffer *rawBuffer, IImageBuffer *yuvBuffer, IImageBuffer *smallyuvBuffer, EIspProfile_T profile, void* /*privateData*/)
{
    ...
    HDR_LOGD("srcRect xywh(%d,%d,%d,%d)", srcRect.x, srcRect.y, srcRect.w, srcRect.h);
    HDR_LOGD("dstRect xywh(%d,%d,%d,%d)", dstRect.x, dstRect.y, dstRect.w, dstRect.h);
    HDR_LOGD("cropRect xywh(%d,%d,%d,%d)", cropRect.x, cropRect.y, cropRect.w, cropRect.h);
    HDR_LOGD("dstRect2 xywh(%d,%d,%d,%d)", dstRect2.x, dstRect2.y, dstRect2.w, dstRect2.h);
    HDR_LOGD("cropRect2 xywh(%d,%d,%d,%d)", cropRect2.x, cropRect2.y, cropRect2.w, cropRect2.h);

    p3A = IHal3A::createInstance(IHal3A::E_Camera_1, getOpenId(), LOG_TAG);
    if(!p3A) {
        HDR_LOGE("IHal3A::createInstance fail");
        ret = MFALSE;
        goto lbExit;
    }
    ...
    if(mPrivateDataSize!=0 && mPrivateData!=NULL) {
        ...
        HDR_LOGE("must init mPrivateData first, mPrivateData=%p, mPrivateDataSize=%d", mPrivateData, mPrivateDataSize);
    }
    ...
    HDR_LOGD("deque result magicNum(%d)", dequeParams.mFrameNo);
    HDR_LOGD("deque mvIn.size(%zu),mvOut.size(%zu)", dequeParams.mvIn.size(), dequeParams.mvOut.size());
            ...
            )
{
    ...
    // (0). debug
    HDR_LOGD("[createJpegImg] - u4Transform=%d", u4Transform);
...
init()
{
    ...
    HDR_LOGD("mDebugMode=%d", mDebugMode);

    // init default thread priority
    GetThreadProp(&mCapturePolicy, &mCapturePriority);
    HDR_LOGD("mCapturePolicy=%d, mCapturePriority=%d", mCapturePolicy, mCapturePriority);
...
#if HDR_PROFILE_MET
    {
        ...
        if(met_ret<0) {
            HDR_LOGD("MET TAG Init Fail\n");
        } else {
            HDR_LOGD("MET TAG Init OK\n");
        }
...
init_imageParams()
{
    ...
    {
        mNrtype = nr;
        HDR_LOGD("force mNrtype as %d", mNrtype);
    }
    ...
    else
    {
        ...
        HDR_LOGD_IF(mDebugMode, "fJpegRatio=%f, fSensorRatio=%f \n", fJpegRatio, fSensorRatio);
    ...
    HDR_LOGD("[updateInfo] HDR:width=%d,height=%d, mu4W_yuv:%d, mu4H_yuv:%d\n"
    ...
    return 0;
        ...
        HDR_LOGD("[updateInfo] SensorHal:sensor width:%d, height:%d\n", u4SensorWidth, u4SensorHeight);
        pSensorHal->uninit();
...
init_decideCaptureMode()
{
    ...
    if(strHDROutputSetting.u4OutputFrameNum) {
        ...
        HDR_LOGD("mu4OutputFrameNum(%d) mu4FinalGainDiff(%d,%d) TargetTone(%d)"
                , mu4OutputFrameNum
                , mu4FinalGainDiff[0]
                , mu4FinalGainDiff[1]
                , mu4TargetTone
...
init_compoment()
{
    ...
    if(!mpIMemDrv) {
        HDR_LOGE("HdrShot::init can't alloc mpIMemDrv");
        goto lbExit;
    }
    ...
    {
        HDR_LOGE("mpIImageBufAllocator is NULL \n");
        return 0;
    }

    //2. get information for HDR Hal
    HDR_LOGD("[init] - HDR Pipe Init");
    ...
    for(MUINT32 i=0; i<OutputFrameNumGet(); i++) {
        HDR_LOGD_IF(mDebugMode, "[init] - mpSourceImgBuf[%d]", i);
    ...
    HDR_LOGD_IF(mDebugMode, "[init] - Create HDR hal object");
    ...
    // For SmallImg Buffer.
    HDR_LOGV("[init] - QuerySmallImgResolution");
    mpHdrHal->QuerySmallImgResolution(mu4W_small, mu4H_small);
    // For SE Image Buffer.
    HDR_LOGV("[init] - QuerySEImgResolution");
...
allocBuffer(IImageBuffer** ppBuf, MUINT32 w, MUINT32 h, MUINT32 fmt)
{
    ...
    if( fmt != eImgFmt_JPEG )
    {
        ...
        if (mpIMemDrv->allocVirtBuf(&bufMap.memBuf)) {
            HDR_LOGE("g_pIMemDrv->allocVirtBuf() error \n");
            return MFALSE;
        }
        CHECK_OBJECT(mpIMemDrv);
        if (mpIMemDrv->mapPhyAddr(&bufMap.memBuf)) {
            HDR_LOGE("mpIMemDrv->mapPhyAddr() error \n");
            return MFALSE;
        }
        HDR_LOGD("allocBuffer at PA(%p) VA(%p) Size(0x%x)"
    ...
    {
        HDR_LOGE("Null allocated or lock Buffer failed\n");
        ret = MFALSE;
    ...
    else
    {
        ...
        HDR_LOGD("allocBuffer addr(%p), width(%d), height(%d), format(0x%x)", pBuf, w, h, fmt);
        *ppBuf = pBuf;
    }
lbExit:
    FUNCTION_LOG_END_MUM;
...
deallocBuffer(IImageBuffer* pBuf)
{
    ...
    if(!pBuf) {
        HDR_LOGD("free a null buffer");
        return;
    }

    #if 0
    if(!pBuf->getStrongCount()) {
        HDR_LOGD("buffer has been free twice");
        return;
    }
    #endif
                    ...
                    it++)
            {
                if (it->pImgBuf == pBuf)
                {
                    if (mpIMemDrv->unmapPhyAddr(&it->memBuf) != 0)
                        HDR_LOGE("m_pIMemDrv->unmapPhyAddr() error");

                    if (mpIMemDrv->freeVirtBuf(&it->memBuf) != 0)
                        HDR_LOGE("m_pIMemDrv->freeVirtBuf() error");
...
makeBufferAlias(IImageBuffer *pBaseBuf, MINT32 w, MINT32 h, MINT32 imgFormat)
{
    ...
    if(!pBaseBuf) {
        HDR_LOGE("pBasebuf is null");
        ret = MFALSE;
        goto lbExit;
    }
    HDR_LOGD("pBasebuf is %p", pBaseBuf);
    ...
    switch(imgFormat) {
        ...
        case eImgFmt_YUY2:
            ...
            break;
        default:
            HDR_LOGE("unsupported imgFormat %d", imgFormat);
            ret = MFALSE;
            goto lbExit;
    }
    #else
    ...
    if(!pBuf) {
        HDR_LOGE("can't makeBufferAlias w(%d) h(%d) format(0x%x)", w, h, imgFormat);
        ret = MFALSE;
        goto lbExit;
    }
    pBuf->incStrong(pBuf);
    ...
    if(!pBaseBuf->unlockBuf(LOG_TAG)) {
        HDR_LOGE("can't unlock pBaseBuf");
    };
...
uninitMemoryMap()
{
    ...
    {
        HDR_LOGE("ImageBuffer leakage here!");
        for (std::vector<ImageBufferMap>::iterator it = mvImgBufMap.begin();
                it != mvImgBufMap.end();
                it++)
        {
            HDR_LOGE("the leakage occur at PA(%p) VA(%p) Size(0x%x)"
...
update3AExif(CamExif *pCamExif)
{
    ...
    //HDR_LOGD("+ (u4CamMode) = (%d)", u4CamMode);
...
PrintThreadProp(const char *name)
{
    ...
    switch(policy)
    {
        //non-real-time
        case SCHED_OTHER:
            HDR_LOGD("thread attr policy=%s, priority=%d, function=%s", "SCHED_OTHER", priority, name);
            break;
        //real-time
        case SCHED_FIFO:
            HDR_LOGD("thread attr policy=%s, priority=%d, function=%s", "SCHED_FIFO", priority, name);
            break;
        case SCHED_RR:
            HDR_LOGD("thread attr policy=%s, priority=%d, function=%s", "SCHED_RR", priority, name);
            break;
    }
    return MTRUE;
}
...
wait(pthread_mutex_t *mutex, const char* note)
{
    ...
    HDR_LOGD("wait %s start", note);
    start = getUs();
    pthread_mutex_lock(mutex);
    pthread_mutex_unlock(mutex);
    end = getUs();
    HDR_LOGD("wait %s ready, pass %d", note, (end-start)/1000);
...
querySensorInfo(NSCam::SensorStaticInfo &mSensorInfo)
{
    ...
    {
        HDR_LOGE("wrong sensor idx(0x%x), sensorNum(%d)", getOpenId(), sensorNum);
        goto lbExit;
    }
    ...
    HDR_LOGD("sensorDevID(0x%x), sensorType(0x%x), sensorFormatOrder(0x%x), rawSensorBit(0x%x)"
...
getCaptureInfo(MUINT32 sensorId, std::vector<NS3A::CaptureParam_T> & vCap3AParam, MUINT32 &hdrFrameNum)
{
    ...
    HDR_LOGD("sensorType(%d)",rSensorInfo.sensorType);
    ...
    HDR_LOGD("u4OutputFrameNum(%d), u4ExpTimeInUS(%d,%d,%d), u4SensorGain(%d,%d,%d)"
            , strHDROutputSetting.u4OutputFrameNum
    ...
    HDR_LOGD("u1FlareOffset(%d,%d,%d), u4FinalGainDiff(%d,%d), u4TargetTone(%d)"
    ...
    for(MUINT32 i=0; i<strHDROutputSetting.u4OutputFrameNum; i++) {
        ...
        HDR_LOGD("[getCaptureInfo] Modify Exposuretime[%d] : %d AfeGain[%d]:%d IspGain[%d]:%d ISO[%d]:%d\n"
    ...
    if(rSensorInfo.sensorType == SENSOR_TYPE_YUV){
        ...
        HDR_LOGD("[getCaptureInfo] Modify i4YuvEvIdx [%d, %d]"
...
getCaptureParams(NS3A::CaptureParam_T *pCap3AParam)
{
    ...
    for(MUINT32 i=0; i<strHDROutputSetting.u4OutputFrameNum; i++) {
        HDR_LOGD("[getCaptureParams] HDR Exposuretime[%d] : %d Gain[%d]:%d Flare[%d]:%d\n"
    ...
    for(MUINT32 i=0; i<strHDROutputSetting.u4OutputFrameNum; i++) {
        ...
        HDR_LOGD("[getCaptureParams] Modify Exposuretime[%d] : %d AfeGain[%d]:%d IspGain[%d]:%d ISO[%d]:%d\n"
    ...
    if(mSensorType == SENSOR_TYPE_YUV)
    {
        ...
        HDR_LOGD("[getCaptureParams] Modify i4YuvEvIdx [%d, %d]"
                , pCap3AParam[0].i4YuvEvIdx
                , pCap3AParam[1].i4YuvEvIdx
                );
    }
            ...
            )
{
    ...
    HDR_LOGD("cropRect xywh(%d,%d,%d,%d)", cropRect.p.x, cropRect.p.y, cropRect.s.w, cropRect.s.h);
