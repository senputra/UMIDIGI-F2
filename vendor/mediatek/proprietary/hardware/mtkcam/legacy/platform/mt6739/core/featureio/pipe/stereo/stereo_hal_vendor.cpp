...
{
    MBOOL bResult = MTRUE;
    mDumpBuf = ::property_get_int32("vendor.camera.stereo.vendump", 0);
    mFakeAlgoTest = ::property_get_int32("vendor.camera.stereo.fakealgo", 0);
    mEnableDebugLog = ::property_get_int32("vendor.camera.stereo.vendlog", 0);
    ...
    else if(OutData.eScenario == STEREO_SCENARIO_CAPTURE)
    {
        ...
        else if(mFakeAlgoTest == 0)
        {
            ...
            for(size_t i=0;i<plane_cnt;i++)
            {
                ...
                MY_LOGD_IF(mEnableDebugLog,"CPU copy capture buffer: plan%d, InBuf=%p, OutBuf=%p, buf_size=%d",i, InBuf,OutBuf,buf_size);
                memcpy(OutBuf,InBuf,buf_size);
            }
