...
public:
    ...
    virtual MBOOL               getRemappingAeSetting(void* pAeSetting);
...
syncAeInit()
{
    ...
    ::memcpy(rAEInitData.main_param.BVOffset, pSyncAEInitInfo->EVOffset_main, sizeof(MINT32)*2);
    ::memcpy(rAEInitData.sub_param.BVOffset, pSyncAEInitInfo->EVOffset_main2, sizeof(MINT32)*2);
...
MBOOL
Sync3AWrapper::
getRemappingAeSetting(void* pAeSetting)
{
    MINT32 i4Err;
    ...
    i4Err = NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_3A, m_i4Master, (void*&)pMaster3aNvram, 0);
    if(i4Err!=0)
        MY_ERR("Master : Nvram 3A pointer NULL err=%d\n",i4Err);
    i4Err = NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_3A, m_i4Slave, (void*&)pSlave3aNvram, 0);
    if(i4Err!=0)
        MY_ERR("Slave : Nvram 3A pointer NULL err=%d\n",i4Err);
    i4Err = NvBufUtil::getInstance().getBufAndRead(CAMERA_DATA_AE_PLINETABLE, m_i4Slave, (void*&)pSlaveAePlineTable, 0);
    if(i4Err!=0)
        MY_ERR("Slave : Nvram AEPline pointer NULL err=%d\n",i4Err);
    rInput.MasterCamAeIdx = rMasterFrameParams.u4AEIndex;
    rInput.MasterCamBVOffset = pMaster3aNvram->rAENVRAM.rCCTConfig.i4BVOffset;
    ...
    if(memcmp(zeroblock, pSlaveAePlineTable, sizeof(zeroblock)))
    {
        ...
        MINT32 flkResult;
        FlickerHalBase::getInstance().getFlickerResult(m_i4Master, flkResult);
        ...
        // Update Slave's info
        rInput.SlaveCamBVOffset = pSlave3aNvram->rAENVRAM.rCCTConfig.i4BVOffset;
