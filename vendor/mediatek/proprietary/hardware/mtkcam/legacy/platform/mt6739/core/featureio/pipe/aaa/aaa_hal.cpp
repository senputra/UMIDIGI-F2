...
init(MINT32 i4SensorDevId, MINT32 i4SensorOpenIndex)
{
    ...
    property_get("vendor.debug.aaa_hal.enable", value, "0");
    ...
    property_get("vendor.debug.aaa.timeout.enable", value, "0");
    ...
    // Dual camera with CamSv don't need Sync 2A mechanism
    String8 const s8DualCamWithCamsv("IS_DUALCAM_CAMSV");
    NSCam::Utils::Property::tryGet(s8DualCamWithCamsv, m_bIsDualCamWithCamSv);
    ...
    MY_LOG_IF(1,"m_i4SensorDev = %d, i4SensorOpenIndex = %d, DualCamWithCamSv = %d\n", m_i4SensorDev, i4SensorOpenIndex, m_bIsDualCamWithCamSv);
...
if (ENABLE_AF & _this->m_3ACtrlEnable) {
    ...
    waitIrq.Type = ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
    waitIrq.Status = CAM_CTL_INT_P1_STATUS_D_AF_DON_ST;
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    case E3ACtrl_SetIspTuningISO:
        ...
        break;
    case E3ACtrl_SetRRZOInfo:
        i4Ret = IspTuningMgr::getInstance().setRRZInfo(m_i4SensorDev, *reinterpret_cast<RRZOInfo*>(iArg1));
        break;
    // --------------------------------- AF ---------------------------------
...
createCollectCmd(MINTPTR i4Arg)
{
    ...
    strncpy(m_rCollectCmd.path, rInputPath, length-1);
