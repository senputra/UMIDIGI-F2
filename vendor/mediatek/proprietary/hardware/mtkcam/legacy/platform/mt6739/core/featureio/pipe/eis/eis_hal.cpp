...
*
********************************************************************************/
Return<void> MySensorListener::onEvent(const Event &e) {
...
MINT32 EisHalImp::Init()
{
    ...
    //although EIS HW is in TG_1, but EisAlg could be create and then destroy in a pair action.
    {
    ...
    mpMySensorListener = new MySensorListener();
    ...
    {
        //enable sensor
        ::android::hardware::Return<Result> retResult = mpEventQueue->enableSensor(mGyroSensorHandle, 33 * 1000 /* sample period */, 0 /* latency */);
        if (!retResult.isOk())
        {
            EIS_ERR("enable GyroSensor FAIL!");
            goto enable_sensor_done;
        }
        retResult = mpEventQueue->enableSensor(mAcceSensorHandle, 33 * 1000 /* sample period */, 0 /* latency */);
        if (!retResult.isOk())
        {
            EIS_ERR("enable AccelSensor FAIL!");
            goto enable_sensor_done;
        }
        mAccEnable = MTRUE;
        mGyroEnable = MTRUE;
...
MINT32 EisHalImp::ConfigEis(const EIS_PASS_ENUM &aEisPass,const EIS_HAL_CONFIG_DATA &aEisConfig)
{
    if(m_pEisAlg == NULL)
    {
        ...
        EIS_ERR("m_pEisAlg is NULL!Now recreating...TG(%d)", mSensorDynamicInfo.TgInfo);
