...
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    ...
    if( dstdata != 0 ) {
        ...
        if(mbMain1Done && mbMain2Done){
            ...
            MY_LOGD("send shutter notify (mbMain1Done=%d,mbMain2Done=%d)",mbMain1Done,mbMain2Done);
        }
...
onNotify(MUINT32 const msg, MUINT32 const ext1, MUINT32 const ext2)
{
    ...
    switch(msg)
    {
        ...
        case PASS1_EOF:
            ...
            if(ext1 != MAGIC_NUM_INVALID ) //check if 3A udpate ready
            {
            ...
            if( ext2 != MAGIC_NUM_INVALID ) // wait for sensor dealy
            {
                ...
                if(mpHal3A)
                {
                    ...
                    if( isDataConnected(CONTROL_CAP_SRC) ) {
                        ...
                        MY_LOGD("3A setIspProfile for imgo: %d, # 0x%x",
                                mIspProfile,
                                SetCap(magicDeque) );
                        //
                        ParamIspProfile_T _3A_profile(
                    ...
                    if( isDataConnected(CONTROL_PRV_SRC) ) {
                        IDbgInfoContainer* pDbgInfo = NULL;
                        EIspProfile_T IspProfileSetting = EIspProfile_Preview;
                        ...
                        if( StereoSettingProvider::IsMain2MonoSensor())
                        {
                            if(getSensorIdx() ==0)
                            {
                                // Stereo TO-DO : need to update preview bayer2mono profile for bayer sensor rrzo
                                IspProfileSetting = EIspProfile_Preview;
                            }
                            else if(getSensorIdx() ==2)
                            {
                                // Stereo TO-DO : need to update preview profile for mono sensor rrzo
                                IspProfileSetting = EIspProfile_Preview;
                            }
                        }

                        MY_LOGD("3A setIspProfile for rrzo: %d, # 0x%x, dev(0x%x)",
                                IspProfileSetting,
                                magicDeque,
                                getSensorDevIdx());
                        ParamIspProfile_T _3A_profile(
