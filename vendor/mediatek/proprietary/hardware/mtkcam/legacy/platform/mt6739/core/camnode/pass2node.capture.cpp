    ...
    , muDumpFlag(0)
{
    ...
    property_get("vendor.debug.p2node.dump", value, "0");
...
pureRawPass(IImageBuffer* pSrcBuf, IImageBuffer* pDstBuf)
{
...

    {
        ...
        if( !pStream->deque(dequeParams) )
        {
            ...
            //AEE_ASSERT("ISP pure-raw p2 deque fail"); // remove for Zion build pass
            return MFALSE;
        }
        MY_LOGD("pure-raw deque - ");
    }
...
handleP2Done(QParams& rParams)
{
    ...
    if(rParams.mDequeSuccess == MFALSE)
    {
        MY_LOGE("type %d pass2 cnt %d: deque fail",
                mPass2Type,
                muDeqFrameCnt);
        //AEE_ASSERT("\nCRDISPATCH_KEY:MtkCam/P1Node:ISP pass2 deque fail"); // remove for Zion build pass
    }
    //
