...
init(MINT32 i4SensorOpenIndex)
{
    ...
    GET_PROP("vendor.debug.aaa_hal_yuv.log", "0", m_u4LogEn);
    GET_PROP("vendor.debug.aaa_hal_yuv.aftrigmode", "0", m_fgAfTrigMode);
    ...
    MBOOL err = mpIspDrv->init("Hal3AYuv");
    if (!err)
    {
        MY_LOG("Isp Driver Init Fail!");
    }
    createThread();
    EnableAFThread(1);
#else
...
queryTGInfoFromSensorHal()
{
    ...
    if ((senInfo.TgInfo != CAM_TG_1) && (senInfo.TgInfo != CAM_TG_2) && (senInfo.TgInfo != CAM_SV_1))
    {
        MY_ERR("YUV sensor is connected with TgInfo: %d\n", senInfo.TgInfo);
        return E_3A_ERR;
    }

    m_TgInfo = senInfo.TgInfo; //now, TG info is obtained! TG1 or TG2 or SV1(For dual camera)
...
waitVSirq()
{
    if ((m_TgInfo != CAM_TG_1) && (m_TgInfo != CAM_TG_2) && (m_TgInfo != CAM_SV_1))
    {
    ...
    if(m_TgInfo == CAM_TG_1){
        waitIrq.Type = ISP_DRV_IRQ_TYPE_INT_P1_ST;
        waitIrq.Status = CAM_CTL_INT_P1_STATUS_VS1_INT_ST;
    }
    else if(m_TgInfo == CAM_SV_1){
        waitIrq.Type = ISP_DRV_IRQ_TYPE_INT_CAMSV;
        waitIrq.Status = CAMSV_INT_STATUS_VS1_ST;
    } else{
        waitIrq.Type = ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
        waitIrq.Status = CAM_CTL_INT_P1_STATUS_D_VS1_INT_ST;
    }
    ...
    case ECmd_Update:
        {
            ...
            if (ISync3AMgr::getInstance()->isActive())
            {
                ...
                MINT32 i4Sync = ISync3AMgr::getSync3A()->getRemappingAeSetting(&rAeSetting);
...
MINT32 Hal3AYuv::updateAeFlashCaptureParams()
{
    ...
    MINT32 i4TestGain = 0;
    GET_PROP("vendor.debug.aaa_hal_yuv.shutter", "-1", i4TestShutter);
    GET_PROP("vendor.debug.aaa_hal_yuv.gain", "-1", i4TestGain);
