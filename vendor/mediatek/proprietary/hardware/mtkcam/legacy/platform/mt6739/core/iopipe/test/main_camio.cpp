...
#include <core/iopipe/CamIO/FakeSensor.h>
//tuning
...
#define ALWAYS_PURE_RAW_TEST (1)//0: processed raw & could dynamic change
...
{
    MUINT sensorIdx = 0;
    INormalPipe* pPipe = NULL;
    MBOOL fgRet = MFALSE;
    switch (output) {
        case 0:
            pPipe = INormalPipe::createInstance(sensorIdx, "aao_buf_mgr");
            fgRet = pPipe->sendCommand(cmd, arg1, arg2, arg3);
            pPipe->destroyInstance("aao_buf_mgr");
        break;
        case 1:
            pPipe = INormalPipe::createInstance(sensorIdx, "afo_buf_mgr");
            fgRet = pPipe->sendCommand(cmd, arg1, arg2, arg3);
...
MBOOL updateTuning(int output, int num)
{
    ...
    case 0:
       ...
       if (MFALSE == sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_GET_MODULE_HANDLE,
                     NSImageio::NSIspio::EModule_AAO, (MINTPTR)&handle, (MINTPTR)(&("AAOBufMgr::DMAConfig()")), output))
       {
           ...
           printf("SET_AAO_CFG_DONE \n");
            ...
            if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_CFG_DONE, handle, MNULL, MNULL, output))
            {
                printf("EPIPECmd_SET_MODULE_CFG_DONE fail\n");
            }
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_RELEASE_MODULE_HANDLE, handle,
    ...
    case 1:
        ...
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_RELEASE_MODULE_HANDLE, handle, (MINTPTR)(&("AF::DMAConfig()")), MNULL, output))
        {
...
int init3A(int output)
{
    ...
    case 0:
        updateTuning(0, 0);
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_AWB, MTRUE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_AWB on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_AAO, MTRUE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_AAO on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_SGG3, MTRUE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_SGG3 on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_SGG5, MTRUE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_SGG5 on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_AWB, MFALSE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_AWB off\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_AAO, MFALSE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_AAO off\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_SGG3, MFALSE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_SGG3 on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_SGG5, MFALSE, NULL, output))
        {
    ...
    case 1:
        updateTuning(1, 0);
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_ESFKO, MTRUE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_ESFKO on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_AFO_D, MTRUE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_AFO_D on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_EISO, MTRUE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_EISO on\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_ESFKO, MFALSE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_ESFKO off\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_AFO_D, MFALSE, NULL, output))
        {
            printf("EPIPECmd_SET_MODULE_En fail: EModule_AFO_D off\n");
            return MFALSE;
        }
        if (MFALSE==sendCommandNormalPipe(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN,
                NSImageio::NSIspio::EModule_EISO, MFALSE, NULL, output))
        {
...
int setTuningPass1(int magicNum)
{
    ...
    TUNING_MGR_WRITE_REG(eSoftwareScenario_Main_Normal_Stream, CAM_DBS_BSTBL_0, magicNum*10+1)
    TUNING_MGR_WRITE_REG(eSoftwareScenario_Main_Normal_Stream, CAM_DBS_BSTBL_1, magicNum*10+2)
    TUNING_MGR_WRITE_REG(eSoftwareScenario_Main_Normal_Stream, CAM_DBS_BSTBL_2, magicNum*10+3)
    TUNING_MGR_WRITE_REG(eSoftwareScenario_Main_Normal_Stream, CAM_DBS_BSTBL_3, magicNum*10+4)
    //
...
int test_camio(int argc, char** argv)
{
    ...
    MUINT32 sensorIdx = 0;
    MUINT32 sensorID = atoi(argv[1]);  // 0x1 for main, 0x10 for fakeSensor, 0x2:sub, 0x4:main2.
    MUINT32 scenario  = atoi(argv[2]);  // prv:0, cap:1
    MUINT32 sensorFps = atoi(argv[3]);  // 30 for 30fps
    MUINT32 enablePort = atoi(argv[4]); // rrzo:0x1, imgo:0x2, rrzo+imgo:0x3, camsv:0x8, camsv2:0x4
    ...
    printf("iopipetest 1 %d %d %d %d %d %d %d %d\n",sensorID,scenario,sensorFps,enablePort,dualSensor,croptestsel,CheckCRC,ISPver);
    printf("sensorID:0x%x,senario:0x%x,sensorFps:0x%x,enablePort = 0x%x,dualsensor = 0x%x,croptestsel = 0x%x,CheckCRC= 0x%x,ISPver = 0x%x\n",sensorID,scenario,sensorFps,enablePort,dualSensor,croptestsel,CheckCRC,ISPver);
    ...
    NSImageio::NSIspio::E_ISP_PIXMODE e_PixMode[3];
