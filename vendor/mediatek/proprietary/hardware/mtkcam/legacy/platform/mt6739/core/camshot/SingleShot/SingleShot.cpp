...
startOne(SensorParam const & rSensorParam)
{
    ...
    {
        mpAllocBufHandler->updateRequest( mapToNodeData(DATA_MAINYUV), mShotParam.u4PictureTransform, eBUFFER_USAGE_SW_MASK | eBUFFER_USAGE_HW_MASK );
    }
...
createNodes(MUINT32 const NodeDataSet, MUINT32 const NodeCbSet, IImageBuffer const *pImgBuffer)
{
    ...
    {
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        property_get( "vendor.debug.capprofile.enable", value, "-1");
...
#define updateData( SHOT_DATA_MSG, inter_data, cb, need_data, need_cb) \
    ...
    *pNeededData = DATA_FULLRAW | DATA_DEBUGINFO;
    ...
    { //for debug
        char value[PROPERTY_VALUE_MAX] = {'\0'};
        property_get( "vendor.debug.shot.img3o", value, "0");
        if( atoi(value) ) {
