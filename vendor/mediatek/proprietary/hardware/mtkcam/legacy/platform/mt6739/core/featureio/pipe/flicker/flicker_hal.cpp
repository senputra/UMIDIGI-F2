...
MINT32 FlickerHal::init(MINT32 i4SensorOpenIndex)
{
    ...
    /* check sensor binning */
    int sensorBinningRatio = cust_getSensorBinningRatio(sensorId, sensorMode);
    m_u4PixelsInLine *= sensorBinningRatio;
    if (sensorBinningRatio != 1)
        logV("Update pixel line with sensor binning ratio (%d).", sensorBinningRatio);


    int propRatio;
    propRatio = FlickerUtil::getPropInt("z.flk_ratio",0);
    if (propRatio) {
    ...
    propPixel = FlickerUtil::getPropInt("z.flk_pixel",0);
    if (propPixel) {
        m_u4PixelsInLine = propPixel;
        logV("Update pixel line(%d).", m_u4PixelsInLine);
    }
    propH = FlickerUtil::getPropInt("z.flk_h",0);
    if (propH) {
        blkH = propH;
        logV("Update window height(%d).", blkH);
    }
    propW = FlickerUtil::getPropInt("z.flk_w",0);
    if (propW) {
        blkW = propW;
        logV("Update window width(%d).", blkW);
    }
    propClk = FlickerUtil::getPropInt("z.flk_clk",0);
    if (propClk) {
        m_u4SensorPixelClkFreq = propClk;
        logV("Update pixel clock frequency(%d).", m_u4SensorPixelClkFreq);
    }
    logV("Modified flicker_init %d %d %d %d\n",m_u4PixelsInLine, blkH*3, blkW, (int)m_u4SensorPixelClkFreq);
...
MINT32 FlickerHal::setFlickerDrv(MBOOL flicker_en)
{
    ...
    if(flicker_en == 1)
    {
        ...
        // SGG3 enable
        if (MFALSE==pipeSendCommand(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN, NSImageio::NSIspio::EModule_SGG3, 1, 0))
        {
            logE("setFlickerDrv() EPIPECmd_SET_MODULE_EN fail");
            return -1;
        }
    ...
    else       //disable flicker
    {
        ...
        // SGG3 disable
        if (MFALSE==pipeSendCommand(NSImageio::NSIspio::EPIPECmd_SET_MODULE_EN, NSImageio::NSIspio::EModule_SGG3, 0, 0))
        {
            logE("setFlickerDrv() EPIPECmd_SET_MODULE_EN fail");
            return -1;
        }
...
//=====================================================
MINT32 FlickerHal::setFlickerSGG3Config()
{
    ...
    retv = pipeSendCommand(NSImageio::NSIspio::EPIPECmd_GET_MODULE_HANDLE,
            NSImageio::NSIspio::EModule_SGG3,
            (MINTPTR)&handle, (MINTPTR)(&("FlickerHal::setFlickerSGG3Config()")));
    if(retv==0)
    {
        logE("setFlickerSGG3Config");
        return -1;
    }
    ...
    IOPIPE_SET_MODUL_REG(handle,  CAM_SGG3_PGN,  reg_sgg3_pgn.Raw);
    ...
    IOPIPE_SET_MODUL_REG(handle,  CAM_SGG3_GMRC_1,  reg_sgg3_gmrc_1.Raw);
    ...
    IOPIPE_SET_MODUL_REG(handle,  CAM_SGG3_GMRC_2,  reg_sgg3_gmrc_2.Raw);
    ...
    retv = pipeSendCommand(NSImageio::NSIspio::EPIPECmd_RELEASE_MODULE_HANDLE, handle, (MINTPTR)(&("FlickerHal::setFlickerSGG3Config()")), MNULL);
    if(retv==0)
        logE("setFlickerSGG3Config");
    return ret;
}
