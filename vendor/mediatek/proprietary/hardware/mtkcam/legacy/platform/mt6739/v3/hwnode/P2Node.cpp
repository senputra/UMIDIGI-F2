    ...
    for( size_t run_idx = 0 ; run_idx < imageIOMapSet.size(); run_idx++ )
    {
        ...
        for( size_t i = 0; i < imageIOMap.vOut.size(); i++ )
        {
            ...
            if(isStream(mpOutFd, streamId))
                continue;
            else if(abs(ratio_s, ratio) > 0.1f && imageIOMap.vOut.size() > 1)
            {
                if(target_streamId >= 0)
                {
                    occupied = 0;
                    MY_LOGD_IF(mLogLevel >= 1,"Because output MAYBE are all have same ratio, so pass once isp");
                }
                else {
                target_streamId = streamId;
                occupied = 1;
                    MY_LOGD_IF(mLogLevel >= 1,"output ratio are not the same as src buffer, so will use mdp to run 2nd.");
                }
            }
        }
        if(occupied == 0){
        ...
        for( size_t i = 0; i < imageIOMap.vOut.size(); i++ )
        {
            ...
            PortID port_p2;
            MBOOL use_mdp = target_streamId>=0 ? ((target_streamId==streamId)? true: false) : false;
            MY_LOGD_IF(mLogLevel >= 1,"use_mdp(%d) tar(%#" PRIx64 ") ",use_mdp, target_streamId );
            if(!use_mdp && OK == mapPortId(streamId, transform, occupied, port_p2) ) {
        ...
        auto SelectP2MaxOutput = [&](){
            ...
            MY_LOGD_IF(mLogLevel >= 1,"SelectP2MaxOutput: %d " , targetIdx );
            return targetIdx;
        };
        ...
        //
        MY_LOGD("frame %d: p2 out %zu, copy out %zu, target_streamId %#" PRIx64 " ",
                pFrame->getFrameNo(), param_p2.vOut.size(), param_mdp.vOut.size(), target_streamId );
        // create job
...
)
{
    ...
    //
    MY_LOGD_IF(mbEnableLog, "cnt %d, in %zu, out %zu",
...
) const
{
    ...
    //
    MY_LOGD_IF( mbEnableLog, "result: cropRegion (%d, %d, %dx%d)",
