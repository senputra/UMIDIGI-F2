...
//	    MY_LOGD("src data=%d, buf=%p, ext=%d",postBufData.data, postBufData.buf, postBufData.ext);
    ...
    {
        MY_LOGD("enter enquePass2 : data=%d, buf=%p (format=%d, %dx%d), ext=%d",
                postBufData.data,
                postBufData.buf, postBufData.buf->getImgFormat(),
                postBufData.buf->getImgSize().w,postBufData.buf->getImgSize().h,
                postBufData.ext);
...
enquePass2(MUINT32 const data, IImageBuffer* const buf, MUINT32 const ext)
{
    ...
    MY_LOGD("data %d, buf 0x%x,va/pa(0x%x/0x%x),size(%dx%d) # 0x%X, cnt %d; crop2(%d,%d,%d,%d)(%dx%d), crop3(%d,%d,%d,%d)(%dx%d)",
...
handleP2Done(QParams& rParams)
{
    ...
    for( iterOut = rParams.mvOut.begin(); iterOut != rParams.mvOut.end(); iterOut++ )
    {
        ...
        Mutex::Autolock lock(mLock);
        MY_LOGD("SNode:handlePostBuffer sensor=%d, data=%d, buf=%p",
...
//	        else
//	        {
//	            MY_LOGD("Stereo_Profile: datatype: %d buf: 0x%08X",
//	                    nodeDataType, iterOut->mBuffer);
