...
AfMgr::AfMgr( ESensorDev_T eSensorDev)
{
    ...
    m_i4TGSzH        = 0;
    m_i4CurrLensId   = 0;
    m_eCurAFMode     = NUM_OF_AF_MODE; /*force to waitting AP send setting mode command. [Ref : setafmode function]*/
    m_bIsFullScan      = FALSE;
...
MRESULT AfMgr::Hal3AUninitState(MINT32 isInitMCU)
{
    ...
    if (m_pMcuDrv)
    {
        CAM_LOGD("[%s] uninitMcuDrv - Dev: %d", __FUNCTION__, m_i4CurrSensorDev);
...
MRESULT AfMgr::uninit( MINT32 /*isInitMCU*/)
{
    ...
    if( m_i4Users==1)
    {
        ...
        /* After modify destroy instance flow inside AF algorithm, This line can be remove.*/
        m_pIAfAlgo = NULL;
...
MBOOL AfMgr::Start()
{
    ...
    m_i4IsSearchDone_PreState = MTRUE;
    m_i4IsSearchDone_CurState = MTRUE;
    ...
    CAM_LOGD( "AF-%-15s: Dev(0x%04x), TG(%d), PixId (%d)(0:B, 1:GB, 2:GR, 3:R), SensorID (0x%04x), LensId (0x%04x), EnableAF (%d), PD_Support(%d)",
              __FUNCTION__,
              m_i4CurrSensorDev,
              m_eSensorTG,
              m_i4PixelId,
    ...
    memcpy( &m_sNVRam, m_ptrLensNVRam, sizeof(NVRAM_LENS_PARA_STRUCT));
    ...
    if(m_eCamScenarioMode < AF_CAM_SCENARIO_NUM)
    {
        ...
        memcpy( &m_sNVRam.rAFNVRAM, &m_sNVRam.rSCENARIONVRAM[u4AFNVRAMIdx], sizeof(m_sNVRam.rSCENARIONVRAM[u4AFNVRAMIdx]));
    }
    else
    {
    ...
    if(i4OperMode==3)
    {
        ...
        {
            m_sNVRam.rAFNVRAM.i4BackJump[iter] = 0;
        }
    }
    ...
    if(m_i4DgbLogLv)
    {
        ...
        LC_NVRAM.Add("AF-%-15s: [CamScenarioMode] %d [nvram][Version] %d %d / ", __FUNCTION__, m_eCamScenarioMode, m_sNVRam.Version,  m_ptrLensNVRam->Version);
        ...
        LC_NVRAM.Add("rest %d / ",
                     MAXIMUM_NVRAM_CAMERA_LENS_FILE_SIZE-sizeof(UINT32)-sizeof(FOCUS_RANGE_T)-sizeof(AF_NVRAM_T)-sizeof(PD_NVRAM_T)-sizeof(DUALCAM_NVRAM_T)-sizeof(m_sNVRam.rSCENARIONVRAM));
        LC_NVRAM.Add("[Normal Num]%d [Macro Num]%d / ", m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4NormalNum, m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4MacroNum);
        LC_NVRAM.Add("[Param][Normal min step]%d [Macro min step]%d", m_sNVRam.rAFNVRAM.i4AFS_STEP_MIN_NORMAL, m_sNVRam.rAFNVRAM.i4AFS_STEP_MIN_MACRO);
        LC_NVRAM.Print();
    }
    ...
    CAM_LOGD_IF( m_i4DgbLogLv, "AF-%-15s: AFExtCoef %d", __FUNCTION__, m_sAFInput.i4IsRevMode2);
#endif
    ...
    pdprofile.u4RawBit            = 10; //[TODO] get_raw_bit_depth();
    ...
    // call CamPwrOnState after readOTP to make sure the m_i4InitPos is correct
    CamPwrOnState();
...
MBOOL AfMgr::CamPwrOnState()
{
    ...
    FUNC_BEGIN_DEV(m_i4CurrSensorDev);
    ...
    if (m_pMcuDrv == NULL)
    {
        ...
        if (!m_pMcuDrv)
        {
            CAM_LOGE("McuDrv::createInstance fail");
            m_i4EnableAF = 0;
        }
        else
        {
...
MINT32 AfMgr::CCTOPAFApplyNVRAMParam(MVOID *a_pAFNVRAM, MUINT32 u4CamScenarioMode)
{
    ...
    memcpy( &m_sNVRam, pAFNVRAM, sizeof(NVRAM_LENS_PARA_STRUCT));
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID AfMgr::pauseAF(MBOOL bIsPause)
{
...
NS3A::EAfState_T AfMgr::UpdateStateContinuousMode( NS3A::EAfState_T &eInCurSate, AF_EVENT_T &sInEvent)
{
    ...
    case EVENT_CMD_STOP : // intentionally fall through (no break)
    ...
    if (a_eCamScenarioMode != m_eCamScenarioMode)
    {
        ...
        if (0 <= a_eCamScenarioMode && a_eCamScenarioMode < AF_CAM_SCENARIO_NUM)
        {
            if (AFNVRAMMapping[a_eCamScenarioMode] != AFNVRAMMapping[m_eCamScenarioMode])
            {
                ...
                memcpy( &m_sNVRam.rAFNVRAM, &m_sNVRam.rSCENARIONVRAM[MappingIdx], sizeof(AF_NVRAM_T));
...
MRESULT AfMgr::setCamMode(MINT32 a_eCamMode)
{
    ...
    MINT32 AFtbl_num = m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4NormalNum;
    m_sDAF_TBL.af_dac_min = m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4Offset + m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4Pos[0];
    m_sDAF_TBL.af_dac_max = m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4Offset + m_sNVRam.rAFNVRAM.sAF_Coef.sTABLE.i4Pos[AFtbl_num-1];
    if( (m_sAFInput.i4IsVDO==TRUE) && (m_eLIB3A_AFMode == LIB3A_AF_MODE_AFC_VIDEO))
    {
        ...
        m_sDAF_TBL.af_dac_min = m_sNVRam.rAFNVRAM.sVAFC_Coef.sTABLE.i4Offset + m_sNVRam.rAFNVRAM.sVAFC_Coef.sTABLE.i4Pos[0];
        m_sDAF_TBL.af_dac_max = m_sNVRam.rAFNVRAM.sVAFC_Coef.sTABLE.i4Offset + m_sNVRam.rAFNVRAM.sVAFC_Coef.sTABLE.i4Pos[AFtbl_num-1];
#endif
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// check if there's a valid set of sensor and lens
// if yes, return 1
// else,   return 0
...
MVOID AfMgr::setAE2AFInfo( AE2AFInfo_T &rAEInfo)
{
    ...
    MINT32 isAEStable = (rAEInfo.i4IsAEStable==1)? 1: 0; //TODO : need 3a framework patch rAEInfo.i4IsAELocked==1
    ...
    {
        CAM_LOGD_IF( m_i4DgbLogLv,
                     "#(%5d,%5d) cmd-%s Dev(%d) GSum(%lld) ISO(%d) AEStb(%d,%d) LV(%d) shutter(%d) flash(%d) AEBlk(%d)(%p)",
                     m_u4ReqMagicNum,
                     m_u4StaMagicNum,
                     __FUNCTION__,
                     m_i4CurrSensorDev,
...
MUINT32 AfMgr::GetHybridAFMode()
{
    ...
    //Depth AF
    if ((m_sDAF_TBL.is_daf_run == 1) && (m_bDAFEn == MTRUE))
    {
        HybridAFMode |= 1;
    }
...
AREA_T& AfMgr::SelROIToFocusing( AF_OUTPUT_T &sInAFInfo)
{
    ...
    if( (m_i4IsSelHWROI_PreState!=m_i4IsSelHWROI_CurState && m_i4IsSelHWROI_CurState==MTRUE) ||
            ...
            (m_i4DgbLogLv))
    {
        CAM_LOGD( "#(%5d,%5d) %s Dev(%d) force(%d) sel(%d) ROI(X,Y,W,H) : [1]AP(%d, %d, %d, %d), [2,3]OTFD(%d, %d, %d, %d), [4]Center(%d, %d, %d, %d), HW(%d, %d, %d, %d)",
                  m_u4ReqMagicNum,
                  m_u4StaMagicNum,
                  __FUNCTION__,
                  m_i4CurrSensorDev,
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    if( m_bRunPDEn==MTRUE)
    {
        ...
        m_bPdInputExpected = 1-m_bNeedPdoResult;
        ...
        if (m_bPdInputExpected)
        {
            ...
            else
            {
                ...
                if (i4curFrmNum >= (AF_START_MAGIC_NUMBER + 1))
                {
                    // 1 frame buffer for ignoring boundary situation, i.e. ignore frameNum < 5
                    CAM_LOGW( "PD res not rdy! ReqMagicNum(%d)", i4curFrmNum);
                }
            }
        }
        LC_doAF.Add(m_i4DgbLogLv, "PDExpected(%d)/bGetPD(%d)/CalPD(%d)/StaNum(#%5d)/",
                    m_bPdInputExpected,
                    m_bNeedPdoResult,
                    bGetPdFromThread,
                    i4curFrmNum);
        ...
        if( bGetPdFromThread)
        {
            ...
            LogCollector LC_PD(LOG_TAG);
            LC_PD.Add(m_i4DgbLogLv, "%s PD Res : ", __FUNCTION__);
            ...
            for(MINT32 i = 0 ; i < PDOutput.numRes ; i++)
            {
                ...
                LC_PD.Add(m_i4DgbLogLv, "#%d Target Pos(%4d) PD(%4d) CL(%4d)/",
                          i,
            ...
            if( m_i4IsSearchDone_CurState==MFALSE)
            {
                LC_PD.Print(m_i4DgbLogLv);
            }
        }

        //direct control
        m_sAFInput.i4PDPureRawfrm = (MINT32)m_bPdInputExpected; /* i4PDPureRawfrm : 1:control lens, 0:not control lens*/
    ...
    if( (m_i4IsSearchDone_PreState!=m_i4IsSearchDone_CurState) ||
            ...
            (m_i4DgbLogLv))
    {
        LC_doAF.Add("[Status] IsSearchDn(%d->%d), IsSelHWROI(%d->%d), ROISel(%d), IsAFDone(%d->%d), IsMonitorFV(%d->%d), Event(0x%x)",
...
MRESULT AfMgr::passPDBuffer( MVOID *ptrInPDData)
{
    ...
    if( m_i4DgbLogLv)
    {
        ptrMsgBuf += sprintf( ptrMsgBuf, "%s Dev(%d) PDAF_support: ", __FUNCTION__, m_i4CurrSensorDev, m_i4PDAF_support_type);
    }
    ...
    if( m_i4PDAF_support_type==PDAF_SUPPORT_RAW_LEGACY)
    {
        ...
        // calculate PD result when raw is pure raw type
        m_bNeedPdoResult = m_pdaf_raw_fmt==1 ? 1 : 0;
        if( !m_bNeedPdoResult)
        {
            if( m_i4DgbLogLv)
            {
                ptrMsgBuf += sprintf( ptrMsgBuf, "skip process raw frames");
                CAM_LOGD_IF( m_i4DgbLogLv, "%s", dbgMsgBuf);
            }
            return S_AF_OK;
        }
    ...
    else if( m_i4PDAF_support_type==PDAF_SUPPORT_CAMSV || m_i4PDAF_support_type==PDAF_SUPPORT_CAMSV_LEGACY)
    {
        ...
        CAM_LOGD_IF( m_i4DgbLogLv, "CamSV Buf id(%d), sz(%d), va(%p), pa(%p)", rBufInfo->memID, rBufInfo->size, rBufInfo->virtAddr, rBufInfo->phyAddr);

        // calculate PD result per 2 frame
    ...
    else
    {
        ...
        if( m_i4DgbLogLv)
        {
            ptrMsgBuf += sprintf( ptrMsgBuf, "abort due to PDAF_support type(%d) is not supported", m_i4PDAF_support_type);
            CAM_LOGD_IF( m_i4DgbLogLv, "%s", dbgMsgBuf);
        }
        return S_AF_OK;
    }
    ...
    if( magicNum<AF_START_MAGIC_NUMBER)
    {
        if( m_i4DgbLogLv)
        {
            ptrMsgBuf += sprintf( ptrMsgBuf, "abort due to cur_frm_num is less than AF_START_MAGIC_NUMBER skip");
            CAM_LOGD_IF( m_i4DgbLogLv, "%s", dbgMsgBuf);
        }
        return S_AF_OK;
    }
    ...
    if( m_i4DgbLogLv)
    {
        CAM_LOGD_IF( m_i4DgbLogLv, "%s dump queue sz(%d)+", __FUNCTION__, m_vFrmInfo.size());
        for( MINT32 i=0; i<m_vFrmInfo.size(); i++)
        {
            CAM_LOGD_IF( m_i4DgbLogLv,
                         "%s %d, Frm %d, RawFmt %d, LensPos %d TS %12llu",
                         __FUNCTION__,
                         i,
                         m_vFrmInfo[i].i4FrmNum,
                         m_vFrmInfo[i].i4RawFmt,
                         m_vFrmInfo[i].i4LensPos,
                         m_vFrmInfo[i].u8MvLensTS);

        }
        CAM_LOGD_IF( m_i4DgbLogLv, "%s dump queue-", __FUNCTION__);
    }
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID AfMgr::doPDBuffer(MVOID * buffer, MINT32 w,MINT32 h, MINT32 stride, MUINT32 i4FrmNum, MUINT32 i4RawFmt)
{
    ...
    CAM_LOGD_IF( m_i4DgbLogLv,
                 "%s frm#%d w(%d) h(%d) stride(%d) va(%p) fmt(%d)",
                 __FUNCTION__,
                 m_pdaf_raw_frmnum,
                 m_pdaf_w,
                 m_pdaf_h,
...
MRESULT AfMgr::readOTP(CAMERA_CAM_CAL_TYPE_ENUM enCamCalEnum)
{
    ...
    else if (enCamCalEnum == CAMERA_CAM_CAL_DATA_PDAF)
    {
        if (result==CAM_CAL_ERR_NO_ERR)
        {
            ...
            CAM_LOGD_IF(m_i4DgbLogLv, "pd calibration data:");
            for (int index = 0; index < m_ptrAfPara->rPdCaliData.i4Size;)
            {
                CAM_LOGD_IF(m_i4DgbLogLv, "%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
...
MBOOL AfMgr::IspMgrAFStatHWPreparing()
{
    ...
    {
        ptrMsgBuf += sprintf( ptrMsgBuf,
                              "bSkip(%d) PauseAF(%d) FrmCnt(%d) Afmode(%d) IsTargetAssitMove(%d) isHWRdy(%d) ROISel(%d) ZECChg(%d)",
                              bSkip,
                              m_bPauseAF,
                              u4FrameCount,
                              m_eCurAFMode,
...
MVOID AfMgr::setCurFrmNum(MUINT32 frm_num)
{
    ...
    m_AFStatusTbl[m_u4ReqMagicNum%16][1]=(MUINT32)m_sAFOutput.i4IsAFDone;
...
{
    MUINT32 CurDafTblIdx = m_u4ReqMagicNum % DAF_TBL_QLEN;

    m_sDAF_TBL.daf_vec[CurDafTblIdx].frm_mun      = m_u4ReqMagicNum;
    ...
    CAM_LOGD_IF(m_i4DgbLogLv, "[%s] FrmNum(%d), DAC(%d), IsStable(%d), ROI[%d %d %d %d]", __FUNCTION__,
                (MUINT32)m_sDAF_TBL.daf_vec[CurDafTblIdx].frm_mun,
                (MUINT32)m_sDAF_TBL.daf_vec[CurDafTblIdx].af_dac_pos,
                (MUINT32)m_sDAF_TBL.daf_vec[CurDafTblIdx].is_af_stable,
                (MUINT32)m_sDAF_TBL.daf_vec[CurDafTblIdx].af_win_start_x,
                (MUINT32)m_sDAF_TBL.daf_vec[CurDafTblIdx].af_win_start_y,
                (MUINT32)m_sDAF_TBL.daf_vec[CurDafTblIdx].af_win_end_x,
                (MUINT32)m_sDAF_TBL.daf_vec[CurDafTblIdx].af_win_end_y);
...
MVOID AfMgr::EnableGGyro()
{
    if(m_bGGyroEn)
    {
        ...
        // i4Coef[5] == 1:enable SensorListener, else: disable
        {
            ...
            if((m_sNVRam.rAFNVRAM.i4SensorEnable > 0) && (mpSensorManager == NULL) && (gListenEnAcce==MFALSE) && (gListenEnGyro==MFALSE))
            {
                //============ enableSensor ============
...
create_fail_exit:
    ...
    else
    {
        ...
        {
            if((m_sNVRam.rAFNVRAM.i4SensorEnable > 0) && (mpSensorManager != NULL))
            {
                if (mpSensorManager != NULL)
                {
