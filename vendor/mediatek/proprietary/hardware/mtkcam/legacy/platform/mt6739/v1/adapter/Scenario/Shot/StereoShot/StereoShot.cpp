...
onCmd_capture()
{
    ...
    MBOOL ret = MTRUE;
    NSCamShot::ICamShot *pSingleShot = NSCamShot::IStereoShot::createInstance(static_cast<EShotMode>(mu4ShotMode), "StereoShot");
    //
    ...
    pSingleShot->enableDataMsg( NSCamShot::ECamShot_DATA_MSG_JPEG |
                                NSCamShot::ECamShot_DATA_MSG_JPEG_CLEANIMG|
                                NSCamShot::ECamShot_DATA_MSG_DEPTHMAP //NSCamShot::ECamShot_DATA_MSG_JPS
#if DNG_META_CB
            //|((isDataMsgEnabled(NSShot::EIShot_DATA_MSG_RAW))?
            //NSCamShot::ECamShot_DATA_MSG_RAW : NSCamShot::ECamShot_DATA_MSG_NONE)
            |(
            (isDataMsgEnabled(NSShot::EIShot_DATA_MSG_RAW))?
            NSCamShot::ECamShot_DATA_MSG_META | NSCamShot::ECamShot_DATA_MSG_RAW
            : NSCamShot::ECamShot_DATA_MSG_NONE)
#endif
        );
    ...
    if(mu4ShotMode == NSCam::eShotMode_ZsdShot)
    {
        ...
        mpSrcImgBuf = pImageBuffer; // 16 raw cb
        IImageBuffer* pImgBuf_Full_0= pImageBuffer;
        IImageBuffer* pImgBuf_Full_1= NULL;
        IImageBuffer* pImgBuf_Prv_0 = NULL;
        IImageBuffer* pImgBuf_Prv_1 = NULL;

        mpCapBufMgr->dequeBuf(pImgBuf_Prv_0, MFALSE);
        mpCapBufMgr_Main2->dequeBuf(pImgBuf_Prv_1, MFALSE);

        MY_LOGD("ZSD_TS final pImgBuf_Full_0=%d, buf=%p", pImgBuf_Full_0->getTimestamp()/1000000, pImgBuf_Full_0);
        MY_LOGD("ZSD_TS final pImgBuf_Prv_0 =%d, buf=%p", pImgBuf_Prv_0->getTimestamp()/1000000, pImgBuf_Prv_0);
        MY_LOGD("ZSD_TS final pImgBuf_Prv_1 =%d, buf=%p", pImgBuf_Prv_1->getTimestamp()/1000000, pImgBuf_Prv_1);
        MY_LOGD("ZSD_TS final Jpeg vs Jps   =%d", abs(pImgBuf_Full_0->getTimestamp() - pImgBuf_Prv_0->getTimestamp()) / 1000000);
        MY_LOGD("ZSD_TS final Jps diff      =%d", abs(pImgBuf_Prv_1->getTimestamp() - pImgBuf_Prv_0->getTimestamp()) / 1000000);
        MY_LOGD("ZSD_TS final ======================================");

        pImgBuf_Prv_0->setTimestamp(0);
        pImgBuf_Full_0->setTimestamp(0);
        pImgBuf_Prv_1->setTimestamp(0);

        ret = pSingleShot->startOne(rSensorParam, pImgBuf_Full_0, pImgBuf_Full_1, pImgBuf_Prv_0, pImgBuf_Prv_1);
        mpCapBufMgr->enqueBuf(pImgBuf_Full_0);
        mpCapBufMgr->enqueBuf(pImgBuf_Prv_0, MFALSE);
        mpCapBufMgr_Main2->enqueBuf(pImgBuf_Prv_1, MFALSE);
    }
    else
    {
...
fgCamShotDataCb(MVOID* user, NSCamShot::CamShotDataInfo const msg)
{
    ...
    if (NULL != pStereoShot)
    {
        ...
        else if (NSCamShot::ECamShot_DATA_MSG_JPEG_CLEANIMG == msg.msgType)
        {
            pStereoShot->handleJpegCleanImgData(
                    (IImageBuffer*)msg.pBuffer,
                    (IImageBuffer*)msg.ext1,
                    (IDbgInfoContainer*)msg.ext2
                    );
        }
        else if (NSCamShot::ECamShot_DATA_MSG_DEPTHMAP == msg.msgType)
        {
            pStereoShot->handleDepthmapData(
                    (IImageBuffer*)msg.pBuffer
                    );
        }
        ...
        {
            pStereoShot->handleRaw16CB(msg.pBuffer);
        }
        else if (NSCamShot::ECamShot_DATA_MSG_META == msg.msgType)//raw data callback to adapter
        {
            pStereoShot->handleDNGMetaCB(msg.ext1);//metadata structure
        }
#endif
    }
...
handleJpegData(IImageBuffer* pJpeg, IImageBuffer* pThumb, IDbgInfoContainer* pDbg)
{
    ...
    makeExifHeader(eAppMode_StereoMode, puThumbBuf, u4ThumbSize, puExifHeaderBuf, u4ExifHeaderSize, pDbg);
    ...
    // if msgType = MTK_CAMERA_MSG_EXT_DATA_COMPRESSED_IMAGE,
    // change to use CAMERA_MSG_COMPRESSED_IMAGE to camera client
...
MBOOL
StereoShot::
handleJpegCleanImgData(IImageBuffer* pJpeg, IImageBuffer* pThumb, IDbgInfoContainer* pDbg)
{
    AutoCPTLog cptlog(Event_Shot_handleJpegData);
    MUINT8* puJpegCleanImgBuf = (MUINT8*)pJpeg->getBufVA(0);
    MUINT32 u4JpegSize = pJpeg->getBitstreamSize();
    ...
    MY_LOGD("+ (puJpgBuf, jpgSize, puThumbBuf, thumbSize, dbg) = (%p, %d, %p, %d, %p)",
            puJpegCleanImgBuf, u4JpegSize, puThumbBuf, u4ThumbSize, pDbg);
    ...
    mpShotCallback->onCB_CompressedImage(0,
                                         u4JpegSize,
                                         reinterpret_cast<uint8_t const*>(puJpegCleanImgBuf),
                                         u4ExifHeaderSize,          //header size
                                         puExifHeaderBuf,           //header buf
                                         0,                         //callback index
                                         false,                     //final image
...
MBOOL
StereoShot::
handleDepthmapData(IImageBuffer* pDepthmap)
{
    AutoCPTLog cptlog(Event_Shot_handleJpegData);
    MUINT8* puDepthmapBuf = (MUINT8*)pDepthmap->getBufVA(0);
    MUINT32 u4DepthmapSize = pDepthmap->getBufSizeInBytes(0);   //getBitstreamSize();

    MY_LOGD("+ (puDepthmapBuf, u4DepthmapSize) = (%p, %d)", puDepthmapBuf, u4DepthmapSize);

    // Depthmap callback (format:Y8)
    CPTLogStr(Event_Shot_handleJpegData, CPTFlagSeparator, "onCB_CompressedImage");
    mpShotCallback->onCB_CompressedImage(0,
                                         u4DepthmapSize,
                                         reinterpret_cast<uint8_t const*>(puDepthmapBuf),
                                         0,                         //header size
                                         NULL,                      //header buf
                                         0,                         //callback index
                                         false,                     //final image
                                         MTK_CAMERA_MSG_EXT_DATA_DEPTHMAP);
    MY_LOGD("-");
    return MTRUE;
}
...
handleJpsData(IImageBuffer* pJpeg, IDbgInfoContainer* pDbg_0, IDbgInfoContainer* pDbg_1)
{
    MUINT8* puJpegBuf = (MUINT8*)pJpeg->getBufVA(0);
    MUINT32 u4JpegSize = pJpeg->getBitstreamSize();
    StereoHalBase* pStereoHal = StereoHalBase::createInstance();

    MY_LOGD("+ (puJpgBuf, jpgSize, pDbg_0, pDbg_1) = (%p, %d, %p, %p)", puJpegBuf, u4JpegSize, pDbg_0, pDbg_1);

    MUINT8 *puExifHeaderBuf = new MUINT8[ STEREO_EXIF_SIZE ];
    MUINT32 u4ExifHeaderSize = 0;

    CPTLogStr(Event_Shot_handleJpegData, CPTFlagSeparator, "makeExifHeader");
    makeExifHeader(eAppMode_StereoMode, NULL, 0, puExifHeaderBuf, u4ExifHeaderSize, pDbg_0, pDbg_1);
    MY_LOGD("(exifHeaderBuf, size) = (%p, %d)", puExifHeaderBuf, u4ExifHeaderSize);
