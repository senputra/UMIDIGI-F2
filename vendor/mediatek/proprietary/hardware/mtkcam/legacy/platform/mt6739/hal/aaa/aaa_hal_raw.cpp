...
MBOOL Hal3ARaw::setParams(Param_T const &rNewParam)
{
    ...
    // ====================================== AF ==============================================
    IAfMgr::getInstance().setCurFrmNum(m_i4SensorDev, rNewParam.u4MagicNum);
    ...
    IAfMgr::getInstance().setZoomWinInfo(m_i4SensorDev, rNewParam.rAFAreas.i4Left, rNewParam.rAFAreas.i4Top, rNewParam.rAFAreas.i4Width, rNewParam.rAFAreas.i4Height);
...
MRESULT Hal3ARaw::EnableCamSVThread(MINT32 a_bEnable)
{
    if (a_bEnable)
    {
        m_CamSVthreadAlive = 1;
        sem_init(&semCamSVThd, 0, 0);
        pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_OTHER, NICE_CAMERA_AF};
        pthread_create(&mCamSVThread, &attr, CamSVThreadFunc, this);
    }
    else
    {
        m_CamSVthreadAlive = 0;
        ::sem_post(&semCamSVThd);
        pthread_join(mCamSVThread, NULL);
    }
...
MVOID * Hal3ARaw::CamSVThreadFunc(void *arg)
{
    MY_LOG("%s tid: %d +", __FUNCTION__,gettid());
    Hal3ARaw *_this = reinterpret_cast<Hal3ARaw*>(arg);
    ...
    while(1)
    {
        ...
        ::sem_getvalue(&(_this->semCamSVThd), &Val);

        MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "%s : sem_wait +, semCamSVThd(%d) before waitting", __FUNCTION__, Val);
        ::sem_wait(&(_this->semCamSVThd));

        MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "%s : sem_wait -, CamSVthreadAlive(%d)", __FUNCTION__, _this->m_CamSVthreadAlive);
        if (!_this->m_CamSVthreadAlive)
        {
            break;
        }
    ...
    MY_LOG("%s close thread -", __FUNCTION__);
    return NULL;
}
