...
init(const ConfigParam& rCfg, const char* strUser)
{
    ...
    MY_LOG_IF(1, "[%s] m_i4User(%d), m_strName(%s), strUser(%s), eventtype(%d->%d)", __FUNCTION__, m_i4User, m_strName.c_str(), strUser, m_rCfgParam.eEventType, rCfg.eEventType);

    m_rCfgParam = rCfg;

    m_rWaitIrq.Clear            = ISP_DRV_IRQ_CLEAR_WAIT;
    m_rWaitIrq.UserInfo.Type    =
        (m_rCfgParam.eEventType == E_Event_Vsync || m_rCfgParam.eEventType == E_Event_Vsync_Sensor) ?
        (rCfg.i4TgInfo == E_TG1) ? ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D :
        ISP_DRV_IRQ_TYPE_INT_P1_ST_D /*AF*/;
    m_rWaitIrq.UserInfo.Status  =
        (m_rCfgParam.eEventType == E_Event_Vsync || m_rCfgParam.eEventType == E_Event_Vsync_Sensor) ?
            ((rCfg.i4TgInfo == E_TG1) ? CAM_CTL_INT_P1_STATUS_SOF1_INT_ST : CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST) :
            CAM_CTL_INT_P1_STATUS_D_AF_DON_ST /*AF*/;
...
setTgInfo(MINT32 i4TgInfo)
{
    m_rWaitIrq.UserInfo.Type    =
        (m_rCfgParam.eEventType == E_Event_Vsync || m_rCfgParam.eEventType == E_Event_Vsync_Sensor) ?
        (i4TgInfo == E_TG1) ? ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D :
