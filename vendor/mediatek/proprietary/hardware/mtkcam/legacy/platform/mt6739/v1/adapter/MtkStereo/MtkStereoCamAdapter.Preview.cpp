...
onHandleStartPreview()
{
    ...
    MUINT32 pass1RingBufNum_main2 = 4;  // for MT6739, main2 pass1 (camsv) ring buffer must set to 4
    pass1RingBufNum_main2 = ::property_get_int32("debug.camera.stereo.p1rm2", 4);
    ...
    if( mbZsdMode )
    {
        ...
        sensorMain2Scenario = SENSOR_SCENARIO_ID_NORMAL_CAPTURE; //SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
#if 0
        SensorCropWinInfo rSensorCropInfo;
        MINT32 err = 0; // 0: no error. other value: error.
        err = pSensorHalObj->sendCommand(SENSOR_DEV_MAIN_2,
        ...
        if(!err)
        {
            ...
            sensorMain2Scenario = SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
#else
        if(StereoSettingProvider::IsMain2ForcePreviewMode(getOpenId()))
        {
            sensorMain2Scenario = SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
        }
#endif
    ...
    //
    MY_LOGD("VHdr(%d),Name(%s),userName(%s),sensorScenario(%d),pass1RingBufNum(%d),pass1RingBufNum_main2(%d),stereo:prvFps(%d),capFps(%d)->frameRate(%d)",
            mpParamsMgr->getVHdr(),
            mName.string(),
            mUserName.string(),
            sensorScenario,
    ...
    Pass1NodeInitCfg p1NodeInitCfg(
                        ...
                        mFrameRate,
                        rawType);
    // for mt6739, main2 pass1node must use camsv mode (Hardware limitation)
    ...
    configInfo.replaceBufNumResize      = configInfo.rollbackBufNum;    //configInfo.rollbackBufNumPrv;
    ...
    cfgInfo_Main2.replaceBufNumResize   = configInfo.rollbackBufNum;    //configInfo.rollbackBufNumPrv;
