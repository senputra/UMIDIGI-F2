...
int CamPathPass2::config( struct CamPathPass2Parameter* p_parameter )
{
    ...
    //MUINT32 vidoXCropRatio, vidoYCropRatio, vidoCropRatio; //not used
    //MUINT32 dispoXCropRatio, dispoYCropRatio, dispoCropRatio; //not used
    MINT32  p2SubMode;
...
//    cdpPipe.conf_cdrz = 0;
    //
    ISP_PATH_DBG("[mdpcrop] imgi in[0x%x, 0x%x] mdpCrop[0x%x, 0x%x, 0x%x, 0x%x]_f(0x%x, 0x%x)",p_parameter->imgi.size.w,p_parameter->imgi.size.h, \
    ...
    ISP_PATH_DBG("[CRZ]en(%d),in[%lu,%lu],out[%lu,%lu],sl2cCrzRatio(%f)",p_parameter->croppara.crz_enable,crzPipe.crz_in.w,crzPipe.crz_in.h,crzPipe.crz_out.w,crzPipe.crz_out.h,ispYuvPipe.sl2cCrzRatio);
    ISP_PATH_DBG("[CRZ][CROP](x,y,w,h)=(%lu,%lu,%lu,%lu),floatx,y=(%u,%u)",crzPipe.crz_crop.x,crzPipe.crz_crop.y,crzPipe.crz_crop.w,crzPipe.crz_crop.h,crzPipe.crz_crop.floatX,crzPipe.crz_crop.floatY);
    ...
    ISP_PATH_DBG("CDRZ:in[0x%x, 0x%x] crop[0x%x 0x%x 0x%x 0x%x]_f(0x%x, 0x%x) out[0x%x,0x%x] ",crzPipe.crz_in.w,crzPipe.crz_in.h, \
    ...
    ISP_PATH_DBG("[imgi](%lu , %lu , %lu) vidoRotate(%d),pa(0x%x) ", p_parameter->imgi.size.w, p_parameter->imgi.size.h, p_parameter->imgi.size.stride,crzPipe.vido_out.Rotation,p_parameter->imgi.memBuf.base_pAddr);
    ...
    ISP_PATH_DBG("[LCEI]bypass(%d),size(0x%x),Va(0x%x),Pa(0x%x),(w,h,s)=(0x%x,0x%x,0x%x),crop(x,y,w,h)=(0x%x,0x%x,0x%x,0x%x)", \
    ...
    if ( CAM_CTL_EN_P2_DMA_IMG2O_EN & ispTopCtrl.isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw){
        ...
        ISP_PATH_DBG("[IMG2O]bypass(%d),size(0x%x),Va(0x%x),Pa(0x%x),(w,h,s)=(0x%x,0x%x,0x%x),crop(x,y,w,h)=(0x%x,0x%x,0x%x,0x%x)", \
