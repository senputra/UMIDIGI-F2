...
MBOOL FeaturePipeDataHandler::sDebugLogEnable = MFALSE;
...
uninit()
{
    MY_LOGD_IF(sDebugLogEnable,"data(%d)", mData);
    //check dequeed buf
    MBOOL ret = MTRUE;
...
{
    MY_LOGD_IF(sDebugLogEnable,"data(%d), %dx%d, fmt(0x%x)", mData, info.w, info.h, info.fmt);
...
MBOOL
FeaturePipeDataHandler::
allocBuffer_GB(AllocInfo const & info, sp<NativeBufferWrapper>* pGraphicBuffer)
{
    MY_LOGD_IF(sDebugLogEnable,"data(%d), %dx%d, fmt(0x%x)", mData, info.w, info.h, info.fmt);
    ...
    buffer_handle_t handle = (*pGraphicBuffer)->getHandle();
    MUINT32 virtAddr;
    ...
    (*pGraphicBuffer)->lock(NativeBufferWrapper::USAGE_SW, (void**)(&virtAddr));
...
{
    MY_LOGD_IF(sDebugLogEnable,"data(%d), trans(%d)", mData, mTransform);
    {   //wait for buffer
        int ret;
...
{
    MY_LOGD_IF(sDebugLogEnable,"data(%d), buf(0x%x)", mData, pImageBuffer);
    MBOOL ret = MFALSE;
    {
        Mutex::Autolock _l(mLock);
...
{
    MY_LOGD_IF(sDebugLogEnable,"data(%d), buf(0x%x) resize %dx%d", mData, *pImageBuffer, width, height);
    MBOOL ret = MFALSE;
    {
        Mutex::Autolock _l(mLock);
        vector<ImageBufferMap>::iterator iter;
...
MBOOL
FeaturePipeDataHandler::
getGraphicBuffer(IImageBuffer const * pImageBuffer, Warp_SPTR* pGraphicBuffer)
{
    MY_LOGD_IF(sDebugLogEnable,"data(%d), buf(0x%x)", mData, pImageBuffer);
    MBOOL ret = MFALSE;
    {
        ...
        for( iter = mvDequeBuf.begin(); iter != mvDequeBuf.end() ; iter++ )
        {
            if( iter->pImgBuf == pImageBuffer )
            {
                ...
                {
                    *pGraphicBuffer = (*(iter->pGB))->getWarp_SPTR();
                    ret = MTRUE;
                }
