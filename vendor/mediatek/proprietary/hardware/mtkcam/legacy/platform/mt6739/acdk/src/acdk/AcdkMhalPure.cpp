    ...
    while((acdkMhalGetState() & ACDK_MHAL_PREVIEW_MASK) || (ACDK_MHAL_PRE_CAPTURE == acdkMhalGetState()))
    {
       ...
       else
       {
        ...
        if(dumpEnable == 1 || dumpEnable == 3)
        {
            ...
            sprintf(szFileName, "%s/acdkPrv1.bin" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
...
#else
        /*Bypass P2*/
        acdkMhalCBHandle(ACDK_CB_PREVIEW, (MUINT8 *)mAcdkMhalPrvParam.dispImemBuf[0].virtAddr, (MUINT8 *)imgo_virA);
        ACDK_LOGD("Bypass P2");
#endif
...
MINT32 AcdkMhalPure::acdkMhalCaptureProc()
{
    ...
    else
    {
        ...
        if(dumpEnable == 4)
        {
            ...
            sprintf(szFileName, "%s/acdkCap1.bin" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
