...
applyToHw_PerFrame_P2(MINT32 flowType, const RAWIspCamInfo& rIspCamInfo, void* pRegBuf)
{
    ...
    if (flowType == 1)
    {
        ...
        fgRet = ISP_MGR_OBC_T::getInstance(m_eSensorDev, m_eSensorTG).apply(rIspCamInfo.eIspProfile, pReg)
            &&  ISP_MGR_DBS_T::getInstance(m_eSensorDev, m_eSensorTG).apply(rIspCamInfo.eIspProfile, pReg)
...
prepareHw_PerFrame_All(const RAWIspCamInfo& rIspCamInfo, const IndexMgr& rIdx)
{
    ...
    MBOOL fgRet = MTRUE
        ...
        &&  prepareHw_PerFrame_NSL2A(rIspCamInfo, rIdx)
...
{
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.isp.nsl2a.disable", value, "0"); // 0: enable, 1: disable
    MBOOL bDisable = atoi(value);
    ...
    if ((bDisable) ||
        ...
        else
        {
            ...
            {   //  Dynamic Tuning: Enable
                ...
                m_pIspTuningCustom->userSetting_EFFECT_GGM(m_rIspCamInfo, m_rIspCamInfo.eIdx_Effect, ggm);
            }
