...
ESoftwareScenario const gSenDevSwScenMap[EFeatureStreamTag_total][SENSOR_DEV_MAIN_3D+1]=
{
    ...
    {eSoftwareScenario_Main_IP_Raw_TPipe_Stream,eSoftwareScenario_Main_IP_Raw_TPipe_Stream,eSoftwareScenario_Sub_IP_Raw_TPipe_Stream,\
        eSoftwareScenario_total_num,eSoftwareScenario_Main2_IP_Raw_TPipe_Stream,eSoftwareScenario_total_num},//EFeatureStreamTag_IPRawTPipe_Stream
};
    ...
    QFeatureData& rFeatureData)
{
    ...
    else
    {
        ...
        ret=mpHalPipeWrapper->enque(mSWScen,(rParams),this->mpPostProcPipe,this->mDequeuedBufList,tag,rFeatureData);
        if(!ret)
        {
            LOG_ERR("enque Fail");
        }
...
{
    FUNCTION_LOG_START;
    LOG_INF("mSWScen(%d),mpHalPipeWrapper()0x%x",mSWScen,(unsigned int*)mpHalPipeWrapper);
    bool ret=true;
    //
    if(mSWScen==eSoftwareScenario_total_num)
    {
