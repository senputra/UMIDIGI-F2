...
PIPE_DBG("+, CQ:%d/%d",m_pass1_Raw_CQ, m_pass1_RawD_CQ);
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ ) {
        ...
        if (vInPorts[i]->index == EPortIndex_CAMSV_TG1I) {
            MUINT32 vfdata = 0x0;
        ...
        } else if (vInPorts[i]->index == EPortIndex_CAMSV_TG2I) {
            MUINT32 vfdata = 0x0;
            IspDrv* pDrv = this->m_pIspDrvShell->getPhyIspDrv();
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ ) {
         ...
         switch (scenarioFmt) /*(meScenarioFmt)*/ {
             ...
             case eScenarioFmt_YUV:
                 ...
                 }else if (EPortIndex_CAMSV_TG1I == vInPorts[i]->index)  {
                     subMode = ISP_SUB_MODE_YUV;
                     subModeD = ISP_SUB_MODE_YUV;
                 }else if (EPortIndex_CAMSV_TG2I == vInPorts[i]->index)  {
                     subMode = ISP_SUB_MODE_YUV;
                     subModeD = ISP_SUB_MODE_YUV;
                 }
    ...
    if (tg_size.w >= min_size.w) {
        ...
        two_pxl_bmx = 0;
        PIPE_DBG("BMX change to 1 pixel mode.(%d/%d)", tg_size, min_size);
    } else {
        PIPE_ERR("TG size(%d) is smaller than min_size(%d) because of AA constraint.", tg_size, min_size);
    }
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_RRZ:
            ...
            else if ( EPortIndex_CAMSV_TG1I == arg2){
                pRrz_cfg = &m_rrz_d_cfg;
                m_rrzd_update_cnt++;
            }
            else if ( EPortIndex_CAMSV_TG2I == arg2){
                pRrz_cfg = &m_rrz_d_cfg;
                m_rrzd_update_cnt++;
            }
            if (pRrz_cfg == NULL){
            ...
            else if (arg2 == EPortIndex_CAMSV_TG1I){
            ...
            else if (arg2 == EPortIndex_CAMSV_TG2I){
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            if (((EPortIndex)arg1 != EPortIndex_TG1I)  && ((EPortIndex)arg1 != EPortIndex_TG2I) &&
                ((EPortIndex)arg1 != EPortIndex_CAMSV_TG1I) && ((EPortIndex)arg1 != EPortIndex_CAMSV_TG2I)) {
