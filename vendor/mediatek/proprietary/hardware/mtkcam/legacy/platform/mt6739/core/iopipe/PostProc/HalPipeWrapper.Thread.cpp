...
)
{
    ...
    if(bufNode.drvSce==NSImageio::NSIspio::eDrvScenario_IP_TPipe || bufNode.drvSce==NSImageio::NSIspio::eDrvScenario_IP)
    {
        isIPRaw=MTRUE;
    }
...
//    LOG_INF("QQ id : (%x)  callback (%x)",bufNode.idxInBufBatchList,mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mpfnCallback);
    ...
    if(retFrame==true)
    {
        ...
        {    //slow motion support,//all buffer nodes in the batch are deque done
            if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].dequedNum == (int)mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvMagicNo.size())
            {dequeSuccess=true;}
        }
