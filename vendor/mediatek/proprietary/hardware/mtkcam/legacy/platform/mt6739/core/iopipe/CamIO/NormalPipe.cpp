...
unsigned long NormalPipe::mTwinEnable[CAM_SV_2+1][2] = {{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},{0,ePxlMode_One_Single},
    ...
    for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
    {
        ...
        if (CAM_TG_2 == _NORMALPIPE_GET_TG_IDX((mpSensorIdx[0]))){
            ...
            if ((rQBuf.mvOut.at(i).mPortID == PORT_IMGO) || (rQBuf.mvOut.at(i).mPortID == PORT_CAMSV_IMGO)){
                ...
                CAM_LOGD("enque: not supported port in CAM_SV_1");
                return false;
            }
            ...
            if ((rQBuf.mvOut.at(i).mPortID == PORT_IMGO) || (rQBuf.mvOut.at(i).mPortID == PORT_CAMSV2_IMGO)){
                ...
                CAM_LOGD("enque: not supported port in CAM_SV_2");
                return false;
            }
        }
        ...
        if ((portID.index == PORT_RRZO.index) ||(portID.index == PORT_RRZO_D.index) ||
            (portID.index == PORT_CAMSV_IMGO.index) || (portID.index == PORT_CAMSV2_IMGO.index)) {
...
replace(BufInfo const& bufOld, BufInfo const& bufNew)
{
    ...
    if (CAM_TG_2 == _NORMALPIPE_GET_TG_IDX(mpSensorIdx[0])){
        ...
        if ((PORT_IMGO.index == bufOld.mPortID.index) ||
            (PORT_CAMSV_IMGO.index == bufOld.mPortID.index)) {
            portID.index = PORT_CAMSV_IMGO.index;
        } else {
            CAM_LOGD("replace: not supported port in CAM_SV_1");
            return false;
        }
    } else if (_NORMALPIPE_GET_TG_IDX((mpSensorIdx[0])) == CAM_SV_2) {
        if ((PORT_IMGO.index == bufOld.mPortID.index) ||
            (PORT_CAMSV2_IMGO.index == bufOld.mPortID.index)) {
            portID.index = PORT_CAMSV2_IMGO.index;
        } else {
            CAM_LOGD("replace: not supported port in CAM_SV_2");
            return false;
        }
    }
...
NormalPipe::deque(QBufInfo& rQBuf, MUINT32 u4TimeoutMs)
{
    ...
    for (MUINT32 ii=0 ; ii<port_cnt ; ii++ ) {
        ...
        if (MFALSE == mpCamIOPipe->dequeOutBuf(portID, rQTSBufInfo) ) {
            ...
            if (CAM_SV_1 == _NORMALPIPE_GET_TG_IDX(mpSensorIdx[0])){
                CAM_LOGE("[MyDeQ]SV deque[PorID=%d] Mnum=0x%x Fail", portID.index, mLastMnum);
            }
            if (CAM_SV_2 == _NORMALPIPE_GET_TG_IDX(mpSensorIdx[0])){
                CAM_LOGE("[MyDeQ]SV_D deque[PorID=%d] Mnum=0x%x Fail", portID.index, mLastMnum);
            }
            ...
            switch ( _NORMALPIPE_GET_TG_IDX((mpSensorIdx[i])) )
            {
               ...
               case CAM_TG_2:
                ...
                case CAM_SV_1:
                    ...
                    CAM_LOGD("CAM_SV_1: SetCQ0 CQ0D Channel&Mode");
                    break;
                case CAM_SV_2:
                    mEnablePath |= NPIPE_PASS_CAMSV_D;
                    mpCamIOPipe->sendCommand(EPIPECmd_SET_CQ_CHANNEL_BYPASS,0,0,0);
                    mpCamIOPipe->sendCommand(EPIPECmd_SET_D_CQ_CHANNEL_BYPASS,0,0,0);
                    CAM_LOGD("CAM_SV_2: SetCQ0 CQ0D Channel&Mode");
                    break;
               default:
                   break;
            }
...
NormalPipe::configFrame(QFrameParam const& rQParam)
{
    ...
    MINT32 i = 0;
    RRZCfg rrz;
    NSImageio::NSIspio::EPortIndex tgIdx = EPortIndex_TG1I;
    ...
    for ( i =0 ; i < rQParam.mvOut.size();i++ ) {
        ...
        if ((port == PORT_RRZO)  || (port == PORT_RRZO_D) || (port == PORT_IMGO) ||
            (port == PORT_CAMSV_IMGO) || (port == PORT_CAMSV2_IMGO))
        {
            if (CAM_TG_2 == _NORMALPIPE_GET_TG_IDX((mpSensorIdx[0]))){
                 port = PORT_RRZO_D;
            } else if (_NORMALPIPE_GET_TG_IDX((mpSensorIdx[0])) == CAM_SV_1){
      ...
      //when you want to enable PDAF func, please check-in with Middleware at the same time
            ...
            if (mbTwinEnable){
               ...
               } else if (port == PORT_CAMSV_IMGO) {
                   tgIdx = EPortIndex_CAMSV_TG1I;
               } else if (port == PORT_CAMSV2_IMGO) {
