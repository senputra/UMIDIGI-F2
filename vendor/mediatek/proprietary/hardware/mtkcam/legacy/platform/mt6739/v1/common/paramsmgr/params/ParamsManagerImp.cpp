...
updateDefaultParams1_ByQuery()
{
    ...
    property_get( "vendor.debug.displayrotation.enable", value, "-1");
...
#undef FALSE
    ...
    mParameters.set(MtkCameraParameters::KEY_ENG_MFLL_SUPPORTED, CameraParameters::FALSE);
    mParameters.set(MtkCameraParameters::KEY_ENG_MFLL_ENABLE, CameraParameters::FALSE);

     //KEY for [Engineer Mode] GIS CALIBRATION
    mParameters.set(MtkCameraParameters::KEY_ENG_GIS_CALIBRATION_SUPPORTED, CameraParameters::FALSE);
...
//
#ifdef MTK_CAM_STEREO_CAMERA_SUPPORT
    {
        ...
        if ( StereoSettingProvider::getStereoParams(sensorId_main, sensorId_main2, stereoParams) )
        {
            ...
            mParameters.set(MtkCameraParameters::KEY_DUALCAM_CALLBACK_BUFFERS, stereoParams.dualcamCallbackItems);
        }
        else
        {
