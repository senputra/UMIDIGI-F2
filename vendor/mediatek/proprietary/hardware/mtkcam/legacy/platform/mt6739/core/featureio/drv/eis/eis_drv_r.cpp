...
MINT32 EisDrvImp_R::Uninit()
{
    ...
    // >= one user
    int userCount = android_atomic_dec(&mUsers);
...
MINT32 EisDrvImp_R::UpdateEisoIdx()
{
...

    {
        ...
        EIS_LOG("eisoData.timeStamp %llu", eisoData.timeStamp);

        mEisoData.push(eisoData);

        EIS_LOG("condition broadcast");
        mEisoCond.broadcast();
    }
...
{
    EIS_LOG("GetEisHwStatistic (%lld)",aTimeStamp);
    ...
    else //MW timestamp is valid
    {
...

        {
            ...
            if(mEisoData.empty() && (hasMatch == 0))
            {
                ...
                status = EisoCond.waitRelative(mQueLock,(long long int)15*1000000LL); //conditaional wait 10ms
                //EIS_LOG("Eiso condition wait done");
                if( OK != status ) {
                    EIS_WRN("Timeout: waitEiso condition status[%s(%d)]", ::strerror(-status), -status);
                }
