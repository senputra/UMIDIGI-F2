...
setManualAEControl(MUINT32 u4ExpTime, MUINT32 u4ExpGain, MUINT32 u4IspGain)
{
        ...
        {
            if(!pHal3A->getParams(rCam3aParam))
            {
                MY_LOGE("!pHal3A->getParams(rCam3aParam)");
                return MFALSE;
            }
            else
            {
                ...
                if(!pHal3A->setParams(rCam3aParam))
                {
                    MY_LOGE("!pHal3A->setParams(rCam3aParam)");
                    return MFALSE;
                }
            }
            if(!pHal3A->getCaptureParams(rCap3AParam))
            {
                MY_LOGE("!pHal3A->getCaptureParams(rCap3AParam)");
                return MFALSE;
            }
            else
            {
