    ...
    // dst
    {
        ...
        while( pData != vP2data.end() )
        {
            for(MUINT32 i = 0; i < MAX_DST_PORT_NUM; i++)
            {
                ...
                NodeDataTypes nodeDataType;
                if(mbIsNeedDisplayRotation)
                    nodeDataType = PASS2_PRV_DST_1;
                else
                    nodeDataType = PASS2_PRV_DST_0;
                //
                if(muDequeOrder[i] == 0)
                    break;
                //
                if( bDequeDisplay == MFALSE &&
                        muDequeOrder[i] == nodeDataType)
                {
                    continue;
                }
                //
                ...
                if(ret)
                {
                    ...
                    if(muDequeOrder[i] == nodeDataType)
                    {
