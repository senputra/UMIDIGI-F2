...
init()
{
    ...
    MINT32 coverity = android_atomic_inc(&mi4User);
...
uninit()
{
    ...
    if (mi4User > 0)
    {
        ...
        MINT32 coverity = android_atomic_dec(&mi4User);
...
setIsp(MINT32 flowType, const MetaSet_T& control, void* pRegBuf, MetaSet_T* pResult)
{
    ...
    if (GET_ENTRY_ARRAY(control.halMeta, MTK_PROCESSOR_CAMINFO, rCamInfo.data, sizeof(NSIspTuning::RAWIspCamInfo_U)))
    {
        ...
        //RPGENable = FALSE
        rRawInfo.rRrzInfo.bEnable = MFALSE;
        rRawInfo.rRrzInfo.OfstX = 0;
        rRawInfo.rRrzInfo.OfstY = 0;
        rRawInfo.rRrzInfo.RtoW = 1.0f;
        rRawInfo.rRrzInfo.RtoH = 1.0f;
        ...
        if (!u1PGN){
            ...
            {
                rRawInfo.rRrzInfo.bEnable = MTRUE;
                rRawInfo.rRrzInfo.OfstX = rP1Crop.p.x;
                rRawInfo.rRrzInfo.OfstY = rP1Crop.p.y;
                rRawInfo.rRrzInfo.RtoW = (MFLOAT)rRzSize.w/rP1Crop.s.w;
                rRawInfo.rRrzInfo.RtoH = (MFLOAT)rRzSize.h/rP1Crop.s.h;
            }
        }
        ...
        MRect Preview_Crop;
        if (QUERY_ENTRY_SINGLE(control.halMeta, MTK_3A_PRV_CROP_REGION, Preview_Crop))
        {
            AFArea_T& Area = mParams.rAFAreas;
            Area.i4Left    = Preview_Crop.p.x;
            Area.i4Top     = Preview_Crop.p.y;
            Area.i4Width   = Preview_Crop.s.w;
            Area.i4Height  = Preview_Crop.s.h;
            MY_LOG_IF(fgLog, "Zoom Info for af_mgr: %d %d %d %d", Preview_Crop.p.x, Preview_Crop.p.y, Preview_Crop.s.w, Preview_Crop.s.h);
        }
    }
