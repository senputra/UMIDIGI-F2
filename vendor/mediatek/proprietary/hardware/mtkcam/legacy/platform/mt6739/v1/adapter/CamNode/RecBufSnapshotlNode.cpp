    ...
    MUINT32             format)
{
    ...
    //allocate buffer
    MUINT32 bufStrideInByte[3] = {0, 0, 0};
    if(format == eImgFmt_NV12 || format == eImgFmt_NV21){
        bufStrideInByte[0] = ALIGN_UP_SIZE(width,    REC_IMG_STRIDE_Y);
        bufStrideInByte[1] = ALIGN_UP_SIZE(width,    REC_IMG_STRIDE_U);
    }
    else
    {
        bufStrideInByte[0] = ALIGN_UP_SIZE(width,    REC_IMG_STRIDE_Y);
        bufStrideInByte[1] = ALIGN_UP_SIZE(width/2,  REC_IMG_STRIDE_U);
        bufStrideInByte[2] = ALIGN_UP_SIZE(width/2,  REC_IMG_STRIDE_V);
    }
