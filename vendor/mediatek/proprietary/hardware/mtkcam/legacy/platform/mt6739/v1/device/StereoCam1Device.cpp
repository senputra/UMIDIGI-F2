...
//   direct link to sensor driver header file
...
onInit()
{
    ...
    MY_LOGD("StereoCam1Device : init : mSensorId_Main = %d ",getOpenId_Main());
    MY_LOGD("StereoCam1Device : init : mSensorId_Main2 = %d ",getOpenId_Main2());
    ...
    //
    if ( pCamMgr->getDeviceCount() != 0 )
    {
        MY_LOGE("no memory: device(%d)", pCamMgr->getDeviceCount());
        goto lbExit;
    }
    //
    //--------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------
    {
        ...
        //  (0) power on sensor (create another thread to power on sensor)
...
onUninit()
{
...
#if '1'==MTKCAM_HAVE_SENSOR_HAL
    {
        ...
        if(mpHalSensor)
        {
            // for mt6739 dualcam, we must disable master/slave and dualcam mode before power off
...
setSensorMode(MBOOL enable)
{
    ...
    if(enable)
    {
        ...
        MY_LOGI("get from sensor: main1_syncMode=%d, main2_syncMode=%d",main1_syncMode,main2_syncMode);
...
powerOnSensor()
{
    ...
    // for mt6739 dualcam, we must query master/slave mode and set sensor to one master and one slave mode
