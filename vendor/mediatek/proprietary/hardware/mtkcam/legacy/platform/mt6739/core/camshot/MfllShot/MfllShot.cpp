...
#define MY_LOGI(fmt, arg...)    do {CAM_LOGI(MFLL_HAL_TAG fmt, ##arg);printf(MFLL_HAL_TAG fmt "\n", ##arg);} while (0)
#define MY_LOGD(fmt, arg...)    do {CAM_LOGD(MFLL_HAL_TAG fmt, ##arg);printf(MFLL_HAL_TAG fmt "\n", ##arg);} while (0)
#define MY_LOGE(fmt, arg...)    do {CAM_LOGE("error" MFLL_HAL_TAG fmt, ##arg);printf(MFLL_HAL_TAG fmt "\n", ##arg);} while (0)
...
doBss()
{
    ...
    param.MF_BSS_ON            = static_cast<MUINT8>(mEnableBss);
    param.MF_BSS_ROI_WIDTH     = roiW;   //1520
    param.MF_BSS_ROI_HEIGHT    = roiH;   //900
    param.MF_BSS_ROI_X0        = roiX;   //40
    param.MF_BSS_ROI_Y0        = roiY;   //150
    param.MF_BSS_SCALE_FACTOR  = CUST_MFLL_BSS_SCALE_FACTOR;
    param.MF_BSS_CLIP_TH0      = CUST_MFLL_BSS_CLIP_TH0;
    param.MF_BSS_CLIP_TH1      = CUST_MFLL_BSS_CLIP_TH1;
    param.MF_BSS_ZERO          = CUST_MFLL_BSS_ZERO;
...
doMfb(MUINT32 ref)
{
    ...
    if(!ret) {
        ...
        MY_LOGE("[MFB BLD] deque magicNum(%x)", dequeParams.mFrameNo);
        MY_LOGE("[MFB BLD] deque mvIn.size(%zu), mvOut.size(%zu)"
...
encodeRawToYuv(sp<IImageBuffer> &rawBuffer, sp<IImageBuffer> &yuvBuffer, EIspProfile_T profile, void* /* privateData */)
{
    ...
    MY_LOGD("deque mvIn.size(%zu),mvOut.size(%zu)", dequeParams.mvIn.size(), dequeParams.mvOut.size());
...
doMixing()
{
    ...
    if(!ret)
    {
        ...
        MY_LOGE("[MFB MIX] deque mvIn.size(%zu), mvOut.size(%zu)", dequeParams.mvIn.size(), dequeParams.mvOut.size());
        goto lbExit;
    }
        ...
        case ECamShot_NRTYPE_HWNR:
            {
                ...
                sp<IImageBuffer> pInternalBuf;
                {
                    ...
                    MINT32 bufBoundaryInBytes[3] = {0, 0, 0};
                    MUINT32 bufStrideInByte[3] = { (MUINT32)pMemoryYuvMixing->getBufStridesInBytes(0), 0, 0 };
...
getCaptureInfo(MUINT32 mfbMode, MUINT32 sensorId, MUINT32 &frameNumber, MUINT32 &exp, MUINT32 &iso,MBOOL &needUpdateAE)
{
    ...
    // remove unused parameter build warnings
    (void)(needUpdateAE);
    (void)(iso);
    (void)(exp);
