...
{
    cfgImgo = ((mSensorInfo.sensorType == SENSOR_TYPE_YUV) || (mInitCfg.muSensorIspIf == SENSOR_ISP_CAMSV)) || isDataConnected(PASS1_FULLRAW) || isDataConnected(PASS1_PURERAW);
    cfgRrzo = (mSensorInfo.sensorType == SENSOR_TYPE_RAW) && (mInitCfg.muSensorIspIf == SENSOR_ISP_TG);
    // ideal
    ...
    fakeResized = ((mSensorInfo.sensorType == SENSOR_TYPE_YUV) || (mInitCfg.muSensorIspIf == SENSOR_ISP_CAMSV)) && isDataConnected(PASS1_RESIZEDRAW);
...
doThreadAllocBuf(void* arg)
{
    ...
    ::sched_setscheduler(0, SCHED_RR, &sched_p);
    //
