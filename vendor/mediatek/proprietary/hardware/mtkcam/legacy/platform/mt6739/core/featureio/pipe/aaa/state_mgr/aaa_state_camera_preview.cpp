...
sendIntent(intent2type<eIntent_VsyncUpdate>)
{
    ...
    MBOOL fgLogEn = (m_pHal3A->m_3ALogEnable & EN_3A_FLOW_LOG) ? MTRUE : MFALSE;
    ...
    if (m_pStateMgr->getFrameCount() < 0) {// AAO statistics is not ready
        ...
        MY_LOG( "[StateCameraPreview::sendIntent<eIntent_VsyncUpdate>] frameCnt=%d, statistics is not ready, dequeue/enqueue/update base address directly", m_pStateMgr->getFrameCount());
...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    MY_LOG_IF(fgLogEn, "[StateCameraPreview::sendIntent<eIntent_VsyncUpdate>] deQHwBufAAO done\n");
}
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    MY_LOG_IF(fgLogEn, "[StateCameraPreview::sendIntent<eIntent_VsyncUpdate>] enQHwBufAAO done\n");

    // Update AAO DMA base address for next frame
    if (m_pScheduler->jobAssignAndTimerStart(E_Job_IspValidate, MFALSE))
        IAAOBufMgr::getInstance().updateDMABaseAddr(m_SensorDevId);
    m_pScheduler->jobTimerEnd(E_Job_IspValidate);
    MY_LOG_IF(fgLogEn, "[StateCameraPreview::sendIntent<eIntent_VsyncUpdate>] updateDMAAddr done\n");
}
...
#endif
    {
        if (m_pScheduler->jobAssignAndTimerStart(E_Job_Flicker))
        {
            ...
            if(flkIn.afFullStat.bValid==1)
            {
                ...
                {
                    MY_LOG_IF(fgLogEn, "setaeflicker 60hz");
                    IAeMgr::getInstance().setAEAutoFlickerMode(m_SensorDevId, 1);
                }
                else
                {
                    MY_LOG_IF(fgLogEn, "setaeflicker 50hz");
            ...
            else
            {
                ...
                MY_LOG_IF(fgLogEn, "flk=%d, m_SensorDevId=%d", flkResult, m_SensorDevId);
                flkOut.flickerResult = flkResult;
                if(flkOut.flickerResult == HAL_FLICKER_AUTO_60HZ)
                {
                    MY_LOG_IF(fgLogEn, "setaeflicker 60hz");
                ...
                MY_LOG_IF(fgLogEn, "skip flicker");
            }
