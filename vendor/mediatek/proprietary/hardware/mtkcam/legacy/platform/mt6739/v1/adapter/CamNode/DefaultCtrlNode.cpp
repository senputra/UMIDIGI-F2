...
onInit()
{
    ...
    if( mPreviewSize.w/2 < LMV_MIN_SIZE || mPreviewSize.h < LMV_MIN_SIZE ||
        (mRecordingHint && (mVideoSize.w/2 < LMV_MIN_SIZE || mVideoSize.h < LMV_MIN_SIZE)) )
    {
         mEisScenario = EIS_SCE_NONE;
         MY_LOGW("Disable LMV because inputSize is too small: PreviewSize(%dx%d), VideoSize(%dx%d), mRecordingHint(%d)",
                 mPreviewSize.w, mPreviewSize.h, mVideoSize.w, mVideoSize.h, mRecordingHint);
    }
#else
    // Check LMV limitation
    if( mPreviewSize.w/2 < LMV_MIN_SIZE || mPreviewSize.h < LMV_MIN_SIZE ||
...
#if 0
                    ALOGE("sensor(%d,%d),Crop(%d,%d),Resize(%d,%d)",
                            mSensorSize.w,mSensorSize.h,
                            inCrop.s.w,inCrop.s.h,
                            outSize.w,outSize.h);
#endif
                    RRZOInfo _RRZO_info(
                            mSensorSize.w,
                            ...
                            outSize.w,
                            outSize.h);
                    mpHal3a->send3ACtrl(E3ACtrl_SetRRZOInfo, reinterpret_cast<MINTPTR>(&_RRZO_info),0);
                }
...
{
    MY_LOGI("RPBuf:Num RRZO(%d) IMGO(%d),RBBuf:Num(%d) Period(%d),PrvMaxFps(%d)",
