	...
	,mCctOn(MFALSE)
{
    ...
    m_pAcdkMhalObj = NULL;
    m_pAcdkMhalObj = AcdkMhalBase::createInstance(0, mCctOn);
    ...
    m_pAcdkMhalObjEng = AcdkMhalBase::createInstance(1, mCctOn);
...
MINT32 AcdkMain::init()
{
    ...
    //====== 3A init ======
        ...
        if (!mpIHal3A->setParams(cam3aParam)) {
            ACDK_LOGE("mpIHal3A->setParams() fail\n");
        }
...
MINT32 AcdkMain::startPreview_CameraDev(Func_CB prvCb,  MUINT32 const mode)
{
    ...
    MUINT32 captureSize = 0, videoSize = 0;
    ...
    captureSize = sensorStaticInfo[0].captureWidth*sensorStaticInfo[0].captureHeight*2;
    videoSize = sensorStaticInfo[0].videoWidth*sensorStaticInfo[0].videoHeight*2;

    //====== Allocate Memory ======
    sensorImgSize = (captureSize > videoSize) ? captureSize : videoSize;
...
MINT32 AcdkMain::saveRAWImg(MINT32 mode)
{
    ...
    else
    {
        ...
        else if(mCapType == PROCESSED_RAW10_TYPE || g_dumpRAW == PROCESSED_RAW10_TYPE)
        {
            ...
            sprintf(szFileName, "%s/acdkCapUnknown.raw" , mCctOn? MEDIA_PATH_CCT:MEDIA_PATH);
        }
    }
