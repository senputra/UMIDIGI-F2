...
HdrHal::init(void *pInitInData)
{
    ...
    for (MUINT32 i = 0; i < MAX_PYRAMID_LEVEL; i++)
    {
...
HdrHal::Do_FeatureExtraction(HDR_PIPE_FEATURE_EXTRACT_INPUT_INFO& rHdrPipeFeatureExtractInputInfo)
{
    ...
    HDR_LOGD("Do_FeatureExtraction match_no(%d),threshold(%d)", gRecPar.m_Match->match_no,CustomHdrMatchPointThreshold());

    if(gRecPar.m_Match->match_no <= CustomHdrMatchPointThreshold()){
        return MFALSE;
    }
...
{
    HDR_LOGD("- E. u4BufAddr: %p. u4BufSize: %d.", u4BufAddr, u4BufSize);
