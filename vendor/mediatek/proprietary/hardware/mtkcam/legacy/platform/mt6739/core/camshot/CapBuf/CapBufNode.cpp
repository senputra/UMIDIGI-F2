...
threadLoopUpdate()
{
    ...
    if(isDataConnected(PASS1_FULLRAW) && (imgoAddr))
    {
        mpIspSyncCtrl->queryImgBufInfo(
                            ...
                            pPrivateData,
                            privateDataSize);
        MY_LOGD("imgoAddr(0x%X),MN(0x%08X), send PASS1_EOF", imgoAddr, magicNum);
        handleNotify(PASS1_EOF, -1, magicNum);
    }
    else if(isDataConnected(PASS1_RESIZEDRAW) && (rrzoAddr))
    {
        mpIspSyncCtrl->queryImgBufInfo(
                            ...
                            privateDataSize);
        //
        MY_LOGD("rrzoAddr(0x%08X),MN(0x%08X), send PASS1_EOF", rrzoAddr, magicNum);
        handleNotify(PASS1_EOF, -1, magicNum);
    }

    if(imgoAddr)
    {
        MY_LOGD("post buffer: PASS1_FULLRAW, imgoAddr=%p",imgoAddr);
        handlePostBuffer(PASS1_FULLRAW, imgoAddr, 0);
    }
    if(rrzoAddr)
    {
        MY_LOGD("post buffer: PASS1_RESIZEDRAW, rrzoAddr=%p",rrzoAddr);
