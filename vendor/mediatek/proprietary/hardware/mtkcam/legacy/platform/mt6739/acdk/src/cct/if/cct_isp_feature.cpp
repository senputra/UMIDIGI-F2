...
IMP_CCT_CTRL( ACDK_CCT_OP_QUERY_ISP_ID )
{
    ...
    *reinterpret_cast<MUINT32 *>(puParaOut) = sizeof(MUINTPTR) == 4 ? 0x67398A00 : 0x67398AFF; // 00 means 32bit; FF means 64bit
...
#define MY_SET_TUNING_INDEX(_category)\
    ...
    switch  (eCategory)
    {
        ...
        case EIsp_Category_BPC:
            if  ( IspNvramRegMgr::NUM_BPC <= u4Index )
                return  CCTIF_BAD_PARAM;
            m_rISPRegsIdx.BNR_BPC = static_cast<MUINT8>(u4Index);
            break;
        case EIsp_Category_NR1:
            if  ( IspNvramRegMgr::NUM_NR1 <= u4Index )
                return  CCTIF_BAD_PARAM;
            m_rISPRegsIdx.BNR_NR1 = static_cast<MUINT8>(u4Index);
            break;
        case EIsp_Category_PDC:
            if  ( IspNvramRegMgr::NUM_PDC <= u4Index )
                return  CCTIF_BAD_PARAM;
            m_rISPRegsIdx.BNR_PDC = static_cast<MUINT8>(u4Index);
            break;
...
#define MY_GET_TUNING_INDEX(_category)\
    ...
    switch  (eCategory)
    {
        ...
        case EIsp_Category_BPC:
            rIndex = m_rISPRegsIdx.BNR_BPC;
            break;
        case EIsp_Category_NR1:
            rIndex = m_rISPRegsIdx.BNR_NR1;
            break;
        case EIsp_Category_PDC:
            rIndex = m_rISPRegsIdx.BNR_PDC;
            break;
...
{
    typedef NVRAM_CAMERA_ISP_PARAM_STRUCT o_type;
    if  ( sizeof(o_type) != u4ParaOutLen || ! pu4RealParaOutLen || ! puParaOut )
        return  CCTIF_BAD_PARAM;

    MINT32 err = CCTIF_NO_ERROR;
    void* pBuf; //temp used
    err = NvBufUtil::getInstance().getBufAndRead(CAMERA_NVRAM_DATA_ISP, m_eSensorEnum, (void*&)pBuf);

    memcpy((void *)puParaOut, (void *)pBuf, sizeof(o_type));
...
{
    typedef NVRAM_CAMERA_ISP_PARAM_STRUCT type;
    if  ( sizeof(type) != u4ParaInLen || ! puParaIn )
        return  CCTIF_BAD_PARAM;
    ...
    memcpy((void *)pBuf, (void *)puParaIn, u4ParaInLen);
    return  CCTIF_NO_ERROR;
}
...
IMP_CCT_CTRL( ACDK_CCT_OP_ISP_SET_PCA_TABLE )
{
    ...
    case 0:
        pBuf_fw = (pAccess->u8Index == 0/*0: origianl PCA, 1 : HDR PCA, 2 : flash PCA */) ?
            &m_rISPPca.PCA_LUTS.lut_lo[u4Offset] : pAccess->u8Index == 1 ? &m_rISPPca.PCA_LUTS.lut_lo2[u4Offset] : &m_rISPPca.PCA_LUTS.lut_lo3[u4Offset];
        break;
    case 1:
        pBuf_fw = (pAccess->u8Index == 0/*0: origianl PCA, 1 : HDR PCA, 2 : flash PCA  */) ?
            &m_rISPPca.PCA_LUTS.lut_md[u4Offset] : pAccess->u8Index == 1 ? &m_rISPPca.PCA_LUTS.lut_md2[u4Offset] : &m_rISPPca.PCA_LUTS.lut_md3[u4Offset];
        break;
    case 2:
        pBuf_fw = (pAccess->u8Index == 0/*0: origianl PCA, 1 : HDR PCA, 2 : flash PCA  */) ?
            &m_rISPPca.PCA_LUTS.lut_hi[u4Offset] : pAccess->u8Index == 1 ? &m_rISPPca.PCA_LUTS.lut_hi2[u4Offset] : &m_rISPPca.PCA_LUTS.lut_hi3[u4Offset];
        break;
    default:
