...
{
    MY_LOG_IF(1, "setISO(%d)", i4ISOValue);
    ...
    if (checkParamChange(m_rIspCamInfo.u4ISOValue, (MUINT32)i4ISOValue)) {
        MY_LOG_IF(1, "[+m_rIspCamInfo.u4ISOValue](old, new)=(%d, %d)", m_rIspCamInfo.u4ISOValue, i4ISOValue);
        m_rIspCamInfo.u4ISOValue = i4ISOValue;
    }

    // ISO index
    if (checkParamChange(m_rIspCamInfo.eIdx_ISO, eIdx_ISO)) {
        MY_LOG_IF(1, "[+m_rIspCamInfo.eIdx_ISO](old, new)=(%d, %d)", m_rIspCamInfo.eIdx_ISO, eIdx_ISO);
...
setAWBInfo(AWB_INFO_T const &rAWBInfo)
{
   ...
   get_CTIdx_neighbor(m_rIspCamInfo, m_rIspCamInfo.eIdx_CT, m_rIspCamInfo.rAWBInfo.i4CCT);
         ...
         bAWBGainChanged)
    {
        ...
        m_pCcmMgr->calculateCCM(m_i4FlashOnOff, bInvokeSmoothCCMwPrefGain, 0, m_rIspCamInfo);
...
MERROR_ENUM
Paramctrl::
setRRZInfo(ISP_RRZ_INFO_T const rRrzInfo)
{
    //MY_LOG_IF(m_bDebugEnable, "[%s] i4TGInfo = %d", __FUNCTION__, i4TGInfo);

    Mutex::Autolock lock(m_Lock);

    m_rIspCamInfo.rRrzInfo = rRrzInfo;
