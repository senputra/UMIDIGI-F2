...
)
{
    ...
    // create imem driver
        ...
        LOG_DBG("[m_pIMemDrv]:0x%08x ",(unsigned int*)m_pIMemDrv);
        if ( NULL == m_pIMemDrv ) {
            ...
            if(misV3){
                        if(rParams.mvTuningData.size()==0) {
                            ...
                            return MFALSE;
                                ...
                                LOG_INF("q(%d),pTuningQue(0x%x)",q,(unsigned int*)pTuningQue);
                        }
                        //pass1 data
    ...
    void* &pTuningQueBuf)
{
    ...
    if(misV3)
    {
        ...
        if(!gTuningQueNode.empty()) {
            ...
            //
            LOG_INF("queNum(%d),detuningQue(0x%08x)\n",gTuningQueNode.size(),(unsigned int*)pTuningQueBuf);
        } else {
    ...
    void* pTuningQueBuf)
{
    ...
    if(misV3)
    {
        ...
        if(isCheckPass == true){
            ...
            LOG_ERR("[Error]tuningque buffer(0x%08x) is invalid",(unsigned int*)pTuningQueBuf);
        }
        //
        for(MUINT32 i=0;i<gTuningQueNode.size();i++){
            LOG_INF("i(%d),pmem(0x%08x)\n",i,(unsigned int*)gTuningQueNode[i]);
        }
    }
    else
    {
