...
{
    FUNCTION_LOG_START;
#define __INIT_STRUCTURE(x) ::memset(&x, 0, sizeof(x))
    __INIT_STRUCTURE(MfbllInitPara);
    __INIT_STRUCTURE(Proc1_get_proc_info);
    __INIT_STRUCTURE(Proc1_set_proc_info);
    __INIT_STRUCTURE(MfbllMemcResult);
    __INIT_STRUCTURE(mTuning);
#undef  __INIT_STRUCTURE
...
MBOOL MfbllHal::setWorkingBuffer(MUINT8 *workingBuffer)
{
    ...
    if(workingBuffer == NULL) {
        MY_LOGD("workingBuffer = %p", workingBuffer);
        return MFALSE;
    }
