...
{
    bool ret = true;
    MY_LOGD_IF(FeaturePipeNode::sDebugLogEnable,"+");
    DpBlitStream dpBlit;
...
initPerf()
{
    LOG_DBG("disable perf control after Android O");
    #if 0
    LOG_ERR("");
...
FeaturePipeP2aNode::onInit()
{
    ...
    )   // For 3DNR only case.
    {
        ...
        LOG_DBG("call IFeatureStream::createInstance()-1: eTag=%d, openedSensorIndex=%d",EFeatureStreamTag_Stream, mOpenedSensorIndex);
    ...
    {
        /* Create IFeatureStream */
        LOG_DBG("call IFeatureStream::createInstance()-3: eTag=%d, openedSensorIndex=%d",EFeatureStreamTag_vFB_Stream, mOpenedSensorIndex);
    ...
    if (FEATURE_MASK_IS_3DNR_ENABLED(mFeatureOnMask))
    {
        ...
        if (!m3dnrPrvFrmQueue.empty())
        {
            ...
            if (m3dnrStateMachine == NR3D_STATE_WORKING)
            {
                ...
                MY_LOGD_IF(sDebugLogEnable,"[P2A sets VIPI mvIn  ] 3dnrSM1(%d S0P1W2),ES(0x%02X FsSzDfLrIn),WH(%d,%d),OfstXY(%d,%d),mem<va(0x%x)/pa(0x%x)/TS(%lld)/size(0x%x)>,mPrvQueSz(%d),imgSizeWH(%d,%d),PrvWH(%d,%d),ExtOfstByte(%d)",
                        m3dnrStateMachine,
                        m3dnrErrorStatus,
...

    {
        ...
        for (MUINT8 i = 0; i < GPU_NODE_BUFFER_NUM; i++)
        {
            AllocInfo allocinfo(FULL_IMAGE_MAX_WIDTH,
                                ...
                                eImgFmt_RGBA8888,
                                eBUFFER_USAGE_HW_CAMERA_READWRITE | eBUFFER_USAGE_SW_READ_OFTEN | eBUFFER_USAGE_SW_WRITE_RARELY);
            mGfxRgbaBuffer[i] = new NativeBufferWrapper(FULL_IMAGE_MAX_WIDTH, FULL_IMAGE_MAX_HEIGHT,
...
threadLoopUpdate()
{
    ...
    else if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        MUINT32 size[3] = {(MUINT32)(FULL_IMAGE_MAX_WIDTH * pImageBuffer->getImgSize().h * 4),0,0};
...
FeaturePipeP2bNode::onInit()
{
    ...
    /* Create IFeatureStream */
    LOG_DBG("call IFeatureStream::createInstance()-4: eTag=%d, openedSensorIndex=%d",EFeatureStreamTag_vFB_FB, mOpenedSensorIndex);
