...
waitForJpegCbDone()
{
    ...
    while( mShotCount > 0 )
    {
        ...
        if  ( !getCallbackType(&cbtype) )
        {
            MY_LOGE("get cb type fail, mShotCount:%d", mShotCount);
            break;
        }
        handleCallback(cbtype);
    }
    return;
}
