...
GAFLensDrv::init(unsigned int a_u4CurrSensorDev )
{
    ...
    else if(a_u4CurrSensorDev==MCU_DEV_SUB_2)
    {
        ...
        m_userCnt_sub2++;
        DRV_DBG("sub lens init() [m_userCnt]%d [fdMCU_sub2]%d - \n", m_userCnt_sub2,m_fdMCU_sub2);
        sprintf(cBuf,"%s",MCUDrv::m_LensInitFunc_sub2[MCUDrv::m_u4CurrLensIdx_sub2].LensDrvName);
    }
    else
        return MCUDrv::MCU_INVALID_DRIVER;

    DRV_DBG("[GLensDriver]%s\n", cBuf);
    DRV_DBG("[GLensDriverName]%ld\n", strtol(cBuf, NULL, 0));
...
GAFLensDrv::uninit(unsigned int a_u4CurrSensorDev )
{
    ...
    {
        DRV_DBG("sub2 lens uninit() [m_userCnt]%d [fdMCU_sub]%d + \n", m_userCnt_sub2, m_fdMCU_sub2);
        Mutex::Autolock lock(mLock);
        if (m_userCnt_sub2 == 1)
        {
        ...
        DRV_DBG("sub2 lens uninit() [fdMCU_sub2]%d - \n", m_fdMCU_sub2);
    }
    else 
        return MCUDrv::MCU_INVALID_DRIVER;
...
********************************************************************************/
int
GAFLensDrv::getMCUCalPos(mcuMotorCalPos* /*a_pMotorCalPos*/, unsigned int /*a_u4CurrSensorDev*/)
{
    return MCUDrv::MCU_NO_ERROR;
}
...
********************************************************************************/
int
GAFLensDrv::setMCUParam(int a_CmdId, int a_Param,unsigned int a_u4CurrSensorDev )
{
    int err, a_fdMCU,a_u4CurrLensIdx;
    ...
    if (a_fdMCU < 0)
    {
        ...
        {
            DRV_ERR("[setMCUParam] invalid m_fdMCU =%d\n", a_fdMCU);
            return MCUDrv::MCU_INVALID_DRIVER;
        }
    }
...
********************************************************************************/
int
GAFLensDrv::getMCUOISInfo(mcuMotorOISInfo* /*a_pMotorOISInfo*/, unsigned int /*a_u4CurrSensorDev*/)
{
    return MCUDrv::MCU_NO_ERROR;
}
