...
getInstance()
{
    ...
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "[Create] &mFDCount:%p &gLock:%p", &mFDCount, &gLock);
...
{
    Mutex::Autolock _l(gLock);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "[Delete] &mFDCount:%p &gLock:%p", &mFDCount, &gLock);
...
*/
    ...
    FDFTInitInfo.FDBufHeight =  FDFTInitInfo.FDBufWidth * fdH/fdW;  
    ...
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "WorkingBufAddr = %d", FDFTInitInfo.WorkingBufAddr);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "WorkingBufSize = %d", FDFTInitInfo.WorkingBufSize);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDThreadNum = %d", FDFTInitInfo.FDThreadNum);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDThreshold = %d", FDFTInitInfo.FDThreshold);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "MajorFaceDecision = %d", FDFTInitInfo.MajorFaceDecision);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "OTRatio = %d", FDFTInitInfo.OTRatio);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "SmoothLevel = %d", FDFTInitInfo.SmoothLevel);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDSkipStep = %d", FDFTInitInfo.FDSkipStep);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDRectify = %d", FDFTInitInfo.FDRectify);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDRefresh = %d", FDFTInitInfo.FDRefresh);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDBufWidth = %d", FDFTInitInfo.FDBufWidth);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDBufHeight = %d", FDFTInitInfo.FDBufHeight);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDTBufWidth = %d", FDFTInitInfo.FDTBufWidth);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDTBufHeight = %d", FDFTInitInfo.FDTBufHeight);

    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDImageArrayNum = %d", FDFTInitInfo.FDImageArrayNum);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDImageWidthArray = ");
    for (int i = 0; i < FD_SCALES; i++)
    {
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "%d, ",FDFTInitInfo.FDImageWidthArray[i]);
    }
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDImageHeightArray = ");
    for (int i = 0; i < FD_SCALES; i++)
    {
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "%d, ",FDFTInitInfo.FDImageHeightArray[i]);
    }
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDMinFaceLevel = %d", FDFTInitInfo.FDMinFaceLevel);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDMaxFaceLevel = %d", FDFTInitInfo.FDMaxFaceLevel);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDImgFmtCH1 = %d", FDFTInitInfo.FDImgFmtCH1);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDImgFmtCH2 = %d", FDFTInitInfo.FDImgFmtCH2);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "SDImgFmtCH1 = %d", FDFTInitInfo.SDImgFmtCH1);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "SDImgFmtCH2 = %d", FDFTInitInfo.SDImgFmtCH2);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "SDThreshold = %d", FDFTInitInfo.SDThreshold);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "SDMainFaceMust = %d", FDFTInitInfo.SDMainFaceMust);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "GSensor = %d", FDFTInitInfo.GSensor);
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "GenScaleImageBySw = %d", FDFTInitInfo.GenScaleImageBySw);
    ...
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "[%s] End", __FUNCTION__);
    m_Inited = 1;
    return err;
}
...
{
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO Extract Y In");
...
*/
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO Extract Y Out");
...
)
{
    ...
    {
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO RGB565 (SW) In");
        doRGB565BufferP1_SW(dstAddr, srcAddr, src_width, src_height);
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO RGB565 (SW) Out");
    }
    else if((src_width == 320) && (g_ucPlane ==1))
    {
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO RGB565 (SW_QVGA) In");
        doRGB565BufferQVGAP1_SW(dstAddr, srcAddr, src_width, src_height);
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO RGB565 (SW_QVGA) Out");
    }
    else
    {
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO RGB565 (DDP) In");
        doRGB565Buffer_DDP(dstAddr, srcAddr);
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "DO RGB565 (DDP) Out");
    }
...
)
{
    ...
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "SD Status: SDEnable:%d, img_w:%d, img_h:%d, Src_Phy_Addr:0x%x,", g_CurrentStatus, IMAGE_MAX_WIDTH, IMAGE_MAX_HEIGHT, ImageBufferPhy);
    ...
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "[halFDDo] IN,  %d", mFDCount);
    ...
    if(EnableSWResizerFlag)
    {
        ...
        else if(g_input_4_3 == 1)
        {
            ...
            else
            {
                if( mode == FDVT_GFD_MODE)
                {
                    ...
                    alreay_have_scale_images = 1;
                    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDVT_GFD_MODE 4:3 out");
                }
            }
        }
        else if(g_input_16_9 == 1)
        {
            ...
            {
                MY_LOGD_IF(MHAL_FDVT_LOG_EN, "g_input_16_9 IN");
                Create640WidthImage_Y((MUINT8*)ImageBuffer2, m_FDW, m_FDH, ImageVGABuffer);
                /*
                static int count1 = 0;
        FILE* fp;
        ...
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDVTMain IN");
        ...
        count++;
        */
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDVTMain out");
        ...
        */
        ...
        if(SDEnable)
        {
            MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDVT_SD_MODE IN");
            m_pMTKFDVTObj->FDVTMain( ImageScaleBuffer, y_vga, FDVT_SD_MODE, direction, 0, ImageBufferPhy);
            MY_LOGD_IF(MHAL_FDVT_LOG_EN, "FDVT_SD_MODE out");
        }
    }
    else
    {
...
{
    MUINT8 i;
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "[GetFaceInfo] NUM_Face:%d,", FdResult_Num);
...
)
{
    ...
    MY_LOGD_IF(MHAL_FDVT_LOG_EN, "[%s]m_DispW(%d) m_DispH(%d)", __FUNCTION__, m_DispW, m_DispH);
...
static void CreateScaleImagesFromPreview_Y(MUINT8* image_buffer_preview, MUINT8* image_buffer, MUINT32 w, MUINT32 h, MUINT32* ImageWidthArray, MUINT32* ImageHeightArray, MUINT32 sacles)
{
    ...
    for ( current_scale = 0 ; current_scale < sacles ; current_scale ++ )
    {
        ...
        {
            MY_LOGD_IF(MHAL_FDVT_LOG_EN, "PipeBilinearResizer break %d", current_scale);
            break;
        }
        PipeBilinearResizer(&UtlRisizerInfo);
        dst_ptr+= ImageWidthArray[current_scale]*ImageHeightArray[current_scale];
        MY_LOGD_IF(MHAL_FDVT_LOG_EN, "PipeBilinearResizer %d", current_scale);
    }
