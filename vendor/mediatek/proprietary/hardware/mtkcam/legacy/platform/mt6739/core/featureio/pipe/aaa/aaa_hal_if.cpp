...
Hal3AIf::createInstance(MINT32 const i4SensorOpenIndex)
{
    ...
    NSCam::Utils::Property::tryGet(s8DualCamWithCamsv, bIsDualCamWithCamSv);
    ...
    if(bIsDualCamWithCamSv && (i4SensorDevId == SENSOR_DEV_MAIN_2))
    {
        MY_LOG("[%s] Dual camera with CamSv", __FUNCTION__);
        return Hal3AYuv::createInstance(i4SensorDevId, i4SensorOpenIndex);
    }
    else if (eSensorType == NSSensorType::eRAW)
