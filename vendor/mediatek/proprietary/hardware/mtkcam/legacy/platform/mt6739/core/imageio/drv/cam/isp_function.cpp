...
cam_isp_bnr_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    if(imageioPackage.isApplyTuning){  //0x15004800 ~ 0x1500482C
        ...
        if (ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_BNR_BPC_CON, BPC_LUT_EN) == 1) {
            ...
            MUINT32 BPCI_BASE_PADDR, BPCI_OFFSET, BPCI_STRIDE, BPCI_XSIZE, BPCI_YSIZE;
        ...
        MUINT32 &tuningDmaTag)
{
    ...
    switch (drvScenario){
            ...
            case NSImageio::NSIspio::eDrvScenario_IP_TPipe:
            if(isTopEngine==MTRUE){
...
ISP_RAW_CENTRAL_CTRL_INFO::config2( ISP_TOP_CTRL * pSrcTopCtrl )
{
   ...
   if ( ISP_PASS1_CAMSV == pSrcTopCtrl->path || ISP_PASS1_CAMSV == pSrcTopCtrl->pathD ){
       ...
       pIspTop->CAMSV_MODULE_EN.Raw    = pSrcTopCtrl->isp_top_ctl.CAMSV_MODULE_EN.Raw;
       pIspTop->CAMSV_FMT_SEL.Raw      = pSrcTopCtrl->isp_top_ctl.CAMSV_FMT_SEL.Raw;
       pIspTop->CAMSV_FMT_SEL_MASK     = pSrcTopCtrl->isp_top_ctl.CAMSV_FMT_SEL_MASK;
       pIspTop->CAMSV_INT_EN.Raw       = pSrcTopCtrl->isp_top_ctl.CAMSV_INT_EN.Raw;
       pIspTop->CAMSV_PAK_MASK         = pSrcTopCtrl->isp_top_ctl.CAMSV_PAK_MASK;
       pIspTop->CAMSV_PAK.Raw          = pSrcTopCtrl->isp_top_ctl.CAMSV_PAK.Raw;
   }
   ...
   if ( ISP_PASS1_CAMSV_D == pSrcTopCtrl->path || ISP_PASS1_CAMSV_D == pSrcTopCtrl->pathD ){
       ...
       pIspTop->CAMSV2_MODULE_EN.Raw    = pSrcTopCtrl->isp_top_ctl.CAMSV2_MODULE_EN.Raw;
       pIspTop->CAMSV2_FMT_SEL.Raw      = pSrcTopCtrl->isp_top_ctl.CAMSV2_FMT_SEL.Raw;
       pIspTop->CAMSV2_FMT_SEL_MASK     = pSrcTopCtrl->isp_top_ctl.CAMSV2_FMT_SEL_MASK;
       pIspTop->CAMSV2_INT_EN.Raw       = pSrcTopCtrl->isp_top_ctl.CAMSV2_INT_EN.Raw;
       pIspTop->CAMSV2_PAK_MASK         = pSrcTopCtrl->isp_top_ctl.CAMSV2_PAK_MASK;
       pIspTop->CAMSV2_PAK.Raw          = pSrcTopCtrl->isp_top_ctl.CAMSV2_PAK.Raw;
   }
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if ( CAM_ISP_PASS1_START == (*start) ) {
        ...
        if( CAMSV_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV_MODULE_EN.Raw){
            ...
            ISP_FUNC_INF("ctrl(%d),id(%d),pExt(0x%x)",buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.pExtend);
            if ( MTRUE != this->m_pIspDrvShell->getPhyIspDrv()->rtBufCtrl((void*)&buf_ctrl) ) {
                ISP_FUNC_ERR("ERROR:rtBufCtrl");
                return -1;
            }
...
///////////////////E
        ...
        if (this->path == ISP_PASS1_CAMSV) {
            ...
            ISP_FUNC_INF("[PIP][TopCtrl:Disable]camsv(0x%x),FBC(0x%x/0x%x),TG(0x%x)", \
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_MODULE_EN),\
        ...
        if (this->pathD == ISP_PASS1_CAMSV_D) {
            ...
            ISP_FUNC_INF("[PIP][TopCtrl:Disable]camsv2(0x%x),FBC(0x%x/0x%x),TG(0x%x)", \
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_MODULE_EN),\
...
int ISP_RAW_PIPE::_config( void )
{
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        if (this->isV3)  //QQ??
        {
            ...
            else if(NSImageio::NSIspio::eDrvScenario_IP_TPipe == this->drvScenario){
                getP1EnTuningTag(MFALSE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag); // get P1 tuning En & Dma Tag
                finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
            }
            ...
            ISP_FUNC_INF("(ISP_RAW_PIPE),isApplyTuning(0x%x),finalEnP1(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%016llx)",this->isApplyTuning, finalEnP1, finalEnP2, tuningUpdateBit);
        }
        else
        {
            getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag); // get P2 tuning En & Dma Tag
        ...
        else if(NSImageio::NSIspio::eDrvScenario_IP_TPipe == this->drvScenario){ // only for scenario ip
            for(i=0;i<CHECK_TUNING_P1_EN_BITS;i++){
                if((finalEnP1 & gIspModuleMappingTableP1En[i].ctrlByte) &&
                        (gIspModuleMappingTableP1En[i].eModule==eIspModuleRaw) &&
                        (gIspModuleMappingTableP1En[i].default_func!=NULL)) {
...
int ISP_RAW_PIPE::_write2CQ(void)
{
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        if(this->isV3)
        {
            ...
            ISP_FUNC_DBG("finalEnP1(0x%08x),finalEnP2(0x%016llx)",finalEnP1,finalEnP2);
        }
        else
        {
        getP1EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag);
...
int ISP_YUV_PIPE::_config( void )
{
    ...
    if (this->isV3)
    {
        ...
        ISP_FUNC_INF("(ISP_YUV_PIPE),isApplyTuning(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%016llx)",this->isApplyTuning,finalEnP2,tuningUpdateBit);
    }
    else
    {
    getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag); // get P2 tuning En & Dma Tag
...
//        ISP_FUNC_INF("QQ,imgxo srcCropW(%d),srcCropH(%d)",gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].dstDma[ISP_MDP_DL_IMGXO].srcCropW ,gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].dstDma[ISP_MDP_DL_IMGXO].srcCropH);
    ...
    // collect crop info for MdpMgr
        ...
        if (this->drvScenario != NSImageio::NSIspio::eDrvScenario_IP && this->drvScenario != NSImageio::NSIspio::eDrvScenario_IP_TPipe)
        {
...
//            }
        ...
        else if(this->drvScenario == NSImageio::NSIspio::eDrvScenario_IP_TPipe)
        {
            getP1EnTuningTag(MTRUE,
                           (ISP_DRV_CQ_ENUM)this->CQ,
                           this->magicNum,
