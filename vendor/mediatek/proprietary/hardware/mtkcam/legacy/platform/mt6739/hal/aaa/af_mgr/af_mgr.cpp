...
MINT32 AfMgr::CCTOPAFGetNVRAMParam(MVOID *a_pAFNVRAM, MUINT32 *a_pOutLen)
{
    ...
    m_sNVRam.rFocusRange = m_ptrLensNVRam->rFocusRange;
    m_sNVRam.rAFNVRAM= m_ptrLensNVRam->rAFNVRAM;
    ...
    NvramDrvMgr::getInstance().getRefBuf(m_ptrLensNVRam);
    NvramDrvMgr::getInstance().uninit();

    m_sNVRam.Version = m_ptrLensNVRam->Version;
    m_sNVRam.rFocusRange = m_ptrLensNVRam->rFocusRange;
    m_sNVRam.rAFNVRAM= m_ptrLensNVRam->rAFNVRAM;
    */
...
MINT32 AfMgr::CCTOPAFApplyNVRAMParam(MVOID *a_pAFNVRAM)
{
    ...
    {
        memcpy( &m_sNVRam.rSCENARIONVRAM[m_eCamScenarioMode], &m_sNVRam.rAFNVRAM, sizeof(m_sNVRam.rSCENARIONVRAM[m_eCamScenarioMode]));
        CAM_LOGD( "Apply to Phone[Scenario mode] %d", __FUNCTION__, m_eCamScenarioMode);
    }
...
MINT32 AfMgr::CCTOPAFSaveNVRAMParam()
{
    ...
    //m_ptrLensNVRam->Version = m_sNVRam.Version;
    m_ptrLensNVRam->rFocusRange = m_sNVRam.rFocusRange;
    m_ptrLensNVRam->rAFNVRAM = m_sNVRam.rAFNVRAM;
...
MRESULT AfMgr::doAF(MVOID *pAFStatBuf)
{
    ...
    {
        i4RequestCount++;
        CAM_LOGD_IF(m_i4DgbLogLv, "[%s] getAFRequest(%d)", __FUNCTION__, rReq.afCmd);
        switch(rReq.afCmd)
        {
        case AF_CMD_AUTOFOCUS:
            ExeAutoFocus(m_sHostCmd.bIsAutoFocusFromHost);
