...
ContinuousShot(
    ...
    , mMaxJpegRatio(80)
{
    mMaxJpegRatio = property_get_int32("debug.camera.cshot.ratio", 80);
}
...
onCmd_capture()
{
...

    {
        ...
        if( mJpegParam.mu4JpegQuality > mMaxJpegRatio ) {
                MY_LOGW("limit jpeg quality (%d -> %d)", mJpegParam.mu4JpegQuality, mMaxJpegRatio);
                mJpegParam.mu4JpegQuality = mMaxJpegRatio;
        }
...
onCmd_setCShotSpeed(uint32_t u4CShotSpeed)
{
    ...
    {
        MY_LOGI("set continuous shot speed: %d", u4CShotSpeed);
