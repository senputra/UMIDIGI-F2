...
threadLoopUpdate()
{
    ...
    PostBufInfo postBufData;
    {
        ...
        MY_LOGD("size(%d)",mlPostBufData.size());
        while(1)
        {
            ...
            handleReturnBuffer(postBufData.data,(MUINTPTR)(postBufData.buf),0);
        }
    }
...
pushBuf(MUINT32 const data, IImageBuffer* const buf, MUINT32 const ext)
{
    ...
    MY_LOGD("size(%d), data(%d), buf(0x%x),ts(%lld)",
            mlPostBufData.size(),
            postBufData.data,
            postBufData.buf,
            postBufData.buf->getTimestamp());

    muPostFrameCnt++;

    if( isReadyToTransfrom() )
    {
        if(mlPostBufData.size() == 1)
        {
            triggerLoop();
        }
    }
