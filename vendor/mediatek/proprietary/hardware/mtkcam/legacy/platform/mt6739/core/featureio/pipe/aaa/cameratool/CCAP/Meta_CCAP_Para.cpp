...
bool FT_ACDK_CCT_V2_OP_ISP_GET_TUNING_PARAS (const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
    ...
    g_PeerBufferLen = sizeof(NVRAM_CAMERA_ISP_PARAM_STRUCT);
    g_pPeerBuffer = (char*)malloc(g_PeerBufferLen);
    if(g_pPeerBuffer == NULL) {
        ACDK_LOGD("[CCAP]:g_pPeerBuffer failed!");
        return false;
    }
...
bool FT_ACDK_CCT_OP_SET_CCM_WB(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
  ...
  memcpy(&ccm_wb_gain, &(pREQ->cmd.set_ccm_wb_gain), sizeof(ccm_wb_gain));
...
void META_CCAP_ADB_OP()
{
    ...
    {
        ACDK_LOGD("[%s] inBuf size(%I64d)",  __FUNCTION__, file_size - sizeof(FT_CCT_REQ));
