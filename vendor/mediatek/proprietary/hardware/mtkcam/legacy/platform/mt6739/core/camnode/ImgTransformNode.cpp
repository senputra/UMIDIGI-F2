...
class ImgTransformNodeImpl : public ImgTransformNode
{
    ...
    public: // ctor & dtor
        ImgTransformNodeImpl(bool bUse_DST_2, bool bRot_DST_0);
...
ImgTransformNode*
ImgTransformNode::
createInstance(bool bUse_DST_2, bool bRot_DST_0)
{
    return new ImgTransformNodeImpl(bUse_DST_2, bRot_DST_0);
}
...
ImgTransformNodeImpl::
    ...
    , muPostFrameCnt(0)
    , muDeqFrameCnt(0)
    , mbUse_DST_2(bUse_DST_2)
    , mbRot_DST_0(bRot_DST_0)
{
    //DATA
    ...
    if(mbUse_DST_2)
        addDataSupport( ENDPOINT_DST, TRANSFORM_DST_2 );
    //
    if(mbUse_DST_2)
    {
        muDequeOrder[0] = TRANSFORM_DST_2;
        muDequeOrder[1] = 0;
    }
    else
    {
...
#define DUMP_IImageBuffer( type, pbuf, fileExt, cnt)               \
        ...
        switch( data )
        {
            ...
            case TRANSFORM_DST_1:
                ...
                break;
            case TRANSFORM_DST_2:
                DUMP_IImageBuffer( TRANSFORM_DST_2, buf, "yuv", muDeqFrameCnt  );
                break;
            default:
...
doImageTransform(MUINT32 const data, IImageBuffer* const buf, MRect const crop)
{
    ...
    MUINT32 transform = 0, transform_dst0 = 0;
    ...
    for(MUINT32 i = 0; i < MAX_DST_PORT_NUM; i++)
    {
        ...
        MY_LOGD("try to get dstDataType(%d) buffer handle",dstData);
        ...
        if( pBufHdl && pBufHdl->dequeBuffer( dstData, &outRequest) )
        {
            ...
            {
                case TRANSFORM_DST_0:
                    if(mbRot_DST_0)     // for stereo case, do mdp twice
                    {
                        transform = transform_dst0 = outRequest.mTransform;
                        MY_LOGD("Enable mbRot_DST_0, will do mdp twice");
                    }
                    else
                    {
...
#undef align2
    ...
    if(mbRot_DST_0 && transform_dst0)    // for stereo case && thumbnail need to transform case, do mdp twice
    {
        ret = ret && mpImgTransform->execute(buf, pDstBuf[0], NULL, newCrop, transform, 0xFFFFFFFF);
        MY_LOGD("do mdp for pDstBuf[0]=%p (ret=%d)", pDstBuf[0], ret);
       
        if(pDstBuf[1]!=NULL)
        {
            ret = ret && mpImgTransform->execute(buf, pDstBuf[1], NULL, newCrop, transform, 0xFFFFFFFF);
            MY_LOGD("do mdp for pDstBuf[1]=%p (ret=%d)", pDstBuf[1], ret);
        }
    }
    else
    {
