...
IMemDrv* IMemDrvImp::getInstance(void)
{
    ...
    IMEM_DBG("singleton[0x%lx],getpid[0x%08x],gettid[0x%08x] ",&singleton,getpid() ,gettid());
    IMEM_DRV_DELAY
    return &singleton;
}
//-----------------------------------------------------------------------------
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    if ( 0 ==  buf_map.count(pInfo->virtAddr) ) {
        ...
        if ( IMEM_MIN_ION_FD > pInfo->memID ) {
            ...
            //
            IMEM_DBG("Physical address=0x%08lu len=0x%lu", sys_data.get_phys_param.phy_addr, sys_data.get_phys_param.len);
