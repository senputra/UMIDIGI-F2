...
int ef_test()
{
    ...
    QTimeStampBufInfo rQTSBufInfo;
        ...
        for(int j=0;j<3;j++)
        {
            ...
            ret = mpImemDrv->allocVirtBuf(&(imgo_buf[j]));
            ret = mpImemDrv->mapPhyAddr(&(imgo_buf[j]));
            ...
            ret =mpImemDrv->mapPhyAddr(&(rrzo_buf[j]));
        ...
        for(int j=0;j<3;j++)
        {
            ret = mpImemDrv->unmapPhyAddr(&(imgo_buf[j]));
            ret = mpImemDrv->freeVirtBuf(&(imgo_buf[j]));
        }
    }
