...
doShadingAlign(MVOID* pMtkLsc)
{
...
#endif
    {
        ...
        if(m_u4LogEn & EN_LSC_LOG_GAIN_TABLE)
        {
...
updateLsc()
{
    ...
    if (IspTuningMgr::getInstance().getOperMode(m_eSensorDev) != NSIspTuning::EOperMode_Meta)
    {
        ISP_MGR_SL2_T::getInstance(m_eSensorDev).setEnable(fgSl2OnOff);

        if(fgSl2OnOff){
            ISP_MGR_SL2_T::getInstance(m_eSensorDev).setPure(rSl2Cfg);
            ISP_MGR_NSL2A_T::getInstance(m_eSensorDev).setPure(rSl2Cfg);
        }
    }
}
...
logGainTbl(const MUINT8* ptrTbl, MUINT32 u4Size, const char* str)
{
    ...
    {
        LSC_LOG("0x%08x    0x%08x    0x%08x    0x%08x\n",
            *(pu4Tbl+idx), *(pu4Tbl+idx+1), *(pu4Tbl+idx+2), *(pu4Tbl+idx+3));
    }

    for (MUINT32 idx = 0; idx < u4Size; idx += 16)
    {
        LSC_LOG("0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x,\n",
