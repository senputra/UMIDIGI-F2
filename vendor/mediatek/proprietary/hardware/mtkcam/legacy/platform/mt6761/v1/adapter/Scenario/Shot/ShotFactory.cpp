...
{
    //   mt6739 stereo force mode
    int32_t forceStereo = ::property_get_int32("debug.camera.force.stereo", 0);
    MBOOL const isStereoShot = forceStereo? MTRUE : ((::strcmp(pParamsMgr->getStr(MtkCameraParameters::KEY_STEREO_CAPTURE_MODE), MtkCameraParameters::ON) == 0) ? MTRUE : MFALSE);
    ...
    case eShotMode_ZsdShot:
        if ( isStereoShot )
        {
            rpShot = createInstance_StereoShot("SZSD", u4ShotMode, i4OpenId);
        }
        else
        {
