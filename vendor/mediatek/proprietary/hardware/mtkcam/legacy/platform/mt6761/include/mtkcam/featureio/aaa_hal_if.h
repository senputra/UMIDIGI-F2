...
private:
    int                 m_i4LogEn;
    List<wp<I3ACallBack>>  m_CallBacks;
    Mutex               m_Mutex;

};

class AFCallBackSet : public I3ACallBack
{
public:
    AFCallBackSet();
    virtual             ~AFCallBackSet();
    ...
    virtual MINT32      addCallback(I3ACallBack* cb);
    virtual MINT32      removeCallback(I3ACallBack* cb);
    virtual List<wp<I3ACallBack>>   getCallback();

    void                init();
    void                uninit();

    static void*        NotifyCbloop(void* arg);
    static void*        DataCbloop(void* arg);
    ...
    struct Notify_Param{
        int32_t notifymsgType, ext1, ext2, ext3;
        MINTPTR ext4;
        Notify_Param()
            : notifymsgType(0)
            , ext1(0)
            ...
            , ext4(NULL)
            {}
        Notify_Param(int32_t _notifymsgType, int32_t _ext1, int32_t _ext2, int32_t _ext3, MINTPTR _ext4)
            : notifymsgType(_notifymsgType)
            , ext1(_ext1)
            , ext2(_ext2)
            , ext3(_ext3)
            , ext4(_ext4)
    ...
    struct Data_Param{
        ...
        Data_Param()
            ...
            , datasize(0)
            {}
        Data_Param(int32_t _datamsgType, void* _datadata, uint32_t _datasize)
    ...
    std::list<Notify_Param> NotifyQ;
    std::list<Data_Param> DataQ;

    int NotifyStatus = ALIVE, DataStatus = ALIVE;
//private:
    int             m_i4LogEn;
    Mutex           m_Mutex;
    Mutex           m_MutexNotify;
    ...
    sem_t           m_NotifySem;
    sem_t           m_DataSem;
    pthread_t       threadNotifyCb;
    pthread_t       threadDataCb;
