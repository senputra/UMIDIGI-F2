...
{
    MY_LOGD("+");
    return mpDefaultCtrlNode_Main2->setMode(mModeType);
}
...
void*
CamAdapter::
doThreadRecord(void* arg)
{
    ::prctl(PR_SET_NAME,"recordThread", 0, 0, 0);
    CamAdapter* pSelf = reinterpret_cast<CamAdapter*>(arg);
    return (void*)pSelf->main2record();
}
...
onHandleStartRecording()
{
    ...
    void* threadRet = NULL;
    mModeType = DefaultCtrlNode::MODE_STEREO_VDO_RECORD;
    //
    if(mpResMgrDrv)
    {
    ...
    //
    if( pthread_create(&mThreadRecord, NULL, doThreadRecord, this) != 0 )
    {
    ...
    {
        mpDefaultCtrlNode->setMode(mModeType);
    }
    if( pthread_join(mThreadRecord, &threadRet) != 0 )
    {
...
onHandleStopRecording()
{
    ...
    status_t ret = OK;
    void* threadRet = NULL;
    mModeType = DefaultCtrlNode::MODE_STEREO_VDO_PREVIEW;
    //
