...
EPDBUF_TYPE_t PDMgr::start(MINT32 i4SensorIdx, SPDProfile_t *iPDProfile, PD_NVRAM_T *ptrInPDNvRam)
{
    ...
    if( m_pIHalSensor)
    {
        m_pIHalSensor->sendCommand( m_i4CurrSensorDev, SENSOR_CMD_GET_SENSOR_PDAF_INFO,     (MINTPTR )&(m_profile.i4SensorMode), (MINTPTR )&(m_PDBlockInfo),                  0);
        m_pIHalSensor->sendCommand( m_i4CurrSensorDev, SENSOR_CMD_GET_SENSOR_PDAF_CAPACITY, (MINTPTR )&(m_profile.i4SensorMode), (MINTPTR )&(m_profile.bSensorModeSupportPD), 0);
        m_pIHalSensor->sendCommand( m_i4CurrSensorDev, SENSOR_CMD_GET_SENSOR_VC_INFO,       (MUINTPTR)&(m_SensorVCInfo),         (MUINTPTR)&(m_profile.i4SensorMode),         0);
    }
    else
    {
    ...
    CAM_LOGD( "[%s] SensorMode(%d) SensorInfo(offset[%d,%d], pitch[%d,%d], pairNum[%d], subBlkSz[%d,%d], blockNum[%d,%d], longExpFirst[%d], mirrorFlip[%d]) VC2 Info(DataType[0x%x], ID[0x%x], SIZEH[0x%x], SIZEV[0x%x])",
              __FUNCTION__,
...
MRESULT PDMgr::SetCaliData2PDCore(PD_ALGO_TUNING_T* ptrInTuningData, PD_CALIBRATION_DATA_T* ptrInCaliData)
{
    ...
    algCfg.i4FullRawWidth = m_profile.uFullXsz;
    algCfg.i4FullRawHeight = m_profile.uFullYsz;
