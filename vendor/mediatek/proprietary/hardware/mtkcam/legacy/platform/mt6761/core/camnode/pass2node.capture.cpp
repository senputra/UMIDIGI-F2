...
moduleInit()
{
    ...
    switch( mPass2Type )
    {
        ...
        case PASS2_CAPTURE:
            ...
            break;
        case PASS2_PURERAW_CAPTURE:
            mpPostProcPipe = IFeatureStream::createInstance(getName(), EFeatureStreamTag_IPRAWTPipe_Stream, getSensorIdx());
            break;
#endif
        case PASS2_VSS:
...
#endif
        MY_LOGD("lock hardware");
        mpIspSyncCtrlHw->lockHw(IspSyncControlHw::HW_PASS1);
        MY_LOGD("unlock hardware");
        mpIspSyncCtrlHw->unlockHw(IspSyncControlHw::HW_PASS1);
        MY_LOGD("end");
...
getPass2Buffer(vector<p2data>& vP2data)
{
    ...
    // src
    {
        ...
        {
            ProcessingParams u4FourCellparams;
            u4FourCellparams.pInBuffer  =  mlPostBufData.front().buf;
            u4FourCellparams.iOpenId    = getSensorIdx();
            if(!doRemosaicRawProcess(u4FourCellparams))
                MY_LOGE("4cell capture fails");
            one.src = mlPostBufData.front();
            pSrcBuf = one.src.buf;
            mlPostBufData.pop_front();
         }
...
doRwbGpu(IImageBuffer* pBuf, IImageBuffer* dst0, IImageBuffer* dst1, MINT32 trans)
{
    ...
    if(mspRwbGpuProc != NULL)
    {
        ...
        if((muNRType==0)
            ...
            &&(dst0->getImageBufferHeap()->getHWBuffer() != NULL ) // gralloc buffer
        )
        {
...
MBOOL
CapPass2::
doRemosaicRawProcess(ProcessingParams const& param)
{
    ...
    sp<IImageBuffer> pUpkOutBuf = NULL;
    ...
    {
        pUpkOutBuf = createOutputBuffer("Fcell",eImgFmt_BAYER10_UNPAK,pInBuf->getImgSize());
    ...
    //Prepare parameters to call Fcell library
//#if !UPK_OUT
    sp<IImageBuffer> pFcellUpkOutBuf = createOutputBuffer("Fcell",eImgFmt_BAYER10_UNPAK,pInBuf->getImgSize());
    ...
    {
        //TODO:Hardcode 1024 for Samsung ALG
        IHal3A* p3AHal = IHal3A::createInstance( IHal3A::E_Camera_1, param.iOpenId,LOG_TAG);
        NS3A::CaptureParam_T Cap3AParam;
        if(p3AHal) p3AHal->getCaptureParams(Cap3AParam);
        ProcessingParam.gain_awb_r  = 1024;
        ProcessingParam.gain_awb_gr = 1024;
        ProcessingParam.gain_awb_gb = 1024;
        ProcessingParam.gain_awb_b  = 1024;
        ProcessingParam.gain_analog = Cap3AParam.u4IspGain;
        MY_LOGD("analog_gain = %d, awb_rgain = %d, awb_ggain = %d, awb_bgain = %d",ProcessingParam.gain_analog,ProcessingParam.gain_awb_r,
            ProcessingParam.gain_awb_gr,ProcessingParam.gain_awb_b);
        if(p3AHal){
            p3AHal->destroyInstance(LOG_TAG);
            p3AHal = NULL;
        }
    }
    android::sp<IPlugProcessing> pPlugProcessing = IPlugProcessing::createInstance((MUINT32)IPlugProcessing::PLUG_ID_FCELL, (NSCam::IPlugProcessing::DEV_ID)param.iOpenId);
...
//#if !UPK_OUT
    // pack algorithm
    IImageBuffer *pOutBuf = param.pInBuffer;
    ...
    MUINT32 buf_size = DNGOP_BUFFER_SIZE(pOutBuf->getBufStridesInBytes(0), nImgHeight);
    MyDngopImgInfo.Buff_size = buf_size;
...
//#endif
    if(muDumpFlag)
    {
        ...
        dumpBuffer(pOutBuf,"4cell-ResultBuf","raw");
    }
    param.pInBuffer->syncCache(eCACHECTRL_FLUSH);
