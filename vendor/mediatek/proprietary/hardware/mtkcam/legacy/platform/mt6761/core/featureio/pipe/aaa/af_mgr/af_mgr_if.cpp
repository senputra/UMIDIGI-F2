...
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).CmdTriggerAF(AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).CmdTriggerAF(AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).CmdTriggerAF(AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).CmdTriggerAF(AF_CMD_CALLER);
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).CmdSetAFMode(a_eAFMode, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).CmdSetAFMode(a_eAFMode, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).CmdSetAFMode(a_eAFMode, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).CmdSetAFMode(a_eAFMode, AF_CMD_CALLER);
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).CmdSetAFArea( a_sAFArea);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).CmdSetAFArea( a_sAFArea);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).CmdSetAFArea( a_sAFArea);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).CmdSetAFArea( a_sAFArea);
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).setZoomWinInfo(u4XOffset, u4YOffset, u4Width, u4Height, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).setZoomWinInfo(u4XOffset, u4YOffset, u4Width, u4Height, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).setZoomWinInfo(u4XOffset, u4YOffset, u4Width, u4Height, AF_CMD_CALLER);
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).getAF2AEInfo(rAFInfo);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).getAF2AEInfo(rAFInfo);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).getAF2AEInfo(rAFInfo);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).getAF2AEInfo(rAFInfo);
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).CmdSetMFPos(a_i4Pos, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).CmdSetMFPos(a_i4Pos, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).CmdSetMFPos(a_i4Pos, AF_CMD_CALLER);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).CmdSetMFPos(a_i4Pos, AF_CMD_CALLER);
...
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).CmdSetFullScanstep(a_i4Step);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).CmdSetFullScanstep(a_i4Step);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).CmdSetFullScanstep(a_i4Step);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).CmdSetFullScanstep(a_i4Step);
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MVOID IAfMgr::doPDBuffer(MINT32 i4SensorDev, MVOID * buffer, MINT32 w,MINT32 h, MINT32 stride, MUINT32 i4FrmNum, MUINT32 i4RawFmt, IImageBuffer *imgbuf)
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).doPDBuffer(buffer, w,h, stride, i4FrmNum, i4RawFmt, imgbuf);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).doPDBuffer(buffer, w,h, stride, i4FrmNum, i4RawFmt, imgbuf);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).doPDBuffer(buffer, w,h, stride, i4FrmNum, i4RawFmt, imgbuf);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).doPDBuffer(buffer, w,h, stride, i4FrmNum, i4RawFmt, imgbuf);
...
MRESULT IAfMgr::enableAF(MINT32 i4SensorDev, MINT32 a_i4En)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).enableAF(a_i4En);
    return AfMgr::getInstance(ESensorDev_Main).enableAF(a_i4En);
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAfMgr::getPdInfoForSttCtrl(MINT32 i4SensorDev, MINT32 i4SensorIdx, MINT32 i4SensorMode, FEATURE_PDAF_STATUS &PDAFStatus)
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).getPdInfoForSttCtrl(i4SensorIdx, i4SensorMode, PDAFStatus);
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).getPdInfoForSttCtrl(i4SensorIdx, i4SensorMode, PDAFStatus);
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).getPdInfoForSttCtrl(i4SensorIdx, i4SensorMode, PDAFStatus);
    if (i4SensorDev & ESensorDev_SubSecond)
        return AfMgr::getInstance(ESensorDev_SubSecond).getPdInfoForSttCtrl(i4SensorIdx, i4SensorMode, PDAFStatus);
...
{
    if (i4SensorDev & ESensorDev_Main)
        AfMgr::getInstance(ESensorDev_Main).CmdAutoFocus();
    if (i4SensorDev & ESensorDev_Sub)
        AfMgr::getInstance(ESensorDev_Sub).CmdAutoFocus();
    if (i4SensorDev & ESensorDev_MainSecond)
        AfMgr::getInstance(ESensorDev_MainSecond).CmdAutoFocus();
    if (i4SensorDev & ESensorDev_SubSecond)
        AfMgr::getInstance(ESensorDev_SubSecond).CmdAutoFocus();
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
{
    if (i4SensorDev & ESensorDev_Main)
        AfMgr::getInstance(ESensorDev_Main).CmdCancelAutoFocus();
    if (i4SensorDev & ESensorDev_Sub)
        AfMgr::getInstance(ESensorDev_Sub).CmdCancelAutoFocus();
    if (i4SensorDev & ESensorDev_MainSecond)
        AfMgr::getInstance(ESensorDev_MainSecond).CmdCancelAutoFocus();
    if (i4SensorDev & ESensorDev_SubSecond)
        AfMgr::getInstance(ESensorDev_SubSecond).CmdCancelAutoFocus();
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAfMgr::setSensorMode(MINT32 i4SensorDev, MINT32 i4NewSensorMode)
{
    MINT32 ret_main, ret_sub, ret_main2, ret_sub2;
    ret_main = ret_sub = ret_main2 = ret_sub2 = S_AE_OK;

    if (i4SensorDev & ESensorDev_Main)
        ret_main = AfMgr::getInstance(ESensorDev_Main).setSensorMode(i4NewSensorMode);
    if (i4SensorDev & ESensorDev_Sub)
        ret_sub = AfMgr::getInstance(ESensorDev_Sub).setSensorMode(i4NewSensorMode);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AfMgr::getInstance(ESensorDev_MainSecond).setSensorMode(i4NewSensorMode);
    if (i4SensorDev & ESensorDev_SubSecond)
        ret_main2 = AfMgr::getInstance(ESensorDev_SubSecond).setSensorMode(i4NewSensorMode);

    return (ret_main | ret_sub | ret_main2 | ret_sub2);
}
MVOID IAfMgr::VSyncIrq(MINT32 i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        AfMgr::getInstance(ESensorDev_Main).VsyncUpdate();
    if (i4SensorDev & ESensorDev_Sub)
        AfMgr::getInstance(ESensorDev_Sub).VsyncUpdate();
    if (i4SensorDev & ESensorDev_MainSecond)
        AfMgr::getInstance(ESensorDev_MainSecond).VsyncUpdate();
    if (i4SensorDev & ESensorDev_SubSecond)
        AfMgr::getInstance(ESensorDev_SubSecond).VsyncUpdate();
}

MBOOL IAfMgr::IsFullScanMode(MINT32 i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        return AfMgr::getInstance(ESensorDev_Main).IsFullScanMode();
    if (i4SensorDev & ESensorDev_Sub)
        return AfMgr::getInstance(ESensorDev_Sub).IsFullScanMode();
    if (i4SensorDev & ESensorDev_MainSecond)
        return AfMgr::getInstance(ESensorDev_MainSecond).IsFullScanMode();
...
MVOID IAfMgr::setCurFrmNum(MINT32 i4SensorDev, MUINT32 frm_num)
{
    ...
    if (i4SensorDev & ESensorDev_SubSecond)
        AfMgr::getInstance(ESensorDev_SubSecond).setCurFrmNum(frm_num);
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// Depth AF API
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
{
    if (i4SensorDev & ESensorDev_Main)
        AfMgr::getInstance(ESensorDev_Main).CmdSetFocusDistance(lens_focusDistance);
    if (i4SensorDev & ESensorDev_Sub)
        AfMgr::getInstance(ESensorDev_Sub).CmdSetFocusDistance(lens_focusDistance);
    if (i4SensorDev & ESensorDev_MainSecond)
        AfMgr::getInstance(ESensorDev_MainSecond).CmdSetFocusDistance(lens_focusDistance);
    if (i4SensorDev & ESensorDev_SubSecond)
        AfMgr::getInstance(ESensorDev_SubSecond).CmdSetFocusDistance(lens_focusDistance);
}
...
MVOID IAfMgr::SaveAfMgrInfo(MINT32 i4SensorDev, const char * fname)
{
    if (i4SensorDev & ESensorDev_Main)
    {
        AfMgr::getInstance(ESensorDev_Main).SaveAfMgrInfo(fname);
    }
    else if (i4SensorDev & ESensorDev_Sub)
    {
        AfMgr::getInstance(ESensorDev_Sub).SaveAfMgrInfo(fname);
    }
    else if (i4SensorDev & ESensorDev_MainSecond)
    {
        AfMgr::getInstance(ESensorDev_MainSecond).SaveAfMgrInfo(fname);
    }
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//    CCT feature
