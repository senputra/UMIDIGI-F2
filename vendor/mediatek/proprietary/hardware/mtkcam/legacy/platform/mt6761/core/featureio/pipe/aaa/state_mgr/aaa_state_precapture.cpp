...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    if ((m_pStateMgr->getPrecapState() == ePrecap_Num) && (m_pStateMgr->mAFStateCntSet.bNeedToDoPrecapAF == 0))
    {
        ...
        m_PrecapCount = 0;
        MY_LOG("Precapture End : %f ms", m_pStateMgr->m_pTimer->End());
    }
    return  err;
}
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    ...
    double dPrecapTime = m_pStateMgr->m_pTimer->printTime();
if (ENABLE_AWB & m_pHal3A->m_3ACtrlEnable) {
    if(bAEStable || dPrecapTime > PRECAP_TIMEOUT) // AE is stable
    {
        ...
        MY_LOG("Precapture AE : %f ms", dPrecapTime);
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    ...
    double dPrecapTime = m_pStateMgr->m_pTimer->printTime();
    if(rep.isEnd==1 || (dPrecapTime > PRECAP_TIMEOUT && !(rep.isFlashCalibrationMode) ) )
    {
        ...
        MY_LOG("Precapture Flash : %f ms", dPrecapTime);
        m_pStateMgr->proceedPrecapState();
    }
...
sendPrecapIntent(intent2type<eIntent_VsyncUpdate>, state2type<ePrecap_AF_BeforeFlash>)
{
    ...
    MRESULT err = S_3A_OK;
    MBOOL isAEAFLock = MFALSE;
    Param_T rParam;
    m_pHal3A->getParams(rParam);
    double dPrecapTime = m_pStateMgr->m_pTimer->printTime();

    isAEAFLock = rParam.bIsAELock && (rParam.u4AfMode == AF_MODE_AFS);
    MY_LOG("[sendPrecapIntent] CUST_ENABLE_PRECAPTURE_AF(%d), CUST_PRECAPTURE_AF_BEFORE_PREFLASH(%d), isAEAFLock(%d)\n"
        , CUST_ENABLE_PRECAPTURE_AF()
        , CUST_PRECAPTURE_AF_AFTER_PREFLASH()
        , isAEAFLock);
    if(CUST_ENABLE_PRECAPTURE_AF() && !CUST_PRECAPTURE_AF_AFTER_PREFLASH() && !isAEAFLock)
    {
        MY_LOG("[sendPrecapIntent] To execute Precapture AF before Flash\n");
        int isDoAF;
        int hasHw;
        MBOOL isSupportAF;
        MBOOL isTimeout = (dPrecapTime > PRECAP_TIMEOUT)? MTRUE : MFALSE;
        ...
        {
            MY_LOG("no flash hw || isSupportAF(%d) || Precapture AF(%f ms)", isSupportAF, dPrecapTime);
            if(isTimeout)
...
sendPrecapIntent(intent2type<eIntent_VsyncUpdate>, state2type<ePrecap_AF_AfterFlash>)
{
    ...
    double dPrecapTime = m_pStateMgr->m_pTimer->printTime();
    isAEAFLock = rParam.bIsAELock && (rParam.u4AfMode == AF_MODE_AFS);
    MY_LOG("[sendPrecapIntent] CUST_ENABLE_PRECAPTURE_AF(%d), CUST_PRECAPTURE_AF_AFTER_PREFLASH(%d), isAEAFLock(%d)\n"
        , CUST_ENABLE_PRECAPTURE_AF()
        , CUST_PRECAPTURE_AF_AFTER_PREFLASH()
        , isAEAFLock);
    if(CUST_ENABLE_PRECAPTURE_AF() && !CUST_PRECAPTURE_AF_AFTER_PREFLASH() && !isAEAFLock)
