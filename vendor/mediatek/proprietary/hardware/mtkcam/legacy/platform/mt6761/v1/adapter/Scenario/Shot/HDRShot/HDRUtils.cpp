...
#define HDR_DEBUG_OUTPUT_FOLDER       "/storage/sdcard0/hdr_output/"
...
HDRExpSettingOutputParam_T HdrShot::mHDROutputSetting;
...
HdrShot::createSourceAndSmallImg(void)
{
    ...
    if(mHDRShotMode == eShotMode_ZsdHdrShot) {
        ...
        if(capBufLen != mu4OutputFrameNum) {
            CAM_LOGD("Capture frame number is not sync, we sync with CapBufMgr");
...
HdrShot::encodeRawToYuv(IImageBuffer *rawBuffer, IImageBuffer *yuvBuffer, IImageBuffer *smallyuvBuffer, EIspProfile_T profile, void* /*privateData*/)
{
    ...
    if (!ret) {
        CAM_LOGD("mpINormalStream->init() error!!");
        return ret;
    }
...
HdrShot::init_decideCaptureMode()
{
    ...
    HDRExpSettingOutputParam_T &strHDROutputSetting(mHDROutputSetting);
    ...
    if(strHDROutputSetting.u4OutputFrameNum == 1) {
        CAM_LOGW("Warning!!!We should be singleShot in previous stage!!!");
        errorHandling();
        ret = MFALSE;
    }
...
HdrShot::getCaptureInfo(MUINT32 sensorId, std::vector<NS3A::CaptureParam_T> & vCap3AParam, MUINT32 &hdrFrameNum)
{
    ...
    std::unique_ptr <
                    ...
                    > hal3a
            (
                IHal3A::createInstance(IHal3A::E_Camera_1, sensorId, LOG_TAG),
                [](IHal3A* p){ if (p) p->destroyInstance(LOG_TAG); }
            );
    CHECK_OBJECT(hal3a.get());
    ...
    #if !HDR_DEBUG_SKIP_3A
        ret = ret && !hal3a->getExposureInfo(strExposureInfo);
    #endif

        copyExpSettingParam(strExposureInfo, strHDRInputSetting);

    #if !HDR_DEBUG_SKIP_3A
        ret = ret && !hal3a->send3ACtrl(E3ACtrl_GetCapPLineTable , (MINTPTR)& strHDRInputSetting.i4aeTableCurrentIndex,(MINTPTR)&strHDRInputSetting.PLineAETable);
    ...
    for(MUINT32 i=0; i<strHDROutputSetting.u4OutputFrameNum; i++) {
        hal3a->getCaptureParams(tmpCap3AParam);
    ...
    //Need delay frame for zsd HDR case
    //This static method is called in zsd HDR
    MUINT32 delayFrameCount = 0;
    delayFrameCount = hal3a->send3ACtrl(E3ACtrl_GetSensorDelayFrame, 0, 0);

    NS3A::CaptureParam_T cap3AParam_bk;
    hal3a->getCaptureParams(cap3AParam_bk);//backup current 3A param

    CAM_LOGD("Current Exposuretime: %d AfeGain: %d IspGain: %d ISO: %d\n"
                , cap3AParam_bk.u4Eposuretime
                , cap3AParam_bk.u4AfeGain
                , cap3AParam_bk.u4IspGain
                , cap3AParam_bk.u4FlareOffset
                );

    for(int i = 0; i < delayFrameCount; i++)
        vCap3AParam.push_back(cap3AParam_bk);
...
MBOOL
HdrShot::isSingleCaptureHDR(MUINT32 sensorId)
{
    MBOOL bIsSingleCap = MFALSE;
    std::vector<NS3A::CaptureParam_T> vCap3AParam;
    MUINT32 hdrFrameNum;

    getCaptureInfo(sensorId, vCap3AParam, hdrFrameNum);
...
HdrShot::getCaptureParams(NS3A::CaptureParam_T *pCap3AParam)
{
    ...
    {
        // for yuv sensor - take 2 pictures, -1.0 & 1.5 ev
        pCap3AParam[0].i4YuvEvIdx = NSFeature::AE_EV_COMP_n10;
        pCap3AParam[1].i4YuvEvIdx = NSFeature::AE_EV_COMP_15;
