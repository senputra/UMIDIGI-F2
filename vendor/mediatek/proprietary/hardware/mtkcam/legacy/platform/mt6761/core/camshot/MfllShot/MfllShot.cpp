...
static const bool g_enableLog = ::property_get_int32("mediatek.mfll.db_level", 2);
...
#endif
    if (mPerfService) {
        // boost CPU for 2 seconds
        if (mCpuCtrl.get()) {
            MY_LOGD("cpuPerformanceMode CPU for 2 seconds");
            mCpuCtrl->cpuPerformanceMode(2);
        }
    }
