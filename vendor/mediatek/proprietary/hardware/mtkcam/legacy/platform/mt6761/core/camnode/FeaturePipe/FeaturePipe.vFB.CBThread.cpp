...
#define LOG_TAG "MtkCam/FeaturePipeVFBCB"
//
#include "FeaturePipe.vFB.CBThread.h"
...
{
    MY_LOGD("+");
    if(!exitPending() && mi4ThreadId != 0)
    {
        MY_LOGD("Do FB Callback thread requestExit");
        Thread::requestExit();
        //
        CMD_Q_T cmd;
        cmd.isStopCmd = true;
    ...
    {
        MY_LOGD("No need to requestExit, because FB CallBack Thread is not running(already Exit or didn't begin to run)");
    }
    MY_LOGD("-");
}
...
readyToRun()
{
    ::prctl(PR_SET_NAME,"VFBCBThread@Preview", 0, 0, 0);
    //
    mi4ThreadId = ::gettid();
...
postCommand(CMD_Q_T &cmd)
{
    ...
    if  ( ! mCmdQue.empty() )
    {
        ...
        MY_LOGD("que size:%zu > 0 ", mCmdQue.size());
        if(!(cmd.isStopCmd)) {
            MY_LOGD("drop cb command since command queue is not empty, isFD(%d), isSD(%d), isGS(%d)",
                                                                      cmd.isFDCB, cmd.isSDCB, cmd.isGSCB);
            return false;
        }
    }
    //
...
performCallback(CMD_Q_T &cmd)
{
    ...
    if(cmd.isStopCmd)
            return ret;
    //MY_LOGD("performCB, isFD(%d), isSD(%d), isGS(%d)", cmd.isFDCB, cmd.isSDCB, cmd.isGSCB);

    if (!exitPending() && cmd.isFDCB && (mpCamMsgCbInfo != NULL))
    {
    ...
    if(!exitPending() && cmd.isSDCB && (mpCamMsgCbInfo != NULL)) {
    ...
    if(!exitPending() && cmd.isGSCB && (mpCamMsgCbInfo != NULL)) {
        mpCamMsgCbInfo->mNotifyCb(
            MTK_CAMERA_MSG_EXT_NOTIFY,
