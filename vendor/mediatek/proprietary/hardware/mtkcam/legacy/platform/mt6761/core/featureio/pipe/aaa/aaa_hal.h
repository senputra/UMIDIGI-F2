...
private:
    ...
    */
    ...
    /**
	* @brief PD control flow thread
    */
    MVOID           changePDCtrlThreadSetting();
	/**
	...
	*/
    static MVOID* PDCtrlThreadLoop(MVOID*);

    /*
        Task-based thread for supervising AFCbSet creating & destroying
       */
    static MVOID  AFCbThread(MVOID*);
    ...
    */
    MVOID           waitVSirq();
    /**
    * @brief non-busy wait of CamSV2 SOF signal
    */
    MVOID           waitSOFirq();
    /**
...
public:
    CallBackSet*    mpCbSet;
    AFCallBackSet*  mpAfCbSet;
    std::mutex      AFCbCreatemutex;
    std::condition_variable AFCbCreatecv;
    std::mutex      AFCbDestroymutex;
    std::condition_variable AFCbDestroycv;
    MINT32          bIsAFCbCreate;  // Considere to upgrade it to std::atomic<MINT32> if race condition takes place
    MINT32          bIsAFCbDestroy;
...
public:
    ...
    double printTime()
    {
        ...
        double ts_elapsed_tv_msec = ((double)ts_elapsed.tv_nsec)/1000000;
        ts_elapsed_tv_msec += ts_elapsed.tv_sec*1000;
        if (mLogEn) ALOGD("[Timer:%s: %s, SensorDevId: %d] =====> time(ms): sum=%f\n", __FUNCTION__, mInfo, mIdx, ts_elapsed_tv_msec);
        return ts_elapsed_tv_msec;
    }
