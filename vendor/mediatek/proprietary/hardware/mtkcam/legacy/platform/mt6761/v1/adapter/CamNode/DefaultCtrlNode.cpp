...
#define ISO_THRESHOLD_4CellSENSOR           (800)  //ISO threshold for single 4cell shot trigger
...
DefaultCtrlNodeImpl(const char* userName)
    ...
    , mThreadHandleEis(NULL)
    , mEisThreadState(0)
    , mEisLosslessSupport(EIS_LOSSLESS_SUPPORT)
    , mbUpdateEis(MFALSE)
    , mbUpdateVHdr(MFALSE)
    , mSensorType(0)
    , mSensorScenario(0)
    ...
    , mbIsPureRawPDAFOn(MFALSE)
{
    ...
    //EIS
    mEisLosslessSupport = property_get_int32("debug.eis.lossless.support",(mEisLosslessSupport?1:0))==1;
}
...
onInit()
{
    ...
    if(mpIspSyncCtrl)
    {
            ...
            mMode == MODE_VIDEO_PREVIEW)
        {
            ...
            MY_LOGD("EisLosslessSupport (%d)",mEisLosslessSupport);
            if(mspParamsMgr->getVideoStabilization() && mEisLosslessSupport)
            {
        ...
        if( pHalSensorList != NULL )
        {
            ...
            if ( pSensorStaticInfo->PDAF_Support == PDAF_SUPPORT_RAW_LEGACY) //PDAF Raw ( pure raw data mode)
            {
    ...
    MBOOL supportAdvEis = isAdvEisSupport_Hal1(); // from camera_custom_eis
    if ( (mVideoSize.w*mVideoSize.h > IMG_1080P_SIZE) || isVFB || isvHDREnabled || !supportAdvEis)
    {
        mEisScenario = EIS_SCE_EIS;
    }
    else
            ...
            if(mpHal3a)
            {
                    ...
                    mMode == MODE_STEREO_ZSD_PREVIEW )
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Stereo TO-DO : need to update to preview profile for mono sensor
                            mIspProfile = EIspProfile_Preview;
                        }
                    }
                }
        ...
        {
            {
                ...
                if(mpHal3a)
                    mpHal3a->notifyP1Done();//set pass 1 done info to 3A;
                else
                    MY_LOGW("mpHal3a is NULL");
            ...
            if(mZsdExtConfiguredFrameCount <  mZsdExtFrameCount)
            {
                ...
                else if (mb3AConfigStep == ZSDEXT_DELAY_COUNT)
                {
                    ...
                    if (mZsdExtFramePassCounter >= mZsdExtDelayFrameCount && mDequeueMagicNum >= mZsdExtDelayBaseMagicNum)
                    {
                        ...
                        // We need to add dummy frame (0 ev) in
                        // order to make AE stable before preview start
                        if(mZsdExtConfiguredFrameCount == (mZsdExtFrameCount-1)) {
                            NS3A::CaptureParam_T Cap3AParam;

                            Cap3AParam = pCap3AParam[mZsdExtFrameCount];
                            mpHal3a->send3ACtrl(E3ACtrl_SetSensorDirectly, (MINTPTR) &Cap3AParam, MTRUE);
                        }
                    }
    ...
    if(mbUpdateEis)
    {
            ...
            mpEisHal)
        {
            ...
            if( mMode == MODE_VIDEO_PREVIEW ||
                ...
                mMode == MODE_STEREO_VDO_RECORD )
            {
                if( mspParamsMgr->getVideoStabilization() && mEisScenario == EIS_SCE_EIS) //EIS 1.2
                {
...
updateShotMode()
{
    ...
    String8 s8ShotMode = mspParamsMgr->getShotModeStr();
    MY_LOGD("enter: <shot mode> %#x(%s)", shotMode, s8ShotMode.string());
    ...
    if(mShotMode == eShotMode_HdrShot) {
        if(android::NSShot::isSingleShotHDR(getSensorIdx())) {
            MY_LOGD("Too dark. We use singleshot");
            mShotMode = eShotMode_NormalShot;
        } else {
            MY_LOGD("HDR capture");
        }
    }
    ...
    switch(mMode)
    {
        ...
        case MODE_STEREO_ZSD_PREVIEW:
            if( !isFlashOn )
            {
                if(mMode == MODE_STEREO_ZSD_PREVIEW)
                    ...
                    case eShotMode_NormalShot:
                        {
                            ...
                            #else
                                MY_LOGI("force disable MFNR since MTK_CAM_ZSDMFB_SUPPORT is no, set shotmode to ZsdShot");
                                mShotMode = eShotMode_ZsdShot;
                            #endif
                            }

                            static const int __force_enable_mfll = ::property_get_int32("debug.mtk_cam_zsdmfb_support", 0);
                            if (__builtin_expect( runMfb && __force_enable_mfll, false ))
                            {
                                mShotMode = eShotMode_ZsdMfllShot;
                                MY_LOGD("set mShotMode = eShotMode_ZsdMfllShot by setprop debug.mtk_cam_zsdmfb_support 1");
                            }
                            //Check if 4CellRemosaic Capture Flow
                            is4CellShot(s8ShotMode,mShotMode);
                            break;
                        }
    ...
    MY_LOGD("exit: <shot mode> %#x", mShotMode);
    //
    return MTRUE;
}
...
is4CellShot(String8& rs8ShotMode, uint32_t& ru4ShotMode) const
{
    ...
    if(CC_UNLIKELY( mspParamsMgr.get() == nullptr )) {
        MY_LOGE("mspParamsMgr is NULL, assume ISO speed to AUTO(0)");
    }
    else {
        appISOSpeed = mspParamsMgr->getInt(MtkCameraParameters::KEY_ISO_SPEED);
    }
    //
    ...
    uint32_t currentIso = 0;
    if(mpHal3a){
        FrameOutputParam_T rPrv3AParam;
        mpHal3a->getRTParams(rPrv3AParam);
        currentIso = rPrv3AParam.u4RealISOValue;
    }else{
        MY_LOGE("IHal3A is NULL");
        return MFALSE;
    }

    MY_LOGD("4cell flow condition: debug:threshold_4Cell(%d), apply:iso_threshold(%d), def:ISO_THRESHOLD_4CellSENSOR(%d), currentIso(%d), appISOSpeed(%d)",
            threshold_4Cell, iso_threshold, ISO_THRESHOLD_4CellSENSOR, currentIso, appISOSpeed);
    //
    // check these conditions:
    //  1. Is supported 4-cell sensor.
    //  2. Flash is disabled.
    //  3. The current ISO < threshold(ISO_THRESHOLD_4CellSENSOR)
    if( get4CellSensorSupported() && currentIso < iso_threshold
            && appISOSpeed < iso_threshold && !IsForceDisableRemosaic)
    {
...
get4CellSensorSupported() const
{
    ...
    //
    MINT32 SensorDev = (MINT32)pHalSensorList->querySensorDevIdx(getSensorIdx());
...
{
    MBOOL ret;
#define START_PREVIEW_TIMEOUT (3000000000LL) //ns
    Mutex::Autolock _l(mLock);
    ...
    if( !ret )
    {
        ...
        MY_LOGI("wait for start preview done +");
        mCond3A.waitRelative(mLock, START_PREVIEW_TIMEOUT);
        MY_LOGI("start preview done -");
    }
...
{
    {
        ...
        // Reset EIS 3.0
        mspParamsMgr->set(MtkCameraParameters::KEY_EIS25_MODE,0);
        mspParamsMgr->set(MtkCameraParameters::KEY_EIS_SUPPORTED_FRAMES, 0);
    }
    {
        Mutex::Autolock _l(mLockStop);
...
updateReadyMagic(MUINT32 const magic)
{
    ...
    if( m3aReadyMagic == MAGIC_NUM_INVALID )
    {
        ...
        m3aReadyMagic = magic;
        MY_LOGI("3A start preview done # 0x%X", m3aReadyMagic);
...
void
DefaultCtrlNodeImpl::
startZsdExtShot(ZSD_EXT_MODE extMode, MINT32 framenum)
{
    FUNC_START;
