...
Pass2NodeImpl(Pass2NodeType const type)
    ...
    , muDeqFrameCnt(0)
    , mCropRatio(1.0f)
    , mbUseSelfDefCrop(MFALSE)
    , mSelfDefCrop(0,0)
    , mIs4CellRemoFlow(false)
{
...
void
Pass2NodeImpl::
setCropSize(MBOOL bUseCrop,MRect crop_region)
{
    Mutex::Autolock lock(mRecordingLock);
    MY_LOGD("bUseCrop(%d), crop_region(%d,%d)(%dx%d)",
        bUseCrop,
        crop_region.p.x, crop_region.p.y,
        crop_region.s.w, crop_region.s.h);
    mbUseSelfDefCrop = bUseCrop;
    mSelfDefCrop = crop_region;
}
