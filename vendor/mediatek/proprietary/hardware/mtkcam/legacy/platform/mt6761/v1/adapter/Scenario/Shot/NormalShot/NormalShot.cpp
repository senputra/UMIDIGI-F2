...
onCmd_capture()
{
    ...
    /* Dynamically create MfllShot or SingleShot base on mu4ShotMode and the current ISO/Exposure */
    ...
    MY_LOGD("%s :SmartShot creates %s", __FUNCTION__, (isMfbShot ? "MfllShot" : "SingleShot"));
    //
    ...
    //4-cell Remosaic Capture
    if(mu4ShotMode == eShotMode_4CellRemosaicShot) scenario = SENSOR_SCENARIO_ID_NORMAL_CAPTURE;
    ...
    {
        if( mpCapBufMgr != NULL ) {
            ret = pSingleShot->setCapBufMgr((MVOID*)mpCapBufMgr);
        }
    }
