...
init(MINT32 i4SensorDevId, MINT32 i4SensorOpenIndex)
{
    ...
    if (m_Users > 0)
    {
        ...
        MINT32 coverity = android_atomic_inc(&m_Users);
        return S_3A_OK;
    }
    ...
    mAFCbThread = thread(AFCbThread, this);
    mAFCbThread.detach();
    //mpAfCbSet = new AFCallBackSet;  // Do not declare AFCBSet arbitrarily
    ...
    MY_LOG("[%s] SensorDev(%d), PDAF_Support(%d), u4RawFmtType(%d)\n", __FUNCTION__, m_i4SensorDev, sensorStaticInfo.PDAF_Support, u4RawFmtType);

    if( /* support PDAF type only */
        sensorStaticInfo.PDAF_Support==PDAF_SUPPORT_CAMSV ||
        sensorStaticInfo.PDAF_Support==PDAF_SUPPORT_CAMSV_LEGACY ||
        sensorStaticInfo.PDAF_Support==PDAF_SUPPORT_RAW_LEGACY)
    {
        mbCreatePDCtrlThd = MTRUE;
    }
    else
    {
        mbCreatePDCtrlThd = MFALSE;
    }
    createThread();
...
if (ENABLE_FLASH & m_3ACtrlEnable) {
    ...
    std::unique_lock<std::mutex> _lock(AFCbCreatemutex);
    while(bIsAFCbCreate != 1)   AFCbCreatecv.wait(_lock);
    ...
    if (ENABLE_AF & m_3ACtrlEnable) {
        ...
        IAfMgr::getInstance().setCallbacks(m_i4SensorDev, mpAfCbSet);
        CAM_LOGD("[%s] AF has received CallBack address %p", __FUNCTION__, mpAfCbSet);
        //
    }
...
uninit()
{
    ...
    MINT32 coverity = android_atomic_dec(&m_Users);
    ...
    if (m_Users == 0) // There is no more User after decrease one User
    {
        ...
        //mpAfCbSet->uninit();
        {
            lock_guard<mutex> _lock(AFCbDestroymutex);
            bIsAFCbDestroy = 1;
            CAM_LOGD("[%s] AFCallBackSet Destroy Order Is Given", __FUNCTION__);
        }
        AFCbDestroycv.notify_all();
...
if (ENABLE_AF & m_3ACtrlEnable) {
    ...
    IAfMgr::getInstance().uninit(m_i4SensorDev, m_i4SensorOpenIdx);
}
if (ENABLE_FLICKER & m_3ACtrlEnable) {
...
MBOOL Hal3A::postCommand(ECmd_T const eCmd, MINTPTR const i4Arg)
{
	...
	if(!m_3ATimeOutEnable)
            ::sem_wait(&semPDCtrlThdEnd); //break wait when it become 1, i.e., AF thread finish and post
        else
        {
            MBOOL bRet = sem_wait_relativeTime(&semPDCtrlThdEnd, SEM_TIME_LIMIT_NS, "postCommand::semPDCtrlThdEnd");
            if (!bRet) AEE_ASSERT_3A_HAL("sem_wait_relativeTime(semPDCtrlThdEnd).");
        }
...
if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
    ...
    r3ADebugInfo.hdr.u4AEPlineDataOffset        =
    r3ADebugInfo.hdr.u4ShadingTableOffset       = r3ADebugInfo.hdr.u4AWBDebugDataOffset + sizeof(AWB_DEBUG_DATA_T);
...
if (ENABLE_AF & _this->m_3ACtrlEnable) {
    IAfMgr::getInstance().AFThreadStart(_this->m_i4SensorDev);
    if (!_this->mpIspDrv_forAF) {
        MY_LOG("[AFThread] m_pIspDrv null\n");
        return NULL;
    }

    if (!IAfMgr::getInstance().isAFSupport(_this->m_i4SensorDev)) {
        MY_LOG("[AFThread] AF isn't support\n");
        return NULL;
    }
    ...
    while (_this->mbAFThreadLoop) {
        ...
        if (_this->mbAFThreadLoop != 0)
        {
            ...
            if ( _this->mpIspDrv_forAF && (_this->mpIspDrv_forAF->waitIrq(&waitIrq) > 0) ) // success
            {
                ...
                MY_LOG_IF(_this->m_3ALogEnable & EN_3A_FLOW_LOG, "[Hal3A::AFThreadFunc] AF waitIrq done\n");
...
*******************************************************************************/
MVOID*
Hal3A::PDCtrlThreadLoop(MVOID *arg)
{
    MY_LOG( "[Hal3A::%s] thread start", __FUNCTION__);

    Hal3A *_this = reinterpret_cast<Hal3A*>(arg);
    // (1) change thread setting
    _this->changePDCtrlThreadSetting();

    ::sem_init( &_this->semPDCtrl, 0, 0);
    ...
    while(1)
    {
        //::sem_wait( &_this->semPDCtrl);

        _this->waitSOFirq();

        MY_LOG_IF( fgLogEn, "[Hal3A::%s] waitSOFirq done", __FUNCTION__);

        if( !_this->mbEnPDCtrlThd)
        {
            MY_LOG_IF( fgLogEn, "[Hal3A::%s] close thread", __FUNCTION__);
            break;
        }

        IAfMgr::getInstance().VSyncIrq(_this->m_i4SensorDev);
        ...
        {
		int Val;
		MY_LOG_IF( fgLogEn, "[Hal3A::PDCtrlThreadLoop] start waiting mPDMtx lock (3)\n");
		Mutex::Autolock autoLock(_this->mPDVCMtx);
		MY_LOG_IF( fgLogEn, "[Hal3A::PDCtrlThreadLoop] get mPDMtx lock (3)\n");

		::sem_getvalue(&_this->semPDCtrlThdEnd, &Val);
		MY_LOG_IF( fgLogEn, "[PDCtrlThreadLoop] semPDCtrlThdEnd before post = %d\n", Val);
		if (Val == 0) ::sem_post(&_this->semPDCtrlThdEnd); //to be 1, 1 means PD task done
        }
        #endif
    }
    MY_LOG( "[Hal3A::%s] thread end", __FUNCTION__);
    return NULL;
}

MVOID
Hal3A::
AFCbThread(MVOID* arg)
{
    ::prctl(PR_SET_NAME,"AFCbThread", 0, 0, 0);

    AFCallBackSet* TempAfCbSet = new AFCallBackSet();

    TempAfCbSet->init();
    // Force current AFCallBackSet to Hal3A member variable;
    Hal3A* _this = reinterpret_cast<Hal3A*>(arg);
    {
        lock_guard<mutex> _lock(_this->AFCbCreatemutex);
        _this->mpAfCbSet = TempAfCbSet;
        _this->bIsAFCbCreate = 1;
        CAM_LOGD("[%s] AFCallBackSet assigned complete (%p to %p)", __FUNCTION__, TempAfCbSet, _this->mpAfCbSet);
    }
    _this->AFCbCreatecv.notify_all();

    std::unique_lock<std::mutex> _lock(_this->AFCbDestroymutex);
    while(_this->bIsAFCbDestroy != 1)   _this->AFCbDestroycv.wait(_lock);
    _lock.unlock();
...
addCallbacks(I3ACallBack* cb)
{
    ...
    i4Ret = mpAfCbSet->addCallback(cb);
...
removeCallbacks(I3ACallBack* cb)
{
    ...
    i4Ret = mpAfCbSet->removeCallback(cb);
    MY_LOG("[%s] i4Ret(%d)", __FUNCTION__, i4Ret);
    return i4Ret;
}
...
Hal3A::waitSOFirq()
{
    ...
    if ((m_TgInfo != CAM_TG_1) && (m_TgInfo != CAM_TG_2))
    {
        MY_ERR("m_TgInfo = %d, Suggestion: Need to call queryTGInfoFromSensorHal() before waitSOFirq().", m_TgInfo);
        return;
    }
    ...
    waitIrq.Type = (m_TgInfo == CAM_TG_1) ?
        ISP_DRV_IRQ_TYPE_INT_P1_ST : ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
    waitIrq.Status = (m_TgInfo == CAM_TG_1) ?
        CAM_CTL_INT_P1_STATUS_SOF1_INT_ST : CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST;
    waitIrq.UserNumber=ISP_DRV_IRQ_USER_3A;
    waitIrq.UserName=const_cast<char *>("Hal3ASOF");
...
send3ACtrl(E3ACtrl_T e3ACtrl, MINTPTR iArg1, MINTPTR iArg2)
{
    ...
    case E3ACtrl_SetAFPause:
        IAfMgr::getInstance().SetPauseAF(m_i4SensorDev, (iArg1 ? MTRUE : MFALSE));
        break;
    // --------------------------------- PDAF ---------------------------------------
...
MBOOL  Hal3A::sem_wait_relativeTime(sem_t *pSem, nsecs_t reltime, const char* info, MBOOL isWaitVsync)
{
    ...
    if (s == -1)
    {
        ...
        MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s][%s]sem_timedwait() succeeded\n", __FUNCTION__, info);
...
MVOID Hal3A::notifyP1Done()
{
   MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] notifyP1Done \n", __FUNCTION__);

    if (ENABLE_3A_GENERAL & m_3ACtrlEnable) {
        MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] notifyP1Done m_3AFrameCount(%d) before\n", __FUNCTION__,m_3AFrameCount);
        m_3AFrameCount++;
        MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] notifyP1Done m_3AFrameCount(%d) after\n", __FUNCTION__,m_3AFrameCount);
    }
}
