...
MRESULT StateAF::exitPreview()
{
    ...
    // AF Stop
    IAfMgr::getInstance().Stop(m_SensorDevId);
...
{m_pStateMgr->proceedAFState(); return S_3A_OK;}
    ...
    if(i4OperMode == EOperMode_Normal &&
        m_pStateMgr->mAFStateCntSet.AFTimeOutFrmCnt >= (AF_TIME_OUT_FRAME_COUNT * m_pScheduler->getNormalizeM()))
    {
