...
#define JPEG_SHIFT             (8)      // Bokeh image
#define THUMBNAIL_SHIFT        (10)
#define DEBUGINFO_JPEG_SHIFT   (12)
#define EIS_SHIFT              (14)
#define JPEG_CLEANIMG_SHIFT    (16)     // clean image
#define DEPTHMAP_SHIFT         (18)
...
class ShotCallbackNodeImpl : public ShotCallbackNode
{
    ...
    private:
        ...
        MINT32                                 mDebugBufUsedCnt;
        MINT32                                 mThumbnailBufUsedCnt;
        //
        list<pictureCallback>                  mlPending;
        MUINT32                                mJpgCallBackCnt;
        Condition                              mCondJpgCallBack;
    ...
    , mJpgCallBackCnt(0)
{
    ...
    addDataSupport( ENDPOINT_SRC, SHOTCB_CLEANIMG_SRC );
    addDataSupport( ENDPOINT_SRC, SHOTCB_DEPTHMAP_SRC );
    //bypass src to dst
...
onInit()
{
    ...
    if( isDataConnected(SHOTCB_CLEANIMG_SRC) && isDataMsgEnabled(ENode_DATA_MSG_JPEG_CLEANIMG) )
        muInternalFlag |= USE(JPEG_CLEANIMG);
    if( isDataConnected(SHOTCB_DEPTHMAP_SRC) && isDataMsgEnabled(ENode_DATA_MSG_DEPTHMAP) )
        muInternalFlag |= USE(DEPTHMAP);
...
onNotify(MUINT32 const msg, MUINT32 const ext1, MUINT32 const ext2)
{
    ...
    switch(msg)
    {
        case CONTROL_SHUTTER:
            ...
            break;
        case CONTROL_P2DONE:
            handleNotifyCallback(ENode_NOTIFY_MSG_P2DONE, 0, 0);
            break;
        default:
            break;
    }
...
onPostBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    ...
    sprintf(szFileName, "/sdcard/mtklog/SCB_%dx%d_dstdata_%d.jpeg",pBuf->getImgSize().w, pBuf->getImgSize().h,data);
    pBuf->saveToFile(szFileName);
    #endif

    MY_LOGD("ShotCallbackNodeImpl::onPostBuffer data=%d", data);
...
keepForCallback(MUINT32 const nodedatatype) const
{
    ...
    switch(nodedatatype)
    {
        ...
        case SHOTCB_DBGINFO_SRC:
            ...
            break;
        case SHOTCB_CLEANIMG_SRC:
            ret = (muInternalFlag & USE(JPEG_CLEANIMG)) != 0;
            break;
        case SHOTCB_DEPTHMAP_SRC:
            ret = (muInternalFlag & USE(DEPTHMAP)) != 0;
            break;
        case SHOTCB_EISDATA_SRC:
...
queueData(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    FUNC_START;
    MINT64 timestamp;
    if( data != SHOTCB_DBGINFO_SRC && data != SHOTCB_EISDATA_SRC )
    {
...

    {
        ...
        switch( data )
        {
            ...
            case SHOTCB_JPEG_SRC:
                ...
                MY_LOGD("setStatus(pPicCB->muFlag, JPEG, PENDING)");
                break;
            case SHOTCB_DBGINFO_SRC:
                if( muInternalFlag & USE(DEBUGINFO_YUV) ){
                    setStatus(pPicCB->muFlag, DEBUGINFO_YUV, PENDING);
                    MY_LOGD("setStatus(pPicCB->muFlag, DEBUGINFO_YUV, PENDING)");
                }else if( muInternalFlag & USE(DEBUGINFO_JPEG) ){
                    setStatus(pPicCB->muFlag, DEBUGINFO_JPEG, PENDING);
                    MY_LOGD("setStatus(pPicCB->muFlag, DEBUGINFO_JPEG, PENDING)");
                }
                break;
            case SHOTCB_THUMBNAIL_SRC:
                setStatus(pPicCB->muFlag, THUMBNAIL, PENDING);
                MY_LOGD("setStatus(pPicCB->muFlag, THUMBNAIL, PENDING)");
                break;
            case SHOTCB_EISDATA_SRC:
                setStatus(pPicCB->muFlag, EIS, PENDING);
                break;
            case SHOTCB_CLEANIMG_SRC:
                setStatus(pPicCB->muFlag, JPEG_CLEANIMG, PENDING);
                MY_LOGD("setStatus(pPicCB->muFlag, JPEG_CLEANIMG, PENDING)");
                break;
            case SHOTCB_DEPTHMAP_SRC:
                setStatus(pPicCB->muFlag, DEPTHMAP, PENDING);
                MY_LOGD("setStatus(pPicCB->muFlag, DEPTHMAP, PENDING)");
                break;
            default:
                break;
        }
...
#define GET_DATA_WITHOUT_ERASE(NODENAME, DATA, piccb, getdata)                         \
                do{                                                              \
                    MBOOL find = MFALSE;                                         \
                    list<callbackdata>::iterator pdata = piccb->mlData.begin();  \
                    while(  pdata != piccb->mlData.end() )                       \
...
#define ERASE_IF_DONE(cblist, item) \
    ...
    if( muInternalFlag & USE(RAW) )
    {
        ...
        {
            MY_LOGD("RAW callback %lld not ready, flag 0x%x, size %d",
    ...
    if( muInternalFlag & USE(EIS) )
    {
        ...
        {
            MY_LOGD("EIS callback %lld not ready, flag 0x%x, size %d",
                    pPicCB->mTimestamp, pPicCB->muFlag, pPicCB->mlData.size() );
            goto lbExit;
        }
    }
    ...
    if( muInternalFlag & USE(YUV) )
    {
        if( (muInternalFlag & USE(DEBUGINFO_YUV) ) )
        {
                ...
                isStatus(pPicCB->muFlag, DEBUGINFO_YUV, PENDING) )
            {
                ...
                return MTRUE;
                    ...
                    isStatus(pPicCB->muFlag, DEBUGINFO_YUV, WAITBUF) )
            {
                MY_LOGD("YUV/DEBUGINFO_YUV callback %lld not ready, flag 0x%x, size %d",
                        pPicCB->mTimestamp, pPicCB->muFlag, pPicCB->mlData.size() );
                goto lbExit;
            }
        }
        else // yuv only
        {
            ...
            {
                MY_LOGD("YUV callback %lld not ready, flag 0x%x, size %d",
                        pPicCB->mTimestamp, pPicCB->muFlag, pPicCB->mlData.size() );
                goto lbExit;
            }
        }
    ...
    if( muInternalFlag & USE(POSTVIEW) )
    {
        ...
        {
            MY_LOGD("POSTVIEW callback %lld not ready, flag 0x%x, size %d",
                    pPicCB->mTimestamp, pPicCB->muFlag, pPicCB->mlData.size() );
            goto lbExit;
        }
    }

    //if depthmap cb: w/wo dbginfo, thumbnail
    if( muInternalFlag & USE(DEPTHMAP) )
    {
        if( isStatus(pPicCB->muFlag, DEPTHMAP, PENDING) )
        {
            callbackdata todo;
            //get data
            GET_DATA(DEPTHMAP, DEPTHMAP, pPicCB, todo);
            ERASE_IF_DONE(mlPending, pPicCB);

            mLock.unlock();
            MY_LOGD("do cb depthmap data: buf 0x%x", todo.buf);
            handleDataCallback(
                    ENode_DATA_MSG_DEPTHMAP,
                    0,
                    0,
                    (IImageBuffer*)(todo.buf)
                    );
        ...
        else if( isStatus(pPicCB->muFlag, DEPTHMAP, WAITBUF) )
        {
            MY_LOGD("DEPTHMAP callback %lld not ready, flag 0x%x, size %d",
                    pPicCB->mTimestamp, pPicCB->muFlag, pPicCB->mlData.size() );
            goto lbExit;
        }
    }

    //if jpeg clean image cb: w/wo dbginfo, thumbnail
    if( (muInternalFlag & USE(JPEG_CLEANIMG)) && !(isStatus(pPicCB->muFlag, JPEG_CLEANIMG, DONE)))
    {
        MBOOL ready = MTRUE;
        if( isStatus(pPicCB->muFlag, JPEG_CLEANIMG, WAITBUF) )
            ready = MFALSE;
        ...
        if( ready )
        {
            ...
            // get data
            GET_DATA(CLEANIMG, JPEG_CLEANIMG, pPicCB, jpeg);

            if( muInternalFlag & USE(THUMBNAIL) )
            {
                flag |= USE_THUMBNAIL;
                GET_DATA_WITHOUT_ERASE(THUMBNAIL, THUMBNAIL, pPicCB, thumbnail);
            }

            if( muInternalFlag & USE(DEBUGINFO_JPEG) )
            {
                flag |= USE_DBGINFO;
                GET_DATA_WITHOUT_ERASE(DBGINFO, DEBUGINFO_JPEG, pPicCB, dbginfo);
            }
            ...
            MBOOL bRtThumbBuf = MTRUE;
            MBOOL bRtDbgBuf = MTRUE;
            ...
            if( flag & USE_THUMBNAIL )
            {
                ...
                if((mThumbnailBufUsedCnt == 0)&&( muInternalFlag & USE(JPEG) )) // share buffer with jpeg
                {
                    mThumbnailBufUsedCnt++;
                    bRtThumbBuf = MFALSE;
                    MY_LOGD("Do not return thumbnail buffer, wait for bokeh jpg callback to return (cnt=%d)",
                            mThumbnailBufUsedCnt);
                }
            }
            ...
            if( flag & USE_DBGINFO )
            {
                ...
                if((mDebugBufUsedCnt == 0)&&( muInternalFlag & USE(JPEG) )) // share buffer with jpeg
                {
                    mDebugBufUsedCnt++;
                    bRtDbgBuf = MFALSE;
                    MY_LOGD("Do not return debug info buffer, wait for bokeh jpg callback to return (cnt=%d)",
                            mDebugBufUsedCnt);
                }
            }

            if(bRtDbgBuf || bRtThumbBuf)
            {
                if(flag & USE_THUMBNAIL)
                    GET_DATA(THUMBNAIL, THUMBNAIL, pPicCB, thumbnail); //clean data in pPicCB

                if(flag & USE_DBGINFO)
                    GET_DATA(DBGINFO, DEBUGINFO_JPEG, pPicCB, dbginfo); //clean data in pPicCB
            ...
            MY_LOGD("do cb clean jpeg data: buf 0x%x, thumb 0x%x, dbg 0x%x", jpegbuf, thumbbuf, pDbg);
            handleDataCallback(
                    ENode_DATA_MSG_JPEG_CLEANIMG,
                    (MUINTPTR)thumbbuf,
                    (MUINTPTR)pDbg,
                    jpegbuf
                    );
            ...
            mLock.lock();
            mJpgCallBackCnt --;
            if(mJpgCallBackCnt!=0)
            {
                MY_LOGD("cb clean jpeg wait start(mJpgCallBackCnt=%d)", mJpgCallBackCnt);
                mCondJpgCallBack.wait(mLock);
                MY_LOGD("cb clean jpeg wait done(mJpgCallBackCnt=%d)", mJpgCallBackCnt);
            }
            else
            {
                MY_LOGD("cb clean jpeg broadcast(mJpgCallBackCnt=%d)", mJpgCallBackCnt);
                mCondJpgCallBack.broadcast();
            }
            mLock.unlock();
            ...
            if( (flag & USE_THUMBNAIL) && bRtThumbBuf)
            {
                MY_LOGD("clean jpeg done, return thumbnail buffer");
                handleReturnBuffer(thumbnail.data, thumbnail.buf, thumbnail.ext);
                mThumbnailBufUsedCnt = 0;
            }
            if( (flag & USE_DBGINFO) && bRtDbgBuf )
            {
                MY_LOGD("clean jpeg done, return debug info buffer");
        ...
        {
            MY_LOGD("JPEG_CLEANIMG callback %lld not ready, flag 0x%x, size %d",
                    pPicCB->mTimestamp, pPicCB->muFlag, pPicCB->mlData.size() );
        }
    }

    //if Bokeh jpeg cb: w/wo dbginfo, thumbnail
    if(( muInternalFlag & USE(JPEG) ) && !(isStatus(pPicCB->muFlag, JPEG, DONE)))
    {
        MBOOL ready = MTRUE;
        ...
        if( ready )
        {
            ...
            {
                thumbbuf = reinterpret_cast<IImageBuffer*>(thumbnail.buf);
                if((mThumbnailBufUsedCnt == 0)&&( muInternalFlag & USE(JPEG_CLEANIMG) )) // share buffer with jpeg cleanimg
                {
                    mThumbnailBufUsedCnt++;
                    bRtThumbBuf = MFALSE;
                    MY_LOGD("Do not return thumbnail buffer, wait for clean jpg callback to return (cnt=%d)",
                            mThumbnailBufUsedCnt);
                }
            }
            ...
            {
                pDbg = reinterpret_cast<IDbgInfoContainer*>(dbginfo.buf);
                if((mDebugBufUsedCnt == 0)&&( muInternalFlag & USE(JPEG_CLEANIMG) )) // share buffer with jpeg cleanimg
                {
                    mDebugBufUsedCnt++;
                    bRtDbgBuf = MFALSE;
                    MY_LOGD("Do not return debug info buffer, wait for clean jpg callback to return (cnt=%d)",
                            mDebugBufUsedCnt);
                }
            }
            ...
            if(mJpgCallBackCnt!=0)
            {
                MY_LOGD("cb jpeg wait start(mJpgCallBackCnt=%d)", mJpgCallBackCnt);
                mCondJpgCallBack.wait(mLock);
                MY_LOGD("cb jpeg wait done(mJpgCallBackCnt=%d)", mJpgCallBackCnt);
            }
            else
            {
                MY_LOGD("cb jpeg broadcast(mJpgCallBackCnt=%d)", mJpgCallBackCnt);
                mCondJpgCallBack.broadcast();
            }
            mLock.unlock();
            ...
            if( (flag & USE_THUMBNAIL) && bRtThumbBuf )
            {
                MY_LOGD("jpeg done, return thumbnail buffer");
                handleReturnBuffer(thumbnail.data, thumbnail.buf, thumbnail.ext);
                mThumbnailBufUsedCnt = 0;
            }
            if( (flag & USE_DBGINFO) && bRtDbgBuf )
            {
                MY_LOGD("jpeg done, return debug info buffer");
                handleReturnBuffer(dbginfo.data, dbginfo.buf, dbginfo.ext);
                mDebugBufUsedCnt = 0;
            }
            return MTRUE;
        ...
        {
            MY_LOGD("JPEG callback %lld not ready, flag 0x%x, size %d",
                    pPicCB->mTimestamp, pPicCB->muFlag, pPicCB->mlData.size() );
        }
    }
