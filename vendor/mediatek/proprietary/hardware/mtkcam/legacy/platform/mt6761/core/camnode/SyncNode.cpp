...
isTimeSync(){
    ...
    MY_LOGD("FrameSync TS(%lld/%lld) diff:%d sync:%d",
...
{
    Mutex::Autolock lock(mLock);
    MY_LOGD("data(%d), buf(0x%x), size(%dx%d)", data, buf, buf->getImgSize().w, buf->getImgSize().h);
    ...
    if(syncResult == SYNC_PASS){
        ...
        if(isCapturePath){
            ...
            if( muFrameCnt % mFPSFactor == 0 ){
                ...
                // push main1 fullsize raw into mlPostBuf_2_return_queue and wait main1/main2 rrz raw buffer back from next node
                {
                    Mutex::Autolock qLock(mQueueLock);  // To avoid thread block in, need to add another autolock
                    post_cnt ++;
                    if(isZSDMode){
                        ...
                        MY_LOGD("preview : queue buffer to mlPostBuf_2_return_queue (): (%d, %d, %d), post_cnt=%d, then post buffer to next node",
                                mlPostBuf_0_return_queue.size(),
                                mlPostBuf_1_return_queue.size(),
                                mlPostBuf_2_return_queue.size(),
                                post_cnt);
