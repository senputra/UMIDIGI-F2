...
onCmd_capture()
{
    ...
    if(mpCpuCtrl)
    {
        MY_LOGD("Enable cFB CpuCtrl");
        mpCpuCtrl->init();

        CpuCtrl::CPU_CTRL_INFO cpuInfo;
        cpuInfo.bigCore = 0;
        cpuInfo.bigFreq = 0;
        cpuInfo.littleCore = 3;
    ...
    struct sched_param pthread_param = {
            .sched_priority = ANDROID_PRIORITY_FOREGROUND
    };
    ...
    property_get("vendor.camera.facebeauty.dump", EnableOption, "0");
    if (EnableOption[0] == '1')
#endif
    {
        char szFileName[100];
...
doCFBFaceAlignVFB()
{
    ...
    int apFaceCount = mBeautifiedFacePos.size();
    for(int i = 0; i < apFaceCount; i++)
    {
        FacePos[i].x = ((mBeautifiedFacePos[i].x + 1000) * (mDSWidth / 2)) / 2000;
        FacePos[i].y = ((mBeautifiedFacePos[i].y + 1000) * (mDSHeight / 2)) / 2000;
//        MY_LOGD("Ap FacePos [%d,%d] -> [%d,%d]", mBeautifiedFacePos[i].x, mBeautifiedFacePos[i].y, FacePos[i].x, FacePos[i].y);
    }

    int iHitIndex[MAX_SUPPORT_FACES];
    int hitIter = 0;
    int hitFaceCount;
    for(int i = 0; i < mvfb_FBmetadata.number_of_faces; i++)
    {
//        MY_LOGD("%d. rect(%d, %d), rect(%d, %d) rop:%d rip:%d ", i,
//          mvfb_FBmetadata.faces[i].rect[0], mvfb_FBmetadata.faces[i].rect[1],
//          mvfb_FBmetadata.faces[i].rect[2], mvfb_FBmetadata.faces[i].rect[3],
//          mvfb_FBmetadata.posInfo[i].rop_dir, mvfb_FBmetadata.posInfo[i].rip_dir);

        for(int j = 0; j < apFaceCount; j++)
        {
            if(FacePos[j].x > mvfb_FBmetadata.faces[i].rect[0] && FacePos[j].x < mvfb_FBmetadata.faces[i].rect[2] &&
               FacePos[j].y > mvfb_FBmetadata.faces[i].rect[1] && FacePos[j].y < mvfb_FBmetadata.faces[i].rect[3])
            {
                iHitIndex[hitIter] = i;
                hitIter++;
            }
    ...
    hitFaceCount = hitIter;
    if(hitFaceCount > 0)
    {
        MY_LOGD("There are %d ap face found in face detection list.", hitFaceCount);
        for(int i = 0; i < hitFaceCount; i++)
        {
            mvfb_FBmetadata.faces[i].rect[0] = mvfb_FBmetadata.faces[iHitIndex[i]].rect[0];
            mvfb_FBmetadata.faces[i].rect[1] = mvfb_FBmetadata.faces[iHitIndex[i]].rect[1];
            mvfb_FBmetadata.faces[i].rect[2] = mvfb_FBmetadata.faces[iHitIndex[i]].rect[2];
            mvfb_FBmetadata.faces[i].rect[3] = mvfb_FBmetadata.faces[iHitIndex[i]].rect[3];

            mvfb_FBmetadata.posInfo[i].rop_dir = mvfb_FBmetadata.posInfo[iHitIndex[i]].rop_dir;
            mvfb_FBmetadata.posInfo[i].rip_dir = mvfb_FBmetadata.posInfo[iHitIndex[i]].rip_dir;
        }
        mvfb_FBmetadata.number_of_faces = hitFaceCount;
        return MTRUE;
    }
    else
    {
        MY_LOGD("There are no ap face found in face detection list.");
        return MFALSE;
    }
}
    ...
    CAM_TRACE_END();
        ...
        if(mSensorType == SENSOR_TYPE_RAW && IssupportVFB == MTRUE && mSDKMode != MTRUE)
        {
            ...
            if (mu4ShotMode != NSCam::eShotMode_FaceBeautyShotCc)//When in non ZSD mode, need to do FD
            {
             ...
             // faces number exceed FB process limit
             // select face which is send by ap
             if(mvfb_FBmetadata.number_of_faces > MAX_SUPPORT_FACES)
             {
                 MY_LOGD("FBmetadata.number_of_faces > faces process limit: %d, do CFB Face Align VFB ap face.", MAX_SUPPORT_FACES);
                 if(!doCFBFaceAlignVFB())
                     MY_LOGD("Use original face detect faces.");
             }
