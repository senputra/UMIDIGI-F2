...
IMP_CCT_CTRL( ACDK_CCT_OP_QUERY_ISP_ID )
{
    ...
    *reinterpret_cast<MUINT32 *>(puParaOut) = sizeof(MUINTPTR) == 4 ? 0x67618A00 : 0x67618AFF; // 00 means 32bit; FF means 64bit
...
u4ParaOutLen
    sizeof(ISP_NVRAM_CCM_POLY22_STRUCT)
pu4RealParaOutLen
    sizeof(ISP_NVRAM_CCM_POLY22_STRUCT)
*/
IMP_CCT_CTRL( ACDK_CCT_V2_OP_GET_DYNAMIC_CCM_COEFF )
{
    ...
    MUINT32 ccm_type = *reinterpret_cast<i_type *>(puParaIn); //0: ISP (default); 1: MDP
    if (ccm_type == 0)
        ::memcpy(rRegs, &m_rBuf_ISP.ISPMulitCCM.Poly22, sizeof(o_type));
    else
        ::memcpy(rRegs, &m_rBuf_ISP.MDPMulitCCM.Poly22, sizeof(o_type));

    *pu4RealParaOutLen = sizeof(o_type);
    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_GET_DYNAMIC_CCM_COEFF ) done, type(%d)\n", ccm_type);
...
/*
puParaIn
    ACDK_CCT_SET_CCM_POLY22_STRUCT
u4ParaInLen
    sizeof(ACDK_CCT_SET_CCM_POLY22_STRUCT);
puParaOut
u4ParaOutLen
pu4RealParaOutLen
*/
IMP_CCT_CTRL( ACDK_CCT_V2_OP_SET_DYNAMIC_CCM_COEFF )
{
    ...
    typedef ACDK_CCT_SET_CCM_POLY22_STRUCT i_type;
    if  ( sizeof(i_type) != u4ParaInLen || ! puParaIn )
        return  CCTIF_BAD_PARAM;

    ISP_NVRAM_CCM_POLY22_STRUCT *rRegs = &reinterpret_cast<i_type *>(puParaIn)->ccm_poly22;
    MUINT8 ccm_type = reinterpret_cast<i_type *>(puParaIn)->u1Type; //0: ISP (default); 1: MDP
    if (ccm_type == 0)
        ::memcpy(&m_rBuf_ISP.ISPMulitCCM.Poly22, rRegs, (sizeof(i_type)-sizeof(MUINT8)));
    else
        ::memcpy(&m_rBuf_ISP.MDPMulitCCM.Poly22, rRegs, (sizeof(i_type)-sizeof(MUINT8)));

    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_SET_DYNAMIC_CCM_COEFF ) done, type(%d)\n", ccm_type);
    return  CCTIF_NO_ERROR;
...
#define MY_GET_TUNING_PARAS(_category)\
    ...
    MY_GET_TUNING_PARAS(PDC);
    //MY_GET_TUNING_PARAS(LSC);
    MY_GET_TUNING_PARAS(NSL2);
...
#define CHECK_INDEX_RANGE_BNR(_category)\
    if (IspNvramRegMgr::NUM_##_category <= u4Index)\
        return CCTIF_BAD_PARAM;\
    m_rISPRegs._category[u4Index] = ((ISP_NVRAM_BNR_##_category##_T*)(bufAccess.pBuffer))[u4Index];\
    m_rISPRegsIdx._category = static_cast<MUINT8>(u4Index);
    ...
    switch(eCategory) {
        ...
        case EIsp_Category_NR1:
            CHECK_INDEX_RANGE_BNR(NR1);
            NSIspTuning::ISP_MGR_BNR_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_BNR_NR1_T*)(bufAccess.pBuffer))[u4Index]);
        ...
        case EIsp_Category_PDC:
            CHECK_INDEX_RANGE_BNR(PDC);
            NSIspTuning::ISP_MGR_BNR_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_BNR_PDC_T*)(bufAccess.pBuffer))[u4Index]);
            break;
...
*/
        ...
        case EIsp_Category_NSL2:
            CHECK_INDEX_RANGE(NSL2);
            ISP_MGR_NSL2_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_NSL2_T*)(bufAccess.pBuffer))[u4Index]);
        ...
        case EIsp_Category_CCM:
            CHECK_INDEX_RANGE(CCM);
            ISP_MGR_CCM_T::getInstance((ESensorDev_T)m_eSensorEnum).put(((ISP_NVRAM_CCM_T*)(bufAccess.pBuffer))[u4Index]);
            break;
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_AWB_GET_CURRENT_CCM )
{
    ...
    i_type const     type = *reinterpret_cast<i_type*>(puParaIn);
    //Type; //0: ISP (default); 1: MDP
    ...
    {
        ISP_MGR_CCM_T::getInstance((ESensorDev_T)m_eSensorEnum).getMdpCCM(ccm);
    }
    ...
    *pu4RealParaOutLen = sizeof(o_type);
    ...
    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_AWB_GET_CURRENT_CCM ) done, type(%d)\n", type);
...
/*
puParaIn
    ACDK_CCT_SET_CURRENT_CCM_STRUCT
u4ParaInLen
    sizeof(ACDK_CCT_SET_CURRENT_CCM_STRUCT)
puParaOut
u4ParaOutLen
pu4RealParaOutLen
*/
...
{
    typedef ACDK_CCT_SET_CURRENT_CCM_STRUCT type;
    if  ( sizeof(type) != u4ParaInLen || ! puParaIn )
        return  CCTIF_BAD_PARAM;
...
*/
    rDst.cnv_1.bits.G2G_CNV_00 = pSrc->ccm.M11;
    rDst.cnv_1.bits.G2G_CNV_01 = pSrc->ccm.M12;
    rDst.cnv_2.bits.G2G_CNV_02 = pSrc->ccm.M13;
    rDst.cnv_3.bits.G2G_CNV_10 = pSrc->ccm.M21;
    rDst.cnv_3.bits.G2G_CNV_11 = pSrc->ccm.M22;
    rDst.cnv_4.bits.G2G_CNV_12 = pSrc->ccm.M23;
    rDst.cnv_5.bits.G2G_CNV_20 = pSrc->ccm.M31;
    rDst.cnv_5.bits.G2G_CNV_21 = pSrc->ccm.M32;
    rDst.cnv_6.bits.G2G_CNV_22 = pSrc->ccm.M33;

    if (pSrc->u1Type == 0) //u1Type; //0: ISP (default); 1: MDP
    {
        ISP_MGR_CCM_T::getInstance((ESensorDev_T)m_eSensorEnum).reset();
    ...
    {
        ISP_MGR_CCM_T::getInstance((ESensorDev_T)m_eSensorEnum).putMdpCCM(rDst);
    }
	...
	usleep(200000);
    MY_LOG("M11 0x%03X", pSrc->ccm.M11);
    MY_LOG("M12 0x%03X", pSrc->ccm.M12);
    MY_LOG("M13 0x%03X", pSrc->ccm.M13);
    MY_LOG("M21 0x%03X", pSrc->ccm.M21);
    MY_LOG("M22 0x%03X", pSrc->ccm.M22);
    MY_LOG("M23 0x%03X", pSrc->ccm.M23);
    MY_LOG("M31 0x%03X", pSrc->ccm.M31);
    MY_LOG("M32 0x%03X", pSrc->ccm.M32);
    MY_LOG("M33 0x%03X", pSrc->ccm.M33);

    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_AWB_SET_CURRENT_CCM ) done, type(%d)\n", pSrc->u1Type);
    return  CCTIF_NO_ERROR;
...
u4ParaInLen
    sizeof(ACDK_CCT_COMMON_IO_STRUCT)
puParaOut
    ACDK_CCT_CCM_STRUCT
u4ParaOutLen
    sizeof(ACDK_CCT_CCM_STRUCT)
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_AWB_GET_NVRAM_CCM )
{
    ...
    typedef ACDK_CCT_COMMON_IO_STRUCT i_type;
    if  ( sizeof(i_type) != u4ParaInLen || ! puParaIn )
        return  CCTIF_BAD_PARAM;

    i_type const inParam = *reinterpret_cast<i_type*>(puParaIn);
    //inParam.data[0]: Type: 0: ISP (default); 1: MDP
    //inParam.data[1]: CCM Index
    MUINT8 const type = inParam.data[0];
    MUINT8 const index = inParam.data[1];
    ...
    ISP_NVRAM_CCM_T& rSrc = (type == 0) ? m_rISPRegs.CCM[index] : m_rISPRegs.MDP_CCM[index];
    o_type*const     pDst = reinterpret_cast<o_type*>(puParaOut);
    ...
    *pu4RealParaOutLen = sizeof(o_type);
    ...
    MY_LOG("Current CCM Index(%d), index to get(%d), type(%d)", m_rISPRegsIdx.CCM, index, type);
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_AWB_SET_NVRAM_CCM )
{
    ...
    MUINT8 const ccm_type = rInParam.u1Type; //u1Type: 0: ISP (default); 1: MDP
    if  ( NVRAM_CCM_TBL_NUM <= index )
    {
    ...
    ISP_NVRAM_CCM_T& rDst = (ccm_type == 0) ? m_rISPRegs.CCM[index] : m_rISPRegs.MDP_CCM[index];
/*
...
*/
    ...
    MY_LOG("Current CCM Index(%d), index to set(%d), type(%d)", m_rISPRegsIdx.CCM, index, ccm_type);
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_AWB_GET_CCM_PARA )
{
    ...
    for (MUINT32 i=0; i<NVRAM_CCM_TBL_NUM; i++)
    {
        pDst->mdp_ccm[i].M11 = m_rISPRegs.MDP_CCM[i].cnv_1.bits.G2G_CNV_00;//conv0a.bits.G2G_CNV_00;
        pDst->mdp_ccm[i].M12 = m_rISPRegs.MDP_CCM[i].cnv_1.bits.G2G_CNV_01;//conv0a.bits.G2G_CNV_01;
        pDst->mdp_ccm[i].M13 = m_rISPRegs.MDP_CCM[i].cnv_2.bits.G2G_CNV_02;//conv0b.bits.G2G_CNV_02;
        pDst->mdp_ccm[i].M21 = m_rISPRegs.MDP_CCM[i].cnv_3.bits.G2G_CNV_10;//conv1a.bits.G2G_CNV_10;
        pDst->mdp_ccm[i].M22 = m_rISPRegs.MDP_CCM[i].cnv_3.bits.G2G_CNV_11;//conv1a.bits.G2G_CNV_11;
        pDst->mdp_ccm[i].M23 = m_rISPRegs.MDP_CCM[i].cnv_4.bits.G2G_CNV_12;//conv1b.bits.G2G_CNV_12;
        pDst->mdp_ccm[i].M31 = m_rISPRegs.MDP_CCM[i].cnv_5.bits.G2G_CNV_20;//conv2a.bits.G2G_CNV_20;
        pDst->mdp_ccm[i].M32 = m_rISPRegs.MDP_CCM[i].cnv_5.bits.G2G_CNV_21;//conv2a.bits.G2G_CNV_21;
        pDst->mdp_ccm[i].M33 = m_rISPRegs.MDP_CCM[i].cnv_6.bits.G2G_CNV_22;//conv2b.bits.G2G_CNV_22;
    }
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_AWB_UPDATE_CCM_PARA )
{
    ...
    for (MUINT32 i=0; i<NVRAM_CCM_TBL_NUM; i++)
    {
        m_rISPRegs.MDP_CCM[i].cnv_1.bits.G2G_CNV_00 = pSrc->mdp_ccm[i].M11;
        m_rISPRegs.MDP_CCM[i].cnv_1.bits.G2G_CNV_01 = pSrc->mdp_ccm[i].M12;
        m_rISPRegs.MDP_CCM[i].cnv_2.bits.G2G_CNV_02 = pSrc->mdp_ccm[i].M13;
        m_rISPRegs.MDP_CCM[i].cnv_3.bits.G2G_CNV_10 = pSrc->mdp_ccm[i].M21;
        m_rISPRegs.MDP_CCM[i].cnv_3.bits.G2G_CNV_11 = pSrc->mdp_ccm[i].M22;
        m_rISPRegs.MDP_CCM[i].cnv_4.bits.G2G_CNV_12 = pSrc->mdp_ccm[i].M23;
        m_rISPRegs.MDP_CCM[i].cnv_5.bits.G2G_CNV_20 = pSrc->mdp_ccm[i].M31;
        m_rISPRegs.MDP_CCM[i].cnv_5.bits.G2G_CNV_21 = pSrc->mdp_ccm[i].M32;
        m_rISPRegs.MDP_CCM[i].cnv_6.bits.G2G_CNV_22 = pSrc->mdp_ccm[i].M33;
    }
...
/*
puParaIn
    ACDK_CCT_SET_CCM_WB_STRUCT
u4ParaInLen
    sizeof(ACDK_CCT_SET_CCM_WB_STRUCT)
puParaOut
u4ParaOutLen
pu4RealParaOutLen
*/
...
{
    typedef ACDK_CCT_SET_CCM_WB_STRUCT i_type;
    if  ( sizeof(i_type) != u4ParaInLen || ! puParaIn )
        return  CCTIF_BAD_PARAM;

    i_type*const       pSrc = reinterpret_cast<i_type*>(puParaIn);

    ISP_NVRAM_CCM_AWB_GAIN_STRUCT* pAwbGain;

    if (pSrc->u1Type == 0) //ISP
        pAwbGain = &(m_rBuf_ISP.ISPMulitCCM.AWBGain = pSrc->ccm_awb_gain);
    else //MDP
        pAwbGain = &(m_rBuf_ISP.MDPMulitCCM.AWBGain = pSrc->ccm_awb_gain);

    MY_LOG("pSrc->rA.i4R = %d, type(%d), nvram buf = %d\n", pSrc->ccm_awb_gain.rA.i4R, pSrc->u1Type, pAwbGain->rA.i4R);
    MY_LOG("pSrc->rA.i4G = %d, type(%d), nvram buf = %d\n", pSrc->ccm_awb_gain.rA.i4G, pSrc->u1Type, pAwbGain->rA.i4G);
    MY_LOG("pSrc->rA.i4B = %d, type(%d), nvram buf = %d\n", pSrc->ccm_awb_gain.rA.i4B, pSrc->u1Type, pAwbGain->rA.i4B);
    MY_LOG("pSrc->rD65.i4R = %d, type(%d), nvram buf = %d\n", pSrc->ccm_awb_gain.rD65.i4R, pSrc->u1Type, pAwbGain->rD65.i4R);
    MY_LOG("pSrc->rD65.i4G = %d, type(%d), nvram buf = %d\n", pSrc->ccm_awb_gain.rD65.i4G, pSrc->u1Type, pAwbGain->rD65.i4G);
    MY_LOG("pSrc->rD65.i4B = %d, type(%d), nvram buf = %d\n", pSrc->ccm_awb_gain.rD65.i4B, pSrc->u1Type, pAwbGain->rD65.i4B);
...
IMP_CCT_CTRL( ACDK_CCT_OP_GET_CCM_WB )
{
    ...
    *pDst = (type == 0) ? m_rBuf_ISP.ISPMulitCCM.AWBGain : m_rBuf_ISP.MDPMulitCCM.AWBGain;
    //Type; //0: ISP (default); 1: MDP
    *pu4RealParaOutLen = sizeof(o_type);

    ISP_NVRAM_CCM_AWB_GAIN_STRUCT* pAwbGain = (type == 0) ? &m_rBuf_ISP.ISPMulitCCM.AWBGain : &m_rBuf_ISP.MDPMulitCCM.AWBGain;

    MY_LOG("pDst->rA.i4R = %d, type(%d), nvram buf = %d\n", pDst->rA.i4R, type, pAwbGain->rA.i4R);
    MY_LOG("pDst->rA.i4G = %d, type(%d), nvram buf = %d\n", pDst->rA.i4G, type, pAwbGain->rA.i4G);
    MY_LOG("pDst->rA.i4B = %d, type(%d), nvram buf = %d\n", pDst->rA.i4B, type, pAwbGain->rA.i4B);
    MY_LOG("pDst->rD65.i4R = %d, type(%d), nvram buf = %d\n", pDst->rD65.i4R, type, pAwbGain->rD65.i4R);
    MY_LOG("pDst->rD65.i4G = %d, type(%d), nvram buf = %d\n", pDst->rD65.i4G, type, pAwbGain->rD65.i4G);
    MY_LOG("pDst->rD65.i4B = %d, type(%d), nvram buf = %d\n", pDst->rD65.i4B, type, pAwbGain->rD65.i4B);
...
MVOID
CctCtrl::
setIspOnOff_NSL2(MBOOL const fgOn)
{
    ISP_MGR_NSL2_T::getInstance((ESensorDev_T)m_eSensorEnum).setEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    //ISP_MGR_SL2_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
    //ISP_MGR_SL2_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(NSIspTuning::EIspProfile_NormalCapture);
    MY_LOG("[%s] fgOn = %d", __FUNCTION__, fgOn);
...
{
    //ISP_MGR_PDC_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTEnable(fgOn);
    //NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    //MY_LOG("[%s] fgOn = %d", __FUNCTION__, fgOn);
}
...
{
    //0811 No CCTBOK enable  //ISP_MGR_NBC2_T::getInstance((ESensorDev_T)m_eSensorEnum).setCCTBOKEnable(fgOn);
    NSIspTuning::IspTuningMgr::getInstance().forceValidate((MINT32)m_eSensorEnum);
    //ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(EIspProfile_Preview);
    //ISP_MGR_NBC_T::getInstance((ESensorDev_T)m_eSensorEnum).apply(NSIspTuning::EIspProfile_NormalCapture);
    MY_LOG("[%s] fgOn = %d", __FUNCTION__, fgOn);
...
{
    //return ISP_MGR_PDC_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTEnable();
    return MFALSE;
}

MBOOL
CctCtrl::
getIspOnOff_NSL2() const
{
    return  ISP_MGR_NSL2::getInstance((ESensorDev_T)m_eSensorEnum).isEnable();
}
...
{
    //0811  noCCTBOK enable //return (ISP_MGR_NBC2_T::getInstance((ESensorDev_T)m_eSensorEnum).isCCTANR1Enable());
...
pu4RealParaOutLen
*/
IMP_CCT_CTRL( ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO )
{
    typedef float o_type;
    if  ( sizeof(o_type) != u4ParaOutLen || ! pu4RealParaOutLen || ! puParaOut )
        return  CCTIF_BAD_PARAM;

    *reinterpret_cast<o_type*>(puParaOut) = m_rBuf_ISP.isp_ccm_ratio;
    *pu4RealParaOutLen = sizeof(o_type);

    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO ) done, %f\n", m_rBuf_ISP.isp_ccm_ratio);
    return  CCTIF_NO_ERROR;
}
...
pu4RealParaOutLen
*/
IMP_CCT_CTRL( ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO )
{
    typedef float i_type;
    if  ( sizeof(i_type) != u4ParaInLen || ! puParaIn )
        return  CCTIF_BAD_PARAM;

    m_rBuf_ISP.isp_ccm_ratio = *reinterpret_cast<i_type*>(puParaIn);

    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO ) done, %f\n", m_rBuf_ISP.isp_ccm_ratio);
    return  CCTIF_NO_ERROR;
...
IMP_CCT_CTRL( ACDK_CCT_V2_OP_QUERY_ISP_INDEX_SET )
{
    ...
    *pu4RealParaOutLen = sizeof(o_type);
    ...
    MY_LOG("IMP_CCT_CTRL( ACDK_CCT_V2_OP_QUERY_ISP_INDEX_SET ) done, DBS(%d) OBC(%d) BPC(%d) NR1(%d) PDC(%d) UDM(%d) GGM(%d) ANR(%d) ANR2(%d) CCR(%d) BOK(%d) EE(%d) NR3D(%d) MFB(%d) LCE(%d)\n",
    pCustNvramRegIdx->DBS,pCustNvramRegIdx->OBC, pCustNvramRegIdx->BPC,  pCustNvramRegIdx->NR1, pCustNvramRegIdx->PDC, pCustNvramRegIdx->UDM, pCustNvramRegIdx->GGM, pCustNvramRegIdx->ANR, pCustNvramRegIdx->ANR2,
    pCustNvramRegIdx->CCR,pCustNvramRegIdx->BOK, pCustNvramRegIdx->EE, pCustNvramRegIdx->NR3D, pCustNvramRegIdx->MFB, pCustNvramRegIdx->LCE);
