...
bool FT_ACDK_CCT_OP_PREVIEW_LCD_START(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
...
bool FT_ACDK_CCT_OP_PREVIEW_LCD_STOP(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
       if (!g_bAcdkOpend )
       {
           return FALSE;
  ...
  int i,j;
    ...
    if (ctgy == EIsp_Category_DBS)
      ...
      for (i = 0; i < NVRAM_OBC_TBL_NUM; i++)
      {
        ...
        for (i = 0; i < NVRAM_BNR_NR1_TBL_NUM; i++)
        {
            ACDK_LOGD("+ BPC[%u]", i);
    ...
    else if (ctgy == EIsp_Category_PDC)
      for (i = 0; i < NVRAM_BNR_PDC_TBL_NUM; i++)
        {
            ACDK_LOGD("+ PDC[%u]", i);
        ...
        for (i = 0; i < NVRAM_CCR_TBL_NUM; i++)
        {
          ...
          for (i = 0; i < NVRAM_BOK_TBL_NUM; i++)
          {
              ...
              {
                  ACDK_LOGD("  - set[%u]=%u", j, ((ISP_NVRAM_BOK_T*)g_set_isp_para_buffer)[i].set[j]);
              }
          }
...
static bool GetSersonInfo(ACDK_CCT_SENSOR_STATIC_INFO* pData)
{
  ...
  ACDK_CCT_SENSOR_STATIC_INFO ACDK_Sensor;
  ...
  if (false == bSendDataToCCT(ACDK_CCT_OP_QUERY_SENSOR,
                                                        ...
                                                        0,
                                                        (UINT8 *)&ACDK_Sensor,
                                                        sizeof(ACDK_CCT_SENSOR_STATIC_INFO),
                                                        &ACDKCCTParaLen))
  {
  ...
  memcpy(pData,&ACDK_Sensor,sizeof(ACDK_CCT_SENSOR_STATIC_INFO));
  return true;
}
...
bool FT_ACDK_CCT_OP_QUERY_SENSOR        (const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
  ...
  for(unsigned int i = 0; i < g_FT_CCT_StateMachine.sensor_onboard_sensors.sensor_count; i++)
  {
    p_on_board_sensors->sensor[i].type    = g_FT_CCT_StateMachine.sensor_onboard_sensors.sensor[i].type;
        ...
        p_on_board_sensors->sensor[i].rawFmtType= g_FT_CCT_StateMachine.sensor_onboard_sensors.sensor[i].rawFmtType;
        ACDK_LOGD("[CCAP]:  Sensor type(%d), id(%d), rawFmtType(%d)"
            , p_on_board_sensors->sensor[i].type
            , p_on_board_sensors->sensor[i].device_id
            , p_on_board_sensors->sensor[i].rawFmtType);
...
{
  ACDK_CCT_SET_CURRENT_CCM_STRUCT ccm;
  ...
  ACDK_LOGD("[CCAP]:FT_ACDK_CCT_V2_OP_AWB_SET_CURRENT_CCM pass! type(%d)", pREQ->cmd.dev_6238_awb_current_ccm.u1Type);
  pCNF->status = FT_CNF_OK;
  return true;
}
...
bool FT_ACDK_CCT_V2_OP_AWB_GET_CURRENT_CCM    (const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
  ...
  memset(&ccm, 0, sizeof(ccm));
    ...
    MUINT32 type = pREQ->cmd.ccm_type;
  ...
  ACDK_LOGD("[CCAP]:FT_ACDK_CCT_V2_OP_AWB_GET_CURRENT_CCM pass! type($d)", pREQ->cmd.ccm_type);
...
bool FT_ACDK_CCT_V2_OP_ISP_GET_TUNING_PARAS_BUF (const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
  ...
  ACDK_LOGD("[%s](ctgy=%d, length=%d, offset=%d, totalLength=%d, isDone=%d)",
    ...
    case EIsp_Category_NR1:
        ...
        break;
    case EIsp_Category_PDC:
        pRefBase = (char*)&g_get_isp_para.stIspNvramRegs.PDC[0];
        RefBufLen = sizeof(g_get_isp_para.stIspNvramRegs.PDC);
        break;
    case EIsp_Category_NSL2:
        pRefBase = (char*)&g_get_isp_para.stIspNvramRegs.NSL2[0];
        RefBufLen = sizeof(g_get_isp_para.stIspNvramRegs.NSL2);
        break;
  ...
  pCNF->status = FT_CNF_FAIL;
    ...
    ACDK_LOGD("[%s](offset, length, totalLength, isDone, RefBufLen) = (%d, %d, %d, %d, %d)"
        , __FUNCTION__, bufAccess.offset, bufAccess.length, bufAccess.totalLength, bufAccess.isDone, RefBufLen);
  ...
  for (i = 0; i < NVRAM_BNR_BPC_TBL_NUM; i++)
        {
            ACDK_LOGD("+ BPC[%u]", i);
        ...
        {
            ACDK_LOGD("+ PDC[%u]", i);
            for (j = 0; j < g_get_isp_para.stIspNvramRegs.PDC[i].COUNT; j++)
            {
                ACDK_LOGD("  - set[%u]=%u", j, g_get_isp_para.stIspNvramRegs.PDC[i].set[j]);
            }
        }
   else if(ctgy == EIsp_Category_UDM)
        for (i = 0; i < NVRAM_UDM_TBL_NUM; i++)
...
bool FT_ACDK_CCT_V2_OP_AWB_SET_NVRAM_CCM(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
      ...
      &u4RetLen))
  {
    ...
    ACDK_LOGD("FT_ACDK_CCT_V2_OP_AWB_SET_NVRAM_CCM pass! type(%d)", inCCM.u1Type);
    pCNF->status = FT_CNF_OK;
    return true;
}
...
bool FT_ACDK_CCT_V2_OP_ISP_GET_SHADING_TABLE_V3(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
  ...
  ACDK_LOGD("[CCAP] ACDK_CCT_V2_OP_ISP_GET_SHADING_TABLE_V3 enter");
   ...
   for (MINT32 j = 0; j  < MSDK_Table.Length; j=j+4)
    ...
    ACDK_LOGD("[CCAP]out buffer size:%d",sizeof(ACDK_CCT_TABLE_SET_STRUCT));
...
bool FT_ACDK_CCT_OP_SET_CCM_WB(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
  ACDK_LOGD("ACDK_CCT_OP_SET_CCM_WB");
    ...
    ACDK_CCT_SET_CCM_WB_STRUCT ccm_wb_struct;
  memset(&ccm_wb_struct, 0, sizeof(ccm_wb_struct));
  memcpy(&ccm_wb_struct, &(pREQ->cmd.set_ccm_wb_gain), sizeof(ccm_wb_struct));

  UINT32 u4RetLen = 0;

  if(!bSendDataToCCT(ACDK_CCT_OP_SET_CCM_WB,
                        (UINT8*)&ccm_wb_struct,
                        sizeof(ccm_wb_struct),
                        NULL,
                        0,
                        &u4RetLen))
      {
  ...
  ACDK_LOGD("FT_ACDK_CCT_OP_SET_CCM_WB pass! type(%d)", pREQ->cmd.set_ccm_wb_gain.u1Type);
      pCNF->status = FT_CNF_OK;
      return true;
}
...
bool FT_ACDK_CCT_V2_OP_GET_DYNAMIC_CCM_COEFF(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
    ...
    MUINT32 ccm_type = pREQ->cmd.ccm_type;
...
bool FT_ACDK_CCT_V2_OP_SET_DYNAMIC_CCM_COEFF(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
    ...
    UINT32 u4RetLen = 0;
    ACDK_CCT_SET_CCM_POLY22_STRUCT SetCCMPoly22;
    memcpy(&SetCCMPoly22, &(pREQ->cmd.set_ccm_poly22), sizeof(SetCCMPoly22));

  //debug log
    ACDK_LOGD("i4R_AVG:%d\n", SetCCMPoly22.ccm_poly22.i4R_AVG);
    ACDK_LOGD("i4R_STD:%d\n", SetCCMPoly22.ccm_poly22.i4R_STD);
    ACDK_LOGD("i4B_AVG:%d\n", SetCCMPoly22.ccm_poly22.i4B_AVG);
    ACDK_LOGD("i4B_STD:%d\n", SetCCMPoly22.ccm_poly22.i4B_STD);
  int i = 0;
  for( i = 0 ; i < 9; i++)
  {
    ACDK_LOGD("[%4d] i4p00:%d\n", i, SetCCMPoly22.ccm_poly22.i4P00[i]);
  }
  for( i = 0 ; i < 9; i++)
  {
    ACDK_LOGD("[%4d] i4p10:%d\n", i, SetCCMPoly22.ccm_poly22.i4P10[i]);
  }
  for( i = 0 ; i < 9; i++)
  {
    ACDK_LOGD("[%4d] i4p01:%d\n", i, SetCCMPoly22.ccm_poly22.i4P01[i]);
  }
  for( i = 0 ; i < 9; i++)
  {
    ACDK_LOGD("[%4d] i4p20:%d\n", i, SetCCMPoly22.ccm_poly22.i4P20[i]);
  }
  for( i = 0 ; i < 9; i++)
  {
    ACDK_LOGD("[%4d] i4p11:%d\n", i, SetCCMPoly22.ccm_poly22.i4P11[i]);
  }
  for( i = 0 ; i < 9; i++)
  {
    ACDK_LOGD("[%4d] i4p02:%d\n", i, SetCCMPoly22.ccm_poly22.i4P02[i]);
  }
    ...
    ACDK_LOGD("FT_ACDK_CCT_V2_OP_SET_DYNAMIC_CCM_COEFF pass! type(%d)", SetCCMPoly22.u1Type);
    pCNF->status = FT_CNF_OK;
...
bool FT_ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
    ACDK_LOGD("FT_ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO\n");

    float isp_ccm_ratio = -1;
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    BOOL bRet = bSendDataToCCT(ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO,
                                                        NULL,
                                                        0,
                                                        (UINT8 *)&isp_ccm_ratio,
                                                        sizeof(isp_ccm_ratio),
                                                        &u4RetLen);
    ...
    {
        pCNF->status = FT_CNF_FAIL;
        ACDK_LOGD("[CCAP]:ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO driver failed!");
        return false;
    }
    if ((isp_ccm_ratio > 1) || (isp_ccm_ratio < 0))
    {
        pCNF->status = FT_CNF_FAIL;
        ACDK_LOGD("[CCAP]:ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO wrong isp_ccm_ratio (%f)!", isp_ccm_ratio);
        return false;
    }

    ACDK_LOGD("isp_ccm_ratio = %f\n", isp_ccm_ratio);

    pCNF->result.isp_ccm_ratio = isp_ccm_ratio;
    pCNF->status = FT_CNF_OK;
    return true;
}

bool FT_ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO(const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
    ACDK_LOGD("FT_ACDK_CCT_K2_CCM_SET_SMOOTH_SWITCH\n");

    float isp_ccm_ratio = pREQ->cmd.isp_ccm_ratio;
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    BOOL bRet = bSendDataToCCT(ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO,
                                            (UINT8 *)&isp_ccm_ratio,
                                            sizeof(isp_ccm_ratio),
                                            NULL,
                                            0,
    ...
    {
        pCNF->status = FT_CNF_FAIL;
        ACDK_LOGD("[CCAP]:ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO driver failed!");
        return false;
    }
...
bool FT_ACDK_CCTIA_AWB_GET_AWB_PARA (const FT_CCT_REQ* pREQ,FT_CCT_CNF* pCNF,char** pBuff)
{
    ACDK_LOGD("[%s]+", __FUNCTION__);
    AWB_PARAM_T rAWBPARAM; // TEST ONLY

    memset(&rAWBPARAM,0, sizeof(AWB_PARAM_T)); // TEST ONLY
    ...
    //! 2. Modify the input data, size, and output data, size
    //! ====================================================
    if(!bSendDataToCCT(ACDK_CCTIA_OP_AWB_GET_PARA,
                                NULL,
                                0,
                                (UINT8 *)&rAWBPARAM,
                                sizeof(AWB_PARAM_T),
    ...
    memcpy(&(pCNF->result.get_awb_param),&rAWBPARAM,sizeof(AWB_PARAM_T));
...
static bool ft_cct_init()
{
  ...
  //init statemachine
    ...
    g_FT_CCT_StateMachine.sensor_onboard_sensors.sensor[0].rawFmtType  = ACDK_Sensor.rawFmtType;
    ACDK_LOGD("[CCAP]: Get Sensor Type(%d), ID(%u), rawFmtType(%d)",(int)ACDK_Sensor.Type, ACDK_Sensor.DeviceId, ACDK_Sensor.rawFmtType);
...
//  {FT_CCT_V2_OP_SAVE_OB_ON_OFF,           &FT_ACDK_CCT_V2_OP_SAVE_OB_ON_OFF,          "FT_CCT_V2_OP_SAVE_OB_ON_OFF"},
  ...
  {FT_CCT_V2_OP_GET_ISP_CCM_RATIO, &FT_ACDK_CCT_V2_OP_GET_ISP_CCM_RATIO,"FT_CCT_V2_OP_GET_ISP_CCM_RATIO"},
  {FT_CCT_V2_OP_SET_ISP_CCM_RATIO, &FT_ACDK_CCT_V2_OP_SET_ISP_CCM_RATIO,"FT_CCT_V2_OP_SET_ISP_CCM_RATIO"},
  ...
  {FT_CCTIA_AWB_GET_AWB_PARA, &FT_ACDK_CCTIA_AWB_GET_AWB_PARA, "FT_CCTIA_AWB_GET_AWB_PARA"},
...
void META_CCAP_ADB_OP()
{
    ...
    ACDK_LOGD("[%s] file size(%I64u), sizeof(FT_CCT_REQ)=(%zu)",  __FUNCTION__, file_size, sizeof(FT_CCT_REQ));

    // get inBuf from file
    if( file_size > sizeof(FT_CCT_REQ) )
    {
        ACDK_LOGD("[%s] inBuf size(%I64u)",  __FUNCTION__, file_size - sizeof(FT_CCT_REQ));
    ...
    {
        char fileName[64];
        sprintf(fileName, "/sdcard/inBuf.bin");
