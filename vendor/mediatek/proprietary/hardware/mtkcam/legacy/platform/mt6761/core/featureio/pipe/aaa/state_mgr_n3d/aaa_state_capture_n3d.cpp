 ...
 ************************************************************************************************/
#define LOG_TAG "aaa_state_cap_n3d"
...
sendIntent(intent2type<eIntent_CaptureEnd>)
{
    ...
    MBOOL fg3DCap = (ISync3AMgr::E_SYNC3AMGR_CAPMODE_3D == ISync3AMgr::getInstance()->getCaptureMode());
...
if (ENABLE_AWB & m_pHal3A->m_3ACtrlEnable) {
    ...
    IAwbMgr::getInstance().doCapAWB(m_SensorDevId, i4SceneLV, reinterpret_cast<MVOID *>(rBufInfo.virtAddr), (fg3DCap ? MFALSE : MTRUE));
