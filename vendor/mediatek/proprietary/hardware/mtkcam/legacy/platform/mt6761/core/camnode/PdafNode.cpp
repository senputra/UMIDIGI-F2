    ...
    , muUserCnt(0)
{
    ...
    addDataSupport( ENDPOINT_SRC, PDAF_CAP_SRC );
}
...
threadLoopUpdate()
{
    ...
    MUINT32      i4RawFmt   = postBufData.data==PDAF_SRC ? 1 : 0; /* 0 represe precess raw, 1 represent pure raw */

    // currently force main sensor (1)
    pIAfMgr->doPDBuffer( 1,
                         (MVOID *)pImgBuf->getBufVA(0),
    ...
    handleReturnBuffer(postBufData.data, (MUINTPTR)pImgBuf);
    FUNC_END;
    return ret;
}
