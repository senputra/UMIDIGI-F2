...
Hal3A::createThread()
{
    ...
    sem_init(&semPDCtrl, 0, 0);
    sem_init(&semPDCtrlThdEnd, 0, 1);
...
Hal3A::createAEThread()
{
    ...
    if(1)//(mbCreatePDCtrlThd)
    {
        mbEnPDCtrlThd = MTRUE; // shoule set as true, before pdvc thread is created.
        pthread_create(&mPDControlThread, NULL, PDCtrlThreadLoop, this);
    }
}
...
Hal3A::destroyAEThread()
{
    ...
    //
    mbEnPDCtrlThd = MFALSE;
    ::sem_post(&semPDCtrl);
    pthread_join(mPDControlThread, NULL);
...
*******************************************************************************/
MVOID
Hal3A::changePDCtrlThreadSetting()
{
#define PD_FLOW_CONTROL_THREAD_NAME "PDFlowCtrlThd"

    // (1) set name
    ::prctl(PR_SET_NAME, PD_FLOW_CONTROL_THREAD_NAME, 0, 0, 0);
    ...
    // (2) set policy/priority
    {
        ...
        getThreadPriority(policy, priority);
        //
        MY_LOG( "%s policy:(expect, result)=(%d, %d), priority:(expect, result)=(%d, %d)",
                PD_FLOW_CONTROL_THREAD_NAME,
                expect_policy,
                policy,
                expect_priority,
                priority
...
Hal3A::sendCommand(ECmd_T const r3ACmd, MINTPTR const i4Arg)
{
	...
	::sem_getvalue(&semPDCtrlThdEnd, &Val);
	if (Val == 0) ::sem_post(&semPDCtrlThdEnd); //to be 1
	MY_LOG_IF(1, "[Hal3A::sendCommand] PreviewStart semPDCtrlThdEnd(%d) before post", Val);
	//sem_init(&semPDCtrlThdEnd, 0, 1);
...
Hal3A::getCommand(CmdQ_T &rCmd, MBOOL &bGetCmd, MBOOL en_timeout)
{
    ...
    rCmd = *mCmdQ.begin();
    MY_LOG_IF(m_3ALogEnable & EN_3A_FLOW_LOG, "CmdQ-size(%d), todo(%d, %d)", mCmdQ.size(), rCmd.eCmd, rCmd.rParamIspProfile.i4MagicNum);
