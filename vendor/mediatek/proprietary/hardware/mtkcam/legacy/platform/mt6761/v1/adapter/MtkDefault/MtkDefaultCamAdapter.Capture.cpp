...
updateShotInstance()
{
    ...
    {
        bRet = createShotInstance(mpShot, u4ShotMode, getOpenId(), getParamsManager());
...
takePicture()
{
    ...
    pCallbackThread = mpZipCallbackThread.promote();
    bool ret_incShotCount = pCallbackThread->incShotCount();
    if (ret_incShotCount == false){
        //ZipImageCallbackThread has been released by itself,
        //so we need to allocate a new one in prepareToCapture
        MY_LOGI("allocate another new ZipCallbackThread");
        if( OK != (status = prepareToCapture()) )
        {
            MY_LOGE("failed to prepareToCapture");
            goto lbExit;
    ...
    // 4-cell Remosaic Cap Flow
    if(mShotMode == NSCam::eShotMode_4CellRemosaicShot) mb4CellReMosaicCapFlow = MTRUE;
    ...
    if(mpStateManager->isState(IState::eState_Recording))
    {
        ...
        else
        {
            ...
            {
                mpRecBufSnapshotlNode->transitToVSS();
            }
            //
        }
    }
    else
    {
        if(mpParamsMgr->getInt(MtkCameraParameters::KEY_CAMERA_MODE) != MtkCameraParameters::CAMERA_MODE_NORMAL)
        {
            ...
            else if(mShotMode == eShotMode_4CellRemosaicShot)
            {
                MY_LOGD("4-Cell Remosaic Shot, stop Preview");
                mbTakePicPrvNotStop = false;
            }
        }
        //
...
onHandleCancelCapture()
{
    ...
    if(mShotMode == NSCam::eShotMode_ContinuousShot)
    {
        ...
        if(mpShot == NULL && mbCaptureFinished == MFALSE)
        {
            ...
            MY_LOGD("pShot (CShot) is NULL, need to wait (%lld) ns +",timeout);
...
onCaptureThreadLoop()
{
    ...
    MUINT32 transform = 0;
    ResMgrDrv::MODE_STRUCT mode;
    sp<JobMonitor::Helper> monitor = new JobMonitor::Helper(String8(__PRETTY_FUNCTION__), 120*1000);
    //
    if(mpResMgrDrv)
    {
        mpResMgrDrv->getMode(&mode);
