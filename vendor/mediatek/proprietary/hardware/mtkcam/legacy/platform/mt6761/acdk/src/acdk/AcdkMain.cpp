...
static MUINT32 get_boot_mode(void)
{
  ...
  if (s > 3) {
      ACDK_LOGE("s:%d is more than 4", s);
      return 0;
  }
...
MINT32 AcdkMain::init()
{
    ...
    //====== 3A init ======
#if (FAKE_SENSOR) || defined(ACDK_DUMMY_ENG)
        ACDK_LOGD("No 3A\n");
#else
...
static MUINT32 mPowerOnTM[] = { //base: 0x15040000
        ...
        MUINT const*pArrayOfIndex)
{
    ...
    TS_LOGD("TG: Total %d registers", nNum);

    for (i = 0; i < nNum; i++) {
        m_pIspDrv->writeReg(mPowerOnTG[i<<1], mPowerOnTG[(i<<1)+1],ISP_DRV_USER_ISPF);
    }
