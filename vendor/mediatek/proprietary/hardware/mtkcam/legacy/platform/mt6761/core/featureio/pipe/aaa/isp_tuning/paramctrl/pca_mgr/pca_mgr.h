...
public:     ////    load
    ...
    inline
    MVOID
    getLut(ISP_NVRAM_PCA_LUT_T*& pca_lower, ISP_NVRAM_PCA_LUT_T*& pca_upper)
    {
        switch (m_u4Idx)
            {
            case LUT_LOW:       //color temperature: low
                pca_lower = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_lo[0]);
                pca_upper = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_md[0]);
                return;

            case LUT_MIDDLE:       //color temperature: low
                pca_lower = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_md[0]);
                pca_upper = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_hi[0]);
                return;

            case LUT_HIGH:       //color temperature: low
                pca_lower = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_hi[0]);
                pca_upper = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_hi[0]);
                return;

            case LUT_LOW_2:       //color temperature: low
                pca_lower = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_lo2[0]);
                pca_upper = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_md2[0]);
                return;


            case LUT_MIDDLE_2:       //color temperature: low
                pca_lower = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_md2[0]);
                pca_upper = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_hi2[0]);
                return;

            case LUT_HIGH_2:       //color temperature: low
                pca_lower = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_hi2[0]);
                pca_upper = reinterpret_cast<ISP_NVRAM_PCA_LUT_T*>(&m_rIspPcaLUTs.lut_hi2[0]);
                return;
