...
FeaturePipeP2aNode::onInit()
{
        ...
        // For Stereo Camera case
    {
        ...
        EFeatureStreamTag eTag = ( SENSOR_DEV_MAIN_2 == pHalSensorList->querySensorDevIdx(mOpenedSensorIndex) )
                                ? EFeatureStreamTag_Stream : EFeatureStreamTag_vFB_Stream;
        ...
        if(EnableOption[0] == '1')
        {
            eTag = ( SENSOR_DEV_SUB == pHalSensorList->querySensorDevIdx(mOpenedSensorIndex) )
                                ? EFeatureStreamTag_Stream : eTag;
        }

        if (eTag == EFeatureStreamTag_Stream)
...
#undef TransCase
        ...
        if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
        {
...

            {
                ...
                {
                    LOG_WRN("X MDP: %d  CRZ: %d\n", newCropX, eisCropRect.x);
                ...
                {
                    LOG_WRN("Y MDP: %d  CRZ: %d\n", newCropY, eisCropRect.y);
...
static const NSCam::NSIoPipe::PortID WDMAO( NSIoPipe::EPortType_Memory, NSImageio::NSIspio::EPortIndex_WDMAO, 1);
    ...
    if (FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask) ||FEATURE_MASK_IS_EIS_CALIBRATION_ENABLED(mFeatureOnMask))
    {
        ...
        // Always queue current EISConfig/GPUTarget data, then determine queue or not by EIS Node
        if(MTRUE != reinterpret_cast<FeaturePipeEisNode*>(mpFeaturePipeObj->mpEisSwNode)->ConfigEis(EIS_PASS_2,eisConfigData))
        {
            LOG_ERR("FPipe ConfigEis fail");
            return MFALSE;
...
threadInit()
{
    ...
    mpGpuWarp = MTKWarpExt::createInstance(
#ifndef _CPUWARP
            DRV_WARP_OBJ_GLES
#else
            DRV_WARP_OBJ_CPU
...
threadLoopUpdate()
{
    ...
    else if(FEATURE_MASK_IS_EIS_ENABLED(mFeatureOnMask))
    {
        ...
        }else
        {
            ...
            // MDP VA use pointer (depends on 32/64-bit), and mVA use 32-bit version
#if GPU_YV12_SUPPORTED

            void* va[3] = {(void*) pImageBuffer->getBufVA(0),(void*) pImageBuffer->getBufVA(1),(void*) pImageBuffer->getBufVA(2)};
            ...
            void* va[3] = {(void*) pImageBuffer->getBufVA(0),0,0};
...
#undef TransCase
                ...
                {
                    va[j]   = (void*)OutQParams.mvOut[i].mBuffer->getBufVA(j);
