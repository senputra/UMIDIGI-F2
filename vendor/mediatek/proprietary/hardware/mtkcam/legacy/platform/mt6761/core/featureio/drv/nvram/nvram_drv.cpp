...
)
{
    ...
    {
        NVRAM_DRV_LOG("[writeNvram] sensorId error, line=%d",__LINE__);
        return NVRAM_READ_PARAMETER_ERROR;
    }
    if(a_pNvramData==0)
    {
        NVRAM_DRV_LOG("[writeNvram] buf adr error (=0), line=%d",__LINE__);
        return NVRAM_READ_PARAMETER_ERROR;
    }
    if(a_eNvramDataType < CAMERA_DATA_TYPE_START || a_eNvramDataType>=CAMERA_DATA_TYPE_NUM)
    {
        NVRAM_DRV_LOG("[writeNvram] date type id error, line=%d",__LINE__);
        return NVRAM_READ_PARAMETER_ERROR;
    }
    if(a_u4NvramDataSize != g_u4NvramDataSize[a_eNvramDataType])
    {
        NVRAM_DRV_LOG("[writeNvram] buf size is error, line=%d",__LINE__);
        return NVRAM_READ_PARAMETER_ERROR;
    }
