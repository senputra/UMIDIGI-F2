...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    MY_LOG("[StateCameraPreview::sendIntent]<eIntent_CameraPreviewEnd> set m_3AFrameCount to 0");
    m_pHal3A->m_3AFrameCount = 0;
...
if (ENABLE_AAOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    DebugUtil::getInstance(m_SensorDevId)->detectPreframe(DBG_AE, m_pHal3A->m_3AFrameCount);
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    ...
    MY_LOG_IF(m_pHal3A->m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] isNeedUpdateI2C = %d, isSlowMotion = %d", __FUNCTION__, isNeedUpdateI2C, isSlowMotionUpdateI2C);
...
if (ENABLE_AE & m_pHal3A->m_3ACtrlEnable) {
    ...
    m_pStateMgr->m_pTimer->start("Precap_Timer", m_SensorDevId, MFALSE);
