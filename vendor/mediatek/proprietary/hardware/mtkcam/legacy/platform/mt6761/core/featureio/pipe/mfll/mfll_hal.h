...
public:
    /**
     *  With new version algorithm, which can be executed parallelly, and ME and
     *  MC can also be executed separately. The argument *triggerMc is a mutex
     *  that will be locked and unlocked after ME done for synchronization usage.
     *
     *  If it's NULL or MEMC_FLOW_V2 is not defined, this argument doesn't effect
     *  anything indeed.
     */
    virtual MBOOL doMemc(MINT32 mvX, MINT32 mvY, MUINT8* baseBufAddr, MUINT8* refBufAddr, MUINT32 *badMvCount, pthread_mutex_t *triggerMc = NULL);
#if TEST_SWMIXER
...
#ifdef MEMC_FLOW_V2
    MFBLL_PROC_ME_OUT_STRUCT MfbllMeResult;
    MFBLL_PROC_MC_OUT_STRUCT MfbllMcResult;
#else
