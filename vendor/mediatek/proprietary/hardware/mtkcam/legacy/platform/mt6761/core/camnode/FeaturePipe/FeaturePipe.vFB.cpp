...
threadLoopUpdate()
{
    ...
    if (FEATURE_MASK_IS_SMILE_SHOT_ENABLED(mFeatureOnMask) && (FaceCount))
    {
        if (!IsDetected_SD)
        {
            ...
            property_get("vendor.camera.featurepipe.fakesmile", EnableOption, "0");
            if (EnableOption[0] == '1')
            {
                LOG_DBG("[fake] enable IsDetected_SD");
                IsDetected_SD = true;
                property_set("vendor.camera.featurepipe.fakesmile", "0");
            }
        }
    ...
    if (FEATURE_MASK_IS_GESTURE_SHOT_ENABLED(mFeatureOnMask) && (FaceCount))
    {
        ...
        {
            char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
            property_get("vendor.camera.featurepipe.fakegesture", EnableOption, "0");
            if (EnableOption[0] == '1')
            {
                GestureDetected = true;
                property_set("vendor.camera.featurepipe.fakegesture", "0");
            }
        }
        if (GestureDetected)
        {
    ...
    MY_LOGD_IF(sDebugLogEnable,"Returning P2A node DS image buffer of frame %d", jobDsImg.mFrameNo);
    handleReturnBuffer(P2A_TO_FD_DSIMG, (MUINTPTR) &jobDsImg.imgReq, jobDsImg.mFrameNo);

    /* Post buffer to next node */
    MY_LOGD_IF(sDebugLogEnable,"Posting FD->P2B node of frame %d", frameNo);
...
{
    //FUNCTION_LOG_START;
    MY_LOGD_IF(sDebugLogEnable,"data(0x%x), buf(0x%x), ext(0x%x)", data, buf, ext);
...
threadLoopUpdate()
{
    ...
    {
        MY_LOGD_IF(sDebugLogEnable,"Found face index %d left %d top %d right %d button %d pose %d (frame %d)",i+1,pFDResult->mFaceMetadata.faces[i].rect[0],pFDResult->mFaceMetadata.faces[i].rect[1],pFDResult->mFaceMetadata.faces[i].rect[2],pFDResult->mFaceMetadata.faces[i].rect[3],pFDResult->mFaceMetadata.posInfo[i].rip_dir, frameNo);
    }
    ...
    MY_LOGD_IF(sDebugLogEnable,"mHalVFBProc Processing QVGA 0x%x, Warped QVGA 0x%x, AlphaCL 0x%x, AlphaNR 0x%x, PCA Table 0x%x, Warp Map 0x%x, 0x%x", VfbProcInfo.DsImgAddr, VfbProcInfo.WarpedDsImage, VfbProcInfo.AlphaCL, VfbProcInfo.AlphaNR, VfbProcInfo.PCATable, VfbProcInfo.WarpMapX, VfbProcInfo.WarpMapY);
    ...
    MY_LOGD_IF(sDebugLogEnable,"cbFaceInfo performCallback +");
    if( !mpFeaturePipeObj->performCallback(CAMERA_MSG_PREVIEW_METADATA, retFaces) )
    {
        LOG_ERR("cbFaceInfo performCallback failed!");
    }
    MY_LOGD_IF(sDebugLogEnable,"cbFaceInfo performCallback -");

#ifndef _DEBUG_DUMP
    char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.featurepipe.dumpvfb", EnableOption, "0");
    if (EnableOption[0] == '1')
#endif
    {
        char szFileName[100];
    ...
    MY_LOGD_IF(sDebugLogEnable,"Returning FD node face metadata buffer of frame %d", jobDsImg.mFrameNo);
    ...
    /* Post buffer to next node */
    MY_LOGD_IF(sDebugLogEnable,"Posting VFB->P2B node of frame %d", frameNo);
    MY_LOGD_IF(sDebugLogEnable,"Posting VFB->P2B ds req(%x) mbuffer(%x)", &dsImgReq, dsImgReq.mBuffer);
...
onReturnBuffer(MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
    //FUNCTION_LOG_START;
    MY_LOGD_IF(sDebugLogEnable,"data(0x%x), buf(0x%x), ext(0x%x)", data, buf, ext);
    MY_LOGD_IF(sDebugLogEnable,"Recycling VFB node %s image of frame %d",
...
{
    MY_LOGD_IF(sDebugLogEnable,"Set vFB PCA@0x%x", pImageBuffer->getBufVA(0));
...
//#define _WORKAROUND_FD_NOT_SUPPORT_YUY2
    ...
    char EnableOption[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.debug.camera.fd.fakefaceen", EnableOption, "0");
...
{
    MY_LOGD_IF(sDebugLogEnable,"Face detection: rotation %d", rotation);
    mRotation = rotation;
    return MTRUE;
}
