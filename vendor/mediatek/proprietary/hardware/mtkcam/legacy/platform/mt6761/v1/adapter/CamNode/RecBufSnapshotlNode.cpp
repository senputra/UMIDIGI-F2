...
class RecBufSnapshotlNodeImpl : public RecBufSnapshotlNode
{
    ...
    public: // operations
        ...
        virtual MBOOL transitToVSS(void);
...
threadLoopUpdate()
{
    ...
    if(mbInVSS)
    {
        MY_LOGD("vss img copied");
        ::sem_post(&semVSSBufReady);
        mbInVSS = MFALSE;
    }
        //
...
stopRecording()
{
    if(mbInVSS)
    {
        MY_LOGD("wait vss buf ready +");
        ::sem_wait(&semVSSBufReady);
        MY_LOGD("wait vss buf ready -");
    }
    Mutex::Autolock lock(mLock);
    //
