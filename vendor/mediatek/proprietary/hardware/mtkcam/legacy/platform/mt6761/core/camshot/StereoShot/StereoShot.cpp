...
enum
{
    ...
    DATA_ITMAINYUV          = 0x00000400,   // for main bokeh image yuv
    DATA_ITMAINYUV_THUMB    = 0x00000800,   // for main thumbnail yuv

    DATA_ITMAINYUV_CLEANIMG = 0x00001000,   // for main clean image yuv

    DATA_MAINJPEG           = 0x00002000,   // main jpeg
    DATA_THUMBJPEG          = 0x00004000,   // thumb jpeg
    DATA_MAINJPEG_CLEANIMG  = 0x00008000,

    DATA_DEBUGINFO          = 0x00010000,   // main debug info
    DATA_DEBUGINFO_S        = 0x00020000,   // main2 debug info

    // extend for connection
    DATA_MAINJPEG_CB        = 0x00040000,
    DATA_THUMBNAILJPEG_CB   = 0x00080000,
    DATA_MAINJPEG_CLEANIMG_CB = 0x00100000,
    DATA_DEPTHMAP_CB        = 0x00200000,
    DATA_FULLRAW_CB         = 0x00400000,
};
...
MAP_TYPE2BIT(STEREO_IMG             , 5);

// StereoCtrlNode IN/OUT
MAP_TYPE2BIT(STEREO_CTRL_IMG_0      , 6);
MAP_TYPE2BIT(STEREO_CTRL_IMG_1      , 7);
MAP_TYPE2BIT(STEREO_CTRL_MAIN_SRC   , 8);
MAP_TYPE2BIT(STEREO_CTRL_DST_M      , 9);
MAP_TYPE2BIT(STEREO_CTRL_MAIN_DST   , 10);  //
MAP_TYPE2BIT(STEREO_CTRL_DEPTHMAP_DST,11);

// Image transfer node IN/OUT
MAP_TYPE2BIT(TRANSFORM_SRC          , 12);
MAP_TYPE2BIT(TRANSFORM_DST_0        , 13);
MAP_TYPE2BIT(TRANSFORM_DST_1        , 14);
MAP_TYPE2BIT(TRANSFORM_DST_2        , 15);

// Jpeg node IN/OUT
MAP_TYPE2BIT(JPEG_ENC_HW_SRC_JPEG   , 16);
MAP_TYPE2BIT(JPEG_ENC_HW_DST_JPEG   , 17);
MAP_TYPE2BIT(JPEG_ENC_SW_SRC        , 18);
MAP_TYPE2BIT(JPEG_ENC_SW_DST        , 19);
MAP_TYPE2BIT(JPEG_ENC_HW_SRC        , 20);
MAP_TYPE2BIT(JPEG_ENC_HW_DST        , 21);

// ShotCallBack Node IN/OUT
MAP_TYPE2BIT(SHOTCB_JPEG_SRC        , 22);
MAP_TYPE2BIT(SHOTCB_CLEANIMG_SRC    , 23);
MAP_TYPE2BIT(SHOTCB_THUMBNAIL_SRC   , 24);
MAP_TYPE2BIT(SHOTCB_DEPTHMAP_SRC    , 25);

// debug info saved in exif
MAP_TYPE2BIT(CONTROL_DBGINFO        , 26);
MAP_TYPE2BIT(SHOTCB_DBGINFO_SRC     , 27);
...
static const struct map_data2node gMapData2Node_Raw[] =
{
    ...
    { DATA_ITMAINYUV_THUMB  , TRANSFORM_DST_0       , bitS2(STEREO_CTRL_MAIN_DST, TRANSFORM_SRC)        } ,
    { DATA_ITMAINYUV_CLEANIMG,TRANSFORM_DST_2       , bitS2(STEREO_CTRL_DST_M, TRANSFORM_SRC)           } ,

    { DATA_MAINJPEG         , JPEG_ENC_HW_DST_JPEG  , bitS2(TRANSFORM_DST_1, JPEG_ENC_HW_SRC_JPEG)      } ,
    { DATA_THUMBJPEG        , JPEG_ENC_SW_DST       , bitS2(TRANSFORM_DST_0, JPEG_ENC_SW_SRC)           } ,
    { DATA_MAINJPEG_CLEANIMG, JPEG_ENC_HW_DST       , bitS2(TRANSFORM_DST_2, JPEG_ENC_HW_SRC)           } ,
    ...
    { DATA_MAINJPEG_CLEANIMG_CB , 0                 , bitS2(JPEG_ENC_HW_DST, SHOTCB_CLEANIMG_SRC)      } ,
    { DATA_DEPTHMAP_CB       , 0                    , bitS2(STEREO_CTRL_DEPTHMAP_DST, SHOTCB_DEPTHMAP_SRC)        } ,
    { 0, 0, 0 } ,
};
...
startOne(SensorParam const & rSensorParam)
{
    ...
    // set buffer handle for stereo ctrl node output (STEREO_CTRL_MAIN_DST : Bokeh image)
    mpGraph->setBufferHandler(   STEREO_CTRL_MAIN_DST,  mpAllocBufHandler,    getOpenId_Main());
    // set buffer handle for stereo ctrl node output (STEREO_CTRL_DEPTHMAP_DST : depthmap image)
    mpGraph->setBufferHandler(   STEREO_CTRL_DEPTHMAP_DST,  mpAllocBufHandler,    getOpenId_Main());
    ...
    if( dataInBit & DATA_ITMAINYUV_THUMB )
    {
        ret = ret && enableData(DATA_ITMAINYUV_THUMB);
    }

    if( dataInBit & DATA_ITMAINYUV_CLEANIMG )
    {
        ret = ret && enableData(DATA_ITMAINYUV_CLEANIMG);
    }
    ...
    if( dataInBit & DATA_MAINJPEG_CLEANIMG )
    {
        ret = ret && enableData(DATA_MAINJPEG_CLEANIMG);
    }
...
registerImageBuffer(ECamShotImgBufType const eBufType, IImageBuffer const *pImgBuffer)
{
    ...
    switch( eBufType )
    {
        ...
        case ECamShot_BUF_TYPE_JPEG:
            ...
            break;
        case ECamShot_BUF_TYPE_JPEG_CLEANIMG:
            internaldata = DATA_MAINJPEG_CLEANIMG;
            break;
        default:
            ret = MFALSE;
            break;
...
MBOOL
StereoShot::
fgNodeNotifyCallback(MVOID* user, NSCamNode::NodeNotifyInfo const msg)
{
...
MBOOL
StereoShot::
fgNodeDataCallback(MVOID* user, NSCamNode::NodeDataInfo const msg)
{
    StereoShot* pStereoShot = reinterpret_cast<StereoShot*>(user);
    if( pStereoShot )
    {
        MUINT32 data = 0;
        MY_LOGD("fgNodeDataCallback(): callback message=0x%x",msg.msgType);
        switch( msg.msgType )
        {
            ...
            case NSCamNode::ENode_DATA_MSG_JPS:
                ...
                break;
            case NSCamNode::ENode_DATA_MSG_JPEG_CLEANIMG:
                data = ECamShot_DATA_MSG_JPEG_CLEANIMG;
                break;
            case NSCamNode::ENode_DATA_MSG_DEPTHMAP:
                data = ECamShot_DATA_MSG_DEPTHMAP;
                break;
            case NSCamNode::ENode_DATA_MSG_META://
...
)
{
    ...
    Pass1NodeInitCfg p1InitCfg_main2( mSensorParam.u4Scenario,                        // scenario
                                3,                                              // ring buf cnt
                                mSensorParam.u4Bitdepth,                        // bit depth
                                mu4FrameRate,                                   // frame rate
                                mSensorParam.u4RawType);                        // raw type
    ...
    MSize verifyImageSize = StereoSettingProvider::getAlgoImageSize(0);
    MSize DepthmapSize = StereoSettingProvider::getDepthmapSize();
    ...
    SCNodeInitCfg SCNodeInitCfg( SCNodeType,
                                 ...
                                 mSensorParam.u4Scenario,
                                 DepthmapSize.w,            // set depthmap size only in capture mode
                                 DepthmapSize.h);           // set depthmap size only in capture mode
    ...
    if( pImgBuf_0 == NULL ) {
        ...
        mpPass1_Main2   = Pass1Node::createInstance(p1InitCfg_main2);
    ...
    mspCtrl         = mpCtrl;
    mpCtrl_Main2    = StereoShotCtrlNode::createInstance(mShotParam, muCapStyle, MFALSE);  // force to use ZsdVssCtrl
    mspCtrl_Main2   = mpCtrl_Main2;
    ...
    MBOOL bUseSensorMaxCrop = MTRUE;
    MRect main1FullCrop;
    StereoSettingProvider::getMain1FullSzCrop(bUseSensorMaxCrop,main1FullCrop);
    MBOOL bUseSelfDefinedMain1Crop = (bUseSensorMaxCrop==MTRUE)? MFALSE: MTRUE;
    MY_LOGD("Set Pass2Node crop: bCrop=%d, CropSize(%d,%d),(%dx%d)",
            bUseSelfDefinedMain1Crop,
            main1FullCrop.p.x,main1FullCrop.p.y,
            main1FullCrop.s.w,main1FullCrop.s.h);

    mpStereoCapNode         = Pass2Node::createInstance( PASS2_CAPTURE );
    mpStereoCapNode->setCropSize(bUseSelfDefinedMain1Crop,
                                 main1FullCrop);     // setup self-defined crop size
    ...
    mpITNode                = ImgTransformNode::createInstance(false,true);       // if DST_0 need rotate, call mdp twice
    mpITNode_CleanImage     = ImgTransformNode::createInstance(true,false);       // use TRANSFORM_DST_2 as output for buffer handle limitation
    ...
    mpITNode_CleanImage->setSensorIdx( getOpenId_Main() );
...
#define NODE_DESTROY( node )     \
    ...
    NODE_DESTROY(mpITNode_CleanImage)
...
#define updateData( SHOT_DATA_MSG, inter_data, cb, need_data, need_cb) \
    ...
    *pNeededData |= (DATA_SIMG | DATA_SFEO | DATA_SRGB | DATA_SMAIN | DATA_SCMAIN);
    ...
    //clean image jpeg
    updateData(
            ECamShot_DATA_MSG_JPEG_CLEANIMG,
            DATA_ITMAINYUV_CLEANIMG | DATA_MAINJPEG_CLEANIMG | DATA_MAINJPEG_CLEANIMG_CB,
            ENode_DATA_MSG_JPEG_CLEANIMG, // callback enable data
            *pNeededData,
            *pNeededCb
            );
    ...
    // depthmap
    updateData(
            ECamShot_DATA_MSG_DEPTHMAP,
            DATA_DEPTHMAP_CB,
            ENode_DATA_MSG_DEPTHMAP, // callback enable data
            *pNeededData,
            *pNeededCb
            );
    ...
    if( mJpegParam.u4ThumbWidth && mJpegParam.u4ThumbHeight )
    {
        ...
        updateData(
                ECamShot_DATA_MSG_JPEG,
                DATA_ITMAINYUV_THUMB | DATA_THUMBJPEG | DATA_THUMBNAILJPEG_CB,
                ENode_DATA_MSG_JPEG, // callback enable data
                *pNeededData,
                *pNeededCb
                );
...
connectNodes(MUINT32 NodeDataSet)
{
    ...
    // [SCNode -- ITNode (bokeh & clean image) ]
    mpGraph->connectData(   STEREO_CTRL_MAIN_DST,   TRANSFORM_SRC,          mpStereoCtrlNode,       mpITNode);
    mpGraph->connectData(   STEREO_CTRL_DST_M   ,   TRANSFORM_SRC,          mpStereoCtrlNode,       mpITNode_CleanImage);

    // [SCNode -- CBNode (depthmap) ]
    mpGraph->connectData(  STEREO_CTRL_DEPTHMAP_DST,SHOTCB_DEPTHMAP_SRC,    mpStereoCtrlNode,       mpCallback);

    // [ITNode(bokeh image) -- JpegEncNode]
    mpGraph->connectData(   TRANSFORM_DST_1,        JPEG_ENC_HW_SRC_JPEG,   mpITNode,               mpJpegencHw);   // Jpeg main for bokeh image
    mpGraph->connectData(   TRANSFORM_DST_0,        JPEG_ENC_SW_SRC,        mpITNode,               mpJpegencSw);   // for thumbnail

    // [ITNode(clean image) -- JpegEncNode]
    mpGraph->connectData(   TRANSFORM_DST_2,        JPEG_ENC_HW_SRC,        mpITNode_CleanImage,    mpJpegencHw);   // Jpeg main for clean image
    ...
    mpGraph->connectData(   JPEG_ENC_HW_DST,        SHOTCB_CLEANIMG_SRC,    mpJpegencHw,            mpCallback);    // cb for clean image
...
getNode(MUINT32 node)
{
    switch(node)
    {
        ...
        case ITNODE:
            return mpITNode;
        case ITNODE_CLEANIMAGE:
            return mpITNode_CleanImage;
        case HWJPEGNODE:
            return mpJpegencHw;
        case CALLBACKNODE:
            return mpCallback;
