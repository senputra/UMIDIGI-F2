...
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
MRESULT IAeMgr::setShutterValue(MINT32 i4SensorDev, MINT32 i4ShutterValue)
{
    MINT32 ret_main, ret_sub, ret_main2;
    ret_main = ret_sub = ret_main2 = S_AE_OK;

    if (i4SensorDev & ESensorDev_Main)
        ret_main = AeMgr::getInstance(ESensorDev_Main).setShutterValue(i4ShutterValue);
    if (i4SensorDev & ESensorDev_Sub)
        ret_sub = AeMgr::getInstance(ESensorDev_Sub).setShutterValue(i4ShutterValue);
    if (i4SensorDev & ESensorDev_MainSecond)
        ret_main2 = AeMgr::getInstance(ESensorDev_MainSecond).setShutterValue(i4ShutterValue);
...
MVOID IAeMgr::getMShutterCaptureParams(MINT32 i4SensorDev, AE_MODE_CFG_T &a_rCaptureInfo)
{

    if (i4SensorDev & ESensorDev_Main)
        AeMgr::getInstance(ESensorDev_Main).getMShutterCaptureParams(a_rCaptureInfo);
    if (i4SensorDev & ESensorDev_Sub)
        AeMgr::getInstance(ESensorDev_Sub).getMShutterCaptureParams(a_rCaptureInfo);
    if (i4SensorDev & ESensorDev_MainSecond)
        AeMgr::getInstance(ESensorDev_MainSecond).getMShutterCaptureParams(a_rCaptureInfo);
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

MINT32 IAeMgr::getShutterValue(MINT32 i4SensorDev)
{
    if (i4SensorDev & ESensorDev_Main)
        return AeMgr::getInstance(ESensorDev_Main).getShutterValue();
    if (i4SensorDev & ESensorDev_Sub)
        return AeMgr::getInstance(ESensorDev_Sub).getShutterValue();
    if (i4SensorDev & ESensorDev_MainSecond)
        return AeMgr::getInstance(ESensorDev_MainSecond).getShutterValue();
    return MFALSE;
}
...
MBOOL IAeMgr::SaveAEMgrInfo(MINT32 i4SensorDev, const char * fname)
{
    ...
    MY_ERR("Err IAEMgr::SaveAEMgrInfo()\n");
