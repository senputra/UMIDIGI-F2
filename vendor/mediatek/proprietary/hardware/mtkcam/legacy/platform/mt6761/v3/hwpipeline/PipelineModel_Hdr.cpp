...
//#include <aaa_hal_common.h> mp0.mp9
//
...
#define SCENARIO_CONTROL_DONE       (0)
...
submitRequest(AppRequest& request)
{
...
#if ENABLE_HDRNODE
    // FIX ME
    CHECK_ERROR( HDRNode::getCaptureParamSet(getOpenId(), mvHdrCaptureParams) );
    //
    MERROR err = OK;
   for (size_t frm_idx = 0; frm_idx < mvHdrCaptureParams.size(); frm_idx++ ) {
        ...
        //
        CHECK_ERROR( evaluateRequestLocked(aParsedRequest, evaluateResult, frm_idx==0) );
        //
        CHECK_ERROR( refineRequestMetaStreamBuffersLocked(evaluateResult, frm_idx) );
        //
    ...
    builder.setNodeEdges( evaluateResult.edges );
        ...
        if (frm_idx==0) {
            ...
            {
                MY_LOGD("replace streaminfo id:%#" PRIx64, evaluateResult.vUpdatedImageInfos.keyAt(i));
        ...
        builder.setIOMap(
                ...
                IOMapSet().add(
                    IOMap()
                        ...
                        .addOut(eSTREAMID_IMAGE_PIPE_RAW_RESIZER_00)
                    ),
                IOMapSet().add(
                    IOMap()
                ...
                IOMapSet().add(
                    IOMap()
                        .addIn(eSTREAMID_IMAGE_PIPE_RAW_OPAQUE_00)
                            .addOut(mvAppYuvImage.editValueAt(0)->getStreamId())
                        .addOut(eSTREAMID_IMAGE_PIPE_YUV_JPEG_00)
            ...
            .setIOMap(
                ...
                IOMapSet().add(
                    IOMap()
                        .addIn(eSTREAMID_IMAGE_PIPE_YUV_JPEG_00)
                        .addIn(eSTREAMID_IMAGE_PIPE_YUV_THUMBNAIL_00)
                        .addOut(mpAppImage_Jpeg->getStreamId())
                    ),
                IOMapSet().add(
                    IOMap()
                        .addIn(mpAppMeta_Control->getStreamId())
...
evaluateRequestResult evaluateResult;
    //
    CHECK_ERROR( evaluateRequestLocked(aParsedRequest, evaluateResult, 1) );
    //
...
configContextLocked_P1Node(sp<PipelineContext> pContext)
{
    ...
    if( err != OK )
        MY_LOGE("build node %#" PRIx64 " failed", nodeId);
    return err;
}
...
configContextLocked_HdrNode(sp<PipelineContext> pContext)
{
    ...
    NodeT::ConfigParams cfgParam;
    {
        ...
        if( mpHalImage_P1_Raw.get() ) {
            ...
            cfgParam.pvInFullRaw.push_back(mpHalImage_P1_Raw);
#endif
            // cfgParam.pInResizedRaw = NULL; Marked by Derek
            cfgParam.pInResizedRaw = mpHalImage_P1_ResizerRaw;
        }
        else
            cfgParam.pInResizedRaw = mpHalImage_P1_ResizerRaw;
...
MERROR
PipelineHdrImp::
evaluateRequestLocked(parsedAppRequest const& request, evaluateRequestResult& result, MBOOL bMainReq)
{
    enum ePath
    {
        eImagePathP1,
// Use P2 enum for hdr case
#if ENABLE_HDRNODE
    ...
    //
    {
        ...
        if( pBuf.get() )
        {
            ...
            // update AE capture parameters
            {
                ...
                memcpy(capParams.editArray(), &mvHdrCaptureParams.itemAt(frameIndex),
...
{
    MY_LOGD_IF( 1, "frameNo:%u + userId:%#" PRIx64 " OAppMeta#(left:%zd this:%zu)",
