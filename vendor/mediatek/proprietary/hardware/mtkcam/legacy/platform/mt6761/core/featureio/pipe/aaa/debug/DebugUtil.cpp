...
getInstance(int i4SensorDevId)
{
    switch (i4SensorDevId)
    {
        ...
        default:
            MY_LOG("Unsupport sensor device ID: %d\n", i4SensorDevId);
            return NULL;
    }
}
...
dump()
{
    ...
    for(it = mMap.begin(); it != mMap.end(); ++it){
        MY_LOG(" (%d) module : %s +", m_i4SensorDevId, it->first.c_str());
        it->second.dump();
        MY_LOG(" (%d) module : %s -", m_i4SensorDevId, it->first.c_str());
    }
}
...
{
    int enable = ::property_get_int32("debug.perframe3a.detect", 0);
    if(!enable)
        return;
    ...
    {
        case DBG_AE:
            MY_LOG("[%s] AE i4CurFrame(%d), m_i4detectAECnt(%d)", __FUNCTION__, i4CurFrame,m_i4detectAECnt);
            ...
            break;
        case DBG_AF:
            MY_LOG("[%s] AF i4CurFrame(%d), m_i4detectAFCnt(%d)", __FUNCTION__, i4CurFrame,m_i4detectAFCnt);
...
getDebugLevel(DEBUG_MODULE module)
{
    ...
    {
        case DBG_3A:
            return ::property_get_int32("debug.3A_level.enable", 0);
        default:
            return 0;
    }
}
