...
if (ENABLE_AFOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    // State transition: eState_TAF --> eState_Uninit
...
if (ENABLE_AF & m_pHal3A->m_3ACtrlEnable) {
    // AF Stop
    MY_LOG("[%s] StateTAF::sendIntent(intent2type<eIntent_CameraPreviewEnd>) mbAFUninit(%d),m_bKeepLastStatus(%d)",__FUNCTION__,m_pHal3A->mbAFUninit,m_pHal3A->m_bKeepLastStatus);
if(!m_pHal3A->mbAFUninit)
{
    if(m_pHal3A->m_bKeepLastStatus){
        MY_LOG("m_bKeepLastStatus = %d\n", m_pHal3A->m_bKeepLastStatus);
        // Keep last status due to capture behavior
        err = IAfMgr::getInstance().Stop(m_SensorDevId); // Don't uninit algo
    }
    else
    err = IAfMgr::getInstance().Stop(m_SensorDevId);
    if (FAILED(err)) {
...
if (ENABLE_AFOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    // State transition: eState_TAF --> eState_Init
...
//{
    ...
    rAEInfo.i4IsAELocked= IAeMgr::getInstance().IsAELock(m_SensorDevId);
    rAEInfo.i4ISO=rPreviewInfo.u4RealISO;
    rAEInfo.u4AfeGain=rPreviewInfo.u4AfeGain;
...
//}//isFocusFinish end
//else
//    MY_LOG_IF(m_pHal3A->m_3ALogEnable & EN_3A_FLOW_LOG, "[%s] isFocusFinish(%d)", __FUNCTION__, isFocusFinish);
}
if (ENABLE_AFOBUF & m_pHal3A->m_3ACtrlEnable) {
