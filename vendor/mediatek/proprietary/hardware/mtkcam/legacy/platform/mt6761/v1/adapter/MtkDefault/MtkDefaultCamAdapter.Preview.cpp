...
previewEnabled() const
{
    ...
    bool isRecord = false;
    {
        ...
        {
            isPreview = mpStateManager->isState(IState::eState_Preview)||
                        mpStateManager->isState(IState::eState_PreviewCapture);
        }
    }
    isRecord = recordingEnabled();
    return isPreview || isRecord;
}
...
onHandleStartPreview()
{
    ...
    if(mName == MtkCameraParameters::APP_MODE_NAME_MTK_ZSD)
    {
        ...
        {
            SensorStaticInfo const*const pSensorStaticInfo = pHalSensorList->querySensorStaticInfo(getOpenId());
            MUINT32 u4RawFmtType = pSensorStaticInfo->rawFmtType;
            if (u4RawFmtType == SENSOR_RAW_4CELL || u4RawFmtType == SENSOR_RAW_4CELL_BAYER || u4RawFmtType == SENSOR_RAW_4CELL_HW_BAYER)
                sensorScenario = SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
        }
    ...
    if(mpPass2Node)
    {
        ...
        //
        MINT32 SensorDev = (MINT32)pHalSensorList->querySensorDevIdx(getOpenId());
        pHalSensorList->querySensorStaticInfo(SensorDev,&sensorStaticInfo);
        ...
        {
            sp<IPlugProcessing> pPlugProcess = IPlugProcessing::createInstance((MUINT32)IPlugProcessing::PLUG_ID_FCELL, (IPlugProcessing::DEV_ID)getOpenId());
    ...
    if(bConnectFullRaw)
    {
        ...
        if (bPdafMode)
        {
            ...
            mpCamGraph->connectData(     CONTROL_CAP_SRC,    PDAF_CAP_SRC, mpDefaultCtrlNode,        mpPdafNode);
        }
    }
    //
    ...
    if ( !mpCamGraph->start() ) {
        ...
        doNotifyCallback(CAMERA_MSG_ERROR, CAMERA_ERROR_SERVER_DIED, 0);
