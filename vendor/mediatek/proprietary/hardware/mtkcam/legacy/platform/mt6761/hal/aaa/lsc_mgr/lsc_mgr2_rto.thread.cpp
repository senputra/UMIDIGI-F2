...
initThreadFunc()
{
    ...
    else
    {
        ...
        m_rTsfRefInfo.prAwbStat = new MUINT16[m_rTsfRefInfo.u4SizeAwbStat];
        m_prAwbStat = new MUINT16[m_rTsfRefInfo.u4SizeAwbStat];
...
tsfSetProcInfo(MTK_TSF_SET_PROC_INFO_STRUCT& rProcInfo, MBOOL fgDump, const char* filename)
{
    ...
    // AWB stat
    const MUINT16* pSrc = m_rTsfRefInfo.prAwbStat;
    ...
    if (fgDump)
    {
        ...
        if (fptr)
        {
            fwrite(pSrc, m_rTsfRefInfo.u4SizeAwbStat, 2, fptr);
