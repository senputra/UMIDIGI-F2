 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (unsigned long)(va) - (unsigned long)(bva) ) + (unsigned long)(bpa) )
    ...
    if ( 0 == (cam_tg_vf & 0x01) ) {
        ...
        if ( _imgo_ == pbuf_ctrl->buf_id || _imgo_d_ == pbuf_ctrl->buf_id) {
            ...
            ISP_WRITE_BITS(this,CAMSV_IMGO_FBC,FB_NUM,size,ISP_DRV_USER_ISPF);
            ISP_WRITE_BITS(this,CAMSV_IMGO_FBC,FBC_EN,1,ISP_DRV_USER_ISPF);
        }
        else if ( _camsv2_imgo_ == pbuf_ctrl->buf_id ) {
            pcqrtbcring_va->imgo_ring_size++;
            size = pcqrtbcring_va->imgo_ring_size;

            ISP_WRITE_BITS(this,CAMSV2_IMGO_FBC,FB_NUM,size,ISP_DRV_USER_ISPF);
            ISP_WRITE_BITS(this,CAMSV2_IMGO_FBC,FBC_EN,1,ISP_DRV_USER_ISPF);
        }
        //
