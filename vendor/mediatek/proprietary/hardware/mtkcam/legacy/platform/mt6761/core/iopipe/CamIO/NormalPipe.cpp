...
MBOOL
NormalPipe::_setTgPixelAndDualMode(MUINT32 ScenId, MUINT32 fps)
{
    ...
    if (mpHalSensor) {
        ...
        CAM_LOGE("Error: mpHalSensor is null");
        return MFALSE;
    }
...
MBOOL
NormalPipe::get_isExecutable(qryExecIn qryIn, MBOOL &bIsExe)
{
    MBOOL ret = MTRUE;
    MUINT32 i = 0, NextTG = 0, OccupiedNum = 0;

    CAM_LOGI("[get_isExecutable]+ SenId(%d)", mpSensorIdx[0]);

    /*Note: avoid race condition, check previous Normalpipe is configpipe done.*/
    for (i = 0; i < (MUINT32)NPIPE_Sensor_RSVD; i++) {
        if (i == mpSensorIdx[0]) {
            continue;
        }
        if (pNormalPipe[i]) {
            if (MFALSE == pNormalPipe[i]->mConfigDone){
                CAM_LOGE("[get_isExecutable] previous Normalpipe[%d] is not config done", i);
                ret = MFALSE;
                goto EXIT;
            }
        }
    ...
    if (!mpHalSensor) {
        mpHalSensor = pAllSensorInfo->mSList->createSensor(mpName, mpEnabledSensorCnt, mpSensorIdx);
        if (!mpHalSensor) {
            CAM_LOGE("[get_isExecutable] mpHalSensor Fail");
            ret = MFALSE;
            goto EXIT;
        }
    }

    for (i = 0; i < (MUINT32)NPIPE_MAX_SENSOR_CNT; i++) {
    ...
    if (NPIPE_MAX_ENABLE_SENSOR_CNT > NextTG) {
        mImgoOut_size = qryIn.imgoSize;
        mRrzoOut_size = qryIn.rrzoSize;

        NormalPipeGLock();
        if (!_setTgPixelAndDualMode(qryIn.ScenId, qryIn.fps)) {
            CAM_LOGE("[get_isExecutable] Error: determine 2PixelInfo fail");
            NormalPipeGUnLock();
            ret = MFALSE;
            goto EXIT;
        }
        ...
        for(i = (MUINT32)CAM_TG_1; i < (MUINT32)CAM_TG_2; i++){
            switch(mTwinEnable[i][1]) {
            ...
            case ePxlMode_Two_Twin:
                ...
                break;
            default:
                CAM_LOGE("[get_isExecutable] Error: undefine status(%d)", mTwinEnable[i][1]);
                break;
            }
        }

        bIsExe = (OccupiedNum > NPIPE_MAX_ENABLE_SENSOR_CNT) ? MFALSE : MTRUE;

        CAM_LOGI("[get_isExecutable] SenIdx(%d) ScenId(%d) fps(%d) OccupiedNum(%d) imgo(%d/%d) rrzo(%d/%d)",
            mpSensorIdx[0], qryIn.ScenId, qryIn.fps, OccupiedNum, qryIn.imgoSize.w, qryIn.imgoSize.h, qryIn.rrzoSize.w, qryIn.rrzoSize.h);
    }
    else {
        bIsExe = MFALSE;
    }
...
EXIT:
    if (!bIsExe) {
        CAM_LOGW("[get_isExecutable] resource not enough. NextTG(%d) OccupiedNum(%d).", NextTG, OccupiedNum);
    }
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    switch (cmd) {
       ...
       case EPIPECmd_GET_HBIN_INFO:
           ...
           CAM_LOGD("EPIPECmd_GET_HBIN_INFO SIdx(%d),*arg3=%d,hbin_en=%d w=0x%x, h=0x%x!",arg1, hbin_en, hbin->mEnabled,hbin->size.w, hbin->size.h);
           break;
       case EPIPECmd_GET_HBIN1_INFO:
           //arg1 : sensor-idx
           //arg2 : NormalPipe_HBIN_Info
           if ((2 < arg1) || (NULL == arg2)) {
                CAM_LOGE("EPIPECmd_GET_HBIN1_INFO Err: Invalid Para arg1(%d), arg2=0x%x!",arg1, arg2);
               return MFALSE;
           }
           tg = _NORMALPIPE_GET_TG_IDX(arg1);
           CAM_LOGD("EPIPECmd_GET_HBIN1_INFO SIdx(%d), BinInfoAddr=0x%x",arg1, arg2);
           ...
           switch (tg) {
               ...
               case CAM_TG_2:
                   ...
                   break;
               default:
                   CAM_LOGE("EPIPECmd_GET_HBIN1_INFO Err: Invalid SensorIdx(%d)!",arg1);
                   return MFALSE;
           }
           CAM_LOGD("EPIPECmd_GET_HBIN1_INFO SIdx(%d),*arg3=%d,hbin_en=%d w=0x%x, h=0x%x!",arg1, hbin_en, hbin->mEnabled,hbin->size.w, hbin->size.h);
           break;
