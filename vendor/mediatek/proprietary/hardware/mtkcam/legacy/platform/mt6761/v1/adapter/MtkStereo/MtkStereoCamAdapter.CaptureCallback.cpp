...
)
{
    ...
    // only check CAMERA_MSG_COMPRESSED_IMAGE message when callback CAMERA_MSG_COMPRESSED_IMAGE
    ...
    //
    if ( pThread->isExtCallback() && (msgType!=MTK_CAMERA_MSG_EXT_DATA_COMPRESSED_IMAGE))
    {
    ...
    if  ( image )
    {
        ...
        //
        ZipImageCallbackThread::callback_type CallbackType=
            (msgType==MTK_CAMERA_MSG_EXT_DATA_COMPRESSED_IMAGE)?
                ZipImageCallbackThread::callback_type_jpeg:        // use CAMERA_MSG_COMPRESSED_IMAGE(256) to call back
                ZipImageCallbackThread::callback_type_ext_data;    // use MTK_CAMERA_MSG_EXT_DATA(536870912) to call back
        ZipImageCallbackThread::callback_data callbackData =
        {
            CallbackType,
            image,
        ...
        };
        {
            ...
            pThread->addCallbackData(&callbackData);
            pThread->doCallback((MUINT32)CallbackType);
        }
        pThread = 0;
    }
    //
