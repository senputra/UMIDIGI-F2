...
*
*******************************************************************************/
#define MY_LOGV(fmt, arg...)        CAM_LOGV("[ZipImageCallbackThread] [%s]" fmt, __FUNCTION__, ##arg)
#define MY_LOGD(fmt, arg...)        CAM_LOGD("[ZipImageCallbackThread] [%s]" fmt, __FUNCTION__, ##arg)
#define MY_LOGI(fmt, arg...)        CAM_LOGI("[ZipImageCallbackThread] [%s]" fmt, __FUNCTION__, ##arg)
#define MY_LOGW(fmt, arg...)        CAM_LOGW("[ZipImageCallbackThread] [%s]" fmt, __FUNCTION__, ##arg)
#define MY_LOGE(fmt, arg...)        CAM_LOGE("[ZipImageCallbackThread] [%s]" fmt, __FUNCTION__, ##arg)
#define MY_LOGA(fmt, arg...)        CAM_LOGA("[ZipImageCallbackThread] [%s]" fmt, __FUNCTION__, ##arg)
#define MY_LOGF(fmt, arg...)        CAM_LOGF("[ZipImageCallbackThread] [%s]" fmt, __FUNCTION__, ##arg)
//
        ...
        case callback_type_jpeg:
            {
                ...
                //
                return handleJpegCallback(one, true);   // force to callback CAMERA_MSG_COMPRESSED_IMAGE
            }
        case callback_type_ext_data:
            {
                callback_data one;
                //
                if( !getCallbackData(callback_type_ext_data, &one) )
                {
                    break;
                }
                //
                return handleJpegCallback(one, false);
            }
        default:
...
bool
ZipImageCallbackThread::
handleJpegCallback(callback_data const& data, bool bForceToDefaultCallBack)
{
    Mutex::Autolock _l(mLock);
    MY_LOGD("mCBDoneCnt=%d, bForceToDefaultCallBack=%d", mCBDoneCnt, bForceToDefaultCallBack);
    ...
    if  ( mCBDoneCnt == 2 )
    {
        ...
        {
            MY_LOGW("do nothing due to fail to wait - Index:%d ShotMode:%d",
                    data.u4CallbackIndex, mu4ShotMode);
            return  false;
        }
    }

    if( mIsExtCallback && (bForceToDefaultCallBack==false))
    {
