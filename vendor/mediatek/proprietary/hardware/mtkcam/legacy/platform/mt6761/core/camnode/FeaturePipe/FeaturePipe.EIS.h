...
public:
    ...
    virtual MVOID removeQueuedInfos();

    virtual MBOOL set(FeaturePipeSetType setType, const MINTPTR setData);

    MVOID startVideoRecord();
    MVOID notifyRecordStopping();
    MVOID stopVideoRecord();
    MVOID applyEISConfig(EIS_HAL_CONFIG_DATA &config, MINT32 algMode, MINT32 algCounter);
...
private:
    ...
    // EIS 3.0 params
    MUINT32         mFrameCount;
    MUINT32         mAlgoCounter;
    MUINT32         mNeedQCount;
    MBOOL           mIsRecStarted; // become true when app really called startRecord()
    MBOOL           mIsRecStopNotified; // become true when app prepare to stop record
    queue<FeaturePipeNodeImgReqJob_s> mqJobWarpMap; // test use
