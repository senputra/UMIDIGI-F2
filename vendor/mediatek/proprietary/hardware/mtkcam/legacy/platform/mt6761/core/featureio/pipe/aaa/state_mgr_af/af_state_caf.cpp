...
if (ENABLE_AFOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    // State transition: eState_CAF --> eState_Uninit
...
if (ENABLE_AF & m_pHal3A->m_3ACtrlEnable) {
    // AF Stop
    MY_LOG("[%s] StateCAF::sendIntent(intent2type<eIntent_CameraPreviewEnd>) mbAFUninit(%d),m_bKeepLastStatus(%d)",__FUNCTION__,m_pHal3A->mbAFUninit,m_pHal3A->m_bKeepLastStatus);
if(!m_pHal3A->mbAFUninit)
{
    if(m_pHal3A->m_bKeepLastStatus){
        ...
        err = IAfMgr::getInstance().Stop(m_SensorDevId); // Don't unint algo
    }
    else
    err = IAfMgr::getInstance().Stop(m_SensorDevId);
    if (FAILED(err)) {
...
if (ENABLE_AFOBUF & m_pHal3A->m_3ACtrlEnable) {
    ...
    // State transition: eState_CAF --> eState_Init
