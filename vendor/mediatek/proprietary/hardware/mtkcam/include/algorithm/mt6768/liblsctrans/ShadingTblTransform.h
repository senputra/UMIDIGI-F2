...
#define SHADINGTBLTRANSFORM_REVISION 6254001
...
typedef struct
{
    unsigned char  in_nNumChannel;
    unsigned short in_nShadeTabHgt;
    unsigned short in_nShadeTabWid;
    float *out_pShadeTable;
}LSC_FLOAT_TBL_STRUCT;
...
//LSC_RESULT shading_transform_float_Gr(LSC_RA_STRUCT *ra_config, LSC_FLOAT_TBL_STRUCT *p_out_float_tbl);
void shading_gain_float_gain(LSC_FLOAT_TBL_STRUCT *ptOut_float_tbl, int *a_pdGain, int a_dChannel);
LSC_RESULT shading_transform_new(SHADIND_TRFM_CONF shading_conf);
LSC_RESULT shading_transform_new(SHADIND_TRFM_CONF_PSO shading_conf);
LSC_RESULT shading_transform(SHADIND_TRFM_CONF shading_conf);
LSC_RESULT shading_align_golden(SHADIND_ALIGN_CONF trans);
LSC_RESULT shading_merge_tbl(int *working_buf_transform, int *working_buf_merge, int *hwtbl_use_RGBG, int *hwtbl_use_G, int *hwtbl_merge, LSC_PARAM_T lsc_config);
void shading_merge_tbl_core(unsigned int *ptr_tbl_grgb, unsigned int *ptr_tbl_g, unsigned *ptr_tbl_out, int x_grid_num, int y_grid_num);
