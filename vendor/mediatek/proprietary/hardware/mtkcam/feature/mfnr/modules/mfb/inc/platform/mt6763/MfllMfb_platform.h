...
#if ((MFLL_MF_TAG_VERSION > 0) && (MFLL_MF_TAG_VERSION != 7))
#   error "MT6763 only supports MF_TAG_VERSION 7"
#endif
...
// {{{
    ...
    case eMfllIspProfile_BeforeBlend_Zsd_Swnr:
        ...
        default:;
            auto customHint = getCustomHint(mode);
            mfllLogD("%s: profile(mode:0x%X, customHint:%d)", __FUNCTION__, mode, customHint);
            switch (customHint) {
            ...
            mfllLogD("%s: use MFNR default", __FUNCTION__);
        }
