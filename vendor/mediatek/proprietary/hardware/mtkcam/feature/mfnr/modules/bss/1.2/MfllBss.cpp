...
#   endif
    ...
    /* main process */
    {
        ...
        /* set fd info */
        {
            ...
            vector<MTKBSSFDInfo> bss_fddata;
                ...
                if (CC_LIKELY( fdData.size() == size )) {
                    for (size_t idx = 0 ; idx < fdData.size() ; idx++) {
                        if (fdData[idx] != nullptr) {
                            ...
                            inParam.Face[idx] = &bss_fddata[idx].facedata;
                        }
                        else
            ...
            // 4. fill fd rect0 to exif
            {
                for (size_t i = 0; i < fdData.size(); i++) {
                    if (fdData[i] != nullptr) {
                        m_pCore->updateExifInfo(MF_TAG_FD_RECT0_X0_00 + i*MFLLBSS_FD_RECT0_PER_FRAME, (uint32_t)fdData[i]->facedata.faces[0].rect[0]);
                        m_pCore->updateExifInfo(MF_TAG_FD_RECT0_Y0_00 + i*MFLLBSS_FD_RECT0_PER_FRAME, (uint32_t)fdData[i]->facedata.faces[0].rect[1]);
                        m_pCore->updateExifInfo(MF_TAG_FD_RECT0_X1_00 + i*MFLLBSS_FD_RECT0_PER_FRAME, (uint32_t)fdData[i]->facedata.faces[0].rect[2]);
                        m_pCore->updateExifInfo(MF_TAG_FD_RECT0_Y1_00 + i*MFLLBSS_FD_RECT0_PER_FRAME, (uint32_t)fdData[i]->facedata.faces[0].rect[3]);
                    }
                }
            }
...
#define MFLLBSS_WRITE_ARRAY_2D_TO_FILE(pre, array, M, N) \
        ...
        for (int f = 0 ; f < MAX_FRAME_NUM ; f++) {
            ...
            BssFaceMetadata* facedata = inParam.Face[f];
