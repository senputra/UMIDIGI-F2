...
// (app1 for thumbnail and stdexif + 4 appn for dbgexif)
...
updateData(IMetadata &rMetadata)
{
...

    {
        ...
        for(size_t i = 0; i < blobEntry.count(); i+=4) {
            ...
            // *2*0.7 + exif max header size
            MINT32 jpegsize = (jpegYuv420Support == 1) ? (maxBlob.size()*15)/10 + EXIF_H_SIZE : maxBlob.size()*2 + EXIF_H_SIZE;
