...
#define LOG_QUERY_DBG       (0x1u << 2)
#define LOG_QUERY_CALLER    (0x1u << 3)

#define IS_LOG_BASIC    (s_bDebugEnable & LOG_QUERY_BASIC)
#define IS_LOG_FULL     (s_bDebugEnable & LOG_QUERY_FULL)
#define IS_LOG_DBG      (s_bDebugEnable & LOG_QUERY_DBG)
#define IS_LOG_CALLER   (s_bDebugEnable & LOG_QUERY_CALLER)
...
static MBOOL s_bDumpDebugEnable =  MFALSE;
...
void IdxBase::mismatchHandling(EModule_T mod, const CAM_IDX_QRY_COMB& qry)
{
    if (s_bDebugEnable || s_bAeeEnable)
    {
        IDX_LOGE_IF(IS_LOG_FULL,
            ...
            strEModule[mod], mod, strEIspProfile[qry.eIspProfile], strESensorMode[qry.eSensorMode], qry.eFrontBin, qry.eSize, qry.eFlash, strEApp[qry.eApp], qry.eFaceDetection, qry.eZoom_Idx, qry.eLV_Idx, qry.eCT_Idx, qry.eISO_Idx[0]);
...
MVOID IdxMask::init(EModule_T mod, IDX_BASE_T* pData, MUINT16* pFactorNs)
{
    ...
    IDX_LOGD_IF(IS_LOG_BASIC, "IdxMask::init() for mod[%s]", strEModule[m_mod]);
    ...
    expand = pMask->base.expand;
    entry_ns = pMask->data.entry_ns;
    key_sz = pMask->data.key_sz;
    entry = pMask->data.entry;

    IDX_LOGD_IF(IS_LOG_DBG, "IdxMask init [Mod:%s] dim_ns %p, dims %p, expand %p", strEModule[m_mod], dim_ns, dims, expand);
    if (expand)
    {
        for (int i = 0; i < 3; i++)
            IDX_LOGD_IF(IS_LOG_DBG, "[Mod:%s] expand[%d] == %d", strEModule[m_mod], i, expand[i]);
    }
...
IDXMGR_QUERY_RESULT IdxMask::query(const CAM_IDX_QRY_COMB& qry)
{
    ...
    IDXMGR_QUERY_RESULT rtnVal = {0, INVALID_SCENARIO, NULL, 0, 0};
    for (i = 0; i < entry_ns; i++)
    {
        if (compareKey(key, entry[i].key))
        {
            if ((entry[i].rule_type == 1) && (expand)) //match LV rule
            {
                rtnVal.idx = entry[i].value;
                rtnVal.scenario = entry[i].scenario;
                rtnVal.group = entry[i].iso_group;
                rtnVal.base = entry[i].value;

                IDX_LOGD_IF(IS_LOG_DBG, "IdxMask(query) [Mod:%s] base %d, result %d [idx (LV %d CT %d ISO %d)] [expand (LV %d CT %d ISO %d)] iso_group(%d)",
                    strEModule[m_mod], entry[i].value, rtnVal.idx, qry.eLV_Idx, qry.eCT_Idx, qry.eISO_Idx[entry[i].iso_group], expand[0], expand[1], expand[2], entry[i].iso_group);
            }
            else if (expand)
            {
                rtnVal.idx = entry[i].value + qry.eLV_Idx * expand[0] + qry.eCT_Idx * expand[1] + qry.eISO_Idx[entry[i].iso_group] * expand[2];
                rtnVal.scenario = entry[i].scenario;
                rtnVal.group = entry[i].iso_group;
                rtnVal.base = entry[i].value;
            ...
            {
                rtnVal.idx = 0;
                rtnVal.scenario = entry[0].scenario;

                IDX_LOGE_IF(IS_LOG_DBG, "IdxMask(query) [Mod:%s] expand info missing", strEModule[m_mod]);
            }
            return rtnVal;
        }
    }
...
IdxDM::IdxDM()
    : m_mod(EModule_NUM), dim_ns(0), acc_factor_ns(NULL), dims(NULL), idx_array(NULL), scenarios(NULL), expand(NULL)
{
}
...
MVOID IdxDM::init(EModule_T mod, IDX_BASE_T* pData, MUINT16* pFactorNs)
{
    ...
    IDX_LOGD_IF(IS_LOG_BASIC, "IdxDM::init() for mod[%s]", strEModule[m_mod]);
    ...
    if (dim_ns)
    {
        ...
        for (MINT32 i = 0; i < dim_ns; i++)
        {
            ...
            IDX_LOGD_IF(IS_LOG_BASIC, "IdxDM::init() [Mod:%s] acc_factor_ns[%d]: %d", strEModule[m_mod], i, acc_factor_ns[i]);
        }
    }
}
...
IDXMGR_QUERY_RESULT IdxDM::query(const CAM_IDX_QRY_COMB& qry)
{
    ...
    if (idx_array[idx] == (MUINT16)(-1))
    {
        ...
        IDX_LOGE_IF(IS_LOG_DBG, "IdxDM(query) [Mod:%s] expand info missing", strEModule[m_mod]);
    }
    else
    {
/*
        if (expand)
        {
            rtnVal.idx = idx_array[idx] + qry.eLV_Idx * expand[0] + qry.eCT_Idx * expand[1] + qry.eISO_Idx * expand[2];
            rtnVal.scenario = scenarios[idx];

            IDX_LOGD_IF(IS_LOG_DBG, "IdxDM(query) [Mod:%s] base %d, result %d [idx (LV %d CT %d ISO %d)] [expand (LV %d CT %d ISO %d)]",
                strEModule[m_mod], idx_array[idx], rtnVal.idx, qry.eLV_Idx, qry.eCT_Idx, qry.eISO_Idx, expand[0], expand[1], expand[2]);
        }
        else*/
        {
            rtnVal.idx = 0;
            rtnVal.scenario = scenarios[0];
...
IdxMgr* IdxMgr::createInstance(MUINT32 eSensorDev)
{
    ...
    s_bDumpDebugEnable = property_get_int32("vendor.debug.atms.dump", 0);
...
IdxMgr::IdxMgr() : m_pIdxMgrImpBase {0},
                   m_flag           {0}
{}
...
{
    IDXMGR_QUERY_RESULT rtnVal = {0, 0, NULL, 0};
    ...
    if (m_pIdxMgrImpBase[idx])
    {
        ...
        IDX_LOGD_IF(IS_LOG_BASIC,
            ...
            eSensorDev, strEModule[mod], mod, rtnVal.idx, rtnVal.pScenario, rtnVal.group);

        IDX_LOGD_IF(IS_LOG_FULL,
        "[Dev:%d-Mod:%s(%d)] (Idx %d) (%s) (PF %s, SM %s, Bin %d, P2 %d, FLASH %d, APP %s, FD %d, ZOOM %d, LV %d, CT %d, ISO %d,)",
        eSensorDev, strEModule[mod], mod, rtnVal.idx, rtnVal.pScenario, strEIspProfile[info.eIspProfile], strESensorMode[info.eSensorMode], info.eFrontBin, info.eSize, info.eFlash, strEApp[info.eApp], info.eFaceDetection, info.eZoom_Idx, info.eLV_Idx, info.eCT_Idx, info.eISO_Idx[rtnVal.group]);
        if (s_bDumpDebugEnable){
            char str_dump[500];
            sprintf(str_dump, "[MappingMgr] [Dev:%d-Mod:%s(%d)] (Idx %d) (%s) (PF %s, SM %s, Bin %d, P2 %d, FLASH %d, APP %s, FD %d, ZOOM %d, LV %d, CT %d, ISO %d,) \n",
        eSensorDev, strEModule[mod], mod, rtnVal.idx, rtnVal.pScenario, strEIspProfile[info.eIspProfile], strESensorMode[info.eSensorMode], info.eFrontBin, info.eSize, info.eFlash, strEApp[info.eApp], info.eFaceDetection, info.eZoom_Idx, info.eLV_Idx, info.eCT_Idx, info.eISO_Idx[rtnVal.group]);
...
{
    IDXMGR_QUERY_RESULT rtnVal = {0, 0, NULL, 0, 0};
    ...
    if (m_pIdxMgrImpBase[idx])
    {
        ...
        IDX_LOGD_IF(IS_LOG_BASIC,
            "[Caller: %s] [Dev:%d-Mod:%s(%d)] (Idx %d) (%s)",
            caller, eSensorDev, strEModule[mod], mod, rtnVal.idx, rtnVal.pScenario, rtnVal.group);

        IDX_LOGD_IF(IS_LOG_FULL,
            "[Caller: %s] [Dev:%d-Mod:%s(%d)] (Idx %d) (%s) (PF %s, SM %s, Bin %d, P2 %d, FLASH %d, APP %s, FD %d, ZOOM %d, LV %d, CT %d, ISO %d, ISO_GROUP %d) (Base_Idx %d)",
            caller, eSensorDev, strEModule[mod], mod, rtnVal.idx, rtnVal.pScenario, strEIspProfile[qry.eIspProfile], strESensorMode[qry.eSensorMode], qry.eFrontBin,
            qry.eSize, qry.eFlash, strEApp[qry.eApp], qry.eFaceDetection, qry.eZoom_Idx, qry.eLV_Idx, qry.eCT_Idx, qry.eISO_Idx[rtnVal.group], rtnVal.group, rtnVal.base);

        if (s_bDumpDebugEnable){
            char str_dump[500];
            sprintf(str_dump, "[MappingMgr] [Caller: %s] [Dev:%d-Mod:%s(%d)] (Idx %d) (%s) (PF %s, SM %s, Bin %d, P2 %d, FLASH %d, APP %s, FD %d, ZOOM %d, LV %d, CT %d, ISO %d, ISO_GROUP %d) \n",
                caller, eSensorDev, strEModule[mod], mod, rtnVal.idx, rtnVal.pScenario, strEIspProfile[qry.eIspProfile], strESensorMode[qry.eSensorMode], qry.eFrontBin,
                qry.eSize, qry.eFlash, strEApp[qry.eApp], qry.eFaceDetection, qry.eZoom_Idx, qry.eLV_Idx, qry.eCT_Idx, qry.eISO_Idx[rtnVal.group], rtnVal.group);
            FILE *pFile = fopen("/sdcard/ATMs.log", "a");
            if (pFile){
                fwrite(str_dump, strlen(str_dump), 1, pFile);
                fclose(pFile);
        ...
        if (IS_LOG_CALLER)
        {
            android::CallStack callStack("IdxMgr Query", 0);
        }
    }
    else
    {
    ...
    result.group = rtnVal.group;
    result.base = rtnVal.base;
...
{
    return query(eSensorDev, mod, qry, result, __FUNCTION__);
}
...
MUINT16 IdxMgr::query(MUINT32 eSensorDev, EModule_T mod, const CAM_IDX_QRY_COMB& qry, const char* caller)
{
    ...
    return query(eSensorDev, mod, qry, result, caller);
}

MUINT16 IdxMgr::query(MUINT32 eSensorDev, EModule_T mod, const CAM_IDX_QRY_COMB& qry, MUINT16& u2ScenarioIdx){
    return 0; //not surpport
}

MUINT32 IdxMgr::addMappingFunction(MUINT32 eSensorDev, MUINT32 dim, MUINT32 (IspTuningCustom::*function)(MUINT32, MVOID*), IspTuningCustom& tuning){
    MUINT32 ret = 0;
    IDX_TRACE_BEGIN("IdxMgr_AddMappingFunction");
    ...
    if (m_pIdxMgrImpBase[idx])
    {
        ret = m_pIdxMgrImpBase[idx]->addMappingFunction(dim, function, tuning);
    }
    else
    {
...
MVOID IdxMgr::setMappingInfoByDim(MUINT32 eSensorDev, MUINT32 FrmId, MUINT32 dim, MVOID* value)
{

    IDX_TRACE_BEGIN("IdxMgr_setInfo_by_dim");
    ...
    if (m_pIdxMgrImpBase[idx])
    {
        m_pIdxMgrImpBase[idx]->updateMapping_List_By_Dim(FrmId, dim, value);
    }
    else
    {
        IDX_LOGE("[Dev:%d] Idx_Mgr Not initialized", eSensorDev);
...
MVOID IdxMgr::setValidFlag(NSIspTuning::ESensorDev_T eSensorDev)
{
    IDX_LOGI("No need to check nvram valid for ATMs 2.0");
    return;
}
