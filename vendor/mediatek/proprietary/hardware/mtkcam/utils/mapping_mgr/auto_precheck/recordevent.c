...
#define PROP_TRIGGER_MAPPING_MGR "vendor.debug.trigger_mapping_mgr.enable"
...
static volatile int triggerOn = 0;
static volatile int isToStop_threadGetProperty = 0;
#define MAXSIZE 1024
pthread_t tid;
FILE *fp;
struct timeval firstEventTime, enableLogTime, disableLogTime;
...
static int read_notify(const char *dirname, int nfd, int print_flags)
{
    ...
    strncpy(devname, dirname, PATH_MAX - 1);
    ...
    *filename++ = '/';
    ...
    while(res >= (int)sizeof(*event)) {
        ...
        if(event->len) {
            strncpy(filename, event->name, PATH_MAX - 2 - strlen(dirname));
    ...
    while((de = readdir(dir))) {
        if(de->d_name[0] == '.' &&
           (de->d_name[1] == '\0' ||
            ...
            continue;
        strncpy(filename, de->d_name, PATH_MAX - 2 - strlen(dirname));
        open_device(devname, print_flags);
    }
    closedir(dir);
    return 0;
...
static void *threadGetProperty(){
    char value[PROPERTY_VALUE_MAX];
    while(1){
        usleep(500000);
        property_get(PROP_TRIGGER_MAPPING_MGR, value, "0");
        triggerOn = atoi(value);
        // printf("TriggerOn: %d, Logging: %d\n", triggerOn, logging);

        // Log Start/End Event
        if(triggerOn == 1 && logging == 0){
            logging = 1;
            gettimeofday(&enableLogTime, NULL);
            int ret = property_set(PROP_MAPPING_MGR, START_LOG_VALUE);
            char *set_result = (ret == 0) ? "Succeed in setting property on." : "Fail to set property.";
            // printf("%s\n", set_result);
            int realEnableTime, tv_sec, tv_usec;
            if(firstEventTime.tv_sec == 0){
                tv_sec = 0;
                tv_usec = 0;
            }
            else{
                realEnableTime = (enableLogTime.tv_sec * 1000000LL + enableLogTime.tv_usec) - (firstEventTime.tv_sec * 1000000LL + firstEventTime.tv_usec);
                tv_sec = realEnableTime / 1000000;
                tv_usec = realEnableTime - tv_sec * 1000000;
            }
            int time_device = (tv_sec << 23) | (tv_usec << 3) | 0;
            int max_int = 0xFFFFFFFF;
            // Write Event to record trigger
            fwrite(&time_device, sizeof(int), 1, fp);
            fwrite(&max_int, sizeof(int), 1, fp);
            fwrite(&max_int, sizeof(int), 1, fp);
            fflush(fp);
        }
        else if(triggerOn == 0 && logging == 1){
            logging = 0;
            gettimeofday(&disableLogTime, NULL);
            int ret = property_set(PROP_MAPPING_MGR, END_LOG_VALUE);
            char *set_result = (ret == 0) ? "Succeed in setting property off." : "Fail to set property.";
            // printf("%s\n", set_result);
            int realDisableTime, tv_sec, tv_usec;
            if(firstEventTime.tv_sec == 0){
                tv_sec = 0;
                tv_usec = 0;
            }
            else{
                realDisableTime = (disableLogTime.tv_sec * 1000000LL + disableLogTime.tv_usec) - (firstEventTime.tv_sec * 1000000LL + firstEventTime.tv_usec);
                tv_sec = realDisableTime / 1000000;
                tv_usec = realDisableTime - tv_sec * 1000000;
            }
            int time_device = (tv_sec << 23) | (tv_usec << 3) | 0;
            int max_int = 0xFFFFFFFF;
            // Write Event to record trigger
        ...
        if (isToStop_threadGetProperty) {
            printf("In threadGetProperty, isToStop_threadGetProperty(%d)\n", isToStop_threadGetProperty);
            break;
        }
    }
    return 0;
...
int main(int argc, char *argv[])
{
    ...
    opterr = 0;
    do {
        c = getopt(argc, argv, "f:tns:Sv::dpilqc:rh");
        if (c == EOF)
            break;
        switch (c) {
        case 'f':
    ...
    isToStop_threadGetProperty = 0;
    printf("start threadGetProperty: isToStop_threadGetProperty(%d)\n", isToStop_threadGetProperty);
    pthread_create(&tid, NULL, &threadGetProperty, NULL);
        ...
        for(i = 1; i < nfds; i++) {
            if(ufds[i].revents) {
                if(ufds[i].revents & POLLIN) {
                    ...
                    if(init_time_sec == 0 && init_time_usec == 0){
                        init_time_sec = event.time.tv_sec;
                        init_time_usec = event.time.tv_usec;
                        gettimeofday(&firstEventTime, NULL);
                    }
                    int tv_sec = event.time.tv_sec - init_time_sec;
                    int tv_usec = event.time.tv_usec - init_time_usec;
                    char device_number[2];
                    strncpy(device_number, device_names[i] + 16, sizeof(device_number));
                    if(tv_usec < 0){
                        tv_sec -= 1;
                        tv_usec += 1000000LL;
                    }

                    // printf("[%04d.%05d] %s: %04x %04x %08x\n", tv_sec, tv_usec, device_names[i], event.type, event.code, event.value);

                    int time_device = (tv_sec << 23) | (tv_usec << 3) | atoi(device_number);
                    int type_code = (event.type << 16) | event.code;
                    // printf("%08x %08x %08x\n", time_device, type_code, event.value);

                    fwrite(&time_device, sizeof(int), 1, fp);
                    fwrite(&type_code, sizeof(int), 1, fp);
                    fwrite(&event.value, sizeof(int), 1, fp);
                    fflush(fp);
                    ...
                    if(event_count && --event_count == 0){
                        fclose(fp);
                        printf("Events binary file closed.\n");
                        return 0;
                    }
                }
            }
    ...
    isToStop_threadGetProperty = 1;
    printf("Stop threadGetProperty.\n");
    return 0;
}
