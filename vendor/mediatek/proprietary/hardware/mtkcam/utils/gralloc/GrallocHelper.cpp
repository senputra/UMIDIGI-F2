...
#define _ENUM_TO_NAME_(_prefix_, _format_) \
    ...
    _ENUM_TO_NAME_(HAL_PIXEL_FORMAT_, NV12);
...
#define MY_QUERY(_extra_get_, _field_) \
    ...
    MY_QUERY(ALLOC_SIZE     , allocSize);   // TDOO : use it to check calculated size is equal to allocate size
...
queryStaticInfo(buffer_handle_t const bh, MyStaticInfo& info)
{
    ...
    const bool isSecureUsage = ((info.usage & GRALLOC_USAGE_PROTECTED) & GRALLOC_USAGE_PROTECTED)>0;
    //
    switch  ( info.format )
    {
    ...
    case HAL_PIXEL_FORMAT_YV12:
        {
            ...
            if(!isSecureUsage)
            {
                ...
                {// Cb
                    ...
                    if ( (extra.producerUsage &  GRALLOC1_USAGE_SMVR) != 0)
                    {
                        ...
                        MY_LOGD_IF(0, "Extra-info(producerUsage=%u, %dx%d, "
                            "plane-y(rsb=%u, sib=%d), "
                            "plane-cr(rsb=%u, sid=%d), "
                            "plane-cb(rsb=%u, sib-ori=%d, sib-adjusted=%d), "
                            "extra.allocSize=%u",
            ...
            else
            {
                ...
                {// Cb
                    ...
                    {
                        size_t oriCbPlane_sizeInBytes = plane.sizeInBytes;
                        plane.sizeInBytes = ((((info.widthInPixels-1)>>5)+1)<<5) * ((((info.heightInPixels-1)>>5)+1)<<5) * 3/2
                                                - ((extra.stride * extra.height + (extra.stride * extra.height)>>2));
                    }
                }
            }
        }break;
...
freeBuffer(buffer_handle_t handle)
{
    ...
    return gMapper->freeBuffer(handle);
}
...
{
    Mutex::Autolock _l(mLock);
    MY_LOGI("Clear the stream info map");
    mMap.clear();
}
