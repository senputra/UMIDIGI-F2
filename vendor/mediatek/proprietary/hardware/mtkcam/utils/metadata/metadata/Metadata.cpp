...
struct ops
{
    ...
    virtual size_t       _set_size(void *, size_t)                  { return 0; };
...
struct opsImp : public ops
{
    ...
    virtual size_t _set_size(void *ptr, size_t size)
    {
        return static_cast<Container_T*>(ptr)->resize(size);
    };
...
private:
    ...
    inline void guarantee_type_id(int typeId)
    {
        if (__builtin_expect( mType != typeId, false )) {
            META_LOGE("unexpected type ID, (lhs, rhs)=(%d, %d)", mType, typeId);
...
operator=(IMetadata::IEntry::Implementor const& other)
{
    if (this != &other)
    {
        ...
        META_LOGW("this(%p) == other(%p)", this, &other);
    }
...
setSize(MUINT size)
{
    std::lock_guard<std::mutex> lk1(mResourceMtx);
    return get_mtk_metadata_tag_type(mTag) == -1 ?
        0 : this->StorageOps()->_set_size(mStoragePtr, size);
}
...
#define ASSERT_CHECK(_defaultT, _T) \
      META_LOGE_IF( TYPE_##_T != _defaultT, "tag(%x), type(%d) should be (%d)", tag(), TYPE_##_T, _defaultT); \
...
#define DUMP_METADATA_STRING(_layer_, _msg_) \
        META_LOGD("(L%d) %s", _layer_, _msg_.string());
...
dump(int layer)
{
    for (size_t i = 0; i < mMap.size(); ++i) {
        ...
        if ( TYPE_IMetadata == entry.type() ) {
            ...
            case TYPE_MUINT8:
                for (size_t j=0; j<entry.count(); j++)
                    msg += String8::format("x%X ", entry.itemAt(j, Type2Type< MUINT8 >() ));
                break;
            case TYPE_MINT32:
                for (size_t j=0; j<entry.count(); j++)
...
#define META_LOGD_TIME(fmt, arg...)     META_LOGD(fmt, ##arg)
#endif
...
struct metadata_buffer_handle {
    ...
    inline MERROR get_entry_info(metadata_buffer_entry_info& info)
    {
        ...
        if (!check_size(sizeof(metadata_buffer_entry_info))) {
            META_LOGE("[get_entry_info] out of boundary");
            return NO_MEMORY;
        }
        ...
        //META_LOGD("[get_info] tag:%d type:%d count:%d offset:%zu", info.tag, info.type, info.count, offset);
        return OK;
    }
    ...
    inline MERROR set_entry_info(metadata_buffer_entry_info& info)
    {
        ...
        if (!check_size(sizeof(metadata_buffer_entry_info))) {
            META_LOGE("[set_entry_info] out of boundary");
            return NO_MEMORY;
        }
        ...
        //META_LOGD("[set_info] tag:%d type:%d count:%d offset:%zu", info.tag, info.type, info.count, offset);
        return OK;
    }
    ...
    inline MBOOL check_size(size_t data_size)
    {
        if (size < offset + data_size) {
            META_LOGE("memory not enough, size=%zu, offset=%zu, data=%zu",
...
flatten(char* buf, size_t buf_size) const
{
    ...
    META_LOGD("layer:%d", ++sFlattenLayer);
#endif
    //
    std::lock_guard<std::mutex> lk(mResourceMtx);
    //
...
#undef CASE_WRITER
            default:
                META_LOGE("[flatten] unsupported format:%ul", info.type);
                ret = BAD_VALUE;
                continue;
        }
    ...
    META_LOGD("layer:%d offset:%zu time:%" PRIu64 "us",
...
#if _DEBUG_TIME_ESTIMATE_
    MINT64 start_time = static_cast<MINT64>(::systemTime());
    META_LOGD("layer:%d", ++sUnflattenLayer);
#endif
    //
    std::lock_guard<std::mutex> lk(mResourceMtx);
    //
...
#undef CASE_READER
            default:
                META_LOGE("[unflatten] unsupported format:%ul", info.type);
                ret = BAD_VALUE;
                continue;
        }
...
~IMetadata()
{
    ...
    if (__builtin_expect( is_alive, false )) {
        META_LOGE("double free at %p", this);
