...
doAllocGB(MINT32& heapID, size_t& stridesInBytes)
{
    ...
    //
    {
        ...
        ::snprintf(debug_info.name, sizeof(gralloc_extra_ion_debug_t::name), "%dx%d-%s-%s", getImgSize().w, getImgSize().h,
            NSCam::Utils::Format::queryImageFormatName(getImgFormat()).c_str(), mCallerName.c_str());
        debug_info.name[sizeof(gralloc_extra_ion_debug_t::name)-1] = '\0';
        err = ::gralloc_extra_perform(handle, GRALLOC_EXTRA_SET_IOCTL_ION_DEBUG, &debug_info);

        if  (CC_UNLIKELY(0 != err)) {
            MY_LOGE("ion_custom_ioctl(GRALLOC_EXTRA_SET_IOCTL_ION_DEBUG) returns %d, heapID:%d", err, heapID);
            goto lbExit;
        }
    }
    //
