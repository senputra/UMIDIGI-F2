...
allocOutputBuffer()
{
    ...
    MUINT32 shiftIndex = mSetting->mvInputData[0].mImageQueue.size() - 1;
    MY_LOGD("image buffer size: %ld final capture size:%dx%d shift: %d",
...
#define SET_INPUT_BUFFER(streamId, bufferPool, name, inputSEQ, FRAME_INDEX) \
    ...
    //SET_INPUT_BUFFER(eSTREAMID_IMAGE_PIPE_YUV_01, pPool_YUV01, "PP_YUV_01");
