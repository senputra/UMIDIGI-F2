...
{
static const int MAX_POSTPROC_REQUSET_CNT = 1;
...
public:
    ...
    virtual bool                               onCB_CompressedImage_packed(
                                                    ...
                                                    uint32_t const    shotMode = eShotMode_ZsdShot
                                               ) override;
public:
    ...
    MVOID                              addToProcessQueue(android::sp<ImagePostProcessData>& data);
    MVOID                              removeFromProcessQueue(MINT64& timeStamp);
private:
    int                                get_fd_count();
    int                                parseLine(char *line);
    uint32_t                           get_process_memory();
private:
...
isAvailable()
{
    MY_LOGD("cur_fd(%d), max(%d), count(%d) memory(%d kb)",
                        get_fd_count(),
                        mFDLimitCount,
                        get_fd_count()+TOLERANCE_COUNT,
                        get_process_memory());
...
)
{
    ...
    if (puBitstreamBuf != NULL)
    {
        ...
        else
        {
            ICallbackClientMgr::getInstance()->onCB_CompressedImage_packed(
                                                    ...
                                                    fgIsFinalImage,
                                                    msgType,
                                                    processType != PostProcessorType::BOKEH);
        }
    }
    else
    {
    ...
    if (fgIsFinalImage)
    {
        ...
        {
            Mutex::Autolock _l(mRequestLock);
            MY_LOGD("mvPostProcRequests size %zu timestamp(%lld)",
                                            mvPostProcRequests.size(),
                                            i8Timestamp);
        ...
        if(size() == 0)
        {
            MY_LOGD("release scenario control");
...
)
{
    ...
    if(pPostProcessor == nullptr)
    {
        ...
        if (pPostProcessor == nullptr)
        {
            ...
            removeFromProcessQueue(data->miTimeStamp);
            return BAD_TYPE;
        }
...
)
{
    ...
    Mutex::Autolock _l(mRequestLock);
    PostProcRequestInfo info;
    info.requestNo    = data->mRequestNo;
    info.timeStamp    = data->miTimeStamp;
    info.pCb          = data->mpCb;
    info.isFinalImage = MFALSE;
    mvPostProcRequests.add(data->miTimeStamp, info);
    FUNCTION_LOG_END
}
/******************************************************************************
 *
...
)
{
    FUNCTION_LOG_START
    Mutex::Autolock _l(mRequestLock);
    ssize_t index = mvPostProcRequests.indexOfKey(timestamp);
    if(index >= 0)
    {
        mvPostProcRequests.removeItem(index);
    }
    else
    {
        MY_LOGE("index not exist, should not happened.");
    }
    FUNCTION_LOG_END
}
/******************************************************************************
...
get_process_memory()
{
    ...
    snprintf(buf, 64, "/proc/%i/status", getpid());
...
threadLoop()
{
    ...
    {
        // get performance mode to high to process request.
        mpProcessImp->enterPerformanceScenario();
        mpProcessImp->onProcessData(pIPPData);
        MY_LOGD("process data timestamp(%lld)", pIPPData->miTimeStamp);
        return true;
    }
