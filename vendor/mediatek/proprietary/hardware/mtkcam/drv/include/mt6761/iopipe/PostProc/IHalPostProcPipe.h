...
#ifndef _MTK_PLATFORM_HARDWARE_INCLUDE_MTKCAM_IOPIPE_POSTPROC_IHALPOSTPROCPIPE_ISP3_H_
#define _MTK_PLATFORM_HARDWARE_INCLUDE_MTKCAM_IOPIPE_POSTPROC_IHALPOSTPROCPIPE_ISP3_H_
//
...
public:     ////                    Constructors.
                                    Input(
                                        NSIoPipeIsp3::PortID const&   rPortID     = NSIoPipeIsp3::PortID(),
...
public:     ////                    Constructors.
                                    Output(
                                        ...
                                        MINT32 const    transform   = 0,
                                        ExtraParams     rExtraParams= ExtraParams(),
                                        MBOOL			bRwbEn	= 0	// add for RWB sensor
                                    )
                                        : mPortID(rPortID)
                                        , mBuffer(buffer)
                                        , mTransform(transform)
                                        , mExtraParams(rExtraParams)
                                        , bIsRwbEn(bRwbEn)	// add for RWB sensor
                                    {
                                    }
};
...
struct  QParams
{
    ...
    MVOID*                          mpCookie;
    MVOID*                          mpPrivaData;                        //v1 usage
    android::Vector<MVOID*>         mvPrivaData;            //v1 usage
    android::Vector<MVOID*>         mvTuningData;         //v3 usage:  for p2 tuning data
#if 0
    ...
    MUINT32                         mFrameNo;                       //v1 usage
    android::Vector<MUINT32>        mvMagicNo;          //v1 usage
    MBOOL                           mDequeSuccess;              //v1&v3 usage
    android::Vector<ModuleInfo>     mvModuleData;   //v1&v3 usage
    // passed from pass1 (resizer information)
    android::Vector<MRect>          mvP1SrcCrop;        //v3 usage
    android::Vector<MSize>          mvP1Dst;                //v3 usage
    android::Vector<MRect>          mvP1DstCrop;        //v3 usage
    MUINT32                         pixIdP2; //QQ
