#ifndef __ISP_P1_HW_CFG_ISP3_H__
#define __ISP_P1_HW_CFG_ISP3_H__
...
#include "imageio/inc/imageio_log.h"
//-----------------------------------------------------------------------------
...
typedef enum
{
   HWRWCTRL_EFSKO_EFSKO = 1,
   HWRWCTRL_EFSKO_EISO = 2,
...
public:
   ...
   MINTPTR getModuleHandle(CAM_MODULE_ENUM module, char const* szCallerName); //ISP_DRV_CQ_ENUM cq
   ...
   void Lock(CAM_MODULE_ENUM module) {mLock[module].lock();}
   void UnLock(CAM_MODULE_ENUM module){mLock[module].unlock();}
   MBOOL dbgDump(MINTPTR handle, MUINT32 tgIdx);

   MBOOL setTGIndex(MUINT32 tgIdx);

protected:
   MBOOL _flushHwCfg2CQ(ObjAccessHW* accessObj, MUINT32 TarDupIdx,IspDrv* pTarVirDrv);
   MBOOL reFlushPendingCfgToCQ(void);
   MBOOL checkModuleSetting(ObjAccessHW* accessObj);

public:
   mutable Mutex     mLock[CAM_MODULE_MAX];         //protection m_hwAccessObj & m_vUpdatedHwMod
...
private:
   ...
   MBOOL             m_chkP1SettingBeforeLatch;
};
...
typedef struct qryExecIn{
    MUINT32 ScenId;
    MUINT32 fps;
    MSize imgoSize;
    MSize rrzoSize;

    qryExecIn(): ScenId(0), fps(0)
    {imgoSize.w = 0;imgoSize.h = 0;rrzoSize.w = 0;rrzoSize.h = 0;}
}_qryExecIn;
