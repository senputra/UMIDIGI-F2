...
typedef enum{
    ...
    _SET_TS_MODE            = (1L<<14),
    _RESET_LMV_FBC_CNT      = (1L<<15),

    _GET_DIP_DBG_INFO       = (1L<<16),     //data size:
    _GET_SOF_CNT            = (1L<<17),     //data size: 2 words, 1st for RAW-path, 2nd for index
    _GET_DMA_ERR            = (1L<<18),     //data size:
    _GET_INT_ERR            = (1L<<19),     //data size: 5 words. 1st for RAW-path, 2 to 5 mapping to corresponding RAW-PATH int_status
    _GET_DROP_FRAME_STATUS  = (1L<<20),     //data size: 2 words. 1st for RAW-path, 2nd for status
    _GET_START_TIME         = (1L<<21),     //data size: 2 words. in:1st word for module. out: 1st word:sec, 2nd word:usec.
    _GET_VSYNC_CNT          = (1L<<22),     //data size: 1 word.  out: current vsync counter
}E_DEVICE_INFO;
...
class IspDrv
{
    ...
    public:
        ...
        MUINT32 __max = REG_SIZE;\
            ...
            __max = (DIP_BASE_RANGE_SPECIAL > __max)?(DIP_BASE_RANGE_SPECIAL):(__max);\
...
class IspDrvImp : public IspDrv
{
    ...
    public:
        ...
        static MUINT32* getMMapRegAddr(void) {return (mpIspDipHwRegAddr);}
    private:
        ...
        static MUINT32* mpIspDipHwRegAddr;//register map from mmap
