 ...
 ************************************************************************************************/
#ifndef _IMEM_BUFFER_ISP3_H_
#define _IMEM_BUFFER_ISP3_H_
...
struct IMEM_BUF_INFO
{
    ...
    IMEM_BUF_INFO(
        ...
        MINT32      _bufSecu = 0,
        MINT32      _bufCohe = 0,
        MINT32      _useNoncache =0,/*default: use cache*/
        MINT32      _status = 0)
    : size(_size)
    , memID(_memID)
    , virtAddr(_virtAddr)
...
typedef enum
{
    PDAF_BUFCTRL_ENUM_AVAIL     =0,//available to use
    PDAF_BUFCTRL_ENUM_OUTER     =1,//is using for outer
    PDAF_BUFCTRL_ENUM_INNER     =2 //is using for inner
}PDAF_BUFCTRL_ENUM;
