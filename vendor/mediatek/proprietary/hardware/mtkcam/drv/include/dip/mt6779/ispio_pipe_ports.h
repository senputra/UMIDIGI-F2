...
struct PipePackageInfo
{
    ...
    void *MDPparam;
    MUINT32 ForceSkipConfig; //Current frame is the same as previous frame, the difference is dma address.
public:     //// constructors.
    PipePackageInfo()
        : StreamTag(NSCam::NSIoPipe::NSPostProc::ENormalStreamTag_Normal)//temp solution
        , burstQIdx(0)
        , p2RingBufIdx(0)
        , p2cqIdx(0)
        , frameNum(0)
