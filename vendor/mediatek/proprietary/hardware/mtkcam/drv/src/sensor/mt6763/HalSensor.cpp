...
extern bool CSI0A_Offset_Calibration;
extern bool CSI0B_Offset_Calibration;
extern bool CSI1A_Offset_Calibration;
extern bool CSI1B_Offset_Calibration;
extern bool CSI2_Offset_Calibration;
...
)
{
    ...
    for (MUINT i = 0; i < uCountOfIndex; i++)
    {
        ...
        if ((ret = setTgPhase(sensorIdx, 1)) < 0) {
            MY_LOGE("setTgPhase fail n");
            ret = MFALSE;
            break;
        }

        if ((ret = setSensorIODrivingCurrent(sensorIdx)) < 0) {
    ...
    ret = MTRUE;
    MY_LOGD("- <%s> ret:%d, meSensorDev = 0x%x\n", szCallerName, ret, gmeSensorDev);
...
)
{
    ...
    for (MUINT i = 0; i < uCountOfIndex; i++)
    {
        ...
        if ((ret = setTgPhase(sensorIdx, 0)) < 0) {
            MY_LOGE("setTgPhase fail n");
            return MFALSE;
        }
        ...
        if(sensorIdx == IMGSENSOR_SENSOR_IDX_SUB) {//sub cam power off  reset cal flag for scam
            MY_LOGD("!!!scam clear calibration flag\n");
            best_delay_value = -1;
            Positive_Calibration = false;//calibration should re-start once sensor was shut down
        }
        //offset calibration should re-start once sensor was shut down
        if(sensorIdx == IMGSENSOR_SENSOR_IDX_MAIN) {
            CSI0A_Offset_Calibration = false;
            CSI0B_Offset_Calibration = false;
        }
        else if(sensorIdx == IMGSENSOR_SENSOR_IDX_SUB) {
            CSI1A_Offset_Calibration = false;
            CSI1B_Offset_Calibration = false;

        }else if(sensorIdx == IMGSENSOR_SENSOR_IDX_MAIN2){
            CSI2_Offset_Calibration = false;
        }else{
            CSI0A_Offset_Calibration = false;
            CSI0B_Offset_Calibration = false;
        }
