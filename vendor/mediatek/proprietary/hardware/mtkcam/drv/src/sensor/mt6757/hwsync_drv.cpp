...
{
    mSemHWSyncLvMain1.count=0;
    mSemHWSyncLvMain2.count=0;
    mSemJumpoutSync.count=0;
    mSem2Idle.count=0;
    mSemGeneralSyncDoneMain1.count=0;
    mSemGeneralSyncDoneMain2.count=0;
...
MINT32 HWSyncDrvImp::init(HW_SYNC_USER_ENUM user,MUINT32 sensorIdx,MINT32 aeCyclePeriod)
{
    ...
    LOG_INF("+, user&cnt(%s,%d),srIdxNum(%d)",(user==HW_SYNC_USER_HALSENSOR)?("HW_SYNC_USER_HALSENSOR"):("HW_SYNC_USER_AE")\
        ...
        case HW_SYNC_USER_HALSENSOR:
                {
                    ...
                    pSeninf_Drv->sendCommand(CMD_GET_SENINF_ADDR, (long unsigned int)&pSeninf);
                    ...
                    if(mSensorIdxList.size() ==1)
                    {
                        ...
                        if(pSeninf)
                            SENINF_WRITE_REG(pSeninf, SENINF_N3D_CTL, RegIO.Raw);
                        else
...
MINT32 HWSyncDrvImp::sendCommand(HW_SYNC_CMD_ENUM cmd,MUINT32 arg1,MUINTPTR arg2,MUINTPTR arg3,MUINTPTR arg4)
{
    ...
    switch(cmd)
    {
        case HW_SYNC_CMD_SET_MODE:
            {
                    ...
                    {   //set sensor dev if sensorHal.Control do configure
                        ...
                        switch(senTg)
                        {
                            case CAM_TG_1:
                                ...
                                LOG_INF("dev(0x%x),scen(%d) cFps(%d), userFps(%d), frmTimeDelay(%d), hwsync_idx(%d)", \
                                    mSensorDevArray[0],mSensorScenArray[0],mSensorCurFpsArray[0],arg3, \
                            ...
                            case CAM_TG_2:
                                ...
                                LOG_INF("dev(0x%x),scen(%d) cFps(%d), userFps(%d), frmTimeDelay(%d), hwsync_idx(%d)", \
                                    mSensorDevArray[1],mSensorScenArray[1],mSensorCurFpsArray[1],arg3, \
                ...
                LOG_INF("cursenSce(0x%x), tg_dev info (0x%x/0x%x)",arg2,mSensorDevArray[0],mSensorDevArray[1]);
        ...
        case HW_SYNC_CMD_DISABLE_SYNC:
            {
                android_atomic_dec(&this->mSyncCount);
        ...
        case HW_SYNC_CMD_SET_PARA:
            if(Dev_temp.size()>2){
                LOG_ERR("Dev_temp.size(%d), can't more than 2 for hwsync",Dev_temp.size());
                ret = 1;
            }
            else{
                if(Dev_temp.size()!=0){
            ...
            switch(arg2){//scenario here is dvfs level
                ...
                default:
                    this->m_DVFS = E_SMI_HIGH;
                    LOG_ERR("unsupported DVFS level:%d\n",arg2);
                    break;
            }
            break;
        case HW_SYNC_CMD_ENABLE_SYNC:
            ...
            if(Dev_temp.size()==2){
                 ...
                 LOG_INF("Total amount of Dev not 2(%d)",Dev_temp.size());
            }
                ...
                for(int i = 0; i < E_TG_MAX; i++) {
                    switch(i) {
                        ...
                        case E_TG_B:
                            ...
                            break;
                        default:
                            LOG_ERR("Unsupported TG index(0x%x)\n", i);
                            break;
                    }

                    this->m_pIspDrv[i]->init("Hwsync");

                }

            }

            android_atomic_inc(&this->mSyncCount);
                ...
                case 2:
                    {
                        ...
                        LOG_INF("2nd lunch devID:%d, TG:%d\n", arg2, dynamicInfo.TgInfo);
                        //change order
...
MVOID* HWSyncDrvImp::DBG_Thread::DBG_Trig(MVOID* arg)
{
    ...
    sprintf((char*)regirq.userName,"Drv/HWsync_DBG");
...
MUINT32 HWSyncDrvImp::doGeneralSync_AE(MUINT32 arg1, MUINTPTR arg2, MUINTPTR arg3)
{
    ...
    LOG_INF("adjSenIdx(0x%x), adjFrmT(%d) us, PreFrmTime(%d/%d) us, CurFrmTime(%d/%d) us, VD(%d) us, order(%d)\n",adj_sensor,\
    ...
    LOG_INF("dev0/dev1(0x%x/0x%x), expTime(%d/%d), frmTime_1(%d/%d), frmTime_2(%d/%d), SOF(%d/%d)\n",\
...
MINT32 HWSyncDrvImp::setSyncFrmTimeMain1()
{
    ...
    if(mState!=HW_SYNC_STATE_IDLE && mState!= HW_SYNC_STATE_READY2LEAVE){
        ...
        LOG_INF("dev0/dev1(0x%x/0x%x), expTime(%d/%d), frmTime(%d/%d) us, frmTimeDelay(%d/%d), sof(%d/%d)\n", \
            this->mSensorDevArray[0], this->mSensorDevArray[1], \
            this->m2ndData.expTime[0], this->m2ndData.expTime[1], \
            this->m2ndData.FrameTime[0], this->m2ndData.FrameTime[1], \
            this->mFrameTimeDelayFrame[0], this->mFrameTimeDelayFrame[1], \
            sofCnt[0], sofCnt[1]);
    }
    else {
        ...
        case E_GET_SOF_CNT:
            switch(tgIdx){
                ...
                case E_TG_B:
                    ...
                    break;
                default:
                    LOG_ERR("unsupported TG idx:0x%x\n",tgIdx);
                    return 0;
                    break;
            }
            break;
