...
#define DEBUG_SENSOR_OPEN_CLOSE 1
...
closeSensor(HalSensor*const pHalSensor, char const* szCallerName)
{
    ...
    for (; it != mOpenSensorList.end(); ++it)
    {
        ...
        if  ( pHalSensor == it->mpHalSensor )
        {
            ...
            MY_LOGD("closeSensor mpHalSensor : %p, pHalSensor = %p, refcnt= %d\n",it->mpHalSensor,pHalSensor,it->miRefCount);
#endif
            //  Last one reference ?
...
#ifdef DEBUG_SENSOR_OPEN_CLOSE
    MY_LOGD("new created vSensorIdx[0] = %d, vSensorIdx[1] = %d, vSensorIdx[2] = %d\n",vSensorIndex[0],vSensorIndex[1],vSensorIndex[2]);
#endif
