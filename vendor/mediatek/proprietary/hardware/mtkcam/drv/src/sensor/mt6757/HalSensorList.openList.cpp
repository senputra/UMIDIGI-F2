...
)
{
    ...
    for (; it != mOpenSensorList.end(); ++it)
    {
        if  ( it->mpHalSensor->isMatch(vSensorIndex) )
        {
            ...
            #ifdef DEBUG_CAMERA_openList
             MY_LOGD("openSensor mpHalSensor : 0x%x,idx %d, %d, %d\n", it->mpHalSensor, vSensorIndex[0], vSensorIndex[1], vSensorIndex[2]);
            #endif
