...
extern SENSOR_CONFIG_STRUCT sensorPara[IMGSENSOR_SENSOR_IDX_MAX_NUM];
...
MUINT HalSensorList::getRawInfo(IMGSENSOR_SENSOR_IDX idx, SENSOR_HAL_RAW_INFO_STRUCT *pInfo)
{
    ...
    SensorDrv *const pSensorDrv = SensorDrv::get();
    pInfo->u1Order = sensorDrvInfo[idx].SensorOutputDataFormat;
...
MVOID HalSensorList::querySensorInfo(IMGSENSOR_SENSOR_IDX idx)
{
    ...
    SensorDrv *const pSensorDrv = SensorDrv::get();
    pSensorDrv->sendCommand(idx, CMD_SENSOR_GET_MCLK_CONNECTION, (MUINTPTR)&data1);
    sensorPara[idx].mclkSrc = (EMclkId)data1;
    pSensorDrv->sendCommand(idx, CMD_SENSOR_GET_MIPI_SENSOR_PORT, (MUINTPTR)&data1);
    sensorPara[idx].mipiPad = (EMipiPort)data1;

    sensorStaticInfo[idx].sensorDevID = pSensorDrv->getSensorID(idx);
     ...
     switch (sensorRawInfo[idx].u1Order) {
       ...
       case SENSOR_OUTPUT_FORMAT_RAW_4CELL_B:
         ...
         case SENSOR_OUTPUT_FORMAT_RAW_4CELL_BAYER_B:
            sensorStaticInfo[idx].rawFmtType = SENSOR_RAW_4CELL_BAYER;
            sensorStaticInfo[idx].sensorFormatOrder = SENSOR_FORMAT_ORDER_RAW_B;
            break;
    ...
    MY_LOGD("MCLK Source[%d]: %d\n", idx, sensorPara[idx].mclkSrc);
    MY_LOGD("MIPI pad[%d]: %d\n", idx, sensorPara[idx].mipiPad);
}
