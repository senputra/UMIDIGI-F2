...
static SENINF_CSI_INFO seninfCSITypeInfo[CUSTOM_CFG_CSI_PORT_MAX_NUM] = {
    ...
    SENINF_CSI_TYPE_MIPI(CUSTOM_CFG_CSI_PORT_1A, SENINF_3),
    SENINF_CSI_TYPE_MIPI(CUSTOM_CFG_CSI_PORT_1B, SENINF_4),
};
...
int SeninfDrvImp::init()
{
    ...
    if (fd < 0) { /* Use v2 device node if v1 device node is removed */
        ...
        if (ret < 0) {
            ...
            mCSI[4] = devinfo.data[3];//0x11C10040
        }
        LOG_MSG("Efuse Data:0x11C1018C= 0x%x, 0x11C10190= 0x%x, 0x11C101BC= 0x%x, 0x11C101C0= 0x%x, 0x11C10040= 0x%x\n",
            mCSI[0], mCSI[1], mCSI[2], mCSI[3], mCSI[4]);
        close(fd);
    }
#endif
...
int SeninfDrvImp::debug()
{
    ...
    for (j = CUSTOM_CFG_CSI_PORT_0A; j < CUSTOM_CFG_CSI_PORT_MAX_NUM; j++) {
        pMipiRx_ana_csiA = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[j];
        SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_WRAPPER80_CSI0A, CSR_CSI_MON_MUX) = 5;
        LOG_MSG("MipiRx_ANA%d: (0x%x)\n",
                j-3,
                SENINF_READ_REG(pMipiRx_ana_csiA, MIPI_RX_WRAPPER84_CSI0A));
    }
...
int SeninfDrvImp::disableCamMUX(SENINF_CAM_MUX_ENUM cam_mux)
{
    ...
    LOG_MSG("SENINF_CAM_MUX_EN 0x%x cam_mux %d, mCamMuxUsage = %d",
        SENINF_READ_REG(pSeninf_cam_mux, SENINF_CAM_MUX_EN),
        cam_mux,
        mCamMuxUsage[cam_mux]);

    if (this->isCamMUXUsed(cam_mux)) {
        SENINF_WRITE_REG(pSeninf_cam_mux, SENINF_CAM_MUX_EN, temp&(~(1<<cam_mux)));
        LOG_MSG("after disable : SENINF_CAM_MUX_EN 0x%x cam_mux %d",
             SENINF_READ_REG(pSeninf_cam_mux, SENINF_CAM_MUX_EN), cam_mux);
...
{
    return (SENINF_CAM_MUX_ENUM)(TgEnum < CAM_SV_1 ?(TgEnum - CAM_TG_1) :(TgEnum - CAM_SV_MASK + CAM_TG_2));
}
...
int SeninfDrvImp::Efuse(unsigned long csi_sel)
{
	...
	if ((mCSI[0] != 0) && (csi_sel == 0)) {
		...
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0P_T0A_HSRT_CODE) = (mCSI[0]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0N_T0B_HSRT_CODE) = (mCSI[0]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1P_T0C_HSRT_CODE) = (mCSI[0]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1N_T1A_HSRT_CODE) = (mCSI[0]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2P_T1B_HSRT_CODE) = (mCSI[0]>>17) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2N_T1C_HSRT_CODE) = (mCSI[0]>>17) & 0x1f;
        LOG_MSG("CSI0A MIPI_RX_ANA08_CSI0A(0x%x) MIPI_RX_ANA0C_CSI0A(0x%x)",
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA08_CSI0A),
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A));
        pMipiRx_ana = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[CUSTOM_CFG_CSI_PORT_0B]; //0x11C8 1000
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0P_T0A_HSRT_CODE) = (mCSI[0]>>12) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0N_T0B_HSRT_CODE) = (mCSI[0]>>12) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1P_T0C_HSRT_CODE) = (mCSI[0]>>7) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1N_T1A_HSRT_CODE) = (mCSI[0]>>7) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2P_T1B_HSRT_CODE) = (mCSI[0]>>2) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2N_T1C_HSRT_CODE) = (mCSI[0]>>2) & 0x1f;
	    LOG_MSG("CSI0B MIPI_RX_ANA08_CSI0A(0x%x) MIPI_RX_ANA0C_CSI0A(0x%x)",
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA08_CSI0A),
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A));
	}
	if ((mCSI[1] != 0)&&(mCSI[2] != 0) && (csi_sel == 1)) {
		...
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0P_T0A_HSRT_CODE) = (mCSI[1]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0N_T0B_HSRT_CODE) = (mCSI[1]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1P_T0C_HSRT_CODE) = (mCSI[1]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1N_T1A_HSRT_CODE) = (mCSI[1]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2P_T1B_HSRT_CODE) = (mCSI[1]>>17) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2N_T1C_HSRT_CODE) = (mCSI[1]>>17) & 0x1f;
        LOG_MSG("CSI1A MIPI_RX_ANA08_CSI0A(0x%x) MIPI_RX_ANA0C_CSI0A(0x%x)",
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA08_CSI0A),
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A));
        pMipiRx_ana = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[CUSTOM_CFG_CSI_PORT_1B]; //0x11C8 5000
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0P_T0A_HSRT_CODE) = (mCSI[2]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0N_T0B_HSRT_CODE) = (mCSI[2]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1P_T0C_HSRT_CODE) = (mCSI[2]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1N_T1A_HSRT_CODE) = (mCSI[2]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2P_T1B_HSRT_CODE) = (mCSI[2]>>17) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2N_T1C_HSRT_CODE) = (mCSI[2]>>17) & 0x1f;
	    LOG_MSG("CSI1B MIPI_RX_ANA08_CSI0A(0x%x) MIPI_RX_ANA0C_CSI0A(0x%x)",
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA08_CSI0A),
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A));
	}
	if((mCSI[2] != 0)&&(mCSI[3] != 0) && (csi_sel == 2)) {
		...
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0P_T0A_HSRT_CODE) = (mCSI[2]>>12) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0N_T0B_HSRT_CODE) = (mCSI[2]>>12) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1P_T0C_HSRT_CODE) = (mCSI[2]>>7) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1N_T1A_HSRT_CODE) = (mCSI[2]>>7) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2P_T1B_HSRT_CODE) = (mCSI[2]>>2) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2N_T1C_HSRT_CODE) = (mCSI[2]>>2) & 0x1f;
		LOG_MSG("CSI2A MIPI_RX_ANA08_CSI0A(0x%x) MIPI_RX_ANA0C_CSI0A(0x%x)",
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA08_CSI0A),
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A));
        pMipiRx_ana = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[CUSTOM_CFG_CSI_PORT_2B]; //0x11C8 9000
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0P_T0A_HSRT_CODE) = (mCSI[3]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0N_T0B_HSRT_CODE) = (mCSI[3]>>27) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1P_T0C_HSRT_CODE) = (mCSI[3]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1N_T1A_HSRT_CODE) = (mCSI[3]>>22) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2P_T1B_HSRT_CODE) = (mCSI[3]>>17) & 0x1f;
		SENINF_BITS(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2N_T1C_HSRT_CODE) = (mCSI[3]>>17) & 0x1f;
		LOG_MSG("CSI2B MIPI_RX_ANA08_CSI0A(0x%x) MIPI_RX_ANA0C_CSI0A(0x%x)",
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA08_CSI0A),
            SENINF_READ_REG(pMipiRx_ana, MIPI_RX_ANA0C_CSI0A));
	}
	return ret;
...
*
********************************************************************************/
void SeninfDrvImp::hal_csirx_dphy_init(CUSTOM_CFG_CSI_PORT mipiPort)
{
    ...
    if (pCsi0_Dphy_Top == NULL) {
        LOG_ERR("error! pCsi0_Dphy_Top is null!");
        return;
    }
    ...
    //no need to set clock lane settle

    /* trail enable */
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DATA_LANE0_HS_PARAMETER, rg_dphy_rx_ld0_hs_trail_parameter) = SENINF_HS_TRAIL_PARAMETER; //0x11c8 2020[15:8]
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DATA_LANE1_HS_PARAMETER, rg_dphy_rx_ld1_hs_trail_parameter) = SENINF_HS_TRAIL_PARAMETER;
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DATA_LANE2_HS_PARAMETER, rg_dphy_rx_ld2_hs_trail_parameter) = SENINF_HS_TRAIL_PARAMETER;
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DATA_LANE3_HS_PARAMETER, rg_dphy_rx_ld3_hs_trail_parameter) = SENINF_HS_TRAIL_PARAMETER;
    ...
    /* hw deskew */
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_CTRL, rg_dphy_rx_deskew_code_unit_sel) = 1;// 0: for 3.2 Gbps and above, 1: for speed under 3.2 Gbps
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_CTRL, rg_dphy_rx_deskew_initial_setup) = 0xF;
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_TIMING_CTRL, rg_dphy_rx_deskew_setup_cnt) = 0xB;
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_TIMING_CTRL, rg_dphy_rx_deskew_hold_cnt) = 1;
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_TIMING_CTRL, rg_dphy_rx_deskew_detect_cnt) = 0x8;
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_TIMING_CTRL, rg_dphy_rx_deskew_cmplength) = 0x3;
    ...
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_CTRL, rg_dphy_rx_deskew_en) = 1; //11c8 2050[8], enable or disable hw deskew
    /* deskew acc mode */
    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_CTRL, rg_dphy_rx_deskew_acc_mode) = 0xF; // 0: stop at 2nd edge, others: go all the way down
#endif
}

void SeninfDrvImp::hal_csirx_cphy_init(CUSTOM_CFG_CSI_PORT mipiPort)
{
    ...
    if (pCsi0_Cphy_Top == NULL) {
        LOG_ERR("Error! pCsi0_Cphy_Top is null!");
        return;
    }
    SENINF_BITS(pCsi0_Cphy_Top, CPHY_RX_DETECT_CTRL_POST, rg_cphy_rx_data_valid_post_en) = 0x1; //0x11C83018
}

void SeninfDrvImp::hal_csirx_phyA_init(CUSTOM_CFG_CSI_PORT mipiPort)
{
    int i = 0;
    int is_4d1c = 0;
    ...
    for(i = 0; i <= is_4d1c; i++) {
        ...
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA04_CSI0A, RG_CSI0A_BG_LPRX_VTH_SEL) = 0x4;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA04_CSI0A, RG_CSI0A_BG_LPRX_VTL_SEL) = 0x4;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA04_CSI0A, RG_CSI0A_BG_HSDET_VTH_SEL) = 0x4;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA04_CSI0A, RG_CSI0A_BG_HSDET_VTL_SEL) = 0x4;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA04_CSI0A, RG_CSI0A_BG_VREF_SEL) = 0x8;

        /* EQ_IS*/
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_L0_T0AB_EQ_IS) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_XX_T0CA_EQ_IS) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_XX_T0BC_EQ_IS) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_L1_T1AB_EQ_IS) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_XX_T1CA_EQ_IS) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_L2_T1BC_EQ_IS) = 0x1;

        /* EQ_BW*/
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_L0_T0AB_EQ_BW) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_XX_T0CA_EQ_BW) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_XX_T0BC_EQ_BW) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_L1_T1AB_EQ_BW) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_XX_T1CA_EQ_BW) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_L2_T1BC_EQ_BW) = 0x1;

        /* EQ_SCA */
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_L0_T0AB_EQ_SCA) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_XX_T0CA_EQ_SCA) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_XX_T0BC_EQ_SCA) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_L1_T1AB_EQ_SCA) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_XX_T1CA_EQ_SCA) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_L2_T1BC_EQ_SCA) = 0x1;

        /* EQ_SRA */
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_L0_T0AB_EQ_SRA) = 0x3;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_XX_T0CA_EQ_SRA) = 0x3;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_XX_T0BC_EQ_SRA) = 0x3;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_L1_T1AB_EQ_SRA) = 0x3;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_XX_T1CA_EQ_SRA) = 0x3;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_L2_T1BC_EQ_SRA) = 0x3;

        /* Termination Control */
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0P_T0A_HSRT_CODE) = 0x10;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L0N_T0B_HSRT_CODE) = 0x10;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1P_T0C_HSRT_CODE) = 0x10;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA08_CSI0A, RG_CSI0A_L1N_T1A_HSRT_CODE) = 0x10;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2P_T1B_HSRT_CODE) = 0x10;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA0C_CSI0A, RG_CSI0A_L2N_T1C_HSRT_CODE) = 0x10;

        /* C-PHY */
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA30_CSI0A, RG_CSI0A_CPHY_T0_CDR_AB_WIDTH) = 8;//HAL_CSIRX_CPHY_CDR_AB_BC_CA_WIDTH;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA30_CSI0A, RG_CSI0A_CPHY_T0_CDR_BC_WIDTH) = 8;//HAL_CSIRX_CPHY_CDR_AB_BC_CA_WIDTH;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA30_CSI0A, RG_CSI0A_CPHY_T0_CDR_CA_WIDTH) = 8;//HAL_CSIRX_CPHY_CDR_AB_BC_CA_WIDTH;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA3C_CSI0A, RG_CSI0A_CPHY_T1_CDR_AB_WIDTH) = 8;//HAL_CSIRX_CPHY_CDR_AB_BC_CA_WIDTH;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA3C_CSI0A, RG_CSI0A_CPHY_T1_CDR_BC_WIDTH) = 8;//HAL_CSIRX_CPHY_CDR_AB_BC_CA_WIDTH;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA3C_CSI0A, RG_CSI0A_CPHY_T1_CDR_CA_WIDTH) = 8;//HAL_CSIRX_CPHY_CDR_AB_BC_CA_WIDTH;

        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA28_CSI0A, RG_CSI0A_CPHY_T0_CDR_DIRECT_EN) = 0x1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA34_CSI0A, RG_CSI0A_CPHY_T1_CDR_DIRECT_EN) = 0x1;

        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA30_CSI0A, RG_CSI0A_CPHY_T0_CDR_CK_DELAY) = 1;//HAL_CSIRX_CPHY_CDR_CK_DELAY;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA3C_CSI0A, RG_CSI0A_CPHY_T1_CDR_CK_DELAY) = 1;//HAL_CSIRX_CPHY_CDR_CK_DELAY;
    }
...
void SeninfDrvImp::hal_csirx_phy_init(CUSTOM_CFG_CSI_PORT mipiPort)
{
    /* phyA init */
    hal_csirx_phyA_init(mipiPort);

    /* phyD init */
    hal_csirx_dphy_init(mipiPort);
    hal_csirx_cphy_init(mipiPort);
}
...
*
********************************************************************************/
void SeninfDrvImp::hal_csirx_seninf_csi2_setting(SENINF_CSI_MIPI *pCsiMipi)
{
    SENINF_CSI_INFO *pCsiInfo = pCsiMipi->pCsiInfo;
    seninf1_csi2_REGS *pSeninf_csi2 = (seninf1_csi2_REGS *)mpSeninfCSIRxConfBaseAddr[SENINF_1]; //0x1A004A00
    ...
    if (pSeninf_csi2 == NULL) {
        LOG_ERR("error! pSeninf_csi2 is null!");
        return;
    }

    /* resync fifo option ctrl */
    SENINF_BITS(pSeninf_csi2, SENINF_CSI2_RESYNC_MERGE_CTRL, rg_csi2_resync_cycle_cnt_opt) = 1;

    /* header mode/ header length */
    if (pCsiMipi->csi_type != CSI2_2_5G_CPHY) { //Dphy
        ...
        SENINF_BITS(pSeninf_csi2, SENINF_CSI2_HDR_MODE_0, rg_csi2_header_len) = 1 << (pCsiMipi->dlaneNum);
        SENINF_BITS(pSeninf_csi2, SENINF_CSI2_OPT, rg_csi2_cphy_sel) = 1;
    }

    /* irq clear mode */
    SENINF_BITS(pSeninf_csi2, SENINF_CSI2_IRQ_EN, rg_csi2_irq_clr_mode) = 1; //IRQ_WRITE_CLR

    int laneNum = pCsiMipi->dlaneNum == SENSOR_MIPI_4_LANE ? 0xF :
                  pCsiMipi->dlaneNum == SENSOR_MIPI_3_LANE ? 0x7 :
                  pCsiMipi->dlaneNum == SENSOR_MIPI_2_LANE ? 0x3 :
                  0x1;
    SENINF_WRITE_REG(pSeninf_csi2, SENINF_CSI2_EN, laneNum); //lane enable

    LOG_MSG("pCsiMipi->dlaneNum:%d, SENINF_CSI2_HDR_MODE_0 0x%x, SENINF_CSI2_EN 0x%x",
            pCsiMipi->dlaneNum,
            SENINF_READ_REG(pSeninf_csi2, SENINF_CSI2_HDR_MODE_0),
            SENINF_READ_REG(pSeninf_csi2, SENINF_CSI2_EN));
}

void SeninfDrvImp::hal_csirx_seninf_setting(SENINF_CSI_MIPI *pCsiMipi)
{
    SENINF_CSI_INFO *pCsiInfo = pCsiMipi->pCsiInfo;
...
void SeninfDrvImp::hal_csirx_seninf_top_setting(SENINF_CSI_MIPI *pCsiMipi)
{
    SENINF_CSI_INFO *pCsiInfo = pCsiMipi->pCsiInfo;
...
void SeninfDrvImp::hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT mipiPort, bool enable)
{
    mipi_rx_ana_cdphy_csi0a_reg_REGS *pMipiRx_ana_base = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[mipiPort];
    int status = 0;
    int count = 0;

    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_L0_T0AB_EQ_OS_CAL_EN) = 0;
    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_XX_T0CA_EQ_OS_CAL_EN) = 0;
    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_XX_T0BC_EQ_OS_CAL_EN) = 0;
    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_L1_T1AB_EQ_OS_CAL_EN) = 0;
    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_XX_T1CA_EQ_OS_CAL_EN) = 0;
    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_L2_T1BC_EQ_OS_CAL_EN) = 0;
    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_BG_LPF_EN) = 0;
    SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_BG_CORE_EN) = 0;
    usleep(200);

    if (enable) {
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_BG_CORE_EN) = 1;
        usleep(30);
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_BG_LPF_EN) = 1;
        usleep(1);

        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_L0_T0AB_EQ_OS_CAL_EN) = 1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA18_CSI0A, RG_CSI0A_XX_T0CA_EQ_OS_CAL_EN) = 1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_XX_T0BC_EQ_OS_CAL_EN) = 1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA1C_CSI0A, RG_CSI0A_L1_T1AB_EQ_OS_CAL_EN) = 1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_XX_T1CA_EQ_OS_CAL_EN) = 1;
        SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA20_CSI0A, RG_CSI0A_L2_T1BC_EQ_OS_CAL_EN) = 1;
        usleep(1);

        /* Check EQ completed */
        while(1) {
            status = SENINF_READ_REG(pMipiRx_ana_base, MIPI_RX_ANA48_CSI0A);
            if ((status&(1<<0)) && (status&(1<<3)) && (status&(1<<5)))
                break;
            else
    ...
    LOG_MSG("power enable = %d, MIPI_RX_ANA00_CSI0A 0x%x, CSI offset calibration end MIPI_RX_ANA48_CSI0A 0x%x",
            enable,
            SENINF_READ_REG(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A),
            SENINF_READ_REG(pMipiRx_ana_base, MIPI_RX_ANA48_CSI0A));
}

void SeninfDrvImp::hal_csirx_phyA_setting(SENINF_CSI_MIPI *pCsiMipi)
{
    SENINF_CSI_INFO *pCsiInfo = pCsiMipi->pCsiInfo;
    mipi_rx_ana_cdphy_csi0a_reg_REGS *pMipiRx_ana_base = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[pCsiInfo->port];
    ...
    if (pMipiRx_ana_csiA == NULL || pMipiRx_ana_csiB == NULL) {
        LOG_ERR("Error! pMipiRx_ana_csi is null!");
        return;
    }


    if ((pCsiMipi->dlaneNum) > SENSOR_MIPI_2_LANE  || (pCsiInfo->port < CUSTOM_CFG_CSI_PORT_0A)) {
        if (pCsiMipi->csi_type != CSI2_2_5G_CPHY) { //Dphy
            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A ,RG_CSI0A_CPHY_EN) = 0;
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A ,RG_CSI0A_CPHY_EN) = 0;
        } else {
            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A ,RG_CSI0A_CPHY_EN) = 1;
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A ,RG_CSI0A_CPHY_EN) = 1;
        }
    } else {
        if (pCsiMipi->csi_type != CSI2_2_5G_CPHY) //Dphy
            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A ,RG_CSI0A_CPHY_EN) = 0;
        else
            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A ,RG_CSI0A_CPHY_EN) = 1;
    }

    if (pCsiMipi->csi_type != CSI2_2_5G_CPHY) { //Dphy
        if (pCsiMipi->dlaneNum == SENSOR_MIPI_4_LANE || (pCsiInfo->port < CUSTOM_CFG_CSI_PORT_0A)) {
            LOG_MSG("4 LANE\n");
            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L0_CKMODE_EN) = 0; //1:clock lane, 0:data lane
            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L1_CKMODE_EN) = 0;
            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L2_CKMODE_EN) = 1;
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L0_CKMODE_EN) = 0; //1:clock lane, 0:data lane
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L1_CKMODE_EN) = 0;
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L2_CKMODE_EN) = 0;

            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L0_CKSEL) = 1;
            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L1_CKSEL) = 1;
            SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L2_CKSEL) = 1;
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L0_CKSEL) = 1;
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L1_CKSEL) = 1;
            SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L2_CKSEL) = 1;
        } else {
            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L0_CKMODE_EN) = 0; //1:clock lane, 0:data lane
            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L1_CKMODE_EN) = 1;
            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L2_CKMODE_EN) = 0;

            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L0_CKSEL) = 0;
            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L1_CKSEL) = 0;
            SENINF_BITS(pMipiRx_ana_base, MIPI_RX_ANA00_CSI0A, RG_CSI0A_DPHY_L2_CKSEL) = 0;
        }
    }
    ...
    switch(pCsiInfo->port) {
    case CUSTOM_CFG_CSI_PORT_0:
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_0A, 1);
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_0B, 1);
        break;
    case CUSTOM_CFG_CSI_PORT_1:
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_1A, 1);
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_1B, 1);
        break;
    case CUSTOM_CFG_CSI_PORT_2:
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_2A, 1);
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_2B, 1);
        break;
    case CUSTOM_CFG_CSI_PORT_0A:
    case CUSTOM_CFG_CSI_PORT_0B:
    case CUSTOM_CFG_CSI_PORT_1A:
    case CUSTOM_CFG_CSI_PORT_1B:
        hal_csirx_phyA_power_on(pCsiInfo->port, 1);
    default:
        break;
    }
}

void SeninfDrvImp::hal_csirx_dphy_setting(SENINF_CSI_MIPI *pCsiMipi)
{
    SENINF_CSI_INFO *pCsiInfo = pCsiMipi->pCsiInfo;
    csi0_dphy_top_reg_REGS *pCsi0_Dphy_Top = (csi0_dphy_top_reg_REGS *)mpCSIDphyTopAddr[0]; //0X1A00 4000
    ...
    if (pCsiMipi->dlaneNum == SENSOR_MIPI_4_LANE || (pCsiInfo->port < CUSTOM_CFG_CSI_PORT_0A)) {
        ...
        if ((pCsiInfo->port == CUSTOM_CFG_CSI_PORT_0A) || (pCsiInfo->port == CUSTOM_CFG_CSI_PORT_1A)) {
            ...
            if (pCsiMipi->dlaneNum == SENSOR_MIPI_2_LANE)
                SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_LANE_EN, dphy_rx_ld1_en) = 1;
        } else { //CUSTOM_CFG_CSI_PORT_0B/ 1B
...
void SeninfDrvImp::hal_csirx_cphy_setting(SENINF_CSI_MIPI *pCsiMipi)
{
    SENINF_CSI_INFO *pCsiInfo = pCsiMipi->pCsiInfo;
    csi0_cphy_top_reg_REGS *pCsi0_Cphy_Top = (csi0_cphy_top_reg_REGS *)mpCSICphyTopAddr[0]; //0X1A00 3000
    switch(pCsiInfo->port) {
    ...
    if (pCsiMipi->dlaneNum == SENSOR_MIPI_3_LANE) { //cphy-3trio
    ...
    } else if (pCsiMipi->dlaneNum == SENSOR_MIPI_2_LANE) { //cphy-2trio
        if ((pCsiInfo->port == CUSTOM_CFG_CSI_PORT_0A) || (pCsiInfo->port == CUSTOM_CFG_CSI_PORT_1A)) {
            SENINF_BITS(pCsi0_Cphy_Top, CPHY_RX_CTRL, cphy_rx_tr0_lprx_en) = 1;
            SENINF_BITS(pCsi0_Cphy_Top, CPHY_RX_CTRL, cphy_rx_tr1_lprx_en) = 1;
        } else {
...
void SeninfDrvImp::hal_csirx_phy_setting(SENINF_CSI_MIPI *pCsiMipi)
{
    /* phyA */
    hal_csirx_phyA_setting(pCsiMipi);

    if (pCsiMipi->csi_type != CSI2_2_5G_CPHY) /* phyD */
        hal_csirx_dphy_setting(pCsiMipi);
    else
        hal_csirx_cphy_setting(pCsiMipi);
}
...
int SeninfDrvImp::setSeninfCsiMipi(SENINF_CSI_MIPI *pCsiMipi)
{
    ...
    hal_csirx_phy_init(pCsiInfo->port);

    /* seninf csi2 */
    hal_csirx_seninf_csi2_setting(pCsiMipi);

    /* seninf */
    hal_csirx_seninf_setting(pCsiMipi);

    /* seninf top */
    hal_csirx_seninf_top_setting(pCsiMipi);

    /* phy */
    hal_csirx_phy_setting(pCsiMipi);
...
int SeninfDrvImp::powerOff(void *pCsi)
{
    ...
    switch(pCsiInfo->port) {
    case CUSTOM_CFG_CSI_PORT_0:
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_0A, 0);
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_0B, 0);
        break;
    case CUSTOM_CFG_CSI_PORT_1:
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_1A, 0);
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_1B, 0);
        break;
    case CUSTOM_CFG_CSI_PORT_2:
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_2A, 0);
        hal_csirx_phyA_power_on(CUSTOM_CFG_CSI_PORT_2B, 0);
        break;
    case CUSTOM_CFG_CSI_PORT_0A:
    case CUSTOM_CFG_CSI_PORT_0B:
    case CUSTOM_CFG_CSI_PORT_1A:
    case CUSTOM_CFG_CSI_PORT_1B:
        hal_csirx_phyA_power_on(pCsiInfo->port, 0);
    default:
        break;
    }
...
static MUINT32 hw_deskew_result[6][3];
static MUINT32 sw_deskew_result[6][3];

void SeninfDrvImp::hal_csirx_dphy_get_dbg_value(CUSTOM_CFG_CSI_PORT mipiPort, unsigned int dbg_mux, unsigned int lane_number, unsigned int *value)
{
    csi0_dphy_top_reg_REGS *pCsi0_Dphy_Top = (csi0_dphy_top_reg_REGS *)mpCSIDphyTopAddr[0];
    ...
    if (lane_number == 0)
        SENINF_BITS(pCsi0_Dphy_Top, CDPHY_DEBUG_PORT_SELECT, rg_csi_cdphy_dbg_port_sel) = 0x03020100;
    else if (lane_number == 1)
        SENINF_BITS(pCsi0_Dphy_Top, CDPHY_DEBUG_PORT_SELECT, rg_csi_cdphy_dbg_port_sel) = 0x1b1a1918;
    else if (lane_number == 2)
        SENINF_BITS(pCsi0_Dphy_Top, CDPHY_DEBUG_PORT_SELECT, rg_csi_cdphy_dbg_port_sel) = 0x33323130;
    else if (lane_number == 3)
        SENINF_BITS(pCsi0_Dphy_Top, CDPHY_DEBUG_PORT_SELECT, rg_csi_cdphy_dbg_port_sel) = 0x4b4a4948;
    else if (lane_number == 4)
        SENINF_BITS(pCsi0_Dphy_Top, CDPHY_DEBUG_PORT_SELECT, rg_csi_cdphy_dbg_port_sel) = 0x63626160;
    else
        SENINF_BITS(pCsi0_Dphy_Top, CDPHY_DEBUG_PORT_SELECT, rg_csi_cdphy_dbg_port_sel) = 0x7b7a7978;

    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_DBG_MUX, rg_dphy_rx_deskew_dbg_mux) = dbg_mux;

    *value = SENINF_READ_REG(pCsi0_Dphy_Top, CDPHY_DEBUG_OUT_READ);
    LOG_MSG("lane_number = %d, dbg_mux = %d, *value = 0x%0x\n",
        lane_number, dbg_mux, *value);
}

void SeninfDrvImp::hal_csirx_dphy_dbg_deskew_result(CUSTOM_CFG_CSI_PORT mipiPort, unsigned int *pResult, unsigned int deskew_lane)
{
    unsigned int delay_code;
    unsigned int code_window;

    /* HSRX delay code, code_begin, code_end */
    hal_csirx_dphy_get_dbg_value(mipiPort, 0x1, deskew_lane, &delay_code);
    hal_csirx_dphy_get_dbg_value(mipiPort, 0x8, deskew_lane, &code_window);
    pResult[0] = (code_window >> 8) & 0x3F;		// window begin
    pResult[1] = (delay_code >> 24) & 0x3F;		// delay code
    pResult[2] = (code_window >> 16) & 0x3F;	// window end
    LOG_MSG("HW deskew result : (%d,%d,%d)\n", pResult[0], pResult[1], pResult[2]);
    LOG_MSG("mipiPort = %d, deskew_lane = %d\n", mipiPort, deskew_lane);
}

void SeninfDrvImp::hal_csirx_ana_get_dbg_value(CUSTOM_CFG_CSI_PORT mipiPort, unsigned int dbg_mux, unsigned int *value_a, unsigned int *value_b)
{
    mipi_rx_ana_cdphy_csi0a_reg_REGS *pMipiRx_ana_csiA = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[CUSTOM_CFG_CSI_PORT_0A];
    mipi_rx_ana_cdphy_csi0a_reg_REGS *pMipiRx_ana_csiB = (mipi_rx_ana_cdphy_csi0a_reg_REGS *)mpCSI2RxAnaBaseAddr[CUSTOM_CFG_CSI_PORT_0B];
    ...
    SENINF_BITS(pMipiRx_ana_csiA, MIPI_RX_WRAPPER80_CSI0A, CSR_CSI_MON_MUX) = dbg_mux;
    SENINF_BITS(pMipiRx_ana_csiB, MIPI_RX_WRAPPER80_CSI0A, CSR_CSI_MON_MUX) = dbg_mux;

    *value_a = SENINF_READ_REG(pMipiRx_ana_csiA, MIPI_RX_WRAPPER84_CSI0A);
    *value_b = SENINF_READ_REG(pMipiRx_ana_csiB, MIPI_RX_WRAPPER84_CSI0A);

    LOG_MSG("MIPI port: %d, MIPI_RX_WRAPPER84_CSI0A A:0x%x , B:0x%x\n",
        mipiPort, *value_a, *value_b);
}

void SeninfDrvImp::hal_csirx_dphy_dbg_deskew_ana_delay_code(CUSTOM_CFG_CSI_PORT mipiPort, unsigned int deskew_lane)
{
    unsigned int value_a = 0, value_b = 0;
    unsigned int delay_code = 0;

    hal_csirx_ana_get_dbg_value(mipiPort, 4, &value_a, &value_b);

    if (deskew_lane == 0)
        delay_code = (value_a >> 24) & 0x3F;
    else if (deskew_lane == 1)
        delay_code = (value_a >> 16) & 0x3F;
    else if (deskew_lane == 2)
        delay_code = (value_a >> 8) & 0x3F;
    else if (deskew_lane == 3)
        delay_code = (value_b >> 24) & 0x3F;
    else if (deskew_lane == 4)
        delay_code = (value_b >> 16) & 0x3F;
    else if (deskew_lane == 5)
        delay_code = (value_b >> 8) & 0x3F;

    LOG_MSG("ana delay code : %d\n", delay_code);
}

#define SKEW_START 0
#define SKEW_APPLIED 1
#define SKEW_END 2
int SeninfDrvImp::hal_csirx_work_on_deskew_result(unsigned int *deskew_data, unsigned int *pResult)
{
    int k, m;
    u32 code_current;
    u32 code_pos;
    ...
    pResult[SKEW_START] = 0;
    pResult[SKEW_APPLIED] = 0;
    pResult[SKEW_END] = 0;
    ...
    for (k=0; k<4; k++) {
        for (m=30; m>=0; m-=2, code_pos += 1) {
            code_current = ((deskew_data[k]) >> m) & 0x3;
            /* Find init code_target */
            if (code_target == 0) {
                if (code_current == 0x1 || code_current == 0x2) {
                    code_target = code_current ^ 0x3;
                }
                continue;
            }

            /* Find window */
            if (code_current == code_target) {
                if (pResult[SKEW_START] == 0) {
                    pResult[SKEW_START] = code_pos;
                    code_window = code_current;
                }
            }
            else {
                if (pResult[SKEW_START] != 0) {
                    pResult[SKEW_END] = code_pos - 1;
                    if ((pResult[SKEW_END] - pResult[SKEW_START]) > eye_open_target) {
                        pResult[SKEW_APPLIED] = (pResult[SKEW_END] + pResult[SKEW_START]) >> 1;
                        return 0;
                    }
                    pResult[SKEW_START] = 0;
                    pResult[SKEW_END] = 0;
                    code_target = code_window ^ 0x3;
                }
            }
        }
    }
...
unsigned int deskew_data[6][4];
void SeninfDrvImp::hal_csirx_dphy_dbg_deskew_raw_data(CUSTOM_CFG_CSI_PORT mipiPort, unsigned int *pResult, unsigned int deskew_lane)
{
    unsigned int i;

    /* get deskew raw data */
    for (i=0; i<4; i++) {
        hal_csirx_dphy_get_dbg_value(mipiPort, 0x2 + i, deskew_lane, &deskew_data[deskew_lane][i]);
    }

    /* sw calculate deskew window/delay code */
    hal_csirx_work_on_deskew_result(&deskew_data[deskew_lane][0], pResult);

    LOG_MSG("deskew raw data = %x%x%x%x, eye window(%d,%d,%d)\n",
        deskew_data[deskew_lane][0], deskew_data[deskew_lane][1], deskew_data[deskew_lane][2], deskew_data[deskew_lane][3],
        pResult[0], pResult[1], pResult[2]);
}
...
int SeninfDrvImp::dphy_hw_deskew_en(CUSTOM_CFG_CSI_PORT mipiPort, MUINT8 enable)
{
    ...
    unsigned int deskew_print = 0; /*print debug log*/
    int count = 0;

    while(1) {
        status = SENINF_READ_REG(pCsi0_Dphy_Top, DPHY_RX_DESKEW_IRQ_STATUS) & 0XFFFF;
        if (status)
            break;
        else
            LOG_MSG("status=0\n");
    ...
    LOG_MSG("deskew status: 0x%x \n", status);
    if (!status)
        return 0;

    /* check deskew irq behavior */
    if (!(status == 0x1111 || status == 0x11 || status == 0x1))
        LOG_MSG("status:0x%x deskew err!\n", status);

    SENINF_BITS(pCsi0_Dphy_Top, DPHY_RX_DESKEW_IRQ_STATUS, ro_dphy_rx_deskew_irq_status) = 0xFFFF;
    SENINF_BITS(pCsi0_Dphy_Top, CDPHY_DEBUG_MODE_SELECT, rg_csi_cdphy_dbg_mod_sel) = 0x1111;

    /* D0 */
    hal_csirx_dphy_dbg_deskew_result(mipiPort, &hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][0], MTK_CSIRX_DESKEW_LANE_A_1);
    hal_csirx_dphy_dbg_deskew_ana_delay_code(mipiPort, MTK_CSIRX_DESKEW_LANE_A_1);
    hal_csirx_dphy_dbg_deskew_raw_data(mipiPort, &sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][0], MTK_CSIRX_DESKEW_LANE_A_1);

    /* D1 */
    hal_csirx_dphy_dbg_deskew_result(mipiPort, &hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][0], MTK_CSIRX_DESKEW_LANE_B_0);
    hal_csirx_dphy_dbg_deskew_ana_delay_code(mipiPort, MTK_CSIRX_DESKEW_LANE_B_0);
    hal_csirx_dphy_dbg_deskew_raw_data(mipiPort, &sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][0], MTK_CSIRX_DESKEW_LANE_B_0);

    /* D2 */
    hal_csirx_dphy_dbg_deskew_result(mipiPort, &hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][0], MTK_CSIRX_DESKEW_LANE_A_0);
    hal_csirx_dphy_dbg_deskew_ana_delay_code(mipiPort, MTK_CSIRX_DESKEW_LANE_A_0);
    hal_csirx_dphy_dbg_deskew_raw_data(mipiPort, &sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][0], MTK_CSIRX_DESKEW_LANE_A_0);

    /* D3 */
    hal_csirx_dphy_dbg_deskew_result(mipiPort, &hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][0], MTK_CSIRX_DESKEW_LANE_B_1);
    hal_csirx_dphy_dbg_deskew_ana_delay_code(mipiPort, MTK_CSIRX_DESKEW_LANE_B_1);
    hal_csirx_dphy_dbg_deskew_raw_data(mipiPort, &sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][0], MTK_CSIRX_DESKEW_LANE_B_1);

    if (deskew_print) {
        LOG_MSG("Lane0 (A1):\n");
        LOG_MSG("HW deskew result : (%d,%d,%d)\n",
            hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][0], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][1], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][2]);
        LOG_MSG("SW deskew result : (%d,%d,%d) %x%x%x%x\n",
            sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][0], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][1], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_1][2],
            deskew_data[MTK_CSIRX_DESKEW_LANE_A_1][0], deskew_data[MTK_CSIRX_DESKEW_LANE_A_1][1], deskew_data[MTK_CSIRX_DESKEW_LANE_A_1][2], deskew_data[MTK_CSIRX_DESKEW_LANE_A_1][3]);

        LOG_MSG("Lane1 (B0):\n");
        LOG_MSG("HW deskew result : (%d,%d,%d)\n",
            hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][0], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][1], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][2]);
        LOG_MSG("SW deskew result : (%d,%d,%d) %x%x%x%x\n",
            sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][0], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][1], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_0][2],
            deskew_data[MTK_CSIRX_DESKEW_LANE_B_0][0], deskew_data[MTK_CSIRX_DESKEW_LANE_B_0][1], deskew_data[MTK_CSIRX_DESKEW_LANE_B_0][2], deskew_data[MTK_CSIRX_DESKEW_LANE_B_0][3]);

        LOG_MSG("Lane2 (A0):\n");
        LOG_MSG("HW deskew result : (%d,%d,%d)\n",
            hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][0], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][1], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][2]);
        LOG_MSG("SW deskew result : (%d,%d,%d) %x%x%x%x\n",
            sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][0], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][1], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_A_0][2],
            deskew_data[MTK_CSIRX_DESKEW_LANE_A_0][0], deskew_data[MTK_CSIRX_DESKEW_LANE_A_0][1], deskew_data[MTK_CSIRX_DESKEW_LANE_A_0][2], deskew_data[MTK_CSIRX_DESKEW_LANE_A_0][3]);

        LOG_MSG("Lane3 (B1):\n");
        LOG_MSG("HW deskew result : (%d,%d,%d)\n",
            hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][0], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][1], hw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][2]);
        LOG_MSG("SW deskew result : (%d,%d,%d) %x%x%x%x\n",
            sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][0], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][1], sw_deskew_result[MTK_CSIRX_DESKEW_LANE_B_1][2],
            deskew_data[MTK_CSIRX_DESKEW_LANE_B_1][0], deskew_data[MTK_CSIRX_DESKEW_LANE_B_1][1], deskew_data[MTK_CSIRX_DESKEW_LANE_B_1][2], deskew_data[MTK_CSIRX_DESKEW_LANE_B_1][3]);
    }

    return 0;
}

int SeninfDrvImp::setAutoDeskew(SENINF_CSI_MIPI *pCsiMipi)
{
    int ret = 0;
...
#define CSI_AUTO_DESKEW_THRESHOLD 1427*1000000 /*5642x4224x240x10= 1427_374080(1.5Gbps)*/

    int bpp;

    switch(pCsiMipi->inDataType) {
    case RAW_8BIT_FMT:
        bpp = 8;
        break;
    case RAW_10BIT_FMT:
    ...
    LOG_MSG("MIPI PORT %d, mipi_pixel_rate %d, u1MIPILaneNum %d, bpp %d\n",
        pCsiInfo->port,
        pCsiMipi->mipi_pixel_rate,
        (pCsiMipi->dlaneNum)+1,
        bpp);

	LOG_MSG("speed = %ld, auto deskew threshold = %ld\n",
			(pCsiMipi->mipi_pixel_rate/1000 * bpp / ((pCsiMipi->dlaneNum)+1)),
			CSI_AUTO_DESKEW_THRESHOLD/1000);

    if ((pCsiMipi->mipi_pixel_rate / 1000 * bpp / ((pCsiMipi->dlaneNum)+1)) > CSI_AUTO_DESKEW_THRESHOLD/1000) {
        LOG_MSG("ENABLE dphy_hw_deskew_en\n");
        dphy_hw_deskew_en(pCsiInfo->port, 1);
    }
