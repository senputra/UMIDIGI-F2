   ...
   // gIspDrvObj[CAM_A].m_pIspHwRegAddr = (MUINT32 *) mmap(0, CAM_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, CAM_A_BASE_HW);
    ...
    if (fd < 0) { /* Use v2 device node if v1 device node is removed */
        ...
        if (ret < 0) {
            ...
            mCSI[0] = devinfo.data[103];//0x11ce018c
            mCSI[1] = devinfo.data[104];//0x11ce0190
            mCSI[2] = devinfo.data[111];//0x11ce01ac
            mCSI[3] = devinfo.data[112];//0x11ce01b0
        }
        LOG_MSG("Efuse Data:0x11ce018c= 0x%x, 0x11ce0190= 0x%x, 0x11ce01ac= 0x%x, 0x11ce01b0= 0x%x\n", mCSI[0], mCSI[1], mCSI[2], mCSI[3]);
        close(fd);
    }
#endif
...
int SeninfDrvImp::configInputForCSI(SENINF_CONFIG_STRUCT *pseninfPara, SensorDynamicInfo *psensorDynamicInfo,
    ...
    if(enable) {
        ...
        /* set integration */
        setIntegration(pseninfPara->sensor_idx, SENINF_MUX_selected, pseninfPara, sensorInfo);
    }
    ...
    if (pseninfPara->csi_para.mipi_pixel_rate != 0) {
        ...
        LOG_MSG("clklevelcnt:%d, traget_clk:%d, mipi pixel rate %lu ",
  ...
  //  psensorDynamicInfo->PDAFPixelMode = HALSENSOR_CAMSV_PIXEL_MODE;
    ...
    switch(pseninfPara->inSrcTypeSel) {
        case MIPI_SENSOR:
            ...
            configInputForCSI(pseninfPara, psensorDynamicInfo, sensorInfo, enable);
            break;
        default:
...
int SeninfDrvImp::calibrateSensor(SENINF_CONFIG_STRUCT *pseninfPara){
    ...
    if (pseninfPara->inSrcTypeSel == MIPI_SENSOR){
        ...
        if(pseninfPara->csi_para.mipi_pixel_rate == 0) {
            ...
            LOG_MSG("pixel_rate %lu, threshold %lu, h %d, w %d, fps %d,pdaf mode %d",
            ...
            LOG_MSG("pixel_rate from driver %lu, threshold %lu, h %d, w %d, fps %d,pdaf mode %d",
                     pixel_rate, csi_skew_threshold, pseninfPara->cropHeight, pseninfPara->cropWidth, pseninfPara->frameRate, pseninfPara->PDAFMode);
            pseninfPara->csi_para.mipi_pixel_rate = ((pixel_rate * bpp)/(pseninfPara->csi_para.dlaneNum+1));
        }

        LOG_MSG("mipi_pixel_rate %lu CSI_DESKEW_THRESHOLD %lu\n",
...
int SeninfDrvImp::setIntegration(IMGSENSOR_SENSOR_IDX sensorIdx, SENINF_MUX_ENUM mux,
            SENINF_CONFIG_STRUCT *psensorPara, ACDK_SENSOR_INFO2_STRUCT *sensorInfo)
{
    struct HW_INTEGRATION_STRUCT config;

    config.cropWidth = psensorPara->cropWidth;
    config.cropHeight = psensorPara->cropHeight;
    config.frameRate = psensorPara->frameRate;
    config.SensroInterfaceType = sensorInfo->SensroInterfaceType;
    config.SensorOutputDataFormat = sensorInfo->SensorOutputDataFormat;
    config.SensorMIPILaneNumber = sensorInfo->SensorMIPILaneNumber;
    config.MIPIsensorType = sensorInfo->MIPIsensorType;
