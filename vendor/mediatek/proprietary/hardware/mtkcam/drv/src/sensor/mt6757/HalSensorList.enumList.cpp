...
enumerateSensor_Locked()
{
    ...
    if  ( (iSensorsList & SENSOR_DEV_SUB_2) == SENSOR_DEV_SUB_2 )
    {
        halSensorDev = SENSOR_DEV_SUB_2;
        pSensorInfo = pSensorDrv->getSub2SensorInfo();
        addAndInitSensorEnumInfo_Locked(halSensorDev, ret_count, mapToSensorType(pSensorInfo->GetType()), pSensorInfo->getDrvMacroName());//ToDo: modify argument
...
#endif
    pSeninfDrv->setMclk1(0, 1, 1, 0, 1, 0, 0, 0);
    pSeninfDrv->setMclk2(0, 1, 1, 0, 1, 0, 0, 0);
...
)
{
    ...
    SensorStaticInfo     *pSensorStaticInfo = &sensorStaticInfo[uSensorIndex];
    ...
    if((eSensorDev & SENSOR_DEV_MAIN_2) == SENSOR_DEV_MAIN_2)
       ...
       u4sensorIndex = 1;
#ifdef MTK_SUB2_IMGSENSOR
    else if((eSensorDev & SENSOR_DEV_SUB_2) == SENSOR_DEV_SUB_2)
       u4sensorIndex = 3;
#endif
    else
       u4sensorIndex = 0;
    buildStaticInfo(Info(u4sensorIndex, eSensorType, bBackSide, szSensorDrvName), rEnumInfo.mMetadata);
    //
    return  &rEnumInfo;
}
