...
MUINT32 sensorDrvInit[KDIMGSENSOR_MAX_INVOKE_DRIVERS] = {0,0,0};
#else
...
#ifdef MTK_SUB2_IMGSENSOR
    memset(&m_SenosrResInfo[2], 0, sizeof(ACDK_SENSOR_RESOLUTION_INFO_STRUCT));
#endif
    //
...
#ifdef MTK_SUB2_IMGSENSOR
    memset((void*)&m_sub2SensorDrv, 0xFF, sizeof(SENSOR_DRIVER_LIST_T));
    m_sub2SensorDrv.number = 0;
    memset(&m_sensorInfo[2], 0, sizeof(ACDK_SENSOR_INFO_STRUCT));
#endif
...
ImgSensorDrv::impSearchSensor(pfExIdChk pExIdChkCbf)
{
        ...
        for (i = 0; i < MAX_NUM_OF_SUPPORT_SENSOR; i++) {
            ...
            socketPos = this->getSocketPosition((CAMERA_DUAL_CAMERA_SENSOR_ENUM)SensorEnum);
                ...
                break;
...
#ifdef MTK_SUB2_IMGSENSOR
                     else if (SensorEnum == DUAL_CAMERA_SUB_2_SENSOR) {
                        //m_subSensorIdx = i;
                        //m_subSensorId = m_pstSensorInitFunc[m_subSensorIdx].SensorId;
                        m_sub2SensorDrv.index[m_sub2SensorDrv.number] = i;
                        m_sub2SensorDrv.type[m_sub2SensorDrv.number] = sensorType;
                        if ( IMAGE_SENSOR_TYPE_RAW == sensorType && BAD_SENSOR_INDEX == m_sub2SensorDrv.firstRawIndex ) {
                            m_sub2SensorDrv.firstRawIndex = i;
                        }
                        else if ( IMAGE_SENSOR_TYPE_YUV == sensorType && BAD_SENSOR_INDEX == m_sub2SensorDrv.firstYuvIndex ) {
                            m_sub2SensorDrv.firstYuvIndex = i;
                        }
                        m_sub2SensorDrv.position = socketPos;
                        m_sub2SensorDrv.sensorID = m_pstSensorInitFunc[m_sub2SensorDrv.index[m_sub2SensorDrv.number]].SensorId;
                        //LOG_MSG("SUB sensor m_subSensorDrv.number=%d, m_subSensorDrv.index=%d\n",m_subSensorDrv.number,m_subSensorDrv.index[m_subSensorDrv.number]);
                        m_sub2SensorDrv.number++;
                        //
                        m_pSub2SensorInfo = m_pstSensorInitFunc[i].pSensorInfo;
                        if  ( m_pSub2SensorInfo )
                        {
                            NSFeature::SensorInfoBase* pSensorInfo = m_pstSensorInitFunc[i].pSensorInfo;
                            LOG_MSG("found <%#x/%s/%s>", pSensorInfo->GetID(), pSensorInfo->getDrvName(), pSensorInfo->getDrvMacroName());
                        }
                        ...
                        {
                            LOG_WRN("m_pSub2SensorInfo==NULL\n");
                        }
                        LOG_MSG("SUB2 sensor found:[%d]/[0x%x]/[%d]/[%d] \n",i,id[KDIMGSENSOR_INVOKE_DRIVER_0],sensorType,socketPos);
                        break;
                    }
#endif
        }//
...
#ifdef MTK_SUB2_IMGSENSOR
     if (BAD_SENSOR_INDEX != m_sub2SensorDrv.index[0]) {
        m_sub2SensorId = m_sub2SensorDrv.sensorID;
        //init to choose first
        m_sub2SensorIdx = m_sub2SensorDrv.index[0];
        sensorDevs |= SENSOR_SUB_2;
    }
#endif
    ...
    //
#ifdef MTK_SUB2_IMGSENSOR
    LOG_MSG("SENSOR search end: 0x%x /[0x%x][%d]/[0x%x][%d]/[0x%x][%d]/[0x%x][%d]\n", sensorDevs,
    m_mainSensorId,m_mainSensorIdx,m_main2SensorId,m_main2SensorIdx,m_subSensorId,m_subSensorIdx,m_sub2SensorId,m_sub2SensorIdx);
#else
...
#define IMGSNESOR_FILL_SET_DRIVER_INFO(a) do{ \
        ...
        if ( DUAL_CAMERA_SUB_2_SENSOR & a ) {  \
            sensorDrvInit[KDIMGSENSOR_INVOKE_DRIVER_2] = \
                    (DUAL_CAMERA_SUB_2_SENSOR << KDIMGSENSOR_DUAL_SHIFT) | m_sub2SensorIdx;   \
        }   \
    }while(0);
#else
#define IMGSNESOR_FILL_SET_DRIVER_INFO(a) do{ \
...
)
{
...
#ifdef MTK_SUB2_IMGSENSOR
    ACDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResInfo[3];
#else
    ...
    LOG_MSG("[init] mUsers = %d, sensorIdx = %d\n", mUsers, sensorIdx);
...
#ifdef MTK_SUB2_IMGSENSOR
    pSensorResInfo[2] = &m_SenosrResInfo[2];
#endif
#ifndef MTK_SUB2_IMGSENSOR
...
#ifdef MTK_SUB2_IMGSENSOR
    if(SENSOR_SUB_2 & sensorIdx ) {
            sensorDevId = SENSOR_SUB_2;
            ...
            if (FeaturePara32) {
                //in setting domain, use preview line time only
                //sensor drv will convert to capture line time when setting to capture mode.
                m_LineTimeInus[2] = (MUINT32)(((MUINT64)pFeaturePara16[2]*1000000+(FeaturePara32>>1))/FeaturePara32);
            }
            LOG_MSG("[init]: m_LineTimeInus[2] = %d\n", m_LineTimeInus[2] );
        }
#endif
...
#ifdef MTK_SUB2_IMGSENSOR
        sensorDrvInit[KDIMGSENSOR_INVOKE_DRIVER_2] = 0;
#endif
        if (m_fdSensor > 0) {
            ::close(m_fdSensor);
        }
...
ImgSensorDrv::setScenario(SENSOR_DRIVER_SCENARIO_T scenarioconf)
{
    ...
    LOG_MSG("[setScenario]: setScenario\n");
            ...
            if (FeaturePara32) {
                if(FeaturePara32 >= 1000) {
                    if(SENSOR_MAIN == scenarioconf.sensorDevId) {
                        ...
                        m_MinFrameLength[0] = (MUINT32)pFeaturePara16[1];
#ifdef MTK_SUB2_IMGSENSOR
                    } else if (SENSOR_SUB_2 == scenarioconf.sensorDevId){
                        m_LineTimeInus[2] = (MUINT32)(((MUINT64)pFeaturePara16[0]*1000000 + ((FeaturePara32/1000)-1))/(FeaturePara32/1000));   // 1000 base , 33657 mean 33.657 us
                        m_MinFrameLength[2] = (MUINT32)pFeaturePara16[1];
#endif
                    }
                    else{
                        ...
                        m_MinFrameLength[1] = (MUINT32)pFeaturePara16[1];
                    }
...
#ifdef MTK_SUB2_IMGSENSOR
                LOG_MSG("[setScenario]DevID = %d, m_LineTimeInus[0] = %d, m_LineTimeInus[1] = %d, m_LineTimeInus[2] = %d Scenario id = %d, PixelClk = %d, PixelInLine = %d, Framelength = %d\n",
                          tempDevId[i], m_LineTimeInus[0], m_LineTimeInus[1], m_LineTimeInus[2], tempId[i], FeaturePara32, pFeaturePara16[0], pFeaturePara16[1]);
#else
                LOG_MSG("[setScenario]DevID = %d, m_LineTimeInus[0] = %d, m_LineTimeInus[1] = %d Scenario id = %d, PixelClk = %d, PixelInLine = %d, Framelength = %d\n",
                          tempDevId[i], m_LineTimeInus[0],m_LineTimeInus[1], tempId[i], FeaturePara32, pFeaturePara16[0], pFeaturePara16[1]);
#endif
            }
...
)
{
...
#ifdef MTK_SUB2_IMGSENSOR
    MUINT32 scenarioId[3] = {SENSOR_SCENARIO_ID_NORMAL_PREVIEW,SENSOR_SCENARIO_ID_NORMAL_PREVIEW};
#else
    ...
    if(SENSOR_MAIN == sensorDevId) {
        ...
        if ( SENSOR_PREVIEW_DELAY == mode) {
            return m_psensorInfo[2]->PreviewDelayFrame;
        }
        else if (SENSOR_VIDEO_DELAY == mode) {
            return m_psensorInfo[2]->VideoDelayFrame;
        }
        else if (SENSOR_CAPTURE_DELAY == mode) {
            return m_psensorInfo[2]->CaptureDelayFrame;
        }
        else if (SENSOR_YUV_AWB_SETTING_DELAY == mode) {
            return m_psensorInfo[2]->YUVAwbDelayFrame;
        }
        else if (SENSOR_YUV_EFFECT_SETTING_DELAY == mode) {
            return m_psensorInfo[2]->YUVEffectDelayFrame;
        }
        else if (SENSOR_AE_SHUTTER_DELAY == mode) {
            return m_psensorInfo[2]->AEShutDelayFrame;
        }
        else if (SENSOR_AE_GAIN_DELAY == mode) {
            return m_psensorInfo[2]->AESensorGainDelayFrame;
        }
        else if (SENSOR_AE_ISP_DELAY == mode) {
            return m_psensorInfo[2]->AEISPGainDelayFrame;
        }
        else {
            return 0;
        }
...
//#define SENDCMD_LOG // Open sendcommand log, for test only
    ...
    case CMD_SENSOR_SET_SENSOR_EXP_TIME:
        ...
        if(SENSOR_MAIN == sensorDevId) {
            LineTimeInus = m_LineTimeInus[0];
        }  else if((SENSOR_MAIN_2 == sensorDevId)||(SENSOR_SUB == sensorDevId)) {
            LineTimeInus =  m_LineTimeInus[1];
#ifdef MTK_SUB2_IMGSENSOR
        } else if(SENSOR_SUB_2 == sensorDevId) {
            LineTimeInus =  m_LineTimeInus[2];
#endif
        } else {
    ...
    case CMD_SENSOR_SET_SENSOR_EXP_FRAME_TIME:
        ...
        if(SENSOR_MAIN == sensorDevId) {
            LineTimeInus = m_LineTimeInus[0];
            MinFrameLength = m_MinFrameLength[0];
        }  else if((SENSOR_MAIN_2 == sensorDevId)||(SENSOR_SUB == sensorDevId)) {
            LineTimeInus =  m_LineTimeInus[1];
            MinFrameLength = m_MinFrameLength[1];
#ifdef MTK_SUB2_IMGSENSOR
        } else if(SENSOR_SUB_2 == sensorDevId) {
            LineTimeInus =  m_LineTimeInus[2];
            MinFrameLength = m_MinFrameLength[2];
#endif
        } else {
            LOG_ERR("sensorDevId is incorrect!! \n");
            break;
        ...
        LOG_MSG("[SET_SENSOR_EXP_FRAME_TIME] ExpTime/FrmTime=(%u/%u) ExpLine/FrmLength/LineTimeInus/MinFrmLength=(%llu/%llu/%u/%u)\n", \
                *parg1, *parg2, FeaturePara[0], FeaturePara[1], LineTimeInus, MinFrameLength);
	
        if(MinFrameLength > FeaturePara[1]) {
            LOG_ERR("[SET_SENSOR_EXP_FRAME_TIME] Error: Set framelength (%llu) is smaller than MinFrmLength (%u)!!!", FeaturePara[1], MinFrameLength);
...
case CMD_SENSOR_SET_SENSOR_EXP_TIME_BUF_MODE:
...
#endif
        break;
       case CMD_SENSOR_SET_SENSOR_SET_WAKE_LOCK:
        FeatureId = 	SENSOR_FEATURE_SET_WAKE_LOCK;
        FeaturePara[0] = *parg1;
        FeatureParaLen = sizeof(MUINT64);
        pFeaturePara =	(MUINT8*)FeaturePara;
#if 1
        LOG_MSG("[CMD_SENSOR_SET_SENSOR_SET_WAKE_LOCK] FeaturePara[0] = %llu, FeaturePara[1] = %llu, FeaturePara[2] = %llu, FeaturePara[3] = %llu \n", FeaturePara[0], FeaturePara[1], FeaturePara[2], FeaturePara[3]);
#endif
        break;
	...
	pFeaturePara =	(MUINT8*)FeaturePara;
	if(*parg1 != 0) {
		LOG_MSG("SET_STREAMING_RESUME, senDev(%d),expTime_us(%6d),LineTimeInus(%d),expLine(%d)\n", \
			sensorDevId, (MUINT32)*parg1, LineTimeInus, FeaturePara[0]);
	} else {
		LOG_MSG("SET_STREAMING_RESUME, senDev(%d),expTime_us(%6d) is default, keep original value\n", \
			sensorDevId, (MUINT32)*parg1);
	}
	break;
...
#ifdef MTK_SUB2_IMGSENSOR
            case SENSOR_SUB_2:
                *parg1 = getSensorPadPclkInv(eDevId_ImgSensor3);
                err = 0;
                break;
#endif
...
#ifdef MTK_SUB2_IMGSENSOR
            case SENSOR_SUB_2:
                *parg1Addr = (MUINTPTR)m_pSub2SensorInfo;
                err = 0;
                break;
#endif
...
#ifdef MTK_SUB2_IMGSENSOR
             case SENSOR_SUB_2:
                 *parg1 = isRetFakeSub2Orientation();
                 err = 0;
                 break;
#endif
             default:
    ...
    case CMD_SENSOR_GET_SENSOR_VC_INFO:
	 ...
	 case CMD_SENSOR_GET_SENSOR_PDAF_REG_SETTING:
		...
		FeatureParaLen = sizeof(MUINT64) * 2;
		pFeaturePara = (MUINT8*)FeaturePara;
		LOG_MSG("[CMD_SENSOR_GET_SENSOR_PDAF_REG_SETTING] 0x%llu 0x%llu 0x%llu", FeaturePara[0], FeaturePara[1]);
		break;
	 case CMD_SENSOR_SET_SENSOR_PDAF_REG_SETTING:
		...
		FeatureParaLen = sizeof(MUINT64) * 2;
		pFeaturePara = (MUINT8*)FeaturePara;
		LOG_MSG("[CMD_SENSOR_SET_SENSOR_PDAF_REG_SETTING] 0x%llu 0x%llu 0x%llu", FeaturePara[0], FeaturePara[1]);
		break;
    ...
    case CMD_SENSOR_SET_IHDR_SHUTTER_GAIN:
        ...
        FeaturePara[0] = (1000 * (*parg1)) / LineTimeInus;
        FeaturePara[1] = (1000 * (*parg2)) / LineTimeInus;
        if(FeaturePara[0] == 0) {   // avoid the line number to zero
            LOG_MSG("[CMD_SENSOR_SET_IHDR_SHUTTER_GAIN] LineTimeInus = %llu %d\n", FeaturePara[0], LineTimeInus);
            FeaturePara[0] = 1;
        }
        if(FeaturePara[1] == 0) {   // avoid the line number to zero
            LOG_MSG("[CMD_SENSOR_SET_IHDR_SHUTTER_GAIN] LineTimeInus = %llu %d\n", FeaturePara[1], LineTimeInus);
            FeaturePara[1] = 1;
        }
...
#ifdef MTK_SUB2_IMGSENSOR
    MUINT32 ScenarioId[3],
    ACDK_SENSOR_INFO_STRUCT *pSensorInfo[3],
    ACDK_SENSOR_CONFIG_STRUCT *pSensorConfigData[3]
#else
...
#ifdef MTK_SUB2_IMGSENSOR
    ACDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution[3]
#else
...
#ifdef MTK_SUB2_IMGSENSOR
    ACDK_SENSOR_PRESOLUTION_STRUCT resolution = {{pSensorResolution[0], pSensorResolution[1], pSensorResolution[2]}};
    LOG_WRN("pSensorResolution[0]:%p, [1]:%p [2]:%p\n", pSensorResolution[0], pSensorResolution[1], pSensorResolution[2]);
#else
...
)
{
...
#ifdef MTK_SUB2_IMGSENSOR
    MUINT32 scenarioId[3] = {SENSOR_SCENARIO_ID_NORMAL_PREVIEW,SENSOR_SCENARIO_ID_NORMAL_PREVIEW,SENSOR_SCENARIO_ID_NORMAL_PREVIEW};
#else
    MUINT32 scenarioId[2] = {SENSOR_SCENARIO_ID_NORMAL_PREVIEW,SENSOR_SCENARIO_ID_NORMAL_PREVIEW};
#endif
    ...
    if(SENSOR_MAIN == sensorDevId ) {
        ...
        if (m_psensorInfo[2]->SensorOutputDataFormat >= SENSOR_OUTPUT_FORMAT_RAW_B &&
             m_psensorInfo[2]->SensorOutputDataFormat <= SENSOR_OUTPUT_FORMAT_RAW_R) {
            return IMAGE_SENSOR_TYPE_RAW;
        }
        if (m_psensorInfo[2]->SensorOutputDataFormat >= SENSOR_OUTPUT_FORMAT_RAW8_B &&
             m_psensorInfo[2]->SensorOutputDataFormat <= SENSOR_OUTPUT_FORMAT_RAW8_R) {
            return IMAGE_SENSOR_TYPE_RAW8;
        }
        else if (m_psensorInfo[2]->SensorOutputDataFormat >= SENSOR_OUTPUT_FORMAT_UYVY &&
                    m_psensorInfo[2]->SensorOutputDataFormat <= SENSOR_OUTPUT_FORMAT_YVYU) {
            return IMAGE_SENSOR_TYPE_YUV;
        }
        else if (m_psensorInfo[2]->SensorOutputDataFormat >= SENSOR_OUTPUT_FORMAT_CbYCrY &&
                    m_psensorInfo[2]->SensorOutputDataFormat <= SENSOR_OUTPUT_FORMAT_YCrYCb) {
            return IMAGE_SENSOR_TYPE_YCBCR;
        }
        else if (m_psensorInfo[2]->SensorOutputDataFormat >= SENSOR_OUTPUT_FORMAT_RAW_RWB_B &&
             m_psensorInfo[2]->SensorOutputDataFormat <= SENSOR_OUTPUT_FORMAT_RAW_RWB_R) {
            return IMAGE_SENSOR_TYPE_RAW;
        }
        else if (m_psensorInfo[2]->SensorOutputDataFormat == SENSOR_OUTPUT_FORMAT_RAW_MONO) {
            return IMAGE_SENSOR_TYPE_RAW;
        }
        else {
            return IMAGE_SENSOR_TYPE_UNKNOWN;
