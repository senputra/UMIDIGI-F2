...
#define CAM_ISP_RANGE 0x1000
#define CAM_MIPIRX_CONFIG_RANGE 0x100
#define CAM_MIPIRX_ANALOG_RANGE 0x2000
...
// SETTLE_DELAY_COUNT:
//  this value will be dependent to platform (Seninf Clock)
//  remember to modify this value when platform is different
#define SETTLE_DELAY_COUNT 35
/*******************************************************************************
*
********************************************************************************/
SeninfDrv*
...
int SeninfDrvImp::init()  //kk test
{
    ...
    MBOOL result;
    MINT32 cam_isp_addr = 0x1A004000;
    MINT32 mipiRx_config = 0x1a040000;
    ...
    mpSeninf2HwRegAddr = mpSeninfHwRegAddr + (0x400/4);
    mpSeninf3HwRegAddr = mpSeninfHwRegAddr + (0x800/4);
    mpSeninf4HwRegAddr = mpSeninfHwRegAddr + (0xc00/4);

    /*SenINF Mux Base address*/
    mpSeninfMux1BaseAddr = mpSeninfHwRegAddr + (0x0120/4); //1a00 8120
    mpSeninfMux2BaseAddr = mpSeninfHwRegAddr + (0x0520/4);
    mpSeninfMux3BaseAddr = mpSeninfHwRegAddr + (0x0920/4);
    mpSeninfMux4BaseAddr = mpSeninfHwRegAddr + (0x0D20/4);
    mpSeninfMux5BaseAddr = mpSeninfHwRegAddr + (0x1120/4);
    mpSeninfMux6BaseAddr = mpSeninfHwRegAddr + (0x1520/4);
    ...
    // Read CIS efuse value
    {
        ...
        else
        {
            ...
            {
                LOG_ERR("Get Devinfo Data Fail:%d\n", ret);
            }
            else
            {
                mCSI1 = devinfo.data[16];
                mCSI2 = devinfo.data[17];
            }
            LOG_MSG("Efuse Data:0x%x,0x%x\n", mCSI1, mCSI2);
...
)
{
    ...
    LOG_MSG("[setTg2Phase]pcEn(%d), clkPol(%d), mMclk2User(%d), TimestampClk(%d)\n",
    (MINT32)pcEn, (MINT32)clkPol, mMclk2User, (MINT32)TimestampClk);
...
)
{
 ...
 #endif
    LOG_MSG("[setTg3Phase]pcEn(%d), clkPol(%d), mMclk3User(%d), TimestampClk(%d)\n",
    (MINT32)pcEn, (MINT32)clkPol, mMclk3User, (MINT32)TimestampClk);
    ...
    //TM Enable 1.turn on sensor interface1"seninf_en"  2. seninf_tg_ph_cnt  bit[1:0]  set to 2'b01
    SENINF_BITS(pSeninf, SENINF_TG1_PH_CNT, TGCLK_SEL) = 1;
    SENINF_WRITE_REG(pSeninf,SENINF_TG3_TM_STP,TimestampClk);
...
{
    int ret = 0;
    static int seninf_pak_cnt[SENINF_NUM] = {0}; // record seninf pak cnt to judge whether sensor output data
    ...
    case CMD_DEBUG_TASK:
        {
             ...
             if((temp1 != 0x9004)){
                LOG_MSG("SENINF1_NCSI2_INT_STATUS(0x1A0403B4) 0x%x \n",temp1 );
             ...
             if((temp2 != 0x9004)){
	        LOG_MSG("SENINF2_NCSI2_INT_STATUS(0x1A0407B4) 0x%x \n",temp2);
            ...
            SENINF_WRITE_REG(pSeninf, SENINF1_NCSI2_DGB_SEL, 0x12);
            SENINF_WRITE_REG(pSeninf, SENINF2_NCSI2_DGB_SEL, 0x12);
            ...
            {
                /* Package count not change, same as previous deque one */
                if(temp1 == seninf_pak_cnt[SENINF_1]){
                    ret = -1;
                }
                if((SENINF_READ_REG(pSeninf,SENINF1_NCSI2_INT_STATUS) & 0xF30)!= 0x0000){
                /* sync error, crc error, frame end not sync */
                    ret = -2;
                }
            }
            ...
            {
                /* Package count not change, same as previous deque one */
                if(temp2 == seninf_pak_cnt[SENINF_2]){
                    ret = -1;
                }
                if((SENINF_READ_REG(pSeninf,SENINF2_NCSI2_INT_STATUS) & 0xF30)!= 0x0000){
                /* sync error, crc error, frame end not sync */
                    ret = -2;
                }
            }
            seninf_pak_cnt[SENINF_1] = temp1;
            seninf_pak_cnt[SENINF_2] = temp2;

            LOG_MSG("Package count: SENINF1(0x%x), SENINF2 (0x%x)\n", temp1, temp2);
	        LOG_MSG("SENINF_TOP_MUX_CTRL 0x1A040008(0x%x), getISPClk (%d) \n",
	            SENINF_READ_REG(pSeninf,SENINF_TOP_MUX_CTRL), getISPClk());

	        /*Sensor Interface Control */
            LOG_MSG("SENINF1_CTRL 0x1A040100(0x%x), SENINF2_CTRL 0x1A040500(0x%x), SENINF4_CTRL 0x1A040D00(0x%x)\n",
                 SENINF_READ_REG(pSeninf,SENINF1_CTRL), SENINF_READ_REG(pSeninf,SENINF2_CTRL), SENINF_READ_REG(pSeninf,SENINF4_CTRL));

            LOG_MSG("SENINF1_NCSI2_CTL 0x1A0403A0(0x%x), SENINF2_NCSI2_CTL 0x1A0407A0(0x%x)\n",
                 SENINF_READ_REG(pSeninf,SENINF1_NCSI2_CTL), SENINF_READ_REG(pSeninf,SENINF2_NCSI2_CTL));

	        LOG_MSG("SenINF1_IRQ 0x1A0403B4(0x%x), SenINF2_IRQ 0x1A0407B4(0x%x)\n",
                    SENINF_READ_REG(pSeninf,SENINF1_NCSI2_INT_STATUS), SENINF_READ_REG(pSeninf,SENINF2_NCSI2_INT_STATUS));
	        /*Mux0 */
	        LOG_MSG("SENINF1_MUX_CTRL 0x1A040120(0x%x), SENINF1_MUX_INTSTA 0x1A040128(0x%x), SENINF1_MUX_DEBUG_2 0x1A040134(0x%x),\n",
	        ...
	        /*Mux1 */
	        LOG_MSG("SENINF2_MUX_CTRL 0x1A040520(0x%x), SENINF2_MUX_INTSTA 0x1A040528(0x%x), SENINF2_MUX_DEBUG_2 0x1A040534(0x%x),\n",
	        ...
	        /*Mux3 for HDR*/
	        LOG_MSG("SENINF3_MUX_CTRL 0x1A040920(0x%x), SENINF3_MUX_INTSTA 0x1A040928(0x%x), SENINF3_MUX_DEBUG_2 0x1A040934(0x%x)\n",
	        ...
	        /*Mux4  for PDAF*/
	        LOG_MSG("SENINF4_MUX_CTRL 0x1A040D20(0x%x), SENINF4_MUX_INTSTA 0x1A040D28(0x%x), SENINF4_MUX_DEBUG_2 0x1A040D34(0x%x)\n",
		...
		case CMD_DEBUG_TASK_CAMSV:
        {
	        seninf_reg_t *pSeninf = (seninf_reg_t *)mpSeninfHwRegAddr;
	        /*Mux3 for HDR*/
	        LOG_MSG("(HDR)SENINF3_CTRL 0x1A040920(0x%x), IRQ 0x1A040928(0x%x), WidthHeight 0x1A040934(0x%x), Debug(0x%x)\n",
	        ...
	        SENINF_WRITE_REG(pSeninf,SENINF3_MUX_INTSTA,0xffffffff);
	        /*Mux4  for PDAF*/
	        LOG_MSG("(PDAF)SENINF4_CTRL 0x1A040D20(0x%x), IRQ 0x1A040D28(0x%x), WidthHeight 0x1A040D34(0x%x), Debug(0x%x)\n",
...
)
{
        ...
        switch (seninfTopIdx){
            case 0:
                SENINF_WRITE_REG(pSeninf,SENINF_TOP_MUX_CTRL,((temp&0xFFFFF0)|(seninfSrc&0xF)));
                break;
            case 1:
                SENINF_WRITE_REG(pSeninf,SENINF_TOP_MUX_CTRL,((temp&0xFFFF0F)|((seninfSrc&0xF)<<4)));
                break;
            case 2:
                SENINF_WRITE_REG(pSeninf,SENINF_TOP_MUX_CTRL,((temp&0xFFF0FF)|((seninfSrc&0xF)<<8)));
                break;
            case 3:
                SENINF_WRITE_REG(pSeninf,SENINF_TOP_MUX_CTRL,((temp&0xFF0FFF)|((seninfSrc&0xF)<<12)));
                break;
            case 4:
                SENINF_WRITE_REG(pSeninf,SENINF_TOP_MUX_CTRL,((temp&0xF0FFFF)|((seninfSrc&0xF)<<16)));
                break;
            case 5:
                SENINF_WRITE_REG(pSeninf,SENINF_TOP_MUX_CTRL,((temp&0x0FFFFF)|((seninfSrc&0xF)<<20)));
                break;
...
*
********************************************************************************/
int SeninfDrvImp::setSeninfVC(unsigned int SenInfsel,
    unsigned int vc0Id, unsigned int vc1Id,
    unsigned int vc2Id, unsigned int vc3Id
)
{
        int ret = 0;
        seninf_reg_t_base *pSeninf = NULL;
        unsigned int temp = 0;

        switch(SenInfsel)
        {
            case 0x01:/*Sensor interface 2*/
                pSeninf = (seninf_reg_t_base *)mpSeninf2HwRegAddr;
                break;
            case 0x02:/*Sensor interface 3*/
                pSeninf = (seninf_reg_t_base *)mpSeninf3HwRegAddr;
                break;
            case 0x03:/*Sensor interface 4*/
                pSeninf = (seninf_reg_t_base *)mpSeninf4HwRegAddr;
                break;
            case 0x00: /*Sensor interface 1*/
            default :
                pSeninf = (seninf_reg_t_base *)mpSeninfHwRegAddr;
                break;
        }

        //0x1A0403DC for VC0~VC3
        ...
        if((vc0Id == 0) && (vc1Id == 0) && (vc2Id == 0) && (vc3Id == 0))
        {
            return ret;
        }
...
{
        LOG_MSG("Muxsel: %d, inSrcTypeSel: %d \n", Muxsel, inSrcTypeSel);
...
//        seninf_reg_t *pSeninf = (seninf_reg_t *)mpSeninfHwRegAddr;
		...
		{
            case 0x02:
                pSeninf = (seninf_mux_reg_t_base *)mpSeninfMux2BaseAddr;
            //pSeninf = (seninf_reg_t *)mpSeninf2HwRegAddr; //1a048520
            break;
            case 0x03:
                pSeninf = (seninf_mux_reg_t_base *)mpSeninfMux3BaseAddr;
            //pSeninf = (seninf_reg_t *)mpSeninf3HwRegAddr; //1a048920
            break;
            case 0x04:
                pSeninf = (seninf_mux_reg_t_base *)mpSeninfMux4BaseAddr;
            //pSeninf = (seninf_reg_t *)mpSeninf4HwRegAddr; //1a048d120
            break;
            case 0x05:
                pSeninf = (seninf_mux_reg_t_base *)mpSeninfMux5BaseAddr;
            break;
            case 0x06:
                pSeninf = (seninf_mux_reg_t_base *)mpSeninfMux6BaseAddr;
            break;
            case 0x01:
            default :
            //pSeninf = (seninf_reg_t *)mpSeninfHwRegAddr; //1a048120
                pSeninf = (seninf_mux_reg_t_base *)mpSeninfMux1BaseAddr;
            break;
		}
        ...
        if ((CSI2 == inSrcTypeSel)||(MIPI_SENSOR <= inSrcTypeSel)) {
            if(JPEG_FMT != inDataType) {
				...
				if(Muxsel == 4) {
					SENINF_BITS(pSeninf, SENINF1_MUX_CTRL, FIFO_FLUSH_EN) = 0x38;
	                SENINF_BITS(pSeninf, SENINF1_MUX_CTRL, FIFO_PUSH_EN) = 0x3E;
				}
				else {
...
*
********************************************************************************
int SeninfDrvImp::setSeninfCtrl(
...
)
{
        ...
        LOG_MSG("padSel: %d, inSrcTypeSel: %d \n", padSel, inSrcTypeSel);
...
*
********************************************************************************/
int SeninfDrvImp::setSeninfNCSI2(Seninf_para *seninf)
{
	int ret = 0;
	seninf_reg_t *pSeninf = (seninf_reg_t *)mpSeninfHwRegAddr;
...
//	mipi_ana_reg_t *pMipirx_all = NULL;//(mipi_ana_reg_t *)mpCSI2RxAnalog0RegAddr;
	...
	unsigned int dataTermDelay, dataSettleDelay, clkTermDelay, vsyncType, dlaneNum, ncsi2En; //Enable;
...
//	ISP_CLK_GET = 0;
	...
	unsigned int *mpCSI2RxAnalogRegAddr = mpCSI2RxAnalog0RegAddr;

	if(seninf->CSI2_IP == SENINF2_CSI1)	{ /*_check_*/
		//pMipirx = (mipi_ana_reg_t_base *)mpCSI2RxAnalog1RegAddr;
		pSeninf = (seninf_reg_t *)(mpSeninfHwRegAddr + (0x400/4));
		mpCSI2RxAnalogRegAddr = mpCSI2RxAnalog1RegAddr;

	}
 	else if(seninf->CSI2_IP == SENINF4_CSI2) {
		//pMipirx = (mipi_ana_reg_t_base *)mpCSI2RxAnalog2RegAddr;
		pSeninf = (seninf_reg_t *)(mpSeninfHwRegAddr + (0x800/4));
		mpCSI2RxAnalogRegAddr = mpCSI2RxAnalog2RegAddr;
	}
	else {
		//pMipirx = (mipi_ana_reg_t_base *)mpCSI2RxAnalog0RegAddr;
		mpCSI2RxAnalogRegAddr = mpCSI2RxAnalog0RegAddr;
	}
    ...
    if(ncsi2En == 1) {  // enable CSI2
        ...
        temp = *(mpCSI2RxAnalogRegAddr + (0x4C/4));//GPI*_IES = 0 for MIPI
       mt65xx_reg_writel(temp&0xFEFBEFBE, mpCSI2RxAnalogRegAddr + (0x4C/4));
        temp = *(mpCSI2RxAnalogRegAddr + (0x50/4));//GPI*_IES = 0 for MIPI
       mt65xx_reg_writel(temp&0xFEFBEFBE, mpCSI2RxAnalogRegAddr + (0x50/4));
...
if( (seninf->CSI2_IP == 1) || (seninf->CSI2_IP == 2) ){
        ...
        mt65xx_reg_writel(temp|0x00000001, mpCSI2RxAnalogRegAddr + (0x24/4));
        usleep(30);
        temp = *(mpCSI2RxAnalogRegAddr + (0x20/4));//bit0:RG_CSI0_LDO_CORE_EN,bit1:RG_CSI0_LNRD_HSRX_BCLK_INVERT
        mt65xx_reg_writel(temp|0x00000103, mpCSI2RxAnalogRegAddr + (0x20/4));
        usleep(1);
...
if( (seninf->CSI2_IP == 1) || (seninf->CSI2_IP == 2) ){
	...
	if(seninf->CSI2_IP == 1) {
        if(mCSI1 != 0)
        {
            ...
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI2&0xf0000000)>>16), mpCSI2RxAnalogRegAddr + (0x04/4));
            temp = *(mpCSI2RxAnalogRegAddr + (0x08/4));
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI2&0x0f000000)>>12), mpCSI2RxAnalogRegAddr + (0x08/4));
            temp = *(mpCSI2RxAnalogRegAddr + (0x0C/4));
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0xf0000000)>>16), mpCSI2RxAnalogRegAddr + (0x0C/4));
            temp = *(mpCSI2RxAnalogRegAddr + (0x10/4));
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x0f000000)>>12), mpCSI2RxAnalogRegAddr + (0x10/4));
            temp = *(mpCSI2RxAnalogRegAddr );
            mt65xx_reg_writel((temp&(~0xf000))|((mCSI1&0x00f00000)>>8), mpCSI2RxAnalogRegAddr);
...
//		SENINF_BITS(pSeninf, MIPI_RX_CON38_CSI2, MIPI_RX_SW_CTRL_MODE) = 0; // 0x15008338[0]= 1'b0;
        ...
        if(mipiskew) {
            ...
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xB0/4)); //0x10217000+B0
            mt65xx_reg_writel(0x80010400, mpCSI2RxAnalogRegAddr + (0xB0/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xB4/4));
            mt65xx_reg_writel(0xFFFFFFFF, mpCSI2RxAnalogRegAddr + (0xB4/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xB8/4));
            mt65xx_reg_writel(0x00750000, mpCSI2RxAnalogRegAddr + (0xB8/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xBC/4));
            mt65xx_reg_writel(0x00050400, mpCSI2RxAnalogRegAddr + (0xBC/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xC0/4));
            mt65xx_reg_writel(0x80000007, mpCSI2RxAnalogRegAddr + (0xC0/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xC4/4));
            mt65xx_reg_writel(0X0, mpCSI2RxAnalogRegAddr + (0xC4/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xD0/4));
            mt65xx_reg_writel(0xF, mpCSI2RxAnalogRegAddr + (0xD0/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0xA0/4));
            mt65xx_reg_writel(0x1, mpCSI2RxAnalogRegAddr + (0xA0/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0x98/4));
            mt65xx_reg_writel(0x1, mpCSI2RxAnalogRegAddr + (0x98/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0x90/4));
            mt65xx_reg_writel(0x1, mpCSI2RxAnalogRegAddr + (0x90/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0x94/4));
            mt65xx_reg_writel(0x1, mpCSI2RxAnalogRegAddr + (0x94/4));
            temp = *(mpCSI2RxAnalogRegStartAddr + (0x8C/4));
            mt65xx_reg_writel(0x1, mpCSI2RxAnalogRegAddr + (0x8C/4));
        }
        ...
        //disable mipi BG
        temp = *(mpCSI2RxAnalogRegAddr + (0x24/4));//RG_CSI_BG_CORE_EN off
        mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxAnalogRegAddr + (0x24/4));
        temp = *(mpCSI2RxAnalogRegAddr + (0x20/4));//RG_CSI0_LDO_CORE_EN off
        mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxAnalogRegAddr + (0x20/4));
        temp = *(mpCSI2RxAnalogRegAddr); //RG_CSI0_LNRC_LDO_OUT_EN off
        mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxAnalogRegAddr);
        temp = *(mpCSI2RxAnalogRegAddr + (0x04/4));//RG_CSI0_LNRD0_LDO_OUT_EN off
        mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxAnalogRegAddr + (0x04/4));
        temp = *(mpCSI2RxAnalogRegAddr + (0x08/4));//RG_CSI0_LNRD1_LDO_OUT_EN off
        mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxAnalogRegAddr + (0x08/4));

if( (seninf->CSI2_IP == 1) || (seninf->CSI2_IP == 2) ){
        temp = *(mpCSI2RxAnalogRegAddr + (0x0C/4));//RG_CSI0_LNRD2_LDO_OUT_EN off
        mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxAnalogRegAddr + (0x0C/4));
        temp = *(mpCSI2RxAnalogRegAddr + (0x10/4));//RG_CSI0_LNRD3_LDO_OUT_EN off
        mt65xx_reg_writel(temp&0xFFFFFFFE, mpCSI2RxAnalogRegAddr + (0x10/4));
}
        // disable mipi pin
        temp = *(mpCSI2RxAnalogRegAddr + (0x4C/4));//GPI*_IES = 0 for GPIO
        mt65xx_reg_writel(temp|0x1041041, mpCSI2RxAnalogRegAddr + (0x4C/4));
        temp = *(mpCSI2RxAnalogRegAddr + (0x50/4));//GPI*_IES = 0 for GPIO
        mt65xx_reg_writel(temp|0x1041041, mpCSI2RxAnalogRegAddr + (0x50/4));
...
/*
	SENINF_BITS(pMipirx, MIPI_RX_ANA4C_CSI0, RG_MIPI_GPI0_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA4C_CSI0, RG_MIPI_GPI1_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA4C_CSI0, RG_MIPI_GPI2_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA4C_CSI0, RG_MIPI_GPI3_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA4C_CSI0, RG_MIPI_GPI4_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA50_CSI0, RG_MIPI_GPI5_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA50_CSI0, RG_MIPI_GPI6_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA50_CSI0, RG_MIPI_GPI7_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA50_CSI0, RG_MIPI_GPI8_IES) = 0;
	SENINF_BITS(pMipirx, MIPI_RX_ANA50_CSI0, RG_MIPI_GPI9_IES) = 0;


	SENINF_BITS(pMipirx, MIPI_RX_ANA00_CSI0, RG_CSI0_LNRC_INPUT_SEL) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA04_CSI0, RG_CSI0_LNRD0_INPUT_SEL) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA08_CSI0, RG_CSI0_LNRD1_INPUT_SEL) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA0C_CSI0, RG_CSI0_LNRD2_INPUT_SEL) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA10_CSI0, RG_CSI0_LNRD3_INPUT_SEL) = 1;

	// CSI Power On Timing
	SENINF_BITS(pMipirx, MIPI_RX_ANA24_CSI0 , RG_CSI0_BG_CORE_EN ) = 1;
	usleep(30);
	SENINF_BITS(pMipirx, MIPI_RX_ANA20_CSI0 , RG_CSI0_LDO_CORE_EN ) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA20_CSI0 , RG_CSI0_LNRD_HSRX_BCLK_INVERT ) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA20_CSI0 , RG_CSI0_PRESERVE) = 1;
	usleep(1);
	SENINF_BITS(pMipirx, MIPI_RX_ANA00_CSI0 , RG_CSI0_LNRC_LDO_OUT_EN) = 1;

	SENINF_BITS(pMipirx, MIPI_RX_ANA04_CSI0, RG_CSI0_LNRD0_LDO_OUT_EN) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA08_CSI0, RG_CSI0_LNRD1_LDO_OUT_EN) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA0C_CSI0, RG_CSI0_LNRD2_LDO_OUT_EN) = 1;
	SENINF_BITS(pMipirx, MIPI_RX_ANA10_CSI0, RG_CSI0_LNRD3_LDO_OUT_EN) = 1;
*/
        //CSI efuse calibration
        //0x10206190 [7:4]gJ0x10217004[15:12]  (CSI0 lane0)
        //0x10206190 [3:0]gJ0x10217008[15:12]  (CSI0 lane1)
...
int SeninfDrvImp::setMclk1IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    property_get("vendor.debug.seninf.drving_current", value, "-1");
    _ioDrivingCurrent = atoi(value);
    if(_ioDrivingCurrent >= 0)
...
int SeninfDrvImp::setMclk2IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    /*get property*/
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    int _ioDrivingCurrent2 = 0;
    property_get("vendor.debug.seninf.drving_current2", value, "-1");
    _ioDrivingCurrent2 = atoi(value);
    if(_ioDrivingCurrent2 >= 0)
        ioDrivingCurrent = _ioDrivingCurrent2;
...
int SeninfDrvImp::setMclk3IODrivingCurrent(unsigned long ioDrivingCurrent)
{
    ...
    /*get property*/
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    int _ioDrivingCurrent3 = 0;
    property_get("vendor.debug.seninf.drving_current3", value, "-1");
    _ioDrivingCurrent3 = atoi(value);
    if(_ioDrivingCurrent3 >= 0)
        ioDrivingCurrent = _ioDrivingCurrent3;
...
int SeninfDrvImp::setN3DCfg(unsigned long n3dEn, unsigned long i2c1En, unsigned long i2c2En, unsigned long n3dMode, unsigned long diffCntEn, unsigned long diffCntThr)
{
    ...
    SENINF_BITS(pSeninf, SENINF_N3D_CTL, N3D_EN) = n3dEn;
    SENINF_BITS(pSeninf, SENINF_N3D_CTL, I2C1_EN) = i2c1En;
    SENINF_BITS(pSeninf, SENINF_N3D_CTL, I2C2_EN) = i2c2En;
    SENINF_BITS(pSeninf, SENINF_N3D_CTL, MODE) = n3dMode;
...
int SeninfDrvImp::setN3DI2CPos(unsigned long n3dPos)
{
    ...
    SENINF_BITS(pSeninf, SENINF_N3D_POS, N3D_POS) = n3dPos;
...
int SeninfDrvImp::setN3DTrigger(bool i2c1TrigOn, bool i2c2TrigOn)
{
    ...
    SENINF_BITS(pSeninf, SENINF_N3D_TRIG, I2CA_TRIG) = i2c1TrigOn;
    SENINF_BITS(pSeninf, SENINF_N3D_TRIG, I2CB_TRIG) = i2c2TrigOn;
...
int SeninfDrvImp::getN3DDiffCnt(MUINT32 *pCnt)
{
    ...
    *pCnt = SENINF_READ_REG(pSeninf,SENINF_N3D_DIFF_CNT);
    *(pCnt+1) = SENINF_READ_REG(pSeninf,SENINF_N3D_CNT0);
    *(pCnt+2) = SENINF_READ_REG(pSeninf,SENINF_N3D_CNT1);
    *(pCnt+3) = SENINF_READ_REG(pSeninf,SENINF_N3D_DBG);
