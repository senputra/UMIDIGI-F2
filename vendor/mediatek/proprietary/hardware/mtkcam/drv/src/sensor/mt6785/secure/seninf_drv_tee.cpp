...
int SeninfDrvTeeImp::init()
{
    ...
    for(int i = SENINF_CAM_MUX0; i < SENINF_CAM_MUX_NUM; i++) {
        disableCamMUX((SENINF_CAM_MUX_ENUM)i);
    }
...
int SeninfDrvTeeImp::sendCommand(int cmd, unsigned long arg1, unsigned long arg2, unsigned long arg3)
{
    ...
    case CMD_SENINF_CHECK_PIPE:
        LOG_MSG("1st lock issue: streaming pipe check without sync register!");
...
*
********************************************************************************/
int SeninfDrvTeeImp::setSeninfTopMuxCtrl(
    SENINF_MUX_ENUM seninfMuXIdx, SENINF_ENUM seninfSrc
)
{
    int ret = 0;

    ret = SeninfDrvImp::setSeninfTopMuxCtrl(seninfMuXIdx, seninfSrc);
...
*
********************************************************************************/
int SeninfDrvTeeImp::setSeninfCamTGMuxCtrl(
    unsigned int targetCamTG, SENINF_MUX_ENUM seninfMuxSrc
)
{
    int ret = 0;

    ret = SeninfDrvImp::setSeninfCamTGMuxCtrl(targetCamTG, seninfMuxSrc);
...
{
    return SeninfDrvImp::getSeninfTopMuxCtrl(seninfMuXIdx);
}
...
{
    return SeninfDrvImp::getSeninfCamTGMuxCtrl(targetCamTG);
}
...
)
{
    ...
    ret = setSeninfMuxCtrl(mux, hsPol, vsPol,
...
{
    SeninfDrvImp::setSeninfMuxCrop(mux, start_x, end_x, enable);
...
{
    return SeninfDrvImp::isMUXUsed(mux);
}
...
{
    return SeninfDrvImp::reset(mipiPort);
}
...
{
    return SeninfDrvImp::isCamMUXUsed(cam_mux);
}
...
{
     return SeninfDrvImp::enableCamMUX(cam_mux);
}
...
{
    return SeninfDrvImp::disableCamMUX(cam_mux);
}
...
{
    return SeninfDrvImp::setSeninfCamMuxSrc(src, target);
}
