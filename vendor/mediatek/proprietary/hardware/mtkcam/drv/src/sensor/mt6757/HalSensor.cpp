...
extern SensorDynamicInfo sensorDynamicInfo[4];
#else
extern SensorDynamicInfo sensorDynamicInfo[3];
#endif
extern MUINT32 mCreateSensorCount;
#ifdef MTK_SUB2_IMGSENSOR
extern SENSORDRV_INFO_STRUCT sensorDrvInfo[4];
#else
...
)
{
    ...
    wake_lock = 1;
    ret = mpSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_SET_SENSOR_SET_WAKE_LOCK, (MUINTPTR)&wake_lock);
    MY_LOGD("!!!get wakelock");
    ...
    muDev.unlock();
    android_atomic_inc(&mPowerRefCount);
...
)
{
...
#ifdef MTK_SUB2_IMGSENSOR
    if( sensorDev & SENSOR_DEV_SUB_2) {
        sensorDynamicInfo[3].TgInfo = CAM_TG_NONE;
    ...
    meSensorDev = meSensorDev & (~sensorDev);
    muDev.unlock();
    android_atomic_dec(&mPowerRefCount);
