...
int SeninfDrvImp::init()  //kk test
{
   ...
   // gIspDrvObj[CAM_A].m_pIspHwRegAddr = (MUINT32 *) mmap(0, CAM_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, CAM_A_BASE_HW);
    ...
    mpIspHwRegBaseAddr[SENINF_TOP_SV3] = mpIspHwRegAddr + (0x800/4)*2;
    mpIspHwRegBaseAddr[SENINF_TOP_SV4] = mpIspHwRegAddr + (0x800/4)*3;
    ...
    case CMD_SENINF_DEBUG_PIXEL_METER:
        {
            ...
            if(meter_array != NULL) {
                *((unsigned int *)(mpSeninfMuxBaseAddr[SENINF_MUX3]+0x3C/4))|= 1<<31;
                meter_array[0] = *((unsigned int *)(mpSeninfMuxBaseAddr[SENINF_MUX3]+0x40/4));
                meter_array[1] = *((unsigned int *)(mpSeninfMuxBaseAddr[SENINF_MUX3]+0x44/4));
                meter_array[2] = *((unsigned int *)(mpSeninfMuxBaseAddr[SENINF_MUX3]+0x48/4));
                meter_array[3] = *((unsigned int *)(mpSeninfMuxBaseAddr[SENINF_MUX3]+0x4c/4));
...
)
{
        ...
        switch (camTarget){
        case SENINF_TOP_TG2:
            SENINF_BITS(pSeninf, SENINF_TOP_CAM_MUX_CTRL, SENINF_CAM1_MUX_SRC_SEL)= seninfMuxSrc;
            break;
        case SENINF_TOP_SV1:
        ...
        case SENINF_TOP_SV2:
            ...
            break;
        case SENINF_TOP_SV3:
            SENINF_BITS(pSeninf, SENINF_TOP_CAM_MUX_CTRL, SENINF_CAM4_MUX_SRC_SEL)= seninfMuxSrc;
            break;
        case SENINF_TOP_SV4:
            SENINF_BITS(pSeninf, SENINF_TOP_CAM_MUX_CTRL, SENINF_CAM5_MUX_SRC_SEL)= seninfMuxSrc;
            break;
        default://SENINF_MUX1
    ...
    if(enable) {
        ...
        if (pixel_rate != 0) {
           ...
           //
                ...
                LOG_ERR("sensor does not provide mipi_pixel_rate %lld or pixel_rate %lld", pseninfPara->csi_para.mipi_pixel_rate, pseninfPara->pixel_rate);
        ...
        if(pseninfPara->HDRMode == HDR_CAMSV){
            ...
            if (SENINF_MUX_selected_hdr < SENINF_MUX_NUM) {
                ...
                SENINF_TOP_P1_ENUM camsv_sel = (TG_SEL == CAM_TG_1 ?SENINF_TOP_SV1 :SENINF_TOP_SV3);
        ...
        if (pseninfPara->PDAFMode == PDAF_SUPPORT_CAMSV ||
            ...
            if(SENINF_MUX_selected_pdaf < SENINF_MUX_NUM) {
                ...
                SENINF_TOP_P1_ENUM camsv_sel = (TG_SEL == CAM_TG_1 ?SENINF_TOP_SV2 :SENINF_TOP_SV4);
                ret = this->setSVGrabRange(camsv_sel,
...
)
{
    ...
    LOG_MSG("[setTgViewFinderMode] %u", tgSel);
...
)
{
    ...
    if(1 == twoPxlMode) {
        ...
        ISP_BITS(pisp, CAM_CTL_FMT_SEL_P1, TWO_PIX) = 0;
         LOG_MSG("[setTgCfg] twoPxlMode == 0 0x%x",ISP_REG(pisp, CAM_CTL_FMT_SEL_P1));
    }
