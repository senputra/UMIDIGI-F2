...
extern SENSOR_CONFIG_STRUCT sensorPara[4];

SensorStaticInfo sensorStaticInfo[4];//0:main, 1:sub, 2:3d
SENSORDRV_INFO_STRUCT sensorDrvInfo[4];//6593, 0:main, 1:sub, 2:main2, 3:sub2
SENSOR_HAL_RAW_INFO_STRUCT sensorRawInfo[4];
IMetadata  gSensorMetadata[4];//main/sub/main2
#else
...
#ifdef MTK_SUB2_IMGSENSOR
    for (sensorDev = SENSOR_MAIN; sensorDev <= SENSOR_SUB_2; sensorDev <<= 1) {
#else
...
#ifdef MTK_SUB2_IMGSENSOR
        case SENSOR_SUB_2:
            pInfo->u1Order = sensorDrvInfo[3].SensorOutputDataFormat;
            break;
#endif
        default:
            break;
    ...
    for(idx=0; idx<1; idx++) {
        ...
        switch (idx) {
            case 0:
                ...
                pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].custom1FrameRate = data2;
                data1 = SENSOR_SCENARIO_ID_CUSTOM2;
                pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].custom2FrameRate = data2;
                data1 = SENSOR_SCENARIO_ID_CUSTOM3;
                pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].custom3FrameRate = data2;
                data1 = SENSOR_SCENARIO_ID_CUSTOM4;
                pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].custom4FrameRate = data2;
                data1 = SENSOR_SCENARIO_ID_CUSTOM5;
                pSensorDrv->sendCommand(SENSOR_MAIN, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].custom5FrameRate = data2;
                pSensorDrv->uninit();
                break;
            case 1:
...
#ifdef MTK_SUB2_IMGSENSOR
             case 3:
                pSensorDrv->init(SENSOR_SUB_2);
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_MCLK_CONNECTION, (MUINTPTR)&data1);
                sensorPara[idx].mclkSrc = data1;
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_MIPI_SENSOR_PORT, (MUINTPTR)&data1);
                sensorPara[idx].mipiPad = data1;

                sensorStaticInfo[idx].sensorDevID = pSensorDrv->getSub2SensorID();
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_SENSOR_ORIENTATION_ANGLE, (MUINTPTR)&data1);
                sensorStaticInfo[idx].orientationAngle = data1;
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_SENSOR_FACING_DIRECTION, (MUINTPTR)&data1);
                sensorStaticInfo[idx].facingDirection = data1;
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_FAKE_ORIENTATION, (MUINTPTR)&data1);
                sensorStaticInfo[idx].fakeOrientation = data1;
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_SENSOR_VIEWANGLE, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].horizontalViewAngle =data1;
                sensorStaticInfo[idx].verticalViewAngle = data2;

                data1 = SENSOR_SCENARIO_ID_NORMAL_PREVIEW;
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].previewFrameRate = data2;
                data1 = SENSOR_SCENARIO_ID_NORMAL_CAPTURE;
                pSensorDrv->sendCommand(SENSOR_SUB_2, CMD_SENSOR_GET_DEFAULT_FRAME_RATE_BY_SCENARIO, (MUINTPTR)&data1, (MUINTPTR)&data2);
                sensorStaticInfo[idx].captureFrameRate = data2;
        ...
        switch (sensorRawInfo[idx].sensorType) {
         ...
         switch (sensorRawInfo[idx].u1Order) {
             ...
             case SENSOR_OUTPUT_FORMAT_RAW_MONO:
                sensorStaticInfo[idx].sensorFormatOrder = SENSOR_FORMAT_ORDER_MONO;
                sensorStaticInfo[idx].rawFmtType = SENSOR_RAW_MONO;
                break;
           case SENSOR_OUTPUT_FORMAT_RAW_4CELL_B:
                sensorStaticInfo[idx].rawFmtType = SENSOR_RAW_4CELL;
                sensorStaticInfo[idx].sensorFormatOrder = SENSOR_FORMAT_ORDER_RAW_B;
                break;
        ...
        switch(sensorDrvInfo[idx].PDAF_Support) {
            case 1: /* 1: PDAF Raw Data mode */
                sensorStaticInfo[idx].rawFmtType = SENSOR_RAW_PD;
                break;
            default:
                break;
        }
        ...
        sensorStaticInfo[idx].HDR_Support = sensorDrvInfo[idx].HDR_Support;
        ...
        sensorStaticInfo[idx].FrameTimeDelayFrame = sensorDrvInfo[idx].FrameTimeDelayFrame;
        sensorStaticInfo[idx].SensorGrabStartX_PRV = sensorDrvInfo[idx].SensorGrabStartX_PRV;
        sensorStaticInfo[idx].SensorGrabStartY_PRV = sensorDrvInfo[idx].SensorGrabStartY_PRV;
        sensorStaticInfo[idx].SensorGrabStartX_CAP = sensorDrvInfo[idx].SensorGrabStartX_CAP;
        sensorStaticInfo[idx].SensorGrabStartY_CAP = sensorDrvInfo[idx].SensorGrabStartY_CAP;
        sensorStaticInfo[idx].SensorGrabStartX_VD = sensorDrvInfo[idx].SensorGrabStartX_VD;
        sensorStaticInfo[idx].SensorGrabStartY_VD = sensorDrvInfo[idx].SensorGrabStartY_VD;
        sensorStaticInfo[idx].SensorGrabStartX_VD1 = sensorDrvInfo[idx].SensorGrabStartX_VD1;
        sensorStaticInfo[idx].SensorGrabStartY_VD1 = sensorDrvInfo[idx].SensorGrabStartY_VD1;
        sensorStaticInfo[idx].SensorGrabStartX_VD2 = sensorDrvInfo[idx].SensorGrabStartX_VD2;
        sensorStaticInfo[idx].SensorGrabStartY_VD2 = sensorDrvInfo[idx].SensorGrabStartY_VD2;
        sensorStaticInfo[idx].SensorGrabStartX_CST1 = sensorDrvInfo[idx].SensorGrabStartX_CST1;
        sensorStaticInfo[idx].SensorGrabStartY_CST1 = sensorDrvInfo[idx].SensorGrabStartY_CST1;
        sensorStaticInfo[idx].SensorGrabStartX_CST2 = sensorDrvInfo[idx].SensorGrabStartX_CST2;
        sensorStaticInfo[idx].SensorGrabStartY_CST2 = sensorDrvInfo[idx].SensorGrabStartY_CST2;
        sensorStaticInfo[idx].SensorGrabStartX_CST3 = sensorDrvInfo[idx].SensorGrabStartX_CST3;
        sensorStaticInfo[idx].SensorGrabStartY_CST3 = sensorDrvInfo[idx].SensorGrabStartY_CST3;
        sensorStaticInfo[idx].SensorGrabStartX_CST4 = sensorDrvInfo[idx].SensorGrabStartX_CST4;
        sensorStaticInfo[idx].SensorGrabStartY_CST4 = sensorDrvInfo[idx].SensorGrabStartY_CST4;
        sensorStaticInfo[idx].SensorGrabStartX_CST5 = sensorDrvInfo[idx].SensorGrabStartX_CST5;
        sensorStaticInfo[idx].SensorGrabStartY_CST5 = sensorDrvInfo[idx].SensorGrabStartY_CST5;
        ...
        sensorStaticInfo[idx].ZHDR_MODE = sensorDrvInfo[idx].ZHDR_MODE;
    }
