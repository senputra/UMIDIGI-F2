...
#define CONFIG_METADATA_END() \
        { \
            int err = myCAPABILITY.update(myENTRY.tag(), myENTRY); \
            if  ( 0 != err ) { \
...
#define APPEND_METADATA_BEGIN(_tag_) \
    { \
        ...
        IMetadata& myCAPABILITY = rMetadata; \
        if ( myENTRY.isEmpty() ) { \
            MY_LOGF("No entry for tag: 0x%x before, use CONFIG_METADATA_BEGIN", _tag_); \
        }
...
#define REMOVE_METADATA(_tag_) \
        MY_LOGW_IF(0, "remove tag: 0x%x (count: %u)", _tag_, rMetadata.entryFor(_tag_).count()); \
