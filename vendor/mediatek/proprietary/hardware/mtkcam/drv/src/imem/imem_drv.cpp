...
//-----------------------------------------------------------------------------
StdIMemDrv* StdIMemDrv::createInstance()
{
    DBG_LOG_CONFIG(drv, imem_drv);
    return StdIMemDrvImp::getInstance();
}
//-----------------------------------------------------------------------------
StdIMemDrvImp::StdIMemDrvImp()
    :mInitCount(0)
{
    mIonDrv = 0;
...
//-----------------------------------------------------------------------------
StdIMemDrv* StdIMemDrvImp::getInstance(void)
{
    static StdIMemDrvImp singleton;
    IMEM_DBG("singleton[0x%lx],getpid[0x%08x],gettid[0x%08x] ",(unsigned long)(&singleton),getpid() ,gettid());
    IMEM_DRV_DELAY
    return &singleton;
}
//-----------------------------------------------------------------------------
void StdIMemDrvImp::destroyInstance(void)
{
}
//-----------------------------------------------------------------------------
///////////////////////////////////////////////////////////////////////
...
MBOOL StdIMemDrvImp::init(void)
{
    MBOOL Result = MTRUE;
    MINT32 ret = 0;
    IMEM_INF("Std_IMem mCount(%d)", mInitCount);
    //
    ...
    if(mInitCount==1)
    {
        ...
        mIonDrv = mt_ion_open("StdIMemDrvImp.cpp");
        if (mIonDrv < 0)
        {
...
///              local count and global count repectively
MBOOL StdIMemDrvImp::uninit(void)
{
    MBOOL Result = MTRUE;
    MINT32 ret = 0;
    ...
    if ( mInitCount <= 0 )
    {
        ...
        // if process A open ionID, then process B open ionID before process A call StdImemDrv_uninit,
...
//-----------------------------------------------------------------------------
MBOOL StdIMemDrvImp::reset(void)
{
    IMEM_DBG("");
    //erase all
    buf_map.clear();
...
//-----------------------------------------------------------------------------
MINT32 StdIMemDrvImp::allocVirtBuf(
    IMEM_BUF_INFO* pInfo)
{
...
//-----------------------------------------------------------------------------
MINT32 StdIMemDrvImp::freeVirtBuf(
    IMEM_BUF_INFO* pInfo)
{
...
//-----------------------------------------------------------------------------
MINT32  StdIMemDrvImp::mapPhyAddr(
    IMEM_BUF_INFO* pInfo)
{
...
#ifdef NEW_M4U_PORT_DEF
            mm_data.config_buffer_param.eModuleID = M4U_PORT_IMGI_D1;
#else
            mm_data.config_buffer_param.eModuleID = M4U_PORT_CAM_IMGI;
#endif
...
//-----------------------------------------------------------------------------
MINT32  StdIMemDrvImp::unmapPhyAddr(
    IMEM_BUF_INFO* pInfo)
{
    stIMEM_MAP_INFO map_info;
...
//-----------------------------------------------------------------------------
MINT32  StdIMemDrvImp::doIonCacheFlush(MINT32 memID,eIONCacheFlushType flushtype,MUINT32 memSize)
{
    #if defined (__ISP_USE_ION__)
    ion_user_handle_t pIonHandle;
    MINT32 IonBufFd;
    ...
    sys_data.cache_sync_param.size = memSize;
...
//-----------------------------------------------------------------------------
MINT32 StdIMemDrvImp::cacheFlushAll(
    void)
{
...
//-----------------------------------------------------------------------------
MINT32 StdIMemDrvImp::cacheSyncbyRange(IMEM_CACHECTRL_ENUM ctrl,IMEM_BUF_INFO* pInfo)
{
#if defined (__ISP_USE_ION__)
    ...
    else
    {
        if((ctrl == IMEM_CACHECTRL_ENUM_FLUSH) && (pInfo->size >= 0x3200000))
        {
            ...
            IMEM_DBG("+ Flush all, c/m/va/sz/pa(%d/0x%x/0x%lx/0x%x/0x%lx)",ctrl,pInfo->memID,(unsigned long)pInfo->virtAddr,pInfo->size,(unsigned long)pInfo->phyAddr);
            return 0;
        }
...
//-----------------------------------------------------------------------------
MINT32 StdIMemDrvImp::initSecureM4U(void)
{
    int m4u_fd;
...
//-----------------------------------------------------------------------------
MINT32 StdIMemDrvImp::allocSecureBuf(
    IMEM_BUF_INFO* pInfo)
{
    if(pInfo->size <= 0)
    {
    ...
    if(ion_custom_ioctl(mIonDrv, ION_CMD_SYSTEM, &sys_data)) {
        ...
        IMEM_DBG("Secure memory allocated OK, handle : 0x%08X, size: %d", sys_data.get_phys_param.phy_addr, sys_data.get_phys_param.len);
...
//-----------------------------------------------------------------------------
MINT32 StdIMemDrvImp::freeSecureBuf(
    IMEM_BUF_INFO* pInfo)
{
