    ...
    for (unsigned int i=0; i<WmfeConfigVec.size(); i++)
    {
        ...
        for (unsigned int j=0; j<pWmfeConfig->mWMFECtrlVec.size(); j++)
        {
            ...
            if (pWmfeCtrl->Wmfe_Dpo.u4Stride != pWmfeCtrl->Wmfe_Dpi.u4Stride)
            {
	            ...
	            {
                    //dpe_WmfeConfig[i].WmfeCtrl[j].WMFE_CTRL = (pWmfeCtrl->WmfeVertScOrd<<17) | (pWmfeCtrl->WmfeHorzScOrd<<16) | (pWmfeCtrl->WmfeFilterSize<<12) | (pWmfeCtrl->WmfeDpiFmt<<10) | (pWmfeCtrl->WmfeImgiFmt<<8) | (pWmfeCtrl->Wmfe_Mask_En<<7) | (pWmfeCtrl->Wmfe_Dpnd_En<<6) | (1<<5) | (1<<4) | (pWmfeCtrl->Wmfe_Enable);
	            ...
	            {
                    //dpe_WmfeConfig[i].WmfeCtrl[j].WMFE_CTRL = (pWmfeCtrl->WmfeVertScOrd<<17) | (pWmfeCtrl->WmfeHorzScOrd<<16) | (pWmfeCtrl->WmfeFilterSize<<12) | (pWmfeCtrl->WmfeDpiFmt<<10) | (pWmfeCtrl->WmfeImgiFmt<<8) | (pWmfeCtrl->Wmfe_Mask_En<<7) | (pWmfeCtrl->Wmfe_Dpnd_En<<6) | (1<<5) | (scale << 4) | (pWmfeCtrl->Wmfe_Enable);
            ...
            //dpe_WmfeConfig[i].WmfeCtrl[j].WMFE_MASK_MODE = pWmfeCtrl->Wmfe_Mask_Mode;
            //dpe_WmfeConfig[i].WmfeCtrl[j].WMFE_MASK_VALUE = pWmfeCtrl->Wmfe_Mask_Value;
