...
template <typename T>
list<DPE_SUB_ENGINE_ID> EgnStream<T>::mEgnList;
...
init()
{
    ...
    LOG_INF("DPE EgnStream Init: mEgnInitCnt(%d)",mInitCount);

    property_get("vendor.cam.egnsavefile.enable", value, "0");
...
uninit()
{
    ...
    LOG_INF("DPE EgnStream Uninit: mEgnUnitCnt(%d)",mInitCount);
    ...
    {
        LOG_INF("DPE EgnStream: mEgnUnitCnt(%d) doesn't come to 0, skip uninit",mInitCount);
    }
...
#define DUMP_EGNBuffer( stream, pbuf, type, dir, cnt, fileExt)      \
    ...
    if (static_cast<DPE_SUB_ENGINE_ID>(rEgnParams.mpEngineID) >= eENGINE_MAX){
    ...
    ret = m_pEngineDrv->enque(egnreq.m_EGNParams.mEGNConfigVec);

    if (ret == MTRUE) {
#if 0
        addCommand(ECmd_EGN_ENQUE);
#else
        addCommand(cmd, (DPE_SUB_ENGINE_ID) rEgnParams.mpEngineID);
#endif
    } else {
        LOG_INF("DPE: Enque Fail!, Ret: %d\n", ret);
    }
    //FUNCTION_LOG_END;
...
EgnStream<T>::destroyThread()
{
    ...
    for(id = 0; id < eENGINE_MAX; id++)
        addCommand(cmd, (DPE_SUB_ENGINE_ID) id);
}
...
MVOID
EgnStream<T>::
addCommand(ECmd const &cmd, DPE_SUB_ENGINE_ID const &eid)
{
...
EgnStream<T>::
getCommand(
    ECmd &cmd, DPE_SUB_ENGINE_ID &eid)
{
    Mutex::Autolock autoLock(mEgnCmdLock);
...
//#define EGN_INT_WAIT_TIMEOUT_MS (10000) //Early Poting is slow.
template <class T>
MVOID*
EgnStream<T>::
onThreadLoop(
                    ...
                    if (_this->m_QueueEGNReqList.size()>0)
                    {
                        ...
                        if (EgnConfigVec.size() == EGNReqVal.m_EGNParams.mEGNConfigVec.size())
                        {
                            ...
                            else
                            {
                                ...
                                for(; deq != EgnConfigVec.end(); enq++, deq++)
                                {
                                    (*enq).Dpe_feedback = (*deq).Dpe_feedback;
                                }
...
#endif
#ifdef FACTORY
template IEgnStream<DPEConfig>* NSCam::NSIoPipe::NSEgn::getEgnStreamImp(char const* szCallerName);
#else
template IEgnStream<DPEConfig>* IEgnStream<DPEConfig>::createInstance(char const* szCallerName);
#endif
