...
#define ISP_VIR_REG_RANGE ISP_BASE_RANGE_NO_CAMSV
...
#define ISP_DRV_P1_PER_CQ_SET_NUM ISP_DRV_P1_CQ_NUM

#ifdef MTK_GMO_RAM_OPTIMIZE
#define ISP_TUNING_QUEUE_NUM        30
#else
...
//
    ...
    stIspTuningQueInf mTuningQueInf[ISP_DRV_P2_CQ_NUM][ISP_TUNING_QUEUE_NUM];
    ...
    =   {
            ...
            {CAM_ISP_AF,             0x46B0,  18   },
            {CAM_ISP_SGG3,           0x4700,  3    },
            {CAM_ISP_SGG5,           0x4710,  3    },
            ...
            {CAM_ISP_AE_D,           0x6650,  19   },
            {CAM_ISP_SGG1_D,         0x66A0,  3    },
            {CAM_ISP_AF_D,           0x66B0,  18   },
            {CAM_ISP_SGG3_D,         0x6700,  3    },
            {CAM_ISP_SGG5_D,         0x6710,  3    },
            ...
            {CAM_DMA_AFO,            0x7480,  7    },
            // {CAM_DMA_FEO,            0x7480,  7    },   // TODO: wait for P2 to modify
...
MINT32 IspDrvImp::isp_fd_close(MINT32 mIspFd)
{
    ...
    if(mIspFd >= 0)
    {
        ...
        for(MUINT32 i=0;i<this->m_FdNd.size();i++){
            if(it->mIspFd == mIspFd)
            {
                mIonFd = it->mIonFd;
                LOG_INF("mIonFd(%d),at(%d/%d).\n", mIonFd,i,(int)this->m_FdNd.size());
...
#define ISP_TUNINGQUE_SIZE ISP_VIR_REG_RANGE
    ...
    {
        // mmap isp reg
        if (strcmp(userName, "FakeSeninf") == 0)
            mpIspHwRegAddr = (MUINT32 *) mmap(0, SENINF_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, mFd, SENINF_BASE_HW);
        else
    ...
    for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
        for(int j=0;j<ISP_TUNING_QUEUE_NUM;j++){
            mTuningQueInf[i][j].queSize = ISP_TUNINGQUE_SIZE;
        ...
        mTuningQueIdx[i].keepP2PreviewQue.pTuningQue.queSize = ISP_TUNINGQUE_SIZE;
        ...
        //
        mTuningQueIdx[i].keepP1Que.pTuningQue.queSize = ISP_TUNINGQUE_SIZE;
    ...
    for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
        ...
        mTuningQueIdx[i].keepP2PreviewQue.queSize = ISP_TUNINGQUE_SIZE;
        ...
        memset((MUINT8*)mTuningQueIdx[i].keepP2PreviewQue.pTuningQue,0,mTuningQueIdx[i].keepP2PreviewQue.queSize);
        //
        mTuningQueIdx[i].keepP1Que.queSize = ISP_TUNINGQUE_SIZE;
...
MBOOL IspDrvImp::cqTableControl(ISP_DRV_CQTABLE_CTRL_ENUM cmd,int burstQnum)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQTABLE_CTRL_ALLOC:
            ...
            mIspVirRegSize = ISP_VIR_REG_RANGE * mTotalCQNum;
            ...
            LOG_VRB("v(0x%x)(0x%x)(%p)(%p)(%p)(%p)",mIspVirRegSize,m_ispVirRegBufInfo.size, \
                                                            (void*)m_ispVirRegBufInfo.virtAddr,(void*)m_ispVirRegBufInfo.phyAddr, \
            ...
            for (int i = 0; i < mTotalCQNum; i++) {
                mpIspVirRegAddrVA[i] = mpIspVirRegBufferBaseAddr + (i * ISP_VIR_REG_RANGE / sizeof(MUINT32));
                mpIspVirRegAddrPA[i] = mpIspVirRegAddrPA[0] + (i * ISP_VIR_REG_RANGE / sizeof(MUINT32));
            ...
            LOG_DBG("cq(0x%x)(0x%x)(%p)(%p)(%p)(%p)",mIspCQDescSize,m_ispCQDescBufInfo.size, \
                                                            (void*)m_ispCQDescBufInfo.virtAddr,(void*)m_ispCQDescBufInfo.phyAddr, \
            ...
            for (int i = 0; i < mTotalCQNum; i++)
            {
                ...
                LOG_DBG("cq:[%d],mpIspCQDescriptor:Virt[%p]/Phy[%p],size/num(%d/%d/%d)",i,mpIspCQDescriptorVirt[i],mpIspCQDescriptorPhy[i],(int)sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT),CAM_MODULE_MAX,(int)sizeof(mIspCQDescInit));
            }
            ...
            if (sizeof(CQ_RTBC_RING_ST) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data too large(%d)>(%d)",(int)sizeof(CQ_RTBC_RING_ST),(int)sizeof(mIspCQDescInit));
                   return MFALSE;
            }
...
MINT32 IspDrvImp::cqNumInfoControl(ISP_DRV_CQNUMINFO_CTRL_ENUM cmd,int newValue)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQNUMINFO_CTRL_GET_CURBURSTQNUM:   //get current supported burst queue number
            ...
            if((int)value != (int)mCurBurstQNum)
            {
...
MBOOL IspDrvImp::dumpCQTable(ISP_DRV_CQ_ENUM cq, MINT32 burstQIdx,MUINT32 dupCqIdx, ISP_DRV_CQ_CMD_DESC_STRUCT* cqDesVa, MUINT32* cqVirVa)
{
    ...
    LOG_INF("+,realCQIdx(%d),cq(%d),burstQIdx(%d),dupCqIdx(%d),cqDesVa(%p),cqVirVa(%p)",realCQIdx,cq,burstQIdx,dupCqIdx,cqDesVa,cqVirVa);
...
MBOOL IspDrvImp::waitIrq(ISP_DRV_WAIT_IRQ_STRUCT* pwaitIrq)
{
    ...
    {   //change to non-clear wait and start to wait again
        ...
        while((waitirq.Timeout > 0) && (Ret== (-SIG_ERESTARTSYS)))//receive restart system call again
        {
            ...
            if(pwaitIrq->UserInfo.UserKey<0) {/*cam1*/
                LOG_INF("Leave ERESTARTSYS: Ret(%d),Type(%d),Status(0x%08x),Timeout(%d),Tb-Ta(%d us).",Ret,pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout,(Tb-Ta));
            } else {
                LOG_INF("Leave ERESTARTSYS: Ret(%d),Type(%d),Status(0x%08x),Timeout(%d),Tb-Ta(%d us).",Ret,pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->Timeout,(Tb-Ta));
            }
    }
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::mmdvfs_ctrl(E_MMDVFS_CMD eCmd,MUINT8* pData, MUINT8* pIncClk)
{
    switch(eCmd){
        case _UPDATE_MMDVFS_STEP:
            {
                ISP_GET_SUPPORTED_ISP_CLK ispclk;
                MUINT32 clk;
                MUINT32 startIdx;
                MBOOL bFindClk = MFALSE;
                if(ioctl(mFd, ISP_GET_ISPCLK, (void*)&ispclk) < 0){
                    LOG_ERR("ISP_SET_MMDVFS error\n");
                    return MFALSE;
                }
                else
                {
                    ...
                    // ispclk[1]: medium    312
                    // ispclk[2]: low       227
                    startIdx = (*(UINT32*)pIncClk) ? (ispclk.clklevelcnt-2) : (ispclk.clklevelcnt-1);
                    for (int i=startIdx; i>=0; i--) {
                        if(!bFindClk) {
                            if(*(UINT32*)pData <= ispclk.clklevel[i]) {
                                clk = ispclk.clklevel[i];
                                bFindClk = MTRUE;
                            }
                        }
                    }
                    if(!bFindClk)
                        clk = ispclk.clklevel[0];
                    LOG_INF("ISP_SET MMDVFS, clk(%d), pIncClk(%d)\n", *(UINT32*)pData,
                        *(UINT32*)pIncClk);
                    if(ioctl(mFd, ISP_SET_ISPCLK, (MUINT8*)&clk) < 0){
                        LOG_ERR("ISP_SET_MMDVFS error\n");
                        return MFALSE;
                    }
                }
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::set_pmqos(E_PMQOS_CMD eCmd,MUINT8* pData)
{
    switch(eCmd){
        case _SET_PM_QOS_INFO:
            if(ioctl(mFd, ISP_SET_PM_QOS_INFO, (ISP_PM_QOS_INFO_STRUCT*)pData) < 0){
        ...
        case _SET_PM_QOS_ON:
            {
                ...
                dbg[1] = *((MUINT32*)pData);
                dbg[0] = 1;
                if(ioctl(mFd, ISP_SET_PM_QOS, (unsigned int*)dbg) < 0){
...
isp_reg_t* IspDrvImp::getCurHWRegValues()
{
    ...
    for(MINT32 i=0;i<(MINT32)(size/sizeof(MUINT32));i++)
    {
    ...
    switch( pbuf_ctrl->ctrl ) {
        ...
        case ISP_RT_BUF_CTRL_CLEAR:
            ...
            if (sizeof(CQ_RTBC_RING_ST) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data exceed buffer size(%d)>(%d)",(int)sizeof(CQ_RTBC_RING_ST),(int)sizeof(mIspCQDescInit));
            }
                    ...
                    if((gIspTuningFuncBitMapp[i].eTuningCtlByte==eTuningCtlByte_P2)||(gIspTuningFuncBitMapp[i].eTuningCtlByte==eTuningCtlByte_P1)){
                        ...
                        switch(i){
                            ...
                            case eIspTuningMgrFuncBit_Bnr:
                                mTuningQueIdx[p2Cq].keepTpipeField.bnr.bpc_en = pReadP2TuningQue->CAM_BNR_BPC_CON.Bits.BPC_EN;
                                mTuningQueIdx[p2Cq].keepTpipeField.bnr.bpc_tbl_en = pReadP2TuningQue->CAM_BNR_BPC_D_CON.Bits.BPC_LUT_EN;
                                break;
...
//set by imageio path                            mTuningQueIdx[p2Cq].keepTpipeField.nr3d.nr3d_on_height = pReadP2TuningQue->CAM_NR3D_ON_SIZ.Bits.NR3D_ON_HT;
        ...
        for (k=0;k<(MUINT32)checkNum;k++){
            ...
            LOG_DBG("camCtlEnP1(0x%x),camCtlEnP1D(0x%x),keepP1UpdateFuncBit(0x%x),eUpdateFuncBit(0x%x)",camCtlEnP1,camCtlEnP1D,(unsigned int)mTuningQueIdx[p2Cq].keepP1UpdateFuncBit,(unsigned int)mTuningQueInf[p2Cq][currQueueIdx].eUpdateFuncBit);
            //
            for(i=0;i<eIspTuningMgrFuncBit_Num;i++){
...
)
{
    ...
    pTuningField.bnr.bpc_tbl_en = pTuningRegMappingQue->CAM_BNR_D_BPC_CON.Bits.BPC_LUT_EN;
    //
...
)
{
    ...
    switch(softScenario){
        ...
        case eSoftwareScenario_Sub2_VSS_Mfb_Capture:
        case eSoftwareScenario_Sub2_VSS_Mfb_Blending:
        case eSoftwareScenario_Sub2_VSS_Mfb_Mixing:
        ...
        case eSoftwareScenario_Sub2_Vfb_Stream_2:
        ...
        case eSoftwareScenario_Sub2_Normal_Stream:
        case eSoftwareScenario_Sub2_Vfb_Stream_1:
        case eSoftwareScenario_Sub2_Normal_Capture:
        case eSoftwareScenario_Sub2_Pure_Raw_Stream:
        case eSoftwareScenario_Sub2_CC_Raw_Stream:
        case eSoftwareScenario_Sub2_ZSD_Capture:
        case eSoftwareScenario_Sub2_Mfb_Capture:
        case eSoftwareScenario_Sub2_Mfb_Blending:
        case eSoftwareScenario_Sub2_Mfb_Mixing:
        case eSoftwareScenario_Sub2_IP_Raw_TPipe_Stream:
            p2Cq = ISP_DRV_P2_CQ3;
            break;
        default:
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::enqueP2Frame(MUINT32 callerID,MINT32 p2burstQIdx,MINT32 p2dupCQIdx ,MINT32 frameNum, MINT32 p2Scenario)
{
    MBOOL ret=MTRUE;
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::dequeP2FrameSuccess(MUINT32 callerID,MINT32 p2dupCQIdx, MINT32 p2Scenario)
{
    MBOOL ret=MTRUE;
    ISP_DRV_ED_BUFQUE_STRUCT edBufQueV;
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::dequeP2FrameFail(MUINT32 callerID,MINT32 p2dupCQIdx,MINT32 p2Scenario)
{
    MBOOL ret=MTRUE;
    ISP_DRV_ED_BUFQUE_STRUCT edBufQueV;
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::waitP2Frame(MUINT32 callerID,MINT32 p2dupCQIdx,MINT32 p2Scenario,MINT32 timeoutUs)
{
    MBOOL ret=MTRUE;
    ISP_DRV_ED_BUFQUE_STRUCT edBufQueV;
...
MBOOL IspDrvImp::getDeviceInfo(E_DEVICE_INFO eCmd, ISP_DRV_PASS1_PATH_ENUM path, MUINT8* pData)
{
    MBOOL rst = MTRUE;

//    LOG_DBG("- E. eCmd = 0x%x, path = %d", eCmd, path);
    switch(eCmd){
        case _GET_VSYNC_CNT:
            switch(path){
                case ISP_DRV_PASS1_PATH_P1:
                    *(MUINT32*)pData = _IRQ;
                    break;
                case ISP_DRV_PASS1_PATH_P1_D:
                    *(MUINT32*)pData = _IRQ_D;
                    break;
                default:
                    LOG_ERR("unsupported path:0x%x\n",path);
                    return MFALSE;
                    break;
            }
            if(ioctl(mFd, ISP_GET_VSYNC_CNT,(unsigned char*)pData) < 0){
        ...
        case _GET_SOF_CNT://for hwsync dbg log
            ...
            if(ioctl(mFd, ISP_GET_CUR_SOF,(unsigned char*)pData) < 0){
...
MBOOL IspDrvImp::setDeviceInfo(E_DEVICE_INFO eCmd, ISP_DRV_PASS1_PATH_ENUM path, MUINT8* pData)
{
//    LOG_DBG("- E. eCmd = 0x%x, path = %d", eCmd, path);
    switch(eCmd){
        case _RESET_VSYNC_CNT:
            if(ioctl(mFd,ISP_RESET_VSYNC_CNT,NULL) < 0){
                LOG_ERR("_RESET_VSYNC_CNT error\n");
                return MFALSE;
            }
            break;
        default:
...
MBOOL IspDrvVirImp::reset(MINT32 rstpath) {
    ...
    memset(mpIspVirRegBuffer,0x00,ISP_VIR_REG_RANGE);
    return MTRUE;
}
//-----------------------------------------------------------------------------
