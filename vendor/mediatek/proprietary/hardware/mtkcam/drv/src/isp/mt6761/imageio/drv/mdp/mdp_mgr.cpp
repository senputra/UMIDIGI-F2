...
//         if(cfgData.dstDma[index].withCropM && cfgData.dstDma[index].enSrcCrop)
        {
            ...
            LOG_INF("index(%d),capbility(%d),fmt(%d),rot(%d),flip(%d),(w,h,s)=(0x%lx,0x%lx,0x%lx),C(w,h,s)=(0x%lx,0x%lx,0x%lx),V(w,h,s)=(0x%lx,0x%lx,0x%lx),dpStreamtype(%d),mdpindx(%d)",
            ...
            if(cfgData.dstDma[index].memBuf.base_vAddr == 0 || cfgData.dstDma[index].memBuf.base_pAddr == 0) {
                ...
                LOG_ERR("dst, W(%lx),H(%lx),W_c(%lx),H_c(%lx),W_v(%lx),H_v(%lx)",cfgData.dstDma[index].size.w,
                                                                            cfgData.dstDma[index].size.h,
                ...
                LOG_ERR("stride(%lx),stride_c(%lx),stride_v(%lx)",cfgData.dstDma[index].size.stride,
                                                               cfgData.dstDma[index].size_c.stride,
            ...
            LOG_INF("dstSizeList=(0x%8x,0x%8x,0x%8x),dstVirList=(0x%8lx,0x%8lx,0x%8lx),dstPhyList=(0x%8x,0x%8x,0x%8x),plane num(%d)",
...
{
    LOG_DBG("+,unInitCount(%d)",mInitCount);
    //
    Mutex::Autolock lock(mLock);
    //
    MBOOL err = MDPMGR_NO_ERROR;
...
MINT32 MdpMgrImp::startMdp(MDPMGR_CFG_STRUCT cfgData)
{
    ...
    LOG_INF("+,cqIndx(%u),sceID(%u),cqPA(0x%x),tpipVa(0x%lx),tpipPa(0x%x),isWaitBuf(%d),venc(%d)", \
...
DpIspStream *MdpMgrImp::selectDpStream(MUINT32 sceID, MUINT32 cqIdx)
{
    ...
    if(cqIdx==ISP_DRV_CQ01) {
        if (m_pCq1DpStream[sceID] == NULL) {
            m_pCq1DpStream[sceID] = allocDpStream(sceID);
        }
        ...
        if (m_pCq2DpStream[sceID] == NULL) {
            m_pCq2DpStream[sceID] = allocDpStream(sceID);
        }
        ...
        if (m_pCq3DpStream[sceID] == NULL) {
            m_pCq3DpStream[sceID] = allocDpStream(sceID);
        }
