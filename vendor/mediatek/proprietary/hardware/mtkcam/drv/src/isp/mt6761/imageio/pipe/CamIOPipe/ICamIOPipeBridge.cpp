...
class ICamIOPipeBridgeDbgTimer
{
protected:
...
public:
    ICamIOPipeBridgeDbgTimer(char const*const pszTitle)
        : mpszName(pszTitle)
        , mIdx(0)
        , mi4StartUs(getUs())
        , mi4LastUs(getUs())
...
public:
    ...
    virtual MBOOL   irq(EPipePass pass, EPipeIRQ irq_int, int userEnum, MUINT32 const u4TimeoutMs);  // old interface
    virtual MBOOL   registerIrq(Irq_t irq);
    virtual MBOOL   irq(Irq_t irq);
    virtual MBOOL   flushIrq(EPipePass pass, EPipeIRQ irq_int, int userEnum, MUINT32 const u4TimeoutMs);
...
MBOOL
ICamIOPipeBridge::
irq(EPipePass pass, EPipeIRQ irq_int, int userEnum, MUINT32 const u4TimeoutMs) {
    return  getImp()->irqDynamicTimeout(pass, irq_int, userEnum, u4TimeoutMs);
}

MBOOL
ICamIOPipeBridge::
flushIrq(EPipePass pass, EPipeIRQ irq_int, int userEnum, MUINT32 const u4TimeoutMs) {
    return  getImp()->flushIrq(pass, irq_int, userEnum, u4TimeoutMs);
}
...
{
#define WAITLOCK_THRESHOLD 2000 // us
    ICamIOPipeBridgeDbgTimer dbgTmr("ICamIOPipeBridgeSendCommand");
    MUINT32 Ta=0, Tb=0;
    ...
    if (Tb - Ta > WAITLOCK_THRESHOLD) {
        PIPE_WRN("[ICamIOPipeBridge] sendCommand wait lock too long! Tb-Ta(%d us)", Tb-Ta);
    }
...
dequeInBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo,EDrvScenario eDrvSce, MINT32 const dequeCq,MINT32 const dequeBurstQIdx, MINT32 const dequeDupCqIdx, MUINT32 const u4TimeoutMs /*= 0xFFFFFFFF*/)
{
    UNUSED(eDrvSce);
    UNUSED(dequeCq);
    UNUSED(dequeBurstQIdx);
    UNUSED(dequeDupCqIdx);
