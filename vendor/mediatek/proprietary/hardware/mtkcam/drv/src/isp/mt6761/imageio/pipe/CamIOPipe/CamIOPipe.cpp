...
#define  AF_WORK_AROUND

#ifdef   AF_WORK_AROUND
#define  AF_FORCE_BIN_WIDTH 2592
#define  AF_D_UPCLK_WIDTH_LB 1976
#define  AF_D_UPCLK_WIDTH_HB 2624
#endif
...
MUINT32 p1ConvModuleID(MUINT32 id) {
    ...
    switch (id) {
		...
		case EModule_SGG5_D: newId = CAM_ISP_SGG5_D; break;
        case EModule_SGG3_D: newId = CAM_ISP_SGG3_D; break;
...
MBOOL cal_isp_clk(MUINT32 pixelRate, MINT32 two_pix, MUINT32* isp_clk)
{
    MUINT32 clkIdx;
    MUINT32 mult = (two_pix) ? 2 : 1;
    // Twin:    isp_clk = pixel_rate / 2 /0.9
    // Single:  isp_clk = pixel_rate / 1 /0.9
    *isp_clk = (MUINT32)ceil((double)pixelRate*10.0/(double)mult/9.0);
    PIPE_INF("pixelRate:(%d), two_pix:(%d), isp_clk:(%d)\n", pixelRate, two_pix,*isp_clk);
    return 0;
}
...
public:
    CamioDbgTimer(char const*const pszTitle)
        : mpszName(pszTitle)
        , mIdx(0)
        , mi4StartUs(getUs())
        , mi4LastUs(getUs())
    ...
    inline MBOOL ProfilingPrint(char const*const pszInfo = "") const
    {
        ...
        if  (0==mIdx)
        {
            PIPE_INF("[%s] %s:(%d-th) ===> [start-->now: %.06f ms]", mpszName, pszInfo, mIdx++, (float)(i4EndUs-mi4StartUs)/1000);
        }
        else
        {
            PIPE_INF("[%s] %s:(%d-th) ===> [start-->now: %.06f ms] [last-->now: %.06f ms]", mpszName, pszInfo, mIdx++, (float)(i4EndUs-mi4StartUs)/1000, (float)(i4EndUs-mi4LastUs)/1000);
        }
        mi4LastUs = i4EndUs;
      ...
      mPath1(0)
{
    ...
    mMagicNum2 = 0x0000; //IMGO
    mMagicNum0_D = 0xFFFF; //RRZO
    mMagicNum1_D = 0xFFFE; //3A
    mMagicNum2_D = 0x0000; //IMGO
    mOpenedPort = 0x0;
    mImgSel0 = 0x0;
    mImgSel1 = 0x0;
...
int path  = CAM_ISP_PASS1_START;
   ...
   if (( CAM_ISP_CQ_NONE != m_pass1_Raw_CQ ) || ( CAM_ISP_CQ_NONE != m_pass1_RawD_CQ )) {
       path  = CAM_ISP_PASS1_START;
       m_CamPathPass1.abort((void*)&path);
   }
...
int int_p1_en = CAM_CTL_INT_P1_EN_VS1_INT_EN | \
                ...
                CAM_CTL_INT_P1_EN_CQ0_ERR_EN;


int int_p1_en2 = CAM_CTL_INT_P1_EN_CQ0_DONE_ST_EN;
int ctl_sel = 0;
...
//int camsv2_tg_fmt = -1;
//int camsv2_tg_sw = -1;
MBOOL bAF_force_bin = MFALSE;
MBOOL bAF_D_inc_clk = MFALSE;
...
PIPE_DBG("+, CQ:%d/%d",m_pass1_Raw_CQ, m_pass1_RawD_CQ);
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ ) {
        ...
        PIPE_INF("P1 vInPorts:[%d]:fmt(0x%x),PM(%d),w/h/str(%d/%d/%d,%d,%d),type(%d),idx(%d),io(%d),fps(%d), pixel_rate(%d)\n",
                ...
                vInPorts[i]->type, vInPorts[i]->index, vInPorts[i]->inout,vInPorts[i]->tgFps, vInPorts[i]->pixelRate);
        //
        switch ( vInPorts[i]->index ) {
            case NSImageio::NSIspio::EPortIndex_TG1I:
                ...
                // always enable dmx for AF work around
                en_p1 |= CAM_CTL_EN_P1_DMX_EN;
...
#ifdef AF_WORK_AROUND
                // if(AF_FORCE_BIN_WIDTH <= vInPorts[i]->u4ImgWidth)
                    bAF_force_bin = MTRUE;
#endif
                if(0!=cal_isp_clk(vInPorts[i]->pixelRate, (int)pass1_twin, &ispclk)) {
                    PIPE_ERR("ERROR:in cal isp raw clock, pixel_rate(%d), twin(%d)!", vInPorts[i]->pixelRate, pass1_twin);
                }
            ...
            case NSImageio::NSIspio::EPortIndex_TG2I:
                ...
                // always enable dmx for AF work around
                en_p1_d |= CAM_CTL_EN_P1_D_DMX_D_EN;
                ...
                if ( pass1D_twin ) {
                    PIPE_ERR("TG2 Not Support Twin");
                    return MFALSE;
                }
                //CAM_ULOGMD("m_P1HWRWctrl[2].init ");
                m_P1HWRWctrl[NSImageio::NSIspio::EPortIndex_TG2I].init((ISP_DRV_CQ_ENUM)pass1_CQ_D, \
                                 (MUINT32*)this->m_pIspDrvShell);
//Marked. Solution in middleware to restrict MAX CAM NUM = 1
//#ifdef AF_WORK_AROUND
//                if(AF_D_UPCLK_WIDTH_LB <= vInPorts[i]->u4ImgWidth && AF_D_UPCLK_WIDTH_HB >= vInPorts[i]->u4ImgWidth)
//                    bAF_D_inc_clk = MTRUE;
//#endif
                if(0!=cal_isp_clk(vInPorts[i]->pixelRate, 0, &ispclk)) {
                    PIPE_ERR("ERROR:in cal isp raw_d clock, pixel_rate(%d), twin(%d)!", vInPorts[i]->pixelRate, pass1_twin);
                }
    ...
    ePixMode = ((1 == two_pxl)? ISP_QUERY_2_PIX_MODE : ISP_QUERY_1_PIX_MODE);
    ...
    if (-1 != idx_imgo ) { // use output dma crop
        ...
        // use output dma crop
        this->configDmaPort(&portInfo_imgo,m_camPass1Param.imgo,(MUINT32)pixel_byte_imgo,(MUINT32)0,(MUINT32)1,ESTRIDE_1ST_PLANE,ePixMode);
...
//        m_camPass1Param.imgo.format = portInfo_imgo.eImgFmt;
    ...
    if (-1 != idx_rrzo ) { // use output dma crop
        ...
        if(portInfo_rrzo.crop1.h < portInfo_rrzo.u4ImgHeight){
         ...
         // use output dma crop
        this->configDmaPort(&portInfo_rrzo,m_camPass1Param.rrzo,(MUINT32)pixel_byte_rrzo,(MUINT32)0,(MUINT32)1,ESTRIDE_1ST_PLANE,ePixMode);
        ...
        PIPE_INF("[3ASync]rrzo crop=%ld,%ld",m_camPass1Param.rrzo.crop.w, m_camPass1Param.rrzo.crop.h );
    ...
    if (-1 != idx_imgo_d ) {
        ...
        this->configDmaPort(&portInfo_imgo_d,m_camPass1Param.imgo_d,(MUINT32)pixel_byte_imgo_d,(MUINT32)0,(MUINT32)1,ESTRIDE_1ST_PLANE,ePixMode);
...
//        m_camPass1Param.imgo_d.format = portInfo_imgo_d.eImgFmt;
    ...
    if (-1 != idx_rrzo_d ) { // use output dma crop
        ...
        if(portInfo_rrzo_d.crop1.h < portInfo_rrzo_d.u4ImgHeight){
         ...
         // use output dma crop
        this->configDmaPort(&portInfo_rrzo_d,m_camPass1Param.rrzo_d,(MUINT32)pixel_byte_rrzo_d,(MUINT32)0,(MUINT32)1,ESTRIDE_1ST_PLANE,ePixMode);
...
//        m_camPass1Param.rrzo_d.format = portInfo_rrzo_d.eImgFmt;
        ...
        m_camPass1Param.rrzo_d.tgFps = tgFps;
    ...
    if(-1 != idx_camsv_imgo){
        ...
        this->configDmaPort(&portInfo_camsv_imgo,m_camPass1Param.camsv_imgo,
                (MUINT32)pixel_byte_camsv_imgo,(MUINT32)0,(MUINT32)1,ESTRIDE_1ST_PLANE,
                ((MTRUE == bCamsv_2pix)? ISP_QUERY_2_PIX_MODE : ISP_QUERY_1_PIX_MODE));
    ...
    if(-1 != idx_camsv2_imgo){
        ...
        this->configDmaPort(&portInfo_camsv2_imgo,m_camPass1Param.camsv2_imgo,
                (MUINT32)pixel_byte_camsv2_imgo,(MUINT32)0,(MUINT32)1,ESTRIDE_1ST_PLANE,
                ((MTRUE == bCamsv2_2pix)? ISP_QUERY_2_PIX_MODE : ISP_QUERY_1_PIX_MODE));
    ...
    m_CamPathPass1.setIspClk(ispclk, bAF_D_inc_clk);
    //
...
MBOOL
CamIOPipe::
irqDynamicTimeout(EPipePass pass, EPipeIRQ irq_int, int userEnum, MUINT32 const u4TimeoutMs) {
    int    ret = 0;
    MINT32 type = 0;
    MUINT32 irq = 0;

    PIPE_DBG("+ tid(%d) (pass,irq_int)=(0x%08x,0x%08x), Timeout=%d", gettid(), pass, irq_int, u4TimeoutMs);
    ...
    //
    PIPE_DBG("(type,user,irq,timeout)=(0x%08x,%d,0x%08x,%d)", type, userEnum, irq, u4TimeoutMs);
    //
    ret = m_CamPathPass1.waitIrqDynamicTimeout(type, irq, userEnum, u4TimeoutMs);
...
MBOOL
CamIOPipe::
flushIrq(EPipePass pass, EPipeIRQ irq_int, int userEnum, MUINT32 const u4TimeoutMs)
{
    int    ret = 0;
    MINT32 type = 0;
    MUINT32 irq = 0;
    ...
    // irq_int
    switch (pass) {
        case EPipePass_PASS1_TG1:
            type = ISP_DRV_IRQ_TYPE_INT_P1_ST;
            break;
        case EPipePass_PASS1_TG1_D:
    ...
    if (EPIPEIRQ_VSYNC == irq_int) {
        switch (pass) {
            case EPipePass_PASS1_TG1:
                irq = CAM_CTL_INT_P1_STATUS_VS1_INT_ST;
                break;
            case EPipePass_PASS1_TG1_D:
                irq = CAM_CTL_INT_P1_STATUS_D_VS1_INT_ST;
                break;
        }
    }
    else if (EPIPEIRQ_PATH_DONE == irq_int) {
        switch (pass) {
            case EPipePass_PASS1_TG1:
                irq = CAM_CTL_INT_P1_STATUS_PASS1_DON_ST;
                break;
            case EPipePass_PASS1_TG1_D:
                irq = CAM_CTL_INT_P1_STATUS_D_PASS1_DON_ST;
                break;
        }
    }
    else if (EPIPEIRQ_SOF == irq_int) {
        switch (pass) {
            case EPipePass_PASS1_TG1:
                irq = CAM_CTL_INT_P1_STATUS_SOF1_INT_ST;
                break;
            case EPipePass_PASS1_TG1_D:
                irq  = CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST;
                break;
        }
    }
    else {
    ...
    PIPE_DBG("(type,user,irq)=(0x%08x,%d,0x%08x)", type, userEnum, irq);
    //
    ret = m_CamPathPass1.flushIrq(type, irq, userEnum, u4TimeoutMs);

    if (ret != 0) {
        PIPE_ERR("flushIrq error!");
        return  MFALSE;
    }
    return  MTRUE;
}
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    CamioDbgTimer dbgTmr("CamioSendCommand");
    MUINT32 Ta=0, Tb=0;
#define WAITLOCK_THRESHOLD 100 // us
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_RRZ:
            ...
            this->mRunTimeCfgLock.lock();
            Tb = dbgTmr.getUs();
            if (Tb - Ta > WAITLOCK_THRESHOLD) {
                PIPE_WRN("[3ASync] SET_RRZ wait lock too long! Tb-Ta(%d us)", Tb-Ta);
            }
            ...
            if ( NSImageio::NSIspio::EPortIndex_TG1I == arg2){
                if (pUserRrzPara->crop.x > this->m_camPass1Param.rrz_in_size.w) {
                    PIPE_ERR("[3ASync]ERROR: Crop invalid(x(%d)>size_w(%ld))",pUserRrzPara->crop.x, this->m_camPass1Param.rrz_in_size.w);
                    m_rrz_update_cnt = 0;
                    this->mRunTimeCfgLock.unlock();
                    return MFALSE;
                }
                if (pUserRrzPara->crop.w > this->m_camPass1Param.rrz_in_size.w) {
                    PIPE_ERR("[3ASync]ERROR: Not Support Scale up(in(%ld)<roi_w(%ld))", pUserRrzPara->crop.w, this->m_camPass1Param.rrz_in_size.w);
                    m_rrz_update_cnt = 0;
                    this->mRunTimeCfgLock.unlock();
                    return MFALSE;
                }
                ...
                if (pUserRrzPara->crop.x > this->m_camPass1Param.rrz_d_in_size.w) {
                    PIPE_ERR("[3ASync]ERROR:D: CropInvalid(x(%d)>size_w(%ld))",pUserRrzPara->crop.x, this->m_camPass1Param.rrz_d_in_size.w);
                    m_rrzd_update_cnt = 0;
                    this->mRunTimeCfgLock.unlock();
                    return MFALSE;
                }
                if (pUserRrzPara->crop.w > this->m_camPass1Param.rrz_d_in_size.w) {
                    PIPE_ERR("[3ASync]ERROR:D: NotSupportScale up roi_w(%ld)>(in(%ld))", pUserRrzPara->crop.w, this->m_camPass1Param.rrz_d_in_size.w);
                    m_rrzd_update_cnt = 0;
                    this->mRunTimeCfgLock.unlock();
                    return MFALSE;
                }
...
#endif
                    m_CamPathPass1.setBandW(m_camPass1Param.rrzo.tgFps);
...
#endif
                    m_CamPathPass1.setBandW(m_camPass1Param.rrzo_d.tgFps);
                    if ( 0 != m_CamPathPass1.updateCQI()){
                        if(m_CamPathPass1.CQ == CAM_ISP_CQ0)
                            PIPE_ERR("[3ASync]updateCQ Fail");
                        else if(m_CamPathPass1.CQ_D == CAM_ISP_CQ0_D)
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            Tb = dbgTmr.getUs();
            if (Tb - Ta > WAITLOCK_THRESHOLD) {
                PIPE_WRN("[3ASync] P1_UPDATE wait lock too long! Tb-Ta(%d us)", Tb-Ta);
            }
        ...
        case EPIPECmd_SET_IMGO:
            {
                ...
                if (_pDma->crop.w > this->m_camPass1Param.tg_out_size.w) {
                    PIPE_ERR("[3ASync]ERROR:TG:0x%x Not Support Scale up(in(%ld)<tg_w(%ld))",arg2, _pDma->crop.w, this->m_camPass1Param.tg_out_size.w);
                    return MFALSE;
                }
                ...
                Tb = dbgTmr.getUs();
                if (Tb - Ta > WAITLOCK_THRESHOLD) {
                    PIPE_WRN("[3ASync] SET_IMGO wait lock too long! Tb-Ta(%d us)", Tb-Ta);
                }
        ...
        case EPIPECmd_GET_MODULE_HANDLE: //handle-based
             ...
             if (NSImageio::NSIspio::EPortIndex_UNKNOW == tg ) {
                 CAM_ULOGME("GET_MODULE_HANDLE:User(%s), tg=%d is Invalid",arg3, tg);
                 return -1;
             }
        ...
        case EPIPECmd_SET_MODULE_CFG_DONE:  //handle-based
             ...
             if (Tb - Ta > WAITLOCK_THRESHOLD) {
                 PIPE_WRN("[3ASync] EPIPECmd_SET_MODULE_CFG_DONE wait lock too long! Tb-Ta(%d us)", Tb-Ta);
             }
             ...
             Tb = dbgTmr.getUs();
             if (Tb - Ta > 2000) {
                 PIPE_WRN("[3ASync] EPIPECmd_SET_MODULE_CFG_DONE does too long! Tb-Ta(%d us)", Tb-Ta);
             }
             break;
        ...
        case EPIPECmd_SET_MODULE_EN:
            ...
            if ( EModule_CAMSV_IMGO == arg1)  {
                ...
                if (true == this->m_pass1TwinMode)
                {
                    ISP_WRITE_REG(pPhyIspDrv,CAMSV2_PAK,0x07,ISP_DRV_USER_ISPF);
                }
                else
                {
                    ISP_WRITE_REG(pPhyIspDrv,CAMSV2_PAK,0x03,ISP_DRV_USER_ISPF);
                }
                ...
                tg = _p1GetModuleTgIdx(arg1,this->m_pass1TwinMode);
                if (NSImageio::NSIspio::EPortIndex_UNKNOW == tg ) {
                    CAM_ULOGME("SET_MODULE_EN:User(%s), tg=%d is Invalid",arg3, tg);
                    return -1;
                }
                if (((NSImageio::NSIspio::EPortIndex_TG1I == tg) && (!(this->mPath1 & ISP_PASS1)))||
                     ((NSImageio::NSIspio::EPortIndex_TG2I == tg) && (!(this->mPath1 & ISP_PASS1_D)))) {
        ...
        case EPIPECmd_SET_MODULE_SEL:
            ...
            if (NSImageio::NSIspio::EPortIndex_UNKNOW == tg ) {
                CAM_ULOGME("SET_MODULE_SEL:User(%s), tg=%d is Invalid",arg3, tg);
                return -1;
            }
            if (((NSImageio::NSIspio::EPortIndex_TG1I == tg) && (!(this->mPath1 & ISP_PASS1)))||
                ((NSImageio::NSIspio::EPortIndex_TG2I == tg) && (!(this->mPath1 & ISP_PASS1_D)))) {
        ...
        case EPIPECmd_GET_HBIN_INFO:
            {
                ...
                IspDrv* pDrv = NULL;
                pDrv = this->m_pIspDrvShell->getPhyIspDrv();
                cam_ctl_en_p1 = ISP_READ_BITS_NOPROTECT(pDrv,CAM_CTL_EN_P1,HBIN2_EN);
                if (cam_ctl_en_p1 == 1) {
                    *(MINT32*)arg3 = 1;
                } else {
                    *(MINT32*)arg3 = 0;
            ...
            break;
        case EPIPECmd_GET_HBIN1_INFO:
            {
                MUINT32 hbin1_en = 0x0;
                IspDrv* pDrv = NULL;

                /* Get virtual register due to sendCommand will before immed trigger.*/
                switch((MUINT32)arg1) {
                case CAM_TG_1:
                    pDrv = this->m_pIspDrvShell->getPhyIspDrv()->getCQInstance(ISP_DRV_CQ0);
                    break;
                case CAM_TG_2:
                    pDrv = this->m_pIspDrvShell->getPhyIspDrv()->getCQInstance(ISP_DRV_CQ0_D);
                    break;
                default:
                    PIPE_ERR("un-support sensor(%d) TG(%d)", (MUINT32)arg1, tg);
                    ret = -1;
                    break;
                }
                if (ret == 0) {
                    hbin1_en = ISP_READ_BITS_NOPROTECT(pDrv,CAM_CTL_EN_P1,HBIN1_EN);
                    if (hbin1_en == 1) {
                        *(MINT32*)arg3 = 1;
                    } else {
                        *(MINT32*)arg3 = 0;
        ...
        case EPIPECmd_SET_FRM_TIME:
            {
                if (MFALSE == m_CamPathPass1.updateFrameTime((MUINT32)arg1, (MUINT32)arg2)) {
