...
MBOOL IspDrvImp::init(const char* userName)
{
    ...
    MUINT32 strLen;
    //
    android::Mutex::Autolock lock(this->m_IspInitMutex);
    //
...
EXIT:
    ...
    LOG_DBG("-,ret: %d. mInitCount:(%d),m_pIspHwRegAddr_DipA(0x%x)", Result, this->m_UserCnt,gIspDrvObj[DIP_A].m_pIspHwRegAddr);
    return Result;
}
...
MBOOL IspDrvImp::uninit(const char* userName)
{
	...
	if(this->m_UserCnt != bMatch) {
		strcpy((char*)m_UserName[bMatch],m_UserName[this->m_UserCnt]);
...
EXIT:
    ...
    LOG_DBG(" - X. ret: %d. m_UserCnt: %d.", Result, this->m_UserCnt);
...
MBOOL IspDrvImp::waitIrq(ISP_WAIT_IRQ_ST* pWaitIrq)
{
    ...
    memcpy(&pWaitIrq->TimeInfo,&wait.EventInfo.TimeInfo,sizeof(ISP_IRQ_TIME_STRUCT));
...
MBOOL IspDrvImp::writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller)
{
    MINT32 ret=0;
    MUINT32 legal_range = REG_SIZE;
    LOG_DBG("Isp_write:m_HWmodule(0x%x),m_regRWMode(0x%x),(0x%x,0x%x)",this->m_HWmodule,this->m_regRWMode,Addr,Data);
    android::Mutex::Autolock lock(this->IspRegMutex);
    (void)caller;
    if(FD_CHK() == -1){
        return MFALSE;
...
MBOOL IspDrvImp::setRWMode(ISP_DRV_RW_MODE rwMode)
{
    ...
    if(rwMode == ISP_DRV_RW_IOCTL){
        org_module = this->m_HWmodule;
        if(this->m_HWmodule == CAM_A)
            this->m_HWmodule = (ISP_HW_MODULE)0XFF;//seninf0
        else if(this->m_HWmodule == CAM_B)
            this->m_HWmodule = (ISP_HW_MODULE)0XFE;//seninf1
        ...
        this->m_HWmodule = (ISP_HW_MODULE)org_module;
    return MTRUE;
}
