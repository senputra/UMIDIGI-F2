...
private:
    ...
    vector<uint64_t> twin_buff_idx[MAX_DMA_PORT_NUM];
...
SecMgrImp::SecMgrImp()
{
    ...
    m_pIspDrv = NULL;
    tee_mem_type = TEE_MAX;

    ::sem_init(&mSemSecDone, 0, 0);
    mState = SEC_STATE_NONE;

    buf_sec_mva.clear();
    buf_sec_mva_idx.clear();
    lsc_buf_sec_mva.clear();
    lsc_buff_idx.clear();
    bpc_buf_sec_mva.clear();
    bpc_buff_idx.clear();
    ...
    for (MUINT32 i = 0; i < MAX_DMA_PORT_NUM; i++) {
        twin_buf_sec_mva[i].clear();
        twin_buff_idx[i].clear();
    }
...
MBOOL SecMgrImp::SecMgr_sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    SecMgr_INF("+ (cmd,arg1,arg2,arg3)=(0x%08x,0x%08" PRIxPTR ",0x%08" PRIxPTR ",0x%08" PRIxPTR ")", cmd, arg1, arg2, arg3);
    switch ( cmd ) {
...
{
    SecMgr_INF("SecMgr_InitCA+ (%p)",ispHandle);
    ...
    this->mState = SEC_STATE_READY2RUN;
...
{
    SecMgr_INF("SecMgr_QuerySecMVAFromHandle+ HandlePA:0x%x",(MUINT32)HandlePA);
...
MBOOL SecMgrImp::SecMgr_P1RegisterShareMem(SecMgr_SecInfo* secinfo)
{
   ...
   switch(secinfo->type)
   {
       case SECMEM_CQ_DESCRIPTOR_TABLE:
       case SECMEM_VIRTUAL_REG_TABLE:
            SecMgr_INF("+ module:0x%x cq:%d dup:%d dmy:%d type:%s buff_va:0x%" PRIx64 " size:0x%x memID:0x%x",
            secinfo->module,secinfo->cq,secinfo->dupq,secinfo->dummy,str(secinfo->type),secinfo->buff_va,secinfo->buff_size,secinfo->memID);
            SecMgr_INF("+ module:0x%x cq:%d dup:%d dmy:%d sub-type:%s buff_va:0x%" PRIx64 " size:0x%x memID:0x%x",
            ...
            {
                SecMgr_ERR("Share mem register failed (module:0x%x type:%s buff:0x%" PRIx64 " size:0x%x)",
       ...
       case SECMEM_FRAME_HEADER:
            ...
            SecMgr_INF("(%s_%d)FH va:0x%" PRIx64 " sec_mva:0x%x",str_dmao(_port),fh_idx,
            ...
            break;
       case SECMEM_LSC:
            SecMgr_INF("+ module:0x%x type:%s buff:0x%" PRIx64 " size:0x%x memID:0x%x",
            ...
            if(this->lsc_buff_idx.size() < 1){
                ...
                for(MUINT32 i=0; i<this->lsc_buff_idx.size(); i++){
                    if(secinfo->buff_va == this->lsc_buff_idx.at(i)){
                        SecMgr_INF("Find LSC buffer:%d_0x%" PRIx64 " sec_mva:0x%x",i,this->lsc_buff_idx.at(i),this->lsc_buf_sec_mva.at(i));
            ...
            if(this->bpc_buff_idx.size() < 1){
                ...
                for(MUINT32 i=0; i<this->bpc_buff_idx.size(); i++){
                    if(secinfo->buff_va == this->bpc_buff_idx.at(i)){
                        SecMgr_INF("Find BPC buffer:%d_0x%" PRIx64 " sec_mva:0x%x",i,this->bpc_buff_idx.at(i),this->bpc_buf_sec_mva.at(i));
       ...
       case SECMEM_TWIN_FRAME_HEADER:
            ...
            if (this->twin_buff_idx[_port].size() < 1) {
                ...
                SecMgr_INF("(%s_%d)FH va:0x%llx sec_mva:0x%x",str_dmao(_port),this->twin_buff_idx[_port].size() - 1,
...
MBOOL SecMgrImp::SecMgr_P1MigrateTable(SecMgr_SecInfo* secinfo)
{
   ...
   switch(secinfo->type)
   {
       ...
       case SECMEM_FRAME_HEADER:
            ...
            SecMgr_INF("DMA port:%s buff:0x%" PRIx64,str_dmao(secinfo->port),secinfo->buff_va);
            break;
       case SECMEM_LSC:
       case SECMEM_BPC:
            sinfo.buff_va = secinfo->buff_va;
...
{
    SecMgr_INF("+ HandlePA(0x%" PRIx64 ") dst(%p) size(0x%x)",HandlePA,dst_buf,size);
...
{
    SecMgr_INF("+ TG:0x%x SenID:0x%x TEE_Type:0x%x chk:0x%" PRIx64,tg,sen_idx,tee_type,chk);
    ...
    {
        case 0:
            this->CamInfo[SEC_CAM_A].CamModule = SEC_CAM_A;
            this->CamInfo[SEC_CAM_A].TwinCamModule = SEC_CAM_MAX;
            this->CamInfo[SEC_CAM_A].SecTG = tg;
            this->CamInfo[SEC_CAM_A].DevID = sen_idx;
            this->CamInfo[SEC_CAM_A].chk_handle = chk;

            if (!g_isp_ca_setcaminfo(ispHandle,this->CamInfo[SEC_CAM_A]))
            {
                SecMgr_ERR("ISP CA set caminfo failed(0x%x)",tg);
                return MFALSE;
            }

            SecMgr_INF("Secure Cam(0x%x) attached",this->CamInfo[SEC_CAM_A].CamModule);
            break;

        case 1:
            this->CamInfo[SEC_CAM_B].CamModule = SEC_CAM_B;
            this->CamInfo[SEC_CAM_B].TwinCamModule = SEC_CAM_MAX;
            this->CamInfo[SEC_CAM_B].SecTG = tg;
            this->CamInfo[SEC_CAM_B].DevID = sen_idx;
            this->CamInfo[SEC_CAM_B].chk_handle = chk;

            if (!g_isp_ca_setcaminfo(ispHandle,this->CamInfo[SEC_CAM_B]))
            {
                SecMgr_ERR("ISP CA set caminfo failed(0x%x)",tg);
                return MFALSE;
            }

            SecMgr_INF("Secure Cam(0x%x) attached",this->CamInfo[SEC_CAM_B].CamModule);
            break;
...
MBOOL SecMgrImp::SecMgr_attach_twin(ISP_HW_MODULE module)
{
    ...
    switch (module) {
        case CAM_C:
            this->CamInfo[master].TwinCamModule = SEC_CAM_C;

            if (!g_isp_ca_setcaminfo(ispHandle,this->CamInfo[master]))
            {
                SecMgr_ERR("ISP CA set twin caminfo failed(0x%x)",this->CamInfo[master].SecTG);
                return MFALSE;
            }
            SecMgr_INF("Secure twin cam(0x%x) attached", this->CamInfo[master].TwinCamModule);
            break;
        default:
            //Only CAM_C can be slave cam for ISP 5.0
...
MBOOL SecMgrImp::SecMgr_detach(MUINT32 tg)
{
    ...
    if(this->Num_SecCam == 1){
        ...
        for (MUINT32 i = 0; i < MAX_DMA_PORT_NUM; i++) {
            this->twin_buf_sec_mva[i].clear();
            this->twin_buff_idx[i].clear();
        }
...
MUINT32 SecMgrImp::SecMgr_GetSecureCamStatus(ISP_HW_MODULE module)
{
    ...
    if(this->Num_SecCam > 0 && this->mState == SEC_STATE_NONE){
    ...
    SecMgr_DBG("+ (0x%x), status = %d", ispHandle, lock);
    return lock;
}
...
MBOOL SecMgrImp::SecMgr_GetSecureTwinStatus(ISP_HW_MODULE module)
{
    ...
    if ((this->CamInfo[SEC_CAM_A].TwinCamModule == module) || (this->CamInfo[SEC_CAM_B].TwinCamModule == module))
        return MTRUE;
    else
        return MFALSE;
}
...
MBOOL SecMgrImp::SecMgr_GetSecurePortStatus(ISP_HW_MODULE module,MUINT32 port)
{
    ...
    if ((this->CamInfo[SEC_CAM_A].CamModule == module) || (this->CamInfo[SEC_CAM_B].CamModule == module) ||
        (this->CamInfo[SEC_CAM_A].TwinCamModule == module) || (this->CamInfo[SEC_CAM_B].TwinCamModule == module)){
        SecMgr_INF("+ (0x%p)module:0x%x dma port:%s",ispHandle,module,str_dmao(port));
        switch(port)
        {
            case _imgo_:
...
{
    SecMgr_DBG("+ ispHandle 0x%x:,gSecMgrf: 0x%x", ispHandle, &gSecMgr);
    return (SecMgr*)&gSecMgr;
}
