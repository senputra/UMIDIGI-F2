...
IspDrvDip* IspDrvDip::createInstance(ISP_HW_MODULE hwModule, E_ISP_DIP_CQ dipCq, MUINT32 burstQueIdx, MUINT32 dupCqIdx, const char* userName)
{
    ...
    if(m_PageTbl[moduleIdx][dipCq][burstQueIdx][dupCqIdx] == NULL){
        ...
        if(sizeof(userName) < MAX_USER_NAME_SIZE) {
            ...
            LOG_ERR("[Error]the size of user name(%d) exceed defaul value(%d)",sizeof(userName),MAX_USER_NAME_SIZE);
            return NULL;
        }
...
MBOOL IspDrvDip::init(const char* userName)
{
    ...
    if(this->m_pIspDescript_vir || this->m_pIspDescript_phy || this->m_pIspVirRegAddr_va || this->m_pIspVirRegAddr_pa){
        LOG_ERR("[Error]cq is not null (0x%x)-(0x%x)-(0x%x)-(0x%x)",this->m_pIspDescript_vir,
            this->m_pIspDescript_phy,this->m_pIspVirRegAddr_va,this->m_pIspVirRegAddr_pa);
        Result = MFALSE;
        goto EXIT;
    }
    ...
    if ((this->m_dipCq == 0) && (this->m_burstQueIdx == 0) && (this->m_dupCqIdx == 0))
    {
        ...
        if(ioctl(this->m_pIspDrvImp->m_Fd,ISP_SET_MEM_INFO,&CmdqMemInfoStruct) < 0){
            LOG_ERR("set cmdq mem info fail, cmd:0x%x, memPa:0x%x, memVa:0x%x, memSizeDiff:0x%x\n", 
                CmdqMemInfoStruct.MemInfoCmd, CmdqMemInfoStruct.MemPa, CmdqMemInfoStruct.MemVa, CmdqMemInfoStruct.MemSizeDiff);
        }
    }
    ...
    if(( (_tmpVirAddr=gAllCqBufInfo.virtAddr+bufOffset) % 0x4) != 0 ){
        LOG_WRN("[Warning]iMem virRegAddr not 4 bytes alignment(0x%x), round up",gAllCqBufInfo.virtAddr+bufOffset);
        _tmpVirAddr = (gAllCqBufInfo.virtAddr+bufOffset + 0x3) & (~0x3);
    }
    if(( (_tmpPhyAddr=gAllCqBufInfo.phyAddr+bufOffset) % 0x4) != 0 ){
        LOG_WRN("[Warning]iMem phyRegAddr not 4 bytes alignment(0x%x), round up",gAllCqBufInfo.phyAddr+bufOffset);
    ...
    //
    LOG_DBG("[CQ]m_TotalUserCnt(%d)--des,va(0x%x),pa(0x%x),size(0x%x)/(0x%x)-vir,va(0x%x),pa(0x%x),size(0x%x)",this->m_TotalUserCnt,
        this->m_pIspDescript_vir,this->m_pIspDescript_phy,GET_MAX_CQ_DESCRIPTOR_SIZE(),cqDesAllocSize,
        this->m_pIspVirRegAddr_va,this->m_pIspVirRegAddr_pa,DIP_BASE_RANGE_SPECIAL);
...
{
    LOG_INF("m_hwModule(%d),m_dipCq(%d),m_burstQueIdx(%d),m_dupCqIdx(%d),va(0x%x),pa(0x%x)",
        this->m_hwModule,this->m_dipCq,this->m_burstQueIdx,this->m_dupCqIdx,this->m_pIspVirRegAddr_va,this->m_pIspDescript_phy);
