...
#include <imageio/ispio_pipe_ports.h>        // For enum EPortCapbility.
...
MINT32 _startMdp(MDPMGR_DPSTREAM_TYPE dpStreamtype,MDPMGR_CFG_STRUCT cfgData,T* pDpStream)
{
    ...
    DP_STATUS_ENUM err2;
    //MUINT32 sceID = cfgData.drvScenario; //not used
...
//         if(cfgData.dstDma[index].withCropM && cfgData.dstDma[index].enSrcCrop)
        {
            ...
            LOG_INF("index(%d),capbility(%d),fmt(%d),rot(%d),flip(%d),(w,h,s)=(%lu,%lu,%lu),C(w,h,s)=(%lu,%lu,%lu),V(w,h,s)=(%lu,%lu,%lu),dpStreamtype(%d),mdpindx(%d)",
                ...
                case ISP_MDP_DL_WDMAO:
                    {
                        if (cfgData.mdpWDMAPQParam != NULL)
                        {
                            ...
                            LOG_DBG("mdpWDMAPQParam is NOT Null");
                        }
                ...
                case ISP_MDP_DL_WROTO:
                    {
                        if (cfgData.mdpWROTPQParam != NULL)
                        {
                            ...
                            LOG_DBG("mdpWROTPQParam is NOT Null");
                        }
            ...
            switch(cfgData.dstDma[index].capbility)
               {
                case NSImageioIsp3::NSIspio::EPortCapbility_Rcrd:
                    dp_rofile=DP_PROFILE_BT601;
                    /*
                    err2 = pDpStream->setSharpness(index, 0);
                    if(err2 != DP_STATUS_RETURN_SUCCESS)
                    {
                        LOG_ERR("setSharpness fail(%d)",err2);
                        return MDPMGR_API_FAIL;
                    }*/
                    break;
                case NSImageioIsp3::NSIspio::EPortCapbility_Disp:
            ...
            if(cfgData.dstDma[index].memBuf.base_vAddr == 0 || cfgData.dstDma[index].memBuf.base_pAddr == 0) {
                ...
                LOG_ERR("dst memAddr is 0,VA(0x%8lu),PA(0x%8lu)",(unsigned long)(cfgData.dstDma[index].memBuf.base_vAddr),(unsigned long)(cfgData.dstDma[index].memBuf.base_pAddr));
            ...
            LOG_INF("dstSizeList=(0x%8x,0x%8x,0x%8x),dstVirList=(0x%lx,0x%lx,0x%lx),dstPhyList=(0x%8x,0x%8x,0x%8x),plane num(%d),WDPQ(0x%8x),WRPQ(0x%8x)",
                ...
                dstPhyList[0],dstPhyList[1],dstPhyList[2],cfgData.dstDma[index].Plane,WDMAPQEnable, WROTPQEnable);
    ...
    if ((NSImageioIsp3::NSIspio::eDrvScenario_VSS != cfgData.drvScenario) &&
        (NSImageioIsp3::NSIspio::eDrvScenario_VSS_MFB_Blending != cfgData.drvScenario) &&
        (NSImageioIsp3::NSIspio::eDrvScenario_VSS_MFB_Mixing != cfgData.drvScenario))
    {
...
MINT32 MdpMgrImp::startMdp(MDPMGR_CFG_STRUCT cfgData)
{
    ...
    //DpIspStream *pDpStream = NULL; //not used
    LOG_DBG("+,cqIndx(%u),sceID(%u),cqPA(0x%x),tpipVa(%lu),tpipPa(0x%x),isWaitBuf(%d),venc(%d)", \
    ...
    if(cfgData.dstPortCfg[ISP_MDP_DL_VENCO] == 1)
    {
        ...
        //DpIspStream *pDpStream_jpgcfg = NULL; //not used
    ...
    else
    {
        ...
        //DpVEncStream *pDpStream_venccfg = NULL; //not used
...
DpIspStream *MdpMgrImp::allocDpStream(MUINT32 sceID)
{
    ...
    LOG_DBG("-,pRetDpIspStream(0x%lx)",(unsigned long)pRetDpIspStream);
