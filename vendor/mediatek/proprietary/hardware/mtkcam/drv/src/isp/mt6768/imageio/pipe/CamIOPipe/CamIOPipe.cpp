...
MBOOL CamIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts,CAMIO_Func func)
{
    ...
    if(this->m_camPass1Param.bypass_tg == MFALSE)
    {
        ...
        switch(func.Bits.DATA_PATTERN){
            case ECamio_4cell:
                ...
                this->m_PathCtrl.m_cfgInParam.datPat = E_4CELL;
                this->m_camPass1Param.m_data_pat = (E_CamPattern)func.Bits.DATA_PATTERN;
                bVBN = MTRUE;
                PIPE_INF("func.Bits.DATA_PATTERN : ECamio_4cell\n");
                break;
            case ECamio_Dual_pix:
                this->m_PathCtrl.m_cfgInParam.bDbn = MTRUE;
                this->m_camPass1Param.m_data_pat = (E_CamPattern)func.Bits.DATA_PATTERN;
                this->m_PathCtrl.m_cfgInParam.datPat = E_DUAL_PIX;
                PIPE_INF("func.Bits.DATA_PATTERN : ECamio_Dual_pix\n");
                break;
            case ECamio_Normal:
            default:
                ...
                PIPE_INF("func.Bits.DATA_PATTERN : ECamio_Normal\n");
                break;
        }
        //
        ...
        if((cfgOutParam.bin_en== MTRUE) || (this->m_PathCtrl.m_cfgInParam.datPat == E_4CELL)) {
            this->m_camPass1Param.m_NonScale_RawType = CamPathPass1Parameter::_PURE_RAW;
            bPureRaw = MTRUE;
    ...
    //ctrl sel
    {
        ...
        switch(func.Bits.DATA_PATTERN){
            case ECamio_4cell:
                ptr->Path_sel(bPureRaw,bPak,this->m_camPass1Param.bypass_tg,Cam_path_sel::_4cell_);
                break;
            case ECamio_Dual_pix:
            ...
            //arg2: rawtype: list<MUINT32>*, 1:proc, 0:pure
            {
                ...
                for(it2=pInput2_L->begin(),it=pInput_L->begin();it!=pInput_L->end();it++,it2++){
                    ...
                    PIPE_DBG("(configpipe_img_format,enque_img_format,configpipe_img_stride,enque_img_stride,tgCropW)=(0x%x,0x%x,%d,%d,%d)\n",\
                               imgo.img_fmt_configpipe, imgo.img_fmt, imgo.out.stride, imgo.enqueue_img_stride, imgo.tgCropW);
                        ...
                        switch(this->m_camPass1Param.m_NonScale_RawType){
                            case CamPathPass1Parameter::_PURE_RAW:
                                switch (this->m_camPass1Param.m_data_pat) {
                                  ...
                                  case Cam_path_sel::_4cell_:
                                      PIPE_ERR("DualPix/4CELL(%d) force pure raw path\n", (int)this->m_camPass1Param.m_data_pat);
                                      break;
