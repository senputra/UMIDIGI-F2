...
#include "imageio/ispio_utility.h" //E_ISP_PIXMODE for configDmaPort
//
...
public:
    ...
    int             abort( void* pParam );


    inline int              waitIrq( int type,unsigned int irq , int userNumber){return this->_waitIrq( type,irq,userNumber );}
    inline int              registerIrq( NSImageioIsp3::NSIspio::Irq_t irq ){return this->_registerIrq( irq );}
    inline int              waitIrq( NSImageioIsp3::NSIspio::Irq_t irq ){return this->_waitIrq( irq );}
...
protected:
    ...
    virtual int _abort( void* pParam );
    virtual int _waitIrq( int type, unsigned int irq , int userNumber) = 0;
    virtual int _registerIrq( NSImageioIsp3::NSIspio::Irq_t irq );
    virtual int _waitIrq( NSImageioIsp3::NSIspio::Irq_t irq );
...
public:
    ...
    MBOOL   setBandW(MINT32 fps);
    MBOOL   setIspClk(MUINT32 clk, MBOOL bAFIssue);
    MBOOL updateFrameTime(MUINT32 timeInMs, MUINT32 reqOffset);
    int waitIrqDynamicTimeout(int type, unsigned int irq, int userNumber, MUINT32 const u4TimeoutMs);
    int flushIrq(int type, unsigned int irq, int userNumber, MUINT32 const u4TimeoutMs);
...
struct CamPathPass2Parameter
{
    ...
    int tcm_en;
    int tdr_en;
    int tdr_dbg_tcm_en; // for control extenstion
...
//    int bypass_ispCdpPipe;
    ...
    NSImageioIsp3::NSIspio::EDrvScenario drvScenario;   //driver scenario
