...
static MDPMGR_STREAMSELECT_STRUCT_s mDpStreamMap[DIP_RING_BUFFER_CQ_SIZE][MAX_CMDQ_RING_BUFFER_SIZE_NUM]; //mapping table for selecting dpStream object
/**************************************************************************
 *                         G L O B A L    D A T A                         *
 **************************************************************************/
...
**************************************************************************/
template <typename T>
MINT32 _startMdp(MDPMGR_DPSTREAM_TYPE dpStreamtype,MDPMGR_CFG_STRUCT &cfgData,T* pDpStream, int p2tpipedumpEnable, int p2disableSkipTpipe)
{
    ...
    //int p2tpipedumpEnable = ::property_get_int32("camera.p2tpipedump.enable", 0);
    //int p2disableSkipTpipe = ::property_get_int32("camera.disable_p2skiptpipe", 0);
            ...
            for(MINT32 index = 0; index < ISP_OUTPORT_NUM; index++) {
                ...
                if(cfgData.dstPortCfg[index] == 1) {
                    if (index != ISP_ONLY_OUT_TPIPE ) { // for non isp only
                        ...
                        LOG_INF("dstFmt(%d,%d)",__LINE__,dstFmt);
            ...
            for(MINT32 index = 0; index < ISP_OUTPORT_NUM; index++) {
                ...
                if(cfgData.dstPortCfg[index] == 1) {
                    if (index != ISP_ONLY_OUT_TPIPE ) { // for non isp only
                        ...
                        { // for isp only mode      ISP ONLY MODE doesn't need to config src & dst, WAIT FOR DELETE!!!
    ...
    if (cfgData.isSecureFra)
    {
        ...
        // BPCI(DMGI)
        if ((cfgData.ispTpipeCfgInfo.drvinfo.dip_dmgi_secure_tag != 0) & (cfgData.ispTpipeCfgInfo.top.bnr2_en == 1))
        {
            isp2mdpcfg.secIspData.ispBufs[3].va = cfgData.ispTpipeCfgInfo.drvinfo.dip_dmgi_base_vaddr;
            isp2mdpcfg.secIspData.ispBufs[3].size = cfgData.ispTpipeCfgInfo.drvinfo.dip_dmgi_size[0];
            isp2mdpcfg.secIspData.BpciHandle = cfgData.ispTpipeCfgInfo.drvinfo.dip_dmgi_base_addr;
        }
        else
        {
        ...
        // LSCI(DEPI)
        if ((cfgData.ispTpipeCfgInfo.drvinfo.dip_depi_secure_tag != 0) & (cfgData.ispTpipeCfgInfo.top.lsc2_en == 1))
        {
            isp2mdpcfg.secIspData.ispBufs[4].va = cfgData.ispTpipeCfgInfo.drvinfo.dip_depi_base_vaddr;
            isp2mdpcfg.secIspData.ispBufs[4].size = cfgData.ispTpipeCfgInfo.drvinfo.dip_depi_size[0];
            isp2mdpcfg.secIspData.LsciHandle = cfgData.ispTpipeCfgInfo.drvinfo.dip_depi_base_addr;
        }
        else
        {
        ...
        // DEPI
        if ((cfgData.ispTpipeCfgInfo.drvinfo.dip_depi_secure_tag != 0) & (cfgData.ispTpipeCfgInfo.top.lsc2_en == 0))
        {
        ...
        // DMGI
        if ((cfgData.ispTpipeCfgInfo.drvinfo.dip_dmgi_secure_tag != 0) & (cfgData.ispTpipeCfgInfo.top.bnr2_en == 0))
        {
    ...
    if(p2disableSkipTpipe)
    {
        LOG_INF("+sceID(%u),cqPa(0x%lx)-Va(0x%lx),tpipVa(0x%lx)-Pa(0x%lx),isWaitB(%d),cqIndx(%u/%u/%u/%u),venc(%d),(va: 0x%lx),\
        ...
        HMyo CQ(%d_%d_%d),(va:0x%lx,dstpa:0x%lx)\nflag(%d_0x%x->0x%x),startStm(%d us),WDPQ(0x%8x),WRPQ(0x%8x),tv_sec(%u),tv_usec(%u)",\
                ...
                cfgData.isWaitBuf,cfgData.ispTpipeCfgInfo.drvinfo.cqIdx, cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx, cfgData.ispTpipeCfgInfo.drvinfo.burstCqIdx, cfgData.ispTpipeCfgInfo.drvinfo.RingBufIdx,\
                cfgData.dstPortCfg[ISP_MDP_DL_VENCO],(unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa,\
                ...
                cfgData.ispTpipeCfgInfo.drvinfo.tpipeTablePa, p2disableSkipTpipe, cfgData.ispTpipeCfgInfo.drvinfo.frameflag, 0, b-a, WDMAPQEnable, WROTPQEnable, cfgData.ExpectedEndTime.tv_sec, cfgData.ExpectedEndTime.tv_usec);
    }
    else
    {
        LOG_INF("+sceID(%u),cqPa(0x%lx)-Va(0x%lx),tpipVa(0x%lx)-Pa(0x%lx),isWaitB(%d),cqIndx(%u/%u/%u/%u),venc(%d),(va: 0x%lx),\
        ...
        (va:0x%lx,dstpa:0x%lx)\nflag(%d_0x%x->0x%x),startStm(%d us),WDPQ(0x%8x),WRPQ(0x%8x),tv_sec(%u),tv_usec(%u)",\
                ...
                mdpPortMapping[ISP_ONLY_OUT_TPIPE].mdpPortIdx,
                (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa, \
                cfgData.ispTpipeCfgInfo.drvinfo.tpipeTablePa, p2disableSkipTpipe, cfgData.ispTpipeCfgInfo.drvinfo.frameflag, cfgData.ispTpipeCfgInfo.drvinfo.frameflag, b-a, WDMAPQEnable, WROTPQEnable, cfgData.ExpectedEndTime.tv_sec, cfgData.ExpectedEndTime.tv_usec);
...
MINT32 MdpMgrImp::uninit()
{
    ...
    LOG_INF("before delete mInitCount(%d)",mInitCount);
...
MINT32 MdpMgrImp::startMdp(MDPMGR_CFG_STRUCT &cfgData)
{
    ...
    if(cfgData.dstPortCfg[ISP_MDP_DL_VENCO] == 1)
    {
        ...
        err=_startMdp(MDPMGR_DPSTREAM_TYPE_VENC,cfgData,pDpStream,m_p2tpipedumpEnable,m_p2disableSkipTpipe);
    }
	/*
	...
	*/
    else
    {
        ...
        err=_startMdp(MDPMGR_DPSTREAM_TYPE_ISP,cfgData,pDpStream,m_p2tpipedumpEnable,m_p2disableSkipTpipe);
    }
    return err;
}
...
DpIspStream *MdpMgrImp::selectDpIspStream(MUINT32 sceID, MUINT32 cqIdx, MUINT32 dupCqIdx, MUINT32 burstCqIdx, MUINT32 ringBufIdx)
{
    ...
    if(m_pDpStream[ObjectCqIdx][ObjectRingBufIdx]){
        ...
        LOG_INF("[selectDpIspStream] cqIdx(%d/%d/%d/%d_%d/%d),sceID(%d)",cqIdx,dupCqIdx, burstCqIdx, ringBufIdx, ObjectCqIdx, ObjectRingBufIdx,sceID);
    } else {
...
{
    bool ret=MTRUE;
    E_ISP_DIP_CQ refObjectCqIdx = static_cast<E_ISP_DIP_CQ>(0x0); //for selecting dpStream
    MUINT32 refObjectRingBufIdx = 0x0; //for selecting dpStream
    ...
    if (cfgData.ispTpipeCfgInfo.top.wpe_mdpcrop_en == 1 && cfgData.ispTpipeCfgInfo.top.wpe_en == 1) // WPE ONLY FRAME MODE sould Skip src & dst settings
    {
    	...
    	for(MINT32 index = 0; index < ISP_OUTPORT_NUM; index++) {
        	...
        	if(cfgData.dstPortCfg[index] == 1) {
             ...
             { // for isp only mode      ISP ONLY MODE doesn't need to config src & dst after mt6797, WAIT FOR DELETE!!!  
