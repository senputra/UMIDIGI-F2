                    ...
                    for (i = enqBufIdx; i < (enqBufIdx + _this->mBurstQNum); i++, _this->mMagicNum++) {
                        ...
                        if (_this->m_enablePort & __RRZO_ENABLE) {
                            ...
                            MSize _rrz;
                             ...
                             _this->m_crop.GetScaleInfo(PORT_RRZO,(EImageFormat)pIBuf->getImgFormat(),_rrz);
...
void TuningNotifyImp_Dump::dumpRegister()
{
    ...
    for(MUINT32 i=0x0;i<this->mSize;i+=0x20) {
        ...
        for(MUINT32 j=i;j<(i+0x20);j+=0x4){
            ...
            strncat(_chr, _tmpchr, (256 - strlen(_tmpchr)) - 1);
        }
        MY_LOGD("%s",_chr);
    }
    MY_LOGD("-");
...
MVOID* camsv_loop(void* arg)
{
    ...
    for (mnum = 0; MFALSE == _this->m_bStop; mnum++) {
        ...
        if (_this->mpCamsvSttPipe->deque(_dq_portID, _dq_rQBufInfo) == MFALSE) {
            MY_LOGD("##############################");
        ...
        if (_dq_rQBufInfo.mvOut.size()) {
            _this->mCamsvBufVa = _dq_rQBufInfo.mvOut.at(0).mVa;
            _this->mCamsvBufSize = _dq_rQBufInfo.mvOut.at(0).mSize;
        ...
        if (_this->mpCamsvSttPipe->enque(_eq_rQBufInfo) == MFALSE) {
            MY_LOGD("##############################");
...
, routineCamsv(camsv_loop)
{
    ...
    for (sttDma = 0; sttDma < __MAXDMAO_STT; sttDma++) {
        routineSttProc[sttDma] = sttProcRoutine[sttDma];
    }
...
#define TSF_ST_EN   1
            ...
            TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PS_AWB_PIXEL_CNT1, awb_pix_cnt, i);
    ...
    //PDO
        if(this->mEnableSttPort & __PDO_ENABLE){
            ...
            if (!this->m_Dualpd.Bits.bDual) {
                MY_LOGD("native PD, BNR, memID(%x)", m_bpciBuf.memID);
                ...
                MY_LOGD("native PD, PDE, memID(%x)", m_bpciBuf.memID);
                ...
                //pde
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PDE_TBLI1,0,i);

                //pdi
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PDI_BASE_ADDR,m_bpciBuf.phyAddr,i);
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PDI_XSIZE,sizeof(bpci_array_s5k2x8),i);
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PDI_YSIZE,1,i);
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PDI_STRIDE,sizeof(bpci_array_s5k2x8),i);
                ...
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PDO_XSIZE,xsize,i);
...
#if 1
                        {
                            ...
                            for(MUINT32 i=0; i<(sizeof(bpci_array_s5k2x8)); )
                            {
                                //
                ...
                if(this->m_Dualpd.Bits.Density == MTRUE){
                    ...
                    xsize = (this->mTgSize.w>>4)<<1;//output 2 pixs per 16 pix input
                    xsize *= 2;// 1 paire of L/R per bayer pix
                    xsize *= 2;// 2 byte per pix
...
#define flk_WIN_V   2
        if (this->mEnableSttPort & __FLKO_ENABLE) {
            ...
            TUNING_MGR_WRITE_REG_UNI(tuningMgr, CAM_UNI_FLK_B_OFST, 0, i);
            //win num
            ...
            TUNING_MGR_WRITE_REG_UNI(tuningMgr, CAM_UNI_FLK_B_NUM, ((flk_WIN_V<<4)|FLK_WIN_H), i);
            //win size
            ...
            TUNING_MGR_WRITE_REG_UNI(tuningMgr, CAM_UNI_FLK_B_SIZE, ((v_size<<16)|h_size), i);
            //xsize
            ...
            TUNING_MGR_WRITE_REG_UNI(tuningMgr, CAM_UNI_SGG3_B_PGN, 0x200, i);
            TUNING_MGR_WRITE_REG_UNI(tuningMgr, CAM_UNI_SGG3_B_GMRC_1, 0x10080402, i);
            TUNING_MGR_WRITE_REG_UNI(tuningMgr, CAM_UNI_SGG3_B_GMRC_2, 0x804020, i);
