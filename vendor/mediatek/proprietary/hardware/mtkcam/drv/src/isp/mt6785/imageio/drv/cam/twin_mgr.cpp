...
MBOOL TwinMgr_IMP::initTwin(DupCmdQMgr* pMainCmdQMgr,const NSImageio::NSIspio::PortID* pInput,MUINT32 subSample,list<MUINT32>* pChannle)
{
    ...
    for (MUINT32 i = 0; i< twinCombinations.size(); i++) {
        for (MUINT32 j = 0; j < twinCombinations.at(i).size(); j++) {
            ...
            if(SecMgr::SecMgr_GetSecureCamStatus(pMainCmdQMgr->CmdQMgr_GetCurModule()) > 0){
...
MINT32 TwinMgr_IMP::dropEnqueueBuf(const TWIN_MGR_PATH_CFG_IN_PARAM& cfgInParam, list<MUINT32>* pLPortNum, list<MUINT32>* pLMagicNum){
    MINT32 ret = 0;

    if (this->getIsTwin(MTRUE)) {
        ret = this->dropTwin(cfgInParam, pLPortNum, pLMagicNum);
    } else {
        LOG_INF("CamPathPass1::dropEnqueueBuf for single ISP\n");
        ret = ((CamPathPass1 *)this)->dropEnqueueBuf(pLPortNum, pLMagicNum);
    }
...
MINT32 TwinMgr_IMP::dropTwin(const TWIN_MGR_PATH_CFG_IN_PARAM& cfgInParam, list<MUINT32>* pLPortNum, list<MUINT32>* pLMagicNum) {
    MINT32 ret = 0;
    MUINT32 _size;
    MUINT32 burstQ = 0;
    list<MUINT32>::iterator it;
    list<MUINT32> listPortNum;
    ...
    for(it = pLPortNum->begin(); it!= pLPortNum->end(); it++) {
        switch (*it){
            case _imgo_:
            case _rrzo_:
                LOG_INF("remove multiChannel port:%d\n", *it);
                break;
            default:
                listPortNum.push_back(*it);
                break;
        }
    }
    ret = ((CamPathPass1 *)this)->dropEnqueueBuf(&listPortNum, pLMagicNum);
    LOG_INF(": - ret:%d\n", ret);
    return ret;
}
...
MBOOL TwinMgr_IMP::updateSuspendDone(void) {
    ...
    if (twin.Bits.TWIN_EN == MTRUE) {
        ...
        for (MUINT32 i = 0; i < this->m_twinVHWModules.size(); i++) {
            ...
            CAM_BUF_CTRL::updateState(CAM_BUF_CTRL::eCamState_suspend, this->m_twinVHWModules.at(i), CAM_BUF_CTRL::eCamDmaType_main);

            dbg_str += MODULE_ID_TO_STR(this->m_twinVHWModules.at(i));
        }
        LOG_INF("suspend done twin module %s\n", dbg_str.c_str());
    }

    ret = ((CamPathPass1*)this)->updateSuspendDone();
    if (ret == MFALSE) {
        LOG_ERR("suspend done flush fail\n");
        return ret;
    }
    return ret;
}
    ...
    switch(cmd){
        ...
        case TWIN_CMD_GET_CUR_TWINMODULE:
            ...
            *(list<ISP_HW_MODULE>*)arg1 = this->getCurModule();
            break;
        case TWIN_CMD_UPDATE_SUSPEND_DONE:
            this->updateSuspendDone();
            break;
        default:
            ret = MFALSE;
            break;
