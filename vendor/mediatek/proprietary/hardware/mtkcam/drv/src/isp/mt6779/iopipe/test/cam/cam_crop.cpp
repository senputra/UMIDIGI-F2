...
MBOOL CROP_TEST::PatchPipeLineIn(MSize size)
{
    ...
    if(this->mFunc == CROP_SPECIFIC){
        ...
        if(this->SetInfo(PORT_IMGO,this->fmt_imgo,this->m_Crop_imgo.s) == MFALSE)
            return MFALSE;
        if(this->SetInfo(PORT_CAMSV_IMGO,this->fmt_sv_imgo,this->m_Crop_sv_imgo.s) == MFALSE)
            return MFALSE;
    }
...
MBOOL CROP_TEST::Generator_SV_IMGO(EImageFormat ifmt,MRect& oStart)
{
    #define CropRatio 8/10
    ...
    switch(this->mFunc){
        case CROP_NONE:
            ...
            //
            input.width = _size.w;
            getNormalPipeModule()->query(PORT_CAMSV_IMGO.index, ENPipeQueryCmd_X_PIX, ifmt, input, qry);
            oStart.s.w = qry.x_pix;
            oStart.s.h = _size.h;
        ...
        case CROP_SYSMATRIC:
            ...
            input.width = oStart.p.x;
            getNormalPipeModule()->query(PORT_CAMSV_IMGO.index, ENPipeQueryCmd_CROP_START_X, ifmt, input, qry);
            oStart.p.x = qry.crop_x;
            //
            _size = _input;
            _size.w /= 2;
            ...
            //
            input.width = _size.w;
            getNormalPipeModule()->query(PORT_CAMSV_IMGO.index, ENPipeQueryCmd_CROP_X_PIX, ifmt, input, qry);
            oStart.s.w = qry.x_pix;
            oStart.s.h = _size.h;
...
MBOOL CROP_TEST::GetCropInfo(PortID iport,EImageFormat ifmt,MRect& oStart,MBOOL bPure)
{
    switch(iport.index){
        ...
        case NSImageio::NSIspio::EPortIndex_CAMSV_IMGO:
            this->Generator_SV_IMGO(ifmt,oStart);
            this->m_status_sv_imgo = _e_crop;
            this->m_Crop_sv_imgo = oStart;
            this->fmt_sv_imgo = ifmt;
            break;
...
MBOOL CROP_TEST::GetScaleInfo(PortID iport,EImageFormat ifmt,MSize& oSize,MUINT32 type,MUINT32 factor)
{
    switch(iport.index){
        case NSImageio::NSIspio::EPortIndex_IMGO:
            ...
            break;
        case NSImageio::NSIspio::EPortIndex_CAMSV_IMGO:
            if(this->m_status_sv_imgo == _e_crop){
                //no scaler
                oSize = this->m_Crop_sv_imgo.s;
            }
            else{
                LOG_ERR("flow err, need to crop 1st before sacle\n");
                return MFALSE;
            ...
            this->m_status_sv_imgo = _e_scale;
            break;
        case NSImageio::NSIspio::EPortIndex_RRZO:
...
MBOOL CROP_TEST::SetInfo(PortID iport,EImageFormat ifmt,MSize iSize)
{
    ...
    switch(iport.index){
        case NSImageio::NSIspio::EPortIndex_IMGO:
            ...
            break;
        case NSImageio::NSIspio::EPortIndex_CAMSV_IMGO:
            this->m_status_sv_imgo = _e_set;

            this->m_Crop_sv_imgo.p.x = this->m_Crop_sv_imgo.p.y = 0;
            this->m_Crop_sv_imgo.s = this->mInput;

            LOG_INF("sv_imgo:%d_%d_%d_%d(%d)\n",this->m_Crop_sv_imgo.p.x,this->m_Crop_sv_imgo.p.y,this->m_Crop_sv_imgo.s.w,this->m_Crop_sv_imgo.s.h,this->m_status_sv_imgo);
            break;
        case NSImageio::NSIspio::EPortIndex_RRZO:
