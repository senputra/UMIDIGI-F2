...
init(const char* userName, MUINT32 secTag)
{
    ...
    /*set init secure m4u after memory creation*/
    if ((MINT32)secTag != 0) {
        ret = m_pIMemDrv->initSecureM4U();
        if (ret == -1) {
            ret = MFALSE;
            goto EXIT;
        }
...
int DIP_ISP_PIPE::ConfigDipPipe(DipWorkingBuffer* pDipWBuf)
{
    ...
    DIP_FUNC_INF("moduleIdx(%d), pDipWBuf(0x%lx), m_pIspDescript_phy(0x%lx), m_pIspDescript_vir(0x%lx), m_pIspVirRegAddr_pa(0x%lx), m_pIspVirRegAddr_va(0x%lx)",
        this->moduleIdx, (unsigned long)pDipWBuf, (unsigned long)pDipWBuf->m_pIspDescript_phy, (unsigned long)pDipWBuf->m_pIspDescript_vir, (unsigned long)pDipWBuf->m_pIspVirRegAddr_pa, (unsigned long)pDipWBuf->m_pIspVirRegAddr_va);
...
int DIP_ISP_PIPE::enable(void* pParam)
{
    ...
    DIP_FUNC_DBG("[_enable]isDipOnly(%d), hwm(%d_%d_%d_%d_%d_%lx_desphy:0x%lx,desvir:0x%lx,virispphy:0x%lx,virispvir:0x%lx)",cqcfginfo.isDipOnly, cqcfginfo.moduleIdx, cqcfginfo.moduleIdx,\
        cqcfginfo.dipTh,cqcfginfo.dipCQ_dupIdx,cqcfginfo.dipCQ_burstIdx, (unsigned long)cqcfginfo.pDipWBuf, (unsigned long)cqcfginfo.pDipWBuf->m_pIspDescript_phy, (unsigned long)cqcfginfo.pDipWBuf->m_pIspDescript_vir,
        (unsigned long)cqcfginfo.pDipWBuf->m_pIspVirRegAddr_pa, (unsigned long)cqcfginfo.pDipWBuf->m_pIspVirRegAddr_va);
