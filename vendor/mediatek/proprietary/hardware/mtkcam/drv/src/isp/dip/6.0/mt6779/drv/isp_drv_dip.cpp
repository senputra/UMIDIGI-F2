...
MBOOL VirDipDrvImp::readRegs(ISP_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller)
{
    if ((m_pIspVirRegAddr_va == NULL) || (pRegIo == NULL)){
        LOG_ERR("[0x%x]. Virtual ISP Addr:%p or pRegIo:%p is NULL!! Count:0x%x, caller:0x%x",this->m_hwModule, m_pIspVirRegAddr_va, pRegIo, Count, caller);
        return MFALSE;
    }  
    //
...
MUINT32 VirDipDrvImp::readReg(MUINT32 Addr,MINT32 caller)
{
    if (m_pIspVirRegAddr_va == NULL){
        LOG_ERR("[0x%x]. Virtual ISP Addr:%p is NULL!! Addr:0x%x, caller:0x%x",this->m_hwModule, m_pIspVirRegAddr_va, Addr, caller);
        return MFALSE;
    }
    //
    android::Mutex::Autolock lock(this->IspRegMutex);
...
MBOOL VirDipDrvImp::writeRegs(ISP_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller)
{
    ...
    for(unsigned int i=0; i<Count; i++){
        ...
        LOG_DBG("this->m_pIspVirRegAddr_va:%p, %d:addr:0x%x,data:0x%x",this->m_pIspVirRegAddr_va, i,pRegIo[i].Addr,this->m_pIspVirRegAddr_va[pRegIo[i].Addr>>2]);
    }
    return MTRUE;
}
...
MBOOL VirDipDrvImp::writeRegs(MUINT32 OffsetAddr,MUINT32 Count,MUINT32* SrcAddr, MINT32 caller)
{
    ...
    if ((m_pIspVirRegAddr_va == NULL) || (SrcAddr == NULL)){
        LOG_ERR("[0x%x]. Virtual ISP Addr:%p or SrcAddr:%p is NULL!! Count:0x%x, caller:0x%x",this->m_hwModule, m_pIspVirRegAddr_va, SrcAddr, Count, caller);
        return MFALSE;
    } 
    //
    android::Mutex::Autolock lock(this->IspRegMutex);
...
{
    if (m_pIspVirRegAddr_va == NULL){
        LOG_ERR("[0x%x]. Virtual ISP Addr:%p is NULL!! Data:0x%lx, caller:0x%x",this->m_hwModule, m_pIspVirRegAddr_va, Data, caller);
        return MFALSE;
    } 
    //
    android::Mutex::Autolock lock(this->IspRegMutex);
    ...
    LOG_DBG("m_pIspVirRegAddr_va:%p, addr:0x%x,Data:0x%lx, m_pIspVirRegAddr_va[0x%x]:0x%x",this->m_pIspVirRegAddr_va, Addr, Data, (Addr>>2), this->m_pIspVirRegAddr_va[Addr>>2]);
...
MBOOL VirDipDrvImp::cqAddModule(MUINT32 moduleId)
{
    ...
    if ((m_pIspVirRegAddr_pa == NULL) || (m_pIspDescript_vir == NULL)){
        LOG_ERR("[0x%x]. Virtual ISP Phy Addr:%p or CMDQ Addr:%p is NULL!! moduleID:0x%x",this->m_hwModule, m_pIspVirRegAddr_pa, m_pIspDescript_vir, moduleId);
        return MFALSE;
    }
    ...
    LOG_DBG("[0x%x]. this->m_pIspDescript_vir:%p, addr:0x%x cmd:0x%x, moduleID:0x%x",this->m_hwModule, this->m_pIspDescript_vir, this->m_pIspDescript_vir[moduleId].v_reg_addr, this->m_pIspDescript_vir[moduleId].u.cmd, moduleId);
...
MBOOL VirDipDrvImp::cqDelModule(MUINT32 moduleId)
{
    ...
    if (m_pIspDescript_vir == NULL){
        LOG_ERR("[0x%x]. Virtual CMDQ Addr:%p is NULL!! moduleID:0x%x",this->m_hwModule, m_pIspDescript_vir, moduleId);
        return MFALSE;
    }
...
{
    LOG_DBG("this->m_hwModule:0x%x, this->m_pIspDescript_phy:0x%lx!!",this->m_hwModule, (unsigned long)this->m_pIspDescript_phy);
    return (MUINT32*)this->m_pIspDescript_phy;
}

MUINT32* VirDipDrvImp::getCQDescBufVirAddr(void)
{
    LOG_DBG("this->m_hwModule:0x%x, this->m_pIspDescript_vir:0x%lx!!",this->m_hwModule, (unsigned long)this->m_pIspDescript_vir);
...
{
    LOG_DBG("this->m_hwModule:0x%x, this->m_pIspVirRegAddr_pa:0x%lx!!",this->m_hwModule, (unsigned long)this->m_pIspVirRegAddr_pa);
    return (MUINT32*)this->m_pIspVirRegAddr_pa;
}

MUINT32* VirDipDrvImp::getIspVirRegVirAddr(void)
{
    LOG_DBG("this->m_hwModule:0x%x, this->m_pIspVirRegAddr_va:0x%lx!!",this->m_hwModule, (unsigned long)this->m_pIspVirRegAddr_va);
...
{
    LOG_DBG("this->m_hwModule:0x%x, this->m_pIspDescript_phy:0x%lx!!",this->m_hwModule, (unsigned long)this->m_IspDescript_oft);
    return (MUINT32)this->m_IspDescript_oft;
}

MUINT32 VirDipDrvImp::getIspVirRegOft(void)
{
    LOG_DBG("this->m_hwModule:0x%x, this->m_pIspVirRegAddr_pa:0x%lx!!",this->m_hwModule, (unsigned long)this->m_IspVirRegAddr_oft);
...
{
    LOG_DBG("[0x%x]+",this->m_hwModule);
    char _str[4096] = {"\0"};
    char _tmp[16] = {"\0"};
    if (m_pIspDescript_vir == NULL){
        LOG_ERR("[0x%x]. Virtual CMDQ Addr:%p is NULL!!",this->m_hwModule, m_pIspDescript_vir);
        return MFALSE;
    }
