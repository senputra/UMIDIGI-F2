...
DupCmdQMgr* CmdQMgrImp4::CmdQMgr_attach_imp(UINT32 fps,MUINT32 subsample,ISP_HW_MODULE module,E_ISP_CAM_CQ cq,CmdQMgrImp*  pMasterCam)
{
    ...
    //
    if((this->mModule != CAM_A_TWIN) && (this->mModule != CAM_B_TWIN) ){
...
MINT32 CmdQMgrImp4::CmdQMgr_update(void)
{
    ...
    CmdQMgr_DBG("CmdQMgrImp4::CmdQMgr_update: 0x%x_0x%p",this->mDupCmdQIdx,this->pCamDrv[this->mDupCmdQIdx][0]->getCQDescBufPhyAddr());
...
typedef struct{
    ...
    CmdQMgrImp4 CQ_TYPE4;                           //support only cq0 at cam_a_twin/cam_b_twin only
}ST_CmdQ;
        ...
        case ISP_DRV_CQ_THRE0:
            switch(module){
                case CAM_A_TWIN:
                    ...
                    break;
                case CAM_B_TWIN:
                    if(gCmdQMgr[module].CQ_TYPE4.CmdQMgr_attach_imp(fps,subsample,module,cq,&gCmdQMgr[CAM_B].CQ_TYPE1)){
                        return (DupCmdQMgr*)&gCmdQMgr[module].CQ_TYPE4;
                    }
                    else
                        return NULL;
