...
MINT32 TwinMgr_BufCtrl::enque_drop(MVOID) {
    MINT32 ret = 0;
    MUINT32 _size = 0;
    list<MUINT32>::iterator it;
    ...
    for (it = this->m_OpenedChannel.begin(); it != this->m_OpenedChannel.end(); it++) {
        switch(*it) {
            case _imgo_:
                ...
                if(_size != 0)
                    this->m_enque_IMGO.sendCmd(QueueMgr<ISP_BUF_INFO_L>::eCmd_pop_back, 0, 0);
                else{
                    ret = 1;
                    LOG_ERR("IMGO have no buffer for drop\n");
                }
                break;
            case _rrzo_:
                this->m_enque_RRZO.sendCmd(QueueMgr<ISP_BUF_INFO_L>::eCmd_getsize,(MUINTPTR)&_size,0);
                if(_size != 0)
                    this->m_enque_RRZO.sendCmd(QueueMgr<ISP_BUF_INFO_L>::eCmd_pop_back, 0, 0);
                else{
                    ret = 1;
                    LOG_ERR("RRZO have no buffer for drop\n");
                }
                break;
            default:
                ...
                if(_tmp_buf.bReplace == MTRUE){
                    ...
                    if (SecMgr::SecMgr_GetSecurePortStatus(it->module,dmaChannel)) {
                        ...
                        if (mpSecMgr->SecMgr_GetSecureTwinStatus(it->module)) {
                            ...
                            pDmao->Header_Addr = secinfo.buff_sec_mva;
                            LOG_INF("[twin]DMAO:%d Sec FH Addr:0x%x->0x%x E_IMG_PA:0x%x",
                                     dmaChannel,secinfo.buff_va,pDmao->Header_Addr,pDmao->dma_cfg.memBuf.base_pAddr);
                        } else {
                            // Master, share memory has been registered in NormalPipe::configPipe()
