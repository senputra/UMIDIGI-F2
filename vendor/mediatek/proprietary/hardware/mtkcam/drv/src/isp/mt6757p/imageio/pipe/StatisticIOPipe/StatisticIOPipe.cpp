    ...
    switch(InPut){
        case TG_A:
            if(gSttioPipe[CAM_A].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gSttioPipe[CAM_A].m_szUsrName,(char const*)szUsrName,strlen((char const*)szUsrName));
        ...
        case TG_B:
            if(gSttioPipe[CAM_B].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gSttioPipe[CAM_B].m_szUsrName,(char const*)szUsrName,strlen((char const*)szUsrName));
...
MBOOL STTIOPipe::uninit()
{
    ...
    if(this->m_byPassAFO == MFALSE){
        if(this->m_byPassAAO == MTRUE){//aao, afo ring buf both use CQ4
            if(this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_detach()){
                PIPE_ERR("[%d]:CQ4 uninit fail\n",this->m_hwModule);
                return MFALSE;
            }
        ...
        if(this->m_TwinMgr->getIsTwin() == MTRUE) {
            ...
            for(it = this->m_TwinMgr->getCurModule()->begin();it!=this->m_TwinMgr->getCurModule()->end();it++){
                ...
                if(gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_detach()){
                    PIPE_ERR("Twin CQ8 uninit fail at cam:0x%x\n",*it);
                    return MFALSE;
                }
#else
                if(gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_detach()){
                    PIPE_ERR("Twin CQ4 uninit fail at cam:0x%x\n",*it);
                    return MFALSE;
                }
#endif
            }
...
MBOOL STTIOPipe::start()
{
    ...
    if(this->m_byPassAFO == MFALSE){
        if(this->m_TwinMgr->getIsTwin() == MTRUE) {
            list<ISP_HW_MODULE>::iterator it;
#if FAST_AF_TWIN_CQ_WORK_AROUND
            if(this->m_byPassFLKO == MTRUE){
                PIPE_ERR("[%d]:must enable FLKO in twin mode\n",this->m_hwModule);
                return MFALSE;
            }
            for(it = this->m_TwinMgr->getCurModule()->begin();it!=this->m_TwinMgr->getCurModule()->end();it++){
                if(gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_start()){
                    PIPE_ERR("Twin CQ8 start fail at cam:0x%x\n",*it);
                    return MFALSE;
                }
            }
#else
            for(it = this->m_TwinMgr->getCurModule()->begin();it!=this->m_TwinMgr->getCurModule()->end();it++){
                if(gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_start()){
                    PIPE_ERR("Twin CQ4 start fail at cam:0x%x\n",*it);
                    return MFALSE;
                }
            }
#endif
    ...
    if(this->m_byPassAAO == MFALSE){//change order after twin afo
        if(this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_start()){
            PIPE_ERR("[%d]:CQ4 start fail\n",this->m_hwModule);
            return MFALSE;
        }
...
MBOOL STTIOPipe::stop(MBOOL bForce, MBOOL detachUni)
{
    ...
    if(this->m_byPassAFO == MFALSE){
        ...
        this->m_AFO_FBC.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetCurCycleObj(0)[0];
        ...
        if(MTRUE == this->m_TwinMgr->getIsTwin()) {
            ...
            for(MUINT32 i = 0 ; i < this->m_TwinMgr->getCurModule()->size() ; i++, it++){
                curModule = *it;
#if FAST_AF_TWIN_CQ_WORK_AROUND
                if(gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_stop()){
                    PIPE_ERR("Twin CQ8 stop fail at cam:0x%x\n",*it);
                    this->m_TwinMgr->Twin_UnLock();
                    return MFALSE;
                }
#else
                if(gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_stop()){
                    PIPE_ERR("Twin CQ4 stop fail at cam:0x%x\n",*it);
                    this->m_TwinMgr->Twin_UnLock();
                    return MFALSE;
                }
#endif
...
#if FAST_AF_TWIN_CQ_WORK_AROUND
                gSttioPipe[curModule].m_AFO_FBC.m_pIspDrv = (IspDrvVir*)gSttioPipe[curModule].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_GetCurCycleObj(0)[0];
#else
                gSttioPipe[curModule].m_AFO_FBC.m_pIspDrv = (IspDrvVir*)gSttioPipe[curModule].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetCurCycleObj(0)[0];
#endif
        ...
        if(MTRUE == this->m_TwinMgr->getIsTwin()) {
            ...
            for(it = this->m_TwinMgr->getCurModule()->begin();it!=this->m_TwinMgr->getCurModule()->end();it++){
                if(gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE7]->CmdQMgr_stop()){
                    PIPE_ERR("Twin CQ7 stop fail at cam:0x%x\n",*it);
                    return MFALSE;
                }
            }
...
*
********************************************************************************/
MVOID STTIOPipe::rstBufCtrl(MBOOL rst)
{
    if(this->m_FSM == op_stop){
        PIPE_ERR("STTIOPipe FSM(%d)", this->m_FSM);
        return;
    ...
    if(this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]){
        this->m_AAO_FBC.clrTSBase(rst,this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetCurCycleObj(0)[0]);
        this->m_AFO_FBC.clrTSBase(rst,this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetCurCycleObj(0)[0]);
    }
    if(this->m_pCmdQMgr[ISP_DRV_CQ_THRE7])
        this->m_PDO_FBC.clrTSBase(rst,this->m_pCmdQMgr[ISP_DRV_CQ_THRE7]->CmdQMgr_GetCurCycleObj(0)[0]);
    if(this->m_pCmdQMgr[ISP_DRV_CQ_THRE12])
        this->m_PSO_FBC.clrTSBase(rst,this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetCurCycleObj(0)[0]);
}
        ...
        switch(dmaChannel){
            case _afo_:
                if(MTRUE == this->m_TwinMgr->getIsTwin()) {
                    ...
                    if(this->m_UniMgr.UniMgr_GetCurLink() != this->m_hwModule){
                        if(this->m_FSM == op_stop && this->m_UniMgr.UniMgr_GetCurLink() == CAM_MAX){
                            PIPE_DBG("warning: [%d] FSM=stop",this->m_hwModule);
                            return 1;
                        }
                        PIPE_ERR("[%d] uni is linked with cam:%d\n",this->m_hwModule,this->m_UniMgr.UniMgr_GetCurLink());
                        return -1;
...
MINT32 STTIOPipe::dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo, MUINT32 const u4TimeoutMs, CAM_STATE_NOTIFY *pNotify)
{
    ...
    CAM_BUF_CTRL* pFBC = NULL;
    BufInfo buf;
    MINT32 ret = ICamIOPipe::ST_SUCCESS, deqRet;
    E_ISP_CAM_CQ cq;
    ...
    if(this->FSM_CHECK(op_endeq) == MFALSE){
        if(this->m_FSM == op_stop)
            return ICamIOPipe::ST_STOP;
        return ICamIOPipe::ST_FAIL;
    }
    ...
    switch(dmaChannel){
        case _afo_:
            ...
            if ( MFALSE == this->m_AFO_FBC.waitBufReady(MTRUE, pNotify)) {
                ...
                if(CAM_BUF_CTRL::m_CamState[this->m_hwModule] == CAM_BUF_CTRL::eCamState_suspending){
                    return ICamIOPipe::ST_SUSPEND;
                }
        ...
        case _pdo_:
            ...
            buf.m_pPrivateSize = sizeof(this->m_Xmx);
        ...
        default:
            if(dmaChannel == _aao_){
                buf.m_pPrivate = (MUINT32*)&this->m_AAinfo;
                buf.m_pPrivateSize = sizeof(this->m_AAinfo);//ob/lsc
            }
    ...
    switch(dmaChannel){
        case _afo_:
            if(MTRUE == this->m_TwinMgr->getIsTwin()) {
                list<ISP_HW_MODULE>::iterator it3;
                if(ret == ICamIOPipe::ST_SUCCESS){
#if FAST_AF_TWIN_CQ_WORK_AROUND
                    if(this->m_UniMgr.UniMgr_GetCurAttachModule() != this->m_hwModule){
                        if(this->m_FSM == op_stop && this->m_UniMgr.UniMgr_GetCurAttachModule() == CAM_MAX){
                            PIPE_DBG("warning: [%d] uni:FSM=stop",this->m_hwModule);
                            return ICamIOPipe::ST_STOP;
                        }
                        PIPE_ERR("[%d] uni is linked with cam:%d\n",this->m_hwModule,this->m_UniMgr.UniMgr_GetCurAttachModule());
                        return ICamIOPipe::ST_FAIL;
                    }
                    cq = ISP_DRV_CQ_THRE8;
#endif
                ...
                if(ret == ICamIOPipe::ST_SUCCESS){
                    for(it3 = this->m_TwinMgr->getCurModule()->begin();it3!=this->m_TwinMgr->getCurModule()->end();it3++){
                        ...
                        for(MUINT32 i=0;i<gSttioPipe[*it3].m_pCmdQMgr[cq]->CmdQMgr_GetBurstQ();i++){
                            ...
                            if ( deqRet = gSttioPipe[*it3].m_PDO_FBC.dequeueHwBuf( buf )  < 0 ) {
                                PIPE_ERR("data is rdy on dram,but deque fail at burst:0x%x\n",i);
                                ret = ICamIOPipe::ST_FAIL;
                            }
                            else if( deqRet > 0){
...
MBOOL STTIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts,CAMIO_Func func)
{
    ...
    this->m_AFO.m_bFastAF = func.Bits.FAST_AF;
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ ) {
        ...
        PIPE_INF("statistic vOutPorts:[%d]: dma(0x%x)\n", i, vOutPorts[i]->index);
        ...
        switch(vOutPorts[i]->index){
            ...
            case EPortIndex_AFO:
                idx_afo = i;
                cq = ISP_DRV_CQ_THRE4;//cq5 for CqTuningMgr (AF tuning)
                this->m_byPassAFO = MFALSE;
                break;
            case EPortIndex_FLKO:
                this->m_UniMgr.m_module = m_UniMgr.UniMgr_getUniModule(this->m_hwModule);
        ...
        //aao & afo both use CQ4, no needs to attach twice
        if (cq == ISP_DRV_CQ_THRE4 && idx_aao >= 0 && idx_afo >= 0) {
            PIPE_DBG("dma_0x%x, CQ_0x%x already attached\n",vOutPorts[i]->index,cq);
            continue;
        }
    ...
    if(MTRUE == this->m_TwinMgr->getIsTwin()) {
        ...
        for(it = this->m_TwinMgr->getCurModule()->begin();it!=this->m_TwinMgr->getCurModule()->end();it++){
            ...
            // If AFO is enable, attach CmdQ thread for twin use
            if(MFALSE == this->m_byPassAFO) {
#if FAST_AF_TWIN_CQ_WORK_AROUND
                if(MTRUE == this->m_byPassFLKO){
                    PIPE_ERR("[%d] must enable FLKO in twin mode\n",this->m_hwModule);
                    return MFALSE;
                }
                cq = ISP_DRV_CQ_THRE8;
    ...
    if(this->m_byPassAFO == MFALSE){
        for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetDuqQ();j++){
            for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetBurstQ();i++){
                this->m_AFO.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetCurCycleObj(j)[i];
                this->m_AFO.Init();
            }
        }
        for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetDuqQ();j++){
        ...
        if(MTRUE == this->m_TwinMgr->getIsTwin()) {
            ...
            for(it = this->m_TwinMgr->getCurModule()->begin();it!=this->m_TwinMgr->getCurModule()->end();it++){
#if FAST_AF_TWIN_CQ_WORK_AROUND
                for(MUINT32 j=0;j<gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_GetDuqQ();j++){
                    for(MUINT32 i=0;i<gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_GetBurstQ();i++){
                        gSttioPipe[*it].m_AFO.m_pIspDrv = (IspDrvVir*)gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_GetCurCycleObj(j)[i];
                        gSttioPipe[*it].m_AFO.Init();
                    }
                }
#else
                for(MUINT32 j=0;j<gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetDuqQ();j++){
                    for(MUINT32 i=0;i<gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetBurstQ();i++){
                        gSttioPipe[*it].m_AFO.m_pIspDrv = (IspDrvVir*)gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetCurCycleObj(j)[i];
                        gSttioPipe[*it].m_AFO.Init();
                    }
                }
#endif
...
#if FAST_AF_TWIN_CQ_WORK_AROUND
                gSttioPipe[*it].m_AFO_FBC.m_pIspDrv = (IspDrvVir*)gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE8]->CmdQMgr_GetCurCycleObj(0)[0];
#else
                gSttioPipe[*it].m_AFO_FBC.m_pIspDrv = (IspDrvVir*)gSttioPipe[*it].m_pCmdQMgr[ISP_DRV_CQ_THRE4]->CmdQMgr_GetCurCycleObj(0)[0];
#endif
