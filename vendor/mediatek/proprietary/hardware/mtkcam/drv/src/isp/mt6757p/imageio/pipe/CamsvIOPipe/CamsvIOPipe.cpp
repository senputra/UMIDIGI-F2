    ...
    switch(InPut){
        ...
        case TG_CAMSV_0:
            if(gCamsvioPipe[CAMSV_0-CAMSV_START].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gCamsvioPipe[CAMSV_0-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(szUsrName));
        ...
        case TG_CAMSV_1:
            if(gCamsvioPipe[CAMSV_1-CAMSV_START].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gCamsvioPipe[CAMSV_1-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(szUsrName));
        ...
        case TG_CAMSV_2:
            if(gCamsvioPipe[CAMSV_2-CAMSV_START].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gCamsvioPipe[CAMSV_2-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(szUsrName));
        ...
        case TG_CAMSV_3:
            if(gCamsvioPipe[CAMSV_3-CAMSV_START].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gCamsvioPipe[CAMSV_3-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(szUsrName));
        ...
        case TG_CAMSV_4:
            if(gCamsvioPipe[CAMSV_4-CAMSV_START].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gCamsvioPipe[CAMSV_4-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(szUsrName));
        ...
        case TG_CAMSV_5:
            if(gCamsvioPipe[CAMSV_5-CAMSV_START].m_occupied == MTRUE){
                ...
                std::strncpy((char*)gCamsvioPipe[CAMSV_5-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(szUsrName));
...
MBOOL CamsvIOPipe::enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo,MBOOL bImdMode)
{
    ...
    if(rQBufInfo.vBufInfo.size() != 1){
        PIPE_ERR("camsviopipe support only enque-1-frame-at-1-time\n");
        return MFALSE;
    }
    ...
    for(MUINT32 i=0; i<rQBufInfo.vBufInfo.size(); i++){
        ...
        PIPE_INF("burstidx:%d: PortID=%d, bufInfo:(MEMID:%d),(VA:0x%08x),(PA:0x%08x),(FH_VA:0x%08x)",\
            i,\
            portID.index,\
...
*
********************************************************************************/
MBOOL CamsvIOPipe::dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQTBufInfo, MUINT32 const u4TimeoutMs, CAM_STATE_NOTIFY *pNotify)
{
    MBOOL ret = MTRUE;
    MUINT32 dmaChannel = 0;
    BufInfo buf;
    ...
    //wait P1_done
    if ( MFALSE == this->m_FbcImgo.waitBufReady()) {
        PIPE_ERR("waitBufReady fail");
        return MFALSE;
    }

    if ( MFALSE == this->m_FbcImgo.dequeueHwBuf(buf) ) {
    ...
    PIPE_INF("PortID=%d, bufInfo:(MEMID:%d),(VA:0x%08x),(PA:0x%08x),(FH_VA:0x%08x)",\
        portID.index, rQTBufInfo.vBufInfo[i].memID[0],\
        rQTBufInfo.vBufInfo[i].u4BufVA[0],\
        rQTBufInfo.vBufInfo[i].u4BufPA[0],\
        rQTBufInfo.vBufInfo[i].Frame_Header.u4BufVA);
...
MBOOL CamsvIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts,CAMIO_Func func)
{
    ...
    this->m_TGCtrl.m_TimeClk = vInPorts[idx_in_src]->tTimeClk;
...
MBOOL CamsvIOPipe::configDmaPort(PortInfo const* portInfo,IspDMACfg &a_dma,MUINT32 pixel_Byte,MUINT32 swap, MUINT32 isBypassOffset,EIMAGE_STRIDE planeNum)
{
    ...
    PIPE_INF("w(%d),h(%d),stride(%d),xsize(%d),crop(%d,%d,%d,%d)_f(0x%x,0x%x),ofst_addr(0x%x),pb((0x%x)(+<<2)),va(0x%x),pa(0x%x),pixel_Byte(%d)",a_dma.size.w,a_dma.size.h,a_dma.size.stride, \
