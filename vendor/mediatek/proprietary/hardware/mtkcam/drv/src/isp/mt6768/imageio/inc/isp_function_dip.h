...
#define CROP_TPIPE_PECISE_BIT   15    // YW revise
...
{
    protected:
        IspDrvShell() : m_p2disableSkipTpipe(0) {}
        virtual             ~IspDrvShell() {m_p2disableSkipTpipe = 0;};
    public:
...
public:
    DIP_ISP_PIPE():irqUserKey(0),m_isp_function_count(0),pDrvDipPhy(NULL),pDrvDip(NULL),dip_cq_thr_ctl(0),tdr_ctl(0),tdr_tpipe(0),
        tdr_tcm_en(0),tdr_tcm2_en(0),tdr_tcm3_en(0),pRegiAddr(0x0),regCount(0),pReadAddrList(NULL),m_pDipWBuf(NULL){
    ...
    int PushFrameCQforMDPStart( void );
...
public:
    DIP_MDP_PIPE():mdp_imgxo_p2_en(0),yuv2_en(0),lastframe(MFALSE),lastframe_dupCqIdx(0),lastframe_burstQueIdx(0),lastframe_dipTh(ISP_DRV_DIP_CQ_THRE0),
        ...
        p2MdpSrcVSize(0),p2MdpSrcPlaneNum(0),WDMAPQParam(NULL),WROTPQParam(NULL),framemode_total_in_w(0),framemode_total_out_w(0), framemode_h(0), total_data_size(0){};
...
public:
    DIP_BUF_CTRL():m_pMdpMgrCfgData(NULL),m_BufdrvScenario(-1),m_BufdipTh(ISP_DRV_DIP_CQ_THRE0),m_BufdupCqIdx(0),m_BufburstQueIdx(0),m_BufmoduleIdx(0)
        {};
