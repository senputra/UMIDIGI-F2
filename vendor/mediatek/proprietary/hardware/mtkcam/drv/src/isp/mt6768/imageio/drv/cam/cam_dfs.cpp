...
MINT32                  CAM_DFS_Mgr::m_curClkLevel = 0; //always use the lowest clk level at first.
...
CAM_DFS_Mgr::~CAM_DFS_Mgr()
{
    ...
    for(MUINT32 i=0; i < CAM_MAX; i++) {
        if (m_moduleTargetLevel[i] != -1)
            ...
            m_curClkLevel = 0; //reset to lowest clk
    }
}
...
MBOOL CAM_DFS_Mgr::CAM_DFS_INIT(vector<MUINT32>* clk_gear)
{
    ...
    if(m_SupportedClk.size() == 0) {
        ...
        CAM_INF("supportedClk.clklevelcnt:%d", supportedClk.clklevelcnt);
        if (supportedClk.clklevelcnt == 0) {
            CAM_INF("Use fake clk for EP!!");
            supportedClk.clklevelcnt = 1;
            supportedClk.clklevel[0] = 546;
        }
...
#if 0 //mark for always using the lowest clk level at first
    if(m_curClkLevel < 0) {
        if(this->m_pDrv->getDeviceInfo(_GET_CUR_ISP_CLOCK, (MUINT8*)&m_curClkLevel) == MFALSE)
            CAM_ERR("Error in _GET_CUR_ISP_CLOCK");

        CAM_DBG("current isp clk lv:%dMhz\n", m_curClkLevel);

        for(MUINT32 i=0; i < m_SupportedClk.size(); i++) {
            if((MUINT32)m_curClkLevel*MHZ == m_SupportedClk.at(i)) {
                m_curClkLevel = i;//record which level in the vector
                break;
            }
        }
    }
...
MBOOL CAM_DFS_Mgr::CAM_DFS_SetLv(MINT32 level,MBOOL immediate)
{
    ...
    if((this->m_Switch_Latnecy == SET_LOW_LATENCY) || ALWAYS_SET || m_DFSLvIsSet == MFALSE) {
...
MBOOL CAM_DFS_Mgr::CAM_DFS_setLv(void)
{
    ...
    if ((this->m_TargetLv != (MUINT32)m_curClkLevel) || ALWAYS_SET || m_DFSLvIsSet == MFALSE) {
        if(dfsdisable) {
            ...
            if(m_SupportedClk.at(this->m_TargetLv)/MHZ > 0){
                ...
                if (m_DFSLvIsSet == MFALSE) {
                    CAM_INF("isp clk lv init to %dMhz\n", setClk);
                    m_DFSLvIsSet = MTRUE;
                }
            } else {
