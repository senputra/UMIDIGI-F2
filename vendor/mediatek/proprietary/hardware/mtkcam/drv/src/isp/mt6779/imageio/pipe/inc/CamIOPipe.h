...
public:
            explicit   CamIO_PathCtrl(CamIOPipe *pCamIOPipe)
                            :m_hwModule(PHY_CAM),m_ThreeRawStage(NoThreeRaw),m_WandTStage(NoWandT),
                             m_CAMIO_CB(NULL),m_CAMIO_SWITCH_CB(NULL),m_IQlv(eCamIQ_MAX),
...
public:
            MBOOL       TwinPath(Res_Meter_OUT &resCfgOutParam, MBOOL bWT_en=MFALSE);
            MBOOL       SinglePath(Res_Meter_OUT &resCfgOutParam, MBOOL bWT_en=MFALSE);
            MBOOL           WandTSwitchPath(MUINT32 op, ST_CAMIO_WT_INFO *wt_info);
            MBOOL           SetWandTStage(E_W_AND_T_STAGE stage);
            E_W_AND_T_STAGE GetWandTStage();
...
public:
    ...
    E_W_AND_T_STAGE             m_WandTStage; //W and T
    mutable Mutex               m_ThreeRawFSMLock;
    mutable Mutex               m_WandTFSMLock;
    ICamIOPipe::CAMIO_CB        m_CAMIO_CB;
    ICamIOPipe::CAMIO_SWITCH_CB m_CAMIO_SWITCH_CB;
    E_CamIQLevel                m_IQlv;
...
class CamIOPipe_BW : public P1_TUNING_NOTIFY
{
public:
                    CamIOPipe_BW(){_this = NULL;}
                    CamIOPipe_BW(MVOID* pCamIO);
public:
    virtual const char* TuningName(void){return "BW-resursive";}
    virtual void    p1TuningNotify(MVOID *pInput,MVOID *pOutput);
...
public:     ////    Instantiation.
    ...
    virtual MINT32   enqueInBuf(PortID const portID, QBufInfo const& rQBufInfo);
    virtual E_BUF_STATUS dequeInBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo, MUINT32 const u4TimeoutMs = 0xFFFFFFFF, CAM_STATE_NOTIFY *pNotify = NULL);
    //
    ...
    virtual MBOOL   configFrame(E_FRM_EVENT event,CAMIO_Func func,ST_CAMIO_WT_INFO *wt_info = NULL);
    ...
    virtual MBOOL   ReadReg(MINTPTR pRegs, MUINT32 RegCount, MUINT32 getRaw, MBOOL bCurPhy = MTRUE);
//
private:
            ...
            MINT32          SrcFmtToHwVal_DMA( ImgInfo::EImgFmt_t imgFmt,MUINT32* pPixel_byte,MBOOL* bUFDataFmt);
...
private:
    ...
    //for RAWI due to no header
    list<QBufInfo>          m_vRawiBufinfo;
};
