...
{
    DIP_RINGBUF_DBG("singleton[0x%08lx].", (unsigned long)&singleton[module]);
...
DipWorkingBuffer* DipRingBufferImp::getDipWorkingBufferbyIdx(E_ISP_DIP_CQ CmdqId, unsigned int BufIdx)
{
...
DipWorkingBuffer* DipRingBufferImp::AcquireDipWorkingBuffer(E_ISP_DIP_CQ CmdqId)
{
        ...
        (DipWBufWRIdx < mDipCmdQRBList[CmdqId].mDipWBList.size()))
    {
        if (mDipCmdQRBList[CmdqId].mDipWBList.at(DipWBufWRIdx)->m_WBStatus == DIP_WB_FREE)
        {
            ...
            this->BufferMutexUnLock();
            DIP_RINGBUF_DBG("CmdqId(%d), mDipCmdQRBList[CmdqId].wIdx(%d), mDipCmdQRBList[CmdqId].iWBSize(%d), pDipWBuf(0x%lx)", CmdqId, mDipCmdQRBList[CmdqId].wIdx, mDipCmdQRBList[CmdqId].iWBSize, (unsigned long)pDipWBuf);
        }
        else    //State Machine Error!!
        {
            pDipWBuf = NULL;
...
bool DipRingBufferImp::ReleaseDipWorkingBuffer(DipWorkingBuffer* pDipWBuf, E_ISP_DIP_CQ CmdqId)
{
        ...
        (DipWBufRdIdx < mDipCmdQRBList[CmdqId].mDipWBList.size()))
    {
        ...
        {
            this->BufferMutexUnLock();
            DIP_RINGBUF_ERR("State Machine of command queue(0x%x) have error, DipWBufRdIdx(0x%x), pDipWBuf(0x%lx)!!",CmdqId, DipWBufRdIdx, (unsigned long)pDipWBuf);
            return false;
        }
    }
...
//Solutin4:
            ...
            else
            {
                ...
                DIP_RINGBUF_INF("mem cpy from  previous tpipe buf(0x%lx), current tpipe buf(0x%lx), cpysize(%d) debug_cpysize(%d!!",(unsigned long)pPrevDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, (unsigned long)pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, cpysize, tmpCpySize);
            }
