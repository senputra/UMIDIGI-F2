...
ISP_DRV_CQ_MODULE_INFO_STRUCT mIspCQModuleInfo[CAM_CAM_MODULE_MAX]    =
{
    ...
    {CAM_ISP_ADBS_,                0x4D50, 0x0D50,  21  }, // CAM_ADBS_CTL
    {CAM_ISP_DCPN_,                0x4DB0, 0x0DB0,  8   }, // CAM_DCPN_HDR_EN
    {CAM_ISP_CPN_,                 0x4DF0, 0x0DF0,  16   }, // CAM_CPN_HDR_CTL_EN
...
MBOOL ISP_DRV_CAM::init(const char* userName)
{
    ...
    this->m_useNameList.push_back(userName);

    if(this->m_UserCnt > 0) {
        tmp = android_atomic_inc(&this->m_UserCnt);
        LOG_INF(" - X. m_UserCnt: %d.", this->m_UserCnt);
        users = std::count(this->m_useNameList.begin(),
            this->m_useNameList.end(), userName);
        //if same username, only the first one need init ispDrv
        if (users == 1) {
...
MBOOL ISP_DRV_CAM::uninit(const char* userName)
{
    ...
    if(this->m_UserCnt <= 0)
    {
        LOG_ERR("no more user in isp_drv_ca , curUser(%s)",userName);
        return MFALSE;
    }

    // remove userName
    auto itr = std::find(this->m_useNameList.begin(),
                         this->m_useNameList.end(),
                         userName);
    if (itr != m_useNameList.end()) {
        m_useNameList.erase(itr);
    } else {
        LOG_ERR("No matching useName:%s\n",userName);
        return MFALSE;
    }
        ...
        case _SET_PM_QOS_RESET:
            {
                ...
                dbg[0] = 0;
                if(ioctl(this->m_pIspDrvImp->m_Fd, ISP_RESET_PM_QOS, (unsigned int*)dbg) < 0){
