...
DupCmdQMgr* CmdQMgrImp::CmdQMgr_attach_imp(UINT32 fps,MUINT32 subsample,ISP_HW_MODULE module,E_ISP_CAM_CQ cq)
{
    ...
    for(MUINT32 j=0;j<this->mDupCmdQ;j++){
        ...
        if (this->pCamDrv[j] == NULL || this->pDummyDrv[j] == NULL ||
            this->m_pUserName[j] == NULL || this->m_pDummyName[j] == NULL) {
            CmdQMgr_ERR("malloc error\n");
            return NULL;
        }
        for(MUINT32 i=0;i<this->mBurstQ;i++){
            this->m_pUserName[j][i] = (char*)malloc(sizeof(char)*STR_LENG);
            if (this->m_pUserName[j][i] == NULL) {
                CmdQMgr_ERR("malloc error\n");
                return NULL;
            }
    ...
    for (MUINT32 j = 0; j < this->mDupCmdQ; j++) {
        ...
        if (m_CQ0.m_bSubsample) { //subsample function enabled
            ...
            //only 1st burst page will set db_hold = 0
            this->m_CQ0.m_pIspDrv = (IspDrvVir*)this->pCamDrv[j][0];
            this->m_CQ0.setDBLoadHold(MFALSE, MFALSE);
            this->m_CQ0.updateDB_byCQ(MTRUE);
            for (MUINT32 i = 1; i < this->mBurstQ; i++) {
                this->m_CQ0.m_pIspDrv = (IspDrvVir*)this->pCamDrv[j][i];
                this->m_CQ0.setDBLoadHold(MFALSE, MFALSE);
                this->m_CQ0.updateDB_byCQ(MFALSE);
            }
...
MINT32 CmdQMgrImp::CmdQMgr_update(void)
{
    ...
    if(pSec){
        if(pSec->SecMgr_GetSecureCamStatus(this->mModule) > 0){
            if(pSec->SecMgr_CheckSecReg(this->mModule) == MFALSE){
                CmdQMgr_ERR("CheckSecReg failed\n");
                return NULL;
            }

            if(pSec->SecMgr_SetDapcRegAddr(this->mModule) == MFALSE){
                CmdQMgr_ERR("SetDapcRegAddr failed\n");
                return NULL;
            }
        }
    }
...
MINT32 CmdQMgrImp::CmdQMgr_start(void)
{
...
#if DCIF_STT_SW_WORK_AROUND
    this->CmdQMgr_specialCQCtrl4DCIF();
#endif
    ...
    if(pSec){
        if(pSec->SecMgr_GetSecureCamStatus(this->mModule) > 0){
            if(pSec->SecMgr_SetDapcReg(this->mModule) == MFALSE){
                CmdQMgr_ERR("SetDapcReg failed\n");
                return NULL;
            }
...
MBOOL CmdQMgrImp::CmdQMgr_specialCQCtrl4DCIF(void)
{
    MBOOL ret = MTRUE;
    if(this->FSM_CHK(op_update,__FUNCTION__) == MFALSE){
        return MFALSE;
    ...
    for(MUINT32 j=0;j<this->mDupCmdQ;j++){
        ...
        //close sub-done to avoid hw counting during drop frame
        this->m_CQ0.m_pIspDrv = (IspDrvVir*)this->pDummyDrv[j][0];
        this->m_CQ0.disableAllDma(this->pCamDrv[0][0]);
    }
