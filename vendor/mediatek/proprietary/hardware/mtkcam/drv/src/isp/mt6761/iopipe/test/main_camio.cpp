...
MVOID* _test_camio::_infiniteloop(void* arg)
{
    ...
    while(1){
        ...
        if( bRet == MTRUE){
            ...
            {
                if(MFALSE ==_this->m_pNormalP_FrmB->sendCommand(EPIPECmd_GET_CUR_SOF_IDX,(MINTPTR)&_sof_idx,0,0)) {
                    printf("sendCommand fail");
                    break;
                }
...
int test_camio(int argc, char** argv)
{
    ...
    MUINT32 sensorIdx = atoi(argv[1]);  // 0 for main 0x10 for fakeSensor
    ...
    MUINT32 RawTypeSwt = atoi(argv[9]); //0 : rawtypeswt off , 1 : rawtypeswt on
    MUINT32 IMGOCropOneLine = atoi(argv[10]); //0 : IMGOCropOneLine off , 1 : IMGOCropOneLine on

    printf("iopipetest 1 %d %d %d %d %d %d %d %d %d %d\n",sensorIdx,scenario,sensorFps,enablePort,dualSensor,croptestsel,CheckCRC,ISPver,RawTypeSwt,IMGOCropOneLine);
    printf("sensorIdx:0x%x,senario:0x%x,sensorFps:0x%x,enablePort = 0x%x,dualsensor = 0x%x,croptestsel = 0x%x,CheckCRC = 0x%x,ISPver = 0x%x,RawTypeSwt = 0x%x, IMGOCropOneLine = 0x%x\n"
            ,sensorIdx,scenario,sensorFps,enablePort,dualSensor,croptestsel,CheckCRC,ISPver,RawTypeSwt, IMGOCropOneLine);
...
#if EXCUTABLE_TEST
    {
        INormalPipe* Qry_mpCamIO = NULL;
        qryExecIn qryIn;
        MBOOL bIsExe = MFALSE, bRet = MFALSE;

        qryIn.fps = sensorFps;
        qryIn.ScenId = scenario;
        Qry_mpCamIO = INormalPipe::createInstance((MINT32)sensorArray[0], LOG_TAG, 1);
        Qry_mpCamIO->init();
        bRet = Qry_mpCamIO->get_isExecutable(qryIn, bIsExe);
        Qry_mpCamIO->uninit();
        Qry_mpCamIO->destroyInstance(LOG_TAG);
    }
#endif
    //main2 only
    if(sensorIdx == 2){
    ...
    */
    ...
    if(CAM_V1 == ISPversion){
        for(int i=sensorIdx;i<dualSensor;i++){
            ...
            mpCamIO[i] = INormalPipe::createInstance((MINT32)sensorArray[i], npipeSnrUserKey,1);
            mpCamIO[i]->init();
        }
    }
    else{
    */
        for(int i=sensorIdx;i<dualSensor;i++){
            ...
            mpCamIO_FrmB[i] = INormalPipe_FrmB::createInstance((MINT32)sensorArray[i], npipeSnrUserKey,1);
            mpCamIO_FrmB[i]->init();
        }
    //}
    ...
    for(int k=sensorIdx;k<dualSensor;k++){
        for(int i=0;i<Enque_buf;i++){
            ...
            if(enablePort & __RRZO_ENABLE){
                ...
                NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo _buf = NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo(PORT_RRZO,pImgBuffer[k][__RRZO][i],_rrz,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__RRZO][i]->getImgFormat(),croptestsel),_cropsize),i+1);
            ...
            if(enablePort & __IMGO_ENABLE){
                ...
                NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo _buf = NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo(PORT_IMGO,pImgBuffer[k][__IMGO][i],_cropsize,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__IMGO][i]->getImgFormat(),0),_cropsize),i+1);
                buf.mvOut.push_back(_buf);
            }
            if(enablePort & __CAMSV_ENABLE){
                _cropsize = MSize(sensorSize[k].w,sensorSize[k].h);
                NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo _buf = NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo(PORT_CAMSV_IMGO,pImgBuffer[k][__CAMSV][i],_cropsize,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__CAMSV][i]->getImgFormat(),0),_cropsize),i+1);
                buf.mvOut.push_back(_buf);
            }
            if(enablePort & __CAMSV2_ENABLE){
                _cropsize = MSize(sensorSize[k].w,sensorSize[k].h);
                NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo _buf = NSCam::NSIoPipeIsp3::NSCamIOPipe::BufInfo(PORT_CAMSV2_IMGO,pImgBuffer[k][__CAMSV2][i],_cropsize,MRect(__CROP_Start(sensorSize[k],_cropsize,pImgBuffer[k][__CAMSV2][i]->getImgFormat(),0),_cropsize),i+1);
                buf.mvOut.push_back(_buf);
            }
            //if(CAM_V1 == ISPversion)
            //   mpCamIO[k]->enque(buf);
