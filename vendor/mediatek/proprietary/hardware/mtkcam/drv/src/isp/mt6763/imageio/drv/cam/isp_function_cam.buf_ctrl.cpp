...
MBOOL CAM_BUF_CTRL::PipeCheck(void)
{
    ...
    twin.Raw = CAM_READ_REG(this->m_pDrv->getPhyObj(),CAM_CTL_TWIN_STATUS);
    if(twin.Bits.TWIN_EN == 1){
        capibility CamInfo;
        if(CamInfo.m_DTwin.GetDTwin() == MFALSE){
            if(twin.Bits.TWIN_MODULE == CAM_A)
                ptr = (ISP_DRV_CAM*)ISP_DRV_CAM::createInstance(CAM_A,ISP_DRV_CQ_THRE0,0,"PIPE_CHK");
            else if(twin.Bits.TWIN_MODULE == CAM_B)
                ptr = (ISP_DRV_CAM*)ISP_DRV_CAM::createInstance(CAM_B,ISP_DRV_CQ_THRE0,0,"PIPE_CHK");
        }
        else{
            if(twin.Bits.TWIN_MODULE == CAM_A)
                ptr = (ISP_DRV_CAM*)ISP_DRV_CAM::createInstance(CAM_B_TWIN,ISP_DRV_CQ_THRE0,0,"PIPE_CHK");
            else if(twin.Bits.TWIN_MODULE == CAM_B)
...
#define _LOOP_  (5)
    ...
    //this logic is for dummy buffer on virtual UNI
    if(_buf.image.mem_info.size == 0) {
        CAM_FUNC_DBG("(%s) Bypass waitBufReady due to dummy data.", this->name_Str());
        return ret;
    }
    ...
    switch(this->id()){
        ...
        case _pdo_:
            ...
            if (cq == ISP_DRV_CQ_THRE7) {
                ...
                CAM_WRITE_BITS(this->m_pDrv,CAM_FBC_PDO_CTL1,SUB_RATIO,0);
...
E_BC_STATUS CAM_BUF_CTRL::enqueueHwBuf( stISP_BUF_INFO& buf_info,MBOOL bImdMode)
{
    ...
    if(buf_info.bReplace == MFALSE){
        switch(this->id()){ /*this->id()*/
            case _imgo_:
            case _rrzo_:
                CAM_FUNC_INF("%s:(0x%x_0x%x),PA(0x%" PRIXPTR "_0x%" PRIXPTR "),FH_VA(0x%" PRIXPTR "),size(0x%x),enque_sof(%d)",\
                    this->name_Str(),\
                    cq,page,\
        ...
        switch(this->id()){
            case _imgo_:
            case _rrzo_:
                CAM_FUNC_INF("%s:(0x%x_0x%x),PA(0x%" PRIXPTR "_0x%" PRIXPTR ",new:0x%" PRIXPTR "_0x%" PRIXPTR "),FH_VA(0x%" PRIXPTR ",new:0x%" PRIXPTR "),size(0x%x),enque_sof(%d)",\
                    this->name_Str(),\
                    cq,page,\
                    ptr->image.mem_info.pa_addr ,\
                    ptr->header.pa_addr ,\
...
E_BC_STATUS CAM_BUF_CTRL::dequeueHwBuf(NSImageio::NSIspio::BufInfo& buf_info )
{
    ...
    //for dummy data on virtual UNI
    if(_buf.image.mem_info.size == 0) {
        CAM_FUNC_DBG("(%s), Bypass checking header Due to dummy data.", this->name_Str());
...
void BUF_CTRL_IMGO::Header_Parser(MUINTPTR fh_va,NSImageio::NSIspio::BufInfo& buf_info,MUINT32* header_data ,char* pStr)
{
    ...
    if(CAM_READ_BITS(pDrv,CAM_CTL_TWIN_STATUS,TWIN_EN)){
        ...
        if(CAM_READ_BITS(pDrv,CAM_CTL_TWIN_STATUS,TWIN_MODULE) != this->m_hwModule){
            this->m_TimeStamp.TimeStamp_mapping(&buf_info.i4TimeStamp_sec,&buf_info.i4TimeStamp_us,(_isp_dma_enum_)this->id(),this->m_hwModule);
        }
    }
    else{
        this->m_TimeStamp.TimeStamp_mapping(&buf_info.i4TimeStamp_sec,&buf_info.i4TimeStamp_us,(_isp_dma_enum_)this->id(),this->m_hwModule);
...
#else
    this->m_pTimeStamp->TimeStamp_mapping(&buf_info.i4TimeStamp_sec,&buf_info.i4TimeStamp_us,(_isp_dma_enum_)this->id(),this->m_hwModule);
#endif
    ...
    //
    if(pStr){
        snprintf(pStr, sizeof(*pStr), "enq_sof:0x%x,mag:0x%x,timestamp:%u.%u,RawType:0x%x",\
...
void BUF_CTRL_RRZO::Header_Parser(MUINTPTR fh_va,NSImageio::NSIspio::BufInfo& buf_info,MUINT32* header_data ,char* pStr)
{
    ...
    //
    if(pStr){
        snprintf(pStr, sizeof(*pStr), "enq_sof:0x%x,mag:0x%x,timestamp:%u.%u",\
...
void BUF_CTRL_PSO::Header_Parser(MUINTPTR fh_va,NSImageio::NSIspio::BufInfo& buf_info,MUINT32* header_data ,char* pStr)
{
    ...
    //
    if(pStr){
        snprintf(pStr, sizeof(*pStr), "enq_sof:0x%x,mag:0x%x,timestamp:%u.%u,bmx_sel:0x%x",\
...
void BUF_CTRL_AFO::Header_Parser(MUINTPTR fh_va,NSImageio::NSIspio::BufInfo& buf_info,MUINT32* header_data ,char* pStr)
{
    ...
    //
    if(pStr){
        snprintf(pStr, sizeof(*pStr), "enq_sof:0x%x,mag:0x%x,timestamp:%u.%u,stride:0x%x",\
    ...
    do{
        ...
        if(this->m_buf_cnt != 0){
            ...
            if( (FbcCnt < this->m_buf_cnt)||(_pa != 0)){
                ...
                if(loopCnt == _LOOP_)
                    snprintf(str, sizeof(str), "dma:%s _pa:0x%x start wait:[enque record:0x%x_0x%x],",this->name_Str(), _pa, this->m_buf_cnt,FbcCnt);
                else{
                    char _tmp[16];
                    snprintf(_tmp, sizeof(_tmp), "[%d]time,",((_LOOP_-loopCnt)+1));
                    strncat(str,_tmp, strlen(_tmp));
