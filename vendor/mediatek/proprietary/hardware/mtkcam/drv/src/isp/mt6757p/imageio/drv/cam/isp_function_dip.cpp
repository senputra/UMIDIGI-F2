...
#define DIP_FUNC_DBG(fmt, arg...)        do { if (function_Dip_DbgLogEnable_VERBOSE && function_Dip_DbgLogEnable_DEBUG  ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
...
const IspTopModuleMappingTable gIspModuleMappingTableRbgEn[CHECK_CTL_RAW_RGB_EN_BITS]
 ={ {ENUM_DIP_CTL_RGB_EN_UNP , eIspModuleRaw, DIP_A_UNP,   IspDrv_B::setUnp},
    ...
    {ENUM_DIP_CTL_RGB_EN_FLC , eIspModuleRaw, DIP_A_FLC,   IspDrv_B::setFlc},
    {ENUM_DIP_CTL_RGB_EN_FLC2, eIspModuleRaw, DIP_A_FLC2,  IspDrv_B::setFlc2}};


const IspTopModuleMapping3LayerTable gIspModuleMappingTableYuvEn[CHECK_CTL_YUV_EN_BITS]
 ={ {ENUM_DIP_CTL_YUV_EN_MFB ,      eIspModuleYuv, DIP_A_MFB,  DIP_A_MFB2,  DIP_A_DUMMY, IspDrv_B::setMfb},
    ...
    {ENUM_DIP_CTL_YUV_EN_NR3D ,     eIspModuleYuv, DIP_A_NR3D, DIP_A_COLOR, DIP_A_DUMMY, IspDrv_B::setNr3d},
...
MINT32
IspDrv_B::
setFlc(IspImageioPackage &imageioPackage)
{
	if (imageioPackage.isApplyTuning == MTRUE){
		MUINT32 addr,size;
		ISP_DRV_REG_IO_STRUCT reg[10];
...
MINT32
IspDrv_B::
setFlc2(IspImageioPackage &imageioPackage)
{
	if (imageioPackage.isApplyTuning == MTRUE){
		MUINT32 addr,size;
		ISP_DRV_REG_IO_STRUCT reg[10];
...
MINT32
IspDrv_B::
setSrz5(IspImageioPackage &imageioPackage)
{
...
setNr3d(IspImageioPackage &imageioPackage)
{
    ...
    if (imageioPackage.isApplyTuning == MTRUE){
        ...
        DIP_BURST_WRITE_REGS(imageioPackage.hwModule, imageioPackage.pDrvDip, DIP_X_CAM_TNR_ENG_CON,reg,size);
        ...
        DIP_BURST_WRITE_REGS(imageioPackage.hwModule, imageioPackage.pDrvDip, DIP_X_CAM_COLOR_CFG_MAIN,reg,size);
        	
    }
    else {
        DIP_WRITE_BITS(imageioPackage.hwModule, imageioPackage.pDrvDip,DIP_X_CAM_TNR_ENG_CON,NR3D_CAM_TNR_EN,0);	// If there's no NR3D tuning data, turn off NR3D
    }
...
int DIP_TOP_CTRL::_config( void )
{
    ...
    DIP_WRITE_PHY_REG(this->hwModule, m_pIspDrvShell->m_pDrvDipPhy,DIP_X_CTL_YUV_DCM_DIS, 0x00000001);//when mfb dcm is enabled, the scenario realted mfb will happen seldom hang in last tile. 
...
int DIP_YUV_PIPE::_config( void )
{
    ...
    //
    if(this->yuv2_en&DIP_X_REG_CTL_YUV2_EN_SRZ5) {
        memcpy(&imageioPackage.srz5Cfg, &this->srz5Cfg,sizeof(SrzCfg));
    }
    ...
    TdriDrvCfg* pSrcImageio)
{
    ...
    ::memcpy( (char*)&pDst->srz5, (char*)&pSrcImageio->srz5, sizeof(ISP_TPIPE_CONFIG_SRZ_STRUCT));
...
else
{
    ...
    if(isDequeMdp==MTRUE){
        ...
        if(mdpRet == MDPMGR_NO_ERROR){
            //
            pthread_mutex_lock(&mMdpCfgDataMutex);
            gpMdpMgrCfgData[this->moduleIdx][this->dipTh][this->burstQueIdx][this->dupCqIdx].isWaitBuf = true;
            pthread_mutex_unlock(&mMdpCfgDataMutex);
            //
