...
private:
    ...
    class TWIN_STT{
        public:
            TWIN_STT(){
                m_TwinStatus = TwinMgr::E_NOTWIN;
                memset((void*)m_pTwinCQMgr,0,sizeof(DupCmdQMgr*)*ISP_DRV_CAM_BASIC_CQ_NUM*CAM_MAX);
                m_hwModule = CAM_MAX;
            }
        public:
            TwinMgr::E_TWIN_STATUS  m_TwinStatus;   //status to ctrl twincqmgr
            DupCmdQMgr*             m_pTwinCQMgr[ISP_DRV_CAM_BASIC_CQ_NUM][CAM_MAX]; //twinCQMgr, to simulate twinmgr's behavior

            DMA_AFO                 m_AFO;      //add afo &afo's fbc is for enque/deque is multi-thread. using mutex will have performance defect(enque is blocking by deque)
            BUF_CTRL_AFO            m_AFO_FBC;
            //img address/header, reserved for dynamic twin
            vector<MUINTPTR>        m_ImgPA_L;
            vector<MUINTPTR>        m_FHPA_L;
        private:
            ISP_HW_MODULE           m_hwModule;
        public:
            MBOOL           clear(void);
            MBOOL           TwinPath(STTIOPipe* _this);
            MBOOL           SinglePath(STTIOPipe* _this);
    };
...
public:     ////    Operations.
    ...
    virtual MBOOL   suspend(E_SUSPEND_MODE){return MFALSE;}
    virtual MBOOL   resume(E_SUSPEND_MODE){return MFALSE;}
...
public:     ////    original style sendCommand method
    ...
    virtual MBOOL   getCamRes(CamResIn const cfgInParam){cfgInParam;return MFALSE;}
