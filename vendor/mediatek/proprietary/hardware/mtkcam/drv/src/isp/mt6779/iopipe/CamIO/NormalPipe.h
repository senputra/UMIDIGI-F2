...
protected:
    ...
    typedef struct IQData_t
    {
        ...
        :IQNotify(NULL)
        {
            ...
            for(MUINT32 i = 0; i < IQSenSum; i++) {
                IQInfo.sensoridx[i] = IOPIPE_MAX_SENSOR_CNT;
            }
        }
    };
...
public:
    struct NDeqInfo
    {
        NSImageio::NSIspio::ICamIOPipe  *pCamIO;
        MBOOL                            bWait;   // under switch, need to wait for all the buffer dequed by another cam before start deque
    };
    struct SLinkInfo
    {
        MUINT32                             mpSenIdx;
        NSImageio::NSIspio::ICamIOPipe*     mpSttIO;
        list<MUINTPTR>                      PA_L[ePort_max];
        MUINT32                             mNextEnqIdx[ePort_max];
        MUINT32                             mNextDeqIdx[ePort_max];
        MUINT32                             mLockIdx;
        QueueMgr<IMEM_BUF_INFO>*            mpImgHdrMgr;
        vector<IMEM_BUF_INFO>*              mpMInf;
        MBOOL                               bInit;
        QueueMgr<QBufInfo>*                 mpUnOrderEnqContainer[ePort_max];
    };
    struct NLinkInfo
    {
        MUINT32                         mpSenIdx;
        ...
        E_INPUT                         mCurTgIdx;
        MSize                           mTgSize;
        NSImageio::NSIspio::ICamIOPipe  *mpCamIO;
        MBOOL                           bInit;
        MUINT32                         mBufCnt;
        MUINT32                         mLockIdx;
        //Res_Meter_IN
        ...
        QueueMgr<NDeqInfo>*             mpDeqContainer;
    ...
    typedef enum {
        ...
        BufCtrl_suspend,
        BufCtrl_switch_set,
        BufCtrl_switch_phase1,
        BufCtrl_switch_phase2,
        BufCtrl_switch_done,
        BufCtrl_uninit,
    }E_BufCtrlFSM;
...
protected:
        ...
        static SLinkInfo                        mSLinkInfo[IQSenSum];
        static NLinkInfo                        mNLinkInfo[IQSenSum];
        static BufCtrlFSM                       BfcFSM[IQSenSum];
        static Mutex                            BfcSGlock;
        static Mutex                            BfcNGlock;
        static Mutex                            BfcFSMlock;
        static Mutex                            BfcReorderlock[IQSenSum][ePort_max];
        static Mutex                            BfcDeqNPipelock[IQSenSum];
        static MUINT32                          mBfcNSwitchCnt;
        static MUINT32                          mBfcSSwitchCnt;
        static MUINT32                          mBfcUsrCnt;
        static P1_TUNING_NOTIFY*                IQNotify;
        static MSize                            mSLinkTgSize;
        static MUINT32                          mSLinkCfgNum;
        static MBOOL                            bSwitch;
        static MBOOL                   updateBufCtrlFSM(E_BufCtrlFSM const op, MUINT32 const npipe_path);
        static MUINT32                 getBfcIdx(MUINT32 const npipe_path);
public:
    typedef enum
    {
        BUF_CTRL_CMD_CLEAR = 0,
        BUF_CTRL_CMD_SET_NLINK,
        BUF_CTRL_CMD_SET_SLINK,
        BUF_CTRL_CMD_GET_SWITCH_NLINK,
        BUF_CTRL_CMD_GET_SWITCH_SLINK,
        BUF_CTRL_CMD_SWITCH_NLINK,
        BUF_CTRL_CMD_SWITCH_SLINK,
        BUF_CTRL_CMD_GET_IO_BY_PA,
        BUF_CTRL_CMD_GET_CUR_NLINK,
        BUF_CTRL_CMD_GET_CUR_SLINK,
        BUF_CTRL_CMD_MAX,
    }BUF_CTRL_CMD;
    ...
    ////
    void                    initBufCtrl();
    void                    uninitBufCtrl(MUINT32 sensorIdx);
    MBOOL                   sendcmdBufCtrl(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3);
    NSImageio::NSIspio::ICamIOPipe* GetCamIOPipeInstance(MUINT32 sensorIdx);
    NSImageio::NSIspio::ICamIOPipe* GetSttIOPipeInstance(MUINT32 sensorIdx);
    MBOOL                           CheckSwitch();
    MUINT32                         GetCfgNum();
    void                            SetMaxTgSize(MSize* _pTgSize);
    void                            GetMaxTgSize(MSize* _pTgSize);
    // for 3-raw switch cam re-order purpose
    MUINTPTR                        GetSttNextEnqPA(SLinkInfo* pslink, MUINT32 portIdx);
    MUINTPTR                        GetSttNextDeqPA(SLinkInfo* pslink, MUINT32 portIdx);
    void                            AddSttEnqIdx(SLinkInfo* pslink, MUINT32 portIdx);
    void                            AddSttDeqIdx(SLinkInfo* pslink, MUINT32 portIdx);
    void                            ResetSttEnqDeqIdx(SLinkInfo* pslink, MUINT32 portIdx);
    NSImageio::NSIspio::ICamIOPipe* PopDeqCamIOInstance(MUINT32 sensorIdx);
    void                            PushDeqCamIOInstance(MUINT32 sensorIdx, NSImageio::NSIspio::ICamIOPipe* pCamIO, MBOOL bWait);
...
class NormalPipe : public INormalPipe, public NpipeIQContol, public BufCtrlCenter{
...
public:
    ...
    typedef enum {
        ...
        _LCSO_ENABLE = 0x0008,
        _RSSO_ENABLE = 0x0020,
        _SVIMGO_ENABLE = 0x0040,
        _YUVO_ENABLE = 0x0080,
        _CRZO_R1_ENABLE = 0x0100,
        _CRZO_R2_ENABLE = 0x0200,
        _RAWI_ENABLE = 0x0400,
    } eDMAO;
    typedef enum {
        _GetPort_Opened     = 0x1,
...
public:
    ...
    virtual MUINT32 getIspReg(IspP1RegIo *pRegs, MUINT32 RegCount, MUINT32 getRaw, MBOOL bPhysical = MTRUE);
    ...
    static MVOID    switchDoneCB(MUINT32 HwModule);
protected:
...
protected:
    ...
    MUINT32                         mBurstQNum;
    MUINT32                         mSVRawBurstNum;
    MUINT32                         mRawType;

    NSImageio::NSIspio::ICamIOPipe  *mpCamIOPipe;
    NSImageio::NSIspio::ICamIOPipe  *mpCamSVIOPipe;
...
private:
    ...
    E_CamDataPathLink               m_dataPath; //specify from Direct link or DCIF from raw-in
    CAM_RESCONFIG                   mCamResConfig;
...
#define MAX_P1_RST_COUNT    (2)
    ...
    QueueMgr<MUINT32>*              mpVirPortPA[ePort_max];
...
public:
    ...
    IMEM_MUTI_BUF_INFO              mImgSVHeaderPool;
    ...
    static Mutex                    NIOGLock;   // NPipe switch Global Lock
    static Mutex                    NPipeCfgGLock; // NormalPipe Global Lock
    static Mutex                    NPipeDbgLock; // for seninf dump callback
    static Mutex                    NPipeSwitchDoneGLock; // for switch done callback
...
private:
    ...
    MBOOL                   check_switch_phase1(MUINT32* result);
