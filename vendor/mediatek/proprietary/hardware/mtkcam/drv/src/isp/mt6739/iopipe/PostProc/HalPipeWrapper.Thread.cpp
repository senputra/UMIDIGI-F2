    ...
    ECmd &cmd)
{
    ...
    {   //no more cmd in cq1/cq2/cq3 dequeueCmd list, block wait for user add cmd
        ...
        mpPostProcPipe->endequeFrameCtrl(NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_WAKE_WAITFRAME,0,0,0,0,0,0);
...
dequeDoneNotify(
    ...
    MINT32 p2Scenario,
    MBOOL  isIPRaw,
    NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd cmd,
    unsigned long callerID)
{
    ...
    if(bufferListTag==EBufferListTag_Batch)
    {
        ...
        LOG_INF("Erase: updateBNList(%d)",mvEnqueueBufNode.size());
    ...
    else
    {
        ...
        mpPostProcPipe->endequeFrameCtrl(cmd,callerID,0,p2dupCQidx,0,p2Scenario,0);
...
dequeueJudgement()
{
    ...
    LOG_DBG("dequeCq(%d),p2cqIdx(%d),p2cqDupIdx(%d),p2BurstQIdx(%d)",dequeP2Cq,bufNode.p2cqIdx,bufNode.p2cqDupIdx,bufNode.p2BurstQIdx);
    ...
    switch(bufNode.bufTag)
    {
        case EBufferTag_Vss:
            ...
            if (dequeP2Cq >= 3)
                    ...
                    {
                        LOG_ERR("Dequeue Buffer Fail,cID(0x%lx)!",bufNode.callerID);
                        ret=false;
                    }
                }
            }
        ...
        default:
            //[2] wait for deque
            ret=mpPostProcPipe->endequeFrameCtrl(NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_WAIT_DEQUE,0,0,0,0,0,0);
            //[3] get signal and do deque
            if(ret==0)
            {
                LOG_ERR("Wait Deque Fail,cID(0x%lx)!",bufNode.callerID);
                ret=false;
            }
            else
            {
...
)
{
    ...
    NSImageioIsp3::NSIspio::BufInfo jpgBufInfo;
    ...
    MINT32 scenario = bufNode.drvSce;
    MUINT32 a=0,b=0;
    MINT32 frameIndex = mvEnqueuedBufBatch[bufNode.idxInBufBatchList].dequedNum;
    //if(bufNode.portEn == ((1<<EPostProcPortIdx_IMGI) | 1<<EPostProcPortIdx_IMGO))
    if(bufNode.drvSce==NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe || bufNode.drvSce==NSImageioIsp3::NSIspio::eDrvScenario_IP)
    {
        isIPRaw=MTRUE;
    }
        ...
        if( bufNode.portEn & (1 << i))
        {
            ...
            switch(deQret)
            {
                case eIspRetStatus_Failed:
                    ...
                    // error handling, keep going on mdp flow
                    LOG_ERR("do dequeMdpFrameEnd");
                    mpPostProcPipe->dequeMdpFrameEnd(bufNode.drvSce);
                    dequeDoneNotify(EBufferListTag_Node,elementIdx,dequeP2Cq,(MINT32)(bufNode.p2cqDupIdx),scenario,isIPRaw,NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_DEQUE_FAIL,bufNode.callerID);
                    //callback to notify deque fail
                    getLock(ELockEnum_BufBatchEnQ);
                    if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut.size()>0)
                    {
                        LOG_INF("Fst mvOut mva(0x%x)",mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut[0].mBuffer->getBufPA(0));
                    }
                    else
                    {
                    ...
                    // error handling
                    if (mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams.size() == 1) {
                        LOG_ERR("dequeu out buffer failed, release batch node");
                        dequeDoneNotify(EBufferListTag_Batch, elementIdx, dequeP2Cq, (MINT32)(bufNode.p2cqDupIdx),
                            scenario, isIPRaw, NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_DEQUE_FAIL,bufNode.callerID);
                    }
                    return deQret;
        ...
        if( bufNode.portEn & (1 << i))
        {
            ...
            switch(deQret)
            {
                case eIspRetStatus_Failed:
                    ...
                    if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut.size()>0)
                    {
                        LOG_INF("Fst mvOut mva(0x%lu)",(unsigned long)(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut[0].mBuffer->getBufPA(0)));
                    }
                    else
                    {
                        LOG_ERR("no output dma");
                    ...
                    // error handling
                    if (mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams.size() == 1) {
                        LOG_ERR("dequeu in buffer failed, release batch node");
                        dequeDoneNotify(EBufferListTag_Batch, elementIdx, dequeP2Cq, (MINT32)(bufNode.p2cqDupIdx),
                            scenario, isIPRaw, NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_DEQUE_FAIL,bufNode.callerID);
                    }
                    return deQret;
    ...
    //LOG_INF("QQ id : (%x)  callback (0x%x)",bufNode.idxInBufBatchList,mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mpfnCallback);
    ...
    if(retFrame==true)
    {
        ...
        if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut.size()>0)
        {
            LOG_INF("FmvOut(0x%lu),dCq(%d/%d)",(unsigned long)(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut[0].mBuffer->getBufPA(0)),bufNode.p2cqIdx,bufNode.p2cqDupIdx);
        }
        else
        {
            LOG_ERR("no output dma");
        ...
        {    //slow motion support,//all buffer nodes in the batch are deque done
            ...
            if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].dequedNum == mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams.size())
                {dequeSuccess=true;}
        }
        ...
        if(dequeSuccess==true)
        {
            ...
            if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mpfnCallback == NULL)
            {
                ...
                if(bufNode.jpgo_idx>=0)
                {
                    ...
                    mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut[bufNode.jpgo_idx].mBuffer->setBitstreamSize(jpgRealSize);    //TODO
                }
                ...
                //pop out buffer node
                dequeDoneNotify(EBufferListTag_Node,elementIdx,dequeP2Cq,(MINT32)(bufNode.p2cqDupIdx),scenario,isIPRaw,NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_DEQUE_SUCCESS,bufNode.callerID);
                //deque notify for whole buffer batch
                dequeDoneNotify(EBufferListTag_Batch,elementIdx,dequeP2Cq,(MINT32)(bufNode.p2cqDupIdx),scenario,isIPRaw,NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_DEQUE_SUCCESS,bufNode.callerID);
            }
            else
            {
...
//                LOG_INF("QQ idx(%d)",bufNode.idxInBufBatchList);
                ...
                ioctrlret=mpPostProcPipe->endequeFrameCtrl(NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_WAIT_FRAME,bufNode.callerID,0, 0, 0,scenario, 1000000);
                if(!ioctrlret)
                {
    ...
    {
        //TODO: error handling of multiple frames for batch node
        if (mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams.size() == 1) {
            LOG_ERR("dequeMdpFrameEnd fail,  release batch node");
            dequeDoneNotify(EBufferListTag_Batch, elementIdx, dequeP2Cq, (MINT32)(bufNode.p2cqDupIdx),
                scenario, isIPRaw, NSImageioIsp3::NSIspio::EPIPE_BUFQUECmd_DEQUE_FAIL,bufNode.callerID);
        }
    ...
    switch(bufNode.drvSce)
    {
        ...
        case NSImageioIsp3::NSIspio::eDrvScenario_VSS_MFB_Mixing:
            ...
            releaseLock(ELockEnum_VssOccupied);  //notify
            break;
        default:
            break;
     }    // For Vss Concurrency Check -----

   LOG_INF("-, All buffer nodes in the batch are deque done = %d, cb(%d us)",dequeSuccess, b-a);
   return ret;
}
/******************************************************************************
 *
