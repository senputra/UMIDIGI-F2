...
//-----------------------------------------------------------------------------
DipRingBuffer* DipRingBuffer::createInstance(ISP_DIP_MODULE_IDX module)
{
    return DipRingBufferImp::getInstance(module);
}

static DipRingBufferImp singleton[ISP_DIP_MODULE_IDX_MAX];

DipRingBuffer* DipRingBufferImp::getInstance(ISP_DIP_MODULE_IDX module)
{
...
bool DipRingBufferImp::init(MUINT32 secTag)
{
    ...
    if (this->mSecureTag == 1)
    {
        ...
        DIP_RINGBUF_INF("[DipSecBufInfo] CQ: size(0x%x),Handle(0x%lx), Tpipe: size(0x%x),Handle(0x%lx)",m_CQSecBufInfo.size,(unsigned long)m_CQSecBufInfo.secHandle,m_TpipeSecBufInfo.size,(unsigned long)m_TpipeSecBufInfo.secHandle);
    }
...
bool DipRingBufferImp::getSecureBufHandle(MUINT32& cqHandle, MUINT32& tpipeHandle)
{
    if(this->mInitCount == 0)
    {
...
bool DipRingBufferImp::getSecureBufSize(MUINT32& cqSize, MUINT32& tpipeSize)
{
    if(this->mInitCount == 0)
    {
