...
#if PRINT_TESTLOG_IN_ADB

#define  MY_LOGI(...)  do {  __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
#define MY_LOGD(...)    do {  __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
#define MY_LOGV(...)    do {  __android_log_print(ANDROID_LOG_VERBOSE, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
#define MY_LOGW(...)  do {  __android_log_print(ANDROID_LOG_WARN, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
#define MY_LOGE(...)  do {  __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
/*
...
public:
    ...
    MUINTPTR                mCamsvBufVa;
    MUINT32                 mCamsvBufSize;
    MSize                   mTgSize;
...
public:
    ...
    {
        static RssP1Cb sRssCb;
        RssP1Cb* (*null_fp)(int, const char*) = 0;
        return (getRssP1CbImpl == null_fp) ? &sRssCb : getRssP1CbImpl(sensorID, name);
    }
