...
dequeJudgement(list<FramePackage>& FramePackListRef)
{
    ...
    {
        //Save Buffer
        if (this->mpHalPipeWrapper->m_iSaveReqToFile || (*it).rParams.mvFrameParams[frm].NeedDump || this->mpHalPipeWrapper->m_BTDump)
        {
            LOG_INF("m_iSaveReqToFile: %d, NeedDump: %d, m_BTDump: %d !!", this->mpHalPipeWrapper->m_iSaveReqToFile, (*it).rParams.mvFrameParams[frm].NeedDump, this->mpHalPipeWrapper->m_BTDump);
            ...
            if(ispVirRegVa != 0)
            {
                if ( (this->mpHalPipeWrapper->m_iSaveReqToFile & (DIP_DUMP_TUNING_BUFFER))
                    || ((*it).rParams.mvFrameParams[frm].NeedDump & this->mpHalPipeWrapper->m_BTDump))
                {
