...
public:
    ...
    MBOOL           get_availNum(ISP_HW_MODULE master, vector<ISP_HW_MODULE>& v_available, char Name[32]);
...
private:
    ...
    MBOOL               Res_Meter_Bayer(const Res_Meter_IN& cfgInParam, Res_Meter_OUT &cfgOutParam, MUINT32 cur_clkL);
    MBOOL               Res_Meter_YUV(const Res_Meter_IN& cfgInParam, Res_Meter_OUT &cfgOutParam);
    MBOOL               Res_Meter_Release(void);


    inline MBOOL        Res_Meter_kernel(const Res_Meter_IN& cfgInParam, Res_Meter_OUT &cfgOutParam, MUINT32& thru_put,
                            MBOOL& bBin_useful, MBOOL &bBin_first, vector<ISP_HW_MODULE>& availCam);
    inline MVOID        K_CLK_CASE(Res_Meter_OUT &cfgOutParam);
    inline MVOID        K_BIN_CASE(Res_Meter_OUT &cfgOutParam,MUINT32& thru_put,MBOOL& bBin_useful);
