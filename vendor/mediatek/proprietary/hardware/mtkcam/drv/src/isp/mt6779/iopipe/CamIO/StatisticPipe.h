...
class StatisticPipe : public IStatisticPipe, public BufCtrlCenter{
public:
...
public:
    ...
    typedef enum {
        ...
        _TSFSO_ENABLE      = 0x4000,
        _PDO_VIR_ENABLE    = 0x8000,        // start of virtual port, which means the STT port not support in raw_c
        _FLKO_VIR_ENABLE   = 0x10000,
    } eDMAO;
    typedef enum {
        _GetPort_Opened     = 0x1,
...
public:
    ...
    static MUINT32  sttDSwitchCB(CAM_STATE_OP state, MVOID *Obj);
...
private:
    ...
    MBOOL           getSTTFunc(NormalPipe_QueryInfo* queryInfo);
    MBOOL           enqueToBuf(QBufInfo const& rQBuf, QueueMgr<IMEM_BUF_INFO>* imghdr, NSImageio::NSIspio::ICamIOPipe* pSttIO, MUINT32 senIdx, MUINT32 _port);

    static MUINT32                   ioPipeCnt;
...
protected:
    ...
    mutable Mutex               mDeqSTTLock[2][SttPortID_MAX];
...
public:
    ...
    static vector<IMEM_BUF_INFO> mpMemInfo[EPIPE_Sensor_RSVD][SttPortID_MAX]; //index by _PortMap
    static QueueMgr<IMEM_BUF_INFO> mpImgHeaderMgr[EPIPE_Sensor_RSVD][SttPortID_MAX];

    static Mutex                SPipeGLock; // StatisticPipe Global Lock
    static Mutex                SIOGLock; // StatisticPipe Global Lock
