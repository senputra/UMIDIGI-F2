...
void StatisticPipe::enqueRequestCB(void *para, void *cookie)
{
    ...
    QBufInfo         _qBuf;
    vector<MUINT32>* pLMagic = (vector<MUINT32>*)para;
    ...
    for (p = 0; p < _this->mOpenedPortNum; p++) {
        NSImageio::NSIspio::PortID      portID;
        NSImageio::NSIspio::QBufInfo    rQBufInfo;
        IMEM_BUF_INFO   imBuf;
        BufInfo*        pBuf;

        portID.index = _this->PortIndex[p];
        _port = _PortMap(portID.index);
        ...
        if (0 == reqQueNum[_port]) {
            ...
            PIPE_DBG("TG_%d port_%d no req at cur frame", _this->mTgInfo, _this->PortIndex[p]);
            continue;
        }
...
MBOOL StatisticPipe::deque(PortID port, QBufInfo& rQBuf, ESTT_CACHE_SYNC_POLICY cacheSyncPolicy, MUINT32 u4TimeoutMs)
{
    ...
    for (idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++) {
        ...
        result.mTimeStamp_B = getTimeStamp_ns(rQTSBufInfo.vBufInfo.at(idx).i4TimeStamp_sec_B,
                                rQTSBufInfo.vBufInfo.at(idx).i4TimeStamp_us_B);
        ...
        PIPE_DBG("SttD:%d: dma:x%x pa(0x%" PRIXPTR ") size(%d) mag(0x%x) stride(%d) type(0x%x) ts(%lld)\n",
                mpSensorIdx, portID.index, pbuf->mPa, pbuf->mSize,
                result.mMagicNum_hal,pbuf->mStride,result.mRawType,
                result.mTimeStamp_B);
...
{
    MBOOL  ret = MTRUE;
    MUINT32     uni_available = 1, MainImgBQNum = 0, totalBufNum = 0;
    ...
    vector<MUINTPTR>    ImgPA_L[MaxStatPortIdx];
    vector<MUINTPTR>    ImgHeaderPA_L[MaxStatPortIdx];
    ...
    do {
        ...
        for (MUINT32 headersize = 0,i = 0; i < mvInPorts->mStatPortInfo.size(); i++) {
            ...
            mImgHeaderSize[_PortMap(mvInPorts->mStatPortInfo.at(i).mPortID.index)] = headersize;
        }
        mImgHeaderPool.size += 8;//+8 for 64bit alignment

        PIPE_INF("ImgHeader allocate+");
        ...
        for (MUINT32 headersize = 0,i = 0; i < mvInPorts->mStatPortInfo.size(); i++) {
            ...
            headersize = mImgHeaderSize[_PortMap(mvInPorts->mStatPortInfo.at(i).mPortID.index)];
    ...
    for (i = 0; i < mOpenedPortNum; i++) {
        ...
        do {
            vector<MUINTPTR>::iterator it, it2;
...
{
    MBOOL       ret = MTRUE, cmdSent = 0;
    MUINT32     MainImgBQNum = 0, totalBufNum = 0, _portNum = 0;
    MUINT32     i = 0, j = 0, _portIdx = 0, imgW = 0, imgH = 0, dma_size = 0;
    MUINTPTR    imgHdrAllocOfst = 0;
    E_INPUT eTgInput;
    ...
    for (i = 0; i < SenInfo.mSList->queryNumberOfSensors(); i++) {
     ...
     do {
        ...
        // update mOpenedPortNum and PortIndex[]
