...
MBOOL RscDrvImp::init(const char* userName)
{
    ...
    MUINT32 resetModule;
    //
    android::Mutex::Autolock lock(this->m_RscInitMutex);
    //
...
EXIT:
    ...
    LOG_DBG("-,ret: %d. mInitCount:(%d),m_pRscHwRegAddr(0x%x)", Result, this->m_UserCnt, m_pRscHwRegAddr);
    return Result;
}
...
MBOOL RscDrvImp::writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller)
{
    MINT32 ret=0;
    MUINT32 legal_range = RSC_REG_RANGE;
    LOG_DBG("Rsc_write:m_regRWMode(0x%x),(0x%x,0x%x)",this->m_regRWMode,Addr,Data);
    android::Mutex::Autolock lock(this->RscRegMutex);
    //(void)caller;
    if(-1 == FD_CHK()){
        return MFALSE;
