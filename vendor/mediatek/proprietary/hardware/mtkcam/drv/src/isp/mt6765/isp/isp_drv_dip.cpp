...
static IspDrvDip singleton[DIP_MAX-DIP_A];
...
IspDrvDip::IspDrvDip()
{
    ...
    m_hwModule = DIP_A;
    m_dipModuleIdx = (ISP_DIP_MODULE_IDX)(DIP_A - DIP_START);

    m_DipRegLegalRange = DIP_REG_RANGE;
    //m_pageIdx = pageIdx;
    //
    //m_TotalUserCnt++;
    m_dipCq = (E_ISP_DIP_CQ)0;
    m_burstQueIdx = 0;
    m_dupCqIdx = 0;
    //LOG_DBG("getpid[0x%08x],gettid[0x%08x],m_TotalUserCnt(%d)\n", getpid() ,gettid(), m_TotalUserCnt);
}
...
IspDrvDip* IspDrvDip::createInstance(ISP_HW_MODULE hwModule)
{
#if 1  //HungWen

    MUINT32 moduleIdx;

    LOG_DBG("hwModule(%d)",hwModule);
...
MBOOL IspDrvDip::init(const char* userName)
{
    ...
    //
    LOG_DBG("+,m_UserCnt(%d),curUser(%s),hwModule(%d),m_dipModuleIdx(%d),m_dipCq(%d),m_burstQueIdx(%d),m_dupCqIdx(%d)",
            this->m_UserCnt,userName,this->m_hwModule,this->m_dipModuleIdx,this->m_dipCq,this->m_burstQueIdx,this->m_dupCqIdx);
    //
    ...
    if(this->m_UserCnt==0) {
        m_pIspDrvImp = IspDrvImp::createInstance(DIP_A);
...
MBOOL IspDrvDip::uninit(const char* userName)
{
    ...
    android::Mutex::Autolock lock(this->IspRegMutex);
    //
    LOG_DBG("+,m_UserCnt(%d),curUser(%s)",this->m_UserCnt,userName);
    //
    if(strlen(userName)<1)
    {
...
MBOOL IspDrvDip::setCQDescBufPhyAddr(MUINT32* pIspDescript_phy)
{
    this->m_pIspDescript_phy = pIspDescript_phy;
    return MTRUE;
}

MBOOL IspDrvDip::setCQDescBufVirAddr(ISP_DRV_CQ_CMD_DESC_STRUCT* pIspDescript_vir)
{
...
MBOOL IspDrvDip::setIspVirRegPhyAddr(MUINT32* pIspVirRegAddr_pa)
{
    this->m_pIspVirRegAddr_pa = pIspVirRegAddr_pa;
    return MTRUE;
}

MBOOL IspDrvDip::setIspVirRegVirAddr(MUINT32* pIspVirRegAddr_va)
{
...
MBOOL IspDrvDip::setCQDescBufOft(MUINT32 IspDescript_oft)
{
    this->m_IspDescript_oft = IspDescript_oft;
    return MTRUE;
}

MBOOL IspDrvDip::setIspVirRegOft(MUINT32 IspVirRegAddr_oft)
{
    this->m_IspVirRegAddr_oft = IspVirRegAddr_oft;
    return MTRUE;
}

MUINT32 IspDrvDip::getCQDescBufOft(void)
{
...
MUINT32 IspDrvDip::getIspVirRegOft(void)
{
...
MBOOL IspDrvDip::readRegs(ISP_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller)
{
    if ((m_pIspVirRegAddr_va == NULL) || (pRegIo == NULL)){
        LOG_ERR("[0x%x]. Virtual ISP Addr:0x%lx or pRegIo:0x%lx is NULL!! Count:0x%x, caller:0x%x",this->m_hwModule, (unsigned long)m_pIspVirRegAddr_va, (unsigned long)pRegIo, Count, caller);
        return MFALSE;
    }
    //
    android::Mutex::Autolock lock(this->IspRegMutex);
...
MUINT32 IspDrvDip::readReg(MUINT32 Addr,MINT32 caller)
{
    if (m_pIspVirRegAddr_va == NULL){
        LOG_ERR("[0x%x]. Virtual ISP Addr:0x%lx is NULL!! Addr:0x%x, caller:0x%x",this->m_hwModule, (unsigned long)m_pIspVirRegAddr_va, Addr, caller);
        return MFALSE;
    }
    //
    android::Mutex::Autolock lock(this->IspRegMutex);
...
//MBOOL IspDrvDip::writeRegs(ISP_DRV_REG_IO_STRUCT*  pRegIo,MUINT32 Count,MINT32 caller)
MBOOL IspDrvDip::writeRegs(MUINT32 OffsetAddr,MUINT32 Count,MUINT32* SrcAddr)

{
    ...
    LOG_DBG("OffsetAddr:0x%x,Count:0x%x", OffsetAddr, Count);
...
MBOOL IspDrvDip::writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller)
{
    ...
    if (m_pIspVirRegAddr_va == NULL){
        LOG_ERR("[0x%x]. Virtual ISP Addr:0x%lx is NULL!! Data:0x%lx, caller:0x%x",this->m_hwModule, (unsigned long)m_pIspVirRegAddr_va, Data, caller);
        return MFALSE;
    }
    //
    android::Mutex::Autolock lock(this->IspRegMutex);
    ...
    LOG_DBG("m_pIspVirRegAddr_va:0x%lx, addr:0x%x,Data:0x%lx, m_pIspVirRegAddr_va[0x%x]:0x%x", (unsigned long)this->m_pIspVirRegAddr_va, Addr, Data, (Addr>>2), this->m_pIspVirRegAddr_va[Addr>>2]);
#if 0
    MUINT32 shift;
    MUINT32 legal_range;
...
MBOOL IspDrvDip::dumpTuningDebugLog(void)
{
	...
	LOG_INF("========== DUMP DIP ==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_CTL_RGB_EN);
	...
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_CTL_YUV2_EN);
	LOG_INF("DIP_X_CTL_YUV2_EN=0x%08x",regR);


//DBS2
	LOG_INF("========== DUMP DBS2 PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_DBS2_SIGMA);
	LOG_INF("DIP_X_DBS2_SIGMA=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_DBS2_BSTBL_0);
	LOG_INF("DIP_X_DBS2_BSTBL_0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_DBS2_BSTBL_1);
	LOG_INF("DIP_X_DBS2_BSTBL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_DBS2_BSTBL_2);
	LOG_INF("DIP_X_DBS2_BSTBL_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_DBS2_BSTBL_3);
	LOG_INF("DIP_X_DBS2_BSTBL_3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_DBS2_CTL);
	LOG_INF("DIP_X_DBS2_CTL=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_DBS2_CTL_2);
	LOG_INF("DIP_X_DBS2_CTL_2=0x%08x",regR);

//OBC2
	LOG_INF("========== DUMP OBC2 PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_OFFST0);
	LOG_INF("DIP_X_OBC2_OFFST0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_OFFST1);
	LOG_INF("DIP_X_OBC2_OFFST1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_OFFST2);
	LOG_INF("DIP_X_OBC2_OFFST2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_OFFST3);
	LOG_INF("DIP_X_OBC2_OFFST3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_GAIN0);
	LOG_INF("DIP_X_OBC2_GAIN0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_GAIN1);
	LOG_INF("DIP_X_OBC2_GAIN1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_GAIN2);
	LOG_INF("DIP_X_OBC2_GAIN2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_OBC2_GAIN3);
	LOG_INF("DIP_X_OBC2_GAIN3=0x%08x",regR);

//LSC2
	LOG_INF("========== DUMP LSC2 PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_CTL1);
	LOG_INF("DIP_X_LSC2_CTL1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_CTL2);
	LOG_INF("DIP_X_LSC2_CTL2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_CTL3);
	LOG_INF("DIP_X_LSC2_CTL3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_LBLOCK);
	LOG_INF("DIP_X_LSC2_LBLOCK=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_RATIO_0);
	LOG_INF("DIP_X_LSC2_RATIO_0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_RATIO_1);
	LOG_INF("DIP_X_LSC2_RATIO_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_TPIPE_OFST);
	LOG_INF("DIP_X_LSC2_TPIPE_OFST=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_TPIPE_SIZE);
	LOG_INF("DIP_X_LSC2_TPIPE_SIZE=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LSC2_GAIN_TH);
	LOG_INF("DIP_X_LSC2_GAIN_TH=0x%08x",regR);

//PGN
	LOG_INF("========== DUMP PGN PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PGN_SATU_1);
	LOG_INF("DIP_X_PGN_SATU_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PGN_SATU_2);
	LOG_INF("DIP_X_PGN_SATU_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PGN_GAIN_1);
	LOG_INF("DIP_X_PGN_GAIN_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PGN_GAIN_2);
	LOG_INF("DIP_X_PGN_GAIN_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PGN_OFST_1);
	LOG_INF("DIP_X_PGN_OFST_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PGN_OFST_2);
	LOG_INF("DIP_X_PGN_OFST_2=0x%08x",regR);


//G2G
	LOG_INF("========== DUMP G2G PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2G_CNV_1);
	LOG_INF("DIP_X_G2G_CNV_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2G_CNV_2);
	LOG_INF("DIP_X_G2G_CNV_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2G_CNV_3);
	LOG_INF("DIP_X_G2G_CNV_3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2G_CNV_4);
	LOG_INF("DIP_X_G2G_CNV_4=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2G_CNV_5);
	LOG_INF("DIP_X_G2G_CNV_5=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2G_CNV_6);
	LOG_INF("DIP_X_G2G_CNV_6=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2G_CTRL);
	LOG_INF("DIP_X_G2G_CTRL=0x%08x",regR);


//G2C
	LOG_INF("========== DUMP G2C PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CONV_0A);
	LOG_INF("DIP_X_G2C_CONV_0A=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CONV_0B);
	LOG_INF("DIP_X_G2C_CONV_0B=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CONV_1A);
	LOG_INF("DIP_X_G2C_CONV_1A=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CONV_1B);
	LOG_INF("DIP_X_G2C_CONV_1B=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CONV_2A);
	LOG_INF("DIP_X_G2C_CONV_2A=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CONV_2B);
	LOG_INF("DIP_X_G2C_CONV_2B=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_SHADE_CON_1);
	LOG_INF("DIP_X_G2C_SHADE_CON_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_SHADE_CON_2);
	LOG_INF("DIP_X_G2C_SHADE_CON_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_SHADE_CON_3);
	LOG_INF("DIP_X_G2C_SHADE_CON_3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_SHADE_TAR);
	LOG_INF("DIP_X_G2C_SHADE_TAR=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_SHADE_SP);
	LOG_INF("DIP_X_G2C_SHADE_SP=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CFC_CON_1);
	LOG_INF("DIP_X_G2C_CFC_CON_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_G2C_CFC_CON_2);
	LOG_INF("DIP_X_G2C_CFC_CON_2=0x%08x",regR);

//MFB
	LOG_INF("========== DUMP MFB PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MFB_CON);
	LOG_INF("DIP_X_MFB_CON=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MFB_LL_CON1);
	LOG_INF("DIP_X_MFB_LL_CON1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MFB_LL_CON2);
	LOG_INF("DIP_X_MFB_LL_CON2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MFB_LL_CON3);
	LOG_INF("DIP_X_MFB_LL_CON3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MFB_LL_CON4);
	LOG_INF("DIP_X_MFB_LL_CON4=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MFB_EDGE);
	LOG_INF("DIP_X_MFB_EDGE=0x%08x",regR);


//MIX3
	LOG_INF("========== DUMP MIX3 PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MIX3_CTRL_0);
	LOG_INF("DIP_X_MIX3_CTRL_0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MIX3_CTRL_1);
	LOG_INF("DIP_X_MIX3_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_MIX3_SPARE);
	LOG_INF("DIP_X_MIX3_SPARE=0x%08x",regR);

//LCE
	LOG_INF("========== DUMP LCE PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_ANR2_LCE);
	LOG_INF("DIP_X_ANR2_LCE=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_CON);
	LOG_INF("DIP_X_LCE25_CON=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_ZR);
	LOG_INF("DIP_X_LCE25_ZR=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_SLM_SIZE);
	LOG_INF("DIP_X_LCE25_SLM_SIZE=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_OFST);
	LOG_INF("DIP_X_LCE25_OFST=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_BIAS);
	LOG_INF("DIP_X_LCE25_BIAS=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_IMAGE_SIZE);
	LOG_INF("DIP_X_LCE25_IMAGE_SIZE=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_BIL_TH0);
	LOG_INF("DIP_X_LCE25_BIL_TH0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_BIL_TH1);
	LOG_INF("DIP_X_LCE25_BIL_TH1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_TM_PARA0);
	LOG_INF("DIP_X_LCE25_TM_PARA0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_TM_PARA1);
	LOG_INF("DIP_X_LCE25_TM_PARA1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_TM_PARA2);
	LOG_INF("DIP_X_LCE25_TM_PARA2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_TM_PARA3);
	LOG_INF("DIP_X_LCE25_TM_PARA3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_TM_PARA4);
	LOG_INF("DIP_X_LCE25_TM_PARA4=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_TM_PARA5);
	LOG_INF("DIP_X_LCE25_TM_PARA5=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_LCE25_TM_PARA6);
	LOG_INF("DIP_X_LCE25_TM_PARA6=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_ANR_LCE);
...
//GGM
	...
	//DIP_X_GGM_LUT_G[144];
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_GGM_CTRL);
...
//PCA
	...
	//DIP_X_PCA_TBL[360];
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PCA_CON1);
	LOG_INF("DIP_X_PCA_CON1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PCA_CON2);
	LOG_INF("DIP_X_PCA_CON2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PCA_CON3);
	LOG_INF("DIP_X_PCA_CON3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PCA_CON4);
	LOG_INF("DIP_X_PCA_CON4=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PCA_CON5);
	LOG_INF("DIP_X_PCA_CON5=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PCA_CON6);
	LOG_INF("DIP_X_PCA_CON6=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_PCA_CON7);
	LOG_INF("DIP_X_PCA_CON7=0x%08x",regR);


//SEEE
	LOG_INF("========== DUMP SEEE PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_CTRL);
	LOG_INF("DIP_X_SEEE_CTRL=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_CLIP_CTRL_1);
	LOG_INF("DIP_X_SEEE_CLIP_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_CLIP_CTRL_2);
	LOG_INF("DIP_X_SEEE_CLIP_CTRL_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_CLIP_CTRL_3);
	LOG_INF("DIP_X_SEEE_CLIP_CTRL_3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_BLND_CTRL_1);
	LOG_INF("DIP_X_SEEE_BLND_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_BLND_CTRL_2);
	LOG_INF("DIP_X_SEEE_BLND_CTRL_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_GN_CTRL);
	LOG_INF("DIP_X_SEEE_GN_CTRL=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_LUMA_CTRL_1);
	LOG_INF("DIP_X_SEEE_LUMA_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_LUMA_CTRL_2);
	LOG_INF("DIP_X_SEEE_LUMA_CTRL_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_LUMA_CTRL_3);
	LOG_INF("DIP_X_SEEE_LUMA_CTRL_3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_LUMA_CTRL_4);
	LOG_INF("DIP_X_SEEE_LUMA_CTRL_4=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SLNK_CTRL_1);
	LOG_INF("DIP_X_SEEE_SLNK_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SLNK_CTRL_2);
	LOG_INF("DIP_X_SEEE_SLNK_CTRL_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_GLUT_CTRL_1);
	LOG_INF("DIP_X_SEEE_GLUT_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_GLUT_CTRL_2);
	LOG_INF("DIP_X_SEEE_GLUT_CTRL_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_GLUT_CTRL_3);
	LOG_INF("DIP_X_SEEE_GLUT_CTRL_3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_GLUT_CTRL_4);
	LOG_INF("DIP_X_SEEE_GLUT_CTRL_4=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_GLUT_CTRL_5);
	LOG_INF("DIP_X_SEEE_GLUT_CTRL_5=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_GLUT_CTRL_6);
	LOG_INF("DIP_X_SEEE_GLUT_CTRL_6=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_OUT_EDGE_CTRL);
	LOG_INF("DIP_X_SEEE_OUT_EDGE_CTRL=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SE_Y_CTRL);
	LOG_INF("DIP_X_SEEE_SE_Y_CTRL=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SE_EDGE_CTRL_1);
	LOG_INF("DIP_X_SEEE_SE_EDGE_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SE_EDGE_CTRL_2);
	LOG_INF("DIP_X_SEEE_SE_EDGE_CTRL_2=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SE_EDGE_CTRL_3);
	LOG_INF("DIP_X_SEEE_SE_EDGE_CTRL_3=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SE_SPECL_CTRL);
	LOG_INF("DIP_X_SEEE_SE_SPECL_CTRL=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SE_CORE_CTRL_1);
	LOG_INF("DIP_X_SEEE_SE_CORE_CTRL_1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SEEE_SE_CORE_CTRL_2);
	LOG_INF("DIP_X_SEEE_SE_CORE_CTRL_2=0x%08x",regR);

//SL2
	LOG_INF("========== DUMP SL2 PART==============");
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_CEN);
	LOG_INF("DIP_X_SL2_CEN=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_RR_CON0);
	LOG_INF("DIP_X_SL2_RR_CON0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_RR_CON1);
	LOG_INF("DIP_X_SL2_RR_CON1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_GAIN);
	LOG_INF("DIP_X_SL2_GAIN=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_RZ);
	LOG_INF("DIP_X_SL2_RZ=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_XOFF);
	LOG_INF("DIP_X_SL2_XOFF=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_YOFF);
	LOG_INF("DIP_X_SL2_YOFF=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_SLP_CON0);
	LOG_INF("DIP_X_SL2_SLP_CON0=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_SLP_CON1);
	LOG_INF("DIP_X_SL2_SLP_CON1=0x%08x",regR);
	regR = DIP_READ_PHY_REG(pDrvDipPhy,DIP_X_SL2_SIZE);
