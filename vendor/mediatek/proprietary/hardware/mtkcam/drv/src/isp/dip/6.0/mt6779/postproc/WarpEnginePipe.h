...
class WarpEnginePipe : public IWarpEnginePipe
{
    ...
    public:     ////    Instantiation.
        virtual MBOOL   init(ISP_WPE_MODULE_IDX module_idx);
        virtual MBOOL   uninit(ISP_WPE_MODULE_IDX module_idx);

        virtual MINT32 configwpe(WpePackage wpepackage);
        virtual MINT32 configWpeCommandData(NSCam::NSIoPipe::WPEConfig wpeconfig, MUINT32 configExtraFeture);
        virtual MVOID  configTpipeData(NSCam::NSIoPipe::WPEConfig wpeconfig, MUINT32 configExtraFeture);
        virtual MBOOL  wpeconfigMdpOutPort(NSImageio::NSIspio::PortInfo const* oImgInfo, MdpRotDMACfg &a_rotDma);
        ...
        virtual MINT32 tpipe_dump(ISP_TPIPE_CONFIG_STRUCT *pTdri);
        virtual MINT32 dequeue(MINT32  wpeDupIdx, MINT32  wpeBurstIdx, MUINT32 wpeDupCmdIdx, QParams rParams);
        ...
        virtual MBOOL  saveToFile(char const* filepath, unsigned char* pBuf, size_t  size);
        virtual MBOOL  saveToTextFile(char const* filepath, unsigned char* pBuf, size_t  size);
        virtual MVOID  DumpDebugBuffes(MINT32  wpeBurstIdx, MINT32  wpeDupIdx, MUINT32 wpeDupCmdIdx, QParams rParams);
    ...
    private:
        ...
        //MINT32                   m_bSaveReqToFile; //no used
        //MINT32                   mDumpCount;       //no used

        IMEM_BUF_INFO            gWPEMdpBufInfo;
        IMEM_BUF_INFO            gWPETpipeBufInfo;
        IMEM_BUF_INFO            gWPEDL_TpipeBufInfo;
