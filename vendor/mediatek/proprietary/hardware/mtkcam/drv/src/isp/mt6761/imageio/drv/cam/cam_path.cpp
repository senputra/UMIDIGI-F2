...
#include <drv/utils.h> // for UNUSED
...
//
int CamPath_B::abort( void* pParam )
{
    ISP_PATH_INF("{%s}::Abort E" DEBUG_STR_BEGIN "", this->name_Str() );

    //Mutex::Autolock lock(this->ispTopCtrl.m_pIspDrvShell->gLock);

    int ret = 0;

    if( this->_abort( pParam ) < 0 ){
        ISP_PATH_ERR("[ERROR] abort(%s) ",this->name_Str());
        //return -1;
        ret = -1;
        goto EXIT;
    }
    ISP_PATH_DBG(DEBUG_STR_END"%s::Abort""", this->name_Str() );
...
int CamPath_B::_abort( void* pParam )
{
    UNUSED(pParam);

    ISP_PATH_DBG("_abort E ");
    ...
    //3. abort or disable (From head to tail)
    PRINT_ELEMENT_INVOLVED("[abort]:", isplist, ispcount, 0, 1);
    for (i = 0; i < ispcount; i++) {
        if (strcmp(isplist[i]->name_Str(), "ISP_TOP") == 0) {
            ISP_PATH_DBG("<%s> abort.", isplist[i]->name_Str());
            ispTopCtrl.abort();
        } else {
            if ((ret_val_temp = isplist[i]->disable()) < 0)
                ret_val = ret_val_temp;
...
int CamPath_B::_registerIrq( NSImageioIsp3::NSIspio::Irq_t irq )
{
    UNUSED(irq);
    return 0;
}
int CamPath_B::_waitIrq( NSImageioIsp3::NSIspio::Irq_t irq )
{
    UNUSED(irq);
    return 0;
}
