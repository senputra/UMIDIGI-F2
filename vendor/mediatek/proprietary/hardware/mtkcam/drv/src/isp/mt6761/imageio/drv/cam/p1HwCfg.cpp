...
#if (AF_MISMATCH_WORKAD == 1)
    //46B0~ 46D8
    m_pAFIspReg = (MUINT32*)malloc(sizeof(MUINT32)*18);
    if (m_pAFIspReg != NULL)
        ::memset(m_pAFIspReg,0,sizeof(MUINT32)*18);
    m_AFsize = 0;
    m_AFofset = 0;
#endif
    for (MUINT32 i = 0; i < CAM_MODULE_MAX; i++) {
        UnLock((CAM_MODULE_ENUM)i);
    }
...
HwRWCtrl::~HwRWCtrl(void)
{
    ...
    for (MUINT32 i = 0; i < CAM_MODULE_MAX; i++)
    {
        Mutex::Autolock lock(mLock[i]);
        if (NULL != m_hwAccessObj[i])
           {
              P1HWRW_DBG("[%p]delete m_hwAccessObj[%d]=%p",this, i, m_hwAccessObj[i]);
   ...
   MUINT32* pDrvShell)
{
    ...
    P1HWRW_DBG("HwRWCtrl::init: cq=%d, pPhyIspDrv=%p, m_pVirIspDrv=%p", cq, m_pPhyIspDrv, m_pVirIspDrv);
    ...
    char const* szCallerName)
{
   ...
   ObjAccessHW* handle = NULL;
    ...
    //Lock();
    //
    Mutex::Autolock lock(mLock[module]);
   ...
   {
        P1HWRW_ERR("%s:NotReady, m_CQ(%d), m_pPhyIspDrv=%p, m(%d)", szCallerName, m_CQ, m_pPhyIspDrv, module);
        //UnLock();
       return NULL;
   }
...
{
   P1HWRW_DBG("handle=%d, Caller=%s", (int)handle, szCallerName);
...
{
    P1HWRW_DBG("module=%d, en=%d, m_pVirIspDrv=%p", module, en, m_pVirIspDrv);
    IspDrv* _VirDrv = NULL;
    ...
    _VirDrv = this->m_pPhyIspDrv->getCQInstance(this->m_pPhyIspDrv->getRealCQIndex(this->m_CQ,this->m_BurstQNum,((this->m_DupCqIdx+i)%ISP_DRV_P1_CQ_DUPLICATION_NUM)));
        ...
        case CAM_ISP_SGG1:
             ...
             break;
        case CAM_ISP_SGG5:
             HWRWCTRL_WRITE_BIT(CAM_CTL_EN_P1, SGG5_EN);
             break;
        case CAM_ISP_SGG3:
             HWRWCTRL_WRITE_BIT(CAM_CTL_EN_P1, SGG3_EN);
             break;
        case CAM_ISP_FLK:
        ...
        case CAM_DMA_AAO:
             ...
             break;
        case CAM_DMA_AFO:
             HWRWCTRL_WRITE_BIT(CAM_CTL_EN_P1_DMA, AFO_EN);
            break;
        case CAM_DMA_ESFKO:
            if (en){
                m_EFSKOUser |= HWRWCTRL_EFSKO_EFSKO;
            }
            else{
                m_EFSKOUser &= (~HWRWCTRL_EFSKO_EFSKO);
            }
        ...
        case CAM_DMA_IMGO_SV:
            ISP_WRITE_REG(_VirDrv,CAMSV_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_INT_EN ,0x481,ISP_DRV_USER_ISPF);
            ISP_WRITE_BITS(_VirDrv,CAMSV_TG_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_MODULE_EN,40000015,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_PAK,0x03,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV_FMT_SEL,0x01000000, ISP_DRV_USER_ISPF);
            break;
        case CAM_DMA_IMGO_SV2:
            ISP_WRITE_REG(_VirDrv,CAMSV2_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV2_INT_EN ,0x481,ISP_DRV_USER_ISPF);
            ISP_WRITE_BITS(_VirDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV2_MODULE_EN,40000015,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV2_PAK,0x03,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_VirDrv,CAMSV2_FMT_SEL,0x01000000, ISP_DRV_USER_ISPF);
            break;
        ...
        case CAM_ISP_SGG1_D:
             ...
             break;
        case CAM_ISP_SGG5_D:
             HWRWCTRL_WRITE_BIT(CAM_CTL_EN_P1_D, SGG5_D_EN);
             break;
        case CAM_ISP_SGG3_D:
             HWRWCTRL_WRITE_BIT(CAM_CTL_EN_P1_D, SGG3_D_EN);
             break;
        case CAM_ISP_AF_D:
...
{
    P1HWRW_DBG("HwRWCtrl::SetModuleSel+ module=%d, val=%d, m_pVirIspDrv=%p", module, val, m_pVirIspDrv);
...
HwRWCtrl::finishModuleCfg(MINTPTR handle)
{
    ...
    Mutex::Autolock lock(mLock[accessObj->m_Module]);
...
#endif
    {
        ...
        if(mod == CAM_DMA_AAO) {
            ...
            AFO.dma_cfg.memBuf.ofst_addr = this->m_pVirtIspReg.CAM_AFO_OFST_ADDR.Raw;
            AFO.dma_cfg.size.h = this->m_pVirtIspReg.CAM_AFO_YSIZE.Raw + 1;
            AFO.dma_cfg.size.stride = this->m_pVirtIspReg.CAM_AFO_STRIDE.Raw;
...
#if (AF_MISMATCH_WORKAD == 1)
            memcpy((((MUINT8*)_VirDrv->getRegAddr()) + m_AFofset), \
                ((MUINT8*)(m_pAFIspReg)), sizeof(MUINT32) * m_AFsize);
#endif
            ...
            if (HWRWCTRL_DBG_DUMP) {
                ...
                P1HWRW_DBG("flushGroupHwsCfg2CQ: cqVirtReg=%p, Offset=0x%x, ptr=%p, size=%d", cqVirtReg, accessObj->m_iOffset, iDstPtr, accessObj->m_iSize);
                if(1){
                    if (CAM_ISP_OBC == mod) {
                        P1HWRW_DBG("==============OBC Dbg=================");
   ...
   IspDrv* pTarVirDrv)//Dst
{
    ...
    {
        //error parameter
        P1HWRW_ERR("_flushHwCfg2CQ: (0 == accessObj->m_iSize(%d)) || ((accessObj->m_iSize*4)(0x%x) > sizeof(isp_reg_t))(%d)",accessObj->m_iSize, (accessObj->m_iSize*4), (int)sizeof(isp_reg_t));
        return MFALSE;
    }
...
#else
#if (AF_MISMATCH_WORKAD == 1)
    if (accessObj->m_Module != CAM_ISP_AF)
#endif
...
#if (AF_MISMATCH_WORKAD == 1)
    if (accessObj->m_Module == CAM_ISP_AF) {
        P1HWRW_DBG("mod=%d, offset=0x%x, size=0x%x", accessObj->m_Module, accessObj->m_iOffset, accessObj->m_iSize);
        m_AFsize = accessObj->m_iSize;
        m_AFofset = accessObj->m_iOffset;
        memcpy((MUINT8*)m_pAFIspReg, (((MUINT8*)(&m_pVirtIspReg)) + (accessObj->m_iOffset)),
            (accessObj->m_iSize) * sizeof(MUINT32));
    }
#endif
    // 2. Add Cq-Module
...
HwRWCtrl::reFlushPendingCfgToCQ(void)
{
    ...
    for (int i=0; i < m_vUpdatedHwMod.size();i++)
    {
        ...
        if (accessObj == NULL) {
            P1HWRW_DBG("flushGroupHwsCfg2CQ+, (NULL == accessObj)");
            return MFALSE;
        }
        mod = accessObj->m_Module;
        if (mod > CAM_MODULE_MAX) {
            P1HWRW_DBG("flushGroupHwsCfg2CQ+, (mod > CAM_MODULE_MAX)");
            return MFALSE;
        }
...
ObjAccessHW::~ObjAccessHW(void){
    P1HWRW_DBG("~ObjAccessHW, this=%p, sz=%d", this, (int)sizeof(ObjAccessHW));
...
ObjAccessHW::initModuleRegVal(void)
{
    ...
    if (HWCFG_STATUS_NONE == getCfgStatus()) {
        ...
        P1HWRW_DBG("m_pVirtIspReg=%p, m_iOffset=0x%x", (void *)&m_pHwRWCtrl->m_pVirtIspReg, m_iOffset);
