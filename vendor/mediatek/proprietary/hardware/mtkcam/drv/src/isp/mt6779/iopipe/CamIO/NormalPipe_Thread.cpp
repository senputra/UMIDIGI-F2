...
MINT32 NormalPipe_Thread::enque(QBufInfo* pQBuf)
{
    ...
    if (EPortIndex_CAMSV_IMGO == portID.index)
        imgHdrBufNum = IMG_HEADER_BUF_NUM * m_pNormalpipe->mBurstQNum * m_pNormalpipe->mSVRawBurstNum;
    else
        imgHdrBufNum = IMG_HEADER_BUF_NUM * m_pNormalpipe->mBurstQNum;
    _idx = m_pNormalpipe->mImgHdrIdx[_port] % imgHdrBufNum;

    if (CHECK_FMT_MISMATCH(portID.index, m_pNormalpipe->GetOpenedPortInfo(NormalPipe::_GetPort_ImgFmt,_port), ImgFormat) == MFALSE) {
        PIPE_ERR("port_%d enque UF/PAK fmt mismatch with configpipe phrase(0x%x/0x%x)",
            portID.index, m_pNormalpipe->GetOpenedPortInfo(NormalPipe::_GetPort_ImgFmt,_port), ImgFormat);
        return -1;
    }
    ...
    for (i = 0; i < _size; i++, _idx++) {
        ...
        if ((CHECK_FMT_UF(ImgFormat)) || (CHECK_FMT_YUV_3_PLANE(ImgFormat)))
            PlaneMax = (MUINT32)ePlane_3rd;
        else if(CHECK_FMT_YUV_2_PLANE(ImgFormat))
        ...
        if (mpSecMgr && pQBuf->mvOut.at(i).mSecon) {
            mpSecMgr->SecMgr_QuerySecMVAFromHandle((MUINT32)pQBuf->mvOut.at(i).mBuffer->getBufPA(0),&SecPA,(MUINT64)pQBuf->mvOut.at(i).mChk_handle);
    ...
    if (ePort_svimgo == _port) {
        if (m_pNormalpipe->mpCamSVIOPipe) {
            ret = m_pNormalpipe->mpCamSVIOPipe->enqueOutBuf(portID, rQBufInfo, 1);
            // Normalsv ret = 0: fail, ret = 1: ok
            // Covert NormalSV error code to normalPipe error code
            if(0 == ret)
                ret = -1;
            else
                ret = 0;
            if (ret < 0) {
                PIPE_ERR("error:SV enqueOutBuf");
                return -1;
            }
            else if(ret > 0) {
        ...
        if (this->m_pNormalpipe->mpCamIOPipe) {
            if (ePort_rawi == _port)
                ret = this->m_pNormalpipe->mpCamIOPipe->enqueInBuf(portID, rQBufInfo);
            else
                ret = this->m_pNormalpipe->mpCamIOPipe->enqueOutBuf(portID, rQBufInfo, 1);
            ...
            if (ret < 0) {
                ...
                for (i = 0; i < _size; i++) {
                    ...
                    if (mpSecMgr && pQBuf->mvOut.at(i).mSecon)
                        ...
                        m_pNormalpipe->mpFrameMgr->enque(pQBuf->mvOut.at(i).mBuffer,0);
                        //  Virtual port , camio would not return PA, causing FrameMgr cannot deque
                        // Keep PA here for further flow remains the same
                        if (NPIPE_CHECK_IS_VIRTUAL(_port))
                            m_pNormalpipe->mpVirPortPA[_port]->sendCmd(QueueMgr<MUINT32>::eCmd_push, (MUINTPTR)&rQBufInfo.vBufInfo.at(i).u4BufPA[ePlane_1st], 0);
                    }
                    #endif
                }
            }
...
MBOOL NormalPipe_Thread::dmaCfg(QBufInfo* pQBuf)
{
    ...
    case EPortIndex_CRZO:
        _arg1 = (MINTPTR)&listImgCrop;
        _arg2 = (MINTPTR)&listImgRsz;
        _cmd = EPIPECmd_SET_CRZOR1;
        break;
    case EPortIndex_CRZO_R2:
        _arg1 = (MINTPTR)&listImgCrop;
        _arg2 = (MINTPTR)&listImgRsz;
        _cmd = EPIPECmd_SET_CRZOR2;
        break;
    case EPortIndex_YUVO:
        _arg1 = (MINTPTR)&listImgCrop;
        _arg2 = (MINTPTR)&listImgRawType;
        _cmd = EPIPECmd_SET_YUVO;
        break;
    ...
    case EPortIndex_RAWI:
        if (NormalPipe::eCamDataPath_DCIF == m_pNormalpipe->m_dataPath) {
            goto _BYPASS_DMA_CFG;
        }
        else {
            _arg1 = (MINTPTR)&listImgCrop;
            _arg2 = (MINTPTR)&listImgRawType;
            _cmd = EPIPECmd_SET_RAWI;
        }
        break;
    default:
    ...
    for (i = 0; i < pQBuf->mvOut.size(); i++) {
        ...
        if ((EPortIndex_IMGO == port.index) || (EPortIndex_RRZO == port.index) ||
            (EPortIndex_CRZO == port.index) || (EPortIndex_CRZO_R2 == port.index) ||
            (EPortIndex_YUVO == port.index) || (EPortIndex_RAWI == port.index)) {
            ...
            switch(port.index){
                case EPortIndex_IMGO://only for IMGO dynamic UFO/PAK/UNPAK
                    ...
                    break;
                case EPortIndex_YUVO://only for YUVO dynamic format change
                    imgCrop.enqueue_img_stride = pBuf->mBuffer->getBufStridesInBytes(ePlane_1st);
                    break;
                default:
                    break;
        ...
        if ((EPortIndex_IMGO == port.index) || (EPortIndex_YUVO == port.index) || (EPortIndex_RAWI == port.index)) {
            listImgRawType.push_back(pBuf->mRawOutFmt);
        }
        if ((EPortIndex_RRZO == port.index) || (EPortIndex_CRZO == port.index) || (EPortIndex_CRZO_R2 == port.index)) {
    ...
    if (m_pNormalpipe->mpCamIOPipe) {
        ...
        if (MFALSE == ret) {
            PIPE_ERR("Error: EPIPECmd_SET_RRZ/IMGO/YUVO/CRZO_R1/CRZO_R2/RAWI(0x%x) Fail\n",_cmd);
...
E_EnqueRst NormalPipe_Thread::frameToEnque(void)
{
    ...
    MUINT32         i, _port = 0, sofIdx = 0, _OpenedPort , AvailableBuf = 0;
    MBOOL           ret = MTRUE;
    MINT32          EqRet;
    ...
    MUINT32 result = MFALSE;
    MBOOL bWait = MFALSE;

    _OpenedPort = m_pNormalpipe->GetOpenedPortInfo(NormalPipe::_GetPort_OpendNum);

    CAM_CAPIBILITY* pinfo = CAM_CAPIBILITY::CreateInsatnce(LOG_TAG);
    if(pinfo->GetCapibility(_port, ENPipeQueryCmd_IQ_SW_VER, qryInput, queryInfo) == MFALSE){
        PIPE_ERR("capability query sw version op fail\n");
        return eEnqueRst_NG;
    }
    ...
    for(i = 0; i < _OpenedPort; i++) {
        ...
        if ((NormalPipe::eCamDataPath_DCIF == m_pNormalpipe->m_dataPath) && (ePort_svimgo == _port)) {
            if (AvailableBuf < m_pNormalpipe->mSVRawBurstNum) {
                PIPE_ERR("N:%d dma:0x%x imgheader buf empty(available:%d, BurstQNum:%d)",
                    ...
                    m_pNormalpipe->mImgHdrCnt[_port], m_pNormalpipe->mSVRawBurstNum);
    ...
    if(capibility::D_IQ_Version::IQ_VER_2 == queryInfo.IQ_SW_Ver){
        if (NormalPipe::BufCtrl_switch_phase1 == NormalPipe::BfcFSM[NormalPipe::getBfcIdx(cam_path)].mFsm) {
            if (MFALSE == check_switch_phase1(&result)){
                retRst = eEnqueRst_Skip;
                goto _ENQ_DROP;
            }
            else {
                if (result) {
                    // switch io
                    NormalPipe::NIOGLock.lock();
                    this->m_pNormalpipe->sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_SWITCH_NLINK, 0, 0, 0);

                    // under switch case, need to wait for another cam finish all deque
                    bWait = MTRUE;

                    // CB to STT
                    if (this->switchNotifyCB.fpNotifyState) {
                        if(this->switchNotifyCB.fpNotifyState(CAM_ENTER_SWITCH, this->switchNotifyCB.Obj)
                            == NOTIFY_FAIL){
                            PIPE_ERR("N:%d switch fail", this->m_pNormalpipe->mpSensorIdx);
                            NormalPipe::NIOGLock.unlock();
                            return eEnqueRst_NG;
                        }
                    }
                    m_pNormalpipe->mpCamIOPipe = m_pNormalpipe->GetCamIOPipeInstance(m_pNormalpipe->mpSensorIdx);
                    NormalPipe::NIOGLock.unlock();
                }
                else {
                    m_pNormalpipe->sendCommand(ENPipeCmd_GET_CUR_SOF_IDX,(MINTPTR)&sofIdx,0,0);
                    PIPE_WRN("Raw switch stage_1 is not ready senIdx(%d),cursof(%d)\n", m_pNormalpipe->mpSensorIdx,sofIdx);
                    retRst = eEnqueRst_Skip;
                    goto _ENQ_DROP;
                }
            }
    ...
    if (_OpenedPort > 0) {
        _port = NPIPE_MAP_PORTID(m_pNormalpipe->GetOpenedPortInfo(NormalPipe::_GetPort_Index, 0));
        m_pNormalpipe->mpEnqueRequest[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_front, (MUINTPTR)&_qBuf, 0);
        m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_SET_DBLOAD_HOLD, MTRUE, (MINTPTR)&_qBuf.mvOut.at(0).FrameBased.mMagicNum_tuning, (MINTPTR)&m_pNormalpipe->mpSensorIdx);
    }

    // IQ Level control by DTwin in 50
    if (capibility::D_IQ_Version::IQ_VER_1 == queryInfo.IQ_SW_Ver) {
    ...
    // IQ Level control by cam_switch in 60
    else if (capibility::D_IQ_Version::IQ_VER_2 == queryInfo.IQ_SW_Ver) {
        switch(NormalPipe::BfcFSM[NormalPipe::getBfcIdx(cam_path)].mFsm)
        {
            case NormalPipe::BufCtrl_switch_set:
                {
                    NormalPipe::updateBufCtrlFSM(NormalPipe::BufCtrl_switch_phase1, cam_path);
                    configEnquePath(NSImageio::NSIspio::ICamIOPipe::CAMIO_SWITCH);
                }
                break;
            case NormalPipe::BufCtrl_switch_phase1:
                {
                    NormalPipe::updateBufCtrlFSM(NormalPipe::BufCtrl_switch_phase2, cam_path);
                    configEnquePath(NSImageio::NSIspio::ICamIOPipe::CAMIO_SWITCH);
                }
                break;
            case NormalPipe::BufCtrl_switch_phase2:
                {
                    NormalPipe::updateBufCtrlFSM(NormalPipe::BufCtrl_switch_done, cam_path);
                    configEnquePath(NSImageio::NSIspio::ICamIOPipe::CAMIO_SWITCH);
                }
                break;
            default:
       ...
       PIPE_ERR("IQ version not supported!\n");
       retRst = eEnqueRst_NG;
       goto _ENQ_DROP;
    }
    ...
    for(i = 0; i < _OpenedPort; i++) {
        ...
        if (EqRet != 0) {
            ...
            m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_SET_DBLOAD_HOLD, MFALSE, 0, 0);
            goto _ENQ_DROP;
        }
        ...
        if (0 == listMagic.size()) {
            MUINT32 m, mag;
            if (1 < m_pNormalpipe->mBurstQNum) {
                mag = _qBuf.mvOut.at(0).FrameBased.mMagicNum_tuning;
                for (m = 1; m < _qBuf.mvOut.size(); m++) {
                    if (mag != _qBuf.mvOut.at(m).FrameBased.mMagicNum_tuning) {
                        PIPE_ERR("Magic are not consistent(%d/%d), idx(%d)\n",
                            mag, _qBuf.mvOut.at(0).FrameBased.mMagicNum_tuning, m);
                        retRst = eEnqueRst_NG;
                        m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_SET_DBLOAD_HOLD, MFALSE, 0, 0);
                        goto _ENQ_DROP;
                    }
                }
                listMagic.push_back(mag);
            }
            else {
    ...
    if (NormalPipe::eCamDataPath_DCIF == m_pNormalpipe->m_dataPath) {
        m_pNormalpipe->mpCamSVIOPipe->sendCommand(EPIPECmd_SET_FRM_TIME, (MINTPTR)(_qBuf.mShutterTimeNs/1000000), 0, 0);
    }
    ...
    NormalPipe::NIOGLock.lock();
    m_pNormalpipe->PushDeqCamIOInstance(m_pNormalpipe->mpSensorIdx, m_pNormalpipe->mpCamIOPipe, bWait);
    NormalPipe::NIOGLock.unlock();
...
E_EnqueRst NormalPipe_Thread::configEnquePath(NSImageio::NSIspio::ICamIOPipe::E_FRM_EVENT frameEvent)
{
    ...
    do { /* for critical section of class static member */
        ...
        else if (CAM_TG_2 == _NORMALPIPE_GET_TG_IDX(this->m_pNormalpipe->mpSensorIdx)) {
            _path = NPIPE_CAM_B;
        }
        else {
            _path = NPIPE_CAM_C;
        ...
        else if (NSImageio::NSIspio::ICamIOPipe::CAMIO_SWITCH == frameEvent) {
            cam_path = NPIPE_MAP_PATH(this->m_pNormalpipe->mpSensorIdx);
            // only 1st time set CBFP
            if (NormalPipe::BufCtrl_switch_phase1 == NormalPipe::BfcFSM[NormalPipe::getBfcIdx(cam_path)].mFsm) {
                this->m_pNormalpipe->mpCamIOPipe->sendCommand(EPIPECmd_SET_SWITCH_CBFP, (MINTPTR)NormalPipe::switchDoneCB, 0, 0);
            }
        }
    } while(0);
    ...
    if (NSImageio::NSIspio::ICamIOPipe::CAMIO_SWITCH == frameEvent) {
        cam_path = NPIPE_MAP_PATH(this->m_pNormalpipe->mpSensorIdx);
        // for done collection setting, need to know another tg index
        if(NormalPipe::BufCtrl_switch_phase1 == NormalPipe::BfcFSM[NormalPipe::getBfcIdx(cam_path)].mFsm){
            this->m_pNormalpipe->sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_GET_SWITCH_NLINK, (MINTPTR)&this->m_pNormalpipe->mpCamIOPipe, (MINTPTR)&plinkInfo, 0);
            wt_info.TgIdx = plinkInfo->mCurTgIdx;
        }
        else{
            this->m_pNormalpipe->sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_GET_CUR_NLINK, (MINTPTR)&this->m_pNormalpipe->mpCamIOPipe, (MINTPTR)&plinkInfo, 0);
            wt_info.TgIdx = plinkInfo->mTgIdx;
        }
        // use cur nlink infomation
        this->m_pNormalpipe->sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_GET_CUR_NLINK, (MINTPTR)&this->m_pNormalpipe->mpCamIOPipe, (MINTPTR)&plinkInfo, 0);
        wt_info.TgSize.w        = plinkInfo->mTgSize.w;
        wt_info.TgSize.h        = plinkInfo->mTgSize.h;

        wt_info.pix_mode_tg     = plinkInfo->pix_mode_tg;
        wt_info.raw_fps         = plinkInfo->raw_fps;
        wt_info.SrcImgFmt       = plinkInfo->SrcImgFmt;
        wt_info.bypass_rrzo     = plinkInfo->bypass_rrzo;
        wt_info.rrz_out_w       = plinkInfo->rrz_out_w;
        wt_info.offBin          = plinkInfo->offBin;
        wt_info.offTwin         = plinkInfo->offTwin;
        wt_info.pattern         = plinkInfo->pattern;
        wt_info.MIPI_pixrate    = plinkInfo->MIPI_pixrate;

        this->m_pNormalpipe->mpCamIOPipe->configFrame(frameEvent, func, &wt_info);
    }
    else {
...
MVOID NormalPipe_Thread::enqueRequest(MUINT32 sof_idx)
{
        ...
        if (reqQueNum > 0) {
            ...
            for (i = 0; i < _OpenedPort; i++) {
                ...
                for (j = 0; j < enqQBuf.mvOut.size(); j++) {
                    PIPE_DBG("N:%d drop dma:x%x PA:(0x%" PRIXPTR ") M:%d isUFO(%d) isYUV2P(%d) isYUV3P(%d)",
                        ...
                        CHECK_FMT_YUV_2_PLANE(enqQBuf.mvOut.at(j).mBuffer->getImgFormat()),
                        CHECK_FMT_YUV_3_PLANE(enqQBuf.mvOut.at(j).mBuffer->getImgFormat()));
    ...
    // drop request checked by frameToEnque()
    if (enqRst == eEnqueRst_Skip) {
        ...
        PIPE_INF("N:%d drop request size:%d mag_num[%d~%d] because the drop return checked by frameToEnque().", m_pNormalpipe->mpSensorIdx,\
            (MUINT32)m_DropEnQ.size(), m_DropEnQ.at(0), m_DropEnQ.at(m_DropEnQ.size()-1));
    }
...
#define NPIPE_MAX_DEQ_COUNT     (3)
    ...
    MUINT32 _size = 0, _OpenedPort = 0, idx = 0, i, PlaneMax = 0, _port = 0, imgHdrBufNum = 0, lockIdx = 0;
    ...
    BufCtrlCenter::NLinkInfo*               plinkInfo = NULL;
    ...
    // pop CamIO instance
    // under switch case, need to wait for another cam finish all deque
    // after get camio instance, use NLink to get lock idx
    // in order to avoid same CamIO instance racing under switch case
    // critical timing: Sen2 using CamIO2 deque not finish, Sen1 switch happens and using CamIO2 deque as well, causing ambiguous
    // solution: use central lock to avoid
    while(1){
        NormalPipe::NIOGLock.lock();
        pCamIOPipe = this->m_pNormalpipe->PopDeqCamIOInstance(this->m_pNormalpipe->mpSensorIdx);
        if(pCamIOPipe){
            this->m_pNormalpipe->sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_GET_CUR_NLINK, (MINTPTR)&pCamIOPipe, (MINTPTR)&plinkInfo, 0);
            lockIdx = plinkInfo->mLockIdx;
            NormalPipe::BfcDeqNPipelock[lockIdx].lock();
            NormalPipe::NIOGLock.unlock();
            break;
        }
        NormalPipe::NIOGLock.unlock();

        PIPE_INF("under switch case, need to wait another cam finish all deque");
        usleep(100);
    }
    ...
    for (i = 0; i < _OpenedPort; i++) {
        ...
        for (deqTryCount = NPIPE_MAX_DEQ_COUNT; deqTryCount > 0; deqTryCount--) {
            if (PORT_CAMSV_IMGO.index == portID.index) {
                if((deqRet = m_pNormalpipe->mpCamSVIOPipe->dequeOutBuf(portID, rQTSBufInfo, NPIPE_DEQUE_TIMEOUT, &notifyObj)) \
                    != eBuf_Fail) {
                    break;
                }
            }
            else if (PORT_RAWI.index == portID.index){
                if((deqRet = m_pNormalpipe->mpCamIOPipe->dequeInBuf(portID, rQTSBufInfo, NPIPE_DEQUE_TIMEOUT)) \
                    != eBuf_Fail) {
                    break;
                }
            }
            else {
                if((deqRet = pCamIOPipe->dequeOutBuf(portID, rQTSBufInfo, NPIPE_DEQUE_TIMEOUT, &notifyObj)) \
                    != eBuf_Fail) {
                    break;
                }
            }
            ...
            if (m_pNormalpipe->mResetCount > 0) {
                ...
                for (; retry > 0; retry--) {
                    ...
                    if (PORT_CAMSV_IMGO.index == portID.index) {
                        if ((deqRet = m_pNormalpipe->mpCamSVIOPipe->dequeOutBuf(portID, rQTSBufInfo, NPIPE_DEQUE_TIMEOUT, &notifyObj)) != eBuf_Fail) {
                            m_pNormalpipe->mResetCount = 0;
                            break;
                        }
                    }
                    else {
                        if ((deqRet = pCamIOPipe->dequeOutBuf(portID, rQTSBufInfo, NPIPE_DEQUE_TIMEOUT, &notifyObj)) != eBuf_Fail) {
                            m_pNormalpipe->mResetCount = 0;
                            break;
                        }
                    }
                ...
                if (0 == retry) {
                    ...
                    NormalPipe::BfcDeqNPipelock[lockIdx].unlock();
                    return -1;
                }
        ...
        switch(deqRet){
            ...
            case eBuf_stopped:
                ...
                break;
            case eBuf_dummy:
                rQTSBufInfo.vBufInfo.resize(m_pNormalpipe->mBurstQNum);
                rQTSBufInfo.vBufInfo.at(0).DstSize = 0;
                break;
            default:
                break;
        }
        ...
        if(vMnum.size() == 0) {//if size != 0 => suspending
            ...
            // DCIF, svimgo need to deque size as mSVRawBurstNum
            if ((NormalPipe::eCamDataPath_DCIF == m_pNormalpipe->m_dataPath) && (PORT_CAMSV_IMGO.index == portID.index)) {
                if(rQTSBufInfo.vBufInfo.size() != m_pNormalpipe->mBurstQNum*m_pNormalpipe->mSVRawBurstNum) {
                    PIPE_ERR("N:%d dma:x%x deque buf num error, %d/%d/%d", m_pNormalpipe->mpSensorIdx,\
                            portID.index, (MUINT32)rQTSBufInfo.vBufInfo.size(), m_pNormalpipe->mBurstQNum, m_pNormalpipe->mSVRawBurstNum);
                    NormalPipe::BfcDeqNPipelock[lockIdx].unlock();
                    return -1;
                }
            }
            ...
            for (idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++) {
                ...
                MBOOL           isYUV2P = CHECK_FMT_YUV_2_PLANE((MINT)rQTSBufInfo.vBufInfo.at(idx).img_fmt);
                MBOOL           isYUV3P = CHECK_FMT_YUV_3_PLANE((MINT)rQTSBufInfo.vBufInfo.at(idx).img_fmt);
                ...
                if (pUFDG) {
                    ...
                    PIPE_INF("UFDG_META_INFO:dma(0x%x),isUF(%d),YUV2P(%d),YUV3P(%d),fmt(0x%x),OFST_ADDR(0x%x),AU_START(0x%x),AU2_SIZE(0x%x),BOND_MODE(0x%x)",
                         portID.index,
                         isUF,
                         isYUV2P,
                         isYUV3P,
                ...
                #ifdef USE_IMAGEBUF_HEAP
                // Get PA in NormalPipe deque if virtual port
                if (NPIPE_CHECK_IS_VIRTUAL(_port)) {
                    m_pNormalpipe->mpVirPortPA[_port]->sendCmd(QueueMgr<MUINT32>::eCmd_at, 0, (MUINTPTR)&rQTSBufInfo.vBufInfo.at(idx).u4BufPA[ePlane_1st]);
                    m_pNormalpipe->mpVirPortPA[_port]->sendCmd(QueueMgr<MUINT32>::eCmd_pop, 0, 0);
                }
...
MVOID* NormalPipe_Thread::ispEnqueThread(void *arg)
{
    ...
    case CAM_TG_2:  ::prctl(PR_SET_NAME,"IspEnqThd_TG2",0,0,0);
        break;
    case CAM_TG_3:  ::prctl(PR_SET_NAME,"IspEnqThd_TG3",0,0,0);
        break;
...
MVOID* NormalPipe_Thread::ispDequeThread(void *arg)
{
    ...
    case CAM_TG_2:  ::prctl(PR_SET_NAME,"IspDeqThd_TG2",0,0,0);
        break;
    case CAM_TG_3:  ::prctl(PR_SET_NAME,"IspDeqThd_TG3",0,0,0);
        break;
...
MBOOL NormalPipe_Thread::check_switch_phase1(MUINT32* result)
{
    MBOOL ret = MTRUE;
    ENPipe_CAM eCamPipe_0, eCamPipe_1;
    MUINT32 eLastP1Done_0, eLastP1Done_1;

    // mpNLinkInfo[0]: mTgIdx <- E_INPUT, CAM_TG_1/CAM_TG_2/CAM_TG_3
    // NPIPE_MAP_PATH to NPIPE_CAM_A/B/C
    // CAM_MAP_NPIPE to ENPipe_CAM_A/B/C
    CAM_MAP_NPIPE(NPIPE_MAP_PATH(this->m_pNormalpipe->mNLinkInfo[0].mpSenIdx), eCamPipe_0);
    CAM_MAP_NPIPE(NPIPE_MAP_PATH(this->m_pNormalpipe->mNLinkInfo[1].mpSenIdx), eCamPipe_1);

    ret = this->m_pNormalpipe->mNLinkInfo[0].mpCamIO->sendCommand(EPIPECmd_GET_SWITCH_STATUS, (MINTPTR)&eLastP1Done_0, 0, 0);
    if (MFALSE == ret) {
        PIPE_ERR("send cmd check switch status fail!\n");
        return ret;
    }
    ret = this->m_pNormalpipe->mNLinkInfo[1].mpCamIO->sendCommand(EPIPECmd_GET_SWITCH_STATUS, (MINTPTR)&eLastP1Done_1, 0, 0);
    if (MFALSE == ret) {
        PIPE_ERR("send cmd check switch status fail!\n");
        return ret;
    }

    PIPE_INF("LastP1DoneRef: NPipe[%d]:%d NPipe[%d]:%d", eCamPipe_0, eLastP1Done_0, eCamPipe_1, eLastP1Done_1);
    if ((eLastP1Done_0 & (1L << (eCamPipe_0 - 1))) && (eLastP1Done_0 & (1L << (eCamPipe_1 - 1))) &&
        (eLastP1Done_1 & (1L << (eCamPipe_0 - 1))) && (eLastP1Done_1 & (1L << (eCamPipe_1 - 1)))) {
            *result = MTRUE;
    }
    else {
        *result = MFALSE;
