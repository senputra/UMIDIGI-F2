...
CamsvIOPipe* CamsvIOPipe::Create(MINT8 const szUsrName[32], E_INPUT& InPut)
{
    ...
    //new CamsvIOPipe object
    pCamsvIOPipe = new(std::nothrow) CamsvIOPipe();
    if(pCamsvIOPipe == NULL){
        BASE_LOG_ERR("new CamsvIOPipe fail\n");
        return NULL;
    }

    //ResMgr attach
    pCamsvIOPipe->m_pResMgr = ResMgr::Res_Attach(user_camsvio, master, (char *)szUsrName);
    if(pCamsvIOPipe->m_pResMgr == NULL){
        BASE_LOG_ERR("Res_attach fail.n");
        goto FAIL_EXIT;
    }

    switch(pCamsvIOPipe->m_pResMgr->Res_GetModule()){
        case CAMSV_0:
            pCamsvIOPipe->m_hwModule = CAMSV_0;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_0);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_0");
            InPut = TG_CAMSV_0;
            break;
        case CAMSV_1:
            pCamsvIOPipe->m_hwModule = CAMSV_1;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_1);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_1");
            InPut = TG_CAMSV_1;
            break;
        case CAMSV_2:
            pCamsvIOPipe->m_hwModule = CAMSV_2;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_2);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_2");
            InPut = TG_CAMSV_2;
            break;
        case CAMSV_3:
            pCamsvIOPipe->m_hwModule = CAMSV_3;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_3);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_3");
            InPut = TG_CAMSV_3;
            break;
        case CAMSV_4:
            pCamsvIOPipe->m_hwModule = CAMSV_4;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_4);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_4");
            InPut = TG_CAMSV_4;
            break;
        case CAMSV_5:
            pCamsvIOPipe->m_hwModule = CAMSV_5;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_5);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_5");
            InPut = TG_CAMSV_5;
            break;
        case CAMSV_6:
            pCamsvIOPipe->m_hwModule = CAMSV_6;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_6);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_6");
            InPut = TG_CAMSV_6;
            break;
        case CAMSV_7:
            pCamsvIOPipe->m_hwModule = CAMSV_7;
            std::strncpy((char*)pCamsvIOPipe->m_szUsrName, (char const*)szUsrName, sizeof(pCamsvIOPipe->m_szUsrName) - 1);
            pCamsvIOPipe->m_pIspDrvCamsv = (IspDrvCamsv*)IspDrvCamsv::createInstance(CAMSV_7);
            pCamsvIOPipe->m_pIspDrvCamsv->init("CamsvIOPipe CamSV_7");
            InPut = TG_CAMSV_7;
            break;
    ...
    BASE_LOG_INF("get input source:0x%x from CamsvResMgr\n", pCamsvIOPipe->m_hwModule);
    return pCamsvIOPipe;
FAIL_EXIT:
    if(NULL != pCamsvIOPipe->m_pResMgr){
        if(MFALSE == pCamsvIOPipe->m_pResMgr->Res_Detach()){
            BASE_LOG_ERR("Res_Detach fail.\n");
        }
        else{
            pCamsvIOPipe->m_pResMgr = NULL;
        }
    }
    if(NULL != pCamsvIOPipe){
        delete pCamsvIOPipe;
        pCamsvIOPipe = NULL;
    }
    return NULL;
...
MBOOL CamsvIOPipe::enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo,MBOOL bImdMode)
{
    ...
    for(MUINT32 i = 0; i < rQBufInfo.vBufInfo.size(); i++){
        ...
        if(MFALSE == this->m_FbcImgo.enqueueHwBuf(bufInfo)){
            PIPE_WRN("enque fail at burst:0x%x, m_FSM %d\n",i, this->m_FSM);
            ret = MFALSE;
        }
...
MBOOL CamsvIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, CAMIO_Func const *pInFunc, CAMIO_Func *pOutFunc)
{
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ ) {
        ...
        PIPE_INF("P1 vInPorts_%d:fmt(0x%x),PM(%d),w/h(%d_%d),crop(%d_%d_%d_%d),tg_idx(%d),dir(%d),fps(%d),MIPI_PixRate(%d),signal(%d_%d_%d),timeclk(0x%x)\n",\
                i, \
                vInPorts[i]->eImgFmt, \
                vInPorts[i]->ePxlMode,  \
