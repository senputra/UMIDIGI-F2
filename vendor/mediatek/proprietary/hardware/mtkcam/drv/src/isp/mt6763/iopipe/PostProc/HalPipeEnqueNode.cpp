    ...
    )
{
    ...
    for(int q=0;q<framePack.frameNum;q++)
    {
        ...
        for(MUINT32 i=0;i< qParam.mvFrameParams[q].mvIn.size() ;i++)
        {
            ...
            LOG_DBG("[mvIn (%d)] fmt(0x%x),w/h(%d/%d),oft(%d/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%x,0x%x,0x%x) ),pixelDivNum:%d,pixelMulNum:%d",\
                    ...
                    ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2], pixelDivNum, pixelMulNum);
        ...
        for(MUINT32 i=0;i< qParam.mvFrameParams[q].mvOut.size() ;i++)
        {
            ...
            LOG_DBG("[mvOut (%d)] fmt(0x%x),w/h(%d/%d),oft(%d/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%x,0x%x,0x%x) ),pixelDivNum:%d,pixelMulNum:%d",\
