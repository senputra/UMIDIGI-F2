    ...
    , m_iDequeFrmNum(0)
{
    ...
    m_iSaveReqToFile = ::property_get_int32("cam.dipsavefile.enable", 0);
    if (m_iSaveReqToFile > 0)
    {
        sprintf( dumppath, "%s/", DUMP_DIPPREFIX);
    ...
    //Add wpe
     switch(streamPipeID)
	 {
		case EStreamPipeID_Normal:
            ...
            LOG_INF("- E (New) mUCnt(%d), cUser(%s), pipeID(%d)",mInitCount,szCallerName, streamPipeID);
            if(mInitCount < MAX_PIPE_USER_NUMBER)
            {
                ...
                for(int i=0;i<mInitCount;i++)
                {
                    if(this->mUserInfo[i].pipeID==ESatremPipeID_None)
                    {
                        ...
                        strncpy((char*)(this->mUserInfo[i].name),szCallerName,strlen(szCallerName));
                ...
                if(!getspace)
                {
                    ...
                    strncpy((char*)(this->mUserInfo[mInitCount].name),szCallerName,strlen(szCallerName));
    		...
    		if(mInitCount==1)
    		{
                ...
                if (NULL ==this->mpPostProcPipe || !(this->mpPostProcPipe->init("HalPipeWrapper", secTag)))
        		{
	   ...
	   case EStreamPipeID_WarpEG:
        LOG_ERR("(%s_%d)DO NOT SUPPORT WPE...",szCallerName, streamPipeID);
        /*
             ...
             if(mWPE_InitCount < MAX_PIPE_WPEUSER_NUMBER)
             {
                ...
                for(int i=0;i<mWPE_InitCount;i++)
                {
                    if(this->mWPEUserInfo[i].pipeID==ESatremPipeID_None)
                    {
                        ...
                        strcpy((char*)(this->mWPEUserInfo[i].name),szCallerName);
                        getspace=true;
                        break;
                    }
                }
                ...
                if(!getspace)
                {
                    ...
                    strcpy((char*)(this->mWPEUserInfo[mWPE_InitCount].name),szCallerName);
                }
            }
            else{
			...
			{                
				...
				if (NULL ==this->mWpeStream || !(this->mWpeStream->init()))
        		{
    ...
    NSCam::NSIoPipe::EStreamPipeID streamPipeID)
{
	...
	switch(streamPipeID)
	{
        ...
        case EStreamPipeID_WarpEG:
            ...
            index = mInitCount = mWPE_InitCount;
    ...
    switch(streamPipeID)
    {
		...
		case NSCam::NSIoPipe::EStreamPipeID_WarpEG:
            ...
            /*
	 		...
	 		if(rParams.mvFrameParams.size() > 1)
    		{
        		LOG_ERR("WPE doesn't support multiple Enqueue requests,unexpected frameParas size(%d)", rParams.mvFrameParams.size());
        		ret = MFALSE;
   			}    		
	        else
	        {
				if(rParams.mvFrameParams[0].mvIn.size() < 1 || rParams.mvFrameParams[0].mvOut.size() < 1)
				{
					LOG_ERR("unexpected mvIn size(%d); mvOut size(%d)", rParams.mvFrameParams[0].mvIn.size(), rParams.mvFrameParams[0].mvOut.size());
					ret = MFALSE;
				}
                else
                {
					this->mWpeStream->WPEenque(rParams);		
                }
	        }
	        */
			break;
    ...
    MINT64 i8TimeoutNs)
{
    ...
    NSImageio::NSIspio::EPIPE_P2engine p2Eng;
    switch(streamPipeID)
    {
        case NSCam::NSIoPipe::EStreamPipeID_Normal:
            p2Eng=NSImageio::NSIspio::EPIPE_P2engine_DIP;
...
{
    //
    LOG_INF("+ thread type/property (%d_%d)",threadType,threadProperty);

    this->mpHalPipeWrapper=obj;
    memset(&mSemThread, 0, sizeof(mSemThread));
