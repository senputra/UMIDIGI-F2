...
/**
    AFO use CQ4 to control ring buffer, CQ4 is trigger by AAO_DON,
    but camb has no AAO_DON in twin mode, so we using cq8 (trigger by FLKO_DON).

    In ISP_DRV_CAM, m_2ndDma is used to distinguish between AFO port other dma ports,
    - MTURE: AFO
    - MFALSE: AAO(cq4)/FLKO(cq8)
*/

#define FAST_AF_TWIN_CQ_WORK_AROUND 1
...
typedef enum {
    ...
    CAM_DMA_PSO_BA,              // 2    4D80~4D84
    CAM_DMA_PSO_,                // 5    4D88~4D98
    ...
    CAM_DMA_FH_AAO_SPARE_SERIAL_,// 1    4cB4
    CAM_DMA_FH_AAO_SPARE_OB_,    // 1    4cB8
    CAM_DMA_FH_AAO_SPARE_LSC_,   // 1    4cBC
    CAM_DMA_FH_AAO_SPARE_,       // 11   4CBC~4CE8
    ...
    CAM_ISP_LSC_RATIO_0,         // 1    47E0
    CAM_ISP_LSC_RATIO_1,         // 1    47F0
...
class ISP_DRV_CAM : public IspDrvVir, public IspDrv
{
    ...
    public:
        ...
        virtual MUINT32     cqRingBuf(CQ_RingBuf_ST *pBuf_ctrl);
    ...
    protected:
        virtual MBOOL       updateEnqCqRingBuf(CQ_RingBuf_ST *pBuf_ctrl);
        virtual MUINT32*    update_FH_Spare(CQ_RingBuf_ST *pBuf_ctrl);
        virtual MBOOL       CQ_Allocate_method2(MUINT32 step);
