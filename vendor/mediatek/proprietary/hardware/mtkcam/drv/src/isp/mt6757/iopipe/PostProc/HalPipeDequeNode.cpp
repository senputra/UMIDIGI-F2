...
eraseNotify(
	...
	MUINT32 callerID)
{
    LOG_INF("+,tag(%d),idx(%d),p2CQ(%d/%d)",bufferListTag,index,p2CQ,p2dupCQidx);
    MBOOL ret=MTRUE;
    ...
    {  //erase frame package from list 
        ...
        LOG_DBG("Lsize(%d), index(%d)",this->mpHalPipeWrapper->mLDIPwaitDQFramePackList.size(),index);
        ...
        LOG_DBG("updateMCList(%d)",this->mpHalPipeWrapper->mLDIPwaitDoMDPStartUnitList.size());
        ...
        LOG_DBG("updateFUList(%d)",this->mpHalPipeWrapper->mLDIPwaitDQFrameUnitList.size());
        ...
        LOG_DBG("updateFPList(%d)",this->mpHalPipeWrapper->mLDIPwaitDQFramePackList.size());
...
dequeJudgement()
{
    ...
    LOG_INF("dequeCq(%d)/(%d)",dequeP2Cq,frameunit.p2cqIdx);
            ...
            case eIspRetStatus_VSS_NotReady:
                {
                    ...
                    {   //do other existed blocking buffer if vss is not ready
                        ...
                        LOG_INF("dCq_2 (%d)/(%d)",dequeP2Cq,frameunit.p2cqIdx);
                        if(frameunit.bufTag==EBufferTag_Vss)
                        {
    ...
    )
{
    ...
    for(int i = EPostProcPortIdx_DEPI ; i > EPostProcPortIdx_NOSUP ; i--)
    {
        if( frameunit.portEn & (1 << i))
        {
            portID.index=mPortIdxMappingP2toAll[i].ePortIndex;
    ...
    if(ret)
    {
        ...
        {
        	LOG_INF("HMyo(0x%x)_FmvOut(0x%x),dCq(%d/%d), idx(%d)",frameunit.callerID, this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[0].mBuffer->getBufPA(0),frameunit.p2cqIdx,frameunit.p2cqDupIdx,index);
        }
        else
        {
        ...
        if (this->mpHalPipeWrapper->m_iSaveReqToFile)
        {
            for (MUINT32 i = 0 ; i < this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut.size() ; i++ )
            {
                ...
                {
                    sprintf(filename, "%s/frm(%d)_vOuputPorts_[%d]_(0x%x)_w(%d)_h(%d)_stride(%d,0x0,0x0)_idx(%d)_dir(%d),Size(0x%08x),VA(0x%lx),PA(0x%lx).%s",DUMP_DIPPREFIX, 
                ...
                switch(this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[i].mPortID.index)
                {
                    case NSImageio::NSIspio::EPortIndex_WDMAO:
                        if (this->mpHalPipeWrapper->m_iSaveReqToFile & (DIP_DUMP_WDMA_BUFFER | DIP_DUMP_P2OUT_BUFFER))
                        {
                    ...
                    case NSImageio::NSIspio::EPortIndex_WROTO:
                        if (this->mpHalPipeWrapper->m_iSaveReqToFile & (DIP_DUMP_WROT_BUFFER | DIP_DUMP_P2OUT_BUFFER))
                        {
                            this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[i].mBuffer->saveToFile(filename);
                        }
                        break;
                    case NSImageio::NSIspio::EPortIndex_IMG2O:
                        if (this->mpHalPipeWrapper->m_iSaveReqToFile & (DIP_DUMP_IMG2O_BUFFER | DIP_DUMP_P2OUT_BUFFER))
                        {
                            this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[i].mBuffer->saveToFile(filename);
                        }
                        break;
                    case NSImageio::NSIspio::EPortIndex_IMG3O:
                        if (this->mpHalPipeWrapper->m_iSaveReqToFile & (DIP_DUMP_IMG3O_BUFFER | DIP_DUMP_P2OUT_BUFFER))
                        {
                            this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[i].mBuffer->saveToFile(filename);
                        }
                        break;
                    case NSImageio::NSIspio::EPortIndex_MFBO:
                        if (this->mpHalPipeWrapper->m_iSaveReqToFile & (DIP_DUMP_MFBO_BUFFER | DIP_DUMP_P2OUT_BUFFER))
                        {
                            this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[i].mBuffer->saveToFile(filename);
                        }
                        break;
        ...
        else
        {
            ...
            else
            {
                ...
                FramePackage framepack=this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index];
                this->mpHalPipeWrapper->releaseLock(ELockEnum_wDQFramePackList);   
                LOG_DBG("=go deque callback (0x%x)=",framepack.rParams.mpfnCallback);
                a=getUs();
