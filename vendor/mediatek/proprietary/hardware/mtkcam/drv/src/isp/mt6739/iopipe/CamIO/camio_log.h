...
5. Use following adb command to control debug log level:
    adb shell getprop vendor.debuglog.XXXXXXXXXX        => See current value of your group debug log. If there is nothing showing up, the property does not exist yet.
    adb shell setprop vendor.debuglog.XXXXXXXXXX ZZZ    => Set group debug log level. ZZZ can be one of the following value: 2(VERBOSE)/3(DEBUG)/4(INFO)/5(WARN)/6(ERROR)/7(ASSERT)/8(SILENT).
    adb shell getprop vendor.debuglog.XXXXXXXXXX.YYYYYYYYYY        => See current value of your module debug log. If there is nothing showing up, the property does not exist yet.
    adb shell setprop vendor.debuglog.XXXXXXXXXX.YYYYYYYYYY ZZZ    => Set module debug log level. ZZZ can be one of the following value: 2(VERBOSE)/3(DEBUG)/4(INFO)/5(WARN)/6(ERROR)/7(ASSERT)/8(SILENT).
...
#include <mtkcam/utils/std/ULog.h>
    ...
    #define BASE_LOG_VRB(fmt, arg...)       do { if (MTKCAM_LOGENABLE_DEFAULT) {CAM_ULOGD(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)	// <Verbose>: Show more detail debug information. E.g. Entry/exit of private function; contain of local variable in function or code block; return value of system function/API...
    #define BASE_LOG_DBG(fmt, arg...)       do { if (MTKCAM_LOGENABLE_DEFAULT) {CAM_ULOGD(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)	// <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
    #define BASE_LOG_INF(fmt, arg...)       do { if (MTKCAM_LOGENABLE_DEFAULT) {CAM_ULOGI(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg); } } while(0)	// <Info>: Show general system information. Like OS version, start/end of Service...
    #define BASE_LOG_WRN(fmt, arg...)       CAM_ULOGW(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] WARNING: " fmt NEW_LINE_CHAR, __func__, ##arg)	// <Warning>: Some errors are encountered, but after exception handling, user won't notice there were errors happened.
    #define BASE_LOG_ERR(fmt, arg...)       CAM_ULOGE(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s, %s, line%04d] ERROR: " fmt NEW_LINE_CHAR, __FILE__, __func__, __LINE__, ##arg)	// When MP, will only show log of this level. // <Fatal>: Serious error that cause program can not execute. <Error>: Some error that causes some part of the functionality can not operate normally.
    ...
    #define BASE_LOG_DBG(fmt, arg...)        {\
        ...
        VV_MSG(DBG_LOG_TAG "[%d][%d.%03d][%s] " fmt NEW_LINE_CHAR,gettid(),(tt1.tv_sec&1023),(tt1.tv_nsec/1000000), __func__, ##arg);\
    }
    ...
    */
  	#define BASE_LOG_ERR(fmt, arg...)        {\
        struct timespec tt1;\
        clock_gettime(CLOCK_REALTIME, &tt1);\
        VV_ERRMSG(DBG_LOG_TAG "[%s, %s, line%04d][%d][%d.%03d] ERROR: " fmt NEW_LINE_CHAR, __FILE__, __func__, __LINE__,gettid(),(tt1.tv_sec&1023),(tt1.tv_nsec/1000000), ##arg);\
    }
    #define BASE_LOG_AST(cond, fmt, arg...)     \
        do {        \
            if (!(cond))        \
...
#undef DIP_PATH_ERR
#undef DIP_PATH_AST
#define DIP_PATH_VRB(fmt, arg...)        do { if (pathDip_DbgLogEnable_VERBOSE) { BASE_LOG_VRB(fmt, ##arg); } } while(0)
#define DIP_PATH_DBG(fmt, arg...)        do { if (pathDip_DbgLogEnable_DEBUG  ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
#define DIP_PATH_INF(fmt, arg...)        do { if (pathDip_DbgLogEnable_INFO   ) { BASE_LOG_INF(fmt, ##arg); } } while(0)
#define DIP_PATH_WRN(fmt, arg...)        do { if (pathDip_DbgLogEnable_WARN   ) { BASE_LOG_WRN(fmt, ##arg); } } while(0)
#define DIP_PATH_ERR(fmt, arg...)        do { if (pathDip_DbgLogEnable_ERROR  ) { BASE_LOG_ERR(fmt, ##arg); } } while(0)
#define DIP_PATH_AST(cond, fmt, arg...)  do { if (pathDip_DbgLogEnable_ASSERT ) { BASE_LOG_AST(cond, fmt, ##arg); } } while(0)
