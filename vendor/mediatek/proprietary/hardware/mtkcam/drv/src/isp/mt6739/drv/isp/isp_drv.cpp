...
#include <drv/imem_drv.h>   // For IMemDrv*.
#include "isp_drv_imp.h"
#include <imageio/ispio_pipe_scenario.h>    // For enum EDrvScenario.
...
MINT32      IspDrv::mCurBurstQNum=P2_Support_BurstQNum;
...
//
    ...
    =   {
            ...
            {CAM_ISP_SGG5,           0x4590,  3     },
            {CAM_ISP_AWB,            0x45B0,  36   },
            {CAM_ISP_AE,             0x4650,  19    },
            ...
            {CAM_ISP_RRZ,            0x47A0,  12    },
            {CAM_ISP_BNR,            0x4800,  32    },
...
#endif
            {CAM_ISP_RRZ_D,          0x67A0,  12    },
            {CAM_ISP_BNR_D,          0x6800,  32    },
            {CAM_ISP_DBS_D,          0x6C00,  13    },
...
#endif
            {CAM_DMA_IMGI_CON3,         0x7500,  1     },
            {CAM_DMA_BPCI_CON3,         0x7504,  1     },
            {CAM_DMA_LSCI_CON3,         0x7508,  1     },
            {CAM_DMA_IMGO_CON3,         0x750C,  1     },
            {CAM_DMA_RRZO_CON3,         0x7510,  1     },
            {CAM_DMA_ESFKO_CON3,        0x7514,  1     },
            {CAM_DMA_AAO_CON3,          0x7518,  1     },
            {CAM_DMA_IMG2O_CON3,        0x751C,  1     },
            {CAM_DMA_BPCI_D_CON3,       0x7520,  1     },
            {CAM_DMA_LSCI_D_CON3,       0x7524,  1     },
            {CAM_DMA_AFO_D,             0x7528,  8     },
#if 0
 ...
 ISP_TURNING_FUNC_BIT_MAPPING gIspTuningFuncBitMapp[eIspTuningMgrFuncBit_Num]
    ={{eIspTuningMgrFuncBit_Dbs             , eTuningCtlByte_P1,  24, -1, -1, -1, -1, -1, CAM_ISP_DBS,          CAM_DUMMY_},
      ...
      {eIspTuningMgrFuncBit_Nsl2a           ,  eTuningCtlByte_P2, -1, -1, 31, -1, -1, -1, CAM_ISP_NSL2A,        CAM_DUMMY_}};
...
MINT32  IspDrv::getRealCQIndex(MINT32 cqBaseEnum,MINT32 burstQIdx,MINT32 dupCqIdx)
{
    ...
    {//pass2 cq
        ...
        realcqIdx=ISP_DRV_P1_PER_CQ_SET_NUM*cqNumInfoControl(ISP_DRV_CQNUMINFO_CTRL_GET_P1DUPCQNUM) + \
...
MBOOL IspDrv::cqAddModule(MINT32 cq,MINT32 burstQIdx,MINT32 dupCqIdx, CAM_MODULE_ENUM moduleId)
{
    int cmd;
    int realCQIdx=getRealCQIndex(cq,burstQIdx,dupCqIdx);
        LOG_DBG("- E. realCQIdx[%d],moduleId[%d],burstQIdx[%d],dupCqIdx[%d]",realCQIdx,(int)moduleId,burstQIdx,dupCqIdx);
    //
    Mutex::Autolock lock(cqVirDesLock);
...
MBOOL IspDrvImp::init(const char* userName)
{
    ...
    LOG_DBG("[m_pIMemDrv]:%p", (unsigned int*)m_pIMemDrv);    //Vent@20121107: Fix build warning: format '%x' expects argument of type 'unsigned int', but argument 5 has type 'IMemDrv*' [-Wformat]
    if ( NULL == m_pIMemDrv ) {
    ...
    //inital total cq nun
    mCurBurstQNum=P2_Support_BurstQNum;
    ...
    for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
        ...
        memset((MUINT8*)mTuningQueIdx[i].keepP1Que.pTuningQue,0,mTuningQueIdx[i].keepP1Que.queSize);
        LOG_INF("p2cq(%d):keepP2PreviewQue(0x%lx),keepP1Que(%p)",i,(unsigned long)(mTuningQueIdx[i].keepP2PreviewQue.pTuningQue),mTuningQueIdx[i].keepP1Que.pTuningQue);
        //
        mTuningQueIdx[i].isApplyTuning = MFALSE;
        mTuningQueIdx[i].pCurReadP1TuningQue = NULL;
        mTuningQueIdx[i].pCurReadP2TuningQue = NULL;
        ...
        for(int i=0;i<ISP_DRV_P2_CQ_NUM;i++){
            for(int j=0;j<ISP_TUNING_QUEUE_NUM;j++){
                if((mTuningQueInf[i][j].queFd>=0) && (mTuningQueInf[i][j].pTuningQue!=0)){
                    ...
                    LOG_ERR("[Error]free TuningQue error i(%d),j(%d),fd(%d),va(%p)",i,j,mTuningQueInf[i][j].queFd,mTuningQueInf[i][j].pTuningQue);
                }
            }
            // free keepP2PreviewQue
            if((mTuningQueIdx[i].keepP2PreviewQue.queFd>=0) && (mTuningQueIdx[i].keepP2PreviewQue.pTuningQue!=0)){
                ...
                LOG_ERR("[Error]free keepP2PreviewQue error i(%d),fd(%d),va(%p)",i,mTuningQueIdx[i].keepP2PreviewQue.queFd,mTuningQueIdx[i].keepP2PreviewQue.pTuningQue);
            }
            // free keepP1Que
            if((mTuningQueIdx[i].keepP1Que.queFd>=0) && (mTuningQueIdx[i].keepP1Que.pTuningQue!=0)){
                m_ispTmpBufInfo.size = mTuningQueIdx[i].keepP1Que.queSize;
...
MBOOL IspDrvImp::cqTableControl(ISP_DRV_CQTABLE_CTRL_ENUM cmd,int burstQnum)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQTABLE_CTRL_ALLOC:
            ...
            LOG_VRB("v(0x%x)(0x%x)(0x%zx)(0x%zx)(%p)(%p)",mIspVirRegSize,m_ispVirRegBufInfo.size, \
            ...
            LOG_DBG("cq(0x%x)(0x%x)(0x%zx)(0x%zx)(%p)(%p)",mIspCQDescSize,m_ispCQDescBufInfo.size, \
            ...
            for (int i = 0; i < mTotalCQNum; i++)
            {
                ...
                LOG_DBG("cq:[%d],mpIspCQDescriptor:Virt[%p]/Phy[%p],size/num(%d/%d/%u)",i,(unsigned int*)mpIspCQDescriptorVirt[i],mpIspCQDescriptorPhy[i],(unsigned int)(sizeof(ISP_DRV_CQ_CMD_DESC_STRUCT)),CAM_MODULE_MAX,sizeof(mIspCQDescInit));
            }
            ...
            if (sizeof(CQ_RTBC_RING_ST) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data too large(%u)>(%u)",sizeof(CQ_RTBC_RING_ST),sizeof(mIspCQDescInit));
                   return MFALSE;
            }
...
MINT32 IspDrvImp::cqNumInfoControl(ISP_DRV_CQNUMINFO_CTRL_ENUM cmd,int newValue)
{
    ...
    switch(cmd)
    {
        case ISP_DRV_CQNUMINFO_CTRL_GET_CURBURSTQNUM:   //get current supported burst queue number
            ...
            if(value != (unsigned int)mCurBurstQNum)
            {
        ...
        case ISP_DRV_CQNUMINFO_CTRL_UPDATE_TOTALCQNUM:          //update total cq number including p1 cq and p2 cq
            mTotalCQNum= (ISP_DRV_P1_CQ_DUPLICATION_NUM * ISP_DRV_P1_PER_CQ_SET_NUM)\
            + mCurBurstQNum * ((ISP_DRV_P2_CQ_DUPLICATION_NUM * ISP_DRV_P2_PER_CQ_SET_NUM));
            break;
...
//QQ    LOG_INF("CAM_FEO_ERR_STAT=0x%08x",readReg((((UINT32)&ispReg.CAM_FEO_ERR_STAT-(UINT32)&ispReg)), ISP_DRV_USER_ISPF));
    ...
    LOG_INF("[Tpipe]va(0x%08lx),pa(0x%08lx),regPa(0x%08lx)",(unsigned long)(pP2Package->tpipeTableVa),(unsigned long)(pP2Package->tpipeTablePa),(unsigned long)(regTpipePA));
    ...
    for(i=0x7000;i<=0x8000;i+=20){
        LOG_INF("(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)(0x%04x,0x%08x)",
            i,readReg(i, ISP_DRV_USER_ISPF),i+4,readReg(i+4, ISP_DRV_USER_ISPF),i+8,readReg(i+8, ISP_DRV_USER_ISPF),i+12,readReg(i+12, ISP_DRV_USER_ISPF),i+16,readReg(i+16, ISP_DRV_USER_ISPF));
    }

    // Check HW rule
	MUINT32 regCtlEnP1;	// 0x15004004
	MUINT32 regCtlEnP2;	// 0x15004018
	MUINT32 regBnrBpc; // 0x15004800
	MUINT32 regAnr1Con1; // 0x15007B00
	MUINT32 nsl2aEn, nbcEn, bpc_En, bnr_En;
	MUINT32 anr1_LceLink;

	regCtlEnP1 = readReg(0x4004, ISP_DRV_USER_ISPF);
	regCtlEnP2 = readReg(0x4018, ISP_DRV_USER_ISPF);
	regBnrBpc = readReg(0x4800, ISP_DRV_USER_ISPF);
	regAnr1Con1 = readReg(0x7A00, ISP_DRV_USER_ISPF);
	LOG_INF("regCtlEnP2(0x%08x) regAnr1Con1(0x%08x) regCtlEnP1(0x%08x)",regCtlEnP2,regAnr1Con1,regCtlEnP1);

	nsl2aEn = (regCtlEnP2&0x80000000)>>31;
	nbcEn = (regCtlEnP2&0x00000800)>>11;
	bnr_En = (regCtlEnP1&0x00000020)>>5;
	bpc_En = (regBnrBpc&0x00000001);
	anr1_LceLink = (regAnr1Con1&0x01000000)>>24;
	LOG_INF("NSL2A(%x) NBC(%x)",nsl2aEn,nbcEn);
	LOG_INF("anr1_LceLink(%x) ",anr1_LceLink);
	LOG_INF("BNR(%x) regBnrBpc(0x%08x) BPC(%x)",bnr_En,regBnrBpc,bpc_En);

	if ((nbcEn == 1) && (nsl2aEn == 0))
	{
		if (anr1_LceLink == 1)
			LOG_ERR("For NBC enable and NSL2A disable, ANR_CON1.ANR1_LCE_LINK must be 0!");
	}
	if ((bnr_En == 1) && (bpc_En == 0))
	{
		LOG_ERR("For BNR enable, BPC must be enable!");
	}
...
MBOOL IspDrvImp::dumpCQTable(ISP_DRV_CQ_ENUM cq, MINT32 burstQIdx,MUINT32 dupCqIdx, ISP_DRV_CQ_CMD_DESC_STRUCT* cqDesVa, MUINT32* cqVirVa)
{
    ...
    LOG_INF("+,realCQIdx(%d),cq(%d),burstQIdx(%d),dupCqIdx(%d),cqDesVa(%p),cqVirVa(%p)",realCQIdx,cq,burstQIdx,dupCqIdx,(unsigned int*)cqDesVa,cqVirVa);
...
MBOOL IspDrvImp::waitIrq(ISP_DRV_WAIT_IRQ_STRUCT* pwaitIrq)
{
    ...
    if (waitirq.UserInfo.UserKey == m3AUserKey)
        LOG_INF("3A(%d) ret(%d)", m3AUserKey, Ret);
    Tb=dbgTmr.getUs();  //us
    ...
    {   //change to non-clear wait and start to wait again
        if((MINT32)pwaitIrq->UserInfo.UserKey >= 0 ) //v3 used
        {
            LOG_INF("ioctrl Ret(%d),Type(%d),Status(0x%08x),UserKey(0x%08x),Timeout(%d),Tb-Ta(%d us).",
            Ret, pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->UserInfo.UserKey, pwaitIrq->Timeout,Tb-Ta);
        } else {
        ...
        while((waitirq.Timeout > 0) && (Ret== (-SIG_ERESTARTSYS)))//receive restart system call again
        {
            ...
            Ta=dbgTmr.getUs();
            if (waitirq.UserInfo.UserKey == m3AUserKey)
                LOG_INF("3A(%d) waitIrq again+", m3AUserKey);
                Ret = ioctl(mFd,ISP_WAIT_IRQ,&waitirq);
            if (waitirq.UserInfo.UserKey == m3AUserKey)
                LOG_INF("3A(%d) waitIrq again-, return(%d)", m3AUserKey, Ret);
            Tb=dbgTmr.getUs();
                waitirq.Timeout=waitirq.Timeout - ((Tb-Ta)/1000);
            }
            LOG_INF("Leave ERESTARTSYS(%d),Type(%d),Status(0x%08x),Timeout(%d),Tb-Ta(%d us).",Ret,pwaitIrq->Type, pwaitIrq->Status, pwaitIrq->Timeout,Tb-Ta);
    }
    ...
    if(Ret < 0) {
        if((MINT32)pwaitIrq->UserInfo.UserKey >= 0 ) //v3 used
        {
            LOG_ERR("ISP_WAIT_IRQ fail(%d). Type(%d),Status(0x%08x),UserKey(0x%08x),Timeout(%d).",
            Ret, pwaitIrq->UserInfo.Type, pwaitIrq->UserInfo.Status, pwaitIrq->UserInfo.UserKey, pwaitIrq->Timeout);
        } else {
    ...
    {   //v3
        ...
        /* It's no need to check unsigned int < 0 */
...
MINT32   IspDrvImp::registerIrq(const char* userName)
{
    ...
    if ( strncmp( userName, "VSIrq", 5) == 0) {
        m3AUserKey = userKeyStruct.userKey;
        LOG_INF("3A userKey(%d)", userKeyStruct.userKey);
    }
...
MBOOL   IspDrvImp::flushIrq(ISP_DRV_WAIT_IRQ_STRUCT Irqinfo)
{
    ...
    if(Irqinfo.UserNumber < 0){//v3 flow
        Irqinfo.Type=ISP_DRV_IRQ_TYPE_INT_P2_ST;    //no use in v3
        Irqinfo.Status=0x0; //no use in v3
        /* driver do not support flushIrq: PASS1_DON_ST, it will break kernel RTBC mechanism*/
        if ((Irqinfo.UserInfo.Type == ISP_DRV_IRQ_TYPE_INT_P1_ST) &&
            (Irqinfo.UserInfo.Status == CAM_CTL_INT_P1_STATUS_PASS1_DON_ST))
            return MTRUE;
    }
    ...
    MINT32      caller)
{
    ...
    switch(userEnum)
    {
        ...
        case ISP_DRV_USER_FD:
            break;
        default:
            LOG_ERR("ad[0x%x] vl[0x%lu] only ispDrv/isp_function/seninf have authorization to directly write hw registers\n",Addr,Data);
            return MFALSE;
    }
    switch(caller)
    {
   ...
   MINT32 Ret = 0;
    ...
    if ( ISP_RT_BUF_CTRL_MAX == pbuf_ctrl->ctrl ) {
        LOG_DBG("[rtbc]cq(%d),vIspV/P(%p,%p),descV/P(%p,%p),mmap(%p)", \
            ...
            (unsigned int*)IspDrv::mpIspCQDescriptorVirt[pbuf_ctrl->buf_id], \
...
MBOOL IspDrvImp::updateCq0bRingBuf(void *pOBval)
{
 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (unsigned long)(va) - (unsigned long)(bva) ) + (unsigned long)(bpa) )
    ...
    LOG_DBG("[rtbc]va(%p),pa(%p)", (unsigned int*)pcqrtbcring_va, (unsigned int*)pcqrtbcring_pa);
    //
...
MBOOL IspDrvImp::_updateEnqCqRingBuf(void *pBuf_ctrl)
{
 ...
 #define RTBC_GET_PA_FROM_VA(va,bva,bpa) ( ( (unsigned long)(va) - (unsigned long)(bva) ) + (unsigned long)(bpa) )
     ...
     UNUSED(pcqrtbcring_va_camsv);
     UNUSED(pcqrtbcring_pa_camsv);
     UNUSED(pcqrtbcring_va_camsv2);
     UNUSED(pcqrtbcring_pa_camsv2);
    ...
    if ( 0 == (cam_tg_vf & 0x01) ) {
        ...
        LOG_DBG("[rtbc]va(%p),pa(%p),ctrl(%d),dma(%d),PA(0x%x),o_size(%d),zo_size(%d)", \
                (unsigned int*)pcqrtbcring_va, \
                (unsigned int*)pcqrtbcring_pa, \
                pbuf_ctrl->ctrl, \
                pbuf_ctrl->buf_id, \
        ...
        //
        {
            ...
            for ( i=0 ; i<(sizeof(CQ_RING_CMD_ST)>>2)*size ; i++ ) {
                LOG_DBG("[rtbc][des] addr:(%p) val:(%d)",pdata+i,pdata[i]);
            }
        }
    }
    //VF_EN==1
    ...
    switch( pbuf_ctrl->ctrl ) {
        ...
        case ISP_RT_BUF_CTRL_CLEAR:
            ...
            if (sizeof(CQ_RTBC_RING_ST) <= sizeof(mIspCQDescInit)) {
                ...
                LOG_ERR("rtbc data exceed buffer size(%u)>(%u)",sizeof(CQ_RTBC_RING_ST),sizeof(mIspCQDescInit));
            }
...
)
{
    ...
    if(path == eTuningQueAccessPath_featureio){
        ...
        LOG_DBG("p2Cq(%d),currQueueIdx(%d),pTuningQue(%p)",p2Cq,currQueueIdx,mTuningQueInf[p2Cq][currQueueIdx].pTuningQue);
        ...
        do {
            ...
            LOG_DBG("camCtlEnP1(0x%x),camCtlEnP2(0x%x),pCurReadP2TuningQue(%p),keepP2PreviewQue(%p),eUpdateFuncBit(0x%08x)", \
                    camCtlEnP1,camCtlEnP2,mTuningQueIdx[p2Cq].pCurReadP2TuningQue,mTuningQueIdx[p2Cq].keepP2PreviewQue.pTuningQue,(unsigned int)(mTuningQueIdx[p2Cq].eCurReadP2UpdateFuncBit));
            //
...
//set by imageio path                            mTuningQueIdx[p2Cq].keepTpipeField.nr3d.nr3d_on_height = pReadP2TuningQue->CAM_NR3D_ON_SIZ.Bits.NR3D_ON_HT;
        ...
        for (k=0;k<(unsigned int)checkNum;k++){
...
#if 1
                if(((unsigned int)checkNum == k+1)){ // get currQueueIdx
            ...
            LOG_DBG("camCtlEnP1(0x%x),camCtlEnP1D(0x%x),keepP1UpdateFuncBit(0x%llx),eUpdateFuncBit(0x%llx)",camCtlEnP1,camCtlEnP1D,mTuningQueIdx[p2Cq].keepP1UpdateFuncBit,mTuningQueInf[p2Cq][currQueueIdx].eUpdateFuncBit);
            //
            for(i=0;i<eIspTuningMgrFuncBit_Num;i++){
...
)
{
    ...
    //
    mapCqToP2Cq(eCq, p2Cq);
    LOG_DBG("path(%d),eCq(%d),p2Cq(%d),magicNum(0x%x),updateFuncBit(0x%08x)",path,eCq,p2Cq,magicNum,(unsigned int)updateFuncBit);
    //
    if(path == eTuningQueAccessPath_featureio){
        if((mTuningQueIdx[p2Cq].curWriteIdx>=ISP_TUNING_QUEUE_NUM)||(mTuningQueIdx[p2Cq].curWriteIdx<0)){
...
{
    MBOOL ret = MTRUE;
    //MUINT32 searchCnt; //not used
    //MINT32 currQueueIdx = -1; //not used
    //MINT32 prevQueueIdx = -1; //not used
    MUINT32 i;
...
)
{
    ...
    //
    pTuningField.nbc.anr_eny = pTuningRegMappingQue->CAM_ANR_CON1.Bits.ANR1_ENY;
    pTuningField.nbc.anr_enc = pTuningRegMappingQue->CAM_ANR_CON1.Bits.ANR1_ENC;
    //pTuningField.nbc.anr_iir_mode = pTuningRegMappingQue->CAM_ANR_CON1.Bits.ANR_FLT_MODE;
    pTuningField.nbc.anr_scale_mode = pTuningRegMappingQue->CAM_ANR_CON1.Bits.ANR1_SCALE_MODE;
    ...
    //
    #endif
    pTuningField.bnr.bpc_en = pTuningRegMappingQue->CAM_BNR_BPC_CON.Bits.BPC_EN;
    pTuningField.bnr.bpc_tbl_en = pTuningRegMappingQue->CAM_BNR_BPC_CON.Bits.BPC_LUT_EN;
...
)
{
    ...
    switch(softScenario){
        ...
        case eSoftwareScenario_Main_IP_Raw_TPipe_Stream:
        ...
        case eSoftwareScenario_Sub_IP_Raw_TPipe_Stream:
        ...
        case eSoftwareScenario_Main2_IP_Raw_TPipe_Stream:
            p2Cq = ISP_DRV_P2_CQ3;
            break;
        default:
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::enqueP2Frame(unsigned long callerID,MINT32 p2burstQIdx,MINT32 p2dupCQIdx ,MINT32 frameNum, MINT32 p2Scenario)
{
    MBOOL ret=MTRUE;
    ...
    edBufQueV.p2Scenario = p2Scenario;
    ret=ioctl(mFd,ISP_ED_QUEBUF_CTRL,&edBufQueV);
    if(ret<0)
    {
        LOG_ERR("ISP_DRV_BUFQUE_CTRL_ENQUE_FRAME fail(%d). callerID(0x%lx).", ret,callerID);
        return MFALSE;
    }
    else
    {
...
MBOOL IspDrvImp::waitP2Deque()
{
    ...
    if(ret<0)
    {
        LOG_ERR(" ISP_DRV_BUFQUE_CTRL_WAIT_DEQUE fail(%d). callerID(0x%lx).", ret,edBufQueV.callerID);
        return MFALSE;
    }
    else
    {
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::dequeP2FrameSuccess(unsigned long callerID,MINT32 p2dupCQIdx, MINT32 p2Scenario)
{
    MBOOL ret=MTRUE;
    ISP_DRV_ED_BUFQUE_STRUCT edBufQueV;
    ...
    if(ret<0)
    {
        LOG_ERR("ISP_DRV_BUFQUE_CTRL_DEQUE_DONE fail(%d). callerID(0x%lx).", ret,callerID);
        return MFALSE;
    }
    else
    {
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::dequeP2FrameFail(unsigned long callerID,MINT32 p2dupCQIdx,MINT32 p2Scenario)
{
    MBOOL ret=MTRUE;
    ISP_DRV_ED_BUFQUE_STRUCT edBufQueV;
...
//-----------------------------------------------------------------------------
MBOOL IspDrvImp::waitP2Frame(unsigned long callerID,MINT32 p2dupCQIdx,MINT32 p2Scenario,MINT32 timeoutUs)
{
    MBOOL ret=MTRUE;
    ISP_DRV_ED_BUFQUE_STRUCT edBufQueV;
    ...
    if(ret<0)
    {
        LOG_ERR("ISP_DRV_BUFQUE_CTRL_WAIT_FRAME fail(%d).callerID(0x%lx), Timeout(%d).", ret,callerID, timeoutUs);
        return MFALSE;
    }
    else
    {
...
//-----------------------------------------------------------------------------
//update magic number
MBOOL IspDrvImp::updateMagicNumber(MUINT32* value)
{
    MINT32 ret = 0;
    MUINT32* qValue;
    if (value == NULL) {
    ...
    // value[0]:MagicNum
    // value[1]:_isp_tg_enum_
    if ((value[1] != _camsv_tg_) && (value[1] != _camsv2_tg_)) {
        LOG_ERR("invalid value[1]: 0x%x", value[1]);
        return 1;
    }
    qValue = value;
    ret = ioctl(mFd, ISP_UPDATE_MAGNUM, qValue);
    if (ret < 0)
    {
        LOG_ERR("ISP_UPDATE_MAGNUM fail(%d).", ret);
        return 1;
    }
    return 0;
}
