...
MINT32 CAMSV_TG_CTRL::_config( void )
{
    ...
    if(((this->m_PixMode == ePixMode_4) && ((this->m_Crop.w % 4) != 0)) ||
        ((this->m_PixMode == ePixMode_2) && ((this->m_Crop.w % 2) != 0))){
        CAM_FUNC_ERR("TG cropping size need pixel-alignment\n");
        return 1;
    }
...
MINT32 CAMSV_TOP_CTRL::_config( void )
{
    ...
    //CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_MODULE_EN,CAMCTL_IMGO_R1_EN,1); // enable IMGO
    ...
    CAM_FUNC_INF("en(0x%08x),sf_en(0x%08x),fmtSel(0x%08x),intEn(0x%08x),SubSample(0x%x)", \
                CAMSV_READ_REG(this->m_pDrv,CAMSV_MODULE_EN), \
                CAMSV_READ_REG(this->m_pDrv,CAMSV_SPECIAL_FUN_EN), \
...
MINT32 CAMSV_TOP_CTRL::_disable( void* pParam )
{
    ...
    CAM_FUNC_INF("CAMSV_TOP_CTRL::_disable+, FRMSIZE_ST %x, TG_INTER %x, GRABPXL %x, GRABLIN %x",
        CAMSV_READ_REG(this->m_pDrv,CAMSV_TG_FRMSIZE_ST), CAMSV_READ_REG(this->m_pDrv,CAMSV_TG_INTER_ST),
        CAMSV_READ_REG(this->m_pDrv,CAMSV_TG_SEN_GRAB_PXL), CAMSV_READ_REG(this->m_pDrv,CAMSV_TG_SEN_GRAB_LIN));
...
MBOOL DMAO_CAMSV_B::BW_Meter(const Sig_Profile& prof,V_DMA_BW* pOut)
{
    DMA_BW fifo;
    //E_ISP_CAM_CQ cq;
    //MUINT32 page;
...
CAMSV_BUF_CTRL::E_BC_STATUS CAMSV_BUF_CTRL::waitBufReady( void )
{
    ...
    MUINT32 loopCntMax = _LOOP_;
    char str[128] = {'\0'};
    char _tmp[16] = {'\0'};
    const MUINT32 sizeStr = sizeof(str);
    const MUINT32 sizeTmpStr = sizeof(_tmp);
    MUINT32 stringSize = 0;
    ...
    irq.Clear = ISP_IRQ_CLEAR_WAIT;
    irq.UserKey = 0;
    // piecewise duration to monitor camsv stopped and
    // waitIrq done prevent corner case, stop/flush IRQ(already trigger SOF once) from another thread
    loopCntMax = (this->estimateTimeout(1)/(MIN_GRPFRM_TIME_MS*2)) >= _LOOP_ ? ((this->estimateTimeout(1)/(MIN_GRPFRM_TIME_MS*2))+1) : _LOOP_;
    irq.Timeout = MIN_GRPFRM_TIME_MS*2;
    loopCnt = loopCntMax;
        ...
        if(this->m_buf_cnt != 0){
            ...
            if(FbcCnt < this->m_buf_cnt){
                if(loopCnt == loopCntMax){
                    CAM_FUNC_INF("dma:CAMSV_BUF_CTRL already have avail_buf on dram, bWaitBufReady = 0, timeout:%d", irq.Timeout);
                    return eCmd_Pass;
                }
                else{
                    goto EXIT;
                ...
                if(loopCnt == loopCntMax){
                    snprintf(str, sizeStr, "dma:CAMSV_BUF_CTRL start wait:[enque record:%d_%d],",this->m_buf_cnt, FbcCnt);
                    stringSize += sizeof("dma:CAMSV_BUF_CTRL start wait:[enque record:%d_%d],");
                }
                else{
                    snprintf(_tmp, sizeof(_tmp), "[%d]time,",((loopCntMax-loopCnt)+1));
                    if((stringSize + sizeTmpStr) < sizeStr){
                        strncat(str, _tmp, strlen(_tmp));
                        stringSize += strlen(_tmp);
                    }
                }
            }
        }
            ...
            if(loopCnt == loopCntMax){
                 CAM_FUNC_INF("dma:CAMSV_BUF_CTRL bWaitBufReady = 1,wait for sw enque[%d],timeout:%d x %d ms,",this->m_buf_cnt,irq.Timeout,loopCnt);
#define FALSE_PASS1_DONE
                ...
                snprintf(_tmp, sizeof(_tmp), "[%d],",((loopCntMax-loopCnt)+1)+1);
                if((stringSize + sizeTmpStr) < sizeStr){
                    strncat(str, _tmp, strlen(_tmp));
                    stringSize += strlen(_tmp);
                }
...
MBOOL CAMSV_BUF_CTRL::enqueueHwBuf( stISP_BUF_INFO& buf_info,MBOOL bImdMode)
{
    ...
    //workaround for FBC handling buffer error
    if(CAMSV_READ_BITS(this->m_pDrv,CAMSV_TG_VF_CON,VFDATA_EN) == 1){
        ISP_WAIT_IRQ_ST irq;
        MUINT32 imgo_fbc = CAMSV_READ_BITS(this->m_pDrv,CAMSV_FBC_IMGO_CTL2,FBC_CNT);
        MUINT32 m_RCNT = CAMSV_READ_BITS(this->m_pDrv,CAMSV_FBC_IMGO_CTL2,RCNT);
        MUINT32 m_DROP_CNT = CAMSV_READ_BITS(this->m_pDrv,CAMSV_FBC_IMGO_CTL2,DROP_CNT);

        if( imgo_fbc>=1 && (((m_RCNT+m_DROP_CNT)%CAMSV_FBC_RCNT) != (_sof_idx%CAMSV_FBC_RCNT)) ) //CAMSV_BUF_DEPTH = 2
        {
            // piecewise duration to monitor camsv stopped and
            // waitIrq done prevent corner case, stop/flush IRQ(already trigger SOF once) from another thread
            MUINT32 loopCnt = 5;
            loopCnt = (this->estimateTimeout(1)/(MIN_GRPFRM_TIME_MS*2)) >= 5 ? ((this->estimateTimeout(1)/(MIN_GRPFRM_TIME_MS*2))+1) : loopCnt;
            irq.Clear = ISP_IRQ_CLEAR_WAIT;
            irq.UserKey = 0;
            irq.Timeout = MIN_GRPFRM_TIME_MS*2;
            irq.St_type = SIGNAL_INT;
            irq.Status = SV_SOF_INT_ST;//SV_SW_PASS1_DON_ST;//SV_SOF_INT_ST;
            ...
            if(this->m_pDrv->waitIrq(&irq) == MTRUE) {
                ...
                if(CAMSV_READ_BITS(this->m_pDrv,CAMSV_TG_VF_CON,VFDATA_EN) == 0) {
                    CAM_FUNC_INF("stopped and exit, cnt %d", loopCnt);
                    break;
                }
            } while ( --loopCnt > 0 );
        }
        ...
        case _camsv_imgo_:
            {
                ...
                if(CAMSV_READ_BITS(this->m_pDrv,CAMSV_FBC_IMGO_CTL1,FBC_EN) != 1){
                    CAM_FUNC_WRN("%s: can't enque before enabe fbc\n",this->name_Str());
                    return MFALSE;
                }
                //enque buf pa
                ...
                //for FBC mode, this setting is useless, but it still needs to set to avoid KE when
                //receiving wrong zero size data.
