...
{
    MBOOL Result = MTRUE, wakelock = MTRUE, bPrint = MTRUE;
    ...
    else    // open success.   // Sometimes GDMA will go this path, too. e.g. File Manager -> Phone Storage -> Photo.
    {
            ...
            switch(this->m_HWmodule){
                ...
                case CAMSV_7:
                    ...
                    gIspDrvObj[CAMSV_6].m_pIspHwRegAddr = (MUINT32 *) mmap(0, CAMSV_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, CAMSV_6_BASE_HW);
                    gIspDrvObj[CAMSV_7].m_pIspHwRegAddr = (MUINT32 *) mmap(0, CAMSV_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, CAMSV_7_BASE_HW);

                    gIspDrvObj[CAM_A].m_pIspHwInnerRegAddr   = (MUINT32 *) mmap(0, CAM_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, CAM_A_INNER_BASE_HW);
                    gIspDrvObj[CAM_B].m_pIspHwInnerRegAddr   = (MUINT32 *) mmap(0, CAM_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, CAM_B_INNER_BASE_HW);
                    gIspDrvObj[CAM_C].m_pIspHwInnerRegAddr   = (MUINT32 *) mmap(0, CAM_BASE_RANGE, (PROT_READ | PROT_WRITE | PROT_NOCACHE), MAP_SHARED, this->m_Fd, CAM_C_INNER_BASE_HW);
                    gIspDrvObj[CAMSV_0].m_pIspHwInnerRegAddr = NULL;
                    gIspDrvObj[CAMSV_1].m_pIspHwInnerRegAddr = NULL;
                    gIspDrvObj[CAMSV_2].m_pIspHwInnerRegAddr = NULL;
                    gIspDrvObj[CAMSV_3].m_pIspHwInnerRegAddr = NULL;
                    gIspDrvObj[CAMSV_4].m_pIspHwInnerRegAddr = NULL;
                    gIspDrvObj[CAMSV_5].m_pIspHwInnerRegAddr = NULL;
                    gIspDrvObj[CAMSV_6].m_pIspHwInnerRegAddr = NULL;
                    gIspDrvObj[CAMSV_7].m_pIspHwInnerRegAddr = NULL;
                    break;
                default:
                    LOG_ERR("#############\n");
            ...
            for(MUINT32 i=0;i<MAX_ISP_HW_MODULE;i++){
                if((gIspDrvObj[i].m_pIspHwRegAddr      == MAP_FAILED) ||
                   (gIspDrvObj[i].m_pIspHwInnerRegAddr == MAP_FAILED))
                {
                ...
                if((gIspDrvObj[i].m_pIspHwRegAddr      != MAP_FAILED) ||
                   (gIspDrvObj[i].m_pIspHwInnerRegAddr != MAP_FAILED)) {
                    ...
                    switch(i){
                        ...
                        case CAM_B:
                        case CAM_C:
                            // mmap & munmap must pair
                            if(gIspDrvObj[i].m_pIspHwRegAddr != MAP_FAILED) {
                                ret  = munmap(gIspDrvObj[i].m_pIspHwRegAddr,
                                        CAM_BASE_RANGE);
                            }
                            if(gIspDrvObj[i].m_pIspHwInnerRegAddr != MAP_FAILED) {
                                ret2 = munmap(gIspDrvObj[i].m_pIspHwInnerRegAddr,
                                        CAM_BASE_RANGE);
                            }
                            if ((ret < 0) || (ret2 < 0)) {
                                LOG_ERR("module:0x%x munmap fail: "
                                        "IspHwRegAddr(munmapRet_addr):(%d_%p), "
                                        "IspHwInnerRegAddr(munmapRet_addr):(%d_%p)",
                                        i,
                                        ret, gIspDrvObj[i].m_pIspHwRegAddr,
                                        ret2, gIspDrvObj[i].m_pIspHwInnerRegAddr);
                                break;
                            }
                            gIspDrvObj[i].m_pIspHwRegAddr      = NULL;
                            gIspDrvObj[i].m_pIspHwInnerRegAddr = NULL;
                            break;
                        case CAMSV_0:
                        case CAMSV_1:
                        case CAMSV_2:
...
{
    MBOOL Result = MTRUE, wakelock = MFALSE, bPrint = MTRUE;
    MINT32 tmp = 0;
    std::string strUserName = userName;
    ...
    if (itr != m_User.end()) {
        ...
        for (MUINT32 i = 0; i < m_User.size(); i++) {
            sprintf(str, "CurUser(%d):%s, ", i, m_User.at(i).c_str());
           ...
           (gIspDrvObj[i].m_pIspHwInnerRegAddr != MAP_FAILED)) {
            ...
            switch(i){
                ...
                case CAM_C:
                    ...
                    if (gIspDrvObj[i].m_pIspHwInnerRegAddr != MAP_FAILED) {
                        ret2 = munmap(gIspDrvObj[i].m_pIspHwInnerRegAddr, CAM_BASE_RANGE);
                    }
                ...
                case CAMSV_7:
                     // mmap & munmap must pair
                    if (gIspDrvObj[i].m_pIspHwRegAddr != MAP_FAILED) {
                        ret = munmap(gIspDrvObj[i].m_pIspHwRegAddr, CAMSV_BASE_RANGE);
                    }
...
MBOOL IspDrvImp::waitIrq(ISP_WAIT_IRQ_ST* pWaitIrq)
{
    ...
    switch(this->m_HWmodule){
        ...
        case CAMSV_5:   wait.Type = ISP_IRQ_TYPE_INT_CAMSV_5_ST;
            break;
        case CAMSV_6:   wait.Type = ISP_IRQ_TYPE_INT_CAMSV_6_ST;
            break;
        case CAMSV_7:   wait.Type = ISP_IRQ_TYPE_INT_CAMSV_7_ST;
            break;
        default:
...
MBOOL IspDrvImp::waitIrq(ISP_WAIT_IRQ_STRUCT* pWaitIrq)
{
    MINT32 Ret = 0;
    MUINT32 OrgTimeOut;
    ISP_IRQ_CLEAR_ENUM OrgClr;
    ...
    LOG_DBG(" - E. Type:0x%x,Status(0x%08x),Timeout(%d).\n", pWaitIrq->Type, pWaitIrq->EventInfo.Status, pWaitIrq->EventInfo.Timeout);
    if(FD_CHK() == -1){
        return MFALSE;
    }

    OrgTimeOut = pWaitIrq->EventInfo.Timeout;
    OrgClr = pWaitIrq->EventInfo.Clear;
    ...
    do{
        ...
        if ((errno == SIG_ERESTARTSYS) && (wait.EventInfo.Timeout > 0)) {
            ...
            wait.EventInfo.Timeout=wait.EventInfo.Timeout - ((Tb-Ta)/1000);
            wait.EventInfo.Clear = ISP_IRQ_CLEAR_NONE;
                LOG_INF("Interrupted by other wait again. Type(%d),Status(0x%08x),Timeout(%dms)\n", wait.Type, pWaitIrq->EventInfo.Status, wait.EventInfo.Timeout);
            } else {
                break;
            }
        }
    ...
    pWaitIrq->EventInfo.Timeout = OrgTimeOut;
    pWaitIrq->EventInfo.Clear = OrgClr;


    if(Ret < 0) {
        LOG_ERR("ISP(0x%x)_WAIT_IRQ fail(%d). Wait Status(0x%08x), Timeout(%d).\n", pWaitIrq->Type, errno, pWaitIrq->EventInfo.Status, pWaitIrq->EventInfo.Timeout);
        return MFALSE;
    }
...
MBOOL IspDrvImp::clearIrq(ISP_CLEAR_IRQ_ST* pClearIrq)
{
    ...
    switch(this->m_HWmodule){
        ...
        case CAMSV_5:   clear.Type = ISP_IRQ_TYPE_INT_CAMSV_5_ST;
            break;
        case CAMSV_6:   clear.Type = ISP_IRQ_TYPE_INT_CAMSV_6_ST;
            break;
        case CAMSV_7:   clear.Type = ISP_IRQ_TYPE_INT_CAMSV_7_ST;
            break;
        default:
            LOG_ERR("unsupported hw module:0x%x\n",this->m_HWmodule);
            return MFALSE;
    ...
    if(this->m_SecOn){
        for(MUINT32 i=0;i<m_DapcReg.size();i++){
            if(Addr == m_DapcReg.at(i)){
                LOG_DBG("[0x%x][SecCam ON]Reg Addr:0x%x is not allowed to read\n",caller,Addr);
                return 0;
            }
        }
    }
...
MUINT32 IspDrvImp::readInnerReg(MUINT32 Addr,MINT32 caller)
{
    MUINT32 value=0x0;
    MUINT32 legal_range = REG_SIZE;

    LOG_DBG("+,Isp_read_Inner:m_HWmodule(0x%x),InnerAddr(0x%x)\n",
        this->m_HWmodule, (Addr + REG_SIZE));
    ...
    switch(this->m_regRWMode){
        case ISP_DRV_RW_MMAP:
            ...
            value = this->m_pIspHwInnerRegAddr[(Addr >> 2)];
            break;

        default:
            LOG_ERR("regRWMode(%d) no support readInnerReg()\n",
                this->m_regRWMode);
            value = 0;
            break;
    }

    LOG_DBG("-,Isp_read_Inner:(0x%x,0x%x)",Addr,value);
    return value;
}
...
MBOOL IspDrvImp::writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller)
{
    ...
    if(this->m_SecOn){
        for(MUINT32 i=0;i<m_DapcReg.size();i++){
            if(Addr == m_DapcReg.at(i)){
                LOG_DBG("[0x%x][SecCam ON]Reg Addr:0x%x is not allowed to write\n",caller,Addr);
                return MFALSE;
            }
        }
    }
...
MUINT32* IspDrvImp::getHwRegAddr(void)
{
    if(FD_CHK() == -1){
        return NULL;
    }

    return gIspDrvObj[this->m_HWmodule].m_pIspHwRegAddr;
}

MBOOL IspDrvImp::readPtrRegs(MUINT32 **ptr, MUINT32 offset, MUINT32 size,MINT32 caller)
{
    MUINT32 legal_range = REG_SIZE;
    MUINT32 _addr = 0;
    ...
    if (this->m_regRWMode == ISP_DRV_RW_MMAP) {
        for(unsigned int i=0; i<size; i++){
            ...
            pIspRegMap[(_addr>>2)] = this->m_pIspHwRegAddr[(_addr>>2)];
            LOG_DBG("%d:addr:0x%x,data:0x%x",i,_addr,pIspRegMap[(_addr>>2)]);
        }

    } else {
        LOG_INF("Not implement readReg from ioctl:ISP_READ_REGISTER");
    }
