...
PortIdxMappingAlltoP2 mPortIdxMappingAlltoP2[NSImageio::NSIspio::EPortIndex_UNKNOW]=
{
    ...
    {NSImageio::NSIspio::EPortIndex_IMGI,   EPostProcPortIdx_IMGI}, //17
    ...
    )
{
    ...
    for(int q=0;q<framePack.frameNum;q++)
    {
        ...
        for(int i=0;i< qParam.mvFrameParams[q].mvIn.size() ;i++)
        {
            ...
            LOG_DBG("[mvIn (%d)] fmt(0x%x),w/h(%d/%d),oft(%d/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%x,0x%x,0x%x)/pa(0x%x,0x%x,0x%x)/size(0x%x,0x%x,0x%x) ),pixelDivNum:%d,pixelMulNum:%d",\
        ...
        for(int i=0;i< qParam.mvFrameParams[q].mvOut.size() ;i++)
        {
            ...
            LOG_DBG("[mvOut (%d)] fmt(0x%x),w/h(%d/%d),oft(%d/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%x,0x%x,0x%x)/pa(0x%x,0x%x,0x%x)/size(0x%x,0x%x,0x%x) ),pixelDivNum:%d,pixelMulNum:%d",\
