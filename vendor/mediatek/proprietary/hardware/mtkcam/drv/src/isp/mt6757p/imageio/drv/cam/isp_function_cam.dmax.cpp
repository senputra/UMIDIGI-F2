...
MBOOL DMA_CQ5::setCQContent(MUINTPTR arg1,MUINTPTR arg2)
{
    E_ISP_CAM_CQ cq;
    MUINT32 page;
    list<NSImageio::NSIspio::ISPIO_REG_CFG>* ptr = (list<NSImageio::NSIspio::ISPIO_REG_CFG>*)arg2;
    ...
    //dbg log
    this->m_pDrv->getCurObjInfo(&this->m_hwModule,&cq,&page);
    CAM_FUNC_DBG("DMA_CQ5::cfg:+ \n");

    //
    pReg = (ISP_DRV_REG_IO_STRUCT*)malloc(sizeof(ISP_DRV_REG_IO_STRUCT) * ptr->size());
    if (NULL == pReg) {
        CAM_FUNC_ERR("DMA_CQ5::setCQContent: alloc buf fail\n");
        return MFALSE;
    }
    for(it = ptr->begin(),i=0;it != ptr->end();it++,i++){
        pReg[i].Addr = it->Addr;
...
MINT32 DMAO_B::_config( void )
{
    ...
    switch(this->id())
    {
        ...
        case CAM_DMA_PSO:
            ...
            CAM_WRITE_REG(this->m_pDrv->getPhyObj(),CAM_PSO_CON3,0x00200020);
#endif
    ...
    CAM_FUNC_DBG("DMAO_B::_write2CQ+[%s]:cq(0x%x),page(0x%x)\n", \
     ...
     switch(this->id())
     {
        ...
        case CAM_DMA_AAO:
            ...
            this->m_pDrv->cqAddModule(CAM_DMA_FH_AAO_SPARE_OB_);
            this->m_pDrv->cqAddModule(CAM_DMA_FH_AAO_SPARE_LSC_);
            break;
        case CAM_DMA_PSO:
