...
*
********************************************************************************/
ICamIOPipe* ICamIOPipe::createInstance( MINT8 const szUsrName[32], E_INPUT& InOutPut, E_PIPE_OPT opt, PortInfo const *portInfo, MUINT32 SensorNum)
{
    switch(InOutPut){
        case TG_A:
        case TG_B:
    ...
    switch(opt){
        case CAMIO:
            return  (ICamIOPipe*)CamIOPipe::Create(szUsrName,InOutPut,portInfo,SensorNum);
            break;
        case STTIO:
...
CamIOPipe* CamIOPipe::Create(MINT8 const szUsrName[32], E_INPUT& OutPut, PortInfo const *portInfo, MUINT32 SensorNum)
{
    ...
    if (OutPut == TG_CAM_MAX) {
        ...
        if (SensorNum == ECamio_2_SEN) {
            pCamIOPipeRet->m_pResMgr = ResMgr::Res_Attach(user_camio, (char *)szUsrName, AdjustmentCrop1w, SensorNum);
        } else {
...
MBOOL CamIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, CAMIO_Func const *pInFunc, CAMIO_Func *pOutFunc)
{
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ ) {
        ...
        PIPE_INF("P1 vInPorts_%d:fmt(0x%x),PM(%d),w/h(%d_%d),crop(%d_%d_%d_%d),tg_idx(%d),dir(%d),fps(%d),timeclk(0x%x),MIPI_PixRate(0x%x),signal(%d_%d_%d),subsample(%d),bin_off(%d),twin_off(%d),dat_pat(%d),sen_num(%d)\n",\
                i, \
                vInPorts[i]->eImgFmt, \
                vInPorts[i]->ePxlMode,  \
    ...
    if(this->m_camPass1Param.bypass_tg == MFALSE)
    {
        ...
        //
        memcpy((void*)&this->m_PathCtrl->m_Src,(void*)dynamic_cast< NSImageio::NSIspio::PortID *> ((PortInfo *) vInPorts[idx_in_src]),sizeof(NSImageio::NSIspio::PortID));
		...
		PIPE_INF("current clk_level=%d\n",this->m_PathCtrl->m_resCfgOutParam.clk_level);
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_SET_P1_UPDATE:
            ...
            //Arg3: sensorDev,MUINT32, not a list
            {
                ...
                list<MUINT32>* pInput_LPort = (list<MUINT32>*)arg1;
                list<MUINT32>::iterator it;
                IspP1TuningCfg cfg;
                list<IspP1TuningCfg> cfg_L;
                list<MUINT32> dmaChannel_L;
                ...
                */
                if ( 0 != this->m_TwinMgr->setP1TuneCfg(&cfg_L) ){
                    PIPE_ERR("P1_UPDATE setP1TuneCfg Fail, magic number:0x%x\n", pInput_L->front());
                    this->m_TwinMgr->dropEnqueueBuf(this->m_PathCtrl->m_twinCfgInParam, &dmaChannel_L, pInput_L);
                    return MFALSE;
                }
                ...
                if (this->m_TwinMgr->setP1Update(pBW_CB)) {
                    PIPE_ERR("setP1Udpate failed, magic number:0x%x\n", pInput_L->front());
                    this->m_TwinMgr->dropEnqueueBuf(this->m_PathCtrl->m_twinCfgInParam, &dmaChannel_L, pInput_L);
                    delete pBW_CB;
                    return MFALSE;
                }
...
MBOOL CamIOPipe::configFrame(E_FRM_EVENT event,CAMIO_Func func)
{
    ...
    this->m_cam_dfs->CAM_DBS_Vsync();//perframe request from DFS
