...
MBOOL IMemDrvImp::init(char const* szCallerName)
{
    ...
    IMEM_DBG("mCount(%s_%d)", szCallerName, mInitCount);
    //
    ...
    if(mInitCount==1)
    {
        ...
        IMEM_INF("mCount(%d) , open ion id(%d).\n", mInitCount, mIonDrv);
   #endif
...
MBOOL IMemDrvImp::uninit(char const* szCallerName)
{
    ...
    //
    //IMEM_DBG("mCount(%d)", mInitCount);
    IMEM_INF("mCount(%s_%d)", szCallerName, mInitCount);
    //
    Mutex::Autolock lock(mLock);
    //
    ...
    if ( mInitCount <= 0 )
    {
        ...
        if (mIonDrv)
        {
            IMEM_INF("mCount(%d), close ion id(%d).\n", mInitCount, mIonDrv);
            ion_close(mIonDrv);
        }
#endif
    }
...
MINT32 IMemDrvImp::allocVirtBuf(
    IMEM_BUF_INFO* pInfo)
{
    return allocVirtBuf("No szCallerName", pInfo);
}
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::allocVirtBuf(
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    if (!pInfo->virtAddr)
    {
        IMEM_ERR("Cannot map ion buffer. memID(0x%x)/size(0x%x)/va(%p)",pInfo->memID,pInfo->size, (void *)pInfo->virtAddr);
        return -1;
    }
    IMEM_DBG("%s mID[0x%x]/size[0x%x]/VA[%p]/Imem usage[0x%x]",szCallerName, pInfo->memID,pInfo->size,(void *)pInfo->virtAddr,total_size);

    //
    IMEM_DBG("ionFd[0x%x]",pInfo->memID);
#endif
    IMEM_DBG("mID[0x%x]/size[0x%x]/VA[%p] \n",pInfo->memID,pInfo->size,(void*)pInfo->virtAddr);
    return 0;
}
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::freeVirtBuf(
    IMEM_BUF_INFO* pInfo)
{
    return freeVirtBuf("No szCallerName", pInfo);
}
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::freeVirtBuf(
    char const* szCallerName,
    IMEM_BUF_INFO* pInfo)
{
    IMEM_DBG("mID[0x%x]/size[0x%x]/VA[%p]/PA[%p]",pInfo->memID,pInfo->size,(void *)pInfo->virtAddr,(void *)pInfo->phyAddr);
    total_size -= pInfo->size;
    IMEM_DBG("szCallerName(%s), mID[0x%x]/size[0x%x]/VA[%p]/PA[%p]/Imem usage[0x%x]",szCallerName, pInfo->memID,pInfo->size,(void *)pInfo->virtAddr,(void *)pInfo->phyAddr,total_size);
    IMEM_DRV_DELAY
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    if ( 0 ==  buf_map.count(pInfo->virtAddr) ) {
        ...
        if ( IMEM_MIN_ION_FD > pInfo->memID ) {
            ...
            mm_data.config_buffer_param.eModuleID = M4U_PORT_CAM_IMGI;    // 1;
            ...
            //
            IMEM_DBG("Physical address=%d len=%lu", sys_data.get_phys_param.phy_addr, sys_data.get_phys_param.len);
    ...
    //
    IMEM_DBG("mID(0x%x),size(0x%x),VA(%lu),PA(%lu),S/C(%d/%d)",pInfo->memID,pInfo->size,(unsigned long)pInfo->virtAddr,(unsigned long)pInfo->phyAddr,pInfo->bufSecu,pInfo->bufCohe);
...
#endif
        //
        IMEM_DBG("[IMEM_BUFFER] - unmap<memID(0x%x),size(0x%x),virtAddr(%lu),phyAddr(%lu)>",pInfo->memID,pInfo->size,(unsigned long)pInfo->virtAddr,(unsigned long)pInfo->phyAddr);
        IMEM_DRV_DELAY
        //
#if     defined(__ISP_USE_PMEM__)
...
//-----------------------------------------------------------------------------
MINT32  IMemDrvImp::doIonCacheFlush(MINT32 memID,void* virAddr,MUINT32 size,eIONCacheFlushType flushtype)
{
    #if defined (__ISP_USE_ION__)
    ion_user_handle_t pIonHandle;
    MINT32 IonBufFd;
    ...
    sys_data.cache_sync_param.va = pVa;
    sys_data.cache_sync_param.size = uSize;
    ...
    {
        IMEM_ERR("ion_free fail, DO NOT SUPPORT ION_CACHE_FLUSH_ALL");
        return -1;
    }
    if(ion_custom_ioctl(mIonDrv, ION_CMD_SYSTEM,&sys_data))
    {
...
MINT32 IMemDrvImp::cacheSyncbyRange(IMEM_CACHECTRL_ENUM ctrl,IMEM_BUF_INFO* pInfo)
{
    ...
    IMEM_DBG("+, c/m/va/sz/pa(%d/0x%x/%p/0x%x/%p)",ctrl,pInfo->memID,(void *)pInfo->virtAddr,pInfo->size,(void *)pInfo->phyAddr);
    ...
    else
    {
        ...
        {
            doIonCacheFlush(pInfo->memID,(void*)pInfo->virtAddr,pInfo->size,eIONCacheFlushType_Range);
            IMEM_DBG("+ Flush by range, c/m/va/sz/pa(%d/0x%x/%p/0x%x/%p)",ctrl,pInfo->memID,(void *)pInfo->virtAddr,pInfo->size,(void *)pInfo->phyAddr);
            return 0;
        }
