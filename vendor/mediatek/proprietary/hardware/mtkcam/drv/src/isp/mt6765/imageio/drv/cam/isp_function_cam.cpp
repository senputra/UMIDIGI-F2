...
MINT32 CAM_RRZ_CTRL::_config(void)
{
    ...
    if(rrz_ctl == 0){
        ret = 1;
        CAM_FUNC_ERR("CAM_RRZ_CTRL::_config+ cq:0x%x,page:0x%x,rrz_in_w/h(%ld/%ld),rrz_crop_x/y/fx/fy/w/h(%d/%d/%d/%d/%ld/%ld),rrz_out_w/h(%ld/%ld),rlb_offset(%d),rrz_h/v_step(%d/%d),dbn/bin(%d/%d),dbg(%d/%d)\n",cq,page,\
                rrz_in_size.w, \
                rrz_in_size.h, \
                this->rrz_in_roi.x, \
                this->rrz_in_roi.y, \
