...
static void*    RunningThread_1(void *arg)
{
     ...
     */
    printf("CAM_1 is writing...\n");
    CAM_BURST_WRITE_REGS(ptr,RRZO_R1_RRZO_FH_SPARE_2,Regs_W,6);
    printf("CAM_1 is reading...\n");
    CAM_BURST_READ_REGS(ptr,RRZO_R1_RRZO_FH_SPARE_2,Regs_R,6);
    printf("CAM_1 reading completed...\n");
    for(int i=0;i<6;i++){
        ...
        case 0:
            {
                ...
                for (i = 0; i < CAM_LOOP_NUM; i++) {
                    ...
                    CAM_WRITE_REG(ptr->getPhyObj(),CAMCTL_R1_CAMCTL_DMA_EN,0x6543);
                    printf("magic number access: 0x%x\n",CAM_READ_REG(ptr->getPhyObj(),CAMCTL_R1_CAMCTL_DMA_EN));

                    printf("marco bit test(bef:0x%x)\n",CAM_READ_REG(ptr->getPhyObj(),CAMCTL_R1_CAMCTL_EN));
                    CAM_WRITE_BITS(ptr->getPhyObj(),CAMCTL_R1_CAMCTL_EN,CAMCTL_DGN_R1_EN,1);
                    printf("p1 enable bit-access: 0x%x\n",CAM_READ_BITS(ptr->getPhyObj(),CAMCTL_R1_CAMCTL_EN,CAMCTL_DGN_R1_EN));
                    ...
                    CAM_BURST_WRITE_REGS(ptr->getPhyObj(),RRZO_R1_RRZO_FH_SPARE_2,Regs_W,6);
                    CAM_BURST_READ_REGS(ptr->getPhyObj(),RRZO_R1_RRZO_FH_SPARE_2,Regs_R,6);
                    for(int i=0;i<6;i++){
                        printf("rrzo spare access: 0x%x_0x%x, 0x%x_0x%x\n",Regs_W[i].Addr,Regs_W[i].Data,Regs_R[i].Addr,Regs_R[i].Data);
                        if(Regs_W[i].Data != Regs_R[i].Data)
                            bFail = MTRUE;
        ...
        case 1:
            {
                ...
                do{
                    ...
                    printf("marco RW test\n");
                    CAM_WRITE_REG(ptr,CAMCTL_R1_CAMCTL_DMA_EN,0x6543);
                    printf("magic number access: 0x%x\n",CAM_READ_REG(ptr,CAMCTL_R1_CAMCTL_DMA_EN));



                    printf("marco bit test(bef:0x%x)\n",CAM_READ_REG(ptr,CAMCTL_R1_CAMCTL_EN));
                    CAM_WRITE_BITS(ptr,CAMCTL_R1_CAMCTL_EN,CAMCTL_DGN_R1_EN,1);
                    printf("p1 enable bit-access: 0x%x\n",CAM_READ_BITS(ptr,CAMCTL_R1_CAMCTL_EN,CAMCTL_DGN_R1_EN));
                    printf("p1 enable access: 0x%x\n",CAM_READ_REG(ptr,CAMCTL_R1_CAMCTL_EN));
                    ...
                    CAM_BURST_WRITE_REGS(ptr,RRZO_R1_RRZO_FH_SPARE_2,Regs_W,regnum);
                    ptr->flushCmdQ();
                    CAM_BURST_READ_REGS(ptr,RRZO_R1_RRZO_FH_SPARE_2,Regs_R,regnum);
                    for(unsigned int i=0;i<regnum;i++){
                        printf("rrzo spare access: 0x%x_0x%x,0x%x_0x%x\n",Regs_W[i].Addr,Regs_W[i].Data,Regs_R[i].Addr,Regs_R[i].Data);
                        if(Regs_W[i].Data != Regs_R[i].Data)
                            bFail = MTRUE;
        ...
        case 4:
            {
                ...
                do{
...
#endif
                    printf("CAM_1:########################\n");
                    printf("CAM_1:LafiteEP_Todo\n");
                    printf("CAM_1:########################\n");
                    //write cq base addr
                    //CAM_WRITE_REG(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_THR0_BASEADDR,(MUINTPTR)ptr->getCQDescBufPhyAddr());

                    printf("CQ0 INFO: desc base address=0x%lu \n",(MUINTPTR)ptr->getCQDescBufPhyAddr());

                    //set cq trig mode
                    //CAM_WRITE_BITS(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_EN,CAMCQ_CQ_DB_LOAD_MODE,1);
                    //CAM_WRITE_BITS(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_THR0_CTL,CAMCQ_CQ_THR0_MODE,1);
                    ...
                    printf("########################\n");
                    //enable cq
                    CAM_WRITE_BITS(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_THR0_CTL,CAMCQ_CQ_THR0_EN,1);
                    //trig cq
                    CAM_WRITE_BITS(ptr->getPhyObj(),CAMCTL_R1_CAMCTL_START,CAMCTL_CQ_THR0_START,1);
                    //pulling cq0 done
                    //LafiteEP_Todo
                    printf("CAM_1:########################\n");
                    printf("CAM_1:LafiteEP_Todo\n");
                    ...
                    /*1. desc base addr:
                     * CAMCQ_R1_CAMCQ_CQ_THR0_BASEADDR (1A00 4168[31:0]) to dma_cfg.memBuf.base_pAddr
                     * which is allocated in init
                     */
                    cq0.dma_cfg.memBuf.base_pAddr = (MUINTPTR) ptr->getCQDescBufPhyAddr();
                    cq0.config();
                    //CAM_WRITE_REG(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_THR0_BASEADDR,cq0.dma_cfg.memBuf.base_pAddr);
                    printf("CQ0 INFO: desc base address=0x%x\n",cq0.dma_cfg.memBuf.base_pAddr);

                    /*burst mode : cqAddModule(CAM_CQ_THRE0_ADDR_)*/
                    //cq0.write2CQ();

                    /*2. Trigger mode :CQ_DB_LOAD_MODE(1A00 4160[8])=1, CAMCQ_CQ_THR0_MODE(1A00 4164[5:4])=1*/
                    cq0.setCQTriggerMode(DMA_CQ0::_immediate_);
                    //CAM_WRITE_BITS(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_THR0_CTL,CAMCQ_CQ_THR0_MODE,0x1);
                    ...
                    /*3. Enable: CAMCQ_CQ_THR0_EN (1A00 4164[0])=1*/
                    cq0.enable(NULL);
                    //CAM_WRITE_BITS(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_THR0_CTL,CAMCQ_CQ_THR0_EN,0x1);

                    /*4. trigger to start : CQ_THR0_START=0 and wait for clr*/
                    cq0.TrigCQ();
                    //          CAM_WRITE_BITS(ptr->getPhyObj(),CAMCQ_R1_CAMCQ_CQ_THR0_CTL,CAMCQ_CQ_THR0_EN,0x11);

                    printf("########################\n");
                    printf("# Vir2Phy Reg CQ Update#\n");
                    printf("########################\n");

                    //CAM_BURST_READ_REGS(ptr->getPhyObj(),RRZO_R1_RRZO_FH_SPARE_2,Regs_R,regnum);
                    ptr->getPhyObj()->readRegs(Regs_R,regnum);
