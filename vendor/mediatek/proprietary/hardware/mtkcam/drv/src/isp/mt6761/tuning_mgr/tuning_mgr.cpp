...
#define IP_BASE_ISP_3_BUILD_PASS    (1) /* no isp_drv implement temporary.*/
...
)
{
    ...
    MBOOL ret = MTRUE;
    LOG_DBG("scenario(%d),engin(%016llx)",scenario,engin);
#if (IP_BASE_ISP_3_BUILD_PASS == 1)
    //
    ...
    mTuningQueUpdateInf[p2Cq].eTuningMgrUpdateFunc = (ETuningMgrFunc)((MUINT64)engin|(MUINT64)mTuningQueUpdateInf[p2Cq].eTuningMgrUpdateFunc);

    LOG_DBG("p2Cq(%d),eTuningMgrUpdateFunc(%016llx)",p2Cq,mTuningQueUpdateInf[p2Cq].eTuningMgrUpdateFunc);
#endif
    return ret;
}
