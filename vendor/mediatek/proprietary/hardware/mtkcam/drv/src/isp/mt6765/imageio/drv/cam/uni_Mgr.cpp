...
public:
    ISP_HW_MODULE   mConnectModule; /* To record uni is actually attached to which cam */
private:
...
MINT32 UniMgrImp::UniMgr_attach(UNI_HW_MODULE module,DupCmdQMgr* pConnectCam,IspDMACfg* pRawIn)
{
    ...
    for(MUINT32 i=0;i<pConnectCam->CmdQMgr_GetDuqQ();i++){
        for(MUINT32 j=0;j<pConnectCam->CmdQMgr_GetBurstQ();j++){
            ...
            //
            this->m_EISO_FBC.m_pTimeStamp = CAM_TIMESTAMP::getInstance(this->mConnectModule,NULL);
            this->m_RSSO_FBC.m_pTimeStamp = CAM_TIMESTAMP::getInstance(this->mConnectModule,NULL);
        }
    }
    ...
    this->m_FLKO_FBC.enable((void*)&subsample);
        //
        this->m_FLKO_FBC.m_pTimeStamp = CAM_TIMESTAMP::getInstance(this->mConnectModule,NULL);
...
MINT32 UniMgrImp::UniMgr_stop(ISP_HW_MODULE module )
{
    ...
    UniMgr_INF("+: module:0x%x, attach_cam:0x%x\n",this->mModule, this->mConnectModule);
...
{
    return (ISP_HW_MODULE)this->mConnectModule;
}
...
UNI_HW_MODULE UniMgr::UniMgr_getUniModule(ISP_HW_MODULE module)
{
    ...
    if(gUniMgr[UNI_A].mConnectModule == module)
        UniModule = UNI_A;
    else if (gUniMgr[UNI_B].mConnectModule == module)
        UniModule = UNI_B;
