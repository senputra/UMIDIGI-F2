...
public:
    ...
    MBOOL getP2EnTuningTag(MBOOL isTopEngine, ISP_DRV_CQ_ENUM cq, MUINT32 magicNum, MUINT32 drvScenario,
            MUINT32 subMode, MUINT32 &tuningEnTag, MUINT32 &tuningDmaTag, MBOOL isV3 = MFALSE);
    MBOOL getP1EnTuningTag(MBOOL isTopEngine, ISP_DRV_CQ_ENUM cq, MUINT32 magicNum, MUINT32 drvScenario,
            MUINT32 subMode, MUINT32 &tuningEnTag, MUINT32 &tuningDmaTag, MBOOL isV3 = MFALSE);
...
public: //isp driver operationr
    ...
    inline int              flushIrq(ISP_DRV_WAIT_IRQ_STRUCT flushIrq){return (int)m_pPhyIspDrv->flushIrq(flushIrq);}
...
public:
    DMAO_B():dupCqIdx(0),burstQueIdx(0),CQ(CAM_ISP_CQ_NONE),isTwinMode(MFALSE),CQ_D(CAM_ISP_CQ_NONE)
        {dma_cfg.pixel_byte = 1;};
...
public:
    ...
    isp_reg_t *pTuningIspReg;  // p2 tuning data set by tuning provider
public:
    ISP_RGB_PIPE():en_p2(0),magicNum(0),dupCqIdx(0),burstQueIdx(0),drvScenario(0),sub_mode(0),CQ(CAM_ISP_CQ_NONE),rrzRatio(1.0),rrzCropX(0),rrzCropY(0),enFgMode(0),isV3(MFALSE),isApplyTuning(MFALSE),pTuningIspReg(NULL)
    {};
...
public:
    ...
    isp_reg_t *pTuningIspReg;  // p2 tuning data set by tuning provider
public:
    ISP_YUV_PIPE():en_p2(0),magicNum(0),dupCqIdx(0),burstQueIdx(0),drvScenario(0),sub_mode(0),CQ(CAM_ISP_CQ_NONE),rrzRatio(1.0),rrzCropX(0),rrzCropY(0),sl2cCrzRatio(1.0),lceInputW(0),lceInputH(0),fe_ctrl(0x0),isV3(MFALSE),isApplyTuning(MFALSE),pTuningIspReg(NULL)
    {};
    ~ISP_YUV_PIPE(){};
...
public:
    ...
    int tcm_en;
    int tdr_en;
    int tdr_dbg_tcm_en; // for control extension
    unsigned int magicNum;
    int drvScenario;
    int sub_mode;
    MUINT32 lceInputW;
...
public:
    ...
    MBOOL cal_and_setBandW(MUINT32 pass1_path, MUINT32 fps);
    MBOOL setBw(MUINT32 total_bw,MUINT32 fps,MBOOL immediate, MUINT32 pass1_path);
    MBOOL setMMDVFS(MUINT32 clk, MBOOL bIncClk);
private:
...
private:
    ...
    static MUINT32 m_recentFrmTimeMs[2];
...
public:
    ...
    static  MUINT32 estimateTimeout(void);
    static  MBOOL   updateFrameTime(MUINT32 timeInMs, MUINT32 reqOffset);
