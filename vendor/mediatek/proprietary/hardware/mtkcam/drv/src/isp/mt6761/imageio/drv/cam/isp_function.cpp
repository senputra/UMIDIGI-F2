...
//tpipe
#include <drv/tpipe_config.h>
#include <drv/utils.h>
#include "imageio/inc/mdp_mgr.h"
...
cam_isp_mfb_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    ISP_FUNC_DBG("+(bld_mode(0x%x),mfbout(%d,%d,%d),out_xofst(%d))", imageioPackage.mfbCfg.bld_mode,(int)imageioPackage.mfbCfg.mfb_out.w,\
        (int)imageioPackage.mfbCfg.mfb_out.h,(int)imageioPackage.mfbCfg.mfb_out.stride,imageioPackage.mfbCfg.out_xofst);
    ...
    if(imageioPackage.isApplyTuning){ //0x15004A20 ~ 0x15004A44
        ...
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_NSL2A){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            // update nsl2a
            // get raw nsl2 data from CQ set by tuningMgr just now
            // ToDo: Not in 6739 IP Base, should we remove?
            nsl2CentrXLsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_CEN, SL2_CENTR_X);
            nsl2CentrYLsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_CEN, SL2_CENTR_Y);
            nsl2Rr0Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON0, SL2_R_0);
            nsl2Rr1Lsc = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_NSL2_RR_CON0, SL2_R_1);
...
cam_isp_sl2c_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
    /*
     L2C_CENTR_X = NSL2_CENTR_X
     L2C_CENTR_Y = NSL2_CENTR_Y
     L2C_RR_0 = NSL2_R_0* NSL2_R_0
     L2C_RR_1 = NSL2_R_1* NSL2_R_1
     L2C_RR_2 = NSL2_R_2* NSL2_R_2
     L2C_HRZ_COMP = NSL2_HRZ_COMP = 2048
    */
...
cam_isp_g2g_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    ISP_FUNC_DBG("+,m_pP2IspDrv(%p),cq(%d),subMode(%d),isApplyTuning(0x%08x)",m_pP2IspDrv,imageioPackage.cq, imageioPackage.subMode,imageioPackage.isApplyTuning);
...
cam_isp_c42_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    UNUSED(imageioPackage);
    UNUSED(tuningPackage);
...
cam_isp_bnr_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    if(imageioPackage.isApplyTuning){  //0x15004800 ~ 0x1500482C
        ...
        //
        ISP_FUNC_DBG("cam_isp_bnr_cfg: BPC_LUT_EN (%d)", ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_BNR_BPC_CON, BPC_LUT_EN));
        if (ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_BNR_BPC_CON, BPC_LUT_EN) == 1) {
            m_pP2IspDrv->getCqModuleInf(CAM_DMA_BPCI,addrOffset,moduleSize);
            ...
            MUINT32 BPCI_XSIZE, BPCI_YSIZE;
            BPCI_XSIZE = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_BPCI_XSIZE, XSIZE);
            BPCI_YSIZE = ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_BPCI_YSIZE, YSIZE);
            // HW design: align P1 to do BPCI XSIZE and YSIZE -1
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_BPCI_XSIZE, XSIZE, BPCI_XSIZE -1);
            ISP_WRITE_BITS(m_pP2IspDrv, CAM_BPCI_YSIZE, YSIZE, BPCI_YSIZE -1);
            ISP_WRITE_REG(m_pP2IspDrv,CAM_BPCI_CON2,0x00080800,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(m_pP2IspDrv,CAM_BPCI_CON,0x80101020,ISP_DRV_USER_ISPF);
            ISP_FUNC_DBG("cam_isp_bnr_cfg: BPCI_XSIZE(%d), BPCI_YSIZE(%d)", BPCI_XSIZE, BPCI_YSIZE);
...
cam_isp_rrz_cfg(IspDrv* pCurDrv,IspSize in,IspRect crop,IspSize out,MUINT32 ctrl,MUINT32 h_step,MUINT32 v_step, MUINT32 isRaw_D) {
    ...
    ISP_FUNC_DBG("+[setRRZToCQ](ctrl(0x%x),in(%d,%d),out(%d,%d),step(0x%x,0x%x),ofst(%d,%d,%d,%d/%d,%d))", \
                 ctrl,(int)in.w,(int)in.h,(int)out.w,(int)out.h,h_step,v_step,crop.x,crop.floatX,crop.y,crop.floatY, (int)crop.w,(int)crop.h);
...
{
    public:
        IspDrvShellImp():mInitCount(0),mpM4UDrv(NULL),lsciSwWorkaroundBufVa(0x0),lsciSwWorkaroundBufFd(0){}

        static IspDrvShell* getInstance(NSImageioIsp3::NSIspio::EDrvScenario   eScenarioID = NSImageioIsp3::NSIspio::eDrvScenario_CC);
...
//
IspDrvShell* IspDrvShell::createInstance(NSImageioIsp3::NSIspio::EDrvScenario   eScenarioID)
{
...
IspDrvShell*
IspDrvShellImp::
getInstance(NSImageioIsp3::NSIspio::EDrvScenario eScenarioID)
{
//    ISP_FUNC_DBG("");
    ISP_FUNC_INF("eScenarioID: %d.", eScenarioID);
...
MUINT32 ISP_BUF_CTRL::m_recentFrmTimeMs[2] = {500, 500};
//
MBOOL
IspDrvShellImp::
...
int ret = 0;
    ...
    ISP_FUNC_DBG("[m_pIMemDrv]: %p",m_pIMemDrv);
    if ( NULL == m_pIMemDrv ) {
    ...
    ISP_FUNC_INF("[m_pPhyIspDrv]: %p",m_pPhyIspDrv);
    if (!m_pPhyIspDrv) {
    ...
    =============================================*/
    MINT32 curBurstQNum=P2_Support_BurstQNum;
    ...
    ISP_FUNC_DBG("[m_pCrzDrv]: %p",m_pP2CrzDrv);
    if ( NULL == m_pP2CrzDrv ) {
    ...
    ISP_FUNC_DBG("[m_pTwinDrv]: %p",m_pTwinDrv);
    if ( NULL == m_pTwinDrv ) {
    ...
    ISP_FUNC_INF("[m_pIspEventThread]: %p",m_pIspEventThread);
...
//js_test move to _disable.
    ...
    /*============================================
     isp driver
    =============================================*/
    ISP_FUNC_DBG("m_pP1IspDrv(%p),m_pP2IspDrv(%p),m_pPhyIspDrv_bak(%p)",m_pP1IspDrv,m_pP2IspDrv,m_pPhyIspDrv_bak);
    if (m_pPhyIspDrv_bak) {
...
MINT32 cq,MINT32 burstQIdx,MINT32 dupCqIdx,MINT32 isP2IspOnly
    ...
    switch(cq) {
        ...
        case CAM_ISP_CQ0B:
            ...
            break;
        case CAM_ISP_CQ0C:
            ISP_FUNC_DBG("CAM_ISP_CQ0C(%p),m_trigSsrc(%d)",m_pPhyIspDrv_bak->getCQDescBufPhyAddr(cq,burstQIdx,dupCqIdx),this->m_trigSsrc);
...
{
    ISP_FUNC_DBG("+,m_pPhyIspDrv(%p)",m_pPhyIspDrv_bak);
    //
    if (m_pPhyIspDrv_bak == NULL) {
        return MFALSE;
    }
...
updateBurstQueueNum(MINT32 burstQNum)
{
    ...
    MINT32 oldBurstQNum=P2_Support_BurstQNum;
...
controlBurstQueBuffer(EIspControlBufferCmd cmd, MUINT32 burstQueueNum)
{
    ...
    if(eIspControlBufferCmd_Alloc == cmd) {
        ...
        for(k=0;k<burstQueueNum;k++){
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    ...
                    ISP_FUNC_INF("[Tpipe]i(%d),j(%d)va(%p),pa(%d)",i,j,gpMdpMgrCfgData[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTableVa,gpMdpMgrCfgData[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTablePa);
                    #endif
                }
            }
        }
        ...
        m_pVirtIspDrv = (IspDrv**)malloc(totalCQNum * sizeof(IspDrv *));
        if(m_pVirtIspDrv==NULL)
        {
        ...
        m_pVirtIspReg=(isp_reg_t**)malloc(totalCQNum * sizeof(isp_reg_t *));
        if(m_pVirtIspReg==NULL)
        {
        ...
        for(int cq=0;cq<totalCQNum;cq++)
        {
            ...
            ISP_FUNC_DBG("[m_pVirtIspDrv[cq%d]]:%p",cq,m_pVirtIspDrv[cq]);
            if (!m_pVirtIspDrv[cq]) {
            ...
            ISP_FUNC_DBG("[m_pVirtIspReg[cq%d]]:%p",cq,m_pVirtIspReg[cq]);
        ...
        MUINT32 &tuningDmaTag)
{
    ...
    switch (drvScenario){
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_vFB_Normal:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_vFB_FB:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_CShot:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_MFB_Blending:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_MFB_Mixing:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_SShot:
            case NSImageioIsp3::NSIspio::eDrvScenario_VSS:
            ...
            break;
            case NSImageioIsp3::NSIspio::eDrvScenario_IP:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_RAW:
            case NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe:
            if(isTopEngine==MTRUE){
        ...
        MBOOL isV3)
{
    ...
    MBOOL ret = MTRUE;
    //
    if (m_pPhyIspDrv->getP2TuningStatus((ISP_DRV_CQ_ENUM)cq) || isV3) {
        // check and set tuning en and DMA tag
        switch (drvScenario){
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_vFB_Normal:
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_vFB_FB:
            ...
            case NSImageioIsp3::NSIspio::eDrvScenario_CC_SShot:
            case NSImageioIsp3::NSIspio::eDrvScenario_VSS:
            case NSImageioIsp3::NSIspio::eDrvScenario_VSS_MFB_Blending:
            case NSImageioIsp3::NSIspio::eDrvScenario_VSS_MFB_Mixing:
                if(isTopEngine==MTRUE){
...
int DMAI_B::_config( void )
{
    ...
    ISP_FUNC_DBG("DMAI(%d),[%s]:m_pP2IspDrv(%p),pa(%p),ofst(0x%08X),size(%d,%d,%d,%d),pixel_byte(%d),crop(%d,%d), flip(%d),isTwin(%d),CQ(0x%x/0x%x) ", \
        (int)this->id(), \
        this->name_Str(), \
        m_pP2IspDrv, \
        (void *)this->dma_cfg.memBuf.base_pAddr, \
        this->dma_cfg.memBuf.ofst_addr,
        (int)this->dma_cfg.size.w, \
        (int)this->dma_cfg.size.h, \
        (int)this->dma_cfg.size.stride, \
        (int)this->dma_cfg.size.xsize, \
    ...
    switch(this->id())
    {
        case ISP_DMA_TDRI:
            ...
            //
            ISP_FUNC_DBG("CAM_TDRI_BASE_ADDR = %p",(void *)this->dma_cfg.memBuf.base_pAddr);
            break;
        case ISP_DMA_CQI:
            ...
            ISP_FUNC_DBG("[afterCQSwitch]m_pP2IspDrv(%p)",m_pP2IspDrv);
            break;
        case ISP_DMA_UFDI:
            ...
            //
            ISP_FUNC_DBG("CAM_UFDI_BASE_ADDR=[%p], user=%d", (void *)(this->dma_cfg.memBuf.base_pAddr + this->dma_cfg.memBuf.ofst_addr),ISP_DRV_USER_ISPF);
            break;
        case ISP_DMA_BPCI:
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_BPCI_D_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
        ...
        case ISP_DMA_LCEI:
            ...
            //
            ISP_FUNC_DBG("CAM_LCEI_BASE_ADDR=[%p], user=%d",(void *)(this->dma_cfg.memBuf.base_pAddr + this->dma_cfg.memBuf.ofst_addr),ISP_DRV_USER_ISPF);
            break;
        case ISP_DMA_IMGI:
            ...
            ISP_FUNC_DBG("DMAI(%d),(%s):reg(0x%08x) ", (int)this->id(), this->name_Str(), ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_CQ_EN));
            break;
        case ISP_DMA_VIPI:
            ...
            #endif
            //
            ISP_FUNC_DBG("ISP_DMA_VIPI = %p",(void *)(this->dma_cfg.memBuf.base_pAddr));
            break;
        case ISP_DMA_VIP2I:
            ...
            #endif
            //
            ISP_FUNC_DBG("CAM_VIP2I_BASE_ADDR = %p",(void *)(this->dma_cfg.memBuf.base_pAddr));
            break;
        case ISP_DMA_VIP3I:
            ...
            #endif
            //
            ISP_FUNC_DBG("CAM_VIP3I_BASE_ADDR = %p",(void *)(this->dma_cfg.memBuf.base_pAddr));
            break;
...
int DMAI_B::_disable( void )
{
    ...
    ISP_FUNC_DBG("[id:0x%08d] ", (int)this->id());
...
unsigned int cq_module_id = 0xFF;
    ...
    ISP_FUNC_DBG("_,[id:0X%08X] ", (int)this->id());
...
int DMAO_B::_config( void )
{
    ...
    IspDrv* _targetVirDrv = NULL;
    MUINT32 _targetDupQIdx = 0;
    ISP_FUNC_DBG("DMAO(%d),[%s]:cq(%d/%d),pa(%p),ofst(0x%X),size(%d,%d,%d,%d), pixel_byte(%d),cropW(%d,%d,%d,%d),format(0x%x),fps(%d),cropX_inbussize(%d)", \
        (int)this->id(), \
        this->name_Str(), \
        this->CQ, this->CQ_D,\
        (void *)this->dma_cfg.memBuf.base_pAddr, \
        ...
        this->dma_cfg.crop.x, \
        this->dma_cfg.crop.y,\
        (int)this->dma_cfg.crop.w,\
        (int)this->dma_cfg.crop.h,\
        this->dma_cfg.format,\
        this->dma_cfg.tgFps,\
    ...
    switch(this->id())
    {
        ...
        case ISP_DMA_AFO:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_AFO_YSIZE,this->dma_cfg.size.h - 1,ISP_DRV_USER_ISPF);//ySize;
            ISP_WRITE_REG(_targetVirDrv,CAM_AFO_STRIDE,this->dma_cfg.size.stride,ISP_DRV_USER_ISPF);//stride;
            ISP_WRITE_REG(_targetVirDrv,CAM_AFO_CON2,0x002A2A00,ISP_DRV_USER_ISPF);
            //ISP_WRITE_REG(_targetVirDrv,CAM_AFO_D_CON,0x80202020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_AFO_CON,0x80404080,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_AFO_OFST_ADDR,this->dma_cfg.memBuf.ofst_addr,ISP_DRV_USER_ISPF);
        ...
        case ISP_DMA_AFO_D:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_AFO_D_CON2,0x002A2A00,ISP_DRV_USER_ISPF);
            //ISP_WRITE_REG(_targetVirDrv,CAM_AFO_D_CON,0x80202020,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_AFO_D_CON,0x80404080,ISP_DRV_USER_ISPF); // ultra-
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_ESFKO_CON,0x80000020,ISP_DRV_USER_ISPF); // ultra-
        ...
        case ISP_DMA_IMGO:
            ...
            // SP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON,0x801010B0,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON2,0x00555500,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON,0x81808000,ISP_DRV_USER_ISPF); // ultra--
        ...
        case ISP_DMA_RRZO:
            ...
            // SP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x80202080,ISP_DRV_USER_ISPF); // ultra-
            ...
            ISP_FUNC_DBG("ISP_DMA_RRZO, pxl_byte=%d, w=0x%x,x=0x%x,stride=0x%x, cropXY=%d,%d",this->dma_cfg.pixel_byte, (int)this->dma_cfg.size.w, (int)this->dma_cfg.size.xsize, (int)this->dma_cfg.size.stride,this->dma_cfg.crop.y, this->dma_cfg.crop.x);
            //currently , slow motion will map to this case
            if(this->m_fps > FPS_BOUNDARY){
#if SRAM_SWITCH
                ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x80404080,ISP_DRV_USER_ISPF); // ultra-
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON2,0x00B0B000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON,0x81808000,ISP_DRV_USER_ISPF); //ultra--
            ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_D_CON2,0x00555500,ISP_DRV_USER_ISPF);
...
//            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_D_CON2,0x00808000,ISP_DRV_USER_ISPF);
            ...
            ISP_FUNC_DBG("ISP_DMA_RRZOD, pxl_byte=%d, w=0x%x,x=0x%x,stride=0x%x, cropXY=%d,%d",\
               this->dma_cfg.pixel_byte, (int)this->dma_cfg.size.w, (int)this->dma_cfg.size.xsize, (int)this->dma_cfg.size.stride,\
        ...
        case ISP_DMA_IMG2O:
            ...
            ISP_WRITE_REG(m_pP2IspDrv,CAM_IMG2O_CON,0x80406060,ISP_DRV_USER_ISPF);//default 0x80404040
            ISP_WRITE_REG(m_pP2IspDrv,CAM_IMG2O_CON2,0x00406000,ISP_DRV_USER_ISPF);//default 0x00404000
...
int DMAO_B::_write2CQ(void)
{
    ...
    unsigned int myCq = CAM_ISP_CQ_NONE;
    ISP_FUNC_DBG("+,");
    ISP_FUNC_DBG("[id:0X%08X],CQ(%d)", (int)this->id(),this->CQ);
...
ISP_RAW_CENTRAL_CTRL_INFO::config2( ISP_TOP_CTRL * pSrcTopCtrl )
{
    ...
    if ( ISP_PASS1 == pSrcTopCtrl->path || ISP_PASS1_D == pSrcTopCtrl->pathD ) {
       ...
       if (0 == mEnablePath) {
           ...
           ISP_FUNC_INF("RAW_CENTRAL, ispdrv/ispdrvD=%p/%p\n", mRawIspDrv, mRawDIspDrv);
...
int ISP_TOP_CTRL::_config( void )
{
    ...
    if ( CAM_ISP_CQ_NONE != this->CQ ) {
        ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_MAX,(_isp_dma_enum_)this->CQ,0,0,0};
        this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl);
    }
    if ( CAM_ISP_CQ_NONE != this->CQ_D ) {
        ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_MAX,(_isp_dma_enum_)this->CQ_D,0,0,0};
        this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl);
    }
    ...
    if(logPath == ISP_PASS2) { // P2
        ...
        ISP_FUNC_DBG("[P2]en(0x%08x),EnDma(0x%08x),ctlSel(0x%08x),intEn(0x%08x),fmtSel(0x%08x),cqEn(0x%08x)", \
        ...
        ISP_FUNC_INF("[CAMSV]EN(0x%08x/0x%08x),fmt(0x%08x/0x%08x)\n", \
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_MODULE_EN),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_MODULE_EN),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_FMT_SEL),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_FMT_SEL));
        #endif
    }
#ifdef _FBC_IN_IPRAW
...
#endif
    ISP_FUNC_VRB("m_pP1IspDrv(%p),m_pP2IspDrv(%p),m_pPhyIspDrv(%p)",m_pP1IspDrv,m_pP2IspDrv,m_pPhyIspDrv);
    ...
    if ( this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Bits.TG_EN ) {
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_INT_EN ,this->isp_top_ctl.CAMSV_CAMSV_INT_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_MODULE_EN,this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_PAK,0x03,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_FMT_SEL,0x01000000, ISP_DRV_USER_ISPF);
    }
    if ( this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Bits.TG_EN ) {
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_INT_EN ,this->isp_top_ctl.CAMSV_CAMSV2_INT_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_MODULE_EN,this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_PAK,0x03,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_FMT_SEL,0x01000000, ISP_DRV_USER_ISPF);
    }
    ...
    if ( ISP_PASS1   == this->path || ISP_PASS1_D == this->pathD ) {
        ...
        /* disable P1 dma dcm + imgi dcm */
        ISP_WRITE_REG(m_pPhyIspDrv,CAM_CTL_DMA_DCM_DIS,0x7EFF001,ISP_DRV_USER_ISPF);
        ISP_FUNC_INF("CAM_CTL_DMA_DCM_DIS: 0x%x\n", ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_DMA_DCM_DIS));
        /* disable TOP_DCM + CQ_DCM */
        ISP_WRITE_REG(m_pPhyIspDrv,CAM_CTL_TOP_DCM_DIS,0x3,ISP_DRV_USER_ISPF);
        ISP_FUNC_INF("CAM_CTL_TOP_DCM_DIS: 0x%x\n", ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_TOP_DCM_DIS));
        ...
        if (ISP_PASS1_CAMSV == this->path) {
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_INT_EN,ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_INT_EN)  |ptop->isp_top_ctl.CAMSV_CAMSV_INT_EN.Raw,ISP_DRV_USER_ISPF);
        } else if (ISP_PASS1_CAMSV_D == this->pathD) {
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_INT_EN,ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_INT_EN)|ptop->isp_top_ctl.CAMSV_CAMSV2_INT_EN.Raw,ISP_DRV_USER_ISPF);
        }
        ...
        for (int ii=0; ii < 2; ii++)  {
           ...
           case ISP_PASS1_CAMSV:
               ...
               #endif
               if(ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv,CAMSV_MODULE_EN,TG_EN)){
                 ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CLK_EN,0x8005,ISP_DRV_USER_ISPF);
               }
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_PAK, \
                     ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_PAK)&(~this->isp_top_ctl.CAMSV_CAMSV_PAK_MASK) ) | \
                     this->isp_top_ctl.CAMSV_CAMSV_PAK.Raw,ISP_DRV_USER_ISPF);
                     ISP_WRITE_REG(m_pPhyIspDrv, \
                     CAMSV_FMT_SEL, \
                     ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_FMT_SEL)&(~this->isp_top_ctl.CAMSV_CAMSV_FMT_SEL_MASK) ) | \
           ...
           case ISP_PASS1_CAMSV_D:
               ...
               #endif
               if(ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv,CAMSV2_MODULE_EN,TG_EN)){
                   ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_CLK_EN,0x8005,ISP_DRV_USER_ISPF);
               }
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_PAK, \
                      ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_PAK)&(~this->isp_top_ctl.CAMSV_CAMSV2_PAK_MASK) ) | \
                      this->isp_top_ctl.CAMSV_CAMSV2_PAK.Raw,ISP_DRV_USER_ISPF);
                      ISP_WRITE_REG(m_pPhyIspDrv, \
                      CAMSV2_FMT_SEL, \
                      ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_FMT_SEL)&(~this->isp_top_ctl.CAMSV_CAMSV2_PAK_MASK) ) | \
    ...
    if ((this->path==ISP_PASS2)||(this->path==ISP_PASS2B)||(this->path==ISP_PASS2C))
    {
        ...
        if (this->isV3)
        {
            ...
            if(this->isp_top_ctl.CAM_CTL_SCENARIO.Bits.SCENARIO==ISP_HW_SCENARIO_IP)
            {
                ...
                if (this->drvScenario == NSImageioIsp3::NSIspio::eDrvScenario_IP)
                {
                    p2EnConfig = 0x40000000; // For IP Raw path, p2 module should not be turned on except for UNP and IMGI port.
                }
            }
            ISP_FUNC_DBG("cq(%d),P2EnDmaConfig(0x%08x),p2EnConfig(0x%08x)", this->CQ,P2EnDmaConfig,p2EnConfig);
        ...
        else
        {
            ...
            if (isApplyTuning) {
                ...
                // get P2 tuning En & Dma Tag
                getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
                        this->sub_mode, p2TuningEnTag, p2TuningDmaTag, this->isV3);
                ...
                if (this->isp_top_ctl.CAM_CTL_SCENARIO.Bits.SCENARIO == ISP_HW_SCENARIO_IP) {
                    // get P2 tuning En & Dma Tag
                    getP1EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum,
                               this->drvScenario, this->sub_mode, p2TuningP1EnTag, p2TuningP1DmaTag,
                               this->isV3);
        ...
        ISP_FUNC_INF("cq(%d),isApplyTuning for HAL1(%d),p2TuningEn(0x%08x),p2TuningEnTag(0x%08x),P2EnDmaConfig(0x%08x),p2TuningDmaTag(0x%08x),p2EnConfig(0x%08x),p2TuningDma(0x%08x)",
        ...
        {
            ISP_WRITE_REG(m_pP2IspDrv,CAM_CTL_EN_P1, 0x0,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(m_pP2IspDrv,CAM_CTL_EN_P1_DMA, 0x0,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(m_pP2IspDrv, CAM_CTL_FMT_SEL_P1, 0x0, ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(m_pP2IspDrv,CAM_CTL_SEL_P1,0x0,ISP_DRV_USER_ISPF);
        }
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if ( CAM_ISP_PASS1_START == (*start) ) {
        ...
        if( CAMSV_CAMSV_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Raw){
            ...
            //ISP_WRITE_BITS(this->m_pPhyIspReg, CAMSV_IMGO_FBC, DROP_INT_EN, 1);
            ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_IMGO_FBC,FB_NUM,size,ISP_DRV_USER_ISPF);
            ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_IMGO_FBC,FBC_EN,0,ISP_DRV_USER_ISPF);
            usleep(10);
            ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_IMGO_FBC,FBC_EN,1,ISP_DRV_USER_ISPF);
#endif
        }
        //CAMSV2 FBC
        if( CAMSV_CAMSV2_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Raw){
            ...
            //ISP_WRITE_BITS(this->m_pPhyIspReg, CAMSV2_IMGO_FBC, DROP_INT_EN, 1);
            ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV2_IMGO_FBC,FB_NUM,size,ISP_DRV_USER_ISPF);
            ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV2_IMGO_FBC,FBC_EN,0,ISP_DRV_USER_ISPF);
            usleep(10);
            ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV2_IMGO_FBC,FBC_EN,1,ISP_DRV_USER_ISPF);
#endif
        }
        //TG1
        ...
        if( CAMSV_CAMSV_MODULE_TG_EN & this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Raw){
              irq_TG_ClrDone.Status = CAMSV_INT_STATUS_PASS1_DON_ST;
         ...
         if( CAMSV_CAMSV2_MODULE_TG_EN & this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Raw){
               irq_TG_ClrDone.Status = CAMSV2_INT_STATUS_PASS1_DON_ST;
    ...
    static int pass2_cnt = 0;
        ...
        if (CAM_ISP_PASS1_START == *start) {
            ...
            ISP_FUNC_INF("camsv: imgo_dma_en(0x%x/0x%x),addr(0x%x/0x%x)(0x%x/0x%x)", \
                ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv,CAMSV_MODULE_EN,IMGO_EN_),\
                ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv,CAMSV2_MODULE_EN,IMGO_EN_), \
...
int ISP_TOP_CTRL::_disable( void )
{
    MBOOL ret = MTRUE;
    MINT32 errSta;
    /* "adb shell setprop vendor.camera.p1wait.ms 3000" for sensor long exposure 3s. */
    char waitValue[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.p1wait.ms", waitValue, "0");
    int waitMs=atoi(waitValue);
    MUINT32 estTimeout = ISP_BUF_CTRL::estimateTimeout();

    if (estTimeout > waitMs) {
        waitMs = estTimeout;
    }

    if (waitMs <= CAM_INT_WAIT_TIMEOUT_MS)
        waitMs = CAM_INT_WAIT_TIMEOUT_MS;
    ISP_FUNC_INF("[property] waitMs(%d)\n", waitMs);
    ...
    irq_csv_int.Status=CAMSV_INT_STATUS_PASS1_DON_ST;
    irq_csv_int.UserNumber=(int)ISP_DRV_IRQ_USER_ISPDRV;
    irq_csv_int.Timeout=waitMs;
    ...
    irq_csv_int.UserInfo.Status=CAMSV_INT_STATUS_PASS1_DON_ST;
    ...
    irq_csv2_int.Status=CAMSV2_INT_STATUS_PASS1_DON_ST;
    irq_csv2_int.UserNumber=(int)ISP_DRV_IRQ_USER_ISPDRV;
    irq_csv2_int.Timeout=waitMs;
    ...
    irq_csv2_int.UserInfo.Status=CAMSV2_INT_STATUS_PASS1_DON_ST;
    ...
    if ( bP1Path ) {
        ...
        */
        if (ISP_PASS1 == this->path) {
            ...
            //if( CAMSV_MODULE_TG_EN & this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Raw)
            {
                ...
                ISP_FUNC_INF("cam_sv module_en:(0x%x)",ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_MODULE_EN));
            }
            //if( CAMSV2_MODULE_TG_EN & this->isp_top_ctl.CAMSV_CAMSV2_MODULE_EN.Raw)
            {
                ...
                ISP_FUNC_INF("cam_sv2 module_en:(0x%x)",ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_MODULE_EN));
            }
        ...
        if ( (ISP_PASS1_CAMSV == this->path) || (ISP_PASS1_CAMSV == this->pathD)) {
            ...
            if( CAMSV_CAMSV_MODULE_TG_EN & this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Raw){
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON, VFDATA_EN,0,ISP_DRV_USER_ISPF);
                irq_vs_int.Status = CAMSV_INT_STATUS_VS1_ST;
            }
            if(CAMSV_CAMSV_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV_CAMSV_MODULE_EN.Raw) {
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_IMGO_FBC,FBC_EN,0,ISP_DRV_USER_ISPF);
            }
...
///////////////////E
        ...
        while(_clrPort--){
            ISP_FUNC_DBG("[TopCtrl::Disable][before]rt_dma(%d),size:hw(%d)",_dma[_clrPort], (int)ISP_BUF_CTRL::m_hwbufL[_dma[_clrPort]].bufInfoList.size());
            buf_ctrl.ctrl = ISP_RT_BUF_CTRL_CLEAR;
            buf_ctrl.buf_id = (_isp_dma_enum_)_dma[_clrPort];
            buf_ctrl.data_ptr = &dummy;
            buf_ctrl.pExtend = 0;
            ISP_FUNC_DBG("rtBufCtrl.ctrl(%d)/id(%d)/ptr(%p)",buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.data_ptr);
            if ( MTRUE != this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl) ) {
                ISP_FUNC_ERR("ERROR:rtBufCtrl");
                return -1;
            }
            ISP_FUNC_DBG("[TopCtrl::Disable][after]rt_dma(%d),size:hw(%d) ",_dma[_clrPort], (int)ISP_BUF_CTRL::m_hwbufL[_dma[_clrPort]].bufInfoList.size());
        }
        ...
        ISP_FUNC_INF("clr buf list,p2Cq(%d),dupCqIdx(%d)",p2Cq,this->dupCqIdx);
...
int ISP_TOP_CTRL::abort(void)
{
    ...
    MUINT32 wait_vs_num = 1;
    Mutex   *_localVar;
    ISP_FUNC_INF("ISP_TOP_CTRL::_abort:E path(%x/%x)",this->path,this->pathD);
        ...
        */
        if (ISP_PASS1 == this->path) {
            ...
            //
            {
                while ((pactive_irq_vs_int[0]) && ( wait_vs_num-- )) {
                    //ret = this->m_pPhyIspDrv->waitIrq( pactive_irq_vs_int[0] );
                    ret = MTRUE; // Due to abort long exposure, skip to wait vsync
                    if ( MFALSE == ret ) {
                        ISP_FUNC_ERR("waitIrq( irq_TG1_DONE ) fail");
                    }
                    else{
                        ...
                        if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAM_TG_INTER_ST,TG_CAM_CS) != 1){
                            ISP_FUNC_INF("[TG1]TG is not idle, wait 1 VD again.\n");
                            //ret = this->m_pPhyIspDrv->waitIrq( pactive_irq_vs_int[0] );
                            ret = MTRUE; // Just abort long exposure, skip to wait vsync. Try SW_RST_Trig if needed
                            if ( MFALSE == ret ) {
                                ISP_FUNC_ERR("waitIrq( irq_TG1_DONE ) fail");
                            }
                            else{
...
///////////////////E
    ...
    ISP_FUNC_DBG("ISP_TOP_CTRL::_abort:X ");
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        if (this->isV3)
        {
            ...
            if(imageioPackage.isApplyTuning==MTRUE){
                ...
                isp_reg_t *pTuningRegMappingQue = (isp_reg_t *)tuningPackage.pTuningBuf;
                tuningEnP1 = pTuningRegMappingQue->CAM_CTL_EN_P1.Raw;
                tuningEnP2 = pTuningRegMappingQue->CAM_CTL_EN_P2.Raw;
                tuningEnP1Dma = pTuningRegMappingQue->CAM_CTL_EN_P1_DMA.Raw;
                ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),pTuningBuf(%p)",(long)tuningUpdateBit,(void *)tuningPackage.pTuningBuf);
            }
            ...
            getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
                    this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag, this->isV3);

            if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario){
                getP1EnTuningTag(MFALSE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
                        this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag, this->isV3);
                finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
            }
            else if(NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe == this->drvScenario){
                getP1EnTuningTag(MFALSE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
                        this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag, this->isV3);
                finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
                ISP_FUNC_DBG("(ISP_RAW_PIPE),finalEnP1(0x%x),(~tuningEnP1Tag)(0x%x),this->en_p1(0x%x),tuningEnP1Tag(0x%x), tuningEnP1(0x%x)",finalEnP1, (~tuningEnP1Tag),this->en_p1, tuningEnP1Tag, tuningEnP1);
            }
            finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
            imageioPackage.finalEnP2 = finalEnP2;

            ISP_FUNC_DBG("(ISP_RAW_PIPE),isApplyTuning(0x%x),finalEnP1(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%016lx), en_p1(0x%x)",this->isApplyTuning, finalEnP1, finalEnP2, (long)tuningUpdateBit, this->en_p1);
        }
        else
        {
            getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
            ...
            if(imageioPackage.isApplyTuning==MTRUE){
                ...
                ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(%p)",(long)tuningUpdateBit,tuningEnP2,tuningEnP2Dma,(void *)tuningPackage.pTuningBuf);
        }
            if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario){
                finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
        ...
        if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario){ // only for scenario ip
        ...
        else if(NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe == this->drvScenario){ // only for scenario ip
            for(i=0;i<CHECK_TUNING_P1_EN_BITS;i++){
                if((finalEnP1 & gIspModuleMappingTableP1En[i].ctrlByte) &&
                        (gIspModuleMappingTableP1En[i].eModule==eIspModuleRaw) &&
                        (gIspModuleMappingTableP1En[i].default_func!=NULL)) {
            ...
            if (finalEnP1 & CAM_CTL_EN_P1_BNR_EN) {
                isp_reg_t *pTuningRegMappingQue;
                pTuningRegMappingQue = (isp_reg_t *)tuningPackage.pTuningBuf ;
                if(1 == pTuningRegMappingQue->CAM_BNR_BPC_CON.Bits.BPC_LUT_EN){
                    ISP_WRITE_BITS(m_pP2IspDrv, CAM_CTL_EN_P1_DMA, BPCI_EN, 1, ISP_DRV_USER_ISPF);
                    ISP_WRITE_BITS(m_pP2IspDrv, CAM_BNR_BPC_TBLI2, BPC_XSIZE, this->src_img_w, ISP_DRV_USER_ISPF);
                    ISP_WRITE_BITS(m_pP2IspDrv, CAM_BNR_BPC_TBLI2, BPC_YSIZE, this->src_img_h-1, ISP_DRV_USER_ISPF);
                }
                else
                    ISP_WRITE_BITS(m_pP2IspDrv, CAM_CTL_EN_P1_DMA, BPCI_EN, 0, ISP_DRV_USER_ISPF);
                ISP_FUNC_DBG("(ISP_RAW_PIPE),src_img_w (%d), src_img_h(%d), BPC_LUT_EN(0x%x)", this->src_img_w, this->src_img_h, pTuningRegMappingQue->CAM_BNR_BPC_CON.Bits.BPC_LUT_EN);
            }
        }
    if (!this->isV3)
    {
        ...
        // work around for AF fifo issue, use DMX line buffer to temp data
        ISP_FUNC_DBG("DMX is always enabled!");
        if( (this->en_p1 & CAM_CTL_EN_P1_DMX_EN) && (ISP_PASS1  == this->path)) {
            ISP_WRITE_REG(_targetVirDrv,CAM_DMX_CROP, ((this->src_img_w-1)<<16),ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_DMX_VSIZE, this->src_img_h,ISP_DRV_USER_ISPF);
        }
        if( (this->en_p1_d & CAM_CTL_EN_P1_D_DMX_D_EN) && (ISP_PASS1_D   == this->pathD)) {
            ISP_WRITE_REG(_targetVirDrv,CAM_DMX_D_CROP, ((this->src_img_w-1)<<16),ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_DMX_D_VSIZE, this->src_img_h,ISP_DRV_USER_ISPF);
        }
#endif

        if(this->isTwinMode && (this->en_p1 &CAM_CTL_EN_P1_HBIN2_EN)){
            ISP_FUNC_DBG("isTwinMode,path(0x%x), tg w/h=0x%x,0x%x",path, this->src_img_w,this->src_img_h);

            ISP_WRITE_REG(_targetVirDrv,CAM_HBN2_SIZE, (this->src_img_h<<16)||this->src_img_w,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(_targetVirDrv,CAM_HBN2_MODE, (0x1<<4),ISP_DRV_USER_ISPF);
        }
        if(this->isTwinMode && (this->en_p1 &CAM_CTL_EN_P1_HBIN1_EN)){ // need to check
            ISP_FUNC_DBG("isTwinMode,path(0x%x), tg w/h=0x%x,0x%x",path, this->src_img_w,this->src_img_h);
        ...
        if(CAM_CTL_EN_P1_RRZ_EN & this->en_p1 && 1 == this->isTwinMode) {
            ...
            ISP_FUNC_DBG("isTwinMode,path(0x%x), m_pIspDrvShell=%p, m_pTwinDrv=%p",path, m_pIspDrvShell, this->m_pIspDrvShell->m_pTwinDrv);
            ...
            ISP_FUNC_DBG("ISP_RAW_PIPE::_config(setP1RrzCfg:0x%x_0x%x):Twin lsc_en=%d E:path(%d),rrz_in_w/h(%d/%d),rrz_crop_x/y/fx/fy/w/h(%d/%d/%d/%d/%d/%d),rrz_out_w/h(%d/%d), sd_lw/xn(%d/%d),m_num(0x%x)", \
                this->path,this->pathD,\
                (int)rrz_cfg.lsc_en,\
                (int)rrz_cfg.pass1_path, \
                (int)rrz_cfg.rrz_in_size.w, \
                (int)rrz_cfg.rrz_in_size.h, \
                rrz_cfg.rrz_in_roi.x, \
                rrz_cfg.rrz_in_roi.y, \
                rrz_cfg.rrz_in_roi.floatX, \
                rrz_cfg.rrz_in_roi.floatY, \
                (int)rrz_cfg.rrz_in_roi.w, \
                (int)rrz_cfg.rrz_in_roi.h, \
                (int)rrz_cfg.rrz_out_size.w, \
                (int)rrz_cfg.rrz_out_size.h, \
                (int)rrz_cfg.sd_lwidth, \
                (int)rrz_cfg.sd_xnum, \
                (int)rrz_cfg.m_num_0);
            ...
            if( ISP_PASS1   == this->path){
                ...
                rrz_cfg.twin_lsc_crop_offx  = this->twin_lsc_crop_offx;
                rrz_cfg.twin_lsc_crop_offy  = this->twin_lsc_crop_offy;
                rrz_cfg.m_num_0             = this->m_num_0_d;
            }
...
int ISP_RAW_PIPE::_write2CQ(void)
{
    ...
    if ( ISP_PASS2 == this->path || ISP_PASS2B == this->path || ISP_PASS2C == this->path) {
        ...
        if(this->isV3)
        {
            ...
            getP1EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
                    this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag, this->isV3);
            getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
                    this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag, this->isV3);
            ...
            if(isApplyTuning==MTRUE){
                ...
                isp_reg_t *pTuningRegMappingQue = (isp_reg_t *)this->pTuningIspReg;
                if (pTuningRegMappingQue == NULL){
                    ISP_FUNC_ERR("[Error]pTuningIspReg is null pointer");
                    return 0;
                }
                tuningEnP1 = pTuningRegMappingQue->CAM_CTL_EN_P1.Raw;
                tuningEnP2 = pTuningRegMappingQue->CAM_CTL_EN_P2.Raw;
                ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),pTuningBuf(%p)",(long)tuningUpdateBit, pTuningRegMappingQue);
            }
            finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);


            if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario || NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe == this->drvScenario)
                finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
        ...
        if(isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x)",(long)tuningUpdateBit,tuningEnP2,tuningEnP2Dma);
        }
        if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario){
            finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
        ...
        if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario || NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe == this->drvScenario) {
            ...
            ISP_FUNC_DBG("+(ISP_RAW_PIPE), src_img_w(%d), src_img_h(%d)", src_img_w, src_img_h);
        }
...
int ISP_RAW_PIPE::_suspendTuneCfg(MUINT32 mNum)
{
    ...
    if(mNum & _DUMMY_MAGIC){
        ...
        if(this->m_Dmao_Reg[_targetDupQueIdx] == 0xffffffff){
            ...
            ISP_FUNC_DBG("path:0x%x B:(0x%x):0x%xn",path,_targetDupQueIdx,_reg);
        return 1;
    }
    return 0;
}
...
int ISP_RAW_PIPE::setP1TuneCfg(MUINT32 mNum, MINT32 senDev)
{
    ...
    ISP_FUNC_DBG("P1Dbg_RPG: senDev=%d, mNum=0x%x, UpdateBit=0x%016lx, EnP1=0x%x, EnP1Dma=0x%x, pBuf=%p", senDev, mNum, (long)cfg.tuningUpdateBit, cfg.tuningEnP1,cfg.tuningEnP1Dma, cfg.pTuningBuf);

    if (!cfg.pTuningBuf) {
        ISP_FUNC_ERR(" pTuningBuf=%p",  cfg.pTuningBuf);
        return -1;
    }
    ...
    if (ISP_PASS1 == this->path){
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Bnr) {
            ...
            MUINT32 grabPxl, grabLin;
            grabPxl = ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv, CAM_TG_SEN_GRAB_PXL, PXL_E) - ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv, CAM_TG_SEN_GRAB_PXL, PXL_S);
            grabLin = ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv, CAM_TG_SEN_GRAB_LIN, LIN_E) - ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv, CAM_TG_SEN_GRAB_LIN, LIN_S);
            ...
            if (ISP_READ_BITS_NOPROTECT(_targetVirDrv, CAM_BNR_BPC_CON, BPC_LUT_EN) == 1) {
            // if (cfg.tuningEnP1Dma & CAM_CTL_EN_P1_DMA_BPCI_EN) {
                ...
                ISP_WRITE_BITS(_targetVirDrv, CAM_BNR_BPC_TBLI2, BPC_XSIZE, grabPxl, ISP_DRV_USER_ISPF);
                ISP_WRITE_BITS(_targetVirDrv, CAM_BNR_BPC_TBLI2, BPC_YSIZE, grabLin-1, ISP_DRV_USER_ISPF);
                ISP_FUNC_DBG("BPCI_EN x=%x y=%x", grabPxl, grabLin-1);
            }else {
                //Disable
            ...
            bpci.dma_cfg.size.h = _pSrcReg->CAM_BPCI_YSIZE.Raw; //align 4.0 BNR & PDC
            bpci.dma_cfg.size.stride = _pSrcReg->CAM_BPCI_STRIDE.Raw;
            bpci.dma_cfg.size.xsize = _pSrcReg->CAM_BPCI_XSIZE.Raw; //align 4.0 BNR & PDC
            bpci.CQ = this->CQ;
            bpci.dupCqIdx = this->dupCqIdx;
            bpci.burstQueIdx = this->burstQueIdx;
            bpci.config();
            if(this->isTwinMode){
                //special case for twin_drv, special case: bpci baseaddr
                ISP_WRITE_REG(_targetVirDrv, CAM_BPCI_D_BASE_ADDR, ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_BPCI_BASE_ADDR), ISP_DRV_USER_ISPF);   //0x749c
            }
        }
    ...
    if (ISP_PASS1_D == this->pathD){
        ...
        ISP_FUNC_INF("_targetVirDrv_D = %p",_targetVirDrv);
        if(_targetVirDrv == NULL){
        ...
        if (cfg.tuningUpdateBit & eIspTuningMgrFunc_Rmg_d) {
            ...
            if (cfg.tuningEnP1 & CAM_CTL_EN_P1_D_RMG_D_EN) {
               //Enable
               memcpy((((MUINT8*)cqDVirtReg)        + (outInfo.addr_ofst)), \
                      (((MUINT8*)(cfg.pTuningBuf))  + (outInfo.addr_ofst)), \
                      (outInfo.reg_num)*sizeof(UINT32));
...
int ISP_RGB_PIPE::_config( void )
{
    ...
    if(this->isV3)
    {
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),pTuningBuf(%p),hrz_crop(%d,%d)", \
                (long)tuningUpdateBit,tuningPackage.pTuningBuf,tuningPackage.rrzCropX,tuningPackage.rrzCropY);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        imageioPackage.finalEnP2 = finalEnP2;

        ISP_FUNC_DBG("(ISP_RGB_PIPE),isApplyTuning(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%016lx)",this->isApplyTuning,finalEnP2,(long)tuningUpdateBit);
    }
    else
    {
    getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
    ...
    if(imageioPackage.isApplyTuning==MTRUE){
        ...
        tuningPackage.rrzCropX = this->rrzCropX;
        tuningPackage.rrzCropY = this->rrzCropY;
        ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(%p)",(long)tuningUpdateBit,tuningEnP2,tuningEnP2Dma,tuningPackage.pTuningBuf);
    }
    finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
    }
    ...
    if(this->en_p2 & CAM_CTL_EN_P2_MFB_EN)
    {
        ...
        ISP_FUNC_DBG("[TEST] imagioPak->mfb_cfg.bld_mode(%d),x(%d),y(%d),xofst(%d)",imageioPackage.mfbCfg.bld_mode,(int)imageioPackage.mfbCfg.mfb_out.w,\
            (int)imageioPackage.mfbCfg.mfb_out.h,imageioPackage.mfbCfg.out_xofst);
    }
    //
    for(i=0;i<CHECK_TUNING_P2_EN_BITS;i++){
        if(     ((finalEnP2 & gIspModuleMappingTableP2En[i].ctrlByte)||(tuningUpdateBit & gIspModuleMappingTableP2En[i].eTuningFunc1)||(tuningUpdateBit & gIspModuleMappingTableP2En[i].eTuningFunc2)) &&
...
int ISP_RGB_PIPE::_write2CQ(void)
{
    ...
    if(this->isV3)
    {
        ...
        if(isApplyTuning==MTRUE){
            ...
            tuningEnP2 = pTuningRegMappingQue->CAM_CTL_EN_P2.Raw;
            ISP_FUNC_DBG("tuningUpdateBit(0x%016lx)",(long)tuningUpdateBit);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
    }
    else
...
int ISP_YUV_PIPE::_config( void )
{
    ...
    if (this->isV3)
    {
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(%p),nr3dGainWeighting(%d)",
                (long)tuningUpdateBit,tuningEnP2,tuningEnP2Dma,tuningPackage.pTuningBuf,tuningPackage.nr3dGainWeighting);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        imageioPackage.finalEnP2 = finalEnP2;

        ISP_FUNC_DBG("(ISP_YUV_PIPE),isApplyTuning(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%016lx)",this->isApplyTuning,finalEnP2,(long)tuningUpdateBit);
    }
    else
    {
        getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario,
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%016lx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(%p),nr3dGainWeighting(0x%x)",(long)tuningUpdateBit,tuningEnP2,tuningEnP2Dma,tuningPackage.pTuningBuf,tuningPackage.nr3dGainWeighting);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        imageioPackage.finalEnP2 = finalEnP2;
    }
...
/*/////////////////////////////////////////////////////////////////////////////
    ISP_MDP_PIPE
/////////////////////////////////////////////////////////////////////////////*/
Mutex ISP_MDP_PIPE::mdp_mgr_Lock;
Mutex ISP_MDP_PIPE::mdp_pipe_config_Lock;
MBOOL ISP_MDP_PIPE::createMdpMgr( void )
{
    Mutex::Autolock autoLock(mdp_mgr_Lock);
    ISP_FUNC_INF("+");
    MBOOL err = MTRUE;
...
MBOOL ISP_MDP_PIPE::startVideoRecord( MINT32 wd,MINT32 ht, MINT32 fps,NSImageioIsp3::NSIspio::EDrvScenario drvScen,MUINT32 cqIdx)
{
    ISP_FUNC_INF("+");
    MBOOL err = MTRUE;
    if(g_pMdpMgr)
...
MBOOL ISP_MDP_PIPE::stopVideoRecord( NSImageioIsp3::NSIspio::EDrvScenario drvScen,MUINT32 cqIdx)
{
    ISP_FUNC_INF("+");
    MBOOL err = MTRUE;
    if(g_pMdpMgr)
...
{
    Mutex::Autolock autoLock(mdp_pipe_config_Lock);
...
//        this->src_crop.h);
    ...
    if(this->CQ!=ISP_DRV_CQ01 && this->CQ!=ISP_DRV_CQ02 && this->CQ!=ISP_DRV_CQ03){
        ...
        gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].mdpWDMAPQParam = this->WDMAPQParam;
        gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].mdpWROTPQParam = this->WROTPQParam;
    }
    ...
    if(enPureMdpOut == MFALSE){
        ISP_FUNC_DBG("imgxo va(0x%p),pa(%p),size(%d)",(void *)this->imgxo_out.memBuf.base_vAddr,(void *)this->imgxo_out.memBuf.base_pAddr,this->imgxo_out.memBuf.size);
...
//        ISP_FUNC_INF("QQ,imgxo srcCropW(%d),srcCropH(%d)",gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].dstDma[ISP_MDP_DL_IMGXO].srcCropW ,gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].dstDma[ISP_MDP_DL_IMGXO].srcCropH);
    ...
    // collect crop info for MdpMgr
        ...
        if (this->drvScenario != NSImageioIsp3::NSIspio::eDrvScenario_IP  && this->drvScenario != NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe)
        {
        ...
        ISP_FUNC_INF("QQ,use this srcCropW(%d),srcCropH(%d)",(int)this->src_crop.w,(int)this->src_crop.h);
    }
...
int CAM_CRZ_PIPE::_setZoom( void )
{
    ...
    if ( this->conf_cdrz ) {
        ISP_FUNC_DBG("data_ptr:%p",data_ptr);
    }
...
/*/////////////////////////////////////////////////////////////////////////////
    CAM_TDRI_PIPE
/////////////////////////////////////////////////////////////////////////////*/
Mutex CAM_TDRI_PIPE::cam_tdri_pipe_Lock;
MBOOL CAM_TDRI_PIPE::configTdriSetting(
    TdriDrvCfg* pSrcImageio)
{
    Mutex::Autolock autoLock(cam_tdri_pipe_Lock);
    MBOOL ret = MTRUE;
    ISP_DRV_P2_CQ_ENUM p2Cq;
    ...
    pDst->top.pca_en = pSrcImageio->top.pca_en;
...
//    pDst->feo.feo_stride = pSrcImageio->feo.feo_stride;
    ...
    if(isApplyTuning==MTRUE)
    {
        ...
        if (this->isV3)
        {
            p1TuningEn = this->pTuningIspReg->CAM_CTL_EN_P1.Raw;
            p1TuningDma = this->pTuningIspReg->CAM_CTL_EN_P1_DMA.Raw;
            // set for scenario ip
            if(this->drvScenario == NSImageioIsp3::NSIspio::eDrvScenario_IP)
            {
                //
                ...
                //if(p1TuningFieldEnTag & CAM_CTL_EN_P1_BNR_EN ){
                    #if 1
                    pDst->bnr.bpc_en = this->pTuningIspReg->CAM_BNR_BPC_CON.Bits.BPC_EN;//tuningField.bnr.bpc_en;
                    pDst->bnr.bpc_tbl_en = this->pTuningIspReg->CAM_BNR_BPC_CON.Bits.BPC_LUT_EN;//tuningField.bnr.bpc_tbl_en;
                    #else
                ...
                //if(p1TuningFieldEnTag & CAM_CTL_EN_P1_LSC_EN ){
                    #if 1
                    pDst->lsc.sdblk_width = this->pTuningIspReg->CAM_LSC_CTL2.Bits.LSC_SDBLK_WIDTH;//tuningField.lsc.sdblk_width;
                    pDst->lsc.sdblk_xnum = this->pTuningIspReg->CAM_LSC_CTL2.Bits.LSC_SDBLK_XNUM;//tuningField.lsc.sdblk_xnum;
                    pDst->lsc.sdblk_last_width = this->pTuningIspReg->CAM_LSC_LBLOCK.Bits.LSC_SDBLK_lWIDTH;//tuningField.lsc.sdblk_last_width;
                    pDst->lsc.sdblk_height = this->pTuningIspReg->CAM_LSC_CTL3.Bits.LSC_SDBLK_HEIGHT;//tuningField.lsc.sdblk_height;
                    pDst->lsc.sdblk_ynum = this->pTuningIspReg->CAM_LSC_CTL3.Bits.LSC_SDBLK_YNUM;//tuningField.lsc.sdblk_ynum;
                    pDst->lsc.sdblk_last_height = this->pTuningIspReg->CAM_LSC_LBLOCK.Bits.LSC_SDBLK_lHEIGHT;//tuningField.lsc.sdblk_last_height;
                    #else
                ...
                //if(p1TuningFieldDmaTag & CAM_CTL_EN_P1_DMA_LSCI_EN ){
                    #if 1
                    pDst->lsci.lsci_stride = this->pTuningIspReg->CAM_LSCI_STRIDE.Bits.STRIDE;//tuningField.lsci.lsci_stride;
                    #else
                    ::memcpy( (char*)&pDst->lsci, (char*)&tuningField.lsci, sizeof(ISP_TPIPE_CONFIG_LSCI_STRUCT));
                    #endif
                //}
            }
            else if(this->drvScenario == NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe)
            {
                //
                //set for p1_en top
                //if(p1TuningTopEnTag & CAM_CTL_EN_P1_DBS_EN ){
        ...
        else
        {
            ...
            getP2EnTuningTag(MFALSE,
                           ...
                           this->sub_mode,
                           p2TuningFieldEnTag,
                           p2TuningFieldDmaTag, this->isV3);

            //
            pTuningBuf = m_pPhyIspDrv->getTuningBuf(eTuningQueAccessPath_imageio_P2, this->CQ);
            p1TuningEn = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P2, eTuningTopEn_p1, this->CQ, this->magicNum);
            p1TuningDma = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P2, eTuningTopEn_p1_dma, this->CQ, this->magicNum);
            p2TuningEn = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P2, eTuningTopEn_p2, this->CQ, this->magicNum);
            p2TuningDma = m_pPhyIspDrv->getTuningTop(eTuningQueAccessPath_imageio_P2, eTuningTopEn_p2_dma, this->CQ, this->magicNum);
            ...
            ISP_FUNC_DBG("p1TuningEn(0x%08x),p1TuningDma(0x%08x)",p1TuningEn,p1TuningDma);
    ...
    pDst->drvinfo.DesCqVa = (unsigned long)(this->m_pIspDrvShell->getPhyIspDrv()->getCQDescBufVirAddr(this->CQ,this->burstQueIdx,this->dupCqIdx));
    pDst->drvinfo.VirCqVa = (unsigned long)(this->m_pIspDrvShell->getPhyIspDrv()->getCQVirBufVirAddr(this->CQ,this->burstQueIdx,this->dupCqIdx));
    pDst->drvinfo.dupCqIdx = this->dupCqIdx;
    pDst->drvinfo.p2Cq = (ISP_TPIPE_P2_CQ_ENUM)p2Cq;

    ISP_FUNC_DBG("DesCqPa(0x%x),dupCqIdx(%d)",pDst->drvinfo.DesCqPa,pDst->drvinfo.dupCqIdx);
...
int CAM_TDRI_PIPE::_config( void )
{
    ...
    MUINT32 en_p2, en_p1;
    en_p2 = ISP_READ_REG_NOPROTECT(m_pP2IspDrv, CAM_CTL_EN_P2,ISP_DRV_USER_ISPF);
    en_p1 = ISP_READ_REG_NOPROTECT(m_pP2IspDrv, CAM_CTL_EN_P1,ISP_DRV_USER_ISPF);
    ISP_FUNC_DBG("(CAM_TDRI_PIPE): P2_EN(0x%08x) P1_EN(0x%08x)", en_p2, en_p1);

    this->tcm_en |= (en_p2 & CAM_CTL_EN_P2_LCE_EN) << 11; //LCE
    this->tcm_en |= (en_p2 & CAM_CTL_EN_P2_NR3D_EN) << 14; //NR3D
    this->tdr_en |= (en_p2 & CAM_CTL_EN_P2_PCA_EN) << 14; //PCA
    this->tdr_en |= (en_p2 & CAM_CTL_EN_P2_SL2C_EN) << 7; //SL2C
    this->tdr_en |= (en_p2 & CAM_CTL_EN_P2_SEEE_EN) << 10; //SEEE
    this->tdr_dbg_tcm_en |= (en_p2 & CAM_CTL_EN_P2_NSL2_EN) << 28; //NSL2
    this->tdr_dbg_tcm_en |= (en_p2 & (CAM_CTL_EN_P2_NSL2A_EN|CAM_CTL_EN_P2_NSL2B_EN)) << 4; //NSL2A, NSL2B
    this->tdr_dbg_tcm_en |= (en_p1 & CAM_CTL_EN_P1_RMG_EN) << 4; //RMG
    this->tdr_dbg_tcm_en |= (en_p1 & CAM_CTL_EN_P1_DBS_EN) << 3; //DBS
    ISP_FUNC_DBG("-,enTdri(%d),tcm_en(0x%08x),tdr_en(0x%08x),tdr_dbg_tcm_en(0x%08x),cq(%d),dupCqIdx(%d),burstQueIdx(%d)",this->enTdri,this->tcm_en,this->tdr_en,this->tdr_dbg_tcm_en,this->CQ,this->dupCqIdx,this->burstQueIdx);
    // for control extension -----
    ...
    ISP_WRITE_REG(m_pP2IspDrv, CAM_CTL_TDR_DBG_STATUS, this->tdr_dbg_tcm_en, ISP_DRV_USER_ISPF);  //0x4088 // for control extension
    if ( this->enTdri ) {
...
int CAM_TDRI_PIPE::_write2CQ( void )
{
    ...
    this->m_pIspDrvShell->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,this->dupCqIdx,(CAM_MODULE_ENUM)CAM_CTL_TDR_DBG_STATUS); // for control extension
...
MBOOL ISP_PASS1_CTRL::setMMDVFS(MUINT32 clk, MBOOL bIncClk)
{
    MBOOL ret = 0;
    ISP_FUNC_INF("cam path set isp clk(%d), bIncClk(%d)\n", clk, (UINT32)bIncClk);

    if((ret = this->m_pIspDrvShell->m_pPhyIspDrv_bak->mmdvfs_ctrl(_UPDATE_MMDVFS_STEP,
            (MUINT8*)&clk, (MUINT8*)&bIncClk)) == MFALSE)
        ISP_FUNC_ERR("Error in SET_MMDVFS");
    return 0;
}

MBOOL ISP_PASS1_CTRL::setBw(MUINT32 total_bw,MUINT32 fps,MBOOL immediate, MUINT32 pass1_path)
{
    MBOOL ret = MTRUE;
    // this->pmqos_param.m_TargetBw = total_bw;
    // this->pmqos_param.m_TargetBw_fps = fps;
    ISP_PM_QOS_INFO_STRUCT pm_qos_info;
    pm_qos_info.bw_value = total_bw;
    pm_qos_info.fps = fps;//for per frame calculation

    if(ISP_PASS1 == pass1_path)
        pm_qos_info.module = ISP_PASS1_PATH_TYPE_RAW;
    else if(ISP_PASS1_D == pass1_path)
        pm_qos_info.module = ISP_PASS1_PATH_TYPE_RAW_D;
    else {
        ISP_FUNC_ERR("Not support pass1 id");
        return MFALSE;
    }
    if((ret = this->m_pIspDrvShell->m_pPhyIspDrv_bak->set_pmqos(_SET_PM_QOS_INFO, (MUINT8*)&pm_qos_info)) == MFALSE)
    ISP_FUNC_ERR("Error in SET_PM_QOS_INFO");

    // 1st frame, immediately set to pmqos
    if(immediate == MTRUE) {
        if((ret = this->m_pIspDrvShell->m_pPhyIspDrv_bak->set_pmqos(_SET_PM_QOS_ON, (MUINT8*)&(pm_qos_info.module))) == MFALSE)
            ISP_FUNC_ERR("Error in SET_PM_QOS");
    }

    return 0;
}

MBOOL ISP_PASS1_CTRL::cal_and_setBandW(MUINT32 pass1_path, MUINT32 fps)
{
    MUINT32 _targetDupQueIdx = 0;
    IspDrv* _targetVirDrv = NULL;
    MUINT32 dmao_bw = 0;
    ...
    if( ISP_PASS1 == pass1_path )
    {
        ...
        _targetVirDrv = (IspDrv*)this->m_pIspDrvShell->ispDrvGetCQIspDrv(this->CQ,this->burstQueIdx,_targetDupQueIdx);
        // IMGO
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA)&CAM_CTL_EN_P1_DMA_IMGO_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, IMGO(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_XSIZE)+1));
        }
        // RRZO
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA)&CAM_CTL_EN_P1_DMA_RRZO_EN)
        {
            dmao_bw+= ((ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_XSIZE)+1));
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, RRZO(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_XSIZE)+1));
        }
        // AFO
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA)&CAM_CTL_EN_P1_DMA_AFO_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, AFO(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_XSIZE)+1));
        }
        // ESFLKO
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA)&CAM_CTL_EN_P1_DMA_ESFKO_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_ESFKO_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_ESFKO_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, ESFKO(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_ESFKO_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_ESFKO_XSIZE)+1));
        }
        // AAO
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA)&CAM_CTL_EN_P1_DMA_AAO_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, AAO(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_XSIZE)+1));
        }
        // LCSO
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA)&CAM_CTL_EN_P1_DMA_LCSO_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_YSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, LCSO(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_YSIZE)+1));
        }
    ...
    if( ISP_PASS1_D == pass1_path )
    {
#ifdef _PASS1_CQ_CONTINUOUS_MODE_
        if(ISP_READ_BITS_NOPROTECT(m_pPhyIspDrv,CAM_TG2_VF_CON,VFDATA_EN) == 0){
            _targetDupQueIdx = this->dupCqIdx;
        }else{
            _targetDupQueIdx = (this->dupCqIdx+1)%ISP_DRV_P1_CQ_DUPLICATION_NUM;
        }
        ...
        _targetVirDrv = (IspDrv*)this->m_pIspDrvShell->ispDrvGetCQIspDrv(this->CQ_D,this->burstQueIdx,_targetDupQueIdx);
        // IMGO_D
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA_D)&CAM_CTL_EN_P1_DMA_D_IMGO_D_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_D_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_D_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, IMGO_D(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_D_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_IMGO_D_XSIZE)+1));
        }
        // RRZO_D
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA_D)&CAM_CTL_EN_P1_DMA_D_RRZO_D_EN)
        {
            // dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_D_YSIZE)+1)* \
            //             (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_D_STRIDE)&0xFFFF);
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_D_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_D_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, RRZO_D(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_D_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_RRZO_D_XSIZE)+1));
        }
        // AFO_D
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA_D)&CAM_CTL_EN_P1_DMA_D_AFO_D_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_D_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_D_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, AFO_D(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_D_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AFO_D_XSIZE)+1));
        }
        // AAO_D
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA_D)&CAM_CTL_EN_P1_DMA_D_AAO_D_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_D_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_D_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, AAO_D(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_D_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_AAO_D_XSIZE)+1));
        }
        // LCSO_D
        if(ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_CTL_EN_P1_DMA_D)&CAM_CTL_EN_P1_DMA_D_LCSO_D_EN)
        {
            dmao_bw+= (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_D_YSIZE)+1)* \
                        (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_D_XSIZE)+1);
            ISP_FUNC_DBG("+(ISP_RAW_PIPE),Total BW:%d, LCSO_D(%dx%d)", \
                                dmao_bw, \
                                ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_D_YSIZE)+1, \
                                (ISP_READ_REG_NOPROTECT(_targetVirDrv, CAM_LCSO_D_XSIZE)+1));
        }
    }
    ISP_FUNC_DBG("dmao(%d), fps(%d)\n", dmao_bw, fps);
    this->setBw(dmao_bw, fps, MFALSE, pass1_path);
    return 0;
}
...
int ISP_PASS1_CTRL::keepP1RrzCfg( IspRrzCfg const rrz_cfg )
{
    if ( ISP_PASS1 == rrz_cfg.pass1_path ) {
        ...
        ISP_FUNC_ERR("rrz keeper path err 0x%x", (int)rrz_cfg.pass1_path);
        return -1;
    }
    return 0;
}
...
int ISP_PASS1_CTRL::setP1RrzCfg( IspRrzCfg const rrz_cfg )
{
    ...
    ISP_FUNC_DBG("twin(%d),path(%d)",(int)rrz_cfg.isTwinMode, (int)rrz_cfg.pass1_path);
    ...
    if ( ISP_PASS1 == rrz_cfg.pass1_path ) {
        ...
        ISP_FUNC_ERR("Not support pass1 path(%d)", (int)rrz_cfg.pass1_path);
        return -1;
    }
    ...
    if ( rrz_cfg.isTwinMode) {
        if((rrz_cfg.rrz_in_roi.x*2 + rrz_cfg.rrz_in_roi.w) > rrz_cfg.rrz_in_size.w){
            ISP_FUNC_DBG("rrz input crop out of range: (x,y,floatx,floatxy,w,h = %d, %d, %d, %d, %d, %d),(size: %d, %d)",\
                rrz_cfg.rrz_in_roi.x,rrz_cfg.rrz_in_roi.y,rrz_cfg.rrz_in_roi.floatX,rrz_cfg.rrz_in_roi.floatY, \
                (int)rrz_cfg.rrz_in_roi.w,(int)rrz_cfg.rrz_in_roi.h,(int)rrz_cfg.rrz_in_size.w,(int)rrz_cfg.rrz_in_size.h);
        }
...
init( MUINT32 dmaChannel )
{
    ...
    if(this->path == ISP_PASS1){
        ...
        ISP_FUNC_INF("0x%08x/%dn",dmaChannel,rt_dma);
        if ( -1 == rt_dma ) {
...
waitBufReady( MUINT32 dmaChannel )
{
    ...
    #define MAX_CNT(x, y) ((x/y)+((x%y)?1:0))

    /* "adb shell setprop vendor.camera.p1wait.ms 3000" for sensor long exposure 3s. */
    char waitValue[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.p1wait.ms", waitValue, "0");
    MUINT32 waitMs = atoi(waitValue);
    MUINT32 maxCnt = 1;
    MUINT32 estTimeout = estimateTimeout();
    ...
    ISP_FUNC_DBG("[property] waitMs(%d), maxCnt(%d)\n", waitMs, maxCnt);
    ...
    if ( _imgo_ == rt_dma || _imgo_d_ == rt_dma || _rrzo_ == rt_dma || _rrzo_d_ == rt_dma || _camsv_imgo_ == rt_dma || _camsv2_imgo_ == rt_dma) {
        ...
        ISP_FUNC_DBG("waitBufReady[%d]:[%x,%x] rtBufCtrl.ctrl(%d)/id(%d)/ptr(0%p)",rt_dma,this->path, this->path_D,buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.data_ptr);
        ...
        //if (bWaitBufRdy) {
            ...
            if (( _imgo_d_ == rt_dma ) || (_rrzo_d_ == rt_dma)) {
                ...
                irq_TG_DONE.Status = CAMSV_INT_STATUS_PASS1_DON_ST;
                irq_TG_DONE.Type = ISP_DRV_IRQ_TYPE_INT_CAMSV;
                irq_TG_DONE.UserInfo.Status=CAMSV_INT_STATUS_PASS1_DON_ST;
                irq_TG_DONE.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_CAMSV;
                irq_TG_DONE.UserNumber=(int)ISP_DRV_IRQ_USER_ISPDRV;
                irq_TG_DONE.UserName = "camsv";
            }
            else if( _camsv2_imgo_ == rt_dma ) {
                irq_TG_DONE.Status = CAMSV2_INT_STATUS_PASS1_DON_ST;
                irq_TG_DONE.Type = ISP_DRV_IRQ_TYPE_INT_CAMSV2;
                irq_TG_DONE.UserInfo.Status=CAMSV2_INT_STATUS_PASS1_DON_ST;
                irq_TG_DONE.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_CAMSV2;
                irq_TG_DONE.UserNumber=(int)ISP_DRV_IRQ_USER_ISPDRV;
                irq_TG_DONE.UserName = "camsv_d";
            }
            //
            while(loopCnt++ < maxCnt){//(this->m_pPhyIspDrv->registerIrq(irq_TG_DONE)){
                if(loopCnt == maxCnt)
...
MUINT32 ISP_BUF_CTRL::estimateTimeout() {
#define _CAM_INT_WAIT_TIMEOUT_MS (1000)

    MUINT32 subSample = 1;

    MUINT32 _t_ms = (m_recentFrmTimeMs[0] >= m_recentFrmTimeMs[1])?
        m_recentFrmTimeMs[0]: m_recentFrmTimeMs[1];

    _t_ms *= subSample;

    _t_ms = (_CAM_INT_WAIT_TIMEOUT_MS > _t_ms)? _CAM_INT_WAIT_TIMEOUT_MS: _t_ms;

    _t_ms *= 2; /* x2 for enque post deque sem timing */

    ISP_FUNC_DBG("_t_ms = %d\n", _t_ms);
    return _t_ms;
}

MBOOL ISP_BUF_CTRL::updateFrameTime(MUINT32 timeInMs, MUINT32 reqOffset) {
    if (reqOffset >= 2) {
        return MFALSE;
    }

    m_recentFrmTimeMs[reqOffset] = timeInMs;

    ISP_FUNC_DBG("m_recentFrmTimeMs[%d] = %d\n", reqOffset, m_recentFrmTimeMs[reqOffset]);
    return MTRUE;
}
...
#define ISP_QUE_DBG(rt_dma,fmt,arg...){\
    ...
    if(path == 1){\
        ...
        ISP_FUNC_DBG("pass1d:" fmt,##arg);\
    }\
    else if(path == 4){\
        ISP_FUNC_DBG("camsv:" fmt,##arg);\
    }\
    else if(path == 5){\
        ISP_FUNC_DBG("camsv2:" fmt,##arg);\
    }\
    else {\
...
enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo,MINT32 enqueCq,MINT32 dupCqIdx,MBOOL bImdMode)
{
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        if( NULL != bufInfo.next ){
            ISP_QUE_DBG(rt_dma,"dma(%d),id(%d),size(0x%x),VA(%p),PA(%p),S/C(%d/%d),bufidx(%d),replace:new(%p),bufidx(%d),bImdMode(%d)",\
                        rt_dma,\
                        buf_info.memID,\
                        buf_info.size,\
                        (void *)buf_info.virtAddr,\
                        (void *)buf_info.phyAddr,\
                        buf_info.bufSecu,\
                        buf_info.bufCohe,\
                        bufInfo.mBufIdx,\
                        (void *)bufInfo.next->base_pAddr,\
                        bufInfo.next->mBufIdx,\
                        bImdMode);
        }
        else{
            ISP_QUE_DBG(rt_dma,"dma(%d),id(%d),size(0x%x),VA(%p),PA(%p),S/C(%d/%d),bufidx(%d),bImdMode(%d)",\
                        rt_dma,\
                        buf_info.memID,\
                        buf_info.size,\
                        (void *)buf_info.virtAddr,\
        ...
        //
        ISP_FUNC_INF("enqueCq(%d),p2Cq(%d),dupCqIdx(%d),dma(%d),memID(0x%x),size(%d),vAddr(0x%p),pAddr(%p)", \
            enqueCq,p2Cq,dupCqIdx,rt_dma, \
            m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.back().memID, \
            (int)m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.size(), \
            (void *)m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.back().base_vAddr,
            (void *)m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.back().base_pAddr);
...
enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo, MVOID* private_data ,MBOOL bImdMode)
{
    ...
    if (0){
        ...
        ISP_QUE_INF(rt_dma,"[MyDeQ]EnQHwBuf::ispDrv=%p/%p, CmdQ-IN/OUT Z=[0x%x,0x%x], HW Z=[0x%x,0x%x].", \
...
{
    if(drvScenario != NSImageioIsp3::NSIspio::eDrvScenario_IP){
        ISP_DRV_P2_CQ_ENUM p2Cq;
...
ISP_BUF_CTRL::returnDeqBufInfo( MINT32 rt_dma, stISP_BUF_INFO* bufInfo)
{
    ...
    if ( NULL == bufInfo->next ){
        ...
        if (i < ISP_BUF_INFO_NODE_MAX_CNT) {
            ISP_QUE_DBG(rt_dma,"[MyDeQ]returnDeqBufInfo:: node=%p", (void *)plist->mN[i].mInfo.base_pAddr);
        ...
        {
            if(bVF_en){
                ISP_FUNC_ERR("dma:0x%x: find no match baseaddr, %p\n",rt_dma,(void *)bufInfo->base_pAddr);
                ISP_FUNC_ERR("current available base addr\n");
                for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++){
                    if(plist->mN[i].mOccupied)
                        ISP_FUNC_ERR("%p", (void *)plist->mN[i].mInfo.base_pAddr);
                }
            }
        }
    }
        ...
        if (i < ISP_BUF_INFO_NODE_MAX_CNT) {
            ...
            {
                if(bVF_en){
                    ISP_QUE_DBG(rt_dma,"dma:0x%x: find no match baseaddr, 0x%p\n",rt_dma, (void *)bufInfo->next->base_vAddr);
                    ISP_QUE_DBG(rt_dma,"current available base addr\n");
                    for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++){
                        if(plist->mN[i].mOccupied)
                            ISP_QUE_DBG(rt_dma,"%p",(void *)plist->mN[i].mInfo.base_pAddr);
                    }
                }
            }
        } else {
            ISP_QUE_DBG(rt_dma,"dma:0x%x:find no match baseaddr,caused this is exchange buf, %p\n",rt_dma, (void *)bufInfo->next->base_pAddr);
            ISP_QUE_DBG(rt_dma,"current available base addr\n");
            for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++){
                if(plist->mN[i].mOccupied)
                    ISP_QUE_DBG(rt_dma,"%p", (void *)plist->mN[i].mInfo.base_pAddr);
...
saveDeqBufInfo( MINT32 rt_dma )
{
    ...
    for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++) {
        ...
        if (!plist->mN[i].mOccupied) {
            ...
            ISP_QUE_DBG(rt_dma,"saveDeqBufInfo[A] plist=%p,[%d]PA=%p, mIdx=%d", plist,i, (void *)ptr->base_pAddr, plist->mIdx);
            return ptr;
        }
    }
#endif
    if (1) {
        for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i+=8) {
            ...
            ISP_FUNC_ERR("saveDeqBufInfo[C] Fail. dma=%d,plist=%p, mOccupied:[%d[%p]/%d[%p]/%d[%p]/%d[%p]/%d[%p]/%d[%p]/%d[%p]/%d[%p]]",rt_dma,plist,\
                plist->mN[i+0].mOccupied, (void *)plist->mN[i+0].mInfo.base_pAddr,\
                plist->mN[i+1].mOccupied, (void *)plist->mN[i+1].mInfo.base_pAddr,\
                plist->mN[i+2].mOccupied, (void *)plist->mN[i+2].mInfo.base_pAddr,\
                plist->mN[i+3].mOccupied, (void *)plist->mN[i+3].mInfo.base_pAddr,\
                plist->mN[i+4].mOccupied, (void *)plist->mN[i+4].mInfo.base_pAddr,\
                plist->mN[i+5].mOccupied, (void *)plist->mN[i+5].mInfo.base_pAddr,\
                plist->mN[i+6].mOccupied, (void *)plist->mN[i+6].mInfo.base_pAddr,\
                plist->mN[i+7].mOccupied, (void *)plist->mN[i+7].mInfo.base_pAddr);
        }
    }
    ...
    if (0) {
        ...
        ISP_QUE_DBG(rt_dma,"saveDeqBufInfo[D][Error] plist=%p, ptr=%p, mIdx=%d", plist,ptr,plist->mIdx);
    }
    if(ptr!= NULL)
        ISP_QUE_DBG(rt_dma,"[MyDeQ]saveDeqBufInfo: node=%p", (void *)ptr->base_pAddr);
    return ptr;
}
//
...
dequeueHwBuf( MUINT32 dmaChannel, stISP_FILLED_BUF_LIST& bufList )
{
    ...
    char value[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.dumpHwBuf.enable", value, "0");
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        if ( _imgo_ == rt_dma || \
            ...
            for(MUINT32 i=0;i<count;i++) {
                ...
                bufInfo->mBufIdx = deque_buf.data[i].bufIdx;
            ...
            if (bufSize) {
                ...
                if (dumpHwBufEnable)
                {
                    ...
                    if((pWorkingBuffer!=NULL)&&(pIspReg!=NULL)) {
                        ...
                        ISP_FUNC_ERR("[Error]pWorkingBuffer(%p),pIspReg(%p)",pWorkingBuffer,pIspReg);
                    }
                ...
                if(isDequeMdp==MTRUE){
                    ...
                    if(mdpRet == MDPMGR_NO_ERROR){
                        gpMdpMgrCfgData[this->dequeBurstQueIdx][p2Cq][this->dequeDupCqIdx].isWaitBuf = MTRUE;
                ...
                //
                ISP_FUNC_DBG("dma(%d),hwsize(%d),p2Cq(%d),dequeDupCqIdx(%d),imgxoEnP2(0x%x),memID(0x%x),size(%d),vaddr(%p),paddr(%p)",
                    rt_dma,\
                    (int)m_p2HwBufL[p2Cq][this->dequeDupCqIdx][rt_dma].bufInfoList.size(),\
                    p2Cq,\
                    this->dequeDupCqIdx,\
                    gpMdpMgrCfgData[this->dequeBurstQueIdx][p2Cq][this->dequeDupCqIdx].imgxoEnP2,\
                    bufList.pBufList->back().memID,\
                    (int)bufList.pBufList->size(),\
                    (void *)bufList.pBufList->back().base_vAddr,\
                    (void *)bufList.pBufList->back().base_pAddr);

                this->m_pIspDrvShell->gPass2Lock.unlock();

            } else {
                ISP_FUNC_ERR("empty HW buffer, dma(%d), p2Cq(%d),dequeDupCqIdx(%d)",
                    rt_dma,\
                    p2Cq,\
                    this->dequeDupCqIdx);
...
getCurrHwBuf( MUINT32 dmaChannel )
{
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        //
        ISP_FUNC_DBG("dma:(%d)/memID(0x%x)/vAddr:(0x%p)/pAddr:(%p) ",rt_dma, \
                                                            m_hwbufL[rt_dma].bufInfoList.front().memID, \
                                                            (void *)m_hwbufL[rt_dma].bufInfoList.front().base_vAddr, \
                                                            (void *)m_hwbufL[rt_dma].bufInfoList.front().base_pAddr);
    } else { // Pass2
        ISP_DRV_P2_CQ_ENUM p2Cq;
        //
        this->m_pIspDrvShell->m_pPhyIspDrv_bak->mapCqToP2Cq((ISP_DRV_CQ_ENUM)this->CQ, p2Cq);
...
MUINT32
ISP_BUF_CTRL::
freeSinglePhyBuf(stISP_BUF_INFO bufInfo)
{
    UNUSED(bufInfo);
    MUINT32 ret = 0;
#if 0
...
debugPrint( MUINT32 dmaChannel )
{
    ...
    for ( it = m_hwbufL[rt_dma].bufInfoList.begin(); it != m_hwbufL[rt_dma].bufInfoList.end(); it++ ) {
        ISP_FUNC_DBG("m_hwbufL[%d].base_vAddr:[0x%p]/base_pAddr:[%p] ",rt_dma,(void *)it->base_vAddr,(void *)it->base_pAddr);
    }
    ISP_FUNC_DBG("X");
...
{
    ISP_FUNC_DBG("E: pIspDrv(%p)",pIspDrv);
    //
...
MBOOL IspEventThreadImp::init(void)
{
    ...
    //pthread_attr_t const attr = {0, NULL, 1024 * 1024, 4096, SCHED_RR, NICE_CAMERA_LOMO};
    //pthread_create(&m_ispEventthread, &attr, ispEventThread, this);
    pthread_attr_t attr;
    if ((r = pthread_attr_init(&attr)) != 0) {
        ISP_FUNC_ERR("thread ctrl [%s] fail: %d", "set thread attr", r);
        return MFALSE;
    }
    if ((r = pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED)) != 0) {
        ISP_FUNC_ERR("thread ctrl [%s] fail: %d", "thread setdetach", r);
        return MFALSE;
    }

    if ((r = pthread_create(&m_ispEventthread, &attr, ispEventThread, this)) != 0) {
        ISP_FUNC_ERR("thread ctrl [%s] fail: %d", "thread create", r);
        return MFALSE;
    }
    if ((r = pthread_attr_destroy(&attr)) != 0) {
        ISP_FUNC_ERR("thread ctrl [%s] fail: %d", "destroy thread attr", r);
        return MFALSE;
    }
    //
    android_atomic_inc(&mInitCount);
...
MVOID* IspEventThreadImp::ispEventThread(void *arg)
{
    ...
    ISP_FUNC_DBG("E: tid=%d m_pPhyIspDrv(%p)", gettid(),_this->m_pPhyIspDrv);

    ::prctl(PR_SET_NAME,"ispEventthread",0,0,0);

    // detach thread => cannot be join
    // join the thread while uninit
