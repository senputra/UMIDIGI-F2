...
MBOOL CAM_DFS_Mgr::CAM_DFS_SetLv(MINT32 level,MBOOL immediate)
{
    ...
    if(this->m_Switch_Latnecy == SET_LOW_LATENCY) {
...
MBOOL CAM_DFS_Mgr::CAM_DFS_setLv(void)
{
    ...
    if (this->m_TargetLv != (MUINT32)m_curClkLevel) { //only target level doesn't equal to current level is needed to set clock level.
        if(dfsdisable) {
