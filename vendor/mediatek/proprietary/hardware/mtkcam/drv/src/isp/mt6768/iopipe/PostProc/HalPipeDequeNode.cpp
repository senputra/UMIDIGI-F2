    ...
    )
{
    ...
    //(1) deque dma
    LOG_DBG("yo(0x%x)_Dequeue path(%d) out buf portID(%d),p2cq(%d),p2cqDupIdx(%d),p2BurstQIdx(%d),RingBufIdx(%d)",frameunit.callerID, frameunit.drvScen, portID.index, frameunit.p2cqIdx, frameunit.p2cqDupIdx, frameunit.p2BurstQIdx, frameunit.p2RingBufIdx);
    deQret=this->mpPostProcPipe->dequeBuf(isVencContained,rQTSBufInfo,tpipeVa,ispVirRegVa,frameunit.drvScen,(MINT32)(frameunit.p2cqIdx),(MINT32)(frameunit.p2BurstQIdx),(MINT32)(frameunit.p2cqDupIdx),(MINT32)(frameunit.p2RingBufIdx));
    ...
    if(ret)
    {
            ...
            for (MUINT32 i = 0 ; i < this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut.size() ; i++ )
            {
                ...
                switch(this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[i].mPortID.index)
                {
                    ...
                    case NSImageio::NSIspio::EPortIndex_MFBO:
                        if (this->mpHalPipeWrapper->m_iSaveReqToFile & (DIP_DUMP_MFBO_BUFFER))
                        {
                            this->mpHalPipeWrapper->mLDIPwaitDQFramePackList[index].rParams.mvFrameParams[frm].mvOut[i].mBuffer->saveToFile(filename);
                        }
                        break;
        ...
        if(!dequeFramePackDone)
        {
            ...
            LOG_INF("+,tag(%d),idx(%d),p2CQ(%d/%d/%d/%d)",\
                         EBufferListTag_Unit,elementIdx,dequeP2Cq,(MINT32)(frameunit.p2cqDupIdx),(MINT32)(frameunit.p2BurstQIdx),(MINT32)(frameunit.p2RingBufIdx));
        }
        else
        {
