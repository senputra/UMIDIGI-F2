...
CamsvIOPipe* CamsvIOPipe::Create(MINT8 const szUsrName[32], E_INPUT InPut)
{
    ...
    switch(InPut){
        // CAMSV
        case TG_CAMSV_0:
            if(gCamsvioPipe[CAMSV_0-CAMSV_START].m_occupied == MTRUE){
                ...
                gCamsvioPipe[CAMSV_0-CAMSV_START].m_occupied = MTRUE;
                std::strncpy((char*)gCamsvioPipe[CAMSV_0-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(gCamsvioPipe[CAMSV_0-CAMSV_START].m_szUsrName)-1);
            ...
            break;
        case TG_CAMSV_1:
            if(gCamsvioPipe[CAMSV_1-CAMSV_START].m_occupied == MTRUE){
                ...
                gCamsvioPipe[CAMSV_1-CAMSV_START].m_occupied = MTRUE;
                std::strncpy((char*)gCamsvioPipe[CAMSV_1-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(gCamsvioPipe[CAMSV_1-CAMSV_START].m_szUsrName)-1);
            ...
            break;
        case TG_CAMSV_2:
            if(gCamsvioPipe[CAMSV_2-CAMSV_START].m_occupied == MTRUE){
                ...
                gCamsvioPipe[CAMSV_2-CAMSV_START].m_occupied = MTRUE;
                std::strncpy((char*)gCamsvioPipe[CAMSV_2-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(gCamsvioPipe[CAMSV_2-CAMSV_START].m_szUsrName)-1);
            ...
            break;
        case TG_CAMSV_3:
            if(gCamsvioPipe[CAMSV_3-CAMSV_START].m_occupied == MTRUE){
                ...
                gCamsvioPipe[CAMSV_3-CAMSV_START].m_occupied = MTRUE;
                std::strncpy((char*)gCamsvioPipe[CAMSV_3-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(gCamsvioPipe[CAMSV_3-CAMSV_START].m_szUsrName)-1);
            ...
            break;
        case TG_CAMSV_4:
            if(gCamsvioPipe[CAMSV_4-CAMSV_START].m_occupied == MTRUE){
                ...
                gCamsvioPipe[CAMSV_4-CAMSV_START].m_occupied = MTRUE;
                std::strncpy((char*)gCamsvioPipe[CAMSV_4-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(gCamsvioPipe[CAMSV_4-CAMSV_START].m_szUsrName)-1);
            ...
            break;
        case TG_CAMSV_5:
            if(gCamsvioPipe[CAMSV_5-CAMSV_START].m_occupied == MTRUE){
                ...
                gCamsvioPipe[CAMSV_5-CAMSV_START].m_occupied = MTRUE;
                std::strncpy((char*)gCamsvioPipe[CAMSV_5-CAMSV_START].m_szUsrName,(char const*)szUsrName, sizeof(gCamsvioPipe[CAMSV_5-CAMSV_START].m_szUsrName)-1);
...
*
********************************************************************************/
MBOOL CamsvIOPipe::stop(MBOOL bForce, MBOOL detachUni)
{
    (void)detachUni;
...
MBOOL CamsvIOPipe::enqueOutBuf(PortID const portID, QBufInfo const& rQBufInfo,MBOOL bImdMode)
{
    ...
    for(MUINT32 i=0; i<rQBufInfo.vBufInfo.size(); i++){
        ...
        PIPE_DBG("burstidx:%d: PortID=%d, bufInfo:(MEMID:%d),(VA:0x%p),(PA:0x%p),(FH_VA:0x%p)",\
            i,\
            portID.index,\
            bufInfo.u_op.enque.image.mem_info.memID,\
            (void*)bufInfo.u_op.enque.image.mem_info.va_addr,\
            (void*)bufInfo.u_op.enque.image.mem_info.pa_addr,\
            (void*)bufInfo.u_op.enque.header.va_addr);
...
E_BUF_STATUS CamsvIOPipe::dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQTBufInfo, MUINT32 const u4TimeoutMs, CAM_STATE_NOTIFY *pNotify)
{
    ...
    #endif
    {
        ...
        PIPE_DBG("PortID=%d, bufInfo:(MEMID:%d),(VA:0x%p),(PA:0x%p),(FH_VA:0x%p)",\
            portID.index, rQTBufInfo.vBufInfo[i].memID[0],\
            (void*)rQTBufInfo.vBufInfo[i].u4BufVA[0],\
            (void*)rQTBufInfo.vBufInfo[i].u4BufPA[0],\
            (void*)rQTBufInfo.vBufInfo[i].Frame_Header.u4BufVA);
    }
...
*
********************************************************************************/
MBOOL CamsvIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts,CAMIO_Func func)
{
    MBOOL   bPak = MTRUE;
    ...
    PIPE_DBG("CamsvIOPipe::configPipe+");
    //
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ ) {
        ...
        switch(vOutPorts[i]->index){
            case EPortIndex_CAMSV_IMGO:
                PIPE_DBG("EPortIndex_CAMSV_IMGO");
                idx_imgo = i;
    ...
    //subsample
    this->m_TopCtrl.SubSample = func.Bits.SUBSAMPLE;
    ...
    this->m_TGCtrl.m_Fmt_Sel.Raw = this->m_TopCtrl.camsv_top_ctl.FMT_SEL.Raw;

    // pixel mode
    switch(vInPorts[idx_in_src]->ePxlMode) {
        case ePxlMode_One_:
            PIPE_INF("idx_imgo(%d) ePxlMode_One_", idx_imgo);
            this->m_TopCtrl.m_PixMode   = _1_pix_;
            this->m_TGCtrl.m_PixMode    = _1_pix_;
            this->m_DmaImgo.m_PixMode   = _1_pix_;
            break;
        case ePxlMode_Two_:
            PIPE_INF("idx_imgo(%d) ePxlMode_Two_", idx_imgo);
            this->m_TopCtrl.m_PixMode   = _2_pix_;
            this->m_TGCtrl.m_PixMode    = _2_pix_;
            this->m_DmaImgo.m_PixMode   = _2_pix_;
            break;
        case ePxlMode_Four_:
            PIPE_INF("idx_imgo(%d) ePxlMode_Four_", idx_imgo);
            this->m_TopCtrl.m_PixMode   = _4_pix_;
            this->m_TGCtrl.m_PixMode    = _4_pix_;
            this->m_DmaImgo.m_PixMode   = _4_pix_;
            break;
        default:
    ...
    //inte en
    this->m_TopCtrl.camsv_top_ctl.INTE_EN.Raw = int_en;

    /// TG setting
    this->m_TGCtrl.m_pDrv = this->m_pIspDrvCamsv;
    this->m_TGCtrl.m_SubSample = func.Bits.SUBSAMPLE;
...
MBOOL CamsvIOPipe::sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    PIPE_DBG("+ tid(%d) (cmd,arg1,arg2,arg3)=(0x%08x,0x%p,0x%p,0x%p)", gettid(), cmd, (void*)arg1, (void*)arg2, (void*)arg3);
...
MINT32 CamsvIOPipe::SrcFmtToHwVal_TG_SW( ImgInfo::EImgFmt_t imgFmt)
{
    switch (imgFmt) {
        ...
        //case eImgFmt_RGB565: return TG_FMT_RGB565;
        //case eImgFmt_RGB888: return TG_FMT_RGB888;
        case eImgFmt_JPEG:   return TG_SW_RGB;
...
MBOOL CamsvIOPipe::configDmaPort(PortInfo const* portInfo,IspDMACfg &a_dma,MUINT32 pixel_Byte,MUINT32 swap, MUINT32 isBypassOffset,EIMAGE_STRIDE planeNum)
{
    (void)isBypassOffset;
    ...
    PIPE_INF("w(%lu),h(%lu),stride(%lu),xsize(%lu),crop(%d,%d,%lu,%lu)_f(0x%x,0x%x),ofst_addr(0x%x),pb((0x%x)(+<<2)),va(0x%p),pa(0x%p),pixel_Byte(%d)",a_dma.size.w,a_dma.size.h,a_dma.size.stride, \
                                                        ...
                                                        (void*)a_dma.memBuf.base_vAddr, (void*)a_dma.memBuf.base_pAddr,\
                                                        pixel_Byte);
