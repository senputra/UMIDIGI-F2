...
MBOOL Central::Register(ISP_HW_MODULE module,char Name[32],MUINT32 logLevel)
{
    ...
    BASE_LOG_DBG("+, cam:%d, Name:%s, m_available:%d\n",module, Name, m_available);
...
ISP_HW_MODULE Central::Register_search(MUINT32 width,char Name[32], MUINT32 SensorNum) {
    tCAM_rst rstCamB;
    capibility CamBInfo(NN::ENPipe_CAM_B);
    NN::NormalPipe_InputInfo info;
    info.width = width;

    if(CamBInfo.GetCapibility(0,NN::ENPipeQueryCmd_UNKNOWN,
                                info,rstCamB,E_CAM_pipeline_size) == MFALSE){
        goto EXIT;
    }

    BASE_LOG_INF("width:%d Name:%s SensorNum:%d\n", width, Name, SensorNum);
    if (SensorNum == NSImageio::NSIspio::ECamio_1_SEN) {
        if(width < rstCamB.pipeSize){
            if(this->Register(CAM_B,Name,0) == MTRUE){        //CAM_B 1st when width < pipeline size
                return CAM_B;
            }
            else if(this->Register(CAM_A,Name) == MTRUE){   //if CAM_B is occupied using CAM_A
        ...
        if(this->Register(CAM_A,Name,0) == MTRUE) {        //CAM_A 1st when width > pipeline size
            BASE_LOG_INF("Multi camera case, CAM_A is used firstly\n");
            return CAM_A;
        }
        else if(this->Register(CAM_B,Name) == MTRUE) {   //if CAM_A is occupied using CAM_B
            return CAM_B;
...
ISP_HW_MODULE Central::Register_search(MUINT32 index, MUINT32 SensorNum, Res_Arbitor_In* inParams, Res_Arbitor_Out* outParams, char Name[32]) {

    MUINT32 masterThruPut = 0, masterWidth = 0;
    MUINT32 slaveThruPut = 0, slaveWidth = 0;

    tCAM_rst rstCamB;
    capibility CamBInfo(NN::ENPipe_CAM_B);

    tCAM_rst rstCamA;
    capibility CamAInfo(NN::ENPipe_CAM_A);

    NN::NormalPipe_InputInfo info;
    info.width = inParams[index].img_crop_w;

    outParams[index].isOffTwin = MFALSE;
    ...
    if(CamAInfo.GetCapibility(0,NN::ENPipeQueryCmd_UNKNOWN,
                                info,rstCamA,E_CAM_pipeline_size) == MFALSE){
        goto EXIT;
    }

    if (index >= SensorNum || SensorNum > 2 || inParams->vRawClks.size() == 0) {
        BASE_LOG_ERR("index:%d SensorNum:%d clock level:%d\n", index, SensorNum, inParams->vRawClks.size());
        return CAM_MAX;
    }

    for (MUINT32 i = 0 ; i < SensorNum; i++) {
        if (inParams[i].mipi_rate == 1000000) {
            inParams[i].mipi_rate = (MUINT32) (((MUINT64)inParams[i].img_crop_w)*inParams[i].img_crop_h*inParams[i].tg_fps / 10);
            BASE_LOG_DBG("Recalculated inParams[i].mipi_rate:%d\n", inParams[i].mipi_rate);
        }
    }
    BASE_LOG_DBG("inParams->vRawClks.at(0):%d\n", inParams->vRawClks.at(0));
    if (SensorNum == 2) {

        masterThruPut = inParams[index].mipi_rate;
        slaveThruPut = inParams[1-index].mipi_rate;
        masterWidth = inParams[index].img_crop_w;
        slaveWidth = inParams[1-index].img_crop_w;

        BASE_LOG_INF("index:%d masterThruPut:%d masterWidth:%d slaveThruPut:%d slaveWidth:%d Name:%s\n",
            index, masterThruPut, inParams[index].img_crop_w, slaveThruPut, inParams[1-index].img_crop_w, Name);
        
        if (masterWidth > slaveWidth) {
            if (IS_THROUGHPUT_OVER(masterThruPut, inParams->vRawClks.at(0))) {
                // Need Twin, A+C vs B
                if (slaveWidth < rstCamB.pipeSize) {
                    return RegisterPriorityCam(CAM_A, Name);   
                } else {
                    BASE_LOG_ERR("SensorNum:%d rstCamB.pipeSize:%d over spec!!\n",
                        SensorNum, rstCamB.pipeSize);
                }
            } else {
                // Use single ISP for master and slave, B vs A or A+C vs B
                outParams[index].isOffTwin = MTRUE;
                return Register_search(masterWidth, Name);
            }
        } else {
            // masterWidth <= slaveWidth
            if (IS_THROUGHPUT_OVER(slaveThruPut, inParams->vRawClks.at(0))) {
                // Need Twin, A+C vs B
                if (masterWidth < rstCamB.pipeSize) {
                    outParams[index].isOffTwin = MTRUE;
                    return RegisterPriorityCam(CAM_B, Name); 
                } else {
                    BASE_LOG_ERR("SensorNum:%d rstCamB.pipeSize:%d over spec!!\n",
                        SensorNum, rstCamB.pipeSize);
                }
                ...
                // Use single ISP for master and slave, B vs A or A+C vs B
                outParams[index].isOffTwin = MTRUE;
                if (masterWidth < rstCamA.pipeSize) {
                    return RegisterPriorityCam(CAM_A, Name);  
                } else {
                    return RegisterPriorityCam(CAM_B, Name);
                }                
        ...
        BASE_LOG_DBG("index:%d masterThruPut:%d masterWidth:%d Name:%s\n",
            index, masterThruPut, inParams[index].img_crop_w, Name);

        if (IS_THROUGHPUT_OVER(masterThruPut, inParams->vRawClks.at(inParams->vRawClks.size()-1))) {
            // Tripple CAM, CAM_A needs to be the master
            return RegisterPriorityCam(CAM_A, Name);                  
        } else {
            return Register_search(masterWidth, Name);
        }
    } else {
        BASE_LOG_ERR("SensorNum:%d can't support\n",SensorNum);
    }
EXIT:
    BASE_LOG_ERR("isp lunch fail due to no correct pipeline size\n");
    return CAM_MAX;
...
ISP_HW_MODULE Central::RegisterPriorityCam(ISP_HW_MODULE priorityCam, char Name[32]) {

    ISP_HW_MODULE firstCam = CAM_MAX, secondCam = CAM_MAX;

    if (priorityCam == CAM_A) {
        firstCam = CAM_A;
        secondCam = CAM_B;
    } else if (priorityCam == CAM_B) {
        firstCam = CAM_B;
        secondCam = CAM_A;
    } else {
        BASE_LOG_ERR("priorityCam:%d\n is not supported", priorityCam);
        return CAM_MAX;
    }

    BASE_LOG_DBG("firstCam:%d, secondCam:%d\n", firstCam, secondCam);
    if(this->Register(firstCam,Name,0) == MTRUE) {
        BASE_LOG_INF("Multi camera case, CAM_%d is used firstly\n", firstCam);
        return firstCam;
    } else if (this->Register(secondCam,Name) == MTRUE) {
        BASE_LOG_INF("Multi camera case, CAM_%d is used firstly\n", secondCam);
        return secondCam;
    } else {
        BASE_LOG_ERR("no available cam, isp lunch fail\n");
...
Cam_ResMgr* Cam_ResMgr::Res_Attach(MUINT32 width, char Name[32], MUINT32 SensorNum)
{
    MBOOL ret = MTRUE;
    ISP_HW_MODULE master;

    BASE_LOG_INF("CAM Res_Attach:: size:%d,name:%s, SensorNum:%d\n", width, Name, SensorNum);
    Mutex::Autolock lock(Cam_ResMgr::mLock);
    if( (master = Cam_ResMgr::m_central.Register_search(width, Name, SensorNum))  == CAM_MAX){
        return NULL;
    }
...
Cam_ResMgr* Cam_ResMgr::Res_Attach(MUINT32 index, MUINT32 SensorNum, Res_Arbitor_In* inParams, Res_Arbitor_Out* outParams, char Name[32])
{
    MBOOL ret = MTRUE;
    ISP_HW_MODULE master;

    BASE_LOG_INF("CAM Res_Attach:: name:%s, SensorNum:%d\n", Name, SensorNum);
    Mutex::Autolock lock(Cam_ResMgr::mLock);
    if( (master = Cam_ResMgr::m_central.Register_search(index, SensorNum, inParams, outParams, Name))  == CAM_MAX) {
        return NULL;
    }
...
#define ClkMargin   95/100
    ...
    if(pCamInfo != NULL){
        info.pixelMode = cfgInParam.pix_mode_tg;
        if(pCamInfo->GetCapibility(0,NN::ENPipeQueryCmd_UNKNOWN, info,
        ...
        if(pCamInfo->GetCapibility(0,NN::ENPipeQueryCmd_UNKNOWN, info,
                                    rst1,E_CAM_pipeline_min_size) == MFALSE){
            ret = MFALSE;
            goto EXIT;
        }
    }
    ...
    // TODO: temp solution, maybe remove later
    if (this->m_hwModule == CAM_B || cfgInParam.tg_crop_w < rst1.pipelineMinSize.w) {
        PIPE_INF("Increase clock level for avoiding CAM_B tripple cam! \n");
        for(MUINT32 i=0;i<cfgInParam.vClk.size();i++){
            if(cfgInParam.MIPI_pixrate < (MUINT32)((MUINT64)cfgInParam.vClk.at(i) * ClkMargin * 2)) {
                cfgOutParam.clk_level = i;
                break;
            }
        }
    }
...
MBOOL Cam_ResMgr::Res_Meter_kernel(const Res_Meter_IN& cfgInParam, Res_Meter_OUT &cfgOutParam,MUINT32& thru_put,MBOOL& bBin_useful,MUINT32& bin_pix_mode,vector<ISP_HW_MODULE>& availCam)
{
    ...
    {
        if((cfgInParam.offTwin == MFALSE && this->m_offTwin == MFALSE) && (availCam.size() != 0)){
            this->K_TWIN_CASE(cfgOutParam,thru_put,availCam);
            PIPE_DBG("Run K_TWIN_CASE\n");
        }
...
MBOOL Cam_ResMgr::Res_Meter_Bayer(const Res_Meter_IN& cfgInParam, Res_Meter_OUT &cfgOutParam,const void *ptr,MUINT32 cur_clkL)
{
    ...
    //check dbn/bin/linebuffer case
    {
        ...
        if(cfgInParam.tg_crop_w > rst->pipeSize){
            ...
            PIPE_INF("m_hwModule:%d, cfgInParam.tg_crop_w:%d, rst->pipeSize:%d\n", this->m_hwModule, cfgInParam.tg_crop_w, rst->pipeSize);
            if (this->m_hwModule == CAM_A && cfgInParam.SenNum != NSImageio::NSIspio::ECamio_1_SEN) {
                this->m_offTwin = MTRUE;
                PIPE_INF("CAM_A twin is disabled for reserving resource\n");
            }
        }
    }
