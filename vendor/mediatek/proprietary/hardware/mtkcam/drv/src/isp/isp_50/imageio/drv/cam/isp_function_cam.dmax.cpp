...
#define SET_CQ0_BA_S(master) ({\
    if(this->m_updatecnt != 0 && this->m_bSubsample == MTRUE) {\
        CAM_WRITE_REG(pDrv[master], CAM_CQ_THR0_BASEADDR, Addr[master]);\
        CAM_WRITE_REG(pDrv[master], CAM_CQ_THRE0_ADDR_INNER, Addr[master]);\
    } else {\
        CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CQ_THR0_BASEADDR, Addr[master]);\
    }\
...
#define SET_CQ0_BA_S_SUB_DUMMY(master) ({\
    CAM_WRITE_REG(pDrv_dummy[master], CAM_CQ_THR0_BASEADDR, Addr_dummy[master]);\
    CAM_WRITE_REG(pDrv_dummy[master], CAM_CQ_THRE0_ADDR_INNER, Addr_dummy[master]);\
    CAM_FUNC_DBG("hw(%d),sub dummy addr(0x%x)\n",master,Addr_dummy[master]);\
    pDrv_dummy[master] = 0;\
    Addr_dummy[master] = 0;\
})

#define SET_CQ0_BA_D(master, slave1) ({\
    CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CTL_CD_DONE_SEL, 0x10000);\
    CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x1);\
    if(this->m_bSubsample == MTRUE){\
        CAM_WRITE_REG(pDrv[master], CAM_CQ_THR0_BASEADDR, Addr[master]);\
        CAM_WRITE_REG(pDrv[master], CAM_CQ_THRE0_ADDR_INNER, Addr[master]);\
        CAM_WRITE_REG(pDrv[slave1], CAM_CQ_THR0_BASEADDR, Addr[slave1]);\
        CAM_WRITE_REG(pDrv[slave1], CAM_CQ_THRE0_ADDR_INNER, Addr[slave1]);\
    }else{\
        CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CQ_THR0_BASEADDR, Addr[master]);\
        CAM_WRITE_REG(pDrv[slave1]->getPhyObj(), CAM_CQ_THR0_BASEADDR, Addr[slave1]);\
    }\
    CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x0);\
    CAM_FUNC_DBG("hw(%d_%d),addr(0x%x_0x%x)\n",master,slave1,Addr[master],Addr[slave1]);\
    pDrv[master] = 0;\
    pDrv[slave1] = 0;\
    Addr[master] = 0;\
    Addr[slave1] = 0;\
...
#define SET_CQ0_BA_D_SUB_DUMMY(master, slave1) ({\
    CAM_WRITE_REG(pDrv_dummy[master]->getPhyObj(), CAM_CTL_CD_DONE_SEL, 0x10000);\
    CAM_WRITE_REG(pDrv_dummy[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x1);\
    CAM_WRITE_REG(pDrv_dummy[master], CAM_CQ_THR0_BASEADDR, Addr_dummy[master]);\
    CAM_WRITE_REG(pDrv_dummy[master], CAM_CQ_THRE0_ADDR_INNER, Addr_dummy[master]);\
    CAM_WRITE_REG(pDrv_dummy[slave1], CAM_CQ_THR0_BASEADDR, Addr_dummy[slave1]);\
    CAM_WRITE_REG(pDrv_dummy[slave1], CAM_CQ_THRE0_ADDR_INNER, Addr_dummy[slave1]);\
    CAM_WRITE_REG(pDrv_dummy[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x0);\
    CAM_FUNC_DBG("hw(%d_%d),sub dummy addr(0x%x_0x%x)\n",master,slave1,Addr_dummy[master],Addr_dummy[slave1]);\
    pDrv_dummy[master] = 0;\
    pDrv_dummy[slave1] = 0;\
    Addr_dummy[master] = 0;\
    Addr_dummy[slave1] = 0;\
...
#define SET_CQ0_BA_T(master, slave1, slave2) ({\
    CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CTL_CD_DONE_SEL, 0x10000);\
    CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x1);\
    if(this->m_bSubsample == MTRUE){\
        CAM_WRITE_REG(pDrv[master], CAM_CQ_THR0_BASEADDR, Addr[master]);\
        ...
        CAM_WRITE_REG(pDrv[slave1], CAM_CQ_THR0_BASEADDR, Addr[slave1]);\
        CAM_WRITE_REG(pDrv[slave1], CAM_CQ_THRE0_ADDR_INNER, Addr[slave1]);\
        CAM_WRITE_REG(pDrv[slave2], CAM_CQ_THR0_BASEADDR, Addr[slave2]);\
        CAM_WRITE_REG(pDrv[slave2], CAM_CQ_THRE0_ADDR_INNER, Addr[slave2]);\
    } else {\
        CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CQ_THR0_BASEADDR, Addr[master]);\
        CAM_WRITE_REG(pDrv[slave1]->getPhyObj(), CAM_CQ_THR0_BASEADDR, Addr[slave1]);\
        CAM_WRITE_REG(pDrv[slave2]->getPhyObj(), CAM_CQ_THR0_BASEADDR, Addr[slave2]);\
    }\
    CAM_WRITE_REG(pDrv[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x0);\
    CAM_FUNC_DBG("hw(%d_%d_%d),addr(0x%x_0x%x_0x%x)\n",master,slave1,slave2,Addr[master],Addr[slave1],Addr[slave2]);\
    pDrv[master] = 0;\
    pDrv[slave1] = 0;\
    pDrv[slave2] = 0;\
    Addr[master] = 0;\
...
#define SET_CQ0_BA_T_SUB_DUMMY(master, slave1, slave2) ({\
    CAM_WRITE_REG(pDrv_dummy[master]->getPhyObj(), CAM_CTL_CD_DONE_SEL, 0x10000);\
    CAM_WRITE_REG(pDrv_dummy[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x1);\
    CAM_WRITE_REG(pDrv_dummy[master], CAM_CQ_THR0_BASEADDR, Addr_dummy[master]);\
    CAM_WRITE_REG(pDrv_dummy[master], CAM_CQ_THRE0_ADDR_INNER, Addr_dummy[master]);\
    CAM_WRITE_REG(pDrv_dummy[slave1], CAM_CQ_THR0_BASEADDR, Addr_dummy[slave1]);\
    CAM_WRITE_REG(pDrv_dummy[slave1], CAM_CQ_THRE0_ADDR_INNER, Addr_dummy[slave1]);\
    CAM_WRITE_REG(pDrv_dummy[slave2], CAM_CQ_THR0_BASEADDR, Addr_dummy[slave2]);\
    CAM_WRITE_REG(pDrv_dummy[slave2], CAM_CQ_THRE0_ADDR_INNER, Addr_dummy[slave2]);\
    CAM_WRITE_REG(pDrv_dummy[master]->getPhyObj(), CAM_CTL_UNI_DONE_SEL, 0x0);\
    CAM_FUNC_DBG("hw(%d_%d_%d),sub dummy addr(0x%x_0x%x_0x%x)\n",master,slave1,slave2,Addr_dummy[master],Addr_dummy[slave1],Addr_dummy[slave2]);\
    pDrv_dummy[master] = 0;\
    pDrv_dummy[slave1] = 0;\
    pDrv_dummy[slave2] = 0;\
    Addr_dummy[master] = 0;\
...
//Use to prevent twin's dummy CQ update inconsistent
uintptr_t    DMA_CQ0::Addr_dummy[PHY_CAM] = {0,};
ISP_DRV_CAM* DMA_CQ0::pDrv_dummy[PHY_CAM] = {0,};

MBOOL DMA_CQ0::Init(void)
{
    pDrv[this->m_hwModule] = 0;
    Addr[this->m_hwModule] = 0;
    pDrv_dummy[this->m_hwModule] = 0;
    Addr_dummy[this->m_hwModule] = 0;
    return 0;
}
...
MINT32 DMA_CQ0::_config( void )
{
    ...
    //dbg log
    this->m_pDrv->getCurObjInfo(&this->m_hwModule,&cq,&page);
    CAM_FUNC_DBG("DMA_CQ0::_config+ [%s]: cq(0x%x),page(0x%x),twin(0x%x),pa(0x%" PRIXPTR "),ofst(0x%08X),size(%ld,%ld,%ld,%ld),pixel_byte(%d),crop(%d,%d)\n",this->name_Str(),cq,page,\
        CAM_READ_REG(this->m_pDrv,CAM_CTL_TWIN_STATUS),
        this->dma_cfg.memBuf.base_pAddr, \
        this->dma_cfg.memBuf.ofst_addr,
        this->dma_cfg.size.w, \
        this->dma_cfg.size.h, \
        ...
        case CAM_DMA_CQ0I:
            {
                ...
                if(CamInfo.m_DTwin.GetDTwin() == MFALSE){
                    ...
                    if((this->m_bSubsample == MTRUE) && (page >= this->m_bBurstQ*DupCmdQMgr::DEFAULT_DUP_NUM)){
                        //Use for Dummy CQ consistent control
                        pDrv_dummy[this->m_hwModule] = this->m_pDrv;
                        Addr_dummy[this->m_hwModule] = this->dma_cfg.memBuf.base_pAddr;
                    }else{
                        ...
                        case 2: /* 3 RAWS to 1 tg for static twin mode */
                            if ((Addr[CAM_A] != 0) && (Addr[CAM_B] != 0) && (Addr[CAM_C] != 0)) {
                                if (multiRAWConfig.master_module == CAM_A) {
                                    ...
                                    SET_CQ0_BA_T(CAM_B, CAM_A, CAM_C);
                                } else {
                                    CAM_FUNC_ERR("un-support hwModule:%d\n",this->m_hwModule);
                                }
                            }
                        break;
                        };
                    ...
                    else if((this->m_bSubsample == MTRUE) && (page >= this->m_bBurstQ*DupCmdQMgr::DEFAULT_DUP_NUM)){
                        //Dummy CQ uses this flow to ensure twin's update consistent since dummy cq does not contain twin's info
                        CAM_FUNC_INF("subsample with dummy case slave num:%d\n",multiRAWConfig.slave_cam_num);

                        switch (multiRAWConfig.slave_cam_num) {
                        case 0:
                            if(this->m_hwModule == CAM_A)
                                SET_CQ0_BA_S_SUB_DUMMY(CAM_A);
                            else if(this->m_hwModule == CAM_B)
                                SET_CQ0_BA_S_SUB_DUMMY(CAM_B);
                            else if(this->m_hwModule == CAM_C)
                                SET_CQ0_BA_S_SUB_DUMMY(CAM_C);
                        break;
                        case 1: /* 2 RAWS to 1 tg for static twin mode */
                            if (multiRAWConfig.master_module == CAM_A &&
                                multiRAWConfig.twin_module == CAM_C) {
                                if ((Addr_dummy[CAM_A] != 0) &&
                                    (Addr_dummy[CAM_C] != 0)) {
                                    SET_CQ0_BA_D_SUB_DUMMY(CAM_A, CAM_C);
                                }
                            } else if (multiRAWConfig.master_module == CAM_B &&
                                    multiRAWConfig.twin_module == CAM_C) {
                                if ((Addr_dummy[CAM_B] != 0) &&
                                    (Addr_dummy[CAM_C] != 0)) {
                                    SET_CQ0_BA_D_SUB_DUMMY(CAM_B, CAM_C);
                                }
                            } else {
                                if ((Addr_dummy[CAM_A] != 0) &&
                                    (Addr_dummy[CAM_B] != 0)) {
                                    SET_CQ0_BA_D_SUB_DUMMY(CAM_B, CAM_A);
                                }
                            }
                        break;
                        case 2: /* 3 RAWS to 1 tg for static twin mode */
                            if ((Addr_dummy[CAM_A] != 0) && (Addr_dummy[CAM_B] != 0) && (Addr_dummy[CAM_C] != 0)) {
                                if (multiRAWConfig.master_module == CAM_A) {
                                    SET_CQ0_BA_T_SUB_DUMMY(CAM_A, CAM_B, CAM_C);
                                } else if(multiRAWConfig.master_module == CAM_B) {
                                    SET_CQ0_BA_T_SUB_DUMMY(CAM_B, CAM_A, CAM_C);
                                } else {
                                    CAM_FUNC_ERR("un-support hwModule:%d\n",this->m_hwModule);
                                }
                            }
