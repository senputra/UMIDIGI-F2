    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    pInfo->ionHandle = (MINT32)pIonHandle;
...
MINT32 IMemDrvImp::freeVirtBuf(
    IMEM_BUF_INFO* pInfo, MBOOL bImport)
{
    ...
    //
        ...
        pIonHandle = (ion_user_handle_t)pInfo->ionHandle;
...
MINT32 IMemDrvImp::freeSecBuf(
    IMEM_BUF_INFO* pInfo)
{
    IMEM_DBG("ionHandle[0x%x]/size[0x%x]/VA[0x%x]/PA[0x%x]",
                pInfo->ionHandle, pInfo->size, pInfo->virtAddr, pInfo->phyAddr);
...
//-----------------------------------------------------------------------------
MINT32  IMemDrvImp::doIonCacheFlush(IMEM_BUF_INFO* pInfo, void* virAddr, MUINT32 size, eIONCacheFlushType flushtype, MBOOL bImport)
{
    #if defined (__ISP_USE_ION__)
    ion_user_handle_t pIonHandle;
    MINT32 IonBufFd;
    ...
    if(ion_custom_ioctl(mIonDrv, ION_CMD_SYSTEM,&sys_data))
    {
        IMEM_ERR("CAN NOT DO SYNC, memID/(0x%x)",IonBufFd);
        if(ion_free(mIonDrv,pIonHandle))
        {
            IMEM_ERR("ion_free fail");
            return -1;
...
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::cacheSyncbyRange(IMEM_CACHECTRL_ENUM ctrl, IMEM_BUF_INFO* pInfo, MBOOL bImport)
{
#if defined (__ISP_USE_ION__)
    ...
    else
    {
        ...
        {
            doIonCacheFlush(pInfo, (void*)pInfo->virtAddr, pInfo->size, eIONCacheFlushType_Range, bImport);
            IMEM_DBG("+ Flush by range, c/m/va/sz/pa(%d/0x%x/0x%lx/0x%x/0x%lx)",ctrl,pInfo->memID,(unsigned long)pInfo->virtAddr,pInfo->size,(unsigned long)pInfo->phyAddr);
            return 0;
        }
...
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::initSecureM4U(void)
{
    int m4u_fd;
...
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::allocSecureBuf(
    IMEM_BUF_INFO* pInfo)
{
    if(pInfo->size <= 0)
    {
    ...
    IMEM_DBG("dbg use (memID_0x%x, ion handle_0x%lx)", pInfo->memID,
        pIonHandle);
...
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::freeSecureBuf(
    IMEM_BUF_INFO* pInfo)
{
    ion_user_handle_t pIonHandle;
    MINT32 IonBufFd;
...
//-----------------------------------------------------------------------------
MINT32 IMemDrvImp::setBufferdbgName(
