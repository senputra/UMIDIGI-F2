...
private:
    ...
    BUF_CTRL_YUVO   m_Yuvo_FBC[CAM_MAX];
    BUF_CTRL_YUVBO  m_Yuvbo_FBC[CAM_MAX];
    BUF_CTRL_YUVCO  m_Yuvco_FBC[CAM_MAX];
    DMA_YUVO        m_Yuvo[CAM_MAX];
    DMA_YUVBO       m_Yuvbo[CAM_MAX];//hide from user
    DMA_YUVCO       m_Yuvco[CAM_MAX];//hide from user

    BUF_CTRL_CRZO_R1    m_CrzoR1_FBC[CAM_MAX];
    BUF_CTRL_CRZBO_R1   m_CrzboR1_FBC[CAM_MAX];
    DMA_CRZO_R1         m_CrzoR1[CAM_MAX];
    DMA_CRZBO_R1        m_CrzboR1[CAM_MAX];//hide from user

    BUF_CTRL_CRZO_R2    m_CrzoR2_FBC[CAM_MAX];
    BUF_CTRL_CRZBO_R2   m_CrzboR2_FBC[CAM_MAX];
    DMA_CRZO_R2         m_CrzoR2[CAM_MAX];
    DMA_CRZBO_R2        m_CrzboR2[CAM_MAX];//hide from user
...
public:
    ...
    virtual MINT32                  setP1Update(P1_TUNING_NOTIFY* pCB);
    virtual MINT32                  setP1RrzCfg(list<IspRrzCfg>* pRrz_cfg_L);
    virtual MINT32                  setP1ImgoCfg(list<DMACfg>* pImgo_cfg_L);
    virtual MINT32                  setP1YuvoCfg(list<DMACfg>* pYuvo_cfg_L);
    virtual MINT32                  setP1CrzoR1Cfg(list<IspCrzrCfg>* pCrzo_cfg_L);
    virtual MINT32                  setP1CrzoR2Cfg(list<IspCrzrCfg>* pCrzo_cfg_L);
    virtual MINT32                  setP1RawiCfg(list<DMACfg>* pRawi_cfg_L);
    ...
    virtual MBOOL                   P1Notify_Mapping(MUINT32 cmd,P1_TUNING_NOTIFY* pObj,MBOOL bInitFBC = MTRUE);
    virtual MBOOL                   P1Notify_Global_Mapping(MUINT32 cmd,P1_TUNING_NOTIFY* pObj);
    ...
    virtual MBOOL                   getIsTwin(MBOOL bDeque = MFALSE,MBOOL bDyuv = MFALSE); // this is for camio only,bDeque can only be true if function is involked at deque()
    ...
    virtual void                    setDBLoadHold(MBOOL bHold, vector<MUINT32>* tuning_moduleEn, void* pCmdQ);
...
private:
    ...
    QueueMgr<MBOOL>         m_isTwinDmao[TWIN_DMAO_MAX];
    MBOOL                   m_latesetDmao[TWIN_DMAO_MAX]; /*AFO: latest para. is design for getisTwin is used between deque to next enque.*/
                                                          /*IMGO: due to drv have no frame_end behavior.*/
    MBOOL                   m_latestIsTwin;
    MBOOL                   m_bDoneCollect;
    E_INPUT                 m_TgIdx;
...
#define MAX_QUE_SIZE    (64)
    ...
    DupCmdQMgr*             m_pMainCmdQMgr; // CmdQMgr for main hwModule
    vector<DupCmdQMgr*>     m_pActiveTwinCQMgrs[ISP_DRV_CAM_BASIC_CQ_NUM];// cq0 depth: CAM_MAX; cq1/cq28 depth: 1~2
    ...
    static  Mutex           m_LastP1DoneMutex;//for protecting "last p1 done" & m_bDoneCollect for multi-sensor

#define DISABLE_TWIN    (1)
#define ENABLE_TWIN     (0)

    //T_DLP                  m_dlp;              //xmxo result after configpath()
