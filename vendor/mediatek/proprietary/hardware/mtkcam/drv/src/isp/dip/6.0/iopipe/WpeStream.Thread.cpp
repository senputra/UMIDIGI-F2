    ...
    MUINT32 callerID)
{
    ...
    {  //erase frame package from list
        ...
        LOG_DBG("PKLsize(%zu)",this->mLWPEDQFramePackList.size());
        this->mLWPEDQFramePackList.pop_front();
        LOG_DBG("updateFPList(%zu)",this->mLWPEDQFramePackList.size());
    ...
    {   //erase frame unit from list
        ...
        LOG_DBG("PULsize(%zu)",this->mLWPEwaitDQFrameUnitList.size());
        this->mLWPEwaitDQFrameUnitList.pop_front();
        LOG_DBG("updatePULsize(%zu)",this->mLWPEwaitDQFrameUnitList.size());
...
dequeueJudgement()
{
    ...
    unsigned long sizeofwaitDequeUnit = 0;
    unsigned long sizeofDQFramePackList = 0;
    ...
    MINT32 mdp_err = -1;
    QParams rParams;
    list<WPEFramePackage>::iterator it_pack;

    this->getWPELock(EWPELockEnum_wDQFramePackList);
    sizeofDQFramePackList=(unsigned long)(this->mLWPEDQFramePackList.size());
    if (sizeofDQFramePackList <=0)
    {
        LOG_ERR("WPE FramePackList size is zero!! It's impossible!! State Machine is wrong!!");
        this->releaseWPELock(EWPELockEnum_wDQFramePackList);
        return MFALSE;
    }
    LOG_DBG("dequeueJudgement, sizeofFramePackList(%ld)",sizeofDQFramePackList);
    it_pack = this->mLWPEDQFramePackList.begin();
    this->releaseWPELock(EWPELockEnum_wDQFramePackList);

    this->getWPELock(EWPELockEnum_wDQFrameUnitList);
    sizeofwaitDequeUnit=(unsigned long)(this->mLWPEwaitDQFrameUnitList.size());
    if (sizeofwaitDequeUnit <=0)
    {
        LOG_ERR("WPE Frame DequeUnit size is zero!! It's impossible!! State Machine is wrong!!");
        this->releaseWPELock(EWPELockEnum_wDQFrameUnitList);
        return MFALSE;
    }
    LOG_DBG("dequeueJudgement, sizeofwaitDequeUnit(%ld)",sizeofwaitDequeUnit);
    ...
    rParams = frameunit.rParams;

    if((wpeDupIdx != (*it_pack).DupIdx) || (callerID != (*it_pack).callerID))
    {
          LOG_WRN("WPE dequeueJudgement frameunit callerID(0x%x) wpeDupIdx(%d);FrmPackList callerID(0x%x) wpeDupIdx(%d)",
                               frameunit.callerID, frameunit.wpeDupIdx, (*it_pack).callerID, (*it_pack).DupIdx);
    }
    ...
    mdp_err = this->mwarpengine->dequeue(wpeDupIdx, wpeBurstIdx, DupCmdIdx, rParams);
    LOG_DBG("dequeueJudgement, mdp_err(%d)", mdp_err);
    ...
    if(mdp_err == 0)
    {
        ...
        frm = (*it_pack).dequedNum;

        if((*it_pack).rParams.mvFrameParams[wpeBurstIdx].mvOut.size()>0)
        {
            LOG_DBG("WPE(0x%x)_FmvOut(0x%" PRIxPTR "),wpeBurstIdx(%d)",frameunit.callerID, (*it_pack).rParams.mvFrameParams[wpeBurstIdx].mvOut[0].mBuffer->getBufPA(0),wpeBurstIdx);
        }
        else
        {
            LOG_ERR("no output dma, index(%d)",wpeBurstIdx);
        }
        LOG_DBG("dequeueJudgement mLWPEDQFramePackList[%d]: dequedNum(%d)", index, (*it_pack).dequedNum);

        (*it_pack).dequedNum ++;; //get real current index in batch list (HalpipeWrapper.Thread would delete element from batch list if deque success)

        LOG_INF("Aftter DQ mLWPEDQFramePackList:dequedNum(%d), frameNum(%d)", (*it_pack).dequedNum, (*it_pack).frameNum);
        if((*it_pack).dequedNum == (*it_pack).frameNum)
        {
            dequeFramePackDone = true;
            (*it_pack).rParams.mDequeSuccess = MTRUE;
        }
        ...
        if(!dequeFramePackDone)
        {
            ...
            LOG_INF("+,[Dequeue FramePack is NOT Done]tag(%d),WPE Caller(0x%x),framenum_dupidx(%d/%d)",\
                     EWPEBufferListTag_Unit, frameunit.callerID, frameunit.wpeBurstIdx, frameunit.wpeDupIdx);
        }
        else
        {
            if((*it_pack).rParams.mpfnCallback == NULL)
            {
                ...
                LOG_INF("+,tag(%d),WPE Caller(0x%x),framenum_dupidx(%d/%d),\n\
            ...
            else
            {
                ...
                MUINT32 a=0,b=0;
                LOG_DBG("idx(%d)",index);
                WPEFramePackage framepack = *it_pack;
                LOG_DBG("WPE deque done callback (%p)=",framepack.rParams.mpfnCallback);
                a=getUs();
