...
int TEST_CAMSVIO::Case_0(void)
{
    ...
    for(MUINT32 i=0;i<(BUF_DEPTH* (this->m_subSample+1));i++){
        ...
        if(this->bypass_imgo == MFALSE){
            ...
            if ( mpImemDrv->allocVirtBuf(&this->buf_imgo[i]) ){
                LOG__I("m_pMemDrv->allocVirtBuf fail");
                return MFALSE;
            }
            else{
                if ( mpImemDrv->mapPhyAddr(&this->buf_imgo[i]) ) {
                    LOG__I("ERROR:m_pIMemDrv->mapPhyAddr");
                    return MFALSE;
                }
            }
            ...
            if ( mpImemDrv->allocVirtBuf(&this->buf_imgo_fh[i]) ){
                LOG__I("m_pMemDrv->allocVirtBuf fail");
                return MFALSE;
            }
            else{
                if ( mpImemDrv->mapPhyAddr(&this->buf_imgo_fh[i]) ) {
                    LOG__I("ERROR:m_pIMemDrv->mapPhyAddr");
                    return MFALSE;
                }
            }
