    ...
    switch(InPut){
        case TG_A:
            if(gCamioPipe[CAM_A].occupied == MTRUE){
                ...
                std::strcpy((char*)gCamioPipe[CAM_A].m_szUsrName,(char const*)szUsrName);
        ...
        case TG_B:
            if(gCamioPipe[CAM_B].occupied == MTRUE){
                ...
                std::strcpy((char*)gCamioPipe[CAM_B].m_szUsrName,(char const*)szUsrName);
...
MBOOL CamIOPipe::suspend(E_SUSPEND_MODE suspendMode)
{
    ...
    return this->m_CamPathPass1.suspend();
...
MBOOL CamIOPipe::resume(E_SUSPEND_MODE suspendMode)
{
    ...
    return this->m_CamPathPass1.resume();
}
        ...
        case EPIPECmd_GET_HBIN_INFO:
            ...
            //arg2: output size_h
            {
                ...
                *(MUINT32*)arg2  = size_v;
                ...
                //qbin3,pix mode must be the same with qbin1
                if(pixMode != CAM_READ_BITS(this->m_pCmdQMgr->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(),CAM_QBN2_MODE, QBN_ACC)){
                    PIPE_ERR("QBIN1 pix mode is different from QBIN3:0x%x_0x%x\n",pixMode\
                        ,UNI_READ_BITS(this->m_UniMgr.UniMgr_GetCurObj(),CAM_UNI_QBN3_A_MODE, QBN_ACC));
                }
    ...
    switch(PortID){
        ...
        case EPortIndex_EISO:
            if(this->m_UniMgr.UniMgr_GetCurLink() ==  this->m_hwModule)
                ...
                PIPE_ERR("EIS is occupied by cam:0x%x",this->m_UniMgr.UniMgr_GetCurLink());
            break;
        case EPortIndex_RSSO:
            if(this->m_UniMgr.UniMgr_GetCurLink() ==  this->m_hwModule)
                ...
                PIPE_ERR("RSS is occupied by cam:0x%x",this->m_UniMgr.UniMgr_GetCurLink());
            break;
        default:
