...
int CamPathPass2::config( struct CamPathPass2Parameter* p_parameter )
{
    ...
    ISP_PATH_DBG("+,CamPathPass2:tdri(%d),dupCqIdx(%d),burstQueIdx(%d)",p_parameter->tpipe,p_parameter->dupCqIdx,p_parameter->burstQueIdx);
...
//////kk test
    ...
    ISP_PATH_INF("cq(%d),drvScenario(%d),hwScenario(%d),sub_mode(%d),p1_en(0x%08x),p2_en(0x%08x),p2_dma(0x%08x),burstQueIdx(%d),dupCqIdx(%d),tdri(%d)", \
            ...
            p_parameter->isp_top_ctl.CAM_CTL_EN_P1.Raw,p_parameter->isp_top_ctl.CAM_CTL_EN_P2.Raw, p_parameter->isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw, \
            p_parameter->burstQueIdx,p_parameter->dupCqIdx,p_parameter->tpipe);
...
//    cdpPipe.conf_cdrz = 0;
    //
    ISP_PATH_DBG("[mdpcrop] imgi in[%lx, %lx] mdpCrop[%d, %d, %lx, %lx]_f(0x%x, 0x%x)",p_parameter->imgi.size.w,p_parameter->imgi.size.h, \
    ...
    ISP_PATH_DBG("[CRZ]en(%d),in[%lx,%lx],out[%lx,%lx],sl2cCrzRatio(%f)",p_parameter->croppara.crz_enable,crzPipe.crz_in.w,crzPipe.crz_in.h,crzPipe.crz_out.w,crzPipe.crz_out.h,ispYuvPipe.sl2cCrzRatio);
    ISP_PATH_DBG("[CRZ][CROP](x,y,w,h)=(%d,%d,%lx,%lx),floatx,y=(%u,%u)",crzPipe.crz_crop.x,crzPipe.crz_crop.y,crzPipe.crz_crop.w,crzPipe.crz_crop.h,crzPipe.crz_crop.floatX,crzPipe.crz_crop.floatY);
    ...
    ISP_PATH_DBG("CDRZ:in[%lx, %lx] crop[%d %d %lx %lx]_f(0x%x, 0x%x) out[%lx,%lx] ",crzPipe.crz_in.w,crzPipe.crz_in.h, \
    ...
    ISP_PATH_DBG("[imgi](%lx , %lx , %lx) vidoRotate(%d),pa(0x%x) ", p_parameter->imgi.size.w, p_parameter->imgi.size.h, p_parameter->imgi.size.stride,crzPipe.vido_out.Rotation,p_parameter->imgi.memBuf.base_pAddr);
    ...
    if ( CAM_CTL_EN_P2_DMA_IMG3O_EN & ispTopCtrl.isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw)
    {
        ...
        ISP_PATH_DBG("[IMG3O]bypass(%d),size(0x%x),Va(0x%x),Pa(0x%x),(w,h,s)=(%lx,%lx,%lx),crop(x,y,w,h)=(%d,%d,%lx,%lx)", \
    ...
    if ( CAM_CTL_EN_P2_DMA_IMG2O_EN & ispTopCtrl.isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw){
        ...
        ISP_PATH_DBG("[IMG2O]bypass(%d),size(0x%x),Va(0x%x),Pa(0x%x),(w,h,s)=(%lx,%lx,%lx),crop(x,y,w,h)=(%d,%d,%lx,%lx)", \
    ...
    tdriPipe.tdr_dbg_tcm_en = p_parameter->tdr_dbg_tcm_en; // for control extension
    //CAM_TDRI_PIPE
    ...
    if ( p_parameter->tpipe == CAM_MODE_TPIPE ) {
        ...
        tdriPipe.tdr_en = (p_parameter->tdr_en&0xdf800000)\
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
    ...
    tdriPipe.tdri.top.ctl_extension_en = (p_parameter->tcm_en&0x40000000)>>30;//0; //kk test	// for control extension
    ISP_PATH_DBG("[Tdri]tcm_en(%d), ctl_extension_en(%d)",p_parameter->tcm_en, tdriPipe.tdri.top.ctl_extension_en);// for control extension
    ...
    tdriPipe.tdri.top.pca_en = pP2En->Bits.PCA_EN;
    ...
    {
        // mfb
        tdriPipe.tdri.mfb.bld_deblock_en = p_parameter->pTuningIspReg->CAM_MFB_LL_CON3.Bits.BLD_LL_DB_EN;
        tdriPipe.tdri.mfb.bld_brz_en = p_parameter->pTuningIspReg->CAM_MFB_LL_CON3.Bits.BLD_LL_BRZ_EN;
    }
