...
MINT32 CAM_TWIN_PIPE::update_afterRunTwin_Dma()
{
    ...
    if(CAM_READ_REG(this->m_pDrv,CAM_CTL_SEL) & IMG_SEL_1){
        if(CAM_READ_BITS(this->m_pTwinIspDrv,CAM_CTL_DMA_EN,IMGO_EN)){
             CAM_FUNC_ERR("force disable slave(%d)'s imgo when master(%d) is pure IMGO", this->m_twinHwModule, this->m_hwModule);
             imgo.disable(NULL);
        }
        if(!CAM_READ_BITS(this->m_pDrv,CAM_CTL_DMA_EN,IMGO_EN)){
            CAM_FUNC_WRN("IMGO disabled with IMG_SEL_1\n");
        }
        /*set CAMB VBN_GAIN,DBN_GAIN here , need match CAMA setting*/
        CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_VBN_GAIN,VBN_GAIN,256);
        CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_DBN_GAIN,DBN_GAIN,256);
    }
    ...
    if(!(CAM_READ_REG(this->m_pDrv,CAM_CTL_SEL) & IMG_SEL_1) &&
        CAM_READ_BITS(this->m_pDrv,CAM_CTL_DMA_EN,IMGO_EN) != (CAM_READ_BITS(this->m_pTwinIspDrv,CAM_CTL_DMA_EN,IMGO_EN) )){
