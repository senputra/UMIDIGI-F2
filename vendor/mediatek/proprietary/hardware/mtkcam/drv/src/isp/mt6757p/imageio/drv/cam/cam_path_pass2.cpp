...
int CamPathPass2::config( struct CamPathPass2Parameter* p_parameter )
{
    ...
    if(ispYuvPipe.bypass == 0) {
		...
		//srz5
        if(MTRUE == p_parameter->isp_top_ctl.YUV2_EN.Bits.SRZ5_EN)
        {

            //ctrl
            MUINT32 ctrl=0x3;   //srz5_vert_en=1 & srz5_hori_en=1
            if(p_parameter->srz5_cfg.crop.w>p_parameter->srz5_cfg.inout_size.in_w)
            {
                ctrl |= 0x10;
            }
            p_parameter->srz5_cfg.ctrl=ctrl;
            //update floating offset for tpipe /*20 bit base*/
            p_parameter->srz5_cfg.crop.floatX= \
                ((p_parameter->srz5_cfg.crop.floatX>>(CROP_FLOAT_PECISE_BIT-CROP_TPIPE_PECISE_BIT)) & ((1<<CROP_TPIPE_PECISE_BIT)-1));/* tpipe calculate via 0~19 bits */
            p_parameter->srz5_cfg.crop.floatY= \
                ((p_parameter->srz5_cfg.crop.floatY>>(CROP_FLOAT_PECISE_BIT-CROP_TPIPE_PECISE_BIT)) & ((1<<CROP_TPIPE_PECISE_BIT)-1));/* tpipe calculate via 0~19 bits */
            //coeffienct value for tpipe structure
            //p_parameter->srz5_cfg.h_step=(( p_parameter->srz5_cfg.crop.w-1)*32768+((p_parameter->srz5_cfg.inout_size.out_w-1)>>1) )/(p_parameter->srz5_cfg.inout_size.out_w-1);
            //p_parameter->srz5_cfg.v_step=(( p_parameter->srz5_cfg.crop.h-1)*32768+((p_parameter->srz5_cfg.inout_size.out_h-1)>>1) )/(p_parameter->srz5_cfg.inout_size.out_h-1);
            p_parameter->srz5_cfg.h_step=((p_parameter->srz5_cfg.crop.w-1)*32768+(((((p_parameter->srz5_cfg.inout_size.out_w+31)>>5)<<5)-1)>>1) )/((((p_parameter->srz5_cfg.inout_size.out_w+31)>>5)<<5)-1);
            p_parameter->srz5_cfg.v_step=((p_parameter->srz5_cfg.crop.h-1)*32768+(((((p_parameter->srz5_cfg.inout_size.out_h+31)>>5)<<5)-1)>>1) )/((((p_parameter->srz5_cfg.inout_size.out_h+31)>>5)<<5)-1);
            p_parameter->srz5_cfg.inout_size.in_w=((((long long)p_parameter->srz5_cfg.h_step*(p_parameter->srz5_cfg.inout_size.out_w-1))+16384)>>15)+1;
            p_parameter->srz5_cfg.inout_size.in_h=((((long long)p_parameter->srz5_cfg.v_step*(p_parameter->srz5_cfg.inout_size.out_h-1))+16384)>>15)+1;
            //
            memcpy(&ispYuvPipe.srz5Cfg, &p_parameter->srz5_cfg, sizeof(SrzCfg));
            //
            ISP_PATH_DBG("SRZ5:ctrl(0x%x), in[%d, %d] crop[%d/%d_0x%x/0x%x]_(%d, %d) out[%d,%d, cof(0x%x/0x%x)] ",p_parameter->srz5_cfg.ctrl, p_parameter->srz5_cfg.inout_size.in_w, p_parameter->srz5_cfg.inout_size.in_h, \
                p_parameter->srz5_cfg.crop.x, p_parameter->srz5_cfg.crop.y, p_parameter->srz5_cfg.crop.floatX, p_parameter->srz5_cfg.crop.floatY, \
                p_parameter->srz5_cfg.crop.w, p_parameter->srz5_cfg.crop.h, p_parameter->srz5_cfg.inout_size.out_w, p_parameter->srz5_cfg.inout_size.out_h,\
                p_parameter->srz5_cfg.h_step, p_parameter->srz5_cfg.v_step);
        }
    ...
    if(ispMdpPipe.bypass == 0) {
        ...
        //ispMdpPipe.DCParam = p_parameter->pDCParam;
        //ispMdpPipe.CZParam = p_parameter->pCZParam;
        //ispMdpPipe.VSDOFPQParam = p_parameter->pVSDOFPQParam;
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
    ...
    tdriPipe.tdri.top.srz5_en = pYuv2En->Bits.SRZ5_EN;
    ...
    tdriPipe.tdri.top.flc_en = pRgbEn->Bits.FLC_EN;
    tdriPipe.tdri.top.flc2_en = pRgbEn->Bits.FLC2_EN;
    ...
    tdriPipe.tdri.mfb.bld_mbd_wt_en = tuningIspReg->DIP_X_MFB_CON.Bits.BLD_MBD_WT_EN;
    //
    ...
    //
    if(tdriPipe.tdri.top.srz5_en)
    {
        tdriPipe.tdri.srz5.srz_input_crop_width = p_parameter->srz5_cfg.inout_size.in_w;
        tdriPipe.tdri.srz5.srz_input_crop_height = p_parameter->srz5_cfg.inout_size.in_h;
        tdriPipe.tdri.srz5.srz_output_width = p_parameter->srz5_cfg.inout_size.out_w;
        tdriPipe.tdri.srz5.srz_output_height = p_parameter->srz5_cfg.inout_size.out_h;
        tdriPipe.tdri.srz5.srz_luma_horizontal_integer_offset = p_parameter->srz5_cfg.crop.x;
        tdriPipe.tdri.srz5.srz_luma_horizontal_subpixel_offset = p_parameter->srz5_cfg.crop.floatX;
        tdriPipe.tdri.srz5.srz_luma_vertical_integer_offset = p_parameter->srz5_cfg.crop.y;
        tdriPipe.tdri.srz5.srz_luma_vertical_subpixel_offset = p_parameter->srz5_cfg.crop.floatY;
        tdriPipe.tdri.srz5.srz_horizontal_coeff_step = p_parameter->srz5_cfg.h_step;
        tdriPipe.tdri.srz5.srz_vertical_coeff_step = p_parameter->srz5_cfg.v_step;
    }
    else
    {
        tdriPipe.tdri.srz5.srz_input_crop_width = 0;
        tdriPipe.tdri.srz5.srz_input_crop_height = 0;
        tdriPipe.tdri.srz5.srz_output_width = 0;
        tdriPipe.tdri.srz5.srz_output_height = 0;
        tdriPipe.tdri.srz5.srz_luma_horizontal_integer_offset = 0;
        tdriPipe.tdri.srz5.srz_luma_horizontal_subpixel_offset = 0;
        tdriPipe.tdri.srz5.srz_luma_vertical_integer_offset = 0;
        tdriPipe.tdri.srz5.srz_luma_vertical_subpixel_offset = 0;
        tdriPipe.tdri.srz5.srz_horizontal_coeff_step = 0;
        tdriPipe.tdri.srz5.srz_vertical_coeff_step = 0;
    }
    //
    ...
    tdriPipe.tdri.nr3d.tnr_color_en = tuningIspReg->DIP_X_CAM_COLOR_START.Bits.COLOR_DISP_COLOR_START;
    tdriPipe.tdri.nr3d.tnr_en = tuningIspReg->DIP_X_CAM_TNR_ENG_CON.Bits.NR3D_CAM_TNR_EN;
    //
