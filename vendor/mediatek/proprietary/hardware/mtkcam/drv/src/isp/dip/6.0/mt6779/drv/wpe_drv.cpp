...
EXIT:
    ...
    LOG_DBG("-,ret: %d. mInitCount:(%d),m_pWpeHwRegAddr(%p)", Result, this->m_UserCnt, m_pWpeHwRegAddr);
    return Result;
}
...
MBOOL WpeDrvImp::writeReg(MUINT32 Addr,unsigned long Data,MINT32 caller)
{
    MINT32 ret=0;
    MUINT32 legal_range = WPE_REG_RANGE;
    LOG_DBG("Wpe_write:m_regRWMode(0x%x),(0x%x,0x%lx)",this->m_regRWMode,Addr,Data);
    android::Mutex::Autolock lock(this->WpeRegMutex);
    (void)caller;
    if(-1 == FD_CHK()){
        return MFALSE;
