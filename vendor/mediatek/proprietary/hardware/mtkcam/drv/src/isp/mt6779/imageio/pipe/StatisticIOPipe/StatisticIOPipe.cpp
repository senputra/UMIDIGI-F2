...
STTIOPipe* STTIOPipe::Create(MINT8 const szUsrName[32], const E_INPUT& InPut)
{
    ...
    switch(InPut){
        ...
        case TG_B:
            ...
            break;
        case TG_C:
            if(*gSttioPipe[CAM_C].m_ResMgr.Get_Res() != '\0'){
                BASE_LOG_ERR("TG_C is occupied by user:%s\n",gSttioPipe[CAM_C].m_ResMgr.Get_Res());
            }
            else{
                gSttioPipe[CAM_C].m_ResMgr.m_hwModule = gSttioPipe[CAM_C].m_hwModule = CAM_C;
                gSttioPipe[CAM_C].m_ResMgr.Register_Res((char*)szUsrName);

                ptr = &gSttioPipe[CAM_C];
            }
            break;
        default:
...
MBOOL STTIOPipe::uninit()
{
    ...
    if(this->m_byPassAFO == MFALSE){
        ...
        //
        if(pTwin->getIsTwin((MUINT32)_afo_)) {
...
bypassAF:
    ...
    if(this->m_byPassPDO == MFALSE){
        ...
        if(CamInfo.m_IQVer.GetInfo(&ver) == MFALSE){
            PIPE_ERR("get version fail\n");
            ver = capibility::D_IQ_Version::IQ_VER_UNKNOWN;
        }
        if(ver == capibility::D_IQ_Version::IQ_VER_2){
            for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetDuqQ();j++){
                for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetBurstQ();i++){
                    this->m_PDO_FBC.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(j)[i];
        ...
        this->m_DPD_CTRL.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(0)[0];
        this->m_DPD_CTRL.disable(NULL);
...
bypassFLK:
    ...
    if(this->m_byPassTSFSO == MFALSE){
        ...
        this->m_TSFSO_FBC.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetCurCycleObj(0)[0];
        this->m_TSFSO_FBC.disable();

        for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetDuqQ();j++){
            for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetBurstQ();i++){
                this->m_TSFSO.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(j)[i];
                this->m_TSFSO.disable();
            }
        }
bypassTSFS:
...
MBOOL STTIOPipe::resume(E_SUSPEND_MODE suspendMode)
{
    ...
    if(this->m_byPassPDO == MFALSE){
        ...
        for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetDuqQ();j++){
            for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetBurstQ();i++){
                ...
                this->m_DPD_CTRL.enable(NULL);

                if(ver == capibility::D_IQ_Version::IQ_VER_2){
                    this->m_PDO_FBC.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(j)[i];
                    this->m_PDO_FBC.config();
                    this->m_PDO_FBC.write2CQ(this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetBurstQ());
    ...
    if(this->m_byPassTSFSO == MFALSE){
        for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetDuqQ();j++){
            for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetBurstQ();i++){
                this->m_TSFSO.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(j)[i];
                this->m_TSFSO.enable(NULL);
            }
        }
        this->m_TSFSO_FBC.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetCurCycleObj(0)[0];
        this->m_TSFSO_FBC.enable((void*)&m_subsample);

        //
        this->m_TSFSO_FBC.m_pTimeStamp = CAM_TIMESTAMP::getInstance(this->m_hwModule,
                this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetCurCycleObj(0)[0]);

        // cq_resume will be executing a immediate-trig.
        // So cq_resume should be invoked after setting of virtual register.
        if(this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_resume()){
            PIPE_ERR("[%d]:CQ12 resume fail in resume\n",this->m_hwModule);
            return MFALSE;
        }
...
MBOOL STTIOPipe::abortDma(PortID const port)
{
    ...
    switch(dmaChannel){
        ...
        case _flko_:
            ...
            break;
        case _tsfso_:
            if(m_byPassTSFSO == MTRUE){
                return MTRUE;
            }
            cq = ISP_DRV_CQ_THRE12;
            irq.St_type = DMA_INT;
            irq.Status = TSFSO_DONE_ST;
            break;
        default:
...
E_BUF_STATUS STTIOPipe::dequeOutBuf(PortID const portID, QTimeStampBufInfo& rQBufInfo, MUINT32 const u4TimeoutMs, CAM_STATE_NOTIFY *pNotify)
{
    ...
    switch(dmaChannel){
        ...
        default:
            ...
            if(dmaChannel == _pdo_){
                buf.m_pPrivate = (MUINT32*)&this->m_Dlp;
            }
            //check if there is already filled buffer
    ...
    for(MUINT32 i=0;i<this->m_pCmdQMgr[cq]->CmdQMgr_GetBurstQ();i++){
        ...
        rQBufInfo.u4SensorId = buf.sensorId;

        //
        PIPE_DBG("burstidx:0x%x: PortID=%d, bufInfo:(MEMID:%d),(VA:0x%08zx),(PA:0x%08zx),(FH_VA:0x%08zx) SensorID:%d\n",\
            i,\
...
MBOOL STTIOPipe::configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, CAMIO_Func const *pInFunc, CAMIO_Func *pOutFunc)
{
    ...
    for (MUINT32 i = 0 ; i < vOutPorts.size() ; i++ ) {
        ...
        PIPE_INF("statistic vOutPorts:[%d]:%d\n", i, vOutPorts[i]->index);
        ...
        switch(vOutPorts[i]->index){
            ...
            case EPortIndex_TSFSO:
                ...
                if (vInPorts.size()) {
                    ...
                    case eImgFmt_VYUY:
                        this->m_byPassTSFSO = MTRUE;
                        PIPE_ERR("TSFSO r not supported at non-bayer format\n");
                        break;
                    default:
                        this->m_byPassTSFSO = MFALSE;
                        break;
                    }
                }
                else
    ...
    //afo
    if(this->m_byPassAFO == MFALSE){
        this->m_STT_CTRL.m_SttSel[STT_SEL_CTRL::E_CRP_R1_SEL] = pInFunc->Bits.SRC_AF;
        ...
        if(pTwin->getIsTwin((MUINT32)_afo_)) {
            ...
            for(it = l_module.begin();it!=l_module.end();it++){
                ...
                for(MUINT32 j=0;j<this->m_Twin.m_pTwinCQMgr[ISP_DRV_CQ_THRE0][*it]->CmdQMgr_GetDuqQ();j++){
                    for(MUINT32 i=0;i<this->m_Twin.m_pTwinCQMgr[ISP_DRV_CQ_THRE0][*it]->CmdQMgr_GetBurstQ();i++){
                        this->m_Twin.m_AFO[*it].m_pIspDrv = (IspDrvVir*)this->m_Twin.m_pTwinCQMgr[ISP_DRV_CQ_THRE0][*it]->CmdQMgr_GetCurCycleObj(j)[i];
                        this->m_Twin.m_AFO[*it].write2CQ(i);
                        //this->m_AFO.config();remove, setting via cam_tuning_ctrl
                        this->m_Twin.m_AFO[*it].enable(NULL);
                    }
                }
    ...
    //flko
    if(this->m_byPassFLKO == MFALSE){
        this->m_STT_CTRL.m_SttSel[STT_SEL_CTRL::E_FLK_SEL] = pInFunc->Bits.SRC_FLK;
    ...
    if(this->m_byPassTSFSO == MFALSE){
        for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetDuqQ();j++){
            for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetBurstQ();i++){
                this->m_TSFSO.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetCurCycleObj(j)[i];
                this->m_TSFSO.Init();
            }
        }
        for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetDuqQ();j++){
            for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetBurstQ();i++){
                this->m_TSFSO.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(j)[i];
                this->m_TSFSO.write2CQ(i);
                //this->m_TSFSO.config();, remove, setting via cam_tuning_ctrl
                this->m_TSFSO.enable(NULL);
            }
        }
        this->m_TSFSO_FBC.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE12]->CmdQMgr_GetCurCycleObj(0)[0];
        this->m_TSFSO_FBC.config();
        this->m_TSFSO_FBC.enable((void*)&subsample);
    ...
    for(MUINT32 j=0;j<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetDuqQ();j++){
        for(MUINT32 i=0;i<this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetBurstQ();i++){
            this->m_STT_CTRL.m_pIspDrv = (IspDrvVir*)this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(j)[i];
            this->m_STT_CTRL.config();
        }
    }
    ...
    switch ( cmd ) {
        ...
        case EPIPECmd_GET_AAO_CONS:
            *(MUINT32*)arg1 = AAO_MAX_SIZE;
            break;
        #if 0//LafiteEP_TODO: PS/PSO removed
        ...
        case EPIPECmd_GET_PDO_CONS:
            ...
            //arg3: output size_h
            {
                ...
                if(CAM_READ_BITS(this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(),CAMCTL_R1_CAMCTL_SEL,CAMCTL_PDO_SEL) == ePDOSel_0){
                    //input size is useless here. max size is clamped by hw
                    switch(this->m_DPD_CTRL.m_Density){
                        case CAM_DPD_CTRL::_level_1:
                            Width = CAM_READ_BITS(this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(),TG_R1_TG_SEN_GRAB_PXL,TG_PXL_E) - \
                                CAM_READ_BITS(this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(),TG_R1_TG_SEN_GRAB_PXL,TG_PXL_S);
                            Height = CAM_READ_BITS(this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(),TG_R1_TG_SEN_GRAB_LIN,TG_LIN_E) - \
                                CAM_READ_BITS(this->m_pCmdQMgr[ISP_DRV_CQ_THRE0]->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(),TG_R1_TG_SEN_GRAB_LIN,TG_LIN_S);
            ...
            break;
        case EPIPECmd_GET_TSFSO_CONS:
            *(MUINT32*)arg1 = TSFSO_MAX_SIZE;
            break;
        case EPIPECmd_GET_STT_CUR_BUF:
            {
                ...
                case EPortIndex_AAO:
                    ...
                    *(MUINTPTR*)arg2 = CAM_READ_REG(this->m_pCmdQMgr[cq]->CmdQMgr_GetCurCycleObj( \
                    this->m_pCmdQMgr[cq]->CmdQMgr_GetDuqQIdx())[0]->getPhyObj(),AAO_R1_AAO_BASE_ADDR);
                    break;
                case EPortIndex_AFO:
                    cq = ISP_DRV_CQ_THRE5;
                    *(MUINTPTR*)arg2 = CAM_READ_REG(this->m_pCmdQMgr[cq]->CmdQMgr_GetCurCycleObj( \
                    this->m_pCmdQMgr[cq]->CmdQMgr_GetDuqQIdx())[0]->getPhyObj(),AFO_R1_AFO_BASE_ADDR);
                    break;
                case EPortIndex_PDO:
                    cq = ISP_DRV_CQ_THRE7;
                    *(MUINTPTR*)arg2 = CAM_READ_REG(this->m_pCmdQMgr[cq]->CmdQMgr_GetCurCycleObj( \
                    this->m_pCmdQMgr[cq]->CmdQMgr_GetDuqQIdx())[0]->getPhyObj(),PDO_R1_PDO_BASE_ADDR);
                    break;
                case EPortIndex_FLKO:
                    cq = ISP_DRV_CQ_THRE8;
                    *(MUINTPTR*)arg2 = CAM_READ_REG(this->m_pCmdQMgr[cq]->CmdQMgr_GetCurCycleObj( \
                    this->m_pCmdQMgr[cq]->CmdQMgr_GetDuqQIdx())[0]->getPhyObj(),FLKO_R1_FLKO_BASE_ADDR);
                    break;
                case EPortIndex_TSFSO:
                    cq = ISP_DRV_CQ_THRE12;
                    *(MUINTPTR*)arg2 = CAM_READ_REG(this->m_pCmdQMgr[cq]->CmdQMgr_GetCurCycleObj( \
                    this->m_pCmdQMgr[cq]->CmdQMgr_GetDuqQIdx())[0]->getPhyObj(),TSFSO_R1_TSFSO_BASE_ADDR);
                    break;
                default:
                    PIPE_ERR("not support dma(0x%x)\n", _portidx);
                    ret = -1;
...
MBOOL STTIOPipe::configFrame(E_FRM_EVENT event,CAMIO_Func func,ST_CAMIO_WT_INFO *wt_info)
{
    return MTRUE;
}
