    ...
    )
{
    ...
    int planeNum=0, pixelDivNum=0, pixelMulNum=0;
    int cropStartIdx=0;
    ...
    for(int q=0;q<framePack.frameNum;q++)
    {
        ...
        for(MUINT32 i=0;i< qParam.mvFrameParams[q].mvIn.size() ;i++)
        {
            ...
            if(IsDMAPortSupport(qParam.mvFrameParams[q].mvIn[i].mPortID.index)== MFALSE)
            {
            ...
            if(qParam.mvFrameParams[q].mvIn[i].mPortID.index == NSImageio::NSIspio::EPortIndex_IMGI)
            {
                ports[cnt].crop1.x      = 0;
                ...
                ports[cnt].crop1.w      = 0;
                ports[cnt].crop1.floatW = 0;
                ports[cnt].crop1.h      = 0;
                ports[cnt].crop1.floatH = 0;
                ...
                ports[cnt].crop2.w      = 0;
                ports[cnt].crop2.floatW = 0;
                ports[cnt].crop2.h      = 0;
                ports[cnt].crop2.floatH = 0;
                ...
                ports[cnt].crop3.w      = 0;
                ports[cnt].crop3.floatW = 0;
                ports[cnt].crop3.h      = 0;
                ports[cnt].crop3.floatH = 0;
                ports[cnt].resize3.tar_w = 0;
                ports[cnt].resize3.tar_h = 0;
                pipePackageInfo.ColorArrangement = qParam.mvFrameParams[q].mvIn[i].mBuffer->getColorArrangement();
                for(MUINT32 k=0;k<qParam.mvFrameParams[q].mvCropRsInfo.size();k++)
                {
                    //crop1/crop2 may be different in each buffer node
                        switch(qParam.mvFrameParams[q].mvCropRsInfo[k].mGroupID)
                        {
                            case 1:
                                ...
                                ports[cnt].crop1.floatW = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.w_fractional;
                                ports[cnt].crop1.h      = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.s.h;
                                ports[cnt].crop1.floatH = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.h_fractional;
                            ...
                            case 2:
                                ...
                                ports[cnt].crop2.floatW = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.w_fractional;
                                ports[cnt].crop2.h      = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.s.h;
                                ports[cnt].crop2.floatH = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.h_fractional;
                            ...
                            case 3:
                                ...
                                ports[cnt].crop3.floatW = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.w_fractional;
                                ports[cnt].crop3.h      = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.s.h;
                                ports[cnt].crop3.floatH = qParam.mvFrameParams[q].mvCropRsInfo[k].mCropRect.h_fractional;
            ...
            ports[cnt].SecureTag   = qParam.mvFrameParams[q].mvIn[i].mSecureTag;
            ports[cnt].SecHandle   = qParam.mvFrameParams[q].mvIn[i].mSecHandle;
            ...
            ports[cnt].xoffset = ((qParam.mvFrameParams[q].mvIn[i].mOffsetInBytes % ports[cnt].u4Stride[0])/pixelDivNum)*pixelMulNum;//dma x-offset for tile calculation
            ports[cnt].yoffset = qParam.mvFrameParams[q].mvIn[i].mOffsetInBytes / ports[cnt].u4Stride[0];//dma y-offset for tile calculation
            LOG_DBG("[mvIn (%d)] fmt(0x%x),w/h(%d/%d),oft(%d/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%x,0x%x,0x%x) ),pixelDivNum:%d,pixelMulNum:%d,secPort:%d",\
                    ports[cnt].index,ports[cnt].eImgFmt,ports[cnt].u4ImgWidth,ports[cnt].u4ImgHeight,qParam.mvFrameParams[q].mvIn[i].mOffsetInBytes,\
                    ports[cnt].xoffset,ports[cnt].yoffset,ports[cnt].u4Stride[0],ports[cnt].u4Stride[1],ports[cnt].u4Stride[2],\
                    ports[cnt].capbility,(unsigned long)ports[cnt].u4BufVA[0],(unsigned long)ports[cnt].u4BufVA[1],(unsigned long)ports[cnt].u4BufVA[2],(unsigned long)ports[cnt].u4BufPA[0],(unsigned long)ports[cnt].u4BufPA[1],(unsigned long)ports[cnt].u4BufPA[2],\
                    ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2], pixelDivNum, pixelMulNum, ports[cnt].SecureTag);
        ...
        //(3) output dma
        for(MUINT32 i=0;i< qParam.mvFrameParams[q].mvOut.size() ;i++)
        {
            ports[cnt].u4Stride[0]=ports[cnt].u4Stride[1]=ports[cnt].u4Stride[2]=0;
            planeNum = qParam.mvFrameParams[q].mvOut[i].mBuffer->getPlaneCount();
            //
            if(IsDMAPortSupport(qParam.mvFrameParams[q].mvOut[i].mPortID.index) == MFALSE)
            {
                LOG_ERR("not support this port: %d",qParam.mvFrameParams[q].mvOut[i].mPortID.index);
                return -1;
            }

            if(qParam.mvFrameParams[q].mvOut[i].mPortID.index==NSImageio::NSIspio::EPortIndex_VENC_STREAMO)
            {
            ...
            ports[cnt].SecureTag   = qParam.mvFrameParams[q].mvOut[i].mSecureTag;
            ports[cnt].SecHandle   = 0;
            for(int k=0;k<planeNum;k++)
            {
            ...
            ports[cnt].xoffset = ((qParam.mvFrameParams[q].mvOut[i].mOffsetInBytes % ports[cnt].u4Stride[0])/pixelDivNum)*pixelMulNum;//dma x-offset for tile calculation
            ports[cnt].yoffset = qParam.mvFrameParams[q].mvOut[i].mOffsetInBytes / ports[cnt].u4Stride[0];//dma y-offset for tile calculation
            LOG_DBG("[mvOut (%d)] fmt(0x%x),w/h(%d/%d),oft(%d/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lx,0x%lx,0x%lx)/pa(0x%lx,0x%lx,0x%lx)/size(0x%x,0x%x,0x%x) ),pixelDivNum:%d,pixelMulNum:%d,secPort:%d",\
                ports[cnt].index,ports[cnt].eImgFmt,ports[cnt].u4ImgWidth,ports[cnt].u4ImgHeight,qParam.mvFrameParams[q].mvOut[i].mOffsetInBytes,\
                ports[cnt].xoffset,ports[cnt].yoffset,ports[cnt].u4Stride[0],ports[cnt].u4Stride[1],ports[cnt].u4Stride[2],\
                ports[cnt].capbility,(unsigned long)ports[cnt].u4BufVA[0],(unsigned long)ports[cnt].u4BufVA[1],(unsigned long)ports[cnt].u4BufVA[2],(unsigned long)ports[cnt].u4BufPA[0],(unsigned long)ports[cnt].u4BufPA[1],(unsigned long)ports[cnt].u4BufPA[2],\
                ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2], pixelDivNum, pixelMulNum, ports[cnt].SecureTag);
        ...
        LOG_INF("sDrvScen:%d, CQIdx:%d, dupCQIdx:%d, burstQIdx:%d, RingBufIndex:%d,  mTuningData ptr:%d\n", drvScen, p2CQ, p2dupCQ, q, RingBufIndex,  framePack.rParams.mvFrameParams[q].mTuningData); 
        ...
        pipePackageInfo.FrameNo = qParam.mvFrameParams[q].FrameNo;
        pipePackageInfo.RequestNo = qParam.mvFrameParams[q].RequestNo;
        pipePackageInfo.Timestamp = qParam.mvFrameParams[q].Timestamp;
        pipePackageInfo.UniqueKey = qParam.mvFrameParams[q].UniqueKey;
        pipePackageInfo.ExpectedEndTime.tv_sec = qParam.mvFrameParams[q].ExpectedEndTime.tv_sec;
        pipePackageInfo.ExpectedEndTime.tv_usec = qParam.mvFrameParams[q].ExpectedEndTime.tv_usec;
        pipePackageInfo.SecureFra = qParam.mvFrameParams[q].mSecureFra;
        ...
        if ((q >= 1) && (framePack.frameNum > 1))//framePack.frameNum
        {
                ...
                && (framePack.rParams.mvFrameParams[q].mvOut.size() == framePack.rParams.mvFrameParams[q-1].mvOut.size()))
            {
                if (this->mpPostProcPipe->checkFrameParamIsTheSame(framePack.rParams.mvFrameParams[q],framePack.rParams.mvFrameParams[q-1])==MTRUE)
                {
                    ...
                    LOG_INF("tuning buffer is the same:0x%x",framePack.rParams.mvFrameParams[q].mTuningData);
                }
            }
        }
        //for(int q=0;q<framePack.frameNum;q++)
        ret=this->mpPostProcPipe->configPipe(vPostProcInPorts, vPostProcOutPorts,&pipePackageInfo, framePack.m_UserName);
    ...
    )
{
    ...
    LOG_INF("frameUnit.idxofwaitDQFrmPackList (0x%x_%d_%d_%d, %d_%d_%lu_%d)",frameUnit.callerID, frameUnit.p2BurstQIdx, frameUnit.p2cqIdx, frameUnit.p2cqDupIdx,\
        frameUnit.idxofwaitDQFrmPackList, tempIdx, (unsigned long)this->mpHalPipeWrapper->mLDIPwaitDQFramePackList.size(),shift);
