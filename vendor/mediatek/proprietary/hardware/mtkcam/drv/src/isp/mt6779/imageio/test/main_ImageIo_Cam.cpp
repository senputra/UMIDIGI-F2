...
int TEST_CAMIO::deque(void)
{
    ...
    if(this->bypass_rrzo == MFALSE){
        ...
        for(UINT32 i=0;i<rQBufInfo.vBufInfo.size();i++){
            printf("##############################\n");
            printf("deque_rrzo_%d: pa:%p_%p,mag:0x%x,time:0x%lx,tg size:%dx%d,crop:%d_%d_%d_%d,out:%d_%d\n",i,\
                (MUINT8*)rQBufInfo.vBufInfo.at(i).u4BufPA[ePlane_1st],\
                (MUINT8*)rQBufInfo.vBufInfo.at(i).Frame_Header.u4BufPA,\
                rQBufInfo.vBufInfo.at(i).m_num,\
                rQBufInfo.vBufInfo.at(i).i4TimeStamp_us,\
...
#define AF_WIN_NUM_Y  32
    ...
    this->tuningMgr->updateEngine(eTuningMgrFunc_AF_R1, MTRUE, 0);
    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AF_R1_AF_CON, 0, 0);

    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AF_R1_AF_SIZE, in_h, 0);//case for no frontal binning
    ...
    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AF_R1_AF_VLD, (start_y<<16 | start_x), 0);

    //window num
    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AF_R1_AF_BLK_1, ((AF_WIN_NUM_Y<<16)|AF_WIN_NUM_X), 0);
    ...
    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AF_R1_AF_BLK_0, ((win_v_size<<16)|win_h_size), 0);


    //xsize/ysize
    xsize = AF_WIN_NUM_X*16;
    ysize = AF_WIN_NUM_Y;
    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AFO_R1_AFO_XSIZE, xsize, 0);
    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AFO_R1_AFO_YSIZE, ysize, 0);
    TUNING_MGR_WRITE_REG_CAM(this->tuningMgr, AFO_R1_AFO_STRIDE, xsize, 0);
    // LafiteEP_Todo  remove:eTuningMgrFunc_SGG1
    #if 0
    this->tuningMgr->updateEngine(eTuningMgrFunc_SGG1, MTRUE, 0);
    #endif

    //LafiteEP_TODO: SGG1 removed
    #if 0
