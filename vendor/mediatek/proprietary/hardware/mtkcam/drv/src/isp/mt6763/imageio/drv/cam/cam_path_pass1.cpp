...
MINT32 CamPathPass1::config( struct CamPathPass1Parameter* p_parameter )
{
    ...
    if(p_parameter->bypass_tg == MFALSE){
        ...
        if (p_parameter->m_data_pat == Cam_path_sel::_dual_pix_) {
            this->m_TgCtrl.m_Datapat = CAM_TG_CTRL::_tg_dual_pix_;
        }
        else if (p_parameter->m_data_pat == Cam_path_sel::_quad_code_) {
            this->m_TgCtrl.m_Datapat = CAM_TG_CTRL::_tg_quad_code_;
        }
        else {
    ...
    case Cam_path_sel::_dual_pix_:
        this->m_RawCtrl.m_DataPat = CAM_RAW_PIPE::E_RawDatPat::_raw_dual_pix_;
        break;
    case Cam_path_sel::_quad_code_:
        this->m_RawCtrl.m_DataPat = CAM_RAW_PIPE::E_RawDatPat::_raw_quad_code_;
        break;
    case Cam_path_sel::_normal_:
    default:
        this->m_RawCtrl.m_DataPat = CAM_RAW_PIPE::E_RawDatPat::_raw_normal_;
        break;
    }
...
MINT32 CamPathPass1::_stop( void* pParam )
{
    ...
    ISP_PATH_WRN("_stop:: CAM_BUF_CTRL::m_CamState[0x%x]=0x%x this->m_FSM=0x%x", this->m_hwModule, CAM_BUF_CTRL::m_CamState[this->m_hwModule],this->m_FSM);//julian dbg only
    if (this->m_FSM == op_suspend) {
        ISP_PATH_INF("_stop when suspending, bypass touch hw reg\n");
        goto EXIT;
    }
...
MINT32 CamPathPass1::enqueueBuf( MUINT32 const dmaChannel, ISP_BUF_INFO_L& bufInfo, MVOID* private_data, MBOOL bImdMode )
{
    ...
    switch(dmaChannel){
        ...
        case _eiso_:
            ...
            //this is for dummy buffer on virtual uni
            if(this->m_UniMgr.UniMgr_GetCurAttachModule() != this->m_hwModule ) {
                ISP_BUF_INFO_L::iterator it = bufInfo.begin();
                for(MUINT32 i=0;i<bufInfo.size();i++,it++)
                    it->u_op.enque.image.mem_info.size = 0;
            }
            break;
        case _rsso_:
            if(this->m_UniMgr.UniMgr_GetCurLink() == this->m_hwModule){
    ...
    for(MUINT32 i=0;i<this->m_pCmdQdrv->CmdQMgr_GetBurstQ();i++){
        ...
        if(eCmd_Fail == pFBC->enqueueHwBuf(bufInfo.front(),bImdMode)){
        ...
        //this size check is for dummy buffer on virtual UNI.
