...
#else
#define CAM_FUNC_VRB(fmt, arg...)        do { { printf(fmt "\n", ##arg); } } while(0)
#define CAM_FUNC_DBG(fmt, arg...)        do { { printf("[0x%x]:" fmt "\n",this->m_hwModule, ##arg);  } } while(0)
#define CAM_FUNC_INF(fmt, arg...)        do { { printf("[0x%x]:" fmt "\n",this->m_hwModule, ##arg);  } } while(0)
#define CAM_FUNC_WRN(fmt, arg...)        do { { printf(fmt "\n", ##arg); } } while(0)
#define CAM_FUNC_ERR(fmt, arg...)        do { { printf("[0x%x]:" fmt  "\n",this->m_hwModule, ##arg); } } while(0)
#endif
...
MINT32 CAMSV_TG_CTRL::_config( void )
{
    ...
    CAM_FUNC_INF("xcropping:(0x%x_0x%x_0x%x_0x%x),is_continuous(0x%x),SubSample(0x%x),pixmode(0x%x)\n", \
                this->m_Crop.x, \
                this->m_Crop.y, \
                this->m_Crop.w, \
                this->m_Crop.h,  \
    ...
    //cropping window
    CAM_FUNC_INF("****** %s %d, this->m_Crop.w(%d 0x%08x)", __FUNCTION__, __LINE__, this->m_Crop.w, this->m_Crop.w);
...
MINT32 CAMSV_TOP_CTRL::_config( void )
{
    ...
    while(CAMSV_READ_REG(this->m_pDrv,CAMSV_SW_CTL) != 0x3){ // Polling IMGO_RST_ST to 1
        CAM_FUNC_INF("CAMSV reseting...");
    }
    ...
    //inte en
    CAMSV_WRITE_REG(this->m_pDrv,CAMSV_INT_EN,this->camsv_top_ctl.INTE_EN.Raw);
    ...
    CAM_FUNC_INF("en(0x%08x),fmtSel(0x%08x),intEn(0x%08x),SubSample(0x%x)", \
                ...
                this->camsv_top_ctl.INTE_EN.Raw,  \
                this->SubSample);
...
/// TODO: JSS
    ...
    if(CAMSV_READ_BITS(this->m_pDrv,CAMSV_TG_SEN_MODE,CMOS_EN)){
        ...
        //
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_VF_CON,VFDATA_EN,1);
...
MINT32 CAMSV_TOP_CTRL::_disable( void* pParam )
{
    ...
    if(CAMSV_READ_BITS(this->m_pDrv,CAMSV_TG_VF_CON,VFDATA_EN) == 1){
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_VF_CON,VFDATA_EN,0);
        do{
            ++_cnt;
...
MINT32 DMAO_CAMSV_B::_config( void )
{
    ...
    CAM_FUNC_INF("port id(%d), memBuf pa(%08X),memBuf ofst(%08X), Header addr(%08X)",\
                  this->id(),\
                ...
                switch( this->dma_cfg.bus_size ) {
                    case 0: //  8-bit bus size
                        if(this->dma_cfg.size.stride % 1 != 0) {
                            CAM_FUNC_ERR("IMGO stride(%d) need 1-alignment", this->dma_cfg.size.stride);
                            return -1;
                        }
                    ...
                    case 1: // 16-bit bus size
                        if(this->dma_cfg.size.stride % 2 != 0) {
                            CAM_FUNC_ERR("IMGO stride(%d) need 2-alignment", this->dma_cfg.size.stride);
                            return -1;
                        }
                    ...
                    case 3:// 32-bit bus size
                        if(this->dma_cfg.size.stride % 4 != 0) {
                            CAM_FUNC_ERR("IMGO stride(%d) need 4-alignment", this->dma_cfg.size.stride);
                            return -1;
                        }
                    ...
                    case 7:// 64-bit bus size
                        if(this->dma_cfg.size.stride % 8 != 0) {
                            CAM_FUNC_ERR("IMGO stride(%d) need 8-alignment", this->dma_cfg.size.stride);
                            return -1;
                        }
...
#if DMA_CON
                CAMSV_WRITE_REG(this->m_pDrv,CAMSV_IMGO_CON,0x800000F0);                                                    /* 0x1A05023C */
                CAMSV_WRITE_REG(this->m_pDrv,CAMSV_IMGO_CON2,0x00F0F000);                                                   /* 0x1A050240 */
                CAMSV_WRITE_REG(this->m_pDrv,CAMSV_IMGO_CON3,0x00F0F000);                                                   /* 0x1A050244 */
#endif
    ...
    CAM_FUNC_INF("size(w %d,h %d,stride %d,xsize %d), pixel_byte(%d)", \
                this->dma_cfg.size.w, \
                this->dma_cfg.size.h, \
    ...
    CAM_FUNC_INF("crop(x %d,y %d,w %d,h %d),format(0x%x),fps(0x%x),cropX_inbussize(%d)", \
                this->dma_cfg.crop.x, \
                this->dma_cfg.crop.y,\
                this->dma_cfg.crop.w,\
                this->dma_cfg.crop.h,\
...
{
    CAM_FUNC_INF("DMAO_B::setBaseAddr+[%s]:pa(0x%08X_0x%08X),ofst(0x%X)", \
        this->name_Str(), \
...
void CAMSV_BUF_CTRL::QueueMgr<typ>::init(void)
{
    //BASE_LOG_INF("****** %s %d", __FUNCTION__, __LINE__);
    this->m_v_pQueue.clear();
}

template<typename typ>
void CAMSV_BUF_CTRL::QueueMgr<typ>::uninit(void)
{
    //BASE_LOG_INF("****** %s %d", __FUNCTION__, __LINE__);
    this->m_v_pQueue.clear();
}
...
template<typename typ>
MBOOL CAMSV_BUF_CTRL::QueueMgr<typ>::sendCmd(QueueMgr_cmd cmd,MUINTPTR arg1,MUINTPTR arg2)
{
    MBOOL ret = MTRUE;
    //mark reason at header file
    //Mutex::Autolock lock(this->mQueueLock);
    switch(cmd){
        case eCmd_getsize:
            //BASE_LOG_INF("****** %s %d, cmd % : eCmd_getsize", __FUNCTION__, __LINE__, cmd);
            *(MUINT32*)arg1 = this->m_v_pQueue.size();
            break;
        case eCmd_push:
            //BASE_LOG_INF("****** %s %d, cmd % : eCmd_push", __FUNCTION__, __LINE__, cmd);
            this->m_v_pQueue.push_back(*(typ*)(arg1));
            break;
        case eCmd_pop:
            //BASE_LOG_INF("****** %s %d, cmd % : eCmd_pop", __FUNCTION__, __LINE__, cmd);
            this->m_v_pQueue.pop_front();
            break;
        case eCmd_front:
            //BASE_LOG_INF("****** %s %d, cmd % : eCmd_front", __FUNCTION__, __LINE__, cmd);
            *(typ*)arg1 = this->m_v_pQueue.front();
            break;
        case eCmd_at:
            //BASE_LOG_INF("****** %s %d, cmd % : eCmd_at", __FUNCTION__, __LINE__, cmd);
            ...
            break;
        case eCmd_pop_back:
            //BASE_LOG_INF("****** %s %d, cmd % : eCmd_pop_back", __FUNCTION__, __LINE__, cmd);
            this->m_v_pQueue.pop_back();
            break;
        case eCmd_end:
            //BASE_LOG_INF("****** %s %d, cmd % : eCmd_end", __FUNCTION__, __LINE__, cmd);
        default:
...
MBOOL CAMSV_BUF_CTRL::PipeCheck(void)
{
    ...
    //
    if(err_status & CAMSV_INT_EN_IMGO_ERR_INT_EN){
        CAM_FUNC_ERR("all dmao err status:\n");
        ppc.DMAO_STATUS();
    }

    if( (chkrst == MTRUE) && (err_status==0)){
...
MBOOL CAMSV_BUF_CTRL::waitBufReady( void )
{
    #define _LOOP_  (5)
    MBOOL ret = MTRUE;
    ...
    do{
        if(this->m_buf_cnt != 0){
            ...
            if(FbcCnt < this->m_buf_cnt){
                ...
                if(loopCnt == _LOOP_)
                    sprintf(str,"dma:CAMSV_BUF_CTRL start wait:[enque record:%d_%d],",this->m_buf_cnt, FbcCnt);
                else{
                    char _tmp[16];
            ...
            if(loopCnt == _LOOP_){
                sprintf(str,"dma:CAMSV_BUF_CTRL bWaitBufReady = 1,wait for sw enque[%d],timeout:%d x %d ms,",this->m_buf_cnt,irq.Timeout,loopCnt);
...
MINT32 CAMSV_BUF_CTRL::_disable( void* pParam )
{
    ...
    if(CAMSV_READ_BITS(this->m_pDrv,CAMSV_TG_VF_CON,VFDATA_EN) == 1){
        CAM_FUNC_INF("warning : FBC at streaming");
    }
...
MBOOL CAMSV_BUF_CTRL::enqueueHwBuf( stISP_BUF_INFO& buf_info,MBOOL bImdMode)
{
    ...
    this->m_Queue.sendCmd(CAMSV_BUF_CTRL::QueueMgr<ST_BUF_INFO>::eCmd_getsize,(MUINTPTR)&_size,0);
    for(MUINT32 i=0;i<_size;i++){
        if(MFALSE == this->m_Queue.sendCmd(CAMSV_BUF_CTRL::QueueMgr<ST_BUF_INFO>::eCmd_at,i,(MUINTPTR)&_buf))
    ...
    //
    this->m_Queue.sendCmd(CAMSV_BUF_CTRL::QueueMgr<ST_BUF_INFO>::eCmd_push,(MUINTPTR)ptr,0);
    ...
    switch(this->id()){
        ...
        default:
            CAM_FUNC_ERR("un-supported DMAO:0x%x\n",this->id());
            this->m_Queue.sendCmd(CAMSV_BUF_CTRL::QueueMgr<ST_BUF_INFO>::eCmd_pop  ,0,0);
            return MFALSE;
            break;
    }
    ...
    if(buf_info.bReplace == MFALSE){
        ...
        CAM_FUNC_INF("PA(0x%08x_0x%08x),FH_VA(0x%x),size(0x%x),enque_sof(%d),reg_timestmp(0x%08x)",\
        ...
        CAM_FUNC_INF("PA(0x%08x_0x%08x,new:0x%08x_0x%08x),FH_VA(0x%x,new:0x%x),size(0x%x),enque_sof(%d)",\
            ptr->image.mem_info.pa_addr ,\
            ptr->header.pa_addr ,\
...
//
MBOOL CAMSV_BUF_CTRL::dequeueHwBuf(NSImageio::NSIspio::BufInfo& buf_info )
{
    MBOOL ret = MTRUE;
    MUINT32 _size;
    MUINT32 _sof_idx;
    MUINT32 _reg_time_stamp;
    MUINT32 _header_time_stamp;
    ST_BUF_INFO _buf;
    //MUINT32 _header_data[E_HEADER_MAX];
    char str[128] = {'\0'};
    Mutex::Autolock lock(this->m_bufctrl_lock);
    ...
    //
    this->m_Queue.sendCmd(CAMSV_BUF_CTRL::QueueMgr<ST_BUF_INFO>::eCmd_front, (MUINTPTR)&_buf, 0);
...
EXIT:
    switch(this->id()){
        case _camsv_imgo_:
            CAM_FUNC_INF("PA(0x%08x_0x%08x),FH_VA(0x%08x),size(0x%08x),cur sof(%d)",\
                _buf.image.mem_info.pa_addr ,\
                _buf.header.pa_addr ,\
                _buf.header.va_addr ,\
                _buf.image.mem_info.size ,\
                _sof_idx);
            _header_time_stamp = *((MUINT32*)_buf.header.va_addr);
            CAM_FUNC_INF("reg timestamp = 0x%08x, header timestamp = 0x%08x",\
                CAMSV_READ_REG(this->m_pDrv, CAMSV_TG_TIME_STAMP),\
                _header_time_stamp);
            break;
        default:
            CAM_FUNC_DBG("PA(0x%08x_0x%08x),FH_VA(0x%x),size(0x%x),cur sof(%d)",\
                _buf.image.mem_info.pa_addr ,\
                _buf.header.pa_addr ,\
                _buf.header.va_addr ,\
                _buf.image.mem_info.size ,\
...
MBOOL CAMSV_BUF_CTRL::TimeStamp_mapping(MUINT32* pSec, MUINT32* pUsec)
{
    MBOOL ret = MTRUE;
    ...
    //current time-axis
    time += ( ((MUINT64)(*pSec) + this->m_nOverFlow * 0xFFFFFFFF) * 10 / this->m_TimeClk ); //us
...
MBOOL CAMSV_PIPE_CHECK::DMAO_STATUS(void)
{
    ...
    char* ptr = add;
    if(dma_err[0] & 0xffff){
        sprintf(ptr,"IMGO ERR:0x%x ",dma_err[0]);
        while (*ptr++ != '\0'){}
    }
