...
public:     ////    Callbacks.
    //
public:     ////    Operations.
    virtual MBOOL    execute(struct timeval *endTime = NULL, MUINT32 const u4TimeoutMs = 0xFFFFFFFF);
...
ISImager*
ISImager::
createInstance(IImageBuffer const * pImageBuffer, MINT32 iSensorIdx)
{
    SImager* pSImagerImp = new SImager(pImageBuffer, iSensorIdx);
    if  ( ! pSImagerImp )
    {
...
{
    Mutex::Autolock _lock(mLock);
    return getImp()->execute(endTime, u4TimeoutMs);
}
...
setEncodeParam(
    ...
    MBOOL const isSrcBufNeedFlush
    )
{
    Mutex::Autolock _lock(mLock);
    return getImp()->setEncodeParam(u4IsSOI, u4Quality, eCodecType, isSrcBufNeedFlush);
}
