...
MINT32 CamPathPass1::dropEnqueueBuf(list<MUINT32>* pLPortNum, list<MUINT32>* pLMagicNum) {
    ...
    if (this->m_pCmdQdrv->CmdQMgr_GetBurstQ() != pLMagicNum->size()) {
        ISP_PATH_ERR("data length mismatch:0x%x_0x%x\n",pLMagicNum->size(),this->m_pCmdQdrv->CmdQMgr_GetBurstQ());
        return 1;
    }
    ...
    ISP_PATH_INF("isTwin:%d\n", isTwin);

    if (isTwin == MFALSE) {
        this->m_PBC.enque_drop();
    }

    for(it = listPortNum.begin(); it!= listPortNum.end(); it++){
        switch(*it){
        case _lcso_:
            pFBC = &this->m_Lcso_FBC;
            pDmao = &this->m_Lcso;
...
MBOOL CamPathPass1::updateSuspendDone()
{
    CAM_BUF_CTRL::updateState(CAM_BUF_CTRL::eCamState_suspend, this->m_hwModule, CAM_BUF_CTRL::eCamDmaType_main);
...
MINT32 CamPath_BufCtrl::enque_drop(MVOID) {
    MINT32 ret = 0;
    MUINT32 _size = 0;
    list<MUINT32>::iterator it;
    ...
    for(it=this->m_OpenedChannel.begin();it!=this->m_OpenedChannel.end();it++){
        switch(*it){
            case _imgo_:
                this->m_enque_IMGO.sendCmd(QueueMgr<ISP_BUF_INFO_L>::eCmd_getsize,(MUINTPTR)&_size,0);
                if(_size != 0)
                    this->m_enque_IMGO.sendCmd(QueueMgr<ISP_BUF_INFO_L>::eCmd_pop_back,(MUINTPTR)&_size,0);
                else{
                    ret = 1;
                    ISP_PATH_ERR("IMGO have no buffer for drop\n");
                }
                break;
            case _rrzo_:
                this->m_enque_RRZO.sendCmd(QueueMgr<ISP_BUF_INFO_L>::eCmd_getsize,(MUINTPTR)&_size,0);
                if(_size != 0)
                    this->m_enque_RRZO.sendCmd(QueueMgr<ISP_BUF_INFO_L>::eCmd_pop_back,(MUINTPTR)&_size,0);
                else{
                    ret = 1;
                    ISP_PATH_ERR("RRZO have no buffer for drop\n");
                }
                break;
            default:
                ISP_PATH_ERR("unsupported DMA:%d\n",*it);
