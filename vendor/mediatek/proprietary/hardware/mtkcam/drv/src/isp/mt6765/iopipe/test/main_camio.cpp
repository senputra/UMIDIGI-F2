...
{
    {6400, 3510,  "No limitation"},
    {6252, 3510,  "Bonding at 20.9M"},
    {5496, 3092,  "Bonding at 16.9M"},
    {4988, 2806,  "Bonding at 13.9M"},
};

static char *mThisName = (char*)LOG_TAG;
#if EXCUTABLE_TEST
static Mutex mQryIsExeMutex;
#endif
...
static MVOID* test_camio_thread(void *ts_arg)
{
    ...
    MUINT32             twinCtrl = 1, sensorPixMode = 1, forceBinOff = 0,Dual_PD = 0, cmdsfiled=0;
    ...
    MUINT32             MIPI_PixRate = 0,tempfps = 0;
    MUINT32             SecEnable = 0;
    ...
    if (arg->_argc < 5) { // iopipeTest 1 0 0 2 0, test pattern no crop
        ...
        MY_LOGI("<enablePort>   : 0x01 rrzo, 0x02 imgo, 0x04 eiso, 0x08 lcso, 0x10 rrzo+ufeo bitmap, 0x100, 0x20 rsso");
        ...
        case 4:
            {
                ...
                for(;*ptr!='\0';ptr++){
                    if(*ptr == ',')
                        ...
                        strncat(_str[i],ptr,(sizeof(_str[i]) - strlen(_str[i])) - 1);
                }
    ...
    if(arg->_argc > CMD_IDX_CMDS_FILED) {
        ...
        if(cmdsfiled & EFUSE_TEST) {
            ...
            sensorPixMode = 2;
            MY_LOGD("eFuse test: sce_%d, sdev_%d, pix_%d", \
                scenario, sensorDev,sensorPixMode);
        }
    ...
    if (arg->_argc > CMD_IDX_SUSPEND) {
        ...
        if(suspendTest){
            ...
            if(suspendFlow < (MUINT32)SUSPEND_FLOW_MAX){
                ...
                MY_LOGE("case not support", suspendFlow);
                return NULL;
            }
        }
    }
...
SEARCH_SENSOR:
...
#if EXCUTABLE_TEST
    if (_switchCam == TWO_SENSOR){
        sCAM_QUERY_ISP_RES QryInfo;
        MUINT32 qry_count = 0;

        mQryIsExeMutex.lock();
        QryInfo.QueryInput.sensorIdx = sensorArray[0];
        QryInfo.QueryInput.scenarioId = scenario;
        QryInfo.QueryInput.bin_off = forceBinOff;

        while(1){
            if(getNormalPipeModule()->query(ENPipeQueryCmd_ISP_RES, (MUINTPTR)&QryInfo)){
                MY_LOGD("excutable result(%d)",QryInfo.QueryOutput);
                break;
            }
            else{
                if(qry_count > 50){
    ...
    for (sidx = 0; sidx < sensorNum; sidx++) {
        ...
        //2-UNI including two EISO, RSSO and FLKO
        pUt[sidx]->m_enablePort = enablePort;
        pUt[sidx]->mEnableSttPort = sttPorts;
    ...
    for (sidx = 0; sidx < sensorNum; sidx++) {
        ...
        pUt[sidx]->mbConfig = MTRUE;

#if EXCUTABLE_TEST
        if (_switchCam == TWO_SENSOR){
            mQryIsExeMutex.unlock();
        }
#endif
        ...
        if (pUt[sidx]->m_enablePort & __RSSO_ENABLE) {
            ...
            RssP1Cb *rssDrv = RssP1Cb::createInstance(0, "Main_CamIO_UT");
            rssDrv->registerP1Notify(pUt[sidx]->mpNPipe);
        }
        if (pUt[sidx]->m_enablePort & __LCSO_ENABLE) {
    ...
    for (sidx = 0; sidx < sensorNum; sidx++) {
#if 0 // Move forward to the create part for security cam
        pUt[sidx]->mpImemDrv = IMemDrv::createInstance();
        if (!pUt[sidx]->mpImemDrv) {
            MY_LOGE("IMem create fail");
            return NULL;
        ...
        // for security cam
        if (SecEnable) {
            for (i = 0; i < __MAXDMAO; i++) {
                pUt[sidx]->mImemSecBuf[i].resize(pUt[sidx]->mPortBufDepth);
            }
        }
        ...
        for (j = 0; j < pUt[sidx]->mPortBufDepth; j++) {
            ...
            if (pUt[sidx]->m_enablePort & __RRZO_ENABLE) {
                ...
                imgiSecBuf.size = pUt[sidx]->mTgSize.h * qry.stride_byte;
                ...
                if (SecEnable) {
                    if(pUt[sidx]->mpImemDrv->allocSecBuf(&imgiSecBuf) < 0){
                        MY_LOGE("error:rrzo(%d) secure imem alloc fail", j);
                        return MFALSE;
                    }
                }
                ...
                if (SecEnable) {
                    pUt[sidx]->mImemSecBuf[__RRZO].at(j) = imgiSecBuf;
                }
                MY_LOGD("[SecEnable=%d]rrzo(%d) imem id:%d va:%p", SecEnable, j, imgiBuf.memID, (void*)imgiBuf.virtAddr);
...

                {
                    ...
                    if (SecEnable) {
                        MY_LOGD("rrzo img(%d) secure img pa:0x%x", j, imgiSecBuf.phyAddr);
                    } else {
            ...
            if(pUt[sidx]->m_ufeo_en == 1){
                if (pUt[sidx]->m_enablePort & __RRZO_ENABLE) {
                        ...
                        imgiBuf.size = pUt[sidx]->mTgSize.h * qry.stride_byte;
                            ...
                            IImageBufferAllocator::ImgParam imgParam(__RRZ_FMT(bitmap(getPortBitDepth(PORT_RRZO, pUt[sidx]->m_enablePort))),
                                                                        pUt[sidx]->mTgSize, bufStridesInBytes, bufBoundaryInBytes, 1);
                            sp<ImageBufferHeap> pHeap = ImageBufferHeap::create(mThisName, imgParam,portBufInfo, MTRUE);
            ...
            if (pUt[sidx]->m_enablePort & __IMGO_ENABLE) {
                ...
                if (SecEnable) {
                    if(pUt[sidx]->mpImemDrv->allocSecBuf(&imgiSecBuf) < 0){
                        MY_LOGE("error:imgo(%d) secure imem alloc fail", j);
                        return MFALSE;
                    }
                }
                ...
                if (SecEnable) {
                    pUt[sidx]->mImemSecBuf[__IMGO].at(j) = imgiSecBuf;
                }
                MY_LOGD("[SecEnable=%d]imgo(%d) imem id:%d va:%p", SecEnable, j, imgiBuf.memID, (void*)imgiBuf.virtAddr);
...

                {
                    ...
                    if (SecEnable) {
                        MY_LOGD("imgo img(%d) secure img pa:0x%x", j, imgiSecBuf.phyAddr);
                    } else {
            ...
            if (pUt[sidx]->m_enablePort & __EISO_ENABLE) {
                ...
                if (SecEnable) {
                    if(pUt[sidx]->mpImemDrv->allocSecBuf(&imgiSecBuf) < 0){
                        MY_LOGE("error:eiso(%d) secure imem alloc fail", j);
                        return MFALSE;
                    }
                }
                pUt[sidx]->mImemBuf[__EISO].at(j) = imgiBuf;
                if (SecEnable) {
                    pUt[sidx]->mImemSecBuf[__EISO].at(j) = imgiSecBuf;
                }
                MY_LOGD("[SecEnable=%d]eiso(%d) imem id:%d va:%p", SecEnable, j, imgiBuf.memID, (void*)imgiBuf.virtAddr);
...

                {
                    ...
                    if (SecEnable) {
                        MY_LOGD("eiso img(%d) secure img pa:0x%x", j, imgiSecBuf.phyAddr);
                    } else {
            ...
            if (pUt[sidx]->m_enablePort & __RSSO_ENABLE) {
                ...
                if (SecEnable) {
                    if(pUt[sidx]->mpImemDrv->allocSecBuf(&imgiSecBuf) < 0){
                        MY_LOGE("error:rsso(%d) secure imem alloc fail", j);
                        return MFALSE;
                    }
                }
                ...
                MY_LOGD("[SecEnable=%d]rsso(%d) imem id:%d va:%p", SecEnable, j, imgiBuf.memID, (void*)imgiBuf.virtAddr);
...

                {
                    ...
                    if (SecEnable) {
                        MY_LOGD("rsso img(%d) secure img pa:0x%x", j, imgiSecBuf.phyAddr);
                    } else {
            ...
            if (pUt[sidx]->m_enablePort & __LCSO_ENABLE) {
                ...
                if (SecEnable) {
                    if(pUt[sidx]->mpImemDrv->allocSecBuf(&imgiSecBuf) < 0){
                        MY_LOGE("error:lsco(%d) secure imem alloc fail", j);
                        return MFALSE;
                    }
                }
                ...
                if (SecEnable) {
                    pUt[sidx]->mImemSecBuf[__LCSO].at(j) = imgiSecBuf;
                }
                MY_LOGD("[SecEnable=%d]lcso(%d) imem id:%d va:%p", SecEnable, j, imgiBuf.memID, (void*)imgiBuf.virtAddr);
...

                {
                    ...
                    if (SecEnable) {
                        MY_LOGD("lcso img(%d) secure img pa:0x%x", j, imgiSecBuf.phyAddr);
                    } else {
                ...
                if (pUt[sidx]->m_enablePort & (1 << dma)) {
                    if (SecEnable) {
                        ...
                        mpSecMgr->SecMgr_DumpSecBuffer((MUINT64)pUt[sidx]->mImemSecBuf[dma].at(j).phyAddr,
                                                      (void*)pUt[sidx]->mpImgBuffer[dma].at(j)->getBufVA(0),pUt[sidx]->mImemSecBuf[dma].at(j).size);
        ...
        for (dma = 0; dma < __MAXDMAO; dma++) {
            if (pUt[sidx]->m_enablePort & (1 << dma)) {
                for (j = 0; j < pUt[sidx]->mImemBuf[dma].size(); j++) {
                    ...
                    if (SecEnable) {
                        pUt[sidx]->mpImemDrv->freeSecBuf(&pUt[sidx]->mImemSecBuf[dma].at(j));
                    }
                }
            }
        }
