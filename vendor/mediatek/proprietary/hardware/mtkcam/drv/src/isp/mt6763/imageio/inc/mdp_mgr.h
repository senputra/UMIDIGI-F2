...
* DpObj STATE
********************************************************************************/
typedef enum eDPOBJ_STATE
{
	DPOBJ_STATE_NONE = -1,
    DPOBJ_STATE_AFTERTPIPECOPY = 0,
    DPOBJ_STATE_AFTERSTARTSTREAM = 1,
    DPOBJ_STATE_AFTERFRAMEDONE = 2,
}DPOBJ_STATE;
...
{
    MUINT32 frameflag; //hint, normal/skip tpipe/fm frame mode
    E_ISP_DIP_CQ dpObjcqIdx; //for selecting dpStream
    MUINT32 dpObjdupCqIdx;//for selecting dpStream
    MUINT32 dpObjburstCqIdx;//for selecting dpStream
    E_ISP_DIP_CQ refcqIdx; //reference cq index for current cq index
    MUINT32 refdupCqIdx;//reference dup cq index for current dup cq index
    MUINT32 refburstCqIdx;//reference burst cq index for current burst cq index
    unsigned int *selfTpipeTableVa; //tdri buffer address for referring(mapping to cqIdx/dupCqIdx/burstCqIdx)
    unsigned int *srcTpipeTableVa; //tdri buffer address for referring(mapping to dpObjcqIdx/dpObjdupCqIdx/dpObjburstCqIdx)
    bool needReturnObj; //need return dpObj for original owner(ex, 0/0/0 ref 0/1/0, 0/0/0 should return obj(0/1/0) if 0/1/0 do recaclulation in some case)
    DPOBJ_STATE dpObjState;//state of dpStreamObj
...
class MdpMgr
{
    public :
        ...
        /**
               ...
               *-MTRUE indicates success, otherwise indicates fail
             */
        virtual MBOOL   updateDpStreamMap(MUINT32 flag, MUINT32 cqIdx,MUINT32 dupcqIdx,MUINT32 burstcqIdx,
                    MUINT32 refcqIdx,MUINT32 refdupcqIdx,MUINT32 refburstcqIdx, unsigned int* srcTdriAddr)=0;
