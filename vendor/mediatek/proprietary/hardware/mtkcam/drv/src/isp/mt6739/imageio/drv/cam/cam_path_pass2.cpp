...
//
//
#include "imageio/inc/cam_path.h"
...
int CamPathPass2::config( struct CamPathPass2Parameter* p_parameter )
{
    ...
    //MUINT32 vidoXCropRatio, vidoYCropRatio, vidoCropRatio;	//unused
    //MUINT32 dispoXCropRatio, dispoYCropRatio, dispoCropRatio;	//unused
    MINT32  p2SubMode;
...
//    cdpPipe.conf_cdrz = 0;
    //
    ISP_PATH_DBG("[mdpcrop] imgi in[%lx, %lx] mdpCrop[%lx, %lx, %lx, %lx]_f(0x%x, 0x%x)",(unsigned long)(p_parameter->imgi.size.w),(unsigned long)(p_parameter->imgi.size.h), \
                (unsigned long)(p_parameter->croppara.mdp_crop.x),(unsigned long)(p_parameter->croppara.mdp_crop.y),(unsigned long)(p_parameter->croppara.mdp_crop.w),(unsigned long)(p_parameter->croppara.mdp_crop.h), \
    ...
    ISP_PATH_DBG("[CRZ]en(%d),in[%lx,%lx],out[%lx,%lx],sl2cCrzRatio(%f)",p_parameter->croppara.crz_enable,(unsigned long)(crzPipe.crz_in.w),(unsigned long)(crzPipe.crz_in.h),(unsigned long)(crzPipe.crz_out.w),(unsigned long)(crzPipe.crz_out.h),ispYuvPipe.sl2cCrzRatio);
    ISP_PATH_DBG("[CRZ][CROP](x,y,w,h)=(%lx,%lx,%lx,%lx),floatx,y=(%u,%u)",(unsigned long)(crzPipe.crz_crop.x),(unsigned long)(crzPipe.crz_crop.y),(unsigned long)(crzPipe.crz_crop.w),(unsigned long)(crzPipe.crz_crop.h),crzPipe.crz_crop.floatX,crzPipe.crz_crop.floatY);
    ...
    ISP_PATH_DBG("CDRZ:in[%lx, %lx] crop[%lx %lx %lx %lx]_f(0x%x, 0x%x) out[%lx,%lx] ",(unsigned long)(crzPipe.crz_in.w),(unsigned long)(crzPipe.crz_in.h), \
            (unsigned long)(crzPipe.crz_crop.x),(unsigned long)(crzPipe.crz_crop.y), (unsigned long)(crzPipe.crz_crop.w),(unsigned long)(crzPipe.crz_crop.h), \
            crzPipe.crz_crop.floatX,crzPipe.crz_crop.floatY,(unsigned long)(crzPipe.crz_out.w),(unsigned long)(crzPipe.crz_out.h));
    ISP_PATH_DBG("hCoeffStep(%d) vCoeffStep(%d) ",hCoeffStep,vCoeffStep);


    //imgi
    DMAImgi.dma_cfg = p_parameter->imgi;
    ISP_PATH_DBG("[imgi](%lx , %lx , %lx) vidoRotate(%d),pa(0x%x) ", (unsigned long)(p_parameter->imgi.size.w), (unsigned long)(p_parameter->imgi.size.h), (unsigned long)(p_parameter->imgi.size.stride),crzPipe.vido_out.Rotation,p_parameter->imgi.memBuf.base_pAddr);
    ...
    ISP_PATH_DBG("[LCEI]bypass(%d),size(0x%x),Va(0x%x),Pa(0x%x),(w,h,s)=(%lx,%lx,%lx),crop(x,y,w,h)=(%lx,%lx,%lx,%lx)", \
            DMALcei.bypass, DMALcei.dma_cfg.memBuf.size, DMALcei.dma_cfg.memBuf.base_vAddr, \
            DMALcei.dma_cfg.memBuf.base_pAddr,(unsigned long)(DMALcei.dma_cfg.size.w), (unsigned long)(DMALcei.dma_cfg.size.h), \
            (unsigned long)(DMALcei.dma_cfg.size.stride), (unsigned long)(DMALcei.dma_cfg.crop.x), (unsigned long)(DMALcei.dma_cfg.crop.y), \
            (unsigned long)(DMALcei.dma_cfg.crop.w), (unsigned long)(DMALcei.dma_cfg.crop.h));
    ...
    if ( CAM_CTL_EN_P2_DMA_IMG3O_EN & ispTopCtrl.isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw)
    {
        ...
        ISP_PATH_DBG("[IMG3O]bypass(%d),size(0x%x),Va(0x%x),Pa(0x%x),(w,h,s)=(%lx,%lx,%lx),crop(x,y,w,h)=(%lx,%lx,%lx,%lx)", \
                DMAImg3o.bypass, DMAImg3o.dma_cfg.memBuf.size, DMAImg3o.dma_cfg.memBuf.base_vAddr, \
                DMAImg3o.dma_cfg.memBuf.base_pAddr,(unsigned long)(DMAImg3o.dma_cfg.size.w), (unsigned long)(DMAImg3o.dma_cfg.size.h), \
                (unsigned long)(DMAImg3o.dma_cfg.size.stride), (unsigned long)(DMAImg3o.dma_cfg.crop.x), (unsigned long)(DMAImg3o.dma_cfg.crop.y), \
                (unsigned long)(DMAImg3o.dma_cfg.crop.w), (unsigned long)(DMAImg3o.dma_cfg.crop.h));
    }
    else
    {   DMAImg3o.bypass = 1;    }
    //img3bo
    ...
    if ( CAM_CTL_EN_P2_DMA_IMG2O_EN & ispTopCtrl.isp_top_ctl.CAM_CTL_EN_P2_DMA.Raw){
        ...
        ISP_PATH_DBG("[IMG2O]bypass(%d),size(0x%x),Va(0x%x),Pa(0x%x),(w,h,s)=(%lx,%lx,%lx),crop(x,y,w,h)=(%lx,%lx,%lx,%lx)", \
                DMAImg2o.bypass, DMAImg2o.dma_cfg.memBuf.size, DMAImg2o.dma_cfg.memBuf.base_vAddr, \
                DMAImg2o.dma_cfg.memBuf.base_pAddr,(unsigned long)(DMAImg2o.dma_cfg.size.w), (unsigned long)(DMAImg2o.dma_cfg.size.h), \
                (unsigned long)(DMAImg2o.dma_cfg.size.stride), (unsigned long)(DMAImg2o.dma_cfg.crop.x), (unsigned long)(DMAImg2o.dma_cfg.crop.y), \
                (unsigned long)(DMAImg2o.dma_cfg.crop.w), (unsigned long)(DMAImg2o.dma_cfg.crop.h));
    } else {
        DMAImg2o.bypass = 1;
    }
    ...
    if ( p_parameter->tpipe == CAM_MODE_TPIPE ) {
        ...
        ispMdpPipe.WDMAPQParam = p_parameter->WDMAPQParam;	// set WDMAPQParam
        ispMdpPipe.WROTPQParam = p_parameter->WROTPQParam;	// set WROTPQParam
    } else {
    ...
    //CAM_TDRI_PIPE
    tdriPipe.pTuningIspReg = p_parameter->pTuningIspReg;
...
int CamPathPass2::configTpipeData( struct CamPathPass2Parameter* p_parameter )
{
    ...
    //tdriPipe.tdri.top.c02_en = pP2En->Bits.C02_EN;//remove
   ...
   // tdriPipe.tdri.top.img3bo_en = pP2DMA->Bits.IMG3BO_EN;
    ...
    tdriPipe.tdri.top.ufd_sel = 0;//pP2Sel->Bits.UFD_SEL;
    ...
    tdriPipe.tdri.top.c24_sel = 0;//pP2Sel->Bits.C24_SEL;
    tdriPipe.tdri.top.srz1_sel = 0;//pP2Sel->Bits.SRZ1_SEL;
    tdriPipe.tdri.top.mix1_sel = 0;//pP2Sel->Bits.MIX1_SEL;
    tdriPipe.tdri.top.crz_sel = 0;//pP2Sel->Bits.CRZ_SEL;
    tdriPipe.tdri.top.nr3d_sel = 0;//pP2Sel->Bits.NR3D_SEL;
    tdriPipe.tdri.top.fe_sel = 0;//pP2Sel->Bits.FE_SEL;
    tdriPipe.tdri.top.mdp_sel = 1;//pP2Sel->Bits.MDP_SEL;
    tdriPipe.tdri.top.pca_sel = 0;//pP2Sel->Bits.PCA_SEL;
    ...
    if(p_parameter->isV3 == MTRUE)
    {
        if(p_parameter->isApplyTuning==MTRUE)
        {
            ...
            //nbc
            tdriPipe.tdri.tuningFunc.nbc.anr_eny = p_parameter->pTuningIspReg->CAM_ANR_CON1.Bits.ANR1_ENY;//p_parameter->nbc.anr_eny;
            tdriPipe.tdri.tuningFunc.nbc.anr_enc = p_parameter->pTuningIspReg->CAM_ANR_CON1.Bits.ANR1_ENC;//p_parameter->nbc.anr_enc;

            //tdriPipe.tdri.tuningFunc.nbc.anr_iir_mode = p_parameter->pTuningIspReg->CAM_ANR_CON1.Bits.ANR1_FLT_MODE;//p_parameter->nbc.anr_iir_mode;

            tdriPipe.tdri.tuningFunc.nbc.anr_scale_mode = p_parameter->pTuningIspReg->CAM_ANR_CON1.Bits.ANR1_SCALE_MODE;//p_parameter->nbc.anr_scale_mode;
            //seee
...
//            tdriPipe.tdri.tuningFunc.sl2b.sl2b_hrz_comp = p_parameter->pTuningIspReg->CAM_SL2_HRZ.Bits.SL2_HRZ_COMP; // sl2b hrz comp get from sl2 hrz comp
            tdriPipe.tdri.tuningFunc.nsl2a.nsl2a_hrz_comp = p_parameter->pTuningIspReg->CAM_NSL2A_RZ.Bits.SL2_HRZ_COMP; //NSL2A
            tdriPipe.tdri.tuningFunc.nsl2a.nsl2a_vrz_comp = p_parameter->pTuningIspReg->CAM_NSL2A_RZ.Bits.SL2_VRZ_COMP;
