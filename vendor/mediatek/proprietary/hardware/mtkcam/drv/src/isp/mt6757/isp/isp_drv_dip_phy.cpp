...
MBOOL IspDrvDipPhy::dumpDipHwReg(IspDumpDbgLogDipPackage* pP2Package){
    ...
    LOG_INF("QQ Tunning buffer address : %x, tpipeTablePa: %x, tpipeTableVa: %x, gIsEverDumpBuffer:%d, tdri(0x%x), imgi(0x%x), dmgi(0x%x)",pP2Package->pTuningQue, pP2Package->tpipeTablePa, pP2Package->tpipeTableVa, gIsEverDumpBuffer, get_dumpinfo.tdri_baseaddr, get_dumpinfo.imgi_baseaddr, get_dumpinfo.dmgi_baseaddr);
	...
	{
		LOG_INF("QQ count : %d",size/sizeof(MUINT32));
...
MBOOL IspDrvDipPhy::setMemInfo(unsigned long tpipeTablePa, unsigned int *tpipeTableVa, unsigned int MemSizeDiff)
{
    ...
    if(ioctl(this->m_pIspDrvImp->m_Fd,ISP_SET_MEM_INFO,&TpipeMemInfoStruct) < 0){
        LOG_ERR("set tpipe mem info fail, cmd:0x%x, memPa:0x%x, memVa:0x%x, memSizeDiff:0x%x\n", 
            TpipeMemInfoStruct.MemInfoCmd, TpipeMemInfoStruct.MemPa, TpipeMemInfoStruct.MemVa, TpipeMemInfoStruct.MemSizeDiff);
    }
