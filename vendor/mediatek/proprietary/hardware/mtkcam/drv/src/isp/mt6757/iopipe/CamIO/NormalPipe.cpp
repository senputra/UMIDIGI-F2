...
static const NSImageio::NSIspio::EPortIndex m_Slot2PortIndex[] = {
    EPortIndex_RRZO, EPortIndex_IMGO, EPortIndex_EISO, EPortIndex_LCSO, EPortIndex_UFEO
};
...
MBOOL NormalPipe::stop(MBOOL bNonblocking)
{
    ...
    if(m_bStarted == MFALSE) {
        PIPE_INF("+ already stopped N:%d", mpSensorIdx);
        return ret;
    }
    ...
    if(this->m_bN3DEn == MTRUE){
        ...
        if(pHwSync->sendCommand(HW_SYNC_CMD_DISABLE_SYNC, \
                ...
                m_sensorConfigParam.scenarioId,\
                m_sensorConfigParam.framerate) != 0){
...
MBOOL NormalPipe::checkEnque(QBufInfo const& rQBuf)
{
    ...
    if (!mConfigDone) {
        PIPE_ERR("N:%d Not Rdy, Call configPipe first", mpSensorIdx);
        return MFALSE;
    }
...
MBOOL NormalPipe::deque(QPortID& rQPort, QBufInfo& rQBuf, MUINT32 u4TimeoutMs)
{
    ...
    if (!mConfigDone) {
        PIPE_ERR("Not Rdy, Call configPipe first, SenIdx=%d", mpSensorIdx);
        return MFALSE;
    }
    ...
    for (ii = 0 ; ii < rQPort.mvPortId.size() ; ii++) {
        ...
        if (queueDepth) {
            ...
            rQBuf.mvOut.resize(0);
            PIPE_INF("N:%d dma:x%x stop, depth %d", mpSensorIdx, portID.index, rQBuf.mvOut.size());
        }
...
MBOOL NormalPipe::configPipe(QInitParam const& vInPorts, MINT32 burstQnum)
{
    ...
    if (!mpCamIOPipe) {
        ...
        PortInfo    tgi, imgo, rrzo, ufeo, eiso, lcso, camsv_imgo, camsv2_imgo;
        PortInfo    *pOut = NULL;
        ...
        if(vInPorts.m_bN3D == MTRUE){
            ...
            if(pHwSync->sendCommand(HW_SYNC_CMD_ENABLE_SYNC, \
                pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId,0,0) != 0){
                pHwSync->destroyInstance();
...
{
    if (obj == 0) {
        PIPE_ERR("create instance fail(0x%x)",obj);
        return NULL;
    }
...
MBOOL NormalPipe_Thread::stop(MBOOL bForce)
{
    ...
    if(bForce == MTRUE) {
        ...
        Irq_t irq;
        _NOMRALPIPE_MAP_SIGNAL(EPipeSignal_SOF, irq.Status, irq.StatusType);
...
MINT32 NormalPipe_Thread::idleStateHold(MUINT32 hold)
{
    if (hold) {
        PIPE_INF("Holding idle locks");
...
MVOID* NormalPipe_Thread::ispEnqueThread(void *arg)
{
    ...
    while (1) {
        ...
        _this->mThreadIdleLock_Enq.lock();
        if (MFALSE == ret) {
            PIPE_ERR("ERROR: wait SOF timeout");
            continue;
        }
