...
MINT32 CAM_TG_CTRL::_config( void )
{
    E_ISP_CAM_CQ cq;
    MUINT32 page;
    MUINT32 SwTimestamp = 0;//1:sw timestamp, 0:hw timestamp
    ...
    if (page == 0) { /*reduce log, only show page 0*/
        CAM_FUNC_INF("CAM_TG_CTRL::_config +:xcropping:(0x%x_0x%x_0x%x_0x%x),is_continuous(0x%x),SubSample(0x%x),pixmode(0x%x),data-pattern(0x%x),%s_TS\n", \
            this->m_Crop.x, \
            this->m_Crop.y, \
            this->m_Crop.w, \
            this->m_Crop.h,  \
    ...
    //timestamp
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_TG_SEN_MODE,TIME_STP_EN,(!SwTimestamp));
    this->m_pDrv->setDeviceInfo(_SET_TS_MODE,(MUINT8*)&SwTimestamp);
...
MINT32 CAM_TOP_CTRL::_config( void )
{
    ...
    //disable TOP DCM for HW timestamp
    CAM_FUNC_INF("disable top dcm");
...
MINT32 CAM_TOP_CTRL::_enable( void* pParam  )
{
    ...
    if(pUni){
        ...
        CAM_FUNC_INF("uni is linked with cur module\n");
    }
...
STOP_ISP:
    ...
    //FBC reset
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_IMGO_CTL1,FBC_RESET,1);
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_RRZO_CTL1,FBC_RESET,1);
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_UFEO_CTL1,FBC_RESET,1);
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_LCSO_CTL1,FBC_RESET,1);

    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_IMGO_CTL1,FBC_RESET,0);
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_RRZO_CTL1,FBC_RESET,0);
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_UFEO_CTL1,FBC_RESET,0);
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_LCSO_CTL1,FBC_RESET,0);
...
MBOOL CAM_TOP_CTRL::resume(void)
{
    ...
    CAM_FUNC_INF("CAM_TOP_CTRL::resume SAVE_FBC:IMGO_0x%08x RRZO_0x%08x UFEO_0x%08x AFO_0x%08x AAO_0x%08x LCSO_0x%08x PDO_0x%08x PSO_0x%08x",
    ...
    if(CAM_READ_BITS(this->m_pDrv->getPhyObj(),CAM_FBC_PSO_CTL1,FBC_EN)){
        ...
        CAM_WRITE_REG(this->m_pDrv->getPhyObj(),CAM_FBC_PSO_CTL2,this->fbc_PSO.Raw);
    }
    ...
    CAM_FUNC_INF("CAM_TOP_CTRL::resume PHYS_FBC:IMGO_0x%08x RRZO_0x%08x UFEO_0x%08x AFO_0x%08x AAO_0x%08x LCSO_0x%08x PDO_0x%08x PSO_0x%08x",
...
MINT32 UNI_TOP_CTRL::SwitchOut(void)
{
    ...
    UNI_WRITE_BITS(this->m_pDrv, CAM_UNI_TOP_MOD_EN, GSE_A_EN, 0);
...
MINT32 UNI_TOP_CTRL::SwitchIn(MBOOL enableEIS, MBOOL enableRSS)
{
    ...
    if (MFALSE == this->m_pDrv->setDeviceInfo(_RESET_LMV_FBC_CNT, NULL)) {
        CAM_FUNC_ERR("LMV FBC cnt reset error!\n");
    }
...
MVOID UNI_TOP_CTRL::ClearFLKSettingOnCQ(void)
{
    ...
    UNI_WRITE_BITS(this->m_pDrv, CAM_UNI_TOP_MOD_EN, QBIN3_A_EN, 0);
...
MVOID UNI_TOP_CTRL::LMVDisable(void)
{
    ...
    switch(this->UNI_Source){
        ...
        case IF_CAM_B:
            ...
            UNI_WRITE_BITS(this->m_pUniDrv, CAM_UNI_TOP_MOD_EN, GSE_A_EN, 0);
            UNI_WRITE_BITS(this->m_pUniDrv, CAM_UNI_TOP_MOD_EN, SGG2_A_EN, 0);
...
MINT32 CAM_RAW_PIPE::_config( void )
{
    ...
    CAM_WRITE_BITS(this->m_pDrv,CAM_CTL_DMA_EN,QBIN4_EN, 1);
    ...
    //qbin4
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_QBN4_MODE, QBN_ACC_MODE,1);//always use average
    CAM_WRITE_BITS(this->m_pDrv->getPhyObj(),CAM_QBN4_MODE, QBN_ACC,qbn_pixmode);
...
MINT32 CAM_TWIN_PIPE::_disable(void* pParam)
{
    ...
    //FBC reset
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_IMGO_CTL1,FBC_RESET,1);
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_RRZO_CTL1,FBC_RESET,1);
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_UFEO_CTL1,FBC_RESET,1);
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_LCSO_CTL1,FBC_RESET,1);

    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_IMGO_CTL1,FBC_RESET,0);
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_RRZO_CTL1,FBC_RESET,0);
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_UFEO_CTL1,FBC_RESET,0);
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_LCSO_CTL1,FBC_RESET,0);
...
MINT32 CAM_TUNING_CTRL::update(MBOOL UpdateFLK)
{
    ...
    if(vpTuningNodes[page]->eUpdateFuncBit & eIspTuningMgrFunc_OBC){
        ...
        //
        Header_AAO  fh_aao;
        fh_aao.Header_Enque(Header_AAO::E_OB,this->m_pDrv,pReg->CAM_OBC_GAIN0.Raw);
...
BYPASS_PBN:
    ...
    if(vpTuningNodes[page]->eUpdateFuncBit & eIspTuningMgrFunc_LSC){
        ...
        //
        Header_AAO  fh_aao;
        fh_aao.Header_Enque(Header_AAO::E_LSC,this->m_pDrv,pReg->CAM_LSC_RATIO_0.Raw);
    ...
    if(UpdateFLK) {
        ...
        if(vpTuningNodes[page]->ctlEn_UNI & FLK_A_EN_){
            ...
            if(1 == UNI_READ_BITS(this->m_pUniDrv,CAM_UNI_FBC_FLKO_A_CTL1,FBC_EN)){
                    CAM_FUNC_DBG("FLK supported no dynamic func off under continuous mode\n");
                goto BYPASS_FLK;
            }
        }
