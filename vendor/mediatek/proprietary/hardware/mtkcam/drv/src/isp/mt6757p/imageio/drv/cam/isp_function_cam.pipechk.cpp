...
MBOOL PIPE_CHECK::FLK_CHECK(void)
{
    ...
    flk2_sel = UNI_READ_BITS(this->m_pDrv->getPhyObj(), CAM_UNI_TOP_PATH_SEL, FLK2_A_SEL);
...
MBOOL PIPE_CHECK::EIS_CHECK(void)
{
    ...
    hds2_sel = UNI_READ_BITS(this->m_pDrv->getPhyObj(), CAM_UNI_TOP_PATH_SEL, HDS2_A_SEL);
...
MBOOL PIPE_CHECK::AF_CHECK(void)
{
    ...
    if(CAM_READ_BITS(this->m_pDrv->getPhyObj(),CAM_CTL_EN,SGG1_EN) == 0){
        CAM_FUNC_ERR("Need to enable SGG1 that AF is needed\n");
        rst = MFALSE;
        goto EXIT;
    }
