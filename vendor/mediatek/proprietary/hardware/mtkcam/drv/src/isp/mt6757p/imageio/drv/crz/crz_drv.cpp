      ...
      m_fgVertFirst(MFALSE)
{
    ...
    LOG_INF("");
    //
    memset(&crz_setting, 0, sizeof(crz_setting));
    memset(&SysramAlloc, 0, sizeof(SysramAlloc));
}
...
MBOOL CrzDrvImp::Reset(void)
{
	...
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CTL_YUV_EN, CRZ_EN,MFALSE);
    return MTRUE;
}
...
MBOOL CrzDrvImp::ResetDefault(void)
{
	...
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_IN_IMG, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_OUT_IMG, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_HORI_STEP, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_VERT_STEP, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_HORI_INT_OFST, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_HORI_SUB_OFST, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_VERT_INT_OFST, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_VERT_SUB_OFST, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_HORI_INT_OFST, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_HORI_SUB_OFST, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_VERT_INT_OFST, 0x00000000);
	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_VERT_SUB_OFST, 0x00000000);
//	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_DER_1, 0x00000000);
//	DIP_WRITE_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_DER_2, 0x00000000);
    //
    return MTRUE;
}
...
MBOOL CrzDrvImp::DumpReg(void)
{
    ...
    LOG_DBG("CONTROL                            = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL));
    LOG_DBG("INPUT_IMAGE                        = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_IN_IMG));
    LOG_DBG("OUTPUT_IMAGE                       = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_OUT_IMG));
    LOG_DBG("HORIZONTAL_COEFF_STEP              = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_HORI_STEP));
    LOG_DBG("VERTICAL_COEFF_STEP                = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_VERT_STEP));
    LOG_DBG("LUMA_HORIZONTAL_INTEGER_OFFSET     = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_HORI_INT_OFST));
    LOG_DBG("LUMA_HORIZONTAL_SUBPIXEL_OFFSET    = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_HORI_SUB_OFST));
    LOG_DBG("LUMA_VERTICAL_INTEGER_OFFSET       = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_VERT_INT_OFST));
    LOG_DBG("LUMA_VERTICAL_SUBPIXEL_OFFSET      = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_VERT_SUB_OFST));
    LOG_DBG("CHROMA_HORIZONTAL_INTEGER_OFFSET   = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_HORI_INT_OFST));
    LOG_DBG("CHROMA_HORIZONTAL_SUBPIXEL_OFFSET  = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_HORI_SUB_OFST));
    LOG_DBG("CHROMA_VERTICAL_INTEGER_OFFSET     = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_VERT_INT_OFST));
    LOG_DBG("CHROMA_VERTICAL_SUBPIXEL_OFFSET    = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_VERT_SUB_OFST));
//    LOG_DBG("DERING_1                           = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_DER_1));
//    LOG_DBG("DERING_2                           = 0x%08X",DIP_READ_REG(DIP_A, m_pIspDrv,DIP_X_CRZ_DER_2));
    //
    return MTRUE;
}
    ...
    CRZ_DRV_IMG_CROP_STRUCT     Crop)
{
    ...
    LOG_INF("In(%d, %d). Crop(%d, %d). Out(%d, %d). CropStart(%f, %f).",SizeIn.Width, SizeIn.Height, Crop.Width.Size, Crop.Height.Size, SizeOut.Width, SizeOut.Height, Crop.Width.Start, Crop.Height.Start);
	...
	//HW Setting
	//Width
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_HORI_EN, MTRUE);
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_HORI_ALGO, Algo_H);
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_TRNC_HORI, CRZ_DRV_DEFAULT_TRUNC_BIT);	
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_HORI_TBL_SEL, Table_H);	
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_IN_IMG, CRZ_IN_WD, SizeIn.Width);
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_OUT_IMG, CRZ_OUT_WD, SizeOut.Width);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_HORI_STEP, CRZ_HORI_STEP, CoeffStep_H);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_HORI_INT_OFST,CRZ_LUMA_HORI_INT_OFST, LumaInt_H);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_HORI_SUB_OFST,CRZ_LUMA_HORI_SUB_OFST, LumaSub_H);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_HORI_INT_OFST,CRZ_CHRO_HORI_INT_OFST, ChromaInt_H);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_HORI_SUB_OFST,CRZ_CHRO_HORI_SUB_OFST, ChromaSub_H);
	
    //Height
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_VERT_EN, MTRUE);	
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_VERT_FIRST, m_fgVertFirst);	
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_VERT_ALGO, Algo_V);
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_TRNC_VERT, CRZ_DRV_DEFAULT_TRUNC_BIT);
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CONTROL, CRZ_VERT_TBL_SEL, Table_V);
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_IN_IMG, CRZ_IN_HT,SizeIn.Height);	
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_OUT_IMG, CRZ_OUT_HT, SizeOut.Height);
    DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_VERT_STEP, CRZ_VERT_STEP, CoeffStep_V);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_VERT_INT_OFST,CRZ_LUMA_VERT_INT_OFST, LumaInt_V);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_LUMA_VERT_SUB_OFST,CRZ_LUMA_VERT_SUB_OFST, LumaSub_V);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_VERT_INT_OFST,CRZ_CHRO_VERT_INT_OFST, ChromaInt_V);
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CRZ_CHRO_VERT_SUB_OFST,CRZ_CHRO_VERT_SUB_OFST, ChromaSub_V);
	
	
	DIP_WRITE_BITS(DIP_A, m_pIspDrv,DIP_X_CTL_YUV_EN, CRZ_EN,MTRUE);
    //
    return MTRUE;
}
