    ...
    if (pPipePackageInfo->pTuningQue != NULL)
    {
        pTuningDipReg = (dip_x_reg_t *)pPipePackageInfo->pTuningQue;
                PIPE_INF("[Imgi][crop_1](x,f_x,y,f_y)=(%d,%d,%d,%d),(w,f_w,h,f_h)=(%d,%d,%d,%d),(tarW,tarH)=(%d,%d)-\n\
                    ...
                    isDipOnly(%d), hwmodule(%d/%d), dipCQ/dup/burst(%d/%d/%d), drvSc(%d), isWtf(%d),tdr_tpipe(%d),en_yuv/yuv2/rgb/dma/fmt_sel/ctl_sel/misc (0x%x/0x%x/0x%x/0x%x/0x%x/0x%x/0x%x), tcm(0x%x/0x%x/0x%x), last(%d), CRZ_EN(%d), img3o ofset(%d,%d),userName(%s),PIX_ID(%d),ImgBuf_PIX_ID(%d)",\
                    portInfo_imgi.crop1.x,portInfo_imgi.crop1.floatX,portInfo_imgi.crop1.y,portInfo_imgi.crop1.floatY, \
                    portInfo_imgi.crop1.w,portInfo_imgi.crop1.floatW,portInfo_imgi.crop1.h,portInfo_imgi.crop1.floatH, \
                    portInfo_imgi.resize1.tar_w,portInfo_imgi.resize1.tar_h, \
                    portInfo_imgi.crop2.x,portInfo_imgi.crop2.floatX,portInfo_imgi.crop2.y,portInfo_imgi.crop2.floatY, \
...
queryCQ(EDrvScenario sDrvScen, MUINT32 sensorDev, MINT32 p2burstQIdx, MINT32 frameNum, MUINT32& RingBufIndex, MUINTPTR& pVirIspAddr)
{
    ...
    {
        PIPE_INF("Block Enqueue Here, P2CQ (%d), eDrvScen/SenDev(0x%x/0x%x)", p2CQ, sDrvScen, sensorDev);
    }
