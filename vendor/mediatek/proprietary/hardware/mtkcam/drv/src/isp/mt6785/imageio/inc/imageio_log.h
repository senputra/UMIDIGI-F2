...
#define MTK_LOG_ENABLE 1
    ...
    #define BASE_LOG_VRB(fmt, arg...)       CAM_LOGV(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)	// <Verbose>: Show more detail debug information. E.g. Entry/exit of private function; contain of local variable in function or code block; return value of system function/API...
    #define BASE_LOG_DBG(fmt, arg...)       CAM_LOGD(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)	// <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
    #define BASE_LOG_INF(fmt, arg...)       CAM_LOGD(DBG_LOG_TAG "[%s] " fmt NEW_LINE_CHAR, __func__, ##arg)	// <Info>: Show general system information. Like OS version, start/end of Service...
    ...
    #define ISP_TRACE_BEGIN(name) ATRACE_BEGIN(name)
    #define ISP_TRACE_END() ATRACE_END()
    #else
...
#undef ISP_PATH_ERR
#undef ISP_PATH_AST
#define ISP_PATH_VRB(fmt, arg...)        do { if (path_DbgLogEnable_VERBOSE) { BASE_LOG_VRB("[0x%x]:" fmt,this->m_hwModule, ##arg); } } while(0)
#define ISP_PATH_DBG(fmt, arg...)        do { if (path_DbgLogEnable_DEBUG  ) { BASE_LOG_DBG("[0x%x]:" fmt,this->m_hwModule, ##arg); } } while(0)
#define ISP_PATH_INF(fmt, arg...)        do { if (path_DbgLogEnable_INFO   ) { BASE_LOG_INF("[0x%x]:" fmt,this->m_hwModule, ##arg); } } while(0)
#define ISP_PATH_WRN(fmt, arg...)        do { if (path_DbgLogEnable_WARN   ) { BASE_LOG_WRN("[0x%x]:" fmt,this->m_hwModule, ##arg); } } while(0)
#define ISP_PATH_ERR(fmt, arg...)        do { if (path_DbgLogEnable_ERROR  ) { BASE_LOG_ERR("[0x%x]:" fmt,this->m_hwModule, ##arg); } } while(0)
#define ISP_PATH_AST(cond, fmt, arg...)  do { if (path_DbgLogEnable_ASSERT ) { BASE_LOG_AST(cond, "[0x%x]:" fmt,this->m_hwModule, ##arg); } } while(0)
