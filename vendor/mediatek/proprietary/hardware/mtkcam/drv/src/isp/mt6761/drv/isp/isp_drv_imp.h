...
class IspDrvImp : public IspDrv
{
    ...
    public:
        ...
        virtual MBOOL   set_pmqos(E_PMQOS_CMD eCmd,MUINT8* pData);
        virtual MBOOL   mmdvfs_ctrl(E_MMDVFS_CMD eCmd,MUINT8* pData, MUINT8* pIncClk);
        ...
        virtual MBOOL   enqueP2Frame(MUINT32 callerID,MINT32 p2burstQIdx,MINT32 p2dupCQIdx,MINT32 frameNum, MINT32 p2Scenario);
        virtual MBOOL   waitP2Deque();
        virtual MBOOL   dequeP2FrameSuccess(MUINT32 callerID,MINT32 p2dupCQIdx, MINT32 p2Scenario);
        virtual MBOOL   dequeP2FrameFail(MUINT32 callerID,MINT32 p2dupCQIdx, MINT32 p2Scenario);
        virtual MBOOL   waitP2Frame(MUINT32 callerID,MINT32 p2dupCQIdx,MINT32 p2Scenario,MINT32 timeoutUs);
...
class IspDrvVirImp : public IspDrv
{
    ...
    virtual MBOOL checkCQBufAllocated(void){/*LOG_ERR("Not support");*/return MTRUE;}
        ...
        virtual MBOOL   deTuningQue(ETuningQueAccessPath path, ESoftwareScenario softScenario, MINT32 magicNum){UNUSED(path);UNUSED(softScenario);UNUSED(magicNum);return MFALSE;};
        virtual MBOOL   enTuningQue(ETuningQueAccessPath path, ESoftwareScenario softScenario, MINT32 magicNum, EIspTuningMgrFunc updateFuncBit){
            UNUSED(path);UNUSED(softScenario);UNUSED(magicNum);UNUSED(updateFuncBit);return MFALSE;
        };
        virtual MBOOL   deTuningQueByCq(ETuningQueAccessPath path, ISP_DRV_CQ_ENUM eCq, MINT32 magicNum){UNUSED(path);UNUSED(eCq);UNUSED(magicNum);return MFALSE;};
        virtual MBOOL   enTuningQueByCq(ETuningQueAccessPath path, ISP_DRV_CQ_ENUM eCq, MINT32 magicNum, EIspTuningMgrFunc updateFuncBit){
        ...
        virtual MBOOL set_pmqos(E_PMQOS_CMD eCmd,MUINT8* pData){UNUSED(eCmd);UNUSED(pData);return MTRUE;};
        virtual MBOOL mmdvfs_ctrl(E_MMDVFS_CMD eCmd,MUINT8* pData, MUINT8* pIncClk){UNUSED(eCmd);UNUSED(pData);UNUSED(pIncClk);return MTRUE;};
    ...
    private:
        virtual MBOOL   searchTuningQue(ISP_DRV_P2_CQ_ENUM p2Cq, MINT32 magicNum, MINT32 &getQueIdx){UNUSED(p2Cq);UNUSED(magicNum);UNUSED(getQueIdx);return MFALSE;};
        virtual MBOOL   orderSearchTuningQue(ISP_DRV_P2_CQ_ENUM p2Cq, MINT32 magicNum, MUINT32 start, MUINT32 end, MINT32 &getQueIdx){
