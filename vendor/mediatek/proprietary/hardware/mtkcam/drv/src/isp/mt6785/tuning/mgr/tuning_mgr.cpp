...
{
    if(sensorIndex >= NTUNING_MAX_SENSOR_CNT || sensorIndex < 0)
    {
...
)
{
    ...
    switch(engine)
    {
        ...
        case eTuningMgrFunc_BNR:
            uTuningBit = (engine == eTuningMgrFunc_LSC) ? eIspTuningMgrFuncBit_LSC : eIspTuningMgrFuncBit_BNR;
...
)
{
    ...
    switch(engine)
    {
        ...
        case eTuningMgrFunc_FLK:
            ...
            TUNING_MGR_WRITE_REG_UNI(this, CAM_UNI_TOP_MOD_EN, this->mCtlEn_UNI, BQIdx);
            this->mCtlEnDMA_UNI = (ctlEn == MTRUE) ? (this->mCtlEnDMA_UNI | FLKO_EN_) : (this->mCtlEnDMA_UNI & ~FLKO_EN_);
...
)
{
    ...
    {
        LOG_ERR("Size of this->mvTuningNodes.size(%d) is not identical to mBQNum(%d)!!!", this->mvTuningNodes.size(), this->mBQNum);
        ret = MFALSE;
        goto EXIT;
    }
