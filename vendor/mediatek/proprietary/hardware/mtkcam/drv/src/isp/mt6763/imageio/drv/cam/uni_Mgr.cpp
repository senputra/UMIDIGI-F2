...
public:
            ISP_HW_MODULE   mConModule; /* To record cur cam is attached or not, maybe virtual */
private:
    ...
    static ISP_HW_MODULE        mConnectModule; /* To record phy. uni is actually attached to which cam */
...
MINT32 UniMgrImp::UniMgr_attach(UNI_HW_MODULE module,DupCmdQMgr* pConnectCam,IspDMACfg* pRawIn)
{
    ...
    if(module == UNI_A)
        this->pUniDrv->init("UNI_A_CTRL", (IspDrvImp *) this->pCamDrv->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj());
    else if (module == UNI_B)
        this->pUniDrv->init("UNI_B_CTRL", (IspDrvImp *) this->pCamDrv->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj());
    ...
    if(this->mConnectModule == this->mConModule) {
        for(MUINT32 i=0;i<pConnectCam->CmdQMgr_GetDuqQ();i++){
            for(MUINT32 j=0;j<pConnectCam->CmdQMgr_GetBurstQ();j++){
                ...
                this->m_EISO_FBC.extArg = (MUINTPTR)CAM_BUF_CTRL::eExtArg_PhyUni;
                this->m_RSSO_FBC.extArg = (MUINTPTR)CAM_BUF_CTRL::eExtArg_PhyUni;

                this->m_EISO_FBC.enable(NULL);
                this->m_RSSO_FBC.enable(NULL);

                //
                this->m_EISO_FBC.m_pTimeStamp = CAM_TIMESTAMP::getInstance(this->mConModule,NULL);
                this->m_RSSO_FBC.m_pTimeStamp = CAM_TIMESTAMP::getInstance(this->mConModule,NULL);
            }
        }
        ...
        //
        this->m_FLKO_FBC.m_pTimeStamp = CAM_TIMESTAMP::getInstance(this->mConModule,NULL);
        ...
        for(MUINT32 i=0;i<pConnectCam->CmdQMgr_GetDuqQ();i++){
            for(MUINT32 j=0;j<pConnectCam->CmdQMgr_GetBurstQ();j++){
                ...
                this->m_EISO_FBC.extArg = (MUINTPTR)CAM_BUF_CTRL::eExtArg_VirUni;
                this->m_RSSO_FBC.extArg = (MUINTPTR)CAM_BUF_CTRL::eExtArg_VirUni;
...
MINT32 UniMgrImp::UniMgr_recover(E_UNI_STEP op,ISP_HW_MODULE module)
{
    switch(op){
        case eCmd_Uni_stop:
            ...
            if(module != this->mConnectModule){
                UniMgr_ERR("fail, unpaired connect cam, input:0x%x, cur:0x%x\n",module,this->mConnectModule);
                return 1;
            }

            if(this->m_UniCtrl.HW_recover(0) == MFALSE)
                return 1;
            else
                return 0;
            break;
        case eCmd_Uni_restart:
            if(this->FSM_CHK(op_stop) == MFALSE){
                return 1;
            }
            ...
            if(this->m_UniCtrl.HW_recover(1) == MFALSE)
                return 1;
            else
                return 0;
            break;
        default:
            UniMgr_ERR("unsuppoed op:%d\n",op);
            return 1;
            break;
    }
