...
MBOOL PIPE_CHECK::AA_CHECK(void)
{
    ...
    MUINT32 aa_pit_size_h,aa_pit_size_v;
    ...
    aa_pit_size_h = (aa_isize_h)/ aa_win_h;
    aa_pit_size_v = (aa_isize_v)/ aa_win_v;
    ...
    if(aa_win_size_h < 4){
        ...
        //h_pitch check
        if (pit_size < 4) {
            CAM_FUNC_ERR("pit: size:0x%x[h-min is 4 ]!!\n", pit_size);
            rst = MFALSE;
        }
        else if (pit_size > aa_pit_size_h) {
            CAM_FUNC_ERR("pit: size need <= ideal size, [0x%x_0x%x]\n",
                pit_size, aa_pit_size_h);
            rst = MFALSE;
        }
        //h_window check
        if(win_size & 0x1) {
            //won't crash
            CAM_FUNC_ERR("win: size:0x%x[h-min is 2-alignment]!!, otherwise overexpcnt will be 0\n",
                win_size);
            rst = MFALSE;
        }
        else if(win_size < 4) {
            CAM_FUNC_ERR("win: size:0x%x[h-min is 4 ]!!\n", win_size);
            rst = MFALSE;
        }
        else if(win_size > aa_win_size_h) {
            CAM_FUNC_ERR("win: size need <= ideal size, [0x%x_0x%x]\n",
                win_size, aa_win_size_h);
            rst = MFALSE;
        }
    ...
    if(aa_win_size_v < 2){
        ...
        //v_pitch check
        if(pit_size < 2) {
            CAM_FUNC_ERR("vpit: size:0x%x[v-min is 2 ]!!\n", pit_size);
            rst = MFALSE;
        }
        else if(pit_size > aa_pit_size_v) {
            CAM_FUNC_ERR("vpit size need <= ideal size, [0x%x_0x%x]\n",
                pit_size, aa_pit_size_v);
            rst = MFALSE;
        }
        //v_window check
        if(win_size & 0x1) {
            //won't crash
            CAM_FUNC_ERR("vwin: size:0x%x[v-min is  2-alignment]!!,otherwise overexpcnt will be 0\n",
                win_size);
            rst = MFALSE;
        }
        else if(win_size < 2) {
                CAM_FUNC_ERR("vwin: size:0x%x[v-min is 2 ]!!\n", win_size);
                rst = MFALSE;
        }
        else if(win_size > aa_win_size_v) {
                CAM_FUNC_ERR("vwin size need <= ideal size, [0x%x_0x%x]\n",
                    win_size, aa_win_size_v);
            rst = MFALSE;
        }
        if(pit_size < win_size) {
            CAM_FUNC_ERR("win size need <= pit size, [0x%x_0x%x]\n",pit_size,win_size);
    ...
    //ae hist
    {
        ...
        aa_xsize += AE_BIN_SIZE(this->m_pDrv->getPhyObj());
...
MBOOL PIPE_CHECK::PS_CHECK(void)
{
    ...
    MUINT32 ps_pit_size_h, ps_pit_size_v;
    ...
    //aa win/pit size, win size <= pit size
    ps_pit_size_h = (aa_isize_h)/ win_num.Bits.AWB_W_HNUM;
    ps_pit_size_v = (aa_isize_v)/ win_num.Bits.AWB_W_VNUM;
    ...
    if(aa_win_size_h < 4){
        ...
        //h pitch check
        if(pit_size < 4){
            CAM_FUNC_ERR("pso h_pit: size:0x%x[h-min is 4]!!\n",
                pit_size);
            rst = MFALSE;
        }
        else if(pit_size > ps_pit_size_h){
            CAM_FUNC_ERR("pso h_pit size need <= ideal size, [0x%x_0x%x]\n",
                pit_size, ps_pit_size_h);
            rst = MFALSE;
        }

        //h window check
        if(win_size & 0x1) {
            CAM_FUNC_ERR("pso h_win: size:0x%x[h-min is 2-alignment]!!, otherwise overexpcnt will be 0\n", win_size);
            rst = MFALSE;
        }
        else if(win_size < 4){
            CAM_FUNC_ERR("pso h_win: size:0x%x[h-min is 4]!!\n",
                win_size);
            rst = MFALSE;
        }
        else if(win_size > aa_win_size_h){
            CAM_FUNC_ERR("pso h_win size need <= ideal size, [0x%x_0x%x]\n",
                win_size, aa_win_size_h);
            rst = MFALSE;
        }
    ...
    if(aa_win_size_v < V_align){
        ...
        //v pitch check
        if(pit_size < 2) {
            CAM_FUNC_ERR("pso v_pit: size:0x%x[v-min is 2 ]!!\n", pit_size);
            rst = MFALSE;
        }
        else if(pit_size > ps_pit_size_v) {
            CAM_FUNC_ERR("pso v_pit size need <= ideal size, [0x%x_0x%x]\n",
                pit_size, ps_pit_size_v);
            rst = MFALSE;
        }

        //v window check
        if(win_size & 0x1) {
            CAM_FUNC_ERR("pso v_win: size:0x%x[v-min is  2-alignment]!!,otherwise overexpcnt will be 0\n",
                win_size);
            rst = MFALSE;
        }
        else if(win_size < 2) {
                CAM_FUNC_ERR("pso v_win: size:0x%x[v-min is 2 ]!!\n", win_size);
                rst = MFALSE;
        }
        else if(win_size > aa_win_size_v) {
                CAM_FUNC_ERR("pso v_win size need <= ideal size, [0x%x_0x%x]\n",
                    win_size, aa_win_size_v);
            rst = MFALSE;
        }
    ...
    aa_xsize += PS_BIN_SIZE(this->m_pDrv->getPhyObj());
    aa_xsize = aa_xsize/8;
