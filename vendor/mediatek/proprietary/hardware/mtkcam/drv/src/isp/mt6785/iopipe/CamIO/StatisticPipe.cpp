...
inline MBOOL StatisticPipe::FSM_CHECK(E_FSM op, const char *callee)
{
    ...
    case op_enque:
        ...
        //    ret = 2;
        //    break;
        /* change design: to avoid over enque case,
           3A enque and resuming enque the same buffer.
           we should measure no enque/deque disturb from MW
 ...
 *
 ******************************************************************************/
MBOOL StatisticPipe::enqueInternal(QBufInfo const& rQBuf)
{
    ...
    for (i = 0; i < _bsize; i++) {
        PIPE_INF("SttE:%d: dma:x%x pa(x%" PRIXPTR "/x%" PRIXPTR ") fh_pa(x%" PRIXPTR ")", mpSensorIdx, portID.index,\
    ...
    for (idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++) {
        ...
        for (buf = 0; buf < mpMemInfo[_port].size(); buf++) {
            if (pbuf->mPa == mpMemInfo[_port].at(buf).phyAddr) {
                ...
                default:
                    mpIMem->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID, &(mpMemInfo[_port].at(buf)), MFALSE);
                    break;
                }
                break;
            }
...
MBOOL StatisticPipe::suspend()
{
    ...
    for(PortIdx = 0; PortIdx < PortNum; PortIdx++) {
        ...
        //change design: ignore DeqContainer usage
        //force deueqe mBufMax to clean buffers in IspDrv
        //for(BufIdx = 0; BufIdx < BufNum; BufIdx++) {
        for(BufIdx = 0; BufIdx < mBufMax; BufIdx++) {
            ...
            switch(buf_status){
                case eBuf_suspending:
                    ...
                    break;
                default:
                    PIPE_ERR("SttD:%d: dequeOutBuf status error(%d)", mpSensorIdx, buf_status);
                    break;
            }
        }
...
MBOOL StatisticPipe::resume()
{
    ...
    for (PortIdx = 0; PortIdx < PortNum; PortIdx++) {
        ...
        /* TBD: following should be removed -- start */
        //DeqContainer is no needed, coz current design: resuming force to use the
        //init addr in CQMgr and buffer sequence should be aligned as initial
        ...
        /* TBD: -- end */
        //TBD: Should consider Burst mode?
        for (i = 0 ; i < mpMemInfo[_PortIdx].size(); i++ ) {
            rQBuf[i].mvOut.resize(mBurstQNum);
            for(BurstQIdx = 0; BurstQIdx < mBurstQNum; BurstQIdx++) {
                rQBuf[i].mvOut.at(BurstQIdx).mPortID.index = portID.index;
                rQBuf[i].mvOut.at(BurstQIdx).mVa = mpMemInfo[_PortIdx].at(i).virtAddr;
                rQBuf[i].mvOut.at(BurstQIdx).mPa = mpMemInfo[_PortIdx].at(i).phyAddr;
                rQBuf[i].mvOut.at(BurstQIdx).mSize = mpMemInfo[_PortIdx].at(i).size;
                rQBuf[i].mvOut.at(BurstQIdx).mMemID = mpMemInfo[_PortIdx].at(i).memID;
            }
            if (MFALSE == enqueInternal(rQBuf[i])) {
                PIPE_ERR("TG_%d port_%d init enq buf fail", mTgInfo, rQBuf[i].mvOut.at(0).mPortID.index);
                return MFALSE;
            }
