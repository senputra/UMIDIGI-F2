...
inline MUINT32 NPIPE_MAP_PORTID(MUINT32 const PortIdx)
{
    ...
    if (PortIdx == PORT_IMGO.index)
       ...
       _idx = ePort_svimgo;
    else if (PortIdx == PORT_YUVO.index)
       _idx = ePort_yuvo;
    else if (PortIdx == PORT_CRZO_R1.index)
       _idx = ePort_crzo_r1;
    else if (PortIdx == PORT_CRZO_R2.index)
       _idx = ePort_crzo_r2;
   else if (PortIdx == PORT_RAWI.index)
       _idx = ePort_rawi;
    else
...
inline MBOOL NPIPE_CHECK_IS_VIRTUAL(MUINT32 _portIdx)
{
    MBOOL ret = MFALSE;
    switch(_portIdx) {
        case ePort_eiso:
...
//switch to INormalPipe cam index
inline ENPipe_CAM NPIPE_MAP_CAM_PIPE(MUINT32 const camIdx)
{
    ENPipe_CAM CamPipe = ENPipe_CAM_MAX;

    switch(camIdx) {
        case CAM_A:
            CamPipe = ENPipe_CAM_A;
            break;
        case CAM_B:
            CamPipe = ENPipe_CAM_B;
            break;
        case CAM_C:
            CamPipe = ENPipe_CAM_C;
            break;
        default:
            PIPE_ERR("Error camPipe idx mapping");
            break;
    }
    return CamPipe;
}
...
inline NPIPE_PATH_E NPIPE_MAP_PATH(MUINT32 const sensorIdx)
{
    ...
    switch(_NORMALPIPE_GET_TG_IDX(sensorIdx)){
        ...
        case CAM_TG_2: cam_path = NPIPE_CAM_B;
            break;
        case CAM_TG_3: cam_path = NPIPE_CAM_C;
            break;
        default:
...
inline NPIPE_PATH_E NPIPE_MAP_PATH_BY_TG(MUINT32 const CAM_TG)
{
    NPIPE_PATH_E cam_path = NPIPE_CAM_MAX;
    ...
    switch(CAM_TG){
        ...
        case CAM_TG_3: cam_path = NPIPE_CAM_C;
            break;
        default:
            PIPE_ERR("Unknown tg (%d)", (MUINT32)CAM_TG);
            break;
    }
    return cam_path;
}
...
inline MBOOL TG_MAP_Port(E_INPUT const tg_input,enum EPortIndex* pOut_TG,enum EPortIndex* pOut_RAWI)
{
    switch(tg_input){
        case TG_A:
            if(pOut_TG)
                *pOut_TG = EPortIndex_TG1I;
            if(pOut_RAWI)
                *pOut_RAWI = EPortIndex_RAWI;
            break;
        case TG_B:
            if(pOut_TG)
                *pOut_TG = EPortIndex_TG2I;
            if(pOut_RAWI)
                *pOut_RAWI = EPortIndex_UNKNOW;
            break;
        case TG_C:
            if(pOut_TG)
                *pOut_TG = EPortIndex_TG3I;
            if(pOut_RAWI)
                *pOut_RAWI = EPortIndex_UNKNOW;
            break;
        default:
...
inline MBOOL CHECK_FMT_YUV_2_PLANE(MINT Fmt)
{
    MBOOL ret = MTRUE;
...
inline MBOOL CHECK_FMT_YUV_3_PLANE(MINT Fmt)
{
    MBOOL ret = MTRUE;
...
{
    MBOOL ret = MFALSE;
    PIPE_DBG("PortIdx = %d CfgFmt = 0x%x EnqFmt = 0x%x", PortIdx, CfgFmt, EnqFmt);
    if (CfgFmt == EnqFmt) {
        ret = MTRUE;
    ...
    NSImageio::NSIspio::BufInfo &dst, MBOOL mEnableSec)
{
    ...
    if(CHECK_FMT_UF(ImgFormat)) {
        ...
        /*pass 1st and 2nd plane*/
        PlaneMax = (MUINT32)ePlane_2nd;
    }
    else if(CHECK_FMT_YUV_3_PLANE(ImgFormat)) {
        /*pass 1st and 2nd plane*/
    ...
    if (mEnableSec) {
        //Secure camera does not support multi-plane
        dst.u4BufSize[ePlane_1st] = (MUINT32)src.mBuffer->getBufSizeInBytes(ePlane_1st);
        dst.u4BufVA[ePlane_1st]   = (MUINTPTR)src.mBuffer->getBufVA(ePlane_1st);
        dst.memID[ePlane_1st]     = (MUINTPTR)src.mBuffer->getFD(ePlane_1st);
    }
    else {
    ...
    PortInfo* pOutPort)
{
    ...
    switch(pOutPort->index){
        ...
        case EPortIndex_RAWI:
            ...
            //camsv_imgo output size due to twin mode is partitioned by seninf
            pOutPort->u4ImgWidth = InPort.mDstSize.w;
            pOutPort->u4ImgHeight = InPort.mDstSize.h;
            if(pOutPort->index == EPortIndex_RAWI)
                pOutPort->inout = EPortDirection_In;
            else
    ...
    pOutPort->crop1.w        = pOutPort->u4ImgWidth;
    pOutPort->crop1.h        = pOutPort->u4ImgHeight;
    ...
    switch(cmd) {
        ...
        case ENPipeQueryCmd_STRIDE_BYTE:
            if (pinfo->GetCapibility(pOut.index, cmd, qryInput, queryRst)) {
                ...
                if ((CHECK_FMT_UF(qryInput.format)) || (CHECK_FMT_YUV_3_PLANE(qryInput.format)))
                    PlaneMax = (MUINT32)ePlane_3rd;
                else if(CHECK_FMT_YUV_2_PLANE(qryInput.format))
                    PlaneMax = (MUINT32)ePlane_2nd;
                else
                    PlaneMax = (MUINT32)ePlane_1st;
        ...
        case ENPipeQueryCmd_NEED_CFG_SENSOR_BY_MW:
            if(pinfo->GetCapibility(0, cmd, qryInput, queryRst)){
                if(queryRst.function.Bits.needCfgSensorByMW == 0){
                    PIPE_INF("needCfgSensorByMW is %d\n", queryRst.function.Bits.needCfgSensorByMW);
                    ret = MFALSE;
                }
            }
            else{
...
inline MVOID CAM_MAP(E_CAMIO_CAM const input, ENPipe_CAM &output)
{
    ...
    if(input & EPipe_CAM_C)
        output = (ENPipe_CAM)((MUINT32)output | ENPipe_CAM_C);
}

inline MVOID CAM_MAP_NPIPE(NPIPE_PATH_E const input, ENPipe_CAM &output)
{
    MBOOL ret = MTRUE;
    output = ENPipe_UNKNOWN;
    switch(input) {
...
inline MUINT32 NSVPIPE_MAP_TGI(MUINT32 tgIdx)
{
    ...
    case CAM_SV_6:
        ...
        break;
    // CAMSV support CAMSV_7, CAMSV_8, but only for pdaf purpose
    // will not involve in DCIF mode
    default:
...
inline MBOOL CHECK_SWITCH_LEGAL(E_CamIQLevel lv, NPIPE_PATH_E path)
{
    return  ((NPIPE_CAM_C == path) && (eCamIQ_H == lv)) ? MFALSE : MTRUE;
}
