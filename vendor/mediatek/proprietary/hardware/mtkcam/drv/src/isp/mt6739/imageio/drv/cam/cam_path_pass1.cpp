...
int CamPathPass1::config( struct CamPathPass1Parameter* p_parameter )
{
    ...
    ISP_PATH_INF("p_parameter rrz_in_roi_size(%d/%d/%d/%d/%lu/%lu)",\
    ...
    ISP_PATH_INF("p_parameter rrzD_in_roi_size(%d/%d/%d/%d/%lu/%lu)",\
                                        p_parameter->rrz_in_roi.x, \
    ...
    //
    if((p_parameter->path != ISP_PASS1_CAMSV) && (p_parameter->pathD != ISP_PASS1_CAMSV_D)){
        DMACQ.bypass = 0;
    ...
    DMACamsvImgo.CQ_D = p_parameter->CQ_D;

    //camsv2
    DMACamsv2Imgo.CQ = p_parameter->CQ;
    DMACamsv2Imgo.CQ_D = p_parameter->CQ_D;
...
int CamPathPass1::_waitIrq( int type, unsigned int irq , int userNumber)
{
    ...
    WaitIrq.UserInfo.Type = (ISP_DRV_IRQ_TYPE_ENUM)type;
    WaitIrq.UserInfo.Status = irq;
...
MUINT32 CamPathPass1::setP1MagicNum(MUINT32 const magicNum)
{
    ...
    if ((this->pass1_path != ISP_PASS1) && (this->pass1_path != ISP_PASS1_D) &&
        (this->pass1_path != ISP_PASS1_CAMSV) && (this->pass1_path != ISP_PASS1_CAMSV_D)) {
...
int CamPathPass1::setP1RrzCfg(IspRrzCfg const rrz_cfg)
{
    ...
    if ((ISP_PASS1 != rrz_cfg.pass1_path)&& (ISP_PASS1_D != rrz_cfg.pass1_path)) {
        ISP_PATH_ERR("Invalid Path to use RRZ (%lu)", rrz_cfg.pass1_path);
        return 0;
    }
