    ...
    for(iter = pPipePackageInfo->pExtraParam->begin();iter<pPipePackageInfo->pExtraParam->end();iter++)
    {
        ...
        switch (CmdInfo.CmdIdx)
        {
            ...
            case NSCam::NSIoPipe::EPIPE_VSDOFPQ_CMD:
                m_camPass2Param.pVSDOFPQParam = (MVOID *)CmdInfo.moduleStruct;
                break;
            case NSCam::NSIoPipe::EPIPE_DC_CMD:
                m_camPass2Param.pDCParam = (MVOID *)CmdInfo.moduleStruct;
                break;
            case NSCam::NSIoPipe::EPIPE_CZ_CMD:
                m_camPass2Param.pCZParam = (MVOID *)CmdInfo.moduleStruct;
                break;
#endif
            default:
...
configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, PipePackageInfo *pPipePackageInfo)
{
    ...
    MUINT32 dip_rgb_tuning_tag = DIP_X_REG_CTL_RGB_EN_PGN|DIP_X_REG_CTL_RGB_EN_SL2|DIP_X_REG_CTL_RGB_EN_LCE\
		...
		|DIP_X_REG_CTL_RGB_EN_G2G|DIP_X_REG_CTL_RGB_EN_GGM|DIP_X_REG_CTL_RGB_EN_RNR|DIP_X_REG_CTL_RGB_EN_SL2G|DIP_X_REG_CTL_RGB_EN_SL2H|DIP_X_REG_CTL_RGB_EN_FLC|DIP_X_REG_CTL_RGB_EN_FLC2;//DIP_X_REG_CTL_RGB_EN_UDM;
                ...
                { // check vip2i
                    ...
                    if (pPipePackageInfo->pTuningQue != NULL)
                    {
                        ...
                        if (pTuningDipReg->DIP_X_CTL_YUV_EN.Bits.NR3D_EN & pTuningDipReg->DIP_X_CAM_TNR_ENG_CON.Bits.NR3D_CAM_TNR_EN)
                        {
                            ...
                            PIPE_INF("[vipi]ofst(%d),rW/H(%d/%d)",pTuningDipReg->DIP_X_NR3D_VIPI_OFFSET.Raw,pTuningDipReg->DIP_X_NR3D_VIPI_SIZE.Bits.NR3D_VIPI_WIDTH,pTuningDipReg->DIP_X_NR3D_VIPI_SIZE.Bits.NR3D_VIPI_HEIGHT);
                        }
                    }
        ...
        switch(pPipePackageInfo->vModuleParams[i].eDipModule)
        {
            ...
            case EDipModule_SRZ5:
                if(reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)) {
                    ...
                    m_camPass2Param.srz5_cfg.inout_size.in_w=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->in_w;
                    m_camPass2Param.srz5_cfg.inout_size.in_h=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->in_h;
                    m_camPass2Param.srz5_cfg.inout_size.out_w=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->out_w;
                    m_camPass2Param.srz5_cfg.inout_size.out_h=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->out_h;
                    m_camPass2Param.srz5_cfg.crop.x=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->crop_x;
                    m_camPass2Param.srz5_cfg.crop.floatX=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->crop_floatX;
                    m_camPass2Param.srz5_cfg.crop.y=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->crop_y;
                    m_camPass2Param.srz5_cfg.crop.floatY=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->crop_floatY;
                    m_camPass2Param.srz5_cfg.crop.w=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->crop_w;
                    m_camPass2Param.srz5_cfg.crop.h=reinterpret_cast<_SRZ_SIZE_INFO_*>(pPipePackageInfo->vModuleParams[i].moduleStruct)->crop_h;

                    PIPE_INF("SRZ5:in(%d,%d),out(%d,%d)",m_camPass2Param.srz5_cfg.inout_size.in_w,m_camPass2Param.srz5_cfg.inout_size.in_h
                    ,m_camPass2Param.srz5_cfg.inout_size.out_w,m_camPass2Param.srz5_cfg.inout_size.out_h);
                }
                break;
            default:
	...
	*/
	  imgi_fmt = DIP_IMGI_FMT_MFB_MIX_MODE;
            ...
            //dip_ctl_yuv_en |= (DIP_X_REG_CTL_YUV_EN_MFBW|DIP_X_REG_CTL_YUV_EN_MIX3);	//remove mfbw since algo don't use MFBW anymore.
	...
	*/
    ...
    if(pPipePackageInfo->pTuningQue != NULL){
        ...
        rgb_en = m_camPass2Param.pTuningIspReg->DIP_X_CTL_RGB_EN.Raw;
            ...
            tdr_tcm2_en |= (rgb_en & (DIP_X_REG_CTL_RGB_EN_FLC|DIP_X_REG_CTL_RGB_EN_FLC2)) << 7; //FLC, FLC2
