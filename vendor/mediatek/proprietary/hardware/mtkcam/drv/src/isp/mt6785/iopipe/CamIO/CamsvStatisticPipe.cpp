...
ICamsvStatisticPipe* ICamsvStatisticPipe::createInstance(MUINT32 SensorIndex, char const* szCallerName, MUINT32 FeatureIdx)
{
    ...
    if (NULL == pPipe) {
        ...
        pPipe = CamsvStatisticPipe::pCamsvStatisticPipe[SensorIndex][FeatureIdx];
        if(NULL == pPipe->mpCamsvStatisticPipeThread){
            pPipe->mpCamsvStatisticPipeThread = CamsvStatisticPipe_Thread::createInstance(pPipe);
            if (NULL == pPipe->mpCamsvStatisticPipeThread){
...
EXIT:
    ...
    if (mTotalUserCnt == 0)  {
        ...
        if (mpCamsvStatisticPipeThread) {
            mpCamsvStatisticPipeThread->destroyInstance();
            mpCamsvStatisticPipeThread = NULL;
        }
...
MBOOL CamsvStatisticPipe::start()
{
    ...
    if (MFALSE == mInitSettingApplied) {
        PIPE_ERR("no enque before start");
        return MFALSE;
    }
    ...
    if (mpCamsvStatisticPipeThread) {
        if (mpCamsvStatisticPipeThread->start() == MFALSE) {
            PIPE_ERR("error: Thread start fail\n");
            return MFALSE;
        }
    }
...
MBOOL CamsvStatisticPipe::stop(MBOOL bNonblocking)
{
    ...
    if (mpCamsvIOPipe) {
        if (mpCamsvStatisticPipeThread == NULL) {
            PIPE_ERR("mpCamsvStatisticPipeThread is null");
        } else {
            /* hold idle lock */
            mpCamsvStatisticPipeThread->idleStateHold(1, CamsvStatisticPipe_Thread::eThdGrp_Enq);
        }
        ...
        if (mpCamsvStatisticPipeThread) {
            mpCamsvStatisticPipeThread->prestop();

            /* release idle lock */
            mpCamsvStatisticPipeThread->idleStateHold(0, CamsvStatisticPipe_Thread::eThdGrp_Enq);
        }
    }

    if (mpCamsvStatisticPipeThread) {
        if (mpCamsvStatisticPipeThread->stop(MTRUE) == MFALSE) {
            PIPE_ERR("error: Thread stop fail\n");
            return MFALSE;
        }
    }
...
MBOOL CamsvStatisticPipe::suspend(void)
{
    ...
    this->mpCamsvStatisticPipeThread->idleStateHold(1, CamsvStatisticPipe_Thread::eThdGrp_Enq);

    this->ring_cnt = 0;
    //also clear all buf record
    ret = mpCamsvIOPipe->suspend(NSImageio::NSIspio::ICamIOPipe::SW_SUSPEND);

    this->signal(EPipeSignal_SOF, this->mpCamsvStatisticPipeThread->m_irqUserKey); // flush SOF signal, since irqUserKey is here

    if(ret == MTRUE)
        this->FSM_UPDATE(CamsvStatisticPipe::op_suspend);

    this->mInitSettingApplied = MFALSE;
...
MBOOL CamsvStatisticPipe::resume(void)
{
    ...
    this->mpCamsvStatisticPipeThread->idleStateHold(0, CamsvStatisticPipe_Thread::eThdGrp_Enq);
...
MBOOL CamsvStatisticPipe::init()
{
    ...
    do {
        ...
        if (mpCamsvStatisticPipeThread) {
            if (mpCamsvStatisticPipeThread->init() == MFALSE) {
                PIPE_ERR("error: Thread init fail\n");
                return MFALSE;
            }
        }

        if (mpEnqueContainer == NULL) {
            SPIPE_MEM_NEW(mpEnqueContainer, QueueMgr<QBufInfo>(), sizeof(QueueMgr<QBufInfo>));
            mpEnqueContainer->init();
        }
    } while(0);
...
MBOOL CamsvStatisticPipe::uninit()
{
    ...
    if (mpCamsvStatisticPipeThread) {
        if (mpCamsvStatisticPipeThread->uninit() == MFALSE) {
            PIPE_ERR("error: Thread uninit fail\n");
            return MFALSE;
        }
    }

    if (mpEnqueContainer) {
        mpEnqueContainer->uninit();
        SPIPE_MEM_DEL(mpEnqueContainer, sizeof(QueueMgr<QBufInfo>));
    }
...
MBOOL CamsvStatisticPipe::enque(QBufInfo const& rQBuf)
{
    ...
    this->mEnqueContainerLock.lock();
    this->mpEnqueContainer->sendCmd(QueueMgr<QBufInfo>::eCmd_push, (MUINTPTR)&rQBuf, 0);
    this->mEnqueContainerLock.unlock();

    this->mpCamsvStatisticPipeThread->updateFrameTime((MUINT32)(rQBuf.mShutterTimeNs/1000000), 1);

    if (this->m_FSM == op_cfg || this->m_FSM == op_suspend) {
        if (this->mInitSettingApplied == MFALSE) {
            PIPE_INF("N:%d immeidateEnque", this->mpSensorIdx);
            this->mpCamsvStatisticPipeThread->enque();
...
MBOOL CamsvStatisticPipe::immediateEnque(QBufInfo const& rQBuf)
{
    ...
    for (MUINT32 i = 0 ; i < CAMSV_BUF_DEPTH; i ++) {
...
MBOOL CamsvStatisticPipe::sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    switch (cmd) {
        ...
        case ECAMSVSPipeCmd_GET_CAMSV_GET_ERROR_CODE:
            ...
            break;
        case ECAMSVSPipeCmd_GET_CAMSV_GET_CUR_SOF_IDX:
            ret = mpNormalPipe->sendCommand(ENPipeCmd_GET_CUR_SOF_IDX, arg1, arg2, arg3);
            if (MFALSE == ret) {
                goto _EXIT_SEND_CMD;
            }
            break;
