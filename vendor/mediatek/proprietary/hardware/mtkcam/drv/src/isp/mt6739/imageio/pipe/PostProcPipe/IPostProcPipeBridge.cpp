...
destroyInstance()
{
    ...
    if  ( ! this )
    {
        PIPE_ERR("[IPostProcPipe] IPostProcPipeBridge is NULL");
    }
    else{
      if  ( ! mpPipeImp )
      {
          PIPE_ERR("[IPostProcPipe] PostProcPipe is NULL");
      }
      else{
...
*
********************************************************************************/
MBOOL IPostProcPipeBridge::endequeFrameCtrl(EPIPE_BUFQUECmd cmd,unsigned long callerID,MINT32 p2burstQIdx,MINT32 p2dupCQIdx,MINT32 frameNum, MINT32 p2Scenario, MINT32 timeout)
{
    Mutex::Autolock _lock(mLock);
