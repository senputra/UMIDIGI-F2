...
#define __SV_DCIF_IMGO_ENABLE  (0x80)
#define __SV_DCIF_UFEO_ENABLE  (0x100)
#define __YUVO_ENABLE          (0x200)
#define __CRZO_ENABLE          (0x400)
#define __CRZO_R2_ENABLE       (0x800)
#define __RAWI_ENABLE          (0x1000)
#define __UFDI_ENABLE          (0x2000)
...
#define __AAO_ENABLE         (1<<__AAO)
#define __AFO_ENABLE         (1<<__AFO)
#define __FLKO_ENABLE        (1<<__FLKO)
#define __PDO_ENABLE         (1<<__PDO)
#define __TSFSO_ENABLE       (1<<__TSFSO)
#define __SV_STT_IMGO_ENABLE (1<<__MAXDMAO_STT) /* sams as enum eDMAO */
...
enum {
    ...
    __UFDI,
    __UFDI_META,
    __GARBAGE,  //due to multiplane is fixed at 3-plane,add this index to avoid memory corruption when crzo/crzo_r2
    __MEM_MAX,
};
...
struct _arg
{
    ...
    MUINT32  bDCIF;
    MUINT32  bDynSwitch;
    void     *mIHalSensorList = NULL;
    MSize    custCropSize;
    E_INPUT  targetTG;
    E_CamIQLevel iqLevel;
    sCAM_QUERY_FUNC qryFunc;
    ...
    , bDCIF(MFALSE)
    , bDynSwitch(MFALSE)
    , mIHalSensorList(NULL)
    , custCropSize(MSize(0, 0))
    , targetTG(TG_CAM_MAX)
    , iqLevel(eCamIQ_MAX)
    {
        qryFunc.QueryOutput = MFALSE;
    }
};
...
class TuningNotifyImp_CQS : public P1_TUNING_NOTIFY {
public:
    TuningNotifyImp_CQS(NPipeUT *_pNPipeUtObj) : mpNPipeUtObj(_pNPipeUtObj) {}
    ~TuningNotifyImp_CQS() {}
    virtual const char* TuningName() { return "UT_CQStart"; }
    virtual void p1TuningNotify(MVOID* pIn, MVOID* pOut);
...
class TuningNotifyImp_CRZCB : public P1_TUNING_NOTIFY {
public:
    TuningNotifyImp_CRZCB(NPipeUT *_pNPipeUtObj) : mpNPipeUtObj(_pNPipeUtObj) {}
    TuningNotifyImp_CRZCB() {}
    virtual const char* TuningName() { return "UT_CRZCB"; }
    virtual void p1TuningNotify(MVOID* pIn, MVOID* pOut);
    NPipeUT     *mpNPipeUtObj;
};

class TuningNotifyImp_CRZR2CB : public P1_TUNING_NOTIFY {
public:
    TuningNotifyImp_CRZR2CB(NPipeUT *_pNPipeUtObj) : mpNPipeUtObj(_pNPipeUtObj) {}
    TuningNotifyImp_CRZR2CB() {}
    virtual const char* TuningName() { return "UT_CRZR2CB"; }
    virtual void p1TuningNotify(MVOID* pIn, MVOID* pOut);
    NPipeUT     *mpNPipeUtObj;
};

class TuningNotifyImp_3ACB : public P1_TUNING_NOTIFY {
public:
    TuningNotifyImp_3ACB(NPipeUT *_pNPipeUtObj) : mpNPipeUtObj(_pNPipeUtObj) {}
    TuningNotifyImp_3ACB() {}
    virtual const char* TuningName() { return "UT_3ACB"; }
    virtual void p1TuningNotify(MVOID* pIn, MVOID* pOut);
    NPipeUT     *mpNPipeUtObj;
};
...
public:
    ...
    ICamsvStatisticPipe*     mpCamsvSttPipe[VC_MAX_NUM];
    TuningMgr*               tuningMgr;
    IMemDrv*                 mpImemDrv;
    vector<IImageBuffer*>    mpImgBuffer[__MEM_MAX];
    vector<IMEM_BUF_INFO>    mImemBuf[__MEM_MAX];
    vector<IMEM_BUF_INFO>    mImemSecBuf[__MAXDMAO];
    vector<IMEM_BUF_INFO>    mImemSecChkBuf[__MAXDMAO];
    vector<sp<IImageBufferHeap>> mpHeap[__MEM_MAX];
    MUINT8                   mpSttBuf[__MAXDMAO_STT][512*1024];
    MUINT32                  mpSttBufSize[__MAXDMAO_STT];
    MUINTPTR                 mCamsvBufVa[VC_MAX_NUM];
    MUINT32                  mCamsvBufSize[VC_MAX_NUM];
    vector<MUINT32>          m_vCamsvFeatureIdx;
    MSize                    mTgSize;
    MSize                    mCrzMaxSize;
    MSize                    mCrzR2MaxSize;
    ...
    TuningNotifyImp_CQS      mCQStartNotify;
    ...
    MUINT32                  mFrameNum;
    MUINT32                  mBurstQNum;
    MUINT32                  mBurstTuningQNum;
    MUINT32                  mPortBufDepth;
    MUINT32                  mDCIFBurstQNum;
    MUINT32                  mDCIFPortBufDepth;
    MUINT32                  mMagicNum;
    MBOOL                    m_bEfuse;
    MINT32                   mEfuseIdx;
    MBOOL                    mEnableTgInt;
    ...
    TuningNotifyImp_CRZR2CB  mCRZR2CBNotify;
    TuningNotifyImp_CRZCB    mCRZCBNotify;
    TuningNotifyImp_3ACB     m3ACBNotify;
    ...
    MSize                    mCurTgOut;
    Mutex                    mCBMutex;
    MINT32                   mSttSofUserKey[__MAXDMAO_STT];
    union{
        struct{
                MUINT32 bDual   :   31;
                MUINT32 Density :   1;
