...
CAM_TIMESTAMP* CAM_TIMESTAMP::getInstance(ISP_HW_MODULE module,const ISP_DRV_CAM* pDrv,const IspDrvCamsv* pSVDrv)
{
    ...
    // only a_twin_b for twin mode case in isp6.0
    if(module == CAM_A_TWIN_B)
        module = CAM_A;
    ...
    if (module < PHY_CAM) {
        ...
        CAM_WRITE_BITS(gTimeStamp[module].m_pDrv->getPhyObj(),TG_R1_TG_SEN_MODE,TG_TIME_STP_EN,1);
        CAM_WRITE_REG(gTimeStamp[module].m_pDrv->getPhyObj(),TG_R1_TG_TIME_STAMP_CNT,0x19); // src clk:26M output clk:26M/n+1=1M => n=25
        ...
        CAMSV_WRITE_REG(gTimeStamp[module].m_pCamSVDrv,CAMSV_TG_TIME_STAMP_CNT,0x19); // src clk:26M output clk:26M/n+1=1M => n=25
    }
...
MBOOL CAM_TIMESTAMP::TimeStamp_SrcClk(MUINT32 clk, TSTMP_MODE mode)
{
    ...
    if (this->m_stmpMode == LOCAL_MODE) {
        if (this->m_hwModule < PHY_CAM) {
            CAM_WRITE_BITS(this->m_pDrv->getPhyObj(), TG_R1_TG_TIME_STAMP_CTL, TG_TG_TIME_STAMP_SEL, 0);
        } else if (CAMSV_0 <= this->m_hwModule && this->m_hwModule < CAMSV_MAX){
            CAMSV_WRITE_BITS(this->m_pCamSVDrv, CAMSV_TG_TIME_STAMP_CTL, TG_TG_TIME_STAMP_SEL, 0);
        }
    } else {
        if (this->m_hwModule < PHY_CAM) {
            CAM_WRITE_BITS(this->m_pDrv->getPhyObj(), TG_R1_TG_TIME_STAMP_CTL, TG_TG_TIME_STAMP_SEL, 1);
        } else if (CAMSV_0 <= this->m_hwModule && this->m_hwModule < CAMSV_MAX){
            CAMSV_WRITE_BITS(this->m_pCamSVDrv, CAMSV_TG_TIME_STAMP_CTL, TG_TG_TIME_STAMP_SEL, 1);
        }
    }
...
MBOOL CAM_TIMESTAMP::TimeStamp_SrcSofSel(TSTMP_SRC_SOF src_sof)
{
    if (this->m_hwModule < PHY_CAM) {
        CAM_WRITE_BITS(this->m_pDrv->getPhyObj(), TG_R1_TG_TIME_STAMP_CTL, TG_TG_TIME_STAMP_SOF_SEL, src_sof);
    } else if (CAMSV_0 <= this->m_hwModule && this->m_hwModule < CAMSV_MAX){
        CAMSV_WRITE_BITS(this->m_pCamSVDrv, CAMSV_TG_TIME_STAMP_CTL, TG_TG_TIME_STAMP_SOF_SEL, src_sof);
    }
