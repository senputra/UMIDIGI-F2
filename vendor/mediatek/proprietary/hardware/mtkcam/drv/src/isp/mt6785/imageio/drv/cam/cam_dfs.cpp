...
CAM_DFS_Mgr::~CAM_DFS_Mgr()
{
    ...
    for(MUINT32 i=0; i < PHY_CAM; i++) {
        if (m_moduleTargetLevel[i] != -1) {
            CAM_ERR("logic err! need stop befroe destroy\n");
            break;
        } else if (i == (PHY_CAM -1))
...
std::string CAM_DFS_Mgr::CAM_DFS_FSM_STR(E_FSM op) {
    switch(op) {
        case e_init: return "e_init";
        case e_get: return "e_get";
        case e_set: return "e_set";
        case e_start: return "e_start";
...
MBOOL CAM_DFS_Mgr::CAM_DFS_FSM_CHECK(E_FSM op)
{
    ...
    if (ret) {
        //for log reducing, don't print set/get
        if (op != e_set && op != e_get) {
            CAM_INF("state:%s op:%s pass.\n",
                CAM_DFS_FSM_STR(this->m_FSM).c_str(),
                CAM_DFS_FSM_STR(op).c_str());
        }
    } else {
        CAM_ERR("state:%s op:%s fail.\n",
            CAM_DFS_FSM_STR(this->m_FSM).c_str(),
            CAM_DFS_FSM_STR(op).c_str());
    }
    return ret;
...
MBOOL CAM_DFS_Mgr::CAM_DFS_INIT(vector<MUINT32>* clk_gear)
{
    ...
    if(m_SupportedClk.size() == 0) {
        ...
        if(atoi(fakeclk[0]) || atoi(fakeclk[1]) || atoi(fakeclk[2])){
            ...
            CAM_INF("clklevelcnt=%d", supportedClk.clklevelcnt);
...
MBOOL CAM_DFS_Mgr::CAM_DFS_SetLv(MINT32 level)
{
    ...
    if(this->m_FSM == e_init) {
        //in this case, pipeline is not started yet! hold setting until start.
