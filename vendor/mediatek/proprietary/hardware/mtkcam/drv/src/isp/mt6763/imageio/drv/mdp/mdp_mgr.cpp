...
MINT32 _startMdp(MDPMGR_DPSTREAM_TYPE dpStreamtype,MDPMGR_CFG_STRUCT &cfgData,T* pDpStream)
{
        ...
        if(cfgData.dstPortCfg[index] == 1) {
            if (index != ISP_ONLY_OUT_TPIPE ) { // for non isp only
                ...
                if(err != MDPMGR_NO_ERROR) {
                    ...
                    LOG_DBG("index(%d),capbility(%d),isoValue(%d),fmt(%d),rot(%d),flip(%d),(w,h,s)=(%lu,%lu,%lu),C(w,h,s)=(%lu,%lu,%lu),V(w,h,s)=(%lu,%lu,%lu),dpStreamtype(%d),mdpindx(%d)",
                ...
                if (cfgData.mdpVSDOFPQParam != NULL) {
                    ISPParam.u.isp.VSDOFPQParam = cfgData.mdpVSDOFPQParam;
                }
                if (cfgData.mdpDCParam != NULL) {
                    ISPParam.u.isp.DCParam = cfgData.mdpDCParam;
                }
    		  ...
    		  // set port type (only for ISP only mode)
                ...
                LOG_DBG("dstSizeList=(0x%8x,0x%8x,0x%8x),dstVirList=(0x%lx,0x%lx,0x%lx),dstPhyList=(0x%8x,0x%8x,0x%8x),plane num(%d)",
                    dstSizeList[0],dstSizeList[1],dstSizeList[2],(unsigned long)dstVirList[0],(unsigned long)dstVirList[1],(unsigned long)dstVirList[2],
                ...
                LOG_DBG("index(%d),capbility(%d),fmt(%d),rot(%d),flip(%d),(w,h,s)=(%lu,%lu,%lu),C(w,h,s)=(%lu,%lu,%lu),V(w,h,s)=(%lu,%lu,%lu),dpStreamtype(%d),mdpindx(%d)",
    ...
    */
    ...
    int frmCQidx=cfgData.ispTpipeCfgInfo.drvinfo.cqIdx;
    int frmdupCQidx=cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx;
    int frmburstCQidx=cfgData.ispTpipeCfgInfo.drvinfo.burstCqIdx;
    int refCQidx=0;
    int refdupCQidx=0;
    int refburstCQidx=0;
    int dpObjCQidx=0;
    int dpObjdupCQidx=0;
    int dpObjburstCQidx=0;
    ...
    if(cfgData.ispTpipeCfgInfo.drvinfo.frameflag & 0x1) //skip tpipe, use the previous dpStream
    {
        ...
        dpObjCQidx=mDpStreamMap[frmCQidx][frmdupCQidx][frmburstCQidx].dpObjcqIdx;
        dpObjdupCQidx=mDpStreamMap[frmCQidx][frmdupCQidx][frmburstCQidx].dpObjdupCqIdx;
        dpObjburstCQidx=mDpStreamMap[frmCQidx][frmdupCQidx][frmburstCQidx].dpObjburstCqIdx;

        refCQidx=mDpStreamMap[frmCQidx][frmdupCQidx][frmburstCQidx].refcqIdx;
        refdupCQidx=mDpStreamMap[frmCQidx][frmdupCQidx][frmburstCQidx].refdupCqIdx;
        refburstCQidx=mDpStreamMap[frmCQidx][frmdupCQidx][frmburstCQidx].refburstCqIdx;

        srcTdriAddr=mDpStreamMap[dpObjCQidx][dpObjdupCQidx][dpObjburstCQidx].srcTpipeTableVa;
        pthread_mutex_unlock(&mMutex_DpStreamMap);

        if(p2tpipedumpEnable == 1)
        {
            LOG_INF("HMyo before (0x%lx _ 0x%lx, %d)", (unsigned long)srcTdriAddr, (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa, \
        ...
        if((cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa == srcTdriAddr) || (srcTdriAddr==NULL))
        {}
        else
        {
        ::memcpy(cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa, srcTdriAddr, MAX_ISP_TILE_TDR_HEX_NO);
        }
        LOG_DBG("HMyo CQ info (%d_%d_%d), ref(%d_%d_%d) dpobj(%d_%d_%d), (va: 0x%lx _ 0x%lx, dstpa: 0x%lx)",frmCQidx, \
			frmdupCQidx, frmburstCQidx,\
            refCQidx,refdupCQidx,refburstCQidx, dpObjCQidx, dpObjdupCQidx, dpObjburstCQidx,\
            (unsigned long)srcTdriAddr, (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa, \
            cfgData.ispTpipeCfgInfo.drvinfo.tpipeTablePa);
        if(p2tpipedumpEnable ==1)
        {
            LOG_INF("HMyo copy tpipe (va: 0x%lx _ 0x%lx, dstpa: 0x%lx, %d)",(unsigned long)srcTdriAddr, (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa,cfgData.ispTpipeCfgInfo.drvinfo.tpipeTablePa, \
            !(::memcmp(srcTdriAddr, cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa,MAX_ISP_TILE_TDR_HEX_NO)));
            for(int q=0;q<20; q++)
            {
    ...
    else
    {
        ...
        LOG_DBG("HMyo CQ info (%d_%d_%d), ref(%d_%d_%d) dpobj(%d_%d_%d), (va: 0x%lx)",frmCQidx, frmdupCQidx, frmburstCQidx,\
            refCQidx,refdupCQidx,refburstCQidx,dpObjCQidx, dpObjdupCQidx, dpObjburstCQidx, (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa);
    }
	pthread_mutex_lock(&mMutex_DpStreamMap);
    mDpStreamMap[dpObjCQidx][dpObjdupCQidx][dpObjburstCQidx].dpObjState=DPOBJ_STATE_AFTERTPIPECOPY;
    pthread_mutex_unlock(&mMutex_DpStreamMap);

    //
    LOG_DBG("HMyo setParameter (tdri va/pa: 0x%lx/0x%lx _ %d)",(unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa,cfgData.ispTpipeCfgInfo.drvinfo.tpipeTablePa, \
                cfgData.ispTpipeCfgInfo.drvinfo.frameflag);
    if(sizeof(cfgData.ispTpipeCfgInfo)!=sizeof(ISP_TPIPE_CONFIG_STRUCT))
    {
        LOG_ERR("ISP_TPIPE_CONFIG mismatch\n");
        return MDPMGR_API_FAIL;
    }
    ...
    pthread_mutex_lock(&mMutex_DpStreamMap);
    mDpStreamMap[dpObjCQidx][dpObjdupCQidx][dpObjburstCQidx].srcTpipeTableVa = \
        cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa;
    mDpStreamMap[dpObjCQidx][dpObjdupCQidx][dpObjburstCQidx].dpObjState=DPOBJ_STATE_AFTERSTARTSTREAM;
    pthread_mutex_unlock(&mMutex_DpStreamMap);
    if(p2disableSkipTpipe)
    {
        LOG_INF("+sceID(%u),cqPa(0x%lx)-Va(0x%lx),tpipVa(0x%lx)-Pa(0x%lx),isWaitB(%d),venc(%d),\
        HMyo CQ info (%d_%d_%d), ref(%d_%d_%d) dpobj(%d_%d_%d), (va: 0x%lx),\
        ...
        HMyo CQ(%d_%d_%d),ref(%d_%d_%d),(va:0x%lx_0x%lx,dstpa:0x%lx)\nflag(%d_0x%x->0x%x),startStm(%d us)",\
                ...
                frmCQidx, frmdupCQidx, frmburstCQidx, refCQidx,refdupCQidx,refburstCQidx,dpObjCQidx, dpObjdupCQidx, dpObjburstCQidx, (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa,\
                cfgData.mdpSrcFmt,cfgData.mdpSrcW,cfgData.mdpSrcH,cfgData.mdpSrcYStride,cfgData.mdpSrcUVStride,cfgData.mdpSrcBufSize,cfgData.mdpSrcCBufSize,cfgData.mdpSrcVBufSize,cfgData.srcVirAddr,cfgData.srcPhyAddr,\
                ...
                refCQidx,refdupCQidx,refburstCQidx, (unsigned long)srcTdriAddr, (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa, \
    ...
    else
    {
        ...
        HMyo CQ(%d_%d_%d),ref(%d_%d_%d),(va:0x%lx_0x%lx,dstpa:0x%lx)\nflag(%d_0x%x->0x%x),startStm(%d us)",\
                ...
                cfgData.dstDma[ISP_ONLY_OUT_TPIPE].size_v.w,cfgData.dstDma[ISP_ONLY_OUT_TPIPE].size_v.h,cfgData.dstDma[ISP_ONLY_OUT_TPIPE].size_v.stride,
                mdpPortMapping[ISP_ONLY_OUT_TPIPE].mdpPortIdx,
                frmCQidx,frmdupCQidx, frmburstCQidx,\
                refCQidx,refdupCQidx,refburstCQidx, (unsigned long)srcTdriAddr, (unsigned long)cfgData.ispTpipeCfgInfo.drvinfo.tpipeTableVa, \
            ...
            for(MINT32 i = 0; i < ISP_DRV_DIP_CQ_NUM; i++) {
                for(MINT32 j = 0; j < MAX_DUP_CQ_NUM; j++) {
                    for(MINT32 k = 0; k < MAX_BURST_QUE_NUM; k++) {
                        ...
                        mDpStreamMap[i][j][k].selfTpipeTableVa=NULL;
                        mDpStreamMap[i][j][k].srcTpipeTableVa=NULL;
                        mDpStreamMap[i][j][k].needReturnObj=MFALSE;
                        mDpStreamMap[i][j][k].dpObjState=DPOBJ_STATE_NONE;
                    }
                }
        }
        pthread_mutex_unlock(&mMutex_DpStreamMap);
...
MINT32 MdpMgrImp::startMdp(MDPMGR_CFG_STRUCT &cfgData)
{
	...
	{
		DpIspStream *pDpStream = NULL;
		pDpStream = selectWpeStream(cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx_wpe);
		if(pDpStream == NULL)
        {
...
MINT32 MdpMgrImp::dequeueFrameEnd(MDPMGR_CFG_STRUCT &cfgData)
{
    ...
    //
    MUINT32 dpObjcqIdx=0;
    MUINT32 dpObjdupcqIdx=0;
    MUINT32 dpObjburstcqIdx=0;
    DPOBJ_STATE dpObjState=DPOBJ_STATE_NONE;
    bool doUpdate=MFALSE;
    pthread_mutex_lock(&mMutex_DpStreamMap);
    dpObjcqIdx=mDpStreamMap[cfgData.ispTpipeCfgInfo.drvinfo.cqIdx][cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx][cfgData.ispTpipeCfgInfo.drvinfo.burstCqIdx].dpObjcqIdx;
    dpObjdupcqIdx=mDpStreamMap[cfgData.ispTpipeCfgInfo.drvinfo.cqIdx][cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx][cfgData.ispTpipeCfgInfo.drvinfo.burstCqIdx].dpObjdupCqIdx;
    dpObjburstcqIdx=mDpStreamMap[cfgData.ispTpipeCfgInfo.drvinfo.cqIdx][cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx][cfgData.ispTpipeCfgInfo.drvinfo.burstCqIdx].dpObjburstCqIdx;
    dpObjState = mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].dpObjState;
    if(mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].needReturnObj == MTRUE)
    {   //wait hw done, update srcTpipebuffer for this dpStreamObj due to the original srcTpipeBuffer might be re-calculated
        // in some usage combination
        //tpipe data woulb be the latest for current user after state is afterstartStream, so we just need to update srcTpipeTable as selfTpipeTable
        mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].srcTpipeTableVa=\
            mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].selfTpipeTableVa;
        //
        mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].needReturnObj=MFALSE;
        doUpdate=MTRUE;
    }
	switch(mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].dpObjState)
	{
		case DPOBJ_STATE_NONE:
		case DPOBJ_STATE_AFTERTPIPECOPY:
		default:
			break;
		case DPOBJ_STATE_AFTERSTARTSTREAM: //in same frame
			mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].dpObjState=DPOBJ_STATE_AFTERFRAMEDONE;
			break;
	}
	pthread_mutex_unlock(&mMutex_DpStreamMap);
    if(doUpdate)
    {
        LOG_INF("HMyo happen d %d_%d_%d,%d_%d_%d, %d\n",cfgData.ispTpipeCfgInfo.drvinfo.cqIdx,\
            cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx,\
            cfgData.ispTpipeCfgInfo.drvinfo.burstCqIdx,\
            dpObjcqIdx,dpObjdupcqIdx,dpObjburstcqIdx,dpObjState);
    }
    //
    LOG_DBG("-,");
    return ret;
...
DpIspStream *MdpMgrImp::selectDpIspStream(MUINT32 sceID, MUINT32 cqIdx, MUINT32 dupCqIdx, MUINT32 burstCqIdx)
{
    ...
    if(m_pDpStream[cqidx][dupcqidx][burstcqidx]){
        ...
        LOG_DBG("HM upt,scenID(%d), cqIndx(%d_%d_%d -> %d_%d_%d), dpStream(0x%lx -> 0x%lx)",sceID, cqIdx, dupCqIdx, burstCqIdx, cqidx, dupcqidx, burstcqidx, \
            (unsigned long)m_pDpStream[cqIdx][dupCqIdx][burstCqIdx], (unsigned long)pRetDpIspStream);
    } else {
                            ...
                            MUINT32 refcqIdx,MUINT32 refdupcqIdx,MUINT32 refburstcqIdx, unsigned int* srcTdriAddr)
{
	...
	unsigned int* debugTdrtAddr2=NULL;
    ...
    if(flag&0x1) //skip tpipe, use the previous dpStream
    {
        ...
        mDpStreamMap[cqIdx][dupcqIdx][burstcqIdx].selfTpipeTableVa=srcTdriAddr;
    ...
    {
        pthread_mutex_lock(&mMutex_DpStreamMap);
        MUINT32 dpObjcqIdx=mDpStreamMap[cqIdx][dupcqIdx][burstcqIdx].dpObjcqIdx;
        MUINT32 dpObjdupcqIdx=mDpStreamMap[cqIdx][dupcqIdx][burstcqIdx].dpObjdupCqIdx;
        MUINT32 dpObjburstcqIdx=mDpStreamMap[cqIdx][dupcqIdx][burstcqIdx].dpObjburstCqIdx;
        DPOBJ_STATE dpObjState=mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].dpObjState;
        bool doUpdate=MFALSE;
        if((mDpStreamMap[cqIdx][dupcqIdx][burstcqIdx].frameflag == 0x1) &&
           (mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].srcTpipeTableVa == srcTdriAddr) &&
           !(cqIdx == dpObjcqIdx && dupcqIdx == dpObjdupcqIdx && burstcqIdx == dpObjburstcqIdx))
        {   //change from reference to re-calculate and other referred dpStreamObj is using the same srcTdriBuffer with the current srcTdriBuffer(cq/dupcq/burstcq) 
            //backup original latest tpipe buffer for referred DpStreamObj 
            doUpdate=MTRUE; //only for print log debug
            debugTdrtAddr2 = mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].srcTpipeTableVa;
            switch(dpObjState)
            {
            	case DPOBJ_STATE_AFTERFRAMEDONE:
            		//only update before another user do tile copy using the same dpobj
                	::memcpy(mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].selfTpipeTableVa, \
                    	mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].srcTpipeTableVa, MAX_ISP_TILE_TDR_HEX_NO);
                	mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].srcTpipeTableVa = \
                    	mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].selfTpipeTableVa;
                	break;
				case DPOBJ_STATE_AFTERSTARTSTREAM:     
                	mDpStreamMap[dpObjcqIdx][dpObjdupcqIdx][dpObjburstcqIdx].needReturnObj = MTRUE;
                	break;
				case DPOBJ_STATE_AFTERTPIPECOPY:
				default:
					break;
        ...
        if(doUpdate)
        {
            LOG_INF("HMyo happen u %d_%d_%d,%d_%d_%d, %d, 0x%lx, 0x%lx\n",cqIdx,dupcqIdx,burstcqIdx,\
                dpObjcqIdx,dpObjdupcqIdx,dpObjburstcqIdx,dpObjState, (unsigned long)debugTdrtAddr2, (unsigned long)srcTdriAddr);
        }
    }

    LOG_DBG("- HM cq(%d_%d_%d_0x%lx), updatedRef(%d_%d_%d_0x%lx)", cqIdx, dupcqIdx, burstcqIdx, (unsigned long)srcTdriAddr,\
        debugCqIdx, debugdupCqIdx, debugburstCqIdx, (unsigned long)debugTdrtAddr);
    return ret;
}
...
MVOID dumpIspTPipeInfo(ISP_TPIPE_CONFIG_STRUCT a_info, MBOOL dump_en)
{
    if(dump_en)
    {
        ...
        LOG_INF("top.hlr2_en         (%d)"                            ,a_info.top.hlr2_en         );
        ...
        LOG_INF("top.sl2k_en         (%d)"                            ,a_info.top.sl2k_en         );
        ...
        LOG_INF("top.srz5_en         (%d)"                            ,a_info.top.srz5_en         );
        ...
        LOG_INF("top.flc2_en         (%d)"                            ,a_info.top.flc2_en         );
        ...
        LOG_INF("mfb.bld_mbd_wt_en(%d)",                              a_info.mfb.bld_mbd_wt_en);
        ...
        LOG_INF("ndg.non_block_base(%d)",                          a_info.ndg.non_block_base);
        LOG_INF("ndg2.non_block_base(%d)",                         a_info.ndg2.non_block_base);
        ...
        LOG_INF("lce.lce_hlr_mode(%d)",                            a_info.lce.lce_hlr_mode);
        ...
        LOG_INF("srz5.srz_input_crop_width(%d)",                   a_info.srz5.srz_input_crop_width);
        LOG_INF("srz5.srz_input_crop_height(%d)",                  a_info.srz5.srz_input_crop_height);
        LOG_INF("srz5.srz_output_width(%d)",                       a_info.srz5.srz_output_width);
        LOG_INF("srz5.srz_output_height(%d)",                      a_info.srz5.srz_output_height);
        LOG_INF("srz5.srz_luma_horizontal_integer_offset(%d)",     a_info.srz5.srz_luma_horizontal_integer_offset);
        LOG_INF("srz5.srz_luma_horizontal_subpixel_offset(%d)",    a_info.srz5.srz_luma_horizontal_subpixel_offset);
        LOG_INF("srz5.srz_luma_vertical_integer_offset(%d)",       a_info.srz5.srz_luma_vertical_integer_offset);
        LOG_INF("srz5.srz_luma_vertical_subpixel_offset(%d)",      a_info.srz5.srz_luma_vertical_subpixel_offset);
        LOG_INF("srz5.srz_horizontal_coeff_step(%d)",              a_info.srz5.srz_horizontal_coeff_step);
        LOG_INF("srz5.srz_vertical_coeff_step(%d)",                a_info.srz5.srz_vertical_coeff_step);
        ...
        LOG_INF("nr3d.tnr_color_en(%d)",                           a_info.nr3d.tnr_color_en);
        LOG_INF("nr3d.tnr_en(%d)",                                 a_info.nr3d.tnr_en);
...
MVOID dumpWpeTPipeInfo(ISP_TPIPE_CONFIG_STRUCT a_info, MBOOL dump_en)
{
    if(dump_en)
    {
		...
		LOG_INF("top.warp_en = %d",                      a_info.top.warp_en);
    ...
    if(mwpeInitCount == 0) {
		...
		{   //wpe stream 
			case TILE_SEL_MODE_WPE:
				...
				{
    				m_wDpStream[i] = new DpIspStream(DpIspStream::WPE_STREAM);
				}
				break;
			default:
...
*
**************************************************************************/
DpIspStream *MdpMgrImp::selectWpeStream(MUINT32 dupCqIdx)
{
    LOG_DBG("+");

    Mutex::Autolock lock(mLock);

    return m_wDpStream[dupCqIdx];
}
...
MINT32 _wpestartMdp(MDPMGR_CFG_STRUCT &cfgData,T* pDpStream)
{
    MINT32 err = MDPMGR_NO_ERROR;
    DP_STATUS_ENUM err2 = DP_STATUS_RETURN_SUCCESS;
    LOG_ERR("DO NOT SUPPORT WPE");
    cfgData; pDpStream; // avoid build warning
    /*
    MUINT32 srcVirList[PLANE_NUM];
    MUINT32 srcPhyList[PLANE_NUM];
    MUINT32 srcSizeList[PLANE_NUM];
    ...
    char value_skiptpipeDBG[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("camera.disable_p2skiptpipe", value_skiptpipeDBG, "0"); //temp, need set 0 in default
    int p2disableSkipTpipe=0;
    p2disableSkipTpipe=atoi(value_skiptpipeDBG);
    ...
    // image configure
    LOG_INF("[WPE]cqIndx(%d), frameflag(%d), srcFmt(0x%x),W(%u),H(%u),stride(%u,%u),size(0x%x-0x%x-0x%x),VA(0x%8x),PA(0x%8x),planeN(%d), \
             ...
             cfgData.ispTpipeCfgInfo.drvinfo.dupCqIdx_wpe, cfgData.ispTpipeCfgInfo.drvinfo.frameflag,\
    	            	...
    	            	case NSImageio::NSIspio::EPortCapbility_Rcrd:
                            {
                                ...
                                if(err2 != DP_STATUS_RETURN_SUCCESS) {
                                    LOG_ERR("index(%d), setPQParameter fail(%d)",index, err);
                                    return MDPMGR_API_FAIL;
                                }
                            }
    	            		break;
