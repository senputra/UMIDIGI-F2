...
MBOOL PIPE_CHECK::DMAO_STATUS(void)
{
    ...
    char str[32] = {"\0"};
    char* ptr = add;
    char const * dma_str[_cam_max_] = {"_imgo_","_rrzo_","_ufeo_","_aao_","_afo_","_lcso_","_pdo_","_eiso_","_flko_","_rsso_","_pso_",
        "_bpci_","_lsci_","_rawi_"};
    ...
    for(MUINT32 i=0;i<_cam_max_;i++){
        if(dma_err[i] & 0xffff){
            snprintf(str, sizeof(str), "%s ERR:0x%x ",dma_str[i],dma_err[i]);
            if(( str_leng - strlen(add) - 1) > strlen(str)){
                ptr += snprintf(ptr, sizeof(*ptr), "%s",str);
            }
            else{
...
MBOOL PIPE_CHECK::AA_CHECK(void)
{
    ...
    aa_in_v = aa_in_v >> (CAM_READ_BITS(this->m_pDrv->getPhyObj(),CAM_CTL_EN,BIN_EN) +
        CAM_READ_BITS(this->m_pDrv->getPhyObj(),CAM_CTL_DMA_EN,VBN_EN));
