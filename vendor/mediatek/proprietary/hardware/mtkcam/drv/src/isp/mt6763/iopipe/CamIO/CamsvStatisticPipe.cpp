...
MBOOL CamsvStatisticPipe::deque(PortID port, QBufInfo& rQBuf, ESTT_CACHE_SYNC_POLICY cacheSyncPolicy, MUINT32 u4TimeoutMs)
{
...

    {
        ...
        for(MUINT32 idx=0; idx<rQTBufInfo.vBufInfo.size(); idx++){
            ...
            PIPE_DBG("deque_imgo_%d: size:0x%08x_0x%08x, pa:0x%p_0x%p, magic:0x%x, time:%d_%d, Tg_Camsv size:%dx%d,crop:%d_%d_%d_%d",\
                idx,\
                rQTBufInfo.vBufInfo.at(idx).u4BufSize[0],\
                rQTBufInfo.vBufInfo.at(idx).Frame_Header.u4BufSize,\
                (void*)rQTBufInfo.vBufInfo.at(idx).u4BufPA[0],\
                (void*)rQTBufInfo.vBufInfo.at(idx).Frame_Header.u4BufPA,\
                ...
                if(pbuf->mPa == mpbuf_camsv_imgo[b].phyAddr) {
                    ...
                    switch(cacheSyncPolicy){
                        ...
                        case ESTT_CacheInvalidAll:
                            //mpIMem->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID_ALL, &(mpbuf_camsv_imgo[b]), MFALSE);
    ...
    do {
        ...
        switch (pxl){
            case NSImageio::NSIspio::ePxlMode_One_:
                qryInput.pixMode  = NSIoPipe::NSCamIOPipe::_1_PIX_MODE;
                break;
            case NSImageio::NSIspio::ePxlMode_Two_:
                qryInput.pixMode  = NSIoPipe::NSCamIOPipe::_2_PIX_MODE;
                break;
            case NSImageio::NSIspio::ePxlMode_Four_:
                qryInput.pixMode  = NSIoPipe::NSCamIOPipe::_4_PIX_MODE;
                break;
            default:
                PIPE_ERR("error pix mode(%d)\n", pxl);
                qryInput.pixMode = NSIoPipe::NSCamIOPipe::_UNKNOWN_PIX_MODE;
                ret = MFALSE;
                break;
        }
        ...
        /*if(this->bypass_imgo == MFALSE)*/ /* _CAM_SV_ : UT : FIX ME */
        {
            ...
            _out_port->u4ImgWidth     = xsize_byte;//bIMG_W;
            ...
            switch( _out_port->ePxlMode ) {
                ...
                case ePxlMode_Four_:
                    ...
                    break;
                default:
                    PIPE_ERR("Unsupported Pixel Mode(%d)", _out_port->ePxlMode);
                    return MFALSE;
                    break;
