...
Mutex  TwinMgr_IMP::m_LastP1DoneMutex;//protect setting last p1 done for multi-sensor
...
TwinMgr_IMP::TwinMgr_IMP()
{
    ...
    m_srcImgSize.w = 0;
    m_srcImgSize.h = 0;
    m_srcImgSize.dbn_en = 0;
    m_srcImgSize.bin_en = 0;
    m_bDoneCollect = MFALSE;
    m_TgIdx = TG_CAM_MAX;
...
void TwinMgr_IMP::destroyInstance()
{
    ...
    LOG_INF("destroyInstance: module(%d)\n", this->m_hwModule);
...
MINT32 TwinMgr_IMP::setP1Update(P1_TUNING_NOTIFY* pCB)
{
    DMA_UFGO ufgo;
    DMA_UFEO ufeo;
    ...
    ufgo.m_pIspDrv = (IspDrvVir*)this->m_pCmdQdrv->CmdQMgr_GetCurCycleObj(targetIdxMain)[0];
    ufeo.m_pIspDrv = (IspDrvVir*)this->m_pCmdQdrv->CmdQMgr_GetCurCycleObj(targetIdxMain)[0];
    ufeo.m_img_size = MSize(m_srcImgSize.w, m_srcImgSize.h);
    ...
    uf.bOFF = off_uf_rrzo;
    v_uf.push_back(uf);
    for(MUINT32 i=1;i<this->m_pCmdQdrv->CmdQMgr_GetBurstQ();i++){
        uf.dmachannel = _imgo_;
        uf.bOFF = off_uf_imgo;
        v_uf.push_back(uf);
        uf.dmachannel = _rrzo_;
    ...
    if (this->getIsTwin() == MTRUE) {
        ...
        for (MUINT32 i = 0; i < this->m_twinVHWModules.size(); i++) {
            pCQMgr = this->m_pActiveTwinCQMgrs[ISP_DRV_CQ_THRE0].at(this->m_twinVHWModules.at(i));
            v_obj.push_back((IspDrvVir*)pCQMgr->CmdQMgr_GetCurCycleObj(pCQMgr->CmdQMgr_GetDuqQIdx())[0]);
        }
    ...
    //
    v_obj.push_back(this->m_pCmdQdrv->CmdQMgr_GetCurCycleObj(targetIdxMain)[0]);

    if(pCB){
        pCB->p1TuningNotify((MVOID*)&v_obj,NULL);
    }
    else{
        LOG_ERR("no QOS 2.0, update fail\n");
        return 1;
    }
...
MBOOL TwinMgr_IMP::pathControl(TWIN_MGR_PATH_CFG_IN_PARAM &cfgInParam)
{
    ...
    LOG_DBG("cfgIn: isTwin:%d tg_crop(%dx%d), rrzo_bypass(%d)\n", cfgInParam.isTwin,
        cfgInParam.img_crop_w, cfgInParam.img_crop_h, cfgInParam.bypass_rrzo);
    ...
    for (MUINT32 i = 0; i < cfgInParam.slave_mod.size(); i++) {
        ...
        if (CamInfo.m_DTwin.GetDTwin() == MFALSE){
            ...
            switch (cfgInParam.slave_mod.at(i)) {
            case CAM_B:
                this->m_twinVHWModules.push_back(CAM_A_TWIN_B);
...
NOTWIN_EXIT:
    ...
    //keep tg ,bin info
    this->m_srcImgSize.w = cfgInParam.img_crop_w;
    this->m_srcImgSize.h = cfgInParam.img_crop_h;
    this->m_srcImgSize.dbn_en = cfgInParam.bDbn;
    this->m_srcImgSize.bin_en = cfgInParam.bBin;
...
MBOOL TwinMgr_IMP::twinInfoUpdate(const TWIN_MGR_PATH_CFG_IN_PARAM& cfgInParam, MBOOL isTwin)
{
    ...
    this->m_isTwinDmao[TWIN_AFO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&_size,0);
    if(_size == 0){// in order to simply logic in getistwin()
        RACING_PROT update(&this->m_latesetDmao[TWIN_AFO_R1],isTwin);
    }
    this->m_isTwinDmao[TWIN_AFO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_push,(MUINTPTR)&isTwin,0);
    if(_size > MAX_QUE_SIZE){
        this->m_isTwinDmao[TWIN_AFO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_erase,1,0);
        LOG_DBG("no deque on AFO over %d frames\n",MAX_QUE_SIZE);
    }

    if(cfgInParam.bypass_imgo == MFALSE){
        this->m_isTwinDmao[TWIN_IMGO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&_size,0);
        if(_size == 0){// in order to simply logic in getistwin()
            RACING_PROT update(&this->m_latesetDmao[TWIN_IMGO_R1],isTwin);
        }
        this->m_isTwinDmao[TWIN_IMGO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_push,(MUINTPTR)&isTwin,0);
        if(_size > MAX_QUE_SIZE){
            this->m_isTwinDmao[TWIN_IMGO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_erase,1,0);
            LOG_WRN("no deque on IMGO over %d frames\n",MAX_QUE_SIZE);
        }
    }
    if(cfgInParam.bypass_rrzo == MFALSE){
        this->m_isTwinDmao[TWIN_RRZO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&_size,0);
        if(_size == 0){// in order to simply logic in getistwin()
            RACING_PROT update(&this->m_latesetDmao[TWIN_RRZO_R1],isTwin);
        }
        this->m_isTwinDmao[TWIN_RRZO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_push,(MUINTPTR)&isTwin,0);
        if(_size > MAX_QUE_SIZE){
            this->m_isTwinDmao[TWIN_RRZO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_erase,1,0);
            LOG_WRN("no deque on RRZO over %d frames\n",MAX_QUE_SIZE);
        }
    }
    if(cfgInParam.bypass_yuvo == MFALSE){
        this->m_isTwinDmao[TWIN_YUVO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&_size,0);
        if(_size == 0){// in order to simply logic in getistwin()
            RACING_PROT update(&this->m_latesetDmao[TWIN_YUVO_R1],isTwin);
        }
        this->m_isTwinDmao[TWIN_YUVO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_push,(MUINTPTR)&isTwin,0);
        if(_size > MAX_QUE_SIZE){
            this->m_isTwinDmao[TWIN_YUVO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_erase,1,0);
            LOG_WRN("no deque on YUVO over %d frames\n",MAX_QUE_SIZE);
        }
    }
    if(cfgInParam.bypass_crzo_r1== MFALSE){
        this->m_isTwinDmao[TWIN_CRZO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&_size,0);
        if(_size == 0){// in order to simply logic in getistwin()
            RACING_PROT update(&this->m_latesetDmao[TWIN_CRZO_R1],isTwin);
        }
        this->m_isTwinDmao[TWIN_CRZO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_push,(MUINTPTR)&isTwin,0);
        if(_size > MAX_QUE_SIZE){
            this->m_isTwinDmao[TWIN_CRZO_R1].sendCmd(QueueMgr<MBOOL>::eCmd_erase,1,0);
            LOG_WRN("no deque on CRZO_R1 over %d frames\n",MAX_QUE_SIZE);
        }
    }
    if(cfgInParam.bypass_crzo_r2 == MFALSE){
        this->m_isTwinDmao[TWIN_CRZO_R2].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&_size,0);
        if(_size == 0){// in order to simply logic in getistwin()
            RACING_PROT update(&this->m_latesetDmao[TWIN_CRZO_R2],isTwin);
        }
        this->m_isTwinDmao[TWIN_CRZO_R2].sendCmd(QueueMgr<MBOOL>::eCmd_push,(MUINTPTR)&isTwin,0);
        if(_size > MAX_QUE_SIZE){
            this->m_isTwinDmao[TWIN_CRZO_R2].sendCmd(QueueMgr<MBOOL>::eCmd_erase,1,0);
            LOG_WRN("no deque on CRZO_R2 over %d frames\n",MAX_QUE_SIZE);
        }
    }
...
{
    MBOOL       ret = MTRUE;
    MUINT32     _size[TWIN_DMAO_MAX], sum = 0;
    string      dbg_str;
    capibility  CamInfo;
    ...
    for(MUINT32 i = 0; i < TWIN_DMAO_MAX; i ++){
        _size[i] = 0;
        this->m_isTwinDmao[i].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&(_size[i]),0);
    }
    if ((_size[TWIN_IMGO_R1] + _size[TWIN_RRZO_R1]) == 0) { //if twin is not needed, twin can't be init
        LOG_ERR("imgo/rrzo both enque %d_%d\n", _size[TWIN_IMGO_R1], _size[TWIN_RRZO_R1]);
        return MFALSE;
    }
    ...
    if (CamInfo.m_DTwin.GetDTwin()) {
        ...
        case CAM_A:
            ...
            twinCombinations.at(0).push_back(CAM_A_TWIN_B);
            break;
        default:
    ...
    if (subSample) {
        this->m_pActiveTwinCQMgrs[ISP_DRV_CQ_THRE28].assign(CAM_MAX, NULL);
    }
    else {
    ...
    for (MUINT32 i = 0; i< twinCombinations.size(); i++) {
        for (MUINT32 j = 0; j < twinCombinations.at(i).size(); j++) {
            ...
            if (subSample) {
                pCqmgr = DupCmdQMgr::CmdQMgr_attach(pInput->tgFps, subSample,_mod,ISP_DRV_CQ_THRE28);
                if (pCqmgr == NULL) {
                    LOG_ERR("Twin CQ28 init fail twinModule: %d\n", _mod);
                    return MFALSE;
                }
                this->m_pActiveTwinCQMgrs[ISP_DRV_CQ_THRE28].at(_mod) = pCqmgr;
            }
            ...
            //For secure camera twin enable
            if(pSec){
                if(pSec->SecMgr_GetSecureCamStatus(pMainCmdQMgr->CmdQMgr_GetCurModule()) > 0){
                    if(MFALSE == pSec->SecMgr_attach_twin(_mod)){
                        LOG_ERR("Secure twin CAM attach failure\n");
                        return MFALSE;
                    }
                }
            }
...
MINT32 TwinMgr_IMP::setP1YuvoCfg(list<DMACfg>* pYuvo_cfg_L)
{
    return ((CamPathPass1 *)this)->setP1YuvoCfg(pYuvo_cfg_L);
}

MINT32 TwinMgr_IMP::setP1CrzoR1Cfg(list<IspCrzrCfg>* pCrzo_cfg_L)
{
    return ((CamPathPass1 *)this)->setP1CrzoR1Cfg(pCrzo_cfg_L);
}

MINT32 TwinMgr_IMP::setP1CrzoR2Cfg(list<IspCrzrCfg>* pCrzo_cfg_L)
{
    return ((CamPathPass1 *)this)->setP1CrzoR2Cfg(pCrzo_cfg_L);
}

MINT32 TwinMgr_IMP::setP1RawiCfg(list<DMACfg>* pRawi_cfg_L)
{
    return ((CamPathPass1 *)this)->setP1RawiCfg(pRawi_cfg_L);
}
...
{
    MBOOL bMultiChannel = MFALSE, bDyuv = MFALSE;
    MINT32 ret = 0;
...
MBOOL  TwinMgr_IMP::P1Notify_Mapping(MUINT32 cmd,P1_TUNING_NOTIFY* pObj,MBOOL bInitFBC)
{
    return ((CamPathPass1 *)this)->P1Notify_Mapping(cmd, pObj, bInitFBC);
}

MBOOL  TwinMgr_IMP::P1Notify_Global_Mapping(MUINT32 cmd,P1_TUNING_NOTIFY* pObj)
{
    return ((CamPathPass1 *)this)->P1Notify_Global_Mapping(cmd, pObj);
}
...
{
    //last p1 done
    LOG_DBG("DoneCollector +, bDoneCollect:%d,tgidx:%d,tgevent:%d,size:%d_%d\n",
        pathParm.bDoneCollect, pathParm.TgIdx, pathParm.TgEvent,pathParm.SrcSize.w,pathParm.SrcSize.h);

    TwinMgr_IMP::m_LastP1DoneMutex.lock();
    this->m_bDoneCollect = pathParm.bDoneCollect;
    this->m_TgIdx = pathParm.TgIdx;
    ((CamPathPass1 *)this)->DoneCollector(this->m_bDoneCollect, this->m_TgIdx);
    TwinMgr_IMP::m_LastP1DoneMutex.unlock();

    LOG_DBG("CamPathPass1::updatePath +\n");
...
MBOOL TwinMgr_IMP::getIsTwin(MBOOL bDeque,MBOOL bDyuv)
{
    MBOOL isTwin;
    MUINT32 Qsize[TWIN_DMAO_MAX],_size = 0;
    ...
    for(MUINT32 i = 0; i < TWIN_DMAO_MAX; i++){
        Qsize[i] = 0;
        this->m_isTwinDmao[i].sendCmd(QueueMgr<MBOOL>::eCmd_getsize,(MUINTPTR)&Qsize[i],0);
    }

    if (bDyuv) {
        if (Qsize[TWIN_YUVO_R1] > 1) {
            _size = Qsize[TWIN_YUVO_R1];
            ptr = &this->m_isTwinDmao[TWIN_YUVO_R1];
            _ptr = &this->m_latesetDmao[TWIN_YUVO_R1];
        }
        else if (Qsize[TWIN_CRZO_R1] > 1) {
            _size = Qsize[TWIN_CRZO_R1];
            ptr = &this->m_isTwinDmao[TWIN_CRZO_R1];
            _ptr = &this->m_latesetDmao[TWIN_CRZO_R1];
        }
        else if (Qsize[TWIN_CRZO_R2] > 1) {
            _size = Qsize[TWIN_CRZO_R2];
            ptr = &this->m_isTwinDmao[TWIN_CRZO_R2];
            _ptr = &this->m_latesetDmao[TWIN_CRZO_R2];
        }
        else {
            LOG_ERR("can't run pipeline with both yuvo/crzo r closed\n");
            return MFALSE;
        }
    }
    else {
        if(Qsize[TWIN_IMGO_R1] > 1){
            _size = Qsize[TWIN_IMGO_R1];
            ptr = &this->m_isTwinDmao[TWIN_IMGO_R1];
            _ptr = &this->m_latesetDmao[TWIN_IMGO_R1];
        }
        else if(Qsize[TWIN_RRZO_R1] > 1){
            _size = Qsize[TWIN_RRZO_R1];
            ptr = &this->m_isTwinDmao[TWIN_RRZO_R1];
            _ptr = &this->m_latesetDmao[TWIN_RRZO_R1];
        }
        else{
...
MBOOL TwinMgr_IMP::popTwinRst(MUINT32 dmaChannel)
{
    ...
    switch(dmaChannel){
        ...
        case _crzo_r2_:
        {
            MUINT32 _port = MAP_TWIN_DMAO(dmaChannel, MFALSE, LOG);
            ptr = &this->m_isTwinDmao[_port];
            _ptr = &this->m_latesetDmao[_port];
        }
            break;
        default:
...
MBOOL TwinMgr_IMP::runTwinDrv()
{
    ...
    //NOTE: burst page only runTwin once
    {
        ...
        this->m_LastP1DoneMutex.lock();
        this->m_TwinCtrl.DoneCollector(this->m_bDoneCollect, this->m_TgIdx);
        this->m_LastP1DoneMutex.unlock();
    }

    //for other burst pages
    burstQ = this->m_pMainCmdQMgr->CmdQMgr_GetBurstQ();
    for (bidx = 1; bidx < burstQ; bidx++) {
        vTwinDrvPtr.clear();
        this->m_TwinCtrl.m_pTwinIspDrv_v.clear();
        for (MUINT32 mod = 0; mod < this->m_twinVHWModules.size(); mod++) {
            DupCmdQMgr  *pCqMgr = this->m_pActiveTwinCQMgrs[ISP_DRV_CQ_THRE0].at(this->m_twinVHWModules.at(mod));
            ...
            vTwinDrvPtr.push_back(pCqMgr->CmdQMgr_GetCurCycleObj(targetIdxTwin)[0]); // using 0 for ref of bidx >=1 pages
            this->m_TwinCtrl.m_pTwinIspDrv_v.push_back(pCqMgr->CmdQMgr_GetCurCycleObj(targetIdxTwin)[bidx]);
        }
        //Lafite_TODO: wait CAM_TWIN_PIPE new API
        this->m_TwinCtrl.m_pIspDrv = (IspDrvVir*)this->m_pMainCmdQMgr->CmdQMgr_GetCurCycleObj(targetIdxMain)[bidx];
        this->m_TwinCtrl.update_afterRunTwin_FH(vTwinDrvPtr.size(), vTwinDrvPtr);
    }
...
MBOOL TwinMgr_IMP::cvtDupCmdQRegtoDualIspReg(vector<ISP_DRV_CAM *> &rTwinDrvs)
{
    ...
    this->m_DualInputParam.dualIspReg->ptr_isp_cq = NULL;
    this->m_DualInputParam.dualIspReg->ptr_isp_cq_d = NULL;

    this->m_pMainIspDrv->getCurObjInfo(&module, &cq, &page);
    switch (module) {
    case CAM_A:
        this->m_DualInputParam.dualIspReg->ptr_isp_reg = (cam_a_reg_t*)this->m_pMainIspDrv->getPhyObj()->getHwRegAddr();
        this->m_DualInputParam.dualIspReg->ptr_isp_cq = (cam_a_reg_t*)this->m_pMainIspDrv->getIspVirRegVirAddr();
        break;
    case CAM_B:
        this->m_DualInputParam.dualIspReg->ptr_isp_reg_d = (cam_b_reg_t*)this->m_pMainIspDrv->getPhyObj()->getHwRegAddr();
        this->m_DualInputParam.dualIspReg->ptr_isp_cq_d = (cam_b_reg_t*)this->m_pMainIspDrv->getIspVirRegVirAddr();
        break;
    default:
        LOG_ERR("unsupported master cam : %d!!\n", module);
        return MFALSE;
    ...
    for (MUINT32 i = 0; i < rTwinDrvs.size(); i++) {
        ...
        switch (module) {
        case CAM_B:
            this->m_DualInputParam.dualIspReg->ptr_isp_reg_d = (cam_b_reg_t*)rTwinDrvs.at(i)->getPhyObj()->getHwRegAddr();
            this->m_DualInputParam.dualIspReg->ptr_isp_cq_d = (cam_b_reg_t*)rTwinDrvs.at(i)->getIspVirRegVirAddr();
            break;
        case CAM_A:
            this->m_DualInputParam.dualIspReg->ptr_isp_reg = (cam_a_reg_t*)rTwinDrvs.at(i)->getPhyObj()->getHwRegAddr();
            this->m_DualInputParam.dualIspReg->ptr_isp_cq = (cam_a_reg_t*)rTwinDrvs.at(i)->getIspVirRegVirAddr();
            break;
        default:
...
MBOOL TwinMgr_IMP::prepareDualInCfg()
{
    ...
    //dual cal will check if RSSO_R2 /CRZO_R1/ CRZO_R2 (DMA) is enabled, then cal settings.
    //RSS_R2
    this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_IN_CROP_HT = 0;// This value can be set 0. But RSS_R2_VERT_STEP must be != 0
    this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_HORI_INT_OFST = this->m_TwinCtrl.m_rss_r2_roi.x;
    this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_HORI_INT_OFST_LAST = \
        this->m_TwinCtrl.m_rss_r2_roi.x + this->m_TwinCtrl.m_rss_r2_roi.w;
    this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_HORI_INT_OFST_LAST += (this->m_TwinCtrl.m_rss_r2_roi.floatX != 0)?(1):(0);
    this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_HORI_INT_OFST_LAST = \
        this->m_TwinCtrl.m_rss_r2_in.w - this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_HORI_INT_OFST_LAST;

    this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_HORI_SUB_OFST = 0; // Can set to 0
    this->m_DualInputParam.dualInCfg->SW.TWIN_RSS_R2_OUT_WD  = this->m_TwinCtrl.m_rss_r2_out_wd;

    //CRZ_R1
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_IN_CROP_HT = 0; // This value can be set 0. But CRZ_VERT_STEP must be != 0
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_HORI_INT_OFST = this->m_TwinCtrl.m_crz_r1_roi.x;
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_HORI_INT_OFST_LAST = \
        this->m_TwinCtrl.m_crz_r1_roi.x + this->m_TwinCtrl.m_crz_r1_roi.w;
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_HORI_INT_OFST_LAST += (this->m_TwinCtrl.m_crz_r1_roi.floatX != 0)?(1):(0);

    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_HORI_INT_OFST_LAST = \
        this->m_TwinCtrl.m_crz_r1_in.w - this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_HORI_INT_OFST_LAST;

    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_HORI_SUB_OFST = 0; // Can set to 0
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R1_OUT_WD = this->m_TwinCtrl.m_crz_r1_out_wd;

    //CRZ_R2
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_IN_CROP_HT = 0; // This value can be set 0. But CRZ_VERT_STEP must be != 0
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_HORI_INT_OFST = this->m_TwinCtrl.m_crz_r2_roi.x;
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_HORI_INT_OFST_LAST = \
        this->m_TwinCtrl.m_crz_r2_roi.x + this->m_TwinCtrl.m_crz_r2_roi.w;
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_HORI_INT_OFST_LAST += (this->m_TwinCtrl.m_crz_r2_roi.floatX != 0)?(1):(0);

    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_HORI_INT_OFST_LAST = \
        this->m_TwinCtrl.m_crz_r2_in.w - this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_HORI_INT_OFST_LAST;

    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_HORI_SUB_OFST = 0; // Can set to 0
    this->m_DualInputParam.dualInCfg->SW.TWIN_CRZ_R2_OUT_WD = this->m_TwinCtrl.m_crz_r2_out_wd;
    ...
    this->m_DualInputParam.dualInCfg->SW.TWIN_RAWI_XSIZE = this->m_srcImgSize.w;//unit: pixel
    this->m_DualInputParam.dualInCfg->SW.TWIN_RAWI_YSIZE = this->m_srcImgSize.h;//unit: lines
...
MUINT32 TwinMgr_IMP::getTwinHeaderSize(MUINT32 dmaChannel)
{
    ...
    case _crzo_:
    case _crzo_r2_:
        return ALIGN_HEAD_SIZE(rst.HeaderSize) * (CAM_C - CAM_A); //for 2aw max 1 slave
        break;
    case _pdo_:
        return 0;
        break;
...
MINT32 TwinMgr_IMP::dequeTwin( MUINT32 const dmaChannel ,vector<NSImageio::NSIspio::BufInfo>& bufInfo,CAM_STATE_NOTIFY *pNotify)
{
    ...
    if(this->getIsTwin(MTRUE,bDyuv) == MFALSE){
...
void TwinMgr_IMP::setDBLoadHold(MBOOL bHold, vector<MUINT32>* tuning_moduleEn, void* pCmdQ)
{
    MUINT32 subSel, subSel2, subSel3, subSel4, subSel5, subSel6;
    ISP_DRV_CAM* pDrv;
    DupCmdQMgr* pMainCmdQ = (DupCmdQMgr*)pCmdQ;

    // not necessary to set sub sel in db load hold disable case
    if(bHold == MFALSE){
        goto EXIT;
    }

#if 1
    subSel = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB_SUB_SEL);
    subSel2 = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB2_SUB_SEL);
    subSel3 = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB3_SUB_SEL);
    subSel4 = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB4_SUB_SEL);
    subSel5 = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB5_SUB_SEL);
    subSel6 = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB6_SUB_SEL);

    for(MUINT32 i = 0; i < this->m_twinVHWModules.size(); i++){
        pDrv = this->m_pActiveTwinCQMgrs[ISP_DRV_CQ_THRE0].at(this->m_twinVHWModules.at(i))->CmdQMgr_GetCurCycleObj(0)[0];

        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB_SUB_SEL, subSel);
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB2_SUB_SEL, subSel2);
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB3_SUB_SEL, subSel3);
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB4_SUB_SEL, subSel4);
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB5_SUB_SEL, subSel5);
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB6_SUB_SEL, subSel6);
    }
#else
    if(tuning_moduleEn->size() != 4){
        LOG_ERR("the size of tuning_moduleEn vector must be 4");
        goto EXIT;
    }
    ...
    subSel3 = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB3_SUB_SEL);
    subSel4 = CAM_READ_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB4_SUB_SEL);
    LOG_DBG("SubSel:0x%x SubSel2:0x%x SubSel3:0x%x SubSel4:0x%x", subSel, subSel2, subSel3, subSel4);

    // set main cam sub sel
    CAM_WRITE_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB_SUB_SEL, ((~tuning_moduleEn->at(0))&subSel));
    CAM_WRITE_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB2_SUB_SEL, ((~tuning_moduleEn->at(1))&subSel2));
    CAM_WRITE_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB3_SUB_SEL, ((~tuning_moduleEn->at(2))&subSel3));
    CAM_WRITE_REG(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_DB4_SUB_SEL, ((~tuning_moduleEn->at(3))&subSel4));

    // set slave cam sub sel
    for(MUINT32 i = 0; i < this->m_twinVHWModules.size(); i++){
        pDrv = this->m_pActiveTwinCQMgrs[ISP_DRV_CQ_THRE0].at(this->m_twinVHWModules.at(i))->CmdQMgr_GetCurCycleObj(0)[0];

        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB_SUB_SEL, ((~tuning_moduleEn->at(0))&subSel));
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB2_SUB_SEL, ((~tuning_moduleEn->at(1))&subSel2));
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB3_SUB_SEL, ((~tuning_moduleEn->at(2))&subSel3));
        CAM_WRITE_REG(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_DB4_SUB_SEL, ((~tuning_moduleEn->at(3))&subSel4));
    }
#endif

EXIT:
    // set main/slave db load hold
    CAM_WRITE_BITS(pMainCmdQ->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(), CAMCTL_R1_CAMCTL_MISC, CAMCTL_DB_LOAD_HOLD, bHold);
    for(MUINT32 i = 0; i < this->m_twinVHWModules.size(); i++){
        pDrv = this->m_pActiveTwinCQMgrs[ISP_DRV_CQ_THRE0].at(this->m_twinVHWModules.at(i))->CmdQMgr_GetCurCycleObj(0)[0];
        CAM_WRITE_BITS(pDrv->getPhyObj(), CAMCTL_R1_CAMCTL_MISC, CAMCTL_DB_LOAD_HOLD, bHold);
    }
}
...
MBOOL TwinMgr_IMP::getTwinALLCropinfo(vector<vector<ISP_HW_MODULE>>* pPath,vector<vector<STImgCrop>>* pcrop)
{
    ...
    if(this->m_hwModule == CAM_A){
        ...
        crop.x = crop.y = 0;
        crop.w = (this->m_srcImgSize.w >>(this->m_srcImgSize.dbn_en + this->m_srcImgSize.bin_en))>> 1;    //this calculation is dealed with twin_drv
        crop.h = (this->m_srcImgSize.h >> this->m_srcImgSize.bin_en);
...
MBOOL TwinMgr_IMP::suspend(void)
{
    ...
    twin.Raw = CAM_READ_REG(this->m_pCmdQdrv->CmdQMgr_GetCurCycleObj(0)[0]->getPhyObj(),CAMCTL_R1_CAMCTL_TWIN_STATUS);
    if (twin.Bits.TWIN_EN == MTRUE) {
        ...
        for (MUINT32 i = 0; i < this->m_twinVHWModules.size(); i++) {
            ...
            ret = CAM_BUF_CTRL::suspend(phy_mod, pCmdQ->CmdQMgr_GetCurCycleObj(pCmdQ->CmdQMgr_GetDuqQIdx())[0], phy_mod, NULL, CAM_BUF_CTRL::eSus_HW_SW_STATE);
            if (ret == MFALSE) {
...
MBOOL TwinMgr_IMP::getTwinReg(vector<ISP_HW_MODULE>* pMaster, vector<MUINT32>* pAddr)
{
    ...
    case CAM_B:
    case CAM_C:
        pMaster->push_back(this->m_hwModule);
        ret = CAM_TWIN_PIPE::GetTwinRegAddr(this->m_hwModule,&addr);
        pAddr->push_back(addr);
        break;
    default:
        ret = MFALSE;
        LOG_ERR("this module is not supported:%d",this->m_hwModule);
        break;
    }
