    ...
    , m_iDequeFrmNum(0)
{
    ...
    property_get("vendor.cam.dipsavefile.enable", value, "0");
    m_iSaveReqToFile = atoi(value);
...
)
{
    ...
    LOG_INF("- E mUCnt(%d), cUser(%s), pipeID(%d)",mInitCount,szCallerName, streamPipeID);
    ...
    if(mInitCount < MAX_PIPE_USER_NUMBER)
    {
        ...
        for(int i=0;i<mInitCount;i++)
        {
            if(this->mUserInfo[i].pipeID==ESatremPipeID_None)
            {
                ...
                strcpy((char*)(this->mUserInfo[i].name),szCallerName);
                getspace=true;
                break;
            }
        }
        ...
        if(!getspace)
        {
            ...
            strcpy((char*)(this->mUserInfo[mInitCount].name),szCallerName);
        }
    }
    else{
    ...
    vector<QParams>& callerDeQList)
{
    ...
    {
        LOG_ERR("unexpected frameParas size(%d)", rParams.mvFrameParams.size());
        return MFALSE;
    }
    ...
    switch(streamPipeID)
    {
		...
		case NSCam::NSIoPipe::EStreamPipeID_WarpEG:
            ...
            {
                LOG_ERR("we do not support warping engine in high frame rate feature, streamPipeID/mfps (%d/%d)",streamPipeID,this->mFps);
                return MFALSE;
            }
            //push to buffer list
            //this->mLWarpEnquePackageList.push_back(framePack);
            //send signal to corresponding enque thread
            //this->mHalPWrapperEQThread[static_cast<int>(EThreadProperty_WarpEG)]->addCmd(ECmd_ENQUE_REQ); //should add high frame rate control, pin-pong
            break;
        #endif
    }
...
{
    MBOOL ret=MTRUE;
    LOG_INF("+,cmd(0x%x),arg1(0x%x),arg2(0x%x),arg3(0x%x)",cmd,arg1,arg2,arg3);
    switch (cmd){
    ...
    ECmd const &cmd)
{
    ...
    LOG_DBG("tp(%d_%d) mCmdList, size(%d) + cmd(%d)",mThreadType,mThreadProperty, mLCmdList.size(), cmd);
    ...
    ECmd &cmd)
{
    ...
    if(mLCmdList.size()>0)
    {
        LOG_DBG("tp(%d_%d) mCmdList curSize(%d), todo(%d)",mThreadType,mThreadProperty, mLCmdList.size(), *mLCmdList.begin());
        cmd = *mLCmdList.begin();
