...
extern "C" INormalPipe* createInstance_INormalSvPipe(MUINT32 SensorIndex, char const* szCallerName);
...
public:
    ...
    static E_CamIQLevel     IQ_LV_TBL[IQSenCombi][IQSenSum];

    static MBOOL            queryFunc(
                                vector<QueryInData_t> const vInData,
                                vector<QueryOutData_t> &vOutData
                            );
    static MBOOL            resAlloc(
                                vector<QueryInResAlloc_t> const vInData,
...
public:     ////                    Operations.
    ...
    virtual MBOOL                   queryLMVSupported(
                                        E_INPUT inputTG,
                                        MBOOL &bSupportHWLMV,
                                        MBOOL &bSupportVirtualLMV
                                    ) const override;
};
};
...
#if 1
        case 1:{
            // Pipeline Selection: 0x0001'Normalpipe(default) 0x0010'NormalSvPipe
            if(pipeSelect == EPipeSelect_Normal){
                *ppInstance = static_cast<INormalPipe*>(createInstance_INormalPipe(sensorIndex, szCallerName));
            }
            else if(pipeSelect == EPipeSelect_NormalSv){
                *ppInstance = static_cast<INormalPipe*>(createInstance_INormalSvPipe(sensorIndex, szCallerName));
            }
            else{
                MY_LOGE("[%s:%d] Unsupported pipeline selection:%d", szCallerName, sensorIndex, pipeSelect);
                return BAD_VALUE;
            }
            break;
        }
...
E_CamIQLevel IQ_CAPABILITY::IQ_LV_TBL[IQSenCombi][IQSenSum] = {
    {eCamIQ_H, eCamIQ_H},
    {eCamIQ_L, eCamIQ_H},
    {eCamIQ_H, eCamIQ_L},
    {eCamIQ_L, eCamIQ_L}
...
inline E_INPUT CAP_MAP_TG(MUINT32 const CAM)
{
    E_INPUT CamTG = TG_CAM_MAX;

    switch(CAM) {
        case CAM_A: CamTG = TG_A;
            break;
        case CAM_B: CamTG = TG_B;
            break;
        case CAM_C: CamTG = TG_C;
            break;
        default:
            MY_LOGE("Not supported CAM(%d)\n", CAM);
            break;
    }
    return CamTG;
}
...
)
{
    ...
    switch(scenarioId) {
        ...
        case SENSOR_SCENARIO_ID_CUSTOM1:
            x = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom1Width;
            y = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom1Height;
            break;
        case SENSOR_SCENARIO_ID_CUSTOM2:
            x = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom2Width;
            y = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom2Height;
            break;
        case SENSOR_SCENARIO_ID_CUSTOM3:
            x = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom3Width;
            y = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom3Height;
            break;
        case SENSOR_SCENARIO_ID_CUSTOM4:
            x = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom4Width;
            y = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom4Height;
            break;
        case SENSOR_SCENARIO_ID_CUSTOM5:
            x = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom5Width;
            y = IQ_CAPABILITY::m_pAllSensorInfo->mSenInfo[sensorIdx].mSInfo.SensorCustom5Height;
            break;
        default:
            x = 0;
            y = 0;
...
)
{
    ...
    if ((len == 0) || (len >= IOPIPE_MAX_USER_NAME_LEN)) {
        MY_LOGE("Invalid user name length: %d", len);
        return MFALSE;
    }
    ...
    if (IQ_CAPABILITY::m_pAllSensorInfo == NULL) {
        ...
        if (TS_FakeSensorList::getFakeMode()) {
...
)
{
    ...
    MY_LOGI("+, Meter_In.offTwin:%d", Meter_In.offTwin);
    IQunit.IQlv = eCamIQ_H;
    Meter_Out.slave_module.clear();

    if(pResMgr->Res_Meter(Meter_In, Meter_Out)){
        IQunit.clklv = Meter_Out.clk_level;
        IQunit.isTwin = Meter_Out.isTwin;
        IQunit.result = MTRUE;
        IQunit.camResConfig.Bits.targetTG = CAP_MAP_TG(pResMgr->Res_GetModule());
        IQunit.camResConfig.Bits.isOffTwin = (IQunit.isTwin == MTRUE)? MFALSE : MTRUE;
    }
    else{
...
{
    MBOOL ret = MTRUE;
    MINT32 SensorSum = 0, Sen_PixMode, Sen_PixRate = 0, i = 0, j = 0;
    ...
    if((SensorSum == 0) || (SensorSum > 2)){
        MY_LOGE("sensor sum is not support:%d", SensorSum);
        return MFALSE;
    }
    if(vOutData.size() > 0){
    ...
    if(IQ_CAPABILITY::m_pAllSensorInfo == NULL) {
        ...
        if(m_pAllSensorInfo->mSList->queryNumberOfSensors() <= 0){
    ...
    Res_Arbitor_In src_arb[SensorSum];
    Res_Arbitor_Out dst_arb[SensorSum];
    memset(dst_arb, 0, sizeof(dst_arb));
    Res_Meter_IN src_meter[SensorSum];
    Res_Meter_OUT dst_meter[SensorSum];
    ...
    for (j = 0; j < SensorSum; j++) {
        ...
        // resmgr arbiter
        CAP_GET_TG_GRAB(vInData.at(j).sensorIdx, vInData.at(j).scenarioId, src_arb[j].img_crop_w, src_arb[j].img_crop_h);
        src_arb[j].raw_fps     = vInData.at(j).minCamProcessedFps;
        src_arb[j].tg_fps      = CAP_GET_SENSOR_FPS(vInData.at(j).sensorIdx, vInData.at(j).scenarioId);

        m_pHalSensor[j]->sendCommand(m_pAllSensorInfo->mSenInfo[vInData.at(j).sensorIdx].mDevId, SENSOR_CMD_GET_SENSOR_PIXELMODE,
                                (MUINTPTR)&vInData.at(j).sensorIdx, (MUINTPTR)&src_arb[j].tg_fps , (MUINTPTR)&Sen_PixMode);
        m_pHalSensor[j]->sendCommand(m_pAllSensorInfo->mSenInfo[vInData.at(j).sensorIdx].mDevId, SENSOR_CMD_GET_SENINF_PIXEL_RATE,
                                (MUINTPTR)&vInData.at(j).scenarioId, (MUINTPTR)&Sen_PixRate, (MUINTPTR)&vInData.at(j).scenarioId);

        // use mipiPixRate if available, otherwise use 1000000
        if(Sen_PixRate){
            src_arb[j].mipi_rate = Sen_PixRate;
        }
        src_arb[j].raw_out_w   = vInData.at(j).rrz_out_w;
        src_arb[j].off_bin      = (vInData.at(j).IQlv == eCamIQ_H) ? MTRUE : MFALSE ;
        if (SensorSum == 2) {
            // Force twin off in ducam scenario. Need to remove this after applying D-switch feature.
            src_arb[j].off_twin     = MTRUE;
        } else
            src_arb[j].off_twin     = MFALSE;

        src_arb[j].pattern      = vInData.at(j).pattern;
        //clk
        src_arb[j].vRawClks.push_back(315 * 1000 * 1000);
        src_arb[j].vRawClks.push_back(416 * 1000 * 1000);
        src_arb[j].vRawClks.push_back(546 * 1000 * 1000);


        MY_LOGI("vInData: sensorIdx(%d) scenarioId(%d) mipi_rate(%d) img_crop_w(%d) img_crop_h(%d) tg_fps(%d) raw_fps(%d) raw_out_w(%d) off_bin(%d) off_twin(%d) pattern(%d)",
            vInData.at(j).sensorIdx, vInData.at(j).scenarioId, (MUINT32)src_arb[j].mipi_rate, src_arb[j].img_crop_w,
            (MUINT32)src_arb[j].img_crop_h, src_arb[j].tg_fps, (MUINT32)src_arb[j].raw_fps,
            (MUINT32)src_arb[j].raw_out_w, (MUINT32)src_arb[j].off_bin, (MUINT32)src_arb[j].off_twin,
            (MUINT32)src_arb[j].pattern);


        CAP_GET_TG_GRAB(vInData.at(j).sensorIdx, vInData.at(j).scenarioId, src_meter[j].img_crop_w, src_meter[j].img_crop_h);

        src_meter[j].bypass_tg   = MFALSE;
        src_meter[j].bypass_rrzo = MFALSE;
        src_meter[j].pix_mode_tg = CAP_MAP_PXLMODE(vInData.at(j).sensorIdx, Sen_PixMode);
        src_meter[j].raw_fps     = CAP_GET_SENSOR_FPS(vInData.at(j).sensorIdx, vInData.at(j).scenarioId);
        src_meter[j].SrcImgFmt   = CAP_GET_TG_HWFMT(vInData.at(j).sensorIdx);

        src_meter[j].rrz_out_w = vInData.at(j).rrz_out_w;
        src_meter[j].pattern = vInData.at(j).pattern;
        //clk
        src_meter[j].vClk.push_back(315 * 1000 * 1000);
        src_meter[j].vClk.push_back(416 * 1000 * 1000);
        src_meter[j].vClk.push_back(546 * 1000 * 1000);
        if (Sen_PixRate) {
            src_meter[j].MIPI_pixrate = src_arb[j].mipi_rate;
        }
        m_pHalSensor[j]->sendCommand(m_pAllSensorInfo->mSenInfo[vInData.at(j).sensorIdx].mDevId, SENSOR_CMD_GET_SENSOR_PIXELMODE,
                                (MUINTPTR)&vInData.at(j).sensorIdx, (MUINTPTR)&src_meter[j].raw_fps , (MUINTPTR)&Sen_PixMode);

        MY_LOGI("vInData: sensorIdx(%d) scenarioId(%d) pix_mode_tg(%d) tg_fps(%d) SrcImgFmt(%d) rrz_out_w(%d) pattern(%d)",
            vInData.at(j).sensorIdx, vInData.at(j).scenarioId, (MUINT32)src_meter[j].pix_mode_tg, src_meter[j].raw_fps,
            (MUINT32)src_meter[j].SrcImgFmt, src_meter[j].rrz_out_w, (MUINT32)src_meter[j].pattern);
    ...
    for(i = 0; i < SensorSum * 2; i++){
        ...
        // since ISP60 3-TG, need to 1st sensor attach->meter, 2nd sensor attach->meter
        for(j = 0; j < SensorSum; j++){
            IQunit[j].sensorIdx = vInData.at(j).sensorIdx;
            IQunit[j].IQlv = IQ_LV_TBL[i][j];
            IQunit[j].result = MFALSE;
            src_arb[j].off_bin  = (IQ_LV_TBL[i][j] == eCamIQ_H) ? MTRUE : MFALSE ;
            dst_arb[j].resmgr_cam = NULL;
            dst_arb[j].resmgr_sv = NULL;
            /*Res_AttachArbiter*/
            if (ResMgr::Res_AattachArbitor(src_arb[j], dst_arb[j], THIS_NAME)) {
                /*Res_Meter*/
                src_meter[j].offTwin = dst_arb[j].isOffTwin;
                IQ_TBL[i][j](dst_arb[j].resmgr_cam, src_meter[j], dst_meter[j], IQunit[j]);
                if(IQunit[j].result == MFALSE){
                    MY_LOGE("Query Fail: i:%d j:%d fps:%d fmt:%d rrz_out_w:%d pattern:%d", i, j, src_meter[j].raw_fps, src_meter[j].SrcImgFmt, src_meter[j].rrz_out_w, src_meter[j].pattern);
                }
            }
            else {
                MY_LOGE("Res_AattachArbitor fail: sensorIdx:%d", IQunit[j].sensorIdx);
            }
        ...
        /*Res_Detach*/
        for(j = 0; j < SensorSum; j++){
            if(dst_arb[j].resmgr_cam){
                dst_arb[j].resmgr_cam->Res_Detach();
            }
            if(dst_arb[j].resmgr_sv){
                dst_arb[j].resmgr_sv->Res_Detach();
            }
        }
    ...
    for(i = 0 ; i < vOutData.size() ; i++){
        for(j = 0 ; j < vOutData[i].size() ; j++){
            MY_LOGI("[%d][%d] sensorIdx:%d isTwin:%d lvIQ:%d lvClk:%d result:%d camResConfig.isOffTwin:%d targetTg:%d",
                i, j, vOutData[i][j].sensorIdx, vOutData[i][j].isTwin,
                vOutData[i][j].IQlv, vOutData[i][j].clklv,
                vOutData[i][j].result, vOutData[i][j].camResConfig.Bits.isOffTwin, vOutData[i][j].camResConfig.Bits.targetTG);
        }
    }
...
{
    MBOOL ret = MTRUE;
    MINT32 SensorSum = 0, Sen_PixMode, Sen_PixRate, i = 0, j = 0;
    char CallerName[32] = "NpipeIQContol";

    SensorSum = vInData.size();

    /*This API only for at most 2 sensor*/
    if((SensorSum == 0) || (SensorSum > 2)){
    ...
    Res_Arbitor_Out dst_arb[SensorSum];
    memset(dst_arb, 0, sizeof(dst_arb));
    Res_Meter_IN src_mter[SensorSum];
    Res_Meter_OUT dst_mter[SensorSum];
    QueryOutData_t FuncUnit[SensorSum];
    ISP_HW_MODULE curHw;
    ENPipe_CAM cam_path = ENPipe_CAM_MAX;
    ...
    for(j = 0; j < SensorSum; j++){
        ...
        if (ResMgr::Res_AattachArbitor(src_arb[j], dst_arb[j], THIS_NAME)) {
            ...
            MY_LOGE("Res_AattachArbitor fail ");
            return MFALSE;
        }
        /*Res_Meter*/

        // ResMgr meter param
        CAP_GET_TG_GRAB(vInData.at(j).sensorIdx, vInData.at(j).scenarioId, src_mter[j].img_crop_w, src_mter[j].img_crop_h);
        src_mter[j].img_crop_w              = src_arb[j].img_crop_w;
        src_mter[j].img_crop_h              = src_arb[j].img_crop_h;
        src_mter[j].bypass_tg               = (dst_arb[j].needFullFpsSVRawDump) ? MTRUE : MFALSE;
        src_mter[j].bypass_rrzo             = MFALSE;
        src_mter[j].pix_mode_tg             = (dst_arb[j].needFullFpsSVRawDump) ? ePixMode_NONE : CAP_MAP_PXLMODE(vInData.at(j).sensorIdx, Sen_PixMode);
        src_mter[j].raw_fps                 = (dst_arb[j].needFullFpsSVRawDump) ? src_arb[j].raw_fps : src_arb[j].tg_fps;
        src_mter[j].SrcImgFmt               = CAP_GET_TG_HWFMT(vInData.at(j).sensorIdx);
        src_mter[j].rrz_out_w               = src_arb[j].raw_out_w;
        src_mter[j].pattern                 = src_arb[j].pattern;
        src_mter[j].offBin                  = src_arb[j].off_bin;
        src_mter[j].offTwin                 = src_arb[j].off_twin;
        src_mter[j].MIPI_pixrate            = src_arb[j].mipi_rate;

        MY_LOGI("src meter[%d]: DCIF(%d) pixMode(%d) rawFps(%d)",
            j, dst_arb[j].needFullFpsSVRawDump, src_mter[j].pix_mode_tg, src_mter[j].raw_fps);

        src_mter[j].vClk.push_back(315 * 1000 * 1000);
        src_mter[j].vClk.push_back(416 * 1000 * 1000);
        src_mter[j].vClk.push_back(546 * 1000 * 1000);

        if (dst_arb[j].resmgr_cam->Res_Meter(src_mter[j], dst_mter[j])) {
            FuncUnit[j].sensorIdx = vInData.at(j).sensorIdx;
            FuncUnit[j].clklv = dst_mter[j].clk_level;
            FuncUnit[j].isTwin = dst_mter[j].isTwin;
            FuncUnit[j].IQlv = vInData.at(j).IQlv;
            FuncUnit[j].result = MTRUE;
            curHw = dst_arb[j].resmgr_cam->Res_GetModule();

            switch(curHw) {
                case CAM_A: cam_path = ENPipe_CAM_A;
                    break;
                case CAM_B: cam_path = ENPipe_CAM_B;
                    break;
                case CAM_C: cam_path = ENPipe_CAM_C;
                    break;
            }
            ...
            if(pinfo){
                ...
                FuncUnit[j].function.Bits.D_TWIN = queryInfo.function.Bits.D_TWIN;
                FuncUnit[j].function.Bits.SensorNum = queryInfo.function.Bits.SensorNum;
                FuncUnit[j].function.Bits.D_BayerENC = queryInfo.function.Bits.D_BayerENC;
                FuncUnit[j].function.Bits.isFLK = queryInfo.function.Bits.isFLK;
                FuncUnit[j].function.Bits.isLMV = queryInfo.function.Bits.isLMV;
                FuncUnit[j].function.Bits.isRSS = queryInfo.function.Bits.isRSS;
                FuncUnit[j].function.Bits.isPDE = queryInfo.function.Bits.isPDE;
                FuncUnit[j].function.Bits.isFull_dir_YUV = queryInfo.function.Bits.isFull_dir_YUV;
                FuncUnit[j].function.Bits.isScaled_Y = queryInfo.function.Bits.isScaled_Y;
                FuncUnit[j].function.Bits.isScaled_YUV = queryInfo.function.Bits.isScaled_YUV;
                FuncUnit[j].function.Bits.isG_Scaled_YUV = queryInfo.function.Bits.isG_Scaled_YUV;
                pinfo->DestroyInstance(LOG_TAG);
            }
            else{
                MY_LOGE("pinfo must not be null\n");
                ret = MFALSE;
            }
        }
        else{
            MY_LOGE("Res_Meter is fail");
            FuncUnit[j].result = MFALSE;
            return MFALSE;
        }
        vOutData.push_back(FuncUnit[j]);
    }
...
)
{
    ...
    Res_Meter_IN src_mter[SensorSum];
    Res_Meter_OUT dst_mter[SensorSum];
    QueryOutResAlloc_t FuncUnit[SensorSum];
    ISP_HW_MODULE curHw;
    ENPipe_CAM cam_path;
    ...
    for (j = 0; j < SensorSum; j++) {
        ...
        src_arb[j].mipi_rate   = Sen_PixRate;
        src_arb[j].raw_out_w   = vInData.at(j).maxCamProcessedOutW;
        src_arb[j].off_bin      = (vInData.at(j).IQlv == eCamIQ_H) ? MTRUE : MFALSE ;
        src_arb[j].off_twin     = MFALSE ;
        src_arb[j].pattern      = vInData.at(j).pattern;
        //clk
    ...
    for(j = 0; j < SensorSum; j++){
        ...
        if (ResMgr::Res_AattachArbitor(src_arb[j], dst_arb[j], THIS_NAME)) {
            FuncUnit[j].sensorIdx = vInData.at(j).sensorIdx;
            FuncUnit[j].clklv = dst_arb[j].clk_level;
            FuncUnit[j].needFullFpsSVRawDump = dst_arb[j].needFullFpsSVRawDump;
            FuncUnit[j].pipeSel = EPipeSelect_Normal;
        }
        ...
        src_mter[j].img_crop_w              = src_arb[j].img_crop_w;
        src_mter[j].img_crop_h              = src_arb[j].img_crop_h;
        src_mter[j].bypass_tg               = (FuncUnit[j].needFullFpsSVRawDump) ? MTRUE : MFALSE;
        src_mter[j].bypass_rrzo             = MFALSE;
        src_mter[j].pix_mode_tg             = (FuncUnit[j].needFullFpsSVRawDump) ? ePixMode_NONE : CAP_MAP_PXLMODE(vInData.at(j).sensorIdx, Sen_PixMode);
        src_mter[j].raw_fps                 = (FuncUnit[j].needFullFpsSVRawDump) ? src_arb[j].raw_fps : src_arb[j].tg_fps;
        src_mter[j].SrcImgFmt               = CAP_GET_TG_HWFMT(vInData.at(j).sensorIdx);
        src_mter[j].rrz_out_w               = src_arb[j].raw_out_w;
        src_mter[j].pattern                 = src_arb[j].pattern;
        src_mter[j].offBin                  = src_arb[j].off_bin;
        ...
        MY_LOGI("src meter[%d]: DCIF(%d) pixMode(%d) rawFps(%d)",
            j, FuncUnit[j].needFullFpsSVRawDump, src_mter[j].pix_mode_tg, src_mter[j].raw_fps);
...
)   const
{
    ...
    switch(eCmd){
        ...
        case ENPipeQueryCmd_D_BayerEnc:
            ...
            break;
        case ENPipeQueryCmd_NEED_CFG_SENSOR_BY_MW:
            {
                sCAM_QUERY_NEED_CONFIG_SENSOR_BY_MW *pQueryStruct = (sCAM_QUERY_NEED_CONFIG_SENSOR_BY_MW*)pIO_struct;
                CAM_CAPIBILITY* pinfo = CAM_CAPIBILITY::CreateInsatnce(LOG_TAG);
                ...
                pQueryStruct->QueryOutput = queryInfo.function.Bits.needCfgSensorByMW;
                pinfo->DestroyInstance(LOG_TAG);
            }
            break;
        case ENPipeQueryCmd_IQ_LEVEL:
            {
                ...
                pQueryStruct->QueryOutput = queryInfo.IQlv; // support iq or not
                pinfo->DestroyInstance(LOG_TAG);
            ...
            break;
        case ENPipeQueryCmd_FUNC:
            {
                sCAM_QUERY_FUNC *pQueryStruct = (sCAM_QUERY_FUNC*)pIO_struct;
                pQueryStruct->QueryOutput = MTRUE;
                IQ_CAPABILITY* pFuncInfo = IQ_CAPABILITY::CreateInsatnce(LOG_TAG);
                if(pFuncInfo->queryFunc(pQueryStruct->QueryInput.vInData, pQueryStruct->QueryInput.vOutData) == MFALSE){
                    MY_LOGE("queryFunc fail\n");
                    return MFALSE;
                }
                pQueryStruct->QueryOutput = MTRUE;
                pFuncInfo->DestroyInstance(LOG_TAG);
            }
            break;
        case ENPipeQueryCmd_HW_RES_ALLOC:
            {
                sCAM_QUERY_HW_RES_ALLOC *pQueryStruct = (sCAM_QUERY_HW_RES_ALLOC*)pIO_struct;
                // clear output data vector
                pQueryStruct->QueryParam.vOutData.clear();
                IQ_CAPABILITY* pFuncInfo = IQ_CAPABILITY::CreateInsatnce(LOG_TAG);
                if(pFuncInfo->resAlloc(pQueryStruct->QueryParam.vInData, pQueryStruct->QueryParam.vOutData) == MFALSE){
                    MY_LOGE("queryFunc fail\n");
                    return MFALSE;
                }
                pFuncInfo->DestroyInstance(LOG_TAG);
                pQueryStruct->result = MTRUE;
            }
            break;
        case ENPipeQueryCmd_TUNING_FREQ:
            {
                sCAM_QUERY_TUNING_FREQ *pQueryStruct = (sCAM_QUERY_TUNING_FREQ*)pIO_struct;
                pQueryStruct->tuningFreq = TUNING_FREQ_ALIGN_ENQ_REQ;
            }
            break;
        ...
        case ENPipeQueryCmd_MAX_PREVIEW_SIZE:
            {
                ...
                pQueryStruct->QueryOutput = 6144; //isp6.0:max rrzo width 6144
            }
            break;
        ...
        case ENPipeQueryCmd_PDO_AVAILABLE:
            {
                ...
                pQueryStruct->QueryOutput = MTRUE; //~isp3.0:pdo not supported, isp4.0/6.0~:pdo supported
            }
            break;
...
)   const
{
    ...
    switch (inputTG)
    {
        ...
        case TG_B:
            ...
            break;
        case TG_C:
            bSupportHWLMV = MFALSE;
            bSupportVirtualLMV = MTRUE;
            break;
        default:
            MY_LOGW("unknown inputTG(%d)\n", inputTG);
            ret = MFALSE;
    }


    MY_LOGI("inputTG[%d]: bSupportHWLMV=%d; bSupportVirtualLMV=%d\n",
        inputTG, bSupportHWLMV, bSupportVirtualLMV);
