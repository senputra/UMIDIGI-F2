...
{
    mLogLevel = ::property_get_int32("vendor.debug.camera.log.simager", 0);
...
)
{
    ...
    {
        MY_LOGE("src(%p), dst(%p)", mpSrcImgBuf, mpDstImgBuf);
        return MFALSE;
    }
    //
    ...
    MY_LOGD_IF(mLogLevel, "src(%p) dst(%p)(fmt, width, height, trans)(0x%x, %d, %d, 0x%x) roi(x, y, w, h) = (%d, %d, %d, %d)",
             mpSrcImgBuf, mpDstImgBuf,
             mpDstImgBuf->getImgFormat(), mpDstImgBuf->getImgSize().w, mpDstImgBuf->getImgSize().h, mu4Transform,
             mCropRect.p.x, mCropRect.p.y, mCropRect.s.w, mCropRect.s.h
             );
    if (eImgFmt_JPEG == mpDstImgBuf->getImgFormat() )
    {
        ret = encode(mpSrcImgBuf, mpDstImgBuf, mCropRect, mu4Transform,
                    mu4Quality, mu4IsSOI, meCodecType, u4TimeoutMs, msrcBufNeedFlush);
    }
    else
    {
        ret = imgTransform(mpSrcImgBuf, mpDstImgBuf, mCropRect, mu4Transform, u4TimeoutMs, endTime);
    }
...
setTargetImgBuffer(IImageBuffer const * pImgBuffer)
{
    ...
    MY_LOGD_IF(mLogLevel, "dst imgbuf(%p)", pImgBuffer);
...
{
    MY_LOGD_IF(mLogLevel, "Ori Transform = 0x%x, Target Transfrom = 0x%x", mu4Transform, u4Transform);
...
{
    MY_LOGD_IF(mLogLevel, "roi (x, y, w, h) = (%d, %d, %d, %d)", rCropRect.p.x, rCropRect.p.y, rCropRect.s.w, rCropRect.s.h);
...
{
    MY_LOGD_IF(mLogLevel, "enc param (SOI, Quality, type, flush) = (%d, %d, %d, %d)", u4IsSOI, u4Quality, eCodecType, srcBufNeedFlush);

    mu4IsSOI = u4IsSOI;
    mu4Quality = u4Quality;
    meCodecType = eCodecType;
    msrcBufNeedFlush = srcBufNeedFlush;
    //
    return MTRUE;
}
    ...
    )
{
    ...
    ret = pJpegCodec->encode(pSrcBufInfo, pDstBufInfo, rROI, u4Transform,
                             u4Quality, u4IsSOI, eCodecType, u4TimeoutMs, srcBufNeedFlush);
...
)
{
    ...
    MBOOL ret = MTRUE;
    //
    IImageTransform *pImgTransform = IImageTransform::createInstance("SImage", miSensorIdx);
    ret =  pImgTransform->execute(pSrcBufInfo, pDstBufInfo, NULL, rROI, u4Transform, u4TimeOutInMs, endTime);
