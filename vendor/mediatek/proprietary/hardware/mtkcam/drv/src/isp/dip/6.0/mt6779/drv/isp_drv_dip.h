...
class VirDipDrvImp : public VirDipDrv
{
    ...
    public:
        ...
        virtual MBOOL    init(const char* userName=""){(void)userName; return MTRUE;}
        virtual MBOOL    uninit(const char* userName=""){(void)userName; return MTRUE;}

        virtual MBOOL    waitIrq(DIP_WAIT_IRQ_ST* pWaitIrq){(void)pWaitIrq; return MTRUE;}
        virtual MBOOL    clearIrq(DIP_CLEAR_IRQ_ST* pClearIrq){(void)pClearIrq; return MTRUE;}
        virtual MBOOL    registerIrq(DIP_REGISTER_USERKEY_STRUCT* pRegIrq){(void)pRegIrq; return MTRUE;}
        virtual MBOOL    signalIrq(DIP_WAIT_IRQ_ST* pWaitIrq){(void)pWaitIrq; return MTRUE;}
        ...
        virtual MBOOL    setDeviceInfo(E_DEVICE_INFO eCmd,MUINT8* pData){(void)eCmd; (void)pData; return MTRUE;}
