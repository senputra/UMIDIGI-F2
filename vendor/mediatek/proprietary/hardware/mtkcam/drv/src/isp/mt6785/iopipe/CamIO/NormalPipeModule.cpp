...
)
{
    ...
    IQunit.targetTg = TG_CAM_MAX;
    Meter_Out.slave_module.clear();
    Meter_Out.isTwin = MFALSE;
    Meter_Out.bin_en = MFALSE;
    ...
    if (pResMgr->Res_Meter(Meter_In, Meter_Out)) {
        ...
        IQunit.result = MTRUE;
        IQunit.targetTg = CAP_MAP_TG(pResMgr->Res_GetModule());
        ...
        MY_LOGI("IQunit.isTwin:%d, IQunit.targetTg:%d\n", IQunit.isTwin, IQunit.targetTg);
    } else {
...
)
{
    ...
    if((SensorSum == 0) || (SensorSum > 2)){
        MY_LOGE("SensorSum(%d) is not support!", SensorSum);
        return MFALSE;
    }
    if(vOutData.size() > 0){
    ...
    for (j = 0; j < SensorSum; j++) {
        ...
        m_pHalSensor[j]->sendCommand(m_pAllSensorInfo->mSenInfo[vInData.at(j).sensorIdx].mDevId, SENSOR_CMD_GET_MIPI_PIXEL_RATE,
        ...
        src_arb[j].vRawClks.push_back(560 * 1000 * 1000);
        ...
        CAP_GET_TG_GRAB(vInData.at(j).sensorIdx, vInData.at(j).scenarioId, src_meter[j].tg_crop_w, src_meter[j].tg_crop_h);
        ...
        src_meter[j].tg_fps      = CAP_GET_SENSOR_FPS(vInData.at(j).sensorIdx, vInData.at(j).scenarioId);
        src_meter[j].SrcImgFmt   = CAP_GET_TG_HWFMT(vInData.at(j).sensorIdx);
        src_meter[j].SenNum      = (SensorSum == 1) ? ECamio_1_SEN : ECamio_2_SEN;
        ...
        src_meter[j].vClk.push_back(560 * 1000 * 1000);
        if (Sen_PixRate) {
            src_meter[j].MIPI_pixrate = src_arb[j].mipi_rate;
        }
        m_pHalSensor[j]->sendCommand(m_pAllSensorInfo->mSenInfo[vInData.at(j).sensorIdx].mDevId,
                                SENSOR_CMD_GET_SENSOR_PIXELMODE, (MUINTPTR)&vInData.at(j).sensorIdx,
                                (MUINTPTR)&src_meter[j].tg_fps , (MUINTPTR)&Sen_PixMode);

        MY_LOGI(
            "vInData: sensorIdx(%d) scenarioId(%d) pix_mode_tg(%d) tg_fps(%d) SrcImgFmt(%d) rrz_out_w(%d) pattern(%d)",
            vInData.at(j).sensorIdx, vInData.at(j).scenarioId, (MUINT32)src_meter[j].pix_mode_tg, src_meter[j].tg_fps,
    ...
    for (i = 0; i < SensorSum * 2; i++) {
        ...
        for (j = 0; j < SensorSum; j++) {
            ...
            MY_LOGD("query loop [%d][%d]", i, j);
            /*Res_AttachArbiter*/
            if (ResMgr::Res_AattachArbitor(j, SensorSum, src_arb, dst_arb, THIS_NAME)) {
                /*Res_Meter*/
                ...
                if (IQunit[j].result == MFALSE) {
                    MY_LOGE("Query Fail: i:%d j:%d fps:%d fmt:%d rrz_out_w:%d pattern:%d",
                        i, j, src_meter[j].tg_fps, src_meter[j].SrcImgFmt, src_meter[j].rrz_out_w,
                        src_meter[j].pattern);
                }
            } else {
    ...
    for (i = 0 ; i < vOutData.size() ; i++) {
        for (j = 0 ; j < vOutData[i].size() ; j++) {
            MY_LOGI("[%d][%d] sensorIdx:%d isTwin:%d lvIQ:%d lvClk:%d result:%d targetTg:%d",
                i, j, vOutData[i][j].sensorIdx, vOutData[i][j].isTwin, vOutData[i][j].IQlv,
                vOutData[i][j].clklv, vOutData[i][j].result, vOutData[i][j].targetTg);
        }
    }
