...
{
    MBOOL Result = MTRUE, wakelock = MTRUE;
...
{
    MBOOL Result = MTRUE, wakelock = MFALSE;
    MINT32 tmp=0;
...
MBOOL IspDrvImp::waitIrq(ISP_WAIT_IRQ_ST* pWaitIrq)
{
    ...
    do{
        Ta=dbgTmr.getUs();
        if((pWaitIrq->UserKey == this->usrkey_3a) && (pWaitIrq->Status == SOF_INT_ST))
            LOG_INF("User/Sts(0x%x/0x%x), Enter ioctl",pWaitIrq->UserKey,pWaitIrq->Status);
        Ret = ioctl(this->m_Fd,ISP_WAIT_IRQ,&wait);
        if((pWaitIrq->UserKey == this->usrkey_3a) && (pWaitIrq->Status == SOF_INT_ST))
            LOG_INF("User/Sts(0x%x/0x%x), Leave ioctl",pWaitIrq->UserKey,pWaitIrq->Status);
        Tb=dbgTmr.getUs();
...
MBOOL IspDrvImp::registerIrq(ISP_REGISTER_USERKEY_STRUCT* pRegIrq)
{
    ...
    if(!(strncmp(pRegIrq->userName,"VSIrq",5))){
        this->usrkey_3a = pRegIrq->userKey;
        LOG_INF("3A_userkey:%d(%s)",this->usrkey_3a,pRegIrq->userName);
    }
    return MTRUE;
}
