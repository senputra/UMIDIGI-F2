...
MBOOL UniDrvImp::init(const char* userName, IspDrvImp* pConnectedObj)
{
    ...
    if((this->m_HWmodule == UNI_A) && (this->pIspdrvimp->init("UNI_A_DRV") == MFALSE)){
        Result = MFALSE;
        goto EXIT;
    }

    if((this->m_HWmodule == UNI_B) && (this->pIspdrvimp->init("UNI_B_DRV") == MFALSE)){
        Result = MFALSE;
        goto EXIT;
    }
...
MBOOL UniDrvImp::uninit(const char* userName)
{
    ...
    //
    if((this->m_HWmodule == UNI_A) && (this->pIspdrvimp->uninit("UNI_A_DRV") == MFALSE)){
        Result = MFALSE;
        goto EXIT;
    }

    if((this->m_HWmodule == UNI_B) && (this->pIspdrvimp->uninit("UNI_B_DRV") == MFALSE)){
        Result = MFALSE;
        goto EXIT;
    }
...
//Only Use for DumpReg
MUINT32 UniDrvImp::Dump_readReg(MUINT32 Addr,MINT32 caller)
{
    MINT32 Ret=0;
    MUINT32 value=0x0;
    LOG_DBG("Isp_read:0x%x_0x%x",this->m_HWmodule,Addr);
    ...
    for(MUINT32 i=0x0;i<UNI_BASE_RANGE;i+=0x20){
        ...
        for(MUINT32 j=i;j<(i+0x20);j+=0x4){
            if(j >= UNI_BASE_RANGE)
                break;
            _tmpchr[0] = '\0';
            snprintf(_tmpchr,sizeof(_tmpchr),"0x%8x - ",this->Dump_readReg(j));
