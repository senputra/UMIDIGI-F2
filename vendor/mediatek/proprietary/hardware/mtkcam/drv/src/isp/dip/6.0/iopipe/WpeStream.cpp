...
wpeGlobalListCtrl(WpePackage wpeframepackage, MUINT32 framenumber, MUINT32 IsFirstFrame, MINT32& GlobalListIndex)
{
    ...
    if(wpeframepackage.wpeBurstIdx == 0 && IsFirstFrame == 0)
    {
        ...
        bufNode.idxofwaitDQFrmPackList = bufPackage.idxofwaitDQFrmPackList;
        mLWPEwaitDQFrameUnitList.push_back(bufNode);

        LOG_INF("EQ GlobalListCtrl,WPE FramePackList_size(%ld), Caller(0x%x), GlobalListIndex(%d), idx_DQFrmPackList(%d), Engine ID(%d), Burstidx_Dupidx(%d/%d), FrameNumber(%d)",
                (unsigned long)mLWPEDQFramePackList.size(), bufNode.callerID, GlobalListIndex, bufPackage.idxofwaitDQFrmPackList, bufNode.wpeModeIdx, bufNode.wpeBurstIdx, bufNode.wpeDupIdx, framenumber);
    ...
    else
    {
        ...
        if (mLWPEDQFramePackList.size()<=0)
        {
        ...
        list<WPEFramePackage>::iterator it_pack = mLWPEDQFramePackList.end();
        it_pack--;

        if((wpeframepackage.wpeDupIdx == (*it_pack).DupIdx) && (wpeframepackage.callerID == (*it_pack).callerID))
        {
                LOG_DBG("wpeDupIdx/callerID(%d/%d), FramePackList.size(%ld)", (*it_pack).DupIdx, (*it_pack).callerID, (unsigned long)mLWPEDQFramePackList.size());
        }
        else
        {
            LOG_ERR("ERR EnQ wpeDupIdx/callerID(%d/%d), PackLis twpeDupIdx/callerID(%d/%d) FramePackList.size(%ld)",
                wpeframepackage.wpeDupIdx, wpeframepackage.callerID, (*it_pack).DupIdx, (*it_pack).callerID, (unsigned long)mLWPEDQFramePackList.size());
            releaseWPELock(EWPELockEnum_wDQFramePackList);
            return MFALSE;
        }
        ...
        bufNode.wpeDupCmdIdx = wpeframepackage.m_wpeDupCmdIdx;
        bufNode.callerID=wpeframepackage.callerID;
        bufNode.idxofwaitDQFrmPackList = mLWPEDQFramePackList.size() -1;
        LOG_INF("EQ GlobalListCtrl,WPE FramePackList_size(%d), Caller(0x%x), WPE Engine ID(%d), Burstidx_Dupidx(%d/%d), GlobalListIndex (%d), idxofwaitDQFrmPackList(%d)",
                                  (unsigned long)mLWPEDQFramePackList.size(), bufNode.callerID, bufNode.wpeModeIdx, bufNode.wpeBurstIdx,bufNode.wpeDupIdx, GlobalListIndex, bufNode.idxofwaitDQFrmPackList);
        mLWPEwaitDQFrameUnitList.push_back(bufNode);
...
WPEenque(QParams const& rParams, MUINT32 callerID)
{
    ...
    if(framenumber > MAX_SUPPORT_WPE_FRAME_NUM)
    {
        LOG_ERR("WPE Enqueue Total Frame Number(%d); Driver support(%d)", framenumber, MAX_SUPPORT_WPE_FRAME_NUM);
        return ret;
    }

    while(checkWPEEnqueuedUserNum())
    {
        LOG_INF("Block WPE Enqueue Here, Max WPE Enqueue user (%d)", MAX_WPE_ENQUEUE_USER);
    ...
    for(MUINT32 q=0;q < rParams.mvFrameParams.size();q++)
    {
        ...
        LOG_INF("m_curEQUserundex(%d), Burst(%d), m_wpeDupCmdIdx(%d), GlobalListIndex(%d)", m_curEQUserundex, q, wpepackage.m_wpeDupCmdIdx, GlobalListIndex);
