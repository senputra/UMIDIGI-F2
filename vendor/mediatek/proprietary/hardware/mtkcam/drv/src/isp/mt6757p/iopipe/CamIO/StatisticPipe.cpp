 ...
 *
 ******************************************************************************/
void StatisticPipe::suspendCB(void *cookie, MBOOL rst)
{
    StatisticPipe*  _this = reinterpret_cast<StatisticPipe*>(cookie);
    _this->mpSttIOPipe->rstBufCtrl(rst);
    return;
}
...
MBOOL StatisticPipe::deque(PortID port, QBufInfo& rQBuf, ESTT_CACHE_SYNC_POLICY cacheSyncPolicy, MUINT32 u4TimeoutMs)
{
    ...
    if (nDeqRet != NSImageio::NSIspio::ICamIOPipe::ST_SUCCESS) {
        if(nDeqRet == NSImageio::NSIspio::ICamIOPipe::ST_STOP) {
            PIPE_DBG("TG%d deque port_%d:already stopped\n", mTgInfo, portID.index);
            rQBuf.mvOut.resize(0);
            mThreadIdleLock[_port].unlock();
            return MTRUE;
        } else if( nDeqRet == NSImageio::NSIspio::ICamIOPipe::ST_SUSPEND){
            PIPE_WRN("warning:TG%d is suspending, port_%d deque fail", mTgInfo, portID.index);
        } else {
    ...
    for (idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++) {
        ...
        result.mMagicNum_tuning = rQTSBufInfo.vBufInfo.at(idx).m_num_tuning;
        ...
        pbuf->mMetaData.mPrivateDataSize = rQTSBufInfo.vBufInfo.at(idx).m_pPrivateSize;
        ...
        curLeng  = snprintf(tmp,127,\
            "[%02d:%02d:%02d.%06d] SttD:%d: dma:x%x pa(0x%x) size(%d) mag(0x%x_0x%x) stride(%d) type(%x)",\
            Timehour, TimeMin, TimeSec, TimeUs,\
            mpSensorIdx,portID.index, pbuf->mPa, pbuf->mSize, result.mMagicNum_tuning, result.mMagicNum_hal,pbuf->mStride,result.mRawType);
        ...
        PIPE_DBG("SttD:%d: dma:x%x pa(0x%x) size(%d) mag(0x%x_0x%x) stride(%d) type(%x)\n", mpSensorIdx,\
                portID.index, pbuf->mPa, pbuf->mSize, result.mMagicNum_tuning, result.mMagicNum_hal,pbuf->mStride,result.mRawType);
...
MBOOL StatisticPipe::configPipe_bh()
{
    ...
    NSImageio::NSIspio::PortInfo    aao, afo, flko, pdo, pso,*_port = NULL;
...
MBOOL StatisticPipe::configPipe(QInitStatParam const& vInPorts, MINT32 burstQnum)
{
    ...
    PIPE_INF("+ SenIdx[%d],fastAf(%d)", mpSensorIdx, vInPorts.m_StatFunc.Bits.FAST_AF);
    ...
    if (mConfigDone) {
     ...
     do {
        ...
        if (MFALSE ==  mpNormalPipe->sendCommand(ENPipeCmd_SET_STT_SUSPEND_CB, (MINTPTR)suspendCB, (MINTPTR)this, 0)) {
            PIPE_ERR("SET Suspend CBFP fail");
            return MFALSE;
        }
         //mCfgLock.lock();
        ...
        //fast af
        func.Bits.FAST_AF = vInPorts.m_StatFunc.Bits.FAST_AF;
        if(func.Bits.FAST_AF)
            mpNormalPipe->sendCommand(ENPipeCmd_SET_FAST_AF, 0, 0, 0);
