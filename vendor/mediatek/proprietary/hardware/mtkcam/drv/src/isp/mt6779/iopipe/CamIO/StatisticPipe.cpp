...
#define D_CHG_SEN_CNT      (2)
#define VIR_PORT_PA        (0x87654321)
...
Mutex                           StatisticPipe::SIOGLock;
StatisticPipe*                  StatisticPipe::pStatisticPipe[EPIPE_Sensor_RSVD] = {NULL, NULL, NULL, NULL, NULL, NULL};
StatisticPipe::_SPipeMemInfo_t  StatisticPipe::mMemInfo = {.SPipeAllocMemSum = 0, .SPipeFreedMemSum = 0};
vector<IMEM_BUF_INFO>           StatisticPipe::mpMemInfo[EPIPE_Sensor_RSVD][StatisticPipe::SttPortID_MAX];
QueueMgr<IMEM_BUF_INFO>         StatisticPipe::mpImgHeaderMgr[EPIPE_Sensor_RSVD][StatisticPipe::SttPortID_MAX];


Mutex                           BufCtrlCenter::BfcSGlock;
Mutex                           BufCtrlCenter::BfcReorderlock[IQSenSum][ePort_max];
BufCtrlCenter::SLinkInfo        BufCtrlCenter::mSLinkInfo[IQSenSum];
MUINT32                         BufCtrlCenter::mBfcSSwitchCnt;
MUINT32                         BufCtrlCenter::mSLinkCfgNum;
MSize                           BufCtrlCenter::mSLinkTgSize;

const EPortIndex m_Slot2PortIndex[StatisticPipe::SttPortID_MAX] = {
    EPortIndex_AAO, EPortIndex_AFO, EPortIndex_FLKO, EPortIndex_PDO, EPortIndex_TSFSO
};
...
inline MUINT32 _PortMap(MUINT32 const PortIdx)
{
    ...
    else if(PortIdx == PORT_TSFSO.index)
        _idx = StatisticPipe::eTSFSO;
...
MBOOL StatisticPipe::getSTTFunc(NormalPipe_QueryInfo* queryInfo)
{
    MUINT32 ret = MTRUE;
    ENPipe_CAM            cam_path;
    NormalPipe_InputInfo  qryInput;
    ret = mpNormalPipe->sendCommand(ENPipeCmd_GET_CUR_RAW, (MINTPTR)&cam_path, 0, 0);
    CAM_CAPIBILITY* pinfo = CAM_CAPIBILITY::CreateInsatnce(LOG_TAG, cam_path);
    if(pinfo){
        if(pinfo->GetCapibility(0, ENPipeQueryCmd_FUNC, qryInput, *queryInfo) == MFALSE){
            PIPE_ERR("Query op func fail\n");
            ret = MFALSE;
        }
...
MBOOL StatisticPipe::switchSTTIOPipe()
{
    this->SIOGLock.lock();
    this->sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_SWITCH_SLINK, 0, 0, 0);
    this->SIOGLock.unlock();
    return MTRUE;
}
...
{
    MINT32 idx = 0;
    NSImageio::NSIspio::ICamIOPipe* pSttIO;
    ...
    if (mTotalUserCnt == 0)  {
        ...
        if (mpNormalPipe) {
            ...
            mpNormalPipe->sendCommand(ENPipeCmd_SET_STT_DSWITCH_CB, (MINTPTR)&notifyObj, 0, 0);
        ...
        if(mbEnDChgSrc) {
            SIOGLock.lock();
            pSttIO = GetSttIOPipeInstance(this->mpSensorIdx);
            SIOGLock.unlock();
        }
        else {
            pSttIO = mpSttIOPipe;
        ...
        if (mpIMem) {
            ...
            for (i = 0; i < StatisticPipe::SttPortID_MAX; i++) {
                if (mpMemInfo[mpSensorIdx][i].size()) {
                    for (j = 0; j < mpMemInfo[mpSensorIdx][i].size(); j++) {
                        if (mpMemInfo[mpSensorIdx][i].at(j).virtAddr) {
                            #if 0
                            ...
                            if (mpIMem->freeVirtBuf(&(mpMemInfo[i].at(j)), MFALSE) < 0) {
                                MUINT32 portIdx = _MapPort(i);
                            ...
                            #endif
                            mpMemInfo[mpSensorIdx][i].at(j).virtAddr = 0;
                            mpMemInfo[mpSensorIdx][i].at(j).phyAddr = 0;
                        }
                    }
                }
            }
    ...
    , mvInPorts(NULL)
{
    ...
    if (NULL == mpNormalPipe) {
        ...
        CAM_STATE_NOTIFY    notifyDSwitchObj = {.fpNotifyState = sttDSwitchCB, .Obj = this};
        ...
        mpNormalPipe->sendCommand(ENPipeCmd_SET_STT_DSWITCH_CB, (MINTPTR)&notifyDSwitchObj, 0, 0);
    }
}
...
MBOOL StatisticPipe::uninit()
{
    ...
    for (i = 0; i < StatisticPipe::SttPortID_MAX; i++) {
        ...
        mpImgHeaderMgr[mpSensorIdx][i].uninit();
    }
 ...
 *
 ******************************************************************************/
// Depart enqueToBuf alone, since in 3-raw switch scenario, multiple enqueToBuf is needed under re-order case
MBOOL StatisticPipe::enqueToBuf(QBufInfo const& rQBuf, QueueMgr<IMEM_BUF_INFO>* imghdr, NSImageio::NSIspio::ICamIOPipe* pSttIO, MUINT32 senIdx, MUINT32 _port)
{
    NSImageio::NSIspio::QBufInfo    rQBufInfo;
    NSImageio::NSIspio::PortID      portID;
    IMEM_BUF_INFO imBuf;
    MUINT32 i, _bsize = 0, ret = 0;
    ...
    for (i = 0; i < _bsize; i++) {
        ...
        if (!imghdr->sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_at, i, (MUINTPTR)&imBuf)) {
    ...
    rQBufInfo.u4SensorId = senIdx;
    if ((ret = pSttIO->enqueOutBuf(portID, rQBufInfo)) != 0) {
        if(ret > 0)
    ...
    for (i = 0; i < _bsize; i++) {
        imghdr->sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_pop, 0, 0);
    }

    for (i = 0; i < _bsize; i++) {
        PIPE_INF("SttE:%d: dma:x%x pa(x%" PRIXPTR "/x%" PRIXPTR ") fh_pa(x%" PRIXPTR ") memID(%x)", mpSensorIdx, portID.index,\
                rQBufInfo.vBufInfo[i].u4BufPA[ePlane_1st], rQBuf.mvOut.at(i).mPa, rQBufInfo.vBufInfo[i].Frame_Header.u4BufPA[ePlane_1st], rQBuf.mvOut.at(i).mMemID);
    }
    return MTRUE;
}
...
MBOOL StatisticPipe::enque(QBufInfo const& rQBuf)
{
    NSImageio::NSIspio::PortID portID;
    QBufInfo _qBuf;
    BufCtrlCenter::SLinkInfo* pslink;
    QueueMgr<IMEM_BUF_INFO>* imghdr;
    NSImageio::NSIspio::ICamIOPipe* pSttIO;

    MUINT32 _port = 0, memIndex = 0, _bsize = 0, BufDepth = 0, senIdx = 0;
    MINT32 ret = 0;
    E_FSM fsm = op_unknown;
    ...
    // Get Corresponding SttIO
    if (mbEnDChgSrc && ((this->m_FSM == op_start) || (this->m_FSM == op_suspend))) {
        if (VIR_PORT_PA == rQBuf.mvOut.at(0).mPa) {
            PIPE_DBG("Buf was from virtual port, should not enque!\n");
            return MTRUE;
        }

        SIOGLock.lock();
        if(MFALSE == sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_GET_IO_BY_PA, (MINTPTR)&_port, (MINTPTR)&rQBuf.mvOut.at(0).mPa, (MINTPTR)&pslink)) {
            SIOGLock.unlock();
            PIPE_ERR("Buf was not allocated: port_%d, PA(0x%08x)\n", portID.index, rQBuf.mvOut.at(0).mPa);
            return MFALSE;
        }
        pSttIO = pslink->mpSttIO;
        imghdr = pslink->mpImgHdrMgr + _port;
        senIdx = pslink->mpSenIdx;
        SIOGLock.unlock();
    }
    else {
        pSttIO = this->mpSttIOPipe;
        imghdr = mpImgHeaderMgr[mpSensorIdx] + _port;
        senIdx = mpSensorIdx;
    }
    ...
    // check if cam enque in sequnce under W+T due to HW timing
    // Sen1: a1, a2, c3, c4
    // Sen2: c1, c2, a3, a4
    // if Sen2 thread enque a3 earlier then Sen1 thread enque a2, mismatch happens
    // queue the enque record until correct sequence happens
    if (mbEnDChgSrc && ((this->m_FSM == op_start) || (this->m_FSM == op_suspend))) {
        BfcReorderlock[pslink->mLockIdx][_port].lock();
        if (rQBuf.mvOut.at(0).mPa != this->GetSttNextEnqPA(pslink, _port)) {
            PIPE_WRN("Enque un-order, port(%d), expected(0x%08x), get(0x%08x)\n", portID.index, this->GetSttNextEnqPA(pslink, _port), rQBuf.mvOut.at(0).mPa);
            // push buffer to un-order enque container
            pslink->mpUnOrderEnqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_push, (MUINTPTR)&rQBuf, 0);
            mpDeqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_pop, 0, 0);
            BfcReorderlock[pslink->mLockIdx][_port].unlock();
            return MTRUE;
        }
        BfcReorderlock[pslink->mLockIdx][_port].unlock();
    }

    if(enqueToBuf(rQBuf, imghdr, pSttIO, senIdx, _port)){
        if(mbEnDChgSrc && ((this->m_FSM == op_start) || (this->m_FSM == op_suspend))){
            this->AddSttEnqIdx(pslink, _port);
        }
    }
    else{
        PIPE_ERR("enqueTobuf fail");
        return MFALSE;
    }
    ...
    // w+t: switch timing enque un-order
    if (mbEnDChgSrc && ((this->m_FSM == op_start) || (this->m_FSM == op_suspend))) {
        BfcReorderlock[pslink->mLockIdx][_port].lock();
        PIPE_DBG("Next SttEnqIdx(%d)\n", pslink->mNextEnqIdx[_port]);
        //
        MUINT32 maxQueSize, curQueSize;
        pslink->mpUnOrderEnqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_getsize, (MUINTPTR)&maxQueSize, 0);
        for(MUINT32 i = 0 ; i < maxQueSize ; i++){
            pslink->mpUnOrderEnqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_getsize, (MUINTPTR)&curQueSize, 0);
            for(MUINT32 j = 0 ; j < curQueSize ; j++){
                pslink->mpUnOrderEnqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_at, j, (MUINTPTR)&_qBuf);
                if(_qBuf.mvOut.at(0).mPa == this->GetSttNextEnqPA(pslink, _port)){
                    if(enqueToBuf(_qBuf, imghdr, pSttIO, senIdx, _port)){
                        this->AddSttEnqIdx(pslink, _port);
                    }
                    else{
                        PIPE_ERR("enqueTobuf fail");
                    ...
                    pslink->mpUnOrderEnqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_pop, 0, 0);
                    break;
                }
            }
        }
...
{
    MUINT32         idx = 0, buf = 0, _port = 0, orderSize = 0;
    IMEM_BUF_INFO   imBuf;
    QBufInfo    _qBuf;
    ...
    NSImageio::NSIspio::ICamIOPipe*         pSttIO;
    BufCtrlCenter::SLinkInfo*               pslink;
    vector<IMEM_BUF_INFO>*                  pMemInf;
    QueueMgr<IMEM_BUF_INFO>*                pImgHdr;
    MUINTPTR                                nextPA;
    ...
    if(mbEnDChgSrc) {
        SIOGLock.lock();
        pSttIO  = GetSttIOPipeInstance(this->mpSensorIdx);
        this->sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_GET_CUR_SLINK, (MINTPTR)&pSttIO, (MINTPTR)&pslink, 0);
        pImgHdr = pslink->mpImgHdrMgr + _port;
        pMemInf = pslink->mpMInf + _port;
        SIOGLock.unlock();

        // Lock corresponding dma and sttio semaphore
        // To avoid switch phase2 racing causing two sensor use same instance to deque -> the later one wait done happen mismatch
        mDeqSTTLock[pslink->mLockIdx][_port].lock();

        // check whether next buffer enqued or not
        while(1){
            BfcReorderlock[pslink->mLockIdx][_port].lock();
            nextPA = this->GetSttNextDeqPA(pslink, _port);
            pslink->mpUnOrderEnqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_getsize, (MUINTPTR)&orderSize, 0);
            for(idx = 0 ; idx < orderSize ; idx++){
                pslink->mpUnOrderEnqContainer[_port]->sendCmd(QueueMgr<QBufInfo>::eCmd_at, idx, (MUINTPTR)&_qBuf);
                if(nextPA == _qBuf.mvOut.at(0).mPa) {
                    BfcReorderlock[pslink->mLockIdx][_port].unlock();
                    continue;
                }
            }
        ...
        pSttIO  = mpSttIOPipe;
        pMemInf = mpMemInfo[mpSensorIdx] + _port;
        pImgHdr = mpImgHeaderMgr[mpSensorIdx] + _port;
    }

    mThreadIdleLock[_port].lock();
    notifyObj.Obj = (MVOID*)&(mThreadIdleLock[_port]);

    switch(pSttIO->dequeOutBuf(portID, rQTSBufInfo, u4TimeoutMs, &notifyObj)){
        case eBuf_Fail:
            PIPE_ERR("error:TG%d port_%d deque fail", mTgInfo, portID.index);
            mThreadIdleLock[_port].unlock();
            if (mbEnDChgSrc)
                mDeqSTTLock[pslink->mLockIdx][_port].unlock();
            return MFALSE;
            break;
        case eBuf_stopped:
            ...
            break;
        case eBuf_dummy:
            PIPE_WRN("TG%d deque port_%d: is virtual port\n", mTgInfo, portID.index);
            rQBuf.mvOut.resize(1);
            rQBuf.mvOut[0].mSize = 0;
            rQBuf.mvOut[0].mPa = VIR_PORT_PA;
            mThreadIdleLock[_port].unlock();
            if (mbEnDChgSrc)
                mDeqSTTLock[pslink->mLockIdx][_port].unlock();
            return MTRUE;
    ...
    rQBuf.mSensorId = rQTSBufInfo.u4SensorId;
    ...
    for (idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++) {
        ...
        pbuf->mSize                  = rQTSBufInfo.vBufInfo.at(idx).u4BufSize[ePlane_1st]; //TBD or returns IMemSize: pMemInf->at(0).size
        ...
        PIPE_INF("SttD:%d_%d: dma:x%x pa(0x%" PRIXPTR ") size(%d) mag(0x%x) stride(%d) type(%x) IQlv(%d) memID(%x)\n", mpSensorIdx, rQBuf.mSensorId,\
                portID.index, pbuf->mPa, pbuf->mSize, result.mMagicNum_hal,pbuf->mStride,result.mRawType, (MUINT32)result.eIQlv, pbuf->mMemID);
        ...
        pSttIO->sendCommand(EPIPECmd_GET_HEADER_SIZE,(MINTPTR)portID.index,(MINTPTR)&imBuf.size,0);
        pImgHdr->sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_push, (MUINTPTR)&imBuf, 0);

        /* cache invalidate */
        for (buf = 0; buf < pMemInf->size(); buf++) {
            if (pbuf->mPa == pMemInf->at(buf).phyAddr) {
                ...
                case ESTT_CacheInvalidAll:
                    // ISP60 does not support ESTT_CacheInvalidAll due to  ION Cache flush optimization
                    // mpIMem->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID_ALL, &(pMemInf->at(buf)), MFALSE);
                ...
                default:
                    mpIMem->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID, &(pMemInf->at(buf)), MFALSE);
                    break;
                }
                break;
            }
    ...
    if (mbEnDChgSrc) {
        this->AddSttDeqIdx(pslink, _port);
        mDeqSTTLock[pslink->mLockIdx][_port].unlock();
    }
    PIPE_DBG("-");
...
MBOOL StatisticPipe::configPipe_bh()
{
    ...
    do {
        ...
        // if W+T used, STT need to wait another User to check larger tgSize for stat buffer allocation
        if (mbEnDChgSrc) {
            tgSize.w = imgW;
            tgSize.h = imgH;
            SIOGLock.lock();
            this->SetMaxTgSize(&tgSize);
            SIOGLock.unlock();
        ...
        if (MFALSE == getSTTFunc(&queryInfo))
            return MFALSE;
        ...
        for (MUINT32 headersize = 0,i = 0; i < mvInPorts->mStatPortInfo.size(); i++) {
            ...
            // skip virtual port
            if ((StatisticPipe::eFLKO == _portIdx) && (MFALSE == queryInfo.function.Bits.isFLK))
                continue;
            if ((StatisticPipe::ePDO == _portIdx) && (MFALSE == queryInfo.function.Bits.isPDE))
                continue;

            this->mpSttIOPipe->sendCommand(EPIPECmd_GET_HEADER_SIZE,(MINTPTR)mvInPorts->mStatPortInfo.at(i).mPortID.index,(MINTPTR)&headersize,0);
            mpImgHeaderMgr[mpSensorIdx][_portIdx].init();
            ...
            case EPortIndex_FLKO:
                ...
                break;
            case EPortIndex_TSFSO:
                mpSttIOPipe->sendCommand(EPIPECmd_GET_TSFSO_CONS,(MINTPTR)&dma_size,(MINTPTR)imgW,(MINTPTR)imgH);
                break;
            case EPortIndex_PDO:
            ...
            mpMemInfo[mpSensorIdx][_portIdx].resize(totalBufNum);
            ...
            for (j = 0; j < totalBufNum; j++) {
                ...
                mpImgHeaderMgr[mpSensorIdx][_portIdx].sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_push, (MUINTPTR)&iHeader, 0);

                pBuf = &(mpMemInfo[mpSensorIdx][_portIdx].at(j));
                pBuf->size = dma_size;
                ...
                slink.PA_L[_portIdx].push_back(pBuf->phyAddr);
            }
    ...
    for (i = 0; i < _portNum; i++) {
        ...
        staticEnqCount = mpMemInfo[mpSensorIdx][_portIdx].size();
        ...
        while (j < staticEnqCount) {
            for (_qi = 0; ((_qi < mBurstQNum) && (j < mpMemInfo[mpSensorIdx][_portIdx].size())); _qi++, j++) {
                rQBuf.mvOut.at(_qi).mPortID.index = _MapPort(_portIdx);
                rQBuf.mvOut.at(_qi).mVa = mpMemInfo[mpSensorIdx][_portIdx].at(j).virtAddr;
                rQBuf.mvOut.at(_qi).mPa = mpMemInfo[mpSensorIdx][_portIdx].at(j).phyAddr;
                rQBuf.mvOut.at(_qi).mSize = mpMemInfo[mpSensorIdx][_portIdx].at(j).size;
                rQBuf.mvOut.at(_qi).mMemID = mpMemInfo[mpSensorIdx][_portIdx].at(j).memID;
            }
            if (_qi < mBurstQNum) {
                PIPE_ERR("TG_%d port_%d burstQNum %d, total num %d", mTgInfo, rQBuf.mvOut.at(_qi).mPortID.index,\
                        mBurstQNum, (MUINT32)mpMemInfo[mpSensorIdx][_portIdx].size());
                return MFALSE;
            }
    ...
    // configure BufCtrlCenter if W+T DSwitch is on
    if (mbEnDChgSrc) {
        slink.mpSttIO       = mpSttIOPipe;
        slink.mpSenIdx      = mpSensorIdx;
        slink.mpImgHdrMgr   = mpImgHeaderMgr[mpSensorIdx];
        slink.mpMInf        = mpMemInfo[mpSensorIdx];
        sendcmdBufCtrl(BufCtrlCenter::BUF_CTRL_CMD_SET_SLINK, (MINTPTR)&slink, 0, 0);
    }
...
MBOOL StatisticPipe::configPipe(QInitStatParam const& vInPorts, MINT32 burstQnum)
{
    MBOOL       ret = MTRUE, cmdSent = 0;
    MUINT32     uni_available = 1, MainImgBQNum = 0, totalBufNum = 0, _portNum = 0;
    MUINT32     i = 0, j = 0, imgW = 0, imgH = 0, dma_size = 0;
    MUINTPTR    imgHdrAllocOfst = 0;
    vector<PortInfo const*>         vCamIOOutPorts, vCamIOInPorts;
    NSImageio::NSIspio::PortInfo    aao, afo, flko, pdo, tsfso, *_port = NULL, tgi;
    list<MUINTPTR>                  ImgPA_L[StatisticPipe::SttPortID_MAX];
    list<MUINTPTR>                  ImgHeaderPA_L[StatisticPipe::SttPortID_MAX];
    ...
    PIPE_INF("+ SenIdx[%d] bEnDChgSrc[%d]", mpSensorIdx, vInPorts.m_StatFunc.Bits.D_CHG_SRC);
...
#if 1
    mbEnDChgSrc = (MBOOL)vInPorts.m_StatFunc.Bits.D_CHG_SRC;
#else
    mbEnDChgSrc = MFALSE;
#endif
    ...
    for (i = 0; i < SenInfo.mSList->queryNumberOfSensors(); i++) {
     ...
     do {
        ...
        getSTTFunc(&queryInfo);

        for (i = 0; i < vInPorts.mStatPortInfo.size(); i++) {
            PIPE_INF("STT: DMA[%d] is configured", vInPorts.mStatPortInfo.at(i).mPortID.index);
            switch (vInPorts.mStatPortInfo.at(i).mPortID.index) {
                ...
                case EPortIndex_AFO:
                    ...
                    break;
                case EPortIndex_FLKO:
                    if (!queryInfo.function.Bits.isFLK) {
                        PIPE_INF("FLKO is virtual port under raw_c\n");
                        _port = NULL;
                        mOpenedPort |= StatisticPipe::_FLKO_VIR_ENABLE;
                    }
                    else {
                        _port = &flko;
                ...
                case EPortIndex_TSFSO:
                    ...
                    mOpenedPort |= StatisticPipe::_TSFSO_ENABLE;
                    break;
                case EPortIndex_PDO:
                    if (!queryInfo.function.Bits.isPDE) {
                        PIPE_INF("PDO is virtual port under raw_c\n");
                        _port = NULL;
                        mOpenedPort |= StatisticPipe::_PDO_VIR_ENABLE;
                    }
                    else if(vInPorts.mStatPortInfo.at(i).mWidth * vInPorts.mStatPortInfo.at(i).mHeight == 0){
        ...
        func.Bits.SRC_AF = vInPorts.m_StatFunc.Bits.CRP_R1_SEL;
        func.Bits.SRC_FLK = vInPorts.m_StatFunc.Bits.FLK_SEL;
...
MBOOL StatisticPipe::suspend()
{
    ...
    MUINT32 BufNum = 0, BufIdx = 0, PortIdx = 0;
    NSImageio::NSIspio::PortID portID;
    CAM_STATE_NOTIFY notifyObj = {.fpNotifyState = sttStateNotify};
    NSImageio::NSIspio::QTimeStampBufInfo   rQTSBufInfo;
    ...
    for(PortIdx = 0; PortIdx < PortNum; PortIdx++) {
        ...
        if(mbEnDChgSrc){
            pImgHdr = pslink->mpImgHdrMgr + _portIdx;
        }
        else{
            pImgHdr = mpImgHeaderMgr[mpSensorIdx] + _portIdx;
        }

        for(BufIdx = 0; BufIdx < BufNum; BufIdx++) {
            notifyObj.Obj = (MVOID*)&(mThreadIdleLock[_portIdx]);

            buf_status = pSttIO->dequeOutBuf(portID, rQTSBufInfo, u4TimeoutMs, &notifyObj);
            rQBuf.mvOut.resize(0);
            ...
            switch(buf_status){
                case eBuf_suspending:
                    ...
                    for (idx = 0; idx < mBurstQNum; idx++) {
                        ...
                        PIPE_INF("SttD:%d: dma:x%x pa(0x%" PRIXPTR ") size(%d) mag(0x%x) stride(%d) type(%x) IQlv(%d)\n", mpSensorIdx,\
    ...
    //
    pSttIO->suspend(ICamIOPipe::SW_SUSPEND);

    // reset EnqIdx/DeqIdx under 3-raw switch scenario, due to resume always enq/deq from 1st PA in MemInfo
    if (mbEnDChgSrc) {
        for(PortIdx = 0; PortIdx < PortNum; PortIdx++){
            MUINT32 _portIdx = 0;
            ...
            ResetSttEnqDeqIdx(pslink, _portIdx);
        }
    }
...
MBOOL StatisticPipe::resume()
{
    ...
    pSttIO->resume(ICamIOPipe::SW_SUSPEND);//reset CQ, start from 1st BA
    ...
    for (PortIdx = 0; PortIdx < PortNum; PortIdx++) {
        ...
        if(mbEnDChgSrc){
            pImgHdr = pslink->mpImgHdrMgr + _PortIdx;
            pMemInf = pslink->mpMInf + _PortIdx;
        }
        else{
            pImgHdr = mpImgHeaderMgr[mpSensorIdx] + _PortIdx;
            pMemInf = mpMemInfo[mpSensorIdx] + _PortIdx;
        }
        ...
        for(BufIdx = 0; BufIdx < DeqDepth; BufIdx++){
            ...
            if(_qBuf.mvOut.at(0).mPa != pMemInf->at(0).phyAddr){
                ...
                pImgHdr->sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_front, (MUINTPTR)&rQHdrRem, 0);
                pImgHdr->sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_pop, 0, 0);
                pImgHdr->sendCmd(QueueMgr<IMEM_BUF_INFO>::eCmd_push, (MUINTPTR)&rQHdrRem, 0);
            }
        }
        ...
        for(BufIdx = 0; BufIdx < DeqDepth; BufIdx++) {
            ...
            for(BurstQIdx = 0; BurstQIdx < mBurstQNum; BurstQIdx++){
                ...
                rQBuf[BufIdx].mvOut.at(BurstQIdx).mSize = _qBuf.mvOut.at(BurstQIdx).mSize;
                ...
                rQBuf[BufIdx].mvOut.at(BurstQIdx).mMemID = _qBuf.mvOut.at(BurstQIdx).mMemID;

                PIPE_INF("SttE:%d: dma:x%x pa(x%" PRIXPTR ") memID(%x)", mpSensorIdx, portID.index,\
                        rQBuf[BufIdx].mvOut.at(BurstQIdx).mPa, rQBuf[BufIdx].mvOut.at(BurstQIdx).mMemID);
            }
...
MBOOL StatisticPipe::sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    case ESPipeCmd_GET_STT_CUR_BUF:
        ...
        ret = pSttIO->sendCommand(EPIPECmd_GET_STT_CUR_BUF, (MUINT32)port.index, arg2, arg3);
        if (MFALSE == ret) {
            goto _EXIT_SEND_CMD;
        }
        _portidx = _PortMap(port.index);
        for (i = 0; i < mpMemInfo[mpSensorIdx][_portidx].size(); i++){
            if(*(MUINTPTR*)arg2 == mpMemInfo[mpSensorIdx][_portidx].at(i).phyAddr){
                *(MUINTPTR*)arg2 = mpMemInfo[mpSensorIdx][_portidx].at(i).virtAddr;
                break;
            }
        }
        if(i == mpMemInfo[mpSensorIdx][_portidx].size()) {
            ret = MFALSE;
...
MUINT32 StatisticPipe::GetOpenedPortInfo(StatisticPipe::ePortInfoCmd cmd, MUINT32 arg1)
{
    ...
    case StatisticPipe::_GetPort_OpendNum:
        ...
        if (mOpenedPort& StatisticPipe::_PDO_ENABLE)
            _num++;
        if (mOpenedPort& StatisticPipe::_TSFSO_ENABLE)
            _num++;
        return _num;
    case StatisticPipe::_GetPort_Index:
    {
        ...
        if (mOpenedPort & StatisticPipe:: _TSFSO_ENABLE) {
            PortIndex[idx] = PORT_TSFSO.index;
            idx++;
        }
...
MUINT32 StatisticPipe::sttDSwitchCB(CAM_STATE_OP state, MVOID *Obj)
{
    StatisticPipe *_this = (StatisticPipe*)Obj;
    _this->switchSTTIOPipe();
    return (MUINT32)NOTIFY_PASS;
}
