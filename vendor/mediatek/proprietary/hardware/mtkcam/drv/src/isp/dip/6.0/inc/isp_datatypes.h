...
public:
    ...
    int             pak_obit;   //input data bit for PAK in-front-of output dma (ex, 8, 10, ..., 16)
    EImageFormat    lIspColorfmt;  //SL TEST_MDP_YUV
    MUINT32         capbility;      //port capbility
    MUINT32         secureTag;
    IspDMACfg():
        pixel_byte(1),swap(0), format_en(0),format(0),bus_size_en(0),bus_size(0),memBuf_c_ofst(0),memBuf_v_ofst(0),v_flip_en(0),yuv_bit(0),yuv_dng(0),pak_obit(10),lIspColorfmt(eImgFmt_IMPLEMENTATION_DEFINED),capbility(0x00),secureTag(0)
        {};
};
