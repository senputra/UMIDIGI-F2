...
MINT32 CAM_TWIN_PIPE::_config( void)
{
    ...
    CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_CTL_MISC,DB_EN,0);
    ...
    if(CAM_READ_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_AAO_CTL1,FBC_EN) && this->fbc_AAO.Raw == 0xF7F7F7F7){
        this->fbc_AAO.Raw = CAM_READ_REG(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_AAO_CTL2);
    }
    if(CAM_READ_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_PSO_CTL1,FBC_EN) && this->fbc_PSO.Raw == 0xF7F7F7F7){
        this->fbc_PSO.Raw = CAM_READ_REG(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_PSO_CTL2);
    }
    if(CAM_READ_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_PDO_CTL1,FBC_EN) && this->fbc_PDO.Raw == 0xF7F7F7F7){
        this->fbc_PDO.Raw = CAM_READ_REG(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_PDO_CTL2);
    }
...
MINT32 CAM_TWIN_PIPE::_enable( void* pParam  )
{
    ...
    if(CAM_READ_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_AAO_CTL1,FBC_EN)){
        for (frm_cnt = 0; frm_cnt < this->fbc_AAO.Bits.FBC_CNT; frm_cnt++) {
            CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_CTL_FBC_RCNT_INC, AAO_RCNT_INC, 1);
        }
    }
    if(CAM_READ_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_PDO_CTL1,FBC_EN)){
        for (frm_cnt = 0; frm_cnt < this->fbc_PDO.Bits.FBC_CNT; frm_cnt++) {
            CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_CTL_FBC_RCNT_INC, PDO_A_RCNT_INC, 1);
        }
    }
    if(CAM_READ_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_FBC_PSO_CTL1,FBC_EN)){
        for (frm_cnt = 0; frm_cnt < this->fbc_PSO.Bits.FBC_CNT; frm_cnt++) {
            CAM_WRITE_BITS(this->m_pTwinIspDrv->getPhyObj(),CAM_CTL_FBC_RCNT_INC, PSO_RCNT_INC, 1);
        }
    }
...
MINT32 CAM_TWIN_PIPE::_disable(void* pParam)
{
    ...
    //Reset to special value.
    this->fbc_AAO.Raw = 0xF7F7F7F7;
    this->fbc_PSO.Raw = 0xF7F7F7F7;
    this->fbc_PDO.Raw = 0xF7F7F7F7;
...
MINT32 CAM_TWIN_PIPE::_write2CQ( void )
{
    ...
    this->m_pTwinIspDrv->cqAddModule(CAM_FBC_AAO_);
    this->m_pTwinIspDrv->cqAddModule(CAM_FBC_PSO_);
    this->m_pTwinIspDrv->cqAddModule(CAM_FBC_PDO_);
...
MINT32 CAM_TWIN_PIPE::update_cq(void)
{
    ...
    //
    _tmp3 = (this->m_hwModule == CAM_A)?(_STRUCT_OFST(cam_reg_t, CAM_CQ_THR0_BASEADDR)+0x4000):(_STRUCT_OFST(cam_reg_t, CAM_CQ_THR0_BASEADDR)+0x5000);
    ...
    for(int i = 0; i < CAM_CAM_MODULE_MAX; i++)
    {
        if(CAM_A == this->m_hwModule) {
            ...
            if((_tmp2>0x4000) && (_tmp2 != CQ_DUMMY_REG) && (_tmp2 != _tmp3)){//UNI r excluded
            ...
            _tmp2 = (tmp&CQ_DES_RANGE);
            //copy only non-dummy descriptor && do not copy cq's baseaddress, baseaddress is for master cam's link-list
            if((_tmp2>0x5000) && (_tmp2 != (CQ_DUMMY_REG+0x1000)) && (_tmp2 != _tmp3)){//UNI r excluded
                *(twin_IspDescript_vir + 2*i) = tmp - 0x1000;
                *(twin_IspDescript_vir + (2*i + 1)) = (MUINT32)((MUINT8*)(MUINTPTR)*(main_IspDescript_vir + (2*i + 1)) - (MUINT8*)main_IspVirReg_phy) + (MUINTPTR)twin_IspVirReg_phy;
            }
        }
...
#if 0
        CAM_FUNC_INF("0x%x(0x%x), 0x%x(0x%x)", twin_IspDescript_vir + 2*i, *(twin_IspDescript_vir + 2*i), \
...
MINT32 CAM_TWIN_PIPE::update_afterRunTwin()
{
    ...
    //Update FBC_CTRL of STT on slave cam based on twinIspDrv's DMA_EN
    if (CAM_READ_BITS(this->m_pTwinIspDrv, CAM_CTL_DMA_EN, AAO_EN) == 0) {
        CAM_WRITE_BITS(this->m_pTwinIspDrv,CAM_FBC_AAO_CTL1,FBC_EN,0);
    }
    if (CAM_READ_BITS(this->m_pTwinIspDrv, CAM_CTL_DMA_EN, PSO_EN) == 0) {
        CAM_WRITE_BITS(this->m_pTwinIspDrv,CAM_FBC_PSO_CTL1,FBC_EN,0);
    }
    if (CAM_READ_BITS(this->m_pTwinIspDrv, CAM_CTL_DMA_EN, PDO_EN) == 0) {
        CAM_WRITE_BITS(this->m_pTwinIspDrv,CAM_FBC_PDO_CTL1,FBC_EN,0);
    }
...
MINT32 CAM_TWIN_PIPE::update_afterRunTwin_Dma()
{
    ...
    if(CAM_READ_BITS(this->m_pTwinIspDrv,CAM_CTL_DMA_EN,IMGO_EN) ){
        ...
        if(CamInfo.m_DTwin.GetDTwin() == MFALSE)
            dma.module = this->m_twinHwModule;
        else{
            if(this->m_twinHwModule == CAM_A)
                dma.module = CAM_B_TWIN;
            else if(this->m_twinHwModule == CAM_B)
                dma.module = CAM_A_TWIN;
            else
                CAM_FUNC_ERR("unsupported twin module:%d\n",this->m_twinHwModule);
        }
    ...
    {//write slave cam's header register from master cam
        ISP_DRV_REG_IO_STRUCT Regs_R[E_HEADER_MAX -1];
...
#if (Arbitrary_Crop == 0)
    if(CAM_READ_BITS(this->m_pDrv,CAM_CTL_DMA_EN,RRZO_EN) != (CAM_READ_BITS(this->m_pTwinIspDrv,CAM_CTL_DMA_EN,RRZO_EN) )){
        //currently , because of no p1_don/p1_sof at slave cam, arbitrary cropping is not suppoeted!!
        //it will be failed at waitbufready()!
        CAM_FUNC_ERR("support no arbitrary cropping\n");
    }
...
//this is only return offset value, so it have no need to take case cam_a or cam_b
