...
#define ISP_DRV_CQ_DUMMY_TOKEN(Module)   ((ISP_CQ_NOP_INST<<26)|((CQ_DUMMY_REG + Module*0x1000) & CQ_DES_RANGE))
...
ISP_DRV_CQ_MODULE_INFO_STRUCT mIspCQModuleInfo[CAM_CAM_MODULE_MAX]    =
{
    ...
    {CAM_FBC_AAO_,                 0X4138, 0X0138,  1   },
    {CAM_FBC_PDO_,                 0X4140, 0X0140,  1   },
    ...
    {CAM_ISP_AE_,                   0x49E0, 0x09E0,  25  },//can't access 4a44, this register is for CCU only (AE on CCU)
...
MBOOL ISP_DRV_CAM::cqAddModule(MUINT32 moduleId, MUINT32 extModuleId)
{
    ...
    switch(moduleId){
        case CAM_NEXT_Thread_:
            ...
            break;
        default:
            if(this->twinPath_CQ_Ctrl(moduleId, &offset) == MFALSE){
...
MBOOL ISP_DRV_CAM::twinPath_CQ_Ctrl(MUINT32 moduleId, MUINT32 *offset)
{
    MBOOL ret = MTRUE;
...
MUINT32* ISP_DRV_CAM::update_FH_Spare(CQ_RingBuf_ST *pBuf_ctrl)
{
    ...
    if(i == pcqrtbcring_va->dma_ring_size){
        ...
        char _tmp[16];
        _tmp[0] = _str[0] = '\0';
        LOG_ERR("find no match pa:0x%x, ring_size:0x%x\n", pBuf_ctrl->dma_PA, pcqrtbcring_va->dma_ring_size);
