...
#include <drv/utils.h>
#include "imageio/p1HwCfg.h" //inclucde this is for EIS moulde
...
int CamPathPass1::config( struct CamPathPass1Parameter* p_parameter )
{
    ...
    ISP_PATH_INF("p_parameter rrz_in_roi_size(%d/%d/%d/%d/%d/%d)",\
                                        ...
                                        (int) p_parameter->rrz_in_roi.w, \
                                        (int) p_parameter->rrz_in_roi.h);
    ...
    ISP_PATH_INF("p_parameter rrzD_in_roi_size(%d/%d/%d/%d/%d/%d)",\
                                        ...
                                        (int) p_parameter->rrz_d_in_roi.w, \
                                        (int) p_parameter->rrz_d_in_roi.h);
...
int CamPathPass1::_waitIrq( int type, unsigned int irq , int userNumber)
{
    ...
    if (waitMs < 500)
        waitMs = 500;
    ISP_PATH_DBG("[property] waitMs(%d)\n", waitMs);
...
int CamPathPass1::_registerIrq( NSImageioIsp3::NSIspio::Irq_t irq )
{
    UNUSED(irq);
    return 0;
}

int CamPathPass1::_waitIrq( NSImageioIsp3::NSIspio::Irq_t irq )
{
    UNUSED(irq);
    return 0;
}
...
MUINT32 CamPathPass1::setP1ImgoCfg(DMACfg const imgo_cfg){
    ...
    if(this->CQ == CAM_ISP_CQ0){
        ...
        if(ISP_QuerySize(NSImageio::NSIspio::EPortIndex_IMGO,ISP_QUERY_CROP_X_BYTE,(EImageFormat)DMAImgo.dma_cfg.lIspColorfmt,imgo_cfg.crop.w,queryRst,e_PixMode) == 0){
            ISP_PATH_ERR("cropping size err(%d)\n", (int)imgo_cfg.crop.w);
            return 1;
        }
        DMAImgo.dupCqIdx = this->DupQIdx;
        DMAImgo.burstQueIdx = this->BurstQNum;
        ...
        if(ISP_QuerySize(NSImageio::NSIspio::EPortIndex_IMGO,ISP_QUERY_CROP_X_BYTE,(EImageFormat)DMAImgo_d.dma_cfg.lIspColorfmt,imgo_cfg.crop.w,queryRst,e_PixMode) == 0){
            ISP_PATH_ERR("cropping size err(%d)\n", (int)imgo_cfg.crop.w);
            return 1;
        }
        DMAImgo_d.dupCqIdx = this->DupQIdx;
...
MBOOL CamPathPass1::setIspClk(MUINT32 clk, MBOOL bAFIssue)
{
    MUINT32 ret = 0;
    if(0!= ispPass1Ctrl.setMMDVFS(clk, bAFIssue)){
        ret = 1;
        ISP_PATH_ERR("set isp clk\n");
    }
    return ret;
}

int CamPathPass1::setBandW(MINT32 fps)
{
    MUINT32 ret = 0;
    if ((ISP_PASS1 != this->pass1_path)&& (ISP_PASS1_D != this->pass1_path)) {
        ISP_PATH_ERR("Invalid Path to set m# (%d)", this->pass1_path);
    ...
    if(0!= ispPass1Ctrl.cal_and_setBandW(this->pass1_path, fps)){
        ret = 1;
        ISP_PATH_ERR("set cal bandW\n");
    }
    return ret;
}

MBOOL CamPathPass1::updateFrameTime(MUINT32 timeInMs, MUINT32 reqOffset) {
    ispBufCtrl.updateFrameTime(timeInMs, reqOffset);

    return MTRUE;
}

int CamPathPass1::waitIrqDynamicTimeout(
    int type, unsigned int irq, int userNumber, MUINT32 const u4TimeoutMs) {
    int ret = 0;
    /* "adb shell setprop vendor.camera.p1wait.ms 3000" for sensor long exposure 3s. */
    char waitValue[PROPERTY_VALUE_MAX] = {'\0'};
    property_get("vendor.camera.p1wait.ms", waitValue, "0");
    ...
    if (u4TimeoutMs > waitMs) {
        waitMs = u4TimeoutMs;
    }
    ISP_PATH_DBG("u4TimeoutMs(%d) waitMs(%d)\n", u4TimeoutMs, waitMs);
    ...
    ISP_PATH_DBG("CamPathPass1::waitIrqDynamicTimeout+, type=%x, irq=%x username:%s timeout=%d",
                    type, irq, WaitIrq.UserName, waitMs);
...
int CamPathPass1::flushIrq(int type, unsigned int irq, int userNumber, MUINT32 const u4TimeoutMs)
{
    int ret = 0;

    ISP_DRV_WAIT_IRQ_STRUCT flushIrq;
    flushIrq.Clear = ISP_DRV_IRQ_CLEAR_WAIT;
    flushIrq.UserNumber = userNumber;
    // for supporting manual long exposure time control
    flushIrq.Timeout = u4TimeoutMs;
    flushIrq.UserInfo.UserKey    = 0;
    flushIrq.UserInfo.Status     = irq;
    flushIrq.UserInfo.Type = (ISP_DRV_IRQ_TYPE_ENUM)type;
    flushIrq.bDumpReg = 0xFE;

    ISP_PATH_DBG("type(%d) u4TimeoutMs(%d)\n", type, u4TimeoutMs);

    if (MFALSE == (MBOOL)ispTopCtrl.flushIrq(flushIrq)) {
        ret = -1;
    }
...
int CamPathPass1::setP1RrzCfg(IspRrzCfg const rrz_cfg)
{
    ...
    if ((ISP_PASS1 != rrz_cfg.pass1_path)&& (ISP_PASS1_D != rrz_cfg.pass1_path)) {
        ISP_PATH_ERR("Invalid Path to use RRZ (%d)", (int)rrz_cfg.pass1_path);
        return 0;
    }
    ...
    if ( rrz_cfg.isTwinMode || (ISP_PASS1 == rrz_cfg.pass1_path) ) {
        ...
        ISP_QuerySize(NSImageio::NSIspio::EPortIndex_RRZO,ISP_QUERY_XSIZE_BYTE,(EImageFormat)DMARrzo.dma_cfg.lIspColorfmt,DMARrzo.dma_cfg.size.w,queryRst,e_PixMode);
        ...
        ISP_QuerySize(NSImageio::NSIspio::EPortIndex_RRZO_D,ISP_QUERY_XSIZE_BYTE,(EImageFormat)DMARrzo_d.dma_cfg.lIspColorfmt,DMARrzo_d.dma_cfg.size.w,queryRst,e_PixMode);
