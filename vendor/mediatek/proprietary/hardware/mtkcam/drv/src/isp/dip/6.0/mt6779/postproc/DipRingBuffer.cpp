...
{
    DIP_RINGBUF_DBG("singleton[%p].", &singleton[module]);
...
bool DipRingBufferImp::init(bool AllocSmxBuf, MUINT32 secTag)
{
    ...
    /*============================================
     alloc mfb tpipe buffer
    =============================================*/
    m_MFBTpipeBufInfo.size = DIP_RING_BUFFER_CQ_SIZE*MAX_CMDQ_RING_BUFFER_SIZE_NUM*TPIPE_BLD_SIZE+(15+1);
    m_MFBTpipeBufInfo.useNoncache = 1; //alloc non-cacheable mem for tpipe.
    if (m_pMemDrv->allocVirtBuf(&m_MFBTpipeBufInfo) ) {
        DIP_RINGBUF_ERR("[ERROR] mfb tpipe buf alloc fail!!,size(0x%x)",m_MFBTpipeBufInfo.size);
        return false;
    }
    if ( m_pMemDrv->mapPhyAddr(&m_MFBTpipeBufInfo) ) {
        DIP_RINGBUF_ERR("[ERROR] mfb tpipe buf mapaddr fail!!");
        return false;
    }
    DIP_RINGBUF_DBG("[MFB TPIPE]size(0x%x),Va(0x%lx),Pa(0x%lx)",m_MFBTpipeBufInfo.size,(unsigned long)m_MFBTpipeBufInfo.virtAddr,(unsigned long)m_MFBTpipeBufInfo.phyAddr);
    ...
    if (m_bInitAllocSmxBuf == true){
        ...
        for (int i=0;i<DIP_RING_BUFFER_CQ_SIZE;i++)
        {
            for (int j=0;j<DIP_DMA_SMXIO_MAX;j++)
            {
                ...
                DIP_RINGBUF_DBG("DmaSmxioId:%d, smxiobufOffset:0x%x, SmxVaddr:0x%" PRIxPTR ", SmxPaddr:0x%" PRIxPTR "!!", j, smxiobufOffset, _tmpVirAddr, _tmpPhyAddr);
                switch (j)
                {
                    case DIP_DMA_SMXI_1:
    ...
    for (int i=0;i<DIP_RING_BUFFER_CQ_SIZE;i++)
    {
        ...
        for (int j=0;j<MAX_CMDQ_RING_BUFFER_SIZE_NUM;j++)
        {
            ...
            //MFB Tpipe Buffer
            dip_workingbuf->mfbtpipeTableVa = (unsigned int*)(m_MFBTpipeBufInfo.virtAddr + mfboffset);
            dip_workingbuf->mfbtpipeTablePa = m_MFBTpipeBufInfo.phyAddr + mfboffset;
            mfboffset += TPIPE_BLD_SIZE;
			...
			dip_workingbuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableOft = dip_workingbuf->tpipeTableOft;
...
bool DipRingBufferImp::uninit()
{
    ...
    /*============================================
     free MFB tpipe buffer
    =============================================*/
    if ( m_pMemDrv->unmapPhyAddr(&m_MFBTpipeBufInfo) ) {
        DIP_RINGBUF_ERR("[ERROR]m_MFBTpipeBufInfo unmap addr fail!!");
        return false;
    }

    if ( m_pMemDrv->freeVirtBuf(&m_MFBTpipeBufInfo) ) {
        DIP_RINGBUF_ERR("[ERROR]m_MFBTpipeBufInfo unmap addr fail!!,size(0x%x)",m_TpipeBufInfo.size);
        return false;
    }
    m_MFBTpipeBufInfo.size = 0;
...
bool DipRingBufferImp::getSMXIOBuffer(E_ISP_DIP_CQ CmdqId, DIP_DMA_SMXIO_ENUM DmaSmxioId, MUINTPTR& SmxVaddr, MUINTPTR& SmxPaddr, MUINT32& SmxOft)
{
    ...
    DIP_RINGBUF_DBG("cmdqId(%d), DmaSmxioId:%d, SmxVaddr:0x%" PRIxPTR ", SmxPaddr:0x%" PRIxPTR ", SmxOft:0x%x !!", CmdqId, DmaSmxioId, SmxVaddr, SmxPaddr, SmxOft);
    return true;
}
...
DipWorkingBuffer* DipRingBufferImp::AcquireDipWorkingBuffer(E_ISP_DIP_CQ CmdqId)
{
        ...
        (DipWBufWRIdx < mDipCmdQRBList[CmdqId].mDipWBList.size()))
    {
        if (mDipCmdQRBList[CmdqId].mDipWBList.at(DipWBufWRIdx)->m_WBStatus == DIP_WB_FREE)
        {
            ...
            this->BufferMutexUnLock();
            DIP_RINGBUF_DBG("CmdqId(%d), mDipCmdQRBList[CmdqId].wIdx(%d), mDipCmdQRBList[CmdqId].iWBSize(%d), pDipWBuf(%p)", CmdqId, mDipCmdQRBList[CmdqId].wIdx, mDipCmdQRBList[CmdqId].iWBSize, pDipWBuf);
        }
        else    //State Machine Error!!
        {
            pDipWBuf = NULL;
...
bool DipRingBufferImp::ReleaseDipWorkingBuffer(DipWorkingBuffer* pDipWBuf, E_ISP_DIP_CQ CmdqId)
{
        ...
        (DipWBufRdIdx < mDipCmdQRBList[CmdqId].mDipWBList.size()))
    {
        ...
        {
            this->BufferMutexUnLock();
            DIP_RINGBUF_ERR("State Machine of command queue(0x%x) have error, DipWBufRdIdx(0x%x), pDipWBuf(%p)!!",CmdqId, DipWBufRdIdx, pDipWBuf);
            return false;
        }
    }
...
bool DipRingBufferImp::skipTpipeCalculatebyIdx(E_ISP_DIP_CQ CmdqId, unsigned int& frameflag, unsigned int& RefRingBufIdx, DipWorkingBuffer* pCurrFrameDipWBuf, unsigned int frameNum)
{
    DipWorkingBuffer* pPrevDipWBuf = NULL;
    //DipWorkingBuffer* pPrevBurstDipWBuf = NULL;
    unsigned int BufIdx;
    unsigned int TmpRingBufIdx;
    unsigned int TmpBurstRingBufIdx;
    unsigned long TmptpipeTablePa = 0;
    unsigned int *TmptpipeTableVa = NULL;
    MUINT32* pTmpIspVirRegAddr_va;
    MUINT32* pTmpIspVirRegAddr_pa;
    ISP_DRV_CQ_CMD_DESC_STRUCT*         pTmpIspDescript_vir;
    MUINT32* pTmpIspDescript_phy;
    ...
    //Reset tpipe table value to working buffer tpipe value.
    ...
    if(pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.top.mfb_en == 1)
    {
        return true;
    }
    ...
    //Burst Mode for slow motion or VSDOF
    TmpBurstRingBufIdx = BufIdx - frameNum;
    if (BufIdx >= frameNum)
    {
        TmpBurstRingBufIdx = BufIdx-frameNum;
    }
    ...
    {
        TmpBurstRingBufIdx = (MAX_CMDQ_RING_BUFFER_SIZE_NUM - frameNum + BufIdx);
    }
    //this->BufferMutexLock();
    ...
    if ((TmpRingBufIdx >= 0) && (TmpRingBufIdx < mDipCmdQRBList[CmdqId].mDipWBList.size()))
    {
        ...
        if(pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.frameflag == 0x3)
        {
            frameflag=0x3;
            this->BufferMutexLock();
            TmptpipeTablePa = pCurrFrameDipWBuf->tpipeTablePa;
            TmptpipeTableVa = pCurrFrameDipWBuf->tpipeTableVa;
            pTmpIspVirRegAddr_va = pCurrFrameDipWBuf->m_pIspVirRegAddr_va;
            pTmpIspVirRegAddr_pa = pCurrFrameDipWBuf->m_pIspVirRegAddr_pa;
            pTmpIspDescript_vir = pCurrFrameDipWBuf->m_pIspDescript_vir;
            pTmpIspDescript_phy = pCurrFrameDipWBuf->m_pIspDescript_phy;
            ...
            pCurrFrameDipWBuf->m_pIspVirRegAddr_va = pPrevDipWBuf->m_pIspVirRegAddr_va;
            pCurrFrameDipWBuf->m_pIspVirRegAddr_pa = pPrevDipWBuf->m_pIspVirRegAddr_pa;
            pCurrFrameDipWBuf->m_pIspDescript_vir = pPrevDipWBuf->m_pIspDescript_vir;
            pCurrFrameDipWBuf->m_pIspDescript_phy = pPrevDipWBuf->m_pIspDescript_phy;
            pPrevDipWBuf->tpipeTablePa = TmptpipeTablePa;
            pPrevDipWBuf->tpipeTableVa = TmptpipeTableVa;
            pPrevDipWBuf->m_pIspVirRegAddr_va = pTmpIspVirRegAddr_va;
            pPrevDipWBuf->m_pIspVirRegAddr_pa = pTmpIspVirRegAddr_pa;
            pPrevDipWBuf->m_pIspDescript_vir = pTmpIspDescript_vir;
            pPrevDipWBuf->m_pIspDescript_phy = pTmpIspDescript_phy;
...
//Solutin4:
            ...
            else
            {
                ...
                DIP_RINGBUF_INF("mem cpy from  previous tpipe buf(%p), current tpipe buf(%p), cpysize(%d) debug_cpysize(%d)!!",pPrevDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, cpysize, tmpCpySize);
            }
...
//Soluton:2 <==will fail.
        ...
        {
            //Compare with the burst mode.
#if 0
            if ((TmpBurstRingBufIdx >= 0) && (TmpBurstRingBufIdx < mDipCmdQRBList[CmdqId].mDipWBList.size() && (frameNum>1)))
            {
                pPrevBurstDipWBuf = mDipCmdQRBList[CmdqId].mDipWBList.at(TmpBurstRingBufIdx);


                //Compare whether skip tpipe calculation.
                if(!(::memcmp(&(pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo),
                                        &(pPrevBurstDipWBuf->m_pMdpCfg->ispTpipeCfgInfo),
                                        sizeof(ISP_TPIPE_CONFIG_STRUCT) - sizeof(ISP_DRIVER_CONFIG_STRUCT))))
                {   //adopt the setting in previous frame and copy previus frame tpipe value to current frame tpipe value
                    frameflag=0x1;
        //Solutin5:
                    if (pPrevBurstDipWBuf->m_WBStatus == DIP_WB_FREE)
                    {
                        this->BufferMutexLock();
                        TmptpipeTablePa = pCurrFrameDipWBuf->tpipeTablePa;
                        TmptpipeTableVa = pCurrFrameDipWBuf->tpipeTableVa;
                        pCurrFrameDipWBuf->tpipeTablePa = pPrevBurstDipWBuf->tpipeTablePa;
                        pCurrFrameDipWBuf->tpipeTableVa = pPrevBurstDipWBuf->tpipeTableVa;
                        pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTablePa = pPrevBurstDipWBuf->tpipeTablePa;
                        pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa = pPrevBurstDipWBuf->tpipeTableVa;
                        pPrevBurstDipWBuf->tpipeTablePa = TmptpipeTablePa;
                        pPrevBurstDipWBuf->tpipeTableVa = TmptpipeTableVa;
                        this->BufferMutexUnLock();
                    }
                    else
                    {
                        // get one tile have how many reigster setting. +1:register num, <<2:one rigster have 4 bytes, +4: 4 byte for loadsize.
                        //LOAD_SIZE = ((((pPrevDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa[0] >> 8) & (0xff)) + 1) << 2) + 4;
                        LOAD_SIZE = ((((pPrevBurstDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa[0] >> 16) & (0xff)) + 1) << 2) + 4;
                        TileNum = (pPrevBurstDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.sw.src_width / TPIPE_WIDTH);
                        if (pPrevBurstDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.sw.src_width % TPIPE_WIDTH > 0)
                        {
                            TileNum = TileNum + 1;
                        }
                        cpysize = TileNum*LOAD_SIZE;
                        ...
                        cpysize = MAX_ISP_TILE_TDR_HEX_NO;
                        //cpysize = MAX_ISP_TILE_TDR_HEX_NO;
                        ::memcpy(pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, pPrevBurstDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, cpysize);
                        DIP_RINGBUF_INF("burst mode: mem cpy from  previous tpipe buf(0x%x), current tpipe buf(0x%x), cpysize(%d) debug_cpysize(%d!!",pPrevBurstDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, pCurrFrameDipWBuf->m_pMdpCfg->ispTpipeCfgInfo.drvinfo.tpipeTableVa, cpysize, tmpCpySize);
                    }        
                    RefRingBufIdx = TmpBurstRingBufIdx;
                }
            }
#endif
        }
