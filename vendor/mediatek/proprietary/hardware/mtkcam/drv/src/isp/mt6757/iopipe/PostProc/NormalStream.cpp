            ...
            , mJpegWorkBufSize(0)
{
   ...
   LOG_INF("pipeID/sidx/sdev/pxlID(%d/0x%x/0x%x/0x%x),swPipe cID(0x%x)",mPipeID,openedSensorIndex,mOpenedSensor,mP2PixId,(&mDequeuedBufList));
}
...
init(char const* szCallerName)
{
    ...
    if(mpHalPipeWrapper)
    {
        ret=mpHalPipeWrapper->init(szCallerName, mPipeID);
        if(!ret)
        {
...
MBOOL
NormalStream::
init(char const* szCallerName, NSCam::NSIoPipe::EStreamPipeID StreamPipeID)
{
    FUNCTION_LOG_START;
    Mutex::Autolock autoLock(mModuleMtx);
    MBOOL ret = MTRUE;
    ...
    if(mpHalPipeWrapper)
    {
        ret=mpHalPipeWrapper->init(szCallerName, StreamPipeID);
        if(!ret)
        {
    ...
    QParams const& rParams)
{
    ...
    if(mpHalPipeWrapper)
    {
        ret=mpHalPipeWrapper->enque(mPipeID, mOpenedSensor, mP2PixId, rParams, this->mDequeuedBufList);
        if(!ret)
        {
            LOG_ERR("enque Fail");
        }
    ...
    MINTPTR arg3)
{
    ...
    if(mpHalPipeWrapper)
    {
        ...
        if(!ret)
        {
            LOG_ERR("[Error]sendCommand(0x%x, 0x%x,0x%x,0x%x,0x%x) Fail",cmd,ccmd,arg1,arg2,arg3);
        }
    }
    else
    {
