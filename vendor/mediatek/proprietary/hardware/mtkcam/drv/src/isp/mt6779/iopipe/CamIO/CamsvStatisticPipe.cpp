...
#define DEFAULT_SHUTTERTIME_MS 16000 //1~32s
...
inline MUINT32 SPIPE_GET_TG_INFO(MUINT32 sensorIdx, MUINT32 featureIdx)
{
    MUINT32 tgInfo = CAM_TG_NONE;

    for(MUINT32 i = 0 ; i < CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos.size() ; i++){
        PIPE_INF("SPIPE_GET_TG_INFO i(%d), featureIdx(%d), feature_type(%d)", i, featureIdx,
            CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos[i].feature_type);
        if(featureIdx == CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos[i].feature_type){
            tgInfo = CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos[i].tg_info;
            PIPE_INF("SPIPE_GET_TG_INFO i(%d), sensorIdx(%d), tgInfo(%d)", i, sensorIdx, tgInfo);
            break;
        }
    }
...
inline MBOOL SPIPE_PARSE_VC_INFO(struct SensorVCInfo2 const VcInfo, MUINT32 featureIdx, MUINT32& imgW, MUINT32& imgH, MBOOL& bRaw10)
{
    MBOOL Ret = MFALSE;

    for(MUINT32 i = 0 ; i < VcInfo.vcInfo2s.size() ; i++){
        PIPE_INF("SPIPE_PARSE_VC_INFO featureIdx(%d), size(%d)", featureIdx, VcInfo.vcInfo2s.size());
        if(featureIdx == VcInfo.vcInfo2s[i].VC_FEATURE){
            imgW = VcInfo.vcInfo2s[i].VC_SIZEH_PIXEL;
            imgH = VcInfo.vcInfo2s[i].VC_SIZEV;
            bRaw10 = (VcInfo.vcInfo2s[i].VC_DataType == 0x2b)?MTRUE:MFALSE;
            PIPE_INF("SPIPE_PARSE_VC_INFO imgW(%d), imgH(%d)", imgW, imgH);
            Ret = MTRUE;
            break;
        }
    }
...
inline E_CamPixelMode SPIPE_MAP_PXLMODE(MUINT32 sensorIdx, MUINT32 featureIdx)
{
    ...
    MBOOL bMatch = MFALSE;
    PIPE_INF("SPIPE_MAP_PXLMODE featureIdx(%d)", featureIdx);
    for(MUINT32 i = 0 ; i < CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos.size() ; i++){
        PIPE_INF("i(%d) mux_pixel_mode(%d)", i,
            CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos[i].mux_pixel_mode);
        if(featureIdx == CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos[i].feature_type){
            tgPxlModeSeninf = CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.config_infos[i].mux_pixel_mode;
            bMatch = MTRUE;
            break;
        }
    }
...
CamsvStatisticPipe*  CamsvStatisticPipe::pCamsvStatisticPipe[EPIPE_Sensor_RSVD][VC_MAX_NUM] = \
    {{NULL}, {NULL}, {NULL}, {NULL}};
...
MVOID CamsvStatisticPipe::dumpSeninfDebugCB(MUINT32 ispModule, MUINT32 tgStat)
{
    ...
    for (idx = 0; idx < EPIPE_Sensor_RSVD; idx++) {
        for(i = 0 ; i < CamsvStatisticPipe::pAllSensorInfo->mSenInfo[idx].mDInfo.config_infos.size() ; i++){
            if(camsvTg == CamsvStatisticPipe::pAllSensorInfo->mSenInfo[idx].mDInfo.config_infos[i].tg_info){
                featureIdx = CamsvStatisticPipe::pAllSensorInfo->mSenInfo[idx].mDInfo.config_infos[i].feature_type;
                goto DUMP_INFO;
            }
        }
    }
...
MVOID CamsvStatisticPipe::destroyInstance(char const* szCallerName)
{
    ...
    for (idx = 0; idx < EPIPE_Sensor_RSVD; idx++) {
        for (featIdx = 0; featIdx < VC_MAX_NUM; featIdx++) {
    ...
    if (mTotalUserCnt == 0)  {
        ...
        for(MUINT32 i=0;i<(CAMSV_BUF_DEPTH * (m_subSample + 1));i++) {
            if( NULL != mpbuf_camsv_imgo ) {
                if(mpIMem->freeVirtBuf(&mpbuf_camsv_imgo[i], MFALSE)){
            ...
            if( NULL != mpbuf_camsv_imgo_fh ) {
                ...
                if(mpIMem->freeVirtBuf(&mpbuf_camsv_imgo_fh[i], MFALSE)){
                    PIPE_ERR("IMem freeVirtBuf Failed");
                    SPipeGLock.unlock();
                    return;
                }
...
MBOOL CamsvStatisticPipe::enque(QBufInfo const& rQBuf)
{
    ...
    if (rQBuf.mShutterTimeNs) {
        ...
        mpCamsvIOPipe->sendCommand(EPIPECmd_SET_FRM_TIME, (MINTPTR)(DEFAULT_SHUTTERTIME_MS), 1, 0);
    }
    if (mpCamsvIOPipe->enqueOutBuf(portID, rQBufInfo) == MFALSE) {
        ret =MFALSE;
        PIPE_ERR("error:TG_Camsv_%d port_%d enque fail at FSM %d", mTgInfoCamsv, portID.index, this->m_FSM);
    }
...
inline MINT32 CamsvStatisticPipe::DumpSenDebug()
{
    MINT32 seninfStatus = 0;
    ...
    switch(mTgInfoCamsv){
        ...
        case TG_CAMSV_7:
            break;
        default:
            PIPE_ERR("unsupport tg(%d)",mTgInfoCamsv);
            return -1;
            break;
    }

    seninfStatus = mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[mpSensorIdx].mDevId,
                ...
                if(pbuf->mPa == mpbuf_camsv_imgo[b].phyAddr) {
                    ...
                    switch(cacheSyncPolicy){
                        ...
                        case ESTT_CacheInvalidAll:
                            PIPE_WRN("Camsv:%d: DMA(x%x) PA:%p not support ESTT_CacheInvalidAll", mpSensorIdx , portID.index, (void*)pbuf->mPa);
                        case ESTT_CacheInvalidByRange:
                        default:
                            mpIMem->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID    , &(mpbuf_camsv_imgo[b]), MFALSE);
                            break;
                    }
                    break;
                }
...
MBOOL CamsvStatisticPipe::configPipe(QInitStatParam const& vInPorts, MINT32 burstQnum)
{
    ...
    E_INPUT eTgInputCamsv, eTgOutputCamsv;
    ...
    do {
        ...
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId, SENSOR_CMD_GET_SENSOR_VC_INFO2, (MUINTPTR)&VcInfo, (MINTPTR)&data1, 0);

        if((mTgInfoCamsv = SPIPE_GET_TG_INFO(mpSensorIdx, mFeatureIdx)) == CAM_TG_NONE){
            PIPE_INF("Sensor not support feature(%d), sensorid(%d), scenario(%d)", mFeatureIdx, mpSensorIdx, mscenarioId);
            mErrorCode = ECAMSVSPipeErrorCode_SENSOR_NOT_SUPPORT_FEATURE;
                    return MFALSE;
                }
        if(SPIPE_PARSE_VC_INFO(VcInfo, mFeatureIdx, imgW, imgH, bRaw10) == MFALSE){
            PIPE_INF("Sensor not support feature(%d), sensorid(%d), scenario(%d)", mFeatureIdx, mpSensorIdx, mscenarioId);
            mErrorCode = ECAMSVSPipeErrorCode_SENSOR_NOT_SUPPORT_FEATURE;
                    return MFALSE;
                }
        ...
        switch (mTgInfoCamsv) {
            case CAM_SV_1: // 0x10
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_0; // 10
                break;
            case CAM_SV_2: // 0x11
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_1; // 11
                break;
            case CAM_SV_3: // 0x12
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_2; // 12
                break;
            case CAM_SV_4: // 0x13
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_3; // 13
                break;
            case CAM_SV_5: // 0x14
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_4; // 14
                break;
            case CAM_SV_6: // 0x15
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_5; // 15
                break;
            case CAM_SV_7: // 0x16
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_6; // 16
                break;
            case CAM_SV_8: // 0x17
                eTgInputCamsv = eTgOutputCamsv = TG_CAMSV_7; // 17
                break;
            default:
        ...
        if( !mpCamsvIOPipe ) {
            mpCamsvIOPipe = ICamIOPipe::createInstance((MINT8 const*)mpName, eTgOutputCamsv, NSImageio::NSIspio::ICamIOPipe::CAMSVIO);
            if (NULL == mpCamsvIOPipe) {
                PIPE_ERR("TG_Camsv_%d %s fail", eTgOutputCamsv, "create");
                mErrorCode = ECAMSVSPipeErrorCode_CREATE_CAMSAV_IOPIPE_FAILE;
                return MFALSE;
            }
        }
        if (MFALSE == mpCamsvIOPipe->init()) {
            PIPE_ERR("TG_Camsv_%d %s fail", eTgOutputCamsv, "init");
            mErrorCode = ECAMSVSPipeErrorCode_INIT_CAMSAV_IOPIPE_FAILE;
            return MFALSE;
        }

        // update TG info
        mTgInfoCamsv = (MINT32)eTgOutputCamsv;
        switch(eTgInputCamsv){
            case TG_CAMSV_0:
                eTgInputCamsv = (E_INPUT)CAM_SV_1;
                break;
            case TG_CAMSV_1:
                eTgInputCamsv = (E_INPUT)CAM_SV_2;
                break;
            case TG_CAMSV_2:
                eTgInputCamsv = (E_INPUT)CAM_SV_3;
                break;
            case TG_CAMSV_3:
                eTgInputCamsv = (E_INPUT)CAM_SV_4;
                break;
            case TG_CAMSV_4:
                eTgInputCamsv = (E_INPUT)CAM_SV_5;
                break;
            case TG_CAMSV_5:
                eTgInputCamsv = (E_INPUT)CAM_SV_6;
                break;
            case TG_CAMSV_6:
                eTgInputCamsv = (E_INPUT)CAM_SV_7;
                break;
            case TG_CAMSV_7:
                eTgInputCamsv = (E_INPUT)CAM_SV_8;
                break;
            default:
                PIPE_ERR("TG_Camsv_%d %s fail", eTgInputCamsv, "unknown idx");
                mErrorCode = ECAMSVSPipeErrorCode_UNKNOWN_TG_CAMSV_ID;
                return MFALSE;
        }
        switch(eTgOutputCamsv){
            case TG_CAMSV_0:
                eTgOutputCamsv = (E_INPUT)CAM_SV_1;
                break;
            case TG_CAMSV_1:
                eTgOutputCamsv = (E_INPUT)CAM_SV_2;
                break;
            case TG_CAMSV_2:
                eTgOutputCamsv = (E_INPUT)CAM_SV_3;
                break;
            case TG_CAMSV_3:
                eTgOutputCamsv = (E_INPUT)CAM_SV_4;
                break;
            case TG_CAMSV_4:
                eTgOutputCamsv = (E_INPUT)CAM_SV_5;
                break;
            case TG_CAMSV_5:
                eTgOutputCamsv = (E_INPUT)CAM_SV_6;
                break;
            case TG_CAMSV_6:
                eTgOutputCamsv = (E_INPUT)CAM_SV_7;
                break;
            case TG_CAMSV_7:
                eTgOutputCamsv = (E_INPUT)CAM_SV_8;
                break;
            default:
                PIPE_ERR("TG_Camsv_%d %s fail", eTgOutputCamsv, "unknown idx");
                mErrorCode = ECAMSVSPipeErrorCode_UNKNOWN_TG_CAMSV_ID;
                return MFALSE;
        }
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId, SENSOR_CMD_SET_SENINF_CAM_TG_MUX, (MUINTPTR)&eTgInputCamsv, (MUINTPTR)&eTgOutputCamsv, 0);
        ...
        for(MUINT32 i=0;i<(CAMSV_BUF_DEPTH*(m_subSample+1));i++) {
            ...
            if(mpIMem->mapPhyAddr(&mpbuf_camsv_imgo[i], MFALSE) < 0 ){
            ...
            if(mpIMem->mapPhyAddr(&mpbuf_camsv_imgo_fh[i], MFALSE) < 0 ){
        ...
        switch(mTgInfoCamsv) {
            ...
            case TG_CAMSV_5: // 15 ( 0x0F )
                ...
                break;
            case TG_CAMSV_6: // 16 ( 0x10 )
                _in_port->index = EPortIndex_CAMSV_6_TGI;
                break;
            case TG_CAMSV_7: // 17 ( 0x11 )
                _in_port->index = EPortIndex_CAMSV_7_TGI;
                break;
            default:
                PIPE_ERR("Unsupported TG Input(%d)", mTgInfoCamsv);
                return MFALSE;
                break;
        }

        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId, SENSOR_CMD_GET_HORIZONTAL_BLANKING,
                                    (MUINTPTR)&mTgInfoCamsv, (MUINTPTR)&_in_port->lineblkT, (MUINTPTR)&_in_port->lineT);
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId, SENSOR_CMD_GET_VERTICAL_BLANKING, \
                                    (MUINTPTR)&_in_port->frameblkT, 0, 0);
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId, SENSOR_CMD_GET_DEFAULT_FRAME_RATE_BY_SCENARIO,
                                    (MUINTPTR)&this->mscenarioId, (MUINTPTR)&_in_port->tgFps, 0);
        ...
        if(mpCamsvIOPipe->configPipe(vCamsvIOInPorts, vCamsvIOOutPorts, &func) == MFALSE){
            ...
            mpCamsvIOPipe->destroyInstance();
            mpCamsvIOPipe = NULL;
            PIPE_ERR("TG_Camsv_%d %s configPipe fail", mTgInfoCamsv, "config");
