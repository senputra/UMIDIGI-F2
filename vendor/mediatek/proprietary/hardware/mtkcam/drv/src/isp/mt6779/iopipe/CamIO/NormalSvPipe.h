...
#ifndef _MTK_PLATFORM_HARDWARE_MTKCAM_IOPIPE_CAMIO_NormalSvPipe_H_
#define _MTK_PLATFORM_HARDWARE_MTKCAM_IOPIPE_CAMIO_NormalSvPipe_H_
...
protected:
    ...
    typedef struct IQData_t
    {
        ...
        {
            E_CamPathIQFSM    FSM[NSVPIPE_CAMSV_MAX];
            E_CamIQLevel      lv[NSVPIPE_CAMSV_MAX];
        };
        ...
        :IQNotify(NULL)
        {
            for(MUINT32 i = 0; i < NSVPIPE_CAMSV_MAX; i++){
...
class NormalSvPipe_Thread;

class NormalSvPipe : public INormalPipe
{
    friend class NormalSvPipe_Thread;

public:
    NormalSvPipe(MUINT32 pSensorIdx, char const* szCallerName);
                   ~NormalSvPipe(){};
public:
    ...
    typedef struct
    {
        MUINT32 NSvPipeAllocMemSum;
        MUINT32 NSvPipeFreedMemSum;
    }_MemInfo_t;
    ...
    MUINT32         GetOpenedPortInfo(NormalSvPipe::ePortInfoCmd cmd,MUINT32 arg1=0);
...
protected:
    ...
    MSize                           mTgOut_size[(MUINT32)TG_CAMSV_MAX];//user marco _TGMapping for array indexing
...
public:
    ...
    NormalSvPipe_Thread             *mpNSvPipeThread;
    fp_DropCB                       m_DropCB;
    ...
    static Mutex                    NSvPipeGLock; // NormalPipe Global Lock
    static Mutex                    NSvPipeCfgGLock; // NormalPipe Global Lock
    static Mutex                    NSvPipeDbgLock; // for seninf dump callback
    static NormalSvPipe             *pNormalSvPipe[EPIPE_Sensor_RSVD];
...
class NormalSvPipe_Thread {
    friend class NormalSvPipe;
public:
    /**
        @param obj:obj address
    */
    NormalSvPipe_Thread(NormalSvPipe* obj);
    ~NormalSvPipe_Thread(){};
    static NormalSvPipe_Thread* createInstance(NormalSvPipe* obj);
