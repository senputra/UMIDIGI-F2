...
MBOOL IspDrvDipPhy::setDeviceInfo(E_DEVICE_INFO eCmd,MUINT8* pData)
{
    ...
    switch(eCmd){
        ...
        case _SET_DIP_BUF_INFO:
            ...
            LOG_INF("[Debug][Buffer control] p2bufQ(%d_%d_%d_0x%x_%d_%d_%d_%d_%d)",p2bufQ.ctrl,p2bufQ.property,p2bufQ.processID,p2bufQ.callerID,\
...
MBOOL IspDrvDipPhy::dumpDipHwReg(IspDumpDbgLogDipPackage* pP2Package){
	...
	LOG_INF("DIP_X_CTL_TDR_TCM3_EN=0x%08x",readReg(((MUINT32)((MUINT8*)&dipReg.DIP_X_CTL_TDR_TCM3_EN-(MUINT8*)&dipReg)), DIP_A));
	//dump tpipe
...
MBOOL IspDrvDipPhy::setMemInfo(unsigned int meminfocmd, unsigned long tpipeTablePa, unsigned int *tpipeTableVa, unsigned int MemSizeDiff)
{
    ISP_MEM_INFO_STRUCT TpipeMemInfoStruct;

    TpipeMemInfoStruct.MemInfoCmd = meminfocmd;  //ISP_MEMORY_INFO_TPIPE_CMD;
