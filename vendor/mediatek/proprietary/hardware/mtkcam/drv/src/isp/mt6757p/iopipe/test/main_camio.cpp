...
#define SWITCH_FRMNUM  (15)

//for additional test
#define _test_efuse         (0x00000001)
#define _test_tgint         (0x00000002)
#define _test_cqtuning_af   (0x00000004)
#define _test_cqtuning_ae   (0x00000008)
#define UNI_DYNAMIC_SWITCH  (0x00000100)
...
public:
    ...
    void                setAFCqTuning(void);
    void                setAACqTuning(void);
    ...
    static void*        _sttProcLoop4_ (void *arg);
...
static MUINT32 cqTuning_mnum[__MAXDMAO_STT][_MAXTG_];
...
void *NPipeUT::_sttProcLoop4_ (void *arg)
{
    NPipeUT::sttProcLoop(4, (NPipeUT*)arg);
    return 0;
}
...
#define AF_WIN_SZ     16
#define AF_WIN_SZ_EXT 32//if AF_CON.AF_EXT_STAT_EN is enabled
    ...
    list<ECQ_TUNING_USER> module_en;
    ...
    for(MUINT32 i=0;i<this->mBurstQNum;i++){
        ...
        if(this->mEnableSttPort & __AFO_ENABLE){
            if(this->m_cqtuning&_test_cqtuning_af){
                tuningMgr->enableEngine(eTuningMgrFunc_AF, MTRUE, i);
                tuningMgr->enableEngine(eTuningMgrFunc_SGG1, MTRUE, i);
                tuningMgr->enableEngine(eTuningMgrFunc_SGG5, MTRUE, i);//SGG5, if AF_EXT is enable
            } else {
                ...
                if (1){//pMagic[i]%3 == 1){
                    MY_LOGD("AF_EXT = 1");
                    TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_AF_CON, (1<<22), i);

                    xsize = AF_WIN_NUM_X*AF_WIN_SZ_EXT;
                    TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_AFO_XSIZE, xsize, i);
                    TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_AFO_STRIDE, xsize, i);

                    tuningMgr->updateEngine(eTuningMgrFunc_SGG5, MTRUE, i);
                    TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_SGG5_PGN, 0x10, i);//default value
                    TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_SGG5_GMRC_1, 0x10080402, i);
                    TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_SGG5_GMRC_2, 0x00804020, i);
                } else {
                    tuningMgr->updateEngine(eTuningMgrFunc_SGG5, MFALSE, i);
                }

                tuningMgr->updateEngine(eTuningMgrFunc_SGG1, MTRUE, i);
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_SGG1_PGN, 0x10, i);//default value
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_SGG1_GMRC_1, 0x10080402, i);
                TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_SGG1_GMRC_2, 0x00804020, i);
            }
        }
    ////////////////////////////////////////////////////////////////////////////////////////
    //AAO
...
#define TSF_ST_EN   1
            ...
            //awb
            awb_pix_cnt = (1<<24) / (aa_win_size_h * aa_win_size_v) *2;
            TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PS_AWB_PIXEL_CNT0, awb_pix_cnt*2, i);
            TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PS_AWB_PIXEL_CNT1, awb_pix_cnt*2, i);
            TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PS_AWB_PIXEL_CNT2, awb_pix_cnt*2, i);
            TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PS_AWB_PIXEL_CNT3, awb_pix_cnt*2, i);
            //ae
            TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PS_AE_YCOEF0, 0x12C912C9, i);
            TUNING_MGR_WRITE_REG_CAM(tuningMgr, CAM_PS_AE_YCOEF1, 0x074B12C9, i);
            //aao
...
MVOID NPipeUT::setAFCqTuning(void)
{
#define AF_WIN_NUM_X  64
#define AF_WIN_NUM_Y  64
#define AF_WIN_SZ     16
#define AF_WIN_SZ_EXT 32//if AF_CON.AF_EXT_STAT_EN is enabled
    ...
    cqTuning_mnum[__AFO][this->mSensorIdx]++;//start from 1
    MY_LOGD("[%d] setAFCqTuning M(%d)", this->mSensorIdx, cqTuning_mnum[__AFO][this->mSensorIdx]);

    //AF image wd
    this->mpNPipe->sendCommand(ENPipeCmd_GET_HBIN_INFO,(MINTPTR)&h_size, (MINTPTR)&v_size, 0);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AF_CON, 0);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AF_SIZE, h_size);//case for no frontal binning
    ...
    //ofset
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AF_VLD, (start_y<<16 | start_x));
    //window num
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AF_BLK_1, ((AF_WIN_NUM_Y<<16)|AF_WIN_NUM_X));
    ...
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AF_BLK_0, ((win_v_size<<16)|win_h_size));

    //xsize/ysize
    xsize = AF_WIN_NUM_X*AF_WIN_SZ;
    ysize = AF_WIN_NUM_Y;
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AFO_XSIZE, xsize);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AFO_YSIZE, ysize);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AFO_STRIDE, xsize);

    //af_ext, sgg5
    if (1){//cqTuning_mnum[__AFO][this->mSensorIdx]%3 == 1){
        MY_LOGD("AF_EXT = 1");
        CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AF_CON, (1<<22));

        xsize = AF_WIN_NUM_X*AF_WIN_SZ_EXT;
        CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AFO_XSIZE, xsize);
        CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_AFO_STRIDE, xsize);

        CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_SGG5_PGN, 0x10);//default value
        CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_SGG5_GMRC_1, 0x10080402);
        CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_SGG5_GMRC_2, 0x00804020);
        module_en.push_back(EAF_TUNING_SGG5);
    }

    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_SGG1_PGN, 0x10);//default value
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_SGG1_GMRC_1, 0x10080402);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_SGG1_GMRC_2, 0x00804020);

    module_en.push_back(EAF_TUNING_AF);
    module_en.push_back(EAF_TUNING_AFO);
    module_en.push_back(EAF_TUNING_SGG1);
    cmd.module = &module_en;
    cmd.serial = cqTuning_mnum[__AFO][this->mSensorIdx];
    cqtuningMgr->update(cmd);
}

MVOID NPipeUT::setAACqTuning(void)
{
    MUINT32 OBgain=0, LSCratio;
    list<ECQ_TUNING_USER> module_en;
    CQ_TUNING_CMD_ST cmd;
    ...
    if(!(this->m_cqtuning&_test_cqtuning_ae))
        return;

    cqTuning_mnum[__AAO][this->mSensorIdx]++;//start from 1
    OBgain = ((cqTuning_mnum[__AAO][this->mSensorIdx]%3) == 2) ? 1024 : 512;

    //AF image wd
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_OBC_GAIN0, OBgain);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_OBC_GAIN1, OBgain);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_OBC_GAIN2, OBgain);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_OBC_GAIN3, OBgain);

    //normal is:0x20202020
    LSCratio = ((cqTuning_mnum[__AAO][this->mSensorIdx]%3) == 2) ? 0x10101010 : 0x30303030;
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_LSC_RATIO_0, LSCratio);
    CQ_TUNING_MGR_WRITE_REG(cqtuningMgr, CAM_LSC_RATIO_1, LSCratio);

    MY_LOGD("[%d]  setAACqTuning M(%d)OB(0x%x)LSC(0x%x)", \
        this->mSensorIdx, cqTuning_mnum[__AAO][this->mSensorIdx],OBgain,LSCratio);

    module_en.push_back(EAA_TUNING_OB);
    module_en.push_back(EAA_TUNING_LSC);
    cmd.module = &module_en;
    cmd.serial = cqTuning_mnum[__AAO][this->mSensorIdx];
    cqtuningMgr->update(cmd);
}
...
MVOID* NPipeUT::main_loop(void* arg)
{
    ...
    for ( ; _this->mLoopCount < _this->mFrameNum; ) {
        ...
        if (_this->mSuspendFrm && (_this->mLoopCount == _this->mSuspendFrm)) {
            MY_LOGD("Enter suspend flow at frame#%d", _this->mLoopCount);
            _this->mSuspendState = 1;
            _this->mpNPipe->suspend();

            MY_LOGD("Wait 4000000us");
            usleep(4000000);
    ...
    while (!_this->m_bStop) {
        ...
        for (i = enqBufIdx; i < (enqBufIdx + _this->mBurstQNum); i++, magicNum++) {
            ...
            if (_this->m_enablePort & __RSSO_ENABLE) {
                ...
                enqQ.mvOut.push_back(BufInfo(PORT_RSSO, pIBuf, _ssize, MRect(MPoint(0,0), _ssize), magicNum, sof_idx, 0xFFFF));
            }
            if (_this->m_enablePort & __LCSO_ENABLE) {
...
int test_camio(int argc, char** argv)
{
    ...
    MUINT32             _addtional = 0, cqtuning = 0;
    ...
    MUINT32             suspendFrm = 0, suspendTestFlow = 0;
...
#include "../../drv/inc/isp_drv_cam.h"//for cfg the test pattern of seninf only
    ...
    if (argc < 6) { // iopipeTest 1 0 0 30 2 0, test patter no crop
        MY_LOGI("Param: 1 <sensorDev> <scenario> <sensorFps> <enablePort> <cropTestSel>"\
            "<sttPorts> <burstQNum> <FrameNum> <stopFlow> <switchCam> <twinCtrl> <additionalcmds> <suspend_test>");
        ...
        MY_LOGI("<enablePort>   : 0x01 rrzo, 0x02 imgo, 0x04 eiso, 0x08 lcso, 0x10 rrzo+ufeo 0x20 rsso,bitmap, 0x100");
        ...
        MY_LOGI("<additional>   : 0 : no, 1: efuse test , 2: tg interrupt test, 4: af cqtuning, 8:aa cqtuning");
        MY_LOGI("               : 0x100: UNI_DYNAMIC_SWITCH");
        MY_LOGI("<suspend_test> : b'[15:0]  : frame# for enter suspend for 3 sec");
        MY_LOGI("               : b'[16]    : 0).suspend->resume 1).suspend->stop");
        return -1;
    }
    ...
    if (argc > CMD_IDX_ADDTIONAL) {
        ...
        MY_LOGD("addtional : 0x%x", _addtional);
        ...
        //CqtuningMgr
        if(_addtional&_test_cqtuning_af){
            cqtuning |= _test_cqtuning_af;
        }
        if(_addtional&_test_cqtuning_ae){
            cqtuning |= _test_cqtuning_ae;
        }
    }

    if (argc > CMD_IDX_SUSPEND) {
        suspendFrm = (atoi(argv[CMD_IDX_SUSPEND]) & 0xFFFF);
        suspendTestFlow = ((atoi(argv[CMD_IDX_SUSPEND]) >> 16) & 0x0001);
        MY_LOGD("Frame#%d enable SW %s test flow", suspendFrm, (suspendTestFlow)?"suspend-stop":"suspend-resume");
    }
    ...
    MY_LOGD("efuse: %d, waitTgInt: %d, cqtuning: %d", _efuse, waitTgInt, cqtuning);
    /*
     * sensor create/query
     */
...
SEARCH_SENSOR:
    ...
    for (sidx = 0; sidx < sensorNum; sidx++) {
        ...
        pUt[sidx]->mSuspendFlow = suspendTestFlow;
        pUt[sidx]->mSuspendState = 0;
        pUt[sidx]->m_cqtuning = cqtuning;
        ...
        if ((enablePort & __EISO_ENABLE) && (sidx > 0)) {
            if(_addtional & UNI_DYNAMIC_SWITCH)
        ...
        if(_addtional & UNI_DYNAMIC_SWITCH) {
            ...
            MY_LOGD("UNI dynamic switch for dual-zomm feature is enabled");
        }
    ...
    for (sidx = 0; sidx < sensorNum; sidx++) {
        ...
        if(pUt[sidx]->m_cqtuning){
            pUt[sidx]->cqtuningMgr = (CqTuningMgr*)CqTuningMgr::getInstance(pUt[sidx]->mSensorIdx);
            if (MFALSE == pUt[sidx]->cqtuningMgr->init(mThisName)) {
                MY_LOGE("cqtuningMgr init error");
                return -1;
            }
        }
        ...
        for (pUt[sidx]->mEnqCount = 0; pUt[sidx]->mEnqCount < pUt[sidx]->mBurstQNum*pUt[sidx]->mStaticEnqCnt;
            ...
            for (j = pUt[sidx]->mEnqCount; j < (pUt[sidx]->mEnqCount + pUt[sidx]->mBurstQNum); j++) {
                ...
                if (pUt[sidx]->m_enablePort & __RSSO_ENABLE) {
                    ...
                    _cropsize = MSize(288, 511);

                    BufInfo _buf(PORT_RSSO, pIBuf, _cropsize, MRect(MPoint(0,0), _cropsize), magicNum, 0, 0xFFFF);
...
FAIL_EXIT:
    for (sidx = 0; sidx < sensorNum; sidx++) {
        ...
        if (pUt[sidx]->cqtuningMgr) {
            pUt[sidx]->cqtuningMgr->uninit(mThisName);
        }
