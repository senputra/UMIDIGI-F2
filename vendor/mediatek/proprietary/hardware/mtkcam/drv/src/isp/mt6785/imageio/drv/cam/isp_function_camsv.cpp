...
MBOOL DMAO_CAMSV_B::BW_Meter(const Sig_Profile& prof,V_DMA_BW* pOut)
{
    ...
    CAM_FUNC_DBG("DMAO_CAMSV_B::BW_Meter+");
#if 0
    //
    if(this->m_FIFO.size() == 0){
        #define FIFO_a(port,regname){\
            fifo.portID = port;\
            fifo.ThruPut = CAMSV_READ_BITS(this->m_pDrv,regname,FIFO_SIZE);\
            fifo.ThruPut = fifo.ThruPut<<4;\
            this->m_FIFO.push_back(fifo);}

        //
        FIFO_a(_camsv_imgo_,CAMSV_IMGO_CON);
    ...
    {
        CAMSV_REG_MODULE_EN dma_en;
        dma_en.Raw = CAMSV_READ_REG(this->m_pDrv,CAMSV_MODULE_EN);

        if(dma_en.Bits.IMGO_EN)
            pOut->push_back(SVBW_l(_camsv_imgo_,dma_en.Bits.IMGO_EN,CAMSV_IMGO_XSIZE,CAMSV_IMGO_YSIZE));
    }
