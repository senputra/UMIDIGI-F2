...
public:     ////    Buffer Quening.
    ...
    virtual MUINT32   dequeBuf(MBOOL& isVencContained, QTimeStampBufInfo& rQBufInfo, MUINTPTR& tpipeVa, MUINTPTR& ispVirRegVa,EDrvScenario eDrvSce=eDrvScenario_P2A, MINT32 dequeCq=0,MINT32 const dequeBurstQIdx=0, MINT32 const dequeDupCqIdx=0, MUINT32 const RingBufIdx=0, MUINT32 const u4TimeoutMs = 0xFFFFFFFF);
...
public:     ////    Settings.
    ...
    virtual MBOOL   configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, PipePackageInfo *pPipePackageInfo, char const* szCallerName);
...
private:
    ...
    MBOOL PrintPipePortInfo(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, MUINT32 FrameNo, MUINT32 RequestNo, MUINT32 Timestamp, MINT32 UniqueKey, MBOOL SecureFra);
