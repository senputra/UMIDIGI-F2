...
IMemDrv* IMemDrvImp::getInstance(void)
{
    ...
    IMEM_DBG("singleton[0x%08lx],getpid[0x%08x],gettid[0x%08x] ",(unsigned long)(&singleton),getpid() ,gettid());
    IMEM_DRV_DELAY
    return &singleton;
}
//-----------------------------------------------------------------------------
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    //struct ion_fd_data fd_data; //not used
    ...
    if (!pInfo->virtAddr)
    {
        IMEM_ERR("Cannot map ion buffer. memID(0x%x)/size(0x%x)/va(0x%lu)",pInfo->memID,pInfo->size,(unsigned long)(pInfo->virtAddr));
        return -1;
    }
...
MINT32 IMemDrvImp::freeVirtBuf(
    IMEM_BUF_INFO* pInfo)
{
    IMEM_DBG("mID[0x%x]/size[0x%x]/VA[0x%lu]/PA[0x%lu]",pInfo->memID,pInfo->size,(unsigned long)(pInfo->virtAddr),(unsigned long)(pInfo->phyAddr));
    IMEM_DRV_DELAY
    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    if ( 0 ==  buf_map.count(pInfo->virtAddr) ) {
        ...
        if ( IMEM_MIN_ION_FD > pInfo->memID ) {
            ...
            //struct ion_custom_data custom_data; //not used
            ...
            //
            IMEM_DBG("Physical address=0x%08u len=0x%lu", sys_data.get_phys_param.phy_addr, sys_data.get_phys_param.len);
    ...
    //
    IMEM_DBG("mID(0x%x),size(0x%x),VA(0x%lu),PA(0x%lu),S/C(%d/%d)",pInfo->memID,pInfo->size,(unsigned long)(pInfo->virtAddr),(unsigned long)(pInfo->phyAddr),pInfo->bufSecu,pInfo->bufCohe);
...
#endif
        //
        IMEM_DBG("[IMEM_BUFFER] - unmap<memID(0x%x),size(0x%x),virtAddr(0x%lu),phyAddr(0x%lu)>",pInfo->memID,pInfo->size,(unsigned long)(pInfo->virtAddr),(unsigned long)(pInfo->phyAddr));
        IMEM_DRV_DELAY
        //
#if     defined(__ISP_USE_PMEM__)
...
MINT32 IMemDrvImp::cacheSyncbyRange(IMEM_CACHECTRL_ENUM ctrl,IMEM_BUF_INFO* pInfo)
{
    ...
    MINT32 err=0;
    //MUINT32 phyAddr = (MUINT32)(pInfo->phyAddr); //not used
    IMEM_DBG("+, c/m/va/sz/pa(%d/0x%x/0x%lu/0x%x/0x%lu)",ctrl,pInfo->memID,(unsigned long)(pInfo->virtAddr),pInfo->size,(unsigned long)(pInfo->phyAddr));
    ...
    else
    {
        if((ctrl == IMEM_CACHECTRL_ENUM_FLUSH) && (pInfo->size >= 0x3200000))
        {
            ...
            IMEM_DBG("+ Flush by range, c/m/va/sz/pa(%d/0x%x/0x%lu/0x%x/0x%lu)",ctrl,pInfo->memID,(unsigned long)(pInfo->virtAddr),pInfo->size,(unsigned long)(pInfo->phyAddr));
            return 0;
        }
