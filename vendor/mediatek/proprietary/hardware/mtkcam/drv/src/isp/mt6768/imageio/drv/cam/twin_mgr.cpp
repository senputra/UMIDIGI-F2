...
MBOOL TwinMgr_IMP::configPath(const TWIN_MGR_PATH_CFG_IN_PARAM& cfgInParam, TWIN_MGR_PATH_CFG_OUT_PARAM &cfgOutParam, MUINT32 navailCam)
{
    ...
    if(cfgInParam.datPat == E_4CELL){
        thru_put = thru_put >> bin_status;
    }
    ...
    // 1. dbn set
    if(cfgInParam.bDbn|| (cfgInParam.datPat == E_4CELL)){
    ...
    case TG_FMT_RAW14:
        ...
        if(bin_in_size > rst.pipeSize){
            ...
            else if(cfgInParam.offBin == MFALSE){
                if(cfgInParam.rrz_out_w <= (bin_in_size >> (bin_status+1))){
                    bBin_useful = MFALSE;
                }
                else{
                    bBin_useful = MTRUE;
            ...
            LOG_ERR("bin_in_size <= rst.pipeSize\n");
        }
