...
#define DPSTREAM_NUM ((NSImageio::NSIspio::eScenarioID_NUM))
...
typedef enum eMDPMGR_MDP_PORT_INDEX
{
    ...
    MDPMGR_ISP_ONLY_TPIPE,
    MDPMGR_ISP_MDP_VENC,
    MDPMGR_ISP_ONLY_FRAME,
    MDPMGR_ISP_MDP_DL2,
...
class MdpMgrImp : public MdpMgr
{
    public :
        ...
        /**
               ...
               *-MTRUE indicates success, otherwise indicates fail
             */
        virtual MBOOL   startVencLink(MINT32 fps, MINT32 wd,MINT32 ht);
        /**
               *@brief notify that stop video record for slow motion
               *@return
               *-MTRUE indicates success, otherwise indicates fail
             */
        virtual MBOOL   stopVencLink();
        /**
                       *@brief update mapping relationship for selecting dpStream
                       *@return
                       *-MTRUE indicates success, otherwise indicates fail
                     */
        virtual MBOOL   updateDpStreamMap(MUINT32 flag, MUINT32 cqIdx,MUINT32 ringbufIdx,MUINT32 refringbufidx);
        ...
        /**
                       ...
                       *-MTRUE indicates success, otherwise indicates fail
                     */
        virtual MINT32 init (MUINT32 tileselmode);
        ...
        /**
                       ...
                       *-MTRUE indicates success, otherwise indicates fail
                    */
        virtual MINT32 uninit(MUINT32 tileselmode);
    ...
    private :
        ...
        /**
             ...
             */
        virtual DpIspStream *selectDpIspStream(MUINT32 sceID, MUINT32 cqIdx, MUINT32 dupCqIdx, MUINT32 burstCqIdx, MUINT32 ringBufIdx);
        virtual DpVEncStream *selectDpVEncStream();
        /**
               *@brief Alloc DpIspStream object according to scenario ID
               *
               *@param[in] sceID : cq number of dip
               *
               *@return
               *-NULL indicates fail, otherwise indicates success
             */
        virtual DpIspStream *allocDpStream(MUINT32 cqIdx);
        virtual DpIspStream *selectWpeStream(MUINT32 drvScenario, MUINT32 dupCqIdx);
        ...
        //DpIspStream *m_pDpStream[ISP_DRV_DIP_CQ_NUM][MAX_DUP_CQ_NUM][MAX_BURST_QUE_NUM];
        DpIspStream *m_pDpStream[DIP_RING_BUFFER_CQ_SIZE][MAX_CMDQ_RING_BUFFER_SIZE_NUM];
        DpIspStream *m_DpVssIspStream;
        //slow motion, direct link
        DpVEncStream *m_DpVencStream;    //only support CC
        volatile MUINT32  mTileSelMode;
        volatile MINT32  mwpeInitCount;
        DpIspStream *m_wpeaDpStream[MAX_DUP_CQ_NUM];
		//DpIspStream *m_wpebDpStream[MAX_DUP_CQ_NUM]; // for wpe_b
};
