...
#define _DMA_Header_Dump(header, name)({\
    MUINTPTR *va = (MUINTPTR *)header;\
    CAM_FUNC_INF("== _DMA_Header_Dump : %s ==", name);\
    for (MUINT32 index = 0; index < 12; index++) {\
        CAM_FUNC_INF("header[%d]:0x%x", index, va[index]);\
    }\
})
...
#define _LOOP_  (5)
    ...
    if(_size <1){
        CAM_FUNC_ERR("dma:%s, queue depth is empty for deque\n", this->name_Str());
        return eCmd_Fail;
    }
    ...
    /*
     *  eCamState_suspending means STT is doing suspend.
     *  eCamState_suspend means STT has finished suspend.
     *
     *  The suspend sequence is NormalPipe suspend firstly and then STT suspend.
     *  The resume sequence is STT resume firstly and then NormalPipe resume.
     *  Therefore, STT waitBufReady() can't pass when in eCamState_suspend in
     *  order to wait P1 done. However, NormalPipe waitBufReady() could pass
     *  when in eCamState_suspend for doing dequeueHwBuf() later to pop up
     *  buffer and then call back to inform drop frame.
     */
            ...
            if(loopCnt == _LOOP_){
                ...
                if(sw_enque_cnt>0){
                    ...
                    snprintf(str, sizeof(str),
                        "dma:%s start wait:[enque record:SW_buf_cnt:0x%x, SW_enq_cnt:0x%x, FbcCnt:0x%x], waiting PA:0x%x",\
                        this->name_Str(), this->m_buf_cnt, sw_enque_cnt, FbcCnt, _buf.image.mem_info.pa_addr);
                }
            }
                ...
                case _imgo_:
                    {
                        ...
                        if( _header_data[Header_IMGO::E_IMG_PA] == 0){
                            CAM_FUNC_INF("%s:wait 1 more signal for header output, waiting PA:0x%x\n",
                                this->name_Str(), _buf.image.mem_info.pa_addr);
                            bWaitPass = MFALSE;
                            _DMA_Header_Dump(_header_data, this->name_Str());
                        }
                        else
                            bWaitPass = MTRUE;
                    }
...
E_BC_STATUS CAM_BUF_CTRL::dropEnqueueHwBuf() {
    E_BC_STATUS ret = eCmd_Pass;
    MUINT32 _size;
...
MBOOL CAM_BUF_CTRL::updateState(CAM_BUF_CTRL::E_CAM_STATE camState, ISP_HW_MODULE camModule, E_CAM_DMA_TYPE dmaType)
{
    ...
    if (camState == CAM_BUF_CTRL::eCamState_suspend) {
        CAM_SFUNC_INF("[%d] updateState to eCamState_suspend\n", camModule);
    }
...
// For StatisticPipe::suspend(), disable this and then do checking in dequeueHwBuf()
#if 0
    this->m_Queue_deque_ptr.sendCmd(QueueMgr<MUINTPTR>::eCmd_getsize,(MUINTPTR)&_size,0);
    if(_size <1){
        CAM_FUNC_ERR("queue depth is empty for deque\n");
