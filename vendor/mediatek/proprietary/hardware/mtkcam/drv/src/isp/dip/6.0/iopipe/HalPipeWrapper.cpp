    ...
    , m_iDequeFrmNum(0)
{
    ...
    m_BTDump = ::property_get_int32("vendor.imgsys.bittruedump.enable", 0);
            ...
            if(pVirIspAddr != 0)
            {
                    if(framePack.rParams.mvFrameParams[q].mTuningData!=NULL) {
                        sprintf(filename, "%s/CompareWBwithTuning_Configured_HWBuf_FN(%d)_RN(%d)_TS(0x%x)_UK(0x%x)_Tag(%d)_pTuningQue_frm(%d)_size(%zu).%s",DUMP_DIPPREFIX,
                                            ...
                                            sizeof(dip_x_reg_t),
                                            "reg");
                        compareTuningWithWorkingBuffer(filename, (MUINT32*)pVirIspAddr, (MUINT32*)framePack.rParams.mvFrameParams[q].mTuningData, sizeof(dip_x_reg_t));
                    }
            }
       }
        if(!ret)
