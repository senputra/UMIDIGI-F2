...
#define  LOG__I(...)  do {  __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
#define  LOG__D(...)  do {__android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
#define  LOG__D(...)  do {__android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, __VA_ARGS__);\
                            printf(__VA_ARGS__); } while(0)
...
saveBufToFile(char const*const fname, MUINT8 *const buf, MUINT32 const size)
{
    ...
    LOG__I("(name, buf, size) = (%s, %p, %d)", fname, reinterpret_cast<void*>(buf), size);
    ...
    */
    ...
    if(this->bypass_imgo == MFALSE){
        ...
        for(MUINT32 i=0;i<(this->m_subSample+1);i++){
            ...
            LOG__I("##############################\n");
            LOG__I("enque_imgo_%d: size:0x%08x_0x%08x, pa:0x%p_0x%p, magic:0x%x, crop:%d_%d_%d_%d, rawtype:%d, ring_cnt:%d\n",\
                i,\
                rQBufInfo.vBufInfo.at(i).u4BufSize[0],\
                rQBufInfo.vBufInfo.at(i).Frame_Header.u4BufSize,\
                (void*)rQBufInfo.vBufInfo.at(i).u4BufPA[0],\
                (void*)rQBufInfo.vBufInfo.at(i).Frame_Header.u4BufPA,\
...
void TEST_CAMSVIO::dump_camsv_reg(void) {
    int addr=0x0500;    //CAMSV_0_TG_SEN_MODE
    int addr1=0x0504;   //CAMSV_0_TG_VF_CON
    int addr2=0x0508;   //CAMSV_0_TG_SEN_GRAB_PXL
    LOG__I("%s %d, Reg(0x1a05%04x)=0x%08x; Reg(0x1a05%04x)=0x%08x; \
Reg(0x1a05%04x)=0x%08x\n", __FUNCTION__, __LINE__,
        addr, g_pDrvCamsv[0]->readReg(addr),
        addr1, g_pDrvCamsv[0]->readReg(addr1),
        addr2, g_pDrvCamsv[0]->readReg(addr2)
        );

    addr=0x050C;    //CAMSV_0_TG_SEN_GRAB_LIN
    addr1=0x0538;   //CAMSV_0_TG_FRMSIZE_ST
    addr2=0x0010;   //CAMSV_MODULE_EN
    LOG__I("%s %d, Reg(0x1a05%04x)=0x%08x; Reg(0x1a05%04x)=0x%08x; \
Reg(0x1a05%04x)=0x%08x\n", __FUNCTION__, __LINE__,
        addr, g_pDrvCamsv[0]->readReg(addr),
        addr1, g_pDrvCamsv[0]->readReg(addr1),
    ...
    addr=0x0014;    //CAMSV_FMT_SEL
    addr1=0x0018;   //CAMSV_INT_EN
    addr2=0x001C;   //CAMSV_INT_STATUS
    LOG__I("%s %d, Reg(0x1a05%04x)=0x%08x; Reg(0x1a05%04x)=0x%08x; \
Reg(0x1a05%04x)=0x%08x\n", __FUNCTION__, __LINE__,
        addr, g_pDrvCamsv[0]->readReg(addr),
        addr1, g_pDrvCamsv[0]->readReg(addr1),
    ...
    addr=0x0230;    //CAMSV_0_IMGO_XSIZE
    addr1=0x0234;   //CAMSV_0_IMGO_YSIZE
    addr2=0x0238;   //CAMSV_0_IMGO_STRIDE
    LOG__I("%s %d, Reg(0x1a05%04x)=0x%08x; Reg(0x1a05%04x)=0x%08x; \
Reg(0x1a05%04x)=0x%08x\n", __FUNCTION__, __LINE__,
        addr, g_pDrvCamsv[0]->readReg(addr),
        addr1, g_pDrvCamsv[0]->readReg(addr1),
...
int TEST_CAMSVIO::deque(void)
{
    ...
    if(this->bypass_imgo == MFALSE) {
        ...
        g_pDrvCamsv[0]->setRWMode(ISP_DRV_RW_IOCTL);//specail control for start seninf , for test code only
        int addr=0x0D14;    //SENINF1_MUX_DEBUG_2   1A040D14
        LOG__I("%s %d, After deque: Reg(0x1A04%04x)=0x%08x\n", __FUNCTION__, __LINE__,
            addr, g_pDrvCamsv[0]->readReg(addr));
        g_pDrvCamsv[0]->setRWMode(ISP_DRV_RW_MMAP);
        dump_camsv_reg();
        ...
        for(UINT32 i=0; i<rQBufInfo.vBufInfo.size(); i++){
            LOG__I("##############################\n");
            LOG__I("deque_imgo_%d: size:0x%08x_0x%08x, pa:0x%p_0x%p, magic:0x%x, time:%d_%d, tg size:%dx%d,crop:%d_%d_%d_%d\n",\
                i,\
                rQBufInfo.vBufInfo.at(i).u4BufSize[0],\
                rQBufInfo.vBufInfo.at(i).Frame_Header.u4BufSize,\
                (void*)rQBufInfo.vBufInfo.at(i).u4BufPA[0],\
        ...
        if (m_imgo_bin == MFALSE){
            ...
            if (stat(SAVE_PATH, &st) == -1) {
                ...
                if(mkdir(SAVE_PATH, 0777) < 0){
                    LOG__I("mkdir path(%s) error", SAVE_PATH);
                }
            }
            else {
            ...
            if (saveBufToFile(filename,
                    reinterpret_cast<MUINT8*>(rQBufInfo.vBufInfo.at(0).u4BufVA[0]),
...
int TEST_CAMSVIO::Case_0(void)
{
        ...
        if(this->bypass_imgo == MFALSE){
            ...
            switch(this->TG_FmtSel){
                ...
                case NSCam::eImgFmt_BAYER12:
                    this->buf_imgo[i].size = tg_size_w*tg_size_h*12/8; //12bit case
                    break;
                case NSCam::eImgFmt_BAYER14:
                    this->buf_imgo[i].size = tg_size_w*tg_size_h*14/8; //14bit case
                    break;
                case NSCam::eImgFmt_JPEG:
                    //To prevent M4U transaction error
                    this->buf_imgo[i].size = tg_size_w*tg_size_h*40/8; //10bit case
                    break;
                case NSCam::eImgFmt_UYVY:
                    this->buf_imgo[i].size = tg_size_w*tg_size_h*2; //10bit case
                    break;
                default:
            ...
            LOG__I("****** %s %d, buf_imgo[%d].virtAddr 0x%p\n", thread, __LINE__, i, (void*)this->buf_imgo[i].virtAddr);
            LOG__I("****** %s %d, buf_imgo[%d].phyAddr  0x%p\n", thread, __LINE__, i, (void*)this->buf_imgo[i].phyAddr);
            //
            ...
            LOG__I("****** %s %d, buf_imgo_fh[%d].virtAddr 0x%p\n", thread, __LINE__, i, (void*)this->buf_imgo_fh[i].virtAddr);
            LOG__I("****** %s %d, buf_imgo_fh[%d].phyAddr  0x%p\n", thread, __LINE__, i, (void*)this->buf_imgo_fh[i].phyAddr);
    ...
    if(this->bypass_imgo == MFALSE){
        capibility CamInfo;
        tCAM_rst rst;
        NSCam::NSIoPipe::NSCamIOPipe::Normalpipe_PIXMODE e_QueryPixMode = NSCam::NSIoPipe::NSCamIOPipe::_1_PIX_MODE;
        //
        ...
        if( this->TG_FmtSel ==  NSCam::eImgFmt_JPEG) {
        ...
        if( this->TG_FmtSel ==  NSCam::eImgFmt_JPEG) {
            this->imgo.u4Stride[ESTRIDE_1ST_PLANE]  = tg_size_w;
        }
        ...
        switch( this->imgo.ePxlMode ) {
            case ePxlMode_One_:  e_QueryPixMode = NSCam::NSIoPipe::NSCamIOPipe::_1_PIX_MODE;break;
            case ePxlMode_Two_:  e_QueryPixMode = NSCam::NSIoPipe::NSCamIOPipe::_2_PIX_MODE;break;
            case ePxlMode_Four_: e_QueryPixMode = NSCam::NSIoPipe::NSCamIOPipe::_4_PIX_MODE;break;
            default: e_QueryPixMode = NSCam::NSIoPipe::NSCamIOPipe::_1_PIX_MODE;break;
        }

        LOG__I("****** %s %d e_QueryPixMode(%d)\n", thread, __LINE__, e_QueryPixMode);
        //C.K. dbg
        LOG__I("index(%d), u4ImgWidth(%d)\n", this->imgo.index, this->imgo.u4ImgWidth);
        if (CamInfo.GetCapibility(
            ...
            if(rst.stride_byte != this->imgo.u4Stride[ESTRIDE_1ST_PLANE]){
                LOG__I("cur dma(%d) stride:0x%x, recommanded:0x%x\n",this->imgo.index,this->imgo.u4Stride[ESTRIDE_1ST_PLANE],rst.stride_byte);
            }
        }
    ...
    //LOG__I("Stop CVD64 here before start\n");
    //scanf("%s", s);
    ...
    LOG__I("###Press any key to continue... or attach CVD here ###\n");
    scanf("%s", s);
...
int main(int argc, char** argv)
{
    int rst = 0;
    char s[16];
    MUINT32 path, subsample = 0, dualCamsv = 0, rawbit=0, pixelm=0;
    MUINT32 _tg_pattern;
    ...
    LOG__I("##############################\n");
    LOG__I("case 0: Camsv, RAW8 IMGO\n");
    LOG__I("case 1: Camsv, RAW10 IMGO\n");
    LOG__I("case 2: Camsv, RAW12 IMGO\n");
    LOG__I("case 3: Camsv, RAW14 IMGO\n");
    LOG__I("case 4: Camsv, JPEG\n");
    LOG__I("case 5: Camsv, YUV\n");
    LOG__I("select RAW format\n");
    LOG__I("##############################\n");
    scanf("%s", s);
    rawbit = atoi((const char*)s);
    if(rawbit < 0 || rawbit > 5)
        rawbit = 0;
    LOG__I("select raw format=%d\n",rawbit);

    LOG__I("##############################\n");
    LOG__I("case 1: 1 pix\n");
    LOG__I("case 2: 2 pix\n");
    LOG__I("case 3: 4 pix\n");
    LOG__I("select pixel mode\n");
    LOG__I("##############################\n");
    scanf("%s", s);
    pixelm = atoi((const char*)s);
    if(pixelm < 1 || pixelm > 3)
        pixelm = 1;
    //C.K. add, YUV pixel must >=2
    if(rawbit==5 && pixelm==1)
        pixelm = 2;
    LOG__I("select pixel mode=%d\n",pixelm);

    path = rawbit*3 + pixelm;

    //getchar();
    LOG__I("##############################\n");
    LOG__I("tg pattern\n");
    LOG__I("  0: single color\n");
    LOG__I("  1: gray level\n");
    LOG__I("  2: color bar\n");
    LOG__I("##############################\n");
    scanf("%s", s);
    ...
    switch(_tg_pattern){
        case 0:
            ...
            scanf("%s", s);
            _tg_pattern = atoi((const char*)s);
            if(_tg_pattern < 0 || _tg_pattern > 7) _tg_pattern = 0;
            break;
        case 1:
            LOG__I("##############################\n");
            LOG__I("Gray level:\n");
            LOG__I("  1: horizontal gray level (Unit 1)\n");
            LOG__I("  2: horizontal gray level (Unit 4)\n");
            LOG__I("  3: horizontal gray level (Take 1024 pixel as one period) (only bayer)\n");
            LOG__I("  4: vertical gray level (Unit 1)\n");
            LOG__I("##############################\n");
            scanf("%s", s);
            _tg_pattern = atoi((const char*)s)+ 7;
            if(_tg_pattern < 8 || _tg_pattern > 11) _tg_pattern = 8;
            break;
        case 2:
            LOG__I("##############################\n");
            LOG__I("Color bar:\n");
            LOG__I("  1: static horizontal color bar\n");
            LOG__I("  2: static vertical color bar (only bayer)\n");
            LOG__I("  3: R,G,B,W flash every two frame (only bayer)\n");
            LOG__I("  4: Dynamic horizontal colorbar (only bayer)\n");
            LOG__I("##############################\n");
            scanf("%s", s);
            _tg_pattern = atoi((const char*)s)+ 11;
            if(_tg_pattern < 12 || _tg_pattern > 15) _tg_pattern = 12;
            break;
        default:
            _tg_pattern = 12;
            break;
    ...
    switch(path){
        ...
        case 9:
            switch(path){
                case 7:  _ePixelMode = ePxlMode_One_;break;
                case 8:  _ePixelMode = ePxlMode_Two_;break;
                case 9:  _ePixelMode = ePxlMode_Four_;break;
            }
            LOG__I("****** %s %d, BAYER12 --------------------------\n", __FUNCTION__, __LINE__);
            Seninf_Pattern(1,MFALSE,_ePixelMode,_tg_pattern);
            LOG__I("****** %s %d ----------------------------------\n", __FUNCTION__, __LINE__);
        ...
        case 12:
            switch(path){
                case 10:  _ePixelMode = ePxlMode_One_;break;
                case 11:  _ePixelMode = ePxlMode_Two_;break;
                case 12:  _ePixelMode = ePxlMode_Four_;break;
            }
            LOG__I("****** %s %d, BAYER14 --------------------------\n", __FUNCTION__, __LINE__);
            Seninf_Pattern(1,MFALSE,_ePixelMode,_tg_pattern);
            LOG__I("****** %s %d ----------------------------------\n", __FUNCTION__, __LINE__);
        ...
        case 15:
            switch(path){
                case 13:  _ePixelMode = ePxlMode_One_;break;
                case 14:  _ePixelMode = ePxlMode_Two_;break;
                case 15:  _ePixelMode = ePxlMode_Four_;break;
            }
            LOG__I("****** %s %d, JPEG --------------------------\n", __FUNCTION__, __LINE__);
            Seninf_Pattern(1,MTRUE,_ePixelMode,_tg_pattern);
            LOG__I("****** %s %d ----------------------------------\n", __FUNCTION__, __LINE__);
            pTEST_CAMSVIO = new TEST_CAMSVIO(0); // sensor index 0
            pTEST_CAMSVIO->Surce = TG_CAMSV_0;
            pTEST_CAMSVIO->Source_Size = E_8M;
            pTEST_CAMSVIO->TG_PixMode = _ePixelMode;
            pTEST_CAMSVIO->TG_FmtSel = NSCam::eImgFmt_JPEG;
            pTEST_CAMSVIO->bypass_imgo = MFALSE;
            pTEST_CAMSVIO->bEnque_before_start = MTRUE;
            LOG__I("****** %s %d ----------------------------------\n", __FUNCTION__, __LINE__);
            pTEST_CAMSVIO->m_threadName = thread0_name;
        ...
        case 18:
            switch(path){
                case 16:  _ePixelMode = ePxlMode_One_;break;
                case 17:  _ePixelMode = ePxlMode_Two_;break;
                case 18:  _ePixelMode = ePxlMode_Four_;break;
            }
            LOG__I("****** %s %d, YUV --------------------------\n", __FUNCTION__, __LINE__);
            Seninf_Pattern(1,MTRUE,_ePixelMode,_tg_pattern);
            LOG__I("****** %s %d ----------------------------------\n", __FUNCTION__, __LINE__);
            ...
            pTEST_CAMSVIO->Source_Size = E_8M;
            pTEST_CAMSVIO->TG_PixMode = _ePixelMode;
            pTEST_CAMSVIO->TG_FmtSel = NSCam::eImgFmt_UYVY;
            pTEST_CAMSVIO->bypass_imgo = MFALSE;
            pTEST_CAMSVIO->bEnque_before_start = MTRUE;
            LOG__I("****** %s %d ----------------------------------\n", __FUNCTION__, __LINE__);
            pTEST_CAMSVIO->m_threadName = thread0_name;
