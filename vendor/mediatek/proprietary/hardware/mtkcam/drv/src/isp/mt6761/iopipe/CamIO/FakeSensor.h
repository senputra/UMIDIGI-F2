...
public:
    ...
    virtual MUINT               queryNumberOfSensors() const { return 2; }
    virtual IMetadata const&    queryStaticInfo(MUINT const index) const { UNUSED(index); return mDummyMetadata; }
    virtual char const*         queryDriverName(MUINT const index) const { UNUSED(index); return "Fakegarnett"; }
    virtual MUINT               queryType(MUINT const index)const { UNUSED(index); return 0; }
    virtual MUINT               queryFacingDirection(MUINT const index) const { UNUSED(index); return 0; }
