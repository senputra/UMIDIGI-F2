...

{
    ...
    this->m_TuningDrvUser = eTuningDrvUser_TuningUserNum;
...
TuningDrv* TuningDrv::getInstance(MUINT32 sensorIndex)
{
    ...
    LOG_DBG("+getInstance, sensorIndex: %d", sensorIndex);
...
MBOOL TuningDrvImp::init(const char* userName, ETuningDrvUser user, MUINT32 BQNum)
{
    ...
    if(user >= eTuningDrvUser_TuningUserNum)
    {
        ...
        goto EXIT;
         ...
         this->m_TuningDrvUser = user; // recode TuningDrvUser for SNVR camiopipe calling
    }
...
MBOOL TuningDrvImp::deTuningQue(ETuningDrvUser user, MINT32* magicNum, MBOOL* isVerify, vector<stTuningNode*> &vpTuningNodes, MBOOL bFirst)
{
    MBOOL ret = MTRUE;
    MUINT32 i;
    ...
    if(magicNum == NULL) {
        LOG_ERR("TuningUser(%d) input NULL magicNum !!!", user);
        ret = MFALSE;
        goto EXIT;
    }
    ...
    if(searchTuningQue(user, magicNum, MFALSE) != MTRUE){
        goto FIRST_DETUNING;
    }
...
FIRST_DETUNING:
    ...
    for(i = 0; i < this->mBQNum; i++)
    {
        ...
        this->mTuningQue[user].pTuningNode[curTuningNodeId].ctlEn3_CAM = 0;
...
{
    MUINT32 ctlEn1_CAM = 0;
    MUINT32 ctlEn2_CAM = 0;
    MUINT32 ctlEn3_CAM = 0;
    MUINT32 ctlEn4_CAM = 0;

    MUINT32 ignoreBit1 = 0xFFFFFFFF;
    MUINT32 ignoreBit2 = 0xFFFFFFFF;
    MUINT32 ignoreBit3 = 0xFFFFFFFF;
    MUINT32 ignoreBit4 = 0xFFFFFFFF;
    ...
    enum {
        ...
        querModuleDMAEn3   = 2,
        querModuleDMAEn4   = 3,
        querModuleDMAEnNUM = 4,
    };

    MBOOL ret = MTRUE;
    MUINT32 subSample, page = 0;
    vector <stTuningNode*> vpTuningNodes;

    if (moduleEn->size() != querModuleDMAEnNUM) {
       LOG_ERR("moduleEn input Number(%lu) Error", moduleEn->size());
       ret = MFALSE;
       goto EXIT;
    }

    if (MFALSE == this->deTuningQueByDrv(this->m_TuningDrvUser,
                                         (MINT32*)&magicNum,
                                         vpTuningNodes)) {
        LOG_ERR("de tuning que fail\n");
        ret = MFALSE;
        goto EXIT;
    }

    subSample = this->getBQNum();
    ...
    // DGN & LTM are controled by CCU.
    ignoreBit1 = ~(DGN_R1_EN_ | LTMS_R1_EN_);

    ctlEn1_CAM = (vpTuningNodes[page]->ctlEn_CAM  & ignoreBit1);
    ctlEn2_CAM = (vpTuningNodes[page]->ctlEn2_CAM & ignoreBit2);
    ctlEn3_CAM = (vpTuningNodes[page]->ctlEn3_CAM & ignoreBit3);
    ctlEn4_CAM = (vpTuningNodes[page]->ctlEn4_CAM & ignoreBit4);


    moduleEn->clear();
    moduleEn->push_back(ctlEn1_CAM);
    moduleEn->push_back(ctlEn2_CAM);
    moduleEn->push_back(ctlEn3_CAM);
    moduleEn->push_back(ctlEn4_CAM);
