...
#define MAKE_SENINF_REG(_a, _r)     (_a - SENINF1_BASE + _r)
...
void TS_FakeDrvSensor::adjustPatternSize(MUINT32 Height)
{
    ...
    if (m_pIspDrv){
        ...
        if(pattern_type == H_BAR){
            ...
            height = (reg >>16) & 0xffff;

            if(height < Height)
                height = Height;

            if((height - Height) < size_diff_threadhold){
                height += size_diff_threadhold;
                reg = ((reg & 0xffff)|(height<<16));
        ...
        MUINT32 const*pSetingTbl)
{
    ...
    case 0x3:
        ...
        break;
    default:
        TS_LOGD("Error: wrong pixel mode %d",
    ...
    if (pSetingTbl != NULL) {
        ...
        for(i=0;pSetingTbl[i] != 0xffffffff;i++){
            num++;
        }
        ...
        for (i = 0; i < num; i+=2) {
            m_pIspDrv->writeReg(MAKE_SENINF_REG(pSetingTbl[i], 0), pSetingTbl[i+1]);
#if 1
            TS_LOGD(" [0x%08x]=0x%08x, 0x%08x\n", pSetingTbl[i], pSetingTbl[i+1],
                m_pIspDrv->readReg(MAKE_SENINF_REG(pSetingTbl[i], 0)));
#endif
        }
...
#if 1
{
    ...
    m_pIspDrv->writeReg(MAKE_SENINF_REG(SENINF1_CTRL, 0), (reg|0x1001));
