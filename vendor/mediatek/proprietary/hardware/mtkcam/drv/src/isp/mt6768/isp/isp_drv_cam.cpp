...
MBOOL ISP_DRV_CAM::CQ_Allocate_method1(MUINT32 step)
{
    ...
    switch(step){
        case 0:
            // allocate virCmdQ
            LOG_DBG("virQ rage:0x%zx",m_IspVirRegSize_method1);
...
MBOOL ISP_DRV_CAM::getDeviceInfo(E_DEVICE_INFO eCmd,MUINT8* pData)
{
    ...
    switch(eCmd){
        ...
        case _GET_START_TIME:
            ...
            LOG_INF("_GET_START_TIME Check pData size:%d",sizeof(pData));
            break;

        case _GET_TRANSFOR_CCU_REG:

            if(ioctl(this->m_pIspDrvImp->m_Fd,ISP_TRANSFOR_CCU_REG,(unsigned char*)pData) < 0){
                LOG_INF("_GET_TRANSFOR_CCU_REG Check pData 0 size:%d",sizeof(pData));
                LOG_ERR("_GET_TRANSFOR_CCU_REG fail\n");
                rst = MFALSE;
            }
            LOG_INF("_GET_TRANSFOR_CCU_REG Check pData 1 size:%d",sizeof(pData));
            break;
