...
//    #include "core\drv\inc\drv_log.h"
//    #define NEW_LINE_CHAR   ""      // ALOG?() already includes a new line char at the end of line, so don't have to add one.
//#include "Log.h"
    #include <mtkcam/utils/std/Log.h>
#include <mtkcam/utils/std/ULog.h>

    #define BASE_LOG_VRB(fmt, arg...)       CAM_ULOGV(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] " fmt, __func__, ##arg)    // <Verbose>: Show more detail debug information. E.g. Entry/exit of private function; contain of local variable in function or code block; return value of system function/API...
    #define BASE_LOG_DBG(fmt, arg...)       CAM_ULOGD(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] " fmt, __func__, ##arg)    // <Debug>: Show general debug information. E.g. Change of state machine; entry point or parameters of Public function or OS callback; Start/end of process thread...
    #define BASE_LOG_INF(fmt, arg...)       CAM_ULOGD(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] " fmt, __func__, ##arg)    // <Info>: Show general system information. Like OS version, start/end of Service...
    #define BASE_LOG_WRN(fmt, arg...)       CAM_ULOGW(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s] WARNING: " fmt, __func__, ##arg)    // <Warning>: Some errors are encountered, but after exception handling, user won't notice there were errors happened.
    #define BASE_LOG_ERR(fmt, arg...)       CAM_ULOGE(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, DBG_LOG_TAG "[%s, %s, line%04d] ERROR: " fmt, __FILE__, __func__, __LINE__, ##arg)    // When MP, will only show log of this level. // <Fatal>: Serious error that cause program can not execute. <Error>: Some error that causes some part of the functionality can not operate normally.
#if 0
    #define BASE_LOG_AST(cond, fmt, arg...)     \
        do {        \
            if (!(cond))        \
                CAM_ULOGE(NSCam::Utils::ULog::MOD_DRV_NORMAL_PIPE, "[%s, %s, line%04d] ASSERTION FAILED! : " fmt NEW_LINE_CHAR, __FILE__, __func__, __LINE__, ##arg);        \
        } while (0)
#endif
