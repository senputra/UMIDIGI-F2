...
SwHwScenarioPathMapping mSwHwPathMapping[eSoftwareScenario_total_num]=
{
    ...
    {eSoftwareScenario_Main2_VSS_Capture,     "Main2_VSSCapture",       NSImageioIsp3::NSIspio::eDrvScenario_VSS,                NSImageioIsp3::NSIspio::EPipePass_PASS2,   EPathCQ_CQ1},
    ...
    {eSoftwareScenario_Main2_IP_Raw_TPipe_Stream, "Main2_IPRawTPipeStream",    NSImageioIsp3::NSIspio::eDrvScenario_IP_TPipe,    NSImageioIsp3::NSIspio::EPipePass_PASS2C,    EPathCQ_CQ3}
    //
};
...
)
{
    ...
    if (0 == callerPostProc || ! ((NSImageioIsp3::NSIspio::IPostProcPipe *)callerPostProc)->init())
    {
    ...
    // create imem driver
        ...
        LOG_DBG("[m_pIMemDrv]:0x%lx ",(unsigned long)m_pIMemDrv);
        if ( NULL == m_pIMemDrv ) {
        ...
        if(!gTuningQueNode.empty() || !gTuningQueData.empty()) {
            ...
            for(int i=0;i<DEFAULT_P2_TUNING_QUE_BUFFER_NUM;i++) {
                ...
                #endif
                LOG_INF("[TuningQue]i(%d),VA(0x%lu),size(0x%x)\n",i,(unsigned long)(tuningQueDataNew[i].bufferInfo.virtAddr),mP2TuningQueBufferSize);
    ...
    NSImageioIsp3::NSIspio::IPostProcPipe * callerPostProc)
{
    ...
    if(mInitCount==0)
    {
        ...
        if(mTuningQueNum != gTuningQueData.size()){
            LOG_ERR("[Error]gTuningQueData number(%d) is different from that be alloced(%lu)",
                mTuningQueNum,(unsigned long)(gTuningQueData.size()));
            return -1;
        }
        if(mTuningQueNum != gTuningQueNode.size()){
            LOG_ERR("[Error]gTuningQueNode number(%d) is different from that be alloced(%lu)",
                mTuningQueNum,(unsigned long)(gTuningQueNode.size()));
            return -1;
        }

        for(unsigned int i=0;i<mTuningQueNum;i++) {
            if((gTuningQueData[i].bufferInfo.memID>=0) && (gTuningQueData[i].bufferInfo.virtAddr!=0)){
                    ...
                    gTuningQueData[i].bufferInfo.virtAddr = 0;
                    gTuningQueData[i].bufferInfo.memID = -1;
                    gTuningQueNode[i] = NULL;
                #endif
            } else {
                LOG_ERR("[Error]free TuningQue error i(%d),fd(%d),va(0x%08lu)",i,gTuningQueData[i].bufferInfo.memID,(unsigned long)(gTuningQueData[i].bufferInfo.virtAddr));
            }
        }
        gTuningQueData.erase(gTuningQueData.begin(),gTuningQueData.end());
        gTuningQueNode.erase(gTuningQueNode.begin(),gTuningQueNode.end());
        LOG_DBG("[TuningQueSize]%lu-%lu",(unsigned long)(gTuningQueData.size()),(unsigned long)(gTuningQueNode.size()));
    ...
    QFeatureData& rFeatureData)
{
    ...
    if(misV3)
    {
        ...
        //bufPackageNum += rParams.mvFrameParams[i].mTuningData;
    }
    ...
    LOG_INF("bufPackageNum(%d),misV3(%d),tuningData(%lu)",bufPackageNum,misV3,(unsigned long)(rParams.mvFrameParams.size()));
    //
    ...
    int pixelInByte=1;
    // IP base no V1
    #if 0 //(IP_BASE_ISP_3 == 0) // IP base no V1
    ...
    {    //original version
        ...
        NSImageio::NSIspio::PortInfo ports[100];
    ...
    {   //enque by vector, v3 and v1 [v3 only support vector enque]
        ...
        for(int q=0;q<bufPackageNum;q++)
        {
            //mvin
            tmpC=0;
            for(int i=inStartIdx;i< rParams.mvFrameParams[q].mvIn.size() ;i++)
            {
                //check cur element is next buffer group or not
                if(tmpC==0)
                {    //first element in this buffer group
                    curgroup=rParams.mvFrameParams[q].mvIn[i].mPortID.group;
                    inNum[q]=0;
                }
                if(rParams.mvFrameParams[q].mvIn[i].mPortID.group != curgroup)
                {
                ...
                //port num
                inNum[q]++;
                inPorts+=rParams.mvFrameParams[q].mvIn[i].mBuffer->getPlaneCount();    //total include plane count
                tmpC++;
            }
            //mvout
            tmpC=0;
           	for(int i=outStartIdx;i<rParams.mvFrameParams[q].mvOut.size();i++)
            {
               	//check cur element is next buffer group or not
	            if(tmpC==0)
                {    //first element in this buffer group
       	            curgroup=rParams.mvFrameParams[q].mvOut[i].mPortID.group;
           	        outNum[q]=0;
               	}
               	if(rParams.mvFrameParams[q].mvOut[i].mPortID.group != curgroup)
	            {
	            ...
	            //port num
                outNum[q]++;
       	        if(rParams.mvFrameParams[q].mvOut[i].mPortID.index==NSImageio::NSIspio::EPortIndex_IMG3O)
           	    {   //img3o support multi-planes
               	    outPorts+=rParams.mvFrameParams[q].mvOut[i].mBuffer->getPlaneCount();        //total include plane count
               	}
	            else
                {   //others are 1 plane
       	            //we do not take care multi-plane output for mdp ouput ports
		...
		for(int j=0;j<rParams.mvFrameParams.size();j++)
		{
			for(int i=0;i<rParams.mvFrameParams[j].mvModuleData.size();i++)
    	    {
        	    moduletag=rParams.mvFrameParams[j].mvModuleData[i].moduleTag;
            	switch(moduletag)
	            {
        ...
        //MINT32 inEndIdx=0,outEndIdx=0; //not used
        MUINT32 dmaEnPort=0x0;
        //int crop1StartIdx=0; //not used
        ...
        for(int q=0;q<bufPackageNum;q++)
        {
            //LOG_INF("start to parse information, (%lu_%lu_%lu)",(unsigned long)rParams.mvFrameParams[q].mvIn.size(),(unsigned long)rParams.mvFrameParams[q].mvOut.size(),(unsigned long)rParams.mvFrameParams[q].mvModuleData.size());
            ...
            for(MUINT32 i=0;i< rParams.mvFrameParams[q].mvIn.size() ;i++)
            {
                ...
                LOG_DBG("mPortID [%d].index: %d, p2PortIdx: %d", i, rParams.mvFrameParams[q].mvIn[i].mPortID.index, p2PortIdx);
                if(p2PortIdx == EPostProcPortIdx_NOSUP)
                {
                ...
                if(rParams.mvFrameParams[q].mvIn[i].mPortID.index == NSImageio::NSIspio::EPortIndex_IMGI)
                {
                    if(rParams.mvFrameParams[q].mvCropRsInfo.size() >0)
                    {
                        ...
                        ports[cnt].resize3.tar_w = 0;
                        ports[cnt].resize3.tar_h = 0;
                        for(MUINT32 k=0;k<rParams.mvFrameParams[q].mvCropRsInfo.size();k++)
                        {
                        ...
                        /*
                        LOG_INF("[Crop] g1 x/fx/y/fy/w/h/tw/th(%d/%d/%d/%d/%d/%d/%d/%d),g2 x/fx/y/fy/w/h/tw/th(%d/%d/%d/%d/%d/%d/%d/%d),g2 x/fx/y/fy/w/h/tw/th(%d/%d/%d/%d/%d/%d/%d/%d)",\
                                ports[cnt].crop1.x,ports[cnt].crop1.floatX,ports[cnt].crop1.y,ports[cnt].crop1.floatY,ports[cnt].crop1.w,ports[cnt].crop1.h,ports[cnt].resize1.tar_w,\
                                ports[cnt].resize1.tar_h,ports[cnt].crop2.x,ports[cnt].crop2.floatX,ports[cnt].crop2.y,ports[cnt].crop2.floatY,ports[cnt].crop2.w,ports[cnt].crop2.h,\
                                ports[cnt].resize2.tar_w,ports[cnt].resize2.tar_h,ports[cnt].crop3.x,ports[cnt].crop3.floatX,ports[cnt].crop3.y,ports[cnt].crop3.floatY,ports[cnt].crop3.w,ports[cnt].crop3.h,\
                                ports[cnt].resize3.tar_w,ports[cnt].resize3.tar_h);
                ...
                ports[cnt].index       = rParams.mvFrameParams[q].mvIn[i].mPortID.index;
...
#endif
                /*
                LOG_INF("[mvIn (%d)] fmt(%d),w/h(%d/%d),oft(%lu/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lu,0x%lu,0x%lu)/pa(0x%lu,0x%lu,0x%lu)/size(0x%x,0x%x,0x%x) )",\
                    ports[cnt].index,ports[cnt].eImgFmt,ports[cnt].u4ImgWidth,ports[cnt].u4ImgHeight,(unsigned long)(rParams.mvFrameParams[q].mvIn[i].mBuffer->getExtOffsetInBytes(0)),\
                    ports[cnt].xoffset,ports[cnt].yoffset,ports[cnt].u4Stride[0],ports[cnt].u4Stride[1],ports[cnt].u4Stride[2],\
                    ports[cnt].capbility,(unsigned long)(ports[cnt].u4BufVA[0]),(unsigned long)(ports[cnt].u4BufVA[1]),(unsigned long)(ports[cnt].u4BufVA[2]),\
                    (unsigned long)(ports[cnt].u4BufPA[0]),(unsigned long)(ports[cnt].u4BufPA[1]),(unsigned long)(ports[cnt].u4BufPA[2]),\
                    ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2]);
                */
                vPostProcInPorts.at(inCnt)=&(ports[cnt]);
                cnt++;
            ...
            for(MUINT32 i=0;i< rParams.mvFrameParams[q].mvOut.size() ;i++)
            {
                ...
                LOG_DBG("mvOut mPortID.index: %d, p2PortIdx: %d",rParams.mvFrameParams[q].mvOut[i].mPortID.index, p2PortIdx);
                if(p2PortIdx == EPostProcPortIdx_NOSUP)
                {
                ...
                ports[cnt].index       = rParams.mvFrameParams[q].mvOut[i].mPortID.index;
...
#endif
                /*
                LOG_INF("[mvOut (%d)] fmt(%d),w/h(%d/%d),oft(%lu/%d/%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lu,0x%lu,0x%lu)/pa(0x%lu,0x%lu,0x%lu)/size(0x%x,0x%x,0x%x) )",\
                    ports[cnt].index,ports[cnt].eImgFmt,ports[cnt].u4ImgWidth,ports[cnt].u4ImgHeight,(unsigned long)(rParams.mvFrameParams[q].mvOut[i].mBuffer->getExtOffsetInBytes(0)),\
                    ports[cnt].xoffset,ports[cnt].yoffset,ports[cnt].u4Stride[0],ports[cnt].u4Stride[1],ports[cnt].u4Stride[2],\
                    ports[cnt].capbility,(unsigned long)(ports[cnt].u4BufVA[0]),(unsigned long)(ports[cnt].u4BufVA[1]),(unsigned long)(ports[cnt].u4BufVA[2]),\
                    (unsigned long)(ports[cnt].u4BufPA[0]),(unsigned long)(ports[cnt].u4BufPA[1]),(unsigned long)(ports[cnt].u4BufPA[2]),\
                    ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2]);
            ...
            MUINT32 featureEntag=0x0;
            MINT32 moduletag=0;
            for(unsigned int i=0;i<rParams.mvFrameParams[q].mvModuleData.size();i++)
            {
                ...
                else
                {
                    ...
                    {
                        case NSImageioIsp3::NSIspio::EP2Module_STA_FEO:
                            LOG_DBG("[mvOut (%d)] fmt(%d),w(%d),h(%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lu,0x%lu,0x%lu)/pa(0x%lu,0x%lu,0x%lu)/size(0x%x,0x%x,0x%x) )",\
                                ports[cnt].index,ports[cnt].eImgFmt,ports[cnt].u4ImgWidth,ports[cnt].u4ImgHeight,ports[cnt].u4Stride[0],ports[cnt].u4Stride[1],ports[cnt].u4Stride[2],\
                                ports[cnt].capbility,(unsigned long)(ports[cnt].u4BufVA[0]),(unsigned long)(ports[cnt].u4BufVA[1]),(unsigned long)(ports[cnt].u4BufVA[2]),\
                                (unsigned long)(ports[cnt].u4BufPA[0]),(unsigned long)(ports[cnt].u4BufPA[1]),(unsigned long)(ports[cnt].u4BufPA[2]),\
                                ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2]);
                            ...
                            break;
                         case NSImageioIsp3::NSIspio::EP2Module_STA_LCEI:
                            LOG_DBG("[mvIn (%d)] fmt(%d),w(%d),h(%d),stride(%d/%d/%d),cap(%d),mem( va(0x%lu,0x%lu,0x%lu)/pa(0x%lu,0x%lu,0x%lu)/size(0x%x,0x%x,0x%x) )",\
                                ports[cnt].index,ports[cnt].eImgFmt,ports[cnt].u4ImgWidth,ports[cnt].u4ImgHeight,ports[cnt].u4Stride[0],ports[cnt].u4Stride[1],ports[cnt].u4Stride[2],\
                                ports[cnt].capbility,(unsigned long)(ports[cnt].u4BufVA[0]),(unsigned long)(ports[cnt].u4BufVA[1]),(unsigned long)(ports[cnt].u4BufVA[2]),\
                                (unsigned long)(ports[cnt].u4BufPA[0]),(unsigned long)(ports[cnt].u4BufPA[1]),(unsigned long)(ports[cnt].u4BufPA[2]),\
                                ports[cnt].u4BufSize[0],ports[cnt].u4BufSize[1],ports[cnt].u4BufSize[2]);
            ...
            bufPackage.callerID=(MUINTPTR)(&callerDeQList);
            bufPackage.bufTag=bufferTag;
            LOG_DBG("ecID(0x%lu),tag(%d),p2cqIdx(%d),p2cqDupIdx(%d),p2BurstQIdx(%d)",bufPackage.callerID,bufPackage.bufTag, bufPackage.p2cqIdx, bufPackage.p2cqDupIdx,q);
            ...
            else
            {
                ...
                bufNode.drvSce=bufPackage.drvSce;
                bufNode.callerID=bufPackage.callerID;
                if((unsigned int)bufPackage.idxINbatchLforBnode == mvEnqueuedBufBatch.size())    //update real index for waited added buffer node in batchA
    ...
    QFeatureData& rFeatureData)
{
    ...
    LOG_DBG("timeout(%d),i8TimeoutNs(%lld),callerID(0x%lu)",timeout,(long long)i8TimeoutNs,(unsigned long)(callerID));
    ...
    MINTPTR arg3)
{
    ...
    LOG_DBG("+,cmd(0x%x),arg1(0x%lu),arg2(0x%lu),arg3(0x%lu)",cmd,(unsigned long)(arg1),(unsigned long)(arg2),(unsigned long)(arg3));
    switch (cmd){
    ...
    void* &pTuningQueBuf)
{
    ...
    if(misV3)
    {
        ...
        if(!gTuningQueNode.empty()) {
            ...
            //
            LOG_INF("queNum(%lu),detuningQue(0x%lx)\n",(unsigned long)(gTuningQueNode.size()),(unsigned long)pTuningQueBuf);
        } else {
            ...
            //
            mTuningQueNum++;
            LOG_INF("[Warning]out of tuningque, add one more(0x%lu)\n",(unsigned long)(gTuningQueData[gTuningQueData.size()-1].bufferInfo.virtAddr));
        }

        LOG_INF("[size]gTuningQueData(%lu),gTuningQueNode(%lu)\n",(unsigned long)(gTuningQueData.size()),(unsigned long)(gTuningQueNode.size()));
    }
    else
    {
