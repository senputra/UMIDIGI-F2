...
MBOOL CROP_TEST::Generator_IMGO(EImageFormat ifmt,MRect& oStart,MBOOL bPure)
{
    ...
    switch(this->mFunc){
        case CROP_NONE:
            ...
            //
            getNormalPipeModule()->query(PORT_IMGO.index, ENPipeQueryCmd_X_PIX, ifmt, _size.w, qry);
        ...
        case CROP_SYSMATRIC:
            ...
            getNormalPipeModule()->query(PORT_IMGO.index, ENPipeQueryCmd_CROP_START_X, ifmt, oStart.p.x, qry);
            ...
            //
            getNormalPipeModule()->query(PORT_IMGO.index, ENPipeQueryCmd_CROP_X_PIX, ifmt, _size.w, qry);
            oStart.s.w = qry.x_pix;
            oStart.s.h = _size.h;
...
MBOOL CROP_TEST::Generator_RRZO(EImageFormat ifmt,MRect& oStart)
{
    ...
    switch(this->mFunc){
        case CROP_NONE:
            ...
            //
            getNormalPipeModule()->query(PORT_RRZO.index, ENPipeQueryCmd_X_PIX, ifmt, _size.w, qry);
            oStart.s.w = qry.x_pix;
            oStart.s.h = _size.h;
            break;
        case CROP_SYSMATRIC:
            ...
            getNormalPipeModule()->query(PORT_RRZO.index, ENPipeQueryCmd_CROP_START_X, ifmt, oStart.p.x, qry);
            oStart.p.x = qry.crop_x;
            //
            _size = this->mInput;
            _size.w /= 2;
...
MBOOL CROP_TEST::Generator_RRZO(EImageFormat ifmt,MSize& oStart)
{
    #define CropRatio 8/10
    ...
    getNormalPipeModule()->query(PORT_RRZO.index, ENPipeQueryCmd_BS_RATIO, ifmt, _size.w, qry);

    switch(this->mFunc){
        case CROP_SPECIFIC:
            if( (MUINT32)(_size.w * 100 / this->mInput.w) < qry.bs_ratio){
                LOG_ERR("H-scaling ratio is not supported:target:%d,max%d\n",(_size.w * 100 / this->mInput.w),qry.bs_ratio);
                return MFALSE;
            }
            if( (MUINT32)(_size.h * 100 / this->mInput.h) < qry.bs_ratio){
                LOG_ERR("V-scaling ratio is not supported:target:%d,max%d\n",(_size.h * 100 / this->mInput.h),qry.bs_ratio);
                return MFALSE;
            }
            ...
            break;
        default:
            _size.w = _size.w*qry.bs_ratio/100;
            _size.h = _size.h*qry.bs_ratio/100;
...
MBOOL CROP_TEST::GetScaleInfo(PortID iport,EImageFormat ifmt,MSize& oSize)
{
    switch(iport.index){
        case NSImageio::NSIspio::EPortIndex_IMGO:
        ...
        case NSImageio::NSIspio::EPortIndex_RRZO:
            if(this->m_status_rrzo == _e_crop){
                this->Generator_RRZO(ifmt,oSize);
            }
            else{
                LOG_ERR("flow err, need to crop 1st before sacle\n");
                return MFALSE;
...
MBOOL CROP_TEST::SetInfo(PortID iport,EImageFormat ifmt,MSize iSize)
{
    ...
    switch(iport.index){
        ...
        case NSImageio::NSIspio::EPortIndex_RRZO:
            ...
            getNormalPipeModule()->query(PORT_RRZO.index, ENPipeQueryCmd_X_PIX, ifmt, iSize.w, qry);
