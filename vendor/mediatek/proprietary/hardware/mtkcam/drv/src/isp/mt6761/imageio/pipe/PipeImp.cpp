...
configMdpOutPort(PortInfo const* oImgInfo, MdpRotDMACfg &a_rotDma)
{
    ...
    a_rotDma.crop.h = 0;
    //
    PIPE_DBG("[a_rotDma]w(%lu),h(%lu),stride(pxl)(%lu),pa(0x%lu),va(0x%lu),size(%d)",a_rotDma.size.w,a_rotDma.size.h,a_rotDma.size.stride,(unsigned long)a_rotDma.memBuf.base_pAddr,(unsigned long)a_rotDma.memBuf.base_vAddr,a_rotDma.memBuf.size);
    PIPE_DBG("[a_rotDma]crop(%x,%x,%lu,%lu)_f(0x%x,0x%x)",a_rotDma.crop.x,a_rotDma.crop.y,a_rotDma.crop.w,a_rotDma.crop.h,a_rotDma.crop.floatX,a_rotDma.crop.floatY);
    ...
    PIPE_DBG("[a_rotDma]va[0x%lu,0x%lu,0x%lu]--pa[0x%lu,0x%lu,0x%lu]",
...
********************************************************************************/
MBOOL
PipeImp::configDmaPort(PortInfo const* portInfo,IspDMACfg &a_dma,MUINT32 pixel_Byte,MUINT32 swap,
    MUINT32 isBypassOffset,EIMAGE_STRIDE planeNum, E_ISP_PIXMODE e_PixMode)
{
    ...
    ISP_QuerySize(portInfo->index,ISP_QUERY_XSIZE_BYTE,(EImageFormat)portInfo->eImgFmt,
        a_dma.size.w,queryRst,e_PixMode);
    ...
    PIPE_DBG("w(%lu),h(%lu),stride(%lu),xsize(%lx),crop(%x,%x,%lu,%lu)_f(0x%x,0x%x),ofst_addr(0x%x),pb((0x%x)(+<<2)),va(0x%x),pa(0x%x),pixel_Byte(%d)",a_dma.size.w,a_dma.size.h,a_dma.size.stride, \
