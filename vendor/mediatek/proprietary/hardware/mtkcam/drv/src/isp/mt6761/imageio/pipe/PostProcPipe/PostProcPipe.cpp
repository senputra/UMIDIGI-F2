...
#define FORCE_EN_DIFFERENR_VIEW_TEST 1
...
configPipe(vector<PortInfo const*>const& vInPorts, vector<PortInfo const*>const& vOutPorts, PipePackageInfo *pPipePackageInfo)
{
    ...
    int tdr_dbg_tcm_en = 0x0;	// for control extension
    int mfb_mode = 0;
    //
    int eis_raw_sel = 0;
    //
...
//    }
    ...
    for (MUINT32 i = 0 ; i < vInPorts.size() ; i++ )
    {
        ...
        switch(vInPorts[i]->index)
        {
            case NSImageio::NSIspio::EPortIndex_IMGI:
                ...
                break;
            case NSImageio::NSIspio::EPortIndex_VIPI:    //may multi-plane, vipi~vip3i
                idx_vipi = i;
                ...
                if(vInPorts[i]->u4Stride[ESTRIDE_2ND_PLANE] && vInPorts[i]->u4BufSize[ESTRIDE_2ND_PLANE] && vInPorts[i]->u4BufVA[ESTRIDE_2ND_PLANE]
                    && vInPorts[i]->u4BufPA[ESTRIDE_2ND_PLANE]) { // check vip2i
                    //
                    ...
                    if(vInPorts[i]->u4Stride[ESTRIDE_3RD_PLANE] && vInPorts[i]->u4BufSize[ESTRIDE_3RD_PLANE] && vInPorts[i]->u4BufVA[ESTRIDE_3RD_PLANE]
                            && vInPorts[i]->u4BufPA[ESTRIDE_3RD_PLANE]) { // check vip3i
                        idx_vip3i = i;
                ...
                break;
            case NSImageio::NSIspio::EPortIndex_VIP3I: //only vip3i input(weighting table), there are only vipi+vip3i without vip2i for mfb mixing path
                idx_vip3i = i;
                en_p2_dma |=  CAM_CTL_EN_P2_DMA_VIP3I_EN;
                tcm_en_collect |= CAM_CTL_TPIPE_VIP3I_TCM_EN;
                portInfo_vip3i =  (PortInfo)*vInPorts[idx_vip3i];
            ...
            case NSImageio::NSIspio::EPortIndex_UFDI:
                ...
                if((drvScenario != eDrvScenario_CC_vFB_FB) && (drvScenario != eDrvScenario_CC_MFB_Mixing) &&
        ...
        switch(vOutPorts[i]->index){
            ...
            case NSImageio::NSIspio::EPortIndex_IMG3O:
                ...
                if(vOutPorts[i]->u4Stride[ESTRIDE_2ND_PLANE] && vOutPorts[i]->u4BufSize[ESTRIDE_2ND_PLANE] && vOutPorts[i]->u4BufVA[ESTRIDE_2ND_PLANE]
                    && vOutPorts[i]->u4BufPA[ESTRIDE_2ND_PLANE]) { // check img3bo
                    //
                    ...
                    if(vOutPorts[i]->u4Stride[ESTRIDE_3RD_PLANE] && vOutPorts[i]->u4BufSize[ESTRIDE_3RD_PLANE] && vOutPorts[i]->u4BufVA[ESTRIDE_3RD_PLANE]
                        && vOutPorts[i]->u4BufPA[ESTRIDE_3RD_PLANE]) { // check img3co
                        idx_img3co = i;
            ...
            case NSImageio::NSIspio::EPortIndex_IMGO: // ip raw scenario
                ...
                break;
            case NSImageio::NSIspio::EPortIndex_MFBO: // mfb blending
                idx_mfbo = i;
                portInfo_mfbo =  (PortInfo)*vOutPorts[idx_mfbo];
                en_p2_dma |= CAM_CTL_EN_P2_DMA_MFBO_EN;
                // for control extension
                // MFBO_TCM_EN for CTL_TCM_EN
                tdr_en |= CAM_CTL_TCM_MFBO_TCM_EN;
                break;
            case NSImageio::NSIspio::EPortIndex_VENC_STREAMO:
                idx_venco = i;
    ...
    if(idx_img3o >= 0)
    {
        ...
        else
        {
            ...
            {    //would enable mdp output
	  		    // for crop4 +++++
			    if (portInfo_imgi.crop4.w != 0)
			    {
				    portInfo_img3o.xoffset = portInfo_imgi.crop4.x & (~1);	//img3o offset must be even
		  		    portInfo_img3o.yoffset = portInfo_imgi.crop4.y & (~1);
			    }
			    else
	  		    // for crop4 -----
			    {
    ...
    // for control extension
    // NR3D_TCM_EN
    tcm_en_collect |= ((m_Nr3dEn)?(CAM_CTL_TPIPE_NR3D_TCM_EN):(0));
...
//                en_p2_dma |=  CAM_CTL_DMA_EN_VIPI_EN;
    ...
    PIPE_DBG("drvScenario:[%d],subMode(%d),eImgFmt(%d),dma(0x%x)",drvScenario,subMode,portInfo_imgi.eImgFmt,en_p2_dma);
    ...
    if(pPipePackageInfo->isV3==true){
        if(pPipePackageInfo->pTuningQue != NULL){
            ...
            // for control extension +++++
            // Need to add check tuning TCM enable
            //
            tcm_en_collect |= (en_p2 & CAM_CTL_EN_P2_LCE_EN) << 11; //LCE
            tcm_en_collect |= (en_p2 & CAM_CTL_EN_P2_NR3D_EN) << 14; //NR3D
            tdr_en |= (en_p2 & CAM_CTL_EN_P2_PCA_EN) << 14; //PCA
            tdr_en |= (en_p2 & CAM_CTL_EN_P2_SL2C_EN) << 7; //SL2C
            tdr_en |= (en_p2 & CAM_CTL_EN_P2_SEEE_EN) << 10; //SEEE
            tdr_dbg_tcm_en |= (en_p2 & CAM_CTL_EN_P2_NSL2_EN) << 28; //NSL2
            tdr_dbg_tcm_en |= (en_p2 & (CAM_CTL_EN_P2_NSL2A_EN|CAM_CTL_EN_P2_NSL2B_EN)) << 4; //NSL2A, NSL2B
            tdr_dbg_tcm_en |= (en_p1 & CAM_CTL_EN_P1_RMG_EN) << 4; //RMG
            tdr_dbg_tcm_en |= (en_p1 & CAM_CTL_EN_P1_DBS_EN) << 3; //DBS
            // for control extension -----
            ...
            if(en_p1_dma & CAM_CTL_EN_P1_DMA_LSCI_EN){
                ...
                PIPE_DBG("[LSCI](x,y)=(%lx,%lx),s(%lx),pa(0x%x)",\
            ...
            //
            PIPE_DBG("[V3]pIspPhyReg(0x%08lx)",(unsigned long)pIspPhyReg);
            ...
            PIPE_DBG("[V3_Tuning]en1(0x%08x),en2(0x%08x),dma(0x%08x)",pIspPhyReg->CAM_CTL_EN_P1.Raw,pIspPhyReg->CAM_CTL_EN_P2.Raw,pIspPhyReg->CAM_CTL_EN_P1_DMA.Raw);
    ...
    switch (drvScenario) {
        ...
        case eDrvScenario_CC_vFB_FB:
            ...
            // for control extenstion
            // Need to add MIX1/MIX2/PCA tcm_en
            tdr_en |= CAM_CTL_TCM_MIX1_TCM_EN|CAM_CTL_TCM_MIX2_TCM_EN|CAM_CTL_TCM_PCA_TCM_EN;
    ...
    {
        #if 1  //kk test default:1 ==> need to fix for different view angle
    ...
    PIPE_DBG("Pass(%d),imgi.fmt(0x%x),isCropG1En(%d),m_CrzEn(%d),mdpSrcFmt(0x%x),mdp_src(%lx,%lx,%lx,%lx),en_p2_dma(0x%08x),crop1TableIdx(%d),mdpCheck1(%d),mdpCheck2(%d),drvScenario(%d),tcm_en(0x%x)",\
    ...
    m_camPass2Param.tdr_dbg_tcm_en = tdr_dbg_tcm_en; //0x4088	// for control extension
    ...
    if (-1 != idx_vipi ) {
        ...
        PIPE_DBG("[vipi]size(0x%08x),Va(0x%08x),Pa(0x%08x),offset(0x%08x),align(0x%08x),stride(0x%08lx)",
        ...
        {    //multi-plane input image, vipi/vip2i/vip3i
            ...
            PIPE_DBG("[vip2i]size[%lx,%lx](0x%08x),offset(0x%08x),align(0x%08x),stride(0x%08lx)",m_camPass2Param.vip2i.size.w,m_camPass2Param.vip2i.size.h,
            ...
            if (-1 != idx_vip3i )
            {
                ...
                PIPE_DBG("[vip3i]size[%lx,%lx](0x%08x),Va(0x%08x),Pa(0x%08x),offset(0x%08x),align(0x%08x),stride(0x%08lx),vipi_uv_swap(%d)",m_camPass2Param.vip3i.size.w,m_camPass2Param.vip3i.size.h,
    ...
    if (-1 != idx_img3o ) {
        ...
        PIPE_DBG("[img3o]size(0x%08x),Va(0x%08x),Pa(0x%08x),offset(0x%08x),align(0x%08x),stride(0x%08lx)",
        ...
        if (-1 != idx_img3bo )
        {
            ...
            PIPE_DBG("[img3bo]size[%lx,%lx](0x%08x),offset(0x%08x),align(0x%08x),stride(0x%08lx)",m_camPass2Param.img3bo.size.w,m_camPass2Param.img3bo.size.h,
            ...
            if (-1 != idx_img3co )
            {
                ...
                PIPE_DBG("[img3co]size[%lx,%lx](0x%08x),Va(0x%08x),Pa(0x%08x),offset(0x%08x),align(0x%08x),stride(0x%08lx),img3o_uv_swap(%d)",m_camPass2Param.img3co.size.w,m_camPass2Param.img3co.size.h,
...
*
********************************************************************************/
MBOOL PostProcPipe::endequeFrameCtrl(EPIPE_BUFQUECmd cmd, MUINT32 callerID, MINT32 p2burstQIdx,
