...
class IMemDrvImp : public IMemDrv
{
    ...
    public:
        ...
        virtual MINT32  allocVirtBuf(
            IMEM_BUF_INFO* pInfo);
        virtual MINT32  allocSecBuf(
            IMEM_BUF_INFO* pInfo);
        virtual MINT32  freeVirtBuf(
            IMEM_BUF_INFO* pInfo, MBOOL bImport = MTRUE);
        virtual MINT32  freeSecBuf(
            IMEM_BUF_INFO* pInfo);
        virtual MINT32  mapPhyAddr(
            IMEM_BUF_INFO* pInfo, MBOOL bImport = MTRUE);
        virtual MINT32  unmapPhyAddr(
            IMEM_BUF_INFO* pInfo);
        virtual MINT32  cacheSyncbyRange(IMEM_CACHECTRL_ENUM ctrl, IMEM_BUF_INFO* pInfo, MBOOL bImport = MTRUE);
        //remove later
        virtual MINT32  cacheFlushAll(void);
        virtual MINT32  doIonCacheFlush(IMEM_BUF_INFO* pInfo, void* virAddr, MUINT32 size, eIONCacheFlushType flushtype, MBOOL bImport = MTRUE);
        virtual MINT32  initSecureM4U(void);
        virtual MINT32  allocSecureBuf(
            IMEM_BUF_INFO* pInfo);
        virtual MINT32  freeSecureBuf(
            IMEM_BUF_INFO* pInfo);
