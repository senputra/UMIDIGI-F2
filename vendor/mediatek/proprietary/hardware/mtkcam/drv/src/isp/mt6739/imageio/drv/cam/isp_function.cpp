...
UNUSED(imageioPackage);UNUSED(tuningPackage);
#if 0
...
cam_isp_nsl2a_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
    if(imageioPackage.isApplyTuning){  //0x15004F60 ~ 0x15004F84
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
...
cam_isp_g2g_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ...
    ISP_FUNC_DBG("+,m_pP2IspDrv(0x%08lx),cq(%d),subMode(%d),isApplyTuning(0x%08x)",(unsigned long)m_pP2IspDrv,imageioPackage.cq, imageioPackage.subMode,imageioPackage.isApplyTuning);
...
cam_isp_dbs_cfg(IspImageioPackage imageioPackage, IspTuningPackage tuningPackage) {
    ISP_FUNC_DBG("+,cq(%d),subMode(%d),isApplyTuning(%d)",imageioPackage.cq,imageioPackage.subMode,imageioPackage.isApplyTuning);
    if(imageioPackage.isApplyTuning){  //0x15004C00 ~ 0x15004C30
        ISP_DRV_REG_IO_STRUCT regsStruct[ISP_DRV_MAX_CQ_GROUP_SIZE];
        MUINT32 addrOffset, moduleSize;
        //
        ISP_FUNC_DBG("eTuningCqFunc1(%d),eTuningCqFunc2(%d)",tuningPackage.eTuningCqFunc1,tuningPackage.eTuningCqFunc2);
    ...
    if(imageioPackage.isApplyTuning){  //0x15004800 ~ 0x1500482C
        ...
        if(tuningPackage.eTuningCqFunc1==CAM_ISP_BNR){
           m_pP2IspDrv->getCqModuleInf(tuningPackage.eTuningCqFunc1,addrOffset,moduleSize);
            ...
            ISP_FUNC_DBG("CAM_ISP_BNR: addrOffset(0x%x),moduleSize(%d)",addrOffset,moduleSize);
            for(MUINT32 i=0;i<moduleSize;i++){
                regsStruct[i].Addr = addrOffset;
                regsStruct[i].Data = tuningPackage.pTuningBuf[addrOffset>>2];
                //ISP_FUNC_INF("CAM_ISP_BNR: addrOffset(%d),Data(%d)",regsStruct[i].Addr, regsStruct[i].Data);
                addrOffset += 4;
            }
            ISP_BURST_WRITE_REGS(m_pP2IspDrv, regsStruct, moduleSize);
        }
        //
        ISP_FUNC_INF("cam_isp_bnr_cfg: BPC_LUT_EN (%d)", ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_BNR_BPC_CON, BPC_LUT_EN));
        if (ISP_READ_BITS_NOPROTECT(m_pP2IspDrv, CAM_BNR_BPC_CON, BPC_LUT_EN) == 1) {
            ...
            for(MUINT32 i=0;i<moduleSize;i++){
                regsStruct[i].Addr = addrOffset;
                regsStruct[i].Data = tuningPackage.pTuningBuf[addrOffset>>2];
                ISP_FUNC_DBG("CAM_DMA_BPCI: addrOffset(%d),Data(%d)",regsStruct[i].Addr, regsStruct[i].Data);
                addrOffset += 4;
            }
            ISP_BURST_WRITE_REGS(m_pP2IspDrv, regsStruct, moduleSize);
 
            MUINT32 BPCI_XSIZE, BPCI_YSIZE; //BPCI_BASE_PADDR, BPCI_OFFSET, BPCI_STRIDE //not used
            ...
            ISP_WRITE_REG(m_pP2IspDrv,CAM_BPCI_CON2,0x00202000,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(m_pP2IspDrv,CAM_BPCI_CON,0x80000020,ISP_DRV_USER_ISPF);
...
#endif
    ISP_FUNC_ERR("[Error]CRSP be enabled");
    return 0;
}
//
int
...
cam_isp_rrz_cfg(IspDrv* pCurDrv,IspSize in,IspRect crop,IspSize out,MUINT32 ctrl,MUINT32 h_step,MUINT32 v_step, MUINT32 isRaw_D) {
    ...
    ISP_FUNC_DBG("+[setRRZToCQ](ctrl(0x%x),in(%lu,%lu),out(%lu,%lu),step(0x%x,0x%x),ofst(%d,%d,%d,%d/%lu,%lu))", \
    ...
    if ( !isRaw_D ) {
        ...
        ISP_WRITE_REG(pCurDrv, CAM_RRZ_CROP_IN,(crop.h<<16|crop.w),ISP_DRV_USER_ISPF);//0x15004094
...
int ret = 0;
    ...
    ISP_FUNC_DBG("[m_pIMemDrv]:%p ",(unsigned int*)m_pIMemDrv);
    if ( NULL == m_pIMemDrv ) {
    ...
    ISP_FUNC_INF("[m_pPhyIspDrv]:%p ",(unsigned int*)m_pPhyIspDrv);
    if (!m_pPhyIspDrv) {
    ...
    ISP_FUNC_INF("m_pPhyIspDrv_bak(%p) ",(unsigned int*)m_pPhyIspDrv_bak);
    /*============================================
     control MDP & Tpipe buffer & virtual isp
    =============================================*/
    MINT32 curBurstQNum=P2_Support_BurstQNum;
    ISP_FUNC_DBG("curBurstQNum : %d ", curBurstQNum);
    ...
    ISP_FUNC_DBG("[m_pCrzDrv]:%p ",(unsigned int*)m_pP2CrzDrv);
    if ( NULL == m_pP2CrzDrv ) {
    ...
    ISP_FUNC_DBG("[m_pTwinDrv]:%p ",(unsigned int*)m_pTwinDrv);
    if ( NULL == m_pTwinDrv ) {
    ...
    ISP_FUNC_INF("[m_pIspEventThread]:%p ",(unsigned int*)m_pIspEventThread);
...
//js_test move to _disable.
    ...
    /*============================================
     isp driver
    =============================================*/
    ISP_FUNC_INF("m_pP1IspDrv(%p),m_pP2IspDrv(%p),m_pPhyIspDrv_bak(%p) ",(unsigned int*)m_pP1IspDrv,(unsigned int*)m_pP2IspDrv,(unsigned int*)m_pPhyIspDrv_bak);
    if (m_pPhyIspDrv_bak) {
...
ispDrvSwitch2Virtual(MINT32 cq,MINT32 p1CqD,MINT32 burstQIdx,MINT32 p2DupCqIdx)
{
    ...
    ISP_FUNC_DBG("[ispDrvSwitch2Virtual]CQ(%d), burstQIdx: %d, p2DupCqIdx: %d, index: %d, totalCQNum: %d.", cq, burstQIdx, p2DupCqIdx, index, totalCQNum);
...
{
    ISP_FUNC_DBG("+,m_pPhyIspDrv(%p) ",(unsigned int*)m_pPhyIspDrv_bak);
    //
...
controlBurstQueBuffer(EIspControlBufferCmd cmd, MUINT32 burstQueueNum)
{
    ...
    if(eIspControlBufferCmd_Alloc == cmd) {
        ...
        for(k=0;k<burstQueueNum;k++){
            for(i=0;i<ISP_DRV_P2_CQ_NUM;i++){
                for(j=0;j<ISP_DRV_P2_CQ_DUPLICATION_NUM;j++){
                    ...
                    ISP_FUNC_INF("[Tpipe]i(%d),j(%d)va(0x%08lx),pa(0x%08lx)",i,j,(unsigned long)(gpMdpMgrCfgData[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTableVa),(unsigned long)(gpMdpMgrCfgData[k][i][j].ispTpipeCfgInfo.drvinfo.tpipeTablePa));
                    #endif
                }
            }
        }
        ...
        for(int cq=0;cq<totalCQNum;cq++)
        {
            ...
            ISP_FUNC_DBG("[m_pVirtIspDrv[cq%d]]:%p ",cq,(unsigned int*)m_pVirtIspDrv[cq]);
            if (!m_pVirtIspDrv[cq]) {
            ...
            ISP_FUNC_DBG("[m_pVirtIspReg[cq%d]]:%p ",cq,(unsigned int*)m_pVirtIspReg[cq]);
...
int DMAI_B::_config( void )
{
    ...
    //unsigned long pBase = 0; //not used
    ISP_DRV_P2_CQ_ENUM p2Cq;
    //MINT32 index=0; //not used
    //MUINT32 cq = 0; //not used
    MUINT32 _targetDupQIdx = 0;

    ISP_FUNC_DBG("DMAI(%lu),[%s]:m_pP2IspDrv(%p),pa(0x%zx),ofst(0x%zx),size(%lu,%lu,%lu,%lu),pixel_byte(%d),crop(%d,%d), flip(%d),isTwin(%d),CQ(0x%x/0x%x) ", \
        this->id(), \
        this->name_Str(), \
        (unsigned int*)m_pP2IspDrv, \
    ...
    switch(this->id())
    {
        case ISP_DMA_TDRI:
            ...
            break;
        case ISP_DMA_CQI:
            if ((this->CQ == CAM_ISP_CQ_BYP) && (this->CQ_D == CAM_ISP_CQ_BYP)) {
                ISP_FUNC_DBG("bypass CQ for CAMSV");
                break;
            }
            ...
            ISP_FUNC_DBG("[afterCQSwitch]m_pP2IspDrv(%p)",(unsigned int*)m_pP2IspDrv);
            break;
#if 0
        case ISP_DMA_UFDI:
        ...
        case ISP_DMA_IMGI:
            ...
            ISP_FUNC_DBG("DMAI(%lu),(%s):reg(0x%08x) ", \
                this->id(), \
                this->name_Str(), \
...
int DMAI_B::_disable( void )
{
    ...
    ISP_FUNC_DBG("[id:0x%08lu] ",this->id());
...
unsigned int cq_module_id = 0xFF;
    ...
    ISP_FUNC_DBG("_,[id:0X%08lu] ",this->id());
...
{
    MINT32 index=0;
    //MUINT32 C2,C3,C4,C5,C6,C7,size; //not used
    MUINT32 stride = 0;
    MUINT32 x = 0;
    //MUINT32 offset = 0; //not used
    IspDrv* pCommonIspDrv;
    IspDrv* _targetVirDrv = NULL;
    MUINT32 _targetDupQIdx = 0;
    UNUSED(index);
    ISP_FUNC_DBG("DMAO(%lu),[%s]:cq(%d/%d),pa(0x%zx),ofst(0x%X),size(%lu,%lu,%lu,%lu), pixel_byte(%d),cropW(%d,%d,%lu,%lu),format(0x%x),fps(%d),cropX_inbussize(%d)", \
        this->id(), \
        this->name_Str(), \
        this->CQ, this->CQ_D,\
        this->dma_cfg.memBuf.base_pAddr, \
    ...
    switch(this->id())
    {
        ...
        case ISP_DMA_AFO:
            ...
            break;
#endif
        case ISP_DMA_AFO_D: // need to notice
            _targetVirDrv = (IspDrv*)this->m_pIspDrvShell->ispDrvGetCQIspDrv(this->CQ,this->burstQueIdx,_targetDupQIdx);
        ...
        case ISP_DMA_AAO:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_AAO_CON2,0x00151500,ISP_DRV_USER_ISPF);
            //ISP_WRITE_REG(_targetVirDrv,CAM_AAO_CON,0x80202020,ISP_DRV_USER_ISPF); // ultra-
            if(this->m_fps > FPS_BOUNDARY)
                ISP_WRITE_REG(_targetVirDrv,CAM_AAO_CON,0x80202040,ISP_DRV_USER_ISPF); // ultra-
            else
                ISP_WRITE_REG(_targetVirDrv,CAM_AAO_CON,0x80202040,ISP_DRV_USER_ISPF); // ultra-
        ...
        case ISP_DMA_ESFKO:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_ESFKO_CON2,0x000A0A00,ISP_DRV_USER_ISPF);
        ...
        case ISP_DMA_IMGO:
            ...
            if(this->m_fps > FPS_BOUNDARY){
#if SRAM_SWITCH
                ISP_WRITE_REG(_targetVirDrv,CAM_IMGO_CON,0x81808000,ISP_DRV_USER_ISPF); // ultra- //how?
#endif
        ...
        case ISP_DMA_RRZO:
            ...
            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x81808000,ISP_DRV_USER_ISPF); // ultra-
            ISP_WRITE_REG(this->m_pIspDrvShell->m_pPhyIspDrv_bak,CAM_IMGI_SLOW_DOWN,0x0c000000,ISP_DRV_USER_ISPF); // ultra-

            ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON2,0x00555500,ISP_DRV_USER_ISPF);
            ...
            ISP_FUNC_DBG("ISP_DMA_RRZO, pxl_byte=%d, w=%lu,x=0x%x,stride=0x%x, cropXY=%d,%d",this->dma_cfg.pixel_byte, this->dma_cfg.size.w, x, stride,this->dma_cfg.crop.y, this->dma_cfg.crop.x);
            //currently , slow motion will map to this case
            if(this->m_fps > FPS_BOUNDARY){
#if SRAM_SWITCH
                ISP_WRITE_REG(_targetVirDrv,CAM_RRZO_CON,0x816060FF,ISP_DRV_USER_ISPF); // ultra- //how?
        ...
        case ISP_DMA_IMG2O:
            ...
            ISP_WRITE_REG(m_pP2IspDrv,CAM_IMG2O_CON,0x80202040,ISP_DRV_USER_ISPF);//default 0x80404040
            ISP_WRITE_REG(m_pP2IspDrv,CAM_IMG2O_CON2,0x002A2A00,ISP_DRV_USER_ISPF);//default 0x00404000
        ...
        case ISP_DMA_CAMSV_IMGO:
            ...
            ISP_WRITE_BITS(m_pPhyIspDrv, CAMSV_IMGO_SV_STRIDE, BUS_SIZE_EN, 0x0, ISP_DRV_USER_ISPF);//HW auto set
            ...
            //enlarge untra
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_CON, 0x80404080,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_CON2,0x002A2A00,ISP_DRV_USER_ISPF);
        ...
        case ISP_DMA_CAMSV2_IMGO:
            ...
            ISP_WRITE_BITS(m_pPhyIspDrv, CAMSV_IMGO_SV_D_STRIDE, BUS_SIZE_EN, 0x0, ISP_DRV_USER_ISPF);//HW auto set
            ...
            //enlarge ultra
            //ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_D_CON,0x08404040,ISP_DRV_USER_ISPF);   // ultra- 0x08404040
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_D_CON,0x80404080,ISP_DRV_USER_ISPF);
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_IMGO_SV_D_CON2,0x002A2A00,ISP_DRV_USER_ISPF);
...
int DMAO_B::_write2CQ(void)
{
    ...
    unsigned int myCq = CAM_ISP_CQ_NONE;
    ISP_FUNC_DBG("+,");
    ISP_FUNC_DBG("[id:0X%08lu],CQ(%d)",this->id(),this->CQ);
     ...
     switch(this->id())
     {
        ...
        case ISP_DMA_AFO:    cq_module_id = CAM_DMA_AFO;
            break;
#endif
        case ISP_DMA_AFO_D:    cq_module_id = CAM_DMA_AFO_D; //Only AFO_D is functional in this project.
            break;
...
ISP_RAW_CENTRAL_CTRL_INFO::config2( ISP_TOP_CTRL * pSrcTopCtrl )
{
    ...
    if (ISP_PASS1 == pSrcTopCtrl->path || ISP_PASS1_D == pSrcTopCtrl->pathD) {
        ...
        //No need implement on ISP_PASS1_CAMSV and ISP_PASS1_CAMSV_D
        //MW might config CAMSV then config TG1, and ISP_DRV_CQ0 will update the wrong settings.
    ...
    if ( ISP_PASS1 == pSrcTopCtrl->path || ISP_PASS1_D == pSrcTopCtrl->pathD ) {
       ...
       if (0 == mEnablePath) {
           ...
           if (ISP_PASS_NONE != (unsigned int)pSrcTopCtrl->path) {
...
int ISP_TOP_CTRL::_config( void )
{
    ...
    //IspDrv* _tmpRawDIspDrv = NULL; //not used
    ...
    if ((CAM_ISP_CQ_NONE != this->CQ) && (CAM_ISP_CQ_BYP != this->CQ)) {
    ...
    if ((CAM_ISP_CQ_NONE != this->CQ_D) && (CAM_ISP_CQ_BYP != this->CQ_D)) {
        ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_MAX,(_isp_dma_enum_)this->CQ_D,0,0,0};
        this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl);
    }

    // check isp path and assign pCommonIspDrv address
    MUINT32 __path = (MUINT32)this->path;
    switch (__path){
        case ISP_PASS1:
    ...
    if(logPath == ISP_PASS2) { // P2
        ...
        ISP_FUNC_INF("[CAMSV]en(0x%08x),2en(0x%08x),intEn(0x%08x),2intEn(0x%08x),fmtSel(0x%08x),2fmtSel(0x%08x)", \
                        this->isp_top_ctl.CAMSV_MODULE_EN.Raw, \
                        this->isp_top_ctl.CAMSV2_MODULE_EN.Raw, \
                        this->isp_top_ctl.CAMSV_INT_EN.Raw, \
                        this->isp_top_ctl.CAMSV2_INT_EN.Raw, \
                        this->isp_top_ctl.CAMSV_FMT_SEL.Raw, \
                        this->isp_top_ctl.CAMSV2_FMT_SEL.Raw);
        #if 0
...
#endif//(ISP_RAW_D_SUPPORT == 1)
#endif
    ISP_FUNC_VRB("m_pP1IspDrv(%p),m_pP2IspDrv(%p),m_pPhyIspDrv(%p)",(unsigned int*)m_pP1IspDrv,(unsigned int*)m_pP2IspDrv,(unsigned int*)m_pPhyIspDrv);
    ...
    if ((ISP_PASS1 == this->path)||(ISP_PASS1_D == this->pathD)){
           ...
           //IspDrv* mRawIspDrv  = this->m_pIspDrvShell->ispDrvGetCQIspDrv(ISP_DRV_CQ0,0/*mTopCtrl.burstQueIdx*/, 0/*mTopCtrl.dupCqIdx*/); //not used
           //IspDrv* mRawDIspDrv = this->m_pIspDrvShell->ispDrvGetCQIspDrv(ISP_DRV_CQ0_D,0, 0/*mTopCtrl.burstQueIdx*/); //not used
    ...
    if (( CAM_ISP_CQ_NONE != this->CQ ) || ( CAM_ISP_CQ_NONE != this->CQ_D )){
        ...
        else if ((this->path == ISP_PASS1_CAMSV) || (this->pathD == ISP_PASS1_CAMSV_D)) {
        }
        ...
        if ((this->path != ISP_PASS1_CAMSV) && (this->pathD != ISP_PASS1_CAMSV_D))
            this->ispDrvSwitch2Virtual(this->CQ,this->CQ_D,this->burstQueIdx,this->dupCqIdx);
    }
    //ISP CLK ENABLE
    ISP_WRITE_REG(m_pPhyIspDrv,CAM_CTL_CLK_EN,0x0000000F,ISP_DRV_USER_ISPF);

    if ( this->isp_top_ctl.CAMSV_MODULE_EN.Bits.TG_EN ) {
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_INT_EN ,this->isp_top_ctl.CAMSV_INT_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_MODULE_EN,this->isp_top_ctl.CAMSV_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_PAK,this->isp_top_ctl.CAMSV_PAK.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_FMT_SEL,this->isp_top_ctl.CAMSV_FMT_SEL.Raw, ISP_DRV_USER_ISPF);
    }
    if ( this->isp_top_ctl.CAMSV2_MODULE_EN.Bits.TG_EN ) {
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_CLK_EN,0x00008005,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_INT_EN ,this->isp_top_ctl.CAMSV2_INT_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_MODULE_EN,this->isp_top_ctl.CAMSV2_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_PAK,this->isp_top_ctl.CAMSV2_PAK.Raw,ISP_DRV_USER_ISPF);
        ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_FMT_SEL,this->isp_top_ctl.CAMSV2_FMT_SEL.Raw, ISP_DRV_USER_ISPF);
    }
    ...
    /********************************************************************************************************************/
    /********************************************************************************************************************/
    //MUINT32 p2PathMux=ISP_PASS2|ISP_PASS2B|ISP_PASS2C; //not used
    ...
    if ( ISP_PASS1   == this->path || ISP_PASS1_D == this->pathD ) {
        ...
        if (ISP_PASS1 == this->path) {
            ...
            /* Special AF_DON from CAM_CTL_INT_P1_EN_D */
            ISP_WRITE_REG(m_pPhyIspDrv,CAM_CTL_INT_P1_EN_D ,ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,
                CAM_CTL_INT_P1_EN_D)  |ptop->isp_top_ctl.CAM_CTL_INT_P1_EN_D.Raw,ISP_DRV_USER_ISPF);
        ...
        for (int ii=0; ii < 2; ii++)  {
           ...
           if (ISP_PASS_NONE == (unsigned int)path[ii])
                continue;
        ...
        MUINT32 temp = 0;
        UNUSED(temp);
        if (this->path == ISP_PASS1_CAMSV) {
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_INT_EN,ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_INT_EN)  |ptop->isp_top_ctl.CAMSV_INT_EN.Raw,ISP_DRV_USER_ISPF);
        } else if (this->pathD == ISP_PASS1_CAMSV_D) {
            ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_INT_EN,ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_INT_EN)|ptop->isp_top_ctl.CAMSV2_INT_EN.Raw,ISP_DRV_USER_ISPF);
        }
        ...
        p1_log[2] = function_DbgLogEnable_VERBOSE;
        p1_log[3] = function_DbgLogEnable_INFO;//function_DbgLogEnable_DEBUG;
        p1_log[4] = function_DbgLogEnable_DEBUG;//kernel log reduction
        p1_log[5] = p1_log[4];
        for (int ii=0; ii < 2; ii++)  {
            ISP_BUFFER_CTRL_STRUCT buf_ctrl;
           //MUINT32 dummy = 0;
           ...
           case ISP_PASS1_CAMSV:
               ...
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_MODULE_EN,this->isp_top_ctl.CAMSV_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
               #ifdef _rtbc_buf_que_2_0_
                   dma_array[_camsv_imgo_] = (this->isp_top_ctl.CAMSV_MODULE_EN.Bits.IMGO_EN_)?(1):(0);
                   buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,(_isp_dma_enum_)_camsv_imgo_,0,0,&dma_array[0]};
                   if (this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl) != MTRUE) {
                       ISP_FUNC_ERR("ERROR:rtBufCtrl");
                       return -1;
                   }
               #endif
               ...
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_PAK, \
                     ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_PAK)&(~this->isp_top_ctl.CAMSV_PAK_MASK) ) | \
                     this->isp_top_ctl.CAMSV_PAK.Raw,ISP_DRV_USER_ISPF);
                     ISP_WRITE_REG(m_pPhyIspDrv, \
                     CAMSV_FMT_SEL, \
                     ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_FMT_SEL)&(~this->isp_top_ctl.CAMSV_FMT_SEL_MASK) ) | \
                     this->isp_top_ctl.CAMSV_FMT_SEL.Raw, ISP_DRV_USER_ISPF);
               break;
           case ISP_PASS1_CAMSV_D:
                //clear kernel status to avoid abnormal exit at previous camera operation
                buf_ctrl.ctrl = ISP_RT_BUF_CTRL_CLEAR;
               ...
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_MODULE_EN,this->isp_top_ctl.CAMSV2_MODULE_EN.Raw,ISP_DRV_USER_ISPF);
               #ifdef _rtbc_buf_que_2_0_
                   dma_array[_camsv2_imgo_] = (this->isp_top_ctl.CAMSV2_MODULE_EN.Bits.IMGO_EN_)?(1):(0);
                   buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,(_isp_dma_enum_)_camsv2_imgo_,0,0,&dma_array[0]};
                   if (this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl) != MTRUE) {
                       ISP_FUNC_ERR("ERROR:rtBufCtrl");
                       return -1;
                   }
               ...
               ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_PAK, \
                      ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_PAK)&(~this->isp_top_ctl.CAMSV2_PAK_MASK) ) | \
                      this->isp_top_ctl.CAMSV2_PAK.Raw,ISP_DRV_USER_ISPF);
                      ISP_WRITE_REG(m_pPhyIspDrv, \
                      CAMSV2_FMT_SEL, \
                      ( ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_FMT_SEL)&(~this->isp_top_ctl.CAMSV2_PAK_MASK) ) | \
                      this->isp_top_ctl.CAMSV2_FMT_SEL.Raw, ISP_DRV_USER_ISPF);
               break;
            default:
               ISP_FUNC_ERR("NOT SUPPORT PATH, path[%d] = %d", ii,  path[ii]);
               break;
        ...
        ISP_FUNC_INF("[CAMSV]EN(0x%08x/0x%08x),fmt(0x%08x/0x%08x),pak(0x%08x)\n", \
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_MODULE_EN),\
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_MODULE_EN),\
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_FMT_SEL),\
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_FMT_SEL),\
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_PAK));
    ...
    if ((this->path==ISP_PASS2)||(this->path==ISP_PASS2B)||(this->path==ISP_PASS2C))
    {
        ...
        if (isApplyTuning) {
            ...
            getP2EnTuningTag(MTRUE,
                           ...
                           p2TuningEnTag,
                           p2TuningDmaTag,
                           this->isV3); // get P2 tuning En & Dma Tag
            //
...
int ISP_TOP_CTRL::_enable( void* pParam  )
{
    ...
    MUINT32 CQ_status=0; //CQ_debugset=0; //not used
    //MBOOL readCQstatus = MFALSE; //not used
    Mutex   *_localVar;
    ...
    if ( CAM_ISP_PASS1_START == (*start) ) {
...
#endif
        //CAMSV FBC
        if( CAMSV_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV_MODULE_EN.Raw){
            buf_ctrl.ctrl = ISP_RT_BUF_CTRL_GET_SIZE;
            buf_ctrl.buf_id = _camsv_imgo_;
            buf_ctrl.data_ptr = 0;
            buf_ctrl.pExtend = (unsigned char*)&size;
            ISP_FUNC_INF("ctrl(%d),id(%d),pExt(%p)",buf_ctrl.ctrl,buf_ctrl.buf_id,buf_ctrl.pExtend);
            if ( MTRUE != this->m_pIspDrvShell->getPhyIspDrv()->rtBufCtrl((void*)&buf_ctrl) ) {
                ISP_FUNC_ERR("ERROR:rtBufCtrl");
                return -1;
            }
            if ( size == ISP_RT_BUF_SIZE ) {
                ISP_FUNC_ERR("real time buffer number FULL:rt_dma(%d)/size(%d)",_camsv_imgo_,size);
                return -1;
            }
        ...
        //CAMSV2 FBC
        if( CAMSV2_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV2_MODULE_EN.Raw){
            buf_ctrl.ctrl = ISP_RT_BUF_CTRL_GET_SIZE;
            ...
            if ( size == ISP_RT_CQ0C_BUF_SIZE ) {
                ISP_FUNC_ERR("real time buffer number FULL:rt_dma(%d)/size(%d)",_camsv2_imgo_,size);
                return -1;
            }
        ...
        if ( CAM_CTL_EN_P1_D_TG1_D_EN & this->isp_top_ctl.CAM_CTL_EN_P1_D.Raw) {
              ...
              irq_TG_ClrDone.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
              irq_TG_ClrDone.UserInfo.Status=CAM_CTL_INT_P1_STATUS_D_PASS1_DON_ST;
              //
              if ( MFALSE == this->m_pPhyIspDrv->waitIrq( &irq_TG_ClrDone )) {
                   ISP_FUNC_ERR("clr irq status fail");
                   return -1;
        ...
        //CAMSV
        if( CAMSV_MODULE_TG_EN & this->isp_top_ctl.CAMSV_MODULE_EN.Raw){
              ...
              irq_TG_ClrDone.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_CAMSV;
              irq_TG_ClrDone.UserInfo.Status=CAMSV_INT_STATUS_PASS1_DON_ST;
              //
              if ( MFALSE == this->m_pPhyIspDrv->waitIrq( &irq_TG_ClrDone )) {
                  ISP_FUNC_ERR("clr irq status fail");
                  return -1;
         ...
         //CAMSV2
         if( CAMSV2_MODULE_TG_EN & this->isp_top_ctl.CAMSV2_MODULE_EN.Raw){
               ...
               irq_TG_ClrDone.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_CAMSV2;
               irq_TG_ClrDone.UserInfo.Status=CAMSV2_INT_STATUS_PASS1_DON_ST;
               //
               if ( MFALSE == this->m_pPhyIspDrv->waitIrq( &irq_TG_ClrDone )) {
                    ISP_FUNC_ERR("clr irq status fail");
                    return -1;
    ...
    static int pass2_cnt = 0;
        ...
        if (CAM_ISP_PASS1_START == *start) {
            ...
            ISP_FUNC_DBG("P2_dma_en(0x%x),imgiPa(0x%x),imgiOffset(0x%x),img2oPa(0x%x),img2oOffset(0x%x)", \
...
int ISP_TOP_CTRL::_disable( void )
{
    ...
    irq_csv_int.Status=CAMSV_INT_STATUS_VS1_ST;
    ...
    irq_csv_int.UserInfo.Status=CAMSV_INT_STATUS_VS1_ST;
    ...
    irq_csv2_int.Status=CAMSV2_INT_STATUS_VS1_ST;
    ...
    irq_csv2_int.UserInfo.Status=CAMSV2_INT_STATUS_VS1_ST;
    ...
    MBOOL bP1Path = ( ISP_PASS1         == this->path  || \
                      ...
                      ISP_PASS1_CAMSV_D == this->pathD );


    ISP_BUFFER_CTRL_STRUCT buf_ctrl;
    //MUINT32 dummy;
    ISP_RT_BUF_INFO_STRUCT dummy;
    MUINT32 wait_vs_num = 2;
    Mutex   *_localVar;
                ...
                while ((pactive_irq_vs_int[0]) && ( wait_vs_num-- )) {
                    ...
                    if ( MFALSE == ret ) {
                        ...
                        if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAM_TG_INTER_ST,TG_CAM_CS) != 1){
                            ISP_FUNC_INF("[TG1]TG is not idle(0x%x), wait %d VD again.\n",
                                ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAM_TG_INTER_ST),
                                wait_vs_num);
                            continue;
                        } else {
                            wait_vs_num = 0; /* Jump out of loop if TG is idle. */
                        }
                    }
                }
            }
...
#if (ISP_RAW_D_SUPPORT == 0)
                    /* Special: AFO_D should be disabled in raw path */
...
///////////////////E
                ...
                ISP_FUNC_INF("[PIP][TopCtrl:Disable]P1_en(0x%x/0x%x/0x%x),CQ-En(0x%x),FBC(0x%x/0x%x),camsv(0x%x)", \
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_EN_P1),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_EN_P1_DMA),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_EN_P1_DMA_D),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAM_CTL_CQ_EN),\
                ...
                while ((pactive_irq_vs_int[0]) && ( wait_vs_num-- )) {
                    ...
                    if ( MFALSE == ret ) {
                        ...
                        if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAM_TG2_INTER_ST,TG_CAM_CS) != 1){
                            ISP_FUNC_INF("[TG2]TG is not idle(0x%x), wait %d VD again.\n",
        ...
        if (this->path == ISP_PASS1_CAMSV) {
            ...
            wait_vs_num = 2;
            ISP_FUNC_INF("[CAMSV_TG]wait %d VD till DMA stop",wait_vs_num);
            //
            {
                while ((pactive_irq_vs_int[0]) && ( wait_vs_num-- )) {
                    ret = this->m_pPhyIspDrv->waitIrq( pactive_irq_vs_int[0] );
                    if ( MFALSE == ret ) {
                        ISP_FUNC_ERR("waitIrq( irq_TG3_DONE ) fail");
                    }
                    else{
                        //check if TG3 is IDLE
                        if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAMSV_TG_INTER_ST,TG_CAM_CS) != 1){
                            ISP_FUNC_INF("[TG3]TG is not idle(0x%x), wait %d VD again.\n",
                                ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAMSV_TG_INTER_ST),
                                wait_vs_num);
                            continue;
                        } else {
                            wait_vs_num = 0; /* Jump out of loop if TG is idle. */
            ...
            if(CAMSV_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV_MODULE_EN.Raw) {
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_IMGO_FBC,FBC_EN,0,ISP_DRV_USER_ISPF);
                ISP_WRITE_REG(m_pPhyIspDrv,CAMSV_MODULE_EN, 1,ISP_DRV_USER_ISPF);
            }
            if (rawCentralCtrl.mEnablePath != 0) {  //workaround
                //the other path is still alive, we can't do the sw reset.
                //use this method to reset FBC
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON,SINGLE_MODE,1,ISP_DRV_USER_ISPF);
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON,VFDATA_EN,1,ISP_DRV_USER_ISPF);
                ISP_FUNC_INF("wait2 VD till FBC Rst");

                {
                    ...
                    //Polling
                    wait_vs_num = 10;
                    MUINT32 temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_IMGO_FBC))&0xFF0000F;
                    while ((temp != 0x1100000) && (wait_vs_num-- > 0)) {
                        ISP_FUNC_INF("[PIP]CAMSV_IMGO FBC=0x%x", temp);
                        temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_IMGO_FBC))&0xFF0000F;
                    }
                }
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON,VFDATA_EN,0,ISP_DRV_USER_ISPF);
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
            }
            ISP_FUNC_INF("[PIP][TopCtrl:Disable]camsv(0x%x),FBC(0x%x),TG(0x%x)", \
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_MODULE_EN),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_IMGO_FBC),\
                    ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_TG_VF_CON));
            #ifdef _rtbc_buf_que_2_0_
            dma_array[_camsv_imgo_] = 0;
            //ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,(_isp_dma_enum_)this->CQ,(unsigned int)dma_array,0};
            ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,(_isp_dma_enum_)_camsv_imgo_,0,0,&dma_array[0]};
            if (this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl) != MTRUE) {
                ISP_FUNC_ERR("ERROR:rtBufCtrl");
                return -1;
            }
        ...
        if (this->pathD == ISP_PASS1_CAMSV_D) {
            ...
            wait_vs_num = 2;
            ISP_FUNC_INF("[CAMSV_TG2]wait %d VD till DMA stop",wait_vs_num);
            //
            {
                while ((pactive_irq_vs_int[0]) && ( wait_vs_num-- )) {
                    ret = this->m_pPhyIspDrv->waitIrq( pactive_irq_vs_int[0] );
                    if ( MFALSE == ret ) {
                        ISP_FUNC_ERR("waitIrq( irq_TG4_DONE ) fail");
                    }
                    else{
                        //check if TG4 is IDLE
                        if(ISP_READ_BITS_NOPROTECT(this->m_pPhyIspDrv,CAMSV_TG2_INTER_ST,TG_CAM_CS) != 1){
                            ISP_FUNC_INF("[TG4]TG is not idle(0x%x), wait %d VD again.\n",
                                ISP_READ_REG_NOPROTECT(this->m_pPhyIspDrv,CAMSV_TG2_INTER_ST),
                                wait_vs_num);
                            continue;
                        } else {
                            wait_vs_num = 0; /* Jump out of loop if TG is idle. */
            ...
            if(CAMSV2_MODULE_IMGO_EN & this->isp_top_ctl.CAMSV2_MODULE_EN.Raw) {
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV2_IMGO_FBC,FBC_EN,0,ISP_DRV_USER_ISPF);
                ISP_WRITE_REG(m_pPhyIspDrv,CAMSV2_MODULE_EN, 1,ISP_DRV_USER_ISPF);
            }
            if (rawCentralCtrl.mEnablePath != 0) {  //workaround
                //the other path is still alive, we can't do the sw reset.
                //use this method to reset FBC
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,1,ISP_DRV_USER_ISPF);
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,VFDATA_EN,1,ISP_DRV_USER_ISPF);
                ISP_FUNC_INF("wait2 VD till FBC Rst");

                {
                    ...
                    //Polling
                    wait_vs_num = 10;
                    MUINT32 temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_IMGO_FBC))&0xFF0000F;
                    while ((temp != 0x1100000) && (wait_vs_num-- > 0)) {
                        ISP_FUNC_INF("[PIP]CAMSV2_IMGO FBC=0x%x", temp);
                        temp = (ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_IMGO_FBC))&0xFF0000F;
                    }
                }
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,VFDATA_EN,0,ISP_DRV_USER_ISPF);
                ISP_WRITE_BITS(m_pPhyIspDrv,CAMSV_TG2_VF_CON,SINGLE_MODE,0,ISP_DRV_USER_ISPF);
            }
            ISP_FUNC_INF("[PIP][TopCtrl:Disable]camsv2(0x%x),FBC(0x%x),TG(0x%x)", \
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_MODULE_EN),\
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV2_IMGO_FBC),\
                ISP_READ_REG_NOPROTECT(m_pPhyIspDrv,CAMSV_TG2_VF_CON));
            #ifdef _rtbc_buf_que_2_0_
            dma_array[_camsv2_imgo_] = 0;
            //ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,(_isp_dma_enum_)this->CQ,(unsigned int)dma_array,0};
            ISP_BUFFER_CTRL_STRUCT buf_ctrl = {ISP_RT_BUF_CTRL_DMA_EN,(_isp_dma_enum_)_camsv2_imgo_,0,0,&dma_array[0]};
            if (this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl) != MTRUE) {
                ISP_FUNC_ERR("ERROR:rtBufCtrl");
                return -1;
            }
        ...
        if (rawCentralCtrl.mEnablePath == 0) {
            ...
            this->m_pPhyIspDrv->reset(ISP_DRV_RST_CAMSV);
            this->m_pPhyIspDrv->reset(ISP_DRV_RST_CAMSV2);
        ...
        while(_clrPort--){
            ISP_FUNC_DBG("[TopCtrl::Disable][before]rt_dma(%d),size:hw(%u)",_dma[_clrPort], ISP_BUF_CTRL::m_hwbufL[_dma[_clrPort]].bufInfoList.size());
            buf_ctrl.ctrl = ISP_RT_BUF_CTRL_CLEAR;
            buf_ctrl.buf_id = (_isp_dma_enum_)_dma[_clrPort];
            buf_ctrl.data_ptr = &dummy;
            buf_ctrl.pExtend = 0;
            ISP_FUNC_DBG("rtBufCtrl.ctrl(%d)/id(%d)/ptr(%p)",buf_ctrl.ctrl,buf_ctrl.buf_id,(unsigned int*)buf_ctrl.data_ptr);
            if ( MTRUE != this->m_pIspDrvShell->m_pPhyIspDrv_bak->rtBufCtrl((void*)&buf_ctrl) ) {
                ISP_FUNC_ERR("ERROR:rtBufCtrl");
                return -1;
            }
            ISP_FUNC_DBG("[TopCtrl::Disable][after]rt_dma(%d),size:hw(%u) ",_dma[_clrPort], ISP_BUF_CTRL::m_hwbufL[_dma[_clrPort]].bufInfoList.size());
        }
...
int ISP_RAW_PIPE::_config( void )
{
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        MUINT64 i, tuningUpdateBit=eIspTuningMgrFunc_Null;
        ...
        if (this->isV3)  //QQ??
        {
            ...
            if(imageioPackage.isApplyTuning==MTRUE){
                ...
                ISP_FUNC_DBG("tuningUpdateBit(0x%llx),pTuningBuf(%p)",tuningUpdateBit,tuningPackage.pTuningBuf);
            }

            getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag, this->isV3); // get P2 tuning En & Dma Tag

            if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario){
                getP1EnTuningTag(MFALSE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag, this->isV3); // get P1 tuning En & Dma Tag
                ...
                getP1EnTuningTag(MFALSE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag, this->isV3); // get P1 tuning En & Dma Tag
                finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
                ISP_FUNC_DBG("(ISP_RAW_PIPEA),finalEnP1(0x%x),(~tuningEnP1Tag)(0x%x),this->en_p1(0x%x),tuningEnP1Tag(0x%x), tuningEnP1(0x%x)",finalEnP1, (~tuningEnP1Tag),this->en_p1, tuningEnP1Tag, tuningEnP1);
            }
            finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
            imageioPackage.finalEnP2 = finalEnP2;

            ISP_FUNC_INF("(ISP_RAW_PIPE),isApplyTuning(0x%x),finalEnP1(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%llx), en_p1(0x%x)",this->isApplyTuning, finalEnP1, finalEnP2, tuningUpdateBit, this->en_p1);
        }
        else
        {
            getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag, this->isV3); // get P2 tuning En & Dma Tag
            ...
            if(imageioPackage.isApplyTuning==MTRUE){
                ...
                ISP_FUNC_DBG("tuningUpdateBit(0x%llx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(%p)",tuningUpdateBit,tuningEnP2,tuningEnP2Dma,tuningPackage.pTuningBuf);
            }
        ...
        ISP_FUNC_DBG("(ISP_RAW_PIPE),cq(%d),drvScenario(%d),sub_mode(%d),en_p2(0x%08x),finalEnP2(0x%08x),en_p1(0x%08x),finalEnP1(0x%08x)",this->CQ,this->drvScenario,this->sub_mode,this->en_p2,finalEnP2,this->en_p1,finalEnP1);
        //
    ...
    if (!this->isV3)
    {
        ...
        IspRrzCfg rrz_cfg;
        MUINT32 rrz_ctl = 0;
        CAM_REG_RRZ_CTL rrz_ctl_reg;
        //MUINT32 mgNum  = this->m_num_0; //not used
        //MUINT32 mgNumd = this->m_num_0_d; //not used
        //int ispPath[2] = {this->path, this->pathD}; //not used
        IspDrv* _targetVirDrv = NULL;
        MUINT32 _targetDupQIdx = 0;
#ifdef _PASS1_CQ_CONTINUOUS_MODE_
        ...
        if(CAM_CTL_EN_P1_RRZ_EN & this->en_p1 && 1 == this->isTwinMode) {
            ...
            rrz_ctl_reg.Raw = 0x0;
            rrz_ctl_reg.Bits.RRZ_HORI_TBL_SEL = this->m_pIspDrvShell->m_pTwinDrv->getCoefTbl(this->rrz_in_roi.w,this->rrz_out_size.w);
            rrz_ctl_reg.Bits.RRZ_VERT_TBL_SEL = this->m_pIspDrvShell->m_pTwinDrv->getCoefTbl(this->rrz_in_roi.h,this->rrz_out_size.h);
            rrz_ctl_reg.Bits.RRZ_HORI_EN = 1;
            rrz_ctl_reg.Bits.RRZ_VERT_EN = 1;
            rrz_ctl_reg.Bits.RRZ_OUTPUT_WAIT_EN = 1; //RRZ_OUTPUT_WAIT_EN=1 to prevent RRZO done too early
            rrz_ctl = rrz_ctl_reg.Raw;
            ...
            ISP_FUNC_INF("ISP_RAW_PIPE::_config(setP1RrzCfg:0x%x_0x%x):Twin lsc_en=%lu E:path(%lu),rrz_in_w/h(%lu/%lu),rrz_crop_x/y/fx/fy/w/h(%d/%d/%d/%d/%lu/%lu),rrz_out_w/h(%lu/%lu), sd_lw/xn(%lu/%lu),m_num(%lu)", \
                this->path,this->pathD,\
                rrz_cfg.lsc_en,\
                rrz_cfg.pass1_path, \
...
#endif
            ISP_FUNC_INF("ISP_RAW_PIPE::_config(setP1RrzCfg:0x%x_0x%x):NonTwin lsc_en=%lu E:path(%lu),rrz_in_w/h(%lu/%lu),rrz_crop_x/y/fx/fy/w/h(%d/%d/%d/%d/%lu/%lu),rrz_out_w/h(%lu/%lu), sd_lw/xn(%lu/%lu),m_num(%lu)", \
                this->path,this->pathD,\
                rrz_cfg.lsc_en,\
                rrz_cfg.pass1_path, \
                rrz_cfg.rrz_in_size.w, \
            ...
            if( ( ISP_PASS1   == this->path) && (CAM_CTL_EN_P1_RRZ_EN & this->en_p1)){
                ...
                rrz_ctl_reg.Raw = 0x0;
                rrz_ctl_reg.Bits.RRZ_HORI_TBL_SEL = this->m_pIspDrvShell->m_pTwinDrv->getCoefTbl(this->rrz_d_in_roi.w,this->rrz_d_out_size.w);
                rrz_ctl_reg.Bits.RRZ_VERT_TBL_SEL = this->m_pIspDrvShell->m_pTwinDrv->getCoefTbl(this->rrz_d_in_roi.h,this->rrz_d_out_size.h);
                rrz_ctl_reg.Bits.RRZ_HORI_EN = 1;
                rrz_ctl_reg.Bits.RRZ_VERT_EN = 1;
                rrz_ctl_reg.Bits.RRZ_OUTPUT_WAIT_EN = 1; //RRZ_OUTPUT_WAIT_EN=1 to prevent RRZO done too early
                rrz_ctl = rrz_ctl_reg.Raw;
...
int ISP_RAW_PIPE::_write2CQ(void)
{
    ...
    if ( ISP_PASS2 == this->path || \
        ...
        if(this->isV3)
        {
            ISP_FUNC_DBG("ISP_RAW_PIPE::_write2CQ");
            getP1EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP1Tag, tuningEnP1DmaTag, this->isV3);
            getP2EnTuningTag(MTRUE, (ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag, this->isV3); // get P2 tuning En & Dma Tag
        ...
        if(isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%llx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x)",tuningUpdateBit,tuningEnP2,tuningEnP2Dma);
        }
        if(NSImageioIsp3::NSIspio::eDrvScenario_IP == this->drvScenario){
            finalEnP1 = ((~tuningEnP1Tag)&this->en_p1)|(tuningEnP1Tag&tuningEnP1);
...
{
   MUINT32 temp; //lsc_xnum, lsc_ynum //not used
...
int ISP_RAW_PIPE::setP1TuneCfg(MUINT32 mNum, MINT32 senDev)
{
    ...
    IspP1TuneCfg cfg;
    ISP_DRV_CQ_ENUM cq;
    //MUINT32* pTuningBuf; //not used
    //MUINT32 i; //not used
    //MUINT64 tuningUpdateBit; //not used
    //MUINT32 tuningEnP1=0x00, tuningEnP1Dma=0x00, tuningEnP1Tag=0x00, tuningEnP1DmaTag=0x00; //not used
    //MUINT32 tuningEnP1D=0x00, tuningEnP1DmaD=0x00, tuningEnP1DTag=0x00, tuningEnP1DmaDTag=0x00; //not used
    IspDrv* _targetVirDrv = NULL;
    MUINT32 _targetDupQueIdx = 0;
    MBOOL isApplyTuning;
    ...
    ISP_FUNC_DBG("P1Dbg_RPG: senDev=%d, mNum=0x%x, UpdateBit=0x%llx, EnP1=0x%x, EnP1Dma=0x%x, pBuf=%p", senDev, mNum, cfg.tuningUpdateBit, cfg.tuningEnP1,cfg.tuningEnP1Dma, cfg.pTuningBuf);
...
int ISP_RGB_PIPE::_config( void )
{
    ...
    if(this->isV3)
    {
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%llx),pTuningBuf(%p),hrz_crop(%d,%d)", \
                tuningUpdateBit,tuningPackage.pTuningBuf,tuningPackage.rrzCropX,tuningPackage.rrzCropY);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        imageioPackage.finalEnP2 = finalEnP2;

        ISP_FUNC_INF("(ISP_RGB_PIPE),isApplyTuning(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%llu)",this->isApplyTuning,finalEnP2,tuningUpdateBit);
    }
    else
    {
    getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag, this->isV3); // get P2 tuning En & Dma Tag
    ...
    if(this->en_p2 & CAM_CTL_EN_P2_MFB_EN)
    {
        ...
        ISP_FUNC_DBG("[TEST] imagioPak->mfb_cfg.bld_mode(%d),x(%lu),y(%lu),xofst(%d)",imageioPackage.mfbCfg.bld_mode,imageioPackage.mfbCfg.mfb_out.w,\
...
int ISP_RGB_PIPE::_write2CQ(void)
{
    ...
    if(this->isV3)
    {
        ...
        if(isApplyTuning==MTRUE){
            ...
            pTuningRegMappingQue = (isp_reg_t *)tuningPackage.pTuningBuf;
            tuningEnP2 = pTuningRegMappingQue->CAM_CTL_EN_P2.Raw;
            ISP_FUNC_DBG("tuningUpdateBit(0x%llx)",tuningUpdateBit);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
    }
    else
...
int ISP_YUV_PIPE::_config( void )
{
    ...
    if (this->isV3)
    {
        ...
        if(imageioPackage.isApplyTuning==MTRUE){
            ...
            ISP_FUNC_DBG("tuningUpdateBit(0x%llx),tuningEnP2(0x%08x),tuningEnP2Dma(0x%08x),pTuningBuf(%p),nr3dGainWeighting(0x%x)",tuningUpdateBit,tuningEnP2,tuningEnP2Dma,tuningPackage.pTuningBuf,tuningPackage.nr3dGainWeighting);
        }
        finalEnP2 = ((~tuningEnP2Tag)&this->en_p2)|(tuningEnP2Tag&tuningEnP2);
        imageioPackage.finalEnP2 = finalEnP2;

        ISP_FUNC_INF("(ISP_YUV_PIPE),isApplyTuning(0x%x),finalEnP2(0x%x),tuningUpdateBit(0x%llx)",this->isApplyTuning,finalEnP2,tuningUpdateBit);
    }
    else
    {
    getP2EnTuningTag(MTRUE,(ISP_DRV_CQ_ENUM)this->CQ, this->magicNum, this->drvScenario, this->sub_mode, tuningEnP2Tag, tuningEnP2DmaTag, this->isV3); // get P2 tuning En & Dma Tag
    ...
    if(this->en_p2 & CAM_CTL_EN_P2_MFB_EN)
    {
        ...
        ISP_FUNC_ERR("[Error]nr3d be enabled");
        return 0;
    }
    ...
    if(this->en_p2 & CAM_CTL_EN_P2_SRZ1_EN)
    {
        ...
        ISP_FUNC_ERR("[Error]SRZ1 be enabled");
    }
...
int ISP_YUV_PIPE::_write2CQ(void)
{
    ...
    if(this->en_p2 & CAM_CTL_EN_P2_LCE_EN){
        //this->m_pIspDrvShell->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,this->dupCqIdx,CAM_ISP_LCE_IMAGE_SIZE);
         ISP_FUNC_ERR("[Error]LCE be enabled");
    }
    // CRSP control
    ...
    if(this->en_p2 & CAM_CTL_EN_P2_FE_EN)
    {
        //this->m_pIspDrvShell->cqAddModule((ISP_DRV_CQ_ENUM)this->CQ,this->burstQueIdx,this->dupCqIdx,CAM_CDP_FE);
         ISP_FUNC_ERR("[Error]FE be enabled");
      }
    ISP_FUNC_DBG("-(ISP_YUV_PIPE),p2TuningEnTag(0x%08x),p2EnConfig(0x%08x)",tuningEnP2Tag,finalEnP2);
...
//        this->src_crop.h);
    ...
    if(enPureMdpOut == MFALSE){
        ISP_FUNC_DBG("imgxo va(0x%lu),pa(0x%lu),size(%d)",(unsigned long)(this->imgxo_out.memBuf.base_vAddr),(unsigned long)(this->imgxo_out.memBuf.base_pAddr),this->imgxo_out.memBuf.size);
...
//        ISP_FUNC_INF("QQ,imgxo srcCropW(%d),srcCropH(%d)",gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].dstDma[ISP_MDP_DL_IMGXO].srcCropW ,gpMdpMgrCfgData[this->burstQueIdx][p2Cq][this->dupCqIdx].dstDma[ISP_MDP_DL_IMGXO].srcCropH);
    ...
    // collect crop info for MdpMgr
        ...
        ISP_FUNC_INF("QQ,use this srcCropW(%lu),srcCropH(%lu)",this->src_crop.w,this->src_crop.h);
    }
...
int CAM_CRZ_PIPE::_config( void )
{
    ...
    if ( this->conf_cdrz ) {
        ...
        //CDRZ
        sizeIn.Width = (unsigned int)this->crz_in.w;
        sizeIn.Height = (unsigned int)this->crz_in.h;
        sizeOut.Width = (unsigned int)this->crz_out.w;
        sizeOut.Height = (unsigned int)this->crz_out.h;
        // crop
...
int CAM_CRZ_PIPE::_setZoom( void )
{
    ...
    if ( this->conf_cdrz ) {
        ISP_FUNC_DBG("data_ptr:0x%08lx",(unsigned long)data_ptr);
    }
...
//            }
        ...
        if(p2TuningFieldEnTag & CAM_CTL_EN_P2_NSL2A_EN){
            ...
            pDst->nsl2a.nsl2a_hrz_comp = tuningField.nsl2a.nsl2a_hrz_comp;
            pDst->nsl2a.nsl2a_vrz_comp = tuningField.nsl2a.nsl2a_vrz_comp;
        ...
        if(this->drvScenario == NSImageioIsp3::NSIspio::eDrvScenario_IP)
        {
            getP1EnTuningTag(MTRUE,
                           ...
                           p1TuningTopEnTag,
                           p1TuningTopDmaTag,
                           this->isV3); // get P1 tuning En & Dma Tag
...
{
    //MBOOL tdri_result = MTRUE; //not used
    //MUINT32 p2Cq; //not used
    //
...
{
    //MUINT32 cq = ISP_DRV_CQ0; //not used
    IspDrv* _targetVirDrv = NULL;
    MUINT32 _targetDupQIdx =  0;

    if (( ISP_PASS1 != pass1_path ) && ( ISP_PASS1_D != pass1_path ) &&
        ( ISP_PASS1_CAMSV != pass1_path ) && ( ISP_PASS1_CAMSV_D != pass1_path )) {
...
#endif//(ISP_RAW_D_SUPPORT == 1)
    else if ((pass1_path == ISP_PASS1_CAMSV) || (pass1_path == ISP_PASS1_CAMSV_D)) {
        MUINT32 ret = 0; // 0:success, 1:fail
        MUINT32 arg1[2] = {0};
        arg1[0] = MagicNum;
        if (pass1_path == ISP_PASS1_CAMSV)
            arg1[1] = _camsv_tg_;
        else if (pass1_path == ISP_PASS1_CAMSV_D)
            arg1[1] = _camsv2_tg_;

        ret = this->m_pPhyIspDrv->updateMagicNumber(arg1);
        if (ret) {
            ISP_FUNC_ERR("update MagicNum err(0x%x)", MagicNum);
            return -1;
        }
    }
    return 0;
...
int ISP_PASS1_CTRL::keepP1RrzCfg( IspRrzCfg const rrz_cfg )
{
    if ( ISP_PASS1 == rrz_cfg.pass1_path ) {
        ...
        ISP_FUNC_ERR("rrz keeper path err %lu",rrz_cfg.pass1_path);
        return -1;
    }
    return 0;
}
...
{
    MBOOL twin_result = MTRUE;
    //MUINT32 rrz_ctl = 0; //not used
    IspDrv* _targetVirDrv = NULL;
    MUINT32 _targetDupQIdx = 0;

    ISP_FUNC_DBG("ISP_PASS1_CTRL::setP1RrzCfg E:path(%lu),rrz_in_w/h(%lu/%lu),rrz_crop_x/y/fx/fy/w/h(%d/%d/%d/%d/%lu/%lu),rrz_out_w/h(%lu/%lu), sd_lw/xn(%lu/%lu),m_num(%lu)", \
        rrz_cfg.pass1_path, \
        rrz_cfg.rrz_in_size.w, \
        rrz_cfg.rrz_in_size.h, \
        rrz_cfg.rrz_in_roi.x, \
    ...
    ISP_FUNC_DBG("twin(%lu),path(%lu)",rrz_cfg.isTwinMode,rrz_cfg.pass1_path);
    ...
    if ( ISP_PASS1 == rrz_cfg.pass1_path ) {
        ...
        ISP_FUNC_ERR("Not support pass1 path(%lu)",rrz_cfg.pass1_path);
        return -1;
    }
    ...
    if ( rrz_cfg.isTwinMode) {
        if((rrz_cfg.rrz_in_roi.x*2 + rrz_cfg.rrz_in_roi.w) > rrz_cfg.rrz_in_size.w){
            ISP_FUNC_DBG("rrz input crop out of range: (x,y,floatx,floatxy,w,h = %d, %d, %d, %d, %lu, %lu),(size: %lu, %lu)",\
            ...
            ISP_FUNC_DBG("rrz view-angle issue: (x,y,floatx,floatxy,w,h = 0x%x,0x%x,0x%x,0x%x,%lu,%lu),(size:%lu,%lu)",\
                rrz_cfg.rrz_in_roi.x,rrz_cfg.rrz_in_roi.y,rrz_cfg.rrz_in_roi.floatX,rrz_cfg.rrz_in_roi.floatY,rrz_cfg.rrz_in_roi.w,rrz_cfg.rrz_in_roi.h,\
                rrz_cfg.rrz_in_size.w,rrz_cfg.rrz_in_size.h);
        }
...
waitBufReady( MUINT32 dmaChannel )
{
    ...
    maxCnt = MAX_CNT(waitMs, _CAM_INT_WAIT_TIMEOUT_MS);
    ...
    if ( _imgo_ == rt_dma || _imgo_d_ == rt_dma || _rrzo_ == rt_dma || _rrzo_d_ == rt_dma || _camsv_imgo_ == rt_dma || _camsv2_imgo_ == rt_dma) {
        ...
        ISP_FUNC_DBG("waitBufReady[%d]:[%x,%x] rtBufCtrl.ctrl(%d)/id(%d)/ptr(%p)",rt_dma,this->path, this->path_D,buf_ctrl.ctrl,buf_ctrl.buf_id,(unsigned int*)buf_ctrl.data_ptr);
        ...
        if (bWaitBufRdy) {
            ...
            if(ret != MTRUE)
            {
                ...
                if (( _imgo_d_ == rt_dma ) || (_rrzo_d_ == rt_dma)) {
                    ...
                    irq_TG_SOF.UserInfo.Status=CAM_CTL_INT_P1_STATUS_D_SOF1_INT_ST;
                    irq_TG_SOF.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_P1_ST_D;
                }
                else if( _camsv_imgo_ == rt_dma ) {
                    irq_TG_SOF.Status = CAMSV_INT_STATUS_TG_SOF1_INT_ST;
                    irq_TG_SOF.Type = ISP_DRV_IRQ_TYPE_INT_CAMSV;
                    irq_TG_SOF.UserNumber=(int)ISP_DRV_IRQ_USER_ISPDRV;
                    irq_TG_SOF.UserName = "camsv";
                    irq_TG_SOF.UserInfo.Status=CAMSV_INT_STATUS_TG_SOF1_ST;
                    irq_TG_SOF.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_CAMSV;
                }
                else if( _camsv2_imgo_ == rt_dma ) {
                    irq_TG_SOF.Status = CAMSV2_INT_STATUS_TG_SOF1_INT_ST;
                    irq_TG_SOF.Type = ISP_DRV_IRQ_TYPE_INT_CAMSV2;
                    irq_TG_SOF.UserNumber=(int)ISP_DRV_IRQ_USER_ISPDRV;
                    irq_TG_SOF.UserName = "camsv_d";
                    irq_TG_SOF.UserInfo.Status=CAMSV2_INT_STATUS_TG_SOF1_ST;
                    irq_TG_SOF.UserInfo.Type=ISP_DRV_IRQ_TYPE_INT_CAMSV;
                }
...
enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo,MINT32 enqueCq,MINT32 dupCqIdx,MBOOL bImdMode)
{
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        if( NULL != bufInfo.next ){
            ISP_QUE_DBG(rt_dma,"dma(%d),id(%d),size(0x%x),VA(0x%zx),PA(0x%zx),S/C(%d/%d),bufidx(%d),replace:new(0x%zx),bufidx(%d),bImdMode(%d)",\
                        rt_dma,\
                        buf_info.memID,\
                        buf_info.size,\
                        buf_info.virtAddr,\
            ...
            ISP_QUE_DBG(rt_dma,"dma(%d),id(%d),size(0x%x),VA(0x%zx),PA(0x%zx),S/C(%d/%d),bufidx(%d),bImdMode(%d)",\
                        rt_dma,\
                        buf_info.memID,\
                        buf_info.size,\
                        buf_info.virtAddr,\
        ...
        //
        ISP_FUNC_DBG("enqueCq(%d),p2Cq(%d),dupCqIdx(%d),dma(%d),memID(0x%x),size(%lu),vAddr(0x%lx),pAddr(0x%lx)", \
                                    enqueCq,p2Cq,dupCqIdx,rt_dma, \
                                    m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.back().memID, \
                                    (unsigned long)(m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.size()), \
                                    (unsigned long)(m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.back().base_vAddr),
                                    (unsigned long)(m_p2HwBufL[p2Cq][dupCqIdx][rt_dma].bufInfoList.back().base_pAddr));
        //
    }
    //
    return 0;
...
enqueueHwBuf( MUINT32 dmaChannel, stISP_BUF_INFO bufInfo, MVOID* private_data ,MBOOL bImdMode)
{
    UNUSED(private_data);
    MINT32 rt_dma = getDmaBufIdx(dmaChannel);
    //stISP_BUF_HEADER_LIST *plist = NULL; //not used
    //MUINT32 i = ISP_BUF_INFO_NODE_MAX_CNT; //not used
    ...
    if (0){
        ...
        ISP_QUE_INF(rt_dma,"[MyDeQ]EnQHwBuf::ispDrv=%p/%p, CmdQ-IN/OUT Z=[0x%x,0x%x], HW Z=[0x%x,0x%x].", \
           (unsigned int*)m_pP1IspDrv, (unsigned int*)m_pP1IspDrvD, \
...
ISP_BUF_CTRL::returnDeqBufInfo( MINT32 rt_dma, stISP_BUF_INFO* bufInfo)
{
    ...
    if ( NULL == bufInfo->next ){
        ...
        if (i < ISP_BUF_INFO_NODE_MAX_CNT) {
            ISP_QUE_DBG(rt_dma,"[MyDeQ]returnDeqBufInfo:: node=0x%zx", plist->mN[i].mInfo.base_pAddr);
        ...
        {
            if(bVF_en){
                ISP_FUNC_DBG("dma:0x%x: find no match baseaddr, 0x%zx\n",rt_dma,bufInfo->base_pAddr);
                ISP_FUNC_DBG("current available base addr\n");
                for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++){
                    if(plist->mN[i].mOccupied)
                        ISP_FUNC_DBG("0x%zx",plist->mN[i].mInfo.base_pAddr);
                }
            }
        }
    }
        ...
        if (i < ISP_BUF_INFO_NODE_MAX_CNT) {
            ...
            {
                if(bVF_en){
                    ISP_QUE_DBG(rt_dma,"dma:0x%x: find no match baseaddr, 0x%zx\n",rt_dma,bufInfo->next->base_vAddr);
                    ISP_QUE_DBG(rt_dma,"current available base addr\n");
                    for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++){
                        if(plist->mN[i].mOccupied)
                            ISP_QUE_DBG(rt_dma,"0x%zx",plist->mN[i].mInfo.base_pAddr);
                    }
                }
            }
        } else {
            ISP_QUE_DBG(rt_dma,"dma:0x%x:find no match baseaddr,caused this is exchange buf, 0x%zx\n",rt_dma,bufInfo->next->base_pAddr);
            ISP_QUE_DBG(rt_dma,"current available base addr\n");
            for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++){
                if(plist->mN[i].mOccupied)
                    ISP_QUE_DBG(rt_dma,"0x%zx",plist->mN[i].mInfo.base_pAddr);
...
saveDeqBufInfo( MINT32 rt_dma )
{
    ...
    for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i++) {
        ...
        if (!plist->mN[i].mOccupied) {
            ...
            ISP_QUE_DBG(rt_dma,"saveDeqBufInfo[A] plist=%p,[%d]PA=0x%zx, mIdx=%d", (unsigned int*)plist,i,ptr->base_pAddr, plist->mIdx);
            return ptr;
        }
    }
#endif
    if (0) { //It's OK in IP-Base path.
        for (i = 0; i < ISP_BUF_INFO_NODE_MAX_CNT; i+=8) {

            ISP_FUNC_ERR("saveDeqBufInfo[C] Fail. dma=%d,plist=%p, mOccupied:[%d[0x%zx]/%d[0x%zx]/%d[0x%zx]/%d[0x%zx]/%d[0x%zx]/%d[0x%zx]/%d[0x%zx]/%d[0x%zx]]",rt_dma,(unsigned int*)plist,\
    ...
    if (0) {
        ...
        ISP_QUE_DBG(rt_dma,"saveDeqBufInfo[D][Error] plist=%p,ptr=%p, mIdx=%d", (unsigned int*)plist,(unsigned int*)ptr, plist->mIdx);
    }
    if(ptr!= NULL)
        ISP_QUE_DBG(rt_dma,"[MyDeQ]saveDeqBufInfo: node=0x%zx", ptr->base_pAddr);
    return ptr;
}
//
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        if ( _imgo_ == rt_dma || \
            ...
            if( (_imgo_ == rt_dma) || (_rrzo_ == rt_dma) ){
                ...
                ISP_QUE_DBG(rt_dma,"camsv/camsv2 fbc: 0x%x/0x%x_0x%lx/0x%lx\n",fbc[0],fbc[1],readReg(0x0000901C),readReg(0x0000981C));
            }
            //
            ...
            for(MUINT32 i=0;i<count;i++) {
                ...
                bufInfo->dma_crop.w -= (bufInfo->dma_crop.w & 0x1);
                ...
                if(bufHeaderInfo)
                    ...
                    ISP_FUNC_DBG("bufHeaderInfo is NULL,this will cause NE issue\n");//It's OK in IP-Base path.
        ...
        while(count--) {
            ...
            if (bufSize) {
                ...
                if(isDequeMdp==MTRUE){
                    ...
                    if(mdpRet == MDPMGR_NO_ERROR){
                        ...
                        if((pWorkingBuffer!=NULL)&&(pIspReg!=NULL)) {
                            ...
                            ISP_FUNC_ERR("[Error]pWorkingBuffer(%08lx),pIspReg(0x%08lx)",(unsigned long)pWorkingBuffer,(unsigned long)pIspReg);
                ...
                //
                ISP_FUNC_DBG("dma(%d),hwsize(%lu),p2Cq(%d),dequeDupCqIdx(%d),imgxoEnP2(0x%x),memID(0x%x),size(%lu),vaddr(0x%lu),paddr(0x%lu)",rt_dma,(unsigned long)(m_p2HwBufL[p2Cq][this->dequeDupCqIdx][rt_dma].bufInfoList.size()),p2Cq,this->dequeDupCqIdx, \
                        gpMdpMgrCfgData[this->dequeBurstQueIdx][p2Cq][this->dequeDupCqIdx].imgxoEnP2, \
                        bufList.pBufList->back().memID,(unsigned long)(bufList.pBufList->size()),(unsigned long)(bufList.pBufList->back().base_vAddr),(unsigned long)(bufList.pBufList->back().base_pAddr));
...
getCurrHwBuf( MUINT32 dmaChannel )
{
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        //
        ISP_FUNC_DBG("dma:(%d)/memID(0x%x)/vAddr:(0x%zx)/pAddr:(0x%zx) ",rt_dma, \
...
debugPrint( MUINT32 dmaChannel )
{
    ...
    for ( it = m_hwbufL[rt_dma].bufInfoList.begin(); it != m_hwbufL[rt_dma].bufInfoList.end(); it++ ) {
        ISP_FUNC_DBG("m_hwbufL[%d].base_vAddr:[0x%zx]/base_pAddr:[0x%zx] ",rt_dma,it->base_vAddr,it->base_pAddr);
    }
    ISP_FUNC_DBG("X");
...
{
    ISP_FUNC_DBG("E: pIspDrv(%p)",(unsigned int*)pIspDrv);
    //
...
MVOID* IspEventThreadImp::ispEventThread(void *arg)
{
    ...
    ISP_FUNC_DBG("E: tid=%d m_pPhyIspDrv(%p)", gettid(),(unsigned int*)_this->m_pPhyIspDrv);
