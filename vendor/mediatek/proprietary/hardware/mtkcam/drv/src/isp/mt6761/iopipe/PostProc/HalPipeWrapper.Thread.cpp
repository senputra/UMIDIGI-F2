...
EXTERN_DBG_LOG_VARIABLE(P2PipeWrapper_Thread);
    ...
    MUINT32 callerID)
{
    ...
    if(bufferListTag==EBufferListTag_Batch)
    {
        ...
        LOG_DBG("Erase: updateBNList(%d)",mvEnqueueBufNode.size());
...
)
{
    ...
    MUINT32 a=0,b=0;
    int frameIndex = mvEnqueuedBufBatch[bufNode.idxInBufBatchList].dequedNum;
        ...
        if( bufNode.portEn & (1 << i))
        {
            ...
            switch(deQret)
            {
                case eIspRetStatus_Failed:
                    ...
                    {
                        LOG_INF("Fst mvOut mva(0x%x)",
                                mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut[0].mBuffer->getBufPA(0));
                    }
                    else
                    {
...
//    LOG_INF("QQ id : (%x)  callback (%x)",bufNode.idxInBufBatchList,mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mpfnCallback);
    ...
    if(retFrame==true)
    {
        ...
        if (mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut.size() > 0)
        {
            LOG_INF("FmvOut(0x%x),dCq(%d/%d)",
                    mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut[0].mBuffer->getBufPA(0),
                    bufNode.p2cqIdx,bufNode.p2cqDupIdx);
        }
        else
        {
            LOG_ERR("no output dma");
        ...
        if(dequeSuccess==true)
        {
            ...
            if(mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mpfnCallback == NULL)
            {
                ...
                if(bufNode.jpgo_idx>=0)
                {
                    ...
                    mvEnqueuedBufBatch[bufNode.idxInBufBatchList].rParams.mvFrameParams[frameIndex].mvOut[bufNode.jpgo_idx].mBuffer->setBitstreamSize(jpgRealSize);
                }
