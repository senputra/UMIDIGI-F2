                                      ...
                                      DUAL_OUT_CONFIG_STRUCT *ptr_out_param)
{
	...
	else
	{
		...
		if (ptr_in_param->CAM_A_AFO_BASE_ADDR.Bits.BASE_ADDR == ptr_in_param->CAM_B_AFO_BASE_ADDR.Bits.BASE_ADDR)
		{
			...
			{/****************************/
				dual_driver_printf("CCU WIN_SCENARIO_AF_FAST_P1: %d\n", TWIN_SCENARIO_AF_FAST_P1);
				dual_driver_printf("CCU ptr_in_param->SW.TWIN_SCENARIO: %d\n", ptr_in_param->SW.TWIN_SCENARIO);
				if (TWIN_SCENARIO_AF_FAST_P1 == ptr_in_param->SW.TWIN_SCENARIO)
				{
					left_loss_af = DUAL_AF_TAPS;
					right_loss_af = DUAL_AF_TAPS;
				...
				if (ptr_in_param->CAM_A_AFO_BASE_ADDR.Bits.BASE_ADDR == ptr_in_param->CAM_B_AFO_BASE_ADDR.Bits.BASE_ADDR)
				{
					...
					if (TWIN_SCENARIO_NORMAL == ptr_in_param->SW.TWIN_SCENARIO)
					{
						...
						else
						{
							...
							if (ptr_in_param->SW.TWIN_AF_BLOCK_XNUM*(int)ptr_in_param->CAM_A_AF_BLK_0.Bits.AF_BLK_XSIZE + 2 +
								ptr_in_param->SW.TWIN_AF_OFFSET <= dmx_valid[0] + (int)ptr_in_param->CAM_A_AF_BLK_0.Bits.AF_BLK_XSIZE)
							{
								ptr_out_param->CAM_A_AF_BLK_1.Bits.AF_BLK_XNUM = ptr_in_param->SW.TWIN_AF_BLOCK_XNUM;
							}
							else
							{
								ptr_out_param->CAM_A_AF_BLK_1.Bits.AF_BLK_XNUM = (dmx_valid[0] +
