...
MBOOL CAM_TIMESTAMP::TimeStamp_mapping(MUINT32* pSec, MUINT32* pUsec, MUINT32* pSec_B, MUINT32* pUsec_B, _isp_dma_enum_ dma,ISP_HW_MODULE module)
{
    ...
    if(m_ccu_support_flag == 1){
        if((this->m_CCU_reg_value_HIGH == 0) && (this->m_CCU_reg_value_LOW == 0)){
            ...
            if((this->m_start_time_CCU[0] == 0) && (this->m_start_time_CCU[1] == 0)){
                m_ccu_support_flag = 0;
                CAM_FUNC_INF("Kernel 32bit project , not support CCU");
            }
        ...
        *pUsec_B = time_CCU % 1000000;
        ...
        if(m_ccu_support_flag == 0){
            *pSec_B  = 0;
            *pUsec_B = 0;
        }
   }
   else if(m_ccu_support_flag == 0){
       *pSec_B  = 0;
       *pUsec_B = 0;
   }
