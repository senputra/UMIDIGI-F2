    ...
    IMEM_BUF_INFO* pInfo)
{
    ...
    IMEM_INF("ionHandle[0x%x]/size[0x%x]/PA[0x%x]/VA[0x%x] \n",pInfo->ionHandle,pInfo->size,pInfo->phyAddr,pInfo->virtAddr);
    return 0;
}
...
MINT32 IMemDrvImp::freeSecBuf(
    IMEM_BUF_INFO* pInfo)
{
    IMEM_DBG("ionHandle[0x%x]/size[0x%x]/VA[0x%x]/PA[0x%x]",pInfo->ionHandle,pInfo->size,pInfo->virtAddr,pInfo->phyAddr);
    ...
    IMEM_BUF_INFO* pInfo, MBOOL bImport)
{
    ...
    if ( 0 ==  buf_map.count(pInfo->virtAddr) ) {
        ...
        if ( IMEM_MIN_ION_FD > pInfo->memID ) {
            ...
            mm_data.config_buffer_param.eModuleID = CAM_M4U_PORT_DMA_MODULEID;    // 1;
...
MINT32  IMemDrvImp::doIonCacheFlush(IMEM_BUF_INFO* pInfo,eIONCacheFlushType flushtype, MBOOL bImport)
{
    ...
    MINT32 IonBufFd;
    MINT32 err;
    void* pVa = (void*)pInfo->virtAddr;
    MUINT32 uSize = pInfo->size;
    IMEM_DBG("+");
    ...
    {
        IMEM_ERR("%s: return. Only support ION_CACHE_FLUSH_BY_RANGE", __func__);
        return -1;
    }
    if(ion_custom_ioctl(mIonDrv, ION_CMD_SYSTEM,&sys_data))
    {
...
MINT32 IMemDrvImp::cacheSyncbyRange(IMEM_CACHECTRL_ENUM ctrl, IMEM_BUF_INFO* pInfo, MBOOL bImport)
{
    ...
    else
    {
        if((ctrl == IMEM_CACHECTRL_ENUM_FLUSH) && (pInfo->size >= 0x3200000))
        {
            ...
            IMEM_DBG("+ Flush by range, c/m/va/sz/pa(%d/0x%x/0x%x/0x%x/0x%x)",ctrl,pInfo->memID,pInfo->virtAddr,pInfo->size,pInfo->phyAddr);
            return 0;
        }
