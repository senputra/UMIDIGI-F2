...
private:
    ...
    MINT32          mp2CQDupIdx[ISP_DRV_DIP_CQ_NUM];
    MINT32          mp2CQUserNum[ISP_DRV_DIP_CQ_NUM];  //current enqueued user num for each CQ
    ...
    _debug_info_ debug)
{
    ...
    DipCQMgr_INF("CQdBG (%d_%d_%d_%d_0x%x), (%d)",p2CQ, p2BurstQIndx, frameNum, debug.eDrvScen, debug.senDev, enquedCQList.size());
    ...
    if(!enquedCQ)
    {
        ...
        {
            DipCQMgr_INF("Block Enqueue Here, P2CQ (%d), eDrvScen/SenDev(0x%x/0x%x)", p2CQ, debug.eDrvScen, debug.senDev);
