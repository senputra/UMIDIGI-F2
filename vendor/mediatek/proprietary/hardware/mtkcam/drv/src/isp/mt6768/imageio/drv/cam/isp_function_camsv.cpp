...
{
    CAM_FUNC_DBG("CAMSV_TG_CTRL::_config+");
    ...
    if (m_Fmt_Sel.Raw == SV_TG_FMT_JPG) {
        //jpeg
        //This is a temp test method and might can be improved
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_SEN_MODE,SOT_MODE, 1);
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_SEN_MODE,SOT_CLR_MODE, 1);
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_SEN_MODE,SOF_SRC, 2);
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_PATH_CFG,JPG_LINEND_EN, 1);
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_PATH_CFG,JPGINF_EN, 1);
        CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_TG_SOF_CNT,SOF_CNT, 0xff);
    }
...
{
    CAM_FUNC_DBG("CAMSV_TOP_CTRL::_config+");
    ...
    switch(this->camsv_top_ctl.FMT_SEL.Bits.TG1_FMT){
        ...
        case SV_TG_FMT_RAW12:
            ...
            break;
        case SV_TG_FMT_RAW14: //DVT test RAW14 without pak
            ...
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_MODULE_EN,PAK_SEL,0);
#endif
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_PAK,PAK_MODE,3);
            break;
        case SV_TG_FMT_YUV422:
            CAM_FUNC_INF("****** %s %d, TG1_FMT(%d) SV_TG_FMT_YUV422", __FUNCTION__, __LINE__, this->camsv_top_ctl.FMT_SEL.Bits.TG1_FMT);
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_MODULE_EN,PAK_EN,0);
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_MODULE_EN,PAK_SEL,1);
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_PAK,PAK_MODE,0);
            break;
            /* Fall through */
        case SV_TG_FMT_JPG:
            CAM_FUNC_INF("****** %s %d, TG1_FMT(%d) SV_TG_FMT_JPG", __FUNCTION__, __LINE__, this->camsv_top_ctl.FMT_SEL.Bits.TG1_FMT);
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_MODULE_EN,PAK_EN,0);
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_MODULE_EN,PAK_SEL,1);
            CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_PAK,PAK_MODE,0);
            break;
...
MINT32 CAMSV_TOP_CTRL::_enable( void* pParam  )
{
    ...
    CAM_FUNC_DBG("CAMSV_TOP_CTRL::_enable+");
#if 0 //move to init in isp_drv.cpp
...
MINT32 CAMSV_TOP_CTRL::_disable( void* pParam )
{
    ...
    CAM_FUNC_DBG("CAMSV_TOP_CTRL::_disable+");
...
{
    CAM_FUNC_DBG("DMAO_CAMSV_B::_config+");
        ...
        case CAMSV_DMA_IMGO:
            {
                ...
                CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_IMGO_STRIDE,FORMAT_EN, this->dma_cfg.format_en);
                CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_IMGO_STRIDE,FORMAT, this->dma_cfg.format);
                //Write below, <<24 & << 16
                //CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_IMGO_STRIDE,BUS_SIZE_EN, this->dma_cfg.bus_size_en);
                //CAMSV_WRITE_BITS(this->m_pDrv,CAMSV_IMGO_STRIDE,BUS_SIZE, this->dma_cfg.bus_size);
                //
                CAM_FUNC_INF("BusSizeCal: TG1_FMT(%d), m_PixMode(%d)",this->fmt_sel.Bits.TG1_FMT, this->m_PixMode);
                ...
                switch( this->dma_cfg.bus_size ) {
                    ...
                    case 1: // 16-bit bus size
                        ...
                        CAM_FUNC_INF("****** %s %d, CAMSV_IMGO_STRIDE write", __FUNCTION__, __LINE__);
                ...
                CAMSV_WRITE_REG(this->m_pDrv,CAMSV_IMGO_CON2,0x0004F004);                                                   /* 0x1A050240 */
                CAMSV_WRITE_REG(this->m_pDrv,CAMSV_IMGO_CON3,0x0002F002);                                                   /* 0x1A050244 */
...
MINT32 DMAO_CAMSV_B::_enable( void* pParam )
{
    ...
    CAM_FUNC_DBG("DMAO_CAMSV_B::_enable+");
...
MINT32 DMAO_CAMSV_B::_disable( void* pParam )
{
    ...
    CAM_FUNC_DBG("DMAO_CAMSV_B::_disable+");
...
MBOOL CAMSV_BUF_CTRL::PipeCheck(void)
{
    ...
    if(ppc.FIFO_FULL_STATUS() == MFALSE) {
        CAM_FUNC_ERR("FIFO check fail\n");
    }
...
EXIT:
    switch(ret){
        case eCmd_Fail:
            ...
            break;
        case eCmd_Pass:
            CAM_FUNC_DBG("%s, waitbufready pass",str);
            break;
...
{
    CAM_FUNC_DBG("BUF_CTRL_CAMSV_IMGO:CAMSV_BUF_CTRL::config+");
...
{
    CAM_FUNC_DBG("BUF_CTRL_CAMSV_IMGO:CAMSV_BUF_CTRL::_enable+");
    if(pParam != NULL)
...
MINT32 CAMSV_BUF_CTRL::_disable( void* pParam )
{
    ...
    CAM_FUNC_DBG("CAMSV_BUF_CTRL::_disable+");
...
MBOOL CAMSV_BUF_CTRL::enqueueHwBuf( stISP_BUF_INFO& buf_info,MBOOL bImdMode)
{
    ...
    if(buf_info.bReplace == MFALSE){
        ...
        CAM_FUNC_DBG("PA(0x%p_0x%p),FH_VA(%p),size(0x%x),enque_sof(%d),reg_timestmp(0x%08x)",\
        ...
        CAM_FUNC_DBG("PA(0x%p_0x%p,new:0x%p_0x%p),FH_VA(%p,new:%p),size(0x%x),enque_sof(%d)",\
            (void*)ptr->image.mem_info.pa_addr ,\
            (void*)ptr->header.pa_addr ,\
            (void*)buf_info.u_op.enque.image.mem_info.pa_addr ,\
            (void*)buf_info.u_op.enque.header.pa_addr ,\
            (void*)ptr->header.va_addr ,\
            (void*)buf_info.u_op.enque.header.va_addr ,\
            ptr->image.mem_info.size ,\
            _sof_idx);
    }
    //
...
EXIT:
    switch(this->id()){
        case _camsv_imgo_:
            CAM_FUNC_DBG("PA(0x%p_0x%p),FH_VA(%p),size(0x%08x),cur sof(%d)",\
            ...
            CAM_FUNC_DBG("reg timestamp = 0x%08x, header timestamp = 0x%08x",\
                CAMSV_READ_REG(this->m_pDrv, CAMSV_TG_TIME_STAMP),\
                _header_time_stamp);
            break;
        default:
            CAM_FUNC_DBG("PA(0x%p_0x%p),FH_VA(%p),size(0x%x),cur sof(%d)",\
                (void*)_buf.image.mem_info.pa_addr ,\
                (void*)_buf.header.pa_addr ,\
                (void*)_buf.header.va_addr ,\
                _buf.image.mem_info.size ,\
...
MBOOL CAMSV_PIPE_CHECK::FIFO_FULL_STATUS(void)
{
    MUINT32 _dbgsel = 0x303;
    MUINT32 _dbgval = 0;
    CAMSV_WRITE_REG(this->m_pDrv, CAMSV_DBG_SET , 0xF0);
    CAMSV_WRITE_REG(this->m_pDrv, CAMSV_DMA_DEBUG_SEL, _dbgsel);
    _dbgval = CAMSV_READ_REG(this->m_pDrv, CAMSV_DBG_PORT);
    CAM_FUNC_INF("FIFO_FULL status: [IMGO]0x%x", _dbgval);
