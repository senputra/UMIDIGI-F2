...
static NormalPipe_FrmB* pNormalPipe_FrmB[NPIPE_Sensor_RSVD] = {NULL, NULL, NULL, NULL, NULL};
...
INormalPipe_FrmB::createInstance(MUINT32 *pSensorIndex, MUINT32 sensorCnt,char const* szCallerName, MINT32 burstQnum)
{
    ...
    if (1 == sensorCnt) {//NPIPE_Sensor_0 ||  NPIPE_Sensor_1
        ...
        if (pSensorIndex[0] > NPIPE_Sensor_4){
        ...
        if (NULL == pNPipe)  {
            ...
            if (pNPipe->mpFrmB_Thread == NULL){
                ...
                PIPE_INF("[%d]createInstance, %s,pNormalPipe_FrmB_Thread[%d]=%p create\n",pSensorIndex[0],szCallerName, pSensorIndex[0], pNPipe->mpFrmB_Thread);
            }
        }
        else {
...
NormalPipe_FrmB::destroyInstance(char const* szCallerName)
{
    ...
    if (pNormalPipe_FrmB[idx]->mUserCnt == 0)  {
        ...
        if (this->m_bStarted == MTRUE) {
            PIPE_ERR("error FrmB_Thread doesn't be uninit\n");
            ret = this->uninit();
            PIPE_DBG("NormalPipe_FrmB::uninit result = %d\n", ret);
        }
    ...
    , mpFrmB_Thread(NULL)
{
    ...
    m_bStarted = MFALSE;
    m_bAbortDeque = MFALSE;
    mRingBufDepth = 0;
    m_semConfigStart.count = 0;

    ::sem_init(&this->m_semConfigStart, 0, 0);
...
NormalPipe_FrmB::start()
{
    ...
    if (this->mpCamIOPipe)
    {
        // Start HW after enque thread is ready to wait SOF
        ::sem_wait(&this->m_semConfigStart);
        PIPE_DBG("mpCamIOPipe->start()\n");
        ret = this->mpCamIOPipe->start();
    }

    if (ret == MTRUE) {
        // Enable HW sync for VSDOF
        if (this->m_bN3DEn == MTRUE) {
            PIPE_DBG("Start HwSync: SenIdx=%d", mpSensorIdx[0]);
            HWSyncDrv* pHwSync = NULL;
            pHwSync = HWSyncDrv::createInstance();
            if (pHwSync->sendCommand(HW_SYNC_CMD_ENABLE_SYNC, \
                0, pAllSensorInfo->mSenInfo[mpSensorIdx[0]].mDevId, HW_SYNC_BLOCKING_EXE) != 0) {
...
NormalPipe_FrmB::stop()
{
    ...
    if (this->m_bN3DEn == MTRUE) {
        PIPE_DBG("Stop HwSync: SenIdx=%d", mpSensorIdx[0]);
        HWSyncDrv* pHwSync = NULL;
        pHwSync = HWSyncDrv::createInstance();
        if (pHwSync->sendCommand(HW_SYNC_CMD_DISABLE_SYNC, \
            HW_SYNC_BLOCKING_EXE, pAllSensorInfo->mSenInfo[mpSensorIdx[0]].mDevId, 0) != 0) {
            PIPE_ERR("N3D sstop fail\n");
        }
        this->m_bN3DEn = MFALSE;
        pHwSync->destroyInstance();
    }
    ...
    for (int i = 0; i < recordMax; i++) {
        for (int j = 0; j < 2; j++) {
            memset(m_replaceTable[i][j], 0, sizeof(_replaceBuf_));
        }
    }
#endif
    this->m_bStarted = MFALSE;
...
NormalPipe_FrmB::abort() {
    ...
    PIPE_DBG("++++ abort[%d] ++++",this->mpSensorIdx[0]);
...
MVOID
NormalPipe_FrmB::abortDeque() {
    this->m_bAbortDeque = MTRUE;
}
...
NormalPipe_FrmB::uninit()
{
    ...
    if (this->m_bStarted == MTRUE) {
        PIPE_ERR("error FrmB_Thread doesn't be stopped\n");
        if(this->stop() == MFALSE){
            PIPE_ERR("error: NormalPipe_FrmB stop fail\n");
            return MFALSE;
        }
    }
...
NormalPipe_FrmB::enque(QBufInfo const& rQBuf)
{
    ...
    if(this->m_bStarted == MFALSE){
        ...
        for (MUINT32 i = 0; i < rQBuf.mvOut.size(); i++)
        {
...
#ifdef USE_IMAGEBUF_HEAP
#if (HAL3_IPBASE == 1)
            PIPE_DBG("Before ReplaceToDummy, PA = 0x%zx, VA = 0x%x, replace = %d\n", rQBuf.mvOut.at(i).mBuffer->getBufPA(0), (MINT32)rQBuf.mvOut.at(i).mBuffer->getBufVA(0), doReplace);
            ...
            if (!this->ReplaceToDummy((MUINT32)i, &_buf, &_dumBuf, &doReplace)){
                return MFALSE;
            }
            PIPE_DBG("After ReplaceToDummy, PA = 0x%zx, VA = 0x%x, replace = %d\n", _buf.mPa, (MINT32)_buf.mBuffer->getBufVA(0), doReplace);
    ...
    //all enque request r all push into enquereqest , but enque before start have special control flow at enquerequest()
    {
        ...
        {
            _buf = rQBuf.mvOut.at(i);
            _buf.mShutterTimeNs = rQBuf.mShutterTimeNs;
            PIPE_DBG("_buf.mShutterTimeNs = %lld\n", _buf.mShutterTimeNs);
            ...
            PIPE_INF("[MyEnq:%d] dma(0x%x) pa(0x%zx) io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x) _buf.mShutterTimeNs = %lld\n",
                ...
                _buf.FrameBased.mMagicNum_tuning, _buf.mShutterTimeNs);
        }
#if (HAL3_IPBASE == 1)
        this->mEnqContainerLock.unlock();
#endif
    ...
    if((this->m_bStarted == MFALSE) && (this->GetRingBufferDepth() == 1) ){
        ...
        if (this->mpCamIOPipe) {
            this->mpCamIOPipe->sendCommand(EPIPECmd_SET_FRM_TIME, (MINTPTR)(rQBuf.mShutterTimeNs/1000000), 1, 0);
            this->mpFrmB_Thread->updateFrameTime((MUINT32)(rQBuf.mShutterTimeNs/1000000), 1);
        }
    }
_enque_exit:
    //
    ...
    if(this->m_PerFrameCtl && (MUINT32)_sof_idx == this->mPrvEnqSOFIdx){
        ...
        if (_popbuf.mBuffer != NULL)
            PIPE_INF("warning:can't push enque into drv more than once within 1 frame,drop request(pa:0x%zx,mag:0x%x)sof(%d/%d)\n",
                _popbuf.mBuffer->getBufPA(0),_popbuf.FrameBased.mMagicNum_tuning, this->mPrvEnqSOFIdx,(MUINT32)_sof_idx);
        if(this->m_DropCB)
...
NormalPipe_FrmB::deque(QBufInfo& rQBuf, MUINT32 u4TimeoutMs)
{
    ...
    #if (HAL3_IPBASE == 1)
    {
        Mutex::Autolock lock(this->mEnqueMutex);
        if (this->mIsCfgUpdating == MTRUE) {
            PIPE_DBG("mEnqueCond.wait ++");
            this->mEnqueCond.wait(this->mEnqueMutex);
            PIPE_DBG("mEnqueCond.wait --");
        }
    }
    #endif
    for (MUINT32 ii=0 ; ii<rQBuf.mvOut.size() ; ii++ ) {
        //
        if (this->m_bStarted == MFALSE || this->m_bAbortDeque == MTRUE) {
            PIPE_DBG("Deque is stopped");
            ret = MFALSE;
            return ret;
        }
        ...
        if ( rQTSBufInfo.vBufInfo.size() >= 1 )  {
            ...
            for (MUINT32 idx = 0; idx < rQTSBufInfo.vBufInfo.size(); idx++)  {
                ...
                if(idx >= 1){
                    ...
                    (MUINT32)rQTSBufInfo.vBufInfo.size(),portID.index);
                ...
                if (NULL == pframe) {
                    PIPE_ERR("[MyDeQ:%d]Fail: PortId(%d), 0==pframe, idx=%d, pa=0x%zx",\
                ...
                if(buff.mMetaData.m_bDummyFrame == MTRUE){
                    ...
                    //
                    PIPE_INF("[MyDeq:%d] dummy frame: dma(0x%x) pa(0x%zx) io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x)\n",SIdx,portID.index,\
                    buff.mPa,\
                    ...
                    PIPE_INF("[MyDeq:%d] dma(0x%x) pa(0x%zx) io(0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x_0x%x) mag(0x%x_0x%x)\n",SIdx,portID.index,\
                    buff.mPa,\
                    result.mCrop_s.p.x,result.mCrop_s.p.y,result.mCrop_s.s.w,result.mCrop_s.s.h, \
                    result.mCrop_d.p.x,result.mCrop_d.p.y,result.mCrop_d.s.w,result.mCrop_d.s.h, \
                    result.mDstSize.w,result.mDstSize.h,\
...
MBOOL
NormalPipe_FrmB::_setTgPixelAndDualMode(MUINT32 ScenId, MUINT32 fps)
{
    MUINT32 tg = 0, SIdx = 0, pixelCnt = 0;
    ...
    if (mpHalSensor) {
        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId,
        ...
        PIPE_ERR("Error: mpHalSensor is null");
        return MFALSE;
    }
    ...
    /*if (mpEnabledSensorCnt == 1)*/
    {
        ...
        if (tg == CAM_TG_1) {
            ...
            if ( (NSCam::SENSOR_TYPE_RAW  == _NORMALPIPE_GET_SENSORTYPE(SIdx)) ||
                ...
                enable2PixelTwin = pixelCnt == TWO_PIXEL_MODE ? 1:0;
...
NormalPipe_FrmB::configPipe(QInitParam const& vInPorts)
{
    ...
    MUINT64                    tgi_pixel_rate[mpEnabledSensorCnt];
    ...
    if (mpEnabledSensorCnt != vInPorts.mSensorCfg.size()) {
        PIPE_ERR("NPipe::configPipe SensorCfgCnt is not match. (%d,%u)", mpEnabledSensorCnt, (MUINT32)vInPorts.mSensorCfg.size());
        return MFALSE;
    }
    ...
    for (MUINT32 i = 0; i < mpEnabledSensorCnt; i++) {
        ...
        // Get pixel rate from sensor
        tgi_pixel_rate[i] = 0;

        mpHalSensor->sendCommand(pAllSensorInfo->mSenInfo[SIdx].mDevId, SENSOR_CMD_GET_PIXEL_RATE,
            (MUINTPTR)&sensorScenId, (MUINTPTR)&(tgi_pixel_rate[i]),
            (MUINTPTR)&(vInPorts.mSensorCfg.at(i).framerate));
        //
    ...
    if (!mpCamIOPipe)
    {
        ...
        CAM_ULOGMD("SenIdx=%d, DynamicRawType=%d, PureRaw=%d", mpSensorIdx[0], this->mDynSwtRawType, this->mPureRaw);

        //determine 2 pixel Mode or not for Raw
        if (!_setTgPixelAndDualMode(sensorScenId, vInPorts.mSensorCfg.at(0).framerate)) { //call this before setting tg information
        ...
        if(CAM_TG_2 == _NORMALPIPE_GET_TG_IDX(mpSensorIdx[0])) //CAM_TG_2
        {
            if (MFALSE == mbTwinEnable)
            {
                ...
                {
                    NormalPipeGUnLock();
                    CAM_ULOGME("Error: ISP-D doesn't enable twin mode, and sensor resultion is bigger than 8M size ");
                    return MFALSE;
                }
            }
        }
        //
        for (UINT32 i = 0; i < mpEnabledSensorCnt;i++)
        {
            //in/out port param
            ...
            if (NSCam::SENSOR_TYPE_YUV == _NORMALPIPE_GET_SENSORTYPE(sensorIdx)) {
                if ((mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1] != ePxlMode_Two_Single) && (mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1] != ePxlMode_One_Single)){
                    CAM_ULOGME("error: YUV sensor not use one/two-pixel mode and one isp, sensorIdx=%d, tg=%d, PixelMode=%lu", sensorIdx, _NORMALPIPE_GET_TG_IDX(sensorIdx), mTwinEnable[_NORMALPIPE_GET_TG_IDX(sensorIdx)][1]);
            ...
            // tgi[i].pixelRate   = (MUINT32)(tgi_pixel_rate[i]/1000000);
            tgi[i].pixelRate   = (MUINT32)ceil((double )tgi_pixel_rate[i]/1000000.0);
                // tgi_pixel_rate[i] = 453200000, tgi[i].pixelRate = 453M
        ...
        this->mpFrmB_Thread->updateFrameTime(0, 0);
        this->mpFrmB_Thread->updateFrameTime(0, 1);
    }
    else {
...
NormalPipe_FrmB::wait(MINT32 sensorIdx, EPipeSignal eType, uint64_t ns_timeout)
{
    ...
    if (this->mpCamIOPipe) {
        if (MFALSE == this->mpCamIOPipe->irq((EPipePass)TG, _irq,(int)ISP_DRV_IRQ_USER_ISPDRV, (MUINT32)ns_timeout))
        {
...
MBOOL
NormalPipe_FrmB::flushIrq(MINT32 sensorIdx, EPipeSignal eType, MINT32 u4TimeoutMs)
{
    PIPE_DBG("flushIrq+(0x%x)\n", eType);
    EPipeIRQ _irq = EPIPEIRQ_VSYNC;
    bool ret = MTRUE;
    MUINT32 TG = _NORMALPIPE_GET_TG_IDX(sensorIdx);
    switch (TG) {
    ...
    if (this->mpCamIOPipe) {
        if (MFALSE == this->mpCamIOPipe->flushIrq(
            (EPipePass)TG, _irq, (int)ISP_DRV_IRQ_USER_ISPDRV, u4TimeoutMs))
        {
            PIPE_ERR("irq(0x%x,0x%x)\n", TG, _irq);
            ret = MFALSE;
        }
...
sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    case EPIPECmd_GET_TG_OUT_SIZE:
        ...
        //arg3 : height
#if 1
        if (NPIPE_Sensor_4 < arg1) {
            PIPE_ERR("EPIPECmd_GET_TG_OUT_SIZE Err: Invalid SensorIdx(%d)!",(MINT32)arg1);
            return MFALSE;
        }
    ...
    case EPIPECmd_GET_HBIN_INFO:
        ...
        if ((NPIPE_Sensor_4 <arg1) || ((MINTPTR)NULL == arg2)) {
            PIPE_ERR("EPIPECmd_GET_HBIN_INFO Err: Invalid Para arg1(%d)!",(MINT32)arg1);
            return MFALSE;
        }
        tg = _NORMALPIPE_GET_TG_IDX(arg1);
        PIPE_DBG("EPIPECmd_GET_HBIN_INFO SIdx(%d), BinInfoAddr(0x%x)",(MINT32)arg1, (MINT32)arg2);
        ...
        switch (tg) {
            ...
            case CAM_TG_2:
                ...
                break;
            default:
                PIPE_ERR("EPIPECmd_GET_HBIN_INFO Err: Invalid SensorIdx(%d)!",(MUINT32)arg1);
                return MFALSE;
        }
        PIPE_DBG("EPIPECmd_GET_HBIN_INFO SIdx(%d),twin=%d,hbin_en=%d w=0x%x, h=0x%x!",(MUINT32)arg1,mbTwinEnable, hbin->mEnabled,hbin->size.w, hbin->size.h);
        break;
       ...
       case EPIPECmd_GET_HBIN1_INFO:
           ...
           //arg2 : NormalPipe_HBIN_Info
           if ((NPIPE_Sensor_4 < arg1) || ((MINTPTR)NULL == arg2)) {
                PIPE_ERR("EPIPECmd_GET_HBIN1_INFO Err: Invalid Para arg1(%d), arg2(0x%x)!",(MUINT32)arg1,(MINT32)arg2);
               return MFALSE;
           }
           tg = _NORMALPIPE_GET_TG_IDX(arg1);
           PIPE_DBG("EPIPECmd_GET_HBIN1_INFO SIdx(%d), BinInfoAddr(0x%x)",(MUINT32)arg1,(MINT32)arg2);

           ret = mpCamIOPipe->sendCommand(cmd, tg, arg2, (MINTPTR)&hbin_en); //readReg:HBIN->*arg3
           if (ret == MTRUE) {
               hbin = (NormalPipe_HBIN_Info*) arg2;
               switch (tg) {
                   case CAM_TG_1:
                   ...
                   case CAM_TG_2:
                       ...
                       break;
                   default:
                       PIPE_ERR("EPIPECmd_GET_HBIN1_INFO Err: Invalid SensorIdx(%d)!", (MUINT32)arg1);
                       return MFALSE;
               }
           }
           PIPE_INF("EPIPECmd_GET_HBIN1_INFO SIdx(%d),*arg3=%d,hbin_en=%d w=0x%x, h=0x%x!",(MUINT32)arg1, hbin_en, hbin->mEnabled,hbin->size.w, hbin->size.h);
           break;
    ...
    case EPIPECmd_GET_RMX_OUT_SIZE:
        ...
        //arg3 : height
        if (2 < arg1) {
            PIPE_ERR("EPIPECmd_GET_RMX_OUT_SIZE Err: Invalid SensorIdx(%d)!", (MUINT32)arg1);
            return MFALSE;
        }
        tg = _NORMALPIPE_GET_TG_IDX(arg1);
        switch (tg) {
            ...
            default:
                ret = MFALSE;
                PIPE_ERR("EPIPECmd_GET_RMX_OUT_SIZE Fail! SIdx=%d, TG=%d",(MUINT32)arg1,tg);
                break;
        }
    ...
    case EPIPECmd_GET_EIS_INFO:
        ...
        if ((MINTPTR)NULL == arg2) {
            PIPE_ERR("EPIPECmd_GET_EIS_INFO Err: Invalid Para arg1(%d)",(MUINT32)arg1);
            return MFALSE;
        }
    ...
    if(ret != MTRUE){
        PIPE_ERR("error: sendCommand fail: (cmd,arg1,arg2,arg3)=(0x%08x,0x%08x,0x%08x,0x%08x)",
                cmd, (MUINT32)arg1, (MINT32)arg2, (MINT32)arg3);
    }
...
enque(IImageBuffer *pframe)
{
    ...
    PIPE_DBG("FrameMgr::enque+, 0x%zx", pframe->getBufPA(0));
...
{
    Mutex::Autolock lock(mFrameMgrLock);
    PIPE_DBG("FrameMgr::deque+ key=0x%x",(MINT32)key);
        ...
        case eCmd_at:
            if(this->m_v_pQueue.size()>arg1)
                *(BufInfo*)arg2 = this->m_v_pQueue.at(arg1);
                ...
                PIPE_INF("size have been reduce to %u(0x%x) by enque\n",
                        (MUINT32)this->m_v_pQueue.size(), (MUINT32)arg1);
                ret = MFALSE;
            }
            break;
        case eCmd_pop_back:
...
{
    int semCount;
    ::sem_getvalue(&this->m_semEnqueThread, &semCount);
    PIPE_INF("E: m_semEnqueThread count = %d", semCount);
...
MBOOL NormalPipe_FrmB_Thread::Stop(void)
{
    ...
    this->m_pNormalpipe_FrmB->flushIrq(
                                this->m_pNormalpipe_FrmB->mpSensorIdx[0],
                                EPipeSignal_SOF,
...
MBOOL NormalPipe_FrmB_Thread::Enque(BufInfo* pBuf)
{
    ...
    if(_buf.mPa == pBuf->mBuffer->getBufPA(0)){
        ...
        PIPE_DBG("EnqueRequest: replace(0x%x) to (0x%x)",
                (MUINT32)pBuf->mBuffer->getBufPA(0), (MUINT32)pBuf->mPa);
#endif
#else
        ...
        PIPE_DBG("EnqueRequest: exchange replace(0x%x) to (0x%x)",
                (MUINT32)pBuf->mBuffer->getBufPA(0), (MUINT32)pBuf->mPa);
#endif
#else
    ...
    if(rQBufInfo.vBufInfo.size() == 1) {
        PIPE_DBG("Enque port:0x%x PA:0x%x,mag:0x%x\n",
            portID.index,(MUINT32)rQBufInfo.vBufInfo[0].u4BufPA[0],pBuf->FrameBased.mMagicNum_tuning);
    } else {
        PIPE_DBG("Enque port:0x%x PA:0x%x,exchange:0x%x, mag:0x%x\n",
            portID.index,
            (MUINT32)rQBufInfo.vBufInfo[0].u4BufPA[0],
            (MUINT32)rQBufInfo.vBufInfo[1].u4BufPA[0],
...
MBOOL NormalPipe_FrmB_Thread::TuningUpdate(MUINT32 magNum)
{
    ...
    if (SENSOR_DEV_SUB == _NORMALPIPE_GET_SENSOR_DEV_ID((this->m_pNormalpipe_FrmB->mpSensorIdx[0]))) {
        ...
        sensorDev = eSoftwareScenario_Main2_N3D_Stream;
    } else if (SENSOR_DEV_SUB_2 == _NORMALPIPE_GET_SENSOR_DEV_ID((this->m_pNormalpipe_FrmB->mpSensorIdx[0]))) {
        sensorDev = eSoftwareScenario_Sub2_Normal_Stream;
    }
    tgidx = _NOMRALPIPE_CVT_TGIDX(this->m_pNormalpipe_FrmB->mpSensorIdx[0]);
    PIPE_DBG("TuningUpdate_%d: mag:0x%x\n",tgidx, magNum);

    ret = this->m_pNormalpipe_FrmB->mpCamIOPipe->sendCommand(EPIPECmd_SET_P1_UPDATE, \
        tgidx, magNum, sensorDev);
    return ret;
}
...
MVOID NormalPipe_FrmB_Thread::DummyFrameToEnque(MUINT32 _OpenedPort)
{
    ...
    if(_size != 0){
        bufferDepth = this->m_pNormalpipe_FrmB->GetRingBufferDepth();
        //enque multi-PortID
        for(MUINT32 i=0;i<_OpenedPort;i++){
        ...
        #if (HAL3_IPBASE == 1)
            _buf.mPa = _buf.mBuffer->getBufPA(0);
            if (this->m_pNormalpipe_FrmB->m_BufIdx >= bufferDepth) {
                if (bufferDepth != 0)
                    this->m_pNormalpipe_FrmB->m_BufIdx = this->m_pNormalpipe_FrmB->m_BufIdx % bufferDepth;
                else
                    PIPE_ERR("bufferDepth error");
            }
        ...
        #if (HAL3_IPBASE == 1)
        if (bufferDepth != 0)
            this->m_pNormalpipe_FrmB->m_BufIdx = (this->m_pNormalpipe_FrmB->m_BufIdx + 1) % bufferDepth;
        else
            PIPE_ERR("bufferDepth error");
        #endif
    }
...
#if (HAL3_IPBASE == 1)
MBOOL NormalPipe_FrmB::ReplaceToDummy(MUINT32 _OpenedIndex, BufInfo* pBuf, BufInfo* dumBuf, MUINT32* _doReplace)
{
    NSCam::NSIoPipeIsp3::PortID port;
    ...
    *_doReplace = 0;
    ...
    if ((((port.index == PORT_IMGO.index) || (port.index == PORT_IMGO_D.index)) && (pBuf->FrameBased.mCropRect.s.h < YsizeofSmallBuf)) ||
        ...
        if (_size != 0) {
            *_doReplace = 1;
            ...
            PIPE_DBG("ReplaceToDummy: replace(0x%zx) to (0x%zx), VA(0x%zx)", pBuf->mBuffer->getBufPA(0), pBuf->mPa,
            ...
            /* MTBF timing issue, the table is clear by stop */
            if (this->m_replaceTable[r_index][arrayIdx] == NULL) {
                PIPE_ERR("ReplaceToDummy err due to stop");
                return MFALSE;
            }
...
MBOOL NormalPipe_FrmB_Thread::updateFrameTime(MUINT32 timeMs, MUINT32 offset) {
    if (offset >= 2) {
    ...
    // TODO: Remove later
    PIPE_DBG("m_ShutterTimeMs[%d] = %d\n", offset, m_ShutterTimeMs[offset]);
    return MTRUE;
}

MUINT32 NormalPipe_FrmB_Thread::estimateFrameTime(void) {
#define SOF_TIMEOUT_TH          (1500)
    ...
    // TODO: Remove later
    PIPE_DBG("m_ShutterTimeMs[0] = %d, m_ShutterTimeMs[1] = %d\n", m_ShutterTimeMs[0], m_ShutterTimeMs[1]);
    PIPE_DBG("_wait_time = %d\n", _wait_time);
    return _wait_time;
}
...
#endif
        if (this->m_pNormalpipe_FrmB->mpCamIOPipe) {
            this->m_pNormalpipe_FrmB->mpCamIOPipe->sendCommand(
                                                        EPIPECmd_SET_FRM_TIME,
                                                        (MINTPTR)(_buf.mShutterTimeNs/1000000),
                                                        0,
                                                        0);
            updateFrameTime((MUINT32)(_buf.mShutterTimeNs/1000000), 0);
        }
...
MVOID NormalPipe_FrmB_Thread::EnqueRequest(MUINT32 sof_idx)
{
    ...
    if(this->m_EnqueCnt++ < (this->m_pNormalpipe_FrmB->GetRingBufferDepth()-1)){ //-1 is because of first tuning is an immediate_request at enque()
        ...
        if((MUINT32)nLost > this->m_EnqueCnt){
    ...
    if(_size == 0){
        ...
        if( (_size % _OpenedPort) !=0 ){
            ...
            _str[0] = '\0';
            for(MUINT32 i=0;i<_OpenedPort;i++){
                sprintf(_pstr,"0x%x",(MUINT32)_ptr[i]);
                while(*_pstr++ == '\0');
            }
    ...
    if(_size == 0){
        ...
        if(this->m_pNormalpipe_FrmB->m_PerFrameCtl){//Full mode means 1 request per frame, if requests r over 2, drop request
            ...
            for(MUINT32 i=0;i<this->m_DropEnQ.size();i++){
                PIPE_INF("drop enque request:0x%x(0x%x)\n",this->m_DropEnQ.at(i),(MUINT32)this->m_DropEnQ.size());
            ...
            if(0 == this->m_DropEnQ.size()){
                ...
                {
                    PIPE_INF("warning: EnqueThread wake up too slow, cur(%d)-buf(%d)>1\n", sof_idx, _buf_sof_idx);
                }
                if(1 == _ReservedReq)
                {
                    #if (HAL3_IPBASE == 1)
        ...
        if(0 == _ReservedReq){
            ...
            if(((NormalPipe_FRM_STATUS)_drop_status) == _drop_frame_status){
                PIPE_INF("warning: frame status is drop frame, use dummy frame\n");
                #if (HAL3_IPBASE == 1)
                this->m_pNormalpipe_FrmB->mDmyContainerLock.lock();
                #endif
                this->DummyFrameToEnque(_OpenedPort);
...
MVOID* NormalPipe_FrmB_Thread::IspEnqueThread(void *arg)
{
    ...
    int semCount;
    ::sem_getvalue(&_this->m_semEnqueThread, &semCount);
    PIPE_INF("After starting : m_semEnqueThread count = %d", semCount);

    PIPE_INF("IspEnqueThread is started and its priority is NICE_CAMERA_P1_ENQUE(PRIORITY_HIGHEST)\n");
    if (pAllSensorInfo == NULL || _this->m_pNormalpipe_FrmB == NULL) {
        PIPE_ERR("error: pAllSensorInfo = %p, _this->m_pNormalpipe_FrmB = %p",
                  pAllSensorInfo, _this->m_pNormalpipe_FrmB);
        PIPE_ERR("error: _this->m_bStart = %d, _this->mCnt = %d", _this->m_bStart, _this->mCnt);
        if (_this->m_pNormalpipe_FrmB != NULL) {
            ::sem_post(&_this->m_pNormalpipe_FrmB->m_semConfigStart);
        }
        goto _exit;
    }
    PIPE_INF("pAllSensorInfo = %p, NormalPipe_FrmB = %p, NormalPipe_FrmB_Thread = %p",
                pAllSensorInfo, _this->m_pNormalpipe_FrmB, _this);
    ...
    while (1)
    {
        ...
        _this->mLock.unlock();
        //
        ret = _this->m_pNormalpipe_FrmB->wait(
                                    _this->m_pNormalpipe_FrmB->mpSensorIdx[0],
                                    EPipeSignal_SOF,
                                    _this->estimateFrameTime());
        //
        ...
        _this->mLock.unlock();
        //  Long exposure may cause SOF come later.
        if (MFALSE == ret) {
        ...
        PIPE_DBG("TG_%d: cur SOF = 0x%x\n",_NORMALPIPE_GET_TG_IDX(_this->m_pNormalpipe_FrmB->mpSensorIdx[0]), (MUINT32)_sof_idx);
        //
        #if (HAL3_IPBASE == 1)
        _this->m_pNormalpipe_FrmB->mIsCfgUpdating = MTRUE;
        #endif
        ...
        if (_sof_idx != _sof_idx2) {
            PIPE_INF("Warning: TG(0x%x): EnqueRequest over vsync(0x%x_0x%x)\n",
                            _NORMALPIPE_GET_TG_IDX((_this->m_pNormalpipe_FrmB->mpSensorIdx[0])),
                            (MUINT32)_sof_idx,
                            (MUINT32)_sof_idx2);
        }
        #if (HAL3_IPBASE == 1)
        _this->m_pNormalpipe_FrmB->mIsCfgUpdating = MFALSE;
        _this->m_pNormalpipe_FrmB->mEnqueCond.broadcast();
        #endif
    }
_exit:
    ::sem_post(&_this->m_semEnqueThread);
    ::sem_getvalue(&_this->m_semEnqueThread, &semCount);
    PIPE_INF("After stopping: m_semEnqueThread count = %d", semCount);
    //
