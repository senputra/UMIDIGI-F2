...
IspDrv* UniDrvImp::createInstance(UNI_HW_MODULE module,IspDrvImp* pConnectedObj)
{
    if(pConnectedObj == NULL){
        LOG_ERR("connected cam can't be NULL\n");
        return NULL;
    }
    gUniDrvObj[module].pIspdrvimp = pConnectedObj;
...
MBOOL UniDrvImp::init(const char* userName)
{
    ...
    if(this->m_HWmodule == UNI_A) {
        if(this->pIspdrvimp->init("UNI_A_DRV") == MFALSE){
        Result = MFALSE;
        goto EXIT;
    }
    } else if (this->m_HWmodule == UNI_B) {
        if(this->pIspdrvimp->init("UNI_B_DRV") == MFALSE){
            Result = MFALSE;
            goto EXIT;
        }
    }
...
MBOOL UniDrvImp::uninit(const char* userName)
{
    ...
    //
    if(this->m_HWmodule == UNI_A) {
        if(this->pIspdrvimp->uninit("UNI_A_DRV") == MFALSE){
        Result = MFALSE;
        goto EXIT;
    }
    } else if (this->m_HWmodule == UNI_B) {
        if(this->pIspdrvimp->uninit("UNI_B_DRV") == MFALSE){
        Result = MFALSE;
        goto EXIT;
    }
    }
...
{
    MINT32 ret=0;
    LOG_DBG("Isp_write:0x%x_0x%x_0x%x",this->m_HWmodule,Addr,Data);
    android::Mutex::Autolock lock(this->IspRegMutex);
    (void)caller;
    if(FD_CHK() == -1){
        return MFALSE;
    ...
    LOG_DBG("Isp_write_0x%x:0x%x_0x%x(0x%x)",this->m_HWmodule,Addr,Data,this->readReg(Addr));
    return MTRUE;
}
    ...
    for(MUINT32 i=0x0;i<UNI_BASE_RANGE;i+=0x20){
        ...
        for(MUINT32 j=i;j<(i+0x20);j+=0x4){
            if(j >= UNI_BASE_RANGE)
                break;
            _tmpchr[0] = '\0';
            sprintf(_tmpchr,"0x%08x - ",this->readReg(j));
            strncat(_chr,_tmpchr,sizeof(_tmpchr));
        }
        LOG_WRN("%s\n",_chr);
    }
    LOG_ERR("###################\n");
