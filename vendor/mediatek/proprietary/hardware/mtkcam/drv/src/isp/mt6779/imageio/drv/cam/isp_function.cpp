...
Mutex   g_EnDbgCfgMutex;
MBOOL   g_EnDbgCfg = MFALSE;
...
IspFunction_B::IspFunction_B()
{
    ...
    g_EnDbgCfgMutex.lock();
    if(g_EnDbgCfg == MFALSE){
        g_EnDbgCfg = MTRUE;
        DBG_LOG_CONFIG(imageio, function);
    }
    g_EnDbgCfgMutex.unlock();
}
...
MINT32 IspFunction_B::write2CQ(MUINT32 burstIndex)
{
    MINT32 retval;
    retval = this->_write2CQ(burstIndex);       //virtual
    if( retval != 0 ){
