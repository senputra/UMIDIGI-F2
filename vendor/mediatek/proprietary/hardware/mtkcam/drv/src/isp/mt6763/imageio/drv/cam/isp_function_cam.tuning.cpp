        ...
        if (pmx_sel == 0) {
            if(CAM_READ_BITS(this->m_pDrv,CAM_CTL_DMA_EN,PDO_EN)){
                ...
                if (pReg->CAM_BNR_PDC_CON.Bits.PDC_EN == MTRUE) {
                    if(0 == (pReg->CAM_BNR_BPC_CON.Bits.BPC_LUT_EN * (vpTuningNodes[page]->ctlEnDMA_CAM & BPCI_EN_))){
                        CAM_FUNC_ERR("bpci & lut_en must be enabled when bnr_en is enabled\n");
                        goto BYPASS_BNR;
                    }

                    if (pReg->CAM_BNR_PDC_CON.Bits.PDC_OUT != 0) {
                        CAM_FUNC_ERR("PDC_OUT must be disabled when PDO_EN is disabled\n");
                        ret = 1;
                        goto BYPASS_BNR;
                    }
...
BYPASS_BNR:
    ...
    if(vpTuningNodes[page]->ctlEn_CAM & PBN_EN_){
        ...
        if ((CAM_READ_REG(this->m_pDrv,CAM_CTL_SEL) & PMX_SEL_1 ) == 0){
