...
public:
    ...
    MINT32  write2CQ( MUINT32 burstIndex );
...
public:
    ...
    #define BW_a(port,en,regname,regname_1) ({\
        ...
        if(en){\
            MUINT32 div = ((prof.frameT - prof.frameblkT)* 100);\
            fifo.Avg_ThruPut = fifo.ThruPut = ( ((CAM_READ_REG(this->m_pDrv,regname)+1) * (CAM_READ_REG(this->m_pDrv,regname_1)+1) * 95) + (div - 1))/ div;\
            CAM_FUNC_DBG("ysize:%d\n",CAM_READ_REG(this->m_pDrv,regname_1));\
    ...
    #define BW_1D(port,en,regname,data_1D) ({\
        DMA_BW fifo;\
        if(en){\
            MUINT32 div = ((prof.frameT - prof.frameblkT)* 100);\
            fifo.ThruPut = ( (((CAM_READ_REG(this->m_pDrv,regname)+1)-data_1D) * 95) + (div - 1)) / div;\
            fifo.Avg_ThruPut = (CAM_READ_REG(this->m_pDrv,regname)+1) * 95 / prof.frameT / 100;\
            CAM_FUNC_DBG("BW_1D-port_%d:xsize:%d,1D:%d,resolut:%d,%d,(%d_%d)\n",port,(CAM_READ_REG(this->m_pDrv,regname)+1),data_1D,fifo.ThruPut,fifo.Avg_ThruPut,prof.frameT,prof.frameblkT);\
    ...
    #define BW_f(port,en,regname,regname_1) ({\
        DMA_BW fifo;\
        if(en){\
            MUINT32 div = (prof.frameT * 100);\
            fifo.ThruPut = ( ((CAM_READ_REG(this->m_pDrv,regname)+1) * (CAM_READ_REG(this->m_pDrv,regname_1)+1) * 95) + (div - 1))/ div;\
            CAM_FUNC_DBG("ysize:%d\n",CAM_READ_REG(this->m_pDrv,regname_1));\
            CAM_FUNC_DBG("BW_f-port_%d:xsize:%d,resolut:%d,(%d)\n",port,CAM_READ_REG(this->m_pDrv,regname),fifo.ThruPut,prof.frameT);\
        }\
        fifo.portID = port;\
        fifo;})
...
public:
    ...
    MUINT32         bufSecu;
    MUINT32         bufCohe;
    MUINT32         magicIdx;   //this term is used at deque when suspending
};
...
typedef struct _stISP_BUF_INFO
{
    ...
    _stISP_BUF_INFO(
        ...
        MUINT32             _sensorId = 5  // default: use max sensor number, sensor id must be less than 5
    )
    : bufidx(_index)
    , bReplace(_bReplace)
    , sensorId(_sensorId)
