...
#define _SPIPE_MAP_TG_IDX(sensorIdx)({\
    MUINT32 tgidx = 0;\
    switch(CamsvStatisticPipe::pAllSensorInfo->mSenInfo[sensorIdx].mDInfo.TgInfo) {\
...
#define _SPIPE_MAP_TG_PXLMODE(sensorIdx, featureIdx) ({\
    NSImageio::NSIspio::EPxlMode tgPxlMode = ePxlMode_One_;\
    MUINT32 tgPxlModeSeninf = 0;\
    switch (featureIdx) {\
    case EPIPE_CAMSV_FEATURE_PDAF: tgPxlModeSeninf = _SPIPE_GET_PDAF_PIX_MODE(sensorIdx); break;\
    case EPIPE_CAMSV_FEATURE_MVHDR: tgPxlModeSeninf = _SPIPE_GET_HDR_PIX_MODE(sensorIdx); break;\
    default: PIPE_ERR("Unknown featureIdx during tg pixel mode mapping"); break;\
    }\
...
ICamsvStatisticPipe* ICamsvStatisticPipe::createInstance(MUINT32 SensorIndex, char const* szCallerName, MUINT32 FeatureIdx)
{
    ...
    if (CamsvStatisticPipe::pAllSensorInfo == NULL) {
        ...
        PIPE_INF("N:%d,%s,pAllSensorInfo=0x%8x", SensorIndex, szCallerName, CamsvStatisticPipe::pAllSensorInfo);
...
EXIT:
    ...
    if (mTotalUserCnt == 0)  {
        ...
        PIPE_INF("Destroy SPipe[%d][%d]=0x%8x, %s", idx, featIdx, pCamsvStatisticPipe[idx][featIdx], szCallerName);
...
MBOOL CamsvStatisticPipe::stop(MBOOL bNonblocking)
{
    ...
    if (!m_bStarted) {
        PIPE_ERR("TG_Camsv_%d not start yet", mTgInfo);
        return MFALSE;
    }
...
MBOOL CamsvStatisticPipe::enque(QBufInfo const& rQBuf)
{
    ...
    for(MUINT32 i=0;i<(this->m_subSample+1);i++){
        ...
        rQBufInfo.vBufInfo[i].u4BufSize[0]           = (MUINT32)this->mpbuf_camsv_imgo[this->ring_cnt*(this->m_subSample+1) + i].size;
        rQBufInfo.vBufInfo[i].u4BufVA[0]             = (MUINTPTR)this->mpbuf_camsv_imgo[this->ring_cnt*(this->m_subSample+1) + i].virtAddr;
        rQBufInfo.vBufInfo[i].u4BufPA[0]             = (MUINTPTR)this->mpbuf_camsv_imgo[this->ring_cnt*(this->m_subSample+1) + i].phyAddr;
        rQBufInfo.vBufInfo[i].Frame_Header.u4BufSize = (MUINT32)this->mpbuf_camsv_imgo_fh[this->ring_cnt*(this->m_subSample+1) + i].size;
        rQBufInfo.vBufInfo[i].Frame_Header.u4BufVA   = (MUINTPTR)this->mpbuf_camsv_imgo_fh[this->ring_cnt*(this->m_subSample+1) + i].virtAddr;
        rQBufInfo.vBufInfo[i].Frame_Header.u4BufPA   = (MUINTPTR)this->mpbuf_camsv_imgo_fh[this->ring_cnt*(this->m_subSample+1) + i].phyAddr;
        ...
        PIPE_DBG("enque_imgo_%d: size:0x%08x_0x%08x, pa:0x%08x_0x%08x, magic:0x%x, crop:%d_%d_%d_%d, ring_cnt:%d",\
            i,\
            rQBufInfo.vBufInfo.at(i).u4BufSize[0],\
            rQBufInfo.vBufInfo.at(i).Frame_Header.u4BufSize,\
            rQBufInfo.vBufInfo.at(i).u4BufPA[0],\
            rQBufInfo.vBufInfo.at(i).Frame_Header.u4BufPA,\
...
MBOOL CamsvStatisticPipe::deque(PortID port, QBufInfo& rQBuf, ESTT_CACHE_SYNC_POLICY cacheSyncPolicy, MUINT32 u4TimeoutMs)
{
...

    {
        ...
        ret = mpCamsvIOPipe->dequeOutBuf( portID, rQTBufInfo);
        ...
        if (rQTBufInfo.vBufInfo.size() != mBurstQNum) {
            PIPE_INF("WARNNING: TG_Camsv_%d port_%d deque buf num unsync, %d/%d", mTgInfoCamsv,\
        ...
        for(MUINT32 idx=0; idx<rQTBufInfo.vBufInfo.size(); idx++){
            ...
            PIPE_DBG("deque_imgo_%d: size:0x%08x_0x%08x, pa:0x%08x_0x%08x, magic:0x%x, time:%d_%d, Tg_Camsv size:%dx%d,crop:%d_%d_%d_%d",\
                idx,\
                rQTBufInfo.vBufInfo.at(idx).u4BufSize[0],\
                rQTBufInfo.vBufInfo.at(idx).Frame_Header.u4BufSize,\
                rQTBufInfo.vBufInfo.at(idx).u4BufPA[0],\
            ...
            result.mTimeStamp       = getTimeStamp_ns(rQTBufInfo.vBufInfo.at(idx).i4TimeStamp_sec,
                                                      rQTBufInfo.vBufInfo.at(idx).i4TimeStamp_us);

            pbuf->mPortID   = portID.index;
            pbuf->mBuffer   = NULL;
            pbuf->mMetaData = result;
            pbuf->mSize     = rQTBufInfo.vBufInfo.at(idx).u4BufSize[0];
            pbuf->mVa       = rQTBufInfo.vBufInfo.at(idx).u4BufVA[0];
            pbuf->mPa       = rQTBufInfo.vBufInfo.at(idx).u4BufPA[0];
                ...
                if(pbuf->mPa == mpbuf_camsv_imgo[b].phyAddr) {
                    ...
                    switch(cacheSyncPolicy){
                        case ESTT_CacheBypass:
                            PIPE_WRN("Camsv:%d: DMA(x%x) PA:0x%x bypass cache sync.", mpSensorIdx , portID.index, pbuf->mPa);
                            break;
                        case ESTT_CacheInvalidAll:
                            //mpIMem->cacheSyncbyRange(IMEM_CACHECTRL_ENUM_INVALID_ALL, &(mpbuf_camsv_imgo[b]));
                            PIPE_WRN("Camsv:%d: DMA(x%x) PA:0x%p no support ESTT_CacheInvalidAll", mpSensorIdx, portID.index, (void*)pbuf->mPa);
                            break;
            ...
            if (b == CAMSV_BUF_DEPTH*(this->m_subSample+1) )
                PIPE_ERR("PA(0x%x) not found to sync cache", pbuf);
...
MBOOL CamsvStatisticPipe::configPipe(QInitStatParam const& vInPorts, MINT32 burstQnum)
{
    ...
    MUINT32     i = 0, j = 0, _portIdx = 0, _pIMG_W = 0, bIMG_W = 0, imgH = 0, dma_size = 0;
    MUINT32     SIdx, sensorDevIdx;
    MUINT32     data1 = 0;
    MUINTPTR    imgHdrAllocOfst = 0;
    NSImageio::NSIspio::ICamIOPipe::E_INPUT eTgInput;
    NSImageio::NSIspio::ICamIOPipe::E_INPUT eTgInputCamsv;
    ...
    if (_SPIPE_GET_TG_IDX(SIdx) <= 0){
        PIPE_ERR("ERROR: mpDynamicInfo.TgInfo <= 0,SIdx=%d,mDevId(0x%x),tg_idx(%d)", \
                  SIdx, pAllSensorInfo->mSenInfo[SIdx].mDevId, _SPIPE_GET_TG_IDX(SIdx));
        mErrorCode = ECAMSVSPipeErrorCode_QUERY_SENSOR_DYINFO_FAIL;
        return MFALSE;
    }
    ...
    do {
        ...
        switch(mFeatureIdx)
        {
            case EPIPE_CAMSV_FEATURE_MVHDR:
                if( _SPIPE_GET_HDR_TG_INFO(mpSensorIdx) == CAM_TG_NONE ) {
                    ...
                    mTgInfoCamsv = _SPIPE_GET_HDR_TG_INFO(mpSensorIdx); /* typically is CAM_SV_1 */
                    // HDR use VC1
            ...
            case EPIPE_CAMSV_FEATURE_PDAF:
                if( _SPIPE_GET_PDAF_TG_INFO(mpSensorIdx) == CAM_TG_NONE ) {
                    ...
                    mTgInfoCamsv = _SPIPE_GET_PDAF_TG_INFO(mpSensorIdx); /* typically is CAM_SV_2 */
                    // PDAF use VC2
        ...
        if(VcInfo.VC2_DataType == 0x2b || VcInfo.VC1_DataType == 0x2b) {
        ...
        switch (mTgInfoCamsv) {
            case CAM_SV_1: // 0x10
                eTgInputCamsv = NSImageio::NSIspio::ICamIOPipe::TG_CAMSV_0; // 10
                break;
            case CAM_SV_2: // 0x11
                eTgInputCamsv = NSImageio::NSIspio::ICamIOPipe::TG_CAMSV_1; // 11
                break;
            case CAM_SV_3: // 0x12
                eTgInputCamsv = NSImageio::NSIspio::ICamIOPipe::TG_CAMSV_2; // 12
                break;
            case CAM_SV_4: // 0x13
                eTgInputCamsv = NSImageio::NSIspio::ICamIOPipe::TG_CAMSV_3; // 13
                break;
            case CAM_SV_5: // 0x14
                eTgInputCamsv = NSImageio::NSIspio::ICamIOPipe::TG_CAMSV_4; // 14
                break;
            case CAM_SV_6: // 0x15
                eTgInputCamsv = NSImageio::NSIspio::ICamIOPipe::TG_CAMSV_5; // 15
                break;
            default:
        ...
        // alocate memory
        stride = bIMG_W;
        if( tgi_eImgFmt[mpSensorIdx] == NSCam::eImgFmt_BAYER10 ) {
            MUINT32 pxl = _SPIPE_MAP_TG_PXLMODE(mpSensorIdx, mFeatureIdx);
            switch(_SPIPE_MAP_TG_PXLMODE(mpSensorIdx, mFeatureIdx)){
                      case NSImageio::NSIspio::ePxlMode_One_:
                       pad = 2 - stride %2;
                       break;
                      case NSImageio::NSIspio::ePxlMode_Two_:
                       pad = 4 - stride %4;
                       break;
            ...
            PIPE_DBG("pixel mode(%d), pad_byte(%d), w_byte(%d)", pxl, pad, stride);
        }
        else if( tgi_eImgFmt[mpSensorIdx] == NSCam::eImgFmt_BAYER8 ) {
            pad = 0;
        }
        stride  += pad;
        ...
        for(MUINT32 i=0;i<(CAMSV_BUF_DEPTH*(m_subSample+1));i++) {
            ...
            mpbuf_camsv_imgo[i].size = mCamsvImgoBufSize;
            ...
            mpbuf_camsv_imgo_fh[i].virtAddr = ((mpbuf_camsv_imgo_fh[i].virtAddr + 15) / 16 * 16);
            mpbuf_camsv_imgo_fh[i].phyAddr = ((mpbuf_camsv_imgo_fh[i].phyAddr + 15) / 16 * 16);
        }
        ...
        switch(eTgInputCamsv) {
            case ICamIOPipe::TG_CAMSV_0: // 10 ( 0x0A )
                _in_port->index = EPortIndex_CAMSV_0_TGI;
                break;
            case ICamIOPipe::TG_CAMSV_1: // 11 ( 0x0B )
                _in_port->index = EPortIndex_CAMSV_1_TGI;
                break;
            case ICamIOPipe::TG_CAMSV_2: // 12 ( 0x0C )
                _in_port->index = EPortIndex_CAMSV_2_TGI;
                break;
            case ICamIOPipe::TG_CAMSV_3: // 13 ( 0x0D )
                _in_port->index = EPortIndex_CAMSV_3_TGI;
                break;
            case ICamIOPipe::TG_CAMSV_4: // 14 ( 0x0D )
                _in_port->index = EPortIndex_CAMSV_4_TGI;
                break;
            case ICamIOPipe::TG_CAMSV_5: // 15 ( 0x0F )
                _in_port->index = EPortIndex_CAMSV_5_TGI;
                break;
        }

        //_port->index      = _SPIPE_MAP_TG_IDX(mpSensorIdx);
        _in_port->ePxlMode     = _SPIPE_MAP_TG_PXLMODE(mpSensorIdx, mFeatureIdx);
        ...
        /*if(this->bypass_imgo == MFALSE)*/ /* _CAM_SV_ : UT : FIX ME */
        {
            ...
            _out_port->u4Stride[ESTRIDE_1ST_PLANE] = mCamsvImgoBufSize / _out_port->crop1.h;

            //prepare for enque if needed
            camsv_imgo_crop_w = _pIMG_W;
            camsv_imgo_crop_h = imgH;

            E_ISP_PIXMODE e_QueryPixMode;

            switch( _out_port->ePxlMode ) {
                case ePxlMode_One_:
                    e_QueryPixMode = ISP_QUERY_1_PIX_MODE;
                    break;
                case ePxlMode_Two_:
                    e_QueryPixMode = ISP_QUERY_2_PIX_MODE;
                    break;
                case ePxlMode_Four_:
                    e_QueryPixMode = ISP_QUERY_4_PIX_MODE;
                    break;
            }

            _out_port->u4Stride[ESTRIDE_2ND_PLANE] = 0;//vInPorts.mPortInfo.at(i).mStride[1];
            _out_port->u4Stride[ESTRIDE_3RD_PLANE] = 0;//vInPorts.mPortInfo.at(i).mStride[2];
        ...
        // enque
        {
            ...
            if( MFALSE == this->enque(_eq_rQBufInfo)){
                ...
                //mpCamsvIOPipe->uninit();
                //mpCamsvIOPipe->destroyInstance();
                mErrorCode = ECAMSVSPipeErrorCode_CAMSAV_ENQUE_FAIL;
                //return MFALSE;
                goto EXIT;
            }
        }
        ...
        if( MFALSE == mpCamsvIOPipe->start() ){
            printf("##############################\n");
            printf("error:start fail\n");
            printf("##############################\n");
            //mpCamsvIOPipe->uninit();
            //mpCamsvIOPipe->destroyInstance();
            mErrorCode = ECAMSVSPipeErrorCode_CAMSAV_START_FAIL;
            //return MFALSE;
            goto EXIT;
        }
        ...
        // deque
        {
            ...
            _dq_portID.index = EPortIndex_IMGO;
            if( MFALSE == this->deque(_dq_portID, _dq_rQBufInfo) ){
                printf("##############################\n");
                ...
                //mpCamsvIOPipe->uninit();
                //mpCamsvIOPipe->destroyInstance();
                mErrorCode = ECAMSVSPipeErrorCode_CAMSAV_DEQUE_FAIL;
                //return MFALSE;
                goto EXIT_STOP;
            }
...
MBOOL CamsvStatisticPipe::sendCommand(MINT32 cmd, MINTPTR arg1, MINTPTR arg2, MINTPTR arg3)
{
    ...
    if (MFALSE == mConfigDone && cmd != ECAMSVSPipeCmd_SET_CAMSV_SENARIO_ID) {
    ...
    switch (cmd) {
        ...
        case ECAMSVSPipeCmd_GET_CAMSV_IMGO_SIZE:
            ...
            break;
        case ECAMSVSPipeCmd_SET_CAMSV_SENARIO_ID:
            if (arg1) {
...
*****************************************************/
template <class _T>
void CamsvStatisticPipe::QueueMgr<_T>::init(void)
{
    this->m_v_pQueue.clear();
}

template <class _T>
void CamsvStatisticPipe::QueueMgr<_T>::uninit(void)
{
    this->m_v_pQueue.clear();
}

template <class _T>
MBOOL CamsvStatisticPipe::QueueMgr<_T>::sendCmd(QueueMgr_cmd cmd, MUINTPTR arg1, MUINTPTR arg2)
{
    MBOOL ret = MTRUE;
