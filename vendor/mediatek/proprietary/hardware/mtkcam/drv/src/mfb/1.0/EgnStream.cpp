...
init()
{
    ...
    LOG_INF("MFB EgnStream Init: mEgnInitCnt(%d)",mInitCount);
...
uninit()
{
    ...
    LOG_INF("MFB EgnStream Uninit: mEgnUnitCnt(%d)",mInitCount);
    ...
    {
        LOG_INF("MFB EgnStream: mEgnUnitCnt(%d) doesn't come to 0, skip uninit",mInitCount);
    }
...
#endif
#ifdef FACTORY
template IEgnStream<MFBConfig>* NSCam::NSIoPipe::NSEgn::getEgnStreamImp(char const* szCallerName);
#else
template IEgnStream<MFBConfig>* IEgnStream<MFBConfig>::createInstance(char const* szCallerName);
#endif
