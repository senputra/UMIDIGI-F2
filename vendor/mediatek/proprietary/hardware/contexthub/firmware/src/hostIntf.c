...
static bool initSensors()
{
    ...
    if (mTotalBlocks > MAX_NUM_BLOCKS) {
        osLog(LOG_INFO, "initSensors: alloc blocks number:%" PRIu32 "\n", mTotalBlocks);
...
static void hostIntfAddBlock(struct HostIntfDataBuffer *data)
{
     ...
     */
    hostIntfSetInterruptType(data->sensType, data->interrupt);
...
static void queueFlush(struct ActiveSensor *sensor)
{
    ...
    mWakeupBlocks = 0;
    mNonWakeupBlocks = 0;
    hostIntfSetInterruptType(sensor->buffer.sensType, sensor->interrupt);
...
static void hostIntfHandleEvent(uint32_t evtType, const void* evtData)
{
    ...
    if (evtType == EVT_APP_START) {
        ...
        if (hostMsg->dataLen <= HOST_HUB_RAW_PACKET_MAX_LEN) {
            ...
            /* cali data don't wakeup ap */
            data->interrupt = NANOHUB_INT_NONWAKEUP;
        ...
        for (i = 0, cnt = 0; i < mNumSensors && cnt < mLatencyCnt; i++) {
            if (mActiveSensorTable[i].latency > 0) {
                ...
                if (mActiveSensorTable[i].firstTime && rtcTime >= mActiveSensorTable[i].firstTime + mActiveSensorTable[i].latency) {
                    ...
                    simpleQueueEnqueue(mOutputQ, &mActiveSensorTable[i].buffer,
                        sizeof(uint32_t) + mActiveSensorTable[i].buffer.length, mActiveSensorTable[i].discard);
                    resetBuffer(&mActiveSensorTable[i]);
                    /* curSamples must reinitialize to 0 when batch timeout, curSamples = 0 will
                     * update firstTime to a new loop batch.
                     * mWakeupBlocks and mNonWakeupBlocks should reinitialize to 0.
                     * firstTime must reinitialize to 0, and this if condition will false.
                     * for example:
                     ...
                     * sensor->firstTime = 0, next latency timer if will false with any data updated.
                     */
                    mWakeupBlocks = 0;
                    mNonWakeupBlocks = 0;
                    mActiveSensorTable[i].curSamples = 0;
                    mActiveSensorTable[i].firstTime = 0;
                    hostIntfSetInterruptType(mActiveSensorTable[i].buffer.sensType, mActiveSensorTable[i].interrupt);
...
#endif
        osLog(LOG_INFO, "hostintf: %lld, chreType:%u, rate:%lu, latency:%lld, cmd:%d!\n",
        ...
        if (cmd->sensType > SENS_TYPE_INVALID && cmd->sensType <= SENS_TYPE_LAST_USER && mSensorList[cmd->sensType - 1] < MAX_REGISTERED_SENSORS) {
            ...
            if (sensor->sensorHandle) {
                if (cmd->cmd == CONFIG_CMD_FLUSH) {
                    ...
                    sensorSelfTest(sensor->sensorHandle);
                } else if (cmd->cmd == CONFIG_CMD_CFG_DATA) {
                    sensorCfgData(sensor->sensorHandle, (void *)(cmd+1));
                }
            } else if (cmd->cmd == CONFIG_CMD_ENABLE) {
        ...
        if (sensor->sensorHandle) {
            ...
            if (sensor->firstTime &&
                ...
                /* mtk add, we should at here to push data to dram, but don't wakeup ap */
                simpleQueueEnqueue(mOutputQ, &sensor->buffer, sizeof(uint32_t) + sensor->buffer.length, sensor->discard);
                resetBuffer(sensor);
                ...
                mWakeupBlocks = 0;
                mNonWakeupBlocks = 0;
                /* don't wakeup ap, if do, may lead wake up ap frequency */
                /*if (!sensor->oneshot) {
                    hostIntfSetInterruptType(sensor->buffer.sensType, sensor->interrupt);
                    hostIntfSetInterrupt(sensor->interrupt);
                    mark_timestamp((evtType & 0xFF), (evtType & 0xFF), HOSTINTF, rtcGetTime());
...
static uint8_t InterruptSensType = SENS_TYPE_INVALID;
void hostIntfSetInterruptType(uint8_t sensType, uint32_t bit)
{
    if (bit == NANOHUB_INT_BOOT_COMPLETE ||
            bit == NANOHUB_INT_BOOT_COMPLETE ||
                bit == NANOHUB_INT_CMD_WAIT)
        return;
    if (mHostIntfTid) {
        if (!atomicBitsetGetBit(mInterruptMask, bit))
            InterruptSensType = sensType;
        else
            InterruptSensType = SENS_TYPE_INVALID;
    }
}

uint8_t hostIntfGetInterruptType(void)
{
    return InterruptSensType;
}
...
void hostIntfSetInterruptMask(uint32_t bit)
{
    ...
    osLog(LOG_ERROR, "hostIntfSetInterruptMask: %lu\n", bit);
    if (mHostIntfTid) {
        if (!atomicBitsetGetBit(mInterruptMask, bit)) {
...
void hostIntfClearInterruptMask(uint32_t bit)
{
    ...
    osLog(LOG_ERROR, "hostIntfClearInterruptMask: %lu\n", bit);
    if (mHostIntfTid) {
        if (atomicBitsetGetBit(mInterruptMask, bit)) {
