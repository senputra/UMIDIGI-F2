...
static inline int i2cMasterTxDelay(uint32_t busId, uint32_t addr, const void *txBuf,
    size_t txSize,I2cCallbackF callback, void *cookie, uint32_t delay)
{
    return i2cMasterTxRxDelay(busId, addr, txBuf, txSize, NULL, 0,
        callback, cookie, delay);
}
static inline int i2cMasterRxDelay(uint32_t busId, uint32_t addr, void *rxBuf,
    size_t rxSize, I2cCallbackF callback, void *cookie, uint32_t delay)
{
    return i2cMasterTxRxDelay(busId, addr, NULL, 0, rxBuf, rxSize,
        callback, cookie, delay);
}
