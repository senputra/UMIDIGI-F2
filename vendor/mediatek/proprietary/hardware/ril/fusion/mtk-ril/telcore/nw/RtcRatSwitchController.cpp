...
int RtcRatSwitchController::getDefaultNetWorkType(int slotId) {
    RFX_UNUSED(slotId);
    int type = PREF_NET_TYPE_GSM_WCDMA;
    ...
    logD(RAT_CTRL_TAG, "getDefaultNetWorkType, C2k=%d, LteFdd=%d, LteTdd=%d, Nr=%d, return=%d",
            RatConfig_isC2kSupported(), RatConfig_isLteFddSupported(),
            RatConfig_isLteTddSupported(), RatConfig_isNrSupported(), type);
    return type;
}
