 ...
 *****************************************************************************/
RtcGsmSmsController::RtcGsmSmsController() :m_timer(NULL) {
...
void RtcGsmSmsController::onTimer() {
...
bool RtcGsmSmsController::onHandleUrc(const sp<RfxMessage>& msg) {
    ...
    if (msg_id ==  RFX_MSG_URC_RESPONSE_NEW_SMS ||
            msg_id == RFX_MSG_URC_RESPONSE_NEW_SMS_STATUS_REPORT) {
        logD(mTag, "Start MT SMS ack timer");
        m_timer = RfxTimer::start(RfxCallback0(this, &RtcGsmSmsController::onTimer),
                s2ns(MT_GSM_SMS_ACK_TIME));
    }
