...
/**
 * "filedToHide" param:
 *   the data part index. start from 0.
 *   (+XXX: column 0, column 1, ..)
 */
void RmcCallControlBaseHandler::logAt(const char *line, const char *name, int fieldToHide) {
    int fieldsToHide[1] = { fieldToHide };
    logAt(line, name, fieldsToHide, 1);
}

/**
 * "filedsToHide" param:
 *   the data part index array. to pass in multiple field to hide.
 *   filedsToHide[x] saves the field index start from 0.
 *   (+XXX: column 0, column 1, ..)
 */
void RmcCallControlBaseHandler::logAt(const char *line, const char *name, int* fieldsToHide,
        int fieldsSize) {
    char *str = strdup(line);
    if (str == NULL) {
        return;
    }
    RfxAtLine atLine = RfxAtLine(str, NULL);
    int ret = 0;
    atLine.atTokStart(&ret);
    if (ret < 0) {
        free(str);
        return;
    }
    ...
    atLog += name;
    int i = 0;
    while (atLine.atTokHasmore()) {
        char *field = atLine.atTokNextstr(&ret);
        if (ret < 0) {
            break;
        }
        if (i != 0) {
        ...
        for (int j=0; j<fieldsSize; j++) {
            if (i == fieldsToHide[j]) {
                atLog += RfxRilUtils::pii(RFX_LOG_TAG, field);
                break;
            }
            if (j == (fieldsSize - 1)) {
                atLog += field;
    ...
    logD(RFX_LOG_TAG, "%s", atLog.string());
    free(str);
}
