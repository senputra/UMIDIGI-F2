...
int RmcCallControlCommonRequestHandler::callFromCLCCSLine(RfxAtLine *pLine, RIL_Call *p_call) {
    ...
    logAt(pLine->getLine(), "+CLCCS: ", 10 /* fieldToHide: <number> */);
...
int RmcCallControlCommonRequestHandler::callFromCLCCLine(RfxAtLine *pLine, RIL_Call *p_call) {
    ...
    logAt(pLine->getLine(), "+CLCC: ", 5 /* fieldToHide: <number> */);
...
void RmcCallControlCommonRequestHandler::requestFlash(const sp<RfxMclMessage>& msg) {
    ...
    char origString[MAX_CDMA_FLASH_NUMBER_LENGTH + 1] = {0};
    sp<RfxAtResponse> p_response;
    ...
    if (number == NULL || strlen(number) == 0) {
        ...
        if (strlen(origNumber.string()) <= MAX_CDMA_FLASH_NUMBER_LENGTH) {
            ...
            strncpy(origString, origNumber.string(), MAX_CDMA_FLASH_NUMBER_LENGTH);
            origString[MAX_CDMA_FLASH_NUMBER_LENGTH] = '\0';
