...
static int cachedUrcList_MTK[] = {
    ...
    RIL_UNSOL_ME_SMS_STORAGE_FULL,
    RIL_UNSOL_PHB_READY_NOTIFICATION,
    RIL_UNSOL_RESPONSE_REGISTRATION_SUSPENDED,
};

static int cachedUrcList_IMS_AOSP[] = {};

static int cachedUrcList_IMS_MTK[] = {
...
RIL_register (const RIL_RadioFunctions *callbacks) {
    RLOGI("RILPROXY getSimCount: %d", getSimCount());
    ...
    RLOGI("radioConfig called register radio config Service");
...
#endif
{
    ...
    if (soc_id >= RIL_SOCKET_NUM) {
        RLOGE("RIL_onUnsolicitedResponse incorrect socket id %d", soc_id);
        return;
    }
...
static bool cacheUrcByClient(int unsolResponse, const void *data, size_t datalen
        , RIL_SOCKET_ID socket_id, ClientType targetClient){
    bool ret = true;
    ...
    if(bIsCallbackReady[socket_id][targetClient] == true) {
        RLOGD("already connected, no need to cache");
        ret = false;
        goto unlock;
    }
    while (urcCur != NULL) {
        RLOGD("Pended URC:%d, RILD:%s, :%s, client %d",
    ...
    RLOGD("New pended URC:%d, RILD:%s, :%s, client %d",
...
bool cacheUrc(int unsolResponse, const void *data, size_t datalen, RIL_SOCKET_ID socket_id){
    ...
    if(socket_id >= RIL_SOCKET_NUM) {
        RLOGD("incorrect input %d", socket_id);
        return false;
    }
    // check if in the aosp list
    for(i = 0; i < sizeof(cachedUrcList_AOSP)/sizeof(int); i++) {
        if(unsolResponse == cachedUrcList_AOSP[i]) {
            ret |= cacheUrcByClient(unsolResponse, data, datalen, socket_id, RADIO_AOSP);
            break;
        }
    }
    // check if in the mtk list
    for(i = 0; i < sizeof(cachedUrcList_MTK)/sizeof(int); i++) {
        if(unsolResponse == cachedUrcList_MTK[i]) {
            ret |= cacheUrcByClient(unsolResponse, data, datalen, socket_id, RADIO_MTK);
            break;
        }
    }
    // check if in the ims aosp list
    for(i = 0; i < sizeof(cachedUrcList_IMS_AOSP)/sizeof(int); i++) {
        if(unsolResponse == cachedUrcList_IMS_AOSP[i]) {
            ret |= cacheUrcByClient(unsolResponse, data, datalen, socket_id, IMS_AOSP);
            break;
        }
    }
    // check if in the ims mtk list
    for(i = 0; i < sizeof(cachedUrcList_IMS_MTK)/sizeof(int); i++) {
        if(unsolResponse == cachedUrcList_IMS_MTK[i]) {
            ret |= cacheUrcByClient(unsolResponse, data, datalen, socket_id, IMS_MTK);
            break;
        }
    }
    if(!ret) {
        RLOGD("No need to cache the request");
    }
...
void sendUrc(RIL_SOCKET_ID socket_id, AtResponseList** urcCached, android::ClientType clientType) {
    ...
    while (urc != NULL) {
        if (bIsCallbackReady[socket_id][clientType] == false) {
            RLOGE("CB is not ready, clientType %d, Socket %d", clientType, socket_id);
            break;
        }
        RLOGI("sendPendedUrcs %s, %s, client %d",
...
void sendPendedUrcs(RIL_SOCKET_ID socket_id, android::ClientType clientType) {
    RLOGD("Ready to send pended URCs, socket:%s, clientType %d",
...
extern "C" int toClientSlot(int slotId, ClientId cliendId) {
    RLOGD("toClientSlot:%d to %d, client %d",
