...
extern "C" RILChannelId RIL_queryCommandChannelId(RIL_Token t);
...
struct RIL_Env RilSapSocket::uimRilEnv = {
        ...
        ,.QueryCommandChannelId = RIL_queryCommandChannelId
...
void RilSapSocket::dispatchRequest(MsgHeader *req) {
    ...
    if (id == RIL_SOCKET_2) {
        currRequest->exec_cid = (RILChannelId)(RIL_CMD_3 + RIL_CHANNEL_OFFSET);
    } else if (id == RIL_SOCKET_3) {
        currRequest->exec_cid = (RILChannelId)(RIL_CMD_3 + RIL_CHANNEL_SET3_OFFSET);
    } else if (id == RIL_SOCKET_4) {
        currRequest->exec_cid = (RILChannelId)(RIL_CMD_3 + RIL_CHANNEL_SET4_OFFSET);
    } else {
        currRequest->exec_cid = (RILChannelId)(RIL_CMD_3);
    }

    pendingResponseQueue.enqueue(currRequest);

    if (uimFuncs) {
        RLOGI("RILD:RilSapSocket::dispatchRequest [%d] > SAP REQUEST type: %d. id: %d. error: %d, \
                token 0x%p, exec_cid: %d", req->token, req->type, req->id, req->error, currRequest,
                currRequest->exec_cid);
