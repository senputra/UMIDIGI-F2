...
/// Keep alive @{
static void dispatchKeepAlive(Parcel &p, RequestInfo *pRI);
static int responseKeepAlive(Parcel &p, void *response, size_t responselen);
/// @}
...
static void dispatchKeepAlive(Parcel &p, RequestInfo *pRI) {
    ...
    request.type = (RIL_KeepaliveType)t;

    // sourceAddress
    tempSource = strdupReadString(p);
    strncpy(request.sourceAddress, tempSource, MAX_INADDR_LEN - 1);
    if (tempSource != NULL) {
        free(tempSource);
    }
    ...
    request.sourcePort = (int)t;

    // destinationAddress
    tempDest = strdupReadString(p);
    strncpy(request.destinationAddress, tempDest, MAX_INADDR_LEN - 1);
    if (tempDest != NULL) {
        free(tempDest);
    }
    ...
    request.maxKeepaliveIntervalMillis = (int)t;
    ...
    startRequest;
    appendPrintBuf("%s%d,%s,%d,%s,%d,%d,%d", printBuf, request.type,
        (char*)request.sourceAddress, request.sourcePort, (char*)request.destinationAddress,
        request.destinationPort, request.maxKeepaliveIntervalMillis, request.cid);
    closeRequest;
    printRequest(pRI->token, pRI->pCI->requestNumber);

    CALL_ONREQUEST(pRI->pCI->requestNumber, &request, sizeof(request), pRI, pRI->socket_id);
...
static int responseKeepAlive(Parcel &p, void *response, size_t responselen) {
    ...
    if (responselen != sizeof (RIL_KeepaliveStatus) ) {
        RLOGE("[%s] invalid response length was %d expected %d", __FUNCTION__,
                (int)responselen, (int)sizeof(RIL_KeepaliveStatus));
        return RIL_ERRNO_INVALID_RESPONSE;
    }

    RIL_KeepaliveStatus *p_cur = (RIL_KeepaliveStatus *) response;
    p.writeInt32(p_cur->sessionHandle);
    p.writeInt32(p_cur->code);

    RLOGD("[%s] sessionHandle: %d, code: %d", __FUNCTION__, p_cur->sessionHandle, p_cur->code);
