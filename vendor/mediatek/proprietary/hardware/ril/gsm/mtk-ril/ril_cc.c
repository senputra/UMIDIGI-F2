    ...
    if (strStartsWith(urc, "+CEN2")) {
        ...
        if (ret >= 0) {
            ...
            if (ptr == NULL) {  // Not found
                snprintf(tmpItem, sizeof(tmpItem) - 1, ";%s,%d", number, category);
                if ((strlen(tmpItem) + strlen(list)) > (PROPERTY_VALUE_MAX - 1)) {
                    LOGD("[%s][Not found] list full(%d), cannot add \"%s\"",
                            __FUNCTION__, strlen(list), tmpItem);
                } else {
                    snprintf(newList, PROPERTY_VALUE_MAX, "%s%s", list, tmpItem);
                ...
                snprintf(tmpItem, sizeof(tmpItem) - 1, ";%s,%d", number, category);
                if ((strlen(tmpItem) + strlen(tmpList)) > (PROPERTY_VALUE_MAX - 1)) {
                    LOGD("[%s][Found] list full(%d), cannot re-add \"%s\"",
                            __FUNCTION__, strlen(tmpList), tmpItem);
                } else {
                    snprintf(newList, PROPERTY_VALUE_MAX, "%s%s", tmpList, tmpItem);
