...
int main(int argc, char **argv) {
    ...
    char prop_mux_start[PROPERTY_VALUE_MAX];
    // check if rild is started by muxd
    property_get("vendor.ril.mux.start", prop_mux_start, "0");
    RLOGD( "RILD is started %s", prop_mux_start);
    if(prop_mux_start[0] == '0') {
        RLOGD( "RILD is not started by muxd, TRM");
        property_set("vendor.ril.mux.report.case","2");
        property_set("vendor.ril.cdma.report.case", "1");
        property_set("vendor.ril.muxreport", "1");
        exit(0);
    }
    property_set("vendor.ril.mux.start", "0");
