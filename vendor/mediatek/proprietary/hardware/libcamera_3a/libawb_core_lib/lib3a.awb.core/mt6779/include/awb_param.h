...
#define AWB_PARAM_REVISION  8284001
...
typedef struct
{
    ...
    MUINT16 u2ErrCnt; // error number
    MUINT8 ucLightSrc; // light source
} AWB_MAIN_STAT_2B_T_;
...
typedef struct
{
    ...
    MUINT16 u2G; // G average of specified AWB window
    MUINT16 u2B; // B average of specified AWB window
    MUINT32 u4RCnt : 9;//R channel  error pixel number
    MUINT32 u4GCnt : 9;//G channel  error pixel number
    MUINT32 u4BCnt : 9;//B channel  error pixel number
    MUINT32 u4Res : 1;
    MUINT32 u4LightSrc : 4;// light source    
} AWB_MAIN_STAT_2B_CUST_T;
...
typedef struct
{
    ...
    MUINT32 u4EL; // error number + light source info
} AWB_MAIN_STAT_4B_T;
...
typedef struct
{
    ...
    // Thresholds
    MINT32 i4LightSrcLowThresholdR;  // Error pixel low threshold of R for light source estimation
    MINT32 i4LightSrcLowThresholdG;  // Error pixel low threshold of G for light source estimation 
    MINT32 i4LightSrcLowThresholdB;  // Error pixel lowthreshold of B for light source estimation
    MINT32 i4LightSrcHighThresholdR; // Error pixel high threshold of R for light source estimation
    MINT32 i4LightSrcHighThresholdG; // Error pixel high threshold of G for light source estimation
    MINT32 i4LightSrcHighThresholdB; // Error pixel high threshold of B for light source estimation
    ...
    // Post-gain values
    MINT32 i4PostGainR; // Post-gain Value for R color (format: 4.9; 1x = 512)
    MINT32 i4PostGainG; // Post-gain Value for G color (format: 4.9; 1x = 512)
    MINT32 i4PostGainB; // Post-gain Value for B color (format: 4.9; 1x = 512)

    // Valid data width
    MINT32 i4ValidDataWidth;

    MINT32 i4LinearOutputEn; // Output format select 1: Linear mode, 0: non-linear mode

    MINT32 i4OutputFormatShift;
...
typedef struct
{
    ...
    MBOOL bAWBLock;
    AWB_GAIN_T rAwbGainPref;   //AWB gain with preference
...
typedef enum
{
    AWB_LANDMARK_X0 = 0, //left
    AWB_LANDMARK_Y0,
    AWB_LANDMARK_X1,    //right
    AWB_LANDMARK_Y1,
    AWB_LANDMARK_UX,    //up
    AWB_LANDMARK_UY,
    AWB_LANDMARK_DX,    //down
    AWB_LANDMARK_DY,
    AWB_LANDMARK_POINT_NUM
} AWB_LANDMARK_POINT_T;
...
typedef struct
{
    ...
    MINT32 i4Motion[2];   // 0: X-axis, 1: Y-axis
    MINT32 i4Landmark[AWB_LANDMARK_INDEX_NUM][AWB_LANDMARK_POINT_NUM];
    MINT32 i4LandmarkCV;  //confidence value of landmark
    MINT32 i4LandmarkRIP; //roll angle
    MINT32 i4LandmarkROP; //Pitch/Yaw angle
...
typedef struct
{
    ...
    MINT32 i4Type; // 0:GFD, 1:LFD, 2:OT
    MINT32 i4Motion[2];
    MINT32 i4LandmarkX0[AWB_LANDMARK_INDEX_NUM];
    MINT32 i4LandmarkY0[AWB_LANDMARK_INDEX_NUM];
    MINT32 i4LandmarkX1[AWB_LANDMARK_INDEX_NUM];
    MINT32 i4LandmarkY1[AWB_LANDMARK_INDEX_NUM];
    MINT32 i4LandmarkCV;
