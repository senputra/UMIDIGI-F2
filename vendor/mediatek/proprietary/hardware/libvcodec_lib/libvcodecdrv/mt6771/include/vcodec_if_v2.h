...
typedef struct {
	union {
		unsigned char           *pu1Buffer_VA;
		UINT64 		        pu1Buffer_VA_dummy;
	};
	union {
		unsigned char           *pu1Buffer_PA;
		UINT64 		        pu1Buffer_PA_dummy;
	};
	unsigned int             eBufferStatus; /* VCODEC_BUFFER_ATTRIBUTE_T */
} VCODEC_BUFFER_X64_T;
...
typedef enum {
    ...
    VCODEC_DEC_QUERY_INFO_TOTAL_OUTPUT_BUFFER,            // 3; Total output buffer
    VCODEC_DEC_QUERY_INFO_DISPLAYABLE_OUTPUT_BUFFER,      // 4; Displayable output buffer
...
typedef struct {
    unsigned int    u4ExtraBufferType;              // must be OUTPUT_BUFFER_EXTRA_VUI
    unsigned int    u4BufferValid;

    unsigned int    u4VUIPresentFlag;               // vui_parameters_present_flag
    unsigned int    u4VideoSignalTypePresentFlag;   // video_signal_type_present_flag
    unsigned int    u4VideoFullRangeFlag;           // video_full_range_flag
    unsigned int    u4ColourDescriptionPresentFlag; // colour_description_present_flag
    unsigned int    u4ColourPrimaries;              // colour_primaries
    unsigned int    u4TransferCharacteristics;      // transfer_characteristics
    unsigned int    u4MatrixCoefficients;           // matrix_coefficients
    unsigned char   profileIdc;                     // profile
    unsigned char   levelIdc;                       // level

    unsigned char   reserved[86];
    void            *prExtra;
} VCODEC_DEC_OUTPUT_BUFFER_EXTRA_VUI_T;
...
typedef struct {
	VCODEC_BUFFER_X64_T rStartAddr;
	VCODEC_BUFFER_X64_T rEndAddr;
	VCODEC_BUFFER_X64_T rWriteAddr;
	VCODEC_BUFFER_X64_T rReadAddr;
	unsigned int    u4BufferLength;
} VCODEC_ENC_BUFFER_INFO_X64_T;
...
typedef enum {
	...
	VCODEC_ENC_PARAM_RESOLUTION_CHANGE,
...
typedef struct {
	...
	union {
		void (*pfnMalloc)(IN HANDLE 							/* hDrv */,
				  ...
				  IN unsigned int						/*u4AlignSize*/,
				  IN VCODEC_MEMORY_TYPE_T					  /* fgCacheable */,
				  OUT VCODEC_BUFFER_X64_T * 				 /* prBuf */
				 );   /*buffer address must cache line align */
		UINT64 		        prCodecHandle_dummy;
	};
	...
	union {
		void (*pfnIntMalloc)(IN HANDLE							   /* hDrv */,
					 IN unsigned int					   /* u4Size */,
					 IN unsigned int					   /*u4AlignSize*/,
					 OUT VCODEC_BUFFER_X64_T *						/* prBuffer_adr */
					);
		UINT64 		        pfnIntMalloc_dummy;
	};


	union {
		void (*pfnFree)(IN HANDLE							  /* hDrv */,
				IN VCODEC_BUFFER_X64_T *				   /* prBuf */
				   );  /* same memory type with malloc */
		UINT64 		        pfnFree_dummy;
	};


	union {
		void (*pfnIntFree)(IN HANDLE							 /* hDrv */,
				   IN VCODEC_BUFFER_X64_T * 				  /* prBuffer_adr */
				  );
		UINT64 		        pfnIntFree_dummy;
	};

	union {
		void (*pfnReleaseYUV)(IN HANDLE 							/* hDrv */,
					  IN VCODEC_BUFFER_X64_T *					 /* prYUVBuf */
					 );
		UINT64 		        pfnReleaseYUV_dummy;
	};

	union {
		void (*pfnPaused)(IN HANDLE 							 /* hDrv */,
				  IN VCODEC_BUFFER_X64_T *					  /* prBitstreamBuf */
				 );
		UINT64 		        pfnPaused_dummy;
	};

	union {
		void (*pfnAllocateBitstreamBuffer)(IN HANDLE							  /* hDrv */,
						   OUT VCODEC_ENC_BUFFER_INFO_X64_T *	   /* prBitstreamBuf */
						  );
		UINT64 		        pfnAllocateBitstreamBuffer_dummy;
	};
	...
	union {
		...
		UINT64 		        pfnUpdateBitstreamWP_dummy;
	};
	union {
		VCODEC_ENC_ERROR_T(*pfnQueryInfo)(IN HANDLE 							 /* hDrv */,
						  IN VCODEC_ENC_QUERY_INFO_TYPE_T		 /* query id*/,
		...
		UINT64				pfnQueryInfo_dummy;
	};

} VCODEC_ENC_CALLBACK_X64_T;
...
typedef struct {
    ...
    VCODEC_ENC_INPUT_FLAG_T      eFlags;
    VCODEC_ENC_INPUT_INFO_T      rInput;
    VCODEC_ENC_BUFFER_INFO_X64_T pBuffInfo;
    VCODEC_COLOR_FORMAT_T        eVEncFormat;
    unsigned int                 u4Width;
    unsigned int                 u4Height;
    unsigned int                 u4YStride;
    ...
    unsigned int                 u4SliceHeight;
    void                         *reserved[4];
} VIDEO_ENCODER_INPUT_PARAM_NC_X64_T;
/* non-callback */
typedef struct {
    UINT64                       u8TimeStamp;
    VCODEC_ENC_INPUT_FLAG_T      eFlags;
    ...
    unsigned int                 u4SliceHeight;
    void                         *reserved[4];
} VIDEO_ENCODER_INPUT_YUV_PARAM_NC_X64_T;
...
typedef struct {
	...
	********************************************************/
	VCODEC_ENC_ERROR_T(*pfnEncodeOneUnit)(IN HANDLE hCodec,
					      ...
					      IN VIDEO_ENCODER_INPUT_PARAM_NC_T *prEncoderInputParamNC);
...
typedef struct {
	union {
		...
		UINT64 		        pfnGetMemoryRequired_dummy;
	};
	...
	********************************************************/
	union {
		VCODEC_ENC_ERROR_T(*pfnOpen)(IN HANDLE hDrv,
						 IN HANDLE hWrapper,
						 IN VCODEC_ENC_CALLBACK_X64_T * ,
						 OUT HANDLE *hCodec
						);
		
		UINT64 		        pfnOpen_dummy;
	};
	...
	********************************************************/
	...
	union {
		VCODEC_ENC_ERROR_T(*pfnGenerateHeader)(IN HANDLE hCodec,
							   IN HANDLE hWrapper,
							   IN VCODEC_ENC_BUFFER_INFO_X64_T *prBufferInfo /* for smart phone */
							  );
		
		UINT64 		        pfnGenerateHeader_dummy;
	};
	/********************************************************
	*  wrapped for smart phone
	********************************************************/
	union {
		VCODEC_ENC_ERROR_T(*pfnEncodeOneUnit)(IN HANDLE hCodec,
							  IN HANDLE hWrapper,
							  IN VIDEO_ENCODER_INPUT_PARAM_NC_X64_T * prEncoderInputParamNC);
		
		UINT64 		        pfnEncodeOneUnit_dummy;
	};
	...
	union {
		...
		UINT64 		        pfnGetNextBitstream_dummy;
	};
} VIDEO_ENC_API_X64_T;
...
typedef struct {
	...
	union {
		VCODEC_ENC_ERROR_T(*pfnOpen)(IN HANDLE							  /*  hDrv	*/,
						 IN VCODEC_ENC_CALLBACK_X64_T * 	   /* pfnCallback */,
						 OUT HANDLE *						   /* hCodec */
						);
...
typedef struct {
	VIDEO_ENC_API_X64_T             rVideoEncAPI;
	union {
		VCODEC_ENC_API_X64_T	    *pfnVcodecEncAPI;
		UINT64 	        	        pfnVcodecEncAPI_dummy;
	};
	union {
		HANDLE                      hDriver;
		UINT64 	        	        hDriver_dummy;
	};
	VCODEC_BUFFER_X64_T             rReleaseYUV;
	VCODEC_ENC_BUFFER_INFO_X64_T    rEncoderBuffInfoNC;
	unsigned int	 			    u4EncType;// for sw venc type
} VIDEO_ENC_WRAP_HANDLE_X64_T;
