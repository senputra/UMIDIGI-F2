...
typedef struct
{
    int ImgWidth;
    int ImgHeight;
} TNR_DSDN_SIZE_INFO_STRUCT, *P_TNR_DSDN_SIZE_INFO_STRUCT;
typedef struct
{
    MINT32 imvx;
    MINT32 imvy;
} TNR_GMV_INT_STRUCT, *P_TNR_GMV_INT_STRUCT;
typedef struct
{
    MFLOAT fmvx;
    MFLOAT fmvy;
} TNR_GMV_FLOAT_STRUCT, *P_TNR_GMV_FLOAT_STRUCT;
typedef struct
{
	MBOOL GyroValid = false;
	MBOOL en_gyro_fusion = false;
    MUINT8 GMV_mode;
    MBOOL debug;
    MINT32 confThrL;
    MINT32 confThrH;
...
typedef struct
{
    TNR_GMV_INT_STRUCT tnr_gmv_int[8];
    TNR_GMV_FLOAT_STRUCT tnr_gmv_float[8];
    MINT32 tnr_gmv_conf;
...
typedef struct
{
    ...
    TNR_DSDN_SIZE_INFO_STRUCT dsdn_size_info[8];
    MINT32 RSSoWidth;
    MINT32 RSSoHeight;
    MINT32 imgiWidth;
    MINT32 imgiHeight;
