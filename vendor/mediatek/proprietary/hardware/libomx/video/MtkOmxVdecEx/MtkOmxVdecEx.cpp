...
//#endif
                ...
                MTK_OMX_LOGU("[OUTPUT] 32x32 Aligned! mOutputPortDef.nBufferSize(%d), nFrameWidth(%d), nFrameHeight(%d), nStride(%d), nSliceHeight(%d) nBufferCountActual(%d)",
                             ((OMX_PARAM_PORTDEFINITIONTYPE *)pCompParam)->nBufferSize, ((OMX_PARAM_PORTDEFINITIONTYPE *)pCompParam)->format.video.nFrameWidth, ((OMX_PARAM_PORTDEFINITIONTYPE *)pCompParam)->format.video.nFrameHeight, VDEC_ROUND_32(srcWStride), VDEC_ROUND_32(srcHStride), mOutputPortDef.nBufferCountActual);
            }
            else
            {
                err = OMX_ErrorBadPortIndex;
