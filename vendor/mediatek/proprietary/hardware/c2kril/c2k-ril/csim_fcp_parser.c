 ...
 *  tag         [IN]        csim_fcp_tag_enum tag to be found
 ...
 *  void
 *****************************************************************************/
unsigned char fcp_tlv_search_tags(unsigned char *in_ptr, unsigned short len, csim_fcp_tag_enum tag, unsigned char **out_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    //LOGD("TEcj [fcp_tlv_search_tags] start");
...
unsigned char prop_info_tlv_search_tag(
            ...
            unsigned short len,
            csim_fcp_proprietary_info_tag_enum tag,
 ...
 *  filled_struct_ptr       [OUT]       csim_file_descriptor_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
 *****************************************************************************/
unsigned char fcp_file_descriptor_query(unsigned char *in_ptr, unsigned short len, csim_file_descriptor_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    csim_file_descriptor_struct *query_ptr = filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_FILE_DES_T, &out_ptr);
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_file_identifier_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
 *****************************************************************************/
unsigned char fcp_file_identifier_query(unsigned char *in_ptr, unsigned short len, csim_file_identifier_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    unsigned char value_len = 0;
    unsigned char *out_ptr = NULL;
    csim_file_identifier_struct *query_ptr = (csim_file_identifier_struct*) filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_FILE_ID_T, &out_ptr);
    if (NULL != out_ptr)
    {
        if(value_len != 2)
        {
 ...
 *  void
 *****************************************************************************/
unsigned char fcp_df_name_query(unsigned char *in_ptr, unsigned short len, csim_df_name_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    unsigned char value_len = 0;
    unsigned char *out_ptr = NULL;
    csim_df_name_struct *query_ptr = (csim_df_name_struct*) filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_DF_NAME_T, &out_ptr);
    if(value_len > 16)
    {
        assert(0);
    }
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_proprietary_information_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
...
unsigned char fcp_proprietary_info_query(
            unsigned char *in_ptr,
            unsigned short len,
            csim_proprietary_information_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    unsigned char value_len = 0;
    unsigned char *out_ptr = NULL;
    csim_proprietary_information_struct *query_ptr = filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    memset(query_ptr, 0x00, sizeof(csim_proprietary_information_struct));

    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_PROPRIETARY_T, &out_ptr);
    if(value_len > 17) {
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_life_cycle_status_integer_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
...
unsigned char fcp_life_cycle_query(
            unsigned char *in_ptr,
            unsigned short len,
            csim_life_cycle_status_integer_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    unsigned char value_len = 0;
    unsigned char *out_ptr = NULL;
    csim_life_cycle_status_integer_struct *query_ptr = filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_LIFE_CYCLE_T, &out_ptr);
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_security_attributes_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
...
unsigned char fcp_security_attribute_query(
            unsigned char *in_ptr,
            unsigned short len,
            csim_security_attributes_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    //unsigned char value_len = 0;
    //unsigned char * out_ptr = NULL;
    //csim_security_attributes_struct * query_ptr = filled_struct_ptr;
    /*LOGD("Tecj [fcp_security_attribute_query] false");*/
    return FALSE;
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_pin_status_temp_do_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
 *****************************************************************************/
unsigned char fcp_pin_status_do_query(unsigned char *in_ptr, unsigned short len, csim_pin_status_temp_do_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    csim_pin_status_temp_do_struct *query_ptr = filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_PIN_DO_T, &out_ptr);
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_total_file_size_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
 *****************************************************************************/
unsigned char fcp_total_file_size_query(unsigned char *in_ptr, unsigned short len, csim_total_file_size_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    unsigned char value_len = 0;
    unsigned char *out_ptr = NULL;
    csim_total_file_size_struct *query_ptr = filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_TOTAL_FILE_SIZE_T, &out_ptr);
    if (NULL != out_ptr)
    {
        unsigned char i = 0;
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_file_size_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
 *****************************************************************************/
unsigned char fcp_file_size_query(unsigned char *in_ptr, unsigned short len, csim_file_size_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    unsigned char value_len = 0;
    unsigned char *out_ptr = NULL;
    csim_file_size_struct *query_ptr = filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_FILE_SIZE_T, &out_ptr);
    if (NULL != out_ptr)
    {
        if(value_len != 2)
        {
 ...
 *  in_ptr                  [IN]        Kal_uint8 * input buffer pointer
 *  len                     [IN]        Kal_uint16  length of the input buffer
 *  filled_struct_ptr       [OUT]       csim_short_file_identifier_struct *
 * RETURNS
 *  unsigned char if the structure is successfully filled.
 * GLOBALS AFFECTED
 *  void
 *****************************************************************************/
unsigned char fcp_sfi_query(unsigned char *in_ptr, unsigned short len, csim_short_file_identifier_struct *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    unsigned char value_len = 0;
    unsigned char *out_ptr = NULL;
    csim_short_file_identifier_struct *query_ptr = filled_struct_ptr;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    value_len = fcp_tlv_search_tags(in_ptr, len, FCP_SFI_T, &out_ptr);
    if (NULL != out_ptr)
    {
        if (value_len == 0)
        {
 ...
 *  void
 *****************************************************************************/
unsigned char csim_fcp_query_tag(unsigned char *in_ptr, unsigned short len, csim_fcp_tag_enum tag, void *filled_struct_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch (tag)
    {
        case FCP_FILE_DES_T:
            ...
            return fcp_file_descriptor_query(data_ptr, value_len, (csim_file_descriptor_struct*) filled_struct_ptr);

            break;
        case FCP_FILE_ID_T:
            return fcp_file_identifier_query(data_ptr, value_len, (csim_file_identifier_struct*) filled_struct_ptr);
            break;
        case FCP_DF_NAME_T:
            return fcp_df_name_query(data_ptr, value_len, (csim_df_name_struct*) filled_struct_ptr);
            break;
        ...
        case FCP_SEC_ATTRIBUTE_QUERY:
            ...
            break;
        case FCP_PIN_DO_T:
            return fcp_pin_status_do_query(data_ptr, value_len, (csim_pin_status_temp_do_struct*) filled_struct_ptr);
            break;
        case FCP_TOTAL_FILE_SIZE_T:
            return fcp_total_file_size_query(data_ptr, value_len, (csim_total_file_size_struct*) filled_struct_ptr);
            break;
        case FCP_FILE_SIZE_T:
            return fcp_file_size_query(data_ptr, value_len, (csim_file_size_struct*) filled_struct_ptr);
            break;
        case FCP_SFI_T:
            return fcp_sfi_query(data_ptr, value_len, (csim_short_file_identifier_struct*) filled_struct_ptr);
            break;
        default:
            assert(0);
            return FALSE;
...
int hexStringToByteArrays(unsigned char* hexString, unsigned char ** byte)
{
