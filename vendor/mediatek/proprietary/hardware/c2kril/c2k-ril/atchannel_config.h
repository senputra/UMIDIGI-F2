...
#ifndef ATCHANNEL_CONFIG_H
#define ATCHANNEL_CONFIG_H 1
...
typedef struct {
    char devicePath[PATH_LENGTH];
    int fd;
    int pseudoIndexMapped;
    pthread_mutex_t* p_channelMutex;
} PhysicalChannel;

typedef struct {
    char clientLink[PATH_LENGTH];
    char clientPath[PATH_LENGTH];
    char serverPath[PATH_LENGTH];
    int serverFd;
    int physicalIndexMapped;
    pthread_mutex_t* p_channelMutex;
} PseudoChannel;
