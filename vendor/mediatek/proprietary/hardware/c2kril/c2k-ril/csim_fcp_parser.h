...
#ifndef CSIM_FCP_PARSER_H
#define CSIM_FCP_PARSER_H

#define TRUE  1
#define FALSE 0

#define CSIM_MAX_RSP_FCP_LEN 128
#define FCP_TEMP_T 0x62

/* Define application template Data Object  */
#define CSIM_APP_TEMP_TAG 0x61
#define CSIM_APP_ID_TAG 0x4F
#define CSIM_APP_LABEL_TAG 0x50
...
} csim_fcp_proprietary_info_tag_enum;
...
typedef enum
{
    ...
    SECURITY_REF_T = 0x8B
} csim_fcp_security_attributes_tag_enum;
...
} csim_file_descriptor_struct;

typedef struct
{
    unsigned short file_id;
} csim_file_identifier_struct;
...
} csim_proprietary_information_struct;

/* ---------End of CSIM proprietary nformation--------- */

typedef struct
{
    unsigned char life_cycle_status;
} csim_life_cycle_status_integer_struct;

typedef struct
{
    unsigned char do_flag;
} csim_security_attributes_struct;
...
} csim_pin_status_temp_do_struct;

typedef struct
{
    unsigned int tot_file_size;
} csim_total_file_size_struct;

typedef struct
{
    unsigned short file_size;
} csim_file_size_struct;
...
} csim_short_file_identifier_struct;
...
extern unsigned char csim_fcp_query_tag(unsigned char *, unsigned short, csim_fcp_tag_enum, void *);
extern unsigned char fcp_tlv_search_tags(unsigned char *in_ptr, unsigned short len, csim_fcp_tag_enum tag, unsigned char **out_ptr);
extern int hexStringToByteArrays(unsigned char * hexString, unsigned char ** byte);
extern unsigned char *byteArrayToHexString(unsigned char* array,int length);

#endif /* CSIM_FCP_PARSER_H */
