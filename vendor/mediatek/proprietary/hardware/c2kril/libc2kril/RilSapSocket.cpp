...
void RilSapSocket::dispatchRequest(MsgHeader *req) {
    ...
    // C2K ril only support one slot, set SIM channel to it
    currRequest->cid = (RILChannelId)(SIM_CHANNEL);

    pendingResponseQueue.enqueue(currRequest);

    if (uimFuncs) {
        RLOGI("CRILD:RilSapSocket::dispatchRequest [%d] > SAP REQUEST type: %d. id: %d. error: %d \
