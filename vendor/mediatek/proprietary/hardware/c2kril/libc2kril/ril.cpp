...
static int responseSimStatus(Parcel &p, void *response, size_t responselen) {
    ...
    if (s_callbacks.version <= LAST_IMPRECISE_RIL_VERSION) {
        ...
        if (responselen == sizeof (RIL_CardStatus_v8)) {
            ...
            RLOGE("responseSimStatus: A RilCardStatus_v8, _v7 or _v6 expected\n");
            if (!isDebuggable()) {
                return RIL_ERRNO_INVALID_RESPONSE;
            } else {
                assert(0);
...
static int responseDcRtInfo(Parcel &p, void *response, size_t responselen)
{
    ...
    status_t status;
    status = p.writeInt64(pDcRtInfo->time);
    if (status != NO_ERROR) {
        RLOGE("responseDcRtInfo: writeInt64 failed. ");
        return RIL_ERRNO_INVALID_RESPONSE;
    }
...
static int responseCarrierRestrictions(Parcel &p, void *response, size_t responselen) {
  ...
  p.writeInt32(p_cr->allowedCarriersPrioritized);
...
RequestInfoProxy* createRpcRequestInfoProxy(RequestInfo* pRI, void *buffer, size_t buflen, int* pId) {
    ...
    if (request >= RIL_LOCAL_REQUEST_VENDOR_C2K_BASE) {
        for (i = 0; i < (int32_t)NUM_ELEMS(s_c2k_local_commands); i++) {
            if (request == s_c2k_local_commands[i].requestNumber) {
                proxyId = s_c2k_local_commands[i].proxyId;
                break;
            }
        }
    } else if (request >= RIL_REQUEST_VENDOR_BASE) {
...
#endif
{
    ...
    switch(unsolResponse) {
        ...
        case RIL_UNSOL_NITZ_TIME_RECEIVED:
            ...
            status_t status;
            status = p.writeInt64(timeReceived);
            if (status != NO_ERROR) {
                RLOGE("ResponseIndex %d: error. writeInt64 error for RIL_UNSOL_NITZ_TIME_RECEIVED\n"
                        , unsolResponseIndex);
                // Problem with the response. Don't continue;
                goto error_exit;
            }
