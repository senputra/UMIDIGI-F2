...
{
        bool bEnableLog = !!::property_get_int32("vendor.debug.camera.tpfb.log", 0);
...
public:
                           ...
                           RequestCallbackPtr pCallback = nullptr)
    {
        ...
        if (pRequest->mIMetadataApp != nullptr) {
            pIMetataApp = pRequest->mIMetadataApp->acquire();
            if (pIMetataApp != NULL)
                MY_LOGD("[IN] App metadata count: %d", pIMetataApp->count());
            else
                MY_LOGD("[IN] App metadata empty");
        }

        if (pIBufferFull != NULL && pIMetataApp != NULL)
        {
            MRect faceRect;
            char* pBufferVa = (char *) (pIBufferFull->getBufVA(0));
            MUINT32 stride = pIBufferFull->getBufStridesInBytes(0);
            IMetadata::IEntry entryFaceRects = pIMetataApp->entryFor(MTK_STATISTICS_FACE_RECTANGLES);
            for (size_t i = 0; i < entryFaceRects.count(); i++) {
                ...
                for (int j = faceRect.p.y + 1; j < faceRect.s.h ; j++) {
