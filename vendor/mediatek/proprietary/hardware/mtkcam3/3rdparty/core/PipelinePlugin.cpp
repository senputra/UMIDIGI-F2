 ...
 */
#define LOG_TAG "MtkCam/ppl_plugin"
...
struct PipelinePlugin<T>::Selection::Token final
{
    ...
    const MINT32 mSensorId;
    const MBOOL  mIsPhysical;

    Token(MINT32 iUniqueKey, MINT32 iReqNum, MINT32 iIndexNum, MINT32 sensorId, MBOOL isPhysical)
...
PipelinePlugin<T>::Selection::createToken(MINT32 iUniqueKey, MINT32 iReqNum, MINT32 iIndexNum, MINT32 sensorId)
{
    return make_shared<Token>(iUniqueKey, iReqNum, iIndexNum, sensorId, MFALSE);
}

template<typename T>
typename PipelinePlugin<T>::Selection::TokenPtr
PipelinePlugin<T>::Selection::createToken(MINT32 iUniqueKey, MINT32 iReqNum, MINT32 iIndexNum, MINT32 sensorId, MBOOL isPhysical)
{
    return make_shared<Token>(iUniqueKey, iReqNum, iIndexNum, sensorId, isPhysical);
}
...
PipelinePlugin<T>::getInstance(MINT32 iUniqueKey)
{
    MINT32 hash = iUniqueKey;
    const MBOOL isEnableMultiInstance = getIsEnableMultiInstance();
    if (!isEnableMultiInstance) {
        hash = 0;
    }

    MY_LOGD_IF(getDebugLevel() > 0, "%s: uniqueKey:%d", getName<T>().c_str(), iUniqueKey);
    Ptr sp =  mInstances[hash].lock();
    if (sp == nullptr) {
        sp = std::make_shared<PipelinePlugin<T>>();
        mInstances[hash] = sp;
        MY_LOGI("%s: create plugin(%p), hash:%d, uniqueKey:%d",
            getName<T>().c_str(), sp.get(), hash, iUniqueKey);
    }
...
: mpInterface(nullptr)
{
    MY_LOGI("%s: ctor(%p)", getName<T>().c_str(), this);
}
...
PipelinePlugin<T>::~PipelinePlugin()
{
    mpInterface = nullptr;
    mpProviders.clear();
    MY_LOGI("%s: dtor(%p)", getName<T>().c_str(), this);
...
MetadataSelection& MetadataSelection::operator =(const MetadataSelection& bs)
{
    if (bs.mImpl != nullptr)
        *mImpl = *bs.mImpl;
...
BufferSelection& BufferSelection::operator =(const BufferSelection& bs)
{
    if (bs.mImpl != nullptr)
        *mImpl = *bs.mImpl;
