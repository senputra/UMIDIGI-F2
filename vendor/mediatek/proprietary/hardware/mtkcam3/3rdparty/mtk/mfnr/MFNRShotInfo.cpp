...
void MFNRShotInfo::updateCurrent3A(ExposureUpdateMode mode)
{
...

    {
        ...
        if (getIsFlashOn() && m_bFlashMfllSupport) {
            ...
            if (m_mainframe_appMetaIn != NULL) {
                ...
                NSCam::IMetadata::getEntry<MUINT8>(m_mainframe_appMetaIn, MTK_CONTROL_AE_MODE, aeMode);
                NSCam::IMetadata::getEntry<MUINT8>(m_mainframe_appMetaIn, MTK_FLASH_MODE, flashMode);
                NSCam::IMetadata::getEntry<MUINT8>(m_mainframe_appMetaIn, MTK_CONTROL_CAPTURE_INTENT, captureIntent);
                MY_LOGD("aeMode:%d, flashMode:%d, captureIntent:%d", aeMode, flashMode, captureIntent);
    ...
    MY_LOGD("3A capture dummy/delayed frames(%u, %u)", dummyFrames, delayedFrames);
