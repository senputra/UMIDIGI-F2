...
#ifndef _MTK_HARDWARE_MTKCAM_PIPELINE_POLICY_REQUESTMETADATAPOLICY_H_
#define _MTK_HARDWARE_MTKCAM_PIPELINE_POLICY_REQUESTMETADATAPOLICY_H_
//
...
public:
    ...
    RequestMetadataPolicy_Default(CreationParams const& params);

private:
    auto    evaluateP1YuvCrop(
                        EvaluateRequestParams const& params
                    ) -> int;
    auto    updateRawImageInfo(
                        EvaluateRequestParams const& params
                    ) -> int;
    CreationParams mPolicyParams;
    std::vector<MSize>    mvTargetRrzoSize;
    std::vector<int64_t>  mvAppStreamId;
    MINT32         mUniqueKey;
    MINT32         mForceExif = 0;
...
public:
    // RequestMetadataPolicy Interfaces.
    RequestMetadataPolicy_DebugDump(CreationParams const& params);
...
#endif  //_MTK_HARDWARE_MTKCAM_PIPELINE_POLICY_REQUESTMETADATAPOLICY_H_
