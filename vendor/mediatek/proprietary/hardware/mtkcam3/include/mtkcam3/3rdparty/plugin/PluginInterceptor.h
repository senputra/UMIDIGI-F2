...
public:
    ...
    typedef typename PipelinePlugin<T>::RecommendedParam RecommendedParam;
    ...
    virtual MERROR queryRecommendedFrameSetting(RecommendedParam &param) {
        return mImpl.queryRecommendedFrameSetting(param);
    }
    ...
    virtual ~Interceptor() {
        ...
        // uninit plugin before destroy instance
        if (mInitCounter > 0) {
            mImpl.uninit();
            mInitCounter = 0;
        }
