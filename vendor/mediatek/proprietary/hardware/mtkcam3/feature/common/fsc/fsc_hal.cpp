                        ...
                        )
{
    ...
    if (!tryGetMetadata<MINT32>(pInAPP,
        ...
        MY_LOGW_IF(0, "no MTK_EIS_FEATURE_EIS_MODE");
    }
    ...
    if (mpFSCProvider)
    {
        ...
        {
            char content[256] = {0};
            snprintf(content, sizeof(content), "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", currMagicNum, iExpTime, mSensorSize.w, mSensorSize.h,
...
void FSCHalImp::createDumpFile()
{
    ...
    char path[256];
    snprintf(path, sizeof(path), FSC_DUMP_FOLDER_PATH);
    ...
    //open file
    snprintf(path, sizeof(path), FSC_DUMP_FOLDER_PATH"/%" PRId64 "_FSC_Init.csv", timewithoutSleep);
    mpFSCInitFp = NSCam::IFileCache::open(path);
    if (mpFSCInitFp)
    {
        char content[256] = {0};
        snprintf(content, sizeof(content), "macro_to_inf_ratio,max_cropped_ratio,dac_Inf,dac_Macro,"
    ...
    snprintf(path, sizeof(path), FSC_DUMP_FOLDER_PATH"/%" PRId64 "_AF_Update.csv", timewithoutSleep);
    mpAFUpdateFp = NSCam::IFileCache::open(path);
    if (mpAFUpdateFp)
    {
        char content[256] = {0};
        snprintf(content, sizeof(content), "request_frame_num,rolling_shutter_percentage,target_DAC\n");
        mpAFUpdateFp->write(&content, sizeof(content));
    }
    else
    {
    ...
    snprintf(path, sizeof(path), FSC_DUMP_FOLDER_PATH"/%" PRId64 "_FSC_Update.csv", timewithoutSleep);
    mpFSCUpdateFp = NSCam::IFileCache::open(path);
    if (mpFSCUpdateFp)
    {
        char content[256] = {0};
        snprintf(content, sizeof(content), "request_frame_num,exposureTime_mus,w_sensor_out,h_sensor_out,"
