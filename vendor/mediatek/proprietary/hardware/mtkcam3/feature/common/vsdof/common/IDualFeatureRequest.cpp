...
{
    return this->pushRequestImageBuffer(setting, pImgBuf.get());
}
...
)
{
    ...
    if (nullptr != pMetaBuf) {
        ...
        MY_LOGE("Passing pMetaBuf is Nullptr, bufferID=%d", setting.bufferID);
        return MFALSE;
    }
    //
    if (setting.ioType == eBUFFER_IOTYPE_INPUT)
...
)
{
    ...
    if (nullptr == pEffParam->getPtr(DUAL_REQ_META_KEY)) {
        MY_LOGE(" get DUAL_REQ_META_KEY from Metadata is null");
        return MFALSE;
    }
...
showContent()
{
    MY_LOGD("reqID=%d input/output frame size=%zu/%zu", this->getRequestNo(),
            this->vInputFrameInfo.size(), this->vOutputFrameInfo.size());
    for(ssize_t index=0;index<this->vInputFrameInfo.size();++index)
    {
        auto key = this->vInputFrameInfo.keyAt(index);
        MY_LOGD("reqID=%u index=%zd input frame_no=%" PRId64 "", this->getRequestNo(), index, key);
    }
    for(ssize_t index=0;index<this->vOutputFrameInfo.size();++index)
    {
        auto key = this->vOutputFrameInfo.keyAt(index);
        MY_LOGD("reqID=%u index=%zd output frame_no=%" PRId64 "", this->getRequestNo(), index, key);
    }
}
