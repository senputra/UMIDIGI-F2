...
struct StereoSensorSetting_T
{
    ...
    std::unordered_map<ENUM_STEREO_RATIO, NSCam::MSize> imgoYuvSize;
    std::unordered_map<ENUM_STEREO_RATIO, NSCam::MSize> rrzoYuvSize;
    ...
    void log(FastLogger &logger) const
    {
        ...
        if(fovHorizontal > 0.0f || fovVertical > 0.0f)
        {
            logger.FastLogD("FOV       H: %.2f / V: %.2f", fovHorizontal, fovVertical);
        }
...
struct StereoSensorConbinationSetting_T
{
    ...
    //For 3rd party
    std::unordered_map<ENUM_STEREO_RATIO, NSCam::MSize> depthmapSize;
    std::unordered_map<ENUM_STEREO_RATIO, NSCam::MSize> depthmapSizeCapture;

    //Customized VSDoF size
    bool            hasSizeConfig = false;
    std::unordered_map<ENUM_STEREO_RATIO, StereoHAL::StereoArea> baseSize;

    //Offline calibration
    std::vector<MUINT8> calibrationData;

    ENUM_DEPTHMAP_REFINE_LEVEL depthmapRefineLevel = E_DEPTHMAP_REFINE_DEFAULT;
    ...
    void log(FastLogger &logger)
    {
        ...
        if(IS_STEREO_MODE(logicalDeviceFeatureSet))
        {
            ...
            if(IS_TK_VSDOF_MODE(logicalDeviceFeatureSet))
            {
                ...
                switch(depthmapRefineLevel) {
                    case E_DEPTHMAP_REFINE_NONE:
                    default:
                        logger.FastLogD("Refine Level:     None");
                        break;
                    case E_DEPTHMAP_REFINE_SW_OPTIMIZED:
                        logger.FastLogD("Refine Level:     Software Optimized");
                        break;
                }
            }
            ...
            for(auto &m1 : depthmapSizeCapture) {
                imageRatioMToN(m1.first, m, n);
                sprintf(msg, "Capture Depthmap Size of %2d:%-2d: %dx%d", m, n, m1.second.w, m1.second.h);
                logger.FastLogD(msg);
            }
