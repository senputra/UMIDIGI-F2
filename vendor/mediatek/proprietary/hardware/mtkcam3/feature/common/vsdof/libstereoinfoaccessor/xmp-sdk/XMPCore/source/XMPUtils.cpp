...
static void FormatFullDateTime ( XMP_DateTime & tempDate, char * buffer, size_t bufferLen )
{
    ...
    // force to convert type for snprintf format
    Int32 tempYear = (Int32) tempDate.year;
    Int32 tempMonth = (Int32) tempDate.month;
    Int32 tempDay = (Int32) tempDate.day;
    Int32 tempHour = (Int32) tempDate.hour;
    Int32 tempMinute = (Int32) tempDate.minute;
    Int32 tempSecond = (Int32) tempDate.second;
    Int32 tempNanoSecond = (Int32) tempDate.nanoSecond;

    if ( (tempDate.second == 0) && (tempDate.nanoSecond == 0) ) {

        // Output YYYY-MM-DDThh:mmTZD.
        snprintf ( buffer, bufferLen, "%.4zd-%02zd-%02zdT%02zd:%02zd",    // AUDIT: Callers pass sizeof(buffer).
                   tempYear, tempMonth, tempDay, tempHour, tempMinute );

    } else if ( tempDate.nanoSecond == 0  ) {

        // Output YYYY-MM-DDThh:mm:ssTZD.
        snprintf ( buffer, bufferLen, "%.4zd-%02zd-%02zdT%02zd:%02zd:%02zd",    // AUDIT: Callers pass sizeof(buffer).
        ...
        snprintf ( buffer, bufferLen, "%.4zd-%02zd-%02zdT%02zd:%02zd:%02zd.%09zd", // AUDIT: Callers pass sizeof(buffer).
                   tempYear, tempMonth, tempDay,
                   tempHour, tempMinute, tempSecond, tempNanoSecond );
                                 ...
                                 XMP_VarString * _fullPath )
{
    ...
    if ( itemIndex == kXMP_ArrayLastItem ) {
        ...
        // force to convert type for snprintf format
        Int32 tempItemIndex = (Int32) itemIndex;
        ...
        snprintf ( buffer, sizeof(buffer), "[%zd]", tempItemIndex );
        fullPath += buffer;
    }
                            ...
                            XMP_VarString *      strValue )
{
    ...
    // force to convert type for snprintf format
    Int32 tempYear = (Int32) binValue.year;
    Int32 tempMonth = (Int32) binValue.month;
    Int32 tempDay = (Int32) binValue.day;
    Int32 tempTzHour = (Int32) binValue.tzHour;
    Int32 tempTzMinute = (Int32) binValue.tzMinute;
    ...
    if ( binValue.month == 0 ) {
        ...
        if ( (binValue.day == 0) && (! binValue.hasTime) ) {
            snprintf ( buffer, sizeof(buffer), "%.4zd", tempYear ); // AUDIT: Using sizeof for snprintf length is safe.
        ...
        snprintf ( buffer, sizeof(buffer), "%.4zd-%02zd", tempYear, tempMonth );    // AUDIT: Using sizeof for snprintf length is safe.
        ...
        snprintf ( buffer, sizeof(buffer), "%.4zd-%02zd-%02zd", tempYear, tempMonth, tempDay ); // AUDIT: Using sizeof for snprintf length is safe.
    ...
    if ( binValue.hasTimeZone ) {
        ...
        if ( binValue.tzSign == 0 ) {
            *strValue += 'Z';
            ...
            snprintf ( buffer, sizeof(buffer), "+%02zd:%02zd", tempTzHour, tempTzMinute );    // AUDIT: Using sizeof for snprintf length is safe.
