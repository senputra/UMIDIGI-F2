...
void
N3D_HAL_KERNEL::setDumpConfig(bool dump, TuningUtils::FILE_DUMP_NAMING_HINT *dumpHint, int dumpLevel)
{
    __isDump = dump;
    __dumpHint = dumpHint;
    __dumpLevel = dumpLevel;
}
...
std::string
N3D_HAL_KERNEL::__getN3DLogPath(const char *postfix)
{
    if(__dumpHint && __dumpLevel > 1) {
        const size_t DUMP_PATH_SIZE = 128;
        char path[DUMP_PATH_SIZE];
        std::string fileName = "N3D_LOG";
        if(postfix) {
            fileName += std::string(postfix);
        }

        genFileName_VSDOF_BUFFER(path, DUMP_PATH_SIZE, __dumpHint, fileName.c_str());
