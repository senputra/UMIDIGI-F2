...
struct N3D_HAL_KERNEL_INIT_PARAM
{
    ...
    std::vector<NSCam::IImageBuffer *> outputWarpMapMain2[3];
    std::vector<NSCam::IImageBuffer *> outputWarpMapMain1[3];   //no need for VSDoF
    ...
    N3D_HAL_KERNEL_INIT_PARAM(N3D_HAL_INIT_PARAM_WPE &initParam)
    {
        eScenario          = initParam.eScenario;
        fefmRound          = initParam.fefmRound;

        for(int i = 0; i < 3; ++i) {
            outputWarpMapMain2[i] = initParam.outputWarpMapMain2[i];
            outputWarpMapMain1[i] = initParam.outputWarpMapMain1[i];
        }
    }
};
...
public:
    ...
    virtual bool WarpMain2(N3D_HAL_PARAM_WPE &n3dParams,
                           STEREO_KERNEL_AF_INFO_STRUCT *afInfoMain1, STEREO_KERNEL_AF_INFO_STRUCT *afInfoMain2,
                           N3D_HAL_OUTPUT_WPE &n3dOutput);

    virtual bool runLearning(HWFEFM_DATA &hwfefmData, STEREO_KERNEL_AF_INFO_STRUCT *afInfoMain1, STEREO_KERNEL_AF_INFO_STRUCT *afInfoMain2);
    ...
    //Dump
    virtual void setDumpConfig(bool dump, TuningUtils::FILE_DUMP_NAMING_HINT *dumpHint=NULL, int dumpLevel=1);
...
private:
    ...
    std::string __getN3DLogPath(const char *postfix=NULL);
