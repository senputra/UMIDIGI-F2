...
N3D_HAL *
N3D_HAL::createInstance(N3D_HAL_INIT_PARAM_WPE &n3dInitParam)
{
    return new N3D_HAL_IMP(n3dInitParam);
}
...
N3D_HAL_IMP::N3D_HAL_IMP(N3D_HAL_INIT_PARAM_WPE &n3dInitParam)
    : __n3dKernel(N3D_HAL_KERNEL_INIT_PARAM(n3dInitParam))
    , __fastLogger(LOG_TAG, LOG_PERPERTY)
{
    __fastLogger.setSingleLineMode(SINGLE_LINE_LOG);
...
N3D_HAL_IMP::__initN3DHAL(N3D_HAL_INIT_PARAM_COMMON *n3dInitParam)
{
    ...
    if(eSTEREO_SCENARIO_CAPTURE != __eScenario) {
        ...
        __algoInitInfo.iio_main.out_h = areaOutput.size.h;
#else
        StereoArea areaInput = StereoSizeProvider::getInstance()->getBufferSize(E_RECT_IN_S, __eScenario);
        __algoInitInfo.iio_auxi.inp_w = areaInput.size.w;
        __algoInitInfo.iio_auxi.inp_h = areaInput.size.h;
        __algoInitInfo.iio_auxi.src_w = areaInput.contentSize().w;
        __algoInitInfo.iio_auxi.src_h = areaInput.contentSize().h;
...
N3D_HAL::refineMask(ENUM_STEREO_SCENARIO scenario, NSCam::IImageBuffer *mask, bool initMask)
{
    ...
    StereoArea maskArea(MASK_SIZE);

    if(initMask) {
        ::memset(image, 0xFF, MASK_SIZE.w * MASK_SIZE.h);
        maskArea = StereoSizeProvider::getInstance()->getBufferSize(E_MASK_IN_S);
        if(maskArea.size.w != MASK_SIZE.w ||
           maskArea.size.h != MASK_SIZE.h)
        {
            MY_LOGE("Mask size mismatch, MASK_SIZE %dx%d, maskArea %dx%d",
                    MASK_SIZE.w, MASK_SIZE.h, maskArea.size.w, maskArea.size.h);
        }
    }

    maskArea.startPt.y += maskMargin.top;
    maskArea.startPt.x += maskMargin.left;
    maskArea.padding.w += (maskMargin.left+maskMargin.right);
    maskArea.padding.h += (maskMargin.top+maskMargin.bottom);
    const MSize CONTENT_SIZE = maskArea.contentSize();

    //Top
    if(maskArea.startPt.y < maskArea.size.h) {
        ::memset(image, 0, maskArea.size.w * maskArea.startPt.y);
    }

    //Bottom
    if(maskArea.startPt.y+CONTENT_SIZE.h-1 < maskArea.size.h) {
        ::memset(image+(maskArea.startPt.y+CONTENT_SIZE.h)*maskArea.size.w, 0,
                 maskArea.size.w * (maskArea.padding.h-maskArea.startPt.y));
    }

    //Left & Right
    MUINT8 *leftPos  = image + (maskArea.startPt.y+CONTENT_SIZE.h-1) * maskArea.size.w;
    MUINT8 *rightPos = leftPos + maskArea.startPt.x + CONTENT_SIZE.w;
    const int RIGHT_SPACE = maskArea.padding.w - maskArea.startPt.x;
    if(initMask) {
        for(int h = CONTENT_SIZE.h-1; h >= 0; --h, leftPos -= maskArea.size.w, rightPos -= maskArea.size.w) {
            ::memset(leftPos, 0, maskArea.startPt.x);
    ...
    else
    {
        ...
        for(int h = CONTENT_SIZE.h-1; h >= 0; --h, leftPos -= maskArea.size.w, rightPos -= maskArea.size.w) {
            ...
            //truncate values
            contentPos = leftPos + maskArea.startPt.x;
...
N3D_HAL_IMP::__loadNVRAM()
{
    ...
    if(__hasCalibration) {
        ::memcpy(calibrationAddr, cal.calibration, CAM_CAL_Stereo_Data_SIZE);
