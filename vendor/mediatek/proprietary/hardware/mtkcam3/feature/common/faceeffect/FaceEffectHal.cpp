...
#include "include/FaceEffectHal.h"
 ...
 *
 ******************************************************************************/
/*#define FUNCTION_LOG_START      CAM_ULOGMD("[%s] - E.", __FUNCTION__)
#define FUNCTION_LOG_END        CAM_ULOGMD("[%s] - X.", __FUNCTION__)*/
...
getGestureResult(MtkCameraFaceMetadata *pGestureInfo, MtkCameraFaceMetadata *pFaceInfo, int FDEnable)
{
    ...
    if(mGDEnable) {
        ...
        CAM_ULOGMD("Original Gesture result : %d", GD_Result);
        ...
        if(pGestureInfo->number_of_faces != 0)
        {
           ...
           // debug message
           if (GD_Result == 0)
              CAM_ULOGMD("Scenario GD: Gesture detected but filtered out by face!!!");
        }
        /////////////////////////////////////////////////////////////////////
        ...
        if (pFaceInfo->number_of_faces == 0)
        {
           ...
           // debug message
           CAM_ULOGMD("Scenario GD: Gesture detected but no face!");
        }
        /////////////////////////////////////////////////////////////////////

        CAM_ULOGMD("Scenario GD Result: %d",GD_Result );
    }
    return 0;
}
...
updateASD(MUINT8* pRGBImg, MINT32 BufWidth, MINT32 BufHeight, MINT32 FaceNum, mhal_ASD_DECIDER_UI_SCENE_TYPE_ENUM &Scene, void *ASDInfo)
{
    ...
    if (mASDWorkingBuffer == NULL || mpASDHalObj == NULL) {
        CAM_ULOGME("The ASD module is not init");
        return -1;
    }
...
RunFaceEffect(Request_T &rFrameIn)
{
    ...
    */
        ...
        if(mFDEnable) {
            ...
            Param.pRGB565Image = (MUINT8 *)mFTImgBuffer;
            Param.pPureYImage  = pDstY;
            Param.pImageBufferPhyP0 = mDupImage.PAddrY;
...

            {
                ...
                CAM_ULOGMD("NumFace = %d, ", numFace);

                if(mSDEnable) {
                    mSD_Result = mpFDHalObj->halSDGetSmileResult();
                    CAM_ULOGMD("Smile Result:%d", mSD_Result);
                }
            }
        }
...
onDequeue(Request_T &rFrameIn)
{
    ...
    //
    if  ( mStop ) {
        CAM_ULOGMD("[StopFD] mInputQ.size:%zu", mInputQ.size());
        return MFALSE;
    }
    mDoFD = true;
    /*
...
~FaceEffectHal()
{
    CAM_ULOGMD("WillDBG delete FaceEffect");
}
...
allParameterConfigured()
{
    ...
    if (mFDSDUsed) {
        ...
        if(mpFDHalObj == NULL) {
            CAM_ULOGMD("Create FD hal instance failed!!");
...
getCaptureRequirementImpl(EffectParameter *inputParam, Vector<EffectCaptureRequirement> &requirements) const
{
    ...
    result.set(BasicParameters::KEY_PICTURE_SIZE, "640x480");
...
updateEffectRequestImpl(const android::sp<EffectRequest> request)
{
    FUNCTION_LOG_START;
    if(request == NULL){
        CAM_ULOGME("[updateEffectRequestImpl] request is NULL\n");
        return BAD_VALUE;
    }
    {
        Mutex::Autolock _l(mFrameQueueLock);
