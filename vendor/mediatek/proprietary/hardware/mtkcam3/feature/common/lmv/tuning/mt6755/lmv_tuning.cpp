...
void LMVP1Cb::p1TuningNotify(MVOID *pInput,MVOID *pOutput)
{
    ...
    MY_LOGD0("TG(%u,%u),RRZ In(%u,%u),RRZ crop x/y(%u,%u),RRZ crop w/h(%u,%u),RMX(%u,%u),HBIN(%u,%u),TWIN(%d),YUV(%d)",
            aLmvInInfo->sTGOut.w, aLmvInInfo->sTGOut.h,
            aLmvInInfo->RRZcrop.full_w, aLmvInInfo->RRZcrop.full_h,
            aLmvInInfo->RRZcrop.x, aLmvInInfo->RRZcrop.y,
            aLmvInInfo->RRZcrop.w, aLmvInInfo->RRZcrop.h,
    ...
    _this->mSensor_Width = aLmvInInfo->RRZcrop.full_w;
    _this->mSensor_Height = aLmvInInfo->RRZcrop.full_h;
    _this->mRrz_crop_Width = aLmvInInfo->RRZcrop.w;
    _this->mRrz_crop_Height = aLmvInInfo->RRZcrop.h;
    _this->mRrz_crop_X = aLmvInInfo->RRZcrop.x;
    _this->mRrz_crop_Y= aLmvInInfo->RRZcrop.y;
    ...
    else
    {
        ...
        MY_P_LOGW("Disable LMV because inputSize(%dx%d) is too small!", _this->mImgWidth, _this->mImgHeight);
    }
    ...
    if( rp_numH < 1)
    {
        MY_LOGE("rp_numH(%u) is invalid! Force set as 1", rp_numH);
        rp_numH = 1;
    }
