...
public:
    ...
    virtual enum AinrErr doDRC(const IAiAlgo::drcParam &params);
    virtual enum AinrErr calToneMatrix(const AinrAeInfo &src,
            const AinrAeInfo &target, std::vector<int> *matrix);
    virtual ~AiAlgo(void);
protected:
    virtual enum AinrErr applyShadingWb(const int openId, NSCam::IImageBuffer* inBuf, NSCam::IImageBuffer* outBuf,
