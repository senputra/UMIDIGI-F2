...
enum MfllErr MfllCore::do_CaptureSingle(void *void_index)
{
    ...
    if (CC_LIKELY(index >= 0 && index < getCaptureFrameNum()))
    {
        /**
         *  N O T I C E
         *
        ...
        if (CC_UNLIKELY(rStatus != 0)) {
            mfllLogW("%s: MfllCapture::captureFrames failed with status %d", __FUNCTION__, rStatus);
        }
    }
...
enum MfllErr MfllCore::do_Mixing(JOB_VOID)
{
...

    {
        ...
        if ( CC_LIKELY(m_ptrImgYuvThumbnail != nullptr) &&
            ...
            if (m_bDoDownscale && m_downscaleDivisor > 1) {
                mfllLogD("%s: enable downscale, crop region before downscale(x:%d, y:%d, w:%d, h:%d)",
...
enum MfllErr MfllCore::do_AllocYuvGolden(JOB_VOID)
{
    ...
    bool is_full_size_golden = (m_bAsSingleFrame && m_bDoDownscale && (m_downscaleDivisor > 0));
