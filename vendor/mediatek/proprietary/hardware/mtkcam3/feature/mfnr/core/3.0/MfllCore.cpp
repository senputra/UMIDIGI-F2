...
#define EVENTTYPE_NAME_ADD_ELEMENT(x) m[x] = #x
    ...
    EVENTTYPE_NAME_ADD_ELEMENT(EventType_AllocateRrzoBuffer);
    ...
    EVENTTYPE_NAME_ADD_ELEMENT(EventType_CaptureRrzo);
...
MfllCore::MfllCore(void)
    ...
    , m_memcMode((enum MemcMode)0)
    , m_rwbMode((enum RwbMode)0)
    , m_mrpMode((enum MrpMode)0)
    , m_width(0)
    , m_height(0)
    , m_imgoWidth(0)
    , m_imgoHeight(0)
    ...
    , m_internalYuvFmt(InputYuvFmt_YUV_P010)
    , m_bDoDownscale(false)
    , m_bDoMemcDownScale(false)
    , m_downscaleDividend(16)
    , m_downscaleDivisor(16)
{
    mfllFunctionIn();
    ...
    for (unsigned int i = 0; i < MFLL_MAX_FRAMES; i++) {
        ...
        lockSyncObject(&m_syncAllocYuvQ[i]);
...
enum MfllErr MfllCore::doMfll()
{
    ...
    if (registerMrpFeature) {
        mfllLogD3("Register plug-in Mrp");
    ...
    if (CC_UNLIKELY(m_spProperty->isDump())) {
        mfllLogD3("Register plug-in Dump");
        ...
        f.mix = m_spProperty->isDumpMix()           ? 1 : (m_spProperty->isDumpMixOut() ? 2 : 0);
    ...
    if (CC_UNLIKELY(m_spProperty->isFeed())) {
        mfllLogD3("Register plug-in Feed");
    ...
    {
        mfllLogD3("=========  M    F    L    L ============");
        mfllLogD3("sensor id         = %d", m_sensorId);
        mfllLogD3("capture num       = %d", m_frameCapture);
        mfllLogD3("blend-frame       = %d", m_frameBlend.load());
        mfllLogD3("blend yuv size    = %dx%d", m_width, m_height);
        mfllLogD3("imgo size         = %dx%d", m_imgoWidth, m_imgoHeight);
        mfllLogD3("rrzo size         = %dx%d", m_rrzoWidth, m_rrzoHeight);
        mfllLogD3("quater size       = %dx%d", m_qwidth, m_qheight);
        mfllLogD3("full size MC      = %d", m_isFullSizeMc);
        mfllLogD3("is zsd shot       = %s", isZsdMode(m_shotMode) ? "Yes" : "No");
        mfllLogD3("iso               = %d", m_iso);
        mfllLogD3("exposure          = %d", m_exposure);
        mfllLogD3("memc mode         = %s", m_memcMode == MemcMode_Sequential ? "sequential" : "parallel");
        mfllLogD3("memc threads      = %d", m_memcInstanceNum);
        mfllLogD3("rwb mode          = %d", m_rwbMode);
        mfllLogD3("feature dump      = %s", m_spProperty->isDump() ? "Yes" : "No");
        if (m_spProperty->isDump()) {
        mfllLogD3("feature dump.raw  = %s", m_spProperty->isDumpRaw() ? "Yes" : "No");
        mfllLogD3("feature dump.yuv  = %s", m_spProperty->isDumpYuv() ? "Yes" : "No");
        mfllLogD3("feature dump.mfb  = %s", m_spProperty->isDumpMfb() ? "Yes" : "No");
        mfllLogD3("feature dump.mix  = %s", m_spProperty->isDumpMix() ? "Yes" : "No");
        mfllLogD3("feature dump.jpeg = %s", m_spProperty->isDumpJpeg() ? "Yes" : "No");
        mfllLogD3("feature dump.pv   = %s", m_spProperty->isDumpPostview() ? "Yes" : "No");
        mfllLogD3("feature dump.exif = %s", m_spProperty->isDumpExif() ? "Yes" : "No");
        }
        //
        mfllLogD3("feature feed      = %s", m_spProperty->isFeed() ? "Yes" : "No");
        //
        if (CC_UNLIKELY( m_spProperty->isFeed() )) {
        mfllLogD3("feature feed.bfbld= %s", m_spProperty->isFeedBfbld() ? "Yes" : "No");
        mfllLogD3("feature feed.bss  = %s", m_spProperty->isFeedBss() ? "Yes" : "No");
        mfllLogD3("feature feed.memc = %s", m_spProperty->isFeedMemc() ? "Yes" : "No");
        mfllLogD3("feature feed.sf   = %s", m_spProperty->isFeedSf() ? "Yes" : "No");
        mfllLogD3("feature feed.mfb  = %s", m_spProperty->isFeedMfb() ? "Yes" : "No");
        mfllLogD3("feature feed.mix  = %s", m_spProperty->isFeedMix() ? "Yes" : "No");
        mfllLogD3("feature feed.jpeg = %s", m_spProperty->isFeedJpeg() ? "Yes" : "No");
        }
        mfllLogD3("feature mrp       = %d", registerMrpFeature);
        mfllLogD3("========================================");
    }
    ...
    // until the MFNR related post-processing threads finished.
    m_futurePhase1.wait();
    m_futurePhase2.wait();
    m_futureMemc.wait();
    m_futureMemory.wait();
...
enum MfllErr MfllCore::setBypassOptionAsSingleFrame()
{
    ...
    if (CC_UNLIKELY(shooted)) {
        mfllLogW("%s: Mfll is shooted, ignore.", __FUNCTION__);
        return MfllErr_Shooted;
    }
...
enum MfllErr MfllCore::setCaptureResolution(unsigned int width, unsigned int height)
{
    ...
    if (m_bDoDownscale && m_downscaleDivisor > 0) {
        m_width = width*m_downscaleDividend/m_downscaleDivisor;
        m_height = height*m_downscaleDividend/m_downscaleDivisor;
    }
    else {
        m_width = width;
        m_height = height;
...
enum MfllErr MfllCore::setRrzoResolution(unsigned int width, unsigned int height)
{
    enum MfllErr err = MfllErr_Ok;
    ...
    /* update debug info too */
    m_dbgInfoCore.rrzo_width = width;
    m_dbgInfoCore.rrzo_height = height;
...
enum MfllErr MfllCore::getBlendFrameSize(int &w, int &h)
{
    w = m_width;
    h = m_height;
...
enum MfllErr MfllCore::setSyncPrivateData(const std::deque<void*>& dataset)
{
    ...
    if (CC_LIKELY( dataset.size() >= 3)) {
        NSCam::IMetadata *pAppMeta = static_cast<NSCam::IMetadata*>(dataset[0]);
        NSCam::IMetadata *pHalMeta = static_cast<NSCam::IMetadata*>(dataset[1]);
        //NSCam::IImageBuffer *pLcsoImg = static_cast<IImageBuffer*>(dataset[2]);

        {
            ...
            mfllLogD3("%s: Get timestamp -> %d, timestamp->: %" PRIi64, __FUNCTION__, timestamp.result, timestamp.val);
            mfllLogD3("%s:=========================", __FUNCTION__);

            m_timestampSync.push_back(timestamp.val);
        }

        if (CC_UNLIKELY( m_timestampSync.size() == 1 )) {
            struct T {
                int val;
                MBOOL result;
                T() : val(-1), result(MFALSE) {};
            ...
            uniqueKey.result = NSCam::IMetadata::getEntry<int>(pHalMeta, MTK_PIPELINE_UNIQUE_KEY, uniqueKey.val);

            mfllLogD3("%s:=========================", __FUNCTION__);
            mfllLogD3("%s: Get uniqueKey -> %d, uniqueKey->: %d", __FUNCTION__, uniqueKey.result, uniqueKey.val);
            mfllLogD3("%s:=========================", __FUNCTION__);

            m_uniqueKey = uniqueKey.val;
        }
...
bool MfllCore::_isNeedResizedYBuffer(void)
{
    /*
     * If downscale denoise enabled, and capture as single frame, we don't need resized Y8
     * because there no MEMC in need
     */
    if (m_bDoDownscale && m_bAsSingleFrame)
        return false;

    if (getMemcInstanceNum() <= 0)
        return false;

    if (m_qwidth <= 0 || m_qheight <= 0) {
        mfllLogE("%s: quarter size width and height(%dx%d) is werid!!!", __FUNCTION__, m_qwidth, m_qheight);
        return false;
    }
...
void MfllCore::_updateSingeFrameBypassConfigLocked(void)
{
    if ( ! m_bAsSingleFrame )
        return ;
    ...
    for (int i = 0; i < MFLL_MAX_FRAMES; i++){
        // no need RAW stage in MfllCore
        m_bypass.bypassAllocRawBuffer[i] = 1;
        m_bypass.bypassAllocRrzoBuffer[i] = 1;

        // no need BFBLD stage, but if Downscale-Denoise enable, need index = 0, see below.
        m_bypass.bypassAllocQyuvBuffer[i] = 1; // no need allocate QYuv Buffer
    ...
    // If Downscale-Denoise enabled, we need the first QYuvBuffer set (do not bypass)
    if (m_bDoDownscale)
        m_bypass.bypassAllocQyuvBuffer[0] = 0;

    // bypass all buffer allocation expect GOLDEN
    m_bypass.bypassAllocWeighting[0] = 1;
    m_bypass.bypassAllocWeighting[1] = 1;
    m_bypass.bypassAllocYuvMcWorking = 1;
    m_bypass.bypassAllocYuvWorking = 1;

    // bypass BFBLD (base YUV)
    m_bypass.bypassAllocYuvBase = 1;
    m_bypass.bypassEncodeYuvBase = 1;
    m_bypass.bypassBss = 1;
...
enum MfllErr MfllCore::setInputYuvFormat(const InputYuvFmt fmt)
{
    MfllErr err = MfllErr_Ok;
...
InputYuvFmt MfllCore::getInputYuvFormat() const
{
    return m_internalYuvFmt.load();
}
...
sp<IMfllImageBuffer> MfllCore::retrieveBuffer(const enum MfllBuffer &s, int index /* = 0 */)
{
    ...
    case MfllBuffer_Raw:
        ...
        break;
    case MfllBuffer_Rrzo:
        if (CC_UNLIKELY(index < 0 || index >= static_cast<int>(getCaptureFrameNum()))) {
            mfllLogE("%s: get rrzo buffer but index is wrong (index, max) = (%d, %d)", __FUNCTION__, index, getCaptureFrameNum());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        p = m_imgRrzos[index].getImgBufLocked();
        break;
...
{
    enum MfllErr err = MfllErr_Ok;
    mfllLogD3("%s: (type:%d, index:%d)", __FUNCTION__, s, index);
    switch (s) {
    case MfllBuffer_Raw:
        if (CC_UNLIKELY(index < 0 || index >= static_cast<int>(getCaptureFrameNum()))) {
    ...
    case MfllBuffer_Rrzo:
        if (CC_UNLIKELY(index < 0 || index >= static_cast<int>(getCaptureFrameNum()))) {
            mfllLogE("%s: rrzo buffer index is out of range (index, max) = (%d, %d)", __FUNCTION__, index, getCaptureFrameNum());
            err = MfllErr_BadArgument;
            goto lbExit;
        }
        m_imgRrzos[index] = 0;
...
void MfllCore::updateImageBufferQueueLimit()
{
    ...
    auto update__ = [](std::shared_ptr<IMfllImageBufferQueue>& q, int l, IMfllImageBuffer_Flag_t f)
    {
        if (q.get() != nullptr)
            ...
            mfllLogD3("ImageBufferQueue(flag=%d) doesn't exist", f);
        }
    };
...
ImageFormat MfllCore::getInputYuvFmtYuvQ() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_Y8;
        case InputYuvFmt_Nv21:       return ImageFormat_Y8;
        case InputYuvFmt_Nv12:       return ImageFormat_Y8;
        default: break;
    }
    mfllLogE("%s: m_internalYuvFmt is undefined", __FUNCTION__);
    *(volatile uint32_t*)(0x00000000) = 0xDEADFEED;
    return ImageFormat_Unknown;
}


ImageFormat MfllCore::getInputYuvFmtYuvFull() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_YUV_P010;
        case InputYuvFmt_Nv21:       return ImageFormat_Nv21;
        case InputYuvFmt_Nv12:       return ImageFormat_Nv12;
        default: break;
    }
    mfllLogE("%s: m_internalYuvFmt is undefined", __FUNCTION__);
    *(volatile uint32_t*)(0x00000000) = 0xDEADFEED;
...
ImageFormat MfllCore::getInputYuvFmtYuvBase() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_YUV_P010;
        case InputYuvFmt_Nv21:       return ImageFormat_Nv21;
...
ImageFormat MfllCore::getInputYuvFmtYuvGolden() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_YUV_P010;
        case InputYuvFmt_Nv21:       return ImageFormat_Nv12; // we always using NV12 for YUV golen
        case InputYuvFmt_Nv12:       return ImageFormat_Nv12;
        default: break;
    }
    mfllLogE("%s: m_internalYuvFmt is undefined", __FUNCTION__);
...
ImageFormat MfllCore::getInputYuvFmtYuvMcWorking() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_YUV_P010;
        case InputYuvFmt_Nv21:       return ImageFormat_Nv21;
...
ImageFormat MfllCore::getInputYuvFmtYuvWorking() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:   return ImageFormat_YUV_P010;
        case InputYuvFmt_Nv21:       return ImageFormat_Nv21;
...
ImageFormat MfllCore::getInputYuvFmtWeightingTable() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:
        case InputYuvFmt_Nv21:
        case InputYuvFmt_Nv12:       return ImageFormat_Raw8;
        default: break;
    }
    mfllLogE("%s: m_internalYuvFmt is undefined", __FUNCTION__);
    *(volatile uint32_t*)(0x00000000) = 0xDEADFEED;
...
ImageFormat MfllCore::getInputYuvFmtMemcWorking() const
{
    switch (m_internalYuvFmt.load()) {
        case InputYuvFmt_YUV_P010:
        case InputYuvFmt_Nv21:
...
enum MfllErr MfllCore::doEncodeYuvGolden(JOB_VOID)
{
    ...
    if (m_bypass.bypassEncodeYuvGolden || status.ignore) {
        ...
        /* if downscale enabled + single frame, we have to do two pass P2,
         * the same as Single Frame capture.
         */
        if (m_bDoDownscale && m_bAsSingleFrame) {
            err = do_EncodeYuvGoldenDownscale(__arg);
            if (err != MfllErr_Ok) {
                goto lbExit;
            }
        }
...
lbExit:
    ...
    m_imgRrzos[0].releaseBufLocked();
    m_imgRaws[0].releaseBufLocked();
...
/**
 *  Encode Raw to QYuv. This operation must be invoked after RAW domain BSS
 *  while these conditions are matched:
 *
 *  1. BSS has been done
 */
enum MfllErr MfllCore::doEncodeQYuv(void *void_index)
{
    unsigned int index = (unsigned int)(long)void_index;
    enum MfllErr err = MfllErr_Ok;
    MfllEventStatus_t status;

    syncWaitDone(&m_syncAllocYuvQ[index]);

    mfllFunctionInInt(index);
    mfllTraceCall();

    /* invokes event */
    m_event->onEvent(EventType_CaptureYuvQ, status, this, (void*)(long)index);

    if (m_bypass.bypassEncodeQYuv[index] || status.ignore) {
        mfllLogD("%s: Bypass encode QYuv(%d)", __FUNCTION__, index);
        status.ignore = 1;
        syncWaitDone(&m_syncQueueYuvCapture[index]);
    }
    else {
        err = do_EncodeQYuv(void_index);
        if (err != MfllErr_Ok) {
            status.ignore = 1;
        }
...
lbExit:
    status.err = err;
    syncAnnounceDone(&m_syncCapturedYuvQ[index]);
    m_event->doneEvent(EventType_CaptureYuvQ, status, this, (void*)(long)index);

    // late release, except RAW index 0 (index 0 will be used by encode yuv base)
    if (index != 0) {
 ...
 *
 *  1. BSS has been done
 *  1. QYuv Base and Ref Image has been done
 */
enum MfllErr MfllCore::doMotionEstimation(void *void_index)
{
    ...
    /* conditions */
    syncWaitDone(&m_syncCapturedYuvQ[0]); //Base Image for Me
    syncWaitDone(&m_syncCapturedYuvQ[index + 1]); //Ref Image for Me
...
enum MfllErr MfllCore::doBlending(void *void_index)
{
    ...
    if (MfllProperty::getMfbP2DirectLink()) {
        if (index+2 >= getFrameCapturedNum()) {// index + 2 = blended + 1 = capture num
            directlink = true;
        }
    }
...
lbExit:
    ...
    if (status.ignore || err != MfllErr_Ok) {
         ...
         */
        if (m_blendedCount > 0 && !directlink) {
            /* ping-pong YUV base <--> YUV blended */
            swap<ImageBufferPack*>(m_ptrImgYuvBase, m_ptrImgYuvBlended);
            swap<ImageBufferPack*>(m_ptrImgWeightingIn, m_ptrImgWeightingOut);
        }
    ...
    if (!directlink) {
        mfllLogD3("YUV and QYUV buffer of index(%d) is released", index);
        /* release buffer */
        m_imgYuvs[index + 1].releaseBufLocked();
        m_imgQYuvs[index + 1].releaseBufLocked();
    } else {
        mfllLogD("YUV and QYUV buffer of index(%d) is NOT released", index);
    }

    if (status.ignore == 0 && err == MfllErr_Ok) {
        mfllLogD("%s: blending (%d) ok", __FUNCTION__, index);
        if (m_blendedCount == 1) {
            /* prevent the best yuv frame (m_imgYuvs[0]) to be over write*/
            mfllLogD3("%s: use base yuv buffer as ping-pong buffer for next round", __FUNCTION__);
...
lbExit:
    ...
    if (MfllProperty::getMfbP2DirectLink()) {
        /* release buffer */
        m_imgYuvs[getFrameCapturedNum() - 1].releaseBufLocked();
        m_imgQYuvs[getFrameCapturedNum() - 1].releaseBufLocked();
    }
...
enum MfllErr MfllCore::doAllocRawBuffer(void *void_index)
{
    ...
    if (m_bypass.bypassAllocRawBuffer[index] || status.ignore) {
        mfllLogD3("%s: Bypass allocate raw buffers", __FUNCTION__);
        status.ignore = 1;
    }
    else {
...
enum MfllErr MfllCore::doAllocRrzoBuffer(void *void_index)
{
    unsigned int index = (unsigned int)(long)void_index;
    enum MfllErr err = MfllErr_Ok;
    MfllEventStatus_t status;
    ...
    /* thread-safe operation lock */
    std::lock_guard<std::mutex> lk(m_opLockDoAllocRrzoBuffer[index]);

    mfllFunctionIn();
    mfllTraceCall();
    /* trigger events */
    m_event->onEvent(EventType_AllocateRrzoBuffer, status, this, (void*)(long)index);

    if (m_bypass.bypassAllocRrzoBuffer[index] || status.ignore) {
        mfllLogD3("%s: Bypass allocate rrzo buffers", __FUNCTION__);
        status.ignore = 1;
    }
    else {
        err = do_AllocRrzoBuffer(void_index);
    }

lbExit:
    status.err = err;
    m_event->doneEvent(EventType_AllocateRrzoBuffer, status, this, (void*)(long)index);
    mfllFunctionOut();
    m_bypass.bypassAllocRrzoBuffer[index] = 1; // ignore the next round
    return err;
}
...
enum MfllErr MfllCore::doAllocQyuvBuffer(void *void_index)
{
    ...
    if (m_bypass.bypassAllocQyuvBuffer[index] || status.ignore) {
        mfllLogD3("%s: Bypass allocate QYUV buffers", __FUNCTION__);
        status.ignore = 1;
    }
    else {
...
lbExit:
    ...
    syncAnnounceDone(&m_syncAllocYuvQ[index]);
    mfllFunctionOut();
    return err;
}
...
enum MfllErr MfllCore::doAllocYuvBase(JOB_VOID)
{
    ...
    if (m_bypass.bypassAllocYuvBase || status.ignore) {
        mfllLogD3("%s: Bypass allocate YUV base buffer", __FUNCTION__);
        status.ignore = 1;
    }
    else {
...
enum MfllErr MfllCore::doAllocYuvGolden(JOB_VOID)
{
    ...
    if (m_bypass.bypassAllocYuvGolden || status.ignore) {
        mfllLogD3("%s: Bypass allocate YUV golen buffer", __FUNCTION__);
        status.ignore = 1;
    }
    else {
...
enum MfllErr MfllCore::doAllocYuvWorking(JOB_VOID)
{
    ...
    if (m_bypass.bypassAllocYuvWorking || status.ignore) {
        mfllLogD3("%s: Bypass allocate YUV working(mixing) buffer", __FUNCTION__);
        status.ignore = 1;
    }
    else {
...
enum MfllErr MfllCore::doAllocYuvMcWorking(JOB_VOID)
{
    ...
    if (m_bypass.bypassAllocYuvMcWorking || status.ignore) {
        mfllLogD3("%s: Bypass allocate YUV MC working buffer", __FUNCTION__);
        status.ignore = 1;
    }
    else {
...
enum MfllErr MfllCore::doAllocWeighting(void *void_index)
{
    ...
    if (m_bypass.bypassAllocWeighting[index] || status.ignore) {
        mfllLogD3("%s: Bypass allocate weighting table(%d)", __FUNCTION__, index);
        status.ignore = 1;
    }
    else {
...
enum MfllErr MfllCore::doAllocMemcWorking(void *void_index)
{
    ...
    if (m_bypass.bypassAllocMemc[index] || status.ignore) {
        mfllLogD3("%s: Bypass allocate MEMC working buffer(%d)", __FUNCTION__, index);
        status.ignore = 1;
    }
    else {
...
void MfllCore::tellsFutureAllocateMemory()
{
    ...
    m_futureMemory.fu = std::async(std::launch::async, [this]()->void
    {
        ...
        int captureNum = (int)getCaptureFrameNum();
        int blendNum = (int)getBlendFrameNum();
        bool bIsMultiFrame = (blendNum - 1) <= 0 ? false : true;
...
void MfllCore::tellsFuturePhase1()
{
    ...
    m_futurePhase1.fu = std::async(std::launch::async, [this]() -> void {
        ...
        for (int i = 0; i < times; i++)
            doEncodeQYuv((void*)(long)i);

        if (m_bAsSingleFrame) {
            // for re-init image buffer size in single frame scenario.
            mfllLogD("wait until golden memory is allocated...");
            m_futureMemory.wait();
...
void MfllCore::tellsFutureMemc()
{
    ...
    m_futureMemc.fu = std::async(std::launch::async, [this]()->void
    {
        ...
        mfllLogD3("times to blend(%d), MEMC instanceNum(%d), threadsNum(%d)",
            times,
            instanceNum,
            threadsNum);
