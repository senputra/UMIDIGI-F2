...
enum MfllErr MfllImageBuffer::convertImageFormat(const enum ImageFormat &f)
{
    ...
    if (!m_bHasOwnership) {
        auto isSupportConvertImageFormat = [this](const enum ImageFormat &f) -> bool {
            if (f != ImageFormat_Yuy2 || m_format != ImageFormat_I422)
                return false;

            // check continuous memory
            MUINT32 imgFmt = mapImageFormat(m_format);
            unsigned int alignedw = ALIGN(m_width, m_alignedWidth);
            unsigned int alignedh = ALIGN(m_height, m_alignedHeight);
            for (size_t i = 0 ; i < m_imgBuffer->getPlaneCount()-1 ; i++) {
                unsigned int bitsPerPixel = queryPlaneBitsPerPixel(imgFmt, i);
                unsigned int w = (m_imgBuffer->getBufStridesInPixel(i)*bitsPerPixel)/8;  // calcuate in bytes
                unsigned int h = (m_imgBuffer->getBufScanlines(i)*bitsPerPixel)/8;  // calcuate in bytes
                unsigned int offset = w * h;

                mfllLogD3("%s: plane(%zu) VA offset(0x%td) PA offset(0x%td) offset(0x%x) w(%d) h(%d)", __FUNCTION__, i,
                    m_imgBuffer->getBufVA(i+1) - m_imgBuffer->getBufVA(i),
                    m_imgBuffer->getBufPA(i+1) - m_imgBuffer->getBufPA(i), offset, w, h);

                if ((m_imgBuffer->getBufVA(i) + offset != m_imgBuffer->getBufVA(i+1))
                ||  (m_imgBuffer->getBufPA(i) + offset != m_imgBuffer->getBufPA(i+1))) {
                    mfllLogE("%s: support only continuous I422 to YUY2", __FUNCTION__);
                    return false;
                }
            }
        ...
        if (!isSupportConvertImageFormat(f)) {
...
enum MfllErr MfllImageBuffer::saveFile(const char *name, bool commonDump)
{
    ...
    if (m_bHasOwnership && !commonDump) {
        ...
        for (size_t i = 0; i < m_imgBuffer->getPlaneCount(); i++) {
            ...
            mfllLogD("%s: write plane(%zu) with size %zu to %s", __FUNCTION__, i, lineSize, name);
        }
    }
    else {
...
enum MfllErr MfllImageBuffer::releaseBufferNoLock(void)
{
    ...
    if (m_bHasOwnership) {
        ...
        if (m_imgBuffer && m_imgOwnFromOther.get() != m_imgBuffer) {
            //restore lockBuf status if imgBuffer has been modified
            m_imgBuffer->unlockBuf(MFLL_LOG_KEYWORD);
            if (m_imgOwnFromOther.get() && !m_imgOwnFromOther->lockBuf(MFLL_LOG_KEYWORD, BUFFER_USAGE)) {
                mfllLogE("%s: lock buffer fail", __FUNCTION__);
            }
        }
        if (m_imgOwnFromOther.get()) {
            m_imgOwnFromOther.get()->decStrong((void*)(m_imgOwnFromOther.get()));
        }
    }
...
MUINT32 MfllImageBuffer::mapImageFormat(const enum ImageFormat &fmt, bool dontCare /* = false */)
{
    ...
    case ImageFormat_Nv12:
        f = eImgFmt_NV12;
        break;
    case ImageFormat_YUV_P010:
        f = eImgFmt_MTK_YUV_P010;
        break;
