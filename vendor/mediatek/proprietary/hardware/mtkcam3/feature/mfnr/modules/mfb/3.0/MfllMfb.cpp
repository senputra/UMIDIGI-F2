...
static void dumpQParams(const QParams& rQParam, bool bForcedDump = false)
{
    ...
    for(size_t index = 0; index < rQParam.mvFrameParams.size(); ++index) {
        ...
        for ( size_t index2 = 0; index2 < frameParam.mvOut.size(); index2++ ) {
            ...
            mfllLogI("%s: mvOut.mBuffer=0x%p", __FUNCTION__, (void*)data.mBuffer);
            if( data.mBuffer != NULL ) {
                ...
                mfllLogI("%s: mvOut.mBuffer->getPlaneCount = %d", __FUNCTION__, data.mBuffer->getPlaneCount());
                for ( size_t j = 0; j < data.mBuffer->getPlaneCount(); j++ ) {
                    mfllLogI("%s: mvOut.mBuffer->getBufVA(%zu) = 0x%p", __FUNCTION__, j, (void*)data.mBuffer->getBufVA(j));
                    mfllLogI("%s: mvOut.mBuffer->getBufPA(%zu) = 0x%p", __FUNCTION__, j, (void*)data.mBuffer->getBufPA(j));
        ...
        )
{
    ...
    mfllLogD3("generate DREParam with userId: %x, iso: %d, sensorId: %d, magicNum: %d",
        ...
        IImageBuffer *mcmv /* = nullptr */)
{
    ...
    // OMC_MFB mirror for base image buffer
    if (m_blendCount == 0 && base && base->getPlaneCount() == 2) {
        mfllLogD3("%s: enable MFB(%d) mirror base-frame", __FUNCTION__, m_blendCount);
        //ref
        fix_mfb_mirror_nv12(
            static_cast<unsigned char*>( (void *)base->getBufVA(0) ),
            static_cast<unsigned char*>( (void *)base->getBufVA(1) ),
            base->getImgSize().w,
            base->getImgSize().h,
            base->getBufStridesInBytes(0),
            base->getBufSizeInBytes(0) / base->getBufStridesInBytes(0)
        );
        base->syncCache();
    }

    // OMC_MFB mirror for reference image buffer
    if (ref && ref->getPlaneCount() == 2) {
        mfllLogD3("%s: enable MFB(%d) mirror ref-frame", __FUNCTION__, m_blendCount);
        //ref
        fix_mfb_mirror_nv12(
            static_cast<unsigned char*>( (void *)ref->getBufVA(0) ),
            static_cast<unsigned char*>( (void *)ref->getBufVA(1) ),
            ref->getImgSize().w,
            ref->getImgSize().h,
            ref->getBufStridesInBytes(0),
            ref->getBufSizeInBytes(0) / ref->getBufStridesInBytes(0)
        );
        ref->syncCache();
    }
    ...
    if (MfllProperty::getMfbP2DirectLink()) {
        if (m_blendCount+2 >= m_pCore->getFrameCapturedNum()) { // index + 2 = blended + 1 = capture num, but this frame is not blended, so it equals to index
            mfllLogD3("%s: MFB direct link at blend(%d), total_frames(%d)", __FUNCTION__, m_blendCount+1, m_pCore->getFrameCapturedNum());
            directLink = true;
        }
    }

    //reset
    m_vMfbConfig = EGNParams<MFBConfig>();
    err = generateMfbParam(base, ref, out, wt_in, wt_out, confmap, mcmv, directLink, m_vMfbConfig);

    if (directLink || err != MfllErr_Ok ) {
        return err;
    }
    ...
    m_vMfbConfig.mpCookie = reinterpret_cast<void*>(&cookie);
    m_vMfbConfig.mpfnCallback = __P2Cb< EGNParams<MFBConfig> >;

    mfllLogD3("%s: MFB EGNenque", __FUNCTION__);
    mfllLogI("%s: mpCookie=%p", __FUNCTION__, m_vMfbConfig.mpCookie);
    {
        mfllTraceName("MFB");
        if (CC_UNLIKELY( ! m_pMfbStream->EGNenque(m_vMfbConfig) )) {
...
// ----------------------------------------------------------------------------
//
enum MfllErr MfllMfb::generateMfbParam
(
        IImageBuffer *base,
        IImageBuffer *ref,
        IImageBuffer *out,
        ...
        EGNParams<MFBConfig>& rMfbParams)
{
...
#if 0
    int _enablePostMfbRefine = 1; //TODO: from NVRAM
    int _enablePostMfbRefineByProp = MfllProperty::readProperty
        (Property_PostMfbRefine);

    if (CC_UNLIKELY( _enablePostMfbRefineByProp >= 0 )) {
        _enablePostMfbRefine = _enablePostMfbRefineByProp;
    }
#else
    int _enablePostMfbRefine = m_pCore->getMiddlewareInfo().postrefine_mfb;
#endif
    m_mutex.unlock();
    ...
    if (!m_tuningBufMfb.isInited) {
        ...
        // FIXME: no need tuning_reg here, but IHalISP needs to fix bug.
        size_t regTableSize = m_pNormalStream->getRegTableSize();
    ...
    if (CC_LIKELY( _enablePostMfbRefine > 0 )) {
        int level = m_pCore->getMemcFrameLvConfs()[index];
        mfllLogD3("%s: enable postMFB(%zu) refine, level = %d", __FUNCTION__, index, level);
        me_refine_mfb_regs(static_cast<void*>( m_tuningBufMfb.data.get() ), level, index);
    }
     ...
     */
    {
        ...
        mfbCfg.Mfb_format_bit = [&]()
        {
            auto f = base->getImgFormat();
            switch (f) {
            case eImgFmt_NV12:
            case eImgFmt_NV21:
                return FMT_4202P_8BIT;
            case eImgFmt_MTK_YUV_P010:
                return FMT_4202P_10BIT_PAK;
            default:
                mfllLogE("wrong data format(%#x) for MFB driver", f);
            }
            return FMT_4202P_10BIT_PAK;
        ...
        mfllLogD3("OMC_MFB img_fmt = %d, mfb_fmt = %d, mfb_fmt_bit = %d", base->getImgFormat(), mfbCfg.Mfb_format, mfbCfg.Mfb_format_bit);
        ...
        if (directLink) {
            mfbCfg.Mfb_outbuf_frame = nullptr;
            mfllLogI("output(blended): nullptr due to direct link");
        }
        else {
            mfbCfg.Mfb_outbuf_frame = out;
            mfllLogI("output(blended): va/pa(%p/%p) fmt(%#x), size=(%d,%d), planes(%zu), stride(%zu)",
                    // va/pa
        ...
        if (directLink) {
            mfbCfg.Mfb_outbuf_blendWeight = nullptr;
            mfllLogI("output(wt_out): nullptr due to direct link");
        }
        else {
        ...
        /**
         *  Motion compensation motion vector (since MFNR v3.0, new OMV_MFB hardware introduced) depends
         *  on platform, not all platform supports motion compensation motion vector.
         */
#if MFB_SUPPORT_MCMV
        mfbCfg.Mfb_inbuf_mv = mcmv;
#endif // MFB_SUPPORT_MCMV
...
enum MfllErr MfllMfb::mix(IMfllImageBuffer *base, IMfllImageBuffer *ref, IMfllImageBuffer *out, IMfllImageBuffer *wt, IMfllImageBuffer *dceso)
{
    ...
    IImageBuffer *img_dceso = dceso?(IImageBuffer*)dceso->getImageBuffer():nullptr;
    ...
    /* execute pass 2 operation */
    {
        ...
        mfllLogD3("%s: DCESO i4DcsMagicNo=%d", __FUNCTION__, p1Magic);

        TuningParam rTuningParam;
        rTuningParam.pRegBuf = tuning_reg.get();
        if (p1Magic >= 0) {
            rTuningParam.pDcsBuf = img_dceso;
        ...
        if (this->setIsp(0, metaset, &rTuningParam, &rMetaSet, img_src) == 0) {
...
#else
            int _enablePostNrRefine = m_pCore->getMiddlewareInfo().postrefine_nr;
#endif
            if (CC_LIKELY( _enablePostNrRefine > 0 )) {
                ...
                bss_refine_postnr_regs(
                        ...
                        m_pCore->getMemcFrameLvConfs().data(),
                        real_blend_cnt,
                        expected_blend_cnt);
        ...
        if (MfllProperty::getMfbP2DirectLink()) {
            mfllLogD("%s: no blended frame input due to direct link", __FUNCTION__);
        }
        else
        ...
        if (MfllProperty::getMfbP2DirectLink()) {
            mfllLogD("%s: no weighting input due to direct link", __FUNCTION__);
        }
        else
        // input: weighting
        if (pltcfg[ePLATFORMCFG_ENABLE_MIX3] && pltcfg[ePLATFORMCFG_REFINE_OK])
        ...
        if (MfllProperty::getMfbP2DirectLink()) {
            // OMC_MFB mirror for reference image buffer
            if (img_ref) {
                mfllLogD3("%s: enable MIX mirror ref-frame", __FUNCTION__);
                //ref
                mfllLogW("%s: aligned height should be calculated by bufsize and stride", __FUNCTION__);
                fix_mfb_mirror_nv12(
                    static_cast<unsigned char*>( (void *)img_ref->getBufVA(0) ),
                    static_cast<unsigned char*>( (void *)img_ref->getBufVA(1) ),
                    img_ref->getImgSize().w,
                    img_ref->getImgSize().h,
                    img_ref->getBufStridesInBytes(0),
                    ref->getAlignedHeight() // img_ref->getBufSizeInBytes(0) / img_ref->getBufStridesInBytes(0)
                );
                img_ref->syncCache();
            }
            //
            ExtraParam extraP;
            extraP.CmdIdx = EPIPE_MFB_INFO_CMD;
            extraP.moduleStruct = static_cast<void*>(&(m_vMfbConfig.mEGNConfigVec.back()));
            params.mvExtraParam.push_back(extraP);
            mfllLogD3("%s: pMfbInfo=%p\n", __FUNCTION__, extraP.moduleStruct);
            mfllLogD("%s: attch MFB pararm for OMC_MFB P2 direct link", __FUNCTION__);
        }
