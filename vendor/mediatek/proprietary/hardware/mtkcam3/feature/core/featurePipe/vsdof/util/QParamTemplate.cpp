...
QParamTemplateFiller&
QParamTemplateFiller::setInfo(MUINT frameID, const FrameInfo& frameInfo, MINT8 dumpFlag)
{
    FrameParams& frameParam = mTargetQParam.mvFrameParams.editItemAt(frameID);

    frameParam.FrameNo = frameInfo.FrameNo;
    frameParam.RequestNo = frameInfo.RequestNo;
    frameParam.Timestamp = frameInfo.Timestamp;
    frameParam.UniqueKey = frameInfo.UniqueKey;
    frameParam.IspProfile = frameInfo.IspProfile;
    IHalSensorList *sensorList = MAKE_HalSensorList();
    if(sensorList!=nullptr)
        frameParam.SensorDev = sensorList->querySensorDevIdx(frameInfo.SensorID);
    else
        MY_LOGW("sensor list is nullptr, fail to get SensorDev");
    frameParam.NeedDump = dumpFlag;
    return *this;
}
