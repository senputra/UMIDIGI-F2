...
{
    VSDOF_LOGD("+  reqID=%d srcNodeID=%d bufferID=%s",
    ...
    {
        VSDOF_LOGD("inside request, srcNodeID=%d bufferID=%d %s", srcNodeID, bufferID, BID2Name(bufferID));
    ...
    {
        VSDOF_LOGD("request buffer mgr, srcNodeID=%d bufferID=%s", srcNodeID, BID2Name(bufferID));
        ...
        {
            VSDOF_LOGD("request GB buffer mgr, srcNodeID=%d bufferID=%s", srcNodeID, BID2Name(bufferID));
...
{
    VSDOF_LOGD("+  bufferID=%s", BID2Name(bufferID));
    IImageBuffer* pOutImgBuf = NULL;
    {
        SmartFatImageBuffer smImgBuf = mpBufferPoolMgr->request(bufferID, mReqAttr.bufferScenario);
        if(smImgBuf.get() != NULL)
...
{
    VSDOF_LOGD("srcNodeID =%d srcBufferID=%s outNodeID=%d outBufferID=%s" , srcNodeID, BID2Name(srcBufferID), outNodeID, BID2Name(outBufferID));
    ...
    else if(bufferType == eBUFFER_GRAPHIC)
    {
        ...
        // read the buffer out
        {
            ...
            if(bufIndex<0)
            {
                MY_LOGE("Cannot find the graphic buffer to config, buffer id:%s", BID2Name(srcBufferID));
                return MFALSE;
            }
            smBuf = queuedBufferMap.valueAt(bufIndex);
        }
    ...
    else if(bufferType == eBUFFER_TUNING)
    {
        ...
        // read the buffer out
        {
            ...
            if(bufIndex<0)
            {
                MY_LOGE("Cannot find the tuning buffer to config, buffer id:%s", BID2Name(srcBufferID));
                return MFALSE;
            }
            smBuf = queuedBufferMap.valueAt(bufIndex);
        }
...
)
{
    ...
    else
    {
        ...
        VSDOF_LOGD("Cannot get enqued buffer, node id:%d, buffer id: %d %s", srcNodeID, bufferID, BID2Name(bufferID));
        return MFALSE;
    }
}
