...
)
{
    ...
    MBOOL bRet = MFALSE;
    // copy template
    rOutParm = mQParam_BAYER_FD_ONLY;
    // filler
    QParamTemplateFiller qParamFiller(rOutParm);
    //
    MUINT iFrameNum = 0;
    ...
    qParamFiller.insertTuningBuf(iFrameNum, tuningRes.tuningBuffer). // insert tuning data
            ...
            insertOutputBuf(iFrameNum, PORT_IMG2O, pFdBuf).
            setCrop(iFrameNum, eCROP_CRZ, MPoint(0,0), frameBuf_RSRAW1->getImgSize(), pFdBuf->getImgSize());

    qParamFiller.setExpectedEndTime(iFrameNum);
...
)
{
    ...
    if(bExistFD)
    {
        ...
        rQFiller.insertOutputBuf(iFrameNum, PORT_IMG2O, pFdBuf).
                setCrop(iFrameNum, eCROP_CRZ, ptCrop, szCrop, pFdBuf->getImgSize());
    }
    else
...
)
{
    ...
    // crop information
    MPoint ptRECT_CROP(0,0);
    MSize szRECT_CROP(0, 0);

    MBOOL bSuccess =
        // WROT: Rect_in1
        generator.addCrop(eCROP_WROT, ptRECT_CROP, szRECT_CROP, rP2AFMSize.mRECT_IN_SIZE_MAIN1).
...
)
{
    ...
    rQFiller.insertOutputBuf(iFrameNum, PORT_WROTO, pImgBuf).
            setCrop(iFrameNum, eCROP_WROT, MPoint(0,0), frameBuf_RSRAW1->getImgSize(), pImgBuf->getImgSize()).
            addExtraParam(iFrameNum, EPIPE_MDP_PQPARAM_CMD, (void*)&mVSDOFPQParamMap.editValueFor(iFrameNum));
    rQFiller.setExpectedEndTime(iFrameNum);
...
MBOOL
BayerMonoQTemplateProvider::
prepareQParamsTemplate_BAYER_FD_ONLY(MINT32 iModuleTrans)
{
    VSDOF_LOGD("+");
    ...
    MBOOL bSuccess =
        QParamTemplateGenerator(iFrameNum, miSensorIdx_Main1, ENormalStreamTag_Normal).
            ...
            generate(mQParam_BAYER_FD_ONLY);
