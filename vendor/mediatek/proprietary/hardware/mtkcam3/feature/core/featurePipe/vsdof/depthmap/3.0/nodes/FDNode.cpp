...
CAM_ULOG_DECLARE_MODULE_ID(MOD_INVALID);
...
onThreadLoop()
{
    ...
    //Do FD
    {
        VSDOF_PRFLOG("halFDDo start, reqId=%d   %p  %p   %d  %#" PRIxPTR "", pRequest->getRequestNo(), mpDDPBuffer,  mpExtractYBuffer, jpegOrientation, pFDBuf->getBufPA(0));
    	// timer
...
mergeExtraData(DepthMapRequestPtr& pRequest)
{
    ...
    if(pExtraDataImgBuf != nullptr)
    {
        ...
        {
            MY_LOGE("Extra data length is larger than the output buffer size, ExtraData size=%zu , output buffer size=%zu", strlen(sExtraData), pExtraDataImgBuf->getBufSizeInBytes(0));
        }
        else
        {
