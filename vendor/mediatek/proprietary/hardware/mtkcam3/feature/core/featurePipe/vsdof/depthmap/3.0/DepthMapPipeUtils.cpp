...
)
{
    ...
    {
        region.gmvX = entry.itemAt(LMV_REGION_INDEX_GMVX, Type2Type<MINT32>());
        region.gmvY = entry.itemAt(LMV_REGION_INDEX_GMVY, Type2Type<MINT32>());
        VSDOF_LOGD("LMV gmvX:%d, gmvY: %d", region.gmvX, region.gmvY);
    }

    if (entry.count() > LMV_REGION_INDEX_MAX_GMV)
    {
        region.confX = entry.itemAt(LMV_REGION_INDEX_CONFX, Type2Type<MINT32>());
        region.confY = entry.itemAt(LMV_REGION_INDEX_CONFY, Type2Type<MINT32>());
        region.maxGMV = entry.itemAt(LMV_REGION_INDEX_MAX_GMV, Type2Type<MINT32>());
        VSDOF_LOGD("LMV confX:%d, confY: %d, maxGMV: %d",
                    region.confX, region.confY, region.maxGMV);
    }
    else if (entry.count() > LMV_REGION_INDEX_CONFY)
    {
        region.confX = entry.itemAt(LMV_REGION_INDEX_CONFX, Type2Type<MINT32>());
        region.confY = entry.itemAt(LMV_REGION_INDEX_CONFY, Type2Type<MINT32>());
        region.maxGMV = LMV_MAX_GMV_64;
        VSDOF_LOGD("LMV confX:%d, confY: %d, fix maxGMV: %d",
                    region.confX, region.confY, region.maxGMV);
    }
    ...
    {
        region.x_int        = entry.itemAt(LMV_REGION_INDEX_XINT, Type2Type<MINT32>());
        region.x_float      = entry.itemAt(LMV_REGION_INDEX_XFLOAT, Type2Type<MINT32>());
        region.y_int        = entry.itemAt(LMV_REGION_INDEX_YINT, Type2Type<MINT32>());
        region.y_float      = entry.itemAt(LMV_REGION_INDEX_YFLOAT, Type2Type<MINT32>());
        region.s.w          = entry.itemAt(LMV_REGION_INDEX_WIDTH, Type2Type<MINT32>());
        region.s.h          = entry.itemAt(LMV_REGION_INDEX_HEIGHT, Type2Type<MINT32>());
        //
        VSDOF_LOGD("LMV Region: %d, %d, %d, %d, %dx%d",
                        region.x_int,
                        region.x_float,
                        region.y_int,
                        region.y_float,
    ...
    MY_LOGE("wrong LMV region count %u", entry.count());
    return MFALSE;
}
    ...
    for(size_t index = 0; index < rInputQParam.mvFrameParams.size(); ++index)
    {
        ...
        for(size_t index2=0;index2<frameParam.mvOut.size(); index2++)
        {
            ...
            if(data.mBuffer != NULL)
            {
                ...
                MY_LOGD("mvOut.mBuffer->getImgFormat = %d", data.mBuffer->getImgFormat());
...
)
{
    ...
    DpPqParam* pDpPqParam = (DpPqParam*)pBufferHandler->requestWorkingTuningBuf(srcNodeID, BID_DP_PQ_PARAM);
    ...
    VSDOF_LOGD("iFrameNo=%d scenario=%d enable=%d timestamp=%u iso=%d frameNo=%d requestNo=%d lensId=%d iMDPPortMask=%d",
...
)
{
    ...
    VSDOF_LOGD("scenario=%d enable=%d timestamp=%d iso=%d frameNo=%d requestNo=%d lensId=%d",
