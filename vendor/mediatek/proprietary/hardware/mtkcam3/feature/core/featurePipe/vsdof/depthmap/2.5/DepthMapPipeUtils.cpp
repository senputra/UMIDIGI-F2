...
QParamTemplateFiller&
QParamTemplateFiller::
setExpectedEndTime(MUINT frameID)
{
    FrameParams& frameParam = mTargetQParam.mvFrameParams.editItemAt(frameID);
    struct timeval curTime;
    gettimeofday(&curTime, NULL);
    frameParam.ExpectedEndTime = curTime;
    return *this;
}
...
MVOID debugQParams(const QParams& rInputQParam)
{
    ...
    for(size_t index = 0; index < rInputQParam.mvFrameParams.size(); ++index)
    {
        ...
        MY_LOGD("Frame Expected End Time (tv_sec) = %ld", frameParam.ExpectedEndTime.tv_sec);
        MY_LOGD("Frame Expected End Time (tv_usec) = %ld", frameParam.ExpectedEndTime.tv_usec);
...
)
{
    ...
    IMetadata* inApp = pBufferHandler->requestMetadata(eDPETHMAP_PIPE_NODEID_P2AFM,
                                                       BID_META_IN_APP);
    MINT32 iso;
    ...
    IMetadata* inHal = pBufferHandler->requestMetadata(eDPETHMAP_PIPE_NODEID_P2AFM,
    ...
    MY_LOGD("scenario=%d enable=%d timestamp=%lu iso=%d frameNo=%d requestNo=%d lensId=%d",
