...
CAM_ULOG_DECLARE_MODULE_ID(MOD_SFPIPE_DEPTH_OCC);
...
{
    VSDOF_INIT_LOG("+");
    CAM_ULOGM_TAGLIFE("OCCNode::onThreadStart");
...
onThreadStop()
{
    CAM_ULOGM_TAGLIFE("OCCNode::onThreadStop");
    VSDOF_INIT_LOG("+");
    cleanUp();
    VSDOF_INIT_LOG("-");
    return MTRUE;
...
onThreadLoop()
{
    ...
    CAM_ULOGM_TAGLIFE("OCCNode::onThreadLoop");
    ...
    CAM_ULOGM_TAG_BEGIN("OCCNode::OCCHALRun");
...
)
{
    ...
    if(pBufferHandler->getEnqueBuffer(getNodeId(), BID_OCC_OUT_NOC, pDepthBuffer))
    {
        ...
        VSDOF_LOGD("reqID=%d distance measure =%.2f", pRequest->getRequestNo(), newDistance);
    }
    return MTRUE;
}
...
)
{
    ...
    {
        MY_LOGE("reqID=%d Failed to get MTK_CONVERGENCE_DEPTH_OFFSET!!", pRequest->getRequestNo());
        return MFALSE;
    }
    // config magic number of Main1
    DepthMapBufferID inHalBID_Main1 = mapQueuedBufferID(pRequest, mpPipeOption, BID_META_IN_HAL_MAIN1);
    ...
    VSDOF_LOGD("reqID=%d distance measure =%.2f", pRequest->getRequestNo(), depthInfo.mfDistance);
    // push into storage
...
)
{
    ...
    if(!checkDumpIndex(pRequest->getRequestNo())) {
        rOCCParams.dumpHint = nullptr;
    } else {
        DepthMapBufferID inHalBID_Main1 = mapQueuedBufferID(pRequest, mpPipeOption, BID_META_IN_HAL_MAIN1);
        IMetadata* pInHalMeta_Main1 = pBufferHandler->requestMetadata(getNodeId(), inHalBID_Main1);
        extract(&mDumpHint_Main1, pInHalMeta_Main1);
        rOCCParams.dumpHint = &mDumpHint_Main1;
    }
    rOCCParams.ldcMain1 = pImgBuf_LDC;
