...
prepareTemplateParams()
{
    ...
    bRet &= prepareQParamsTemplate_NOFEFM(iModuleTrans, mQParam_NORMAL_NOFEFM);
...
{
    ENUM_PASS2_ROUND pass2Round = PASS2_ROUND_COUNT;
...
)
{
    ...
    MBOOL bSuccess = prepareQParam_NOFEFM_frame0(iModuleTrans, rQParam);
    bSuccess &= prepareQParam_NOFEFM_frame1(iModuleTrans, rQParam);
...
)
{
    ...
    MBOOL bSuccess =
        QParamTemplateGenerator(iFrameNum, miSensorIdx_Main2, ENormalStreamTag_Normal).
            addInput(PORT_IMGI).
            // Rect_in2
            addCrop(eCROP_WROT, MPoint(0,0), MSize(0,0), rP2AFMSize.mRECT_IN_SIZE_MAIN2).
...
MBOOL
BayerBayerQTemplateProvider::
prepareQParam_NOFEFM_frame1(
    MINT32 iModuleTrans,
    QParams& rQParam
)
{
    MUINT iFrameNum = 1;
    const P2ABufferSize& rP2AFMSize = mpSizeMgr->getP2A(eSTEREO_SCENARIO_PREVIEW);
    //
    QParamTemplateGenerator generator =
        QParamTemplateGenerator(iFrameNum, miSensorIdx_Main1, ENormalStreamTag_Normal).
        addInput(PORT_IMGI);
    ...
    // crop information
    MPoint ptFD_CROP(0,0), ptMVF_CROP(0,0), ptRECT_CROP(0,0);
    MSize szFD_CROP(0, 0);
    MSize szMVF_CROP(0, 0);
    MSize szRECT_CROP(0, 0);
    ...
    MBOOL bSuccess =
        ...
        generator.addCrop(eCROP_CRZ, ptFD_CROP, szFD_CROP, rP2AFMSize.mFD_IMG_SIZE).
            ...
            addOutput(mapToPortID(BID_P2A_OUT_MV_F), 0, EPortCapbility_Disp).
            // WROT: Rect_in1
            addCrop(eCROP_WROT, ptRECT_CROP, szRECT_CROP, rP2AFMSize.mRECT_IN_SIZE_MAIN1).
...
{
    CAM_ULOGM_TAGLIFE("P2AFMNode::buildQParams_NORMAL");
...
{
    CAM_ULOGM_TAGLIFE("P2AFMNode::buildQParams_NORMAL_NOFEFM");
    VSDOF_LOGD("+, reqID=%d", rEffReqPtr->getRequestNo());
    // copy template
    ...
    MBOOL bRet = buildQParam_frame0_NOFEFM(rEffReqPtr, tuningResult, qParamFiller);
    bRet &= buildQParam_frame1_NOFEFM(rEffReqPtr, tuningResult, qParamFiller);
    if(!bRet)
        return MFALSE;
...
{
    CAM_ULOGM_TAGLIFE("P2AFMNode::buildQParams_CAPTURE");
    VSDOF_LOGD("+, reqID=%d", rEffReqPtr->getRequestNo());
    // copy template
...
)
{
    ...
    rQFiller.setExpectedEndTime(iFrameNum);
...
)
{
    ...
    AAATuningResult tuningRes = tuningResult.tuningRes_main2;
    // output: Rect_in2 input(WROT)
    IImageBuffer* pImgBuf = pBufferHandler->requestBuffer(nodeID, BID_P2A_OUT_RECT_IN2);
    ENUM_STEREO_SCENARIO scenario = (rEffReqPtr->getRequestAttr().isEISOn) ?
                                        eSTEREO_SCENARIO_RECORD : eSTEREO_SCENARIO_PREVIEW;
    pImgBuf->setExtParam(mpSizeMgr->getP2A(scenario).mRECT_IN_CONTENT_SIZE_MAIN2);
    // filler buffers and the raw size crop info
    rQFiller.insertTuningBuf(iFrameNum, tuningRes.tuningBuffer).  // insert tuning data
        insertInputBuf(iFrameNum, PORT_IMGI, frameBuf_RSRAW2). // input: Main2 RSRAW
        setCrop(iFrameNum, eCROP_WROT, MPoint(0,0), frameBuf_RSRAW2->getImgSize(), pImgBuf->getImgSize()).
        insertOutputBuf(iFrameNum, PORT_WROTO, pImgBuf).// output: FE2B input(WROT)
        addExtraParam(iFrameNum, EPIPE_MDP_PQPARAM_CMD, (void*)&mVSDOFPQParamMap.editValueFor(iFrameNum));
    rQFiller.setExpectedEndTime(iFrameNum);
...
)
{
    ...
    mVSDOFPQParamMap.editValueFor(iFrameNum).WDMAPQParam = nullptr;
    // ouput: Rect_in1
    IImageBuffer* pImgBuf = pBufferHandler->requestBuffer(nodeID, BID_P2A_OUT_RECT_IN1);
    ENUM_STEREO_SCENARIO scenario = (rEffReqPtr->getRequestAttr().isEISOn) ?
                                        eSTEREO_SCENARIO_RECORD : eSTEREO_SCENARIO_PREVIEW;
    pImgBuf->setExtParam(mpSizeMgr->getP2A(scenario).mRECT_IN_CONTENT_SIZE_MAIN1);
    rQFiller.insertOutputBuf(iFrameNum, PORT_WDMAO, frameBuf_MV_F).  // MV_F
            insertOutputBuf(iFrameNum, PORT_WROTO, pImgBuf). //Rect_in1
            setCrop(iFrameNum, eCROP_WROT, MPoint(0,0), frameBuf_RSRAW1->getImgSize(), pImgBuf->getImgSize()).
            addExtraParam(iFrameNum, EPIPE_MDP_PQPARAM_CMD, (void*)&mVSDOFPQParamMap.editValueFor(iFrameNum));
