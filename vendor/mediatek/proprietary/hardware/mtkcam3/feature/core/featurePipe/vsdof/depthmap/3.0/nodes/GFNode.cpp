    ...
    DepthMapRequestPtr pRequest)
{
    ...
    GF_HAL_OUT_DATA sOutData;
    MINT32 iReqIdx = pRequest->getRequestNo();
    if(!requireAlgoDataFromRequest(pRequest, sInData, sOutData, true))
    {
        MY_LOGE("get algo buffer fail, reqID=%d", iReqIdx);
        return MFALSE;
    }
    //
    sInData.isCapture = true;
    //
    debugGFParams(sInData, sOutData);
    //
    pRequest->mTimer.startGFDepth();
    //
    ...
    //
    pRequest->mTimer.stopGFDepth();
    VSDOF_PRFTIME_LOG("[CapturePass]gf algo processing time(%d ms) reqID=%d",
                                pRequest->mTimer.getElapsedGFDepth(), pRequest->getRequestNo());
    //
    pRequest->setOutputBufferReady(GF_OUT_DEPTHMAP);
    this->handleDataAndDump(GF_OUT_DEPTHMAP, pRequest);
    CAM_ULOGM_TAG_END();
    return MTRUE;
}
...
)
{
    ...
    MY_LOGD("images.size()=%zu", inData.images.size());
    for(ssize_t idx=0;idx<inData.images.size();++idx)
    {
        MY_LOGD("images[%zd]=%p", idx, inData.images[idx]);
        if(inData.images[idx])
        {
            MY_LOGD("images[%zd], image size=%dx%d", idx,
    ...
    bool bIsCapture)
{
    ...
    bRet &= pBufferHandler->getEnqueBuffer(getNodeId(), cfmID, pImgCFM);
    if(bRet)
    {
