...
CameraDeviceManagerBase(char const* type)
    ...
    //
    , mDataRWLock()
    , mIsSetTorchModeSupported(true)    //From API Level 29, always return true for isSetTorchModeSupported
    , mPhysEnumDeviceMap()
    , mVirtEnumDeviceMap()
    , mOpenDeviceMap()
    , mCallback(nullptr)
    ...
    , mActiveOperationCommandList()
{
    ...
    mIsCameraHiddenEnabled = ::property_get_bool("vendor.camprovider.enable_hidden", false);
...
) -> ::android::status_t
{
    ...
    for (size_t i = 0; i < mVirtEnumDeviceMap.size(); i++) {
        auto &deviceInfo = mVirtEnumDeviceMap.valueAt(i);
        if(!isHiddenCamera(deviceInfo->mInstanceId))
        {
            auto const& name = mVirtEnumDeviceMap.keyAt(i);
            rDeviceNames.push_back(name);
        } else {
            MY_LOGD("Camera %d is hidden", deviceInfo->mInstanceId);
        }
    }
    //
    return OK;
