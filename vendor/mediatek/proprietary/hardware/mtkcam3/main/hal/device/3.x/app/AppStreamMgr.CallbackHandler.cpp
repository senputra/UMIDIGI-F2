    ...
    , mResultMetadataQueue(pResultMetadataQueue)
{
    ...
    MY_LOGI("enableMetaMerge %d", bEnableMetaMerge);
}
...
performCallback() -> void
{
    ...
    std::vector<V3_4::CaptureResult> vBufferResult;
    ...
    //
    {
        ...
        for (auto const& cbParcel : mQueue2) {
            for (size_t i = 0; i < cbParcel.vError.size(); i++) {
                if (cbParcel.vError[i].errorCode != ErrorCode::ERROR_BUFFER){
                    ErroFrameNumbers.insert(cbParcel.frameNo);
                }
            }
        }
        //
        for (auto const& cbParcel : mQueue2) {
            CAM_ULOGM_DTAG_BEGIN(ATRACE_ENABLED(),
                                    "Dev-%d:convert|request:%d", mCommonInfo->mInstanceId, cbParcel.frameNo);
            ...
            if (ErroFrameNumbers.count(cbParcel.frameNo) == 0){
                if ( bEnableMetaMerge == 1 ){
                    convertMetaToHidlWithMergeEnabled(cbParcel, vCaptureResult, vTempMetadataResult);
                }
                else{
                ...
                MY_LOGI("Ignore Metadata callback because error already occured, frame(%d)/partialResult(%u)",
                        cbParcel.frameNo, cbParcel.vOutputMetaItem[i].bufferNo);
                }
            }
            //
        ...
        if  ( auto pResultMetadataQueue = mResultMetadataQueue ) {
            size_t startId = 0;
            for(size_t endId = 0; endId < vCaptureResult.size(); ++endId) {
                auto& item = vCaptureResult[endId];
                if  ( item.v3_2.result.size() == 0) {
                    continue;
                }
                bool bEnableCB = false, bFillFMQSuccess = false;
                CAM_ULOGM_DTAG_BEGIN(ATRACE_ENABLED(),
                                    "result metadata -> fmq|request:%d =fmqResultSize:%zu",
                                    item.v3_2.frameNumber, item.v3_2.result.size());
                if (item.v3_2.result.size() > CAMERA_RESULT_METADATA_QUEUE_SIZE)
                {
                    MY_LOGE("result(%zu) > FMQ total size(%zu), force write to aviod dead lock and must set larger fmq size", item.v3_2.result.size(), CAMERA_RESULT_METADATA_QUEUE_SIZE);
                    bEnableCB = true;
                }
                // check FMQ & can not callback
                while(!bFillFMQSuccess)
                {
                    if  ( pResultMetadataQueue->availableToWrite() > item.v3_2.result.size() )
                    {
                        ...
                        if(item.v3_2.partialResult == mCommonInfo->mAtMostMetaStreamCount)
                        {
                            for(size_t i = 0; i<item.physicalCameraMetadata.size() ;i++)
                            {
                                if( CC_LIKELY(pResultMetadataQueue->write(
                                                    item.physicalCameraMetadata[i].metadata.data(),
                                                    item.physicalCameraMetadata[i].metadata.size())) )
                                {
                                    item.physicalCameraMetadata[i].fmqMetadataSize = item.physicalCameraMetadata[i].metadata.size();
                                    item.physicalCameraMetadata[i].metadata = hidl_vec<uint8_t>();  // resize(0)
                                }
                                else
                                {
                                    item.physicalCameraMetadata[i].fmqMetadataSize = 0;
                                    MY_LOGW("fail to write to mResultMetadataQueue");
                                }
                            }
                        }
                        bFillFMQSuccess = true;
                        bEnableCB = (endId == vCaptureResult.size()-1);
                    }
                    else
                    {
                        MY_LOGI("available size(%zu) is less than result size(%zu), need enlarge FMQ size", pResultMetadataQueue->availableToWrite(), item.v3_2.result.size());
                        bFillFMQSuccess = false;
                        bEnableCB = (startId < endId);
                        if(!bEnableCB)
                        {
                            MY_LOGW("Can not callback, sleep 5ms for waiting FWK");
                            usleep(5000);
                        }
                    }
                    // callback meta
                    if(bEnableCB)
                    {
                        CAM_ULOGM_TAG_BEGIN("callback meta to FWK");
                        size_t cbSize = bFillFMQSuccess? endId - startId + 1 : endId - startId;
                        MY_LOGD_IF(getLogLevel() >= 1, "available FMQ size=%zu",pResultMetadataQueue->availableToWrite());
                        MY_LOGD_IF(getLogLevel() >= 1, "MetaCB : startId=%zu, size=%zu, total=%zu", startId, cbSize, vCaptureResult.size());
                        hidl_vec<V3_4::CaptureResult> vecCaptureResult;
                        vecCaptureResult.setToExternal(vCaptureResult.data()+startId, cbSize);
...
convertShutterToHidl(CallbackParcel const& cbParcel, std::vector<NotifyMsg>& rvMsg) -> void
{
    ...
    if  ( cbParcel.shutter != 0 )
    {
        MY_LOGI_IF(cbParcel.needIgnore,
                       "Ignore Shutter callback because error already occured, frame(%d)/timestamp(%" PRIu64 ")",
        ...
        if  ( !cbParcel.needIgnore )
        {
            CAM_ULOGM_DTAG_BEGIN(ATRACE_ENABLED(),
                                ...
                                mCommonInfo->mInstanceId, cbParcel.frameNo, cbParcel.shutter->timestamp, cbParcel.needIgnore);
            CAM_ULOGM_DTAG_END();
...
convertErrorToHidl(CallbackParcel const& cbParcel, std::vector<NotifyMsg>& rvMsg) -> void
{
    ...
    for (size_t i = 0; i < cbParcel.vError.size(); i++) {
        ...
        CAM_ULOGM_DTAG_BEGIN(ATRACE_ENABLED(),
                            ...
                            mCommonInfo->mInstanceId, cbParcel.frameNo, rError.errorCode, errorStreamId);
        CAM_ULOGM_DTAG_END();
...
convertMetaToHidl(
    ...
    std::vector<V3_4::CaptureResult>& rvResult,
        ...
        if  (!cbParcel.needIgnore) {
            CAM_ULOGM_DTAG_BEGIN(ATRACE_ENABLED(),
                                "Dev-%d:convertMeta|request:%d =partialResult:%d size:%zu",
                                    mCommonInfo->mInstanceId, cbParcel.frameNo, rCbMetaItem.bufferNo, camera_metadata_size);
            hidl_vec<uint8_t> temp;
            temp.setToExternal((uint8_t *)p_camera_metadata, camera_metadata_size);
            V3_4::CaptureResult res = V3_4::CaptureResult{
                .v3_2.frameNumber    = cbParcel.frameNo,
                .v3_2.fmqResultSize  = 0,
                .v3_2.result         = std::move(temp),
            ...
            //
            if ( res.v3_2.partialResult == mCommonInfo->mAtMostMetaStreamCount){
                convertPhysicMetaToHidl(cbParcel, res, rvResultMetadata);
            }
            //
...
auto
ThisNamespace::
convertMetaToHidlWithMergeEnabled(
    CallbackParcel const& cbParcel,
    std::vector<V3_4::CaptureResult>& rvResult,
    std::vector<camera_metadata*>& rvResultMetadata
) -> void
{
    ...
    if ( cbParcel.vOutputMetaItem.size() > 0 ){
        ...
        String8 log = String8::format("frame:%u, bufferNo:", cbParcel.frameNo);
        //Merge the metadata items
        {
            for (size_t i = 0; i < cbParcel.vOutputMetaItem.size(); i++) {
                ...
                MY_LOGI_IF(cbParcel.needIgnore,
                           "Ignore Metadata callback because error already occured, frame(%d)/partialResult(%u)",
                           cbParcel.frameNo, SrcMetaItem.bufferNo);
        ...
        size_t camera_metadata_size = 0;
        {
            ...
            {
            case 3:
                mCommonInfo->mMetadataConverter->dumpAll(resultMeta, cbParcel.frameNo);
                break;
            case 2:
                mCommonInfo->mMetadataConverter->dump(resultMeta, cbParcel.frameNo);
                break;
            default:
                break;
            }
        ...
        if  (!cbParcel.needIgnore) {
            CAM_ULOGM_DTAG_BEGIN(ATRACE_ENABLED(),
                                "Dev-%d:convertMeta|request:%d =partialResult:%zu size:%zu",
                                        mCommonInfo->mInstanceId, cbParcel.frameNo, resultBufNo, camera_metadata_size);
            hidl_vec<uint8_t> temp;
            temp.setToExternal((uint8_t *)p_camera_metadata, camera_metadata_size);
            V3_4::CaptureResult res = V3_4::CaptureResult{
                .v3_2.frameNumber    = cbParcel.frameNo,
                ...
                // .v3_2.outputBuffers  = hidl_vec<StreamBuffer>(),
                .v3_2.inputBuffer    = {.streamId = -1},     // force assign -1 indicating no input buffer
                .v3_2.partialResult  = (uint32_t)resultBufNo
            };
            //
            if ( res.v3_2.partialResult == mCommonInfo->mAtMostMetaStreamCount){
                convertPhysicMetaToHidl(cbParcel, res, rvResultMetadata);
            ...
            if (CC_UNLIKELY(getLogLevel() >= 1))
            {
                log += String8::format(", totalSize:%zu", res.v3_2.result.size());
                MY_LOGD("%s", log.c_str());
            }
            rvResult.push_back(res);
            CAM_ULOGM_DTAG_END();
        ...
        //
        updateStatistics_finalPartialMetadata(resultBufNo == mCommonInfo->mAtMostMetaStreamCount, cbParcel.frameNo);
    }
}
...
auto
ThisNamespace::
convertPhysicMetaToHidl(
    CallbackParcel const& cbParcel,
    V3_4::CaptureResult& rvResult,
    std::vector<camera_metadata*>& rvResultMetadata
) -> void
{
    ...
    for (size_t i = 0; i < cbParcel.vOutputPhysicMetaItem.size(); i++) {
        ...
        camera_metadata *metadata = nullptr;
        size_t metadata_size = 0;
        IMetadata* pPhysicMetadata = rCbPhysicMetaItem.buffer->tryReadLock(mInstanceName.c_str());
        {
            if (CC_UNLIKELY(
                ! mCommonInfo->mMetadataConverter->convert(*pPhysicMetadata, metadata, &metadata_size)
            ||  ! metadata
            ||  0 == metadata_size ))
            {
                MY_LOGE("fail: Physical IMetadata -> camera_metadata(%p) size:%zu", metadata, metadata_size);
            }
            rvResultMetadata.push_back(metadata);

            switch (getLogLevel())
            {
                case 3:
                    mCommonInfo->mMetadataConverter->dumpAll(*pPhysicMetadata, cbParcel.frameNo);
                    break;
                case 2:
                    mCommonInfo->mMetadataConverter->dump(*pPhysicMetadata, cbParcel.frameNo);
                    break;
                default:
                    break;
            }
        ...
        MINT32 deviceId = atoi(rCbPhysicMetaItem.camId.c_str());
        auto vid = MAKE_HalLogicalDeviceList()->getRemappingSensorId(deviceId);
        char vidStr[5];
        sprintf(vidStr,"%d",vid[0]);
...
convertImageToHidl(CallbackParcel const& cbParcel, std::vector<V3_4::CaptureResult>& rvResult) -> void
{
    ...
    auto convert = [=](auto const& rCbImageItem, auto& rStreamBuffer){
        ...
        CAM_ULOGM_DTAG_BEGIN(ATRACE_ENABLED(),
                            ...
                            mCommonInfo->mInstanceId, rCbImageItem.stream->getStreamId(), cbParcel.frameNo, toString(rStreamBuffer).c_str());
        CAM_ULOGM_DTAG_END();
    };
    //
...
updateStatistics_shutter(uint64_t timestamp, uint32_t const frameNo)
{
    if  (CC_UNLIKELY(timestamp < mStatistics.mLatestTimestamp_Shutter.load().timestamp) ) {
        ...
        MY_LOGI(
            "shutter:%" PRId64 " @f%u < shutter:%" PRId64 " @f%u, only available in zsl capture",
...
endStatistics()
{
    ...
    if  ( st - mStatistics.mStartTime_Output >= mStatistics.mMaxDuration_Output ) {
        ...
        //
        MY_LOGD_IF(1, "%s", strDebugInfo.c_str());
    }
}
