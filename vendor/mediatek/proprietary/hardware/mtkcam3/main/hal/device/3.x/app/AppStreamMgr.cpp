...
AppStreamMgr(const CreationInfo& creationInfo)
    ...
    , mStreamInfoBuilderFactory(std::make_shared<Camera3StreamInfoBuilderFactory>())
{
...
initialize()
{
    ...
    //
    {
        ...
        //
        mConfigHandler = new ConfigHandler(mCommonInfo, mFrameHandler, mBatchHandler, mResultMetadataQueue);
...
)   -> void
{
    auto prepareBuffer = [this](StreamBuffer const& buffer, auto& rvImageItem) {
        if  ( -1 != buffer.streamId && 0 != buffer.bufferId ) {
            ...
            if ( buffer.buffer!=nullptr ) {
                ...
                mRequestHandler->importStreamBuffer(buffer, bufferHandle, appBufferHandle, acquire_fence);
                //
                if (appBufferHandle == nullptr){
                    appBufferHandle = std::make_shared<AppStreamBufferHandle>(bufferHandle);
                }
                mFrameHandler->cacheBuffer(buffer.streamId, buffer.bufferId, bufferHandle, appBufferHandle);
                if(acquire_fence != -1){
                    close(acquire_fence);
                }
    ...
    for (auto const& req : requests) {
        ...
        //  cbParcel.vInputImageItem <- req.v3_2.inputBuffer
        prepareBuffer(req.v3_2.inputBuffer, cbParcel.vInputImageItem);

        //  cbParcel.vOutputImageItem <- req.v3_2.outputBuffers
        for (auto const& buffer : req.v3_2.outputBuffers) {
...
{
    String8 str = String8::format("User %#"  PRIxPTR " of frame %u, last partial:%d; ", params.userId, params.frameNo, params.hasLastPartial);
    //logical metadata
    const android::Vector<android::sp<IMetaStreamBuffer>>& streamBuf = params.resultMeta;
    for(size_t i = 0; i < streamBuf.size(); i++){
        IMetadata* pMetadata = streamBuf[i]->tryReadLock(mInstanceName.c_str());;
        str += String8::format("[Logical Meta(%zu/%zu): ", i, streamBuf.size());
        if(pMetadata != nullptr){
            str += String8::format("entry count=%u]; ", pMetadata->count());
            if ( mCommonInfo->mLogLevel>=3 )
                mCommonInfo->mMetadataConverter->dumpAll(*pMetadata, params.frameNo);
            streamBuf[i]->unlock(mInstanceName.c_str(), pMetadata);
        }
        else{
            str += String8::format("null in streamBuf(%s)]; ", streamBuf[i]->getName());
        }
    }
    //physical metadata
    for(size_t j = 0; j < params.physicalResult.size(); j++){
        ...
        str += String8::format("[Physical Meta(%zu/%zu) for physical Cam(%s): ",
                               j, params.physicalResult.size(), physicRes->physicalCameraId.c_str());
        if(pPhysicMetadata != nullptr){
            str += String8::format("entry count=%u]; ",pPhysicMetadata->count());
            // MY_LOGD("Physical Meta %d: size = %u", j, count);
            if ( mCommonInfo->mLogLevel>=3 )
                mCommonInfo->mMetadataConverter->dumpAll(*pPhysicMetadata, params.frameNo);
        }
        else{
            str += String8::format("null in streamBuf(%s)]; ",physicRes->physicalResultMeta->getName());
        }
        (physicRes->physicalResultMeta)->unlock(mInstanceName.c_str(), pPhysicMetadata);
    }
    //
    MY_LOGD_IF(mCommonInfo->mLogLevel>=1, "%s", str.string());
    //
