...
public class BluetoothPhoneServiceImpl {
                ...
                try {
                    ...
                    */
                    ...
                    if (account != null && account.getLabel() != null) {
                        ...
                        */
                        operator = account.getLabel().toString();
                        Log.i(TAG, "getNetworkOperator1 " + operator);
                        return operator;
                    } else {
                        ...
                        */
                        operator = TelephonyManager.from(mContext)
                                .getNetworkOperatorName();
                        Log.i(TAG, "getNetworkOperator2 " + operator);
                        return operator;
                    }
                } finally {
                    Binder.restoreCallingIdentity(token);
    ...
    public CallsManagerListener mCallsManagerListener = new CallsManagerListenerBase() {
        ...
        public void onCallStateChanged(Call call, int oldState, int newState) {
            ...
            /// M: ALPS04889890 cache the last conference call's index, the last participant will
            /// change to a normal call, old connection will disconnect, conference call changed
            /// to the last normal call, it will re-create index, this violate CLCC command spec.
            if (oldState == CallState.ACTIVE && newState == CallState.DISCONNECTED) {
                DisconnectCause cause = call.getDisconnectCause();
                if (cause != null
                        && DisconnectCause.REASON_EMULATING_SINGLE_CALL.equals(cause.getReason())) {
                    mEmulatingSingleCallIndex = getIndexForCall(call);
                    Log.d(TAG, "onCallStateChanged REASON_EMULATING_SINGLE_CALL, cache index: " +
                            mEmulatingSingleCallIndex);
                }
            }
            updateHeadsetWithCallState(false /* force */);
        }
        ...
        /// M: Handle ringing call after call is rejected.
        @Override
    ...
    /// M: cache index of REASON_EMULATING_SINGLE_CALL
    private int mEmulatingSingleCallIndex = 0;
    ...
    private boolean processChld(int chld) {
        ...
        if (chld == CHLD_TYPE_RELEASEHELD) {
            ...
            /**
             * M: Google AOSP code for CHLD_TYPE_RELEASEACTIVE_ACCEPTHELD:
    ...
    private void sendListOfCalls(boolean shouldLog) {
        /// M: Remove the conference call from mClccIndexMap if the conference call has children.
    ...
    private int getIndexForCall(Call call) {
        ...
        /// M: use mEmulatingSingleCallIndex
        if (mEmulatingSingleCallIndex > 0) {
            i = mEmulatingSingleCallIndex;
            mEmulatingSingleCallIndex = 0;
        }
    ...
    private void updateHeadsetWithCallState(boolean force) {
        ...
        if (mBluetoothHeadset != null &&
                (force ||
                        (!callsPendingSwitch &&
                                /// M: Check whether conference call has childre.
    ...
    private int getBtCallState(Call call, boolean isForeground) {
        switch (call.getState()) {
            ...
            case CallState.PULLING:
                ...
                /// M: Return ALERTING state when the call in alerting state, else return DIALING
                // state if dialingCall is not null. @{
    ...
    /**
     * M: Remove the conference call from mClccIndexMap if the conference call has children.
     * In conference call, there are 3 calls(one conference, two children), the setup flow
     * is add conference call first and then add two child calls, the index of child calls
     * will begin from 2 as index of conference call is 1 which will make some bluetooth
     * devices in confuse.
     */
    private void removeConferenceCallIfNeeded() {
        for (Call call : mClccIndexMap.keySet()) {
            if (call.isConference()
                    && !call.can(Connection.CAPABILITY_CONFERENCE_HAS_NO_CHILDREN)) {
                Log.i(TAG, "remove conference call from mClccIndexMap");
                mClccIndexMap.remove(call);
                return;
            }
        }
