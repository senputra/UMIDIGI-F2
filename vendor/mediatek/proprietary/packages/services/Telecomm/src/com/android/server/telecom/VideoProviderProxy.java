...
public class VideoProviderProxy extends Connection.VideoProvider {
    ...
    /// M: Modify video session flag.
    private boolean mIsModifyingSession = false;
    ...
    public final class VideoCallListenerBinder extends IVideoCallback.Stub {
        ...
        public void receiveSessionModifyRequest(VideoProfile videoProfile) {
            try {
                ...
                synchronized (mLock) {
                    /// M: Set modify video session flag.
                    mIsModifyingSession = true;
                    ...
                    if (((!mCall.isVideoCallingSupportedByPhoneAccount()
                            ...
                            || (MtkUtil.isInSingleVideoCallMode(mCall)
                            && ((mCall.getConnectionCapabilities()
                            & Connection.CAPABILITY_SUPPORTS_VT_LOCAL_TX) == 0))) {
        ...
        public void receiveSessionModifyResponse(int status, VideoProfile requestProfile,
            ...
            synchronized (mLock) {
                /// M: ALPS04920024/ALPS04899925 @{
                /// scenario1:
                /// 1.UE-A calls to UE-B and establish VoLTE call.
                /// 2.UE-A upgrade from VoLTE to ViLTE call.
                /// 3.UE-B reject upgrade from UE-A.
                /// Scenario2:
                /// 1.UE-A calls to UE-B and establish VoLTE call.
                /// 2.UE-A hold call
                /// 2.UE-A upgrade from VoLTE to ViLTE call, will fail
                /// Error: UE-A turn on speaker
                /// Solution: try to turn on speaker when receiving/sending modify response.
                if (!VideoProfile.isVideo(mCall.getVideoState())
                        && (requestProfile != null)
                        && VideoProfile.isVideo(requestProfile.getVideoState())
                        && (responseProfile != null)
                        && VideoProfile.isVideo(responseProfile.getVideoState())) {
                    mCall.maybeEnableSpeakerForVideoUpgrade(responseProfile.getVideoState());
    ...
    /**
     * M: Return modify video session flag value.
     *
     * @return Modifying video session or not.
     */
    boolean isModifyingVideoSession() {
        return mIsModifyingSession;
    }
}
