...
public class Ringer {
     ...
     */
    // M: Change to volatie, vibration start and stop may run in differenct thread.
    ...
    public boolean startRinging(Call foregroundCall, boolean isHfpDeviceAttached) {
        ...
        if (hapticsFuture != null) {
            mVibrateFuture =
                ...
                /// M: ALPS04424863, fix issue device always vibrate after end MT call. @{
                // This is a timing issue which maybe occured when stopRinging in the process of
                // executing startRinging.
                // Cause:
                // 1. In startRinging, vibrateFuture is waiting for hapticsFuture execute
                // mRingtonePlayer.play complete then it will start vibration and set mIsVibration
                // true.
                // 2. During this period, stopRinging is coming, because mIsVibration is
                // false, it will not stop vibration.
                // 3. After hapticsFuture complete, vibrateFuture start vibration, because
                // stopRinging is already finished, no chance to stop vibration.
                // Solution:
                // Adding judge mRingingCall is non-null before start vibration to avoid this case.
                if (mRingingCall != null && (!isUsingAudioCoupledHaptics
                        || !mIsHapticPlaybackSupportedByDevice)) {
                /// @}
    ...
    public void stopRinging() {
        ...
        CompletableFuture<Boolean> stopHapticsFuture  = mRingtonePlayer.stop();
        ...
        if (stopHapticsFuture != null) {
            stopHapticsFuture.thenAccept(v-> {
                Log.d(this, "stopRinging mIsVibrating = " + mIsVibrating);
                if (mIsVibrating) {
            ...
            Log.d(this, "stopRinging directly, mIsVibrating = " + mIsVibrating);
            if (mIsVibrating) {
                Log.addEvent(mVibratingCall, LogUtils.Events.STOP_VIBRATOR);
                mVibrator.cancel();
                mIsVibrating = false;
