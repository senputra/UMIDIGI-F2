...
public class CallForwardEditPreference extends EditPhoneNumberPreference implements
        MobileDataDialogInterface {
    ...
    public void onClick(DialogInterface dialog, int which) {
        ...
        if (which != DialogInterface.BUTTON_NEGATIVE) {
            ...
            if (carrierConfig.getBoolean(
                    CarrierConfigManager.KEY_CALL_FORWARDING_OVER_UT_WARNING_BOOL, true)
    ...
    private void handleCallForwardResult(CallForwardInfo cf) {
        ...
        /// M: Fixed JE issue when this function called but mPhone is null. @{
        if (mPhone != null && TextUtils.isEmpty(callForwardInfo.number)) {
        ...
        String voicemailNumber = (mPhone != null) ? mPhone.getVoiceMailNumber() : null;
        /// @}
        ...
        if (mTcpListener != null) {
            mTcpListener.onUpdated(CallForwardEditPreference.this);
        }
        /// @}
    }
    ...
    private class MyHandler extends Handler {
        ...
        private void handleGetCFResponse(Message msg) {
            ...
            if (ar.exception != null) {
                ...
                /// M: Check cfInfoArray is null before use it. @{
    ...
    /**
     * Call forward has UT error or not.
     * @return true if has UT error
     */
    ...
    private void resetUtError(AsyncResult ar) {
        ...
        if (mHasUtError && CallSettingUtils.isCtVolteMix()) {
    ...
    private void setCallForwardingOption(String number, int time, int action) {
        if (mPhone instanceof MtkGsmCdmaPhone) {
            Log.d(LOG_TAG, "setCallForwardingOption mServiceClass: " + mServiceClass);
    ...
    private void handleCommandException(CommandException exception) {
        if (mHasUtError) {
            ...
            if (mEnableVolteTips && imsManager != null
        ...
        } else if (exception.getCommandError() ==
    ...
    /**
     * Do action when user click data traffic dialog's positive button.
     * @param number the call forwarding number
     * @param time the call forwarding time
     * @param action the call forwarding action
     */
    public void doAction(String number, int time, int action) {
        // set to network
        Log.d(LOG_TAG, "reason=" + reason + ", action=" + action);
    ...
    /**
     * Do action when user click data traffic dialog's negative button.
     */
    public void doCancel() {
        Log.d(LOG_TAG, "doCancel callForwardInfo: " + callForwardInfo);
