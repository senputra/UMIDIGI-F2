...
public class OtaspActivationService extends Service {
    ...
    private void onStartOtaspCall() {
        ...
        /// M: Get  otasp call number by different operator. @{
        String number = getOtaspCallNumber();
        if (TextUtils.isEmpty(number)) {
            number = OTASP_NUMBER;
        }
        if (number.equals(USCC_OTASP_NUMBER)) {
            ServiceState ss = mPhone.getServiceState();
            if (ss != null) {
                int type = ss.getDataNetworkType();
                logd("getDataNetworkType: " + type);
                //if (type != TelephonyManager.NETWORK_TYPE_1xRTT) {
                    // Only dial otasp call when ps network type is 1xRTT for USCC.
                    //onComplete();
                    //return;
                //}
            }
        }
        logd("startNonInteractiveOtasp: placing call to '" + number + "'...");
        ...
        if (mPhone.getState().equals(PhoneConstants.State.IDLE)) {
            ...
            if (mIsOtaspCallCommitted) {
                ...
                Toast.makeText(context,
                        context.getString(R.string.ota_title_activate_success),
                ...
                /// M: Otasp call resault @{
                Toast.makeText(context,
                        context.getString(R.string.ota_title_problem_with_activation),
                        Toast.LENGTH_LONG).show();
                /// @}
            }
            onComplete();
    ...
    // -------------------------------MTK-----------------------------
    private static final String USCC_OTASP_NUMBER = "*228";

    private String getOtaspCallNumber() {
        String number = "";
        final Context context = mPhone.getContext();
        if (context != null) {
            ...
            if (b != null) {
                number = b.getString(MtkCarrierConfigManager.MTK_KEY_OTASP_CALL_NUMBER_STRING);
            }
        }
        logd("getOtaspCallNumber: " + number);
        return number;
    }
}
