...
class AdapterProperties {
    ...
    private void sendConnectionStateChange(int profile, Intent connIntent) {
        ...
        if (!isNormalStateTransition(prevState, state)) {
            ...
            if (profile == BluetoothProfile.HEADSET &&
                ...
                Log.d(TAG, "PROFILE_CONNECTION_STATE_CHANGE: Only " +
                        "handle 2-->0 except hfp has connected before.");
                return;
            }
        }
    ...
    void sendConnectionStateChange(BluetoothDevice device, int profile, int state, int prevState) {
        ...
        /// M: Check adapter state before update connection state
        if(mState == BluetoothAdapter.STATE_BLE_ON
                || mState == BluetoothAdapter.STATE_BLE_TURNING_OFF
                || mState == BluetoothAdapter.STATE_OFF) {
            debugLog("Do not update connection state after turn off to ble on");
            return;
        }
    ...
    void onBluetoothReady() {
        ...
        synchronized (mObject) {
            ...
            if (mProfilesConnected == 0 && mProfilesConnecting == 0 && mProfilesDisconnecting == 0) {
    ...
    void onBluetoothDisableDone() {
        /// M: When Bluetooth is disabled to BLE_ON state,
        /// reset adapter and profile connection states
        debugLog("Reset adapter and profile connection states.");
        setConnectionState(BluetoothAdapter.STATE_DISCONNECTED);
        mProfileConnectionState.clear();
        mProfilesConnected = 0;
        mProfilesConnecting = 0;
