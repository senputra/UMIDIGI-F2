...
public class MediaPlayerList {
    ...
    void getPlayerRoot(int playerId, GetPlayerRootCallback cb) {
        ...
        if (mBrowsablePlayers.containsKey(playerId)) {
            ...
            BrowsedPlayerWrapper wrapper = mBrowsablePlayers.get(playerId);
            ...
            wrapper.getFolderItems(itemId, (status, id, results) -> {
                ...
                cb.run(playerId, true, "", results.size());
            });
        } else
        cb.run(playerId, false, "", 0);
    }
    ...
    List<PlayerInfo> getMediaPlayerList() {
        ...
        info.id = getCurrentPlayerId();
        if (player != null)
            info.name = Util.getDisplayName(mContext, player.getPackageName());
        else
            return null;
        info.browsable = mBrowsablePlayers.containsKey(getCurrentPlayerId());
    ...
    String getCurrentMediaId() {
        ...
        /** M: Fix PTS AVRCP/TG/MCN/CB/BV-04-I fail @{ */
        if (Utils.isPtsTestMode() && state.getState() != PlaybackState.STATE_PLAYING) {
            d("PTS test mode: Sending empty mediaId: PlaybackState=" + state);
            return "";
        }
        /** @} */
    ...
    private void playFolderItem(String mediaId) {
        ...
        if (!mBrowsablePlayers.containsKey(getCurrentPlayerId())) {
            e("playFolderItem: Do not have the a browsable player");
            return;
        }
        BrowsedPlayerWrapper player = mBrowsablePlayers.get(getCurrentPlayerId());
        if (player != null)
    ...
    int addMediaPlayer(android.media.session.MediaController controller) {
        ...
        /** M: Skip adding telecom to the media player list @{ */
    ...
    void setActivePlayer(int playerId) {
        /** M: Add synchronized to fix JE issue @{ */
    ...
    private final BroadcastReceiver mPackageChangedBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_PACKAGE_REMOVED)
                ...
                if (packageName != null && mMediaPlayerIds!= null
                        && mMediaPlayerIds.containsKey(packageName)) {
    ...
    private final MediaPlayerWrapper.Callback mMediaPlayerCallback =
        ...
        @Override
        public void sessionDestroyed(String packageName) {
            if (packageName != null && mMediaPlayerIds.containsKey(packageName)) {
                Log.d(TAG, "mediaUpdatedCallback(): sessionDestroyed," + packageName + " removed");
                removeMediaPlayer(mMediaPlayerIds.get(packageName));
            }
        }
    };
            ...
            new MediaSessionManager.Callback() {
                ...
                public void onAddressedPlayerChanged(MediaSession.Token token) {
                    ...
                    if (!mMediaPlayerIds.containsKey(controller.getPackageName())) {
                        ...
                        /** M: When adding media player fail, return to avoid JE @{ */
                        if (addMediaPlayer(controller) == -1) {
                            Log.d(TAG, "Fail to add media player: " + controller.getPackageName());
                            return;
                        }
                        /** @} */
                    }
                    ...
                    /** M: Return to fix JE, when mMediaPlayerIds is null @{ */
                    if (mMediaPlayerIds == null) return;
                    /** @} */
                ...
                public void onAddressedPlayerChanged(ComponentName receiver) {
                    ...
                    if (mMediaPlayerIds == null || !mMediaPlayerIds.containsKey(receiver.getPackageName())) {
