...
public class GattService extends ProfileService {
    ...
    /** M: ALPS04703937 ClientMap clear before onDisconnected @{ */
    void updateClientState() {
        if (DBG) Log.d(TAG, "updateClientState()");
        for (Integer appId : mClientMap.getAllAppsIds()) {
            if (DBG) Log.d(TAG, "updateClientState:" + appId);
            Map<Integer, String> connMap = mClientMap.getConnectedMap();
            if(connMap != null && connMap.containsKey(appId)) {
                ClientMap.App app = mClientMap.getById(appId);
                if (app != null) {
                    String address = connMap.get(appId);
                    Log.d(TAG, "updateClientState: address="+address);
                    try {
                        app.callback.onClientConnectionState(BluetoothGatt.GATT_SUCCESS, appId, false, address);
                    } catch (RemoteException e) {
                        Log.e(TAG, "updateClientState RemoteException");
                    }
                }
                else {
                    Log.w(TAG, "updateClientState: no client app");
                }
            }
            else {
                Log.w(TAG, "updateClientState: no connected map");
            }
        }
    }
    /** @} */
    ...
    private boolean permissionCheck(UUID characteristicUuid) {
        ...
        boolean bIsHidUUID = isHidCharUuid(characteristicUuid);
        boolean bPrivileged = (checkCallingOrSelfPermission(BLUETOOTH_PRIVILEGED)
                        == PERMISSION_GRANTED);
        if (DBG && !(!bIsHidUUID || bPrivileged)) {
            Log.e(TAG, "permissionCheck(): UUID=" + characteristicUuid +
                ";permission: " + checkCallingOrSelfPermission(BLUETOOTH_PRIVILEGED));
        }
        return !bIsHidUUID || bPrivileged;
    }
    ...
    private boolean permissionCheck(int connId, int handle) {
        ...
        boolean bPrivileged = (checkCallingOrSelfPermission(BLUETOOTH_PRIVILEGED)
                == PERMISSION_GRANTED);
        if (DBG && !bPrivileged) {
            Log.e(TAG, "permissionCheck(): connId=" + connId +
                ";handle=" + handle + ";permission: " +
    ...
    private boolean permissionCheck(ClientMap.App app, int connId, int handle) {
        ...
        /** M: Enable debugging log for sqc test mode @{ */
        if (DBG && !app.hasBluetoothPrivilegedPermission) {
            Log.e(TAG, "permissionCheck(): app name= " + app.name +
                 ";connId=" + connId + ";handle=" + handle + ";permission: " +
                checkCallingOrSelfPermission(BLUETOOTH_PRIVILEGED));
        }
    ...
    void startScan(int scannerId, ScanSettings settings, List<ScanFilter> filters,
        ...
        if (DBG) {
            Log.d(TAG, "start scan with filters - scannerId=" + scannerId + ";callingPackage=" + callingPackage);
        }
    ...
    void registerPiAndStartScan(PendingIntent pendingIntent, ScanSettings settings,
        ...
        if (DBG) {
            Log.d(TAG, "start scan with filters, for PendingIntent - callingPackage=" + callingPackage);
        }
        enforceAdminPermission();
        if (needsPrivilegedPermissionForScan(settings)) {
            enforcePrivilegedPermission();
    ...
    /** M: ALPS04670323 Disconnect all Gatt connections @{ */
    ...
    void unregAll() {
        if (DBG) Log.d(TAG, "unregAll");
        /** M: ALPS04703937 ClientMap clear before onDisconnected @{ */
        updateClientState();
        /** @} */
        for (Integer appId : mClientMap.getAllAppsIds()) {
            if (DBG) Log.d(TAG, "ClientMap unreg:" + appId);
            unregisterClient(appId);
        }
        /** M: ALPS04670323 Disconnect all Gatt connections @{ */
        for (Integer appId : mServerMap.getAllAppsIds()) {
            if (DBG) Log.d(TAG, "ServerMap unreg:" + appId);
            unregisterServer(appId);
        }
        for (Integer appId : mScannerMap.getAllAppsIds()) {
            if (DBG) Log.d(TAG, "ScannerMap unreg:" + appId);
