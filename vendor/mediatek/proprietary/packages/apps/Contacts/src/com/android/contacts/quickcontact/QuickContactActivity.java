...
public class QuickContactActivity extends ContactsActivity {
    ...
    /// M: [Op01 Plugin]Video Entry @{
    // Used in AOSP static method dataItemToEntry, must be static
    private static IOp01Extension sOp01Extension;
    /// @}
    ...
    /// M:[Google Issue][ALPS03439245][ALPS03428584] 1/4 @{
    ...
    final OnClickListener mEntryClickHandler = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            /// M: [OP12] check video icon clickable. @{
            if (Intent.ACTION_CALL.equals(intent.getAction())
                    && ActionType.VIDEOCALL == intent.getIntExtra(EXTRA_ACTION_TYPE, -1)) {
                if (!ExtensionManager.getInstance().getContactsCommonPresenceExtension().
                        isVideoIconClickable(intent.getData())) {
                    return;
                }
            }
            /// @}
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.sensitive(TAG, "[onCreate()] bundle = " + savedInstanceState);

        if (RequestPermissionsActivity.startPermissionActivityIfNeeded(this)) {
            return;
        }
        /// M:[Sim Contact Flow]][ALPS04213859] User maybe jump to this activity directly
        /// from other app(Dialer). Contacts have no chance to initiate phbinfo and fail to diplay
        /// all columne in the editor activity. So try to init phbinfo here to avoid this case.@{
    ...
    private void processIntent(Intent intent) {
        ...
        Log.sensitive(TAG, "The original uri from intent: " + lookupUri);
    ...
    private void bindContactData(final Contact data) {
        ...
        mEntriesAndActionsTask = new AsyncTask<Void, Void, Cp2DataCardModel>() {
            ...
            protected Cp2DataCardModel doInBackground(
                    Void... params) {
                Log.d(TAG, "[Cp2DataCardModel] doInBackground(). " + mEntriesAndActionsTask);
            ...
            protected void onPostExecute(Cp2DataCardModel cardDataModel) {
                ...
                Log.d(TAG, "[Cp2DataCardModel] onPostExecute(). " + mEntriesAndActionsTask);
                ...
                /// M: clear mEntriesAndActinosTask after excute done @{
        ...
        Log.d(TAG, "[bindContactData]mEntriesAndActionsTask.execute()." + mEntriesAndActionsTask);
    }
    ...
    protected void onResume() {
        Log.w(TAG, "[onResume] beg: mHasIntentLaunched = " + mHasIntentLaunched
                + ", mCachedCp2DataCardModel = " + mCachedCp2DataCardModel);
        super.onResume();
        /// M:[Google Issue][ALPS03439245][ALPS03428584] 2/4 @{
        mIsResumed = true;
        /// @}
        ...
        ///@}
        /// M: [OP12] add refresh menu for Op12 RCS request. @{
        ExtensionManager.getInstance().getContactsCommonPresenceExtension().
                onHostActivityResumed(this);
        /// @}
    ...
    protected void onPause() {
        ...
        super.onPause();
        dismissProgressBar();
        /// M:[Google Issue][ALPS03439245][ALPS03428584] 3/4 @{
        mIsResumed = false;
        /// @}
        /// M: [OP12] add refresh menu for Op12 RCS request. @{
        ExtensionManager.getInstance().getContactsCommonPresenceExtension().
    ...
    private void populateContactAndAboutCard(Cp2DataCardModel cp2DataCardModel,
        ...
        /* M: [Google Issue]ALPS01775443, after deleted name in editor,
         * need reset title. @{
        ...
        /* M: [Google Issue]ALPS01763309, after deleted all about card informations
    ...
    private void initializeNoContactDetailCard(boolean areAllRawContactsSimAccounts) {
        Log.d(TAG, "[initializeNoContactDetailCard]"
                + "areAllRawContactsSimAccounts is" + areAllRawContactsSimAccounts);
        ...
        /* M: [Sim Contact Flow]ALPS02782438. not support to show email
         * for sim and Ruim type. 1/2
         * Original code: @{
        if (!areAllRawContactsSimAccounts) {
         * @}
         * New code: @{ */
        if (QuickContactUtils.isSupportShowEmailData(mContactData,
        ...
        mNoContactDetailsCard.initialize(promptEntries,
                /* M: [Sim Contact Flow]ALPS02782438. 2/2
                 * Orignal code: @{
                 * 2
                 * @}
                 * New code: @{ */
    ...
    private Cp2DataCardModel generateDataModelFromContact(
        ...
        Log.sensitive(TAG, "[generateDataModelFromContact] start contact: " + data);
        ...
        Log.sensitive(TAG, "[generateDataModelFromContact] end contact: " + data +
                ", areAllRawContactsSimAccounts : " + dataModel.areAllRawContactsSimAccounts);
        return dataModel;
    }
    ...
    private static Entry dataItemToEntry(DataItem dataItem, DataItem secondDataItem,
        ...
        Log.sensitive(TAG, "[dataItemToEntry] contact:" + contactData
                + " dataItem:" + dataItem.getClass());
            ...
            if (!TextUtils.isEmpty(phone.getNumber())) {
                ...
                if (phone.hasKindTypeColumn(kind)) {
                    ...
                    if (kindTypeColumn == Phone.TYPE_CUSTOM && TextUtils.isEmpty(label)) {
                        ...
                        /// M: [Sim Contact Flow][AAS] show Primary/Additional Number @{
                        int subId = contactData.getIndicate();
                        subHeader = GlobalEnv.getSimAasEditor().getSubheaderString(
                                subId, dataItem.getContentValues().getAsInteger(Data.DATA2));
                        text = (String) GlobalEnv.getSimAasEditor().getTypeLabel(
                ...
                /// M: [Google Issue] to enable Sms dynamically.@{
                ...
                /// M: [Op01 Plugin]Video Entry @{
                if (sOp01Extension == null) {
                    // Should reach here only one time
                    sOp01Extension = ExtensionManager.getInstance().getOp01Extension();
                }

                if (sOp01Extension != null) {
                    isVideoEnabled = sOp01Extension.isVideoButtonEnabled(isVideoEnabled,
                            contactData.getLookupUri(), context);
                }
                ///@}
                ...
                Log.d(TAG, "[dataItemToEntry] videoCapability = " + videoCapability
                        + ", after Op01Extension, isVideoEnabled = " + isVideoEnabled
                        + ", carrierPresence = " + carrierPresence
                        + ", isPresent = " + isPresent
                        + ", CommonPresenceExtension().isShowVideoIcon() = "
                        + ExtensionManager.getInstance().getContactsCommonPresenceExtension().isShowVideoIcon());
                ...
                ///M: add presence: @{
                ExtensionManager.getInstance().getContactsCommonPresenceExtension().
                    setVideoIconAlpha(phone.getNumber(), thirdIcon, isVideoEnabled);
                /// @}
            }
        ...
        /// M: add group title @{
            ...
            if (!TextUtils.isEmpty(groupTitle)) {
                header = res.getString(R.string.groupsLabel);
    ...
    private void extractAndApplyTintFromPhotoViewAsynchronously() {
        ...
        new AsyncTask<Void, Void, MaterialPalette>() {
            ...
            protected MaterialPalette doInBackground(Void... params) {
                Log.d(TAG, "[extractAndApplyTintFromPhotoViewAsynchronously] doInBackground()."
                        + this);
            ...
            protected void onPostExecute(MaterialPalette palette) {
                ...
                /// @}
                Log.d(TAG, "[extractAndApplyTintFromPhotoViewAsynchronously]"
                        + " onPostExecute()" + this
                        + ", mHasComputedThemeColor = " + mHasComputedThemeColor
                        + ", mColorFilterColor = " + mColorFilterColor
                        + ", palette.mPrimaryColor = " + palette.mPrimaryColor);
                /* M: [Google Issue] ALPS03824392
                 * Should update the color filter (in setThemeColor()) if contact
                 * photo updated in background, otherwise the old color filter may
                 * causes the new photo displays wrong color, e.g. blank.
                 * AOSP code:  @{
                 * if (mHasComputedThemeColor) {
                 * @} New code: @{*/
                if (mHasComputedThemeColor && (mColorFilterColor == palette.mPrimaryColor)) {
                /* @} */
        ...
        Log.d(TAG, "[extractAndApplyTintFromPhotoViewAsynchronously] execute(). " + this);
    }

    private void setThemeColor(MaterialPalette palette) {
        Log.d(TAG, "[setThemeColor]");
    ...
    protected void onStop() {
        ...
        Log.w(TAG, "[onStop]mEntriesAndActionsTask=" + mEntriesAndActionsTask);
    ...
    public void onDestroy() {
        ...
        /// M: [Op01 Plugin]Video Entry @{
        if (sOp01Extension != null) {
            sOp01Extension.resetVideoState();
            sOp01Extension = null;
        }
        ///@}
    }
    ...
    private boolean isContactEditable() {
        /* M: [SDN] sdn contact isn't possible to edit
         * Original Code: @{
    ...
    private void createLauncherShortcutWithContact() {
        /// M:[Google Issue][ALPS03439245][ALPS03428584][ALPS04170129]
        /// Ignore if activity already exit or not resumed. 4/4 @{
        if (!mIsResumed) {
            Log.d(TAG, "[onShortcutIntentCreated]ignore! mIsResumed = " + mIsResumed);
            return;
        }
        /// @}
    ...
    private boolean isShortcutCreatable() {
        ...
        /// M: [Sim Contact Flow] hide shortcut menu if sim contact. @{
        if (mContactData.getIndicate() >= 0) {
            Log.d(TAG, "[isShortcutCreatable()] contact indicator: " +
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (mContactData != null) {
            ...
            /// M: [SDN] hide edit menu if sdn contact. @{
            ...
            /* M: [Sim Contact Flow] hide star, join, shortcut, ringTone,
             * sendToVoiceMail menu item if sim contact. @{ */
            if (mContactData.getIndicate() >= 0) {
                ...
                sendToVoiceMailMenuItem.setVisible(false);
            }
            /* @} */
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        /// M: [OP12] add refresh menu for Op12 RCS request. @{
        if (mContactData != null && ExtensionManager.getInstance().
                getContactsCommonPresenceExtension().onOptionsItemSelected(
                        id, mContactData.getContactId())) {
            return true;
        }
        ///
