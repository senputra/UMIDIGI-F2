...
public class ViewGnss extends View {
    ...
    public void setGnssStatus(GnssStatus gnssStatus) {
        ...
        for (i=0; i < gnssStatus.getSatelliteCount(); i++) {
            if (i >= MAX_SATELLITE_NUM) {
                log("ERR: the number of satellite is over " + MAX_SATELLITE_NUM);
                break;
            }
            SvData svData = new SvData();
            svData.svid = gnssStatus.getSvid(i);
            svData.snr = gnssStatus.getCn0DbHz(i);
            svData.ele = gnssStatus.getElevationDegrees(i);
            svData.azi = gnssStatus.getAzimuthDegrees(i);
            svData.used = gnssStatus.usedInFix(i);
            svData.type = gnssStatus.getConstellationType(i);
            svData.freq = gnssStatus.getCarrierFrequencyHz(i);
            svData.color = getSvColor(svData.type);
    ...
    private int getSvColor(int constellationType) {
    ...
    public void resetGnssView() {
        mSatInView = 0;
        invalidate();
    }

    public ViewGnss(Context context) {
        this(context, null, 0);
    }
    public ViewGnss(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }
    public ViewGnss(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void drawGpsView(Canvas c, float left, float top, float right,
            float bottom) {
    ...
    private void drawSignals(Canvas c, float left, float top, float right, float bottom) {
        ...
        float sh = (h - 2*mTextSize) / 50; // signal height per CN
        float by = bottom - 2*mTextSize; // base Y
        ...
        for (int i = 0; i < satInView; i++) {
            ...
            String svid = "" + svData.svid;
            String svType = LbsUtility.map2SvTypes(svData.type);
            String freq = "" + svType + LbsUtility.getFrequencyBand(svData.freq);
            float cn = svData.snr;
            boolean inUsed = svData.used;
            mColoredText.setColor(svData.color);
            float bx = i * sw + sw / 2;
            ...
            // draw freq
            drawTextInCenter(c, freq, bx, by + mTextSize + mTextSize / 2, mText);
