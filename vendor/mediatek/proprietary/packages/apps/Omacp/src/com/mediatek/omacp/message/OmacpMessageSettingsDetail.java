...
import com.mediatek.omacp.parser.ImsClass;
...
public class OmacpMessageSettingsDetail extends Activity implements OnClickListener {
    ...
    private static final boolean DEBUG = MTKlog.MORE_LOGS_ENABLE;

    private static final String[] INSTALLATION_PROJECTION = {
            OmacpProviderDatabase._ID, OmacpProviderDatabase.SUB_ID,
    ...
    private static final String APP_SETTING_PROTECT_RESULT_ACTION =
                                 "com.mediatek.omacp.protected.settings.result";

    private final BroadcastReceiver mPhoneStateChangeReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            boolean isInCall = TelecomManager.from(OmacpMessageSettingsDetail.this).isInCall();
            MTKlog.i(TAG, "OmacpMessageSettingsDetail phone state changed isInCall = " + isInCall);
            if (mFullInstallBtn != null) {
                mFullInstallBtn.setEnabled(!isInCall);
            }
            if (mCustomInstallBtn != null) {
                mCustomInstallBtn.setEnabled(!isInCall);
            }
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        MTKlog.d(TAG, "OmacpMessageSettingsDetail onCreate !!");
        ...
        boolean isInCall = TelecomManager.from(this).isInCall();
        mFullInstallBtn.setEnabled(!isInCall);
        mCustomInstallBtn.setEnabled(!isInCall);
        }
        ...
        registerReceiver(mResultReceiver,
                        new IntentFilter(APP_SETTING_RESULT_ACTION),
                        "com.mediatek.permission.omacp.install",
                        null);
        registerReceiver(mResultReceiver,
                        new IntentFilter(APP_SETTING_PROTECT_RESULT_ACTION),
                        "com.mediatek.permission.omacp.install",
                        null);
        registerReceiver(mPhoneStateChangeReceiver, new IntentFilter(TelephonyManager.ACTION_PHONE_STATE_CHANGED));
        setProgressBarIndeterminateVisibility(false);
        MTKlog.d(TAG, "OmacpMessageSettingsDetail register phone state listener !!");
    }
    ...
    protected void onDestroy() {
        ...
        unregisterReceiver(mPhoneStateChangeReceiver);
                ...
                .setPositiveButton(R.string.yes, new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        if (!isPinCorrect(inputPin.getBytes())) {
                            ...
                            Toast.makeText(OmacpMessageSettingsDetail.this,
    ...
    private boolean isPinCorrect(byte[] pin) {
        ...
        if (sSec == USERNETWPIN) {
            String imsi = getSimImsi(this, sSubId);
    ...
    /** Gets IMSI from telephony.
     * @param context context
     * @param subId subId
     * @return String
     */
    public static String getSimImsi(Context context, int subId) {
        String imsi;
        ...
        try {
            ...
            imsi = null;
            MTKlog.d(TAG, "OmacpMessageSettingsDetail getSimImsi, Exception" + e);
        }
        if (DEBUG) {
    ...
    private void initActivityState(Intent intent) {
        ...
        if (intentData != null) {
            ...
            if (DEBUG) {
                ...
                MTKlog.d(TAG, "OmacpMessageSettingsDetail mImsApp is : " + mImsApp);
            }
            ...
            SpannableStringBuilder settingsDetailInfo = OmacpMessageUtils.getSettingsDetailInfo(
                    this, mApList, mNapList, mPxList, mImsApp);
    ...
    private void sendIntentsToApn() {
        ...
        if (!apnIntentList.isEmpty()) {
            ...
            this.sendBroadcast(intent, "com.mediatek.permission.omacp.install");
        ...
        for (int d = 0; d < mApList.size(); d++) {
            if (mApList.get(d).mAppid.equalsIgnoreCase(OmacpMessageUtils.MMS_APPID)
                ...
                if (proxyClass != null && proxyClass.mPxphysical != null
                    ...
                    if (DEBUG) {
                        MTKlog.i(TAG, "apn MMS PROXY is : "
                                       + proxyClass.mPxphysical.get(0).mPxaddr);
                    }
    ...
    private void sendIntentsToApplications() {
        ...
        if (mImsApp != null && mImsApp.mAppid != null) {
            int index = mApSettingsListName.indexOf(OmacpMessageUtils.getAppName(this,
                    mImsApp.mAppid));
            if ((!sIsFullInstallation && mApSettingsListNameChecked.get(index))
                    || sIsFullInstallation) {
                //configureImsParams();
                sendIntentForImsConfiguration();
            }
        }
    ...
    private BroadcastReceiver mResultReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(APP_SETTING_RESULT_ACTION)
                || intent.getAction().equals(APP_SETTING_PROTECT_RESULT_ACTION)) {
                ...
                if (appId != null && appId.equalsIgnoreCase(OmacpMessageUtils.APN_APPID)) {
                    if (mApnResultObj == null) {
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions,
        ...
        if (requestCode == REQUEST_READ_PHONE_STATE) {
            // Received permission result for phone state read permission.
            MTKlog.i(TAG, "Received response for phonestate");
            // Check if the only required permission has been granted
            if (grantResults.length == 1 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                boolean isInCall = TelecomManager.from(OmacpMessageSettingsDetail.this).isInCall();
                MTKlog.i(TAG, "phone state changed isInCall = " + isInCall);
                if (mFullInstallBtn != null) {
                    mFullInstallBtn.setEnabled(!isInCall);
                }
                if (mCustomInstallBtn != null) {
                ...
                Toast.makeText(this, "Permission Denied. You can change"+
                                "it in Settings->Apps", Toast.LENGTH_SHORT).show();
                finish();
            }
        }
    }

    private void sendIntentForImsConfiguration() {
        Intent intent = new Intent();
        intent.setAction(APP_SETTING_ACTION);
        String mimeType = "application/com.mediatek.omacp-" + mImsApp.mAppid;
        intent.setType(mimeType);
        intent.putExtra(APP_ID_KEY, mImsApp.mAppid);
        intent.putExtra("context", sContextIdentifier); // currently not
                                                        // used
        intent.putExtra("subId", sSubId);
        intent.putExtra("epdg-fqdn", mImsApp.mFqdn);

        ResultType result = new ResultType(mImsApp.mAppid, RESULT_CONSTANT_NOT_RETURNED);
        if (DEBUG) {
            MTKlog.d(TAG, "sendIntentForImsConfiguration, intent: " + intent);
            MTKlog.d(TAG, "subId: " + sSubId + " fqdn:" + mImsApp.mFqdn);
        }
        intent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
        this.sendBroadcast(intent, "com.mediatek.permission.omacp.install");
        mApplicationResultList.add(result);
