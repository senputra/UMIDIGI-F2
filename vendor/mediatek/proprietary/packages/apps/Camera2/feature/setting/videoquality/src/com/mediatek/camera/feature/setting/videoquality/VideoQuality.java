...
public class VideoQuality extends SettingBase implements
    ...
    public IParametersConfigure getParametersConfigure() {
        ...
        if (mSettingChangeRequester == null) {
            parametersConfig = new VideoQualityParametersConfig(
                    this, mSettingDeviceRequester);
    ...
    private void updateRestriction(String value){
        VideoQualityCaptureRequestConfig videoQualityCaptureRequestConfig
                = (VideoQualityCaptureRequestConfig)mSettingChangeRequester;
        String eisValue = mSettingController.queryValue("key_eis");
        String fps60Value = mSettingController.queryValue("key_fps60");
        boolean fps60Support = videoQualityCaptureRequestConfig.isFPS60Support(value);
        boolean eisSupport = videoQualityCaptureRequestConfig.isSupportEIS(value);
        if(fps60Support){
            if(eisSupport){
                LogHelper.d(TAG, "[updateRestriction], 60fps = on , eis = on" );
                mSettingController.postRestriction(
                        VideoQualityRestriction.getFpsOnEisOn(value,eisValue,fps60Value).getRelation(value, true));
            }else {
                LogHelper.d(TAG, "[updateRestriction], 60fps = on , eis = off" );
                mSettingController.postRestriction(
                        VideoQualityRestriction.getFpsOnEisOff(value,fps60Value).getRelation(value, true));
            }
        }else {
            if(eisSupport){
                LogHelper.d(TAG, "[updateRestriction], 60fps = off , eis = on" );
                mSettingController.postRestriction(
                        VideoQualityRestriction.getFpsOffEisOn(value,eisValue).getRelation(value, true));
            }else {
                LogHelper.d(TAG, "[updateRestriction], 60fps = off , eis = off" );
                mSettingController.postRestriction(
                        VideoQualityRestriction.getFpsOffEisOff(value).getRelation(value, true));
            }
        }
