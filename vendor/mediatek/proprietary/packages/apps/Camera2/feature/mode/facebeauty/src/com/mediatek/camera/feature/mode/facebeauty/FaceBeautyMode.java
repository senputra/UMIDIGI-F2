...
public class FaceBeautyMode extends CameraModeBase
        implements IFaceBeautyDeviceController.CaptureDataCallback,
        IFaceBeautyDeviceController.DeviceCallback, IFaceBeautyDeviceController.PreviewSizeCallback,
    ...
    public void init(@Nonnull IApp app, @Nonnull ICameraContext cameraContext,
        ...
        mIDeviceController = new FaceBeautyDeviceController(app.getActivity(), mICameraContext);
        ...
        mMainHandler = new FaceBeautyHandler(mIApp.getActivity().getMainLooper());
        mFaceBeautyView = new FaceBeautyView(app, Integer.valueOf(mCameraId), mICameraContext);
    ...
    public void onDataReceived(IFaceBeautyDeviceController.DataCallbackInfo dataCallbackInfo) {
        ...
        //performance.
        byte[] results = addExifToImageBuffer(data,format);
        if (data != null) {
    ...
    @Override
    public void onStillCaptureResultDelivered(CaptureResult stillCaptureResult) {
        mCaptureResult = stillCaptureResult;
    }
    ...
    private void prepareAndOpenCamera(boolean needOpenCameraSync, String cameraId,
        ...
        //prepare device info.
        FaceBeautyDeviceInfo info = new FaceBeautyDeviceInfo();
    ...
    private void doCameraSelect(String oldCamera, String newCamera) {
        ...
        mFaceBeautyView.setmCameraId(Integer.parseInt(newCamera));
    }
    ...
    private class FaceBeautyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case MSG_INIT_VIEW:
                    mFaceBeautyView.init();
                    mFaceBeautyView.update(ON_CAMERA_PARAMETERS_READY);
                ...
                case MSG_SHOW_VIEW:
                    ...
                    break;
                case MSG_HIDE_VIEW:
                    mFaceBeautyView.hide();
                    break;
                case MSG_UNINIT_VIEW:
                    mFaceBeautyView.unInit();
                    break;
                default:
                    break;
            }
    ...
    private byte[] addExifToImageBuffer(byte[] data, int format) {
        if (format != ImageFormat.JPEG) {
        ...
        try {
            ...
            if (mCaptureResult != null) {
                ...
                LogHelper.i(TAG, "[addExifToImageBuffer] aperture " + aperture);

                // F-stop number
                Float fNumber = mCaptureResult.get(CaptureResult.LENS_APERTURE);
                addExifTag(exifInterface, ExifInterface.TAG_F_NUMBER, rational(fNumber,
                        F_NUMBER_PRECISION));
                Double apertureValue = 2 * log2(fNumber);
                exifInterface.setTag(exifInterface.buildTag(ExifInterface.TAG_APERTURE_VALUE,
                        rational(apertureValue, APERTURE_VALUE_PRECISION)));

                Long exposureTime = mCaptureResult.get(CaptureResult.SENSOR_EXPOSURE_TIME);
                LogHelper.i(TAG, "[addExifToImageBuffer] exposureTime " + exposureTime);
                exifInterface.setTag(exifInterface.buildTag(ExifInterface.TAG_EXPOSURE_TIME,
                        ratio(exposureTime, NS_TO_S)));

                Integer iso = mCaptureResult.get(CaptureResult.SENSOR_SENSITIVITY);
                LogHelper.i(TAG, "[addExifToImageBuffer] iso " + iso);
                exifInterface.setTag(exifInterface.buildTag(ExifInterface.TAG_ISO_SPEED_RATINGS,
                        iso));

                Float focalLength = mCaptureResult.get(CaptureResult.LENS_FOCAL_LENGTH);
                LogHelper.i(TAG, "[addExifToImageBuffer] focalLength " + focalLength);
                exifInterface.setTag(exifInterface.buildTag(ExifInterface.TAG_FOCAL_LENGTH,
                        rational(focalLength, FOCAL_LENGTH_PRECISION)));

                mCaptureResult = null;
            }

            ByteArrayOutputStream outStream = new ByteArrayOutputStream();
            OutputStream exifWriterStream = exifInterface.getExifWriterStream(outStream);
            exifInterface.writeExif(data, exifWriterStream);
    ...
    private void addExifTag(ExifInterface exifInterface, int tagId, Object val) {
        if (val != null) {
            exifInterface.setTag(exifInterface.buildTag(tagId, val));
        }
    }
