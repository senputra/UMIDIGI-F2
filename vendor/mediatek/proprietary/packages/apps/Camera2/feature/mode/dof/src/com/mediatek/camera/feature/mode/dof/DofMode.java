...
import com.mediatek.camera.feature.mode.dof.DofDeviceController.DeviceCallback;
import com.mediatek.camera.feature.mode.dof.DofDeviceController
        .PreviewSizeCallback;
 ...
 */
public class DofMode extends CameraModeBase implements CaptureDataCallback,
        DeviceCallback, PreviewSizeCallback, IMemoryManager.IMemoryListener {
    private static final LogUtil.Tag TAG = new LogUtil.Tag(DofMode.class.getSimpleName());
    ...
    protected DofDeviceController mIDeviceController;
    protected DofModeHelper mPhotoModeHelper;
    ...
    private DofView dofView = new DofView();
    ...
    public void init(@Nonnull IApp app, @Nonnull ICameraContext cameraContext,
        ...
        DofControllerFactory deviceControllerFactory = new DofControllerFactory();
        ...
        dofView.setViewChangeListener(mViewChangeListener);
        dofView.init(app);
        ...
        mPhotoModeHelper = new DofModeHelper(cameraContext);
        LogHelper.d(TAG, "[init]- ");
    }
    ...
    public void resume(@Nonnull DeviceUsage deviceUsage) {
        ...
        mIsResumed = true;
        dofView.showView();
        dofView.onOrientationChanged(mIApp.getGSensorOrientation());
        initSettingManager(mCameraId);
        initStatusMonitor();
...
//        }
    ...
    public void onOrientationChanged(int orientation) {
        dofView.onOrientationChanged(orientation);
    }
    @Override
    ...
    private void prepareAndOpenCamera(boolean needOpenCameraSync, String cameraId,
        ...
        mIDeviceController.setDeviceCallback((DofDeviceController.DeviceCallback) this);
        mIDeviceController.setPreviewSizeReadyCallback(this);
        //prepare device info.
        DofDeviceInfo info = new DofDeviceInfo();
    ...
    private DofDeviceController.StereoWarningCallback mWarningCallback
            = new DofDeviceController.StereoWarningCallback() {
        @Override
        public void onWarning(int type) {
            LogHelper.i(TAG, "[StereoWarningCallback onWarning] " + type);
            switch (type) {
                case DofView.DUAL_CAMERA_LOW_LIGHT:
                    break;
                case DofView.DUAL_CAMERA_READY:
                    break;
                case DofView.DUAL_CAMERA_TOO_CLOSE:
                    break;
                case DofView.DUAL_CAMERA_LENS_COVERED:
                    break;
                default:
                    LogHelper.w(TAG, "Warning message don't need to show");
                    break;
            }
            dofView.showWarningView(type);
        }
    };
    ...
    private DofView.ViewChangeListener mViewChangeListener
            = new DofView.ViewChangeListener() {
        @Override
        public void onVsDofLevelChanged(int level) {
            mIDeviceController.setVsDofLevelParameter(level);
        }
