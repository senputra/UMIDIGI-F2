...
public class CameraSysTrace {
    private static final int SYSTRACE_DISABLED = 0;
    private static int enabledValue = SystemProperties.getInt("vendor.debug.camap.performance",
                     SYSTRACE_DISABLED);
    ...
    public static void onEventSystrace(String evtName, boolean isBegin) {
        if (enabledValue == SYSTRACE_DISABLED || LogUtil.getAndroidSDKVersion() < Build.VERSION_CODES.JELLY_BEAN_MR2) {
            return;
        }
        if (isBegin) {
    ...
    public static void onEventSystrace(String evtName, boolean isBegin, boolean isDefault) {
        if (LogUtil.getAndroidSDKVersion() < Build.VERSION_CODES.JELLY_BEAN_MR2) {
            return;
        }
        if (isBegin) {
            CameraPerformanceTrace.beginSection(evtName);
