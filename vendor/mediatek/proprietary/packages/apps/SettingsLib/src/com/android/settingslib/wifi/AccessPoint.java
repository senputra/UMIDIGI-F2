...
public class AccessPoint implements Comparable<AccessPoint> {
    ...
    /// M: Updated from 9 to 11 for 2 entries from WAPi security
    public static final int SECURITY_MAX_VAL = 11; // Has to be the last
    ...
    public AccessPoint(Context context, Bundle savedState) {
        ...
        if (savedState.containsKey(KEY_SCOREDNETWORKCACHE)) {
            ...
            /// M: Need to handle if timedScore is AOSP TimestampedScoredNetwork @{
            for (Object timedScore : scoredNetworkArrayList) {
                Class timedScoreClass = timedScore.getClass();
                String className = timedScoreClass.getName();
                Log.d(TAG, "className: " + className);
                if(className.contains("com.android.settingslib.wifi.TimestampedScoredNetwork")) {
                     TimestampedScoredNetwork mtkTimedScore
                       = TimestampedScoredNetwork.getMtkTSSNObj(timedScore);
                     mScoredNetworkCache.put(mtkTimedScore.getScore().networkKey.wifiKey.bssid,
                                             mtkTimedScore);
                } else {
                     TimestampedScoredNetwork timedScoreTemp= (TimestampedScoredNetwork)timedScore;
                     mScoredNetworkCache.put(timedScoreTemp.getScore().networkKey.wifiKey.bssid,
                                             timedScoreTemp);
                }
            }
          /// M: @}
