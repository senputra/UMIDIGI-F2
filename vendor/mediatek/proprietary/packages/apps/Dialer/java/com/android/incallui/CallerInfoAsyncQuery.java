...
import com.android.incallui.ContactInfoCache.DialerCallCookieWrapper;
...
public class CallerInfoAsyncQuery {
  ...
  static void startQuery(
    ...
    Log.d(LOG_TAG, "- number: " + InCallUtils.formatSensitiveValue(info.phoneNumber));
    /// @}
  ...
  private static void startQueryInternal(
    ...
    /// M: [Performance] Check voicemail number in ayncQueryHandler. @ {
    /// Google code:
      ...
      try {
        ...
        if (callerInfo == null) {
          ...
          if (cw.event == EVENT_EMERGENCY_NUMBER) {
            ...
            /// M: ALPS03484999 always set phone number to avoid re-duplicative queries @{
            ...
            final DialerCallCookieWrapper callCookie = (DialerCallCookieWrapper) cw.cookie;
            final DialerCall call = CallList.getInstance().getCallById(callCookie.callId);
            if (call != null) {
              callerInfo = new CallerInfo().markAsVoiceMail(queryContext, call.getCallSubId());
            } else {
              callerInfo = new CallerInfo().markAsVoiceMail(queryContext);
            }
            /// @}
          ...
          /// M: ALPS05030235, update CNAP name, or else
          /// InCallUI App will refresh the CNAP name to empty. @{
          DialerCallCookieWrapper callCookie = (DialerCallCookieWrapper) cw.cookie;
          if (callCookie != null) {
            callerInfo.cnapName = callCookie.cnapName;
          }
          /// @}
        ...
        if (cw == null) {
          ...
          switch (cw.event) {
            case EVENT_NEW_QUERY:
              /// M: [Performance] Check voicemail number in ayncQueryHandler. @ {
              if (PhoneNumberHelper.isLocalEmergencyNumber(queryContext, cw.number)) {
                cw.event = EVENT_EMERGENCY_NUMBER;
                updateData(msg.arg1, cw, (Cursor) args.result);
                Log.d(this, "Processing event, set event type as EMERGENCY");
                break;
              }
              /// @}
