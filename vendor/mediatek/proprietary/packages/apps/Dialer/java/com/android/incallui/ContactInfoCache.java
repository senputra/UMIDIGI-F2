...
public class ContactInfoCache implements OnImageLoadCompleteListener {
  ...
  private ContactInfoCache(Context context) {
    ...
    CallDetailChangeHandler.getInstance()
        .addCallDetailChangeListener(callDetailChangeListener);
    /// @}
    Trace.endSection();
  }
      ...
      if (TextUtils.isEmpty(number) && TextUtils.isEmpty(info.cnapName)) {
        ...
        Log.d(
            ...
            "  ==> cnapName available: displayName '"
                ...
                /// Google code:
                /// + displayName
                + MoreStrings.toSafeString(displayName)
                /// @}
                + "', displayNumber '"
                /// M: format sensitive log. @{
                /// Google code:
                /// + displayNumber
                + InCallUtils.formatSensitiveValue(displayNumber)
                /// @}
                + "'");
      } else {
  ...
  private ContactCacheEntry updateCallerInfoInCacheOnAnyThread(
    ...
    if (existingCacheEntry != null) {
      if (existingCacheEntry.isEmergencyNumber) {
        ...
        /// M: ALPS04725388. Add subId to get exact voice mail tag @{
        // Google orignal code:
        // callerInfo.markAsVoiceMail(context);
        final DialerCall call = CallList.getInstance().getCallById(callId);
        if (call != null) {
          callerInfo.markAsVoiceMail(context, call.getCallSubId());
        } else {
          callerInfo.markAsVoiceMail(context);
        }
        /// @}
    ...
    if (didLocalLookup) {
      if (cacheEntry.displayPhotoUri != null) {
        ...
        if (existingCacheEntry != null
          ...
          Log.d(TAG, "Same picture. Do not need start image load." + cacheEntry);
          cacheEntry.photo = existingCacheEntry.photo;
          cacheEntry.photoType = existingCacheEntry.photoType;
          /// M: ALPS04237491 contact cache info update issue @{
  ...
  private void maybeUpdateFromCequintCallerId(
    ...
    if (!CequintCallerIdManager.isCequintCallerIdEnabled(context)) {
      Log.d(TAG, "maybeUpdateFromCequintCallerId not enable");
      return;
    }
  ...
  private void sendInfoNotifications(String callId, ContactCacheEntry entry) {
    ...
    final Set<ContactInfoCacheCallback> callBacks =
        new ArraySet<ContactInfoCacheCallback>(this.callBacks.get(callId));
    /// @}
  ...
  private void sendImageNotifications(String callId, ContactCacheEntry entry) {
    ...
    // final Set<ContactInfoCacheCallback> callBacks = this.callBacks.get(callId);
    if (entry == null) {
      Log.d(TAG, "sendImageNotifications, entry is null");
      return;
    }
  ...
  public static class ContactCacheEntry {
    ...
    public String toString() {
      return "ContactCacheEntry{"
          ...
          + InCallUtils.formatSensitiveValue(number)
          /// @}
          + '\''
          + ", location='"
          ...
          /// Google code:
          /// + originalPhoneNumber
          + InCallUtils.formatSensitiveValue(originalPhoneNumber)
          /// @}
          + ", shouldShowLocation="
          + shouldShowLocation
          + ", isEmergencyNumber="
  ...
  /// M: ALPS04725388. Add subId to get exact voice mail tag @{
  // change to pulic to get callId at CallerInfoAsyncQuery
  // Google orignal code:
  ...
  private boolean needForceQuery(DialerCall call, ContactCacheEntry cacheEntry) {
    ...
    if (!TextUtils.equals(oldPhoneNumber, newPhoneNumber)) {
      ...
      Log.d(TAG, "phone number has changed: " + InCallUtils.formatSensitiveValue(oldPhoneNumber) +
          " -> " + InCallUtils.formatSensitiveValue(newPhoneNumber));
      /// @}
      return true;
    }
  ...
  /**
   * M: [VoLTE Conference] Add listener, which will get notified onContactInfoUpdated(),
   * when re-query of certain call complete.
   */
  ...
  /**
   * M: [VoLTE Conference] Restore all ContactInfoUpdatedListener, who will get notified later.
   */
  private final List<ContactInfoUpdatedListener> contactInfoUpdatedListener =
      new ArrayList<ContactInfoUpdatedListener>();

  /**
   * M: [VoLTE Conference] Add ContactInfoUpdatedListener.
   * @param listener ContactInfoUpdatedListener
   */
  public void addContactInfoUpdatedListener(ContactInfoUpdatedListener listener) {
    if (!contactInfoUpdatedListener.contains(listener)) {
      contactInfoUpdatedListener.add(listener);
    }
  }

  /**
   * M: [VoLTE Conference] Remove ContactInfoUpdatedListener.
   * @param listener ContactInfoUpdatedListener
   */
  public void removeContactInfoUpdatedListener(ContactInfoUpdatedListener listener) {
    if (contactInfoUpdatedListener.contains(listener)) {
      contactInfoUpdatedListener.remove(listener);
    }
  }

  /**
   * M: [VoLTE Conference] Listen onVolteMarkedEccChanged() and onPhoneNumberChanged()
   * from CallDetailsChangeHandler.
   */
  private final CallDetailChangeListener callDetailChangeListener =
  ...
  /**
   * M: [VoLTE Conference] When certain call is marked as Ecc by NW, notify listners.
   * @param call
   */
  private void handleIsEmergencyChanged(DialerCall call) {
    ...
    if (call != null && call.isVolteMarkedEcc()) {
      final String callId = call.getId();
      final ContactCacheEntry cacheEntry = infoMap.get(callId);
      final boolean isIncoming = call.getState() == DialerCallState.INCOMING;
      Set<ContactInfoCacheCallback> callBacks = this.callBacks.get(callId);
      ...
      CallerInfo callerInfo = new CallerInfo().markAsEmergency(context);
      final CallerInfoQueryToken queryToken = new CallerInfoQueryToken(queryId, callId);
      queryId++;
      if (cacheEntry != null) {
        // We should not override the old cache item until the new query is
      ...
      for (ContactInfoUpdatedListener listener : contactInfoUpdatedListener) {
  ...
  /**
   * M: [VoLTE Conference] When number of certain call changed, re-query for it.
  ...
  private void handlePhoneNumberChanged(DialerCall call) {
    ...
    if (call != null && !call.isVolteMarkedEcc()) {
      findInfo(call, call.getState() == DialerCallState.INCOMING,
