...
public final class RttTranscriptUtil {
  ...
  /// M: Delete rtt messages.
  @WorkerThread
  public static int deleteRttTranscript(Context context, Collection<String> transcriptIds) {
    RttTranscriptDatabaseHelper databaseHelper = new RttTranscriptDatabaseHelper(context);
    ...
    try {
      count = databaseHelper.getWritableDatabase()
          .delete(
              ...
              selection.getSelection(),
              selection.getSelectionArgs());
      LogUtil.i("RttTranscriptUtil.deleteRttTranscript ",  "deleted count :" + count);
    } catch (SQLException e) {
      LogUtil.e("RttTranscriptUtil.deleteRttTranscript ",  "delete happen exception");
      e.printStackTrace();
    }
    return count;
  }
  ...
  public static int clearAllRttTranscript(Context context) {
    Cursor cursor = context.getContentResolver().query(Calls.CONTENT_URI,
        new String[] {Calls.DATE}, null, null, null);
    if (cursor == null) {
      return 0;
    }
    ...
    do {
      ...
      while (cursor.moveToNext() && count <= 500) {
        builder.add(String.valueOf(cursor.getLong(0)));
        count = count + 1;
      }
      LogUtil.i("RttTranscriptUtil.clearAllRttTranscript ",  "this time del count :" + count);
      delCount = delCount + deleteRttTranscript(context, builder.build());
    } while (!cursor.isAfterLast());

    cursor.close();

    LogUtil.i("RttTranscriptUtil.clearAllRttTranscript ",  "deleted count :" + delCount);
    return delCount;
  }
  /// @}
}
