...
public final class NotificationChannelManager {
  ...
  /// M: ALPS04880502, dialer crash during make outgoing call after reset data in Setting.
  /// Need recreate notification channel if call channels do not exist. @{
  public static boolean shouldInitChannels(Context context) {
    Set<String> existingChannelIds = getAllExistingChannelIds(context);
    if (existingChannelIds.contains(NotificationChannelId.INCOMING_CALL)
        && existingChannelIds.contains(NotificationChannelId.ONGOING_CALL)
        && existingChannelIds.contains(NotificationChannelId.MISSED_CALL)) {
      return false;
    }
    LogUtil.d("NotificationChannelManager.shouldInitChannels", "Reinit channels");
    return true;
  }
  /// @}
