...
public abstract class TelecomUtil {
  ...
  public static class TelecomUtilImpl {
    ...
    public boolean isInCall(Context context) {
      ...
      // MTK API instead
      return hasReadPhoneStatePermission(context) && MtkTelecomManager.getInstance().isInCall(context);
    }
  ...
  /// M: --------------- MediaTek features ---------------------
  /**
   * M: ALPS04424558 guest mode can't get userhandle
   * Compose sytem phone account handle for guest mode.
   */
  @Nullable
  public static PhoneAccountHandle composePhoneAccountHandleEx(
      @Nullable String componentString, @Nullable String accountId) {
    if (TextUtils.isEmpty(componentString) || TextUtils.isEmpty(accountId)) {
      return null;
    }
    ...
    PhoneAccountHandle accountHandle =
          new PhoneAccountHandle(componentName, accountId, UserHandle.SYSTEM);
