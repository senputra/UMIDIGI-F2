...
public class MainSearchController implements SearchBarListener {
  ...
  private void showDialpad(boolean animate, boolean fromNewIntent) {
    ...
    if (stateSaved) {
      LogUtil.e("MainSearchController.showDialpad", "state already saved.");
      return;
    }
    /// @}
  ...
  private void hideDialpad(boolean animate) {
    ...
    ///M: add for ALPS04409841 @{
    if (stateSaved) {
      LogUtil.e("MainSearchController.hideDialpad", "state already saved.");
      return;
    }
    /// @}
    fab.show();
        ...
        new AnimationListener() {
          ...
          public void onAnimationEnd(Animation animation) {
            if (activity.isSafeToCommitTransactions()
              ...
              ///M: Add for ALPS04409841 @{
              searchFragment.hideDialpad();
              /// @}
              LogUtil.i("MainSearchController.hideDialpad", "onAnimationEnd..");
            }
          }
  ...
  private void closeSearch(boolean animate) {
    ...
    ///M: add for ALPS04409841 @{
    if (stateSaved) {
      LogUtil.e("MainSearchController.closeSearch", "state already saved.");
      return;
    }
    ///@}
    if (isDialpadVisible()) {
    ...
    ///M: add justification dialpadFragment.getDigitsWidget() != null for ALPS04677765
  ...
  private boolean isSearchVisible() {
    if (searchFragment != null) {
      LogUtil.i("isSearchVisible", "searchFragment.isAdded() : " + searchFragment.isAdded());
      LogUtil.i("isSearchVisible", "!searchFragment.isHidden() :" + !searchFragment.isHidden());
    }
  ...
  private void openSearch(Optional<String> query) {
    ...
    ///M: add for ALPS04409841 @{
    if (stateSaved) {
      LogUtil.e("MainSearchController.openSearch", "state already saved.");
      return;
    }
    ///@}
    fab.hide();
  ...
  public void onSearchQueryUpdated(String query) {
    /// M: Fix google issue.
    /// When rotate phone or change to multi-window mode, this function will
    /// be called and it will enter regular search.
    /// But if dialpad fragment is shown at this time, it will lead to UI abnormal.
    /// So we will add judgement condition, when dialpad is shown, it will NOT
    /// enter regular search. @{
    if (dialpadFragment != null && dialpadFragment.isAdded() && !dialpadFragment.isHidden()) {
        LogUtil.i("MainSearchController.onSearchQueryUpdated", "Dialpad is visible.");
        return;
    }
    /// @}
    if (searchFragment != null) {
      ///M: Add for ALPS04699065 @{
  ...
  public boolean onMenuItemClicked(MenuItem menuItem) {
    LogUtil.i("MainSearchController.onMenuItemClicked",
    ...
    } ///M: Add it for one key conference call @ {
    else if (menuItem.getItemId() == R.id.menu_volte_conf_call) {
      DialerVolteUtils.handleMenuVolteConfCall(activity);
      return true;
    }
    ///@}
    ///M: Add it for GCF test @ {
    else if (menuItem.getItemId() == R.id.menu_empty_voice_conf_call) {
      DialerVolteUtils.handleMenuEmptyVoiceConfCall(activity);
      return true;
    } else if (menuItem.getItemId() == R.id.menu_empty_video_conf_call) {
      DialerVolteUtils.handleMenuEmptyVideoConfCall(activity);
      return true;
    }
    /// @}
    return false;
  ...
  public void onActivityPause() {
    ...
    LogUtil.i("onActivityPause", "closeSearchOnPause = " + closeSearchOnPause);
    LogUtil.i("onActivityPause", "callPlacedFromSearch = " + callPlacedFromSearch);
  ...
  public void onCallPlacedFromSearch() {
    ...
    if (fragmentContainer != null) {
        LogUtil.i("MainSearchController","onCallPlacedFromSearch(), hideInputMethod()");
        DialerUtils.hideInputMethod(fragmentContainer);
    }
    /// @}
  ...
  ///M: Add for ALPS04767295, Called when permission allowed finished @{
  @Override
  public void unRequestingPermission() {
    LogUtil.enterBlock("MainSearchController.unRequestingPermission");
