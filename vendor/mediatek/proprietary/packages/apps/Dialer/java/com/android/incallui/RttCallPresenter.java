...
public class RttCallPresenter implements RttCallScreenDelegate, InCallStateListener {
  ...
  public void onLocalMessage(String message) {
    ...
    /// M: for AT&T RTT feature. AT&T requirments replace "\n" with "\r\n". @{
    DialerCall call = CallList.getInstance().getCallById(rttCallScreen.getCallId());
    if (call.getVideoFeatures().supportsRttCallMerge()) {
      message = message.replace(Constants.BUBBLE_BREAKER,Constants.CARRIAGE_RETURN_LINEFEED);
    }
    /// @}
  ...
  public void onStateChange(InCallState oldState, InCallState newState, CallList callList) {
    ...
    if (newState == InCallState.INCALL) {
      ...
      /// M: ALPS04386067 AT&T RTT requirments user can't edit text when remote hold call. @{
      DialerCall call = CallList.getInstance().getCallById(rttCallScreen.getCallId());
      if (call == null) {
        LogUtil.i("RttCallPresenter.onStateChange", "call does not exist");
        return;
      }
      /// M: While remote hold call,since TMO has no requirements of hide editText, messages still
      /// can be sent.if the call ends without resuming,these message seems to be buffered in the
      /// network and be sent to remote side at next RTT call.so we also hide editText for TMO and
      //  Vzw.
      rttCallScreen.updateCallHeldState(call.isRemotelyHeld());
      /// @}
    }
  }
  ...
  private static class RemoteMessageHandler extends Handler {
    ...
    public void handleMessage(android.os.Message msg) {
      switch (msg.what) {
        ...
        case READ_MESSAGE:
          try {
            /// M: for AT&T RTT feature. AT&T requirments take "\u2028" and "\r\n" as "\n" when
            /// recevie message. @{
            /// original code:
            ///final String message = rttCall.readImmediately();
            ///ThreadUtil.postOnUiThread(() -> rttCallScreen.onRemoteMessage(message));
            String message = rttCall.readImmediately();
            if (message != null) {
              if(message.contains(Constants.LINE_SEPARATOR_CHART)) {
                message = message.replace(Constants.LINE_SEPARATOR_CHART, Constants.BUBBLE_BREAKER);
              }
              if(message.contains(Constants.CARRIAGE_RETURN)) {
                message = message.replace(Constants.CARRIAGE_RETURN, "");
              }
              final String recvFinalMessage = message;
              ThreadUtil.postOnUiThread(() -> rttCallScreen.onRemoteMessage(recvFinalMessage));
            }
            /// @}
          } catch (IOException e) {
