...
public class ThermalThrottle extends CameraAddition {
     ...
     */
    public ThermalThrottle(ICameraContext app) {
	    super(app);
        ...
        int contentLaunch = mRes.getIdentifier("pref_thermal_dialog_content2", "string",
                mActivity.getPackageName());
    ...
    private void updateCountDownTime(final Activity activity) {
        Log.d(TAG, "[updateCountDownTime]mCountDown = " + mWaitingTime + ",mIsResumed = "
                + mIsResumed);
                ...
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (activity.isFinishing() || activity.isDestroyed()) {
                            Log.d(TAG, "[updateCountDownTime] don't need finish activity");
                        } else {
                            activity.finish();
                        }
                    }
            ...
            switch (msg.what) {
                case MSG_READ_THERMAL:
                    ...
                    if (mThermalStatus == THERMAL_BUFFER_VALUE
                            ...
                            && !mIsShowing) {
                        Log.d(TAG, "[handleMessage]WorkerHandler, mCountDown = "
                                + mWaitingTime);
    ...
    private int getTemperStatus() {
        ...
        try {
            ...
            if (mThermalStatus != temperInt) {
                Log.i(TAG, "Camera Thermal status :" + temperInt);
            }
        }
