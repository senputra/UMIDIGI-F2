...
public final class AsyncRingtonePlayer {
    ...
    private class RingtonePlaybackDelegate implements PlaybackDelegate {
        ...
        public boolean play(Context context, Uri ringtoneUri, long crescendoDuration) {
            ...
            mMediaPlayer = new MediaPlayer();
            boolean ringtonePresent = true;
            /// M: [ALPS03538536] If path is inaccessible, fallback to default alarm tone @{
            if (UserManager.get(context.getApplicationContext()).isUserUnlocked()) {
                try {
                    mMediaPlayer.setDataSource(context, ringtoneUri);
                } catch (Exception e) {
                    LOGGER.e("Unable to locate alarm ringtone, it means ringtone doesn't exist");
                    ringtonePresent = false;
                }
                if(ringtonePresent == false){
                    // if ringtone doesn't exist, fallback to default
                    mRingtone = null;
                }
            }
            /// @}
            ...
            /// M: [ALPS03538536] Check ringtone before enabling looping @{
