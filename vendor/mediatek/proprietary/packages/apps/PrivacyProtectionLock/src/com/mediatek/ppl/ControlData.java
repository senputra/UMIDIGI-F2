...
public class ControlData {
    ...
    public static final int SECRET_SIZE = 32; //SHA-1:20, SHA-256:32
        ...
        if (TrustedNumberList != null) {
            for (int i = 0; i < TrustedNumberList.size(); ++i) {
                ...
                if (buffer.length > TRUSTED_NUMBER_LENGTH) {
                    Log.w(TAG, "Trusted number is too long. Truncate it.");
                }
    ...
    public String toString() {
        ...
        if (SimFingerPrintList != null) {
            for (int i = 0; i < SimFingerPrintList.size(); i++) {
                sb.append(Utils.byteForSecureLog(SimFingerPrintList.get(i)));
                sb.append("|");
            }
        } else {
            sb.append(SimFingerPrintList);
        }
        sb.append(", ");
        if (TrustedNumberList != null) {
            for (int i = 0; i < TrustedNumberList.size(); i++) {
                sb.append(Utils.stringForSecureLog(TrustedNumberList.get(i)));
                sb.append("|");
            }
        } else {
            sb.append(TrustedNumberList);
        }
        sb.append(", ")
