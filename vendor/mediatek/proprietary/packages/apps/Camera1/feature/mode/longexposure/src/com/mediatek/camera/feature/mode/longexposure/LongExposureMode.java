...
import com.mediatek.camera.feature.mode.longexposure.ILongExposureDeviceController.JpegCallback;
...
public class LongExposureMode extends CameraModeBase implements IAppUiListener.OnGestureListener,
        JpegCallback, DeviceCallback, PreviewSizeCallback {
    ...
    private void updateUiState(LongExposureViewState state) {
        ...
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                LogHelper.d(TAG, "[updateUiState] state = " + state);
