...
/// M: Add confirm dialog when changing the default APN.
...
public class ApnEditor extends SettingsPreferenceFragment
    ...
    /// M: Add for supporting read only APN.
     ...
     */
    /// M: Revise for APN setting plugin.
    private static String[] sProjection = new String[] {
            ...
            /// M: Add for supporting the new APN type.
            MtkTelephony.Carriers.SOURCE_TYPE   //25
    };
    ...
    public void onCreate(Bundle icicle) {
        ...
        /// M: Add for receiving the broadcasts. maybe finish in onCreate method @{
        ...
        if (action.equals(Intent.ACTION_EDIT)) {
            ...
            Log.d(TAG, "onCreate, readOnlyMode=" + mReadOnlyMode);
            ...
            Log.d(TAG, "onCreate, mvnoType=" + mMvnoTypeStr
                    + ", mvnoMatchData=" + mMvnoMatchDataStr);
        } else {
            finish();
            return;
        }
        ...
        if (uri != null) {
            ...
            Log.d(TAG, "onCreate, projectionLength=" + sProjection.length);
        }
    ...
    void fillUI(boolean firstTime) {
        ...
        if (firstTime) {
            ...
            if (mNewApn) {
                ...
                String numeric = TelephonyUtils.getSimOperatorNumeric(mSubId, mTelephonyManager);
                    mTelephonyManager.getSimOperator(mSubId);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        /// M: Add for supporting read only APN.
        Log.d(TAG, "onCreateOptionsMenu, readOnlyMode=" + mReadOnlyMode);
        if (mReadOnlyMode) {
            return;
        }
        /// @}
        ...
        /// M: Don't add the delete menu for the default APN.
    ...
    boolean validateAndSaveApnData() {
        Log.d(TAG, "validateAndSaveApnData");
        ...
        Log.d(TAG, "validateAndSaveApnData, Save apn=" + values.getAsString(Telephony.Carriers.APN));
    ...
    String validateApnData() {
        ...
        String mcc = checkNotSet(mMcc.getText());
        String mnc = checkNotSet(mMnc.getText());
        /// M: The empty apn field is allowed for IA APN.
    ...
    private String getUserEnteredApnType() {
        ...
        if ((TextUtils.isEmpty(userEnteredApnType)
            ...
            for (String apnType : MtkPhoneConstants.MTK_APN_TYPES) {
                /// M: Add for ALPS03566855, skip IMS apn when apn type is not set. @{
                if (TextUtils.isEmpty(userEnteredApnType)
    ...
    /// M: Add for unregister the receivers and listeners. @{
    @Override
    ...
    private void updateScreenEnableState() {
        ...
        Log.d(TAG, "updateScreenEnableState, enable=" + enable + ", readOnlyMode=" + mReadOnlyMode);
    ...
    private boolean isSimReadyAndRadioOn() {
        ...
        boolean airplaneModeEnabled = TelephonyUtils.isAirplaneModeOn(getContext());
        boolean enable = !airplaneModeEnabled && (simState == TelephonyManager.SIM_STATE_READY);

        Log.d(TAG, "isSimReadyAndRadioOn, subId=" + mSubId + ", airplaneMode="
                + airplaneModeEnabled + ", simState=" + simState);
        return enable;
    }
    /// @}
}
