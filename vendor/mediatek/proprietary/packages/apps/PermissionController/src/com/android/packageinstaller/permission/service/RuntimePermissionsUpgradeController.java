...
class RuntimePermissionsUpgradeController {
    ...
    private static void whitelistAllSystemAppPermissions(@NonNull Context context) {
        ...
        // Update apps permission flags that have been confirmed in review UI
        if (CtaManagerFactory.getInstance().makeCtaManager().isCtaSupported()) {
            SharedPreferences reviewConfirmSharedPreferences
              = context.getSharedPreferences("reviewConfirm", Context.MODE_PRIVATE);
            String pkgNames = reviewConfirmSharedPreferences.getString("package", "");
            Log.d(LOG_TAG, "whitelistReviewConfirmPkgNames: " + pkgNames);
            for (int i = 0; i < appCount; i++) {
                final PackageInfo app = apps.get(i);

                boolean contained = false;
                for (String pkgName : pkgNames.split(",")) {
                ...
                for (String requestedPermission : app.requestedPermissions) {
                    ...
                    if ((permInfo.getProtection() & PermissionInfo.PROTECTION_DANGEROUS) == 0) {
                        continue;
                    }

                    Log.d(LOG_TAG, "whitelistReviewConfirmPkgPermission: " + requestedPermission
                          + " ,whitelistReviewConfirmPkgName: " + app.packageName);
                    UserHandle user = UserHandle.getUserHandleForUid(app.applicationInfo.uid);
                    context.getPackageManager().updatePermissionFlags(requestedPermission,
                        app.packageName,
