...
static void *CLIENTAPI_Init_Hal_Thread(void *arg)
{
    int notused = (int)arg;
 ...
 *
 *****************************************************************************/
static int CLIENTAPI_Proxy_adapter_SEND(CLIENTAPI_EventMsg_t *event)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (!event || event->source != EventSource_IMS_DISPATCHER) {
        ErrMsg("CLIENTAPI_Proxy_adapter_SEND error");
        return -1;
    }
...
void send_test_event_to_dispatcher()
{
    ...
    event = (CLIENTAPI_EventMsg_t *)CLIENTAPI_COMM_GetEvent(data_len);
    if (!event)
    {
...
//initialize CLIENTAPI proxy adapater
//it startes the read and write thread for handling with CLIENTAPI_proxy adapter

/*****************************************************************************
 * FUNCTION
 *  CLIENTAPI_Proxy_adapter_Init_hal
 * DESCRIPTION
 *
 * PARAMETERS
 *  source_id       [IN]
 ...
 *
 *****************************************************************************/
void CLIENTAPI_Proxy_adapter_Init_hal(int source_id)
{
    DbgMsg("CLIENTAPI_Proxy_adapter_Init_hal");
    ...
    if (pthread_create(&g_recv_thread, &(attr), CLIENTAPI_Init_Hal_Thread, (void*)0))
    {
        ErrMsg("Can't create CLIENTAPI_Init_Hal_Thread");
        return;
    }

    /* set callback for sending event directly to clientapi_adapter */
    CLIENTAPI_COMM_SetSend(source_id, CLIENTAPI_Proxy_adapter_SEND);
}
...
void playbackTest(CLIENTAPI_EventMsg_t *event) {
    //playback for testing
    CLIENTAPI_EventMsg_t *event_back = 0;
    event_back = (CLIENTAPI_EventMsg_t *)CLIENTAPI_COMM_GetEvent(event->data_len);
    if (!event_back)
    {
    ...
    CLIENTAPI_Proxy_adapter_SEND(event_back);
}
...
void writeEventHal(int32_t request_id, int32_t length, char* data) {
    ...
    /* assign a event with the size of data length */
    event = (CLIENTAPI_EventMsg_t *)CLIENTAPI_COMM_GetEvent(length);
    if (!event)
    {
        ErrMsg("Can't allocat the event");
        return;
