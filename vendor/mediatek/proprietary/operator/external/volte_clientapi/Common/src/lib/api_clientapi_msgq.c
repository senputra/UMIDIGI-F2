 ...
 *
 *****************************************************************************/
int CLIENTAPI_CMSGQ_Put(void *o, void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (msgq->func)
    {
        ...
        while (c-- > 0)
        {
            ...
            if (ret == CLIENTAPI_CMSGQ_CHECK_DROP)
            {
                pthread_mutex_unlock(&(msgq->mutex));
                return 2;
            }
 ...
 *
 *****************************************************************************/
int CLIENTAPI_CMSGQ_Put2(void *o, void *msg, int priority)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *
 *****************************************************************************/
int CLIENTAPI_CMSGQ_DelayPut(void *o, void *data, int delay, int priority)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        if (TIME_Init(msg->timer, "CLIENTAPI_CMSGQ_TIMER", msgq_expire_handler, msg, delay, 0))
        {
            MEM_Free(msg->timer);
            MEM_Free(msg);
 ...
 *  void
 *****************************************************************************/
void *CLIENTAPI_CMSGQ_Get(void *o, int flag)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (flag == CLIENTAPI_CMSGQ_FLAG_WAIT)
    {
        while (msgq->count == 0)
 ...
 *  void
 *****************************************************************************/
void *CLIENTAPI_CMSGQ_GetMsg(void *o, CLIENTAPI_CMSGQ_callback func, void *data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
        ...
        if (func)
        {
            ...
            while (c-- > 0)
            {
                if (find == 0)
                {
                    ...
                    if (ret == CLIENTAPI_CMSGQ_CHECK_RETURN)
                    {
                        ret_data = msg->data;
                        msgq->count--;
                        find = 1;
 ...
 *
 *****************************************************************************/
int CLIENTAPI_CMSGQ_Free(void *o)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
int test1()
{
    ...
    q = CLIENTAPI_CMSGQ_Init(5, "test", 0);

    ret = CLIENTAPI_CMSGQ_Put(q, (void*)1);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put(q, (void*)2);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put(q, (void*)3);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put(q, (void*)4);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put(q, (void*)5);
    UT_CHECK(ret == 0);

    ret = (int)CLIENTAPI_CMSGQ_Get(q, CLIENTAPI_CMSGQ_FLAG_NOWAIT);
    UT_CHECK(ret == 1);
    ret = (int)CLIENTAPI_CMSGQ_Get(q, CLIENTAPI_CMSGQ_FLAG_NOWAIT);
    UT_CHECK(ret == 2);
    ret = (int)CLIENTAPI_CMSGQ_Get(q, CLIENTAPI_CMSGQ_FLAG_NOWAIT);
...
int test2()
{
    ...
    q = CLIENTAPI_CMSGQ_Init(10, "test", 0);

    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)1, 10);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)2, 8);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)3, 6);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)4, 10);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)5, 12);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)6, 12);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)7, 12);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)8, 6);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)9, 8);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_Put2(q, (void*)10, 9);
    UT_CHECK(ret == 0);
...
int test3()
{
    ...
    ret = CLIENTAPI_CMSGQ_DelayPut(q, (void*)1, 400, 3);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_DelayPut(q, (void*)2, 300, 4);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_DelayPut(q, (void*)3, 200, 2);
    UT_CHECK(ret == 0);
    ret = CLIENTAPI_CMSGQ_DelayPut(q, (void*)4, 100, 5);
    UT_CHECK(ret == 0);

    ret = (int)CLIENTAPI_CMSGQ_Get(q, CLIENTAPI_CMSGQ_FLAG_WAIT);
    UT_CHECK(ret == 4);
    ret = (int)CLIENTAPI_CMSGQ_Get(q, CLIENTAPI_CMSGQ_FLAG_WAIT);
    UT_CHECK(ret == 3);
    ret = (int)CLIENTAPI_CMSGQ_Get(q, CLIENTAPI_CMSGQ_FLAG_WAIT);
...
int check_cb(void *new_msg, void *msg)
{
    ...
    if (curr > priv)
    {
        return CLIENTAPI_CMSGQ_CHECK_RETURN;
    }
    return CLIENTAPI_CMSGQ_CHECK_CONTINUE;
}
...
int test5()
{
    ...
    ret = (int)CLIENTAPI_CMSGQ_GetMsg(q, check_cb, (void*)5);
    UT_CHECK(ret == 6);
    ret = (int)CLIENTAPI_CMSGQ_GetMsg(q, check_cb, (void*)5);
    UT_CHECK(ret == 7);
    ret = (int)CLIENTAPI_CMSGQ_GetMsg(q, check_cb, (void*)5);
    ...
    ret = (int)CLIENTAPI_CMSGQ_GetMsg(q, check_cb, (void*)5);
    UT_CHECK(ret == 0);
    ret = (int)CLIENTAPI_CMSGQ_GetMsg(q, check_cb, (void*)0);
    UT_CHECK(ret == 5);
    ret = (int)CLIENTAPI_CMSGQ_GetMsg(q, check_cb, (void*)0);
    UT_CHECK(ret == 1);
    ret = (int)CLIENTAPI_CMSGQ_GetMsg(q, check_cb, (void*)0);
