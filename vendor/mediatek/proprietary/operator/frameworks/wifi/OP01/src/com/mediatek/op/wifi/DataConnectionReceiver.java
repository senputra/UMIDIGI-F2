...
public class DataConnectionReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (!Op01WifiUtils.ACTION_WIFI_FAILOVER_GPRS_DIALOG.equals(action)) {
            return;
        }

        final int reminderType = Settings.System.getInt(
            context.getContentResolver(),
            MtkSettingsExt.System.WIFI_CONNECT_REMINDER,
                Op01WifiUtils.WIFI_CONNECT_REMINDER_ALWAYS);
        if (reminderType != Op01WifiUtils.WIFI_CONNECT_REMINDER_ALWAYS) {
            return;
        }
        ...
        if (shutdownRequest == null || shutdownRequest.length() == 0 || "0".equals(shutdownRequest)) {
            ...
            start.setClassName(Op01WifiUtils.PACKAGE_NAME_MANIFEST,
                    Op01WifiUtils.CLASS_NAME_DATA_DIALOG);
