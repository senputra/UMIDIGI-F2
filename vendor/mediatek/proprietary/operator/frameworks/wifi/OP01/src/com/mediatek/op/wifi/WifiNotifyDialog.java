...
public class WifiNotifyDialog extends AlertActivity implements DialogInterface.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!action.equals(IWifiFwkExt.WIFI_NOTIFICATION_ACTION) || mNetworkId == -1) {
            Log.e("@M_" + TAG, "Error: this activity may be started only with intent WIFI_NOTIFICATION_ACTION");
            finish();
        }
        createDialog();
    }
    ...
    private View createView() {
        ...
        if (value == MtkSettingsExt.System.WIFI_CONNECT_TYPE_MANUL) {
            contentView.setText(getString(R.string.msg_wlan_signal_found_manul));
        } else if (value == MtkSettingsExt.System.WIFI_CONNECT_TYPE_ASK) {
            contentView.setText(getString(R.string.msg_wlan_signal_found, mSsid));
        }
    ...
    private void onOK() {
        ...
        int value = Settings.System.getInt(getContentResolver(),
                MtkSettingsExt.System.WIFI_CONNECT_TYPE,
                MtkSettingsExt.System.WIFI_CONNECT_TYPE_AUTO);
        mConnectApType = Settings.System.getInt(getContentResolver(),
                MtkSettingsExt.System.WIFI_CONNECT_AP_TYPE,
                MtkSettingsExt.System.WIFI_CONNECT_AP_TYPE_AUTO) == 0;
        if (mConnectApType  && (value != MtkSettingsExt.System.WIFI_CONNECT_TYPE_MANUL)) {
    ...
    private void onCancel() {
        ...
        // wait for wifi framework ready
        //mWm.suspendNotification(IWifiFwkExt.NOTIFY_TYPE_SWITCH);
        Intent intent = new Intent(Op01WifiUtils.ACTION_SUSPEND_NOTIFICATION_NON_SYS);
        intent.putExtra(IWifiFwkExt.EXTRA_SUSPEND_TYPE, IWifiFwkExt.NOTIFY_TYPE_SWITCH);
