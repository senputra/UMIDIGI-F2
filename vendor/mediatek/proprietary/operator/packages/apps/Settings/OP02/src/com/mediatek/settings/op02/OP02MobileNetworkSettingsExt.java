...
public class OP02MobileNetworkSettingsExt extends
        DefaultMobileNetworkSettingsExt {

    private static final String TAG = "OP02MobileNetworkSettingsExt";
    ...
    public boolean customizeVolteState(int subId) {
        ...
        Log.d(TAG, "customizeVolteState, subid=" + subId + ", ret=" + ret);
        return ret;
    }
    ...
    public boolean customizeVolteStateReceiveIntent() {
        boolean ret = true;
        Log.d(TAG, "customizeVolteStateReceiveIntent, ret=" + ret);
        return ret;
    }

    private boolean isCuSim(int subId) {
        boolean cuSim = false;
        ...
        Log.d(TAG, "isCuSim, subId=" + subId + ", cuSim=" + cuSim);
        return cuSim;
    }
    ...
    private String getSimOperatorBySubId(int subId) {
        ...
        String simOperator = null;
        int simStatus = TelephonyManager.getDefault().getSimState(slotId);

        if (simStatus == TelephonyManager.SIM_STATE_READY) {
            simOperator = TelephonyManager.getDefault().getSimOperator(subId);
        }

        Log.d(TAG, "getSimOperatorBySubId, subId=" + subId
                + ", simStatus=" + simStatus
                + ", simOperator=" + (simOperator == null ? "null" : simOperator));
        return simOperator;
    }
}
