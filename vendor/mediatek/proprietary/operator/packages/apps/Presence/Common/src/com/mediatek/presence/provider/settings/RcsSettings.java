...
public class RcsSettings {
    ...
    */
    ...
    public Capabilities getMyCapabilities() {
        ...
        capabilities.setGeolocationPushviaSmsSupport(isGeoLocationPushViaSmsSupported());
    ...
    */
    ...
    public boolean isGeoLocationPushSupported() {
     ...
     /**
     * Is geolocation push via Sms supported
     *
     * @return Boolean
     */
    public boolean isGeoLocationPushViaSmsSupported() {
        boolean result = false;
        if (instance != null) {
            result = Boolean.parseBoolean(readParameter(RcsSettingsData.CAPABILITY_GEOLOCATION_PUSH_VIA_SMS));
        }
        return result;
    }
...
// The observer list
    ...
    */
    ...
    public List<String> getSupportedRcsFeatureTags() {
        List<String> result = new ArrayList<String>();
        if (isImSessionSupported()) {
            result.add(FeatureTags.FEATURE_RCSE_CPM_SESSION);
        }

        if (isFileTransferSupported()) {
            result.add(FeatureTags.FEATURE_RCSE_CPM_FT);
        }

        if (isFileTransferHttpSupported()) {
            result.add(FeatureTags.FEATURE_3GPP_FT_HTTP);
        }

        if (isGeoLocationPushSupported()) {
            result.add(FeatureTags.FEATURE_3GPP_GEOLOCATION_PUSH);
        }

        if (isGeoLocationPullSupported()) {
            result.add(FeatureTags.FEATURE_GEO_LOCATION_PULL);
        }

        if (isGeoLocationPullUsingFileTransferSupported()) {
            result.add(FeatureTags.FEATURE_GEO_LOCATION_PULL_USING_FILE_TRANSFER);
        }

        if (isStandaloneMsgSupport()) {
            result.add(FeatureTags.FEATURE_3GPP_STANDALONE_MSG);
        }

        if (isChatbotSupported()) {
            result.add(FeatureTags.FEATURE_CHATBOT);
        }

        if (isChatbotSaSupported()) {
            result.add(FeatureTags.FEATURE_CHATBOT_SA);
        }

        if (isCallcomposerSupported()) {
            result.add(FeatureTags.FEATURE_CALLCOMPOSER);
        }
