...
public class RcsUaAdapter {
    ...
    private int RCS_CAPABILITY_IP_GEO_LOCATION_PUSH_VIA_SMS_SUPPORT = 0x100;
    ...
    public String getPAccessNetworkInfo() {
        return mRegistrationInfo == null ?
                "" : mRegistrationInfo.getImsConfig().getPAccessNetworkInfo();
    }
    ...
    private void updateRcsCapabilities(int rcsFeatureTag) {
        ...
        if ((rcsFeatureTag & RCS_CAPABILITY_IP_GEO_LOCATION_PUSH_VIA_SMS_SUPPORT) != 0) {
            RcsSettingsManager.getRcsSettingsInstance(mSlotId).writeParameter(
                    RcsSettingsData.CAPABILITY_GEOLOCATION_PUSH_VIA_SMS, "true");
        } else {
            RcsSettingsManager.getRcsSettingsInstance(mSlotId).writeParameter(
                    RcsSettingsData.CAPABILITY_GEOLOCATION_PUSH_VIA_SMS, "false");
        }
    ...
    public void triggerImsReRegistration() {
        if (mRcsUaService == null) {
            logger.error("triggerImsReRegistration fail. mRcsUaService is null !");
            return;
        }
        mRcsUaService.triggerReregistration();
    }
}
