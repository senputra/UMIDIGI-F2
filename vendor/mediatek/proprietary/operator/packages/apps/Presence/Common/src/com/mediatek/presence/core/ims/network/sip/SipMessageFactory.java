    ...
    public static SipRequest createSubscribe(int slotId, SipDialogPath dialog, int expirePeriod)
        ...
        try {
            ...
            if (SimUtils.isTmoSimCard(slotId)) {
                SipUtils.setContactFeatureTags(subscribe,
                        RcsSettingsManager.getRcsSettingsInstance(slotId).getSupportedRcsFeatureTags());
                ContactHeader contactHeader = (ContactHeader)subscribe.getHeader(ContactHeader.NAME);
                Context context = AndroidFactory.getApplicationContext();
                String sipInstance = DeviceUtils.getInstanceId(slotId, context);
                if (sipInstance != null) {
                    contactHeader.setParameter(SipUtils.SIP_INSTANCE_PARAM, sipInstance);
                }
            }
...
/**
    ...
    public static SipRequest createPresenceSubscribe(int slotId, SipDialogPath dialog,
        ...
        try {
         ...
         // Set Contact header
            ...
            if (SimUtils.isTmoSimCard(slotId)) {
                logger.debug("createPresenceSubscribe - Add Feature tags for TMO only");
                SipUtils.setContactFeatureTags(subscribe,
                        RcsSettingsManager.getRcsSettingsInstance(slotId).getSupportedRcsFeatureTags());
                ContactHeader contactHeader = (ContactHeader)subscribe.getHeader(ContactHeader.NAME);
                Context context = AndroidFactory.getApplicationContext();
                ...
                logger.debug("createPresenceSubscribe - add normal Feature tags");
    ...
    public static SipRequest createPublish(int slotId, SipDialogPath dialog, int expirePeriod,
        ...
        try {
         ...
         // Update Contact header
            ...
            if (SimUtils.isTmoSimCard(slotId)) {
                SipUtils.setContactFeatureTags(publish,
                        RcsSettingsManager.getRcsSettingsInstance(slotId).getSupportedRcsFeatureTags());
                ContactHeader contactHeader = (ContactHeader)publish.getHeader(ContactHeader.NAME);
                Context context = AndroidFactory.getApplicationContext();
                String sipInstance = DeviceUtils.getInstanceId(slotId, context);
            ...
            if (SimUtils.isAttSimCard(slotId) || SimUtils.isVzwSimCard(slotId)) {
                String PANI = RcsUaAdapterManager.getRcsUaAdapter(slotId).getPAccessNetworkInfo();
                if (logger.isActivated()) {
                    logger.debug("P-Access-Network-Info:" + PANI);
                }
                if (PANI != null && PANI != "") {
    ...
    public static SipRequest createAck(int slotId, SipDialogPath dialog) throws SipException {
        try {
            ...
            if (SimUtils.isTmoSimCard(slotId)) {
                ContactHeader contactHeader = (ContactHeader)ack.getHeader(ContactHeader.NAME);
                Context context = AndroidFactory.getApplicationContext();
                String sipInstance = DeviceUtils.getInstanceId(slotId, context);
                if (sipInstance != null) {
                    contactHeader.setParameter(SipUtils.SIP_INSTANCE_PARAM, sipInstance);
     ...
     * @throws SipException
     */
    public static SipResponse createResponse(int slotId, ContactHeader contactHeader, SipRequest request, int code) throws SipException {
        try {
            // Create the response
     ...
     * @return SIP response
     */
    public static SipResponse createResponse(int slotId, ContactHeader contactHeader, SipRequest request, String localTag, int code)
            throws SipException {
        try {
            // Create the response
            Response response = SipUtils.MSG_FACTORY.createResponse(
