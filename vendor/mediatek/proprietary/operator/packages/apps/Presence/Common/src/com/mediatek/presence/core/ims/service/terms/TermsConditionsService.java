...
package com.mediatek.presence.core.ims.service.terms;
...
public class TermsConditionsService extends ImsService {
    ...
    private static String id_term=""; 
    private static String pin_term="";
    ...
    public TermsConditionsService(ImsModule parent) throws CoreException {
        ...
        remoteServer = RcsSettingsManager.getRcsSettingsInstance(mSlotId)
                .getEndUserConfirmationRequestUri();
    }
    ...
    public void receiveMessage(SipRequest message) {
        ...
        try {
            ...
            int slotId = getImsModule().getSlotId();
            ContactHeader contactHeader = getImsModule().getSipManager().getSipStack().getContact();
            SipResponse response = SipMessageFactory.createResponse(slotId, contactHeader, message,
        ...
        try {
            ...
            if (message.getContentType().equals(REQUEST_MIME_TYPE)) {
                ...
                TermsRequestParser parser = new TermsRequestParser(mSlotId, input, lang);
    ...
    public boolean acceptTerms(String id, String pin) {
        ...
        AsyncTask.execute(new Runnable(){
            public void run() {
                result= sendSipMessage(remoteServer, id_term, ACCEPT_RESPONSE, pin_term);                        
                }
            });
        return result;
    }
    ...
    public boolean rejectTerms(String id, String pin) {
        ...
        AsyncTask.execute(new Runnable(){
            public void run() {
                result= sendSipMessage(remoteServer, id_term, DECLINE_RESPONSE, pin_term);                        
                }
            });
        return result;
    }
    ...
    private boolean sendSipMessage(String remote, String id, String value, String pin) {
        ...
        try {
            ...
            if(!RcsSettingsManager.getRcsSettingsInstance(mSlotId)
                    .isCPMSupported()){
                if (logger.isActivated()) {
                    logger.info("sendSipMessage no CPM");
                }
                msg = SipMessageFactory.createMessage(mSlotId, dialogPath, RESPONSE_MIME_TYPE, response);
            }
            else{
                if (logger.isActivated()) {
                    logger.info("TermsConditions sendSipMessage CPMS");
                }
                msg = SipMessageFactory.createCpimMessage(mSlotId, dialogPath, RESPONSE_MIME_TYPE, response);
            }
            ...
            if (ctx.getStatusCode() == 407) {
                ...
                if(!RcsSettingsManager.getRcsSettingsInstance(mSlotId).isCPMSupported()){
                    ...
                    if (logger.isActivated()) {
                        logger.info("TermsConditions sendSipMessage CPMS1");
                    }
                    msg = SipMessageFactory.createCpimMessage(mSlotId, dialogPath, RESPONSE_MIME_TYPE, response);
                }
