...
public abstract class ImsServiceSession extends Thread {
    ...
    public void receiveCancel(SipRequest cancel) {
        ...
        try {
            ...
            ContactHeader contactHeader = getImsService().getImsModule().getSipManager().getSipStack().getContact();
            SipResponse terminatedResp = SipMessageFactory.createResponse(mSlotId, contactHeader, getDialogPath().getInvite(),
    ...
    public void send180Ringing(SipRequest request, String localTag) {
        try {
            ...
            if(!SimUtils.isCmccSimCard(mSlotId)) {
                ContactHeader contactHeader = getImsService().getImsModule().getSipManager().getSipStack().getContact();
                progress = SipMessageFactory.createResponse(mSlotId, contactHeader, request, localTag, 180);
            } else {
    ...
    public void sendErrorResponse(SipRequest request, String localTag, int code) {
        try {
            ...
            ContactHeader contactHeader = getImsService().getImsModule().getSipManager().getSipStack().getContact();
            SipResponse resp = SipMessageFactory.createResponse(mSlotId, contactHeader, request, localTag, code);
    ...
    public void send603Decline(SipRequest request, String localTag) {
        try {
            ...
            ContactHeader contactHeader = getImsService().getImsModule().getSipManager().getSipStack().getContact();
            SipResponse resp = SipMessageFactory.createResponse(mSlotId, contactHeader, request, localTag, 603);
            getImsService().getImsModule().getSipManager().sendSipResponse(resp);
        } catch(Exception e) {
            if (logger.isActivated()) {
    ...
    public void send486Busy(SipRequest request, String localTag) {
        try {
            ...
            ContactHeader contactHeader = getImsService().getImsModule().getSipManager().getSipStack().getContact();
            SipResponse resp = SipMessageFactory.createResponse(mSlotId, contactHeader, request, localTag, 486);
            getImsService().getImsModule().getSipManager().sendSipResponse(resp);
        } catch(Exception e) {
            if (logger.isActivated()) {
    ...
    public void send415Error(SipRequest request) {
        try {
            ...
            ContactHeader contactHeader = getImsService().getImsModule().getSipManager().getSipStack().getContact();
            SipResponse resp = SipMessageFactory.createResponse(mSlotId, contactHeader, request, 415);
