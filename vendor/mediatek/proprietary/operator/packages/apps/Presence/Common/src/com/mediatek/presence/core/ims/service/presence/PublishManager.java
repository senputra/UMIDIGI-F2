...
public class PublishManager extends PeriodicRefresher
    ...
    */
           ...
           switch(mRetryCounter)
            {
                ...
                break;
                case 1:
                    logger.info("400 response received, start retry in 20 sec");
                    startTimer(20, 1);
                break;
                case 2:
                    logger.info("400 response received, start retry in 30 sec");
                    startTimer(30, 1);
                break;
                case 3:
                    logger.info("400 response received, start retry in 40 sec");
                    startTimer(40, 1);
                break;
                case 4:
    ...
    private void handleTmoErrorRetry(SipTransactionContext ctx) throws Exception {
        if (!SimUtils.isTmoSimCard(mSlotId)) {
            return;
        }

        if (logger.isActivated()) {
            logger.info("handleTmoErrorRetry and error code is: " + ctx.getStatusCode());
        }
    ...
    private void handleTmo403ErrorImsReregistration(SipTransactionContext ctx) {
        if (!SimUtils.isTmoSimCard(mSlotId)) {
            return;
        }

        if (logger.isActivated()) {
            logger.info("handleTmo403ErrorImsReregistration");
        }
        RcsUaAdapterManager.getRcsUaAdapter(mSlotId).triggerImsReRegistration();
