...
public class ImsServiceDispatcher implements Runnable {
    ...
    private void dispatch(SipRequest request) {
        ...
        if (request.getMethod().equals(Request.OPTIONS)) {
            ...
            try {
                ...
                ContactHeader contactHeader = imsModule.getSipManager().getSipStack().getContact();
                SipResponse cancelResp = SipMessageFactory.createResponse(slotId, contactHeader, newRequest, 200);
    ...
    private void dispatchNotify(SipRequest notify) {
        try {
            ...
            int slotId = imsModule.getSlotId();
            ContactHeader contactHeader = imsModule.getSipManager().getSipStack().getContact();
            SipResponse resp = SipMessageFactory.createResponse(slotId, contactHeader, notify, 200);
    ...
    private void send100Trying(SipRequest request) {
        try {
            ...
            SipResponse trying = SipMessageFactory.createResponse(slotId, contactHeader, request, null, 100);
    ...
    private void sendFinalResponse(SipRequest request, int code) {
        try {
            int slotId = imsModule.getSlotId();
            ContactHeader contactHeader = imsModule.getSipManager().getSipStack().getContact();
            SipResponse resp = SipMessageFactory.createResponse(slotId, contactHeader, request, IdGenerator.getIdentifier(), code);
