...
public class AnonymousFetchManager implements DiscoveryManager {
    ...
    */
    ...
    private HashMap<String, HashSet<String>> tmoCallIdContactNumberMappingTable =
    ...
    public boolean requestCapabilities(String contact) {
        ...
        if (canSendRequestforContact(contact)) {
            ...
            task.start();
            String callId = task.getTmoSubscribeCallId();
            HashSet<String> contactNumbers = new HashSet<String>();
            contactNumbers.add(contact);
            tmoCallIdContactNumberMappingTable.put(callId, contactNumbers);
    ...
    public boolean requestCapabilities(List<String> contacts) {
        ...
        while(contactsSize >0) {
            ...
            String callId = task.getTmoSubscribeCallId();
            HashSet<String> contactNumbers = new HashSet<String>();
            for (String numbers : subContactList) {
                contactNumbers.add(numbers);
            }
            tmoCallIdContactNumberMappingTable.put(callId, contactNumbers);
    ...
    public void receiveNotification(SipRequest notify) {
        ...
        HashSet<String> validContactNumbers = new HashSet<String>();
                    ...
                    if (rlmiPartArray != null) {
                        try {
                            ...
                            for(int i=0; i < list.size(); i++) {
                                ...
                                if (contact != null) {
                                    validContactNumbers.add(contact);
                                }
                    ...
                    try {
                        ...
                        if(pidfPartArray != null) {
                            for (String pidfPart : pidfPartArray) {
                                ...
                                if (presenceInfo != null && presenceInfo.getEntity() != null) {
                                    validContactNumbers.add(presenceInfo.getEntity());
                                }
        ...
        String callId = notify.getCallId();
        if (SimUtils.isTmoSimCard(mSlotId) && callId != null) {
            HashSet<String> subscribeContactNumbers = tmoCallIdContactNumberMappingTable.get(callId);
            for (String contact : subscribeContactNumbers) {
                if (!validContactNumbers.contains(contact)) {
                    ContactsManager.getInstance().setContactCapabilities(
                            contact, new Capabilities(), ContactInfo.NOT_RCS, ContactInfo.REGISTRATION_STATUS_OFFLINE);
                }
            }
        }
    }
        ...
        if(presence!=null) {
            ...
            if(tuples.size()>0) {
                ...
                for(int i=0; i < tuples.size(); i++) {
                    ...
                    }else if (id.equalsIgnoreCase(PresenceUtils.FEATURE_GEOLOCATION_PUSH)) {
                        if (logger.isActivated()) {
                            logger.debug("Geolocation push Supported: "+contact+ " : "+state);
                        }
                        capabilities.setGeolocationPushSupport(state);
                    } else if (id.equalsIgnoreCase(PresenceUtils.FEATURE_GEOLOCATION_PUSH_VIA_SMS)){
                        if (logger.isActivated()) {
                            logger.debug("gelocation push via sms Supported: "+contact+ " : "+state);
                        }
                        capabilities.setGeolocationPushviaSmsSupport(state);
                    }
                }
            } else {
                if (logger.isActivated()) {
