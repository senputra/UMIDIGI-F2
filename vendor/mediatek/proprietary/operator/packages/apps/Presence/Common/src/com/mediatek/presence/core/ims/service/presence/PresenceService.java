...
protected String buildCapabilities(String timestamp, Capabilities capabilities) {
    ...
    String Geopush = "Geopush";
    String GeoPushviaSms = "GeoPushviaSms";
    String publishXML = "";
  ...
  //Standalone Mode Messaging
    ...
    if(capabilities.isGeolocationPushSupported()){
      publishXML+="<tuple id=\""+Geopush+"\">" + SipUtils.CRLF +
            "  <status><basic>" + buildBooleanStatus(capabilities.isGeolocationPushSupported()) + "</basic></status>" + SipUtils.CRLF +
            "  <op:service-description>" + SipUtils.CRLF +
            "    <op:service-id>" + PresenceUtils.FEATURE_GEOLOCATION_PUSH + "</op:service-id>" + SipUtils.CRLF +
            "    <op:version>1.0</op:version>" + SipUtils.CRLF +
            "  </op:service-description>" + SipUtils.CRLF +
            "  <contact>" + ImsModule.IMS_USER_PROFILE.getPublicUri() + "</contact>" + SipUtils.CRLF +
            "  <timestamp>" + timestamp + "</timestamp>" + SipUtils.CRLF +
    ...
    if(capabilities.isGeolocationPushviaSmsSupported()){
        publishXML+="<tuple id=\""+GeoPushviaSms+"\">" + SipUtils.CRLF +
            "  <status><basic>" + buildBooleanStatus(capabilities.isGeolocationPushviaSmsSupported()) + "</basic></status>" + SipUtils.CRLF +
            "  <op:service-description>" + SipUtils.CRLF +
            "    <op:service-id>" + PresenceUtils.FEATURE_GEOLOCATION_PUSH_VIA_SMS+ "</op:service-id>" + SipUtils.CRLF +
            "    <op:version>1.0</op:version>" + SipUtils.CRLF +
            "  </op:service-description>" + SipUtils.CRLF +
            "  <contact>" + ImsModule.IMS_USER_PROFILE.getPublicUri() + "</contact>" + SipUtils.CRLF +
            "  <timestamp>" + timestamp + "</timestamp>" + SipUtils.CRLF +
 ...
 " xmlns:ci=\"urn:ietf:params:xml:ns:pidf:cipid\""
    ...
    public void updateRcsCapabilities(boolean isModify, Capabilities caps) {
	    ...
	    if(myCaps.isGeolocationPushviaSmsSupported() != caps.isGeolocationPushviaSmsSupported()) {
                needPublish = true;
                RcsSettingsManager.getRcsSettingsInstance(mSlotId).writeParameter(RcsSettingsData.CAPABILITY_GEOLOCATION_PUSH_VIA_SMS,
                    Boolean.toString(caps.isGeolocationPushviaSmsSupported()));
            }
