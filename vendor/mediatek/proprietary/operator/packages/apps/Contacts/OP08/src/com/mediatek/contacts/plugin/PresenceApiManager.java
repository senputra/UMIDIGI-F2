...
public class PresenceApiManager {
     ...
     *            the query mode
     */
    public void requestContactPresenceDelay(final String number, final boolean force) {
        if (isServiceConnected()) {
            requestContactPresence(number, force);
        } else {
            MessageData data = new MessageData(number, force);
            Message msg = mHandler.obtainMessage(MESSAGE_DELAY, data);
            LogUtils.d(TAG, "requestContactPresenceDelay, number is "
                    + LogUtils.anonymize(number));
